...
if [[ ${CI_TAGS} == *'osx-i386'* ]]; then
  ...
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-libgdiplus=/Library/Frameworks/Mono.framework/Versions/Current/lib/libgdiplus.dylib --host=i386-apple-darwin13.0.0 --build=i386-apple-darwin13.0.0"
fi
if [[ ${CI_TAGS} == *'osx-amd64'* ]]; then
  EXTRA_CFLAGS="$EXTRA_CFLAGS -m64 -arch x86_64 -mmacosx-version-min=10.9"
  EXTRA_LDFLAGS="$EXTRA_LDFLAGS -m64 -arch x86_64" EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-libgdiplus=/Library/Frameworks/Mono.framework/Versions/Current/lib/libgdiplus.dylib"
fi
if [[ ${CI_TAGS} == *'win-i386'* ]]; then
  PLATFORM=Win32
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --host=i686-w64-mingw32 --enable-btls"
  export MONO_EXECUTABLE="$MONO_REPO_ROOT/msvc/build/sgen/Win32/bin/Release/mono-sgen.exe"
fi
if [[ ${CI_TAGS} == *'win-amd64'* && ${CI_TAGS} != *'sdks-android'* ]]; then
  PLATFORM=x64
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --host=x86_64-w64-mingw32 --disable-boehm --enable-btls"
  export MONO_EXECUTABLE="$MONO_REPO_ROOT/msvc/build/sgen/x64/bin/Release/mono-sgen.exe"
fi
if [[ ${CI_TAGS} == *'freebsd-amd64'* ]]; then
  export CC="clang"
  export CXX="clang++"
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --disable-dtrace --disable-boehm ac_cv_header_sys_inotify_h=no ac_cv_func_inotify_init=no ac_cv_func_inotify_add_watch=no ac_cv_func_inotify_rm_watch=no"
fi
if [[ ${CI_TAGS} == *'make-install'* ]]; then EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --enable-llvm --prefix=$MONO_REPO_ROOT/tmp/monoprefix"; fi

if [[ ${CI_TAGS} == *'coop-suspend'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --disable-hybrid-suspend --enable-cooperative-suspend"
elif [[ ${CI_TAGS} == *'hybrid-suspend'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --enable-hybrid-suspend"
elif [[ ${CI_TAGS} == *'preemptive-suspend'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --disable-hybrid-suspend"
fi
...
if [[ ${CI_TAGS} == *'fullaot_llvm'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --enable-llvm=yes --with-runtime-preset=fullaot_llvm "
elif [[ ${CI_TAGS} == *'hybridaot_llvm'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --enable-llvm=yes --with-runtime-preset=hybridaot_llvm"
elif [[ ${CI_TAGS} == *'winaot_llvm'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --enable-llvm=yes --with-runtime-preset=winaot_llvm"
elif [[ ${CI_TAGS} == *'aot_llvm'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --enable-llvm=yes --with-runtime-preset=aot_llvm "
elif [[ ${CI_TAGS} == *'jit_llvm'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --enable-llvm=yes"
  export MONO_ENV_OPTIONS="$MONO_ENV_OPTIONS --llvm"
elif [[ ${CI_TAGS} == *'fullaotinterp_llvm'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --enable-llvm=yes --with-runtime-preset=fullaotinterp_llvm"
elif [[ ${CI_TAGS} == *'fullaotinterp'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-runtime-preset=fullaotinterp"
elif [[ ${CI_TAGS} == *'winaotinterp'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-runtime-preset=winaotinterp"
elif [[ ${CI_TAGS} == *'winaotinterp_llvm'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --enable-llvm=yes --with-runtime-preset=winaotinterp_llvm"
elif [[ ${CI_TAGS} == *'fullaot'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-runtime-preset=fullaot"
elif [[ ${CI_TAGS} == *'hybridaot'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-runtime-preset=hybridaot"
elif [[ ${CI_TAGS} == *'winaot'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-runtime-preset=winaot"
elif [[ ${CI_TAGS} == *'aot'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-runtime-preset=aot"
elif [[ ${CI_TAGS} == *'bitcodeinterp'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-runtime-preset=bitcodeinterp"
  export PATH="$PATH:$MONO_REPO_ROOT/llvm/usr/bin"
elif [[ ${CI_TAGS} == *'bitcode'* ]]; then
  EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-runtime-preset=bitcode"
  export PATH="$PATH:$MONO_REPO_ROOT/llvm/usr/bin"
elif [[ ${CI_TAGS} == *'acceptance-tests'* ]]; then
...
if [[ ${CI_TAGS} != *'mac-sdk'* ]]; then # Mac SDK builds Mono itself
...
if [[ ${CI_TAGS} == *'acceptance-tests'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-acceptance-tests.sh
elif [[ ${CI_TAGS} == *'microbench'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-microbench.sh
elif [[ ${CI_TAGS} == *'profiler-stress-tests'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-profiler-stress-tests.sh
elif [[ ${CI_TAGS} == *'stress-tests'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-stress-tests.sh
elif [[ ${CI_TAGS} == *'interpreter'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-interpreter.sh
elif [[ ${CI_TAGS} == *'mcs-compiler'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-mcs.sh
elif [[ ${CI_TAGS} == *'mac-sdk'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-mac-sdk.sh
elif [[ ${CI_TAGS} == *'helix-tests'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-helix.sh
elif [[ ${CI_TAGS} == *'compile-msbuild-source'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-msbuild.sh
elif [[ ${CI_TAGS} == *'make-install'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-make-install.sh
elif [[ ${CI_TAGS} == *'compiler-server-tests'* ]]; then
  "$MONO_REPO_ROOT"/scripts/ci/run-test-compiler-server.sh
elif [[ ${CI_TAGS} == *'no-tests'* ]]; then
  echo "Skipping tests."
else
  make check-ci
fi
...
if [[ $CI_TAGS == *'apidiff'* ]]; then
  if "$TESTCMD" --label=apidiff --timeout=15m --fatal make -w -C mcs -j "$CI_CPU_COUNT" mono-api-diff; then
    report_github_status "success" "API Diff" "No public API changes found." || true
  else
    report_github_status "error" "API Diff" "The public API changed." "$BUILD_URL/Public_20API_20Diff/" || true
  fi
fi
if [[ $CI_TAGS == *'csprojdiff'* ]]; then
  make update-solution-files
  if "$TESTCMD" --label=csprojdiff --timeout=5m --fatal make -w -C mcs mono-csproj-diff; then
    report_github_status "success" "Project Files Diff" "No csproj file changes found." || true
  else
    report_github_status "error" "Project Files Diff" "The csproj files changed." "$BUILD_URL/Project_20Files_20Diff/" || true
  fi
fi
