...
void *Mono_Posix_Stdlib_SIG_DFL(void) { return SIG_DFL; }

void *Mono_Posix_Stdlib_SIG_ERR(void) { return SIG_ERR; }

void *Mono_Posix_Stdlib_SIG_IGN(void) { return SIG_IGN; }
...
// unless WAPI_NO_ATOMIC_ASM is defined, in which case atomic.h calls linked
// functions.
...
static int keep_trying(int r) { return r == -1 && errno == EINTR; }
...
// - If default_handler starts while a teardown_pipes is ongoing, it must *not*
// block Locks are implemented as ints.
...
#define PIPELOCK_INCR_COUNT(x, by)                                             \
  (((x)&PIPELOCK_TEARDOWN_BIT) |                                               \
   (PIPELOCK_GET_COUNT(PIPELOCK_GET_COUNT(x) + (by))))

static void acquire_pipelock_teardown(int *lock) {
  int lockvalue_draining;
  // First mark that a teardown is occurring, so handlers will stop entering the
  // lock.
  while (1) {
...
static void release_pipelock_teardown(int *lock) {
  while (1) {
    int lockvalue = mph_int_get(lock);
    int lockvalue_new = lockvalue & ~PIPELOCK_TEARDOWN_BIT;
    // Technically this can't fail, because we hold both the pipelock and the
    // mutex, but
...
static int acquire_pipelock_handler(int *lock) {
  while (1) {
    int lockvalue = mph_int_get(lock);
...
static void release_pipelock_handler(int *lock) {
  while (1) {
    int lockvalue = mph_int_get(lock);
    int lockvalue_new = PIPELOCK_INCR_COUNT(lockvalue, -1);
    if (mph_int_test_and_set(lock, lockvalue, lockvalue_new))
...
// This handler is registered once for each UnixSignal object. A pipe is
// maintained for each one; Wait users read at one end of this pipe, and
// default_handler sends a write on the pipe for each signal received while the
// Wait is ongoing.
//
// Notice a fairly unlikely race condition exists here: Because we synchronize
// with pipe teardown, but not install/uninstall (in other words, we are only
// trying to protect against writing on a closed pipe) it is technically
// possible a full uninstall and then an install could complete after signum is
// checked but before the remaining instructions execute. In this unlikely case
// count could be incremented or a byte written on the wrong signal handler.
static void default_handler(int signum) {
  ...
  for (i = 0; i < NUM_SIGNALS; ++i) {
    ...
    if (fd > 0) { // If any listener exists to write to
      ...
      pipecounter =
          mph_int_get(&h->pipecnt); // Write one byte per pipe listener
...
void *Mono_Unix_UnixSignal_install(int sig) {
...
#if defined(SIGRTMIN) && defined(SIGRTMAX)
  /*The runtime uses some rt signals for itself so it's important to not
   * override them.*/
  if (sig >= SIGRTMIN && sig <= SIGRTMAX && count_handlers(sig) == 0) {
    struct sigaction sinfo;
    sigaction(sig, NULL, &sinfo);
    if (sinfo.sa_handler != SIG_DFL ||
        (void *)sinfo.sa_sigaction != (void *)SIG_DFL) {
  ...
  // Scan through signals list looking for (1) an unused spot (2) a usable value
  // for handler
  for (i = 0; i < NUM_SIGNALS; ++i) {
    ...
    // Check if this slot has a "usable" (not installed by this file)
    // handler-to-restore-later: (On the first signal to be installed, signals
    // [i] will be == h when this happens.)
    if (!have_handler &&
        (just_installed || mph_int_get(&signals[i].signum) == sig) &&
  ...
  if (h) {
    ...
    // was set as the signal handler before the first UnixSignal object was
    // installed.
    g_assert(have_handler);

    // Overwrite the tenative handler we set a moment ago with a known-usable
    // one
    h->handler = handler;
    h->have_handler = 1;
...
static int setup_pipes(signal_info **signals, int count,
                       struct pollfd *fd_structs, int *currfd) {
  int i;
  int r = 0;
  for (i = 0; i < count; ++i) {
    signal_info *h;
...
static void teardown_pipes(signal_info **signals, int count) {
  int i;
  for (i = 0; i < count; ++i) {
    signal_info *h = signals[i];
...
static int wait_for_any(signal_info **signals, int count, int *currfd,
                        struct pollfd *fd_structs, int timeout,
 ...
 */
int Mono_Unix_UnixSignal_WaitAny(
    void **_signals, int count, int timeout /* milliseconds */,
    Mono_Posix_RuntimeIsShuttingDown shutting_down) {
  int r;
  int currfd = 0;
