...
typedef void (*MonoLogCallback)(const char *log_domain, const char *log_level,
                                const char *message, int32_t fatal,
                                void *user_data);
...
typedef void *(*MonoDlFallbackLoad)(const char *name, int flags, char **err,
                                    void *user_data);
typedef void *(*MonoDlFallbackSymbol)(void *handle, const char *name,
                                      char **err, void *user_data);
typedef void *(*MonoDlFallbackClose)(void *handle, void *user_data);

typedef void *(*mono_dl_fallback_register_fn)(MonoDlFallbackLoad load_func,
                                              MonoDlFallbackSymbol symbol_func,
                                              MonoDlFallbackClose close_func,
                                              void *user_data);

typedef MonoDomain *(*mono_jit_init_version_fn)(const char *root_domain_name,
                                                const char *runtime_version);
typedef MonoAssembly *(*mono_assembly_open_fn)(const char *filename,
...
typedef MonoClass *(*mono_class_from_name_fn)(MonoImage *image,
                                              const char *name_space,
                                              const char *name);
typedef MonoMethod *(*mono_class_get_method_from_name_fn)(MonoClass *klass,
                                                          const char *name,
                                                          int param_count);
typedef MonoObject *(*mono_runtime_invoke_fn)(MonoMethod *method, void *obj,
...
typedef void (*mono_domain_set_config_fn)(MonoDomain *, const char *,
                                          const char *);
typedef int (*mono_runtime_set_main_args_fn)(int argc, char *argv[]);
typedef void (*mono_trace_init_fn)(void);
typedef void (*mono_trace_set_log_handler_fn)(MonoLogCallback callback,
                                              void *user_data);
...
typedef MonoArray *(*mono_array_new_fn)(MonoDomain *domain, MonoClass *eclass,
                                        uintptr_t n);
typedef MonoClass *(*mono_get_string_class_fn)(void);
typedef void (*mono_dllmap_insert_fn)(MonoImage *assembly, const char *dll,
...
static void *my_dlsym(void *handle, const char *name, char **err,
                      void *user_data);
static void *my_dlopen(const char *name, int flags, char **err,
                       void *user_data);
...
static void _runtime_log(const char *log_domain, const char *log_level,
                         const char *message, int32_t fatal, void *user_data) {
  JNIEnv *env;
  jstring j_message;
  ...
  if (AndroidRunner_klass == NULL)
    __android_log_assert("", "mono-sdks", "%s: AndroidRunner_klass is NULL",
                         __func__);
  if (AndroidRunner_WriteLineToInstrumentation_method == NULL)
    __android_log_assert(
        "", "mono-sdks",
        "%s: AndroidRunner_WriteLineToInstrumentation_method is NULL",
        __func__);
  ...
  (*env)->CallStaticVoidMethod(env, AndroidRunner_klass,
                               AndroidRunner_WriteLineToInstrumentation_method,
                               j_message);
...
static void _log(const char *format, ...) {
  ...
  if (buf == NULL || nbuf == -1)
    __android_log_assert(
        "", "mono-sdks",
        "%s: vasprintf failed, error: \"%s\" (%d), nbuf = %d, buf = \"%s\"",
        __func__, strerror(errno), errno, nbuf, buf ? buf : "(null)");
...
static void strncpy_str(JNIEnv *env, char *buff, jstring str, int nbuff) {
  jboolean isCopy = 0;
...
static int m_make_directory(const char *path, int mode) {
#if WINDOWS
  return mkdir(path);
#else
...
static int m_create_directory(const char *pathname, int mode) {
  if (mode <= 0)
...
void monodroid_clear_lldb_wait(void) { wait_for_lldb = 0; }

static void wait_for_unmanaged_debugger() {
  while (wait_for_lldb) {
    _log("Waiting for lldb to attach, run \"make -C sdks/android attach-lldb\" "
         "in another terminal");
    sleep(5);
  }
}

static void create_and_set(const char *home, const char *relativePath,
                           const char *envvar) {
...
void Java_org_mono_android_AndroidRunner_runTests(
    JNIEnv *env, jobject thiz, jstring j_files_dir, jstring j_cache_dir,
    jstring j_native_library_dir, jstring j_assembly_dir,
    jstring j_assembly_name, jboolean is_debugger, jboolean is_profiler,
    jboolean wait_for_lldb) {
  MonoDomain *root_domain;
  MonoMethod *run_tests_method;
  void **params;
  char **argv;
  char buff[1024], file_dir[2048], cache_dir[2048], native_library_dir[2048],
      assembly_dir[2048], assembly_name[2048];
  int argc;
  ...
  strncpy_str(env, native_library_dir, j_native_library_dir,
              sizeof(native_library_dir));
...
#undef DLSYM
  ...
  if (is_debugger) {
    char *debug_options[] = {"--debugger-agent=transport=dt_socket,loglevel=10,"
                             "address=127.0.0.1:6100,embedding=1",
                             "--soft-breakpoints"};
  ...
  // Note: sets up domains. If the debugger is configured after this line is
  // run, then lookup_data_table will fail.
  ...
  if (is_debugger) {
    ...
    MonoClass *tests_class = mono_class_from_name(
        mono_assembly_get_image(main_assembly), "", "Tests");
    if (!tests_class) {
    ...
    MonoClass *driver_class =
        mono_class_from_name(mono_assembly_get_image(main_assembly), "Xamarin",
                             "AndroidTestAssemblyRunner/Driver");
    if (!driver_class) {
    ...
    run_tests_method =
        mono_class_get_method_from_name(driver_class, "RunTests", 1);
    if (!run_tests_method) {
...
# Issues with the monodroid BCL profile
        This pinvoke should not be on __Internal by libmonodroid.so:
System.TimeZoneInfo+AndroidTimeZones:monodroid_get_system_property This depends
on monodroid native code:
System.TimeZoneInfo+AndroidTimeZones.GetDefaultTimeZoneName
*/
...
MONO_API int monodroid_get_system_property(const char *name, char **value) {
  char *pvalue;
  char sp_value[PROP_VALUE_MAX + 1] = {
      0,
  };
  int len;
...
MONO_API void monodroid_free(void *ptr) { free(ptr); }
...
MONO_API int _monodroid_get_android_api_level(void) { return 24; }

MONO_API int _monodroid_get_network_interface_up_state(void *ifname,
                                                       int *is_up) {
  *is_up = 1;
  return 1;
}
...
MONO_API int
_monodroid_get_network_interface_supports_multicast(void *ifname,
...
static void mono_jvm_initialize(JavaVM *vm) {
  ...
  if (!env)
    __android_log_assert("", "mono-sdks",
                         "%s: fatal error: Could not create env, res = %d",
                         __func__, res);

  AndroidRunner_klass = lref_to_gref(
      env, (*env)->FindClass(env, "org/mono/android/AndroidRunner"));
  if (!AndroidRunner_klass)
    __android_log_assert("", "mono-sdks",
                         "%s: fatal error: Could not find AndroidRunner_klass",
                         __func__);

  AndroidRunner_WriteLineToInstrumentation_method = (*env)->GetStaticMethodID(
      env, AndroidRunner_klass, "WriteLineToInstrumentation",
      "(Ljava/lang/String;)V");
  if (!AndroidRunner_WriteLineToInstrumentation_method)
    __android_log_assert("", "mono-sdks",
                         "%s: fatal error: Could not find "
                         "AndroidRunner_WriteLineToInstrumentation_method",
                         __func__);
...
static JNIEnv *mono_jvm_get_jnienv(void) {
  ...
  if (!initialized)
    __android_log_assert("", "mono-sdks",
                         "%s: Fatal error: jvm not initialized", __func__);
  ...
  __android_log_assert("", "mono-sdks", "%s: Fatal error: Could not create env",
                       __func__);
}
