...
static unsigned char *load_aot_data(MonoAssembly *assembly, int size,
...
static void free_aot_data(MonoAssembly *assembly, int size, void *user_data,
                          void *handle) {
  munmap(handle, size);
}

static MonoAssembly *load_assembly(const char *name, const char *culture) {
  const char *bundle = get_bundle_path();
  char filename[1024];
  char path[1024];
  int res;
  ...
  int has_extension =
      len > 3 && name[len - 4] == '.' &&
      (!strcmp("exe", name + (len - 3)) || !strcmp("dll", name + (len - 3)));
  ...
  if (culture && strcmp(culture, ""))
    res =
        snprintf(path, sizeof(path) - 1, "%s/%s/%s", bundle, culture, filename);
  else
...
static MonoAssembly *assembly_preload_hook(MonoAssemblyName *aname,
...
char *strdup_printf(const char *msg, ...) {
  va_list args;
  char *formatted = NULL;
...
static MonoObject *fetch_exception_property(MonoObject *obj, const char *name,
                                            bool is_virtual) {
  MonoMethod *get = NULL;
  ...
  if (get) {
    ...
    os_log_error(OS_LOG_DEFAULT,
                 "Could not find the property System.Exception.%{public}s.",
                 name);
  }
...
static char *fetch_exception_property_string(MonoObject *obj, const char *name,
                                             bool is_virtual) {
  MonoString *str =
      (MonoString *)fetch_exception_property(obj, name, is_virtual);
...
void unhandled_exception_handler(MonoObject *exc, void *user_data) {
  ...
  char *type_name = strdup_printf("%s.%s", mono_class_get_namespace(type),
                                  mono_class_get_name(type));
...
void log_callback(const char *log_domain, const char *log_level,
...
static void register_dllmap(void) {
  mono_dllmap_insert(NULL, "System.Native", NULL, "__Internal", NULL);
  mono_dllmap_insert(NULL, "System.IO.Compression.Native", NULL, "__Internal",
                     NULL);
  mono_dllmap_insert(NULL, "System.Security.Cryptography.Native.Apple", NULL,
                     "__Internal", NULL);
}
...
void mono_ios_runtime_init(void) {
  ...
  //
  path = [[NSString alloc]
      initWithFormat:@"%@/config.json", [main_bundle bundlePath]];
  ...
  if (data) {
    ...
    config_args[0] =
        strdup([((NSString *)[args_array objectAtIndex:0]) UTF8String]);
...
void mono_sdks_ui_register_test_result_fields(int *passed, int *skipped,
                                              int *failed,
                                              char **summaryMessage) {
  testPassed = passed;
  testSkipped = skipped;
...
void mono_sdks_ui_increment_testcase_result(int type) {
  switch (type) {
  case 0:
    (*testPassed)++;
    break;
...
void mono_sdks_ui_set_test_summary_message(const char *summaryMessage) {
...
//
// NOTE: The timezone functions are duplicated in XI, so if you're going to
// modify here, you have to modify there.
//
...
const char *xamarin_GetFolderPath(int folder) {
  // COOP: no managed memory access: any mode.
  // NSUInteger-based enum (and we do not want corlib exposed to 32/64 bits
  // differences)
...
void xamarin_log(const unsigned short *unicodeMessage) {
  ...
  NSString *msg =
      [[NSString alloc] initWithBytes:unicodeMessage
                               length:length
