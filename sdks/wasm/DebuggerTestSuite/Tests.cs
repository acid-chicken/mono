...
public class SourceList : DebuggerTestBase {
  ...
  CreateGoodBreakpointAndHit() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      CheckLocation("dotnet://debugger-test.dll/debugger-test.cs", 5, 2,
                    scripts, top_frame["location"]);
  ...
  InspectLocalsWithDelegatesAtBreakpointSite() =>
      await CheckInspectLocalsAtBreakpointSite(
          ...
          : () => JObject.FromObject(new {
                ...
                columnNumber = 2,
                url =
                    dicFileToUrl["dotnet://debugger-test.dll/debugger-test.cs"],
            }),
            eval_req_fn
          : () => JObject.FromObject(new {
                expression =
  ...
  async Task CheckInspectLocalsAtBreakpointSite(Func<JObject>bp_req_fn,
                                                Func<JObject>eval_req_fn,
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var bp_res =
          await cli.SendCommand("Debugger.setBreakpointByUrl", bp_req, token);
      ...
      var frame_props =
          await cli.SendCommand("Runtime.getProperties", get_prop_req, token);
  ...
  TrivalStepping() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var top_frame2 = pause_location2 ["callFrames"]
      [0];
      System.Console.WriteLine("TrivalStepping6");
      CheckLocation("dotnet://debugger-test.dll/debugger-test.cs", 3, 41,
                    scripts, top_frame2["functionLocation"]);
      System.Console.WriteLine("TrivalStepping7");
      CheckLocation("dotnet://debugger-test.dll/debugger-test.cs", 6, 2,
                    scripts, top_frame2["location"]); // it moved one line!
  ...
  InspectLocalsInPreviousFramesDuringSteppingIn2() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var bp1_req = JObject.FromObject(new {lineNumber = 24, columnNumber = 2,
  ...
  async Task<JObject>
  SendCommandAndCheck(JObject args, string method, DebugTestContext ctx,
                      Func<JObject, Task>wait_for_event_fn = null,
                      Action<JToken>locals_fn = null,
