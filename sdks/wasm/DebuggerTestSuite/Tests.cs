...
public class SourceList : DebuggerTestBase {
  ...
  Dictionary<string, string>SubscribeToScripts(Inspector insp) {
    ...
    insp.On("Debugger.scriptParsed", async (args, c) => {
      ...
      if (script_id.StartsWith("dotnet://")) {
        ...
        dbgUrl = arrStr[0] + "/" + arrStr[1] + "/" + arrStr[2] + "/" +
  ...
  void CheckLocation(string script_loc, int line, int column,
                     Dictionary<string, string>scripts, JToken location) {
  ...
  CheckThatAllSourcesAreSent() {
    ...
    await insp.Ready();
    Assert.Contains("dotnet://debugger-test.dll/debugger-test.cs",
                    scripts.Values);
    Assert.Contains("dotnet://debugger-test.dll/debugger-test2.cs",
                    scripts.Values);
    Assert.Contains("dotnet://Simple.Dependency.dll/dependency.cs",
                    scripts.Values);
  }
  ...
  CreateGoodBreakpoint() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      Assert.EndsWith("debugger-test.cs",
                      bp1_res.Value["breakpointId"].ToString());
            Assert.Equal (1, bp1_res.Value ["locations"]?.Value<JArray> ()?.Count);

            var loc = bp1_res.Value["locations"]?.Value<JArray>()[0];

            Assert.NotNull(loc["scriptId"]);
            Assert.Equal("dotnet://debugger-test.dll/debugger-test.cs",
  ...
  CreateBadBreakpoint() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var bp1_res =
          await cli.SendCommand("Debugger.setBreakpointByUrl", bp1_req, token);

      Assert.True(bp1_res.IsOk);
      Assert.Empty(bp1_res.Value["locations"].Values<object>());
      // Assert.Equal ((int)MonoErrorCodes.BpNotFound, bp1_res.Error
      // ["code"]?.Value<int> ());
    });
  }
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await EvaluateAndCheck(
          ...
          : (pause_location) => {
            ...
            Assert.Equal(bp.Value["breakpointId"]?.ToString(),
                         pause_location ["hitBreakpoints"]?
                         [0]?
                             .Value<string>());

            var top_frame = pause_location ["callFrames"]
            [0];
            Assert.Equal("IntAdd", top_frame["functionName"].Value<string>());
            Assert.Contains("debugger-test.cs",
                            top_frame["url"].Value<string>());

            CheckLocation("dotnet://debugger-test.dll/debugger-test.cs", 3, 41,
                          scripts, top_frame["functionLocation"]);

            // now check the scope
            var scope = top_frame ["scopeChain"]
            [0];
            ...
            Assert.Equal("object", scope ["object"]
                                   ["type"]);
            Assert.Equal("dotnet:scope:0", scope ["object"]
                                           ["objectId"]);
            CheckLocation("dotnet://debugger-test.dll/debugger-test.cs", 3, 41,
                          scripts, scope["startLocation"]);
            CheckLocation("dotnet://debugger-test.dll/debugger-test.cs", 9, 1,
                          scripts, scope["endLocation"]);
  ...
  ExceptionThrownInJSOutOfBand() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var ex = await Assert.ThrowsAsync<ArgumentException>(
          async () => await insp.WaitFor("Runtime.exceptionThrown"));
      var ex_json = JObject.Parse(ex.Message);
      Assert.Equal(dicFileToUrl["/debugger-driver.html"], ex_json
                                                          ["exceptionDetails"]?
                                                          ["url"]?
                                                              .Value<string>());
    });
  ...
  JToken CheckObject(JToken locals, string name, string class_name,
                     string subtype = null, bool is_null = false) {
    var l = GetAndAssertObjectWithName(locals, name);
    var val = l["value"];
    Assert.Equal("object", val["type"]?.Value<string>());
    Assert.True(val["isValueType"] == null ||
                !val["isValueType"].Value<bool>());
    Assert.Equal(class_name, val["className"]?.Value<string>());

    var has_null_subtype =
        val["subtype"] != null && val["subtype"]?.Value<string>() == "null";
  ...
  async Task CheckDateTime(JToken locals, string name, DateTime expected) {
    var obj = locals.Where(jt => jt["name"]?.Value<string>() == name)
                  .FirstOrDefault();
    Assert.Equal(expected.ToString(), obj ["value"]?
                                      ["description"]?
                                          .Value<string>());

    await CheckObjectOnLocals(locals, name, test_fn
                              : (members) => {
  ...
  JToken CheckBool(JToken locals, string name, bool expected) {
    ...
    if (val["value"] == null)
      Assert.True(false,
                  "expected bool value not found for variable named {name}");
  ...
  JToken CheckEnum(JToken locals, string name, string class_name,
                   string descr) {
    var l = GetAndAssertObjectWithName(locals, name);
    var val = l["value"];
    Assert.Equal("object", val["type"]?.Value<string>());
  ...
  void CheckFunction(JToken locals, string name, string description,
                     string subtype = null) {
  ...
  [Fact]
  public async Task
  InspectLocalsAtBreakpointSite() => await CheckInspectLocalsAtBreakpointSite(
      "dotnet://debugger-test.dll/debugger-test.cs", 5, 2, "IntAdd",
      "window.setTimeout(function() { invoke_add(); }, 1);", test_fn
      : (locals) => {
  ...
  InspectPrimitiveTypeLocalsAtBreakpointSite() =>
      await CheckInspectLocalsAtBreakpointSite(
          "dotnet://debugger-test.dll/debugger-test.cs", 143, 2,
          "PrimitiveTypesTest",
  ...
  [Fact]
  public async Task
  InspectLocalsWithDelegatesAtBreakpointSite() =>
      await CheckInspectLocalsAtBreakpointSite(
          ...
          : (locals) => {
            ...
            CheckObject(locals, "fn_func_null", "System.Func<Math, bool>",
                        is_null
                        : true);
            CheckArray(locals, "fn_func_arr", "System.Func<Math, bool>[]");
            CheckFunction(locals, "fn_del", "Math.IsMathNull");
            CheckObject(locals, "fn_del_null", "Math.IsMathNull", is_null
                        : true);
            CheckArray(locals, "fn_del_arr", "Math.IsMathNull[]");

            // Unused locals
            CheckObject(locals, "fn_func_unused", "System.Func<Math, bool>");
            CheckObject(locals, "fn_func_null_unused",
                        "System.Func<Math, bool>", is_null
                        : true);
            CheckObject(locals, "fn_func_arr_unused",
                        "System.Func<Math, bool>[]");

            CheckFunction(locals, "fn_del_unused", "Math.IsMathNull");
            CheckObject(locals, "fn_del_null_unused", "Math.IsMathNull", is_null
                        : true);
            CheckObject(locals, "fn_del_arr_unused", "Math.IsMathNull[]",
                        is_null
                        : true);
          });
  ...
  [Fact]
  public async Task
  InspectLocalsWithGenericTypesAtBreakpointSite() => await CheckInspectLocalsAtBreakpointSite(
      ...
      : (locals) => {
        ...
        CheckObject(
            ...
            "System.Collections.Generic.Dictionary<Math[], Math.IsMathNull>");
        CheckObject(
            locals, "list_null_unused",
            "System.Collections.Generic.Dictionary<Math[], Math.IsMathNull>",
            is_null
            : true);

        CheckObject(
            locals, "list_arr_unused",
            "System.Collections.Generic.Dictionary<Math[], Math.IsMathNull>[]");
        CheckObject(
            locals, "list_arr_null_unused",
            "System.Collections.Generic.Dictionary<Math[], Math.IsMathNull>[]",
            is_null
            : true);
      });

  async Task CheckInspectLocalsAtBreakpointSite(string url_key, int line,
                                                int column,
                                                string function_name,
                                                string eval_expression,
                                                Action<JToken>test_fn) {
    var insp = new Inspector();
    // Collect events
    var scripts = SubscribeToScripts(insp);
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await EvaluateAndCheck(
          ...
          : async (pause_location) => {
            ...
            var frame_props = await cli.SendCommand("Runtime.getProperties",
                                                    get_prop_req, token);
  ...
  TrivalStepping() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await StepAndCheck(
          StepKind.Over, "dotnet://debugger-test.dll/debugger-test.cs", 6, 2,
          "IntAdd", wait_for_event_fn
          : (pause_location) => {
            var top_frame = pause_location ["callFrames"]
            [0];
            CheckLocation("dotnet://debugger-test.dll/debugger-test.cs", 3, 41,
  ...
  InspectLocalsDuringStepping() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await EvaluateAndCheck(
          "window.setTimeout(function() { invoke_add(); }, 1);",
          debugger_test_loc, 4, 2, "IntAdd", locals_fn
          : (locals) => {
            CheckNumber(locals, "a", 10);
            CheckNumber(locals, "b", 20);
  ...
  [Fact]
  public async Task
  InspectLocalsInPreviousFramesDuringSteppingIn2() {
    var insp = new Inspector();
    // Collect events
    var scripts = SubscribeToScripts(insp);
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var pause_location = await EvaluateAndCheck(
          ...
          dep_cs_loc, 24, 2, "DoEvenMoreStuff", locals_fn
          : (locals) => {
            Assert.Single(locals);
            CheckObject(locals, "this", "Simple.Complex");
          });

      await CheckObjectOnFrameLocals(pause_location ["callFrames"]
                                     [0],
                                     "this", test_fn
                                     : (props) => {
      ...
      await CheckLocalsOnFrame(pause_location ["callFrames"]
                               [3],
                               debugger_test_loc, 17, 2, "UseComplex", test_fn
                               : (locals_m1) => {
                                 Assert.Equal(7, locals_m1.Count());

                                 CheckNumber(locals_m1, "a", 10);
                                 CheckNumber(locals_m1, "b", 20);
                                 CheckObject(locals_m1, "complex",
                                             "Simple.Complex");
      ...
      pause_location = await StepAndCheck(StepKind.Over, dep_cs_loc, 16, 2,
                                          "DoStuff", times : 2);
  ...
  [Fact]
  public async Task
  InspectLocalsInPreviousFramesDuringSteppingIn() {
    var insp = new Inspector();
    // Collect events
    var scripts = SubscribeToScripts(insp);
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var wait_res = await EvaluateAndCheck(
          ...
          debugger_test_loc, 100, 3, "InnerMethod", locals_fn
          : (locals) => {
      ...
      var this_props = await CheckObjectOnFrameLocals(
          wait_res ["callFrames"]
          [0],
          "this", test_fn
          : (props) => {
      ...
      var ss_props = await CheckObjectOnLocals(
          this_props, "SimpleStructProperty", test_fn
          : (props) => {
            Assert.Equal(2, props.Count());
      ...
      await CheckLocalsOnFrame(wait_res ["callFrames"]
                               [1],
                               debugger_test_loc, 76, 2, "OuterMethod", test_fn
                               : (locals_m1) => {
                                 Assert.Equal(5, locals_m1.Count());
                                 // FIXME: Failing test CheckNumber (locals_m1,
                                 // "i", 5);
                                 // FIXME: Failing test CheckString (locals_m1,
                                 // "text", "Hello");
                                 CheckNumber(locals_m1, "new_i", 0);
                                 CheckNumber(locals_m1, "k", 0);
                                 CheckObject(locals_m1, "nim",
                                             "Math.NestedInMath");
                               });

      // step back into OuterMethod
      await StepAndCheck(StepKind.Over, debugger_test_loc, 80, 2, "OuterMethod",
                         times : 9, locals_fn
                         : (locals) => {
                           Assert.Equal(5, locals.Count());

                           // FIXME: Failing test CheckNumber (locals_m1, "i",
                           // 5);
                           CheckString(locals, "text", "Hello");
                           // FIXME: Failing test CheckNumber (locals, "new_i",
                           // 24);
                           CheckNumber(locals, "k", 19);
                           CheckObject(locals, "nim", "Math.NestedInMath");
                         });

      // await StepAndCheck (StepKind.Over,
      // "dotnet://debugger-test.dll/debugger-test.cs", 81, 2, "OuterMethod",
      // times: 2);

      // step into InnerMethod2
      await StepAndCheck(StepKind.Into,
                         "dotnet://debugger-test.dll/debugger-test.cs", 85, 1,
                         "InnerMethod2", locals_fn
                         : (locals) => {
      ...
      await StepAndCheck(StepKind.Over,
                         "dotnet://debugger-test.dll/debugger-test.cs", 89, 1,
                         "InnerMethod2", times : 4, locals_fn
                         : (locals) => {
                           Assert.Equal(3, locals.Count());
      ...
      await StepAndCheck(StepKind.Over,
                         "dotnet://debugger-test.dll/debugger-test.cs", 81, 2,
                         "OuterMethod", times : 2, locals_fn
                         : (locals) => {
                           Assert.Equal(5, locals.Count());
  ...
  InspectLocalsDuringSteppingIn() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await StepAndCheck(StepKind.Over,
                         "dotnet://debugger-test.dll/debugger-test.cs", 76, 2,
                         "OuterMethod", locals_fn
                         : (locals) => {
                           Assert.Equal(5, locals.Count());
      ...
      await StepAndCheck(StepKind.Into,
                         "dotnet://debugger-test.dll/debugger-test.cs", 94, 2,
                         "InnerMethod");
      await StepAndCheck(StepKind.Over,
                         "dotnet://debugger-test.dll/debugger-test.cs", 98, 3,
                         "InnerMethod", times : 5, locals_fn
                         : (locals) => {
                           Assert.Equal(4, locals.Count());
  ...
  InspectLocalsInAsyncMethods() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await CheckLocalsOnFrame(wait_res ["callFrames"]
                               ...
                               : (locals) => {
                                 Assert.Equal(4, locals.Count());
                                 CheckString(locals, "ls",
                                             "string from jstest");
                                 CheckNumber(locals, "li", 52);
                               });

      // TODO: previous frames have async machinery details, so no point
      // checking that right now

      var pause_loc = await SendCommandAndCheck(
          null, "Debugger.resume", debugger_test_loc, 123, 3,
          /*FIXME: "AsyncMethodNoReturn"*/ "MoveNext", locals_fn
          : (locals) => {
            Assert.Equal(4, locals.Count());
      ...
      await CheckObjectOnFrameLocals(
          pause_loc ["callFrames"]
          [0],
          "this", test_fn
          : (props) => {
            Assert.Equal(2, props.Count());
  ...
  InspectLocalsWithStructs() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var pause_location = await EvaluateAndCheck(
          ...
          debugger_test_loc, 16, 2, "MethodWithLocalStructs", locals_fn
          : (locals) => {
            Assert.Equal(3, locals.Count());

            CheckValueType(locals, "ss_local",
      ...
      var ss_local_props = await CompareObjectPropertiesOnFrameLocals(
          pause_location ["callFrames"]
          [0],
          "ss_local",
          new {
              str_member = TString(
                  "set in MethodWithLocalStructs#SimpleStruct#str_member"),
...

      {
        ...
        // Check ss_local.gs
        await CheckObjectOnLocals(
            ss_local_props, "gs", test_fn
            : (props) => {
              CheckString(
                  props, "StringField",
                  "set in MethodWithLocalStructs#SimpleStruct#gs#StringField");
              CheckObject(props, "List",
                          "System.Collections.Generic.List<System.DateTime>");
            });
      }

      // Check gs_local's properties
      await CompareObjectPropertiesOnFrameLocals(
          pause_location ["callFrames"]
          [0],
          "gs_local",
          new {
      ...
      var vt_local_props = await CheckObjectOnFrameLocals(
          pause_location ["callFrames"]
          [0],
          "vt_local", test_fn
          : async (props) => {
            ...
            CheckValueType(props, "SimpleStructField",
                           "DebuggerTests.ValueTypesTest.SimpleStruct");
            CheckValueType(props, "SimpleStructProperty",
                           "DebuggerTests.ValueTypesTest.SimpleStruct");
...

      {
        ...
        var ssp_props = await CompareObjectPropertiesFor(
            vt_local_props, "SimpleStructProperty",
            new {
                str_member = TString(
                    "SimpleStructProperty#string#0#SimpleStruct#str_member"),
                dt = TValueType("System.DateTime", dt.ToString()),
                gs = TValueType(
                    "DebuggerTests.ValueTypesTest.GenericStruct<System.DateTime>"),
                Kind = TEnum("System.DateTimeKind", "Utc")},
        ...
        var ssf_props = await CompareObjectPropertiesFor(
            vt_local_props, "SimpleStructField",
            new {
                str_member = TString(
                    "SimpleStructField#string#0#SimpleStruct#str_member"),
                dt = TValueType("System.DateTime", dt.ToString()),
                gs = TValueType(
                    "DebuggerTests.ValueTypesTest.GenericStruct<System.DateTime>"),
  ...
  InspectValueTypeMethodArgs() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var pause_location = await EvaluateAndCheck(
          ...
          debugger_test_loc, 27, 3, "MethodWithStructArgs", locals_fn
          : (locals) => {
            Assert.Equal(3, locals.Count());

            CheckString(locals, "label", "TestStructsAsMethodArgs#label");
            CheckValueType(locals, "ss_arg",
                           "DebuggerTests.ValueTypesTest.SimpleStruct");
      ...
      var ss_arg_props = await CompareObjectPropertiesOnFrameLocals(
          pause_location ["callFrames"]
          [0],
      ...
      pause_location = await StepAndCheck(
          StepKind.Over, debugger_test_loc, 31, 3, "MethodWithStructArgs",
          times : 4, locals_fn
          : (locals) => {
            Assert.Equal(3, locals.Count());
      ...
      ss_arg_props = await CompareObjectPropertiesOnFrameLocals(
          pause_location ["callFrames"]
          [0],
      ...
      pause_location = await StepAndCheck(
          StepKind.Over, debugger_test_loc, 22, 3, "TestStructsAsMethodArgs",
          times : 2, locals_fn
          : (l) => {/* non-null to make sure that locals get fetched */});
      await CheckLocalsOnFrame(
          pause_location ["callFrames"]
          [0],
          new {ss_local =
      ...
      {
        // Check ss_local.gs
        await CompareObjectPropertiesFor(ss_arg_props, "gs", ss_local_gs, label
                                         : "ss_local_gs");
      }
  ...
  [Fact]
  public async Task
  InspectLocalsWithStructsStaticAsync() {
    var insp = new Inspector();
    // Collect events
    var scripts = SubscribeToScripts(insp);
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await CheckLocalsOnFrame(
          pause_location ["callFrames"]
          [0],
          new {ss_local = TObject("DebuggerTests.ValueTypesTest.SimpleStruct"),
               gs_local = TValueType(
                   "DebuggerTests.ValueTypesTest.GenericStruct<int>"),
               result = TBool(true)},
          "locals#0");
      ...
      await CompareObjectPropertiesOnFrameLocals(
          ...
          [0],
          "gs_local",
          new {StringField = TString(
                   "gs_local#GenericStruct<ValueTypesTest>#StringField"),
  ...
  public async Task
  InspectPrimitiveTypeArrayLocals(int line, int col, string method_name,
                                  bool test_prev_frame, int frame_idx) =>
      await TestSimpleArrayLocals(
          line, col, entry_method_name
          : "[debugger-test] DebuggerTests.ArrayTestsClass:PrimitiveTypeLocals",
            method_name
          : method_name, etype_name
          : "int", local_var_name_prefix
          : "int", array
          : new[]{TNumber(4), TNumber(70), TNumber(1)}, array_elements
          : null, test_prev_frame
          : test_prev_frame, frame_idx
          : frame_idx);

  static Func<int, int, string, string, object>TSimpleClass =
      (X, Y, Id, Color) =>
          new {X = TNumber(X), Y = TNumber(Y), Id = TString(Id),
  ...
  static Func<int, int, string, string, object>TPoint = (X, Y, Id, Color) =>
      new {
          X = TNumber(X),
          Y = TNumber(Y),
          Id = TString(Id),
  ...
  [InlineData(93, 2, "YetAnotherMethod", true, 2)]
  public async Task
  InspectObjectArrayLocals(int line, int col, string method_name,
                           bool test_prev_frame, int frame_idx) =>
      await TestSimpleArrayLocals(
          line, col, entry_method_name
          : "[debugger-test] DebuggerTests.ArrayTestsClass:ObjectTypeLocals",
            method_name
          : method_name, etype_name
          : "DebuggerTests.SimpleClass", local_var_name_prefix
          : "class", array
          : new[]{TObject("DebuggerTests.SimpleClass"),
                  TObject("DebuggerTests.SimpleClass", is_null
                          : true),
                  TObject("DebuggerTests.SimpleClass")},
            array_elements
          : new[]{TSimpleClass(5, -2, "class_arr#Id#0", "Green"),
  ...
  [InlineData(93, 2, "YetAnotherMethod", true, 2)]
  public async Task
  InspectGenericTypeArrayLocals(int line, int col, string method_name,
                                bool test_prev_frame, int frame_idx) =>
      await TestSimpleArrayLocals(
          line, col, entry_method_name
          : "[debugger-test] DebuggerTests.ArrayTestsClass:GenericTypeLocals",
            method_name
          : method_name, etype_name
          : "DebuggerTests.GenericClass<int>", local_var_name_prefix
          : "gclass", array
          : new[]{TObject("DebuggerTests.GenericClass<int>", is_null
                          : true),
                  TObject("DebuggerTests.GenericClass<int>"),
                  TObject("DebuggerTests.GenericClass<int>")},
            array_elements
          : new[]{null, // Element is null
                  new {Id = TString("gclass_arr#1#Id"),
                       Color = TEnum("DebuggerTests.RGB", "Red"),
                       Value = TNumber(5)},
                  new {Id = TString("gclass_arr#2#Id"),
  ...
  [InlineData(93, 2, "YetAnotherMethod", true, 2)]
  public async Task
  InspectGenericValueTypeArrayLocals(int line, int col, string method_name,
                                     bool test_prev_frame, int frame_idx) =>
      await TestSimpleArrayLocals(
          line, col, entry_method_name
          : "[debugger-test] DebuggerTests.ArrayTestsClass:GenericValueTypeLocals",
            method_name
          : method_name, etype_name
          ...
          : new[]{TValueType(
                  ...
                  TValueType(
                      "DebuggerTests.SimpleGenericStruct<DebuggerTests.Point>")},
            array_elements
          : new[]{new {Id = TString("gvclass_arr#1#Id"),
                       Color = TEnum("DebuggerTests.RGB", "Red"),
                       Value = TPoint(100, 200, "gvclass_arr#1#Value#Id",
                                      "Red")},
                  new {Id = TString("gvclass_arr#2#Id"),
                       Color = TEnum("DebuggerTests.RGB", "Blue"),
                       Value = TPoint(10, 20, "gvclass_arr#2#Value#Id",
                                      "Green")}},
            test_prev_frame
          : test_prev_frame, frame_idx
          : frame_idx);
  ...
  [InlineData(93, 2, "YetAnotherMethod", true, 2)]
  public async Task
  InspectGenericValueTypeArrayLocals2(int line, int col, string method_name,
                                      bool test_prev_frame, int frame_idx) =>
      await TestSimpleArrayLocals(
          line, col, entry_method_name
          : "[debugger-test] DebuggerTests.ArrayTestsClass:GenericValueTypeLocals2",
            method_name
          : method_name, etype_name
          : "DebuggerTests.SimpleGenericStruct<DebuggerTests.Point[]>",
            local_var_name_prefix
          ...
          : new[]{new {
                      ...
                      Color = TEnum("DebuggerTests.RGB", "Red"),
                      Value = new[]{TPoint(100, 200, "gvclass_arr#0#0#Value#Id",
                                           "Red"),
                                    TPoint(100, 200, "gvclass_arr#0#1#Value#Id",
                                           "Green")}},
                  new {
                      Id = TString("gvclass_arr#1#Id"),
                      Color = TEnum("DebuggerTests.RGB", "Blue"),
                      Value = new[]{TPoint(100, 200, "gvclass_arr#1#0#Value#Id",
                                           "Green"),
                                    TPoint(100, 200, "gvclass_arr#1#1#Value#Id",
                                           "Blue")}}},
            test_prev_frame
          : test_prev_frame, frame_idx
          : frame_idx);

  async Task TestSimpleArrayLocals(int line, int col, string entry_method_name,
                                   string method_name, string etype_name,
                                   string local_var_name_prefix, object[] array,
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var pause_location = await EvaluateAndCheck(eval_expr, debugger_test_loc,
      ...
      await CheckLocalsOnFrame(
          ...
          : (locals) => {
            Assert.Equal(4, locals.Count());
            CheckArray(locals, $"{local_var_name_prefix}_arr",
                       $"{etype_name}[]");
            CheckArray(locals, $"{local_var_name_prefix}_arr_empty",
                       $"{etype_name}[]");
            CheckObject(locals, $"{local_var_name_prefix}_arr_null",
                        $"{etype_name}[]", is_null
                        : true);
            CheckBool(locals, "call_other", test_prev_frame);
          });

      var prefix_arr = await CompareObjectPropertiesOnFrameLocals(
          pause_location ["callFrames"]
          [frame_idx],
      ...
      if (array_elements?.Length > 0) {
        for (int i = 0; i < array_elements.Length; i++) {
          ...
          if (array_elements[i] == null) {
            ...
            Assert.True(act_i != null,
                        $"[{label}] Couldn't find array element {i_str}");

            await CheckValue(act_i["value"],
                             TObject(etype_name, is_null
                                     : true),
                             label);
          } else {
            await CompareObjectPropertiesFor(prefix_arr, i_str,
                                             array_elements[i], label
                                             : label);
          }
        }
      }
  ...
  InspectObjectArrayMembers() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await CheckLocalsOnFrame(pause_location ["callFrames"]
                               ...
                               : (locals) => {
                                 Assert.Single(locals);
                                 CheckObject(locals, "c",
                                             "DebuggerTests.Container");
                               });

      var c_props = await CompareObjectPropertiesOnFrameLocals(
          pause_location ["callFrames"]
          [frame_idx],
          "c",
          new {id = TString("c#id"),
      ...
      await CompareObjectPropertiesFor(
          c_props, "ClassArrayProperty",
          new[]{TSimpleClass(5, -2, "ClassArrayProperty#Id#0", "Green"),
      ...
      await CompareObjectPropertiesFor(
          c_props, "ClassArrayField",
          new[]{TObject("DebuggerTests.SimpleClass", is_null
                        : true),
  ...
  [Fact]
  public async Task
  InspectValueTypeArrayLocalsStaticAsync() {
    var insp = new Inspector();
    // Collect events
    var scripts = SubscribeToScripts(insp);
    int line = 143;
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var eval_expr =
          "window.setTimeout(function() { invoke_static_method_async (" +
          $"'{entry_method_name}', false" // *false* here keeps us only in the
                                          // static method
          + "); }, 1);";

      var pause_location = await EvaluateAndCheck(eval_expr, debugger_test_loc,
                                                  line, col, method_name);
      var frame_locals = await CheckLocalsOnFrame(
          pause_location ["callFrames"]
          [frame_idx],
          new {call_other = TBool(false),
               gvclass_arr = TArray(
                   "DebuggerTests.SimpleGenericStruct<DebuggerTests.Point>[]"),
               ...
               points = TObject("DebuggerTests.Point[]", is_null
                                : true)},
          "ValueTypeLocalsAsync#locals");

      var local_var_name_prefix = "gvclass";
      await CompareObjectPropertiesFor(
          frame_locals, local_var_name_prefix,
          new {Id = TString(null), Color = TEnum("DebuggerTests.RGB", "Red"),
               Value = TPoint(0, 0, null, "Red")});

      await CompareObjectPropertiesFor(
          frame_locals, $"{local_var_name_prefix}_arr",
          new[]{
              new {Id = TString("gvclass_arr#1#Id"),
                   Color = TEnum("DebuggerTests.RGB", "Red"),
      ...
      await CompareObjectPropertiesFor(
          frame_locals, $"{local_var_name_prefix}_arr_empty", new object[0]);
    });
  }
  ...
  [Fact]
  public async Task
  InspectValueTypeArrayLocalsInstanceAsync() {
    var insp = new Inspector();
    // Collect events
    var scripts = SubscribeToScripts(insp);
    int line = 155;
  ...
  [Fact]
  public async Task
  InspectValueTypeArrayLocalsInAsyncStaticStructMethod() {
    var insp = new Inspector();
    // Collect events
    var scripts = SubscribeToScripts(insp);
    int line = 222;
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var frame_locals = await CheckLocalsOnFrame(
          pause_location ["callFrames"]
          [frame_idx],
          new {call_other = TBool(false), local_i = TNumber(5),
  ...
  [Fact]
  public async Task
  InspectValueTypeArrayLocalsInAsyncInstanceStructMethod() {
    var insp = new Inspector();
    // Collect events
    var scripts = SubscribeToScripts(insp);
    int line = 229;
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var frame_locals = await CheckLocalsOnFrame(
          pause_location ["callFrames"]
          [frame_idx],
          new {sc_arg = TObject("DebuggerTests.SimpleClass"),
               @this = TValueType("DebuggerTests.Point"),
               local_gs = TValueType("DebuggerTests.SimpleGenericStruct<int>")},
          "locals#0");

      await CompareObjectPropertiesFor(
          frame_locals, "local_gs",
          new {Id = TString("local_gs#Id"),
               Color = TEnum("DebuggerTests.RGB", "Green"), Value = TNumber(4)},
          label
          : "local_gs#0");

      await CompareObjectPropertiesFor(
          frame_locals, "sc_arg", TSimpleClass(10, 45, "sc_arg#Id", "Blue"),
          label
          : "sc_arg#0");

      await CompareObjectPropertiesFor(
          frame_locals, "this", TPoint(90, -4, "point#Id", "Green"), label
          : "this#0");
    });
  }
  ...
  public async Task
  InspectLocalsForToStringDescriptions(int line, int col, string method_name,
                                       bool call_other, bool invoke_async) {
    var insp = new Inspector();
    // Collect events
    var scripts = SubscribeToScripts(insp);
    string entry_method_name =
        $"[debugger-test] DebuggerTests.ValueTypesTest:MethodWithLocalsForToStringTest{(invoke_async ? " Async
        " : String.Empty)}";
    int frame_idx = 0;
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var eval_expr = "window.setTimeout(function() {" +
                      (invoke_async ? "invoke_static_method_async ("
      ...
      var pause_location = await EvaluateAndCheck(
          eval_expr, debugger_test_loc, line, col, invoke_async ? "MoveNext"
          : method_name);
      ...
      var frame_locals = await CheckLocalsOnFrame(
          pause_location ["callFrames"]
          [frame_idx],
          new {call_other = TBool(call_other),
               ...
               guid = TValueType("System.Guid",
                                 "3D36E07E-AC90-48C6-B7EC-A481E289D014"),
      ...
      await CompareObjectPropertiesFor(
          frame_locals, "dto",
          new {Day = TNumber(2), Year = TNumber(2020),
      ...
      var obj_props = await CompareObjectPropertiesFor(
          frame_locals, "obj",
          new {DT = TValueType("System.DateTime", DT.ToString()),
               DTO = TValueType("System.DateTimeOffset", DTO.ToString()),
               TS = TValueType("System.TimeSpan", ts.ToString()),
  ...
  EvaluateThisProperties() {
    ...
    int col = 16;
    string entry_method_name =
        "[debugger-test] DebuggerTests.EvaluateTestsClass:EvaluateLocals";

    await Ready();
    await insp.Ready(async (cli, token) => {
      ctx = new DebugTestContext(cli, insp, token, scripts);
      var debugger_test_loc =
          "dotnet://debugger-test.dll/debugger-evaluate-test.cs";
      ...
      var pause_location = await EvaluateAndCheck(
          eval_expr, debugger_test_loc, line, col, "run", wait_for_event_fn
          : async (pause_location) => {
            var locals = await GetProperties(pause_location ["callFrames"]
                                             [0]
                                             ["callFrameId"]
                                                 .Value<string>());
            var evaluate =
                await EvaluateOnCallFrame(pause_location ["callFrames"]
                                          [0]
                                          ["callFrameId"]
                                              .Value<string>(),
                                          "a");
            CheckContentValue(evaluate, "1");
            evaluate = await EvaluateOnCallFrame(pause_location ["callFrames"]
                                                 [0]
                                                 ["callFrameId"]
                                                     .Value<string>(),
                                                 "b");
  ...
  void AssertLocation(JObject args, string methodName) {
    Assert.Equal(methodName, args ["callFrames"]?
                             [0]?
                             ["functionName"]?
                                 .Value<string>());
  }
  ...
  InspectLocals() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var locals = await GetProperties(wait_res ["callFrames"]
                                       [1]
                                       ["callFrameId"]
                                           .Value<string>());
    });
  ...
  async Task<JObject>StepAndCheck(StepKind kind, string script_loc, int line,
                                  int column, string function_name,
                                  Func<JObject, Task>wait_for_event_fn = null,
                                  Action<JToken>locals_fn = null,
                                  int times = 1) {
    for (int i = 0; i < times - 1; i++) {
      await SendCommandAndCheck(null, $"Debugger.step{kind.ToString ()}", null,
                                -1, -1, null);
    }

    // Check for method/line etc only at the last step
    return await SendCommandAndCheck(null, $"Debugger.step{kind.ToString ()}",
                                     script_loc, line, column, function_name,
                                     wait_for_event_fn
                                     : wait_for_event_fn, locals_fn
                                     : locals_fn);
  }

  async Task<JObject>
  EvaluateAndCheck(string expression, string script_loc, int line, int column,
                   string function_name,
                   Func<JObject, Task>wait_for_event_fn = null,
                   Action<JToken>locals_fn = null) =>
      await SendCommandAndCheck(JObject.FromObject(new {
                                    expression = expression}),
                                "Runtime.evaluate", script_loc, line, column,
                                function_name, wait_for_event_fn
                                : wait_for_event_fn, locals_fn
                                : locals_fn);

  async Task<JObject>SendCommandAndCheck(
      JObject args, string method, string script_loc, int line, int column,
      string function_name, Func<JObject, Task>wait_for_event_fn = null,
    ...
    if (function_name != null)
      Assert.Equal(function_name, wait_res ["callFrames"]?
                                  [0]?
                                  ["functionName"]?
                                      .Value<string>());

    if (script_loc != null)
      CheckLocation(script_loc, line, column, ctx.scripts,
                    wait_res ["callFrames"]
                    [0]
                    ["location"]);
  ...
  async Task CheckProps(JToken actual, object exp_o, string label,
                        int num_fields = -1) {
    if (exp_o.GetType().IsArray || exp_o is JArray) {
      if (!(actual is JArray actual_arr)) {
        Assert.True(
            false, $"[{label}] Expected to get an array here but got {actual}");
        return;
      }
      ...
      for (int i = 0; i < exp_v_arr.Count; i++) {
        ...
        Assert.True(act_i["name"]?.Value<string>() == $"[{i}]",
                    $"{label}-[{i}].name");
    ...
    foreach(var kvp in exp) {
      ...
      var actual_obj =
          actual.FirstOrDefault(jt => jt["name"]?.Value<string>() == exp_name);
      if (actual_obj == null) {
        Console.WriteLine(
            $"actual: {actual}, exp_name: {exp_name}, exp_val: {exp_val}");
        Assert.True(actual_obj != null,
      ...
      if (exp_val.Type == JTokenType.Array) {
        var actual_props =
            await GetProperties(actual_val["objectId"]?.Value<string>());
  ...
  async Task CheckValue(JToken actual_val, JToken exp_val, string label) {
    if (exp_val["type"] == null && actual_val["objectId"] != null) {
      ...
      await CheckProps(new_val, exp_val,
    ...
    foreach(var jp in exp_val.Values<JProperty>()) {
      ...
      Assert.True(actual_field_val != null,
  ...
  async Task<JToken>CheckLocalsOnFrame(JToken frame, string script_loc,
                                       int line, int column,
                                       string function_name,
  ...
  async Task<JToken>CheckLocalsOnFrame(JToken frame, object expected,
                                       string label) {
  ...
  // Find an object with @name in the *frame*, *fetch* the object, and check
  // against @o
  async Task<JToken>CompareObjectPropertiesOnFrameLocals(JToken locals,
    ...
    try {
      ...
      Console.WriteLine(
          $"CheckObjectOnFrameLocals failed for locals: {obj_props}");
      throw;
    }
    return obj_props;
  }

  async Task<JToken>CheckObjectOnFrameLocals(JToken frame, string name,
                                             Action<JToken>test_fn) {
    var locals = await GetProperties(frame["callFrameId"].Value<string>());
  ...
  async Task<JToken>CompareObjectPropertiesFor(JToken locals, string name,
                                               object o, string label = null,
                                               int num_fields = -1) {
    if (label == null)
      label = name;
  ...
  async Task<JToken>CheckObjectOnLocals(JToken locals, string name,
                                        Action<JToken>test_fn = null) {
    var obj = locals.Where(jt => jt["name"]?.Value<string>() == name)
                  .FirstOrDefault();
    if (obj == null) {
      ...
      Assert.True(false,
                  $"Could not find a var with name {name} and type object");
    }

    var objectId = obj ["value"]
                   ["objectId"]?
                       .Value<string>();
    Assert.True(!String.IsNullOrEmpty(objectId),
  ...
  async Task<JToken>GetProperties(string id) {
    ...
    var frame_props = await ctx.cli.SendCommand("Runtime.getProperties",
                                                get_prop_req, ctx.token);
    if (!frame_props.IsOk)
      Assert.True(
          false,
          $"Runtime.getProperties failed for {get_prop_req.ToString ()}, with Result: {frame_props}");
  ...
  async Task<JToken>EvaluateOnCallFrame(string id, string expression) {
    var evaluate_req =
        JObject.FromObject(new {callFrameId = id, expression = expression});

    var frame_evaluate = await ctx.cli.SendCommand(
        "Debugger.evaluateOnCallFrame", evaluate_req, ctx.token);
    if (!frame_evaluate.IsOk)
      Assert.True(
          false,
          $"Debugger.evaluateOnCallFrame failed for {evaluate_req.ToString ()}, with Result: {frame_evaluate}");

    var evaluate_result = frame_evaluate.Value["result"];
    return evaluate_result;
  }

  async Task<Result>SetBreakpoint(string url_key, int line, int column,
                                  bool expect_ok = true) {
    var bp1_req = JObject.FromObject(new {
        lineNumber = line,
        columnNumber = column,
    ...
    var bp1_res = await ctx.cli.SendCommand("Debugger.setBreakpointByUrl",
                                            bp1_req, ctx.token);
  ...
  async Task<Result>SetBreakpointInMethod(string assembly, string type,
                                          string method) {
    var req = JObject.FromObject(
        new {assemblyName = assembly, typeName = type, methodName = method});

    // Protocol extension
    var res = await ctx.cli.SendCommand("Dotnet-test.setBreakpointByMethod",
                                        req, ctx.token);
  ...
  static JObject TString(string value) => value == null
    ?
                                                   TObject("string", is_null
                                                           : true)
      : JObject.FromObject(
            new {type = "string", value = @value, description = @value});

  static JObject TNumber(int value) => JObject.FromObject(new {
      type = "number", value = @value.ToString(),
      description = value.ToString()});

  static JObject TValueType(string className, string description = null,
                            object members = null) => JObject.FromObject(new {
      type = "object", isValueType = true, className = className,
      description = description ?? className});

  static JObject TEnum(string className, string descr, object members = null) =>
      JObject.FromObject(new {type = "object", isEnum = true,
                              className = className, description = descr});

  static JObject
  TObject(string className, string description = null,
          bool is_null = false) => is_null
    ? JObject
                                       .FromObject(new {
                                         type = "object",
                                         className = className,
      ...
      : JObject.FromObject(new {type = "object", className = className,
                                description = description ?? className});

  static JObject TArray(string className) => JObject.FromObject(new {
      type = "object", className = className, description = className,
      subtype = "array"});

  static JObject TBool(bool value) => JObject.FromObject(new {
    type = "boolean",
    value = @value,
    description = @value ? "true" : "false"
  });

  static JObject TSymbol(string value) => JObject.FromObject(new {
      type = "symbol", value = @value, description = @value});
...
class DebugTestContext {
  ...
  public DebugTestContext(InspectorClient cli, Inspector insp,
