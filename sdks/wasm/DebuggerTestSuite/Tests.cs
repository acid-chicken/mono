...
public class SourceList : DebuggerTestBase {
  ...
  CheckThatAllSourcesAreSent() {
    ...
    await insp.Ready();
    Assert.True(
        scripts.ContainsValue("dotnet://debugger-test.dll/debugger-test.cs"));
    Assert.True(
        scripts.ContainsValue("dotnet://debugger-test.dll/debugger-test2.cs"));
    Assert.True(
        scripts.ContainsValue("dotnet://Simple.Dependency.dll/dependency.cs"));
  }
  ...
  CreateGoodBreakpointAndHit() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      Assert.True(
          top_frame["url"].Value<string>().Contains("debugger-test.cs"));
