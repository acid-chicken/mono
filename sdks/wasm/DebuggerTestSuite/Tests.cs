...
public class SourceList : DebuggerTestBase {
  ...
  CreateGoodBreakpoint() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var bp1_res = await SetBreakpoint(
          "dotnet://debugger-test.dll/debugger-test.cs", 5, 2, ctx);
  ...
  CreateGoodBreakpointAndHit() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await EvaluateAndCheck(
          ...
          "dotnet://debugger-test.dll/debugger-test.cs", 5, 2, "IntAdd", ctx,
          wait_for_event_fn
          : (pause_location) => {
  ...
  void CheckObject(JToken locals, string name, string class_name,
                   string subtype = null) {
    foreach(var l in locals) {
      if (name != l["name"]?.Value<string>())
        continue;
  ...
  InspectLocalsWithDelegatesAtBreakpointSite() =>
      await CheckInspectLocalsAtBreakpointSite(
          ...
          : (locals) => {
            ...
            CheckObject(locals, "fn_del_null", "Math.IsMathNull", subtype
                        : "null");
            ...
            CheckObject(locals, "fn_func_null_unused",
                        "System.Func<Math, bool>", subtype
                        : "null");
            CheckObject(locals, "fn_func_arr_unused",
                        "System.Func<Math, bool>[]", subtype
                        : "null");

            CheckObject(locals, "fn_del_unused", "Math.IsMathNull", subtype
                        : "null");
            CheckObject(locals, "fn_del_null_unused", "Math.IsMathNull", subtype
                        : "null");
  ...
  RuntimeGetPropertiesWithInvalidScopeIdTest() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var bp = await SetBreakpoint(
          "dotnet://debugger-test.dll/debugger-test.cs", 41, 2, ctx);

      await EvaluateAndCheck(
          "window.setTimeout(function() { invoke_delegates_test (); }, 1);",
          "dotnet://debugger-test.dll/debugger-test.cs", 41, 2, "IntAdd", ctx,
          wait_for_event_fn
  ...
  InspectLocalsDuringStepping() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await EvaluateAndCheck(
          "window.setTimeout(function() { invoke_add(); }, 1);",
          debugger_test_loc, 4, 2, "IntAdd", ctx, locals_fn
          : (locals) => {
            CheckNumber(locals, "a", 10);
            CheckNumber(locals, "b", 20);
  ...
  InspectLocalsInPreviousFramesDuringSteppingIn2() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var wait_res = await EvaluateAndCheck(
          ...
          dep_cs_loc, 24, 2, "DoEvenMoreStuff", ctx, locals_fn
          : (locals) => {
      ...
      await CheckLocalsOnFrame(
          wait_res ["callFrames"]
          [3],
          debugger_test_loc, 17, 2, "UseComplex", ctx, test_fn
          : (locals_m1) => {
      ...
      wait_res = await StepAndCheck(StepKind.Over, dep_cs_loc, 16, 2, "DoStuff",
                                    ctx, times : 2);
  ...
  InspectLocalsInPreviousFramesDuringSteppingIn() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var wait_res = await EvaluateAndCheck(
          ...
          debugger_test_loc, 100, 3, "InnerMethod", ctx, locals_fn
          : (locals) => {
      ...
      await CheckLocalsOnFrame(
          wait_res ["callFrames"]
          [1],
          debugger_test_loc, 76, 2, "OuterMethod", ctx, test_fn
          : (locals_m1) => {
      ...
      await StepAndCheck(StepKind.Over,
                         "dotnet://debugger-test.dll/debugger-test.cs", 80, 2,
                         "OuterMethod", ctx, times : 7, locals_fn
                         : (locals) => {
      ...
      await StepAndCheck(StepKind.Over,
                         ...
                         "InnerMethod2", ctx, times : 4, locals_fn
                         : (locals) => {
                           // umm use JOBject instead of JToken?
                           Assert.Equal(3, locals.Count());
      ...
      await StepAndCheck(StepKind.Over,
                         ...
                         "OuterMethod", ctx, times : 2, locals_fn
                         : (locals) => {
                           Assert.Equal(5, locals.Count());
  ...
  InspectLocalsDuringSteppingIn() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await StepAndCheck(StepKind.Over,
                         ...
                         "InnerMethod", ctx, times : 5, locals_fn
                         : (locals) => {
                           Assert.Equal(4, locals.Count());
      ...
      await StepAndCheck(StepKind.Over,
                         ...
                         "OuterMethod", ctx, times : 5, locals_fn
                         : (locals) => {
                           Assert.Equal(5, locals.Count());
  ...
  InspectLocalsInAsyncMethods() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      await SendCommandAndCheck(
          ...
          "AsyncMethodNoReturn", ctx, locals_fn
          : (locals) => {
            Assert.Equal(4, locals.Count());
  ...
  InspectLocalsWithStructs() {
    ...
    await insp.Ready(async (cli, token) => {
      ...
      var wait_res = await EvaluateAndCheck(
          ...
          debugger_test_loc, 139, 3, "MethodWithStructs", ctx, locals_fn
          : (locals) => {
            Assert.Equal(4, locals.Count());
            CheckObject(locals, "m", "Math", subtype : "null");
            CheckObject(locals, "ss", "Math.SimpleStruct", subtype : "null");
            CheckObject(locals, "gs", "Math.GenericStruct<Math>", subtype
                        : "null");
            CheckArray(locals, "ss_arr", "Math.SimpleStruct[]");
          });
    });
  ...
  EvaluateAndCheck(string expression, string script_loc, int line, int column,
                   string function_name, DebugTestContext ctx,
                   Func<JObject, Task>wait_for_event_fn = null,
                   Action<JToken>locals_fn = null) =>
      await SendCommandAndCheck(JObject.FromObject(new {
                                    expression = expression}),
                                "Runtime.evaluate", script_loc, line, column,
                                function_name, ctx, wait_for_event_fn
                                : wait_for_event_fn, locals_fn
                                : locals_fn);

  async Task<JObject>
  SendCommandAndCheck(JObject args, string method, string script_loc, int line,
                      int column, string function_name, DebugTestContext ctx,
                      Func<JObject, Task>wait_for_event_fn = null,
                      Action<JToken>locals_fn = null,
  ...
  async Task CheckLocalsOnFrame(JToken frame, string script_loc, int line,
                                int column, string function_name,
                                DebugTestContext ctx,
  ...
  async Task CheckLocalsOnFrame(JToken frame, DebugTestContext ctx,
                                Action<JToken>test_fn = null) {
    var get_prop_req =
        JObject.FromObject(new {objectId = frame["callFrameId"]});
    ...
    if (!frame_props.IsOk)
      Assert.True(
          false,
          $"Runtime.getProperties failed for {get_prop_req.ToString ()}");
  ...
  async Task<Result>SetBreakpoint(string url_key, int line, int column,
                                  DebugTestContext ctx, bool expect_ok = true) {
    var bp1_req = JObject.FromObject(new {
        lineNumber = line,
        columnNumber = column,
