...
class Inspector {
  ...
  public async Task
  Ready(Func<InspectorClient, CancellationToken, Task>cb = null,
        TimeSpan? span = null) {
    using(var cts = new CancellationTokenSource()) {
      cts.CancelAfter(span?.Milliseconds ?? 60 *
                      1000); // tests have 1 minute to complete by default
      var uri = new Uri(
      ...
      using(var client =
                new InspectorClient(loggerFactory.CreateLogger<Inspector>())) {
        await client.Connect(uri, OnMessage, async token => {
          Task[] init_cmds = {
              ...
              client.SendCommand("Runtime.runIfWaitingForDebugger", null,
                                 token),
              WaitFor(READY),
          };
...
public class DebuggerTestBase {
  ...
  public DebuggerTestBase(string driver = "debugger-driver.html") {
    startTask =
        TestHarnessProxy.Start(FindChromePath(), FindTestPath(), driver);
  }
  ...
  internal async Task CheckInspectLocalsAtBreakpointSite(
      string url_key, int line, int column, string function_name,
      string eval_expression, Action<JToken>test_fn = null,
      Func<JObject, Task>wait_for_event_fn = null, bool use_cfo = false) {
  ...
  internal void CheckLocation(string script_loc, int line, int column,
  ...
  internal JToken CheckObject(JToken locals, string name, string class_name,
  ...
  internal async Task CheckDateTime(JToken locals, string name,
                                    DateTime expected) {
  ...
  internal async Task CheckDateTimeValue(JToken value, DateTime expected) {
    AssertEqual("System.DateTime", value["className"]?.Value<string>(),
                "className");
    AssertEqual(expected.ToString(), value["description"]?.Value<string>(),
                "className");
  ...
  internal JToken CheckValueType(JToken locals, string name,
                                 string class_name) {
    var l = GetAndAssertObjectWithName(locals, name);
    var val = l["value"];
    Assert.Equal("object", val["type"]?.Value<string>());
  ...
  internal JToken CheckEnum(JToken locals, string name, string class_name,
                            string descr) {
    var l = GetAndAssertObjectWithName(locals, name);
    var val = l["value"];
    Assert.Equal("object", val["type"]?.Value<string>());
  ...
  internal async Task<JObject>
  StepAndCheck(StepKind kind, string script_loc, int line, int column,
               string function_name,
               Func<JObject, Task>wait_for_event_fn = null,
               Action<JToken>locals_fn = null, int times = 1) {
  ...
  internal async Task CheckDelegate(JToken locals, string name,
                                    string className, string target) {
    var l = GetAndAssertObjectWithName(locals, name);
    var val = l["value"];
  ...
  internal async Task CheckDelegate(JToken actual_val, JToken exp_val,
                                    string label) {
    AssertEqual("object", actual_val["type"]?.Value<string>(), $"{label}-type");
    AssertEqual(exp_val["className"]?.Value<string>(),
                actual_val["className"]?.Value<string>(), $"{label}-className");
    ...
    void CheckDelegateTarget(string actual_target, string exp_target) {
      var parts = exp_target.Split(new char[]{'|'},
      ...
      if (parts.Length == 1) {
        ...
        bool prefix =
            actual_target.StartsWith(parts[0], StringComparison.Ordinal);
        Assert.True(
            prefix,
  ...
  internal async Task CheckCustomType(JToken actual_val, JToken exp_val,
                                      string label) {
    ...
    case "pointer": {
      AssertEqual("symbol", actual_val["type"]?.Value<string>(),
                  $"{label}-type");

      if (exp_val["is_null"]?.Value<bool>() == false) {
        var exp_prefix = $"({exp_val [" type_name "]?.Value<string>()})";
        AssertStartsWith(exp_prefix, actual_val["value"]?.Value<string>(),
                         $"{label}-type_name");
        AssertStartsWith(exp_prefix, actual_val["description"]?.Value<string>(),
                         $"{label}-description");
      } else {
        var exp_prefix = $"({exp_val [" type_name "]?.Value<string>()}) 0";
        AssertEqual(exp_prefix, actual_val["value"]?.Value<string>(),
                    $"{label}-type_name");
        AssertEqual(exp_prefix, actual_val["description"]?.Value<string>(),
                    $"{label}-description");
      }
      break;
    }
  ...
  internal async Task CheckProps(JToken actual, object exp_o, string label,
                                 int num_fields = -1) {
    if (exp_o.GetType().IsArray || exp_o is JArray) {
      ...
      for (int i = 0; i < exp_v_arr.Count; i++) {
        ...
        AssertEqual(i.ToString(), act_i["name"]?.Value<string>(),
                    $"{label}-[{i}].name");
  ...
  internal async Task CheckValue(JToken actual_val, JToken exp_val,
                                 string label) {
    ...
    foreach(var jp in exp_val.Values<JProperty>()) {
      if (jp.Value.Type == JTokenType.Object) {
        var new_val =
            await GetProperties(actual_val["objectId"].Value<string>());
        await CheckProps(new_val, jp.Value,
                         $"{label}-{actual_val[" objectId
                         "]?.Value<string>()}");
  ...
  internal async Task<JToken>GetLocalsForFrame(JToken frame, string script_loc,
                                               int line, int column,
                                               string function_name) {
  ...
  internal async Task<JToken>CompareObjectPropertiesFor(JToken locals,
  ...
  internal async Task<JToken>GetProperties(string id, JToken fn_args = null) {
    if (ctx.UseCallFunctionOnBeforeGetProperties &&
        !id.StartsWith("dotnet:scope:")) {
      ...
      var result = await ctx.cli.SendCommand("Runtime.callFunctionOn", cfo_args,
                                             ctx.token);
      AssertEqual(
          true, result.IsOk,
          $"Runtime.getProperties failed for {cfo_args.ToString ()}, with Result: {result}");
      id = result
               .Value ["result"]?
               ["objectId"]?
               .Value<string>();
    ...
    // FIXME: Should be done when generating the list in library_mono.js, but
    // not sure yet
    //        whether to remove it, and how to do it correctly.
    if (locals is JArray) {
      foreach(var p in locals) {
        if (p["name"]?.Value<string>() == "length" &&
  ...
  internal async Task<Result>SetBreakpoint(string url_key, int line, int column,
                                           bool expect_ok = true) {
  ...
  internal async Task<Result>SetBreakpointInMethod(string assembly, string type,
                                                   string method) {
  ...
  internal void
  AssertStartsWith(string expected, string actual, string label) => Assert.True(
      actual?.StartsWith(expected),
      $"[{label}] Does not start with the expected string\nExpected: {expected}\nActual:  {actual}");

  internal static Func<int, int, string, string, object>TSimpleClass =
      (X, Y, Id, Color) =>
  ...
  internal static Func<int, int, string, string, object>TPoint =
      (X, Y, Id, Color) => new {
          X = TNumber(X),
          Y = TNumber(Y),
          Id = TString(Id),
  ...
  internal static JObject TString(string value) => value == null
    ?
                                                            TObject("string",
                                                                    is_null
                                                                    : true)
  ...
  internal static JObject TNumber(int value) => JObject.FromObject(new {
      type = "number", value = @value.ToString(),
      description = value.ToString()});

  internal static JObject
  TValueType(string className, string description = null,
  ...
  internal static JObject TEnum(string className, string descr,
                                object members = null) =>
  ...
  internal static JObject TArray(string className,
                                 int length = 0) => JObject.FromObject(new {
      type = "object", className = className,
      description = $"{className}({length})", subtype = "array"});

  internal static JObject TBool(bool value) => JObject.FromObject(new {
    type = "boolean",
    value = @value,
    description = @value ? "true" : "false"
  });

  internal static JObject TSymbol(string value) => JObject.FromObject(new {
  ...
  /*
      ...
      .. pass target "as `target: "void
     <ActionTSignatureTest>|(Math.GenericStruct<int[]>)"`
  */
  internal static JObject TDelegate(string className,
                                    string target) => JObject.FromObject(new {
      __custom_type = "delegate", className = className, target = target});

  internal static JObject
  TPointer(string type_name, bool is_null = false) => JObject.FromObject(new {
      __custom_type = "pointer", type_name = type_name, is_null = is_null});

  internal static JObject TIgnore() => JObject.FromObject(new {
      __custom_type = "ignore_me"});
}
