...
namespace WebAssembly.Net.Debugging {
  internal class DevToolsClient : IDisposable {
    ...
    public DevToolsClient(ILogger logger) { this.logger = logger; }

    ~DevToolsClient() { Dispose(false); }
    ...
    Task Pump(Task task, CancellationToken token) {
      ...
      if (pending_writes.Count > 0) {
        current_write =
            socket.SendAsync(new ArraySegment<byte>(pending_writes[0]),
    ...
    async Task<string>ReadOne(CancellationToken token) {
      ...
      while (true) {
        var result =
            await this.socket.ReceiveAsync(new ArraySegment<byte>(buff), token);
    ...
    async Task MarkCompleteAfterward(Func<CancellationToken, Task>send,
    ...
    ConnectWithMainLoops(Uri uri, Func<string, CancellationToken, Task>receive,
