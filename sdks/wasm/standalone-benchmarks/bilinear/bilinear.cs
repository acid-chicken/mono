...
public partial class Benchmark {
  ...
  private static double[] BilinearInterpol(double[] x, double[] z, double[] A,
                                           double minXA, double maxXA,
                                           double[] B, double minXB,
                                           double maxXB, double weightB) {
  ...
  private double[] BilinearInterpol_Vector(double[] x, double[] A, double minXA,
                                           double maxXA, double[] B,
                                           double minXB, double maxXB,
                                           double weightB) {
    ...
    for (var i = 0; i < x.Length; i += Vector<double>.Count) {
      ...
      // Determine the largest a, such that A[i] = f(xA) and xA <= x[i].
      // This involves casting from double to int; here we use two Vector
      // conversions.
      ...
      // Now, compute the lambda for our A reference point.
      Vector<double>currentXNormA =
          Vector.Max(vMinXA, Vector.Min(currentX, vMaxXA));
      Vector<double>lambdaA = (currentXNormA - xA) * vInvDeltaA;

      // Now, we need to load up our reference points.
      // This is basically a "gather" operation, for which we use a temporary
      // double array.
      ...
      Vector<double>currentXNormB =
          Vector.Max(vMinXB, Vector.Min(currentX, vMaxXB));
  ...
  private static unsafe double[] BilinearInterpol_AVX(
      double[] x, double[] A, double minXA, double maxXA, double[] B,
      double minXB, double maxXB, double weightB) {
    double[] z = new double[outputVectorSize];
    ...
    fixed(double *pX = &x[0], pA = &A[0], pB = &B[0], pZ = &z[0]) {
      ...
      for (var i = 0; i < x.Length; i += Vector256<double>.Count) {
        ...
        // Determine the largest a, such that A[i] = f(xA) and xA <= x[i].
        // This involves casting from double to int; here we use a Vector
        // conversion.
        Vector256<double>aDouble =
            Avx.Multiply(Avx.Subtract(currentX, vMinXA), vInvDeltaA);
        ...
        Vector256<double>xA = Avx.Add(
            Avx.Multiply(Avx.ConvertToVector256Double(a), vDeltaA), vMinXA);

        // Now, compute the lambda for our A reference point.
        Vector256<double>currentXNormA =
            Avx.Max(vMinXA, Avx.Min(currentX, vMaxXA));
        Vector256<double>lambdaA =
            Avx.Multiply(Avx.Subtract(currentXNormA, xA), vInvDeltaA);

        // Now, we need to load up our reference points using Vector Gather
        // operations.
        Vector256<double>AVector = Avx2.GatherVector256(pA, a, 8);
        Vector256<double>AVectorPlusOne = Avx2.GatherVector256(pA, aPlusOne, 8);

        // Now, do the all of the above for our B reference point.
        Vector256<double>bDouble =
            Avx.Multiply(Avx.Subtract(currentX, vMinXB), vInvDeltaB);
        ...
        Vector256<double>xB = Avx.Add(
            Avx.Multiply(Avx.ConvertToVector256Double(b), vDeltaB), vMinXB);
        Vector256<double>currentXNormB =
            Avx.Max(vMinXB, Avx.Min(currentX, vMaxXB));
        Vector256<double>lambdaB =
            Avx.Multiply(Avx.Subtract(currentXNormB, xB), vInvDeltaB);
        ...
        Vector256<double>newZ = Avx.Add(
            Avx.Multiply(
                vWeightA,
                Avx.Add(AVector,
                        Avx.Multiply(lambdaA,
                                     Avx.Subtract(AVectorPlusOne, AVector)))),
            Avx.Multiply(
                vWeightB,
                Avx.Add(BVector,
                        Avx.Multiply(lambdaB,
                                     Avx.Subtract(BVectorPlusOne, BVector)))));
        Avx.Store(pZ + i, newZ);
      }
    }
    return z;
  ...
  // This method is currently unused. It is useful, when generating a new
  // vectorized version of the benchmark (e.g. with a different set of HW
  // intrinsics), to ensure that it is correct (i.e. produces the same result as
  // the scalar version).
  //
  public static bool CheckResult(double[] output, double[] vectorOutput) {
    ...
    for (int i = 0; i < output.Length; i++) {
      if (Math.Abs(output[i] - vectorOutput[i]) > eps) {
        Console.WriteLine("Failed at " + i + ": output is " + output[i] +
                          ", vectorOutput is " + vectorOutput[i]);
        return false;
      }
    }
    return true;
  ...
  public double[] runIteration() {
    ...
    for (int i = 0; i < InnerIterationCount; i++)
      BilinearInterpol(input, output, A, minXA, maxXA, B, minXB, maxXB,
                       weightB);
  ...
  public double[] Interpol_Vector() {
    double[] vectorOutput = BilinearInterpol_Vector(input, A, minXA, maxXA, B,
                                                    minXB, maxXB, weightB);
    return vectorOutput;
  }
  ...
  public double[] Interpol_AVX() {
    if (Avx2.IsSupported) {
      double[] vectorOutput;
      vectorOutput = BilinearInterpol_AVX(input, A, minXA, maxXA, B, minXB,
                                          maxXB, weightB);
      return vectorOutput;
    }
    return null;
