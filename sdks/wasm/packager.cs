...
class Driver {
  ...
  int Run(string[] args) {
    ...
    var opts = new WasmOptions(){AddBinding = true,    Debug = false,
                                 DebugRuntime = false, Linker = false,
                                 ILStrip = true,       LinkerVerbose = false,
                                 EnableZLib = false,   EnableFS = false,
                                 NativeStrip = true,   Simd = false};
    ...
    if (is_netcore)
      wasm_runtime_dir = Path.Combine(tool_prefix, use_release_runtime ?
                                      "builds/netcore-release"
                                      : "builds/netcore-debug");
      ...
      wasm_runtime_dir =
          Path.Combine(tool_prefix, use_release_runtime ? "builds/release"
                       : "builds/debug");
    if (!emit_ninja) {
    ...
    ninja.WriteLine(
        "  command = echo > aot-dummy.cs; csc /deterministic /out:$out /target:library aot-dummy.cs");
    ...
    if (build_wasm) {
      ...
      ninja.WriteLine(
          "build $appdir/mono.js: cpifdiff $wasm_runtime_dir/mono.js");
      ninja.WriteLine(
          "build $appdir/mono.wasm: cpifdiff $wasm_runtime_dir/mono.wasm");
      if (enable_threads) {
        ninja.WriteLine(
    ...
    if (enable_dedup) {
       ...
       * mono --aot=<args>,dedup-include=aot-dummy.dll <assemblies>
       * aot-dummy.dll This will process all assemblies and emit all instances
       * into the aot image of aot-dummy.dll
       */
      var a = dedup_asm;
      /*
    ...
    if (build_wasm) {
      ...
      ninja.WriteLine(
          $"build $appdir/mono.js $appdir/mono.wasm: emcc-link $builddir/driver.o {zlibhelper} {wasm_core_bindings} {ofiles} {profiler_libs} {extra_link_libs} {runtime_libs} | $tool_prefix/src/library_mono.js $tool_prefix/src/dotnet_support.js {wasm_core_support} $emsdk_env");
