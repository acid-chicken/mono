...
class Driver {
  ...
  int Run(string[] args) {
    ...
    AddFlag(p,
            new BoolFlag("collation", "enable unicode collation support",
                         opts.EnableCollation, b => opts.EnableCollation = b));
    ...
    {
      foreach(var file in new string[]{"linker-subs.xml",
                                       "linker-disable-collation.xml",
                                       "linker-preserves.xml"}) {
        var source_file =
            Path.GetFullPath(Path.Combine(tool_prefix, "src", file));
