...
class BoolFlag : Flag {
  public BoolFlag(string name, string description, bool def_value,
                  Action<bool>action)
...
class Driver {
  ...
  static string app_prefix, framework_prefix, bcl_tools_prefix,
      bcl_facades_prefix, out_prefix;
  ...
  static void Usage() {
    ...
    foreach(var flag in option_list) {
      if (flag is BoolFlag) {
        ...
        Console.WriteLine("        type: bool  default: " +
                          ((flag as BoolFlag).DefaultValue ? "true"
                           : "false"));
      }
    }
  }
  ...
  static string FindFrameworkAssembly(string asm) { return asm; }
  ...
  static bool is_sdk_assembly(string filename) {
    foreach(var prefix in bcl_prefixes) if (filename.StartsWith(
                                                prefix)) return true;
    return false;
  }
  ...
  static void Import(string ra, AssemblyKind kind) {
    ...
    foreach(var prefix in bcl_prefixes) resolver.AddSearchDirectory(prefix);
    ...
    if (add_pdb &&
        (kind == AssemblyKind.User || kind == AssemblyKind.Framework)) {
  ...
  void GenDriver(string builddir, List<string>profilers, ExecMode ee_mode,
    ...
    foreach(var adata in assemblies) {
      if (adata.aot)
        symbols.Add(
            String.Format("mono_aot_module_{0}_info",
    ...
    foreach(var profiler in profilers) {
      ...
      w.WriteLine("EMSCRIPTEN_KEEPALIVE void mono_wasm_load_profiler_" +
                  profiler + " (const char *desc) { mono_profiler_init_" +
                  profiler + " (desc); }");
    }
  ...
  public static int Main(string[] args) { return new Driver().Run(args); }

  enum CopyType { Default, Always, IfNewer }

  enum ExecMode { Interp = 1, Aot = 2, AotInterp = 3 }

  enum LinkMode { SdkOnly, All }
  ...
  int Run(string[] args) {
    ...
    var opts = new WasmOptions(){AddBinding = true,
                                 Debug = false,
                                 DebugRuntime = false,
                                 Linker = false,
                                 ILStrip = true,
    ...
    AddFlag(p, new BoolFlag("debug", "enable c# debugging", opts.Debug,
                            b => opts.Debug = b));
    AddFlag(p, new BoolFlag("debugrt", "enable debug runtime",
                            opts.DebugRuntime, b => opts.DebugRuntime = b));
    AddFlag(p, new BoolFlag("linker", "enable the linker", opts.Linker,
                            b => opts.Linker = b));
    AddFlag(p, new BoolFlag("binding", "enable the binding engine",
                            opts.AddBinding, b => opts.AddBinding = b));
    AddFlag(p, new BoolFlag("link-icalls", "link away unused icalls",
                            opts.LinkIcalls, b => opts.LinkIcalls = b));
    AddFlag(p, new BoolFlag("il-strip",
                            "strip IL code from assemblies in AOT mode",
                            opts.ILStrip, b => opts.ILStrip = b));
    AddFlag(p, new BoolFlag("linker-verbose", "set verbose option on linker",
                            opts.LinkerVerbose, b => opts.LinkerVerbose = b));
    AddFlag(p, new BoolFlag(
                   "zlib",
                   "enable the use of zlib for System.IO.Compression support",
                   opts.EnableZLib, b => opts.EnableZLib = b));
    AddFlag(
        p,
        new BoolFlag(
            "enable-fs",
            "enable filesystem support (through Emscripten's file_packager.py in a later phase)",
            opts.EnableFS, b => opts.EnableFS = b));
    AddFlag(p, new BoolFlag("threads", "enable threads", opts.EnableThreads,
                            b => opts.EnableThreads = b));
    AddFlag(p,
            new BoolFlag(
                "dynamic-runtime",
                "enable dynamic runtime (support for Emscripten's dlopen)",
                opts.EnableDynamicRuntime, b => opts.EnableDynamicRuntime = b));
    AddFlag(p, new BoolFlag("native-strip", "strip final executable",
                            opts.NativeStrip, b => opts.NativeStrip = b));
    AddFlag(p, new BoolFlag("simd", "enable SIMD support", opts.Simd,
                            b => opts.Simd = b));
    AddFlag(p, new BoolFlag("linker-exclude-deserialization",
    ...
    if (enable_aot || link_icalls || gen_pinvoke || profilers.Count > 0 ||
        native_libs.Count > 0 || preload_files.Count > 0 ||
        embed_files.Count > 0) {
      build_wasm = true;
      emit_ninja = true;
    }
    ...
    if (ee_mode != ExecMode.Aot)
      // Can't strip out IL code in mixed mode, since the interpreter might
      // execute some methods even if they have AOTed code available
      il_strip = false;

    if (aot_assemblies != "") {
      if (ee_mode != ExecMode.AotInterp) {
        Console.Error.WriteLine(
            "The --aot-assemblies= argument requires --aot-interp.");
        return 1;
      }
    }
    if (link_icalls && !enable_linker) {
      Console.Error.WriteLine(
          "The --link-icalls option requires the --linker option.");
      return 1;
    }
    if (framework != "") {
      if (framework.StartsWith("netcoreapp")) {
        ...
        if (netcore_sdkdir == "") {
          Console.Error.WriteLine(
              "The --netcore-sdkdir= argument is required.");
          return 1;
        }
        if (!Directory.Exists(netcore_sdkdir)) {
          Console.Error.WriteLine(
              $"The directory '{netcore_sdkdir}' doesn't exist.");
          return 1;
        }
      } else {
        Console.Error.WriteLine(
            "The only valid value for --framework is 'netcoreapp...'");
        return 1;
      }
    }
    ...
    if (sdkdir != null) {
      framework_prefix = Path.Combine(
          tool_prefix, "framework"); // all framework assemblies are currently
                                     // side built to packager.exe
    } else if (Directory.Exists(
                   Path.Combine(tool_prefix, "../out/wasm-bcl/wasm"))) {
      framework_prefix = Path.Combine(
          tool_prefix, "framework"); // all framework assemblies are currently
                                     // side built to packager.exe
    ...
    if (enable_aot) {
      ...
      if (aot_assemblies != "") {
        foreach(var s in aot_assemblies.Split(',')) to_aot[s] = true;
      }
      ...
      if (to_aot.Count > 0) {
        Console.Error.WriteLine("Unknown assembly name '" +
    ...
    var runtime_js = Path.Combine(emit_ninja ? builddir
                                  : out_prefix, "runtime.js");
    if (emit_ninja) {
      File.Delete(runtime_js);
      File.Copy(runtimeTemplate, runtime_js);
    } else {
      if (File.Exists(runtime_js) && (File.Exists(runtimeTemplate))) {
        CopyFile(runtimeTemplate, runtime_js, CopyType.IfNewer,
    ...
    var config = String.Format(
        "config = {{\n \tvfs_prefix: \"{0}\",\n \tdeploy_prefix: \"{1}\",\n \tenable_debugging: {2},\n \tfile_list: [ {3} ],\n",
        vfs_prefix, deploy_prefix, enable_debug ? "1"
        : "0", file_list_str);
    config += "}\n";
    var config_js = Path.Combine(emit_ninja ? builddir
                                 : out_prefix, "mono-config.js");
    ...
    if (is_netcore)
      wasm_runtime_dir =
          Path.Combine(wasm_runtime_path, use_release_runtime ? "netcore-release"
                       : "netcore-debug");
    else if (enable_threads)
      wasm_runtime_dir =
          Path.Combine(wasm_runtime_path, use_release_runtime ? "threads-release"
                       : "threads-debug");
    else if (enable_dynamic_runtime)
      wasm_runtime_dir =
          Path.Combine(wasm_runtime_path, use_release_runtime ? "dynamic-release"
                       : "dynamic-debug");
    else
      wasm_runtime_dir =
          Path.Combine(wasm_runtime_path, use_release_runtime ? "release"
                       : "debug");
    if (!emit_ninja) {
      ...
      foreach(var fname in interp_files) {
        ...
        File.Copy(Path.Combine(wasm_runtime_dir, fname),
                  Path.Combine(out_prefix, fname));
      }

      foreach(var asset in assets) {
        CopyFile(asset, Path.Combine(out_prefix, Path.GetFileName(asset)),
                 copyType, "Asset: ");
      }
    }
    ...
    foreach(var a in assemblies) {
      ...
      if (filenames.ContainsKey(filename)) {
        Console.WriteLine("Duplicate input assembly: " + assembly + " " +
                          filenames[filename]);
        return 1;
      }
    ...
    string runtime_dir = is_netcore ? "$mono_sdkdir/wasm-runtime-netcore-release"
        : "$mono_sdkdir/wasm-runtime-release";
    string runtime_libdir = $"{runtime_dir}/lib";

    string runtime_libs = "";
    if (ee_mode == ExecMode.Interp || ee_mode == ExecMode.AotInterp ||
        link_icalls) {
      runtime_libs +=
          $"$runtime_libdir/libmono-ee-interp.a $runtime_libdir/libmono-ilgen.a ";
      // We need to link the icall table because the interpreter uses it to
      // lookup icalls even if the aot-ed icall wrappers are available
      if (!link_icalls)
    ...
    foreach(var lib in native_libs) extra_link_libs += lib + " ";
    if (aot_profile != null) {
      CopyFile(aot_profile,
    ...
    foreach(var pf in preload_files) emcc_flags += "--preload-file " + pf + " ";
    foreach(var f in embed_files) emcc_flags += "--embed-file " + f + " ";
    ...
    if (add_binding) {
      ...
      ninja.WriteLine(
          $"wasm_core_support_library = {wasm_core_support_library}");
    } else {
    ...
    if (is_netcore)
      ninja.WriteLine(
          "cross = $mono_sdkdir/wasm-cross-netcore-release/bin/wasm32-unknown-none-mono-sgen");
    else
      ninja.WriteLine(
    ...
    ninja.WriteLine(
        "  command = bash -c '$emcc $emcc_flags $flags -c -o $out $in'");
    ...
    ninja.WriteLine(
        "  command = mono $tools_dir/monolinker.exe -out $builddir/linker-out -l none --deterministic --disable-opt unreachablebodies --exclude-feature com --exclude-feature remoting --exclude-feature etw $linker_args || exit 1; mono $tools_dir/wasm-tuner.exe --gen-empty-assemblies $out");
    ...
    ninja.WriteLine(
        "  command = cp $in $out; mono $tools_dir/mono-cil-strip.exe -q $out");
    ...
    ninja.WriteLine(
        "build $appdir/mono-config.js: cpifdiff $builddir/mono-config.js");
    if (build_wasm) {
      var source_file =
          Path.GetFullPath(Path.Combine(tool_prefix, "src", "driver.c"));
      ninja.WriteLine($"build $builddir/driver.c: cpifdiff {source_file}");
      ninja.WriteLine(
          $"build $builddir/driver-gen.c: cpifdiff $builddir/driver-gen.c.in");

      var pinvoke_file_name = is_netcore ? "pinvoke-tables-default-netcore.h"
          : "pinvoke-tables-default.h";
      var pinvoke_file =
          Path.GetFullPath(Path.Combine(tool_prefix, "src", pinvoke_file_name));
      ninja.WriteLine(
          $"build $builddir/{pinvoke_file_name}: cpifdiff {pinvoke_file}");
      ...
      if (add_binding) {
        var bindings_source_file = Path.GetFullPath(
            Path.Combine(tool_prefix, "src", "corebindings.c"));
        ninja.WriteLine(
            $"build $builddir/corebindings.c: cpifdiff {bindings_source_file}");
      ...
      if (enable_zlib) {
        var zlib_source_file =
            Path.GetFullPath(Path.Combine(tool_prefix, "src", "zlib-helper.c"));
        ninja.WriteLine(
            $"build $builddir/zlib-helper.c: cpifdiff {zlib_source_file}");

        ninja.WriteLine(
            $"build $builddir/zlib-helper.o: emcc $builddir/zlib-helper.c | $emsdk_env");
        ninja.WriteLine(
            $"  flags = -s USE_ZLIB=1 -I{runtime_dir}/include/mono-2.0");
      }
    } else {
      ninja.WriteLine(
          "build $appdir/dotnet.js: cpifdiff $wasm_runtime_dir/dotnet.js");
      ninja.WriteLine(
          "build $appdir/dotnet.wasm: cpifdiff $wasm_runtime_dir/dotnet.wasm");
      if (enable_threads) {
        ninja.WriteLine(
            "build $appdir/mono.worker.js: cpifdiff $wasm_runtime_dir/mono.worker.js");
      }
    }
    if (enable_aot)
    ...
    {
      var source_file =
          Path.GetFullPath(Path.Combine(tool_prefix, "src", "linker-subs.xml"));
      ninja.WriteLine(
          $"build $builddir/linker-subs.xml: cpifdiff {source_file}");
    }
    var ofiles = "";
    var bc_files = "";
    ...
    foreach(var a in assemblies) {
      ...
      if (enable_linker) {
        ...
        ninja.WriteLine(
            $"build $builddir/{filename}: cpifdiff {source_file_path}");
      ...
      if (il_strip) {
        ninja.WriteLine(
            $"build $builddir/ilstrip-out/{filename} : ilstrip {infile}");
        a.final_path = $"$builddir/ilstrip-out/{filename}";
      }

      ninja.WriteLine(
          $"build $appdir/$deploy_prefix/{filename}: cpifdiff {a.final_path}");
      ...
      if (a.aot) {
        ...
        if (filename == "mscorlib.dll") {
          // mscorlib has no dependencies so we can skip the aot step if the
          // input didn't change The other assemblies depend on their references
    ...
    if (enable_dedup) {
       ...
       * mono --aot=<args>,dedup-include=aot-instances.dll <assemblies>
       * aot-instances.dll This will process all assemblies and emit all
       * instances into the aot image of aot-instances.dll
       */
      var a = dedup_asm;
      /*
       * The dedup process will read in the .dedup files created when running
       * with dedup-skip, so add all the .bc files as dependencies.
       */
      ninja.WriteLine(
          $"build {a.bc_path}.tmp: aot-instances | {bc_files} {a.linkout_path}");
    ...
    if (link_icalls) {
      ...
      ninja.WriteLine(
          "build $builddir/icall-table.json: gen-runtime-icall-table");
      ninja.WriteLine(
          $"build $builddir/icall-table.h: gen-icall-table {icall_assemblies}");
      ninja.WriteLine($"  runtime_table=$builddir/icall-table.json");
    }
    if (gen_pinvoke) {
      string pinvoke_assemblies = "";
      foreach(var a in assemblies) pinvoke_assemblies += $"{a.linkout_path} ";
      ninja.WriteLine(
          $"build $builddir/pinvoke-table.h: gen-pinvoke-table {pinvoke_assemblies}");
      ninja.WriteLine($"  pinvoke_libs=System.Native,{pinvoke_libs}");
    }
    if (build_wasm) {
      string zlibhelper = enable_zlib ? "$builddir/zlib-helper.o" : "";
      ninja.WriteLine(
          $"build $appdir/dotnet.js $appdir/dotnet.wasm: emcc-link $builddir/driver.o {zlibhelper} {wasm_core_bindings} {ofiles} {profiler_libs} {extra_link_libs} {runtime_libs} | $tool_prefix/src/library_mono.js $tool_prefix/src/dotnet_support.js {wasm_core_support} $emsdk_env");
  ...
  static void CopyFile(string sourceFileName, string destFileName,
                       CopyType copyType, string typeFile = "") {
    Console.WriteLine(
        $"{typeFile}cp: {copyType} - {sourceFileName} -> {destFileName}");
    switch (copyType) {
    ...
    case CopyType.IfNewer:
      if (!File.Exists(destFileName)) {
        ...
        if (srcInfo.LastWriteTime.Ticks > dstInfo.LastWriteTime.Ticks ||
