...
namespace WebAssembly.Net.WebSockets {
  ...
  public sealed class ClientWebSocket : WebSocket {
    ...
    private ActionQueue<ReceivePayload>receiveMessageQueue =
        new ActionQueue<ReceivePayload>();
    ...
    /// cref="T:WebAssembly.Net.WebSockets.ClientWebSocket"/> class.
    /// </summary>
...
#region Properties
    ...
    /// <summary>
    /// Gets the WebSocket state of the <see
    /// cref="T:WebAssembly.Net.WebSockets.ClientWebSocket"/> instance.
    /// </summary>
    ...
    /// <summary>
    /// Gets the reason why the close handshake was initiated on <see
    /// cref="T:WebAssembly.Net.WebSockets.ClientWebSocket"/> instance.
    /// </summary>
    ...
    /// <summary>
    /// Gets a description of the reason why the <see
    /// cref="T:WebAssembly.Net.WebSockets.ClientWebSocket"/> instance was
    /// closed.
    /// </summary>
    ...
    /// <summary>
    /// Gets the supported WebSocket sub-protocol for the <see
    /// cref="T:WebAssembly.Net.WebSockets.ClientWebSocket"/>s instance.
    /// </summary>
    ...
    public Task ConnectAsync(Uri uri, CancellationToken cancellationToken) {
      ...
      int priorState =
          Interlocked.CompareExchange(ref state, connecting, created);
    ...
    private async Task
    ConnectAsyncJavaScript(Uri uri, CancellationToken cancellationToken) {
      var tcsConnect = new TaskCompletionSource<bool>();

      // For Abort/Dispose.  Calling Abort on the request at any point will
      // close the connection.
      cts.Token.Register(AbortRequest);

      // Wrap the cancellationToken in a using so that it can be disposed of
      // whether we successfully connected or failed trying. Otherwise any
      // timeout/cancellation would apply to the full session. In the failure
      // case we need to release the references and dispose of the objects.
      using(cancellationToken.Register(() => tcsConnect.TrySetCanceled())) {
        try {
          ...
          innerWebSocket =
              new HostObject("WebSocket", uri.ToString(), subProtocols);

          subProtocols?.Dispose();

          // Setup the onError callback
          onError = new Action<JSObject>((errorEvt) => { errorEvt.Dispose(); });
          ...
          onClose = new Action<JSObject>((closeEvt) => {
            innerWebSocketCloseStatus =
                (WebSocketCloseStatus) closeEvt.GetObjectProperty("code");
            innerWebSocketCloseStatusDescription =
                closeEvt.GetObjectProperty("reason") ?.ToString();
            var mess = new ReceivePayload(WebSocketHelpers.EmptyPayload,
                                          WebSocketMessageType.Close);
            receiveMessageQueue.BufferPayload(mess);

            if (!tcsConnect.Task.IsCanceled && !tcsConnect.Task.IsCompleted &&
          ...
          onOpen = new Action<JSObject>((evt) => {
            if (!cancellationToken.IsCancellationRequested) {
              // Change internal state to 'connected' to enable the other
              // methods
              if (Interlocked.CompareExchange(ref state, connected,
          ...
          onMessage = new Action<JSObject>((messageEvent) => {
            ...
            // If the messageEvent's data property is marshalled as a JSObject
            // then we are dealing with binary data
            if (eventData is JSObject) {
              // TODO: Handle ArrayBuffer binary type but have only seen 'blob'
              // so far without changing the default websocket binary type
              // manually.
              if (innerWebSocket.GetObjectProperty("binaryType").ToString() ==
                  "blob") {
                ...
                using(var reader = new HostObject("FileReader")) {
                  loadend = new Action<JSObject>((loadEvent) => {
                    using(var target = (JSObject) loadEvent.GetObjectProperty(
                              "target")) {
                      if ((int) target.GetObjectProperty("readyState") == 2) {
                        using(var binResult =
                                  (ArrayBuffer)
                                      target.GetObjectProperty("result")) {
                          var mess = new ReceivePayload(
                              binResult, WebSocketMessageType.Binary);
                          receiveMessageQueue.BufferPayload(mess);
                  ...
                  using(var blobData =
                            (JSObject) messageEvent.GetObjectProperty("data"))
              ...
              var mess = new ReceivePayload(
                  Encoding.UTF8.GetBytes(((string) eventData).ToString()),
          ...
          WebSocketException wex =
              new WebSocketException("WebSocket connection failure.", wse);
          throw wex;
        }
      }
    }

    private void ConnectExceptionCleanup() { Dispose(); }
    ...
    // This method is registered by the CancellationTokenSource cts in the
    // connect method and called by Dispose or Abort so that any open websocket
    // connection can be closed.
    private void AbortRequest() {
      if (State == WebSocketState.Open) {
        var closeResult =
            CloseAsyncCore(WebSocketCloseStatus.NormalClosure,
                           "Connection was aborted", CancellationToken.None);
      }
    }

    /// <summary>
    /// Send data on <see cref="T:WebAssembly.Net.WebSockets.ClientWebSocket"/>
    ...
    public override async Task SendAsync(ArraySegment<byte>buffer,
      ...
      // Wrap the cancellationToken in a using so that it can be disposed of
      // whether we successfully send or not. Otherwise any timeout/cancellation
      // would apply to the full session.
      using(cancellationToken.Register(() => tcsSend.TrySetCanceled())) {
        ...
        if (messageType == WebSocketMessageType.Text) {
          ...
          try {
            ...
            Buffer.BlockCopy(buffer.Array, buffer.Offset, bytesToSend, 0,
                             buffer.Count);

            var strBuffer =
                Encoding.UTF8.GetString(bytesToSend, 0, bytesToSend.Length);
    ...
    /// <summary>
    /// Receives data on <see
    /// cref="T:WebAssembly.Net.WebSockets.ClientWebSocket"/> as an asynchronous
    /// operation.
    /// </summary>
    /// <returns>The async.</returns>
    /// <param name="buffer">Buffer.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    public override async Task<WebSocketReceiveResult>
    ReceiveAsync(ArraySegment<byte>buffer,
      ...
      // Wrap the cancellationToken in a using so that it can be disposed of
      // whether we successfully receive or not. Otherwise any
      // timeout/cancellation would apply to the full session.
      using(cancellationToken.Register(() => tcsReceive.TrySetCanceled())) {

        if (bufferedPayload == null)
          bufferedPayload =
              await receiveMessageQueue.DequeuePayloadAsync(cancellationToken);

        try {
          var endOfMessage = bufferedPayload.BufferPayload(
              buffer, out WebSocketReceiveResult receiveResult);
    ...
    private async Task CloseAsyncCore(WebSocketCloseStatus closeStatus,
                                      string statusDescription,
                                      CancellationToken cancellationToken) {
      ThrowOnInvalidState(State, WebSocketState.Open,
    ...
    public sealed class ClientWebSocketOptions {
      private bool isReadOnly; // After ConnectAsync is called the options
                               // cannot be modified.
...
#region HTTP Settings
      ...
      public System.Net.Security.RemoteCertificateValidationCallback
...
#region WebSocket Settings
      ...
      public void AddSubProtocol(string subProtocol) {
        ...
        foreach(string item in requestedSubProtocols) {
          if (string.Equals(item, subProtocol,
...
#region Helpers
      ...
      internal void SetToReadOnly() { isReadOnly = true; }

      private void ThrowIfReadOnly() {
        if (isReadOnly) {
          throw new InvalidOperationException(
              "WebSocket has already been started.");
        }
      }
