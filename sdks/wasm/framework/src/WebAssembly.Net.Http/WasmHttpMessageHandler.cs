...
namespace WebAssembly.Net.Http.HttpClient {
  public class WasmHttpMessageHandler : HttpMessageHandler {
    ...
    /// <summary>
    /// Gets or sets the default value of the 'credentials' option on outbound
    /// HTTP requests. Defaults to <see
    /// cref="FetchCredentialsOption.SameOrigin"/>.
    /// </summary>
    ...
    public static RequestCache Cache {
      get;
      set;
    }
    = RequestCache.Default;

    public static RequestMode Mode {
      get;
      set;
    }
    = RequestMode.Cors;
    ...
    public WasmHttpMessageHandler() { handlerInit(); }
    ...
    SendAsync(HttpRequestMessage request, CancellationToken cancellationToken) {
      // There is a race condition on Safari as a result of using
      // TaskCompletionSource that causes a stack exceeded error being thrown.
...
#pragma warning restore 4014
    ...
    private async Task doFetch(TaskCompletionSource<HttpResponseMessage>tcs,
      ...
      try {
        ...
        // for standard values and meanings
        ...
        if (request.Content != null) {
          if (request.Content is StringContent) {
            ...
            // using (var uint8Buffer = Uint8Array.From(await
            // request.Content.ReadAsByteArrayAsync ())) so we split it up into
            // two lines.
        ...
        JSObject signal =
            (JSObject) abortController.GetObjectProperty("signal");
        ...
        CancellationTokenRegistration abortRegistration =
            abortCts.Token.Register((Action)(() => {
        ...
        HttpResponseMessage httpresponse =
            new HttpResponseMessage((HttpStatusCode) Enum.Parse(
                typeof (HttpStatusCode), status.Status.ToString()));

        httpresponse.Content =
            StreamingSupported &&
            StreamingEnabled
                                   ? new StreamContent(
                wasmHttpReadStream = new WasmHttpReadStream(status))
            : (HttpContent) new WasmHttpContent(status);

        // Fill the response headers
        // CORS will only allow access to certain headers.
        // If a request is made for a resource on another origin which returns
        // the CORs headers, then the type is cors. cors and basic responses are
        // almost identical except that a cors response restricts the headers
        // you can view to `Cache-Control`, `Content-Language`, `Content-Type`,
        // `Expires`, `Last-Modified`, and `Pragma`. View more information
        // https://developers.google.com/web/updates/2015/03/introduction-to-fetch#response_types
        //
        // Note: Some of the headers may not even be valid header types in .NET
        // thus we use TryAddWithoutValidation
        using(var respHeaders = (JSObject) status.Headers) {
          if (respHeaders != null) {
            using(var entriesIterator =
                      (JSObject) respHeaders.Invoke("entries")) {
    ...
    class WasmFetchResponse : IDisposable {
      ...
      public WasmFetchResponse(
          JSObject fetchResponse, JSObject abortController,
          CancellationTokenSource abortCts,
          CancellationTokenRegistration abortRegistration) {
      ...
      public bool IsRedirected =>
          (bool) fetchResponse.GetObjectProperty("redirected");
      public int Status => (int) fetchResponse.GetObjectProperty("status");
      public string StatusText =>
          (string) fetchResponse.GetObjectProperty("statusText");
      public string ResponseType =>
          (string) fetchResponse.GetObjectProperty("type");
      public string Url => (string) fetchResponse.GetObjectProperty("url");
      // public bool IsUseFinalURL =>
      // (bool)managedJSObject.GetObjectProperty("useFinalUrl");
      public bool IsBodyUsed =>
          (bool) fetchResponse.GetObjectProperty("bodyUsed");
      public JSObject Headers =>
          (JSObject) fetchResponse.GetObjectProperty("headers");
      public JSObject Body =>
          (JSObject) fetchResponse.GetObjectProperty("body");

      public Task<object>
      ArrayBuffer() => (Task<object>) fetchResponse.Invoke("arrayBuffer");
    ...
    class WasmHttpContent : HttpContent {
      ...
      public WasmHttpContent(WasmFetchResponse status) { _status = status; }
      ...
      private async Task<byte[]>GetResponseData() {
        ...
        using(ArrayBuffer dataBuffer =
                  (ArrayBuffer) await _status.ArrayBuffer()) {
    ...
    class WasmHttpReadStream : Stream {
      ...
      public WasmHttpReadStream(WasmFetchResponse status) { _status = status; }
      ...
      ReadAsync(byte[] buffer, int offset, int count,
        ...
        try {
          ...
          using(var read = (JSObject) await t) {
            ...
            using(Uint8Array binValue = (Uint8Array) read.GetObjectProperty(
                      "value")) _bufferedBytes = binValue.ToArray();
          }
        } catch (JSException) {
          cancellationToken.ThrowIfCancellationRequested();
          throw;
  ...
  public enum FetchCredentialsOption {
        /// <summary>
        /// Advises the browser never to send credentials (such as cookies or
        /// HTTP auth headers).
            /// </summary>
            [Export(EnumValue = ConvertEnum.ToLower)] Omit,

            /// <summary>
            /// Advises the browser to send credentials (such as cookies or HTTP
            /// auth headers)
            /// only if the target URL is on the same origin as the calling
            /// application.
            /// </summary>
            [ Export("same-origin") ] SameOrigin,
            ...
            /// </summary>
            [ Export(EnumValue = ConvertEnum.ToLower) ] Include,
  }
  ...
  public enum RequestCache {
        ...
        /// </summary>
        [ Export(EnumValue = ConvertEnum.ToLower) ] Reload,

        /// <summary>
        /// The browser looks for a matching request in its HTTP cache.
        /// </summary>
        [ Export("no-cache") ] NoCache,

        /// <summary>
        /// The browser looks for a matching request in its HTTP cache.
        /// </summary>
        [ Export("force-cache") ] ForceCache,

        /// <summary>
        /// The browser looks for a matching request in its HTTP cache.
        /// Mode can only be used if the request's mode is "same-origin"
        /// </summary>
        [ Export("only-if-cached") ] OnlyIfCached,
  }

  /// <summary>
  /// The mode of the request. This is used to determine if cross-origin
  /// requests lead to valid responses
  /// </summary>
