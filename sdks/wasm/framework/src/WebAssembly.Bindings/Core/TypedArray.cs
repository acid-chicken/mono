...
namespace WebAssembly.Core {
  ...
  /// </summary>
  public abstract class TypedArray<T, U>
      : CoreObject, ITypedArray, ITypedArray<T, U>where U : struct {
    ...
    public void Fill(U value, int start, int end) => Invoke("fill", value,
                                                            start, end);
    ...
    public void Set(ITypedArray typedArray,
                    int offset) => Invoke("set", typedArray, offset);
    ...
    /// <summary>
    /// Gets or sets the <see cref="T:WebAssembly.Core.TypedArray`2"/> with the
    /// specified i.
    /// </summary>
    ...
    /// <summary>
    /// Copies the contents from the <see
    /// cref="T:WebAssembly.Core.TypedArray`2"/> memory into a new array.
    /// </summary>
    public U[] ToArray() {
    ...
    /// <summary>
    /// Creates a new <see cref="T:WebAssembly.Core.TypedArray`2"/> from the
    /// ReadOnlySpan.
    /// </summary>
    ...
    public unsafe static T From(ReadOnlySpan<U>span) {
      ...
      TypedArrayTypeCode type =
          (TypedArrayTypeCode) Type.GetTypeCode(typeof (U));
      // Special case for Uint8ClampedArray, a clamped array which represents an
      // array of 8-bit unsigned integers clamped to 0-255;
      if (type == TypedArrayTypeCode.Uint8Array &&
          typeof (T) == typeof (Uint8ClampedArray))
        type = TypedArrayTypeCode
                   .Uint8ClampedArray; // This is only passed to the JavaScript
                                       // side so it knows it will be a
                                       // Uint8ClampedArray

      var bytes = MemoryMarshal.AsBytes(span);
      fixed(byte *ptr = bytes) {
        var res = Runtime.TypedArrayFrom((int) ptr, 0, span.Length,
                                         Marshal.SizeOf<U>(), (int) type,
                                         out int exception);
        if (exception != 0)
          throw new JSException((string) res);
        return (T) res;
    ...
    /// <summary>
    /// Copies the underlying <see cref="T:WebAssembly.Core.TypedArray`2"/> data
    /// to a Span.
    /// </summary>
    ...
    public unsafe int CopyTo(Span<U>span) {
      ...
      fixed(byte *ptr = bytes) {
        var res =
            Runtime.TypedArrayCopyTo(JSHandle, (int) ptr, 0, span.Length,
                                     Marshal.SizeOf<U>(), out int exception);
        if (exception != 0)
          throw new JSException((string) res);
    ...
    private unsafe int CopyFrom(void *ptrSource, int offset, int count) {
      var res = Runtime.TypedArrayCopyFrom(JSHandle, (int) ptrSource, offset,
                                           offset + count, Marshal.SizeOf<U>(),
                                           out int exception);
      if (exception != 0)
        throw new JSException((string) res);
      return (int) res / Marshal.SizeOf<U>();
    ...
    /// <summary>
    /// Copies from a <see cref="T:Span{T}"/> to the <see
    /// cref="T:WebAssembly.Core.TypedArray`2"/> memory.
    /// </summary>
    /// <returns>Total copied</returns>
    /// <param name="span">ReadOnlySpan.</param>
    public unsafe int CopyFrom(ReadOnlySpan<U>span) {
      ...
      fixed(byte *ptr = bytes) {
        var res =
            Runtime.TypedArrayCopyFrom(JSHandle, (int) ptr, 0, span.Length,
                                       Marshal.SizeOf<U>(), out int exception);
        if (exception != 0)
          throw new JSException((string) res);
        return (int) res / Marshal.SizeOf<U>();
    ...
    protected static void ValidateFromSource(U[] source, int offset,
                                             int count) {
      // target array has to be instantiated.
      if (source == null) {
        throw new System.ArgumentException(
