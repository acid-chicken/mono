...
namespace WebAssembly.Net.Debugging {
  ...
  internal class MonoCommands {
    ...
    public static MonoCommands
    IsRuntimeReady() => new MonoCommands("MONO.mono_wasm_runtime_is_ready");
    ...
    public static MonoCommands
    GetLoadedFiles() => new MonoCommands("MONO.mono_wasm_get_loaded_files()");
    ...
    public static MonoCommands
    GetScopeVariables(int scopeId, params int[] vars) => new MonoCommands(
        $"MONO.mono_wasm_get_variables({scopeId}, [ {string.Join (",
        ", vars)} ])");

    public static MonoCommands SetBreakpoint(string assemblyName,
  ...
  class Breakpoint {
    ...
    public Breakpoint(string stackId, SourceLocation loc,
  ...
  enum BreakpointState { Active, Disabled, Pending }

  enum StepKind { Into, Out, Over }
  ...
  internal class MonoProxy : DevToolsProxy {
    ...
    protected override async Task<bool>AcceptCommand(MessageId id,
      ...
      case "Runtime.getProperties": {
        ...
        if (objId.StartsWith("dotnet:", StringComparison.Ordinal)) {
          ...
          case "object": {
            await GetDetails(
                id, MonoCommands.GetObjectProperties(int.Parse(parts[2])),
                token);
            break;
          }
          case "array": {
            await GetDetails(
                id, MonoCommands.GetArrayValues(int.Parse(parts[2])), token);
            break;
          }
          }
          return true;
    ...
    async Task GetDetails(MessageId msg_id, MonoCommands cmd,
      ...
      try {
        ...
        // Trying to inspect the stack frame for
        // DotNetDispatcher::InvokeSynchronously results in a "Memory access out
        // of bounds", causing 'values' to be null, so skip returning variable
        // values in that case.
        for (int i = 0; i < values.Length; i += 2) {
          string fieldName = FormatFieldName((string) values [i]
                                             ["name"]);
          ...
          var_list.Add(JObject.FromObject(new {name = fieldName, value}));
        }
    ...
    async Task GetScopeProperties(MessageId msg_id, int scope_id,
      ...
      try {
        var scope =
            GetContext(msg_id).CallStack.FirstOrDefault(s => s.Id == scope_id);
        var vars =
        ...
        while (i < vars.Length && i < values.Length) {
          ...
          var_list.Add(new {name = vars[i].Name, value});
          i++;
        }
        // Async methods are special in the way that local variables can be
        // lifted to generated class fields value of "this" comes here either
        while (i < values.Length) {
          ...
          if (name.IndexOf(">", StringComparison.Ordinal) > 0)
            name = name.Substring(
                1, name.IndexOf(">", StringComparison.Ordinal) - 1);
