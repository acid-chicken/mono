  ...
  internal class MonoProxy : DevToolsProxy {
    ...
    protected override async Task<bool>AcceptCommand(MessageId id,
      ...
      case "Runtime.releaseObject": {
        if (!(DotnetObjectId.TryParse(args["objectId"], out var objectId) &&
              objectId.Scheme == "cfo_res"))
          break;

        await SendMonoCommand(
            id,
      ...
      case "Runtime.callFunctionOn": {
        ...
        if (objectId.Scheme == "scope") {
          var fail = silent ? Result.OkFromObject(new {result = new {}})
        ...
        var cmd = new MonoCommands(
            $"MONO.mono_wasm_call_function_on ({args.ToString ()}, {(returnByValue ? " true " : " false ")})");

        var res = await SendMonoCommand(id, cmd, token);
        if (!returnByValue &&
            DotnetObjectId.TryParse(res.Value? ["result"]?
                                    ["value"]?
                                    ["objectId"],
                                    out var resultObjectId) &&
            resultObjectId.Scheme == "cfo_res")
          res = Result.OkFromObject(new {result = res.Value ["result"]
                                                  ["value"]});
    ...
    async Task<Result>RuntimeGetProperties(MessageId id,
                                           DotnetObjectId objectId, JToken args,
                                           CancellationToken token) {
      if (objectId.Scheme == "scope")
      ...
      var res = await SendMonoCommand(
          id,
          new MonoCommands(
              $"MONO.mono_wasm_get_details ('{objectId}', {args})"),
          token);
      if (res.IsErr)
        return res;
      ...
      if (objectId.Scheme == "cfo_res") {
        ...
        var value_json_str = res.Value ["result"]?
                             ["value"]?
        ...
        res = Result.Ok(JObject.FromObject(new {result = res.Value ["result"]
                                                         ["value"]}));
      }
    ...
    TryGetVariableValue(MessageId msg_id, int scope_id, string expression,
      ...
      // search in scope
      if (thisValue != null) {
        if (!DotnetObjectId.TryParse(thisValue ["value"]
                                     ["objectId"],
                                     out var objectId))
          return null;
        ...
        res = await SendMonoCommand(
            msg_id,
            MonoCommands.GetObjectProperties(objectId, expandValueTypes
                                             : false),
            token);
        values = res.Value? ["result"]?
                 ["value"]?
    ...
    async Task<bool>OnEvaluateOnCallFrame(MessageId msg_id, int scope_id,
                                          string expression,
                                          CancellationToken token) {
      try {
        var context = GetContext(msg_id);
        ...
        logger.LogDebug(
            e, $"Error in EvaluateOnCallFrame for expression '{expression}.");
      }
      return false;
    }

    async Task<Result>GetScopeProperties(MessageId msg_id, int scope_id,
                                         CancellationToken token) {
      try {
        var ctx = GetContext(msg_id);
        var scope = ctx.CallStack.FirstOrDefault(s => s.Id == scope_id);
        if (scope == null)
          return Result.Err(JObject.FromObject(
