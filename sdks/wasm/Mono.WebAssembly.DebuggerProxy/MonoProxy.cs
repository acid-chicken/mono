...
namespace WebAssembly.Net.Debugging {
  ...
  internal class MonoProxy : DevToolsProxy {
    ...
    Dictionary<SessionId, ExecutionContext>contexts =
        new Dictionary<SessionId, ExecutionContext>();

    public MonoProxy(ILoggerFactory loggerFactory, bool hideWebDriver = true)
        : base(loggerFactory) {}
    ...
    internal ExecutionContext GetContext(SessionId sessionId) {
      ...
      throw new ArgumentException($"Invalid Session: \"{sessionId}\"",
                                  nameof(sessionId));
    }

    bool UpdateContext(SessionId sessionId, ExecutionContext executionContext,
                       out ExecutionContext previousExecutionContext) {
      var previous =
          contexts.TryGetValue(sessionId, out previousExecutionContext);
      contexts[sessionId] = executionContext;
      return previous;
    }

    internal Task<Result>SendMonoCommand(SessionId id, MonoCommands cmd,
                                         CancellationToken token) =>
        SendCommand(id, "Runtime.evaluate", JObject.FromObject(cmd), token);

    protected override async Task<bool>AcceptEvent(SessionId sessionId,
                                                   string method, JObject args,
                                                   CancellationToken token) {
      switch (method) {
      case "Runtime.consoleAPICalled": {
        ...
        if (type == "debug") {
          if (args ["args"]?
                  [0]?
                  ["value"]?
                      .ToString() == MonoConstants.RUNTIME_IS_READY &&
              args ["args"]?
                  [1]?
                  ["value"]?
                      .ToString() == "fe00e07a-5519-4dfe-b35a-f867dbaf2e28")
        ...
        if (aux_data != null) {
          ...
          if (is_default == true) {
            await OnDefaultContext(
                sessionId, new ExecutionContext{Id = id, AuxData = aux_data},
                token);
          }
        }
        return true;
      ...
      case "Debugger.paused": {
        // TODO figure out how to stich out more frames and, in particular what
        // happens when real wasm is on the stack
        var top_func = args? ["callFrames"]?
                       [0]?
                       ["functionName"]?
                           .Value<string>();

        if (top_func == "mono_wasm_fire_bp" ||
            top_func == "_mono_wasm_fire_bp") {
      ...
      case "Debugger.scriptParsed": {
        ...
        Log("verbose",
            $"proxying Debugger.scriptParsed ({sessionId.sessionId}) {url} {args}");
        break;
      }
      ...
      case "Target.attachedToTarget": {
        if (args ["targetInfo"]
            ["type"]?
                .ToString() == "page")
          await DeleteWebDriver(new SessionId(args["sessionId"]?.ToString()),
                                token);
        break;
      }
      }
    ...
    async Task<bool>IsRuntimeAlreadyReadyAlready(SessionId sessionId,
                                                 CancellationToken token) {
      var res = await SendMonoCommand(sessionId, MonoCommands.IsRuntimeReady(),
                                      token);
      return res
          .Value? ["result"]?
          ["value"]?
    ...
    protected override async Task<bool>AcceptCommand(MessageId id,
                                                     string method,
                                                     JObject args,
                                                     CancellationToken token) {
      ...
      case "Target.attachToTarget": {
        ...
        await DeleteWebDriver(
            new SessionId(resp.Value["sessionId"]?.ToString()), token);
        break;
      }
      ...
      case "Debugger.getPossibleBreakpoints": {
        ...
        // FIXME support variant where restrictToFunction=true and end is
        // omitted
        var end = SourceLocation.Parse(args?["end"] as JObject);
        if (start != null && end != null &&
            await GetPossibleBreakpoints(id, start, end, token))
          return true;
      ...
      case "Debugger.setBreakpointByUrl": {
        ...
        SendResponse(
            id, Result.OkFromObject(request.AsSetBreakpointByUrlResponse()),
            token);
        return true;
      }
      ...
      case "Debugger.evaluateOnCallFrame": {
        ...
        if (objId.StartsWith("dotnet:", StringComparison.Ordinal)) {
          ...
          switch (parts[1]) {
          case "scope": {
            await GetEvaluateOnCallFrame(id, int.Parse(parts[2]),
                                         args?["expression"]?.Value<string>(),
                                         token);
            break;
          }
          }
      ...
      case "Runtime.getProperties": {
        ...
        if (objId.StartsWith("dotnet:", StringComparison.Ordinal)) {
          ...
          case "object": {
            await GetDetails(id,
                             MonoCommands.GetObjectProperties(
                                 int.Parse(parts[2]), expandValueTypes
                                 : false),
                             token);
            break;
          }
          ...
          case "valuetype": {
            await GetDetailsForValueType(
                id, objId, get_props_cmd_fn
                : () => {
                  if (parts.Length < 4)
                    return null;

                  var containerObjId = int.Parse(parts[2]);
                  return MonoCommands.GetObjectProperties(containerObjId,
                                                          expandValueTypes
                                                          : true);
                }, token);
            break;
      ...
      case "Dotnet-test.setBreakpointByMethod": {
        ...
        if (aname == null || typeName == null || methodName == null) {
          SendResponse(id,
                       Result.Err("Invalid protocol message '" + args + "'."),
                       token);
          return true;
        }
        ...
        if (assembly == null) {
          SendResponse(id, Result.Err("Assembly '" + aname + "' not found."),
                       token);
          return true;
        }
        ...
        if (methodInfo == null) {
          SendResponse(
              id, Result.Err($"Method '{typeName}:{methodName}' not found."),
              token);
          return true;
        }
        ...
        var resolvedLocation =
            new {breakpointId = bpid, location = loc.AsLocation()};

        SendEvent(id, "Debugger.breakpointResolved",
                  JObject.FromObject(resolvedLocation), token);

        SendResponse(
            id,
            Result.OkFromObject(new {
                result = new {breakpointId = bpid,
                              locations = new object[]{loc.AsLocation()}}}),
            token);
    ...
    async Task GetArrayDetails(MessageId id, string objId, string[] objIdParts,
                               CancellationToken token) {
      switch (objIdParts.Length) {
      case 3: {
        await GetDetails(
            id, MonoCommands.GetArrayValues(int.Parse(objIdParts[2])), token);
        break;
      }
      case 4: {
      ...
      default:
        SendResponse(id,
                     Result.Exception(new ArgumentException(
                         $"Unknown objectId format for array: {objId}")),
                     token);
        break;
      }
    }
    ...
    async Task<bool>OnBreakpointHit(SessionId sessionId, JObject args,
                                    CancellationToken token) {
      // FIXME we should send release objects every now and then? Or intercept
      // those we inject and deal in the runtime
      var res =
          await SendMonoCommand(sessionId, MonoCommands.GetCallStack(), token);
      ...
      var res_value = res.Value? ["result"]?
                      ["value"];
      ...
      var bp = context.BreakpointRequests.Values.SelectMany(v => v.Locations)
      ...
      foreach(var frame in orig_callframes) {
        ...
        if ("mono_wasm_fire_bp" == function_name ||
            "_mono_wasm_fire_bp" == function_name) {
          var frames = new List<Frame>();
          int frame_id = 0;
          var the_mono_frames = res.Value? ["result"]?
                                ["value"]?
                                ["frames"]?
                                    .Values<JObject>();
          ...
          foreach(var mono_frame in the_mono_frames) {
            ...
            if (method == null) {
              Log("info",
                  $"Unable to find il offset: {il_pos} in method token: {method_token} assembly name: {assembly_name}");
              continue;
            }

            var location = method?.GetLocationByIl(il_pos);

            // When hitting a breakpoint on the "IncrementCount" method in the
            // standard Blazor project template, one of the stack frames is
            // inside mscorlib.dll and we get location==null for it. It will
            // trigger a NullReferenceException if we don't skip over that stack
            // frame.
            if (location == null) {
              continue;
            }
            ...
            Log("info",
                $"frame il offset: {il_pos} method token: {method_token} assembly name: {assembly_name}");
            Log("info", $"\tmethod {method.Name} location: {location}");
            frames.Add(new Frame(method, location, frame_id - 1));

            callFrames.Add(
                new {functionName = method.Name,
        ...
        } else if (!(function_name.StartsWith("wasm-function",
    ...
    async Task OnDefaultContext(SessionId sessionId, ExecutionContext context,
                                CancellationToken token) {
      Log("verbose",
    ...
    async Task<bool>Step(MessageId msg_id, StepKind kind,
                         CancellationToken token) {
      ...
      var res = await SendMonoCommand(
          msg_id, MonoCommands.StartSingleStepping(kind), token);

      var ret_code = res.Value? ["result"]?
                     ["value"]?
                         .Value<int>();
    ...
    async Task GetDetails(MessageId msg_id, MonoCommands cmd,
                          CancellationToken token, bool send_response = true) {
      ...
      try {
        ...
        var response = JObject.FromObject(new {result = var_list});
    ...
    internal bool TryFindVariableValueInCache(ExecutionContext ctx,
                                              string expression,
                                              bool only_search_on_this,
                                              out JToken obj) {
    ...
    internal async Task<JToken>
    TryGetVariableValue(MessageId msg_id, int scope_id, string expression,
                        bool only_search_on_this, CancellationToken token) {
      JToken thisValue = null;
      var context = GetContext(msg_id);
      if (context.CallStack == null)
        return null;

      if (TryFindVariableValueInCache(context, expression, only_search_on_this,
                                      out JToken obj))
        return obj;
      ...
      int[] var_ids = {};
      var res = await SendMonoCommand(
          msg_id, MonoCommands.GetScopeVariables(scope.Id, var_ids), token);
      var values = res.Value? ["result"]?
                   ["value"]?
                       .Values<JObject>()
                       .ToArray();
      thisValue =
          values.FirstOrDefault(v => v["name"].Value<string>() == "this");
      ...
      if (!only_search_on_this) {
        ...
        if (var_id != null) {
          res = await SendMonoCommand(
              msg_id,
              MonoCommands.GetScopeVariables(scope.Id, new int[]{var_id.Index}),
              token);
          values = res.Value? ["result"]?
                   ["value"]?
                       .Values<JObject>()
                       .ToArray();
          return values[0];
      ...
      // search in scope
      if (thisValue != null) {
        var objectId = thisValue ["value"]
                       ["objectId"]
                           .Value<string>();
        var parts = objectId.Split(new char[]{':'});
        res = await SendMonoCommand(msg_id,
                                    MonoCommands.GetObjectProperties(
                                        int.Parse(parts[2]), expandValueTypes
                                        : false),
                                    token);
        values = res.Value? ["result"]?
                 ["value"]?
                     ...
                     .ToArray();
        var foundValue =
            values.FirstOrDefault(v => v["name"].Value<string>() == expression);
        if (foundValue != null) {
    ...
    async Task GetEvaluateOnCallFrame(MessageId msg_id, int scope_id,
                                      string expression,
                                      CancellationToken token) {
      try {
        var context = GetContext(msg_id);
        ...
        var varValue = await TryGetVariableValue(msg_id, scope_id, expression,
                                                 false, token);
        ...
        if (varValue != null) {
          ...
          ["className"];
          SendResponse(msg_id,
                       Result.OkFromObject(new {result = varValue["value"]}),
                       token);
          return;
        }

        string retValue = await EvaluateExpression.CompileAndRunTheExpression(
            this, msg_id, scope_id, expression, token);
        SendResponse(msg_id,
                     Result.OkFromObject(new {result = new {value = retValue}}),
                     token);
      } catch (Exception e) {
    ...
    async Task GetScopeProperties(MessageId msg_id, int scope_id,
                                  CancellationToken token) {
      ...
      try {
        ...
        if (scope == null) {
          SendResponse(
              msg_id,
              Result.Err(JObject.FromObject(
                  new {message = $"Could not find scope with id #{scope_id}"})),
              token);
          return;
        }
        var vars =
            scope.Method.GetLiveVarsAt(scope.Location.CliLocation.Offset);
        ...
        if (values == null) {
          SendResponse(
              msg_id, Result.OkFromObject(new {result = Array.Empty<object>()}),
              token);
          return;
        }
        ...
        for (; i < vars.Length && i < values.Length; i++) {
          ...
          var_list.Add(new {name = vars[i].Name, value = values [i]
                                                 ["value"]});
        }
        for (; i < values.Length; i++) {
          ctx.LocalsCache [values [i]
                           ["name"]
                               .ToString()]
          = values [i]
          ["value"];
        ...
        SendResponse(msg_id, Result.OkFromObject(new {result = var_list}),
                     token);
    ...
    IEnumerable<JObject>
    ExtractAndCacheValueTypes(ExecutionContext ctx,
                              IEnumerable<JObject>var_list) {
      foreach(var jo in var_list) {
        ...
        if (value["type"]?.Value<string>() != "object")
          continue;
            ...
            // Expanded ValueType
            var members = value["members"]?.Values<JObject>().ToArray()
                ?? Array.Empty<JObject>();
            var objectId = value["objectId"]?.Value<string>()
                ?? $"dotnet:valuetype:{ctx.NextValueTypeId ()}";
    ...
    async Task<bool>GetDetailsForValueType(MessageId msg_id, string object_id,
                                           Func<MonoCommands>get_props_cmd_fn,
                                           CancellationToken token) {
      var ctx = GetContext(msg_id);
      ...
      if (ctx.ValueTypesCache.TryGetValue(object_id, out var var_list)) {
        ...
        var response = JObject.FromObject(
            new {result = $"Unable to get details for {object_id}"});
    ...
    async Task<Breakpoint>SetMonoBreakpoint(SessionId sessionId, string reqId,
      ...
      var res = await SendMonoCommand(
          sessionId,
          MonoCommands.SetBreakpoint(asm_name, method_token, il_offset), token);
      var ret_code = res.Value? ["result"]?
                     ["value"]?
                         .Value<int>();
      ...
      if (ret_code.HasValue) {
        ...
        // Log ("verbose", $"BP local id {bp.LocalId} enabled with remote id
        // {bp.RemoteId}");
      }
    ...
    async Task<DebugStore>LoadStore(SessionId sessionId,
                                    CancellationToken token) {
      var context = GetContext(sessionId);

      if (Interlocked.CompareExchange(ref context.store, new DebugStore(logger),
                                      null) != null)
        return await context.Source.Task;

      try {
        var loaded_pdbs = await SendMonoCommand(
            sessionId, MonoCommands.GetLoadedFiles(), token);
        var the_value = loaded_pdbs.Value? ["result"]?
                        ["value"];
        var the_pdbs = the_value?.ToObject<string[]>();

        await foreach(
            var source in context.store.Load(sessionId, the_pdbs, token)
                .WithCancellation(token)) {
          var scriptSource = JObject.FromObject(
              source.ToScriptSource(context.Id, context.AuxData));
          Log("verbose",
              $"\tsending {source.Url} {context.Id} {sessionId.sessionId}");
    ...
    async Task<DebugStore>RuntimeReady(SessionId sessionId,
                                       CancellationToken token) {
      var context = GetContext(sessionId);
      if (Interlocked.CompareExchange(ref context.ready,
                                      new TaskCompletionSource<DebugStore>(),
                                      null) != null)
        return await context.ready.Task;

      var clear_result = await SendMonoCommand(
          sessionId, MonoCommands.ClearAllBreakpoints(), token);
    ...
    async Task RemoveBreakpoint(MessageId msg_id, JObject args,
                                CancellationToken token) {
      var bpid = args?["breakpointId"]?.Value<string>();

      var context = GetContext(msg_id);
      if (!context.BreakpointRequests.TryGetValue(bpid,
                                                  out var breakpointRequest))
        return;

      foreach(var bp in breakpointRequest.Locations) {
        var res = await SendMonoCommand(
            msg_id, MonoCommands.RemoveBreakpoint(bp.RemoteId), token);
        var ret_code = res.Value? ["result"]?
                       ["value"]?
                           .Value<int>();
    ...
    async Task SetBreakpoint(SessionId sessionId, DebugStore store,
                             BreakpointRequest req, CancellationToken token) {
      var context = GetContext(sessionId);
      ...
      logger.LogDebug(
          "BP request for '{req}' runtime ready {context.RuntimeReady}", req,
          GetContext(sessionId).IsRuntimeReady);
      ...
      foreach(var loc in locations) {
        ...
        var resolvedLocation =
            new {breakpointId = req.Id, location = loc.AsLocation()};

        SendEvent(sessionId, "Debugger.breakpointResolved",
                  JObject.FromObject(resolvedLocation), token);
      }
    ...
    async Task<bool>GetPossibleBreakpoints(MessageId msg, SourceLocation start,
                                           SourceLocation end,
                                           CancellationToken token) {
      var bps =
          (await RuntimeReady(msg, token)).FindPossibleBreakpoints(start, end);
    ...
    async Task<bool>OnGetScriptSource(MessageId msg_id, string script_id,
                                      CancellationToken token) {
      ...
      try {
        ...
        using(var data = await src_file.GetSourceAsync(checkHash
                                                       : false, token
                                                       : token)) {
          if (data.Length == 0)
            return false;

          using(var reader = new StreamReader(data)) source =
              await reader.ReadToEndAsync();
        }
        SendResponse(msg_id, Result.OkFromObject(new {scriptSource = source}),
                     token);
      } catch (Exception e) {
        var o = new {scriptSource =
    ...
    async Task DeleteWebDriver(SessionId sessionId, CancellationToken token) {
      ...
      if (hideWebDriver && sessions.Add(sessionId)) {
        var res = await SendCommand(
            sessionId, "Page.addScriptToEvaluateOnNewDocument",
            JObject.FromObject(new {
                source = "delete navigator.constructor.prototype.webdriver"}),
            token);
