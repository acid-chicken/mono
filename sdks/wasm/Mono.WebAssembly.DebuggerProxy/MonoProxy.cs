...
namespace WebAssembly.Net.Debugging {
  ...
  internal class ExecutionContext {
    ...
    public int NextBreakpointId() => Interlocked.Increment(ref breakpointIndex);
  ...
  internal class MonoProxy : DevToolsProxy {
    ...
    async Task<bool>OnBreakpointHit(SessionId sessionId, JObject args,
      ...
      var bp =
          context.Breakpoints.FirstOrDefault(b => b.RemoteId == bp_id.Value);
    ...
    async Task<Result>EnableBreakpoint(SessionId sessionId, Breakpoint bp,
                                       CancellationToken token) {
    ...
    async Task RuntimeReady(SessionId sessionId, CancellationToken token) {
      ...
      foreach(var s in store.AllSources()) {
        var scriptSource =
            JObject.FromObject(s.ToScriptSource(context.Id, context.AuxData));
    ...
    async Task<Result>RemoveBreakpoint(SessionId sessionId, Breakpoint bp,
                                       CancellationToken token) {
      var res = await SendMonoCommand(
          sessionId, MonoCommands.RemoveBreakpoint(bp.RemoteId), token);
      var ret_code = res.Value? ["result"]?
                     ["value"]?
                         .Value<int>();
    ...
    async Task<bool>SetBreakpoint(MessageId msg_id, BreakpointRequest req,
                                  CancellationToken token) {
      var context = GetContext(msg_id);
      var bp_loc = context.Store.FindBestBreakpoint(req);
      Log("info",
          $"BP request for '{req}' runtime ready {context.RuntimeReady} location '{bp_loc}'");
      if (bp_loc == null) {
      ...
      Breakpoint bp = null;
      if (!context.RuntimeReady) {
        bp = new Breakpoint(bp_loc, context.NextBreakpointId(),
      ...
      var ok = new {
          ...
          locations = new[]{bp_loc.AsLocation()},
      };
    ...
    async Task<bool>GetPossibleBreakpoints(MessageId msg_id,
                                           SourceLocation start,
                                           SourceLocation end,
                                           CancellationToken token) {
      var bps =
          (await LoadStore(msg_id, token)).FindPossibleBreakpoints(start, end);
      if (bps == null)
        return false;

      SendResponse(msg_id,
                   Result.OkFromObject(
                       new {locations = bps.Select(b => b.AsLocation())}),
                   token);
      return true;
    }
