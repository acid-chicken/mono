...
namespace WebAssembly.Net.Debugging {
  ...
  public class MonoProxy : DevToolsProxy {
    ...
    protected override async Task<bool>AcceptEvent(SessionId sessionId,
      ...
      case "Runtime.executionContextCreated": {
        ...
        if (aux_data != null) {
          ...
          if (is_default == true) {
            var id = new MessageId{id = ctx["id"].Value<int>(),
      ...
      case "Debugger.paused": {
        ...
        if (top_func == MonoConstants.RUNTIME_IS_READY) {
          await OnRuntimeReady(new SessionId{sessionId = sessionId.sessionId},
                               token);
          return true;
        }
        break;
      ...
      case "Debugger.enabled": {
        await LoadStore(
            new SessionId{sessionId = args?["sessionId"]?.Value<string>()},
            token);
        break;
      }
      }
    ...
    protected override async Task<bool>AcceptCommand(MessageId id,
      ...
      case "Runtime.getProperties": {
        ...
        if (objId.StartsWith("dotnet:")) {
          ...
          case "object": {
            await GetDetails(id, int.Parse(parts[2]), token,
                             MonoCommands.GET_OBJECT_PROPERTIES);
            break;
          }
          case "array": {
            await GetDetails(id, int.Parse(parts[2]), token,
                             MonoCommands.GET_ARRAY_VALUES);
            break;
          }
          }
          return true;
    ...
    async Task OnBreakPointHit(SessionId sessionId, JObject args,
                               CancellationToken token) {
      // FIXME we should send release objects every now and then? Or intercept
      // those we inject and deal in the runtime
      var o = JObject.FromObject(new {expression = MonoCommands.GET_CALL_STACK,
                                      objectGroup = "mono_debugger",
                                      includeCommandLineAPI = false,
                                      silent = false, returnByValue = true});
      ...
      foreach(var frame in orig_callframes) {
        ...
        if ("mono_wasm_fire_bp" == function_name ||
          ...
          foreach(var mono_frame in the_mono_frames) {
            ...
            callFrames.Add(JObject.FromObject(
            ...
            this.current_callstack = frames;
                         ...
                         "wasm-function", StringComparison.InvariantCulture) ||
    ...
    async Task OnDefaultContext(MessageId ctx_id, JObject aux_data,
                                CancellationToken token) {
      Log("verbose",
          "Default context created, clearing state and sending events");

      // reset all bps
      foreach(var b in this.breakpoints) { b.State = BreakPointState.Pending; }
      this.runtime_ready = false;

      var o = JObject.FromObject(
          new {expression = MonoCommands.IS_RUNTIME_READY_VAR,
               objectGroup = "mono_debugger", includeCommandLineAPI = false,
               silent = false, returnByValue = true});
    ...
    async Task Step(MessageId msg_id, StepKind kind, CancellationToken token) {
      ...
      var o = JObject.FromObject(new {
          expression =
              string.Format(MonoCommands.START_SINGLE_STEPPING, (int) kind),
          objectGroup = "mono_debugger",
          includeCommandLineAPI = false,
          silent = false,
          returnByValue = true,
    ...
    async Task GetDetails(MessageId msg_id, int object_id,
      ...
      try {
        ...
        o = JObject.FromObject(new {result = var_list});
      } catch (Exception e) {
    ...
    async Task GetScopeProperties(MessageId msg_id, int scope_id,
      ...
      var o = JObject.FromObject(new {
          expression = string.Format(MonoCommands.GET_SCOPE_VARIABLES, scope.Id,
                                     var_ids),
          objectGroup = "mono_debugger",
          includeCommandLineAPI = false,
          silent = false,
      ...
      try {
        ...
        while (values != null && i < vars.Length && i < values.Length) {
          ...
          var_list.Add(JObject.FromObject(new {name = vars[i].Name, value}));
          i++;
        }
        ...
        while (i < values.Length) {
          ...
          var_list.Add(JObject.FromObject(new {name, value}));
          i = i + 2;
        }
        o = JObject.FromObject(new {result = var_list});
        SendResponse(msg_id, Result.Ok(o), token);
    ...
    async Task<Result>EnableBreakPoint(SessionId sessionId, Breakpoint bp,
      ...
      var o = JObject.FromObject(new {
          expression = string.Format(MonoCommands.SET_BREAK_POINT, asm_name,
                                     method_token, il_offset),
          objectGroup = "mono_debugger",
          includeCommandLineAPI = false,
          silent = false,
          returnByValue = true,
    ...
    async Task LoadStore(SessionId sessionId, CancellationToken token) {
      ...
      var loaded_pdbs =
          await SendCommand(sessionId, "Runtime.evaluate", o, token);
    ...
    async Task<Result>RemoveBreakPoint(SessionId sessionId, Breakpoint bp,
      ...
      var o = JObject.FromObject(new {
          expression =
              string.Format(MonoCommands.REMOVE_BREAK_POINT, bp.RemoteId),
          objectGroup = "mono_debugger",
          includeCommandLineAPI = false,
          silent = false,
          returnByValue = true,
    ...
    async Task SetBreakPoint(MessageId msg_id, BreakPointRequest req,
      ...
      Log("info",
          $"BP request for '{req}' runtime ready {runtime_ready} location '{bp_loc}'");
      if (bp_loc == null) {
      ...
      Breakpoint bp = null;
      if (!runtime_ready) {
        bp = new Breakpoint(bp_loc, local_breakpoint_id++,
      ...
      locations.Add(JObject.FromObject(new {scriptId = bp_loc.Id.ToString(),
    ...
    bool GetPossibleBreakpoints(MessageId msg_id, SourceLocation start,
      ...
      foreach(var b in bps) { loc.Add(b.ToJObject()); }

      var o = JObject.FromObject(new {locations = loc});
    ...
    async Task OnGetScriptSource(MessageId msg_id, string script_id,
                                 CancellationToken token) {
      ...
      try {
        ...
        if (uri.IsFile && File.Exists(uri.LocalPath)) {
          ...
          var o = JObject.FromObject(new {scriptSource = res.ToString()});
