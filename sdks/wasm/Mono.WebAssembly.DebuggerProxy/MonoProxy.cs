...
public class MonoProxy : WsProxy {
  ...
  protected override async Task<bool>AcceptEvent(string method, JObject args,
  ...
  AcceptCommand(int id, string method, JObject args, CancellationToken token) {
    switch (method) {
    ...
    case "Runtime.getProperties": {
      ...
      if (objId.StartsWith("dotnet:scope:",
                           StringComparison.InvariantCulture)) {
        await GetScopeProperties(
            id, int.Parse(objId.Substring("dotnet:scope:".Length)), token);
        return true;
      }
      if (objId.StartsWith("dotnet:", StringComparison.InvariantCulture)) {
        if (objId.StartsWith("dotnet:object:",
                             StringComparison.InvariantCulture))
          await GetDetails(id,
                           int.Parse(objId.Substring("dotnet:object:".Length)),
                           token, MonoCommands.GET_OBJECT_PROPERTIES);
        if (objId.StartsWith("dotnet:array:",
                             StringComparison.InvariantCulture))
          await GetDetails(id,
                           int.Parse(objId.Substring("dotnet:array:".Length)),
                           token, MonoCommands.GET_ARRAY_VALUES);
        return true;
      }
      break;
    }
  ...
  async Task OnBreakPointHit(JObject args, CancellationToken token) {
    ...
    var o = JObject.FromObject(new {
        expression = MonoCommands.GET_CALL_STACK, objectGroup = "mono_debugger",
        includeCommandLineAPI = false, silent = false, returnByValue = true});
    ...
    foreach(var frame in orig_callframes) {
      ...
      if ("mono_wasm_fire_bp" == function_name ||
        ...
        foreach(var mono_frame in the_mono_frames) {
          ...
          // standard Blazor project template, one of the stack frames is inside
          // mscorlib.dll and we get location==null for it. It will trigger a
          // NullReferenceException if we don't skip over that stack frame.
          if (location == null) {
            continue;
          }
  ...
  async Task OnDefaultContext(int ctx_id, JObject aux_data,
                              CancellationToken token) {
  ...
  async Task GetDetails(int msg_id, int object_id, CancellationToken token,
                        string command) {
    var o = JObject.FromObject(new {
  ...
  async Task GetScopeProperties(int msg_id, int scope_id,
                                CancellationToken token) {
    ...
    var o = JObject.FromObject(new {
        expression =
            string.Format(MonoCommands.GET_SCOPE_VARIABLES, scope.Id, var_ids),
        objectGroup = "mono_debugger",
        includeCommandLineAPI = false,
        silent = false,
        returnByValue = true,
    ...
    try {
      ...
      // Async methods are special in the way that local variables can be lifted
      // to generated class fields value of "this" comes here either
      while (i < values.Length) {
        ...
        if (name.IndexOf(">", StringComparison.Ordinal) > 0)
          name = name.Substring(1, name.IndexOf(">", StringComparison.Ordinal) -
                                       1);
  ...
  async Task<Result>EnableBreakPoint(Breakpoint bp, CancellationToken token) {
    ...
    if (ret_code.HasValue) {
      ...
      // Debug ($"BP local id {bp.LocalId} enabled with remote id
      // {bp.RemoteId}");
    }
  ...
  async Task RuntimeReady(CancellationToken token) {
    ...
    foreach(var s in store.AllSources()) {
      var ok = JObject.FromObject(
          new {scriptId = s.SourceId.ToString(), url = s.Url,
               executionContextId = this.ctx_id, hash = s.DocHashCode,
  ...
  async Task<bool>RemoveBreakpoint(int msg_id, JObject args,
                                   CancellationToken token) {
  ...
  bool GetPossibleBreakpoints(int msg_id, SourceLocation start,
  ...
  async Task OnGetScriptSource(int msg_id, string script_id,
                               CancellationToken token) {
