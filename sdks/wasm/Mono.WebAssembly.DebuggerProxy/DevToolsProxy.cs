...
namespace WebAssembly.Net.Debugging {
  public class SessionId { public string sessionId; }

  public class MessageId : SessionId { public int id; }
  ...
  public struct Result {
    ...
    public static Result Ok(JObject ok) { return new Result(ok, null); }

    public static Result Err(JObject err) { return new Result(null, err); }

    public JObject ToJObject(MessageId target) {
      if (IsOk) {
        return JObject.FromObject(
            new {target.id, target.sessionId, result = Value});
      } else {
        return JObject.FromObject(
            new {target.id, target.sessionId, error = Error});
      }
    }
  }
  ...
  class DevToolsQueue {
    ...
    public Task Send(byte[] bytes, CancellationToken token) {
      ...
      if (pending.Count == 1) {
        if (current_send != null)
          throw new Exception(
              "current_send MUST BE NULL IF THERE'S no pending send");
        // Console.WriteLine ("sending {0} bytes", bytes.Length);
        current_send = Ws.SendAsync(new ArraySegment<byte>(bytes),
    ...
    public Task Pump(CancellationToken token) {
      ...
      if (pending.Count > 0) {
        ...
        current_send = Ws.SendAsync(new ArraySegment<byte>(pending[0]),
                                    WebSocketMessageType.Text, true, token);
        return current_send;
      }
      return null;
  ...
  public class DevToolsProxy {
    ...
    TaskCompletionSource<bool>client_initiated_close =
        new TaskCompletionSource<bool>();
    List<(MessageId, TaskCompletionSource<Result>)>pending_cmds =
        new List<(MessageId, TaskCompletionSource<Result>)>();
    ...
    protected virtual Task<bool>AcceptEvent(SessionId sessionId, string method,
                                            JObject args,
                                            CancellationToken token) {
      return Task.FromResult(false);
    }

    protected virtual Task<bool>AcceptCommand(MessageId id, string method,
                                              JObject args,
                                              CancellationToken token) {
      return Task.FromResult(false);
    }
    ...
    async Task<string>ReadOne(WebSocket socket, CancellationToken token) {
      ...
      while (true) {
        ...
        var result =
            await socket.ReceiveAsync(new ArraySegment<byte>(buff), token);
    ...
    async Task OnEvent(SessionId sessionId, string method, JObject args,
                       CancellationToken token) {
      try {
    ...
    async Task OnCommand(MessageId id, string method, JObject args,
                         CancellationToken token) {
      try {
    ...
    void OnResponse(MessageId id, Result result) {
      ...
      // Fixme
      var idx = pending_cmds.FindIndex(e => e.Item1.id == id.id &&
                                            e.Item1.sessionId == id.sessionId);
    ...
    void ProcessBrowserMessage(string msg, CancellationToken token) {
      ...
      if (res["id"] == null)
        pending_ops.Add(OnEvent(
            new SessionId{sessionId = res["sessionId"]?.Value<string>()},
            res["method"].Value<string>(), res["params"] as JObject, token));
      else
        OnResponse(new MessageId{id = res["id"].Value<int>(),
                                 sessionId = res["sessionId"]?.Value<string>()},
                   Result.FromJson(res));
    }
    ...
    void ProcessIdeMessage(string msg, CancellationToken token) {
      ...
      if (!string.IsNullOrEmpty(msg)) {
        ...
        pending_ops.Add(OnCommand(
            new MessageId{id = res["id"].Value<int>(),
                          sessionId = res["sessionId"]?.Value<string>()},
            res["method"].Value<string>(), res["params"] as JObject, token));
      }
    }

    internal async Task<Result>SendCommand(SessionId id, string method,
                                           JObject args,
                                           CancellationToken token) {
    ...
    Task<Result>SendCommandInternal(SessionId sessionId, string method,
                                    JObject args, CancellationToken token) {
      int id = ++next_cmd_id;

      var o = JObject.FromObject(
          new { sessionId.sessionId, id, method, @params = args });
    ...
    public void SendEvent(SessionId sessionId, string method, JObject args,
                          CancellationToken token) {
      // Log ("verbose", $"sending event {method}: {args}");
      SendEventInternal(sessionId, method, args, token);
    }

    void SendEventInternal(SessionId sessionId, string method, JObject args,
                           CancellationToken token) {
      var o = JObject.FromObject(
          new { sessionId.sessionId, method, @params = args });

      Send(this.ide, o, token);
    }

    internal void SendResponse(MessageId id, Result result,
                               CancellationToken token) {
      // Log ("verbose", $"sending response: {id}: {result.ToJObject (id)}");
      SendResponseInternal(id, result, token);
    }

    void SendResponseInternal(MessageId id, Result result,
                              CancellationToken token) {
    ...
    public async Task Run(Uri browserUri, WebSocket ideSocket) {
      ...
      using(this.ide = ideSocket) {
        Log("verbose",
            $"DevToolsProxy: IDE waiting for connection on {browserUri}");
        ...
        using(this.browser = new ClientWebSocket()) {
          ...
          try {
            while (!x.IsCancellationRequested) {
              ...
              // Console.WriteLine ("pump {0} {1}", task, pending_ops.IndexOf
              // (task));
