...
namespace WebAssembly.Net.Debugging {
  internal struct SessionId {
    ...
    public SessionId(string sessionId) { this.sessionId = sessionId; }

    public override int GetHashCode() => sessionId?.GetHashCode() ?? 0;

    public override bool Equals(object obj) => (obj is SessionId)
  ...
  internal struct MessageId {
    ...
    operator SessionId(MessageId id) => new SessionId(id.sessionId);
    ...
    public override bool Equals(object obj) => (obj is MessageId)
                                                   ? ((MessageId) obj).sessionId
                                                             == sessionId
                                                         && ((MessageId) obj).id
                                                                == id
                                                   : false;
  ...
  internal class DevToolsProxy {
    ...
    void ProcessBrowserMessage(string msg, CancellationToken token) {
      ...
      if (res["id"] == null)
        ...
        OnResponse(new MessageId(res["sessionId"]?.Value<string>(),
                                 res["id"].Value<int>()),
                   Result.FromJson(res));
    }
    ...
    void ProcessIdeMessage(string msg, CancellationToken token) {
      ...
      if (!string.IsNullOrEmpty(msg)) {
        ...
        pending_ops.Add(OnCommand(
            new MessageId(res["sessionId"]?.Value<string>(),
                          res["id"].Value<int>()),
            res["method"].Value<string>(), res["params"] as JObject, token));
      }
    }
