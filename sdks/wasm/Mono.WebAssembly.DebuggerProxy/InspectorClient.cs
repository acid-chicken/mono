...
namespace WebAssembly.Net.Debugging {
  public class InspectorClient : DevToolsClient {
    ...
    Task HandleMessage(string msg, CancellationToken token) {
      ...
      if (res["id"] == null)
        DumpProtocol(string.Format("Event method: {0} params: {1}",
                                   res["method"], res["params"]));
      else
        DumpProtocol(
            string.Format("Response id: {0} res: {1}", res["id"], res));

      if (res["id"] == null)
        return onEvent(res["method"].Value<string>(), res["params"] as JObject,
                       token);
    ...
    public async Task
    Connect(Uri uri, Func<string, JObject, CancellationToken, Task>onEvent,
            Func<CancellationToken, Task>send, CancellationToken token) {

      this.onEvent = onEvent;
      await ConnectWithMainLoops(uri, HandleMessage, send, token);
    }

    public Task<Result>SendCommand(string method, JObject args,
      ...
      var str = o.ToString();
      // Log ("protocol", $"SendCommand: id: {id} method: {method} params:
      // {args}");
