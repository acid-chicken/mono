...
public struct Result {
  ...
  public JObject ToJObject(int id) {
    if (IsOk) {
      return JObject.FromObject(new {id = id, result = Value});
    } else {
      return JObject.FromObject(new {id = id, error = Error});
    }
  }
}
  ...
  public Task Send(byte[] bytes, CancellationToken token) {
    ...
    if (pending.Count == 1) {
      if (current_send != null)
        throw new Exception(
            "WTF, current_send MUST BE NULL IF THERE'S no pending send");
...
public class WsProxy {
  ...
  protected virtual Task<bool>AcceptEvent(string method, JObject args,
                                          CancellationToken token) {
    return Task.FromResult(false);
  }

  protected virtual Task<bool>AcceptCommand(int id, string method, JObject args,
                                            CancellationToken token) {
    return Task.FromResult(false);
  }
  ...
  async Task OnCommand(int id, string method, JObject args,
                       CancellationToken token) {
    try {
  ...
  void ProcessBrowserMessage(string msg, CancellationToken token) {
    ...
    if (res["id"] == null)
      pending_ops.Add(OnEvent(res["method"].Value<string>(),
  ...
  void ProcessIdeMessage(string msg, CancellationToken token) {
    if (!string.IsNullOrEmpty(msg)) {
      var res = JObject.Parse(msg);
      pending_ops.Add(OnCommand(res["id"].Value<int>(),
  ...
  Task<Result>SendCommandInternal(string method, JObject args,
                                  CancellationToken token) {
    int id = ++next_cmd_id;

    var o =
        JObject.FromObject(new { id = id, method = method, @params = args });
  ...
  void SendEventInternal(string method, JObject args, CancellationToken token) {
    var o = JObject.FromObject(new { method = method, @params = args });
  ...
  protected void Debug(string msg) { Console.WriteLine(msg); }

  protected void Info(string msg) { Console.WriteLine(msg); }
}
}
