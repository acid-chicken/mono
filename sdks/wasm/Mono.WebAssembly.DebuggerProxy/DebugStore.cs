...
namespace WebAssembly.Net.Debugging {
  internal class BreakpointRequest {
    ...
    public static BreakpointRequest Parse(JObject args, DebugStore store) {
      // Events can potentially come out of order, so DebugStore may not be
      // initialized The BP being set in these cases are JS ones, which we can
      // safely ignore
      ...
      if (url != null &&
          !url.StartsWith("dotnet://", StringComparison.Ordinal)) {
      ...
      return new BreakpointRequest(){Assembly = parts.Assembly,
  ...
  internal class DebugStore {
    ...
    public async Task Load(SessionId sessionId, string[] loaded_files,
    ...
    */
    ...
    public SourceLocation FindBestBreakpoint(BreakpointRequest req) {
      var asm = assemblies.FirstOrDefault(
          a => a.Name.Equals(req.Assembly, StringComparison.OrdinalIgnoreCase));
      var src = asm ?.Sources?.FirstOrDefault(
                         s => s.DebuggerFileName.Equals(
                             req.File, StringComparison.OrdinalIgnoreCase));
    ...
    public SourceFile GetFileByUrlRegex(string urlRegex) {
      ...
      return AllSources().FirstOrDefault(
          file =>
              regex.IsMatch(file.Url.ToString()) || regex.IsMatch(file.DocUrl));
    }
