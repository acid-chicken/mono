...
namespace WebAssembly.Net.Debugging {
  internal class BreakpointRequest {
    ...
    public List<Breakpoint>Locations { get; }
    = new List<Breakpoint>();
    ...
    public object AsSetBreakpointByUrlResponse() => new {
        breakpointId = Id,
        locations = Locations.Select(l => l.Location.AsLocation())};

    public static BreakpointRequest Parse(string id, JObject args) {
      var breakRequest = new BreakpointRequest(){Id = id, request = args};
      return breakRequest;
    }

    public BreakpointRequest Clone() => new BreakpointRequest{
    ...
    public bool IsMatch(SourceFile sourceFile) {
      ...
      if (url == null) {
        ...
        return regex.IsMatch(sourceFile.Url.ToString()) ||
               regex.IsMatch(sourceFile.DocUrl);
      }
    ...
    static(string Assembly, string DocumentPath) ParseDocumentUrl(string url) {
      if (Uri.TryCreate(url, UriKind.Absolute, out var docUri) &&
          docUri.Scheme == "dotnet") {
        return (docUri.Host, docUri.PathAndQuery.Substring(1));
      } else {
        return (null, null);
      }
    }
  ...
  internal class SourceLocation {
    ...
    public override string ToString() { return $"{id}:{Line}:{Column}"; }
    ...
    internal object AsLocation() => new {
        scriptId = id.ToString(), lineNumber = line, columnNumber = column};
  }
  ...
  internal class MethodInfo {
    ...
    public MethodInfo(AssemblyInfo assembly, MethodDefinition methodDef,
                      SourceFile source) {
      this.assembly = assembly;
      ...
      foreach(var sp in sps) {
        if (sp.StartLine < start.StartLine)
          start = sp;
        else if (sp.StartLine == start.StartLine &&
                 sp.StartColumn < start.StartColumn)
          start = sp;
    ...
    public VarInfo[] GetLiveVarsAt(int offset) {
      ...
      res.AddRange(
          methodDef.DebugInformation.GetScopes()
              .Where(s => s.Start.Offset <= offset &&
  ...
  internal class AssemblyInfo {
    ...
    public AssemblyInfo(string url, byte[] assembly, byte[] pdb) {
      ...
      try {
        ...
        this.image =
            ModuleDefinition.ReadModule(new MemoryStream(assembly), rp);
      } catch (BadImageFormatException ex) {
        logger.LogWarning(
            $"Failed to read assembly as portable PDB: {ex.Message}");
    ...
    public AssemblyInfo(ILogger logger) { this.logger = logger; }
    ...
    void Populate() {
      ...
      Func<Document, SourceFile>get_src = (doc) => {
        ...
        if (d2s.ContainsKey(doc))
          return d2s[doc];
        var src =
            new SourceFile(this, sources.Count, doc, GetSourceLinkUrl(doc.Url));
        sources.Add(src);
        d2s[doc] = src;
        return src;
      };
      ...
      foreach(var m in image.GetTypes().SelectMany(t => t.Methods)) {
        ...
        if (first_doc == null) {
          // all generated files
          first_doc =
              m.DebugInformation.SequencePoints.FirstOrDefault() ?.Document;
        }
    ...
    private void ProcessSourceLink() {
      ...
      if (sourceLinkDebugInfo != null) {
        var sourceLinkContent =
            ((SourceLinkDebugInformation) sourceLinkDebugInfo).Content;
    ...
    private string GetRelativePath(string relativeTo, string path) {
      ...
      var rel =
          Uri.UnescapeDataString(
                 uri.MakeRelativeUri(new Uri(path, UriKind.RelativeOrAbsolute))
                     .ToString())
  ...
  internal class SourceFile {
    ...
    internal SourceFile(AssemblyInfo assembly, int id, Document doc,
      ...
      this.SourceUri =
          new Uri((Path.IsPathRooted(doc.Url) ? "file://" : "") + doc.Url,
    ...
    internal void AddMethod(MethodInfo mi) { this.methods.Add(mi); }
    ...
    public(int startLine, int startColumn, int endLine, int endColumn)
        GetExtents() {
      var start = methods.OrderBy(m => m.StartLocation.Line)
                      .ThenBy(m => m.StartLocation.Column)
                      .First();
      var end = methods.OrderByDescending(m => m.EndLocation.Line)
                    .ThenByDescending(m => m.EndLocation.Column)
                    .First();
      return (start.StartLocation.Line, start.StartLocation.Column,
              end.EndLocation.Line, end.EndLocation.Column);
    }
    ...
    public object ToScriptSource(int executionContextId,
  ...
  internal class DebugStore {
    ...
    public DebugStore(ILogger logger) { this.logger = logger; }
    ...
    public async IAsyncEnumerable<SourceFile>
    Load(SessionId sessionId, string[] loaded_files,
      ...
      foreach(var step in steps) {
        ...
        foreach(var source in assembly.Sources) yield return source;
      }
    }

    public IEnumerable<SourceFile>
    AllSources() => assemblies.SelectMany(a => a.Sources);

    public SourceFile GetFileById(SourceId id) =>
        AllSources().SingleOrDefault(f => f.SourceId.Equals(id));

    public AssemblyInfo
    GetAssemblyByName(string name) => assemblies.FirstOrDefault(
    ...
    */
    static bool Match(SequencePoint sp, SourceLocation start,
                      SourceLocation end) {
    ...
    public List<SourceLocation>FindPossibleBreakpoints(SourceLocation start,
                                                       SourceLocation end) {
      ...
      foreach(var method in doc.Methods) {
        foreach(var sequencePoint in
                    method.methodDef.DebugInformation.SequencePoints) {
    ...
    */
    static bool Match(SequencePoint sp, int line, int column) {
      ...
      // Chrome sends a zero column even if getPossibleBreakpoints say something
      // else
      if (column == 0)
        return true;
    ...
    public IEnumerable<SourceLocation>
    FindBreakpointLocations(BreakpointRequest request) {
      request.TryResolve(this);

      var asm = assemblies.FirstOrDefault(
          a => a.Name.Equals(request.Assembly,
                             StringComparison.OrdinalIgnoreCase));
      var sourceFile =
          asm ?.Sources?.SingleOrDefault(
                   s => s.DebuggerFileName.Equals(
                       request.File, StringComparison.OrdinalIgnoreCase));
