...
module.exports = function(config) {
  config.set({
    ...
    files : [
      {
        pattern : './publish/http-spec.js',
        watched : true,
        served : true,
        included : true
      },
      {
        pattern : './publish/http-spec-with-prom-lib.js',
        watched : true,
        served : true,
        included : true
      },
      {
        pattern : './publish/core-bindings-spec.js',
        watched : true,
        served : true,
        included : true
      },
      {
        pattern : './publish/issues-spec.js',
        watched : true,
        served : true,
        included : true
      },
      {
        pattern : './publish/zip-spec.js',
        watched : true,
        served : true,
        included : true
      },
      {
        pattern : './publish/dotnet.wasm',
        included : false,
        served : true,
        type : 'wasm'
      },
      {
        pattern : './publish/dotnet.worker.js',
        included : false,
        served : true,
        type : 'wasm'
      },
      {
        pattern : './publish/dotnet.js.mem',
        included : false,
        served : true,
        type : 'wasm'
      },
    ...
    junitReporter : {
      ...
      classNameFormatter :
          undefined,  // function (browser, result) to customize the classname
                      // attribute in xml testcase element
      properties : {} // key value pair of properties to add to the <properties>
                      // section of the report
    },
    // load karma-jasmine-dom and karma-jasmine
    frameworks :
        [ 'jasmine-dom', 'jasmine', 'mocha', 'chai', 'websocket-server' ],
    // load karma-chrome-launcher
    browsers : [ 'ChromeHeadless', 'NoSandBoxHeadless', 'SafariNative' ],
    customLaunchers : {
      NoSandBoxHeadless : {base : 'ChromeHeadless', flags : [ '--no-sandbox' ]}
    },
    logLevel : config.LOG_INFO,
    client : {
      // If false, Karma will not remove iframes upon the completion of running
      // the tests
      clearContext : false,
      ...
      karmaHTML : {
        source : [
          ...
          // the 'index' tag will be used to get the access to the Document
          // object of 'index.html'
          {src : './publish/http-spec.html', tag : 'httpspec'}, {
            src : './publish/http-spec-with-prom-lib.html',
            tag : 'httpspecwithpromlib'
          },
    ...
    websocketServer : {
      ...
      afterStart : (server) => { console.log('Server now listening!'); }
    }
  });
};
