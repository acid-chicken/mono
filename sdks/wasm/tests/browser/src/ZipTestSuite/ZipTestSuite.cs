...
public class Program {
  ...
  public static async Task<object>ZipGetEntryDeleteUpdateMode() {
    ...
    using(var memoryStream = await GetArchiveStreamAsync()) {
      using(var archive =
                new ZipArchive(memoryStream, ZipArchiveMode.Update, true)) {
        var entry = archive.GetEntry("foo.txt");
        entry.Delete();
      }
      using(var archive = new ZipArchive(memoryStream, ZipArchiveMode.Read)) {
  ...
  public static async Task<WebAssembly.Core.Array>
  ZipGetArchiveEntryStreamLengthPosition(ZipArchiveMode mode) {
  ...
  ZipGetArchiveEntryStreamLengthPositionReadMode() {
    var requestTcs = new TaskCompletionSource<object>();
    var resultsArray =
        await ZipGetArchiveEntryStreamLengthPosition(ZipArchiveMode.Read);
    requestTcs.SetResult(resultsArray);
    return requestTcs.Task;
  }
  public static async Task<object>
  ZipGetArchiveEntryStreamLengthPositionUpdateMode() {
    var requestTcs = new TaskCompletionSource<object>();
    var resultsArray =
        await ZipGetArchiveEntryStreamLengthPosition(ZipArchiveMode.Update);
    requestTcs.SetResult(resultsArray);
    return requestTcs.Task;
  }
  ...
  public static string Compress(string uncompressedString) {
    ...
    using(var uncompressedStream =
              new MemoryStream(Encoding.UTF8.GetBytes(uncompressedString))) {
      using(var compressedStream = new MemoryStream()) {
        // setting the leaveOpen parameter to true to ensure that
        // compressedStream will not be closed when compressorStream is disposed
        // this allows compressorStream to close and flush its buffers to
        // compressedStream and guarantees that compressedStream.ToArray() can
        // be called afterward although MSDN documentation states that ToArray()
        // can be called on a closed MemoryStream, I don't want to rely on that
        // very odd behavior should it ever change
        using(var compressorStream = new DeflateStream(
                  compressedStream, CompressionLevel.Fastest, true)) {
          uncompressedStream.CopyTo(compressorStream);
        }

        // call compressedStream.ToArray() after the enclosing DeflateStream has
        // closed and flushed its buffer to compressedStream
  ...
  static async Task<MemoryStream>GetArchiveStreamAsync() {
    ...
    using(HttpClient client = CreateHttpClient()) using(
        Stream stream =
            await client.GetStreamAsync("base/publish/archive.zip")) {
  ...
  static async Task<MemoryStream>GetPackageArchiveStreamAsync() {
    ...
    using(HttpClient client = CreateHttpClient()) using(
        Stream stream =
            await client.GetStreamAsync("base/publish/test.nupkg")) {
      await stream.CopyToAsync(archiveStream);
    }
    return archiveStream;
  }
  ...
  static HttpClient CreateHttpClient() {
    ...
    using(var window = (JSObject) WebAssembly.Runtime.GetGlobalObject(
              "window")) using(var location =
                                   (JSObject)
                                       window.GetObjectProperty("location")) {
