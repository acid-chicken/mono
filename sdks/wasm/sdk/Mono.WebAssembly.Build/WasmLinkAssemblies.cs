...
namespace Mono.WebAssembly.Build {
  public class WasmLinkAssemblies : ToolTask {
    ...
    /// </summary>
    public ITaskItem[] RuntimeCopyLocalAssemblies {
      get;
      set;
    }
    ...
    /// </summary>
    public ITaskItem[] LinkDescriptions {
      get;
      set;
    }
    ...
    /// </summary>
    [Required]
    public string BindingsDir {
      get;
      set;
    }

    protected override string ToolName =>
        (InvokeLinkerUsingMono) ? "mono" : "monolinker.exe";
    bool IsWindows =>
        System.Runtime.InteropServices.RuntimeInformation.IsOSPlatform(
            OSPlatform.Windows);
    bool InvokeLinkerUsingMono =>
        (!IsWindows && !String.IsNullOrEmpty(GetNetCoreVersion()));
    protected override string GenerateFullPathToTool() {
      Log.LogMessage(MessageImportance.High,
                     $"InvokeLinkerUsingMono {InvokeLinkerUsingMono}");
      if (!InvokeLinkerUsingMono) {
        var toolsPath = GetPathToMonoLinker();
        Log.LogMessage(MessageImportance.High,
                       $"Running monolinker from {toolsPath}");
        return toolsPath;
      } else
        return ToolName;
    }
    ...
    static List<string>bindingNames =
        new List<string>{"WebAssembly.Bindings", "WebAssembly.Net.Http",
    ...
    protected override string GenerateCommandLineCommands() {
      ...
      arguments = arguments.AddRange("--exclude-feature", "remoting",
                                     "--exclude-feature", "com",
                                     "--exclude-feature", "etw");
      ...
      arguments =
          arguments.AddRange("-d", Path.Combine(FrameworkDir, "Facades"));

      arguments = arguments.AddRange("-b", Debug.ToString());
      arguments = arguments.AddRange("-v", Debug.ToString());

      // add custom link descriptions
      // to ensure the type, methods and/or fields are not eliminated from your
      // application.
      ...
      arguments =
          arguments.AddRange("-a", RootAssembly[0].GetMetadata("FullPath"));

      // we'll normally have to check most of the files because the SDK
      // references most framework asm by default so let's enumerate upfront
      ...
      if (RuntimeCopyLocalAssemblies != null) {
        foreach(var copyAsm in RuntimeCopyLocalAssemblies) {
          ...
          if (frameworkAssemblies.Contains(
      ...
      if (Assemblies != null) {
        foreach(var asm in Assemblies) {
          ...
          if (runtimeCopyLocal.TryGetValue(Path.GetFileNameWithoutExtension(p),
                                           out var runtimePath)) {
            if (!string.IsNullOrEmpty(BindingsDir) &&
                bindingNames.Contains(Path.GetFileNameWithoutExtension(p))) {
    ...
    static string GetNetCoreVersion() {
      ...
      int netCoreAppIndex =
          Array.IndexOf(assemblyPath, "Microsoft.NETCore.App");
    ...
    string GetPathToMonoLinker() {
      ...
      var toolsPath =
          Path.Combine(Path.GetDirectoryName(dir), "tools", "monolinker.exe");
      if (!File.Exists(toolsPath))
        toolsPath = Path.GetFullPath(
            Path.Combine(GetParentDirectoryOf(dir, 6), "out", "wasm-bcl",
                         "wasm_tools", "monolinker.exe"));
      return toolsPath;
    }
  ...
  public enum WasmLinkMode { None, SdkOnly, Full }
}
