...
extern void *mono_wasm_invoke_js_marshalled(MonoString **exceptionMessage,
                                            void *asyncHandleLongPtr,
                                            MonoString *funcName,
                                            MonoString *argsJson);
extern void *mono_wasm_invoke_js_unmarshalled(MonoString **exceptionMessage,
                                              MonoString *funcName, void *arg0,
                                              void *arg1, void *arg2);
...
typedef struct {
  int version;
  void *(*lookup)(MonoMethod *method, char *classname, char *methodname,
                  char *sigstart, int32_t *uses_handles);
...
int mono_regression_test_step(int verbose_level, char *image,
                              char *method_name);
...
static MonoString *mono_wasm_invoke_js(MonoString *str, int *is_exception) {
  if (str == NULL)
    return NULL;
...
static void wasm_logger(const char *log_domain, const char *log_level,
                        const char *message, mono_bool fatal, void *user_data) {
  if (fatal) {
    EM_ASM(var err = new Error(); console.log("Stacktrace: \n");
...
EMSCRIPTEN_KEEPALIVE void mono_wasm_add_assembly(const char *name,
...
EMSCRIPTEN_KEEPALIVE void mono_wasm_setenv(const char *name,
                                           const char *value) {
...
static mono_bool wasm_dl_is_pinvoke_tables(void *handle) {
  for (int i = 0; i < sizeof(pinvoke_tables) / sizeof(void *); ++i) {
...
static void *wasm_dl_symbol(void *handle, const char *name, char **err,
                            void *user_data) {
#ifdef ENABLE_NETCORE
...
static int compare_int(const void *k1, const void *k2) {
  return *(int *)k1 - *(int *)k2;
}

static void *icall_table_lookup(MonoMethod *method, char *classname,
                                char *methodname, char *sigstart,
  ...
  *uses_handles = 0;
  ...
  if (!strcmp(image_name, "mscorlib") ||
      !strcmp(image_name, "System.Private.CoreLib")) {
  ...
  *uses_handles = handles[idx];
  ...
  // printf ("ICALL: %s %x %d %d\n", methodname, token, idx, (int)(funcs
  // [idx]));
...
static const char *icall_table_lookup_symbol(void *func) {
  assert(0);
  return NULL;
}
...
void mono_initialize_internals() {
  ...
  // TODO: what happens when two types in different assemblies have the same
  // FQN?

  // Blazor specific custom routines - see dotnet_support.js for backing code
  mono_add_internal_call(
      "WebAssembly.JSInterop.InternalCalls::InvokeJSMarshalled",
      mono_wasm_invoke_js_marshalled);
  mono_add_internal_call(
      "WebAssembly.JSInterop.InternalCalls::InvokeJSUnmarshalled",
...
EMSCRIPTEN_KEEPALIVE void mono_wasm_load_runtime(const char *managed_path,
  ...
  static const MonoIcallTableCallbacks mono_icall_table_callbacks = {
      MONO_ICALL_TABLE_CALLBACKS_VERSION, icall_table_lookup,
  ...
  if (assembly_count) {
    MonoBundledAssembly **bundle_array =
        g_new0(MonoBundledAssembly *, assembly_count + 1);
    ...
    mono_register_bundled_assemblies(
        (const MonoBundledAssembly **)bundle_array);
  }
...
EMSCRIPTEN_KEEPALIVE MonoAssembly *mono_wasm_assembly_load(const char *name) {
...
mono_wasm_assembly_find_class(MonoAssembly *assembly, const char *namespace,
                              const char *name) {
  return mono_class_from_name(mono_assembly_get_image(assembly), namespace,
                              name);
}

EMSCRIPTEN_KEEPALIVE MonoMethod *
mono_wasm_assembly_find_method(MonoClass *klass, const char *name,
                               int arguments) {
  return mono_class_get_method_from_name(klass, name, arguments);
}

EMSCRIPTEN_KEEPALIVE MonoObject *mono_wasm_invoke_method(MonoMethod *method,
                                                         MonoObject *this_arg,
                                                         void *params[],
                                                         MonoObject **out_exc) {
  MonoObject *exc = NULL;
  ...
  if (exc) {
    ...
    if (exc2)
      res =
          (MonoObject *)mono_string_new(root_domain, "Exception Double Fault");
    return res;
  }
...
EMSCRIPTEN_KEEPALIVE char *mono_wasm_string_get_utf8(MonoString *str) {
  return mono_string_to_utf8(str); // XXX JS is responsible for freeing this
}

EMSCRIPTEN_KEEPALIVE void mono_wasm_string_convert(MonoString *str) {
  if (str == NULL)
    return;
  ...
  EM_ASM({ MONO.string_decoder.decode($0, $0 + $1, true); }, (int)native_val,
         native_len);
}

EMSCRIPTEN_KEEPALIVE MonoString *mono_wasm_string_from_js(const char *str) {
  if (str)
...
static int class_is_task(MonoClass *klass) {
  if (!strcmp("System.Threading.Tasks", mono_class_get_namespace(klass)) &&
      (!strcmp("Task", mono_class_get_name(klass)) ||
...
EMSCRIPTEN_KEEPALIVE int mono_wasm_get_obj_type(MonoObject *obj) {
  if (!obj)
    return 0;
...
EMSCRIPTEN_KEEPALIVE double mono_wasm_unbox_float(MonoObject *obj) {
  if (!obj)
    return 0;
  MonoType *type = mono_class_get_type(mono_object_get_class(obj));
...
EMSCRIPTEN_KEEPALIVE int mono_wasm_array_length(MonoArray *array) {
  return mono_array_length(array);
}

EMSCRIPTEN_KEEPALIVE MonoObject *mono_wasm_array_get(MonoArray *array,
                                                     int idx) {
  return mono_array_get(array, MonoObject *, idx);
}

EMSCRIPTEN_KEEPALIVE MonoArray *mono_wasm_obj_array_new(int size) {
  return mono_array_new(root_domain, mono_get_object_class(), size);
}

EMSCRIPTEN_KEEPALIVE void mono_wasm_obj_array_set(MonoArray *array, int idx,
                                                  MonoObject *obj) {
  mono_array_setref(array, idx, obj);
}

EMSCRIPTEN_KEEPALIVE MonoArray *mono_wasm_string_array_new(int size) {
  return mono_array_new(root_domain, mono_get_string_class(), size);
}

EMSCRIPTEN_KEEPALIVE int mono_wasm_exec_regression(int verbose_level,
                                                   char *image) {
  return mono_regression_test_step(verbose_level, image, NULL) ? 0 : 1;
}

EMSCRIPTEN_KEEPALIVE int mono_wasm_exit(int exit_code) { exit(exit_code); }

EMSCRIPTEN_KEEPALIVE void mono_wasm_set_main_args(int argc, char *argv[]) {
  mono_runtime_set_main_args(argc, argv);
}

EMSCRIPTEN_KEEPALIVE int mono_wasm_strdup(const char *s) {
  return (int)strdup(s);
}

EMSCRIPTEN_KEEPALIVE void mono_wasm_parse_runtime_options(int argc,
                                                          char *argv[]) {
  mono_jit_parse_options(argc, argv);
}

EMSCRIPTEN_KEEPALIVE void mono_wasm_enable_on_demand_gc(void) {
...
void mono_timezone_get_local_name(MonoString **result) {
  // WASM returns back an int pointer to a string UTF16 buffer.
  // We then cast to `mono_unichar2*`.  Returning `mono_unichar2*` from the
  // JavaScript call will result in cast warnings from the compiler.
  mono_unichar2 *tzd_local_name =
      (mono_unichar2 *)mono_wasm_timezone_get_local_name();
