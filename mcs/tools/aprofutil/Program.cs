...
class MainClass {
  ...
  static string ProcessArguments(string[] args) {
    ...
    var options = new OptionSet{
        ...
        {"d|modules", "Show modules in the profile", v => Modules = true},
        ...
        {"m|methods", "Show methods in the profile", v => Methods = true},
        {"o|output=", "Write profile to {OUTPUT} file", v => Output = v},
        {"p|port=",
         "Read profile from aot profiler using local connection on {PORT}",
         v => int.TryParse(v, out Port)},
        {"s|summary", "Show summary of the profile", v => Summary = true},
        {"t|types", "Show types in the profile", v => Types = true},
        {"v|verbose",
    ...
    if (remaining.Count != 1 && Port < 0) {
      Error(
          "Please specify one <aotprofile-file> to process or network PORT with -p.");
      Environment.Exit(2);
    }
  ...
  static ProfileData ReadProfileFromPort(ProfileReader reader) {
    ...
    using(var client = new TcpClient("127.0.0.1", Port)) {
      using(var stream = client.GetStream()) {
        ...
        if (Verbose)
          ColorWriteLine($"Reading from '127.0.0.1:{Port}'...",
                         ConsoleColor.Yellow);
        ...
        using(var memoryStream = new MemoryStream(128 * 1024)) {
          ...
          if (Verbose)
            ColorWriteLine($"Read total {memoryStream.Length} bytes...",
                           ConsoleColor.Yellow);
  ...
  public static void Main(string[] args) {
    ...
    if (Types) {
      ...
      foreach(var type in types) WriteLine($"\t{type}");
    }

    if (Methods) {
      ColorWriteLine($"Methods:", ConsoleColor.Green);

      foreach(var method in methods) WriteLine($"\t{method}");
    }

    if (Summary) {
      ColorWriteLine($"Summary:", ConsoleColor.Green);
      WriteLine($"\tModules: {modules.Count.ToString (" N0
                "),10}{(modules.Count != pd.Modules.Length ? $"(
                    of{pd.Modules.Length}) " : " " )}");
      WriteLine($"\tTypes:   {types.Count.ToString (" N0
                "),10}{(types.Count != pd.Types.Length ? $"(
                    of{pd.Types.Length}) " : " ")}");
      WriteLine($"\tMethods: {methods.Count.ToString (" N0
                "),10}{(methods.Count != pd.Methods.Length ? $"(
                    of{pd.Methods.Length}) " : " ")}");
    }

    if (!string.IsNullOrEmpty(Output)) {
      if (Verbose)
        ColorWriteLine($"Going to write the profile to '{Output}'",
                       ConsoleColor.Yellow);
      ...
      var updatedPD =
          new ProfileData(modulesArray, typesArray, methods.ToArray());
  ...
  static void ColorMessage(string message, ConsoleColor color,
                           TextWriter writer, bool writeLine = true) {
  ...
  public static void ColorWriteLine(string message, ConsoleColor color) =>
      ColorMessage(message, color, Out);

  public static void ColorWrite(string message, ConsoleColor color) =>
      ColorMessage(message, color, Out, false);

  public static void
  Error(string message) => ColorMessage($"Error: {Name}: {message}",
                                        ConsoleColor.Red, Console.Error);

  public static void
  Warning(string message) => ColorMessage($"Warning: {Name}: {message}",
