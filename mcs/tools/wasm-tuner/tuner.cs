...
public class WasmTuner {
  ...
  void Usage() {
    ...
    Console.WriteLine(
        "--gen-interp-to-native <output file name> <assemblies>.");
  ...
  int GenInterpToNative(String[] args) {
    ...
    foreach(var pinvoke in pinvokes) gen.AddSignature(pinvoke.Method);
    using(var w = File.CreateText(outfileName)) { gen.Emit(w); }
    return 0;
  }
}