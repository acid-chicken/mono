...
//
// This class generates the icall_trampoline_dispatch () function used by the
// interpreter to call native code on WASM. It should be kept in sync with
// mono_wasm_interp_to_native_trampoline () in the runtime.
//
...
class InterpToNativeGenerator {
  ...
  public InterpToNativeGenerator() { extra_signatures = new List<string>(); }
  ...
  static char typeToChar(TypeReference type) {
    ...
    case MetadataType.ValueType:
      return 'I';
    default:
      throw new Exception("Can't handle type '" + type.FullName + "' (" +
                          type.MetadataType + ").");
    }
  }
  ...
  public void Emit(StreamWriter w) {
    ...
    if (args.Any(a => a == "--flat")) {
            for (int i = 0; i < cookies.Length; ++i) {
                    ...
                    w.WriteLine ($"\t\twasm_invoke_{c.ToLower ()} (target_func,
    margs);");
            }
            w.WriteLine ("\telse {");
            w.WriteLine ("\t\tg_error (\"CANNOT HANDLE COOKIE %s\\n\",
    cookie);"); w.WriteLine ("\t}"); } else {
    */
  ...
  static void WritePartition(StreamWriter w, IEnumerable<string>set,
    ...
    var groups = set.OrderBy(s => -s.Length)
                     .Where(s => s.Length > pos)
                     .GroupBy(s => s.Skip(pos).First().ToString());
    ...
    if (hits.Any()) {
      ...
      w.WriteLine(
          $"{prefix}\twasm_invoke_{h.ToLower ()} (target_func, margs);");
      w.WriteLine($"{prefix}\treturn;");
      w.WriteLine($"{prefix}}}");
    }

    string Elif(int c) { return c == 0 ? "if" : "else if"; }
  }
