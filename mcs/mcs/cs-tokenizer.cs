...
namespace Mono.CSharp {
  ...
  public class Tokenizer : yyParser.yyInput {
    ...
    sealed class IdentifiersComparer : IEqualityComparer<char[]> {
      ...
      public IdentifiersComparer(int length) { this.length = length; }
    ...
    public class LocatedTokenBuffer {
      ...
      public LocatedTokenBuffer() { buffer = new LocatedToken[0]; }
      ...
      public LocatedToken Create(string value, SourceFile file, int row,
                                 int column) {
        //
    ...
    //
    // This character is only tested just before the tokenizer is about to
    // report an error;   So on the regular operation mode, this addition will
    // have no impact on the tokenizer's performance.
    //
    ...
    static readonly KeywordEntry<PreprocessorDirective>[]
    ...
    public bool IsEscapedIdentifier(ATypeNameExpression name) {
      return escaped_identifiers != null &&
             escaped_identifiers.Contains(name.Location);
    }
    ...
    class Position {
      ...
      public Position(Tokenizer t) {
        ...
        string_interpolation_section = t.string_interpolation_section;
        if (t.parsing_string_interpolation_quoted != null &&
    ...
    public void PushPosition() { position_stack.Push(new Position(this)); }
    ...
    public void PopPosition() {
      ...
      parsing_string_interpolation_quoted =
    ...
    public void DiscardPosition() { position_stack.Pop(); }
    ...
    static void AddPreprocessorKeyword(string kw,
                                       PreprocessorDirective directive) {
      AddKeyword(keywords_preprocessor, kw, directive);
    }

    static void AddKeyword<T>(KeywordEntry<T>[][] keywords, string kw,
                              T token) {
      ...
      var kwe = keywords [length]
      [char_index];
      if (kwe == null) {
        keywords [length]
        [char_index] = new KeywordEntry<T>(kw, token);
        return;
      }
    ...
    int GetKeyword(char[] id, int id_len) {
      ...
      case Token.FROM:
        //
        // A query expression is any expression that starts with `from
        ...
        if (!query_parsing) {
          ...
          PushPosition();
          // HACK: to disable generics micro-parser, because PushPosition does
          // not store identifiers array
          ...
          case Token.ULONG:
            next_token = xtoken();
            if (next_token == Token.SEMICOLON || next_token == Token.COMMA ||
                next_token == Token.EQUALS || next_token == Token.ASSIGN)
              goto default;
          ...
          default:
            PopPosition();
            // HACK: A token is not a keyword so we need to restore identifiers
            // buffer which has been overwritten before we grabbed the
            // identifier
            id_builder[0] = 'f';
            id_builder[1] = 'r';
            id_builder[2] = 'o';
      ...
      case Token.PARTIAL:
        ...
        next_token = token();
        bool ok = next_token == Token.CLASS || next_token == Token.STRUCT ||
                  next_token == Token.INTERFACE || next_token == Token.VOID;
        ...
        // HACK: A token is not a keyword so we need to restore identifiers
        // buffer which has been overwritten before we grabbed the identifier
        id_builder[0] = 'p';
        id_builder[1] = 'a';
        id_builder[2] = 'r';
        id_builder[3] = 't';
      ...
      case Token.ASYNC:
        ...
        if (res == Token.ASYNC &&
            context.Settings.Version <= LanguageVersion.V_4) {
          Report.FeatureIsNotAvailable(context, Location,
    ...
    static PreprocessorDirective GetPreprocessorDirective(char[] id,
                                                          int id_len) {
      //
      // Keywords are stored in an array of arrays grouped by their
      // length and then by the first character
      //
      if (id_len >= keywords_preprocessor.Length ||
          keywords_preprocessor[id_len] == null)
      ...
      var kwe = keywords_preprocessor [id_len]
      [first_index];
      if (kwe == null)
        return PreprocessorDirective.Invalid;
    ...
    int TokenizePossibleNullableType() {
      ...
      if (next_token == -1) {
        ...
        default:
          ...
          next_token = colons != interrs && braces == 0 ? Token.INTERR_NULLABLE
                                                        : Token.INTERR;
          break;
        }
      }
    ...
    ILiteralConstant adjust_real(TypeCode t, Location loc) {
      ...
      case TypeCode.Decimal:
        try {
          return new DecimalLiteral(
              ...
              decimal.Parse(s, styles, csharp_format_info), loc);
        } catch (OverflowException) {
      ...
      case TypeCode.Single:
        try {
          return new FloatLiteral(context.BuiltinTypes,
      ...
      default:
        try {
          return new DoubleLiteral(context.BuiltinTypes,
    ...
    int escape(int c, out int surrogate) {
      ...
      default:
        surrogate = 0;
        Report.Error(1009, Location, "Unrecognized escape sequence `\\{0}'",
                     ((char) d).ToString());
        return d;
      }
    ...
    public bool advance() { return peek_char() != -1 || CompleteOnEOF; }
    ...
    PreprocessorDirective get_cmd_arg(out string arg) {
      ...
      var cmd = GetPreprocessorDirective(id_builder,
                                         TokenizePreprocessorKeyword(out c));
      ...
      int has_identifier_argument =
          (int)(cmd & PreprocessorDirective.RequiresArgument);
      int pos = 0;
    ...
    //
    void PreProcessDefinition(bool is_define, string ident,
                              bool caller_is_taking) {
      if (ident.Length == 0 || ident == "true" || ident == "false") {
        Report.Error(1001, Location,
    ...
    bool ParsePragmaChecksum() {
      ...
      //
      // guid is predefined hash algorithm guid
      // {406ea660-64cf-4c82-b6f0-42d48172a799} for md5
      //
      int c = get_char();
      ...
      if (context.Settings.GenerateDebugInfo) {
        ...
        if (file.HasChecksum) {
          if (!ArrayComparer.IsEqual(file.Checksum, chsum)) {
            ...
            Report.Warning(1697, 1, checksum_location,
    ...
    int TokenizePragmaWarningIdentifier(ref int c, ref bool identifier) {
      if ((c >= '0' && c <= '9') || is_identifier_start_character(c)) {
        ...
        if (c >= '0' && c <= '9') {
          ...
          //
          // LAMESPEC v6: No spec what identifier really is in this context, it
          // seems keywords are allowed too
          //
          int pos = 0;
          number = -1;
          id_builder[pos++] = (char) c;
    ...
    void ReadSingleLineComment() {
      if (peek_char() != '/')
        Report.Warning(1696, 1, Location,
    ...
    void ParsePragmaDirective() {
      int c;
      int length = TokenizePreprocessorKeyword(out c);
      if (length == pragma_warning.Length &&
              ...
              do {
                ...
                if (code > 0) {
                  if (identifier) {
                    ...
                    Report.RegisterWarningRegion(loc).WarningDisable(
                        loc, code, context.Report);
                  } else {
                    Report.RegisterWarningRegion(loc).WarningEnable(loc, code,
                                                                    context);
                  }
                }
              } while (code >= 0 && c != '\n' && c != -1 && c != UnicodeLS &&
      ...
      // #pragma checksum
      //
      if (length == pragma_checksum.Length &&
    ...
    void Error_UnexpectedDirective(string extra) {
      Report.Error(1028, Location, "Unexpected processor directive ({0})",
                   extra);
    }
    ...
    int consume_string(bool quoted) {
      ...
      while (true) {
        ...
        if (c == '"') {
          ...
          ILiteralConstant res =
              new StringLiteral(context.BuiltinTypes,
                                CreateStringFromBuilder(pos), start_location);
          val = res;
#if FULL_AST
          res.ParsedValue = quoted ? reader
                                .ReadChars(reader_pos - 2, reader.Position - 1)
        ...
        if (c == '\n' || c == UnicodeLS || c == UnicodePS) {
          if (!quoted) {
            ...
            val = new StringLiteral(context.BuiltinTypes,
    ...
    int consume_identifier(int c, bool quoted) {
      ...
      if (c == '\\') {
        ...
        if (quoted || is_identifier_start_character(c)) {
          ...
          Report.Error(1056, Location, "Unexpected character `\\{0}'",
                       c.ToString("x4"));
          return Token.ERROR;
        }
      }
      ...
      if (id_builder[0] >= '_' && !quoted) {
        ...
        if (keyword != -1) {
          val = ltb.Create(keyword == Token.AWAIT ? "await"
                           : null, current_source, ref_line, column);
          return keyword;
        }
      }
    ...
    string InternIdentifier(char[] charBuffer, int length) {
      ...
      if (identifiers_group != null) {
        ...
        identifiers_group = new Dictionary<char[], string>(
            capacity, new IdentifiersComparer(length));
    ...
    public int xtoken() {
      ...
      while ((c = get_char()) != -1) {
        ...
        /*				This is required for compatibility with
           .NET case 0xEF: if (peek_char () == 0xBB) { PushPosition (); get_char
           (); if (get_char () == 0xBF) continue; PopPosition ();
                                            }
                                            break;
        */
        case '\\':
        ...
        case '>':
          ...
          if (parsing_generic_less_than > 1 ||
        ...
        case '#':
          ...
          while ((c = get_char()) != -1) {
            if (col == 1) {
              ...
              // TODO: Implement comment support for disabled code and uncomment
              // this code
              //							if (c ==
              //'#') { 								Eror_WrongPreprocessorLocation (); 								return Token.ERROR;
              //							}
              continue;
            }
    ...
    int TokenizeBackslash() {
      ...
      ILiteralConstant res =
          new CharLiteral(context.BuiltinTypes, (char) c, start_location);
      val = res;
      c = get_char();

      if (c != '\'') {
        Report.Error(1012, start_location,
      ...
      while (true) {
        ...
        case '}':
          if (braces == 0) {
            putback_char = ch;
            if (pos == 0) {
              Report.Error(8089, Location,
                           "Empty interpolated expression format specifier");
            } else if (Array.IndexOf(simple_whitespaces,
                                     value_builder[pos - 1]) >= 0) {
              Report.Error(
                  8088, Location,
    ...
    private void update_formatted_doc_comment(int current_comment_start) {
      ...
      string[] lines =
          xml_comment_buffer.ToString(current_comment_start, length)
              .Replace("\r", "")
              .Split('\n');
      ...
      for (int i = 1; i < lines.Length; i++) {
        ...
        foreach(char c in head) if (c != ' ') return;
        lines[i] = s.Substring(idx + 1);
      }
      xml_comment_buffer.Remove(current_comment_start, length);
      xml_comment_buffer.Insert(current_comment_start,
    ...
    void reset_doc_comment() { xml_comment_buffer.Length = 0; }
