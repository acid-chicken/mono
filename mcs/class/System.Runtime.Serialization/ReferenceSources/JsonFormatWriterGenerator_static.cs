...
namespace System.Runtime.Serialization.Json {
  internal partial class JsonFormatWriterGenerator {
    partial class CriticalHelper {
      internal JsonFormatClassWriterDelegate
      GenerateClassWriter(ClassDataContract classContract) {
        return (XmlWriterDelegator xmlWriter, object obj,
                XmlObjectSerializerWriteContextComplexJson context,
                ClassDataContract dataContract,
                XmlDictionaryString[] memberNames) =>
                   new JsonFormatWriterInterpreter(classContract)
                       .WriteToJson(xmlWriter, obj, context, dataContract,
                                    memberNames);
      }
      ...
      GenerateCollectionWriter(CollectionDataContract collectionContract) {
        return (XmlWriterDelegator xmlWriter, object obj,
                ...
                CollectionDataContract dataContract) =>
                   new JsonFormatWriterInterpreter(collectionContract)
                       .WriteCollectionToJson(xmlWriter, obj, context,
                                              dataContract);
      }
    }
  }
  ...
  class JsonFormatWriterInterpreter {
    ...
    public JsonFormatWriterInterpreter(
    ...
    public void WriteToJson(XmlWriterDelegator xmlWriter, object obj,
                            XmlObjectSerializerWriteContextComplexJson context,
                            ClassDataContract dataContract,
                            XmlDictionaryString[] memberNames) {
      ...
      if (classContract.IsReadOnlyContract) {
        DataContract.ThrowInvalidDataContractException(
            classContract.SerializationExceptionMessage, null);
      }
    ...
    public void
    WriteCollectionToJson(XmlWriterDelegator xmlWriter, object obj,
                          XmlObjectSerializerWriteContextComplexJson context,
                          CollectionDataContract dataContract) {
      this.writer = xmlWriter;
      this.obj = obj;
      ...
      if (collectionContract.IsReadOnlyContract) {
        DataContract.ThrowInvalidDataContractException(
            collectionContract.SerializationExceptionMessage, null);
      }
    ...
    void InitArgs(Type objType) {
      if (objType == Globals.TypeOfDateTimeOffsetAdapter) {
        objLocal = DateTimeOffsetAdapter.GetDateTimeOffsetAdapter(
            (DateTimeOffset) obj);
      } else
    ...
    void InvokeOnSerializing(ClassDataContract classContract,
      ...
      if (classContract.OnSerializing != null) {
        classContract.OnSerializing.Invoke(
            objSerialized, new object[]{context.GetStreamingContext()});
      }
    }
      ...
      if (classContract.IsISerializable)
        ...
        if (classContract.HasExtensionData) {
          ExtensionDataObject extensionData =
              ((IExtensibleDataObject) objLocal).ExtensionData;
      ...
      if (collectionContract.Kind == CollectionKind.Array) {
        ...
        if (objLocal.GetType().GetElementType() != itemType)
          throw new InvalidCastException(
              string.Format("Cannot cast array of {0} to array of {1}",
                            objLocal.GetType().GetElementType(), itemType));
        ...
        // but there is no other way to check type mismatch.
        // CollectionSerialization.ArrayContract() shows that it is required.
        if (!collectionContract.UnderlyingType.IsAssignableFrom(
                objLocal.GetType()))
          throw new InvalidCastException(
              string.Format("Cannot cast {0} to {1}", objLocal.GetType(),
        ...
        case CollectionKind.GenericDictionary:
          incrementCollectionCountMethod =
              XmlFormatGeneratorStatics.IncrementCollectionCountGenericMethod
                  .MakeGenericMethod(Globals.TypeOfKeyValuePair.MakeGenericType(
                      collectionContract.ItemType.GetGenericArguments()));
          break;
        }
        if (incrementCollectionCountMethod != null)
          incrementCollectionCountMethod.Invoke(context,
        ...
        if (collectionContract.Kind == CollectionKind.GenericDictionary) {
          ...
          enumeratorType =
              Globals.TypeOfGenericDictionaryEnumerator.MakeGenericType(
                  keyValueTypes);
        } else if (collectionContract.Kind == CollectionKind.Dictionary) {
          isDictionary = true;
          keyValueTypes =
              new Type[]{Globals.TypeOfObject, Globals.TypeOfObject};
          ...
          if (enumeratorType.IsInterface) {
            ...
            if (moveNextMethod == null)
              moveNextMethod = CollectionDataContract.GetTargetMethodWithName(
                  Globals.MoveNextMethodName, enumeratorType,
                  ienumeratorInterface);
            if (getCurrentMethod == null)
              getCurrentMethod = CollectionDataContract.GetTargetMethodWithName(
                  Globals.GetCurrentMethodName, enumeratorType,
                  ienumeratorInterface);
          }
        }
        ...
        var enumerator =
            (IEnumerator) collectionContract.GetEnumeratorMethod.Invoke(
                objLocal, new object[0]);
        if (isDictionary) {
          ConstructorInfo dictEnumCtor = enumeratorType.GetConstructor(
              Globals.ScanAllMembers, null,
              new Type[]{Globals.TypeOfIDictionaryEnumerator}, null);
          enumerator =
          ...
          ConstructorInfo dictEnumCtor = enumeratorType.GetConstructor(
              Globals.ScanAllMembers, null, new Type[]{ctorParam}, null);
          enumerator = (IEnumerator) Activator.CreateInstance(
        ...
        if (writeSimpleDictionary) {
          ...
          while ((bool) moveNextMethod.Invoke(enumerator, empty_args)) {
            ...
            key = CodeInterpreter.GetMember(genericDictionaryKeyProperty,
                                            currentValue);
            value = CodeInterpreter.GetMember(genericDictionaryValueProperty,
                                              currentValue);
          ...
          while (enumerator != null && enumerator.MoveNext()) {
            ...
            if (!TryWritePrimitive(elementType, () => currentValue, null, null,
                                   itemName, 0)) {
              WriteStartElement(itemName, 0);
              if (isGenericDictionary || isDictionary) {
                ...
                // FIXME: this TypeHandle might be wrong; there is no easy way
                // to get Type for currentValue though.
                DataContractJsonSerializer.WriteJsonValue(
                    jc, writer, currentValue, context,
    ...
    int WriteMembers(ClassDataContract classContract,
      ...
      int memberCount =
          (classContract.BaseContract == null)
              ? 0
              : WriteMembers(classContract.BaseContract, extensionData,
      ...
      for (int i = 0; i < classContract.Members.Count; i++, memberCount++) {
        ...
        if (doWrite) {
          ...
          if (requiresNameAttribute ||
              !TryWritePrimitive(memberType, hasMemberValue ?() => memberValue
                                 : (Func<object>) null, member.MemberInfo,
                                   null /*arrayItemIndex*/, null /*nameLocal*/,
                                   i + childElementIndex)) {

            // Note: DataContractSerializer has member-conflict logic here to
            // deal with the schema export
            //       requirement that the same member can't be of two different
            //       types.
            if (requiresNameAttribute)
              XmlObjectSerializerWriteContextComplexJson
                  .WriteJsonNameWithMapping(writer, memberNames,
                                            i + childElementIndex);
            else
    ...
    void WriteEndElement() { writer.WriteEndElement(); }

    void WriteArrayAttribute() {
      writer.WriteAttributeString(
          null /* prefix */, JsonGlobals.typeString /* local name */,
          string.Empty /* namespace */, JsonGlobals.arrayString /* value */);
    }

    void WriteObjectAttribute() {
      writer.WriteAttributeString(
          null /* prefix */, JsonGlobals.typeString /* local name */,
          null /* namespace */, JsonGlobals.objectString /* value */);
    }

    void WriteValue(Type memberType, object memberValue) {
      Pointer memberValueRefPointer = null;
      if (memberType.IsPointer)
        memberValueRefPointer =
            (Pointer) JsonFormatGeneratorStatics.BoxPointer.Invoke(
                null, new object[]{memberValue, memberType});
      bool isNullableOfT =
      ...
      if (memberType.IsValueType && !isNullableOfT) {
        ...
        if (primitiveContract != null)
          primitiveContract.XmlFormatContentWriterMethod.Invoke(
              writer, new object[]{memberValue});
        else
          InternalSerialize(XmlFormatGeneratorStatics.InternalSerializeMethod,
                            () => memberValue, memberType, false);
      } else {
        bool isNull;
        if (isNullableOfT)
          memberValue =
              UnwrapNullableObject(() => memberValue, ref memberType,
                                   out isNull); // Leaves !HasValue on stack
        else
          isNull = memberValue == null;
        if (isNull)
          XmlFormatGeneratorStatics.WriteNullMethod.Invoke(
              context,
              new object[]{writer, memberType,
          ...
          if (primitiveContract != null &&
            ...
            if (isNull2) {
              ...
              InternalSerialize((isNullableOfT ? XmlFormatGeneratorStatics
    ...
    void InternalSerialize(MethodInfo methodInfo, Func<object>memberValue,
                           Type memberType, bool writeXsiType) {
      var v = memberValue();
    ...
    object UnwrapNullableObject(Func<object>memberValue, ref Type memberType,
                                out bool isNull) // Leaves !HasValue on stack
    {
      object v = memberValue();
      isNull = false;
      while (memberType.IsGenericType &&
             memberType.GetGenericTypeDefinition() == Globals.TypeOfNullable) {
        Type innerType = memberType.GetGenericArguments()[0];
        if ((bool) XmlFormatGeneratorStatics.GetHasValueMethod
    ...
    bool TryWritePrimitiveArray(Type type, Type itemType, Func<object>value,
      ...
      if (writeArrayMethod != null) {
        ...
        typeof (JsonWriterDelegator)
            .GetMethod(writeArrayMethod, Globals.ScanAllMembers, null,
                       new Type[]{type, typeof (XmlDictionaryString),
