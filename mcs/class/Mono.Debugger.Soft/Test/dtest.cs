...
public class DebuggerTests {
  ...
  public static bool listening =
      Environment.GetEnvironmentVariable("DBG_SUSPEND") != null;
  public static string runtime =
      Environment.GetEnvironmentVariable("DBG_RUNTIME");
  public static string runtime_args =
      Environment.GetEnvironmentVariable("DBG_RUNTIME_ARGS");
  public static string agent_args =
      Environment.GetEnvironmentVariable("DBG_AGENT_ARGS");
  ...
  Event GetNextEvent() {
    ...
    if (step_req != null && es[0] != null && es[0].Request != null &&
        es[0].Request is StepEventRequest &&
        ((StepEventRequest) es[0].Request).GetId() != step_req.GetId()) {
...
#if !MONODROID_TOOLS
  Diag.ProcessStartInfo CreateStartInfo(string app, string method = null,
    ...
    pi.Arguments = string.Join(
        " ",
        new string[]{runtime_args ?? "", runtimeParameters ?? "",
  ...
  void Start(string app, string method = null, bool forceExit = false) {
    ...
    if (!listening) {
      ...
      vm = VirtualMachineManager.Launch(
          pi, new LaunchOptions{AgentArgs = agent_args});
    } else
#endif
    {
  ...
  BreakpointEvent run_until(string name) {
    ...
    Assert.IsNotNull(m);
    // Console.WriteLine ("X: " + name + " " + m.ILOffsets.Count + " " +
    // m.Locations.Count);
  ...
  class ReusableBreakpoint {
    ...
    public void Disable() { req.Disable(); }
  }
  ...
  public static void ss_recursive_chaotic_trap(int n,
                                               List<RecursiveChaoticPoint>trace,
                                               ref bool didLast,
                                               ref bool didAny) {
    // Depth is calculated as:
    // Main + single_stepping + ss_recursive_chaotic + (n is 5 at outermost
    // frame and 0 at innermost frame) + ss_recursive_chaotic_trap
    trace.Add(new RecursiveChaoticPoint(true, "ss_recursive_chaotic_trap",
                                        5 - n + 5));
    didLast = true;
  }

  public static void ss_recursive_chaotic_at(string at, int n,
                                             List<RecursiveChaoticPoint>trace,
                                             ref bool didLast,
                                             ref bool didAny) {
    // This will be called after every return from a function. The other
    // function will return whether "step out" is currently active, and it will
    // be passed in here as didLast.
    if (didLast) {
      // Depth is calculated as:
      // Main + single_stepping + ss_recursive_chaotic + (n is 5 at outermost
      // frame and 0 at innermost frame)
      trace.Add(new RecursiveChaoticPoint(false, "ss_recursive_chaotic_" + at,
                                          5 - n + 4));
      didAny = true;
      didLast = false;
    }
  ...
  public static bool
  ss_recursive_chaotic_fizz(int n, List<RecursiveChaoticPoint>trace) {
  ...
  public static bool
  ss_recursive_chaotic_buzz(int n, List<RecursiveChaoticPoint>trace) {
    bool didLast = false, didAny = false;
    if (n > 0) {
      int next = n - 1;
  ...
  public static bool
  ss_recursive_chaotic_fizzbuzz(int n, List<RecursiveChaoticPoint>trace) {
    bool didLast = false, didAny = false;
    if (n > 0) {
      int next = n - 1;
      didLast = ss_recursive_chaotic_fizz(next, trace);
  ...
  public static void
  trace_ss_recursive_chaotic(List<RecursiveChaoticPoint>trace) {
  ...
  void AssertValue(object expected, object val) {
    ...
    } else if (val is StructMirror &&
               (val as StructMirror).Type.Name == "IntPtr") {
  ...
  MetadataAndPdbTest() {
    run_until("bp1");
    var assemblyMirrors =
        entry_point.DeclaringType.Assembly.Domain.GetAssemblies();
    ...
    foreach(var assemblyMirror in assemblyMirrors) {
      ...
      Assert.AreEqual(metadata.MainModule.Mvid,
      ...
      foreach(var type in metadata.Modules.SelectMany(x => x.GetTypes())) {
        ...
        foreach(var method in type.Methods) {
          var methodMirror =
              assemblyMirror.GetMethod(method.MetadataToken.ToUInt32());
          Assert.AreEqual(method.Name, methodMirror.Name);
          Assert.AreEqual(method.MetadataToken.ToInt32(),
                          methodMirror.MetadataToken);
  ...
  [Test]
  public void
  ClassLocalReflection() {
    vm.Detach();

    Start(dtest_app_path, "local-reflect");

    MethodMirror m =
        entry_point.DeclaringType.Assembly.GetType("LocalReflectClass")
            .GetMethod("RunMe");
    ...
    foreach(var location in m.Locations) if (location.LineNumber ==
                                             method_base_linum + 2) {
    ...
    foreach(var fi in thisType.GetFields()) if (fi.Name == "someField") thisFi =
        fi;
  ...
  SingleStepping() {
    ...
    // Testing stepping in, over and out with exception handlers in same or
    // caller method
    ...
    assert_location(
        step_over(),
        "ss7_2"); // step over ss7_2_2();, assume we are at "catch" now
    ...
    assert_location(
        step_over(),
        "ss7_2_1"); // move from { of method to "throw new Exception ();"
    assert_location(
        step_over(),
        "ss7_2"); // step over exception, being in ss7_2 means we are at catch
    ...
    step_req.AssemblyFilter = new AssemblyMirror[]{
        (e as BreakpointEvent).Method.DeclaringType.Assembly};
    assert_location(step_into(), "ss7_3"); // call "throw new Exception ();", we
                                           // assume we end up at "catch"
    ...
    assert_location(
        step_into(),
        "ss7_2"); // step in exception, being in ss7_2 means we are at catch
    step_req.Disable();
    ...
    // Check that step-over doesn't stop at inner frames with recursive
    // functions
    ...
    // Check that step-over stops correctly when inner frames with recursive
    // functions contain breakpoints
    e = run_until("ss_recursive2");
    ReusableBreakpoint breakpoint =
        new ReusableBreakpoint(this, "ss_recursive2_trap");
    try {
      breakpoint.Continue();
      e = breakpoint.lastEvent;
      req = create_step(e);
      for (int c = 1; c <= 4; c++) {
        // The first five times we try to step over this function, the
        // breakpoint will stop us
        ...
        e = step_out();
        e = step_over(); // Stepout gets us to ss_recursive2_trap ();, move to
                         // ss_recursive2 (next); line
        assert_location(e, "ss_recursive2");

        // Stack should consist of Main + single_stepping + (1 ss_recursive2
        // frame per loop iteration)
        Assert.AreEqual(c + 2, e.Thread.GetFrames().Length);
        e = step_over_or_breakpoint();
      }
      // At this point we should have escaped the breakpoints and this will be a
      // normal step stop
      assert_location(e, "ss_recursive2");
    ...
    // Check that step-out stops correctly when inner frames with recursive
    // functions contain breakpoints
    e = run_until("ss_recursive2");
    ...
    e = step_in_await("ss_await", e); // ss_await_1_exc (true, true).Wait
                                      // ();//in
    e = step_in_await("MoveNext", e); //{
    e = step_in_await("MoveNext", e); // var a = 1;
    e = step_in_await("MoveNext", e); // await Task.Delay (10);
    ...
    e = step_in_await("ss_await",
                      e); // ss_await_1_exc (true, true).Wait ();//over
    e = step_in_await("MoveNext", e);   //{
    e = step_over_await("MoveNext", e); // var a = 1;
    e = step_over_await("MoveNext", e); // await Task.Delay (10);
    ...
    e = step_over_await("MoveNext", e); // return a + 2;
    e = step_over_await("MoveNext", e); //}
    e = step_over_await("ss_await",
                        e); // ss_await_1_exc (true, true).Wait ();//over

    e = step_in_await("ss_await",
                      e); // ss_await_1_exc (true, true).Wait ();//out
    e = step_in_await("MoveNext", e); //{
    e = step_out_await("ss_await",
                       e); // ss_await_1_exc (true, true).Wait ();//out

    e = step_in_await("ss_await", e); // try {
    e = step_in_await("ss_await",
                      e); // ss_await_1_exc (true, false).Wait ();//in
    e = step_in_await("MoveNext", e); //{
    e = step_in_await("MoveNext", e); // var a = 1;
    e = step_in_await("MoveNext", e); // await Task.Delay (10);
    e = step_in_await("MoveNext", e); // if (exc)
    ...
    e = step_in_await("ss_await",
                      e); // ss_await_1_exc (true, false).Wait ();//over
    e = step_in_await("MoveNext", e);   //{
    e = step_over_await("MoveNext", e); // var a = 1;
    e = step_over_await("MoveNext", e); // await Task.Delay (10);
    e = step_over_await("MoveNext", e); // if (exc)
    ...
    e = step_in_await("ss_await",
                      e); // ss_await_1_exc (true, false).Wait ();//out
    e = step_in_await("MoveNext", e); //{
    e = step_out_await("ss_await",
                       e); // ss_await_1_exc (true, true).Wait ();//out
  }
  ...
  Event step_in_await(string method, Event e) {
    if (step_req != null)
      step_req.Disable();
    create_step(e);
    step_req.AssemblyFilter =
        new List<AssemblyMirror>(){entry_point.DeclaringType.Assembly};
    var ef = step_into();
  ...
  SingleSteppingNoFrames() {
    ...
    AssertThrows<Exception>(delegate { step_req.Enable(); });
  }
  ...
  Arrays() {
    ...
    arr.SetValues(0, new Value[]{vm.RootDomain.CreateString("D1"),
                                 vm.RootDomain.CreateString("D2")});
    ...
    // Argument checking
    AssertThrows<IndexOutOfRangeException>(delegate() { val = arr[2]; });

    AssertThrows<IndexOutOfRangeException>(
        delegate() { val = arr[Int32.MinValue]; });

    AssertThrows<IndexOutOfRangeException>(
        delegate() { vals = arr.GetValues(0, 3); });

    AssertThrows<IndexOutOfRangeException>(
        delegate() { arr[2] = vm.CreateValue(null); });

    AssertThrows<IndexOutOfRangeException>(
        delegate() { arr[Int32.MinValue] = vm.CreateValue(null); });
    ...
    AssertThrows<ArgumentOutOfRangeException>(
        delegate() { arr.GetLength(-1); });
    AssertThrows<ArgumentOutOfRangeException>(delegate() { arr.GetLength(2); });

    AssertThrows<ArgumentOutOfRangeException>(
        delegate() { arr.GetLowerBound(-1); });
    AssertThrows<ArgumentOutOfRangeException>(
        delegate() { arr.GetLowerBound(2); });
  ...
  Object_GetValue() {
    ...
    // generic instances
    ObjectMirror o2 =
        frame.GetValue(frame.Method.GetParameters()[1]) as ObjectMirror;
    ...
    ObjectMirror o3 =
        frame.GetValue(frame.Method.GetParameters()[2]) as ObjectMirror;
    ...
    // Argument checking
    AssertThrows<ArgumentNullException>(delegate() { o.GetValue(null); });
  }
  ...
  Object_GetValues() {
    ...
    ObjectMirror val2 =
        frame.GetValue(frame.Method.GetParameters()[0]) as ObjectMirror;
    ...
    // Argument checking
    AssertThrows<ArgumentNullException>(delegate() { o.GetValues(null); });

    AssertThrows<ArgumentNullException>(
        delegate() { o.GetValues(new FieldInfoMirror[]{null}); });

    // field of another class
    AssertThrows<ArgumentException>(
        delegate() { o.GetValue(val2.Type.GetField("field_j")); });
  }

  void TestSetValue(ObjectMirror o, string field_name, object val) {
    if (val is string)
      o.SetValue(o.Type.GetField(field_name),
                 vm.RootDomain.CreateString((string) val));
    else
  ...
  Object_SetValues() {
    ...
    // Argument checking
    AssertThrows<ArgumentNullException>(
        delegate() { o.SetValues(null, new Value[0]); });

    AssertThrows<ArgumentNullException>(
        delegate() { o.SetValues(new FieldInfoMirror[0], null); });
    ...
    AssertThrows<ArgumentException>(delegate() {
      o.SetValue(o.Type.GetField("field_struct"),
                 o.GetValue(o.Type.GetField("field_enum")));
    });

    // reference type not assignment compatible
    AssertThrows<ArgumentException>(
        delegate() { o.SetValue(o.Type.GetField("field_class"), o); });
  ...
  Type_SetValue() {
    ...
    o.Type.SetValue(o.Type.GetField("static_s"),
                    vm.RootDomain.CreateString("B"));
    f = o.Type.GetValue(o.Type.GetField("static_s"));
    AssertValue("B", f);

    // Argument checking
    AssertThrows<ArgumentNullException>(
        delegate() { o.Type.SetValue(null, vm.CreateValue(0)); });

    AssertThrows<ArgumentNullException>(
        delegate() { o.Type.SetValue(o.Type.GetField("static_i"), null); });
  ...
  TypeInfo() {
    ...
    var nested =
        (from nt in t.GetNestedTypes() where nt.IsNestedPublic select nt)
            .ToArray();
    ...
    foreach(var attr in attrs) {
      if (attr.Constructor.DeclaringType.Name == "DebuggerDisplayAttribute") {
        ...
        Assert.IsInstanceOfType(typeof (TypeMirror),
                                attr.NamedArguments[1].TypedValue.Value);
        Assert.AreEqual(
            "Int32",
            (attr.NamedArguments[1].TypedValue.Value as TypeMirror).Name);
      } else if (attr.Constructor.DeclaringType.Name ==
                 "DebuggerTypeProxyAttribute") {
        Assert.AreEqual(1, attr.ConstructorArguments.Count);
        Assert.IsInstanceOfType(typeof (TypeMirror),
                                attr.ConstructorArguments[0].Value);
        Assert.AreEqual(
            "Tests", (attr.ConstructorArguments[0].Value as TypeMirror).Name);
  ...
  FieldInfo() {
    ...
    var attr = attrs[0];
    Assert.AreEqual("DebuggerBrowsableAttribute",
                    attr.Constructor.DeclaringType.Name);
    Assert.AreEqual(1, attr.ConstructorArguments.Count);
    Assert.IsInstanceOfType(typeof (EnumMirror),
                            attr.ConstructorArguments[0].Value);
    Assert.AreEqual((int) System.Diagnostics.DebuggerBrowsableState.Collapsed,
  ...
  Type_GetValue() {
    ...
    // Argument checking
    AssertThrows<ArgumentNullException>(delegate() { t.GetValue(null); });

    // instance fields
    AssertThrows<ArgumentException>(
        delegate() { t.GetValue(o.Type.GetField("field_i")); });

    // field on another type
    AssertThrows<ArgumentException>(
        delegate() { t.GetValue(val2.Type.GetField("static_field_j")); });

    // special static field
    AssertThrows<ArgumentException>(
        delegate() { t.GetValue(t.GetField("tls_i")); });
  }
  ...
  Type_GetValues() {
    ...
    // Argument checking
    AssertThrows<ArgumentNullException>(delegate() { t.GetValues(null); });

    AssertThrows<ArgumentNullException>(
        delegate() { t.GetValues(new FieldInfoMirror[]{null}); });
  }
  ...
  VTypes() {
    ...
    // Argument checking
    s = frame.GetArgument(0) as StructMirror;
    AssertThrows<ArgumentException>(delegate() { obj = s["FOO"]; });
  ...
  AssemblyInfo() {
    ...
    // frame.Method.DeclaringType.Assembly.GetAssemblyObject ().Type.Name);
  ...
  Locals() {
    ...
    // GetValue () null
    AssertThrows<ArgumentNullException>(
        delegate() { frame.GetValue((LocalVariable) null); });
    // GetValue () local from another method
    AssertThrows<ArgumentException>(
        delegate() { frame.GetValue(m1.GetLocal("foo")); });

    // GetValue () null
    AssertThrows<ArgumentNullException>(
        delegate() { frame.GetValue((ParameterInfoMirror) null); });
    // GetValue () local from another method
    AssertThrows<ArgumentException>(
        delegate() { frame.GetValue(m1.GetParameters()[0]); });

    // GetValues () null
    AssertThrows<ArgumentNullException>(delegate() { frame.GetValues(null); });
    // GetValues () embedded null
    AssertThrows<ArgumentNullException>(
        delegate() { frame.GetValues(new LocalVariable[]{null}); });
    ...
    // return value
    AssertThrows<ArgumentException>(
        delegate() { val = frame.GetValue(frame.Method.ReturnParameter); });
    ...
    AssertThrows<InvalidStackFrameException>(
        delegate() { frame.GetValue(frame.Method.GetLocal("i")); });
  ...
  Exit() {
    ...
    if (p != null) {
      ...
      // error handling
      AssertThrows<VMDisconnectedException>(delegate() { vm.Resume(); });
    }
  ...
  public void
  Crash() {
    string[] existingCrashFileEntries =
        Directory.GetFiles(".", "mono_crash*.json");
      ...
      try {
        ...
        while (!success) {
          ...
          break;
                   ...
                   vmDisconnect) { // expected behavior because of unreliability
        ...
        try {
          vm.Detach();
                     ...
                     vmDisconnect) { // expected behavior because of
                                     // unreliability of the crash reporter.
          success = false;
        } finally {
          vm = null;
        }
  ...
  ColumnNumbers() {
    ...
    // Do an additional step over so we are not on the beginning line of the
    // method
    step_req.Disable();
    step_req.Depth = StepDepth.Over;
    step_req.Enable();
  ...
  Suspend() {
    ...
    AssertThrows<InvalidOperationException>(delegate() { vm.Resume(); });
  ...
  AssemblyLoad() {
    ...
    Assert.IsTrue(
        (e as AssemblyLoadEvent).Assembly.Location.EndsWith("System.dll"));
  ...
  CreateValue() {
    ...
    // Argument checking
    AssertThrows<ArgumentException>(delegate() { v = vm.CreateValue("FOO"); });
  }
  ...
  CreateString() {
    ...
    // Argument checking
    AssertThrows<ArgumentNullException>(
        delegate() { s = vm.RootDomain.CreateString(null); });
  }
  ...
  CreateBoxedValue() {
    ...
    // Argument checking
    AssertThrows<ArgumentNullException>(
        delegate() { vm.RootDomain.CreateBoxedValue(null); });

    AssertThrows<ArgumentException>(
        delegate() { vm.RootDomain.CreateBoxedValue(o); });
  }
  ...
  Invoke() {
    ...
    // m = vm.RootDomain.Corlib.GetType ("System.Object").GetMethod
    // ("ToString");
    ...
    Value[] args = new Value[]{vm.CreateValue((byte) Byte.MaxValue),
    ...
    AssertValue((int) Byte.MaxValue + (int) SByte.MaxValue + 1 + 1 + 1 + 1 + 1 +
                    1 + 'A' + 1 + 3 + 3,
                v);

    // pass ref
    m = t.GetMethod("invoke_pass_ref");
    v = this_obj.InvokeMethod(e.Thread, m,
    ...
    var out_task = this_obj.InvokeMethodAsyncWithResult(
        e.Thread, m, new Value[]{vm.CreateValue(1), vm.CreateValue(null)},
    ...
    var task = this_obj.InvokeMethodAsync(
        e.Thread, m, new Value[]{vm.RootDomain.CreateString("ABC")});
    AssertValue("ABC", task.Result);

    // static
    m = t.GetMethod("invoke_static_pass_ref");
    task = t.InvokeMethodAsync(e.Thread, m,
                               new Value[]{vm.RootDomain.CreateString("ABC")});
    AssertValue("ABC", task.Result);

    // string constructor
    var stringType = vm.RootDomain.Corlib.GetType("System.String");
    var stringConstructor = stringType.GetMethods().Single(
        c => c.Name == ".ctor" && c.GetParameters().Length == 2 &&
             c.GetParameters()[0].ParameterType.Name == "Char" &&
             c.GetParameters()[1].ParameterType.Name == "Int32");
    var str = stringType.NewInstance(
        e.Thread, stringConstructor,
    ...
    // null method
    AssertThrows<ArgumentNullException>(delegate {
      v = this_obj.InvokeMethod(e.Thread, null,
                                new Value[]{vm.CreateValue(null)});
    });

    // invalid number of arguments
    m = t.GetMethod("invoke_pass_ref");
    AssertThrows<ArgumentException>(
        delegate { v = this_obj.InvokeMethod(e.Thread, m, null); });
  ...
  InvokeVType() {
    ...
    t = s.Type;
    m = t.GetMethods().First(method => method.Name == ".ctor" &&
    ...
    var task = s.InvokeMethodAsyncWithResult(e.Thread, m, null,
  ...
  InvokeSingleThreaded() {
    ...
    // Check that the counter value incremented by the other thread does not
    // increase during the invoke.
    object counter1 =
        (obj.GetValue(obj.Type.GetField("counter")) as PrimitiveValue).Value;
    ...
    obj.BeginInvokeMethod(e.Thread, m, null, InvokeOptions.SingleThreaded,
                          delegate { vm.Resume(); }, null);
  ...
  InvokeMultiple() {
    ...
    var r = this_obj.BeginInvokeMultiple(e.Thread, methods, null,
                                         InvokeOptions.SingleThreaded,
                                         invoke_multiple_cb, this_obj);
  ...
  void invoke_multiple_cb(IAsyncResult ar) {
    ...
    lock(invoke_results) invoke_results.Add(res);
  }
  ...
  InvokeAbort() {
    ...
    // Invoke multiple times to check that the subsequent invokes are aborted
    // too
    var res = (IInvokeAsyncResult) obj.BeginInvokeMultiple(
        e.Thread, new MethodMirror[]{m, m, m, m}, null, InvokeOptions.None,
        delegate{}, null);
    Thread.Sleep(500);
    res.Abort();
    AssertThrows<CommandException>(delegate { obj.EndInvokeMethod(res); });
  }
  ...
  Threads() {
    ...
    Assert.IsTrue(state == ThreadState.Running ||
  ...
  Exceptions() {
    ...
    Assert.AreEqual("OverflowException",
                    (e as ExceptionEvent).Exception.Type.Name);
    ...
    e = GetNextEvent();
    Assert.IsInstanceOfType(typeof (ExceptionEvent), e);
    Assert.AreEqual("ArgumentException",
                    (e as ExceptionEvent).Exception.Type.Name);
    req.Disable();
    ...
    e = GetNextEvent();
    Assert.IsInstanceOfType(typeof (ExceptionEvent), e);
    Assert.AreEqual("NullReferenceException",
                    (e as ExceptionEvent).Exception.Type.Name);
    req.Disable();
    ...
    // Argument checking
    AssertThrows<ArgumentException>(
        delegate { vm.CreateExceptionRequest(e.Thread.Type); });
  }
  ...
  ExceptionFilter() {
    ...
    MethodMirror m =
        entry_point.DeclaringType.GetMethod("exception_filter_filter");
    Assert.IsNotNull(m);
  ...
  // handler, since the sigsegv signal is disabled until control returns from
  // the signal handler.
  //
  [Test]
  [Category("only3")]
  public void
  NullRefExceptionAndSingleThreadedInvoke() {
    Event e = run_until("exceptions");
    ...
    var tostring_method =
        vm.RootDomain.Corlib.GetType("System.Object").GetMethod("ToString");
    ex.InvokeMethod(e.Thread, tostring_method, null,
                    InvokeOptions.SingleThreaded);
  }
  ...
  MemberInOtherDomain() {
    ...
    var crossDomainField = (ObjectMirror) inOtherDomain.GetValue(
        inOtherDomain.Type.GetField("printMe"));
  ...
  Domains() {
    ...
    var v =
        o.InvokeMethod(e.Thread, cross_domain_type.GetMethod("invoke_3"), null);
    AssertValue(42, v);
    ...
    var anenum = vm.CreateEnumMirror(
        d_method.DeclaringType.Assembly.GetType("AnEnum"), vm.CreateValue(1));
    ...
    while (true) {
      ...
      if (e is AssemblyUnloadEvent) {
        AssertThrows<Exception>(delegate() {
          var assembly_obj =
              (e as AssemblyUnloadEvent).Assembly.GetAssemblyObject();
        });
        continue;
      } else {
        break;
    ...
    AssertThrows<Exception>(delegate {
      d_method.DeclaringType.GetValue(
          d_method.DeclaringType.GetField("static_i"));
    });
    ...
    foreach(var f in e.Thread.GetFrames()) { var dom = f.Domain; }
  }
  ...
  StackTraceInNative() {
    ...
    AssertThrows<InvalidOperationException>(
        delegate { t.InvokeMethod(e.Thread, m, null); });

    // Check that the frame info is invalidated
    run_until("frames_in_native_2");

    AssertThrows<InvalidStackFrameException>(
        delegate { Console.WriteLine(frames[frame_index].GetThis()); });
  }

  [Test]
  public void
  VirtualMachine_CreateEnumMirror() {
    var e = run_until("o1");
    var frame = e.Thread.GetFrames()[0];
    ...
    // Argument checking
    AssertThrows<ArgumentNullException>(
        delegate() { vm.CreateEnumMirror(enumType, null); });

    AssertThrows<ArgumentNullException>(
        delegate() { vm.CreateEnumMirror(null, vm.CreateValue(1)); });

    // null value
    AssertThrows<ArgumentException>(
        delegate() { vm.CreateEnumMirror(enumType, vm.CreateValue(null)); });
  ...
  [Test]
  public void
  VirtualMachine_EnableEvents_Breakpoint() {
    AssertThrows<ArgumentException>(
        delegate() { vm.EnableEvents(EventType.Breakpoint); });
  }

  [Test]
  public void
  SingleStepRegress654694() {
    int il_offset = -1;
  ...
  TypeGetMethodsByNameFlags() {
    ...
    mm = type.GetMethodsByNameFlags(
        null, BindingFlags.Static | BindingFlags.Public, false);
    Assert.AreEqual(1, mm.Length, "#1");
    Assert.AreEqual("M1", mm[0].Name, "#2");

    mm = type.GetMethodsByNameFlags(
        null, BindingFlags.Static | BindingFlags.NonPublic, false);
    Assert.AreEqual(1, mm.Length, "#3");
    Assert.AreEqual("M2", mm[0].Name, "#4");

    mm = type.GetMethodsByNameFlags(
        null, BindingFlags.Instance | BindingFlags.Public, false);
    Assert.AreEqual(
        7, mm.Length,
        "#5"); // M3 plus Equals, GetHashCode, GetType, ToString, .ctor
    ...
    mm = type.GetMethodsByNameFlags(null,
    ...
    // Now with name
    mm = type.GetMethodsByNameFlags(
        "M1", BindingFlags.Static | BindingFlags.Public, false);
    Assert.AreEqual(1, mm.Length, "#12");
    Assert.AreEqual("M1", mm[0].Name, "#13");

    mm = type.GetMethodsByNameFlags(
        "m1", BindingFlags.Static | BindingFlags.Public, true);
  ...
  TypeLoadSourceFileFilter() {
    ...
    string srcfile =
        (e as BreakpointEvent).Method.DeclaringType.GetSourceFiles(true)[0];
  ...
  String_GetChars() {
    ...
    AssertThrows<ArgumentException>(delegate() { s.GetChars(2, 2); });
  }
  ...
  [Test]
  public void
  StackAlloc_Breakpoints_Regress2775() {
  ...
  MethodInfo() {
    ...
    Assert.AreEqual(1, attrs.Length);
    Assert.AreEqual("StateMachineAttribute",
                    attrs[0].Constructor.DeclaringType.Name);
  }
  ...
  UnhandledException_2() {
    ...
    MethodMirror m =
        entry_point.DeclaringType.GetMethod("unhandled_exception_endinvoke_2");
    Assert.IsNotNull(m);
  ...
  GCWhileSuspended() {
    ...
    var o = entry_point.DeclaringType.GetValue(
        entry_point.DeclaringType.GetField("gc_suspend_field")) as ObjectMirror;
    // Console.WriteLine (o);
    ...
    var o2 = entry_point.DeclaringType.GetValue(
        entry_point.DeclaringType.GetField("gc_suspend_field")) as ObjectMirror;
    Assert.IsNull(o2);
  }
  ...
  MakeGenericMethod() {
    ...
    // Error checking
    AssertThrows<ArgumentNullException>(
        delegate { gm.MakeGenericMethod(null); });
    AssertThrows<ArgumentNullException>(
        delegate { gm.MakeGenericMethod(new TypeMirror[]{null}); });
    AssertThrows<ArgumentException>(delegate {
      gm.MakeGenericMethod(new TypeMirror[]{stringm, stringm});
    });
    AssertThrows<InvalidOperationException>(
        delegate { gm.MakeGenericMethod(new TypeMirror[]{intm}); });
    AssertThrows<InvalidOperationException>(delegate {
      entry_point.DeclaringType.GetMethod("Main").MakeGenericMethod(
          new TypeMirror[]{intm});
    });
  }
  ...
  [Test]
  public void
  InspectThreadSuspenedOnWaitOne() {
    TearDown();
  ...
  SetIP() {
    ...
    var next_loc =
        locs.First(l => (l.LineNumber == frames[0].Location.LineNumber + 3));
    ...
    // Error handling
    AssertThrows<ArgumentNullException>(delegate { e.Thread.SetIP(null); });

    AssertThrows<ArgumentException>(delegate { e.Thread.SetIP(invalid_loc); });
  }
  ...
  SetIP2() {
    ...
    Assert.AreEqual(next_loc.ILOffset,
                    e.Thread.GetFrames()[0].Location.ILOffset);
    /* Check that i ++; j = 5; was skipped */
    bevent = run_until("set_ip_2");
    var f = bevent.Thread.GetFrames()[1];
    AssertValue(2, f.GetValue(f.Method.GetLocal("i")));
  ...
  SetIPSingleStep() {
    ...
    var locs = frames[0].Method.Locations;
    var prev_loc =
        locs.First(l => (l.LineNumber == frames[0].Location.LineNumber - 1));
  ...
  [Test]
  public void
  StepOverOnExitFromArgsAfterStepInMethodParameter2() {
  ...
  [Test]
  public void
  StepOverOnExitFromArgsAfterStepInMethodParameter3() {
  ...
  [Test]
  public void
  ShouldCorrectlyStepOverOnExitFromArgsAfterStepInMethodParameter() {
  ...
  [Test]
  public void
  InspectEnumeratorInGenericStruct() {
    // files.myBucket.GetEnumerator().get_Current().Key watching this generates
    // an exception in Debugger
    ...
    TypeMirror thisType = thisObj.Type;
    variable =
        thisObj.InvokeMethod(e.Thread, thisType.GetMethod("get_Current"), null);
    ...
    while (true) {
      ...
      if (e is AssemblyLoadEvent) {
        ...
        if (amirror.GetName().Name == "dtest-app")
          ...
          vm.SetBreakpoint(
              amirror.EntryPoint.DeclaringType.GetMethod("attach_break"), 0);
      }
  ...
  Hash() {
    ...
    using(FileStream fs =
              new FileStream(l.SourceFile, FileMode.Open, FileAccess.Read)) {
  ...
  Pointer_GetValue() {
    ...
    var pointerValue2 =
        new PointerValue(pointerValue.VirtualMachine, pointerValue.Type,
                         pointerValue.Address +
                             pointerValue.Type.GetElementType().GetValueSize());
  ...
  FieldWithUnsafeCastValue() {
    ...
    TypeMirror thisType = thisObj.Type;
    variable =
        thisObj.InvokeMethod(e.Thread, thisType.GetMethod("ToString"), null);
  ...
  IfPropertyStepping() {
    ...
    e = step_into();
    e = step_into();
    Assert.IsTrue((e as StepEvent).Method.Name == "op_Equality" ||
  ...
  void ShouldNotSendUnexpectedTypeLoadEventsAndInvalidSuspendPolicyAfterAttach(
      int port) {
    string failMessage = null;
    try {
      vm = ConnectToPort(port);
  ...
  [Test]
  public void
  ShouldNotSendUnexpectedTypeLoadEventsAndInvalidSuspendPolicyAfterAttachIfSuspendIsTrue() {
    vm.Exit(0);
    var port = GetFreePort();
    ...
    ShouldNotSendUnexpectedTypeLoadEventsAndInvalidSuspendPolicyAfterAttach(
        port);
  }
  ...
  [Category("NotWorking")]
  public void
  ShouldNotSendUnexpectedTypeLoadEventsAndInvalidSuspendPolicyAfterAttachIfSuspendIsFalse() {
    vm.Exit(0);
    var port = GetFreePort();
  ...
  InvokeMethodFixedArray() {
    ...
    var min =
        (StringMirror) n.InvokeMethod(thread, get_Min, Array.Empty<Value>());
  ...
  [Test]
  public void
  TestExceptionFilterWin() {
    Event e = run_until("test_new_exception_filter1");
    var req2 = vm.CreateExceptionRequest(
        vm.RootDomain.Corlib.GetType("System.Exception"), true, true, false);
    req2.Enable();
    vm.Resume();
    ...
    var req3 = vm.CreateExceptionRequest(
        vm.RootDomain.Corlib.GetType("System.ArgumentException"), false, true,
        false);
    req3.Enable();
  ...
  [Test]
  public void
  TestRuntimeInvokeHybridSuspendExceptions() {
    TearDown();
  ...
  [Test]
  public void
  TestNewThreadHybridSuspendException() {
    TearDown();
  ...
  InvalidPointer_GetValue() {
    ...
    var pointerValue2 =
        new PointerValue(pointerValue.VirtualMachine, pointerValue.Type, 200);
  ...
  [Test]
  public void
  InvalidArgumentAssemblyGetType() {
  ...
  InvokeSingleStepMultiThread() {
    ...
    e = breakpoint.lastEvent;
    req = create_step(e);
    while (line_first_counter > 0 || line_second_counter > 0 ||
           line_third_counter > 0) {
      var thread = e.Thread;
      ...
      try {
        if ((e.Thread.GetFrames()[0].Location.LineNumber ==
                 firstLineFound + 1 &&
             (e.Thread.Name.Equals("Thread_0") ||
              e.Thread.Name.Equals("Thread_1"))) ||
  ...
  [Test]
  public void
  CheckSuspendPolicySentWhenLaunchSuspendYes() {
    vm.Exit(0);
    var port = GetFreePort();
