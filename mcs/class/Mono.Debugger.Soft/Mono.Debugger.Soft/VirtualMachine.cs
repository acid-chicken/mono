...
namespace Mono.Debugger.Soft {
  public class VirtualMachine : Mirror {
    ...
    /*
     * It is impossible to determine when to resume when using this method,
     * since the debuggee is suspended only once per event-set, not event.
     */
    ...
    GetNextEvent() {
      lock(queue_monitor) {
        if (current_es == null ||
            current_es_index == current_es.Events.Length) {
          if (queue.Count == 0)
    ...
    public EventSet GetNextEventSet(int timeoutInMilliseconds){
        lock(queue_monitor){if(queue.Count == 0){
            if(!Monitor.Wait(queue_monitor, timeoutInMilliseconds)){return null;
  }
}
...
[Obsolete("Use GetNextEventSet () instead")]
public T
GetNextEvent<T>() where T : Event {
  return GetNextEvent() as T;
}

public void Suspend() { conn.VM_Suspend(); }
...
public void Exit(int exitCode) { conn.VM_Exit(exitCode); }
...
public void ForceDisconnect() { conn.ForceDisconnect(); }
...
void InvalidateThreadAndFrameCaches() {
  lock(threadsToInvalidate) {
    foreach(var thread in threadsToInvalidate) thread.InvalidateFrames();
    threadsToInvalidate.Clear();
  }
  threadCache = null;
}

internal void InvalidateThreadCache() { threadCache = null; }

internal void AddThreadToInvalidateList(ThreadMirror threadMirror) {
  lock(threadsToInvalidate) { threadsToInvalidate.Add(threadMirror); }
}
...
public IList<ThreadMirror>GetThreads() {
  ...
  if (threads == null) {
    ...
    if (WaitHandle.WaitAny(new[]{vm.conn.DisconnectedEvent, fetchingEvent}) ==
        0) {
      throw new VMDisconnectedException();
    }
    for (int i = 0; i < ids.Length; ++i)
      threads[i] = GetThread(ids[i]);
    // Uncomment lines below if you want to re-fetch threads if new threads were
    // started/stopped while featching threads... This is probably more correct
    // but might cause deadlock of this method if runtime is starting/stopping
    // threads nonstop, need way to prevent this(counting number of recursions?)
    // possiblity before uncommenting
    // if (threadCache != threads) {//While fetching threads threadCache was
    // invalidated(thread was created/destoyed) 	return GetThreads ();
    //}
...
public void SetSocketTimeouts(int send_timeout, int receive_timeout,
...
//
public BreakpointEventRequest CreateBreakpointRequest(MethodMirror method,
                                                      long il_offset) {
...
CreateExceptionRequest(TypeMirror exc_type, bool caught, bool uncaught) {
  return new ExceptionEventRequest(this, exc_type, caught, uncaught);
}

public ExceptionEventRequest CreateExceptionRequest(TypeMirror exc_type,
                                                    bool caught, bool uncaught,
                                                    bool everything_else) {
  if (Version.AtLeast(2, 54))
    return new ExceptionEventRequest(this, exc_type, caught, uncaught, true,
                                     everything_else);
  else
    return new ExceptionEventRequest(this, exc_type, caught, uncaught);
}
...
public void EnableEvents(EventType[] events, SuspendPolicy suspendPolicy) {
  foreach(EventType etype in events) {
    if (etype == EventType.Breakpoint)
      throw new ArgumentException(
          "Breakpoint events cannot be requested using EnableEvents", "events");
    conn.EnableEvent(etype, suspendPolicy, null);
  }
}

public BreakpointEventRequest SetBreakpoint(MethodMirror method,
                                            long il_offset) {
...
public void ClearAllBreakpoints() { conn.ClearAllBreakpoints(); }
...
internal void connect() {
  ...
  if (version.MajorVersion != Connection.MAJOR_VERSION)
    throw new NotSupportedException(String.Format(
        ...
        version.MajorVersion, version.MinorVersion, Connection.MAJOR_VERSION,
...
internal void notify_vm_event(EventType evtype, SuspendPolicy spolicy,
                              int req_id, long thread_id, string vm_uri,
                              int exit_code) {
  ...
  case EventType.VMStart:
    ...
    lock(startup_monitor) { Monitor.Pulse(startup_monitor); }
    queue_event_set(new EventSet(
        this, spolicy, new Event[]{new VMStartEvent(vm, req_id, thread_id)}));
    break;
  case EventType.VMDeath:
    queue_event_set(new EventSet(
        this, spolicy, new Event[]{new VMDeathEvent(vm, req_id, exit_code)}));
    break;
  case EventType.VMDisconnect:
    queue_event_set(new EventSet(
        this, spolicy, new Event[]{new VMDisconnectEvent(vm, req_id)}));
    break;
  default:
    throw new Exception();
  }
...
internal void InvalidateAssemblyCaches() {
  lock(domains_lock) {
    foreach(var d in domains.Values) d.InvalidateAssembliesCache();
  }
}
...
internal T GetObject<T>(long id, long domain_id, long type_id) where T
    : ObjectMirror {
  ...
  if (obj == null) {
    ...
    if (t.Assembly == d.Corlib && t.Namespace == "System.Threading" &&
        t.Name == "Thread")
      obj = new ThreadMirror(this, id, t, d);
    else if (t.Assembly == d.Corlib && t.Namespace == "System" &&
             t.Name == "String")
...
internal ThreadMirror GetThread(long id) { return GetObject<ThreadMirror>(id); }
...
internal void AddRequest(EventRequest req, int id) {
  lock(requests_lock) { requests[id] = req; }
}

internal void RemoveRequest(EventRequest req, int id) {
  lock(requests_lock) { requests.Remove(id); }
}

internal EventRequest GetRequest(int id) {
  lock(requests_lock) { return requests[id]; }
}

internal Value DecodeValue(ValueImpl v) { return DecodeValue(v, null); }
...
internal ValueImpl EncodeValue(Value v, List<Value>duplicates = null) {
  if (v is PrimitiveValue) {
    ...
    if (val == null)
      return new ValueImpl{Type = (ElementType) ValueTypeId.VALUE_TYPE_ID_NULL,
                           Objid = 0};
    else
      return new ValueImpl{Value = val};
  } else if (v is ObjectMirror) {
    return new ValueImpl{Type = ElementType.Object,
                         Objid = (v as ObjectMirror).Id};
    ...
    return new ValueImpl{
        Type = ElementType.ValueType, Klass = (v as StructMirror).Type.Id,
        Fields = EncodeFieldValues((v as StructMirror).Fields,
                                   (v as StructMirror).Type.GetFields(),
                                   duplicates, 1)};
  } else if (v is PointerValue) {
    PointerValue val = (PointerValue) v;
    return new ValueImpl{Type = ElementType.Ptr, Klass = val.Type.Id,
                         Value = val.Address};
  } else {
...
internal ValueImpl EncodeValueFixedSize(Value v, List<Value>duplicates,
                                        int len_fixed_size) {
  if (v is PrimitiveValue) {
    object val = (v as PrimitiveValue).Value;
    if (val == null)
...
internal ValueImpl[] EncodeFieldValues(IList<Value>values,
                                       FieldInfoMirror[] field_info,
                                       List<Value>duplicates, int fixedSize) {
  ValueImpl[] res = new ValueImpl[values.Count];
  for (int i = 0; i < values.Count; ++i) {
    if (fixedSize > 1 || field_info[i].FixedSize > 1)
      res[i] = EncodeValueFixedSize(values[i], duplicates,
                                    fixedSize > 1 ? fixedSize
                                                  : field_info[i].FixedSize);
    else
      res[i] = EncodeValue(values[i], duplicates);
  }
  return res;
...
class EventHandler : MarshalByRefObject, IEventHandler {
  ...
  public void Events(SuspendPolicy suspend_policy, EventInfo[] events) {
    ...
    for (int i = 0; i < events.Length; ++i) {
      ...
      switch (ei.EventType) {
      case EventType.VMStart:
        vm.notify_vm_event(EventType.VMStart, suspend_policy, req_id, thread_id,
                           null, 0);
        break;
      case EventType.VMDeath:
        vm.notify_vm_event(EventType.VMDeath, suspend_policy, req_id, thread_id,
                           null, ei.ExitCode);
        break;
      ...
      case EventType.UserLog:
        l.Add(new UserLogEvent(vm, req_id, thread_id, ei.Level, ei.Category,
                               ei.Message));
        break;
  ...
  public void VMDisconnect(int req_id, long thread_id, string vm_uri) {
    vm.notify_vm_event(EventType.VMDisconnect, SuspendPolicy.None, req_id,
                       thread_id, vm_uri, 0);
  }
}
