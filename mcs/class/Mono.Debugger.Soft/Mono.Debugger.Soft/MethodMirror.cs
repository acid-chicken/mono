...
namespace Mono.Debugger.Soft {
  public class MethodMirror : Mirror {
    ...
    /*
     * Creating the custom attributes themselves could modify the behavior of
     * the debuggee, so we return objects similar to the CustomAttributeData
     * objects used by the reflection-only functionality on .net. Since protocol
     * version 2.21
     */
    ...
    public CustomAttributeDataMirror[] GetCustomAttributes(
        TypeMirror attributeType, bool inherit) {
    ...
    CustomAttributeDataMirror[] GetCAttrs(TypeMirror type, bool inherit) {
      ...
      foreach(var attr in cattrs) if (type == null ||
                                      attr.Constructor.DeclaringType == type)
          res.Add(attr);
      return res.ToArray();
    }
    ...
    public ParameterInfoMirror[] GetParameters() {
      if (param_info == null) {
        ...
        // Return
        ret_param = new ParameterInfoMirror(this, -1, vm.GetType(pi.ret_type),
                                            null, ParameterAttributes.Retval);
        // FIXME: this
        // FIXME: Attributes
        for (int i = 0; i < pi.param_count; ++i) {
          param_info[i] = new ParameterInfoMirror(
              this, i, vm.GetType(pi.param_types[i]), pi.param_names[i], 0);
        }
      }
    ...
    public LocalVariable[] GetLocals() {
      if (locals == null) {
        ...
        for (int i = 0; i < pi.Length; ++i)
          locals[i] = new LocalVariable(vm, this, i, pi[i].ParameterType.Id,
                                        pi[i].Name, -1, -1, true);

        for (int i = 0; i < li.names.Length; ++i)
          locals[i + pi.Length] = new LocalVariable(
              vm, this, i, li.types[i], li.names[i], li.live_range_start[i],
              li.live_range_end[i], false);

        if (vm.Version.AtLeast(2, 43)) {
          scopes = new LocalScope[li.scopes_start.Length];
          for (int i = 0; i < scopes.Length; ++i)
            scopes[i] =
                new LocalScope(vm, this, li.scopes_start[i], li.scopes_end[i]);
        }
      }
      return locals;
    }
    ...
    public MethodMirror MakeGenericMethod(TypeMirror[] args) {
      ...
      foreach(var a in args) if (a ==
                                 null) throw new ArgumentNullException("args");
      ...
      long id = -1;
      try {
        id = vm.conn.Method_MakeGenericMethod(Id,
                                              args.Select(t => t.Id).ToArray());
      } catch (CommandException) {
        throw new InvalidOperationException();
      }
      ...
      get {
        if (locations == null) {
          ...
          for (int i = 0; i < il_offsets.Count; ++i)
            res[i] = new Location(vm, this, -1, il_offsets[i],
                                  debug_info.source_files[i].source_file,
                                  line_numbers[i], debug_info.column_numbers[i],
      ...
      get {
        if (meta == null)
          meta = (C.MethodDefinition)
                     DeclaringType.Assembly.Metadata.MainModule.LookupToken(
                         MetadataToken);
        return meta;
      }
    }
