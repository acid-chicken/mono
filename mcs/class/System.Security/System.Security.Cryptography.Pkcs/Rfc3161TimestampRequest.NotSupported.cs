...
namespace System.Security.Cryptography.Pkcs {
  public sealed class Rfc3161TimestampRequest {
    ...
    /// <summary>Create a timestamp request using a pre-computed hash
    /// value.</summary> <param name="hash">The pre-computed hash value to be
    /// timestamped.</param> <param name="hashAlgorithmId">The Object Identifier
    /// (OID) for the hash algorithm that produced <paramref name="hash"
    /// />.</param> <param name="requestedPolicyId">The Object Identifier (OID)
    /// for a timestamp policy the Timestamp Authority (TSA) should use, or <see
    /// langword="null" /> to express no preference.</param> <param
    /// name="nonce">An optional nonce (number used once) to uniquely identify
    /// this request to pair it with the response. The value is interpreted as
    /// an unsigned big-endian integer and may be normalized to the encoding
    /// format.</param> <param name="requestSignerCertificates">
    ///   <see langword="true" /> to indicate the Timestamp Authority (TSA) must
    ///   include the signing certificate in the issued timestamp token;
    ///   otherwise, <see langword="false" />.</param>
    /// <param name="extensions">RFC3161 extensions to present with the
    /// request.</param> <returns>An <see
    /// cref="T:System.Security.Cryptography.Pkcs.Rfc3161TimestampRequest" />
    ...
    public Rfc3161TimestampToken
    ProcessResponse(ReadOnlyMemory<byte>responseBytes, out int bytesConsumed) {
      throw new PlatformNotSupportedException();
    }
    ...
    /// <param name="request" />
    /// <param name="bytesConsumed" />
    public static bool TryDecode(ReadOnlyMemory<byte>encodedBytes,
