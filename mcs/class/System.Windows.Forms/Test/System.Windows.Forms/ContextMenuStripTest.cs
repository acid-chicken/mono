...
namespace MonoTests.System.Windows.Forms {
  ...
  public class ContextMenuStripTest : TestHelper {
    ...
    static readonly Lazy<Control>[] testCaseExplicitMenuSources =
        new Lazy<Control>[]{
            new Lazy<Control>(() => null),
            new Lazy<Control>(
                () => explicitMenuSrcLabel)}; // Involve `Lazy` to use
    ...
    static readonly Lazy<ITestExtendedControl>[] testCaseAssociatedControls =
        new Lazy<ITestExtendedControl>[]{
            ...
            new Lazy<ITestExtendedControl>(
                () => testExtendedLabel)}; // Involve `Lazy` to use
                                           // `TestCaseSource` attribute.
    ...
    public void
    DirectShowTest01(Lazy<Control>explicitMenuSrc) {
    ...
    [Test, TestCaseSource("testCaseExplicitMenuSources")]
    public void
    DirectShowTest02(Lazy<Control>explicitMenuSrc) {
      AssingOwner(explicitMenuSrc.Value);
    ...
    [Test, TestCaseSource("testCaseExplicitMenuSources")]
    public void
    DirectShowTest03(Lazy<Control>explicitMenuSrc) {
      AssingOwner(explicitMenuSrc.Value);
      contextMenuStrip.Show(explicitMenuSrcLabel, Point.Empty);
      Assert.AreEqual(explicitMenuSrcLabel, contextMenuStrip.SourceControl,
                      "SourceControl");
    }

    [Test, TestCaseSource("testCaseExplicitMenuSources")]
    public void
    DirectShowTest04(Lazy<Control>explicitMenuSrc) {
      AssingOwner(explicitMenuSrc.Value);
      contextMenuStrip.Show(testExtendedLabel, Point.Empty);
      Assert.AreEqual(testExtendedLabel, contextMenuStrip.SourceControl,
                      "SourceControl");
    }

    [Test, TestCaseSource("testCaseExplicitMenuSources")]
    public void
    DirectShowTest05(Lazy<Control>explicitMenuSrc) {
      AssingOwner(explicitMenuSrc.Value);
      contextMenuStrip.Show(form, Point.Empty);
      contextMenuStrip.Close();
      contextMenuStrip.Show();
    ...
    public void
    ContextShowTest(Lazy<ITestExtendedControl>associatedControl) {
      bool menuHasBeenOpened = false;
      contextMenuStrip.Opened +=
          (sender, args) => { menuHasBeenOpened = true; };
...
#region Helpers
    ...
    static class TestExtendedControlHelper {
      public static Message MakeWmContextMenu() {
        return new Message(){Msg = (int) Msg.WM_CONTEXTMENU,
                             LParam = IntPtr.Zero};
      }
    }
