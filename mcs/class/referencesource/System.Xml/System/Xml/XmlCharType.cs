...
//#define XMLCHARTYPE_USE_RESOURCE    // load the character properties from
//resources (XmlCharType.bin must be linked to System.Xml.dll) #define
//XMLCHARTYPE_GEN_RESOURCE    // generate the character properties into
//XmlCharType.bin
...
/// </devdoc>
  ...
  [System.Runtime.CompilerServices
       .FriendAccessAllowed] // Used by System.ServiceModel.dll and
  ...
  internal struct XmlCharType {
    ...
    // Star NCName characters     -- Section 2.3 [4]  (NameStartChar characters
    // without ':') NCName characters          -- Section 2.3 [4a] (NameChar
    // characters without ':') Character data characters  -- Section 2.2 [2]
    // Public ID characters       -- Section 2.3 [13]

    // Characters defined in the XML 1.0 Fourth Edition
    // NCNameCharacters           -- Appending B: Characters Classes in XML 1.0
    // 4th edition and earlier - minus the ':' char per the Namespaces in XML
    // spec Letter characters          -- Appending B: Characters Classes in
    // XML 1.0 4th edition and earlier
    //                               This appendix has been deprecated in
    //                               XML 1.0 5th edition, but we still need to
    //                               use the Letter and NCName definitions from
    //                               the 4th edition in some places because of
    ...
    internal const int fNCNameXml4e =
        32; // NCName char according to the XML 1.0 4th edition
    ...
    // Starting NCName characters -- Section 2.3 [5] (Starting Name characters
    // without ':') NCName characters -- Section 2.3 [4]          (Name
    // characters without ':') Character data characters -- Section 2.2 [2]
    ...
    // bitmap for public ID characters - 1 bit per character 0x0 - 0x80; no
    // character > 0x80 is a PUBLIC ID char
    private const string s_PublicIdBitmap =
        "\u2400\u0000\uffbb\uafff\uffff\u87ff\ufffe\u07ff";
    ...
    const string s_Text = // TextChar = CharData - { 0xA | 0xD | '<' | '&' | 0x9
                          // | ']' | 0xDC00 - 0xDFFF }
        "\u0020\u0025\u0027\u003b\u003d\u005c\u005e\ud7ff\ue000\ufffd";

    const string
        s_AttrValue = // AttrValueChar = CharData - { 0xA | 0xD | 0x9 | '<' |
                      // '>' | '&' | '\'' | '"' | 0xDC00 - 0xDFFF }
        "\u0020\u0021\u0023\u0025\u0028\u003b\u003d\u003d\u003f\ud7ff\ue000\ufffd";
    ...
    InitInstance() {
      lock(StaticLock) {
        ...
        UnmanagedMemoryStream memStream =
            (UnmanagedMemoryStream) Assembly.GetExecutingAssembly()
                .GetManifestResourceStream("XmlCharType.bin");
    ...
    private static void SetProperties(byte[] chProps, string ranges,
                                      byte value) {
    ...
    // NOTE: This method will not be inlined (because it uses byte*
    // charProperties)
#if SILVERLIGHT && !SILVERLIGHT_DISABLE_SECURITY && XMLCHARTYPE_USE_RESOURCE
    [System.Security.SecuritySafeCritical]
#endif
...
#if !SILVERLIGHT
    public bool IsExtender(char ch) { return (ch == 0xb7); }
#endif
    ...
    // Surrogate characters for names are the same for NameChar and
    // StartNameChar, so this method can be used for both
...
#endif
    public bool
    IsStartNCNameSingleChar(char ch) {
      return (charProperties[ch] & fNCStartNameSC) != 0;
    }

#if XML10_FIFTH_EDITION
    // !!! NOTE: These is no IsStartNCNameSurrogateChar, use
    // IsNCNameSurrogateChar instead. Surrogate ranges for start name charaters
    // are the same as for name characters.
#endif
    ...
    // NOTE: This method will not be inlined (because it uses byte*
    // charProperties) This method uses the XML 4th edition name character
    // ranges
#if SILVERLIGHT && !SILVERLIGHT_DISABLE_SECURITY && XMLCHARTYPE_USE_RESOURCE
    [System.Security.SecuritySafeCritical]
#endif
    public bool
    IsNCNameCharXml4e(char ch) {
    ...
    // Digit methods
    public static bool IsDigit(char ch) { return InRange(ch, 0x30, 0x39); }

#if !SILVERLIGHT
    public static bool IsHexDigit(char ch) {
      return InRange(ch, 0x30, 0x39) || InRange(ch, 'a', 'f') ||
             InRange(ch, 'A', 'F');
    }
#endif
    ...
    internal static int CombineSurrogateChar(int lowChar, int highChar) {
      return (lowChar - SurLowStart) |
             ((highChar - SurHighStart) << 10) + 0x10000;
    }

    internal static void SplitSurrogateChar(int combinedChar, out char lowChar,
                                            out char highChar) {
...
#endif
    internal int
    IsOnlyWhitespaceWithPos(string str) {
      if (str != null) {
    ...
    // This method tests whether a value is in a given range with just one test;
    // start and end should be constants
    ...
    public static void Main(string[] args) {
      try {
        ...
        for (int i = 0; i < s_PublicID.Length; i += 2) {
          for (int j = s_PublicID[i], last = s_PublicID[i + 1]; j <= last;
               j++) {
        ...
        Console.Write("Writing XmlCharType character properties to {0}...",
                      fileName);
