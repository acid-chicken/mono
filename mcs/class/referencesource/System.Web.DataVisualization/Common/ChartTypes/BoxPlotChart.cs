...
#region Painting and Selection methods
    ...
    virtual public void Paint(ChartGraphics graph, CommonElements common,
                              ChartArea area, Series seriesToDraw) {
    ...
    /// </summary>
    /// <param name="selection">If True selection mode is active, otherwise
    /// paint mode is active.</param> <param name="graph">The Chart Graphics
    /// object.</param> <param name="common">The Common elements object.</param>
    /// <param name="area">Chart area for this chart.</param>
    /// <param name="seriesToDraw">Chart series to draw.</param>
    virtual protected void ProcessChartType(bool selection, ChartGraphics graph,
      ...
      // Zero X values mode.
      bool indexedSeries =
          ChartHelper.IndexedSeries(area.Common, typeSeries.ToArray());
      ...
      foreach(Series ser in common.DataManager.Series) {
        ...
        if (String.Compare(ser.ChartTypeName, this.Name,
        ...
        double interval = (indexedSeries)
                              ? 1
                              : area.GetPointsInterval(hAxis.IsLogarithmic,
                                                       hAxis.logarithmBase);
        ...
        if (ser.IsCustomPropertySet(CustomPropertyName.DrawSideBySide)) {
          ...
          if (String.Compare(attribValue, "False",
                             StringComparison.OrdinalIgnoreCase) == 0) {
            currentShowSideBySide = false;
          } else if (String.Compare(attribValue, "True",
                                    StringComparison.OrdinalIgnoreCase) == 0) {
            currentShowSideBySide = true;
          } else if (String.Compare(attribValue, "Auto",
                                    StringComparison.OrdinalIgnoreCase) == 0) {
          } else {
        ...
        if (!selection) {
          common.Chart.CallOnPrePaint(new ChartPaintEventArgs(
              ser, graph, common, area.PlotAreaPosition));
        }
        ...
        foreach(DataPoint point in ser.Points) {
          ...
          if (point.YValues.Length < this.YValuesPerPoint) {
            throw (new InvalidOperationException(
                SR.ExceptionChartTypeRequiresYValues(
                    this.Name, this.YValuesPerPoint.ToString(
          ...
          if (indexedSeries) {
            xValue = (double) index;
            xPosition = (float)(hAxis.GetPosition((double) index) -
                                width * ((double) numOfSeries) / 2.0 +
                                width / 2 + seriesIndx * width);
          ...
          point.positionRel =
              new PointF((float) xPosition, (float) Math.Min(high, low));
          ...
          if (common.ProcessModePaint) {
            ...
            graph.DrawLineRel(
                lineColor, point.BorderWidth, point.BorderDashStyle,
            ...
            rectSize.Height = (float) Math.Abs(
                rectSize.Y - vAxis.GetPosition(point.YValues[2]));
            graph.FillRectangleRel(
                rectSize, point.Color, point.BackHatchStyle, point.BackImage,
                point.BackImageWrapMode, point.BackImageTransparentColor,
                point.BackImageAlignment, point.BackGradientStyle,
                point.BackSecondaryColor, point.BorderColor, point.BorderWidth,
            ...
            if (point.IsCustomPropertySet(
                ...
                ser.IsCustomPropertySet(
                    CustomPropertyName.BoxPlotShowAverage)) {
              string showAverageValue =
                  ser[CustomPropertyName.BoxPlotShowAverage];
              if (point.IsCustomPropertySet(
                      CustomPropertyName.BoxPlotShowAverage)) {
                showAverageValue = point[CustomPropertyName.BoxPlotShowAverage];
              }
              if (String.Compare(showAverageValue, "True",
                                 StringComparison.OrdinalIgnoreCase) == 0) {
                // default - do nothing
              } else if (String.Compare(showAverageValue, "False",
            ...
            if (markerLinesColor == point.Color) {
              double brightness = Math.Sqrt(point.Color.R * point.Color.R +
                                            point.Color.G * point.Color.G +
                                            point.Color.B * point.Color.B);
              if (brightness > 220) {
                markerLinesColor = ChartGraphics.GetGradientColor(
                    point.Color, Color.Black, 0.4);
              } else {
                markerLinesColor = ChartGraphics.GetGradientColor(
                    point.Color, Color.White, 0.4);
              }
            }
            if (!double.IsNaN(point.YValues[4]) && showAverage) {
              graph.DrawLineRel(
                  markerLinesColor, 1, ChartDashStyle.Solid,
                  new PointF(rectSize.Left + relBorderWidth.Width,
                             (float) vAxis.GetPosition(point.YValues[4])),
                  new PointF(rectSize.Right - relBorderWidth.Width,
                             (float) vAxis.GetPosition(point.YValues[4])),
                  Color.Empty, 0);
            }
            ...
            if (point.IsCustomPropertySet(
              ...
              string showMedianValue =
                  ser[CustomPropertyName.BoxPlotShowMedian];
              if (point.IsCustomPropertySet(
                      CustomPropertyName.BoxPlotShowMedian)) {
                showMedianValue = point[CustomPropertyName.BoxPlotShowMedian];
              }
              if (String.Compare(showMedianValue, "True",
                                 StringComparison.OrdinalIgnoreCase) == 0) {
                // default - do nothing
              } else if (String.Compare(showMedianValue, "False",
                                        StringComparison.OrdinalIgnoreCase) ==
                         0) {
                showMedian = false;
              } else {
            ...
            if (!double.IsNaN(point.YValues[5]) && showMedian) {
              ...
              float dashWidth =
                  (rectSize.Width - relBorderWidth.Width * 2) / 9f;
              ...
              for (float curPosition = rectSize.Left + relBorderWidth.Width;
                   curPosition < (rectSize.Right - relBorderWidth.Width);
                   curPosition += dashWidth * 2f) {
                graph.DrawLineRel(
                    markerLinesColor, 1, ChartDashStyle.Solid,
                    new PointF(curPosition, medianValue),
                    new PointF((float) Math.Min(rectSize.Right,
                                                curPosition + dashWidth),
                               medianValue),
                    Color.Empty, 0);
              }
            }
          ...
          if (common.ProcessModeRegions) {
            ...
            // Add area
            common.HotRegionsList.AddHotRegion(areaRect, point, ser.Name,
                                               index - 1);
          }
          ++index;
        }
        ...
        if (!selection) {
          ...
          foreach(DataPoint point in ser.Points) {
            ...
            for (int valueIndex = 0; valueIndex < point.YValues.Length;
                 valueIndex++) {
              if (!double.IsNaN(point.YValues[valueIndex])) {
                double currentValue =
                    vAxis.GetLogValue(point.YValues[valueIndex]);
            ...
            // Draw label
            DrawLabel(common, area, graph, ser, point,
                      new PointF(xPosition, (float) high), index);
        ...
        // Call Paint event
        if (!selection) {
          common.Chart.CallOnPostPaint(new ChartPaintEventArgs(
              ser, graph, common, area.PlotAreaPosition));
        }
    ...
    virtual protected void DrawBoxPlotMarks(ChartGraphics graph, ChartArea area,
                                            Series ser, DataPoint point,
                                            float xPosition, float width) {
      // Get markers style
      ...
      DrawBoxPlotSingleMarker(graph, area, point, markerStyle, xPosition,
                              (float) yPosition, 0f, width, false);
      ...
      for (int valueIndex = 6; valueIndex < point.YValues.Length;
           valueIndex++) {
        if (!double.IsNaN(point.YValues[valueIndex])) {
    ...
    private void DrawBoxPlotSingleMarker(ChartGraphics graph, ChartArea area,
                                         DataPoint point, string markerStyle,
                                         float xPosition, float yPosition,
                                         float zPosition, float width,
                                         bool draw3D) {
      markerStyle = markerStyle.ToUpper(CultureInfo.InvariantCulture);
      if (markerStyle.Length > 0 &&
          String.Compare(markerStyle, "None",
                         StringComparison.OrdinalIgnoreCase) != 0) {
        ...
        if (String.Compare(markerStyle, "Line",
                           StringComparison.OrdinalIgnoreCase) == 0) {
          graph.DrawLineRel(
              lineColor, point.BorderWidth, point.BorderDashStyle,
        ...
        // Draw standard marker
          ...
          MarkerStyle marker =
              (MarkerStyle) Enum.Parse(typeof (MarkerStyle), markerStyle, true);

          // Get marker size
          SizeF markerSize = GetMarkerSize(graph, area.Common, area, point,
                                           point.MarkerSize, point.MarkerImage);

          // Get marker color
          Color markerColor = (point.MarkerColor == Color.Empty)
                                  ? point.BorderColor
                                  : point.MarkerColor;
          ...
          // Draw the marker
          graph.DrawMarkerRel(
              new PointF(xPosition, yPosition), marker, point.MarkerSize,
              markerColor, point.MarkerBorderColor, point.MarkerBorderWidth,
              point.MarkerImage, point.MarkerImageTransparentColor,
              (point.series != null) ? point.series.ShadowOffset : 0,
              (point.series != null) ? point.series.ShadowColor : Color.Empty,
              new RectangleF(xPosition, yPosition, markerSize.Width,
                             markerSize.Height));
        }
      }
    }
    ...
    virtual protected SizeF GetMarkerSize(ChartGraphics graph,
                                          CommonElements common, ChartArea area,
                                          DataPoint point, int markerSize,
                                          string markerImage) {
      ...
      if (markerImage.Length > 0)
        common.ImageLoader.GetAdjustedImageSize(markerImage, graph.Graphics,
                                                ref size);
    ...
    virtual protected void DrawLabel(CommonElements common, ChartArea area,
                                     ChartGraphics graph, Series ser,
                                     DataPoint point, PointF position,
                                     int pointIndex) {
      if (ser.IsValueShownAsLabel || point.IsValueShownAsLabel ||
        ...
        using(StringFormat format = new StringFormat()) {
          ...
          if (point.Label.Length == 0) {
            text = ValueConverter.FormatValue(
                ser.Chart, point, point.Tag, point.YValues[0],
                point.LabelFormat, ser.YValueType, ChartElementType.DataPoint);
          } else {
          ...
          if (text.Trim().Length != 0) {
            ...
            if (ser.SmartLabelStyle.Enabled) {
              ...
              position = area.smartLabels.AdjustSmartLabelPosition(
                  common, graph, area, ser.SmartLabelStyle, position, sizeFont,
                  format, position, markerSizes, LabelAlignmentStyles.Top);
            ...
            if (!position.IsEmpty) {
              // Get text size
              if (sizeFont.IsEmpty) {
                sizeFont = graph.GetRelativeSize(graph.MeasureString(
                    text, point.Font, new SizeF(1000f, 1000f),
                    StringFormat.GenericTypographic));
              }
              ...
              labelBackPosition = PointChart.GetLabelPosition(
                  graph, position, sizeLabel, format, true);

              // Draw label text
              using(Brush brush = new SolidBrush(point.LabelForeColor)) {
                graph.DrawPointLabelStringRel(
                    common, text, point.Font, brush, position, format,
                    textAngle, labelBackPosition, point.LabelBackColor,
                    point.LabelBorderColor, point.LabelBorderWidth,
                    point.LabelBorderDashStyle, ser, point, pointIndex - 1);
              }
            }
          }
        }
...
#region 3D Drawing and Selection methods
    ...
    /// <param name="area">Chart area for this chart.</param>
    /// <param name="seriesToDraw">Chart series to draw.</param>
    virtual protected void ProcessChartType3D(bool selection,
                                              ChartGraphics graph,
                                              CommonElements common,
                                              ChartArea area,
                                              Series seriesToDraw) {
      ...
      // Zero X values mode.
      bool indexedSeries =
          ChartHelper.IndexedSeries(common, typeSeries.ToArray());
      ...
      foreach(Series ser in common.DataManager.Series) {
        ...
        if (ser.YValuesPerPoint < 6) {
          throw (new ArgumentException(SR.ExceptionChartTypeRequiresYValues(
        ...
        area.GetSeriesZPositionAndDepth(ser, out seriesDepth,
                                        out seriesZPosition);
        ...
        foreach(DataPoint point in ser.Points) {
          ...
          points[0] = new Point3D(xPosition, (float) low,
                                  seriesZPosition + seriesDepth / 2f);
          points[1] = new Point3D(xPosition, (float) high,
                                  seriesZPosition + seriesDepth / 2f);
          points[2] = new Point3D(xPosition,
                                  (float) vAxis.GetPosition(point.YValues[2]),
                                  seriesZPosition + seriesDepth / 2f);
          points[3] = new Point3D(xPosition,
                                  (float) vAxis.GetPosition(point.YValues[3]),
                                  seriesZPosition + seriesDepth / 2f);
          points[4] = new Point3D(xPosition,
                                  (float) vAxis.GetPosition(point.YValues[4]),
                                  seriesZPosition + seriesDepth / 2f);
          points[5] = new Point3D(xPosition,
          ...
          if (common.ProcessModePaint) {
            ...
            graph.DrawLineRel(lineColor, point.BorderWidth,
                              point.BorderDashStyle, points[0].PointF,
                              points[2].PointF, ser.ShadowColor,
                              ser.ShadowOffset);

            // Draw upper whisker line
            graph.DrawLineRel(lineColor, point.BorderWidth,
                              point.BorderDashStyle, points[1].PointF,
                              points[3].PointF, ser.ShadowColor,
                              ser.ShadowOffset);
            ...
            if (!double.IsNaN(point.YValues[4]) && showAverage) {
              graph.DrawLineRel(markerLinesColor, 1, ChartDashStyle.Solid,
            ...
            if (!double.IsNaN(point.YValues[5]) && showMedian) {
              ...
              for (float curPosition = rectSize.Left;
            ...
            // Draw Box Plot marks
            DrawBoxPlotMarks3D(graph, area, ser, point, xPosition, width,
                               seriesZPosition, seriesDepth);
            xPosition = points[0].X;
            high = points[0].Y;
            low = points[1].Y;
        ...
        if (!selection) {
          ...
          foreach(DataPoint point in ser.Points) {
            ...
            points[0] = new Point3D(xPosition, (float) high,
                                    seriesZPosition + seriesDepth / 2f);
            points[1] = new Point3D(xPosition, (float) low,
                                    seriesZPosition + seriesDepth / 2f);
            area.matrix3D.TransformPoints(points);
            xPosition = points[0].X;
            high = points[0].Y;
    ...
    virtual protected void DrawBoxPlotMarks3D(ChartGraphics graph,
                                              ChartArea area, Series ser,
                                              DataPoint point, float xPosition,
                                              float width, float zPosition,
                                              float depth) {
      // Get markers style
      string markerStyle = "LINE";
      if (point.MarkerStyle != MarkerStyle.None) {
      ...
      double yPosition = vAxis.GetLogValue(point.YValues[0]);
      DrawBoxPlotSingleMarker(graph, area, point, markerStyle, xPosition,
                              (float) yPosition, zPosition + depth / 2f, width,
                              true);
      ...
      for (int valueIndex = 6; valueIndex < point.YValues.Length;
        ...
        if (!double.IsNaN(point.YValues[valueIndex])) {
          yPosition = vAxis.GetLogValue(point.YValues[valueIndex]);
          DrawBoxPlotSingleMarker(graph, area, point, markerStyle, xPosition,
                                  (float) yPosition, zPosition + depth / 2f,
                                  width, true);
        }
      }
    }
...
#region Y values related methods
    ...
    virtual public double GetYValue(CommonElements common, ChartArea area,
                                    Series series, DataPoint point,
                                    int pointIndex, int yValueIndex) {
...
#region Automatic Values Calculation methods
    ...
    /// <summary>
    /// Populates box plot chart type using series data specified in
    /// "BoxPlotSeries" custom attribute.
    /// </summary>
    /// <param name="boxPlotSeries">Box Plot chart type series.</param>
    internal static void
    CalculateBoxPlotFromLinkedSeries(Series boxPlotSeries) {
      // Check input parameters
      if (String.Compare(boxPlotSeries.ChartTypeName, ChartTypeNames.BoxPlot,
                         StringComparison.OrdinalIgnoreCase) != 0) {
        return;
      }
      ...
      if (boxPlotSeries.IsCustomPropertySet(CustomPropertyName.BoxPlotSeries)) {
        ...
        string[] attrValues =
            boxPlotSeries[CustomPropertyName.BoxPlotSeries].Split(';');
        ...
        foreach(string val in attrValues) {
          ...
          [CustomPropertyName.BoxPlotSeries] = val.Trim();
        }
      }

      // Calculate box plot for every data point
      for (int pointIndex = 0; pointIndex < boxPlotSeries.Points.Count;
           pointIndex++) {
        ...
        if (point.IsCustomPropertySet(CustomPropertyName.BoxPlotSeries)) {
          ...
          String valueName = "Y";
          int valueTypeIndex =
              linkedSeriesName.IndexOf(":", StringComparison.OrdinalIgnoreCase);
          if (valueTypeIndex >= 0) {
            ...
            if (control.Series.IndexOf(linkedSeriesName) == -1) {
              throw (new InvalidOperationException(
                  SR.ExceptionCustomAttributeSeriesNameNotFound(
                      "BoxPlotSeries", linkedSeriesName)));
            }
    ...
    private static void CalculateBoxPlotValues(ref DataPoint boxPoint,
                                               Series linkedSeries,
                                               string valueName) {
      ...
      foreach(DataPoint point in linkedSeries.Points) {
        if (!point.IsEmpty) {
          yValues.Add((point.IsEmpty) ? double.NaN
      ...
      string boxPercentile =
          (boxPoint.IsCustomPropertySet(CustomPropertyName.BoxPlotPercentile))
              ...
              : String.Empty;
      if (boxPercentile.Length == 0 && boxPoint.series != null &&
          boxPoint.series.IsCustomPropertySet(
      ...
      string boxWhiskerPercentile =
          (boxPoint.IsCustomPropertySet(
              CustomPropertyName.BoxPlotWhiskerPercentile))
              ? boxPoint[CustomPropertyName.BoxPlotWhiskerPercentile]
              : String.Empty;
      if (boxWhiskerPercentile.Length == 0 && boxPoint.series != null &&
          boxPoint.series.IsCustomPropertySet(
              CustomPropertyName.BoxPlotWhiskerPercentile)) {
        boxWhiskerPercentile =
            boxPoint.series[CustomPropertyName.BoxPlotWhiskerPercentile];
      }
      ...
      if (boxPercentile.Length > 0) {
        double percentile;
        bool parseSucceed =
            double.TryParse(boxPercentile, NumberStyles.Any,
                            CultureInfo.InvariantCulture, out percentile);
        if (parseSucceed) {
          requiredPercentile[2] = percentile;
        }

        if (!parseSucceed || requiredPercentile[2] < 0 ||
            requiredPercentile[2] > 50) {
          throw (new InvalidOperationException(
              SR.ExceptionCustomAttributeIsNotInRange0to50(
                  "BoxPlotPercentile")));
        }
      ...
      if (boxWhiskerPercentile.Length > 0) {
        ...
        double percentile;
        bool parseSucceed =
            double.TryParse(boxWhiskerPercentile, NumberStyles.Any,
                            CultureInfo.InvariantCulture, out percentile);
        if (parseSucceed) {
          requiredPercentile[0] = percentile;
        }

        if (!parseSucceed || requiredPercentile[0] < 0 ||
            requiredPercentile[0] > 50) {
          throw (new InvalidOperationException(
              SR.ExceptionCustomAttributeIsNotInRange0to50(
                  "BoxPlotPercentile")));
        }
      ...
      double[] percentileValues =
          CalculatePercentileValues(yValues, requiredPercentile);
      ...
      string showUnusualValues =
          (boxPoint.IsCustomPropertySet(
              ...
              : String.Empty;
      if (showUnusualValues.Length == 0 && boxPoint.series != null &&
          boxPoint.series.IsCustomPropertySet(
              CustomPropertyName.BoxPlotShowUnusualValues)) {
        showUnusualValues =
            boxPoint.series[CustomPropertyName.BoxPlotShowUnusualValues];
      }
      if (showUnusualValues.Length > 0) {
        if (String.Compare(showUnusualValues, "True",
                           StringComparison.OrdinalIgnoreCase) == 0) {
          addUnusualValues = true;
        } else if (String.Compare(showUnusualValues, "False",
                                  StringComparison.OrdinalIgnoreCase) == 0) {
          addUnusualValues = false;
        } else {
          throw (new InvalidOperationException(
              SR.ExceptionCustomAttributeValueInvalid2(
    ...
    static private void BoxPlotAddUnusual(ref DataPoint boxPoint,
                                          List<double>yValues) {
      // Get unusual values
    ...
    static private double[] CalculatePercentileValues(
      ...
      foreach(double percentile in requiredPercentile) {
        ...
        double percentPointIndexReminder =
            percentPointIndex - percentPointIndexInteger;

        result[index] = 0.0;
        if ((int) percentPointIndexInteger < yValues.Count) {
          result[index] += (1.0 - percentPointIndexReminder) *
                           yValues[(int) percentPointIndexInteger];
        }
        if ((int)(percentPointIndexInteger + 1) < yValues.Count) {
          result[index] += percentPointIndexReminder *
                           yValues[(int) percentPointIndexInteger + 1];
        }
...
#region SmartLabelStyle methods
    ...
    /// <summary>
    /// Adds markers position to the list. Used to check SmartLabelStyle
    /// overlapping.
    /// </summary>
    ...
    public void AddSmartLabelMarkerPositions(CommonElements common,
