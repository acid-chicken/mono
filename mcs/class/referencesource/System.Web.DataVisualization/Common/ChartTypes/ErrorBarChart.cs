...
#region Painting and Selection methods
    ...
    virtual protected void ProcessChartType(bool selection, ChartGraphics graph,
      ...
      foreach(Series ser in common.DataManager.Series) {
        ...
        if (ser.IsCustomPropertySet(CustomPropertyName.ErrorBarSeries)) {
          ...
          int valueTypeIndex =
              linkedSeriesName.IndexOf(":", StringComparison.Ordinal);
          if (valueTypeIndex >= 0) {
            linkedSeriesName = linkedSeriesName.Substring(0, valueTypeIndex);
          }

          // All linked data series from chart area which have Error bar chart
          // type
          string linkedSeriesChartType =
              common.DataManager.Series[linkedSeriesName].ChartTypeName;
          ChartArea linkedSeriesArea =
              common.ChartPicture.ChartAreas
                  [common.DataManager.Series[linkedSeriesName].ChartArea];
          List<string>typeLinkedSeries =
              linkedSeriesArea.GetSeriesFromChartType(linkedSeriesChartType);
          ...
          if (String.Compare(linkedSeriesChartType, ChartTypeNames.Column,
                             StringComparison.OrdinalIgnoreCase) == 0 ||
              String.Compare(linkedSeriesChartType, ChartTypeNames.RangeColumn,
                             StringComparison.OrdinalIgnoreCase) == 0) {
          ...
          if (currentDrawSeriesSideBySide) {
            ...
            if (!indexedSeries) {
              area.GetPointsInterval(typeLinkedSeries, hAxis.IsLogarithmic,
                                     hAxis.logarithmBase, true,
                                     out showSideBySide);
            }
        ...
        foreach(DataPoint point in ser.Points) {
          ...
          if (indexedSeries) {
            ...
            //sideBySideWidth/2 + indexOfLinkedSeries * sideBySideWidth);
          }
          ...
          if (point.IsCustomPropertySet(CustomPropertyName.ErrorBarStyle) ||
            ...
            if (String.Compare(errorBarStyle, "Both",
                               StringComparison.OrdinalIgnoreCase) == 0) {
              // default - do nothing
            } else if (String.Compare(errorBarStyle, "UpperError",
                                      StringComparison.OrdinalIgnoreCase) ==
                       0) {
              barStyle = ErrorBarStyle.UpperError;
              low = vAxis.GetLogValue(point.YValues[0]);
              high = vAxis.GetLogValue(point.YValues[2]);
            } else if (String.Compare(errorBarStyle, "LowerError",
                                      StringComparison.OrdinalIgnoreCase) ==
                       0) {
          ...
          if (common.ProcessModePaint) {
            ...
            DrawErrorBarMarks(graph, barStyle, area, ser, point, xPosition,
                              width);
    ...
    virtual protected void DrawErrorBarMarks(ChartGraphics graph,
      ...
      if (barStyle == ErrorBarStyle.Both ||
          barStyle == ErrorBarStyle.LowerError) {
        // Get Y position
        ...
        // Draw marker
        DrawErrorBarSingleMarker(graph, area, point, markerStyle, xPosition,
    ...
    private void DrawErrorBarSingleMarker(ChartGraphics graph, ChartArea area,
      ...
      if (markerStyle.Length > 0 &&
        ...
        if (String.Compare(markerStyle, "Line",
                           StringComparison.OrdinalIgnoreCase) == 0) {
          graph.DrawLineRel(
              point.Color, point.BorderWidth, point.BorderDashStyle,
...
#region 3D Drawing and Selection methods
    ...
    virtual protected void ProcessChartType3D(bool selection,
      ...
      foreach(Series ser in common.DataManager.Series) {
        ...
        if (ser.IsCustomPropertySet(CustomPropertyName.ErrorBarSeries)) {
          ...
          int valueTypeIndex =
              attribValue.IndexOf(":", StringComparison.Ordinal);
          if (valueTypeIndex >= 0) {
          ...
          // type
          string linkedSeriesChartType =
              common.DataManager.Series[attribValue].ChartTypeName;
          List<string>typeLinkedSeries =
              area.GetSeriesFromChartType(linkedSeriesChartType);
        ...
        foreach(DataPoint point in ser.Points) {
          ...
          if (common.ProcessModePaint) {
            ...
            graph.DrawLineRel(point.Color, point.BorderWidth,
                              ...
                              points[1].PointF, ser.ShadowColor,
                              ser.ShadowOffset);

            // Draw Error Bar marks
            DrawErrorBarMarks3D(graph, barStyle, area, ser, point, xPosition,
                                width, seriesZPosition, seriesDepth);
            xPosition = points[0].X;
            high = points[0].Y;
            low = points[1].Y;
    ...
    virtual protected void DrawErrorBarMarks3D(ChartGraphics graph,
                                               ErrorBarStyle barStyle,
                                               ChartArea area, Series ser,
                                               DataPoint point, float xPosition,
      ...
      if (barStyle == ErrorBarStyle.Both ||
        ...
        // Draw marker
        DrawErrorBarSingleMarker(graph, area, point, markerStyle, xPosition,
                                 yPosition, zPosition + depth / 2f, width,
                                 true);
      }
...
#region Automatic Values Calculation methods
    ...
    internal static void CalculateErrorAmount(Series errorBarSeries) {
      ...
      if (String.Compare(errorBarSeries.ChartTypeName, ChartTypeNames.ErrorBar,
                         StringComparison.OrdinalIgnoreCase) != 0) {
        return;
      }
      ...
      if (errorBarSeries.IsCustomPropertySet(CustomPropertyName.ErrorBarType)) {
        ...
        if (typeName.StartsWith("FixedValue",
                                StringComparison.OrdinalIgnoreCase)) {
          errorBarType = ErrorBarType.FixedValue;
        } else if (typeName.StartsWith("Percentage",
                                       StringComparison.OrdinalIgnoreCase)) {
          errorBarType = ErrorBarType.Percentage;
        } else if (typeName.StartsWith("StandardDeviation",
                                       StringComparison.OrdinalIgnoreCase)) {
          errorBarType = ErrorBarType.StandardDeviation;
        } else if (typeName.StartsWith("StandardError",
                                       StringComparison.OrdinalIgnoreCase)) {
          errorBarType = ErrorBarType.StandardError;
        } else if (typeName.StartsWith("None",
                                       StringComparison.OrdinalIgnoreCase)) {
          return;
        } else {
          throw (new InvalidOperationException(SR.ExceptionErrorBarTypeInvalid(
        ...
        if (typeName.Length > 0) {
          ...
          if (typeName.Length > 0) {
            if (!double.TryParse(typeName, NumberStyles.Any,
                                 CultureInfo.InvariantCulture, out param)) {
              throw (new InvalidOperationException(
                  SR.ExceptionErrorBarTypeFormatInvalid(
                      errorBarSeries[CustomPropertyName.ErrorBarType])));
            }
      ...
      if (errorBarType == ErrorBarType.FixedValue) {
        ...
        if (pointNumber > 1) {
          ...
          errorAmount =
              param *
              Math.Sqrt(errorAmount / (pointNumber * (pointNumber - 1))) / 2.0;
        } else {
          errorAmount = 0;
        }
      }
      ...
      foreach(DataPoint point in errorBarSeries.Points) {
        if (errorBarType == ErrorBarType.Percentage) {
          point.YValues[1] =
              point.YValues[0] - point.YValues[0] * param / 100.0;
          point.YValues[2] =
              point.YValues[0] + point.YValues[0] * param / 100.0;
        } else {
    ...
    internal static double DefaultErrorBarTypeValue(ErrorBarType errorBarType) {
      ...
      case ErrorBarType.StandardError:
        return 1.0;
      default:
        System.Diagnostics.Debug.Fail("Unknown ErrorBarType=" +
    ...
    internal static void GetDataFromLinkedSeries(Series errorBarSeries) {
      ...
      string linkedSeriesName =
          errorBarSeries[CustomPropertyName.ErrorBarSeries];
      String valueName = "Y";
      int valueTypeIndex =
          linkedSeriesName.IndexOf(":", StringComparison.Ordinal);
      if (valueTypeIndex >= 0) {
      ...
      if (control != null) {
        ...
        foreach(DataPoint point in linkedSeries.Points) {
          ...
          errorBarSeries.Points.AddXY(point.XValue,
