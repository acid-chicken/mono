...
namespace System.Data.OleDb {
  ...
  internal sealed class OleDbMetaDataFactory : DbMetaDataFactory { // V1.2.3300
    ...
    internal OleDbMetaDataFactory(Stream XMLStream, string serverVersion,
      ...
      _schemaMapping = new SchemaRowsetName[]{
          ...
          new SchemaRowsetName(DbMetaDataCollectionNames.DataTypes,
                               OleDbSchemaGuid.Provider_Types),
          new SchemaRowsetName(OleDbMetaDataCollectionNames.Catalogs,
                               OleDbSchemaGuid.Catalogs),
          new SchemaRowsetName(OleDbMetaDataCollectionNames.Collations,
                               OleDbSchemaGuid.Collations),
          new SchemaRowsetName(OleDbMetaDataCollectionNames.Columns,
                               OleDbSchemaGuid.Columns),
          new SchemaRowsetName(OleDbMetaDataCollectionNames.Indexes,
                               OleDbSchemaGuid.Indexes),
          new SchemaRowsetName(OleDbMetaDataCollectionNames.Procedures,
                               OleDbSchemaGuid.Procedures),
          new SchemaRowsetName(OleDbMetaDataCollectionNames.ProcedureColumns,
                               OleDbSchemaGuid.Procedure_Columns),
          new SchemaRowsetName(OleDbMetaDataCollectionNames.ProcedureParameters,
                               OleDbSchemaGuid.Procedure_Parameters),
          new SchemaRowsetName(OleDbMetaDataCollectionNames.Tables,
                               OleDbSchemaGuid.Tables),
          new SchemaRowsetName(OleDbMetaDataCollectionNames.Views,
      ...
      if ((null == collectionName) ||
          (typeof (System.String) != collectionName.DataType)) {
        throw ADP.InvalidXmlMissingColumn(
            DbMetaDataCollectionNames.MetaDataCollections, _collectionName);
      }
      DataColumn restrictionCollectionName = null;
      if (restrictionsTable != null) {
        restrictionCollectionName = restrictionsTable.Columns[_collectionName];
        if ((null == restrictionCollectionName) ||
            (typeof (System.String) != restrictionCollectionName.DataType)) {
          throw ADP.InvalidXmlMissingColumn(
              DbMetaDataCollectionNames.Restrictions, _collectionName);
        }
      }

      foreach(DataRow collection in metaDataCollectionsTable.Rows) {

        string populationMechanismValue =
            collection[populationMechanism] as string;
        ...
        if (populationMechanismValue == _prepareCollection) {
          ...
          if (schemaSupport != null) {
            for (int i = 0; i < schemaSupport.Length; i++) {
              if (_schemaMapping[mapping]._schemaRowset ==
    ...
    private String BuildRegularExpression(string invalidChars,
    ...
    private DataTable
    GetDataSourceInformationTable(OleDbConnection connection,
      ...
      string catalogSeparatorPattern =
          internalConnection.GetLiteralInfo(ODB.DBLITERAL_CATALOG_SEPARATOR);
      string schemaSeparatorPattern =
          internalConnection.GetLiteralInfo(ODB.DBLITERAL_SCHEMA_SEPARATOR);
      ...
      if (catalogSeparatorPattern != null) {
        ...
        if ((schemaSeparatorPattern != null) &&
      ...
      SetIdentifierCase(DbMetaDataColumnNames.IdentifierCase,
                        ODB.DBPROP_IDENTIFIERCASE, dataSourceInformation,
                        connection);
      SetIdentifierCase(DbMetaDataColumnNames.QuotedIdentifierCase,
                        ODB.DBPROP_QUOTEDIDENTIFIERCASE, dataSourceInformation,
                        connection);
      ...
      if (infoLiterals != null) {
        DataRow[] tableNameRow = infoLiterals.Select(
            "Literal = " +
            ODB.DBLITERAL_TABLE_NAME.ToString(CultureInfo.InvariantCulture));
        if (tableNameRow != null) {
          object invalidCharsObject = tableNameRow [0]
          ["InvalidChars"];
          if (invalidCharsObject.GetType() == typeof (string)) {
            string invalidChars = (string) invalidCharsObject;
            object invalidStartingCharsObject = tableNameRow [0]
      ...
      string quotePrefix;
      string quoteSuffix;
      connection.GetLiteralQuotes(ADP.GetSchema, out quotePrefix,
                                  out quoteSuffix);

      if (quotePrefix != null) {

        // if the quote suffix is null assume that it is the same as the prefix
        // (See OLEDB spec IDBInfo::GetLiteralInfo DBLITERAL_QUOTE_SUFFIX.)
        if (quoteSuffix == null) {
    ...
    private DataTable GetDataTypesTable(OleDbConnection connection) {
      ...
      DataTable providerTypesTable =
          connection.GetOleDbSchemaTable(OleDbSchemaGuid.Provider_Types, null);
      ...
      DataColumn[] sourceColumns =
          new DataColumn[]{providerTypesTable.Columns["TYPE_NAME"],
      ...
      foreach(DataRow sourceRow in providerTypesTable.Rows) {
        ...
        short nativeDataType = (short) Convert.ChangeType(
            sourceRow[sourceOleDbType], typeof (short),
            CultureInfo.InvariantCulture);
        NativeDBType nativeType = NativeDBType.FromDBType(
            nativeDataType, (bool) newRow[isLong], (bool) newRow[isFixed]);
        ...
        if ((isSearchable != null) && (isSearchableWithLike != null) &&
            (searchable != null)) {
    ...
    private DataTable
    GetReservedWordsTable(OleDbConnectionInternal internalConnection) {
      ...
      if (!internalConnection.AddInfoKeywordsToTable(reservedWordsTable,
                                                     reservedWordColumn)) {
    ...
    protected override DataTable PrepareCollection(String collectionName,
      ...
      OleDbConnectionInternal oleDbInternalConnection =
          (OleDbConnectionInternal)(oleDbConnection.InnerConnection);
      ...
      if (collectionName == DbMetaDataCollectionNames.DataSourceInformation) {
        ...
        resultTable = GetDataSourceInformationTable(oleDbConnection,
        ...
        for (int i = 0; i < _schemaMapping.Length; i++) {
          if (_schemaMapping[i]._schemaName == collectionName) {
            ...
            // need to special case the oledb schema rowset restrictions on
            // columns that are not string tpyes
            ...
            if (restrictions != null) {
              ...
              foreach(DataRow row in metaDataCollectionsTable.Rows) {
                string candidateCollectionName =
                    ((string) row[DbMetaDataColumnNames.CollectionName,
                                  DataRowVersion.Current]);

                if (collectionName == candidateCollectionName) {
                  numberOfSupportedRestictions =
                      (int) row[DbMetaDataColumnNames.NumberOfRestrictions];
              ...
              // the 4th restrictionon the indexes schema rowset(type) is an I2
              // - enum
              ...
              if ((collectionName == OleDbMetaDataCollectionNames.Indexes) &&
                ...
                if ((restrictions[indexRestrictionTypeSlot] ==
                     "DBPROPVAL_IT_BTREE") ||
                    (restrictions[indexRestrictionTypeSlot] == "1")) {
                  indexTypeValue = 1;
                } else if ((restrictions[indexRestrictionTypeSlot] ==
                            "DBPROPVAL_IT_HASH") ||
              ...
              // the 4th restrictionon the procedures schema rowset(type) is an
              // I2 - enum
              ...
              if ((collectionName == OleDbMetaDataCollectionNames.Procedures) &&
                ...
                if ((restrictions[procedureRestrictionTypeSlot] ==
                     "DB_PT_UNKNOWN") ||
                    (restrictions[procedureRestrictionTypeSlot] == "1")) {
                  procedureTypeValue = 1;
                } else if ((restrictions[procedureRestrictionTypeSlot] ==
                            "DB_PT_PROCEDURE") ||
    ...
    private void SetIdentifierCase(string columnName, int propertyID,
