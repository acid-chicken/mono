...
namespace System.Data.Odbc {
  ...
  internal class OdbcMetaDataFactory : DbMetaDataFactory {
    ...
    internal OdbcMetaDataFactory(Stream XMLStream, string serverVersion,
      ...
      if (metaDataCollectionsTable == null) {
        throw ADP.UnableToBuildCollection(
            DbMetaDataCollectionNames.MetaDataCollections);
      }

      // copy the table filtering out any rows that don't apply to the current
      ...
      if (restrictionsTable != null) {
        ...
        restrictionsTable = CloneAndFilterCollection(
            DbMetaDataCollectionNames.Restrictions, null);
      }
      ...
      DataColumn populationMechanism =
          metaDataCollectionsTable.Columns[_populationMechanism];
      DataColumn collectionName =
      ...
      foreach(DataRow collection in metaDataCollectionsTable.Rows) {
        if ((string) collection[populationMechanism] == _prepareCollection) {
          ...
          for (int i = 0; i < _schemaMapping.Length; i++) {
            if (_schemaMapping[i]._schemaName ==
    ...
    private object BooleanFromODBC(object odbcSource) {
      ...
      if (odbcSource != DBNull.Value) {
        ...
        // some odbc drivers report the odbcSource value as unsigned, in which
        // case we will have upgraded the type to Int32, and thus can't cast
        // directly to short
    ...
    private DataTable DataTableFromDataReader(IDataReader reader,
                                              string tableName) {

      // set up the column structure of the data table from the reader
      object[] values;
      DataTable resultTable =
          NewDataTableFromReader(reader, out values, tableName);
    ...
    private void DataTableFromDataReaderDataTypes(DataTable dataTypesTable,
      ...
      DataColumn typeNameColumn =
          dataTypesTable.Columns[DbMetaDataColumnNames.TypeName];
      DataColumn providerDbTypeColumn =
      ...
      DataColumn isSearchableColumn =
          dataTypesTable.Columns[DbMetaDataColumnNames.IsSearchable];
      DataColumn isSearchableWithLikeColumn =
          dataTypesTable.Columns[DbMetaDataColumnNames.IsSearchableWithLike];
      DataColumn isUnsignedColumn =
          dataTypesTable.Columns[DbMetaDataColumnNames.IsUnsigned];
      DataColumn maximumScaleColumn =
          dataTypesTable.Columns[DbMetaDataColumnNames.MaximumScale];
      DataColumn minimumScaleColumn =
          dataTypesTable.Columns[DbMetaDataColumnNames.MinimumScale];
      DataColumn literalPrefixColumn =
          dataTypesTable.Columns[DbMetaDataColumnNames.LiteralPrefix];
      DataColumn literalSuffixColumn =
          dataTypesTable.Columns[DbMetaDataColumnNames.LiteralSuffix];
      DataColumn SQLTypeNameColumn =
          dataTypesTable.Columns[OdbcMetaDataColumnNames.SQLType];
      ...
      while (dataReader.Read()) {
        ...
        sqlType = (ODBC32.SQL_TYPE)(Int32) Convert.ChangeType(
            getTypeInfoValues[indexDATA_TYPE], typeof (Int32),
            (System.IFormatProvider) null);
        // if the driver is pre version 3 and it returned the v2 SQL_DATE or
        // SQL_TIME types they need to be mapped to thier v3 equlivants
        ...
        // FromSqlType will throw an argument exception if it does not recognize
        // the SqlType. This is not an error since the GetTypeInfo DATA_TYPE may
        // be a SQL data type or a driver specific type. If there is no TypeMap
        // for the type its not an error but it will degrade our level of
        // understanding of/ support for the type.
        catch (ArgumentException) {
          typeMap = null;
        }

        // if we have a type map we can determine the dbType and the CLR type if
        // not leave them null
        if (typeMap != null) {
          dataTypesRow[providerDbTypeColumn] = typeMap._odbcType;
          dataTypesRow[dataTypeColumn] = typeMap._type.FullName;
          // setting isLong and isFixedLength only if we have a type map because
          // for provider specific types we have no idea what the types
          // attributes are if GetTypeInfo did not tell us
          switch (sqlType) {
        ...
        dataTypesRow[createParametersColumn] =
            getTypeInfoValues[indexCREATE_PARAMS];

        if ((getTypeInfoValues[indexAUTO_UNIQUE_VALUE] == DBNull.Value) ||
            (Convert.ToInt16(getTypeInfoValues[indexAUTO_UNIQUE_VALUE], null) ==
             0)) {
        ...
        dataTypesRow[isCaseSensitiveColumn] =
            BooleanFromODBC(getTypeInfoValues[indexCASE_SENSITIVE]);
        dataTypesRow[isFixedPrecisionScaleColumn] =
            BooleanFromODBC(getTypeInfoValues[indexFIXED_PREC_SCALE]);

        if (getTypeInfoValues[indexNULLABLE] != DBNull.Value) {
          // Use Convert.ToInt16 instead of direct cast to short because the
          // value will be Int32 in some cases
          switch ((ODBC32.SQL_NULLABILITY)
        ...
        if (DBNull.Value != getTypeInfoValues[indexSEARCHABLE]) {
          ...
          // Use Convert.ToInt16 instead of direct cast to short because the
          // value will be Int32 in some cases
          Int16 searchableValue =
              Convert.ToInt16(getTypeInfoValues[indexSEARCHABLE], null);
        ...
        dataTypesRow[isUnsignedColumn] =
            BooleanFromODBC(getTypeInfoValues[indexUNSIGNED_ATTRIBUTE]);

        // For assignment to the DataSet, don't cast the data types -- let the
        // DataSet take care of any conversion
        if (getTypeInfoValues[indexMAXIMUM_SCALE] != DBNull.Value) {
          dataTypesRow[maximumScaleColumn] =
              getTypeInfoValues[indexMAXIMUM_SCALE];
        }

        if (getTypeInfoValues[indexMINIMUM_SCALE] != DBNull.Value) {
          dataTypesRow[minimumScaleColumn] =
              getTypeInfoValues[indexMINIMUM_SCALE];
        }

        if (getTypeInfoValues[indexLITERAL_PREFIX] != DBNull.Value) {
          dataTypesRow[literalPrefixColumn] =
              getTypeInfoValues[indexLITERAL_PREFIX];
        }

        if (getTypeInfoValues[indexLITERAL_SUFFIX] != DBNull.Value) {
          dataTypesRow[literalSuffixColumn] =
              getTypeInfoValues[indexLITERAL_SUFFIX];
        }
    ...
    private DataTable
    DataTableFromDataReaderIndex(IDataReader reader, string tableName,
      ...
      while (reader.Read()) {
        reader.GetValues(values);
        if (IncludeIndexRow(values[positionOfIndexName], restrictionIndexName,
    ...
    private DataTable DataTableFromDataReaderProcedureColumns(
        IDataReader reader, string tableName, Boolean isColumn) {
      ...
      while (reader.Read()) {
        ...
        if (values[positionOfColumnType].GetType() == typeof (short)) {
          if ((((short)
                    values[positionOfColumnType] == ODBC32.SQL_RESULT_COL) &&
               (isColumn == true)) ||
              (((short)
                    values[positionOfColumnType] != ODBC32.SQL_RESULT_COL) &&
               (isColumn == false))) {
    ...
    private DataTable DataTableFromDataReaderProcedures(IDataReader reader,
                                                        string tableName,
                                                        Int16 procedureType) {
    ...
    private DataTable GetColumnsCollection(String[] restrictions,
      ...
      try {
        ...
        FillOutRestrictions(columnsRestrictionsCount, restrictions,
                            allRestrictions,
        ...
        resultTable = DataTableFromDataReader(
            dataReader, OdbcMetaDataCollectionNames.Columns);
      }
    ...
    private DataTable
    GetDataSourceInformationCollection(string[] restrictions,
                                       OdbcConnection connection) {

      if (ADP.IsEmptyArray(restrictions) == false) {
        throw ADP.TooManyRestrictions(
            DbMetaDataCollectionNames.DataSourceInformation);
      }

      // verify that the data source information table is in the data set
      DataTable dataSourceInformationTable =
          CollectionDataSet
              .Tables[DbMetaDataCollectionNames.DataSourceInformation];
      ...
      dataSourceInformationTable = CloneAndFilterCollection(
          DbMetaDataCollectionNames.DataSourceInformation, null);
      ...
      stringValue = connection.GetInfoStringUnhandled(
          ODBC32.SQL_INFO.CATALOG_NAME_SEPARATOR);
      if (!ADP.IsEmpty(stringValue)) {
        ...
        dataSourceInformation[DbMetaDataColumnNames
      ...
      // get the DBMS Name
      stringValue =
          connection.GetInfoStringUnhandled(ODBC32.SQL_INFO.DBMS_NAME);
      if (stringValue != null) {
      ...
      dataSourceInformation[DbMetaDataColumnNames.DataSourceProductVersion] =
          ServerVersion;
      dataSourceInformation[DbMetaDataColumnNames
      ...
      // leave the column null if the GetInfo fails. There is no explicit value
      // for unknown.
      if (connection.IsV3Driver) {

        retcode = connection.GetInfoInt32Unhandled(
            ODBC32.SQL_INFO.SQL_OJ_CAPABILITIES_30, out int32Value);
      } else {
        retcode = connection.GetInfoInt32Unhandled(
            ODBC32.SQL_INFO.SQL_OJ_CAPABILITIES_20, out int32Value);
      }
      ...
      retcode = connection.GetInfoInt16Unhandled(ODBC32.SQL_INFO.GROUP_BY,
                                                 out int16Value);
      ...
      if ((retcode == ODBC32.RetCode.SUCCESS) ||
        ...
        case (Int16) ODBC32.SQL_GROUP_BY.NO_RELATION:
          ...
          break;
          /* COLLATE is new in ODBC 3.0 and GroupByBehavior does not have a
             value for it. case ODBC32.SQL_GROUP_BY.COLLATE: groupByBehavior =
             Common.GroupByBehavior.Unknown; break;
          */
        }
      }

      dataSourceInformation[DbMetaDataColumnNames.GroupByBehavior] =
          groupByBehavior;
      ...
      dataSourceInformation[DbMetaDataColumnNames.IdentifierCase] =
          identifierCase;
      ...
      // build the QuotedIdentifierPattern using the quote prefix and suffix
      // from the provider and assuming that the quote suffix is escaped via
      // repetion (i.e " becomes "")
      stringValue = connection.QuoteChar(ADP.GetSchema);

      if (stringValue != null) {

        // by spec a blank identifier quote char indicates that the provider
        // does not suppport quoted identifiers
        if (stringValue != " ") {

          // only know how to build the parttern if the quote characters is 1
          // character in all other cases just leave the field null
      ...
      Common.IdentifierCase quotedIdentifierCase =
          Common.IdentifierCase.Unknown;
    ...
    private DataTable GetDataTypesCollection(String[] restrictions,
      ...
      DataTable dataTypesTable =
          CollectionDataSet.Tables[DbMetaDataCollectionNames.DataTypes];
      ...
      dataTypesTable =
          CloneAndFilterCollection(DbMetaDataCollectionNames.DataTypes, null);
      ...
      try {
        ...
        DataTableFromDataReaderDataTypes(dataTypesTable, dataReader,
                                         connection);
      }
    ...
    private DataTable GetIndexCollection(String[] restrictions,
      ...
      try {
        ...
        FillOutRestrictions(indexRestrictionsCount, restrictions,
                            allRestrictions,
        ...
        resultTable = DataTableFromDataReaderIndex(
            dataReader, OdbcMetaDataCollectionNames.Indexes, indexName);
      }
    ...
    private DataTable GetProcedureColumnsCollection(String[] restrictions,
      ...
      try {
        ...
        String[] allRestrictions =
            new string[procedureColumnsRestrictionsCount];
        FillOutRestrictions(procedureColumnsRestrictionsCount, restrictions,
                            allRestrictions,
                            OdbcMetaDataCollectionNames.Columns);
        ...
        resultTable = DataTableFromDataReaderProcedureColumns(
            dataReader, collectionName, isColumns);
      }
    ...
    private DataTable GetReservedWordsCollection(string[] restrictions,
      ...
      // verify the existence of the table in the data set
      DataTable reservedWordsTable =
          CollectionDataSet.Tables[DbMetaDataCollectionNames.ReservedWords];
      if (reservedWordsTable == null) {
        throw ADP.UnableToBuildCollection(
            DbMetaDataCollectionNames.ReservedWords);
      }

      // copy the table filtering out any rows that don't apply to tho current
      // version of the provider
      reservedWordsTable = CloneAndFilterCollection(
          DbMetaDataCollectionNames.ReservedWords, null);

      DataColumn reservedWordColumn =
          reservedWordsTable.Columns[DbMetaDataColumnNames.ReservedWord];
      ...
      string keywords =
          connection.GetInfoStringUnhandled(ODBC32.SQL_INFO.KEYWORDS);
    ...
    private DataTable GetTablesCollection(String[] restrictions,
      ...
      try {
        ...
        FillOutRestrictions(tablesRestrictionsCount, restrictions, allArguments,
                            dataTableName);
    ...
    private Boolean IncludeIndexRow(object rowIndexName,
      ...
      if ((restrictionIndexName != null) &&
          (restrictionIndexName != (string) rowIndexName)) {
        return false;
      }
    ...
    private DataTable NewDataTableFromReader(IDataReader reader,
                                             out object[] values,
                                             string tableName) {
      ...
      foreach(DataRow row in schemaTable.Rows) {
        resultTable.Columns.Add(row["ColumnName"] as string,
    ...
    protected override DataTable PrepareCollection(String collectionName,
                                                   String[] restrictions,
      ...
      if (collectionName == OdbcMetaDataCollectionNames.Tables) {
        ...
        resultTable =
            GetProcedureColumnsCollection(restrictions, odbcConnection, true);
      } else if (collectionName ==
                 OdbcMetaDataCollectionNames.ProcedureParameters) {
        resultTable =
            GetProcedureColumnsCollection(restrictions, odbcConnection, false);
        ...
        resultTable = GetDataTypesCollection(restrictions, odbcConnection);
                 ...
                 DbMetaDataCollectionNames.DataSourceInformation) {
        resultTable =
            GetDataSourceInformationCollection(restrictions, odbcConnection);
