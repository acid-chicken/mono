...
namespace System.Data.Common {
  ...
  [Serializable] // MDAC 83147
    ...
    // this class is serializable with Everett, so ugly field names can't be
    // changed
    ...
    // a linked list of key/value and their length in
    // _encryptedUsersConnectionString
    readonly private NameValuePair _keychain;

    // track the existence of "password" or "pwd" in the connection string
    // not used for anything anymore but must keep it set correct for V1.1
    // serialization
...
#pragma warning restore 169
    ...
    internal DBConnectionString(string value, string restrictions,
        ...
        : this(new DbConnectionOptions(value, synonyms, useOdbcRules),
               restrictions, behavior, synonyms, false) {
      // useOdbcRules is only used to parse the connection string, not to parse
      // restrictions because values don't apply there the hashtable doesn't
      // need clone since it isn't shared with anything else
    }

    internal DBConnectionString(DbConnectionOptions connectionOptions)
        : this(connectionOptions, (string) null,
               KeyRestrictionBehavior.AllowOnly, (Hashtable) null, true) {
      // used by DBDataPermission to convert from DbConnectionOptions to
      // DBConnectionString since backward compatability requires Everett level
      // classes
    }

    private DBConnectionString(
        DbConnectionOptions connectionOptions, string restrictions,
        KeyRestrictionBehavior behavior, Hashtable synonyms,
        bool mustCloneDictionary) { // used by DBDataPermission
      ...
      _encryptedUsersConnectionString =
          connectionOptions.UsersConnectionString(false);
      _hasPassword = connectionOptions.HasPasswordKeyword;
      _parsetable = connectionOptions.Parsetable;
      _keychain = connectionOptions._keyChain;

      // we do not want to serialize out user password unless directed so by
      // "persist security info=true" otherwise all instances of user's password
      ...
      if (_hasPassword && !connectionOptions.HasPersistablePassword) {
        ...
        if (mustCloneDictionary) {
          ...
          // we only need to clone if coming from DbConnectionOptions and
          // password exists
        ...
        // replace user's password/pwd value with "*" in the linked list and
        // build a new string
        _keychain = connectionOptions.ReplacePasswordPwd(
            out _encryptedUsersConnectionString, true);
      }
    ...
    private DBConnectionString(DBConnectionString connectionString,
                               string[] restrictionValues,
                               KeyRestrictionBehavior behavior) {
      // used by intersect for two equal connection strings with different
      // restrictions
      _encryptedUsersConnectionString =
          connectionString._encryptedUsersConnectionString;
    ...
    internal DBConnectionString Intersect(DBConnectionString entry) {
      ...
      if (null == entry) {
        ...
        behavior = KeyRestrictionBehavior.AllowOnly;
                 ...
                 entry._behavior) { // subset of the AllowOnly array
        behavior = KeyRestrictionBehavior.AllowOnly;

        if (KeyRestrictionBehavior.AllowOnly ==
          ...
          if (!ADP.IsEmptyArray(_restrictionValues)) {
            if (!ADP.IsEmptyArray(entry._restrictionValues)) {
              // Debug.WriteLine("1 this PreventUsage with restrictions and
              // entry AllowOnly with restrictions");
              restrictionValues = NewRestrictionAllowOnly(
                  entry._restrictionValues, _restrictionValues);
            } else {
              // Debug.WriteLine("2 this PreventUsage with restrictions and
              // entry AllowOnly with no restrictions");
            }
          } else {
            // Debug.WriteLine("3/4 this PreventUsage with no restrictions and
            // entry AllowOnly");
            restrictionValues = entry._restrictionValues;
          }
        } else if (!ADP.IsEmptyArray(
                       _restrictionValues)) { // this AllowOnly and entry
                                              // PreventUsage
          if (!ADP.IsEmptyArray(entry._restrictionValues)) {
            // Debug.WriteLine("5 this AllowOnly with restrictions and entry
            // PreventUsage with restrictions");
            restrictionValues = NewRestrictionAllowOnly(
                _restrictionValues, entry._restrictionValues);
          } else {
            // Debug.WriteLine("6 this AllowOnly and entry PreventUsage with no
            // restrictions");
            restrictionValues = _restrictionValues;
          }
        } else {
          // Debug.WriteLine("7/8 this AllowOnly with no restrictions and entry
          // PreventUsage");
        }
      } else if (KeyRestrictionBehavior.PreventUsage ==
                 this._behavior) { // both PreventUsage
        if (ADP.IsEmptyArray(_restrictionValues)) {
          // Debug.WriteLine("9/10 both PreventUsage and this with no
          // restrictions");
          restrictionValues = entry._restrictionValues;
        } else if (ADP.IsEmptyArray(entry._restrictionValues)) {
          // Debug.WriteLine("11 both PreventUsage and entry with no
          // restrictions");
          restrictionValues = _restrictionValues;
        } else {
          // Debug.WriteLine("12 both PreventUsage with restrictions");
          restrictionValues =
              NoDuplicateUnion(_restrictionValues, entry._restrictionValues);
        }
      } else if (!ADP.IsEmptyArray(_restrictionValues) &&
                 !ADP.IsEmptyArray(
                     entry._restrictionValues)) { // both AllowOnly with
                                                  // restrictions
        if (this._restrictionValues.Length <= entry._restrictionValues.Length) {
          // Debug.WriteLine("13a this AllowOnly with restrictions and entry
          // AllowOnly with restrictions");
          restrictionValues = NewRestrictionIntersect(_restrictionValues,
                                                      entry._restrictionValues);
        } else {
          // Debug.WriteLine("13b this AllowOnly with restrictions and entry
          // AllowOnly with restrictions");
          restrictionValues = NewRestrictionIntersect(entry._restrictionValues,
                                                      _restrictionValues);
        }
      } else { // both AllowOnly
        // Debug.WriteLine("14/15/16 this AllowOnly and entry AllowOnly but no
        // restrictions");
      }

      // verify _hasPassword & _parsetable are in sync between Everett/Whidbey
      Debug.Assert(!_hasPassword || ContainsKey(KEY.Password) ||
                       ContainsKey(KEY.Pwd),
                   "OnDeserialized password mismatch this");
      Debug.Assert(null == entry || !entry._hasPassword ||
                       entry.ContainsKey(KEY.Password) ||
                       entry.ContainsKey(KEY.Pwd),
                   "OnDeserialized password mismatch entry");

      DBConnectionString value =
          new DBConnectionString(this, restrictionValues, behavior);
    ...
    [Conditional("DEBUG")]
    private void
    ValidateCombinedSet(DBConnectionString componentSet,
                        DBConnectionString combinedSet) {
      Debug.Assert(combinedSet != null,
                   "The combined connection string should not be null");
      if ((componentSet != null) && (combinedSet._restrictionValues != null) &&
        ...
        if (componentSet._behavior == KeyRestrictionBehavior.AllowOnly) {
          if (combinedSet._behavior == KeyRestrictionBehavior.AllowOnly) {
            ...
            // All values in the Combined Set should also be in the Component
            // Set Combined - Component == null
            Debug.Assert(
            ...
            Debug.Assert(false,
                         string.Format("Unknown behavior for combined set: {0}",
                                       combinedSet._behavior));
          }
        } else if (componentSet._behavior ==
                   KeyRestrictionBehavior.PreventUsage) {
          if (combinedSet._behavior == KeyRestrictionBehavior.AllowOnly) {
            // Component==PreventUsage, Combined==Allow
            // There shouldn't be any of the values from the Component Set in
            // the Combined Set Intersect(Component, Combined) == null
            Debug.Assert(
                combinedSet._restrictionValues
            ...
            // All values in the Component Set should also be in the Combined
            // Set Component - Combined == null
            Debug.Assert(
          ...
          Debug.Assert(false,
                       string.Format("Unknown behavior for component set: {0}",
                                     componentSet._behavior));
        }
      }
    }
    ...
    private bool IsRestrictedKeyword(string key) {
      ...
      return ((null == _restrictionValues) ||
              (0 > Array.BinarySearch(_restrictionValues, key,
    ...
    internal bool IsSupersetOf(DBConnectionString entry) {
      Debug.Assert(!_hasPassword || ContainsKey(KEY.Password) ||
                       ContainsKey(KEY.Pwd),
                   "OnDeserialized password mismatch this");
      Debug.Assert(!entry._hasPassword || entry.ContainsKey(KEY.Password) ||
                       entry.ContainsKey(KEY.Pwd),
                   "OnDeserialized password mismatch entry");

      switch (_behavior) {
      case KeyRestrictionBehavior.AllowOnly:
        // every key must either be in the resticted connection string or in the
        // allowed keywords keychain may contain duplicates, but it is better
        // than GetEnumerator on _parsetable.Keys
        for (NameValuePair current = entry.KeyChain; null != current;
        ...
        break;
      case KeyRestrictionBehavior.PreventUsage:
        // every key can not be in the restricted keywords (even if in the
        // restricted connection string)
    ...
    static private string[] NewRestrictionAllowOnly(string[] allowonly,
                                                    string[] preventusage) {
      List<string>newlist = null;
      for (int i = 0; i < allowonly.Length; ++i) {
        if (0 > Array.BinarySearch(preventusage, allowonly[i],
                                   StringComparer.Ordinal)) {
          if (null == newlist) {
    ...
    private static string[] ParseRestrictions(string restrictions,
                                              Hashtable synonyms) {
#if DEBUG
      if (Bid.AdvancedOn) {
      ...
      while (nextStartPosition < endPosition) {
        ...
        string keyname,
            keyvalue; // since parsing restrictions ignores values, it doesn't
                      // matter if we use ODBC rules or OLEDB rules
        nextStartPosition = DbConnectionOptions.GetKeyValuePair(
            restrictions, startPosition, buffer, false, out keyname,
            out keyvalue);
...
#endif
          string realkeyname = ((null != synonyms) ? (string) synonyms[keyname]
                                                   : keyname); // MDAC 85144
    ...
    private static void
    Verify(string[] restrictionValues) {
      if (null != restrictionValues) {
        for (int i = 1; i < restrictionValues.Length; ++i) {
          Debug.Assert(!ADP.IsEmpty(restrictionValues[i - 1]),
                       "empty restriction");
          Debug.Assert(!ADP.IsEmpty(restrictionValues[i]), "empty restriction");
          Debug.Assert(0 >=
                       StringComparer.Ordinal.Compare(restrictionValues[i - 1],
                                                      restrictionValues[i]));
        }
      }
    }
