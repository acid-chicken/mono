...
namespace System.Resources {
  ...
  // System.Resources.WindowsRuntimeResourceManager, allowing us to ask for a
  // WinRT-specific ResourceManager. It is important to have
  // WindowsRuntimeResourceManagerBase as regular class with virtual methods and
  // default implementations. Defining WindowsRuntimeResourceManagerBase as
  // abstract class or interface will cause issues when adding more methods to
  // it because it�ll create dependency between mscorlib and
  // System.Runtime.WindowsRuntime which will require always shipping both DLLs
  // together. Also using interface or abstract class will not play nice with
  // FriendAccessAllowed.
  //
  [FriendAccessAllowed]
  [SecurityCritical]
  internal class WindowsRuntimeResourceManagerBase {
    [SecurityCritical]
    public virtual bool
    Initialize(string libpath, string reswFilename,
    ...
    GetString(String stringName, String startingCulture,
              String neutralResourcesCulture) {
      return null;
    }

    public virtual CultureInfo
        GlobalResourceContextBestFitCultureInfo{[SecurityCritical] get{
            return null;
  }
}
...
[SecurityCritical]
public virtual bool
SetGlobalResourceContextDefaultCulture(CultureInfo ci) {
  return false;
}
}
...
[FriendAccessAllowed]
    ...
    // made a breaking change such that ResourceMap.GetSubtree returns null when
    // a subtree is not found instead of throwing an exception. As a result the
    // name of this class is no longer accurate. It should be called
    // PRIErrorInfo. However changing the name of this internal class would
    // cause mscorlib.asmmeta and System.Runtime.WindowsRuntime.asmmeta to
    // change, which would in turn require updating of the mscorlib and
    // System.Runtime.WindowsRuntime reference assemblies under InternalApis.
    // This would not meet the Ask Mode bar at this time. To get an idea of
    // which files may need to be updated when updating this name, see changeset
    // 399234 in the DevDiv2 database, though the update procedure may have
...
#pragma warning restore
  ...
  private static volatile Hashtable
      _installedSatelliteInfo; // Give the user the option
  ...
  private static volatile bool
      _checkedConfigFile; // Did we read the app's config file?
#endif
  ...
  // 4) IResourceReader type name for this file (bytelength-prefixed UTF-8
  // String) 5) ResourceSet type name for this file (bytelength-prefixed UTF8
  // String)
  public static readonly int HeaderVersionNumber = 1;

  //
  // It would be better if we could use _neutralCulture instead of calling
  // CultureInfo.InvariantCulture everywhere, but we run into problems with the
  // .cctor.  CultureInfo initializes assembly, which initializes
  // ResourceManager, which tries to get a CultureInfo which isn't there yet
  // because CultureInfo's class initializer hasn't finished.  If we move
  // SystemResMgr off of Assembly (or at least make it an internal property) we
  ...
  internal static readonly String ResReaderTypeName =
      typeof(ResourceReader).FullName;
  internal static readonly String ResSetTypeName =
      typeof(RuntimeResourceSet).FullName;
  internal static readonly String MscorlibName =
      typeof(ResourceReader).Assembly.FullName;
  ...
  internal static readonly int DEBUG =
      0; // Making this const causes C# to consider all of the code that it
         // guards unreachable.
#if FEATURE_APPX
  ...
  [MethodImplAttribute(
      MethodImplOptions.NoInlining)] // Methods containing StackCrawlMark local
                                     // var has to be marked non-inlineable
      private void
      Init() {
    try {
...
#if !FEATURE_CORECLR
    // This constructor is not designed to be used under AppX and is not in the
    // Win8 profile. However designers may use them even if they are running
    // under AppX since they are not subject to the restrictions of the Win8
    // profile.
...
#pragma warning restore 618
...
#if !FEATURE_CORECLR && !MONO // PAL doesn't support eventing, and we don't
                              // compile event providers for coreclr
    if (FrameworkEventSource.IsInitialized &&
      ...
      if (resourceGroveler.HasNeutralResources(culture, defaultResName)) {
        FrameworkEventSource.Log.ResourceManagerNeutralResourcesFound(
            BaseNameField, MainAssembly, defaultResName);
      } else {
  ...
  [MethodImplAttribute(
      MethodImplOptions.NoInlining)] // Methods containing StackCrawlMark local
                                     // var have to be marked non-inlineable
  ...
  [MethodImplAttribute(
    ...
    if (usingResourceSet != null && (usingResourceSet != _minResourceSet) &&
  ...
  [OnDeserializing]
  private void
  OnDeserializing(StreamingContext ctx) {
  ...
  [OnDeserialized]
  private void
  OnDeserialized(StreamingContext ctx) {
    _resourceSets = new Dictionary<String, ResourceSet>();
    _lastUsedResourceCache = new CultureNameResourceSetPair();
  ...
  [OnSerializing]
  private void
  OnSerializing(StreamingContext ctx) {
...
#if !FEATURE_CORECLR && FEATURE_APPX // PAL doesn't support eventing, and we
                                     // don't compile event providers for
                                     // coreclr
    if (_bUsingModernResourceManagement == false) {
      if (FrameworkEventSource.IsInitialized &&
          FrameworkEventSource.Log.IsEnabled()) {
        ...
        if (resourceGroveler.HasNeutralResources(culture, defaultResName)) {
          ...
          FrameworkEventSource.Log.ResourceManagerNeutralResourcesNotFound(
              BaseNameField, MainAssembly, outputResName);
        }
      }
#pragma warning disable 618
      ResourceSets = new Hashtable(); // for backward compatibility
#pragma warning restore 618
  ...
  public virtual Type ResourceSetType {
    get {
      return (_userResourceSet == null) ? typeof (RuntimeResourceSet)
                                        : _userResourceSet;
    }
  }
  ...
  public virtual void ReleaseAllResources() {
    ...
    if (FrameworkEventSource.IsInitialized) {
      FrameworkEventSource.Log.ResourceManagerReleasingResources(BaseNameField,
                                                                 MainAssembly);
    }
#endif
...
#pragma warning restore 618
  ...
  CreateFileBasedResourceManager(String baseName, String resourceDir,
                                 Type usingResourceSet) {
  ...
  // ResourceFallbackManager.GetEnumerator() Return the first ResourceSet, based
  // on the first culture ResourceFallbackManager would return
  internal ResourceSet GetFirstResourceSet(CultureInfo culture) {
    // Logic from ResourceFallbackManager.GetEnumerator()
    if (_neutralResourcesCulture != null &&
        culture.Name == _neutralResourcesCulture.Name) {
  ...
  [MethodImplAttribute(
      MethodImplOptions.NoInlining)] // Methods containing StackCrawlMark local
                                     // var have to be marked non-inlineable
      public virtual ResourceSet
      GetResourceSet(CultureInfo culture, bool createIfNotExists,
                     bool tryParents) {
    if (null == culture)
    ...
    if (UseManifest && culture.HasInvariantCultureName) {
      ...
      Stream stream = mainAssembly.GetManifestResourceStream(
          _locationInfo, fileName, m_callingAssembly == MainAssembly,
          ref stackMark);
    ...
    // Note: ideally we could plumb through the stack crawl mark here, but we
    // must call the virtual 3-argument InternalGetResourceSet method for
    // compatibility. Security-wise, we're not overly interested in protecting
    // access to resources, since full-trust callers can get them already and
    // most resources are public. Also, the JIT inliner could always inline a
    // caller into another assembly's method, so the entire idea of a security
    // check written this way is ----. So if we happen to return some resources
    // in cases where we should really be doing a demand for member access
    // permissions, we're not overly concerned.
    // <
  ...
  [System.Security.SecuritySafeCritical] // auto-generated
      ...
      [MethodImplAttribute(
          MethodImplOptions.NoInlining)] // Methods containing StackCrawlMark
                                         // local var has to be marked
                                         // non-inlineable
      protected virtual ResourceSet
      InternalGetResourceSet(CultureInfo culture, bool createIfNotExists,
                             bool tryParents) {
  ...
  [ResourceConsumption(ResourceScope.Machine, ResourceScope.Machine)]
  private ResourceSet
  InternalGetResourceSet(CultureInfo requestedCulture, bool createIfNotExists,
                         bool tryParents, ref StackCrawlMark stackMark) {
    Dictionary<String, ResourceSet>localResourceSets = _resourceSets;
    ResourceSet rs = null;
      ...
      if (localResourceSets.TryGetValue(requestedCulture.Name, out rs)) {
        ...
        if (FrameworkEventSource.IsInitialized) {
          FrameworkEventSource.Log.ResourceManagerFoundResourceSetInCache(
              BaseNameField, MainAssembly, requestedCulture.Name);
        }
#endif
        return rs;
      }
    ...
    ResourceFallbackManager mgr = new ResourceFallbackManager(
        requestedCulture, _neutralResourcesCulture, tryParents);

    foreach(CultureInfo currentCultureInfo in mgr) {
#if !FEATURE_CORECLR && !MONO
      if (FrameworkEventSource.IsInitialized) {
        FrameworkEventSource.Log.ResourceManagerLookingForResourceSet(
            BaseNameField, MainAssembly, currentCultureInfo.Name);
      }
#endif
      lock(localResourceSets) {
      ...
      rs = resourceGroveler.GrovelForResourceSet(
          currentCultureInfo, localResourceSets, tryParents, createIfNotExists,
          ref stackMark);
  ...
  private static void
  AddResourceSet(Dictionary<String, ResourceSet>localResourceSets,
                 String cultureName, ref ResourceSet rs) {
    ...
    lock(localResourceSets) {
      ...
      if (localResourceSets.TryGetValue(cultureName, out lostRace)) {
        if (!Object.ReferenceEquals(lostRace, rs)) {
          // Note: In certain cases, we can be trying to add a ResourceSet for
          // multiple cultures on one thread, while a second thread added
          // another ResourceSet for one of those cultures.  So when we lose the
          // ----, we must make sure our ResourceSet isn't in our dictionary
          // before closing it.
  ...
  protected static Version GetSatelliteContractVersion(Assembly a) {
    ...
    if (a == null) {
      throw new ArgumentNullException(
          "a", Environment.GetResourceString("ArgumentNull_Assembly"));
    }
    Contract.EndContractBlock();
    ...
    String v = null;
    if (a.ReflectionOnly) {
      foreach(CustomAttributeData data in
                  CustomAttributeData.GetCustomAttributes(a)) {
        if (data.Constructor.DeclaringType ==
    ...
    try {
      ...
      throw new ArgumentException(
          Environment.GetResourceString("Arg_InvalidSatelliteContract_Asm_Ver",
                                        a.ToString(), v),
          e);
    }
    return ver;
#else
      // On the phone return null. The calling code will use the assembly
      // version instead to avoid potential type and library loads caused by CA
      // lookup. NetCF uses the assembly version always.
      return null;
#endif
  }

  [System.Security.SecuritySafeCritical] // auto-generated
      protected static CultureInfo
  ...
  // IGNORES VERSION
  internal static bool CompareNames(String asmTypeName1, String typeName2,
    ...
    // case sensitive
    if (String.Compare(asmTypeName1, 0, typeName2, 0, typeName2.Length,
                       StringComparison.Ordinal) != 0)
      return false;
    if (comma == -1)
      return true;

    // Now, compare assembly display names (IGNORES VERSION AND
    // PROCESSORARCHITECTURE) also, for  mscorlib ignores everything, since
    // that's what the binder is going to do
    while (Char.IsWhiteSpace(asmTypeName1[++comma]))
      ;

    // case insensitive
    AssemblyName an1 = new AssemblyName(asmTypeName1.Substring(comma));
    if (String.Compare(an1.Name, asmName2.Name,
                       StringComparison.OrdinalIgnoreCase) != 0)
      return false;

    // to match IsMscorlib() in VM
    if (String.Compare(an1.Name, "mscorlib",
                       StringComparison.OrdinalIgnoreCase) == 0)
      return true;
  ...
  [SecuritySafeCritical]
      ...
      private string
      GetStringFromPRI(String stringName, String startingCulture,
                       String neutralResourcesCulture) {
    ...
    // Do not handle exceptions. See the comment in SetAppXConfiguration about
    // throwing exception types that the ResourceManager class is not documented
    // to throw.
  ...
  // Since we can't directly reference System.Runtime.WindowsRuntime from
  // mscorlib, we have to get the type via reflection. It would be better if we
  // could just implement WindowsRuntimeResourceManager in mscorlib, but we
  // can't, because we can do very little with WinRT in mscorlib.
  [SecurityCritical]
  internal static WindowsRuntimeResourceManagerBase
  GetWinRTResourceManager() {
    Type WinRTResourceManagerType =
        Type.GetType("System.Resources.WindowsRuntimeResourceManager, " +
  ...
  [NonSerialized]
  private bool
      _bUsingModernResourceManagement; // Written only by SetAppXConfiguration

  [NonSerialized]
  [SecurityCritical]
  private WindowsRuntimeResourceManagerBase
      _WinRTResourceManager; // Written only by SetAppXConfiguration

  [NonSerialized]
  private bool _PRIonAppXInitialized; // Written only by SetAppXConfiguration

  [NonSerialized]
  private PRIExceptionInfo
  ...
  // 2) For non-FX assemblies, we use modern resource manager, with the premise
  // being that app package
  //    contains the PRI resources since such assemblies are expected to be
  ...
  //
  //    a) If the assembly lives under PLATFORM_RESOURCE_ROOTS (as specified by
  //    the host during AppDomain creation),
  //       then we will use satellite assembly based lookup in assemblies like
  //       *.resources.dll.
  //
  //    b) For any other non-FX assembly, we will use the modern resource
  //    manager with the premise that app package
  ...
  ShouldUseSatelliteAssemblyResourceLookupUnderAppX(
    ...
    if (!fUseSatelliteAssemblyResourceLookupUnderAppX) {
      // Check to see if the assembly is under PLATFORM_RESOURCE_ROOTS. If it
      // is, then we should use satellite assembly lookup for it.
      String platformResourceRoots =
          (String)(AppDomain.CurrentDomain.GetData("PLATFORM_RESOURCE_ROOTS"));
      if ((platformResourceRoots != null) &&
          (platformResourceRoots != String.Empty)) {
        string resourceAssemblyPath = resourcesAssembly.Location;

        // Loop through the PLATFORM_RESOURCE_ROOTS and see if the assembly is
        // contained in it.
        foreach(string pathPlatformResourceRoot in platformResourceRoots.Split(
            Path.PathSeparator)) {
          if (resourceAssemblyPath.StartsWith(
                  pathPlatformResourceRoot,
                  StringComparison.CurrentCultureIgnoreCase)) {
            // Found the resource assembly to be present in one of the
            // PLATFORM_RESOURCE_ROOT, so stop the enumeration loop.
  ...
  [SecuritySafeCritical]
#endif // FEATURE_APPX
      // Only call SetAppXConfiguration from ResourceManager constructors, and
      // nowhere else. Throws MissingManifestResourceException and WinRT
      // HResults
    ...
    Contract.Assert(_WinRTResourceManager ==
                    null); // Only this function writes to this member
    Contract.Assert(_PRIonAppXInitialized ==
                    false); // Only this function writes to this member
    Contract.Assert(_PRIExceptionInfo ==
                    null); // Only this function writes to this member
    ...
    if (resourcesAssembly != null) {
      if (resourcesAssembly !=
          typeof (Object).Assembly) // We are not loading resources for mscorlib
      {
        // Cannot load the WindowsRuntimeResourceManager when in a compilation
        // process, since it lives in System.Runtime.WindowsRuntime and only
        // mscorlib may be loaded for execution.
        if (AppDomain.IsAppXModel() && !AppDomain.IsAppXNGen) {
          s_IsAppXModel = true;

          // If we have the type information from the ResourceManager(Type)
          // constructor, we use it. Otherwise, we use BaseNameField.
          String reswFilename =
              _locationInfo == null ? BaseNameField : _locationInfo.FullName;

          // The only way this can happen is if a class inherited from
          // ResourceManager and did not set the BaseNameField before calling
          // the protected ResourceManager() constructor. For other
          // constructors, we would already have thrown an ArgumentNullException
          // by now. Throwing an ArgumentNullException now is not the right
          // thing to do because technically ResourceManager() takes no
          // arguments, and because it is not documented as throwing any
          // exceptions. Instead, let's go through the rest of the
          // initialization with this set to an empty string. We may in fact
          // fail earlier for another reason, but otherwise we will throw a
          // MissingManifestResourceException when GetString is called
          // indicating that a resW filename called "" could not be found.
          ...
          if (AppDomain.IsAppXDesignMode()) {
            ...
            try {
              PRIExceptionInfo exceptionInfo; // If the exception info is filled
                                              // in, we will ignore it.
              bWRRM_Initialized = WRRM.Initialize(
                  resourcesAssembly.Location, reswFilename, out exceptionInfo);
          ...
          if (!bUsingSatelliteAssembliesUnderAppX) {
            // See AssemblyNative::IsFrameworkAssembly for details on which
            // kinds of assemblies are considered Framework assemblies. The
            // Modern Resource Manager is not used for such assemblies - they
            // continue to use satellite assemblies (i.e. .resources.dll files).
            ...
            if (_bUsingModernResourceManagement) {
              ...
              // Note that if IsAppXDesignMode is false, we haven't checked if
              // the PRI file exists. This is by design. We will find out in the
              // call to WindowsRuntimeResourceManager.Initialize below.

              // At this point it is important NOT to set
              // _bUsingModernResourceManagement to false if the PRI file does
              // not exist because we are now certain we need to load PRI
              // resources. We want to fail by throwing a
              // MissingManifestResourceException if
              // WindowsRuntimeResourceManager.Initialize fails to locate the
              // PRI file. We do not want to fall back to using satellite
              // assemblies anymore. Note that we would not throw the
              // MissingManifestResourceException from this function, but from
              // GetString. See the comment below on the reason for this.
              ...
              if (WRRM != null && bWRRM_Initialized) {
                ...
                try {
                  _PRIonAppXInitialized = _WinRTResourceManager.Initialize(
                      resourcesAssembly.Location, reswFilename,
                ...
                // We would like to be able to throw a
                // MissingManifestResourceException here if PRI resources could
                // not be loaded for a recognized reason. However, the
                // ResourceManager constructors that call SetAppXConfiguration
                // are not documented as throwing
                // MissingManifestResourceException, and since they are part of
                // the portable profile, we cannot start throwing a new
                // exception type as that would break existing portable
                // libraries. Hence we must save the exception information now
                // and throw the exception on the first call to GetString.
                catch (FileNotFoundException) {
                  // We will throw MissingManifestResource_NoPRIresources from
                  // GetString when we see that _PRIonAppXInitialized is false.
                } catch (Exception e) {
                  // ERROR_MRM_MAP_NOT_FOUND can be thrown by the call to
                  // ResourceManager.get_AllResourceMaps in
                  // WindowsRuntimeResourceManager.Initialize. In this case
                  // _PRIExceptionInfo is now null and we will just throw the
                  // generic MissingManifestResource_NoPRIresources exception.
                  // See the implementation of GetString for more details.
                  if (e.HResult != __HResults.ERROR_MRM_MAP_NOT_FOUND)
                    throw; // Unexpected exception code. Bubble it up to the
                           // caller.
                }
                // Allow all other exception types to bubble up to the caller.

                // Yes, this causes us to potentially throw exception types that
                // are not documented.

                // Ultimately the tradeoff is the following:
                // -We could ignore unknown exceptions or rethrow them as inner
                // exceptions of exceptions that the ResourceManager class is
                // already documented as throwing. This would allow existing
                // portable libraries to gracefully recover if they don't care
                // too much about the ResourceManager object they are using.
                // However it could mask potentially fatal errors that we are
                // not aware of, such as a disk drive failing.

                // The alternative, which we chose, is to throw unknown
                // exceptions. This may tear down the process if the portable
                // library and app don't expect this exception type. On the
                // other hand, this won't mask potentially fatal errors we don't
                // know about.
              }
            }
          }
    ...
    // resourcesAssembly == null should not happen but it can. See the comment
    // on Assembly.GetCallingAssembly. However for the sake of 100% backwards
    // compatibility on Win7 and below, we must leave
  ...
  public virtual String GetString(String name, CultureInfo culture) {
...
#if FEATURE_APPX
    if (s_IsAppXModel) {
      // If the caller explictily passed in a culture that was obtained by
      // calling CultureInfo.CurrentUICulture, null it out, so that we
      // re-compute it.  If we use modern resource lookup, we may end up getting
      // a "better" match, since CultureInfo objects can't represent all the
      // different languages the AppX resource model supports. For classic
      // resources, this causes us to ignore the languages list and instead use
      // the older Win32 behavior, which is the design choice we've made. (See
      // the call a little later to GetCurrentUICultureNoAppX()).
    ...
    if (_bUsingModernResourceManagement) {
      if (_PRIonAppXInitialized == false) {
        // Always throw if we did not fully succeed in initializing the WinRT
        // Resource Manager.

        if (_PRIExceptionInfo != null &&
      ...
      // Throws WinRT hresults.
      return GetStringFromPRI(name, culture == null ? null
                              : culture.Name, _neutralResourcesCulture.Name);
    } else
#endif // FEATURE_APPX
    {
      if (null == culture) {
        // When running inside AppX we want to ignore the languages list when
        // trying to come up with our CurrentUICulture. This line behaves the
        // same way as CultureInfo.CurrentUICulture would have in .NET 4
      ...
      if (FrameworkEventSource.IsInitialized) {
        FrameworkEventSource.Log.ResourceManagerLookupStarted(
            BaseNameField, MainAssembly, culture.Name);
      }
#endif
      ...
      // lookup logic.
      ResourceFallbackManager mgr =
          new ResourceFallbackManager(culture, _neutralResourcesCulture, true);
      foreach(CultureInfo currentCultureInfo in mgr) {
  ...
  private Object GetObject(String name, CultureInfo culture,
    ...
    if (s_IsAppXModel) {
      // If the caller explictily passed in a culture that was obtained by
      // calling CultureInfo.CurrentUICulture, null it out, so that we
      // re-compute it based on the Win32 value and not the AppX language list
      // value. (See the call a little later to GetCurrentUICultureNoAppX()).
      if (Object.ReferenceEquals(culture, CultureInfo.CurrentUICulture)) {
        culture = null;
      }
    }
    ...
    if (null == culture) {
      // When running inside AppX we want to ignore the languages list when
      // trying to come up with our CurrentUICulture. This line behaves the same
      // way as CultureInfo.CurrentUICulture would have in .NET 4
      culture = Thread.CurrentThread.GetCurrentUICultureNoAppX();
    }
    ...
    foreach(CultureInfo currentCultureInfo in mgr) {
      // Note: Technically this method should be passed in a stack crawl mark
      // that we then pass to InternalGetResourceSet for ensuring we demand
      // permissions to read your private resources if you're reading resources
      // from an assembly other than yourself.  But, we must call our three
      // argument overload (without the stack crawl mark) for compatibility.
      // After consideration, we aren't worried about the security impact.
      ResourceSet rs = InternalGetResourceSet(currentCultureInfo, true, true);
      if (rs == null)
        break;
  ...
  GetStream(String name, CultureInfo culture) {
    ...
    if (ums == null && obj != null)
      throw new InvalidOperationException(Environment.GetResourceString(
          "InvalidOperation_ResourceNotStream_Name", name));
    return ums;
  }
  ...
  [System.Security.SecurityCritical] // auto-generated
      private bool
      TryLookingForSatellite(CultureInfo lookForCulture) {
    ...
    String[] installedSatellites =
        (String[]) _installedSatelliteInfo[MainAssembly.FullName];
  ...
  [System.Security.SecurityCritical] // auto-generated
    ...
    foreach(ConfigNode assemblyNode in node.Children) {
      if (!String.Equals(assemblyNode.Name, "assembly"))
        throw new ApplicationException(Environment.GetResourceString(
            "XMLSyntax_InvalidSyntaxSatAssemTag", Path.GetFileName(fileName),
            assemblyNode.Name));
      ...
      if (!String.Equals(de.Key, "name") ||
          String.IsNullOrEmpty(assemblyName) ||
          assemblyNode.Attributes.Count > 1)
        throw new ApplicationException(Environment.GetResourceString(
            "XMLSyntax_InvalidSyntaxSatAssemTagBadAttr",
            Path.GetFileName(fileName), de.Key, de.Value));

      ArrayList list = new ArrayList(5);
      foreach(ConfigNode child in assemblyNode.Children) if (child.Value !=
                                                             null)
      ...
      for (int i = 0; i < satellites.Length; i++) {
        ...
        if (FrameworkEventSource.IsInitialized) {
          FrameworkEventSource.Log.ResourceManagerAddingCultureFromConfigFile(
              BaseNameField, MainAssembly, cultureName);
        }
#endif
      }
  ...
  internal class ResourceManagerMediator {
    ...
    // this is weird because we have BaseNameField accessor above, but we're
    // sticking with it for compat.
