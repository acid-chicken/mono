...
namespace System.Globalization {
  ...
  // This class has only static members and therefore doesn't need to be
  // serialized.
  ...
  public static class CharUnicodeInfo {
    ...
    //
    // Native methods to access the Unicode category data tables in
    // charinfo.nlp.
    //
...
#pragma warning restore
    ...
    // The native pointer to the 12:4:4 index table of the Unicode cateogry
    // data.
    [SecurityCritical] unsafe static ushort *s_pCategoryLevel1Index;
    [SecurityCritical] unsafe static byte *s_pCategoriesValue;

    // The native pointer to the 12:4:4 index table of the Unicode numeric data.
    // The value of this index table is an index into the real value table
    // stored in s_pNumericValues.
    [SecurityCritical] unsafe static ushort *s_pNumericLevel1Index;
    ...
    // To get around the IA64 alignment issue.  Our double data is aligned in
    // 8-byte boundary, but loader loads the embeded table starting at 4-byte
    // boundary.  This cause a alignment issue since double is 8-byte.
    [SecurityCritical] unsafe static byte *s_pNumericValues;

    // The digit value table, which is indexed by s_pNumericLevel1Index.  It
    // shares the same indice as s_pNumericValues. Every item contains the value
    // for decimal digit/digit value.
    [SecurityCritical] unsafe static DigitValues *s_pDigitValues;

    internal const String UNICODE_INFO_FILE_NAME = "charinfo.nlp";
    // The starting codepoint for Unicode plane 1.  Plane 1 contains 0x010000 ~
    // 0x01ffff.
    internal const int UNICODE_PLANE01_START = 0x10000;

    //
    // This is the header for the native data table that we load from
    // UNICODE_INFO_FILE_NAME.
    //
    // Excplicit layout is used here since a syntax like char[16] can not be
    // used in sequential layout.
    ...
    // NOTE: It's important to specify pack size here, since the size of the
    // structure is 2 bytes.  Otherwise, the default pack size will be 4.
    ...
    // We need to allocate the underlying table that provides us with the
    // information that we use.  We allocate this once in the class initializer
    // and then we don't need to worry about it again.
    //
    [System.Security.SecuritySafeCritical] // auto-generated
        [ResourceExposure(ResourceScope.None)]
        [ResourceConsumption(ResourceScope.Process,
                             ResourceScope.Process)] unsafe static bool
        InitTable() {
      ...
      byte *pDataTable = GlobalizationAssembly.GetGlobalizationResourceBytePtr(
          typeof (CharUnicodeInfo).Assembly, UNICODE_INFO_FILE_NAME);
      ...
      s_pNumericValues =
          (byte *)(pDataTable + EndianSwap(mainHeader->OffsetToNumbericValue));
    ...
    // it does not throw exceptions when invalid surrogate characters are passed
    // in.
    //
    // WARNING: since it doesn't throw an exception it CAN return a value
    //          in the surrogate range D800-DFFF, which are not legal unicode
    //          values.
    //
    ////////////////////////////////////////////////////////////////////////
    ...
    internal static int InternalConvertToUtf32(String s, int index,
      ...
      Contract.Assert(index >= 0 && index < s.Length,
    ...
    internal static bool IsWhiteSpace(String s, int index) {
      ...
      // In Unicode 3.0, U+2028 is the only character which is under the
      // category "LineSeparator". And U+2029 is th eonly character which is
      // under the category "ParagraphSeparator".
      switch (uc) {
    ...
    //
    // Note that for ch in the range D800-DFFF we just treat it as any other
    // non-numeric character
    //
    [System.Security.SecuritySafeCritical] // auto-generated
        internal unsafe static double
        InternalGetNumericValue(int ch) {
      Contract.Assert(ch >= 0 && ch <= 0x10ffff,
                      "ch is not in valid Unicode range.");
      // Get the level 2 item from the highest 12 bit (8 - 19) of ch.
      ushort index = EndianSwap(s_pNumericLevel1Index[ch >> 8]);
      // Get the level 2 WORD offset from the 4 - 7 bit of ch.  This provides
      // the base offset of the level 3 table. The offset is referred to an
      // float item in m_pNumericFloatData. Note that & has the lower precedence
      // than addition, so don't forget the parathesis.
      ...
      // boundary.  This cause a alignment issue since double is 8-byte.
      byte *pSourcePtr =
          &(s_pNumericValues[pBytePtr[(ch & 0x000f)] * sizeof(double)]);
    ...
    //
    [System.Security.SecuritySafeCritical] // auto-generated
        internal unsafe static DigitValues *
        InternalGetDigitValues(int ch) {
      Contract.Assert(ch >= 0 && ch <= 0x10ffff,
                      "ch is not in valid Unicode range.");
      // Get the level 2 item from the highest 12 bit (8 - 19) of ch.
    ...
    [System.Security.SecuritySafeCritical] // auto-generated
        internal unsafe static sbyte
        InternalGetDecimalDigitValue(int ch) {
      return (InternalGetDigitValues(ch)->decimalDigit);
    }

    [System.Security.SecuritySafeCritical] // auto-generated
        internal unsafe static sbyte
        InternalGetDigitValue(int ch) {
      return (InternalGetDigitValues(ch)->digit);
    }

    ////////////////////////////////////////////////////////////////////////
    //
    // Returns the numeric value associated with the character c. If the
    // character is a fraction,
    // the return value will not be an integer. If the character does not have a
    // numeric value, the return value is -1.
    //
    // Returns:
    //  the numeric value for the specified Unicode character.  If the character
    //  does not have a numeric value, the return value is -1.
    // Arguments:
    ...
    public static double GetNumericValue(String s, int index) {
      ...
      if (index < 0 || index >= s.Length) {
        throw new ArgumentOutOfRangeException(
            "index", Environment.GetResourceString("ArgumentOutOfRange_Index"));
      }
    ...
    // From Unicode.org: Decimal Digits. Digits that can be used to form
    // decimal-radix numbers.
    // Returns:
    //  the decimal digit value for the specified Unicode character.  If the
    //  character does not have a decimal digit value, the return value is -1.
    // Arguments:
    //      ch  a Unicode character
    // Exceptions:
    //      ArgumentNullException
    ...
    // If the character does not have a numeric value, the return value is -1.
    // From Unicode.org: If the character represents a digit, not necessarily a
    // decimal digit, the value is here. This covers digits which do not form
    // decimal radix forms, such as the compatibility superscript digits.
    //
    // An example is: U+2460 IRCLED DIGIT ONE. This character has digit value 1,
    // but does not have associcated decimal digit value.
    //
    // Returns:
    //  the digit value for the specified Unicode character.  If the character
    //  does not have a digit value, the return value is -1.
    // Arguments:
    //      ch  a Unicode character
    // Exceptions:
    //      ArgumentNullException
    ...
    GetUnicodeCategory(int codePoint) => GetUnicodeCategory((char) codePoint);
#endif
    ...
    [System.Security.SecuritySafeCritical] // auto-generated
        ...
        InternalGetCategoryValue(int ch, int offset) {
      ...
      // Get the level 2 WORD offset from the 4 - 7 bit of ch.  This provides
      // the base offset of the level 3 table. Note that & has the lower
      // precedence than addition, so don't forget the parathesis.
      ...
      //
      // Make sure that OtherNotAssigned is the last category in
      // UnicodeCategory. If that changes, change the following assertion as
      // well.
      //
      // Contract.Assert(uc >= 0 && uc <= UnicodeCategory.OtherNotAssigned,
      // "Table returns incorrect Unicode category");
      return (uc);
    }

    //      internal static BidiCategory GetBidiCategory(char ch) {
    //          return ((BidiCategory)InternalGetCategoryValue(c,
    ...
    internal static BidiCategory GetBidiCategory(String s, int index) {
      ...
      Contract.EndContractBlock();
      return ((BidiCategory) InternalGetCategoryValue(
          InternalConvertToUtf32(s, index), BIDI_CATEGORY_OFFSET));
    }
    ...
    internal static UnicodeCategory InternalGetUnicodeCategory(String value,
                                                               int index) {
    ...
    ////////////////////////////////////////////////////////////////////////
    //
    // Get the Unicode category of the character starting at index.  If the
    // character is in BMP, charLength will return 1. If the character is a
    // valid surrogate pair, charLength will return 2.
    //
    ////////////////////////////////////////////////////////////////////////
    ...
    InternalGetUnicodeCategory(String str, int index, out int charLength) {
      ...
      ;
      Contract.Assert(index >= 0 && index < str.Length,
                      "index >= 0 && index < str.Length");

      return (InternalGetUnicodeCategory(
