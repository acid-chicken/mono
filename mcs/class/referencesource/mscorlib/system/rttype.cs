...
namespace System {
// this is a work around to get the concept of a calli. It's not as fast but it
// would be interesting to see how it compares to the current implementation.
...
internal enum TypeNameFormatFlags {
  ...
  FormatNamespace = 0x00000001, // Include namespace and/or enclosing class
                                // names in type names
  FormatFullInst = 0x00000002,  // Include namespace and assembly in generic
                                // types (regardless of other flag settings)
  ...
  FormatNoVersion = 0x00000010, // Suppress version and culture information in
                                // all assembly names
#if _DEBUG
  ...
  // If we want to be able to distinguish between overloads whose parameter
  // types have the same name but come from different assemblies, we can add
  // FormatAssembly | FormatNoVersion to FormatSerialization. But we are
  // omitting it because it is not a useful scenario and including the assembly
  // name will normally increase the size of the serialized data and also
...
[Serializable]
internal partial class RuntimeType : System.Reflection.TypeInfo,
...
#region Definitions
  ...
  // Helper to build lists of MemberInfos. Special cased to avoid allocations
  // for lists of one element.
...
#region Definitions
    ...
    private struct Filter {
      ...
      [System.Security.SecurityCritical] // auto-generated
          public unsafe
          Filter(byte *pUtf8Name, int cUtf8Name, MemberListType listType) {
      ...
      public bool Match(Utf8String name) {
        ...
        // Currently the callers of UsesStringComparison assume that if it
        // returns false then the match always succeeds and can be skipped.
        // Assert that this is maintained.
      ...
      // If not, we know Match will always return true and the call can be
      // skipped If so, we know we can have a valid hash to check against from
      // GetHashToMatch
...
#region Private Data Members
      ...
      // List of MemberInfos given out. When m_cacheComplete is false, it may
      // have null entries at the end to avoid reallocating the list every time
...
#region Constructor
      ...
      private void
      InitializeAndUpdateAmbiguousPropertiesList(RuntimePropertyInfo parent,
        ...
        lock(m_ambiguousProperties) {
          ...
          Contract.Assert(
              child.DeclaringType.IsSubclassOf(parent.DeclaringType));
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
          ...
          AddMethod(RuntimeType declaringType,
                    RuntimeMethodHandleInternal method, CacheType cacheType) {
        T[] list = null;
        MethodAttributes methodAttributes =
            RuntimeMethodHandle.GetAttributes(method);
        bool isPublic =
        ...
        BindingFlags bindingFlags =
            RuntimeType.FilterPreCalculate(isPublic, isInherited, isStatic);
        switch (cacheType) {
        case CacheType.Method:
          list = (T[])(object) new RuntimeMethodInfo[1]{
              new RuntimeMethodInfo(method, declaringType, m_runtimeTypeCache,
                                    methodAttributes, bindingFlags, null)};
          break;
        case CacheType.Constructor:
          list = (T[])(object) new RuntimeConstructorInfo[1]{
              new RuntimeConstructorInfo(method, declaringType,
                                         m_runtimeTypeCache, methodAttributes,
                                         bindingFlags)};
          break;
        }
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
        ...
        FieldAttributes fieldAttributes =
            RuntimeFieldHandle.GetAttributes(field);
        bool isPublic = (fieldAttributes & FieldAttributes.FieldAccessMask) ==
        ...
        T[] list = (T[])(object) new RuntimeFieldInfo[1]{new RtFieldInfo(
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
          private unsafe T[] Populate(string name, MemberListType listType,
                                      CacheType cacheType) {
        T[] list = null;

        if (name == null || name.Length == 0 ||
            (cacheType == CacheType.Constructor && name.FirstChar != '.' &&
             name.FirstChar != '*')) {
          ...
          fixed(char *pName = name) {
            ...
            // no one should be looking for a member whose name is longer than
            // 1024
            if (cUtf8Name > MAXNAMELEN) {
              fixed(byte *pUtf8Name = new byte[cUtf8Name]) {
                list = GetListByName(pName, cNameLen, pUtf8Name, cUtf8Name,
                                     listType, cacheType);
              }
            } else {
      ...
      [System.Security.SecurityCritical] // auto-generated
          private unsafe T[] GetListByName(char *pName, int cNameLen,
                                           byte *pUtf8Name, int cUtf8Name,
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
          ...
          internal void
          Insert(ref T[] list, string name, MemberListType listType) {
...
// We want the behavior where the results are returned in the same order on the
// phone
#if !FEATURE_LEGACYNETCF
            else
#endif
        ...
        for (int i = 0; i < list.Length; i++) {
          ...
          if (!foundInCache) {
            ...
            if (freeSlotIndex >= cachedMembers.Length) {
              ...
              if (m_cacheComplete) {
                //
                // In theory, we should never add more elements to the cache
                // when it is complete.
                //
                // Unfortunately, we shipped with bugs that cause changes of the
                // complete cache (DevDiv #339308). Grow the list by exactly one
                // element in this case to avoid null entries at the end.
                //

                // DevDiv #339308 is fixed, but we are keeping this code here
                // for Dev11 in case there are other instances of this bug.
                // Remove for Dev12.
                ...
                newSize = Math.Max(Math.Max(4, 2 * cachedMembers.Length),
                                   list.Length);
              }

              // Use different variable for ref argument to Array.Resize to
              // allow enregistration of cachedMembers by the JIT
...
#region Population Logic
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
        ...
        ListBuilder<RuntimeMethodInfo>list =
            new ListBuilder<RuntimeMethodInfo>();
...
#region IsInterface
          ...
          foreach(RuntimeMethodHandleInternal methodHandle in
                      RuntimeTypeHandle.GetIntroducedMethods(declaringType)) {
            if (filter.RequiresStringComparison()) {
              if (!RuntimeMethodHandle.MatchesNameHash(
                      methodHandle, filter.GetHashToMatch())) {
...
#region Loop through all methods on the interface
            ...
            // Except for .ctor, .cctor, IL_STUB*, and static methods, all
            // interface methods should be abstract, virtual, and
            // non-RTSpecialName. Note that this assumption will become invalid
            // when we add support for non-abstract or static methods on
            // interfaces.
            Contract.Assert(
                (RuntimeMethodHandle.GetAttributes(methodHandle) &
                 (MethodAttributes.RTSpecialName | MethodAttributes.Abstract |
...
#region Calculate Binding Flags
            ...
            RuntimeMethodHandleInternal instantiatedHandle =
                RuntimeMethodHandle.GetStubIfNeeded(methodHandle, declaringType,
                                                    null);

            RuntimeMethodInfo runtimeMethodInfo = new RuntimeMethodInfo(
                instantiatedHandle, declaringType, m_runtimeTypeCache,
                methodAttributes, bindingFlags, null);
...
#region Continue if this is a private declared on a base type
              ...
              if ((methodAttributes & MethodAttributes.Virtual) != 0) {
                // only virtual if actually in the vtableslot range, but GetSlot
                // will assert if an EnC method, which can't be virtual, so
                // narrow down first before calling GetSlot
              ...
              // NetCF actually includes private methods from parent classes in
              // Reflection results We will mimic that in Mango Compat mode.
...
#region Continue if this is a virtual and is already overridden
              if (isVirtual) {
                ...
                if ((methodAttributes & (MethodAttributes.Virtual |
...
#region Calculate Binding Flags
              ...
              BindingFlags bindingFlags = RuntimeType.FilterPreCalculate(
                  isPublic, isInherited, isStatic);
#endregion

              // get the unboxing stub or instantiating stub if needed
              RuntimeMethodHandleInternal instantiatedHandle =
                  RuntimeMethodHandle.GetStubIfNeeded(methodHandle,
                                                      declaringType, null);
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
        ...
        foreach(RuntimeMethodHandleInternal methodHandle in
                    RuntimeTypeHandle.GetIntroducedMethods(declaringType)) {
          if (filter.RequiresStringComparison()) {
            if (!RuntimeMethodHandle.MatchesNameHash(methodHandle,
          ...
          Contract.Assert((methodAttributes & MethodAttributes.Abstract) == 0 &&
                          (methodAttributes & MethodAttributes.Virtual) == 0);
...
#region Calculate Binding Flags
          ...
          RuntimeConstructorInfo runtimeConstructorInfo =
              new RuntimeConstructorInfo(instantiatedHandle, ReflectedType,
                                         m_runtimeTypeCache, methodAttributes,
                                         bindingFlags);
...
#region Populate Literal Fields on Interfaces
        if (ReflectedType.IsGenericParameter) {
          ...
          for (int i = 0; i < interfaces.Length; i++) {
            // Populate literal fields defined on any of the interfaces
            // implemented by the declaring type
            PopulateLiteralFields(filter, (RuntimeType) interfaces[i],
                                  ref list);
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
          private unsafe void
          PopulateRtFields(Filter filter, RuntimeType declaringType,
        ...
        if (!RuntimeTypeHandle.GetFields(declaringType, pResult, &count)) {
          fixed(IntPtr *pBigResult = new IntPtr[count]) {
            ...
            PopulateRtFields(filter, pBigResult, count, declaringType,
                             ref list);
          }
        } else if (count > 0) {
      ...
      [System.Security.SecurityCritical] // auto-generated
          private unsafe void
          PopulateRtFields(Filter filter, IntPtr *ppFieldHandles, int count,
        ...
        for (int i = 0; i < count; i++) {
          ...
          if (filter.RequiresStringComparison()) {
            if (!RuntimeFieldHandle.MatchesNameHash(runtimeFieldHandle,
                                                    filter.GetHashToMatch())) {
              Contract.Assert(!filter.Match(
...
#region Calculate Binding Flags
          ...
          if (needsStaticFieldForGeneric && isStatic)
            runtimeFieldHandle =
                RuntimeFieldHandle.GetStaticFieldForGenericType(
                    runtimeFieldHandle, declaringType);

          RuntimeFieldInfo runtimeFieldInfo =
              new RtFieldInfo(runtimeFieldHandle, declaringType,
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
          private unsafe void
          PopulateLiteralFields(Filter filter, RuntimeType declaringType,
                                ref ListBuilder<RuntimeFieldInfo>list) {
        Contract.Requires(declaringType != null);
        Contract.Requires(ReflectedType != null);
        ...
        MetadataImport scope =
            RuntimeTypeHandle.GetMetadataImport(declaringType);
...
#region Calculate Binding Flags
            ...
            RuntimeFieldInfo runtimeFieldInfo = new MdFieldInfo(
                tkField, fieldAttributes, declaringType.GetTypeHandleInternal(),
                m_runtimeTypeCache, bindingFlags);
      ...
      private void AddSpecialInterface(ref ListBuilder<RuntimeType>list,
                                       Filter filter, RuntimeType iList,
        ...
        if (!RuntimeTypeHandle.IsGenericVariable(declaringType)) {
          ...
          if (ReflectedType.IsSzArray) {
            RuntimeType arrayType =
                (RuntimeType) ReflectedType.GetElementType();
            ...
            if (!arrayType.IsPointer) {
              ...
              // To avoid adding a duplicate IEnumerable<T>, we don't add the
              // sub interfaces of IReadOnlyList. Instead, we add
              // IReadOnlyCollection<T> separately.
              AddSpecialInterface(ref list, filter,
                                  (RuntimeType) typeof (IReadOnlyList<>)
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
        ...
        for (int i = 0; i < tkNestedClasses.Length; i++) {
          ...
          try {
            ...
            // In a reflection emit scenario, we may have a token for a class
            // which has not been baked and hence cannot be loaded.
            continue;
          }
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
        ...
        // Do not create the dictionary if we are filtering the properties by
        // name already
        Dictionary<String, RuntimeEventInfo>csEventInfos =
            filter.CaseSensitive() ? null
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
          private unsafe void
          PopulateEvents(Filter filter, RuntimeType declaringType,
                         Dictionary<String, RuntimeEventInfo>csEventInfos,
                         ref ListBuilder<RuntimeEventInfo>list) {
        int tkDeclaringType = RuntimeTypeHandle.GetToken(declaringType);

        // Arrays, Pointers, ByRef types and others generated only the fly by
        // the RT do not have tokens.
        if (MdToken.IsNullToken(tkDeclaringType))
          return;
...
#region Remove Duplicates
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
        ...
        // m_csMemberInfos can be null at this point. It will be initialized
        // when Insert is called in Populate after this returns.

        RuntimeType declaringType = ReflectedType;
        Contract.Assert(declaringType != null);

        ListBuilder<RuntimePropertyInfo>list =
            new ListBuilder<RuntimePropertyInfo>();
        ...
        if (!RuntimeTypeHandle.IsInterface(declaringType)) {
          ...
          bool[] usedSlots =
              new bool[RuntimeTypeHandle.GetNumVirtuals(declaringType)];

          // Populate associates off of the class hierarchy
          do {
            PopulateProperties(filter, declaringType, csPropertyInfos,
                               usedSlots, ref list);
            declaringType = RuntimeTypeHandle.GetBaseType(declaringType);
          } while (declaringType != null);
        } else {
      ...
      [System.Security.SecuritySafeCritical] // auto-generated
          private unsafe void
          PopulateProperties(
              Filter filter, RuntimeType declaringType,
              Dictionary<String, List<RuntimePropertyInfo>>csPropertyInfos,
              bool[] usedSlots, ref ListBuilder<RuntimePropertyInfo>list) {
        int tkDeclaringType = RuntimeTypeHandle.GetToken(declaringType);
        ...
        Contract.Assert((declaringType.IsInterface && usedSlots == null &&
                         csPropertyInfos == null) ||
                        (!declaringType.IsInterface && usedSlots != null &&
        ...
        for (int i = 0; i < tkProperties.Length; i++) {
          ...
          if (filter.RequiresStringComparison()) {
            if (!ModuleHandle.ContainsPropertyMatchingHash(
                    declaringModuleHandle, tkProperty,
                    filter.GetHashToMatch())) {
              Contract.Assert(!filter.Match(
                  declaringType.GetRuntimeModule().MetadataImport.GetName(
                      tkProperty)));
              continue;
            }

            Utf8String name;
            name = declaringType.GetRuntimeModule().MetadataImport.GetName(
                tkProperty);
...
#region Duplicate check based on vtable slots
            ...
            // The inheritance of properties are defined by the inheritance of
            // their getters and setters. A property on a base type is
            // "overriden" by a property on a sub type if the getter/setter of
            // the latter occupies the same vtable slot as the getter/setter of
            // the former.

            MethodInfo associateMethod = propertyInfo.GetGetMethod();
            if (associateMethod == null) {
              // We only need to examine the setter if a getter doesn't exist.
              // It is not logical for the getter to be virtual but not the
              // setter.
...
#region Duplicate check based on name and signature
            ...
            // For backward compatibility, even if the vtable slots don't match,
            // we will still treat a property as duplicate if the names and
            // signatures match.
            ...
            if (csPropertyInfos != null) {
              ...
              List<RuntimePropertyInfo>cache =
                  csPropertyInfos.GetValueOrDefault(name);
              ...
              for (int j = 0; j < cache.Count; j++) {
                if (propertyInfo.EqualsSig(cache[j])) {
                  ...
                  if (CompatibilitySwitches.IsAppEarlierThanWindowsPhone8 &&
                    ...
                    InitializeAndUpdateAmbiguousPropertiesList(propertyInfo,
                                                               cache[j]);
                  } else
#endif
                  {
...
#region NonPrivate Members
      internal T[] GetMemberList(MemberListType listType, string name,
                                 CacheType cacheType) {
        T[] list = null;
...
#region Private Data Members
    ...
    private static CerHashtable<RuntimeMethodInfo, RuntimeMethodInfo>
    ...
    private Object
        m_genericCache; // Generic cache for rare scenario specific data. It is
                        // used to cache Enum names and values.
#endregion
...
#region Private Members
    private string ConstructName(ref string name,
    ...
    private T[] GetMemberList<T>(ref MemberInfoCache<T>m_cache,
                                 MemberListType listType, string name,
                                 CacheType cacheType) where T : MemberInfo {
    ...
    private T[] GetMemberList<T>(
        ref MemberInfoCache<T>m_cache, MemberListType listType, string name,
        CacheType cacheType,
        out IReadOnlyList<RuntimePropertyInfo>ambiguousProperties) where T
        : MemberInfo {
      ...
      Contract.Assert(ambiguousProperties == null ||
                      CompatibilitySwitches.IsAppEarlierThanWindowsPhone8);
      return results;
    }
#endif

    private MemberInfoCache<T>
    GetMemberCache<T>(ref MemberInfoCache<T>m_cache) where T : MemberInfo {
      MemberInfoCache<T>existingCache = m_cache;

      if (existingCache == null) {
        MemberInfoCache<T>newCache = new MemberInfoCache<T>(this);
        existingCache =
            Interlocked.CompareExchange(ref m_cache, newCache, null);
        if (existingCache == null)
...
#region Internal Members
    ...
    internal string GetName(TypeNameKind kind) {
      ...
      case TypeNameKind.FullName:
        // We exclude the types that contain generic parameters because their
        // names cannot be roundtripped. We allow generic type definitions (and
        // their refs, ptrs, and arrays) because their names can be roundtriped.
        // Theoretically generic types instantiated with generic type
        // definitions can be roundtripped, e.g. List`1<Dictionary`2>. But these
        // kind of types are useless, rare, and hard to identity. We would need
        // to recursively examine all the generic arguments with the same
        // criteria. We will exclude them unless we see a real user scenario.
        if (!m_runtimeType.GetRootElementType().IsGenericTypeDefinition &&
            m_runtimeType.ContainsGenericParameters)
          return null;

        // No assembly.
        return ConstructName(ref m_fullname,
      ...
      case TypeNameKind.ToString:
        ...
        return ConstructName(ref m_toString,
                             TypeNameFormatFlags.FormatNamespace);

      case TypeNameKind.SerializationName:
        // Use FormatGenericParam in serialization. Otherwise we won't be able
        // to distinguish between a generic parameter and a normal type with the
        // same name. e.g. Foo<T>.Bar(T t), the parameter type T could be !1 or
        // a real type named "T". Excluding the version number in the assembly
        // name for VTS.
        return ConstructName(ref m_serializationname,
    ...
    internal TypeCode TypeCode{get { return m_typeCode;
  }
  set { m_typeCode = value; }
}

[System.Security.SecuritySafeCritical] // auto-generated
    internal unsafe RuntimeType
    GetEnclosingType() {
  if (m_enclosingType == null) {
    // Use void as a marker of null enclosing type
    RuntimeType enclosingType =
        RuntimeTypeHandle.GetDeclaringType(GetRuntimeType());
...
internal RuntimeType GetRuntimeType() { return m_runtimeType; }
...
internal RuntimeConstructorInfo GetSerializationCtor() {
  if (m_serializationCtor == null) {
    ...
    m_serializationCtor = m_runtimeType.GetConstructor(
        ...
        null, CallingConventions.Any, s_SICtorParamTypes, null)
  ...
  if (m_defaultMemberName == null) {
    ...
    for (RuntimeType t = m_runtimeType; t != null; t = t.GetBaseType()) {
      ...
      for (int i = 0; i < attrs.Count; i++) {
        if (Object.ReferenceEquals(attrs[i].Constructor.DeclaringType,
...
[System.Security.SecurityCritical] // auto-generated
  ...
  if (s_methodInstantiationsLock == null)
    Interlocked.CompareExchange(ref s_methodInstantiationsLock, new Object(),
                                null);
...
internal RuntimeMethodInfo[] GetMethodList(MemberListType listType,
                                           string name) {
  return GetMemberList<RuntimeMethodInfo>(ref m_methodInfoCache, listType, name,
                                          CacheType.Method);
}

internal RuntimeConstructorInfo[] GetConstructorList(MemberListType listType,
                                                     string name) {
  return GetMemberList<RuntimeConstructorInfo>(
      ref m_constructorInfoCache, listType, name, CacheType.Constructor);
}

internal RuntimePropertyInfo[] GetPropertyList(MemberListType listType,
                                               string name) {
  return GetMemberList<RuntimePropertyInfo>(ref m_propertyInfoCache, listType,
                                            name, CacheType.Property);
}
...
internal RuntimeEventInfo[] GetEventList(MemberListType listType, string name) {
  return GetMemberList<RuntimeEventInfo>(ref m_eventInfoCache, listType, name,
                                         CacheType.Event);
}

internal RuntimeFieldInfo[] GetFieldList(MemberListType listType, string name) {
  return GetMemberList<RuntimeFieldInfo>(ref m_fieldInfoCache, listType, name,
                                         CacheType.Field);
}

internal RuntimeType[] GetInterfaceList(MemberListType listType, string name) {
  return GetMemberList<RuntimeType>(ref m_interfaceCache, listType, name,
                                    CacheType.Interface);
}

internal RuntimeType[] GetNestedTypeList(MemberListType listType, string name) {
  return GetMemberList<RuntimeType>(ref m_nestedClassesCache, listType, name,
...
internal MethodBase GetConstructor(RuntimeType declaringType,
  ...
  return m_constructorInfoCache.AddMethod(declaringType, constructor,
...
// The size of CachedData is accounted for by BaseObjectWithCachedData in
// object.h. This member is currently being used by Remoting for caching
// remoting data. If you need to cache data here, talk to the Remoting team to
// work out a mechanism, so that both caching systems can happily work together.
...
internal RemotingTypeCachedData RemotingCache {
  get {
    ...
    if (cache == null) {
      ...
      RemotingTypeCachedData ret =
          Interlocked.CompareExchange(ref m_cachedData, cache, null);
      if (ret != null)
        cache = ret;
    }
    return cache;
...
internal static RuntimeType GetType(String typeName, bool throwOnError,
  ...
  return RuntimeTypeHandle.GetTypeByName(typeName, throwOnError, ignoreCase,
                                         reflectionOnly, ref stackMark, false);
}

#if !MONO
internal static MethodBase GetMethodBase(RuntimeModule scope,
...
GetMethodBase(RuntimeType reflectedType, IRuntimeMethodInfo methodHandle) {
  MethodBase retval =
      RuntimeType.GetMethodBase(reflectedType, methodHandle.Value);
  GC.KeepAlive(methodHandle);
  return retval;
}

[System.Security.SecurityCritical] // auto-generated
    internal unsafe static MethodBase
  ...
  if (reflectedType != declaredType &&
      !reflectedType.IsSubclassOf(declaredType)) {
    // object[] is assignable from string[].
    if (reflectedType.IsArray) {
      //

      // The whole purpose of this chunk of code is not only for error checking.
      // GetMember has a side effect of populating the member cache of
      // reflectedType, doing so will ensure we construct the correct
      // MethodInfo/ConstructorInfo objects. Without this the
      // reflectedType.Cache.GetMethod call below may return a MethodInfo object
      // whose ReflectedType is string[] and DeclaringType is object[]. That
      // would be (arguabally) incorrect because string[] is not a subclass of
      // object[].
      MethodBase[] methodBases =
          reflectedType.GetMember(RuntimeMethodHandle.GetName(methodHandle),
                                  MemberTypes.Constructor | MemberTypes.Method,
                                  BindingFlags.Public | BindingFlags.NonPublic |
                                      BindingFlags.Instance) as MethodBase[];
    ...
    // Action<in string> is assignable from, but not a subclass of Action<in
    // object>.
    else if (declaredType.IsGenericType) {
      // ignoring instantiation is the ReflectedType a subtype of the
      // DeclaringType
      ...
      while (baseType != null) {
        ...
        if (baseDefinition.IsGenericType && !baseType.IsGenericTypeDefinition)
          baseDefinition =
              (RuntimeType) baseDefinition.GetGenericTypeDefinition();
      ...
      if (baseType == null) {
        // ignoring instantiation is the ReflectedType is not a subtype of the
        // DeclaringType
        throw new ArgumentException(String.Format(
            CultureInfo.CurrentCulture,
            Environment.GetResourceString("Argument_ResolveMethodHandle"),
      ...
      // if the original methodHandle was the definition then we don't need to
      // rebind generic method arguments because all RuntimeMethodHandles
      // retrieved off of the canonical method table are definitions. That's why
      // for everything else we need to rebind the generic method arguments.
      ...
      // type
      methodHandle = RuntimeMethodHandle.GetMethodFromCanonical(methodHandle,
                                                                declaredType);
    } else if (!declaredType.IsAssignableFrom(reflectedType)) {
      // declaredType is not Array, not generic, and not assignable from
      // reflectedType
      throw new ArgumentException(String.Format(
          CultureInfo.CurrentCulture,
          Environment.GetResourceString("Argument_ResolveMethodHandle"),
  ...
  // If methodInstantiation is not null, GetStubIfNeeded will rebind the generic
  // method arguments if declaredType is an instantiated generic type and
  // methodHandle is not generic, get the instantiated MethodDesc (if needed) if
  // declaredType is a value type, get the unboxing stub (if needed)
  ...
  methodHandle = RuntimeMethodHandle.GetStubIfNeeded(methodHandle, declaredType,
                                                     methodInstantiation);
  MethodBase retval;
  ...
  if (RuntimeMethodHandle.IsConstructor(methodHandle)) {
    ...
    // Method case
    if (RuntimeMethodHandle.HasMethodInstantiation(methodHandle) &&
...
internal bool DomainInitialized{get { return Cache.DomainInitialized;
}
...
[System.Security.SecuritySafeCritical] // auto-generated
  ...
  return GetFieldInfo(RuntimeFieldHandle.GetApproxDeclaringType(fieldHandle),
                      fieldHandle);
}

[System.Security.SecuritySafeCritical] // auto-generated
    internal unsafe static FieldInfo
    GetFieldInfo(RuntimeType reflectedType, IRuntimeFieldInfo field) {
  ...
  if (reflectedType == null) {
    ...
    if (reflectedType != declaredType) {
      if (!RuntimeFieldHandle.AcquiresContextFromThis(fieldHandle) ||
          !RuntimeTypeHandle.CompareCanonicalHandles(declaredType,
                                                     reflectedType)) {
        throw new ArgumentException(String.Format(
            CultureInfo.CurrentCulture,
...
// Called internally
private unsafe static PropertyInfo GetPropertyInfo(RuntimeType reflectedType,
                                                   int tkProperty) {
...
private static void ThrowIfTypeNeverValidGenericArgument(RuntimeType type) {
  if (type.IsPointer || type.IsByRef || type == typeof (void))
    throw new ArgumentException(Environment.GetResourceString(
        "Argument_NeverValidGenericArgument", type.ToString()));
}

internal static void
SanityCheckGenericArguments(RuntimeType[] genericArguments,
  ...
  if (genericArguments.Length != genericParamters.Length)
    throw new ArgumentException(Environment.GetResourceString(
        "Argument_NotEnoughGenArguments", genericArguments.Length,
        genericParamters.Length));
}
#if !MONO
[System.Security.SecuritySafeCritical] // auto-generated
    internal static void
    ValidateGenericArguments(MemberInfo definition,
  ...
  if (definition is Type) {
    ...
    RuntimeType declaringType =
        (RuntimeType) genericMethodDefinition.DeclaringType;
    if (declaringType != null) {
      typeContext =
          declaringType.GetTypeHandleInternal().GetInstantiationInternal();
    }
  }
  ...
  for (int i = 0; i < genericArguments.Length; i++) {
    ...
    if (!RuntimeTypeHandle.SatisfiesConstraints(
...
#region Filters
internal static BindingFlags FilterPreCalculate(bool isPublic, bool isInherited,
                                                bool isStatic) {
  BindingFlags bindingFlags =
      isPublic ? BindingFlags.Public : BindingFlags.NonPublic;

  if (isInherited) {
    // We arrange things so the DeclaredOnly flag means "include inherited
    // members"
...
private static void FilterHelper(BindingFlags bindingFlags, ref string name,
                                 bool allowPrefixLookup, out bool prefixLookup,
                                 out bool ignoreCase,
...
// Used by the singular GetXXX APIs (Event, Field, Interface, NestedType) where
// prefixLookup is not supported.
private static void FilterHelper(BindingFlags bindingFlags, ref string name,
                                 out bool ignoreCase,
                                 out MemberListType listType) {
  bool prefixLookup;
  FilterHelper(bindingFlags, ref name, false, out prefixLookup, out ignoreCase,
               out listType);
}

// Only called by GetXXXCandidates, GetInterfaces, and GetNestedTypes when
// FilterHelper has set "prefixLookup" to true. Most of the plural GetXXX
// methods allow prefix lookups while the singular GetXXX methods mostly do not.
private static bool FilterApplyPrefixLookup(MemberInfo memberInfo, string name,
                                            bool ignoreCase) {
...
// Used by FilterApplyType to perform all the filtering based on name and
// BindingFlags
private static bool FilterApplyBase(MemberInfo memberInfo,
                                    BindingFlags bindingFlags, bool isPublic,
                                    bool isNonProtectedInternal, bool isStatic,
...
#region Filter by Public &Private
  ...
  bool isInherited = !Object.ReferenceEquals(memberInfo.DeclaringType,
...
#region Filter by name wrt prefixLookup and implicitly by case sensitivity
  if (prefixLookup == true) {
    if (!FilterApplyPrefixLookup(memberInfo, name,
                                 (bindingFlags & BindingFlags.IgnoreCase) != 0))
      return false;
  }
#endregion
...
#region Asymmetries
  // @Asymmetry - Internal, inherited, instance, non-protected, non-virtual,
  // non-abstract members returned
  //              iff BindingFlags !DeclaredOnly, Instance and Public are
  //              present except for fields
...
// Used by GetInterface and GetNestedType(s) which don't need parameter type
// filtering.
private static bool FilterApplyType(Type type, BindingFlags bindingFlags,
                                    string name, bool prefixLookup, string ns) {
  ...
  if (!RuntimeType.FilterApplyBase(type, bindingFlags, isPublic,
                                   type.IsNestedAssembly, isStatic, name,
                                   prefixLookup))
    return false;
...
private static bool FilterApplyMethodInfo(RuntimeMethodInfo method,
                                          BindingFlags bindingFlags,
                                          CallingConventions callConv,
                                          Type[] argumentTypes) {
  // Optimization: Pre-Calculate the method binding flags to avoid casting.
  return FilterApplyMethodBase(method, method.BindingFlags, bindingFlags,
                               callConv, argumentTypes);
}

private static bool
FilterApplyConstructorInfo(RuntimeConstructorInfo constructor,
                           BindingFlags bindingFlags,
                           CallingConventions callConv, Type[] argumentTypes) {
  // Optimization: Pre-Calculate the method binding flags to avoid casting.
  return FilterApplyMethodBase(constructor, constructor.BindingFlags,
                               bindingFlags, callConv, argumentTypes);
}

// Used by GetMethodCandidates/GetConstructorCandidates, InvokeMember, and
// CreateInstanceImpl to perform the necessary filtering. Should only be called
// by FilterApplyMethodInfo and FilterApplyConstructorInfo.
private static bool FilterApplyMethodBase(MethodBase methodBase,
...
#region Invoke Member, Get\Set &Create Instance specific case
      // If the number of supplied arguments differs than the number in the
      // signature AND we are not filtering for a dynamic call -- InvokeMethod
      // or CreateInstance -- filter out the method.
      if ((bindingFlags &
           (BindingFlags.InvokeMethod | BindingFlags.CreateInstance |
            BindingFlags.GetProperty | BindingFlags.SetProperty)) == 0)
        return false;

      bool testForParamArray = false;
      bool excessSuppliedArguments =
          argumentTypes.Length > parameterInfos.Length;

      if (excessSuppliedArguments) { // more supplied arguments than parameters,
                                     // additional arguments could be vararg
#region Varargs If
                                     // method is not vararg, additional
                                     // arguments can not be passed as vararg
        if ((methodBase.CallingConvention & CallingConventions.VarArgs) == 0) {
          testForParamArray = true;
        } else {
          // If Binding flags did not include varargs we would have filtered
          // this vararg method. This Invariant established during callConv
          // check.
          Contract.Assert((callConv & CallingConventions.VarArgs) != 0);
        }
#endregion
      } else { // fewer supplied arguments than parameters, missing arguments
               // could be optional
#region OptionalParamBinding
        if ((bindingFlags & BindingFlags.OptionalParamBinding) == 0) {
          ...
          // Thus, iff the first parameterInfo is not optional then this
          // MethodInfo is no longer a canidate.
...
#region Exact Binding
      if ((bindingFlags & BindingFlags.ExactBinding) != 0) {
        // Legacy behavior is to ignore ExactBinding when InvokeMember is
        // specified. Why filter by InvokeMember? If the answer is we leave this
        // to the binder then why not leave all the rest of this  to the binder
        // too? Further, what other semanitc would the binder use for
        // BindingFlags.ExactBinding besides this one? Further, why not include
        // CreateInstance in this if statement? That's just InvokeMethod with a
        // constructor, right?
        if ((bindingFlags & (BindingFlags.InvokeMethod)) == 0) {
          for (int i = 0; i < parameterInfos.Length; i++) {
            // a null argument type implies a null arg which is always a perfect
            // match
#if MONO
            if ((object) argumentTypes[i] != null &&
                !argumentTypes[i].MatchesParameterTypeExactly(
                    parameterInfos[i]))
#else
              if ((object) argumentTypes[i] != null &&
                  !Object.ReferenceEquals(parameterInfos[i].ParameterType,
                                          argumentTypes[i]))
#endif
              return false;
          }
...
#if !MONO
private object
    m_keepalive; // This will be filled with a LoaderAllocator reference when
                 // this RuntimeType represents a collectible type
...
private bool IsSimpleTypeNonW8PFrameworkAPI() {
  ...
  if (rtAssembly.IsFrameworkAssembly()) {
    ...
    if (System.Reflection.MetadataToken.IsNullToken(ctorToken) ||
        !CustomAttribute.IsAttributeDefined(GetRuntimeModule(), MetadataToken,
                                            ctorToken))
      return true;
  }
...
private RuntimeTypeCache Cache {
  [System.Security.SecuritySafeCritical] // auto-generated
    ...
    if (cache == null) {
      ...
      RuntimeTypeCache existingCache = GCHandle.InternalCompareExchange(
          m_cache, cache, null, false) as RuntimeTypeCache;
      if (existingCache != null)
        cache = existingCache;
    }
...
internal bool IsSpecialSerializableType() {
  ...
  do {
    ...
    // have a base type constraint that is Delegate or even a real delegate
    // type. Let's maintain compatibility and return true for them.
...
#if !MONO
private string GetDefaultMemberName() { return Cache.GetDefaultMemberName(); }
...
private ListBuilder<MethodInfo>
GetMethodCandidates(String name, BindingFlags bindingAttr,
                    CallingConventions callConv, Type[] types,
                    int genericParamCount, bool allowPrefixLookup) {
  bool prefixLookup, ignoreCase;
  MemberListType listType;
  RuntimeType.FilterHelper(bindingAttr, ref name, allowPrefixLookup,
                           out prefixLookup, out ignoreCase, out listType);

#if MONO
  RuntimeMethodInfo[] cache =
      GetMethodsByName(name, bindingAttr, listType, this);
#else
...
//
private ListBuilder<ConstructorInfo>
GetConstructorCandidates(string name, BindingFlags bindingAttr,
                         CallingConventions callConv, Type[] types,
  ...
  if ((!prefixLookup && name?.Length == 0) ||
      (!string.IsNullOrEmpty(name) && name != ConstructorInfo.ConstructorName &&
...
#endif
  ListBuilder<ConstructorInfo>candidates =
      new ListBuilder<ConstructorInfo>(cache.Length);
  for (int i = 0; i < cache.Length; i++) {
    RuntimeConstructorInfo constructorInfo = cache[i];
    if (FilterApplyConstructorInfo(constructorInfo, bindingAttr, callConv,
                                   types) &&
        (!prefixLookup || RuntimeType.FilterApplyPrefixLookup(
                              constructorInfo, name, ignoreCase))) {
...
private ListBuilder<PropertyInfo>GetPropertyCandidates(String name,
  ...
  RuntimePropertyInfo[] cache =
      GetPropertiesByName(name, bindingAttr, listType, this);
#else
#if FEATURE_LEGACYNETCF
    // Dev11 466969 quirk
    IReadOnlyList<RuntimePropertyInfo>ambiguousProperties = null;
    RuntimePropertyInfo[] cache =
        Cache.GetPropertyList(listType, name, out ambiguousProperties);
#else
  ...
  ListBuilder<PropertyInfo>candidates =
      new ListBuilder<PropertyInfo>(cache.Length);
  for (int i = 0; i < cache.Length; i++) {
    RuntimePropertyInfo propertyInfo = cache[i];
    if ((bindingAttr & propertyInfo.BindingFlags) ==
            propertyInfo.BindingFlags &&
        (!prefixLookup ||
         RuntimeType.FilterApplyPrefixLookup(propertyInfo, name, ignoreCase)) &&
        (types == null ||
         (propertyInfo.GetIndexParameters().Length == types.Length))) {
  ...
  // Dev11 466969 quirk
  if (CompatibilitySwitches.IsAppEarlierThanWindowsPhone8 &&
      candidates.Count > 1 && ambiguousProperties != null &&
...
private ListBuilder<PropertyInfo>PruneAmbiguousProperties(
  ...
  lock(ambiguousProperties) {
    for (int outerIndex = 0; outerIndex < ambiguousProperties.Count;
         ++outerIndex) {
  ...
  if (countRemoved > 0) {
    newCandidates =
        new ListBuilder<PropertyInfo>(candidates.Count - countRemoved);
...
private ListBuilder<EventInfo>GetEventCandidates(String name,
                                                 BindingFlags bindingAttr,
                                                 bool allowPrefixLookup) {
  bool prefixLookup, ignoreCase;
  MemberListType listType;
  ...
  RuntimeEventInfo[] cache =
      GetEvents_internal(name, bindingAttr, listType, this);
#else
...
private ListBuilder<FieldInfo>GetFieldCandidates(String name,
                                                 BindingFlags bindingAttr,
                                                 bool allowPrefixLookup) {
  bool prefixLookup, ignoreCase;
  MemberListType listType;
  ...
  RuntimeFieldInfo[] cache =
      GetFields_internal(name, bindingAttr, listType, this);
#else
  ...
  for (int i = 0; i < cache.Length; i++) {
    ...
    if ((bindingAttr & fieldInfo.BindingFlags) == fieldInfo.BindingFlags &&
        (!prefixLookup ||
         FilterApplyPrefixLookup(fieldInfo, name, ignoreCase))) {
...
private ListBuilder<Type>GetNestedTypeCandidates(String fullname,
                                                 BindingFlags bindingAttr,
                                                 bool allowPrefixLookup) {
  bool prefixLookup, ignoreCase;
  ...
  for (int i = 0; i < cache.Length; i++) {
    ...
    if (RuntimeType.FilterApplyType(nestedClass, bindingAttr, name,
                                    prefixLookup, ns)) {
...
public override MethodInfo[] GetMethods(BindingFlags bindingAttr) {
  return GetMethodCandidates(null, bindingAttr, CallingConventions.Any, null,
                             -1, false)
      .ToArray();
}
...
public override ConstructorInfo[] GetConstructors(BindingFlags bindingAttr) {
  return GetConstructorCandidates(null, bindingAttr, CallingConventions.Any,
                                  null, false)
      .ToArray();
}
...
[System.Security.SecuritySafeCritical] // auto-generated
  ...
  RuntimeType[] candidates =
      this.Cache.GetInterfaceList(MemberListType.All, null);
...
public override MemberInfo[] GetMembers(BindingFlags bindingAttr) {
  ListBuilder<MethodInfo>methods = GetMethodCandidates(
      null, bindingAttr, CallingConventions.Any, null, -1, false);
  ListBuilder<ConstructorInfo>constructors = GetConstructorCandidates(
      null, bindingAttr, CallingConventions.Any, null, false);
  ListBuilder<PropertyInfo>properties =
      GetPropertyCandidates(null, bindingAttr, null, false);
  ...
  ListBuilder<Type>nestedTypes =
      GetNestedTypeCandidates(null, bindingAttr, false);
  // Interfaces are excluded from the result of GetMembers

  MemberInfo[] members =
      new MemberInfo[methods.Count + constructors.Count + properties.Count +
                     events.Count + fields.Count + nestedTypes.Count];
...
[System.Security.SecuritySafeCritical] // auto-generated
  ...
  // SZArrays implement the methods on IList`1, IEnumerable`1, and ICollection`1
  // with SZArrayHelper and some runtime magic. We don't have accurate interface
  // maps for them.
  ...
  for (int i = 0; i < ifaceInstanceMethodCount; i++) {
    RuntimeMethodHandleInternal ifaceRtMethodHandle =
        RuntimeTypeHandle.GetMethodAt(ifaceRtType, i);

    // GetMethodBase will convert this to the instantiating/unboxing stub if
    // necessary
    MethodBase ifaceMethodBase =
        RuntimeType.GetMethodBase(ifaceRtType, ifaceRtMethodHandle);
    ...
    int slot = GetTypeHandleInternal().GetInterfaceMethodImplementationSlot(
        ifaceRtTypeHandle, ifaceRtMethodHandle);

    if (slot == -1)
      continue;

    RuntimeMethodHandleInternal classRtMethodHandle =
        RuntimeTypeHandle.GetMethodAt(this, slot);

    // GetMethodBase will convert this to the instantiating/unboxing stub if
    // necessary
    MethodBase rtTypeMethodBase =
        RuntimeType.GetMethodBase(this, classRtMethodHandle);
    // a class may not implement all the methods of an interface (abstract
    // class) so null is a valid value
    Contract.Assert(rtTypeMethodBase == null ||
                    rtTypeMethodBase is RuntimeMethodInfo);
...
protected override MethodInfo GetMethodImpl(string name,
  ...
  return GetMethodImpl(name, -1, bindingAttr, binder, callConvention, types,
                       modifiers);
}
...
protected override MethodInfo GetMethodImpl(String name, int genericParamCount,
  ...
  ListBuilder<MethodInfo>candidates = GetMethodCandidates(
      name, bindingAttr, callConv, types, genericParamCount, false);
  ...
  if (types == null || types.Length == 0) {
    ...
    if (candidates.Count == 1) {
      ...
      // All the methods have the exact same name and sig so return the most
      // derived one.
  ...
  return binder.SelectMethod(bindingAttr, candidates.ToArray(), types,
                             modifiers) as MethodInfo;
}
#endif
...
#endif
{
  ListBuilder<ConstructorInfo>candidates = GetConstructorCandidates(
      null, bindingAttr, CallingConventions.Any, types, false);
...
#endif
{
  ...
  ListBuilder<PropertyInfo>candidates =
      GetPropertyCandidates(name, bindingAttr, types, false);
  ...
  if (types == null || types.Length == 0) {
    ...
    if (candidates.Count == 1) {
      ...
      if ((object) returnType == null)
        // if we are here we have no args or property type to select over and we
        // have more than one property with that name
        throw new AmbiguousMatchException(
            Environment.GetResourceString("Arg_AmbiguousMatchException"));
    }
  }
  ...
  if ((bindingAttr & BindingFlags.ExactBinding) != 0)
    return System.DefaultBinder.ExactPropertyBinding(
        candidates.ToArray(), returnType, types, modifiers);
...
public override FieldInfo GetField(String name, BindingFlags bindingAttr) {
  ...
  for (int i = 0; i < cache.Length; i++) {
    ...
    if ((bindingAttr & fieldInfo.BindingFlags) == fieldInfo.BindingFlags) {
      if (match != null) {
        if (Object.ReferenceEquals(fieldInfo.DeclaringType,
                                   match.DeclaringType))
          throw new AmbiguousMatchException(
              Environment.GetResourceString("Arg_AmbiguousMatchException"));

        if ((match.DeclaringType.IsInterface == true) &&
...
public override MemberInfo[] GetMember(String name, MemberTypes type,
  ...
  if ((type & MemberTypes.Method) != 0) {
    methods = GetMethodCandidates(name, bindingAttr, CallingConventions.Any,
                                  null, -1, true);
  ...
  if ((type & MemberTypes.Constructor) != 0) {
    constructors = GetConstructorCandidates(name, bindingAttr,
                                            CallingConventions.Any, null, true);
...
public override MethodBase DeclaringMethod {
  get {
    ...
    IRuntimeMethodInfo declaringMethod =
        RuntimeTypeHandle.GetDeclaringMethod(this);

    if (declaringMethod == null)
      return null;

    return GetMethodBase(RuntimeMethodHandle.GetDeclaringType(declaringMethod),
                         declaringMethod);
  }
}
#endif
...
#endif
{
  ...
  if (RuntimeFeature.IsDynamicCodeSupported &&
      c is System.Reflection.Emit.TypeBuilder) {
  ...
  if (RuntimeTypeHandle.IsGenericVariable(this)) {
    ...
    for (int i = 0; i < constraints.Length; i++) {
      ...
      if (constraint.IsGenericParameter) {
        ...
        if ((special & GenericParameterAttributes.ReferenceTypeConstraint) ==
                0 &&
            (special &
...
public override String AssemblyQualifiedName {
  get {
    ...
    // FullName is null if this type contains generic parameters but is not a
...
public override Guid
    GUID{[System.Security.SecuritySafeCritical] // auto-generated
         get {
...
IsWindowsRuntimeObjectType(RuntimeType type);

[MethodImplAttribute(MethodImplOptions.InternalCall)]
[SecurityCritical]
private static extern bool
IsTypeExportedToWindowsRuntime(RuntimeType type);
...
protected override bool IsValueTypeImpl() {
  // We need to return true for generic parameters with the ValueType
  // constraint. So we cannot use the faster RuntimeTypeHandle.IsValueType
  // because it returns false for all generic parameters.
...
public override GenericParameterAttributes GenericParameterAttributes {
  [System.Security.SecuritySafeCritical] // auto-generated
    ...
    return GetGenericParameterAttributes();
      ...
      RuntimeTypeHandle.GetMetadataImport(this).GetGenericParamProps(
          MetadataToken, out attributes);
...
public override string[] GetEnumNames() {
  ...
  // Make a copy since we can't hand out the same array since users can modify
  // them
...
public override bool IsEnumDefined(object value) {
  ...
  // If the value is an Enum then we need to extract the underlying value from
  // it
  if (valueType.IsEnum) {
    if (!valueType.IsEquivalentTo(this))
      throw new ArgumentException(
          Environment.GetResourceString("Arg_EnumAndObjectMustBeSameType",
                                        valueType.ToString(), this.ToString()));
  ...
  if (Type.IsIntegerType(valueType)) {
    ...
    if (underlyingType != valueType)
      throw new ArgumentException(Environment.GetResourceString(
          "Arg_EnumUnderlyingTypeAndObjectMustBeSameType", valueType.ToString(),
    ...
    // if at this point the value type is not an integer type, then its type
    // doesn't match the enum type NetCF used to throw an argument exception in
    // this case
    throw new ArgumentException(Environment.GetResourceString(
        "Arg_EnumUnderlyingTypeAndObjectMustBeSameType", valueType.ToString(),
...
public override string GetEnumName(object value) {
  ...
  if (!(valueType.IsEnum || IsIntegerType(valueType)))
    throw new ArgumentException(
        Environment.GetResourceString("Arg_MustBeEnumBaseTypeOrEnum"), "value");
...
// Initialized when a TypeBuilder is created to avoid link-time SRE dependencies
// in this file
...
[System.Security.SecuritySafeCritical] // auto-generated
  ...
  RuntimeType[] instantiationRuntimeType =
      new RuntimeType[instantiation.Length];
...
public override StructLayoutAttribute StructLayoutAttribute {
  [System.Security.SecuritySafeCritical] // overrides transparent public member
    ...
    return (StructLayoutAttribute)
        StructLayoutAttribute.GetCustomAttribute(this);
  }
}
#endregion
#endif
...
private const BindingFlags ClassicBindingMask =
    ...
    BindingFlags.SetProperty | BindingFlags.PutDispProperty |
...
[System.Security.SecurityCritical] // auto-generated
    ...
    CanValueSpecialCast(RuntimeType valueType, RuntimeType targetType);

[System.Security.SecurityCritical] // auto-generated
    [ResourceExposure(ResourceScope.None)]
    [MethodImplAttribute(MethodImplOptions.InternalCall)]
    static private extern Object
    AllocateValueType(RuntimeType type, object value, bool fForceTypeChange);

[System.Security.SecuritySafeCritical] // auto-generated
    internal unsafe Object
    CheckValue(Object value, Binder binder, CultureInfo culture,
               BindingFlags invokeAttr) {
  // this method is used by invocation in reflection to check whether a value
  // can be assigned to type.
  if (IsInstanceOfType(value)) {
    // Since this cannot be a generic parameter, we use
    // RuntimeTypeHandle.IsValueType here because it is faster than
...
#if FEATURE_REMOTING
    // For the remoting objects Object.GetType goes through proxy. Avoid the
    // proxy call and just get the type directly. It is necessary to support
    // proxies that do not handle GetType.
    RealProxy realProxy =
        System.Runtime.Remoting.RemotingServices.GetRealProxy(value);
    ...
    if (!Object.ReferenceEquals(type, this) &&
        RuntimeTypeHandle.IsValueType(this)) {
  ...
  if (isByRef) {
    ...
    if (elementType.IsInstanceOfType(value) || value == null) {
      // need to create an instance of the ByRef if null was provided, but only
      // if primitive, enum or value type
  ...
  if ((invokeAttr & BindingFlags.ExactBinding) == BindingFlags.ExactBinding)
    throw new ArgumentException(String.Format(
        ...
        Environment.GetResourceString("Arg_ObjObjEx"), value.GetType(), this));
...
private Object
TryChangeType(Object value, Binder binder, CultureInfo culture,
...
[System.Security.SecuritySafeCritical] // auto-generated
    ...
    InvokeMember(
        ...
        Object?[]? providedArgs, ParameterModifier[]? modifiers,
        CultureInfo? culture, String[]? namedParams)
#else
      String name, BindingFlags bindingFlags, Binder binder, Object target,
      Object[] providedArgs, ParameterModifier[] modifiers, CultureInfo culture,
      String[] namedParams)
#endif
{
  if (IsGenericParameter)
...
#region Preconditions
  if ((bindingFlags & InvocationMask) == 0)
    ...
    throw new ArgumentException(
        Environment.GetResourceString("Arg_NoAccessSpec"), "bindingFlags");
...
#region Preconditions
    if ((bindingFlags & ClassicBindingMask) == 0)
      throw new ArgumentException(
          Environment.GetResourceString("Arg_COMAccess"), "bindingFlags");

    if ((bindingFlags & BindingFlags.GetProperty) != 0 &&
        (bindingFlags & ClassicBindingMask &
         ~(BindingFlags.GetProperty | BindingFlags.InvokeMethod)) != 0)
      throw new ArgumentException(
          Environment.GetResourceString("Arg_PropSetGet"), "bindingFlags");

    if ((bindingFlags & BindingFlags.InvokeMethod) != 0 &&
        (bindingFlags & ClassicBindingMask &
         ~(BindingFlags.GetProperty | BindingFlags.InvokeMethod)) != 0)
      throw new ArgumentException(
          Environment.GetResourceString("Arg_PropSetInvoke"), "bindingFlags");

    if ((bindingFlags & BindingFlags.SetProperty) != 0 &&
        (bindingFlags & ClassicBindingMask & ~BindingFlags.SetProperty) != 0)
      throw new ArgumentException(
          Environment.GetResourceString("Arg_COMPropSetPut"), "bindingFlags");

    if ((bindingFlags & BindingFlags.PutDispProperty) != 0 &&
        (bindingFlags & ClassicBindingMask & ~BindingFlags.PutDispProperty) !=
            0)
      throw new ArgumentException(
          Environment.GetResourceString("Arg_COMPropSetPut"), "bindingFlags");

    if ((bindingFlags & BindingFlags.PutRefDispProperty) != 0 &&
        (bindingFlags & ClassicBindingMask &
         ~BindingFlags.PutRefDispProperty) != 0)
      throw new ArgumentException(
          Environment.GetResourceString("Arg_COMPropSetPut"), "bindingFlags");
#endregion
...
#region Non - TransparentProxy case
      ...
      // pass LCID_ENGLISH_US if no explicit culture is specified to match the
      // behavior of VB
      int lcid = (culture == null ? 0x0409 : culture.LCID);

      return InvokeDispMethod(name, bindingFlags, target, providedArgs, isByRef,
                              lcid, namedParams);
#endif
#endregion
    }
...
#region TransparentProxy case
      return ((MarshalByRefObject) target)
          .InvokeMember(name, bindingFlags, binder, providedArgs, modifiers,
                        culture, namedParams);
#endregion
#endif
    }
...
#region Check that any named paramters are not null
  if (namedParams != null && Array.IndexOf(namedParams, null) != -1)
    ...
    throw new ArgumentException(
        Environment.GetResourceString("Arg_NamedParamNull"), "namedParams");
#endregion
...
#region Delegate to Activator.CreateInstance
  if ((bindingFlags & BindingFlags.CreateInstance) != 0) {
    if ((bindingFlags & BindingFlags.CreateInstance) != 0 &&
      ...
      throw new ArgumentException(
          Environment.GetResourceString("Arg_CreatInstAccess"), "bindingFlags");

    return Activator.CreateInstance(this, bindingFlags, binder, providedArgs,
                                    culture);
  }
#endregion
  ...
  if ((bindingFlags &
       (BindingFlags.PutDispProperty | BindingFlags.PutRefDispProperty)) != 0)
...
#region Name
  ...
  if (name.Length == 0 || name.Equals(@"[DISPID=0]")) {
    ...
    if (name == null) {
      // in InvokeMember we always pretend there is a default member if none is
      // provided and we make it ToString
      name = "ToString";
    }
  }
#endregion
...
#region Preconditions
    if (IsGetField) {
      if (IsSetField)
        ...
        throw new ArgumentException(
            Environment.GetResourceString("Arg_FldSetGet"), "bindingFlags");

      if ((bindingFlags & BindingFlags.SetProperty) != 0)
        // "Can not specify both GetField and SetProperty."
        throw new ArgumentException(
            Environment.GetResourceString("Arg_FldGetPropSet"), "bindingFlags");
    } else {
      ...
      if ((bindingFlags & BindingFlags.GetProperty) != 0)
        ...
        throw new ArgumentException(
            Environment.GetResourceString("Arg_FldSetPropGet"), "bindingFlags");

      if ((bindingFlags & BindingFlags.InvokeMethod) != 0)
        // "Can not specify Set on a Field and Invoke on a method."
        throw new ArgumentException(
            Environment.GetResourceString("Arg_FldSetInvoke"), "bindingFlags");
    }
#endregion
...
#region Lookup Field
    ...
    FieldInfo[] flds =
        GetMember(name, MemberTypes.Field, bindingFlags) as FieldInfo[];
    ...
    if (flds.Length == 1) {
      ...
      selFld = binder.BindToField(bindingFlags, flds, IsGetField ? Empty.Value
                                  : providedArgs[0], culture);
    }
#endregion

    if (selFld != null) {
#region Invocation on a field
      if (selFld.FieldType.IsArray ||
...
if (useCache)
{
    MethodBase invokeMethod = GetMethodFromCache (name, bindingFlags, argCnt,
providedArgs);

    if (invokeMethod != null)
        return ((MethodInfo) invokeMethod).Invoke(target, bindingFlags, binder,
providedArgs, culture);
}
*/
#endregion
...
#region Lookup Methods
    ...
    for (int i = 0; i < semiFinalists.Length; i++) {
      ...
      if (!FilterApplyMethodInfo((RuntimeMethodInfo) semiFinalist, bindingFlags,
                                 CallingConventions.Any, new Type[argCnt]))
        continue;
...
#region Invoke
    if (finalists == null && argCnt == 0 &&
      ...
      //    AddMethodToCache(name, bindingFlags, argCnt, providedArgs,
      //    props[0]);

      return finalist.Invoke(target, bindingFlags, binder, providedArgs,
                             culture);
    }
    ...
    try {
      invokeMethod =
          binder.BindToMethod(bindingFlags, finalists, ref providedArgs,
                              modifiers, culture, namedParams, out state);
...
#endif
{
  // ComObjects are identified by the instance of the Type object and not the
  // TypeHandle.
...
#else
    return CustomAttribute.GetCustomAttributes(this, RuntimeType.ObjectType,
                                               inherit);
#endif
}
...
[System.Security.SecuritySafeCritical] // auto-generated
  ...
  RuntimeType attributeRuntimeType =
      attributeType.UnderlyingSystemType as RuntimeType;
...
#endif
// This is used by the ToString() overrides of all reflection types. The legacy
// behavior has the following problems:
//  1. Use only Name for nested types, which can be confused with global types
//  and generic parameters of the same name.
//  2. Use only Name for generic parameters, which can be confused with nested
//  types and global types of the same name.
//  3. Remove the namespace ("System") for all primitive types, which is not
//  language neutral.
//  4. MethodBase.ToString() use "ByRef" for byref parameters which is different
//  than Type.ToString().
//  5. ConstructorInfo.ToString() outputs "Void" as the return type. Why Void?
// Since it could be a breaking changes to fix these legacy behaviors, we only
// use the better and more unambiguous format in serialization
...
internal override string FormatTypeName(bool serialization) {
  if (serialization) {
    ...
    // Legacy: this doesn't make sense, why use only Name for nested types but
    // otherwise ToString() which contains namespace.
    if (elementType.IsNested)
      return Name;

    string typeName = ToString();

    // Legacy: why removing "System"? Is it just because C# has keywords for
    // these types? If so why don't we change it to lower case to match the C#
    // keyword casing?
    if (elementType.IsPrimitive || elementType == typeof (void) ||
...
private string GetCachedName(TypeNameKind kind) { return Cache.GetName(kind); }
#endif
...
private void CreateInstanceCheckThis() {
  ...
  if (Object.ReferenceEquals(elementType, typeof (void)))
    throw new NotSupportedException(
        Environment.GetResourceString("Acc_CreateVoid"));
}

[System.Security.SecurityCritical] // auto-generated
    internal Object
    CreateInstanceImpl(BindingFlags bindingAttr, Binder binder, Object[] args,
                       CultureInfo culture
#if !NETCORE
                       ,
  ...
  try {
    try {
      ...
      // deal with the __COMObject case first. It is very special because from a
      // reflection point of view it has no ctors so a call to GetMemberCons
      // would fail
      bool publicOnly = (bindingAttr & BindingFlags.NonPublic) == 0;
      bool wrapExceptions =
          (bindingAttr & BindingFlags.DoNotWrapExceptions) == 0;
      if (argCnt == 0 && (bindingAttr & BindingFlags.Public) != 0 &&
          (bindingAttr & BindingFlags.Instance) != 0 &&
          (IsGenericCOMObjectImpl() || IsValueType)) {
        server =
            CreateInstanceDefaultCtor(publicOnly, false, true, wrapExceptions
#if !NETCORE
                                      ,
                                      ref stackMark
#endif
        ...
        // We cannot use Type.GetTypeArray here because some of the args might
        // be null
        ...
        for (int i = 0; i < candidates.Length; i++) {
          if (FilterApplyConstructorInfo((RuntimeConstructorInfo) candidates[i],
                                         bindingAttr, CallingConventions.Any,
                                         argsType))
...
#endif
          throw new MissingMethodException(Environment.GetResourceString(
              "MissingConstructor_Name", FullName));
        }
        ...
        try {
          invokeMethod = binder.BindToMethod(bindingAttr, cons, ref args, null,
                                             culture, null, out state);
        } catch (MissingMethodException) {
          invokeMethod = null;
        }
        ...
        // name or MethodInfo.
        // if (isDelegate)
        if (RuntimeType.DelegateType.IsAssignableFrom(
                invokeMethod.DeclaringType)) {
#if FEATURE_CORECLR
          // In CoreCLR, CAS is not exposed externally. So what we really are
          // looking for is to see if the external caller of this API is
          // transparent or not. We get that information from the fact that a
          // Demand will succeed only if the external caller is not transparent.
          try {
...
#pragma warning restore 618
        ...
        if (invokeMethod.GetParametersNoCopy().Length == 0) {
          ...
          if (activationAttributes != null &&
              activationAttributes.Length != 0) {
            server =
                ActivationCreateInstance(invokeMethod, bindingAttr, binder,
                                         args, culture, activationAttributes);
          } else {
#endif
            // fast path??
            server = Activator.CreateInstance(this, nonPublic
                                              : true, wrapExceptions
                                              : wrapExceptions);
#if MONO && FEATURE_REMOTING
          }
...
//
object ActivationCreateInstance(MethodBase invokeMethod,
                                BindingFlags bindingAttr, Binder binder,
                                Object[] args, CultureInfo culture,
                                Object[] activationAttributes) {
  var server =
      ActivationServices.CreateProxyFromAttributes(this, activationAttributes);
  if (server != null)
  ...
  class ActivatorCacheEntry {
    ...
    // the delegate containing the call to the ctor, will be replaced by an
    // IntPtr to feed a calli with
    ...
    [System.Security.SecurityCritical]
    internal
    ActivatorCacheEntry(RuntimeType t, RuntimeMethodHandleInternal rmh,
      ...
      if (!m_hCtorMethodHandle.IsNullHandle())
        m_ctorAttributes =
            RuntimeMethodHandle.GetAttributes(m_hCtorMethodHandle);
    }
  }
  ...
  class ActivatorCache {
    ...
    private void InitializeDelegateCreator() {
      // No synchronization needed here. In the worst case we create extra
      // garbage
...
#pragma warning restore 618
    ...
    [System.Security.SecuritySafeCritical] // auto-generated
        private void
        InitializeCacheEntry(ActivatorCacheEntry ace) {
      if (!ace.m_type.IsValueType) {
        ...
        // No synchronization needed here. In the worst case we create extra
        // garbage
        CtorDelegate ctor = (CtorDelegate) delegateCtorInfo.Invoke(new Object[]{
            null,
    ...
    internal void SetEntry(ActivatorCacheEntry ace) {
      // fill the the array backwards to hit the most recently filled entries
      // first in GetEntry
  ...
  [System.Security.SecuritySafeCritical] // auto-generated
      internal Object
      CreateInstanceSlow(bool publicOnly, bool skipCheckThis, bool fillCache
#if !NETCORE
                         ,
                         ref StackCrawlMark stackMark
#endif
      ) {
    RuntimeMethodHandleInternal runtime_ctor =
    ...
    if ((invocationFlags & INVOCATION_FLAGS.INVOCATION_FLAGS_NON_W8P_FX_API) !=
        0) {
      RuntimeAssembly caller =
      ...
      if (caller != null && !caller.IsSafeForReflection())
        throw new InvalidOperationException(Environment.GetResourceString(
            "InvalidOperation_APIInvalidForCurrentContext", this.FullName));

      // Allow it because the caller is framework code, but don't cache the
      // result because we need to do the stack walk every time this type is
      // instantiated.
    ...
    Object instance = RuntimeTypeHandle.CreateInstance(
        this, publicOnly, bSecurityCheckOff, ref bCanBeCached, ref runtime_ctor,
    ...
    if (bCanBeCached && fillCache) {
      ...
      ActivatorCacheEntry ace =
          new ActivatorCacheEntry(this, runtime_ctor, bNeedSecurityCheck);
...
// fillCache is set in the SL2/3 compat mode or when called from
// Marshal.PtrToStructure.
[System.Security.SecuritySafeCritical] // auto-generated
    [DebuggerStepThroughAttribute]
    [Diagnostics.DebuggerHidden]
    internal Object
    CreateInstanceDefaultCtor(bool publicOnly, bool skipCheckThis,
                              bool fillCache, bool wrapExceptions
#if !NETCORE
                              ,
                              ref StackCrawlMark stackMark
#endif
  ...
  if (GetType() == typeof (ReflectionOnlyType))
    throw new InvalidOperationException(Environment.GetResourceString(
        "InvalidOperation_NotAllowedInReflectionOnly"));
#if !MONO
  ActivatorCache activatorCache = s_ActivatorCache;
  if (activatorCache != null) {
      ...
      if (ace.m_ctor != null) {
        ...
        if (ace.m_bNeedSecurityCheck)
          RuntimeMethodHandle.PerformSecurityCheck(
              instance, ace.m_hCtorMethodHandle, this,
  ...
  return CreateInstanceSlow(publicOnly, wrapExceptions, skipCheckThis,
                            fillCache, ref stackMark);
#else
    return CreateInstanceSlow(publicOnly, wrapExceptions, skipCheckThis,
                              fillCache);
#endif
...
_CreateEnum(RuntimeType enumType, long value);
[System.Security.SecuritySafeCritical] // auto-generated
    internal static Object
    CreateEnum(RuntimeType enumType, long value) {
...
[System.Security.SecurityCritical] // auto-generated
    ...
    InvokeDispMethod(String name, BindingFlags invokeAttr, Object target,
                     Object[] args, bool[] byrefModifiers, int culture,
...
[System.Security.SecurityCritical] // auto-generated
    [ResourceExposure(ResourceScope.None)]
    [MethodImplAttribute(MethodImplOptions.InternalCall)]
    internal static extern Type
    GetTypeFromProgIDImpl(String progID, String server, bool throwOnError);

[System.Security.SecurityCritical] // auto-generated
    [ResourceExposure(ResourceScope.None)]
    [MethodImplAttribute(MethodImplOptions.InternalCall)]
    internal static extern Type
    GetTypeFromCLSIDImpl(Guid clsid, String server, bool throwOnError);
#else  // FEATURE_COMINTEROP_UNMANAGED_ACTIVATION
internal static Type GetTypeFromProgIDImpl(String progID, String server,
                                           bool throwOnError) {
  throw new NotImplementedException(
      "CoreCLR_REMOVED -- Unmanaged activation removed"); // @
}

internal static Type GetTypeFromCLSIDImpl(Guid clsid, String server,
                                          bool throwOnError) {
  throw new NotImplementedException(
      "CoreCLR_REMOVED -- Unmanaged activation removed"); // @
}
...
[System.Security.SecuritySafeCritical] // auto-generated
    private Object
    ForwardCallToInvokeMember(String memberName, BindingFlags flags,
                              Object target, int[] aWrapperTypes,
  ...
  // If the method has a void return type, then set the IgnoreReturn binding
  // flag.
  ...
  try {
    ...
    ret = InvokeMember(memberName, flags, null, target, aArgs, aParamMod, null,
                       null);
  } catch (TargetInvocationException e) {
    // For target invocation exceptions, we need to unwrap the inner exception
    // and re-throw it.
...
[SecuritySafeCritical]
private void
WrapArgsForInvokeCall(Object[] aArgs, int[] aWrapperTypes) {
  int cArgs = aArgs.Length;
  for (int i = 0; i < cArgs; i++) {
    if (aWrapperTypes[i] == 0)
      continue;

    if (((DispatchWrapperType) aWrapperTypes[i] &
         DispatchWrapperType.SafeArray) != 0) {
      ...
      Object[] newArray =
          (Object[]) Array.UnsafeCreateInstance(wrapperType, numElems);
      ...
      for (int currElem = 0; currElem < numElems; currElem++) {
        if (isString) {
          ...
          newArray[currElem] =
              wrapperCons.Invoke(new Object[]{oldArray.GetValue(currElem)});
        }
      }
...
private OleAutBinder ForwardCallBinder{get {
  ...
  if(s_ForwardCallBinder == null) s_ForwardCallBinder = new OleAutBinder();
...
private enum DispatchWrapperType : int {
  // This enum must stay in sync with the DispatchWrapperType enum defined in
  // MLInfo.h
  Unknown = 0x00000001,
  Dispatch = 0x00000002,
  Record = 0x00000004,
...
// this is the introspection only type. This type overrides all the functions
// with runtime semantics and throws an exception. The idea behind this type is
// that it relieves RuntimeType from doing honerous checks about ReflectionOnly
// context.
// This type should not derive from RuntimeType but it's doing so for
// convinience. That should not present a security threat though it is risky as
// a direct call to one of the base method method (RuntimeType) and an instance
// of this type will work around the reason to have this type in the first
// place. However given RuntimeType is not public all its methods are protected
// and require full trust to be accessed
[Serializable]
...
internal unsafe struct Utf8String {
  [System.Security.SecurityCritical] // auto-generated
      ...
      EqualsCaseSensitive(void *szLhs, void *szRhs, int cSz);
  ...
  [System.Security.SecurityCritical] // auto-generated
      ...
      private static extern unsafe bool
      EqualsCaseInsensitive(void *szLhs, void *szRhs, int cSz);

  [System.Security.SecurityCritical] // auto-generated
      [ResourceExposure(ResourceScope.None)]
      [DllImport(JitHelpers.QCall, CharSet = CharSet.Unicode)]
      [SuppressUnmanagedCodeSecurity]
      private static extern unsafe uint
      HashCaseInsensitive(void *sz, int cSz);

  [System.Security.SecurityCritical] // auto-generated
      private static int
      GetUtf8StringByteLength(void *pUtf8String) {
    int len = 0;
  ...
  [System.Security.SecurityCritical] // auto-generated
      internal unsafe
      Utf8String(void *pUtf8String, int cUtf8String) {
  ...
  [System.Security.SecuritySafeCritical] // auto-generated
      internal unsafe bool
      EqualsCaseInsensitive(Utf8String s) {
    if (m_pStringHeap == null) {
      return s.m_StringHeapByteLength == 0;
    }
    if ((s.m_StringHeapByteLength == m_StringHeapByteLength) &&
  ...
  [System.Security.SecuritySafeCritical] // auto-generated
    ...
    unsafe {
      ...
      for (int currentPos = 0; currentPos < m_StringHeapByteLength;
           currentPos++) {
        buf[currentPos] = *pItr;
        pItr++;
      }
...
namespace System.Reflection {
  // Reliable hashtable thread safe for multiple readers and single writer. Note
  // that the reliability goes together with thread safety. Thread safety for
  // multiple readers requires atomic update of the state that also makes makes
  // the table reliable in the presence of asynchronous exceptions.
  internal struct CerHashtable<K, V>where K : class {
    private class Table {
      // Note that m_keys and m_values arrays are immutable to allow lock-free
      // reads. A new instance of CerHashtable has to be allocated to grow the
      ...
      internal void Insert(K key, V value) {
        ...
        while (true) {
          ...
          if (hit == null) {
            ...
            // This volatile write has to be last. It is going to publish the
            // result atomically.
            //
            // Note that incrementing the count or setting the value does not do
            // any harm without setting the key. The inconsistency will be
            // ignored and it will go away completely during next rehash.
    ...
    private static int GetHashCodeHelper(K key) {
      ...
      // For strings we don't want the key to differ across domains as
      // CerHashtable might be shared.
      if (sKey == null) {
    ...
    internal V this[K key] {
      ...
      get {
        ...
        while (true) {
          // This volatile read has to be first. It is reading the atomically
          // published result.
