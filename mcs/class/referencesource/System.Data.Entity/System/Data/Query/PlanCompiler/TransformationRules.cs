...
// using System.Diagnostics; // Please use PlanCompiler.Assert instead of
// Debug.Assert in this class...

// It is fine to use Debug.Assert in cases where you assert an obvious thing
// that is supposed to prevent from simple mistakes during development (e.g.
// method argument validation in cases where it was you who created the
// variables or the variables had already been validated or in "else" clauses
// where due to code changes (e.g. adding a new value to an enum type) the
// default "else" block is chosen why the new condition should be treated
// separately). This kind of asserts are (can be) helpful when developing new
// code to avoid simple mistakes but have no or little value in the shipped
// product. PlanCompiler.Assert *MUST* be used to verify conditions in the
// trees. These would be assumptions about how the tree was built etc. - in
// these cases we probably want to throw an exception (this is what
// PlanCompiler.Assert does when the condition is not met) if either the
// assumption is not correct or the tree was built/rewritten not the way we
// thought it was. Use your judgment - if you rather remove an assert than ship
// it use Debug.Assert otherwise use PlanCompiler.Assert.
...
#region public methods and properties
    ...
    /// <summary>
    /// Whether any rule was applied that may have caused modifications such
    /// that projection pruning may be useful
    /// </summary>
    ...
    /// <summary>
    /// Whether any rule was applied that may have caused modifications such
    /// that reapplying the nullability rules may be useful
    /// </summary>
    ...
    /// <summary>
    /// "Remap" an expression tree, replacing all references to vars in varMap
    /// with copies of the corresponding expression The subtree is modified
    /// *inplace* - it is the caller's responsibility to make a copy of the
    /// subtree if necessary. The "replacement" expression (the replacement for
    /// the VarRef) is copied and then inserted into the appropriate location
    /// into the subtree.
    ///
    /// Note: we only support replacements in simple ScalarOp trees. This must
    ...
    internal Node ReMap(Node node, Dictionary<Var, Node>varMap) {
      PlanCompiler.Assert(node.Op.IsScalarOp,
                          "Expected a scalarOp: Found " +
                              Dump.AutoString.ToString(node.Op.OpType));
    ...
    /// <summary>
    /// Makes a copy of the appropriate subtree - with a simple accelerator for
    /// VarRefOp since that's likely to be the most command case
    /// </summary>
    ...
    internal bool IsNonNullable(Var var ) {
      foreach(Node relOpAncestor in m_relOpAncestors) {
        // Rules applied to the children of the relOpAncestor may have caused it
        // change. Thus, if the node is used, it has to have its node info
        // recomputed
    ...
    /// 1. The top most sort includes null sentinels. If the null sentinel is
    /// replaced with a different value and is used as a sort key it may change
    /// the sorting results
    /// 2. If any of the ancestors is Distinct, GroupBy, Intersect or Except,
    /// because the null sentinel may be used as a key.
    /// 3. If the null sentinel is defined in the left child of an apply it may
    /// be used at the right side, thus in these cases we also verify that the
    /// right hand side does not have any Distinct, GroupBy, Intersect or
    /// Except.
    /// </summary>
    internal bool CanChangeNullSentinelValue {
      get {
        ...
        if (this.m_relOpAncestors.Any(
                a => IsOpNotSafeForNullSentinelValueChange(a.Op.OpType))) {
          return false;
        }

        // Is the null sentinel defined in the left child of an apply and if so,
        // does the right hand side have any Distinct, GroupBy, Intersect or
        // Except.
        var applyAncestors =
            this.m_relOpAncestors.Where(a => a.Op.OpType == OpType.CrossApply ||
        ...
        foreach(Node applyAncestor in applyAncestors) {
          if (!this.m_relOpAncestors.Contains(applyAncestor.Child1) &&
    ...
    internal static bool IsOpNotSafeForNullSentinelValueChange(OpType optype) {
      return optype == OpType.Distinct || optype == OpType.GroupBy ||
             optype == OpType.Intersect || optype == OpType.Except;
    }
    ...
    /// <summary>
    /// Is this is a scalar-op tree? Also return a dictionary of var refcounts
    /// (ie) for each var encountered in the tree, determine the number of times
    /// it has been seen
    /// </summary>
    /// <param name="node">current subtree</param>
    ...
    /// <summary>
    /// Get a mapping from Var->Expression for a VarDefListOp tree. This
    /// information will be used by later stages to replace all references to
    /// the Vars by the corresponding expressions
    ///
    ...
    internal Dictionary<Var, Node>GetVarMap(Node varDefListNode,
      ...
      foreach(Node chi in varDefListNode.Children) {
        ...
        // the defining expression for the Var is "expensive" (ie) has larger
        // than 100 nodes, then simply pretend that this is too hard to do Note:
        // we check for more than 2 references, (rather than just more than 1) -
        // this is simply to let some additional cases through
        //
        if ((nonLeafNodeCount > 100) && (varRefMap != null) &&
        ...
        if (varMap.TryGetValue(varDefOp.Var, out n)) {
          PlanCompiler.Assert(n == chi.Child0,
    ...
    /// where v is the conditionVar parameter, and expr is the value of the
    /// expression when v is non-null
    /// </summary>
    ...
    internal Node BuildNullIfExpression(Var conditionVar, Node expr) {
      ...
      Node elseNode = expr;
      Node thenNode =
          this.Command.CreateNode(this.Command.CreateNullOp(elseNode.Op.Type));
      Node caseNode =
          this.Command.CreateNode(this.Command.CreateCaseOp(elseNode.Op.Type),
...
#region Rule Interactions
    ...
    /// </summary>
    /// <param name="n"></param>
    internal void SuppressFilterPushdown(Node n) { m_suppressions[n] = n; }
    ...
    /// <returns></returns>
    internal static bool TryGetInt32Var(IEnumerable<Var>varList,
                                        out Var int32Var) {
      foreach(Var v in varList) {
        // Any Int32 var regardless of the fasets will do
        System.Data.Metadata.Edm.PrimitiveTypeKind typeKind;
        if (System.Data.Common.TypeHelpers.TryGetPrimitiveTypeKind(
                v.Type, out typeKind) &&
...
#region private state
    ...
    /// </summary>
    internal readonly System.Text.StringBuilder appliedRules =
...
#region RuleProcessingContext Overrides
    ...
    internal override void PreProcessSubTree(Node subTree) {
      ...
      // We need to do remapping only if m_remappedVars overlaps with
    ...
    /// <summary>
    /// If the given node has a RelOp it is popped from the relOp ancestors
    /// stack.
    /// </summary>
    /// <param name="subtree"></param>
    ...
    /// If the rule is among the rules after which projection pruning may be
    /// beneficial, m_projectionPruningRequired is set to true. If the rule is
    /// among the rules after which reapplying the nullability rules may be
    /// beneficial, m_reapplyNullabilityRules is set to true.
    /// </summary>
    /// <param name="n"></param>
...
#endif
        if (!this.m_projectionPruningRequired &&
            TransformationRules.RulesRequiringProjectionPruning.Contains(
                rule)) {
...
#region private methods
    ...
    /// Additionally, compute the number of non-leaf nodes (ie) nodes with at
    /// least one child that are found in the subtree. Note that this count is
    /// approximate - it is only intended to be used as a hint. It is the
    /// caller's responsibility to initialize nodeCount to a sane value on entry
    /// into this function And finally, if the varRefMap parameter is non-null,
    /// we keep track of how often a Var is referenced within the subtree
    ///
    /// The non-leaf-node count and the varRefMap are used by GetVarMap to
    /// determine if expressions can be composed together
    /// </summary>
    /// <param name="node">root of the subtree</param>
    /// <param name="varRefMap">Ref counts for each Var encountered in the
    /// subtree</param> <param name="nonLeafNodeCount">count of non-leaf nodes
    /// encountered in the subtree</param> <returns>true, if this node only
    /// contains scalarOps</returns>
    private bool IsScalarOpTree(Node node, Dictionary<Var, int>varRefMap,
  ...
  internal static class TransformationRules {
    ...
    /// The lookup table is an array indexed by OpType and each entry has a list
    /// of rules.
    /// </summary>
    internal static readonly ReadOnlyCollection<
        ReadOnlyCollection<InternalTrees.Rule>>AllRulesTable =
        BuildLookupTableForRules(AllRules);
    ...
    /// </summary>
    internal static readonly ReadOnlyCollection<
        ReadOnlyCollection<InternalTrees.Rule>>ProjectRulesTable =
    ...
    /// </summary>
    internal static readonly ReadOnlyCollection<
        ReadOnlyCollection<InternalTrees.Rule>>PostJoinEliminationRulesTable =
        BuildLookupTableForRules(PostJoinEliminationRules);

    /// <summary>
    /// A lookup table built only from rules that rely on nullability of vars
    /// and other rules that may be able to perform simplification if these have
    /// been applied. The lookup table is an array indexed by OpType and each
    /// entry has a list of rules.
    /// </summary>
    internal static readonly ReadOnlyCollection<
        ReadOnlyCollection<InternalTrees.Rule>>NullabilityRulesTable =
        BuildLookupTableForRules(NullabilityRules);

    /// <summary>
    /// A look-up table of rules that may cause modifications such that
    /// projection pruning may be useful after they have been applied.
    /// </summary>
    internal static readonly HashSet<InternalTrees.Rule>
        RulesRequiringProjectionPruning =
            InitializeRulesRequiringProjectionPruning();

    /// <summary>
    /// A look-up table of rules that may cause modifications such that
    /// reapplying the nullability rules may be useful after they have been
    /// applied.
    /// </summary>
    internal static readonly HashSet<InternalTrees.Rule>
...
#region private state maintenance
      ...
      get {
        if (postJoinEliminationRules == null) {
          ...
          postJoinEliminationRules.AddRange(
              ProjectOpRules.Rules); // these don't use key info per-se, but can
                                     // help after the distinct op rules.
    ...
    private static ReadOnlyCollection<ReadOnlyCollection<InternalTrees.Rule>>
    BuildLookupTableForRules(IEnumerable<InternalTrees.Rule>rules) {
      ...
      List<InternalTrees.Rule>[] lookupTable =
          new List<InternalTrees.Rule>[ (int) OpType.MaxMarker ];
      ...
      return new ReadOnlyCollection<ReadOnlyCollection<InternalTrees.Rule>>(
          rulesPerType);
    }

    private static HashSet<InternalTrees.Rule>
    InitializeRulesRequiringProjectionPruning() {
    ...
    /// <summary>
    /// Apply the rules that belong to the specified group to the given query
    /// tree.
    /// </summary>
    ...
    internal static bool Process(PlanCompiler compilerState,
      ...
      // If any rule has been applied after which reapplying nullability rules
      // may be useful, reapply nullability rules.
      ...
      if (Process(compilerState, rulesTable, out projectionPruningRequired)) {
        ...
        Process(compilerState, NullabilityRulesTable,
                out projectionPruningRequired2);
        projectionPruningRequired =
            projectionPruningRequired || projectionPruningRequired2;
      }
      return projectionPruningRequired;
    }

    /// <summary>
    /// Apply the rules that belong to the specified rules table to the given
    /// query tree.
    /// </summary>
    /// <param name="compilerState"></param>
    /// <param name="rulesTable"></param>
    /// <param name="projectionPruningRequired">is projection pruning  required
    /// after the rule application</param> <returns>Whether any rule has been
    /// applied after which reapplying nullability rules may be useful</returns>
    private static bool Process(
      ...
      TransformationRulesContext context =
          new TransformationRulesContext(compilerState);
      compilerState.Command.Root = ruleProcessor.ApplyRulesToSubtree(
          context, rulesTable, compilerState.Command.Root);
  ...
  /// </summary>
  internal enum TransformationRulesGroup { All, Project, PostJoinElimination }
...
#region CaseOp Rules
    internal static readonly SimpleRule Rule_SimplifyCase =
        new SimpleRule(OpType.Case, ProcessSimplifyCase);
    internal static readonly SimpleRule Rule_FlattenCase =
        new SimpleRule(OpType.Case, ProcessFlattenCase);
    /// <summary>
    /// We perform the following simple transformation for CaseOps. If every
    /// single then/else expression in the CaseOp is equivalent, then we can
    /// simply replace the Op with the first then/expression. Specifically, case
    /// when w1 then t1 when w2 then t2 ... when wn then tn else e end
    ///   => t1
    ...
    static bool ProcessSimplifyCase(RuleProcessingContext context,
                                    Node caseOpNode, out Node newNode) {
      CaseOp caseOp = (CaseOp) caseOpNode.Op;
      newNode = caseOpNode;

      //
      // Can I collapse the entire case-expression into a single expression -
      // yes, if all the then/else clauses are the same expression
      //
      ...
      //
      if (ProcessSimplifyCase_EliminateWhenClauses(context, caseOp, caseOpNode,
                                                   out newNode)) {
        return true;
      }
    ...
    /// If every single then/else expression in the CaseOp is equivalent, then
    /// we can simply replace the CaseOp with the first then/expression.
    /// Specifically, case when w1 then t1 when w2 then t2 ... when wn then tn
    /// else e end
    ///   => t1
    ...
    /// If any of the WHEN clauses is the 'TRUE' expression, then all branches
    /// to the right of it are irrelevant - eliminate them. Eliminate this
    /// branch as well, and make the THEN expression of this branch the ELSE
    /// expression for the entire Case expression. If the WHEN expression
    /// represents the first branch, then replace the entire case expression by
    /// the corresponding THEN expression
    /// </summary>
    /// <param name="context">rule processing context</param>
    ...
    private static bool
    ProcessSimplifyCase_EliminateWhenClauses(RuleProcessingContext context,
                                             CaseOp caseOp, Node caseOpNode,
                                             out Node newNode) {
      ...
      for (int i = 0; i < caseOpNode.Children.Count;) {
        ...
        if (i == caseOpNode.Children.Count - 1) {
          ...
          // This really belongs in more general SoftCastOp logic in the
          // CTreeGenerator that converts SoftCasts that could affect the result
          // type of the query into a real cast or a trivial case statement, to
          // preserve the result type. This is tracked by SQL PT Work Item
          // #300003327.
        ...
        // If the current then clause is a SoftCast then we do not attempt to
        // simplify the case operation, since this may change the result type.
        // Again, this really belongs in the CTreeGenerator as per SQL PT Work
        // Item #300003327.
        ...
        ConstantPredicateOp constPred =
            (ConstantPredicateOp) caseOpNode.Children[i].Op;
        ...
        // the succeeding arguments. We make the "then" clause of this
        // when-clause as the "else-clause" of the resulting caseOp
        if (constPred.IsTrue) {
          newNodeArgs.Add(caseOpNode.Children[i + 1]);
          break;
        } else {
      ...
      PlanCompiler.Assert(newNodeArgs.Count > 0,
                          "new args list must not be empty");
      // Is there only one expression in the args list - simply return that
      // expression
    ...
    /// <summary>
    /// If the else clause of the CaseOp is another CaseOp, when two can be
    /// collapsed into one. In particular,
    ///
    /// CASE
    ///     WHEN W1 THEN T1
    ///     WHEN W2 THEN T2 ...
...
#region EqualsOverConstant Rules
    ...
    /// <summary>
    /// Convert an Equals(X, Y) to a "true" predicate if X=Y, or a "false"
    /// predicate if X!=Y Convert a NotEquals(X,Y) in the reverse fashion
    /// </summary>
    /// <param name="context">Rule processing context</param>
    ...
    static bool ProcessComparisonsOverConstant(RuleProcessingContext context,
                                               Node node, out Node newNode) {
      newNode = node;
      PlanCompiler.Assert(node.Op.OpType == OpType.EQ ||
                              node.Op.OpType == OpType.NE,
      ...
      bool result = (node.Op.OpType == OpType.EQ) ? (bool) comparisonStatus
                                                  : !((bool) comparisonStatus);
      ConstantPredicateOp newOp =
          context.Command.CreateConstantPredicateOp(result);
...
#region LikeOp Rules
    private static bool ? MatchesPattern(string str, string pattern) {
      // What we're trying to see is if the pattern is something that ends with
      // a '%' And if the "str" is something that matches everything before that

      // Make sure that the terminal character of the pattern is a '%'
      // character. Also ensure that this character does not occur anywhere
      // else. And finally, ensure that the pattern is at most one character
    ...
    static bool ProcessLikeOverConstant(RuleProcessingContext context, Node n,
                                        out Node newNode) {
      newNode = n;
      ...
      bool ? match =
                 MatchesPattern((string) strOp.Value, (string) patternOp.Value);
      if (match == null) {
        return false;
      }

      ConstantPredicateOp constOp =
          context.Command.CreateConstantPredicateOp((bool) match);
...
#region LogicalOp(and, or, not) Rules
    ...
    /// <param name="otherNode">The other child of the LogOp (possibly
    /// null)</param> <param name="newNode">new subtree</param>
    /// <returns>transformation status</returns>
    static bool ProcessLogOpOverConstant(RuleProcessingContext context,
                                         Node node, Node constantPredicateNode,
                                         Node otherNode, out Node newNode) {
      PlanCompiler.Assert(constantPredicateNode != null,
                          "null constantPredicateOp?");
      ...
      case OpType.Or:
        ...
        break;
      case OpType.Not:
        PlanCompiler.Assert(otherNode == null,
                            "Not Op with more than 1 child. Gasp!");
    ...
    static bool ProcessAndOverConstantPredicate1(RuleProcessingContext context,
                                                 Node node, out Node newNode) {
      return ProcessLogOpOverConstant(context, node, node.Child1, node.Child0,
                                      out newNode);
    }
    static bool ProcessAndOverConstantPredicate2(RuleProcessingContext context,
                                                 Node node, out Node newNode) {
      return ProcessLogOpOverConstant(context, node, node.Child0, node.Child1,
                                      out newNode);
    }
    ...
    static bool ProcessNotOverConstantPredicate(RuleProcessingContext context,
                                                Node node, out Node newNode) {
      return ProcessLogOpOverConstant(context, node, node.Child0, null,
                                      out newNode);
    }
#endregion
...
#region IsNull Rules
    ...
    static bool ProcessIsNullOverConstant(RuleProcessingContext context,
                                          Node isNullNode, out Node newNode) {
...
#region CastOp(NullOp) Rule
    internal static readonly PatternMatchRule Rule_NullCast =
        new PatternMatchRule(new Node(CastOp.Pattern, new Node(NullOp.Pattern)),
                             ProcessNullCast);

    /// <summary>
    /// eliminates nested null casts into a single cast of the outermost cast
    /// type. basically the transformation applied is: cast(null[x] as T) =>
    /// null[t]
    /// </summary>
    /// <param name="context"></param>
    ...
    /// <returns></returns>
    static bool ProcessNullCast(RuleProcessingContext context, Node castNullOp,
                                out Node newNode) {
      newNode = context.Command.CreateNode(
...
#region All ScalarOp Rules
    internal static readonly InternalTrees.Rule[] Rules =
        new InternalTrees.Rule[]{
            Rule_SimplifyCase,
            Rule_FlattenCase,
...
#region Helpers
    /// <summary>
    /// Split up a predicate into 2 parts - the pushdown and the non-pushdown
    /// predicate.
    ///
    /// If the filter node has no external references *and* the "columns"
    /// parameter is null, then the entire predicate can be pushed down
    ///
    /// We then compute the set of valid column references - if the "columns"
    /// parameter is non-null, this set is used. Otherwise, we get the
    /// definitions of the input relop node of the filterOp, and use that.
    ///
    /// We use this list of valid column references to identify which parts of
    /// the filter predicate can be pushed down - only those parts of the
    /// predicate that do not reference anything beyond these columns are
    /// considered for pushdown. The rest are stuffed into the
    /// nonPushdownPredicate output parameter
    ///
    /// </summary>
    /// <param name="command">Command object</param>
    /// <param name="filterNode">the FilterOp subtree</param>
    /// <param name="columns">(Optional) List of columns to consider for
    /// "pushdown"</param> <param name="nonPushdownPredicateNode">(output) Part
    /// of the predicate that cannot be pushed down</param> <returns>part of the
    /// predicate that can be pushed down</returns>
    private static Node
    GetPushdownPredicate(Command command, Node filterNode, VarVec columns,
      ...
      if (columns == null) {
        ExtendedNodeInfo inputNodeInfo =
            command.GetExtendedNodeInfo(filterNode.Child0);
      ...
      predicate =
          predicate.GetSingleTablePredicates(columns, out nonPushdownPredicate);
...
#region FilterOverFilter
    internal static readonly PatternMatchRule Rule_FilterOverFilter =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(FilterOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
    ...
    static bool ProcessFilterOverFilter(RuleProcessingContext context,
                                        Node filterNode, out Node newNode) {
      ...
      newNode =
          context.Command.CreateNode(context.Command.CreateFilterOp(),
...
#region FilterOverProject
    internal static readonly PatternMatchRule Rule_FilterOverProject =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(ProjectOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
    ...
    static bool ProcessFilterOverProject(RuleProcessingContext context,
      ...
      //
      // If the filter is a constant predicate, then don't push the filter below
      // the project
      //
      ...
      Dictionary<Var, Node>varMap =
          trc.GetVarMap(projectNode.Child1, varRefMap);
      if (varMap == null) {
        return false;
      }
      ...
      //
      Node newFilterNode =
          trc.Command.CreateNode(trc.Command.CreateFilterOp(),
...
#region FilterOverSetOp
    internal static readonly PatternMatchRule Rule_FilterOverUnionAll =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(UnionAllOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
    ...
    internal static readonly PatternMatchRule Rule_FilterOverIntersect =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(IntersectOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
            ProcessFilterOverSetOp);
    internal static readonly PatternMatchRule Rule_FilterOverExcept =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(ExceptOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
            ProcessFilterOverSetOp);
    /// <summary>
    /// Transform Filter(UnionAll(X1, X2), p) => UnionAll(Filter(X1, p1),
    /// Filter(X, p2))
    ///           Filter(Intersect(X1, X2), p) => Intersect(Filter(X1, p1),
    ///           Filter(X2, p2)) Filter(Except(X1, X2), p) => Except(Filter(X1,
    ///           p1), X2)
    /// where p1 and p2 are the "mapped" versions of the predicate "p" for each
    /// branch
    /// </summary>
    /// <param name="context">Rule processing context</param>
    /// <param name="filterNode">FilterOp subtree</param>
    ...
    static bool ProcessFilterOverSetOp(RuleProcessingContext context,
      ...
      //
      // Identify parts of the filter predicate that can be pushed down, and
      // parts that cannot be. If nothing can be pushed down, then return
      //
      ...
      //
      // Now push the predicate (the part that can be pushed down) into each of
      // the branches (as appropriate)
      //
      ...
      foreach(VarMap varMap in setOp.VarMap) {
        ...
        foreach(KeyValuePair<Var, Var>kv in varMap) {
          Node varRefNode =
              trc.Command.CreateNode(trc.Command.CreateVarRefOp(kv.Value));
          remapMap.Add(kv.Key, varRefNode);
        }

        //
        // Now fix up the predicate.
        // Make a copy of the predicate first - except if we're dealing with the
        // last branch, in which case, we can simply reuse the predicate
        //
        ...
        Node newFilterNode = trc.Command.CreateNode(
            trc.Command.CreateFilterOp(), setOpNode.Children[branchId],
            newPredicateNode);
      ...
      Node newSetOpNode =
          trc.Command.CreateNode(setOpNode.Op, newSetOpChildren);

      //
      // We've now pushed down the relevant parts of the filter below the SetOps
      // We may still however some predicates left over - create a new filter
      // node to account for that
      //
      if (nonPushdownPredicate != null) {
        newNode = trc.Command.CreateNode(trc.Command.CreateFilterOp(),
...
#region FilterOverDistinct
    ...
    ///    where p2 is the part of the filter that can be pushed down, while p1
    ///    represents any external references
    /// </summary>
    /// <param name="context">Rule processing context</param>
    /// <param name="filterNode">FilterOp subtree</param>
    /// <param name="newNode">modified subtree</param>
    ...
    static bool ProcessFilterOverDistinct(RuleProcessingContext context,
      ...
      newNode = filterNode;
      //
      // Split up the filter predicate into two parts - the part that can be
      // pushed down and the part that can't. If there is no part that can be
      // pushed down, simply return
      //
      Node nonPushdownPredicate;
      Node pushdownPredicate = GetPushdownPredicate(
      ...
      //
      // Create a new filter node below the current distinct node for the
      // predicate that can be pushed down - create a new distinct node as well
      //
      Node distinctNode = filterNode.Child0;
      Node pushdownFilterNode =
          context.Command.CreateNode(context.Command.CreateFilterOp(),
                                     distinctNode.Child0, pushdownPredicate);
      Node newDistinctNode =
          context.Command.CreateNode(distinctNode.Op, pushdownFilterNode);
...
#region FilterOverGroupBy
    internal static readonly PatternMatchRule Rule_FilterOverGroupBy =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(GroupByOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
    ...
    ///    respectively - specifically, p1 must only reference the key columns
    ///    from the GroupByOp.
    ...
    static bool ProcessFilterOverGroupBy(RuleProcessingContext context,
      ...
      //
      // Split up the predicate into two parts - the part that can be pushed
      // down below the groupByOp (specifically, the part that only refers to
      // keys of the groupByOp), and the part that cannot be pushed below If
      // nothing can be pushed below, quit now
      //
      Node nonPushdownPredicate;
      Node pushdownPredicate =
          GetPushdownPredicate(context.Command, filterNode, groupByOp.Keys,
      ...
      // that any references to variables defined locally by the groupBy are
      // fixed up Make sure that the predicate is not too complex to remap
      //
      Dictionary<Var, Node>varMap =
          trc.GetVarMap(groupByNode.Child1, varRefMap);
      if (varMap == null) {
      ...
      Node newGroupByNode =
          trc.Command.CreateNode(groupByNode.Op, subFilterNode,
                                 groupByNode.Child1, groupByNode.Child2);

      //
      // If there was any part of the original predicate that could not be
      // pushed down, create a new filterOp node above the new groupBy node to
...
#region FilterOverJoin
    internal static readonly PatternMatchRule Rule_FilterOverCrossJoin =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(CrossJoinOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
    ...
    internal static readonly PatternMatchRule Rule_FilterOverInnerJoin =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(InnerJoinOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
            ProcessFilterOverJoin);
    internal static readonly PatternMatchRule Rule_FilterOverLeftOuterJoin =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(LeftOuterJoinOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
            ProcessFilterOverJoin);
    ...
    static bool ProcessFilterOverJoin(RuleProcessingContext context,
      ...
      //
      ExtendedNodeInfo rightTableNodeInfo =
          command.GetExtendedNodeInfo(rightInputNode);
      ...
      ExtendedNodeInfo leftTableInfo =
          command.GetExtendedNodeInfo(leftInputNode);

      //
      // Check to see if the predicate contains any "single-table-filters". In
      // those cases, we could simply push that filter down to the child. We can
      // do this for inner joins and cross joins - for both inputs. For
      // left-outer joins, however, we can only do this for the left-side input
      // Further note that we only want to do the pushdown if it will help us -
      // if the join input is a ScanTable (or some other cases), then it doesn't
      // help us.
      //
      Node leftSingleTablePredicateNode = null;
      if (leftInputNode.Op.OpType != OpType.ScanTable) {
        Predicate leftSingleTablePredicates =
            predicate.GetSingleTablePredicates(leftTableInfo.Definitions,
                                               out predicate);
      ...
      if ((rightInputNode.Op.OpType != OpType.ScanTable) &&
        ...
        Predicate rightSingleTablePredicates =
            predicate.GetSingleTablePredicates(rightTableNodeInfo.Definitions,
                                               out predicate);
        rightSingleTablePredicateNode =
            rightSingleTablePredicates.BuildAndTree();
      }

      //
      // Now check to see if the predicate contains some "join predicates". We
      // can add these to the existing join predicate (if any). We can only do
      // this for inner joins and cross joins - not for LOJs
      //
      ...
      if (joinOp.OpType == OpType.CrossJoin ||
        ...
        Predicate joinPredicate = predicate.GetJoinPredicates(
            leftTableInfo.Definitions, rightTableNodeInfo.Definitions,
            out predicate);
        newJoinPredicateNode = joinPredicate.BuildAndTree();
      }

      //
      // Now for the dirty work. We've identified some predicates that could be
      // pushed into the left table, some predicates that could be pushed into
      // the right table and some that could become join predicates.
      //
      if (leftSingleTablePredicateNode != null) {
        leftInputNode =
            command.CreateNode(command.CreateFilterOp(), leftInputNode,
                               leftSingleTablePredicateNode);
        needsTransformation = true;
      }
      if (rightSingleTablePredicateNode != null) {
        rightInputNode =
            command.CreateNode(command.CreateFilterOp(), rightInputNode,
                               rightSingleTablePredicateNode);
        needsTransformation = true;
      }
      ...
      if (newJoinPredicateNode != null) {
        ...
        if (joinOp.OpType == OpType.CrossJoin) {
          ...
          PlanCompiler.Assert(joinOp.OpType == OpType.InnerJoin,
        ...
        newJoinPredicateNode =
            (joinOp.OpType == OpType.CrossJoin) ? null : joinNode.Child2;
      }
      ...
      //
      // Build up a new filterNode above this join node. But only if we have a
      // filter left
      //
      ...
      if (newFilterPredicateNode == null) {
        ...
        newNode = command.CreateNode(command.CreateFilterOp(), newJoinNode,
...
#region Filter over OuterApply
    internal static readonly PatternMatchRule Rule_FilterOverOuterApply =
        new PatternMatchRule(
            new Node(FilterOp.Pattern,
                     new Node(OuterApplyOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern)),
                     new Node(LeafOp.Pattern)),
    ...
    /// if "p" is not null-preserving for Y (ie) "p" does not preserve null
    /// values from Y
    /// </summary>
    /// <param name="context">Rule processing context</param>
    ...
    static bool ProcessFilterOverOuterApply(RuleProcessingContext context,
      ...
      //
      // Check to see if the current predicate preserves nulls for the right
      // table. If it doesn't then we can convert the outer apply into a
      // cross-apply,
      //
      ExtendedNodeInfo rightTableNodeInfo =
          command.GetExtendedNodeInfo(applyRightInputNode);
      Predicate predicate = new Predicate(command, filterNode.Child1);
      if (!predicate.PreservesNulls(rightTableNodeInfo.Definitions, true)) {
        Node newApplyNode =
            command.CreateNode(command.CreateCrossApplyOp(), applyNode.Child0,
                               applyRightInputNode);
...
#region FilterWithConstantPredicate
    ...
    /// <returns>transformation status</returns>
    static bool
    ProcessFilterWithConstantPredicate(RuleProcessingContext context, Node n,
                                       out Node newNode) {
      newNode = n;
      ...
      TransformationRulesContext trc = (TransformationRulesContext) context;
      ExtendedNodeInfo childNodeInfo =
          trc.Command.GetExtendedNodeInfo(n.Child0);
      ...
      foreach(Var v in childNodeInfo.Definitions) {
        ...
        Var computedVar;
        Node varDefNode =
            trc.Command.CreateVarDefNode(constNode, out computedVar);
      ...
      Node singleRowTableNode =
          trc.Command.CreateNode(trc.Command.CreateSingleRowTableOp());
...
#region ProjectOverProject
    ...
    static bool ProcessProjectOverProject(RuleProcessingContext context,
                                          Node projectNode, out Node newNode) {
      newNode = projectNode;
      ...
      Dictionary<Var, Node>varMap =
          trc.GetVarMap(subProjectNode.Child1, varRefMap);
      if (varMap == null) {
        return false;
      }

      // create a new varDefList node...
      Node newVarDefListNode =
          trc.Command.CreateNode(trc.Command.CreateVarDefListOp());
      ...
      ExtendedNodeInfo projectNodeInfo =
          trc.Command.GetExtendedNodeInfo(projectNode);
      ...
      //
      // now that we have remapped all our computed vars, simply bypass the
      // subproject node
      //
...
#region ProjectWithNoLocalDefinitions
    ...
    /// of the ProjectOp (the VarDefListOp child) has no children, then the
    /// ProjectOp is serving no useful purpose. Get rid of the ProjectOp, and
    ...
    /// <returns>transformation status</returns>
    static bool
    ProcessProjectWithNoLocalDefinitions(RuleProcessingContext context, Node n,
                                         out Node newNode) {
      newNode = n;
      NodeInfo nodeInfo = context.Command.GetNodeInfo(n);

      // We cannot eliminate this node because it can break other rules,
      // e.g. ProcessApplyOverAnything which relies on existence of external
      // refs to substitute CrossApply(x, y) => CrossJoin(x, y). See SQLBU
      // #481719.
...
#region ProjectOpWithSimpleVarRedefinitions
    internal static readonly SimpleRule
        Rule_ProjectOpWithSimpleVarRedefinitions = new SimpleRule(
            OpType.Project, ProcessProjectWithSimpleVarRedefinitions);
    /// <summary>
    /// If the ProjectOp defines some computedVars, but those computedVars are
    /// simply redefinitions of other Vars, then eliminate the computedVars.
    ///
    ...
    /// <returns>transformation status</returns>
    static bool
    ProcessProjectWithSimpleVarRedefinitions(RuleProcessingContext context,
                                             Node n, out Node newNode) {
      newNode = n;
      ...
      // Note: Even if we don't have any local var definitions left, we should
      // not remove this project yet because:
      ...
      Node newVarDefListNode =
          command.CreateNode(command.CreateVarDefListOp(), newVarDefNodes);
...
#region ProjectOpWithNullSentinel
    internal static readonly SimpleRule Rule_ProjectOpWithNullSentinel =
        new SimpleRule(OpType.Project, ProcessProjectOpWithNullSentinel);
    /// <summary>
    /// Tries to remove null sentinel definitions by replacing them to vars that
    /// are guaranteed to be non-nullable and of integer type, or with reference
    /// to other constants defined in the same project. In particular,
    ///
    ///  - If based on the ancestors, the value of the null sentinel can be
    ///  changed and the
    /// input of the project has a var that is guaranteed to be non-nullable and
    /// is of integer type, then the definitions of the vars defined as
    /// NullSentinels in the ProjectOp are replaced with a reference to that
    /// var. I.eg:
    ///
    ...
    ///
    /// - Else, if based on the ancestors, the value of the null sentinel can be
    /// changed and the project already has definitions of other int constants,
    /// the definitions of the null sentinels are removed and the respective
    /// vars are remapped to the var representing the constant.
    ///
    /// - Else, the definitions of the all null sentinels except for one are
    /// removed, and the the respective vars are remapped to the remaining null
    /// sentinel.
    /// </summary>
    /// <param name="context">Rule processing context</param>
    /// <param name="n">current subtree</param>
    ...
    static bool ProcessProjectOpWithNullSentinel(RuleProcessingContext context,
      ...
      TransformationRulesContext trc = (TransformationRulesContext) context;
      Command command = trc.Command;
      ExtendedNodeInfo relOpInputNodeInfo =
      ...
      if (!canChangeNullSentinelValue ||
        ...
        if (!canChangeNullSentinelValue ||
            !TransformationRulesContext.TryGetInt32Var(
                n.Child1.Children
                    .Where(child => child.Child0.Op.OpType == OpType.Constant ||
                                    child.Child0.Op.OpType ==
...
#region ApplyOverFilter
    internal static readonly PatternMatchRule Rule_CrossApplyOverFilter =
        new PatternMatchRule(
            new Node(CrossApplyOp.Pattern, new Node(LeafOp.Pattern),
                     new Node(FilterOp.Pattern, new Node(LeafOp.Pattern),
                              new Node(LeafOp.Pattern))),
    ...
    static bool ProcessApplyOverFilter(RuleProcessingContext context,
                                       Node applyNode, out Node newNode) {
      newNode = applyNode;
      Node filterNode = applyNode.Child1;
      Command command = context.Command;

      NodeInfo filterInputNodeInfo = command.GetNodeInfo(filterNode.Child0);
      ExtendedNodeInfo applyLeftChildNodeInfo =
          command.GetExtendedNodeInfo(applyNode.Child0);

      //
      // check to see if the inputNode to the FilterOp has any external
      // references to the left child of the ApplyOp. If it does, we simply
      // return, we can't do much more here
      //
      if (filterInputNodeInfo.ExternalReferences.Overlaps(
              applyLeftChildNodeInfo.Definitions)) {
        return false;
      }

      //
      // We've now gotten to the stage where the only external references (if
      // any) are from the filter predicate. We can now simply convert the apply
      // into an inner/leftouter join with the filter predicate acting as the
      // join condition
      //
      ...
      newNode = command.CreateNode(joinOp, applyNode.Child0, filterNode.Child0,
                                   filterNode.Child1);
      return true;
    }

    internal static readonly PatternMatchRule
        Rule_OuterApplyOverProjectInternalConstantOverFilter =
            new PatternMatchRule(
                new Node(
                    OuterApplyOp.Pattern, new Node(LeafOp.Pattern),
                    new Node(
                        ProjectOp.Pattern,
                        new Node(FilterOp.Pattern, new Node(LeafOp.Pattern),
                                 new Node(LeafOp.Pattern)),
    ...
    internal static readonly PatternMatchRule
        Rule_OuterApplyOverProjectNullSentinelOverFilter = new PatternMatchRule(
            new Node(
                OuterApplyOp.Pattern, new Node(LeafOp.Pattern),
                new Node(ProjectOp.Pattern,
                         new Node(FilterOp.Pattern, new Node(LeafOp.Pattern),
    ...
    /// if "Y" has no external references to X
    ///
    /// In an ideal world, we would be able to push the Project below the
    /// Filter, and then have the normal ApplyOverFilter rule handle this - but
    /// that causes us problems because we always try to pull up ProjectOp's as
    /// high as possible. Hence, the special case for this rule
    ///
    /// </summary>
    /// <param name="context">Rule processing context</param>
    /// <param name="applyNode">Current ApplyOp</param>
    ...
    static bool ProcessOuterApplyOverDummyProjectOverFilter(
      ...
      ExtendedNodeInfo filterInputNodeInfo =
          command.GetExtendedNodeInfo(filterInputNode);
      ExtendedNodeInfo applyLeftChildNodeInfo =
          command.GetExtendedNodeInfo(applyNode.Child0);
      ...
      //
      // We've now gotten to the stage where the only external references (if
      // any) are from the filter predicate. First, push the Project node down
      // below the filter - but make sure that all the Vars needed by the Filter
      // are projected out
      //
      ...
      //
      // Check to see whether there is a sentinel var available - if there is,
      // then we can simply move the ProjectOp above the join we're going to
      // construct and of course, build a NullIf expression for the constant.
      // Otherwise, the ProjectOp will need to be the child of the joinOp that
      // we're building - and we'll need to make sure that the ProjectOp
      // projects out any vars that are required for the Filter in the first
      // place
      //
      TransformationRulesContext trc = (TransformationRulesContext) context;
      Var sentinelVar;
      ...
      if (TransformationRulesContext.TryGetInt32Var(
      ...
      if (sentinelVar != null) {
        ...
        //
        NodeInfo filterPredicateNodeInfo =
            command.GetNodeInfo(filterNode.Child1);
      ...
      //
      // We can now simply convert the apply into an inner/leftouter join with
      // the filter predicate acting as the join condition
      //
      Node joinNode =
          command.CreateNode(command.CreateLeftOuterJoinOp(), applyNode.Child0,
...
#region ApplyOverProject
    ...
    ///
    /// The second (simpler) form is used if a "sentinel" var can be located
    /// (ie) some Var of Y that is guaranteed to be non-null. Otherwise, we
    /// create a dummy ProjectNode as the right child of the Apply - which
    /// simply projects out all the vars of the Y, and adds on a constant (say
    /// "1"). This constant is now treated as the sentinel var
    ///
    /// Then the existing ProjectOp is pulled up above the the outer-apply, but
    /// all the locally defined Vars have their defining expressions now
    /// expressed as
    ...
    ///
    /// Special cases.
    /// * If the oldDefiningExpr is itself an internal constant equivalent to
    /// the null sentinel ("1"),
    ///   we simply project a ref to the null sentinel, no need for cast
    /// * If the ProjectOp contained exactly one locally defined Var, and it was
    /// a constant, then
    ///   we simply return - we will be looping endlessly otherwise
    /// * If the ProjectOp contained no local definitions, then we don't need to
    /// create the
    ///   dummy projectOp - we can simply pull up the Project
    /// * If any of the defining expressions of the local definitions was simply
    /// a VarRefOp
    ///   referencing a Var that was defined by Y, then there is no need to add
    ///   the case expression for that.
    ///
    /// </summary>
    /// <param name="context">RuleProcessing context</param>
    /// <param name="applyNode">The ApplyOp subtree</param>
    ...
    static bool ProcessOuterApplyOverProject(RuleProcessingContext context,
      ...
      TransformationRulesContext trc = (TransformationRulesContext) context;
      ExtendedNodeInfo inputNodeInfo =
          context.Command.GetExtendedNodeInfo(projectNode.Child0);
      Var sentinelVar = inputNodeInfo.NonNullableDefinitions.First;

      //
      // special case handling first - we'll end up in an infinite loop
      // otherwise. If the ProjectOp is the dummy ProjectOp that we would be
      // building (ie) it defines only 1 var - and the defining expression is
      // simply a constant
      //
      if (sentinelVar == null && varDefListNode.Children.Count == 1 &&
      ...
      ExtendedNodeInfo projectInputNodeInfo =
          command.GetExtendedNodeInfo(projectNode.Child0);

      //
      // Build up a dummy project node.
      // Walk through each local definition of the current project Node, and
      // convert all expressions into case expressions whose value depends on
      // the var produced by the dummy project node
      //

      // Dev10 #480443: If any of the definitions changes we need to recompute
      // the node info.
      bool anyVarDefChanged = false;
      foreach(Node varDefNode in varDefListNode.Children) {
        PlanCompiler.Assert(varDefNode.Op.OpType == OpType.VarDef,
                            "Expected VarDefOp. Found " + varDefNode.Op.OpType +
                                " instead");
        VarRefOp varRefOp = varDefNode.Child0.Op as VarRefOp;
        if (varRefOp == null ||
          ...
          if (sentinelVar == null) {
            nullSentinelDefinitionOp =
                command.CreateInternalConstantOp(command.IntegerType, 1);
            Node dummyConstantExpr =
                command.CreateNode(nullSentinelDefinitionOp);
          ...
          // If the null sentinel was just created, and the local definition of
          // the current project Node is an internal constant equivalent to the
          // null sentinel, it can be rewritten as a reference to the null
          // sentinel.
          if (nullSentinelDefinitionOp != null &&
            ...
            currentDefinition =
                command.CreateNode(command.CreateVarRefOp(sentinelVar));
          } else {
            currentDefinition =
                trc.BuildNullIfExpression(sentinelVar, varDefNode.Child0);
          }
      ...
      //
      // If we've created a dummy project node, make that the new child of the
      // applyOp
      //
      applyNode.Child1 =
      ...
      //
      // Pull up the project node above the apply node now. Also, make sure that
      // every Var of the applyNode's definitions actually shows up in the new
      // Project
      //
      projectNode.Child0 = applyNode;
      ExtendedNodeInfo applyLeftChildNodeInfo =
...
#region ApplyOverAnything
    ...
    static bool ProcessApplyOverAnything(RuleProcessingContext context,
      ...
      ExtendedNodeInfo applyRightChildNodeInfo =
          command.GetExtendedNodeInfo(applyRightChild);
      ExtendedNodeInfo applyLeftChildNodeInfo =
          command.GetExtendedNodeInfo(applyLeftChild);

      //
      // If we're currently dealing with an OuterApply, and the right child is
      // guaranteed to produce at least one row, then we can convert the
      // outer-apply into a cross apply
      //
      ...
      //
      // Does the right child reference any of the definitions of the left
      // child? If it does, then simply return from this function
      //
      if (applyRightChildNodeInfo.ExternalReferences.Overlaps(
              applyLeftChildNodeInfo.Definitions)) {
        if (convertedToCrossApply) {
          newNode =
              command.CreateNode(applyOp, applyLeftChild, applyRightChild);
          return true;
        } else {
          return false;
        }
      ...
      } else // outer apply
      {
        ...
        newNode = command.CreateNode(joinOp, applyLeftChild, applyRightChild,
                                     trueNode);
      }
      return true;
    }
...
#region ApplyIntoScalarSubquery
    internal static readonly PatternMatchRule
        Rule_CrossApplyIntoScalarSubquery = new PatternMatchRule(
            new Node(CrossApplyOp.Pattern, new Node(LeafOp.Pattern),
                     new Node(LeafOp.Pattern)),
            ProcessApplyIntoScalarSubquery);
    internal static readonly PatternMatchRule
        Rule_OuterApplyIntoScalarSubquery = new PatternMatchRule(
            new Node(OuterApplyOp.Pattern, new Node(LeafOp.Pattern),
                     new Node(LeafOp.Pattern)),
            ProcessApplyIntoScalarSubquery);

    /// <summary>
    /// Converts a Apply(X,Y) => Project(X, Y1), where Y1 is a scalar subquery
    /// version of Y The transformation is valid only if all of the following
    /// conditions hold:
    ///     1. Y produces only one output
    ///     2. Y produces at most one row
    ///     3. Y produces at least one row, or the Apply operator in question is
    ///     an OuterApply
    /// </summary>
    /// <param name="context">Rule processing context</param>
    /// <param name="applyNode">The ApplyOp subtree</param>
    ...
    static bool ProcessApplyIntoScalarSubquery(RuleProcessingContext context,
      ...
      Command command = context.Command;
      ExtendedNodeInfo applyRightChildNodeInfo =
          command.GetExtendedNodeInfo(applyNode.Child1);
      OpType applyKind = applyNode.Op.OpType;

      if (!CanRewriteApply(applyNode.Child1, applyRightChildNodeInfo,
                           applyKind)) {
        newNode = applyNode;
        return false;
      }

      // Create the project node over the original input with element over the
      // apply as new projected var
      ExtendedNodeInfo applyLeftChildNodeInfo =
          command.GetExtendedNodeInfo(applyNode.Child0);
      ...
      VarVec projectOpOutputs =
          command.CreateVarVec(applyLeftChildNodeInfo.Definitions);

      //
      // Remap the var defining tree to get it into a consistent state
      // and then remove all references to oldVar from it to avoid them being
      // wrongly remapped to newVar in subsequent remappings.
      //
      TransformationRulesContext trc = (TransformationRulesContext) context;
      ...
      Var newVar;
      Node varDefListNode =
          command.CreateVarDefListNode(elementNode, out newVar);
      projectOpOutputs.Set(newVar);

      newNode = command.CreateNode(command.CreateProjectOp(projectOpOutputs),
                                   applyNode.Child0, varDefListNode);
    ...
    /// <summary>
    /// Determines whether an applyNode can be rewritten into a projection with
    /// a scalar subquery. It can be done if all of the following conditions
    /// hold:
    ///     1. The right child or the apply has only one output
    ///     2. The right child of the apply produces at most one row
    ///     3. The right child of the apply produces at least one row, or the
    ...
    /// <returns></returns>
    private static bool
    CanRewriteApply(Node rightChild, ExtendedNodeInfo applyRightChildNodeInfo,
                    OpType applyKind) {
      ...
      // Dev10 #488632: Make sure the right child not only declares to produce
      // only one definition,
      // but has exactly one output. For example, ScanTableOp really outputs all
      // the columns from the table, but in its ExtendedNodeInfo.Definitions
      // only these that are referenced are shown. This is to allow for
      // projection pruning of the unreferenced columns.
    ...
    /// with a given root
    /// </summary>
    internal class OutputCountVisitor
        : BasicOpVisitorOfT<int>{
#region Constructors
...
#region Helpers
  ...
  internal new int VisitChildren(Node n) {
    int result = 0;
    foreach(Node child in n.Children) { result += VisitNode(child); }
    return result;
  }
  ...
  /// <param name="n"></param>
  /// <returns>/returns>
  protected override int VisitDefault(Node n) { return VisitChildren(n); }
...
#region SetOp Visitors
  ...
  /// <param name="n"></param>
  /// <returns></returns>
  public override int Visit(DistinctOp op, Node n) { return op.Keys.Count; }
  ...
  /// <param name="n"></param>
  /// <returns></returns>
  public override int Visit(FilterOp op, Node n) { return VisitNode(n.Child0); }
  ...
  /// <param name="n"></param>
  /// <returns></returns>
  public override int Visit(GroupByOp op, Node n) { return op.Outputs.Count; }
  ...
  /// <param name="n"></param>
  /// <returns></returns>
  public override int Visit(ProjectOp op, Node n) { return op.Outputs.Count; }
...
#region TableOps
  ...
  /// <param name="n"></param>
  /// <returns></returns>
  public override int Visit(SingleRowTableOp op, Node n) { return 0; }
...
/// <summary>
/// A utility class that remaps a given var at its definition and also remaps
/// all its references. The given var is remapped to an arbitrary new var. If
/// the var is defined by a ScanTable, all the vars defined by that table and
/// all their references are remapped as well.
/// </summary>
internal class VarDefinitionRemapper : VarRemapper {
  ...
  /// Unlike the base implementation, we want to visit the childrent, even if no
  /// vars are in the remapping dictionary.
  /// </summary>
  /// <param name="subTree"></param>
  internal override void RemapSubtree(Node subTree) {
    foreach(Node chi in subTree.Children) { RemapSubtree(chi); }
  ...
  /// <summary>
  /// If the columnVars defined by the table contain the var that needs to be
  /// remapped all the column vars produces by the table are remaped to new
  /// vars.
  /// </summary>
  /// <param name="op"></param>
  /// <param name="n"></param>
  ...
  public override void Visit(ScanTableOp op, Node n) {
    if (op.Table.Columns.Contains(m_oldVar)) {
      ScanTableOp newScanTableOp =
          m_command.CreateScanTableOp(op.Table.TableMetadata);
...
internal static readonly PatternMatchRule
    Rule_CrossApplyOverLeftOuterJoinOverSingleRowTable = new PatternMatchRule(
        new Node(CrossApplyOp.Pattern, new Node(LeafOp.Pattern),
...
#region JoinOverProject
  ...
  /// LeftOuterJoin(Project(A), B, p) => Project(LeftOuterJoin(A, B, p'),
  /// modifiedvars)
  /// </summary>
  /// <param name="context">Rule processing context</param>
  ...
  static bool ProcessJoinOverProject(RuleProcessingContext context,
                                     Node joinNode, out Node newNode) {
    newNode = joinNode;
    ...
    if ((joinNode.Op.OpType != OpType.LeftOuterJoin) &&
      ...
      Dictionary<Var, Node>varMap1 =
          trc.GetVarMap(joinNode.Child0.Child1, varRefMap);
      Dictionary<Var, Node>varMap2 =
          trc.GetVarMap(joinNode.Child1.Child1, varRefMap);
      ...
      foreach(Var v in projectOp2.Outputs) { newVarSet.Set(v); }
      ...
      Node varDefListNode =
          command.CreateNode(command.CreateVarDefListOp(), varDefNodes);
      newProjectNode =
          command.CreateNode(newProjectOp, newJoinNode, varDefListNode);
      newNode = newProjectNode;
      return true;
    }
    ...
    if (joinNode.Child0.Op.OpType == OpType.Project) {
      ...
      PlanCompiler.Assert(joinNode.Op.OpType != OpType.LeftOuterJoin,
    ...
    ExtendedNodeInfo otherChildInfo =
        command.GetExtendedNodeInfo(joinNode.Children[otherNodeIdx]);
    VarVec vec = command.CreateVarVec(projectOp.Outputs);
    ...
    joinNode.Children[projectNodeIdx] =
        projectNode.Child0; // bypass the projectOp
    context.Command.RecomputeNodeInfo(joinNode);

    newNode =
        context.Command.CreateNode(projectOp, joinNode, projectNode.Child1);
    return true;
  }
#endregion
...
#region JoinOverFilter
  ...
  ///
  /// Note that the predicate on the right table in a left-outer-join cannot be
  /// pulled up above the join.
  ///
  /// </summary>
  /// <param name="context">Rule processing context</param>
  /// <param name="joinNode">Current JoinOp tree to process</param>
  ...
  static bool ProcessJoinOverFilter(RuleProcessingContext context,
    ...
    if (joinNode.Child1.Op.OpType == OpType.Filter &&
      ...
      if (predicateNode == null) {
        ...
        predicateNode =
            command.CreateNode(command.CreateConditionalOp(OpType.And),
                               predicateNode, joinNode.Child1.Child1);
      }
    ...
    if (joinNode.Op.OpType == OpType.CrossJoin) {
      newJoinNode =
          command.CreateNode(joinNode.Op, newLeftInput, newRightInput);
    } else {
...
#region Join over SingleRowTable
  ...
  internal static readonly PatternMatchRule
      Rule_LeftOuterJoinOverSingleRowTable = new PatternMatchRule(
          new Node(LeftOuterJoinOp.Pattern, new Node(LeafOp.Pattern),
                   new Node(SingleRowTableOp.Pattern),
                   new Node(LeafOp.Pattern)),
          ProcessJoinOverSingleRowTable);
  /// <summary>
  /// Convert a CrossJoin(SingleRowTable, X) or CrossJoin(X, SingleRowTable) or
  /// LeftOuterJoin(X, SingleRowTable)
  ///    into just "X"
  /// </summary>
  /// <param name="context">rule processing context</param>
  /// <param name="joinNode">the join node</param>
...
internal static class SingleRowOpRules {
  ...
  static bool ProcessSingleRowOpOverAnything(RuleProcessingContext context,
    ...
    TransformationRulesContext trc = (TransformationRulesContext) context;
    ExtendedNodeInfo childNodeInfo =
        context.Command.GetExtendedNodeInfo(singleRowNode.Child0);

    // If the input to this Op can produce at most one row, then we don't need
    // the singleRowOp - simply return the input
    ...
    if (singleRowNode.Child0.Op.OpType == OpType.Filter) {
      Predicate predicate =
          new Predicate(context.Command, singleRowNode.Child0.Child1);
      if (predicate.SatisfiesKey(childNodeInfo.Keys.KeyVars,
...
#region SetOpOverFilters
  ...
  /// <param name="filterNodeIndex">Index of the filter node in the
  /// setop</param> <param name="newNode">transformed subtree</param>
  /// <returns>transformation status</returns>
  private static bool ProcessSetOpOverEmptySet(RuleProcessingContext context,
                                               Node setOpNode,
                                               out Node newNode) {
    bool leftChildIsEmptySet =
        context.Command.GetExtendedNodeInfo(setOpNode.Child0).MaxRows ==
        RowCount.Zero;
    bool rightChildIsEmptySet =
        context.Command.GetExtendedNodeInfo(setOpNode.Child1).MaxRows ==
        RowCount.Zero;
...
#region GroupByOpWithSimpleVarRedefinitions
  ...
  /// <summary>
  /// If the GroupByOp defines some computedVars as part of its keys, but those
  /// computedVars are simply redefinitions of other Vars, then eliminate the
  /// computedVars.
  ///
  ...
  /// <returns>transformation status</returns>
  static bool
  ProcessGroupByWithSimpleVarRedefinitions(RuleProcessingContext context,
                                           Node n, out Node newNode) {
    newNode = n;
...
#region GroupByOverProject
  ...
  ///
  /// We only do this if each c1, ..ck is referenced (in aggregates) at most
  /// once or it is a constant.
  /// </summary>
  /// <param name="context">Rule processing context</param>
  /// <param name="projectNode">Current ProjectOp node</param>
  /// <param name="newNode">modified subtree</param>
  /// <returns>Transformation status</returns>
  static bool ProcessGroupByOverProject(RuleProcessingContext context, Node n,
                                        out Node newNode) {
    newNode = n;
    ...
    VarVec projectDefinitions =
        command.GetExtendedNodeInfo(projectNode).LocalDefinitions;

    // If any of the defined vars is output, than we need the extra project
    // anyway.
    ...
    // If there are any constants remove them from the list that needs to be
    // tested, These can safely be replaced
    ...
    if (VarRefUsageFinder.AnyVarUsedMoreThanOnce(projectDefinitions, aggregates,
                                                 command)) {
      return false;
    }

    // If we got here it means that all vars were either constants, or used at
    // most once
    // Create a dictionary to be used for remapping the keys and the aggregates
    Dictionary<Var, Node>varToDefiningNode =
        new Dictionary<Var, Node>(projectNodeVarDefList.Children.Count);
  ...
  internal class VarRefReplacer : BasicOpVisitorOfNode {
    ...
    private VarRefReplacer(Dictionary<Var, Node>varReplacementTable,
                           Command command) {
    ...
    /// <param name="command"></param>
    /// <returns></returns>
    internal static Node Replace(Dictionary<Var, Node>varReplacementTable,
                                 Node root, Command command) {
      VarRefReplacer replacer =
          new VarRefReplacer(varReplacementTable, command);
  ...
  internal class VarRefUsageFinder : BasicOpVisitor {
    ...
    /// <summary>
    /// Public entry point. Returns true if at least one of the given vars
    /// occurs more than once in the subtree rooted at the given root.
    /// </summary>
    /// <param name="varVec"></param>
    /// <param name="root"></param>
    /// <param name="command"></param>
    /// <returns></returns>
    internal static bool AnyVarUsedMoreThanOnce(VarVec varVec, Node root,
                                                Command command) {
...
#region GroupByOpWithNoAggregates
  internal static readonly PatternMatchRule Rule_GroupByOpWithNoAggregates =
      new PatternMatchRule(new Node(GroupByOp.Pattern, new Node(LeafOp.Pattern),
                                    new Node(LeafOp.Pattern),
                                    new Node(VarDefListOp.Pattern)),
  ...
  ///
  /// (1) and if it includes all all the keys of the input, than it is
  /// unnecessary GroupBy (X, keys) -> Project(X, keys) where keys includes all
  /// keys of X.
  ///
...
#region GroupByOpOnAllInputColumnsWithAggregateOperation
  ...
  internal static readonly SimpleRule
      Rule_GroupByOpOnAllInputColumnsWithAggregateOperation = new SimpleRule(
          OpType.GroupBy,
          ProcessGroupByOpOnAllInputColumnsWithAggregateOperation);

  /// <summary>
  /// Converts a GroupBy(X, Y, Z) => OuterApply(X', GroupBy(Filter(X, key(X') ==
  /// key(X)), Y, Z)) if and only if X is a ScanTableOp, and Z is the upper node
  /// of an aggregate function and the group by operation uses all the columns
  /// of X as the key. Additionally, the top-level physical projection must only
  /// expose one variable. If it exposes more than one (more than just the
  /// aggregate itself), then this rule must not apply. This is a fix for devdiv
  /// bug 851732. Since now we're supporting NewRecordOp nodes as part of the
  /// GroupBy aggregate variable computations, we are also respecting the fact
  /// that group by (e => e) means that we're grouping by all columns of entity
  /// e. This was not a problem when the NewRecordOp node was not being
  /// processed since this caused the GroupBy statement to be simplified to a
  /// form with no keys and no output columns. The generated SQL is correct, but
  /// it is different from what it used to be and may be incompatible if the
  /// entity contains fields with datatypes that do not support being grouped
  /// by, such as blobs and images. This rule simplifies the tree so that we
  /// remain compatible with the way we were generating queries that contain
  /// group by (e => e). What this does is enabling the tree to take a shape
  /// that further optimization can convert into an expression that groups by
  /// the key of the table and calls the aggregate function as expected.
  /// </summary>
  /// <param name="context"> Rule processing context </param>
  ...
  private static bool ProcessGroupByOpOnAllInputColumnsWithAggregateOperation(
      RuleProcessingContext context, Node n, out Node newNode) {
    newNode = n;
    ...
    if (n.Child2 == null || n.Child2.Child0 == null ||
    ...
    // All the columns of Child0 are used, so remove them from the outputs and
    // the keys
    foreach(var column in allInputColumns) {
    ...
    // Build the OuterApply and also set the filter around the GroupBy's scan
    // table.
    ...
    var outerApplyNode =
        command.CreateNode(command.CreateOuterApplyOp(), scanTable, n);
    ...
    newNode = command.CreateNode(command.CreateProjectOp(newVar),
                                 outerApplyNode, varDefListNode);
    ...
    for (int i = 0; i < sourceTable.Keys.Count; ++i) {
      ...
      if (equality != null) {
        equality = command.CreateNode(command.CreateConditionalOp(OpType.And),
                                      equality, comparison);
      } else {
        equality = comparison;
      }
    ...
    var filter =
        command.CreateNode(command.CreateFilterOp(), n.Child0, equality);
    n.Child0 = filter;
...
#region SortOpOverAtMostOneRow
  internal static readonly SimpleRule Rule_SortOpOverAtMostOneRow =
      new SimpleRule(OpType.Sort, ProcessSortOpOverAtMostOneRow);
  /// <summary>
  /// If the SortOp's input is guaranteed to produce at most 1 row, remove the
  /// node with the SortOp:
...
#region ConstrainedSortOpOverEmptySet
  internal static readonly SimpleRule Rule_ConstrainedSortOpOverEmptySet =
      new SimpleRule(OpType.ConstrainedSort,
                     ProcessConstrainedSortOpOverEmptySet);
  /// <summary>
  /// If the ConstrainedSortOp's input is guaranteed to produce no rows, remove
  /// the ConstrainedSortOp completely:
  ...
  /// <returns>transformation status</returns>
  static bool
  ProcessConstrainedSortOpOverEmptySet(RuleProcessingContext context, Node n,
                                       out Node newNode) {
    ExtendedNodeInfo nodeInfo = ((TransformationRulesContext) context)
                                    .Command.GetExtendedNodeInfo(n.Child0);
...
#region DistinctOpOfKeys
  internal static readonly SimpleRule Rule_DistinctOpOfKeys =
      new SimpleRule(OpType.Distinct, ProcessDistinctOpOfKeys);
  /// <summary>
  /// If the DistinctOp includes all all the keys of the input, than it is
  /// unnecessary. Distinct (X, distinct_keys) -> Project( X, distinct_keys)
  /// where distinct_keys includes all keys of X.
  /// </summary>
  /// <param name="context">Rule processing context</param>
  /// <param name="n">current subtree</param>
  /// <param name="newNode">transformed subtree</param>
  /// <returns>transformation status</returns>
  static bool ProcessDistinctOpOfKeys(RuleProcessingContext context, Node n,
                                      out Node newNode) {
    Command command = context.Command;
    ...
    // If we know the keys of the input and the list of distinct keys includes
    // them all, omit the distinct
    if (!nodeInfo.Keys.NoKeys && op.Keys.Subsumes(nodeInfo.Keys.KeyVars)) {
      ProjectOp newOp = command.CreateProjectOp(op.Keys);
