...
#region Constructor
    ...
    internal
    ForeignConstraint(AssociationSet i_fkeySet, EntitySet i_parentTable,
...
#region Externally available Methods
    ...
    internal static List<ForeignConstraint>
    GetForeignConstraints(EntityContainer container) {
      List<ForeignConstraint>foreignKeyConstraints =
          new List<ForeignConstraint>();
      ...
      foreach(EntitySetBase extent in container.BaseEntitySets) {
        ...
        Dictionary<string, EntitySet>endToExtents =
            new Dictionary<string, EntitySet>();
        ...
        foreach(ReferentialConstraint constraint in
                    relationType.ReferentialConstraints) {
          ...
          ForeignConstraint foreignKeyConstraint = new ForeignConstraint(
              relationSet, parentExtent, childExtent, constraint.FromProperties,
    ...
    internal void CheckConstraint(Set<Cell>cells, QueryRewriter childRewriter,
                                  QueryRewriter parentRewriter,
                                  ErrorLog errorLog,
                                  ConfigViewGenerator config) {
      if (IsConstraintRelevantForCells(cells) == false) {
        // if the constraint does not deal with any cell in this group, ignore
        // it
        return;
      }
      ...
      if (childRewriter == null) {
        string message = System.Data.Entity.Strings
        ...
        ErrorLog.Record record = new ErrorLog.Record(
            true, ViewGenErrorCode.ForeignKeyMissingTableMapping, message,
            parentRewriter.UsedCells, String.Empty);
      ...
      if (parentRewriter == null) {
        ...
        ErrorLog.Record record = new ErrorLog.Record(
            true, ViewGenErrorCode.ForeignKeyMissingTableMapping, message,
            childRewriter.UsedCells, String.Empty);
        errorLog.AddEntry(record);
        return;
      }
      ...
      // If we find this, we don't need to check for independent associations.
      // If user maps the Fk to both FK and independent associations, the regular
      // round tripping validation will catch the error.
      if (CheckIfConstraintMappedToForeignKeyAssociation(
              childRewriter, parentRewriter, cells, errorLog)) {
        return;
      }

      // Check if the foreign key in the child table corresponds to the primary
      // key, i.e., if the foreign key (e.g., pid, pid2) is a superset of the
      // actual key members (e.g., pid), it means that the foreign key is also
      // the primary key for this table -- so we can propagate the queries upto
      // C-Space rather than doing the cell check

      int initialErrorLogSize = errorLog.Count;
      if (IsForeignKeySuperSetOfPrimaryKeyInChildTable()) {
        GuaranteeForeignKeyConstraintInCSpace(childRewriter, parentRewriter,
                                              errorLog, config);
      } else {
        GuaranteeMappedRelationshipForForeignKey(childRewriter, parentRewriter,
                                                 cells, errorLog, config);
      }
...
#region Methods(mostly) for Query Containment Check via Keys
    ...
    private void GuaranteeForeignKeyConstraintInCSpace(
      ...
      FragmentQueryProcessor qp = FragmentQueryProcessor.Merge(
          childContext.RightFragmentQP, parentContext.RightFragmentQP);
      bool cImpliesP =
          qp.IsContainedIn(cNode.RightFragmentQuery, pNode.RightFragmentQuery);

      if (false == cImpliesP) {
        // Foreign key constraint not being ensured in C-space
        string childExtents =
            LeftCellWrapper.GetExtentListAsUserString(cNode.GetLeaves());
        string parentExtents =
            LeftCellWrapper.GetExtentListAsUserString(pNode.GetLeaves());
        string message =
            System.Data.Entity.Strings
                .ViewGen_Foreign_Key_Not_Guaranteed_InCSpace(ToUserString());
        // Add all wrappers into allWrappers
        Set<LeftCellWrapper>allWrappers =
            new Set<LeftCellWrapper>(pNode.GetLeaves());
        allWrappers.AddRange(cNode.GetLeaves());
        ErrorLog.Record record = new ErrorLog.Record(
            true, ViewGenErrorCode.ForeignKeyNotGuaranteedInCSpace, message,
            allWrappers, String.Empty);
        errorLog.AddEntry(record);
      }
    }
...
#region Methods for Foreign Keys mapped to association
    ...
    // effects: Ensures that there is a relationship mapped into the C-space for
    // some cell in m_cellGroup. Else adds an error to errorLog
    private void GuaranteeMappedRelationshipForForeignKey(
        QueryRewriter childRewriter, QueryRewriter parentRewriter,
        IEnumerable<Cell>cells, ErrorLog errorLog, ConfigViewGenerator config) {
      ...
      ExtentKey primaryKey =
          ExtentKey.GetPrimaryKeyForEntityType(prefix, ChildTable.ElementType);
      IEnumerable<MemberPath>primaryKeyFields = primaryKey.KeyFields;
      bool foundCell = false;

      bool foundValidParentColumnsForForeignKey =
          false; // we need to find only one, dont error on any one check being
                 // false
      ...
      foreach(Cell cell in cells) {
        ...
        // Check that all the columns of the foreign key and the primary key in
        // the child table are mapped to some property in the C-space

        AssociationEndMember parentEnd =
            GetRelationEndForColumns(cell, ChildColumns);
        if (parentEnd != null &&
            CheckParentColumnsForForeignKey(
                cell, cells, parentEnd,
                ref errorListForInvalidParentColumnsForForeignKey) == false) {
        ...
        AssociationEndMember childEnd =
            GetRelationEndForColumns(cell, primaryKeyFields);
        Debug.Assert(
        ...
        if (childEnd != null && parentEnd != null &&
            FindEntitySetForColumnsMappedToEntityKeys(
                cells, primaryKeyFields) != null) {
          foundCell = true;
          CheckConstraintWhenParentChildMapped(cell, errorLog, parentEnd,
                                               config);
          break; // Done processing for the foreign key - either it was mapped
                 // correctly or it was not
          ...
          EntitySet parentSet =
              MetadataHelper.GetEntitySetAtEnd(assocSet, parentEnd);
          foundCell = CheckConstraintWhenOnlyParentMapped(
              cell, parentSet, assocSet, parentEnd, childRewriter,
              parentRewriter, config);
          if (foundCell) {
            break;
          }
        }
      ...
      if (!foundValidParentColumnsForForeignKey) {
        Debug.Assert(errorListForInvalidParentColumnsForForeignKey != null &&
                     errorListForInvalidParentColumnsForForeignKey.Count > 0);
        foreach(
            var errorRecord in errorListForInvalidParentColumnsForForeignKey) {
      ...
      if (foundCell == false) {
        ...
        IEnumerable<LeftCellWrapper>parentWrappers =
            GetWrappersFromContext(parentContext, ParentTable);
        IEnumerable<LeftCellWrapper>childWrappers =
            GetWrappersFromContext(childContext, ChildTable);
        Set<LeftCellWrapper>bothExtentWrappers =
            new Set<LeftCellWrapper>(parentWrappers);
        bothExtentWrappers.AddRange(childWrappers);
        ErrorLog.Record record = new ErrorLog.Record(
            true, ViewGenErrorCode.ForeignKeyMissingRelationshipMapping,
            message, bothExtentWrappers, String.Empty);
        errorLog.AddEntry(record);
      }
    }
    ...
    private bool CheckIfConstraintMappedToForeignKeyAssociation(
      ...
      // First collect the sets of properties that the principal and dependant
      // ends of this FK are mapped to in the Edm side.
      ...
      // Now Check if the properties on the Edm side are connected via an FK
      // relationship.
      if ((childPropertiesSet.Count != 0) && (parentPropertiesSet.Count != 0)) {
        var foreignKeyAssociations =
            childContext.EntityContainerMapping.EdmEntityContainer
        ...
        foreach(AssociationType association in foreignKeyAssociations) {
          ...
          // mapped from S side are present as dependant properties of this ref
          // constraint on the Edm side. We need to do the same for principal
          // side but we can not enforce equality since the order of the
          // properties participating in the constraint on the S side and C side
          // could be different. This is OK as long as they are mapped
          // appropriately. We also can not use Existence as a sufficient
          // condition since it will allow invalid mapping where FK columns
          // could have been flipped when mapping to the Edm side. So we make
          // sure that the index of the properties in the principal and dependant
          // are same on the Edm side even if they are in different order for ref
          // constraints for Edm and store side.
          var childRefPropertiesCollection =
              childPropertiesSet.Where(it => it.SetEquals(new Set<EdmProperty>(
                                           refConstraint.ToProperties)));
          var parentRefPropertiesCollection =
              parentPropertiesSet.Where(it => it.SetEquals(new Set<EdmProperty>(
                                            refConstraint.FromProperties)));
          if ((childRefPropertiesCollection.Count() != 0 &&
    ...
    // Return a set of integers that represent the indexes of first set of
    // properties in the second set
    private Set<int>
    GetPropertyIndexes(IEnumerable<EdmProperty>properties1,
    ...
    private bool CheckConstraintWhenOnlyParentMapped(
        Cell cell, EntitySet parentSet, AssociationSet assocSet,
        AssociationEndMember endMember, QueryRewriter childRewriter,
        QueryRewriter parentRewriter, ConfigViewGenerator config) {
      ...
      RoleBoolean endRoleBoolean =
          new RoleBoolean(assocSet.AssociationSetEnds[endMember.Name]);
      // use query in pNode as a factory to create a bool expression for the
      // endRoleBoolean
      BoolExpression endCondition =
          pNode.RightFragmentQuery.Condition.Create(endRoleBoolean);
      FragmentQuery cNodeQuery = FragmentQuery.Create(
          pNode.RightFragmentQuery.Attributes, endCondition);
    ...
    private bool
    CheckConstraintWhenParentChildMapped(Cell cell, ErrorLog errorLog,
                                         AssociationEndMember parentEnd,
                                         ConfigViewGenerator config) {
      bool ok = true;
      ...
      if (parentEnd.RelationshipMultiplicity == RelationshipMultiplicity.Many) {
        ...
        string message =
            System.Data.Entity.Strings.ViewGen_Foreign_Key_UpperBound_MustBeOne(
                ToUserString(), cell.CQuery.Extent.Name, parentEnd.Name);
        ErrorLog.Record record = new ErrorLog.Record(
            true, ViewGenErrorCode.ForeignKeyUpperBoundMustBeOne, message, cell,
            String.Empty);
        errorLog.AddEntry(record);
        ok = false;
      }
      ...
      if (MemberPath.AreAllMembersNullable(ChildColumns) == false &&
        ...
        string message =
            System.Data.Entity.Strings.ViewGen_Foreign_Key_LowerBound_MustBeOne(
                ToUserString(), cell.CQuery.Extent.Name, parentEnd.Name);
        ErrorLog.Record record = new ErrorLog.Record(
            true, ViewGenErrorCode.ForeignKeyLowerBoundMustBeOne, message, cell,
            String.Empty);
        errorLog.AddEntry(record);
        ok = false;
      }
      ...
      if (config.IsNormalTracing && ok) {
        Trace.WriteLine("Foreign key mapped to relationship " +
    ...
    private bool
    CheckParentColumnsForForeignKey(Cell cell, IEnumerable<Cell>cells,
                                    AssociationEndMember parentEnd,
                                    ref List<ErrorLog.Record>errorList) {
      // The child columns are mapped to some end of cell.CQuery.Extent.
      // ParentColumns must correspond to the EntitySet for this end
      AssociationSet relationSet = (AssociationSet) cell.CQuery.Extent;
      EntitySet endSet =
          MetadataHelper.GetEntitySetAtEnd(relationSet, parentEnd);

      // Check if the ParentColumns are mapped to endSet's keys

      // Find the entity set that they map to - if any
      EntitySet entitySet =
          FindEntitySetForColumnsMappedToEntityKeys(cells, ParentColumns);
      if (entitySet == null || endSet.Equals(entitySet) == false) {
        ...
        // childColumns are mapped to parentEnd but ParentColumns are not mapped
        // to the end corresponding to the parentEnd -- this is an error
        string message =
            System.Data.Entity.Strings
                .ViewGen_Foreign_Key_ParentTable_NotMappedToEnd(
                    ToUserString(), ChildTable.Name, cell.CQuery.Extent.Name,
                    parentEnd.Name, ParentTable.Name, endSet.Name);
        ErrorLog.Record record = new ErrorLog.Record(
            true, ViewGenErrorCode.ForeignKeyParentTableNotMappedToEnd, message,
            cell, String.Empty);
...
#region Static Helper Methods
    ...
    private static EntitySet FindEntitySetForColumnsMappedToEntityKeys(
      ...
      foreach(Cell cell in cells) {
        ...
        Set<EdmProperty>cSideMembers =
            cell.GetCSlotsForTableColumns(tableColumns);
        ...
        Set<EdmProperty>keyMembers =
            new Set<EdmProperty>(propertyList).MakeReadOnly();
    ...
    // an entityset returns null. If the columns are not mapped in this cell to
    // an end exactly or columns are not projected in cell, returns null
    private static AssociationEndMember
    GetRelationEndForColumns(Cell cell, IEnumerable<MemberPath>columns) {
      ...
      foreach(AssociationSetEnd relationEnd in relationSet.AssociationSetEnds) {
        ...
        // end's keyfields
        List<int>endIndexes =
            cell.CQuery.GetProjectedPositions(primaryKey.KeyFields);
        if (endIndexes != null) {
          // Get all the slots corresponding to the columns
          // But stick to the slots with in these ends since the same column
          // might be
          // projected twice in different ends
          List<int>columnIndexes =
              cell.SQuery.GetProjectedPositions(columns, endIndexes);
          ...
          if (Helpers.IsSetEqual(columnIndexes, endIndexes,
    ...
    // effects: Returns wrappers for extent if there are some available in the
    // context. Else returns an empty enumeration
    private static List<LeftCellWrapper>
    GetWrappersFromContext(ViewgenContext context, EntitySetBase extent) {
      ...
      if (context == null) {
        ...
        Debug.Assert(context.Extent.Equals(extent),
...
#region Regular Helper Methods
    ...
    // in m_cellGroup in the right oder. If not, adds an error to m_errorLog and
    ...
    private bool CheckForeignKeyColumnOrder(Set<Cell>cells, ErrorLog errorLog) {
      ...
      foreach(Cell childCell in childCells) {
        List<List<int>>allChildSlotNums =
            GetSlotNumsForColumns(childCell, ChildColumns);
        ...
        foreach(List<int>childSlotNums in allChildSlotNums) {
          ...
          foreach(Cell parentCell in parentCells) {
            List<List<int>>allParentSlotNums =
                GetSlotNumsForColumns(parentCell, ParentColumns);
            ...
            foreach(List<int>parentSlotNums in allParentSlotNums) {
              ...
              foreach(int parentSlotNum in parentSlotNums) {
                MemberProjectedSlot parentSlot =
                    (MemberProjectedSlot)
              ...
              // or the paths are essentially equivalent via referential
              // constraints We need to check that the last member is
              // essentially the same because it could be a regular scenario
              // where aid is mapped to PersonAddress and Address - there is no
              // ref constraint. So when projected into C-Space, we will get
              // Address.aid and PersonAddress.Address.aid
              if (childPaths.Count == parentPaths.Count) {
                bool notAllPathsMatched = false;
                for (int i = 0; i < childPaths.Count && !notAllPathsMatched;
                     i++) {
                  MemberPath parentPath = parentPaths[i];
                  MemberPath childPath = childPaths[i];

                  if (!parentPath.LeafEdmMember.Equals(
                          childPath.LeafEdmMember)) // Child path did not match
                  {
                    if (parentPath.IsEquivalentViaRefConstraint(childPath)) {
                      // Specifying the referential constraint once in the C
                      // space should be enough. This is the only way possible
                      // today. We might be able to derive more knowledge by
                      // using boolean logic
                      return true;
                    } else {
        ...
        // If execution is at this point, no parent cell's end has matched
        // (otherwise it would have returned true)
        ...
        // using EntityRes. instead of Strings. because the generated method
        // includes 6 instead of 9 parameters
        string message = EntityRes.GetString(
            EntityRes.ViewGen_Foreign_Key_ColumnOrder_Incorrect, ToUserString(),
        ...
        ErrorLog.Record record = new ErrorLog.Record(
            true, ViewGenErrorCode.ForeignKeyColumnOrderIncorrect, message,
            new Cell[]{errorParentCell, childCell}, String.Empty);
        errorLog.AddEntry(record);
        return false;
      }
      Debug.Assert(foundParentCell == true,
    ...
    GetSlotNumsForColumns(Cell cell, IEnumerable<MemberPath>columns) {
      ...
      if (set != null) {
        foreach(AssociationSetEnd setEnd in set.AssociationSetEnds) {
          ...
          List<int>localslotNums =
              cell.SQuery.GetProjectedPositions(columns, endSlots);
...
#region String methods
    internal string ToUserString() {
      string childColsString =
          MemberPath.PropertiesToUserString(m_childColumns, false);
      string parentColsString =
          MemberPath.PropertiesToUserString(m_parentColumns, false);
      string result = System.Data.Entity.Strings.ViewGen_Foreign_Key(
          m_fKeySet.Name, m_childTable.Name, childColsString,
          m_parentTable.Name, parentColsString);
      return result;
    }
