...
namespace MonoTests.System.IO {
  ...
  public class DirectoryInfoTest {
    string TempFolder =
        Path.Combine(Path.GetTempPath(), "MonoTests.System.IO.Tests");
    ...
    [Test] // ctor (String)
        public void
        Constructor1_Path_InvalidPathChars() {
      string path = string.Empty;
      foreach(char c in Path.InvalidPathChars) path += c;
      try {
    ...
    Parent() {
      ...
      try {
        ...
        info = new DirectoryInfo("test");
        Assert.AreEqual(Directory.GetCurrentDirectory(), info.Parent.FullName,
                        "#3");
    ...
    [Test] // bug #75443
      ...
      if (RunningOnUnix) {
        ...
        Assert.AreEqual(Path.Combine(Directory.GetCurrentDirectory(), "test/"),
                        di.FullName, "#D2");
    ...
    [Test]
    public void
    FullName_RootDirectory() {
      DirectoryInfo di =
          new DirectoryInfo(String.Empty + Path.DirectorySeparatorChar);
      if (RunningOnUnix) {
    ...
    [Test] // GetDirectories ()
        public void
        GetDirectories1_DirectoryDoesNotExist() {
      string path = TempFolder + DSC +
                    "DIT.GetDirectoriesDirectoryNotFoundException1.Test";
      DeleteDir(path);
      ...
      try {
        ...
        Assert.AreEqual(typeof (DirectoryNotFoundException), ex.GetType(),
                        "#2");
        Assert.IsNull(ex.InnerException, "#3");
        Assert.IsNotNull(ex.Message, "#4");
    ...
    [Test] // GetDirectories (String)
      ...
      try {
        ...
        Assert.AreEqual(
            6, info.GetDirectories("*", SearchOption.AllDirectories).Length,
            "#9");
      } finally {
        DeleteDir(path);
      }
    ...
    [Test] // GetDirectories (String)
        public void
        GetDirectories2_DirectoryDoesNotExist() {
      string path = TempFolder + DSC +
                    "DIT.GetDirectoriesDirectoryNotFoundException2.Test";
      DeleteDir(path);
    ...
    [Test] // GetDirectories (String)
        public void
        GetDirectories2_SearchPattern_Null() {
      DirectoryInfo info = new DirectoryInfo(TempFolder);
      try {
    ...
    [Test] // GetDirectories (String, SearchOptions)
        public void
        GetDirectiories_SearchOptionAllDirectories() {
      string directoryToBeLookedFor = "lookforme";
      DirectoryInfo baseDir = Directory.CreateDirectory(Path.Combine(
          TempFolder, "GetDirectiories_SearchOptionAllDirectories"));
      DirectoryInfo subdir = baseDir.CreateSubdirectory("subdir");
      DirectoryInfo subsubdir =
          subdir.CreateSubdirectory(directoryToBeLookedFor);
      DirectoryInfo[] directoriesFound = baseDir.GetDirectories(
          directoryToBeLookedFor, SearchOption.AllDirectories);
      Assert.AreEqual(
          1, directoriesFound.Length,
          "There should be exactly one directory with the specified name.");
      Assert.AreEqual(
          directoryToBeLookedFor, directoriesFound[0].Name,
          "The name of the directory found should match the expected one.");
    }

    [Test] // GetDirectories (String, SearchOption)
        public void
        GetDirectories3_SearchPattern_Null() {
      DirectoryInfo info = new DirectoryInfo(TempFolder);
      try {
    ...
    [Test] // GetFiles ()
        public void
        GetFiles1_DirectoryDoesNotExist() {
      string path =
          TempFolder + DSC + "DIT.GetFilesDirectoryNotFoundException1.Test";
      DeleteDir(path);
    ...
    [Test] // GetFiles (String)
        public void
        GetFiles2_DirectoryDoesNotExist() {
      string path =
          TempFolder + DSC + "DIT.GetFilesDirectoryNotFoundException2.Test";
      DeleteDir(path);
    ...
    [Test]
    public void
    GetFileSystemInfos2_SearchPattern_Null() {
      DirectoryInfo info = new DirectoryInfo(TempFolder);
      try {
    ...
    [Test]
    public void
    MoveTo_DestDirName_InvalidPathChars() {
    ...
    [Test]
    public void
    MoveTo_SourceDest_NotDifferent() {
    ...
    DirectoryNameWithSpace() {
      ...
      string dummy = null;
      foreach(FileInfo f in i.GetFiles()) dummy = f.Name;
    }
    ...
    EnumerateFileSystemInfosTest() {
      ...
      foreach(var info in dirInfo.EnumerateFileSystemInfos(
          "*", SearchOption.AllDirectories)) l.Add(info.Name);
    ...
    Serialization() {
      ...
      info = new DirectoryInfo("Test");
      si = new SerializationInfo(typeof (DirectoryInfo),
