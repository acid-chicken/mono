...
namespace MonoTests.System.IO {
  ...
  public class PathTest {
    ...
    bool Mac{get{return OS == OsType.Mac;
  }
}
...
[Test]
public void
ChangeExtension_Extension_InvalidPathChars() {
...
[Test]
public void
ChangeExtension_Path_InvalidPathChars() {
  try {
...
[Test]
public void
Combine_Path1_InvalidPathChars() {
  try {
...
[Test]
public void
Combine_Path2_InvalidPathChars() {
  try {
...
GetDirectoryName() {
  ...
  if (Windows) {
    Assert.AreEqual("C:\\foo", Path.GetDirectoryName("C:\\foo\\foo.txt"),
                    "#B1");
    ...
    Assert.AreEqual("/foo/bar", Path.GetDirectoryName("/foo//bar/dingus"),
                    "#B2");
...
[Test]
public void
GetDirectoryName_Path_InvalidPathChars() {
  try {
...
[Test]
public void
GetDirectoryName_Path_Null() {
  Assert.IsNull(Path.GetDirectoryName(null));
}

[Test]
public void
GetDirectoryName_Path_Whitespace() {
  try {
...
[Test]
public void
GetDirectoryName_Replaces_AltDirectorySeparatorChar() {
  Assert.AreEqual($"foo{DSC}bar",
                  Path.GetDirectoryName($"foo{ADSC}bar{ADSC}dingus"), "#1");
}
...
[Test]
public void
GetExtension_Path_InvalidPathChars() {
  try {
...
[Test]
public void
GetFileName_Path_InvalidPathChars() {
  try {
...
GetFileNameWithoutExtension() {
  ...
  Assert.AreEqual(String.Empty, testFileName,
                  "GetFileNameWithoutExtension #03");
}

[Test]
public void
GetFileNameWithoutExtension_Path_InvalidPathChars() {
  try {
...
GetFullPath_Unix() {
  ...
  string[, ] test = new string[, ]{
      ...
      // No trimming occurs but the paths are not equal. That's why the test
      // fails. Commented out.
      //{"root/..                      /", "root/..                   /"},
      {".//", String.Empty}};
...
GetFullPath_Windows() {
  ...
  string root = "C:\\";
  string[, ] test =
      new string[, ]{{"root////././././././../root/././../root", "root"},
                     {"root/", "root\\"},
                     {"root/./", "root\\"},
                     {"root/./", "root\\"},
                     {"root/../", ""},
  ...
  for (int i = 0; i < test.GetUpperBound(0); i++) {
    ...
    try {
      ...
      Assert.Fail(
          String.Format("GetFullPathWindows UNC #{0} (\"{1}\") failed: {2}", i,
                        root + test[i, 0], ex.GetType()));
    }
  }

  test = new string[, ]{{"root////././././././../root/././../root", "root"},
                        {"root/", "root\\"},
                        {"root/./", "root\\"},
                        {"root/./", "root\\"},
                        {"root/../", ""},
  ...
  for (int i = 0; i < test.GetUpperBound(0); i++) {
    ...
    try {
      ...
      Assert.Fail(
          String.Format("GetFullPathWindows UNC[2] #{0} (\"{1}\") failed: {2}",
                        i, root + test[i, 0], ex.GetType()));
    }
  }

  // These cases require that we don't pass a root to GetFullPath - it should
  // return the proper drive root.
...
[Test]
public void
GetFullPath_Path_EndingSeparator() {
...
[Test]
public void
GetFullPath_Path_InvalidPathChars() {
  try {
...
GetPathRoot2() {
  ...
  if (Windows) {
    ...
    Assert.AreEqual("c:\\", Path.GetPathRoot("c:\\abc\\def"),
                    "GetPathRoot w#02");
    ...
    Assert.AreEqual(String.Empty, Path.GetPathRoot("readme.txt"),
                    "GetPathRoot w#07");
    Assert.AreEqual(String.Empty, Path.GetPathRoot("c"), "GetPathRoot w#08");
    Assert.AreEqual(String.Empty, Path.GetPathRoot("abc\\def"),
                    "GetPathRoot w#09");
    Assert.AreEqual("\\", Path.GetPathRoot("\\abc\\def"), "GetPathRoot w#10");
    Assert.AreEqual("\\\\abc\\def", Path.GetPathRoot("\\\\abc\\def"),
                    "GetPathRoot w#11");
    Assert.AreEqual(String.Empty, Path.GetPathRoot("abc//def"),
                    "GetPathRoot w#12");
...
[Test]
public void
GetPathRoot_Path_InvalidPathChars() {
  try {
...
GetTempPath() {
  ...
  Assert.AreEqual(Path.DirectorySeparatorChar,
                  getTempPath[getTempPath.Length - 1], "GetTempPath #03");
}
...
[Test]
public void
HasExtension_Path_InvalidPathChars() {
  try {
...
// To fix issue https://github.com/mono/mono/issues/18933 we set the
// AltDirectorySeparatorChar as well. This causes the Assert.IsTrue
// (!Path.IsPathRooted ("\\"), "IsPathRooted #09"); to fail.
// Path.IsPathFullyQualified Method handles paths that use both the
// DirectorySeparatorChar and the AltDirectorySeparatorChar characters.
#if !WASM
      Assert.IsTrue(!Path.IsPathRooted("\\"), "IsPathRooted #09");
      Assert.IsTrue(!Path.IsPathRooted("\\\\"), "IsPathRooted #10");
      Assert.IsTrue(!Path.IsPathRooted("z:"), "IsPathRooted #11");
...
[Test]
public void
IsPathRooted_Path_Empty() {
  Assert.IsTrue(!Path.IsPathRooted(String.Empty));
}

[Test]
public void
IsPathRooted_Path_InvalidPathChars() {
  try {
...
[Test]
public void
IsPathRooted_Path_Null() {
  Assert.IsTrue(!Path.IsPathRooted(null));
}

[Test]
public void
IsPathRooted_Path_Whitespace() {
...
WindowsSystem32_77007() {
  ...
  try {
    ...
    Assert.AreEqual(Path.Combine(system, "dir"), Path.GetFullPath(dir),
                    "current dir");
  } finally {
    Directory.SetCurrentDirectory(curdir);
  }
...
WindowsDriveC14N_77058() {
  ...
  Assert.AreEqual(@"C:\Windows\dir",
                  Path.GetFullPath(@"C:\Windows\System32\..\dir"), "1");
  Assert.AreEqual(@"C:\dir", Path.GetFullPath(@"C:\Windows\System32\..\..\dir"),
                  "2");
  Assert.AreEqual(@"C:\dir",
                  Path.GetFullPath(@"C:\Windows\System32\..\..\..\dir"), "3");
  Assert.AreEqual(
      @"C:\dir", Path.GetFullPath(@"C:\Windows\System32\..\..\..\..\dir"), "4");
        Assert.AreEqual (@"C:\dir\", Path.GetFullPath (@"C:\Windows\System32\..\.\..\.\..\dir\"), "5");
}

[Test]
public void
InvalidPathChars_Values() {
...
[Test]
public void
GetInvalidFileNameChars_Values() {
...
[Test]
public void
GetInvalidFileNameChars_Modify() {
...
[Test]
public void
GetInvalidPathChars_Order() {
  if (Windows) {
    char[] invalid = Path.GetInvalidPathChars();
    char[] expected = new char[36]{
        '\x22', '\x3C', '\x3E', '\x7C', '\x00', '\x01', '\x02', '\x03', '\x04',
...
[Test]
public void
GetRandomFileNameIsAlphaNumerical() {
...
Combine_3Params() {
  ...
  Assert.AreEqual(Concat(sep, "one", "two", "three"),
                  Path.Combine("one", "two", "three"), "#A2-1");
  Assert.AreEqual(Concat(sep, sep + "one", "two", "three"),
                  Path.Combine(sep + "one", "two", "three"), "#A2-2");
  Assert.AreEqual(Concat(sep, sep + "one", "two", "three"),
                  Path.Combine(sep + "one" + sep, "two", "three"), "#A2-3");
  ...
  Assert.AreEqual(Concat(sep, sep + "one" + sep, "two", "three"),
                  Path.Combine(sep + "one" + sep + sep, "two", "three"), "#A3");
...
Combine_4Params() {
  ...
  Assert.AreEqual(Concat(sep, "one", "two", "three", "four"),
                  Path.Combine("one", "two", "three", "four"), "#A2-1");
  Assert.AreEqual(Concat(sep, sep + "one", "two", "three", "four"),
                  Path.Combine(sep + "one", "two", "three", "four"), "#A2-2");
  Assert.AreEqual(Concat(sep, sep + "one", "two", "three", "four"),
...
Combine_ManyParams() {
  ...
  Assert.AreEqual(Concat(sep, "one", "two", "three", "four", "five"),
                  Path.Combine("one", "two", "three", "four", "five"), "#A2-1");
  ...
  Assert.AreEqual(
      ...
      Path.Combine(sep + "one" + sep, "two", "three", "four", "five"), "#A2-3");
  Assert.AreEqual(
