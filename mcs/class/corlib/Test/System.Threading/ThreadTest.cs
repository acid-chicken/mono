...
namespace MonoTests.System.Threading {
  ...
  [Category("MobileNotWorking")] // Abort #10240
    ...
    [Category("NotWorking")] // we're not sharing (read-only) CultureInfo
        public void
        CultureInfo_Shared_Across_Threads() {
      Thread t = new Thread(TestCtor1);
      Assert.AreSame(t.CurrentCulture, t.CurrentUICulture, "Culture");

      Assert.AreSame(t.CurrentCulture, CultureInfo.CurrentCulture,
                     "CultureInfo.CurrentCulture");
      Assert.AreSame(t.CurrentUICulture, CultureInfo.CurrentUICulture,
                     "CultureInfo.CurrentUICulture");

      Assert.AreSame(t.CurrentCulture, Thread.CurrentThread.CurrentCulture,
                     "Thread.CurrentThread.CurrentCulture");
      Assert.AreSame(t.CurrentUICulture, Thread.CurrentThread.CurrentUICulture,
    ...
    [Category("NotDotNet")] // it hangs.
        [Category("MultiThreaded")]
        public void
        TestStart(){{C1Test test1 = new C1Test();
  ...
  }
  {
    ...
    Assert.AreEqual(started, test1.run,
...
[Test]
[Category("NotWorking")] // setting the priority of a Thread before it is
                         // started isn't implemented in Mono yet
    public void
    TestPriority1() {
  C2Test test1 = new C2Test();
  Thread TestThread = new Thread(new ThreadStart(test1.TestMethod));
  try {
    ...
    Assert.AreEqual(ThreadPriority.BelowNormal, before,
...
#if MONO_FEATURE_THREAD_ABORT
[Test]
[Category("NotDotNet")] // on MS, Thread is still in AbortRequested state when
                        // Start is invoked
    public void
    AbortUnstarted() {
  C2Test test1 = new C2Test();
...
[Category("NotDotNet")] // on MS, ThreadState is immediately Stopped after Abort
  ...
  Thread TestThread = new Thread(new ThreadStart(test1.TestMethod));
  try {
    Assert.AreEqual(ThreadPriority.Normal, TestThread.Priority,
...
[Category("NotWorking")] // this is a MonoTODO -> no support for Priority
  ...
  try {
    ...
    Assert.AreEqual(ThreadPriority.Lowest, TestThread.Priority,
                    "#45A Incorrect Priority:");
    TestThread.Priority = ThreadPriority.BelowNormal;
    Assert.AreEqual(ThreadPriority.BelowNormal, TestThread.Priority,
    ...
    Assert.AreEqual(ThreadPriority.AboveNormal, TestThread.Priority,
                    "#45D Incorrect Priority:");
    TestThread.Priority = ThreadPriority.Highest;
    Assert.AreEqual(ThreadPriority.Highest, TestThread.Priority,
...
[Category("MultiThreaded")]
public void
TestUndivisibleByPageSizeMaxStackSize() {
...
TestName() {
  ...
  try {
    ...
    Assert.AreEqual(newname, TestThread.Name,
...
[ExpectedException(typeof(ArgumentOutOfRangeException))]
public void
Join_TimeSpan_TooLarge() {
  Thread.CurrentThread.Join(TooLarge);
}

[Test]
public void
Join_TimeSpan_SmallNegative() {
...
TestThreadState() {
  ...
  C2Test test1 = new C2Test();
  Thread TestThread = new Thread(new ThreadStart(test1.TestMethod));
  Assert.AreEqual(ThreadState.Unstarted, TestThread.ThreadState,
                  "#101 Wrong Thread State");
  try {
    TestThread.Start();
    // while(!TestThread.IsAlive); //In the MS Documentation this is not
    // necessary but in the MS SDK it is
    Assert.IsTrue(TestThread.ThreadState == ThreadState.Running ||
                      (TestThread.ThreadState & ThreadState.Unstarted) != 0,
                  "#102 Wrong Thread State: " +
                      TestThread.ThreadState.ToString());
  } finally {
#if MONO_FEATURE_THREAD_ABORT
    TestThread.Abort();
#else
  ...
  Assert.IsTrue((ThreadState.Stopped & TestThread.ThreadState) != 0 ||
                    (ThreadState.Aborted & TestThread.ThreadState) != 0,
                "#103 Wrong Thread State: " +
                    TestThread.ThreadState.ToString());
}
...
public void
CurrentPrincipal_PrincipalPolicy_NoPrincipal() {
  // note: switching from PrincipalPolicy won't work inside the same thread
  // because as soon as a Principal object is created the Policy doesn't matter
  // anymore
...
CurrentPrincipal_PrincipalPolicy_UnauthenticatedPrincipal() {
  ...
  // anymore
  Thread t = new Thread(
      new ThreadStart(ThreadedPrincipalTest.UnauthenticatedPrincipal));
  try {
    t.Start();
    t.Join();
  } catch {
...
[Test]
public void
CurrentPrincipal_PrincipalPolicy_WindowsPrincipal() {
  // note: switching from PrincipalPolicy won't work inside the same thread
  // because as soon as a Principal object is created the Policy doesn't matter
  // anymore
  Thread t =
      new Thread(new ThreadStart(ThreadedPrincipalTest.WindowsPrincipal));
  try {
    t.Start();
    t.Join();
  } catch {
...
[Test]
[Category("NotDotNet")] // On MS, ThreadStateException is thrown on Abort:
                        // "Thread is suspended; attempting to abort"
    [Category("MultiThreaded")]
    public void
    TestSuspendAbort() {
  Thread t = new Thread(new ThreadStart(DoCount));
...
Test_Interrupt() {
  ...
  Assert.IsTrue(interruptedExceptionThrown,
...
public void
TestQueueUserWorkItemNullCallback() {
...
[Test]
public void
GetNamedDataSlotTest() {
  Assert.IsNotNull(Thread.GetNamedDataSlot("te#st"), "#1");
  Assert.AreSame(Thread.GetNamedDataSlot("te#st"),
                 Thread.GetNamedDataSlot("te#st"), "#2");
}
...
[Category("NotDotNet")]
public void
CurrentThread_Domains() {
  AppDomain ad = AppDomain.CreateDomain("foo");
  ad.Load(typeof (DomainClass).Assembly.GetName());
  var o = (DomainClass) ad.CreateInstanceAndUnwrap(
      typeof (DomainClass).Assembly.FullName, typeof (DomainClass).FullName);
  Assert.IsTrue(o.Run());
  AppDomain.Unload(ad);
}
...
void WaitSuspended(string s, Thread t) {
  ...
  while ((state & ThreadState.Suspended) == 0) {
    Assert.IsTrue((state & ThreadState.SuspendRequested) != 0,
...
public class ThreadStateTest {
  void Start(){}
    ...
    Assert.AreEqual(ThreadState.Unstarted | ThreadState.Background,
                    t2.ThreadState, "#B2");
  ...
  [Test] // bug #60031
      [Category("MultiThreaded")]
      public void
      StoppedThreadsThrowThreadStateException() {
    var t = new Thread(() => {});
    t.Start();
    t.Join();

    Assert.Throws<ThreadStateException>(() => { var isb = t.IsBackground; },
                                        "IsBackground getter");
    Assert.Throws<ThreadStateException>(() => { var isb = t.ApartmentState; },
                                        "ApartmentState getter");
    Assert.Throws<ThreadStateException>(
        () => t.ApartmentState = ApartmentState.MTA, "ApartmentState setter");
...
#endif
    Assert.Throws<ThreadStateException>(() => t.GetApartmentState(),
                                        "GetApartmentState ()");
    Assert.Throws<ThreadStateException>(
        () => t.SetApartmentState(ApartmentState.MTA), "SetApartmentState ()");
    Assert.Throws<ThreadStateException>(
...
public class ThreadTest_ManagedThreadId {
  ...
  [Test]
  public void
  ManagedThreadId_AppDomains() {
    AppDomain currentDomain = AppDomain.CurrentDomain;
    ad1 = AppDomain.CreateDomain("AppDomain 1", currentDomain.Evidence,
                                 currentDomain.SetupInformation);
    ad2 = AppDomain.CreateDomain("AppDomain 2", currentDomain.Evidence,
                                 currentDomain.SetupInformation);
    ...
    // execute on AppDomain 1 thread B - must have same ManagedThreadId as Ad 2
    // on thread B
    a.Start();
    a.Join();
    b.Start();
    ...
    // Console.WriteLine ("Done id_a1: {0} id_b1: {1} id_b2: {2} ad_a1: {3}
    // ad_b1: {4} ad_b2: {5}", mbro.id_a1, mbro.id_b1, mbro.id_b2, mbro.ad_a1,
    // mbro.ad_b1, mbro.ad_b2);
    ...
    Assert.AreNotEqual(mbro.id_a1, Thread.CurrentThread.ManagedThreadId,
                       "Id #4");
    Assert.AreNotEqual(mbro.id_b1, Thread.CurrentThread.ManagedThreadId,
                       "Id #5");
    Assert.AreNotEqual(mbro.id_b2, Thread.CurrentThread.ManagedThreadId,
                       "Id #6");
    Assert.AreNotEqual(mbro.ad_a1, AppDomain.CurrentDomain.FriendlyName,
                       "Name #4");
    Assert.AreNotEqual(mbro.ad_b1, AppDomain.CurrentDomain.FriendlyName,
                       "Name #5");
    Assert.AreNotEqual(mbro.ad_b2, AppDomain.CurrentDomain.FriendlyName,
                       "Name #6");
  }
#endif
  void A1() {
...
public class ThreadApartmentTest {
  ...
  TestApartmentState() {
    ...
    Assert.AreEqual(ApartmentState.Unknown, t1.ApartmentState,
                    "Thread1 Default");
    Assert.AreEqual(ApartmentState.Unknown, t2.ApartmentState,
                    "Thread2 Default");
    Assert.AreEqual(ApartmentState.Unknown, t3.ApartmentState,
                    "Thread3 Default");
  ...
  [Test]
  public void
  TestSetApartmentStateSameState() {
    Thread t1 = new Thread(new ThreadStart(Start));
  ...
  [ExpectedException(typeof(InvalidOperationException))]
  public void
  TestSetApartmentStateDiffState() {
    Thread t1 = new Thread(new ThreadStart(Start));
    t1.SetApartmentState(ApartmentState.STA);
    Assert.AreEqual(ApartmentState.STA, t1.ApartmentState, "Thread1 Set Once");
  ...
  [Category("MultiThreaded")]
  public void
  TestTrySetApartmentStateRunning() {
    Thread t1 = new Thread(new ThreadStart(Start));
    t1.SetApartmentState(ApartmentState.STA);
    Assert.AreEqual(ApartmentState.STA, t1.ApartmentState, "#1");
  ...
  GetStackTraces() {
    var m = typeof (Thread).GetMethod(
        "Mono_GetStackTraces", BindingFlags.NonPublic | BindingFlags.Static);
    if (m != null) {
      var res = (Dictionary<Thread, SD.StackTrace>) typeof (Thread)
                    .GetMethod("Mono_GetStackTraces",
                               BindingFlags.NonPublic | BindingFlags.Static)
                    .Invoke(null, null);
      foreach(var t in res.Keys) { var st = res[t].ToString(); }
    }
  }
}
