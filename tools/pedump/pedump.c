...
static MonoAssembly *pedump_preload(MonoAssemblyLoadContext *alc,
                                    MonoAssemblyName *aname,
                                    char **assemblies_path, gboolean refonly,
                                    gpointer user_data, MonoError *error);
static void pedump_assembly_load_hook(MonoAssemblyLoadContext *alc,
                                      MonoAssembly *assembly,
                                      gpointer user_data, MonoError *error);
static MonoAssembly *
pedump_assembly_search_hook(MonoAssemblyLoadContext *alc,
...
static void hex8(const char *label, unsigned char x) {
  printf("\t%s: 0x%02x\n", label, (unsigned char)x);
}

static void hex16(const char *label, guint16 x) {
  printf("\t%s: 0x%04x\n", label, x);
}

static void hex32(const char *label, guint32 x) {
  printf("\t%s: 0x%08x\n", label, x);
}

static void dump_coff_header(MonoCOFFHeader *coff) {
  printf("\nCOFF Header:\n");
...
static void dump_pe_header(MonoPEHeader *pe) {
  printf("\nPE Header:\n");
...
static void dump_nt_header(MonoPEHeaderNT *nt) {
  printf("\nNT Header:\n");
...
static void dent(const char *label, MonoPEDirEntry de) {
  printf("\t%s: 0x%08x [0x%08x]\n", label, de.rva, de.size);
}

static void dump_blob(const char *desc, const char *p, guint32 size) {
  int i;
...
static void dump_public_key(MonoImage *m) {
  guint32 size;
  const char *p;
...
static void dump_strong_name(MonoImage *m) {
  guint32 size;
  const char *p;
...
static void dump_datadir(MonoPEDatadir *dd) {
  printf("\nData directories:\n");
...
static void dump_dotnet_header(MonoDotNetHeader *header) {
...
static void dump_section_table(MonoSectionTable *st) {
...
static void dump_sections(MonoCLIImageInfo *iinfo) {
...
static void dump_cli_header(MonoCLIHeader *ch) {
  printf("\n");
  ...
  printf("                    Flags: %s, %s, %s, %s\n",
         ...
         (ch->ch_flags & CLI_FLAGS_TRACKDEBUGDATA ? "trackdebug"
                                                  : "no-trackdebug"),
         (ch->ch_flags & CLI_FLAGS_STRONGNAMESIGNED ? "strongnamesigned"
                                                    : "notsigned"));
...
static void dsh(const char *label, MonoImage *meta, MonoStreamHeader *sh) {
  printf("%s: 0x%08x - 0x%08x [%d == 0x%08x]\n", label,
         (int)(sh->data - meta->raw_metadata),
         (int)(sh->data + sh->size - meta->raw_metadata), sh->size, sh->size);
}

static void dump_metadata_header(MonoImage *meta) {
  printf("\nMetadata header:\n");
  printf("           Version: %d.%d\n", meta->md_version_major,
         meta->md_version_minor);
  printf("    Version string: %s\n", meta->version);
}

static void dump_metadata_ptrs(MonoImage *meta) {
  printf("\nMetadata pointers:\n");
...
static void dump_metadata(MonoImage *meta) {
  int table;
  ...
  for (table = 0; table < MONO_TABLE_NUM; table++) {
    ...
    printf("Table %s: %d records (%d bytes, at %x)\n",
           mono_meta_table_name(table), meta->tables[table].rows,
           meta->tables[table].row_size,
           (unsigned int)(meta->tables[table].base - meta->raw_data));
  }
}

static void dump_methoddef(MonoImage *metadata, guint32 token) {
  const char *loc;
...
static void dump_dotnet_iinfo(MonoImage *image) {
...
static int dump_verify_info(MonoImage *image, int flags, gboolean valid_only) {
  GSList *errors, *tmp;
  int count = 0, verifiable = 0;
  const char *desc[] = {"Ok", "Error", "Warning",       NULL, "CLS", NULL,
                        NULL, NULL,    "Not Verifiable"};
...
static void usage(void) {
  printf("Usage is: pedump [--verify "
         "error,warn,cls,all,code,fail-on-verifiable,non-strict,valid-only,"
         "metadata] file.exe\n");
  exit(1);
}

static int verify_image_file(const char *fname) {
  ERROR_DECL(error);
  ...
  if (!strstr(fname, "mscorlib.dll")) {
    image = mono_image_open_raw(mono_domain_default_alc(mono_get_root_domain()),
                                fname, &status);
    if (!image) {
    ...
    mono_install_assembly_search_hook_v2(pedump_assembly_search_hook, NULL,
                                         FALSE, FALSE, FALSE);

    mono_init_version("pedump", image->version);

    mono_install_assembly_preload_hook_v2(
        pedump_preload, GUINT_TO_POINTER(FALSE), FALSE, FALSE);
  ...
  for (i = 1; i <= table->rows; ++i) {
    ...
    if (mono_class_has_failure(klass)) {
      ...
      printf("Could not initialize vtable of class(0x%08x) %s.%s due to %s\n",
             token, m_class_get_name_space(klass), m_class_get_name(klass),
             mono_error_get_message(type_load_error));
      mono_error_cleanup(type_load_error);
      ++count;
    }
...
static void thread_state_init(MonoThreadUnwindState *ctx) {}
...
int main(int argc, char *argv[]) {
  ...
  const char *flag_desc[] = {"error",      "warn",       "cls",
                             "all",        "code",       "fail-on-verifiable",
                             "non-strict", "valid-only", "metadata",
                             "partial-md", NULL};
  guint flag_vals[] = {MONO_VERIFY_ERROR,
                       MONO_VERIFY_WARNING,
                       MONO_VERIFY_CLS,
                       MONO_VERIFY_ALL,
                       VERIFY_CODE_ONLY,
                       ...
                       0};
  int i, verify_flags = MONO_VERIFY_REPORT_ALL_ERRORS,
  ...
  // We have to force the runtime to load the corlib under verification as its
  // own corlib so core types are properly populated in mono_defaults.
  ...
  if (verify_pe) {
    ...
    if (!assembly && (image = mono_image_open(file, &status)) &&
        image->tables[MONO_TABLE_ASSEMBLY].rows == 0) {
      assembly = g_new0(MonoAssembly, 1);
      assembly->in_gac = FALSE;
      assembly->image = image;
      image->assembly = assembly;
