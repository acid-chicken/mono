...
static int read_entry(EntryStream *stream, void *data, unsigned char *windex) {
  unsigned char type;
  ssize_t size;
...
#define BEGIN_PROTOCOL_ENTRY5(method, t1, f1, t2, f2, t3, f3, t4, f4, t5, f5)  \
  case PROTOCOL_ID(method):                                                    \
    size = sizeof(PROTOCOL_STRUCT(method));                                    \
    break;
#define BEGIN_PROTOCOL_ENTRY6(method, t1, f1, t2, f2, t3, f3, t4, f4, t5, f5,  \
                              t6, f6)                                          \
  case PROTOCOL_ID(method):                                                    \
    size = sizeof(PROTOCOL_STRUCT(method));                                    \
    break;

#define BEGIN_PROTOCOL_ENTRY_HEAVY0(method) BEGIN_PROTOCOL_ENTRY0(method)
...
#define BEGIN_PROTOCOL_ENTRY_HEAVY5(method, t1, f1, t2, f2, t3, f3, t4, f4,    \
                                    t5, f5)                                    \
  BEGIN_PROTOCOL_ENTRY5(method, t1, f1, t2, f2, t3, f3, t4, f4, t5, f5)
#define BEGIN_PROTOCOL_ENTRY_HEAVY6(method, t1, f1, t2, f2, t3, f3, t4, f4,    \
                                    t5, f5, t6, f6)                            \
...
static gboolean is_always_match(int type) {
  switch (TYPE(type)) {
#define BEGIN_PROTOCOL_ENTRY0(method) case PROTOCOL_ID(method):
#define BEGIN_PROTOCOL_ENTRY1(method, t1, f1) case PROTOCOL_ID(method):
#define BEGIN_PROTOCOL_ENTRY2(method, t1, f1, t2, f2) case PROTOCOL_ID(method):
#define BEGIN_PROTOCOL_ENTRY3(method, t1, f1, t2, f2, t3, f3)                  \
  case PROTOCOL_ID(method):
#define BEGIN_PROTOCOL_ENTRY4(method, t1, f1, t2, f2, t3, f3, t4, f4)          \
  case PROTOCOL_ID(method):
...
#define IS_ALWAYS_MATCH(is_always_match) return is_always_match;
#define MATCH_INDEX(_)
#define IS_VTABLE_MATCH(_)
...
static void print_entry_content(int entries_size, PrintEntry *entries,
...
static int index_color(int index, int num_nums, int *match_indices) {
  int result;
...
static void print_entry(int type, void *data, int num_nums, int *match_indices,
                        gboolean color_output, unsigned char worker_index) {
...
#define DEFAULT_PRINT() print_entry_content(pes_size, pes, color_output);
#define CUSTOM_PRINT(print) print;
...
#define END_PROTOCOL_ENTRY_FLUSH END_PROTOCOL_ENTRY
#define END_PROTOCOL_ENTRY_HEAVY END_PROTOCOL_ENTRY
...
static gboolean matches_interval(mword ptr, mword start, int size) {
 ...
 */
static int match_index(mword ptr, int type, void *data) {
  switch (TYPE(type)) {

#define BEGIN_PROTOCOL_ENTRY0(method) case PROTOCOL_ID(method): {
#define BEGIN_PROTOCOL_ENTRY1(method, t1, f1)                                  \
  case PROTOCOL_ID(method): {                                                  \
    PROTOCOL_STRUCT(method) *entry G_GNUC_UNUSED =                             \
        (PROTOCOL_STRUCT(method) *)data;
#define BEGIN_PROTOCOL_ENTRY2(method, t1, f1, t2, f2)                          \
  case PROTOCOL_ID(method): {                                                  \
    PROTOCOL_STRUCT(method) *entry G_GNUC_UNUSED =                             \
        (PROTOCOL_STRUCT(method) *)data;
...
#define IS_ALWAYS_MATCH(_)
#define MATCH_INDEX(block) return (block);
#define IS_VTABLE_MATCH(_)
...
static gboolean is_vtable_match(mword ptr, int type, void *data) {
  switch (TYPE(type)) {
...
static gboolean sgen_binary_protocol_read_header(EntryStream *stream) {
#ifdef BINPROT_HAS_HEADER
  ...
  if (type == PROTOCOL_ID(binary_protocol_header)) {
    PROTOCOL_STRUCT(binary_protocol_header) *str =
        (PROTOCOL_STRUCT(binary_protocol_header) *)data;
    if (str->check == PROTOCOL_HEADER_CHECK &&
        str->ptr_size == BINPROT_SIZEOF_VOID_P) {
      if (str->version > PROTOCOL_HEADER_VERSION) {
        fprintf(stderr,
                "The file contains a newer version %d. We support up to %d. "
                "Please update.\n",
...
#define GREP_ENTRIES_FUNCTION_NAME                                             \
  CONC(sgen_binary_protocol_grep_entries, CONC(ARCH_SUFFIX, PACKED_SUFFIX))

gboolean
GREP_ENTRIES_FUNCTION_NAME(EntryStream *stream, int num_nums, long nums[],
                           int num_vtables, long vtables[], gboolean dump_all,
                           gboolean pause_times, gboolean color_output,
  ...
  entry_index = 0;
  while ((type = read_entry(stream, data, &worker_index)) !=
         SGEN_PROTOCOL_EOF) {
    ...
    if (pause_times) {
      ...
      case PROTOCOL_ID(binary_protocol_world_stopping): {
        PROTOCOL_STRUCT(binary_protocol_world_stopping) *entry =
            (PROTOCOL_STRUCT(binary_protocol_world_stopping) *)data;
      ...
      case PROTOCOL_ID(binary_protocol_world_restarted): {
        PROTOCOL_STRUCT(binary_protocol_world_restarted) *entry =
            (PROTOCOL_STRUCT(binary_protocol_world_restarted) *)data;
        assert(pause_times_stopped);
        printf("pause-time %d %d %d %lld %lld\n", entry->generation,
               pause_times_concurrent, pause_times_finish,
               entry->timestamp - pause_times_ts, pause_times_ts);
      ...
      match_indices[num_nums] =
          num_nums == 0 ? match_index(0, type, data) : BINARY_PROTOCOL_NO_MATCH;
