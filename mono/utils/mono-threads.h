...
#define MONO_THREADS_PLATFORM_HAS_ATTR_SETSCHED
    ...
    defined(_DEBUG)
// Need more memory on Windows debug build (due to less optimization) to handle
// stack overflows.
...
#define MONO_NATIVE_THREAD_HANDLE_TO_GPOINTER(handle)                          \
  ((gpointer)(gsize)(handle))
#define MONO_GPOINTER_TO_NATIVE_THREAD_HANDLE(handle)                          \
  ((MonoNativeThreadHandle)(gsize)(handle))
...
/*
THREAD_INFO_TYPE is a way to make the mono-threads module parametric - or sort
of. The GC using mono-threads might extend the MonoThreadInfo struct to add its
own data, this avoid a pointer indirection on what is on a lot of hot paths.

But extending MonoThreadInfo has de disavantage that all functions here return
type would require a cast, something like the following:
...
While porting sgen to use mono-threads, the number of casts required was too
much and code ended up looking horrible. So we use this cute little hack. The
idea is that whomever is including this header can set the expected type to be
used by functions here and reduce the number of casts drastically.
...
/* If this is defined, use the signals backed on Mach. Debug only as signals
 * can't be made usable on OSX. */
...
typedef struct _MonoThreadInfo {
  ...
  /* Max stack bounds, all valid addresses must be between [stack_start_limit,
   * stack_end) */
  ...
  /* This memory pool is used by coop GC to save stack data roots between GC
   * unsafe regions */
  GByteArray *stackdata;

  /*In theory, only the posix backend needs this, but having it on mach/win32
   * simplifies things a lot.*/
  MonoThreadUnwindState
      thread_saved_state[2]; // 0 is self suspend, 1 is async suspend.

  /*async call machinery, thread MUST be suspended before accessing those
   * fields*/
  void (*async_target)(void *);
  void *user_data;

  /*
  If true, this thread is running a critical region of code and cannot be
  suspended. A critical session is implicitly started when you call
  mono_thread_info_safe_suspend_sync and is ended when you call either
  mono_thread_info_resume or mono_thread_info_finish_suspend.
  */
  gboolean inside_critical_region;

  /*
   * If TRUE, the thread is in async context. Code can use this information to
   * avoid async-unsafe operations like locking without having to pass an
   * 'async' parameter around.
   */
  ...
  /*
   * This is where we store tools tls data so it follows our lifecycle and
   * doesn't depends on posix tls cleanup ordering
   *
...
typedef struct {
  ...
  The thread must remain operational between this call and
  thread_detach_with_lock. It must be possible to successfully suspend it after
  thread_detach completes.
  */
  ...
  */
  ...
  void (*thread_flags_changing)(MonoThreadInfoFlags old,
                                MonoThreadInfoFlags new_);
  void (*thread_flags_changed)(MonoThreadInfoFlags old,
                               MonoThreadInfoFlags new_);
} MonoThreadInfoCallbacks;

#define MONO_THREAD_INFO_RUNTIME_CALLBACKS(macro, prefix)                      \
  macro(prefix, void, setup_async_callback,                                    \
        (MonoContext * ctx, void (*async_cb)(void *fun), gpointer user_data))  \
      macro(prefix, gboolean, thread_state_init_from_sigctx,                   \
            (MonoThreadUnwindState * state, void *sigctx))                     \
          macro(prefix, gboolean, thread_state_init_from_handle,               \
                (MonoThreadUnwindState * tctx, MonoThreadInfo * info,          \
                 /*optional*/ void *sigctx))                                   \
              macro(prefix, void, thread_state_init,                           \
                    (MonoThreadUnwindState * tctx))
...
typedef SuspendThreadResult (*MonoSuspendThreadCallback)(THREAD_INFO_TYPE *info,
                                                         gpointer user_data);

MONO_API MonoThreadInfoFlags mono_thread_info_get_flags(THREAD_INFO_TYPE *info);
 ...
 * async unsafe.
 */
MONO_API void mono_thread_info_set_flags(MonoThreadInfoFlags flags);

static inline gboolean
mono_threads_filter_exclude_flags(THREAD_INFO_TYPE *info,
...
#define FOREACH_THREAD_ALL(thread)                                             \
  MONO_LLS_FOREACH_FILTERED(mono_thread_info_list_head(), THREAD_INFO_TYPE,    \
                            thread, mono_lls_filter_accept_all, NULL)

#define FOREACH_THREAD_EXCLUDE(thread, not_flags)                              \
  MONO_LLS_FOREACH_FILTERED(mono_thread_info_list_head(), THREAD_INFO_TYPE,    \
                            thread, mono_threads_filter_exclude_flags,         \
                            not_flags)

#define FOREACH_THREAD_END MONO_LLS_FOREACH_END

/* Snapshot iteration; can be done anytime but is slower. */

#define FOREACH_THREAD_SAFE_ALL(thread)                                        \
  MONO_LLS_FOREACH_FILTERED_SAFE(mono_thread_info_list_head(),                 \
                                 THREAD_INFO_TYPE, thread,                     \
                                 mono_lls_filter_accept_all, NULL)

#define FOREACH_THREAD_SAFE_EXCLUDE(thread, not_flags)                         \
  MONO_LLS_FOREACH_FILTERED_SAFE(mono_thread_info_list_head(),                 \
                                 THREAD_INFO_TYPE, thread,                     \
                                 mono_threads_filter_exclude_flags, not_flags)

#define FOREACH_THREAD_SAFE_END MONO_LLS_FOREACH_SAFE_END
...
static inline void mono_thread_info_set_tid(THREAD_INFO_TYPE *info,
                                            MonoNativeThreadId tid) {
  ((MonoThreadInfo *)info)->node.key =
      (uintptr_t)MONO_NATIVE_THREAD_ID_TO_UINT(tid);
}

void mono_thread_info_cleanup(void);

/*
 * @thread_info_size is sizeof (GcThreadInfo), a struct the GC defines to make
 * it possible to have a single block with info from both camps.
 */
void mono_thread_info_init(size_t thread_info_size);
...
MONO_API int mono_thread_info_register_small_id(void);

MONO_API THREAD_INFO_TYPE *mono_thread_info_attach(void);

MONO_API void mono_thread_info_detach(void);

gboolean
mono_thread_info_try_get_internal_thread_gchandle(THREAD_INFO_TYPE *info,
                                                  MonoGCHandle *gchandle);

void mono_thread_info_set_internal_thread_gchandle(THREAD_INFO_TYPE *info,
                                                   MonoGCHandle gchandle);
...
MONO_API gboolean mono_thread_info_set_tools_data(void *data);

MONO_API void *mono_thread_info_get_tools_data(void);

MONO_PROFILER_API THREAD_INFO_TYPE *mono_thread_info_current_unchecked(void);

MONO_API int mono_thread_info_get_small_id(void);

MonoLinkedListSet *mono_thread_info_list_head(void);

THREAD_INFO_TYPE *mono_thread_info_lookup(MonoNativeThreadId id);
...
void mono_thread_info_setup_async_call(THREAD_INFO_TYPE *info,
...
MONO_API gboolean mono_thread_info_yield(void);
...
MONO_PAL_API void
mono_thread_info_install_interrupt(void (*callback)(gpointer data),
                                   gpointer data, gboolean *interrupted);

MONO_PAL_API void mono_thread_info_uninstall_interrupt(gboolean *interrupted);
...
gboolean mono_thread_info_is_interrupt_state(THREAD_INFO_TYPE *info);

void mono_thread_info_describe_interrupt_token(THREAD_INFO_TYPE *info,
                                               GString *text);
...
MonoThreadHandle *mono_threads_open_thread_handle(MonoThreadHandle *handle);

void mono_threads_close_thread_handle(MonoThreadHandle *handle);

MonoNativeThreadHandle
mono_threads_open_native_thread_handle(MonoNativeThreadHandle handle);

void mono_threads_close_native_thread_handle(MonoNativeThreadHandle handle);

#if !defined(HOST_WIN32)

/*Use this instead of pthread_kill */
int mono_threads_pthread_kill(THREAD_INFO_TYPE *info, int signum);
...
/*
This is called very early in the runtime, it cannot access any runtime
facilities.
...
-Register the thread for pending suspend with
mono_threads_add_to_pending_operation_set if needed.

If begin suspend fails the following should be done:
-The thread must be left uninterrupted and resumed.
-Call mono_threads_transition_abort_async_suspend to make sure thread has
correct state. -No pending suspend should be registered with
mono_threads_add_to_pending_operation_set for this operation.
*/
gboolean mono_threads_suspend_begin_async_suspend(THREAD_INFO_TYPE *info,
                                                  gboolean interrupt_kernel);

/*
This verifies the outcome of an async suspend operation.

Some targets, such as posix, verify suspend results assynchronously. Suspend
results must be available (in a non blocking way) after
mono_threads_wait_pending_operations completes.
*/
...
- Register the thread for pending ack with
mono_threads_add_to_pending_operation_set if needed.
*/
...
gboolean mono_thread_platform_create_thread(MonoThreadStart thread_fn,
                                            gpointer thread_data,
                                            gsize *const stack_size,
                                            MonoNativeThreadId *tid);
...
MONO_API MonoNativeThreadId mono_native_thread_id_get(void);

/*
 * This does _not_ return the same value as mono_native_thread_id_get, except on
 * Windows. On POSIX, mono_native_thread_id_get returns the value from
 * pthread_self, which is then passed around as an identifier to other pthread
 * functions. However this function, where possible, returns the OS-unique
 * thread id value, fetched in a platform-specific manner. It will not work with
 * the various pthread functions, should never be used as a MonoNativeThreadId,
 * and is intended solely to match the output of various diagonistic tools.
 */
guint64 mono_native_thread_os_id_get(void);

gint32 mono_native_thread_processor_id_get(void);

MONO_API gboolean mono_native_thread_id_equals(MonoNativeThreadId id1,
                                               MonoNativeThreadId id2);

// FIXMEcxx typedef mono_native_thread_return_t (MONO_STDCALL *
// MonoNativeThreadStart)(void*);
// mono_native_thread_return_t func
// and remove the template

MONO_API gboolean mono_native_thread_create(MonoNativeThreadId *tid,
                                            gpointer func, gpointer arg);

#ifdef __cplusplus
template <typename T>
inline gboolean mono_native_thread_create(MonoNativeThreadId *tid, T func,
                                          gpointer arg) {
  return mono_native_thread_create(tid, (gpointer)func, arg);
}
#endif

MONO_API void mono_native_thread_set_name(MonoNativeThreadId tid,
                                          const char *name);

size_t mono_native_thread_get_name(MonoNativeThreadId tid, char *name_out,
                                   size_t max_len);

MONO_API gboolean mono_native_thread_join(MonoNativeThreadId tid);
...
/*
This tells the suspend initiator that we completed suspend and will now be
waiting for resume.
*/
void mono_threads_notify_initiator_of_suspend(THREAD_INFO_TYPE *info);
/*
This tells the resume initiator that we completed resume duties and will return
to runnable state.
*/
void mono_threads_notify_initiator_of_resume(THREAD_INFO_TYPE *info);

/*
This tells the resume initiator that we completed abort duties and will return
to previous state.
*/
...
typedef enum {
  ...
  DoBlockingPollAndRetry, // async suspend raced blocking and won, pool and
                          // retry
...
MonoRequestSuspendResult
mono_threads_transition_request_suspension(THREAD_INFO_TYPE *info);
...
MonoDoBlockingResult mono_threads_transition_do_blocking(THREAD_INFO_TYPE *info,
                                                         const char *func);
MonoDoneBlockingResult
mono_threads_transition_done_blocking(THREAD_INFO_TYPE *info, const char *func);
MonoAbortBlockingResult
mono_threads_transition_abort_blocking(THREAD_INFO_TYPE *info,
                                       const char *func);
gboolean
mono_threads_transition_peek_blocking_suspend_requested(THREAD_INFO_TYPE *info);
void mono_threads_transition_begin_no_safepoints(THREAD_INFO_TYPE *info,
                                                 const char *func);
void mono_threads_transition_end_no_safepoints(THREAD_INFO_TYPE *info,
                                               const char *func);

G_EXTERN_C // due to THREAD_INFO_TYPE varying
    MonoThreadUnwindState *
    mono_thread_info_get_suspend_state(THREAD_INFO_TYPE *info);

gpointer mono_threads_enter_gc_unsafe_region_cookie(void);
...
G_EXTERN_C // due to THREAD_INFO_TYPE varying
    gboolean
    mono_thread_info_is_running(THREAD_INFO_TYPE *info);
gboolean mono_thread_info_is_live(THREAD_INFO_TYPE *info);
G_EXTERN_C // due to THREAD_INFO_TYPE varying
    int
    mono_thread_info_suspend_count(THREAD_INFO_TYPE *info);
...
G_EXTERN_C // due to THREAD_INFO_TYPE varying
    gboolean
    mono_thread_info_will_not_safepoint(THREAD_INFO_TYPE *info);
...
G_EXTERN_C // due to THREAD_INFO_TYPE varying
    gboolean
    mono_thread_info_in_critical_location(THREAD_INFO_TYPE *info);
G_EXTERN_C // due to THREAD_INFO_TYPE varying
    MonoThreadBeginSuspendResult
    mono_thread_info_begin_suspend(THREAD_INFO_TYPE *info,
                                   MonoThreadSuspendPhase phase);
G_EXTERN_C // due to THREAD_INFO_TYPE varying
    gboolean
    mono_thread_info_begin_resume(THREAD_INFO_TYPE *info);
G_EXTERN_C // due to THREAD_INFO_TYPE varying
    gboolean
    mono_thread_info_begin_pulse_resume_and_request_suspension(
        THREAD_INFO_TYPE *info);

void mono_threads_add_to_pending_operation_set(
    THREAD_INFO_TYPE *info); // XXX rename to something to reflect the fact that
                             // this is used for both suspend and resume
...
gboolean mono_thread_info_is_current(THREAD_INFO_TYPE *info);
...
MonoThreadInfoWaitRet mono_thread_info_wait_one_handle(MonoThreadHandle *handle,
                                                       guint32 timeout,
                                                       gboolean alertable);
...
void mono_win32_enter_alertable_wait(THREAD_INFO_TYPE *info);

void mono_win32_leave_alertable_wait(THREAD_INFO_TYPE *info);

void mono_win32_enter_blocking_io_call(THREAD_INFO_TYPE *info,
                                       HANDLE io_handle);

void mono_win32_leave_blocking_io_call(THREAD_INFO_TYPE *info,
                                       HANDLE io_handle);

void mono_win32_interrupt_wait(PVOID thread_info, HANDLE native_thread_handle,
                               DWORD tid);

void mono_win32_abort_blocking_io_call(THREAD_INFO_TYPE *info);
