...
static void request_interrupt(gpointer thread_info, HANDLE native_thread_handle,
                              gint32 pending_apc_slot, PAPCFUNC apc_callback,
                              DWORD tid) {
  /*
   ...
   * Uncontrolled queuing of APC's could flood the APC queue preventing the
   * target thread to return from its alertable OS wait call, blocking the
   * interrupt/abort requests to complete. This check makes sure that only one
   * APC per type gets queued, preventing potential flooding of the APC queue.
   * NOTE, this code will execute regardless if targeted thread is currently in
   * an alertable wait or not. This is done to prevent races between
   * interrupt/abort requests and alertable wait calls. Threads already in an
   * alertable wait should handle WAIT_IO_COMPLETION return scenarios and
   * restart the alertable wait operation if needed or take other actions (like
   * service the interrupt/abort request).
   */
  ...
  } while (mono_atomic_cas_i32(&info->win32_apc_info, new_apc_info,
                               old_apc_info) != old_apc_info);

  THREADS_INTERRUPT_DEBUG("%06d - Interrupting/Aborting syscall in thread %06d",
                          GetCurrentThreadId(), tid);
  QueueUserAPC(apc_callback, native_thread_handle, (ULONG_PTR)NULL);
}

static void CALLBACK interrupt_apc(ULONG_PTR param) {
  THREADS_INTERRUPT_DEBUG("%06d - interrupt_apc () called",
                          GetCurrentThreadId());
}

void mono_win32_interrupt_wait(PVOID thread_info, HANDLE native_thread_handle,
                               DWORD tid) {
  request_interrupt(thread_info, native_thread_handle,
                    WIN32_APC_INFO_PENDING_INTERRUPT_SLOT, interrupt_apc, tid);
}
...
static void CALLBACK abort_apc(ULONG_PTR param) {
  ...
  if (info) {
    // Check if pending interrupt is still relevant and current thread has not
    // left alertable wait region. NOTE, can only be reset by current thread,
    // currently running this APC.
    gint32 win32_apc_info = mono_atomic_load_i32(&info->win32_apc_info);
    if (win32_apc_info & WIN32_APC_INFO_BLOCKING_IO_SLOT) {
      // Check if current thread registered an IO handle when entering alertable
      // wait (blocking IO call). No need for CAS on win32_apc_info_io_handle
      // since its only loaded/stored by current thread currently running APC.
      ...
      if (io_handle != INVALID_HANDLE_VALUE) {
        ...
        // Start to cancel IO requests for the registered IO handle issued by
        // current thread. NOTE, this is NOT a blocking call.
        CancelIo(io_handle);
      }
    }
  }
...
// .NET Framework and .NET Core currently don't support the ability to abort a
// thread blocked on sync IO calls, see
// https://github.com/dotnet/corefx/issues/5749. Since there is no solution
// covering all scenarios aborting blocking syscall this will be on best effort
// and there might still be a slight risk that the blocking call won't abort
// (depending on overlapped IO support for current file, socket).
static void suspend_abort_syscall(PVOID thread_info,
                                  HANDLE native_thread_handle, DWORD tid) {
  request_interrupt(thread_info, native_thread_handle,
                    WIN32_APC_INFO_PENDING_ABORT_SLOT, abort_apc, tid);
}

static void enter_alertable_wait_ex(MonoThreadInfo *info, HANDLE io_handle) {
  // Only loaded/stored by current thread, here or in APC (also running on
  // current thread).
  ...
  mono_atomic_xchg_i32(&info->win32_apc_info,
                       (io_handle == INVALID_HANDLE_VALUE)
                           ? WIN32_APC_INFO_ALERTABLE_WAIT_SLOT
                           : WIN32_APC_INFO_BLOCKING_IO_SLOT);
}

static void leave_alertable_wait_ex(MonoThreadInfo *info, HANDLE io_handle) {
  // Clear any previous flags. Thread is exiting alertable wait region, and info
  // around pending interrupt/abort APC's can now be discarded, thread is out of
  // wait operation and can proceed execution.
...
gboolean mono_threads_suspend_begin_async_suspend(MonoThreadInfo *info,
  ...
  /* Suspend logic assumes thread is really suspended before continuing below.
   * Surprisingly SuspendThread */
  /* is just an async request to the scheduler, meaning that the suspended
   * thread can continue to run */
  /* user mode code until scheduler gets around and process the request. This
   * will cause a thread state race */
  /* in mono's thread state machine implementation on Windows. By requesting a
   * threads context after issuing a */
  /* suspended request, this will wait until thread is suspended and thread
   * context has been collected */
  /* and returned. */
  CONTEXT context;
  context.ContextFlags =
      CONTEXT_INTEGER | CONTEXT_FLOATING_POINT | CONTEXT_CONTROL;
  ...
  if (!mono_threads_transition_finish_async_suspend(info)) {
    ...
    THREADS_SUSPEND_DEBUG("\tlost race with self suspend %p\n",
                          GUINT_TO_POINTER(id));
    g_assert(mono_threads_is_hybrid_suspension_enabled());
    // XXX interrupt_kernel doesn't make sense in this case as the target is not
    // in a syscall
    return TRUE;
  }
  info->suspend_can_continue =
      mono_threads_get_runtime_callbacks()->thread_state_init_from_handle(
          &info->thread_saved_state[ASYNC_SUSPEND_STATE_INDEX], info, &context);
...
void mono_win32_abort_blocking_io_call(MonoThreadInfo *info) {
#if HAVE_API_SUPPORT_WIN32_CANCEL_SYNCHRONOUS_IO
  // In case thread is blocked on sync IO preventing it from running above
  // queued APC, cancel all outputstanding sync IO for target thread. If its not
  // blocked on a sync IO request, below call will just fail and nothing will be
  // canceled. If thread is waiting on overlapped IO, the queued APC will take
  // care of cancel specific outstanding IO requests.
  gint32 win32_apc_info = mono_atomic_load_i32(&info->win32_apc_info);
  if (win32_apc_info & WIN32_APC_INFO_BLOCKING_IO_SLOT) {
...
gboolean mono_threads_suspend_begin_async_resume(MonoThreadInfo *info) {
  ...
  if (info->async_target) {
    ...
    mono_threads_get_runtime_callbacks()->setup_async_callback(
        &ctx, info->async_target, info->user_data);
...
void mono_threads_suspend_register(MonoThreadInfo *info) {
  g_assert(!info->native_handle);
  info->native_handle =
...
gint mono_threads_suspend_get_suspend_signal(void) { return -1; }

gint mono_threads_suspend_get_restart_signal(void) { return -1; }

gint mono_threads_suspend_get_abort_signal(void) { return -1; }
...
gboolean mono_thread_platform_create_thread(MonoThreadStart thread_fn,
  ...
  result = CreateThread(NULL, set_stack_size, (LPTHREAD_START_ROUTINE)thread_fn,
                        thread_data, 0, &thread_id);
  if (!result)
    return FALSE;
...
gboolean mono_native_thread_id_equals(MonoNativeThreadId id1,
                                      MonoNativeThreadId id2) {
  return id1 == id2;
}

gboolean mono_native_thread_create(MonoNativeThreadId *tid, gpointer func,
                                   gpointer arg) {
  return CreateThread(NULL, MONO_WIN32_DEFAULT_NATIVE_STACK_SIZE,
                      (LPTHREAD_START_ROUTINE)func, arg, 0, tid) != NULL;
}

gboolean mono_native_thread_join_handle(HANDLE thread_handle,
...
void mono_threads_platform_get_stack_bounds(guint8 **staddr, size_t *stsize) {
#if _WIN32_WINNT >= 0x0602 // Windows 8 or newer and very fast, just a few
  ...
  *stsize = (size_t)((NT_TIB *)NtCurrentTeb())->StackBase -
            (size_t)info.AllocationBase;
#endif
}
...
#if SIZEOF_VOID_P == 4 && HAVE_API_SUPPORT_WIN32_IS_WOW64_PROCESS
  LPFN_ISWOW64PROCESS is_wow64_func = (LPFN_ISWOW64PROCESS)GetProcAddress(
...
/*
 * When running x86 process under x64 system syscalls are done through WoW64.
 * This needs to do a transition from x86 mode to x64 so it can syscall into the
 * x64 system. Apparently this transition invalidates the ESP that we would get
 * from calling GetThreadContext, so we would fail to scan parts of the thread
 * stack. We attempt to query whether the thread is in such a transition so we
 * try to restart it later. We check CONTEXT_EXCEPTION_ACTIVE for this, which is
 * highly undocumented.
 */
gboolean mono_threads_platform_in_critical_region(THREAD_INFO_TYPE *info) {
...
#if defined(CONTEXT_EXCEPTION_REQUEST) &&                                      \
    defined(CONTEXT_EXCEPTION_REPORTING) && defined(CONTEXT_EXCEPTION_ACTIVE)
  if (is_wow64 && thread_is_cooperative_suspend_aware(info)) {
    ...
    if (thread_state == STATE_SELF_SUSPENDED ||
        thread_state == STATE_BLOCKING_SELF_SUSPENDED)
      return FALSE;
  }

  if (is_wow64) {
    HANDLE handle =
        OpenThread(THREAD_ALL_ACCESS, FALSE, mono_thread_info_get_tid(info));
    if (handle) {
      CONTEXT context;
...
gboolean mono_threads_platform_yield(void) { return SwitchToThread(); }

void mono_threads_platform_exit(gsize exit_code) { ExitThread(exit_code); }
...
void mono_memory_barrier_process_wide(void) { FlushProcessWriteBuffers(); }
