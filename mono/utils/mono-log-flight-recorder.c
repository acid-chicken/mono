...
static void mono_log_dump_recorder_internal(MonoFlightRecorder *recorder,
                                            LogQueueDumpRequest *req);

static void init(gpointer *out) {
  MonoFlightRecorder *recorder =
      mono_flight_recorder_init(MAX_RECORDER_LOG_LEN, sizeof(LogMessage));
  *out = (gpointer)recorder;
}

static void handle_command(gpointer state, gpointer payload,
...
static void cleanup(gpointer state) {
...
void mono_log_open_recorder(const char *path, void *userData) {
  ...
  logger_thread = mono_utility_thread_launch(sizeof(LogMessage), &callbacks,
 ...
 */
void mono_log_write_recorder(const char *log_domain, GLogLevelFlags level,
                             mono_bool hdr, const char *message) {
  ...
  g_snprintf((gchar *)&entry.message.message, MAX_RECORDER_MSG_LEN, "%s",
             message);
...
#if MONO_PRINT_DROPPED_MESSAGES
    fprintf(stderr,
            "Dropping attempt to close recorder, thread not attached yet\n");
#endif
    return;
  } else if (!logger_thread->run_thread) {
    return;
...
#if MONO_PRINT_DROPPED_MESSAGES
    fprintf(stderr,
            "Dropping attempt to dump recorder, thread not attached yet\n");
#endif
    return;
  }
...
static void mono_log_dump_recorder_internal(MonoFlightRecorder *recorder,
  ...
  while (mono_flight_recorder_iter_next(&diter, &header,
                                        (gpointer *)&req->messages[index++]))
    ;
