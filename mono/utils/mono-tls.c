...
/* Runtime offset detection */
    ...
    !defined(HOST_WIN32) /* __thread likely not tested on mac/win */
...
#define MONO_THREAD_VAR_OFFSET(var, offset)                                    \
  do {                                                                         \
    guint64 foo;                                                               \
    __asm("movq " #var "@GOTTPOFF(%%rip), %0" : "=r"(foo));                    \
    offset = foo;                                                              \
  } while (0)
#else
#define MONO_THREAD_VAR_OFFSET(var, offset)                                    \
...
#define MONO_THREAD_VAR_OFFSET(var, offset)                                    \
  do {                                                                         \
    int tmp;                                                                   \
    __asm("call 1f; 1: popl %0; addl $_GLOBAL_OFFSET_TABLE_+[.-1b], %0; "      \
          "movl " #var "@gotntpoff(%0), %1"                                    \
          : "=r"(tmp), "=r"(offset));                                          \
  } while (0)
#else
#define MONO_THREAD_VAR_OFFSET(var, offset)                                    \
...
#define MONO_THREAD_VAR_OFFSET(var, offset)                                    \
  __asm("     ldr     %0, 1f; b 2f; 1: .word " #var "(tpoff); 2:"              \
        : "=r"(offset))
...
#ifdef MONO_KEYWORD_THREAD
  MONO_THREAD_VAR_OFFSET(mono_tls_sgen_thread_info,
                         mono_tls_offsets[TLS_KEY_SGEN_THREAD_INFO]);
#elif defined(DISABLE_THREADS)
#else
  mono_native_tls_alloc(&mono_tls_key_sgen_thread_info, NULL);
  MONO_THREAD_VAR_OFFSET(mono_tls_key_sgen_thread_info,
                         mono_tls_offsets[TLS_KEY_SGEN_THREAD_INFO]);
#endif
}
...
void mono_tls_init_runtime_keys(void) {
  ...
  MONO_THREAD_VAR_OFFSET(mono_tls_key_jit_tls,
                         mono_tls_offsets[TLS_KEY_JIT_TLS]);
  ...
  MONO_THREAD_VAR_OFFSET(mono_tls_key_lmf_addr,
                         mono_tls_offsets[TLS_KEY_LMF_ADDR]);
#endif
}
