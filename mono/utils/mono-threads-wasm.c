...
static int wasm_get_stack_base(void) {
  // Return the bottom limit of the stack
  return EM_ASM_INT({ return STACK_MAX; });
}

EMSCRIPTEN_KEEPALIVE
static int wasm_get_stack_size(void) {
  return EM_ASM_INT({ return TOTAL_STACK; });
}
...
gint32 mono_native_thread_processor_id_get(void) { return -1; }
...
gboolean mono_threads_platform_yield(void) { return TRUE; }
...
void mono_threads_platform_get_stack_bounds(guint8 **staddr, size_t *stsize) {
  ...
  *stsize = (size_t)-1;
  ...
  if (G_UNLIKELY(res != 0))
    g_error("%s: pthread_attr_init failed with \"%s\" (%d)", __func__,
            g_strerror(res), res);

  res = pthread_getattr_np(pthread_self(), &attr);
  if (G_UNLIKELY(res != 0))
    g_error("%s: pthread_getattr_np failed with \"%s\" (%d)", __func__,
            g_strerror(res), res);

  res = pthread_attr_getstack(&attr, (void **)staddr, stsize);
  if (G_UNLIKELY(res != 0))
    g_error("%s: pthread_attr_getstack failed with \"%s\" (%d)", __func__,
            g_strerror(res), res);

  res = pthread_attr_destroy(&attr);
  if (G_UNLIKELY(res != 0))
    g_error("%s: pthread_attr_destroy failed with \"%s\" (%d)", __func__,
            g_strerror(res), res);
...
gboolean mono_thread_platform_create_thread(MonoThreadStart thread_fn,
  ...
  res = pthread_create(&thread, &attr, (gpointer(*)(gpointer))thread_fn,
                       thread_data);
  if (res) {
    res = pthread_attr_destroy(&attr);
    if (res != 0)
