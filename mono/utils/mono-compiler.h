...
// Quiet Visual Studio linker warning, LNK4221: This object file does not define
// any previously undefined public symbols, so it will not be used by any link
// operation that consumes this library. And other linkers, e.g. older Apple.
#define MONO_EMPTY_SOURCE_FILE(x)                                              \
  extern const char mono_quash_linker_empty_file_warning_##x;                  \
...
#define MONO_RESTORE_WARNING MONO_PRAGMA_WARNING_POP()
#else
...
/* Used to tell Clang's ThreadSanitizer to not report data races that occur
...
/* Used to tell Clang's AddressSanitizer to turn off instrumentation for a
 * certain function */
...
/* Unified headers before API 21 do not declare mmap when LARGE_FILES are used
 * (via -D_FILE_OFFSET_BITS=64) which is always the case when Mono build targets
 * Android. The problem here is that the unified headers map `mmap` to `mmap64`
 * if large files are enabled but this api exists only in API21 onwards.
 * Therefore we must carefully declare the 32-bit mmap here without changing the
 * ABI along the way. Carefully because in this instance off_t is redeclared to
 * be 64-bit and that's not what we want.
 */
...
/* Similar thing as with mmap happens with sendfile, except that the off_t is
 * always used (and mono expects 64-bit offset here */
