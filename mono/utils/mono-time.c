...
gint64 mono_msec_ticks(void) { return mono_100ns_ticks() / 10 / 1000; }
...
/* Returns the number of 100ns ticks from unspecified time: this should be
 * monotonic */
 ...
 */
/* Adapted from CoreCLR:
 * https://github.com/dotnet/coreclr/blob/66d2738ea96fcce753dec1370e79a0c78f7b6adb/src/pal/src/misc/time.cpp
 */
gint64 mono_msec_boottime(void) {
  /* clock_gettime () is found by configure on Apple builds, but its only
   * present from ios 10, macos 10.12, tvos 10 and watchos 3 */
#if !defined(TARGET_WASM) &&                                                   \
    ((defined(HAVE_CLOCK_MONOTONIC_COARSE) ||                                  \
      defined(HAVE_CLOCK_MONOTONIC)) &&                                        \
     !(defined(TARGET_IOS) || defined(TARGET_OSX) ||                           \
       defined(TARGET_WATCHOS) || defined(TARGET_TVOS)))
  clockid_t clockType =
  ...
  if (clock_gettime(clockType, &ts) != 0) {
    g_error("clock_gettime(CLOCK_MONOTONIC*) failed; errno is %d", errno,
            strerror(errno));
    return 0;
  }
  return (ts.tv_sec * tccSecondsToMillieSeconds) +
  ...
  if (!timebase_inited) {
    ...
    // This barrier prevents reading s_TimebaseInfo before reading
    // timebase_inited.
    mono_memory_barrier();
  }
  return (mach_absolute_time() * s_TimebaseInfo.numer / s_TimebaseInfo.denom) /
  ...
  if (time_base_to_time(&tb, TIMEBASE_SZ) != 0) {
    g_error("time_base_to_time() failed; errno is %d (%s)", errno,
            strerror(errno));
    return 0;
  }
  return (tb.tb_high * tccSecondsToMillieSeconds) +
  ...
  return (tv.tv_sec * tccSecondsToMillieSeconds) +
