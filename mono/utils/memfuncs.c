...
/*
 * SGen cannot deal with invalid pointers on the heap or in registered roots.
 * Sometimes we need to copy or zero out memory in code that might be
 * interrupted by collections.  To guarantee that those operations will not
 * result in invalid pointers, we must do it word-atomically.
 *
 * libc's bzero() and memcpy()/memmove() functions do not guarantee
 * word-atomicity, even in cases where one would assume so.  For instance, some
 * implementations (like Darwin's on x86) have variants of memcpy() using vector
 * instructions.  Those may copy bytewise for the region preceding the first
 * vector-aligned address.  That region could be word-aligned, but it would
 * still be copied byte-wise.
 *
 * All our memory writes here are to "volatile" locations.  This is so that C
 * compilers don't "optimize" our code back to calls to bzero()/memmove(). LLVM,
...
#define MEMMOVE_WORDS_UPWARD(dest, src, words)                                 \
  do {                                                                         \
    void *volatile *__d = (void *volatile *)(dest);                            \
    void **__s = (void **)(src);                                               \
    size_t __n = (words);                                                      \
...
#define MEMMOVE_WORDS_DOWNWARD(dest, src, words)                               \
  do {                                                                         \
    void *volatile *__d = (void *volatile *)(dest);                            \
    void **__s = (void **)(src);                                               \
    size_t __n = (words);                                                      \
...
void mono_gc_memmove_aligned(void *dest, const void *src, size_t size) {
  ...
  */
   ...
   * Pointers to managed objects must always be stored in word aligned
   * addresses, so even if dest is misaligned, src will be by the same amount -
   * this ensure proper atomicity of reads.
   *
   * We don't need to case when source and destination have different alignments
   * since we only do word stores using memmove, which must handle it.
   */
  if (dest > src &&
      ((size_t)((char *)dest - (char *)src) < size)) { /*backward copy*/
...
guint64 mono_determine_physical_ram_size(void) {
  ...
  /* sysconf works on most *NIX operating systems, if your system doesn't have
   * it or if it
  ...
  if (!page_size || !num_pages) {
    g_warning("Your operating system's sysconf (3) function doesn't correctly "
              "report physical memory size!");
    return 134217728;
  }
