...
#if !defined(__APPLE__) // returning virtual addresses <4G requires entitlement
                        // on Apple platforms, do not use it
#ifndef MAP_32BIT
#define MAP_32BIT 0
#endif
#endif
...
void *mono_malloc_shared_area(int pid) {
...
char *mono_aligned_address(char *mem, size_t size, size_t alignment) {
  char *aligned = (char *)((size_t)(mem + (alignment - 1)) & ~(alignment - 1));
  g_assert(aligned >= mem && aligned + size <= mem + size + alignment &&
           !((size_t)aligned & (alignment - 1)));
  return aligned;
}
...
void mono_account_mem(MonoMemAccountType type, ssize_t size) {
  mono_atomic_fetch_add_word(&allocation_count[type], size);
  mono_atomic_fetch_add_word(&total_allocation_count, size);
}

void mono_valloc_set_limit(size_t size) { alloc_limit = size; }

gboolean mono_valloc_can_alloc(size_t size) {
  if (alloc_limit)
    return (total_allocation_count + size) < alloc_limit;
  return TRUE;
}

const char *mono_mem_account_type_name(MonoMemAccountType type) {
  static const char *names[] = {"code",
                                "hazard pointers",
                                "domain",
                                "SGen internal",
                                "SGen nursery",
...
void mono_mem_account_register_counters(void) {
  for (int i = 0; i < MONO_MEM_ACCOUNT_MAX; ++i) {
    ...
    mono_counters_register(descr,
                           MONO_COUNTER_WORD | MONO_COUNTER_RUNTIME |
                               MONO_COUNTER_BYTES | MONO_COUNTER_VARIABLE,
                           (void *)&allocation_count[i]);
  }
}
...
int mono_valloc_granule(void) { return mono_pagesize(); }

static int prot_from_flags(int flags) {
  int prot = PROT_NONE;
...
static guint32 get_darwin_version(void) {
...
/**
 * mono_setmmapjit:
 * \param flag indicating whether to enable or disable the use of MAP_JIT in
 * mmap
 *
 ...
 * as an App in MacOS
 */
void mono_setmmapjit(int flag) { use_mmap_jit = flag; }
...
void *mono_valloc(void *addr, size_t length, int flags,
  ...
  if (get_darwin_version() >= DARWIN_VERSION_MOJAVE) {
    ...
    if (is_hardened_runtime == 0 && !use_mmap_jit) {
      ptr = mmap(NULL, getpagesize(), PROT_WRITE | PROT_EXEC,
...
void *mono_file_map_error(size_t length, int flags, int fd, guint64 offset,
                          void **ret_handle, const char *filepath,
                          char **error_message) {
  void *ptr;
  int mflags = 0;
  int prot = prot_from_flags(flags);
  ...
  if (length == 0)
    /* emscripten throws an exception on 0 length */
    *error_message = g_stdrup_printf(
        "%s failed file:%s length:0x%" G_GSIZE_FORMAT "x offset:0x%" PRIu64
        "X error:%s\n",
        __func__, filepath ? filepath : "", length, (guint64)offset,
  ...
  if (ptr == MAP_FAILED) {
    if (error_message) {
      *error_message =
          g_strdup_printf("%s failed file:%s length:0x%" G_GSIZE_FORMAT
                          "X offset:0x%" PRIu64 "X error:%s(0x%X)\n",
                          __func__, filepath ? filepath : "", length,
                          (guint64)offset, g_strerror(errno), errno);
    }
    return NULL;
  }
...
int mono_file_unmap(void *addr, void *handle) {
  ...
  // No GC safe transition because this is called early in driver.c via
  // mono_debug_init (with a few layers of indirection)
  BEGIN_CRITICAL_SECTION;
...
int mono_mprotect(void *addr, size_t length, int flags) {
  ...
  if (flags & MONO_MMAP_DISCARD) {
    /* on non-linux the pages are not guaranteed to be zeroed (*bsd, osx at
     * least) */
#ifdef __linux__
  ...
  // No GC safe transition because this is called early in mini_init via
  // mono_arch_init (with a few layers of indirection)
...
int mono_pagesize(void) { return 4096; }
...
void *mono_valloc_aligned(size_t size, size_t alignment, int flags,
                          MonoMemAccountType type) {
  void *res = NULL;
...
static gboolean shared_area_disabled(void) {
  if (!use_shared_area) {
...
static int mono_shared_area_instances_slow(void **array, int count,
                                           gboolean cleanup) {
  int i, j = 0;
  int num;
  void *data;
...
static int mono_shared_area_instances_helper(void **array, int count,
                                             gboolean cleanup) {
  const char *name;
  int i = 0;
  int curpid = getpid();
  ...
  while ((name = g_dir_read_name(dir))) {
    ...
    if (curpid != pid && kill(pid, 0) == -1 &&
        (errno == ESRCH || errno == ENOMEM)) {
      char buf[128];
...
void *mono_shared_area_for_pid(void *pid) { return NULL; }

void mono_shared_area_unload(void *area) {}

int mono_shared_area_instances(void **array, int count) { return 0; }
