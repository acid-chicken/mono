...
typedef union _MonoErrorInternal {
  ...
  struct {
    ...
    /*These name are suggestions of their content. MonoError internals might use
     * them for something else.*/
    // type_name must be right after error_code and flags, see
    // mono_error_init_deferred.
...
#define ERROR_LOCAL_BEGIN(local, parent, skip_overwrite)                       \
  do {                                                                         \
    MonoError local;                                                           \
...
#define return_val_if_nok(error, val)                                          \
  do {                                                                         \
    if (!is_ok((error)))                                                       \
      return (val);                                                            \
  } while (0)
 ...
 * 1. mono_error_assert_ok(e) when you just want to print the error's message on
 * failure
 * 2. mono_error_assert_ok(e,msg) when you want to print "msg, due to <e's
 * message>"
 * 3. mono_error_assertf_ok(e,fmt,args...) when you want to print "<formatted
 * msg>, due to <e's message>" (fmt should specify the formatting just for
 * args).
 *
 ...
 */
#define mono_error_assert_ok(error)                                            \
  g_assertf(is_ok(error), "%s", mono_error_get_message(error))
#define mono_error_assert_msg_ok(error, msg)                                   \
  g_assertf(is_ok(error), msg ", due to %s", mono_error_get_message(error))
#define mono_error_assertf_ok(error, fmt, ...)                                 \
  g_assertf(is_ok(error), fmt ", due to %s", __VA_ARGS__,                      \
            mono_error_get_message(error))

void mono_error_dup_strings(MonoError *error, gboolean dup_strings);

/* This function is not very useful as you can't provide any details beyond the
 * message.*/
void mono_error_set_error(MonoError *error, int error_code,
                          const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(3, 4);

void mono_error_set_type_load_class(MonoError *error, MonoClass *klass,
                                    const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(3, 4);

void mono_error_vset_type_load_class(MonoError *error, MonoClass *klass,
                                     const char *msg_format, va_list args);

void mono_error_set_type_load_name(MonoError *error, const char *type_name,
                                   const char *assembly_name,
                                   const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(4, 5);

void mono_error_set_out_of_memory(MonoError *error, const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(2, 3);

void mono_error_set_argument_format(MonoError *error, const char *argument,
                                    const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(3, 4);

void mono_error_set_argument(MonoError *error, const char *argument,
                             const char *msg);

void mono_error_set_argument_null(MonoError *oerror, const char *argument,
                                  const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(3, 4);

void mono_error_set_argument_out_of_range(MonoError *error, const char *name,
                                          const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(3, 4);

void mono_error_set_not_verifiable(MonoError *oerror, MonoMethod *method,
                                   const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(3, 4);

void mono_error_set_generic_error(MonoError *error, const char *name_space,
                                  const char *name, const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(4, 5);

void mono_error_set_execution_engine(MonoError *error, const char *msg_format,
                                     ...) MONO_ATTR_FORMAT_PRINTF(2, 3);

void mono_error_set_not_implemented(MonoError *error, const char *msg_format,
                                    ...) MONO_ATTR_FORMAT_PRINTF(2, 3);

void mono_error_set_not_supported(MonoError *error, const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(2, 3);

void mono_error_set_ambiguous_implementation(MonoError *error,
                                             const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(2, 3);

void mono_error_set_invalid_operation(MonoError *error, const char *msg_format,
                                      ...) MONO_ATTR_FORMAT_PRINTF(2, 3);

void mono_error_set_exception_instance(MonoError *error, MonoException *exc);

void mono_error_set_invalid_program(MonoError *oerror, const char *msg_format,
                                    ...) MONO_ATTR_FORMAT_PRINTF(2, 3);

void mono_error_set_member_access(MonoError *error, const char *msg_format, ...)
    MONO_ATTR_FORMAT_PRINTF(2, 3);

void mono_error_set_invalid_cast(MonoError *oerror);

static inline void mono_error_set_remoting(MonoError *error,
                                           const char *message) {
  mono_error_set_generic_error(error, "System.Runtime.Remoting",
                               "RemotingException", "%s", message);
}

static inline void mono_error_set_divide_by_zero(MonoError *error) {
  mono_error_set_generic_error(error, "System", "DivideByZeroException", NULL);
}

static inline void mono_error_set_index_out_of_range(MonoError *error) {
  mono_error_set_generic_error(error, "System", "IndexOutOfRangeException",
                               NULL);
}

static inline void mono_error_set_overflow(MonoError *error) {
  mono_error_set_generic_error(error, "System", "OverflowException", NULL);
}

static inline void mono_error_set_synchronization_lock(MonoError *error,
                                                       const char *message) {
  mono_error_set_generic_error(error, "System.Threading",
                               "SynchronizationLockException", "%s", message);
}

static inline void mono_error_set_thread_interrupted(MonoError *error) {
  mono_error_set_generic_error(error, "System.Threading",
                               "ThreadInterruptedException", NULL);
}

static inline void mono_error_set_null_reference(MonoError *error) {
  mono_error_set_generic_error(error, "System", "NullReferenceException", NULL);
}

static inline void mono_error_set_duplicate_wait_object(MonoError *error) {
  mono_error_set_generic_error(error, "System", "DuplicateWaitObjectException",
                               "Duplicate objects in argument.");
}

static inline void mono_error_set_cannot_unload_appdomain(MonoError *error,
                                                          const char *message) {
  mono_error_set_generic_error(error, "System",
                               "CannotUnloadAppDomainException", "%s", message);
}

MonoException *mono_error_prepare_exception(MonoError *error,
                                            MonoError *error_out);

MonoException *mono_error_convert_to_exception(MonoError *error);

void mono_error_move(MonoError *dest, MonoError *src);

MonoErrorBoxed *mono_error_box(const MonoError *error, MonoImage *image);

gboolean mono_error_set_from_boxed(MonoError *error,
                                   const MonoErrorBoxed *from);

const char *mono_error_get_exception_name(MonoError *oerror);

void mono_error_set_specific(MonoError *error, int error_code,
                             const char *missing_method);

void mono_error_set_first_argument(MonoError *oerror,
                                   const char *first_argument);
...
static inline unsigned long __stdcall GetLastError(void) {
#if HOST_X86
...
//
static inline void __stdcall mono_SetLastError(unsigned long err) {
#if HOST_X86
