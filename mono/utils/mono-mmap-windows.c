...
int mono_mmap_win_prot_from_flags(int flags) {
  ...
  case MONO_MMAP_READ:
    prot = PAGE_READONLY;
    break;
  case MONO_MMAP_READ | MONO_MMAP_EXEC:
    prot = PAGE_EXECUTE_READ;
    break;
  case MONO_MMAP_READ | MONO_MMAP_WRITE:
    prot = PAGE_READWRITE;
    break;
  case MONO_MMAP_READ | MONO_MMAP_WRITE | MONO_MMAP_EXEC:
    prot = PAGE_EXECUTE_READWRITE;
    break;
  case MONO_MMAP_WRITE:
    prot = PAGE_READWRITE;
    break;
  case MONO_MMAP_WRITE | MONO_MMAP_EXEC:
    prot = PAGE_EXECUTE_READWRITE;
    break;
  case MONO_MMAP_EXEC:
    prot = PAGE_EXECUTE;
 ...
 * as an App in MacOS
 */
void mono_setmmapjit(int flag) { /* Ignored on HOST_WIN32 */ }
...
static void remove_trailing_whitespace_utf16(wchar_t *s) {
...
void *mono_file_map_error(size_t length, int flags, int fd, guint64 offset,
  ...
  mapping = CreateFileMappingW(file, NULL, prot, (DWORD)(mapping_length >> 32),
                               (DWORD)mapping_length, NULL);
  if (mapping == NULL)
    goto exit;

  failed_function = "MapViewOfFile";
  ptr = MapViewOfFile(mapping, mflags, (DWORD)(offset >> 32), (DWORD)offset,
                      length);
  if (ptr == NULL)
    goto exit;
...
exit:
  if (!ptr && (mapping || error_message)) {
    ...
    if (mapping)
      CloseHandle(mapping);
    if (error_message) {
      WCHAR win32_error_string[100] = {
          0}; // FIXME LocalFree not initially in UWP but it is now.
      ...
      *error_message =
          g_strdup_printf("%s failed file:%s length:0x%IX offset:0x%" PRIX64
                          " function:%s error:%ls(0x%X)\n",
                          __func__, filepath ? filepath : "", length, offset,
                          failed_function, win32_error_string, win32_error);
    }
