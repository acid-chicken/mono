...
static const char *kernel_version_string(void) {
...
static gboolean starts_with(const char *pre, size_t pre_sz, const char *str) {
  return strncmp(pre, str, pre_sz) == 0;
}

static const char *macos_version_string(void) {
  ...
  if (!version_string) {
    ...
    if (sysctlbyname("kern.osproductversion", NULL, &size, NULL, 0) < 0 ||
        size >= buf_size) {
      /* if we couldn't get the size or if it needs more space that we have in
       * buf, leave it empty */
      version_string = "";
      return version_string;
    }
...
typedef struct {
  ...
  MerpArch archArg; // Arch, MacOS only, bails out if not found also required
                    // for bucketization. (required)
  MERPExcType exceptionArg; // Exception type (refer to merpcommon.h and
                            // mach/exception_types.h for more info (optional)

  const char *serviceNameArg; // This is the Bootstrap service name that MERP
                              // GUI will create to receive mach_task_self on a
                              // port created. Bails out if MERP GUI fails to
                              // receive mach_task_self from the crashed app.
                              // (Required for crash log generation)
  const char *servicePathArg; // The path to the executable, used to relaunch
                              // the crashed app.
...
static const char *get_merp_bitness(MerpArch arch) {
  switch (arch) {
  case MerpArchx86_64:
    return "x64";
  case MerpArchx86:
...
static MerpArch get_merp_arch(void) {
#ifdef TARGET_X86
  return MerpArchx86;
...
static const char *get_merp_exctype(MERPExcType exc) {
  switch (exc) {
...
static MERPExcType parse_exception_type(const char *signal) {
...
static gboolean mono_merp_write_params(MERPStruct *merp) {
  int handle = g_open(merp->merpFilePath, O_TRUNC | O_WRONLY | O_CREAT,
...
static gboolean mono_merp_send(MERPStruct *merp) {
...
static void get_apple_model(char *buffer, size_t max_length) {
  size_t sz = 0;
...
static gboolean mono_merp_write_fingerprint_payload(const char *non_param_data,
                                                    const MERPStruct *merp) {
  int handle = g_open(merp->crashLogPath, O_TRUNC | O_WRONLY | O_CREAT,
                      merp_file_permissions);
  g_assertf(handle != -1, "Could not open crash log file at %s",
            merp->crashLogPath);
...
static gboolean mono_write_wer_template(MERPStruct *merp) {
  // Note about missing ProcessInformation block: we have no PID that makes
  // sense and when mono is embedded and used to run functions without an entry
  // point, there is no image that would make any semantic sense to send either.
  // It's a nuanced problem, each way we can run mono would need a separate fix.

  int handle = g_open(merp->werXmlPath, O_WRONLY | O_CREAT | O_TRUNC,
                      merp_file_permissions);
  g_assertf(handle != -1, "Could not open WER XML file at %s",
            merp->werXmlPath);
  ...
  i++;
  g_async_safe_fprintf(handle, "<Parameter%d>%s</Parameter%d>\n", i,
                       get_merp_bitness(merp->archArg), i);
  i++;
  g_async_safe_fprintf(handle, "<Parameter%d>%s</Parameter%d>\n", i,
                       merp->serviceNameArg, i);
  i++;
  g_async_safe_fprintf(handle, "<Parameter%d>%s</Parameter%d>\n", i,
                       merp->moduleName, i);
  i++;
  g_async_safe_fprintf(handle, "<Parameter%d>%s</Parameter%d>\n", i,
                       merp->moduleVersion, i);
  i++;
  g_async_safe_fprintf(handle,
                       "<Parameter%d>0x%" G_GSIZE_FORMAT "x</Parameter%d>\n", i,
                       merp->moduleOffset, i);
  i++;
  g_async_safe_fprintf(handle, "<Parameter%d>%s</Parameter%d>\n", i,
                       get_merp_exctype(merp->exceptionArg), i);
  i++;
  g_async_safe_fprintf(handle, "<Parameter%d>0x%" PRIx64 "</Parameter%d>\n", i,
                       merp->hashes.offset_free_hash, i);
  i++;
  g_async_safe_fprintf(handle, "<Parameter%d>0x%" PRIx64 "</Parameter%d>\n", i,
                       merp->hashes.offset_rich_hash, i);
  i++;
  g_async_safe_fprintf(handle, "<Parameter%d>%s</Parameter%d>\n", i,
                       merp->osVersion, i);
  i++;
...
// Returns success
gboolean mono_merp_invoke(const intptr_t crashed_pid, const char *signal,
                          const char *non_param_data, MonoStackHash *hashes) {
  MonoStateMem mem;
  int merp_tmp_file_tag = 2;
  gboolean alloc_success =
      mono_state_alloc_mem(&mem, merp_tmp_file_tag, sizeof(MERPStruct));
  if (!alloc_success)
    return FALSE;
...
void mono_merp_add_annotation(const char *key, const char *value) {
...
void mono_merp_enable(const char *appBundleID, const char *appSignature,
                      const char *appVersion, const char *merpGUIPath,
                      const char *appPath, const char *configDir) {
  mono_memory_barrier();
  ...
  config.merpFilePath =
      g_strdup_printf("%s%s", prefix, "MERP.uploadparams.txt");
...
gboolean mono_merp_enabled(void) { return config.enable_merp; }
