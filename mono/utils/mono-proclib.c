...
/**
 * mono_process_list:
 * \param size a pointer to a location where the size of the returned array is
 * stored \returns an array of pid values for the processes currently running on
 * the system. The size of the array is stored in \p size.
 */
...
static G_GNUC_UNUSED char *get_pid_status_item_buf(int pid, const char *item,
                                                   char *rbuf, int blen,
...
static gboolean sysctl_kinfo_proc(gpointer pid, KINFO_PROC *processi) {
  int res;
...
char *mono_process_get_name(gpointer pid, char *buf, int len) {
  ...
  if (p)
    return p + 1;
  if (r == 0) {
    return get_pid_status_item_buf(GPOINTER_TO_INT(pid), "Name", buf, len,
                                   NULL);
  }
  return buf;
#endif
...
void mono_process_get_times(gpointer pid, gint64 *start_time, gint64 *user_time,
                            gint64 *kernel_time) {
  if (user_time)
  ...
  if (start_time) {
    *start_time = 0;
...
#else
        *start_time =
            mono_100ns_datetime_from_timeval(processi.kinfo_starttime_member);
#endif
      }
    }
#endif
    ...
    if (*start_time == 0) {
      ...
      *start_time =
          boot_time + mono_process_get_data(pid, MONO_PROCESS_ELAPSED);
    }
  }
}
 ...
 * pid (cmdname) S
 * 	[0] ppid pgid sid tty_nr tty_pgrp flags min_flt cmin_flt maj_flt
 * cmaj_flt [10] utime stime cutime cstime prio nice threads 0 start_time vsize
 * 	[20] rss rsslim start_code end_code start_stack esp eip pending blocked
 * sigign [30] sigcatch wchan 0 0 exit_signal cpu rt_prio policy
 */
...
static gint64 get_process_stat_item(int pid, int pos, int sum,
                                    MonoProcessError *error) {
#if defined(__APPLE__)
  double process_user_time = 0,
         process_system_time = 0; //, process_percent = 0;
  task_t task;
  ...
  for (i = 0; i < th_count; i++) {
    ...
    do {
      ret = thread_info(th_array[i], THREAD_BASIC_INFO, (thread_info_t)&th_info,
                        &th_info_count);
    } while (ret == KERN_ABORTED);

    if (ret == KERN_SUCCESS) {
      thread_user_time =
          th_info.user_time.seconds + th_info.user_time.microseconds / 1e6;
      thread_system_time =
          th_info.system_time.seconds + th_info.system_time.microseconds / 1e6;
  ...
  process_user_time +=
      t_info.user_time.seconds + t_info.user_time.microseconds / 1e6;
  process_system_time +=
      t_info.system_time.seconds + t_info.system_time.microseconds / 1e6;
...
static gint64 get_process_stat_time(int pid, int pos, int sum,
                                    MonoProcessError *error) {
...
static gint64 get_pid_status_item(int pid, const char *item,
                                  MonoProcessError *error, int multiplier) {
#if defined(__APPLE__)
  ...
  if (strcmp(item, "VmData") == 0)
    ...
    do {
      mach_ret =
          task_info(task, TASK_BASIC_INFO, (task_info_t)&t_info, &th_count);
    } while (mach_ret == KERN_ABORTED);
 ...
 */
gint64 mono_process_get_data_with_error(gpointer pid, MonoProcessData data,
                                        MonoProcessError *error) {
  gint64 val;
...
int mono_cpu_count(void) {
#ifdef HOST_ANDROID
  /* Android tries really hard to save power by powering off CPUs on SMP phones
   * which means the normal way to query cpu count returns a wrong value with
   * userspace API. Instead we use /sys entries to query the actual hardware CPU
   * count.
   */
  int count = 0;
  char buffer[8] = {'\0'};
  ...
  if (count > 0)
     ...
     *
     * When we merged the change from PR #2722, we started seeing random
     * failures on ARM in the
     * MonoTests.System.Threading.ThreadPoolTests.SetAndGetMaxThreads and
     * MonoTests.System.Threading.ManualResetEventSlimTests.Constructor_Defaults
     * tests. Both of those tests are dealing with Environment.ProcessorCount to
     * verify some implementation details.
     *
     * It turns out that on the Jetson TK1 board we use on public Jenkins and on
     * ARM kernels in general, the value returned by sched_getaffinity (or
     * _SC_NPROCESSORS_ONLN) doesn't contain CPUs/cores that are powered off for
     * power saving reasons. This is contrary to what happens on x86, where even
     * cores in deep-sleep state are returned [1], [2]. This means that we would
     * get a processor count of 1 at one point in time and a higher value when
     * load increases later on as the system wakes CPUs.
     *
     * Various runtime pieces like the threadpool and also user code however
     * relies on the value returned by Environment.ProcessorCount e.g. for
     * deciding how many parallel tasks to start, thereby limiting the
     * performance when that code thinks we only have one CPU.
     *
     * Talking to a few people, this was the reason why we changed to
     * _SC_NPROCESSORS_CONF in mono#1688 and why we added a special case for
     * Android in mono@de3addc to get the "real" number of processors in the
     * system.
     *
     * Because of those issues Android/Dalvik also switched from _ONLN to
     * _SC_NPROCESSORS_CONF for the Java API Runtime.availableProcessors() too
     * [3], citing: > Traditionally this returned the number currently online,
     * but many mobile devices are able to take unused cores offline to save
     * power, so releases newer than Android 4.2 (Jelly Bean) return the maximum
     * number of cores that could be made available if there were no power or
     * heat constraints.
     *
     * The problem with sticking to _SC_NPROCESSORS_CONF however is that it
     * breaks down in constrained environments like Docker or with an explicit
     * CPU affinity set by the Linux `taskset` command, They'd get a higher CPU
     * count than can be used, start more threads etc. which results in
     * unnecessary context switches and overloaded systems. That's why we need
     * to respect sched_getaffinity.
     *
     * So while in an ideal world we would be able to rely on
     * sched_getaffinity/_SC_NPROCESSORS_ONLN to return the number of
     * theoretically available CPUs regardless of power saving measures
     * everywhere, we can't do this on ARM.
     *
     * I think the pragmatic solution is the following:
     * * use sched_getaffinity (+ fallback to _SC_NPROCESSORS_ONLN in case of
     * error) on x86. This ensures we're inline with what OpenJDK [4] and
     * CoreCLR [5] do
     * * use _SC_NPROCESSORS_CONF exclusively on ARM (I think we could
     * eventually even get rid of the HOST_ANDROID special case)
     *
     * Helpful links:
     *
     ...
     * [3]
     * https://android.googlesource.com/platform/libcore/+/750dc634e56c58d1d04f6a138734ac2b772900b5%5E1..750dc634e56c58d1d04f6a138734ac2b772900b5/
     * [4] https://bugs.openjdk.java.net/browse/JDK-6515172
     * [5]
     * https://github.com/dotnet/coreclr/blob/7058273693db2555f127ce16e6b0c5b40fb04867/src/pal/src/misc/sysinfo.cpp#L148
     */
...
static void get_cpu_times(int cpu_id, gint64 *user, gint64 *systemt,
                          gint64 *irq, gint64 *sirq, gint64 *idle) {
  char buf[256];
  char *s;
  int uhz = get_user_hz();
  guint64 user_ticks = 0, nice_ticks = 0, system_ticks = 0, idle_ticks = 0,
          irq_ticks = 0, sirq_ticks = 0;
  ...
  while ((s = fgets(buf, sizeof(buf), f))) {
    ...
    if (cpu_id < 0 && strncmp(s, "cpu", 3) == 0 && g_ascii_isspace(s[3])) {
      data = s + 4;
    } else if (cpu_id >= 0 && strncmp(s, "cpu", 3) == 0 &&
               strtol(s + 3, &data, 10) == cpu_id) {
      if (data == s + 3)
        continue;
      data++;
    } else {
 ...
 * Get data about a processor on the system, like time spent in user space or
 * idle time.
 */
gint64 mono_cpu_get_data(int cpu_id, MonoCpuData data,
                         MonoProcessError *error) {
  gint64 value = 0;
...
gboolean mono_pe_file_time_date_stamp(const gunichar2 *filename, guint32 *out) {
  void *map_handle;
  gint32 map_size;
  ...
  if (1) {
    ...
    IMAGE_NT_HEADERS32 *nt_headers =
        (IMAGE_NT_HEADERS32 *)((guint8 *)file_map +
...
gpointer mono_pe_file_map(const gunichar2 *filename, gint32 *map_size,
                          void **handle) {
  ...
  // This block was added to diagnose https://github.com/mono/mono/issues/14730,
  ...
  if (G_UNLIKELY(filename_ext == NULL)) {
    ...
    g_assertf(filename_ext != NULL,
              "%s: unicode conversion returned NULL; %s; input was: %s",
              __func__, mono_error_get_message(error), raw_bytes->str);
  ...
  if (fd == -1 && (errno == ENOENT || errno == ENOTDIR) && IS_PORTABILITY_SET) {
    ...
    if (!located_filename) {
      ...
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s: Error opening file %s (3): %s", __func__, filename_ext,
                 strerror(errno));
      goto exit;
    }
  ...
  if (fstat(fd, &statbuf) == -1) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: Error stat()ing file %s: %s", __func__, filename_ext,
               strerror(errno));
    goto exit;
  }
  *map_size = statbuf.st_size;
  ...
  if (statbuf.st_size < sizeof(IMAGE_DOS_HEADER)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: File %s is too small: %" PRId64, __func__, filename_ext,
               (gint64)statbuf.st_size);

    goto exit;
  }

  file_map = mono_file_map(statbuf.st_size, MONO_MMAP_READ | MONO_MMAP_PRIVATE,
                           fd, 0, handle);
  if (file_map == NULL) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: Error mmap()int file %s: %s", __func__, filename_ext,
               strerror(errno));
    goto exit;
  }
exit:
...
void mono_pe_file_unmap(gpointer file_map, void *handle) {
  gint res;

  res = mono_file_unmap(file_map, handle);
  if (G_UNLIKELY(res != 0))
    g_error("%s: mono_file_unmap failed, error: \"%s\" (%d)", __func__,
...
gint32 mono_cpu_usage(MonoCpuUsageState *prev) {
  ...
  kernel_time = resource_usage.ru_stime.tv_sec * 1000 * 1000 * 10 +
                resource_usage.ru_stime.tv_usec * 10;
  user_time = resource_usage.ru_utime.tv_sec * 1000 * 1000 * 10 +
              resource_usage.ru_utime.tv_usec * 10;

  cpu_busy_time = (user_time - (prev ? prev->user_time : 0)) +
                  (kernel_time - (prev ? prev->kernel_time : 0));
  cpu_total_time =
      (current_time - (prev ? prev->current_time : 0)) * mono_cpu_count();
