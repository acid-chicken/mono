...
static void file_for_summary_stage(const char *directory,
...
static void create_stage_mark_file(void) {
  char out_file[200];
  file_for_summary_stage(log.directory, log.level, out_file, sizeof(out_file));
  int handle = g_open(out_file, O_WRONLY | O_CREAT,
...
void mono_summarize_timeline_start(void) {
