...
#if defined(ENABLE_CHECKED_BUILD_CRASH_REPORTING) &&                           \
    defined(ENABLE_OVERRIDABLE_ALLOCATORS)
// Fixme: put behind preprocessor symbol?
static void assert_not_reached_mem(const char *msg) {
...
static void assert_not_reached_fn_ptr_free(gpointer ptr) {
  // Wrap the macro to provide as a function pointer
  assert_not_reached_mem("Attempted to call free during merp dump");
}

static gpointer assert_not_reached_fn_ptr_malloc(gsize size) {
  // Wrap the macro to provide as a function pointer
  assert_not_reached_mem("Attempted to call malloc during merp dump");
  return NULL;
}

static gpointer assert_not_reached_fn_ptr_realloc(gpointer obj, gsize size) {
  // Wrap the macro to provide as a function pointer
  assert_not_reached_mem("Attempted to call realloc during merp dump");
  return NULL;
}

static gpointer assert_not_reached_fn_ptr_calloc(gsize n, gsize x) {
  // Wrap the macro to provide as a function pointer
  assert_not_reached_mem("Attempted to call calloc during merp dump");
  return NULL;
}
#endif /* defined(ENABLE_CHECKED_BUILD_CRASH_REPORTING) && defined             \
          (ENABLE_OVERRIDABLE_ALLOCATORS) */

void mono_summarize_toggle_assertions(gboolean enable) {
#if defined(ENABLE_CHECKED_BUILD_CRASH_REPORTING) &&                           \
    defined(ENABLE_OVERRIDABLE_ALLOCATORS)
  ...
  if (enable) {
    ...
    GMemVTable g_mem_vtable_assert = {
        assert_not_reached_fn_ptr_malloc, assert_not_reached_fn_ptr_realloc,
        assert_not_reached_fn_ptr_free, assert_not_reached_fn_ptr_calloc};
...
static void file_for_stage_breadcrumb(const char *directory,
                                      MonoSummaryStage stage, gchar *buff,
                                      size_t sizeof_buff) {
  g_snprintf(buff, sizeof_buff, "%s%scrash_stage_%d", directory,
             G_DIR_SEPARATOR_S, stage);
}

static void file_for_dump_reason_breadcrumb(const char *directory,
                                            const char *dump_reason,
                                            gchar *buff, size_t sizeof_buff) {
  g_snprintf(buff, sizeof_buff, "%s%scrash_reason_%s", directory,
             G_DIR_SEPARATOR_S, dump_reason);
}

static void file_for_hash_breadcrumb(const char *directory,
                                     MonoStackHash hashes, gchar *buff,
                                     size_t sizeof_buff) {
  g_snprintf(buff, sizeof_buff, "%s%scrash_hash_0x%" PRIx64 "", directory,
             G_DIR_SEPARATOR_S, (uint64_t)hashes.offset_rich_hash);
}

static void create_breadcrumb(const char *path) {
  int handle =
      g_open(path, O_WRONLY | O_CREAT, S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH);
  if (handle < 0) {
...
static void create_stage_breadcrumb(void) {
  char out_file[200];
  file_for_stage_breadcrumb(log.directory, log.level, out_file,
                            sizeof(out_file));
  create_breadcrumb(out_file);
}

static void create_dump_reason_breadcrumb(const char *dump_reason) {
  char out_file[200];
  file_for_dump_reason_breadcrumb(log.directory, dump_reason, out_file,
                                  sizeof(out_file));
  create_breadcrumb(out_file);
}

void mono_create_crash_hash_breadcrumb(MonoThreadSummary *thread) {
  char out_file[200];
  file_for_hash_breadcrumb(log.directory, thread->hashes, out_file,
                           sizeof(out_file));
  create_breadcrumb(out_file);
}

gboolean mono_summarize_set_timeline_dir(const char *directory) {
  if (directory) {
...
void mono_summarize_timeline_start(const char *dump_reason) {
...
void mono_summarize_double_fault_log(void) {
  mono_summarize_timeline_phase_log(MonoSummaryDoubleFault);
}

void mono_summarize_timeline_phase_log(MonoSummaryStage next) {
  if (!log.directory)
    return;
  ...
  case MonoSummaryDone:
    g_async_safe_printf(
        "Trying to log crash reporter timeline, already at done %d\n",
        log.level);
    return;
  default:
    g_async_safe_printf(
        "Trying to log crash reporter timeline, illegal state %d\n", log.level);
    return;
  }

  g_assertf(
      out_level == next || next == MonoSummaryDoubleFault,
  ...
  //
  // g_assert (out_level == mono_summarize_timeline_read_level (log.directory,
  // FALSE));
...
static void mem_file_name(long tag, char *name, size_t limit) {
  name[0] = '\0';
  pid_t pid = getpid();
  g_snprintf(name, limit, "mono_crash.mem.%d.%lx.blob", pid, tag);
}

gboolean mono_state_alloc_mem(MonoStateMem *mem, long tag, size_t size) {
  char name[100];
  ...
  if (!g_hasenv("MONO_CRASH_NOFILE"))
    mem->handle = g_open(name, O_RDWR | O_CREAT | O_EXCL,
                         S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH);

  if (mem->handle < 1) {
    mem->mem = (gpointer *)mmap(0, mem->size, PROT_READ | PROT_WRITE,
...
void mono_state_free_mem(MonoStateMem *mem) {
  if (!mem->mem)
    return;
...
static gboolean timeline_has_level(const char *directory, char *log_file,
                                   size_t log_file_size, gboolean clear,
...
MonoSummaryStage mono_summarize_timeline_read_level(const char *directory,
                                                    gboolean clear) {
  char out_file[200];
  ...
  gboolean has_level_done = timeline_has_level(
      directory, out_file, sizeof(out_file), clear, MonoSummaryDone);
  gboolean has_level_cleanup = timeline_has_level(
      directory, out_file, sizeof(out_file), clear, MonoSummaryCleanup);
  gboolean has_level_merp_invoke = timeline_has_level(
      directory, out_file, sizeof(out_file), clear, MonoSummaryMerpInvoke);
  gboolean has_level_merp_writer = timeline_has_level(
      directory, out_file, sizeof(out_file), clear, MonoSummaryMerpWriter);
  gboolean has_level_state_writer = timeline_has_level(
      directory, out_file, sizeof(out_file), clear, MonoSummaryStateWriter);
  gboolean has_level_state_writer_done = timeline_has_level(
      directory, out_file, sizeof(out_file), clear, MonoSummaryStateWriterDone);
  gboolean has_level_managed_stacks = timeline_has_level(
      directory, out_file, sizeof(out_file), clear, MonoSummaryManagedStacks);
  gboolean has_level_unmanaged_stacks = timeline_has_level(
      directory, out_file, sizeof(out_file), clear, MonoSummaryUnmanagedStacks);
  gboolean has_level_suspend_handshake =
      timeline_has_level(directory, out_file, sizeof(out_file), clear,
                         MonoSummarySuspendHandshake);
  gboolean has_level_setup = timeline_has_level(
      directory, out_file, sizeof(out_file), clear, MonoSummarySetup);
...
static void assert_has_space(MonoStateWriter *writer) {
  // Each individual key/value append should be roughly less than this many
  // characters
  const int margin = 35;
  ...
  g_assertf(
      writer->allocated_len - writer->len >= margin,
      "Ran out of memory to create crash dump json blob. Current state:\n%s\n",
      writer->output_str);
}

static void mono_state_writer_printf(MonoStateWriter *writer,
                                     const gchar *format, ...) {
  g_assert(writer->len == strlen(writer->output_str));

  va_list args;
  va_start(args, format);
  int written = vsnprintf(&writer->output_str[writer->len],
                          writer->allocated_len - writer->len, format, args);
  va_end(args);
...
static void mono_state_writer_indent(MonoStateWriter *writer) {
  for (int i = 0; i < writer->indent; ++i)
    mono_state_writer_printf(writer, " ");
}

static void mono_state_writer_object_key(MonoStateWriter *writer,
                                         const char *key) {
  mono_state_writer_indent(writer);
  mono_state_writer_printf(writer, "\"%s\" : ", key);
}

static void mono_native_state_add_ctx(MonoStateWriter *writer,
                                      MonoContext *ctx) {
  // Context
  mono_state_writer_indent(writer);
...
static void mono_native_state_add_frame(MonoStateWriter *writer,
  ...
  if (frame->is_managed) {
    ...
    if (frame->unmanaged_data.ip) {
      mono_state_writer_printf(writer, "\"0x%" PRIx64 "\"",
                               (guint64)frame->unmanaged_data.ip);
    } else
...
static void mono_native_state_add_frames(MonoStateWriter *writer,
                                         int num_frames,
...
static void mono_native_state_add_managed_exc(MonoStateWriter *writer,
                                              MonoExcSummary *exc) {
  mono_state_writer_indent(writer);
  mono_state_writer_printf(writer, "{\n");
  writer->indent++;
  ...
  mono_state_writer_printf(writer, "\"%s.%s\",\n",
                           m_class_get_name_space(exc->managed_exc_type),
                           m_class_get_name(exc->managed_exc_type));

  mono_native_state_add_frames(writer, exc->num_managed_frames,
                               exc->managed_frames, "managed_frames");
...
static void mono_native_state_add_managed_excs(MonoStateWriter *writer,
                                               int num_excs,
                                               MonoExcSummary *excs) {
  mono_state_writer_indent(writer);
...
void mono_native_state_add_thread(MonoStateWriter *writer,
                                  MonoThreadSummary *thread, MonoContext *ctx,
  ...
  if (thread->num_exceptions > 0) {
    mono_state_writer_printf(writer, ",\n");
    mono_native_state_add_managed_excs(writer, thread->num_exceptions,
                                       thread->exceptions);
  }

  if (thread->num_managed_frames > 0) {
    mono_state_writer_printf(writer, ",\n");
    mono_native_state_add_frames(writer, thread->num_managed_frames,
                                 thread->managed_frames, "managed_frames");
  }

  if (thread->num_unmanaged_frames > 0) {
    mono_state_writer_printf(writer, ",\n");
    mono_native_state_add_frames(writer, thread->num_unmanaged_frames,
                                 thread->unmanaged_frames, "unmanaged_frames");
  }
...
static void mono_native_state_add_ee_info(MonoStateWriter *writer) {
#ifndef MONO_PRIVATE_CRASHES
...
static void mono_native_state_add_version(MonoStateWriter *writer) {
  assert_has_space(writer);
  mono_state_writer_indent(writer);
...
static void mono_native_state_add_memory(MonoStateWriter *writer) {
  assert_has_space(writer);
  mono_state_writer_indent(writer);
...
static void mono_native_state_add_process_map(MonoStateWriter *writer) {
#if defined(__linux__) && !defined(HOST_ANDROID)
  int handle = g_open("/proc/self/maps", O_RDONLY,
                      S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH);
  if (handle == -1) {
    g_async_safe_printf(
        "Couldn't find /proc/self/maps on Linux system. Continuing.");
    return;
  }
  ...
  while (mapping < MONO_CRASH_REPORTING_MAPPING_LINE_LIMIT) {
    ...
    while (TRUE) {
      ...
      int charsCopied =
          g_async_safe_fgets(line, sizeof(line), handle, &newline);
...
static void mono_native_state_add_logged_message(MonoStateWriter *writer,
...
static void mono_native_state_add_prologue(MonoStateWriter *writer) {
  mono_state_writer_printf(writer, "{\n");
  writer->indent++;
  ...
  mono_native_state_add_logged_message(writer, "assertion_message",
                                       assertion_msg);

  const char *failfast_msg = mono_crash_get_failfast_msg();
  mono_native_state_add_logged_message(writer, "failfast_message",
                                       failfast_msg);
...
static void mono_native_state_add_epilogue(MonoStateWriter *writer) {
  mono_state_writer_printf(writer, "\n");
  mono_state_writer_indent(writer);
  mono_state_writer_printf(writer, "]\n");
  writer->indent--;
...
void mono_native_state_init(MonoStateWriter *writer) {
  mono_native_state_add_prologue(writer);
}

char *mono_native_state_emit(MonoStateWriter *writer) {
  mono_native_state_add_epilogue(writer);
  return writer->output_str;
}

char *mono_native_state_free(MonoStateWriter *writer, gboolean free_data) {
  mono_native_state_add_epilogue(writer);
  char *output = NULL;
...
void mono_state_writer_init(MonoStateWriter *writer, gchar *output_str,
                            int len) {
...
void mono_summarize_native_state_begin(MonoStateWriter *writer, gchar *mem,
                                       int size) {
  g_assert(mem);
...
char *mono_summarize_native_state_end(MonoStateWriter *writer) {
  return mono_native_state_emit(writer);
}

void mono_summarize_native_state_add_thread(MonoStateWriter *writer,
                                            MonoThreadSummary *thread,
                                            MonoContext *ctx,
                                            gboolean crashing_thread) {
...
void mono_crash_dump(const char *jsonFile, MonoStackHash *hashes) {
  if (g_hasenv("MONO_CRASH_NOFILE"))
    return;
  ...
  for (int increment = 0; increment < 100; increment++) {
    char name[100];
    name[0] = '\0';
    g_snprintf(name, sizeof(name), "mono_crash.%" PRIx64 ".%d.json",
               hashes->offset_free_hash, increment);
...
gboolean mono_dump_start(void) {
  return (mono_atomic_xchg_i32(&dump_status, 1) ==
          0); // return true if we started the dump
}

gboolean mono_dump_complete(void) {
  return (mono_atomic_xchg_i32(&dump_status, 0) ==
          1); // return true if we completed the dump
}
 ...
 */
char *mono_crash_save_failfast_msg(char *msg) {
  return (char *)mono_atomic_xchg_ptr((gpointer *)&saved_failfast_msg,
                                      (void *)msg);
}

const char *mono_crash_get_failfast_msg(void) { return saved_failfast_msg; }
