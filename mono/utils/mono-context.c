...
#elif (defined(__x86_64__) && !defined(MONO_CROSS_COMPILE)) ||                 \
    (defined(TARGET_AMD64)) /* defined(__i386__) */
...
void mono_sigctx_to_monoctx(void *sigctx, MonoContext *mctx) {
  ...
  memcpy(&(mctx->fregs[AMD64_XMM0]), &(context->Xmm0),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM1]), &(context->Xmm1),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM2]), &(context->Xmm2),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM3]), &(context->Xmm3),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM4]), &(context->Xmm4),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM5]), &(context->Xmm5),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM6]), &(context->Xmm6),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM7]), &(context->Xmm7),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM8]), &(context->Xmm8),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM9]), &(context->Xmm9),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM10]), &(context->Xmm10),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM11]), &(context->Xmm11),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM12]), &(context->Xmm12),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM13]), &(context->Xmm13),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM14]), &(context->Xmm14),
         sizeof(MonoContextSimdReg));
  memcpy(&(mctx->fregs[AMD64_XMM15]), &(context->Xmm15),
         sizeof(MonoContextSimdReg));
...
void mono_monoctx_to_sigctx(MonoContext *mctx, void *sigctx) {
  ...
  memcpy(&(context->Xmm0), &(mctx->fregs[AMD64_XMM0]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm1), &(mctx->fregs[AMD64_XMM1]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm2), &(mctx->fregs[AMD64_XMM2]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm3), &(mctx->fregs[AMD64_XMM3]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm4), &(mctx->fregs[AMD64_XMM4]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm5), &(mctx->fregs[AMD64_XMM5]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm6), &(mctx->fregs[AMD64_XMM6]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm7), &(mctx->fregs[AMD64_XMM7]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm8), &(mctx->fregs[AMD64_XMM8]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm9), &(mctx->fregs[AMD64_XMM9]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm10), &(mctx->fregs[AMD64_XMM10]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm11), &(mctx->fregs[AMD64_XMM11]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm12), &(mctx->fregs[AMD64_XMM12]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm13), &(mctx->fregs[AMD64_XMM13]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm14), &(mctx->fregs[AMD64_XMM14]),
         sizeof(MonoContextSimdReg));
  memcpy(&(context->Xmm15), &(mctx->fregs[AMD64_XMM15]),
         sizeof(MonoContextSimdReg));
#elif defined(__HAIKU__)
  // Haiku uses sigcontext because there's no ucontext
  struct sigcontext *ctx = (struct sigcontext *)sigctx;
...
void mono_sigctx_to_monoctx(void *ctx, MonoContext *mctx) {
...
void mono_monoctx_to_sigctx(MonoContext *mctx, void *ctx) {
  memcpy(ctx, mctx, sizeof(MonoContext));
}

/*========================= End of Function ========================*/

#elif (defined(__arm__) && !defined(MONO_CROSS_COMPILE)) ||                    \
    (defined(TARGET_ARM))
...
#elif (defined(__aarch64__) && !defined(MONO_CROSS_COMPILE)) ||                \
...
void mono_sigctx_to_monoctx(void *sigctx, MonoContext *mctx) {
  ...
  struct fpsimd_context *fpctx =
      (struct fpsimd_context *)&((ucontext_t *)sigctx)->uc_mcontext.__reserved;
  int i;
...
void mono_sigctx_to_monoctx(void *sigctx, MonoContext *mctx) {
  ...
  memcpy(&mctx->regs, &UCONTEXT_REG_Rn(uc, 0),
         sizeof(host_mgreg_t) * MONO_MAX_IREGS);
  memcpy(&mctx->fregs, &UCONTEXT_REG_FPRn(uc, 0),
...
void mono_monoctx_to_sigctx(MonoContext *mctx, void *sigctx) {
  ...
  memcpy(&UCONTEXT_REG_Rn(uc, 0), &mctx->regs,
         sizeof(host_mgreg_t) * MONO_MAX_IREGS);
  memcpy(&UCONTEXT_REG_FPRn(uc, 0), &mctx->fregs,
         sizeof(double) * MONO_MAX_FREGS);
...
#elif ((defined(HOST_RISCV) || defined(HOST_RISCV64)) &&                       \
       !defined(MONO_CROSS_COMPILE)) ||                                        \
...
void mono_sigctx_to_monoctx(void *sigctx, MonoContext *mctx) {
  ...
  memcpy(&mctx->gregs, &uctx->uc_mcontext.__gregs,
         sizeof(host_mgreg_t) * G_N_ELEMENTS(mctx->gregs));
  memcpy(&mctx->fregs, &uctx->uc_mcontext.__fpregs,
         sizeof(double) * G_N_ELEMENTS(mctx->fregs));
#endif
}
...
void mono_monoctx_to_sigctx(MonoContext *mctx, void *sigctx) {
  ...
  memcpy(&uctx->uc_mcontext.__gregs, &mctx->gregs,
         sizeof(host_mgreg_t) * G_N_ELEMENTS(mctx->gregs));
  memcpy(&uctx->uc_mcontext.__fpregs, &mctx->fregs,
         sizeof(double) * G_N_ELEMENTS(mctx->fregs));
#endif
}
