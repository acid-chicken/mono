...
#define mono_get_tls_key_to_jit_icall_id(a)                                    \
  ((MonoJitICallId)((a) + MONO_JIT_ICALL_mono_tls_get_domain_extern))
...
// That is presumably catastrophic when combined with mono_amd64_emit_tls_get /
// mono_x86_emit_tls_get. It also is not consistent. FLS is a reasonable idea
// perhaps, but we would need to be consistent and to adjust JIT. And there is
// __declspec(fiber).
#undef TlsAlloc
#undef TlsFree
#undef TlsGetValue
...
#define mono_native_tls_alloc(key, destructor)                                 \
  ((*(key) = TlsAlloc()) != TLS_OUT_OF_INDEXES && destructor == NULL)
#define mono_native_tls_free TlsFree
#define mono_native_tls_set_value TlsSetValue

#include <winternl.h>

// TlsGetValue always writes 0 to LastError. Which can cause problems. This
// never changes LastError.
//
MONO_INLINE
void *mono_native_tls_get_value(unsigned index) {
...
MONO_INLINE int mono_native_tls_alloc(MonoNativeTlsKey *key, void *destructor) {
  return pthread_key_create(key, (void (*)(void *))destructor) == 0;
}

MONO_INLINE void mono_native_tls_free(MonoNativeTlsKey key) {
  pthread_key_delete(key);
}

MONO_INLINE int mono_native_tls_set_value(MonoNativeTlsKey key,
                                          gpointer value) {
...
/*
 * On all platforms we should be able to use either __thread or __declspec
 * (thread) or pthread/TlsGetValue. Certain platforms will support fast tls only
 * when using one of the thread local storage backends. By default this is
 * __thread if we have MONO_KEYWORD_THREAD defined.
 *
 * By default all platforms will call into these native getters whenever they
 * need to get a tls value. On certain platforms we can try to be faster than
 * this and avoid the call. We call this fast tls and each platform defines its
 * own way to achieve this. For this, a platform has to define
 * MONO_ARCH_HAVE_INLINED_TLS, and provide alternative getters/setters for a
 * MonoTlsKey. In order to have fast getter/setters, the platform has to declare
 * a way to fetch an internal offset (MONO_THREAD_VAR_OFFSET) which is stored
 * here, and in the arch specific file probe the system to see if we can use the
 * offset initialized here. If these run-time checks don't succeed we just use
 * the fallbacks.
 *
...
/*
 * Define this if you want a faster libmono, which cannot be loaded dynamically
 * as a module.
 */
//#define PIC_INITIAL_EXEC
...
extern MONO_KEYWORD_THREAD SgenThreadInfo *mono_tls_sgen_thread_info
    MONO_TLS_FAST;
...
#else
#define MONO_TLS_GET_VALUE(tls_var, tls_key)                                   \
  (mono_native_tls_get_value(tls_key))
#define MONO_TLS_SET_VALUE(tls_var, tls_key, value)                            \
  (mono_native_tls_set_value(tls_key, value))
#endif
