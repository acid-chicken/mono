...
#define mono_thread_info_run_state(info)                                       \
  (((MonoThreadInfo *)info)->thread_state.state)
...
static BeginSuspendResult begin_cooperative_suspend(MonoThreadInfo *info) {
  /* There's nothing else to do after we async request the thread to suspend */
  mono_threads_add_to_pending_operation_set(info);
  return BeginSuspendOkCooperative;
}

static BeginSuspendResult begin_preemptive_suspend(MonoThreadInfo *info,
...
begin_suspend_for_running_thread(MonoThreadInfo *info,
                                 gboolean interrupt_kernel) {
...
static gboolean thread_is_cooperative_suspend_aware(MonoThreadInfo *info) {
  return (mono_threads_is_cooperative_suspension_enabled() ||
          mono_atomic_load_i32(&(info->coop_aware_thread)));
}

static BeginSuspendResult begin_suspend_for_blocking_thread(
    MonoThreadInfo *info, gboolean interrupt_kernel,
    MonoThreadSuspendPhase phase, gboolean coop_aware_thread,
  ...
  // with hybrid suspend, preemptively suspend blocking threads (if thread is
  // not coop aware), otherwise blocking already counts as suspended.
...
static gboolean check_async_suspend(MonoThreadInfo *info,
...
static void resume_async_suspended(MonoThreadInfo *info) {
...
static void resume_self_suspended(MonoThreadInfo *info) {
...
static void resume_blocking_suspended(MonoThreadInfo *info) {
  THREADS_SUSPEND_DEBUG("**BEGIN blocking-resume %p\n",
                        mono_thread_info_get_tid(info));
  mono_os_sem_post(&info->resume_semaphore);
}

void mono_threads_add_to_pending_operation_set(MonoThreadInfo *info) {
...
void mono_threads_begin_global_suspend(void) {
  ...
  THREADS_SUSPEND_DEBUG("------ BEGIN GLOBAL OP sp %d rp %d ap %d wd %d po %d "
                        "(sp + rp + ap == wd) (wd == po)\n",
                        suspend_posts, resume_posts, abort_posts, waits_done,
                        pending_ops);
...
static void dump_threads(void) {
  ...
  g_async_safe_printf("STATE CUE CARD: (? means a positive number, usually 1 "
  ...
  FOREACH_THREAD_SAFE_ALL(info) {
    ...
    g_async_safe_printf("--thread %p id %p [%p] (%s) state %x  %s\n", info,
                        ...
                        (void *)(size_t)info->native_handle, thread_name,
                        info->thread_state, info == cur ? "GC INITIATOR" : "");
#else
    g_async_safe_printf("--thread %p id %p [%p] state %x  %s\n", info,
                        (void *)mono_thread_info_get_tid(info),
                        (void *)(size_t)info->native_handle, info->thread_state,
                        info == cur ? "GC INITIATOR" : "");
#endif
  }
...
gboolean mono_threads_wait_pending_operations(void) {
  ...
  if (pending_suspends) {
    ...
    for (i = 0; i < pending_suspends; ++i) {
      ...
      if (mono_os_sem_timedwait(&suspend_semaphore, sleepAbortDuration,
                                MONO_SEM_FLAGS_NONE) ==
      ...
      g_error("suspend_thread suspend took %d ms, which is more than the "
              "allowed %d ms",
...
static void mono_hazard_pointer_clear_all(MonoThreadHazardPointers *hp,
                                          int retain) {
  if (retain != 0)
...
*/
MonoThreadInfo *mono_thread_info_lookup(MonoNativeThreadId id) {
...
static gboolean mono_thread_info_insert(MonoThreadInfo *info) {
  MonoThreadHazardPointers *hp = mono_hazard_pointer_get();
...
static gboolean mono_thread_info_remove(MonoThreadInfo *info) {
  MonoThreadHazardPointers *hp = mono_hazard_pointer_get();
  gboolean res;
...
static void free_thread_info(gpointer mem) {
 ...
 * ID is returned.
 */
int mono_thread_info_register_small_id(void) {
...
static void thread_handle_destroy(gpointer data) {
...
static gboolean register_thread(MonoThreadInfo *info) {
  size_t stsize = 0;
  guint8 *staddr = NULL;
  gboolean result;
  ...
  THREADS_DEBUG("registering info %p tid %p small id %x\n", info,
                mono_thread_info_get_tid(info), info->small_id);
  ...
  /*
  Transition it before taking any locks or publishing itself to reduce the
  chance of others witnessing a detached thread. We can reasonably expect that
  until this thread gets published, no other thread will try to manipulate it.
  */
  mono_threads_transition_attach(info);
  mono_thread_info_suspend_lock();
  /*If this fail it means a given thread has been registered twice, which
   * doesn't make sense. */
...
static void mono_thread_info_suspend_lock_with_info(MonoThreadInfo *info);

static void mono_threads_signal_thread_handle(MonoThreadHandle *thread_handle);
...
static void unregister_thread(void *arg) {
  ...
  /* We only enter the GC unsafe region, as when exiting this function, the
   * thread
  ...
  mono_threads_enter_gc_unsafe_region_unbalanced_with_info(
      info, &gc_unsafe_stackdata);

  /* Need to be in GC Unsafe to pump the HP queue - some of the cleanup
   * methods need to use coop-aware locks. For example:
   * jit_info_table_free_duplicate.
   */
  ...
  */
  ...
  // clear the small_id thread local, in case this thread so that if it is
  // reattached while running other TLS key dtors it will get a new small id
#ifdef MONO_KEYWORD_THREAD
  tls_small_id = -1;
#else
...
static void thread_exited_dtor(void *arg) {
#if defined(__MACH__)
  /*
...
MonoThreadInfo *mono_thread_info_current_unchecked(void) {
  return mono_threads_inited
             ? (MonoThreadInfo *)mono_native_tls_get_value(thread_info_key)
             : NULL;
}

MonoThreadInfo *mono_thread_info_current(void) {
  MonoThreadInfo *info =
      (MonoThreadInfo *)mono_native_tls_get_value(thread_info_key);
  if (info)
    return info;
  ...
  /*
  We might be called during thread cleanup, but we cannot be called after
  cleanup as happened. The way to distinguish between before, during and after
  cleanup is the following:

  -If the TLS key is set, cleanup has not begun;
  -If the TLS key is clean, but the thread remains registered, cleanup is in
  progress; -If the thread is nowhere to be found, cleanup has finished.

  We cannot function after cleanup since there's no way to ensure what will
  happen.
  */
  g_assertf(info, ""); // f includes __func__

  /*We're looking up the current thread which will not be freed until we finish
   * running, so no need to keep it on a HP */
...
MonoLinkedListSet *mono_thread_info_list_head(void) { return &thread_list; }

MonoThreadInfo *mono_thread_info_attach(void) {
  MonoThreadInfo *info;
  ...
  if (!mono_threads_inited) {
    ...
    THREADS_DEBUG(
        "mono_thread_info_attach called before mono_thread_info_init\n");
    return NULL;
  }
#endif
...
void mono_thread_info_detach(void) {
  ...
  if (!mono_threads_inited) {
    ...
    THREADS_DEBUG(
        "mono_thread_info_detach called before mono_thread_info_init\n");
    return;
  }
#endif
...
gboolean
mono_thread_info_try_get_internal_thread_gchandle(MonoThreadInfo *info,
...
void mono_thread_info_set_internal_thread_gchandle(MonoThreadInfo *info,
                                                   MonoGCHandle gchandle) {
  g_assertf(info, ""); // f includes __func__
  g_assert(mono_thread_info_is_current(info));
...
void mono_thread_info_unset_internal_thread_gchandle(THREAD_INFO_TYPE *info) {
  g_assertf(info, ""); // f includes __func__
  g_assert(mono_thread_info_is_current(info));
  info->internal_thread_gchandle = NULL;
}
 ...
 * dtors.
 */
gboolean mono_thread_info_is_exiting(void) {
#if defined(__MACH__)
...
#ifndef HOST_WIN32
static void thread_info_key_dtor(void *arg) {
...
MonoThreadInfoFlags mono_thread_info_get_flags(MonoThreadInfo *info) {
  return (MonoThreadInfoFlags)mono_atomic_load_i32(&info->flags);
}

void mono_thread_info_set_flags(MonoThreadInfoFlags flags) {
  MonoThreadInfo *info = mono_thread_info_current();
  MonoThreadInfoFlags old =
      (MonoThreadInfoFlags)mono_atomic_load_i32(&info->flags);
...
void mono_thread_info_wait_inited(void) {
  MonoSemType cb;
  ...
  while (mono_threads_inited != TRUE) {
    GSList *old_read = (GSList *)mono_atomic_cas_ptr(
        (gpointer *)&init_callbacks, &wait_request, old);
  ...
  while (mono_threads_inited != TRUE) {
    gboolean timedout = mono_os_sem_timedwait(&cb, 1000, MONO_SEM_FLAGS_NONE) ==
...
static void mono_thread_info_set_inited(void) {
  ...
  while (TRUE) {
    GSList *old_read = (GSList *)mono_atomic_cas_ptr(
        (gpointer *)&init_callbacks, MONO_END_INIT_CB, (gpointer)old);
    if (old == old_read)
      break;
    else
      old = old_read;
...
void mono_thread_info_callbacks_init(MonoThreadInfoCallbacks *callbacks) {
  threads_callbacks = *callbacks;
}

void mono_thread_info_signals_init(void) {
  mono_threads_suspend_init_signals();
}

void mono_thread_info_runtime_init(
    const MonoThreadInfoRuntimeCallbacks *callbacks) {
  mono_runtime_callbacks = callbacks;
}

static gboolean mono_thread_info_core_resume(MonoThreadInfo *info) {
  gboolean res = FALSE;
 ...
 */
static gboolean mono_thread_info_core_pulse(MonoThreadInfo *info) {
  gboolean res = FALSE;
...
gboolean mono_thread_info_resume(MonoNativeThreadId tid) {
  gboolean result; /* don't initialize it so the compiler can catch unitilized
                      paths. */
  MonoThreadHazardPointers *hp = mono_hazard_pointer_get();
  MonoThreadInfo *info;
...
mono_thread_info_begin_suspend(MonoThreadInfo *info,
  ...
  if (phase == MONO_THREAD_SUSPEND_PHASE_MOPUP &&
      mono_threads_is_hybrid_suspension_enabled())
...
begin_suspend_request_suspension_cordially(MonoThreadInfo *info) {
  ...
  case ReqSuspendInitSuspendBlocking:
    ...
    // nothing to do, in hybrid coop preempt the thread. If thread is coop
    // aware, handle its as normal cooperative mode.
    if (mono_threads_is_blocking_transition_enabled())
      coop_aware_thread = thread_is_cooperative_suspend_aware(info);

    switch (begin_suspend_for_blocking_thread(info, FALSE,
                                              MONO_THREAD_SUSPEND_PHASE_INITIAL,
                                              coop_aware_thread, NULL)) {
    case BeginSuspendFail:
...
begin_suspend_peek_and_preempt(MonoThreadInfo *info) {
   ...
   * requires that a suspension request transition was already performed for
   * 'info', and if it is still in blocking_suspend_requested, preemptively
   * suspends it.
   */
  ...
  if (mono_threads_transition_peek_blocking_suspend_requested(info)) {
    ...
    switch (begin_suspend_for_blocking_thread(
        info, FALSE, MONO_THREAD_SUSPEND_PHASE_MOPUP, FALSE, NULL)) {
    case BeginSuspendFail:
      return MONO_THREAD_BEGIN_SUSPEND_SKIP;
    case BeginSuspendOkNoWait:
    case BeginSuspendOkCooperative:
...
gboolean mono_thread_info_begin_pulse_resume_and_request_suspension(
    MonoThreadInfo *info) {
...
/*
FIXME fix cardtable WB to be out of line and check with the runtime if the
target is not the WB trampoline. Another option is to encode wb ranges in
MonoJitInfo, but that is somewhat hard.
*/
static gboolean is_thread_in_critical_region(MonoThreadInfo *info) {
  ...
  if (threads_callbacks.thread_in_critical_region &&
      threads_callbacks.thread_in_critical_region(info)) {
    return TRUE;
  }

  /* The target thread might be shutting down and the domain might be null,
   * which means no managed code left to run. */
  ...
  /* altstack signal handler, sgen can't handle them, so we treat them as
   * critical */
  if (stack_start < info->stack_start_limit || stack_start >= info->stack_end)
    return TRUE;

  if (threads_callbacks.ip_in_critical_region)
    return threads_callbacks.ip_in_critical_region(
...
/*
The return value is only valid until a matching mono_thread_info_resume is
called
*/
static MonoThreadInfo *suspend_sync(MonoNativeThreadId tid,
                                    gboolean interrupt_kernel) {
  MonoThreadHazardPointers *hp = mono_hazard_pointer_get();
  ...
  case ReqSuspendAlreadySuspended:
    mono_hazard_pointer_clear(
        hp, 1); // XXX this is questionable we got to clean the suspend/resume
                // nonsense of critical sections
    return info;
  case ReqSuspendInitSuspendRunning:
  ...
  case ReqSuspendInitSuspendBlocking: {
    ...
    suspend_result = begin_suspend_for_blocking_thread(
        info, interrupt_kernel, MONO_THREAD_SUSPEND_PHASE_MOPUP, FALSE,
        &did_interrupt);
    if (suspend_result == BeginSuspendFail) {
      mono_hazard_pointer_clear(hp, 1);
      return NULL;
...
static MonoThreadInfo *suspend_sync_nolock(MonoNativeThreadId id,
                                           gboolean interrupt_kernel) {
...
void mono_thread_info_safe_suspend_and_run(MonoNativeThreadId id,
                                           gboolean interrupt_kernel,
  ...
  case MonoResumeThread:
    ...
    mono_threads_wait_pending_operations();
#ifdef USE_WINDOWS_BACKEND
    // If we interrupt kernel but have blocking sync IO requests preventing the
    // thread from running APC's try to abort all sync blocking IO request. This
    // must be done after thread has been resumed, but before releasing global
    // suspend lock (preventing other threads from supsending the thread).
    if (interrupt_kernel)
...
/**
Inject an assynchronous call into the target thread. The target thread must be
suspended and only a single async call can be setup for a given suspend cycle.
This async call must cause stack unwinding as the current implementation doesn't
save enough state to resume execution of the top-of-stack function. It's an
acceptable limitation since this is currently used only to deliver exceptions.
*/
void mono_thread_info_setup_async_call(MonoThreadInfo *info,
                                       void (*target_func)(void *),
                                       void *user_data) {
  if (!mono_threads_are_safepoints_enabled()) {
    /* In non-coop mode, an async call can only be setup on an async suspended
     * thread, but in coop mode, a thread may be in blocking state, and will
     * execute the async call when leaving the safepoint, leaving a gc safe
     * region or entering a gc unsafe region */
    g_assert(mono_thread_info_run_state(info) == STATE_ASYNC_SUSPENDED);
  }
  /*FIXME this is a bad assert, we probably should do proper locking and fail if
   * one is already set*/
...
void mono_thread_info_suspend_lock(void) {
  MonoThreadInfo *info;
  gint res;
...
void mono_thread_info_suspend_unlock(void) {
...
mono_thread_info_get_suspend_state(MonoThreadInfo *info) {
  ...
  case STATE_BLOCKING_SUSPEND_REQUESTED:
    // This state is only valid for full cooperative suspend or cooparative
    // suspend aware threads. If we're preemptively suspending blocking threads,
  ...
  STATE_BLOCKING_SUSPEND_REQUESTED: Invalid if we're preemptively suspending
  blocking threads.
  */
  g_error("Cannot read suspend state when target %p is in the %s state",
          mono_thread_info_get_tid(info), mono_thread_state_name(cur_state));
}
 ...
 * facility.
 */
void mono_thread_info_abort_socket_syscall_for_close(MonoNativeThreadId tid) {
 ...
 *
 *   Set whenever the current thread is in an async context. Some runtime
 * functions might behave differently while in an async context in order to be
 * async safe.
 */
void mono_thread_info_set_is_async_context(gboolean async_context) {
  MonoThreadInfo *info = mono_thread_info_current();
...
gboolean mono_thread_info_is_async_context(void) {
  MonoThreadInfo *info = mono_thread_info_current();
 ...
 *
 *   Return the address and size of the current threads stack. Return NULL as
 * the stack address if the stack address cannot be determined.
 */
void mono_thread_info_get_stack_bounds(guint8 **staddr, size_t *stsize) {
...
gboolean mono_thread_info_yield(void) { return mono_threads_platform_yield(); }
...
static void sleep_interrupt(gpointer data) {
  mono_coop_mutex_lock(&sleep_mutex);
  mono_coop_cond_broadcast(&sleep_cond);
  mono_coop_mutex_unlock(&sleep_mutex);
}

static guint32 sleep_interruptable(guint32 ms, gboolean *alerted) {
  gint64 now, end;
...
gint mono_thread_info_sleep(guint32 ms, gboolean *alerted) {
  if (ms == 0) {
    MonoThreadInfo *info;
  ...
  if (ms == MONO_INFINITE_WAIT) {
    ...
    /* Use clock_nanosleep () to prevent time drifting problems when nanosleep
...
gint mono_thread_info_usleep(guint64 us) {
  MONO_ENTER_GC_SAFE;
  g_usleep(us);
  MONO_EXIT_GC_SAFE;
  return 0;
...
gpointer mono_thread_info_tls_get(THREAD_INFO_TYPE *info, MonoTlsKey key) {
 ...
 * This should only be used for infrequently changing TLS variables, and it
 * should be paired with setting the real TLS variable since this provides no GC
 * tracking.
 */
void mono_thread_info_tls_set(THREAD_INFO_TYPE *info, MonoTlsKey key,
                              gpointer value) {
 ...
 */
void mono_thread_info_exit(gsize exit_code) {
...
void mono_threads_close_thread_handle(MonoThreadHandle *thread_handle) {
  if (!thread_handle)
    return;
...
mono_threads_open_native_thread_handle(MonoNativeThreadHandle thread_handle) {
  ...
  return DuplicateHandle(GetCurrentProcess(), thread_handle,
                         GetCurrentProcess(), &new_thread_handle, 0, FALSE,
...
void mono_threads_close_native_thread_handle(
...
/*
 * mono_thread_info_install_interrupt: install an interruption token for the
 * current thread.
 *
 *  - @callback: must be able to be called from another thread and always cancel
 * the wait
 *  - @data: passed to the callback
 *  - @interrupted: will be set to TRUE if a token is already installed, FALSE
 * otherwise if set to TRUE, it must mean that the thread is in interrupted
 * state
 */
void mono_thread_info_install_interrupt(void (*callback)(gpointer data),
                                        gpointer data, gboolean *interrupted) {
  MonoThreadInfo *info;
  ...
  *interrupted = FALSE;
   ...
   *  - if the token is not interrupted: it will be freed in uninstall, as
   * info->interrupt_token has not been replaced by the INTERRUPT_STATE flag
   * value, and it still contains the pointer to the memory location
   *  - if the token is interrupted: it will be freed in finish, as the token is
   * now owned by the prepare/finish functions, and info->interrupt_token does
   * not contains a pointer to the memory anymore */
  token = g_new0(MonoThreadInfoInterruptToken, 1);
  token->callback = callback;
  token->data = data;

  previous_token = (MonoThreadInfoInterruptToken *)mono_atomic_cas_ptr(
      (gpointer *)&info->interrupt_token, token, NULL);

  if (previous_token) {
    if (previous_token != INTERRUPT_STATE)
      g_error("mono_thread_info_install_interrupt: previous_token should be "
              "INTERRUPT_STATE (%p), but it was %p",
              INTERRUPT_STATE, previous_token);
  ...
  THREADS_INTERRUPT_DEBUG(
      "interrupt install    tid %p token %p previous_token %p interrupted %s\n",
      mono_thread_info_get_tid(info), token, previous_token,
      *interrupted ? "TRUE" : "FALSE");
}

void mono_thread_info_uninstall_interrupt(gboolean *interrupted) {
  MonoThreadInfo *info;
  MonoThreadInfoInterruptToken *previous_token;

  /* Common to uninstall interrupt handler around OS API's affecting last error.
   */
  /* This method could call OS API's on some platforms that will reset last
   * error so make sure to restore */
  ...
  *interrupted = FALSE;
  ...
  previous_token = (MonoThreadInfoInterruptToken *)mono_atomic_xchg_ptr(
      (gpointer *)&info->interrupt_token, NULL);
...
static MonoThreadInfoInterruptToken *set_interrupt_state(MonoThreadInfo *info) {
  ...
  } while (mono_atomic_cas_ptr((gpointer *)&info->interrupt_token,
                               INTERRUPT_STATE,
 ...
 *
 * The state of the thread info interrupt token is set to 'interrupted' which
 * means that :
 *  - if the thread calls one of the WaitFor functions, the function will return
 * with WAIT_IO_COMPLETION instead of waiting
 *  - if the thread was waiting when this function was called, the wait will be
 * broken
 *
 ...
 * call the wait function again. This essentially means that the target thread
 * will busy wait until it is ready to process the interruption.
 */
...
void mono_thread_info_finish_interrupt(MonoThreadInfoInterruptToken *token) {
...
void mono_thread_info_self_interrupt(void) {
  MonoThreadInfo *info;
  MonoThreadInfoInterruptToken *token;
...
void mono_thread_info_clear_self_interrupt(void) {
  MonoThreadInfo *info;
  MonoThreadInfoInterruptToken *previous_token;
  ...
  previous_token = (MonoThreadInfoInterruptToken *)mono_atomic_cas_ptr(
      (gpointer *)&info->interrupt_token, NULL, INTERRUPT_STATE);
...
gboolean mono_thread_info_is_interrupt_state(MonoThreadInfo *info) {
  g_assertf(info, ""); // f includes __func__
  return mono_atomic_load_ptr((gpointer *)&info->interrupt_token) ==
         INTERRUPT_STATE;
}

void mono_thread_info_describe_interrupt_token(MonoThreadInfo *info,
                                               GString *text) {
  g_assertf(info, ""); // f includes __func__

  if (!mono_atomic_load_ptr((gpointer *)&info->interrupt_token))
    g_string_append_printf(text, "not waiting");
  else if (mono_atomic_load_ptr((gpointer *)&info->interrupt_token) ==
           INTERRUPT_STATE)
...
gboolean mono_thread_info_is_current(MonoThreadInfo *info) {
  return mono_thread_info_get_tid(info) == mono_native_thread_id_get();
}

MonoThreadInfoWaitRet
mono_thread_info_wait_one_handle(MonoThreadHandle *thread_handle,
...
MonoThreadInfoWaitRet mono_thread_info_wait_multiple_handle(
    MonoThreadHandle **thread_handles, gsize nhandles,
    MonoOSEvent *background_change_event, gboolean waitall, guint32 timeout,
    gboolean alertable) {
  MonoOSEventWaitRet res;
  ...
  res = mono_os_event_wait_multiple(thread_events, nhandles, waitall, timeout,
                                    alertable);
  if (res >= MONO_OS_EVENT_WAIT_RET_SUCCESS_0 &&
      res <= MONO_OS_EVENT_WAIT_RET_SUCCESS_0 + nhandles - 1)
    return (MonoThreadInfoWaitRet)(MONO_THREAD_INFO_WAIT_RET_SUCCESS_0 +
                                   (res - MONO_OS_EVENT_WAIT_RET_SUCCESS_0));
  else if (res == MONO_OS_EVENT_WAIT_RET_ALERTED)
    return MONO_THREAD_INFO_WAIT_RET_ALERTED;
  else if (res == MONO_OS_EVENT_WAIT_RET_TIMEOUT)
    return MONO_THREAD_INFO_WAIT_RET_TIMEOUT;
 ...
 *
 *   This mutex is used to avoid races between pthread_create () and
 * pthread_join () on osx, see
...
gboolean mono_thread_info_set_tools_data(void *data) {
...
void *mono_thread_info_get_tools_data(void) {
  MonoThreadInfo *info = mono_thread_info_current_unchecked();
