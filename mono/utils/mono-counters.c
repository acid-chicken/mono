 ...
 *
 * Variance specifies how the counter value is expected to behave between any
 * two samplings.
 *
...
const char *mono_counter_get_name(MonoCounter *counter) {
  return counter->name;
}
 ...
 */
size_t mono_counter_get_size(MonoCounter *counter) { return counter->size; }
...
static void register_internal(const char *name, int type, void *addr,
                              int size) {
  MonoCounter *counter;
  ...
  for (counter = counters; counter; counter = counter->next) {
    if (counter->addr == addr) {
      g_warning("you are registering the same counter address twice: %s at %p",
                name, addr);
  ...
  for (register_callback = register_callbacks; register_callback;
       register_callback = register_callback->next)
 ...
 * \param type One of the possible \c MONO_COUNTER types, or \c
 * MONO_COUNTER_CALLBACK for a function pointer. \param addr The address to
 * register.
 *
 ...
 *
 * This function should not be used with counter types that require an explicit
 * size such as string as the counter size will be set to zero making them
 * effectively useless.
 *
 * It may be a function pointer if \c MONO_COUNTER_CALLBACK is specified:
 * the function should return the value and take no arguments.
 */
void mono_counters_register(const char *name, int type, void *addr) {
  int size;
 ...
 * \param name The name for this counters.
 * \param type One of the possible MONO_COUNTER types, or MONO_COUNTER_CALLBACK
 * for a function pointer. \param addr The address to register. \param size Max
 * size of the counter data.
 *
 ...
 * the function should return the value and take no arguments.
 *
 * The value of \p size is ignored for types with fixed size such as int and
 * long.
 *
 ...
 */
void mono_counters_register_with_size(const char *name, int type, void *addr,
                                      int size) {
  if (!initialized)
    g_debug("counters not enabled");
  else
...
static gint64 user_time(void) {
  return mono_process_get_data(GINT_TO_POINTER(mono_process_current_pid()),
                               MONO_PROCESS_USER_TIME);
}

static gint64 system_time(void) {
  return mono_process_get_data(GINT_TO_POINTER(mono_process_current_pid()),
                               MONO_PROCESS_SYSTEM_TIME);
}

static gint64 total_time(void) {
  return mono_process_get_data(GINT_TO_POINTER(mono_process_current_pid()),
                               MONO_PROCESS_TOTAL_TIME);
}

static gint64 working_set(void) {
  return mono_process_get_data(GINT_TO_POINTER(mono_process_current_pid()),
                               MONO_PROCESS_WORKING_SET);
}

static gint64 private_bytes(void) {
  return mono_process_get_data(GINT_TO_POINTER(mono_process_current_pid()),
                               MONO_PROCESS_PRIVATE_BYTES);
}

static gint64 virtual_bytes(void) {
  return mono_process_get_data(GINT_TO_POINTER(mono_process_current_pid()),
                               MONO_PROCESS_VIRTUAL_BYTES);
}

static gint64 page_faults(void) {
  return mono_process_get_data(GINT_TO_POINTER(mono_process_current_pid()),
                               MONO_PROCESS_FAULTS);
}

static gint64 paged_bytes(void) {
  return mono_process_get_data(GINT_TO_POINTER(mono_process_current_pid()),
                               MONO_PROCESS_PAGED_BYTES);
}

// If cpu_load gets inlined on Windows then cpu_load_1min, cpu_load_5min and
// cpu_load_15min can be folded into a single function and that will cause a
// failure when registering counters since the same function address will be
// used by all three functions. Preventing this method from being inlined will
// make sure the registered callback functions remains unique.
#ifdef _MSC_VER
__declspec(noinline)
#endif
    static double cpu_load(int kind) {
...
#define SYSCOUNTER_TIME                                                        \
  (MONO_COUNTER_SYSTEM | MONO_COUNTER_LONG | MONO_COUNTER_TIME |               \
   MONO_COUNTER_MONOTONIC | MONO_COUNTER_CALLBACK)
#define SYSCOUNTER_BYTES                                                       \
  (MONO_COUNTER_SYSTEM | MONO_COUNTER_LONG | MONO_COUNTER_BYTES |              \
   MONO_COUNTER_VARIABLE | MONO_COUNTER_CALLBACK)
#define SYSCOUNTER_COUNT                                                       \
  (MONO_COUNTER_SYSTEM | MONO_COUNTER_LONG | MONO_COUNTER_COUNT |              \
   MONO_COUNTER_MONOTONIC | MONO_COUNTER_CALLBACK)
#define SYSCOUNTER_LOAD                                                        \
  (MONO_COUNTER_SYSTEM | MONO_COUNTER_DOUBLE | MONO_COUNTER_PERCENTAGE |       \
   MONO_COUNTER_VARIABLE | MONO_COUNTER_CALLBACK)

static void initialize_system_counters(void) {
  register_internal("User Time", SYSCOUNTER_TIME, (gpointer)&user_time,
                    sizeof(gint64));
  register_internal("System Time", SYSCOUNTER_TIME, (gpointer)&system_time,
                    sizeof(gint64));
  register_internal("Total Time", SYSCOUNTER_TIME, (gpointer)&total_time,
                    sizeof(gint64));
  register_internal("Working Set", SYSCOUNTER_BYTES, (gpointer)&working_set,
                    sizeof(gint64));
  register_internal("Private Bytes", SYSCOUNTER_BYTES, (gpointer)&private_bytes,
                    sizeof(gint64));
  register_internal("Virtual Bytes", SYSCOUNTER_BYTES, (gpointer)&virtual_bytes,
                    sizeof(gint64));
  register_internal("Page File Bytes", SYSCOUNTER_BYTES, (gpointer)&paged_bytes,
                    sizeof(gint64));
  register_internal("Page Faults", SYSCOUNTER_COUNT, (gpointer)&page_faults,
                    sizeof(gint64));
  register_internal("CPU Load Average - 1min", SYSCOUNTER_LOAD,
                    (gpointer)&cpu_load_1min, sizeof(double));
  register_internal("CPU Load Average - 5min", SYSCOUNTER_LOAD,
                    (gpointer)&cpu_load_5min, sizeof(double));
  register_internal("CPU Load Average - 15min", SYSCOUNTER_LOAD,
                    (gpointer)&cpu_load_15min, sizeof(double));
}
 ...
 * Iterate over all counters and call \p cb for each one of them. Stop iterating
 * if the callback returns FALSE.
 */
void mono_counters_foreach(CountersEnumCallback cb, gpointer user_data) {
  MonoCounter *counter;
...
#define COPY_COUNTER(type, functype)                                           \
  do {                                                                         \
    size = sizeof(type);                                                       \
    if (buffer_size < size)                                                    \
      size = -1;                                                               \
      ...
      *(type *)buffer =                                                        \
          cb ? ((functype)counter->addr)() : *(type *)counter->addr;           \
  } while (0);

/* lockless */
static int sample_internal(MonoCounter *counter, void *buffer,
                           int buffer_size) {
...
// We want to default to g_print if outfile is not specified. This matters
// because on some platforms (e.g. Android), printing to stdout is not the
// actual way to see output, and we've wrapped that logic in `g_print`.
#define FPRINTF_OR_G_PRINT(outfile, ...)                                       \
  if (outfile) {                                                               \
...
static void dump_counter(MonoCounter *counter, FILE *outfile) {
  ...
  case MONO_COUNTER_STRING:
    FPRINTF_OR_G_PRINT(outfile, ENTRY_FMT "%s\n", counter->name,
                       (size == 0) ? "(null)" : (char *)buffer);
    break;
  case MONO_COUNTER_TIME_INTERVAL:
    FPRINTF_OR_G_PRINT(outfile, ENTRY_FMT "%.2f ms\n", counter->name,
                       (double)(*(gint64 *)buffer) / 1000.0);
    break;
  }
...
static const char section_names[][12] = {
    "JIT",      "GC",      "Metadata", "Generics",
    "Security", "Runtime", "System",
    "", // MONO_COUNTER_PERFCOUNTERS - not used.
    "Profiler", "Interp",  "Tiered",
};

static void mono_counters_dump_section(int section, int variance,
                                       FILE *outfile) {
  MonoCounter *counter = counters;
  while (counter) {
    if ((counter->type & section) &&
        (mono_counter_get_variance(counter) & variance))
 ...
 * To filter by variance, you can OR one or more variance with the specific
 * section you want. Use \c MONO_COUNTER_SECTION_MASK to dump all categories of
 * a specific variance.
 */
void mono_counters_dump(int section_mask, FILE *outfile) {
  ...
  for (j = 0, i = MONO_COUNTER_JIT; i < MONO_COUNTER_LAST_SECTION;
       j++, i <<= 1) {
 ...
 * This function sets the soft and hard limit for runtime resources. When the
 * limit is reached, a user-specified callback is called. The callback runs in a
 * restricted environment, in which the world coult be stopped, so it can't take
 * locks, perform allocations etc. The callback may be called multiple times
 * once a limit has been reached if action is not taken to decrease the resource
 * use. \returns 0 on error or a positive integer otherwise.
 */
int mono_runtime_resource_limit(int resource_type, uintptr_t soft_limit,
 ...
 * The callback will receive the resource type, the resource amount in
 * resource-specific units and a flag indicating whether the soft or hard limit
 * was reached.
 */
