...
// Returns true for check modes which are allowed by both the current DISABLE_
// macros and the MONO_CHECK_MODE env var. Argument may be a bitmask; if so,
// result is true if at least one specified mode is enabled.
...
static int mono_check_transition_limit(void) {
...
static gboolean backtrace_mutex_trylock(void) {
  return mono_os_mutex_trylock(&backtrace_mutex) == 0;
}

static void backtrace_mutex_unlock(void) {
  return mono_os_mutex_unlock(&backtrace_mutex);
}

static CheckState *get_state(void) {
  CheckState *state = (CheckState *)mono_native_tls_get_value(thread_status);
  if (!state) {
    state = (CheckState *)g_malloc0(sizeof(CheckState) +
                                    sizeof(ThreadTransition) * MAX_TRANSITIONS);
...
static void ringbuf_unpack(gint32 ringbuf, guint16 *buf_start,
                           guint16 *buf_end) {
  *buf_start = (guint16)(ringbuf >> 16);
  *buf_end = (guint16)(ringbuf & 0x00FF);
}

static gint32 ringbuf_pack(guint16 buf_start, guint16 buf_end) {
  return ((((gint32)buf_start) << 16) | ((gint32)buf_end));
}

static int ringbuf_size(guint32 ringbuf, int n) {
...
static guint16 ringbuf_push(gint32 *ringbuf, int n) {
...
static int collect_backtrace(gpointer out_data[]) {
#if defined(__GNUC__) && !defined(__clang__)
  /* GNU libc backtrace calls _Unwind_Backtrace in libgcc, which internally may
   * take a lock. */
   ...
   * suspend initiator: switches T2 to BLOCKING_SUSPEND_REQUESTED and sends a
   * suspend signal T1: calls mono_threads_transition_state_poll (), T1:
   * switches to SELF_SUSPENDED and starts trace_state_change () T2: is still in
   * checked_build_thread_transition for the RUNNING->BLOCKING transition and
   * calls backtrace () T2: suspend signal lands while T2 is in backtrace()
   * holding a lock; T2 switches to BLOCKING_ASYNC_SUSPENDED () and waits for
   * resume T1: calls backtrace (), waits for the lock () suspend initiator:
   * waiting for T1 to suspend.
   *
...
static char *translate_backtrace(gpointer native_trace[], int size) {
  if (size == 0)
    return g_strdup("");
...
static int collect_backtrace(gpointer out_data[]) { return 0; }
...
void checked_build_thread_transition(const char *transition, void *info,
                                     int from_state, int suspend_count,
                                     int next_state, int suspend_count_delta,
                                     gboolean capture_backtrace) {
  if (!mono_check_mode_enabled(MONO_CHECK_MODE_THREAD))
    return;

  /* We currently don't record external changes as those are hard to reason
   * about. */
...
void mono_fatal_with_history(const char *volatile msg, ...) {
  GString *err = g_string_sized_new(100);

  g_string_append_printf(err, "Assertion failure in thread %p due to: ",
  ...
  if (mono_check_mode_enabled(MONO_CHECK_MODE_THREAD)) {
    ...
    while (cur != end) {
      ...
      g_string_append_printf(
          ...
          mono_thread_state_name(t->next_state), t->suspend_count,
          t->suspend_count_delta > 0
              ? "+"
              : "", // I'd like to see this sort of values: -1, 0, +1
          t->suspend_count_delta, bt);
      g_free(bt);
...
void assert_gc_safe_mode(const char *file, int lineno) {
  ...
  if (!cur)
    mono_fatal_with_history(
        "%s:%d: Expected GC Safe mode but thread is not attached", file,
        lineno);
  ...
  case STATE_BLOCKING_SUSPEND_REQUESTED:
    break;
  default:
    mono_fatal_with_history("%s:%d: Expected GC Safe mode but was in %s state",
                            file, lineno, mono_thread_state_name(state));
  }
}
...
void assert_gc_unsafe_mode(const char *file, int lineno) {
  ...
  if (!cur)
    mono_fatal_with_history(
        "%s:%d: Expected GC Unsafe mode but thread is not attached", file,
        lineno);
  ...
  default:
    mono_fatal_with_history(
        "%s:%d: Expected GC Unsafe mode but was in %s state", file, lineno,
...
void assert_gc_neutral_mode(const char *file, int lineno) {
  ...
  if (!cur)
    mono_fatal_with_history(
        "%s:%d: Expected GC Neutral mode but thread is not attached", file,
        lineno);
  ...
  default:
    mono_fatal_with_history(
        "%s:%d: Expected GC Neutral mode but was in %s state", file, lineno,
        mono_thread_state_name(state));
  }
}
...
void assert_not_in_gc_critical_region(void) {
  ...
  if (state->in_gc_critical_region > 0) {
    mono_fatal_with_history(
        "Expected GC Unsafe mode, but was in %s state",
        mono_thread_state_name(
            mono_thread_info_current_state(mono_thread_info_current())));
  }
}
...
// check_metadata_store et al: The goal of these functions is to verify that if
// there is a pointer from one mempool into another, that the pointed-to memory
// is protected by the reference count mechanism for MonoImages.
//
// Note: The code below catches only some kinds of failures. Failures outside
// its scope notably incode:
// * Code below absolutely assumes that no mempool is ever held as "mempool"
// member by more than one Image or ImageSet at once
// * Code below assumes reference counts never underflow (ie: if we have a
// pointer to something, it won't be deallocated while we're looking at it)
...
static gboolean check_mempool_owner_eq(MonoMemPoolOwner a, MonoMemPoolOwner b) {
  return a.image == b.image && a.image_set == b.image_set;
}

// Say image X "references" image Y if X either contains Y in its modules field,
// or X’s "references" field contains an assembly whose image is Y. Say image X
// transitively references image Y if there is any chain of
// images-referencing-images which leads from X to Y. Once the mempools for two
// pointers have been looked up, there are four possibilities:

// Case 1. Image FROM points to Image TO: Legal if FROM transitively references
// TO

// We'll do a simple BFS graph search on images. For each image we visit:
static void check_image_search(GHashTable *visited, GPtrArray *next,
                               MonoImage *candidate, MonoImage *goal,
                               gboolean *success) {
...
static gboolean check_image_may_reference_image(MonoImage *from,
                                                MonoImage *to) {
  if (to == from) // Shortcut
    return TRUE;

  // Corlib is never unloaded, and all images implicitly reference it.
  // Some images avoid explicitly referencing it as an optimization, so
  ...
  // We allow this because the dynamic image code is known "messy", and in
  // theory it is already protected because dynamic images can only reference
  // classes their assembly has retained. However, long term, we should make
  // this rigorous.
  ...
  // Because in practice the image graph contains cycles, we must track which
  // images we've visited
  GHashTable *visited = g_hash_table_new(NULL, NULL);

#define CHECK_IMAGE_VISIT(i)                                                   \
  check_image_search(visited, next, (i), to, &success)

  CHECK_IMAGE_VISIT(from); // Initially "next" contains only from node

  // For each pass exhaust the "to check" queue while filling up the "check
  // next" queue
  while (!success &&
         next->len > 0) // Halt on success or when out of nodes to process
  {
    ...
    for (current_idx = 0; current_idx < current->len; current_idx++) {
      MonoImage *checking =
          (MonoImage *)g_ptr_array_index(current, current_idx);
      ...
      // 'files' and 'modules' semantically contain the same items but because
      // of lazy loading we must check both
      for (inner_idx = 0; !success && inner_idx < checking->file_count;
           inner_idx++) {
        CHECK_IMAGE_VISIT(checking->files[inner_idx]);
      }

      for (inner_idx = 0; !success && inner_idx < checking->module_count;
           inner_idx++) {
        CHECK_IMAGE_VISIT(checking->modules[inner_idx]);
      }

      for (inner_idx = 0; !success && inner_idx < checking->nreferences;
           inner_idx++) {
        // Assembly references are lazy-loaded and thus allowed to be NULL.
        // If they are NULL, we don't care about them for this search, because
        // their images haven't impacted ref_count yet.
...
// Case 2. ImageSet FROM points to Image TO: One of FROM's "images" either is,
// or transitively references, TO.
static gboolean check_image_set_may_reference_image(MonoImageSet *from,
                                                    MonoImage *to) {
...
// Case 3. ImageSet FROM points to ImageSet TO: The images in TO are a strict
// subset of FROM (no transitive relationship is important here)
static gboolean check_image_set_may_reference_image_set(MonoImageSet *from,
                                                        MonoImageSet *to) {
  if (to == from)
    return TRUE;
  ...
  for (to_idx = 0; valid && to_idx < to->nimages; to_idx++) {
    ...
    // If the to->images item is not found in from->images, the subset check has
    // failed
    if (!seen)
      valid = FALSE;
  }
...
// Case 4. Image FROM points to ImageSet TO: FROM transitively references *ALL*
// of the “images” listed in TO
static gboolean check_image_may_reference_image_set(MonoImage *from,
                                                    MonoImageSet *to) {
...
static const char *check_mempool_owner_name(MonoMemPoolOwner owner) {
...
// Helper -- surf various image-locating functions looking for the owner of this
// pointer
static MonoMemPoolOwner mono_find_mempool_owner(void *ptr) {
...
static void check_mempool_may_reference_mempool(void *from_ptr, void *to_ptr,
  ...
  MonoMemPoolOwner from = mono_find_mempool_owner(from_ptr),
                   to = mono_find_mempool_owner(to_ptr);

  if (require_local) {
    if (!check_mempool_owner_eq(from, to))
      check_mempool_assert_message("Pointer in image %s should have been "
                                   "internal, but instead pointed to image %s",
                                   check_mempool_owner_name(from),
                                   check_mempool_owner_name(to));
  }

  // Writing into unknown mempool
  else if (check_mempool_owner_eq(from, mono_mempool_no_owner)) {
    check_mempool_assert_message(
        "Non-image memory attempting to write pointer to image %s",
        check_mempool_owner_name(to));
  }
  ...
  // Reading from unknown mempool
    ...
    check_mempool_assert_message(
        "Attempting to write pointer from image %s to non-image memory",
        check_mempool_owner_name(from));
  }
  ...
  // Split out the four cases described above:
    ...
    if (!check_image_may_reference_image(from.image, to.image))
      check_mempool_assert_message("Image %s tried to point to image %s, but "
    ...
    if (!check_image_may_reference_image_set(from.image, to.image_set))
      check_mempool_assert_message("Image %s tried to point to image set %s, "
                                   "but does not retain a reference",
                                   check_mempool_owner_name(from),
                                   check_mempool_owner_name(to));
  }
    ...
    if (!check_image_set_may_reference_image_set(from.image_set, to.image_set))
      check_mempool_assert_message("Image set %s tried to point to image set "
                                   "%s, but does not retain a reference",
                                   check_mempool_owner_name(from),
                                   check_mempool_owner_name(to));
  }
    ...
    if (!check_image_set_may_reference_image(from.image_set, to.image))
      check_mempool_assert_message("Image set %s tried to point to image %s, "
                                   "but does not retain a reference",
                                   check_mempool_owner_name(from),
                                   check_mempool_owner_name(to));
  }
