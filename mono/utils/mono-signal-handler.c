...
#if defined(SIGBUS)
    {SIGBUS, "SIGBUS"} // How come this is seems not available on Android, but
                       // is used unconditionally in
                       // mini-posix.c:mono_runtime_posix_install_handlers ?
#endif
};
#endif
...
void mono_load_signames() {
  if (sigpair_buf)
    return;
#if defined(HAVE_SYSSIGNAME)
  sigpair_buflen = sizeof(sys_signame) / sizeof(sys_signame[0]);
  sigpair_buf = (struct mono_sigpair *)g_malloc(sigpair_buflen *
                                                sizeof(struct mono_sigpair));
