...
static pthread_mutex_t memory_barrier_process_wide_mutex =
...
gboolean mono_thread_platform_create_thread(MonoThreadStart thread_fn,
  ...
  if (res != 0)
    g_error("%s: pthread_attr_init failed, error: \"%s\" (%d)", __func__,
            g_strerror(res), res);
  ...
  if (res != 0)
    g_error("%s: pthread_attr_setstacksize failed, error: \"%s\" (%d)",
            __func__, g_strerror(res), res);
#endif /* HAVE_PTHREAD_ATTR_SETSTACKSIZE */

  /* Actually start the thread */
  res = mono_gc_pthread_create(&thread, &attr, (gpointer(*)(gpointer))thread_fn,
                               thread_data);
  if (res) {
    res = pthread_attr_destroy(&attr);
    if (res != 0)
      g_error("%s: pthread_attr_destroy failed, error: \"%s\" (%d)", __func__,
              g_strerror(res), res);
  ...
  if (stack_size) {
    ...
    if (res != 0)
      g_error("%s: pthread_attr_getstacksize failed, error: \"%s\" (%d)",
              __func__, g_strerror(res), res);
  }
...
gboolean mono_threads_platform_yield(void) { return sched_yield() == 0; }
...
int mono_threads_pthread_kill(MonoThreadInfo *info, int signum) {
  THREADS_SUSPEND_DEBUG("sending signal %d to %p[%p]\n", signum, info,
                        mono_thread_info_get_tid(info));

  const int signal_queue_ovf_retry_count G_GNUC_UNUSED = 5;
  const gulong signal_queue_ovf_sleep_us G_GNUC_UNUSED =
...
#ifdef USE_TKILL_ON_ANDROID
{
  ...
  if (result && result != ESRCH
    ...
    g_error("%s: pthread_kill failed with error %d - potential kernel OOM or "
            "signal queue overflow",
            __func__, result);

#if defined(__linux__)
  if (result == EAGAIN && retry_count < signal_queue_ovf_retry_count) {
    /* HACK: if the signal queue overflows on linux, try again a couple of
     * times. Tries to address https://github.com/dotnet/runtime/issues/32377
     */
    g_warning("%s: pthread_kill failed with error %d - potential kernel OOM or "
              "signal queue overflow, sleeping for %ld microseconds",
              __func__, result, signal_queue_ovf_sleep_us);
...
MonoNativeThreadId mono_native_thread_id_get(void) { return pthread_self(); }
...
void mono_memory_barrier_process_wide(void) {
  ...
  if (memory_barrier_process_wide_helper_page == NULL) {
    status = posix_memalign(&memory_barrier_process_wide_helper_page,
                            mono_pagesize(), mono_pagesize());
    g_assert(status == 0);
  }
  ...
  status = mono_mprotect(memory_barrier_process_wide_helper_page,
...
gboolean mono_threads_suspend_begin_async_suspend(MonoThreadInfo *info,
  ...
  int sig = interrupt_kernel ? mono_threads_suspend_get_abort_signal()
...
void mono_threads_suspend_abort_syscall(MonoThreadInfo *info) {
  ...
  if (mono_threads_pthread_kill(info,
