...
#ifdef _MSC_VER
// __builtin_unwind_init not available under MSVC but equivalent implementation
// is done using copy_stack_data_internal_win32_wrapper.
#define SAVE_REGS_ON_STACK                                                     \
  do {                                                                         \
  } while (0)
...
// FIXME: This would be more efficient if instead of instantiating the stack it
// just pushed a simple depth counter up and down, perhaps with a per-thread
// cookie in the high bits.
#ifdef ENABLE_CHECKED_BUILD_GC

// Maintains a single per-thread stack of ints, used to ensure nesting is not
// violated
static MonoNativeTlsKey coop_reset_count_stack_key;

static void coop_tls_push(gpointer cookie) {
  GArray *stack;
...
static void coop_tls_pop(gpointer received_cookie) {
  GArray *stack;
...
static void check_info(MonoThreadInfo *info, const gchar *action,
                       const gchar *state, const char *func) {
  if (!info)
    g_error("%s Cannot %s GC %s region if the thread is not attached", func,
            action, state);
  if (!mono_thread_info_is_current(info))
    g_error("%s [%p] Cannot %s GC %s region on a different thread", func,
            mono_thread_info_get_tid(info), action, state);
  if (!mono_thread_info_is_live(info))
    g_error("%s [%p] Cannot %s GC %s region if the thread is not live", func,
            mono_thread_info_get_tid(info), action, state);
}
...
static void mono_threads_state_poll_with_info(MonoThreadInfo *info);
...
static void mono_threads_state_poll_with_info(MonoThreadInfo *info) {
  ...
  ++coop_save_count;
  mono_threads_get_runtime_callbacks()->thread_state_init(
      &info->thread_saved_state[SELF_SUSPEND_STATE_INDEX]);
...
static MONO_NEVER_INLINE void *return_stack_ptr(gpointer *i) {
  dummy_global = i;
  return i;
}

static void copy_stack_data_internal(MonoThreadInfo *info,
                                     MonoStackData *stackdata_begin,
  ...
  stackdata_size = (char *)mono_stackdata_get_stackpointer(stackdata_begin) -
                   (char *)stackdata_end;

  const char *function_name = mono_stackdata_get_function_name(stackdata_begin);

  if (((gsize)stackdata_begin & (SIZEOF_VOID_P - 1)) != 0)
    g_error("%s stackdata_begin (%p) must be %d-byte aligned", function_name,
            stackdata_begin, SIZEOF_VOID_P);
  if (((gsize)stackdata_end & (SIZEOF_VOID_P - 1)) != 0)
    g_error("%s stackdata_end (%p) must be %d-byte aligned", function_name,
            stackdata_end, SIZEOF_VOID_P);

  if (stackdata_size <= 0)
    g_error("%s stackdata_size = %d, but must be > 0, stackdata_begin = %p, "
            "stackdata_end = %p",
            function_name, stackdata_size, stackdata_begin, stackdata_end);
...
#ifdef _MSC_VER
typedef void (*CopyStackDataFunc)(MonoThreadInfo *, MonoStackData *,
                                  gconstpointer, gconstpointer);

#ifdef TARGET_AMD64
// Implementation of __builtin_unwind_init under MSVC, dumping nonvolatile
// registers into MonoBuiltinUnwindInfo.
typedef struct {
  __m128d fregs[10];
...
G_EXTERN_C void copy_stack_data_internal_win32_wrapper(MonoThreadInfo *,
                                                       MonoStackData *,
                                                       MonoBuiltinUnwindInfo *,
                                                       CopyStackDataFunc);
#else
...
// Implementation of __builtin_unwind_init under MSVC, dumping nonvolatile
// registers into MonoBuiltinUnwindInfo *.
__declspec(naked) void __cdecl copy_stack_data_internal_win32_wrapper(
    MonoThreadInfo *info, MonoStackData *stackdata_begin,
    MonoBuiltinUnwindInfo *unwind_info_data, CopyStackDataFunc func) {
  __asm {
...
static void copy_stack_data(MonoThreadInfo *info,
                            MonoStackData *stackdata_begin) {
  MonoBuiltinUnwindInfo unwind_info_data;
  copy_stack_data_internal_win32_wrapper(
      info, stackdata_begin, &unwind_info_data, copy_stack_data_internal);
}
#else
static void copy_stack_data(MonoThreadInfo *info,
                            MonoStackData *stackdata_begin) {
...
static gpointer mono_threads_enter_gc_safe_region_unbalanced_with_info(
    MonoThreadInfo *info, MonoStackData *stackdata);

gpointer mono_threads_enter_gc_safe_region_internal(MonoStackData *stackdata) {
  return mono_threads_enter_gc_safe_region_with_info(
      mono_thread_info_current_unchecked(), stackdata);
}

gpointer mono_threads_enter_gc_safe_region(gpointer *stackpointer) {
  MONO_STACKDATA(stackdata);
  stackdata.stackpointer = stackpointer;
  return mono_threads_enter_gc_safe_region_internal(&stackdata);
}

gpointer mono_threads_enter_gc_safe_region_with_info(MonoThreadInfo *info,
  ...
  cookie =
      mono_threads_enter_gc_safe_region_unbalanced_with_info(info, stackdata);
...
gpointer mono_threads_enter_gc_safe_region_unbalanced_internal(
    MonoStackData *stackdata) {
  return mono_threads_enter_gc_safe_region_unbalanced_with_info(
      mono_thread_info_current_unchecked(), stackdata);
}

gpointer mono_threads_enter_gc_safe_region_unbalanced(gpointer *stackpointer) {
  MONO_STACKDATA(stackdata);
  stackdata.stackpointer = stackpointer;
...
static gpointer mono_threads_enter_gc_safe_region_unbalanced_with_info(
  ...
  // NOTE, copy_stack_data needs to be done. One problem it solves is
  // optimization taking place between stackdata snapshot and thread_state_init,
  // storing changed register(s) on stack and if those register(s) include
  // managed references (that are not previously stored anywhere on the stack),
  // then GC won't detect that reference(s). Storing the stack and registers
  // into a separate location makes sure we still see any registers temporary
  // stored on stack due to optimizations done between stackdata snapshot and
  // thread_state_init.
...
void mono_threads_exit_gc_safe_region_internal(gpointer cookie,
                                               MonoStackData *stackdata) {
  if (!mono_threads_is_blocking_transition_enabled())
    return;
...
void mono_threads_exit_gc_safe_region(gpointer cookie, gpointer *stackpointer) {
  MONO_STACKDATA(stackdata);
  stackdata.stackpointer = stackpointer;
  mono_threads_exit_gc_safe_region_internal(cookie, &stackdata);
}

void mono_threads_exit_gc_safe_region_unbalanced_internal(
    gpointer cookie, MonoStackData *stackdata) {
  MonoThreadInfo *info;
...
void mono_threads_exit_gc_safe_region_unbalanced(gpointer cookie,
...
mono_threads_enter_gc_unsafe_region_internal(MonoStackData *stackdata) {
  return mono_threads_enter_gc_unsafe_region_with_info(
      mono_thread_info_current_unchecked(), stackdata);
}

gpointer mono_threads_enter_gc_unsafe_region(gpointer *stackpointer) {
  MONO_STACKDATA(stackdata);
  stackdata.stackpointer = stackpointer;
...
gpointer
mono_threads_enter_gc_unsafe_region_with_info(THREAD_INFO_TYPE *info,
                                              MonoStackData *stackdata) {
  gpointer cookie;
  ...
  cookie =
      mono_threads_enter_gc_unsafe_region_unbalanced_with_info(info, stackdata);
...
gpointer mono_threads_enter_gc_unsafe_region_unbalanced_internal(
    MonoStackData *stackdata) {
  return mono_threads_enter_gc_unsafe_region_unbalanced_with_info(
      mono_thread_info_current_unchecked(), stackdata);
}
...
void mono_threads_exit_gc_unsafe_region_internal(gpointer cookie,
                                                 MonoStackData *stackdata) {
  if (!mono_threads_is_blocking_transition_enabled())
    return;
...
void mono_threads_exit_gc_unsafe_region(gpointer cookie,
                                        gpointer *stackpointer) {
  MONO_STACKDATA(stackdata);
  stackdata.stackpointer = stackpointer;
...
void mono_threads_exit_gc_unsafe_region_unbalanced_internal(
    gpointer cookie, MonoStackData *stackdata) {
  if (!mono_threads_is_blocking_transition_enabled())
    return;
...
void mono_threads_exit_gc_unsafe_region_unbalanced(gpointer cookie,
                                                   gpointer *stackpointer) {
  MONO_STACKDATA(stackdata);
  stackdata.stackpointer = stackpointer;
  mono_threads_exit_gc_unsafe_region_unbalanced_internal(cookie, &stackdata);
...
void mono_threads_assert_gc_unsafe_region(void) { MONO_REQ_GC_UNSAFE_MODE; }

static int threads_suspend_policy_default(void) {
 ...
 * alternative
 */
static gboolean hasenv_obsolete(const char *name, const char *newval) {
  // If they already set MONO_THREADS_SUSPEND to something, maybe they're
  // keeping the old var set for compatability with old Mono - in that case
  // don't nag.
  ...
  if (g_hasenv(name)) {
    ...
    if (!quiet)
      g_warning(
          "%s environment variable is obsolete.  Use MONO_THREADS_SUSPEND=%s",
          name, newval);
    return TRUE;
  }
  return FALSE;
...
static int threads_suspend_policy_getenv_compat(void) {
  int policy = 0;
  if (hasenv_obsolete("MONO_ENABLE_COOP", "coop") ||
      hasenv_obsolete("MONO_ENABLE_COOP_SUSPEND", "coop")) {
    g_assertf(!hasenv_obsolete("MONO_ENABLE_HYBRID_SUSPEND", "hybrid"),
              "Environment variables set to enable both hybrid and cooperative "
              "suspend simultaneously");
...
static int threads_suspend_policy_getenv(void) {
  int policy = 0;
  if (g_hasenv("MONO_THREADS_SUSPEND")) {
    ...
    if (!strcmp(str, "coop"))
      ...
      g_error("MONO_THREADS_SUSPEND environment variable set to '%s', must be "
              "one of coop, hybrid, preemptive.",
              str);
    g_free(str);
  }
  return policy;
...
void mono_threads_suspend_policy_init(void) {
  int policy = 0;
  {
...
void mono_threads_suspend_override_policy(MonoThreadsSuspendPolicy new_policy) {
  mono_threads_suspend_policy_hidden_dont_modify =
      (char)mono_threads_suspend_validate_policy(new_policy);
  g_warning("Overriding suspend policy.  Using %s suspend.",
            mono_threads_suspend_policy_name(mono_threads_suspend_policy()));
}

const char *mono_threads_suspend_policy_name(MonoThreadsSuspendPolicy policy) {
  switch (policy) {
  case MONO_THREADS_SUSPEND_FULL_COOP:
    return "cooperative";
  case MONO_THREADS_SUSPEND_FULL_PREEMPTIVE:
...
gboolean mono_threads_is_cooperative_suspension_enabled(void) {
  return (mono_threads_suspend_policy() == MONO_THREADS_SUSPEND_FULL_COOP);
}

gboolean mono_threads_is_hybrid_suspension_enabled(void) {
...
void mono_threads_coop_init(void) {
  ...
  mono_counters_register("Coop Reset Blocking",
                         MONO_COUNTER_GC | MONO_COUNTER_INT,
                         &coop_reset_blocking_count);
  mono_counters_register("Coop Try Blocking",
                         MONO_COUNTER_GC | MONO_COUNTER_INT,
                         &coop_try_blocking_count);
  mono_counters_register("Coop Do Blocking", MONO_COUNTER_GC | MONO_COUNTER_INT,
                         &coop_do_blocking_count);
  mono_counters_register("Coop Do Polling", MONO_COUNTER_GC | MONO_COUNTER_INT,
                         &coop_do_polling_count);
  mono_counters_register("Coop Save Count", MONO_COUNTER_GC | MONO_COUNTER_INT,
                         &coop_save_count);
...
void mono_threads_enter_no_safepoints_region(const char *func) {
  MONO_REQ_GC_UNSAFE_MODE;
  mono_threads_transition_begin_no_safepoints(mono_thread_info_current(), func);
}

void mono_threads_exit_no_safepoints_region(const char *func) {
  MONO_REQ_GC_UNSAFE_MODE;
  mono_threads_transition_end_no_safepoints(mono_thread_info_current(), func);
}

void mono_thread_set_coop_aware(void) {
...
mono_bool mono_thread_get_coop_aware(void) {
...
void mono_threads_set_runtime_startup_finished(void) {
