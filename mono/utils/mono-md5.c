 ...
 * mono_md5_init:
 */
void mono_md5_init(MonoMD5Context *ctx) { CC_MD5_Init(ctx); }
 ...
 */
static void _byte_reverse(guchar *buf, guint32 longs) {
  guint32 t;
  do {
 ...
 */
void mono_md5_get_digest(const guchar *buffer, gint buffer_size,
                         guchar digest[16]) {
  MonoMD5Context ctx;
