 ...
 * and trace mask.
 */
void mono_tracev_inner(GLogLevelFlags level, MonoTraceMask mask,
                       const char *format, va_list args) {
  char *log_message;
  if (level_stack == NULL) {
    mono_trace_init();
    if (level > mono_internal_current_level ||
        !(mask & mono_internal_current_mask))
      return;
  }

  g_assert(
      logCallback.opener); // mono_trace_init should have provided us with one!
 ...
 * value.
 */
void mono_trace_set_level(GLogLevelFlags level) {
  if (level_stack == NULL)
    mono_trace_init();
 ...
 * value.
 */
void mono_trace_set_mask(MonoTraceMask mask) {
  if (level_stack == NULL)
    mono_trace_init();
 ...
 * syslog.
 */
void mono_trace_set_logdest_string(const char *dest) {
  ...
  if (dest && !strcmp("flight-recorder", dest)) {
    ...
    if (mono_internal_current_level == G_LOG_LEVEL_ERROR ||
        mono_internal_current_level == G_LOG_LEVEL_CRITICAL)
 ...
 */
void mono_trace_set_logheader_string(const char *head) {
  if (head == NULL) {
 ...
 */
void mono_trace_push(GLogLevelFlags level, MonoTraceMask mask) {
  if (level_stack == NULL)
    g_error(
        "%s: cannot use mono_trace_push without calling mono_trace_init first.",
        __func__);
  else {
    MonoLogLevelEntry *entry =
        (MonoLogLevelEntry *)g_malloc(sizeof(MonoLogLevelEntry));
...
void mono_trace_pop(void) {
  if (level_stack == NULL)
    ...
    if (!g_queue_is_empty(level_stack)) {
      MonoLogLevelEntry *entry =
          (MonoLogLevelEntry *)g_queue_pop_head(level_stack);
...
void mono_trace_set_level_string(const char *value) {
  int i = 0;
  const char *valid_vals[] = {"error", "critical", "warning", "message",
                              "info",  "debug",    NULL};
  const GLogLevelFlags valid_ids[] = {G_LOG_LEVEL_ERROR,   G_LOG_LEVEL_CRITICAL,
...
void mono_trace_set_mask_string(const char *value) {
  ...
  static const struct {
    ...
    const MonoTraceMask mask;
      ...
      {"io-layer",
       (MonoTraceMask)(MONO_TRACE_IO_LAYER_PROCESS |
                       MONO_TRACE_IO_LAYER_SOCKET | MONO_TRACE_IO_LAYER_FILE |
                       MONO_TRACE_IO_LAYER_EVENT |
                       MONO_TRACE_IO_LAYER_SEMAPHORE |
                       MONO_TRACE_IO_LAYER_MUTEX | MONO_TRACE_IO_LAYER_HANDLE)},
      {"w32handle", MONO_TRACE_IO_LAYER_HANDLE},
      {"tailcall", MONO_TRACE_TAILCALL},
      {"profiler", MONO_TRACE_PROFILER},
      {"tiered", MONO_TRACE_TIERED},
      {"all", (MonoTraceMask)~0}, // FIXMEcxx there is a better way -- operator
                                  // overloads of enums
  ...
  while (*tok) {
    ...
    for (i = 0; flag_mask_map[i].flag; i++) {
      ...
      if (strncmp(tok, flag_mask_map[i].flag, len) == 0 &&
          (tok[len] == 0 || tok[len] == ',')) {
 ...
 */
gboolean mono_trace_is_traced(GLogLevelFlags level, MonoTraceMask mask) {
 ...
 */
static const char *log_level_get_name(GLogLevelFlags log_level) {
 ...
 *
 * This adapts the old callback writer exposed by MonoCallback to the newer
 * method of logging. We ignore the header request as legacy handlers never had
 * headers.
 */
static void callback_adapter(const char *domain, GLogLevelFlags level,
                             mono_bool fatal, const char *message) {
  UserSuppliedLoggerUserData *ll =
      (UserSuppliedLoggerUserData *)logCallback.user_data;

  ll->legacy_callback(domain, log_level_get_name(level), message, fatal,
                      ll->user_data);
}

static void eglib_log_adapter(const gchar *log_domain, GLogLevelFlags log_level,
                              const gchar *message, gpointer user_data) {
  UserSuppliedLoggerUserData *ll =
      (UserSuppliedLoggerUserData *)logCallback.user_data;

  ll->legacy_callback(log_domain, log_level_get_name(log_level), message,
                      log_level & G_LOG_LEVEL_ERROR, ll->user_data);
}
 ...
 */
static void legacy_opener(const char *path, void *user_data) {
  /* nothing to do */
}
 ...
 */
static void legacy_closer(void) {
  if (logCallback.user_data != NULL) {
    g_free(logCallback
               .user_data); /* This is a UserSuppliedLoggerUserData struct */
 ...
 *  @user_data Argument passed to @callback
 *
 * The log handler replaces the default runtime logger. All logging requests
 * with be routed to it. If the fatal argument in the callback is true, the
 * callback must abort the current process. The runtime expects that execution
 * will not resume after a fatal error.
 */
void mono_trace_set_log_handler(MonoLogCallback callback, void *user_data) {
  ...
  if (logCallback.closer != NULL)
    logCallback.closer();
  UserSuppliedLoggerUserData *ll = (UserSuppliedLoggerUserData *)g_malloc(
      sizeof(UserSuppliedLoggerUserData));
...
static void structured_log_adapter(const gchar *log_domain,
 ...
 * will not resume after a fatal error.
 */
void mono_trace_set_log_handler_internal(MonoLogCallParm *callback,
                                         void *user_data) {
  g_assert(callback);
  if (logCallback.closer != NULL)
    logCallback.closer();
...
static void print_handler(const char *string) { print_callback(string, TRUE); }

static void printerr_handler(const char *string) {
  printerr_callback(string, FALSE);
}

/**
 * mono_trace_set_print_handler:
 * \param callback The callback that will replace the default runtime behavior
 * for stdout output. The print handler replaces the default runtime stdout
 * output handler. This is used by free form output done by the runtime.
 */
 ...
 * \param callback The callback that will replace the default runtime behavior
 * for stderr output. The print handler replaces the default runtime stderr
 * output handler. This is used by free form output done by the runtime.
 */
...
static gchar conv_ascii_char(gchar s) {
  if (s < 0x20)
    return '.';
  if (s > 0x7e)
    return '.';
