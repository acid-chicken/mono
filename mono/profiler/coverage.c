 ...
 *
 * Elements:
 *   <coverage> - The root element of the documentation. It can contain any
 * number of <assembly>, <class> or <method> elements. Attributes:
 *                   - version: The version number for the file format - (eg:
 * "0.3") <assembly> - Contains data about assemblies. Has no child elements
 *                Attributes:
 ...
 *                - full: The number of fully covered methods
 *                - partial: The number of partially covered methods
 *   <method> - Contains data about methods. Can contain any number of
 * <statement> elements Attributes:
 ...
 *                 - filename: The name of the source file containing this
 * method
 *                 - token
...
static gint32 coverage_methods_ctr, coverage_statements_ctr,
    coverage_classes_ctr, coverage_assemblies_ctr;
...
static char *escape_string_for_xml(const char *string) {
...
static void free_coverage_entry(gpointer data, gpointer userdata) {
  CoverageEntry *entry = (CoverageEntry *)data;
  g_free(entry->filename);
  g_free(entry);
}

static void obtain_coverage_for_method(MonoProfiler *prof,
                                       const MonoProfilerCoverageData *entry) {
...
static char *parse_generic_type_names(char *name) {
  char *new_name, *ret;
...
static void dump_method(gpointer key, gpointer value, gpointer userdata) {
  MonoMethod *method = (MonoMethod *)value;
  MonoClass *klass;
  MonoImage *image;
  char *class_name, *escaped_image_name, *escaped_class_name,
      *escaped_method_name, *escaped_method_signature, *escaped_method_filename;
  ...
  mono_profiler_get_coverage_data(coverage_profiler.handle, method,
  ...
  class_name = parse_generic_type_names(
  ...
  g_string_append_printf(s,
                         "\t<method assembly=\"%s\" class=\"%s\" name=\"%s "
                         "(%s)\" filename=\"%s\" token=\"%d\">\n",
                         escaped_image_name, escaped_class_name,
                         escaped_method_name, escaped_method_signature,
                         escaped_method_filename,
  ...
  for (i = 0; i < coverage_profiler.data->len; i++) {
    ...
    g_string_append_printf(s,
                           "\t\t<statement offset=\"%d\" counter=\"%d\" "
                           "line=\"%d\" column=\"%d\"/>\n",
                           entry->offset, entry->counter, entry->line,
                           entry->column);
  }
...
static guint count_queue(MonoLockFreeQueue *queue) {
  MonoLockFreeQueueNode *node;
  guint count = 0;
...
static void dump_classes_for_image(gpointer key, gpointer value,
                                   gpointer userdata) {
  ...
  if (!image_name ||
      strcmp(image_name, mono_image_get_name(((MonoImage *)userdata))) != 0)
    return;
...
static void get_coverage_for_image(MonoImage *image, int *number_of_methods,
                                   guint *fully_covered,
                                   int *partially_covered) {
  MonoLockFreeQueue *image_methods =
      (MonoLockFreeQueue *)mono_conc_hashtable_lookup(
          coverage_profiler.image_to_methods, image);
...
static void dump_assembly(gpointer key, gpointer value, gpointer userdata) {
  ...
  get_coverage_for_image(image, &number_of_methods, &fully_covered,
                         &partially_covered);
  ...
  g_string_append_printf(s,
                         "\t<assembly name=\"%s\" guid=\"%s\" filename=\"%s\" "
                         "method-count=\"%d\" full=\"%d\" partial=\"%d\"/>\n",
                         escaped_image_name, image_guid, escaped_image_filename,
                         number_of_methods, fully_covered, partially_covered);

  g_free(escaped_image_name);
  g_free(escaped_image_filename);

  mono_conc_hashtable_foreach(coverage_profiler.class_to_methods,
                              dump_classes_for_image, image);
}

static void dump_coverage(MonoProfiler *prof) {
  GString *s;
  ...
  mono_conc_hashtable_foreach(coverage_profiler.assemblies, dump_assembly,
                              NULL);
  ...
  if (prof->config->send_to) {
    ...
    while (g_hash_table_iter_next(&iter, (void **)&image, (void **)&id)) {
      send_method =
          mono_method_desc_search_in_image(prof->config->send_to, image);
      if (send_method)
        break;
    }
    if (!send_method) {
      mono_profiler_printf_err("Cannot find method in loaded assemblies: '%s'.",
                               prof->config->send_to_str);
      exit(1);
    }
    ...
    if (sig->param_count != 2 || sig->params[0]->type != MONO_TYPE_STRING ||
      ...
      mono_profiler_printf_err(
          "Method '%s' should have signature void (string,string).",
          prof->config->send_to_str);
      exit(1);
    }
    ...
    if (prof->config->send_to_arg) {
      extra_arg = mono_string_new_checked(mono_domain_get(),
                                          prof->config->send_to_arg, error);
      mono_error_assert_ok(error);
    }

    MonoString *data =
        mono_string_new_checked(mono_domain_get(), s->str, error);
    mono_error_assert_ok(error);
    ...
    printf("aot-profiler | Passing data to '%s': %s\n",
           mono_method_full_name(send_method, 1),
           prof->config->send_to_arg ? prof->config->send_to_arg : "(null)");
...
static MonoLockFreeQueueNode *create_method_node(MonoMethod *method) {
...
static gboolean consider_image(MonoImage *image) {
  // Don't handle coverage for the core assemblies
  if (mono_conc_hashtable_lookup(coverage_profiler.suppressed_assemblies,
                                 (gpointer)mono_image_get_name(image)) != NULL)
    return FALSE;

  return TRUE;
}

static gboolean consider_class(MonoImage *image, MonoClass *klass) {
  if (coverage_profiler.filters) {
    ...
    char *fqn =
        g_strdup_printf("[%s]%s", mono_image_get_name(image), classname);
    ...
    if (has_positive && !found) {
      mono_os_mutex_lock(&coverage_profiler.mutex);
      mono_conc_hashtable_insert(coverage_profiler.filtered_classes, klass,
                                 klass);
      mono_os_mutex_unlock(&coverage_profiler.mutex);
      g_free(fqn);
      g_free(classname);
...
static MonoLockFreeQueue *register_image(MonoImage *image) {
   ...
   */
  image_methods = (MonoLockFreeQueue *)mono_conc_hashtable_lookup(
      coverage_profiler.image_to_methods, image);

  if (image_methods == NULL) {
    image_methods = (MonoLockFreeQueue *)g_malloc(sizeof(MonoLockFreeQueue));
    mono_lock_free_queue_init(image_methods);
    mono_conc_hashtable_insert(coverage_profiler.image_to_methods, image,
                               image_methods);
    ...
    mono_conc_hashtable_insert(coverage_profiler.assemblies, assembly,
                               assembly);
  }
...
static MonoLockFreeQueue *register_class(MonoClass *klass) {
  // First try the fast path...
  MonoLockFreeQueue *class_methods =
      (MonoLockFreeQueue *)mono_conc_hashtable_lookup(
          coverage_profiler.class_to_methods, klass);
   ...
   * check again now that we're holding the lock.
   */
  class_methods = (MonoLockFreeQueue *)mono_conc_hashtable_lookup(
      coverage_profiler.class_to_methods, klass);

  if (class_methods == NULL) {
    class_methods = (MonoLockFreeQueue *)g_malloc(sizeof(MonoLockFreeQueue));
    mono_lock_free_queue_init(class_methods);
    mono_conc_hashtable_insert(coverage_profiler.class_to_methods, klass,
                               class_methods);
  }
...
static void assembly_loaded(MonoProfiler *prof, MonoAssembly *assembly) {
  ...
  if (!mono_atomic_load_i32(&coverage_profiler.runtime_inited)) {
     ...
     */
    mono_assembly_addref(assembly);
    g_hash_table_insert(coverage_profiler.deferred_assemblies, assembly,
                        assembly);
    return;
  }
  ...
  for (int i = 1; i <= rows; i++) {
    ...
    MonoClass *klass =
        mono_class_get_checked(image, MONO_TOKEN_TYPE_DEF | i, error);
...
static void process_deferred_assembly(gpointer key, gpointer value,
                                      gpointer userdata) {
...
static gboolean coverage_filter(MonoProfiler *prof, MonoMethod *method) {
  guint32 iflags, flags;

  if (prof->done)
    return FALSE;
  if (prof->config->write_at &&
      mono_method_desc_match(prof->config->write_at, method)) {
...
static char *get_file_content(const gchar *filename) {
  char *buffer;
  ssize_t bytes_read;
  long filesize;
  int res, offset = 0;
...
static char *get_next_line(char *contents, char **next_start) {
  char *p = contents;
...
static void init_suppressed_assemblies(void) {
  char *content;
  char *line;

  coverage_profiler.suppressed_assemblies =
      mono_conc_hashtable_new(g_str_hash, g_str_equal);

  /* Don't need to free content as it is referred to by the lines stored in
   * @filters */
  content =
      get_file_content(SUPPRESSION_DIR "/mono-profiler-coverage.suppression");
  if (content == NULL)
    return;
  ...
  while ((line = get_next_line(content, &content))) {
    ...
    mono_conc_hashtable_insert(coverage_profiler.suppressed_assemblies, line,
                               line);
  }
}

static void parse_cov_filter_file(GPtrArray *filters, const char *file) {
  char *content;
  char *line;
...
static void unref_coverage_assemblies(gpointer key, gpointer value,
                                      gpointer userdata) {
  MonoAssembly *assembly = (MonoAssembly *)value;
  mono_assembly_close(assembly);
}

static void cov_shutdown(MonoProfiler *prof) {
  mono_atomic_store_i32(&coverage_profiler.in_shutdown, TRUE);

  dump_coverage(prof);

  mono_os_mutex_lock(&coverage_profiler.mutex);
  mono_conc_hashtable_foreach(coverage_profiler.assemblies,
                              unref_coverage_assemblies, NULL);
  mono_os_mutex_unlock(&coverage_profiler.mutex);
...
static void runtime_initialized(MonoProfiler *profiler) {
  mono_atomic_store_i32(&coverage_profiler.runtime_inited, TRUE);

  mono_counters_register("Event: Coverage methods",
                         MONO_COUNTER_UINT | MONO_COUNTER_PROFILER |
                             MONO_COUNTER_MONOTONIC,
                         &coverage_methods_ctr);
  mono_counters_register("Event: Coverage statements",
                         MONO_COUNTER_UINT | MONO_COUNTER_PROFILER |
                             MONO_COUNTER_MONOTONIC,
                         &coverage_statements_ctr);
  mono_counters_register("Event: Coverage classes",
                         MONO_COUNTER_UINT | MONO_COUNTER_PROFILER |
                             MONO_COUNTER_MONOTONIC,
                         &coverage_classes_ctr);
  mono_counters_register("Event: Coverage assemblies",
                         MONO_COUNTER_UINT | MONO_COUNTER_PROFILER |
                             MONO_COUNTER_MONOTONIC,
                         &coverage_assemblies_ctr);

  // See the comment in assembly_loaded ().
  g_hash_table_foreach(coverage_profiler.deferred_assemblies,
                       process_deferred_assembly, profiler);
}

static void usage(void);

static gboolean match_option(const char *arg, const char *opt_name,
                             const char **rval) {
  if (rval) {
    ...
    *rval = NULL;
    ...
    if (strncmp(arg, opt_name, strlen(opt_name)) ||
        (end - arg) > strlen(opt_name) + 1)
      return FALSE;
    *rval = end + 1;
    return TRUE;
  } else {
    // FIXME how should we handle passing a value to an arg that doesn't expect
    // it?
    return !strcmp(arg, opt_name);
  }
}
...
static void parse_args(const char *desc) {
  const char *p;
...
static void usage(void) {
  ...
  mono_profiler_printf(
      "Usage: mono --profile=coverage[:OPTION1[,OPTION2...]] program.exe\n");
  mono_profiler_printf("Options:");
  mono_profiler_printf("\thelp                 show this usage info");

  // mono_profiler_printf ("\tcovfilter=ASSEMBLY   add ASSEMBLY to the code
  // coverage filters"); mono_profiler_printf ("\t                     prefix a
  // + to include the assembly or a - to exclude it"); mono_profiler_printf ("\t
  // e.g. covfilter=-mscorlib");
  mono_profiler_printf("\tcovfilter-file=FILE  use FILE to generate the list "
                       "of assemblies to be filtered");
  mono_profiler_printf("\toutput=FILENAME      write the data to file FILENAME "
                       "(the file is always overwritten)");
  mono_profiler_printf("\toutput=+FILENAME     write the data to file "
                       "FILENAME.pid (the file is always overwritten)");
  mono_profiler_printf(
      "\toutput=|PROGRAM      write the data to the stdin of PROGRAM");
  mono_profiler_printf(
      "\toutput=|PROGRAM      write the data to the stdin of PROGRAM");
  mono_profiler_printf(
  ...
  mono_profiler_printf(
      "\tsend-to-arg=STR      extra argument to pass to METHOD.");
  // mono_profiler_printf ("\tzip                  compress the output data");

  exit(0);
}

MONO_API void mono_profiler_init_coverage(const char *desc);

void mono_profiler_init_coverage(const char *desc) {
  if (mono_jit_aot_compiling()) {
    mono_profiler_printf_err("The coverage profiler does not currently support "
                             "instrumenting AOT code.");
    exit(1);
  }
  ...
  if (!coverage_config.output_filename)
    ...
    coverage_config.output_filename =
        g_strdup_printf("%s.%d", coverage_config.output_filename + 1, getpid());

  if (*coverage_config.output_filename == '|')
    coverage_profiler.file = popen(coverage_config.output_filename + 1, "w");
  else if (*coverage_config.output_filename == '#')
    coverage_profiler.file =
        fdopen(strtol(coverage_config.output_filename + 1, NULL, 10), "a");
  else
    coverage_profiler.file = fopen(coverage_config.output_filename, "w");

  if (!coverage_profiler.file) {
    mono_profiler_printf_err(
        "Could not create coverage profiler output file '%s': %s",
        coverage_config.output_filename, g_strerror(errno));
    exit(1);
  }
  ...
  MonoProfilerHandle handle = coverage_profiler.handle =
      mono_profiler_create(&coverage_profiler);
