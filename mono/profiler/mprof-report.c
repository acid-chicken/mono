/*
 * mprof-report.c: mprof-report program source: decode and analyze the log
 * profiler data
 *
 * Authors:
...
static int32_t read_int16(unsigned char *p) {
  int32_t value = *p++;
  value |= (*p++) << 8;
  return value;
}

static int32_t read_int32(unsigned char *p) {
  int32_t value = *p++;
  value |= (*p++) << 8;
  value |= (*p++) << 16;
  value |= (uint32_t)(*p++) << 24;
...
static int64_t read_int64(unsigned char *p) {
  uint64_t value = *p++;
  value |= (*p++) << 8;
  value |= (*p++) << 16;
  value |= (uint64_t)(*p++) << 24;
...
enum { COUNTERS_SORT_TIME, COUNTERS_SORT_CATEGORY };

static int counters_sort_mode = COUNTERS_SORT_TIME;

static void add_counter_to_section(Counter *counter) {
...
static void add_counter(const char *section, const char *name, int type,
                        int unit, int variance, int index) {
  CounterList *list, *l;
  Counter *counter;
...
static void add_counter_to_timestamp(uint64_t timestamp, Counter *counter) {
  ...
  for (ctimestamp = counters_timestamps; ctimestamp;
       ctimestamp = ctimestamp->next) {
    if (ctimestamp->value == timestamp) {
      for (csection = ctimestamp->sections; csection;
...
static void add_counter_value(int index, CounterValue *value) {
  CounterList *list;
...
static const char *section_name(int section) {
  ...
  case MONO_COUNTER_RUNTIME:
    return "Mono Runtime";
  case MONO_COUNTER_SYSTEM:
    return "Mono System";
  case MONO_COUNTER_PROFILER:
...
static const char *unit_name(int unit) {
  switch (unit) {
  case MONO_COUNTER_RAW:
    return "Raw";
  case MONO_COUNTER_BYTES:
    return "Bytes";
  case MONO_COUNTER_TIME:
    return "Time";
  case MONO_COUNTER_COUNT:
    return "Count";
  case MONO_COUNTER_PERCENTAGE:
    return "Percentage";
  default:
    return "<unknown>";
  }
...
static const char *variance_name(int variance) {
  switch (variance) {
  case MONO_COUNTER_MONOTONIC:
    return "Monotonic";
  case MONO_COUNTER_CONSTANT:
    return "Constant";
  case MONO_COUNTER_VARIABLE:
    return "Variable";
  default:
    return "<unknown>";
  }
...
static void dump_counters_value(Counter *counter, const char *key_format,
...
#endif
    case MONO_COUNTER_TIME_INTERVAL:
      if (counter->type == MONO_COUNTER_LONG &&
          counter->unit == MONO_COUNTER_TIME) {
...
static void dump_counters(void) {
  ...
  if (!verbose) {
    char counters_to_print[][64] = {"Methods from AOT",
      ...
      for (clist = csection->counters; clist; clist = clist->next) {
        ...
        for (i = 0; counters_to_print[i][0] != 0; i++) {
          if (strcmp(counters_to_print[i], counter->name) == 0) {
            ...
            dump_counters_value(counter, "\t\t%-30s", counter->name,
                                counter->values_last->buffer);
            break;
          }
        }
      }
    ...
    for (ctimestamp = counters_timestamps; ctimestamp;
         ctimestamp = ctimestamp->next) {
      fprintf(outfile,
              "\t%" PRIu64 ":%02" PRIu64 ":%02" PRIu64 ":%02" PRIu64
              ".%03" PRIu64 ":\n",
    ...
    for (csection = counters_sections; csection; csection = csection->next) {
      ...
      for (clist = csection->counters; clist; clist = clist->next) {
        ...
        fprintf(outfile, "\t\t%s: [type: %s, unit: %s, variance: %s]\n",
                counter->name, type_name(counter->type),
                unit_name(counter->unit), variance_name(counter->variance));
        ...
        for (cvalue = counter->values; cvalue; cvalue = cvalue->next) {
          ...
          dump_counters_value(counter, "\t\t\t%s", strtimestamp,
                              cvalue->buffer);
        }
      }
    }
...
static void add_image(intptr_t image, char *name) {
...
static void add_assembly(intptr_t assembly, char *name) {
...
static ClassDesc *add_class(intptr_t klass, const char *name) {
...
static ClassDesc *lookup_class(intptr_t klass) {
  int slot = ((klass >> 2) & 0xffff) % HASH_SIZE;
...
static VTableDesc *add_vtable(intptr_t vtable, intptr_t klass) {
...
static VTableDesc *lookup_vtable(intptr_t vtable) {
  int slot = ((vtable >> 2) & 0xffff) % HASH_SIZE;
  VTableDesc *vt = vtable_hash[slot];
...
struct _MethodDesc {
  ...
  int recurse_count;
  int sample_hits;
  int ignore_jit; /* when this is set, we collect the metadata but don't count
                     this method fot jit time and code size, when filtering
                     events */
  uint64_t calls;
  uint64_t total_time;
...
static MethodDesc *add_method(intptr_t method, const char *name, intptr_t code,
                              int len) {
...
static MethodDesc *lookup_method(intptr_t method) {
  int slot = ((method >> 2) & 0xffff) % HASH_SIZE;
...
static void add_stat_sample(int type, uintptr_t ip) {
  if (num_stat_samples == size_stat_samples) {
    size_stat_samples *= 2;
    if (!size_stat_samples)
      size_stat_samples = 32;
    stat_samples = (uintptr_t *)g_realloc(stat_samples, size_stat_samples *
                                                            sizeof(uintptr_t));
    stat_sample_desc =
        (int *)g_realloc(stat_sample_desc, size_stat_samples * sizeof(int));
  }
  stat_samples[num_stat_samples] = ip;
  stat_sample_desc[num_stat_samples++] = type;
}

static MethodDesc *lookup_method_by_ip(uintptr_t ip) {
  int i;
  MethodDesc *m;
  /* dumb */
...
static int compare_method_samples(const void *a, const void *b) {
...
static int compare_usymbol_addr(const void *a, const void *b) {
...
static int compare_usymbol_samples(const void *a, const void *b) {
  UnmanagedSymbol *const *A = (UnmanagedSymbol *const *)a;
  UnmanagedSymbol *const *B = (UnmanagedSymbol *const *)b;
  if ((*B)->sample_hits == (*A)->sample_hits)
    return 0;
...
static void add_unmanaged_symbol(uintptr_t addr, char *name, uintptr_t size) {
  UnmanagedSymbol *sym;
  if (usymbols_num == usymbols_size) {
    ...
    if (!new_size)
      new_size = 16;
    usymbols =
        (UnmanagedSymbol **)g_realloc(usymbols, sizeof(void *) * new_size);
...
static UnmanagedSymbol *lookup_unmanaged_symbol(uintptr_t addr) {
...
static void add_unmanaged_binary(uintptr_t addr, char *name, uintptr_t size) {
  UnmanagedSymbol *sym;
  if (ubinaries_num == ubinaries_size) {
    ...
    if (!new_size)
      new_size = 16;
    ubinaries =
        (UnmanagedSymbol **)g_realloc(ubinaries, sizeof(void *) * new_size);
...
static UnmanagedSymbol *lookup_unmanaged_binary(uintptr_t addr) {
  int i;
...
static const char *sample_type_name(int type) {
  switch (type) {
  case SAMPLE_CYCLES:
    return "cycles";
  case SAMPLE_INSTRUCTIONS:
    return "instructions retired";
  case SAMPLE_CACHE_MISSES:
    return "cache misses";
  case SAMPLE_CACHE_REFS:
    return "cache references";
  case SAMPLE_BRANCHES:
    return "executed branches";
  case SAMPLE_BRANCH_MISSES:
    return "unpredicted branches";
  }
  return "unknown";
}

static void set_usym_parent(UnmanagedSymbol **cachedus, int count) {
  int i;
  for (i = 0; i < count; ++i) {
...
static void print_usym(UnmanagedSymbol *um) {
  if (um->parent)
    fprintf(outfile, "\t%6zd %6.2f %-36s in %s\n", um->sample_hits,
            um->sample_hits * 100.0 / num_stat_samples, um->name,
            um->parent->name);
  else
...
static int sym_percent(uintptr_t sample_hits) {
  double pc;
  if (verbose)
    return 1;
...
static void dump_samples(void) {
  ...
  if (!num_stat_samples)
    return;
  mono_qsort(usymbols, usymbols_num, sizeof(UnmanagedSymbol *),
        ...
        if (!usym->sample_hits) {
          if (ucount == usize) {
            ...
            if (!usize)
              usize = 4;
            cachedus =
                (UnmanagedSymbol **)g_realloc(cachedus, sizeof(void *) * usize);
          }
  ...
  mono_qsort(cachedus, ucount, sizeof(UnmanagedSymbol *),
  ...
  fprintf(outfile, "\tUnmanaged hits:  %6d (%4.1f%%)\n", unmanaged_hits,
          (100.0 * unmanaged_hits) / num_stat_samples);
  fprintf(outfile, "\tManaged hits:    %6d (%4.1f%%)\n",
          num_stat_samples - unmanaged_hits,
          (100.0 * (num_stat_samples - unmanaged_hits)) / num_stat_samples);
  fprintf(outfile, "\tUnresolved hits: %6d (%4.1f%%)\n", unresolved_hits,
          (100.0 * unresolved_hits) / num_stat_samples);
  ...
  while (i < count || u < ucount) {
    if (i < count) {
      ...
      fprintf(outfile, "\t%6d %6.2f %s\n", m->sample_hits,
              m->sample_hits * 100.0 / num_stat_samples, m->name);
      i++;
      continue;
    }
    if (u < ucount) {
...
static int add_rev_class_hashed(HeapClassRevRef *rev_hash, uintptr_t size,
                                HeapClassDesc *hklass, uint64_t value) {
  uintptr_t i;
  uintptr_t start_pos;
...
static void add_heap_class_rev(HeapClassDesc *from, HeapClassDesc *to) {
  uintptr_t i;
  if (to->rev_count * 2 >= to->rev_hash_size) {
    ...
    if (to->rev_hash_size == 0)
      to->rev_hash_size = 4;
    n = (HeapClassRevRef *)g_calloc(sizeof(HeapClassRevRef) * to->rev_hash_size,
                                    1);
    for (i = 0; i < old_size; ++i) {
      if (to->rev_hash[i].klass)
        add_rev_class_hashed(n, to->rev_hash_size, to->rev_hash[i].klass,
                             to->rev_hash[i].count);
    }
    if (to->rev_hash)
      g_free(to->rev_hash);
  ...
  to->rev_count +=
      add_rev_class_hashed(to->rev_hash, to->rev_hash_size, from, 1);
}
...
static HeapShot *new_heap_shot(uint64_t timestamp) {
...
static HeapClassDesc *heap_class_lookup(HeapShot *hs, ClassDesc *klass) {
  int i;
...
static int add_heap_hashed(HeapClassDesc **hash, HeapClassDesc **retv,
                           uintptr_t hsize, ClassDesc *klass, uint64_t size,
                           uint64_t count) {
  uintptr_t i;
  uintptr_t start_pos;
...
static HeapClassDesc *add_heap_shot_class(HeapShot *hs, ClassDesc *klass,
                                          uint64_t size) {
  HeapClassDesc *res;
  int i;
  if (hs->class_count * 2 >= hs->hash_size) {
    ...
    for (i = 0; i < old_size; ++i) {
      res = hs->class_hash[i];
      if (hs->class_hash[i])
        add_heap_hashed(n, &res, hs->hash_size, hs->class_hash[i]->klass,
                        hs->class_hash[i]->total_size,
                        hs->class_hash[i]->count);
    }
    if (hs->class_hash)
  ...
  res = NULL;
  hs->class_count +=
      add_heap_hashed(hs->class_hash, &res, hs->hash_size, klass, size, 1);
  // if (res->count == 1)
  //	printf ("added heap class: %s\n", res->klass->name);
  return res;
}

static HeapObjectDesc *alloc_heap_obj(uintptr_t objaddr, HeapClassDesc *hklass,
                                      uintptr_t num_refs) {
  HeapObjectDesc *ho = (HeapObjectDesc *)g_calloc(
      sizeof(HeapObjectDesc) + num_refs * sizeof(uintptr_t), 1);
  ho->objaddr = objaddr;
  ho->hklass = hklass;
...
static uintptr_t heap_shot_find_obj_slot(HeapShot *hs, uintptr_t objaddr) {
  uintptr_t i;
  uintptr_t start_pos;
...
static HeapObjectDesc *heap_shot_obj_add_refs(HeapShot *hs, uintptr_t objaddr,
                                              uintptr_t num,
  ...
  if (i >= 0) {
    HeapObjectDesc *ho =
        alloc_heap_obj(objaddr, hash[i]->hklass, hash[i]->num_refs + num);
...
static uintptr_t add_heap_hashed_obj(HeapObjectDesc **hash, uintptr_t hsize,
                                     HeapObjectDesc *obj) {
  uintptr_t i;
  uintptr_t start_pos;
...
static void add_heap_shot_obj(HeapShot *hs, HeapObjectDesc *obj) {
  uintptr_t i;
  ...
  hs->objects_count +=
      add_heap_hashed_obj(hs->objects_hash, hs->objects_hash_size, obj);
}

static void heap_shot_resolve_reverse_refs(HeapShot *hs) {
  uintptr_t i;
...
static void heap_shot_mark_objects(HeapShot *hs) {
  uintptr_t i, oi, r;
  unsigned char *marks;
  ...
  if (!debug)
    return;
  /* consistency checks: it seems not all the objects are walked in the heap in
   * some cases */
  ...
  for (i = 0; i < hs->num_roots; ++i) {
    ...
    if (oi == -1) {
      fprintf(outfile, "root type 0x%x for obj %p (%s) not found in heap\n",
              hs->roots_types[i], (void *)hs->roots[i],
              lookup_class(hs->roots_extra[i])->name);
      continue;
    }
    obj = hs->objects_hash[oi];
    if (!marks[oi]) {
  ...
  for (i = 0; i < hs->objects_hash_size; ++i) {
    if (hs->objects_hash[i] && !marks[i]) {
      num_unmarked++;
      fprintf(outfile, "object %p (%s) unmarked\n", (void *)hs->objects_hash[i],
              hs->objects_hash[i]->hklass->klass->name);
    }
  }
  fprintf(outfile,
          "Total unmarked: %" G_GSIZE_FORMAT "d/%" G_GSIZE_FORMAT "d\n",
          num_unmarked, hs->objects_count);
  g_free(marks);
}

static void heap_shot_free_objects(HeapShot *hs) {
  uintptr_t i;
  for (i = 0; i < hs->objects_hash_size; ++i) {
    HeapObjectDesc *ho = hs->objects_hash[i];
    if (ho)
...
static int hash_backtrace(int count, MethodDesc **methods) {
  int hash = count;
  int i;
  for (i = 0; i < count; ++i) {
...
static int compare_backtrace(BackTrace *bt, int count, MethodDesc **methods) {
  int i;
  if (bt->count != count)
    return 0;
  for (i = 0; i < count; ++i)
...
static BackTrace *add_backtrace(int count, MethodDesc **methods) {
  ...
  bt =
      (BackTrace *)g_malloc(sizeof(BackTrace) + ((count - 1) * sizeof(void *)));
  ...
  if (next_backtrace == num_backtraces) {
    ...
    if (!num_backtraces)
      num_backtraces = 16;
    backtraces =
        (BackTrace **)g_realloc(backtraces, sizeof(void *) * num_backtraces);
  }
...
static void ensure_buffer(ProfContext *ctx, int size) {
  if (ctx->size < size) {
...
static int load_data(ProfContext *ctx, int size) {
...
static ThreadContext *get_thread(ProfContext *ctx, intptr_t thread_id) {
  ...
  thread->time_stack =
      (uint64_t *)g_malloc(thread->stack_size * sizeof(uint64_t));
  thread->callee_time_stack =
      (uint64_t *)g_malloc(thread->stack_size * sizeof(uint64_t));
  return thread;
}

static DomainContext *get_domain(ProfContext *ctx, intptr_t domain_id) {
...
static RemCtxContext *get_remctx(ProfContext *ctx, intptr_t remctx_id) {
...
static ThreadContext *load_thread(ProfContext *ctx, intptr_t thread_id) {
  ThreadContext *thread = get_thread(ctx, thread_id);
  ctx->current_thread = thread;
  return thread;
}

static void ensure_thread_stack(ThreadContext *thread) {
  if (thread->stack_id == thread->stack_size) {
    thread->stack_size *= 2;
    thread->stack = (MethodDesc **)g_realloc(thread->stack, thread->stack_size *
                                                                sizeof(void *));
    thread->time_stack = (uint64_t *)g_realloc(
        thread->time_stack, thread->stack_size * sizeof(uint64_t));
    thread->callee_time_stack = (uint64_t *)g_realloc(
        thread->callee_time_stack, thread->stack_size * sizeof(uint64_t));
  }
}

static int add_trace_hashed(CallContext *traces, int size, BackTrace *bt,
                            uint64_t value) {
  int i;
  unsigned int start_pos;
...
static void add_trace_bt(BackTrace *bt, TraceDesc *trace, uint64_t value) {
  int i;
  if (!collect_traces)
    return;
  if (trace->count * 2 >= trace->size) {
    ...
    for (i = 0; i < old_size; ++i) {
      if (trace->traces[i].bt)
        add_trace_hashed(n, trace->size, trace->traces[i].bt,
                         trace->traces[i].count);
    }
    if (trace->traces)
      g_free(trace->traces);
    trace->traces = n;
...
static BackTrace *add_trace_thread(ThreadContext *thread, TraceDesc *trace,
                                   uint64_t value) {
  BackTrace *bt;
...
static BackTrace *add_trace_methods(MethodDesc **methods, int count,
                                    TraceDesc *trace, uint64_t value) {
  BackTrace *bt;
  if (!collect_traces)
    return NULL;
  if (count > trace_max)
...
static void thread_add_root(ThreadContext *ctx, uintptr_t obj, int root_type,
  ...
  if (ctx->num_roots == ctx->size_roots) {
    ...
    if (!new_size)
      new_size = 4;
    ctx->roots =
        (uintptr_t *)g_realloc(ctx->roots, new_size * sizeof(uintptr_t));
    ctx->roots_extra =
        (uintptr_t *)g_realloc(ctx->roots_extra, new_size * sizeof(uintptr_t));
    ctx->roots_types =
        (int *)g_realloc(ctx->roots_types, new_size * sizeof(int));
...
static int compare_callc(const void *a, const void *b) {
...
static void sort_context_array(TraceDesc *traces) {
  int i, j;
...
static void push_method(ThreadContext *thread, MethodDesc *method,
                        uint64_t timestamp) {
...
static void pop_method(ThreadContext *thread, MethodDesc *method,
                       uint64_t timestamp) {
  method->recurse_count--;
...
static const char *gc_event_name(int ev) {
  switch (ev) {
...
static const char *sync_point_name(int type) {
  switch (type) {
...
static const char *clause_name(int type) {
  switch (type) {
  case MONO_EXCEPTION_CLAUSE_NONE:
    return "catch";
  case MONO_EXCEPTION_CLAUSE_FILTER:
    return "filter";
  case MONO_EXCEPTION_CLAUSE_FINALLY:
    return "finally";
  case MONO_EXCEPTION_CLAUSE_FAULT:
    return "fault";
  default:
    return "invalid";
  }
...
static MonitorDesc *lookup_monitor(uintptr_t objid) {
...
static const char *monitor_ev_name(int ev) {
  switch (ev) {
...
static const char *get_handle_name(int htype) {
  switch (htype) {
  case 0:
    return "weak";
  case 1:
...
static const char *get_root_name(int rtype) {
  switch (rtype & MONO_PROFILER_GC_ROOT_TYPEMASK) {
  case MONO_PROFILER_GC_ROOT_STACK:
    return "stack";
  case MONO_PROFILER_GC_ROOT_FINALIZER:
    return "finalizer";
  case MONO_PROFILER_GC_ROOT_HANDLE:
    return "handle";
  case MONO_PROFILER_GC_ROOT_OTHER:
    return "other";
  case MONO_PROFILER_GC_ROOT_MISC:
    return "misc";
  default:
    return "unknown";
  }
...
static uint64_t decode_uleb128(uint8_t *buf, uint8_t **endbuf) {
  uint64_t res = 0;
  int shift = 0;
...
static intptr_t decode_sleb128(uint8_t *buf, uint8_t **endbuf) {
  uint8_t *p = buf;
  intptr_t res = 0;
  int shift = 0;
...
static MethodDesc **decode_bt(ProfContext *ctx, MethodDesc **sframes, int *size,
                              unsigned char *p, unsigned char **endp,
                              intptr_t ptr_base, intptr_t *method_base) {
  MethodDesc **frames;
  int i;
...
static void tracked_creation(uintptr_t obj, ClassDesc *cd, uint64_t size,
                             BackTrace *bt, uint64_t timestamp) {
  int i;
  for (i = 0; i < num_tracked_objects; ++i) {
    if (tracked_objects[i] != obj)
      continue;
    fprintf(outfile,
            "Object %p created (%s, %" PRIu64 " bytes) at %.3f secs.\n",
            (void *)obj, cd->name, (guint64)size,
            (timestamp - startup_time) / 1000000000.0);
    if (bt && bt->count) {
      int k;
...
static void track_handle(uintptr_t obj, int htype, uint32_t handle,
                         BackTrace *bt, uint64_t timestamp) {
  int i;
  for (i = 0; i < num_tracked_objects; ++i) {
    if (tracked_objects[i] != obj)
...
static void track_move(uintptr_t src, uintptr_t dst) {
  int i;
  for (i = 0; i < num_tracked_objects; ++i) {
...
static void track_obj_reference(uintptr_t obj, uintptr_t parent,
                                ClassDesc *cd) {
  int i;
  for (i = 0; i < num_tracked_objects; ++i) {
    if (tracked_objects[i] == obj)
...
static void found_object(uintptr_t obj) {
  num_tracked_objects++;
  tracked_objects = (uintptr_t *)g_realloc(
      tracked_objects, num_tracked_objects * sizeof(tracked_objects[0]));
...
static const char *code_buffer_desc(int type) {
  switch (type) {
...
#define LOG_TIME(base,                                                         \
                 diff) /*fprintf("outfile, time %" PRIu64 " + %" PRIu64 " near \
                          offset %d\n", base, diff, p - ctx->buf)*/
...
static void record_event_stats(int type, int size) {
  ++stats[type].count;
...
static int decode_buffer(ProfContext *ctx) {
  unsigned char *p;
  unsigned char *end;
  intptr_t thread_id;
  intptr_t ptr_base;
  ...
  if (debug)
    fprintf(outfile,
            "buf: thread:%" G_GSIZE_FORMAT "x, len: %d, time: %" PRIu64
            ", file offset: %" PRIu64 "\n",
  ...
  while (p < end) {
    ...
    case TYPE_GC: {
      ...
      if (subtype == TYPE_GC_RESIZE) {
        ...
        if (debug)
          fprintf(outfile,
                  "gc event for gen%d: %s at %" PRIu64
                  " (thread: 0x%" G_GSIZE_FORMAT "x)\n",
                  gen, gc_event_name(ev), (guint64)time_base,
                  thread->thread_id);
        if (gen > 2) {
        ...
        for (j = 0; j < num; j += 2) {
          ...
          if (debug) {
            fprintf(outfile, "moved obj %p to %p\n", (void *)OBJ_ADDR(obj1diff),
                 ...
                 subtype == TYPE_GC_HANDLE_CREATED_BT) {
        ...
        if (has_bt) {
          num_bt = 8;
          frames =
              decode_bt(ctx, sframes, &num_bt, p, &p, ptr_base, &method_base);
          if (!frames) {
        ...
        if ((thread_filter && thread_filter == thread->thread_id) ||
            (time_base >= time_from && time_base < time_to)) {
          ...
          BackTrace *bt;
          if (has_bt)
            bt = add_trace_methods(frames, num_bt, &handle_info[htype].traces,
                                   1);
          else
        ...
        if (frames != sframes)
          g_free(frames);
                 ...
                 subtype == TYPE_GC_HANDLE_DESTROYED_BT) {
        ...
        if ((thread_filter && thread_filter == thread->thread_id) ||
          ...
          BackTrace *bt;
          if (has_bt)
            bt = add_trace_methods(frames, num_bt,
                                   &handle_info[htype].destroy_traces, 1);
          else
            bt =
                add_trace_thread(thread, &handle_info[htype].destroy_traces, 1);
          /* TODO: track_handle_free () - would need to record and keep track of
           * the associated object address... */
        }
        if (debug)
        ...
        if (debug)
          fprintf(outfile,
                  "gc finalizer queue being processed at %" PRIu64 "\n",
                  (guint64)time_base);
      } else if (subtype == TYPE_GC_FINALIZE_END) {
        if (debug)
          fprintf(outfile,
                  "gc finalizer queue finished processing at %" PRIu64 "\n",
                  (guint64)time_base);
      ...
      if (mtype == TYPE_CLASS) {
        ...
        if (debug)
          fprintf(outfile, "%s class %p (%s in %p) at %" PRIu64 "\n", load_str,
                  ...
                  (void *)(ptr_base + imptrdiff), (guint64)time_base);
        ...
        if (debug)
          fprintf(outfile, "%s image %p (%s) at %" PRIu64 "\n", load_str,
                  (void *)(ptr_base + ptrdiff), p, (guint64)time_base);
        ...
        if (debug) {
          if (subtype == 0)
            ...
            fprintf(outfile, "%s thread %p at %" PRIu64 "\n", load_str,
                    (void *)(ptr_base + ptrdiff), (guint64)time_base);
        }
        if (subtype == 0) {
          while (*p)
            p++;
        ...
        if (debug)
          fprintf(outfile, "%s context %p (%p) at %" PRIu64 "\n", load_str,
                  (void *)(ptr_base + ptrdiff), (void *)(ptr_base + domaindiff),
                  (guint64)time_base);
        if (subtype == TYPE_END_LOAD)
          get_remctx(ctx, ptr_base + ptrdiff)->domain_id =
              ptr_base + domaindiff;
        ...
        if (debug)
          fprintf(outfile,
                  "vtable %p for class %p in domain %p at %" PRIu64 "\n",
                  (void *)(ptrdiff + ptr_base), (void *)(classdiff + ptr_base),
                  (void *)(domaindiff + ptr_base), (guint64)time_base);
    ...
    case TYPE_ALLOC: {
      ...
      if (debug)
        fprintf(outfile,
                "alloced object %p, size %" PRIu64 " (%s) at %" PRIu64 "\n",
                (void *)OBJ_ADDR(objdiff), (guint64)len, cd->name,
                (guint64)time_base);
      if (has_bt) {
        num_bt = 8;
        frames =
      ...
      if (subtype == TYPE_JIT) {
        ...
        if (debug)
          fprintf(outfile, "%s method %s\n",
                  subtype == TYPE_ENTER
                      ? "enter"
                      : subtype == TYPE_EXC_LEAVE ? "exleave" : "leave",
                  method->name);
      }
      break;
    }
    case TYPE_HEAP: {
      ...
      if (subtype == TYPE_HEAP_OBJECT) {
        ...
        if (size) {
          HeapClassDesc *hcd =
              add_heap_shot_class(thread->current_heap_shot, cd, size);
          if (collect_traces) {
          ...
          if (collect_traces)
            ho = heap_shot_obj_add_refs(thread->current_heap_shot,
                                        OBJ_ADDR(objdiff), num, &ref_offset);
        }
        for (i = 0; i < num; ++i) {
          /* FIXME: use object distance to measure how good
           * the GC is at keeping related objects close
           */
          uintptr_t offset = ctx->data_version > 1
                                 ? last_obj_offset + decode_uleb128(p, &p)
                                 : -1;
          intptr_t obj1diff = decode_sleb128(p, &p);
        ...
        if (debug && size)
          fprintf(outfile,
                  "traced object %p, size %" PRIu64
                  " (%s), refs: %" G_GSIZE_FORMAT "d\n",
        ...
        if (ctx->data_version > 14) {
          ...
          for (i = 0; i < num; ++i) {
            ...
            if (collect_traces)
              thread_add_root(thread, OBJ_ADDR(objdiff),
                              MONO_PROFILER_GC_ROOT_MISC, 0);
          }
        } else {
          if (ctx->data_version > 12) {
            uint64_t tdiff = decode_uleb128(p + 1, &p);
        ...
        if (debug)
          fprintf(outfile,
                  "root register address %p size %" PRId64
                  " type %d key %p name %s\n",
                  (void *)(ptr_base + ptrdiff), (guint64)size, type,
                  (void *)(ptr_base + keydiff), desc);
      ...
      if (subtype == TYPE_CLAUSE) {
        ...
        if (debug)
          fprintf(outfile, "clause %s (%d) in method %s\n",
                  clause_name(clause_type), clause_num,
                  lookup_method(method_base)->name);
      } else {
        intptr_t objdiff = decode_sleb128(p, &p);
        if (record)
          throw_count++;
        if (has_bt) {
          has_bt = 8;
          frames =
              decode_bt(ctx, sframes, &has_bt, p, &p, ptr_base, &method_base);
          if (!frames) {
            fprintf(outfile, "Cannot load backtrace\n");
            return 0;
          }
    ...
    case TYPE_SAMPLE: {
      ...
      if (subtype == TYPE_SAMPLE_HIT) {
        ...
        for (i = 0; i < len; i++) {
          ...
          add_counter(section_str, name, (int)type, (int)unit, (int)variance,
                      (int)index);
        }
        ...
        uint64_t time_between =
            timestamp / 1000 * 1000 * 1000 * 1000 + startup_time;
        while (1) {
          ...
          case MONO_COUNTER_WORD:
            ...
            *(int32_t *)value->buffer =
                (int32_t)decode_sleb128(p, &p) +
                (previous ? (*(int32_t *)previous->buffer) : 0);
            break;
          case MONO_COUNTER_UINT:
            value->buffer = (unsigned char *)g_malloc(sizeof(uint32_t));
            *(uint32_t *)value->buffer =
                (uint32_t)decode_uleb128(p, &p) +
                (previous ? (*(uint32_t *)previous->buffer) : 0);
            break;
          case MONO_COUNTER_LONG:
#if SIZEOF_VOID_P == 8
          case MONO_COUNTER_WORD:
          ...
          case MONO_COUNTER_TIME_INTERVAL:
            ...
            *(int64_t *)value->buffer =
                (int64_t)decode_sleb128(p, &p) +
                (previous ? (*(int64_t *)previous->buffer) : 0);
            break;
          case MONO_COUNTER_ULONG:
            value->buffer = (unsigned char *)g_malloc(sizeof(uint64_t));
            *(uint64_t *)value->buffer =
                (uint64_t)decode_uleb128(p, &p) +
                (previous ? (*(uint64_t *)previous->buffer) : 0);
            break;
          case MONO_COUNTER_DOUBLE:
    ...
    default:
      fprintf(outfile,
              "unhandled profiler event: 0x%x at file offset: %" PRIu64
              " + %" PRId64 " (len: %d\n)\n",
...
static int read_header_string(ProfContext *ctx, char **field) {
  if (!load_data(ctx, 4))
    return 0;
...
static ProfContext *load_file(char *name) {
  unsigned char *p;
...
enum { ALLOC_SORT_BYTES, ALLOC_SORT_COUNT };
static int alloc_sort_mode = ALLOC_SORT_BYTES;

static int compare_class(const void *a, const void *b) {
...
static void dump_header(ProfContext *ctx) {
...
static void dump_traces(TraceDesc *traces, const char *desc) {
  int j;
  if (!show_traces)
    return;
  if (!traces->count)
...
static void dump_threads(ProfContext *ctx) {
  ThreadContext *thread;
  fprintf(outfile, "\nThread summary\n");
...
static void dump_domains(ProfContext *ctx) {
  fprintf(outfile, "\nDomain summary\n");
...
static void dump_remctxs(ProfContext *ctx) {
  fprintf(outfile, "\nContext summary\n");

  for (RemCtxContext *remctx = ctx->remctxs; remctx; remctx = remctx->next)
    fprintf(outfile, "\tContext: %p, domain: %p\n", (void *)remctx->remctx_id,
            (void *)remctx->domain_id);
}

static void dump_exceptions(void) {
  int i;
  fprintf(outfile, "\nException summary\n");
  ...
  for (i = 0; i <= MONO_EXCEPTION_CLAUSE_FAULT; ++i) {
    ...
    fprintf(outfile, "\tExecuted %s clauses: %" PRIu64 "\n", clause_name(i),
            (guint64)clause_summary[i]);
  }
}

static int compare_monitor(const void *a, const void *b) {
...
static void dump_monitors(void) {
  ...
  for (i = 0; i < num_monitors; ++i) {
    ...
    fprintf(outfile, "\tLock object %p: %d contentions\n", (void *)mdesc->objid,
            (int)mdesc->contentions);
    fprintf(outfile, "\t\t%.6f secs total wait time, %.6f max, %.6f average\n",
            mdesc->wait_time / 1000000000.0,
            mdesc->max_wait_time / 1000000000.0,
            mdesc->wait_time / 1000000000.0 / mdesc->contentions);
...
static void dump_gcs(void) {
  ...
  for (i = 0; i < 3; ++i) {
    if (!gc_info[i].count)
      continue;
    fprintf(outfile,
            "\tGen%d collections: %d, max time: %" PRIu64
            "us, total time: %" PRIu64 "us, average: %" PRIu64 "us\n",
            i, gc_info[i].count, (guint64)(gc_info[i].max_time / 1000),
  ...
  for (i = 0; i < 3; ++i) {
    if (!handle_info[i].max_live)
      continue;
    fprintf(outfile,
            "\tGC handles %s: created: %" PRIu64 ", destroyed: %" PRIu64
            ", max: %" PRIu64 "\n",
            get_handle_name(i), (guint64)(handle_info[i].created),
...
static void dump_allocations(void) {
  ...
  for (i = 0; i < num_classes; ++i) {
    ...
    fprintf(outfile, "%10" PRIu64 " %10zd %8" PRIu64 " %s\n",
            ...
            (guint64)(cd->alloc_size / cd->allocs), cd->name);
    dump_traces(&cd->traces, "bytes");
  }
  if (allocs)
    fprintf(outfile,
            "Total memory allocated: %" PRIu64 " bytes in %" G_GSIZE_FORMAT
            "d objects\n",
            (guint64)size, allocs);
}

enum { METHOD_SORT_TOTAL, METHOD_SORT_SELF, METHOD_SORT_CALLS };

static int method_sort_mode = METHOD_SORT_TOTAL;

static int compare_method(const void *a, const void *b) {
  MethodDesc *const *A = (MethodDesc *const *)a;
  MethodDesc *const *B = (MethodDesc *const *)b;
  uint64_t vala, valb;
...
static void dump_methods(void) {
  ...
  for (i = 0; i < num_methods; ++i) {
    ...
    fprintf(outfile, "%8" PRIu64 " %8" PRIu64 " %10" PRIu64 " %s\n",
            (guint64)(msecs), (guint64)(smsecs), (guint64)(cd->calls),
            cd->name);
...
static int compare_heap_class(const void *a, const void *b) {
...
static int compare_rev_class(const void *a, const void *b) {
...
static void dump_rev_claases(HeapClassRevRef *revs, int count) {
  int j;
  if (!show_traces)
    return;
  if (!count)
  ...
  for (j = 0; j < count; ++j) {
    ...
    fprintf(outfile, "\t\t%" PRIu64 " references from: %s\n",
            (guint64)(revs[j].count), cd->klass->name);
  }
}

static void heap_shot_summary(HeapShot *hs, int hs_num, HeapShot *last_hs) {
  uint64_t size = 0;
  uint64_t count = 0;
  int ccount = 0;
  int i;
  ...
  fprintf(outfile,
          "\n\tHeap shot %d at %.3f secs: size: %" PRIu64
          ", object count: %" PRIu64
          ", class count: %d, roots: %" G_GSIZE_FORMAT "d\n",
          hs_num, (hs->timestamp - startup_time) / 1000000000.0,
          (guint64)(size), (guint64)(count), ccount, hs->num_roots);
  ...
  for (i = 0; i < ccount; ++i) {
    ...
    fprintf(outfile, "\t%10" PRIu64 " %10" PRIu64 " %8" PRIu64 " %s",
            (guint64)(cd->total_size), (guint64)(cd->count),
            (guint64)(cd->total_size / cd->count), cd->klass->name);
    if (ocd) {
    ...
    if (!collect_traces)
      continue;
    rev_sorted =
        (HeapClassRevRef *)g_malloc(cd->rev_count * sizeof(HeapClassRevRef));
    k = 0;
    ...
    mono_qsort(rev_sorted, cd->rev_count, sizeof(HeapClassRevRef),
               compare_rev_class);
    if (cd->root_references)
      fprintf(outfile,
              "\t\t%" G_GSIZE_FORMAT "d root references (%" G_GSIZE_FORMAT
              "d pinning)\n",
...
static int compare_heap_shots(const void *a, const void *b) {
...
static void dump_heap_shots(void) {
  HeapShot **hs_sorted;
  HeapShot *hs;
...
#define DUMP_EVENT_STAT(EVENT, SUBTYPE)                                        \
  dump_event(#EVENT, #SUBTYPE, EVENT, SUBTYPE);

static void dump_event(const char *event_name, const char *subtype_name,
                       int event, int subtype) {
  ...
  fprintf(outfile, "\t%16s\t%26s\tcount %6d\tmin %3d\tmax %6d\tbytes %d\n",
          event_name, subtype_name, evt.count, evt.min_size, evt.max_size,
          evt.bytes);
}
...
static void flush_context(ProfContext *ctx) {
  ThreadContext *thread;
  ...
  for (thread = ctx->threads; thread; thread = thread->next) {
    while (thread->stack_id) {
      ...
      pop_method(thread, thread->stack[thread->stack_id - 1],
                 thread->last_time);
    }
  }
}
...
static const char *reports =
    "header,jit,gc,sample,alloc,call,metadata,exception,monitor,thread,domain,"
    "context,heapshot,counters";

static const char *match_option(const char *p, const char *opt) {
  int len = strlen(opt);
...
static int print_reports(ProfContext *ctx, const char *reps, int parse_only) {
  const char *opt;
  const char *p;
  for (p = reps; *p; p = opt) {
    ...
    if ((opt = match_option(p, "coverage")) != p) {
      printf("The log profiler no longer supports code coverage. Please use "
             "the dedicated coverage profiler instead. See mono-profilers(1) "
...
static int add_find_spec(const char *p) {
  if (p[0] == 'S' && p[1] == ':') {
    char *vale;
...
static void usage(void) {
  ...
  printf("\t                     %s\n", reports);
  printf("\t--method-sort=MODE   sort methods according to MODE: total, self, "
         "calls\n");
  printf("\t--alloc-sort=MODE    sort allocations according to MODE: bytes, "
         "count\n");
  printf("\t--counters-sort=MODE sort counters according to MODE: time, "
         "category\n");
  printf("\t                     only accessible in verbose mode\n");
  printf(
      "\t--track=OB1[,OB2...] track what happens to objects OBJ1, O2 etc.\n");
  printf("\t--find=FINDSPEC      find and track objects matching FINFSPEC, "
         "where FINDSPEC is:\n");
  printf("\t                     S:minimum_size or T:partial_name\n");
  printf("\t--thread=THREADID    consider just the data for thread THREADID\n");
  printf("\t--time=FROM-TO       consider data FROM seconds from startup up to "
         "TO seconds\n");
  printf("\t--verbose            increase verbosity level\n");
  printf("\t--debug              display decoding debug info for mprof-report "
         "devs\n");
}
