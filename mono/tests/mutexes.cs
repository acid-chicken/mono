...
// between each lock.When the second thread has called ReleaseMutex on the mutex
// 10 times it terminates and the first thread can carry on its cycle of locking
// and releasing mutexes until it exits.
//
/////////////////////////////////////////////////////////////////////////////////
...
class MutexTest {
  ...
  public static void ThreadMethod_B() {
    ...
    for (int h = 0; h < 10; h++) {
      int i = 5;
      Console.WriteLine("[Thread B] - Trying to lock mutex " + i + " for " + h +
                        " time...");
      m[i].WaitOne();
      Console.WriteLine("[Thread B] - m[" + i + "] Locked recursively [" + h +
                        "] times!");
      Thread.Sleep(4500);
    }
    for (int h = 0; h < 10; h++) {
      int i = 5;
      m[i].ReleaseMutex();
      Console.WriteLine("[Thread B] - Unlocked the mutex [" + i + "] for [" +
                        h + "] times");
    }
