...
effect:
        ('set-byte' | 'set-ushort' | 'set-uint' | 'set-bit' | 'or-byte' |
'or-ushort' | 'or-uint' | 'truncate' ) expression
...
enum { TEST_TYPE_VALID, TEST_TYPE_INVALID, TEST_TYPE_BADRT };
...
static const char *test_validity_name(int validity) {
  switch (validity) {
...
static char *read_whole_file_and_close(const char *name, int *file_size) {
...
static void init_test_set(test_set_t *test_set) {
  MonoImageOpenStatus status;
  if (test_set->init)
    return;
  test_set->assembly_data =
      read_whole_file_and_close(test_set->assembly, &test_set->assembly_size);
  test_set->image = mono_image_open_from_data_internal(
      mono_domain_default_alc(mono_root_domain_get()), test_set->assembly_data,
      test_set->assembly_size, FALSE, &status, FALSE, FALSE, NULL);
...
static char *make_test_name(test_entry_t *entry, test_set_t *test_set) {
  return g_strdup_printf("%s-%s-%d.exe", test_validity_name(entry->validity),
                         test_set->name, test_set->count++);
}

#define READ_VAR(KIND, PTR) GUINT32_FROM_LE((guint32) * ((KIND *)(PTR)))
#define SET_VAR(KIND, PTR, VAL)                                                \
  do {                                                                         \
    *((KIND *)(PTR)) = GUINT32_TO_LE((KIND)VAL);                               \
  } while (0)

#define READ_BIT(PTR, OFF)                                                     \
  ((((guint8 *)(PTR))[(OFF / 8)] & (1 << ((OFF) % 8))) != 0)
#define SET_BIT(PTR, OFF)                                                      \
  do {                                                                         \
    ((guint8 *)(PTR))[(OFF / 8)] |= (1 << ((OFF) % 8));                        \
  } while (0)

static guint32 get_pe_header(test_entry_t *entry) {
  return READ_VAR(guint32, entry->data + 0x3c) + 4;
}

static guint32 translate_rva(test_entry_t *entry, guint32 rva) {
...
static guint32 get_cli_header(test_entry_t *entry) {
...
static guint32 get_cli_metadata_root(test_entry_t *entry) {
...
static guint32 get_metadata_stream_header(test_entry_t *entry, guint32 idx) {
  guint32 offset;
...
static guint32 lookup_var(test_entry_t *entry, const char *name) {
...
static guint32 call_func(test_entry_t *entry, const char *name, GSList *args) {
...
static guint32 expression_eval(expression_t *exp, test_entry_t *entry) {
  ...
  case EXPRESSION_ADD:
    return expression_eval(exp->data.bin.left, entry) +
           expression_eval(exp->data.bin.right, entry);
  case EXPRESSION_SUB:
    return expression_eval(exp->data.bin.left, entry) -
           expression_eval(exp->data.bin.right, entry);
...
static guint32 apply_selector(patch_selector_t *selector, test_entry_t *entry) {
  guint32 value = 0;
...
static void apply_effect(patch_effect_t *effect, test_entry_t *entry,
                         guint32 offset) {
  gint32 value = 0;
...
static void apply_patch(test_entry_t *entry, test_patch_t *patch) {
  guint32 offset = apply_selector(patch->selector, entry);
  apply_effect(patch->effect, entry, offset);
}

static void process_test_entry(test_set_t *test_set, test_entry_t *entry) {
  GSList *tmp;
  char *file_name;
  FILE *f;
...
static void patch_free(test_patch_t *patch) {
  free(patch->selector);
  free(patch->effect);
  free(patch);
}

static void test_set_free(test_set_t *set) {
  free(set->name);
  free(set->assembly);
...
static void test_entry_free(test_entry_t *entry) {
  GSList *tmp;
...
static int is_eof(scanner_t *scanner) { return scanner->idx >= scanner->size; }

static int ispunct_char(int c) { return c == '{' || c == '}' || c == ','; }

static void skip_spaces(scanner_t *scanner) {
start:
...
static char *token_text_dup(scanner_t *scanner, token_t *token) {
...
#if SCANNER_DEBUG
static void dump_token(scanner_t *scanner, token_t *token) {
...
static scanner_t *scanner_new(const char *file_name) {
  scanner_t *res;
...
static void scanner_free(scanner_t *scanner) {
  free(scanner->input);
  free(scanner);
}

static token_t *scanner_get_current_token(scanner_t *scanner) {
  return &scanner->current;
}

static int scanner_get_type(scanner_t *scanner) {
  return scanner_get_current_token(scanner)->type;
}

static int scanner_get_line(scanner_t *scanner) {
  return scanner_get_current_token(scanner)->line;
}

static char *scanner_text_dup(scanner_t *scanner) {
  return token_text_dup(scanner, scanner_get_current_token(scanner));
}

static int scanner_text_parse_number(scanner_t *scanner, long *res) {
...
static int match_current_type(scanner_t *scanner, int type) {
  return scanner_get_type(scanner) == type;
}

static int match_current_text(scanner_t *scanner, const char *text) {
  token_t *t = scanner_get_current_token(scanner);
  return !strncmp(scanner->input + t->start, text, t->end - t->start);
}

static int match_current_type_and_text(scanner_t *scanner, int type,
                                       const char *text) {
  return match_current_type(scanner, type) && match_current_text(scanner, text);
}

/*******************************************************************************************************/
#define FAIL(MSG, REASON)                                                      \
  do {                                                                         \
    printf("%s at line %d for rule %s\n", MSG, scanner_get_line(scanner),      \
           __FUNCTION__);                                                      \
    exit(REASON);                                                              \
  } while (0);
...
#define EXPECT_TOKEN(TYPE)                                                     \
  do {                                                                         \
    if (scanner_get_type(scanner) != TYPE) {                                   \
      printf(                                                                  \
          "Expected %s but got %s '%s' at line %d for rule %s\n",              \
          token_type_name(TYPE), token_type_name(scanner_get_type(scanner)),   \
          scanner_text_dup(scanner), scanner_get_line(scanner), __FUNCTION__); \
      exit(INVALID_TOKEN_TYPE);                                                \
    }                                                                          \
  } while (0)

#define CONSUME_SPECIFIC_PUNCT(TEXT)                                           \
  do {                                                                         \
...
#define CONSUME_IDENTIFIER(DEST)                                               \
  do {                                                                         \
...
#define CONSUME_SPECIFIC_IDENTIFIER(TEXT)                                      \
  do {                                                                         \
    EXPECT_TOKEN(TOKEN_ID);                                                    \
    if (!match_current_text(scanner, TEXT)) {                                  \
      char *__tmp = scanner_text_dup(scanner);                                 \
  ...
  do {                                                                         \
    ...
    if (scanner_text_parse_number(scanner, &__tmp_num)) {                      \
      ...
      printf("Expected a number but got '%s' at line %d for rule %s\n", __tmp, \
             scanner_get_line(scanner), __FUNCTION__);                         \
      free(__tmp);                                                             \
      exit(INVALID_NUMBER);                                                    \
    }                                                                          \
    DEST = __tmp_num;                                                          \
...
#define LA_ID(TEXT)                                                            \
  (scanner_get_type(scanner) == TOKEN_ID && match_current_text(scanner, TEXT))
#define LA_PUNCT(TEXT)                                                         \
  (scanner_get_type(scanner) == TOKEN_PUNC && match_current_text(scanner, TEXT))

/*******************************************************************************************************/

static expression_t *parse_atom(scanner_t *scanner) {
  ...
  if (scanner_get_type(scanner) == TOKEN_NUM) {
    ...
    if (LA_ID("(")) {
      ...
      while (!LA_ID(")") && !match_current_type(scanner, TOKEN_EOF))
        atom->data.func.args =
            g_slist_append(atom->data.func.args, parse_expression(scanner));
...
static patch_selector_t *parse_selector(scanner_t *scanner) {
  patch_selector_t *selector;
...
static patch_effect_t *parse_effect(scanner_t *scanner) {
  patch_effect_t *effect;
  char *name;
  int type;
  ...
  if (!strcmp("set-byte", name))
    ...
    FAIL(g_strdup_printf(
             "Invalid effect kind, expected one of: (set-byte set-ushort "
             "set-uint set-bit or-byte or-ushort or-uint truncate) but got %s",
             name),
         INVALID_ID_TEXT);
...
static test_patch_t *parse_patch(scanner_t *scanner) {
  test_patch_t *patch;
...
static int parse_validity(scanner_t *scanner) {
  char *name = NULL;
  int validity;
  CONSUME_IDENTIFIER(name);
  ...
  if (!strcmp(name, "valid"))
    ...
    printf("Expected either 'valid', 'invalid' or 'badtr' but got '%s' at the "
           "beginning of a test entry at line %d\n",
...
static void parse_test_entry(scanner_t *scanner, test_set_t *test_set) {
...
static void parse_test(scanner_t *scanner) {
  test_set_t set = {0};
  ...
  while (!match_current_type(scanner, TOKEN_EOF) &&
         !match_current_type_and_text(scanner, TOKEN_PUNC, "}"))
...
static void parse_program(scanner_t *scanner) {
  while (!match_current_type(scanner, TOKEN_EOF))
    parse_test(scanner);
}

static void digest_file(const char *file) {
