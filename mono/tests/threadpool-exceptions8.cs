...
public class Tests {
  ...
  public static void runner() {
    // need to run the test in a domain so that we can deal with unhandled
    // exceptions
    ...
    var helper = (TaskAwaiterOnCompletedHelper) ad.CreateInstanceAndUnwrap(
        helperType.Assembly.ToString(), helperType.FullName);
    var holder = new ResultHolder();
    helper.TheTest(holder);
    // HACK: If everything went well, a thread is running in the other domain
    // and is blocked in OnUnhandled waiting for AllDone().  Don't send it.
    // Instead just exit without waiting.  If we send AllDone, the process will
    // terminate with a 255.  Don't try to unload the domain either, since the
    // other thread will never finish.
    //
    // helper.AllDone();
    // AppDomain.Unload (ad);
  ...
  public class TaskAwaiterOnCompletedHelper : MarshalByRefObject {
    ...
    public int TheRealTest() {
      ...
      //  capture the stack trace of the SpecialExn and allow WaitForExn() to
      //  finish waiting.
      ...
      // this is TaskToApm.Begin (..., callback) where the callback is
      // helper.WhenCompleted
      Task.Delay(100).GetAwaiter().OnCompleted(helper.WhenCompleted);

      var wasSet = helper.WaitForExn(
          10000); // wait upto 10 seconds for the task to throw
    ...
    public MethodBase ThrowerMethodInfo =>
        typeof(TaskAwaiterOnCompletedHelper)
            .GetMethod(nameof(WhenCompletedThrower));
    ...
    public void AllDone() { coord2.Set(); }
  }
}
