...
class C {
  class CrasherClass {
    ...
    static CrasherClass() {
      ...
      // Basic functionality
      Crashers.Add(
          new Tuple<String, Action>("MerpCrashManaged", MerpCrashManaged));
      ...
      Crashers.Add(
          new Tuple<String, Action>("MerpCrashMalloc", MerpCrashMalloc));

      Crashers.Add(
          new Tuple<String, Action>("MerpCrashNullFp", MerpCrashNullFp));
      Crashers.Add(new Tuple<String, Action>("MerpCrashExceptionHook",
                                             MerpCrashUnhandledExceptionHook));

      // Specific Edge Cases
      Crashers.Add(
          new Tuple<String, Action>("MerpCrashDladdr", MerpCrashDladdr));
      Crashers.Add(
          new Tuple<String, Action>("MerpCrashSnprintf", MerpCrashSnprintf));
      Crashers.Add(new Tuple<String, Action>("MerpCrashDomainUnload",
                                             MerpCrashDomainUnload));
      Crashers.Add(new Tuple<String, Action>("MerpCrashUnbalancedGCSafe",
                                             MerpCrashUnbalancedGCSafe));
      Crashers.Add(new Tuple<String, Action>("MerpCrashSignalTerm",
                                             MerpCrashSignalTerm));
      Crashers.Add(new Tuple<String, Action>("MerpCrashSignalTerm",
                                             MerpCrashSignalAbrt));
      Crashers.Add(
          new Tuple<String, Action>("MerpCrashSignalKill", MerpCrashSignalFpe));
      Crashers.Add(
          new Tuple<String, Action>("MerpCrashSignalKill", MerpCrashSignalBus));
      Crashers.Add(new Tuple<String, Action>("MerpCrashSignalSegv",
                                             MerpCrashSignalSegv));
      Crashers.Add(
          new Tuple<String, Action>("MerpCrashSignalIll", MerpCrashSignalIll));
    }
...
public static void MerpCrashSnprintf() { mono_test_MerpCrashSnprintf(); }

[DllImport("libtest")]
public static extern void
mono_test_MerpCrashDladdr();

public static void MerpCrashDladdr() { mono_test_MerpCrashDladdr(); }

[DllImport("libtest")]
public static extern void
mono_test_MerpCrashMalloc();

public static void MerpCrashMalloc() { mono_test_MerpCrashMalloc(); }

[DllImport("libtest")]
public static extern void
mono_test_MerpCrashLoaderLock();

public static void MerpCrashLoaderLock() { mono_test_MerpCrashLoaderLock(); }
...
public static void MerpCrashNullFp() { mono_test_MerpCrashNullFp(); }
...
public static void MerpCrashSignalTerm() { mono_test_MerpCrashSignalTerm(); }

[DllImport("libtest")]
public static extern void
mono_test_MerpCrashSignalAbrt();

public static void MerpCrashSignalAbrt() { mono_test_MerpCrashSignalAbrt(); }

[DllImport("libtest")]
public static extern void
mono_test_MerpCrashSignalFpe();

public static void MerpCrashSignalFpe() { mono_test_MerpCrashSignalFpe(); }

[DllImport("libtest")]
public static extern void
mono_test_MerpCrashSignalBus();

public static void MerpCrashSignalBus() { mono_test_MerpCrashSignalBus(); }

[DllImport("libtest")]
public static extern void
mono_test_MerpCrashSignalSegv();

public static void MerpCrashSignalSegv() { mono_test_MerpCrashSignalSegv(); }

[DllImport("libtest")]
public static extern void
mono_test_MerpCrashSignalIll();

public static void MerpCrashSignalIll() { mono_test_MerpCrashSignalIll(); }
...
public static void HandleException(object sender,
...
public static void SetupCrash(string configDir) {
  ...
  var m = monoType.GetMethod("EnableMicrosoftTelemetry",
  ...
  var m_params =
      new object[]{appBundleId, appSignature, appVersion, merpGUIPath,
                   eventType,   appPath,      configDir};
...
public static void TestValidate(string configDir, bool silent) {
  DumpLogCheck();
...
static void DumpLogSet() {
  ...
  var convert = monoType.GetMethod(
      "EnableCrashReportLog", BindingFlags.NonPublic | BindingFlags.Static);
...
static void DumpLogCheck() {
  var monoType = Type.GetType("Mono.Runtime", false);
  var convert = monoType.GetMethod(
      "CheckCrashReportLog", BindingFlags.NonPublic | BindingFlags.Static);
  var result = (int) convert.Invoke(null, new object[]{"./", true});
  // Value of enum
  string[] levels = new string[]{"None",
                                 "Setup",
                                 "SuspendHandshake",
                                 "UnmanagedStacks",
                                 "ManagedStacks",
  ...
  if ("MerpInvoke" == levels[result]) {
    ...
    throw new Exception(String.Format(
        "Crash level not done, failed in stage: {0}", levels[result]));
  }
}

public static void SpawnCrashingRuntime(string runtime, int testNum,
                                        bool silent) {
  ...
  if (args.Length == 0) {
    ...
    if (processExe == null)
      ...
      throw new ArgumentException(
          String.Format("Running native app {0}  isn't 'mono'"));
    ...
    if (failure_count > 0) {
      for (int i = 0; i < CrasherClass.Crashers.Count; i++) {
        if (failures[i] != null) {
          Console.WriteLine("Crash reporter failed test {0}",
                            CrasherClass.Crashers[i].Item1);
