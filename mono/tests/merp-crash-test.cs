...
class C {
  class CrasherClass {
    public struct Crasher {
      ...
      public Crasher(string name, Action action,
                     Action<object>validator = null) {
        Name = name;
        Action = action;
        Validator = validator;
      }
    ...
    static CrasherClass() {
      ...
      Crashers.Add(new Crasher("MerpCrashFailFast", MerpCrashFailFast,
                               ValidateFailFastMsg));

      Crashers.Add(new Crasher("MerpCrashNullFp", MerpCrashNullFp));
      Crashers.Add(new Crasher("MerpCrashExceptionHook",
      ...
      Crashers.Add(
          new Crasher("MerpCrashUnbalancedGCSafe", MerpCrashUnbalancedGCSafe));
    ...
    jsonGetKey(object o, string key) => (o as Dictionary<string, object>)[key];
    private static object jsonGetKeys(object o, params string[] keys) {
      try {
        foreach(var key in keys) { o = jsonGetKey(o, key); }
        return o;
      } catch (KeyNotFoundException e) {
        throw new ValidationException(
            String.Format("{0}, key not found, looking for key path [{1}]",
  ...
  public static void TestValidate(string configDir, bool silent,
                                  Action<object>validator = null) {
    DumpLogCheck(expected_level
                 : "MerpInvoke"); // we are expecting merp invoke to fail
    ...
    if (crashFileExists) {
      ...
      try {
        ...
        throw new Exception(String.Format("Validation failed '{0}', json: {1}",
                                          e.Message, crashFile));
      } catch (Exception e) {
  ...
  static void DumpLogCheck(string expected_level = "Done") {
    ...
    if (expected_level != levels[result])
      throw new Exception(
          String.Format("Crash level {0} does not match expected {1}",
  ...
  public static void SpawnCrashingRuntime(string runtime, int testNum,
    ...
    if (!silent) {
      ...
      Console.WriteLine("MONO_PATH={0} {1} {2} {3}", env, runtime, asm,
                        testNum);
    }
  ...
  public static void TestManagedException() {
    ...
    SetupCrash(configDir);
    var monoType = Type.GetType("Mono.Runtime", false);
    var m = monoType.GetMethod("ExceptionToState",
                               BindingFlags.NonPublic | BindingFlags.Static);
