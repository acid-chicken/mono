...
public class TestAssemblyLoad {
  ...
  public static int test_0_ReflectionOnlyLoadBytesBindingRedirect() {
    ...
    string path1 = Path.Combine(AppDomain.CurrentDomain.BaseDirectory,
                                "assembly-load-dir1", "LibStrongName.dll");

    // Try to load from dir1/LibStrongName.dll, despite
    // the assembly-refonly-load-bytes-bindingredirect.exe.config redirecting
    // old versions to be mapped to version 2.0.0.0 which is in the
    // assembly-load-dir2 directory
    //
    ...
    if (f1 == null) {
      Console.Error.WriteLine(
          "expected to find field OnlyInVersion1, but got null");
      return 3;
    }

    if (f1.FieldType != typeof (int)) {
      Console.Error.WriteLine("Field OnlyInVersion1 has type {0}, expected int",
                              f1.FieldType);
      return 5;
    }
    ...
    if (f2 != null) {
      Console.Error.WriteLine(
          "expected not to find field OnlyInVersion2, but got {0}", f2);
      return 4;
    }
