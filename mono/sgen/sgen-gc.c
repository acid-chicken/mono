 ...
 * 3) 8 byte alignment is the minimum and enough (not true for special
 * structures (SIMD), FIXME) 4) there is a function to get an object's size and
 * the number of elements in an array. 5) we know the special way bounds are
 * allocated for complex arrays 6) we know about proxies and how to treat them
 * when domains are unloaded
 *
 ...
 * technique. When the nursery is full we start a nursery collection: this is
 * performed with a copying GC. When the old generation is full we start a
 * copying GC of the old generation as well: this will be changed to mark&sweep
 * with copying when fragmentation becomes to severe in the future.  Maybe we'll
 * even do both during the same collection like IMMIX.
 *
 ...
 *    quickly find the objects that may be referenced conservatively and pin
 * them (this makes the first issues more important)
 * *) large objects are too expensive to be dealt with using copying GC: we
 * handle them with mark/sweep during major collections
 * *) some objects need to not move even if they are small (interned strings,
 * Type handles): we use mark/sweep for them, too: they are not allocated in the
 * nursery, but inside PinnedChunks regions
 */
 ...
 *) experiment with max small object size (very small right now - 2kb,
  ...
  *) add an option to mmap the whole heap in one chunk: it makes for many
     simplifications in the checks (put the nursery at the top and just use a
 single check for inclusion/exclusion): the issue this has is that on 32 bit
 systems it's not flexible (too much of the address space may be used by default
 or we can't increase the heap as needed) and we'd need a race-free mechanism to
 return memory back to the system (mprotect(PROT_NONE) will still keep the
 memory allocated if it was written to, munmap is needed, but the following mmap
 may not find the same segment free...)

 *) memzero the major fragments after restarting the world and optionally a
 smaller chunk at a time
...
/* If set, do a mod union consistency check before each finishing collection
 * pause */
...
gboolean sgen_nursery_canaries_enabled(void) { return enable_nursery_canaries; }
...
static SgenPointerQueue fin_ready_queue =
    SGEN_POINTER_QUEUE_INIT(INTERNAL_MEM_FINALIZE_READY);
static SgenPointerQueue critical_fin_queue =
    SGEN_POINTER_QUEUE_INIT(INTERNAL_MEM_FINALIZE_READY);

/* registered roots: the key to the hash is the root start address */
/*
 * Different kinds of roots are kept separate to speed up pin_from_roots () for
 * example.
 */
SgenHashTable sgen_roots_hash[ROOT_TYPE_NUM] = {
    SGEN_HASH_TABLE_INIT(INTERNAL_MEM_ROOTS_TABLE, INTERNAL_MEM_ROOT_RECORD,
                         sizeof(RootRecord), sgen_aligned_addr_hash, NULL),
    SGEN_HASH_TABLE_INIT(INTERNAL_MEM_ROOTS_TABLE, INTERNAL_MEM_ROOT_RECORD,
                         sizeof(RootRecord), sgen_aligned_addr_hash, NULL),
    SGEN_HASH_TABLE_INIT(INTERNAL_MEM_ROOTS_TABLE, INTERNAL_MEM_ROOT_RECORD,
                         sizeof(RootRecord), sgen_aligned_addr_hash, NULL)};
...
/* The size of a TLAB */
/* The bigger the value, the less often we have to go to the slow path to
 * allocate a new one, but the more space is wasted by threads not allocating
 * much memory.
 * FIXME: Tune this.
...
static void scan_from_registered_roots(char *addr_start, char *addr_end,
                                       int root_type, ScanCopyContext ctx);
...
#include <pthread.h>
static pthread_mutex_t sgen_atomic_spin_lock G_GNUC_UNUSED =
    PTHREAD_MUTEX_INITIALIZER;

static gint64 mono_sgen_atomic_cas_i64(volatile gint64 *dest, gint64 exch,
                                       gint64 comp) {
  gint64 old;
  int ret;

  pthread_cleanup_push((void (*)(void *))pthread_mutex_unlock,
                       (void *)&sgen_atomic_spin_lock);
...
sgen_workers_get_job_gray_queue(WorkerData *worker_data,
                                SgenGrayQueue *default_gray_queue) {
  if (worker_data)
    return &worker_data->private_gray_queue;
  SGEN_ASSERT(0, default_gray_queue,
              "Why don't we have a default gray queue when we're not running "
              "in a worker thread?");
  return default_gray_queue;
}

static void gray_queue_redirect(SgenGrayQueue *queue) {
  sgen_workers_take_from_queue(sgen_current_collection_generation, queue);
}

void sgen_scan_area_with_callback(char *start, char *end,
                                  IterateObjectCallbackFunc callback,
                                  void *data, gboolean allow_flags,
...
void sgen_add_to_global_remset(gpointer ptr, GCObject *obj) {
  SGEN_ASSERT(5, sgen_ptr_in_nursery(obj),
  ...
  if (!sgen_major_collector.is_concurrent) {
    SGEN_ASSERT(5, sgen_current_collection_generation != -1,
                "Global remsets can only be added during collections");
  } else {
    if (sgen_current_collection_generation == -1)
      SGEN_ASSERT(5, sgen_get_concurrent_collection_in_progress(),
                  "Global remsets outside of collection pauses can only be "
                  "added by the concurrent collector");
  }

  if (!object_is_pinned(obj))
    SGEN_ASSERT(5,
                sgen_minor_collector.is_split ||
                    sgen_get_concurrent_collection_in_progress(),
                "Non-pinned objects can only remain in nursery if it is a "
                "split nursery");
 ...
 *
 *   Scan objects in the gray stack until the stack is empty. This should be
 * called frequently after each object is copied, to achieve better locality and
 * cache usage.
 *
 */
gboolean sgen_drain_gray_stack(ScanCopyContext ctx) {
  SGEN_ASSERT(0, ctx.ops->drain_gray_stack,
              "Why do we have a scan/copy context with a missing drain gray "
              "stack function?");
 ...
 */
static int pin_objects_from_nursery_pin_queue(gboolean do_scan_objects,
                                              ScanCopyContext ctx) {
  ...
  while (start < end) {
    ...
    SGEN_ASSERT(0, addr >= start_nursery && addr < end_nursery,
    ...
    SGEN_ASSERT(0, idx < section->num_scan_start,
    ...
    do {
      ...
      canarified_obj_size = obj_size =
          ALIGN_UP(safe_object_get_size((GCObject *)search_start));
      ...
      if (!sgen_client_object_is_array_fill((GCObject *)search_start)) {
        ...
        if (addr >= search_start &&
            (char *)addr < (char *)search_start + obj_size) {
    ...
    if (do_scan_objects) {
      ...
      SGEN_LOG(4, "Pinned object %p, vtable %p (%s), count %d\n", obj_to_pin,
               *(void **)obj_to_pin,
               ...
               count);
      sgen_binary_protocol_pin(obj_to_pin, (gpointer)LOAD_VTABLE(obj_to_pin),
...
static void pin_objects_in_nursery(gboolean do_scan_objects,
                                   ScanCopyContext ctx) {
  size_t reduced_to;

  if (sgen_nursery_section->pin_queue_first_entry ==
      sgen_nursery_section->pin_queue_last_entry)
    return;

  reduced_to = pin_objects_from_nursery_pin_queue(do_scan_objects, ctx);
  sgen_nursery_section->pin_queue_last_entry =
      sgen_nursery_section->pin_queue_first_entry + reduced_to;
}

/*
 * This function is only ever called (via `collector_pin_object()` in
 * `sgen-copy-object.h`) when we can't promote an object because we're out of
 * memory.
 */
void sgen_pin_object(GCObject *object, SgenGrayQueue *queue) {
  SGEN_ASSERT(0, sgen_ptr_in_nursery(object),
              "We're only supposed to use this for pinning nursery objects "
              "when out of memory.");

  /*
   * All pinned objects are assumed to have been staged, so we need to stage as
   * well. Also, the count of staged objects shows that "late pinning" happened.
   */
  sgen_pin_stage_ptr(object);

  SGEN_PIN_OBJECT(object);
  sgen_binary_protocol_pin(object, (gpointer)LOAD_VTABLE(object),
                           safe_object_get_size(object));

  ++objects_pinned;
  sgen_pin_stats_register_object(object, GENERATION_NURSERY);

  GRAY_OBJECT_ENQUEUE_SERIAL(queue, object,
                             sgen_obj_get_descriptor_safe(object));
}

/* Sort the addresses in array in increasing order.
 * Done using a by-the book heap sort. Which has decent and stable performance,
 * is pretty cache efficient.
 */
...
/*
 * Scan the memory between start and end and queue values which could be
 * pointers to the area between start_nursery and end_nursery for later
 * consideration. Typically used for thread stacks.
 */
MONO_NO_SANITIZE_ADDRESS
void sgen_conservatively_pin_objects_from(void **start, void **end,
                                          void *start_nursery,
                                          void *end_nursery, int pin_type) {
  int count = 0;

  SGEN_ASSERT(0, ((mword)start & (SIZEOF_VOID_P - 1)) == 0,
...
static void pin_from_roots(void *start_nursery, void *end_nursery,
  ...
  void **start_root;
  RootRecord *root;
  SGEN_LOG(2, "Scanning pinned roots (%d bytes, %d/%d entries)",
           (int)roots_size, sgen_roots_hash[ROOT_TYPE_NORMAL].num_entries,
           sgen_roots_hash[ROOT_TYPE_PINNED].num_entries);
  /* objects pinned from the API are inside these roots */
  SGEN_HASH_TABLE_FOREACH(&sgen_roots_hash[ROOT_TYPE_PINNED], void **,
                          start_root, RootRecord *, root) {
    SGEN_LOG(6, "Pinned roots %p-%p", start_root, root->end_root);
    sgen_conservatively_pin_objects_from(start_root, (void **)root->end_root,
...
static void single_arg_user_copy_or_mark(GCObject **obj, void *gc_data) {
 ...
 */
static void precisely_scan_objects_from(void **start_root, void **end_root,
                                        char *n_start, char *n_end,
                                        SgenDescriptor desc,
                                        ScanCopyContext ctx) {
...
static void reset_heap_boundaries(void) {
...
void sgen_update_heap_boundaries(mword low, mword high) {
  ...
  do {
    ...
    if (low >= old)
      break;
  } while (SGEN_CAS_PTR((gpointer *)&lowest_heap_address, (gpointer)low,
  ...
  do {
    ...
    if (high <= old)
      break;
  } while (SGEN_CAS_PTR((gpointer *)&highest_heap_address, (gpointer)high,
                        (gpointer)old) != (gpointer)old);
}
 ...
 */
static void alloc_nursery(gboolean dynamic, size_t min_size, size_t max_size) {
  char *data;
  size_t scan_starts;
  ...
  if (dynamic) {
    ...
    SGEN_ASSERT(0, min_size == max_size,
                "We can't have nursery ranges for static configuration.");
    if (!min_size)
      min_size = max_size = SGEN_DEFAULT_NURSERY_SIZE;
  }

  SGEN_ASSERT(0, !sgen_nursery_section,
              "Why are we allocating the nursery twice?");
  SGEN_LOG(2,
           "Allocating nursery size: %" G_GSIZE_FORMAT
           "u, initial %" G_GSIZE_FORMAT "u",
           max_size, min_size);

  /* FIXME: handle OOM */
  sgen_nursery_section =
      (GCMemSection *)sgen_alloc_internal(INTERNAL_MEM_SECTION);
  ...
  sgen_nursery_section->scan_starts = (char **)sgen_alloc_internal_dynamic(
      sizeof(char *) * scan_starts, INTERNAL_MEM_SCAN_STARTS, TRUE);
  sgen_nursery_section->num_scan_start = scan_starts;

  sgen_nursery_allocator_set_nursery_bounds(data, min_size, max_size);
}

FILE *mono_gc_get_logfile(void) { return sgen_gc_debug_file; }
...
static void scan_finalizer_entries(SgenPointerQueue *fin_queue,
                                   ScanCopyContext ctx) {
  CopyOrMarkObjectFunc copy_func = ctx.ops->copy_or_mark_object;
  SgenGrayQueue *queue = ctx.queue;
  size_t i;
...
static const char *generation_name(int generation) {
  switch (generation) {
...
static void finish_gray_stack(int generation, ScanCopyContext ctx) {
  ...
  char *start_addr =
      generation == GENERATION_NURSERY ? sgen_get_nursery_start() : NULL;
  char *end_addr =
      generation == GENERATION_NURSERY ? sgen_get_nursery_end() : (char *)-1;
  SgenGrayQueue *queue = ctx.queue;

  sgen_binary_protocol_finish_gray_stack_start(sgen_timestamp(), generation);
  /*
   * We copied all the reachable objects. Now it's the time to copy
   * the objects that were not referenced by the roots, but by the copied
   * objects. we built a stack of objects pointed to by gray_start: they are
   ...
   * (use a flag since this is needed only on major collections). We need to
   * loop here as well, so keep a counter of marked LO (increasing it in
   * copy_object). To achieve better cache locality and cache usage, we drain
   * the gray stack frequently, after each object is copied, and just finish the
   * work here.
   */
  ...
  /*
  Reset bridge data, we might have lingering data from a previous collection if
  this is a major collection trigged by minor overflow.

  We must reset the gathered bridges since their original block might be
  evacuated due to major fragmentation in the meanwhile and the bridge code
  ...
  */
  ...
  /*
   * Mark all strong toggleref objects. This must be done before we walk
   * ephemerons or finalizers to ensure they see the full set of live objects.
   */
  sgen_client_mark_togglerefs(start_addr, end_addr, ctx);

  /*
   * Walk the ephemeron tables marking all values with reachable keys. This must
   * be completely done before processing finalizable objects and non-tracking
   * weak links to avoid finalizing/clearing objects that are in fact reachable.
   */
  ...
  if (sgen_client_bridge_need_processing()) {
    /*Make sure the gray stack is empty before we process bridge objects so we
     * get liveness right*/
    sgen_drain_gray_stack(ctx);
    ...
    /*
    Do the first bridge step here, as the collector liveness state will become
    useless after that.

    An important optimization is to only proccess the possibly dead part of the
    object graph and skip over all live objects as we transitively know
    everything they point must be alive too.

    The above invariant is completely wrong if we let the gray queue be drained
    and mark/copy everything.

    This has the unfortunate side effect of making overflow collections perform
    the first step twice, but given we now have heuristics that perform major GC
    in anticipation of minor overflows this should not be a big deal.
    */
    sgen_client_bridge_processing_stw_step();
  }

  /*
  Make sure we drain the gray stack before processing disappearing links and
  finalizers. If we don't make sure it is empty we might wrongly see a live
  object as dead.
  */
  sgen_drain_gray_stack(ctx);

  /*
  We must clear weak links that don't track resurrection before processing
  object ready for finalization so they can be cleared before that.
  */
   ...
   * finalized: use the finalized objects as new roots so the objects they
   * depend on are also not reclaimed. As with the roots above, only objects in
   * the nursery are marked/copied.
   */
  ...
  /*
   * This must be done again after processing finalizable objects since CWL
   * slots are cleared only after the key is finalized.
   */
  done_with_ephemerons = 0;
  do {
    done_with_ephemerons = sgen_client_mark_ephemerons(ctx);
  ...
  TV_GETTIME(btv);
  SGEN_LOG(2,
           "Finalize queue handling scan for %s generation: %" PRId64
           " usecs %d ephemeron rounds",
           generation_name(generation), (gint64)(TV_ELAPSED(atv, btv) / 10),
           ephemeron_rounds);
   ...
   * This also deals with a possible issue with the monitor reclamation: with
   * the Boehm GC a finalized object my lose the monitor because it is cleared
...
void sgen_check_section_scan_starts(GCMemSection *section) {
  ...
  for (i = 0; i < section->num_scan_start; ++i) {
    if (section->scan_starts[i]) {
      ...
      SGEN_ASSERT(0,
                  size >= SGEN_CLIENT_MINIMUM_OBJECT_SIZE &&
                      size <= MAX_SMALL_OBJ_SIZE,
...
static void scan_from_registered_roots(char *addr_start, char *addr_end,
  ...
  void **start_root;
  RootRecord *root;
  SGEN_HASH_TABLE_FOREACH(&sgen_roots_hash[root_type], void **, start_root,
                          RootRecord *, root) {
    SGEN_LOG(6, "Precise root scan %p-%p (desc: %p)", start_root,
             root->end_root, (void *)(uintptr_t)root->root_desc);
    precisely_scan_objects_from(start_root, (void **)root->end_root, addr_start,
                                addr_end, root->root_desc, ctx);
  }
  SGEN_HASH_TABLE_FOREACH_END;
}
...
static void init_stats(void) {
  ...
  mono_counters_register("Collection max time",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME | MONO_COUNTER_MONOTONIC,
                         &time_max);

  mono_counters_register("Minor fragment clear",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_minor_pre_collection_fragment_clear);
  mono_counters_register(
      "Minor pinning", MONO_COUNTER_GC | MONO_COUNTER_ULONG | MONO_COUNTER_TIME,
      &time_minor_pinning);
  mono_counters_register("Minor scan remembered set",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_minor_scan_remsets);
  mono_counters_register("Minor scan major blocks",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_minor_scan_major_blocks);
  mono_counters_register("Minor scan los",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_minor_scan_los);
  mono_counters_register("Minor scan pinned",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_minor_scan_pinned);
  mono_counters_register("Minor scan roots",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_minor_scan_roots);
  mono_counters_register("Minor fragment creation",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_minor_fragment_creation);

  mono_counters_register("Major fragment clear",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_major_pre_collection_fragment_clear);
  mono_counters_register(
      "Major pinning", MONO_COUNTER_GC | MONO_COUNTER_ULONG | MONO_COUNTER_TIME,
      &time_major_pinning);
  mono_counters_register("Major scan pinned",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_major_scan_pinned);
  mono_counters_register("Major scan roots",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
  ...
  mono_counters_register("Major scan mod union los",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_major_scan_mod_union_los);
  mono_counters_register("Major finish gray stack",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
  ...
  mono_counters_register("Major LOS sweep",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_major_los_sweep);
  mono_counters_register(
      "Major sweep", MONO_COUNTER_GC | MONO_COUNTER_ULONG | MONO_COUNTER_TIME,
      &time_major_sweep);
  mono_counters_register("Major fragment creation",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG |
                             MONO_COUNTER_TIME,
                         &time_major_fragment_creation);

  mono_counters_register("Number of pinned objects",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_pinned_objects);

#ifdef HEAVY_STATISTICS
  mono_counters_register("WBarrier remember pointer",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_wbarrier_add_to_global_remset);
  mono_counters_register("WBarrier arrayref copy",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_wbarrier_arrayref_copy);
  mono_counters_register("WBarrier generic store called",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_wbarrier_generic_store);
  mono_counters_register("WBarrier generic atomic store called",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_wbarrier_generic_store_atomic);
  mono_counters_register("WBarrier set root",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_wbarrier_set_root);

  mono_counters_register("# objects allocated degraded",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_objects_alloced_degraded);
  mono_counters_register("bytes allocated degraded",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_bytes_alloced_degraded);

  mono_counters_register("# copy_object() called (nursery)",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_copy_object_called_nursery);
  mono_counters_register("# objects copied (nursery)",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_objects_copied_nursery);
  mono_counters_register("# copy_object() called (major)",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_copy_object_called_major);
  mono_counters_register("# objects copied (major)",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_objects_copied_major);

  mono_counters_register("# scan_object() called (nursery)",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_scan_object_called_nursery);
  mono_counters_register("# scan_object() called (major)",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_scan_object_called_major);

  mono_counters_register("Slots allocated in vain",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_slots_allocated_in_vain);

  mono_counters_register("# nursery copy_object() failed from space",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_nursery_copy_object_failed_from_space);
  mono_counters_register("# nursery copy_object() failed forwarded",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_nursery_copy_object_failed_forwarded);
  mono_counters_register("# nursery copy_object() failed pinned",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_nursery_copy_object_failed_pinned);
  mono_counters_register("# nursery copy_object() failed to space",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
...
static void reset_pinned_from_failed_allocation(void) {
...
gboolean sgen_collection_is_concurrent(void) {
  ...
  default:
    g_error("Invalid current generation %d",
            sgen_current_collection_generation);
  }
  return FALSE;
}
...
static ScanCopyContext scan_copy_context_for_scan_job(void *worker_data_untyped,
                                                      ScanJob *job) {
  ...
  if (!job->ops) {
     ...
     */
    SGEN_ASSERT(0, sgen_workers_is_worker_thread(mono_native_thread_id_get()),
                "We need a context for the scan job");
    job->ops = sgen_workers_get_idle_func_object_ops(worker_data);
  }

  return CONTEXT_FROM_OBJECT_OPERATIONS(
      job->ops,
      sgen_workers_get_job_gray_queue(worker_data, job->gc_thread_gray_queue));
}
...
static void job_scan_from_registered_roots(void *worker_data_untyped,
                                           SgenThreadPoolJob *job) {
  ScanFromRegisteredRootsJob *job_data = (ScanFromRegisteredRootsJob *)job;
  ScanCopyContext ctx =
      scan_copy_context_for_scan_job(worker_data_untyped, &job_data->scan_job);

  scan_from_registered_roots(job_data->heap_start, job_data->heap_end,
                             job_data->root_type, ctx);
}
...
static void job_scan_thread_data(void *worker_data_untyped,
                                 SgenThreadPoolJob *job) {
...
static void job_scan_finalizer_entries(void *worker_data_untyped,
                                       SgenThreadPoolJob *job) {
...
static void job_scan_wbroots(void *worker_data_untyped,
                             SgenThreadPoolJob *job) {
  ScanJob *job_data = (ScanJob *)job;
  ScanCopyContext ctx =
      scan_copy_context_for_scan_job(worker_data_untyped, job_data);

  sgen_wbroots_scan_card_table(ctx);
}

static void job_scan_major_card_table(void *worker_data_untyped,
                                      SgenThreadPoolJob *job) {
  SGEN_TV_DECLARE(atv);
  SGEN_TV_DECLARE(btv);
  ParallelScanJob *job_data = (ParallelScanJob *)job;
  ScanCopyContext ctx =
      scan_copy_context_for_scan_job(worker_data_untyped, (ScanJob *)job_data);

  SGEN_TV_GETTIME(atv);
  sgen_major_collector.scan_card_table(
      CARDTABLE_SCAN_GLOBAL, ctx, job_data->job_index,
      job_data->job_split_count, job_data->data);
  SGEN_TV_GETTIME(btv);
...
static void job_scan_los_card_table(void *worker_data_untyped,
                                    SgenThreadPoolJob *job) {
  SGEN_TV_DECLARE(atv);
  SGEN_TV_DECLARE(btv);
  ParallelScanJob *job_data = (ParallelScanJob *)job;
  ...
  SGEN_TV_GETTIME(atv);
  sgen_los_scan_card_table(CARDTABLE_SCAN_GLOBAL, ctx, job_data->job_index,
                           job_data->job_split_count);
  SGEN_TV_GETTIME(btv);
...
static void job_scan_major_mod_union_card_table(void *worker_data_untyped,
                                                SgenThreadPoolJob *job) {
  SGEN_TV_DECLARE(atv);
  SGEN_TV_DECLARE(btv);
  ParallelScanJob *job_data = (ParallelScanJob *)job;
  ...
  SGEN_TV_GETTIME(atv);
  sgen_major_collector.scan_card_table(
      CARDTABLE_SCAN_MOD_UNION, ctx, job_data->job_index,
      job_data->job_split_count, job_data->data);
  SGEN_TV_GETTIME(btv);
...
static void job_scan_los_mod_union_card_table(void *worker_data_untyped,
                                              SgenThreadPoolJob *job) {
  SGEN_TV_DECLARE(atv);
  SGEN_TV_DECLARE(btv);
  ParallelScanJob *job_data = (ParallelScanJob *)job;
  ...
  g_assert(sgen_concurrent_collection_in_progress);
  SGEN_TV_GETTIME(atv);
  sgen_los_scan_card_table(CARDTABLE_SCAN_MOD_UNION, ctx, job_data->job_index,
                           job_data->job_split_count);
  SGEN_TV_GETTIME(btv);
...
static void job_major_mod_union_preclean(void *worker_data_untyped,
                                         SgenThreadPoolJob *job) {
  SGEN_TV_DECLARE(atv);
  SGEN_TV_DECLARE(btv);
  ParallelScanJob *job_data = (ParallelScanJob *)job;
  ...
  SGEN_TV_GETTIME(atv);
  sgen_major_collector.scan_card_table(
      CARDTABLE_SCAN_MOD_UNION_PRECLEAN, ctx, job_data->job_index,
      job_data->job_split_count, job_data->data);
  SGEN_TV_GETTIME(btv);

  g_assert(worker_data_untyped);
  ((WorkerData *)worker_data_untyped)->major_scan_time +=
      SGEN_TV_ELAPSED(atv, btv);
}

static void job_los_mod_union_preclean(void *worker_data_untyped,
                                       SgenThreadPoolJob *job) {
  SGEN_TV_DECLARE(atv);
  SGEN_TV_DECLARE(btv);
  ParallelScanJob *job_data = (ParallelScanJob *)job;
  ...
  sgen_los_scan_card_table(CARDTABLE_SCAN_MOD_UNION_PRECLEAN, ctx,
                           job_data->job_index, job_data->job_split_count);
  SGEN_TV_GETTIME(btv);

  g_assert(worker_data_untyped);
  ((WorkerData *)worker_data_untyped)->los_scan_time +=
      SGEN_TV_ELAPSED(atv, btv);
}

static void job_scan_last_pinned(void *worker_data_untyped,
                                 SgenThreadPoolJob *job) {
  ScanJob *job_data = (ScanJob *)job;
  ScanCopyContext ctx =
      scan_copy_context_for_scan_job(worker_data_untyped, job_data);
...
static void workers_finish_callback(void) {
  ParallelScanJob *psj;
  ScanJob *sj;
  ...
  for (i = 0; i < split_count; i++) {
    psj = (ParallelScanJob *)sgen_thread_pool_job_alloc(
        "preclean major mod union cardtable", job_major_mod_union_preclean,
  ...
  for (i = 0; i < split_count; i++) {
    psj = (ParallelScanJob *)sgen_thread_pool_job_alloc(
        "preclean los mod union cardtable", job_los_mod_union_preclean,
        sizeof(ParallelScanJob));
    psj->scan_job.gc_thread_gray_queue = NULL;
    psj->job_index = i;
    psj->job_split_count = split_count;
  ...
  sj = (ScanJob *)sgen_thread_pool_job_alloc(
      "scan last pinned", job_scan_last_pinned, sizeof(ScanJob));
  sj->gc_thread_gray_queue = NULL;
  sgen_workers_enqueue_job(GENERATION_OLD, &sj->job, TRUE);
}

static void init_gray_queue(SgenGrayQueue *gc_thread_gray_queue) {
  sgen_gray_object_queue_init(gc_thread_gray_queue, NULL, TRUE);
}

static void
enqueue_scan_remembered_set_jobs(SgenGrayQueue *gc_thread_gray_queue,
                                 SgenObjectOperations *ops, gboolean enqueue) {
  int i, split_count = sgen_workers_get_job_split_count(GENERATION_NURSERY);
  size_t num_major_sections = sgen_major_collector.get_num_major_sections();
  ScanJob *sj;

  sj = (ScanJob *)sgen_thread_pool_job_alloc("scan wbroots", job_scan_wbroots,
                                             sizeof(ScanJob));
  sj->ops = ops;
  ...
  for (i = 0; i < split_count; i++) {
    ...
    psj = (ParallelScanJob *)sgen_thread_pool_job_alloc(
        "scan major remsets", job_scan_major_card_table,
        sizeof(ParallelScanJob));
    ...
    psj = (ParallelScanJob *)sgen_thread_pool_job_alloc(
        "scan LOS remsets", job_scan_los_card_table, sizeof(ParallelScanJob));
    psj->scan_job.ops = ops;
    psj->scan_job.gc_thread_gray_queue = gc_thread_gray_queue;
    psj->job_index = i;
    psj->job_split_count = split_count;
...
static void enqueue_scan_from_roots_jobs(SgenGrayQueue *gc_thread_gray_queue,
  ...
  scrrj = (ScanFromRegisteredRootsJob *)sgen_thread_pool_job_alloc(
      "scan from registered roots normal", job_scan_from_registered_roots,
  ...
  sgen_workers_enqueue_job(sgen_current_collection_generation,
                           &scrrj->scan_job.job, enqueue);

  if (sgen_current_collection_generation == GENERATION_OLD) {
    /* During minors we scan the cardtable for these roots instead */
    scrrj = (ScanFromRegisteredRootsJob *)sgen_thread_pool_job_alloc(
        "scan from registered roots wbarrier", job_scan_from_registered_roots,
        sizeof(ScanFromRegisteredRootsJob));
    scrrj->scan_job.ops = ops;
    scrrj->scan_job.gc_thread_gray_queue = gc_thread_gray_queue;
    scrrj->heap_start = heap_start;
  ...
  stdj = (ScanThreadDataJob *)sgen_thread_pool_job_alloc(
      "scan thread data", job_scan_thread_data, sizeof(ScanThreadDataJob));
  ...
  sgen_workers_enqueue_job(sgen_current_collection_generation,
                           &stdj->scan_job.job, enqueue);

  /* Scan the list of objects ready for finalization. */

  sfej = (ScanFinalizerEntriesJob *)sgen_thread_pool_job_alloc(
      "scan finalizer entries", job_scan_finalizer_entries,
  ...
  sgen_workers_enqueue_job(sgen_current_collection_generation,
                           &sfej->scan_job.job, enqueue);

  sfej = (ScanFinalizerEntriesJob *)sgen_thread_pool_job_alloc(
      "scan critical finalizer entries", job_scan_finalizer_entries,
      sizeof(ScanFinalizerEntriesJob));
  sfej->scan_job.ops = ops;
  sfej->scan_job.gc_thread_gray_queue = gc_thread_gray_queue;
 ...
 */
static gboolean collect_nursery(const char *reason, gboolean is_overflow) {
  ...
  sgen_binary_protocol_collection_begin(
      mono_atomic_load_i32(&mono_gc_stats.minor_gc_count), GENERATION_NURSERY);
  ...
  SGEN_ASSERT(0, !sgen_collection_is_concurrent(),
  ...
  SGEN_LOG(1, "Start nursery collection %" G_GINT32_FORMAT " %p-%p, size: %d",
           mono_atomic_load_i32(&mono_gc_stats.minor_gc_count),
           sgen_nursery_section->data, sgen_nursery_section->end_data,
           (int)(sgen_nursery_section->end_data - sgen_nursery_section->data));
  ...
  pin_from_roots(sgen_nursery_section->data, sgen_nursery_section->end_data,
                 ctx);
  ...
  SGEN_LOG(2,
           "Finding pinned pointers: %" G_GSIZE_FORMAT "d in %" PRId64 " usecs",
           sgen_get_pinned_count(), (gint64)(TV_ELAPSED(btv, atv) / 10));
  SGEN_LOG(4, "Start scan with %" G_GSIZE_FORMAT "d pinned objects",
  ...
  SGEN_LOG(2, "Minor scan copy/clear remsets: %lld usecs",
           (long long)(TV_ELAPSED(atv, btv) / 10));

  TV_GETTIME(atv);
  enqueue_scan_remembered_set_jobs(&gc_thread_gray_queue,
                                   is_parallel ? NULL : object_ops_nopar,
                                   is_parallel);
  TV_GETTIME(btv);

  if (!is_parallel) {
    time_minor_scan_remsets += TV_ELAPSED(atv, btv);
    SGEN_LOG(2, "Old generation scan: %" PRId64 " usecs",
             (gint64)(TV_ELAPSED(atv, btv) / 10));
  }
  ...
  enqueue_scan_from_roots_jobs(
      &gc_thread_gray_queue, sgen_nursery_section->data,
      sgen_nursery_section->end_data, is_parallel ? NULL : object_ops_nopar,
      is_parallel);

  if (is_parallel) {
    gray_queue_redirect(&gc_thread_gray_queue);
    sgen_workers_start_all_workers(GENERATION_NURSERY, object_ops_nopar,
                                   object_ops_par, NULL);
  ...
  if (!is_parallel) {
    ...
    SGEN_LOG(
        2,
        "Minor scan remsets: accumulated major scan=%lld usecs, accumulated "
        "los scan=%lld usecs, workers=%d",
   ...
   * Invariants: Heap's finished, no more moves left, objects still pinned in
   * nursery.
   */
  sgen_client_collecting_minor_report_roots(&fin_ready_queue,
                                            &critical_fin_queue);

  /* walk the pin_queue, build up the fragment list of free memory, unmark
   * pinned objects as we go, memzero() the empty fragments so they are ready
   * for the next allocations.
   */
  ...
  SGEN_LOG(2, "Fragment creation: %" PRId64 " usecs, %lu bytes available",
           (gint64)TV_ELAPSED(atv, btv), (unsigned long)fragment_total);
  ...
  UnlockedAdd64(
      &mono_gc_stats.minor_gc_time,
      TV_ELAPSED(last_minor_collection_start_tv, last_minor_collection_end_tv));

  sgen_debug_dump_heap(
      "minor", mono_atomic_load_i32(&mono_gc_stats.minor_gc_count) - 1, NULL);
  ...
  /*objects are late pinned because of lack of memory, so a major is a good
   * call*/
  ...
  if (is_parallel)
    sgen_binary_protocol_collection_end_stats(
        0, 0, time_minor_finish_gray_stack - finish_gray_start);
  else
    sgen_binary_protocol_collection_end_stats(
  ...
  sgen_binary_protocol_collection_end(
      mono_atomic_load_i32(&mono_gc_stats.minor_gc_count) - 1,
      GENERATION_NURSERY, 0, 0);
...
static void major_copy_or_mark_from_roots(
    SgenGrayQueue *gc_thread_gray_queue, size_t *old_next_pin_slot,
    CopyOrMarkFromRootsMode mode, SgenObjectOperations *object_ops_nopar,
    SgenObjectOperations *object_ops_par) {
  TV_DECLARE(atv);
  TV_DECLARE(btv);
  ...
  ScanCopyContext ctx =
      CONTEXT_FROM_OBJECT_OPERATIONS(object_ops_nopar, gc_thread_gray_queue);
  gboolean concurrent = mode != COPY_OR_MARK_FROM_ROOTS_SERIAL;

  SGEN_ASSERT(0, !!concurrent == !!sgen_concurrent_collection_in_progress,
  ...
  sgen_los_pin_objects(gc_thread_gray_queue,
                       mode == COPY_OR_MARK_FROM_ROOTS_FINISH_CONCURRENT);
  ...
  SGEN_LOG(2,
           "Finding pinned pointers: %" G_GSIZE_FORMAT "d in %" PRId64 " usecs",
           sgen_get_pinned_count(), (gint64)(TV_ELAPSED(atv, btv) / 10));
  SGEN_LOG(4, "Start scan with %" G_GSIZE_FORMAT "d pinned objects",
           sgen_get_pinned_count());
  sgen_client_pinning_end();
  ...
  SGEN_ASSERT(0, sgen_workers_all_done(),
              "Why are the workers not done when we start or finish a major "
              "collection?");
  if (mode == COPY_OR_MARK_FROM_ROOTS_FINISH_CONCURRENT) {
    ...
    if (object_ops_par == NULL && sgen_workers_have_idle_work(GENERATION_OLD)) {
       ...
       */
      sgen_workers_start_all_workers(GENERATION_OLD, object_ops_nopar,
                                     object_ops_par, NULL);
  ...
  enqueue_scan_from_roots_jobs(gc_thread_gray_queue, heap_start, heap_end,
                               object_ops_nopar, FALSE);
  ...
  if (mode == COPY_OR_MARK_FROM_ROOTS_START_CONCURRENT) {
    ...
    if (precleaning_enabled) {
      sgen_workers_start_all_workers(GENERATION_OLD, object_ops_nopar,
                                     object_ops_par, workers_finish_callback);
    } else {
      sgen_workers_start_all_workers(GENERATION_OLD, object_ops_nopar,
                                     object_ops_par, NULL);
    }
  ...
  if (mode == COPY_OR_MARK_FROM_ROOTS_FINISH_CONCURRENT) {
    ...
    for (i = 0; i < split_count; i++) {
      ...
      psj = (ParallelScanJob *)sgen_thread_pool_job_alloc(
          "scan mod union cardtable", job_scan_major_mod_union_card_table,
          sizeof(ParallelScanJob));
      ...
      psj = (ParallelScanJob *)sgen_thread_pool_job_alloc(
          "scan LOS mod union cardtable", job_scan_los_mod_union_card_table,
          sizeof(ParallelScanJob));
      psj->scan_job.ops = parallel ? NULL : object_ops_nopar;
      psj->scan_job.gc_thread_gray_queue = gc_thread_gray_queue;
      psj->job_index = i;
    ...
    if (parallel) {
      /*
       * If we enqueue a job while workers are running we need to
       * sgen_workers_ensure_awake in order to make sure that we are running the
       * idle func and draining all worker gray queues. The operation of
       * starting workers implies this, so we start them after in order to avoid
       * doing this operation twice. The workers will drain the main gray stack
       * that contained roots and pinned objects and also scan the mod union
       * card table.
       */
      sgen_workers_start_all_workers(GENERATION_OLD, object_ops_nopar,
                                     object_ops_par, NULL);
...
static void major_start_collection(SgenGrayQueue *gc_thread_gray_queue,
                                   const char *reason, gboolean concurrent,
  ...
  sgen_binary_protocol_collection_begin(
      mono_atomic_load_i32(&mono_gc_stats.major_gc_count), GENERATION_OLD);
  ...
  sgen_degraded_mode = 0;
  SGEN_LOG(1, "Start major collection %" G_GINT32_FORMAT,
           mono_atomic_load_i32(&mono_gc_stats.major_gc_count));
  ...
  major_copy_or_mark_from_roots(gc_thread_gray_queue, old_next_pin_slot,
                                concurrent
                                    ? COPY_OR_MARK_FROM_ROOTS_START_CONCURRENT
                                    : COPY_OR_MARK_FROM_ROOTS_SERIAL,
                                object_ops_nopar, object_ops_par);
}

static void major_finish_collection(SgenGrayQueue *gc_thread_gray_queue,
                                    const char *reason, gboolean is_overflow,
                                    size_t old_next_pin_slot, gboolean forced) {
  ...
  if (sgen_concurrent_collection_in_progress) {
    ...
    major_copy_or_mark_from_roots(gc_thread_gray_queue, NULL,
                                  COPY_OR_MARK_FROM_ROOTS_FINISH_CONCURRENT,
                                  object_ops_nopar, object_ops_par);
  ...
  if (objects_pinned) {
     ...
     */
    sgen_pin_queue_clear_discarded_entries(sgen_nursery_section,
                                           old_next_pin_slot);
    /*
  ...
  sgen_update_heap_boundaries((mword)sgen_get_nursery_start(),
                              (mword)sgen_get_nursery_end());

  /*
   * We collect the roots before unpinning objects in the nursery since we need
   * to have object liveness information for ephemeron root reporting.
   */
  sgen_client_collecting_major_report_roots(&fin_ready_queue,
                                            &critical_fin_queue);
  ...
  sgen_binary_protocol_sweep_begin(GENERATION_OLD,
                                   !sgen_major_collector.sweeps_lazily);
  ...
  sgen_binary_protocol_sweep_end(GENERATION_OLD,
                                 !sgen_major_collector.sweeps_lazily);
  ...
  sgen_debug_dump_heap(
      "major", mono_atomic_load_i32(&mono_gc_stats.major_gc_count) - 1, reason);
  ...
  sgen_memgov_major_collection_end(
      forced, sgen_concurrent_collection_in_progress, reason, is_overflow);
  sgen_current_collection_generation = -1;
  ...
  if (sgen_major_collector.is_parallel)
    sgen_binary_protocol_collection_end_stats(
        0, 0, time_major_finish_gray_stack - finish_gray_start);
  else
    sgen_binary_protocol_collection_end_stats(
  ...
  sgen_binary_protocol_collection_end(
      mono_atomic_load_i32(&mono_gc_stats.major_gc_count) - 1, GENERATION_OLD,
      counts.num_scanned_objects, counts.num_unique_scanned_objects);

  if (sgen_concurrent_collection_in_progress)
    sgen_concurrent_collection_in_progress = FALSE;
}

static gboolean major_do_collection(const char *reason, gboolean is_overflow,
                                    gboolean forced) {
  ...
  init_gray_queue(&gc_thread_gray_queue);
  major_start_collection(&gc_thread_gray_queue, reason, FALSE,
                         &old_next_pin_slot);
  major_finish_collection(&gc_thread_gray_queue, reason, is_overflow,
                          old_next_pin_slot, forced);
  sgen_gray_object_queue_dispose(&gc_thread_gray_queue);
...
static void major_start_concurrent_collection(const char *reason) {
  TV_DECLARE(time_start);
  TV_DECLARE(time_end);
 ...
 */
static gboolean major_should_finish_concurrent_collection(void) {
  return sgen_workers_all_done();
}

static void major_update_concurrent_collection(void) {
  ...
  UnlockedAdd64(&mono_gc_stats.major_gc_time,
                TV_ELAPSED(total_start, total_end));
}

static void major_finish_concurrent_collection(gboolean forced) {
  SgenGrayQueue gc_thread_gray_queue;
  TV_DECLARE(total_start);
  TV_DECLARE(total_end);
  ...
  UnlockedAdd64(&mono_gc_stats.major_gc_time_concurrent,
  ...
  sgen_cement_reset();
  init_gray_queue(&gc_thread_gray_queue);
  major_finish_collection(&gc_thread_gray_queue, "finishing", FALSE, -1,
                          forced);
  sgen_gray_object_queue_dispose(&gc_thread_gray_queue);
...
void sgen_ensure_free_space(size_t size, int generation) {
  ...
  if (generation == GENERATION_OLD) {
    ...
    if (sgen_degraded_mode) {
      ...
      reason = sgen_concurrent_collection_in_progress
                   ? "Forced finish concurrent collection"
                   : "Minor allowance";
      generation_to_collect = GENERATION_OLD;
    } else {
 ...
 */
static void sgen_perform_collection_inner(size_t requested_size,
  ...
  gboolean finish_concurrent = sgen_concurrent_collection_in_progress &&
                               (major_should_finish_concurrent_collection() ||
                                generation_to_collect == GENERATION_OLD);

  sgen_binary_protocol_collection_requested(
      generation_to_collect, requested_size, forced_serial ? 1 : 0);
  ...
  if (stw)
    sgen_stop_world(generation_to_collect,
                    forced_serial || !sgen_major_collector.is_concurrent);
  else
    SGEN_ASSERT(0, sgen_is_world_stopped(),
                "We can only collect if the world is stopped");
  ...
  if (generation_to_collect == GENERATION_NURSERY && !finish_concurrent) {
    ...
    if (collect_nursery(reason, FALSE) &&
    ...
    SGEN_ASSERT(0, generation_to_collect == GENERATION_OLD,
  ...
  if (overflow_generation_to_collect != -1) {
    SGEN_ASSERT(0, !sgen_concurrent_collection_in_progress,
                "We don't yet support overflow collections with the concurrent "
                "collector");
  ...
  if (generation_to_collect == GENERATION_NURSERY &&
    ...
    SGEN_LOG(1,
             "nursery collection didn't find enough room for %" G_GSIZE_FORMAT
             "d alloc (%" G_GSIZE_FORMAT "d pinned)",
...
static void gc_pump_callback(void) {
  sgen_perform_collection_inner(gc_request.requested_size,
                                gc_request.generation_to_collect,
                                gc_request.reason, TRUE, TRUE);
...
void sgen_perform_collection(size_t requested_size, int generation_to_collect,
                             const char *reason, gboolean forced_serial,
                             gboolean stw) {
#ifdef HOST_WASM
  if (!mono_wasm_enable_gc) {
    ...
    // There's a window for racing where we're executing other bg jobs before
    // the GC, they trigger a GC request and it overrides this one. I belive this
    // case to be benign as it will, in the worst case, upgrade a minor to a
    // major collection.
  ...
  sgen_perform_collection_inner(requested_size, generation_to_collect, reason,
                                forced_serial, stw);
}
/*
 * ######################################################################
 ...
 * Debug reporting.
 */
G_GNUC_UNUSED static void report_internal_mem_usage(void) {
 ...
 */
static int sgen_is_object_alive_and_on_current_collection(GCObject *object) {
...
void sgen_queue_finalization_entry(GCObject *obj) {
  ...
  sgen_pointer_queue_add(critical ? &critical_fin_queue : &fin_ready_queue,
                         obj);
...
/*
 * `System.GC.WaitForPendingFinalizers` first checks
 * `sgen_have_pending_finalizers()` to determine whether it can exit quickly.
 * The latter must therefore only return FALSE if all finalizers have really
 * finished running.
 *
 * `sgen_gc_invoke_finalizers()` first dequeues a finalizable object, and then
 * finalizes it. This means that just checking whether the queues are empty
 * leaves the possibility that an object might have been dequeued but not yet
 * finalized.  That's why we need the additional flag
...
void sgen_set_suspend_finalizers(void) { sgen_suspend_finalizers = TRUE; }
...
int sgen_gc_invoke_finalizers(void) {
  ...
  while (sgen_have_pending_finalizers()) {
    ...
    if (obj)
      SGEN_LOG(7, "Finalizing object %p (%s)", obj,
               sgen_client_vtable_get_name(SGEN_LOAD_VTABLE(obj)));
     ...
     * sgen_client_object_safe_name (obj));*/
...
gboolean sgen_have_pending_finalizers(void) {
  if (sgen_suspend_finalizers)
    return FALSE;
  return pending_unqueued_finalizer ||
 ...
 */
int sgen_register_root(char *start, size_t size, SgenDescriptor descr,
                       int root_type, MonoGCRootSource source, void *key,
                       const char *msg) {
  RootRecord new_root;
  int i;
  ...
  for (i = 0; i < ROOT_TYPE_NUM; ++i) {
    RootRecord *root =
        (RootRecord *)sgen_hash_table_lookup(&sgen_roots_hash[i], start);
    ...
    if (root) {
      ...
      SGEN_ASSERT(0, !!root->root_desc == !!descr,
                  "Can't change whether a root is precise or conservative.");
      SGEN_ASSERT(0, root->source == source,
  ...
  SGEN_LOG(3, "Added root for range: %p-%p, descr: %" PRIx64 "  (%d/%d bytes)",
           start, new_root.end_root, (gint64)descr, (int)size, (int)roots_size);
...
void sgen_wbroots_iterate_live_block_ranges(sgen_cardtable_block_callback cb) {
  void **start_root;
  RootRecord *root;
  SGEN_HASH_TABLE_FOREACH(&sgen_roots_hash[ROOT_TYPE_WBARRIER], void **,
                          start_root, RootRecord *, root) {
...
static void sgen_wbroot_scan_card_table(void **start_root, mword size,
                                        ScanCopyContext ctx) {
  ScanPtrFieldFunc scan_field_func = ctx.ops->scan_ptr_field;
  guint8 *card_data = sgen_card_table_get_card_scan_address((mword)start_root);
  guint8 *card_base = card_data;
  mword card_count =
      sgen_card_table_number_of_cards_in_range((mword)start_root, size);
  ...
  if (card_data_end >= SGEN_SHADOW_CARDTABLE_END) {
    overflow_scan_end =
        sgen_shadow_cardtable + (card_data_end - SGEN_SHADOW_CARDTABLE_END);
...
LOOP_HEAD:
  ...
  for (; card_data < card_data_end;
       card_data = sgen_find_next_card(card_data + 1, card_data_end)) {
...
void sgen_wbroots_scan_card_table(ScanCopyContext ctx) {
  ...
  SGEN_HASH_TABLE_FOREACH(&sgen_roots_hash[ROOT_TYPE_WBARRIER], void **,
                          start_root, RootRecord *, root) {
    SGEN_ASSERT(0, (root->root_desc & ROOT_DESC_TYPE_MASK) == ROOT_DESC_VECTOR,
                "Unsupported root type");

    sgen_wbroot_scan_card_table(start_root,
                                (mword)root->end_root - (mword)start_root, ctx);
  }
  SGEN_HASH_TABLE_FOREACH_END;
}
...
void *sgen_thread_attach(SgenThreadInfo *info) {
  info->tlab_start = info->tlab_next = info->tlab_temp_end =
...
/*
 * Note: the write barriers first do the needed GC work and then do the actual
 * store: this way the value is visible to the conservative GC scan after the
 * write barrier itself. If a GC interrupts the barrier in the middle, value
 * will be kept alive by the conservative scan, otherwise by the remembered set
 * scan.
 */
 ...
 */
void mono_gc_wbarrier_arrayref_copy_internal(gpointer dest_ptr,
                                             gconstpointer src_ptr, int count) {
  HEAVY_STAT(++stat_wbarrier_arrayref_copy);
  /*This check can be done without taking a lock since dest_ptr array is
   * pinned*/
 ...
 */
void mono_gc_wbarrier_generic_nostore_internal(gpointer ptr) {
  gpointer obj;
 ...
 */
void mono_gc_wbarrier_generic_store_internal(void volatile *ptr,
                                             GCObject *value) {
  SGEN_LOG(8, "Wbarrier store at %p to %p (%s)", ptr, value,
           value ? sgen_client_vtable_get_name(SGEN_LOAD_VTABLE(value))
                 : "null");
 ...
 */
void mono_gc_wbarrier_generic_store_atomic_internal(gpointer ptr,
                                                    GCObject *value) {
  HEAVY_STAT(++stat_wbarrier_generic_store_atomic);

  SGEN_LOG(8, "Wbarrier atomic store at %p to %p (%s)", ptr, value,
           value ? sgen_client_vtable_get_name(SGEN_LOAD_VTABLE(value))
                 : "null");
...
int sgen_gc_collection_count(int generation) {
  return mono_atomic_load_i32(generation == GENERATION_NURSERY
                                  ? &mono_gc_stats.minor_gc_count
                                  : &mono_gc_stats.major_gc_count);
}
...
void sgen_env_var_error(const char *env_var, const char *fallback,
...
static gboolean parse_double_in_interval(const char *env_var,
                                         const char *opt_name, const char *opt,
                                         double min, double max,
                                         double *result) {
  char *endptr;
  double val = strtod(opt, &endptr);
  if (endptr == opt) {
    sgen_env_var_error(env_var, "Using default value.",
                       "`%s` must be a number.", opt_name);
    return FALSE;
  } else if (val < min || val > max) {
    sgen_env_var_error(env_var, "Using default value.",
                       "`%s` must be between %.2f - %.2f.", opt_name, min, max);
    return FALSE;
  }
  *result = val;
  return TRUE;
...
static SgenMinor parse_sgen_minor(const char *opt) {
  if (!opt)
  ...
  if (!strcmp(opt, "simple")) {
    ...
    sgen_env_var_error(MONO_GC_PARAMS_NAME, "Using default instead.",
                       "Unknown minor collector `%s'.", opt);
    return SGEN_MINOR_DEFAULT;
  }
}

static SgenMajor parse_sgen_major(const char *opt) {
  if (!opt)
  ...
  if (!strcmp(opt, "marksweep")) {
    ...
    sgen_env_var_error(MONO_GC_PARAMS_NAME, "Using default instead.",
                       "Unknown major collector `%s'.", opt);
    return SGEN_MAJOR_DEFAULT;
  }
}

static SgenMode parse_sgen_mode(const char *opt) {
  if (!opt)
  ...
  if (!strcmp(opt, "balanced")) {
    ...
    sgen_env_var_error(MONO_GC_PARAMS_NAME, "Using default configurations.",
                       "Unknown mode `%s'.", opt);
    return SGEN_MODE_NONE;
  }
}

static void init_sgen_minor(SgenMinor minor) {
  switch (minor) {
...
static void init_sgen_major(SgenMajor major) {
 ...
 * are parsed and processed after major/minor initialization, so it can
 * potentially override some knobs set by the sgen mode. We can consider locking
 * out additional configurations when gc_modes are used.
 */
static void init_sgen_mode(SgenMode mode) {
...
void sgen_gc_init(void) {
  ...
  if ((env = g_getenv(MONO_GC_PARAMS_NAME)) || gc_params_options) {
    params_opts = g_strdup_printf(
        "%s,%s", gc_params_options ? gc_params_options : "", env ? env : "");
    g_free(env);
  }
  ...
  sgen_register_fixed_internal_mem_type(INTERNAL_MEM_SECTION,
                                        SGEN_SIZEOF_GC_MEM_SECTION);
  sgen_register_fixed_internal_mem_type(INTERNAL_MEM_GRAY_QUEUE,
  ...
  if (sgen_mode != SGEN_MODE_NONE) {
    if (sgen_minor != SGEN_MINOR_DEFAULT || sgen_major != SGEN_MAJOR_DEFAULT)
      sgen_env_var_error(
          MONO_GC_PARAMS_NAME, "Ignoring major/minor configuration",
          "Major/minor configurations cannot be used with sgen modes");
  ...
  if (opts) {
    ...
    for (ptr = opts; *ptr; ++ptr) {
      ...
      if (g_str_has_prefix(opt, "mode=")) {
        if (g_str_has_prefix(opt, "mode=pause:")) {
          ...
          if (pause)
            ...
            sgen_env_var_error(
                MONO_GC_PARAMS_NAME, "Using default",
                "Invalid maximum pause time for `pause` sgen mode");
        }
        continue;
      }
      if (g_str_has_prefix(opt, "max-heap-size=")) {
        ...
        opt = strchr(opt, '=') + 1;
        if (*opt && mono_gc_parse_environment_string_extract_number(
                        opt, &max_heap_candidate)) {
          max_heap =
              (max_heap_candidate + page_size - 1) & ~(size_t)(page_size - 1);
          if (max_heap != max_heap_candidate)
            sgen_env_var_error(MONO_GC_PARAMS_NAME, "Rounding up.",
                               "`max-heap-size` size must be a multiple of %d.",
                               page_size);
        } else {
          sgen_env_var_error(MONO_GC_PARAMS_NAME, NULL,
      ...
      if (g_str_has_prefix(opt, "soft-heap-limit=")) {
        opt = strchr(opt, '=') + 1;
        if (*opt &&
            mono_gc_parse_environment_string_extract_number(opt, &soft_limit)) {
          if (soft_limit <= 0) {
            sgen_env_var_error(MONO_GC_PARAMS_NAME, NULL,
      ...
      if (g_str_has_prefix(opt, "nursery-size=")) {
        ...
        opt = strchr(opt, '=') + 1;
        if (*opt &&
            mono_gc_parse_environment_string_extract_number(opt, &val)) {
          if ((val & (val - 1))) {
          ...
          if (val > SGEN_MAX_NURSERY_SIZE) {
            sgen_env_var_error(MONO_GC_PARAMS_NAME, "Using default value.",
                               "`nursery-size` must be smaller than %" PRId64
                               " bytes.",
      ...
      if (g_str_has_prefix(opt, "save-target-ratio=")) {
        double val;
        opt = strchr(opt, '=') + 1;
        if (parse_double_in_interval(MONO_GC_PARAMS_NAME, "save-target-ratio",
                                     opt, SGEN_MIN_SAVE_TARGET_RATIO,
                                     SGEN_MAX_SAVE_TARGET_RATIO, &val)) {
          save_target = val;
        }
        continue;
      }
      if (g_str_has_prefix(opt, "default-allowance-ratio=")) {
        double val;
        opt = strchr(opt, '=') + 1;
        if (parse_double_in_interval(
                MONO_GC_PARAMS_NAME, "default-allowance-ratio", opt,
                SGEN_MIN_ALLOWANCE_NURSERY_SIZE_RATIO,
                SGEN_MAX_ALLOWANCE_NURSERY_SIZE_RATIO, &val)) {
          allowance_ratio = val;
        }
        continue;
      }
      ...
      if (!strcmp(opt, "dynamic-nursery")) {
        if (sgen_minor_collector.is_split)
          sgen_env_var_error(
              MONO_GC_PARAMS_NAME, "Using default value.",
      ...
      if (sgen_major_collector.handle_gc_param &&
          sgen_major_collector.handle_gc_param(opt))
        continue;

      if (sgen_minor_collector.handle_gc_param &&
          sgen_minor_collector.handle_gc_param(opt))
        continue;

      if (sgen_client_handle_gc_param(opt))
        continue;

      sgen_env_var_error(MONO_GC_PARAMS_NAME, "Ignoring.",
      ...
      fprintf(stderr,
              "\n%s must be a comma-delimited list of one or more of the "
              "following:\n",
              MONO_GC_PARAMS_NAME);
      fprintf(stderr, "  max-heap-size=N (where N is an integer, possibly with "
                      "a k, m or a g suffix)\n");
      fprintf(stderr, "  soft-heap-limit=n (where N is an integer, possibly "
                      "with a k, m or a g suffix)\n");
      fprintf(stderr, "  mode=MODE (where MODE is 'balanced', 'throughput' or "
                      "'pause[:N]' and N is maximum pause in milliseconds)\n");
      fprintf(stderr, "  nursery-size=N (where N is an integer, possibly with "
                      "a k, m or a g suffix)\n");
      fprintf(stderr, "  major=COLLECTOR (where COLLECTOR is `marksweep', "
                      "`marksweep-conc', `marksweep-par')\n");
      fprintf(stderr,
      ...
      fprintf(stderr,
              "  default-allowance-ratio=R (where R must be between %.2f - "
              "%.2f).\n",
              SGEN_MIN_ALLOWANCE_NURSERY_SIZE_RATIO,
  ...
  if ((env = g_getenv(MONO_GC_DEBUG_NAME)) || gc_debug_options) {
    debug_opts = g_strdup_printf(
        "%s,%s", gc_debug_options ? gc_debug_options : "", env ? env : "");
    g_free(env);
  }
    ...
    for (ptr = opts; ptr && *ptr; ptr++) {
      ...
      if (opt[0] >= '0' && opt[0] <= '9') {
        ...
        if (*opt && mono_gc_parse_environment_string_extract_number(
          ...
          sgen_env_var_error(MONO_GC_DEBUG_NAME, NULL,
        ...
        if (!sgen_major_collector.is_concurrent) {
          sgen_env_var_error(MONO_GC_DEBUG_NAME, "Ignoring.",
                             "`mod-union-consistency-check` only works with "
                             "concurrent major collector.");
          continue;
        }
        ...
        if (!sgen_major_collector.is_concurrent) {
          sgen_env_var_error(MONO_GC_DEBUG_NAME, "Ignoring.",
                             "`check-concurrent` only works with concurrent "
                             "major collectors.");
          continue;
        }
        sgen_nursery_clear_policy = CLEAR_AT_GC;
        ...
        size_t limit = 0;
        if (colon) {
          if (!mono_gc_parse_environment_string_extract_number(colon + 1,
                                                               &limit)) {
            sgen_env_var_error(
                MONO_GC_DEBUG_NAME, "Ignoring limit.",
        ...
        /* If aot code is used, allocation from there won't expect the layout
         * with canaries enabled */
  ...
  sgen_register_root(
      NULL, 0, sgen_make_user_root_descriptor(sgen_mark_normal_gc_handles),
      ROOT_TYPE_NORMAL, MONO_ROOT_SOURCE_GC_HANDLE, NULL,
...
gboolean sgen_gc_initialized() { return gc_initialized > 0; }

NurseryClearPolicy sgen_get_nursery_clear_policy(void) {
  return sgen_nursery_clear_policy;
}

void sgen_gc_lock(void) { mono_coop_mutex_lock(&sgen_gc_mutex); }

void sgen_gc_unlock(void) { mono_coop_mutex_unlock(&sgen_gc_mutex); }

void sgen_major_collector_iterate_live_block_ranges(
    sgen_cardtable_block_callback callback) {
  sgen_major_collector.iterate_live_block_ranges(callback);
}

void sgen_major_collector_iterate_block_ranges(
    sgen_cardtable_block_callback callback) {
...
SgenRememberedSet *sgen_get_remset(void) { return &remset; }

static void count_cards(long long *major_total, long long *major_marked,
                        long long *los_total, long long *los_marked) {
...
void sgen_stop_world(int generation, gboolean serial_collection) {
  long long major_total = -1, major_marked = -1, los_total = -1,
            los_marked = -1;

  SGEN_ASSERT(0, !world_is_stopped, "Why are we stopping a stopped world?");

  sgen_binary_protocol_world_stopping(
      generation, sgen_timestamp(),
  ...
  sgen_binary_protocol_world_stopped(generation, sgen_timestamp(), major_total,
                                     major_marked, los_total, los_marked);
}
...
void sgen_restart_world(int generation, gboolean serial_collection) {
  ...
  if (sgen_binary_protocol_is_heavy_enabled())
    count_cards(&major_total, &major_marked, &los_total, &los_marked);
  sgen_binary_protocol_world_restarting(generation, sgen_timestamp(),
                                        major_total, major_marked, los_total,
                                        los_marked);
...
gboolean sgen_is_world_stopped(void) { return world_is_stopped; }
...
void sgen_check_canary_for_object(gpointer addr) {
  if (sgen_nursery_canaries_enabled()) {
    ...
    if (!CANARY_VALID(canary_ptr)) {
      ...
      fprintf(stderr, "\nCANARY ERROR - Type:%s Size:%d Address:%p Data:\n",
              sgen_client_vtable_get_name(SGEN_LOAD_VTABLE((addr))), size,
              (char *)addr);
