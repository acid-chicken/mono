...
static char *filename_for_index(int index) {
  char *filename;

  SGEN_ASSERT(0, file_size_limit > 0,
              "Indexed binary protocol filename must only be used with file "
              "size limit");

  filename = (char *)sgen_alloc_internal_dynamic(
      strlen(filename_or_prefix) + 32, INTERNAL_MEM_BINARY_PROTOCOL, TRUE);
...
static void free_filename(char *filename) {
  ...
  sgen_free_internal_dynamic(filename, strlen(filename_or_prefix) + 32,
                             INTERNAL_MEM_BINARY_PROTOCOL);
}

static void binary_protocol_open_file(gboolean assert_on_failure) {
  char *filename;
#ifdef F_SETLK
  struct flock lock;
  lock.l_type = F_WRLCK;
...
#if defined(HOST_WIN32)
  binary_protocol_file =
      CreateFileA(filename, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
...
void sgen_binary_protocol_init(const char *filename, long long limit) {
  ...
  sgen_binary_protocol_header(PROTOCOL_HEADER_CHECK, PROTOCOL_HEADER_VERSION,
                              SIZEOF_VOID_P, G_BYTE_ORDER == G_LITTLE_ENDIAN);
}
...
static void close_binary_protocol_file(void) {
#if defined(HOST_WIN32)
...
static gboolean try_lock_exclusive(void) {
  do {
...
static void unlock_exclusive(void) {
  mono_memory_barrier();
  SGEN_ASSERT(0, binary_protocol_use_count == -1,
...
static void lock_recursive(void) {
  ...
  } while (mono_atomic_cas_i32(&binary_protocol_use_count, old_count + 1,
                               old_count) != old_count);
  mono_memory_barrier();
}

static void unlock_recursive(void) {
  int old_count;
  mono_memory_barrier();
  do {
    old_count = binary_protocol_use_count;
...
static void binary_protocol_flush_buffer(BinaryProtocolBuffer *buffer) {
...
#if defined(HOST_WIN32)
    DWORD tmp_written;
    if (WriteFile(binary_protocol_file, buffer->buffer + written,
                  to_write - written, &tmp_written, NULL))
      written += tmp_written;
#elif defined(HAVE_UNISTD_H)
    ssize_t ret = write(binary_protocol_file, buffer->buffer + written,
                        to_write - written);
    if (ret >= 0)
      written += ret;
    else if (errno == EINTR)
...
static void binary_protocol_check_file_overflow(void) {
...
/*
 * Flushing buffers takes an exclusive lock, so it must only be done when the
 * world is stopped, otherwise we might end up with a deadlock because a stopped
...
gboolean sgen_binary_protocol_flush_buffers(gboolean force) {
  ...
  for (buf = header; buf != NULL; buf = buf->next)
    ++num_buffers;
  bufs = (BinaryProtocolBuffer **)sgen_alloc_internal_dynamic(
      num_buffers * sizeof(BinaryProtocolBuffer *),
      INTERNAL_MEM_BINARY_PROTOCOL, TRUE);
  for (buf = header, i = 0; buf != NULL; buf = buf->next, i++)
    bufs[i] = buf;
  SGEN_ASSERT(0, i == num_buffers, "Binary protocol buffer count error");

  /*
   * This might be incorrect when forcing, but all bets are off in that case,
   * anyway, because we're trying to figure out a bug in the debugger.
   */
  ...
  sgen_free_internal_dynamic(buf, num_buffers * sizeof(BinaryProtocolBuffer *),
                             INTERNAL_MEM_BINARY_PROTOCOL);
...
static BinaryProtocolBuffer *binary_protocol_get_buffer(int length) {
...
retry:
  ...
  new_buffer = (BinaryProtocolBuffer *)sgen_alloc_os_memory(
      sizeof(BinaryProtocolBuffer),
      (SgenAllocFlags)(SGEN_ALLOC_INTERNAL | SGEN_ALLOC_ACTIVATE),
      "debugging memory", MONO_MEM_ACCOUNT_SGEN_BINARY_PROTOCOL);
  new_buffer->next = buffer;
  new_buffer->index = 0;

  if (mono_atomic_cas_ptr((void **)&binary_protocol_buffers, new_buffer,
                          buffer) != buffer) {
...
static void protocol_entry(unsigned char type, gpointer data, int size) {
  int index;
  gboolean include_worker_index = type != PROTOCOL_ID(binary_protocol_header);
  int entry_size =
      size + 1 + (include_worker_index ? 1 : 0); // type + worker_index + size
