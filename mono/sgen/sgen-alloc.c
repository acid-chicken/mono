...
/* The total number of bytes allocated so far in program execution by all
 * attached threads. This is not constantly syncrhonized, but only updated on
 * each GC. */
...
/*
 * Allocation is done from a Thread Local Allocation Buffer (TLAB). TLABs are
 * allocated from nursery fragments. tlab_next is the pointer to the space
 * inside the TLAB where the next object will be allocated. tlab_temp_end is the
 * pointer to the end of the temporary space reserved for the allocation: it
 * allows us to set the scan starts at reasonable intervals. tlab_real_end
 * points to the end of the TLAB.
 */
...
static void increment_thread_allocation_counter(size_t byte_size) {
  mono_thread_info_current()->total_bytes_allocated += byte_size;
}

static GCObject *alloc_degraded(GCVTable vtable, size_t size,
                                gboolean for_mature) {
  GCObject *p;
  ...
  if (!for_mature) {
    ...
    if (sgen_need_major_collection(size, &forced))
      sgen_perform_collection(size, GENERATION_OLD, "mature allocation failure",
                              !for_mature || forced, TRUE);
  }

  p = sgen_major_collector.alloc_degraded(vtable, size);

  if (!for_mature)
    sgen_binary_protocol_alloc_degraded(p, vtable, size,
                                        sgen_client_get_provenance());

  return p;
}

static void zero_tlab_if_necessary(void *p, size_t size) {
  if (sgen_nursery_clear_policy == CLEAR_AT_TLAB_CREATION ||
 ...
 * Note: every SGEN_SCAN_START_SIZE or so we are given the chance to do some
 * special processing. We can keep track of where objects start, for example, so
 * when we scan the thread stacks for pinned objects, we can start a search for
 * the pinned object in SGEN_SCAN_START_SIZE chunks.
 */
GCObject *sgen_alloc_obj_nolock(GCVTable vtable, size_t size) {
  ...
  SGEN_ASSERT(6, sgen_vtable_get_descriptor(vtable),
  ...
  if (G_UNLIKELY(sgen_has_per_allocation_action)) {
    ...
    if (sgen_collect_before_allocs) {
      if (((current_alloc % sgen_collect_before_allocs) == 0) &&
          sgen_nursery_section) {
        sgen_perform_collection(0, GENERATION_NURSERY,
                                "collect-before-alloc-triggered", TRUE, TRUE);
        if (!sgen_degraded_mode && sgen_can_alloc_size(size) &&
            real_size <= SGEN_MAX_SMALL_OBJ_SIZE) {
          // FIXME:
  ...
  if (real_size > SGEN_MAX_SMALL_OBJ_SIZE) {
    ...
    /* tlab_next and tlab_temp_end are TLS vars so accessing them might be
     * expensive */
    ...
    if (G_LIKELY(new_next < TLAB_TEMP_END)) {
      ...
      SGEN_LOG(
          6, "Allocated object %p, vtable: %p (%s), size: %" G_GSIZE_FORMAT "d",
          p, vtable, sgen_client_vtable_get_name(vtable), size);
    ...
    /* there are two cases: the object is too big or we run out of space in the
     * TLAB */
    /* we also reach here when the thread does its first allocation after a
     * minor collection, since the tlab_ variables are initialized to NULL.
     * there can be another case (from ORP), if we cooperate with the runtime a
     * bit: objects that need finalizers can have the high bit set in their size
     * so the above check fails and we can readily add the object to the queue.
     * This avoids taking again the GC lock when registering, but this is moot
     * when doing thread-local allocation, so it may not be a good idea.
     */
    if (TLAB_NEXT >= TLAB_REAL_END) {
      ...
      available_in_tlab =
          (int)(TLAB_REAL_END - TLAB_NEXT); // We'll never have tlabs > 2Gb
      if (size > sgen_tlab_size || available_in_tlab > SGEN_MAX_NURSERY_WASTE) {
        ...
        size_t alloc_size = 0;
        if (TLAB_START)
          SGEN_LOG(3, "Retire TLAB: %p-%p [%ld]", TLAB_START, TLAB_REAL_END,
                   (long)(TLAB_REAL_END - TLAB_NEXT - size));
        sgen_nursery_retire_region(p, available_in_tlab);

        p = (void **)sgen_nursery_alloc_range(sgen_tlab_size, size,
                                              &alloc_size);
        if (!p) {
  ...
  if (G_LIKELY(p)) {
    SGEN_LOG(6,
             "Allocated object %p, vtable: %p (%s), size: %" G_GSIZE_FORMAT "d",
             p, vtable, sgen_client_vtable_get_name(vtable), size);
    sgen_binary_protocol_alloc(p, vtable, size, sgen_client_get_provenance());
    mono_atomic_store_seq(p, vtable);
  }
...
GCObject *sgen_try_alloc_obj_nolock(GCVTable vtable, size_t size) {
  ...
  size = ALIGN_UP(size);
  SGEN_ASSERT(9, real_size >= SGEN_CLIENT_MINIMUM_OBJECT_SIZE,
              "Object too small");
  ...
  if (G_UNLIKELY(size > sgen_tlab_size)) {
    ...
    available_in_tlab =
        (int)(real_end - (char *)p); // We'll never have tlabs > 2Gb
    ...
    if (G_LIKELY(new_next < real_end)) {
      ...
      sgen_nursery_retire_region(p, available_in_tlab);
      new_next =
          (char *)sgen_nursery_alloc_range(sgen_tlab_size, size, &alloc_size);
      p = (void **)new_next;
      if (!p)
        return NULL;
...
GCObject *sgen_alloc_obj_pinned(GCVTable vtable, size_t size) {
  ...
  if (size > SGEN_MAX_SMALL_OBJ_SIZE) {
    ...
    SGEN_ASSERT(9, sgen_client_vtable_is_inited(vtable),
                "class %s:%s is not initialized",
                sgen_client_vtable_get_namespace(vtable),
                sgen_client_vtable_get_name(vtable));
    p = sgen_major_collector.alloc_small_pinned_obj(
        vtable, size, SGEN_VTABLE_HAS_REFERENCES(vtable));
  }
  if (G_LIKELY(p)) {
    SGEN_LOG(
        6,
        "Allocated pinned object %p, vtable: %p (%s), size: %" G_GSIZE_FORMAT
        "d",
        p, vtable, sgen_client_vtable_get_name(vtable), size);
    increment_thread_allocation_counter(size);
    sgen_binary_protocol_alloc_pinned(p, vtable, size,
                                      sgen_client_get_provenance());
  }
  UNLOCK_GC;
  return p;
...
/*
 * Used to allocate thread objects during attach. Doesn't trigger collections
 * since the thread is not yet attached.
 */
...
#ifdef HEAVY_STATISTICS
  mono_counters_register("# objects allocated",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_objects_alloced);
  mono_counters_register("bytes allocated",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_bytes_alloced);
  mono_counters_register("bytes allocated in LOS",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
