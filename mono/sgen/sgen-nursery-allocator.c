 ...
 * Thread local allocation is done from areas of memory Hotspot calls Thread
 * Local Allocation Buffers (TLABs).
 */
#include "config.h"
#ifdef HAVE_SGEN_GC
...
static const char *get_reason_name(AllocRecord *rec) {
  switch (rec->reason) {
  case FIXED_ALLOC:
    return "fixed-alloc";
  case RANGE_ALLOC:
...
static void reset_alloc_records(void) {
  next_record = 0;
  alloc_count = 0;
}

static void add_alloc_record(char *addr, size_t size, int reason) {
...
static int comp_alloc_record(const void *_a, const void *_b) {
...
void dump_alloc_records(void) {
  int i;
  sgen_qsort(alloc_records, next_record, sizeof(AllocRecord),
             comp_alloc_record);
  ...
  for (i = 0; i < next_record; ++i) {
    ...
    printf("obj [%p, %p] size %d reason %s seq %d tid %x\n", rec->address,
           rec_end(rec), (int)rec->size, get_reason_name(rec), rec->seq,
           (size_t)rec->tid);
  }
}
...
void verify_alloc_records(void) {
  ...
  for (i = 0; i < next_record; ++i) {
    ...
    printf("obj [%p, %p] size %d hole to prev %d reason %s seq %d tid "
           "%" G_GSIZE_FORMAT "x\n",
           rec->address, rec_end(rec), (int)rec->size, hole_size,
           get_reason_name(rec), rec->seq, (size_t)rec->tid);
    prev = rec;
  }
...
static gpointer mask(gpointer n, uintptr_t bit) {
  return (gpointer)(((uintptr_t)n) | bit);
}

static gpointer unmask(gpointer p) {
  return (gpointer)((uintptr_t)p & ~(uintptr_t)0x3);
}

static uintptr_t get_mark(gpointer n) { return (uintptr_t)n & 0x1; }
...
void sgen_fragment_allocator_add(SgenFragmentAllocator *allocator, char *start,
  ...
  fragment->next_in_order = fragment->next =
      (SgenFragment *)unmask(allocator->region_head);
...
static SgenFragment **
find_previous_pointer_fragment(SgenFragmentAllocator *allocator,
                               SgenFragment *frag) {
  SgenFragment **prev;
...
static gboolean claim_remaining_size(SgenFragment *frag, char *alloc_end) {
  /* All space used, nothing to claim. */
  if (frag->fragment_end <= alloc_end)
    return FALSE;

  /* Try to alloc all the remaining space. */
  return mono_atomic_cas_ptr((volatile gpointer *)&frag->fragment_next,
                             frag->fragment_end, alloc_end) == alloc_end;
}

static void *par_alloc_from_fragment(SgenFragmentAllocator *allocator,
                                     SgenFragment *frag, size_t size) {
  char *p = frag->fragment_next;
  char *end = p + size;

  if (end > frag->fragment_end ||
      end > (sgen_nursery_start + sgen_nursery_size))
    return NULL;

  /* p = frag->fragment_next must happen before */
  mono_memory_barrier();

  if (mono_atomic_cas_ptr((volatile gpointer *)&frag->fragment_next, end, p) !=
      p)
    return NULL;
  ...
  if (frag->fragment_end - end < SGEN_MAX_NURSERY_WASTE) {
     ...
     * allocating from this dying fragment as it doesn't respect
     * SGEN_MAX_NURSERY_WASTE when doing second chance allocation.
     */
    if ((sgen_get_nursery_clear_policy() == CLEAR_AT_TLAB_CREATION ||
         sgen_get_nursery_clear_policy() == CLEAR_AT_TLAB_CREATION_DEBUG) &&
        claim_remaining_size(frag, end)) {
    ...
    while (prev_ptr) {
      ...
      if (!get_mark(next)) {
        ...
        if (mono_atomic_cas_ptr((volatile gpointer *)&frag->next, mask(next, 1),
                                next) != next) {
          continue;
        }
      }
      ...
      if (mono_atomic_cas_ptr((volatile gpointer *)prev_ptr, unmask(next),
                              frag) != frag) {
        prev_ptr = find_previous_pointer_fragment(allocator, frag);
        continue;
      }
...
static void *serial_alloc_from_fragment(SgenFragment **previous,
                                        SgenFragment *frag, size_t size) {
  char *p = frag->fragment_next;
  char *end = p + size;
  ...
  if (frag->fragment_end - end < SGEN_MAX_NURSERY_WASTE) {
    *previous = frag->next;
    ...
    /* Clear the remaining space, pinning depends on this. FIXME move this to
     * use phony arrays */
...
restart:
  for (frag = (SgenFragment *)unmask(allocator->alloc_head); unmask(frag);
       frag = (SgenFragment *)unmask(frag->next)) {
...
restart:
  ...
  for (frag = (SgenFragment *)unmask(allocator->alloc_head); frag;
       frag = (SgenFragment *)unmask(frag->next)) {
    size_t frag_size = frag->fragment_end - frag->fragment_next;
  ...
  /* The second fragment_next read should be ordered in respect to the first
   * code block */
  mono_memory_barrier();
  ...
  if (min_frag) {
    ...
    *out_alloc_size = frag_size;
    ...
    /*XXX restarting here is quite dubious given this is already second chance
     * allocation. */
    if (!p) {
      HEAVY_STAT(++stat_alloc_retries);
      goto restart;
...
void sgen_clear_allocator_fragments(SgenFragmentAllocator *allocator) {
  ...
  for (frag = (SgenFragment *)unmask(allocator->alloc_head); frag;
       frag = (SgenFragment *)unmask(frag->next)) {
    SGEN_LOG(4, "Clear nursery frag %p-%p", frag->fragment_next,
             frag->fragment_end);
    sgen_clear_range(frag->fragment_next, frag->fragment_end);
#ifdef NALLOC_DEBUG
    add_alloc_record(frag->fragment_next,
                     frag->fragment_end - frag->fragment_next,
                     CLEAR_NURSERY_FRAGS);
#endif
  }
}
...
void sgen_clear_range(char *start, char *end) {
  ...
  if (sgen_client_array_fill_range(start, size)) {
    ...
    SGEN_ASSERT(0, start + sgen_safe_object_get_size((GCObject *)start) == end,
 ...
 * allocation.
 */
static void add_nursery_frag(SgenFragmentAllocator *allocator, size_t frag_size,
                             char *frag_start, char *frag_end) {
  SGEN_LOG(4, "Found empty fragment: %p-%p, size: %" G_GSIZE_FORMAT "d",
           frag_start, frag_end, frag_size);
  sgen_binary_protocol_empty(frag_start, frag_size);
  /* Not worth dealing with smaller fragments: need to tune */
  if (frag_size >= SGEN_MAX_NURSERY_WASTE) {
    /* memsetting just the first chunk start is bound to provide better cache
     * locality */
...
static void fragment_list_reverse(SgenFragmentAllocator *allocator) {
 ...
 */
static void add_nursery_frag_checks(SgenFragmentAllocator *allocator,
                                    char *frag_start, char *frag_end) {
  char *nursery_limit = sgen_nursery_start + sgen_nursery_size;

  if (frag_start < nursery_limit && frag_end > nursery_limit) {
    add_nursery_frag(allocator, nursery_limit - frag_start, frag_start,
                     nursery_limit);
    add_nursery_frag(allocator, frag_end - nursery_limit, nursery_limit,
                     frag_end);
  } else {
...
mword sgen_build_nursery_fragments(GCMemSection *nursery_section) {
  ...
  /* The current nursery might give us a fragment list to exclude [start,
   * next[*/
  frags_ranges = sgen_minor_collector.build_fragments_get_exclude_head();

  /* clear scan starts */
  memset(nursery_section->scan_starts, 0,
         nursery_section->num_scan_start * sizeof(gpointer));

  pin_start = pin_entry =
      sgen_pinning_get_entry(nursery_section->pin_queue_first_entry);
  ...
  /* First we reorder the fragment list to be in ascending address order. This
   * makes H/W prefetchers happier. */
  fragment_list_reverse(&mutator_allocator);

  /*The collector might want to do something with the final nursery fragment
   * list.*/
  ...
  if (!unmask(mutator_allocator.alloc_head)) {
    ...
    for (pin_entry = pin_start; pin_entry < pin_end; ++pin_entry) {
      ...
      SGEN_LOG(3, "Bastard pinning obj %p (%s), size: %ld", p,
...
gboolean sgen_can_alloc_size(size_t size) {
  ...
  for (frag = (SgenFragment *)unmask(mutator_allocator.alloc_head); frag;
       frag = (SgenFragment *)unmask(frag->next)) {
...
void *sgen_nursery_alloc(size_t size) {
  SGEN_ASSERT(1,
              size >= (SGEN_CLIENT_MINIMUM_OBJECT_SIZE + CANARY_SIZE) &&
                  size <= (SGEN_MAX_SMALL_OBJ_SIZE + CANARY_SIZE),
...
void *sgen_nursery_alloc_range(size_t desired_size, size_t minimum_size,
                               size_t *out_alloc_size) {
  SGEN_LOG(4,
           "Searching for byte range desired size: %" G_GSIZE_FORMAT
           "d minimum size %" G_GSIZE_FORMAT "d",
           desired_size, minimum_size);

  HEAVY_STAT(++stat_nursery_alloc_range_requests);

  return sgen_fragment_allocator_par_range_alloc(
      &mutator_allocator, desired_size, minimum_size, out_alloc_size);
}
...
void sgen_nursery_allocator_init_heavy_stats(void) {
  mono_counters_register("bytes wasted trailer fragments",
                         MONO_COUNTER_GC | MONO_COUNTER_WORD |
                             MONO_COUNTER_BYTES,
                         &stat_wasted_bytes_trailer);
  mono_counters_register("bytes wasted small areas",
                         MONO_COUNTER_GC | MONO_COUNTER_WORD |
                             MONO_COUNTER_BYTES,
                         &stat_wasted_bytes_small_areas);
  mono_counters_register("bytes wasted discarded fragments",
                         MONO_COUNTER_GC | MONO_COUNTER_WORD |
                             MONO_COUNTER_BYTES,
                         &stat_wasted_bytes_discarded_fragments);

  mono_counters_register("# nursery alloc requests",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_nursery_alloc_requests);
  mono_counters_register("# nursery alloc iterations",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_alloc_iterations);
  mono_counters_register("# nursery alloc retries",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_alloc_retries);

  mono_counters_register("# nursery alloc range requests",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_nursery_alloc_range_requests);
  mono_counters_register("# nursery alloc range iterations",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_alloc_range_iterations);
  mono_counters_register("# nursery alloc range restries",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
...
void sgen_init_nursery_allocator(void) {
  sgen_register_fixed_internal_mem_type(INTERNAL_MEM_FRAGMENT,
                                        sizeof(SgenFragment));
#ifdef NALLOC_DEBUG
  alloc_records = sgen_alloc_os_memory(
      sizeof(AllocRecord) * ALLOC_RECORD_COUNT,
      SGEN_ALLOC_INTERNAL | SGEN_ALLOC_ACTIVATE, "debugging memory");
#endif
}

void sgen_nursery_alloc_prepare_for_minor(void) {
  sgen_minor_collector.prepare_to_space(sgen_space_bitmap,
...
void sgen_nursery_allocator_set_nursery_bounds(char *start, size_t min_size,
                                               size_t max_size) {
  ...
  /*
   * This will not divide evenly for tiny nurseries (<4kb), so we make sure to
   * be on the right side of things and round up.  We could just do a MIN(1,x)
   * instead, since the nursery size must be a power of 2.
   */
  sgen_space_bitmap_size = (sgen_nursery_end - sgen_nursery_start +
                            SGEN_TO_SPACE_GRANULE_IN_BYTES * 8 - 1) /
                           (SGEN_TO_SPACE_GRANULE_IN_BYTES * 8);
  sgen_space_bitmap = (char *)g_malloc0(sgen_space_bitmap_size);

  /* Setup the single first large fragment */
  sgen_minor_collector.init_nursery(&mutator_allocator, sgen_nursery_start,
                                    sgen_nursery_end);
}
...
void sgen_resize_nursery(gboolean need_shrink) {
  ...
  major_size = sgen_major_collector.get_num_major_sections() *
                   sgen_major_collector.section_size +
   ...
   * section).
   */
  if ((sgen_nursery_size * 2) <
          (major_size / SGEN_DEFAULT_ALLOWANCE_NURSERY_SIZE_RATIO) &&
      (sgen_nursery_size * 2) <= sgen_nursery_max_size && !need_shrink) {
    if ((sgen_nursery_section->end_data - sgen_nursery_section->data) ==
        sgen_nursery_size)
      sgen_nursery_section->end_data += sgen_nursery_size;
    sgen_nursery_size *= 2;
  } else if ((sgen_nursery_size >
                  (major_size / SGEN_DEFAULT_ALLOWANCE_NURSERY_SIZE_RATIO) ||
              need_shrink) &&
