...
#define LOS_SECTION_OBJECT_LIMIT                                               \
  (LOS_SECTION_SIZE - LOS_CHUNK_SIZE - sizeof(LOSObject))
//#define LOS_SECTION_OBJECT_LIMIT	0
#define LOS_SECTION_NUM_CHUNKS ((LOS_SECTION_SIZE >> LOS_CHUNK_BITS) - 1)

#define LOS_SECTION_FOR_OBJ(obj)                                               \
  ((LOSSection *)((mword)(obj) & ~(mword)(LOS_SECTION_SIZE - 1)))
#define LOS_CHUNK_INDEX(obj, section)                                          \
  (((char *)(obj) - (char *)(section)) >> LOS_CHUNK_BITS)
...
static SgenArrayList sgen_los_object_array_list = SGEN_ARRAY_LIST_INIT(
    NULL, sgen_array_list_default_is_slot_set, NULL, INTERNAL_MEM_PIN_QUEUE);
...
#define LOS_OBJECT_TAG(bl)                                                     \
  (SGEN_OBJECT_HAS_REFERENCES((bl)->data)                                      \
       ? LOS_OBJECT_TAG_HAS_REFERENCES((bl))                                   \
       : (bl))
...
#define END_FOREACH_LOS_OBJECT_NO_LOCK                                         \
  }                                                                            \
...
#define END_FOREACH_LOS_OBJECT_HAS_REFERENCES_NO_LOCK                          \
  }                                                                            \
  SGEN_ARRAY_LIST_END_FOREACH_SLOT;                                            \
  }
...
#define FOREACH_LOS_OBJECT_RANGE_NO_LOCK(bl, begin, end, index)                \
  {                                                                            \
    volatile gpointer *slot;                                                   \
    SGEN_ARRAY_LIST_FOREACH_SLOT_RANGE(&sgen_los_object_array_list, begin,     \
                                       end, slot, index) {                     \
      (bl) = LOS_OBJECT_UNTAG(*slot);                                          \
      if (!(bl))                                                               \
        continue;

#define END_FOREACH_LOS_OBJECT_RANGE_NO_LOCK                                   \
  }                                                                            \
  SGEN_ARRAY_LIST_END_FOREACH_SLOT_RANGE;                                      \
  }

#define FOREACH_LOS_OBJECT_RANGE_HAS_REFERENCES_NO_LOCK(bl, begin, end, index, \
                                                        hr)                    \
  {                                                                            \
    volatile gpointer *slot;                                                   \
    SGEN_ARRAY_LIST_FOREACH_SLOT_RANGE(&sgen_los_object_array_list, begin,     \
...
#define END_FOREACH_LOS_OBJECT_RANGE_HAS_REFERENCES_NO_LOCK                    \
  }                                                                            \
  SGEN_ARRAY_LIST_END_FOREACH_SLOT_RANGE;                                      \
  }
...
static LOSFreeChunks
    *los_fast_free_lists[LOS_NUM_FAST_SIZES]; /* 0 is for larger sizes */
...
mword sgen_los_object_size(LOSObject *obj) { return obj->size & ~1L; }

#ifdef LOS_CONSISTENCY_CHECK
static void los_consistency_check(void) {
  LOSSection *section;
  LOSObject *obj;
  int i;
  ...
  FOREACH_LOS_OBJECT_NO_LOCK(obj) {
    ...
    num_chunks =
        (obj_size + sizeof(LOSObject) + LOS_CHUNK_SIZE - 1) >> LOS_CHUNK_BITS;
  ...
  for (i = 0; i < LOS_NUM_FAST_SIZES; ++i) {
    ...
    for (size_chunks = los_fast_free_lists[i]; size_chunks;
         size_chunks = size_chunks->next_size) {
...
static void add_free_chunk(LOSFreeChunks *free_chunks, size_t size) {
...
static LOSFreeChunks *get_from_size_list(LOSFreeChunks **list, size_t size) {
  ...
  *list = free_chunks->next_size;
  ...
  if (free_chunks->size > size)
    add_free_chunk((LOSFreeChunks *)((char *)free_chunks + size),
                   free_chunks->size - size);
...
static LOSObject *randomize_los_object_start(gpointer addr, size_t obj_size,
                                             size_t alloced_size,
  ...
  if (alloced_size != obj_size) {
     ...
     * power by 2 factors.
     */
    offset = SGEN_ALIGN_DOWN((((size_t)addr - 1) * 2654435761u) %
                             (alloced_size - obj_size));
  }
  SGEN_ASSERT(0, (alloced_size - obj_size) < addr_alignment,
              "Why are we wasting one entire chunk for a los object ?");
  /* Randomize the location within the reserved chunks to improve cache
   * performance */
  return (LOSObject *)((guint8 *)addr + offset);
}

static LOSObject *get_los_section_memory(size_t size) {
  LOSSection *section;
...
retry:
  ...
  if (free_chunks) {
    return randomize_los_object_start(free_chunks, obj_size, size,
                                      LOS_CHUNK_SIZE);
  }
  ...
  section = (LOSSection *)sgen_alloc_os_memory_aligned(
      LOS_SECTION_SIZE, LOS_SECTION_SIZE,
      (SgenAllocFlags)(SGEN_ALLOC_HEAP | SGEN_ALLOC_ACTIVATE), NULL,
...
static void free_los_section_memory(LOSObject *obj, size_t size) {
  ...
  add_free_chunk(
      (LOSFreeChunks *)SGEN_ALIGN_DOWN_TO((mword)obj, LOS_CHUNK_SIZE), size);
}

void sgen_los_free_object(LOSObject *obj) {
  if (obj->cardtable_mod_union)
    sgen_card_table_free_mod_union(obj->cardtable_mod_union, (char *)obj->data,
                                   sgen_los_object_size(obj));

#ifndef LOS_DUMMY
  mword size = sgen_los_object_size(obj);
  SGEN_LOG(4, "Freed large object %p, size %lu", obj->data,
           (unsigned long)size);
  ...
  if (size > LOS_SECTION_OBJECT_LIMIT) {
    ...
    sgen_free_os_memory((gpointer)SGEN_ALIGN_DOWN_TO((mword)obj, pagesize),
                        size, SGEN_ALLOC_HEAP, MONO_MEM_ACCOUNT_SGEN_LOS);
...
void *sgen_los_alloc_large_inner(GCVTable vtable, size_t size) {
...
#ifdef LOS_DUMMY
  if (!los_segment)
    los_segment = sgen_alloc_os_memory(
        LOS_SEGMENT_SIZE, SGEN_ALLOC_HEAP | SGEN_ALLOC_ACTIVATE, NULL);
  ...
  if (size > LOS_SECTION_OBJECT_LIMIT) {
    ...
    if (sgen_memgov_try_alloc_space(alloc_size, SPACE_LOS)) {
      obj = (LOSObject *)sgen_alloc_os_memory(
          alloc_size, (SgenAllocFlags)(SGEN_ALLOC_HEAP | SGEN_ALLOC_ACTIVATE),
          NULL, MONO_MEM_ACCOUNT_SGEN_LOS);
      if (obj) {
  ...
  *vtslot = vtable;
  ...
  sgen_array_list_add(&sgen_los_object_array_list, LOS_OBJECT_TAG(obj), 0,
                      FALSE);
  sgen_los_memory_usage += size;
  los_num_objects++;
  SGEN_LOG(4, "Allocated large object %p, vtable: %p (%s), size: %zd",
           obj->data, vtable, sgen_client_vtable_get_name(vtable), size);
  sgen_binary_protocol_alloc(obj->data, vtable, size,
...
void sgen_los_sweep(void) {
  ...
  FOREACH_LOS_OBJECT_NO_LOCK(obj) {
    SGEN_ASSERT(0, !SGEN_OBJECT_IS_PINNED(obj->data),
                "Who pinned a LOS object?");

    if (sgen_los_object_is_pinned(obj->data)) {
      if (obj->cardtable_mod_union) {
        mword obj_size = sgen_los_object_size(obj);
        mword num_cards = sgen_card_table_number_of_cards_in_range(
            (mword)obj->data, obj_size);
        memset(obj->cardtable_mod_union, 0, num_cards);
      }

      sgen_los_unpin_object(obj->data);
      sgen_update_heap_boundaries((mword)obj->data,
                                  (mword)obj->data + sgen_los_object_size(obj));
    } else {
  ...
  while (section) {
    ...
    for (i = 0; i <= LOS_SECTION_NUM_CHUNKS; ++i) {
      if (section->free_chunk_map[i]) {
        int j;
        for (j = i + 1;
             j <= LOS_SECTION_NUM_CHUNKS && section->free_chunk_map[j]; ++j)
          ;
        add_free_chunk(
            (LOSFreeChunks *)((char *)section + (i << LOS_CHUNK_BITS)),
            (j - i) << LOS_CHUNK_BITS);
        i = j - 1;
      }
    }
  ...
  los_num_objects, sgen_los_memory_usage); for (i = 0; i < LOS_NUM_FAST_SIZES;
  ++i) { int num_chunks = 0; LOSFreeChunks *free_chunks; for (free_chunks =
  los_fast_free_lists [i]; free_chunks; free_chunks = free_chunks->next_size)
              ++num_chunks;
      g_print ("  %d: %d\n", i, num_chunks);
  }
  */
...
gboolean mono_sgen_los_describe_pointer(char *ptr) {
  ...
  FOREACH_LOS_OBJECT_NO_LOCK(obj) {
    ...
    if ((char *)obj->data > ptr ||
        (char *)obj->data + sgen_los_object_size(obj) <= ptr)
      continue;
    ...
    if ((char *)obj->data == ptr) {
      ...
      SGEN_LOG(0, "%s (interior-ptr offset %zd size %d pin %d)", los_kind,
               ptr - (char *)obj->data, (int)size, pinned ? 1 : 0);
    }
...
void sgen_los_iterate_live_block_ranges(
...
static guint8 *get_cardtable_mod_union_for_object(LOSObject *obj) {
  mword size = sgen_los_object_size(obj);
  ...
  other = (guint8 *)SGEN_CAS_PTR((gpointer *)&obj->cardtable_mod_union,
                                 mod_union, NULL);
  if (!other) {
    SGEN_ASSERT(0, obj->cardtable_mod_union == mod_union,
...
void sgen_los_scan_card_table(CardTableScanType scan_type, ScanCopyContext ctx,
  ...
  sgen_binary_protocol_los_card_table_scan_start(
  ...
  FOREACH_LOS_OBJECT_RANGE_HAS_REFERENCES_NO_LOCK(
      obj, first_object, last_object, index, has_references) {
    mword num_cards = 0;
    guint8 *cards;
    ...
    if (scan_type & CARDTABLE_SCAN_MOD_UNION) {
      ...
      if (scan_type == CARDTABLE_SCAN_MOD_UNION_PRECLEAN) {
        ...
        mword obj_size = sgen_los_object_size(obj);
        num_cards = sgen_card_table_number_of_cards_in_range((mword)obj->data,
                                                             obj_size);
        cards_preclean = (guint8 *)sgen_alloc_internal_dynamic(
            num_cards, INTERNAL_MEM_CARDTABLE_MOD_UNION, TRUE);
    ...
    sgen_cardtable_scan_object(obj->data, sgen_los_object_size(obj), cards,
                               ctx);

    if (scan_type == CARDTABLE_SCAN_MOD_UNION_PRECLEAN)
      sgen_free_internal_dynamic(cards, num_cards,
                                 INTERNAL_MEM_CARDTABLE_MOD_UNION);
  }
  END_FOREACH_LOS_OBJECT_RANGE_HAS_REFERENCES_NO_LOCK;
  sgen_binary_protocol_los_card_table_scan_end(
      sgen_timestamp(), scan_type & CARDTABLE_SCAN_MOD_UNION);
}
...
void sgen_los_count_cards(long long *num_total_cards,
  ...
  FOREACH_LOS_OBJECT_HAS_REFERENCES_NO_LOCK(obj, has_references) {
    ...
    guint8 *cards_end = sgen_card_table_get_card_scan_address(
        (mword)obj->data + sgen_los_object_size(obj) - 1);
...
void sgen_los_pin_object(GCObject *data) {
  LOSObject *obj = sgen_los_header_for_object(data);
  obj->size = obj->size | 1;
  sgen_binary_protocol_pin(data, (gpointer)SGEN_LOAD_VTABLE(data),
...
void sgen_los_pin_objects(SgenGrayQueue *gray_queue,
  ...
  FOREACH_LOS_OBJECT_NO_LOCK(obj) {
    size_t dummy;
    if (sgen_find_optimized_pin_queue_area(
            (char *)obj->data, (char *)obj->data + sgen_los_object_size(obj),
            &dummy, &dummy)) {
      sgen_binary_protocol_pin(obj->data, (gpointer)SGEN_LOAD_VTABLE(obj->data),
                               sgen_safe_object_get_size(obj->data));

      if (sgen_los_object_is_pinned(obj->data)) {
        SGEN_ASSERT(
            0, finish_concurrent_mode == TRUE,
      ...
      if (SGEN_OBJECT_HAS_REFERENCES(obj->data))
        GRAY_OBJECT_ENQUEUE_SERIAL(
            gray_queue, obj->data,
            sgen_obj_get_descriptor((GCObject *)obj->data));
...
gboolean sgen_los_pin_object_par(GCObject *data) {
  LOSObject *obj = sgen_los_header_for_object(data);
  mword old_size = obj->size;
  if (old_size & 1)
    return FALSE;
#if SIZEOF_VOID_P == 4
  old_size = mono_atomic_cas_i32((volatile gint32 *)&obj->size, old_size | 1,
                                 old_size);
#else
  old_size = mono_atomic_cas_i64((volatile gint64 *)&obj->size, old_size | 1,
                                 old_size);
#endif
  if (old_size & 1)
    return FALSE;
...
gboolean sgen_los_object_is_pinned(GCObject *data) {
  LOSObject *obj = sgen_los_header_for_object(data);
  return obj->size & 1;
}
...
void sgen_los_mark_mod_union_card(GCObject *mono_obj, void **ptr) {
  ...
  size_t offset = sgen_card_table_get_card_offset(
      (char *)ptr, (char *)sgen_card_table_align_pointer((char *)mono_obj));
