...
/* Clearing at nursery collections is the safest, but has bad interactions with
 * caches. Clearing at TLAB creation is much faster, but more complex and it
 * might expose hard to find bugs.
 */
typedef enum {
  CLEAR_AT_GC,
...
struct _GCMemSection {
  ...
  char *end_data;
  /*
   * scan starts is an array of pointers to objects equally spaced in the
   * allocation area They let use quickly find pinned objects from pinning
   * pointers.
   */
  char **scan_starts;
  /* in major collections indexes in the pin_queue for objects that pin this
   * section */
...
#define SGEN_ATOMIC_ADD_P(x, i)                                                \
  do {                                                                         \
    size_t __old_x;                                                            \
    do {                                                                       \
      __old_x = (x);                                                           \
    } while (mono_atomic_cas_ptr((void **)&(x), (void *)(__old_x + (i)),       \
                                 (void *)__old_x) != (void *)__old_x);         \
  } while (0)

#ifndef MONO_ATOMIC_USES_LOCK
#define SGEN_ATOMIC_ADD_I64(x, i)                                              \
  do {                                                                         \
...
#define SGEN_ATOMIC_ADD_I64(x, i)                                              \
  do {                                                                         \
    ...
    do {                                                                       \
      __old_x = (x);                                                           \
    } while (mono_sgen_atomic_cas_i64((volatile gint64 *)&(x), __old_x + (i),  \
                                      __old_x) != __old_x);                    \
  } while (0)
...
#define LOG_TIMESTAMP                                                          \
  do {                                                                         \
    ...
    time(&t);                                                                  \
    localtime_r(&t, &tod);                                                     \
    strftime(logTime, sizeof(logTime), MONO_STRFTIME_F " " MONO_STRFTIME_T,    \
             &tod);                                                            \
  } while (0)
#else
#define LOG_TIMESTAMP                                                          \
...
#define SGEN_LOG(level, format, ...)                                           \
  do {                                                                         \
    if (G_UNLIKELY((level) <= SGEN_MAX_DEBUG_LEVEL &&                          \
                   (level) <= sgen_gc_debug_level)) {                          \
      char logTime[80];                                                        \
      LOG_TIMESTAMP;                                                           \
...
#define SGEN_COND_LOG(level, cond, format, ...)                                \
  do {                                                                         \
    if (G_UNLIKELY((level) <= SGEN_MAX_DEBUG_LEVEL &&                          \
                   (level) <= sgen_gc_debug_level)) {                          \
      if (cond) {                                                              \
...
static inline guint sgen_aligned_addr_hash(gconstpointer ptr) {
  return GPOINTER_TO_UINT(ptr) >> 3;
}

#define SGEN_PTR_IN_NURSERY(p, bits, start, end)                               \
  (((mword)(p) & ~(((mword)1 << (bits)) - 1)) == (mword)(start))
...
static inline MONO_ALWAYS_INLINE gboolean sgen_ptr_in_nursery(void *p) {
  return SGEN_PTR_IN_NURSERY((p), sgen_nursery_bits, sgen_nursery_start,
                             sgen_nursery_end);
}

static inline MONO_ALWAYS_INLINE char *sgen_get_nursery_start(void) {
  return sgen_nursery_start;
}

static inline MONO_ALWAYS_INLINE char *sgen_get_nursery_end(void) {
  return sgen_nursery_end;
}

/*
 * We use the lowest three bits in the vtable pointer of objects to tag whether
 * they're forwarded, pinned, and/or cemented.  These are the valid states:
 *
 * | State            | bits |
 * |------------------+------+
 * | default          |  000 |
 ...
 * | pinned, cemented |  110 |
 *
 * We store them in the vtable slot because the bits are used in the sync block
 * for other purposes: if we merge them and alloc the sync blocks aligned to 8
 * bytes, we can change this and use bit 3 in the syncblock (with the lower two
 * bits both set for forwarded, that would be an invalid combination for the
 * monitor and hash code).
 */
...
#define SGEN_VTABLE_IS_FORWARDED(vtable)                                       \
  ((GCObject *)(SGEN_POINTER_IS_TAGGED_FORWARDED((vtable))                     \
                    ? SGEN_POINTER_UNTAG_VTABLE((vtable))                      \
                    : NULL))
#define SGEN_OBJECT_IS_FORWARDED(obj)                                          \
  ((GCObject *)SGEN_VTABLE_IS_FORWARDED(((mword *)(obj))[0]))
...
#define SGEN_OBJECT_IS_CEMENTED(obj)                                           \
  (SGEN_POINTER_IS_TAGGED_CEMENTED(((mword *)(obj))[0]))

/* set the forwarded address fw_addr for object obj */
#define SGEN_FORWARD_OBJECT(obj, fw_addr)                                      \
  do {                                                                         \
    *(void **)(obj) = SGEN_POINTER_TAG_FORWARDED((fw_addr));                   \
  } while (0)
#define SGEN_FORWARD_OBJECT_PAR(obj, fw_addr, final_fw_addr)                   \
  do {                                                                         \
    ...
    old_vtable_word = mono_atomic_cas_ptr((gpointer *)obj, new_vtable_word,    \
                                          old_vtable_word);                    \
    final_fw_addr = SGEN_VTABLE_IS_FORWARDED(old_vtable_word);                 \
    if (!final_fw_addr)                                                        \
      final_fw_addr = (fw_addr);                                               \
  } while (0)
#define SGEN_PIN_OBJECT(obj)                                                   \
  do {                                                                         \
    *(void **)(obj) = SGEN_POINTER_TAG_PINNED(*(void **)(obj));                \
  } while (0)
#define SGEN_CEMENT_OBJECT(obj)                                                \
  do {                                                                         \
    *(void **)(obj) = SGEN_POINTER_TAG_CEMENTED(*(void **)(obj));              \
  } while (0)
/* Unpins and uncements */
#define SGEN_UNPIN_OBJECT(obj)                                                 \
  do {                                                                         \
 ...
 */
#define SGEN_LOAD_VTABLE(obj)                                                  \
  ((GCVTable)(                                                                 \
      SGEN_POINTER_UNTAG_ALL(SGEN_LOAD_VTABLE_UNCHECKED((GCObject *)(obj)))))

/*
List of what each bit on of the vtable gc bits means.
*/
enum {
  // When the Java bridge has determined an object is "bridged", it uses these
  // two bits to cache that information.
  SGEN_GC_BIT_BRIDGE_OBJECT = 1,
  SGEN_GC_BIT_BRIDGE_OPAQUE_OBJECT = 2,
  SGEN_GC_BIT_FINALIZER_AWARE = 4,
};

void sgen_gc_init(void) MONO_PERMIT(need(sgen_lock_gc));
...
enum { GENERATION_NURSERY, GENERATION_OLD, GENERATION_MAX };
...
/* the runtime can register areas of memory as roots: we keep two lists of
 * roots, a pinned root set for conservatively scanned roots and a normal one
 * for precisely scanned roots (currently implemented as a single list).
 */
...
int sgen_register_root(char *start, size_t size, SgenDescriptor descr,
                       ...
                       const char *msg) MONO_PERMIT(need(sgen_lock_gc));
void sgen_deregister_root(char *addr) MONO_PERMIT(need(sgen_lock_gc));
...
typedef void (*ScanObjectFunc)(GCObject *obj, SgenDescriptor desc,
                               SgenGrayQueue *);
typedef void (*ScanVTypeFunc)(GCObject *full_object, char *start,
                              SgenDescriptor desc,
                              SgenGrayQueue *BINARY_PROTOCOL_ARG(size_t size));
typedef void (*ScanPtrFieldFunc)(GCObject *obj, GCObject **ptr,
                                 SgenGrayQueue *queue);
...
// error C4576: a parenthesized type followed by an initializer list is a
// non-standard explicit type conversion An inline function or constructor would
// work here too.
#ifdef _MSC_VER
#define MONO_MSC_WARNING_SUPPRESS(warn, body)                                  \
  __pragma(warning(suppress : warn)) body
#else
#define MONO_MSC_WARNING_SUPPRESS(warn, body) body
#endif
#define CONTEXT_FROM_OBJECT_OPERATIONS(ops, queue)                             \
  MONO_MSC_WARNING_SUPPRESS(4576, ((ScanCopyContext){(ops), (queue)}))
...
void *sgen_alloc_internal_dynamic(size_t size, int type,
...
struct _SgenFragment {
  ...
  char *fragment_end;
  SgenFragment *next_in_order; /* We use a different entry for all active
                                  fragments so we can avoid SMR. */
};

typedef struct {
  SgenFragment *alloc_head; /* List head to be used when allocating memory. Walk
                               with fragment_next. */
  SgenFragment *region_head; /* List head of the region used by this allocator.
                                Walk with next_in_order. */
} SgenFragmentAllocator;

void sgen_fragment_allocator_add(SgenFragmentAllocator *allocator, char *start,
                                 char *end);
void sgen_fragment_allocator_release(SgenFragmentAllocator *allocator);
void *sgen_fragment_allocator_par_alloc(SgenFragmentAllocator *allocator,
                                        size_t size);
void *sgen_fragment_allocator_serial_range_alloc(
    SgenFragmentAllocator *allocator, size_t desired_size, size_t minimum_size,
    size_t *out_alloc_size);
void *sgen_fragment_allocator_par_range_alloc(SgenFragmentAllocator *allocator,
                                              size_t desired_size,
                                              size_t minimum_size,
                                              size_t *out_alloc_size);
...
/*
This is a space/speed compromise as we need to make sure the from/to space check
is both O(1) and only hit cache hot memory. On a 4Mb nursery it requires 1024
bytes, or 3% of your average L1 cache. On small configs with a 512kb nursery,
this goes to 0.4%.
...
static inline gboolean sgen_nursery_is_to_space(void *object) {
  size_t idx =
      ((char *)object - sgen_nursery_start) >> SGEN_TO_SPACE_GRANULE_BITS;
  size_t byte = idx >> 3;
  size_t bit = idx & 0x7;

  SGEN_ASSERT(4, sgen_ptr_in_nursery(object),
              "object %p is not in nursery [%p - %p]", object,
              sgen_get_nursery_start(), sgen_get_nursery_end());
  SGEN_ASSERT(4, byte < sgen_space_bitmap_size,
              "byte index %" G_GSIZE_FORMAT "d out of range (%" G_GSIZE_FORMAT
              "d)",
...
static inline gboolean sgen_nursery_is_from_space(void *object) {
  return !sgen_nursery_is_to_space(object);
}

static inline gboolean sgen_nursery_is_object_alive(GCObject *obj) {
...
typedef struct {
  ...
  GCObject *(*alloc_for_promotion)(GCVTable vtable, GCObject *obj,
                                   size_t objsize, gboolean has_references);
  GCObject *(*alloc_for_promotion_par)(GCVTable vtable, GCObject *obj,
                                       size_t objsize, gboolean has_references);
  ...
  void (*init_nursery)(SgenFragmentAllocator *allocator, char *start,
                       char *end);
...
static inline void sgen_update_reference(GCObject **p, GCObject *o,
                                         gboolean allow_null) {
  if (!allow_null)
  ...
  SGEN_ASSERT(0, !sgen_workers_is_worker_thread(mono_native_thread_id_get()),
              "Can't update a reference in the worker thread");
  *p = o;
}

#define SGEN_UPDATE_REFERENCE_ALLOW_NULL(p, o)                                 \
  sgen_update_reference((GCObject **)(p), (GCObject *)(o), TRUE)
#define SGEN_UPDATE_REFERENCE(p, o)                                            \
  sgen_update_reference((GCObject **)(p), (GCObject *)(o), FALSE)
#else
#define SGEN_UPDATE_REFERENCE_ALLOW_NULL(p, o)                                 \
  (*(GCObject **)(p) = (GCObject *)(o))
...
typedef enum {
  ...
  ITERATE_OBJECTS_SWEEP_NON_PINNED =
      ITERATE_OBJECTS_SWEEP | ITERATE_OBJECTS_NON_PINNED,
  ITERATE_OBJECTS_SWEEP_PINNED = ITERATE_OBJECTS_SWEEP | ITERATE_OBJECTS_PINNED,
  ITERATE_OBJECTS_SWEEP_ALL = ITERATE_OBJECTS_SWEEP |
...
struct _SgenMajorCollector {
  ...
  GCObject *(*alloc_small_pinned_obj)(GCVTable vtable, size_t size,
  ...
  GCObject *(*alloc_object)(GCVTable vtable, size_t size,
                            gboolean has_references);
  GCObject *(*alloc_object_par)(GCVTable vtable, size_t size,
                                gboolean has_references);
  void (*free_pinned_object)(GCObject *obj, size_t size);

  /*
   * This is used for domain unloading, heap walking from the logging profiler,
   * and debugging.  Can assume the world is stopped.
   */
  void (*iterate_objects)(IterateObjectsFlags flags,
                          IterateObjectCallbackFunc callback, void *data);
  ...
  void (*scan_card_table)(CardTableScanType scan_type, ScanCopyContext ctx,
                          int job_index, int job_split_count, int block_count);
...
typedef struct _SgenRememberedSet {
  void (*wbarrier_set_field)(GCObject *obj, gpointer field_ptr,
                             GCObject *value);
  void (*wbarrier_arrayref_copy)(gpointer dest_ptr, gconstpointer src_ptr,
                                 int count);
  void (*wbarrier_value_copy)(gpointer dest, gconstpointer src, int count,
                              size_t element_size);
  ...
  gboolean (*find_address_with_cards)(char *cards_start, guint8 *cards,
                                      char *addr);
} SgenRememberedSet;
...
/*
 * These must be kept in sync with object.h.  They're here for using SGen
 * independently of Mono.
 */
void mono_gc_wbarrier_arrayref_copy(gpointer dest_ptr, /*const*/ void *src_ptr,
                                    int count);
...
static inline SgenDescriptor sgen_obj_get_descriptor(GCObject *obj) {
...
static inline SgenDescriptor sgen_obj_get_descriptor_safe(GCObject *obj) {
...
static inline mword sgen_safe_object_get_size(GCObject *obj) {
  GCObject *forwarded;
  GCVTable vtable = SGEN_LOAD_VTABLE_UNCHECKED(obj);

  /*
   * Once we load the vtable, we must always use it, in case we are in parallel
   * case. Otherwise the object might get forwarded in the meantime and we would
   * read an invalid vtable. An object cannot be forwarded for a second time
   * during same GC.
   */
  if ((forwarded = SGEN_VTABLE_IS_FORWARDED(vtable)))
    return sgen_client_par_object_get_size(SGEN_LOAD_VTABLE(forwarded), obj);
  else
    return sgen_client_par_object_get_size(
        (GCVTable)SGEN_POINTER_UNTAG_ALL(vtable), obj);
}

static inline gboolean sgen_safe_object_is_small(GCObject *obj, int type) {
  if (type <= DESC_TYPE_MAX_SMALL_OBJ)
    return TRUE;
  return SGEN_ALIGN_UP(sgen_safe_object_get_size((GCObject *)obj)) <=
 ...
 */
static inline guint sgen_safe_object_get_size_unaligned(GCObject *obj) {
  GCObject *forwarded;
...
void sgen_mark_bridge_object(GCObject *obj) MONO_PERMIT(need(sgen_gc_locked));
...
void sgen_null_links_if(SgenObjectPredicateFunc predicate, void *data,
                        int generation, gboolean track)
...
void sgen_gc_lock(void)
    MONO_PERMIT(use(sgen_lock_gc), grant(sgen_gc_locked), revoke(sgen_lock_gc));
void sgen_gc_unlock(void)
    MONO_PERMIT(use(sgen_gc_locked), revoke(sgen_gc_locked),
                grant(sgen_lock_gc));
...
void sgen_finalize_in_range(int generation, ScanCopyContext ctx)
    MONO_PERMIT(need(sgen_gc_locked));
void sgen_null_link_in_range(int generation, ScanCopyContext ctx,
                             gboolean track)
    MONO_PERMIT(need(sgen_gc_locked, sgen_world_stopped));
void sgen_process_fin_stage_entries(void) MONO_PERMIT(need(sgen_gc_locked));
gboolean sgen_have_pending_finalizers(void);

typedef void (*SGenFinalizationProc)(
    gpointer, gpointer); // same as MonoFinalizationProc, GC_finalization_proc

void sgen_object_register_for_finalization(GCObject *obj,
                                           SGenFinalizationProc user_data)
    MONO_PERMIT(need(sgen_lock_gc));

void sgen_finalize_if(SgenObjectPredicateFunc predicate, void *user_data)
    MONO_PERMIT(need(sgen_lock_gc));
void sgen_remove_finalizers_if(SgenObjectPredicateFunc predicate,
                               void *user_data, int generation);
...
void sgen_register_disappearing_link(GCObject *obj, void **link, gboolean track,
                                     gboolean in_gc);
...
enum { SPACE_NURSERY, SPACE_MAJOR, SPACE_LOS };
...
void sgen_perform_collection(size_t requested_size, int generation_to_collect,
                             const char *reason, gboolean wait_to_finish,
                             gboolean stw)
    MONO_PERMIT(need(sgen_gc_locked, sgen_stop_world));

int sgen_gc_collection_count(int generation);
/* FIXME: what exactly does this return? */
size_t sgen_gc_get_used_size(void) MONO_PERMIT(need(sgen_lock_gc));
size_t sgen_gc_get_total_heap_allocation(void);

/* STW */

void sgen_stop_world(int generation, gboolean serial_collection)
    MONO_PERMIT(need(sgen_gc_locked), use(sgen_stop_world),
                grant(sgen_world_stopped), revoke(sgen_stop_world));
void sgen_restart_world(int generation, gboolean serial_collection)
    MONO_PERMIT(need(sgen_gc_locked), use(sgen_world_stopped),
                revoke(sgen_world_stopped), grant(sgen_stop_world));
...
struct _LOSObject {
  ...
  guint8
      *volatile cardtable_mod_union; /* only used by the concurrent collector */
...
void sgen_los_iterate_objects_free(IterateObjectResultCallbackFunc cb,
                                   void *user_data);
...
void sgen_los_scan_card_table(CardTableScanType scan_type, ScanCopyContext ctx,
                              int job_index, int job_split_count);
void sgen_los_update_cardtable_mod_union(void);
void sgen_los_count_cards(long long *num_total_cards,
...
void sgen_los_pin_objects(SgenGrayQueue *gray_queue,
                          gboolean finish_concurrent_mode);
...
void sgen_nursery_allocator_set_nursery_bounds(char *nursery_start,
                                               size_t min_size,
                                               size_t max_size);
...
void *sgen_nursery_alloc_range(size_t size, size_t min_size,
                               size_t *out_alloc_size);
...
GCObject *sgen_alloc_for_promotion(GCObject *obj, size_t objsize,
                                   gboolean has_references);
...
static inline gboolean sgen_major_is_object_alive(GCObject *object) {
  mword objsize;
 ...
 */
static inline gboolean sgen_is_object_alive(GCObject *object) {
...
/*
 * This function returns true if @object is either alive or it belongs to the
 * old gen and we're currently doing a minor collection.
 */
static inline int sgen_is_object_alive_for_current_gen(GCObject *object) {
  if (sgen_ptr_in_nursery(object))
    return sgen_nursery_is_object_alive(object);
...
int sgen_gc_invoke_finalizers(void) MONO_PERMIT(need(sgen_lock_gc));
...
typedef gpointer (*SgenGCHandleIterateCallback)(gpointer hidden,
...
void sgen_gchandle_iterate(GCHandleType handle_type, int max_generation,
                           SgenGCHandleIterateCallback callback, gpointer user)
    MONO_PERMIT(need(sgen_world_stopped));
void sgen_gchandle_set_target(guint32 gchandle, GCObject *obj);
void sgen_mark_normal_gc_handles(void *addr, SgenUserMarkFunc mark_func,
                                 void *gc_data)
    MONO_PERMIT(need(sgen_world_stopped));
void sgen_gc_handles_report_roots(SgenUserReportRootFunc mark_func,
                                  void *gc_data)
    MONO_PERMIT(need(sgen_world_stopped));
...
static inline void sgen_set_nursery_scan_start(char *p) {
...
void sgen_qsort(void *const array, const size_t count,
...
#define CANARY_VALID(addr)                                                     \
  (strncmp((char *)(addr), CANARY_STRING, CANARY_SIZE) == 0)
...
#define CHECK_CANARY_FOR_OBJECT(addr, ignored)                                 \
  (sgen_nursery_canaries_enabled() ? sgen_check_canary_for_object(addr)        \
                                   : (void)0)

/*
 * This causes the compile to extend the liveness of 'v' till the call to
 * dummy_use
 */
static inline void sgen_dummy_use(gpointer v) {
