 ...
 * major, to reduce unnecessary collections. We make sure we don't set the
 * minimum allowance too high when using a soft heap limit.
 */
#define MIN_MINOR_COLLECTION_ALLOWANCE                                         \
  (MIN(((mword)(sgen_nursery_size * default_allowance_nursery_size_ratio)),    \
       (soft_heap_limit * SGEN_DEFAULT_ALLOWANCE_HEAP_SIZE_RATIO)))

static SgenPointerQueue log_entries =
    SGEN_POINTER_QUEUE_INIT(INTERNAL_MEM_TEMPORARY);
...
static double default_allowance_nursery_size_ratio =
...
static void sgen_memgov_calculate_minor_collection_allowance(void) {
  ...
  SGEN_ASSERT(0, sgen_major_collector.have_swept(),
  ...
  /*
   * We allow the heap to grow by one third its current size before we start the
  ...
  /*
   * For the concurrent collector, we decrease the allowance relative to the
   * memory growth during the M&S phase, survival rate of the collection and the
   * allowance ratio.
   */
  decrease = (major_pre_sweep_heap_size - major_start_heap_size) *
             ((float)new_heap_size / major_pre_sweep_heap_size) *
             (SGEN_DEFAULT_ALLOWANCE_HEAP_SIZE_RATIO + 1);
  ...
  if (new_heap_size + allowance > soft_heap_limit) {
    if (new_heap_size > soft_heap_limit)
      ...
      allowance =
          MAX(soft_heap_limit - new_heap_size, MIN_MINOR_COLLECTION_ALLOWANCE);
  }

  /* FIXME: Why is this here? */
  if (sgen_major_collector.free_swept_blocks)
    sgen_major_collector.free_swept_blocks(
        sgen_major_collector.get_num_major_sections() *
  ...
  if (debug_print_allowance) {
    SGEN_LOG(0, "Surviving sweep: %ld bytes (%ld major, %ld LOS)",
             (long)new_heap_size, (long)new_major,
             (long)last_collection_los_memory_usage);
...
static size_t get_heap_size(void) {
  return sgen_major_collector.get_num_major_sections() *
...
gboolean sgen_need_major_collection(mword space_needed, gboolean *forced) {
  ...
  *forced = FALSE;
  ...
  if (sgen_get_concurrent_collection_in_progress()) {
    ...
    /*
     * The more the heap grows, the more we need to decrease the allowance
     * above, in order to have similar trigger sizes as the synchronous
     * collector. If the heap grows so much that we would need to have a
     * negative allowance, we force the finishing of the collection, to avoid
     * increased memory usage.
     */
    if ((heap_size - major_start_heap_size) >
        major_start_heap_size * SGEN_DEFAULT_ALLOWANCE_HEAP_SIZE_RATIO)
      return TRUE;
    return FALSE;
  }
...
static void sgen_add_log_entry(SgenLogEntry *log_entry) {
...
void sgen_memgov_minor_collection_end(const char *reason,
  ...
  if (mono_trace_is_traced(G_LOG_LEVEL_INFO, MONO_TRACE_GC)) {
    SgenLogEntry *log_entry =
        (SgenLogEntry *)sgen_alloc_internal(INTERNAL_MEM_LOG_ENTRY);
    ...
    log_entry->promoted_size =
        sgen_total_promoted_size - total_promoted_size_start;
    log_entry->major_size = sgen_major_collector.get_num_major_sections() *
                            sgen_major_collector.section_size;
    log_entry->major_size_in_use = last_used_slots_size +
                                   sgen_total_allocated_major -
...
void sgen_memgov_major_post_sweep(mword used_slots_size) {
  if (mono_trace_is_traced(G_LOG_LEVEL_INFO, MONO_TRACE_GC)) {
    ...
    log_entry->major_size = sgen_major_collector.get_num_major_sections() *
                            sgen_major_collector.section_size;
    log_entry->major_size_in_use = used_slots_size +
                                   sgen_total_allocated_major -
                                   total_allocated_major_end;
  ...
  sgen_gc_info.heap_size_bytes = sgen_major_collector.get_num_major_sections() *
                                     sgen_major_collector.section_size +
                                 sgen_los_memory_usage_total;
  sgen_gc_info.fragmented_bytes =
      sgen_gc_info.heap_size_bytes - sgen_los_memory_usage -
      (used_slots_size + sgen_total_allocated_major -
       total_allocated_major_end);
...
void sgen_memgov_major_collection_start(gboolean concurrent,
                                        const char *reason) {
  need_calculate_minor_collection_allowance = TRUE;
  major_start_heap_size = get_heap_size();

  if (debug_print_allowance) {
    SGEN_LOG(0, "Starting collection with heap size %ld bytes",
             (long)major_start_heap_size);
  }
...
void sgen_memgov_major_collection_end(gboolean forced, gboolean concurrent,
                                      const char *reason,
                                      gboolean is_overflow) {
  if (mono_trace_is_traced(G_LOG_LEVEL_INFO, MONO_TRACE_GC)) {
    SgenLogEntry *log_entry =
...
static void sgen_output_log_entry(SgenLogEntry *entry, gint64 stw_time,
                                  int generation) {
  ...
  case SGEN_LOG_NURSERY:
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
               "GC_MINOR%s: (%s) time %.2fms, %s promoted %luK major size: "
               "%luK in use: %luK los size: %luK in use: %luK",
               entry->is_overflow ? "_OVERFLOW" : "",
               entry->reason ? entry->reason : "", entry->time / 10000.0f,
  ...
  case SGEN_LOG_MAJOR_SERIAL:
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
               "GC_MAJOR%s: (%s) time %.2fms, %s los size: %luK in use: %luK",
               entry->is_overflow ? "_OVERFLOW" : "",
               entry->reason ? entry->reason : "", (int)entry->time / 10000.0f,
               full_timing_buff, (unsigned long)entry->los_size / 1024,
               (unsigned long)entry->los_size_in_use / 1024);
    break;
  case SGEN_LOG_MAJOR_CONC_START:
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
               "GC_MAJOR_CONCURRENT_START: (%s)",
               entry->reason ? entry->reason : "");
    break;
  case SGEN_LOG_MAJOR_CONC_FINISH:
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
               "GC_MAJOR_CONCURRENT_FINISH: (%s) time %.2fms, %s los size: "
               "%luK in use: %luK",
               entry->reason ? entry->reason : "", entry->time / 10000.0f,
               full_timing_buff, (unsigned long)entry->los_size / 1024,
               (unsigned long)entry->los_size_in_use / 1024);
  ...
  case SGEN_LOG_MAJOR_SWEEP_FINISH:
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
               "GC_MAJOR_SWEEP: major size: %luK in use: %luK",
               (unsigned long)entry->major_size / 1024,
               (unsigned long)entry->major_size_in_use / 1024);
    break;
  default:
...
void sgen_memgov_collection_end(int generation, gint64 stw_time) {
  /*
   * At this moment the world has been restarted which means we can log all
   * pending entries without risking deadlocks.
   */
  if (mono_trace_is_traced(G_LOG_LEVEL_INFO, MONO_TRACE_GC)) {
    size_t i;
    SGEN_ASSERT(0, !sgen_is_world_stopped(),
                "We can't log if the world is stopped");
    mono_os_mutex_lock(&log_entries_mutex);
    for (i = 0; i < log_entries.next_slot; i++) {
      sgen_output_log_entry((SgenLogEntry *)log_entries.data[i], stw_time,
                            generation);
...
static unsigned long prot_flags_for_activate(int activate) {
  unsigned long prot_flags =
      activate ? MONO_MMAP_READ | MONO_MMAP_WRITE : MONO_MMAP_NONE;
  return prot_flags | MONO_MMAP_PRIVATE | MONO_MMAP_ANON;
}

void sgen_assert_memory_alloc(void *ptr, size_t requested_size,
                              const char *assert_description) {
  if (ptr || !assert_description)
    return;
  fprintf(stderr,
          "Error: Garbage collector could not allocate %" G_GSIZE_FORMAT
          "u bytes of memory for %s.\n",
          requested_size, assert_description);
  exit(1);
}

/*
 * Allocate a big chunk of memory from the OS (usually 64KB to several
 * megabytes). This must not require any lock.
 */
void *sgen_alloc_os_memory(size_t size, SgenAllocFlags flags,
                           const char *assert_description,
  ...
  ptr = mono_valloc(0, size,
                    prot_flags_for_activate(flags & SGEN_ALLOC_ACTIVATE), type);
...
void *sgen_alloc_os_memory_aligned(size_t size, mword alignment,
  ...
  ptr = mono_valloc_aligned(
      size, alignment, prot_flags_for_activate(flags & SGEN_ALLOC_ACTIVATE),
      type);
  sgen_assert_memory_alloc(ptr, size, assert_description);
  if (ptr) {
    SGEN_ATOMIC_ADD_P(total_alloc, size);
...
size_t sgen_gc_get_total_heap_allocation(void) { return total_alloc; }
...
gboolean sgen_memgov_try_alloc_space(mword size, int space) {
  if (sgen_memgov_available_free_space() < size) {
    SGEN_ASSERT(4, !sgen_workers_is_worker_thread(mono_native_thread_id_get()),
...
void sgen_memgov_init(size_t max_heap, size_t soft_limit,
                      gboolean debug_allowance, double allowance_ratio,
                      double save_target) {
  if (soft_limit)
  ...
  mono_counters_register("Memgov alloc",
                         MONO_COUNTER_GC | MONO_COUNTER_WORD |
                             MONO_COUNTER_BYTES | MONO_COUNTER_VARIABLE,
                         (void *)&total_alloc);
  mono_counters_register("Memgov max alloc",
                         MONO_COUNTER_GC | MONO_COUNTER_WORD |
                             MONO_COUNTER_BYTES | MONO_COUNTER_MONOTONIC,
                         (void *)&total_alloc_max);

  mono_os_mutex_init(&log_entries_mutex);

  sgen_register_fixed_internal_mem_type(INTERNAL_MEM_LOG_ENTRY,
                                        sizeof(SgenLogEntry));

  if (max_heap == 0) {
    sgen_gc_info.total_available_memory_bytes =
        mono_determine_physical_ram_size();

    // This threshold is commonly used by software caches to detect when they
    // are approaching the limit of available memory. In sgen it is not adjusted
    // dynamically, since sgen does not adjust compaction strategies based on a
    // threshold.
    sgen_gc_info.high_memory_load_threshold_bytes =
        .9 * sgen_gc_info.total_available_memory_bytes;

    if (!sgen_gc_info.total_available_memory_bytes) {
      SGEN_LOG(
          9, "Warning: Unable to determine physical ram size for GCMemoryInfo");
    }
  ...
  if (max_heap < soft_limit) {
    sgen_env_var_error(
        MONO_GC_PARAMS_NAME, "Setting to minimum.",
