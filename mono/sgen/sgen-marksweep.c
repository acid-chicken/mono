...
#define MS_NUM_MARK_WORDS                                                      \
  ((ms_block_size / SGEN_ALLOC_ALIGN + sizeof(guint32) * 8 - 1) /              \
   (sizeof(guint32) * 8))
 ...
 */
#define SIZEOF_MS_BLOCK_INFO                                                   \
  (sizeof(MSBlockInfo) +                                                       \
   sizeof(guint32) * (MS_NUM_MARK_WORDS - MONO_ZERO_LEN_ARRAY))
...
#define MS_BLOCK_OBJ(b, i)                                                     \
  ((GCObject *)(MS_BLOCK_FOR_BLOCK_INFO(b) + MS_BLOCK_SKIP +                   \
                (b)->obj_size * (i)))
#define MS_BLOCK_OBJ_FOR_SIZE(b, i, obj_size)                                  \
  (MS_BLOCK_FOR_BLOCK_INFO(b) + MS_BLOCK_SKIP + (obj_size) * (i))
#define MS_BLOCK_DATA_FOR_OBJ(o)                                               \
  ((char *)((mword)(o) & ~(mword)(ms_block_size - 1)))

typedef struct {
  MSBlockInfo info;
} MSBlockHeader;

#define MS_BLOCK_FOR_OBJ(o)                                                    \
  (&((MSBlockHeader *)MS_BLOCK_DATA_FOR_OBJ((o)))->info)

/* object index will always be small */
#define MS_BLOCK_OBJ_INDEX(o, b)                                               \
  ((int)(((char *)(o) - (MS_BLOCK_FOR_BLOCK_INFO(b) + MS_BLOCK_SKIP)) /        \
         (b)->obj_size))

// casting to int is fine since blocks are 32k
#define MS_CALC_MARK_BIT(w, b, o)                                              \
  do {                                                                         \
    int i = ((int)((char *)(o)-MS_BLOCK_DATA_FOR_OBJ((o)))) >>                 \
            SGEN_ALLOC_ALIGN_BITS;                                             \
    (w) = i >> 5;                                                              \
    (b) = i & 31;                                                              \
  } while (0)
...
#define MS_SET_MARK_BIT_PAR(bl, w, b, first)                                   \
  do {                                                                         \
    ...
    while (!(tmp_mark_word & (ONE_P << (b)))) {                                \
      ...
      tmp_mark_word =                                                          \
          mono_atomic_cas_i32((volatile gint32 *)&(bl)->mark_words[w],         \
                              old_mark_word | (ONE_P << (b)), old_mark_word);  \
...
#define MS_OBJ_ALLOCED(o, b)                                                   \
  (*(void **)(o) &&                                                            \
   (*(char **)(o) < MS_BLOCK_FOR_BLOCK_INFO(b) ||                              \
    *(char **)(o) >= MS_BLOCK_FOR_BLOCK_INFO(b) + ms_block_size))
...
#define BLOCK_TAG(bl)                                                          \
  ((bl)->has_references ? BLOCK_TAG_HAS_REFERENCES((bl)) : (bl))

/* all allocated blocks in the system */
static SgenArrayList allocated_blocks = SGEN_ARRAY_LIST_INIT(
    NULL, sgen_array_list_default_is_slot_set,
    sgen_array_list_default_cas_setter, INTERNAL_MEM_PIN_QUEUE);
...
#define FOREACH_BLOCK_RANGE_HAS_REFERENCES_NO_LOCK(bl, begin, end, index, hr)  \
  {                                                                            \
    volatile gpointer *slot;                                                   \
    SGEN_ARRAY_LIST_FOREACH_SLOT_RANGE(&allocated_blocks, begin, end, slot,    \
                                       index) {                                \
      (bl) = (MSBlockInfo *)(*slot);                                           \
      if (!(bl))                                                               \
        continue;                                                              \
      (hr) = BLOCK_IS_TAGGED_HAS_REFERENCES((bl));                             \
      (bl) = BLOCK_UNTAG((bl));
#define END_FOREACH_BLOCK_RANGE_NO_LOCK                                        \
  }                                                                            \
...
/*
 * One free block list for each block object size.  We add and remove blocks
 * from these lists lock-free via CAS.
 *
 ...
 *   in the finishing pause of concurrent major collections (whole list is
 * cleared)
 *
 ...
 *
 * The only item of those that doesn't require the GC lock is the sweep thread.
 * The sweep thread only ever adds blocks to the free list, so the ABA problem
 * can't occur.
 */
...
static void add_scanned_object(void *ptr) {
...
static int ms_find_block_obj_size_index(size_t size) {
  int i;
  SGEN_ASSERT(9, size <= SGEN_MAX_SMALL_OBJ_SIZE,
              "size %" G_GSIZE_FORMAT
              "d is bigger than max small object size %d",
...
#define FREE_BLOCKS_FROM(lists, p, r)                                          \
  (lists[((p) ? MS_BLOCK_FLAG_PINNED : 0) | ((r) ? MS_BLOCK_FLAG_REFS : 0)])
#define FREE_BLOCKS(p, r) (FREE_BLOCKS_FROM(free_block_lists, (p), (r)))
#define FREE_BLOCKS_LOCAL(p, r)                                                \
  (FREE_BLOCKS_FROM(((MSBlockInfo ***)mono_native_tls_get_value(               \
                        worker_block_free_list_key)),                          \
                    (p), (r)))
...
static void *major_alloc_heap(mword nursery_size, mword nursery_align) {
  char *start;
  if (nursery_align)
    start = (char *)sgen_alloc_os_memory_aligned(
        nursery_size, nursery_align,
        (SgenAllocFlags)(SGEN_ALLOC_HEAP | SGEN_ALLOC_ACTIVATE), "nursery",
        MONO_MEM_ACCOUNT_SGEN_NURSERY);
  else
    start = (char *)sgen_alloc_os_memory(
        nursery_size, (SgenAllocFlags)(SGEN_ALLOC_HEAP | SGEN_ALLOC_ACTIVATE),
        "nursery", MONO_MEM_ACCOUNT_SGEN_NURSERY);

  return start;
}

static void update_heap_boundaries_for_block(MSBlockInfo *block) {
  sgen_update_heap_boundaries((mword)MS_BLOCK_FOR_BLOCK_INFO(block),
                              (mword)MS_BLOCK_FOR_BLOCK_INFO(block) +
                                  ms_block_size);
}
 ...
 * Thread safe
 */
static void *ms_get_empty_block(void) {
  char *p;
  int i;
...
retry:
  if (!empty_blocks) {
    ...
    for (;;) {
      p = (char *)sgen_alloc_os_memory_aligned(
          ms_block_size * alloc_num, ms_block_size,
...
/*
 * This doesn't actually free a block immediately, but enqueues it into the
 * `empty_blocks` list, where it will either be freed later on, or reused in
 * nursery collections.
 */
static void ms_free_block(MSBlockInfo *info) {
  void *empty;
  char *block = MS_BLOCK_FOR_BLOCK_INFO(info);

  sgen_memgov_release_space(ms_block_size, SPACE_MAJOR);
  if (info->cardtable_mod_union)
    sgen_card_table_free_mod_union(info->cardtable_mod_union, block,
                                   ms_block_size);
...
static gboolean sweep_in_progress(void) {
...
static gboolean block_is_swept_or_marking(MSBlockInfo *block) {
...
static void check_block_free_list(MSBlockInfo *block, int size,
                                  gboolean pinned) {
  SGEN_ASSERT(0, !sweep_in_progress(),
              "Can't examine allocated blocks during sweep");
  for (; block; block = block->next_free) {
    SGEN_ASSERT(0, block->state != BLOCK_STATE_CHECKING,
    ...
    g_assert(sgen_array_list_find(&allocated_blocks, BLOCK_TAG(block)) !=
             (guint32)-1);
  }
}

static void check_empty_blocks(void) {
  void *p;
  size_t i = 0;
...
static void consistency_check(void) {
  ...
  for (i = 0; i < num_block_obj_sizes; ++i) {
    ...
    for (j = 0; j < MS_BLOCK_TYPE_MAX; ++j)
      check_block_free_list(free_block_lists[j][i], block_obj_sizes[i],
                            j & MS_BLOCK_FLAG_PINNED);
  }
...
static void add_free_block(MSBlockInfo *volatile *free_blocks, int size_index,
                           MSBlockInfo *block) {
  MSBlockInfo *old;
  do {
    block->next_free = old = free_blocks[size_index];
  } while (SGEN_CAS_PTR((volatile gpointer *)&free_blocks[size_index], block,
                        old) != old);
}

static void major_finish_sweep_checking(void);

static gboolean ms_alloc_block(int size_index, gboolean pinned,
  ...
  SGEN_ASSERT(9, count >= 2, "block with %d objects, it must hold at least 2",
              count);
  ...
  /*
   * Blocks that are to-space are not evacuated from.  During an major
   * collection blocks are allocated for two reasons: evacuating objects from
   * the nursery and evacuating them from major blocks marked for evacuation. In
   * both cases we don't want further evacuation. We also don't want to evacuate
   * objects allocated during the concurrent mark since it would add pointless
   * stress on the finishing pause.
   */
  info->is_to_space =
  ...
  SGEN_ASSERT(6, !sweep_in_progress() || info->state == BLOCK_STATE_SWEPT,
...
static gboolean ptr_is_in_major_block(char *ptr, char **start,
                                      gboolean *pinned) {
  MSBlockInfo *block;

  FOREACH_BLOCK_NO_LOCK(block) {
    if (ptr >= MS_BLOCK_FOR_BLOCK_INFO(block) &&
        ptr <= MS_BLOCK_FOR_BLOCK_INFO(block) + ms_block_size) {
      int count = MS_BLOCK_FREE / block->obj_size;
      int i;
      ...
      if (start)
        *start = NULL;
      for (i = 0; i <= count; ++i) {
        if (ptr >= (char *)MS_BLOCK_OBJ(block, i) &&
            ptr < (char *)MS_BLOCK_OBJ(block, i + 1)) {
          if (start)
...
static gboolean ptr_is_from_pinned_alloc(char *ptr) {
  gboolean pinned;
...
static void ensure_can_access_block_free_list(MSBlockInfo *block) {
retry:
  for (;;) {
    switch (block->state) {
...
static void *
unlink_slot_from_free_list_uncontested(MSBlockInfo *volatile *free_blocks,
                                       int size_index) {
...
retry:
  ...
  SGEN_ASSERT(9, block,
              "no free block to unlink from free_blocks %p size_index %d",
              free_blocks, size_index);

  ensure_can_access_block_free_list(block);

  obj = block->free_list;
  SGEN_ASSERT(6, obj,
              "block %p in free list had no available object to alloc from",
              block);
  ...
  if (SGEN_CAS_PTR((volatile gpointer *)&free_blocks[size_index],
                   next_free_block, block) != block)
    goto retry;
...
static GCObject *alloc_obj(GCVTable vtable, size_t size, gboolean pinned,
                           gboolean has_references) {
...
static GCObject *major_alloc_object(GCVTable vtable, size_t size,
                                    gboolean has_references) {
 ...
 */
static GCObject *major_alloc_object_par(GCVTable vtable, size_t size,
                                        gboolean has_references) {
  int size_index = MS_BLOCK_OBJ_SIZE_INDEX(size);
  ...
  get_block:
    ...
    if (!block) {
      ...
      /*
       * Once a block is removed from the main list, it cannot return on the
       * list until all the workers are finished and sweep is starting. This
       * means we don't need to account for ABA problems.
       */
      if (SGEN_CAS_PTR((volatile gpointer *)&free_blocks[size_index], next_free,
                       block) != block)
        goto get_block;
 ...
 */
static void free_object(GCObject *obj, size_t size, gboolean pinned) {
  MSBlockInfo *block = MS_BLOCK_FOR_OBJ(obj);
  int word, bit;
  gboolean in_free_list;

  SGEN_ASSERT(9, sweep_state == SWEEP_STATE_SWEPT,
              "Should have waited for sweep to free objects.");

  ensure_can_access_block_free_list(block);
  SGEN_ASSERT(
      9, (pinned && block->pinned) || (!pinned && !block->pinned),
      "free-object pinning mixup object %p pinned %d block %p pinned %d", obj,
      pinned, block, block->pinned);
  ...
  *(void **)obj = block->free_list;
  ...
  if (!in_free_list) {
    MSBlockInfo *volatile *free_blocks =
        FREE_BLOCKS(pinned, block->has_references);
    int size_index = MS_BLOCK_OBJ_SIZE_INDEX(size);
    SGEN_ASSERT(9, !block->next_free,
                "block %p doesn't have a free-list of object but belongs to a "
                "free-list of blocks",
                block);
...
static void major_free_non_pinned_object(GCObject *obj, size_t size) {
  free_object(obj, size, FALSE);
}

/* size is a multiple of SGEN_ALLOC_ALIGN */
static GCObject *major_alloc_small_pinned_obj(GCVTable vtable, size_t size,
                                              gboolean has_references) {
  void *res;
...
static void free_pinned_object(GCObject *obj, size_t size) {
 ...
 */
static GCObject *major_alloc_degraded(GCVTable vtable, size_t size) {
  GCObject *obj;
 ...
 */
static gboolean major_is_object_live(GCObject *obj) {
  MSBlockInfo *block;
  int word, bit;
  mword objsize;
...
static gboolean major_ptr_is_in_non_pinned_space(char *ptr, char **start) {
  gboolean pinned;
...
static gboolean try_set_sweep_state(int new_, int expected) {
  int old = SGEN_CAS(&sweep_state, new_, expected);
  return old == expected;
}

static void set_sweep_state(int new_, int expected) {
  gboolean success = try_set_sweep_state(new_, expected);
  SGEN_ASSERT(0, success, "Could not set sweep state.");
}

static gboolean ensure_block_is_checked_for_sweeping(guint32 block_index,
                                                     gboolean wait,
...
retry:
  ...
  case SWEEP_STATE_SWEEPING:
    if (try_set_sweep_state(SWEEP_STATE_SWEEPING_AND_ITERATING,
                            SWEEP_STATE_SWEEPING))
      break;
    goto retry;
  ...
  /*
   * We're running with the world stopped and the only other thread doing work
   * is the sweep thread, which doesn't add blocks to the array, so we can
   * safely access `next_slot`.
   */
...
wait:
  ...
  SGEN_ASSERT(0, sweep_state == SWEEP_STATE_SWEPT,
              "How is the sweep job done but we're not swept?");
}

static void major_iterate_objects(IterateObjectsFlags flags,
  ...
  /* No actual sweeping will take place if we are in the middle of a major
   * collection. */
  ...
  FOREACH_BLOCK_NO_LOCK(block) {
    ...
    if (sweep && lazy_sweep && !block_is_swept_or_marking(block)) {
      sweep_block(block);
      SGEN_ASSERT(6, block->state == BLOCK_STATE_SWEPT,
...
static gboolean major_is_valid_object(char *object) {
  MSBlockInfo *block;
  ...
  FOREACH_BLOCK_NO_LOCK(block) {
    ...
    if ((MS_BLOCK_FOR_BLOCK_INFO(block) > object) ||
        ((MS_BLOCK_FOR_BLOCK_INFO(block) + ms_block_size) <= object))
      continue;
...
static GCVTable major_describe_pointer(char *ptr) {
  MSBlockInfo *block;
  ...
  FOREACH_BLOCK_NO_LOCK(block) {
    ...
    if ((MS_BLOCK_FOR_BLOCK_INFO(block) > ptr) ||
        ((MS_BLOCK_FOR_BLOCK_INFO(block) + ms_block_size) <= ptr))
      continue;

    SGEN_LOG(0, "major-ptr (block %p sz %d pin %d ref %d)\n",
             MS_BLOCK_FOR_BLOCK_INFO(block), block->obj_size, block->pinned,
             block->has_references);
...
static void major_check_scan_starts(void) {}

static void major_dump_heap(FILE *heap_dump_file) {
  MSBlockInfo *block;
  ...
  FOREACH_BLOCK_NO_LOCK(block) {
    ...
    fprintf(heap_dump_file,
            "<section type=\"%s\" size=\"%" G_GSIZE_FORMAT "u\">\n", "old",
            (size_t)MS_BLOCK_FREE);
    ...
    for (i = 0; i <= count; ++i) {
      if ((i < count) && MS_OBJ_ALLOCED(MS_BLOCK_OBJ(block, i), block)) {
        ...
        if (start >= 0) {
          sgen_dump_occupied((char *)MS_BLOCK_OBJ(block, start),
                             (char *)MS_BLOCK_OBJ(block, i),
...
static guint8 *get_cardtable_mod_union_for_block(MSBlockInfo *block,
                                                 gboolean allocate) {
  ...
  if (mod_union)
    ...
    return NULL;
  mod_union = sgen_card_table_alloc_mod_union(MS_BLOCK_FOR_BLOCK_INFO(block),
                                              ms_block_size);
  other = (guint8 *)SGEN_CAS_PTR((gpointer *)&block->cardtable_mod_union,
                                 mod_union, NULL);
  if (!other) {
    SGEN_ASSERT(0, block->cardtable_mod_union == mod_union,
                "Why did CAS not replace?");
    return mod_union;
  }
  sgen_card_table_free_mod_union(mod_union, MS_BLOCK_FOR_BLOCK_INFO(block),
                                 ms_block_size);
  return other;
}

static guint8 *major_get_cardtable_mod_union_for_reference(char *ptr) {
  MSBlockInfo *block = MS_BLOCK_FOR_OBJ(ptr);
  size_t offset = sgen_card_table_get_card_offset(
      ptr,
  ...
  SGEN_ASSERT(0, mod_union,
              "FIXME: optionally allocate the mod union if it's not here and "
              "CAS it in.");
  return &mod_union[offset];
}

/*
 * Mark the mod-union card for `ptr`, which must be a reference within the
 * object `obj`.
 */
static void mark_mod_union_card(GCObject *obj, void **ptr,
                                GCObject *value_obj) {
  ...
  if (sgen_safe_object_is_small(obj, type)) {
    ...
    SGEN_ASSERT(0, MS_BLOCK_FOR_OBJ(obj) == MS_BLOCK_FOR_OBJ(ptr),
                "How can an object and a reference inside it not be in the "
                "same block?");
    *card_byte = 1;
  } else {
    sgen_los_mark_mod_union_card(obj, ptr);
  }
  sgen_binary_protocol_mod_union_remset(obj, ptr, value_obj,
                                        SGEN_LOAD_VTABLE(value_obj));
}

static gboolean major_block_is_evacuating(MSBlockInfo *block) {
  if (evacuate_block_obj_sizes[block->obj_size_index] && !block->has_pinned &&
      !block->is_to_space)
    return TRUE;
  return FALSE;
}

#define MS_MARK_OBJECT_AND_ENQUEUE(obj, desc, block, queue)                    \
  do {                                                                         \
    int __word, __bit;                                                         \
    MS_CALC_MARK_BIT(__word, __bit, (obj));                                    \
    SGEN_ASSERT(9, MS_OBJ_ALLOCED((obj), (block)), "object %p not allocated",  \
                obj);                                                          \
    if (!MS_MARK_BIT((block), __word, __bit)) {                                \
      MS_SET_MARK_BIT((block), __word, __bit);                                 \
      if (sgen_gc_descr_has_references(desc))                                  \
        GRAY_OBJECT_ENQUEUE_SERIAL((queue), (obj), (desc));                    \
      sgen_binary_protocol_mark((obj), (gpointer)SGEN_LOAD_VTABLE((obj)),      \
                                sgen_safe_object_get_size((obj)));             \
      INC_NUM_MAJOR_OBJECTS_MARKED();                                          \
    }                                                                          \
  } while (0)
#define MS_MARK_OBJECT_AND_ENQUEUE_PAR(obj, desc, block, queue)                \
  do {                                                                         \
    int __word, __bit;                                                         \
    gboolean first;                                                            \
    MS_CALC_MARK_BIT(__word, __bit, (obj));                                    \
...
static void pin_major_object(GCObject *obj, SgenGrayQueue *queue) {
  MSBlockInfo *block;
...
static long long major_get_and_reset_num_major_objects_marked(void) {
#ifdef SGEN_COUNT_NUMBER_OF_MAJOR_OBJECTS_MARKED
...
/* gcc 4.2.1 from xcode4 crashes on sgen_card_table_get_card_address () when
 * this is enabled */
...
#define COPY_OR_MARK_FUNCTION_NAME                                             \
  major_copy_or_mark_object_concurrent_no_evacuation
...
#define COPY_OR_MARK_CONCURRENT
#define COPY_OR_MARK_FUNCTION_NAME                                             \
  major_copy_or_mark_object_concurrent_par_no_evacuation
...
#define SCAN_PTR_FIELD_FUNCTION_NAME                                           \
  major_scan_ptr_field_concurrent_with_evacuation
#define DRAIN_GRAY_STACK_FUNCTION_NAME                                         \
  drain_gray_stack_concurrent_with_evacuation
#include "sgen-marksweep-drain-gray-stack.h"

#define COPY_OR_MARK_PARALLEL
#define COPY_OR_MARK_CONCURRENT_WITH_EVACUATION
#define COPY_OR_MARK_FUNCTION_NAME                                             \
  major_copy_or_mark_object_concurrent_par_with_evacuation
#define SCAN_OBJECT_FUNCTION_NAME                                              \
...
#define DRAIN_GRAY_STACK_FUNCTION_NAME                                         \
  drain_gray_stack_concurrent_par_with_evacuation
#include "sgen-marksweep-drain-gray-stack.h"

static gboolean major_is_evacuating(void) {
  int i;
  for (i = 0; i < num_block_obj_sizes; ++i) {
...
static gboolean drain_gray_stack(SgenGrayQueue *queue) {
...
static gboolean drain_gray_stack_par(SgenGrayQueue *queue) {
  if (major_is_evacuating())
...
static gboolean drain_gray_stack_concurrent(SgenGrayQueue *queue) {
  if (major_is_evacuating())
...
static gboolean drain_gray_stack_concurrent_par(SgenGrayQueue *queue) {
  if (major_is_evacuating())
...
static void major_copy_or_mark_object_canonical(GCObject **ptr,
                                                SgenGrayQueue *queue) {
  major_copy_or_mark_object_with_evacuation(ptr, *ptr, queue);
}

static void
major_copy_or_mark_object_concurrent_canonical(GCObject **ptr,
                                               SgenGrayQueue *queue) {
  major_copy_or_mark_object_concurrent_with_evacuation(ptr, *ptr, queue);
}

static void
major_copy_or_mark_object_concurrent_par_canonical(GCObject **ptr,
                                                   SgenGrayQueue *queue) {
  major_copy_or_mark_object_concurrent_par_with_evacuation(ptr, *ptr, queue);
}

static void
major_copy_or_mark_object_concurrent_finish_canonical(GCObject **ptr,
                                                      SgenGrayQueue *queue) {
  major_copy_or_mark_object_with_evacuation(ptr, *ptr, queue);
}

static void major_copy_or_mark_object_concurrent_par_finish_canonical(
    GCObject **ptr, SgenGrayQueue *queue) {
  major_copy_or_mark_object_par_with_evacuation(ptr, *ptr, queue);
}

static void mark_pinned_objects_in_block(MSBlockInfo *block, size_t first_entry,
                                         size_t last_entry,
                                         SgenGrayQueue *queue) {
  void **entry, **end;
  int last_index = -1;
  ...
  for (; entry < end; ++entry) {
    ...
    GCObject *obj;
    SGEN_ASSERT(9, index >= 0 && index < MS_BLOCK_FREE / block->obj_size,
                "invalid object %p index %d max-index %d", *entry, index,
                (int)(MS_BLOCK_FREE / block->obj_size));
    if (index == last_index)
      continue;
  ...
  /*
   * There might have been potential pinning "pointers" into this block, but
   * none of them pointed to occupied slots, in which case we don't have to pin
   * the block.
   */
  if (last_index >= 0)
    block->has_pinned = TRUE;
...
static void sweep_block_for_size(MSBlockInfo *block, int count, int obj_size) {
  int obj_index;
  void *obj = MS_BLOCK_OBJ_FOR_SIZE(block, 0, obj_size);

  for (obj_index = 0; obj_index < count;
       ++obj_index, obj = (void *)((mword)obj + obj_size)) {
    int word, bit;
...
static gboolean try_set_block_state(MSBlockInfo *block, gint32 new_state,
  ...
  if (success)
    sgen_binary_protocol_block_set_state(block, ms_block_size, old_state,
                                         new_state);
  return success;
}

static void set_block_state(MSBlockInfo *block, gint32 new_state,
                            gint32 expected_state) {
  SGEN_ASSERT(6, block->state == expected_state,
              "Block state incorrect before set");
  block->state = new_state;
  sgen_binary_protocol_block_set_state(block, ms_block_size, expected_state,
                                       new_state);
}
 ...
 *
 * Sweeping means iterating through the block's slots and building the free-list
 * from the unmarked ones.  They will also be zeroed.  The mark bits will be
 * reset.
 */
static gboolean sweep_block(MSBlockInfo *block) {
  int count;
...
retry:
  ...
  case BLOCK_STATE_CHECKING:
    SGEN_ASSERT(0, FALSE,
                "How did we get to sweep a block that's being marked or being "
                "checked?");
    goto retry;
  case BLOCK_STATE_SWEEPING:
    /* FIXME: Do this more elegantly */
    ...
    goto retry;
  case BLOCK_STATE_NEED_SWEEPING:
    if (!try_set_block_state(block, BLOCK_STATE_SWEEPING,
                             BLOCK_STATE_NEED_SWEEPING))
      goto retry;
    break;
  default:
  ...
  SGEN_ASSERT(6, block->state == BLOCK_STATE_SWEEPING,
  ...
  /* Use inline instances specialized to constant sizes, this allows the
   * compiler to replace the memset calls with inline code */
...
static int bitcount(mword d) {
  int count = 0;

#if defined(__GNUC__) && !defined(HOST_WIN32)
  // The builtins do work on Win32, but can cause a not worthwhile runtime
  // dependency. See https://github.com/mono/mono/pull/14248.
  if (sizeof(mword) == 8)
...
static void sgen_worker_clear_free_block_lists(WorkerData *worker) {
  int i, j;
...
static void sgen_worker_clear_free_block_lists_evac(WorkerData *worker) {
  int i, j;
  ...
  for (i = 0; i < MS_BLOCK_TYPE_MAX; i++) {
    for (j = 0; j < num_block_obj_sizes; j++) {
      if (((MSBlockInfo ***)worker->free_block_lists)[i][j])
        SGEN_ASSERT(
            0, !((MSBlockInfo ***)worker->free_block_lists)[i][j]->next_free,
...
/*
 * If `wait` is TRUE and the block is currently being checked, this function
 * will wait until the checking has finished.
 *
 * Returns whether the block is still there.  If `wait` is FALSE, the return
 * value will not be correct, i.e. must not be used.
 */
static gboolean ensure_block_is_checked_for_sweeping(guint32 block_index,
                                                     gboolean wait,
                                                     gboolean *have_checked) {
  ...
  void *tagged_block;
  MSBlockInfo *block;
  volatile gpointer *block_slot =
      sgen_array_list_get_slot(&allocated_blocks, block_index);

  SGEN_ASSERT(6, sweep_in_progress(),
              "Why do we call this function if there's no sweep in progress?");
...
retry:
  ...
  if (SGEN_CAS_PTR(block_slot, BLOCK_TAG_CHECKING(tagged_block),
  ...
  if (!sweep_in_progress()) {
    SGEN_ASSERT(6,
                block_state != BLOCK_STATE_SWEEPING &&
                    block_state != BLOCK_STATE_CHECKING,
                "Invalid block state.");
    if (!lazy_sweep)
      SGEN_ASSERT(6, block_state != BLOCK_STATE_NEED_SWEEPING,
                  "Invalid block state.");
  }
  ...
  SGEN_ASSERT(6, block->state == BLOCK_STATE_MARKING,
  ...
  if (have_live) {
    ...
    if (have_free) {
      MSBlockInfo *volatile *free_blocks =
          FREE_BLOCKS(block->pinned, block->has_references);
     ...
     */
    SGEN_ASSERT(6, block_index < allocated_blocks.next_slot,
                "How did the number of blocks shrink?");
    SGEN_ASSERT(6, *block_slot == BLOCK_TAG_CHECKING(tagged_block),
                "How did the block move?");

    sgen_binary_protocol_empty(MS_BLOCK_OBJ(block, 0),
                               (char *)MS_BLOCK_OBJ(block, count) -
                                   (char *)MS_BLOCK_OBJ(block, 0));
    ms_free_block(block);
...
static void sweep_blocks_job_func(void *thread_data_untyped,
                                  SgenThreadPoolJob *job) {
  volatile gpointer *slot;
  MSBlockInfo *bl;
...
static void sweep_job_func(void *thread_data_untyped, SgenThreadPoolJob *job) {
  guint32 block_index;
  guint32 num_blocks = num_major_sections_before_sweep;

  SGEN_ASSERT(0, sweep_in_progress(), "Sweep thread called with wrong state");
  SGEN_ASSERT(0, num_blocks <= allocated_blocks.next_slot,
              "How did we lose blocks?");

  /*
   * We traverse the block array from high to low.  Nursery collections will
   * have to cooperate with the sweep thread to finish sweeping, and they will
   * traverse from low to high, to avoid constantly colliding on the same
   * blocks.
   */
  ...
  if (SGEN_MAX_ASSERT_LEVEL >= 6) {
    for (block_index = num_blocks; block_index < allocated_blocks.next_slot;
         ++block_index) {
      MSBlockInfo *block = BLOCK_UNTAG(
          *sgen_array_list_get_slot(&allocated_blocks, block_index));
      SGEN_ASSERT(6, block && block->state == BLOCK_STATE_SWEPT,
  ...
  if (concurrent_sweep && lazy_sweep) {
    sweep_blocks_job = sgen_thread_pool_job_alloc(
        "sweep_blocks", sweep_blocks_job_func, sizeof(SgenThreadPoolJob));
...
static void major_sweep(void) {
  ...
  if (concurrent_sweep) {
    sweep_job = sgen_thread_pool_job_alloc("sweep", sweep_job_func,
...
static gboolean major_have_swept(void) {
...
static void count_nonpinned_callback(GCObject *obj, size_t size, void *data) {
...
static void count_pinned_callback(GCObject *obj, size_t size, void *data) {
  GCVTable vtable = SGEN_LOAD_VTABLE(obj);
...
static G_GNUC_UNUSED void count_ref_nonref_objs(void) {
  int total;
  ...
  major_iterate_objects(ITERATE_OBJECTS_SWEEP_NON_PINNED,
                        count_nonpinned_callback, NULL);
  major_iterate_objects(ITERATE_OBJECTS_SWEEP_PINNED, count_pinned_callback,
                        NULL);

  total = count_pinned_nonref + count_nonpinned_nonref + count_pinned_ref +
          count_nonpinned_ref;

  g_print("ref: %d pinned %d non-pinned   non-ref: %d pinned %d non-pinned  -- "
          " %.1f\n",
          count_pinned_ref, count_nonpinned_ref, count_pinned_nonref,
          count_nonpinned_nonref,
          (count_pinned_nonref + count_nonpinned_nonref) * 100.0 / total);
}

static int ms_calculate_block_obj_sizes(double factor, int *arr) {
  double target_size;
  int num_sizes = 0;
  int last_size = 0;
  ...
  for (int size = SGEN_CLIENT_MINIMUM_OBJECT_SIZE;
       size <= 4 * SGEN_CLIENT_MINIMUM_OBJECT_SIZE; size += SGEN_ALLOC_ALIGN) {
    if (arr)
      arr[num_sizes] = size;
    ++num_sizes;
    last_size = size;
  ...
  do {
    ...
    int size = MIN((MS_BLOCK_FREE / target_count) & ~(SGEN_ALLOC_ALIGN - 1),
...
static void major_start_nursery_collection(void) {
#ifdef MARKSWEEP_CONSISTENCY_CHECK
  consistency_check();
#endif
  ...
  /* Compact the block list if it hasn't been compacted in a while and nobody is
   * using it */
  if (compact_blocks && !sweep_in_progress() && !sweep_blocks_job &&
     ...
     * balanced sections for parallel modes. We compact as soon as possible
     * after sweep.
     */
...
static void major_finish_nursery_collection(void) {
#ifdef MARKSWEEP_CONSISTENCY_CHECK
  consistency_check();
#endif
}

static int block_usage_comparer(const void *bl1, const void *bl2) {
...
static void sgen_evacuation_freelist_blocks(MSBlockInfo *volatile *block_list,
                                            int size_index) {
   ...
   */
  evacuated_blocks = (MSBlockInfo **)sgen_alloc_internal_dynamic(
      sizeof(MSBlockInfo *) * num_blocks, INTERNAL_MEM_TEMPORARY, TRUE);
  ...
  sgen_qsort(evacuated_blocks, num_blocks, sizeof(gpointer),
  ...
  *prev = NULL;
  ...
  sgen_free_internal_dynamic(evacuated_blocks,
                             sizeof(MSBlockInfo *) * num_blocks,
                             INTERNAL_MEM_TEMPORARY);
}

static void major_start_major_collection(void) {
  MSBlockInfo *block;
  int i;
  ...
  /*
   * Clear the free lists for block sizes where we do evacuation.  For those
   * block sizes we will have to allocate new blocks.
   */
  for (i = 0; i < num_block_obj_sizes; ++i) {
    if (!evacuate_block_obj_sizes[i])
      continue;
    ...
    sgen_evacuation_freelist_blocks(&free_block_lists[MS_BLOCK_FLAG_REFS][i],
                                    i);
  }

  /* We expect workers to have very few blocks on the freelist, just evacuate
   * them */
  sgen_workers_foreach(GENERATION_NURSERY,
  ...
  FOREACH_BLOCK_NO_LOCK(block) {
    if (lazy_sweep && !concurrent_sweep)
      sweep_block(block);
    SGEN_ASSERT(0, block->state == BLOCK_STATE_SWEPT,
...
static void major_finish_major_collection(ScannedObjectCounts *counts) {
#ifdef SGEN_HEAVY_BINARY_PROTOCOL
  if (sgen_binary_protocol_is_enabled()) {
...
static int compare_pointers(const void *va, const void *vb) {
 ...
 */
static void major_free_swept_blocks(size_t section_reserve) {
  SGEN_ASSERT(0, sweep_state == SWEEP_STATE_SWEPT,
              "Sweeping must have finished before freeing blocks");

#if defined(HOST_WIN32) || defined(HOST_ORBIS) || defined(HOST_WASM)
  /*
   * sgen_free_os_memory () asserts in mono_vfree () because windows doesn't
   * like freeing the middle of a VirtualAlloc ()-ed block.
   */
  return;
#endif

  {
    ...
    empty_block_arr = (void **)sgen_alloc_internal_dynamic(
        sizeof(void *) * num_empty_blocks_orig, INTERNAL_MEM_MS_BLOCK_INFO_SORT,
        FALSE);
    if (!empty_block_arr)
      goto fallback;
    ...
    sgen_qsort(empty_block_arr, num_empty_blocks, sizeof(void *),
               compare_pointers);
    ...
    while (num_empty_blocks > section_reserve && num_blocks > 1) {
      ...
      for (i = 0; i < arr_length; ++i) {
        ...
        if (d + 1 - first == num_blocks) {
           ...
           */
          int j;
          sgen_free_os_memory(empty_block_arr[first],
                              ms_block_size * num_blocks, SGEN_ALLOC_HEAP,
                              MONO_MEM_ACCOUNT_SGEN_MARKSWEEP);
          for (j = first; j <= d; ++j)
    ...
    *rebuild_next = NULL;
    ...
    /* free array */
    sgen_free_internal_dynamic(empty_block_arr,
                               sizeof(void *) * num_empty_blocks_orig,
                               INTERNAL_MEM_MS_BLOCK_INFO_SORT);
  }

  SGEN_ASSERT(0, num_empty_blocks >= 0,
              "we freed more blocks than we had in the first place?");

fallback:
  /*
   * This is our threshold.  If there's not more empty than used blocks, we
   * won't release uncontiguous blocks, in fear of fragmenting the address
   * space.
   */
...
static void major_pin_objects(SgenGrayQueue *queue) {
  MSBlockInfo *block;

  FOREACH_BLOCK_NO_LOCK(block) {
    size_t first_entry, last_entry;
    SGEN_ASSERT(6, block_is_swept_or_marking(block),
                "All blocks must be swept when we're pinning.");
    sgen_find_optimized_pin_queue_area(
        MS_BLOCK_FOR_BLOCK_INFO(block) + MS_BLOCK_SKIP,
        MS_BLOCK_FOR_BLOCK_INFO(block) + ms_block_size, &first_entry,
        &last_entry);
...
static void major_init_to_space(void) {}

static void major_report_pinned_memory_usage(void) { g_assert_not_reached(); }

static gint64 major_get_used_size(void) {
  gint64 size = 0;
  MSBlockInfo *block;

  /*
   * We're holding the GC lock, but the sweep thread might be running.  Make
   * sure it's finished, then we can iterate over the block array.
   */
  major_finish_sweep_checking();
...
static size_t get_num_major_sections(void) { return num_major_sections; }

/*
 * Returns the number of bytes in blocks that were present when the last sweep
 * was initiated, and were not freed during the sweep.  They are the basis for
 * calculating the allowance.
 */
static size_t get_bytes_survived_last_sweep(void) {
  SGEN_ASSERT(0, sweep_state == SWEEP_STATE_SWEPT,
...
static gboolean major_handle_gc_param(const char *opt) {
...
static void major_print_gc_param_usage(void) {
  fprintf(stderr, ""
                  "  evacuation-threshold=P (where P is a percentage, an "
                  "integer in 0-100)\n"
                  "  (no-)lazy-sweep\n"
                  "  (no-)concurrent-sweep\n");
}
...
/*
 * This callback is used to clear cards, move cards to the shadow table and do
 * counting.
 */
static void major_iterate_block_ranges(sgen_cardtable_block_callback callback) {
  MSBlockInfo *block;
  gboolean has_references;
 ...
 */
static guint8 *initial_skip_card(guint8 *card_data) {
...
#define MS_BLOCK_OBJ_INDEX_FAST(o, b, os)                                      \
  (((char *)(o) - ((b) + MS_BLOCK_SKIP)) / (os))
#define MS_BLOCK_OBJ_FAST(b, os, i) ((b) + MS_BLOCK_SKIP + (os) * (i))
#define MS_OBJ_ALLOCED_FAST(o, b)                                              \
  (*(void **)(o) &&                                                            \
   (*(char **)(o) < (b) || *(char **)(o) >= (b) + ms_block_size))

static void scan_card_table_for_block(MSBlockInfo *block,
   ...
   * Cards aliasing happens in powers of two, so as long as major blocks are
   * aligned to their sizes, they won't overflow the cardtable overlap modulus.
   */
  if (scan_type & CARDTABLE_SCAN_MOD_UNION) {
    ...
    if (scan_type == CARDTABLE_SCAN_MOD_UNION_PRECLEAN) {
      sgen_card_table_preclean_mod_union(card_data, cards_preclean,
                                         CARDS_PER_BLOCK);
    ...
    card_data = card_base =
        sgen_card_table_get_card_scan_address((mword)block_start);
#else
    if (!sgen_card_table_get_card_data(cards_copy, (mword)block_start,
                                       CARDS_PER_BLOCK))
      return;
  ...
  while (card_data < card_data_end) {
    ...
    if (card_index <= (MS_BLOCK_SKIP >> CARD_BITS))
      ...
      first_object_index =
          MS_BLOCK_OBJ_INDEX_FAST(start, block_start, block_obj_size);

    obj = first_obj = (char *)MS_BLOCK_OBJ_FAST(block_start, block_obj_size,
                                                first_object_index);
    ...
    while (obj < end) {
      ...
      if (small_objects) {
        ...
        sgen_cardtable_scan_object(object, block_obj_size, card_base + offset,
                                   ctx);
      }
    next_object:
      obj += block_obj_size;
...
static void major_scan_card_table(CardTableScanType scan_type,
                                  ScanCopyContext ctx, int job_index,
                                  int job_split_count, int block_count) {
  MSBlockInfo *block;
  ...
  if (scan_type != CARDTABLE_SCAN_GLOBAL)
    SGEN_ASSERT(0, !sweep_in_progress(),
                "Sweep should be finished when we scan mod union card table");
  was_sweeping = sweep_in_progress();

  sgen_binary_protocol_major_card_table_scan_start(
      sgen_timestamp(), scan_type & CARDTABLE_SCAN_MOD_UNION);
  FOREACH_BLOCK_RANGE_HAS_REFERENCES_NO_LOCK(block, first_block, last_block,
                                             index, has_references) {
#ifdef PREFETCH_CARDS
    int prefetch_index = index + 6;
    if (prefetch_index < allocated_blocks.next_slot) {
      MSBlockInfo *prefetch_block = BLOCK_UNTAG(
          *sgen_array_list_get_slot(&allocated_blocks, prefetch_index));
      PREFETCH_READ(prefetch_block);
      if (scan_type == CARDTABLE_SCAN_GLOBAL) {
        guint8 *prefetch_cards = sgen_card_table_get_card_scan_address(
            (mword)MS_BLOCK_FOR_BLOCK_INFO(prefetch_block));
    ...
    if (scan_type == CARDTABLE_SCAN_GLOBAL) {
      gpointer *card_start = (gpointer *)sgen_card_table_get_card_scan_address(
          (mword)MS_BLOCK_FOR_BLOCK_INFO(block));
  ...
  sgen_binary_protocol_major_card_table_scan_end(
      sgen_timestamp(), scan_type & CARDTABLE_SCAN_MOD_UNION);
}

static void major_count_cards(long long *num_total_cards,
  ...
  FOREACH_BLOCK_HAS_REFERENCES_NO_LOCK(block, has_references) {
    guint8 *cards = sgen_card_table_get_card_scan_address(
        (mword)MS_BLOCK_FOR_BLOCK_INFO(block));
    int i;
...
static void update_cardtable_mod_union(void) {
  MSBlockInfo *block;

  FOREACH_BLOCK_NO_LOCK(block) {
    gpointer *card_start = (gpointer *)sgen_card_table_get_card_address(
        (mword)MS_BLOCK_FOR_BLOCK_INFO(block));
    gboolean has_dirty_cards = FALSE;
    int i;
    for (i = 0; i < CARDS_PER_BLOCK / sizeof(gpointer); i++) {
    ...
    if (has_dirty_cards) {
      size_t num_cards;
      guint8 *mod_union = get_cardtable_mod_union_for_block(block, TRUE);
      sgen_card_table_update_mod_union(
          mod_union, MS_BLOCK_FOR_BLOCK_INFO(block), ms_block_size, &num_cards);
      SGEN_ASSERT(6, num_cards == CARDS_PER_BLOCK,
...
static void post_param_init(SgenMajorCollector *collector) {
 ...
 */
static void sgen_init_block_free_lists(gpointer *list_p) {
  int i;
  MSBlockInfo ***worker_free_blocks =
      (MSBlockInfo ***)mono_native_tls_get_value(worker_block_free_list_key);
  ...
  worker_free_blocks = (MSBlockInfo ***)sgen_alloc_internal_dynamic(
      sizeof(MSBlockInfo **) * MS_BLOCK_TYPE_MAX, INTERNAL_MEM_MS_TABLES, TRUE);

  for (i = 0; i < MS_BLOCK_TYPE_MAX; i++)
    worker_free_blocks[i] = (MSBlockInfo **)sgen_alloc_internal_dynamic(
        sizeof(MSBlockInfo *) * num_block_obj_sizes, INTERNAL_MEM_MS_TABLES,
        TRUE);
...
static void sgen_marksweep_init_internal(SgenMajorCollector *collector,
  ...
  sgen_register_fixed_internal_mem_type(INTERNAL_MEM_MS_BLOCK_INFO,
  ...
  num_block_obj_sizes =
      ms_calculate_block_obj_sizes(MS_BLOCK_OBJ_SIZE_FACTOR, NULL);
  block_obj_sizes = (int *)sgen_alloc_internal_dynamic(
      sizeof(int) * num_block_obj_sizes, INTERNAL_MEM_MS_TABLES, TRUE);
  ms_calculate_block_obj_sizes(MS_BLOCK_OBJ_SIZE_FACTOR, block_obj_sizes);

  evacuate_block_obj_sizes = (gboolean *)sgen_alloc_internal_dynamic(
      sizeof(gboolean) * num_block_obj_sizes, INTERNAL_MEM_MS_TABLES, TRUE);
  for (i = 0; i < num_block_obj_sizes; ++i)
    evacuate_block_obj_sizes[i] = FALSE;

  sweep_slots_available = (size_t *)sgen_alloc_internal_dynamic(
      sizeof(size_t) * num_block_obj_sizes, INTERNAL_MEM_MS_TABLES, TRUE);
  sweep_slots_used = (size_t *)sgen_alloc_internal_dynamic(
      sizeof(size_t) * num_block_obj_sizes, INTERNAL_MEM_MS_TABLES, TRUE);
  sweep_num_blocks = (size_t *)sgen_alloc_internal_dynamic(
      sizeof(size_t) * num_block_obj_sizes, INTERNAL_MEM_MS_TABLES, TRUE);
  ...
  */
  ...
  for (i = 0; i < MS_BLOCK_TYPE_MAX; ++i)
    free_block_lists[i] = (MSBlockInfo * volatile *)sgen_alloc_internal_dynamic(
        sizeof(MSBlockInfo *) * num_block_obj_sizes, INTERNAL_MEM_MS_TABLES,
        TRUE);
  ...
  mono_counters_register("# major blocks allocated",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_major_blocks_alloced);
  mono_counters_register("# major blocks freed",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_major_blocks_freed);
  mono_counters_register("# major blocks lazy swept",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_major_blocks_lazy_swept);
  mono_counters_register("# major blocks freed ideally",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_major_blocks_freed_ideal);
  mono_counters_register("# major blocks freed less ideally",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_major_blocks_freed_less_ideal);
  mono_counters_register("# major blocks freed individually",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_major_blocks_freed_individual);
  mono_counters_register("# major blocks allocated less ideally",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
  ...
  collector->get_and_reset_num_major_objects_marked =
  ...
  if (is_concurrent) {
    ...
    collector->get_cardtable_mod_union_for_reference =
  ...
  collector->major_ops_serial.copy_or_mark_object =
      major_copy_or_mark_object_canonical;
  collector->major_ops_serial.scan_object = major_scan_object_with_evacuation;
  collector->major_ops_serial.scan_ptr_field =
      major_scan_ptr_field_with_evacuation;
  collector->major_ops_serial.drain_gray_stack = drain_gray_stack;
#ifndef DISABLE_SGEN_MAJOR_MARKSWEEP_CONC
  if (is_concurrent) {
    collector->major_ops_concurrent_start.copy_or_mark_object =
        major_copy_or_mark_object_concurrent_canonical;
    collector->major_ops_concurrent_start.scan_object =
        major_scan_object_concurrent_with_evacuation;
    collector->major_ops_concurrent_start.scan_vtype =
        major_scan_vtype_concurrent_with_evacuation;
    collector->major_ops_concurrent_start.scan_ptr_field =
        major_scan_ptr_field_concurrent_with_evacuation;
    collector->major_ops_concurrent_start.drain_gray_stack =
        drain_gray_stack_concurrent;

    collector->major_ops_concurrent_finish.copy_or_mark_object =
        major_copy_or_mark_object_concurrent_finish_canonical;
    collector->major_ops_concurrent_finish.scan_object =
        major_scan_object_with_evacuation;
    collector->major_ops_concurrent_finish.scan_vtype =
        major_scan_vtype_with_evacuation;
    collector->major_ops_concurrent_finish.scan_ptr_field =
        major_scan_ptr_field_with_evacuation;
    collector->major_ops_concurrent_finish.drain_gray_stack = drain_gray_stack;

    if (is_parallel) {
      collector->major_ops_conc_par_start.copy_or_mark_object =
          major_copy_or_mark_object_concurrent_par_canonical;
      collector->major_ops_conc_par_start.scan_object =
          major_scan_object_concurrent_par_with_evacuation;
      collector->major_ops_conc_par_start.scan_vtype =
          major_scan_vtype_concurrent_par_with_evacuation;
      collector->major_ops_conc_par_start.scan_ptr_field =
          major_scan_ptr_field_concurrent_par_with_evacuation;
      collector->major_ops_conc_par_start.drain_gray_stack =
          drain_gray_stack_concurrent_par;

      collector->major_ops_conc_par_finish.copy_or_mark_object =
          major_copy_or_mark_object_concurrent_par_finish_canonical;
      collector->major_ops_conc_par_finish.scan_object =
          major_scan_object_par_with_evacuation;
      collector->major_ops_conc_par_finish.scan_vtype =
          major_scan_vtype_par_with_evacuation;
      collector->major_ops_conc_par_finish.scan_ptr_field =
...
#ifdef HEAVY_STATISTICS
  mono_counters_register("Optimized copy", MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_copy);
  mono_counters_register("Optimized copy nursery",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_copy_nursery);
  mono_counters_register("Optimized copy nursery forwarded",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_copy_nursery_forwarded);
  mono_counters_register("Optimized copy nursery pinned",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_copy_nursery_pinned);
  mono_counters_register("Optimized copy major",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_copy_major);
  mono_counters_register("Optimized copy major small fast",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_copy_major_small_fast);
  mono_counters_register("Optimized copy major small slow",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_copy_major_small_slow);
  mono_counters_register("Optimized copy major small evacuate",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_copy_major_small_evacuate);
  mono_counters_register("Optimized copy major large",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_copy_major_large);
  mono_counters_register("Optimized major scan",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_major_scan);
  mono_counters_register("Optimized major scan no refs",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_optimized_major_scan_no_refs);

  mono_counters_register("Gray stack drain loops",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_drain_loops);
  mono_counters_register("Gray stack prefetch fills",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_drain_prefetch_fills);
  mono_counters_register("Gray stack prefetch failures",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
  ...
  SGEN_ASSERT(0, SGEN_MAX_SMALL_OBJ_SIZE <= MS_BLOCK_FREE / 2,
              "MAX_SMALL_OBJ_SIZE must be at most MS_BLOCK_FREE / 2");
  ...
  if (concurrent_sweep)
    sweep_pool_context =
        sgen_thread_pool_create_context(1, NULL, NULL, NULL, NULL, NULL);
#endif
}
