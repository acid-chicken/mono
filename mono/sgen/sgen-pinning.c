...
void sgen_pinning_init(void) { mono_os_mutex_init(&pin_queue_mutex); }
...
gboolean sgen_find_optimized_pin_queue_area(void *start, void *end,
                                            size_t *first_out,
                                            size_t *last_out) {
  size_t first = sgen_pointer_queue_search(&pin_queue, start);
  size_t last = sgen_pointer_queue_search(&pin_queue, end);
  SGEN_ASSERT(0, last == pin_queue.next_slot || pin_queue.data[last] >= end,
...
void sgen_find_section_pin_queue_start_end(GCMemSection *section) {
  SGEN_LOG(6, "Pinning from section %p (%p-%p)", section, section->data,
           section->end_data);

  sgen_find_optimized_pin_queue_area(section->data, section->end_data,
                                     &section->pin_queue_first_entry,
                                     &section->pin_queue_last_entry);

  SGEN_LOG(6, "Found %" G_GSIZE_FORMAT "d pinning addresses in section %p",
           section->pin_queue_last_entry - section->pin_queue_first_entry,
           section);
}
...
void sgen_pinning_trim_queue_to_section(GCMemSection *section) {
  SGEN_ASSERT(
      0, section->pin_queue_first_entry == 0,
 ...
 *
 * After collecting potential pin entries and sorting the array, this is what it
 * looks like:
 *
 * +--------------------+---------------------------------------------+--------------------+
 * | major heap entries |               nursery entries               | major
 * heap entries |
 * +--------------------+---------------------------------------------+--------------------+
 *
 * Of course there might not be major heap entries before and/or after the
 * nursery entries, depending on where the major heap sections are in the
 * address space, and whether there were any potential pointers there.
 *
 * When we pin nursery objects, we compact the nursery part of the pin array,
 * which leaves discarded entries after the ones that actually pointed to
 * nursery objects:
 *
 * +--------------------+-----------------+---------------------------+--------------------+
 * | major heap entries | nursery entries | discarded nursery entries | major
 * heap entries |
 * +--------------------+-----------------+---------------------------+--------------------+
 *
 * When, due to being out of memory, we late pin more objects, the pin array
 * looks like this:
 *
 * +--------------------+-----------------+---------------------------+--------------------+--------------+
 * | major heap entries | nursery entries | discarded nursery entries | major
 * heap entries | late entries |
 * +--------------------+-----------------+---------------------------+--------------------+--------------+
 *
 * This function gets rid of the discarded nursery entries by nulling them out.
 * Note that we can late pin objects not only in the nursery but also in the
 * major heap, which happens when evacuation fails.
 */
void sgen_pin_queue_clear_discarded_entries(GCMemSection *section,
                                            size_t max_pin_slot) {
...
/* reduce the info in the pin queue, removing duplicate pointers and sorting
 * them */
void sgen_optimize_pin_queue(void) { sgen_pointer_queue_sort_uniq(&pin_queue); }

size_t sgen_get_pinned_count(void) { return pin_queue.next_slot; }
...
void sgen_dump_pin_queue(void) {
  ...
  for (i = 0; i < last_num_pinned; ++i) {
    ...
    SGEN_LOG(3, "Bastard pinning obj %p (%s), size: %ld", ptr,
...
void sgen_cement_init(gboolean enabled) { cement_enabled = enabled; }
...
void sgen_cement_force_pinned(void) {
  ...
  for (i = 0; i < SGEN_CEMENT_HASH_SIZE; i++) {
    ...
    if (cement_hash[i].count < SGEN_CEMENT_THRESHOLD)
      continue;
    SGEN_ASSERT(0, !cement_hash[i].forced,
    ...
    if (index == pin_queue.next_slot)
      continue;
    SGEN_ASSERT(
        0, pin_queue.data[index] >= (gpointer)obj,
        "Binary search should return a pointer greater than the search target");
    if (pin_queue.data[index] <
        (gpointer)((char *)obj + sgen_safe_object_get_size(obj)))
...
gboolean sgen_cement_lookup_or_register(GCObject *obj) {
  ...
  if (!hash[i].obj) {
    GCObject *old_obj;
    old_obj =
        (GCObject *)mono_atomic_cas_ptr((gpointer *)&hash[i].obj, obj, NULL);
  ...
  if (mono_atomic_inc_i32((gint32 *)&hash[i].count) == SGEN_CEMENT_THRESHOLD) {
    SGEN_ASSERT(9, sgen_get_current_collection_generation() >= 0,
                "We can only cement objects when we're in a collection pause.");
    SGEN_ASSERT(9, SGEN_OBJECT_IS_PINNED(obj),
...
static void pin_from_hash(CementHashEntry *hash, gboolean has_been_reset) {
  int i;
  for (i = 0; i < SGEN_CEMENT_HASH_SIZE; ++i) {
    if (!hash[i].count)
      continue;
    ...
    if (has_been_reset)
      SGEN_ASSERT(5, hash[i].count >= SGEN_CEMENT_THRESHOLD,
...
void sgen_pin_cemented_objects(void) { pin_from_hash(cement_hash, TRUE); }
