...
// If no symbols in an object file in a static library are referenced, its
// exports will not be exported. There are a few workarounds:
...
static void get_default_field_value(MonoDomain *domain, MonoClassField *field,
                                    void *value,
                                    MonoStringHandleOut string_handle,
                                    MonoError *error);

static void mono_ldstr_metadata_sig(MonoDomain *domain, const char *sig,
                                    MonoStringHandleOut string_handle,
                                    MonoError *error);

static void free_main_args(void);

static char *mono_string_to_utf8_internal(MonoMemPool *mp, MonoImage *image,
                                          MonoString *s, MonoError *error);

static char *mono_string_to_utf8_mp(MonoMemPool *mp, MonoString *s,
                                    MonoError *error);

static void array_full_copy_unchecked_size(MonoArray *src, MonoArray *dest,
                                           MonoClass *klass, uintptr_t size);

/* Class lazy loading functions */
static GENERATE_GET_CLASS_WITH_CACHE(pointer, "System.Reflection", "Pointer") static GENERATE_GET_CLASS_WITH_CACHE(remoting_services, "System.Runtime.Remoting", "RemotingServices") static GENERATE_GET_CLASS_WITH_CACHE(
    unhandled_exception_event_args, "System",
    "UnhandledExceptionEventArgs") static GENERATE_GET_CLASS_WITH_CACHE(sta_thread_attribute,
                                                                        "Syste"
                                                                        "m",
                                                                        "STAThr"
                                                                        "eadAtt"
                                                                        "ribut"
                                                                        "e") static GENERATE_GET_CLASS_WITH_CACHE(activation_services,
                                                                                                                  "System.Runtime.Remoting.Activation",
                                                                                                                  "ActivationServices") static GENERATE_TRY_GET_CLASS_WITH_CACHE(execution_context,
                                                                                                                                                                                 "System.Threading",
                                                                                                                                                                                 "ExecutionContext") static GENERATE_GET_CLASS_WITH_CACHE(asyncresult,
 ...
 */
gboolean mono_runtime_object_init_handle(MonoObjectHandle this_obj,
                                         MonoError *error) {
  ...
  MonoMethod *const method =
      mono_class_get_method_from_name_checked(klass, ".ctor", 0, 0, error);
  mono_error_assert_msg_ok(error, "Could not lookup zero argument constructor");
  g_assertf(method, "Could not lookup zero argument constructor for class %s",
 ...
 * on error and sets \p error.
 */
gboolean mono_runtime_object_init_checked(MonoObject *this_obj_raw,
                                          MonoError *error) {
  HANDLE_FUNCTION_ENTER();
...
static void mono_type_initialization_lock(void) {
  /* The critical sections protected by this lock in
   * mono_runtime_class_init_full () can block */
  mono_coop_mutex_lock(&type_initialization_section);
}

static void mono_type_initialization_unlock(void) {
  mono_coop_mutex_unlock(&type_initialization_section);
}

static void mono_type_init_lock(TypeInitializationLock *lock) {
  MONO_REQ_GC_NEUTRAL_MODE;

  mono_coop_mutex_lock(&lock->mutex);
}

static void mono_type_init_unlock(TypeInitializationLock *lock) {
...
void mono_thread_set_main(MonoThread *thread) {
  ...
  if (!registered) {
    void *key = thread->internal_thread ? (void *)MONO_UINT_TO_NATIVE_THREAD_ID(
                                              thread->internal_thread->tid)
                                        : NULL;
    MONO_GC_REGISTER_ROOT_SINGLE(main_thread, MONO_ROOT_SOURCE_THREADING, key,
                                 "Thread Main Object");
    registered = TRUE;
  }
...
void mono_type_initialization_init(void) {
  mono_coop_mutex_init_recursive(&type_initialization_section);
  type_initialization_hash = g_hash_table_new(NULL, NULL);
  blocked_thread_hash = g_hash_table_new(NULL, NULL);
  mono_coop_mutex_init(&ldstr_section);
  mono_register_jit_icall(ves_icall_string_alloc, mono_icall_sig_object_int,
                          FALSE);
}

void mono_type_initialization_cleanup(void) {
#if 0
...
static MonoException *mono_get_exception_type_initialization_checked(
    const gchar *type_name, MonoException *inner_raw, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MONO_HANDLE_DCL(MonoException, inner);
  HANDLE_FUNCTION_RETURN_OBJ(
      mono_get_exception_type_initialization_handle(type_name, inner, error));
}
 ...
 */
static MonoException *get_type_init_exception_for_vtable(MonoVTable *vtable) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (domain->type_init_exception_hash)
    ex = (MonoException *)mono_g_hash_table_lookup(
        domain->type_init_exception_hash, klass);
 ...
 */
static gboolean unref_type_lock(TypeInitializationLock *lock) {
  --lock->waiting_count;
 ...
 */
gboolean mono_runtime_run_module_cctor(MonoImage *image, MonoDomain *domain,
                                       MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * \param error set on error
 * \returns TRUE if class constructor \c .cctor has been initialized
 * successfully, or FALSE otherwise and sets \p error.
 */
gboolean mono_runtime_class_init_full(MonoVTable *vtable, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  lock = (TypeInitializationLock *)g_hash_table_lookup(type_initialization_hash,
                                                       vtable);
  if (lock == NULL) {
    ...
    if (mono_domain_get() != domain) {
      ...
      if (!mono_domain_set_fast(domain, FALSE)) {
        ...
        mono_error_set_exception_instance(
            error, mono_get_exception_appdomain_unloaded());
        goto return_false;
      }
    }
    ...
    /* see if the thread doing the initialization is already blocked on this
     * thread */
    gboolean is_blocked = TRUE;
    blocked =
        GUINT_TO_POINTER(MONO_NATIVE_THREAD_ID_TO_UINT(lock->initializing_tid));
    while ((pending_lock = (TypeInitializationLock *)g_hash_table_lookup(
                blocked_thread_hash, blocked))) {
      ...
      blocked = GUINT_TO_POINTER(
          MONO_NATIVE_THREAD_ID_TO_UINT(pending_lock->initializing_tid));
    }
    ++lock->waiting_count;
  ...
  if (do_initialization) {
    ...
    if (!(!exc || (klass_image == mono_defaults.corlib &&
      ...
      MonoException *exc_to_throw =
          mono_get_exception_type_initialization_checked(full_name, exc, error);
      MONO_HANDLE_NEW(MonoException, exc_to_throw);
      g_free(full_name);

      mono_error_assert_ok(error); // We can't recover from this, no way to fail
                                   // a type we can't alloc a failure.
      ...
      mono_domain_lock(domain);
      if (!domain->type_init_exception_hash)
        domain->type_init_exception_hash = mono_g_hash_table_new_type_internal(
            mono_aligned_addr_hash, NULL, MONO_HASH_VALUE_GC,
            MONO_ROOT_SOURCE_DOMAIN, domain,
            "Domain Type Initialization Exception Table");
      mono_g_hash_table_insert_internal(domain->type_init_exception_hash, klass,
                                        exc_to_throw);
      mono_domain_unlock(domain);
    }
     ...
     */
    if (exc &&
        mono_object_class(exc) == mono_defaults.threadabortexception_class) {
      pending_tae = TRUE;
  ...
  if (vtable->init_failed && !pending_tae) {
    /* Either we were the initializing thread or we waited for the
     * initialization */
    mono_error_set_exception_instance(
        error, get_type_init_exception_for_vtable(vtable));
    goto return_false;
  }
return_true:
  ret = TRUE;
...
MonoDomain *mono_vtable_domain_internal(MonoVTable *vtable) {
...
MonoClass *mono_vtable_class_internal(MonoVTable *vtable) {
  return vtable->klass;
}
...
static gboolean release_type_locks(gpointer key, gpointer value,
                                   gpointer user) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  if (mono_native_thread_id_equals(
          lock->initializing_tid,
          MONO_UINT_TO_NATIVE_THREAD_ID(GPOINTER_TO_UINT(user))) &&
      !lock->done) {
    lock->done = TRUE;
    /*
     * Have to set this since it cannot be set by the normal code in
     * mono_runtime_class_init (). In this case, the exception object is not
     * stored, and get_type_init_exception_for_class () needs to be aware of
     * this.
     */
    mono_type_init_lock(lock);
    vtable->init_failed = 1;
...
void mono_release_type_locks(MonoInternalThread *thread) {
  MONO_REQ_GC_UNSAFE_MODE;

  mono_type_initialization_lock();
  g_hash_table_foreach_remove(type_initialization_hash, release_type_locks,
                              GUINT_TO_POINTER(thread->tid));
  mono_type_initialization_unlock();
}

#ifndef DISABLE_REMOTING

static gpointer create_remoting_trampoline(MonoDomain *domain,
                                           MonoMethod *method,
...
void mono_install_callbacks(MonoRuntimeCallbacks *cbs) {
  memcpy(&callbacks, cbs, sizeof(*cbs));
}

MonoRuntimeCallbacks *mono_get_runtime_callbacks(void) { return &callbacks; }

void mono_install_imt_trampoline_builder(MonoImtTrampolineBuilder func) {
  imt_trampoline_builder = func;
}

void mono_set_always_build_imt_trampolines(gboolean value) {
...
gpointer mono_runtime_create_jump_trampoline(MonoDomain *domain,
                                             MonoMethod *method,
  ...
  error_init(error);
  res =
      callbacks.create_jump_trampoline(domain, method, add_sync_wrapper, error);
  return res;
}
 ...
 */
void mono_runtime_free_method(MonoDomain *domain, MonoMethod *method) {
  MONO_REQ_GC_NEUTRAL_MODE
...
#define MONO_OBJECT_HEADER_BITS                                                \
  (MONO_ABI_SIZEOF(MonoObject) / MONO_ABI_SIZEOF(gpointer))

static gsize *compute_class_bitmap(MonoClass *klass, gsize *bitmap, int size,
                                   int offset, int *max_set,
  ...
  if (max_size > size) {
    g_assert(offset <= 0);
    bitmap = (gsize *)g_malloc0((max_size + BITMAP_EL_SIZE - 1) /
                                BITMAP_EL_SIZE * sizeof(gsize));
    size = max_size;
  }

  /* An Ephemeron cannot be marked by sgen */
  if (mono_gc_is_moving() && !static_fields &&
    ...
    while ((field = mono_class_get_fields_internal(p, &iter))) {
      ...
      case MONO_TYPE_VALUETYPE: {
        ...
        if (m_class_has_references(fclass)) {
          ...
          compute_class_bitmap(fclass, bitmap, size,
                               pos - MONO_OBJECT_HEADER_BITS, max_set, FALSE);
        }
        break;
      }
      case MONO_TYPE_I1:
      ...
      default:
        g_error("compute_class_bitmap: Invalid type %x for field %s:%s\n",
                type->type, mono_type_get_full_name(field->parent),
                field->name);
        break;
      }
    }
 ...
 */
gsize *mono_class_compute_bitmap(MonoClass *klass, gsize *bitmap, int size,
                                 int offset, int *max_set,
                                 gboolean static_fields) {
  MONO_REQ_GC_NEUTRAL_MODE;

  return compute_class_bitmap(klass, bitmap, size, offset, max_set,
                              static_fields);
}
...
MonoStringHandle ves_icall_string_alloc_impl(int length, MonoError *error) {
  MonoString *s =
      mono_string_new_size_checked(mono_domain_get(), length, error);
...
void mono_class_compute_gc_descriptor(MonoClass *klass) {
  ...
  if (klass == mono_defaults.string_class) {
    ...
    if (MONO_TYPE_IS_REFERENCE(m_class_get_byval_arg(klass_element_class))) {
      gsize abm = 1;
      gc_descr = mono_gc_make_descr_for_array(
          m_class_get_byval_arg(klass)->type == MONO_TYPE_SZARRAY, &abm, 1,
          sizeof(gpointer));
      /*printf ("new array descriptor: 0x%x for %s.%s\n", class->gc_descr,
          class->name_space, class->name);*/
    } else {
      /* remove the object header */
      bitmap = mono_class_compute_bitmap(
          klass_element_class, default_bitmap, sizeof(default_bitmap) * 8,
          -(int)(MONO_OBJECT_HEADER_BITS), &max_set, FALSE);
      gc_descr = mono_gc_make_descr_for_array(
          m_class_get_byval_arg(klass)->type == MONO_TYPE_SZARRAY, bitmap,
          mono_array_element_size(klass) / sizeof(gpointer),
    ...
    if (count++ > 58)
            return;*/
    bitmap = mono_class_compute_bitmap(
        klass, default_bitmap, sizeof(default_bitmap) * 8, 0, &max_set, FALSE);
    /*
    if (class->gc_descr == MONO_GC_DESCRIPTOR_NULL)
            g_print ("disabling typed alloc (%d) for %s.%s\n", max_set,
    class->name_space, class->name);
    */
    /*printf ("new descriptor: %p 0x%x for %s.%s\n", class->gc_descr, bitmap
     * [0], class->name_space, class->name);*/

    if (m_class_has_weak_fields(klass)) {
      gsize *weak_bitmap = NULL;
      int weak_bitmap_nbits = 0;

      weak_bitmap = (gsize *)mono_class_alloc0(
          klass, m_class_get_instance_size(klass) / sizeof(gsize));
        ...
        for (MonoClass *p = klass; p != NULL; p = m_class_get_parent(p)) {
          ...
          while ((field = mono_class_get_fields_internal(p, &iter))) {
            ...
            if (MONO_TYPE_IS_REFERENCE(field->type) &&
                mono_assembly_is_weak_field(p_image, field_idx + 1)) {
              int pos = field->offset / sizeof(gpointer);
              if (pos + 1 > weak_bitmap_nbits)
                weak_bitmap_nbits = pos + 1;
              weak_bitmap[pos / BITMAP_EL_SIZE] |= ((gsize)1)
                                                   << (pos % BITMAP_EL_SIZE);
            }
          }
        }
      ...
      for (int pos = 0; pos < weak_bitmap_nbits; ++pos) {
        if (weak_bitmap[pos / BITMAP_EL_SIZE] & ((gsize)1)
                                                    << (pos % BITMAP_EL_SIZE)) {
          /* Clear the normal bitmap so these refs don't keep an object alive */
          bitmap[pos / BITMAP_EL_SIZE] &=
    ...
    gc_descr = mono_gc_make_descr_for_object(bitmap, max_set + 1,
 ...
 *
 * Returns: SPECIAL_STATIC_THREAD if the field is thread static,
 * SPECIAL_STATIC_CONTEXT if it is context static, SPECIAL_STATIC_NONE
 * otherwise.
 */
static gint32 field_is_special_static(MonoClass *fklass,
                                      MonoClassField *field) {
  MONO_REQ_GC_NEUTRAL_MODE;
 ...
 *
 *   The IMT slot is embedded into AOTed code, so this must return the same
 * value for the same method across all executions. This means:
 * - pointers shouldn't be used as hash values.
 * - mono_metadata_str_hash () should be used for hashing strings.
 */
guint32 mono_method_get_imt_slot(MonoMethod *method) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
static void add_imt_builder_entry(MonoImtBuilderEntry **imt_builder,
                                  MonoMethod *method,
                                  guint32 *imt_collisions_bitmap,
                                  int vtable_slot, int slot_num) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
#if DEBUG_IMT
  {
    ...
    printf("Added IMT slot for method (%p) %s: imt_slot = %d, vtable_slot = "
           "%d, colliding with other %d entries\n",
...
#if DEBUG_IMT
static void print_imt_entry(const char *message, MonoImtBuilderEntry *e,
                            int num) {
  if (e != NULL) {
...
static int compare_imt_builder_entries(const void *p1, const void *p2) {
...
static int imt_emit_ir(MonoImtBuilderEntry **sorted_array, int start, int end,
                       GPtrArray *out_array) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
static GPtrArray *imt_sort_slot_entries(MonoImtBuilderEntry *entries) {
  MONO_REQ_GC_NEUTRAL_MODE;

  int number_of_entries = entries->children + 1;
  MonoImtBuilderEntry **sorted_array = (MonoImtBuilderEntry **)g_malloc(
      sizeof(MonoImtBuilderEntry *) * number_of_entries);
  GPtrArray *result = g_ptr_array_new();
  MonoImtBuilderEntry *current_entry;
  int i;

  for (current_entry = entries, i = 0; current_entry != NULL;
       current_entry = current_entry->next, i++) {
    sorted_array[i] = current_entry;
  }
  mono_qsort(sorted_array, number_of_entries, sizeof(MonoImtBuilderEntry *),
...
static gpointer initialize_imt_slot(MonoVTable *vtable, MonoDomain *domain,
                                    MonoImtBuilderEntry *imt_builder_entry,
                                    gpointer fail_tramp) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  if (imt_builder_entry != NULL) {
    if (imt_builder_entry->children == 0 && !fail_tramp &&
      ...
      result = imt_trampoline_builder(vtable, domain,
                                      (MonoIMTCheckItem **)imt_ir->pdata,
                                      imt_ir->len, fail_tramp);
...
static MonoImtBuilderEntry *get_generic_virtual_entries(MonoDomain *domain,
 ...
 *
 */
static void build_imt_slots(MonoClass *klass, MonoVTable *vt,
                            MonoDomain *domain, gpointer *imt,
                            GSList *extra_interfaces, int slot_num) {
  MONO_REQ_GC_NEUTRAL_MODE;

  int i;
  GSList *list_item;
  guint32 imt_collisions_bitmap = 0;
  MonoImtBuilderEntry **imt_builder = (MonoImtBuilderEntry **)g_calloc(
      MONO_IMT_SIZE, sizeof(MonoImtBuilderEntry *));
  int method_count = 0;
  ...
  for (i = 0; i < klass_interface_offsets_count; ++i) {
    ...
    for (method_slot_in_interface = 0; method_slot_in_interface < mcount;
      ...
      if (slot_num >= 0 && mono_class_is_ginst(iface)) {
         ...
         */
        method = mono_class_get_method_by_index(
            mono_class_get_generic_class(iface)->container_class,
            method_slot_in_interface);
      ...
      if (method->flags & METHOD_ATTRIBUTE_VIRTUAL) {
        add_imt_builder_entry(imt_builder, method, &imt_collisions_bitmap,
                              vt_slot, slot_num);
        vt_slot++;
      }
    }
  ...
  if (extra_interfaces) {
    ...
    for (list_item = extra_interfaces; list_item != NULL;
      ...
      for (method_slot_in_interface = 0; method_slot_in_interface < mcount;
           method_slot_in_interface++) {
        MonoMethod *method =
            mono_class_get_method_by_index(iface, method_slot_in_interface);

        if (method->is_generic)
          has_generic_virtual = TRUE;
        add_imt_builder_entry(imt_builder, method, &imt_collisions_bitmap,
                              interface_offset + method_slot_in_interface,
                              slot_num);
      }
  ...
  for (i = 0; i < MONO_IMT_SIZE; ++i) {
    ...
    if (slot_num < 0 || i == slot_num) {
      MonoImtBuilderEntry *entries =
          get_generic_virtual_entries(domain, &imt[i]);
    ...
    if (imt_builder[i] != NULL) {
      ...
      if (methods_in_slot >
          UnlockedRead(&mono_stats.imt_max_collisions_in_slot)) {
  ...
  if (record_method_count_for_max_collisions) {
    UnlockedWrite(&mono_stats.imt_method_count_when_max_collisions,
                  method_count);
  }
...
static void build_imt(MonoClass *klass, MonoVTable *vt, MonoDomain *domain,
                      gpointer *imt, GSList *extra_interfaces) {
  MONO_REQ_GC_NEUTRAL_MODE;
 ...
 */
void mono_vtable_build_imt_slot(MonoVTable *vtable, int imt_slot) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  /* we change the slot only if it wasn't changed from the generic imt
   * trampoline already */
 ...
 */
gpointer(mono_method_alloc_generic_virtual_trampoline)(MonoDomain *domain,
                                                       int size) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  if (!inited) {
    mono_counters_register("Generic virtual trampoline bytes",
                           MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
...
static MonoImtBuilderEntry *get_generic_virtual_entries(MonoDomain *domain,
  ...
  if (!domain->generic_virtual_cases)
    domain->generic_virtual_cases =
        g_hash_table_new(mono_aligned_addr_hash, NULL);

  list = (GenericVirtualCase *)g_hash_table_lookup(
      domain->generic_virtual_cases, vtable_slot);
 ...
 * instantiation or variant interface method.  If the number of calls reaches a
 * threshold (THUNK_THRESHOLD), the method is added to the vtable slot's generic
 * virtual method trampoline.
 */
void mono_method_add_generic_virtual_invocation(MonoDomain *domain,
                                                MonoVTable *vtable,
                                                gpointer *vtable_slot,
                                                MonoMethod *method,
                                                gpointer code) {
  ...
  if (!inited) {
    mono_counters_register("Generic virtual cases",
                           MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                           &num_added);
    mono_counters_register("Freed IMT trampolines",
                           MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                           &num_freed);
    inited = TRUE;
  }
  ...
  if (!gvc) {
    gvc = (GenericVirtualCase *)mono_domain_alloc(domain,
                                                  sizeof(GenericVirtualCase));
    gvc->method = method;
    gvc->code = code;
    gvc->count = 0;
    gvc->next = (GenericVirtualCase *)g_hash_table_lookup(
        domain->generic_virtual_cases, vtable_slot);
  ...
  if (++gvc->count == THUNK_THRESHOLD) {
    ...
    if ((gpointer)vtable_slot < (gpointer)vtable) {
      ...
      *vtable_slot = imt_trampoline;
      ...
      vtable_trampoline = callbacks.get_vtable_trampoline
                              ? callbacks.get_vtable_trampoline(
                                    vtable, (gpointer *)vtable_slot -
                                                (gpointer *)vtable->vtable)
                              : NULL;
      ...
      *vtable_slot = imt_trampoline_builder(NULL, domain,
                                            (MonoIMTCheckItem **)sorted->pdata,
                                            sorted->len, vtable_trampoline);
...
static MonoVTable *mono_class_create_runtime_vtable(MonoDomain *domain,
                                                    MonoClass *klass,
                                                    MonoError *error);
 ...
 * they contain the domain specific static class data.
 */
MonoVTable *mono_class_vtable_checked(MonoDomain *domain, MonoClass *klass,
                                      MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
MonoVTable *mono_class_try_get_vtable(MonoDomain *domain, MonoClass *klass) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
static gpointer *alloc_vtable(MonoDomain *domain, size_t vtable_size,
  ...
  /*
   * We want the pointer to the MonoVTable aligned to 8 bytes because SGen uses
   * three address bits.  The IMT has an odd number of entries, however, so on
   * 32 bits the alignment will be off.  In that case we allocate 4 more bytes
   * and skip over them.
   */
  ...
  return (gpointer *)((char *)mono_domain_alloc0(domain, vtable_size) +
                      alloc_offset);
}
...
static MonoVTable *mono_class_create_runtime_vtable(MonoDomain *domain,
  ...
  if (m_class_get_byval_arg(klass)->type == MONO_TYPE_ARRAY ||
    ...
    /*mono_class_init_internal can leave the vtable layout to be lazily done and
     * we can't afford this here*/
  ...
  /*
   * For some classes, mono_class_init_internal () already computed
   * klass->vtable_size, and that is all that is needed because of the vtable
   * trampolines.
   */
  ...
  /* we add an additional vtable slot to store the pointer to static field data
   * only when needed */
  ...
  vtable_size =
      imt_table_bytes + MONO_SIZEOF_VTABLE + vtable_slots * sizeof(gpointer);
  ...
  if (use_interpreter)
    interface_offsets =
        (gpointer *)((char *)interface_offsets + imt_table_bytes / 2);
  g_assert(!((gsize)vt & 7));
   ...
   */
  if (!mono_gc_is_moving() && domain != mono_get_root_domain() &&
  ...
  if (class_size) {
    /* we store the static field pointer at the end of the vtable: vt->vtable
     * [class->vtable_size] */
    if (m_class_has_static_refs(klass)) {
      ...
      bitmap = compute_class_bitmap(
          klass, default_bitmap, sizeof(default_bitmap) * 8, 0, &max_set, TRUE);
      /*g_print ("bitmap 0x%x for %s.%s (size: %d)\n", bitmap [0],
       * klass->name_space, klass->name, class_size);*/
      statics_gc_descr = mono_gc_make_descr_from_bitmap(bitmap, max_set + 1);
      vt->vtable[m_class_get_vtable_size(klass)] =
          mono_gc_alloc_fixed(class_size, statics_gc_descr,
                              MONO_ROOT_SOURCE_STATIC, vt, "Static Fields");

      if (bitmap != default_bitmap)
        g_free(bitmap);
    } else {
      vt->vtable[m_class_get_vtable_size(klass)] =
          mono_domain_alloc0(domain, class_size);
    }
  ...
  while ((field = mono_class_get_fields_internal(klass, &iter))) {
    ...
    if (mono_field_is_deleted(field))
      continue;
    if (!(field->type->attrs & FIELD_ATTRIBUTE_LITERAL)) {
      gint32 special_static = m_class_has_no_special_static_fields(klass)
                                  ? SPECIAL_STATIC_NONE
                                  : field_is_special_static(klass, field);
      if (special_static != SPECIAL_STATIC_NONE) {
        ...
        if (mono_type_is_reference(field->type)) {
          ...
          bitmap = compute_class_bitmap(
              fclass, default_bitmap, sizeof(default_bitmap) * 8,
              -(int)(MONO_OBJECT_HEADER_BITS), &max_set, FALSE);
          numbits = max_set + 1;
        } else {
          default_bitmap[0] = 0;
        ...
        offset = mono_alloc_special_static_data(special_static, size, align,
                                                (uintptr_t *)bitmap, numbits);
        if (!domain->special_static_fields)
          domain->special_static_fields = g_hash_table_new(NULL, NULL);
        g_hash_table_insert(domain->special_static_fields, field,
    ...
    if ((field->type->attrs & FIELD_ATTRIBUTE_HAS_FIELD_RVA)) {
      ...
      /* some fields don't really have rva, they are just zeroed (bss? bug
       * #343083) */
      if (!data)
        continue;
      if (m_class_is_valuetype(fklass)) {
        ...
        g_assert((m_class_get_byval_arg(fklass)->type == MONO_TYPE_PTR) ||
                 (m_class_get_byval_arg(fklass)->type == MONO_TYPE_FNPTR));
        *t = *(char *)data;
      }
      continue;
    }
  ...
  /*
   * FIXME: Is it ok to allocate while holding the domain/loader locks ? If not,
   * we can release them, allocate, then re-acquire them and check if another
   * thread has created the vtable in the meantime.
   */
  /* Special case System.MonoType to avoid infinite recursion */
  if (klass != mono_defaults.runtimetype_class) {
    MonoReflectionTypeHandle vt_type = mono_type_get_object_handle(
    ...
    if (mono_handle_class(vt_type) != mono_defaults.runtimetype_class)
      /* This is unregistered in
         ...
         domain.c. */
      MONO_GC_REGISTER_ROOT_IF_MOVING(vt->type, MONO_ROOT_SOURCE_REFLECTION, vt,
   ...
   * klass->runtime_info is accessed without locking, so this do this last after
   * the vtable has been constructed.
   */
  mono_memory_barrier();
 ...
 */
gboolean mono_remote_class_is_interface_proxy(MonoRemoteClass *remote_class) {
 ...
 */
static MonoVTable *mono_class_proxy_vtable(MonoDomain *domain,
  ...
  for (j = 0; j < remote_class->interface_count; j++) {
    ...
    if (MONO_CLASS_IMPLEMENTS_INTERFACE(klass,
    ...
    if (m_class_get_max_interface_id(iclass) > max_interface_id)
      max_interface_id = m_class_get_max_interface_id(iclass);
  }
  ...
  vtsize = imt_table_bytes + MONO_SIZEOF_VTABLE +
           m_class_get_vtable_size(klass) * sizeof(gpointer);

  UnlockedAdd(&mono_stats.class_vtable_size, vtsize + extra_interface_vtsize);

  interface_offsets =
      alloc_vtable(domain, vtsize + extra_interface_vtsize, imt_table_bytes);
  ...
  /* we need to keep the GC descriptor for a transparent proxy or we confuse the
   * precise GC */
  ...
  for (i = 0; i < m_class_get_vtable_size(klass); ++i) {
    ...
    if ((cm = klass_vtable[i])) {
      pvt->vtable[i] =
          create_remoting_trampoline(domain, cm, target_type, error);
      goto_if_nok(error, failure);
    } else
      pvt->vtable[i] = NULL;
  }
    ...
    for (k = klass; k; k = m_class_get_parent(k)) {
      ...
      while ((m = mono_class_get_methods(k, &iter)))
        if (!pvt->vtable[m->slot]) {
          pvt->vtable[m->slot] =
              create_remoting_trampoline(domain, m, target_type, error);
          goto_if_nok(error, failure);
        }
    }
  }
  ...
  for (i = 0; i < m_class_get_interface_offsets_count(klass); ++i) {
    int interface_id =
        m_class_get_interface_id(m_class_get_interfaces_packed(klass)[i]);
 ...
 * \returns \c SPECIAL_STATIC_THREAD if the field is thread static, \c
 * SPECIAL_STATIC_CONTEXT if it is context static, \c SPECIAL_STATIC_NONE
 * otherwise.
 */
 ...
 * Creates an array of pointers that can be used as a hash key for a remote
 * class. The first element of the array is the number of pointers.
 */
static gpointer *create_remote_class_key(MonoRemoteClass *remote_class,
                                         MonoClass *extra_class) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  if (remote_class == NULL) {
    ...
    if (extra_class != NULL && mono_class_is_interface(extra_class)) {
      key = (void **)g_malloc(sizeof(gpointer) *
                              (remote_class->interface_count + 3));
      ...
      key = (void **)g_malloc(sizeof(gpointer) *
                              (remote_class->interface_count + 2));
 ...
 */
static gpointer *copy_remote_class_key(MonoDomain *domain, gpointer *key) {
  MONO_REQ_GC_NEUTRAL_MODE
 ...
 * On failure returns NULL and sets \p error
 */
MonoRemoteClass *mono_remote_class(MonoDomain *domain,
                                   MonoStringHandle class_name,
                                   MonoClass *proxy_class, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  mono_atomic_fetch_add_i32(
      &mono_perfcounters->loader_bytes,
      mono_string_length_internal(MONO_HANDLE_RAW(class_name)) + 1);
#endif
 ...
 */
static MonoRemoteClass *clone_remote_class(MonoDomain *domain,
                                           MonoRemoteClass *remote_class,
                                           MonoClass *extra_class) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  if (mono_class_is_interface(extra_class)) {
    int i, j;
    rc = (MonoRemoteClass *)mono_domain_alloc(
        domain, MONO_SIZEOF_REMOTE_CLASS +
                    sizeof(MonoClass *) * (remote_class->interface_count + 1));
    ...
    rc = (MonoRemoteClass *)mono_domain_alloc(
        domain, MONO_SIZEOF_REMOTE_CLASS +
                    sizeof(MonoClass *) * remote_class->interface_count);
    rc->proxy_class = extra_class;
    rc->interface_count = remote_class->interface_count;
    if (rc->interface_count > 0)
      memcpy(rc->interfaces, remote_class->interfaces,
             rc->interface_count * sizeof(MonoClass *));
  }
...
gpointer mono_remote_class_vtable(MonoDomain *domain,
                                  MonoRemoteClass *remote_class,
                                  MonoRealProxyHandle rp, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;

  gpointer result = NULL;
  error_init(error);

  mono_loader_lock(); /*FIXME mono_class_from_mono_type_internal and
                         mono_class_proxy_vtable take it*/
  mono_domain_lock(domain);
  gint32 target_domain_id = MONO_HANDLE_GETVAL(rp, target_domain_id);
  if (target_domain_id != -1) {
    if (remote_class->xdomain_vtable == NULL)
      remote_class->xdomain_vtable = mono_class_proxy_vtable(
          domain, remote_class, MONO_REMOTING_TARGET_APPDOMAIN, error);
    goto_if_nok(error, leave);
    result = remote_class->xdomain_vtable;
    goto leave;
  }
  if (remote_class->default_vtable == NULL) {
    MonoReflectionTypeHandle reftype =
        MONO_HANDLE_NEW(MonoReflectionType, NULL);
    ...
    if (mono_class_is_com_object(klass) ||
        (mono_class_get_com_object_class() &&
         klass == mono_class_get_com_object_class())) {
      MonoVTable *klass_vtable =
    ...
    if (target_is_com)
      remote_class->default_vtable = mono_class_proxy_vtable(
          domain, remote_class, MONO_REMOTING_TARGET_COMINTEROP, error);
    else
#endif
      remote_class->default_vtable = mono_class_proxy_vtable(
          domain, remote_class, MONO_REMOTING_TARGET_UNKNOWN, error);
 ...
 * class or an interface.  On success returns TRUE, on failure returns FALSE and
 * sets \p error.
 */
gboolean mono_upgrade_remote_class(MonoDomain *domain,
                                   MonoObjectHandle proxy_object,
                                   MonoClass *klass, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;

  error_init(error);

  MonoTransparentProxyHandle tproxy =
      MONO_HANDLE_CAST(MonoTransparentProxy, proxy_object);
  ...
  mono_domain_lock(domain);
  if (redo_vtable) {
    MonoRemoteClass *fresh_remote_class =
        clone_remote_class(domain, remote_class, klass);
    MONO_HANDLE_SETVAL(tproxy, remote_class, MonoRemoteClass *,
                       fresh_remote_class);
    MonoRealProxyHandle real_proxy = MONO_HANDLE_NEW(MonoRealProxy, NULL);
    MONO_HANDLE_GET(real_proxy, tproxy, rp);
    MONO_HANDLE_SETVAL(proxy_object, vtable, MonoVTable *,
                       (MonoVTable *)mono_remote_class_vtable(
                           domain, fresh_remote_class, real_proxy, error));
    goto_if_nok(error, leave);
  }
...
MonoMethod *mono_object_get_virtual_method_internal(MonoObject *obj_raw,
                                                    MonoMethod *method) {
  HANDLE_FUNCTION_ENTER();
  MonoMethod *result;
  ERROR_DECL(error);
 ...
 * \param method method
 * Retrieves the \c MonoMethod that would be called on \p obj if \p obj is
 * passed as the instance of a callvirt of \p method.
 */
MonoMethod *mono_object_get_virtual_method(MonoObject *obj,
                                           MonoMethod *method) {
  MONO_EXTERNAL_ONLY_GC_UNSAFE(
      MonoMethod *, mono_object_get_virtual_method_internal(obj, method));
}
 ...
 * passed as the instance of a callvirt of \p method.
 */
MonoMethod *mono_object_handle_get_virtual_method(MonoObjectHandle obj,
                                                  MonoMethod *method,
                                                  MonoError *error) {
  error_init(error);
  ...
  if (mono_class_is_transparent_proxy(klass)) {
    MonoRemoteClass *remote_class = MONO_HANDLE_GETVAL(
        MONO_HANDLE_CAST(MonoTransparentProxy, obj), remote_class);
...
MonoMethod *mono_class_get_virtual_method(MonoClass *klass, MonoMethod *method,
                                          gboolean is_proxy, MonoError *error) {
  MONO_REQ_GC_NEUTRAL_MODE;
  error_init(error);

  if (!is_proxy && ((method->flags & METHOD_ATTRIBUTE_FINAL) ||
  ...
  if (method->slot != -1) {
    if (mono_class_is_interface(method->klass)) {
      if (!is_proxy) {
        ...
        int iface_offset = mono_class_interface_offset_with_variance(
            klass, method->klass, &variance_used);
...
#ifndef DISABLE_COM
      if (klass == mono_class_get_com_object_class() ||
...
#endif
  {
    if (method->is_inflated) {
      ...
      res = mono_class_inflate_generic_method_checked(
          res, &((MonoMethodInflated *)method)->context, error);
    }
  }
...
static MonoObject *do_runtime_invoke(MonoMethod *method, void *obj,
...
MonoObject *mono_runtime_invoke(MonoMethod *method, void *obj, void **params,
  ...
  if (exc) {
    ...
    mono_error_raise_exception_deprecated(
        error); /* OK to throw, external only without a good alternative */
  }
  MONO_EXIT_GC_UNSAFE;
  return res;
}
 ...
 * reference.
 */
MonoObject *mono_runtime_try_invoke(MonoMethod *method, void *obj,
                                    void **params, MonoObject **exc,
                                    MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
MonoObjectHandle mono_runtime_try_invoke_handle(MonoMethod *method,
                                                MonoObjectHandle obj,
                                                void **params,
                                                MonoError *error) {
  // FIXME? typing of params
  MonoException *exc = NULL;
  MonoObject *obj_raw = mono_runtime_try_invoke(
      method, MONO_HANDLE_RAW(obj), params, (MonoObject **)&exc, error);
 ...
 * reference.
 */
MonoObject *mono_runtime_invoke_checked(MonoMethod *method, void *obj,
                                        void **params, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
MonoObjectHandle mono_runtime_invoke_handle(MonoMethod *method,
                                            MonoObjectHandle obj, void **params,
                                            MonoError *error) {
...
void mono_runtime_invoke_handle_void(MonoMethod *method, MonoObjectHandle obj,
                                     void **params, MonoError *error) {
  mono_runtime_invoke_checked(method, MONO_HANDLE_RAW(obj), params, error);
}
 ...
 *
 * C:  <code>typedef MonoBoolean (*Equals)(MonoObject*, MonoObject*,
 * MonoException**);</code>
 *
 ...
 *
 * C:  <code>typedef MonoBoolean (*ReferenceEquals)(MonoObject*, MonoObject*,
 * MonoException**);</code>
 *
 ...
 *
 * C: <code>typedef MonoBoolean (__stdcall *Equals)(MonoObject*, MonoObject*,
 * MonoException**);</code>
 *
 * LIMITATIONS
 *
 ...
 *
 * C#: <code>public static Rectangle Intersect (Rectangle a, Rectangle
 * b);</code> C:  <code>typedef MonoObject* (*Intersect)(MonoObject*,
 * MonoObject*, MonoException**);</code>
 *
...
void mono_copy_value(MonoType *type, void *dest, void *value,
                     int deref_pointer) {
  MONO_REQ_GC_UNSAFE_MODE;
...
handle_enum:
  ...
  case MONO_TYPE_ARRAY:
    mono_gc_wbarrier_generic_store_internal(
        dest, deref_pointer ? *(MonoObject **)value : (MonoObject *)value);
    return;
  case MONO_TYPE_FNPTR:
  case MONO_TYPE_PTR: {
    gpointer *p = (gpointer *)dest;
...
void mono_field_set_value_internal(MonoObject *obj, MonoClassField *field,
                                   void *value) {
  void *dest;
...
#if ENABLE_NETCORE
  mono_copy_value(field->type, dest, value,
                  value && field->type->type == MONO_TYPE_PTR);
#else
 ...
 *
 * Sets the value of the field described by \p field in the object instance \p
 * obj to the value passed in \p value.   This method should only be used for
 * instance fields.   For static fields, use \c mono_field_static_set_value.
 *
...
void mono_field_set_value(MonoObject *obj, MonoClassField *field, void *value) {
  MONO_EXTERNAL_ONLY_GC_UNSAFE_VOID(
      mono_field_set_value_internal(obj, field, value));
}

void mono_field_static_set_value_internal(MonoVTable *vt, MonoClassField *field,
                                          void *value) {
  void *dest;
 ...
 * The value must be in the native format of the field type.
 */
void mono_field_static_set_value(MonoVTable *vt, MonoClassField *field,
                                 void *value) {
  MONO_EXTERNAL_ONLY_GC_UNSAFE_VOID(
      mono_field_static_set_value_internal(vt, field, value));
}

/**
 * mono_vtable_get_static_field_data:
 *
 * Internal use function: return a pointer to the memory holding the static
 * fields for a class or NULL if there are no static fields. This is exported
 * only for use by the debugger.
 */
...
static guint8 *mono_field_get_addr(MonoObject *obj, MonoVTable *vt,
                                   MonoClassField *field) {
  MONO_REQ_GC_UNSAFE_MODE;
...
void mono_field_get_value_internal(MonoObject *obj, MonoClassField *field,
                                   void *value) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * field represents a value type, the value is boxed.
 */
MonoObjectHandle mono_static_field_get_value_handle(MonoDomain *domain,
                                                    MonoClassField *field,
                                                    MonoError *error)
// FIXMEcoop invert
{
  HANDLE_FUNCTION_ENTER();
  HANDLE_FUNCTION_RETURN_REF(
      MonoObject,
      MONO_HANDLE_NEW(MonoObject, mono_field_get_value_object_checked(
                                      domain, field, NULL, error)));
}
 ...
 * \param error Set on error.
 * \returns a new \c MonoObject with the value from the given field.  If the
 * field represents a value type, the value is boxed.  On error returns NULL and
 * sets \p error.
 */
MonoObject *mono_field_get_value_object_checked(MonoDomain *domain,
                                                MonoClassField *field,
                                                MonoObject *obj,
                                                MonoError *error) {
  // FIXMEcoop
  ...
  if (is_ref) {
    if (is_literal) {
      ...
      mono_field_static_get_value_checked(vtable, field, &o, string_handle,
                                          error);
      goto_if_nok(error, return_null);
    } else {
  ...
  if (is_ptr) {
    ...
    m = mono_class_get_method_from_name_checked(ptr_klass, "Box", 2,
                                                METHOD_ATTRIBUTE_STATIC, error);
    goto_if_nok(error, return_null);
    g_assert(m);
    ...
    if (is_literal) {
      ...
      mono_field_static_get_value_checked(vtable, field, v, string_handle,
                                          error);
      goto_if_nok(error, return_null);
    } else {
  ...
  if (mono_class_is_nullable(klass)) {
    o = mono_nullable_box(mono_field_get_addr(obj, vtable, field), klass,
                          error);
    goto exit;
  }
...
/*
 * Important detail, if type is MONO_TYPE_STRING we return a blob encoded string
 * (ie, utf16 + leb128 prefixed size)
 */
gboolean mono_metadata_read_constant_value(const char *blob, MonoTypeEnum type,
                                           void *value, MonoError *error) {
  error_init(error);
  ...
  default:
    ...
    mono_error_set_execution_engine(
        error, "Type 0x%02x should not be in constant table", type);
  }
  return retval;
}

gboolean mono_get_constant_value_from_blob(MonoDomain *domain,
                                           MonoTypeEnum type, const char *blob,
                                           void *value,
                                           MonoStringHandleOut string_handle,
                                           MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (type == MONO_TYPE_STRING) {
    mono_ldstr_metadata_sig(domain, *(const char **)value, string_handle,
                            error);
...
static void get_default_field_value(MonoDomain *domain, MonoClassField *field,
  ...
  (void)mono_get_constant_value_from_blob(domain, def_type, data, value,
                                          string_handle, error);
}

void mono_field_static_get_value_for_thread(MonoInternalThread *thread,
                                            MonoVTable *vt,
  ...
  if (field->offset == -1) {
    /* Special static */
    gpointer addr =
        g_hash_table_lookup(vt->domain->special_static_fields, field);
    src =
        mono_get_special_static_data_for_thread(thread, GPOINTER_TO_UINT(addr));
  } else {
 ...
 * </pre>
 */
void mono_field_static_get_value(MonoVTable *vt, MonoClassField *field,
                                 void *value) {
  MONO_REQ_GC_NEUTRAL_MODE;

  ERROR_DECL(error);
  mono_field_static_get_value_checked(vt, field, value,
 ...
 */
void mono_field_static_get_value_checked(MonoVTable *vt, MonoClassField *field,
                                         void *value,
                                         MonoStringHandleOut string_handle,
                                         MonoError *error) {
  MONO_REQ_GC_NEUTRAL_MODE;

  mono_field_static_get_value_for_thread(mono_thread_internal_current(), vt,
                                         field, value, string_handle, error);
}
 ...
 * \c MonoObject* result from the function.
 */
void mono_property_set_value(MonoProperty *prop, void *obj, void **params,
                             MonoObject **exc) {
  MONO_ENTER_GC_UNSAFE;
 ...
 */
gboolean mono_property_set_value_handle(MonoProperty *prop,
                                        MonoObjectHandle obj, void **params,
                                        MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
MonoObject *mono_property_get_value(MonoProperty *prop, void *obj,
                                    void **params, MonoObject **exc) {
  MonoObject *val;
  MONO_ENTER_GC_UNSAFE;
 ...
 * \c MonoObject* result from the function.  The exception will be propagated
 * via \p error.
 *
 * \returns the value from invoking the get method on the property. On
 * failure returns NULL and sets \p error.
 */
MonoObject *mono_property_get_value_checked(MonoProperty *prop, void *obj,
                                            void **params, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
static MonoClassField *nullable_class_get_value_field(MonoClass *klass) {
...
static MonoClassField *nullable_class_get_has_value_field(MonoClass *klass) {
  mono_class_setup_fields(klass);
  g_assert(m_class_is_fields_inited(klass));
...
static gpointer nullable_get_has_value_field_addr(guint8 *nullable,
                                                  MonoClass *klass) {
...
static gpointer nullable_get_value_field_addr(guint8 *nullable,
                                              MonoClass *klass) {
 ...
 * structure for them.
 */
void mono_nullable_init(guint8 *buf, MonoObject *value, MonoClass *klass) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (value) {
    if (m_class_has_references(param_class))
      mono_gc_wbarrier_value_copy_internal(
          value_field_addr, mono_object_unbox_internal(value), 1, param_class);
    else
      mono_gc_memmove_atomic(value_field_addr,
                             mono_object_unbox_internal(value),
                             mono_class_value_size(param_class, NULL));
  } else {
    mono_gc_bzero_atomic(value_field_addr,
                         mono_class_value_size(param_class, NULL));
  }
}
 ...
 * structure for them.
 */
void mono_nullable_init_from_handle(guint8 *buf, MonoObjectHandle value,
                                    MonoClass *klass) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
void mono_nullable_init_unboxed(guint8 *buf, gpointer value, MonoClass *klass) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  *(guint8 *)(has_value_field_addr) = (value == NULL) ? 0 : 1;
  if (value) {
    if (m_class_has_references(param_class))
      mono_gc_wbarrier_value_copy_internal(value_field_addr, value, 1,
                                           param_class);
    else
      mono_gc_memmove_atomic(value_field_addr, value,
                             mono_class_value_size(param_class, NULL));
  } else {
    mono_gc_bzero_atomic(value_field_addr,
                         mono_class_value_size(param_class, NULL));
 ...
 */
MonoObject *mono_nullable_box(gpointer vbuf, MonoClass *klass,
                              MonoError *error) {
  ...
  g_assertf(!m_class_is_byreflike(param_class),
            "Unexpected Nullable<%s> - generic type instantiated with "
            "IsByRefLike type",
            mono_type_get_full_name(param_class));

  if (*(guint8 *)(has_value_field_addr)) {
    MonoObject *o =
        mono_object_new_checked(mono_domain_get(), param_class, error);
    return_val_if_nok(error, NULL);
    if (m_class_has_references(param_class))
      mono_gc_wbarrier_value_copy_internal(mono_object_unbox_internal(o),
                                           value_field_addr, 1, param_class);
    else
      mono_gc_memmove_atomic(mono_object_unbox_internal(o), value_field_addr,
                             mono_class_value_size(param_class, NULL));
    return o;
  } else
    return NULL;
...
MonoObjectHandle mono_nullable_box_handle(gpointer buf, MonoClass *klass,
                                          MonoError *error) {
  // FIXMEcoop gpointer buf needs more attention
  return MONO_HANDLE_NEW(MonoObject, mono_nullable_box(buf, klass, error));
}

MonoMethod *mono_get_delegate_invoke_internal(MonoClass *klass) {
  MonoMethod *result;
  ERROR_DECL(error);
 ...
 * \returns the \c MonoMethod for the \c Invoke method in the delegate class or
 * NULL if \p klass is a broken delegate type
 */
 ...
 * \param error set on error
 * \returns the \c MonoMethod for the \c Invoke method in the delegate class or
 * NULL if \p klass is a broken delegate type or not a delegate class.
 *
 * Sets \p error on error
 */
MonoMethod *mono_get_delegate_invoke_checked(MonoClass *klass,
                                             MonoError *error) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
MonoMethod *mono_get_delegate_begin_invoke_internal(MonoClass *klass) {
  MonoMethod *result;
  ERROR_DECL(error);
 ...
 * \param klass The delegate class
 * \returns the \c MonoMethod for the \c BeginInvoke method in the delegate
 * class or NULL if \p klass is a broken delegate type
 */
MonoMethod *mono_get_delegate_begin_invoke(MonoClass *klass) {
  MONO_EXTERNAL_ONLY(MonoMethod *,
 ...
 * \param error set on error
 * \returns the \c MonoMethod for the \c BeginInvoke method in the delegate
 * class or NULL if \p klass is a broken delegate type or not a delegate class.
 *
 * Sets \p error on error
 */
MonoMethod *mono_get_delegate_begin_invoke_checked(MonoClass *klass,
                                                   MonoError *error) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  if (mono_class_has_failure(klass))
    return NULL;
  im = mono_class_get_method_from_name_checked(klass, "BeginInvoke", -1, 0,
                                               error);
  return im;
}

MonoMethod *mono_get_delegate_end_invoke_internal(MonoClass *klass) {
  MonoMethod *result;
  ERROR_DECL(error);
 ...
 * \param klass The delegate class
 * \returns the \c MonoMethod for the \c EndInvoke method in the delegate class
 * or NULL if \p klass is a broken delegate type
 */
 ...
 * \param error set on error
 * \returns the \c MonoMethod for the \c EndInvoke method in the delegate class
 * or NULL if \p klass is a broken delegate type or not a delegate class.
 *
 * Sets \p error on error
 */
MonoMethod *mono_get_delegate_end_invoke_checked(MonoClass *klass,
                                                 MonoError *error) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  if (mono_class_has_failure(klass))
    return NULL;
  im =
      mono_class_get_method_from_name_checked(klass, "EndInvoke", -1, 0, error);
  return im;
}
 ...
 * \c MonoObject* result from the function.
 */
MonoObject *mono_runtime_delegate_invoke(MonoObject *delegate, void **params,
                                         MonoObject **exc) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (exc) {
    MonoObject *result =
        mono_runtime_delegate_try_invoke(delegate, params, exc, error);
    if (*exc) {
      mono_error_cleanup(error);
      return NULL;
    } else {
    ...
    MonoObject *result =
        mono_runtime_delegate_invoke_checked(delegate, params, error);
    mono_error_raise_exception_deprecated(
        error); /* OK to throw, external only without a good alternative */
    return result;
  }
 ...
 * \c MonoObject* result from the function.
 */
MonoObject *mono_runtime_delegate_try_invoke(MonoObject *delegate,
                                             void **params, MonoObject **exc,
                                             MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  g_assertf(im, "Could not lookup delegate invoke method for delegate %s",
            mono_type_get_full_name(klass));
...
mono_runtime_delegate_try_invoke_handle(MonoObjectHandle delegate,
                                        void **params, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
MonoObject *mono_runtime_delegate_invoke_checked(MonoObject *delegate,
                                                 void **params,
                                                 MonoError *error) {
  error_init(error);
...
static gboolean handle_main_arg_array_set(MonoDomain *domain, int idx,
                                          MonoArrayHandle dest,
                                          MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoStringHandle value =
      mono_string_new_handle(domain, main_args[idx], error);
  goto_if_nok(error, leave);
 ...
 */
MonoArrayHandle mono_runtime_get_main_args_handle(MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  array = mono_array_new_handle(domain, mono_defaults.string_class,
                                num_main_args, error);
  if (!is_ok(error)) {
 ...
 * Set the command line arguments from an embedding application that doesn't
 * otherwise call \c mono_runtime_run_main.
 */
  ...
  for (i = 0; i < argc; ++i) {
    ...
    if (utf8_arg == NULL) {
      g_print("\nCannot determine the text encoding for argument %d (%s).\n", i,
              argv[i]);
      g_print("Please add the correct encoding to MONO_EXTERNAL_ENCODINGS and "
              "try again.\n");
      exit(-1);
    }
 ...
 *
 */
static MonoArray *prepare_run_main(MonoMethod *method, int argc, char *argv[]) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (!g_path_is_absolute(argv[0])) {
    ...
    gchar *fullpath =
        g_build_filename(m_class_get_image(method->klass)->assembly->basedir,
                         basename, (const char *)NULL);
  ...
  if (sig->param_count) {
    args = (MonoArray *)mono_array_new_checked(
        domain, mono_defaults.string_class, argc, error);
    mono_error_assert_ok(error);
    for (i = 0; i < argc; ++i) {
    ...
    args = (MonoArray *)mono_array_new_checked(
        domain, mono_defaults.string_class, 0, error);
    mono_error_assert_ok(error);
  }
 ...
 * <code>Main</code>) \param argc number of arguments from the command line
 * \param argv array of strings from the command line
 ...
 */
int mono_runtime_run_main(MonoMethod *method, int argc, char *argv[],
                          MonoObject **exc) {
  int res;
  ...
  if (!exc)
    mono_error_raise_exception_deprecated(
        error); /* OK to throw, external only without a better alternative */
 ...
 */
int mono_runtime_run_main_checked(MonoMethod *method, int argc, char *argv[],
                                  MonoError *error) {
  error_init(error);
  MonoArray *args = prepare_run_main(method, argc, argv);
 ...
 */
int mono_runtime_try_run_main(MonoMethod *method, int argc, char *argv[],
                              MonoObject **exc) {
  g_assert(exc);
  MonoArray *args = prepare_run_main(method, argc, argv);
...
static MonoObjectHandle
serialize_or_deserialize_object(MonoObjectHandle obj, const gchar *method_name,
                                MonoMethod **method, MonoError *error) {
  if (!*method) {
    MonoClass *klass = mono_class_get_remoting_services_class();
    *method = mono_class_get_method_from_name_checked(klass, method_name, -1, 0,
                                                      error);
...
static MonoObjectHandle serialize_object(MonoObjectHandle obj,
                                         MonoError *error) {
  g_assert(!mono_class_is_marshalbyref(mono_handle_class(obj)));
  return serialize_or_deserialize_object(obj, "SerializeCallData",
                                         &serialize_method, error);
}

static MonoMethod *deserialize_method;

static MonoObjectHandle deserialize_object(MonoObjectHandle obj,
                                           MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  return serialize_or_deserialize_object(obj, "DeserializeCallData",
                                         &deserialize_method, error);
}

#ifndef DISABLE_REMOTING
static MonoObjectHandle make_transparent_proxy(MonoObjectHandle obj,
                                               MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (!get_proxy_method) {
    get_proxy_method = mono_class_get_method_from_name_checked(
        mono_defaults.real_proxy_class, "GetTransparentProxy", 0, 0, error);
    mono_error_assert_ok(error);
  }
  ...
  MonoDomain *domain = mono_domain_get();
  MonoRealProxyHandle real_proxy = MONO_HANDLE_CAST(
      MonoRealProxy,
      mono_object_new_handle(domain, mono_defaults.real_proxy_class, error));
  goto_if_nok(error, return_null);
  MonoReflectionTypeHandle reflection_type;
  reflection_type = mono_type_get_object_handle(
      domain, m_class_get_byval_arg(mono_handle_class(obj)), error);
  goto_if_nok(error, return_null);

  MONO_HANDLE_SET(real_proxy, class_to_proxy, reflection_type);
  MONO_HANDLE_SET(real_proxy, unwrapped_server, obj);

  return mono_runtime_try_invoke_handle(
      get_proxy_method, MONO_HANDLE_CAST(MonoObject, real_proxy), NULL, error);
return_null:
  return mono_new_null();
}
#endif /* DISABLE_REMOTING */
 ...
 */
MonoObjectHandle mono_object_xdomain_representation(MonoObjectHandle obj,
...
create_unhandled_exception_eventargs(MonoObjectHandle exc, MonoError *error) {
  ...
  MonoMethod *const method = mono_class_get_method_from_name_checked(
      klass, ".ctor", 2, METHOD_ATTRIBUTE_PUBLIC, error);
  goto_if_nok(error, return_null);
  g_assert(method);

  {
    ...
    MonoObjectHandle obj =
        mono_object_new_handle(mono_domain_get(), klass, error);
    goto_if_nok(error, return_null);
...
static void call_unhandled_exception_delegate(MonoDomain *domain,
  ...
  if (MONO_HANDLE_DOMAIN(exc) != domain) {
    ...
    if (MONO_HANDLE_IS_NULL(exc)) {
      ...
      if (!is_ok(error)) {
        MonoExceptionHandle serialization_exc =
            mono_error_convert_to_exception_handle(error);
        exc = mono_object_xdomain_representation(
            MONO_HANDLE_CAST(MonoObject, serialization_exc), domain,
            inner_error);
      } else {
        exc = MONO_HANDLE_CAST(
            MonoObject,
            mono_exception_new_serialization(
                "Could not serialize unhandled exception.", inner_error));
      }
  ...
  gpointer pa[] = {
      domain->domain, MONO_HANDLE_RAW(create_unhandled_exception_eventargs(
                          exc, error)) // FIXMEcoop
  };
  mono_error_assert_ok(error);
...
static MonoRuntimeUnhandledExceptionPolicy runtime_unhandled_exception_policy =
 ...
 */
void mono_runtime_unhandled_exception_policy_set(
    MonoRuntimeUnhandledExceptionPolicy policy) {
...
void mono_unhandled_exception_internal(MonoObject *exc_raw) {
  ERROR_DECL(error);
  HANDLE_FUNCTION_ENTER();
 ...
 * a warning to the console
 */
void mono_unhandled_exception_checked(MonoObjectHandle exc, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;

  MonoClassField *field;
  MonoDomain *current_domain, *root_domain;
  MonoObjectHandle current_appdomain_delegate =
      MONO_HANDLE_NEW(MonoObject, NULL);

  MonoClass *klass = mono_handle_class(exc);
  /*
   * AppDomainUnloadedException don't behave like unhandled exceptions unless
   * thrown from a thread started in unmanaged world.
  ...
  field = mono_class_get_field_from_name_full(mono_defaults.appdomain_class,
  ...
  MonoObjectHandle root_appdomain_delegate = MONO_HANDLE_NEW(
      MonoObject, mono_field_get_value_object_checked(
                      root_domain, field, (MonoObject *)root_domain->domain,
                      error)); /* FIXME use handles for
                                  mono_field_get_value_object_checked */
  return_if_nok(error);
    ...
    MONO_HANDLE_ASSIGN(
        ...
        MONO_HANDLE_NEW(MonoObject,
                        mono_field_get_value_object_checked(
                            ...
                            (MonoObject *)current_domain->domain,
                            error))); /* FIXME use handles for
                                         mono_field_get_value_object_checked */
    return_if_nok(error);
  }
  ...
  if (MONO_HANDLE_IS_NULL(current_appdomain_delegate) &&
    ...
    mono_print_unhandled_exception_internal(MONO_HANDLE_RAW(
        exc)); /* FIXME use handles for mono_print_unhandled_exception */
  } else {
    ...
    if (!MONO_HANDLE_IS_NULL(root_appdomain_delegate))
      call_unhandled_exception_delegate(root_domain, root_appdomain_delegate,
                                        exc);
    if (!MONO_HANDLE_IS_NULL(current_appdomain_delegate))
      call_unhandled_exception_delegate(current_domain,
                                        current_appdomain_delegate, exc);
    mono_threads_end_abort_protected_block();
  }

  /* set exitcode only if we will abort the process */
  if ((main_thread &&
       mono_thread_internal_current() == main_thread->internal_thread) ||
      mono_runtime_unhandled_exception_policy_get() ==
          MONO_UNHANDLED_POLICY_CURRENT) {
...
void mono_runtime_exec_managed_code(MonoDomain *domain,
                                    MonoMainThreadFunc mfunc, gpointer margs) {
...
static void prepare_thread_to_exec_main(MonoDomain *domain,
                                        MonoMethod *method) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (!domain->entry_assembly) {
    ...
    /* Domains created from another domain already have application_base and
     * configuration_file set */
    if (domain->setup->application_base == NULL) {
      MonoString *basedir =
          mono_string_new_checked(domain, assembly->basedir, error);
      mono_error_assert_ok(error);
    ...
    if (domain->setup->configuration_file == NULL) {
      ...
      mono_error_assert_ok(error);
      MONO_OBJECT_SETREF_INTERNAL(domain->setup, configuration_file,
                                  config_file);
      g_free(str);
  ...
  if (cinfo) {
    has_stathread_attribute = mono_custom_attrs_has_attr(
        cinfo, mono_class_get_sta_thread_attribute_class());
    if (!cinfo->cached)
...
static int do_exec_main_checked(MonoMethod *method, MonoArray *args,
                                MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
static int do_try_exec_main(MonoMethod *method, MonoArray *args,
                            MonoObject **exc) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * executable name).
 */
int mono_runtime_exec_main(MonoMethod *method, MonoArray *args,
                           MonoObject **exc) {
  int rval;
  MONO_ENTER_GC_UNSAFE;
  ERROR_DECL(error);
  ...
  if (exc) {
    ...
    mono_error_raise_exception_deprecated(
        error); /* OK to throw, external only with no better option */
  }
  MONO_EXIT_GC_UNSAFE;
  return rval;
}
 ...
 */
int mono_runtime_exec_main_checked(MonoMethod *method, MonoArray *args,
                                   MonoError *error) {
  error_init(error);
  prepare_thread_to_exec_main(mono_object_domain(args), method);
 ...
 * executable name).
 *
 * On failure sets @error if Main couldn't be executed, or @exc if it threw an
 * exception.
 */
int mono_runtime_try_exec_main(MonoMethod *method, MonoArray *args,
                               MonoObject **exc) {
  prepare_thread_to_exec_main(mono_object_domain(args), method);
 ...
 * @has_byref_nullables: outarg - TRUE if method expects a byref nullable
 * argument
 * @error: set on error.
 *
 * Given an array of method arguments, return the ith one using the
 * corresponding type to perform necessary unboxing.  If method expects a ref
 * nullable argument, writes TRUE to @has_byref_nullables.
 *
 * On failure sets @error and returns NULL.
 */
static gpointer invoke_array_extract_argument(MonoArray *params, int i,
                                              MonoType *t,
...
again:
  ...
  case MONO_TYPE_VALUETYPE:
    if (t->type == MONO_TYPE_VALUETYPE &&
      ...
      /* The runtime invoke wrapper needs the original boxed vtype, it does
       * handle byref values as well. */
      ...
      if (!mono_array_get_internal(params, MonoObject *, i)) {
        MonoObject *o = mono_object_new_checked(
            mono_domain_get(), mono_class_from_mono_type_internal(t_orig),
            error);
        return_val_if_nok(error, NULL);
      ...
      if (t->byref) {
        ...
        MonoObject *copy =
            mono_value_box_checked(mono_domain_get(), orig->vtable->klass,
                                   mono_object_unbox_internal(orig), error);
        return_val_if_nok(error, NULL);
        mono_array_setref_internal(params, i, copy);
      }

      result = mono_object_unbox_internal(
          mono_array_get_internal(params, MonoObject *, i));
      if (!t->byref && was_null)
...
MonoObject *mono_runtime_invoke_array(MonoMethod *method, void *obj,
                                      MonoArray *params, MonoObject **exc) {
  ERROR_DECL(error);
  if (exc) {
    MonoObject *result =
        mono_runtime_try_invoke_array(method, obj, params, exc, error);
    if (*exc) {
      mono_error_cleanup(error);
      return NULL;
    } else {
    ...
    MonoObject *result =
        mono_runtime_try_invoke_array(method, obj, params, NULL, error);
    mono_error_raise_exception_deprecated(
        error); /* OK to throw, external only without a good alternative */
    return result;
  }
 ...
 * reference.
 */
MonoObject *mono_runtime_invoke_array_checked(MonoMethod *method, void *obj,
                                              MonoArray *params,
                                              MonoError *error) {
  error_init(error);
 ...
 * reference.
 */
MonoObject *mono_runtime_try_invoke_array(MonoMethod *method, void *obj,
                                          MonoArray *params, MonoObject **exc,
                                          MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (NULL != params) {
    ...
    for (i = 0; i < mono_array_length_internal(params); i++) {
      ...
      pa[i] = invoke_array_extract_argument(params, i, t, &has_byref_nullables,
                                            error);
      return_val_if_nok(error, NULL);
    }
  }
  ...
  if (!strcmp(method->name, ".ctor") &&
    ...
    if (mono_class_is_nullable(method->klass)) {
      ...
      if (!params)
        ...
        return mono_value_box_checked(mono_domain_get(),
                                      m_class_get_cast_class(method->klass),
                                      pa[0], error);
      }
    }
    ...
    if (!obj) {
      ...
      if (mono_object_is_transparent_proxy(obj)) {
        method = mono_marshal_get_remoting_invoke(
            method->slot == -1
                ? method
                : m_class_get_vtable(method->klass)[method->slot],
            error);
        return_val_if_nok(error, NULL);
      }
#endif
      ...
      obj =
          mono_value_box_checked(mono_domain_get(), method->klass, obj, error);
      return_val_if_nok(error, NULL);
    }
    ...
    if (mono_class_is_nullable(method->klass)) {
      if (method->flags & METHOD_ATTRIBUTE_STATIC) {
        ...
        nullable =
            mono_object_new_checked(mono_domain_get(), method->klass, error);
        return_val_if_nok(error, NULL);

        MonoObject *boxed = mono_value_box_checked(
            mono_domain_get(), m_class_get_cast_class(method->klass), obj,
            error);
        return_val_if_nok(error, NULL);
        mono_nullable_init((guint8 *)mono_object_unbox_internal(nullable),
                           boxed, method->klass);
    ...
    if (sig->ret->type == MONO_TYPE_PTR) {
      ...
      box_method = mono_class_get_method_from_name_checked(pointer_class, "Box",
                                                           -1, 0, error);
      mono_error_assert_ok(error);
      ...
      if (sig->ret->byref) {
        ...
        tmpret->byref = FALSE;
        box_args[1] =
            mono_type_get_object_checked(mono_domain_get(), tmpret, error);
        mono_metadata_free_type(tmpret);
      } else {
        box_args[1] =
            mono_type_get_object_checked(mono_domain_get(), sig->ret, error);
      }
      return_val_if_nok(error, NULL);

      res =
          mono_runtime_try_invoke(box_method, NULL, box_args, &box_exc, error);
    ...
    if (has_byref_nullables) {
      ...
      for (i = 0; i < mono_array_length_internal(params); i++) {
        ...
        if (t->byref && t->type == MONO_TYPE_GENERICINST &&
...
static MonoObjectHandle mono_object_new_by_vtable(MonoVTable *vtable,
                                                  MonoError *error);
 ...
 * i.e. calling mono_object_register_finalizer and
 * mono_gc_register_obj_with_weak_fields, and setting error.
 */
static MonoObject *object_new_common_tail(MonoObject *o, MonoClass *klass,
                                          MonoError *error) {
  error_init(error);
 ...
 */
static MonoObjectHandle object_new_handle_common_tail(MonoObjectHandle o,
                                                      MonoClass *klass,
                                                      MonoError *error) {
  error_init(error);
...
MonoObject *ves_icall_object_new(MonoDomain *domain, MonoClass *klass) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
MonoObject *mono_object_new_checked(MonoDomain *domain, MonoClass *klass,
                                    MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * It returns NULL on failure and sets \p error.
 */
MonoObjectHandle mono_object_new_handle(MonoDomain *domain, MonoClass *klass,
                                        MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
MonoObjectHandle mono_object_new_pinned_handle(MonoDomain *domain,
                                               MonoClass *klass,
                                               MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
MonoObject *mono_object_new_pinned(MonoDomain *domain, MonoClass *klass,
                                   MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  MonoObject *o =
      mono_gc_alloc_pinned_obj(vtable, mono_class_instance_size(klass));
...
MonoObject *mono_object_new_specific_checked(MonoVTable *vtable,
                                             MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (mono_vtable_is_remote(vtable) ||
      mono_class_is_com_object(vtable->klass)) {
    gpointer pa[1];
    ...
    if (im == NULL) {
      ...
      im = mono_class_get_method_from_name_checked(klass, "CreateProxyForType",
                                                   1, 0, error);
      return_val_if_nok(error, NULL);
      if (!im) {
    ...
    pa[0] = mono_type_get_object_checked(
        mono_domain_get(), m_class_get_byval_arg(vtable->klass), error);
    if (!is_ok(error))
      return NULL;
...
static MonoObjectHandle mono_object_new_by_vtable(MonoVTable *vtable,
  ...
  if (mono_vtable_is_remote(vtable) ||
    ...
    // FIXMEcoop
    gpointer pa[] = {mono_type_get_object_checked(
        mono_domain_get(), m_class_get_byval_arg(vtable->klass), error)};
    return_val_if_nok(error, MONO_HANDLE_NEW(MonoObject, NULL));

    // FIXMEcoop
    o = MONO_HANDLE_NEW(MonoObject,
                        mono_runtime_invoke_checked(im, NULL, pa, error));
    return_val_if_nok(error, MONO_HANDLE_NEW(MonoObject, NULL));
...
MonoObject *ves_icall_object_new_specific(MonoVTable *vtable) {
  ERROR_DECL(error);
  MonoObject *o = mono_object_new_specific_checked(vtable, error);
  mono_error_set_pending_exception(error);
 ...
 */
MonoObject *mono_object_new_alloc_specific_checked(MonoVTable *vtable,
                                                   MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;

  MonoObject *o =
      mono_gc_alloc_obj(vtable, m_class_get_instance_size(vtable->klass));

  return object_new_common_tail(o, vtable->klass, error);
}

MonoObjectHandle mono_object_new_alloc_by_vtable(MonoVTable *vtable,
                                                 MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
MonoObject *mono_object_new_mature(MonoVTable *vtable, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
MonoObjectHandle mono_object_new_handle_mature(MonoVTable *vtable,
                                               MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
MonoObject *mono_object_new_from_token(MonoDomain *domain, MonoImage *image,
                                       guint32 token) {
  MONO_REQ_GC_UNSAFE_MODE;
...
MonoObject *mono_object_clone_checked(MonoObject *obj_raw, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  HANDLE_FUNCTION_ENTER();
  MONO_HANDLE_DCL(MonoObject, obj);
...
MonoObjectHandle mono_object_clone_handle(MonoObjectHandle obj,
                                          MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * Copies the content of one array to another with exactly the same type and
 * size.
 */
void mono_array_full_copy(MonoArray *src, MonoArray *dest) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (mono_gc_is_moving()) {
    ...
    if (m_class_is_valuetype(element_class)) {
      if (m_class_has_references(element_class))
        mono_value_copy_array_internal(
            dest, 0, mono_array_addr_with_size_fast(src, 0, 0),
            mono_array_length_internal(src));
      else
        mono_gc_memmove_atomic(&dest->vector, &src->vector, size);
    } else {
      mono_array_memcpy_refs_internal(dest, 0, src, 0,
                                      mono_array_length_internal(src));
    }
  } else {
    mono_gc_memmove_atomic(&dest->vector, &src->vector, size);
 ...
 */
MonoArrayHandle mono_array_clone_in_domain(MonoDomain *domain,
  ...
  /* Pin source array here - if bounds is non-NULL, it's a pointer into the
   * object data */
  uint32_t src_handle = mono_gchandle_from_handle(
      MONO_HANDLE_CAST(MonoObject, array_handle), TRUE);
  ...
  array_full_copy_unchecked_size(MONO_HANDLE_RAW(array_handle),
                                 MONO_HANDLE_RAW(o), klass, size);
 ...
 * failure returns NULL and sets \p error.
 */
MonoArray *mono_array_clone_checked(MonoArray *array_raw, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  HANDLE_FUNCTION_ENTER();
  ...
  MonoArrayHandle result =
      mono_array_clone_in_domain(MONO_HANDLE_DOMAIN(array), array, error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}
...
gboolean mono_array_calc_byte_len(MonoClass *klass, uintptr_t len,
                                  uintptr_t *res) {
  MONO_REQ_GC_NEUTRAL_MODE;
 ...
 * \param lower_bounds lower bounds for each dimension in the array (may be
 * NULL) This routine creates a new array object with the given dimensions,
 * lower bounds and type.
 */
MonoArray *mono_array_new_full(MonoDomain *domain, MonoClass *array_class,
                               uintptr_t *lengths, intptr_t *lower_bounds) {
  ERROR_DECL(error);
  MonoArray *array = mono_array_new_full_checked(domain, array_class, lengths,
                                                 lower_bounds, error);
  mono_error_cleanup(error);
...
MonoArray *mono_array_new_full_checked(MonoDomain *domain,
   ...
   */
  if (array_class_rank == 1 &&
      ((m_class_get_byval_arg(array_class)->type == MONO_TYPE_SZARRAY) ||
  ...
  if (G_UNLIKELY(!o)) {
    mono_error_set_out_of_memory(
        error, "Could not allocate %" G_GSIZE_FORMAT "d bytes",
        (gsize)byte_len);
    return NULL;
  }
 ...
 * On failure returns NULL and sets \p error.
 */
MonoArray *mono_array_new_checked(MonoDomain *domain, MonoClass *eclass,
                                  uintptr_t n, MonoError *error) {
  MonoClass *ac;
...
MonoArray *ves_icall_array_new(MonoDomain *domain, MonoClass *eclass,
                               uintptr_t n) {
  ERROR_DECL(error);
...
MonoArray *mono_array_new_specific_checked(MonoVTable *vtable, uintptr_t n,
                                           MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
MonoArrayHandle mono_array_new_specific_handle(MonoVTable *vtable, uintptr_t n,
                                               MonoError *error) {
  // FIXMEcoop invert relationship with mono_array_new_specific_checked
  return MONO_HANDLE_NEW(MonoArray,
                         mono_array_new_specific_checked(vtable, n, error));
}

MonoArray *ves_icall_array_new_specific(MonoVTable *vtable, uintptr_t n) {
  ERROR_DECL(error);
  MonoArray *arr = mono_array_new_specific_checked(vtable, n, error);
  mono_error_set_pending_exception(error);
...
MonoString *mono_string_empty_internal(MonoDomain *domain) {
  g_assert(domain);
...
MonoStringHandle mono_string_empty_handle(MonoDomain *domain) {
 ...
 */
MonoString *mono_string_new_utf16(MonoDomain *domain, const mono_unichar2 *text,
                                  gint32 len) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
MonoString *mono_string_new_utf16_checked(MonoDomain *domain,
                                          const gunichar2 *text, gint32 len,
                                          MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * On error, returns NULL and sets \p error.
 */
MonoStringHandle mono_string_new_utf16_handle(MonoDomain *domain,
                                              const gunichar2 *text, gint32 len,
                                              MonoError *error) {
  return MONO_HANDLE_NEW(
      MonoString, mono_string_new_utf16_checked(domain, text, len, error));
}
 ...
 * \param len the length of the string
 * \param error set on failure.
 * \returns A newly created string object which contains \p text. On failure
 * returns NULL and sets \p error.
 */
static MonoString *mono_string_new_utf32_checked(MonoDomain *domain,
                                                 const mono_unichar4 *text,
                                                 gint32 len, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * \returns A newly created string object which contains \p text.
 */
MonoString *mono_string_new_utf32(MonoDomain *domain, const mono_unichar4 *text,
                                  gint32 len) {
  ERROR_DECL(error);
...
MonoStringHandle mono_string_new_size_handle(MonoDomain *domain, gint32 len,
                                             MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  /* check for overflow */
  if (len < 0 ||
      len > ((SIZE_MAX - G_STRUCT_OFFSET(MonoString, chars) - 8) / 2)) {
  ...
  if (G_UNLIKELY(MONO_HANDLE_IS_NULL(s)))
    mono_error_set_out_of_memory(
        error, "Could not allocate %" G_GSIZE_FORMAT " bytes", size);

  return s;
}

MonoString *mono_string_new_size_checked(MonoDomain *domain, gint32 length,
                                         MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  HANDLE_FUNCTION_RETURN_OBJ(
      mono_string_new_size_handle(domain, length, error));
}
 ...
 * \returns A newly created string object which contains \p text.
 */
MonoString *mono_string_new_len(MonoDomain *domain, const char *text,
                                guint length) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
 ...
 * failure returns NULL and sets \p error.
 */
MonoStringHandle mono_string_new_utf8_len(MonoDomain *domain, const char *text,
                                          guint length, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  ut =
      eg_utf8_to_utf16_with_nuls(text, length, NULL, &items_written, &eg_error);
  ...
  if (eg_error) {
    ...
    // mono_error_set_execution_engine (error, "String conversion error: %s",
    // eg_error->message);
...
MonoString *mono_string_new_len_checked(MonoDomain *domain, const char *text,
                                        guint length, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  HANDLE_FUNCTION_RETURN_OBJ(
      mono_string_new_utf8_len(domain, text, length, error));
}

static MonoString *mono_string_new_internal(MonoDomain *domain,
                                            const char *text) {
  ERROR_DECL(error);
  MonoString *res = NULL;
...
MonoString *mono_string_new(MonoDomain *domain, const char *text) {
  MONO_EXTERNAL_ONLY_GC_UNSAFE(MonoString *,
                               mono_string_new_internal(domain, text));
}
 ...
 */
MonoString *mono_string_new_checked(MonoDomain *domain, const char *text,
                                    MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  /*FIXME g_utf8_get_char, g_utf8_next_char and g_utf8_validate are not part of
   * eglib.*/
#if 0
    gunichar2 *str;
    const gchar *end;
 ...
 * \param text a pointer to an wtf8 string (see
 * https://simonsapin.github.io/wtf-8/) \param length number of bytes in \p text
 * to consider \param merror set on error \returns A newly created string object
 * which contains \p text. On error returns NULL and sets \p merror.
 */
MonoString *mono_string_new_wtf8_len_checked(MonoDomain *domain,
                                             const char *text, guint length,
                                             MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
MonoStringHandle mono_string_new_wrapper_internal_impl(const char *text,
                                                       MonoError *error) {
  return MONO_HANDLE_NEW(MonoString,
 ...
 */
MonoObject *mono_value_box(MonoDomain *domain, MonoClass *klass,
                           gpointer value) {
  MonoObject *result;
  MONO_ENTER_GC_UNSAFE;
  ERROR_DECL(error);
 ...
 * returns NULL and sets \p error.
 */
MonoObjectHandle mono_value_box_handle(MonoDomain *domain, MonoClass *klass,
                                       gpointer value, MonoError *error) {
  ...
  if (G_UNLIKELY(m_class_is_byreflike(klass))) {
    ...
    mono_error_set_execution_engine(error, "Cannot box IsByRefLike type %s",
                                    full_name);
    g_free(full_name);
    return NULL_HANDLE;
  }
 ...
 * returns NULL and sets \p error.
 */
MonoObject *mono_value_box_checked(MonoDomain *domain, MonoClass *klass,
                                   gpointer value, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  HANDLE_FUNCTION_RETURN_OBJ(
      mono_value_box_handle(domain, klass, value, error));
}

void mono_value_copy_internal(gpointer dest, gconstpointer src,
                              MonoClass *klass) {
  MONO_REQ_GC_UNSAFE_MODE;
...
void mono_value_copy_array_internal(MonoArray *dest, int dest_idx,
                                    gconstpointer src, int count) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  g_assert(size ==
           mono_class_value_size(
               m_class_get_element_class(mono_object_class(dest)), NULL));
  // FIXME remove (gpointer) cast.
  mono_gc_wbarrier_value_copy_internal(
      d, (gpointer)src, count,
      m_class_get_element_class(mono_object_class(dest)));
}

void mono_value_copy_array_handle(MonoArrayHandle dest, int dest_idx,
                                  gconstpointer src, int count) {
 ...
 * Copy \p count valuetype items from \p src to the array \p dest at index \p
 * dest_idx. This function must be used when \p klass contains references
 * fields. Overlap is handled.
 */
void mono_value_copy_array(MonoArray *dest, int dest_idx, void *src,
                           int count) {
  MONO_EXTERNAL_ONLY_VOID(
      mono_value_copy_array_internal(dest, dest_idx, src, count));
}

MonoVTable *mono_object_get_vtable_internal(MonoObject *obj) {
...
MonoDomain *mono_object_get_domain_internal(MonoObject *obj) {
  MONO_REQ_GC_UNSAFE_MODE;
...
guint mono_object_get_size_internal(MonoObject *o) {
  MONO_REQ_GC_UNSAFE_MODE;

  MonoClass *klass = mono_object_class(o);
  if (klass == mono_defaults.string_class) {
    return MONO_SIZEOF_MONO_STRING +
           2 * mono_string_length_internal((MonoString *)o) + 2;
  } else if (o->vtable->rank) {
    MonoArray *array = (MonoArray *)o;
    size_t size =
        MONO_SIZEOF_MONO_ARRAY +
        mono_array_element_size(klass) * mono_array_length_internal(array);
    if (array->bounds) {
      size += 3;
      size &= ~3;
 ...
 */
unsigned mono_object_get_size(MonoObject *o) {
 ...
 */
MonoObject *mono_object_isinst(MonoObject *obj_raw, MonoClass *klass) {
  HANDLE_FUNCTION_ENTER();
 ...
 * On failure returns NULL and sets \p error.
 */
MonoObject *mono_object_isinst_checked(MonoObject *obj_raw, MonoClass *klass,
                                       MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * On failure returns NULL and sets \p error.
 */
MonoObjectHandle mono_object_handle_isinst(MonoObjectHandle obj,
                                           MonoClass *klass, MonoError *error) {
  error_init(error);
  ...
  if (!MONO_HANDLE_IS_NULL(obj) &&
      mono_class_is_assignable_from_internal(klass, mono_handle_class(obj)))
...
MonoObject *mono_object_isinst_mbyref(MonoObject *obj_raw, MonoClass *klass) {
  ...
  mono_error_cleanup(
      error); /* FIXME better API that doesn't swallow the error */
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

MonoObjectHandle mono_object_handle_isinst_mbyref(MonoObjectHandle obj,
                                                  MonoClass *klass,
                                                  MonoError *error) {
...
gboolean mono_object_handle_isinst_mbyref_raw(MonoObjectHandle obj,
                                              MonoClass *klass,
                                              MonoError *error) {
  error_init(error);
  ...
  if (mono_class_is_interface(klass)) {
    ...
    /*If the above check fails we are in the slow path of possibly raising an
       exception. So it's ok to it this way.*/
  ...
  if (mono_class_is_transparent_proxy(vt->klass)) {
    MonoBoolean custom_type_info = MONO_HANDLE_GETVAL(
        MONO_HANDLE_CAST(MonoTransparentProxy, obj), custom_type_info);
    if (!custom_type_info)
      goto leave;
    MonoDomain *domain = mono_domain_get();
    ...
    im = mono_class_get_method_from_name_checked(rpklass, "CanCastTo", -1, 0,
                                                 error);
    goto_if_nok(error, leave);
    if (!im) {
      mono_error_set_not_supported(error, "Linked away.");
    ...
    MonoReflectionTypeHandle reftype = mono_type_get_object_handle(
        domain, m_class_get_byval_arg(klass), error);
    goto_if_nok(error, leave);
    ...
    MonoObject *res =
        mono_runtime_invoke_checked(im, MONO_HANDLE_RAW(rp), pa, error);
    goto_if_nok(error, leave);

    if (*(MonoBoolean *)mono_object_unbox_internal(res)) {
      /* Update the vtable of the remote type, so it can safely cast to this new
       * type */
 ...
 */
MonoObject *mono_object_castclass_mbyref(MonoObject *obj_raw,
                                         MonoClass *klass) {
  MONO_REQ_GC_UNSAFE_MODE;
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  ...
  if (MONO_HANDLE_IS_NULL(obj))
    goto leave;
  MONO_HANDLE_ASSIGN(result,
                     mono_object_handle_isinst_mbyref(obj, klass, error));
  mono_error_cleanup(error);
leave:
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

static MonoStringHandle mono_string_get_pinned(MonoStringHandle str,
                                               MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  MonoStringHandle news = MONO_HANDLE_CAST(
      MonoString, mono_gc_alloc_handle_pinned_obj(
                      MONO_HANDLE_GETVAL(str, object.vtable), size));
...
MonoStringHandle mono_string_is_interned_lookup(MonoStringHandle str,
                                                gboolean insert,
                                                MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  ldstr_lock();
  MonoString *res =
      (MonoString *)mono_g_hash_table_lookup(ldstr_table, MONO_HANDLE_RAW(str));
  ldstr_unlock();
  if (res)
  ...
  if (res)
    ...
    mono_g_hash_table_insert_internal(ldstr_table, MONO_HANDLE_RAW(s),
                                      MONO_HANDLE_RAW(s));
  ldstr_unlock();
  return s;
}
 ...
 */
MonoString *mono_string_is_interned(MonoString *str_raw) {
  ERROR_DECL(error);
  HANDLE_FUNCTION_ENTER();
 ...
 */
MonoString *mono_string_intern(MonoString *str_raw) {
  ERROR_DECL(error);
  HANDLE_FUNCTION_ENTER();
  MONO_HANDLE_DCL(MonoString, str);
  MONO_ENTER_GC_UNSAFE;
 ...
 * On failure returns NULL and sets \p error.
 */
MonoString *mono_ldstr_checked(MonoDomain *domain, MonoImage *image,
                               guint32 idx, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  error_init(error);
  ...
  if (image->dynamic) {
    MONO_HANDLE_ASSIGN_RAW(str,
                           (MonoString *)mono_lookup_dynamic_token(
                               image, MONO_TOKEN_STRING | idx, NULL, error));
    goto exit;
  }
  if (!mono_verifier_verify_string_signature(image, idx, error))
    goto exit;
  mono_ldstr_metadata_sig(domain, mono_metadata_user_string(image, idx), str,
                          error);
exit:
  HANDLE_FUNCTION_RETURN_OBJ(str);
}

MonoStringHandle mono_ldstr_handle(MonoDomain *domain, MonoImage *image,
                                   guint32 idx, MonoError *error) {
  // FIXME invert mono_ldstr_handle and mono_ldstr_checked.
  return MONO_HANDLE_NEW(MonoString,
                         mono_ldstr_checked(domain, image, idx, error));
}

char *mono_string_from_blob(const char *str, MonoError *error) {
...
static void mono_ldstr_metadata_sig(MonoDomain *domain, const char *sig,
  ...
  const gsize len =
      mono_metadata_decode_blob_size(sig, &sig) / sizeof(gunichar2);

  // FIXMEcoop excess handle, use mono_string_new_utf16_checked and
  // string_handle parameter

  MonoStringHandle o =
      mono_string_new_utf16_handle(domain, (gunichar2 *)sig, len, error);
  return_if_nok(error);
  ...
  MONO_HANDLE_ASSIGN_RAW(string_handle,
                         MONO_HANDLE_RAW(mono_string_intern_checked(o, error)));
}
 ...
 * of an object.
 */
char *mono_ldstr_utf8(MonoImage *image, guint32 idx, MonoError *error) {
  const char *str;
  size_t len2;
  long written = 0;
  char *as;
  ...
  /* g_utf16_to_utf8 may not be able to complete the conversion (e.g. NULL
   * values were found, #335488) */
  if (len2 > written) {
    /* allocate the total length and copy the part of the string that has been
     * converted */
 ...
 * \deprecated Use \c mono_string_to_utf8_checked_internal to avoid having an
 * exception arbitrarily raised. \returns the UTF-8 representation for \p s. The
 * resulting buffer needs to be freed with \c mono_free().
 */
 ...
 */
char *mono_utf16_to_utf8len(const gunichar2 *s, gsize slength,
                            gsize *utf8_length, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * mono_utf16_to_utf8:
 */
char *mono_utf16_to_utf8(const gunichar2 *s, gsize slength, MonoError *error) {
  gsize utf8_length = 0;
  return mono_utf16_to_utf8len(s, slength, &utf8_length, error);
}

char *mono_string_to_utf8_checked_internal(MonoString *s, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
char *mono_string_to_utf8len(MonoStringHandle s, gsize *utf8len,
                             MonoError *error) {
  *utf8len = 0;
  ...
  utf8 = mono_utf16_to_utf8len(mono_string_chars_internal(MONO_HANDLE_RAW(s)),
                               mono_string_handle_length(s), utf8len, error);
...
char *mono_string_to_utf8_checked(MonoString *string_obj, MonoError *error) {
  MONO_EXTERNAL_ONLY_GC_UNSAFE(
      char *, mono_string_to_utf8_checked_internal(string_obj, error));
}

char *mono_string_handle_to_utf8(MonoStringHandle s, MonoError *error) {
 ...
 * The resulting buffer should be freed with \c mono_free().
 */
char *mono_string_to_utf8_ignore(MonoString *s) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  as = g_utf16_to_utf8(mono_string_chars_internal(s), s->length, NULL, &written,
                       NULL);
...
mono_unichar2 *mono_string_to_utf16_internal_impl(MonoStringHandle s,
                                                  MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  mono_unichar2 *const as =
      (mono_unichar2 *)g_malloc((length + 1) * sizeof(*as));
  if (as) {
    as[length] = 0;
    if (length)
      memcpy(as, mono_string_chars_internal(MONO_HANDLE_RAW(s)),
             length * sizeof(*as));
  }
  return as;
}
...
mono_unichar2 *mono_string_to_utf16(MonoString *string_obj) {
  if (!string_obj)
    return NULL;
  MONO_EXTERNAL_ONLY(mono_unichar2 *,
                     mono_string_to_utf16_internal(string_obj));
}

mono_unichar4 *mono_string_to_utf32_internal_impl(MonoStringHandle s,
                                                  MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  return g_utf16_to_ucs4(MONO_HANDLE_RAW(s)->chars,
                         mono_string_handle_length(s), NULL, NULL, NULL);
}
...
mono_unichar4 *mono_string_to_utf32(MonoString *string_obj) {
  MONO_EXTERNAL_ONLY(mono_unichar4 *,
 ...
 */
MonoString *mono_string_from_utf16_checked(const gunichar2 *data,
                                           MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  error_init(error);
  if (!data)
    return NULL;
  return mono_string_new_utf16_checked(mono_domain_get(), data,
                                       g_utf16_len(data), error);
}
 ...
 * \returns a \c MonoString.
 */
MonoString *mono_string_from_utf32(/*const*/ mono_unichar4 *data) {
  ERROR_DECL(error);
 ...
 */
MonoString *mono_string_from_utf32_checked(const mono_unichar4 *data,
                                           MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * \param s a \c System.String
 * Same as \c mono_string_to_utf8, but allocate the string from the image
 * mempool.
 */
char *mono_string_to_utf8_image(MonoImage *image, MonoStringHandle s,
                                MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;

  return mono_string_to_utf8_internal(NULL, image, MONO_HANDLE_RAW(s),
                                      error); /* FIXME pin the string */
}
...
void mono_install_eh_callbacks(MonoRuntimeExceptionHandlingCallbacks *cbs) {
  eh_callbacks = *cbs;
}

MonoRuntimeExceptionHandlingCallbacks *mono_get_eh_callbacks(void) {
  return &eh_callbacks;
}

void mono_raise_exception_internal(MonoException *ex) {
  /* raise_exception doesn't return, so the transition to GC Unsafe is
   * unbalanced */
  MONO_STACKDATA(stackdata);
  mono_threads_enter_gc_unsafe_region_unbalanced_with_info(
      mono_thread_info_current(), &stackdata);
 ...
 * Signal the runtime that the exception \p ex has been raised in unmanaged
 * code. DEPRECATED. DO NOT ADD NEW CALLERS FOR THIS FUNCTION.
 */
 ...
 */
void mono_raise_exception_deprecated(MonoException *ex) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 * DEPRECATED. DO NOT ADD NEW CALLERS FOR THIS FUNCTION.
 */
void mono_reraise_exception_deprecated(MonoException *ex) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
void mono_raise_exception_with_context(MonoException *ex, MonoContext *ctx) {
  MONO_REQ_GC_UNSAFE_MODE;
 ...
 */
MonoWaitHandle *mono_wait_handle_new(MonoDomain *domain, HANDLE handle,
                                     MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  error_init(error);
  res = (MonoWaitHandle *)mono_object_new_checked(
      domain, mono_defaults.manualresetevent_class, error);
  return_val_if_nok(error, NULL);

  /* Even though this method is virtual, it's safe to invoke directly, since the
   * object type matches.  */
  if (!handle_set)
    handle_set = mono_class_get_property_from_name_internal(
                     mono_defaults.manualresetevent_class, "Handle")
                     ->set;
...
mono_wait_handle_get_handle(MonoWaitHandle *handle) {
  ...
  f_safe_handle = mono_class_get_field_from_name_full(
      mono_defaults.manualresetevent_class, "safeWaitHandle", NULL);
...
MonoMethod *mono_get_context_capture_method(void) {
  ...
  method = mono_class_get_method_from_name_checked(execution_context, "Capture",
                                                   0, 0, error);
  mono_error_assert_ok(error);
...
static MonoObject *mono_runtime_capture_context(MonoDomain *domain,
                                                MonoError *error) {
#ifdef HOST_WASM
  return mono_runtime_invoke_checked(mono_get_context_capture_method(), NULL,
                                     NULL, error);
#else
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (!domain->capture_context_runtime_invoke ||
      !domain->capture_context_method) {
    MonoMethod *method = mono_get_context_capture_method();
    MonoMethod *wrapper;
    if (!method)
      return NULL;
    wrapper = mono_marshal_get_runtime_invoke(method, FALSE);
    domain->capture_context_runtime_invoke =
        mono_compile_method_checked(wrapper, error);
    return_val_if_nok(error, NULL);
  ...
  runtime_invoke =
      (RuntimeInvokeFunction)domain->capture_context_runtime_invoke;
 ...
 * On failure returns NULL and sets \p error.
 */
MonoAsyncResult *mono_async_result_new(MonoDomain *domain, HANDLE handle,
  ...
  error_init(error);
  MonoAsyncResult *res = (MonoAsyncResult *)mono_object_new_checked(
      domain, mono_class_get_asyncresult_class(), error);
  return_val_if_nok(error, NULL);
...
MonoObjectHandle ves_icall_System_Runtime_Remoting_Messaging_AsyncResult_Invoke(
    MonoAsyncResultHandle aresh, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (!ac) {
    MONO_HANDLE_ASSIGN_RAW(
        res, mono_runtime_delegate_invoke_checked(
                 ares->async_delegate, (void **)&ares->async_state, error));
    return_val_if_nok(error, NULL_HANDLE);
  } else {
    ...
    MONO_HANDLE_ASSIGN_RAW(
        res,
        mono_message_invoke(mono_thread_info_current_var, ares->async_delegate,
                            ac->msg, &ac->msg->exc, &ac->out_args, error));

    /* The exit side of the invoke must not be aborted as it would leave the
    ...
    MonoObjectHandle handle =
        MONO_HANDLE_NEW(MonoObject, NULL); // Create handle outside of lock.
    ...
    if (ac->cb_method) {
      ...
      mono_runtime_invoke_checked(ac->cb_method, ac->cb_target,
                                  (gpointer *)&ares, error);
    }
...
gboolean mono_message_init(MonoDomain *domain, MonoMethodMessage *this_obj,
  ...
  init_message_method = mono_class_get_method_from_name_checked(
      mono_defaults.mono_method_message_class, "InitMessage", 2, 0, error);
  mono_error_assert_ok(error);
 ...
 * This is used to call \c RealProxy::Invoke(). \c RealProxy::Invoke() returns
 * an \c IMessage interface and it is not trivial to extract results from there.
 * So we call an helper method \c PrivateInvoke instead of calling \c
 * RealProxy::Invoke() directly. \returns the result object.
 */
MonoObject *mono_remoting_invoke(MonoObject *real_proxy, MonoMethodMessage *msg,
  ...
  /*static MonoObject *(*invoke) (gpointer, gpointer, MonoObject **, MonoArray
   * **) = NULL;*/

  if (!im) {
    im = mono_class_get_method_from_name_checked(mono_defaults.real_proxy_class,
...
mono_message_invoke(MonoThreadInfo *mono_thread_info_current_var,
  ...
  if (target && mono_object_is_transparent_proxy(target)) {
    ...
    gboolean const is_contextbound =
        mono_class_is_contextbound(tp->remote_class->proxy_class);

    MonoObject *const context =
        is_contextbound ? MONO_HANDLE_RAW(MONO_HANDLE_NEW(
                              MonoObject, (MonoObject *)mono_context_get()))
                        : NULL;
  ...
  *exc = NULL;
  ...
  MonoObject *ret = mono_runtime_try_invoke_array(
      method,
      m_class_is_valuetype(method->klass) ? mono_object_unbox_internal(target)
                                          : target,
      msg->args, exc, error);
  return_val_if_nok(error, NULL);
 ...
 *
 * Returns: the ToString override for @obj. If @obj is a valuetype, @target is
 * unboxed otherwise it's @obj.
 */
static MonoMethod *prepare_to_string_method(MonoObject *obj, void **target) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  *target = obj;
  ...
  ERROR_DECL(error);
  to_string = mono_class_get_method_from_name_checked(
      mono_get_object_class(), "ToString", 0,
      METHOD_ATTRIBUTE_VIRTUAL | METHOD_ATTRIBUTE_PUBLIC, error);
  mono_error_assert_ok(error);
 ...
 * method cannot be invoked sets \p error, if it raises an exception sets \p
 * exc, and returns NULL.
 */
MonoString *mono_object_try_to_string(MonoObject *obj, MonoObject **exc,
                                      MonoError *error) {
  g_assert(exc);
  error_init(error);
  void *target;
  MonoMethod *method = prepare_to_string_method(obj, &target);
  return (MonoString *)mono_runtime_try_invoke(method, target, NULL, exc,
                                               error);
}

static char *get_native_backtrace(MonoException *exc_raw) {
  HANDLE_FUNCTION_ENTER();
 ...
 */
void mono_print_unhandled_exception_internal(MonoObject *exc) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (exc == (MonoObject *)mono_object_domain(exc)->out_of_memory_ex) {
    ...
    message = g_strdup(
        "StackOverflowException"); // if we OVF, we can't expect to have stack
                                   // space to JIT Exception::ToString.
    free_message = TRUE;
  } else {
   ...
   *exc->vtable->klass->name_space, exc->vtable->klass->name, message);
   */
  g_printerr("\nUnhandled Exception:\n%s\n", message);
 ...
 */
gboolean mono_delegate_ctor_with_method(MonoObjectHandle this_obj,
                                        MonoObjectHandle target, gpointer addr,
                                        MonoMethod *method, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
...
#ifndef DISABLE_REMOTING
  if (!MONO_HANDLE_IS_NULL(target) &&
      mono_class_is_transparent_proxy(mono_handle_class(target))) {
    if (callbacks.interp_get_remoting_invoke) {
      MONO_HANDLE_SETVAL(
          delegate, interp_method, gpointer,
          callbacks.interp_get_remoting_invoke(method, addr, error));
    } else {
      g_assert(method);
      method = mono_marshal_get_remoting_invoke(method, error);
      return_val_if_nok(error, FALSE);
      MONO_HANDLE_SETVAL(delegate, method_ptr, gpointer,
  ...
  MONO_HANDLE_SETVAL(
      delegate, invoke_impl, gpointer,
      callbacks.create_delegate_trampoline(MONO_HANDLE_DOMAIN(delegate),
 ...
 * On failure returns FALSE and sets \p error.
 */
gboolean mono_delegate_ctor(MonoObjectHandle this_obj, MonoObjectHandle target,
                            gpointer addr, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (!ji && domain != mono_get_root_domain())
    ji = mono_jit_info_table_find(mono_get_root_domain(),
 ...
 */
MonoMethodMessage *
mono_method_call_message_new(MonoMethod *method, gpointer *params,
                             MonoMethod *invoke, MonoDelegate **cb,
                             MonoObject **state, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  msg = (MonoMethodMessage *)mono_object_new_checked(
      domain, mono_defaults.mono_method_message_class, error);
  return_val_if_nok(error, NULL);

  if (invoke) {
    MonoReflectionMethod *rm =
        mono_method_get_object_checked(domain, invoke, NULL, error);
    return_val_if_nok(error, NULL);
    ...
    MonoReflectionMethod *rm =
        mono_method_get_object_checked(domain, method, NULL, error);
    return_val_if_nok(error, NULL);
    mono_message_init(domain, msg, rm, NULL, error);
    return_val_if_nok(error, NULL);
 ...
 */
void mono_method_return_message_restore(MonoMethod *method, gpointer *params,
                                        MonoArray *out_args, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  for (i = 0, j = 0; i < sig->param_count; i++) {
    ...
    if (pt->byref) {
      ...
      if (MONO_TYPE_IS_REFERENCE(pt)) {
        mono_gc_wbarrier_generic_store_internal(*((MonoObject ***)params[i]),
                                                (MonoObject *)arg);
      } else {
        if (arg) {
          ...
          if (m_class_has_references(klass))
            mono_gc_wbarrier_value_copy_internal(
                *((gpointer *)params[i]),
                mono_object_get_data((MonoObject *)arg), 1, klass);
          else
            mono_gc_memmove_atomic(*((gpointer *)params[i]),
                                   mono_object_get_data((MonoObject *)arg),
                                   size);
        } else {
          size = mono_class_value_size(mono_class_from_mono_type_internal(pt),
                                       NULL);
 ...
 * transparent proxy objects. \p this points to such TP, \p klass is the class
 * of the object containing \p field. \p res is a storage location which can be
 * used to store the result.
 * \returns an address pointing to the value of field.
 */
gpointer mono_load_remote_field(MonoObject *this_obj, MonoClass *klass,
                                MonoClassField *field, gpointer *res) {
  gpointer result;
 ...
 * of the object containing \p field. \p res is a storage location which can be
 * used to store the result.
 * \returns an address pointing to the value of field.  On failure returns NULL
 * and sets \p error.
 */
gpointer mono_load_remote_field_checked(MonoObject *this_obj, MonoClass *klass,
                                        MonoClassField *field, gpointer *res,
                                        MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (mono_class_is_contextbound(tp->remote_class->proxy_class) &&
      tp->rp->context == (MonoObject *)mono_context_get()) {
  ...
  getter = mono_class_get_method_from_name_checked(mono_defaults.object_class,
  ...
  field_name =
      mono_string_new_checked(domain, mono_field_get_name(field), error);
  goto_if_nok(error, return_null);
  ...
  mono_gc_wbarrier_generic_store_internal(
      res, mono_array_get_internal(out_args, MonoObject *, 0));
 ...
 */
MonoObject *mono_load_remote_field_new(MonoObject *this_obj, MonoClass *klass,
                                       MonoClassField *field) {
  MonoObject *result;
 ...
 * This method is called by the runtime on attempts to load fields of
 * transparent proxy objects. \p this points to such TP, \p klass is the class
 * of the object containing \p field. \returns a freshly allocated object
 * containing the value of the field.  On failure returns NULL and sets \p
 * error.
 */
MonoObject *mono_load_remote_field_new_checked(MonoObject *this_obj,
                                               MonoClass *klass,
                                               MonoClassField *field,
                                               MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  tp_load = mono_class_get_method_from_name_checked(
      mono_defaults.transparent_proxy_class, "LoadRemoteFieldNew", -1, 0,
      error);
  return_val_if_nok(error, NULL);
  if (!tp_load) {
    mono_error_set_not_supported(error, "Linked away.");
 ...
 * transparent proxy objects. \p this_obj points to such TP, \p klass is the
 * class of the object containing \p field. \p val is the new value to store in
 * \p field.
 */
void mono_store_remote_field(MonoObject *this_obj, MonoClass *klass,
                             MonoClassField *field, gpointer val) {
  MONO_ENTER_GC_UNSAFE;
 ...
 * transparent proxy objects. \p this_obj points to such TP, \p klass is the
 * class of the object containing \p field. \p val is the new value to store in
 * \p field. \returns on success returns TRUE, on failure returns FALSE and sets
 * \p error.
 */
gboolean mono_store_remote_field_checked(MonoObject *this_obj, MonoClass *klass,
                                         MonoClassField *field, gpointer val,
                                         MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  return mono_store_remote_field_new_checked(this_obj, klass, field, arg,
                                             error);
}
 ...
 * Missing documentation
 */
void mono_store_remote_field_new(MonoObject *this_obj, MonoClass *klass,
                                 MonoClassField *field, MonoObject *arg) {
  MONO_ENTER_GC_UNSAFE;
 ...
 * Missing documentation
 */
gboolean mono_store_remote_field_new_checked(MonoObject *this_obj,
                                             MonoClass *klass,
                                             MonoClassField *field,
                                             MonoObject *arg,
                                             MonoError *error) {
  ...
  tp_store = mono_class_get_method_from_name_checked(
      mono_defaults.transparent_proxy_class, "StoreRemoteField", -1, 0, error);
  return_val_if_nok(error, FALSE);
  if (!tp_store) {
    mono_error_set_not_supported(error, "Linked away.");
    return FALSE;
 ...
 * This is only needed on some platforms.
 */
gpointer mono_get_addr_from_ftnptr(gpointer descr) {
 ...
 * \returns the length in characters of the string
 */
int mono_string_handle_length(MonoStringHandle s) {
  MONO_REQ_GC_UNSAFE_MODE;
...
char *mono_array_addr_with_size(MonoArray *array, int size, uintptr_t idx) {
  MONO_EXTERNAL_ONLY(char *,
                     mono_array_addr_with_size_internal(array, size, idx));
}

MonoArray *mono_glist_to_array(GList *list, MonoClass *eclass,
                               MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  MonoArray *res;
  int len, i;
 ...
 */
gpointer mono_vtype_get_field_addr(gpointer vtype, MonoClassField *field) {
 ...
 * \param element_type A C type name, this macro will use the sizeof(type) to
 * determine the element size \param index index into the array \param value
 * value to set Value Type version: This sets the \p index's element of the \p
 * array with elements of size sizeof(type) to the provided \p value.
 *
 ...
 */
void mono_array_set(MonoArray *array, Type element_type, uintptr_t index,
                    Value value) {}
 ...
 * \param element_type C element type (example: \c MonoString*, \c int, \c
 * MonoObject*) \param index index into the array
 *
 ...
 */
Type mono_array_get_internal(MonoArray *array, Type element_type,
                             uintptr_t index) {}
#endif
