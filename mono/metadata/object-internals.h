...
#define MONO_CHECK_ARG(arg, expr, retval)                                      \
  do {                                                                         \
    if (G_UNLIKELY(!(expr))) {                                                 \
      if (0) {                                                                 \
        (void)(arg);                                                           \
      ...
      ERROR_DECL(error);                                                       \
      mono_error_set_argument_format(error, #arg, "assertion `%s' failed",     \
                                     #expr);                                   \
...
#define MONO_CHECK_ARG_NULL_NAMED(arg, argname, retval)                        \
  do {                                                                         \
    if (G_UNLIKELY(!(arg))) {                                                  \
      ERROR_DECL(error);                                                       \
...
#define MONO_CHECK_ARG_NULL(arg, retval)                                       \
  do {                                                                         \
    if (G_UNLIKELY(!(arg))) {                                                  \
...
#define MONO_CHECK_ARG_NULL_HANDLE(arg, retval)                                \
  do {                                                                         \
...
#define MONO_CHECK_ARG_NULL_HANDLE_NAMED(arg, argname, retval)                 \
  do {                                                                         \
    if (G_UNLIKELY(MONO_HANDLE_IS_NULL(arg))) {                                \
      mono_error_set_argument_null(error, (argname), "");                      \
      return retval;                                                           \
...
#define MONO_CHECK_NULL(arg, retval)                                           \
  do {                                                                         \
    if (G_UNLIKELY(!(arg))) {                                                  \
      ERROR_DECL(error);                                                       \
...
// If you get an error using this macro, you need to manually instantiate the
// MonoClass *foo ## _array cache. See for example object_class_array.
#define mono_array_class_get_cached(eclass)                                    \
  (mono_array_class_get_cached_function((eclass), &(eclass##_array)))

static inline MonoArray *mono_array_new_cached_function(MonoDomain *domain,
                                                        MonoClass *aclass,
                                                        int size,
                                                        MonoError *error) {
...
#define mono_array_new_cached(domain, eclass, size, error)                     \
  mono_array_new_cached_function(                                              \
      (domain), mono_array_class_get_cached(eclass), (size), (error))

static inline MonoArrayHandle
mono_array_new_cached_handle_function(MonoDomain *domain, MonoClass *aclass,
...
#define mono_array_new_cached_handle(domain, eclass, size, error)              \
  mono_array_new_cached_handle_function(                                       \
      (domain), mono_array_class_get_cached(eclass), (size), (error))
...
struct _MonoArray {
  ...
  /* we use mono_64bitaligned_t to ensure proper alignment on platforms that
   * need it */
...
static inline uintptr_t mono_array_handle_length(MonoArrayHandle arr) {
...
#define mono_array_addr_with_size_fast(array, size, index)                     \
  (((char *)(array)->vector) + (size) * (index))

#define mono_array_addr_fast(array, type, index)                               \
  ((type *)(void *)mono_array_addr_with_size_fast(array, sizeof(type), index))
#define mono_array_get_fast(array, type, index)                                \
  (*(type *)mono_array_addr_fast((array), type, (index)))
...
#define mono_array_setref_fast(array, index, value)                            \
  do {                                                                         \
    ...
    mono_gc_wbarrier_set_arrayref_internal((array), __p,                       \
                                           (MonoObject *)(value));             \
    /* *__p = (value);*/                                                       \
  } while (0)
...
// _internal is like _fast, but preserves the preexisting subtlety of the closed
// types of things:
//  	int size
//	uintptr_t idx
// in order to mimic non-_internal but without the GC mode transitions, or at
// least, to avoid the runtime using the embedding API, whether or not it has GC
// mode transitions.
static inline char *
...
#define mono_array_addr_internal(array, type, index)                           \
  ((type *)(void *)mono_array_addr_with_size_internal(array, sizeof(type),     \
                                                      index))
#define mono_array_get_internal(array, type, index)                            \
  (*(type *)mono_array_addr_internal((array), type, (index)))
...
static inline gboolean mono_handle_array_has_bounds(MonoArrayHandle arr) {
  return MONO_HANDLE_GETVAL(arr, bounds) != NULL;
}

static inline void mono_handle_array_get_bounds_dim(MonoArrayHandle arr,
                                                    gint32 dim,
                                                    MonoArrayBounds *bounds) {
...
/* Safely access System.AppDomainSetup from native code.  (struct is in
 * domain-internals.h) */
...
struct _MonoStringBuilder {
  ...
  MonoStringBuilder
      *chunkPrevious; // Link to the block logically before this block
  int chunkLength;    // The index in ChunkChars that represent the end of the
                      // block
  int chunkOffset;    // The logial offset (sum of all characters in previous
                      // blocks)
  int maxCapacity;
};

static inline int mono_string_builder_capacity(MonoStringBuilderHandle sbh) {
...
struct _MonoIUnknownVTable {
  int(STDCALL *QueryInterface)(MonoIUnknown *pUnk, gconstpointer riid,
                               gpointer *ppv);
...
/* Safely access System.Runtime.Remoting.Proxies.TransparentProxy from native
 * code */
...
void mono_gstring_append_thread_name(GString *, MonoInternalThread *);
...
struct _MonoInternalThread {
  ...
  MonoObject obj;
  volatile int lock_thread_id; /* to be used as the pre-shifted thread id in
                                  thin locks. Used for appdomain_ref push/pop */
  ...
  guint64 tid; /* This is accessed as a gsize in the code (so it can hold a
                  64bit pointer on systems that need it), but needs to reserve
                  64 bits of space on all machines as it corresponds to a field
                  in managed code */
  gsize debugger_thread; // FIXME switch to bool as soon as CI testing with
                         // corlib version bump works
...
typedef struct {
  ...
  gpointer (*create_jump_trampoline)(MonoDomain *domain, MonoMethod *method,
                                     gboolean add_sync_wrapper,
                                     MonoError *error);
  gpointer (*create_jit_trampoline)(MonoDomain *domain, MonoMethod *method,
                                    MonoError *error);
  /* used to free a dynamic method */
  void (*free_method)(MonoDomain *domain, MonoMethod *method);
  gpointer (*create_remoting_trampoline)(MonoDomain *domain, MonoMethod *method,
                                         MonoRemotingTarget target,
                                         MonoError *error);
  gpointer (*create_delegate_trampoline)(MonoDomain *domain, MonoClass *klass);
  gpointer (*interp_get_remoting_invoke)(MonoMethod *method, gpointer imethod,
                                         MonoError *error);
...
typedef gboolean (*MonoInternalStackWalk)(MonoStackFrameInfo *frame,
                                          MonoContext *ctx, gpointer data);
typedef gboolean (*MonoInternalExceptionFrameWalk)(MonoMethod *method,
                                                   gpointer ip,
...
typedef struct {
  void (*mono_walk_stack_with_ctx)(MonoInternalStackWalk func, MonoContext *ctx,
                                   MonoUnwindOptions options, void *user_data);
  void (*mono_walk_stack_with_state)(MonoInternalStackWalk func,
  ...
  gboolean (*mono_exception_walk_trace)(MonoException *ex,
                                        ...
                                        gpointer user_data);
  gboolean (*mono_install_handler_block_guard)(
  ...
  void (*mono_register_native_library)(const char *module_path,
...
MonoAsyncResult *mono_async_result_new(MonoDomain *domain, gpointer handle,
                                       MonoObject *state, gpointer data,
                                       MonoObject *object_data,
                                       MonoError *error);
MonoWaitHandle *mono_wait_handle_new(MonoDomain *domain, gpointer handle,
                                     MonoError *error);

gpointer mono_wait_handle_get_handle(MonoWaitHandle *handle);
...
MonoRuntimeCallbacks *mono_get_runtime_callbacks(void);
...
int mono_thread_kill(MonoInternalThread *thread, int signal);

MonoNativeTlsKey mono_thread_get_tls_key(void);

gint32 mono_thread_get_tls_offset(void);

MonoNativeTlsKey mono_domain_get_tls_key(void);

gint32 mono_domain_get_tls_offset(void);
 ...
 *
 *   Fields defined as System.Type in managed code should be defined as
 * MonoObject* in unmanaged structures, and the monotype_cast () function should
 * be used for casting them to MonoReflectionType* to avoid crashes/security
 * issues when encountering instances of user defined subclasses of System.Type.
 */

#define IS_MONOTYPE(obj)                                                       \
  (!(obj) ||                                                                   \
   (m_class_get_image(mono_object_class((obj))) == mono_defaults.corlib &&     \
    ((MonoReflectionType *)(obj))->type != NULL))

#define IS_MONOTYPE_HANDLE(obj) IS_MONOTYPE(MONO_HANDLE_RAW(obj))

/* This should be used for accessing members of Type[] arrays */
#define mono_type_array_get(arr, index)                                        \
  monotype_cast(mono_array_get_internal((arr), gpointer, (index)))

/*
 * Cast an object to MonoReflectionType, making sure it is a System.MonoType or
 * a subclass of it.
 */
static inline MonoReflectionType *monotype_cast(MonoObject *obj) {
...
/* Safely access System.Reflection.Emit.ArrayType (in DerivedTypes.cs) from
 * native code */
...
/* Safely access System.Reflection.Emit.SymbolType and subclasses (in
 * DerivedTypes.cs) from native code */
...
/* Safely access System.Reflection.Emit.GenericTypeParameterBuilder from native
 * code */
...
struct _MonoReflectionGenericClass {
  MonoReflectionType type;
  MonoReflectionType
      *generic_type; /*Can be either a MonoType or a TypeBuilder*/
  MonoArray *type_arguments;
};

/* Safely access System.Reflection.Emit.TypeBuilderInstantiation from native
 * code */
...
/* All MonoInternalThread instances should be pinned, so it's safe to use the
 * raw ptr.  However for uniformity, icall wrapping will make handles anyway. So
 * this is the method for getting the payload.
 */
...
gboolean mono_image_create_pefile(MonoReflectionModuleBuilder *module,
                                  gpointer file, MonoError *error);
...
ICALL_EXPORT
void ves_icall_SymbolType_create_unmanaged_type(MonoReflectionType *type);
...
MonoClass *mono_class_bind_generic_parameters(MonoClass *klass, int type_argc,
                                              MonoType **types,
                                              gboolean is_dynamic);
MonoType *mono_reflection_bind_generic_parameters(MonoReflectionTypeHandle type,
                                                  int type_argc,
                                                  MonoType **types,
                                                  MonoError *error);
void mono_reflection_generic_class_initialize(MonoReflectionGenericClass *type,
                                              MonoArray *fields);

ICALL_EXPORT
MonoReflectionEvent *
ves_icall_TypeBuilder_get_event_info(MonoReflectionTypeBuilder *tb,
...
gpointer mono_reflection_lookup_dynamic_token(MonoImage *image, guint32 token,
...
gboolean mono_image_build_metadata(MonoReflectionModuleBuilder *module,
                                   MonoError *error);
...
MonoArray *mono_array_clone_checked(MonoArray *array, MonoError *error);
...
gboolean mono_upgrade_remote_class(MonoDomain *domain, MonoObjectHandle tproxy,
                                   MonoClass *klass, MonoError *error);

void *mono_load_remote_field_checked(MonoObject *this_obj, MonoClass *klass,
                                     MonoClassField *field, void **res,
                                     MonoError *error);
...
MonoObject *mono_value_box_checked(MonoDomain *domain, MonoClass *klass,
                                   void *val, MonoError *error);

MonoObjectHandle mono_value_box_handle(MonoDomain *domain, MonoClass *klass,
                                       gpointer val, MonoError *error);

MonoObject *mono_nullable_box(gpointer buf, MonoClass *klass, MonoError *error);

MonoObjectHandle mono_nullable_box_handle(gpointer buf, MonoClass *klass,
                                          MonoError *error);

// A code size optimization (source and object) equivalent to MONO_HANDLE_NEW
// (MonoObject, NULL);
MonoObjectHandle mono_new_null(void);
...
typedef gpointer (*MonoImtTrampolineBuilder)(MonoVTable *vtable,
                                             MonoDomain *domain,
                                             MonoIMTCheckItem **imt_entries,
                                             int count, gpointer fail_trunk);
...
#define mono_method_alloc_generic_virtual_trampoline(domain, size)             \
  (g_cast(mono_method_alloc_generic_virtual_trampoline((domain), (size))))
...
MONO_API void *mono_vtable_get_static_field_data(MonoVTable *vt);
...
gboolean mono_monitor_is_il_fastpath_wrapper(MonoMethod *method);
 ...
 */
#define mono_string_intern_checked(str, error)                                 \
  (mono_string_is_interned_lookup((str), TRUE, (error)))
 ...
 */
#define mono_string_is_interned_internal(str, error)                           \
  (mono_string_is_interned_lookup((str), FALSE, (error)))

char *mono_exception_handle_get_native_backtrace(MonoExceptionHandle exc);

char *mono_exception_get_managed_backtrace(MonoException *exc);

void mono_copy_value(MonoType *type, void *dest, void *value,
                     int deref_pointer);

void mono_error_raise_exception_deprecated(MonoError *target_error);

gboolean mono_error_set_pending_exception_slow(MonoError *error);

static inline gboolean mono_error_set_pending_exception(MonoError *error) {
...
MonoObject *mono_object_clone_checked(MonoObject *obj, MonoError *error);

MonoObjectHandle mono_object_clone_handle(MonoObjectHandle obj,
                                          MonoError *error);

MonoObject *mono_object_isinst_checked(MonoObject *obj, MonoClass *klass,
                                       MonoError *error);
...
MonoString *mono_string_new_size_checked(MonoDomain *domain, gint32 len,
                                         MonoError *error);

MonoString *mono_ldstr_checked(MonoDomain *domain, MonoImage *image,
                               uint32_t str_index, MonoError *error);

MonoStringHandle mono_ldstr_handle(MonoDomain *domain, MonoImage *image,
                                   uint32_t str_index, MonoError *error);

MONO_PROFILER_API MonoString *mono_string_new_checked(MonoDomain *domain,
                                                      const char *text,
                                                      MonoError *merror);
...
MonoString *mono_string_from_utf16_checked(const mono_unichar2 *data,
                                           MonoError *error);
...
char *mono_utf16_to_utf8(const mono_unichar2 *s, gsize slength,
                         MonoError *error);

char *mono_utf16_to_utf8len(const mono_unichar2 *s, gsize slength,
                            gsize *utf8_length, MonoError *error);

gboolean mono_runtime_object_init_checked(MonoObject *this_obj,
                                          MonoError *error);

MONO_PROFILER_API MonoObject *mono_runtime_try_invoke(MonoMethod *method,
                                                      void *obj, void **params,
                                                      MonoObject **exc,
                                                      MonoError *error);

// The exc parameter is deliberately missing and so far this has proven to
// reduce code duplication. In particular, if an exception is returned from
// underlying otherwise succeeded call, is set into the MonoError with
// mono_error_set_exception_instance. The result is that caller need only check
// MonoError.
...
void *mono_compile_method_checked(MonoMethod *method, MonoError *error);
...
void mono_gc_wbarrier_object_copy_handle(MonoObjectHandle obj,
 ...
 */
static inline gpointer mono_object_get_data(MonoObject *o) {
  return (guint8 *)o + MONO_ABI_SIZEOF(MonoObject);
}

#define mono_handle_get_data_unsafe(handle)                                    \
  ((gpointer)((guint8 *)MONO_HANDLE_RAW(handle) + MONO_ABI_SIZEOF(MonoObject)))

gpointer mono_vtype_get_field_addr(gpointer vtype, MonoClassField *field);

#define MONO_OBJECT_SETREF_INTERNAL(obj, fieldname, value)                     \
  do {                                                                         \
    mono_gc_wbarrier_set_field_internal(                                       \
        (MonoObject *)(obj), &((obj)->fieldname), (MonoObject *)value);        \
    /*(obj)->fieldname = (value);*/                                            \
  } while (0)

/* This should be used if 's' can reside on the heap */
#define MONO_STRUCT_SETREF_INTERNAL(s, field, value)                           \
  do {                                                                         \
    mono_gc_wbarrier_generic_store_internal(&((s)->field),                     \
                                            (MonoObject *)(value));            \
  } while (0)

static inline gunichar2 *mono_string_chars_internal(MonoString *s) {
  MONO_REQ_GC_UNSAFE_MODE;
  return s->chars;
}

static inline int mono_string_length_internal(MonoString *s) {
  MONO_REQ_GC_UNSAFE_MODE;
  return s->length;
}
...
char *mono_string_to_utf8_checked_internal(MonoString *string_obj,
                                           MonoError *error);

mono_bool mono_string_equal_internal(MonoString *s1, MonoString *s2);

unsigned mono_string_hash_internal(MonoString *s);

int mono_object_hash_internal(MonoObject *obj);

ICALL_EXTERN_C
void mono_value_copy_internal(void *dest, const void *src, MonoClass *klass);
...
static inline gpointer mono_object_unbox_internal(MonoObject *obj) {
...
ICALL_EXPORT
void mono_monitor_exit_internal(MonoObject *obj);
...
MonoMethod *mono_object_get_virtual_method_internal(MonoObject *obj,
                                                    MonoMethod *method);
...
void mono_unhandled_exception_internal(MonoObject *exc);
...
guint8 *mono_runtime_get_aotid_arr(void);
 ...
 * mono_gchandle_new () is used to prevent an object from being garbage
 * collected until mono_gchandle_free() is called. Use a TRUE value for the
 * pinned argument to prevent the object from being moved (this should be
 * avoided as much as possible and this should be used only for shorts periods
 * of time or performance will suffer). To create a weakref use
 * mono_gchandle_new_weakref (): track_resurrection should usually be false (see
 * the GC docs for more details). mono_gchandle_get_target () can be used to get
 * the object referenced by both kinds of handle: for a weakref handle, if an
 * object has been collected, it will return NULL.
 */
MonoGCHandle mono_gchandle_new_internal(MonoObject *obj, mono_bool pinned);

MonoGCHandle mono_gchandle_new_weakref_internal(MonoObject *obj,
                                                mono_bool track_resurrection);

ICALL_EXTERN_C
MonoObject *mono_gchandle_get_target_internal(MonoGCHandle gchandle);
 ...
 * Call mono_gc_reference_queue_new to create a new queue and pass the callback
 * that will be invoked when registered objects are collected. Call
 * mono_gc_reference_queue_add to register a pair of objects and data within a
 * queue. The callback will be triggered once an object is both unreachable and
 * finalized.
 */
MonoReferenceQueue *
mono_gc_reference_queue_new_internal(mono_reference_queue_callback callback);

void mono_gc_reference_queue_free_internal(MonoReferenceQueue *queue);

mono_bool mono_gc_reference_queue_add_internal(MonoReferenceQueue *queue,
                                               MonoObject *obj,
                                               void *user_data);

#define mono_gc_reference_queue_add_handle(queue, obj, user_data)              \
  (mono_gc_reference_queue_add_internal(                                       \
      (queue), MONO_HANDLE_RAW(MONO_HANDLE_CAST(MonoObject, obj)),             \
      (user_data)))

/* GC write barriers support */
void mono_gc_wbarrier_set_field_internal(MonoObject *obj, void *field_ptr,
                                         MonoObject *value);

void mono_gc_wbarrier_set_arrayref_internal(MonoArray *arr, void *slot_ptr,
                                            MonoObject *value);

void mono_gc_wbarrier_arrayref_copy_internal(void *dest_ptr,
                                             const void *src_ptr, int count);

void mono_gc_wbarrier_generic_store_internal(void volatile *ptr,
                                             MonoObject *value);

void mono_gc_wbarrier_generic_store_atomic_internal(void *ptr,
                                                    MonoObject *value);

ICALL_EXTERN_C
void mono_gc_wbarrier_generic_nostore_internal(void *ptr);

void mono_gc_wbarrier_value_copy_internal(void *dest, const void *src,
                                          int count, MonoClass *klass);

void mono_gc_wbarrier_object_copy_internal(MonoObject *obj, MonoObject *src);
