...
#define CONFIG_WORDSIZE "32"
#elif defined(__ppc64__) || defined(__powerpc64__) || defined(_ARCH_64) ||     \
 ...
 * mono_config_get_os:
 *
 * Returns the operating system that Mono is running on, as used for dllmap
 * entries.
 */
const char *mono_config_get_os(void) { return CONFIG_OS; }
 ...
 */
const char *mono_config_get_cpu(void) { return CONFIG_CPU; }
 ...
 */
const char *mono_config_get_wordsize(void) { return CONFIG_WORDSIZE; }
 ...
 */
const char *mono_get_machine_config(void) { return bundled_machine_config; }
...
static void parse_text(GMarkupParseContext *context, const gchar *text,
                       gsize text_len, gpointer user_data, GError **gerror);

static void passthrough(GMarkupParseContext *context, const gchar *text,
                        gsize text_len, gpointer user_data, GError **gerror);

static void parse_error(GMarkupParseContext *context, GError *gerror,
                        gpointer user_data);

static const GMarkupParser mono_parser = {start_element, end_element,
                                          parse_text, passthrough, parse_error};
...
struct MonoParseHandler {
  ...
  void (*start)(gpointer user_data, const gchar *name, const gchar **attributes,
                const gchar **values);
...
static void start_element(GMarkupParseContext *context,
  ...
  if (!state->current) {
    state->current =
        (MonoParseHandler *)g_hash_table_lookup(config_handlers, element_name);
    if (state->current && state->current->init)
      state->user_data = state->current->init(state->assembly);
  }
  if (state->current && state->current->start)
    state->current->start(state->user_data, element_name, attribute_names,
                          attribute_values);
}
...
static void parse_error(GMarkupParseContext *context, GError *gerror,
  ...
  filename =
      state && state->user_data ? (gchar *)state->user_data : "<unknown>";
  msg = gerror && gerror->message ? gerror->message : "";
  g_warning("Error parsing %s: %s", filename, msg);
}

static int arch_matches(const char *arch, const char *value) {
  char **splitted, **p;
  int found = FALSE;
  if (value[0] == '!')
...
static void *dllmap_init(MonoImage *assembly) {
  DllInfo *info = g_new0(DllInfo, 1);
  info->assembly = assembly;
  return info;
}

static void dllmap_start(gpointer user_data, const gchar *element_name,
                         const gchar **attribute_names,
    ...
    for (i = 0; attribute_names[i]; ++i) {
      if (strcmp(attribute_names[i], "dll") == 0)
        ...
        if (p != NULL) {
          ...
          result = (char *)g_malloc(libdir_len - strlen("$mono_libdir") +
                                    strlen(attribute_values[i]) + 1);
      ...
      } else if (strcmp(attribute_names[i], "os") == 0 &&
                 !arch_matches(CONFIG_OS, attribute_values[i]))
        info->ignore = TRUE;
      else if (strcmp(attribute_names[i], "cpu") == 0 &&
               !arch_matches(CONFIG_CPU, attribute_values[i]))
        info->ignore = TRUE;
      else if (strcmp(attribute_names[i], "wordsize") == 0 &&
               !arch_matches(CONFIG_WORDSIZE, attribute_values[i]))
        info->ignore = TRUE;
    }
    if (!info->ignore)
      mono_dllmap_insert_internal(info->assembly, info->dll, NULL, info->target,
                                  NULL);
...
static void dllmap_finish(gpointer user_data) {
  DllInfo *info = (DllInfo *)user_data;
...
static const MonoParseHandler dllmap_handler = {"dllmap",     dllmap_init,
                                                dllmap_start, NULL, /* text */
                                                NULL,               /* end */
                                                dllmap_finish};
#endif

static void legacyUEP_start(gpointer user_data, const gchar *element_name,
                            const gchar **attribute_names,
                            const gchar **attribute_values) {
...
static void aot_cache_start(gpointer user_data, const gchar *element_name,
                            const gchar **attribute_names,
                            const gchar **attribute_values) {
  int i;
  ...
  for (i = 0; attribute_names[i]; ++i) {
    if (!strcmp(attribute_names[i], "app")) {
      config->apps =
          g_slist_prepend(config->apps, g_strdup(attribute_values[i]));
    }
  }
  ...
  for (i = 0; attribute_names[i]; ++i) {
    if (!strcmp(attribute_names[i], "assemblies")) {
      ...
      for (ptr = parts; ptr && *ptr; ptr++) {
        part = *ptr;
        config->assemblies =
            g_slist_prepend(config->assemblies, g_strdup(part));
      }
      g_strfreev(parts);
...
static const MonoParseHandler aot_cache_handler = {
    "aotcache",
    NULL, /* init */
    aot_cache_start,
    NULL, /* text */
...
static void mono_config_init(void) {
  inited = 1;
...
#endif
  g_hash_table_insert(config_handlers, (gpointer)legacyUEP_handler.element_name,
                      (gpointer)&legacyUEP_handler);
  g_hash_table_insert(config_handlers, (gpointer)aot_cache_handler.element_name,
                      (gpointer)&aot_cache_handler);
}
...
static void mono_config_parse_xml_with_context(MonoConfigParseState *state,
...
static int mono_config_parse_file_with_context(MonoConfigParseState *state,
                                               const char *filename) {
  gchar *text;
  gsize len;
  gint offset;
 ...
 * 	aname
 */
static char *get_assembly_filename(MonoImage *image, int state) {
  switch (state) {
  case 0:
...
void mono_config_for_assembly_internal(MonoImage *assembly) {
  ...
  if (bundled_config) {
    ...
    mono_config_parse_xml_with_context(&state, bundled_config,
  ...
  for (i = 0; (aname = get_assembly_filename(assembly, i)) != NULL; ++i) {
    cfg = g_build_filename(cfg_dir, "mono", "assemblies", aname, cfg_name,
                           (const char *)NULL);
    ...
    cfg = g_build_filename(home, ".mono", "assemblies", aname, cfg_name,
                           (const char *)NULL);
    got_it += mono_config_parse_file_with_context(&state, cfg);
    g_free(cfg);
#endif
...
void mono_config_parse(const char *filename) {
  ...
  if (cfg_dir) {
    char *mono_cfg =
        g_build_filename(cfg_dir, "mono", "config", (const char *)NULL);
  ...
  home = g_get_home_dir();
  char *user_cfg =
      g_strconcat(home, G_DIR_SEPARATOR_S, ".mono/config", (const char *)NULL);
  mono_config_parse_file(user_cfg);
  g_free(user_cfg);
#endif
}

static void assembly_binding_end(gpointer user_data, const char *element_name) {
...
static void publisher_policy_start(gpointer user_data,
                                   const gchar *element_name,
                                   const gchar **attribute_names,
                                   const gchar **attribute_values) {
  ParserUserData *pud;
    ...
    for (n = 0; attribute_names[n]; n++) {
      ...
      if (!strcmp(attribute_name, "name"))
        ...
        if (strlen(attribute_values[n]) == MONO_PUBLIC_KEY_TOKEN_LENGTH - 1)
          g_strlcpy((char *)info->public_key_token, attribute_values[n],
...
static MonoParseHandler publisher_policy_parser = {
    "", /* We don't need to use declare an xml element */
    NULL, publisher_policy_start, NULL, NULL, NULL};

void mono_config_parse_publisher_policy(const gchar *filename,
                                        MonoAssemblyBindingInfo *info) {
  ParserUserData user_data = {info, NULL, NULL};
...
static MonoParseHandler config_assemblybinding_parser = {
    "", /* We don't need to use declare an xml element */
    NULL, publisher_policy_start, NULL, assembly_binding_end, NULL};

void mono_config_parse_assembly_bindings(
    const char *filename, int amajor, int aminor, void *user_data,
 ...
 */
mono_bool mono_config_is_server_mode(void) { return mono_server_mode; }
