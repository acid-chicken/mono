...
static gboolean get_pe_debug_info(MonoImage *image, guint8 *out_guid,
                                  gint32 *out_age, gint32 *out_timestamp,
                                  guint8 **ppdb_data,
                                  int *ppdb_uncompressed_size,
  ...
  *ppdb_data = NULL;
  ...
  debug_dir_entry =
      (MonoPEDirEntry *)&image->image_info->cli_header.datadir.pe_debug;
  if (!debug_dir_entry->size)
    return FALSE;

  int offset = mono_cli_rva_image_map(image, debug_dir_entry->rva);
  for (idx = 0; idx < debug_dir_entry->size / sizeof(ImageDebugDirectory);
       ++idx) {
    ...
    if (debug_dir.type == DEBUG_DIR_ENTRY_CODEVIEW &&
        debug_dir.major_version == 0x100 && debug_dir.minor_version == 0x504d) {
    ...
    if (debug_dir.type == DEBUG_DIR_ENTRY_PPDB &&
        debug_dir.major_version >= 0x100 && debug_dir.minor_version == 0x100) {
      /* Embedded PPDB blob */
      /* See
       * src/System.Reflection.Metadata/src/System/Reflection/PortableExecutable/PEReader.EmbeddedPortablePdb.cs
       * in corefx */
      data = (guint8 *)(image->raw_data + debug_dir.pointer);
...
static void doc_free(gpointer key) {
...
static MonoPPDBFile *create_ppdb_file(MonoImage *ppdb_image) {
  MonoPPDBFile *ppdb;

  ppdb = g_new0(MonoPPDBFile, 1);
  ppdb->image = ppdb_image;
  ppdb->doc_hash =
      g_hash_table_new_full(NULL, NULL, NULL, (GDestroyNotify)doc_free);
  ppdb->method_hash =
      g_hash_table_new_full(NULL, NULL, NULL, (GDestroyNotify)g_free);
  return ppdb;
}

MonoPPDBFile *mono_ppdb_load_file(MonoImage *image, const guint8 *raw_contents,
                                  int size) {
  ...
  if (!get_pe_debug_info(image, pe_guid, &pe_age, &pe_timestamp, &ppdb_data,
                         &ppdb_size, &ppdb_compressed_size)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
               "Image '%s' has no debug directory.", image->name);
    return NULL;
  }
  ...
  if (raw_contents) {
    if (size > 4 && strncmp((char *)raw_contents, "BSJB", 4) == 0)
      ppdb_image = mono_image_open_from_data_internal(
          alc, (char *)raw_contents, size, TRUE, &status, FALSE, TRUE, NULL);
  } else {
    /* ppdb files drop the .exe/.dll extension */
    filename = mono_image_get_filename(image);
    if (strlen(filename) > 4 &&
        (!strcmp(filename + strlen(filename) - 4, ".exe") ||
         !strcmp(filename + strlen(filename) - 4, ".dll"))) {
      s = g_strdup(filename);
...
MonoDebugMethodInfo *mono_ppdb_lookup_method(MonoDebugHandle *handle,
                                             MonoMethod *method) {
...
static MonoDebugSourceInfo *get_docinfo(MonoPPDBFile *ppdb, MonoImage *image,
                                        int docidx) {
  ...
  mono_debugger_lock();
  cached = (MonoDebugSourceInfo *)g_hash_table_lookup(ppdb->doc_hash,
                                                      GUINT_TO_POINTER(docidx));
  mono_debugger_unlock();
  if (cached)
    return cached;

  mono_metadata_decode_row(&tables[MONO_TABLE_DOCUMENT], docidx - 1, cols,
                           MONO_DOCUMENT_SIZE);
  ...
  res->guid = NULL;
  res->hash =
      (guint8 *)mono_metadata_blob_heap(image, cols[MONO_DOCUMENT_HASH]);
...
static char *get_docname(MonoPPDBFile *ppdb, MonoImage *image, int docidx) {
 ...
 * \param minfo A \c MonoDebugMethodInfo which can be retrieved by
 * mono_debug_lookup_method(). \param offset IL offset within the corresponding
 * method's CIL code.
 *
 ...
 */
MonoDebugSourceLocation *mono_ppdb_lookup_location(MonoDebugMethodInfo *minfo,
                                                   uint32_t offset) {
  ...
  const char *end;
  char *docname;
  int idx, size, docidx, iloffset, delta_il, delta_lines, delta_cols,
      start_line, start_col, adv_line, adv_col;
  ...
  mono_metadata_decode_row(&tables[MONO_TABLE_METHODBODY], idx - 1, cols,
...
MonoImage *mono_ppdb_get_image(MonoPPDBFile *ppdb) { return ppdb->image; }

void mono_ppdb_get_seq_points(MonoDebugMethodInfo *minfo, char **source_file,
  ...
  int i, method_idx, size, docidx, iloffset, delta_il, delta_lines, delta_cols,
      start_line, start_col, adv_line, adv_col;
  gboolean first = TRUE, first_non_hidden = TRUE;
  GArray *sps;
  MonoSymSeqPoint sp;
  ...
  if (G_UNLIKELY(method_idx - 1 >= methodbody_table->rows)) {
    ...
    g_error("Method idx %d is greater than number of rows (%d) in PPDB "
            "MethodDebugInformation table, for method %s in '%s'. Likely a "
            "malformed PDB file.",
            method_idx - 1, methodbody_table->rows, method_name, image->name);
    g_free(method_name);
  }
  mono_metadata_decode_row(methodbody_table, method_idx - 1, cols,
                           MONO_METHODBODY_SIZE);
...
MonoDebugLocalsInfo *mono_ppdb_lookup_locals(MonoDebugMethodInfo *minfo) {
  ...
  int i, lindex, sindex, method_idx, start_scope_idx, scope_idx, locals_idx,
      locals_end_idx, nscopes;
  ...
  mono_metadata_decode_row(&tables[MONO_TABLE_LOCALSCOPE], scope_idx - 1, cols,
  ...
  //
  // The variableList attribute in the pdb metadata table is a contiguous array
  // that starts at a given offset (locals_idx) above and
  //
  // """
  // continues to the smaller of:
  //
  // the last row of the LocalVariable table
  // the next run of LocalVariables, found by inspecting the VariableList of the
  ...
  while (scope_idx <= tables[MONO_TABLE_LOCALSCOPE].rows) {
    mono_metadata_decode_row(&tables[MONO_TABLE_LOCALSCOPE], scope_idx - 1,
                             cols, MONO_LOCALSCOPE_SIZE);
  ...
  if (scope_idx > tables[MONO_TABLE_LOCALSCOPE].rows) {
    ...
    // found by inspecting the VariableList of the next row in this LocalScope
    // table."
  ...
  for (sindex = 0; sindex < nscopes; ++sindex) {
    ...
    if (scope_idx == tables[MONO_TABLE_LOCALSCOPE].rows) {
      ...
      locals_end_idx = mono_metadata_decode_row_col(
          &tables[MONO_TABLE_LOCALSCOPE], scope_idx - 1 + 1,
          MONO_LOCALSCOPE_VARIABLELIST);
    }

    res->code_blocks[sindex].start_offset = cols[MONO_LOCALSCOPE_STARTOFFSET];
    res->code_blocks[sindex].end_offset =
        cols[MONO_LOCALSCOPE_STARTOFFSET] + cols[MONO_LOCALSCOPE_LENGTH];

    // printf ("Scope: %s %d %d %d-%d\n", mono_method_full_name (method, 1),
    // cols [MONO_LOCALSCOPE_STARTOFFSET], cols [MONO_LOCALSCOPE_LENGTH],
    // locals_idx, locals_end_idx);

    for (i = locals_idx; i < locals_end_idx; ++i) {
      mono_metadata_decode_row(&tables[MONO_TABLE_LOCALVARIABLE], i - 1,
                               locals_cols, MONO_LOCALVARIABLE_SIZE);

      res->locals[lindex].name = g_strdup(mono_metadata_string_heap(
          image, locals_cols[MONO_LOCALVARIABLE_NAME]));
      res->locals[lindex].index = locals_cols[MONO_LOCALVARIABLE_INDEX];
      res->locals[lindex].block = &res->code_blocks[sindex];
      lindex++;

      // printf ("\t %s %d\n", mono_metadata_string_heap (image, locals_cols
      // [MONO_LOCALVARIABLE_NAME]), locals_cols [MONO_LOCALVARIABLE_INDEX]);
    }
  }
...
static gboolean compare_guid(guint8 *guid1, guint8 *guid2) {
  for (int i = 0; i < 16; i++) {
    if (guid1[i] != guid2[i])
      return FALSE;
  }
...
static const char *lookup_custom_debug_information(MonoImage *image,
                                                   guint32 token,
                                                   uint8_t parent_type,
                                                   guint8 *guid) {
  MonoTableInfo *tables = image->tables;
  ...
  loc.idx = (mono_metadata_token_index(token) << MONO_HAS_CUSTOM_DEBUG_BITS) |
            parent_type;
  ...
  if (!mono_binary_search(&loc, table->base, table->rows, table->row_size,
                          table_locator))
    return NULL;
  // Great we found one of possibly many CustomDebugInformations of this entity
  // they are distinguished by KIND guid First try on this index found by binary
  // search...(it's most likeley to be only one and binary search found the one
  // we want)
  if (compare_guid(guid, (guint8 *)mono_metadata_guid_heap(
                             image, mono_metadata_decode_row_col(
                                        table, loc.result,
                                        MONO_CUSTOMDEBUGINFORMATION_KIND))))
    return mono_metadata_blob_heap(
        image, mono_metadata_decode_row_col(table, loc.result,
  ...
  for (int i = loc.result + 1; i < table->rows; i++) {
    if (mono_metadata_decode_row_col(
            table, i, MONO_CUSTOMDEBUGINFORMATION_PARENT) != loc.idx)
      break;
    if (compare_guid(
            guid, (guint8 *)mono_metadata_guid_heap(
                      image, mono_metadata_decode_row_col(
                                 table, i, MONO_CUSTOMDEBUGINFORMATION_KIND))))
      return mono_metadata_blob_heap(
          image, mono_metadata_decode_row_col(
                     table, i, MONO_CUSTOMDEBUGINFORMATION_VALUE));
  }
...
mono_ppdb_lookup_method_async_debug_info(MonoDebugMethodInfo *minfo) {
  ...
  guint8 async_method_stepping_information_guid[16] = {
      0xC5, 0x2A, 0xFD, 0x54, 0x25, 0xE9, 0x1A, 0x40,
      0x9C, 0x2A, 0xF9, 0x4F, 0x17, 0x10, 0x72, 0xF8};
  char const *blob = lookup_custom_debug_information(
      image, method->token, MONO_HAS_CUSTOM_DEBUG_METHODDEF,
  ...
  res->catch_handler_offset = read32(pointer);
  pointer += 4;
  for (int i = 0; i < res->num_awaits; i++) {
    res->yield_offsets[i] = read32(pointer);
    pointer += 4;
    res->resume_offsets[i] = read32(pointer);
    pointer += 4;
    res->move_next_method_token[i] =
        mono_metadata_decode_value(pointer, &pointer);
  }
  return res;
}
...
char *mono_ppdb_get_sourcelink(MonoDebugHandle *handle) {
  ...
  guint8 sourcelink_guid[16] = {0x56, 0x05, 0x11, 0xCC, 0x91, 0xA0, 0x38, 0x4D,
                                0x9F, 0xEC, 0x25, 0xAB, 0x9A, 0x35, 0x1A, 0x6A};
  /* The module table only has 1 row */
  char const *blob = lookup_custom_debug_information(
      image, 1, MONO_HAS_CUSTOM_DEBUG_MODULE, sourcelink_guid);
  if (!blob)
    return NULL;
  int blob_len = mono_metadata_decode_blob_size(blob, &blob);
