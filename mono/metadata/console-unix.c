...
/* The string used to set the terminal into keypad xmit mode after SIGCONT is
 * received */
...
static MonoBoolean set_property(gint property, gboolean value) {
  struct termios attr;
...
MonoBoolean ves_icall_System_ConsoleDriver_SetEcho(MonoBoolean want_echo,
                                                   MonoError *error) {
...
#ifdef TIOCGWINSZ
static int terminal_get_dimensions(void) {
  struct winsize ws;
  int ret;
...
#else
static int terminal_get_dimensions(void) { return -1; }
#endif

static void tty_teardown(void) {
...
static void do_console_cancel_event(void) {
  ...
  if (System_Console_DoConsoleCancelEventBackground_method ==
    ...
    System_Console_DoConsoleCancelEventBackground_method =
        mono_class_get_method_from_name_checked(
            mono_class_try_get_console_class(),
            "DoConsoleCancelEventInBackground", 0, 0, error);
  ...
  mono_runtime_invoke_checked(
      System_Console_DoConsoleCancelEventBackground_method, NULL, NULL, error);
  mono_error_assert_ok(error);
}
 ...
 */
static void console_set_signal_handlers() {
...
//
void console_restore_signal_handlers() {
...
static void set_control_chars(gchar *control_chars, const guchar *cc) {
...
MonoBoolean ves_icall_System_ConsoleDriver_TtySetup(
    MonoStringHandle keypad, MonoStringHandle teardown,
    MonoArrayHandleOut control_chars, int **size, MonoError *error) {
  ...
  *size = &cols_and_lines;
  ...
  MonoArrayHandle control_chars_arr = mono_array_new_handle(
      mono_domain_get(), mono_defaults.byte_class, 17, error);
  ...
  MonoGCHandle h;
  set_control_chars(MONO_ARRAY_HANDLE_PIN(control_chars_arr, gchar, 0, &h),
                    mono_attr.c_cc);
