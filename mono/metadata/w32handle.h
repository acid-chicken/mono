...
typedef enum {
  ...
  MONO_W32HANDLE_WAIT_RET_ABANDONED_0 =
      MONO_W32HANDLE_WAIT_RET_SUCCESS_0 + MONO_W32HANDLE_MAXIMUM_WAIT_OBJECTS,
...
typedef struct {
   ...
   */
  MonoW32HandleWaitRet (*special_wait)(MonoW32Handle *handle_data,
...
void mono_w32handle_register_ops(MonoW32Type type, const MonoW32HandleOps *ops);
...
mono_w32handle_convert_wait_ret(guint32 res, guint32 numobjects) {
  if (res >= WAIT_OBJECT_0 && res <= WAIT_OBJECT_0 + numobjects - 1)
    return (MonoW32HandleWaitRet)(MONO_W32HANDLE_WAIT_RET_SUCCESS_0 +
                                  (res - WAIT_OBJECT_0));
  else if (res >= WAIT_ABANDONED_0 && res <= WAIT_ABANDONED_0 + numobjects - 1)
    return (MonoW32HandleWaitRet)(MONO_W32HANDLE_WAIT_RET_ABANDONED_0 +
