...
MonoBoolean ves_icall_System_Diagnostics_Process_ShellExecuteEx_internal(
  ...
  shellex.fMask =
      (gulong)(SEE_MASK_NOASYNC | SEE_MASK_NOCLOSEPROCESS | SEE_MASK_UNICODE);
  shellex.nShow = (gulong)MONO_HANDLE_GETVAL(proc_start_info, window_style);
  shellex.nShow = (gulong)(
      (shellex.nShow == 0) ? 1 : (shellex.nShow == 1 ? 0 : shellex.nShow));
  ...
  if (MONO_HANDLE_GETVAL(proc_start_info, error_dialog))
    shellex.hwnd =
        (HWND)MONO_HANDLE_GETVAL(proc_start_info, error_dialog_parent_handle);
  else
...
static void mono_process_init_startup_info(HANDLE stdin_handle,
...
static gboolean mono_process_create_process(
    MonoCreateProcessCoop *coop, MonoW32ProcessInfo *mono_process_info,
    MonoStringHandle cmd, guint32 creation_flags, gunichar2 *env_vars,
    gunichar2 *dir, STARTUPINFO *start_info,
    PROCESS_INFORMATION *process_info) {
  gboolean result = FALSE;
  MonoGCHandle cmd_gchandle = NULL;
  gunichar2 *cmd_chars = MONO_HANDLE_IS_NULL(cmd)
                             ? NULL
                             : mono_string_handle_pin_chars(cmd, &cmd_gchandle);

  MONO_ENTER_GC_SAFE;
  if (coop->username) {
    guint32 logon_flags =
        mono_process_info->load_user_profile ? LOGON_WITH_PROFILE : 0;

    result = CreateProcessWithLogonW(coop->username, coop->domain,
                                     mono_process_info->password, logon_flags,
                                     NULL, cmd_chars, creation_flags, env_vars,
                                     dir, start_info, process_info);
  } else {
    result = CreateProcessW(NULL, cmd_chars, NULL, NULL, TRUE, creation_flags,
...
static gchar *process_unquote_application_name(gchar *appname) {
...
static gchar *process_quote_path(const gchar *path) {
...
static gchar *process_complete_path(const gunichar2 *appname) {
...
static gboolean
process_get_shell_arguments(MonoCreateProcessCoop *proc_start_info,
                            MonoStringHandle *cmd, MonoError *error) {
  char *spath = NULL;
  char *new_cmd = NULL;
  char *cmd_utf8 = NULL;
  ...
  *cmd = proc_start_info->coophandle.arguments;
  ...
  if (!MONO_HANDLE_IS_NULL(*cmd)) {
    ...
    *cmd = mono_string_new_utf8_len(mono_domain_get(), new_cmd, strlen(new_cmd),
                                    error);
    goto_if_nok(error, error);
  } else {
    *cmd = mono_string_new_utf8_len(mono_domain_get(), spath, strlen(spath),
                                    error);
    goto_if_nok(error, error);
  }
...
MonoBoolean ves_icall_System_Diagnostics_Process_CreateProcess_internal(
  ...
  mono_process_init_startup_info(stdin_handle, stdout_handle, stderr_handle,
                                 &startinfo);
  ...
  if (process_info->env_variables) {
    ...
    for (gsize i = 0; i < array_length; i++) {
      ...
      memcpy(ptr, mono_string_handle_pin_chars(var, &gchandle),
             mono_string_handle_length(var) * sizeof(gunichar2));
  ...
  ret = mono_process_create_process(&coop, process_info, cmd, creation_flags,
                                    env_vars, dir, &startinfo, &procinfo);
...
ves_icall_System_Diagnostics_Process_GetProcesses_internal(MonoError *error) {
  ...
  memcpy(mono_array_addr_internal(MONO_HANDLE_RAW(procs), guint32, 0), pids,
         needed);
...
MonoBoolean ves_icall_Microsoft_Win32_NativeMethods_GetProcessTimes(
    gpointer handle, gint64 *creationtime, gint64 *exittime, gint64 *kerneltime,
    gint64 *usertime) {
  return GetProcessTimes(handle, (LPFILETIME)creationtime, (LPFILETIME)exittime,
