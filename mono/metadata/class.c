...
/* Function supplied by the runtime to find classes by name using information
 * from the AOT file */
static MonoGetClassFromName get_class_from_name = NULL;

static gboolean can_access_type(MonoClass *access_klass,
                                MonoClass *member_klass);

static char *mono_assembly_name_from_token(MonoImage *image,
                                           guint32 type_token);
static guint32 mono_field_resolve_flags(MonoClassField *field);

static MonoClass *
mono_class_from_name_checked_aux(MonoImage *image, const char *name_space,
                                 const char *name, GHashTable *visited_images,
                                 gboolean case_sensitive, MonoError *error);

GENERATE_GET_CLASS_WITH_CACHE(valuetype, "System", "ValueType")
GENERATE_TRY_GET_CLASS_WITH_CACHE(handleref, "System.Runtime.InteropServices",
                                  "HandleRef")

#define CTOR_REQUIRED_FLAGS                                                    \
  (METHOD_ATTRIBUTE_SPECIAL_NAME | METHOD_ATTRIBUTE_RT_SPECIAL_NAME)
...
static MonoImage *mono_method_get_image(MonoMethod *method) {
 ...
 * \returns The \c MonoClass* representing the typeref token, or NULL if it
 * could not be loaded.
 */
 ...
 * error.
 */
MonoClass *mono_class_from_typeref_checked(MonoImage *image, guint32 type_token,
                                           MonoError *error) {
  ...
  if (!mono_verifier_verify_typeref_row(image, (type_token & 0xffffff) - 1,
                                        error))
    return NULL;

  mono_metadata_decode_row(t, (type_token & 0xffffff) - 1, cols,
                           MONO_TYPEREF_SIZE);
  ...
  case MONO_RESOLUTION_SCOPE_MODULE:
    /*
    LAMESPEC The spec says that a null module resolution scope should go through
    the exported type table. This is not the observed behavior of existing
    implementations. The defacto behavior is that it's just a typedef in
    disguise.
    */
  ...
  case MONO_RESOLUTION_SCOPE_TYPEREF: {
    ...
    if (idx == mono_metadata_token_index(type_token)) {
      mono_error_set_bad_image(
          error, image, "Image with self-referencing typeref token %08x.",
          type_token);
      return NULL;
    }

    enclosing = mono_class_from_typeref_checked(
        image, MONO_TOKEN_TYPE_REF | idx, error);
    ...
    if (m_class_is_nested_classes_inited(enclosing) && nested_classes) {
      /* Micro-optimization: don't scan the metadata tables if enclosing is
       * already inited */
      ...
      /* Don't call mono_class_init_internal as we might've been called by it
       * recursively */
      int i = mono_metadata_nesting_typedef(enclosing_image,
                                            enclosing_type_token, 1);
      while (i) {
        guint32 class_nested = mono_metadata_decode_row_col(
            &enclosing_image->tables[MONO_TABLE_NESTEDCLASS], i - 1,
            MONO_NESTED_CLASS_NESTED);
        guint32 string_offset = mono_metadata_decode_row_col(
            &enclosing_image->tables[MONO_TABLE_TYPEDEF], class_nested - 1,
            MONO_TYPEDEF_NAME);
        const char *nname =
            mono_metadata_string_heap(enclosing_image, string_offset);

        if (strcmp(nname, name) == 0)
          return mono_class_create_from_typedef(
              enclosing_image, MONO_TOKEN_TYPE_DEF | class_nested, error);

        i = mono_metadata_nesting_typedef(enclosing_image, enclosing_type_token,
                                          i + 1);
      }
    }
    g_warning(
        "TypeRef ResolutionScope not yet handled (%d) for %s.%s in image %s",
        idx, nspace, name, image->name);
    goto done;
  }
  ...
  res = mono_class_from_name_checked(image->references[idx - 1]->image, nspace,
                                     name, error);
  ...
  if (!res && is_ok(error)) {
    ...
    mono_error_set_type_load_name(
        error, name, assembly,
        "Could not resolve type with token %08x from typeref (expected class "
        "'%s' in assembly '%s')",
        type_token, name, assembly);
  }
  return res;
}

static void *mono_image_memdup(MonoImage *image, void *data, guint size) {
...
MonoArrayType *mono_dup_array_type(MonoImage *image, MonoArrayType *a) {
  if (image) {
    ...
    if (a->sizes)
      a->sizes =
          (int *)mono_image_memdup(image, a->sizes, a->numsizes * sizeof(int));
    if (a->lobounds)
      a->lobounds = (int *)mono_image_memdup(image, a->lobounds,
                                             a->numlobounds * sizeof(int));
  } else {
...
static void _mono_type_get_assembly_name(MonoClass *klass, GString *str) {
...
static void mono_type_name_check_byref(MonoType *type, GString *str) {
  if (type->byref)
    g_string_append_c(str, '&');
}

static char *escape_special_chars(const char *identifier) {
...
char *mono_identifier_escape_type_name_chars(const char *identifier) {
  ...
  // If the string has any special characters escape the whole thing, otherwise
...
static void mono_type_get_name_recurse(MonoType *type, GString *str,
  ...
  case MONO_TYPE_ARRAY: {
    ...
    mono_type_get_name_recurse(m_class_get_byval_arg(type->data.array->eklass),
                               str, FALSE, nested_format);
    g_string_append_c(str, '[');
    if (rank == 1)
      g_string_append_c(str, '*');
    for (i = 1; i < rank; i++)
  ...
  case MONO_TYPE_SZARRAY: {
    ...
    mono_type_get_name_recurse(m_class_get_byval_arg(type->data.klass), str,
                               FALSE, nested_format);
    g_string_append(str, "[]");
  ...
  case MONO_TYPE_PTR: {
    ...
    mono_type_get_name_recurse(type->data.type, str, FALSE, nested_format);
    g_string_append_c(str, '*');
    ...
    if (format == MONO_TYPE_NAME_FORMAT_ASSEMBLY_QUALIFIED)
      _mono_type_get_assembly_name(
          mono_class_from_mono_type_internal(type->data.type), str);
    break;
  }
  case MONO_TYPE_VAR:
  case MONO_TYPE_MVAR:
  ...
  default:
    ...
    if (m_class_get_nested_in(klass)) {
      mono_type_get_name_recurse(
          m_class_get_byval_arg(m_class_get_nested_in(klass)), str, TRUE,
          format);
    ...
    if (mono_class_is_ginst(klass)) {
      ...
      for (i = 0; i < inst->type_argc; i++) {
        ...
        if ((nested_format == MONO_TYPE_NAME_FORMAT_ASSEMBLY_QUALIFIED) &&
          ...
          g_string_append_c(str, '[');
        mono_type_get_name_recurse(inst->type_argv[i], str, FALSE,
                                   nested_format);
        if ((nested_format == MONO_TYPE_NAME_FORMAT_ASSEMBLY_QUALIFIED) &&
            (t->type != MONO_TYPE_VAR) && (type->type != MONO_TYPE_MVAR))
          g_string_append_c(str, ']');
      ...
      for (i = 0; i < mono_class_get_generic_container(klass)->type_argc; i++) {
        if (i)
          g_string_append_c(str, ',');
        g_string_append(str, mono_generic_container_get_param_info(
                                 mono_class_get_generic_container(klass), i)
                                 ->name);
      }
      if (format == MONO_TYPE_NAME_FORMAT_IL)
        g_string_append_c(str, '>');
...
char *mono_type_get_full_name(MonoClass *klass) {
  return mono_type_get_name_full(m_class_get_byval_arg(klass),
 ...
 * mono_type_get_name:
 * \param type a type
 * \returns The string representation for type as it would be represented in IL
 * code.
 */
...
MonoType *mono_type_get_underlying_type(MonoType *type) {
  if (type->type == MONO_TYPE_VALUETYPE &&
      m_class_is_enumtype(type->data.klass) && !type->byref)
...
gboolean mono_class_is_open_constructed_type(MonoType *t) {
  ...
  case MONO_TYPE_MVAR:
    return TRUE;
  case MONO_TYPE_SZARRAY:
    return mono_class_is_open_constructed_type(
        m_class_get_byval_arg(t->data.klass));
  case MONO_TYPE_ARRAY:
    return mono_class_is_open_constructed_type(
        m_class_get_byval_arg(t->data.array->eklass));
  case MONO_TYPE_PTR:
...
/*
This is a simple function to catch the most common bad instances of generic
types. Specially those that might lead to further failures in the runtime.
*/
...
static gboolean can_inflate_gparam_with(MonoGenericParam *gparam,
                                        MonoType *type) {
  if (!mono_type_is_valid_generic_argument(type))
    return FALSE;
#if FALSE
  /* Avoid inflating gparams with valuetype constraints with ref types during
   * gsharing */
  MonoGenericParamInfo *info = mono_generic_param_info(gparam);
  if (info &&
      (info->flags & GENERIC_PARAMETER_ATTRIBUTE_VALUE_TYPE_CONSTRAINT)) {
    if (type->type == MONO_TYPE_VAR || type->type == MONO_TYPE_MVAR) {
      MonoGenericParam *inst_gparam = type->data.generic_param;
      if (inst_gparam->gshared_constraint &&
          inst_gparam->gshared_constraint->type == MONO_TYPE_OBJECT)
        return FALSE;
    }
  }
#endif
...
static MonoType *inflate_generic_custom_modifiers(MonoImage *image,
                                                  const MonoType *type,
                                                  MonoGenericContext *context,
                                                  MonoError *error);

static MonoType *inflate_generic_type(MonoImage *image, MonoType *type,
                                      MonoGenericContext *context,
                                      MonoError *error) {
  ...
  if (type->has_cmods) {
    MonoType *new_type =
        inflate_generic_custom_modifiers(image, type, context, error);
    return_val_if_nok(error, NULL);
    if (new_type != NULL) {
      type = new_type;
      changed = TRUE;
  ...
  case MONO_TYPE_MVAR: {
    ...
    if (num >= inst->type_argc) {
      ...
      mono_error_set_bad_image(error, image,
                               "MVAR %d (%s) cannot be expanded in this "
    ...
    if (!can_inflate_gparam_with(gparam, inst->type_argv[num])) {
      const char *pname = mono_generic_param_name(gparam);
      mono_error_set_bad_image(
          error, image, "MVAR %d (%s) cannot be expanded with type 0x%x", num,
          pname ? pname : "", inst->type_argv[num]->type);
      return NULL;
    }
    /*
     * Note that the VAR/MVAR cases are different from the rest.  The other
     * cases duplicate @type, while the VAR/MVAR duplicates a type from the
     * context.  So, we need to ensure that the
  ...
  case MONO_TYPE_VAR: {
    ...
    if (num >= inst->type_argc) {
      const char *pname = mono_generic_param_name(gparam);
      mono_error_set_bad_image(error, image,
                               "VAR %d (%s) cannot be expanded in this context "
                               "with %d instantiations",
                               num, pname ? pname : "", inst->type_argc);
      return NULL;
    }
    if (!can_inflate_gparam_with(gparam, inst->type_argv[num])) {
      const char *pname = mono_generic_param_name(gparam);
      mono_error_set_bad_image(
          error, image, "VAR %d (%s) cannot be expanded with type 0x%x", num,
          pname ? pname : "", inst->type_argv[num]->type);
      return NULL;
    }
  ...
  case MONO_TYPE_SZARRAY: {
    ...
    MonoType *nt, *inflated = inflate_generic_type(
                      NULL, m_class_get_byval_arg(eclass), context, error);
  ...
  case MONO_TYPE_GENERICINST: {
    ...
    inst = mono_metadata_inflate_generic_inst(gclass->context.class_inst,
                                              context, error);
    return_val_if_nok(error, NULL);

    if (inst != gclass->context.class_inst)
      gclass = mono_metadata_lookup_generic_class(gclass->container_class, inst,
                                                  gclass->is_dynamic);
  ...
  case MONO_TYPE_VALUETYPE: {
    ...
    /* We can't use context->class_inst directly, since it can have more
     * elements */
    inst = mono_metadata_inflate_generic_inst(container->context.class_inst,
                                              context, error);
    return_val_if_nok(error, NULL);
    ...
    gclass = mono_metadata_lookup_generic_class(
        klass, inst, image_is_dynamic(m_class_get_image(klass)));
  ...
  case MONO_TYPE_PTR: {
    MonoType *nt, *inflated = inflate_generic_type(image, type->data.type,
                                                   context, error);
    if ((!inflated && !changed) || !is_ok(error))
      return NULL;
    if (!inflated && changed)
...
static MonoType *inflate_generic_custom_modifiers(MonoImage *image,
  ...
  if (changed) {
    /* if we're going to make a new type, fill in any modifiers that weren't
     * affected by inflation with copies of the original values. */
    for (int i = 0; i < count; ++i) {
      if (candidate_mods->modifiers[i].type == NULL) {
        candidate_mods->modifiers[i].type = mono_metadata_type_dup(
            NULL, mono_type_get_custom_modifier(type, i, NULL, error));
  ...
  if (changed) {
    ...
    /* first init just the non-modifier portion of new_type before populating
     * the new modifiers */
 ...
 *
 * If \p type is a generic type and \p context is not NULL, instantiate it using
 * the generics context \p context.
 *
 * \returns The instantiated type or a copy of \p type. The returned \c MonoType
 * is allocated on the heap and is owned by the caller. Returns NULL on error.
 */
 ...
 *
 * Returns: The instantiated type or a copy of @type. The returned MonoType is
 * allocated on the heap and is owned by the caller.
 */
MonoType *mono_class_inflate_generic_type_checked(MonoType *type,
                                                  MonoGenericContext *context,
                                                  MonoError *error) {
 ...
 */
static MonoType *
mono_class_inflate_generic_type_no_copy(MonoImage *image, MonoType *type,
                                        MonoGenericContext *context,
                                        MonoError *error) {
  MonoType *inflated = NULL;
 ...
 */
MonoClass *mono_class_inflate_generic_class_checked(MonoClass *gklass,
                                                    MonoGenericContext *context,
                                                    MonoError *error) {
  MonoClass *res;
  MonoType *inflated;
  ...
  inflated = mono_class_inflate_generic_type_checked(
      m_class_get_byval_arg(gklass), context, error);
  return_val_if_nok(error, NULL);
...
inflate_generic_context(MonoGenericContext *context,
                        MonoGenericContext *inflate_with, MonoError *error) {
  ...
  if (context->class_inst) {
    class_inst = mono_metadata_inflate_generic_inst(context->class_inst,
                                                    inflate_with, error);
    if (!is_ok(error))
      goto fail;
  }
 ...
 * \param context a generics context
 *
 * Instantiate the generic method \p method using the generics context \p
 * context.
 *
 * \returns The new instantiated method
 */
MonoMethod *mono_class_inflate_generic_method(MonoMethod *method,
                                              MonoGenericContext *context) {
  ERROR_DECL(error);
  MonoMethod *res = mono_class_inflate_generic_method_full_checked(
      method, NULL, context, error);
  mono_error_assert_msg_ok(error, "Could not inflate generic method");
  return res;
}

MonoMethod *mono_class_inflate_generic_method_checked(
    MonoMethod *method, MonoGenericContext *context, MonoError *error) {
  return mono_class_inflate_generic_method_full_checked(method, NULL, context,
                                                        error);
}
 ...
 *
 * BEWARE: All non-trivial fields are invalid, including klass, signature, and
 * header. Use mono_method_signature_internal () and mono_method_get_header ()
 * to get the correct values.
 */
MonoMethod *mono_class_inflate_generic_method_full_checked(
    MonoMethod *method, MonoClass *klass_hint, MonoGenericContext *context,
    MonoError *error) {
  MonoMethod *result;
  ...
  /* The `method' has already been instantiated before => we need to peel out
   * the instantiation and create a new context */
  ...
  /*
   * A method only needs to be inflated if the context has argument for which it
   * is parametric. Eg:
   *
   * class Foo<T> { void Bar(); } - doesn't need to be inflated if only mvars'
   * are supplied class Foo { void Bar<T> (); } - doesn't need to be if only
   * vars' are supplied
   *
   */
  ...
  if (!context->class_inst) {
    ...
    if (mono_class_is_gtd(iresult->declaring->klass))
      iresult->context.class_inst =
          mono_class_get_generic_container(iresult->declaring->klass)
              ->context.class_inst;
  }
  /* This can happen with some callers like
   * mono_object_get_virtual_method_internal () */
  if (!mono_class_is_gtd(iresult->declaring->klass) &&
      !mono_class_is_ginst(iresult->declaring->klass))
  ...
  cached =
      (MonoMethodInflated *)g_hash_table_lookup(set->gmethod_cache, iresult);
  ...
  if (!sig) {
    ...
    mono_error_set_bad_image(error, mono_method_get_image(method),
  ...
  if (iresult->context.method_inst) {
    ...
    /* Set the generic_container of the result to the generic_container of
     * method */
    MonoGenericContainer *generic_container =
        mono_method_get_generic_container(method);

    if (generic_container &&
        method_inst == generic_container->context.method_inst) {
      result->is_generic = 1;
    ...
    for (int i = 0; i < method_inst->type_argc; i++) {
      if (!mono_type_is_valid_generic_argument(method_inst->type_argv[i])) {
        mono_error_set_bad_image(error, mono_method_get_image(method),
                                 "MVAR %d cannot be expanded with type 0x%x", i,
                                 method_inst->type_argv[i]->type);
        goto fail;
      }
    }
  }
  ...
  if (klass_hint) {
    MonoGenericClass *gklass_hint =
        mono_class_try_get_generic_class(klass_hint);
    if (gklass_hint && (gklass_hint->container_class != method->klass ||
                        gklass_hint->context.class_inst != context->class_inst))
      klass_hint = NULL;
  }
  ...
  if (!result->klass) {
    MonoType *inflated = inflate_generic_type(
        NULL, m_class_get_byval_arg(method->klass), context, error);
    if (!is_ok(error))
      goto fail;

    result->klass =
        inflated ? mono_class_from_mono_type_internal(inflated) : method->klass;
    if (inflated)
      mono_metadata_free_type(inflated);
  }
   ...
   *		mono_method_get_context (result)->method_inst ==
   *mono_method_get_generic_container
   *(((MonoMethodInflated*)result)->declaring)->context.method_inst) { g_assert
   *(result->is_generic);
   * }
   *
 ...
 */
MonoMethod *mono_get_inflated_method(MonoMethod *method) { return method; }
 ...
 */
MonoGenericContext *mono_method_get_context_general(MonoMethod *method,
                                                    gboolean uninflated) {
 ...
 * NULL.
 */
MonoGenericContext *mono_method_get_context(MonoMethod *method) {
 ...
 * mono_method_get_generic_container:
 *
 *   Returns the generic container of METHOD, which should be a generic method
 * definition. Returns NULL if METHOD is not a generic method definition.
 * LOCKING: Acquires the loader lock.
 */
MonoGenericContainer *mono_method_get_generic_container(MonoMethod *method) {
  MonoGenericContainer *container;

  if (!method->is_generic)
    return NULL;

  container = (MonoGenericContainer *)mono_image_property_lookup(
      mono_method_get_image(method), method,
 ...
 */
void mono_method_set_generic_container(MonoMethod *method,
                                       MonoGenericContainer *container) {
  g_assert(method->is_generic);

  mono_image_property_insert(mono_method_get_image(method), method,
                             MONO_METHOD_PROP_GENERIC_CONTAINER, container);
}
 ...
 *
 *   Determine the basetype of an enum by iterating through its fields. We do
 * this in a separate function since it is cheaper than calling
...
MonoType *mono_class_find_enum_basetype(MonoClass *klass, MonoError *error) {
  ...
  for (i = 0; i < top; i++) {
    ...
    mono_metadata_decode_table_row(image, MONO_TABLE_FIELD, idx, cols,
                                   MONO_FIELD_SIZE);

    if (cols[MONO_FIELD_FLAGS] &
        FIELD_ATTRIBUTE_STATIC) // no need to decode static fields
      continue;

    if (!mono_verifier_verify_field_signature(image, cols[MONO_FIELD_SIGNATURE],
                                              error))
      goto fail;
    ...
    if (*sig != 0x06) {
      mono_error_set_bad_image(
          error, image, "Invalid field signature %x, expected 0x6 but got %x",
          cols[MONO_FIELD_SIGNATURE], *sig);
      goto fail;
    }

    ftype = mono_metadata_parse_type_checked(
        image, container, cols[MONO_FIELD_FLAGS], FALSE, sig + 1, &sig, error);
    if (!ftype)
      goto fail;

    if (mono_class_is_ginst(klass)) {
      // FIXME do we leak here?
      ftype = mono_class_inflate_generic_type_checked(
          ftype, mono_class_get_context(klass), error);
      if (!is_ok(error))
        goto fail;
...
/*
 * Checks for MonoClass::has_failure without resolving all MonoType's into
 * MonoClass'es
 */
gboolean mono_type_has_exceptions(MonoType *type) {
  ...
  case MONO_TYPE_GENERICINST:
    return mono_class_has_failure(
        mono_class_create_generic_inst(type->data.generic_class));
  default:
    return FALSE;
  }
}

void mono_error_set_for_class_failure(MonoError *oerror,
                                      const MonoClass *klass) {
 ...
 * mono_class_alloc:
 *
 *   Allocate memory for some data belonging to CLASS, either from its image's
 * mempool, or from the heap.
 */
 ...
 *
 * \returns TRUE if a failiure was set, or FALSE if \p caused_by doesn't have a
 * failure.
 */
gboolean mono_class_set_type_load_failure_causedby_class(
    MonoClass *klass, const MonoClass *caused_by, const gchar *msg) {
  if (mono_class_has_failure(caused_by)) {
    ERROR_DECL(cause_error);
    mono_error_set_for_class_failure(cause_error, caused_by);
    mono_class_set_type_load_failure(klass, "%s, due to: %s", msg,
...
MonoType *mono_type_get_basic_type_from_generic(MonoType *type) {
  /* When we do generic sharing we let type variables stand for
   * reference/primitive types. */
  if (!type->byref &&
      (type->type == MONO_TYPE_VAR || type->type == MONO_TYPE_MVAR) &&
      (!type->data.generic_param->gshared_constraint ||
       type->data.generic_param->gshared_constraint->type == MONO_TYPE_OBJECT))
...
MonoMethod *mono_class_get_method_by_index(MonoClass *klass, int index) {
  ...
  if (gklass && !m_class_get_methods(klass)) {
    ...
    m = mono_class_inflate_generic_method_full_checked(
        m_class_get_methods(gklass->container_class)[index], klass,
        mono_class_get_context(klass), error);
    g_assert(is_ok(error)); /* FIXME don't swallow the error */
    /*
     * If setup_methods () is called later for this class, no duplicates are
     * created, since inflate_generic_method guarantees that only one instance
     * of a method is created for each context.
     */
    /*
 ...
 */
MonoMethod *mono_class_get_inflated_method(MonoClass *klass, MonoMethod *method,
                                           MonoError *error) {
  MonoClass *gklass = mono_class_get_generic_class(klass)->container_class;
  int i, mcount;
  ...
  for (i = 0; i < mcount; ++i) {
    if (gklass_methods[i] == method) {
      ...
      if (klass_methods) {
        ...
        inflated_method = mono_class_inflate_generic_method_full_checked(
            gklass_methods[i], klass, mono_class_get_context(klass), error);
        return_val_if_nok(error, NULL);
      }
 ...
 *
 *   Returns klass->vtable [offset], computing it if neccesary. Returns NULL on
 * failure. LOCKING: Acquires the loader lock.
 */
MonoMethod *mono_class_get_vtable_entry(MonoClass *klass, int offset) {
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    m = mono_class_inflate_generic_method_full_checked(
        m, klass, mono_class_get_context(klass), error);
...
static void collect_implemented_interfaces_aux(MonoClass *klass,
                                               GPtrArray **res,
                                               GHashTable **ifaces,
                                               MonoError *error) {
  int i;
...
GPtrArray *mono_class_get_implemented_interfaces(MonoClass *klass,
                                                 MonoError *error) {
  GPtrArray *res = NULL;
...
/*FIXME verify all callers if they should switch to
 ...
 * mono_class_interface_offset_with_variance:
 *
 * Return the interface offset of \p itf in \p klass. Sets \p non_exact_match to
 * TRUE if the match required variance check If \p itf is an interface with
 * generic variant arguments, try to find the compatible one.
 *
 * Note that this function is responsible for resolving ambiguities. Right now
 * we use whatever ordering interfaces_packed gives us.
 *
 * FIXME figure out MS disambiguation rules and fix this function.
 */
int mono_class_interface_offset_with_variance(MonoClass *klass, MonoClass *itf,
  ...
  *non_exact_match = FALSE;
  ...
  if (m_class_is_array_special_interface(itf) && m_class_get_rank(klass) < 2) {
    ...
    for (i = 0; i < klass_interface_offsets_count; i++) {
      if (mono_class_is_variant_compatible(
              itf, m_class_get_interfaces_packed(klass)[i], FALSE)) {
        found = i;
    ...
    for (i = 0; i < klass_interface_offsets_count; i++) {
      if (mono_class_get_generic_type_definition(
              m_class_get_interfaces_packed(klass)[i]) == gtd) {
        found = i;
        *non_exact_match = TRUE;
        break;
      }
  ...
  if (method->slot == -1) {
    ...
    if (method->slot == -1) {
      ...
      if (!mono_class_is_ginst(method->klass)) {
        ...
        return mono_method_get_vtable_slot(
            ((MonoMethodInflated *)method)->declaring);
      }

      /* This can happen for abstract methods of generic instances due to the
       * shortcut code in mono_class_setup_vtable_general (). */
 ...
 *
 *   Return whenever KLASS has a finalizer, initializing klass->has_finalizer in
 * the process.
 *
 * LOCKING: Acquires the loader lock;
 */
...
gboolean mono_class_is_nullable(MonoClass *klass) {
  MonoGenericClass *gklass = mono_class_try_get_generic_class(klass);
  return gklass &&
         gklass->container_class == mono_defaults.generic_nullable_class;
}

/** if klass is T? return T */
MonoClass *mono_class_get_nullable_param_internal(MonoClass *klass) {
  g_assert(mono_class_is_nullable(klass));
  return mono_class_from_mono_type_internal(
...
gboolean mono_type_is_primitive(MonoType *type) {
  return (type->type >= MONO_TYPE_BOOLEAN && type->type <= MONO_TYPE_R8) ||
         type->type == MONO_TYPE_I || type->type == MONO_TYPE_U;
}

static MonoImage *get_image_for_container(MonoGenericContainer *container) {
  MonoImage *result;
 ...
 */
MonoClass *mono_class_from_generic_parameter(MonoGenericParam *param,
                                             MonoImage *arg2 G_GNUC_UNUSED,
 ...
 * \returns a \c MonoClass for the specified \c MonoType, the value is never
 * NULL.
 */
...
MonoClass *mono_class_from_mono_type_internal(MonoType *type) {
  ...
  case MONO_TYPE_TYPEDBYREF:
    return type->data.klass ? type->data.klass
                            : mono_defaults.typed_reference_class;
  case MONO_TYPE_ARRAY:
    return mono_class_create_bounded_array(type->data.array->eklass,
                                           type->data.array->rank, TRUE);
  case MONO_TYPE_PTR:
 ...
 */
static MonoType *mono_type_retrieve_from_typespec(MonoImage *image,
                                                  guint32 type_spec,
                                                  MonoGenericContext *context,
                                                  gboolean *did_inflate,
                                                  MonoError *error) {
 ...
 * \param context the generic context used to evaluate generic instantiations in
 */
static MonoClass *mono_class_create_from_typespec(MonoImage *image,
                                                  guint32 type_spec,
                                                  MonoGenericContext *context,
                                                  MonoError *error) {
  MonoClass *ret;
 ...
 * \returns A class object describing the array with element type \p
 * element_type and dimension \p rank.
 */
MonoClass *mono_bounded_array_class_get(MonoClass *eclass, guint32 rank,
                                        gboolean bounded) {
 ...
 * \param klass a class
 *
 * Use to get the computed minimum alignment requirements for the specified
 * class.
 *
 ...
 */
static MonoClassField *mono_class_get_field_idx(MonoClass *klass, int idx) {
  mono_class_setup_fields(klass);
  if (mono_class_has_failure(klass))
    return NULL;
  ...
  while (klass) {
    ...
    if (klass_image->uncompressed_metadata) {
       ...
       */
      /*FIXME this is broken for types with multiple fields with the same
       * name.*/
      const char *name = mono_metadata_string_heap(
          klass_image,
          mono_metadata_decode_row_col(&klass_image->tables[MONO_TABLE_FIELD],
                                       idx, MONO_FIELD_NAME));
      int i;
 ...
 * class.
 */
MonoClassField *mono_class_get_field(MonoClass *klass, guint32 field_token) {
 ...
 *
 * Search the class \p klass and it's parents for a field with the name \p name
 * and type \p type.
 *
 * If \p klass is an inflated generic type, the type comparison is done with the
 * equivalent field of its generic type definition.
 *
 * \returns The MonoClassField pointer of the named field or NULL
 */
MonoClassField *mono_class_get_field_from_name_full(MonoClass *klass,
                                                    const char *name,
                                                    MonoType *type) {
 ...
 * the field was loaded from. Don't use this function for fields in dynamic
 * types.
 *
    ...
    for (i = 0; i < fcount; ++i) {
      if (&klass_fields[i] == field) {
        ...
        if (m_class_get_image(klass)->uncompressed_metadata)
          idx = mono_metadata_translate_token_index(m_class_get_image(klass),
                                                    MONO_TABLE_FIELD, idx);
...
static int mono_field_get_index(MonoClassField *field) {
 ...
 */
const char *mono_class_get_field_default_value(MonoClassField *field,
  ...
  if (!def_values) {
    def_values = (MonoFieldDefaultValue *)mono_class_alloc0(
        klass,
        sizeof(MonoFieldDefaultValue) * mono_class_get_field_count(klass));
  ...
  if (!def_values[field_index].data) {
    ...
    cindex = mono_metadata_get_constant_index(
        field_parent_image, mono_class_get_field_token(field), 0);
    if (!cindex)
      return NULL;

    g_assert(!(field->type->attrs & FIELD_ATTRIBUTE_HAS_FIELD_RVA));

    mono_metadata_decode_row(&field_parent_image->tables[MONO_TABLE_CONSTANT],
                             cindex - 1, constant_cols, MONO_CONSTANT_SIZE);
    def_values[field_index].def_type =
        (MonoTypeEnum)constant_cols[MONO_CONSTANT_TYPE];
    mono_memory_barrier();
    def_values[field_index].data = (const char *)mono_metadata_blob_heap(
        field_parent_image, constant_cols[MONO_CONSTANT_VALUE]);
  }
...
static int mono_property_get_index(MonoProperty *prop) {
 ...
 * Return the default value of the field as a pointer into the metadata blob.
 */
const char *mono_class_get_property_default_value(MonoProperty *property,
                                                  MonoTypeEnum *def_type) {
  guint32 cindex;
  guint32 constant_cols[MONO_CONSTANT_SIZE];
  ...
  cindex = mono_metadata_get_constant_index(
      klass_image, mono_class_get_property_token(property), 0);
  if (!cindex)
    return NULL;

  mono_metadata_decode_row(&klass_image->tables[MONO_TABLE_CONSTANT],
                           cindex - 1, constant_cols, MONO_CONSTANT_SIZE);
  *def_type = (MonoTypeEnum)constant_cols[MONO_CONSTANT_TYPE];
  return (const char *)mono_metadata_blob_heap(
      klass_image, constant_cols[MONO_CONSTANT_VALUE]);
}
    ...
    if (info) {
      for (i = 0; i < info->count; ++i) {
        if (&info->events[i] == event)
          return mono_metadata_make_token(MONO_TABLE_EVENT,
                                          info->first + i + 1);
      }
    }
    klass = m_class_get_parent(klass);
...
MonoProperty *mono_class_get_property_from_name_internal(MonoClass *klass,
                                                         const char *name) {
  MONO_REQ_GC_UNSAFE_MODE;
  while (klass) {
    MonoProperty *p;
  ...
  while (klass) {
    ...
    while ((p = mono_class_get_properties(klass, &iter))) {
      if (&info->properties[i] == prop)
        return mono_metadata_make_token(MONO_TABLE_PROPERTY,
                                        info->first + i + 1);
...
char *mono_class_name_from_token(MonoImage *image, guint32 type_token) {
  ...
  case MONO_TOKEN_TYPE_REF: {
    ...
    if (!mono_verifier_verify_typeref_row(image, tidx - 1, error)) {
      char *msg = g_strdup_printf("Invalid type token 0x%08x due to '%s'",
                                  type_token, mono_error_get_message(error));
      mono_error_cleanup(error);
      return msg;
    }
...
static char *mono_assembly_name_from_token(MonoImage *image,
  ...
  case MONO_TOKEN_TYPE_DEF:
    ...
    return g_strdup_printf(
        "%s", image->name ? image->name : "[Could not resolve assembly name");
  case MONO_TOKEN_TYPE_REF: {
    ERROR_DECL(error);
    MonoAssemblyName aname;
    guint32 cols[MONO_TYPEREF_SIZE];
 ...
 * \param context the generic context used to evaluate generic instantiations in
 * \deprecated Functions that expose \c MonoGenericContext are going away in
 * mono 4.0 \returns The \c MonoClass that represents \p type_token in \p image
 */
MonoClass *mono_class_get_full(MonoImage *image, guint32 type_token,
                               MonoGenericContext *context) {
  ERROR_DECL(error);
  MonoClass *klass;
  ...
  if (klass && context &&
      mono_metadata_token_table(type_token) == MONO_TABLE_TYPESPEC)
...
MonoClass *mono_class_get_and_inflate_typespec_checked(
    MonoImage *image, guint32 type_token, MonoGenericContext *context,
    MonoError *error) {
  MonoClass *klass;
 ...
 *
 * \returns The MonoClass that represents \p type_token in \p image, or NULL on
 * error.
 */
MonoClass *mono_class_get_checked(MonoImage *image, guint32 type_token,
                                  MonoError *error) {
  ...
  if (image_is_dynamic(image)) {
    ...
    klass =
        (MonoClass *)mono_lookup_dynamic_token(image, type_token, NULL, error);
    goto done;
  }
  ...
  case MONO_TOKEN_TYPE_SPEC:
    ...
    break;
  default:
    mono_error_set_bad_image(error, image, "Unknown type token %x",
                             type_token & 0xff000000);
  }
  ...
  if (!klass && is_ok(error)) {
    char *name = mono_class_name_from_token(image, type_token);
    char *assembly = mono_assembly_name_from_token(image, type_token);
    mono_error_set_type_load_name(error, name, assembly,
                                  "Could not resolve type with token %08x "
                                  "(expected class '%s' in assembly '%s')",
                                  type_token, name, assembly);
  }
 ...
 *
 * This functions exists to fullfill the fact that sometimes it's desirable to
 * have access to the
 *
 * \returns The MonoType that represents \p type_token in \p image
 */
MonoType *mono_type_get_checked(MonoImage *image, guint32 type_token,
  ...
  // FIXME: this will not fix the very issue for which mono_type_get_full exists
  // -but how to do it then?
  if (image_is_dynamic(image)) {
    MonoClass *klass = (MonoClass *)mono_lookup_dynamic_token(image, type_token,
                                                              context, error);
    return_val_if_nok(error, NULL);
  ...
  if (inflated) {
    ...
    /* FIXME: This is a workaround fo the fact that a typespec token sometimes
     * reference to the generic type definition. A MonoClass::_byval_arg of a
     * generic type definion has type CLASS. Some parts of mono create a
     * GENERICINST to reference a generic type definition and this generates
     * confict with _byval_arg.
     *
     * The long term solution is to chaise this places and make then set
     * MonoType::type correctly.
     * */
 ...
 */
void mono_image_init_name_cache(MonoImage *image) {
  ...
  for (i = 1; i <= t->rows; ++i) {
    ...
    /*
     * Nested types are accessed from the nesting name.  We use the fact that
     * nested types use different visibility flags than toplevel types, thus
     * avoiding the need to grovel through the NESTED_TYPE table
     */
    if (visib >= TYPE_ATTRIBUTE_NESTED_PUBLIC &&
        visib <= TYPE_ATTRIBUTE_NESTED_FAM_OR_ASSEM)
      continue;
    name = mono_metadata_string_heap(image, cols[MONO_TYPEDEF_NAME]);
    nspace = mono_metadata_string_heap(image, cols[MONO_TYPEDEF_NAMESPACE]);

    nspace_index = cols[MONO_TYPEDEF_NAMESPACE];
    nspace_table = (GHashTable *)g_hash_table_lookup(
        name_cache2, GUINT_TO_POINTER(nspace_index));
    if (!nspace_table) {
  ...
  /* Load type names from EXPORTEDTYPES table */
  {
    ...
    for (i = 0; i < t->rows; ++i) {
      ...
      g_hash_table_insert(nspace_table, (char *)name,
                          GUINT_TO_POINTER(mono_metadata_make_token(
                              MONO_TABLE_EXPORTEDTYPE, i + 1)));
    }
  }
 ...
 */
void mono_image_add_to_name_cache(MonoImage *image, const char *nspace,
  ...
  if ((old_index =
           GPOINTER_TO_UINT(g_hash_table_lookup(nspace_table, (char *)name))))
    g_error("overrwritting old token %x on image %s for type %s::%s", old_index,
            image->name, nspace, name);
...
static void find_all_nocase(gpointer key, gpointer value, gpointer user_data) {
  char *name = (char *)key;
...
static void find_nocase(gpointer key, gpointer value, gpointer user_data) {
  char *name = (char *)key;
 ...
 */
MonoClass *mono_class_from_name_case(MonoImage *image, const char *name_space,
                                     const char *name) {
  ERROR_DECL(error);
  MonoClass *res =
      mono_class_from_name_case_checked(image, name_space, name, error);
  mono_error_cleanup(error);
 ...
 * lookups are case insensitive.
 *
 * \returns The MonoClass if the given namespace and name were found, or NULL if
 * it was not found.   The \p error object will contain information about the
 * problem in that case.
 */
MonoClass *mono_class_from_name_case_checked(MonoImage *image,
  ...
  klass = mono_class_from_name_checked_aux(image, name_space, name,
                                           visited_images, FALSE, error);
...
static MonoClass *return_nested_in(MonoClass *klass, char *nested,
...
static MonoClass *search_modules(MonoImage *image, const char *name_space,
                                 const char *name, gboolean case_sensitive,
                                 MonoError *error) {
  ...
  for (i = 0; i < file_table->rows; i++) {
    ...
    if (file_image) {
      if (case_sensitive)
        klass =
            mono_class_from_name_checked(file_image, name_space, name, error);
      else
        klass = mono_class_from_name_case_checked(file_image, name_space, name,
                                                  error);
...
mono_class_from_name_checked_aux(MonoImage *image, const char *name_space,
  ...
  // Checking visited images avoids stack overflows when cyclic references
  // exist.
   ...
   */
  // The AOT cache in get_class_from_name is case-sensitive, so don't bother
  // with it for case-insensitive lookups
  ...
  if (case_sensitive) {
    nspace_table =
        (GHashTable *)g_hash_table_lookup(image->name_cache, name_space);
    ...
    // We're forced to check all matching namespaces, not just the first one
    // found, because our desired type could be in any of the ones that match
    // case-insensitively.
  ...
  if (!token && image_is_dynamic(image) && image->modules) {
    ...
    for (i = 0; i < image->module_count; ++i) {
      ...
      if (case_sensitive)
        ...
        klass =
            mono_class_from_name_case_checked(module, name_space, name, error);
  ...
  if (mono_metadata_token_table(token) == MONO_TABLE_EXPORTEDTYPE) {
    ...
    if ((impl & MONO_IMPLEMENTATION_MASK) == MONO_IMPLEMENTATION_FILE) {
      loaded_image = mono_assembly_load_module_checked(
          image->assembly, impl >> MONO_IMPLEMENTATION_BITS, error);
      if (!loaded_image)
        return NULL;
      klass = mono_class_from_name_checked_aux(loaded_image, name_space, name,
                                               visited_images, case_sensitive,
                                               error);
      if (nested)
        return klass ? return_nested_in(klass, nested, case_sensitive) : NULL;
      return klass;
    } else if ((impl & MONO_IMPLEMENTATION_MASK) ==
      ...
      if (image->references[assembly_idx - 1] == (gpointer)-1)
        return NULL;
      klass = mono_class_from_name_checked_aux(
          image->references[assembly_idx - 1]->image, name_space, name,
          visited_images, case_sensitive, error);
      if (nested)
        return klass ? return_nested_in(klass, nested, case_sensitive) : NULL;
      return klass;
    } else {
 ...
 *
 * Works like mono_class_from_name, but error handling is tricky. It can return
 * NULL and have no error set if the class was not found or it will return NULL
 * and set the error if there was a loading error.
 */
MonoClass *mono_class_from_name_checked(MonoImage *image,
                                        const char *name_space,
                                        const char *name, MonoError *error) {
  MonoClass *klass;
  GHashTable *visited_images;
 ...
 * \param name the type short name.
 *
 * This function works exactly like mono_class_from_name but it will abort if
 * the class is not found. This function should be used by the runtime for
 * critical types to which there's no way to recover but crash If they are
 * missing. Thing of System.Object or System.String.
 */
MonoClass *mono_class_load_from_name(MonoImage *image, const char *name_space,
                                     const char *name) {
  ERROR_DECL(error);
  MonoClass *klass;
  ...
  mono_error_assertf_ok(error, "Could not load runtime critical type %s.%s",
                        name_space, name);
  return klass;
}
 ...
 * \param name the type short name.
 *
 * This function tries to load a type, returning the class was found or NULL
 * otherwise. This function should be used by the runtime when probing for
 * optional types, those that could have being linked out.
 *
 * Big design consideration. This function aborts if there was an error loading
 * the type. This prevents us from missing a type that we would otherwise assume
 * to be available but was not due some error.
 *
 */
MonoClass *mono_class_try_load_from_name(MonoImage *image,
                                         const char *name_space,
                                         const char *name) {
  ERROR_DECL(error);
  MonoClass *klass;
...
static gboolean
mono_interface_implements_interface(MonoClass *interface_implementer,
                                    MonoClass *interface_implemented) {
  int i;
  ERROR_DECL(error);
...
gboolean mono_class_is_subclass_of_internal(MonoClass *klass, MonoClass *klassc,
  ...
  if (check_interfaces && MONO_CLASS_IS_INTERFACE_INTERNAL(klassc) &&
      !MONO_CLASS_IS_INTERFACE_INTERNAL(klass)) {
    if (MONO_CLASS_IMPLEMENTS_INTERFACE(klass,
                                        m_class_get_interface_id(klassc)))
      return TRUE;
  } else if (check_interfaces && MONO_CLASS_IS_INTERFACE_INTERNAL(klassc) &&
             MONO_CLASS_IS_INTERFACE_INTERNAL(klass)) {
    int i;
    ...
    if (!MONO_CLASS_IS_INTERFACE_INTERNAL(klass) &&
        mono_class_has_parent(klass, klassc))
      return TRUE;
  }
...
gboolean mono_class_has_variant_generic_params(MonoClass *klass) {
  ...
  container = mono_class_get_generic_container(
      mono_class_get_generic_class(klass)->container_class);

  for (i = 0; i < container->type_argc; ++i)
    if (mono_generic_container_get_param_info(container, i)->flags &
...
static gboolean
mono_gparam_is_reference_conversible(MonoClass *target, MonoClass *candidate,
  ...
  if (check_for_reference_conv &&
    ...
    MonoGenericParam *gparam =
        m_class_get_byval_arg(candidate)->data.generic_param;
    MonoGenericParamInfo *pinfo = mono_generic_param_info(gparam);

    if (!pinfo || (pinfo->flags &
                   GENERIC_PARAMETER_ATTRIBUTE_REFERENCE_TYPE_CONSTRAINT) == 0)
      return FALSE;
  }
 ...
 */
gboolean mono_class_is_variant_compatible(MonoClass *klass, MonoClass *oklass,
                                          gboolean check_for_reference_conv) {
  int j;
  ...
  klass_argv =
      &mono_class_get_generic_class(klass)->context.class_inst->type_argv[0];
  oklass_argv =
      &mono_class_get_generic_class(oklass)->context.class_inst->type_argv[0];

  for (j = 0; j < container->type_argc; ++j) {
    MonoClass *param1_class = mono_class_from_mono_type_internal(klass_argv[j]);
    MonoClass *param2_class =
        mono_class_from_mono_type_internal(oklass_argv[j]);

    if (m_class_is_valuetype(param1_class) !=
            m_class_is_valuetype(param2_class) ||
        (m_class_is_valuetype(param1_class) && param1_class != param2_class))
      return FALSE;
    ...
    if (param1_class != param2_class) {
      if (mono_generic_container_get_param_info(container, j)->flags &
          MONO_GEN_PARAM_VARIANT) {
        if (!mono_gparam_is_reference_conversible(param1_class, param2_class,
                                                  check_for_reference_conv))
          return FALSE;
      } else if (mono_generic_container_get_param_info(container, j)->flags &
                 MONO_GEN_PARAM_COVARIANT) {
        if (!mono_gparam_is_reference_conversible(param2_class, param1_class,
                                                  check_for_reference_conv))
          return FALSE;
      } else
        return FALSE;
...
static gboolean mono_gparam_is_assignable_from(MonoClass *target,
                                               MonoClass *candidate) {
  ...
  if (cinfo->constraints) {
    for (candidate_class = cinfo->constraints; *candidate_class;
      ...
      if (mono_type_is_reference(cc_byval_arg) &&
          !MONO_CLASS_IS_INTERFACE_INTERNAL(cc))
        class_constraint_satisfied = TRUE;
      else if (!mono_type_is_reference(cc_byval_arg) &&
               !MONO_CLASS_IS_INTERFACE_INTERNAL(cc))
 ...
 */
gboolean mono_class_is_assignable_from_internal(MonoClass *klass,
                                                MonoClass *oklass) {
 ...
 */
void mono_class_is_assignable_from_checked(MonoClass *klass, MonoClass *oklass,
                                           gboolean *result, MonoError *error) {
  g_assert(result);
  if (klass == oklass) {
    *result = TRUE;
    return;
  ...
  /* This can happen if oklass is a tyvar that has a constraint which is another
   * tyvar which in turn has a constraint which is a class type:
   *
   *  class Foo { }
  ...
  if (mono_type_is_generic_argument(oklass_byval_arg)) {
    ...
    MonoClass **constraints =
        mono_generic_container_get_param_info(gparam->owner, gparam->num)
            ->constraints;
    int i;
  ...
  if (MONO_CLASS_IS_INTERFACE_INTERNAL(klass)) {
    ...
    if (mono_class_get_ref_info_handle(oklass) &&
        !m_class_get_interface_bitmap(oklass)) {
      /*
       * oklass might be a generic type parameter but they have
       * interface_offsets set.
       */
      gboolean assign_result =
          mono_reflection_call_is_assignable_to(oklass, klass, error);
      return_if_nok(error);
    ...
    if (MONO_CLASS_IMPLEMENTS_INTERFACE(oklass,
    ...
    if (m_class_is_array_special_interface(klass) &&
      ...
      if (mono_class_get_generic_type_definition(klass) ==
          mono_defaults.generic_ienumerator_class) {
        *result = FALSE;
        return;
      }

      // XXX we could offset this by having the cast target computed at JIT time
      // XXX we could go even further and emit a wrapper that would do the extra
      // type check
      MonoClass *iface_klass = mono_class_from_mono_type_internal(
          mono_class_get_generic_class(klass)
              ->context.class_inst->type_argv[0]);
      MonoClass *obj_klass = m_class_get_cast_class(
          oklass); // This gets us the cast class of element type of the array

      // If the target we're trying to cast to is a valuetype, we must account
      // of weird valuetype equivalences such as IntEnum <> int or uint <> int
      // We can't apply it for ref types as this would go wrong with arrays -
      // IList<byte[]> would have byte tested
      if (!mono_class_is_nullable(iface_klass)) {
        ...
        if (!(m_class_is_valuetype(obj_klass) &&
              !m_class_is_valuetype(iface_klass)) &&
            mono_class_is_assignable_from_internal(iface_klass, obj_klass)) {
          *result = TRUE;
          return;
        }
      }
    ...
    if (mono_class_has_variant_generic_params(klass)) {
      ...
      /*klass is a generic variant interface, We need to extract from oklass a
       * list of ifaces which are viable candidates.*/
    ...
    *result = FALSE;
    ...
    if (mono_class_has_variant_generic_params(klass) &&
        mono_class_is_variant_compatible(klass, oklass, FALSE)) {
      *result = TRUE;
      return;
    }
     ...
     *         var t =
     * typeof(G<>).GetTypeInfo().GenericTypeParameters[0].MakeArrayType(); var i
     * = typeof(I).MakeArrayType(); var u =
     * typeof(H<>).GetTypeInfo().GenericTypeParameters[0].MakeArrayType();
     *         Console.WriteLine("I[] assignable from T[] ? {0}",
     * i.IsAssignableFrom(t)); Console.WriteLine("I[] assignable from U[] ?
     * {0}", i.IsAssignableFrom(u));
     *     }
     * }
     * ```
     * This should print:
    ...
    if (MONO_CLASS_IS_INTERFACE_INTERNAL(eclass)) {
      ...
      if (mono_type_is_generic_argument(eoclass_byval_arg)) {
        ...
        int eomask = eoinfo->flags &
                     GENERIC_PARAMETER_ATTRIBUTE_SPECIAL_CONSTRAINTS_MASK;
        // check for class constraint
        if ((eomask & GENERIC_PARAMETER_ATTRIBUTE_REFERENCE_TYPE_CONSTRAINT) ==
            0) {
          *result = FALSE;
          return;
        }
    ...
    if (mono_class_is_nullable(oklass))
      mono_class_is_assignable_from_checked(m_class_get_cast_class(klass),
...
static gboolean mono_class_is_variant_compatible_slow(MonoClass *klass,
                                                      MonoClass *oklass) {
  int j;
  MonoType **klass_argv, **oklass_argv;
  MonoClass *klass_gtd = mono_class_get_generic_type_definition(klass);
...
static gboolean mono_class_implement_interface_slow(MonoClass *target,
                                                    MonoClass *candidate) {
  ERROR_DECL(error);
  int i;
  ...
  do {
    ...
    /*A TypeBuilder can have more interfaces on tb->interfaces than on
     * candidate->interfaces*/
    if (image_is_dynamic(m_class_get_image(candidate)) &&
        !m_class_was_typebuilder(candidate)) {
      MonoReflectionTypeBuilder *tb =
          mono_class_get_ref_info_raw(candidate); /* FIXME use handles */
      int j;
      if (tb && tb->interfaces) {
        for (j = mono_array_length_internal(tb->interfaces) - 1; j >= 0; --j) {
          MonoReflectionType *iface =
              mono_array_get_internal(tb->interfaces, MonoReflectionType *, j);
          MonoClass *iface_class;

          /* we can't realize the type here since it can do pretty much
           * anything. */
          if (!iface->type)
            continue;
          ...
          if (iface_class == target)
            return TRUE;
          if (is_variant &&
              mono_class_is_variant_compatible_slow(target, iface_class))
            return TRUE;
      ...
      ICollection<sbyte> x byte [] won't work because candidate->interfaces, for
      byte[], won't have IList<sbyte>. A possible way to fix this would be to
      move that to setup_interfaces from setup_interface_offsets.
      */
      ...
      for (i = 0; i < candidate_interface_count; ++i) {
        ...
        if (is_variant && mono_class_is_variant_compatible_slow(
                              target, candidate_interfaces[i]))
          return TRUE;

        if (mono_class_implement_interface_slow(target,
                                                candidate_interfaces[i]))
          return TRUE;
      }
    }
 ...
 * This function does the same as mono_class_is_assignable_from_internal but is
 * safe to be used from mono_class_init_internal context.
 */
gboolean mono_class_is_assignable_from_slow(MonoClass *target,
                                            MonoClass *candidate) {
  if (candidate == target)
    return TRUE;
  ...
  if (m_class_get_rank(target)) {
    ...
    /* vectors vs. one dimensional arrays */
    if (m_class_get_byval_arg(target)->type !=
        m_class_get_byval_arg(candidate)->type)
      return FALSE;
  ...
  if (constraints) {
    ...
    for (i = 0; constraints[i]; ++i) {
      ...
      if (mono_type_is_generic_argument(constraint_type)) {
        MonoGenericParam *constraint_param =
            constraint_type->data.generic_param;
        MonoGenericParamInfo *constraint_info =
            mono_generic_param_info(constraint_param);
        if ((constraint_info->flags &
             GENERIC_PARAMETER_ATTRIBUTE_REFERENCE_TYPE_CONSTRAINT) == 0 &&
            (constraint_info->flags &
             GENERIC_PARAMETER_ATTRIBUTE_VALUE_TYPE_CONSTRAINT) == 0)
          continue;
      }
...
MonoMethod *mono_class_get_cctor(MonoClass *klass) {
  ...
  if (image_is_dynamic(m_class_get_image(klass))) {
    /*
     * has_cctor is not set for these classes because mono_class_init_internal
     * () is not run for them.
     */
    result = mono_class_get_method_from_name_checked(
        klass, ".cctor", -1, METHOD_ATTRIBUTE_SPECIAL_NAME, error);
    mono_error_assert_msg_ok(error,
  ...
  if (mono_class_is_ginst(klass) && !m_class_get_methods(klass)) {
    result = mono_class_get_inflated_method(
        klass,
        mono_class_get_cctor(
            mono_class_get_generic_class(klass)->container_class),
        error);
    mono_error_assert_msg_ok(
        error, "Could not lookup inflated class cctor"); /* FIXME do proper
                                                            error handling */
    return result;
  }

  if (mono_class_get_cached_class_info(klass, &cached_info)) {
    result = mono_get_method_checked(
        m_class_get_image(klass), cached_info.cctor_token, klass, NULL, error);
    mono_error_assert_msg_ok(
        error, "Could not lookup class cctor from cached metadata");
    return result;
  }
...
MonoMethod *mono_class_get_finalizer(MonoClass *klass) {
  ...
  if (mono_class_get_cached_class_info(klass, &cached_info)) {
    ERROR_DECL(error);
    MonoMethod *result =
        mono_get_method_checked(cached_info.finalize_image,
                                cached_info.finalize_token, NULL, NULL, error);
    mono_error_assert_msg_ok(error,
 ...
 * \param klass
 *
 * \returns The number of bytes an element of type \p klass uses when stored
 * into an array.
 */
 ...
 * mono_ldtoken:
 */
gpointer mono_ldtoken(MonoImage *image, guint32 token, MonoClass **handle_class,
                      MonoGenericContext *context) {
  ERROR_DECL(error);
  gpointer res =
      mono_ldtoken_checked(image, token, handle_class, context, error);
  mono_error_assert_ok(error);
  return res;
}

gpointer mono_ldtoken_checked(MonoImage *image, guint32 token,
  ...
  if (image_is_dynamic(image)) {
    ...
    gpointer obj = mono_lookup_dynamic_token_class(
        image, token, TRUE, &tmp_handle_class, context, error);
  ...
  case MONO_TOKEN_FIELD_DEF: {
    MonoClass *klass;
    guint32 type = mono_metadata_typedef_from_field(
        image, mono_metadata_token_index(token));
    if (!type) {
    ...
    klass = mono_class_get_and_inflate_typespec_checked(
        image, MONO_TOKEN_TYPE_DEF | type, context, error);
    if (!klass)
      return NULL;
  ...
  case MONO_TOKEN_MEMBER_REF: {
    ...
    mono_metadata_decode_row(&image->tables[MONO_TABLE_MEMBERREF],
                             mono_metadata_token_index(token) - 1, cols,
                             MONO_MEMBERREF_SIZE);
    ...
    if (*sig == 0x6) { /* it's a field */
      ...
      field =
          mono_field_from_token_checked(image, token, &klass, context, error);
      if (handle_class)
        *handle_class = mono_defaults.fieldhandle_class;
      return field;
    } else {
...
gpointer mono_lookup_dynamic_token(MonoImage *image, guint32 token,
                                   MonoGenericContext *context,
                                   MonoError *error) {
  MonoClass *handle_class;
  error_init(error);
  return mono_reflection_lookup_dynamic_token(image, token, TRUE, &handle_class,
                                              context, error);
}

gpointer mono_lookup_dynamic_token_class(MonoImage *image, guint32 token,
                                         gboolean valid_token,
                                         MonoClass **handle_class,
                                         MonoGenericContext *context,
                                         MonoError *error) {
  return mono_reflection_lookup_dynamic_token(image, token, valid_token,
                                              handle_class, context, error);
}
...
gboolean mono_class_get_cached_class_info(MonoClass *klass,
                                          MonoCachedClassInfo *res) {
 ...
 * \param klass the \c MonoClass to act on
 *
 * Use this method to determine if the provided \c MonoClass* represents a value
 * type, or a reference type.
 *
 * \returns TRUE if the \c MonoClass represents a \c ValueType, FALSE if it
 ...
 * \param klass the \c MonoClass to act on
 *
 * Use this function to determine if the provided \c MonoClass* represents an
 * enumeration.
 *
 ...
 */
MonoType *mono_class_enum_basetype_internal(MonoClass *klass) {
 ...
 * \param klass the \c MonoClass to act on
 *
 * Use this function to obtain the class that the provided \c MonoClass* is
 * nested on.
 *
 * If the return is NULL, this indicates that this class is not nested.
 *
 * \returns The container type where this type is nested or NULL if this type is
 * not a nested type.
 */
 ...
 */
int mono_class_get_rank(MonoClass *klass) { return m_class_get_rank(klass); }
 ...
 *
 * You must pass a \c gpointer that points to zero and is treated as an opaque
 * handle to iterate over all of the elements.  When no more values are
 * available, the return value is NULL.
 *
 * \returns a \c MonoClassField* on each iteration, or NULL when no more fields
 * are available.
 */
...
MonoClassField *mono_class_get_fields_internal(MonoClass *klass,
                                               gpointer *iter) {
  MonoClassField *field;
  if (!iter)
    return NULL;
 ...
 * available, the return value is NULL.
 *
 * \returns a \c MonoMethod on each iteration or NULL when no more methods are
 * available.
 */
MonoMethod *mono_class_get_methods(MonoClass *klass, gpointer *iter) {
  ...
  if (!*iter) {
    ...
    MonoMethod **klass_methods = m_class_get_methods(klass);
    /*
     * We can't fail lookup of methods otherwise the runtime will burst in
     * flames on all sort of places.
  ...
  method++;
  if (method <
      &m_class_get_methods(klass)[mono_class_get_method_count(klass)]) {
    *iter = method;
    return *method;
  }
  return NULL;
 ...
 *
 * You must pass a gpointer that points to zero and is treated as an opaque
 * handle to iterate over all of the elements.  When no more values are
 * available, the return value is NULL.
 *
 * Returns: a \c MonoProperty* on each invocation, or NULL when no more are
 * available.
 */
 ...
 * available, the return value is NULL.
 *
 * \returns a \c MonoEvent* on each invocation, or NULL when no more are
 * available.
 */
 ...
 * \param klass the \c MonoClass to act on
 *
 * This routine is an iterator routine for retrieving the interfaces implemented
 * by this class.
 *
 * You must pass a \c gpointer that points to zero and is treated as an opaque
 * handle to iterate over all of the elements.  When no more values are
 * available, the return value is NULL.
 *
 * \returns a \c MonoClass* on each invocation, or NULL when no more are
 * available.
 */
MonoClass *mono_class_get_interfaces(MonoClass *klass, gpointer *iter) {
  ...
  iface++;
  if (iface <
      &m_class_get_interfaces(klass)[m_class_get_interface_count(klass)]) {
    *iter = iface;
    return *iface;
  }
  return NULL;
 ...
 * \param klass the \c MonoClass to act on
 *
 * This routine is an iterator routine for retrieving the nested types of a
 * class. This works only if \p klass is non-generic, or a generic type
 * definition.
 *
 * You must pass a \c gpointer that points to zero and is treated as an opaque
 * handle to iterate over all of the elements.  When no more values are
...
static mono_bool class_implements_interface_ignore_generics(MonoClass *klass,
                                                            MonoClass *iface) {
  int i;
  ERROR_DECL(error);
 ...
 */
const char *mono_field_get_name(MonoClassField *field) { return field->name; }
...
MonoType *mono_field_get_type_internal(MonoClassField *field) {
  ...
  if (!is_ok(error)) {
    mono_trace_warning(MONO_TRACE_TYPE, "Could not load field's type due to %s",
...
static const char *mono_field_get_rva(MonoClassField *field) {
  guint32 rva;
  int field_index;
  MonoClass *klass = field->parent;
  MonoFieldDefaultValue *def_values;
  ...
  if (!def_values[field_index].data &&
    ...
    int first_field_idx = mono_class_get_first_field_idx(klass);
    mono_metadata_field_info(m_class_get_image(field->parent),
                             first_field_idx + field_index, NULL, &rva, NULL);
    if (!rva)
      g_warning("field %s in %s should have RVA data, but hasn't",
                mono_field_get_name(field), m_class_get_name(field->parent));
    def_values[field_index].data =
        mono_image_rva_map(m_class_get_image(field->parent), rva);
  }
 ...
 */
const char *mono_property_get_name(MonoProperty *prop) { return prop->name; }
 ...
 */
MonoClass *mono_property_get_parent(MonoProperty *prop) { return prop->parent; }
 ...
 */
guint32 mono_property_get_flags(MonoProperty *prop) { return prop->attrs; }
 ...
 */
const char *mono_event_get_name(MonoEvent *event) { return event->name; }
 ...
 */
MonoMethod *mono_event_get_add_method(MonoEvent *event) { return event->add; }
 ...
 */
MonoClass *mono_event_get_parent(MonoEvent *event) { return event->parent; }
 ...
 */
guint32 mono_event_get_flags(MonoEvent *event) { return event->attrs; }
...
MonoMethod *mono_class_get_method_from_name(MonoClass *klass, const char *name,
  ...
  MONO_ENTER_GC_UNSAFE;
  ERROR_DECL(error);
  result = mono_class_get_method_from_name_checked(klass, name, param_count, 0,
                                                   error);
  mono_error_cleanup(error);
  MONO_EXIT_GC_UNSAFE;
  return result;
...
MonoMethod *mono_find_method_in_metadata(MonoClass *klass, const char *name,
  ...
  for (i = 0; i < mcount; ++i) {
    ...
    mono_metadata_decode_table_row(klass_image, MONO_TABLE_METHOD,
                                   first_idx + i, cols, MONO_METHOD_SIZE);

    if (!strcmp(mono_metadata_string_heap(klass_image, cols[MONO_METHOD_NAME]),
                name)) {
      method = mono_get_method_checked(
          klass_image, MONO_TOKEN_METHOD_DEF | (first_idx + i + 1), klass, NULL,
          error);
      if (!method) {
        mono_error_cleanup(error); /* FIXME don't swallow the error */
        continue;
 ...
 * It only works if there are no multiple signatures for any given method name.
 */
MonoMethod *mono_class_get_method_from_name_flags(MonoClass *klass,
                                                  const char *name,
                                                  int param_count, int flags) {
  MonoMethod *method;
  MONO_ENTER_GC_UNSAFE;
  ERROR_DECL(error);
  method = mono_class_get_method_from_name_checked(klass, name, param_count,
                                                   flags, error);
  mono_error_cleanup(error);
  MONO_EXIT_GC_UNSAFE;
  return method;
 ...
 * It only works if there are no multiple signatures for any given method name.
 */
MonoMethod *mono_class_get_method_from_name_checked(MonoClass *klass,
                                                    const char *name,
                                                    int param_count, int flags,
                                                    MonoError *error) {
  MonoMethod *res = NULL;
  ...
  if (mono_class_is_ginst(klass) && !m_class_get_methods(klass)) {
    ...
    if (res)
      res = mono_class_inflate_generic_method_full_checked(
          res, klass, mono_class_get_context(klass), error);
  ...
  if (m_class_get_methods(klass) || !MONO_CLASS_HAS_STATIC_METADATA(klass)) {
    mono_class_setup_methods(klass);
    /*
    We can't fail lookup of methods otherwise the runtime will burst in flames
    on all sort of places. See mono/tests/array_load_exception.il
    FIXME we should better report this error to the caller
     */
    MonoMethod **klass_methods = m_class_get_methods(klass);
    if (!klass_methods)
 ...
 * The error is stored as a MonoErrorBoxed as with
 * mono_error_set_type_load_class() Note that only the first failure is kept.
 *
 * LOCKING: Acquires the loader lock.
 *
 * \returns FALSE if a failure was already set on the class, or TRUE otherwise.
 */
gboolean mono_class_set_type_load_failure(MonoClass *klass, const char *fmt,
                                          ...) {
...
static gboolean is_nesting_type(MonoClass *outer_klass,
                                MonoClass *inner_klass) {
 ...
 */
gboolean mono_class_has_parent_and_ignore_generics(MonoClass *klass,
                                                   MonoClass *parent) {
  int i;
  ...
  for (i = 0; i < m_class_get_idepth(klass); ++i) {
    if (parent == mono_class_get_generic_type_definition(
                      m_class_get_supertypes(klass)[i]))
      return TRUE;
  }

  if (MONO_CLASS_IS_INTERFACE_INTERNAL(parent) &&
      class_implements_interface_ignore_generics(klass, parent))
    return TRUE;

  return FALSE;
}
/*
 * Subtype can only access parent members with family protection if the site
 *object is subclass of Subtype. For example: class A { protected int x; } class
 *B : A { void valid_access () { B b; b.x = 0;
 *  }
 *  void invalid_access () {
 *		A a;
 * 		a.x = 0;
 ...
 * }
 * */
static gboolean is_valid_family_access(MonoClass *access_klass,
                                       MonoClass *member_klass,
                                       MonoClass *context_klass) {
  if (MONO_CLASS_IS_INTERFACE_INTERNAL(member_klass) &&
    ...
    if (!class_implements_interface_ignore_generics(access_klass, member_klass))
      return FALSE;
  } else if (member_klass != access_klass &&
  ...
  if (access_klass != member_klass &&
      !mono_class_has_parent_and_ignore_generics(context_klass, access_klass))
    return FALSE;
  return TRUE;
}

static gboolean can_access_internals(MonoAssembly *accessing,
                                     MonoAssembly *accessed) {
  GSList *tmp;
  ...
  /* extra safety under CoreCLR - the runtime does not verify the strongname
   * signatures
   * anywhere so untrusted friends are not safe to access platform's code
   * internals */
  if (mono_security_core_clr_enabled()) {
    if (!mono_security_core_clr_can_access_internals(accessing->image,
                                                     accessed->image))
      return FALSE;
  }
  ...
  for (tmp = accessed->friend_assembly_names; tmp; tmp = tmp->next) {
    ...
    if (friend_->public_key_token[0]) {
      if (!accessing->aname.public_key_token[0])
        continue;
      if (!mono_public_tokens_are_equal(friend_->public_key_token,
                                        accessing->aname.public_key_token))
        continue;
    }
    return TRUE;
  }
...
/*
 * If klass is a generic type or if it is derived from a generic type, return
 * the MonoClass of the generic definition Returns NULL if not found
 */
static MonoClass *get_generic_definition_class(MonoClass *klass) {
  while (klass) {
    MonoGenericClass *gklass = mono_class_try_get_generic_class(klass);
...
static gboolean can_access_instantiation(MonoClass *access_klass,
  ...
  for (i = 0; i < ginst->type_argc; ++i) {
    ...
    case MONO_TYPE_ARRAY:
      ...
      break;
    case MONO_TYPE_PTR:
      if (!can_access_type(access_klass,
                           mono_class_from_mono_type_internal(type->data.type)))
        return FALSE;
      break;
    case MONO_TYPE_CLASS:
    case MONO_TYPE_VALUETYPE:
...
static gboolean can_access_type(MonoClass *access_klass,
  ...
  MonoAssembly *access_klass_assembly =
      m_class_get_image(access_klass)->assembly;
  MonoAssembly *member_klass_assembly =
      m_class_get_image(member_klass)->assembly;

  if (access_klass_assembly &&
      m_class_get_image(access_klass)->assembly->corlib_internal)
    return TRUE;

  if (m_class_get_element_class(access_klass) &&
  ...
  if (m_class_get_element_class(member_klass) &&
  ...
  access_level =
      mono_class_get_flags(member_klass) & TYPE_ATTRIBUTE_VISIBILITY_MASK;

  if (mono_type_is_generic_argument(m_class_get_byval_arg(member_klass)))
    return TRUE;

  if (mono_class_is_ginst(member_klass) &&
      !can_access_instantiation(
          access_klass,
          mono_class_get_generic_class(member_klass)->context.class_inst))
    return FALSE;

  if (is_nesting_type(access_klass, member_klass) ||
      (m_class_get_nested_in(access_klass) &&
       is_nesting_type(m_class_get_nested_in(access_klass), member_klass)))
    return TRUE;

  /*Non nested type with nested visibility. We just fail it.*/
  if (access_level >= TYPE_ATTRIBUTE_NESTED_PRIVATE &&
      access_level <= TYPE_ATTRIBUTE_NESTED_FAM_OR_ASSEM &&
      m_class_get_nested_in(member_klass) == NULL)
    return FALSE;
  ...
  case TYPE_ATTRIBUTE_NESTED_PUBLIC:
    return member_klass_nested_in &&
           can_access_type(access_klass, member_klass_nested_in);
  ...
  case TYPE_ATTRIBUTE_NESTED_FAMILY:
    return mono_class_has_parent_and_ignore_generics(
        access_klass, m_class_get_nested_in(member_klass));
  ...
  case TYPE_ATTRIBUTE_NESTED_FAM_AND_ASSEM:
    return can_access_internals(
               access_klass_assembly,
               m_class_get_image(member_klass_nested_in)->assembly) &&
           mono_class_has_parent_and_ignore_generics(access_klass,
                                                     member_klass_nested_in);
...
static gboolean can_access_member(MonoClass *access_klass,
                                  MonoClass *member_klass,
                                  MonoClass *context_klass, int access_level) {
  ...
  MonoGenericClass *access_gklass =
  ...
  if (((access_gklass && access_gklass->container_class) ||
    ...
    if (can_access_member(access_container, member_generic_def, context_klass,
                          access_level))
      return TRUE;
  }
  ...
  case FIELD_ATTRIBUTE_FAM_AND_ASSEM:
    if (is_valid_family_access(access_klass, member_klass, context_klass) &&
        can_access_internals(access_klass_assembly,
                             member_klass_image->assembly))
      return TRUE;
    return FALSE;
  case FIELD_ATTRIBUTE_ASSEMBLY:
    return can_access_internals(access_klass_assembly,
                                member_klass_image->assembly);
 ...
 *
 * \returns TRUE if the given \p method is allowed to access the \p field while
 * following the accessibility rules of the CLI.
 */
gboolean mono_method_can_access_field(MonoMethod *method,
                                      MonoClassField *field) {
  /* FIXME: check all overlapping fields */
  int can = can_access_member(method->klass, field->parent, NULL,
                              mono_field_get_type_internal(field)->attrs &
                                  FIELD_ATTRIBUTE_FIELD_ACCESS_MASK);
  if (!can) {
    MonoClass *nested = m_class_get_nested_in(method->klass);
    while (nested) {
      can = can_access_member(nested, field->parent, NULL,
                              mono_field_get_type_internal(field)->attrs &
                                  FIELD_ATTRIBUTE_FIELD_ACCESS_MASK);
      if (can)
        return TRUE;
...
static MonoMethod *mono_method_get_method_definition(MonoMethod *method) {
  while (method->is_inflated)
 ...
 *
 * Used to determine if the \p method is allowed to access the specified \p
 * called method.
 *
 * \returns TRUE if the given \p method is allowed to invoke the \p called while
 * following the accessibility rules of the CLI.
 */
gboolean mono_method_can_access_method(MonoMethod *method, MonoMethod *called) {
 ...
 *
 * This function must be used with instance calls, as they have more strict
 * family accessibility. It can be used with static methods, but context_klass
 * should be NULL.
 *
 * Returns: TRUE if caller have proper visibility and acessibility to @called
 */
gboolean mono_method_can_access_method_full(MonoMethod *method,
                                            MonoMethod *called,
                                            MonoClass *context_klass) {
  /* Wrappers are except from access checks */
  if (method->wrapper_type != MONO_WRAPPER_NONE ||
      called->wrapper_type != MONO_WRAPPER_NONE)
    return TRUE;

  MonoClass *access_class = method->klass;
  MonoClass *member_class = called->klass;
  int can =
      can_access_member(access_class, member_class, context_klass,
                        called->flags & METHOD_ATTRIBUTE_MEMBER_ACCESS_MASK);
  if (!can) {
    MonoClass *nested = m_class_get_nested_in(access_class);
    while (nested) {
      can = can_access_member(nested, member_class, context_klass,
                              called->flags &
 ...
 *
 * This function must be used with instance fields, as they have more strict
 * family accessibility. It can be used with static fields, but context_klass
 * should be NULL.
 *
 * Returns: TRUE if caller have proper visibility and acessibility to @field
 */
gboolean mono_method_can_access_field_full(MonoMethod *method,
                                           MonoClassField *field,
                                           MonoClass *context_klass) {
  MonoClass *access_class = method->klass;
  ...
  int can =
      can_access_member(access_class, member_class, context_klass,
                        field->type->attrs & FIELD_ATTRIBUTE_FIELD_ACCESS_MASK);
  if (!can) {
    MonoClass *nested = m_class_get_nested_in(access_class);
    while (nested) {
      can = can_access_member(nested, member_class, context_klass,
 ...
 */
gboolean mono_class_can_access_class(MonoClass *source_class,
                                     MonoClass *target_class) {
 ...
 *
 * FIXME: TypeBuilder enums are allowed to implement interfaces, but since they
 * cannot have methods, only empty interfaces are possible
 ...
 * FIXME: TypeBuilder enums can have any kind of static fields, but the spec is
 * very explicit about that (P II 14.3)
 *
...
gboolean mono_class_is_valid_enum(MonoClass *klass) {
  ...
  g_assert(m_class_is_enumtype(klass));
  MonoClass *klass_parent = m_class_get_parent(klass);
  /* we cannot test against mono_defaults.enum_class, or mcs won't be able to
   * compile the System namespace*/
  if (!klass_parent || strcmp(m_class_get_name(klass_parent), "Enum") ||
      strcmp(m_class_get_name_space(klass_parent), "System")) {
    return FALSE;
  }
...
mono_generic_class_is_generic_type_definition(MonoGenericClass *gklass) {
  return gklass->context.class_inst ==
...
void mono_field_resolve_type(MonoClassField *field, MonoError *error) {
  MonoClass *klass = field->parent;
  MonoImage *image = m_class_get_image(klass);
  MonoClass *gtd = mono_class_is_ginst(klass)
  ...
  if (gtd) {
    ...
    if (!is_ok(error)) {
      ...
      mono_class_set_type_load_failure(
          klass, "Could not load generic type of field '%s:%s' (%d) due to: %s",
          full_name, gfield->name, field_idx, mono_error_get_message(error));
      g_free(full_name);
    }

    ftype = mono_class_inflate_generic_type_no_copy(
        image, gtype, mono_class_get_context(klass), error);
    if (!is_ok(error)) {
      char *full_name = mono_type_get_full_name(klass);
      mono_class_set_type_load_failure(
          klass,
          "Could not load instantiated type of field '%s:%s' (%d) due to: %s",
          full_name, field->name, field_idx, mono_error_get_message(error));
      g_free(full_name);
    }
  } else {
    const char *sig;
    ...
    if (!mono_verifier_verify_field_signature(image, cols[MONO_FIELD_SIGNATURE],
                                              error)) {
      mono_class_set_type_load_failure(klass, "%s",
                                       mono_error_get_message(error));
      return;
    }
    ...
    if (!ftype) {
      char *full_name = mono_type_get_full_name(klass);
      mono_class_set_type_load_failure(
          klass, "Could not load type of field '%s:%s' (%d) due to: %s",
          full_name, field->name, field_idx, mono_error_get_message(error));
      g_free(full_name);
    }
  }
...
static guint32 mono_field_resolve_flags(MonoClassField *field) {
  ...
  if (gtd) {
    ...
    return mono_metadata_decode_table_row_col(image, MONO_TABLE_FIELD, idx,
                                              MONO_FIELD_FLAGS);
  }
}
...
GENERATE_TRY_GET_CLASS_WITH_CACHE(safehandle, "System.Runtime.InteropServices",
                                  "SafeHandle")
 ...
 * method from an ancestor.  If \p definition is FALSE, returns the method in
 * the superclass of the given method.  If \p definition is TRUE, return the
 * method in the ancestor class where it was first declared.  The type arguments
 * will be inflated in the ancestor classes.  If the method is not associated
 * with a class, or isn't virtual, returns the method itself.  On failure
 * returns NULL and sets \p error.
 */
MonoMethod *mono_method_get_base_method(MonoMethod *method, gboolean definition,
                                        MonoError *error) {
  ...
  klass = method->klass;
  if (mono_class_is_gtd(klass)) {
    /* If we get a GTD like Foo`2 replace look instead at its instantiation with
     * its own generic params: Foo`2<!0, !1>. */
...
retry:
  if (definition) {
    /* At the end of the loop, klass points to the eldest class that has this
     * virtual function slot. */
    for (parent = m_class_get_parent(klass); parent != NULL;
         parent = m_class_get_parent(parent)) {
      /* on entry, klass is either a plain old non-generic class and
         generic_inst == NULL or klass is the generic container class and
         generic_inst is the instantiation.

         when we go to the parent, if the parent is an open constructed type, we
         need to replace the type parameters by the definitions from the
         generic_inst, and then take it apart again into the klass and the
         generic_inst.
         ...
         if at each iteration the parent isn't open, we can skip inflating it.
         if at some iteration the parent isn't generic (after possible
         inflation), we set generic_inst to NULL;
      */
      MonoGenericContext *parent_inst = NULL;
      if (mono_class_is_open_constructed_type(m_class_get_byval_arg(parent))) {
        parent = mono_class_inflate_generic_class_checked(parent, generic_inst,
                                                          error);
        return_val_if_nok(error, NULL);
      }
    ...
    if (mono_class_is_open_constructed_type(m_class_get_byval_arg(klass))) {
      klass =
          mono_class_inflate_generic_class_checked(klass, generic_inst, error);
      return_val_if_nok(error, NULL);
...
gboolean mono_class_has_default_constructor(MonoClass *klass,
  ...
  for (i = 0; i < mcount; ++i) {
    ...
    if (mono_method_is_constructor(method) &&
        ...
        (!public_only ||
         (method->flags & METHOD_ATTRIBUTE_MEMBER_ACCESS_MASK) ==
