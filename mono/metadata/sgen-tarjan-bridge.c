...
static MonoGCBridgeObjectKind class_kind(MonoClass *klass) {
  MonoGCBridgeObjectKind res = mono_bridge_callbacks.bridge_class_kind(klass);

  /* If it's a bridge, nothing we can do about it. */
  if (res == GC_BRIDGE_TRANSPARENT_BRIDGE_CLASS ||
      res == GC_BRIDGE_OPAQUE_BRIDGE_CLASS)
    return res;

  /* Non bridge classes with no pointers will never point to a bridge, so we can
   * savely ignore them. */
  ...
  if (m_class_get_rank(klass) == 1) {
    ...
    /* FIXME the bridge check can be quite expensive, cache it at the class
     * level. */
    /* An array of a sealed type that is not a bridge will never get to a bridge
     */
    if ((mono_class_get_flags(elem_class) & TYPE_ATTRIBUTE_SEALED) &&
        !m_class_has_references(elem_class) &&
        !mono_bridge_callbacks.bridge_class_kind(elem_class)) {
      SGEN_LOG(6, "class %s is opaque\n", m_class_get_name(klass));
      return GC_BRIDGE_OPAQUE_CLASS;
    }
  }
 ...
 * The idea here is as long as the reference fanin and fanout on a node are both
 * 2 or greater, then removing that node will result in a net increase in edge
 * count. So the question is which node removals are counterproductive (i.e.,
 * how many edges saved balances out one node added). The number of edges saved
 * by preserving a node is (fanin*fanout - fanin - fanout).
 *
 * With all that in mind:
 *
 * - HEAVY_REFS_MIN is the number that *both* fanin and fanout must meet to
 * preserve the node.
 * - HEAVY_COMBINED_REFS_MIN is the number (fanin*fanout) must meet to preserve
 * the node.
 *
 * Note HEAVY_COMBINED_REFS_MIN must be <= 2*INCOMING_COLORS_MAX, or we won't
 * know the true fanin.
 */
...
/* Used in ColorData:
 * The higher INCOMING_COLORS_BITS is the higher HEAVY_COMBINED_REFS_MIN can be
 * (see above). However, API_INDEX_BITS + INCOMING_COLORS_BITS must be equal to
 * 31, and if API_INDEX_BITS is too low then terrible things will happen if too
 * many colors are generated. (The number of colors we will ever attempt to
 * generate is currently naturally limited by the JNI GREF limit.)
 */
...
// ScanData state
enum { INITIAL, SCANNED, FINISHED_ON_STACK, FINISHED_OFF_STACK };

/*
Optimizations:
        We can split this data structure in two, those with bridges and those
without (and only bridgeless need to record incoming_colors)
*/
typedef struct {
  ...
  DynPtrArray bridges;
  // Index of this color's MonoGCBridgeSCC in the array passed to the client (or
  // -1 for none)
...
typedef struct _ScanData {
  // FIXME this can be eliminated; if we have a ScanData we generally looked it
  // up from its GCObject
  GCObject *obj;
  // We use the sgen lock_word in GCObject to store a pointer to the ScanData.
  // Cache the original here to restore later:
  mword lock_word;

  ColorData *color;
  // If this object isn't the scc root, we still need to store the xref colors
  // on it, after computing low index, so we can add them to the scc that this
  // object is part of.
  DynPtrArray xrefs;
  ...
  // Similar to lock_word, we use these bits in the GCObject as scratch space
  // then restore them when done
  unsigned obj_state : 2;
} ScanData;

// If true, disable an optimization where sometimes SCC nodes don't contain any
// bridge objects, in order to reduce total xrefs.
static gboolean disable_non_bridge_scc;

/* Should color be made visible to client even though it has no bridges?
 * True if we predict the number of reduced edges to be enough to justify the
 * extra node.
 */
static gboolean bridgeless_color_is_heavy(ColorData *data) {
...
static gboolean color_visible_to_client(ColorData *data) {
...
// The Tarjan algorithm is normally defined recursively; here scan_stack
// simulates the call stack of a recursive algorithm, and loop_stack is the
// stack structure used by the algorithm itself.
...
// As we traverse the graph, which ColorData objects are accessible from our
// current position?
...
static size_t setup_time, tarjan_time, scc_setup_time, gather_xref_time,
    xref_setup_time, cleanup_time;
...
static ObjectBucket *new_object_bucket(void) {
  ObjectBucket *res =
      (ObjectBucket *)sgen_alloc_internal(INTERNAL_MEM_TARJAN_OBJ_BUCKET);
  res->next_data = &res->data[0];
  return res;
}

static void object_alloc_init(void) {
  root_object_bucket = cur_object_bucket = new_object_bucket();
}

static ScanData *alloc_object_data(void) {
  ScanData *res;
retry:
...
static void free_object_buckets(void) {
...
// ColorData buckets
#define NUM_COLOR_ENTRIES                                                      \
  ((BUCKET_SIZE - SIZEOF_VOID_P * 2) / sizeof(ColorData))
...
static ColorBucket *new_color_bucket(void) {
  ColorBucket *res =
      (ColorBucket *)sgen_alloc_internal(INTERNAL_MEM_TARJAN_OBJ_BUCKET);
  res->next_data = &res->data[0];
  return res;
}

static void color_alloc_init(void) {
  root_color_bucket = cur_color_bucket = new_color_bucket();
}

static ColorData *alloc_color_data(void) {
  ColorData *res;
retry:
...
static void free_color_buckets(void) {
  ColorBucket *cur, *tmp;
...
static ScanData *create_data(GCObject *obj) {
  mword *o = (mword *)obj;
...
static ScanData *find_data(GCObject *obj) {
  ScanData *a = NULL;
  mword *o = (mword *)obj;
...
static void clear_after_processing(void) {
  ObjectBucket *cur;
...
static GCObject *bridge_object_forward(GCObject *obj) {
  GCObject *fwd;
  mword *o = (mword *)obj;
  if ((o[0] & SGEN_VTABLE_BITS_MASK) == SGEN_VTABLE_BITS_MASK)
    return obj;
...
#ifdef DUMP_GRAPH
static const char *safe_name_bridge(GCObject *obj) {
  GCVTable vt = SGEN_LOAD_VTABLE(obj);
  return vt->klass->name;
}

static ScanData *find_or_create_data(GCObject *obj) {
...
/*
The merge cache maps an ordered list of ColorDatas [the color_merge_array] to a
single ColorData.

About cache bucket tuning: We tried 2/32, 2/128, 4/32, 4/128, 6/128 and 8/128.

The performance cost between 4/128 and 8/128 is so small since cache movement
happens completely in the same cacheline, making the extra space pretty much
free.

The cost between 32 and 128 itens is minimal too, it's mostly a fixed, setup
cost.
...
// If true, disable an optimization where sometimes SCC nodes are merged without
// a perfect check
...
static unsigned int mix_hash(uintptr_t source) {
  ...
  // The full hash determines whether two colors can be merged-- sometimes
  // exclusively. This value changes every GC, so XORing it in before performing
  // the hash will make the chance that two different colors will produce the
  // same hash on successive GCs very low.
...
static void reset_cache(void) {
  ...
  // When using the precise merge debug option, we do not want the inconsistency
  // caused by hash_perturb.
  if (!scc_precise_merge)
    ++hash_perturb;
}

static gboolean dyn_array_ptr_contains(DynPtrArray *da, void *x) {
  int i;
...
static gboolean match_colors_estimate(DynPtrArray *a, DynPtrArray *b) {
  return dyn_array_ptr_size(a) == dyn_array_ptr_size(b);
}

static gboolean match_colors(DynPtrArray *a, DynPtrArray *b) {
  int i;
...
// If scc_precise_merge, "semihits" refers to find_in_cache calls aborted
// because the merge array was too large. Otherwise "semihits" refers to cache
// hits where the match was only estimated.
...
static ColorData *find_in_cache(int *insert_index) {
  ...
  /* Color equality checking is very expensive with a lot of elements, so when
   * there are many elements we switch to a cheap comparison method which allows
   * false positives. When false positives occur the worst that can happen is
   * two items will be inappropriately merged and memory will be retained longer
   * than it should be. We assume that will correct itself on the next GC (the
   * hash_perturb mechanism increases the probability of this).
   *
   * Because this has *some* potential to create problems, if the user set the
   * debug option 'enable-tarjan-precise-merge' we bail out early (and never
   * merge SCCs with >3 colors).
   */
  ...
  for (i = 0; i < ELEMENTS_PER_BUCKET; ++i) {
    ...
    if (color_merge_array_large) {
      if (match_colors_estimate(&bucket[i].color->other_colors,
                                &color_merge_array)) {
        ++cache_semihits;
...
// Populate other_colors for a give color (other_colors represent the xrefs for
// this color)
static void add_other_colors(ColorData *color, DynPtrArray *other_colors) {
  for (int i = 0; i < dyn_array_ptr_size(other_colors); ++i) {
    ...
    points_to->incoming_colors =
        MIN(points_to->incoming_colors + 1, INCOMING_COLORS_MAX);
  }
}

// A color is needed for an SCC. If the SCC has bridges, the color MUST be newly
// allocated. If the SCC lacks bridges, the allocator MAY use the cache to merge
// it with an existing one.
static ColorData *new_color(gboolean has_bridges) {
  int cacheSlot = -1;
  ColorData *cd;
  ...
  /* if cacheSlot >= 0, it means we prepared a given slot to receive the new
   * color */
  if (cacheSlot >= 0)
...
static void register_bridge_object(GCObject *obj) {
  create_data(obj)->is_bridge = TRUE;
}

static gboolean is_opaque_object(GCObject *obj) {
  MonoVTable *vt = SGEN_LOAD_VTABLE(obj);
  if ((vt->gc_bits & SGEN_GC_BIT_BRIDGE_OPAQUE_OBJECT) ==
...
// Called during DFS; visits one child. If it is a candidate to be scanned,
// pushes it to the stacks.
static void push_object(GCObject *obj) {
  ScanData *data;
  ...
  /* Already marked - XXX must be done this way as the bridge themselves are
   * alive. */
...
#if DUMP_GRAPH
  printf("+scanning %s (%p) index %d color %p\n", safe_name_bridge(data->obj),
         data->obj, data->index, data->color);
#endif

#include "sgen/sgen-scan-object.h"
}

static void compute_low_index(ScanData *data, GCObject *obj) {
  ScanData *other;
  ColorData *cd;
  ...
  printf("\tcompute low %p ->%p (%s) %p (%d / %d, color %p)\n", data->obj, obj,
         safe_name_bridge(obj), other, other ? other->index : -2,
         other ? other->low_index : -2, other->color);
#endif
  if (!other)
    return;
  ...
  if ((other->state == SCANNED || other->state == FINISHED_ON_STACK) &&
  ...
  // The scc for the referenced object was already created, meaning this is an
  // xref. Add it to the color merge array so we can handle it later when
  // creating the scc for the current object (data)
  if (!cd->visited) {
...
static ColorData *reduce_color(void) {
...
static void create_scc(ScanData *data) {
  ...
  while (dyn_array_ptr_size(&loop_stack) > 0) {
    ...
    printf("\tmember %s (%p) index %d low-index %d color %p state %d\n",
           safe_name_bridge(other->obj), other->obj, other->index,
           other->low_index, other->color, other->state);
#endif
    ...
    // Maybe we should make sure we are not adding duplicates here. It is not
    // really a problem since we will get rid of duplicates before submitting
    // the SCCs to the client in gather_xrefs
    if (color_data)
...
static void dfs(void) {
  ...
  while (dyn_array_ptr_size(&scan_stack) > 0) {
     ...
     * C -> A
     *
     * We start scanning from A and push C before B. So, after the first
     * iteration, the scan stack will have: A C B. We then visit B, which will
     * find C in its initial state and push again. Finally after finish with C
     * and B, the stack will be left with "A C" and at this point C should be
     * ignored.
     *
     * The above explains FINISHED_ON_STACK, to explain FINISHED_OFF_STACK,
     * consider if the root was D, which pointed to A and C. A is processed
     * first, leaving C on stack after that in the mentioned state.
     */
    ...
    if (data->state == INITIAL) {
      ...
      printf("-finishing %s (%p) index %d low-index %d color %p\n",
             safe_name_bridge(data->obj), data->obj, data->index,
             data->low_index, data->color);
#endif
      ...
      if (data->index == data->low_index) {
        ...
        // We need to clear colo_merge_array from all xrefs. We flush them to
        // the current color and will add them to the scc when we reach the root
        // of the scc.
        g_assert(dyn_array_ptr_size(&data->xrefs) == 0);
        dyn_array_ptr_set_all(&data->xrefs, &color_merge_array);
      }
      // We populated color_merge_array while scanning the object with each
      // neighbor color. Clear it now
...
static void register_finalized_object(GCObject *obj) {
  g_assert(sgen_need_bridge_processing());
  dyn_array_ptr_push(&registered_bridges, obj);
}

static void reset_data(void) { dyn_array_ptr_empty(&registered_bridges); }
...
#ifdef DUMP_GRAPH
static void dump_color_table(const char *why, gboolean do_index) {
  ColorBucket *cur;
  int i = 0, j;
  printf("colors%s:\n", why);
...
static gint64 step_timer(gint64 *timer) {
  gint64 curtime, diff;
...
static void processing_stw_step(void) {
  int i;
  int bridge_count;
  gint64 curtime;
  ...
  for (i = 0; i < bridge_count; ++i)
    register_bridge_object(
        (GCObject *)dyn_array_ptr_get(&registered_bridges, i));

  setup_time = step_timer(&curtime);

  for (i = 0; i < bridge_count; ++i) {
    ScanData *sd =
        find_data((GCObject *)dyn_array_ptr_get(&registered_bridges, i));
    if (sd->state == INITIAL) {
  ...
  for (int i = 0; i < bridge_count; ++i) {
    ...
    printf("\t%s (%p) index %d color %p\n", safe_name_bridge(sd->obj), sd->obj,
           sd->index, sd->color);
  }
...
static void processing_build_callback_data(int generation) {
  int j;
  gint64 curtime;
  ColorBucket *cur;
  ...
  /* This is a straightforward translation from colors to the bridge callback
   * format. */
  MonoGCBridgeSCC **api_sccs = (MonoGCBridgeSCC **)sgen_alloc_internal_dynamic(
      sizeof(MonoGCBridgeSCC *) * num_sccs, INTERNAL_MEM_BRIDGE_DATA, TRUE);
  int api_index = 0;
  xref_count = 0;

  // Convert visible SCCs, along with their bridged object list, to
  // MonoGCBridgeSCCs in the client's SCC list
  for (cur = root_color_bucket; cur; cur = cur->next) {
    ColorData *cd;
    for (cd = &cur->data[0]; cd < cur->next_data; ++cd) {
      ...
      api_sccs[api_index] = (MonoGCBridgeSCC *)sgen_alloc_internal_dynamic(
          sizeof(MonoGCBridgeSCC) + sizeof(MonoObject *) * bridges,
          INTERNAL_MEM_BRIDGE_DATA, TRUE);
      ...
      for (j = 0; j < bridges; ++j)
        api_sccs[api_index]->objs[j] =
            (MonoObject *)dyn_array_ptr_get(&cd->bridges, j);
  ...
  MonoGCBridgeXRef *api_xrefs = (MonoGCBridgeXRef *)sgen_alloc_internal_dynamic(
      sizeof(MonoGCBridgeXRef) * xref_count, INTERNAL_MEM_BRIDGE_DATA, TRUE);
  int xref_index = 0;
  for (cur = root_color_bucket; cur; cur = cur->next) {
    ColorData *src;
    for (src = &cur->data[0]; src < cur->next_data; ++src) {
      ...
      for (j = 0; j < dyn_array_ptr_size(&src->other_colors); ++j) {
        ...
        g_assert(color_visible_to_client(
            dest)); /* Supposedly we already eliminated all xrefs to non-visible
                       objects. */
  ...
  g_assertf(xref_count == xref_index, "xref_count is %d but we added %d xrefs",
            xref_count, xref_index);
  xref_setup_time = step_timer(&curtime);

#if defined(DUMP_GRAPH)
  printf("---xrefs:\n");
  for (int i = 0; i < xref_count; ++i)
    printf("\t%d -> %d\n", api_xrefs[i].src_scc_index,
           api_xrefs[i].dst_scc_index);
#endif
...
static void processing_after_callback(int generation) {
  gint64 curtime;
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
             "GC_TAR_BRIDGE bridges %d objects %d opaque %d colors %d "
             "colors-bridged %d colors-visible %d xref %d cache-hit %d "
             "cache-%s %d cache-miss %d setup %.2fms tarjan %.2fms scc-setup "
             "%.2fms gather-xref %.2fms xref-setup %.2fms cleanup %.2fms",
             bridge_count, object_count, ignored_objects, color_count,
             colors_with_bridges_count, num_sccs, xref_count, cache_hits,
             (scc_precise_merge ? "abstain" : "semihit"), cache_semihits,
             cache_misses, setup_time / 10000.0f, tarjan_time / 10000.0f,
             scc_setup_time / 10000.0f, gather_xref_time / 10000.0f,
             xref_setup_time / 10000.0f, cleanup_time / 10000.0f);

  cache_hits = cache_semihits = cache_misses = 0;
  ignored_objects = 0;
}

static void describe_pointer(GCObject *obj) {
  // HashEntry *entry;
  int i;
...
static void set_config(const SgenBridgeProcessorConfig *config) {
...
void sgen_tarjan_bridge_init(SgenBridgeProcessor *collector) {
  ...
  sgen_register_fixed_internal_mem_type(INTERNAL_MEM_TARJAN_OBJ_BUCKET,
                                        BUCKET_SIZE);
