 ...
 * FIXME: Move SRE class construction to class-init.c and unify with ordinary
 * class construction.
 */
...
static GENERATE_GET_CLASS_WITH_CACHE(marshal_as_attribute,
                                     "System.Runtime.InteropServices",
                                     "MarshalAsAttribute");
#ifndef DISABLE_REFLECTION_EMIT
static GENERATE_GET_CLASS_WITH_CACHE(module_builder, "System.Reflection.Emit",
                                     "ModuleBuilder");
#endif

static char *string_to_utf8_image_raw(MonoImage *image, MonoString *s,
                                      MonoError *error);

#ifndef DISABLE_REFLECTION_EMIT
static guint32
mono_image_get_sighelper_token(MonoDynamicImage *assembly,
...
reflection_init_generic_class(MonoReflectionTypeBuilderHandle tb,
                              MonoError *error);
static gboolean reflection_setup_class_hierarchy(GHashTable *unparented,
                                                 MonoError *error);

static gpointer register_assembly(MonoDomain *domain,
...
static guint32 mono_image_get_methodspec_token(MonoDynamicImage *assembly,
                                               MonoMethod *method);
static guint32 mono_image_get_inflated_method_token(MonoDynamicImage *assembly,
                                                    MonoMethod *m);
static guint32 mono_image_create_method_token(MonoDynamicImage *assembly,
                                              MonoObjectHandle obj,
                                              MonoArrayHandle opt_param_types,
                                              MonoError *error);

#ifndef DISABLE_REFLECTION_EMIT
static MonoType *mono_type_array_get_and_resolve_raw(MonoArray *array, int idx,
                                                     MonoError *error);
#endif
...
void mono_reflection_emit_init(void) { mono_dynamic_images_init(); }

char *string_to_utf8_image_raw(MonoImage *image, MonoString *s_raw,
                               MonoError *error) {
...
static char *type_get_fully_qualified_name(MonoType *type) {
  MONO_REQ_GC_NEUTRAL_MODE;

  return mono_type_get_name_full(type,
 ...
 * mp_g_alloc:
 *
 * Allocate memory from the @image mempool if it is non-NULL. Otherwise,
 * allocate memory from the C heap.
 */
static gpointer image_g_malloc(MonoImage *image, guint size) {
  MONO_REQ_GC_NEUTRAL_MODE;
 ...
 * allocate memory from the C heap.
 */
gpointer(mono_image_g_malloc0)(MonoImage *image, guint size) {
  MONO_REQ_GC_NEUTRAL_MODE;
 ...
 */
static void image_g_free(MonoImage *image, gpointer ptr) {
  if (image == NULL)
    g_free(ptr);
}

#ifndef DISABLE_REFLECTION_EMIT
static char *image_strdup(MonoImage *image, const char *s) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
#define image_g_new0(image, struct_type, n_structs)                            \
  ((struct_type *)mono_image_g_malloc0(image, ((gsize)sizeof(struct_type)) *   \
                                                  ((gsize)(n_structs))))

static void alloc_table(MonoDynamicTable *table, guint nrows) {
  mono_dynimage_alloc_table(table, nrows);
}

static guint32 string_heap_insert(MonoDynamicStream *sh, const char *str) {
  return mono_dynstream_insert_string(sh, str);
}

static guint32 mono_image_add_stream_data(MonoDynamicStream *stream,
 ...
 */
static guint32 mono_image_typedef_or_ref(MonoDynamicImage *assembly,
                                         MonoType *type) {
 ...
 */
static void swap_with_size(gpointer void_dest, gconstpointer void_val, int len,
                           int nelem) {
...
guint32 mono_reflection_method_count_clauses(MonoReflectionILGen *ilgen) {
  ...
  for (i = 0; i < mono_array_length_internal(ilgen->ex_handlers); ++i) {
    ex_info = (MonoILExceptionInfo *)mono_array_addr_internal(
...
#ifndef DISABLE_REFLECTION_EMIT
static MonoExceptionClause *method_encode_clauses(MonoImage *image,
  ...
  for (i = mono_array_length_internal(ilgen->ex_handlers) - 1; i >= 0; --i) {
    ...
    for (j = 0; j < mono_array_length_internal(ex_info->handlers); ++j) {
      ex_block = (MonoILExceptionBlock *)mono_array_addr_internal(
          ex_info->handlers, MonoILExceptionBlock, j);
      ...
      if (ex_block->extype) {
        MonoType *extype = mono_reflection_type_get_handle(
            (MonoReflectionType *)ex_block->extype, error);
 ...
 */
static void mono_save_custom_attrs(MonoImage *image, void *obj,
                                   MonoArray *cattrs) {
  MONO_REQ_GC_UNSAFE_MODE;
...
#else
// FIXME some code compiled under DISABLE_REFLECTION_EMIT depends on this
// function, we should be more aggressively disabling things
static void mono_save_custom_attrs(MonoImage *image, void *obj,
                                   MonoArray *cattrs) {}
#endif

guint32 mono_reflection_resolution_scope_from_image(MonoDynamicImage *assembly,
                                                    MonoImage *image) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if ((token =
           GPOINTER_TO_UINT(g_hash_table_lookup(assembly->handleref, image))))
    return token;
  ...
  if (assembly_is_dynamic(image->assembly) &&
    ...
    values[MONO_MODULEREF_NAME] =
        string_heap_insert(&assembly->sheap, image->module_name);
  ...
  if (assembly_is_dynamic(image->assembly))
    ...
    mono_metadata_decode_row(&image->tables[MONO_TABLE_ASSEMBLY], 0, cols,
                             MONO_ASSEMBLY_SIZE);
  }
  ...
  values[MONO_ASSEMBLYREF_NAME] =
      string_heap_insert(&assembly->sheap, image->assembly_name);
  ...
  if (strcmp("", image->assembly->aname.culture)) {
    values[MONO_ASSEMBLYREF_CULTURE] =
        string_heap_insert(&assembly->sheap, image->assembly->aname.culture);
  }
  ...
  if ((pubkey = mono_image_get_public_key(image, &publen))) {
    ...
    values[MONO_ASSEMBLYREF_PUBLIC_KEY] =
        mono_image_add_stream_data(&assembly->blob, (char *)pubtoken, 9);
  } else {
...
#ifndef DISABLE_REFLECTION_EMIT
gboolean mono_reflection_methodbuilder_from_method_builder(
...
gboolean
mono_reflection_methodbuilder_from_ctor_builder(ReflectionMethodBuilder *rmb,
  ...
  rmb->ilgen = mb->ilgen;
  rmb->rtype = mono_type_get_object_checked(mono_domain_get(),
                                            mono_get_void_type(), error);
  return_val_if_nok(error, FALSE);
  rmb->parameters = mb->parameters;
...
#ifndef DISABLE_REFLECTION_EMIT
static guint32 mono_image_add_memberef_row(MonoDynamicImage *assembly,
                                           guint32 parent, const char *name,
                                           guint32 sig) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  if (assembly->save) {
    ...
    values[MONO_MEMBERREF_CLASS] =
        pclass | (parent << MONO_MEMBERREF_PARENT_BITS);
 ...
 * memberref is returned. Caching is done in the caller
 * (mono_image_get_methodref_token() or mono_image_get_fieldref_token()). The
 * sig param is an index to an already built signature.
 */
static guint32 mono_image_get_memberref_token(MonoDynamicImage *assembly,
                                              MonoType *type, const char *name,
                                              guint32 sig) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
guint32 mono_image_get_methodref_token(MonoDynamicImage *assembly,
  ...
  create_typespec = create_typespec && method->is_generic &&
                    method->klass->image != &assembly->image;

  if (create_typespec) {
    token = GPOINTER_TO_UINT(g_hash_table_lookup(
        assembly->handleref, GUINT_TO_POINTER(GPOINTER_TO_UINT(method) + 1)));
    if (token)
      return token;
  }
  ...
  if (!token) {
    ...
    token = mono_image_get_memberref_token(
        assembly, m_class_get_byval_arg(method->klass), method->name,
  ...
  if (create_typespec) {
    ...
    token = (mono_metadata_token_index(token) << MONO_METHODDEFORREF_BITS) |
    ...
    if (assembly->save) {
      ...
      values[MONO_METHODSPEC_SIGNATURE] =
          mono_dynimage_encode_generic_method_sig(
              assembly, &mono_method_get_generic_container(method)->context);
    }

    token = MONO_TOKEN_METHOD_SPEC | table->next_idx;
    table->next_idx++;
    /*methodspec and memberef tokens are diferent, */
    g_hash_table_insert(assembly->handleref,
                        GUINT_TO_POINTER(GPOINTER_TO_UINT(method) + 1),
...
static guint32 mono_image_get_varargs_method_token(MonoDynamicImage *assembly,
                                                   guint32 original,
                                                   const gchar *name,
                                                   guint32 sig) {
  MonoDynamicTable *table;
...
static gboolean is_field_on_inst(MonoClassField *field) {
  return mono_class_is_ginst(field->parent) &&
         mono_class_get_generic_class(field->parent)->is_dynamic;
}

static gboolean is_field_on_gtd(MonoClassField *field) {
  return mono_class_is_gtd(field->parent);
}

#ifndef DISABLE_REFLECTION_EMIT
static guint32 mono_image_get_fieldref_token(MonoDynamicImage *assembly,
  ...
  if (mono_class_is_ginst(field->parent) &&
      ...
      mono_class_get_generic_class(field->parent)->container_class->fields) {
    int index = field - field->parent->fields;
    type = mono_field_get_type_internal(
        &mono_class_get_generic_class(field->parent)
             ->container_class->fields[index]);
  } else {
    type = mono_field_get_type_internal(field);
  }
  token = mono_image_get_memberref_token(
      assembly, m_class_get_byval_arg(field->parent),
      mono_field_get_name(field),
      mono_dynimage_encode_fieldref_signature(assembly, field->parent->image,
                                              type));
  g_hash_table_insert(assembly->handleref, field, GUINT_TO_POINTER(token));
  return token;
}

static guint32 method_encode_methodspec(MonoDynamicImage *assembly,
                                        MonoMethod *method) {
  MonoDynamicTable *table;
  guint32 *values;
  ...
  sig = mono_dynimage_encode_method_signature(
      assembly, mono_method_signature_internal(declaring));
  mtoken = mono_image_get_memberref_token(
      assembly, m_class_get_byval_arg(method->klass), declaring->name, sig);
  ...
  case MONO_TABLE_MEMBERREF:
    mtoken = (mono_metadata_token_index(mtoken) << MONO_METHODDEFORREF_BITS) |
             MONO_METHODDEFORREF_METHODREF;
    break;
  ...
  sig = mono_dynimage_encode_generic_method_sig(
      assembly, mono_method_get_context(method));
...
static int reflection_cc_to_file(int call_conv) {
  switch (call_conv & 0x3) {
  case 0:
  case 1:
    return MONO_CALL_DEFAULT;
  case 2:
    return MONO_CALL_VARARG;
  default:
    g_assert_not_reached();
  }
  return 0;
...
void mono_sre_array_method_free(ArrayMethod *am) {
  g_free(am->sig);
  g_free(am->name);
  g_free(am);
}
...
#ifndef DISABLE_REFLECTION_EMIT
static guint32 mono_image_get_array_token(MonoDynamicImage *assembly,
  ...
  sig = (MonoMethodSignature *)g_malloc0(MONO_SIZEOF_METHOD_SIGNATURE +
                                         sizeof(MonoType *) * nparams);
  sig->hasthis = 1;
  sig->sentinelpos = -1;
  sig->call_convention =
      reflection_cc_to_file(MONO_HANDLE_GETVAL(m, call_conv));
  sig->param_count = nparams;
  MonoReflectionTypeHandle ret =
      MONO_HANDLE_NEW_GET(MonoReflectionType, m, ret);
  ...
  am->sig = sig;
  am->parent = mtype;
  am->token = mono_image_get_memberref_token(
      assembly, am->parent, name,
      mono_dynimage_encode_method_signature(assembly, sig));
 ...
 */
guint32 mono_image_insert_string(MonoReflectionModuleBuilderHandle ref_module,
                                 MonoStringHandle str, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  guint32 idx;
  char buf[16];
  char *b = buf;
  ...
  if (assembly->save) {
    ...
    /* pinned */
    uint32_t gchandle =
        mono_gchandle_from_handle(MONO_HANDLE_CAST(MonoObject, str), TRUE);
    const char *p =
  ...
  mono_dynamic_image_register_token(assembly, token,
                                    MONO_HANDLE_CAST(MonoObject, str),
...
static guint32 create_method_token(MonoDynamicImage *assembly,
                                   MonoMethod *method,
                                   MonoArrayHandle opt_param_types,
                                   MonoError *error) {
  ...
  MonoMethodSignature *sig =
      mono_metadata_signature_alloc(&assembly->image, old->param_count + nargs);
  ...
  for (int i = 0; i < nargs; i++) {
    ...
    sig->params[old->param_count + i] =
        mono_reflection_type_handle_mono_type(rt, error);
    goto_if_nok(error, fail);
  }

  parent =
      mono_image_typedef_or_ref(assembly, m_class_get_byval_arg(method->klass));
  ...
  guint32 token;
  token = mono_image_get_varargs_method_token(assembly, parent, method->name,
                                              sig_token);
...
guint32 mono_image_create_method_token(MonoDynamicImage *assembly,
  ...
  if (strcmp(klass->name, "RuntimeMethodInfo") == 0 ||
      strcmp(klass->name, "RuntimeConstructorInfo") == 0) {
    MonoReflectionMethodHandle ref_method =
        MONO_HANDLE_CAST(MonoReflectionMethod, obj);
    MonoMethod *method = MONO_HANDLE_GETVAL(ref_method, method);
    g_assert(!MONO_HANDLE_IS_NULL(opt_param_types) &&
             (mono_method_signature_internal(method)->sentinelpos >= 0));
 ...
 * The metadata emission routines need to pass FALSE as REGISTER_TOKEN, since by
 * that time, the table_idx-es were recomputed, so registering the token would
 * overwrite an existing entry.
 */
guint32 mono_image_create_token(MonoDynamicImage *assembly,
                                MonoObjectHandle obj,
                                gboolean create_open_instance,
                                gboolean register_token, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  guint32 token = 0;
  ...
  /* TypeDelegator is the only corlib type which doesn't look like a
   * MonoReflectionType */
  if (klass->image != mono_defaults.corlib ||
      (strcmp(klass->name, "TypeDelegator") == 0)) {
    mono_error_set_not_supported(
        error, "User defined subclasses of System.Type are not yet supported");
    goto leave;
  }

  /* This function is called from ModuleBuilder:getToken multiple times for the
   * same objects */
  int how_collide;
  how_collide = MONO_DYN_IMAGE_TOK_SAME_OK;

  if (strcmp(klass->name, "RuntimeType") == 0) {
    MonoType *type = mono_reflection_type_handle_mono_type(
        MONO_HANDLE_CAST(MonoReflectionType, obj), error);
    goto_if_nok(error, leave);
    MonoClass *mc = mono_class_from_mono_type_internal(type);
    token =
        mono_metadata_token_from_dor(mono_dynimage_encode_typedef_or_ref_full(
            assembly, type, !mono_class_is_gtd(mc) || create_open_instance));
    ...
    if (method->is_inflated) {
      if (create_open_instance) {
        guint32 methodspec_token =
            mono_image_get_methodspec_token(assembly, method);
        MonoReflectionMethodHandle canonical_obj =
            mono_method_get_object_handle(MONO_HANDLE_DOMAIN(obj), method, NULL,
                                          error);
        goto_if_nok(error, leave);
      ...
      guint32 methodref_token = mono_image_get_methodref_token(
          assembly, method, create_open_instance);
       ...
       * of the method) */
      MonoReflectionMethodHandle canonical_obj = mono_method_get_object_handle(
          MONO_HANDLE_DOMAIN(obj), method, NULL, error);
      goto_if_nok(error, leave);
      MONO_HANDLE_ASSIGN(register_obj, canonical_obj);
    ...
    if ((field->parent->image == &assembly->image) && !is_field_on_gtd(field) &&
      ...
      MonoReflectionFieldHandle canonical_obj = mono_field_get_object_handle(
          MONO_HANDLE_DOMAIN(obj), field->parent, field, error);
      goto_if_nok(error, leave);
      MONO_HANDLE_ASSIGN(register_obj, canonical_obj);
    ...
    MonoReflectionArrayMethodHandle m =
        MONO_HANDLE_CAST(MonoReflectionArrayMethod, obj);
    ...
    MonoReflectionSigHelperHandle s =
        MONO_HANDLE_CAST(MonoReflectionSigHelper, obj);
    /* always returns a fresh token */
    guint32 sig_token = MONO_TOKEN_SIGNATURE |
                        mono_image_get_sighelper_token(assembly, s, error);
    goto_if_nok(error, leave);
    token = sig_token;
    how_collide = MONO_DYN_IMAGE_TOK_NEW;
  } else {
  ...
  if (register_token)
    mono_dynamic_image_register_token(assembly, token, register_obj,
                                      how_collide);
...
static gboolean assemblybuilderaccess_can_refonlyload(guint32 access) {
  return (access & 0x4) != 0;
}

static gboolean assemblybuilderaccess_can_run(guint32 access) {
  return (access & MonoAssemblyBuilderAccess_Run) != 0;
}

static gboolean assemblybuilderaccess_can_save(guint32 access) {
 ...
 */
void mono_reflection_dynimage_basic_init(
    MonoReflectionAssemblyBuilder *assemblyb, MonoError *error) {
  ...
  assembly->assembly.basedir =
      mono_string_to_utf8_checked_internal(assemblyb->dir, error);
  return_if_nok(error);
  if (assemblyb->culture) {
    assembly->assembly.aname.culture =
        mono_string_to_utf8_checked_internal(assemblyb->culture, error);
    return_if_nok(error);
  } else
  ...
  if (assemblyb->version) {
    char *vstr =
        mono_string_to_utf8_checked_internal(assemblyb->version, error);
  ...
  assembly->assembly.context.kind =
      assemblybuilderaccess_can_refonlyload(assemblyb->access)
          ? MONO_ASMCTX_REFONLY
  ...
  char *assembly_name =
      mono_string_to_utf8_checked_internal(assemblyb->name, error);
  return_if_nok(error);
  image = mono_dynamic_image_create(
      assembly, assembly_name, g_strdup("RefEmit_YouForgotToDefineAModule"));
  ...
  if (assemblyb->pktoken && assemblyb->pktoken->max_length) {
    ...
    if (assemblyb->pktoken->max_length != MONO_PUBLIC_KEY_TOKEN_LENGTH - 1) {
      g_error("Public key token length invalid for assembly %s: %i",
              assembly->assembly.aname.name, assemblyb->pktoken->max_length);
    }
    memcpy(&assembly->assembly.aname.public_key_token,
           mono_array_addr_internal(assemblyb->pktoken, guint8, 0),
           assemblyb->pktoken->max_length);
  }

  mono_domain_assemblies_lock(domain);
  domain->domain_assemblies =
      g_slist_append(domain->domain_assemblies, assembly);
  ...
  register_assembly(mono_object_domain(assemblyb), &assemblyb->assembly,
                    &assembly->assembly);
...
register_module(MonoDomain *domain, MonoReflectionModuleBuilderHandle res,
                MonoDynamicImage *module) {
  return CACHE_OBJECT_HANDLE(MonoReflectionModuleBuilder, module,
                             MONO_HANDLE_CAST(MonoObject, res), NULL);
}

static gboolean
image_module_basic_init(MonoReflectionModuleBuilderHandle moduleb,
                        MonoError *error) {
  error_init(error);
  MonoDomain *domain = MONO_HANDLE_DOMAIN(moduleb);
  MonoDynamicImage *image = MONO_HANDLE_GETVAL(moduleb, dynamic_image);
  MonoReflectionAssemblyBuilderHandle ab =
      MONO_HANDLE_NEW(MonoReflectionAssemblyBuilder, NULL);
  MONO_HANDLE_GET(ab, moduleb, assemblyb);
  if (!image) {
    /*
     * FIXME: we already created an image in mono_reflection_dynimage_basic_init
     * (), but we don't know which module it belongs to, since that is only
    ...
    return_val_if_nok(error, FALSE);
    MonoStringHandle modfqname = MONO_HANDLE_NEW_GET(
        MonoString, MONO_HANDLE_CAST(MonoReflectionModule, moduleb), fqname);
    ...
    MonoDynamicAssembly *dynamic_assembly =
        MONO_HANDLE_GETVAL(ab, dynamic_assembly);
    image = mono_dynamic_image_create(dynamic_assembly, name, fqname);

    MONO_HANDLE_SETVAL(MONO_HANDLE_CAST(MonoReflectionModule, moduleb), image,
                       MonoImage *, &image->image);
...
static gboolean
mono_image_module_basic_init(MonoReflectionModuleBuilderHandle moduleb,
                             MonoError *error) {
  error_init(error);
...
static gboolean is_corlib_type(MonoClass *klass) {
  return klass->image == mono_defaults.corlib;
}

#define check_corlib_type_cached(_class, _namespace, _name)                    \
  do {                                                                         \
    static MonoClass *cached_class;                                            \
    if (cached_class)                                                          \
      return cached_class == _class;                                           \
    if (is_corlib_type(_class) && !strcmp(_name, _class->name) &&              \
        !strcmp(_namespace, _class->name_space)) {                             \
      cached_class = _class;                                                   \
      return TRUE;                                                             \
    }                                                                          \
    return FALSE;                                                              \
...
MonoType *mono_type_array_get_and_resolve(MonoArrayHandle array, int idx,
                                          MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
...
static MonoType *add_custom_modifiers_to_type(MonoType *without_mods,
  ...
  result = mono_image_g_malloc0(image,
                                mono_sizeof_type_with_mods(total_mods, FALSE));
  ...
  for (int i = 0; i < num_req_mods; i++) {
    ...
    cmods->modifiers[modifier_index].token =
        mono_image_create_token(allocator, mod_handle, FALSE, TRUE, error);
    modifier_index--;
  }
...
static MonoType *mono_type_array_get_and_resolve_with_modifiers(
    MonoArrayHandle types, MonoArrayHandle required_modifiers,
    MonoArrayHandle optional_modifiers, int idx, MonoImage *image,
    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  ...
  result = (MonoType *)add_custom_modifiers_to_type(
      result, req_mods_handle, opt_mods_handle, image, error);
...
static gboolean is_sre_generic_instance(MonoClass *klass) {
  check_corlib_type_cached(klass, "System.Reflection.Emit",
...
gboolean mono_is_sre_ctor_builder(MonoClass *klass) {
  check_corlib_type_cached(klass, "System.Reflection.Emit",
                           "ConstructorBuilder");
}
...
gboolean mono_is_sre_method_on_tb_inst(MonoClass *klass) {
  check_corlib_type_cached(klass, "System.Reflection.Emit",
                           "MethodOnTypeBuilderInst");
}

gboolean mono_is_sre_ctor_on_tb_inst(MonoClass *klass) {
  check_corlib_type_cached(klass, "System.Reflection.Emit",
...
mono_reflection_type_get_underlying_system_type(MonoReflectionTypeHandle t,
  ...
  method_get_underlying_system_type = mono_class_get_method_from_name_checked(
      mono_defaults.systemtype_class, "get_UnderlyingSystemType", 0, 0, error);
  ...
  MonoMethod *usertype_method = mono_object_handle_get_virtual_method(
      MONO_HANDLE_CAST(MonoObject, t), method_get_underlying_system_type,
      error);
  goto_if_nok(error, leave);
  ...
  MONO_HANDLE_ASSIGN(
      ...
      MONO_HANDLE_CAST(
          MonoReflectionType,
          MONO_HANDLE_NEW(MonoObject, mono_runtime_invoke_checked(
                                          usertype_method, MONO_HANDLE_RAW(t),
                                          NULL, error))));
...
MonoType *mono_reflection_type_get_handle(MonoReflectionType *ref_raw,
                                          MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
...
static MonoType *reflection_instance_handle_mono_type(
    MonoReflectionGenericClassHandle ref_gclass, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoType *result = NULL;
  MonoType **types = NULL;

  MonoArrayHandle typeargs =
      MONO_HANDLE_NEW_GET(MonoArray, ref_gclass, type_arguments);
  ...
  /* Need to resolve the generic_type in order for it to create its generic
   * context. */
  ...
  if (is_sre_type_builder(mono_handle_class(ref_gtd))) {
    reflection_setup_internal_class(
        MONO_HANDLE_CAST(MonoReflectionTypeBuilder, ref_gtd), error);
    goto_if_nok(error, leave);
  }
  g_assert(count == 0 || mono_class_is_gtd(gtd_klass));
  result =
      mono_reflection_bind_generic_parameters(ref_gtd, count, types, error);
  goto_if_nok(error, leave);
  g_assert(result);
  MONO_HANDLE_SETVAL(MONO_HANDLE_CAST(MonoReflectionType, ref_gclass), type,
                     MonoType *, result);
leave:
  g_free(types);
  HANDLE_FUNCTION_RETURN_VAL(result);
...
static MonoType *
reflection_param_handle_mono_type(MonoReflectionGenericParamHandle ref_gparam,
                                  MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoType *result = NULL;

  MonoReflectionTypeBuilderHandle ref_tbuilder =
      MONO_HANDLE_NEW_GET(MonoReflectionTypeBuilder, ref_gparam, tbuilder);
  MonoReflectionModuleBuilderHandle ref_module =
      MONO_HANDLE_NEW_GET(MonoReflectionModuleBuilder, ref_tbuilder, module);
  ...
  MonoReflectionMethodBuilderHandle ref_mbuilder =
      MONO_HANDLE_NEW_GET(MonoReflectionMethodBuilder, ref_gparam, mbuilder);
  if (!MONO_HANDLE_IS_NULL(ref_mbuilder)) {
    MonoGenericContainer *generic_container =
        MONO_HANDLE_GETVAL(ref_mbuilder, generic_container);
    if (!generic_container) {
      ...
      MONO_HANDLE_SETVAL(ref_mbuilder, generic_container,
                         MonoGenericContainer *, generic_container);
    }
    param->owner = generic_container;
  } else {
    MonoType *type = mono_reflection_type_handle_mono_type(
        MONO_HANDLE_CAST(MonoReflectionType, ref_tbuilder), error);
    goto_if_nok(error, leave);
    MonoClass *owner;
  ...
  MONO_HANDLE_SETVAL(MONO_HANDLE_CAST(MonoReflectionType, ref_gparam), type,
                     MonoType *, result);
...
static MonoType *mono_type_array_get_and_resolve_raw(MonoArray *array_raw,
                                                     int idx,
                                                     MonoError *error) {
  HANDLE_FUNCTION_ENTER(); /* FIXME callers of
                              mono_type_array_get_and_resolve_raw should use
                              handles */
  error_init(error);
...
MonoType *mono_reflection_type_handle_mono_type(MonoReflectionTypeHandle ref,
                                                MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  ...
  if (mono_reflection_is_usertype(ref)) {
    ...
    if (!is_ok(error) || MONO_HANDLE_IS_NULL(ref) ||
  ...
  if (is_sre_array(klass)) {
    MonoReflectionArrayTypeHandle sre_array =
        MONO_HANDLE_CAST(MonoReflectionArrayType, ref);
    MonoReflectionTypeHandle ref_element =
        MONO_HANDLE_NEW_GET(MonoReflectionType, sre_array, element_type);
    ...
    if (rank == 0) {
      ...
      MonoArrayType *at = (MonoArrayType *)mono_image_alloc0(
          eclass->image, sizeof(MonoArrayType));
    ...
    MonoReflectionDerivedTypeHandle sre_byref =
        MONO_HANDLE_CAST(MonoReflectionDerivedType, ref);
    MonoReflectionTypeHandle ref_element =
        MONO_HANDLE_NEW_GET(MonoReflectionType, sre_byref, element_type);
    MonoType *base = mono_reflection_type_handle_mono_type(ref_element, error);
    goto_if_nok(error, leave);
    g_assert(base);
    ...
    MonoReflectionDerivedTypeHandle sre_pointer =
        MONO_HANDLE_CAST(MonoReflectionDerivedType, ref);
    MonoReflectionTypeHandle ref_element =
        MONO_HANDLE_NEW_GET(MonoReflectionType, sre_pointer, element_type);
    MonoType *base = mono_reflection_type_handle_mono_type(ref_element, error);
    goto_if_nok(error, leave);
    g_assert(base);
    ...
    result = reflection_instance_handle_mono_type(
        MONO_HANDLE_CAST(MonoReflectionGenericClass, ref), error);
  } else if (is_sre_gparam_builder(klass)) {
    result = reflection_param_handle_mono_type(
        MONO_HANDLE_CAST(MonoReflectionGenericParam, ref), error);
  } else if (is_sre_enum_builder(klass)) {
    MonoReflectionEnumBuilderHandle ref_ebuilder =
        MONO_HANDLE_CAST(MonoReflectionEnumBuilder, ref);

    MonoReflectionTypeHandle ref_tb = MONO_HANDLE_CAST(
        MonoReflectionType,
        MONO_HANDLE_NEW_GET(MonoReflectionTypeBuilder, ref_ebuilder, tb));
    result = mono_reflection_type_handle_mono_type(ref_tb, error);
  } else if (is_sre_type_builder(klass)) {
    MonoReflectionTypeBuilderHandle ref_tb =
        MONO_HANDLE_CAST(MonoReflectionTypeBuilder, ref);

    /* This happens when a finished type references an unfinished one. Have to
     * create the minimal type */
    reflection_setup_internal_class(ref_tb, error);
    mono_error_assert_ok(error);
    result = MONO_HANDLE_GETVAL(ref, type);
  } else {
    g_error("Cannot handle corlib user type %s",
            mono_type_full_name(m_class_get_byval_arg(mono_handle_class(ref))));
  }
leave:
  HANDLE_FUNCTION_RETURN_VAL(result);
}
...
parameters_to_signature(MonoImage *image, MonoArrayHandle parameters,
                        MonoArrayHandle required_modifiers,
                        MonoArrayHandle optional_modifiers, MonoError *error) {
  MonoMethodSignature *sig;
  int count, i;

  error_init(error);

  count = MONO_HANDLE_IS_NULL(parameters)
              ? 0
              : mono_array_handle_length(parameters);

  sig = (MonoMethodSignature *)mono_image_g_malloc0(
      image, MONO_SIZEOF_METHOD_SIGNATURE + sizeof(MonoType *) * count);
  sig->param_count = count;
  sig->sentinelpos = -1; /* FIXME */
  for (i = 0; i < count; ++i) {
    sig->params[i] = mono_type_array_get_and_resolve_with_modifiers(
        parameters, required_modifiers, optional_modifiers, i, image, error);
    if (!is_ok(error)) {
 ...
 * LOCKING: Assumes the loader lock is held.
 */
static MonoMethodSignature *ctor_builder_to_signature(
    MonoImage *image, MonoReflectionCtorBuilderHandle ctor, MonoError *error) {
  MonoMethodSignature *sig;

  error_init(error);
  MonoArrayHandle params = MONO_HANDLE_NEW_GET(MonoArray, ctor, parameters);
  MonoArrayHandle required_modifiers =
      MONO_HANDLE_NEW_GET(MonoArray, ctor, param_modreq);
  MonoArrayHandle optional_modifiers =
      MONO_HANDLE_NEW_GET(MonoArray, ctor, param_modopt);

  sig = parameters_to_signature(image, params, required_modifiers,
                                optional_modifiers, error);
  return_val_if_nok(error, NULL);
  sig->hasthis =
      MONO_HANDLE_GETVAL(ctor, attrs) & METHOD_ATTRIBUTE_STATIC ? 0 : 1;
  sig->ret = mono_get_void_type();
  return sig;
}

static MonoMethodSignature *ctor_builder_to_signature_raw(
    MonoImage *image, MonoReflectionCtorBuilder *ctor_raw, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MONO_HANDLE_DCL(MonoReflectionCtorBuilder, ctor);
  MonoMethodSignature *const sig =
      ctor_builder_to_signature(image, ctor, error);
 ...
 */
static MonoMethodSignature *
method_builder_to_signature(MonoImage *image,
  ...
  MonoArrayHandle required_modifiers =
      MONO_HANDLE_NEW_GET(MonoArray, method, param_modreq);
  MonoArrayHandle optional_modifiers =
      MONO_HANDLE_NEW_GET(MonoArray, method, param_modopt);
  MonoArrayHandle ret_req_modifiers =
      MONO_HANDLE_NEW_GET(MonoArray, method, return_modreq);
  MonoArrayHandle ret_opt_modifiers =
      MONO_HANDLE_NEW_GET(MonoArray, method, return_modopt);

  sig = parameters_to_signature(image, params, required_modifiers,
                                optional_modifiers, error);
  return_val_if_nok(error, NULL);
  sig->hasthis =
      MONO_HANDLE_GETVAL(method, attrs) & METHOD_ATTRIBUTE_STATIC ? 0 : 1;
  MonoReflectionTypeHandle rtype;
  rtype = MONO_HANDLE_CAST(MonoReflectionType,
                           MONO_HANDLE_NEW_GET(MonoObject, method, rtype));
  if (!MONO_HANDLE_IS_NULL(rtype)) {
    sig->ret = mono_reflection_type_handle_mono_type(rtype, error);
    sig->ret = add_custom_modifiers_to_type(sig->ret, ret_req_modifiers,
                                            ret_opt_modifiers, image, error);
    if (!is_ok(error)) {
      image_g_free(image, sig);
      return NULL;
    }
  ...
  MonoArrayHandle generic_params =
      MONO_HANDLE_NEW_GET(MonoArray, method, generic_params);
  sig->generic_param_count = MONO_HANDLE_IS_NULL(generic_params)
                                 ? 0
...
static MonoMethodSignature *
dynamic_method_to_signature(MonoReflectionDynamicMethodHandle method,
                            MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoMethodSignature *sig = NULL;
  ...
  sig = parameters_to_signature(
      NULL, MONO_HANDLE_NEW_GET(MonoArray, method, parameters),
      MONO_HANDLE_CAST(MonoArray, NULL_HANDLE),
      MONO_HANDLE_CAST(MonoArray, NULL_HANDLE), error);
  goto_if_nok(error, leave);
  sig->hasthis =
      MONO_HANDLE_GETVAL(method, attrs) & METHOD_ATTRIBUTE_STATIC ? 0 : 1;
  MonoReflectionTypeHandle rtype;
...
static void get_prop_name_and_type(MonoObject *prop, char **name,
                                   MonoType **type, MonoError *error) {
  error_init(error);
  ...
  if (strcmp(klass->name, "PropertyBuilder") == 0) {
    ...
    *name = mono_string_to_utf8_checked_internal(pb->name, error);
    return_if_nok(error);
    *type =
        mono_reflection_type_get_handle((MonoReflectionType *)pb->type, error);
  } else {
    ...
    *name = g_strdup(p->property->name);
    if (p->property->get)
      ...
      *type = mono_method_signature_internal(p->property->set)
                  ->params[mono_method_signature_internal(p->property->set)
                               ->param_count -
                           1];
  }
}

static void get_field_name_and_type(MonoObject *field, char **name,
                                    MonoType **type, MonoError *error) {
  error_init(error);
  ...
  if (strcmp(klass->name, "FieldBuilder") == 0) {
    ...
    *name = mono_string_to_utf8_checked_internal(fb->name, error);
    return_if_nok(error);
    *type =
        mono_reflection_type_get_handle((MonoReflectionType *)fb->type, error);
  } else {
...
static gboolean is_sre_type_builder(MonoClass *klass) { return FALSE; }

static gboolean is_sre_generic_instance(MonoClass *klass) { return FALSE; }

gboolean mono_is_sre_ctor_builder(MonoClass *klass) { return FALSE; }

gboolean mono_is_sre_method_on_tb_inst(MonoClass *klass) { return FALSE; }

gboolean mono_is_sre_ctor_on_tb_inst(MonoClass *klass) { return FALSE; }

#endif /* !DISABLE_REFLECTION_EMIT */

gboolean mono_is_sr_mono_property(MonoClass *klass) {
...
gboolean mono_is_sr_mono_cmethod(MonoClass *klass) {
  check_corlib_type_cached(klass, "System.Reflection",
                           "RuntimeConstructorInfo");
}

gboolean mono_class_is_reflection_method_or_constructor(MonoClass *klass) {
  return is_sr_mono_method(klass) || mono_is_sr_mono_cmethod(klass);
}

gboolean mono_is_sre_type_builder(MonoClass *klass) {
  return is_sre_type_builder(klass);
}

gboolean mono_is_sre_generic_instance(MonoClass *klass) {
 ...
 * @buflen contains the size of the buffer and is used to return the new buffer
 * size if this needs to be realloced.
 * @retbuffer and @retp return the start and the position of the buffer
 * @error set on error.
 */
static void encode_cattr_value(MonoAssembly *assembly, char *buffer, char *p,
                               char **retbuffer, char **retp, guint32 *buflen,
                               MonoType *type, MonoObject *arg,
                               gconstpointer void_argval, MonoError *error) {
...
handle_enum:
  ...
  case MONO_TYPE_VALUETYPE:
    if (type->data.klass->enumtype) {
      ...
      g_warning(
          "generic valuetype %s not handled in custom attr value decoding",
          type->data.klass->name);
    }
    break;
  ...
  handle_type:
    arg_type =
        mono_reflection_type_get_handle((MonoReflectionType *)arg, error);
    return_if_nok(error);
  ...
  case MONO_TYPE_SZARRAY: {
    ...
    *retbuffer = buffer;
    ...
    if (eclass == mono_defaults.object_class && arg_eclass->valuetype) {
      ...
      for (i = 0; i < len; ++i) {
        encode_cattr_value(assembly, buffer, p, &buffer, &p, buflen,
                           m_class_get_byval_arg(arg_eclass), NULL, elptr,
                           error);
        return_if_nok(error);
        elptr += elsize;
      }
      ...
      for (i = 0; i < len; ++i) {
        encode_cattr_value(assembly, buffer, p, &buffer, &p, buflen,
                           m_class_get_byval_arg(eclass), NULL, elptr, error);
        return_if_nok(error);
        elptr += elsize;
      }
    } else {
      for (i = 0; i < len; ++i) {
        encode_cattr_value(
            assembly, buffer, p, &buffer, &p, buflen,
            m_class_get_byval_arg(eclass),
            mono_array_get_internal((MonoArray *)arg, MonoObject *, i), NULL,
            error);
        return_if_nok(error);
      }
    }
  ...
  case MONO_TYPE_OBJECT: {
    ...
    if (mono_object_isinst_checked(arg, mono_defaults.systemtype_class,
                                   error)) {
      *p++ = 0x50;
      goto handle_type;
    } else {
    ...
    if (klass->enumtype) {
      ...
      *p++ = 0x1D;
      ...
      encode_cattr_value(assembly, buffer, p, &buffer, &p, buflen,
                         klass_byval_arg, arg, NULL, error);
      return_if_nok(error);
      break;
    } else if (klass_byval_arg->type >= MONO_TYPE_BOOLEAN &&
               klass_byval_arg->type <= MONO_TYPE_R8) {
  ...
  default:
    mono_error_set_not_supported(
        error, "type 0x%02x not yet supported in custom attr encoder",
        simple_type);
  }
  *retp = p;
  *retbuffer = buffer;
...
static void encode_field_or_prop_type(MonoType *type, char *p, char **retp) {
...
#ifndef DISABLE_REFLECTION_EMIT
static void encode_named_val(MonoReflectionAssembly *assembly, char *buffer,
                             char *p, char **retbuffer, char **retp,
                             guint32 *buflen, MonoType *type, char *name,
                             MonoObject *value, MonoError *error) {
  int len;
  ...
  if (type->type == MONO_TYPE_VALUETYPE && type->data.klass->enumtype) {
    ...
    char *str =
        type_get_qualified_name(m_class_get_byval_arg(type->data.klass), NULL);
    len = strlen(str);
    g_free(str);
  } else {
    len = 0;
  ...
  memcpy(p, name, len);
  p += len;
  encode_cattr_value(assembly->assembly, buffer, p, &buffer, &p, buflen, type,
                     value, NULL, error);
  return_if_nok(error);
  *retp = p;
  *retbuffer = buffer;
 ...
 * \param fields
 * \param fieldValues
 * Creates the blob of data that needs to be saved in the metadata and that
 * represents the custom attributed described by \p ctor, \p ctorArgs etc.
 * \returns a \c Byte array representing the blob of data.
 */
MonoArray *mono_reflection_get_custom_attrs_blob(
    MonoReflectionAssembly *assembly, MonoObject *ctor, MonoArray *ctorArgs,
    MonoArray *properties, MonoArray *propValues, MonoArray *fields,
  ...
  result = mono_reflection_get_custom_attrs_blob_checked(
      assembly, ctor, ctorArgs, properties, propValues, fields, fieldValues,
      error);
 ...
 * Creates the blob of data that needs to be saved in the metadata and that
 * represents the custom attributed described by \p ctor, \p ctorArgs etc.
 * \returns a \c Byte array representing the blob of data.  On failure returns
 * NULL and sets \p error.
 */
MonoArrayHandle mono_reflection_get_custom_attrs_blob_checked(
    MonoReflectionAssembly *assembly, MonoObject *ctor, MonoArray *ctorArgs,
    MonoArray *properties, MonoArray *propValues, MonoArray *fields,
    MonoArray *fieldValues, MonoError *error) {
  ...
  if (strcmp(ctor->vtable->klass->name, "RuntimeConstructorInfo")) {
    ...
    sig = ctor_builder_to_signature_raw(NULL, (MonoReflectionCtorBuilder *)ctor,
                                        error); /* FIXME use handles */
    sig_free = sig;
    goto_if_nok(error, leave);
  } else {
    sig =
        mono_method_signature_internal(((MonoReflectionMethod *)ctor)->method);
  }
  ...
  *p++ = 0;
  ...
  for (i = 0; i < sig->param_count; ++i) {
    ...
    encode_cattr_value(assembly->assembly, buffer, p, &buffer, &p, &buflen,
                       sig->params[i], arg, NULL, error);
    goto_if_nok(error, leave);
  }
  i = 0;
  if (properties)
  ...
  *p++ = (i >> 8) & 0xff;
  ...
  if (properties) {
    ...
    for (i = 0; i < mono_array_length_internal(properties); ++i) {
      ...
      *p++ = 0x54; /* PROPERTY signature */
      ...
      encode_named_val(assembly, buffer, p, &buffer, &p, &buflen, ptype, pname,
                       prop, error);
      g_free(pname);
      goto_if_nok(error, leave);
    }
  ...
  if (fields) {
    ...
    for (i = 0; i < mono_array_length_internal(fields); ++i) {
      ...
      *p++ = 0x53; /* FIELD signature */
      ...
      encode_named_val(assembly, buffer, p, &buffer, &p, &buflen, ftype, fname,
                       field, error);
      g_free(fname);
      goto_if_nok(error, leave);
    }
  ...
  buflen = p - buffer;
  result = mono_array_new_handle(mono_domain_get(), mono_defaults.byte_class,
                                 buflen, error);
  goto_if_nok(error, leave);
...
static gboolean reflection_setup_class_hierarchy(GHashTable *unparented,
  ...
  while (g_hash_table_iter_next(&iter, (gpointer *)&child_type,
    ...
    } else if (strcmp(child_class->name, "Object") == 0 &&
               strcmp(child_class->name_space, "System") == 0) {
...
reflection_setup_internal_class_internal(MonoReflectionTypeBuilderHandle ref_tb,
  ...
  gint32 entering_state = MONO_HANDLE_GETVAL(
      MONO_HANDLE_CAST(MonoReflectionTypeBuilder, ref_tb), state);
  if (entering_state != MonoTypeBuilderNew) {
    g_assert(
        MONO_HANDLE_GETVAL(MONO_HANDLE_CAST(MonoReflectionType, ref_tb), type));
    goto leave;
  }

  MONO_HANDLE_SETVAL(ref_tb, state, gint32 /*MonoTypeBuilderState*/,
   ...
   * reflection_setup_internal_class is called too early, well before we know
   * whether the type will be a GTD or DEF, meaning we need to alloc enough
   * space to morth a def into a gtd.
   */
  MonoClass *klass;
  klass = (MonoClass *)mono_image_alloc0(
      &dynamic_image->image, MAX(sizeof(MonoClassDef), sizeof(MonoClassGtd)));
  ...
  goto_if_nok(error, leave);
  klass->name_space =
      mono_string_to_utf8_image(klass->image, ref_nspace, error);
  goto_if_nok(error, leave);
  ...
  ref_nesting_type =
      MONO_HANDLE_NEW_GET(MonoReflectionType, ref_tb, nesting_type);
  ...
  /*
    We must register all types as we cannot rely on the name_cache hashtable
    since we find the class by performing a mono_class_get which does the full
    resolution.

    Working around this semantics would require us to write a lot of code for no
    clear advantage.
  */
  mono_image_append_class_to_reflection_info_set(klass);

  mono_dynamic_image_register_token(
      dynamic_image, MONO_TOKEN_TYPE_DEF | table_idx,
      MONO_HANDLE_CAST(MonoObject, ref_tb), MONO_DYN_IMAGE_TOK_NEW);

  if ((!strcmp(klass->name, "ValueType") &&
       !strcmp(klass->name_space, "System")) ||
      (!strcmp(klass->name, "Object") &&
       !strcmp(klass->name_space, "System")) ||
  ...
  MONO_HANDLE_SETVAL(ref_tb_type, type, MonoType *,
  ...
  // Do here so that the search inside of the parent can see the above type
  // that's been set.
  MonoReflectionTypeHandle ref_parent;
  ref_parent = MONO_HANDLE_CAST(
      MonoReflectionType, MONO_HANDLE_NEW_GET(MonoObject, ref_tb, parent));
  MonoType *parent_type;
  parent_type = NULL;
  if (!MONO_HANDLE_IS_NULL(ref_parent)) {
    ...
    if (is_sre_type_builder(parent_klass)) {
      MonoTypeBuilderState parent_state =
          (MonoTypeBuilderState)MONO_HANDLE_GETVAL(
              MONO_HANDLE_CAST(MonoReflectionTypeBuilder, ref_parent), state);
    ...
    if (recursive_init) {
      // If we haven't encountered a cycle, force the creation of ref_parent's
      // type
  ...
  g_assert(
      !g_hash_table_lookup(unparented_classes, m_class_get_byval_arg(klass)));
  g_hash_table_insert(unparented_classes, m_class_get_byval_arg(klass),
                      parent_type);

  if (!MONO_HANDLE_IS_NULL(ref_nesting_type)) {
    if (!reflection_setup_internal_class(
            MONO_HANDLE_CAST(MonoReflectionTypeBuilder, ref_nesting_type),
            error))
      goto leave;

    MonoType *nesting_type =
        mono_reflection_type_handle_mono_type(ref_nesting_type, error);
    goto_if_nok(error, leave);
 ...
 */
static gboolean
reflection_init_generic_class(MonoReflectionTypeBuilderHandle ref_tb,
                              MonoError *error) {
  HANDLE_FUNCTION_ENTER();

  error_init(error);

  MonoTypeBuilderState ref_state =
      (MonoTypeBuilderState)MONO_HANDLE_GETVAL(ref_tb, state);
  ...
  MonoArrayHandle generic_params =
      MONO_HANDLE_NEW_GET(MonoArray, ref_tb, generic_params);
  int count = MONO_HANDLE_IS_NULL(generic_params)
                  ? 0
                  : mono_array_handle_length(generic_params);
  ...
  MonoGenericContainer *generic_container;
  generic_container = (MonoGenericContainer *)mono_image_alloc0(
      klass->image, sizeof(MonoGenericContainer));

  generic_container->owner.klass = klass;
  generic_container->type_argc = count;
  generic_container->type_params = (MonoGenericParamFull *)mono_image_alloc0(
      klass->image, sizeof(MonoGenericParamFull) * count);
  ...
  for (int i = 0; i < count; i++) {
    ...
    MonoType *param_type = mono_reflection_type_handle_mono_type(
        MONO_HANDLE_CAST(MonoReflectionType, ref_gparam), error);
    goto_if_nok(error, leave);
    MonoGenericParamFull *param =
        (MonoGenericParamFull *)param_type->data.generic_param;
    ...
    generic_container->type_params[i].info.flags =
        MONO_HANDLE_GETVAL(ref_gparam, attrs);

    g_assert(generic_container->type_params[i].owner);
  }

  generic_container->context.class_inst =
  ...
  canonical_inst->data.generic_class =
      mono_metadata_lookup_generic_class(klass, context->class_inst, FALSE);
...
mono_marshal_spec_from_builder(MonoImage *image, MonoAssembly *assembly,
  ...
  case MONO_NATIVE_CUSTOM:
    if (minfo->marshaltyperef) {
      MonoType *marshaltyperef = mono_reflection_type_get_handle(
          (MonoReflectionType *)minfo->marshaltyperef, error);
      if (!is_ok(error)) {
    ...
    if (minfo->mcookie) {
      res->data.custom_data.cookie =
          mono_string_to_utf8_checked_internal(minfo->mcookie, error);
      if (!is_ok(error)) {
        image_g_free(image, res);
        return NULL;
      }
...
mono_reflection_marshal_as_attribute_from_marshal_spec(MonoDomain *domain,
                                                       MonoClass *klass,
  ...
  MonoReflectionMarshalAsAttributeHandle minfo = MONO_HANDLE_CAST(
      MonoReflectionMarshalAsAttribute,
      mono_object_new_handle(
          domain, mono_class_get_marshal_as_attribute_class(), error));
  goto_if_nok(error, fail);
  guint32 utype;
  utype = spec->native;
  ...
  switch (utype) {
  case MONO_NATIVE_LPARRAY:
    MONO_HANDLE_SETVAL(minfo, array_subtype, guint32,
                       spec->data.array_data.elem_type);
    if (spec->data.array_data.num_elem != -1)
      MONO_HANDLE_SETVAL(minfo, size_const, gint32,
                         spec->data.array_data.num_elem);
    if (spec->data.array_data.param_num != -1)
      MONO_HANDLE_SETVAL(minfo, size_param_index, gint16,
                         spec->data.array_data.param_num);
    break;
  ...
  case MONO_NATIVE_CUSTOM:
    if (spec->data.custom_data.custom_name) {
      MonoType *mtype = mono_reflection_type_from_name_checked(
          spec->data.custom_data.custom_name, alc, klass->image, error);
      goto_if_nok(error, fail);

      if (mtype) {
        MonoReflectionTypeHandle rt =
            mono_type_get_object_handle(domain, mtype, error);
        goto_if_nok(error, fail);

        MONO_HANDLE_SET(minfo, marshal_type_ref, rt);
      }

      MonoStringHandle custom_name = mono_string_new_handle(
          domain, spec->data.custom_data.custom_name, error);
      goto_if_nok(error, fail);
      MONO_HANDLE_SET(minfo, marshal_type, custom_name);
    }
    if (spec->data.custom_data.cookie) {
      MonoStringHandle cookie =
          mono_string_new_handle(domain, spec->data.custom_data.cookie, error);
      goto_if_nok(error, fail);
...
#ifndef DISABLE_REFLECTION_EMIT
static MonoMethod *reflection_methodbuilder_to_mono_method(
    MonoClass *klass, ReflectionMethodBuilder *rmb, MonoMethodSignature *sig,
    MonoError *error) {
  MonoMethod *m;
  ...
  if (m->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) {
    ...
    method_aux->dllentry =
        rmb->dllentry ? string_to_utf8_image_raw(image, rmb->dllentry, error)
                      : image_strdup(image, m->name);
    mono_error_assert_ok(error);
    method_aux->dll = string_to_utf8_image_raw(image, rmb->dll, error);
    mono_error_assert_ok(error);

    ((MonoMethodPInvoke *)m)->piflags =
        (rmb->native_cc << 8) | (rmb->charset ? (rmb->charset - 1) * 2 : 0) |
        rmb->extra_flags;

    if (image_is_dynamic(klass->image))
      g_hash_table_insert(((MonoDynamicImage *)klass->image)->method_aux_hash,
                          m, method_aux);
    ...
    if (rmb->ilgen) {
      ...
      num_locals = rmb->ilgen->locals
                       ? mono_array_length_internal(rmb->ilgen->locals)
                       : 0;
    ...
    header = (MonoMethodHeader *)mono_image_g_malloc0(
        image, MONO_SIZEOF_METHOD_HEADER + num_locals * sizeof(MonoType *));
    ...
    for (i = 0; i < num_locals; ++i) {
      MonoReflectionLocalBuilder *lb = mono_array_get_internal(
          rmb->ilgen->locals, MonoReflectionLocalBuilder *, i);

      header->locals[i] = image_g_new0(image, MonoType, 1);
      MonoType *type = mono_reflection_type_get_handle(
          (MonoReflectionType *)lb->type, error);
      mono_error_assert_ok(error);
    ...
    if (num_clauses) {
      header->clauses =
          method_encode_clauses(image, (MonoDynamicImage *)klass->image,
  ...
  if (rmb->generic_params) {
    ...
    for (i = 0; i < count; i++) {
      MonoReflectionGenericParam *gp = mono_array_get_internal(
          rmb->generic_params, MonoReflectionGenericParam *, i);
      MonoType *gp_type =
          mono_reflection_type_get_handle((MonoReflectionType *)gp, error);
      mono_error_assert_ok(error);
      MonoGenericParamFull *param =
          (MonoGenericParamFull *)gp_type->data.generic_param;
      container->type_params[i] = *param;
      container->type_params[i].owner = container;

      gp->type.type->data.generic_param =
          (MonoGenericParam *)&container->type_params[i];
    ...
    /*
     * The method signature might have pointers to generic parameters that
     * belong to other methods. This is a valid SRE case, but the resulting
     * method signature must be encoded using the proper generic parameters.
     */
    for (i = 0; i < m->signature->param_count; ++i) {
      ...
      if (t->type == MONO_TYPE_MVAR) {
        ...
        if (gparam->num < count) {
          m->signature->params[i] =
              mono_metadata_type_dup(image, m->signature->params[i]);
          m->signature->params[i]->data.generic_param =
              mono_generic_container_get_param(container, gparam->num);
        }
      }
    }
  ...
  if (rmb->pinfo) {
    if (!method_aux)
      method_aux = image_g_new0(image, MonoReflectionMethodAux, 1);
    method_aux->param_names = image_g_new0(
        image, char *, mono_method_signature_internal(m)->param_count + 1);
    for (i = 0; i <= m->signature->param_count; ++i) {
      MonoReflectionParamBuilder *pb;
      if ((pb = mono_array_get_internal(rmb->pinfo,
                                        MonoReflectionParamBuilder *, i))) {
        if ((i > 0) && (pb->attrs)) {
          /* Make a copy since it might point to a shared type structure */
          m->signature->params[i - 1] =
              mono_metadata_type_dup(klass->image, m->signature->params[i - 1]);
        ...
        if (pb->attrs & PARAM_ATTRIBUTE_HAS_DEFAULT) {
          ...
          if (!method_aux->param_defaults) {
            method_aux->param_defaults =
                image_g_new0(image, guint8 *, m->signature->param_count + 1);
            method_aux->param_default_types =
                image_g_new0(image, guint32, m->signature->param_count + 1);
          }
          assembly = (MonoDynamicImage *)klass->image;
          idx =
              mono_dynimage_encode_constant(assembly, pb->def_value, &def_type);
        ...
        if (pb->name) {
          method_aux->param_names[i] =
              string_to_utf8_image_raw(image, pb->name, error);
          mono_error_assert_ok(error);
        }
        if (pb->cattrs) {
          if (!method_aux->param_cattr)
            method_aux->param_cattr = image_g_new0(
                image, MonoCustomAttrInfo *, m->signature->param_count + 1);
          method_aux->param_cattr[i] =
              mono_custom_attrs_from_builders(image, klass->image, pb->cattrs);
        }
      }
    }
  }
    ...
    for (i = 0; i < mono_array_length_internal(rmb->pinfo); ++i) {
      ...
      if ((pb = mono_array_get_internal(rmb->pinfo,
        ...
        if (pb->marshal_info) {
          if (specs == NULL)
            specs =
                image_g_new0(image, MonoMarshalSpec *, sig->param_count + 1);
          specs[pb->position] = mono_marshal_spec_from_builder(
              image, klass->image->assembly, pb->marshal_info, error);
          goto_if_nok(error, fail);
        }
      }
    }
  ...
  if (image_is_dynamic(klass->image) && method_aux)
    g_hash_table_insert(((MonoDynamicImage *)klass->image)->method_aux_hash, m,
                        method_aux);
...
static MonoMethod *ctorbuilder_to_mono_method(MonoClass *klass,
                                              MonoReflectionCtorBuilder *mb,
                                              MonoError *error) {
  ...
  sig = ctor_builder_to_signature_raw(klass->image, mb,
                                      error); /* FIXME use handles */
  mono_loader_unlock();
  return_val_if_nok(error, NULL);

  mb->mhandle =
      reflection_methodbuilder_to_mono_method(klass, &rmb, sig, error);
  return_val_if_nok(error, NULL);
...
static MonoMethod *
methodbuilder_to_mono_method(MonoClass *klass,
  ...
  MonoReflectionMethodBuilder *mb =
      MONO_HANDLE_RAW(ref_mb); /* FIXME use handles */
...
static MonoMethod *methodbuilder_to_mono_method_raw(
    MonoClass *klass, MonoReflectionMethodBuilder *mb_raw, MonoError *error) {
  HANDLE_FUNCTION_ENTER(); /* FIXME change callers of
                              methodbuilder_to_mono_method_raw to use handles */
  error_init(error);
 ...
 *
 */
static gboolean fix_partial_generic_class(MonoClass *klass, MonoError *error) {
  ...
  if (klass->parent != gklass->parent) {
    MonoType *parent_type = mono_class_inflate_generic_type_checked(
        m_class_get_byval_arg(m_class_get_parent(gklass)),
        &mono_class_get_generic_class(klass)->context, error);
    if (is_ok(error)) {
  ...
  if (mcount != gmcount) {
    ...
    klass->methods = (MonoMethod **)mono_image_alloc(
        klass->image, sizeof(MonoMethod *) * (gmcount + 1));
  ...
  if (klass->interface_count &&
    ...
    klass->interface_count = gklass->interface_count;
    klass->interfaces = (MonoClass **)mono_image_alloc(
        klass->image, sizeof(MonoClass *) * gklass->interface_count);
    ...
    for (i = 0; i < gklass->interface_count; ++i) {
      MonoType *iface_type = mono_class_inflate_generic_type_checked(
          m_class_get_byval_arg(gklass_interfaces[i]),
  ...
  if (fcount != gfcount) {
    ...
    for (i = 0; i < gfcount; i++) {
      ...
      klass->fields[i].type = mono_class_inflate_generic_type_checked(
          gklass->fields[i].type, mono_class_get_context(klass), error);
      return_val_if_nok(error, FALSE);
    }
  }
 ...
 */
static gboolean ensure_generic_class_runtime_vtable(MonoClass *klass,
                                                    MonoError *error) {
  MonoClass *gklass = mono_class_get_generic_class(klass)->container_class;
...
static gboolean ensure_runtime_vtable(MonoClass *klass, MonoError *error) {
  MonoReflectionTypeBuilder *tb =
      mono_class_get_ref_info_raw(klass); /* FIXME use handles */
  int i, num, j;

  error_init(error);

  if (!image_is_dynamic(klass->image) || (!tb && !mono_class_is_ginst(klass)) ||
  ...
  if (tb) {
    ...
    klass->methods = (MonoMethod **)mono_image_alloc(
        klass->image, sizeof(MonoMethod *) * num);
    num = tb->ctors ? mono_array_length_internal(tb->ctors) : 0;
    for (i = 0; i < num; ++i) {
      MonoMethod *ctor = ctorbuilder_to_mono_method(
          klass,
    ...
    j = i;
    for (i = 0; i < num; ++i) {
      MonoMethod *meth = methodbuilder_to_mono_method_raw(
          klass,
          mono_array_get_internal(tb->methods, MonoReflectionMethodBuilder *,
                                  i),
          error); /* FIXME use handles */
      if (!meth)
        return FALSE;
    ...
    if (tb->interfaces) {
      ...
      klass->interfaces = (MonoClass **)mono_image_alloc(
          klass->image, sizeof(MonoClass *) * klass->interface_count);
      for (i = 0; i < klass->interface_count; ++i) {
        MonoType *iface = mono_type_array_get_and_resolve_raw(
            tb->interfaces, i, error); /* FIXME use handles */
        return_val_if_nok(error, FALSE);
    ...
    if (!ensure_generic_class_runtime_vtable(klass, error)) {
      mono_class_set_type_load_failure(
          klass, "Could not initialize vtable for generic class due to: %s",
...
static MonoMethod *mono_reflection_method_get_handle(MonoObject *method,
  ...
  if (mono_is_sre_method_on_tb_inst(klass)) {
    ...
    MonoMethod *result = (MonoMethod *)mono_reflection_resolve_object(
        NULL, method, &handle_class, NULL, error);
    return_val_if_nok(error, NULL);
  ...
  if (tb->methods) {
    for (i = 0; i < tb->num_methods; ++i) {
      MonoReflectionMethodBuilder *mb = mono_array_get_internal(
          tb->methods, MonoReflectionMethodBuilder *, i);
    ...
    for (i = 0; i < tb->num_methods; ++i) {
      ...
      if (mb->override_methods) {
        for (j = 0; j < mono_array_length_internal(mb->override_methods); ++j) {
          m = mono_array_get_internal(mb->override_methods,
                                      MonoReflectionMethod *, j);

          (*overrides)[onum * 2] =
              mono_reflection_method_get_handle((MonoObject *)m, error);
          return_if_nok(error);
...
static gint32
modulebuilder_get_next_table_index(MonoReflectionModuleBuilder *mb,
                                   gint32 table, gint32 num_fields,
                                   MonoError *error) {
  error_init(error);

  if (mb->table_indexes == NULL) {
    MonoArray *arr = mono_array_new_checked(mono_object_domain(&mb->module.obj),
                                            mono_defaults.int_class, 64, error);
    return_val_if_nok(error, 0);
    for (int i = 0; i < 64; i++) {
...
static void typebuilder_setup_one_field(MonoDynamicImage *dynamic_image,
                                        MonoClass *klass, int32_t first_idx,
                                        MonoArray *tb_fields, int i,
                                        MonoFieldDefaultValue *def_value_out,
                                        MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  {
    ...
    fb = (MonoReflectionFieldBuilder *)mono_array_get_internal(tb_fields,
                                                               gpointer, i);
    field = &klass->fields[i];
    field->parent = klass;
    field->name = string_to_utf8_image_raw(image, fb->name,
                                           error); /* FIXME use handles */
    goto_if_nok(error, leave);
    if (fb->attrs) {
      MonoType *type = mono_reflection_type_get_handle(
          (MonoReflectionType *)fb->type, error);
      goto_if_nok(error, leave);
      field->type = mono_metadata_type_dup(klass->image, type);
      field->type->attrs = fb->attrs;
    } else {
      field->type = mono_reflection_type_get_handle(
          (MonoReflectionType *)fb->type, error);
      goto_if_nok(error, leave);
    }
    ...
    if (!klass->enumtype && !mono_type_get_underlying_type(field->type)) {
      mono_class_set_type_load_failure(
          klass, "Field '%s' is an enum type with a bad underlying type",
          field->name);
      goto leave;
    }
    ...
    if (fb->def_value) {
      ...
      idx = mono_dynimage_encode_constant(assembly, fb->def_value,
                                          &def_value_out->def_type);
      /* Copy the data from the blob since it might get realloc-ed */
      p = assembly->blob.data + idx;
      len = mono_metadata_decode_blob_size(p, &p2);
      len += p2 - p;
    ...
    MonoObjectHandle field_builder_handle = MONO_HANDLE_CAST(
        MonoObject, MONO_HANDLE_NEW(MonoReflectionFieldBuilder, fb));
    mono_dynamic_image_register_token(
        dynamic_image,
        mono_metadata_make_token(MONO_TABLE_FIELD, first_idx + i),
        field_builder_handle, MONO_DYN_IMAGE_TOK_NEW);
  }
leave:
...
static void typebuilder_setup_fields(MonoClass *klass, MonoError *error) {
  MonoReflectionTypeBuilder *tb =
      mono_class_get_ref_info_raw(klass); /* FIXME use handles */
  ...
  gint32 first_idx = 0;
  if (tb->num_fields > 0) {
    first_idx = modulebuilder_get_next_table_index(
        tb->module, MONO_TABLE_FIELD, (gint32)tb->num_fields, error);
    return_if_nok(error);
  }
  mono_class_set_first_field_idx(
      klass, first_idx - 1); /* Why do we subtract 1? because
                                mono_class_create_from_typedef does it, too. */
  ...
  The issue is that the runtime doesn't know how to setup the fields of a
  typebuider and crash. On the static path no field class is resolved, only
  types are built. This is the right thing to do but we suck. Setting
  size_inited is harmless because we're doing the same job as
  mono_class_setup_fields anyway.
  */
  klass->size_inited = 1;

  for (i = 0; i < fcount; ++i) {
    typebuilder_setup_one_field(tb->module->dynamic_image, klass, first_idx,
                                tb->fields, i, &def_values[i], error);
    if (!is_ok(error))
      return;
  }

  if (!mono_class_has_failure(klass))
    mono_class_layout_fields(klass, instance_size, packing_size, tb->class_size,
                             TRUE);
}

static void typebuilder_setup_properties(MonoClass *klass, MonoError *error) {
  MonoReflectionTypeBuilder *tb =
      mono_class_get_ref_info_raw(klass); /* FIXME use handles */
  ...
  for (i = 0; i < info->count; ++i) {
    pb = mono_array_get_internal(tb->properties,
                                 MonoReflectionPropertyBuilder *, i);
    properties[i].parent = klass;
    properties[i].attrs = pb->attrs;
    properties[i].name = string_to_utf8_image_raw(
        image, pb->name, error); /* FIXME use handles */
    if (!is_ok(error))
      return;
    if (pb->get_method)
    ...
    if (pb->def_value) {
      ...
      if (!info->def_values)
        info->def_values =
            image_g_new0(image, MonoFieldDefaultValue, info->count);
      properties[i].attrs |= PROPERTY_ATTRIBUTE_HAS_DEFAULT;
      idx = mono_dynimage_encode_constant(assembly, pb->def_value,
                                          &info->def_values[i].def_type);
      /* Copy the data from the blob since it might get realloc-ed */
      p = assembly->blob.data + idx;
      len = mono_metadata_decode_blob_size(p, &p2);
      len += p2 - p;
...
static void typebuilder_setup_events(MonoClass *klass, MonoError *error) {
  MonoReflectionTypeBuilder *tb =
      mono_class_get_ref_info_raw(klass); /* FIXME use handles */
  ...
  for (i = 0; i < info->count; ++i) {
    ...
    events[i].name = string_to_utf8_image_raw(image, eb->name,
                                              error); /* FIXME use handles */
    if (!is_ok(error))
      return;
    if (eb->add_method)
    ...
    if (eb->other_methods) {
      int j;
      events[i].other =
          image_g_new0(image, MonoMethod *,
                       mono_array_length_internal(eb->other_methods) + 1);
      for (j = 0; j < mono_array_length_internal(eb->other_methods); ++j) {
        MonoReflectionMethodBuilder *mb = mono_array_get_internal(
            eb->other_methods, MonoReflectionMethodBuilder *, j);
...
static gboolean
remove_instantiations_of_and_ensure_contents(gpointer key, gpointer value,
                                             gpointer user_data) {
  struct remove_instantiations_user_data *data =
      (struct remove_instantiations_user_data *)user_data;
  ...
  if ((type->type == MONO_TYPE_GENERICINST) &&
      (type->data.generic_class->container_class == klass)) {
    ...
    if (!fix_partial_generic_class(inst_klass, error)) {
      mono_class_set_type_load_failure(
          inst_klass, "Could not initialized generic type instance due to: %s",
          mono_error_get_message(error));
      // Marked the class with failure, but since some other instantiation
      // already failed, just report that one, and swallow the error from this
      // one.
      if (already_failed)
        mono_error_cleanup(error);
    }
 ...
 */
static gboolean
reflection_setup_internal_class(MonoReflectionTypeBuilderHandle ref_tb,
                                MonoError *error) {
  HANDLE_FUNCTION_ENTER();

  MonoReflectionModuleBuilderHandle module_ref =
      MONO_HANDLE_NEW_GET(MonoReflectionModuleBuilder, ref_tb, module);
  GHashTable *unparented_classes =
      MONO_HANDLE_GETVAL(module_ref, unparented_classes);
  gboolean ret_val;
  ...
  if (unparented_classes) {
    ...
    MONO_HANDLE_SETVAL(module_ref, unparented_classes, GHashTable *,
                       unparented_classes);
...
MonoReflectionTypeHandle ves_icall_TypeBuilder_create_runtime_class(
    MonoReflectionTypeBuilderHandle ref_tb, MonoError *error) {
  error_init(error);
  ...
  mono_save_custom_attrs(klass->image, klass,
                         MONO_HANDLE_RAW(cattrs)); /* FIXME use handles */
  ...
  if (!MONO_HANDLE_IS_NULL(nested_types)) {
    ...
    MonoReflectionTypeHandle nested_tb =
    ...
    for (int i = 0; i < num_nested; ++i) {
      ...
      if (MONO_HANDLE_GETVAL(nested_tb, type) == NULL) {
        reflection_setup_internal_class(
            MONO_HANDLE_CAST(MonoReflectionTypeBuilder, nested_tb), error);
        mono_error_assert_ok(error);
      }

      MonoType *subtype =
          mono_reflection_type_handle_mono_type(nested_tb, error);
      goto_if_nok(error, failure);
      nested = mono_g_list_prepend_image(
          klass->image, nested, mono_class_from_mono_type_internal(subtype));
    }
  ...
  if (!MONO_HANDLE_IS_NULL(generic_params)) {
    ...
    MonoReflectionTypeHandle ref_gparam =
        MONO_HANDLE_NEW(MonoReflectionType, NULL);
    for (int i = 0; i < num_params; i++) {
      MONO_HANDLE_ARRAY_GETREF(ref_gparam, generic_params, i);
      MonoType *param_type =
          mono_reflection_type_handle_mono_type(ref_gparam, error);
      goto_if_nok(error, failure);
  ...
  /*
   * If we are a generic TypeBuilder, there might be instantiations in the type
   * cache which have type System.Reflection.MonoGenericClass, but after the
   * type is created, we want to return normal System.MonoType objects, so clear
   * these out from the cache.
   *
   * Together with this we must ensure the contents of all instances to match
   * the created type.
   */
  if (domain->type_hash && mono_class_is_gtd(klass)) {
    ...
    mono_g_hash_table_foreach_remove(
        domain->type_hash, remove_instantiations_of_and_ensure_contents, &data);
    goto_if_nok(error, failure);
  }
...
failure:
  mono_class_set_type_load_failure(
      klass, "TypeBuilder could not create runtime class due to: %s",
      mono_error_get_message(error));
  klass->wastypebuilder = TRUE;
  mono_domain_unlock(domain);
  mono_loader_unlock();
...
static void free_dynamic_method(void *dynamic_method) {
  ...
  mono_domain_lock(domain);
  dis_link = (guint32)(size_t)g_hash_table_lookup(domain->method_to_dyn_method,
                                                  method);
...
reflection_create_dynamic_method(MonoReflectionDynamicMethodHandle ref_mb,
  ...
  MonoReflectionDynamicMethod *mb =
      MONO_HANDLE_RAW(ref_mb); /* FIXME convert reflection_create_dynamic_method
                                  to use handles */
  ...
  for (i = 0; i < mb->nrefs; i += 2) {
    ...
    if (strcmp(obj->vtable->klass->name, "DynamicMethod") == 0) {
      ...
      ref = mono_reflection_resolve_object(mb->module->image, obj,
      ...
      if (!ref)
        ...
        ex = mono_security_core_clr_ensure_dynamic_method_resolved_object(
            ref, handle_class);
    ...
    rmb.refs[i] = ref; /* FIXME: GC object stored in unmanaged memory (change
                          also resolve_object() signature) */
  ...
  if (mb->owner) {
    MonoType *owner_type =
        mono_reflection_type_get_handle((MonoReflectionType *)mb->owner, error);
    if (!is_ok(error)) {
      g_free(rmb.refs);
      return FALSE;
    }
    ...
    ass =
        (mb->module && mb->module->image) ? mb->module->image->assembly : NULL;
  }
  ...
  if (!mono_gc_reference_queue_add_internal(queue, (MonoObject *)mb,
                                            release_data))
    g_free(release_data);

  /* Fix up refs entries pointing at us */
  for (l = mb->referenced_by; l; l = l->next) {
    MonoReflectionDynamicMethod *method =
        (MonoReflectionDynamicMethod *)l->data;
    ...
    for (i = 0; i < GPOINTER_TO_UINT(data[0]); i += 2) {
      if ((data[i + 1] == mb) &&
          (data[i + 1 + 1] == mono_defaults.methodhandle_class))
        data[i + 1] = mb->mhandle;
    }
  }
  ...
  g_hash_table_insert(domain->method_to_dyn_method, handle,
                      (gpointer)(size_t)mono_gchandle_new_weakref_internal(
                          (MonoObject *)mb, TRUE));
  mono_domain_unlock(domain);
...
void ves_icall_DynamicMethod_create_dynamic_method(
    MonoReflectionDynamicMethodHandle mb, MonoError *error) {
  (void)reflection_create_dynamic_method(mb, error);
}

#endif /* DISABLE_REFLECTION_EMIT */

MonoMethodSignature *mono_reflection_lookup_signature(MonoImage *image,
                                                      MonoMethod *method,
                                                      guint32 token,
                                                      MonoError *error) {
  MonoMethodSignature *sig;
  ...
  sig = (MonoMethodSignature *)g_hash_table_lookup(
      ((MonoDynamicImage *)image)->vararg_aux_hash, GUINT_TO_POINTER(token));
  if (sig)
    return sig;
 ...
 * dynamic types.
 */
static void ensure_complete_type(MonoClass *klass, MonoError *error) {
  HANDLE_FUNCTION_ENTER();

  error_init(error);

  if (image_is_dynamic(klass->image) && !klass->wastypebuilder &&
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    for (i = 0; i < inst->type_argc; ++i) {
      ensure_complete_type(
          mono_class_from_mono_type_internal(inst->type_argv[i]), error);
      goto_if_nok(error, exit);
    }
  }
...
gpointer mono_reflection_resolve_object(MonoImage *image, MonoObject *obj,
                                        MonoClass **handle_class,
                                        MonoGenericContext *context,
                                        MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  if (strcmp(oklass->name, "String") == 0) {
    result = MONO_HANDLE_RAW(mono_string_intern_checked(
        MONO_HANDLE_NEW(MonoString, (MonoString *)obj), error));
    goto_if_nok(error, return_null);
    *handle_class = mono_defaults.string_class;
    g_assert(result);
  } else if (strcmp(oklass->name, "RuntimeType") == 0) {
    MonoType *type =
        mono_reflection_type_get_handle((MonoReflectionType *)obj, error);
    goto_if_nok(error, return_null);
    MonoClass *mc = mono_class_from_mono_type_internal(type);
    ...
    if (context) {
      MonoType *inflated =
          mono_class_inflate_generic_type_checked(type, context, error);
      goto_if_nok(error, return_null);
    ...
    *handle_class = mono_defaults.typehandle_class;
    ...
    if (context) {
      result = mono_class_inflate_generic_method_checked((MonoMethod *)result,
                                                         context, error);
      mono_error_assert_ok(error);
    }
    *handle_class = mono_defaults.methodhandle_class;
    ...
    if (context) {
      MonoType *inflated = mono_class_inflate_generic_type_checked(
          m_class_get_byval_arg(field->parent), context, error);
      goto_if_nok(error, return_null);
    ...
    *handle_class = mono_defaults.fieldhandle_class;
    ...
    MonoReflectionTypeBuilderHandle tb = MONO_HANDLE_NEW(
        MonoReflectionTypeBuilder, (MonoReflectionTypeBuilder *)obj);
    MonoType *type =
        mono_reflection_type_get_handle(&MONO_HANDLE_RAW(tb)->type, error);
    goto_if_nok(error, return_null);
    MonoClass *klass;
    ...
    *handle_class = mono_defaults.typehandle_class;
    ...
    for (i = 0; i < nargs; ++i) {
      sig->params[i] = mono_type_array_get_and_resolve_raw(
          helper->arguments, i, error); /* FIXME use handles */
      if (!is_ok(error)) {
        image_g_free(image, sig);
        goto return_null;
      }
    ...
    *handle_class = mono_defaults.methodhandle_class;
             ...
             mono_is_sre_ctor_builder(oklass) || is_sre_field_builder(oklass) ||
             is_sre_gparam_builder(oklass) || is_sre_generic_instance(oklass) ||
             is_sre_array(oklass) || is_sre_byref(oklass) ||
    ...
    if (!resolve_method) {
      MonoMethod *m = mono_class_get_method_from_name_checked(
          mono_class_get_module_builder_class(), "RuntimeResolve", 1, 0, error);
      mono_error_assert_ok(error);
      g_assert(m);
      mono_memory_barrier();
      resolve_method = m;
    ...
    goto_if_nok(error, return_null);
    g_assert(obj);
    result = mono_reflection_resolve_object(image, obj, handle_class, context,
                                            error);
    goto exit;
  } else {
...
gpointer mono_reflection_resolve_object_handle(MonoImage *image,
                                               MonoObjectHandle obj,
                                               MonoClass **handle_class,
                                               MonoGenericContext *context,
                                               MonoError *error) {
  return mono_reflection_resolve_object(image, MONO_HANDLE_RAW(obj),
...
void mono_reflection_dynimage_basic_init(
    MonoReflectionAssemblyBuilder *assemblyb, MonoError *error) {
  g_error(
      "This mono runtime was configured with --enable-minimal=reflection_emit, "
      "so System.Reflection.Emit is not supported.");
}
...
guint32 mono_image_insert_string(MonoReflectionModuleBuilderHandle module,
                                 MonoStringHandle str, MonoError *error) {
  g_assert_not_reached();
  return 0;
}
...
ves_icall_TypeBuilder_create_runtime_class(MonoReflectionTypeBuilderHandle tb,
                                           MonoError *error) {
  g_assert_not_reached();
  return MONO_HANDLE_CAST(MonoReflectionType, NULL_HANDLE);
}
...
void mono_sre_generic_param_table_entry_free(GenericParamTableEntry *entry) {
  MONO_GC_UNREGISTER_ROOT_IF_MOVING(entry->gparam);
  g_free(entry);
}

gint32 ves_icall_ModuleBuilder_getToken(MonoReflectionModuleBuilderHandle mb,
                                        MonoObjectHandle obj,
  ...
  return mono_image_create_token(MONO_HANDLE_GETVAL(mb, dynamic_image), obj,
                                 create_open_instance, TRUE, error);
}

gint32 ves_icall_ModuleBuilder_getMethodToken(
    MonoReflectionModuleBuilderHandle mb, MonoReflectionMethodHandle method,
    MonoArrayHandle opt_param_types, MonoError *error) {
  if (MONO_HANDLE_IS_NULL(method)) {
    mono_error_set_argument_null(error, "method", "");
    return 0;
  }

  return mono_image_create_method_token(MONO_HANDLE_GETVAL(mb, dynamic_image),
                                        MONO_HANDLE_CAST(MonoObject, method),
                                        opt_param_types, error);
}

#ifndef ENABLE_NETCORE
void ves_icall_ModuleBuilder_WriteToFile(MonoReflectionModuleBuilderHandle mb,
                                         HANDLE file, MonoError *error) {
  mono_image_create_pefile(MONO_HANDLE_RAW(mb), file, error);
}

void ves_icall_ModuleBuilder_build_metadata(
    MonoReflectionModuleBuilderHandle mb, MonoError *error) {
  mono_image_build_metadata(MONO_HANDLE_RAW(mb), error);
}
#endif

void ves_icall_ModuleBuilder_RegisterToken(MonoReflectionModuleBuilderHandle mb,
                                           MonoObjectHandle obj, guint32 token,
                                           MonoError *error) {
  /* This function may be called by ModuleBuilder.FixupTokens to update
   * an existing token, so replace is okay here. */
  mono_dynamic_image_register_token(MONO_HANDLE_GETVAL(mb, dynamic_image),
                                    token, obj, MONO_DYN_IMAGE_TOK_REPLACE);
}

MonoObjectHandle
ves_icall_ModuleBuilder_GetRegisteredToken(MonoReflectionModuleBuilderHandle mb,
                                           guint32 token, MonoError *error) {
...
MonoArrayHandle ves_icall_CustomAttributeBuilder_GetBlob(
    MonoReflectionAssemblyHandle assembly, MonoObjectHandle ctor,
  ...
  return mono_reflection_get_custom_attrs_blob_checked(
      MONO_HANDLE_RAW(assembly), MONO_HANDLE_RAW(ctor),
...
void ves_icall_AssemblyBuilder_basic_init(
    MonoReflectionAssemblyBuilderHandle assemblyb, MonoError *error) {
  uint32_t gchandle =
      mono_gchandle_from_handle(MONO_HANDLE_CAST(MonoObject, assemblyb), TRUE);
  mono_reflection_dynimage_basic_init(MONO_HANDLE_RAW(assemblyb), error);
  mono_gchandle_free_internal(gchandle);
}

void ves_icall_AssemblyBuilder_UpdateNativeCustomAttributes(
    MonoReflectionAssemblyBuilderHandle assemblyb, MonoError *error) {
  MonoArrayHandle cattrs = MONO_HANDLE_NEW_GET(MonoArray, assemblyb, cattrs);

  MonoReflectionAssemblyHandle assembly_handle =
      MONO_HANDLE_CAST(MonoReflectionAssembly, assemblyb);
...
void ves_icall_EnumBuilder_setup_enum_type(MonoReflectionTypeHandle enumtype,
                                           MonoReflectionTypeHandle t,
                                           MonoError *error) {
  MONO_HANDLE_SETVAL(enumtype, type, MonoType *, MONO_HANDLE_GETVAL(t, type));
}

void ves_icall_ModuleBuilder_basic_init(
    MonoReflectionModuleBuilderHandle moduleb, MonoError *error) {
  mono_image_module_basic_init(moduleb, error);
}

guint32
ves_icall_ModuleBuilder_getUSIndex(MonoReflectionModuleBuilderHandle module,
                                   MonoStringHandle str, MonoError *error) {
  return mono_image_insert_string(module, str, error);
}

void ves_icall_ModuleBuilder_set_wrappers_type(
    MonoReflectionModuleBuilderHandle moduleb,
