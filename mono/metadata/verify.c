 ...
 *
 */
void mono_verifier_set_mode(MiniVerifierMode mode) { verifier_mode = mode; }

void mono_verifier_enable_verify_all() { verify_all = TRUE; }

/*
 * Returns TURE if @type is VAR or MVAR
 */
static gboolean mono_type_is_generic_argument(MonoType *type) {
...
*/
static gboolean is_valid_generic_instantiation(MonoGenericContainer *gc,
  ...
  for (i = 0; i < gc->type_argc; ++i) {
    MonoGenericParamInfo *param_info =
        mono_generic_container_get_param_info(gc, i);
     ...
     *
     * Due to how types are encoded we must check for the case of a genericinst
     * MonoType and GTD MonoClass. This happens in cases such as: class Foo<T>
     * { void X() { new Bar<T> (); } }
     *
     * Open instantiations can have GTDs as this happens when one type is
     * instantiated with others params and the former has an expansion into the
     * later. For example: class B<K> {} class A<T>: B<K> {} The type A <K> has
     * a parent B<K>, that is inflated into the GTD B<>. Since A<K> is open,
     * thus not instantiatable, this is valid.
     */
    if (mono_class_is_gtd(paramClass) &&
        param_type->type != MONO_TYPE_GENERICINST && !ginst->is_open)
      return FALSE;
    ...
    if ((param_info->flags &
        ...
        !m_class_is_valuetype(paramClass) &&
        !mono_class_has_default_constructor(paramClass, TRUE))
      return FALSE;
    ...
    for (constraints = param_info->constraints; *constraints; ++constraints) {
      ...
      inflated = mono_class_inflate_generic_type_checked(
          m_class_get_byval_arg(ctr), context, error);
      if (!is_ok(error)) {
...
gboolean mono_verifier_class_is_valid_generic_instantiation(MonoClass *klass) {
  ...
  MonoGenericContainer *gc =
      mono_class_get_generic_container(gklass->container_class);
...
mono_verifier_is_method_valid_generic_instantiation(MonoMethod *method) {
  ...
  MonoGenericContainer *gc =
      mono_method_get_generic_container(gmethod->declaring);
...
#define IS_FAIL_FAST_MODE(ctx)                                                 \
  (((ctx)->level & MONO_VERIFY_FAIL_FAST) == MONO_VERIFY_FAIL_FAST)
#define IS_SKIP_VISIBILITY(ctx)                                                \
  (((ctx)->level & MONO_VERIFY_SKIP_VISIBILITY) == MONO_VERIFY_SKIP_VISIBILITY)
#define IS_REPORT_ALL_ERRORS(ctx)                                              \
  (((ctx)->level & MONO_VERIFY_REPORT_ALL_ERRORS) ==                           \
   MONO_VERIFY_REPORT_ALL_ERRORS)
#define CLEAR_PREFIX(ctx, prefix)                                              \
  do {                                                                         \
...
#define CHECK_ADD4_OVERFLOW_UN(a, b)                                           \
  ((guint32)(0xFFFFFFFFU) - (guint32)(b) < (guint32)(a))
#define CHECK_ADD8_OVERFLOW_UN(a, b)                                           \
  ((guint64)(0xFFFFFFFFFFFFFFFFUL) - (guint64)(b) < (guint64)(a))
...
#define ADDP_IS_GREATER_OR_OVF(a, b, c)                                        \
  (((a) + (b) > (c)) || CHECK_ADDP_OVERFLOW_UN(a, b))
#define ADD_IS_GREATER_OR_OVF(a, b, c)                                         \
  (((a) + (b) > (c)) || CHECK_ADD4_OVERFLOW_UN(a, b))
...
enum {
  ...
  /*This instruction is part of the delegate construction sequence, it cannot be
     target of a branch.*/
...
typedef struct {
  ...
  /*TODO get rid of target here, need_merge in mono_method_verify and hoist the
   * merging code in the branching code*/
  int target;
  ...
  /*This flag helps solving a corner case of delegate verification in that you
   *cannot have a "starg 0"
   *on a method that creates a delegate for a non-final virtual method using
   *ldftn*/
  gboolean has_this_store;

  /*This flag is used to control if the contructor of the parent class has been
   *called. If the this pointer is pushed on the eval stack and it's a reference
   *type constructor and super_ctor_called is false, the uninitialized flag is
   *set on the pushed value.
   *
   * Poping an uninitialized this ptr from the eval stack is an unverifiable
   *operation unless the safe variant is used. Only a few opcodes can use it :
   *dup, pop, ldfld, stfld and call to a constructor.
   */
...
static void merge_stacks(VerifyContext *ctx, ILCodeDesc *from, ILCodeDesc *to,
                         gboolean start, gboolean external);

static int get_stack_type(MonoType *type);

static gboolean mono_delegate_signature_equal(MonoMethodSignature *delegate_sig,
                                              MonoMethodSignature *method_sig,
                                              gboolean is_static_ldftn);

static gboolean mono_class_is_valid_generic_instantiation(VerifyContext *ctx,
                                                          MonoClass *klass);

static gboolean mono_method_is_valid_generic_instantiation(VerifyContext *ctx,
                                                           MonoMethod *method);

static MonoGenericParam *
verifier_get_generic_param_from_type(VerifyContext *ctx, MonoType *type);

static gboolean verifier_class_is_assignable_from(MonoClass *target,
                                                  MonoClass *candidate);
//////////////////////////////////////////////////////////////////
...
enum {
  ...
  TYPE_NATIVE_INT = 3,
  TYPE_R8 = 4,
  /* Used by operator tables to resolve pointer types (managed & unmanaged) and
     by unmanaged pointer types*/
  TYPE_PTR = 5,
...
static const char *const type_names[TYPE_MAX + 1] = {
    "Invalid", "Int32",          "Int64",  "Native Int",
    "Float64", "Native Pointer", "Complex"};
...
#define _MEM_ALLOC(amt)                                                        \
  do {                                                                         \
    allocated_memory += (amt);                                                 \
    working_set += (amt);                                                      \
  } while (0)
#define _MEM_FREE(amt)                                                         \
  do {                                                                         \
    working_set -= (amt);                                                      \
  } while (0)
...
static void finish_collect_stats(void) {
...
static void init_verifier_stats(void) {
  static gboolean inited;
  if (!inited) {
    inited = TRUE;
    mono_counters_register("Maximum memory allocated during verification",
                           MONO_COUNTER_METADATA | MONO_COUNTER_INT,
                           &max_allocated_memory);
    mono_counters_register("Maximum memory used during verification",
                           MONO_COUNTER_METADATA | MONO_COUNTER_INT,
                           &max_working_set);
    mono_counters_register("Total memory allocated for verification",
                           MONO_COUNTER_METADATA | MONO_COUNTER_INT,
 ...
 */
static gboolean token_bounds_check(MonoImage *image, guint32 token) {
...
static MonoType *mono_type_create_fnptr_from_mono_method(VerifyContext *ctx,
                                                         MonoMethod *method) {
 ...
 */
static gboolean mono_type_is_enum_type(MonoType *type) {
 ...
 *
 */
static gboolean mono_type_is_value_type(MonoType *type, const char *namespace_,
                                        const char *name) {
 ...
 *
 * This functions is just like mono_type_get_underlying_type but it doesn't care
 * if the type is byref.
 *
 * Returns the underlying type of @type regardless if it is byref or not.
 */
static MonoType *mono_type_get_underlying_type_any(MonoType *type) {
  if (type->type == MONO_TYPE_VALUETYPE &&
      m_class_is_enumtype(type->data.klass))
...
static G_GNUC_UNUSED const char *mono_type_get_stack_name(MonoType *type) {
  return type_names[get_stack_type(type) & TYPE_MASK];
}

/*
 * Verify if @type is valid for the given @ctx verification context.
 * this function checks for VAR and MVAR types that are invalid under the
 * current verifier,
 */
static gboolean mono_type_is_valid_type_in_context_full(
    MonoType *type, MonoGenericContext *context, gboolean check_gtd) {
  int i;
  ...
  case MONO_TYPE_MVAR:
    if (!context)
      return FALSE;
    inst = type->type == MONO_TYPE_VAR ? context->class_inst
                                       : context->method_inst;
  ...
  case MONO_TYPE_SZARRAY:
    return mono_type_is_valid_type_in_context_full(
        m_class_get_byval_arg(type->data.klass), context, check_gtd);
  case MONO_TYPE_ARRAY:
    return mono_type_is_valid_type_in_context_full(
        m_class_get_byval_arg(type->data.array->eklass), context, check_gtd);
  case MONO_TYPE_PTR:
  ...
  case MONO_TYPE_VALUETYPE: {
    ...
    /*
     * It's possible to encode generic'sh types in such a way that they disguise
     * themselves as class or valuetype. Fixing the type decoding is really
     * tricky since under some cases this behavior is needed, for example, to
     * have a 'class' type pointing to a 'genericinst' class.
     *
     * For the runtime these non canonical (weird) encodings work fine, the
     * worst they can cause is some reflection oddities which are harmless  - to
     * security at least.
     */
    if (klass_byval_arg->type != type->type)
      return mono_type_is_valid_type_in_context_full(klass_byval_arg, context,
                                                     check_gtd);
...
static gboolean
mono_type_is_valid_type_in_context(MonoType *type,
                                   MonoGenericContext *context) {
...
static MonoType *verifier_inflate_type(VerifyContext *ctx, MonoType *type,
                                       MonoGenericContext *context) {
  ERROR_DECL(error);
  MonoType *result;
 ...
 *
 * This means that \p candidate constraints are a super set of \p target
 * constaints
 */
static gboolean mono_generic_param_is_constraint_compatible(
    VerifyContext *ctx, MonoGenericParam *target, MonoGenericParam *candidate,
    MonoClass *candidate_param_class, MonoGenericContext *context) {
  ...
  int tmask =
      tinfo->flags & GENERIC_PARAMETER_ATTRIBUTE_SPECIAL_CONSTRAINTS_MASK;
  int cmask =
      cinfo->flags & GENERIC_PARAMETER_ATTRIBUTE_SPECIAL_CONSTRAINTS_MASK;

  if (cinfo->constraints) {
    for (candidate_class = cinfo->constraints; *candidate_class;
         ++candidate_class) {
      MonoClass *cc;
      MonoType *inflated = verifier_inflate_type(
          ctx, m_class_get_byval_arg(*candidate_class), ctx->generic_context);
      if (!inflated)
        return FALSE;
      cc = mono_class_from_mono_type_internal(inflated);
      mono_metadata_free_type(inflated);

      if (mono_type_is_reference(m_class_get_byval_arg(cc)) &&
          !MONO_CLASS_IS_INTERFACE_INTERNAL(cc))
        class_constraint_satisfied = TRUE;
      else if (!mono_type_is_reference(m_class_get_byval_arg(cc)) &&
               !MONO_CLASS_IS_INTERFACE_INTERNAL(cc))
        valuetype_constraint_satisfied = TRUE;
    }
  }
  class_constraint_satisfied |=
      (cmask & GENERIC_PARAMETER_ATTRIBUTE_REFERENCE_TYPE_CONSTRAINT) != 0;
  valuetype_constraint_satisfied |=
      (cmask & GENERIC_PARAMETER_ATTRIBUTE_VALUE_TYPE_CONSTRAINT) != 0;

  if ((tmask & GENERIC_PARAMETER_ATTRIBUTE_REFERENCE_TYPE_CONSTRAINT) &&
      !class_constraint_satisfied)
    return FALSE;
  if ((tmask & GENERIC_PARAMETER_ATTRIBUTE_VALUE_TYPE_CONSTRAINT) &&
      !valuetype_constraint_satisfied)
    return FALSE;
  if ((tmask & GENERIC_PARAMETER_ATTRIBUTE_CONSTRUCTOR_CONSTRAINT) &&
      !((cmask & GENERIC_PARAMETER_ATTRIBUTE_CONSTRUCTOR_CONSTRAINT) ||
        valuetype_constraint_satisfied)) {
    return FALSE;
  }
  ...
  if (tinfo->constraints) {
    ...
    for (target_class = tinfo->constraints; *target_class; ++target_class) {
      MonoClass *tc;
      MonoType *inflated = verifier_inflate_type(
          ctx, m_class_get_byval_arg(*target_class), context);
      if (!inflated)
        return FALSE;
      ...
      /*
       * A constraint from @target might inflate into @candidate itself and in
       * that case we don't need check it's constraints since it satisfy the
       * constraint by itself.
       */
      if (mono_metadata_type_equal(
      ...
      for (candidate_class = cinfo->constraints; *candidate_class;
           ++candidate_class) {
        MonoClass *cc;
        inflated = verifier_inflate_type(
            ctx, m_class_get_byval_arg(*candidate_class), ctx->generic_context);
        if (!inflated)
          return FALSE;
        cc = mono_class_from_mono_type_internal(inflated);
         ...
         * 	...
         * 	Bar<T> <- T here satisfy K constraint transitively through to
         * U's constraint
         *
         */
        if (mono_type_is_generic_argument(m_class_get_byval_arg(cc))) {
          MonoGenericParam *other_candidate =
              verifier_get_generic_param_from_type(ctx,
                                                   m_class_get_byval_arg(cc));

          if (mono_generic_param_is_constraint_compatible(
                  ctx, target, other_candidate, cc, context)) {
            break;
          }
        }
      }
 ...
 * Verify if @type is valid for the given @ctx verification context.
 * this function checks for VAR and MVAR types that are invalid under the
 * current verifier, This means that it either
 */
static gboolean is_valid_type_in_context(VerifyContext *ctx, MonoType *type) {
  return mono_type_is_valid_type_in_context(type, ctx->generic_context);
}

static gboolean is_valid_generic_instantiation_in_context(
    VerifyContext *ctx, MonoGenericInst *ginst, gboolean check_gtd) {
  int i;
  for (i = 0; i < ginst->type_argc; ++i) {
    MonoType *type = ginst->type_argv[i];

    if (!mono_type_is_valid_type_in_context_full(type, ctx->generic_context,
                                                 TRUE))
      return FALSE;
  }
  return TRUE;
...
static gboolean generic_arguments_respect_constraints(
    VerifyContext *ctx, MonoGenericContainer *gc, MonoGenericContext *context,
    MonoGenericInst *ginst) {
  int i;
  for (i = 0; i < ginst->type_argc; ++i) {
    MonoType *type = ginst->type_argv[i];
    ...
    if (!mono_generic_param_is_constraint_compatible(ctx, target, candidate,
...
static gboolean mono_method_repect_method_constraints(VerifyContext *ctx,
                                                      MonoMethod *method) {
  MonoMethodInflated *gmethod = (MonoMethodInflated *)method;
  MonoGenericInst *ginst = gmethod->context.method_inst;
  MonoGenericContainer *gc =
      mono_method_get_generic_container(gmethod->declaring);
  return !gc || generic_arguments_respect_constraints(ctx, gc,
                                                      &gmethod->context, ginst);
}

static gboolean mono_class_repect_method_constraints(VerifyContext *ctx,
                                                     MonoClass *klass) {
  MonoGenericClass *gklass = mono_class_get_generic_class(klass);
  MonoGenericInst *ginst = gklass->context.class_inst;
  MonoGenericContainer *gc =
      mono_class_get_generic_container(gklass->container_class);
  return !gc || generic_arguments_respect_constraints(ctx, gc, &gklass->context,
                                                      ginst);
}
...
static gboolean mono_type_is_valid_in_context(VerifyContext *ctx,
                                              MonoType *type) {
  MonoClass *klass;

  if (type == NULL) {
    ADD_VERIFY_ERROR2(
        ctx, g_strdup_printf("Invalid null type at 0x%04x", ctx->ip_offset),
  ...
  if (!is_valid_type_in_context(ctx, type)) {
    ...
    ADD_VERIFY_ERROR2(
        ctx,
        g_strdup_printf(
            "Invalid generic type (%s%s) (argument out of range or %s is not "
            "generic) at 0x%04x",
            str[0] == '!' ? "" : type->type == MONO_TYPE_VAR ? "!" : "!!", str,
            type->type == MONO_TYPE_VAR ? "class" : "method", ctx->ip_offset),
        MONO_EXCEPTION_BAD_IMAGE);
    g_free(str);
    return FALSE;
  }
  ...
  if (mono_class_has_failure(klass)) {
    if (mono_class_is_ginst(klass) &&
        !mono_class_is_valid_generic_instantiation(NULL, klass))
      ADD_VERIFY_ERROR2(
          ctx,
          g_strdup_printf(
              "Invalid generic instantiation of type %s.%s at 0x%04x",
              m_class_get_name_space(klass), m_class_get_name(klass),
              ctx->ip_offset),
          MONO_EXCEPTION_TYPE_LOAD);
    else
      ADD_VERIFY_ERROR2(ctx,
                        g_strdup_printf("Could not load type %s.%s at 0x%04x",
  ...
  if (mono_class_is_ginst(klass) &&
    ...
    ADD_VERIFY_ERROR2(ctx,
                      g_strdup_printf("Could not load type %s.%s at 0x%04x",
                                      m_class_get_name_space(klass),
                                      m_class_get_name(klass), ctx->ip_offset),
                      MONO_EXCEPTION_TYPE_LOAD);
    return FALSE;
  }
  ...
  if (!mono_class_is_valid_generic_instantiation(ctx, klass)) {
    ADD_VERIFY_ERROR2(
        ctx,
        g_strdup_printf(
            "Invalid generic type instantiation of type %s.%s at 0x%04x",
            m_class_get_name_space(klass), m_class_get_name(klass),
            ctx->ip_offset),
        MONO_EXCEPTION_TYPE_LOAD);
    return FALSE;
  ...
  if (!mono_class_repect_method_constraints(ctx, klass)) {
    ADD_VERIFY_ERROR2(
        ctx,
        g_strdup_printf(
            "Invalid generic type instantiation of type %s.%s (generic args "
            "don't respect target's constraints) at 0x%04x",
            m_class_get_name_space(klass), m_class_get_name(klass),
            ctx->ip_offset),
        MONO_EXCEPTION_TYPE_LOAD);
    return FALSE;
...
static verify_result_t mono_method_is_valid_in_context(VerifyContext *ctx,
                                                       MonoMethod *method) {
  ...
  if (!mono_method_is_valid_generic_instantiation(ctx, method)) {
    ADD_VERIFY_ERROR2(ctx,
                      g_strdup_printf("Invalid generic method instantiation of "
                                      "method %s.%s::%s at 0x%04x",
                                      m_class_get_name_space(method->klass),
                                      m_class_get_name(method->klass),
                                      method->name, ctx->ip_offset),
                      MONO_EXCEPTION_UNVERIFIABLE_IL);
    return RESULT_INVALID;
  }
  ...
  if (!mono_method_repect_method_constraints(ctx, method)) {
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf(
            "Invalid generic method instantiation of method %s.%s::%s (generic "
            "args don't respect target's constraints) at 0x%04x",
            m_class_get_name_space(method->klass),
            m_class_get_name(method->klass), method->name, ctx->ip_offset));
    return RESULT_UNVERIFIABLE;
  }
  return RESULT_VALID;
}

static MonoClassField *verifier_load_field(VerifyContext *ctx, int token,
                                           MonoClass **out_klass,
                                           const char *opcode) {
  ERROR_DECL(error);
  ...
  if (ctx->method->wrapper_type != MONO_WRAPPER_NONE) {
    field = (MonoClassField *)mono_method_get_wrapper_data(ctx->method,
                                                           (guint32)token);
    ...
    if (!IS_FIELD_DEF_OR_REF(token) || !token_bounds_check(ctx->image, token)) {
      ADD_VERIFY_ERROR2(
          ...
          g_strdup_printf("Invalid field token 0x%08x for %s at 0x%04x", token,
                          opcode, ctx->ip_offset),
          MONO_EXCEPTION_BAD_IMAGE);
      return NULL;
    }

    field = mono_field_from_token_checked(ctx->image, token, &klass,
                                          ctx->generic_context, error);
  ...
  if (!field || !field->parent || !klass) {
    ADD_VERIFY_ERROR2(
        ctx,
        g_strdup_printf("Cannot load field from token 0x%08x for %s at 0x%04x",
                        token, opcode, ctx->ip_offset),
        MONO_EXCEPTION_BAD_IMAGE);
    return NULL;
  }
  ...
  if (mono_field_get_flags(field) & FIELD_ATTRIBUTE_LITERAL) {
    ...
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Cannot reference literal field %s::%s at 0x%04x",
                             type_name, field->name, ctx->ip_offset));
    g_free(type_name);
    return NULL;
  }
...
static MonoMethod *verifier_load_method(VerifyContext *ctx, int token,
                                        const char *opcode) {
  MonoMethod *method;

  if (ctx->method->wrapper_type != MONO_WRAPPER_NONE) {
    method =
        (MonoMethod *)mono_method_get_wrapper_data(ctx->method, (guint32)token);
  } else {
    ERROR_DECL(error);
    if (!IS_METHOD_DEF_OR_REF_OR_SPEC(token) ||
        !token_bounds_check(ctx->image, token)) {
      ADD_VERIFY_ERROR2(
          ctx,
    ...
    method = mono_get_method_checked(ctx->image, token, NULL,
                                     ctx->generic_context, error);
  ...
  if (!method) {
    ADD_VERIFY_ERROR2(
        ctx,
        g_strdup_printf("Cannot load method from token 0x%08x for %s at 0x%04x",
                        token, opcode, ctx->ip_offset),
        MONO_EXCEPTION_BAD_IMAGE);
    return NULL;
  }
...
static MonoType *verifier_load_type(VerifyContext *ctx, int token,
                                    const char *opcode) {
  MonoType *type;

  if (ctx->method->wrapper_type != MONO_WRAPPER_NONE) {
    MonoClass *klass =
        (MonoClass *)mono_method_get_wrapper_data(ctx->method, (guint32)token);
    type = klass ? m_class_get_byval_arg(klass) : NULL;
  } else {
    ERROR_DECL(error);
    if (!IS_TYPE_DEF_OR_REF_OR_SPEC(token) ||
        !token_bounds_check(ctx->image, token)) {
      ADD_VERIFY_ERROR2(ctx,
                        g_strdup_printf("Invalid type token 0x%08x at 0x%04x",
                                        token, ctx->ip_offset),
                        MONO_EXCEPTION_BAD_IMAGE);
      return NULL;
    }
    type =
        mono_type_get_checked(ctx->image, token, ctx->generic_context, error);
    mono_error_cleanup(error); /*FIXME don't swallow the error */
  }
  ...
  if (!type) {
    ADD_VERIFY_ERROR2(
        ctx,
        g_strdup_printf("Cannot load type from token 0x%08x for %s at 0x%04x",
                        token, opcode, ctx->ip_offset),
        MONO_EXCEPTION_BAD_IMAGE);
    return NULL;
  }
 ...
 */
static gint32 stack_slot_get_type(ILStackDesc *value) {
 ...
 *
 * Use this function is cases where the fact that the value could be a managed
 * pointer is irrelevant. For example, field load doesn't care about this fact
 * of type on stack.
 */
static gint32 stack_slot_get_underlying_type(ILStackDesc *value) {
 ...
 */
static gboolean stack_slot_is_managed_pointer(ILStackDesc *value) {
 ...
 */
static gboolean stack_slot_is_null_literal(ILStackDesc *value) {
 ...
 */
static gboolean stack_slot_is_this_pointer(ILStackDesc *value) {
 ...
 */
static gboolean stack_slot_is_boxed_value(ILStackDesc *value) {
 ...
 */
static gboolean stack_slot_is_safe_byref(ILStackDesc *value) {
  return (value->stype & SAFE_BYREF_MASK) == SAFE_BYREF_MASK;
}

static const char *stack_slot_get_name(ILStackDesc *value) {
  return type_names[value->stype & TYPE_MASK];
}

enum { SAFE_BYREF_LOCAL = 1, UNSAFE_BYREF_LOCAL = 2 };
static gboolean local_is_safe_byref(VerifyContext *ctx, unsigned int arg) {
  return ctx->locals_verification_state[arg] == SAFE_BYREF_LOCAL;
}

static gboolean local_is_unsafe_byref(VerifyContext *ctx, unsigned int arg) {
  return ctx->locals_verification_state[arg] == UNSAFE_BYREF_LOCAL;
}

#define APPEND_WITH_PREDICATE(PRED, NAME)                                      \
  do {                                                                         \
    if (PRED(value)) {                                                         \
      if (!first)                                                              \
        g_string_append(str, ", ");                                            \
...
static char *stack_slot_stack_type_full_name(ILStackDesc *value) {
  ...
  if (mono_type_is_generic_argument(value->type) &&
...
static char *stack_slot_full_name(ILStackDesc *value) {
...
#define CHECK_STACK_UNDERFLOW(num)                                             \
  do {                                                                         \
    if (cur_stack < (num))                                                     \
      ADD_INVALID(list,                                                        \
                  g_strdup_printf(                                             \
                      "Stack underflow at 0x%04x (%d items instead of %d)",    \
                      ip_offset, cur_stack, (num)));                           \
  } while (0)
...
#define CHECK_STACK_OVERFLOW()                                                 \
  do {                                                                         \
    if (cur_stack >= max_stack)                                                \
      ADD_INVALID(list,                                                        \
                  g_strdup_printf("Maxstack exceeded at 0x%04x", ip_offset));  \
  } while (0)

static int in_any_block(MonoMethodHeader *header, guint offset) {
  int i;
 ...
 *
 * Returns TRUE is @offset is part of any exception clause (filter, handler,
 * catch, finally or fault).
 */
static gboolean in_any_exception_block(MonoMethodHeader *header, guint offset) {
  int i;
  MonoExceptionClause *clause;
 ...
 */
static int is_valid_branch_instruction(MonoMethodHeader *header, guint offset,
                                       guint target) {
  int i;
  MonoExceptionClause *clause;
  ...
  for (i = 0; i < header->num_clauses; ++i) {
    ...
    if (clause->try_offset != target && (MONO_OFFSET_IN_CLAUSE(clause, offset) ^
    ...
    if (MONO_OFFSET_IN_HANDLER(clause, offset) ^
      ...
      return 1;
    if (MONO_OFFSET_IN_FILTER(clause, offset) ^
        MONO_OFFSET_IN_FILTER(clause, target))
      return 1;
  }
  return 0;
}
 ...
 *
 * Verify if it's valid to perform a branch from @offset to @target.
 * This should be used with binary comparison branching instruction, like beq,
 * bge and similars. It returns 0 if valid, 1 for unverifiable and 2 for
 * invalid.
 *
 * The major differences from other similar functions are that most errors lead
 * to invalid code and only branching out of finally, filter or fault clauses is
 * unverifiable.
 */
static int is_valid_cmp_branch_instruction(MonoMethodHeader *header,
                                           guint offset, guint target) {
  int i;
  MonoExceptionClause *clause;
 ...
 */
static int is_correct_leave(MonoMethodHeader *header, guint offset,
                            guint target) {
  int i;
  MonoExceptionClause *clause;
 ...
 */
static int is_correct_rethrow(MonoMethodHeader *header, guint offset) {
  int i;
  MonoExceptionClause *clause;
 ...
 */
static int is_correct_endfinally(MonoMethodHeader *header, guint offset) {
  int i;
  MonoExceptionClause *clause;
 ...
 */
static MonoExceptionClause *is_correct_endfilter(VerifyContext *ctx,
                                                 guint offset) {
  int i;
  MonoExceptionClause *clause;
 ...
 */
static int is_unverifiable_endfilter(VerifyContext *ctx, guint offset) {
  int i;
  MonoExceptionClause *clause;
...
static gboolean is_valid_bool_arg(ILStackDesc *arg) {
  if (stack_slot_is_managed_pointer(arg) || stack_slot_is_boxed_value(arg) ||
  ...
  case TYPE_COMPLEX:
    ...
    case MONO_TYPE_GENERICINST:
       ...
       */
      return !m_class_is_valuetype(
          arg->type->data.generic_class->container_class);
    default:
      return FALSE;
    }
  default:
...
static MonoType *mono_type_get_type_byref(MonoType *type) {
  if (type->byref)
    return type;
...
static MonoType *mono_type_from_stack_slot(ILStackDesc *slot) {
...
static void ensure_stack_size(ILCodeDesc *stack, int required) {
  int new_size = 8;
  ILStackDesc *tmp;
  ...
  /* We don't have to worry about the exponential growth since stack_copy prune
   * unused space */
...
static int check_underflow(VerifyContext *ctx, int size) {
  if (ctx->eval.size < size) {
    ADD_VERIFY_ERROR(
        ctx,
        g_strdup_printf("Stack underflow, required %d, but have %d at 0x%04x",
                        size, ctx->eval.size, ctx->ip_offset));
    return 0;
  }
  return 1;
}
...
static int check_overflow(VerifyContext *ctx) {
  if (ctx->eval.size >= ctx->max_stack) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Method doesn't have stack-depth %d at 0x%04x",
                             ctx->eval.size + 1, ctx->ip_offset));
    return 0;
  }
  return 1;
}
...
static gboolean check_unmanaged_pointer(VerifyContext *ctx,
                                        ILStackDesc *value) {
  if (stack_slot_get_type(value) == TYPE_PTR) {
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Unmanaged pointer is not a verifiable type at 0x%04x",
                        ctx->ip_offset));
    return 0;
  }
  return 1;
...
static gboolean check_unverifiable_type(VerifyContext *ctx, MonoType *type) {
...
static ILStackDesc *stack_push_val(VerifyContext *ctx, int stype,
                                   MonoType *type) {
...
static ILStackDesc *stack_pop(VerifyContext *ctx) {
  ...
  if ((ret->stype & UNINIT_THIS_MASK) == UNINIT_THIS_MASK)
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Found use of uninitialized 'this ptr' ref at 0x%04x",
                        ctx->ip_offset));
  return ret;
}
 ...
 */
static ILStackDesc *stack_pop_safe(VerifyContext *ctx) {
  g_assert(ctx->eval.size > 0);
  return ctx->eval.stack + --ctx->eval.size;
}

/*Positive number distance from stack top. [0] is stack top, [1] is the one
 * below*/
static ILStackDesc *stack_peek(VerifyContext *ctx, int distance) {
  g_assert(ctx->eval.size - distance > 0);
  return ctx->eval.stack + (ctx->eval.size - 1 - distance);
}

static ILStackDesc *stack_push_stack_val(VerifyContext *ctx,
                                         ILStackDesc *value) {
  ILStackDesc *top = stack_push(ctx);
 ...
 *
 * A boxable type can be either a reference or value type, but cannot be a byref
 * type or an unmanaged pointer
 * */
static MonoType *get_boxable_mono_type(VerifyContext *ctx, int token,
                                       const char *opcode) {
  MonoType *type;
  MonoClass *klass;
  ...
  if (type->byref && type->type != MONO_TYPE_TYPEDBYREF) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Invalid use of byref type for %s at 0x%04x",
                             opcode, ctx->ip_offset));
    return NULL;
  }
  ...
  if (type->type == MONO_TYPE_VOID) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Invalid use of void type for %s at 0x%04x",
                             opcode, ctx->ip_offset));
    return NULL;
  }
  ...
  if (type->type == MONO_TYPE_TYPEDBYREF)
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid use of typedbyref for %s at 0x%04x",
                             opcode, ctx->ip_offset));

  if (!(klass = mono_class_from_mono_type_internal(type)))
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Could not retrieve type token for %s at 0x%04x",
                             opcode, ctx->ip_offset));

  if (mono_class_is_gtd(klass) && type->type != MONO_TYPE_GENERICINST)
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Cannot use the generic type definition in a "
                             "boxable type position for %s at 0x%04x",
                             opcode, ctx->ip_offset));
...
static const unsigned char add_table[TYPE_MAX][TYPE_MAX] = {
    {TYPE_I4, TYPE_INV, TYPE_NATIVE_INT, TYPE_INV,
     TYPE_PTR | NON_VERIFIABLE_RESULT, TYPE_INV},
    {TYPE_INV, TYPE_I8, TYPE_INV, TYPE_INV, TYPE_INV, TYPE_INV},
    {TYPE_NATIVE_INT, TYPE_INV, TYPE_NATIVE_INT, TYPE_INV,
     TYPE_PTR | NON_VERIFIABLE_RESULT, TYPE_INV},
    {TYPE_INV, TYPE_INV, TYPE_INV, TYPE_R8, TYPE_INV, TYPE_INV},
    {TYPE_PTR | NON_VERIFIABLE_RESULT, TYPE_INV,
     TYPE_PTR | NON_VERIFIABLE_RESULT, TYPE_INV, TYPE_INV, TYPE_INV},
    {TYPE_INV, TYPE_INV, TYPE_INV, TYPE_INV, TYPE_INV, TYPE_INV},
};
...
static const unsigned char sub_table[TYPE_MAX][TYPE_MAX] = {
    ...
    {TYPE_PTR | NON_VERIFIABLE_RESULT, TYPE_INV,
     TYPE_PTR | NON_VERIFIABLE_RESULT, TYPE_INV,
     TYPE_NATIVE_INT | NON_VERIFIABLE_RESULT, TYPE_INV},
    {TYPE_INV, TYPE_INV, TYPE_INV, TYPE_INV, TYPE_INV, TYPE_INV},
};
...
static const unsigned char cmp_br_eq_op[TYPE_MAX][TYPE_MAX] = {
    {TYPE_I4, TYPE_INV, TYPE_I4, TYPE_INV, TYPE_INV, TYPE_INV},
    {TYPE_INV, TYPE_I4, TYPE_INV, TYPE_INV, TYPE_INV, TYPE_INV},
    {TYPE_I4, TYPE_INV, TYPE_I4, TYPE_INV, TYPE_I4 | NON_VERIFIABLE_RESULT,
     TYPE_INV},
    {TYPE_INV, TYPE_INV, TYPE_INV, TYPE_I4, TYPE_INV, TYPE_INV},
    {TYPE_INV, TYPE_INV, TYPE_I4 | NON_VERIFIABLE_RESULT, TYPE_INV, TYPE_I4,
     TYPE_INV},
...
static void dump_stack_value(ILStackDesc *value) {
  ...
  case TYPE_COMPLEX:
    ...
    case MONO_TYPE_SZARRAY:
      ...
      return;
    case MONO_TYPE_ARRAY:
      printf("complex] (%s [%d %d %d])", value->type->data.array->eklass->name,
             value->type->data.array->rank, value->type->data.array->numsizes,
             value->type->data.array->numlobounds);
      return;
    case MONO_TYPE_GENERICINST:
      printf("complex] (inst of %s )",
             value->type->data.generic_class->container_class->name);
      return;
    case MONO_TYPE_VAR:
      printf("complex] (type generic param !%d - %s) ",
             value->type->data.generic_param->num,
             mono_generic_param_name(value->type->data.generic_param));
      return;
    case MONO_TYPE_MVAR:
      printf("complex] (method generic param !!%d - %s) ",
             value->type->data.generic_param->num,
             mono_generic_param_name(value->type->data.generic_param));
      return;
    default: {
 ...
 *
 * Both parameters MUST be of type MONO_TYPE_ARRAY (target->type ==
 * MONO_TYPE_ARRAY)
 */
static gboolean is_array_type_compatible(MonoType *target,
                                         MonoType *candidate) {
...
handle_enum:
  ...
  /* FIXME: the spec says that you cannot have a pointer to method pointer, do
...
static gboolean set_stack_value(VerifyContext *ctx, ILStackDesc *stack,
                                MonoType *type, int take_addr) {
  int mask = 0;
  int type_kind = type->type;
...
handle_enum:
  ...
  /*FIXME: Do we need to check if it's a pointer to the method pointer? The spec
   * says it' illegal to have that.*/
  case MONO_TYPE_FNPTR:
  case MONO_TYPE_PTR:
  case MONO_TYPE_TYPEDBYREF:
  ...
  case MONO_TYPE_VALUETYPE:
    if (mono_type_is_enum_type(type)) {
      ...
      if (!utype) {
        ADD_VERIFY_ERROR(
            ctx,
            g_strdup_printf("Could not resolve underlying type of %x at %d",
                            type->type, ctx->ip_offset));
        return FALSE;
      }
      type = utype;
      type_kind = type->type;
  ...
  default:
    VERIFIER_DEBUG(
        printf("unknown type 0x%02x in eval stack type\n", type->type););
    ADD_VERIFY_ERROR(ctx,
                     g_strdup_printf("Illegal value set on stack 0x%02x at %d",
                                     type->type, ctx->ip_offset));
    return FALSE;
  }
  return TRUE;
 ...
 */
static void init_stack_with_value_at_exception_boundary(VerifyContext *ctx,
                                                        ILCodeDesc *code,
                                                        MonoClass *klass) {
  ERROR_DECL(error);
  MonoType *type = mono_class_inflate_generic_type_checked(
      m_class_get_byval_arg(klass), ctx->generic_context, error);
  ...
  if (!ctx->max_stack) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Stack overflow at 0x%04x", ctx->ip_offset));
    return;
  }
...
static GENERATE_GET_CLASS_WITH_CACHE(ienumerable, "System.Collections.Generic", "IEnumerable`1") static GENERATE_GET_CLASS_WITH_CACHE(
    icollection, "System.Collections.Generic",
    "ICollection`1") static GENERATE_GET_CLASS_WITH_CACHE(ireadonly_list,
                                                          "System.Collections."
                                                          "Generic",
                                                          "IReadOnlyList`"
                                                          "1") static GENERATE_GET_CLASS_WITH_CACHE(ireadonly_collection,
                                                                                                    "System.Collections.Generic",
                                                                                                    "IReadOnlyCollection`1")

    static MonoClass *get_ienumerable_class(void) {
  return mono_class_get_ienumerable_class();
}

static MonoClass *get_icollection_class(void) {
  return mono_class_get_icollection_class();
}

static MonoClass *get_ireadonlylist_class(void) {
  return mono_class_get_ireadonly_list_class();
}

static MonoClass *get_ireadonlycollection_class(void) {
  return mono_class_get_ireadonly_collection_class();
}

static MonoClass *inflate_class_one_arg(MonoClass *gtype, MonoClass *arg0) {
  MonoType *args[1];
...
static gboolean verifier_inflate_and_check_compat(MonoClass *target,
                                                  MonoClass *gtd,
                                                  MonoClass *arg) {
  MonoClass *tmp;
...
static gboolean verifier_class_is_assignable_from(MonoClass *target,
  ...
  if (mono_class_has_variant_generic_params(target)) {
    if (MONO_CLASS_IS_INTERFACE_INTERNAL(target)) {
      if (MONO_CLASS_IS_INTERFACE_INTERNAL(candidate) &&
          mono_class_is_variant_compatible(target, candidate, TRUE))
        return TRUE;

      if (m_class_get_rank(candidate) == 1) {
        MonoClass *candidate_element_class =
            m_class_get_element_class(candidate);
        if (verifier_inflate_and_check_compat(target,
                                              mono_defaults.generic_ilist_class,
                                              candidate_element_class))
          return TRUE;
        if (verifier_inflate_and_check_compat(target, get_icollection_class(),
                                              candidate_element_class))
          return TRUE;
        if (verifier_inflate_and_check_compat(target, get_ienumerable_class(),
                                              candidate_element_class))
          return TRUE;
        if (verifier_inflate_and_check_compat(target, get_ireadonlylist_class(),
                                              candidate_element_class))
          return TRUE;
        if (verifier_inflate_and_check_compat(target,
        ...
        while (candidate && candidate != mono_defaults.object_class) {
          ...
          /*klass is a generic variant interface, We need to extract from oklass
           * a list of ifaces which are viable candidates.*/
          guint16 candidate_interface_offsets_count =
              m_class_get_interface_offsets_count(candidate);
          MonoClass **candidate_interfaces_packed =
          ...
          guint16 candidate_interface_count =
  ...
  if (!MONO_CLASS_IS_INTERFACE_INTERNAL(target) ||
      !mono_class_is_ginst(target) || m_class_get_rank(candidate) != 1)
    return FALSE;

  iface_gtd = mono_class_get_generic_class(target)->container_class;
  if (iface_gtd != mono_defaults.generic_ilist_class &&
      iface_gtd != get_icollection_class() &&
      iface_gtd != get_ienumerable_class())
    return FALSE;

  target = mono_class_from_mono_type_internal(
      mono_class_get_generic_class(target)->context.class_inst->type_argv[0]);
 ...
 */
static gboolean verify_type_compatibility_full(VerifyContext *ctx,
                                               MonoType *target,
                                               MonoType *candidate,
                                               gboolean strict) {
...
#define IS_ONE_OF2(T, A, B) (T == A || T == B)
  ...
  VERIFIER_DEBUG(printf("checking type compatibility %s x %s strict %d\n",
                        ...
                        mono_type_full_name(candidate), strict););
  ...
  if (candidate->byref ^ target->byref) {
    ...
    if (get_stack_type(candidate) == TYPE_NATIVE_INT && target->byref) {
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf("using byref native int at 0x%04x", ctx->ip_offset));
      return TRUE;
    }
    return FALSE;
  }
  strict |= target->byref;
  /*From now on we don't care about byref anymore, so it's ok to discard it
   * here*/
...
handle_enum:
  ...
  case MONO_TYPE_BOOLEAN:
    if (strict)
      return IS_ONE_OF3(candidate->type, MONO_TYPE_I1, MONO_TYPE_U1,
                        MONO_TYPE_BOOLEAN);
  case MONO_TYPE_I2:
  case MONO_TYPE_U2:
  case MONO_TYPE_CHAR:
    if (strict)
      return IS_ONE_OF3(candidate->type, MONO_TYPE_I2, MONO_TYPE_U2,
                        MONO_TYPE_CHAR);
  case MONO_TYPE_I4:
  case MONO_TYPE_U4: {
    gboolean is_native_int =
        IS_ONE_OF2(candidate->type, MONO_TYPE_I, MONO_TYPE_U);
  ...
  case MONO_TYPE_PTR:
    ...
    return verify_type_compatibility_full(ctx, target->data.type,
                                          candidate->data.type, TRUE);
  ...
  case MONO_TYPE_FNPTR: {
    ...
    return mono_metadata_signature_equal(left, right) &&
           left->call_convention == right->call_convention;
  }
  ...
  case MONO_TYPE_GENERICINST: {
    ...
    /*
     * VAR / MVAR compatibility must be checked by
     * verify_stack_type_compatibility to take boxing status into account.
     */
  ...
  case MONO_TYPE_CLASS:
    ...
    /* If candidate is an enum it should return true for System.Enum and
     * supertypes. That's why here we use the original type and not the
     * underlying type.
     */
    return verifier_class_is_assignable_from(
        target->data.klass,
  ...
  case MONO_TYPE_VAR:
    if (candidate->type != MONO_TYPE_VAR)
      return FALSE;
    return mono_type_get_generic_param_num(candidate) ==
...
static gboolean verify_type_compatibility(VerifyContext *ctx, MonoType *target,
                                          MonoType *candidate) {
 ...
 *
 */
static MonoGenericParam *get_generic_param(VerifyContext *ctx,
                                           MonoType *param) {
  guint16 param_num = mono_type_get_generic_param_num(param);
  if (param->type == MONO_TYPE_VAR) {
    if (!ctx->generic_context->class_inst ||
        ctx->generic_context->class_inst->type_argc <= param_num) {
      ADD_VERIFY_ERROR(
          ctx, g_strdup_printf("Invalid generic type argument %d", param_num));
      return NULL;
    }
    return ctx->generic_context->class_inst->type_argv[param_num]
        ->data.generic_param;
  }

  /*param must be a MVAR */
  if (!ctx->generic_context->method_inst ||
      ctx->generic_context->method_inst->type_argc <= param_num) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Invalid generic method argument %d", param_num));
    return NULL;
  }
  return ctx->generic_context->method_inst->type_argv[param_num]
      ->data.generic_param;
}

static gboolean
recursive_boxed_constraint_type_check(VerifyContext *ctx, MonoType *type,
                                      MonoClass *constraint_class,
                                      int recursion_level) {
  MonoType *constraint_type = m_class_get_byval_arg(constraint_class);
  if (recursion_level <= 0)
    return FALSE;

  if (verify_type_compatibility_full(
          ctx, type, mono_type_get_type_byval(constraint_type), FALSE))
    return TRUE;
  ...
  if (mono_type_is_generic_argument(constraint_type)) {
    ...
    if (!param)
      return FALSE;
    for (klass = mono_generic_param_info(param)->constraints; klass && *klass;
         ++klass) {
      if (recursive_boxed_constraint_type_check(ctx, type, *klass,
                                                recursion_level - 1))
        return TRUE;
    }
  }
 ...
 * @stack stack slot of the boxed valuetype, separate from @candidade since one
 * could be changed before calling this function
 * @strict if TRUE candidate must be boxed compatible to the target type
 *
 */
static gboolean is_compatible_boxed_valuetype(VerifyContext *ctx,
                                              MonoType *type,
                                              MonoType *candidate,
                                              ILStackDesc *stack,
                                              gboolean strict) {
  ...
  if (mono_type_is_generic_argument(candidate)) {
    ...
    for (klass = mono_generic_param_info(param)->constraints; klass && *klass;
         ++klass) {
      /*256 should be enough since there can't be more than 255 generic
       * arguments.*/
...
static int
verify_stack_type_compatibility_full(VerifyContext *ctx, MonoType *type,
                                     ILStackDesc *stack, gboolean drop_byref,
                                     gboolean valuetype_must_be_boxed) {
  MonoType *candidate = mono_type_from_stack_slot(stack);
  if (MONO_TYPE_IS_REFERENCE(type) && !type->byref &&
  ...
  if (drop_byref)
    return verify_type_compatibility_full(
        ctx, type, mono_type_get_type_byval(candidate), FALSE);

  /* Handle how Roslyn emit fixed statements by encoding it as byref */
  if (type->byref && candidate->byref && (type->type == MONO_TYPE_I) &&
      !mono_type_is_reference(candidate)) {
...
static int verify_stack_type_compatibility(VerifyContext *ctx, MonoType *type,
                                           ILStackDesc *stack) {
  return verify_stack_type_compatibility_full(ctx, type, stack, FALSE, FALSE);
}

static gboolean mono_delegate_type_equal(MonoType *target,
                                         MonoType *candidate) {
  if (candidate->byref ^ target->byref)
    return FALSE;
  ...
  case MONO_TYPE_FNPTR:
    if (candidate->type != MONO_TYPE_FNPTR)
      return FALSE;
    return mono_delegate_signature_equal(
        mono_type_get_signature_internal(target),
        mono_type_get_signature_internal(candidate), FALSE);
  ...
  case MONO_TYPE_CLASS:
    return verifier_class_is_assignable_from(
        target->data.klass, mono_class_from_mono_type_internal(candidate));

  case MONO_TYPE_SZARRAY:
    if (candidate->type != MONO_TYPE_SZARRAY)
      return FALSE;
    return verifier_class_is_assignable_from(
        m_class_get_element_class(mono_class_from_mono_type_internal(target)),
  ...
  case MONO_TYPE_VALUETYPE:
    /*FIXME handle nullables and enum*/
    return mono_class_from_mono_type_internal(candidate) ==
           mono_class_from_mono_type_internal(target);

  case MONO_TYPE_VAR:
    return candidate->type == MONO_TYPE_VAR &&
           mono_type_get_generic_param_num(target) ==
               mono_type_get_generic_param_num(candidate);
    return FALSE;

  case MONO_TYPE_MVAR:
    return candidate->type == MONO_TYPE_MVAR &&
           mono_type_get_generic_param_num(target) ==
               mono_type_get_generic_param_num(candidate);
    return FALSE;
...
static gboolean mono_delegate_param_equal(MonoType *delegate,
                                          MonoType *method) {
...
static gboolean mono_delegate_ret_equal(MonoType *delegate, MonoType *method) {
  if (mono_metadata_type_equal_full(delegate, method, TRUE))
    return TRUE;
 ...
 *
 * This function only exists due to the fact that it should ignore the
 * 'has_this' part of the signature.
 *
 * FIXME can this function be eliminated and proper metadata functionality be
 * used?
 */
static gboolean mono_delegate_signature_equal(MonoMethodSignature *delegate_sig,
                                              MonoMethodSignature *method_sig,
...
gboolean mono_verifier_is_signature_compatible(MonoMethodSignature *target,
                                               MonoMethodSignature *candidate) {
 ...
 */
static void verify_ldftn_delegate(VerifyContext *ctx, MonoClass *delegate,
                                  ILStackDesc *value, ILStackDesc *funptr) {
  ...
  if ((method->flags & METHOD_ATTRIBUTE_VIRTUAL) &&
      ...
      !mono_class_is_sealed(method->klass) &&
      !stack_slot_is_boxed_value(value)) {
    /*A stdarg 0 must not happen, we fail here only in fail fast mode to avoid
     * double error reports*/
    if (IS_FAIL_FAST_MODE(ctx) && ctx->has_this_store)
      CODE_NOT_VERIFIABLE(ctx,
                          g_strdup_printf("Invalid ldftn with virtual function "
                                          "in method with stdarg 0 at  0x%04x",
                                          ctx->ip_offset));

    /*current method must not be static*/
    if (ctx->method->flags & METHOD_ATTRIBUTE_STATIC)
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Invalid ldftn with virtual function at 0x%04x",
                               ctx->ip_offset));
    ...
    if (!stack_slot_is_this_pointer(value))
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf("Invalid object argument, it is not the this "
                          "pointer, to ldftn with virtual method at  0x%04x",
                          ctx->ip_offset));
 ...
 *
 */
static void verify_delegate_compatibility(VerifyContext *ctx,
                                          MonoClass *delegate,
                                          ILStackDesc *value,
                                          ILStackDesc *funptr) {
#define IS_VALID_OPCODE(offset, opcode)                                        \
  (ip[ip_offset - offset] == opcode &&                                         \
   (ctx->code[ip_offset - offset].flags & IL_CODE_FLAG_SEEN))
#define IS_LOAD_FUN_PTR(kind)                                                  \
  (IS_VALID_OPCODE(6, CEE_PREFIX1) && ip[ip_offset - 5] == kind)
  ...
  if (stack_slot_get_type(funptr) != TYPE_PTR || !funptr->method) {
    CODE_NOT_VERIFIABLE(ctx,
                        g_strdup_printf("Invalid function pointer parameter "
                                        "for delegate constructor at 0x%04x",
                                        ctx->ip_offset));
    return;
  }
  ...
  if (!method || !mono_method_signature_internal(method)) {
    ...
    ADD_VERIFY_ERROR(ctx, g_strdup_printf("Invalid method on stack to create "
                                          "delegate %s construction at 0x%04x",
                                          name, ctx->ip_offset));
    g_free(name);
    return;
  }
  ...
  if (!invoke || !mono_method_signature_internal(invoke)) {
    ...
    ADD_VERIFY_ERROR(
        ctx,
        g_strdup_printf("Delegate type %s with bad Invoke method at 0x%04x",
                        name, ctx->ip_offset));
    g_free(name);
    return;
  }

  is_static_ldftn = (ip_offset > 5 && IS_LOAD_FUN_PTR(CEE_LDFTN)) &&
                    method->flags & METHOD_ATTRIBUTE_STATIC;

  if (is_static_ldftn)
    is_first_arg_bound =
        mono_method_signature_internal(invoke)->param_count + 1 ==
        mono_method_signature_internal(method)->param_count;

  if (!mono_delegate_signature_equal(mono_method_signature_internal(invoke),
                                     mono_method_signature_internal(method),
                                     is_first_arg_bound)) {
    char *fun_sig =
        mono_signature_get_desc(mono_method_signature_internal(method), FALSE);
    char *invoke_sig =
        mono_signature_get_desc(mono_method_signature_internal(invoke), FALSE);
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Function pointer signature '%s' doesn't match "
                             "delegate's signature '%s' at 0x%04x",
                             fun_sig, invoke_sig, ctx->ip_offset));
    g_free(fun_sig);
    g_free(invoke_sig);
  }
  ...
  } else if (ip_offset > 6 && IS_VALID_OPCODE(7, CEE_DUP) &&
             IS_LOAD_FUN_PTR(CEE_LDVIRTFTN)) {
    ctx->code[ip_offset - 6].flags |= IL_CODE_DELEGATE_SEQUENCE;
  } else {
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Invalid code sequence for delegate creation at 0x%04x",
                        ctx->ip_offset));
  }
  ...
  if (is_first_arg_bound) {
    if (mono_method_signature_internal(method)->param_count == 0 ||
        !verify_stack_type_compatibility_full(
            ctx, mono_method_signature_internal(method)->params[0], value,
            FALSE, TRUE))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("This object not compatible with function "
                               "pointer for delegate creation at 0x%04x",
                               ctx->ip_offset));
  } else {
    if (method->flags & METHOD_ATTRIBUTE_STATIC) {
      if (!stack_slot_is_null_literal(value))
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf("Non-null this args used with static function "
                                 "for delegate creation at 0x%04x",
                                 ctx->ip_offset));
    } else {
      if (!verify_stack_type_compatibility_full(
              ctx, m_class_get_byval_arg(method->klass), value, FALSE, TRUE) &&
  ...
  if (stack_slot_get_type(value) != TYPE_COMPLEX)
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Invalid first parameter for delegate creation at 0x%04x",
                 ctx->ip_offset));
...
static gboolean is_this_arg_of_struct_instance_method(unsigned int arg,
                                                      VerifyContext *ctx) {
  if (arg != 0)
    return FALSE;
  if (ctx->method->flags & METHOD_ATTRIBUTE_STATIC)
...
static void push_arg(VerifyContext *ctx, unsigned int arg, int take_addr) {
  ILStackDesc *top;

  if (arg >= ctx->max_args) {
    if (take_addr)
      ADD_VERIFY_ERROR(
          ctx, g_strdup_printf("Method doesn't have argument %d", arg + 1));
    else {
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Method doesn't have argument %d", arg + 1));
    ...
    /*We must let the value be pushed, otherwise we would get an underflow
     * error*/
    check_unverifiable_type(ctx, ctx->params[arg]);
    if (ctx->params[arg]->byref && take_addr)
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("ByRef of ByRef at 0x%04x", ctx->ip_offset));
    top = stack_push(ctx);
    ...
    if (arg == 0 && !(ctx->method->flags & METHOD_ATTRIBUTE_STATIC)) {
      ...
      if (mono_method_is_constructor(ctx->method) && !ctx->super_ctor_called &&
          !m_class_is_valuetype(ctx->method->klass))
        top->stype |= UNINIT_THIS_MASK;
    }
    if (!take_addr && ctx->params[arg]->byref &&
        !is_this_arg_of_struct_instance_method(arg, ctx))
      top->stype |= SAFE_BYREF_MASK;
  }
}

static void push_local(VerifyContext *ctx, guint32 arg, int take_addr) {
  if (arg >= ctx->num_locals) {
    ADD_VERIFY_ERROR(ctx,
                     g_strdup_printf("Method doesn't have local %d", arg + 1));
  } else if (check_overflow(ctx)) {
    /*We must let the value be pushed, otherwise we would get an underflow
     * error*/
...
static void store_arg(VerifyContext *ctx, guint32 arg) {
  ...
  if (arg >= ctx->max_args) {
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Method doesn't have argument %d at 0x%04x",
                             arg + 1, ctx->ip_offset));
  ...
  if (check_underflow(ctx, 1)) {
    ...
    if (!verify_stack_type_compatibility(ctx, ctx->params[arg], value)) {
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf("Incompatible type %s in argument store at 0x%04x",
                          stack_slot_get_name(value), ctx->ip_offset));
    }
  }
  if (arg == 0 && !(ctx->method->flags & METHOD_ATTRIBUTE_STATIC))
...
static void store_local(VerifyContext *ctx, guint32 arg) {
  ...
  if (arg >= ctx->num_locals) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Method doesn't have local var %d at 0x%04x",
                             arg + 1, ctx->ip_offset));
    return;
  }
  ...
  if (ctx->locals[arg]->byref) {
    if (stack_slot_is_managed_mutability_pointer(value))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Cannot use a readonly managed reference when "
                               "storing on a local variable at 0x%04x",
                               ctx->ip_offset));

    if (local_is_safe_byref(ctx, arg) && !stack_slot_is_safe_byref(value))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Cannot store an unsafe ret byref to a local that was "
                   "previously stored a save ret byref value at 0x%04x",
                   ctx->ip_offset));
  ...
  if (!verify_stack_type_compatibility(ctx, ctx->locals[arg], value)) {
    ...
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Incompatible type '%s' on stack cannot be stored "
                             "to local %d with type '%s' at 0x%04x",
                             found, arg, expected, ctx->ip_offset));
    g_free(expected);
    g_free(found);
  }
}
...
static void do_binop(VerifyContext *ctx, unsigned int opcode,
                     const unsigned char table[TYPE_MAX][TYPE_MAX]) {
  ILStackDesc *a, *b, *top;
  ...
  if (res == TYPE_INV) {
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 ...
                 stack_slot_get_name(a), stack_slot_get_name(b)));
  ...
  if (res & NON_VERIFIABLE_RESULT) {
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Binary instruction is not verifiable (%s x %s)",
                             stack_slot_get_name(a), stack_slot_get_name(b)));
...
static void do_boolean_branch_op(VerifyContext *ctx, int delta) {
  ...
  if (target < 0 || target >= ctx->code_size) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Boolean branch target out of code at 0x%04x",
                             ctx->ip_offset));
    return;
  }
  ...
  case 1:
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Branch target escapes out of exception block at 0x%04x",
                 ctx->ip_offset));
    break;
  case 2:
    ADD_VERIFY_ERROR(
  ...
  if (!is_valid_bool_arg(top))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Argument type %s not valid for brtrue/brfalse at 0x%04x",
                 stack_slot_get_name(top), ctx->ip_offset));
...
stack_slot_is_complex_type_not_reference_type(ILStackDesc *slot) {
  return stack_slot_get_type(slot) == TYPE_COMPLEX &&
         !MONO_TYPE_IS_REFERENCE(slot->type) &&
         !stack_slot_is_boxed_value(slot);
}

static gboolean stack_slot_is_reference_value(ILStackDesc *slot) {
  return stack_slot_get_type(slot) == TYPE_COMPLEX &&
         (MONO_TYPE_IS_REFERENCE(slot->type) ||
          stack_slot_is_boxed_value(slot));
}

static void do_branch_op(VerifyContext *ctx, signed int delta,
                         const unsigned char table[TYPE_MAX][TYPE_MAX]) {
  ILStackDesc *a, *b;
  int idxa, idxb;
  unsigned char res;
  ...
  if (target < 0 || target >= ctx->code_size) {
    ADD_VERIFY_ERROR(ctx, g_strdup_printf("Branch target out of code at 0x%04x",
                                          ctx->ip_offset));
    return;
  }

  switch (
      is_valid_cmp_branch_instruction(ctx->header, ctx->ip_offset, target)) {
  ...
  if (stack_slot_is_complex_type_not_reference_type(a) ||
  ...
  if (res == TYPE_INV) {
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Compare and Branch instruction applyed to ill "
                             "formed stack (%s x %s) at 0x%04x",
                             stack_slot_get_name(a), stack_slot_get_name(b),
                             ctx->ip_offset));
  } else if (res & NON_VERIFIABLE_RESULT) {
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Compare and Branch instruction is not verifiable "
                             "(%s x %s) at 0x%04x",
                             stack_slot_get_name(a), stack_slot_get_name(b),
                             ctx->ip_offset));
    res = res & ~NON_VERIFIABLE_RESULT;
...
static void do_cmp_op(VerifyContext *ctx,
                      const unsigned char table[TYPE_MAX][TYPE_MAX],
                      guint32 opcode) {
  ILStackDesc *a, *b;
  int idxa, idxb;
  ...
  if (res == TYPE_INV) {
    ...
    CODE_NOT_VERIFIABLE(ctx,
                        g_strdup_printf("Compare instruction applyed to ill "
                                        "formed stack (%s x %s) at 0x%04x",
                                        left_type, right_type, ctx->ip_offset));
    g_free(left_type);
    g_free(right_type);
  } else if (res & NON_VERIFIABLE_RESULT) {
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf(
            "Compare instruction is not verifiable (%s x %s) at 0x%04x",
            stack_slot_get_name(a), stack_slot_get_name(b), ctx->ip_offset));
    res = res & ~NON_VERIFIABLE_RESULT;
  }
  stack_push_val(ctx, TYPE_I4, mono_get_int32_type());
}
...
static void do_ret(VerifyContext *ctx) {
  ...
  if (ret->type != MONO_TYPE_VOID) {
    ...
    if (!verify_stack_type_compatibility(ctx, ctx->signature->ret, top)) {
      ...
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf("Incompatible return value on stack with method "
                          "signature, expected '%s' but got '%s' at 0x%04x",
                          ret_type, stack_type, ctx->ip_offset));
      g_free(stack_type);
      g_free(ret_type);
      return;
    }
    ...
    if (ret->byref && !stack_slot_is_safe_byref(top))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Method returns byref and return value is not a "
                               "safe-to-return-byref at 0x%04x",
                               ctx->ip_offset));

    if (ret->type == MONO_TYPE_TYPEDBYREF ||
        mono_type_is_value_type(ret, "System", "ArgIterator") ||
        mono_type_is_value_type(ret, "System", "RuntimeArgumentHandle"))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Method returns byref, TypedReference, "
                               "ArgIterator or RuntimeArgumentHandle at 0x%04x",
                               ctx->ip_offset));
  }

  if (ctx->eval.size > 0) {
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Stack not empty (%d) after ret at 0x%04x",
                             ctx->eval.size, ctx->ip_offset));
  }
  if (in_any_block(ctx->header, ctx->ip_offset))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("ret cannot escape exception blocks at 0x%04x",
                             ctx->ip_offset));
}

/*
 * FIXME we need to fix the case of a non-virtual instance method defined in the
 * parent but call using a token pointing to a subclass. This is illegal but
 * mono_get_method_full decoded it.
 * TODO handle calling .ctor outside one or calling the .ctor for other class
 * but super
 */
static void do_invoke_method(VerifyContext *ctx, int method_token,
                             gboolean virtual_) {
  ERROR_DECL(error);
  int param_count, i;
  ...
  if (!(method = verifier_load_method(ctx, method_token,
                                      virtual_ ? "callvirt" : "call")))
    return;
  ...
  if (virtual_) {
    ...
    if (m_class_is_valuetype(method->klass)) // && !constrained ???
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Cannot use callvirtual with valuetype method at 0x%04x",
                   ctx->ip_offset));

    if ((method->flags & METHOD_ATTRIBUTE_STATIC))
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf("Cannot use callvirtual with static method at 0x%04x",
                          ctx->ip_offset));
    ...
    if (method->flags & METHOD_ATTRIBUTE_ABSTRACT)
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf("Cannot use call with an abstract method at 0x%04x",
                          ctx->ip_offset));
  ...
  if (!(sig = mono_method_get_signature_checked(
            method, ctx->image, method_token, ctx->generic_context, error))) {
    mono_error_cleanup(error);
    sig = mono_method_get_signature_checked(method, ctx->image, method_token,
                                            NULL, error);
  }
  ...
  if (!sig) {
    ...
    ADD_VERIFY_ERROR(
        ctx,
        g_strdup_printf(
            "Could not resolve signature of %s:%s at 0x%04x due to: %s", name,
            method->name, ctx->ip_offset, mono_error_get_message(error)));
    mono_error_cleanup(error);
    g_free(name);
    return;
  }
  ...
  for (i = sig->param_count - 1; i >= 0; --i) {
    ...
    if (!verify_stack_type_compatibility(ctx, sig->params[i], value)) {
      ...
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf("Incompatible parameter with function signature: "
                          "Calling method with signature (%s) but for argument "
                          "%d there is a (%s) on stack at 0x%04x",
                          sig_name, i, stack_name, ctx->ip_offset));
      g_free(stack_name);
      g_free(sig_name);
    }
    ...
    if (stack_slot_is_managed_mutability_pointer(value))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Cannot use a readonly pointer as argument of %s at 0x%04x",
                   virtual_ ? "callvirt" : "call", ctx->ip_offset));

    if ((ctx->prefix_set & PREFIX_TAIL) &&
        stack_slot_is_managed_pointer(value)) {
      ADD_VERIFY_ERROR(
          ctx, g_strdup_printf(
                   "Cannot  pass a byref argument to a tail %s at 0x%04x",
                   virtual_ ? "callvirt" : "call", ctx->ip_offset));
      return;
    }
    if (stack_slot_is_managed_pointer(value) &&
  ...
  if (sig->hasthis) {
    ...
    if (mono_method_is_constructor(method) &&
      ...
      if (IS_STRICT_MODE(ctx) && !mono_method_is_constructor(ctx->method))
        CODE_NOT_VERIFIABLE(
            ctx,
            g_strdup_printf("Cannot call a constructor outside one at 0x%04x",
                            ctx->ip_offset));
      if (IS_STRICT_MODE(ctx) &&
          method->klass != m_class_get_parent(ctx->method->klass) &&
          method->klass != ctx->method->klass)
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf("Cannot call a constructor of a type "
                                 "different from this or super at 0x%04x",
                                 ctx->ip_offset));

      ctx->super_ctor_called = TRUE;
      value = stack_pop_safe(ctx);
      if (IS_STRICT_MODE(ctx) &&
          (value->stype & THIS_POINTER_MASK) != THIS_POINTER_MASK)
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf(
                     "Invalid 'this ptr' argument for constructor at 0x%04x",
                     ctx->ip_offset));
      if (!(value->stype & UNINIT_THIS_MASK))
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf("Calling the base constructor on an "
                                 "initialized this pointer at 0x%04x",
                                 ctx->ip_offset));
    } else {
      value = stack_pop(ctx);
    }
    ...
    if (virt_check_this && !stack_slot_is_this_pointer(value) &&
        !(m_class_is_valuetype(method->klass) ||
          stack_slot_is_boxed_value(value)))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Cannot use the call opcode with a non-final virtual method "
                   "on an object different than the 'this' pointer at 0x%04x",
                   ctx->ip_offset));

    if (constrained && virtual_) {
      if (!stack_slot_is_managed_pointer(value))
        CODE_NOT_VERIFIABLE(ctx,
                            g_strdup_printf("Object is not a managed pointer "
                                            "for a constrained call at 0x%04x",
                                            ctx->ip_offset));
      if (!mono_metadata_type_equal_full(
              mono_type_get_type_byval(value->type),
              mono_type_get_underlying_type(ctx->constrained_type), TRUE))
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf(
                     "Object not compatible with constrained type at 0x%04x",
                     ctx->ip_offset));
      ...
      if (stack_slot_is_managed_pointer(value) &&
          !m_class_is_valuetype(
              mono_class_from_mono_type_internal(value->type)))
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf("Cannot call a reference type using a managed "
                                 "pointer to the this arg at 0x%04x",
                                 ctx->ip_offset));
      ...
      if (!virtual_ &&
          ...
          !stack_slot_is_boxed_value(value))
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf("Cannot call a valuetype baseclass at 0x%04x",
                                 ctx->ip_offset));
      ...
      if (virtual_ &&
        ...
        CODE_NOT_VERIFIABLE(
            ctx,
            g_strdup_printf("Cannot use a valuetype with callvirt at 0x%04x",
                            ctx->ip_offset));
      ...
      if (m_class_is_valuetype(method->klass) &&
          (stack_slot_is_boxed_value(value) ||
           !stack_slot_is_managed_pointer(value)))
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf("Cannot use a boxed or literal valuetype to "
                                 "call a valuetype method at 0x%04x",
                                 ctx->ip_offset));
    }
    if (!verify_stack_type_compatibility(ctx, type, &copy)) {
      ...
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Incompatible this argument on stack with method signature "
                   "expected '%s' but got '%s' for a call to '%s' at 0x%04x",
    ...
    if (!IS_SKIP_VISIBILITY(ctx) &&
        !mono_method_can_access_method_full(
            ctx->method, method,
            mono_class_from_mono_type_internal(value->type))) {
      char *name = mono_method_full_name(method, TRUE);
      CODE_NOT_VERIFIABLE2(
          ctx,
  ...
  } else if (!IS_SKIP_VISIBILITY(ctx) &&
             !mono_method_can_access_method_full(ctx->method, method, NULL)) {
    char *name = mono_method_full_name(method, TRUE);
    CODE_NOT_VERIFIABLE2(
        ctx,
        g_strdup_printf("Method %s is not accessible at 0x%04x", name,
  ...
  if (sig->ret->type != MONO_TYPE_VOID) {
    ...
    if (check_overflow(ctx)) {
      ...
      if ((ctx->prefix_set & PREFIX_READONLY) &&
          m_class_get_rank(method->klass) && !strcmp(method->name, "Address")) {
  ...
  if ((ctx->prefix_set & PREFIX_TAIL)) {
    if (!mono_delegate_ret_equal(
            mono_method_signature_internal(ctx->method)->ret, sig->ret))
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf("Tail call with incompatible return type at 0x%04x",
                          ctx->ip_offset));
    if (ctx->header->code[ctx->ip_offset + 5] != CEE_RET)
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Tail call not followed by ret at 0x%04x",
                               ctx->ip_offset));
  }
}

static void do_push_static_field(VerifyContext *ctx, int token,
                                 gboolean take_addr) {
  MonoClassField *field;
  MonoClass *klass;
  if (!check_overflow(ctx))
  ...
  if (!(field = verifier_load_field(ctx, token, &klass,
                                    take_addr ? "ldsflda" : "ldsfld")))
    return;

  if (!(field->type->attrs & FIELD_ATTRIBUTE_STATIC)) {
    ADD_VERIFY_ERROR(ctx,
                     g_strdup_printf("Cannot load non static field at 0x%04x",
                                     ctx->ip_offset));
    return;
  }
  ...
  if (take_addr && (field->type->attrs & FIELD_ATTRIBUTE_INIT_ONLY) &&
      !(field->parent == ctx->method->klass &&
        (ctx->method->flags &
         (METHOD_ATTRIBUTE_SPECIAL_NAME | METHOD_ATTRIBUTE_STATIC)) &&
        !strcmp(".cctor", ctx->method->name)))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Cannot take the address of a init-only field at 0x%04x",
                 ctx->ip_offset));

  if (!IS_SKIP_VISIBILITY(ctx) &&
      !mono_method_can_access_field_full(ctx->method, field, NULL))
    CODE_NOT_VERIFIABLE2(
        ctx,
        g_strdup_printf("Type at stack is not accessible at 0x%04x",
                        ctx->ip_offset),
...
static void do_store_static_field(VerifyContext *ctx, int token) {
  MonoClassField *field;
  MonoClass *klass;
  ILStackDesc *value;
  CLEAR_PREFIX(ctx, PREFIX_VOLATILE);
  ...
  if (!(field->type->attrs & FIELD_ATTRIBUTE_STATIC)) {
    ADD_VERIFY_ERROR(ctx,
                     g_strdup_printf("Cannot store non static field at 0x%04x",
                                     ctx->ip_offset));
    return;
  }
  ...
  if (field->type->type == MONO_TYPE_TYPEDBYREF) {
    ADD_VERIFY_ERROR(ctx,
                     g_strdup_printf("Typedbyref field is an unverfiable type "
                                     "in store static field at 0x%04x",
                                     ctx->ip_offset));
    return;
  }
  ...
  if (!verify_stack_type_compatibility(ctx, field->type, value)) {
    ...
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Incompatible type in static field store expected "
                             "'%s' but found '%s' at 0x%04x",
...
static gboolean check_is_valid_type_for_field_ops(VerifyContext *ctx, int token,
                                                  ILStackDesc *obj,
                                                  MonoClassField **ret_field,
                                                  const char *opcode) {
  MonoClassField *field;
  MonoClass *klass;
  gboolean is_pointer;

  /*must be a reference type, a managed pointer, an unamanaged pointer, or a
   * valuetype*/
  ...
  *ret_field = field;
  ...
  is_pointer = stack_slot_get_type(obj) == TYPE_PTR ||
               (stack_slot_get_type(obj) == TYPE_NATIVE_INT &&
                !get_stack_type(m_class_get_byval_arg(field->parent)));

  if (field->type->type == MONO_TYPE_TYPEDBYREF) {
    ADD_VERIFY_ERROR(
        ctx,
        g_strdup_printf("Typedbyref field is an unverfiable type at 0x%04x",
                        ctx->ip_offset));
    return FALSE;
  }
  g_assert(obj->type);

  /*The value on the stack must be a subclass of the defining type of the
   * field*/
  /* we need to check if we can load the field from the stack value*/
  if (is_pointer) {
    if (stack_slot_get_underlying_type(obj) == TYPE_NATIVE_INT)
      CODE_NOT_VERIFIABLE(ctx,
                          g_strdup_printf("Native int is not a verifiable type "
                                          "to reference a field at 0x%04x",
                                          ctx->ip_offset));
    ...
    if (!m_class_is_valuetype(field->parent) &&
      ...
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Type at stack is a managed pointer to a reference type and "
                   "is not compatible to reference the field at 0x%04x",
                   ctx->ip_offset));

    /*a value type can be loaded from a value or a managed pointer, but not a
     * boxed object*/
    if (m_class_is_valuetype(field->parent) && stack_slot_is_boxed_value(obj))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Type at stack is a boxed valuetype and is not "
                               "compatible to reference the field at 0x%04x",
                               ctx->ip_offset));

    if (!stack_slot_is_null_literal(obj) &&
        !verify_stack_type_compatibility_full(
            ctx, m_class_get_byval_arg(field->parent), obj, TRUE, FALSE)) {
      char *found = stack_slot_full_name(obj);
      char *expected =
          mono_type_full_name(m_class_get_byval_arg(field->parent));
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Expected type '%s' but found '%s' referencing "
                               "the 'this' argument at 0x%04x",
                               expected, found, ctx->ip_offset));
      g_free(found);
      g_free(expected);
    }
    ...
    if (!IS_SKIP_VISIBILITY(ctx) &&
        !mono_method_can_access_field_full(
            ctx->method, field, mono_class_from_mono_type_internal(obj->type)))
      CODE_NOT_VERIFIABLE2(
          ctx,
          g_strdup_printf("Type at stack is not accessible at 0x%04x",
                          ctx->ip_offset),
...
static void do_push_field(VerifyContext *ctx, int token, gboolean take_addr) {
  ILStackDesc *obj;
  MonoClassField *field;
  ...
  if (!check_is_valid_type_for_field_ops(ctx, token, obj, &field,
                                         take_addr ? "ldflda" : "ldfld"))
    return;

  if (take_addr && m_class_is_valuetype(field->parent) &&
      !stack_slot_is_managed_pointer(obj))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Cannot take the address of a temporary value-type at 0x%04x",
                 ctx->ip_offset));
  ...
  is_safe_byref = take_addr && (stack_slot_is_reference_value(obj) ||
...
static void do_store_field(VerifyContext *ctx, int token) {
  ...
  if (!verify_stack_type_compatibility(ctx, field->type, value))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Incompatible type %s in field store at 0x%04x",
                             stack_slot_get_name(value), ctx->ip_offset));
}

/*TODO proper handle for Nullable<T>*/
static void do_box_value(VerifyContext *ctx, int klass_token) {
  ILStackDesc *value;
  ...
  if (stack_slot_get_underlying_type(value) == TYPE_COMPLEX &&
      MONO_TYPE_IS_REFERENCE(value->type) && MONO_TYPE_IS_REFERENCE(type)) {
  ...
  if (!verify_stack_type_compatibility(ctx, type, value))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Invalid type at stack for boxing operation at 0x%04x",
                        ctx->ip_offset));
...
static void do_unbox_value(VerifyContext *ctx, int klass_token) {
  ILStackDesc *value;
  ...
  if (!m_class_is_valuetype(mono_class_from_mono_type_internal(type)))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid reference type for unbox at 0x%04x",
                             ctx->ip_offset));
  ...
  if (!(stack_slot_get_type(value) == TYPE_COMPLEX &&
    ...
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Invalid type %s at stack for unbox operation at 0x%04x",
                 stack_slot_get_name(value), ctx->ip_offset));

  set_stack_value(ctx, value = stack_push(ctx), mono_type_get_type_byref(type),
                  FALSE);
  value->stype |= CMMP_MASK;
}

static void do_unbox_any(VerifyContext *ctx, int klass_token) {
  ILStackDesc *value;
  ...
  if (!(stack_slot_get_type(value) == TYPE_COMPLEX &&
    ...
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Invalid type %s at stack for unbox.any operation at 0x%04x",
                 stack_slot_get_name(value), ctx->ip_offset));

  set_stack_value(ctx, stack_push(ctx), type, FALSE);
}

static void do_unary_math_op(VerifyContext *ctx, int op) {
  ILStackDesc *value;
  if (!check_underflow(ctx, 1))
    return;
  value = stack_pop(ctx);
  ...
  default:
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid type at stack for unary not at 0x%04x",
                             ctx->ip_offset));
  }
  stack_push_stack_val(ctx, value);
}

static void do_conversion(VerifyContext *ctx, int kind) {
  ILStackDesc *value;
  if (!check_underflow(ctx, 1))
    return;
  value = stack_pop(ctx);
  ...
  default:
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid type (%s) at stack for conversion "
                             "operation. Numeric type expected at 0x%04x",
                             stack_slot_get_name(value), ctx->ip_offset));
  }
...
static void do_load_token(VerifyContext *ctx, int token) {
  ERROR_DECL(error);
  gpointer handle;
  ...
  if (ctx->method->wrapper_type != MONO_WRAPPER_NONE) {
    ...
    handle_class =
        (MonoClass *)mono_method_get_wrapper_data(ctx->method, token + 1);
    ...
    case MONO_TOKEN_MEMBER_REF:
      if (!token_bounds_check(ctx->image, token)) {
        ADD_VERIFY_ERROR(ctx,
                         g_strdup_printf("Table index out of range 0x%x for "
                                         "token %x for ldtoken at 0x%04x",
                                         mono_metadata_token_index(token),
                                         token, ctx->ip_offset));
        return;
      }
    ...
    default:
      ADD_VERIFY_ERROR(
          ctx, g_strdup_printf(
                   "Invalid table 0x%x for token 0x%x for ldtoken at 0x%04x",
                   mono_metadata_token_table(token), token, ctx->ip_offset));
      return;
    }

    handle = mono_ldtoken_checked(ctx->image, token, &handle_class,
                                  ctx->generic_context, error);
  }
  ...
  if (!handle) {
    ADD_VERIFY_ERROR(
        ctx,
        g_strdup_printf("Invalid token 0x%x for ldtoken at 0x%04x due to %s",
                        token, ctx->ip_offset, mono_error_get_message(error)));
    mono_error_cleanup(error);
    return;
  }
  if (handle_class == mono_defaults.typehandle_class) {
    ...
    mono_type_is_valid_in_context(
        ctx, m_class_get_byval_arg(((MonoClassField *)handle)->parent));
  } else {
    ADD_VERIFY_ERROR2(ctx,
                      g_strdup_printf("Invalid ldtoken type %x at 0x%04x",
                                      token, ctx->ip_offset),
                      MONO_EXCEPTION_BAD_IMAGE);
  }
...
static void do_ldobj_value(VerifyContext *ctx, int token) {
  ILStackDesc *value;
  ...
  if (!stack_slot_is_managed_pointer(value) &&
      ...
      !(stack_slot_get_type(value) == TYPE_PTR &&
        value->type->type != MONO_TYPE_FNPTR)) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf("Invalid argument %s to ldobj at 0x%04x",
                             stack_slot_get_name(value), ctx->ip_offset));
    return;
  }
  ...
  if (stack_slot_get_type(value) == TYPE_NATIVE_INT)
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Using native pointer to ldobj at 0x%04x",
                             ctx->ip_offset));

  /*We have a byval on the stack, but the comparison must be strict. */
  if (!verify_type_compatibility_full(
          ctx, type, mono_type_get_type_byval(value->type), TRUE))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Invalid type at stack for ldojb operation at 0x%04x",
                        ctx->ip_offset));

  set_stack_value(ctx, stack_push(ctx), type, FALSE);
}

static void do_stobj(VerifyContext *ctx, int token) {
  ...
  if (stack_slot_is_managed_mutability_pointer(dest))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Cannot use a readonly pointer with stobj at 0x%04x",
                        ctx->ip_offset));

  if (!stack_slot_is_managed_pointer(dest))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid destination of stobj operation at 0x%04x",
                             ctx->ip_offset));

  if (stack_slot_is_boxed_value(src) && !MONO_TYPE_IS_REFERENCE(src->type) &&
      !MONO_TYPE_IS_REFERENCE(type))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Cannot use stobj with a boxed source value that "
                             "is not a reference type at 0x%04x",
                             ctx->ip_offset));
  ...
  if (!verify_stack_type_compatibility(ctx, type, src)) {
    ...
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Token '%s' and source '%s' of stobj don't match ' at 0x%04x",
                 type_name, src_name, ctx->ip_offset));
    g_free(type_name);
    g_free(src_name);
  }

  if (!verify_type_compatibility(ctx, mono_type_get_type_byval(dest->type),
                                 type))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Destination and token types of stobj don't match at 0x%04x",
                 ctx->ip_offset));
}

static void do_cpobj(VerifyContext *ctx, int token) {
  ILStackDesc *dest, *src;
  ...
  if (!stack_slot_is_managed_pointer(src))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid source of cpobj operation at 0x%04x",
                             ctx->ip_offset));

  if (!stack_slot_is_managed_pointer(dest))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid destination of cpobj operation at 0x%04x",
                             ctx->ip_offset));

  if (stack_slot_is_managed_mutability_pointer(dest))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Cannot use a readonly pointer with cpobj at 0x%04x",
                        ctx->ip_offset));

  if (!verify_type_compatibility(ctx, type,
                                 mono_type_get_type_byval(src->type)))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Token and source types of cpobj don't match at 0x%04x",
                        ctx->ip_offset));
  ...
  if (!verify_type_compatibility(ctx, mono_type_get_type_byval(dest->type),
    ...
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Destination and token types of cpobj don't match at 0x%04x",
                 ctx->ip_offset));
}

static void do_initobj(VerifyContext *ctx, int token) {
  ILStackDesc *obj;
  ...
  if (!stack_slot_is_managed_pointer(obj))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid object address for initobj at 0x%04x",
                             ctx->ip_offset));

  if (stack_slot_is_managed_mutability_pointer(obj))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Cannot use a readonly pointer with initobj at 0x%04x",
                        ctx->ip_offset));
  ...
  if (MONO_TYPE_IS_REFERENCE(stack)) {
    if (!verify_type_compatibility(ctx, stack, type))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Type token of initobj not compatible with "
                               "value on stack at 0x%04x",
                               ctx->ip_offset));
    ...
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf(
                 "Initobj %s not compatible with value on stack %s at 0x%04x",
                 expected_name, stack_name, ctx->ip_offset));
    g_free(expected_name);
    g_free(stack_name);
  }
}

static void do_newobj(VerifyContext *ctx, int token) {
  ILStackDesc *value;
  int i;
  MonoMethodSignature *sig;
  MonoMethod *method;
  ...
  if (!mono_method_is_constructor(method)) {
    ADD_VERIFY_ERROR(
        ctx,
        g_strdup_printf("Method from token 0x%08x not a constructor at 0x%04x",
                        token, ctx->ip_offset));
    return;
  }

  if (mono_class_get_flags(method->klass) &
      (TYPE_ATTRIBUTE_ABSTRACT | TYPE_ATTRIBUTE_INTERFACE))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf(
            "Trying to instantiate an abstract or interface type at 0x%04x",
            ctx->ip_offset));
  ...
  if (!sig) {
    ADD_VERIFY_ERROR(
        ctx,
        g_strdup_printf("Invalid constructor signature to newobj at 0x%04x",
                        ctx->ip_offset));
    return;
  }
  ...
  if (!sig->hasthis) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf(
                 "Invalid constructor signature missing hasthis at 0x%04x",
                 ctx->ip_offset));
    return;
  }
  ...
  is_delegate = m_class_get_parent(method->klass) ==
  ...
  if (is_delegate) {
    ...
    if (sig->param_count != 2) {
      ADD_VERIFY_ERROR(ctx,
                       g_strdup_printf("Invalid delegate constructor at 0x%04x",
                                       ctx->ip_offset));
      return;
    }
    ...
    for (i = sig->param_count - 1; i >= 0; --i) {
      ...
      if (!verify_stack_type_compatibility(ctx, sig->params[i], value)) {
        ...
        char *sig_name = mono_type_full_name(sig->params[i]);
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf("Incompatible parameter value with "
                                 "constructor signature: %s X %s at 0x%04x",
                                 sig_name, stack_name, ctx->ip_offset));
        g_free(stack_name);
        g_free(sig_name);
      }
      ...
      if (stack_slot_is_managed_mutability_pointer(value))
        CODE_NOT_VERIFIABLE(
            ctx,
            g_strdup_printf(
                "Cannot use a readonly pointer as argument of newobj at 0x%04x",
                ctx->ip_offset));
    }
  }

  if (check_overflow(ctx))
    set_stack_value(ctx, stack_push(ctx), m_class_get_byval_arg(method->klass),
                    FALSE);
}

static void do_cast(VerifyContext *ctx, int token, const char *opcode) {
  ILStackDesc *value;
  MonoType *type;
  gboolean is_boxed;
  gboolean do_box;
  ...
  if (type->byref) {
    ADD_VERIFY_ERROR(ctx, g_strdup_printf("Invalid %s type at 0x%04x", opcode,
                                          ctx->ip_offset));
    return;
  }
  ...
  if (stack_slot_is_managed_pointer(value))
    CODE_NOT_VERIFIABLE(ctx, g_strdup_printf("Invalid value for %s at 0x%04x",
                                             opcode, ctx->ip_offset));
    ...
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf(
            "Expected a reference type on stack for %s but found %s at 0x%04x",
            opcode, name, ctx->ip_offset));
    g_free(name);
  }
...
static void do_load_indirect(VerifyContext *ctx, int opcode) {
  ILStackDesc *value;
  CLEAR_PREFIX(ctx, PREFIX_UNALIGNED | PREFIX_VOLATILE);
  ...
  if (!stack_slot_is_managed_pointer(value)) {
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Load indirect not using a manager pointer at 0x%04x",
                        ctx->ip_offset));
  ...
  if (opcode == CEE_LDIND_REF) {
    if (stack_slot_get_underlying_type(value) != TYPE_COMPLEX ||
        m_class_is_valuetype(mono_class_from_mono_type_internal(value->type)))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Invalid type at stack for ldind_ref expected "
                               "object byref operation at 0x%04x",
                               ctx->ip_offset));
    set_stack_value(ctx, stack_push(ctx), mono_type_get_type_byval(value->type),
                    FALSE);
  } else {
    if (!verify_type_compatibility_full(ctx, mono_type_from_opcode(opcode),
                                        mono_type_get_type_byval(value->type),
                                        TRUE))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Invalid type at stack for ldind 0x%x operation at 0x%04x",
                   opcode, ctx->ip_offset));
    set_stack_value(ctx, stack_push(ctx), mono_type_from_opcode(opcode), FALSE);
  }
}

static void do_store_indirect(VerifyContext *ctx, int opcode) {
  ...
  if (!stack_slot_is_managed_pointer(addr) &&
      stack_slot_get_type(addr) != TYPE_PTR) {
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid non-pointer argument to stind at 0x%04x",
                             ctx->ip_offset));
    return;
  }
  ...
  if (stack_slot_is_managed_mutability_pointer(addr)) {
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Cannot use a readonly pointer with stind at 0x%04x",
                        ctx->ip_offset));
    return;
  }

  if (!verify_type_compatibility_full(ctx, mono_type_from_opcode(opcode),
                                      mono_type_get_type_byval(addr->type),
                                      TRUE))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf(
            "Invalid addr type at stack for stind 0x%x operation at 0x%04x",
            opcode, ctx->ip_offset));
  ...
  if (!verify_stack_type_compatibility(ctx, mono_type_from_opcode(opcode), val))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf(
            "Invalid value type at stack for stind 0x%x operation at 0x%04x",
            opcode, ctx->ip_offset));
}

static void do_newarr(VerifyContext *ctx, int token) {
  ILStackDesc *value;
  ...
  value = stack_pop(ctx);
  if (stack_slot_get_type(value) != TYPE_I4 &&
      stack_slot_get_type(value) != TYPE_NATIVE_INT)
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf(
            "Array size type on stack (%s) is not a verifiable type at 0x%04x",
            stack_slot_get_name(value), ctx->ip_offset));

  set_stack_value(ctx, stack_push(ctx),
                  m_class_get_byval_arg(mono_class_create_array(
                      mono_class_from_mono_type_internal(type), 1)),
                  FALSE);
}

/*FIXME handle arrays that are not 0-indexed*/
static void do_ldlen(VerifyContext *ctx) {
  ILStackDesc *value;
  ...
  if (stack_slot_get_type(value) != TYPE_COMPLEX ||
      value->type->type != MONO_TYPE_SZARRAY)
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid array type for ldlen at 0x%04x",
                             ctx->ip_offset));
...
static void do_ldelema(VerifyContext *ctx, int klass_token) {
  ...
  if (stack_slot_get_type(index) != TYPE_I4 &&
      stack_slot_get_type(index) != TYPE_NATIVE_INT)
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Index type(%s) for ldelema is not an int or a "
                             "native int at 0x%04x",
                             stack_slot_get_name(index), ctx->ip_offset));

  if (!stack_slot_is_null_literal(array)) {
    if (stack_slot_get_type(array) != TYPE_COMPLEX ||
        array->type->type != MONO_TYPE_SZARRAY)
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Invalid array type(%s) for ldelema at 0x%04x",
                               stack_slot_get_name(array), ctx->ip_offset));
    else {
      if (get_stack_type(type) == TYPE_I4 ||
          get_stack_type(type) == TYPE_NATIVE_INT) {
        valid = verify_type_compatibility_full(
            ctx, type, m_class_get_byval_arg(array->type->data.klass), TRUE);
      } else {
        valid = mono_metadata_type_equal(
            type, m_class_get_byval_arg(array->type->data.klass));
      }
      if (!valid)
        CODE_NOT_VERIFIABLE(
            ctx,
            g_strdup_printf("Invalid array type on stack for ldelema at 0x%04x",
                            ctx->ip_offset));
    }
  }
 ...
 * FIXME handle readonly prefix and CMMP
 */
static void do_ldelem(VerifyContext *ctx, int opcode, int token) {
#define IS_ONE_OF2(T, A, B) (T == A || T == B)
  ...
  if (opcode == CEE_LDELEM) {
    if (!(type = verifier_load_type(ctx, token, "ldelem.any"))) {
      ADD_VERIFY_ERROR(ctx, g_strdup_printf("Type (0x%08x) not found at 0x%04x",
                                            token, ctx->ip_offset));
      return;
    }
  } else {
  ...
  if (stack_slot_get_type(index) != TYPE_I4 &&
      stack_slot_get_type(index) != TYPE_NATIVE_INT)
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Index type(%s) for ldelem.X is not an int or a "
                             "native int at 0x%04x",
                             stack_slot_get_name(index), ctx->ip_offset));
  ...
  if (!stack_slot_is_null_literal(array)) {
    if (stack_slot_get_type(array) != TYPE_COMPLEX ||
        array->type->type != MONO_TYPE_SZARRAY)
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Invalid array type(%s) for ldelem.X at 0x%04x",
                               stack_slot_get_name(array), ctx->ip_offset));
    else {
      if (opcode == CEE_LDELEM_REF) {
        if (m_class_is_valuetype(array->type->data.klass))
          CODE_NOT_VERIFIABLE(
              ctx, g_strdup_printf("Invalid array type is not a reference type "
                                   "for ldelem.ref 0x%04x",
                                   ctx->ip_offset));
        type = m_class_get_byval_arg(array->type->data.klass);
      } else {
        ...
        if (IS_STRICT_MODE(ctx)) {
          ...
          MonoType *underlying_candidate =
              mono_type_get_underlying_type_any(candidate);
          if ((IS_ONE_OF2(underlying_type->type, MONO_TYPE_I4, MONO_TYPE_U4) &&
               IS_ONE_OF2(underlying_candidate->type, MONO_TYPE_I,
                          MONO_TYPE_U)) ||
              (IS_ONE_OF2(underlying_candidate->type, MONO_TYPE_I4,
                          MONO_TYPE_U4) &&
               IS_ONE_OF2(underlying_type->type, MONO_TYPE_I, MONO_TYPE_U)))
            CODE_NOT_VERIFIABLE(
                ctx, g_strdup_printf(
                         "Invalid array type on stack for ldelem.X at 0x%04x",
                         ctx->ip_offset));
        }
 ...
 * FIXME handle arrays that are not 0-indexed
 */
static void do_stelem(VerifyContext *ctx, int opcode, int token) {
  ...
  if (stack_slot_get_type(index) != TYPE_I4 &&
      stack_slot_get_type(index) != TYPE_NATIVE_INT)
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Index type(%s) for stdelem.X is not an int or a "
                             "native int at 0x%04x",
                             stack_slot_get_name(index), ctx->ip_offset));
  ...
  if (!stack_slot_is_null_literal(array)) {
    if (stack_slot_get_type(array) != TYPE_COMPLEX ||
        array->type->type != MONO_TYPE_SZARRAY) {
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Invalid array type(%s) for stelem.X at 0x%04x",
                               stack_slot_get_name(array), ctx->ip_offset));
    } else {
      if (opcode == CEE_STELEM_REF) {
        if (m_class_is_valuetype(array->type->data.klass))
          CODE_NOT_VERIFIABLE(
              ctx, g_strdup_printf("Invalid array type is not a reference type "
                                   "for stelem.ref 0x%04x",
                                   ctx->ip_offset));
      } else if (!verify_type_compatibility_full(
                     ctx, m_class_get_byval_arg(array->type->data.klass), type,
                     TRUE)) {
        CODE_NOT_VERIFIABLE(
            ctx, g_strdup_printf(
                     "Invalid array type on stack for stdelem.X at 0x%04x",
                     ctx->ip_offset));
      }
    }
  }
  if (opcode == CEE_STELEM_REF) {
    if (!stack_slot_is_boxed_value(value) &&
      ...
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf(
              "Invalid value is not a reference type for stelem.ref 0x%04x",
              ctx->ip_offset));
  } else if (opcode != CEE_STELEM_REF) {
    if (!verify_stack_type_compatibility(ctx, type, value))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Invalid value on stack for stdelem.X at 0x%04x",
                               ctx->ip_offset));

    if (stack_slot_is_boxed_value(value) &&
        !MONO_TYPE_IS_REFERENCE(value->type) && !MONO_TYPE_IS_REFERENCE(type))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Cannot use stobj with a boxed source value "
                               "that is not a reference type at 0x%04x",
                               ctx->ip_offset));
  }
}

static void do_throw(VerifyContext *ctx) {
  ILStackDesc *exception;
  if (!check_underflow(ctx, 1))
    return;
  exception = stack_pop(ctx);

  if (!stack_slot_is_null_literal(exception) &&
      !(stack_slot_get_type(exception) == TYPE_COMPLEX &&
        !m_class_is_valuetype(
            mono_class_from_mono_type_internal(exception->type))))
    CODE_NOT_VERIFIABLE(ctx,
                        g_strdup_printf("Invalid type on stack for throw, "
                                        "expected reference type at 0x%04x",
                                        ctx->ip_offset));

  if (mono_type_is_generic_argument(exception->type) &&
      !stack_slot_is_boxed_value(exception)) {
    char *name = mono_type_full_name(exception->type);
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Invalid type on stack for throw, expected "
                             "reference type but found unboxed %s  at 0x%04x ",
                             name, ctx->ip_offset));
    g_free(name);
  }
...
static void do_endfilter(VerifyContext *ctx) {
  MonoExceptionClause *clause;
  ...
  if (IS_STRICT_MODE(ctx)) {
    if (ctx->eval.size != 1)
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Stack size must have one item for endfilter at 0x%04x",
                   ctx->ip_offset));

    if (ctx->eval.size >= 1 && stack_slot_get_type(stack_pop(ctx)) != TYPE_I4)
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Stack item type is not an int32 for endfilter at 0x%04x",
                   ctx->ip_offset));
  }
  ...
  if ((clause = is_correct_endfilter(ctx, ctx->ip_offset))) {
    if (IS_STRICT_MODE(ctx)) {
      if (ctx->ip_offset != clause->handler_offset - 2)
        ADD_VERIFY_ERROR(
            ctx, g_strdup_printf("endfilter is not the last instruction of the "
                                 "filter clause at 0x%04x",
                                 ctx->ip_offset));
    } else {
      if ((ctx->ip_offset != clause->handler_offset - 2) &&
          !MONO_OFFSET_IN_HANDLER(clause, ctx->ip_offset))
        ADD_VERIFY_ERROR(
            ctx, g_strdup_printf("endfilter is not the last instruction of the "
                                 "filter clause at 0x%04x",
                                 ctx->ip_offset));
    ...
    if (IS_STRICT_MODE(ctx) && !is_unverifiable_endfilter(ctx, ctx->ip_offset))
      ADD_VERIFY_ERROR(
          ctx, g_strdup_printf("endfilter outside filter clause at 0x%04x",
                               ctx->ip_offset));
    else
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("endfilter outside filter clause at 0x%04x",
...
static void do_leave(VerifyContext *ctx, int delta) {
  ...
  if (!is_correct_leave(ctx->header, ctx->ip_offset, target))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Leave not allowed in finally block at 0x%04x",
                             ctx->ip_offset));
  ctx->eval.size = 0;
  ctx->target = target;
}
 ...
 */
static void do_static_branch(VerifyContext *ctx, int delta) {
  int target = ctx->ip_offset + delta;
  if (target < 0 || target >= ctx->code_size) {
    ADD_VERIFY_ERROR(ctx, g_strdup_printf("branch target out of code at 0x%04x",
                                          ctx->ip_offset));
...
static void do_switch(VerifyContext *ctx, int count,
  ...
  if (stack_slot_get_type(value) != TYPE_I4 &&
      stack_slot_get_type(value) != TYPE_NATIVE_INT)
    CODE_NOT_VERIFIABLE(ctx,
                        g_strdup_printf("Invalid argument to switch at 0x%04x",
                                        ctx->ip_offset));
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (target < 0 || target >= ctx->code_size) {
      ADD_VERIFY_ERROR(ctx,
                       g_strdup_printf("Switch target %x out of code at 0x%04x",
                                       i, ctx->ip_offset));
      return;
    }
    ...
    case 1:
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf(
                   "Switch target %x escapes out of exception block at 0x%04x",
                   i, ctx->ip_offset));
      break;
    case 2:
      ADD_VERIFY_ERROR(
...
static void do_load_function_ptr(VerifyContext *ctx, guint32 token,
                                 gboolean virtual_) {
  ILStackDesc *top;
  MonoMethod *method;
  ...
  if (ctx->method->wrapper_type != MONO_WRAPPER_NONE) {
    ...
    if (!method) {
      ADD_VERIFY_ERROR2(ctx,
                        g_strdup_printf("Invalid token %x for ldftn  at 0x%04x",
                                        token, ctx->ip_offset),
                        MONO_EXCEPTION_BAD_IMAGE);
      return;
    }
    ...
    if (!(method = verifier_load_method(ctx, token,
                                        virtual_ ? "ldvirtfrn" : "ldftn")))
      return;
  }

  if (mono_method_is_constructor(method))
    CODE_NOT_VERIFIABLE(
        ctx, g_strdup_printf("Cannot use ldftn with a constructor at 0x%04x",
                             ctx->ip_offset));

  if (virtual_) {
    ILStackDesc *top = stack_pop(ctx);

    if (stack_slot_get_type(top) != TYPE_COMPLEX ||
        top->type->type == MONO_TYPE_VALUETYPE)
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Invalid argument to ldvirtftn at 0x%04x",
                               ctx->ip_offset));

    if (method->flags & METHOD_ATTRIBUTE_STATIC)
      CODE_NOT_VERIFIABLE(
          ctx,
          g_strdup_printf("Cannot use ldvirtftn with a constructor at 0x%04x",
                          ctx->ip_offset));

    if (!verify_stack_type_compatibility(
            ctx, m_class_get_byval_arg(method->klass), top))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Unexpected object for ldvirtftn at 0x%04x",
                               ctx->ip_offset));
  }
  ...
  if (!IS_SKIP_VISIBILITY(ctx) &&
    ...
    CODE_NOT_VERIFIABLE2(
        ctx,
        g_strdup_printf(
            "Loaded method is not visible for ldftn/ldvirtftn at 0x%04x",
            ctx->ip_offset),
        MONO_EXCEPTION_METHOD_ACCESS);
...
static void do_sizeof(VerifyContext *ctx, int token) {
  MonoType *type;
  ...
  if (type->byref && type->type != MONO_TYPE_TYPEDBYREF) {
    ADD_VERIFY_ERROR(ctx, g_strdup_printf("Invalid use of byref type at 0x%04x",
                                          ctx->ip_offset));
    return;
  }

  if (type->type == MONO_TYPE_VOID) {
    ADD_VERIFY_ERROR(ctx, g_strdup_printf("Invalid use of void type at 0x%04x",
                                          ctx->ip_offset));
    return;
  }
  ...
  if (check_overflow(ctx))
    set_stack_value(ctx, stack_push(ctx),
                    m_class_get_byval_arg(mono_defaults.uint32_class), FALSE);
}

/* Stack top can be of any type, the runtime doesn't care and treat everything
 * as an int. */
static void do_localloc(VerifyContext *ctx) {
  if (ctx->eval.size != 1) {
    ADD_VERIFY_ERROR(
        ctx,
        g_strdup_printf("Stack must have only size item in localloc at 0x%04x",
                        ctx->ip_offset));
    return;
  }
  ...
  CODE_NOT_VERIFIABLE(
      ctx, g_strdup_printf("Instruction localloc in never verifiable at 0x%04x",
                           ctx->ip_offset));
}

static void do_ldstr(VerifyContext *ctx, guint32 token) {
  ERROR_DECL(error);
  if (ctx->method->wrapper_type == MONO_WRAPPER_NONE &&
      !image_is_dynamic(ctx->image)) {
    if (mono_metadata_token_code(token) != MONO_TOKEN_STRING) {
      ADD_VERIFY_ERROR2(ctx,
                        g_strdup_printf("Invalid string token %x at 0x%04x",
                                        token, ctx->ip_offset),
                        MONO_EXCEPTION_BAD_IMAGE);
      return;
    }

    if (!mono_verifier_verify_string_signature(
            ctx->image, mono_metadata_token_index(token), error)) {
      ADD_VERIFY_ERROR2(
          ctx,
          g_strdup_printf("Invalid string index %x at 0x%04x due to: %s", token,
                          ctx->ip_offset, mono_error_get_message(error)),
          MONO_EXCEPTION_BAD_IMAGE);
      mono_error_cleanup(error);
      return;
    }
  ...
  if (check_overflow(ctx))
    stack_push_val(ctx, TYPE_COMPLEX,
                   m_class_get_byval_arg(mono_defaults.string_class));
}

static void do_refanyval(VerifyContext *ctx, int token) {
  ILStackDesc *top;
  MonoType *type;
  if (!check_underflow(ctx, 1))
    return;
  ...
  if (top->stype != TYPE_PTR || top->type->type != MONO_TYPE_TYPEDBYREF)
    ADD_VERIFY_ERROR(ctx,
                     g_strdup_printf("Expected a typedref as argument for "
                                     "refanyval, but found %s at 0x%04x",
                                     stack_slot_get_name(top), ctx->ip_offset));

  set_stack_value(ctx, stack_push(ctx), type, TRUE);
}

static void do_refanytype(VerifyContext *ctx) {
  ILStackDesc *top;
  ...
  if (top->stype != TYPE_PTR || top->type->type != MONO_TYPE_TYPEDBYREF)
    ADD_VERIFY_ERROR(ctx,
                     g_strdup_printf("Expected a typedref as argument for "
                                     "refanytype, but found %s at 0x%04x",
                                     stack_slot_get_name(top), ctx->ip_offset));

  set_stack_value(ctx, stack_push(ctx),
                  m_class_get_byval_arg(mono_defaults.typehandle_class), FALSE);
}

static void do_mkrefany(VerifyContext *ctx, int token) {
  ILStackDesc *top;
  MonoType *type;
  if (!check_underflow(ctx, 1))
    return;
  ...
  if (stack_slot_is_managed_mutability_pointer(top))
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf("Cannot use a readonly pointer with mkrefany at 0x%04x",
                        ctx->ip_offset));
  ...
  if (!stack_slot_is_managed_pointer(top)) {
    CODE_NOT_VERIFIABLE(
        ctx,
        g_strdup_printf(
            "Expected a managed pointer for mkrefany, but found %s at 0x%04x",
            stack_slot_get_name(top), ctx->ip_offset));
  } else {
    MonoType *stack_type = mono_type_get_type_byval(top->type);
    if (MONO_TYPE_IS_REFERENCE(type) &&
        !mono_metadata_type_equal(type, stack_type))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Type not compatible for mkrefany at 0x%04x",
                               ctx->ip_offset));

    if (!MONO_TYPE_IS_REFERENCE(type) &&
        !verify_type_compatibility_full(ctx, type, stack_type, TRUE))
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Type not compatible for mkrefany at 0x%04x",
                               ctx->ip_offset));
  }
  ...
  set_stack_value(ctx, stack_push(ctx),
                  m_class_get_byval_arg(mono_defaults.typed_reference_class),
                  FALSE);
}

static void do_ckfinite(VerifyContext *ctx) {
  ILStackDesc *top;
  if (!check_underflow(ctx, 1))
    return;
  ...
  if (stack_slot_get_underlying_type(top) != TYPE_R8)
    ADD_VERIFY_ERROR(ctx,
                     g_strdup_printf("Expected float32 or float64 on stack for "
                                     "ckfinit but found %s at 0x%04x",
                                     stack_slot_get_name(top), ctx->ip_offset));
  stack_push_stack_val(ctx, top);
}
/*
 * merge_stacks:
 * Merge the stacks and perform compat checks. The merge check if types of @from
 * are mergeable with type of @to
 *
...
static void merge_stacks(VerifyContext *ctx, ILCodeDesc *from, ILCodeDesc *to,
  ...
  if (from->size != to->size) {
    ...
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf(
                 "Could not merge stacks, different sizes (%d x %d) at 0x%04x",
                 from->size, to->size, ctx->ip_offset));
    goto end_verify;
  }
  ...
  for (i = 0; i < from->size; ++i) {
    ...
    if (stack_slot_is_safe_byref(old_slot) ^
        stack_slot_is_safe_byref(new_slot)) {
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Cannot merge stack at depth %d byref types are "
                               "safe byref incompatible at %0x04x ",
                               i, ctx->ip_offset));
      goto end_verify;
    }

    // S := T then U = S (new value is compatible with current value, keep
    // current)
    ...
    if (mono_type_is_generic_argument(old_type) ||
      ...
      CODE_NOT_VERIFIABLE(
          ctx, g_strdup_printf("Could not merge stack at depth %d, types not "
                               "compatible: %s X %s at 0x%04x",
                               i, old_name, new_name, ctx->ip_offset));
      g_free(old_name);
      g_free(new_name);
      goto end_verify;
    }
    ...
    if (!m_class_is_valuetype(mono_class_from_mono_type_internal(old_type)) &&
      ...
      for (j = MIN(m_class_get_idepth(old_class),
                   m_class_get_idepth(new_class)) -
               1;
           j > 0; --j) {
        if (mono_metadata_type_equal(
                m_class_get_byval_arg(old_class_supertypes[j]),
                m_class_get_byval_arg(new_class_supertypes[j]))) {
      ...
      if (!is_ok(error)) {
        CODE_NOT_VERIFIABLE(
            ctx,
            g_strdup_printf(
                "Cannot merge stacks due to a TypeLoadException %s at 0x%04x",
                mono_error_get_message(error), ctx->ip_offset));
        mono_error_cleanup(error);
        goto end_verify;
      }
      ...
      if (mono_class_is_interface(old_class)) {
        ...
        guint16 old_class_interface_count =
            m_class_get_interface_count(old_class);
        for (j = 0; j < old_class_interface_count; ++j) {
          if (verifier_class_is_assignable_from(old_class_interfaces[j],
                                                new_class)) {
      ...
      if (mono_class_is_interface(new_class)) {
        ...
        guint16 new_class_interface_count =
            m_class_get_interface_count(new_class);
        for (j = 0; j < new_class_interface_count; ++j) {
          if (verifier_class_is_assignable_from(new_class_interfaces[j],
                                                old_class)) {
      ...
      goto match_found;
    } else if (is_compatible_boxed_valuetype(ctx, old_type, new_type, new_slot,
                                             FALSE) ||
               is_compatible_boxed_valuetype(ctx, new_type, old_type, old_slot,
                                             FALSE)) {
      match_class = mono_defaults.object_class;
      goto match_found;
    }
    ...
    set_stack_value(ctx, old_slot, m_class_get_byval_arg(new_class),
                    stack_slot_is_managed_pointer(old_slot));
    goto end_verify;

  match_found:
    g_assert(match_class);
    set_stack_value(ctx, old_slot, m_class_get_byval_arg(match_class),
                    stack_slot_is_managed_pointer(old_slot));
    set_stack_value(ctx, new_slot, m_class_get_byval_arg(match_class),
                    stack_slot_is_managed_pointer(old_slot));
    continue;
  }
...
#define HANDLER_START(clause)                                                  \
  ((clause)->flags == MONO_EXCEPTION_CLAUSE_FILTER                             \
       ? (clause)->data.filter_offset                                          \
       : clause->handler_offset)
#define IS_CATCH_OR_FILTER(clause)                                             \
  ((clause)->flags == MONO_EXCEPTION_CLAUSE_FILTER ||                          \
 ...
 * is_clause_in_range :
 *
 * Returns TRUE if either the protected block or the handler of @clause is in
 * the @start - @end range.
 */
static gboolean is_clause_in_range(MonoExceptionClause *clause, guint32 start,
                                   guint32 end) {
 ...
 */
static gboolean is_clause_inside_range(MonoExceptionClause *clause,
                                       guint32 start, guint32 end) {
  if (clause->try_offset < start ||
      (clause->try_offset + clause->try_len) > end)
    return FALSE;
  if (HANDLER_START(clause) < start ||
      (clause->handler_offset + clause->handler_len) > end)
    return FALSE;
  return TRUE;
}
 ...
 */
static gboolean is_clause_nested(MonoExceptionClause *clause,
                                 MonoExceptionClause *nested) {
  if (clause->flags == MONO_EXCEPTION_CLAUSE_FILTER &&
      is_clause_inside_range(nested, clause->data.filter_offset,
                             clause->handler_offset))
    return TRUE;
  return is_clause_inside_range(nested, clause->try_offset,
                                clause->try_offset + clause->try_len) ||
         is_clause_inside_range(nested, clause->handler_offset,
                                clause->handler_offset + clause->handler_len);
}

/* Test the relationship between 2 exception clauses. Follow  P.1 12.4.2.7 of
 * ECMA the each pair of exception must have the following properties:
 *  - one is fully nested on another (the outer must not be a filter clause)
 * (the nested one must come earlier)
 *  - completely disjoin (none of the 3 regions of each entry overlap with the
 * other 3)
 *  - mutual protection (protected block is EXACT the same, handlers are disjoin
 * and all handler are catch or all handler are filter)
 */
static void verify_clause_relationship(VerifyContext *ctx,
                                       MonoExceptionClause *clause,
                                       MonoExceptionClause *to_test) {
  /*clause is nested*/
  if (to_test->flags == MONO_EXCEPTION_CLAUSE_FILTER &&
      is_clause_inside_range(clause, to_test->data.filter_offset,
                             to_test->handler_offset)) {
  ...
  if (is_clause_nested(clause, to_test)) {
    ADD_VERIFY_ERROR(
        ctx, g_strdup_printf(
                 "Nested exception clause appears after enclosing clause"));
    return;
  }

  /*mutual protection*/
  if (clause->try_offset == to_test->try_offset &&
      clause->try_len == to_test->try_len) {
    /*handlers are not disjoint*/
    if (is_clause_in_range(to_test, HANDLER_START(clause),
                           clause->handler_offset + clause->handler_len)) {
    ...
    if (!IS_CATCH_OR_FILTER(clause) || !IS_CATCH_OR_FILTER(to_test)) {
      ADD_VERIFY_ERROR(
          ctx, g_strdup_printf("Exception clauses with shared protected block "
                               "are neither catch or filter"));
      return;
    }
    /*OK*/
    return;
  ...
  if ((is_clause_in_range(to_test, clause->try_offset,
                          clause->try_offset + clause->try_len) ||
       is_clause_in_range(to_test, HANDLER_START(clause),
                          clause->handler_offset + clause->handler_len)) &&
      !is_clause_nested(to_test, clause))
...
#define code_bounds_check(size)                                                \
  if (ADDP_IS_GREATER_OR_OVF(ip, size, end)) {                                 \
    ADD_VERIFY_ERROR(                                                          \
        &ctx, g_strdup_printf("Code overrun starting with 0x%x at 0x%04x",     \
                              *ip, ctx.ip_offset));                            \
    break;                                                                     \
  }

static gboolean mono_opcode_is_prefix(int op) {
  switch (op) {
...
GSList *mono_method_verify(MonoMethod *method, int level) {
  ...
  if (method->iflags & (METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL |
      ...
      (method->flags &
       (METHOD_ATTRIBUTE_PINVOKE_IMPL | METHOD_ATTRIBUTE_ABSTRACT))) {
    return NULL;
  }
  ...
  if (!method->is_generic && !mono_class_is_gtd(method->klass) &&
      ctx.signature->has_type_parameters) {
    ADD_VERIFY_ERROR(
        &ctx, g_strdup_printf(
                  "Method and signature don't match in terms of genericity"));
    finish_collect_stats();
    return ctx.list;
  }
  ...
  if (!ctx.header) {
    ADD_VERIFY_ERROR(&ctx,
                     g_strdup_printf("Could not decode method header due to %s",
  ...
  ctx.locals = (MonoType **)g_memdup(
      ctx.header->locals, sizeof(MonoType *) * ctx.header->num_locals);
  ...
  if (ctx.num_locals > 0 && !ctx.header->init_locals)
    CODE_NOT_VERIFIABLE(
        &ctx, g_strdup_printf(
                  "Method with locals variable but without init locals set"));

  ctx.params = g_new(MonoType *, ctx.max_args);
  _MEM_ALLOC(sizeof(MonoType *) * ctx.max_args);

  if (ctx.signature->hasthis)
    ctx.params[0] = m_class_is_valuetype(method->klass)
                        ? m_class_get_this_arg(method->klass)
                        : m_class_get_byval_arg(method->klass);
  memcpy(ctx.params + ctx.signature->hasthis, ctx.signature->params,
         sizeof(MonoType *) * ctx.signature->param_count);

  if (ctx.signature->is_inflated)
    ctx.generic_context = generic_context = mono_method_get_context(method);

  if (!generic_context &&
      (mono_class_is_gtd(method->klass) || method->is_generic)) {
    if (method->is_generic)
      ctx.generic_context = generic_context =
          &(mono_method_get_generic_container(method)->context);
    else
      ctx.generic_context = generic_context =
          &mono_class_get_generic_container(method->klass)->context;
  }

  for (i = 0; i < ctx.num_locals; ++i) {
    MonoType *uninflated = ctx.locals[i];
    ctx.locals[i] = mono_class_inflate_generic_type_checked(
        ctx.locals[i], ctx.generic_context, error);
    if (!is_ok(error)) {
      char *name =
          mono_type_full_name(ctx.locals[i] ? ctx.locals[i] : uninflated);
      ADD_VERIFY_ERROR(&ctx,
                       g_strdup_printf("Invalid local %d of type %s", i, name));
      g_free(name);
      mono_error_cleanup(error);
      /* we must not free (in cleanup) what was not yet allocated (but only
       * copied) */
      ctx.num_locals = i;
      ctx.max_args = 0;
      goto cleanup;
  ...
  for (i = 0; i < ctx.max_args; ++i) {
    ...
    ctx.params[i] = mono_class_inflate_generic_type_checked(
        ctx.params[i], ctx.generic_context, error);
    if (!is_ok(error)) {
      char *name =
          mono_type_full_name(ctx.params[i] ? ctx.params[i] : uninflated);
      ADD_VERIFY_ERROR(
          &ctx, g_strdup_printf("Invalid parameter %d of type %s", i, name));
      g_free(name);
      mono_error_cleanup(error);
      /* we must not free (in cleanup) what was not yet allocated (but only
       * copied) */
  ...
  for (i = 0; i < ctx.num_locals; ++i) {
    ...
    if (get_stack_type(ctx.locals[i]) == TYPE_INV) {
      ...
      ADD_VERIFY_ERROR(&ctx,
                       g_strdup_printf("Invalid local %i of type %s", i, name));
      g_free(name);
      break;
    }
  }
  ...
  for (i = 0; i < ctx.max_args; ++i) {
    ...
    if (get_stack_type(ctx.params[i]) == TYPE_INV) {
      ...
      ADD_VERIFY_ERROR(
          &ctx, g_strdup_printf("Invalid parameter %i of type %s", i, name));
      g_free(name);
      break;
    }
  }
  ...
  for (i = 0; i < ctx.header->num_clauses && ctx.valid; ++i) {
    ...
    VERIFIER_DEBUG(
        printf("clause try %x len %x filter at %x handler at %x len %x\n",
               clause->try_offset, clause->try_len, clause->data.filter_offset,
               clause->handler_offset, clause->handler_len););

    if (clause->try_offset > ctx.code_size ||
        ADD_IS_GREATER_OR_OVF(clause->try_offset, clause->try_len,
                              ctx.code_size))
      ADD_VERIFY_ERROR(&ctx,
                       g_strdup_printf("try clause out of bounds at 0x%04x",
                                       clause->try_offset));

    if (clause->try_len <= 0)
      ADD_VERIFY_ERROR(&ctx, g_strdup_printf("try clause len <= 0 at 0x%04x",
                                             clause->try_offset));

    if (clause->handler_offset > ctx.code_size ||
        ADD_IS_GREATER_OR_OVF(clause->handler_offset, clause->handler_len,
                              ctx.code_size))
      ADD_VERIFY_ERROR(&ctx,
                       g_strdup_printf("handler clause out of bounds at 0x%04x",
                                       clause->try_offset));

    if (clause->handler_len <= 0)
      ADD_VERIFY_ERROR(&ctx,
                       g_strdup_printf("handler clause len <= 0 at 0x%04x",
                                       clause->try_offset));
    ...
    if (clause->try_offset < clause->handler_offset &&
      ...
      ADD_VERIFY_ERROR(
          &ctx, g_strdup_printf(
                    "try block (at 0x%04x) includes handler block (at 0x%04x)",
                    clause->try_offset, clause->handler_offset));

    if (clause->flags == MONO_EXCEPTION_CLAUSE_FILTER) {
      if (clause->data.filter_offset > ctx.code_size)
        ADD_VERIFY_ERROR(
            &ctx, g_strdup_printf("filter clause out of bounds at 0x%04x",
                                  clause->try_offset));
      ...
      if (clause->data.filter_offset >= clause->handler_offset)
        ADD_VERIFY_ERROR(
            &ctx,
            g_strdup_printf(
                "filter clause must come before the handler clause at 0x%04x",
                clause->data.filter_offset));
    }
    ...
    if (clause->try_offset + clause->try_len < ctx.code_size)
      ctx.code[clause->try_offset + clause->try_len].flags |=
          IL_CODE_FLAG_WAS_TARGET;
    if (clause->handler_offset + clause->handler_len < ctx.code_size)
      ctx.code[clause->handler_offset + clause->handler_len].flags |=
          IL_CODE_FLAG_WAS_TARGET;

    if (clause->flags == MONO_EXCEPTION_CLAUSE_NONE) {
      if (!clause->data.catch_class) {
        ADD_VERIFY_ERROR(
            &ctx, g_strdup_printf("Catch clause %d with invalid type", i));
        break;
      }
      if (!mono_type_is_valid_in_context(
              &ctx, m_class_get_byval_arg(clause->data.catch_class)))
        break;

      init_stack_with_value_at_exception_boundary(
          &ctx, ctx.code + clause->handler_offset, clause->data.catch_class);
    } else if (clause->flags == MONO_EXCEPTION_CLAUSE_FILTER) {
      init_stack_with_value_at_exception_boundary(
          &ctx, ctx.code + clause->data.filter_offset,
          mono_defaults.exception_class);
      init_stack_with_value_at_exception_boundary(
          &ctx, ctx.code + clause->handler_offset,
          mono_defaults.exception_class);
    }
  }
  ...
  if (!is_ok(error)) {
    ADD_VERIFY_ERROR(&ctx, g_strdup_printf("Invalid branch target: %s",
                                           mono_error_get_message(error)));
    mono_error_cleanup(error);
    goto cleanup;
  }
  ...
  while (ip < end && ctx.valid) {
    ...
    ip_offset = (guint)(ip - code_start);
    {
      ...
      if (ip_offset > bb->end) {
        ADD_VERIFY_ERROR(&ctx,
                         g_strdup_printf("Branch or EH block at [0x%04x] "
                                         "targets middle instruction at 0x%04x",
                                         bb->end, ip_offset));
        goto cleanup;
      }
      ...
      if (op_size == -1) {
        ADD_VERIFY_ERROR(&ctx,
                         g_strdup_printf("Invalid instruction %x at 0x%04x",
                                         *ip, ip_offset));
        goto cleanup;
      }
      ...
      if (ADD_IS_GREATER_OR_OVF(ip_offset, op_size, bb->end)) {
        ADD_VERIFY_ERROR(
            &ctx,
            g_strdup_printf(
                "Branch or EH block targets middle of instruction at 0x%04x",
                ip_offset));
        goto cleanup;
      }
      ...
      if (ip_offset + op_size == bb->end && mono_opcode_is_prefix(op)) {
        ADD_VERIFY_ERROR(
            &ctx, g_strdup_printf("Branch or EH block targets between prefix "
                                  "'%s' and instruction at 0x%04x",
                                  mono_opcode_name(op), ip_offset));
        goto cleanup;
      }
    }
     ...
     * For fallout we check the once a protected block ends, if the start flag
     * is not set. Likewise for fallthru in, we check if ip is the start of a
     * protected block and start is not set
    ...
    for (i = 0; i < ctx.header->num_clauses && ctx.valid; ++i) {
      ...
      if ((clause->handler_offset + clause->handler_len == ip_offset) &&
          start == 0) {
        if (clause->flags == MONO_EXCEPTION_CLAUSE_FILTER)
          ADD_VERIFY_ERROR(
              &ctx,
      ...
      if (clause->flags == MONO_EXCEPTION_CLAUSE_FILTER &&
          clause->handler_offset == ip_offset && start == 0) {
        ADD_VERIFY_ERROR(
            &ctx,
      ...
      if (clause->try_offset == ip_offset && ctx.eval.size > 0 && start == 0) {
        ADD_VERIFY_ERROR(
            &ctx, g_strdup_printf(
                      "Try to enter try block with a non-empty stack at 0x%04x",
                      ip_offset));
        start = 1;
      }
    }
    ...
    if (bb->dead) {
      /*FIXME remove this once we move all bad branch checking code to use BB
       * only*/
    ...
    /*TODO we can fast detect a forward branch or exception block targeting code
     * after prefix, we should fail fast*/
#ifdef MONO_VERIFIER_DEBUG
    {
      char *discode;
    ...
    case CEE_LDNULL:
      if (check_overflow(&ctx))
        stack_push_val(&ctx, TYPE_COMPLEX | NULL_LITERAL_MASK,
    ...
    case CEE_JMP:
      ...
      if (ctx.eval.size)
        ADD_VERIFY_ERROR(
            &ctx, g_strdup_printf("Eval stack must be empty in jmp at 0x%04x",
                                  ip_offset));
      /* token = read32 (ip + 1); */
      if (in_any_block(ctx.header, ip_offset))
        ADD_VERIFY_ERROR(
            &ctx,
            g_strdup_printf("jmp cannot escape exception blocks at 0x%04x",
                            ip_offset));

      CODE_NOT_VERIFIABLE(
          &ctx, g_strdup_printf("Intruction jmp is not verifiable at 0x%04x",
                                ctx.ip_offset));
      /*
    ...
    case CEE_CALLI:
       ...
       */
      CODE_NOT_VERIFIABLE(
          &ctx, g_strdup_printf("Intruction calli is not verifiable at 0x%04x",
                                ctx.ip_offset));
      ip += 5;
      break;
    case CEE_BR_S:
    ...
    case CEE_SWITCH: {
      ...
      if (entries > 0xFFFFFFFFU / sizeof(guint32))
        ADD_VERIFY_ERROR(&ctx,
                         g_strdup_printf("Too many switch entries %x at 0x%04x",
                                         entries, ctx.ip_offset));
    ...
    case CEE_ISINST:
      ...
      do_cast(&ctx, read32(ip + 1),
              *ip == CEE_CASTCLASS ? "castclass" : "isinst");
      ip += 5;
      break;
    ...
    case CEE_ENDFINALLY:
      if (!is_correct_endfinally(ctx.header, ip_offset))
        ADD_VERIFY_ERROR(&ctx,
                         g_strdup_printf("endfinally must be used inside a "
                                         "finally/fault handler at 0x%04x",
                                         ctx.ip_offset));
      ctx.eval.size = 0;
      start = 1;
      ++ip;
    ...
    case CEE_PREFIX1:
      ...
      case CEE_ARGLIST:
        ...
        if (ctx.signature->call_convention != MONO_CALL_VARARG)
          ADD_VERIFY_ERROR(
              &ctx, g_strdup_printf("Cannot use arglist on method without "
                                    "VARGARG calling convention at 0x%04x",
                                    ctx.ip_offset));
        set_stack_value(
            &ctx, stack_push(&ctx),
            m_class_get_byval_arg(mono_defaults.argumenthandle_class), FALSE);
        ++ip;
        break;
      ...
      case CEE_TAIL_:
        ...
        ++ip;
        if (ip < end &&
            (*ip != CEE_CALL && *ip != CEE_CALLI && *ip != CEE_CALLVIRT))
          ADD_VERIFY_ERROR(
              &ctx,
              g_strdup_printf(
                  "tail prefix must be used only with call opcodes at 0x%04x",
                  ip_offset));
        break;
      ...
      case CEE_CONSTRAINED_:
        code_bounds_check(5);
        ctx.constrained_type =
            get_boxable_mono_type(&ctx, read32(ip + 1), "constrained.");
      ...
      case CEE_CPBLK:
        ...
        CODE_NOT_VERIFIABLE(
            &ctx,
            g_strdup_printf("Instruction cpblk is not verifiable at 0x%04x",
                            ctx.ip_offset));
        ip++;
        break;
      ...
      case CEE_INITBLK:
        ...
        CODE_NOT_VERIFIABLE(
            &ctx,
            g_strdup_printf("Instruction initblk is not verifiable at 0x%04x",
                            ctx.ip_offset));
        ip++;
        break;
      ...
      case CEE_RETHROW:
        if (!is_correct_rethrow(ctx.header, ip_offset))
          ADD_VERIFY_ERROR(
              &ctx, g_strdup_printf(
                        "rethrow must be used inside a catch handler at 0x%04x",
                        ctx.ip_offset));
        ctx.eval.size = 0;
        start = 1;
        ++ip;
      ...
      default:
        ADD_VERIFY_ERROR(&ctx,
                         g_strdup_printf("Invalid instruction FE %x at 0x%04x",
                                         *ip, ctx.ip_offset));
        ++ip;
      }
      break;
    ...
    default:
      ADD_VERIFY_ERROR(&ctx, g_strdup_printf("Invalid instruction %x at 0x%04x",
                                             *ip, ctx.ip_offset));
      ++ip;
    }
    ...
    if (prefix) {
      ...
      if (ctx.prefix_set & PREFIX_CONSTRAINED)
        ADD_VERIFY_ERROR(
            &ctx, g_strdup_printf(
                      "Invalid instruction after constrained prefix at 0x%04x",
                      ctx.ip_offset));
      if (ctx.prefix_set & PREFIX_READONLY)
        ADD_VERIFY_ERROR(
            &ctx, g_strdup_printf(
                      "Invalid instruction after readonly prefix at 0x%04x",
                      ctx.ip_offset));
      if (ctx.prefix_set & PREFIX_VOLATILE)
        ADD_VERIFY_ERROR(
            &ctx, g_strdup_printf(
                      "Invalid instruction after volatile prefix at 0x%04x",
                      ctx.ip_offset));
      if (ctx.prefix_set & PREFIX_UNALIGNED)
        ADD_VERIFY_ERROR(
            &ctx, g_strdup_printf(
                      "Invalid instruction after unaligned prefix at 0x%04x",
                      ctx.ip_offset));
  ...
  if ((ip != end || !start) && ctx.verifiable && !ctx.list) {
    ADD_VERIFY_ERROR(
        &ctx, g_strdup_printf("Run ahead of method code at 0x%04x", ip_offset));
  }

  /*We should guard against the last decoded opcode, otherwise we might add
   * errors that doesn't make sense.*/
  for (i = 0; i < ctx.code_size && i < ip_offset; ++i) {
    if (ctx.code[i].flags & IL_CODE_FLAG_WAS_TARGET) {
      if (!(ctx.code[i].flags & IL_CODE_FLAG_SEEN))
        ADD_VERIFY_ERROR(&ctx,
                         g_strdup_printf("Branch or exception block target "
                                         "middle of instruction at 0x%04x",
                                         i));
    ...
    if ((ctx.code[i].flags & IL_CODE_LDFTN_DELEGATE_NONFINAL_VIRTUAL) &&
        ctx.has_this_store)
      CODE_NOT_VERIFIABLE(&ctx,
                          g_strdup_printf("Invalid ldftn with virtual function "
                                          "in method with stdarg 0 at  0x%04x",
    ...
    if ((ctx.code[i].flags & IL_CODE_CALL_NONFINAL_VIRTUAL) &&
        ctx.has_this_store)
      CODE_NOT_VERIFIABLE(
          &ctx,
          g_strdup_printf("Invalid call to a non-final virtual function in "
                          "method with stdarg.0 or ldarga.0 at  0x%04x",
                          i));
  }

  if (mono_method_is_constructor(ctx.method) && !ctx.super_ctor_called &&
      !m_class_is_valuetype(ctx.method->klass) &&
      ctx.method->klass != mono_defaults.object_class) {
    char *method_name = mono_method_full_name(ctx.method, TRUE);
    char *type = mono_type_get_full_name(ctx.method->klass);
    if (m_class_get_parent(ctx.method->klass) &&
        mono_class_has_failure(m_class_get_parent(ctx.method->klass)))
      CODE_NOT_VERIFIABLE(
          &ctx,
          g_strdup_printf("Constructor %s for type %s not calling base type "
                          "ctor due to a TypeLoadException on base type.",
                          method_name, type));
    else
      CODE_NOT_VERIFIABLE(
          &ctx, g_strdup_printf(
                    "Constructor %s for type %s not calling base type ctor.",
                    method_name, type));
    g_free(method_name);
    g_free(type);
  }
...
gboolean mono_verifier_is_enabled_for_method(MonoMethod *method) {
  return mono_verifier_is_enabled_for_class(method->klass) &&
...
/*
 * Dynamic methods are not considered full trust since if the user is trusted
 * and need to generate unsafe code, make the method skip verification - this is
 * a known good way to do it.
 */
 ...
 *
 * Under verify_all all user code must be verifiable if no security option was
 * set
 *
 */
gboolean mono_verifier_is_class_full_trust(MonoClass *klass) {
  MonoImage *image = m_class_get_image(klass);
  /* under CoreCLR code is trusted if it is part of the "platform" otherwise all
   * code inside the GAC is trusted */
  ...
  return verifier_mode < MONO_VERIFIER_MODE_VERIFIABLE || trusted_location ||
         image == mono_defaults.corlib;
}

GSList *mono_method_verify_with_current_settings(MonoMethod *method,
                                                 gboolean skip_visibility,
                                                 gboolean is_fulltrust) {
  return mono_method_verify(
      method,
      (verifier_mode != MONO_VERIFIER_MODE_STRICT ? MONO_VERIFY_NON_STRICT
                                                  : 0) |
...
static int get_field_end(MonoClassField *field) {
  int align;
...
verify_class_for_overlapping_reference_fields(MonoClass *klass) {
  ...
  /*We can't skip types with !has_references since this is calculated after we
   * have run.*/
  if (!mono_class_is_explicit_layout(klass))
    return TRUE;

  /*We must check for stuff overlapping reference fields.
    The outer loop uses mono_class_get_fields_internal to ensure that
    MonoClass:fields get inited.
  */
  while ((field = mono_class_get_fields_internal(klass, &iter))) {
    ...
    for (j = i; j < fcount; ++j) {
      ...
      if (mono_field_is_deleted(other) ||
          (is_valuetype && !MONO_TYPE_IS_REFERENCE(other->type)) ||
          (other->type->attrs & FIELD_ATTRIBUTE_STATIC))
        continue;

      if (!is_valuetype && MONO_TYPE_IS_REFERENCE(other->type) &&
          field->offset == other->offset && is_fulltrust)
        continue;

      if ((otherEnd > field->offset && otherEnd <= fieldEnd) ||
          (other->offset >= field->offset && other->offset < fieldEnd))
        return FALSE;
    }
  }
  return TRUE;
...
static guint field_hash(gconstpointer key) {
  const MonoClassField *field = (const MonoClassField *)key;
  return g_str_hash(field->name) ^ mono_metadata_type_hash(field->type); /**/
}

static gboolean field_equals(gconstpointer _a, gconstpointer _b) {
  const MonoClassField *a = (const MonoClassField *)_a;
  const MonoClassField *b = (const MonoClassField *)_b;
  return !strcmp(a->name, b->name) &&
         mono_metadata_type_equal(a->type, b->type);
}

static gboolean verify_class_fields(MonoClass *klass) {
  gpointer iter = NULL;
  MonoClassField *field;
  MonoGenericContext *context = mono_class_get_context(klass);
  GHashTable *unique_fields =
      g_hash_table_new_full(&field_hash, &field_equals, NULL, NULL);
...
static gboolean verify_interfaces(MonoClass *klass) {
  int i;
...
static gboolean verify_valuetype_layout_with_target(MonoClass *klass,
                                                    MonoClass *target_class) {
  int type;
  gpointer iter = NULL;
  MonoClassField *field;
  ...
  while ((field = mono_class_get_fields_internal(klass, &iter)) != NULL) {
    ...
    if (field->type->attrs &
        (FIELD_ATTRIBUTE_STATIC | FIELD_ATTRIBUTE_HAS_FIELD_RVA))
      continue;

    field_class = mono_class_get_generic_type_definition(
        mono_class_from_mono_type_internal(field->type));

    if (field_class == target_class || klass == field_class ||
        !verify_valuetype_layout_with_target(field_class, target_class))
      return FALSE;
  }
...
static gboolean verify_valuetype_layout(MonoClass *klass) {
  gboolean res;
  res = verify_valuetype_layout_with_target(klass, klass);
  return res;
}

static gboolean recursive_mark_constraint_args(MonoBitSet *used_args,
                                               MonoGenericContainer *gc,
                                               MonoType *type) {
  int idx;
  MonoClass **constraints;
  ...
  for (constraints = param_info->constraints; *constraints; ++constraints) {
    ...
    if (mono_type_is_generic_argument(constraint_type) &&
        !recursive_mark_constraint_args(used_args, gc, constraint_type))
      return FALSE;
  }
  return TRUE;
}

static gboolean verify_generic_parameters(MonoClass *klass) {
  int i;
  ...
  for (i = 0; i < gc->type_argc; ++i) {
    ...
    for (constraints = param_info->constraints; *constraints; ++constraints) {
      ...
      if (mono_type_is_generic_argument(constraint_type) &&
          !recursive_mark_constraint_args(used_args, gc, constraint_type))
        goto fail;
      if (mono_class_is_ginst(ctr) &&
          !mono_class_is_valid_generic_instantiation(NULL, ctr))
        goto fail;
    }
  }
 ...
 *
 * Right now there are no conditions that make a class a valid but not
 * verifiable. Both overlapping reference field and invalid generic
 * instantiation are fatal errors.
 *
 * This method must be safe to be called from mono_class_init_internal and all
 * code must be carefull about that.
 *
 */
gboolean mono_verifier_verify_class(MonoClass *klass) {
  ...
  if (!klass_parent && klass != mono_defaults.object_class &&
  ...
  if (m_class_get_parent(klass)) {
    ...
    if (mono_class_is_ginst(klass_parent) && !mono_class_is_ginst(klass)) {
      ...
      if (mono_class_is_gtd(klass))
        context = &mono_class_get_generic_container(klass)->context;
      if (!mono_type_is_valid_type_in_context(
              m_class_get_byval_arg(klass_parent), context))
        return FALSE;
    }
  }
