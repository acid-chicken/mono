...
// In DllImportSearchPath enum, bit 0x2 represents AssemblyDirectory. It is not
// passed on and is instead handled by the runtime.
#define DLLIMPORTSEARCHPATH_ASSEMBLYDIRECTORY 0x2

// This lock may be taken within an ALC lock, and should never be the other way
// around.
static MonoCoopMutex native_library_module_lock;
static GHashTable *native_library_module_map;
/*
 * This blacklist is used as a set for cache invalidation purposes with netcore
 * pinvokes. When pinvokes are resolved with anything other than the last-chance
 * managed event, the results of that lookup are added to an ALC-level cache.
 * However, if a library is then unloaded with NativeLibrary.Free(), this cache
 * should be invalidated so that a newly called pinvoke will not attempt to use
 * it, hence the blacklist. This design means that if another library is loaded
 * at the same address, it will function with a perf hit, as the entry will
 * repeatedly be added and removed from the cache due to its presence in the
 * blacklist. This is a rare scenario and considered a worthwhile tradeoff.
 */
...
static GHashTable *global_module_map; // should only be accessed with the global
                                      // loader data lock
...
GENERATE_GET_CLASS_WITH_CACHE(appdomain_unloaded_exception, "System",
                              "AppDomainUnloadedException")
GENERATE_TRY_GET_CLASS_WITH_CACHE(appdomain_unloaded_exception, "System",
                                  "AppDomainUnloadedException")
#ifdef ENABLE_NETCORE
GENERATE_GET_CLASS_WITH_CACHE(native_library, "System.Runtime.InteropServices",
                              "NativeLibrary");
#endif
 ...
 * For the global DllMap, this is `global_loader_data_mutex`, and for images
 * it's their internal lock.
 */
static int mono_dllmap_lookup_list(MonoDllMap *dll_map, const char *dll,
                                   const char *func, const char **rdll,
                                   const char **rfunc) {
  int found = 0;
  ...
  *rfunc = func;
  ...
  for (; dll_map; dll_map = dll_map->next) {
    ...
    gboolean case_insensitive_match =
        strncmp(dll_map->dll, "i:", 2) == 0 &&
        g_ascii_strcasecmp(dll_map->dll + 2, dll) == 0;
...
/*
 * The locking and GC state transitions here are wonky due to the fact the image
 * lock is a coop lock and the global loader data lock is an OS lock.
 */
static int mono_dllmap_lookup(MonoImage *assembly, const char *dll,
                              const char *func, const char **rdll,
                              const char **rfunc) {
  int res;
...
static void dllmap_insert_global(const char *dll, const char *func,
...
static void dllmap_insert_image(MonoImage *assembly, const char *dll,
                                const char *func, const char *tdll,
                                const char *tfunc) {
  MonoDllMap *entry;
  ...
  entry->target_func = tfunc
                           ? mono_image_strdup(assembly, tfunc)
                           : (func ? mono_image_strdup(assembly, func) : NULL);
 ...
 * it's their internal lock.
 */
static void free_dllmap(MonoDllMap *map) {
  while (map) {
...
void mono_dllmap_insert_internal(MonoImage *assembly, const char *dll,
                                 const char *func, const char *tdll,
                                 const char *tfunc) {
  mono_loader_init();
  // The locking in here is _really_ wonky, and I'm not convinced this function
  // should exist. I've split it into an internal version to offer flexibility
  // in the future.
  if (!assembly)
...
void mono_global_dllmap_cleanup(void) {
  // No need for a transition here since the thread is already detached from the
  // runtime
  mono_global_loader_data_lock();
  ...
  // We don't care about freeing native_library_module_map on netcore because of
...
/**
 * mono_dllmap_insert:
 * \param assembly if NULL, this is a global mapping, otherwise the remapping of
 * the dynamic library will only apply to the specified assembly \param dll The
 * name of the external library, as it would be found in the \c DllImport
 * declaration.  If prefixed with <code>i:</code> the matching of the library
 * name is done without case sensitivity \param func if not null, the mapping
 * will only applied to the named function (the value of
 * <code>EntryPoint</code>) \param tdll The name of the library to map the
 * specified \p dll if it matches. \param tfunc The name of the function that
 * replaces the invocation.  If NULL, it is replaced with a copy of \p func.
 *
 * LOCKING: Acquires the image lock, or the loader data lock if an image is not
 * passed.
 *
 ...
 * references in a \c DllImport attribute from the \p dll library name into the
 * \p tdll name. If the \p dll name contains the prefix <code>i:</code>, the
 * comparison of the library name is done without case sensitivity.
 *
 * If you pass \p func, this is the name of the \c EntryPoint in a \c DllImport
 * if specified or the name of the function as determined by \c DllImport. If
 * you pass \p func, you must also pass \p tfunc which is the name of the target
 * function to invoke on a match.
 *
 * Example:
 *
 * <code>mono_dllmap_insert (NULL, "i:libdemo.dll", NULL, relocated_demo_path,
 * NULL);</code>
 *
 * The above will remap \c DllImport statements for \c libdemo.dll and \c
 * LIBDEMO.DLL to the contents of \c relocated_demo_path for all assemblies in
 * the Mono process.
 *
 * NOTE: This can be called before the runtime is initialized, for example from
 * \c mono_config_parse.
 */
void mono_dllmap_insert(MonoImage *assembly, const char *dll, const char *func,
                        const char *tdll, const char *tfunc) {
#ifndef DISABLE_DLLMAP
...
#ifdef ENABLE_NETCORE
static MonoDl *mono_loader_register_module_locking(const char *name,
                                                   MonoDl *module) {
  MonoDl *result = NULL;
...
static void remove_cached_module(gpointer key, gpointer value,
                                 gpointer user_data) {
...
void mono_global_loader_cache_cleanup(void) {
  ...
  // No need to clean up the native library hash tables since they're
  // netcore-only, where this is never called
}

static gboolean is_absolute_path(const char *path) {
  // FIXME: other platforms have similar prefixes, such as $ORIGIN
#ifdef HOST_DARWIN
  if (!strncmp(path, "@executable_path/", 17) ||
      !strncmp(path, "@loader_path/", 13) || !strncmp(path, "@rpath/", 7))
    return TRUE;
#endif
  return g_path_is_absolute(path);
}

static gpointer lookup_pinvoke_call_impl(MonoMethod *method,
                                         MonoLookupPInvokeStatus *status_out);

static gpointer pinvoke_probe_for_symbol(MonoDl *module,
                                         MonoMethodPInvoke *piinfo,
                                         const char *import,
                                         char **error_msg_out);

static void
pinvoke_probe_convert_status_for_api(MonoLookupPInvokeStatus *status,
...
static void
pinvoke_probe_convert_status_to_error(MonoLookupPInvokeStatus *status,
                                      MonoError *error) {
  /* Note: this has to return a MONO_ERROR_GENERIC because
   * mono_mb_emit_exception_for_error only knows how to decode generic errors.
   */
  switch (status->err_code) {
  case LOOKUP_PINVOKE_ERR_OK:
    return;
  case LOOKUP_PINVOKE_ERR_NO_LIB:
    mono_error_set_generic_error(error, "System", "DllNotFoundException", "%s",
                                 status->err_arg);
    g_free(status->err_arg);
    status->err_arg = NULL;
    break;
  case LOOKUP_PINVOKE_ERR_NO_SYM:
    mono_error_set_generic_error(error, "System", "EntryPointNotFoundException",
                                 "%s", status->err_arg);
    g_free(status->err_arg);
    status->err_arg = NULL;
    break;
 ...
 */
gpointer mono_lookup_pinvoke_call(MonoMethod *method, const char **exc_class,
                                  const char **exc_arg) {
  gpointer result;
...
gpointer mono_lookup_pinvoke_call_internal(MonoMethod *method,
                                           MonoError *error) {
  gpointer result;
  MonoLookupPInvokeStatus status;
  memset(&status, 0, sizeof(status));
...
static void native_library_lock(void) {
  mono_coop_mutex_lock(&native_library_module_lock);
}

static void native_library_unlock(void) {
  mono_coop_mutex_unlock(&native_library_module_lock);
}

static void alc_pinvoke_lock(MonoAssemblyLoadContext *alc) {
  mono_coop_mutex_lock(&alc->pinvoke_lock);
}

static void alc_pinvoke_unlock(MonoAssemblyLoadContext *alc) {
  mono_coop_mutex_unlock(&alc->pinvoke_lock);
}

// LOCKING: expects you to hold native_library_module_lock
static MonoDl *netcore_handle_lookup(gpointer handle) {
  return (MonoDl *)g_hash_table_lookup(native_library_module_map, handle);
}

// LOCKING: expects you to hold native_library_module_lock
static gboolean netcore_check_blacklist(MonoDl *module) {
  return g_hash_table_contains(native_library_module_blacklist, module);
}

static MonoDl *netcore_probe_for_module_variations(const char *mdirname,
                                                   const char *file_name) {
  void *iter = NULL;
  char *full_name;
  MonoDl *module = NULL;

  // This does not actually mirror CoreCLR's algorithm; if that becomes a
  // problem, potentially use theirs
...
static MonoDl *netcore_probe_for_module(MonoImage *image, const char *file_name,
                                        int flags) {
  MonoDl *module = NULL;
  ...
  for (int i = 0; i < pinvoke_search_directories_count && module == NULL; ++i)
    module = netcore_probe_for_module_variations(pinvoke_search_directories[i],
                                                 file_name);
...
static MonoDl *netcore_resolve_with_dll_import_resolver(
    MonoAssemblyLoadContext *alc, MonoAssembly *assembly, const char *scope,
    guint32 flags, MonoError *error) {
  MonoDl *result = NULL;
  gpointer lib = NULL;
  ...
  resolve = mono_class_get_method_from_name_checked(
      native_lib_class, "MonoLoadLibraryCallbackStub", -1, 0, local_error);
...
static MonoDl *netcore_resolve_with_dll_import_resolver_nofail(
  ...
  result = netcore_resolve_with_dll_import_resolver(alc, assembly, scope, flags,
                                                    error);
  if (!is_ok(error))
    mono_trace(
        G_LOG_LEVEL_DEBUG, MONO_TRACE_DLLIMPORT,
        "Error while invoking ALC DllImportResolver(\"%s\") delegate: '%s'",
        scope, mono_error_get_message(error));
...
static MonoDl *netcore_resolve_with_load(MonoAssemblyLoadContext *alc,
                                         const char *scope, MonoError *error) {
  MonoDl *result = NULL;
  gpointer lib = NULL;
  ...
  g_assert(alc_class);
  resolve = mono_class_get_method_from_name_checked(
      alc_class, "MonoResolveUnmanagedDll", -1, 0, local_error);
  mono_error_assert_ok(local_error);
  ...
  MonoStringHandle scope_handle =
      mono_string_new_handle(mono_alc_domain(alc), scope, error);
  goto_if_nok(error, leave);
...
static MonoDl *netcore_resolve_with_load_nofail(MonoAssemblyLoadContext *alc,
                                                const char *scope) {
  MonoDl *result = NULL;
  ERROR_DECL(error);
  ...
  if (!is_ok(error))
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_DLLIMPORT,
               "Error while invoking ALC LoadUnmanagedDll(\"%s\") method: '%s'",
               scope, mono_error_get_message(error));
...
static MonoDl *
netcore_resolve_with_resolving_event(MonoAssemblyLoadContext *alc,
                                     MonoAssembly *assembly, const char *scope,
                                     MonoError *error) {
  MonoDl *result = NULL;
  gpointer lib = NULL;
  MonoDomain *domain = mono_alc_domain(alc);
  ...
  g_assert(alc_class);
  resolve = mono_class_get_method_from_name_checked(
      alc_class, "MonoResolveUnmanagedDllUsingEvent", -1, 0, local_error);
  mono_error_assert_ok(local_error);
...
static MonoDl *netcore_resolve_with_resolving_event_nofail(
    MonoAssemblyLoadContext *alc, MonoAssembly *assembly, const char *scope) {
  MonoDl *result = NULL;
  ERROR_DECL(error);
  ...
  if (!is_ok(error))
    mono_trace(
        G_LOG_LEVEL_DEBUG, MONO_TRACE_DLLIMPORT,
        "Error while invoking ALC ResolvingUnmangedDll(\"%s\") event: '%s'",
        scope, mono_error_get_message(error));
...
static MonoDl *netcore_check_alc_cache(MonoAssemblyLoadContext *alc,
                                       const char *scope) {
  MonoDl *result = NULL;
...
static MonoDl *netcore_lookup_native_library(MonoAssemblyLoadContext *alc,
                                             MonoImage *image,
                                             const char *scope, guint32 flags) {
  MonoDl *module = NULL;
  MonoDl *cached;
  ...
  // We allow a special name to dlopen from the running process namespace, which
  ...
  if (strcmp(scope, "__Internal") == 0) {
    ...
    if (!module) {
      mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_DLLIMPORT,
                 "DllImport error loading library '__Internal': '%s'.",
                 error_msg);
      g_free(error_msg);
    }
  ...
  module = (MonoDl *)netcore_resolve_with_dll_import_resolver_nofail(
      alc, assembly, scope, flags);
  if (module)
  ...
  /* As this is last chance, I've opted not to put it in a cache, but that is
   * not necessarily the correct decision. It is rather convenient here,
   * however, because it means the global cache will only be populated by
   * libraries resolved via netcore_probe_for_module and not NativeLibrary,
   * eliminating potential races/conflicts.
   */
...
add_to_alc_cache:
   ...
   * 1. A managed callback that made use of NativeLibrary.Load, in which case
   * closing is dependent on NativeLibrary.Free
   * 2. The global cache, which is only populated by results of
   * netcore_probe_for_module. When adding to the global cache, we free the new
   * MonoDl if another thread beat us, so we don't have to repeat that here.
   */
  alc_pinvoke_lock(alc);
...
static MonoDl *cached_module_load(const char *name, int flags, char **err) {
  MonoDl *res;
 ...
 * did not apply.
 */
static gboolean legacy_probe_transform_path(const char *new_scope, int phase,
                                            char **file_name_out,
                                            char **base_name_out,
                                            char **dir_name_out,
                                            gboolean *is_absolute_out) {
  ...
  case 2:
    if (is_absolute) {
      ...
      if (strstr(base_name, "lib") != base_name) {
        ...
        base_name = tmp;
        file_name =
            g_strdup_printf("%s%s%s", dir_name, G_DIR_SEPARATOR_S, base_name);
        changed = TRUE;
      }
...
static MonoDl *legacy_probe_for_module_in_directory(const char *mdirname,
                                                    const char *file_name) {
  void *iter = NULL;
  char *full_name;
  MonoDl *module = NULL;
...
static MonoDl *
legacy_probe_for_module_relative_directories(MonoImage *image,
                                             const char *file_name) {
  MonoDl *module = NULL;
  ...
  for (int j = 0; j < 3; ++j) {
    ...
    case 1: // @executable_path@/../lib
    {
      ...
      if (binl != -1) {
        ...
        // /system/bin/app_process{32,64} depending on the application's
        // architecture. However, libraries for the different architectures live
        // in different subdirectories of `/system`: `lib` for 32-bit apps and
        // `lib64` for 64-bit ones. Thus appending `/lib` below will fail to
        // load the DSO for a 64-bit app, even if it exists there, because it
        // will have a different architecture. This is the cause of
        // https://github.com/xamarin/xamarin-android/issues/2780 and the ifdef
        // below is the fix.
...
static MonoDl *legacy_probe_for_module(MonoImage *image,
  ...
  for (i = 0; i < 5; ++i) {
    ...
    gboolean changed = legacy_probe_transform_path(
        new_scope, i, &file_name, &base_name, &dir_name, &is_absolute);
    if (!changed)
      continue;
    ...
    if (!module && is_absolute) {
      ...
      if (!module) {
        mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_DLLIMPORT,
                   "DllImport error loading library '%s': '%s'.", file_name,
                   error_msg);
        g_free(error_msg);
      }
    }
...
static MonoDl *legacy_lookup_native_library(MonoImage *image,
                                            const char *scope) {
  MonoDl *module = NULL;
...
gpointer lookup_pinvoke_call_impl(MonoMethod *method,
                                  MonoLookupPInvokeStatus *status_out) {
  ...
  if (!module) {
    mono_trace(G_LOG_LEVEL_WARNING, MONO_TRACE_DLLIMPORT,
               "DllImport unable to load library '%s'.", new_scope);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_DLLIMPORT,
             "DllImport searching in: '%s' ('%s').", new_scope,
             module->full_name);
...
static gpointer pinvoke_probe_for_symbol(MonoDl *module,
...
#endif
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_DLLIMPORT, "Searching for '%s'.",
             import);

#if !defined(ENABLE_NETCORE) ||                                                \
    defined(HOST_WIN32) // For netcore, name mangling is Windows-exclusive
  if (piinfo->piflags & PINVOKE_ATTRIBUTE_NO_MANGLE)
     ...
     */
    for (int mangle_stdcall = 0; mangle_stdcall <= 1 && addr == NULL;
         mangle_stdcall++) {
...
#endif
      for (int mangle_charset = 0; mangle_charset <= 1 && addr == NULL;
           mangle_charset++) {
        for (int mangle_param_count = 0;
             mangle_param_count <= max_managle_param_count && addr == NULL;
             mangle_param_count += 4) {
          ...
          /*
           * gcc under windows creates mangled names without the underscore, but
           * MS.NET doesn't support it, so we doesn't support it either.
           */
          if (mangle_stdcall == 1) {
            ...
            if (mangle_param_count == 0)
              ...
              /* Try brute force, since it would be very hard to compute the
               * stack usage correctly */
              param_count = mangle_param_count;

            char *mangled_stdcall_name =
                g_strdup_printf("_%s@%d", mangled_name, param_count);
...
#endif
          mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_DLLIMPORT, "Probing '%s'.",
                     mangled_name);
          ...
          if (addr)
            ...
            mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_DLLIMPORT,
                       "Could not find '%s' due to '%s'.", mangled_name,
                       error_msg);
...
gpointer ves_icall_System_Runtime_InteropServices_NativeLibrary_GetSymbol(
    gpointer lib, MonoStringHandle symbol_name_handle,
    MonoBoolean throw_on_error, MonoError *error) {
  MonoDl *module;
  ...
  module = netcore_handle_lookup(lib);
  if (!module)
    mono_error_set_generic_error(error, "System", "DllNotFoundException",
                                 "%p: %s", lib, symbol_name);
  goto_if_nok(error, leave);
  ...
  if (!symbol)
    mono_error_set_generic_error(error, "System", "EntryPointNotFoundException",
                                 "%s: %s", module->full_name, symbol_name);
  goto_if_nok(error, leave);
...
// LOCKING: expects you to hold native_library_module_lock
static MonoDl *check_native_library_cache(MonoDl *module) {
...
gpointer ves_icall_System_Runtime_InteropServices_NativeLibrary_LoadByName(
    MonoStringHandle lib_name_handle,
    MonoReflectionAssemblyHandle assembly_handle, MonoBoolean has_search_flag,
    guint32 search_flag, MonoBoolean throw_on_error, MonoError *error) {
  MonoDl *module;
  ...
  module = netcore_probe_for_module(image, lib_name,
                                    has_search_flag ? search_flag : 0x2);
  if (!module)
    mono_error_set_generic_error(error, "System", "DllNotFoundException", "%s",
                                 lib_name);
  goto_if_nok(error, leave);
...
gpointer ves_icall_System_Runtime_InteropServices_NativeLibrary_LoadFromPath(
...
#ifdef HAVE_ATEXIT
static void delete_bundled_libraries(void) {
  GSList *list;
...
static void bundle_save_library_initialize(void) {
...
void mono_loader_save_bundled_library(int fd, uint64_t offset, uint64_t size,
