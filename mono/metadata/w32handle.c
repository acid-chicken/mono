...
/*
 * This is an internal handle which is used for handling waiting for multiple
 * handles. Threads which wait for multiple handles wait on this one handle, and
 * when a handle is signalled, this handle is signalled too.
 */
...
static const gchar *mono_w32handle_ops_typename(MonoW32Type type);

const gchar *mono_w32handle_get_typename(MonoW32Type type) {
  return mono_w32handle_ops_typename(type);
}

void mono_w32handle_set_signal_state(MonoW32Handle *handle_data, gboolean state,
                                     gboolean broadcast) {
#ifdef DEBUG
  g_message("%s: setting state of %p to %s (broadcast %s)", __func__, handle,
            state ? "TRUE" : "FALSE", broadcast ? "TRUE" : "FALSE");
#endif
...
gboolean mono_w32handle_issignalled(MonoW32Handle *handle_data) {
  return handle_data->signalled;
}

static void mono_w32handle_set_in_use(MonoW32Handle *handle_data,
                                      gboolean in_use) {
  handle_data->in_use = in_use;
}

static void mono_w32handle_lock_signal_mutex(void) {
#ifdef DEBUG
...
static void mono_w32handle_unlock_signal_mutex(void) {
#ifdef DEBUG
...
void mono_w32handle_lock(MonoW32Handle *handle_data) {
  mono_coop_mutex_lock(&handle_data->signal_mutex);
}

gboolean mono_w32handle_trylock(MonoW32Handle *handle_data) {
  return mono_coop_mutex_trylock(&handle_data->signal_mutex) == 0;
}

void mono_w32handle_unlock(MonoW32Handle *handle_data) {
  mono_coop_mutex_unlock(&handle_data->signal_mutex);
}

void mono_w32handle_init(void) {
...
void mono_w32handle_cleanup(void) {
...
static gsize mono_w32handle_ops_typesize(MonoW32Type type);
 ...
 */
static MonoW32Handle *mono_w32handle_new_internal(MonoW32Type type,
  ...
  /* A linear scan should be fast enough. Start from the last allocation,
   * assuming that handles are allocated more often than they're freed. */
...
retry:
  for (; slot; slot = slot->next) {
    for (; index < HANDLES_PER_SLOT; index++) {
      ...
      if (handle_data->type == MONO_W32TYPE_UNUSED) {
        ...
        if (handle_specific)
          handle_data->specific =
              g_memdup(handle_specific, mono_w32handle_ops_typesize(type));
  ...
  handles_slots_last =
      (handles_slots_last->next = g_new0(MonoW32HandleSlot, 1));
  goto retry_from_beginning;

  /* We already went around and didn't find a slot, so let's put ourselves on
   * the empty slot we just allocated */
  slot_last = handles_slots_last;
  index_last = 0;
}

gpointer mono_w32handle_new(MonoW32Type type, gpointer handle_specific) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
             "%s: create %s handle %p", __func__,
             mono_w32handle_ops_typename(type), handle_data);

  return (gpointer)handle_data;
}

static gboolean mono_w32handle_ref_core(MonoW32Handle *handle_data);

static gboolean mono_w32handle_unref_core(MonoW32Handle *handle_data);

static void w32handle_destroy(MonoW32Handle *handle_data);

gpointer mono_w32handle_duplicate(MonoW32Handle *handle_data) {
...
gboolean mono_w32handle_lookup_and_ref(gpointer handle,
                                       MonoW32Handle **handle_data) {
  g_assert(handle_data);
...
void mono_w32handle_foreach(gboolean (*on_each)(MonoW32Handle *handle_data,
                                                gpointer user_data),
                            gpointer user_data) {
  MonoW32HandleSlot *slot;
...
static gboolean mono_w32handle_ref_core(MonoW32Handle *handle_data) {
  ...
  do {
    ...
    new_ = old + 1;
  } while (mono_atomic_cas_i32((gint32 *)&handle_data->ref, (gint32)new_,
                               (gint32)old) != (gint32)old);

  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
             "%s: ref %s handle %p, ref: %d -> %d", __func__,
             mono_w32handle_ops_typename(handle_data->type), handle_data, old,
             new_);
...
static gboolean mono_w32handle_unref_core(MonoW32Handle *handle_data) {
  ...
  do {
    old = handle_data->ref;
    if (!(old >= 1))
      g_error("%s: handle %p has ref %d, it should be >= 1", __func__,
              handle_data, old);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
             "%s: unref %s handle %p, ref: %d -> %d destroy: %s", __func__,
             mono_w32handle_ops_typename(type), handle_data, old, new_,
             new_ == 0 ? "true" : "false");

  return new_ == 0;
}

static void mono_w32handle_ops_close(MonoW32Type type,
                                     gpointer handle_specific);
...
void mono_w32handle_unref(MonoW32Handle *handle_data) {
  gboolean destroy;
...
void mono_w32handle_register_ops(MonoW32Type type,
                                 const MonoW32HandleOps *ops) {
  handle_ops[type] = ops;
}
...
static gboolean mono_w32handle_test_capabilities(MonoW32Handle *handle_data,
                                                 MonoW32HandleCapability caps) {
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
             "%s: testing 0x%x against 0x%x (%d)", __func__,
             handle_caps[handle_data->type], caps,
             handle_caps[handle_data->type] & caps);

  return (handle_caps[handle_data->type] & caps) != 0;
}

static void mono_w32handle_ops_close(MonoW32Type type, gpointer data) {
  const MonoW32HandleOps *ops = handle_ops[type];
  if (ops && ops->close)
    ops->close(data);
}

static void mono_w32handle_ops_details(MonoW32Handle *handle_data) {
  if (handle_ops[handle_data->type] &&
      handle_ops[handle_data->type]->details != NULL)
...
static gint32 mono_w32handle_ops_signal(MonoW32Handle *handle_data) {
...
static gboolean mono_w32handle_ops_own(MonoW32Handle *handle_data,
                                       gboolean *abandoned) {
  if (handle_ops[handle_data->type] &&
      handle_ops[handle_data->type]->own_handle)
    return handle_ops[handle_data->type]->own_handle(handle_data, abandoned);

  return FALSE;
}

static gboolean mono_w32handle_ops_isowned(MonoW32Handle *handle_data) {
...
mono_w32handle_ops_specialwait(MonoW32Handle *handle_data, guint32 timeout,
                               gboolean *alerted) {
  if (handle_ops[handle_data->type] &&
      handle_ops[handle_data->type]->special_wait)
    return handle_ops[handle_data->type]->special_wait(handle_data, timeout,
                                                       alerted);

  return MONO_W32HANDLE_WAIT_RET_FAILED;
}

static void mono_w32handle_ops_prewait(MonoW32Handle *handle_data) {
  if (handle_ops[handle_data->type] && handle_ops[handle_data->type]->prewait)
    handle_ops[handle_data->type]->prewait(handle_data);
}

static void mono_w32handle_lock_handles(MonoW32Handle **handles_data,
                                        gsize nhandles) {
  gint i, j, iter = 0;
#ifndef HOST_WIN32
...
static void mono_w32handle_unlock_handles(MonoW32Handle **handles_data,
                                          gsize nhandles) {
  gint i;
...
static int mono_w32handle_timedwait_signal_naked(MonoCoopCond *cond,
                                                 MonoCoopMutex *mutex,
                                                 guint32 timeout, gboolean poll,
                                                 gboolean *alerted) {
  int res;
...
static void signal_global(gpointer unused) {
...
static int mono_w32handle_timedwait_signal(guint32 timeout, gboolean poll,
                                           gboolean *alerted) {
  int res;
  ...
  res = mono_w32handle_timedwait_signal_naked(
      &global_signal_cond, &global_signal_mutex, timeout, poll, alerted);
...
static void signal_handle_and_unref(gpointer handle_duplicate) {
  MonoW32Handle *handle_data;
  MonoCoopCond *cond;
  MonoCoopMutex *mutex;
...
static int mono_w32handle_timedwait_signal_handle(MonoW32Handle *handle_data,
                                                  guint32 timeout,
                                                  gboolean poll,
                                                  gboolean *alerted) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
             "%s: waiting for %p (type %s)", __func__, handle_data,
  ...
  if (alerted) {
    mono_thread_info_install_interrupt(
        signal_handle_and_unref,
        handle_duplicate = mono_w32handle_duplicate(handle_data), alerted);
    if (*alerted) {
      mono_w32handle_close(handle_duplicate);
      return 0;
    }
  ...
  res = mono_w32handle_timedwait_signal_naked(&handle_data->signal_cond,
                                              &handle_data->signal_mutex,
                                              timeout, poll, alerted);

  if (alerted) {
    mono_thread_info_uninstall_interrupt(alerted);
    if (!*alerted) {
      /* if it is alerted, then the handle_duplicate is closed in the interrupt
       * callback */
      mono_w32handle_close(handle_duplicate);
    }
  }
...
static gboolean dump_callback(MonoW32Handle *handle_data, gpointer user_data) {
  g_print("%p [%7s] signalled: %5s ref: %3d ", handle_data,
          mono_w32handle_ops_typename(handle_data->type),
          handle_data->signalled ? "true" : "false",
          handle_data->ref - 1 /* foreach increase ref by 1 */);
...
void mono_w32handle_dump(void) { mono_w32handle_foreach(dump_callback, NULL); }

static gboolean own_if_signalled(MonoW32Handle *handle_data,
                                 gboolean *abandoned) {
...
static gboolean own_if_owned(MonoW32Handle *handle_data, gboolean *abandoned) {
...
gboolean mono_w32handle_handle_is_signalled(gpointer handle) {
  MonoW32Handle *handle_data;
  gboolean res;
...
gboolean mono_w32handle_handle_is_owned(gpointer handle) {
  MonoW32Handle *handle_data;
  gboolean res;
...
#ifdef HOST_WIN32
MonoW32HandleWaitRet mono_w32handle_wait_one(gpointer handle, guint32 timeout,
                                             gboolean alertable) {
  return mono_w32handle_convert_wait_ret(
      mono_coop_win32_wait_for_single_object_ex(handle, timeout, alertable), 1);
}
#else
MonoW32HandleWaitRet mono_w32handle_wait_one(gpointer handle, guint32 timeout,
                                             gboolean alertable) {
  ...
  if (mono_w32handle_test_capabilities(handle_data,
                                       MONO_W32HANDLE_CAP_SPECIAL_WAIT)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
               "%s: handle %p has special wait", __func__, handle_data);

    mono_w32handle_unref(handle_data);
    return mono_w32handle_ops_specialwait(handle_data, timeout,
                                          alertable ? &alerted : NULL);
  }

  if (!mono_w32handle_test_capabilities(handle_data, MONO_W32HANDLE_CAP_WAIT)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
               "%s: handle %p can't be waited for", __func__, handle_data);
  ...
  if (mono_w32handle_test_capabilities(handle_data, MONO_W32HANDLE_CAP_OWN)) {
    if (own_if_owned(handle_data, &abandoned)) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
                 "%s: handle %p already owned", __func__, handle_data);

      ret = abandoned ? MONO_W32HANDLE_WAIT_RET_ABANDONED_0
  ...
  for (;;) {
    ...
    if (own_if_signalled(handle_data, &abandoned)) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
                 "%s: handle %p signalled", __func__, handle_data);
    ...
    if (timeout == MONO_INFINITE_WAIT) {
      waited = mono_w32handle_timedwait_signal_handle(
          handle_data, MONO_INFINITE_WAIT, FALSE, alertable ? &alerted : NULL);
    } else {
      gint64 elapsed;
      ...
      waited = mono_w32handle_timedwait_signal_handle(
          handle_data, timeout - elapsed, FALSE, alertable ? &alerted : NULL);
    }
...
static MonoW32Handle *mono_w32handle_has_duplicates(MonoW32Handle *handles[],
                                                    gsize nhandles) {
...
static void mono_w32handle_clear_duplicates(MonoW32Handle *handles[],
                                            gsize nhandles) {
...
static void mono_w32handle_check_duplicates(MonoW32Handle *handles[],
                                            gsize nhandles, gboolean waitall,
                                            MonoError *error) {
  ...
  if (waitall) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
               "mono_w32handle_wait_multiple: handle %p is duplicated",
               duplicate);
    return;
  }
...
mono_w32handle_wait_multiple(gpointer *handles, gsize nhandles,
                             gboolean waitall, guint32 timeout,
                             gboolean alertable, MonoError *error) {
  DWORD const wait_result =
      (nhandles != 1)
          ? mono_coop_win32_wait_for_multiple_objects_ex(
                nhandles, handles, waitall, timeout, alertable, error)
          : mono_coop_win32_wait_for_single_object_ex(handles[0], timeout,
                                                      alertable);
...
mono_w32handle_wait_multiple(gpointer *handles, gsize nhandles,
  ...
  if (nhandles > MONO_W32HANDLE_MAXIMUM_WAIT_OBJECTS) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
               "%s: too many handles: %" G_GSIZE_FORMAT "d", __func__,
               nhandles);
  ...
  for (i = 0; i < nhandles; ++i) {
    if (!mono_w32handle_test_capabilities(handles_data[i],
                                          MONO_W32HANDLE_CAP_WAIT) &&
        !mono_w32handle_test_capabilities(handles_data[i],
                                          MONO_W32HANDLE_CAP_SPECIAL_WAIT)) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
  ...
  for (;;) {
    ...
    for (i = 0; i < nhandles; i++) {
      ...
      if ((mono_w32handle_test_capabilities(handles_data[i],
           ...
           mono_w32handle_ops_isowned(handles_data[i])) ||
    ...
    if (signalled) {
      for (i = 0; i < nhandles; i++) {
        ...
        if (own_if_signalled(handles_data[i], &abandoned[i]) && !waitall) {
          /* if we are calling WaitHandle.WaitAny, .NET only owns the first one;
           * it matters for Mutex which
           * throw AbandonedMutexException in case we owned it but didn't
           * release it */
          break;
        }
      }
    ...
    if (signalled) {
      ...
      for (i = lowest; i < nhandles; i++) {
        if (abandoned[i]) {
          ret = (MonoW32HandleWaitRet)(MONO_W32HANDLE_WAIT_RET_ABANDONED_0 +
                                       lowest);
          break;
        }
      }
    ...
    for (i = 0; i < nhandles; i++) {
      ...
      if (mono_w32handle_test_capabilities(handles_data[i],
                                           MONO_W32HANDLE_CAP_SPECIAL_WAIT) &&
          !mono_w32handle_issignalled(handles_data[i])) {
        mono_w32handle_ops_specialwait(handles_data[i], 0,
                                       alertable ? &alerted : NULL);
      }
    }
    ...
    if (!signalled) {
      if (timeout == MONO_INFINITE_WAIT) {
        waited = mono_w32handle_timedwait_signal(MONO_INFINITE_WAIT, poll,
                                                 alertable ? &alerted : NULL);
      } else {
        gint64 elapsed;
        ...
        waited = mono_w32handle_timedwait_signal(timeout - elapsed, poll,
                                                 alertable ? &alerted : NULL);
      }
    }
...
#ifdef HOST_WIN32
MonoW32HandleWaitRet mono_w32handle_signal_and_wait(gpointer signal_handle,
                                                    gpointer wait_handle,
                                                    guint32 timeout,
                                                    gboolean alertable) {
  return mono_w32handle_convert_wait_ret(
      mono_coop_win32_signal_object_and_wait(signal_handle, wait_handle,
                                             timeout, alertable),
      1);
}
#else
MonoW32HandleWaitRet mono_w32handle_signal_and_wait(gpointer signal_handle,
  ...
  if (!mono_w32handle_test_capabilities(signal_handle_data,
                                        MONO_W32HANDLE_CAP_SIGNAL)) {
    mono_w32handle_unref(wait_handle_data);
    mono_w32handle_unref(signal_handle_data);
    return MONO_W32HANDLE_WAIT_RET_FAILED;
  }
  if (!mono_w32handle_test_capabilities(wait_handle_data,
                                        MONO_W32HANDLE_CAP_WAIT)) {
    mono_w32handle_unref(wait_handle_data);
    mono_w32handle_unref(signal_handle_data);
    return MONO_W32HANDLE_WAIT_RET_FAILED;
  ...
  if (mono_w32handle_test_capabilities(wait_handle_data,
                                       MONO_W32HANDLE_CAP_SPECIAL_WAIT)) {
    g_warning("%s: handle %p has special wait, implement me!!", __func__,
              wait_handle_data);
    mono_w32handle_unref(wait_handle_data);
    mono_w32handle_unref(signal_handle_data);
    return MONO_W32HANDLE_WAIT_RET_FAILED;
  ...
  if (mono_w32handle_test_capabilities(wait_handle_data,
    ...
    if (own_if_owned(wait_handle_data, &abandoned)) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
                 "%s: handle %p already owned", __func__, wait_handle_data);
  ...
  for (;;) {
    ...
    if (own_if_signalled(wait_handle_data, &abandoned)) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_HANDLE,
                 "%s: handle %p signalled", __func__, wait_handle_data);
    ...
    if (timeout == MONO_INFINITE_WAIT) {
      waited = mono_w32handle_timedwait_signal_handle(
          wait_handle_data, MONO_INFINITE_WAIT, FALSE,
          alertable ? &alerted : NULL);
    } else {
      gint64 elapsed;
      ...
      waited = mono_w32handle_timedwait_signal_handle(
          wait_handle_data, timeout - elapsed, FALSE,
          alertable ? &alerted : NULL);
    }
