...
// The graph of managed objects/HashEntries is reduced to a graph of strongly
...
typedef struct _SCC {
   ...
   *
   * New and old xrefs are typically mutually exclusive.  Only when
   * TEST_NEW_XREFS is enabled we do both, and compare the results.  This should
   * only be done for debugging, obviously.
   */
#ifdef OLD_XREFS
...
static SgenHashTable hash_table = SGEN_HASH_TABLE_INIT(
    INTERNAL_MEM_BRIDGE_HASH_TABLE, INTERNAL_MEM_BRIDGE_HASH_TABLE_ENTRY,
    sizeof(HashEntry), mono_aligned_addr_hash, NULL);
...
static void dyn_array_scc_uninit(DynSCCArray *da) {
  dyn_array_uninit(&da->array, sizeof(SCC));
}

static int dyn_array_scc_size(DynSCCArray *da) { return da->array.size; }

static SCC *dyn_array_scc_add(DynSCCArray *da) {
  return (SCC *)dyn_array_add(&da->array, sizeof(SCC));
}

static SCC *dyn_array_scc_get_ptr(DynSCCArray *da, int x) {
...
#ifdef NEW_XREFS
static gboolean dyn_array_int_contains(DynIntArray *da, int x) {
  int i;
...
static void set_config(const SgenBridgeProcessorConfig *config) {
  if (config->accounting) {
    SgenHashTable table = SGEN_HASH_TABLE_INIT(
        INTERNAL_MEM_BRIDGE_HASH_TABLE, INTERNAL_MEM_BRIDGE_HASH_TABLE_ENTRY,
        sizeof(HashEntryWithAccounting), mono_aligned_addr_hash, NULL);
...
static HashEntry *get_hash_entry(MonoObject *obj, gboolean *existing) {
...
static void add_source(HashEntry *entry, HashEntry *src) {
...
static HashEntry *register_bridge_object(MonoObject *obj) {
  HashEntry *entry = get_hash_entry(obj, NULL);
  entry->is_bridge = TRUE;
  return entry;
}

static void register_finishing_time(HashEntry *entry, guint32 t) {
...
static gboolean is_opaque_object(MonoObject *obj) {
  if ((obj->vtable->gc_bits & SGEN_GC_BIT_BRIDGE_OPAQUE_OBJECT) ==
...
static gboolean object_needs_expansion(MonoObject **objp) {
  MonoObject *obj = *objp;
...
static HashEntry *follow_forward(HashEntry *entry) {
...
/*
 * DFS1 maintains a stack, where each two entries are effectively one entry.
 * (FIXME: Optimize this via pointer tagging.)  There are two different types of
 * entries:
 *
...
#define HANDLE_PTR(ptr, obj)                                                   \
  do {                                                                         \
    ...
    if (dst && object_needs_expansion(&dst)) {                                 \
      ...
      dyn_array_ptr_push(&dfs_stack,                                           \
                         follow_forward(get_hash_entry(dst, NULL)));           \
    }                                                                          \
  } while (0)

static void dfs1(HashEntry *obj_entry) {
  HashEntry *src;
...
#include "sgen/sgen-scan-object.h"
        ...
        if (!obj_entry->is_bridge && num_links == 1) {
          ...
          HashEntry *obj_entry_again =
              (HashEntry *)dyn_array_ptr_pop(&dfs_stack);
      ...
      if (src) {
        // g_print ("link %s -> %s\n", sgen_safe_name (src->obj), sgen_safe_name
        // (obj));
        g_assert(!obj_entry->v.dfs1.forwarded_to);
...
/*
 * At the end of bridge processing we need to end up with an (acyclyc) graph of
 * bridge object SCCs, where the links between the nodes (each one an SCC) in
 * that graph represent the presence of a direct or indirect link between those
 * SCCs.  An example:
 *
 *                       D
 *                       |
 ...
 *        A -> B -> c -> e -> F
 *
 * A, B, D and F are SCCs that contain bridge objects, c and e don't contain
 * bridge objects. The graph we need to produce from this is:
 *
 *                  D
 *                  |
 *                  v
 *        A -> B -> F
 *
 * Note that we don't need to produce an edge from A to F.  It's sufficient that
 * F is indirectly reachable from A.
 *
 * The old algorithm would create a set, for each SCC, of bridge SCCs that can
 * reach it, directly or indirectly, by merging the ones sets for those that
 * reach it directly.  The sets it would build up are these:
 *
 *   A: {}
 *   B: {A}
 *   c: {B}
 ...
 *
 * The new algorithm proceeds in two passes: During DFS2, it only builds up the
 * sets of SCCs that directly point to each SCC:
 *
 *   A: {}
 *   B: {A}
 *   c: {B}
 ...
 *   F: {e}
 *
 * This is the adjacency list for the SCC graph, in other words.  In a separate
 * step afterwards, it does a depth-first traversal of that graph, for each
 * bridge node, to get to the final list.  It uses a flag to avoid traversing
 * any node twice.
 */
static void scc_add_xref(SCC *src, SCC *dst) {
  g_assert(src != dst);
  g_assert(src->index != dst->index);

#ifdef NEW_XREFS
  /*
   * FIXME: Right now we don't even unique the direct ancestors, but just add to
   * the list.  Doing a containment check slows this algorithm down to almost
   * the speed of the old one.  Use the flag instead!
   */
...
static void scc_add_entry(SCC *scc, HashEntry *entry) {
...
static void dfs2(HashEntry *entry) {
  int i;
  ...
  do {
    ...
    if (entry->v.dfs2.scc_index >= 0) {
      if (entry->v.dfs2.scc_index != current_scc->index)
        scc_add_xref(dyn_array_scc_get_ptr(&sccs, entry->v.dfs2.scc_index),
                     current_scc);
      continue;
    }
...
#ifdef NEW_XREFS
static void gather_xrefs(SCC *scc) {
  int i;
...
static void reset_flags(SCC *scc) {
  int i;
  for (i = 0; i < dyn_array_int_size(&scc->new_xrefs); ++i) {
    int index = dyn_array_int_get(&scc->new_xrefs, i);
    SCC *src = dyn_array_scc_get_ptr(&sccs, index);
...
static void dump_graph(void) {
  ...
  fprintf(file, "<gexf xmlns=\"http://www.gexf.net/1.2draft\" "
  ...
  SGEN_HASH_TABLE_FOREACH(&hash_table, MonoObject *, obj, HashEntry *, entry) {
    ...
    fprintf(file,
            "<node id=\"%p\"><attvalues><attvalue for=\"0\" "
            "value=\"%s.%s\"/><attvalue for=\"1\" "
            "value=\"%s\"/></attvalues></node>\n",
            obj, m_class_get_name_space(vt->klass), m_class_get_name(vt->klass),
            entry->is_bridge ? "true" : "false");
  }
  SGEN_HASH_TABLE_FOREACH_END;
  fprintf(file, "</nodes>\n");
...
static int compare_hash_entries(const HashEntry *e1, const HashEntry *e2) {
...
static void processing_stw_step(void) {
  ...
  It must be done in 2 steps since the bridge arrays doesn't come in reverse
  topological order, which means that we can have entry N pointing to entry N
  + 1.

  If we dfs1 entry N before N + 1 is registered we'll not consider N + 1 for
  this bridge pass and not create the required xref between the two.
  */
  bridge_count = dyn_array_ptr_size(&registered_bridges);
  for (i = 0; i < bridge_count; ++i)
    register_bridge_object(
        (MonoObject *)dyn_array_ptr_get(&registered_bridges, i));

  for (i = 0; i < bridge_count; ++i)
    dfs1(get_hash_entry((MonoObject *)dyn_array_ptr_get(&registered_bridges, i),
                        NULL));
...
static void processing_build_callback_data(int generation) {
  ...
  all_entries = (HashEntry **)sgen_alloc_internal_dynamic(
      sizeof(HashEntry *) * hash_table.num_entries, INTERNAL_MEM_BRIDGE_DATA,
      TRUE);
  ...
  for (i = 0; i < hash_table.num_entries; ++i) {
    ...
    if (entry->v.dfs2.scc_index < 0) {
       ...
       */
      if (!current_scc->num_bridge_entries &&
          dyn_array_int_size(&current_scc->new_xrefs) == 1) {
        SCC *src;
  ...
  for (i = 0; i < dyn_array_scc_size(&sccs); ++i) {
    ...
    g_assert(dyn_array_int_size(&scc->new_xrefs) ==
             dyn_array_int_size(&scc->old_xrefs));
    for (j = 0; j < dyn_array_int_size(&scc->new_xrefs); ++j)
      g_assert(dyn_array_int_contains(&scc->old_xrefs,
                                      dyn_array_int_get(&scc->new_xrefs, j)));
  }
#endif
#endif

  /*
   * Compute the weight of each object. The weight of an object is its size plus
   * the size of all objects it points do. When the an object is pointed by
   * multiple objects we distribute it's weight equally among them. This
   * distribution gives a rough estimate of the real impact of making the object
   * go away.
   *
   * The reasoning for this model is that complex graphs with single roots will
   * have a bridge with very high value in comparison to others.
   *
   * The all_entries array has all objects topologically sorted. To correctly
   * propagate the weights it must be done in reverse topological order - so we
   * calculate the weight of the pointed-to objects before processing
   * pointer-from objects.
   *
   * We log those objects in the opposite order for no particular reason. The
   * other constrain is that it should use the same direction as the other
   * logging loop that records live/dead information.
   */
  if (bridge_accounting_enabled) {
    for (i = hash_table.num_entries - 1; i >= 0; --i) {
      double w;
      HashEntryWithAccounting *entry =
          (HashEntryWithAccounting *)all_entries[i];

      entry->weight += (double)sgen_safe_object_get_size(
      ...
      for (j = 0; j < dyn_array_ptr_size(&entry->entry.srcs); ++j) {
        HashEntryWithAccounting *other =
            (HashEntryWithAccounting *)dyn_array_ptr_get(&entry->entry.srcs, j);
        other->weight += w;
      }
    }
    for (i = 0; i < hash_table.num_entries; ++i) {
  ...
  api_sccs = (MonoGCBridgeSCC **)sgen_alloc_internal_dynamic(
  ...
  for (i = 0; i < dyn_array_scc_size(&sccs); ++i) {
    ...
    api_sccs[j] = (MonoGCBridgeSCC *)sgen_alloc_internal_dynamic(
        sizeof(MonoGCBridgeSCC) +
            sizeof(MonoObject *) * scc->num_bridge_entries,
  ...
  SGEN_HASH_TABLE_FOREACH(&hash_table, MonoObject *, obj, HashEntry *, entry) {
    if (entry->is_bridge) {
      ...
      api_sccs[scc->api_index]->objs[scc->num_bridge_entries++] =
          sgen_hash_table_key_for_value_pointer(entry);
    }
  }
  SGEN_HASH_TABLE_FOREACH_END;

  api_xrefs = (MonoGCBridgeXRef *)sgen_alloc_internal_dynamic(
      sizeof(MonoGCBridgeXRef) * num_xrefs, INTERNAL_MEM_BRIDGE_DATA, TRUE);
  j = 0;
  for (i = 0; i < dyn_array_scc_size(&sccs); ++i) {
    int k;
    SCC *scc = dyn_array_scc_get_ptr(&sccs, i);
    ...
    for (k = 0; k < dyn_array_int_size(&scc->XREFS); ++k) {
      SCC *src_scc =
          dyn_array_scc_get_ptr(&sccs, dyn_array_int_get(&scc->XREFS, k));
  ...
  sgen_free_internal_dynamic(all_entries,
                             sizeof(HashEntry *) * hash_table.num_entries,
  ...
  // g_print ("%d sccs containing bridges - %d max bridge objects - %d max
...
static void processing_after_callback(int generation) {
    ...
    for (i = 0; i < num_sccs; ++i) {
      for (j = 0; j < api_sccs[i]->num_objs; ++j) {
        ...
        mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
                   ...
                   sgen_client_vtable_get_name(vtable), api_sccs[i]->objs[j], i,
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
             "GC_NEW_BRIDGE num-objects %d num_hash_entries %d sccs size %d "
             "init %.2fms df1 %.2fms sort %.2fms dfs2 %.2fms setup-cb %.2fms "
             "free-data %.2fms links %d/%d/%d/%d dfs passes %d/%d ignored %d",
             num_registered_bridges, hash_table_size, dyn_array_scc_size(&sccs),
             step_1 / 10000.0f, step_2 / 10000.0f, step_3 / 10000.0f,
             step_4 / 10000.0f, step_5 / 10000.0f, step_6 / 10000.0f,
