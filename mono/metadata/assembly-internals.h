...
typedef gboolean (*MonoAssemblyAsmCtxFromPathFunc)(
    const char *absfname, MonoAssembly *requesting_assembly, gpointer user_data,
...
typedef MonoAssembly *(*MonoAssemblyPreLoadFuncV2)(MonoAssemblyLoadContext *alc,
                                                   MonoAssemblyName *aname,
...
typedef void (*MonoAssemblyLoadFuncV2)(MonoAssemblyLoadContext *alc,
...
/* MonoAssemblyCandidatePredicate that compares the assembly name (name,
 * version, culture, public key token) of the candidate with the wanted name, if
 * the wanted name has a public key token (if not present, always return true).
