...
/*
 * This file defines macros to compute sizes/alignments/field offsets which
 * depend on the ABI. It is needed during cross compiling since the generated
 * code needs to contain offsets which correspond to the ABI of the target, not
 * the host. It defines the following macros:
 ...
 * When not cross compiling, these correspond to the host ABI (i.e.
 * sizeof/offsetof). When cross compiling, these are defined in a generated
 * header file which is generated by the offsets tool in tools/offsets-tool. The
 * name of the file is given by the --with-cross-offsets= configure argument.
 */
...
#define DECL_OFFSET2(struct, field, offset)                                    \
  MONO_OFFSET_##struct##_##field = offset,
...
#define MONO_STRUCT_OFFSET_CONSTANT(struct, field)                             \
  MONO_STRUCT_OFFSET(struct, field)
 ...
 */
#define MONO_STRUCT_OFFSET(struct, field)                                      \
  (MONO_OFFSET_##struct##_##field == -1, G_STRUCT_OFFSET(struct, field))
#define MONO_STRUCT_OFFSET_CONSTANT(struct, field)                             \
  G_STRUCT_OFFSET(struct, field)
#define MONO_STRUCT_SIZE(struct)                                               \
  (MONO_SIZEOF_##struct == -1, (int)sizeof(struct))
#endif
