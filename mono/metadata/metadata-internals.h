 ...
 * Roslyn and C++/CLI sometimes create modifiers that mention generic parameters
 * that must be inflated, like: void .CL1`1.Test(!0
 * modopt(System.Nullable`1<!0>)) So we have to store a resolved MonoType*.
 *
...
static inline void mono_type_with_mods_init(MonoType *dest, uint8_t num_mods,
                                            gboolean is_aggregate) {
  if (num_mods == 0) {
    dest->has_cmods = 0;
    return;
...
typedef enum MonoAssemblyContextKind {
  ...
  /* Reflection-only only context: ReflectionOnlyLoad and
     ReeflectionOnlyLoadFrom */
...
struct _MonoImage {
  ...
  /* The path to the file for this image or an arbitrary name for images loaded
   * from data. */
  char *name;
  ...
  /* Code files in the assembly. The main assembly has a "file" table and also a
   * "module" table, where the module table is a subset of the file table. We
   * track both lists, and because we can lazy-load them at different times we
   * reference-increment both.
   */
  /* No netmodules in netcore, but for System.Reflection.Emit support we still
   * use modules */
  MonoImage **modules;
  ...
  // Containers for MonoGenericParams associated with this image but not with
  // any specific class or method. Created on demand. This could happen, for
  // example, for MonoTypes associated with TypeSpec table entries.
   ...
   * It's meant to be used only to mutate and query structures part of this
   * image.
   */
  mono_mutex_t lock;
};
...
/*
 * Generic instances and aggregated custom modifiers depend on many images, and
 * they need to be deleted if one of the images they depend on is unloaded. For
 * example, List<Foo> depends on both List's image and Foo's image. A
 * MonoImageSet is the owner of all generic instances depending on the same set
 * of images.
 */
typedef struct {
  int nimages;
  ...
  /*
   * Memory for generic instances owned by this image set should be allocated
   * from this mempool, using the mono_image_set_alloc family of functions.
   */
  MonoMemPool *mempool;
} MonoImageSet;
...
struct _MonoDynamicImage {
  ...
  MonoClass
      *wrappers_type; /*wrappers are bound to this type instead of <Module>*/
};
...
struct _MonoMethodHeader {
  ...
  unsigned int is_transient : 1; /* mono_metadata_free_mh () will actually free
                                    this header */
...
#define MONO_SIZEOF_METHOD_HEADER                                              \
  (sizeof(struct _MonoMethodHeader) - MONO_ZERO_LEN_ARRAY * SIZEOF_VOID_P)
...
#define MONO_SIZEOF_METHOD_SIGNATURE                                           \
  (sizeof(struct _MonoMethodSignature) - MONO_ZERO_LEN_ARRAY * SIZEOF_VOID_P)
...
#define mono_image_new0(image, type, size)                                     \
  ((type *)mono_image_alloc0(image, sizeof(type) * (size)))
...
#define mono_image_set_new0(image, type, size)                                 \
  ((type *)mono_image_set_alloc0(image, sizeof(type) * (size)))
...
const char *mono_metadata_string_heap_checked(MonoImage *meta,
                                              uint32_t table_index,
                                              MonoError *error);
const char *mono_metadata_blob_heap_checked(MonoImage *meta,
                                            uint32_t table_index,
                                            MonoError *error);
...
gboolean mono_metadata_interfaces_from_typedef_full(
    MonoImage *image, guint32 table_index, MonoClass ***interfaces,
    guint *count, gboolean heap_alloc_result, MonoGenericContext *context,
    MonoError *error);

MONO_API MonoMethodSignature *mono_metadata_parse_method_signature_full(
    MonoImage *image, MonoGenericContainer *generic_container, int def,
    const char *ptr, const char **rptr, MonoError *error);

MONO_API MonoMethodHeader *
mono_metadata_parse_mh_full(MonoImage *image, MonoGenericContainer *container,
...
MonoGenericInst *mono_metadata_parse_generic_inst(
    MonoImage *image, MonoGenericContainer *container, int count,
    const char *ptr, const char **rptr, MonoError *error);
...
MonoGenericClass *mono_metadata_lookup_generic_class(MonoClass *gclass,
...
void mono_config_parse_publisher_policy(const char *filename,
                                        MonoAssemblyBindingInfo *binding_info);
void mono_config_parse_assembly_bindings(
    const char *filename, int major, int minor, void *user_data,
...
MONO_API guint32 mono_metadata_get_generic_param_row(MonoImage *image,
                                                     guint32 token,
                                                     guint32 *owner);
...
MonoType *mono_metadata_type_dup_with_cmods(MonoImage *image,
                                            const MonoType *original,
                                            const MonoType *cmods_source);

MonoMethodSignature *mono_metadata_signature_dup_full(MonoImage *image,
...
MONO_API void mono_type_get_desc(GString *res, MonoType *type,
...
MONO_API void
mono_metadata_field_info_with_mempool(MonoImage *meta, guint32 table_index,
...
void mono_loader_set_strict_strong_names(gboolean enabled);

gboolean mono_loader_get_strict_strong_names(void);
...
#define MONO_CLASS_IS_INTERFACE_INTERNAL(c)                                    \
  ((mono_class_get_flags(c) & TYPE_ATTRIBUTE_INTERFACE) ||                     \
   mono_type_is_generic_parameter(m_class_get_byval_arg(c)))
...
static inline MonoAssemblyLoadContext *
