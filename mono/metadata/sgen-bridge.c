...
static BridgeProcessorSelection bridge_processor_selection =
...
// FIXME: The current usage pattern for this function is unsafe. Bridge
// processing could start immediately after unlock
/**
...
void mono_gc_register_bridge_callbacks(MonoGCBridgeCallbacks *callbacks) {
  ...
  // Note: This line is unsafe if we are on a separate thread from the one the
  // runtime was initialized on.
...
static BridgeProcessorSelection bridge_processor_name(const char *name) {
  if (!strcmp("old", name)) {
...
static gboolean bridge_processor_started(void) {
  return bridge_processor.reset_data != NULL;
}

// Initialize a single bridge processor
static void init_bridge_processor(SgenBridgeProcessor *processor,
...
void sgen_set_bridge_implementation(const char *name) {
  ...
  if (selection == BRIDGE_PROCESSOR_INVALID)
    g_warning("Invalid value for bridge processor implementation, valid values "
              "are: 'new', 'old' and 'tarjan'.");
  else if (bridge_processor_started())
    g_warning("Cannot set bridge processor implementation once bridge has "
              "already started");
  else
...
gboolean sgen_is_bridge_object(GCObject *obj) {
  if ((obj->vtable->gc_bits & SGEN_GC_BIT_BRIDGE_OBJECT) !=
...
static gboolean compare_bridge_processors(void) {
...
static gboolean is_bridge_object_dead(GCObject *obj, void *data) {
...
static void null_weak_links_to_dead_objects(SgenBridgeProcessor *processor,
                                            int generation) {
  int i, j;
  int num_sccs = processor->num_sccs;
  MonoGCBridgeSCC **api_sccs = processor->api_sccs;
  SgenHashTable alive_hash =
      SGEN_HASH_TABLE_INIT(INTERNAL_MEM_BRIDGE_ALIVE_HASH_TABLE,
                           INTERNAL_MEM_BRIDGE_ALIVE_HASH_TABLE_ENTRY, 1,
...
static void free_callback_data(SgenBridgeProcessor *processor) {
  int i;
  int num_sccs = processor->num_sccs;
  ...
  for (i = 0; i < num_sccs; ++i) {
    sgen_free_internal_dynamic(api_sccs[i],
                               sizeof(MonoGCBridgeSCC) +
                                   sizeof(MonoObject *) * api_sccs[i]->num_objs,
                               INTERNAL_MEM_BRIDGE_DATA);
  }
  sgen_free_internal_dynamic(api_sccs, sizeof(MonoGCBridgeSCC *) * num_sccs,
                             INTERNAL_MEM_BRIDGE_DATA);

  sgen_free_internal_dynamic(api_xrefs, sizeof(MonoGCBridgeXRef) * num_xrefs,
                             INTERNAL_MEM_BRIDGE_DATA);
...
static int compare_xrefs(const void *a_ptr, const void *b_ptr) {
...
dump_processor_state (SgenBridgeProcessor *p)
{
        ...
        for (i = 0; i < p->num_sccs; ++i) {
                ...
                for (j = 0; j < scc->num_objs; ++j) {
                        ...
                        printf (" %p(%s)", obj, SGEN_LOAD_VTABLE
(obj)->klass->name);
                }
                printf ("\n");
        }
        ...
        for (i = 0; i < p->num_xrefs; ++i)
                printf ("\t%d -> %d\n", p->api_xrefs [i].src_scc_index,
p->api_xrefs [i].dst_scc_index);

        printf ("-------\n");
}
*/

static gboolean sgen_compare_bridge_processor_results(SgenBridgeProcessor *a,
                                                      SgenBridgeProcessor *b) {
  int i;
  SgenHashTable obj_to_a_scc =
      SGEN_HASH_TABLE_INIT(INTERNAL_MEM_BRIDGE_DEBUG, INTERNAL_MEM_BRIDGE_DEBUG,
                           sizeof(int), mono_aligned_addr_hash, NULL);
  SgenHashTable b_scc_to_a_scc =
      SGEN_HASH_TABLE_INIT(INTERNAL_MEM_BRIDGE_DEBUG, INTERNAL_MEM_BRIDGE_DEBUG,
                           sizeof(int), g_direct_hash, NULL);
  ...
  /*
   * First we build a hash of each object in `a` to its respective SCC index
   * within `a`.  Along the way we also assert that no object is more than one
   * SCC.
   */
  for (i = 0; i < a->num_sccs; ++i) {
    ...
    for (j = 0; j < scc->num_objs; ++j) {
      ...
      gboolean new_entry =
          sgen_hash_table_replace(&obj_to_a_scc, obj, &i, NULL);
      g_assert(new_entry);
    }
  }

  /*
   * Now we check whether each of the objects in `b` are in `a`, and whether the
   * SCCs of `b` contain the same sets of objects as those of `a`.
   *
   * While we're doing this, build a hash table to map from `b` SCC indexes to
   * `a` SCC indexes.
   */
  for (i = 0; i < b->num_sccs; ++i) {
    ...
    g_assert(scc->num_objs > 0);
    a_scc_index_ptr =
        (int *)sgen_hash_table_lookup(&obj_to_a_scc, scc->objs[0]);
    ...
    for (j = 1; j < scc->num_objs; ++j) {
      a_scc_index_ptr =
          (int *)sgen_hash_table_lookup(&obj_to_a_scc, scc->objs[j]);
      g_assert(a_scc_index_ptr);
      g_assert(*a_scc_index_ptr == a_scc_index);
    }

    new_entry = sgen_hash_table_replace(&b_scc_to_a_scc, GINT_TO_POINTER(i),
                                        &a_scc_index, NULL);
    g_assert(new_entry);
  }

  /*
   * Finally, check that we have the same xrefs.  We do this by making copies of
   * both xref arrays, and replacing the SCC indexes in the copy for `b` with
   * the corresponding indexes in `a`.  Then we sort both arrays and assert that
   * they're the same.
   *
   * At the same time, check that no xref is self-referential and that there are
   * no duplicate ones.
   */

  xrefs_alloc_size = a->num_xrefs * sizeof(MonoGCBridgeXRef);
  a_xrefs = (MonoGCBridgeXRef *)sgen_alloc_internal_dynamic(
      xrefs_alloc_size, INTERNAL_MEM_BRIDGE_DEBUG, TRUE);
  b_xrefs = (MonoGCBridgeXRef *)sgen_alloc_internal_dynamic(
      xrefs_alloc_size, INTERNAL_MEM_BRIDGE_DEBUG, TRUE);
  ...
  for (i = 0; i < b->num_xrefs; ++i) {
    ...
    scc_index_ptr = (int *)sgen_hash_table_lookup(
        &b_scc_to_a_scc, GINT_TO_POINTER(xref->src_scc_index));
    g_assert(scc_index_ptr);
    b_xrefs[i].src_scc_index = *scc_index_ptr;

    scc_index_ptr = (int *)sgen_hash_table_lookup(
        &b_scc_to_a_scc, GINT_TO_POINTER(xref->dst_scc_index));
    g_assert(scc_index_ptr);
  ...
  sgen_free_internal_dynamic(a_xrefs, xrefs_alloc_size,
                             INTERNAL_MEM_BRIDGE_DEBUG);
  sgen_free_internal_dynamic(b_xrefs, xrefs_alloc_size,
                             INTERNAL_MEM_BRIDGE_DEBUG);
...
void sgen_bridge_processing_finish(int generation) {
  ...
  mono_bridge_callbacks.cross_references(
      bridge_processor.num_sccs, bridge_processor.api_sccs,
      bridge_processor.num_xrefs, bridge_processor.api_xrefs);

  if (compare_bridge_processors())
    sgen_compare_bridge_processor_results(&bridge_processor,
...
after_callback:
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
             "GC_BRIDGE: Complete, was running for %.2fms",
             mono_time_since_last_stw() / 10000.0f);
...
static MonoGCBridgeObjectKind bridge_test_bridge_class_kind(MonoClass *klass) {
  if (!strcmp(bridge_class, m_class_get_name(klass)) ||
      (m_class_get_parent(klass) &&
       !strcmp(bridge_class, m_class_get_name(m_class_get_parent(klass)))))
    return GC_BRIDGE_TRANSPARENT_BRIDGE_CLASS;
  return GC_BRIDGE_TRANSPARENT_CLASS;
}

static gboolean bridge_test_is_bridge_object(MonoObject *object) {
  return TRUE;
}

static void bridge_test_cross_reference(int num_sccs, MonoGCBridgeSCC **sccs,
                                        int num_xrefs,
...
static gboolean test_scc(MonoGCBridgeSCC *scc, int i) {
  int status = BRIDGE_DEAD;
  mono_field_get_value_internal(scc->objs[i], mono_bridge_test_field, &status);
  return status > 0;
}

static void mark_scc(MonoGCBridgeSCC *scc, int value) {
  int i;
  for (i = 0; i < scc->num_objs; ++i) {
    if (!test_scc(scc, i)) {
      int status = value;
      mono_field_set_value_internal(scc->objs[i], mono_bridge_test_field,
                                    &status);
    }
  }
}

static void bridge_test_cross_reference2(int num_sccs, MonoGCBridgeSCC **sccs,
                                         int num_xrefs,
                                         MonoGCBridgeXRef *xrefs) {
  int i;
  gboolean modified;
  ...
  if (!mono_bridge_test_field) {
    mono_bridge_test_field = mono_class_get_field_from_name_full(
        mono_object_class(sccs[0]->objs[0]), "__test", NULL);
  ...
  for (i = 0; i < num_sccs; ++i) {
    ...
    for (j = 0; j < sccs[i]->num_objs; ++j) {
      if (!test_scc(sccs[i], j)) {
        ...
        mono_field_set_value_internal(sccs[i]->objs[j], mono_bridge_test_field,
                                      &status);
      }
    }
  }
  ...
  while (modified) {
    ...
    for (i = 0; i < num_xrefs; ++i) {
      ...
      if (test_scc(sccs[ref.src_scc_index], 0) &&
          !test_scc(sccs[ref.dst_scc_index], 0)) {
        modified = TRUE;
...
static void bridge_test_positive_status(int num_sccs, MonoGCBridgeSCC **sccs,
                                        int num_xrefs,
                                        MonoGCBridgeXRef *xrefs) {
  int i;
...
gboolean sgen_bridge_handle_gc_debug(const char *opt) { return FALSE; }

gboolean sgen_bridge_handle_gc_param(const char *opt) { return FALSE; }
...
gboolean sgen_is_bridge_object(GCObject *obj) { return FALSE; }

gboolean sgen_need_bridge_processing(void) { return FALSE; }
