...
static void get_default_param_value_blobs(MonoMethod *method, char **blobs,
                                          guint32 *types);
static MonoType *mono_reflection_get_type_with_rootimage(
    MonoAssemblyLoadContext *alc, MonoImage *rootimage, MonoImage *image,
    MonoTypeNameParse *info, gboolean ignorecase, gboolean search_mscorlib,
    gboolean *type_resolve, MonoError *error);

/* Class lazy loading functions */
static GENERATE_GET_CLASS_WITH_CACHE(mono_assembly, "System.Reflection", "RuntimeAssembly") static GENERATE_GET_CLASS_WITH_CACHE(
    mono_module, "System.Reflection",
    "RuntimeModule") static GENERATE_GET_CLASS_WITH_CACHE(mono_method,
                                                          "System.Reflection",
                                                          "RuntimeMethodInfo");
static GENERATE_GET_CLASS_WITH_CACHE(mono_cmethod, "System.Reflection",
                                     "RuntimeConstructorInfo");
static GENERATE_GET_CLASS_WITH_CACHE(mono_field, "System.Reflection",
                                     "RuntimeFieldInfo");
static GENERATE_GET_CLASS_WITH_CACHE(mono_event, "System.Reflection",
                                     "RuntimeEventInfo");
static GENERATE_GET_CLASS_WITH_CACHE(mono_property, "System.Reflection",
                                     "RuntimePropertyInfo");
static GENERATE_GET_CLASS_WITH_CACHE(mono_parameter_info, "System.Reflection",
...
static GENERATE_GET_CLASS_WITH_CACHE(method_body, "System.Reflection",
                                     "RuntimeMethodBody");
static GENERATE_GET_CLASS_WITH_CACHE(local_variable_info, "System.Reflection",
                                     "RuntimeLocalVariableInfo");
static GENERATE_GET_CLASS_WITH_CACHE(exception_handling_clause,
                                     "System.Reflection",
...
#endif
static GENERATE_GET_CLASS_WITH_CACHE(type_builder, "System.Reflection.Emit",
                                     "TypeBuilder");
 ...
 */
MonoReflectionTypeBuilderHandle mono_class_get_ref_info(MonoClass *klass) {
  ...
  return MONO_HANDLE_CAST(MonoReflectionTypeBuilder,
                          mono_gchandle_get_target_handle(ref_info_handle));
}

gboolean mono_class_has_ref_info(MonoClass *klass) {
  MONO_REQ_GC_UNSAFE_MODE;
  return 0 != mono_class_get_ref_info_handle(klass);
}

MonoReflectionTypeBuilder *mono_class_get_ref_info_raw(MonoClass *klass) {
  ...
  if (ref_info_handle == 0)
    return NULL;
  return (MonoReflectionTypeBuilder *)mono_gchandle_get_target_internal(
      ref_info_handle);
}

void mono_class_set_ref_info(MonoClass *klass, MonoObjectHandle obj) {
  MONO_REQ_GC_UNSAFE_MODE;
...
void mono_class_free_ref_info(MonoClass *klass) {
...
gboolean mono_reflected_equal(gconstpointer a, gconstpointer b) {
...
guint mono_reflected_hash(gconstpointer a) {
  const ReflectedEntry *ea = (const ReflectedEntry *)a;
  ...
  seed ^= mono_aligned_addr_hash(ea->refclass) + 0x9e3779b9 + (seed << 6) +
          (seed >> 2);
  return seed;
}

static void clear_cached_object(MonoDomain *domain, gpointer o,
                                MonoClass *klass) {
  ...
  if (domain->refobject_hash) {
    ...
    if (mono_conc_g_hash_table_lookup_extended(domain->refobject_hash, &pe,
                                               &orig_pe, &orig_value)) {
...
static void cleanup_refobject_hash(gpointer key, gpointer value,
                                   gpointer user_data) {
...
void mono_reflection_cleanup_domain(MonoDomain *domain) {
  if (domain->refobject_hash) {
    mono_conc_g_hash_table_foreach(domain->refobject_hash,
                                   cleanup_refobject_hash, NULL);
 ...
 * \returns a \c System.Reflection.Assembly object representing the \c
 * MonoAssembly \p assembly.
 */
MonoReflectionAssembly *mono_assembly_get_object(MonoDomain *domain,
...
assembly_object_construct(MonoDomain *domain, MonoClass *unused_klass,
                          MonoAssembly *assembly, gpointer user_data,
                          MonoError *error) {
  error_init(error);
  MonoReflectionAssemblyHandle res = MONO_HANDLE_CAST(
      MonoReflectionAssembly,
      mono_object_new_handle(domain, mono_class_get_mono_assembly_class(),
                             error));
 ...
 *
 * Return an System.Reflection.Assembly object representing the MonoAssembly
 * @assembly.
 */
MonoReflectionAssemblyHandle
mono_assembly_get_object_handle(MonoDomain *domain, MonoAssembly *assembly,
                                MonoError *error) {
  error_init(error);
  return CHECK_OR_CONSTRUCT_HANDLE(MonoReflectionAssembly, assembly, NULL,
                                   assembly_object_construct, NULL);
}
...
MonoReflectionModule *mono_module_get_object(MonoDomain *domain,
  ...
  MonoReflectionModuleHandle result =
      mono_module_get_object_handle(domain, image, error);
  mono_error_cleanup(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

static MonoReflectionModuleHandle
module_object_construct(MonoDomain *domain, MonoClass *unused_klass,
                        MonoImage *image, gpointer user_data,
                        MonoError *error) {
  char *basename;

  error_init(error);
  MonoReflectionModuleHandle res =
      MONO_HANDLE_CAST(MonoReflectionModule,
                       mono_object_new_handle(
                           domain, mono_class_get_mono_module_class(), error));
  ...
  MONO_HANDLE_SET(res, fqname,
                  mono_string_new_handle(domain, image->name, error));
  goto_if_nok(error, fail);
  basename = g_path_get_basename(image->name);
  MONO_HANDLE_SET(res, name, mono_string_new_handle(domain, basename, error));
  goto_if_nok(error, fail);
  MONO_HANDLE_SET(res, scopename,
                  mono_string_new_handle(domain, image->module_name, error));
  goto_if_nok(error, fail);
...
MonoReflectionModuleHandle mono_module_get_object_handle(MonoDomain *domain,
                                                         MonoImage *image,
                                                         MonoError *error) {
  error_init(error);
  return CHECK_OR_CONSTRUCT_HANDLE(MonoReflectionModule, image, NULL,
                                   module_object_construct, NULL);
}

/**
 * mono_module_file_get_object:
 */
MonoReflectionModule *mono_module_file_get_object(MonoDomain *domain,
                                                  MonoImage *image,
                                                  int table_index) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MonoReflectionModuleHandle result =
      mono_module_file_get_object_handle(domain, image, table_index, error);
  mono_error_cleanup(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

MonoReflectionModuleHandle
mono_module_file_get_object_handle(MonoDomain *domain, MonoImage *image,
                                   int table_index, MonoError *error) {
  MonoTableInfo *table;
  ...
  goto_if_nok(error, fail);
  MONO_HANDLE_SETVAL(res, is_resource, MonoBoolean,
                     cols[MONO_FILE_FLAGS] & FILE_CONTAINS_NO_METADATA);
  MONO_HANDLE_SETVAL(
      res, token, guint32,
      mono_metadata_make_token(MONO_TABLE_FILE, table_index + 1));
...
static MonoType *mono_type_normalize(MonoType *type) {
  int i;
  ...
  for (i = 0; i < ginst->type_argc; ++i) {
    ...
    if (t->type != MONO_TYPE_VAR || t->data.generic_param->num != i ||
        t->data.generic_param->owner != gcontainer)
      is_denorm_gtd = FALSE;
  ...
  if (is_denorm_gtd)
    return type->byref == m_class_get_byval_arg(gtd)->byref
               ? m_class_get_byval_arg(gtd)
               : m_class_get_this_arg(gtd);

  if (requires_rebind) {
    MonoClass *klass = mono_class_bind_generic_parameters(
        gtd, ginst->type_argc, argv, gclass->is_dynamic);
    return type->byref == m_class_get_byval_arg(klass)->byref
...
MonoReflectionType *mono_type_get_object_checked(MonoDomain *domain,
                                                 MonoType *type,
                                                 MonoError *error) {
  MonoType *norm_type;
   ...
   *
   * We cannot do this for TypeBuilders as mono_reflection_create_runtime_class
   * expects that the resulting object is different.
   */
  if (type == m_class_get_byval_arg(klass) &&
  ...
  mono_loader_lock(); /*FIXME mono_class_init_internal and mono_class_vtable
                         acquire it*/
  mono_domain_lock(domain);
  if (!domain->type_hash)
    domain->type_hash = mono_g_hash_table_new_type_internal(
        (GHashFunc)mono_metadata_type_hash,
        (GCompareFunc)mono_metadata_type_equal, MONO_HASH_VALUE_GC,
        MONO_ROOT_SOURCE_DOMAIN, domain, "Domain Reflection Type Table");
  if ((res = (MonoReflectionType *)mono_g_hash_table_lookup(domain->type_hash,
                                                            type))) {
    mono_domain_unlock(domain);
    mono_loader_unlock();
    return res;
  ...
  /*Types must be normalized so a generic instance of the GTD get's the same
   * inner type. For example in: Foo<A,B>; Bar<A> : Foo<A, Bar<A>> The second
   * Bar will be encoded a generic instance of Bar with <A> as parameter. On all
   * other places, Bar<A> will be encoded as the GTD itself. This is an
   * implementation artifact of how generics are encoded and should be
   * transparent to managed code so we need to weed out this diference when
   * retrieving managed System.Type objects.
   */
  ...
  if ((type->type == MONO_TYPE_GENERICINST) &&
      type->data.generic_class->is_dynamic &&
      !m_class_was_typebuilder(type->data.generic_class->container_class)) {
    ...
    /* I would have expected ReflectionTypeLoadException, but evidently .NET
     * throws TLE in this case. */
  ...
  if (mono_class_has_ref_info(klass) && !m_class_was_typebuilder(klass) &&
      !type->byref) {
    mono_domain_unlock(domain);
    mono_loader_unlock();
  ...
  res = (MonoReflectionType *)mono_object_new_pinned(
      domain, mono_defaults.runtimetype_class, error);
  if (!is_ok(error)) {
    mono_domain_unlock(domain);
    mono_loader_unlock();
    return NULL;
...
MonoReflectionTypeHandle mono_type_get_object_handle(MonoDomain *domain,
                                                     MonoType *type,
                                                     MonoError *error) {
   ...
   */
  return MONO_HANDLE_NEW(MonoReflectionType,
                         mono_type_get_object_checked(domain, type, error));
}
 ...
 * \returns A \c System.Reflection.MonoMethod object representing the method \p
 * method.
 */
...
static MonoReflectionMethodHandle method_object_construct(MonoDomain *domain,
                                                          MonoClass *refclass,
                                                          MonoMethod *method,
                                                          gpointer user_data,
                                                          MonoError *error) {
  ...
  if (*method->name == '.' && (strcmp(method->name, ".ctor") == 0 ||
                               strcmp(method->name, ".cctor") == 0)) {
    klass = mono_class_get_mono_cmethod_class();
  } else {
    klass = mono_class_get_mono_method_class();
  }
  MonoReflectionMethodHandle ret = MONO_HANDLE_CAST(
      MonoReflectionMethod, mono_object_new_handle(domain, klass, error));
  goto_if_nok(error, fail);
  MONO_HANDLE_SETVAL(ret, method, MonoMethod *, method);

  MonoReflectionTypeHandle rt;
  rt = mono_type_get_object_handle(domain, m_class_get_byval_arg(refclass),
                                   error);
  goto_if_nok(error, fail);
 ...
 * @error: set on error.
 *
 * Return an System.Reflection.MonoMethod object representing the method
 * @method. Returns NULL and sets @error on error.
 */
MonoReflectionMethodHandle mono_method_get_object_handle(MonoDomain *domain,
                                                         MonoMethod *method,
                                                         MonoClass *refclass,
                                                         MonoError *error) {
  error_init(error);
  ...
  return CHECK_OR_CONSTRUCT_HANDLE(MonoReflectionMethod, method, refclass,
                                   method_object_construct, NULL);
}
/*
 ...
 * @method. Returns NULL and sets @error on error.
 */
MonoReflectionMethod *mono_method_get_object_checked(MonoDomain *domain,
                                                     MonoMethod *method,
                                                     MonoClass *refclass,
                                                     MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoReflectionMethodHandle result =
      mono_method_get_object_handle(domain, method, refclass, error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}
 ...
 */
void mono_method_clear_object(MonoDomain *domain, MonoMethod *method) {
  MonoClass *klass;
 ...
 * \param klass a type
 * \param field a field
 * \returns A \c System.Reflection.MonoField object representing the field \p
 * field in class \p klass.
 */
MonoReflectionField *mono_field_get_object(MonoDomain *domain, MonoClass *klass,
                                           MonoClassField *field) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MonoReflectionFieldHandle result =
      mono_field_get_object_handle(domain, klass, field, error);
  mono_error_cleanup(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

static MonoReflectionFieldHandle field_object_construct(MonoDomain *domain,
                                                        MonoClass *klass,
                                                        MonoClassField *field,
                                                        gpointer user_data,
                                                        MonoError *error) {
  ...
  MonoReflectionFieldHandle res = MONO_HANDLE_CAST(
      MonoReflectionField,
      mono_object_new_handle(domain, mono_class_get_mono_field_class(), error));
  goto_if_nok(error, fail);
  ...
  if (field->type) {
    MonoReflectionTypeHandle rt =
        mono_type_get_object_handle(domain, field->type, error);
    goto_if_nok(error, fail);
 ...
 */
MonoReflectionFieldHandle mono_field_get_object_handle(MonoDomain *domain,
                                                       MonoClass *klass,
                                                       MonoClassField *field,
                                                       MonoError *error) {
  error_init(error);
  return CHECK_OR_CONSTRUCT_HANDLE(MonoReflectionField, field, klass,
                                   field_object_construct, NULL);
}
 ...
 * in class @klass. On error, returns NULL and sets @error.
 */
MonoReflectionField *mono_field_get_object_checked(MonoDomain *domain,
                                                   MonoClass *klass,
                                                   MonoClassField *field,
                                                   MonoError *error) {
  HANDLE_FUNCTION_ENTER();
 ...
 * @property in class @klass.
 */
MonoReflectionProperty *mono_property_get_object(MonoDomain *domain,
                                                 MonoClass *klass,
                                                 MonoProperty *property) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MonoReflectionPropertyHandle result =
      mono_property_get_object_handle(domain, klass, property, error);
  mono_error_cleanup(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

static MonoReflectionPropertyHandle
property_object_construct(MonoDomain *domain, MonoClass *klass,
                          MonoProperty *property, gpointer user_data,
                          MonoError *error) {
  error_init(error);

  MonoReflectionPropertyHandle res = MONO_HANDLE_CAST(
      MonoReflectionProperty,
      mono_object_new_handle(domain, mono_class_get_mono_property_class(),
                             error));
  goto_if_nok(error, fail);
  MONO_HANDLE_SETVAL(res, klass, MonoClass *, klass);
 ...
 *
 * \returns A \c System.Reflection.MonoProperty object representing the property
 * \p property in class \p klass.  On error returns NULL and sets \p error.
 */
MonoReflectionPropertyHandle
mono_property_get_object_handle(MonoDomain *domain, MonoClass *klass,
                                MonoProperty *property, MonoError *error) {
  return CHECK_OR_CONSTRUCT_HANDLE(MonoReflectionProperty, property, klass,
                                   property_object_construct, NULL);
}
 ...
 * \p property in class \p klass.  On error returns NULL and sets \p error.
 */
MonoReflectionProperty *mono_property_get_object_checked(MonoDomain *domain,
                                                         MonoClass *klass,
                                                         MonoProperty *property,
                                                         MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoReflectionPropertyHandle res =
      mono_property_get_object_handle(domain, klass, property, error);
 ...
 * \param klass a type
 * \param event a event
 * \returns A \c System.Reflection.MonoEvent object representing the event \p
 * event in class \p klass.
 */
MonoReflectionEvent *mono_event_get_object(MonoDomain *domain, MonoClass *klass,
                                           MonoEvent *event) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MonoReflectionEventHandle result =
      mono_event_get_object_handle(domain, klass, event, error);
  mono_error_cleanup(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

static MonoReflectionEventHandle
event_object_construct(MonoDomain *domain, MonoClass *klass, MonoEvent *event,
                       gpointer user_data, MonoError *error) {

  error_init(error);
  MonoReflectionMonoEventHandle mono_event = MONO_HANDLE_CAST(
      MonoReflectionMonoEvent,
      mono_object_new_handle(domain, mono_class_get_mono_event_class(), error));
  if (!is_ok(error))
 ...
 * \param error set on error
 * \returns a \c System.Reflection.MonoEvent object representing the event \p
 * event in class \p klass. On failure sets \p error and returns NULL
 */
MonoReflectionEventHandle mono_event_get_object_handle(MonoDomain *domain,
                                                       MonoClass *klass,
                                                       MonoEvent *event,
                                                       MonoError *error) {
  error_init(error);
  return CHECK_OR_CONSTRUCT_HANDLE(MonoReflectionEvent, event, klass,
                                   event_object_construct, NULL);
}
 ...
 * is present
 */
static MonoObjectHandle mono_get_reflection_missing_object(MonoDomain *domain) {
  ERROR_DECL(error);
  ...
  missing_value_field =
      mono_class_get_field_from_name_full(missing_klass, "Value", NULL);
  ...
  MonoObjectHandle obj = MONO_HANDLE_NEW(
      MonoObject, mono_field_get_value_object_checked(
                      domain, missing_value_field, NULL, error));
  mono_error_assert_ok(error);
  return obj;
}

static MonoObjectHandle get_dbnull_object(MonoDomain *domain,
                                          MonoError *error) {
  error_init(error);
  ...
  dbnull_value_field =
      mono_class_get_field_from_name_full(dbnull_klass, "Value", NULL);
  ...
  MonoObjectHandle obj =
      MONO_HANDLE_NEW(MonoObject, mono_field_get_value_object_checked(
                                      domain, dbnull_value_field, NULL, error));
  return obj;
}

static MonoObjectHandle get_dbnull(MonoDomain *domain, MonoObjectHandle dbnull,
                                   MonoError *error) {
  error_init(error);
...
get_reflection_missing(MonoDomain *domain,
                       MonoObjectHandleOut reflection_missing) {
  if (MONO_HANDLE_IS_NULL(reflection_missing))
    MONO_HANDLE_ASSIGN(reflection_missing,
                       mono_get_reflection_missing_object(domain));
  return reflection_missing;
}

static gboolean add_parameter_object_to_array(
    MonoDomain *domain, MonoMethod *method, MonoObjectHandle member, int idx,
    const char *name, MonoType *sig_param, guint32 blob_type_enum,
    const char *blob, MonoMarshalSpec *mspec, MonoObjectHandle missing,
    MonoObjectHandle dbnull, MonoArrayHandle dest, MonoError *error) {
  HANDLE_FUNCTION_ENTER();

  error_init(error);
  MonoReflectionParameterHandle param = MONO_HANDLE_CAST(
      MonoReflectionParameter,
      mono_object_new_handle(domain, mono_class_get_mono_parameter_info_class(),
                             error));
  ...
  if (!ctor) {
    MonoMethod *m = mono_class_get_method_from_name_checked(
        mono_class_get_mono_parameter_info_class(), ".ctor", 7, 0, error);
    g_assert(m);
    mono_memory_barrier();
    ctor = m;
  }
  ...
  if (!(sig_param->attrs & PARAM_ATTRIBUTE_HAS_DEFAULT)) {
    ...
    else if ((sig_param->type == MONO_TYPE_VALUETYPE) &&
             m_class_is_enumtype(sig_param->data.klass)) {
    ...
    def_value = mono_get_object_from_blob(
        domain, &blob_type, blob, MONO_HANDLE_NEW(MonoString, NULL), error);
    goto_if_nok(error, leave);
  ...
  if (mspec) {
    mobj = mono_reflection_marshal_as_attribute_from_marshal_spec(
        domain, method->klass, mspec, error);
    goto_if_nok(error, leave);
  }

  /* internal RuntimeParameterInfo (string name, Type type, int position, int
   * attrs, object defaultValue, MemberInfo member, MarshalAsAttribute
   * marshalAs) */
  {
    int attrs = sig_param->attrs;

    void *args[] = {MONO_HANDLE_RAW(name_str),
                    MONO_HANDLE_RAW(rt),
                    &idx,
                    &attrs,
    ...
    mono_runtime_invoke_handle_void(ctor, MONO_HANDLE_CAST(MonoObject, param),
                                    args, error);
  }
  goto_if_nok(error, leave);
...
param_objects_construct(MonoDomain *domain, MonoClass *refclass,
                        MonoMethodSignature **addr_of_sig, gpointer user_data,
                        MonoError *error) {
  MonoMethod *method = (MonoMethod *)user_data;
  MonoMethodSignature *sig =
      *addr_of_sig; /* see note in mono_param_get_objects_internal */
  ...
  res =
      mono_array_new_handle(domain, mono_class_get_mono_parameter_info_class(),
                            sig->param_count, error);
  ...
  /* Handles missing and dbnull are assigned in add_parameter_object_to_array
   * when needed */
  ...
  for (i = 0; i < sig->param_count; ++i) {
    if (!add_parameter_object_to_array(
            domain, method, MONO_HANDLE_CAST(MonoObject, member), i, names[i],
            sig->params[i], types ? types[i] : 0, blobs ? blobs[i] : NULL,
            mspecs[i + 1], missing, dbnull, res, error))
      goto leave;
  }
 ...
 * @method: a method
 *
 * Return an System.Reflection.ParameterInfo array object representing the
 * parameters in the method @method.
 */
MonoArrayHandle mono_param_get_objects_internal(MonoDomain *domain,
                                                MonoMethod *method,
                                                MonoClass *refclass,
                                                MonoError *error) {
  error_init(error);
  ...
  if (!sig->param_count) {
    MonoArrayHandle res = mono_array_new_handle(
        domain, mono_class_get_mono_parameter_info_class(), 0, error);
    goto_if_nok(error, fail);
   ...
   */
  return CHECK_OR_CONSTRUCT_HANDLE(MonoArray, &method->signature, refclass,
                                   param_objects_construct, method);
fail:
...
MonoArray *mono_param_get_objects(MonoDomain *domain, MonoMethod *method) {
  ...
  MonoArrayHandle result =
      mono_param_get_objects_internal(domain, method, NULL, error);
  mono_error_assert_ok(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

static gboolean add_local_var_info_to_array(MonoDomain *domain,
                                            MonoMethodHeader *header, int idx,
                                            MonoArrayHandle dest,
                                            MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoReflectionLocalVariableInfoHandle info = MONO_HANDLE_CAST(
      MonoReflectionLocalVariableInfo,
      mono_object_new_handle(domain, mono_class_get_local_variable_info_class(),
                             error));
  goto_if_nok(error, leave);
...
static gboolean add_exception_handling_clause_to_array(MonoDomain *domain,
  ...
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoReflectionExceptionHandlingClauseHandle info = MONO_HANDLE_CAST(
      MonoReflectionExceptionHandlingClause,
      mono_object_new_handle(
          domain, mono_class_get_exception_handling_clause_class(), error));
  goto_if_nok(error, leave);
  ...
  if (clause->flags == MONO_EXCEPTION_CLAUSE_FILTER)
    ...
    MonoReflectionTypeHandle rt = mono_type_get_object_handle(
        mono_domain_get(), m_class_get_byval_arg(clause->data.catch_class),
        error);
    goto_if_nok(error, leave);
 ...
 * \param domain an app domain
 * \param method a method
 * \return A \c System.Reflection.MethodBody/RuntimeMethodBody object
 * representing the method \p method.
 */
MonoReflectionMethodBody *mono_method_body_get_object(MonoDomain *domain,
  ...
  MonoReflectionMethodBodyHandle result =
      mono_method_body_get_object_handle(domain, method, error);
  mono_error_cleanup(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}
...
method_body_object_construct(MonoDomain *domain, MonoClass *unused_class,
                             MonoMethod *method, gpointer user_data,
                             MonoError *error) {
  ...
  if (!image_is_dynamic(image)) {
    ...
    method_rva = mono_metadata_decode_row_col(
        &image->tables[MONO_TABLE_METHOD],
        mono_metadata_token_index(method->token) - 1, MONO_METHOD_RVA);
  ...
  static MonoMethod *ctor;
  if (!ctor) {
    MonoMethod *tmp = mono_class_get_method_from_name_checked(
        mono_class_get_method_body_class(), ".ctor", 6, 0, error);
    mono_error_assert_ok(error);
    g_assert(tmp);
  ...
  ret =
      MONO_HANDLE_CAST(MonoReflectionMethodBody,
                       mono_object_new_handle(
                           domain, mono_class_get_method_body_class(), error));
  goto_if_nok(error, fail);

  MonoArrayHandle il_arr;
  il_arr = mono_array_new_handle(domain, mono_defaults.byte_class,
                                 header->code_size, error);
  goto_if_nok(error, fail);
  ...
  locals_arr =
      mono_array_new_handle(domain, mono_class_get_local_variable_info_class(),
                            header->num_locals, error);
  goto_if_nok(error, fail);
  ...
  exn_clauses = mono_array_new_handle(
      domain, mono_class_get_exception_handling_clause_class(),
      header->num_clauses, error);
  goto_if_nok(error, fail);
  for (i = 0; i < header->num_clauses; ++i) {
    if (!add_exception_handling_clause_to_array(domain, header, i, exn_clauses,
                                                error))
      goto fail;
  }
  ...
  */
  ...
  mono_runtime_invoke_handle_void(ctor, MONO_HANDLE_CAST(MonoObject, ret),
                                  params, error);
  mono_error_assert_ok(error);
...
mono_method_body_get_object_handle(MonoDomain *domain, MonoMethod *method,
                                   MonoError *error) {
  error_init(error);
  return CHECK_OR_CONSTRUCT_HANDLE(MonoReflectionMethodBody, method, NULL,
                                   method_body_object_construct, NULL);
}
...
static void get_default_param_value_blobs(MonoMethod *method, char **blobs,
  ...
  if (image_is_dynamic(image)) {
    ...
    aux = (MonoReflectionMethodAux *)g_hash_table_lookup(
        ((MonoDynamicImage *)m_class_get_image(method->klass))->method_aux_hash,
        method);
    if (aux && aux->param_defaults) {
      memcpy(blobs, &(aux->param_defaults[1]),
             methodsig->param_count * sizeof(char *));
      memcpy(types, &(aux->param_default_types[1]),
             methodsig->param_count * sizeof(guint32));
    }
    return;
  }
  ...
  param_index =
      mono_metadata_decode_row_col(methodt, idx, MONO_METHOD_PARAMLIST);
  if (idx + 1 < methodt->rows)
    lastp =
        mono_metadata_decode_row_col(methodt, idx + 1, MONO_METHOD_PARAMLIST);
  else
    lastp = paramt->rows + 1;
  ...
  for (i = param_index; i < lastp; ++i) {
    ...
    crow = mono_metadata_get_constant_index(image, MONO_TOKEN_PARAM_DEF | i,
                                            crow + 1);
    if (!crow) {
      continue;
    }
    ...
    blobs[paramseq - 1] =
        (char *)mono_metadata_blob_heap(image, const_cols[MONO_CONSTANT_VALUE]);
...
MonoObjectHandle mono_get_object_from_blob(MonoDomain *domain, MonoType *type,
                                           const char *blob,
...
static int assembly_name_to_aname(MonoAssemblyName *assembly, char *p) {
  int found_sep;
  char *s;
  ...
  assembly->name = p;
  while (*p && (isalnum(*p) || *p == '.' || *p == '-' || *p == '_' ||
                *p == '$' || *p == '@' || g_ascii_isspace(*p)))
    p++;
  if (quoted) {
    if (*p != '"')
      return 1;
  ...
  *p = 0;
  ...
  while (*p) {
    if ((*p == 'V' || *p == 'v') &&
        g_ascii_strncasecmp(p, "Version=", 8) == 0) {
      p += 8;
      ...
      p = s;
               ...
               g_ascii_strncasecmp(p, "Culture=", 8) == 0) {
      p += 8;
      if (g_ascii_strncasecmp(p, "neutral", 7) == 0) {
        ...
        while (*p && *p != ',') {
          p++;
               ...
               g_ascii_strncasecmp(p, "PublicKeyToken=", 15) == 0) {
      p += 15;
 ...
 * the name param will be mangled, so, make a copy before passing it to this
 * function. The fields in info will be valid until the memory pointed to by
 * name is valid.
 *
 ...
 */
static int _mono_reflection_parse_type(char *name, char **endptr,
    ...
    case '[':
      ...
      *p++ = 0;
      ...
      if (*p == ',' || *p == '*' || *p == ']') { // array
        ...
        if (bounded)
          info->modifiers =
              g_list_append(info->modifiers, GUINT_TO_POINTER(-2));
        info->modifiers =
            g_list_append(info->modifiers, GUINT_TO_POINTER(rank));
      } else {
        ...
        while (*p) {
          ...
          /*MS is lenient on [] delimited parameters that aren't fqn - and F#
           * uses them.*/
          if (fqname && (*p != ']')) {
            char *aname;
            ...
            *p++ = 0;
            ...
            if (!*aname || !assembly_name_to_aname(&subinfo->assembly, aname))
              return 0;
          } else if (fqname && (*p == ']')) {
            *p++ = 0;
          }
 ...
 */
char *mono_identifier_unescape_type_name_chars(char *identifier) {
  char *w, *r;
  if (!identifier)
    return NULL;
...
void mono_identifier_unescape_info(MonoTypeNameParse *info);

static void unescape_each_type_argument(void *data, void *user_data) {
  MonoTypeNameParse *info = (MonoTypeNameParse *)data;
  mono_identifier_unescape_info(info);
}

static void unescape_each_nested_name(void *data, void *user_data) {
 ...
 *
 * \param info a parsed display form of an (optionally assembly qualified) full
 * type name.
 *
...
void mono_identifier_unescape_info(MonoTypeNameParse *info) {
  ...
  if (info->type_arguments)
    g_ptr_array_foreach(info->type_arguments, &unescape_each_type_argument,
                        NULL);
  if (info->nested)
 ...
 *
 * \returns TRUE if parsing succeeded, otherwise returns FALSE and sets \p
 * error.
 *
 */
gboolean mono_reflection_parse_type_checked(char *name, MonoTypeNameParse *info,
                                            MonoError *error) {
  error_init(error);
...
#if ENABLE_NETCORE
    mono_error_set_argument_format(error, "typeName@0", "failed parse: %s",
                                   name);
#else
...
static MonoType *_mono_reflection_get_type_from_info(
    MonoAssemblyLoadContext *alc, MonoTypeNameParse *info, MonoImage *image,
    gboolean ignorecase, gboolean search_mscorlib, MonoError *error) {
  ...
  if (info->assembly.name) {
    MonoAssembly *assembly =
        mono_assembly_loaded_internal(alc, &info->assembly, FALSE);
    if (!assembly && image && image->assembly &&
        mono_assembly_check_name_match(&info->assembly,
  ...
  type = mono_reflection_get_type_with_rootimage(alc, rootimage, image, info,
                                                 ignorecase, search_mscorlib,
                                                 &type_resolve, error);
 ...
 */
static MonoType *
mono_reflection_get_type_internal(MonoAssemblyLoadContext *alc,
                                  MonoImage *rootimage, MonoImage *image,
                                  MonoTypeNameParse *info, gboolean ignorecase,
                                  gboolean search_mscorlib, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoClass *klass;
  GList *mod;
  int modval;
  ...
  if (ignorecase)
    klass = mono_class_from_name_case_checked(image, info->name_space,
                                              info->name, error);
  else
    klass = mono_class_from_name_checked(image, info->name_space, info->name,
                                         error);
  ...
  for (mod = info->nested; mod; mod = mod->next) {
    ...
    while ((klass = mono_class_get_nested_types(parent, &iter))) {
      ...
      if (nested_nspace) {
        ...
        if (ignorecase) {
          if (!(klass_name_space &&
                mono_utf8_strcasecmp(klass_name_space, nested_nspace) == 0))
            match = FALSE;
        } else {
          if (!(klass_name_space &&
                strcmp(klass_name_space, nested_nspace) == 0))
            match = FALSE;
        }
      }
      if (match) {
  ...
  if (info->type_arguments) {
    ...
    for (i = 0; i < info->type_arguments->len; i++) {
      MonoTypeNameParse *subinfo =
          (MonoTypeNameParse *)g_ptr_array_index(info->type_arguments, i);

      type_args[i] = _mono_reflection_get_type_from_info(
          alc, subinfo, rootimage, ignorecase, search_mscorlib, error);
      if (!type_args[i]) {
        g_free(type_args);
        goto leave;
      }
    ...
    the_type = mono_type_get_object_handle(mono_domain_get(),
                                           m_class_get_byval_arg(klass), error);
 ...
 *
 */
MonoType *mono_reflection_get_type(MonoImage *image, MonoTypeNameParse *info,
                                   gboolean ignorecase,
                                   gboolean *type_resolve) {
  ERROR_DECL(error);
  MonoDomain *domain = mono_domain_get();
  MonoType *result = mono_reflection_get_type_with_rootimage(
      mono_domain_default_alc(domain), image, image, info, ignorecase, TRUE,
      type_resolve, error);
  mono_error_cleanup(error);
  return result;
}
 ...
 * \param
 * \param error set on error.
 * Build a \c MonoType from the type description in \p info. On failure returns
 * NULL and sets \p error.
 */
MonoType *mono_reflection_get_type_checked(
    MonoAssemblyLoadContext *alc, MonoImage *rootimage, MonoImage *image,
    MonoTypeNameParse *info, gboolean ignorecase, gboolean search_mscorlib,
    gboolean *type_resolve, MonoError *error) {
  error_init(error);
  return mono_reflection_get_type_with_rootimage(alc, rootimage, image, info,
                                                 ignorecase, search_mscorlib,
                                                 type_resolve, error);
}

static MonoType *module_builder_array_get_type(
    MonoAssemblyLoadContext *alc, MonoArrayHandle module_builders, int i,
    MonoImage *rootimage, MonoTypeNameParse *info, gboolean ignorecase,
    gboolean search_mscorlib, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoType *type = NULL;
  ...
  type = mono_reflection_get_type_internal(alc, rootimage,
                                           &dynamic_image->image, info,
                                           ignorecase, search_mscorlib, error);
  HANDLE_FUNCTION_RETURN_VAL(type);
}

static MonoType *
module_array_get_type(MonoAssemblyLoadContext *alc, MonoArrayHandle modules,
                      int i, MonoImage *rootimage, MonoTypeNameParse *info,
                      gboolean ignorecase, gboolean search_mscorlib,
                      MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoType *type = NULL;
  ...
  type = mono_reflection_get_type_internal(alc, rootimage, image, info,
                                           ignorecase, search_mscorlib, error);
  HANDLE_FUNCTION_RETURN_VAL(type);
}

static MonoType *mono_reflection_get_type_internal_dynamic(
    MonoAssemblyLoadContext *alc, MonoImage *rootimage, MonoAssembly *assembly,
    MonoTypeNameParse *info, gboolean ignorecase, gboolean search_mscorlib,
    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoType *type = NULL;
  ...
  MonoReflectionAssemblyBuilderHandle abuilder = MONO_HANDLE_CAST(
      MonoReflectionAssemblyBuilder,
      mono_assembly_get_object_handle(((MonoDynamicAssembly *)assembly)->domain,
                                      assembly, error));
  goto_if_nok(error, leave);
  ...
  if (!MONO_HANDLE_IS_NULL(modules)) {
    ...
    for (i = 0; i < n; ++i) {
      type = module_builder_array_get_type(alc, modules, i, rootimage, info,
                                           ignorecase, search_mscorlib, error);
      if (type)
        break;
      goto_if_nok(error, leave);
  ...
  if (!type && !MONO_HANDLE_IS_NULL(loaded_modules)) {
    ...
    for (i = 0; i < n; ++i) {
      type = module_array_get_type(alc, loaded_modules, i, rootimage, info,
                                   ignorecase, search_mscorlib, error);
      if (type)
        break;
      goto_if_nok(error, leave);
...
MonoType *mono_reflection_get_type_with_rootimage(
    MonoAssemblyLoadContext *alc, MonoImage *rootimage, MonoImage *image,
    MonoTypeNameParse *info, gboolean ignorecase, gboolean search_mscorlib,
    gboolean *type_resolve, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  if (image && image_is_dynamic(image))
    type = mono_reflection_get_type_internal_dynamic(
        alc, rootimage, image->assembly, info, ignorecase, search_mscorlib,
        error);
  else
    type = mono_reflection_get_type_internal(
        alc, rootimage, image, info, ignorecase, search_mscorlib, error);
  ...
  reflection_assembly = mono_domain_try_type_resolve_name(
      domain, image->assembly, name_handle, error);
  goto_if_nok(error, return_null);
  ...
  if (MONO_HANDLE_BOOL(reflection_assembly)) {
    ...
    if (assembly_is_dynamic(assembly))
      type = mono_reflection_get_type_internal_dynamic(
          alc, rootimage, assembly, info, ignorecase, search_mscorlib, error);
    else
      type = mono_reflection_get_type_internal(alc, rootimage, assembly->image,
                                               info, ignorecase,
                                               search_mscorlib, error);
  }
  goto_if_nok(error, return_null);
  goto exit;
...
MonoType *mono_reflection_type_from_name(char *name, MonoImage *image) {
  ...
  MonoType *const result =
      mono_reflection_type_from_name_checked(name, alc, image, error);
 ...
 */
MonoType *mono_reflection_type_from_name_checked(char *name,
  ...
  type = _mono_reflection_get_type_from_info(alc, &info, image, FALSE, TRUE,
                                             error);
leave:
  g_free(tmp);
 ...
 */
void mono_reflection_get_param_info_member_and_pos(
    MonoReflectionParameterHandle p, MonoObjectHandle member_impl,
    int *out_position) {
  MonoClass *klass = mono_class_get_mono_parameter_info_class();

  /* These two fields are part of ParameterInfo instead of RuntimeParameterInfo,
   * and they cannot be moved */

  static MonoClassField *member_field;
  if (!member_field) {
    MonoClassField *f =
        mono_class_get_field_from_name_full(klass, "MemberImpl", NULL);
    g_assert(f);
    member_field = f;
  }
  MonoObject *member;
  mono_field_get_value_internal(
      MONO_HANDLE_RAW(MONO_HANDLE_CAST(MonoObject, p)), member_field, &member);
  MONO_HANDLE_ASSIGN_RAW(member_impl, member);

  static MonoClassField *pos_field;
  if (!pos_field) {
    MonoClassField *f =
        mono_class_get_field_from_name_full(klass, "PositionImpl", NULL);
    g_assert(f);
    pos_field = f;
  }
  mono_field_get_value_internal(
      MONO_HANDLE_RAW(MONO_HANDLE_CAST(MonoObject, p)), pos_field,
      out_position);
}
 ...
 */
guint32 mono_reflection_get_token_checked(MonoObjectHandle obj,
                                          MonoError *error) {
  guint32 token = 0;
  ...
  if (strcmp(klass_name, "MethodBuilder") == 0) {
    MonoReflectionMethodBuilderHandle mb =
        MONO_HANDLE_CAST(MonoReflectionMethodBuilder, obj);

    token = MONO_HANDLE_GETVAL(mb, table_idx) | MONO_TOKEN_METHOD_DEF;
  } else if (strcmp(klass_name, "ConstructorBuilder") == 0) {
    MonoReflectionCtorBuilderHandle mb =
        MONO_HANDLE_CAST(MonoReflectionCtorBuilder, obj);
    ...
    MonoReflectionTypeBuilderHandle tb =
        MONO_HANDLE_CAST(MonoReflectionTypeBuilder, obj);
    ...
    MonoReflectionPropertyHandle p =
        MONO_HANDLE_CAST(MonoReflectionProperty, obj);

    token = mono_class_get_property_token(MONO_HANDLE_GETVAL(p, property));
  } else if (strcmp(klass_name, "RuntimeEventInfo") == 0) {
    MonoReflectionMonoEventHandle p =
        MONO_HANDLE_CAST(MonoReflectionMonoEvent, obj);

    token = mono_class_get_event_token(MONO_HANDLE_GETVAL(p, event));
  } else if (strcmp(klass_name, "ParameterInfo") == 0 ||
             strcmp(klass_name, "RuntimeParameterInfo") == 0) {
    MonoReflectionParameterHandle p =
        MONO_HANDLE_CAST(MonoReflectionParameter, obj);
    ...
    MonoMethod *method = MONO_HANDLE_GETVAL(
        MONO_HANDLE_CAST(MonoReflectionMethod, member_impl), method);

    token = mono_method_get_param_token(method, position);
  } else if (strcmp(klass_name, "RuntimeModule") == 0 ||
             strcmp(klass_name, "ModuleBuilder") == 0) {
    ...
    mono_error_set_not_implemented(
        error, "MetadataToken is not supported for type '%s.%s'",
...
gboolean mono_reflection_is_usertype(MonoReflectionTypeHandle ref) {
  MonoClass *klass = mono_handle_class(ref);
  return m_class_get_image(klass) != mono_defaults.corlib ||
         strcmp("TypeDelegator", m_class_get_name(klass)) == 0;
}

/**
 * mono_reflection_bind_generic_parameters:
 * \param type a managed type object (which should be some kind of generic
 * (instance? definition?)) \param type_args the number of type arguments to
 * bind \param types array of type arguments \param error set on error Given a
 * managed type object for a generic type instance, binds each of its arguments
 * to the specified types. \returns the \c MonoType* for the resulting type
 * instantiation.  On failure returns NULL and sets \p error.
 */
MonoType *
mono_reflection_bind_generic_parameters(MonoReflectionTypeHandle reftype,
  ...
  if (mono_is_sre_type_builder(klass)) {
    ...
    /* Does this ever make sense?  what does instantiating a generic instance
     * even mean? */
    g_assert_not_reached();
    MonoReflectionGenericClassHandle rgi =
        MONO_HANDLE_CAST(MonoReflectionGenericClass, reftype);
    MonoReflectionTypeHandle gtd =
        MONO_HANDLE_NEW_GET(MonoReflectionType, rgi, generic_type);
  ...
  if (!mono_class_is_gtd(klass)) {
    mono_loader_unlock();
    mono_error_set_type_load_class(
        error, klass, "Cannot bind generic parameters of a non-generic type");
    return NULL;
  }
  ...
  if (gtd_type_argc != type_argc) {
    mono_loader_unlock();
    mono_error_set_argument_format(error, "types",
                                   "The generic type definition needs %d type "
                                   "arguments, but was instantiated with %d ",
                                   gtd_type_argc, type_argc);
    return NULL;
  }
  ...
  geninst =
      mono_class_bind_generic_parameters(klass, type_argc, types, is_dynamic);

  return m_class_get_byval_arg(geninst);
}

MonoClass *mono_class_bind_generic_parameters(MonoClass *klass, int type_argc,
                                              MonoType **types,
                                              gboolean is_dynamic) {
  MonoGenericClass *gclass;
...
static MonoMethod *reflection_bind_generic_method_parameters(
    MonoMethod *method, MonoArrayHandle types, MonoError *error) {
  MonoClass *klass;
  MonoMethod *inflated;
...
MonoReflectionMethodHandle ves_icall_RuntimeMethodInfo_MakeGenericMethod_impl(
    MonoReflectionMethodHandle rmethod, MonoArrayHandle types,
    MonoError *error) {
  error_init(error);
  g_assert(0 != strcmp(m_class_get_name(mono_handle_class(rmethod)),
                       "MethodBuilder"));

  MonoMethod *method = MONO_HANDLE_GETVAL(rmethod, method);
  MonoMethod *imethod =
      reflection_bind_generic_method_parameters(method, types, error);
  ...
  if (image_is_dynamic(m_class_get_image(method->klass))) {
    ...
    mono_g_hash_table_insert_internal(image->generic_def_objects, imethod,
                                      MONO_HANDLE_RAW(rmethod));
    mono_image_unlock((MonoImage *)image);
  }

  return mono_method_get_object_handle(MONO_HANDLE_DOMAIN(rmethod), imethod,
                                       refclass, error);
}
...
const static guint32 declsec_flags_map[] = {
    0x00000000,                          /* empty */
    MONO_DECLSEC_FLAG_REQUEST,           /* SECURITY_ACTION_REQUEST
                                            (x01) */
    ...
    MONO_DECLSEC_FLAG_LINKDEMAND,        /* SECURITY_ACTION_LINKDEMAND			(x06)
                                          */
    MONO_DECLSEC_FLAG_INHERITANCEDEMAND, /* SECURITY_ACTION_INHERITANCEDEMAND
                                            (x07) */
    MONO_DECLSEC_FLAG_REQUEST_MINIMUM,   /* SECURITY_ACTION_REQUEST_MINIMUM
                                            (x08) */
    MONO_DECLSEC_FLAG_REQUEST_OPTIONAL,  /* SECURITY_ACTION_REQUEST_OPTIONAL
                                            (x09) */
    MONO_DECLSEC_FLAG_REQUEST_REFUSE,    /* SECURITY_ACTION_REQUEST_REFUSE
                                            (x0A) */
    ...
    MONO_DECLSEC_FLAG_NONCAS_DEMAND,     /* SECURITY_ACTION_NONCAS_DEMAND		(x0D)
                                          */
    MONO_DECLSEC_FLAG_NONCAS_LINKDEMAND, /* SECURITY_ACTION_NONCAS_LINKDEMAND
                                            (x0E) */
    MONO_DECLSEC_FLAG_NONCAS_INHERITANCEDEMAND, /* SECURITY_ACTION_NONCAS_INHERITANCEDEMAND
                                                   (x0F) */
    MONO_DECLSEC_FLAG_LINKDEMAND_CHOICE, /* SECURITY_ACTION_LINKDEMAND_CHOICE
                                            (x10) */
    MONO_DECLSEC_FLAG_INHERITANCEDEMAND_CHOICE, /* SECURITY_ACTION_INHERITANCEDEMAND_CHOICE
                                                   (x11) */
...
/*
 * Returns flags that includes all available security action associated to the
 * handle.
 * @token: metadata token (either for a class or a method)
 * @image: image where resides the metadata.
 */
static guint32 mono_declsec_get_flags(MonoImage *image, guint32 token) {
  ...
  for (i = index; i < t->rows; i++) {
    ...
    if ((action >= MONO_DECLSEC_ACTION_MIN) &&
        (action <= MONO_DECLSEC_ACTION_MAX)) {
 ...
 * Get the security actions (in the form of flags) associated with the specified
 * method. To keep \c MonoMethod size down we do not cache the declarative
 * security flags (except for the stack modifiers which are kept in the
 * MonoJitInfo structure) \returns the declarative security flags for the method
 * (only).
 */
 ...
 * Get the security actions (in the form of flags) associated with the specified
 * class. We cache the flags inside the \c MonoClass structure as this will get
 ...
 * \param assembly The assembly for which we want the declarative security
 * flags. Get the security actions (in the form of flags) associated with the
 * specified assembly. \returns the declarative security flags for the assembly.
 */
...
/*
 * Fill actions for the specific index (which may either be an encoded class
 * token or an encoded method token) from the metadata image. Returns TRUE if
 * some actions requiring code generation are present, FALSE otherwise.
 */
static MonoBoolean fill_actions_from_index(MonoImage *image, guint32 token,
                                           MonoDeclSecurityActions *actions,
                                           guint32 id_std, guint32 id_noncas,
                                           guint32 id_choice) {
  ...
  for (i = index; i < t->rows; i++) {
    ...
    if (cols[MONO_DECL_SECURITY_ACTION] == id_std) {
      if (!actions->demand.blob) {
        const char *blob = mono_metadata_blob_heap(
            image, cols[MONO_DECL_SECURITY_PERMISSIONSET]);
...
static MonoBoolean mono_declsec_get_class_demands_params(
    MonoClass *klass, MonoDeclSecurityActions *demands, guint32 id_std,
    guint32 id_noncas, guint32 id_choice) {
  guint32 idx = mono_metadata_token_index(m_class_get_type_token(klass));
  idx <<= MONO_HAS_DECL_SECURITY_BITS;
  idx |= MONO_HAS_DECL_SECURITY_TYPEDEF;
  return fill_actions_from_index(m_class_get_image(klass), idx, demands, id_std,
                                 id_noncas, id_choice);
}

static MonoBoolean mono_declsec_get_method_demands_params(
    MonoMethod *method, MonoDeclSecurityActions *demands, guint32 id_std,
    guint32 id_noncas, guint32 id_choice) {
  guint32 idx = mono_method_get_index(method);
  idx <<= MONO_HAS_DECL_SECURITY_BITS;
  idx |= MONO_HAS_DECL_SECURITY_METHODDEF;
  return fill_actions_from_index(m_class_get_image(method->klass), idx, demands,
                                 id_std, id_noncas, id_choice);
}
 ...
 */
MonoBoolean mono_declsec_get_demands(MonoMethod *method,
  ...
  if (method->flags & METHOD_ATTRIBUTE_HAS_SECURITY) {
    ...
    result = mono_declsec_get_method_demands_params(
        method, demands, SECURITY_ACTION_DEMAND, SECURITY_ACTION_NONCASDEMAND,
  ...
  if (flags & mask) {
    ...
    result |= mono_declsec_get_class_demands_params(
        method->klass, demands, SECURITY_ACTION_DEMAND,
        SECURITY_ACTION_NONCASDEMAND, SECURITY_ACTION_DEMANDCHOICE);
  }
 ...
 * Collect all Link actions: \c LinkDemand, \c NonCasLinkDemand and \c
 * LinkDemandChoice (2.0). Don't use the content of actions if the function
 * return FALSE.
 */
MonoBoolean mono_declsec_get_linkdemands(MonoMethod *method,
                                         MonoDeclSecurityActions *klass,
                                         MonoDeclSecurityActions *cmethod) {
  MonoBoolean result = FALSE;
  guint32 flags;
  ...
  if (method->flags & METHOD_ATTRIBUTE_HAS_SECURITY) {
    ...
    result = mono_declsec_get_method_demands_params(
        method, cmethod, SECURITY_ACTION_LINKDEMAND,
        SECURITY_ACTION_NONCASLINKDEMAND, SECURITY_ACTION_LINKDEMANDCHOICE);
  }

  /* Here we use (or create) the class declarative cache to look for demands */
  flags = mono_declsec_flags_from_class(method->klass);
  if (flags &
      (MONO_DECLSEC_FLAG_LINKDEMAND | MONO_DECLSEC_FLAG_NONCAS_LINKDEMAND |
       MONO_DECLSEC_FLAG_LINKDEMAND_CHOICE)) {
    mono_class_init_internal(method->klass);

    result |= mono_declsec_get_class_demands_params(
        method->klass, klass, SECURITY_ACTION_LINKDEMAND,
        SECURITY_ACTION_NONCASLINKDEMAND, SECURITY_ACTION_LINKDEMANDCHOICE);
  }
 ...
 * \param klass	The inherited class - this is the class that provides the
 * security check (attributes) \param demands Collect all Inherit actions - \c
 * InheritanceDemand, \c NonCasInheritanceDemand and \c InheritanceDemandChoice
 * (2.0). Don't use the content of actions if the function return FALSE.
 * \returns TRUE if inheritance demands (any kind) are present, FALSE otherwise.
 */
MonoBoolean
mono_declsec_get_inheritdemands_class(MonoClass *klass,
                                      MonoDeclSecurityActions *demands) {
  MonoBoolean result = FALSE;
  guint32 flags;
  ...
  if (flags & (MONO_DECLSEC_FLAG_INHERITANCEDEMAND |
    ...
    result |= mono_declsec_get_class_demands_params(
        klass, demands, SECURITY_ACTION_INHERITDEMAND,
        SECURITY_ACTION_NONCASINHERITANCE, SECURITY_ACTION_INHERITDEMANDCHOICE);
  }
 ...
 * Collect all Inherit actions: \c InheritanceDemand, \c NonCasInheritanceDemand
 * and \c InheritanceDemandChoice (2.0). Don't use the content of actions if the
 * function return FALSE.
 */
MonoBoolean
mono_declsec_get_inheritdemands_method(MonoMethod *method,
                                       MonoDeclSecurityActions *demands) {
  /* quick exit if no declarative security is present in the metadata */
  if (!m_class_get_image(method->klass)->tables[MONO_TABLE_DECLSECURITY].rows)
    return FALSE;
  ...
  if (method->flags & METHOD_ATTRIBUTE_HAS_SECURITY) {
    ...
    return mono_declsec_get_method_demands_params(
        method, demands, SECURITY_ACTION_INHERITDEMAND,
        SECURITY_ACTION_NONCASINHERITANCE, SECURITY_ACTION_INHERITDEMANDCHOICE);
  }
  return FALSE;
}

static MonoBoolean get_declsec_action(MonoImage *image, guint32 token,
                                      guint32 action,
  ...
  for (i = index; i < t->rows; i++) {
    ...
    if (cols[MONO_DECL_SECURITY_ACTION] == action) {
      const char *metadata = mono_metadata_blob_heap(
          image, cols[MONO_DECL_SECURITY_PERMISSIONSET]);
...
MonoBoolean mono_declsec_get_method_action(MonoMethod *method, guint32 action,
                                           MonoDeclSecurityEntry *entry) {
  if (method->flags & METHOD_ATTRIBUTE_HAS_SECURITY) {
    guint32 idx = mono_method_get_index(method);
    idx <<= MONO_HAS_DECL_SECURITY_BITS;
    idx |= MONO_HAS_DECL_SECURITY_METHODDEF;
    return get_declsec_action(m_class_get_image(method->klass), idx, action,
                              entry);
  }
  return FALSE;
}
 ...
 */
MonoBoolean mono_declsec_get_class_action(MonoClass *klass, guint32 action,
                                          MonoDeclSecurityEntry *entry) {
  /* use cache */
 ...
 */
MonoBoolean mono_declsec_get_assembly_action(MonoAssembly *assembly,
                                             guint32 action,
                                             MonoDeclSecurityEntry *entry) {
  guint32 idx = 1; /* there is only one assembly */
  idx <<= MONO_HAS_DECL_SECURITY_BITS;
...
gboolean mono_reflection_call_is_assignable_to(MonoClass *klass,
                                               MonoClass *oklass,
                                               MonoError *error) {
  MonoObject *res, *exc;
  void *params[1];
  ...
  if (method == NULL) {
    method = mono_class_get_method_from_name_checked(
        mono_class_get_type_builder_class(), "IsAssignableTo", 1, 0, error);
    mono_error_assert_ok(error);
    g_assert(method);
  }

  /*
   * The result of mono_type_get_object_checked () might be a System.MonoType
   * but we need a TypeBuilder so use mono_class_get_ref_info (klass).
   */
  g_assert(mono_class_has_ref_info(klass));
  g_assert(!strcmp(m_class_get_name(mono_object_class(
                       &mono_class_get_ref_info_raw(klass)->type.object)),
                   "TypeBuilder")); /* FIXME use handles */

  params[0] = mono_type_get_object_checked(
      mono_domain_get(), m_class_get_byval_arg(oklass), error);
  return_val_if_nok(error, FALSE);

  ERROR_DECL(inner_error);
  res = mono_runtime_try_invoke(
      method, &mono_class_get_ref_info_raw(klass)->type.object, params, &exc,
      inner_error); /* FIXME use handles */
 ...
 * \returns the \c MonoType* associated with the C# \c System.Type object \p
 * reftype.
 */
 ...
 * \returns the \c MonoAssembly* associated with the C# \c
 * System.Reflection.Assembly object \p refassembly.
 */
MonoAssembly *
 ...
 */
MonoClass *mono_class_from_mono_type_handle(MonoReflectionTypeHandle reftype) {
  return mono_class_from_mono_type_internal(MONO_HANDLE_RAW(reftype)->type);
}

// This is called by icalls, it will return NULL and set pending exception (in
// wrapper) on failure.
MonoReflectionTypeHandle mono_type_from_handle_impl(MonoType *handle,
                                                    MonoError *error) {
