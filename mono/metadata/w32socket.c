...
/* Bizarrely, IBM i implements this, but AIX doesn't, so on i, it has a
 * different name... */
...
#if defined(_MSC_VER) &&                                                       \
    G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT | HAVE_UWP_WINAPI_SUPPORT)
#include <MSWSock.h>
#endif
...
static gboolean addrinfo_to_IPHostEntry_handles(MonoAddressInfo *info,
...
create_object_handle_from_sockaddr(struct sockaddr *saddr, int sa_size,
                                   gint32 *werror, MonoError *error);

static struct sockaddr *create_sockaddr_from_handle(MonoObjectHandle saddr_obj,
                                                    socklen_t *sa_size,
                                                    gint32 *werror,
                                                    MonoError *error);
...
static SOCKET mono_w32socket_socket(int domain, int type, int protocol) {
  SOCKET ret;
  MONO_ENTER_GC_SAFE;
...
static gint mono_w32socket_bind(SOCKET sock, struct sockaddr *addr,
                                socklen_t addrlen) {
  gint ret;
  MONO_ENTER_GC_SAFE;
...
static gint mono_w32socket_getpeername(SOCKET sock, struct sockaddr *name,
                                       socklen_t *namelen) {
  gint ret;
  MONO_ENTER_GC_SAFE;
...
static gint mono_w32socket_getsockname(SOCKET sock, struct sockaddr *name,
                                       socklen_t *namelen) {
  gint ret;
  MONO_ENTER_GC_SAFE;
...
static gint mono_w32socket_getsockopt(SOCKET sock, gint level, gint optname,
                                      gpointer optval, socklen_t *optlen) {
  gint ret;
  MONO_ENTER_GC_SAFE;
...
static gint mono_w32socket_setsockopt(SOCKET sock, gint level, gint optname,
                                      gconstpointer optval, socklen_t optlen) {
  gint ret;
  MONO_ENTER_GC_SAFE;
...
static gint mono_w32socket_listen(SOCKET sock, gint backlog) {
  gint ret;
  MONO_ENTER_GC_SAFE;
...
static gint mono_w32socket_shutdown(SOCKET sock, gint how) {
  gint ret;
  MONO_ENTER_GC_SAFE;
...
static gint mono_w32socket_ioctl(SOCKET sock, gint32 command, gchar *input,
                                 gint inputlen, gchar *output, gint outputlen,
                                 DWORD *written) {
  gint ret;
  MONO_ENTER_GC_SAFE;
  ret = WSAIoctl(sock, command, input, inputlen, output, outputlen, written,
                 NULL, NULL);
  MONO_EXIT_GC_SAFE;
  return ret;
}

static gboolean mono_w32socket_close(SOCKET sock) {
  gboolean ret;
  MONO_ENTER_GC_SAFE;
...
static gint32 convert_family(MonoAddressFamily mono_family) {
  ...
  case AddressFamily_NetworkDesigners:
    g_warning("System.Net.Sockets.AddressFamily has unsupported value 0x%x",
              mono_family);
    return -1;
...
#endif
  default:
    g_warning("System.Net.Sockets.AddressFamily has unknown value 0x%x",
              mono_family);
    return -1;
  }
}
...
static gint32 convert_type(MonoSocketType mono_type) {
  ...
  case SocketType_Unknown:
    g_warning("System.Net.Sockets.SocketType has unsupported value 0x%x",
              mono_type);
    return -1;
  default:
    g_warning("System.Net.Sockets.SocketType has unknown value 0x%x",
              mono_type);
    return -1;
  }
}
...
static gint32 convert_proto(MonoProtocolType mono_proto) {
  ...
  case ProtocolType_Unknown:
    ...
    g_warning("System.Net.Sockets.ProtocolType has unsupported value 0x%x",
              mono_proto);
    return -1;
  default:
    return -1;
...
static gint32 convert_socketflags(gint32 sflags) {
  ...
  if (sflags &
      ~(SocketFlags_OutOfBand | SocketFlags_MaxIOVectorLength |
        SocketFlags_Peek | SocketFlags_DontRoute | SocketFlags_Partial))
...
/*
 * Returns:
 *    0 on success (mapped mono_level and mono_name to system_level and
 * system_name -1 on error -2 on non-fatal error (ie, must ignore)
 */
static gint32 convert_sockopt_level_and_name(MonoSocketOptionLevel mono_level,
...
#endif
    default:
      g_warning("System.Net.Sockets.SocketOptionName 0x%x is not supported at "
                "Socket level",
                mono_name);
      return -1;
    }
  ...
  case SocketOptionLevel_Udp:
    g_warning("System.Net.Sockets.SocketOptionLevel has unsupported value 0x%x",
              mono_level);
  ...
  default:
    g_warning("System.Net.Sockets.SocketOptionLevel has unknown value 0x%x",
              mono_level);
    return -1;
  }
...
gpointer ves_icall_System_Net_Sockets_Socket_Socket_internal(
    MonoObjectHandle this_obj, gint32 family, gint32 type, gint32 proto,
    gint32 *werror, MonoError *error) {
  SOCKET sock;
  gint32 sock_family;
  gint32 sock_proto;
 ...
 */
void ves_icall_System_Net_Sockets_Socket_Close_internal(gsize sock,
                                                        gint32 *werror,
                                                        MonoError *error) {
...
ves_icall_System_Net_Sockets_SocketException_WSAGetLastError_internal(void) {
  LOGDEBUG(
      g_message("%s: returning %d", __func__, mono_w32socket_get_last_error()));

  return mono_w32socket_get_last_error();
}

gint32 ves_icall_System_Net_Sockets_Socket_Available_internal(
    gsize sock, gint32 *werror, MonoError *error) {
  int ret;
  guint64 amount;
...
void ves_icall_System_Net_Sockets_Socket_Blocking_internal(gsize sock,
                                                           MonoBoolean block,
                                                           gint32 *werror,
                                                           MonoError *error) {
  int ret;
...
gpointer ves_icall_System_Net_Sockets_Socket_Accept_internal(
    gsize sock, gint32 *werror, MonoBoolean blocking, MonoError *error) {
  SOCKET newsock;
...
void ves_icall_System_Net_Sockets_Socket_Listen_internal(gsize sock,
                                                         guint32 backlog,
                                                         gint32 *werror,
                                                         MonoError *error) {
  int ret;
...
static gboolean is_ipv4_mapped_any(const struct in6_addr *addr) {
  int i;
...
create_object_handle_from_sockaddr(struct sockaddr *saddr, int sa_size,
  ...
  if (!domain->sockaddr_class)
    domain->sockaddr_class = mono_class_load_from_name(
        get_socket_assembly(), "System.Net", "SocketAddress");
  MonoObjectHandle sockaddr_obj =
      mono_object_new_handle(domain, domain->sockaddr_class, error);
  return_val_if_nok(error, MONO_HANDLE_NEW(MonoObject, NULL));

  /* Locate the SocketAddress data buffer in the object */
  if (!domain->sockaddr_data_field) {
    domain->sockaddr_data_field = mono_class_get_field_from_name_full(
        domain->sockaddr_class, "m_Buffer", NULL);
  ...
  if (!domain->sockaddr_data_length_field) {
    domain->sockaddr_data_length_field = mono_class_get_field_from_name_full(
        domain->sockaddr_class, "m_Size", NULL);
  ...
  /* We can't really avoid the +2 as all code below depends on this size -
   * INCLUDING unix domain sockets.*/
  MonoArrayHandle data =
      mono_array_new_handle(domain, mono_get_byte_class(), sa_size + 2, error);
  return_val_if_nok(error, MONO_HANDLE_NEW(MonoObject, NULL));
  ...
  if (saddr->sa_family == AF_INET) {
    ...
    mono_field_set_value_internal(
        MONO_HANDLE_RAW(sockaddr_obj), domain->sockaddr_data_field,
        MONO_HANDLE_RAW(
            data)); /* FIXME: use handles for mono_field_set_value */
    mono_field_set_value_internal(
        MONO_HANDLE_RAW(sockaddr_obj), domain->sockaddr_data_length_field,
        &buffer_size); /* FIXME: use handles for mono_field_set_value */
...
static int get_sockaddr_size(int family) {
  int size;
...
static MonoObjectHandle mono_w32socket_getname(gsize sock, gint32 af,
                                               gboolean local, gint32 *werror,
                                               MonoError *error) {
  gpointer sa = NULL;
  socklen_t salen = 0;
  int ret;
  ...
  *werror = 0;
  ...
  ret = (local ? mono_w32socket_getsockname : mono_w32socket_getpeername)(
      sock, (struct sockaddr *)sa, &salen);
  if (ret == SOCKET_ERROR) {
    *werror = mono_w32socket_get_last_error();
    goto exit;
  }

  LOGDEBUG(g_message("%s: %s to %s port %d", __func__,
                     local ? "bound" : "connected",
                     inet_ntoa(((struct sockaddr_in *)&sa)->sin_addr),
                     ntohs(((struct sockaddr_in *)&sa)->sin_port)));

  result = create_object_handle_from_sockaddr((struct sockaddr *)sa, salen,
                                              werror, error);
exit:
  if (salen > 128)
    g_free(sa);
...
MonoObjectHandle ves_icall_System_Net_Sockets_Socket_LocalEndPoint_internal(
    gsize sock, gint32 af, gint32 *werror, MonoError *error) {
  return mono_w32socket_getname(sock, af, TRUE, werror, error);
}

MonoObjectHandle ves_icall_System_Net_Sockets_Socket_RemoteEndPoint_internal(
    gsize sock, gint32 af, gint32 *werror, MonoError *error) {
...
static struct sockaddr *create_sockaddr_from_handle(MonoObjectHandle saddr_obj,
  ...
  MonoArrayHandle data = MONO_HANDLE_NEW_GET_FIELD(saddr_obj, MonoArray,
   ...
   */
  len = MONO_HANDLE_GET_FIELD_VAL(saddr_obj, int,
                                  domain->sockaddr_data_length_field);
  g_assert(len >= 2);
    ...
    *sa_size = sizeof(struct sockaddr_in6);
    ...
    if (len - 2 >= sizeof(sock_un->sun_path)) {
      mono_error_set_argument_out_of_range(
          error, "SocketAddress.Size",
          "MonoArgumentException:SocketAddress.Size");
      mono_gchandle_free_internal(gchandle);
      return NULL;
    }
...
void ves_icall_System_Net_Sockets_Socket_Bind_internal(
    gsize sock, MonoObjectHandle sockaddr, gint32 *werror, MonoError *error) {
  struct sockaddr *sa;
  socklen_t sa_size;
  int ret;
  ...
  *werror = 0;
  ...
  LOGDEBUG(g_message("%s: binding to %s port %d", __func__,
                     inet_ntoa(((struct sockaddr_in *)sa)->sin_addr),
                     ntohs(((struct sockaddr_in *)sa)->sin_port)));
...
enum { SelectModeRead, SelectModeWrite, SelectModeError };

MonoBoolean ves_icall_System_Net_Sockets_Socket_Poll_internal(
    gsize sock, gint mode, gint timeout, gint32 *werror, MonoError *error) {
...
void ves_icall_System_Net_Sockets_Socket_Connect_internal(
    gsize sock, MonoObjectHandle sockaddr, gint32 *werror, MonoBoolean blocking,
    MonoError *error) {
  struct sockaddr *sa;
  socklen_t sa_size;
  int ret;
  ...
  *werror = 0;
  ...
  LOGDEBUG(g_message("%s: connecting to %s port %d", __func__,
                     inet_ntoa(((struct sockaddr_in *)sa)->sin_addr),
                     ntohs(((struct sockaddr_in *)sa)->sin_port)));
...
void ves_icall_System_Net_Sockets_Socket_Disconnect_internal(gsize sock,
                                                             MonoBoolean reuse,
                                                             gint32 *werror,
                                                             MonoError *error) {
  error_init(error);

  LOGDEBUG(g_message("%s: disconnecting from socket %p (reuse %d)", __func__,
                     sock, reuse));
...
MonoBoolean ves_icall_System_Net_Sockets_Socket_Duplicate_internal(
    gpointer handle, gint32 targetProcessId, gpointer *duplicate_handle,
    gint32 *werror, MonoError *error) {
  error_init(error);
...
gint32 ves_icall_System_Net_Sockets_Socket_Receive_internal(
    gsize sock, gchar *buffer, gint32 count, gint32 flags, gint32 *werror,
...
gint32 ves_icall_System_Net_Sockets_Socket_Receive_array_internal(
    gsize sock, WSABUF *buffers, gint32 count, gint32 flags, gint32 *werror,
    MonoBoolean blocking, MonoError *error) {
  int ret;
  guint32 recv;
  guint32 recvflags = 0;
  ...
  *werror = 0;
  ...
  ret = mono_w32socket_recvbuffers(sock, buffers, count, &recv, &recvflags,
                                   NULL, NULL, blocking);
  if (ret == SOCKET_ERROR) {
    *werror = mono_w32socket_get_last_error();
    return 0;
...
gint32 ves_icall_System_Net_Sockets_Socket_ReceiveFrom_internal(
    gsize sock, gchar *buffer, gint32 count, gint32 flags,
    MonoObjectHandleInOut sockaddr, gint32 *werror, MonoBoolean blocking,
    MonoError *error) {
  int ret;
  int recvflags = 0;
  struct sockaddr *sa;
  ...
  *werror = 0;
  ...
  ret = mono_w32socket_recvfrom(sock, buffer, count, recvflags, sa, &sa_size,
                                blocking);
  if (ret == SOCKET_ERROR) {
    *werror = mono_w32socket_get_last_error();
    g_free(sa);
   ...
   */
  if (sa_size) {
    MONO_HANDLE_ASSIGN(sockaddr, create_object_handle_from_sockaddr(
                                     sa, sa_size, werror, error));
    if (!is_ok(error)) {
      g_free(sa);
      return 0;
    }
...
gint32 ves_icall_System_Net_Sockets_Socket_Send_internal(
    gsize sock, gchar *buffer, gint32 count, gint32 flags, gint32 *werror,
    MonoBoolean blocking, MonoError *error) {
  int ret;
  int sendflags = 0;
...
gint32 ves_icall_System_Net_Sockets_Socket_Send_array_internal(
    gsize sock, WSABUF *buffers, gint32 count, gint32 flags, gint32 *werror,
    MonoBoolean blocking, MonoError *error) {
  int ret;
  guint32 sent;
  ...
  *werror = 0;
  ...
  ret = mono_w32socket_sendbuffers(sock, buffers, count, &sent, sendflags, NULL,
                                   NULL, blocking);
  if (ret == SOCKET_ERROR) {
    *werror = mono_w32socket_get_last_error();
    return 0;
...
gint32 ves_icall_System_Net_Sockets_Socket_SendTo_internal(
    gsize sock, gchar *buffer, gint32 count, gint32 flags,
    MonoObjectHandle sockaddr, gint32 *werror, MonoBoolean blocking,
    MonoError *error) {
  int ret;
  int sendflags = 0;
  struct sockaddr *sa;
  ...
  *werror = 0;
  ...
  ret = mono_w32socket_sendto(sock, buffer, count, sendflags, sa, sa_size,
                              blocking);
  if (ret == SOCKET_ERROR) {
    *werror = mono_w32socket_get_last_error();
    g_free(sa);
...
static SOCKET Socket_to_SOCKET(MonoObjectHandle sockobj) {
  MonoClassField *field;

  field = mono_class_get_field_from_name_full(mono_handle_class(sockobj),
                                              "m_Handle", NULL);
  MonoSafeHandleHandle safe_handle =
      MONO_HANDLE_NEW_GET_FIELD(sockobj, MonoSafeHandle, field);
...
static gboolean collect_pollfds_from_array(MonoArrayHandle sockets, int i,
                                           int nfds, mono_pollfd *pfds,
                                           int *idx, int *mode) {
  ...
  pfds[*idx].events =
      (*mode == 0) ? MONO_POLLIN : (*mode == 1) ? MONO_POLLOUT : POLL_ERRORS;
  (*idx)++;
leave:
  HANDLE_FUNCTION_RETURN_VAL(result);
}

static void set_socks_array_from_pollfds(MonoArrayHandle sockets, int i,
                                         mono_pollfd *pfds, int *ret, int *mode,
                                         MonoArrayHandle socks, int *idx) {
  HANDLE_FUNCTION_ENTER();
  mono_pollfd *pfd;
...
void ves_icall_System_Net_Sockets_Socket_Select_internal(
    MonoArrayHandleOut sockets, gint32 timeout, gint32 *werror,
    MonoError *error) {
  MonoInternalThread *thread = mono_thread_internal_current();
  mono_pollfd *pfds;
  int nfds, idx;
  ...
  *werror = 0;
  ...
  MonoArrayHandle socks = MONO_HANDLE_NEW(
      MonoArray, mono_array_new_full_checked(mono_domain_get(), sock_arr_class,
                                             &socks_size, NULL, error));
  if (!is_ok(error)) {
    g_free(pfds);
    return;
  }
...
static MonoObjectHandle int_to_object_handle(MonoDomain *domain, int val,
                                             MonoError *error) {
  return MONO_HANDLE_NEW(
      MonoObject,
      mono_value_box_checked(domain, mono_get_int32_class(), &val, error));
}

void ves_icall_System_Net_Sockets_Socket_GetSocketOption_obj_internal(
    gsize sock, gint32 level, gint32 name, MonoObjectHandleOut obj_val,
    gint32 *werror, MonoError *error) {
  int system_level = 0;
  int system_name = 0;
  int ret;
  ...
  *werror = 0;
  ...
  if (level == SocketOptionLevel_Socket &&
      name == SocketOptionName_ExclusiveAddressUse) {
    system_level = SOL_SOCKET;
    system_name = SO_REUSEADDR;
    ret = 0;
  } else
#endif
  {
    ret = convert_sockopt_level_and_name((MonoSocketOptionLevel)level,
                                         (MonoSocketOptionName)name,
  ...
  case SocketOptionName_Linger:
  case SocketOptionName_DontLinger:
    ret = mono_w32socket_getsockopt(sock, system_level, system_name, &linger,
                                    &lingersize);
    break;

  case SocketOptionName_SendTimeout:
  case SocketOptionName_ReceiveTimeout:
    ret = mono_w32socket_getsockopt(sock, system_level, system_name, &time_ms,
                                    &time_ms_size);
    break;

#ifdef SO_PEERCRED
  case SocketOptionName_PeerCred:
    ret = mono_w32socket_getsockopt(sock, system_level, system_name, &cred,
                                    &credsize);
    break;
#endif

  default:
    ret = mono_w32socket_getsockopt(sock, system_level, system_name, &val,
                                    &valsize);
  }
  ...
  case SocketOptionName_PeerCred: {
    ...
    if (mono_posix_image == NULL) {
      ...
      if (!mono_posix_image) {
        ...
        mono_assembly_request_prepare_open(&req, MONO_ASMCTX_DEFAULT, alc);
        MonoAssembly *sa =
            mono_assembly_request_open("Mono.Posix.dll", &req, NULL);
        if (!sa) {
          *werror = WSAENOPROTOOPT;
          return;
        } else {
    ...
    obj_class = mono_class_load_from_name(mono_posix_image, "Mono.Posix",
                                          "PeerCredData");
    MonoPeerCredDataHandle cred_data = MONO_HANDLE_CAST(
        MonoPeerCredData, mono_object_new_handle(domain, obj_class, error));
    return_if_nok(error);
...
void ves_icall_System_Net_Sockets_Socket_GetSocketOption_arr_internal(
    gsize sock, gint32 level, gint32 name, MonoArrayHandle byte_val,
    gint32 *werror, MonoError *error) {
  int system_level = 0;
  int system_name = 0;
  int ret;
  ...
  *werror = 0;
  ...
  ret =
      mono_w32socket_getsockopt(sock, system_level, system_name, buf, &valsize);
...
ipaddress_handle_to_struct_in_addr(MonoObjectHandle ipaddr) {
  ...
  field = mono_class_get_field_from_name_full(mono_handle_class(ipaddr),
   ...
   */
  inaddr.s_addr =
      GUINT_FROM_LE((guint32)MONO_HANDLE_GET_FIELD_VAL(ipaddr, guint32, field));
...
#if !(defined(HAVE_GETIFADDRS) || defined(HAVE_QP2GETIFADDRS)) ||              \
...
void ves_icall_System_Net_Sockets_Socket_SetSocketOption_internal(
    gsize sock, gint32 level, gint32 name, MonoObjectHandle obj_val,
    MonoArrayHandle byte_val, gint32 int_val, gint32 *werror,
    MonoError *error) {
  struct linger linger;
  int system_level = 0;
  int system_name = 0;
  ...
  *werror = 0;
  ...
  if (!MONO_HANDLE_IS_NULL(obj_val)) {
    ...
    case SocketOptionName_Linger:
      ...
      field =
          mono_class_get_field_from_name_full(obj_class, "lingerTime", NULL);
      linger.l_linger = MONO_HANDLE_GET_FIELD_VAL(obj_val, guint32, field);

      valsize = sizeof(linger);
      ret = mono_w32socket_setsockopt(sock, system_level, system_name, &linger,
                                      valsize);
      break;
    case SocketOptionName_AddMembership:
    case SocketOptionName_DropMembership:
#if defined(HAVE_STRUCT_IP_MREQN) || defined(HAVE_STRUCT_IP_MREQ)
    {
      ...
      if (system_level == sol_ipv6) {
        ...
        g_assert(field);
        MONO_HANDLE_ASSIGN(
            address, MONO_HANDLE_NEW_GET_FIELD(obj_val, MonoObject, field));

        if (!MONO_HANDLE_IS_NULL(address))
          mreq6.ipv6mr_multiaddr = ipaddress_handle_to_struct_in6_addr(address);

        field =
            mono_class_get_field_from_name_full(obj_class, "m_Interface", NULL);
        mreq6.ipv6mr_interface =
            MONO_HANDLE_GET_FIELD_VAL(obj_val, guint64, field);
        ...
        ret = mono_w32socket_setsockopt(sock, system_level, system_name, &mreq6,
                                        sizeof(mreq6));
      ...
      if (system_level == sol_ip) {
        ...
        field = mono_class_get_field_from_name_full(obj_class, "localAddress",
                                                    NULL);
        MONO_HANDLE_ASSIGN(
            address, MONO_HANDLE_NEW_GET_FIELD(obj_val, MonoObject, field));
        ...
        ret = mono_w32socket_setsockopt(sock, system_level, system_name, &mreq,
                                        sizeof(mreq));
      }
      break;
    }
    ...
    case SocketOptionName_DontLinger:
      if (valsize == 1) {
        ...
        linger.l_linger = 0;
        ret = mono_w32socket_setsockopt(sock, system_level, system_name,
      ...
      break;
    default:
      ret = mono_w32socket_setsockopt(sock, system_level, system_name, buf,
                                      valsize);
      break;
    }
    mono_gchandle_free_internal(gchandle);
...
#endif /* HOST_WIN32 */
      /* int_val is in_addr */
      ret = mono_w32socket_setsockopt(sock, system_level, system_name, &int_val,
                                      sizeof(int_val));
      break;
    case SocketOptionName_DontFragment:
...
void ves_icall_System_Net_Sockets_Socket_Shutdown_internal(gsize sock,
                                                           gint32 how,
                                                           gint32 *werror,
                                                           MonoError *error) {
  int ret;
...
gint ves_icall_System_Net_Sockets_Socket_IOControl_internal(
    gsize sock, gint32 code, MonoArrayHandle input, MonoArrayHandle output,
    gint32 *werror, MonoError *error) {
#ifdef HOST_WIN32
  ...
  *werror = 0;
  ...
  ret = mono_w32socket_ioctl(sock, code, i_buffer, i_len, o_buffer, o_len,
                             &output_bytes);
...
static gboolean addrinfo_add_string(MonoDomain *domain, const char *s,
                                    MonoArrayHandle arr, int index,
                                    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
...
static int addrinfo_add_local_ips(MonoDomain *domain,
                                  MonoArrayHandleOut h_addr_list,
                                  MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  local_in6 =
      (struct in6_addr *)mono_get_local_interfaces(AF_INET6, &nlocal_in6);
  if (nlocal_in || nlocal_in6) {
    char addr[INET6_ADDRSTRLEN];
    MONO_HANDLE_ASSIGN(h_addr_list,
                       mono_array_new_handle(domain, mono_get_string_class(),
                                             nlocal_in + nlocal_in6, error));
    goto_if_nok(error, leave);
    ...
    if (nlocal_in) {
      ...
      for (i = 0; i < nlocal_in; i++) {
        ...
        if (mono_networking_addr_to_str(&maddr, addr, sizeof(addr))) {
          if (!addrinfo_add_string(domain, addr, h_addr_list, addr_index,
                                   error))
            goto leave;
          addr_index++;
        }
...
static gboolean addrinfo_to_IPHostEntry_handles(MonoAddressInfo *info,
  ...
  error_init(error);
  MONO_HANDLE_ASSIGN(h_aliases, mono_array_new_handle(
                                    domain, mono_get_string_class(), 0, error));
  goto_if_nok(error, leave);
  if (add_local_ips) {
  ...
  MONO_HANDLE_ASSIGN(
      h_addr_list,
      mono_array_new_handle(domain, mono_get_string_class(), count, error));
  goto_if_nok(error, leave);
  ...
  for (ai = info->entries; ai != NULL; ai = ai->next) {
    ...
    if (!name_assigned) {
      name_assigned = TRUE;
      const char *name =
          ai->canonical_name != NULL ? ai->canonical_name : buffer;
...
MonoBoolean ves_icall_System_Net_Dns_GetHostByName_internal(
    MonoStringHandle host, MonoStringHandleOut h_name,
    MonoArrayHandleOut h_aliases, MonoArrayHandleOut h_addr_list, gint32 hint,
    MonoError *error) {
  ...
  add_local_ips = FALSE;
  if (mono_get_address_info(hostname, 0, MONO_HINT_CANONICAL_NAME | hint,
                            &info))
    add_info_ok = FALSE;
#else
  if (*hostname && mono_get_address_info(
                       hostname, 0, MONO_HINT_CANONICAL_NAME | hint, &info))
    add_info_ok = FALSE;
#endif
  ...
  if (add_info_ok) {
    MonoBoolean result = addrinfo_to_IPHostEntry_handles(
        info, h_name, h_aliases, h_addr_list, add_local_ips, error);
    return result;
  }
  return FALSE;
}

MonoBoolean ves_icall_System_Net_Dns_GetHostByAddr_internal(
    MonoStringHandle addr, MonoStringHandleOut h_name,
    MonoArrayHandleOut h_aliases, MonoArrayHandleOut h_addr_list, gint32 hint,
    MonoError *error) {
  char *address;
...
#endif
    MONO_ENTER_GC_SAFE;
    ret = getnameinfo((struct sockaddr *)&saddr, sizeof(saddr), hostname,
...
#endif
    MONO_ENTER_GC_SAFE;
    ret = getnameinfo((struct sockaddr *)&saddr6, sizeof(saddr6), hostname,
                      sizeof(hostname), NULL, 0, 0) == 0;
    MONO_EXIT_GC_SAFE;
    break;
  }
  ...
  if (mono_get_address_info(hostname, 0,
                            hint | MONO_HINT_CANONICAL_NAME |
                                MONO_HINT_CONFIGURED_ONLY,
                            &info) != 0)
    return FALSE;

  MonoBoolean result = addrinfo_to_IPHostEntry_handles(
      info, h_name, h_aliases, h_addr_list, FALSE, error);
  return result;
}

MonoBoolean
ves_icall_System_Net_Dns_GetHostName_internal(MonoStringHandleOut h_name,
                                              MonoError *error) {
  gchar hostname[NI_MAXHOST] = {0};
  int ret;
  ...
  MONO_HANDLE_ASSIGN(
      h_name, mono_string_new_handle(mono_domain_get(), hostname, error));
  return TRUE;
}

#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT | HAVE_UWP_WINAPI_SUPPORT)
MonoBoolean ves_icall_System_Net_Sockets_Socket_SendFile_internal(
    gsize sock, MonoStringHandle filename, MonoArrayHandle pre_buffer,
    MonoArrayHandle post_buffer, gint flags, gint32 *werror,
    MonoBoolean blocking, MonoError *error) {
  HANDLE file;
  gboolean ret;
  ...
  *werror = 0;
  ...
  /* FIXME: replace file by a proper fd that we can call open and close on, as
   * they are interruptible */

  uint32_t filename_gchandle;
  gunichar2 *filename_chars =
      mono_string_handle_pin_chars(filename, &filename_gchandle);
  file = mono_w32file_create(filename_chars, GENERIC_READ, FILE_SHARE_READ,
                             OPEN_EXISTING, 0);
  ...
  if (!MONO_HANDLE_IS_NULL(pre_buffer)) {
    buffers.Head =
        MONO_ARRAY_HANDLE_PIN(pre_buffer, guchar, 0, &pre_buffer_gchandle);
    buffers.HeadLength = mono_array_handle_length(pre_buffer);
  }
  if (!MONO_HANDLE_IS_NULL(post_buffer)) {
    buffers.Tail =
        MONO_ARRAY_HANDLE_PIN(post_buffer, guchar, 0, &post_buffer_gchandle);
...
void ves_icall_cancel_blocking_socket_operation(MonoThreadObjectHandle thread,
                                                MonoError *error) {
  error_init(error);
  MonoInternalThreadHandle internal =
      MONO_HANDLE_NEW_GET(MonoInternalThread, thread, internal_thread);
