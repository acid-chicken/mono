...
/*
 * This TLS variable holds how many times the current thread has acquired the
 * loader lock.
 */
...
void mono_loader_lock(void) {
  ...
  if (G_UNLIKELY(loader_lock_track_ownership)) {
    mono_native_tls_set_value(
        loader_lock_nest_id,
        GUINT_TO_POINTER(
            GPOINTER_TO_UINT(mono_native_tls_get_value(loader_lock_nest_id)) +
            1));
  }
}
 ...
 *
 *   Set whenever the runtime should track ownership of the loader lock. If set
 * to TRUE, the mono_loader_lock_is_owned_by_self () can be called to query
 * whenever the current thread owns the loader lock.
 */
 ...
 *
 *   Acquire the loader lock if it has been initialized, no-op otherwise. This
 * can be used in runtime initialization code which can be executed before
 * mono_loader_init ().
 */
 ...
 * We use a gpointer since the cache stores both MonoTypes and
 * MonoMethodSignatures. A cache is needed since the type/signature parsing
 * routines allocate everything from a mempool, so without a cache, multiple
 * requests for the same signature would lead to unbounded memory growth. For
 * normal methods/fields this is not a problem since the resulting
 * methods/fields are cached, but inflated methods/fields cannot be cached.
 * LOCKING: Acquires the loader lock.
 */
static gpointer find_cached_memberref_sig(MonoImage *image, guint32 sig_idx) {
  gpointer res;

  mono_image_lock(image);
  res = g_hash_table_lookup(image->memberref_signatures,
...
static gpointer cache_memberref_sig(MonoImage *image, guint32 sig_idx,
                                    gpointer sig) {
  gpointer prev_sig;

  mono_image_lock(image);
  prev_sig = g_hash_table_lookup(image->memberref_signatures,
                                 GUINT_TO_POINTER(sig_idx));
  if (prev_sig) {
    /* Somebody got in before us */
    sig = prev_sig;
  } else {
    g_hash_table_insert(image->memberref_signatures, GUINT_TO_POINTER(sig_idx),
                        sig);
...
static MonoClassField *field_from_memberref(MonoImage *image, guint32 token,
                                            MonoClass **retklass,
  ...
  if (!mono_verifier_verify_memberref_field_signature(
          image, cols[MONO_MEMBERREF_SIGNATURE], error))
    return NULL;
  ...
  case MONO_MEMBERREF_PARENT_TYPEREF:
    klass = mono_class_from_typeref_checked(image, MONO_TOKEN_TYPE_REF | nindex,
                                            error);
    break;
  case MONO_MEMBERREF_PARENT_TYPESPEC:
    klass = mono_class_get_and_inflate_typespec_checked(
        image, MONO_TOKEN_TYPE_SPEC | nindex, context, error);
    break;
  default:
    mono_error_set_bad_image(error, image,
                             "Bad field field '%u' signature 0x%08x",
                             class_index, token);
  }
   ...
   * we ask mono_metadata_parse_type_checked () to allocates everything from a
   * mempool.
   * FIXME part2, mono_metadata_parse_type_checked actually allows for a
   * transient type instead.
   * FIXME part3, transient types are not 100% transient, so we need to take
   * care of that first.
   */
  sig_type = (MonoType *)find_cached_memberref_sig(
      image, cols[MONO_MEMBERREF_SIGNATURE]);
  if (!sig_type) {
    ERROR_DECL(inner_error);
    sig_type = mono_metadata_parse_type_checked(image, NULL, 0, FALSE, ptr,
                                                &ptr, inner_error);
    if (sig_type == NULL) {
      mono_error_set_field_missing(
          error, klass, fname, NULL,
    ...
    sig_type = (MonoType *)cache_memberref_sig(
        image, cols[MONO_MEMBERREF_SIGNATURE], sig_type);
  }
  ...
  if (!field) {
    mono_error_set_field_missing(error, klass, fname, sig_type,
 ...
 */
MonoClassField *mono_field_from_token(MonoImage *image, guint32 token,
                                      MonoClass **retklass,
                                      MonoGenericContext *context) {
  ERROR_DECL(error);
  MonoClassField *res =
      mono_field_from_token_checked(image, token, retklass, context, error);
  mono_error_assert_ok(error);
  return res;
}

MonoClassField *mono_field_from_token_checked(MonoImage *image, guint32 token,
                                              MonoClass **retklass,
                                              MonoGenericContext *context,
                                              MonoError *error) {
  MonoClass *k;
  ...
  if (image_is_dynamic(image)) {
    ...
    *retklass = NULL;
    ERROR_DECL(inner_error);
    result = (MonoClassField *)mono_lookup_dynamic_token_class(
        image, token, TRUE, &handle_class, context, inner_error);
    mono_error_cleanup(inner_error);
  ...
  if ((field = (MonoClassField *)mono_conc_hashtable_lookup(
           image->field_cache, GUINT_TO_POINTER(token)))) {
  ...
  if (mono_metadata_token_table(token) == MONO_TABLE_MEMBERREF) {
    ...
    type = mono_metadata_typedef_from_field(image,
                                            mono_metadata_token_index(token));
    if (!type) {
      mono_error_set_bad_image(error, image, "Invalid field token 0x%08x",
                               token);
      return NULL;
    }
    ...
    if (mono_class_has_failure(k)) {
      ...
      mono_error_set_bad_image(
          error, image, "Could not resolve field token 0x%08x, due to: %s",
          token, mono_error_get_message(causedby_error));
  ...
  if (field && field->parent && !mono_class_is_ginst(field->parent) &&
      !mono_class_is_gtd(field->parent)) {
    mono_image_lock(image);
    mono_conc_hashtable_insert(image->field_cache, GUINT_TO_POINTER(token),
                               field);
    mono_image_unlock(image);
  }
...
static gboolean
mono_metadata_signature_vararg_match(MonoMethodSignature *sig1,
                                     MonoMethodSignature *sig2) {
  int i;

  if (sig1->hasthis != sig2->hasthis || sig1->sentinelpos != sig2->sentinelpos)
    return FALSE;
...
static MonoMethod *find_method_in_class(MonoClass *klass, const char *name,
                                        const char *qname, const char *fqname,
                                        MonoMethodSignature *sig,
                                        MonoClass *from_class,
                                        MonoError *error) {
  int i;
  ...
  if (m_class_get_type_token(klass) && !image_is_dynamic(klass_image) &&
      !m_class_get_methods(klass) && !m_class_get_rank(klass) &&
      klass == from_class && !mono_class_is_ginst(from_class)) {
  ...
  /*
  We can't fail lookup of methods otherwise the runtime will fail with
  MissingMethodException instead of TypeLoadException. See
  mono/tests/generic-type-load-exception.2.il
  ...
  if (!m_class_get_methods(klass) || mono_class_has_failure(klass)) {
    ...
    mono_error_set_type_load_class(
        error, klass, "Could not find method '%s' due to a type load error: %s",
        name, mono_error_get_message(cause_error));
...
static MonoMethod *find_method(MonoClass *in_class, MonoClass *ic,
                               const char *name, MonoMethodSignature *sig,
                               MonoClass *from_class, MonoError *error) {
  int i;
  ...
  if (ic) {
    class_name = mono_type_get_name_full(m_class_get_byval_arg(ic),
    ...
    if (ic_name_space && ic_name_space[0])
      fqname = g_strconcat(ic_name_space, ".", class_name, ".", name,
                           (const char *)NULL);
    else
      fqname = NULL;
  } else
  ...
  while (in_class) {
    g_assert(from_class);
    result = find_method_in_class(in_class, name, qname, fqname, sig,
                                  from_class, error);
    ...
    /*
     * This happens when we fail to lazily load the interfaces of one of the
     * types. On such case we can't just bail out since user code depends on us
     * trying harder.
     */
    if (m_class_get_interface_offsets_count(from_class) !=
    ...
    int in_class_interface_offsets_count =
        m_class_get_interface_offsets_count(in_class);
    MonoClass **in_class_interfaces_packed =
        m_class_get_interfaces_packed(in_class);
    MonoClass **from_class_interfaces_packed =
    ...
    for (i = 0; i < in_class_interface_offsets_count; i++) {
      ...
      ic_class_name = mono_type_get_name_full(m_class_get_byval_arg(in_ic),
                                              MONO_TYPE_NAME_FORMAT_IL);
      ...
      if (in_ic_name_space && in_ic_name_space[0])
        ic_fqname = g_strconcat(in_ic_name_space, ".", ic_class_name, ".", name,
                                (const char *)NULL);
      else
        ic_fqname = NULL;

      result = find_method_in_class(in_ic, ic ? name : NULL, ic_qname,
                                    ic_fqname, sig, from_ic, error);
  ...
  if (is_interface)
    result =
        find_method_in_class(mono_defaults.object_class, name, qname, fqname,
                             sig, mono_defaults.object_class, error);
...
inflate_generic_signature_checked(MonoImage *image, MonoMethodSignature *sig,
                                  MonoGenericContext *context,
                                  MonoError *error) {
  ...
  res = (MonoMethodSignature *)g_malloc0(MONO_SIZEOF_METHOD_SIGNATURE +
                                         ((gint32)sig->param_count) *
                                             sizeof(MonoType *));
  ...
  for (i = 0; i < sig->param_count; ++i) {
    res->params[i] =
        mono_class_inflate_generic_type_checked(sig->params[i], context, error);
    if (!is_ok(error))
      goto fail;
 ...
 *
 * Inflate \p sig with \p context, and return a canonical copy. On error, set \p
 * error, and return NULL.
 */
MonoMethodSignature *mono_inflate_generic_signature(MonoMethodSignature *sig,
                                                    MonoGenericContext *context,
                                                    MonoError *error) {
...
static MonoMethodHeader *inflate_generic_header(MonoMethodHeader *header,
                                                MonoGenericContext *context,
                                                MonoError *error) {
  ...
  for (int i = 0; i < header->num_locals; ++i) {
    res->locals[i] = mono_class_inflate_generic_type_checked(header->locals[i],
                                                             context, error);
  ...
  if (res->num_clauses) {
    for (int i = 0; i < header->num_clauses; ++i) {
      ...
      if (clause->flags != MONO_EXCEPTION_CLAUSE_NONE)
        continue;
      clause->data.catch_class = mono_class_inflate_generic_class_checked(
          clause->data.catch_class, context, error);
      goto_if_nok(error, fail);
    }
  }
  return res;
...
mono_method_get_signature_full(MonoMethod *method, MonoImage *image,
                               guint32 token, MonoGenericContext *context) {
  ERROR_DECL(error);
  MonoMethodSignature *res =
      mono_method_get_signature_checked(method, image, token, context, error);
  mono_error_cleanup(error);
  return res;
}

MonoMethodSignature *
mono_method_get_signature_checked(MonoMethod *method, MonoImage *image,
                                  guint32 token, MonoGenericContext *context,
                                  MonoError *error) {
  ...
  if (image_is_dynamic(image)) {
    ...
    mono_metadata_decode_row(&image->tables[MONO_TABLE_MEMBERREF], idx - 1,
                             cols, MONO_MEMBERREF_SIZE);
    sig_idx = cols[MONO_MEMBERREF_SIGNATURE];

    sig = (MonoMethodSignature *)find_cached_memberref_sig(image, sig_idx);
    if (!sig) {
      if (!mono_verifier_verify_memberref_method_signature(image, sig_idx,
                                                           error))
        return NULL;
      ...
      sig = mono_metadata_parse_method_signature_full(image, NULL, 0, ptr, NULL,
                                                      error);
      if (!sig)
        return NULL;
    ...
    /* FIXME: we probably should verify signature compat in the dynamic case
     * too*/
    if (!mono_verifier_is_sig_compatible(image, method, sig)) {
      ...
      mono_error_set_bad_image(error, image,
                               "Incompatible method signature class token "
                               "0x%08x field name %s token 0x%08x",
                               klass, fname, token);
      return NULL;
    }
  }
  ...
  if (context) {
    ...
    if (cached != sig)
      ...
      mono_atomic_fetch_add_i32(&inflated_signatures_size,
...
mono_method_get_signature(MonoMethod *method, MonoImage *image, guint32 token) {
  ERROR_DECL(error);
  MonoMethodSignature *res =
      mono_method_get_signature_checked(method, image, token, NULL, error);
  mono_error_cleanup(error);
  return res;
}

/* this is only for the typespec array methods */
MonoMethod *mono_method_search_in_array_class(MonoClass *klass,
                                              const char *name,
                                              MonoMethodSignature *sig) {
  int i;
  ...
  g_assert(
      !mono_class_has_failure(klass)); /*FIXME this should not fail, right?*/
  int mcount = mono_class_get_method_count(klass);
  MonoMethod **klass_methods = m_class_get_methods(klass);
  for (i = 0; i < mcount; ++i) {
    MonoMethod *method = klass_methods[i];
    if (strcmp(method->name, name) == 0 &&
        sig->param_count == method->signature->param_count)
      return method;
  }
  return NULL;
}

static MonoMethod *method_from_memberref(MonoImage *image, guint32 idx,
   ...
   * This is used to tell our caller whether or not it's safe to insert the
   * returned method into a cache.
   */
  if (used_context)
  ...
  case MONO_MEMBERREF_PARENT_TYPESPEC:
     ...
     */
    klass = mono_class_get_and_inflate_typespec_checked(
        image, MONO_TOKEN_TYPE_SPEC | nindex, typespec_context, error);
    if (!klass)
      goto fail;
    break;
  case MONO_MEMBERREF_PARENT_TYPEDEF:
  ...
  case MONO_MEMBERREF_PARENT_METHODDEF: {
    method = mono_get_method_checked(image, MONO_TOKEN_METHOD_DEF | nindex,
                                     NULL, NULL, error);
    if (!method)
      goto fail;
    return method;
  ...
  case MONO_MEMBERREF_PARENT_TYPESPEC: {
    ...
    /* we're an array and we created these methods already in klass in
  ...
  if (!method && is_ok(error))
    mono_error_set_method_missing(error, klass, mname, sig,
...
static MonoMethod *method_from_methodspec(MonoImage *image,
                                          MonoGenericContext *context,
  ...
  mono_metadata_decode_row(&tables[MONO_TABLE_METHODSPEC], idx - 1, cols,
                           MONO_METHODSPEC_SIZE);
  token = cols[MONO_METHODSPEC_METHOD];
  nindex = token >> MONO_METHODDEFORREF_BITS;

  if (!mono_verifier_verify_methodspec_signature(
          image, cols[MONO_METHODSPEC_SIGNATURE], error))
    return NULL;
  ...
  new_context.class_inst =
      mono_class_is_ginst(klass)
          ? mono_class_get_generic_class(klass)->context.class_inst
          : NULL;
  new_context.method_inst = inst;

  method = mono_class_inflate_generic_method_full_checked(method, klass,
                                                          &new_context, error);
  return method;
}
 ...
 */
static MonoMethod *mono_get_method_from_token(MonoImage *image, guint32 token,
                                              MonoClass *klass,
                                              MonoGenericContext *context,
                                              gboolean *used_context,
                                              MonoError *error) {
  ...
  if (image_is_dynamic(image)) {
    ...
    result = (MonoMethod *)mono_lookup_dynamic_token_class(
        image, token, TRUE, &handle_class, context, error);
    return_val_if_nok(error, NULL);

    // This checks the memberref type as well
    if (result && handle_class != mono_defaults.methodhandle_class) {
      mono_error_set_bad_image(
          error, image, "Bad method token 0x%08x on dynamic image", token);
      return NULL;
    }
    return result;
  }
  ...
  mono_metadata_decode_row(&image->tables[MONO_TABLE_METHOD], idx - 1, cols,
                           MONO_METHOD_SIZE);
  ...
  if (cols[MONO_METHOD_IMPLFLAGS] & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) {
    if (result->klass == mono_defaults.string_class &&
        !strcmp(result->name, ".ctor"))
    ...
    if (m_image_is_module_handle(image) &&
        (cols[MONO_METHOD_IMPLFLAGS] & METHOD_IMPL_ATTRIBUTE_NATIVE)) {
    ...
    if (piinfo->implmap_idx)
      piinfo->piflags = mono_metadata_decode_row_col(
          &tables[MONO_TABLE_IMPLMAP], piinfo->implmap_idx - 1,
          MONO_IMPLMAP_FLAGS);
  }
...
MonoMethod *mono_get_method(MonoImage *image, guint32 token, MonoClass *klass) {
  ERROR_DECL(error);
  MonoMethod *result =
      mono_get_method_checked(image, token, klass, NULL, error);
  mono_error_cleanup(error);
  return result;
}
 ...
 */
MonoMethod *mono_get_method_full(MonoImage *image, guint32 token,
                                 MonoClass *klass,
                                 MonoGenericContext *context) {
  ERROR_DECL(error);
  MonoMethod *result =
      mono_get_method_checked(image, token, klass, context, error);
  mono_error_cleanup(error);
  return result;
}

MonoMethod *mono_get_method_checked(MonoImage *image, guint32 token,
                                    MonoClass *klass,
                                    MonoGenericContext *context,
                                    MonoError *error) {
  MonoMethod *result = NULL;
  ...
  if (mono_metadata_token_table(token) == MONO_TABLE_METHOD) {
    ...
    result = (MonoMethod *)g_hash_table_lookup(
        image->method_cache, GINT_TO_POINTER(mono_metadata_token_index(token)));
  } else if (!image_is_dynamic(image)) {
    if (!image->methodref_cache)
      image->methodref_cache = g_hash_table_new(NULL, NULL);
    result = (MonoMethod *)g_hash_table_lookup(image->methodref_cache,
  ...
  if (!used_context && !result->is_inflated) {
    ...
    if (mono_metadata_token_table(token) == MONO_TABLE_METHOD)
      result2 = (MonoMethod *)g_hash_table_lookup(
          image->method_cache,
          GINT_TO_POINTER(mono_metadata_token_index(token)));
    else if (!image_is_dynamic(image))
      result2 = (MonoMethod *)g_hash_table_lookup(image->methodref_cache,
                                                  GINT_TO_POINTER(token));
    ...
    if (mono_metadata_token_table(token) == MONO_TABLE_METHOD)
      g_hash_table_insert(image->method_cache,
                          GINT_TO_POINTER(mono_metadata_token_index(token)),
                          result);
    else if (!image_is_dynamic(image))
      g_hash_table_insert(image->methodref_cache, GINT_TO_POINTER(token),
                          result);
  }
...
static MonoMethod *get_method_constrained(MonoImage *image, MonoMethod *method,
  ...
  if (!mono_class_is_assignable_from_internal(base_class, constrained_class)) {
    ...
    mono_error_set_invalid_operation(
        error, "constrained call: %s is not assignable from %s",
        base_class_name, constrained_class_name);
  ...
  if (method->is_inflated) {
     ...
     * we will get at the end a refined HSubclass<...>.M<U> and we will need to
     * re-instantiate it with D. to get HSubclass<...>.M<D>
     *
     */
  ...
  if (!MONO_CLASS_IS_INTERFACE_INTERNAL(base_class)) {
    ...
    int itf_base = mono_class_interface_offset_with_variance(
        constrained_class, base_class, &variant);
  ...
  if (inflated_generic_method) {
    ...
    res = mono_class_inflate_generic_method_checked(res, &inflated_method_ctx,
                                                    error);
    return_val_if_nok(error, NULL);
  }
  return res;
...
MonoMethod *mono_get_method_constrained_with_method(
    MonoImage *image, MonoMethod *method, MonoClass *constrained_class,
 ...
 */
MonoMethod *mono_get_method_constrained(MonoImage *image, guint32 token,
                                        MonoClass *constrained_class,
                                        MonoGenericContext *context,
                                        MonoMethod **cil_method) {
  ERROR_DECL(error);
  MonoMethod *result = mono_get_method_constrained_checked(
      image, token, constrained_class, context, cil_method, error);
  mono_error_cleanup(error);
  return result;
}

MonoMethod *mono_get_method_constrained_checked(MonoImage *image, guint32 token,
                                                MonoClass *constrained_class,
                                                MonoGenericContext *context,
                                                MonoMethod **cil_method,
                                                MonoError *error) {
  ...
  *cil_method = mono_get_method_checked(image, token, NULL, context, error);
  ...
  return get_method_constrained(image, *cil_method, constrained_class, context,
                                error);
}
...
void mono_free_method(MonoMethod *method) {
  ...
  /* FIXME: This hack will go away when the profiler will support freeing
   * methods */
...
void mono_method_get_param_names(MonoMethod *method, const char **names) {
  ...
  /*FIXME this check is somewhat redundant since the caller usally will have to
    get the signature to figure out the number of arguments and allocate a
  ...
  if (method->wrapper_type) {
    ...
    if (klass_image->wrapper_param_names)
      pnames = (char **)g_hash_table_lookup(klass_image->wrapper_param_names,
                                            method);
    ...
    for (i = param_index; i < lastp; ++i) {
      ...
      if (cols[MONO_PARAM_SEQUENCE] &&
          cols[MONO_PARAM_SEQUENCE] <=
              signature
                  ->param_count) /* skip return param spec and bounds check*/
        names[cols[MONO_PARAM_SEQUENCE] - 1] =
            mono_metadata_string_heap(klass_image, cols[MONO_PARAM_NAME]);
    }
  }
}
...
void mono_method_get_marshal_info(MonoMethod *method,
  ...
  signature = mono_method_signature_internal(method);
  g_assert(
      signature); /*FIXME there is no way to signal error from this function*/
      ...
      for (i = 0; i < signature->param_count + 1; ++i) {
        if (dyn_specs[i]) {
          ...
          if (mspecs[i]->native == MONO_NATIVE_CUSTOM) {
            mspecs[i]->data.custom_data.custom_name =
                g_strdup(dyn_specs[i]->data.custom_data.custom_name);
            mspecs[i]->data.custom_data.cookie =
                g_strdup(dyn_specs[i]->data.custom_data.cookie);
          }
        }
      }
    }
  ...
  if (idx > 0) {
    ...
    for (i = param_index; i < lastp; ++i) {
      ...
      if (cols[MONO_PARAM_FLAGS] & PARAM_ATTRIBUTE_HAS_FIELD_MARSHAL &&
          cols[MONO_PARAM_SEQUENCE] <= signature->param_count) {
        const char *tp;
        tp = mono_metadata_get_marshal_info(klass_image, i - 1, FALSE);
        g_assert(tp);
        mspecs[cols[MONO_PARAM_SEQUENCE]] =
            mono_metadata_parse_marshal_spec(klass_image, tp);
      }
    }
...
gboolean mono_method_has_marshal_info(MonoMethod *method) {
  ...
  if (image_is_dynamic(m_class_get_image(method->klass))) {
    ...
    MonoMarshalSpec **dyn_specs = method_aux->param_marshall;
    if (dyn_specs) {
      for (i = 0; i < mono_method_signature_internal(method)->param_count + 1;
           ++i)
        if (dyn_specs[i])
          return TRUE;
    }
...
static gboolean stack_walk_adapter(MonoStackFrameInfo *frame, MonoContext *ctx,
                                   gpointer data) {
  ...
  case FRAME_TYPE_INTERP:
    g_assert(frame->ji);
    return d->func(frame->actual_method, frame->native_offset, frame->il_offset,
                   frame->managed, d->user_data);
    break;
  default:
...
void mono_stack_walk(MonoStackWalk func, gpointer user_data) {
  ...
  mono_get_eh_callbacks()->mono_walk_stack_with_ctx(
      stack_walk_adapter, NULL, MONO_UNWIND_LOOKUP_ALL, &ud);
}
...
static gboolean async_stack_walk_adapter(MonoStackFrameInfo *frame,
  ...
  case FRAME_TYPE_INTERP:
    ...
    return d->func(method, frame->domain, frame->ji->code_start,
                   frame->native_offset, d->user_data);
  default:
    g_assert_not_reached();
    return FALSE;
  }
...
void mono_stack_walk_async_safe(MonoStackWalkAsyncSafe func,
  ...
  mono_get_eh_callbacks()->mono_walk_stack_with_ctx(
      async_stack_walk_adapter, &ctx, MONO_UNWIND_SIGNAL_SAFE, &ud);
}

static gboolean last_managed(MonoMethod *m, gint no, gint ilo, gboolean managed,
                             gpointer data) {
 ...
 *
 * Return the signature of the method M. On failure, returns NULL, and ERR is
 * set. Call mono_method_signature_checked instead.
 */
MonoMethodSignature *mono_method_signature_checked_slow(MonoMethod *m,
                                                        MonoError *error) {
  int idx;
  MonoImage *img;
  const char *sig;
  ...
  if (m->is_inflated) {
    ...
    signature = inflate_generic_signature_checked(
        m_class_get_image(imethod->declaring->klass), signature,
        mono_method_get_context(m), error);
    if (!is_ok(error))
      return NULL;
  ...
  sig = mono_metadata_blob_heap(img, sig_offset = mono_metadata_decode_row_col(
                                         &img->tables[MONO_TABLE_METHOD],
                                         idx - 1, MONO_METHOD_SIGNATURE));
  ...
  can_cache_signature = !(m->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) &&
  ...
  if (can_cache_signature) {
    mono_image_lock(img);
    signature =
        (MonoMethodSignature *)g_hash_table_lookup(img->method_signatures, sig);
    mono_image_unlock(img);
  }
  ...
  if (!signature) {
    ...
    signature = mono_metadata_parse_method_signature_full(
        img, container, idx, sig_body, NULL, error);
    if (!signature)
      return NULL;

    if (can_cache_signature) {
      mono_image_lock(img);
      sig2 = (MonoMethodSignature *)g_hash_table_lookup(img->method_signatures,
                                                        sig);
      if (!sig2)
    ...
    mono_atomic_fetch_add_i32(&signatures_size,
                              mono_metadata_signature_size(signature));
  }
  ...
  if (signature->generic_param_count) {
    if (!container || !container->is_method) {
      mono_error_set_method_missing(
          error, m->klass, m->name, signature,
          "Signature claims method has generic parameters, but generic_params "
          "table says it doesn't for method 0x%08x from image %s",
          idx, img->name);
      return NULL;
    }
    if (container->type_argc != signature->generic_param_count) {
      mono_error_set_method_missing(
          error, m->klass, m->name, signature,
          "Inconsistent generic parameter count.  Signature says %d, "
          "generic_params table says %d for method 0x%08x from image %s",
    ...
    mono_error_set_method_missing(
        error, m->klass, m->name, signature,
        "generic_params table claims method has generic parameters, but "
        "signature says it doesn't for method 0x%08x from image %s",
        idx, img->name);
    return NULL;
  }
  if (m->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) {
    ...
    default: {
      mono_error_set_method_missing(error, m->klass, m->name, signature,
                                    "Unsupported calling convention : 0x%04x "
                                    "for method 0x%08x from image %s",
                                    piinfo->piflags, idx, img->name);
    }
      return NULL;
    }
 ...
 */
MonoMethodSignature *mono_method_signature_internal_slow(MonoMethod *m) {
  ERROR_DECL(error);
  MonoMethodSignature *sig = mono_method_signature_checked(m, error);
  if (sig)
    return sig;
  char *type_name = mono_type_get_full_name(m->klass);
  g_warning("Could not load signature of %s:%s due to: %s", type_name, m->name,
 ...
 */
const char *mono_method_get_name(MonoMethod *method) { return method->name; }

/**
 * mono_method_get_class:
 */
MonoClass *mono_method_get_class(MonoMethod *method) { return method->klass; }

/**
 * mono_method_get_token:
 */
guint32 mono_method_get_token(MonoMethod *method) { return method->token; }
...
MonoMethodHeader *mono_method_get_header_internal(MonoMethod *method,
                                                  MonoError *error) {
  int idx;
  guint32 rva;
  MonoImage *img;
  ...
  // FIXME: for internal callers maybe it makes sense to do this check at the
  // call site, not here?
  if (mono_method_has_no_body(method)) {
    if (method->is_reabstracted == 1)
      mono_error_set_generic_error(
          error, "System", "EntryPointNotFoundException", "%s", method->name);
    else
  ...
  if (method->is_inflated) {
    ...
    iheader =
        inflate_generic_header(header, mono_method_get_context(method), error);
  ...
  /*
   * When parsing the types of local variables, we must pass any container
   * available to ensure that both VAR and MVAR will get the right owner.
   */
...
MonoMethodHeader *mono_method_get_header_checked(MonoMethod *method,
                                                 MonoError *error)
  ...
  for (i = 0; i < mcount; ++i) {
    if (method == klass_methods[i]) {
      if (m_class_get_image(klass)->uncompressed_metadata)
        return mono_metadata_translate_token_index(
            m_class_get_image(klass), MONO_TABLE_METHOD, first_idx + i + 1);
      else
