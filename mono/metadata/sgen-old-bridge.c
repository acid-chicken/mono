...
typedef struct _HashEntry {
  GCObject
      *obj; /* This is a duplicate - it's already stored in the hash table */
...
static void dyn_array_init(DynArray *da) {
  da->size = 0;
  da->capacity = 0;
  da->data = NULL;
}

static void dyn_array_uninit(DynArray *da, int elem_size) {
  if (da->capacity <= 0)
    return;

  sgen_free_internal_dynamic(da->data, elem_size * da->capacity,
                             INTERNAL_MEM_BRIDGE_DATA);
  da->data = NULL;
}

static void dyn_array_ensure_capacity(DynArray *da, int capacity,
                                      int elem_size) {
  ...
  new_data = (char *)sgen_alloc_internal_dynamic(
      elem_size * da->capacity, INTERNAL_MEM_BRIDGE_DATA, TRUE);
  memcpy(new_data, da->data, elem_size * da->size);
  sgen_free_internal_dynamic(da->data, elem_size * old_capacity,
                             INTERNAL_MEM_BRIDGE_DATA);
  da->data = new_data;
}

static void *dyn_array_add(DynArray *da, int elem_size) {
  void *p;
...
/* int */
static void dyn_array_int_init(DynIntArray *da) { dyn_array_init(&da->array); }

static void dyn_array_int_uninit(DynIntArray *da) {
  dyn_array_uninit(&da->array, sizeof(int));
}

static int dyn_array_int_size(DynIntArray *da) { return da->array.size; }

static void dyn_array_int_set_size(DynIntArray *da, int size) {
  da->array.size = size;
}

static void dyn_array_int_add(DynIntArray *da, int x) {
  int *p = (int *)dyn_array_add(&da->array, sizeof(int));
  *p = x;
}

static int dyn_array_int_get(DynIntArray *da, int x) {
  return ((int *)da->array.data)[x];
}

static void dyn_array_int_set(DynIntArray *da, int idx, int val) {
  ((int *)da->array.data)[idx] = val;
}

static void dyn_array_int_ensure_capacity(DynIntArray *da, int capacity) {
  dyn_array_ensure_capacity(&da->array, capacity, sizeof(int));
}

static void dyn_array_int_set_all(DynIntArray *dst, DynIntArray *src) {
...
static void dyn_array_ptr_uninit(DynPtrArray *da) {
  dyn_array_uninit(&da->array, sizeof(void *));
}

static int dyn_array_ptr_size(DynPtrArray *da) { return da->array.size; }

static void dyn_array_ptr_set_size(DynPtrArray *da, int size) {
  da->array.size = size;
}

static void *dyn_array_ptr_get(DynPtrArray *da, int x) {
  return ((void **)da->array.data)[x];
}

static void dyn_array_ptr_add(DynPtrArray *da, void *ptr) {
...
static void *dyn_array_ptr_pop(DynPtrArray *da) {
  void *p;
...
static void dyn_array_int_merge(DynIntArray *dst, DynIntArray *src) {
  int i, j;

  dyn_array_int_ensure_capacity(&merge_array, dyn_array_int_size(dst) +
...
static void dyn_array_int_merge_one(DynIntArray *array, int value) {
  int i;
  int tmp;
...
static HashEntry *get_hash_entry(GCObject *obj, gboolean *existing) {
...
static HashEntry *register_bridge_object(GCObject *obj) {
  HashEntry *entry = get_hash_entry(obj, NULL);
  entry->is_bridge = TRUE;
  return entry;
}

static void register_finishing_time(HashEntry *entry, int t) {
  g_assert(entry->finishing_time < 0);
  entry->finishing_time = t;
}

static gboolean object_is_live(GCObject **objp) {
  GCObject *obj = *objp;
  ...
  do {
    ...
    if (entry->scc_index >= 0) {
      if (entry->scc_index != current_scc->index)
        scc_add_xref(dyn_array_scc_get_ptr(&sccs, entry->scc_index),
                     current_scc);
      continue;
    }
...
static void reset_data(void) { dyn_array_ptr_set_size(&registered_bridges, 0); }
...
static void processing_stw_step(void) {
  ...
  */
  ...
  for (i = 0; i < bridge_count; ++i)
    dfs1(get_hash_entry((GCObject *)dyn_array_ptr_get(&registered_bridges, i),
                        NULL));
...
static void processing_after_callback(int generation) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_GC,
             "GC_OLD_BRIDGE num-objects %d num_hash_entries %d sccs size %d "
             "init %.2fms df1 %.2fms sort %.2fms dfs2 %.2fms setup-cb %.2fms "
             "free-data %.2fms links %d/%d/%d/%d dfs passes %d/%d",
