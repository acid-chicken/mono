...
#define SET_APPCONTEXT(x)                                                      \
  MONO_OBJECT_SETREF_INTERNAL(mono_thread_internal_current(),                  \
                              current_appcontext, (x))
...
static const MonoRuntimeInfo supported_runtimes[] = {
    ...
    {"mobile",
     "2.1",
     {{2, 0, 5, 0}, {10, 0, 0, 0}, {2, 0, 5, 0}, {2, 0, 5, 0}, {4, 0, 0, 0}}},
    {"moonlight",
     "2.1",
     {{2, 0, 5, 0}, {9, 0, 0, 0}, {3, 5, 0, 0}, {3, 0, 0, 0}, NOT_AVAIL}},
};
...
static GSList *get_runtimes_from_exe(const char *exe_file,
                                     MonoImage **exe_image);

static const MonoRuntimeInfo *get_runtime_by_version(const char *version);

static void mono_domain_alcs_destroy(MonoDomain *domain);

#ifdef ENABLE_NETCORE
static void mono_domain_alcs_lock(MonoDomain *domain);

static void mono_domain_alcs_unlock(MonoDomain *domain);

static void mono_domain_create_default_alc(MonoDomain *domain);
#endif

static LockFreeMempool *lock_free_mempool_new(void) {
  return g_new0(LockFreeMempool, 1);
}

static void lock_free_mempool_free(LockFreeMempool *mp) {
 ...
 * This is async safe
 */
static LockFreeMempoolChunk *lock_free_mempool_chunk_new(LockFreeMempool *mp,
                                                         int len) {
  ...
  while (size - sizeof(LockFreeMempoolChunk) < len)
    size += mono_pagesize();
  chunk = (LockFreeMempoolChunk *)mono_valloc(
      0, size, MONO_MMAP_READ | MONO_MMAP_WRITE, MONO_MEM_ACCOUNT_DOMAIN);
  g_assert(chunk);
  chunk->mem =
      (guint8 *)ALIGN_PTR_TO((char *)chunk + sizeof(LockFreeMempoolChunk), 16);
  ...
  while (TRUE) {
    prev = mp->chunks;
    if (mono_atomic_cas_ptr((volatile gpointer *)&mp->chunks, chunk, prev) ==
        prev)
      break;
  }
  chunk->prev = prev;
 ...
 * This is async safe
 */
static gpointer lock_free_mempool_alloc0(LockFreeMempool *mp, guint size) {
...
gboolean mono_string_equal_internal(MonoString *s1, MonoString *s2) {
  ...
  return memcmp(mono_string_chars_internal(s1), mono_string_chars_internal(s2),
                l1 * 2) == 0;
}
...
guint mono_string_hash_internal(MonoString *s) {
...
static gboolean mono_ptrarray_equal(gpointer *s1, gpointer *s2) {
...
static guint mono_ptrarray_hash(gpointer *s) {
  int i;
...
static void *gc_alloc_fixed_non_heap_list(size_t size) {
  if (mono_gc_is_moving())
    return g_malloc0(size);
  else
    return mono_gc_alloc_fixed(size, MONO_GC_DESCRIPTOR_NULL,
                               MONO_ROOT_SOURCE_DOMAIN, NULL, "Domain List");
}

static void gc_free_fixed_non_heap_list(void *ptr) {
  if (mono_gc_is_moving())
    g_free(ptr);
  else
 ...
 */
static int domain_id_alloc(MonoDomain *domain) {
  int id = -1, i;
  if (!appdomains_list) {
    appdomain_list_size = 2;
    appdomains_list = (MonoDomain **)gc_alloc_fixed_non_heap_list(
        appdomain_list_size * sizeof(void *));
  }
  ...
  if (id == -1) {
    ...
    new_list =
        (MonoDomain **)gc_alloc_fixed_non_heap_list(new_size * sizeof(void *));
...
MonoDomain *mono_domain_create(void) {
  ...
  if (!domain_gc_desc) {
    ...
    for (i = G_STRUCT_OFFSET(MonoDomain, MONO_DOMAIN_FIRST_OBJECT);
         i < G_STRUCT_OFFSET(MonoDomain, MONO_DOMAIN_FIRST_GC_TRACKED);
         i += sizeof(gpointer)) {
      bit = i / sizeof(gpointer);
      domain_gc_bitmap[bit / 32] |= (gsize)1 << (bit % 32);
    }
    domain_gc_desc =
        mono_gc_make_descr_from_bitmap((gsize *)domain_gc_bitmap, bit + 1);
  }
  mono_appdomains_unlock();

  if (!mono_gc_is_moving())
    domain = (MonoDomain *)mono_gc_alloc_fixed(
        sizeof(MonoDomain), MONO_GC_DESCRIPTOR_NULL, MONO_ROOT_SOURCE_DOMAIN,
        NULL, "Domain Structure");
  else
    domain = (MonoDomain *)mono_gc_alloc_fixed(
        sizeof(MonoDomain), domain_gc_desc, MONO_ROOT_SOURCE_DOMAIN, NULL,
        "Domain Structure");
  ...
  domain->env = mono_g_hash_table_new_type_internal(
      (GHashFunc)mono_string_hash_internal,
      (GCompareFunc)mono_string_equal_internal, MONO_HASH_KEY_VALUE_GC,
      MONO_ROOT_SOURCE_DOMAIN, domain, "Domain Environment Variable Table");
  domain->domain_assemblies = NULL;
  domain->assembly_bindings = NULL;
  domain->assembly_bindings_parsed = FALSE;
  domain->class_vtable_array = g_ptr_array_new();
  domain->proxy_vtable_hash = g_hash_table_new(
      (GHashFunc)mono_ptrarray_hash, (GCompareFunc)mono_ptrarray_equal);
  mono_jit_code_hash_init(&domain->jit_code_hash);
  domain->ldstr_table = mono_g_hash_table_new_type_internal(
      (GHashFunc)mono_string_hash_internal,
      (GCompareFunc)mono_string_equal_internal, MONO_HASH_KEY_VALUE_GC,
      MONO_ROOT_SOURCE_DOMAIN, domain, "Domain String Pool Table");
  domain->num_jit_info_table_duplicates = 0;
  domain->jit_info_table = mono_jit_info_table_new(domain);
  domain->jit_info_free_queue = NULL;
  domain->finalizable_objects_hash =
 ...
 */
static MonoDomain *mono_init_internal(const char *filename,
  ...
  mono_counters_register("Max code space allocated in a domain",
                         MONO_COUNTER_INT | MONO_COUNTER_JIT,
                         &max_domain_code_alloc);
  mono_counters_register("Total code space allocated",
                         MONO_COUNTER_INT | MONO_COUNTER_JIT,
                         &total_domain_code_alloc);

  mono_counters_register("Max HashTable Chain Length",
                         MONO_COUNTER_INT | MONO_COUNTER_METADATA,
  ...
  if (exe_filename != NULL) {
    /*
     * This function will load the exe file as a MonoImage. We need to close it,
     * but that would mean it would be reloaded later. So instead, we save it to
    ...
    if (!exe_image) {
      exe_image = mono_assembly_open_from_bundle(
          mono_domain_default_alc(domain), exe_filename, NULL, FALSE);
      if (!exe_image)
  ...
  if (runtimes == NULL) {
    const MonoRuntimeInfo *default_runtime =
    ...
    if (runtime_version != NULL)
      ...
      g_print("WARNING: The runtime version supported by this application is "
              "unavailable.\n");
    g_print("Using default runtime: %s\n", default_runtime->runtime_version);
  }

  /* The selected runtime will be the first one for which there is a
   * mscrolib.dll */
  GSList *tmp = runtimes;
  while (tmp != NULL) {
  ...
  if ((status != MONO_IMAGE_OK) || (ass == NULL)) {
    ...
    case MONO_IMAGE_ERROR_ERRNO: {
      char *corlib_file = g_build_filename(mono_assembly_getrootdir(), "mono",
                                           current_runtime->framework_version,
  ...
  mono_defaults.object_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Object");

  mono_defaults.void_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Void");

  mono_defaults.boolean_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Boolean");

  mono_defaults.byte_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Byte");

  mono_defaults.sbyte_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "SByte");

  mono_defaults.int16_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Int16");

  mono_defaults.uint16_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "UInt16");

  mono_defaults.int32_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Int32");

  mono_defaults.uint32_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "UInt32");

  mono_defaults.uint_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "UIntPtr");

  mono_defaults.int_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "IntPtr");

  mono_defaults.int64_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Int64");

  mono_defaults.uint64_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "UInt64");

  mono_defaults.single_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Single");

  mono_defaults.double_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Double");

  mono_defaults.char_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Char");

  mono_defaults.string_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "String");

  mono_defaults.enum_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Enum");

  mono_defaults.array_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Array");

  mono_defaults.delegate_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Delegate");
  ...
  mono_defaults.systemtype_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Type");

  mono_defaults.runtimetype_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "RuntimeType");

  mono_defaults.exception_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Exception");
  ...
  mono_defaults.transparent_proxy_class = mono_class_load_from_name(
      mono_defaults.corlib, "System.Runtime.Remoting.Proxies",
      "TransparentProxy");
  ...
  /*
  Not using GENERATE_TRY_GET_CLASS_WITH_CACHE_DECL as this type is heavily
  checked by sgen when computing finalization.
  */
  mono_defaults.critical_finalizer_object = mono_class_try_load_from_name(
      mono_defaults.corlib, "System.Runtime.ConstrainedExecution",
      "CriticalFinalizerObject");

  mono_assembly_load_friends(ass);

  mono_defaults.attribute_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Attribute");

  mono_class_init_internal(mono_defaults.array_class);
  mono_defaults.generic_nullable_class =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Nullable`1");
 ...
 *
 * Returns: the root appdomain, to obtain the current domain, use
 * mono_domain_get ()
 */
 ...
 */
MonoDomain *mono_domain_get() { return GET_APPDOMAIN(); }

void mono_domain_unset(void) { SET_APPDOMAIN(NULL); }

void mono_domain_set_internal_with_options(MonoDomain *domain,
  ...
  if (migrate_exception) {
    ...
    MONO_OBJECT_SETREF_INTERNAL(thread, abort_exc,
 ...
 * \param user_data user-defined pointer that is passed to the supplied \p func
 * fo reach domain
 *
...
void mono_domain_foreach(MonoDomainFunc func, gpointer user_data) {
  ...
  copy = (MonoDomain **)gc_alloc_fixed_non_heap_list(appdomain_list_size *
                                                     sizeof(void *));
...
void mono_domain_ensure_entry_assembly(MonoDomain *domain,
...
MonoAssembly *mono_domain_assembly_open(MonoDomain *domain, const char *name) {
  MonoAssembly *result;
  MONO_ENTER_GC_UNSAFE;
  result = mono_domain_assembly_open_internal(
      domain, mono_domain_default_alc(domain), name);
  MONO_EXIT_GC_UNSAFE;
  return result;
}
...
MonoAssembly *mono_domain_assembly_open_internal(MonoDomain *domain,
  ...
  // On netcore, this is necessary because we check the AppContext.BaseDirectory
  // property as part of the assembly lookup algorithm AppContext.BaseDirectory
  // can sometimes fall back to checking the location of the entry_assembly,
...
static void unregister_vtable_reflection_type(MonoVTable *vtable) {
 ...
 * \param force if TRUE, it allows the root domain to be released (used at
 * shutdown only).
 *
 ...
 * when necessary.
 *
 * In theory, this is dead code on netcore and thus does not need to be
 * ALC-aware.
 */
void mono_domain_free(MonoDomain *domain, gboolean force) {
  ...
  /* Have to zero out reference fields since they will be invalidated by the
   * clear_domain () call below */
  for (p = (gpointer *)&domain->MONO_DOMAIN_FIRST_OBJECT;
       p < (gpointer *)&domain->MONO_DOMAIN_FIRST_GC_TRACKED; ++p)
    *p = NULL;
  ...
  for (tmp = domain->domain_assemblies; tmp; tmp = tmp->next) {
    ...
    mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_ASSEMBLY,
               ...
               domain->friendly_name, domain, ass->aname.name, ass,
               ass->ref_count);
  ...
  /*
   * Send this after the assemblies have been unloaded and the domain is still
   * in a usable state.
   */
  ...
  if (debug_domain_unload) {
    ...
    mono_atomic_fetch_add_i32(&mono_perfcounters->loader_bytes,
                              -1 * mono_mempool_get_allocated(domain->mp));
#endif
 ...
 */
gint32 mono_domain_get_id(MonoDomain *domain) { return domain->domain_id; }
 ...
 * LOCKING: Acquires the domain lock.
 */
void *(mono_domain_code_reserve)(MonoDomain *domain, int size) {
  gpointer res;
 ...
 * LOCKING: Acquires the domain lock.
 */
void *(mono_domain_code_reserve_align)(MonoDomain *domain, int size,
                                       int alignment) {
  gpointer res;
 ...
 * LOCKING: Acquires the domain lock.
 */
void mono_domain_code_commit(MonoDomain *domain, void *data, int size,
                             int newsize) {
  mono_domain_lock(domain);
 ...
 *
 * The @func callback MUST not take any locks. If it really needs to, it must
 * respect the locking rules of the runtime:
 * http://www.mono-project.com/Mono:Runtime:Documentation:ThreadSafety LOCKING:
 * Acquires the domain lock.
 */

void mono_domain_code_foreach(MonoDomain *domain, MonoCodeManagerFunc func,
                              void *user_data) {
  mono_domain_lock(domain);
 ...
 */
MonoAppContext *mono_context_get(void) { return GET_APPCONTEXT(); }
 ...
 */
MonoImage *mono_get_corlib(void) { return mono_defaults.corlib; }

/**
 * mono_get_object_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Object . \returns The \c MonoClass* for the \c System.Object type.
 */
MonoClass *mono_get_object_class(void) { return mono_defaults.object_class; }

/**
 * mono_get_byte_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Byte . \returns The \c MonoClass* for the \c System.Byte type.
 */
MonoClass *mono_get_byte_class(void) { return mono_defaults.byte_class; }

/**
 * mono_get_void_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Void . \returns The \c MonoClass* for the \c System.Void type.
 */
MonoClass *mono_get_void_class(void) { return mono_defaults.void_class; }

/**
 * mono_get_boolean_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Boolean . \returns The \c MonoClass* for the \c System.Boolean type.
 */
MonoClass *mono_get_boolean_class(void) { return mono_defaults.boolean_class; }

/**
 * mono_get_sbyte_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.SByte. \returns The \c MonoClass* for the \c System.SByte type.
 */
MonoClass *mono_get_sbyte_class(void) { return mono_defaults.sbyte_class; }

/**
 * mono_get_int16_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Int16 . \returns The \c MonoClass* for the \c System.Int16 type.
 */
MonoClass *mono_get_int16_class(void) { return mono_defaults.int16_class; }

/**
 * mono_get_uint16_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.UInt16 . \returns The \c MonoClass* for the \c System.UInt16 type.
 */
MonoClass *mono_get_uint16_class(void) { return mono_defaults.uint16_class; }

/**
 * mono_get_int32_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Int32 . \returns The \c MonoClass* for the \c System.Int32 type.
 */
MonoClass *mono_get_int32_class(void) { return mono_defaults.int32_class; }

/**
 * mono_get_uint32_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.UInt32 . \returns The \c MonoClass* for the \c System.UInt32 type.
 */
MonoClass *mono_get_uint32_class(void) { return mono_defaults.uint32_class; }

/**
 * mono_get_intptr_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.IntPtr . \returns The \c MonoClass* for the \c System.IntPtr type.
 */
MonoClass *mono_get_intptr_class(void) { return mono_defaults.int_class; }

/**
 * mono_get_uintptr_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.UIntPtr . \returns The \c MonoClass* for the \c System.UIntPtr type.
 */
MonoClass *mono_get_uintptr_class(void) { return mono_defaults.uint_class; }

/**
 * mono_get_int64_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Int64 . \returns The \c MonoClass* for the \c System.Int64 type.
 */
MonoClass *mono_get_int64_class(void) { return mono_defaults.int64_class; }

/**
 * mono_get_uint64_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.UInt64 . \returns The \c MonoClass* for the \c System.UInt64 type.
 */
MonoClass *mono_get_uint64_class(void) { return mono_defaults.uint64_class; }

/**
 * mono_get_single_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Single  (32-bit floating points). \returns The \c MonoClass* for the
 * \c System.Single type.
 */
MonoClass *mono_get_single_class(void) { return mono_defaults.single_class; }

/**
 * mono_get_double_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Double  (64-bit floating points). \returns The \c MonoClass* for the
 * \c System.Double type.
 */
MonoClass *mono_get_double_class(void) { return mono_defaults.double_class; }

/**
 * mono_get_char_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Char . \returns The \c MonoClass* for the \c System.Char type.
 */
MonoClass *mono_get_char_class(void) { return mono_defaults.char_class; }

/**
 * mono_get_string_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.String . \returns The \c MonoClass* for the \c System.String type.
 */
MonoClass *mono_get_string_class(void) { return mono_defaults.string_class; }

/**
 * mono_get_enum_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Enum . \returns The \c MonoClass* for the \c System.Enum type.
 */
MonoClass *mono_get_enum_class(void) { return mono_defaults.enum_class; }

/**
 * mono_get_array_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Array . \returns The \c MonoClass* for the \c System.Array type.
 */
MonoClass *mono_get_array_class(void) { return mono_defaults.array_class; }

/**
 * mono_get_thread_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Threading.Thread . \returns The \c MonoClass* for the \c
 * System.Threading.Thread type.
 */
MonoClass *mono_get_thread_class(void) { return mono_defaults.thread_class; }

/**
 * mono_get_exception_class:
 * Use this function to get the \c MonoClass* that the runtime is using for \c
 * System.Exception . \returns The \c MonoClass* for the \c  type.
 */
...
static void start_element(GMarkupParseContext *context,
  ...
  if (strcmp(element_name, "requiredRuntime") == 0) {
    app_config->required_runtime =
        get_attribute_value(attribute_names, attribute_values, "version");
  } else if (strcmp(element_name, "supportedRuntime") == 0) {
    char *version =
        get_attribute_value(attribute_names, attribute_values, "version");
    app_config->supported_runtimes =
        g_slist_append(app_config->supported_runtimes, version);
  }
}
...
static AppConfigInfo *app_config_parse(const char *exe_filename) {
...
static void app_config_free(AppConfigInfo *app_config) {
  char *rt;
...
static GSList *get_runtimes_from_exe(const char *file, MonoImage **out_image) {
  AppConfigInfo *app_config;
  char *version;
  ...
  if (app_config != NULL) {
    ...
    if (app_config->required_runtime != NULL) {
      const MonoRuntimeInfo *runtime =
          get_runtime_by_version(app_config->required_runtime);
      if (runtime != NULL)
        runtimes = g_slist_prepend(runtimes, (gpointer)runtime);
      app_config_free(app_config);
      return runtimes;
  ...
  image = mono_assembly_open_from_bundle(
      mono_domain_default_alc(mono_domain_get()), file, NULL, FALSE);
 ...
 */
const MonoRuntimeInfo *mono_get_runtime_info(void) { return current_runtime; }
...
#ifdef ENABLE_NETCORE
static inline void mono_domain_alcs_lock(MonoDomain *domain) {
  mono_coop_mutex_lock(&domain->alcs_lock);
}

static inline void mono_domain_alcs_unlock(MonoDomain *domain) {
  mono_coop_mutex_unlock(&domain->alcs_lock);
}
#endif

#ifdef ENABLE_NETCORE
static MonoAssemblyLoadContext *create_alc(MonoDomain *domain,
                                           gboolean is_default) {
...
#ifdef ENABLE_NETCORE
void mono_domain_create_default_alc(MonoDomain *domain) {
...
mono_domain_create_individual_alc(MonoDomain *domain, uint32_t this_gchandle,
                                  gboolean collectible, MonoError *error) {
...
static void mono_alc_free(MonoAssemblyLoadContext *alc) {
  mono_alc_cleanup(alc);
  g_free(alc);
}
#endif

void mono_domain_alcs_destroy(MonoDomain *domain) {
#ifdef ENABLE_NETCORE
  mono_domain_alcs_lock(domain);
