...
static GENERATE_GET_CLASS_WITH_CACHE(custom_attribute_typed_argument,
                                     "System.Reflection",
                                     "CustomAttributeTypedArgument");
static GENERATE_GET_CLASS_WITH_CACHE(custom_attribute_named_argument,
                                     "System.Reflection",
                                     "CustomAttributeNamedArgument");
static GENERATE_TRY_GET_CLASS_WITH_CACHE(customattribute_data,
                                         "System.Reflection",
                                         "CustomAttributeData");

static MonoCustomAttrInfo *
mono_custom_attrs_from_builders_handle(MonoImage *alloc_img, MonoImage *image,
                                       MonoArrayHandle cattrs);

static gboolean bcheck_blob(const char *ptr, int bump, const char *endp,
                            MonoError *error);

static gboolean decode_blob_value_checked(const char *ptr, const char *endp,
                                          guint32 *size_out, const char **retp,
                                          MonoError *error);

static guint32 custom_attrs_idx_from_class(MonoClass *klass);

static void metadata_foreach_custom_attr_from_index(
    MonoImage *image, guint32 idx, MonoAssemblyMetadataCustomAttrIterFunc func,
    gpointer user_data);

/*
 * LOCKING: Acquires the loader lock.
 */
static MonoCustomAttrInfo *lookup_custom_attr(MonoImage *image,
                                              gpointer member) {
  MONO_REQ_GC_NEUTRAL_MODE;

  MonoCustomAttrInfo *res;

  res = (MonoCustomAttrInfo *)mono_image_property_lookup(
      image, member, MONO_PROP_DYNAMIC_CATTR);

  if (!res)
    return NULL;

  res = (MonoCustomAttrInfo *)g_memdup(res, MONO_SIZEOF_CUSTOM_ATTR_INFO +
...
static gboolean custom_attr_visible(MonoImage *image,
                                    MonoReflectionCustomAttrHandle cattr,
                                    MonoReflectionMethodHandle ctor_handle,
                                    MonoMethod **ctor_method)
// ctor_handle is local to this function, allocated by its caller for
// efficiency.
{
  ...
  if (*ctor_method) {
    ...
    if (m_class_get_image(klass) != image) {
      const int visibility =
          (mono_class_get_flags(klass) & TYPE_ATTRIBUTE_VISIBILITY_MASK);
...
static void free_param_data(MonoMethodSignature *sig, void **params) {
  int i;
 ...
 */
static guint32 find_field_index(MonoClass *klass, MonoClassField *field) {
 ...
 */
static guint32 find_property_index(MonoClass *klass, MonoProperty *property) {
  int i;
 ...
 */
static guint32 find_event_index(MonoClass *klass, MonoEvent *event) {
  int i;
...
/*
 * Load the type with name @n on behalf of image @image.  On failure sets @error
 * and returns NULL. The @is_enum flag only affects the error message that's
 * displayed on failure.
 */
static MonoType *cattr_type_from_name(char *n, MonoImage *image,
                                      gboolean is_enum, MonoError *error) {
  ERROR_DECL(inner_error);
  MonoAssemblyLoadContext *alc = mono_domain_ambient_alc(mono_domain_get());
  MonoType *t =
      mono_reflection_type_from_name_checked(n, alc, image, inner_error);
  if (!t) {
    mono_error_set_type_load_name(
        error, g_strdup(n), NULL,
        "Could not load %s %s while decoding custom attribute: %s",
        is_enum ? "enum type" : "type", n, mono_error_get_message(inner_error));
...
static MonoClass *load_cattr_enum_type(MonoImage *image, const char *p,
                                       const char *boundp, const char **end,
                                       MonoError *error) {
  char *n;
  MonoType *t;
  guint32 slen;
...
static MonoType *load_cattr_type(MonoImage *image, MonoType *t, gboolean header,
                                 const char *p, const char *boundp,
                                 const char **end, MonoError *error,
                                 guint32 *slen) {
  MonoType *res;
  char *n;
...
/*
 * If OUT_OBJ is non-NULL, created objects are stored into it and NULL is
 * returned. If OUT_OBJ is NULL, assert if objects were to be created.
 */
static void *load_cattr_value(MonoImage *image, MonoType *t,
                              MonoObject **out_obj, const char *p,
                              const char *boundp, const char **end,
                              MonoError *error) {
  int type = t->type;
  guint32 slen;
  ...
  if (type == MONO_TYPE_GENERICINST) {
    ...
    if (m_class_is_enumtype(cc)) {
      ...
      g_error("Unhandled type of generic instance in load_cattr_value: %s",
              m_class_get_name(cc));
    }
  }
...
handle_enum:
  ...
  case MONO_TYPE_VALUETYPE:
    if (m_class_is_enumtype(t->data.klass)) {
      ...
      if (mono_is_corlib_image(m_class_get_image(k)) &&
          strcmp(m_class_get_name_space(k), "System") == 0 &&
          strcmp(m_class_get_name(k), "DateTime") == 0) {
        guint64 *val = (guint64 *)g_malloc(sizeof(guint64));
        if (!bcheck_blob(p, 7, boundp, error))
          return NULL;
        *val = read64(p);
    ...
    g_error("generic valutype %s not handled in custom attr value decoding",
            m_class_get_name(t->data.klass));
    break;
  ...
  case MONO_TYPE_STRING:
    ...
    *out_obj = (MonoObject *)mono_string_new_wtf8_len_checked(mono_domain_get(),
                                                              p, slen, error);
    return NULL;
  case MONO_TYPE_CLASS: {
    MonoType *type =
        load_cattr_type(image, t, TRUE, p, boundp, end, error, &slen);
    if (out_obj) {
      if (!type)
        return NULL;
      *out_obj = (MonoObject *)mono_type_get_object_checked(mono_domain_get(),
                                                            type, error);
      return NULL;
    } else {
      return type;
  ...
  case MONO_TYPE_OBJECT: {
    ...
    if (subt == CATTR_TYPE_SYSTEM_TYPE) {
      MonoType *type =
          load_cattr_type(image, t, FALSE, p, boundp, end, error, &slen);
      if (out_obj) {
        if (!type)
          return NULL;
        *out_obj = (MonoObject *)mono_type_get_object_checked(mono_domain_get(),
      ...
      g_error("Unknown type 0x%02x for object type encoding in custom attr",
              subt);
    }
    val = load_cattr_value(image, m_class_get_byval_arg(subc), NULL, p, boundp,
                           end, error);
    if (is_ok(error)) {
      obj = mono_object_new_checked(mono_domain_get(), subc, error);
      g_assert(!m_class_has_references(subc));
      if (is_ok(error))
        mono_gc_memmove_atomic(mono_object_get_data(obj), val,
                               mono_class_value_size(subc, NULL));
      g_assert(out_obj);
      *out_obj = obj;
    }
  ...
  case MONO_TYPE_SZARRAY: {
    ...
    case MONO_TYPE_SZARRAY:
      for (i = 0; i < alen; i++) {
        ...
        load_cattr_value(image, m_class_get_byval_arg(tklass), &item, p, boundp,
                         &p, error);
        if (!is_ok(error))
          return NULL;
      ...
      break;
    default:
      g_error("Type 0x%02x not handled in custom attr array decoding",
              basetype);
    }
    *end = p;
    g_assert(out_obj);
...
static MonoObject *load_cattr_value_boxed(MonoDomain *domain, MonoImage *image,
                                          MonoType *t, const char *p,
                                          const char *boundp, const char **end,
                                          MonoError *error) {
  error_init(error);
  ...
  if (is_ref) {
    ...
    MonoObject *boxed = mono_value_box_checked(
        domain, mono_class_from_mono_type_internal(t), val, error);
    g_free(val);
    return boxed;
  }
}

static MonoObject *create_cattr_typed_arg(MonoType *t, MonoObject *val,
                                          MonoError *error) {
  MonoObject *retval;
  ...
  ctor = mono_class_get_method_from_name_checked(
      mono_class_get_custom_attribute_typed_argument_class(), ".ctor", 2, 0,
      error);
  ...
  params[1] = val;
  retval = mono_object_new_checked(
      mono_domain_get(), mono_class_get_custom_attribute_typed_argument_class(),
      error);
  return_val_if_nok(error, NULL);
...
static MonoObject *create_cattr_named_arg(void *minfo, MonoObject *typedarg,
                                          MonoError *error) {
  MonoObject *retval;
  ...
  ctor = mono_class_get_method_from_name_checked(
      mono_class_get_custom_attribute_named_argument_class(), ".ctor", 2, 0,
      error);
  mono_error_assert_ok(error);
  ...
  params[1] = typedarg;
  retval = mono_object_new_checked(
      mono_domain_get(), mono_class_get_custom_attribute_named_argument_class(),
      error);
  return_val_if_nok(error, NULL);
...
mono_custom_attrs_from_builders_handle(MonoImage *alloc_img, MonoImage *image,
  ...
  MonoReflectionCustomAttrHandle cattr =
      MONO_HANDLE_NEW(MonoReflectionCustomAttr, NULL);
  ...
  for (int i = 0; i < count; ++i) {
    ...
    count_visible +=
        custom_attr_visible(image, cattr, ctor_handle, &ctor_method);
  }

  MonoCustomAttrInfo *ainfo;
  ainfo = (MonoCustomAttrInfo *)mono_image_g_malloc0(
      alloc_img, MONO_SIZEOF_CUSTOM_ATTR_INFO +
                     sizeof(MonoCustomAttrEntry) * count_visible);
  ...
  for (int i = 0; i < count; ++i) {
    ...
    unsigned char *saved = (unsigned char *)mono_image_alloc(
        image, mono_array_handle_length(cattr_data));
    MonoGCHandle gchandle = NULL;
    memcpy(saved, MONO_ARRAY_HANDLE_PIN(cattr_data, char, 0, &gchandle),
...
MonoCustomAttrInfo *mono_custom_attrs_from_builders(MonoImage *alloc_img,
                                                    MonoImage *image,
                                                    MonoArray *cattrs) {
  HANDLE_FUNCTION_ENTER();
  MonoCustomAttrInfo *const result = mono_custom_attrs_from_builders_handle(
      alloc_img, image, MONO_HANDLE_NEW(MonoArray, cattrs));
  HANDLE_FUNCTION_RETURN_VAL(result);
}

static void set_custom_attr_fmt_error(MonoError *error) {
  error_init(error);
  mono_error_set_generic_error(
      error, "System.Reflection", "CustomAttributeFormatException",
 ...
 * \param endp upper bound for \p ptr - one past the last valid value for \p
 * ptr. \param error set on error
 *
 ...
 */
static gboolean decode_blob_size_checked(const char *ptr, const char *endp,
                                         guint32 *size_out, const char **retp,
                                         MonoError *error) {
  error_init(error);
 ...
 */
static gboolean decode_blob_value_checked(const char *ptr, const char *endp,
                                          guint32 *value_out, const char **retp,
                                          MonoError *error) {
...
static MonoObjectHandle create_custom_attr(MonoImage *image, MonoMethod *method,
                                           const guchar *data, guint32 len,
                                           MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  if (sig->param_count < 32) {
    ...
    params = (void **)mono_gc_alloc_fixed(
        sig->param_count * sizeof(void *), MONO_GC_DESCRIPTOR_NULL,
        MONO_ROOT_SOURCE_REFLECTION, NULL,
  ...
  for (i = 0; i < mono_method_signature_internal(method)->param_count; ++i) {
    ...
    params[i] = load_cattr_value(
        image, mono_method_signature_internal(method)->params[i], &param_obj, p,
        data_end, &p, error);
    if (param_obj)
      params[i] = param_obj;
    goto_if_nok(error, fail);
  ...
  for (j = 0; j < num_named; j++) {
    ...
    if (named_type == CATTR_TYPE_FIELD) {
      ...
      field = mono_class_get_field_from_name_full(mono_handle_class(attr), name,
                                                  NULL);
      if (!field) {
        mono_error_set_generic_error(
            error, "System.Reflection", "CustomAttributeFormatException",
            "Could not find a field with name %s", name);
        goto fail;
      }

      MonoObject *param_obj;
      val = load_cattr_value(image, field->type, &param_obj, named, data_end,
                             &named, error);
      if (param_obj)
        val = param_obj;
      goto_if_nok(error, fail);

      mono_field_set_value_internal(MONO_HANDLE_RAW(attr), field,
                                    val); // FIXMEcoop
    } else if (named_type == CATTR_TYPE_PROPERTY) {
      MonoProperty *prop;
      prop = mono_class_get_property_from_name_internal(mono_handle_class(attr),
                                                        name);
      if (!prop) {
        mono_error_set_generic_error(
            error, "System.Reflection", "CustomAttributeFormatException",
            "Could not find a property with name %s", name);
        goto fail;
      }

      if (!prop->set) {
        mono_error_set_generic_error(error, "System.Reflection",
                                     "CustomAttributeFormatException",
                                     "Could not find the setter for %s", name);
        goto fail;
      }
      ...
      prop_type = prop->get
                      ...
                      : mono_method_signature_internal(prop->set)
                            ->params[mono_method_signature_internal(prop->set)
                                         ->param_count -
                                     1];

      MonoObject *param_obj;
      pparams[0] = load_cattr_value(image, prop_type, &param_obj, named,
                                    data_end, &named, error);
      if (param_obj)
        pparams[0] = param_obj;
      goto_if_nok(error, fail);
...
static void create_custom_attr_into_array(MonoImage *image, MonoMethod *method,
                                          const guchar *data, guint32 len,
                                          MonoArrayHandle array, int index,
                                          MonoError *error) {
 ...
 *
 *   Create an array of typed and named arguments from the cattr blob given by
 * DATA. TYPED_ARGS and NAMED_ARGS will contain the objects representing the
 * arguments, NAMED_ARG_INFO will contain information about the named arguments.
 */
void mono_reflection_create_custom_attr_data_args(
    MonoImage *image, MonoMethod *method, const guchar *data, guint32 len,
    MonoArrayHandleOut typed_args_h, MonoArrayHandleOut named_args_h,
  ...
  *named_arg_info = NULL;
   ...
   */
  typed_args = mono_array_new_checked(
      domain, mono_get_object_class(),
      mono_method_signature_internal(method)->param_count, error);
  return_if_nok(error);
  ...
  for (i = 0; i < mono_method_signature_internal(method)->param_count; ++i) {
    ...
    obj = load_cattr_value_boxed(
        domain, image, mono_method_signature_internal(method)->params[i], p,
        data_end, &p, error);
    return_if_nok(error);
  ...
  num_named = read16(named);
  named_args =
      mono_array_new_checked(domain, mono_get_object_class(), num_named, error);
  return_if_nok(error);
  ...
  for (j = 0; j < num_named; j++) {
    ...
    if (named_type == CATTR_TYPE_FIELD) {
      ...
      MonoClassField *field =
          mono_class_get_field_from_name_full(attrklass, name, NULL);
      ...
      obj = load_cattr_value_boxed(domain, image, field->type, named, data_end,
                                   &named, error);
      if (!is_ok(error)) {
        g_free(name);
        return;
      ...
      MonoType *prop_type;
      MonoProperty *prop =
          mono_class_get_property_from_name_internal(attrklass, name);
      ...
      obj = load_cattr_value_boxed(domain, image, prop_type, named, data_end,
                                   &named, error);
      if (!is_ok(error)) {
        g_free(name);
        return;
 ...
 *
 * Same as mono_reflection_create_custom_attr_data_args_noalloc but allocate no
 * managed objects, return values using C arrays. Only usable for cattrs with
 * primitive/type arguments. TYPED_ARGS, NAMED_ARGS, and NAMED_ARG_INFO should
 * be freed using g_free ().
 */
void mono_reflection_create_custom_attr_data_args_noalloc(
    MonoImage *image, MonoMethod *method, const guchar *data, guint32 len,
  ...
  *named_arg_info = NULL;
  ...
  for (i = 0; i < sig->param_count; ++i) {
    typed_args[i] =
        load_cattr_value(image, sig->params[i], NULL, p, data_end, &p, error);
    return_if_nok(error);
  }
  ...
  *num_named_args = num_named;
  ...
  for (j = 0; j < num_named; j++) {
    ...
    if (named_type == CATTR_TYPE_FIELD) {
      ...
      named_args[j] = load_cattr_value(image, field->type, NULL, named,
                                       data_end, &named, error);
      if (!is_ok(error)) {
        g_free(name);
        goto fail;
      ...
      named_args[j] = load_cattr_value(image, prop_type, NULL, named, data_end,
                                       &named, error);
      if (!is_ok(error)) {
        g_free(name);
        goto fail;
...
void ves_icall_System_Reflection_CustomAttributeData_ResolveArgumentsInternal(
    MonoReflectionMethodHandle ref_method_h,
    MonoReflectionAssemblyHandle assembly_h, gpointer data, guint32 len,
  ...
  // FIXME: Handles
  mono_reflection_create_custom_attr_data_args(
      image, method, (const guchar *)data, len, ctor_args_h, named_args_h,
      &arginfo, error);
  ...
  for (i = 0; i < mono_array_length_internal(named_args); ++i) {
    ...
    MONO_HANDLE_ASSIGN_RAW(obj_h, obj);
    if (arginfo[i].prop) {
      minfo = (MonoObject *)mono_property_get_object_checked(
          domain, arginfo[i].prop->parent, arginfo[i].prop, error);
      if (!minfo)
        goto leave;
    } else {
      minfo = (MonoObject *)mono_field_get_object_checked(
          domain, NULL, arginfo[i].field, error);
      goto_if_nok(error, leave);
    }
...
static MonoClass *try_get_cattr_data_class(MonoError *error) {
  error_init(error);
  MonoClass *res = mono_class_try_get_customattribute_data_class();
  if (!res)
    mono_error_set_execution_engine(
        error, "Class System.Reflection.CustomAttributeData not found, "
               "probably removed by the linker");
  return res;
}

static MonoObjectHandle create_custom_attr_data(MonoImage *image,
                                                MonoCustomAttrEntry *cattr,
                                                MonoError *error) {
  HANDLE_FUNCTION_ENTER();
...
static void create_custom_attr_data_into_array(MonoImage *image,
                                               MonoCustomAttrEntry *cattr,
                                               MonoArrayHandle result,
                                               int index, MonoError *error) {
  // This function serves to avoid creating handles in a loop.
  HANDLE_FUNCTION_ENTER();
...
mono_custom_attrs_construct_by_type(MonoCustomAttrInfo *cinfo,
                                    MonoClass *attr_klass, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  for (i = 0; i < cinfo->num_attrs; ++i) {
    ...
    if (!centry->ctor) {
      ...
      mono_error_set_type_load_name(
          error, NULL, NULL,
          "Custom attribute constructor is null because the custom attribute "
          "type is not finished yet.");
      goto return_null;
    }
  }
  ...
  result = mono_array_new_cached_handle(
      mono_domain_get(), mono_defaults.attribute_class, n, error);
  goto_if_nok(error, return_null);
  n = 0;
  for (i = 0; i < cinfo->num_attrs; ++i) {
    MonoCustomAttrEntry *centry = &cinfo->attrs[i];
    if (!attr_klass || mono_class_is_assignable_from_internal(
                           attr_klass, centry->ctor->klass)) {
...
MonoArray *mono_custom_attrs_construct(MonoCustomAttrInfo *cinfo) {
  ...
  ERROR_DECL(error);
  MonoArrayHandle result =
      mono_custom_attrs_construct_by_type(cinfo, NULL, error);
...
mono_custom_attrs_data_construct(MonoCustomAttrInfo *cinfo, MonoError *error) {
  ...
  result = mono_array_new_handle(mono_domain_get(), cattr_data,
                                 cinfo->num_attrs, error);
  goto_if_nok(error, return_null);
  for (int i = 0; i < cinfo->num_attrs; ++i) {
    create_custom_attr_data_into_array(cinfo->image, &cinfo->attrs[i], result,
                                       i, error);
    goto_if_nok(error, return_null);
  }
  goto exit;
 ...
 */
MonoCustomAttrInfo *mono_custom_attrs_from_index(MonoImage *image,
                                                 guint32 idx) {
  ERROR_DECL(error);
  MonoCustomAttrInfo *result =
      mono_custom_attrs_from_index_checked(image, idx, FALSE, error);
  mono_error_cleanup(error);
  return result;
}
/**
 * mono_custom_attrs_from_index_checked:
 * \returns NULL if no attributes are found.  On error returns NULL and sets \p
 * error.
 */
MonoCustomAttrInfo *mono_custom_attrs_from_index_checked(
    MonoImage *image, guint32 idx, gboolean ignore_missing, MonoError *error) {
  ...
  ainfo = (MonoCustomAttrInfo *)g_malloc0(MONO_SIZEOF_CUSTOM_ATTR_INFO +
                                          sizeof(MonoCustomAttrEntry) * len);
  ainfo->num_attrs = len;
  ainfo->image = image;
  for (i = 0; i < len; ++i) {
    mono_metadata_decode_row(ca, g_array_index(attr_array, guint32, i), cols,
    ...
    case MONO_CUSTOM_ATTR_TYPE_MEMBERREF:
      ...
      break;
    default:
      g_error("Unknown table for custom attr type %08x",
              cols[MONO_CUSTOM_ATTR_TYPE]);
      break;
    }
    attr = &ainfo->attrs[i];
    attr->ctor = mono_get_method_checked(image, mtoken, NULL, NULL, error);
    if (!attr->ctor) {
      g_warning("Can't find custom attr constructor image: %s mtoken: 0x%08x "
                "due to: %s",
    ...
    if (!mono_verifier_verify_cattr_blob(image, cols[MONO_CUSTOM_ATTR_VALUE],
                                         error)) {
      g_array_free(attr_array, TRUE);
      g_free(ainfo);
      return NULL;
...
MonoCustomAttrInfo *mono_custom_attrs_from_method(MonoMethod *method) {
  ...
  mono_error_cleanup(
      error); /* FIXME want a better API that doesn't swallow the error */
  return result;
}

MonoCustomAttrInfo *mono_custom_attrs_from_method_checked(MonoMethod *method,
                                                          MonoError *error) {
  guint32 idx;
   ...
   * definition.
   *
   * LAMESPEC: The .NET SRE throws an exception for instantiations of generic
   * method builders Note that this stanza is not necessary for non-SRE types,
  ...
  return mono_custom_attrs_from_index_checked(m_class_get_image(method->klass),
                                              idx, FALSE, error);
}
...
guint32 custom_attrs_idx_from_class(MonoClass *klass) {
  guint32 idx;
  ...
  if (m_class_get_byval_arg(klass)->type == MONO_TYPE_VAR ||
    ...
    idx =
        mono_metadata_token_index(m_class_get_sizes(klass).generic_param_token);
    idx <<= MONO_CUSTOM_ATTR_BITS;
...
MonoCustomAttrInfo *mono_custom_attrs_from_class_checked(MonoClass *klass,
                                                         MonoError *error) {
  guint32 idx;
  ...
  return mono_custom_attrs_from_index_checked(m_class_get_image(klass), idx,
                                              FALSE, error);
}
...
MonoCustomAttrInfo *mono_custom_attrs_from_assembly(MonoAssembly *assembly) {
  ERROR_DECL(error);
  MonoCustomAttrInfo *result =
      mono_custom_attrs_from_assembly_checked(assembly, FALSE, error);
  mono_error_cleanup(error);
  return result;
}

MonoCustomAttrInfo *mono_custom_attrs_from_assembly_checked(
    MonoAssembly *assembly, gboolean ignore_missing, MonoError *error) {
  guint32 idx;
  ...
  return mono_custom_attrs_from_index_checked(assembly->image, idx,
                                              ignore_missing, error);
}

static MonoCustomAttrInfo *mono_custom_attrs_from_module(MonoImage *image,
                                                         MonoError *error) {
  guint32 idx;
 ...
 */
MonoCustomAttrInfo *mono_custom_attrs_from_property(MonoClass *klass,
                                                    MonoProperty *property) {
  ERROR_DECL(error);
  MonoCustomAttrInfo *result =
      mono_custom_attrs_from_property_checked(klass, property, error);
  mono_error_cleanup(error);
  return result;
}

MonoCustomAttrInfo *mono_custom_attrs_from_property_checked(
    MonoClass *klass, MonoProperty *property, MonoError *error) {
  guint32 idx;
 ...
 */
MonoCustomAttrInfo *mono_custom_attrs_from_event(MonoClass *klass,
                                                 MonoEvent *event) {
  ERROR_DECL(error);
  MonoCustomAttrInfo *result =
      mono_custom_attrs_from_event_checked(klass, event, error);
  mono_error_cleanup(error);
  return result;
}

MonoCustomAttrInfo *mono_custom_attrs_from_event_checked(MonoClass *klass,
                                                         MonoEvent *event,
                                                         MonoError *error) {
  guint32 idx;
  ...
  if (image_is_dynamic(m_class_get_image(klass))) {
    event = mono_metadata_get_corresponding_event_from_generic_type_definition(
        event);
 ...
 */
MonoCustomAttrInfo *mono_custom_attrs_from_field(MonoClass *klass,
                                                 MonoClassField *field) {
  ERROR_DECL(error);
  MonoCustomAttrInfo *result =
      mono_custom_attrs_from_field_checked(klass, field, error);
  mono_error_cleanup(error);
  return result;
}

MonoCustomAttrInfo *mono_custom_attrs_from_field_checked(MonoClass *klass,
                                                         MonoClassField *field,
                                                         MonoError *error) {
  guint32 idx;
  error_init(error);
  ...
  if (image_is_dynamic(m_class_get_image(klass))) {
    field = mono_metadata_get_corresponding_field_from_generic_type_definition(
        field);
 ...
 * \param method handle to the method that we want to retrieve custom parameter
 * information from \param param parameter number, where zero represent the
 * return value, and one is the first parameter in the method
 *
 * The result must be released with mono_custom_attrs_free().
 *
 * \returns the custom attribute object for the specified parameter, or NULL if
 * there are none.
 */
MonoCustomAttrInfo *mono_custom_attrs_from_param(MonoMethod *method,
                                                 guint32 param) {
  ERROR_DECL(error);
  MonoCustomAttrInfo *result =
      mono_custom_attrs_from_param_checked(method, param, error);
  mono_error_cleanup(error);
  return result;
}
 ...
 * \param method handle to the method that we want to retrieve custom parameter
 * information from \param param parameter number, where zero represent the
 * return value, and one is the first parameter in the method \param error set
 * on error
 *
 * The result must be released with mono_custom_attrs_free().
 *
 * \returns the custom attribute object for the specified parameter, or NULL if
 * there are none.  On failure returns NULL and sets \p error.
 */
MonoCustomAttrInfo *mono_custom_attrs_from_param_checked(MonoMethod *method,
                                                         guint32 param,
                                                         MonoError *error) {
  MonoTableInfo *ca;
  ...
  if (image_is_dynamic(m_class_get_image(method->klass))) {
    ...
    if (!ainfo)
      return NULL;
    size = MONO_SIZEOF_CUSTOM_ATTR_INFO +
           sizeof(MonoCustomAttrEntry) * ainfo->num_attrs;
  ...
  param_list =
      mono_metadata_decode_row_col(ca, method_index - 1, MONO_METHOD_PARAMLIST);
 ...
 */
gboolean mono_custom_attrs_has_attr(MonoCustomAttrInfo *ainfo,
  ...
  for (i = 0; i < ainfo->num_attrs; ++i) {
    ...
    if (klass == attr_klass || mono_class_has_parent(klass, attr_klass) ||
        (MONO_CLASS_IS_INTERFACE_INTERNAL(attr_klass) &&
         mono_class_is_assignable_from_internal(attr_klass, klass)))
      return TRUE;
  }
  return FALSE;
}
 ...
 */
MonoObject *mono_custom_attrs_get_attr(MonoCustomAttrInfo *ainfo,
                                       MonoClass *attr_klass) {
  ERROR_DECL(error);
  MonoObject *res =
      mono_custom_attrs_get_attr_checked(ainfo, attr_klass, error);
  mono_error_assert_ok(error); /*FIXME proper error handling*/
  return res;
}

MonoObject *mono_custom_attrs_get_attr_checked(MonoCustomAttrInfo *ainfo,
                                               MonoClass *attr_klass,
                                               MonoError *error) {
  int i;
  ...
  for (i = 0; i < ainfo->num_attrs; ++i) {
    ...
    if (attr_klass == klass ||
        mono_class_is_assignable_from_internal(attr_klass, klass)) {
      HANDLE_FUNCTION_ENTER();
      MonoObjectHandle result = create_custom_attr(
          ainfo->image, centry->ctor, centry->data, centry->data_size, error);
      HANDLE_FUNCTION_RETURN_OBJ(result);
    }
  }
 ...
 */
MonoCustomAttrInfo *mono_reflection_get_custom_attrs_info(MonoObject *obj_raw) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MONO_HANDLE_DCL(MonoObject, obj);
  MonoCustomAttrInfo *const result =
 ...
 *
 * \returns the custom attribute info for attributes defined for the
 * reflection handle \p obj. The objects. On failure returns NULL and sets \p
 * error.
 *
 * FIXME this function leaks like a sieve for SRE objects.
 */
MonoCustomAttrInfo *
mono_reflection_get_custom_attrs_info_checked(MonoObjectHandle obj,
                                              MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoClass *klass;
  ...
  if (klass == mono_defaults.runtimetype_class) {
    ...
    /*We cannot mono_class_init_internal the class from which we'll load the
     * custom attributes since this must work with broken types.*/
    cinfo = mono_custom_attrs_from_class_checked(klass, error);
    goto_if_nok(error, leave);
  } else if (strcmp("Assembly", klass_name) == 0 ||
             strcmp("RuntimeAssembly", klass_name) == 0) {
    MonoReflectionAssemblyHandle rassembly =
        MONO_HANDLE_CAST(MonoReflectionAssembly, obj);
    cinfo = mono_custom_attrs_from_assembly_checked(
        MONO_HANDLE_GETVAL(rassembly, assembly), FALSE, error);
    goto_if_nok(error, leave);
  } else if (strcmp("RuntimeModule", klass_name) == 0) {
    MonoReflectionModuleHandle module =
        MONO_HANDLE_CAST(MonoReflectionModule, obj);
    cinfo =
        mono_custom_attrs_from_module(MONO_HANDLE_GETVAL(module, image), error);
    goto_if_nok(error, leave);
  } else if (strcmp("RuntimePropertyInfo", klass_name) == 0) {
    MonoReflectionPropertyHandle rprop =
        MONO_HANDLE_CAST(MonoReflectionProperty, obj);
    MonoProperty *property = MONO_HANDLE_GETVAL(rprop, property);
    cinfo = mono_custom_attrs_from_property_checked(property->parent, property,
                                                    error);
    goto_if_nok(error, leave);
  } else if (strcmp("RuntimeEventInfo", klass_name) == 0) {
    MonoReflectionMonoEventHandle revent =
    ...
    MonoReflectionFieldHandle rfield =
        MONO_HANDLE_CAST(MonoReflectionField, obj);
    MonoClassField *field = MONO_HANDLE_GETVAL(rfield, field);
    cinfo = mono_custom_attrs_from_field_checked(field->parent, field, error);
    goto_if_nok(error, leave);
  } else if ((strcmp("RuntimeMethodInfo", klass_name) == 0) ||
             (strcmp("RuntimeConstructorInfo", klass_name) == 0)) {
    MonoReflectionMethodHandle rmethod =
        MONO_HANDLE_CAST(MonoReflectionMethod, obj);
    cinfo = mono_custom_attrs_from_method_checked(
        MONO_HANDLE_GETVAL(rmethod, method), error);
    goto_if_nok(error, leave);
  } else if (strcmp("ParameterInfo", klass_name) == 0 ||
             strcmp("RuntimeParameterInfo", klass_name) == 0) {
    MonoReflectionParameterHandle param =
        MONO_HANDLE_CAST(MonoReflectionParameter, obj);

    MonoObjectHandle member_impl = MONO_HANDLE_NEW(MonoObject, NULL);
    int position;
    mono_reflection_get_param_info_member_and_pos(param, member_impl,
                                                  &position);

    MonoClass *member_class = mono_handle_class(member_impl);
    if (mono_class_is_reflection_method_or_constructor(member_class)) {
      MonoReflectionMethodHandle rmethod =
          MONO_HANDLE_CAST(MonoReflectionMethod, member_impl);
      cinfo = mono_custom_attrs_from_param_checked(
          MONO_HANDLE_GETVAL(rmethod, method), position + 1, error);
      goto_if_nok(error, leave);
    } else if (mono_is_sr_mono_property(member_class)) {
      MonoReflectionPropertyHandle prop =
          MONO_HANDLE_CAST(MonoReflectionProperty, member_impl);
    ...
    else if (mono_is_sre_method_on_tb_inst(
                 member_class)) { /*XXX This is a workaround for Compiler
                                     Context*/
      // FIXME: Is this still needed ?
      g_assert_not_reached();
    } else if (mono_is_sre_ctor_on_tb_inst(
                   member_class)) { /*XX This is a workaround for Compiler
                                       Context*/
      // FIXME: Is this still needed ?
      g_assert_not_reached();
    }
    ...
    MonoReflectionAssemblyBuilderHandle assemblyb =
        MONO_HANDLE_CAST(MonoReflectionAssemblyBuilder, obj);
    ...
    MonoReflectionModuleBuilderHandle module =
        MONO_HANDLE_NEW_GET(MonoReflectionModuleBuilder, tb, module);
    ...
    cinfo = mono_custom_attrs_from_builders_handle(NULL, &dynamic_image->image,
                                                   cattrs);
  } else if (strcmp("ModuleBuilder", klass_name) == 0) {
    MonoReflectionModuleBuilderHandle mb =
        MONO_HANDLE_CAST(MonoReflectionModuleBuilder, obj);
    ...
    goto leave;
#else
    MonoReflectionCtorBuilderHandle cb =
    ...
    cinfo = mono_custom_attrs_from_builders_handle(
        NULL, m_class_get_image(mhandle->klass), cattrs);
#endif
    ...
    MonoReflectionFieldBuilderHandle fb =
        MONO_HANDLE_CAST(MonoReflectionFieldBuilder, obj);
    MonoReflectionTypeBuilderHandle tb =
        MONO_HANDLE_CAST(MonoReflectionTypeBuilder,
                         MONO_HANDLE_NEW_GET(MonoReflectionType, fb, typeb));
    MonoReflectionModuleBuilderHandle mb =
        MONO_HANDLE_NEW_GET(MonoReflectionModuleBuilder, tb, module);
    MonoDynamicImage *dynamic_image = MONO_HANDLE_GETVAL(mb, dynamic_image);
    ...
    MonoReflectionGenericClassHandle gclass =
        MONO_HANDLE_CAST(MonoReflectionGenericClass, obj);
    MonoReflectionTypeHandle generic_type =
        MONO_HANDLE_NEW_GET(MonoReflectionType, gclass, generic_type);
    cinfo = mono_reflection_get_custom_attrs_info_checked(
        MONO_HANDLE_CAST(MonoObject, generic_type), error);
    goto_if_nok(error, leave);
  } else { /* handle other types here... */
    g_error("get custom attrs not yet supported for %s",
 ...
 * reflection handle \p obj. If \p attr_klass is non-NULL, only custom
 * attributes of that type are returned. The objects are fully build. Return
 * NULL if a loading error occurs.
 */
MonoArray *mono_reflection_get_custom_attrs_by_type(MonoObject *obj_raw,
                                                    MonoClass *attr_klass,
                                                    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MONO_HANDLE_DCL(MonoObject, obj);
  MonoArrayHandle result =
      mono_reflection_get_custom_attrs_by_type_handle(obj, attr_klass, error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

MonoArrayHandle mono_reflection_get_custom_attrs_by_type_handle(
    MonoObjectHandle obj, MonoClass *attr_klass, MonoError *error) {
  ...
  if (cinfo) {
    ...
    MONO_HANDLE_ASSIGN(
        result, mono_array_new_handle(mono_domain_get(),
                                      mono_defaults.attribute_class, 0, error));
  }
 ...
 * reflection handle \p obj. The objects are fully build. Return NULL if a
 * loading error occurs.
 */
MonoArray *mono_reflection_get_custom_attrs(MonoObject *obj_raw) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MONO_HANDLE_DCL(MonoObject, obj);
  MonoArrayHandle result =
      mono_reflection_get_custom_attrs_by_type_handle(obj, NULL, error);
  mono_error_cleanup(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}
 ...
 */
MonoArray *mono_reflection_get_custom_attrs_data(MonoObject *obj_raw) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MONO_HANDLE_DCL(MonoObject, obj);
  MonoArrayHandle result =
      mono_reflection_get_custom_attrs_data_checked(obj, error);
  mono_error_cleanup(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}
 ...
 */
MonoArrayHandle
mono_reflection_get_custom_attrs_data_checked(MonoObjectHandle obj,
                                              MonoError *error) {
  MonoArrayHandle result = MONO_HANDLE_NEW(MonoArray, NULL);
  MonoCustomAttrInfo *cinfo;
...
static gboolean
custom_attr_class_name_from_methoddef(MonoImage *image, guint32 method_token,
                                      const gchar **nspace,
 ...
 * @nspace: out argument set to namespace (a string in the string heap of
 * @image) of the custom attr
 ...
 */
static gboolean custom_attr_class_name_from_method_token(
    MonoImage *image, guint32 method_token, guint32 *assembly_token,
    const gchar **nspace, const gchar **class_name) {
  ...
  if (mono_metadata_token_table(method_token) == MONO_TABLE_MEMBERREF) {
    ...
    mono_metadata_decode_row(&image->tables[MONO_TABLE_MEMBERREF], idx - 1,
                             cols, 3);
    ...
    if (class_index == MONO_MEMBERREF_PARENT_TYPEREF) {
      ...
      /* mono_class_from_typeref_checked () */
      {
        ...
        if (nspace)
          *nspace =
              mono_metadata_string_heap(image, cols[MONO_TYPEREF_NAMESPACE]);
        if (assembly_token)
      ...
      if (assembly_token)
        *assembly_token = 0;
      return custom_attr_class_name_from_methoddef(image, methoddef_token,
                                                   nspace, class_name);
    } else {
    ...
    if (assembly_token)
      *assembly_token = 0;
    return custom_attr_class_name_from_methoddef(image, method_token, nspace,
                                                 class_name);
  }
}
 ...
 * Calls \p func for each custom attribute type on the given assembly until \p
 * func returns TRUE. Everything is done using low-level metadata APIs, so it is
 * safe to use during assembly loading.
 */
void mono_assembly_metadata_foreach_custom_attr(
    MonoAssembly *assembly, MonoAssemblyMetadataCustomAttrIterFunc func,
    gpointer user_data) {
  MonoImage *image;
  guint32 idx;
...
/**
 * iterate over the custom attributes that belong to the given index and call
 * func, passing the assembly ref (if any) and the namespace and name of the
 * custom attribute.
 *
 * Everything is done using low-level metadata APIs, so it is safe to use
 * during assembly loading and class initialization.
 */
void metadata_foreach_custom_attr_from_index(
    MonoImage *image, guint32 idx, MonoAssemblyMetadataCustomAttrIterFunc func,
    gpointer user_data) {
  guint32 mtoken, i;
  guint32 cols[MONO_CUSTOM_ATTR_SIZE];
  ...
  while (!stop_iterating && i < ca->rows) {
    ...
    case MONO_CUSTOM_ATTR_TYPE_MEMBERREF:
      ...
      break;
    default:
      g_warning("Unknown table for custom attr type %08x",
                cols[MONO_CUSTOM_ATTR_TYPE]);
      continue;
    }
    ...
    if (!custom_attr_class_name_from_method_token(
            image, mtoken, &assembly_token, &nspace, &name))
      continue;

    stop_iterating =
        func(image, assembly_token, nspace, name, mtoken, user_data);
  }
}
 ...
 * \param user_data passed to \p func
 *
 * Calls \p func for each custom attribute type on the given class until \p func
 * returns TRUE.
 *
 ...
 * \c MonoClass:type_token, \c MonoClass:sizes.generic_param_token,
 * MonoClass:byval_arg
 */
void mono_class_metadata_foreach_custom_attr(
    MonoClass *klass, MonoAssemblyMetadataCustomAttrIterFunc func,
    gpointer user_data) {
...
static void init_weak_fields_inner(MonoImage *image, GHashTable *indexes) {
  MonoTableInfo *tdef;
  ERROR_DECL(error);
  ...
  if (image == mono_get_corlib()) {
    /* Typedef */
    klass =
        mono_class_from_name_checked(image, "System", "WeakAttribute", error);
    if (!is_ok(error)) {
      mono_error_cleanup(error);
      return;
    }
    ...
    for (int i = 0; i < tdef->rows; ++i) {
      ...
      /* 1 based index */
      idx = mtoken - 1;
      if ((col & MONO_CUSTOM_ATTR_TYPE_MASK) ==
        ...
        if (idx >= first_method_idx && idx < first_method_idx + method_count)
          g_hash_table_insert(indexes, GUINT_TO_POINTER(field_idx),
                              GUINT_TO_POINTER(1));
      }
    }
  } else {
    ...
    for (int i = 0; i < tdef->rows; ++i) {
      guint32 string_offset =
          mono_metadata_decode_row_col(tdef, i, MONO_TYPEREF_NAME);
    ...
    for (int i = 0; i < tdef->rows; ++i) {
      ...
      guint32 class_index =
          cols[MONO_MEMBERREF_CLASS] & MONO_MEMBERREF_PARENT_MASK;
      const char *fname =
          mono_metadata_string_heap(image, cols[MONO_MEMBERREF_NAME]);

      if (!strcmp(fname, ".ctor") &&
          class_index == MONO_MEMBERREF_PARENT_TYPEREF) {
        MonoTableInfo *typeref_table = &image->tables[MONO_TABLE_TYPEREF];
        guint32 cols[MONO_TYPEREF_SIZE];

        mono_metadata_decode_row(typeref_table, nindex - 1, cols,
                                 MONO_TYPEREF_SIZE);
        ...
        if (!strcmp(nspace, "System") && !strcmp(name, "WeakAttribute")) {
          MonoClass *klass = mono_class_from_typeref_checked(
              image, MONO_TOKEN_TYPE_REF | nindex, error);
          if (!is_ok(error)) {
            mono_error_cleanup(error);
            return;
          }
          g_assert(!strcmp(m_class_get_name(klass), "WeakAttribute"));
          /* Allow a testing dll as well since some profiles don't have
           * WeakAttribute */
          if (klass && (m_class_get_image(klass) == mono_get_corlib() ||
                        strstr(m_class_get_image(klass)->name,
...
void mono_assembly_init_weak_fields(MonoImage *image) {
  ...
  if (!indexes) {
    ...
    /*
     * To avoid lookups for every field, we scan the customattr table for
     * entries whose parent is a field and whose type is WeakAttribute.
     */
...
gboolean mono_assembly_is_weak_field(MonoImage *image, guint32 field_idx) {
  ...
  return g_hash_table_lookup(image->weak_field_indexes,
                             GINT_TO_POINTER(field_idx)) != NULL;
}
