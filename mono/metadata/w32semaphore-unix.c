...
static gint32 sem_handle_signal(MonoW32Handle *handle_data) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
             "%s: signalling %s handle %p", __func__,
             mono_w32handle_get_typename(handle_data->type), handle_data);

  /* No idea why max is signed, but thats the spec :-( */
  if (sem_handle->val + 1 > (guint32)sem_handle->max) {
    mono_trace(
        G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
        ...
        sem_handle->val, 1, sem_handle->max);
    return MONO_W32HANDLE_WAIT_RET_TOO_MANY_POSTS;
  } else {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
               "%s: %s handle %p val %d count %d max %d", __func__,
               mono_w32handle_get_typename(handle_data->type), handle_data,
               sem_handle->val, 1, sem_handle->max);
...
static gboolean sem_handle_own(MonoW32Handle *handle_data,
                               gboolean *abandoned) {
  MonoW32HandleSemaphore *sem_handle;
...
static void namedsema_details(MonoW32Handle *handle_data) {
  MonoW32HandleNamedSemaphore *namedsem =
      (MonoW32HandleNamedSemaphore *)handle_data->specific;
  g_print("val: %5u, max: %5d, name: \"%s\"", namedsem->s.val, namedsem->s.max,
          namedsem->sharedns.name);
}

static const gchar *sema_typename(void) { return "Semaphore"; }

static gsize sema_typesize(void) { return sizeof(MonoW32HandleSemaphore); }

static const gchar *namedsema_typename(void) { return "N.Semaphore"; }

static gsize namedsema_typesize(void) {
  return sizeof(MonoW32HandleNamedSemaphore);
}

void mono_w32semaphore_init(void) {
  ...
  mono_w32handle_register_capabilities(
      MONO_W32TYPE_SEM, (MonoW32HandleCapability)(MONO_W32HANDLE_CAP_WAIT |
...
static gpointer sem_handle_create(MonoW32HandleSemaphore *sem_handle,
                                  MonoW32Type type, gint32 initial,
                                  gint32 max) {
  ...
  if (handle == INVALID_HANDLE_VALUE) {
    g_warning("%s: error creating %s handle", __func__,
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
             "%s: created %s handle %p", __func__,
             mono_w32handle_get_typename(type), handle);
...
static gpointer sem_create(gint32 initial, gint32 max) {
  MonoW32HandleSemaphore sem_handle;
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
             "%s: creating %s handle, initial %d max %d", __func__,
             mono_w32handle_get_typename(MONO_W32TYPE_SEM), initial, max);
  return sem_handle_create(&sem_handle, MONO_W32TYPE_SEM, initial, max);
}

static gpointer namedsem_create(gint32 initial, gint32 max,
                                const gunichar2 *name, gint32 name_length,
                                MonoError *error) {
  g_assert(name);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
             "%s: creating %s handle, initial %d max %d name \"%s\"", __func__,
             mono_w32handle_get_typename(MONO_W32TYPE_NAMEDSEM), initial, max,
             (const char *)name);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
             ...
             utf8_name, initial, max);

  // Opening named objects does not race.
  mono_w32handle_namespace_lock();

  handle =
      mono_w32handle_namespace_search_handle(MONO_W32TYPE_NAMEDSEM, utf8_name);
  if (handle == INVALID_HANDLE_VALUE) {
    /* The name has already been used for a different object. */
    handle = NULL;
    mono_w32error_set_last(ERROR_INVALID_HANDLE);
  } else if (handle) {
    /* Not an error, but this is how the caller is informed that the semaphore
     * wasn't freshly created */
    ...
    handle = sem_handle_create((MonoW32HandleSemaphore *)&namedsem_handle,
                               MONO_W32TYPE_NAMEDSEM, initial, max);
  }
...
// These functions appear to be using coop-aware locking functions, and so this
// file does not include explicit GC-safe transitions like its corresponding
// Windows version

gpointer ves_icall_System_Threading_Semaphore_CreateSemaphore_icall(
  ...
  gpointer sem = name ? namedsem_create(initialCount, maximumCount, name,
                                        name_length, error)
...
MonoBoolean ves_icall_System_Threading_Semaphore_ReleaseSemaphore_internal(
  ...
  if (handle_data->type != MONO_W32TYPE_SEM &&
      handle_data->type != MONO_W32TYPE_NAMEDSEM) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
             "%s: releasing %s handle %p", __func__,
             mono_w32handle_get_typename(handle_data->type), handle);
  ...
  if (sem_handle->val + releaseCount > (guint32)sem_handle->max) {
    mono_trace(
        ...
        sem_handle->val, releaseCount, sem_handle->max);
...
gpointer ves_icall_System_Threading_Semaphore_OpenSemaphore_icall(
  ...
  *win32error = ERROR_SUCCESS;
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
             "%s: Opening named sem [%s]", __func__, utf8_name);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_SEMAPHORE,
             "%s: returning named sem handle %p", __func__, handle);
