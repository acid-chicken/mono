...
/* Rationale: MonoClass field offsets are computed here.  Need to see the
 * definition.
 */
...
void mono_dump_metadata_offsets(void);
...
void mono_dump_metadata_offsets(void) {
  ...
  g_print("#if defined (USE_CROSS_COMPILE_OFFSETS) || defined "
          "(MONO_CROSS_COMPILE)\n");
...
#define DECL_OFFSET(struct, field)                                             \
  g_print("DECL_OFFSET2(%s,%s,%d)\n", #struct, #field,                         \
          (int)MONO_STRUCT_OFFSET(struct, field));
#define DECL_ALIGN(type)                                                       \
  g_print("DECL_ALIGN2(%s,%d)\n", #type, (int)MONO_ABI_ALIGNOF(type));
#define DECL_SIZE(type)                                                        \
  g_print("DECL_SIZE2(%s,%d)\n", #type, (int)MONO_ABI_SIZEOF(type));
...
void mono_metadata_cross_helpers_run(void);
...
#define DECL_OFFSET2(struct, field, offset)                                    \
  if ((int)G_STRUCT_OFFSET(struct, field) != offset) {                         \
    g_print(#struct ":" #field " invalid struct offset %d (expected %d)\n",    \
            offset, (int)G_STRUCT_OFFSET(struct, field));                      \
    is_broken = TRUE;                                                          \
  }
