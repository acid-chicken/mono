...
static gint32 class_def_count, class_gtd_count, class_ginst_count,
    class_gparam_count, class_array_count, class_pointer_count;
...
static int setup_interface_offsets(MonoClass *klass, int cur_slot,
                                   gboolean overwrite);
...
static void setup_generic_array_ifaces(MonoClass *klass, MonoClass *iface,
                                       MonoMethod **methods, int pos,
                                       GHashTable *cache);
...
/* This TLS variable points to a GSList of classes which have setup_fields ()
 * executing */
static MonoNativeTlsKey setup_fields_tls_id;

static MonoNativeTlsKey init_pending_tls_id;

static void classes_lock(void) {
  mono_locks_os_acquire(&classes_mutex, ClassesLock);
}

static void classes_unlock(void) {
  mono_locks_os_release(&classes_mutex, ClassesLock);
}

/*
We use gclass recording to allow recursive system f types to be referenced by a
parent.
...
To load TextBoxBase<> to do so it must resolve the parent which is
TextInput<TextBox>. To instantiate TextInput<TextBox> it must resolve
TextInput<> and TextBox. To load TextBox it must resolve the parent which is
TextBoxBase<TextBox>.

At this point the runtime must instantiate TextBoxBase<TextBox>. Both types are
partially loaded at this point, iow, both are registered in the type map and
both and a NULL parent. This means that the resulting generic instance will have
a NULL parent, which is wrong and will cause breakage.

To fix that what we do is to record all generic instantes created while
resolving the parent of any generic type definition and, after resolved, correct
the parent field if needed.
...
/*
 * LOCKING: loader lock must be held until pairing disable_gclass_recording is
 * called.
 */
static void enable_gclass_recording(void) { ++record_gclass_instantiation; }

/*
 * LOCKING: loader lock must be held since pairing enable_gclass_recording was
 * called.
 */
static void disable_gclass_recording(gclass_record_func func, void *user_data) {
...
void mono_class_setup_basic_field_info(MonoClass *klass) {
  ...
  if (gklass && image_is_dynamic(gklass->container_class->image) &&
      !gklass->container_class->wastypebuilder) {
    /*
     * This happens when a generic instance of an unfinished generic typebuilder
     * is used as an element type for creating an array type. We can't
     * initialize the fields of this class using the fields of gklass, since
     * gklass is not finished yet, fields could be added to it later.
     */
    return;
  }
  ...
  fields =
      (MonoClassField *)mono_class_alloc0(klass, sizeof(MonoClassField) * top);

  /*
   * Fetch all the field information.
   */
  int first_field_idx = mono_class_has_static_metadata(klass)
  ...
  for (i = 0; i < top; i++) {
    ...
    if (gtd) {
      ...
      guint32 name_idx = mono_metadata_decode_table_row_col(
          image, MONO_TABLE_FIELD, idx, MONO_FIELD_NAME);
 ...
 * typebuilder_setup_fields () is the corresponding function for dynamic
 * classes. Sets the following fields in \p klass:
...
void mono_class_setup_fields(MonoClass *klass) {
  ...
  MonoClass *gtd =
      gklass ? mono_class_get_generic_type_definition(klass) : NULL;
  ...
  if (gtd) {
    ...
    if (mono_class_set_type_load_failure_causedby_class(
            klass, gtd, "Generic type definition failed"))
      return;
  }
  ...
  if (klass->parent) {
    ...
    if (mono_class_set_type_load_failure_causedby_class(
            klass, klass->parent, "Could not set up parent class"))
      return;
  ...
  /* Get the real size */
  explicit_size = mono_metadata_packing_from_typedef(
      klass->image, klass->type_token, &packing_size, &real_size);
  if (explicit_size)
    instance_size += real_size;

#ifdef ENABLE_NETCORE
  if (mono_is_corlib_image(klass->image) &&
      !strcmp(klass->name_space, "System.Numerics") &&
      !strcmp(klass->name, "Register")) {
  ...
  for (i = 0; i < top; i++) {
    ...
    if (layout == TYPE_ATTRIBUTE_EXPLICIT_LAYOUT) {
      ...
      if (offset == (guint32)-1 &&
        ...
        mono_class_set_type_load_failure(
            klass, "Missing field layout info for %s", field->name);
        break;
      }
      if (offset < -1) { /*-1 is used to encode special static fields */
        mono_class_set_type_load_failure(
            klass, "Field '%s' has a negative offset %d", field->name, offset);
        break;
      }
      if (mono_class_is_gtd(klass)) {
        mono_class_set_type_load_failure(
            klass, "Generic class cannot have explicit layout.");
        break;
      }
    }
    if (mono_type_has_exceptions(field->type)) {
      ...
      mono_class_set_type_load_failure(
          klass, "Invalid type %s for instance field %s:%s", type_name,
  ...
  if (!mono_class_has_failure(klass)) {
    mono_loader_lock();
    mono_class_layout_fields(klass, instance_size, packing_size, real_size,
                             FALSE);
    mono_loader_unlock();
  }
...
static gboolean discard_gclass_due_to_failure(MonoClass *gclass,
                                              void *user_data) {
  return mono_class_get_generic_class(gclass)->container_class == user_data;
}

static gboolean fix_gclass_incomplete_instantiation(MonoClass *gclass,
                                                    void *user_data) {
...
static void mono_class_set_failure_and_error(MonoClass *klass, MonoError *error,
                                             const char *msg) {
 ...
 */
MonoClass *mono_class_create_from_typedef(MonoImage *image, guint32 type_token,
                                          MonoError *error) {
  ...
  if (mono_metadata_token_table(type_token) != MONO_TABLE_TYPEDEF ||
      tidx > tt->rows) {
    mono_error_set_bad_image(error, image, "Invalid typedef token %x",
                             type_token);
    return NULL;
  }
  ...
  if ((klass = (MonoClass *)mono_internal_hash_table_lookup(
           &image->class_cache, GUINT_TO_POINTER(type_token)))) {
    mono_loader_unlock();
    return klass;
  }
  ...
  mono_internal_hash_table_insert(&image->class_cache,
                                  GUINT_TO_POINTER(type_token), klass);

  /*
   * Check whether we're a generic type definition.
   */
  if (mono_class_is_gtd(klass)) {
    MonoGenericContainer *generic_container = mono_metadata_load_generic_params(
        image, klass->type_token, NULL, klass);
  ...
  if (cols[MONO_TYPEDEF_EXTENDS]) {
    ...
    guint32 parent_token =
        mono_metadata_token_from_dor(cols[MONO_TYPEDEF_EXTENDS]);
    ...
    for (tmp = parent; tmp; tmp = tmp->parent) {
      if (tmp == klass) {
        mono_class_set_failure_and_error(klass, error,
                                         "Cycle found while resolving parent");
        goto parent_failure;
      }
      if (mono_class_is_gtd(klass) && mono_class_is_ginst(tmp) &&
          mono_class_get_generic_class(tmp)->container_class == klass) {
        mono_class_set_failure_and_error(
            klass, error, "Parent extends generic instance of this type");
        goto parent_failure;
      }
    }
  }
  ...
  /*
   * This might access klass->_byval_arg for recursion generated by generic
   * constraints, so it has to come after setup_mono_type ().
   */
  if ((nesting_tokeen = mono_metadata_nested_in_typedef(image, type_token))) {
    klass->nested_in =
        mono_class_create_from_typedef(image, nesting_tokeen, error);
    if (!is_ok(error)) {
  ...
  if ((mono_class_get_flags(klass) & TYPE_ATTRIBUTE_STRING_FORMAT_MASK) ==
  ...
  if (!klass->enumtype) {
    ...
    /* This is required now that it is possible for more than 2^16 interfaces to
     * exist. */
  ...
  if (mono_is_corlib_image(image) && !strcmp(nspace, "System") &&
      !strcmp(name, "Array")) {
  ...
  if (klass->enumtype) {
    ...
    if (!enum_basetype) {
      /*set it to a default value as the whole runtime can't handle this to be
       * null*/
   ...
   * We must do this after the class has been constructed to make certain
   * recursive scenarios work.
   */
  if (mono_class_is_gtd(klass) &&
      !mono_metadata_load_generic_param_constraints_checked(
          image, type_token, mono_class_get_generic_container(klass), error)) {
    mono_class_set_type_load_failure(
        klass, "Could not load generic parameter constrains due to %s",
        mono_error_get_message(error));
    mono_loader_unlock();
    MONO_PROFILER_RAISE(class_failed, (klass));
    return NULL;
  ...
  if (klass->image->assembly_name &&
      !strcmp(klass->image->assembly_name, "Mono.Simd") &&
      !strcmp(nspace, "Mono.Simd")) {
    if (!strncmp(name, "Vector", 6))
      klass->simd_type = !strcmp(name + 6, "2d") || !strcmp(name + 6, "2ul") ||
                         !strcmp(name + 6, "2l") || !strcmp(name + 6, "4f") ||
                         !strcmp(name + 6, "4ui") || !strcmp(name + 6, "4i") ||
                         !strcmp(name + 6, "8s") || !strcmp(name + 6, "8us") ||
                         !strcmp(name + 6, "16b") || !strcmp(name + 6, "16sb");
  } else if (klass->image->assembly_name &&
             !strcmp(klass->image->assembly_name, "System.Numerics") &&
             !strcmp(nspace, "System.Numerics")) {
    /* The JIT can't handle SIMD types with != 16 size yet */
    // if (!strcmp (name, "Vector2") || !strcmp (name, "Vector3") || !strcmp
    // (name, "Vector4"))
...
static void mono_generic_class_setup_parent(MonoClass *klass, MonoClass *gtd) {
  if (gtd->parent) {
    ...
    klass->parent = mono_class_inflate_generic_class_checked(
        gtd->parent, mono_generic_class_get_context(gclass), error);
    if (!is_ok(error)) {
      /*Set parent to something safe as the runtime doesn't handle well this
       * kind of failure.*/
      klass->parent = mono_defaults.object_class;
      mono_class_set_type_load_failure(
          klass,
          "Parent is a generic type instantiation that failed due to: %s",
          mono_error_get_message(error));
      mono_error_cleanup(error);
    }
  }
...
static gboolean
has_isbyreflike_attribute_func(MonoImage *image, guint32 typeref_scope_token,
                               const char *nspace, const char *name,
                               guint32 method_token, gpointer user_data) {
  struct HasIsByrefLikeUD *has_isbyreflike =
      (struct HasIsByrefLikeUD *)user_data;
  if (!strcmp(name, "IsByRefLikeAttribute") &&
      !strcmp(nspace, "System.Runtime.CompilerServices")) {
...
static gboolean class_has_isbyreflike_attribute(MonoClass *klass) {
  ...
  mono_class_metadata_foreach_custom_attr(klass, has_isbyreflike_attribute_func,
                                          &has_isbyreflike);
  return has_isbyreflike.has_isbyreflike;
}

static gboolean check_valid_generic_inst_arguments(MonoGenericInst *inst,
                                                   MonoError *error) {
  for (int i = 0; i < inst->type_argc; i++) {
    if (!mono_type_is_valid_generic_argument(inst->type_argv[i])) {
      ...
      mono_error_set_invalid_program(
          error, "generic type cannot be instantiated with type '%s'",
          type_name);
      g_free(type_name);
      return FALSE;
    }
...
MonoClass *mono_class_create_generic_inst(MonoGenericClass *gclass) {
  ...
  klass = (MonoClass *)mono_image_set_alloc0(gclass->owner,
                                             sizeof(MonoClassGenericInst));

  gklass = gclass->container_class;

  if (gklass->nested_in) {
    /* The nested_in type should not be inflated since it's possible to produce
     * a nested type with less generic arguments*/
  ...
  klass->this_arg.data.generic_class = klass->_byval_arg.data.generic_class =
      gclass;
  klass->this_arg.byref = TRUE;
  klass->enumtype = gklass->enumtype;
  klass->valuetype = gklass->valuetype;

  if (gklass->image->assembly_name &&
      !strcmp(gklass->image->assembly_name, "System.Numerics.Vectors") &&
      !strcmp(gklass->name_space, "System.Numerics") &&
  ...
  if (gclass->is_dynamic) {
    /*
     * We don't need to do any init workf with unbaked typebuilders. Generic
     * instances created at this point will be later unregistered and/or fixed.
     * This is to avoid work that would probably give wrong results as fields
     * change as we build the TypeBuilder. See
     * remove_instantiations_of_and_ensure_contents in reflection.c and its
     * usage in reflection.c to understand the fixup stage of SRE banking.
     */
...

  {
    ...
    if (!check_valid_generic_inst_arguments(gclass->context.class_inst,
                                            error_inst)) {
      char *gklass_name = mono_type_get_full_name(gklass);
      mono_class_set_type_load_failure(
          klass, "Could not instantiate %s due to %s", gklass_name,
...
static gboolean class_kind_may_contain_generic_instances(MonoTypeKind kind) {
  /* classes of type generic inst may contain generic arguments from other
   * images, as well as arrays and pointers whose element types (recursively)
   * may be a generic inst */
  return (kind == MONO_CLASS_GINST || kind == MONO_CLASS_ARRAY ||
          kind == MONO_CLASS_POINTER);
}
 ...
 */
MonoClass *mono_class_create_bounded_array(MonoClass *eclass, guint32 rank,
                                           gboolean bounded) {
  MonoImage *image;
  ...
  image = eclass->image;
  image_set =
      class_kind_may_contain_generic_instances((MonoTypeKind)eclass->class_kind)
  ...
  if (rank == 1 && !bounded) {
    if (image_set) {
      ...
      cached =
          (MonoClass *)g_hash_table_lookup(image_set->szarray_cache, eclass);
       ...
       * Array:CreateInstance (), etc, so use a separate cache + a separate
       * lock.
       */
    ...
    if (image_set) {
      ...
      for (list = rootlist; list; list = list->next) {
        ...
        if ((m_class_get_rank(k) == rank) &&
            (m_class_get_byval_arg(k)->type ==
             (((rank > 1) || bounded) ? MONO_TYPE_ARRAY : MONO_TYPE_SZARRAY))) {
          cached = k;
          break;
        }
      }
  ...
  klass = image_set
              ? (MonoClass *)mono_image_set_alloc0(image_set,
  ...
  klass->name = image_set ? mono_image_set_strdup(image_set, name)
                          : mono_image_strdup(image, name);
  g_free(name);
  ...
  if (m_class_get_byval_arg(eclass)->type == MONO_TYPE_TYPEDBYREF) {
    ...
    mono_error_set_invalid_program(
        prepared_error, "Arrays of System.TypedReference types are invalid.");
    ...
    /* .NET Core throws a type load exception: "Could not create array type
     * 'fullname[]'" */
    char *full_name = mono_type_get_full_name(eclass);
    mono_class_set_type_load_failure(
        klass, "Could not create array type '%s[]'", full_name);
    g_free(full_name);
    ...
    if (!ref_info_handle || eclass->wastypebuilder) {
      g_warning("Only incomplete TypeBuilder objects are allowed to be an enum "
                "without base_type");
  ...
  mono_class_set_type_load_failure_causedby_class(
      klass, eclass, "Could not load array element type");
  /*FIXME we fail the array type, but we have to let other fields be set.*/

  klass->has_references =
  ...
  if ((rank > 1) || bounded) {
    MonoArrayType *at =
        image_set
            ? (MonoArrayType *)mono_image_set_alloc0(image_set,
                                                     sizeof(MonoArrayType))
            : (MonoArrayType *)mono_image_alloc0(image, sizeof(MonoArrayType));
  ...
  if (rank > 32) {
    ...
    mono_error_set_type_load_class(prepared_error, klass,
 ...
 * element_type and dimension \p rank.
 */
MonoClass *mono_class_create_array(MonoClass *eclass, guint32 rank) {
  return mono_class_create_bounded_array(eclass, rank, FALSE);
}

// This is called by mono_class_create_generic_parameter when a new class must
// be created.
static MonoClass *make_generic_param_class(MonoGenericParam *param) {
  MonoClass *klass, **ptr;
  ...
  if (!is_anonymous) {
    ...
    CHECKED_METADATA_WRITE_PTR_LOCAL(klass->name,
                                     mono_make_generic_name_string(image, n));
  }
  ...
  if (is_anonymous) {
    ...
    CHECKED_METADATA_WRITE_PTR_EXEMPT(
        klass->name_space,
        (omethod && omethod->klass) ? omethod->klass->name_space : "");
  } else {
    MonoClass *oklass = container->owner.klass;
    CHECKED_METADATA_WRITE_PTR_EXEMPT(klass->name_space,
                                      oklass ? oklass->name_space : "");
  }
  ...
  if ((count > 0) && !MONO_CLASS_IS_INTERFACE_INTERNAL(pinfo->constraints[0])) {
    ...
    pos++;
             ...
             pinfo->flags & GENERIC_PARAMETER_ATTRIBUTE_VALUE_TYPE_CONSTRAINT) {
    CHECKED_METADATA_WRITE_PTR(
        klass->parent,
        mono_class_load_from_name(mono_defaults.corlib, "System", "ValueType"));
  } else {
  ...
  if (count - pos > 0) {
    ...
    CHECKED_METADATA_WRITE_PTR_LOCAL(
        klass->interfaces, (MonoClass **)mono_image_alloc0(
                               image, sizeof(MonoClass *) * (count - pos)));
    klass->interfaces_inited = TRUE;
    for (i = pos; i < count; i++)
      CHECKED_METADATA_WRITE_PTR(klass->interfaces[i - pos],
                                 pinfo->constraints[i]);
  }
  ...
  /* We don't use type_token for VAR since only classes can use it (not arrays,
   * pointer, VARs, etc) */
  klass->sizes.generic_param_token = !is_anonymous ? pinfo->token : 0;

  if (param->gshared_constraint) {
    MonoClass *constraint_class =
        mono_class_from_mono_type_internal(param->gshared_constraint);
    mono_class_init_sizes(constraint_class);
    klass->has_references = m_class_has_references(constraint_class);
  }
  /*
   * This makes sure the the value size of this class is equal to the size of
   * the types the gparam is constrained to, the JIT depends on this.
   */
  klass->instance_size =
      MONO_ABI_SIZEOF(MonoObject) +
      mono_type_size(m_class_get_byval_arg(klass), &min_align);
...
MonoClass *mono_class_create_generic_parameter(MonoGenericParam *param) {
  ...
  // But since we wait to grab the lock until after creating the klass, we need
  // to check to make sure another thread did not get in and cache a klass ahead
  // of us. In that case, return their klass and allow our newly-created klass
  // object to just leak.
  mono_memory_barrier();
...
MonoClass *mono_class_create_ptr(MonoType *type) {
  ...
  image_set = class_kind_may_contain_generic_instances(
                  (MonoTypeKind)el_class->class_kind)
  ...
  if (image_set) {
    ...
    if (image_set->ptr_cache) {
      if ((result = (MonoClass *)g_hash_table_lookup(image_set->ptr_cache,
                                                     el_class))) {
        mono_image_set_unlock(image_set);
        return result;
      }
    ...
    if (image->ptr_cache) {
      if ((result =
               (MonoClass *)g_hash_table_lookup(image->ptr_cache, el_class))) {
        mono_image_unlock(image);
        return result;
      }
    }
  ...
  result =
      image_set
          ...
          : (MonoClass *)mono_image_alloc0(image, sizeof(MonoClassPointer));
  ...
  result->name = image_set ? mono_image_set_strdup(image_set, name)
                           : mono_image_strdup(image, name);
  ...
  result->this_arg.data.type = result->_byval_arg.data.type =
  ...
  if (image_set) {
    ...
    if (image_set->ptr_cache) {
      MonoClass *result2;
      if ((result2 = (MonoClass *)g_hash_table_lookup(image_set->ptr_cache,
                                                      el_class))) {
        mono_image_set_unlock(image_set);
...
static void print_implemented_interfaces(MonoClass *klass) {
  char *name;
  ERROR_DECL(error);
  ...
  for (i = 0; i < klass->interface_offsets_count; i++) {
    ...
    printf("  [%03d][UUID %03d][SLOT %03d][SIZE  %03d] interface %s\n", i,
           ...
           mono_class_get_method_count(klass->interfaces_packed[i]), ic_name);
    g_free(ic_name);
  }
...
#else
  for (i = 0; i < ((((klass->max_interface_id + 1) >> 3)) +
                   (((klass->max_interface_id + 1) & 7) ? 1 : 0));
       i++)
    ...
    if (!is_ok(error)) {
      ...
      for (i = 0; i < ifaces->len; i++) {
        ...
        printf("  [%03d][UUID %03d][SLOT %03d][SIZE  %03d] interface %s.%s\n",
               i, ic->interface_id, mono_class_interface_offset(klass, ic),
               mono_class_get_method_count(ic), ic->name_space, ic->name);
      }
...
static gboolean method_is_reabstracted(guint16 flags) {
 ...
 * Even for interfaces we can't simply return the number of methods as all CLR
 * types are allowed to have static methods. Return -1 on failure.
 * FIXME It would be nice if this information could be cached somewhere.
 */
static int count_virtual_methods(MonoClass *klass) {
  int i, mcount, vcount = 0;
  guint32 flags;
  klass = mono_class_get_generic_type_definition(
      klass); /*We can find this information by looking at the GTD*/
  ...
  if (klass->methods || !MONO_CLASS_HAS_STATIC_METADATA(klass)) {
    ...
    mcount = mono_class_get_method_count(klass);
    for (i = 0; i < mcount; ++i) {
      flags = mono_metadata_decode_table_row_col(
          klass->image, MONO_TABLE_METHOD, first_idx + i, MONO_METHOD_FLAGS);
...
static mono_bool set_interface_and_offset(int num_ifaces,
                                          MonoClass **interfaces_full,
                                          int *interface_offsets_full,
                                          MonoClass *ic, int offset,
                                          mono_bool force_set) {
  int i;
  for (i = 0; i < num_ifaces; ++i) {
    if (interfaces_full[i] &&
        interfaces_full[i]->interface_id == ic->interface_id) {
      if (!force_set)
        return TRUE;
 ...
 *
 * Each two-byte element can represent up to 2048 bitmap bits, but as few as a
 * single bitmap byte for non-sparse sequences. In practice the interface
 * bitmaps created during a gmcs bootstrap are reduced to less tha 5% of the
 * original size.
 */
...
/*
 * Return -1 on failure and set klass->has_failure and store a MonoErrorBoxed
 * with the details. LOCKING: Acquires the loader lock.
 */
static int setup_interface_offsets(MonoClass *klass, int cur_slot,
                                   gboolean overwrite) {
  ERROR_DECL(error);
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    for (int i = 0; i < num_ifaces; ++i) {
      ...
      MonoClass *inflated = mono_class_inflate_generic_class_checked(
          gklass_ic, mono_class_get_context(klass), error);
      if (!is_ok(error)) {
        char *name = mono_type_get_full_name(gklass_ic);
        mono_class_set_type_load_failure(
            klass, "Error calculating interface offset of %s", name);
        g_free(name);
        cur_slot = -1;
        goto end;
      }
  ...
  for (j = 0; j < klass->idepth; j++) {
    ...
    if (!is_ok(error)) {
      ...
      mono_class_set_type_load_failure(
          klass, "Error getting the interfaces of %s due to %s", name,
          mono_error_get_message(error));
      g_free(name);
      mono_error_cleanup(error);
      cur_slot = -1;
    ...
    if (ifaces) {
      for (i = 0; i < ifaces->len; ++i) {
        ...
        g_assertf(io >= 0, "class %s parent %s has no offset for iface %s",
                  mono_type_get_full_name(klass), mono_type_get_full_name(k),
                  mono_type_get_full_name(ic));

        set_interface_and_offset(num_ifaces, interfaces_full,
                                 interface_offsets_full, ic, io, TRUE);
      }
    }
  }
  ...
  if (ifaces) {
    for (i = 0; i < ifaces->len; ++i) {
      int count;
      ic = (MonoClass *)g_ptr_array_index(ifaces, i);
      if (set_interface_and_offset(num_ifaces, interfaces_full,
                                   interface_offsets_full, ic, cur_slot, FALSE))
        continue;
  ...
  if (MONO_CLASS_IS_INTERFACE_INTERNAL(klass))
    set_interface_and_offset(num_ifaces, interfaces_full,
                             interface_offsets_full, klass, cur_slot, TRUE);
...
publish:
   ...
   * mono_class_setup_interface_offsets () passes 0 as CUR_SLOT, so the computed
   * interface offsets will be invalid. This means we have to overwrite those
   * when called from other places (#4440).
   */
  if (klass->interfaces_packed) {
    ...
    klass->interfaces_packed = (MonoClass **)mono_class_alloc(
        klass, sizeof(MonoClass *) * interface_offsets_count);
    klass->interface_offsets_packed = (guint16 *)mono_class_alloc(
        klass, sizeof(guint16) * interface_offsets_count);
    bsize =
        (sizeof(guint8) * ((max_iid + 1) >> 3)) + (((max_iid + 1) & 7) ? 1 : 0);
#ifdef COMPRESSED_INTERFACE_BITMAP
...
#if (TRACE_INTERFACE_VTABLE_CODE | DEBUG_INTERFACE_VTABLE_CODE)
static char *mono_signature_get_full_desc(MonoMethodSignature *sig,
...
static void print_method_signatures(MonoMethod *im, MonoMethod *cm) {
  char *im_sig =
      mono_signature_get_full_desc(mono_method_signature_internal(im), TRUE);
  char *cm_sig =
      mono_signature_get_full_desc(mono_method_signature_internal(cm), TRUE);
...
static gboolean is_wcf_hack_disabled(void) {
...
static gboolean check_interface_method_override(
    MonoClass *klass, MonoMethod *im, MonoMethod *cm, gboolean require_newslot,
  ...
  if (strcmp(im->name, cm->name) == 0) {
      ...
      if (require_newslot) {
        if (!interface_is_explicitly_implemented_by_class) {
          TRACE_INTERFACE_VTABLE(
              printf("[NOT EXPLICIT IMPLEMENTATION IN FULL SLOT REFUSED]"));
          return FALSE;
        }
    ...
    if (!cmsig || !imsig) {
      mono_class_set_type_load_failure(
          klass, "Could not resolve the signature of a virtual method");
      return FALSE;
    }
    ...
    if (is_wcf_hack_disabled() &&
        !mono_method_can_access_method_full(cm, im, NULL)) {
      char *body_name = mono_method_full_name(cm, TRUE);
      char *decl_name = mono_method_full_name(im, TRUE);
      mono_class_set_type_load_failure(
          klass, "Method %s overrides method '%s' which is not accessible",
          body_name, decl_name);
      g_free(body_name);
      g_free(decl_name);
      return FALSE;
    ...
    if ((ic_name_space == NULL) ||
        (strcmp(ic_name_space, "System.Collections.Generic") != 0)) {
      TRACE_INTERFACE_VTABLE(printf("[INTERFACE NAMESPACE CHECK FAILED]"));
      return FALSE;
    }
    if ((ic_name == NULL) ||
        ((strcmp(ic_name, "IEnumerable`1") != 0) &&
         (strcmp(ic_name, "ICollection`1") != 0) &&
         (strcmp(ic_name, "IList`1") != 0) &&
         (strcmp(ic_name, "IReadOnlyList`1") != 0) &&
         (strcmp(ic_name, "IReadOnlyCollection`1") != 0))) {
...
#if (TRACE_INTERFACE_VTABLE_CODE | DEBUG_INTERFACE_VTABLE_CODE)
static void foreach_override(gpointer key, gpointer value, gpointer user_data) {
...
static void print_overrides(GHashTable *override_map, const char *message) {
  if (override_map) {
...
static void print_vtable_full(MonoClass *klass, MonoMethod **vtable, int size,
                              int first_non_interface_slot, const char *message,
  ...
  for (i = 0; i < size; ++i) {
    ...
    char newness =
        (i < parent_size) ? 'O' : ((i < first_non_interface_slot) ? 'I' : 'N');

    printf("  [%c][%03d][INDEX %03d] %s [%p]\n", newness, i, cm ? cm->slot : -1,
           cm_name, cm);
    g_free(cm_name);
  }
...
#if VERIFY_INTERFACE_VTABLE_CODE
static int mono_method_try_get_vtable_index(MonoMethod *method) {
...
static void mono_class_verify_vtable(MonoClass *klass) {
  int i, count;
  char *full_name = mono_type_full_name(m_class_get_byval_arg(klass));
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (slot >= 0) {
      ...
      if (slot >= 0 && klass->vtable[slot] != cm && (klass->vtable[slot])) {
        char *other_name =
            klass->vtable[slot]
                ? mono_method_full_name(klass->vtable[slot], TRUE)
                : g_strdup("[null value]");
...
static gboolean verify_class_overrides(MonoClass *klass, MonoMethod **overrides,
                                       int onum) {
  int i;
  ...
  for (i = 0; i < onum; ++i) {
    ...
    if (mono_class_get_generic_type_definition(body->klass) !=
        mono_class_get_generic_type_definition(klass)) {
      mono_class_set_type_load_failure(
          klass, "Method belongs to a different class than the declared one");
      return FALSE;
    }

    if (!(body->flags & METHOD_ATTRIBUTE_VIRTUAL) ||
        (body->flags & METHOD_ATTRIBUTE_STATIC)) {
      if (body->flags & METHOD_ATTRIBUTE_STATIC)
        mono_class_set_type_load_failure(
            klass, "Method must not be static to override a base type");
      else
        mono_class_set_type_load_failure(
            klass, "Method must be virtual to override a base type");
      return FALSE;
    }

    if (!(decl->flags & METHOD_ATTRIBUTE_VIRTUAL) ||
        (decl->flags & METHOD_ATTRIBUTE_STATIC)) {
      if (body->flags & METHOD_ATTRIBUTE_STATIC)
        mono_class_set_type_load_failure(
            klass, "Cannot override a static method in a base type");
      else
        mono_class_set_type_load_failure(
            klass, "Cannot override a non virtual method in a base type");
      return FALSE;
    }

    if (!mono_class_is_assignable_from_slow(decl->klass, klass)) {
      mono_class_set_type_load_failure(
          klass, "Method overrides a class or interface that is not extended "
                 "or implemented by this type");
      return FALSE;
    }
    ...
    if (is_wcf_hack_disabled() &&
        !mono_method_can_access_method_full(body, decl, NULL)) {
 ...
 *
 */
static gboolean mono_class_has_gtd_parent(MonoClass *klass, MonoClass *parent) {
  ...
  return klass->idepth >= parent->idepth &&
         mono_class_get_generic_type_definition(
             klass->supertypes[parent->idepth - 1]) == parent;
}

gboolean mono_class_check_vtable_constraints(MonoClass *klass,
                                             GList *in_setup) {
  ...
  mono_class_setup_vtable_full(mono_class_get_generic_type_definition(klass),
                               in_setup);
  if (mono_class_set_type_load_failure_causedby_class(
  ...
  for (i = 0; i < ginst->type_argc; ++i) {
    ...
    if (mono_class_has_gtd_parent(klass, arg) ||
        mono_class_has_gtd_parent(arg, klass))
      continue;
    if (!mono_class_check_vtable_constraints(arg, in_setup)) {
      mono_class_set_type_load_failure(
          klass, "Failed to load generic parameter %d", i);
      return FALSE;
    }
  }
  return TRUE;
...
static void mono_class_setup_vtable_full(MonoClass *klass, GList *in_setup) {
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    type_token =
        mono_class_get_generic_class(klass)->container_class->type_token;
  } else {
    context = (MonoGenericContext *)mono_class_try_get_generic_container(
        klass); // FIXME is this a case of a try?
    type_token = klass->type_token;
  }

  if (image_is_dynamic(klass->image)) {
    /* Generic instances can have zero method overrides without causing any
     * harm. This is true since we don't do layout all over again for them, we
     * simply inflate the layout of the parent.
     */
    mono_reflection_get_dynamic_overrides(klass, &overrides, &onum, error);
    if (!is_ok(error)) {
      mono_class_set_type_load_failure(
          klass, "Could not load list of method overrides due to %s",
          mono_error_get_message(error));
      goto done;
    }
  } else {
    ...
    mono_class_get_overrides_full(klass->image, type_token, &overrides, &onum,
                                  context, error);
    if (!is_ok(error)) {
      mono_class_set_type_load_failure(
          klass, "Could not load list of method overrides due to %s",
...
static gboolean mono_class_need_stelemref_method(MonoClass *klass) {
  return klass->rank == 1 && MONO_TYPE_IS_REFERENCE(m_class_get_byval_arg(
                                 m_class_get_element_class(klass)));
}

static int apply_override(MonoClass *klass, MonoClass *override_class,
                          MonoMethod **vtable, MonoMethod *decl,
                          MonoMethod *override, GHashTable **override_map,
                          GHashTable **override_class_map,
                          GHashTable **conflict_map) {
  int dslot;
  ...
  // check if the override comes from an interface and the overrided method is
  // from a class, if this is the case it shouldn't be changed
  if (vtable[dslot] && vtable[dslot]->klass &&
      MONO_CLASS_IS_INTERFACE_INTERNAL(override->klass) &&
      !MONO_CLASS_IS_INTERFACE_INTERNAL(vtable[dslot]->klass))
    return TRUE;

  vtable[dslot] = override;
  if (!MONO_CLASS_IS_INTERFACE_INTERNAL(override->klass)) {
    /*
     * If override from an interface, then it is an override of a default
     * interface method, don't override its slot.
     */
  ...
  MonoClass *prev_override_class =
      (MonoClass *)g_hash_table_lookup(class_map, decl);

  g_hash_table_insert(map, decl, override);
  g_hash_table_insert(class_map, decl, override_class);

  /* Collect potentially conflicting overrides which are introduced by default
   * interface methods */
  if (prev_override) {
    ERROR_DECL(error);

    /*
     * The override methods are part of the generic definition, need to inflate
     * them so their parent class becomes the actual interface/class containing
     * the override, i.e. IFace<T> in: class Foo<T> : IFace<T> This is needed so
     * the mono_class_is_assignable_from_internal () calls in the conflict
     * resolution work.
     */
    if (mono_class_is_ginst(override_class)) {
      override = mono_class_inflate_generic_method_checked(
          override, &mono_class_get_generic_class(override_class)->context,
          error);
      mono_error_assert_ok(error);
    }

    if (mono_class_is_ginst(prev_override_class)) {
      prev_override = mono_class_inflate_generic_method_checked(
          prev_override,
          &mono_class_get_generic_class(prev_override_class)->context, error);
      mono_error_assert_ok(error);
    }
...
static void handle_dim_conflicts(MonoMethod **vtable, MonoClass *klass,
                                 GHashTable *conflict_map) {
  GHashTableIter iter;
  MonoMethod *decl;
  GSList *entries, *l, *l2;
  ...
  while (
      g_hash_table_iter_next(&iter, (gpointer *)&decl, (gpointer *)&entries)) {
    /*
     * Iterate over the candidate methods, remove ones whose class is less
     * concrete than the class of another one.
     */
    ...
    if (nentries > 1) {
      ...
      /*
        ...
        mono_method_full_name (decl, TRUE), mono_method_full_name (l->data,
        TRUE));
        }
      */
    } else {
  ...
  if (dim_conflicts) {
    ...
    /*
     * Exceptions are thrown at method call time and only for the methods which
     * have conflicts, so just save them in the class.
     */

    /* Make a copy of the list from the class mempool */
    GSList *conflicts = (GSList *)mono_class_alloc0(
        klass, g_slist_length(dim_conflicts) * sizeof(GSList));
    int i = 0;
...
static void
print_unimplemented_interface_method_info(MonoClass *klass, MonoClass *ic,
                                          MonoMethod *im, int im_slot,
  ...
  for (index = 0; index < onum; ++index) {
    mono_trace_warning(
        MONO_TRACE_TYPE, " at slot %d: %s (%d) overrides %s (%d)", im_slot,
        overrides[index * 2 + 1]->name, overrides[index * 2 + 1]->slot,
        overrides[index * 2]->name, overrides[index * 2]->slot);
  }
  method_signature =
      mono_signature_get_desc(mono_method_signature_internal(im), FALSE);
  type_name = mono_type_full_name(m_class_get_byval_arg(klass));
  mono_trace_warning(
      MONO_TRACE_TYPE,
      "no implementation for interface method %s::%s(%s) in class %s",
      mono_type_get_name(m_class_get_byval_arg(ic)), im->name, method_signature,
      type_name);
  ...
  for (index = 0; index < mcount; ++index) {
    ...
    method_signature =
        mono_signature_get_desc(mono_method_signature_internal(cm), TRUE);

    mono_trace_warning(MONO_TRACE_TYPE, "METHOD %s(%s)", cm->name,
                       method_signature);
    g_free(method_signature);
  }
}
 ...
 *
 * LOCKING: Assumes the loader lock is held (because of the klass->methods
 * check).
 */
static MonoMethod *mono_class_get_virtual_methods(MonoClass *klass,
                                                  gpointer *iter) {
  ...
  /*
   * If the lowest bit of the iterator is 1, this is an iterator for static
   * metadata, and the upper bits contain an index. Otherwise, the iterator is a
   * pointer into klass->methods.
   */
  if ((gsize)(*iter) & 1)
    static_iter = TRUE;
  /* Use the static metadata only if klass->methods is not yet initialized */
  if (!static_iter &&
      !(klass->methods || !MONO_CLASS_HAS_STATIC_METADATA(klass)))
    static_iter = TRUE;
  ...
  if (!static_iter) {
    ...
    if (i < mcount) {
      ERROR_DECL(error);
      res = mono_get_method_checked(klass->image,
                                    MONO_TOKEN_METHOD_DEF | (first_idx + i + 1),
                                    klass, NULL, error);
...
static void print_vtable_layout_result(MonoClass *klass, MonoMethod **vtable,
                                       int cur_slot) {
  int i, icount = 0;
  ...
  printf("VTable %s (vtable entries = %d, interfaces = %d)\n",
         mono_type_full_name(m_class_get_byval_arg(klass)), klass->vtable_size,
         icount);
  ...
  if (icount) {
    ...
    for (i = 0; i < klass->interface_count; i++) {
      ...
      printf("  slot offset: %03d, method count: %03d, iid: %03d %s\n",
             mono_class_interface_offset(klass, ic), count_virtual_methods(ic),
             ic->interface_id, mono_type_full_name(m_class_get_byval_arg(ic)));
    }
    ...
    for (MonoClass *k = klass->parent; k; k = k->parent) {
      for (i = 0; i < k->interface_count; i++) {
        ...
        printf("  parent slot offset: %03d, method count: %03d, iid: %03d %s\n",
               ...
               count_virtual_methods(ic), ic->interface_id,
 ...
 */
static int setup_class_vtsize(MonoClass *klass, GList *in_setup, int *cur_slot,
                              int *stelemref_slot, MonoError *error) {
  GPtrArray *ifaces = NULL;
  int i, max_vtsize = 0;
  ifaces = mono_class_get_implemented_interfaces(klass, error);
  if (!is_ok(error)) {
    char *name = mono_type_get_full_name(klass);
    mono_class_set_type_load_failure(
        klass, "Could not resolve %s interfaces due to %s", name,
        mono_error_get_message(error));
    g_free(name);
    mono_error_cleanup(error);
    return -1;
  ...
  if (klass->parent) {
    ...
    if (mono_class_set_type_load_failure_causedby_class(
            klass, klass->parent, "Parent class failed to load"))
      return -1;
 ...
 * LOCKING: this is supposed to be called with the loader lock held.
 */
static void mono_class_setup_vtable_ginst(MonoClass *klass, GList *in_setup) {
  ERROR_DECL(error);
  int i;
  MonoClass *gklass = mono_class_get_generic_class(klass)->container_class;
  MonoMethod **tmp;
  ...
  if (mono_class_set_type_load_failure_causedby_class(
          klass, gklass, "Could not load generic definition"))
    return;

  tmp = (MonoMethod **)mono_class_alloc0(klass, sizeof(gpointer) *
                                                    gklass->vtable_size);
  klass->vtable_size = gklass->vtable_size;
  for (i = 0; i < gklass->vtable_size; ++i)
    if (gklass->vtable[i]) {
      MonoMethod *inflated = mono_class_inflate_generic_method_full_checked(
          gklass->vtable[i], klass, mono_class_get_context(klass), error);
      if (!is_ok(error)) {
        char *name = mono_type_get_full_name(klass);
        mono_class_set_type_load_failure(
            klass, "VTable setup of type %s failed due to: %s", name,
            mono_error_get_message(error));
        mono_error_cleanup(error);
        g_free(name);
        return;
 ...
 * LOCKING: this is supposed to be called with the loader lock held.
 */
void mono_class_setup_vtable_general(MonoClass *klass, MonoMethod **overrides,
                                     int onum, GList *in_setup) {
  ERROR_DECL(error);
  MonoClass *k, *ic;
  ...
  max_vtsize =
      setup_class_vtsize(klass, in_setup, &cur_slot, &stelemref_slot, error);
  if (max_vtsize == -1)
    return;
  ...
  if (klass->parent && klass->parent->vtable_size)
    memcpy(vtable, klass->parent->vtable,
           sizeof(gpointer) * klass->parent->vtable_size);
  ...
  TRACE_INTERFACE_VTABLE(
      print_vtable_full(klass, vtable, cur_slot, first_non_interface_slot,
  ...
  for (int ifindex = 0; ifindex < klass->interface_offsets_count; ifindex++) {
    ...
    int iface_onum;
    mono_class_get_overrides_full(ic->image, ic->type_token, &iface_overrides,
                                  &iface_onum, mono_class_get_context(ic),
                                  error);
    ...
    for (int i = 0; i < iface_onum; i++) {
      ...
      if (decl->is_inflated) {
        override = mono_class_inflate_generic_method_checked(
            override, mono_method_get_context(decl), error);
        mono_error_assert_ok(error);
      } else if (mono_class_is_gtd(override->klass)) {
        override = mono_class_inflate_generic_method_full_checked(
            override, ic, mono_class_get_context(ic), error);
      }
      if (!apply_override(klass, ic, vtable, decl, override, &override_map,
                          &override_class_map, &conflict_map))
        goto fail;
    }
  ...
  for (i = 0; i < onum; i++) {
    ...
    if (MONO_CLASS_IS_INTERFACE_INTERNAL(decl->klass)) {
      if (!apply_override(klass, klass, vtable, decl, override, &override_map,
                          &override_class_map, &conflict_map))
        goto fail;
    }
  }
  ...
  TRACE_INTERFACE_VTABLE(
      print_overrides(override_map, "AFTER OVERRIDING INTERFACE METHODS"));
  TRACE_INTERFACE_VTABLE(
      print_vtable_full(klass, vtable, cur_slot, first_non_interface_slot,
  ...
  for (i = 0; i < klass->interface_offsets_count; i++) {
    ...
    for (im_index = 0; im_index < mcount; im_index++) {
      ...
      MonoMethod *override_im =
          (override_map != NULL)
              ? (MonoMethod *)g_hash_table_lookup(override_map, im)
              : NULL;
      ...
      TRACE_INTERFACE_VTABLE(
          printf("\tchecking iface method %s\n", mono_method_full_name(im, 1)));

      if (override_im == NULL ||
          (override_im &&
           MONO_CLASS_IS_INTERFACE_INTERNAL(override_im->klass))) {
        int cm_index;
        MonoMethod *cm;
        ...
        for (l = virt_methods; l; l = l->next) {
          ...
          TRACE_INTERFACE_VTABLE(
              printf("    For slot %d ('%s'.'%s':'%s'), trying method "
                     "'%s'.'%s':'%s'... [EXPLICIT IMPLEMENTATION = %d][SLOT IS "
                     "NULL = %d]",
                     im_slot, ic->name_space, ic->name, im->name,
                     cm->klass->name_space, cm->klass->name, cm->name,
                     interface_is_explicitly_implemented_by_class,
                     (vtable[im_slot] == NULL)));
          if (check_interface_method_override(
                  klass, im, cm, TRUE,
                  interface_is_explicitly_implemented_by_class,
                  (vtable[im_slot] == NULL))) {
          ...
          if (mono_class_has_failure(
                  klass)) /*Might be set by check_interface_method_override*/
            goto fail;
        }

        // If the slot is still empty, look in all the inherited virtual
        // methods...
        if ((vtable[im_slot] == NULL) && klass->parent != NULL) {
          ...
          for (cm_index = parent->vtable_size - 1; cm_index >= 0; cm_index--) {
            ...
            TRACE_INTERFACE_VTABLE(
                (cm != NULL) &&
                printf("    For slot %d ('%s'.'%s':'%s'), trying (ancestor) "
                       "method '%s'.'%s':'%s'... ",
                       im_slot, ic->name_space, ic->name, im->name,
                       cm->klass->name_space, cm->klass->name, cm->name));
            if ((cm != NULL) && check_interface_method_override(
                                    klass, im, cm, FALSE, FALSE, TRUE)) {
  ...
  // The check is done here and not directly at the end of the loop above
  // because it can happen (for injected generic array interfaces) that the same
  // slot is processed multiple times (those interfaces have overlapping slots),
  // and it will not always be the first pass the one that fills the slot. Now
  // it is okay to implement a class that is not abstract and implements a
  // interface that has an abstract method because it's reabstracted
  if (!mono_class_is_abstract(klass)) {
    for (i = 0; i < klass->interface_offsets_count; i++) {
      ...
      for (im_index = 0; im_index < mcount; im_index++) {
        ...
        TRACE_INTERFACE_VTABLE(
            printf("      [class is not abstract, checking slot %d for "
                   "interface '%s'.'%s', method %s, slot check is %d]\n",
                   im_slot, ic->name_space, ic->name, im->name,
                   (vtable[im_slot] == NULL)));
        if (vtable[im_slot] == NULL) {
          print_unimplemented_interface_method_info(klass, ic, im, im_slot,
                                                    overrides, onum);
          goto fail;
        }
      }
  ...
  for (l = virt_methods; l; l = l->next) {
    ...
    if (!(cm->flags & METHOD_ATTRIBUTE_NEW_SLOT)) {
      ...
      for (k = klass->parent; k; k = k->parent) {
        ...
        while ((m1 = mono_class_get_virtual_methods(k, &k_iter))) {
          ...
          if (!cmsig ||
              !m1sig) /* FIXME proper error message, use signature_checked? */
            goto fail;
          ...
          if (!strcmp(cm->name, m1->name) &&
            ...
            if (is_wcf_hack_disabled() &&
                !mono_method_can_access_method_full(cm, m1, NULL)) {
              char *body_name = mono_method_full_name(cm, TRUE);
            ...
            if (!override_map)
              override_map = g_hash_table_new(mono_aligned_addr_hash, NULL);
            TRACE_INTERFACE_VTABLE(
                printf("adding iface override from %s [%p] to %s [%p]\n",
    ...
    if ((cm->flags & METHOD_ATTRIBUTE_NEW_SLOT) &&
        !(cm->flags & METHOD_ATTRIBUTE_FINAL) && cm->slot >= 0)
      cm->slot = -1;
  ...
  for (i = 0; i < onum; i++) {
    ...
    if (!MONO_CLASS_IS_INTERFACE_INTERNAL(decl->klass)) {
      ...
      if (!override_map)
        override_map = g_hash_table_new(mono_aligned_addr_hash, NULL);
      TRACE_INTERFACE_VTABLE(
          printf("adding explicit override from %s [%p] to %s [%p]\n",
                 mono_method_full_name(decl, 1), decl,
                 mono_method_full_name(overrides[i * 2 + 1], 1),
                 overrides[i * 2 + 1]));
  ...
  if (override_map) {
    ...
    for (i = 0; i < max_vtsize; ++i)
      if (vtable[i]) {
        TRACE_INTERFACE_VTABLE(
            printf("checking slot %d method %s[%p] for overrides\n", i,
                   mono_method_full_name(vtable[i], 1), vtable[i]));
  ...
  // Now it is okay to implement a class that is not abstract and implements a
  // interface that has an abstract method because it's reabstracted
  if (!mono_class_is_abstract(klass)) {
    for (i = 0; i < cur_slot; ++i) {
      if (vtable[i] == NULL || (vtable[i]->flags & (METHOD_ATTRIBUTE_ABSTRACT |
        ...
        char *method_name = vtable[i] ? mono_method_full_name(vtable[i], TRUE)
                                      : g_strdup("none");
        mono_class_set_type_load_failure(
            klass, "Type %s has invalid vtable method slot %d with method %s",
            type_name, i, method_name);
        g_free(type_name);
        g_free(method_name);
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    /* Check that the vtable_size value computed in mono_class_init_internal ()
     * is correct */
  ...
  if (klass->parent && (klass->parent->vtable_size == klass->vtable_size) &&
      (memcmp(klass->parent->vtable, vtable,
              sizeof(gpointer) * klass->vtable_size) == 0)) {
    mono_memory_barrier();
    klass->vtable = klass->parent->vtable;
  } else {
    MonoMethod **tmp = (MonoMethod **)mono_class_alloc0(
        klass, sizeof(gpointer) * klass->vtable_size);
    memcpy(tmp, vtable, sizeof(gpointer) * klass->vtable_size);
    mono_memory_barrier();
    klass->vtable = tmp;
  }

  DEBUG_INTERFACE_VTABLE(
      print_vtable_full(klass, klass->vtable, klass->vtable_size,
                        first_non_interface_slot, "FINALLY", FALSE));
  if (mono_print_vtable)
    print_vtable_layout_result(klass, vtable, cur_slot);
...
fail : {
  ...
  if (!is_ok(error))
    ...
    mono_class_set_type_load_failure(klass, "VTable setup of type %s failed",
                                     name);
  mono_error_cleanup(error);
  g_free(name);
  if (mono_print_vtable)
...
static char *concat_two_strings_with_zero(MonoImage *image, const char *s1,
                                          const char *s2) {
  int null_length = strlen("(null)");
  int len =
      (s1 ? strlen(s1) : null_length) + (s2 ? strlen(s2) : null_length) + 2;
  char *s = (char *)mono_image_alloc(image, len);
  int result;

  result = g_snprintf(s, len, "%s%c%s", s1 ? s1 : "(null)", '\0',
                      s2 ? s2 : "(null)");
...
static void init_sizes_with_info(MonoClass *klass,
 ...
 *
 *   Initializes the size related fields of @klass without loading all field
 data if possible.
...
static gboolean class_has_references(MonoClass *klass) {
  mono_class_init_sizes(klass);

  /*
   * has_references is not set if this is called recursively, but this is not a
   * problem since this is only used during field layout, and instance fields
   * are initialized before static fields, and instance fields can't embed
   * themselves.
   */
  return klass->has_references;
}

static gboolean type_has_references(MonoClass *klass, MonoType *ftype) {
  if (MONO_TYPE_IS_REFERENCE(ftype) || IS_GC_REFERENCE(klass, ftype) ||
      ((MONO_TYPE_ISSTRUCT(ftype) &&
        class_has_references(mono_class_from_mono_type_internal(ftype)))))
    return TRUE;
  if (!ftype->byref &&
      (ftype->type == MONO_TYPE_VAR || ftype->type == MONO_TYPE_MVAR)) {
    MonoGenericParam *gparam = ftype->data.generic_param;

    if (gparam->gshared_constraint)
      return class_has_references(
          mono_class_from_mono_type_internal(gparam->gshared_constraint));
  }
  return FALSE;
}
 ...
 *
 * If the paramter is a generic sharing parameter, we look at its
 * gshared_constraint->blittable bit.
 */
static gboolean
mono_class_is_gparam_with_nonblittable_parent(MonoClass *klass) {
  ...
  if ((mono_generic_param_info(gparam)->flags &
  ...
  /* Parent can only be: System.Object, System.ValueType or some specific base
   * class.
   *
   * If the parent_class is ValueType, the valuetype constraint would be set,
   * above, so we wouldn't get here.
   *
   * If there was a reference constraint, the parent_class would be
   * System.Object, but we would have returned early above.
   *
   * So if we get here, there is either no base class constraint at all,
   * in which case parent_class would be set to System.Object, or there is none
   * at all.
   */
 ...
 * LOCKING: Acquires the loader lock
 */
void mono_class_layout_fields(MonoClass *klass, int base_instance_size,
                              int packing_size, int explicit_size,
                              gboolean sre) {
  int i;
  ...
  /*
   * We want to avoid doing complicated work inside locks, so we compute all the
   * required information and write it to @klass inside a lock.
   */
  if (klass->fields_inited)
    return;
  ...
  if ((packing_size & 0xffffff00) != 0) {
    mono_class_set_type_load_failure(
        klass, "Could not load struct '%s' with packing size %d >= 256",
        klass->name, packing_size);
    return;
  }
  ...
  The whole layout/instance size code must be reviewed because we do alignment
  calculation in terms of the boxed instance, which leads to unexplainable holes
  at the beginning of an object embedding a simd type. Bug #506144 is an example
  of this issue.
  ...
  if (klass->enumtype) {
    ...
    if (!mono_class_enum_basetype_internal(klass)) {
      mono_class_set_type_load_failure(
          klass, "The enumeration's base type is invalid.");
      return;
    }
  }
  ...
  if (layout == TYPE_ATTRIBUTE_AUTO_LAYOUT &&
      !(mono_is_corlib_image(klass->image) &&
        !strcmp(klass->name_space, "System") &&
        !strcmp(klass->name, "ValueType")) &&
      top)
    blittable = FALSE;
  for (i = 0; i < top; i++) {
    field = &klass->fields[i];
      ...
      if (field->type->byref || MONO_TYPE_IS_REFERENCE(field->type)) {
        ...
        if (field_class) {
          ...
          if (mono_class_has_failure(field_class)) {
            ...
            mono_class_set_type_load_failure(
                klass, "Could not set up field '%s' due to: %s", field->name,
  ...
  case TYPE_ATTRIBUTE_SEQUENTIAL_LAYOUT:
    ...
    if (klass->parent) {
      mono_class_setup_fields(klass->parent);
      if (mono_class_set_type_load_failure_causedby_class(
              klass, klass->parent, "Cannot initialize parent class"))
        return;
    ...
    for (pass = 0; pass < passes; ++pass) {
      for (i = 0; i < top; i++) {
        ...
        /* FIXME (LAMESPEC): should we also change the min alignment according
         * to pack? */
    ...
    for (i = 0; i < top; i++) {
      ...
      if (type_has_references(klass, ftype)) {
        if (field_offsets[i] % TARGET_SIZEOF_VOID_P) {
          mono_class_set_type_load_failure(
              klass,
              "Reference typed field '%s' has explicit offset that is not "
    ...
    if (has_references) {
      ...
      for (i = 0; i < top && !mono_class_has_failure(klass); i++) {
        ...
        for (int j = 0; j < size; j++) {
          if (layout_check[field_offsets[i] + j] != 0 &&
              layout_check[field_offsets[i] + j] != type) {
            mono_class_set_type_load_failure(
                klass,
                "Could not load type '%s' because it contains an object field "
                "at offset %d that is incorrectly aligned or overlapped by a "
                "non-object field.",
                klass->name, field->offset);
            break;
          }
          layout_check[field_offsets[i] + j] = type;
        }
  ...
  if (layout != TYPE_ATTRIBUTE_EXPLICIT_LAYOUT) {
     ...
     * performance, and since the JIT memset/memcpy code assumes this and
     * generates unaligned accesses otherwise. See #78990 for a testcase.
     */
  ...
  if (klass_byval_arg->type == MONO_TYPE_VAR ||
      klass_byval_arg->type == MONO_TYPE_MVAR)
    instance_size = MONO_ABI_SIZEOF(MonoObject) +
                    mono_type_size(klass_byval_arg, &min_align);
  else if (klass_byval_arg->type == MONO_TYPE_PTR)
    instance_size = MONO_ABI_SIZEOF(MonoObject) + MONO_ABI_SIZEOF(gpointer);

  if (klass_byval_arg->type == MONO_TYPE_SZARRAY ||
      klass_byval_arg->type == MONO_TYPE_ARRAY)
  ...
  for (i = 0; i < top; i++) {
    ...
    if (mono_type_has_exceptions(field->type)) {
      mono_class_set_type_load_failure(klass, "Field '%s' has an invalid type.",
                                       field->name);
      break;
    }
  ...
  if (has_static_fields && class_size == 0)
    /* Simplify code which depends on class_size != 0 if the class has static
     * fields */
    class_size = 8;
  ...
  if (klass->valuetype &&
      (klass->instance_size <= 0 ||
       klass->instance_size > (0x100000 + MONO_ABI_SIZEOF(MonoObject)))) {
    /* Special case compiler generated types */
    /* Hard to check for [CompilerGenerated] here */
    if (!strstr(klass->name, "StaticArrayInitTypeSize") &&
        !strstr(klass->name, "$ArrayType"))
      mono_class_set_type_load_failure(klass,
                                       "Value type instance size (%d) cannot "
                                       "be zero, negative, or bigger than 1Mb",
                                       klass->instance_size);
  }
    ...
    for (MonoClass *p = klass; p != NULL; p = p->parent) {
      ...
      while ((field = mono_class_get_fields_internal(p, &iter))) {
        ...
        if (MONO_TYPE_IS_REFERENCE(field->type) &&
            mono_assembly_is_weak_field(p->image, field_idx + 1)) {
          has_weak_fields = TRUE;
          mono_trace_message(MONO_TRACE_TYPE,
                             "Field %s:%s at offset %x is weak.",
                             field->parent->name, field->name, field->offset);
        }
      }
    }
  }
  ...
  for (i = 0; i < top; i++) {
    ...
    /* have to be careful not to recursively invoke mono_class_init on a static
     * field. for example - if the field is an array of a subclass of klass, we
     * can loop.
     */
    ...
    if ((field->type->attrs & FIELD_ATTRIBUTE_STATIC)) {
      mono_class_set_type_load_failure(
          klass, "Static ByRefLike field '%s' is not allowed", field->name);
      return;
    } else {
      /* instance field */
      if (allow_isbyreflike_fields)
        continue;
      mono_class_set_type_load_failure(
          klass, "Instance ByRefLike field '%s' not in a ref struct",
          field->name);
      return;
    }
  }
...
static void initialize_object_slots(MonoClass *klass) {
  int i;
...
int mono_class_get_object_finalize_slot() { return finalize_slot; }

MonoMethod *mono_class_get_default_finalize_method() {
...
static void setup_generic_array_ifaces(MonoClass *klass, MonoClass *iface,
  ...
  for (i = 0; i < generic_array_method_num; i++) {
    ...
    inflated =
        mono_class_inflate_generic_method_checked(m, &tmp_context, error);
    mono_error_assert_ok(error);
    helper = (MonoMethod *)g_hash_table_lookup(cache, inflated);
    if (!helper) {
      helper = mono_marshal_get_generic_array_helper(
          klass, generic_array_method_info[i].name, inflated);
...
static int generic_array_methods(MonoClass *klass) {
  ...
  g_assert(!mono_class_has_failure(
      klass->parent)); /*So hitting this assert is a huge problem*/
  ...
  generic_array_method_info = (GenericArrayMethodInfo *)mono_image_alloc(
      mono_defaults.corlib, sizeof(GenericArrayMethodInfo) * count_generic);
  i = 0;
  for (tmp = list; tmp; tmp = tmp->next) {
    ...
    name = (gchar *)mono_image_alloc(mono_defaults.corlib,
                                     strlen(iname) + strlen(mname) + 1);
    strcpy(name, iname);
...
static int array_get_method_count(MonoClass *klass) {
  ...
  if (klass_byval_arg->type == MONO_TYPE_ARRAY)
    ...
    /* Address */
    return 5;
  else if (klass_byval_arg->type == MONO_TYPE_SZARRAY && klass->rank == 1 &&
           klass->element_class->rank)
    /* Jagged arrays are typed as MONO_TYPE_SZARRAY but have an extra ctor in
     * .net which creates an array of arrays */
    /* ctor([int32]) */
    /* ctor([int32], [int32]) */
    /* Get */
    /* Set */
    ...
    /* Vectors don't have additional constructor since a zero lower bound is
     * assumed */
    /* ctor([int32]*rank) */
    /* Get */
    /* Set */
 ...
 * \returns The new ID.
 */
static guint32 mono_get_unique_iid(MonoClass *klass) {
  int iid;
  ...
  if (iid < 0) {
    ...
    MonoBitSet *new_set =
        mono_bitset_clone(global_interface_bitset, old_size * 2);
  ...
  if (!mono_class_is_ginst(klass)) {
    if (klass->image->interface_bitset) {
      if (iid >= mono_bitset_size(klass->image->interface_bitset)) {
        MonoBitSet *new_set =
            mono_bitset_clone(klass->image->interface_bitset, iid + 1);
  ...
  if (mono_print_vtable) {
    ...
    printf("Interface: assigned id %d to %s|%s|%d\n", iid,
           klass->image->assembly_name, type_name, generic_id);
    g_free(type_name);
  }
#endif

  /* I've confirmed iids safe past 16 bits, however bitset code uses a signed
   * int while testing. Once this changes, it should be safe for us to allow
   * 2^32-1 interfaces, until then 2^31-2 is the max. */
...
gboolean mono_class_init_internal(MonoClass *klass) {
  ...
  if (g_slist_find(init_list, klass)) {
    mono_class_set_type_load_failure(klass,
                                     "Recursive type definition detected %s.%s",
                                     klass->name_space, klass->name);
  ...
  if (mono_verifier_is_enabled_for_class(klass) &&
      !mono_verifier_verify_class(klass)) {
    mono_class_set_type_load_failure(
        klass, "%s",
        concat_two_strings_with_zero(klass->image, klass->name,
  ...
  if (klass_byval_arg->type == MONO_TYPE_ARRAY ||
      klass_byval_arg->type == MONO_TYPE_SZARRAY) {
    ...
    if (mono_class_set_type_load_failure_causedby_class(
            klass, element_class, "Could not load array element class"))
      goto leave;
    if (!cast_class->inited)
      mono_class_init_internal(cast_class);
    if (mono_class_set_type_load_failure_causedby_class(
            klass, cast_class, "Could not load array cast class"))
      goto leave;
  }
  ...
  if (mono_class_is_ginst(klass) &&
      !mono_class_get_generic_class(klass)->is_dynamic) {
    MonoClass *gklass = mono_class_get_generic_class(klass)->container_class;

    mono_class_init_internal(gklass);
    if (mono_class_set_type_load_failure_causedby_class(
            klass, gklass, "Generic Type Definition failed to init"))
      goto leave;
  ...
  /*
   * Initialize the rest of the data without creating a generic vtable if
   * possible. If possible, also compute vtable_size, so
   * mono_class_create_runtime_vtable () can also avoid computing a generic
   * vtable.
   */
  if (has_cached_info) {
    /* AOT case */
    ...
    /* SZARRAY can have 3 vtable layouts, with and without the stelemref method
     * and enum element type The first slot if for array with.
     */
    ...
    if (!MONO_CLASS_IS_INTERFACE_INTERNAL(klass) ||
      ...
      if (mono_class_is_ginst(klass)) {
        ...
        cmethod = mono_find_method_in_metadata(klass, ".cctor", 0,
  ...
  if (klass->parent) {
    if (!klass->parent->vtable_size)
      mono_class_setup_vtable(klass->parent);
    if (mono_class_set_type_load_failure_causedby_class(
            klass, klass->parent, "Parent class vtable failed to initialize"))
      goto leave;
  ...
  if (mono_class_is_ginst(klass) || image_is_dynamic(klass->image) ||
      !klass->type_token ||
      (has_cached_info && !cached_info.has_nested_classes))
 ...
 */
static void init_com_from_comimport(MonoClass *klass) {
  /* we don't always allow COM initialization under the CoreCLR (e.g. Moonlight
   * does not require it) */
  if (mono_security_core_clr_enabled()) {
    /* but some other CoreCLR user could requires it for their platform (i.e.
     * trusted) code */
    if (!mono_security_core_clr_determine_platform_image(klass->image)) {
      /* but it can not be made available for application (i.e. user code) since
       * all COM calls are considered native calls. In this case we fail with a
       * TypeLoadException (just like Silverlight 2 does */
      mono_class_set_type_load_failure(klass, "");
      return;
    }
  }

  /* FIXME : we should add an extra checks to ensure COM can be initialized
...
void mono_class_setup_parent(MonoClass *klass, MonoClass *parent) {
  ...
  if (!MONO_CLASS_IS_INTERFACE_INTERNAL(klass)) {
...
#endif
    if (!parent) {
      /* set the parent to something useful and safe, but mark the type as
       * broken */
    ...
    if (klass->parent->enumtype ||
        (mono_is_corlib_image(klass->parent->image) &&
         (strcmp(klass->parent->name, "ValueType") == 0) &&
         (strcmp(klass->parent->name_space, "System") == 0)))
      klass->valuetype = 1;
    if (mono_is_corlib_image(klass->parent->image) &&
...
static void mono_class_setup_interface_id_internal(MonoClass *klass) {
  ...
  if (mono_is_corlib_image(klass->image) &&
      !strcmp(m_class_get_name_space(klass), "System.Collections.Generic")) {
    // FIXME IEnumerator needs to be special because GetEnumerator uses magic
    // under the hood
    /* FIXME: System.Array/InternalEnumerator don't need all this interface
     *fabrication machinery. MS returns diferrent types based on which instance
     *is called. For example: object obj = new byte[10][]; Type a =
     *((IEnumerable<byte[]>)obj).GetEnumerator ().GetType (); Type b =
     *((IEnumerable<IList<byte>>)obj).GetEnumerator ().GetType (); a != b ==>
     *true
     */
    const char *name = m_class_get_name(klass);
    if (!strcmp(name, "IList`1") || !strcmp(name, "ICollection`1") ||
        !strcmp(name, "IEnumerable`1") || !strcmp(name, "IEnumerator`1"))
...
static MonoMethod *create_array_method(MonoClass *klass, const char *name,
                                       MonoMethodSignature *sig) {
  MonoMethod *method;

  method =
      (MonoMethod *)mono_image_alloc0(klass->image, sizeof(MonoMethodPInvoke));
  method->klass = klass;
 ...
 * from 0.
 *
 * On failure this function sets klass->has_failure and stores a MonoErrorBoxed
 * with details
 */
void mono_class_setup_methods(MonoClass *klass) {
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    if (mono_class_set_type_load_failure_causedby_class(
            klass, gklass, "Generic type definition failed to load"))
      return;

    /* The + 1 makes this always non-NULL to pass the check in
     * mono_class_setup_methods () */
    count = mono_class_get_method_count(gklass);
    methods = (MonoMethod **)mono_class_alloc0(klass, sizeof(MonoMethod *) *
                                                          (count + 1));
    ...
    for (i = 0; i < count; i++) {
      ...
      if (!is_ok(error)) {
        ...
        mono_class_set_type_load_failure(
            klass, "Could not inflate method %s due to %s", method,
            mono_error_get_message(error));
    ...
    methods =
        (MonoMethod **)mono_class_alloc0(klass, sizeof(MonoMethod *) * count);
    ...
    for (i = 0; i < klass->interface_count; i++)
      setup_generic_array_ifaces(klass, klass->interfaces[i], methods,
                                 first_generic + i * count_generic, cache);
    ...
    count = mono_class_get_method_count(klass);
    methods =
        (MonoMethod **)mono_class_alloc(klass, sizeof(MonoMethod *) * count);
    for (i = 0; i < count; ++i) {
      int idx = mono_metadata_translate_token_index(
          klass->image, MONO_TABLE_METHOD, first_idx + i + 1);
      methods[i] = mono_get_method_checked(
          klass->image, MONO_TOKEN_METHOD_DEF | idx, klass, NULL, error);
      if (!methods[i]) {
        mono_class_set_type_load_failure(klass,
  ...
  if (MONO_CLASS_IS_INTERFACE_INTERNAL(klass)) {
    int slot = 0;
    /*Only assign slots to virtual methods as interfaces are allowed to have
     * static methods.*/
    for (i = 0; i < count; ++i) {
...
void mono_class_setup_properties(MonoClass *klass) {
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    first = mono_metadata_properties_from_typedef(
        klass->image, mono_metadata_token_index(klass->type_token) - 1, &last);
    count = last - first;
    ...
    properties =
        (MonoProperty *)mono_class_alloc0(klass, sizeof(MonoProperty) * count);
    for (i = first; i < last; ++i) {
      mono_metadata_decode_table_row(klass->image, MONO_TABLE_PROPERTY, i, cols,
                                     MONO_PROPERTY_SIZE);
      properties[i - first].parent = klass;
      properties[i - first].attrs = cols[MONO_PROPERTY_FLAGS];
      properties[i - first].name =
          mono_metadata_string_heap(klass->image, cols[MONO_PROPERTY_NAME]);
      ...
      for (j = startm; j < endm; ++j) {
        ...
        if (klass->image->uncompressed_metadata) {
           ...
           */
          method = mono_get_method_checked(klass->image,
                                           MONO_TOKEN_METHOD_DEF |
                                               cols[MONO_METHOD_SEMA_METHOD],
                                           klass, NULL, error);
          mono_error_cleanup(error); /* FIXME don't swallow this error */
        } else {
          method =
              klass->methods[cols[MONO_METHOD_SEMA_METHOD] - 1 - first_idx];
        }
  ...
  info = (MonoClassPropertyInfo *)mono_class_alloc0(
      klass, sizeof(MonoClassPropertyInfo));
  info->first = first;
  info->count = count;
...
static MonoMethod **inflate_method_listz(MonoMethod **methods, MonoClass *klass,
                                         MonoGenericContext *context) {
  ...
  for (om = methods, count = 0; *om; ++om, ++count) {
    ERROR_DECL(error);
    retval[count] = mono_class_inflate_generic_method_full_checked(
        *om, klass, context, error);
    g_assert(is_ok(error)); /*FIXME proper error handling*/
  }
...
void mono_class_setup_events(MonoClass *klass) {
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    for (i = 0; i < count; i++) {
      ...
      event->add = gevent->add ? mono_class_inflate_generic_method_full_checked(
                                     gevent->add, klass, context, error)
                               : NULL;
      g_assert(is_ok(error)); /*FIXME proper error handling*/
      event->remove = gevent->remove
                          ? mono_class_inflate_generic_method_full_checked(
                                gevent->remove, klass, context, error)
                          : NULL;
      g_assert(is_ok(error)); /*FIXME proper error handling*/
      event->raise = gevent->raise
                         ? mono_class_inflate_generic_method_full_checked(
                               gevent->raise, klass, context, error)
                         : NULL;
      g_assert(is_ok(error)); /*FIXME proper error handling*/

#ifndef MONO_SMALL_CONFIG
      event->other = gevent->other
                         ? inflate_method_listz(gevent->other, klass, context)
                         : NULL;
#endif
      event->attrs = gevent->attrs;
    }
  } else {
    first = mono_metadata_events_from_typedef(
        klass->image, mono_metadata_token_index(klass->type_token) - 1, &last);
    count = last - first;
    ...
    for (i = first; i < last; ++i) {
      ...
      event->name =
          mono_metadata_string_heap(klass->image, cols[MONO_EVENT_NAME]);
      ...
      for (j = startm; j < endm; ++j) {
        ...
        case METHOD_SEMANTIC_OTHER: {
          ...
          if (event->other == NULL) {
            ...
            while (event->other[n])
              n++;
            event->other = (MonoMethod **)g_realloc(
                event->other, (n + 2) * sizeof(MonoMethod *));
          }
  ...
  info = (MonoClassEventInfo *)mono_class_alloc0(klass,
...
void mono_class_setup_interfaces(MonoClass *klass, MonoError *error) {
  ...
  if (klass->rank == 1 &&
      m_class_get_byval_arg(klass)->type != MONO_TYPE_ARRAY) {
    MonoType *args[1];

    /* IList and IReadOnlyList -> 2x if enum*/
    interface_count = klass->element_class->enumtype ? 4 : 2;
    interfaces = (MonoClass **)mono_image_alloc0(
        klass->image, sizeof(MonoClass *) * interface_count);
    ...
    for (i = 0; i < interface_count; i++) {
      interfaces[i] = mono_class_inflate_generic_class_checked(
          gklass->interfaces[i],
...
void mono_class_setup_has_finalizer(MonoClass *klass) {
  ...
  if (!(MONO_CLASS_IS_INTERFACE_INTERNAL(klass) ||
    ...
    if (m_class_get_rank(klass) == 1 &&
    ...
    } else if (m_class_get_parent(klass) &&
      ...
      if (m_class_get_parent(klass)) {
        ...
        if (mono_class_has_failure(klass))
          ...
          cmethod =
              m_class_get_vtable(klass)[mono_class_get_object_finalize_slot()];
      }

      if (cmethod) {
        g_assert(m_class_get_vtable_size(klass) >
...
void mono_class_setup_supertypes(MonoClass *klass) {
  ...
  if (klass->parent) {
    ...
    int supertype_idx;
    for (supertype_idx = 0; supertype_idx < klass->parent->idepth;
         supertype_idx++)
      CHECKED_METADATA_WRITE_PTR(supertypes[supertype_idx],
                                 klass->parent->supertypes[supertype_idx]);
  } else {
 ...
 *
 * Initialize the nested_classes property for the given MonoClass if it hasn't
...
void mono_class_setup_nested_types(MonoClass *klass) {
  ...
  while (i) {
    ...
    mono_metadata_decode_row(&klass->image->tables[MONO_TABLE_NESTEDCLASS],
                             i - 1, cols, MONO_NESTED_CLASS_SIZE);
    nclass = mono_class_create_from_typedef(
        klass->image, MONO_TOKEN_TYPE_DEF | cols[MONO_NESTED_CLASS_NESTED],
        error);
    if (!is_ok(error)) {
  ...
  for (l = classes; l; l = l->next)
    nested_classes =
        mono_g_list_prepend_image(klass->image, nested_classes, l->data);
  g_list_free(classes);
 ...
 */
void mono_class_setup_runtime_info(MonoClass *klass, MonoDomain *domain,
                                   MonoVTable *vtable) {
  ...
  if (old_info && old_info->max_domain >= domain->domain_id) {
    ...
    MonoClassRuntimeInfo *runtime_info =
        (MonoClassRuntimeInfo *)mono_image_alloc0(
            m_class_get_image(klass),
            MONO_SIZEOF_CLASS_RUNTIME_INFO + new_size * sizeof(gpointer));
    runtime_info->max_domain = new_size - 1;
    /* copy the stuff from the older info */
    if (old_info) {
      memcpy(runtime_info->domain_vtables, old_info->domain_vtables,
             (old_info->max_domain + 1) * sizeof(gpointer));
    }
 ...
 *
 * Returns a \c MonoClass that is used by SGen to fill out nursery fragments
 * before a collection.
 */
 ...
 * Known racy counters: `class_gparam_count`, `classes_size` and
