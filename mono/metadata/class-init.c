...
static MonoClass *make_generic_param_class(MonoGenericParam *param) {
  ...
  klass->instance_size =
      ...
      mono_type_stack_size_internal(m_class_get_byval_arg(klass), NULL, TRUE);
  mono_memory_barrier();
  klass->size_inited = 1;
  ...
  if (klass->parent && klass->parent->vtable_size) {
    ...
    for (i = 0; i < parent->interface_offsets_count; i++) {
      ...
      int interface_offset =
          mono_class_interface_offset(klass, parent_interface);
      /*FIXME this is now dead code as this condition will never hold true.
      Since interface offsets are inherited then the offset of an interface
      implemented by a parent will never be the out of it's vtable boundary.
      */
      if (interface_offset >= parent->vtable_size) {
        int parent_interface_offset =
            mono_class_interface_offset(parent, parent_interface);
        int j;

        mono_class_setup_methods(
            parent_interface); /*FIXME Just kill this whole chunk of dead code*/
        TRACE_INTERFACE_VTABLE(printf("    +++ Inheriting interface %s.%s\n",
                                      parent_interface->name_space,
        ...
        for (j = 0; j < mcount && !mono_class_has_failure(klass); j++) {
          vtable[interface_offset + j] =
              parent->vtable[parent_interface_offset + j];
          TRACE_INTERFACE_VTABLE(
              printf("    --- Inheriting: [%03d][(%03d)+(%03d)] => "
                     "[%03d][(%03d)+(%03d)]\n",
...
void mono_class_layout_fields(MonoClass *klass, int base_instance_size,
  ...
  if (klass_byval_arg->type == MONO_TYPE_VAR ||
    ...
    instance_size = MONO_ABI_SIZEOF(MonoObject) +
                    mono_type_stack_size_internal(klass_byval_arg, NULL, TRUE);
...
static void setup_generic_array_ifaces(MonoClass *klass, MonoClass *iface,
  ...
  tmp_context.method_inst =
      mono_class_get_generic_class(iface)->context.class_inst;
