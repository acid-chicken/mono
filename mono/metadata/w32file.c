...
static guint32 convert_mode(MonoFileMode mono_mode) {
  ...
  case FileMode_Append:
    ...
    break;
  default:
    g_warning("System.IO.FileMode has unknown value 0x%x", mono_mode);
    /* Safe fallback */
    mode = OPEN_EXISTING;
  }
...
static guint32 convert_access(MonoFileAccess mono_access) {
  ...
  case FileAccess_ReadWrite:
    ...
    break;
  default:
    g_warning("System.IO.FileAccess has unknown value 0x%x", mono_access);
    /* Safe fallback */
    access = GENERIC_READ;
  }
...
static guint32 convert_share(MonoFileShare mono_share) {
  ...
  if (mono_share & ~(FileShare_Read | FileShare_Write | FileShare_Delete)) {
    g_warning("System.IO.FileShare has unknown value 0x%x", mono_share);
    /* Safe fallback */
    share = 0;
  }
...
MonoBoolean ves_icall_System_IO_MonoIO_CreateDirectory(const gunichar2 *path,
                                                       gint32 *error) {
  gboolean ret;
...
MonoBoolean ves_icall_System_IO_MonoIO_RemoveDirectory(const gunichar2 *path,
                                                       gint32 *error) {
  gboolean ret;
...
ves_icall_System_IO_MonoIO_FindFirstFile(const gunichar2 *path_with_pattern,
                                         ...
                                         gint32 *file_attr, gint32 *ioerror,
                                         MonoError *error) {
  HANDLE hnd;
  ...
  int len = 0;
  while (data.cFileName[len])
    len++;
  MONO_HANDLE_ASSIGN(file_name,
                     mono_string_new_utf16_handle(mono_domain_get(),
                                                  data.cFileName, len, error));
...
MonoBoolean ves_icall_System_IO_MonoIO_FindNextFile(
    HANDLE hnd, MonoStringHandleOut file_name, gint32 *file_attr,
    gint32 *ioerror, MonoError *error) {
  MonoBoolean res;
  WIN32_FIND_DATA data;
...
MonoBoolean ves_icall_System_IO_MonoIO_FindCloseFile(HANDLE hnd) {
...
ves_icall_System_IO_MonoIO_GetCurrentDirectory(gint32 *io_error,
  ...
  *io_error = ERROR_SUCCESS;
  ...
  if (res_len) {
    ...
    MONO_HANDLE_ASSIGN(result, mono_string_new_utf16_handle(mono_domain_get(),
                                                            buf, len, error));
  } else {
...
MonoBoolean ves_icall_System_IO_MonoIO_MoveFile(const gunichar2 *path,
                                                const gunichar2 *dest,
                                                gint32 *error) {
  *error = ERROR_SUCCESS;
  return mono_w32file_move(path, dest, error);
}

MonoBoolean ves_icall_System_IO_MonoIO_ReplaceFile(
    const gunichar2 *source_file_name, const gunichar2 *destination_file_name,
    const gunichar2 *destination_backup_file_name,
    MonoBoolean ignore_metadata_errors, gint32 *error) {
  ...
  *error = ERROR_SUCCESS;
  ...
  /* FIXME: source and destination file names must not be NULL, but apparently
   * they might be! */
  return mono_w32file_replace(destination_file_name, source_file_name,
                              destination_backup_file_name, replace_flags,
                              error);
}

MonoBoolean ves_icall_System_IO_MonoIO_CopyFile(const gunichar2 *path,
                                                const gunichar2 *dest,
                                                MonoBoolean overwrite,
                                                gint32 *error) {
  *error = ERROR_SUCCESS;
  return mono_w32file_copy(path, dest, overwrite, error);
}

MonoBoolean ves_icall_System_IO_MonoIO_DeleteFile(const gunichar2 *path,
                                                  gint32 *error) {
  gboolean ret;
...
gint32 ves_icall_System_IO_MonoIO_GetFileAttributes(const gunichar2 *path,
                                                    gint32 *error) {
  gint32 ret;
  *error = ERROR_SUCCESS;
...
MonoBoolean ves_icall_System_IO_MonoIO_SetFileAttributes(const gunichar2 *path,
                                                         gint32 attrs,
                                                         gint32 *error) {
  gboolean ret;
  *error = ERROR_SUCCESS;
...
gint32 ves_icall_System_IO_MonoIO_GetFileType(HANDLE handle, gint32 *error) {
  gboolean ret;
...
MonoBoolean ves_icall_System_IO_MonoIO_GetFileStat(const gunichar2 *path,
                                                   MonoIOStat *stat,
                                                   gint32 *error) {
  gboolean result;
...
ves_icall_System_IO_MonoIO_Open(const gunichar2 *filename, gint32 mode,
  ...
  *error = ERROR_SUCCESS;
  ...
  ret =
      mono_w32file_create(filename, convert_access((MonoFileAccess)access_mode),
...
MonoBoolean ves_icall_System_IO_MonoIO_Close(HANDLE handle, gint32 *error) {
  gboolean ret;
  *error = ERROR_SUCCESS;
...
MonoBoolean ves_icall_System_IO_MonoIO_Cancel(HANDLE handle, gint32 *error) {
  gboolean ret;
  *error = ERROR_SUCCESS;
...
gint32 ves_icall_System_IO_MonoIO_Read(HANDLE handle, MonoArrayHandle dest,
                                       gint32 dest_offset, gint32 count,
                                       gint32 *io_error, MonoError *error) {
  void *buffer;
  gboolean result;
  guint32 n;
...
gint32 ves_icall_System_IO_MonoIO_Write(HANDLE handle, MonoArrayHandle src,
...
gint64 ves_icall_System_IO_MonoIO_Seek(HANDLE handle, gint64 offset,
                                       gint32 origin, gint32 *error) {
  gint32 offset_hi;
...
MonoBoolean ves_icall_System_IO_MonoIO_Flush(HANDLE handle, gint32 *error) {
  gboolean ret;
...
gint64 ves_icall_System_IO_MonoIO_GetLength(HANDLE handle, gint32 *error) {
  *error = ERROR_SUCCESS;
  return mono_w32file_get_file_size(handle, error);
}

/* FIXME make gc suspendable */
MonoBoolean ves_icall_System_IO_MonoIO_SetLength(HANDLE handle, gint64 length,
                                                 gint32 *error) {
  ...
  *error = ERROR_SUCCESS;
  ...
  offset_set =
      mono_w32file_seek(handle, offset & 0xFFFFFFFF, &offset_hi, FILE_BEGIN);
  if (offset_set == INVALID_SET_FILE_POINTER) {
    *error = mono_w32error_get_last();
    return (FALSE);
  }
...
MonoBoolean ves_icall_System_IO_MonoIO_SetFileTime(HANDLE handle,
  ...
  *error = ERROR_SUCCESS;
  ...
  ret = mono_w32file_set_times(handle, creation_filetime, access_filetime,
                               write_filetime);
  if (ret == FALSE) {
    *error = mono_w32error_get_last();
  }
...
MonoBoolean ves_icall_System_IO_MonoIO_CreatePipe(HANDLE *read_handle,
                                                  HANDLE *write_handle,
                                                  gint32 *error) {
  *error = ERROR_SUCCESS;
...
MonoBoolean ves_icall_System_IO_MonoIO_DuplicateHandle(
    HANDLE source_process_handle, HANDLE source_handle,
    HANDLE target_process_handle, HANDLE *target_handle, gint32 access,
    gint32 inherit, gint32 options, gint32 *error) {
#ifndef HOST_WIN32
  ...
  *target_handle = mono_w32handle_duplicate(source_handle_data);
  ...
  MONO_ENTER_GC_SAFE;
  ret = DuplicateHandle(source_process_handle, source_handle,
                        target_process_handle, target_handle, access, inherit,
                        options);
  MONO_EXIT_GC_SAFE;
...
#ifndef HOST_WIN32
gunichar2 ves_icall_System_IO_MonoIO_get_VolumeSeparatorChar(void) {
  return (gunichar2)'/'; /* forward slash */
}

gunichar2 ves_icall_System_IO_MonoIO_get_DirectorySeparatorChar(void) {
  return (gunichar2)'/'; /* forward slash */
}

gunichar2 ves_icall_System_IO_MonoIO_get_AltDirectorySeparatorChar(void) {
#if TARGET_WASM
  return (gunichar2)'\\'; /* backslash issue
...
gunichar2 ves_icall_System_IO_MonoIO_get_PathSeparator(void) {
  return (gunichar2)':'; /* colon */
}
#endif /* !HOST_WIN32 */

static const gunichar2 invalid_path_chars[] = {
#if defined(TARGET_WIN32)
    0x0022, /* double quote, which seems allowed in MS.NET but should be
               rejected */
    0x003c, /* less than */
    0x003e, /* greater than */
    0x007c, /* pipe */
    0x0008, 0x0010, 0x0011, 0x0012, 0x0014,
...
ves_icall_System_IO_MonoIO_get_InvalidPathChars(MonoError *error) {
  ...
  MONO_HANDLE_ASSIGN(
      chars, mono_array_new_handle(domain, mono_defaults.char_class, n, error));
...
void ves_icall_System_IO_MonoIO_DumpHandles(void) { mono_w32handle_dump(); }
#endif /* !HOST_WIN32 */
...
gint64 mono_filesize_from_path(MonoString *string) {
  ERROR_DECL(error);
  struct stat buf;
  gint64 res;
...
gint64 mono_filesize_from_fd(int fd) {
  struct stat buf;
  int res;
