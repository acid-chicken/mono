...
#define WRITE_UNALIGNED(type, addr, val) memcpy(addr, &val, sizeof(type))
#define READ_UNALIGNED(type, addr, val) memcpy(&val, addr, sizeof(type))
#else
#define WRITE_UNALIGNED(type, addr, val) (*(type *)(addr) = (val))
#define READ_UNALIGNED(type, addr, val) val = (*(type *)(addr))
#endif
...
static void add_assembly(MonoAssemblyLoadContext *alc, MonoAssembly *assembly,
                         gpointer user_data, MonoError *error);

static MonoDebugHandle *open_symfile_from_bundle(MonoImage *image);

static DebugDomainInfo *get_domain_info(MonoDomain *domain) {
  g_assert(domain->debug_info);

  return (DebugDomainInfo *)domain->debug_info;
}

static void free_debug_handle(MonoDebugHandle *handle) {
  if (handle->ppdb)
...
void mono_debug_init(MonoDebugFormat format) {
  ...
  mono_debug_handles = g_hash_table_new_full(NULL, NULL, NULL,
                                             (GDestroyNotify)free_debug_handle);
...
void mono_debug_open_image_from_memory(MonoImage *image,
                                       const guint8 *raw_contents, int size) {
...
MonoDebugHandle *mono_debug_get_handle(MonoImage *image) {
...
mono_debug_open_image(MonoImage *image, const guint8 *raw_contents, int size) {
  ...
  if (!handle->ppdb)
    handle->symfile =
        mono_debug_open_mono_symbols(handle, raw_contents, size, FALSE);
...
static void lookup_method_func(gpointer key, gpointer value,
                               gpointer user_data) {
...
static MonoDebugMethodInfo *lookup_method(MonoMethod *method) {
...
static void lookup_image_func(gpointer key, gpointer value,
                              gpointer user_data) {
  MonoDebugHandle *handle = (MonoDebugHandle *)value;
...
static void write_leb128(guint32 value, guint8 *ptr, guint8 **rptr) {
  do {
...
static void write_sleb128(gint32 value, guint8 *ptr, guint8 **rptr) {
  gboolean more = 1;
...
static void write_variable(MonoDebugVarInfo *var, guint8 *ptr, guint8 **rptr) {
 ...
 */
MonoDebugMethodAddress *mono_debug_add_method(MonoMethod *method,
  ...
  max_size =
      (5 * LEB128_MAX_SIZE) + 1 + (2 * LEB128_MAX_SIZE * jit->num_line_numbers);
  if (jit->has_var_info) {
    /* this */
    max_size += 1;
    if (jit->this_var)
  ...
  if (method_is_dynamic(method)) {
    ...
    address =
        (MonoDebugMethodAddress *)mono_mempool_alloc(info->mp, total_size);
  }
...
void mono_debug_remove_method(MonoMethod *method, MonoDomain *domain) {
  ...
  address =
      (MonoDebugMethodAddress *)g_hash_table_lookup(info->method_hash, method);
  if (address)
    g_free(address);
...
static guint32 read_leb128(guint8 *ptr, guint8 **rptr) {
...
static gint32 read_sleb128(guint8 *ptr, guint8 **rptr) {
  gint32 result = 0;
  guint32 shift = 0;
...
static void read_variable(MonoDebugVarInfo *var, guint8 *ptr, guint8 **rptr) {
...
static void free_method_jit_info(MonoDebugMethodJitInfo *jit, gboolean stack) {
  if (!jit)
    return;
...
static MonoDebugMethodJitInfo *find_method(MonoMethod *method,
                                           MonoDomain *domain,
                                           MonoDebugMethodJitInfo *jit) {
  DebugDomainInfo *info;
  MonoDebugMethodAddress *address;
...
MonoDebugMethodJitInfo *mono_debug_find_method(MonoMethod *method,
                                               MonoDomain *domain) {
...
static gint32 il_offset_from_address(MonoMethod *method, MonoDomain *domain,
 ...
 */
gint32 mono_debug_il_offset_from_address(MonoMethod *method, MonoDomain *domain,
                                         guint32 native_offset) {
  gint32 res;
 ...
 */
MonoDebugSourceLocation *mono_debug_lookup_source_location(MonoMethod *method,
                                                           guint32 address,
                                                           MonoDomain *domain) {
  MonoDebugMethodInfo *minfo;
  ...
  if (!minfo->handle->ppdb &&
      (!minfo->handle->symfile ||
       !mono_debug_symfile_is_loaded(minfo->handle->symfile))) {
    mono_debugger_unlock();
    return NULL;
  }
...
mono_debug_lookup_source_location_by_il(MonoMethod *method, guint32 il_offset,
                                        MonoDomain *domain) {
  MonoDebugMethodInfo *minfo;
  MonoDebugSourceLocation *location;
...
static int (*get_seq_point)(MonoDomain *domain, MonoMethod *method,
 ...
 */
gchar *mono_debug_print_stack_frame(MonoMethod *method, guint32 native_offset,
                                    MonoDomain *domain) {
  MonoDebugSourceLocation *location;
  gchar *fname, *ptr, *res;
  int offset;
  ...
  if (!location) {
    ...
    if (offset < 0)
      ...
      char *mvid = mono_guid_to_string_minimal(
          (uint8_t *)m_class_get_image(method->klass)->heap_guid.data);
      char *aotid = mono_runtime_get_aotid();
      if (aotid)
        res = g_strdup_printf("at %s [0x%05x] in <%s#%s>:0", fname, offset,
                              mvid, aotid);
      else
...
gboolean mono_is_debugger_attached(void) { return is_attached; }
 ...
 */
void mono_register_symfile_for_assembly(const char *assembly_name,
                                        const mono_byte *raw_contents,
                                        int size) {
 ...
 * mono_debug_enabled:
 *
 * Returns true is debug information is enabled. This doesn't relate if a
...
void mono_debug_get_seq_points(MonoDebugMethodInfo *minfo, char **source_file,
                               GPtrArray **source_file_list, int **source_files,
                               MonoSymSeqPoint **seq_points,
                               int *n_seq_points) {
  if (minfo->handle->ppdb)
    mono_ppdb_get_seq_points(minfo, source_file, source_file_list, source_files,
                             seq_points, n_seq_points);
  else
    mono_debug_symfile_get_seq_points(minfo, source_file, source_file_list,
                                      source_files, seq_points, n_seq_points);
}
