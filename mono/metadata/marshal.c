...
MonoMethod *mono_marshal_get_native_wrapper(MonoMethod *method,
  ...
  if (piinfo->addr == ves_icall_System_String_ctor_RedirectToCreateString) {
    ...
    while ((m = mono_class_get_methods(mono_defaults.string_class, &iter))) {
      /*
       * Find the corresponding String::Ctor () method which has the same
       * signature but its static and returns a string.
       */
...
void mono_marshal_free_co_task_mem(void *ptr) { g_free(ptr); }
...
char *ves_icall_System_Runtime_InteropServices_Marshal_StringToHGlobalAnsi(
    const gunichar2 *utf16, int length);
...
void *mono_marshal_alloc_hglobal(size_t size) { return g_try_malloc(size); }

#endif /* !HOST_WIN32 */

gunichar2 *ves_icall_System_Runtime_InteropServices_Marshal_StringToHGlobalUni(
    const gunichar2 *s, int length);
...
void *mono_marshal_alloc_hglobal_error(gsize size, MonoError *error) {
  if (size == 0)
    /* This returns a valid pointer for size 0 on MS.NET */
    size = 4;
...
gpointer
ves_icall_System_Runtime_InteropServices_Marshal_ReAllocHGlobal(gpointer ptr,
                                                                gsize size) {
...
void mono_marshal_free_hglobal(gpointer ptr) { g_free(ptr); }
...
gpointer
ves_icall_System_Runtime_InteropServices_Marshal_ReAllocCoTaskMem(gpointer ptr,
                                                                  int size) {
