...
void mono_marshal_init(void) {
  ...
  if (!module_initialized) {
    ...
    register_icall(mono_gchandle_get_target_internal, mono_icall_sig_object_ptr,
                   TRUE);
...
static void delegate_hash_table_remove(MonoDelegate *d) {
  ...
  gchandle = (MonoGCHandle)g_hash_table_lookup(delegate_hash_table,
                                               d->delegate_trampoline);
...
static void delegate_hash_table_add(MonoDelegateHandle d) {
  ...
  if (has_target) {
    ...
    MonoGCHandle gchandle = (MonoGCHandle)g_hash_table_lookup(
        delegate_hash_table, delegate_trampoline);
    if (gchandle) {
...
static gsize copy_managed_common(MonoArrayHandle managed, gconstpointer native,
                                 ...
                                 gpointer *managed_addr, MonoGCHandle *gchandle,
                                 MonoError *error) {
...
void ves_icall_System_Runtime_InteropServices_Marshal_copy_from_unmanaged(
  ...
  if (length != 0) {
    MonoGCHandle gchandle = 0;
    gsize const bytes = copy_managed_common(
        dest, src, start_index, length, &managed_dest_addr, &gchandle, error);
    if (bytes)
      memmove(managed_dest_addr, src,
              bytes); // no references should be involved
    mono_gchandle_free_internal(gchandle);
  }
}
