...
typedef struct _Process {
  pid_t pid; /* the pid of the process. This value is only valid until the
                process has exited. */
  MonoCoopSem
      exit_sem; /* this semaphore will be released when the process exits */
...
/* MonoW32HandleProcess is a structure containing all the required information
 ...
 * VS_VERSIONINFO:
 *
 * 2 bytes: Length in bytes (this block, and all child blocks. does _not_
 * include alignment padding between blocks) 2 bytes: Length in bytes of
 * VS_FIXEDFILEINFO struct 2 bytes: Type (contains 1 if version resource
 * contains text data and 0 if version resource contains binary data) Variable
 * length unicode string (null terminated): Key (currently "VS_VERSION_INFO")
 ...
 * Child struct (zero or one StringFileInfo structs, zero or one VarFileInfo
 * structs)
 */
 ...
 * 2 bytes: Type (contains 1 if version resource contains text data and 0 if
 * version resource contains binary data) Variable length unicode string: Key
 * (currently "StringFileInfo") Variable length padding to align Child struct on
 * a 32-bit boundary Child structs ( one or more StringTable structs.  Each
 * StringTable struct's Key member indicates the appropriate language and code
 * page for displaying the text in that StringTable struct.)
 */
 ...
 *
 * 2 bytes: Length in bytes (includes this block as well as all Child blocks,
 * but excludes any padding between String blocks) 2 bytes: Value length (always
 * zero) 2 bytes: Type (contains 1 if version resource contains text data and 0
 * if version resource contains binary data) Variable length unicode string:
 * Key. An 8-digit hex number stored as a unicode string.  The four most
 ...
 * Variable length padding to align Child struct on a 32-bit boundary
 * Child structs (an array of one or more String structs (each aligned on a
 * 32-bit boundary)
 */
 ...
 * 2 bytes: Type (contains 1 if version resource contains text data and 0 if
 * version resource contains binary data) Variable length unicode string: Key.
 * arbitrary string, identifies data. Variable length padding to align Value on
 * a 32-bit boundary Value: Variable length unicode string, holding data.
 */
 ...
 * 2 bytes: Type (contains 1 if version resource contains text data and 0 if
 * version resource contains binary data) Variable length unicode string: Key
 * (currently "VarFileInfo") Variable length padding to align Child struct on a
 * 32-bit boundary Child structs (a Var struct)
 */
 ...
 * 2 bytes: Type (contains 1 if version resource contains text data and 0 if
 * version resource contains binary data) Variable length unicode string: Key
 * ("Translation") Variable length padding to align Value on a 32-bit boundary
 * Value: an array of one or more 4 byte values that are language and code page
 * identifier pairs, low-order word containing a language identifier, and the
 * high-order word containing a code page number.  Either word can be zero,
...
#define MAKELANGID(primary, secondary)                                         \
  ((guint16)((secondary << 10) | (primary)))

#define ALIGN32(ptr)                                                           \
  ptr = (gpointer)((char *)ptr + 3);                                           \
  ptr = (gpointer)((char *)ptr - ((gsize)ptr & 3));

#if HAVE_SIGACTION
static mono_lazy_init_t process_sig_chld_once =
...
static MonoBoolean mono_get_exit_code_process(gpointer handle,
                                              gint32 *exitcode);

/* Check if a pid is valid - i.e. if a process exists with this pid. */
static gboolean process_is_alive(pid_t pid) {
...
static void process_details(MonoW32Handle *handle_data) {
  MonoW32HandleProcess *process_handle =
      (MonoW32HandleProcess *)handle_data->specific;
  g_print("pid: %d, exited: %s, exitstatus: %d", process_handle->pid,
          process_handle->exited ? "true" : "false",
          process_handle->exitstatus);
}

static const gchar *process_typename(void) { return "Process"; }

static gsize process_typesize(void) { return sizeof(MonoW32HandleProcess); }

static MonoW32HandleWaitRet process_wait(MonoW32Handle *handle_data,
                                         guint32 timeout, gboolean *alerted) {
  MonoW32HandleProcess *process_handle;
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s (%p, %" G_GUINT32_FORMAT ")", __func__, handle_data, timeout);
  ...
  if (process_handle->exited) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s (%p, %" G_GUINT32_FORMAT "): Process already exited",
  ...
  if (pid == mono_process_current_pid()) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s (%p, %" G_GUINT32_FORMAT "): waiting on current process",
               __func__, handle_data, timeout);
    return MONO_W32HANDLE_WAIT_RET_TIMEOUT;
  }

  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s (%p, %" G_GUINT32_FORMAT "): PID: %d", __func__, handle_data,
             timeout, pid);

  if (!process_handle->child) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s (%p, %" G_GUINT32_FORMAT "): waiting on non-child process",
               __func__, handle_data, timeout);
    ...
    if (!process_is_alive(pid)) {
      ...
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 ...
                 "): non-child process is not alive anymore (2)",
                 __func__, handle_data, timeout);
      return MONO_W32HANDLE_WAIT_RET_SUCCESS_0;
    }
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s (%p, %" G_GUINT32_FORMAT
               "): non-child process wait failed, error : %s (%d))",
               __func__, handle_data, timeout, g_strerror(errno), errno);
  ...
  while (1) {
    if (timeout != MONO_INFINITE_WAIT) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s (%p, %" G_GUINT32_FORMAT
                 "): waiting on semaphore for %" G_GINT64_FORMAT " ms...",
                 __func__, handle_data, timeout, timeout - (now - start));
      ret = mono_coop_sem_timedwait(
          &process->exit_sem, (timeout - (now - start)),
          alerted ? MONO_SEM_FLAGS_ALERTABLE : MONO_SEM_FLAGS_NONE);
    } else {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s (%p, %" G_GUINT32_FORMAT
                 "): waiting on semaphore forever...",
                 __func__, handle_data, timeout);
      ret = mono_coop_sem_wait(&process->exit_sem,
                               alerted ? MONO_SEM_FLAGS_ALERTABLE
                                       : MONO_SEM_FLAGS_NONE);
    }
    ...
    if (ret == MONO_SEM_TIMEDWAIT_RET_TIMEDOUT) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s (%p, %" G_GUINT32_FORMAT "): wait timeout (timeout = 0)",
                 __func__, handle_data, timeout);
      return MONO_W32HANDLE_WAIT_RET_TIMEOUT;
    }
    ...
    if (now - start >= timeout) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s (%p, %" G_GUINT32_FORMAT "): wait timeout", __func__,
                 handle_data, timeout);
      return MONO_W32HANDLE_WAIT_RET_TIMEOUT;
    }
    ...
    if (alerted && ret == MONO_SEM_TIMEDWAIT_RET_ALERTED) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s (%p, %" G_GUINT32_FORMAT "): wait alerted", __func__,
                 handle_data, timeout);
      *alerted = TRUE;
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s (%p, %" G_GUINT32_FORMAT "): Waited successfully", __func__,
             handle_data, timeout);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             ...
             __func__, handle_data, timeout, process_handle->pid,
             process_handle->exitstatus);
...
static void processes_cleanup(void) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS, "%s done",
             __func__);

  mono_atomic_xchg_i32(&cleaning_up, 0);
}

static void process_close(gpointer data) {
  MonoW32HandleProcess *process_handle;
...
static void process_set_defaults(MonoW32HandleProcess *process_handle) {
...
static void process_set_name(MonoW32HandleProcess *process_handle) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: using [%s] as prog name", __func__, progname);
...
void mono_w32process_init(void) {
  MonoW32HandleProcess process_handle;
  ...
  mono_w32handle_register_capabilities(
      ...
      (MonoW32HandleCapability)(MONO_W32HANDLE_CAP_WAIT |
...
void mono_w32process_cleanup(void) { g_free(cli_launcher); }

static int len16(const gunichar2 *str) {
  int len = 0;
...
static gunichar2 *utf16_concat(const gunichar2 *first, ...) {
  va_list args;
  int total = 0, i;
  const gunichar2 *s;
  const gunichar2 *p;
  ...
  for (s = first; *s != 0; s++)
    ret[i++] = *s;
  va_start(args, first);
  for (s = va_arg(args, gunichar2 *); s != NULL;
       s = va_arg(args, gunichar2 *)) {
    for (p = s; *p != 0; p++)
      ret[i++] = *p;
  }
  va_end(args);
...
guint32 mono_w32process_get_pid(gpointer handle) {
  ...
  if (!mono_w32handle_lookup_and_ref(handle, &handle_data)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: unknown handle %p", __func__, handle);
  ...
  if (handle_data->type != MONO_W32TYPE_PROCESS) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: unknown process handle %p", __func__, handle);
    mono_w32error_set_last(ERROR_INVALID_HANDLE);
...
static gboolean get_process_foreach_callback(MonoW32Handle *handle_data,
                                             gpointer user_data) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: looking at process %d", __func__, process_handle->pid);
...
ves_icall_System_Diagnostics_Process_GetProcess_internal(guint32 pid,
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: looking for process %d", __func__, pid);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: Can't find pid %d", __func__, pid);
...
static gboolean match_procname_to_modulename(char *procname, char *modulename) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: procname=\"%s\", modulename=\"%s\"", __func__, procname,
             modulename);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: result is %" G_GINT32_FORMAT, __func__, result);
  return result;
}

gboolean mono_w32process_try_get_modules(gpointer handle, gpointer *modules,
...
gboolean mono_w32process_module_get_filename(gpointer handle, gpointer module,
                                             gunichar2 **str, guint32 *len) {
  gint pid;
  gsize bytes = 0;
  gchar *path;
  gunichar2 *proc_path;
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: Getting module file name, process handle %p module "
             "%p " G_GUINT32_FORMAT,
...
gboolean mono_w32process_module_get_name(gpointer handle, gpointer module,
                                         gunichar2 **str, guint32 *len) {
  MonoW32Handle *handle_data;
  MonoW32HandleProcess *process_handle;
  pid_t pid;
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: Getting module base name, process handle %p module "
             "%p " G_GUINT32_FORMAT,
             __func__, handle, module);
  ...
  *len = 0;
  ...
  if (!mods && module != NULL) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: Can't get modules %p", __func__, handle);
    g_free(pname);
    mono_w32handle_unref(handle_data);
    return FALSE;
  }
   ...
   * The best we can do for now check it the module name end with the process
   * name.
   */
  for (mods_iter = mods; mods_iter; mods_iter = g_slist_next(mods_iter)) {
    ...
    if (procname_ext == NULL &&
        ((module == NULL &&
          match_procname_to_modulename(pname, found_module->filename)) ||
  ...
  if (procname_ext == NULL) {
    ...
    if (!procname_ext)
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s: Can't find procname_ext from proc_get_name %p pid %d",
                 __func__, handle, pid);
  }
  ...
  if (procname_ext) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: Process name is [%s]", __func__, procname_ext);

    procname = mono_unicode_from_external(procname_ext, &bytes);
    if (procname == NULL) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s: Can't get procname %p", __func__, handle);
      /* bugger */
      g_free(procname_ext);
      mono_w32handle_unref(handle_data);
      return FALSE;
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: Can't find procname_ext %p", __func__, handle);
  mono_w32handle_unref(handle_data);
  return FALSE;
}
...
gboolean mono_w32process_module_get_information(gpointer handle,
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: Getting module info, process handle %p module %p", __func__,
             handle, module);
  ...
  for (mods_iter = mods; mods_iter; mods_iter = g_slist_next(mods_iter)) {
    ...
    if (ret == FALSE &&
      ...
      modinfo->SizeOfImage = (gsize)(found_module->address_end) -
                             (gsize)(found_module->address_start);
...
MONO_SIGNAL_HANDLER_FUNC(static, mono_sigchld_signal_handler,
                         (int _dummy, siginfo_t *info, void *context)) {
  /*
   * Don't want to do any complicated processing here so just wake up the
   * finalizer thread which will call mono_w32process_signal_finished ().
   */
  int old_errno = errno;
 ...
 */
void mono_w32process_signal_finished(void) {
  mono_coop_mutex_lock(&processes_mutex);
...
#if defined(HAVE_FORK) && defined(HAVE_EXECVE)
static void switch_dir_separators(char *path) {
...
static gboolean is_readable_or_executable(const char *prog) {
  struct stat buf;
...
static gboolean is_executable(const char *prog) {
  struct stat buf;
...
static gboolean is_managed_binary(const char *filename) {
  ...
  if ((num_read != 4) || (buffer[0] != 'P') || (buffer[1] != 'E') ||
   ...
   */
  new_offset =
      lseek(file, optional_header_offset + clr_header_offset, SEEK_SET);
 ...
 * in close_my_fds.
 */
static guint32 max_fd_count(void) {
#if defined(_AIX)
  ...
  if (getprocs64(&pe, sizeof(pe), NULL, 0, &p, 1) != -1) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: maximum returned fd in child is %u", __func__,
               pe.pi_maxofile);
...
//         illumos?) or just walk /proc/pid/fd like Linux?
  ...
  if (d) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: opening fd dir failed, using fallback", __func__);
  }
#elif defined(__FreeBSD__)
  ...
  if (kif) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: kinfo_getfile failed, using fallback", __func__);
  }
#elif defined(_AIX)
  struct procentry64 pe;
  ...
  if (!fds) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: fdsinfo alloc failed, using fallback", __func__);
    goto fallback;
  }

  if (getprocs64(&pe, sizeof(pe), fds, sizeof(struct fdsinfo_100K), &p, 1) !=
      -1) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: getprocs64 failed, using fallback", __func__);
  }
fallback:
#endif
...
static gboolean process_create(const gunichar2 *appname,
                               const gunichar2 *cmdline, const gunichar2 *cwd,
                               StartupHandles *startup_handles,
                               MonoW32ProcessInfo *process_info) {
#if defined(HAVE_FORK) && defined(HAVE_EXECVE)
  char *cmd = NULL, *prog = NULL, *full_prog = NULL, *args = NULL,
  ...
  if (cmd != NULL) {
    ...
    if (unquoted[0] == '/') {
      ...
      if (!is_readable_or_executable(prog)) {
        mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                   "%s: Couldn't find executable %s", __func__, prog);
        g_free(unquoted);
    ...
    if (token[0] == '/') {
      ...
      if (!is_readable_or_executable(prog)) {
        mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                   "%s: Couldn't find executable %s", __func__, token);
        g_free(token);
        mono_w32error_set_last(ERROR_FILE_NOT_FOUND);
        goto free_strings;
      }
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: Exec prog [%s] args [%s]", __func__, prog, args_after_prog);
  ...
  if (is_managed_binary(prog)) {
    ...
    newapp = mono_unicode_from_external(cli_launcher ? cli_launcher : "mono",
                                        &bytes_ignored);
    if (newapp) {
      if (appname)
        newcmd =
            utf16_concat(utf16_quote, newapp, utf16_quote, utf16_space, appname,
                         utf16_space, cmdline, (const gunichar2 *)NULL);
      else
        newcmd = utf16_concat(utf16_quote, newapp, utf16_quote, utf16_space,
                              cmdline, (const gunichar2 *)NULL);
    ...
    if (!is_executable(prog)) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s: Executable permisson not set on %s", __func__, prog);
  ...
  if (process_info->env_variables) {
    MonoArrayHandle array =
        MONO_HANDLE_NEW(MonoArray, process_info->env_variables);
    ...
    /* Copy each environ string into 'strings' turning it into utf8 (or the
     * requested encoding) at the same time */
    for (gsize i = 0; i < array_length; ++i) {
      MONO_HANDLE_ARRAY_GETREF(var, array, i);
      gchandle_t gchandle = 0;
      env_strings[i] = mono_unicode_to_external(
          mono_string_handle_pin_chars(var, &gchandle));
  ...
  if (pipe(startup_pipe) == -1) {
    /* Could not create the pipe to synchroniz process startup. We'll just not
     * synchronize. This is just for a very hard to hit race condition in the
     * first place */
    startup_pipe[0] = startup_pipe[1] = -1;
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: new process startup not synchronized. We may not notice if "
               "the newly created process exits immediately.",
               __func__);
  }
...
#ifdef DEBUG_ENABLED
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: exec()ing [%s] in dir [%s]", __func__, cmd,
               dir == NULL ? "." : dir);
...
free_strings:
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
             "%s: returning handle %p for pid %d", __func__, handle, pid);
...
MonoBoolean ves_icall_System_Diagnostics_Process_ShellExecuteEx_internal(
    MonoW32ProcessStartInfoHandle proc_start_info,
    MonoW32ProcessInfo *process_info, MonoError *error) {
   ...
   */
  gunichar2 *args;
  args = utf16_concat(utf16_quote, lpFile, utf16_quote,
                      lpParameters ? utf16_space : NULL, lpParameters,
                      (const gunichar2 *)NULL);
  if (args == NULL) {
  ...
  if (!ret) {
    ...
    // don't try the "open" handlers on iOS/Android, they don't exist there
    // anyway
    goto done;
#endif
    ...
    if (handler != NULL)
      ...
      if (handler == NULL) {
        ...
        if (handler == NULL) {
          ...
          char *old = handler;
          handler = g_strconcat(old, " exec", (const char *)NULL);
          g_free(old);
        }
      }
    }
     ...
     * 371567.
     */
    args = utf16_concat(handler_utf16, utf16_space, utf16_quote, lpFile,
                        utf16_quote, lpParameters ? utf16_space : NULL,
                        lpParameters, (const gunichar2 *)NULL);
    if (args == NULL) {
      mono_w32error_set_last(ERROR_INVALID_DATA);
      ret = FALSE;
      goto done;
    ...
    if (handler_needswait) {
      ...
      waitret = process_wait((MonoW32Handle *)process_info->process_handle,
                             MONO_INFINITE_WAIT, NULL);
      (void)waitret;
      mono_get_exit_code_process(process_info->process_handle, &exitcode);
      if (exitcode != 0)
        ret = FALSE;
    }
    /* Shell exec should not return a process handle when it spawned a GUI
     * thing, like a browser. */
...
static gboolean process_get_complete_path(const gunichar2 *appname,
                                          gchar **completed) {
  char *found = NULL;
  gboolean result = FALSE;
  ...
  if (g_file_test(utf8app, G_FILE_TEST_IS_EXECUTABLE) &&
      !g_file_test(utf8app, G_FILE_TEST_IS_DIR)) {
    *completed = g_shell_quote(utf8app);
    result = TRUE;
    goto exit;
  }
...
static gboolean process_get_shell_arguments(MonoCreateProcessCoop *coop,
...
MonoBoolean ves_icall_System_Diagnostics_Process_CreateProcess_internal(
    MonoW32ProcessStartInfoHandle proc_start_info, HANDLE stdin_handle,
  ...
  /* The default dir name is "".  Turn that into NULL to mean "current
   * directory" */
  gunichar2 *dir;
...
ves_icall_System_Diagnostics_Process_GetProcesses_internal(MonoError *error) {
  ...
  procs = mono_array_new_handle(mono_domain_get(), mono_get_int32_class(),
                                count, error);
  if (!is_ok(error)) {
...
void mono_w32process_set_cli_launcher(gchar *path) {
  g_free(cli_launcher);
  cli_launcher = g_strdup(path);
}

gpointer ves_icall_Microsoft_Win32_NativeMethods_GetCurrentProcess(void) {
  return current_process;
}

MonoBoolean
ves_icall_Microsoft_Win32_NativeMethods_GetExitCodeProcess(gpointer handle,
                                                           gint32 *exitcode) {
...
static MonoBoolean mono_get_exit_code_process(gpointer handle,
  ...
  *exitcode = mono_w32handle_issignalled(handle_data)
                  ? process_handle->exitstatus
                  : STILL_ACTIVE;
...
MonoBoolean
ves_icall_Microsoft_Win32_NativeMethods_TerminateProcess(gpointer handle,
                                                         gint32 exitcode) {
#ifdef HAVE_KILL
  MonoW32Handle *handle_data;
  int ret;
...
MonoBoolean ves_icall_Microsoft_Win32_NativeMethods_GetProcessWorkingSetSize(
    gpointer handle, gsize *min, gsize *max) {
  MonoW32Handle *handle_data;
  MonoW32HandleProcess *process_handle;
...
static void ticks_to_processtime(guint64 ticks, ProcessTime *processtime) {
  processtime->lowDateTime = ticks & 0xFFFFFFFF;
  processtime->highDateTime = ticks >> 32;
}

MonoBoolean ves_icall_Microsoft_Win32_NativeMethods_GetProcessTimes(
    gpointer handle, gint64 *creation_time, gint64 *exit_time,
    gint64 *kernel_time, gint64 *user_time) {
  MonoW32Handle *handle_data;
  MonoW32HandleProcess *process_handle;
  ProcessTime *creation_processtime, *exit_processtime, *kernel_processtime,
      *user_processtime;
  ...
  if (!process_handle->child) {
    ...
    mono_process_get_times(GINT_TO_POINTER(process_handle->pid), &start_ticks,
  ...
  if (process_handle->pid == getpid()) {
    ...
    if (getrusage(RUSAGE_SELF, &time_data) == 0) {
      ticks_to_processtime((guint64)time_data.ru_utime.tv_sec * 10000000 +
                               (guint64)time_data.ru_utime.tv_usec * 10,
                           user_processtime);
      ticks_to_processtime((guint64)time_data.ru_stime.tv_sec * 10000000 +
                               (guint64)time_data.ru_stime.tv_usec * 10,
                           kernel_processtime);
    }
  }
#endif
...
get_enclosing_section_header(guint32 rva, IMAGE_NT_HEADERS32 *nt_headers) {
  ...
  for (i = 0; i < GUINT16_FROM_LE(nt_headers->FileHeader.NumberOfSections);
       i++, section++) {
 ...
 */
static gpointer get_ptr_from_rva(guint32 rva, IMAGE_NT_HEADERS32 *ntheaders,
                                 gpointer file_map) {
...
static gpointer scan_resource_dir(IMAGE_RESOURCE_DIRECTORY *root,
  ...
  if (level == 0) {
     ...
     * resource
     */
    if ((is_string == FALSE && name_offset != res_id) || (is_string == TRUE)) {
      return (NULL);
    }
  } else if (level == 1) {
     ...
     */
    if ((is_string == FALSE && name_offset != lang_id && lang_id != 0) ||
        (is_string == TRUE)) {
      return (NULL);
    }
  } else {
  ...
  if (is_dir == TRUE) {
    IMAGE_RESOURCE_DIRECTORY *res_dir =
        (IMAGE_RESOURCE_DIRECTORY *)((guint8 *)root + dir_offset);
    IMAGE_RESOURCE_DIRECTORY_ENTRY *sub_entries =
        (IMAGE_RESOURCE_DIRECTORY_ENTRY *)(res_dir + 1);
    guint32 entries, i;

    entries = GUINT16_FROM_LE(res_dir->NumberOfNamedEntries) +
              GUINT16_FROM_LE(res_dir->NumberOfIdEntries);

    for (i = 0; i < entries; i++) {
      IMAGE_RESOURCE_DIRECTORY_ENTRY *sub_entry = &sub_entries[i];
      gpointer ret;

      ret = scan_resource_dir(root, nt_headers, file_map, sub_entry, level + 1,
                              res_id, lang_id, size);
      if (ret != NULL) {
        return (ret);
      }
    ...
    IMAGE_RESOURCE_DATA_ENTRY *data_entry =
        (IMAGE_RESOURCE_DATA_ENTRY *)((guint8 *)root + data_offset);
    *size = GUINT32_FROM_LE(data_entry->Size);

    return (get_ptr_from_rva(GUINT32_FROM_LE(data_entry->OffsetToData),
                             nt_headers, file_map));
  }
}
...
static gpointer find_pe_file_resources32(gpointer file_map, guint32 map_size,
  ...
  if (dos_header->e_magic != IMAGE_DOS_SIGNATURE) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: Bad dos signature 0x%x", __func__, dos_header->e_magic);
  ...
  if (map_size <
      sizeof(IMAGE_NT_HEADERS32) + GUINT32_FROM_LE(dos_header->e_lfanew)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: File is too small: %" G_GUINT32_FORMAT, __func__, map_size);

    mono_w32error_set_last(ERROR_BAD_LENGTH);
    return (NULL);
  }

  nt_headers = (IMAGE_NT_HEADERS32 *)((guint8 *)file_map +
                                      GUINT32_FROM_LE(dos_header->e_lfanew));
  if (nt_headers->Signature != IMAGE_NT_SIGNATURE) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: Bad NT signature 0x%x", __func__, nt_headers->Signature);
  ...
  if (nt_headers->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
    /* Do 64-bit stuff */
    resource_rva = GUINT32_FROM_LE(
        ((IMAGE_NT_HEADERS64 *)nt_headers)
            ->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE]
            .VirtualAddress);
  } else {
    resource_rva = GUINT32_FROM_LE(
        nt_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_RESOURCE]
            .VirtualAddress);
  }
  ...
  resource_dir = (IMAGE_RESOURCE_DIRECTORY *)get_ptr_from_rva(
      resource_rva, (IMAGE_NT_HEADERS32 *)nt_headers, file_map);
  ...
  entries = GUINT16_FROM_LE(resource_dir->NumberOfNamedEntries) +
            GUINT16_FROM_LE(resource_dir->NumberOfIdEntries);
  ...
  for (i = 0; i < entries; i++) {
    ...
    ret = scan_resource_dir(resource_dir, (IMAGE_NT_HEADERS32 *)nt_headers,
                            file_map, direntry, 0, res_id, lang_id, size);
    if (ret != NULL) {
      return (ret);
    }
  }
...
static gpointer find_pe_file_resources64(gpointer file_map, guint32 map_size,
  ...
  if (map_size <
      sizeof(IMAGE_NT_HEADERS64) + GUINT32_FROM_LE(dos_header->e_lfanew)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: File is too small: %" G_GUINT32_FORMAT, __func__, map_size);
  ...
  nt_headers = (IMAGE_NT_HEADERS64 *)((guint8 *)file_map +
                                      GUINT32_FROM_LE(dos_header->e_lfanew));
  if (nt_headers->Signature != IMAGE_NT_SIGNATURE) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
               "%s: Bad NT signature 0x%x", __func__, nt_headers->Signature);
...
static gpointer find_pe_file_resources(gpointer file_map, guint32 map_size,
  ...
  if (1) {
    return find_pe_file_resources32(file_map, map_size, res_id, lang_id, size);
  } else {
    return find_pe_file_resources64(file_map, map_size, res_id, lang_id, size);
  }
}
 ...
 */
static gboolean unicode_string_equals(const gunichar2 *str1,
                                      const gchar *str2) {
  while (*str1 && *str2) {
...
static gconstpointer get_string_block(gconstpointer data_ptr,
  ...
  while (((char *)data_ptr - (char *)orig_data_ptr) < data_len) {
    ...
    if (string_key != NULL && string_value != NULL &&
 ...
 */
static gconstpointer get_stringtable_block(gconstpointer data_ptr, gchar *lang,
                                           const gunichar2 *string_key,
                                           gpointer *string_value,
                                           guint32 *string_value_len,
                                           version_data *block) {
  ...
  while (string_len < data_len) {
    ...
    if (lang != NULL && !strcmp(found_lang, lang)) {
      ...
      data_ptr = get_string_block(data_ptr, string_key, string_value,
                                  string_value_len, block);
    } else {
      data_ptr = get_string_block(data_ptr, NULL, NULL, NULL, block);
    }
...
static gconstpointer big_up_string_block(gconstpointer data_ptr,
  ...
  while (((char *)data_ptr - (char *)orig_data_ptr) < data_len) {
    ...
    big_value = g_convert((gchar *)block->key, unicode_chars(block->key) * 2,
                          "UTF-16BE", "UTF-16LE", NULL, NULL, NULL);
    if (big_value == NULL) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
...
static gconstpointer big_up_stringtable_block(gconstpointer data_ptr,
  ...
  while (string_len < data_len) {
    ...
    big_value = g_convert((gchar *)block->key, 16, "UTF-16BE", "UTF-16LE", NULL,
                          NULL, NULL);
    if (big_value == NULL) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_PROCESS,
                 "%s: Didn't find a valid string, giving up", __func__);
...
gboolean mono_w32process_get_fileversion_info(const gunichar2 *filename,
  ...
  *data = NULL;
  ...
  versioninfo =
      find_pe_file_resources(file_map, map_size, RT_VERSION, 0, &datasize);
  if (!versioninfo) {
  ...
  *data = g_malloc0(datasize);
   ...
   * mono_w32process_ver_query_value() doesn't have to follow the data blocks
   * every time. But hey, these functions aren't likely to appear in many
   * profiles. */
...
gboolean mono_w32process_ver_query_value(gconstpointer datablock,
                                         const gunichar2 *subblock,
    ...
    if (data_ptr != NULL) {
      ...
      while (data_len > 0) {
        ...
        if (unicode_string_equals(block.key, "VarFileInfo")) {
          ...
          data_ptr =
              get_stringtable_block(data_ptr, lang_utf8, string_key,
                                    &string_value, &string_value_len, &block);
          if (want_string && string_value != NULL && string_value_len != 0) {
            *buffer = string_value;
            *len = unicode_chars((const gunichar2 *)string_value) +
...
guint32 mono_w32process_ver_language_name(guint32 lang, gunichar2 *lang_out,
                                          guint32 lang_len) {
...
void mono_w32process_cleanup(void) {}
