...
typedef enum {
  ...
  // These values match System.Diagnostics.ThreadPriorityLevel and Windows, but
  // are offset by 2.
...
mono_thread_create_internal(MonoDomain *domain, T func, gpointer arg,
...
mono_thread_create_internal_handle(MonoDomain *domain, T func, gpointer arg,
                                   MonoThreadCreateFlags flags,
                                   MonoError *error) {
  return mono_thread_create_internal_handle(domain, (gpointer)func, arg, flags,
                                            error);
}
#endif
...
ICALL_EXPORT
void ves_icall_System_Threading_Thread_SetName_internal(
    MonoInternalThread *this_obj, MonoString *name);

ICALL_EXPORT
MonoObject *ves_icall_System_Threading_Thread_GetCachedCurrentCulture(
...
ICALL_EXPORT
MonoObject *ves_icall_System_Threading_Thread_GetCachedCurrentUICulture(
    MonoInternalThread *this_obj);
...
void mono_thread_set_name_windows(HANDLE thread_handle, PCWSTR thread_name);
...
MONO_PROFILER_API
void mono_thread_set_name(MonoInternalThread *thread, const char *name8,
...
MonoExceptionHandle mono_thread_interruption_checkpoint_handle(void);
...
uint32_t mono_alloc_special_static_data(uint32_t static_type, uint32_t size,
                                        uint32_t align, uintptr_t *bitmap,
                                        int numbits);
...
#ifdef __cplusplus
template <typename T>
inline gboolean mono_thread_create_checked(MonoDomain *domain, T func,
                                           gpointer arg, MonoError *error) {
...
MONO_API gpointer mono_threads_attach_coop(MonoDomain *domain, gpointer *dummy);

MONO_API void mono_threads_detach_coop(gpointer cookie, gpointer *dummy);

MonoDomain *mono_threads_attach_coop_internal(MonoDomain *domain,
                                              gpointer *cookie,
                                              MonoStackData *stackdata);

void mono_threads_detach_coop_internal(MonoDomain *orig_domain, gpointer cookie,
                                       MonoStackData *stackdata);
...
void mono_thread_internal_describe(MonoInternalThread *internal, GString *str);
...
void mono_set_thread_dump_dir(gchar *dir);
