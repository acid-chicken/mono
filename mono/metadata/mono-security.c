...
static gchar *GetTokenName(uid_t uid) {
  gchar *uname = NULL;
...
static gboolean IsMemberInList(uid_t user, struct group *g) {
...
static gboolean IsDefaultGroup(uid_t user, gid_t group) {
  gboolean result = FALSE;
...
static gboolean IsMemberOf(gid_t user, struct group *g) {
  if (!g)
    return FALSE;
...
static gint32 internal_get_token_name(gpointer token, gunichar2 **uniname) {
  gint32 size = 0;
...
ves_icall_System_Security_Principal_WindowsPrincipal_IsMemberOfGroupName(
    gpointer user, const gchar *utf8_groupname, MonoError *error) {
  gboolean result = FALSE;
...
#ifndef HOST_WIN32
static gboolean IsProtected(const gunichar2 *path, gint32 protection) {
  gboolean result = FALSE;
...
static gboolean Protect(const gunichar2 *path, gint32 file_mode,
                        gint32 add_dir_mode) {
  gboolean result = FALSE;
  gchar *utf8_name = mono_unicode_to_external(path);
  if (utf8_name) {
...
ves_icall_Mono_Security_Cryptography_KeyPairPersistence_IsUserProtected(
  ...
  ret = IsProtected(
      path, (S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IWOTH | S_IXOTH));
  return (MonoBoolean)ret;
}
...
ves_icall_Mono_Security_Cryptography_KeyPairPersistence_ProtectMachine(
  ...
  ret = Protect(path, (S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH),
 ...
 */
MonoBoolean ves_icall_System_Security_Policy_Evidence_IsAuthenticodePresent(
    MonoReflectionAssemblyHandle refass, MonoError *error) {
  error_init(error);
...
static void mono_invoke_protected_memory_method(MonoArrayHandle data,
                                                MonoObjectHandle scope,
  ...
  if (!*method) {
    ...
    if (system_security_assembly == NULL) {
      system_security_assembly =
          mono_image_loaded_internal(alc, "System.Security", FALSE);
      if (!system_security_assembly) {
        MonoAssemblyOpenRequest req;
        mono_assembly_request_prepare_open(&req, MONO_ASMCTX_DEFAULT, alc);
        MonoAssembly *sa =
            mono_assembly_request_open("System.Security.dll", &req, NULL);
        g_assert(sa);
    ...
    MonoClass *klass = mono_class_load_from_name(system_security_assembly,
                                                 ...
                                                 "ProtectedMemory");
    *method = mono_class_get_method_from_name_checked(klass, method_name, 2, 0,
                                                      error);
...
void ves_icall_System_Security_SecureString_DecryptInternal(
    MonoArrayHandle data, MonoObjectHandle scope, MonoError *error) {
  mono_invoke_protected_memory_method(data, scope, "Unprotect",
                                      &mono_method_securestring_decrypt, error);
}
void ves_icall_System_Security_SecureString_EncryptInternal(
    MonoArrayHandle data, MonoObjectHandle scope, MonoError *error) {
  mono_invoke_protected_memory_method(data, scope, "Protect",
                                      &mono_method_securestring_encrypt, error);
}
