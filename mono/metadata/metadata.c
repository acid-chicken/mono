...
const char *mono_metadata_string_heap_checked(MonoImage *meta, guint32 index,
  ...
  if (G_UNLIKELY(!(index < meta->heap_strings.size))) {
    ...
    mono_error_set_bad_image_by_name(
        error, image_name, "string heap index %ud out bounds %u: %s", index,
        meta->heap_strings.size, image_name);
    return NULL;
  }
  return meta->heap_strings.data + index;
}
...
const char *mono_metadata_blob_heap_checked(MonoImage *meta, guint32 index,
  ...
  if (G_UNLIKELY(!(index < meta->heap_blob.size))) {
    ...
    mono_error_set_bad_image_by_name(error, image_name,
                                     "blob heap index %u out of bounds %u: %s",
                                     index, meta->heap_blob.size, image_name);
    return NULL;
  }
  return meta->heap_blob.data + index;
}
...
gboolean mono_metadata_decode_row_checked(const MonoImage *image,
  ...
  if (G_UNLIKELY(!(idx < t->rows && idx >= 0))) {
    mono_error_set_bad_image_by_name(error, image_name,
                                     "row index %d out of bounds: %d rows: %s",
                                     idx, t->rows, image_name);
    return FALSE;
  }
...
MonoImageSet *mono_metadata_merge_image_sets(MonoImageSet *set1,
                                             MonoImageSet *set2) {
