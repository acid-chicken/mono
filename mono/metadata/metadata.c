...
static gboolean do_mono_metadata_parse_type(MonoType *type, MonoImage *m,
                                            ...
                                            gboolean transient, const char *ptr,
                                            const char **rptr,
                                            MonoError *error);

static gboolean do_mono_metadata_type_equal(MonoType *t1, MonoType *t2,
                                            gboolean signature_only);
static gboolean mono_metadata_class_equal(MonoClass *c1, MonoClass *c2,
                                          gboolean signature_only);
static gboolean mono_metadata_fnptr_equal(MonoMethodSignature *s1,
                                          MonoMethodSignature *s2,
                                          gboolean signature_only);
static gboolean _mono_metadata_generic_class_equal(const MonoGenericClass *g1,
                                                   const MonoGenericClass *g2,
                                                   gboolean signature_only);
...
static void mono_metadata_field_info_full(MonoImage *meta, guint32 index,
                                          guint32 *offset, guint32 *rva,
                                          MonoMarshalSpec **marshal_spec,
                                          gboolean alloc_from_image);

static MonoType *mono_signature_get_params_internal(MonoMethodSignature *sig,
                                                    gpointer *iter);
...
enum {
  ...
  /* MemberRefParent coded index: typeref, moduleref, method, memberref,
     typesepc, typedef */
  MONO_MT_MRP_IDX,
...
const static unsigned char table_description[] = {
...
/* On legacy, if TRUE (but also see DISABLE_DESKTOP_LOADER #define), Mono will
 * check that the public key token, culture and version of a candidate assembly
 * matches the requested strong name. On netcore, it will check the culture and
 * version. If FALSE, as long as the name matches, the candidate will be
 * allowed.
 */
...
If  e is a coded token(see clause 23.1.7) that points into table ti out of n
possible tables t0, .. tn-1, then it is stored as e << (log n) & tag{ t0, ..
tn-1}[ ti] using 2 bytes if the maximum number of rows of tables t0, ..tn-1, is
less than 2^16 - (log n), and using 4 bytes otherwise. The family of finite maps
tag{ t0, ..tn-1} is defined below. Note that to decode a physical row, you need
the inverse of this mapping.

 */
#define rtsize(meta, s, b) (((s) < (1 << (b)) ? 2 : 4))

static int idx_size(MonoImage *meta, int idx) {
  if (meta->referenced_tables &&
      (meta->referenced_tables & ((guint64)1 << idx)))
    return meta->referenced_table_rows[idx] < 65536 ? 2 : 4;
  else
    return meta->tables[idx].rows < 65536 ? 2 : 4;
}

static int get_nrows(MonoImage *meta, int idx) {
  if (meta->referenced_tables &&
      (meta->referenced_tables & ((guint64)1 << idx)))
 ...
 * \p result_bitfield return value along with the number of columns in the
 * table. the resulting bitfield should be handed to the \c
 * mono_metadata_table_size and \c mono_metadata_table_count macros. This is a
 * Mono runtime internal only function.
 */
int mono_metadata_compute_size(MonoImage *meta, int tableindex,
  ...
  const unsigned char *description =
      TableSchemas + table_description[tableindex];
  ...
  for (i = 0; (code = description[i]) != MONO_MT_END; i++) {
    ...
    case MONO_MT_BLOB_IDX:
      field_size = meta->idx_blob_wide ? 4 : 2;
      break;

    case MONO_MT_STRING_IDX:
      field_size = meta->idx_string_wide ? 4 : 2;
      break;

    case MONO_MT_GUID_IDX:
      field_size = meta->idx_guid_wide ? 4 : 2;
      break;
    ...
    case MONO_MT_TABLE_IDX:
      ...
      case MONO_TABLE_ASSEMBLYREFOS:
        g_assert(i == 3);
        field_size = idx_size(meta, MONO_TABLE_ASSEMBLYREF);
        break;
      ...
      case MONO_TABLE_CLASSLAYOUT:
        g_assert(i == 2);
        field_size = idx_size(meta, MONO_TABLE_TYPEDEF);
        break;
      ...
      case MONO_TABLE_EVENT_POINTER:
        g_assert(i == 0);
        field_size = idx_size(meta, MONO_TABLE_EVENT);
        break;
      ...
      case MONO_TABLE_FIELDLAYOUT:
        g_assert(i == 1);
        field_size = idx_size(meta, MONO_TABLE_FIELD);
        break;
      ...
      case MONO_TABLE_IMPLMAP:
        g_assert(i == 3);
        field_size = idx_size(meta, MONO_TABLE_MODULEREF);
        break;
      ...
      case MONO_TABLE_METHOD:
        g_assert(i == 5);
        field_size = idx_size(meta, MONO_TABLE_PARAM);
        break;
      ...
      case MONO_TABLE_PROPERTY_POINTER:
        g_assert(i == 0);
        field_size = idx_size(meta, MONO_TABLE_PROPERTY);
        break;
      ...
      case MONO_TABLE_STATEMACHINEMETHOD:
        ...
        break;
      default:
        g_error("Can't handle MONO_MT_TABLE_IDX for table %d element %d",
                tableindex, i);
      }
      break;
    ...
    /*
     * MemberRefParent: TypeDef, TypeRef, MethodDef, ModuleRef, TypeSpec,
     * MemberRef
     */
...
void mono_metadata_compute_table_bases(MonoImage *meta) {
  ...
  for (i = 0; i < MONO_TABLE_NUM; i++) {
    ...
    table->row_size =
        mono_metadata_compute_size(meta, i, &table->size_bitfield);
    table->base = base;
 ...
 */
const char *mono_metadata_string_heap_checked(MonoImage *meta, guint32 index,
                                              MonoError *error) {
  if (G_UNLIKELY(!(index < meta->heap_strings.size))) {
    mono_error_set_bad_image_by_name(
        error, meta->name ? meta->name : "unknown image",
        "string heap index %ud out bounds %u", index, meta->heap_strings.size);
    return NULL;
  }
  return meta->heap_strings.data + index;
}
 ...
 * \returns an in-memory pointer to the \p index in the user string heap
 * (<code>#US</code>).
 */
 ...
 * If the Blob heap is empty or missing and index is 0 returns NULL, instead of
 * asserting.
 */
 ...
 * \param index index into the blob.
 * \param error set on error
 * \returns an in-memory pointer to the \p index in the Blob heap.  On failure
 * sets \p error and returns NULL; If the Blob heap is empty or missing and \p
 * index is 0 returns NULL, without setting error.
 *
 */
const char *mono_metadata_blob_heap_checked(MonoImage *meta, guint32 index,
                                            MonoError *error) {
  if (G_UNLIKELY(index == 0 && meta->heap_blob.size == 0))
    return NULL;
  if (G_UNLIKELY(!(index < meta->heap_blob.size))) {
    mono_error_set_bad_image_by_name(
        error, meta->name ? meta->name : "unknown image",
        "blob heap index %u out of bounds %u", index, meta->heap_blob.size);
    return NULL;
  }
  return meta->heap_blob.data + index;
}
 ...
 */
void mono_metadata_decode_row(const MonoTableInfo *t, int idx, guint32 *res,
                              int res_size) {
 ...
 * into the \c guint32 \p res array that has \p res_size elements.
 *
 * \returns TRUE if the read succeeded. Otherwise sets \p error and returns
 * FALSE.
 */
gboolean mono_metadata_decode_row_checked(const MonoImage *image,
                                          const MonoTableInfo *t, int idx,
                                          guint32 *res, int res_size,
                                          MonoError *error) {
  guint32 bitfield = t->size_bitfield;
  int i, count = mono_metadata_table_count(bitfield);
  ...
  if (G_UNLIKELY(!(idx < t->rows && idx >= 0))) {
    mono_error_set_bad_image_by_name(
        error, image_name, "row index %d out of bounds: %d rows", idx, t->rows);
    return FALSE;
  }
  ...
  if (G_UNLIKELY(res_size != count)) {
    mono_error_set_bad_image_by_name(
        error, image_name, "res_size %d != count %d", res_size, count);
    return FALSE;
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    case 4:
      ...
      break;
    default:
      mono_error_set_bad_image_by_name(error, image_name,
                                       "unexpected table [%d] size %d", i, n);
      return FALSE;
    }
    data += n;
  }
 ...
 */
guint32 mono_metadata_decode_row_col(const MonoTableInfo *t, int idx,
                                     guint col) {
  guint32 bitfield = t->size_bitfield;
  int i;
  const char *data;
 ...
 *
 * This decodes a compressed size as described by 24.2.4 (#US and #Blob a blob
 * or user string object)
 *
...
guint32 mono_metadata_decode_blob_size(const char *xptr, const char **rptr) {
  ...
  if ((*ptr & 0x80) == 0) {
    ...
    size = ((ptr[0] & 0x1f) << 24) + (ptr[1] << 16) + (ptr[2] << 8) + ptr[3];
    ptr += 4;
  }
  if (rptr)
    *rptr = (char *)ptr;
...
guint32 mono_metadata_decode_value(const char *_ptr, const char **rptr) {
  ...
  if ((b & 0x80) == 0) {
    ...
    len = ((b & 0x1f) << 24) | (ptr[1] << 16) | (ptr[2] << 8) | ptr[3];
    ptr += 4;
  }
  if (rptr)
    *rptr = (char *)ptr;
...
gint32 mono_metadata_decode_signed_value(const char *ptr, const char **rptr) {
  ...
  if (ival < 0x40)
    /* 6 bits = 7 bits for compressed representation (top bit is '0') - 1 sign
     * bit */
    return ival - 0x40;
  if (ival < 0x2000)
    /* 13 bits = 14 bits for compressed representation (top bits are '10') - 1
     * sign bit */
    return ival - 0x2000;
  if (ival < 0x10000000)
    /* 28 bits = 29 bits for compressed representation (top bits are '110') - 1
     * sign bit */
    return ival - 0x10000000;
  g_assert(ival < 0x20000000);
  g_warning("compressed signed value appears to use 29 bits for compressed "
            "representation: %x (raw: %8x)",
            ival, uval);
 ...
 * Translates the given 1-based index into the \c Method, \c Field, \c Event, or
 * \c Param tables using the \c *Ptr tables in uncompressed metadata, if they
 * are available.
 *
 * FIXME: The caller is not forced to call this function, which is error-prone,
 * since forgetting to call it would only show up as a bug on uncompressed
 * metadata.
 */
guint32 mono_metadata_translate_token_index(MonoImage *image, int table,
                                            guint32 idx) {
  ...
  case MONO_TABLE_METHOD:
    if (image->tables[MONO_TABLE_METHOD_POINTER].rows)
      return mono_metadata_decode_row_col(
          &image->tables[MONO_TABLE_METHOD_POINTER], idx - 1,
  ...
  case MONO_TABLE_FIELD:
    if (image->tables[MONO_TABLE_FIELD_POINTER].rows)
      return mono_metadata_decode_row_col(
          &image->tables[MONO_TABLE_FIELD_POINTER], idx - 1,
  ...
  case MONO_TABLE_EVENT:
    if (image->tables[MONO_TABLE_EVENT_POINTER].rows)
      return mono_metadata_decode_row_col(
          &image->tables[MONO_TABLE_EVENT_POINTER], idx - 1,
  ...
  case MONO_TABLE_PROPERTY:
    if (image->tables[MONO_TABLE_PROPERTY_POINTER].rows)
      return mono_metadata_decode_row_col(
          &image->tables[MONO_TABLE_PROPERTY_POINTER], idx - 1,
  ...
  case MONO_TABLE_PARAM:
    if (image->tables[MONO_TABLE_PARAM_POINTER].rows)
      return mono_metadata_decode_row_col(
          &image->tables[MONO_TABLE_PARAM_POINTER], idx - 1,
 ...
 *
 * Same as \c mono_metadata_decode_row, but takes an \p image + \p table ID
 * pair, and takes uncompressed metadata into account, so it should be used to
 * access the \c Method, \c Field, \c Param and \c Event tables when the access
 * is made from metadata, i.e. \p idx is retrieved from a metadata table, like
 * \c MONO_TYPEDEF_FIELD_LIST.
 */
void mono_metadata_decode_table_row(MonoImage *image, int table, int idx,
                                    guint32 *res, int res_size) {
  if (image->uncompressed_metadata)
 ...
 *
 * Same as \c mono_metadata_decode_row_col, but takes an \p image + \p table ID
 * pair, and takes uncompressed metadata into account, so it should be used to
 * access the \c Method, \c Field, \c Param and \c Event tables.
 */
guint32 mono_metadata_decode_table_row_col(MonoImage *image, int table, int idx,
                                           guint col) {
  if (image->uncompressed_metadata)
    idx = mono_metadata_translate_token_index(image, table, idx + 1) - 1;
 ...
 */
guint32 mono_metadata_parse_typedef_or_ref(MonoImage *m, const char *ptr,
                                           const char **rptr) {
  guint32 token;
 ...
 * \param m a metadata context.
 * \param dest storage where the info about the custom modifier is stored (may
 * be NULL) \param ptr a pointer to (possibly) the start of a custom modifier
 * list \param rptr pointer updated to match the end of the decoded stream
 *
 ...
 */
int mono_metadata_parse_custom_mod(MonoImage *m, MonoCustomMod *dest,
 ...
 * and dimensions. Memory is allocated from the heap or from the image mempool,
 * depending on the value of @transient.
 *
 * LOCKING: Acquires the loader lock
 */
static MonoArrayType *mono_metadata_parse_array_internal(
    MonoImage *m, MonoGenericContainer *container, gboolean transient,
    const char *ptr, const char **rptr, MonoError *error) {
  int i;
  MonoArrayType *array;
  MonoType *etype;

  etype = mono_metadata_parse_type_checked(
      m, container, 0, FALSE, ptr, &ptr,
      error); // FIXME this doesn't respect @transient
  if (!etype)
    return NULL;

  array = transient
              ? (MonoArrayType *)g_malloc0(sizeof(MonoArrayType))
              : (MonoArrayType *)mono_image_alloc0(m, sizeof(MonoArrayType));
  ...
  if (array->numsizes)
    array->sizes =
        transient ? (int *)g_malloc0(sizeof(int) * array->numsizes)
                  : (int *)mono_image_alloc0(m, sizeof(int) * array->numsizes);
  ...
  if (array->numlobounds)
    array->lobounds =
        transient
            ? (int *)g_malloc0(sizeof(int) * array->numlobounds)
            : (int *)mono_image_alloc0(m, sizeof(int) * array->numlobounds);
 ...
 */
MonoArrayType *mono_metadata_parse_array(MonoImage *m, const char *ptr,
                                         const char **rptr) {
  ERROR_DECL(error);
  MonoArrayType *ret =
      mono_metadata_parse_array_internal(m, NULL, FALSE, ptr, rptr, error);
 ...
 */
static const MonoType builtin_types[] = {
...
/*
 * MonoTypes with modifies are never cached, so we never check or use that
 * field.
 */
static guint mono_type_hash(gconstpointer data) {
...
static gint mono_type_equal(gconstpointer ka, gconstpointer kb) {
  const MonoType *a = (const MonoType *)ka;
  const MonoType *b = (const MonoType *)kb;

  if (a->type != b->type || a->byref != b->byref || a->attrs != b->attrs ||
      a->pinned != b->pinned)
    return 0;
  /* need other checks */
  return 1;
...
static gboolean mono_generic_inst_equal_full(const MonoGenericInst *a,
                                             const MonoGenericInst *b,
                                             gboolean signature_only) {
  int i;

  // An optimization: if the ids of two insts are the same, we know they are the
  // same inst and don't check contents. Furthermore, because we perform early
  // de-duping, if the ids differ, we know the contents differ.
#ifndef MONO_SMALL_CONFIG // Optimization does not work in MONO_SMALL_CONFIG:
                          // There are no IDs
  if (a->id && b->id) {   // "id 0" means "object has no id"-- de-duping hasn't
                          // been performed yet, must check contents.
    if (a->id == b->id)
      return TRUE;
    // In signature-comparison mode id equality implies object equality, but
    // this is not true for inequality. Two separate objects could have
    // signature-equavalent contents.
    if (!signature_only)
      return FALSE;
  }
#endif
  ...
  for (i = 0; i < a->type_argc; ++i) {
    if (!do_mono_metadata_type_equal(a->type_argv[i], b->type_argv[i],
                                     signature_only))
      return FALSE;
  }
  return TRUE;
...
static guint mono_generic_class_hash(gconstpointer data) {
  ...
  guint hash =
      mono_metadata_type_hash(m_class_get_byval_arg(gclass->container_class));
...
static gboolean mono_generic_class_equal(gconstpointer ka, gconstpointer kb) {
...
void mono_metadata_init(void) {
  ...
  /* We guard against double initialization due to how pedump in verification
  mode works. Until runtime initialization is properly factored to work with
  what it needs we need workarounds like this.
  ...
  */
  ...
  for (i = 0; i < NBUILTIN_TYPES(); ++i)
    g_hash_table_insert(type_cache, (gpointer)&builtin_types[i],
                        (gpointer)&builtin_types[i]);

  mono_os_mutex_init_recursive(&image_sets_mutex);

  mono_counters_register("ImgSet Cache Hit",
                         MONO_COUNTER_METADATA | MONO_COUNTER_INT,
                         &img_set_cache_hit);
  mono_counters_register("ImgSet Cache Miss",
                         MONO_COUNTER_METADATA | MONO_COUNTER_INT,
                         &img_set_cache_miss);
  mono_counters_register(
      "ImgSet Count", MONO_COUNTER_METADATA | MONO_COUNTER_INT, &img_set_count);
}
 ...
 * This is a Mono runtime internal function.
 */
void mono_metadata_cleanup(void) {
 ...
 * \param rptr pointer updated to match the end of the decoded stream
 * \param transient whenever to allocate the result from the heap or from a
 * mempool
 *
 * Decode a compressed type description found at \p ptr in \p m .
 * \p mode can be one of \c MONO_PARSE_MOD_TYPE, \c MONO_PARSE_PARAM, \c
 * MONO_PARSE_RET, \c MONO_PARSE_FIELD, \c MONO_PARSE_LOCAL, \c MONO_PARSE_TYPE.
 ...
 * (the \c generic_container field in the <code>MonoClass</code>) or the current
 * generic method's (stored in <code>image->property_hash</code>) generic
 * container. When we encounter a \c MONO_TYPE_VAR or \c MONO_TYPE_MVAR, it's
 * looked up in this \c MonoGenericContainer.
 *
 * LOCKING: Acquires the loader lock.
 *
 * \returns a \c MonoType structure representing the decoded type.
 */
static MonoType *mono_metadata_parse_type_internal(
    MonoImage *m, MonoGenericContainer *container, short opt_attrs,
  ...
  if (count) { // There are mods, so the MonoType will be of nonstandard size.
    if (count > 64) {
      mono_error_set_bad_image(error, m,
                               "Invalid type with more than 64 modifiers");
      return NULL;
    }

    size_t size = mono_sizeof_type_with_mods(count, FALSE);
    type = transient ? (MonoType *)g_malloc0(size)
                     : (MonoType *)mono_image_alloc0(m, size);
    type->has_cmods = TRUE;
   ...
   * "int32 modopt (Foo) modopt (Bar)" is encoded as "cmod_opt [typedef_or_ref
   * "Bar"] cmod_opt [typedef_or_ref "Foo"] I4"
   */
  while (found) {
    switch (*ptr) {
    case MONO_TYPE_PINNED:
    ...
    case MONO_TYPE_CMOD_REQD:
    case MONO_TYPE_CMOD_OPT:
      mono_metadata_parse_custom_mod(m, &(cmods->modifiers[--count]), ptr,
                                     &ptr);
      break;
    default:
      found = FALSE;
  ...
  // either there were no cmods, or else we iterated through all of cmods
  // backwards to populate it.
  g_assert(count == 0);
  ...
  // Possibly we can return an already-allocated type instead of the one we
  // decoded
  if (!type->has_cmods && !transient) {
    ...
    if ((type->type == MONO_TYPE_CLASS || type->type == MONO_TYPE_VALUETYPE) &&
        !type->pinned && !type->attrs) {
      MonoType *ret = type->byref ? m_class_get_this_arg(type->data.klass)
      ...
      /* Consider the case:
         ...
         When Foo<Test> is being expanded, 'Test' isn't yet initialized.  It's
         actually in a really pristine state: it doesn't even know whether
         'Test' is a reference or a value type.
         ...
         If we can't canonicalize 'type', it doesn't matter, since later users
         of 'type' will do it.

         LOCKING: even though we don't explicitly hold a lock, in the
         problematic case 'ret' is a field of a MonoClass which currently holds
         the loader lock.  'type' is local.
      */
  ...
  /* printf ("%x %x %c %s\n", type->attrs, type->num_mods, type->pinned ? 'p' :
   * ' ', mono_type_full_name (type)); */

  if (type == &stype) { // Type was allocated on the stack, so we need to copy
                        // it to safety
    type = transient ? (MonoType *)g_malloc(MONO_SIZEOF_TYPE)
                     : (MonoType *)mono_image_alloc(m, MONO_SIZEOF_TYPE);
...
MonoType *mono_metadata_parse_type_checked(MonoImage *m,
                                           MonoGenericContainer *container,
                                           short opt_attrs, gboolean transient,
                                           const char *ptr, const char **rptr,
                                           MonoError *error) {
  return mono_metadata_parse_type_internal(m, container, opt_attrs, transient,
                                           ptr, rptr, error);
}
 ...
 * LOCKING: Acquires the loader lock.
 */
MonoType *mono_metadata_parse_type(MonoImage *m, MonoParseTypeMode mode,
                                   short opt_attrs, const char *ptr,
                                   const char **rptr) {
  ERROR_DECL(error);
  MonoType *type = mono_metadata_parse_type_internal(m, NULL, opt_attrs, FALSE,
                                                     ptr, rptr, error);
  mono_error_cleanup(error);
  return type;
}
...
gboolean mono_metadata_method_has_param_attrs(MonoImage *m, int def) {
  ...
  guint lastp, i,
      param_index =
          mono_metadata_decode_row_col(methodt, def - 1, MONO_METHOD_PARAMLIST);
  ...
  for (i = param_index; i < lastp; ++i) {
    guint32 flags =
        mono_metadata_decode_row_col(paramt, i - 1, MONO_PARAM_FLAGS);
    if (flags)
      return TRUE;
  }
 ...
 *
 *   Return the parameter attributes for the method whose MethodDef index is
 * DEF. The returned memory needs to be freed by the caller. If all the param
 * attributes are 0, then NULL is returned.
 */
...
MonoMethodSignature *mono_metadata_parse_signature_checked(MonoImage *image,
  ...
  return mono_metadata_parse_method_signature_full(image, NULL, 0, ptr, NULL,
                                                   error);
}
 ...
 *
 * Allocate a \c MonoMethodSignature structure with the specified number of
 * params. The return type and the params types need to be filled later. This is
 * a Mono runtime internal function.
 *
 * LOCKING: Assumes the loader lock is held.
 *
 * \returns the new \c MonoMethodSignature structure.
 */
MonoMethodSignature *mono_metadata_signature_alloc(MonoImage *m,
                                                   guint32 nparams) {
  MonoMethodSignature *sig;

  sig = (MonoMethodSignature *)mono_image_alloc0(
      m, MONO_SIZEOF_METHOD_SIGNATURE + ((gint32)nparams) * sizeof(MonoType *));
...
static MonoMethodSignature *mono_metadata_signature_dup_internal_with_padding(
    MonoImage *image, MonoMemPool *mp, MonoMethodSignature *sig,
    size_t padding) {
  int sigsize, sig_header_size;
  MonoMethodSignature *ret;
  sigsize = sig_header_size = MONO_SIZEOF_METHOD_SIGNATURE +
                              sig->param_count * sizeof(MonoType *) + padding;
  if (sig->ret)
...
mono_metadata_signature_dup_internal(MonoImage *image, MonoMemPool *mp,
                                     MonoMethodSignature *sig) {
 ...
 */
MonoMethodSignature *
mono_metadata_signature_dup_add_this(MonoImage *image, MonoMethodSignature *sig,
                                     MonoClass *klass) {
  MonoMethodSignature *ret;
  ret = mono_metadata_signature_dup_internal_with_padding(image, NULL, sig,
                                                          sizeof(MonoType *));
  ...
  ret->params[0] = m_class_is_valuetype(klass) ? m_class_get_this_arg(klass)
                                               : m_class_get_byval_arg(klass);

  for (int i = sig->param_count - 1; i >= 0; i--)
    g_assert(ret->params[i + 1]->type == sig->params[i]->type &&
             ret->params[i + 1]->type != MONO_TYPE_END);
...
mono_metadata_signature_dup_full(MonoImage *image, MonoMethodSignature *sig) {
  MonoMethodSignature *ret =
      mono_metadata_signature_dup_internal(image, NULL, sig);
 ...
 */
MonoMethodSignature *mono_metadata_parse_method_signature_full(
    MonoImage *m, MonoGenericContainer *container, int def, const char *ptr,
    const char **rptr, MonoError *error) {
  ...
  if (def)
    pattrs = mono_metadata_get_param_attrs(
        m, def, param_count + 1); /*Must be + 1 since signature's param count
                                     doesn't account for the return value */
  ...
  if (call_convention != 0xa) {
    method->ret = mono_metadata_parse_type_checked(
        m, container, pattrs ? pattrs[0] : 0, FALSE, ptr, &ptr, error);
    if (!method->ret) {
  ...
  for (i = 0; i < method->param_count; ++i) {
    ...
    method->params[i] = mono_metadata_parse_type_checked(
        m, container, pattrs ? pattrs[i + 1] : 0, FALSE, ptr, &ptr, error);
    if (!method->params[i]) {
      mono_metadata_free_method_signature(method);
      g_free(pattrs);
      return NULL;
  ...
  /* The sentinel could be missing if the caller does not pass any additional
   * arguments */
  if (!def && method->call_convention == MONO_CALL_VARARG &&
      method->sentinelpos < 0)
 ...
 * \returns a \c MonoMethodSignature describing the signature.
 */
MonoMethodSignature *mono_metadata_parse_method_signature(MonoImage *m, int def,
                                                          const char *ptr,
                                                          const char **rptr) {
  /*
   * This function MUST NOT be called by runtime code as it does error handling
   * incorrectly. Use mono_metadata_parse_method_signature_full instead. It's ok
   * to asser on failure as we no longer use it.
   */
  ERROR_DECL(error);
  MonoMethodSignature *ret;
  ret =
      mono_metadata_parse_method_signature_full(m, NULL, def, ptr, rptr, error);
...
void mono_metadata_free_inflated_signature(MonoMethodSignature *sig) {
  int i;
...
static gboolean inflated_method_equal(gconstpointer a, gconstpointer b) {
...
static guint inflated_method_hash(gconstpointer a) {
  const MonoMethodInflated *ma = (const MonoMethodInflated *)a;
  return (mono_metadata_generic_context_hash(&ma->context) ^
          mono_aligned_addr_hash(ma->declaring));
}

static gboolean inflated_signature_equal(gconstpointer a, gconstpointer b) {
  const MonoInflatedMethodSignature *sig1 =
      (const MonoInflatedMethodSignature *)a;
  const MonoInflatedMethodSignature *sig2 =
      (const MonoInflatedMethodSignature *)b;
...
static guint inflated_signature_hash(gconstpointer a) {
  const MonoInflatedMethodSignature *sig =
      (const MonoInflatedMethodSignature *)a;

  /* sig->sig is assumed to be canonized */
  return mono_metadata_generic_context_hash(&sig->context) ^
         mono_aligned_addr_hash(sig->sig);
}
...
static gboolean aggregate_modifiers_equal(gconstpointer ka, gconstpointer kb) {
  ...
  for (int i = 0; i < amods1->count; ++i) {
    if (amods1->modifiers[i].required != amods2->modifiers[i].required)
      return FALSE;
    if (!mono_metadata_type_equal_full(amods1->modifiers[i].type,
                                       amods2->modifiers[i].type, TRUE))
      return FALSE;
  }
  return TRUE;
}

static guint aggregate_modifiers_hash(gconstpointer a) {
...
static gboolean aggregate_modifiers_in_image(MonoAggregateModContainer *amods,
                                             MonoImage *image);

static gboolean signature_in_image(MonoMethodSignature *sig, MonoImage *image) {
  gpointer iter = NULL;
  MonoType *p;
...
static gboolean ginst_in_image(MonoGenericInst *ginst, MonoImage *image) {
  int i;
...
static gboolean gclass_in_image(MonoGenericClass *gclass, MonoImage *image) {
...
retry:
  ...
  default:
    /* At this point, we should've avoided all potential allocations in
...
gboolean aggregate_modifiers_in_image(MonoAggregateModContainer *amods,
                                      MonoImage *image) {
  for (int i = 0; i < amods->count; i++)
...
static void image_sets_lock(void) { mono_os_mutex_lock(&image_sets_mutex); }

static void image_sets_unlock(void) { mono_os_mutex_unlock(&image_sets_mutex); }

// 1103, 1327, 1597
#define HASH_TABLE_SIZE 1103
static MonoImageSet *img_set_cache[HASH_TABLE_SIZE];

static guint32 mix_hash(uintptr_t source) {
...
static guint32 hash_images(MonoImage **images, int nimages) {
  guint32 res = 0;
  int i;
  for (i = 0; i < nimages; ++i)
...
static gboolean compare_img_set(MonoImageSet *set, MonoImage **images,
                                int nimages) {
  int j, k;
  ...
  for (j = 0; j < nimages; ++j) {
    ...
    // If we iterated all the way through set->images, images[j] was *not*
    // found.
    if (k == nimages)
      break; // Break on "image not found"
  }

  // If we iterated all the way through images without breaking, all items in
  // images were found in set->images
  return j == nimages;
}

static MonoImageSet *img_set_cache_get(MonoImage **images, int nimages) {
...
static void img_set_cache_add(MonoImageSet *set) {
  guint32 hash_code = hash_images(set->images, set->nimages);
  int index = hash_code % HASH_TABLE_SIZE;
  img_set_cache[index] = set;
}

static void img_set_cache_remove(MonoImageSet *is) {
 ...
 */
static MonoImageSet *get_image_set(MonoImage **images, int nimages) {
  int i, j, k;
  MonoImageSet *set;
  GSList *l;

  /* Common case: Image set contains corlib only. If we've seen that case
   * before, we cached the set. */
  ...
  // FIXME: Is corlib the correct thing to return here? If so, why? This may be
  // an artifact of generic instances previously defaulting to allocating from
  // corlib.
  if (nimages == 0)
    return mscorlib_image_set;
  ...
  // Before we go on, we should check to see whether a MonoImageSet with these
  // images already exists. We can search the referred-by imagesets of any one
  // of our images to do this. Arbitrarily pick one here:
  if (images[0] == mono_defaults.corlib && nimages > 1)
    l = images[1]->image_sets; // Prefer not to search the imagesets of corlib--
                               // that will be a long list.
  else
    l = images[0]->image_sets;

  set = NULL;
  while (l) // Iterate over selected list, looking for an imageset with members
            // equal to our target one
  {
    ...
    if (set->nimages == nimages) { // Member count differs, this can't be it
      ...
      // If we iterated all the way through images without breaking, all items
      // in images were found in set->images
      if (j == nimages) {
        // Break on "found a set with equal members".
        // This happens in case of a hash collision with a previously cached
        // set.
        break;
      }
    }
  ...
  // If we iterated all the way through l without breaking, the imageset does
  // not already exist and we should create it
  if (!l) {
    ...
    set->gclass_cache = mono_conc_hashtable_new_full(
        mono_generic_class_hash, mono_generic_class_equal, NULL,
        (GDestroyNotify)free_generic_class);
    set->ginst_cache = g_hash_table_new_full(
        mono_metadata_generic_inst_hash, mono_metadata_generic_inst_equal, NULL,
        (GDestroyNotify)free_generic_inst);
    set->gmethod_cache =
        g_hash_table_new_full(inflated_method_hash, inflated_method_equal, NULL,
                              (GDestroyNotify)free_inflated_method);
    set->gsignature_cache =
        g_hash_table_new_full(inflated_signature_hash, inflated_signature_equal,
                              NULL, (GDestroyNotify)free_inflated_signature);

    set->szarray_cache =
        g_hash_table_new_full(mono_aligned_addr_hash, NULL, NULL, NULL);
    set->array_cache =
        g_hash_table_new_full(mono_aligned_addr_hash, NULL, NULL, NULL);

    set->aggregate_modifiers_cache = g_hash_table_new_full(
        aggregate_modifiers_hash, aggregate_modifiers_equal, NULL,
        (GDestroyNotify)free_aggregate_modifiers);

    for (i = 0; i < nimages; ++i)
      set->images[i]->image_sets =
          g_slist_prepend(set->images[i]->image_sets, set);

    g_ptr_array_add(image_sets, set);
    UnlockedIncrement(&img_set_count); /* locked by image_sets_lock () */
  }

  /* Cache the set. If there was a cache collision, the previously cached value
   * will be replaced. */
...
static void delete_image_set(MonoImageSet *set) {
  int i;
  ...
  for (i = 0; i < set->nimages; ++i)
    set->images[i]->image_sets =
        g_slist_remove(set->images[i]->image_sets, set);
...
void mono_image_set_lock(MonoImageSet *set) { mono_os_mutex_lock(&set->lock); }
...
char *mono_image_set_description(MonoImageSet *set) {
...
static void collect_data_init(CollectData *data) {
  data->images = data->image_buf;
  data->images_len = 64;
  data->nimages = 0;
}

static void collect_data_free(CollectData *data) {
  if (data->images != data->image_buf)
    g_free(data->images);
}

static void enlarge_data(CollectData *data) {
...
static void add_image(MonoImage *image, CollectData *data) {
  int i;
...
static void collect_type_images(MonoType *type, CollectData *data);

static void collect_ginst_images(MonoGenericInst *ginst, CollectData *data) {
  int i;
...
static void collect_gclass_images(MonoGenericClass *gclass, CollectData *data) {
...
static void collect_signature_images(MonoMethodSignature *sig,
                                     CollectData *data) {
  gpointer iter = NULL;
  MonoType *p;
...
static void collect_inflated_signature_images(MonoInflatedMethodSignature *sig,
                                              CollectData *data) {
...
static void collect_method_images(MonoMethodInflated *method,
                                  CollectData *data) {
  ...
  /*
   * Dynamic assemblies have no references, so the images they depend on can be
   * unloaded before them.
   */
  if (image_is_dynamic(m_class_get_image(m->klass)))
    collect_signature_images(mono_method_signature_internal(m), data);
}

static void collect_aggregate_modifiers_images(MonoAggregateModContainer *amods,
                                               CollectData *data) {
  for (int i = 0; i < amods->count; ++i)
...
retry:
  ...
  case MONO_TYPE_MVAR: {
    MonoImage *image =
        mono_get_image_for_generic_param(type->data.generic_param);
    add_image(image, data);
    break;
  }
  case MONO_TYPE_CLASS:
  case MONO_TYPE_VALUETYPE:
    add_image(m_class_get_image(mono_class_from_mono_type_internal(type)),
              data);
    break;
  default:
...
static gboolean steal_gclass_in_image(gpointer key, gpointer value,
                                      gpointer data) {
...
static gboolean steal_ginst_in_image(gpointer key, gpointer value,
                                     gpointer data) {
...
static gboolean steal_aggregate_modifiers_in_image(gpointer key, gpointer value,
                                                   gpointer data) {
...
static gboolean inflated_method_in_image(gpointer key, gpointer value,
                                         gpointer data) {
  ...
  g_assert(m_class_get_image(method->declaring->klass) == image ||
           (method->context.class_inst &&
            ginst_in_image(method->context.class_inst, image)) ||
           (method->context.method_inst &&
            ginst_in_image(method->context.method_inst, image)) ||
           (((MonoMethod *)method)->signature &&
            signature_in_image(
                mono_method_signature_internal((MonoMethod *)method), image)));

  return TRUE;
}

static gboolean inflated_signature_in_image(gpointer key, gpointer value,
                                            gpointer data) {
  MonoImage *image = (MonoImage *)data;
  MonoInflatedMethodSignature *sig = (MonoInflatedMethodSignature *)key;

  return signature_in_image(sig->sig, image) ||
         (sig->context.class_inst &&
          ginst_in_image(sig->context.class_inst, image)) ||
         (sig->context.method_inst &&
          ginst_in_image(sig->context.method_inst, image));
}

static gboolean class_in_image(gpointer key, gpointer value, gpointer data) {
  MonoImage *image = (MonoImage *)data;
...
static void check_gmethod(gpointer key, gpointer value, gpointer data) {
  MonoMethodInflated *method = (MonoMethodInflated *)key;
  MonoImage *image = (MonoImage *)data;
 ...
 */
static G_GNUC_UNUSED void check_image_sets(MonoImage *image) {
  int i;
...
void mono_metadata_clean_for_image(MonoImage *image) {
  ...
  /*
   * The data structures could reference each other so we delete them in two
   * phases. This is required because of the hashing functions in
   * gclass/ginst_cache.
   */
  ...
  for (l = image->image_sets; l; l = l->next) {
    ...
    mono_image_set_lock(set);
    mono_conc_hashtable_foreach_steal(set->gclass_cache, steal_gclass_in_image,
                                      &gclass_data);
    g_hash_table_foreach_steal(set->ginst_cache, steal_ginst_in_image,
                               &ginst_data);
    g_hash_table_foreach_remove(set->gmethod_cache, inflated_method_in_image,
                                image);
    g_hash_table_foreach_remove(set->gsignature_cache,
                                inflated_signature_in_image, image);
    ...
    g_hash_table_foreach_steal(set->aggregate_modifiers_cache,
                               steal_aggregate_modifiers_in_image, &amods_data);
...
static void free_inflated_method(MonoMethodInflated *imethod) {
...
static void free_generic_class(MonoGenericClass *gclass) {
 ...
 *
 *   Given an inflated signature and a generic context, return a canonical copy
 * of the signature. The returned signature might be equal to SIG or it might be
 * a cached copy.
 */
MonoMethodSignature *
mono_metadata_get_inflated_signature(MonoMethodSignature *sig,
                                     MonoGenericContext *context) {
  ...
  res = (MonoInflatedMethodSignature *)g_hash_table_lookup(
      set->gsignature_cache, &helper);
  if (!res) {
...
MonoImageSet *mono_metadata_get_image_set_for_aggregate_modifiers(
...
static gboolean type_is_gtd(MonoType *type) {
  switch (type->type) {
  case MONO_TYPE_CLASS:
  case MONO_TYPE_VALUETYPE:
 ...
 *
 */
MonoGenericInst *mono_metadata_get_generic_inst(int type_argc,
                                                MonoType **type_argv) {
...
mono_metadata_get_canonical_generic_inst(MonoGenericInst *candidate) {
  ...
  MonoGenericInst *ginst =
      (MonoGenericInst *)g_hash_table_lookup(set->ginst_cache, candidate);
  if (!ginst) {
    int size = MONO_SIZEOF_GENERIC_INST + type_argc * sizeof(MonoType *);
    ...
    for (int i = 0; i < type_argc; ++i)
      ginst->type_argv[i] =
          mono_metadata_type_dup(NULL, candidate->type_argv[i]);
...
MonoAggregateModContainer *mono_metadata_get_canonical_aggregate_modifiers(
    MonoAggregateModContainer *candidate) {
  ...
  MonoAggregateModContainer *amods =
      (MonoAggregateModContainer *)g_hash_table_lookup(
          set->aggregate_modifiers_cache, candidate);
  if (!amods) {
    ...
    for (int i = 0; i < candidate->count; ++i) {
      ...
      amods->modifiers[i].type =
          mono_metadata_type_dup(NULL, candidate->modifiers[i].type);
    }
...
static gboolean mono_metadata_is_type_builder_generic_type_definition(
    MonoClass *container_class, MonoGenericInst *inst, gboolean is_dynamic) {
  MonoGenericContainer *container =
      mono_class_get_generic_container(container_class);

  if (!is_dynamic || m_class_was_typebuilder(container_class) ||
 ...
 *
 */
MonoGenericClass *mono_metadata_lookup_generic_class(MonoClass *container_class,
                                                     MonoGenericInst *inst,
                                                     gboolean is_dynamic) {
  MonoGenericClass *gclass;
  MonoGenericClass helper;
  gboolean is_tb_open = mono_metadata_is_type_builder_generic_type_definition(
      container_class, inst, is_dynamic);
  MonoImageSet *set;
  CollectData data;

  g_assert(mono_class_get_generic_container(container_class)->type_argc ==
           inst->type_argc);

  memset(&helper, 0, sizeof(helper)); // act like g_new0
  helper.container_class = container_class;
  helper.context.class_inst = inst;
  helper.is_dynamic = is_dynamic; /* We use this in a hash lookup, which does
                                     not attempt to downcast the pointer */
  ...
  gclass = (MonoGenericClass *)mono_conc_hashtable_lookup(set->gclass_cache,
                                                          &helper);
  ...
  gclass->owner = set;
  if (inst == mono_class_get_generic_container(container_class)
                  ->context.class_inst &&
      !is_tb_open)
  ...
  MonoGenericClass *gclass2 = (MonoGenericClass *)mono_conc_hashtable_insert(
 ...
 *
 */
MonoGenericInst *mono_metadata_inflate_generic_inst(MonoGenericInst *ginst,
                                                    MonoGenericContext *context,
                                                    MonoError *error) {
  MonoType **type_argv;
  ...
  for (i = 0; i < ginst->type_argc; i++) {
    type_argv[i] = mono_class_inflate_generic_type_checked(ginst->type_argv[i],
                                                           context, error);
    if (!is_ok(error))
      goto cleanup;
    ++count;
...
mono_metadata_parse_generic_inst(MonoImage *m, MonoGenericContainer *container,
  ...
  for (i = 0; i < count; i++) {
    /* this can be a transient type, mono_metadata_get_generic_inst will
     * allocate a canonical one, if needed.
     */
    MonoType *t = mono_metadata_parse_type_checked(m, container, 0, TRUE, ptr,
                                                   &ptr, error);
    if (!t)
      goto cleanup;
    type_argv[i] = t;
...
static gboolean do_mono_metadata_parse_generic_class(
    MonoType *type, MonoImage *m, MonoGenericContainer *container,
    const char *ptr, const char **rptr, MonoError *error) {
  MonoGenericInst *inst;
  MonoClass *gklass;
  MonoType *gtype;
  ...
  inst =
      mono_metadata_parse_generic_inst(m, container, count, ptr, &ptr, error);
  if (inst == NULL)
    return FALSE;
  ...
  type->data.generic_class =
      mono_metadata_lookup_generic_class(gklass, inst, FALSE);
  return TRUE;
}
 ...
 * @type: The kind of generic parameters the resulting generic-container should
 * contain
 */

static MonoGenericContainer *select_container(MonoGenericContainer *gc,
                                              MonoTypeEnum type) {
...
MonoGenericContainer *mono_get_anonymous_container_for_image(MonoImage *image,
                                                             gboolean is_mvar) {
  ...
  if (!result) {
    // Note this is never deallocated anywhere-- it exists for the lifetime of
    // the image it's allocated from
    result = (MonoGenericContainer *)mono_image_alloc0(
        image, sizeof(MonoGenericContainer));
    result->owner.image = image;
    result->is_anonymous = TRUE;
    result->is_method = is_mvar;

    // If another thread already made a container, use that and leak this new
    // one. (Technically it would currently be safe to just assign instead of
    // CASing.)
    MonoGenericContainer *exchange =
        (MonoGenericContainer *)mono_atomic_cas_ptr(
            (volatile gpointer *)container_pointer, result, NULL);
    if (exchange)
      result = exchange;
  }
  return result;
...
static MonoGenericParam *lookup_anon_gparam(MonoImage *image,
                                            MonoGenericContainer *container,
                                            gint32 param_num,
                                            gboolean is_mvar) {
  if (param_num >= 0 && param_num < FAST_GPARAM_CACHE_SIZE) {
    MonoGenericParam *cache =
        is_mvar ? image->mvar_gparam_cache_fast : image->var_gparam_cache_fast;
    if (!cache)
      return NULL;
    ...
    MonoConcurrentHashTable *cache =
        is_mvar ? image->mvar_gparam_cache : image->var_gparam_cache;
    if (!cache)
      return NULL;
...
publish_anon_gparam_fast(MonoImage *image, MonoGenericContainer *container,
                         gint32 param_num) {
  g_assert(param_num >= 0 && param_num < FAST_GPARAM_CACHE_SIZE);
  MonoGenericParam **cache = container->is_method
                                 ? &image->mvar_gparam_cache_fast
                                 : &image->var_gparam_cache_fast;
  if (!*cache) {
    mono_image_lock(image);
    if (!*cache) {
      *cache = (MonoGenericParam *)mono_image_alloc0(
          image, sizeof(MonoGenericParam) * FAST_GPARAM_CACHE_SIZE);
 ...
 *
 * Publish \p gparam anonymous generic parameter to the anon gparam cache for \p
 * image.
 *
 * LOCKING: takes the image lock.
 */
static MonoGenericParam *publish_anon_gparam_slow(MonoImage *image,
                                                  MonoGenericParam *gparam) {
  MonoConcurrentHashTable **cache = gparam->owner->is_method
                                        ? &image->mvar_gparam_cache
                                        : &image->var_gparam_cache;
  if (!*cache) {
    mono_image_lock(image);
    if (!*cache) {
      MonoConcurrentHashTable *ht = mono_conc_hashtable_new(
          (GHashFunc)mono_metadata_generic_param_hash,
          (GEqualFunc)mono_metadata_generic_param_equal);
      mono_atomic_store_release(cache, ht);
    }
    mono_image_unlock(image);
  }
  MonoGenericParam *other =
      (MonoGenericParam *)mono_conc_hashtable_insert(*cache, gparam, gparam);
  // If another thread published first return their param, otherwise return
  // ours.
 ...
 * \param is_mvar TRUE if this is a method generic parameter, FALSE if it's a
 * class generic parameter.
 *
 * Returns: a new, or exisisting \c MonoGenericParam for an anonymous generic
 * parameter with the given properties.
 *
 * LOCKING: takes the image lock.
 */
MonoGenericParam *mono_metadata_create_anon_gparam(MonoImage *image,
                                                   gint32 param_num,
                                                   gboolean is_mvar) {
  MonoGenericContainer *container =
      mono_get_anonymous_container_for_image(image, is_mvar);
  MonoGenericParam *gparam =
      lookup_anon_gparam(image, container, param_num, is_mvar);
  if (gparam)
    return gparam;
  if (param_num >= 0 && param_num < FAST_GPARAM_CACHE_SIZE) {
    ...
    gparam =
        (MonoGenericParam *)mono_image_alloc0(image, sizeof(MonoGenericParam));
 ...
 * LOCKING: Acquires the loader lock
 */
static MonoGenericParam *mono_metadata_parse_generic_param(
    MonoImage *m, MonoGenericContainer *generic_container, MonoTypeEnum type,
    const char *ptr, const char **rptr, MonoError *error) {
  ...
  if (!generic_container) {
    ...
    default:
      g_error(
          "Cerating generic param object with invalid MonoType"); // This is not
                                                                  // a generic
                                                                  // param
    }
  ...
  if (index >= generic_container->type_argc) {
    mono_error_set_bad_image(
        error, m, "Invalid generic %s parameter index %d, max index is %d",
        generic_container->is_method ? "method" : "type", index,
 ...
 * Shared MonoType instances help save memory. Their contents should not be
 * modified by the caller. They do not need to be freed as their lifetime is
 * bound by either the lifetime of the runtime (builtin types), or the lifetime
 * of the MonoClass instance they are embedded in. If they are freed, they
 * should be freed using mono_metadata_free_type () instead of g_free ().
 */
...
static gboolean compare_type_literals(MonoImage *image, int class_type,
                                      int type_type, MonoError *error) {
  error_init(error);

  /* _byval_arg.type can be zero if we're decoding a type that references a
   * class been loading. See mcs/test/gtest-440. and #650936.
   * FIXME This better be moved to the metadata verifier as it can catch more
   * cases.
   */
  if (!class_type)
    return TRUE;
  ...
  if (type_type == MONO_TYPE_CLASS) {
    ...
    mono_error_set_bad_image(error, image,
                             "Expected reference type but got type kind %d",
                             class_type);
    return FALSE;
  }
  ...
  default:
    // XXX stringify this argument
    mono_error_set_bad_image(
        error, image, "Expected value type but got type kind %d", class_type);
    return FALSE;
  }
}

static gboolean verify_var_type_and_container(MonoImage *image, int var_type,
                                              MonoGenericContainer *container,
                                              MonoError *error) {
  error_init(error);
  if (var_type == MONO_TYPE_MVAR) {
    ...
    if (!(!container->is_method || // VAR and class container
          (container->is_method &&
           container->parent))) { // VAR and method container with parent
      mono_error_set_bad_image(
          error, image, "VAR parsed in a context without a class container");
      return FALSE;
    }
  }
  return TRUE;
...
static gboolean do_mono_metadata_parse_type(MonoType *type, MonoImage *m,
  ...
  case MONO_TYPE_CLASS: {
    ...
    if (!compare_type_literals(m, m_class_get_byval_arg(klass)->type,
                               type->type, error))
      return FALSE;
  ...
  case MONO_TYPE_SZARRAY: {
    MonoType *etype = mono_metadata_parse_type_checked(
        m, container, 0, transient, ptr, &ptr, error);
    if (!etype)
      return FALSE;
    ...
    g_assert(
        type->data.klass); // This was previously a check for NULL, but mcfmt
                           // should never fail. It can return a borken
                           // MonoClass, but should return at least something.
    break;
  }
  case MONO_TYPE_PTR: {
    type->data.type = mono_metadata_parse_type_checked(
        m, container, 0, transient, ptr, &ptr, error);
    if (!type->data.type)
      return FALSE;
    break;
  ...
  case MONO_TYPE_FNPTR: {
    type->data.method = mono_metadata_parse_method_signature_full(
        m, container, 0, ptr, &ptr, error);
    if (!type->data.method)
      return FALSE;
    break;
  }
  case MONO_TYPE_ARRAY: {
    type->data.array = mono_metadata_parse_array_internal(
        m, container, transient, ptr, &ptr, error);
    if (!type->data.array)
      return FALSE;
    break;
  }
  ...
  case MONO_TYPE_VAR: {
    if (container &&
        !verify_var_type_and_container(m, type->type, container, error))
      return FALSE;

    type->data.generic_param = mono_metadata_parse_generic_param(
        m, container, type->type, ptr, &ptr, error);
  ...
  case MONO_TYPE_GENERICINST: {
    if (!do_mono_metadata_parse_generic_class(type, m, container, ptr, &ptr,
                                              error))
      return FALSE;
    break;
  }
...
void mono_metadata_free_type(MonoType *type) {
  ...
  case MONO_TYPE_VALUETYPE:
    if (type == m_class_get_byval_arg(type->data.klass) ||
        type == m_class_get_this_arg(type->data.klass))
      return;
    break;
  case MONO_TYPE_PTR:
 ...
 */
static MonoExceptionClause *parse_section_data(MonoImage *m, int *num_clauses,
  ...
  while (1) {
    ...
    if (sect_data_flags & METHOD_HEADER_SECTION_EHTABLE) {
      ...
      *num_clauses = is_fat ? sect_data_len / 24 : sect_data_len / 12;
      ...
      clauses = (MonoExceptionClause *)g_malloc0(sizeof(MonoExceptionClause) *
                                                 (*num_clauses));
      for (i = 0; i < *num_clauses; ++i) {
        ...
        /* g_print ("try %d: %x %04x-%04x %04x\n", i, ec->flags, ec->try_offset,
         * ec->try_offset+ec->try_len, ec->try_len); */
      }
    }
 ...
 */
gboolean mono_method_get_header_summary(MonoMethod *method,
                                        MonoMethodHeaderSummary *summary) {
  int idx;
  guint32 rva;
  MonoImage *img;
  const char *ptr;
  ...
  rva = mono_metadata_decode_row_col(&img->tables[MONO_TABLE_METHOD], idx - 1,
                                     MONO_METHOD_RVA);
 ...
 */
MonoMethodHeader *mono_metadata_parse_mh_full(MonoImage *m,
                                              MonoGenericContainer *container,
                                              const char *ptr,
                                              MonoError *error) {
  ...
  case METHOD_HEADER_FAT_FORMAT:
    ...
    break;
  default:
    mono_error_set_bad_image(error, m, "Invalid method header format %d",
                             format);
    return NULL;
  }
  ...
  if (local_var_sig_tok) {
    ...
    if (idx >= t->rows || idx < 0) {
      mono_error_set_bad_image(
          error, m, "Invalid method header local vars signature token 0x%8x",
          idx);
      goto fail;
    }
    ...
    if (!mono_verifier_verify_standalone_signature(
            m, cols[MONO_STAND_ALONE_SIGNATURE], error))
      goto fail;
  }
  if (fat_flags & METHOD_HEADER_MORE_SECTS) {
    clauses =
        parse_section_data(m, &num_clauses, (const unsigned char *)ptr, error);
  ...
  if (local_var_sig_tok) {
    ...
    mh = (MonoMethodHeader *)g_malloc0(
        MONO_SIZEOF_METHOD_HEADER + len * sizeof(MonoType *) +
        num_clauses * sizeof(MonoExceptionClause));
    mh->num_locals = len;
    for (i = 0; i < len; ++i) {
      mh->locals[i] = mono_metadata_parse_type_internal(
          m, container, 0, TRUE, locals_ptr, &locals_ptr, error);
      goto_if_nok(error, fail);
    }
  } else {
    mh = (MonoMethodHeader *)g_malloc0(
        MONO_SIZEOF_METHOD_HEADER + num_clauses * sizeof(MonoExceptionClause));
  }
  mh->code = code;
  ...
  if (clauses) {
    MonoExceptionClause *clausesp =
        (MonoExceptionClause *)&mh->locals[mh->num_locals];
 ...
 */
const unsigned char *mono_method_header_get_code(MonoMethodHeader *header,
                                                 guint32 *code_size,
                                                 guint32 *max_stack) {
  if (code_size)
 ...
 */
MonoType **mono_method_header_get_locals(MonoMethodHeader *header,
                                         guint32 *num_locals,
 ...
 * \param clause pointer to a \c MonoExceptionClause structure which will be
 * filled with the info
 *
 * Get the info about the exception clauses in the method. Set \c *iter to NULL
 * to initiate the iteration, then call the method repeatedly until it returns
 * FALSE. At each iteration, the structure pointed to by clause if filled with
 * the exception clause information.
 *
 * \returns TRUE if clause was filled with info, FALSE if there are no more
 * exception clauses.
 */
int mono_method_header_get_clauses(MonoMethodHeader *header, MonoMethod *method,
                                   gpointer *iter,
                                   MonoExceptionClause *clause) {
  MonoExceptionClause *sc;
  /* later we'll be able to use this interface to parse the clause info on
   * demand, without allocating anything.
   */
 ...
 */
MonoType *mono_metadata_parse_field_type(MonoImage *m, short field_flags,
                                         const char *ptr, const char **rptr) {
  ERROR_DECL(error);
  MonoType *type = mono_metadata_parse_type_internal(m, NULL, field_flags,
                                                     FALSE, ptr, rptr, error);
  mono_error_cleanup(error);
  return type;
}
 ...
 * \returns The \c MonoType that was extracted from \p ptr .
 */
MonoType *mono_metadata_parse_param(MonoImage *m, const char *ptr,
                                    const char **rptr) {
  ERROR_DECL(error);
  MonoType *type =
      mono_metadata_parse_type_internal(m, NULL, 0, FALSE, ptr, rptr, error);
  mono_error_cleanup(error);
  return type;
}
 ...
 *
 */
static int typedef_locator(const void *a, const void *b) {
  ...
  if (typedef_index + 1 < loc->t->rows) {
    col_next =
        mono_metadata_decode_row_col(loc->t, typedef_index + 1, loc->col_idx);
    if (loc->idx >= col_next)
      return 1;
...
static int table_locator(const void *a, const void *b) {
  locator_t *loc = (locator_t *)a;
  const char *bb = (const char *)b;
...
static int declsec_locator(const void *a, const void *b) {
  locator_t *loc = (locator_t *)a;
  const char *bb = (const char *)b;
  guint32 table_index = (bb - loc->t->base) / loc->t->row_size;
  guint32 col;
 ...
 * which contains IDX.
 */
static guint32 search_ptr_table(MonoImage *image, int table, int idx) {
...
guint32 mono_metadata_typedef_from_field(MonoImage *meta, guint32 index) {
  ...
  if (!mono_binary_search(&loc, tdef->base, tdef->rows, tdef->row_size,
                          typedef_locator))
    return 0;
 ...
 * \param error set on error
 *
 * The array of interfaces that the \p index typedef token implements is
 * returned in \p interfaces. The number of elements in the array is returned in
 * \p count.
 *
 * \returns \c TRUE on success, \c FALSE on failure and sets \p error.
 */
gboolean mono_metadata_interfaces_from_typedef_full(
    MonoImage *meta, guint32 index, MonoClass ***interfaces, guint *count,
    gboolean heap_alloc_result, MonoGenericContext *context, MonoError *error) {
  ...
  *count = 0;
  ...
  while (start > 0) {
    if (loc.idx ==
        mono_metadata_decode_row_col(tdef, start - 1, MONO_INTERFACEIMPL_CLASS))
      start--;
    else
      break;
  }
  ...
  if (heap_alloc_result)
    ...
    result = (MonoClass **)mono_image_alloc0(meta, sizeof(MonoClass *) *
                                                       (pos - start));
  ...
  while (pos < tdef->rows) {
    ...
    iface = mono_class_get_and_inflate_typespec_checked(
        meta, mono_metadata_token_from_dor(cols[MONO_INTERFACEIMPL_INTERFACE]),
        context, error);
    if (iface == NULL)
      return FALSE;
 ...
 * The array of interfaces that the \p index typedef token implements is
 * returned in \p interfaces. The number of elements in the array is returned in
 * \p count. The returned array is allocated with \c g_malloc and the caller
 * must free it.
 *
 * LOCKING: Acquires the loader lock .
 *
 ...
 */
MonoClass **mono_metadata_interfaces_from_typedef(MonoImage *meta,
                                                  guint32 index, guint *count) {
  ERROR_DECL(error);
  MonoClass **interfaces = NULL;
  gboolean rv;

  rv = mono_metadata_interfaces_from_typedef_full(meta, index, &interfaces,
                                                  count, TRUE, NULL, error);
  mono_error_assert_ok(error);
  if (rv)
    return interfaces;
  else
...
guint32 mono_metadata_nested_in_typedef(MonoImage *meta, guint32 index) {
  ...
  /* loc_result is 0..1, needs to be mapped to table index (that is +1) */
  return mono_metadata_decode_row_col(tdef, loc.result,
...
guint32 mono_metadata_nesting_typedef(MonoImage *meta, guint32 index,
  ...
  while (start <= tdef->rows) {
    if (class_index == mono_metadata_decode_row_col(
 ...
 * \returns the info stored in the \c ClassLayout table for the given typedef
 * token into the \p packing and \p size pointers. Returns 0 if the info is not
 * found.
 */
guint32 mono_metadata_packing_from_typedef(MonoImage *meta, guint32 index,
...
guint32 mono_metadata_custom_attrs_from_index(MonoImage *meta, guint32 index) {
  ...
  while ((loc.result > 0) &&
         (mono_metadata_decode_row_col(tdef, loc.result - 1,
                                       MONO_CUSTOM_ATTR_PARENT) == index))
    loc.result--;
...
guint32 mono_metadata_declsec_from_index(MonoImage *meta, guint32 index) {
  ...
  while ((loc.result > 0) &&
         (mono_metadata_decode_row_col(tdef, loc.result - 1,
                                       MONO_DECL_SECURITY_PARENT) == index))
    loc.result--;
 ...
 */
guint32 mono_metadata_localscope_from_methoddef(MonoImage *meta,
                                                guint32 index) {
...
#ifdef DEBUG
static void mono_backtrace(int limit) {
  void *array[limit];
  char **names;
  int i;
 ...
 *
 *   Set the alignment used by runtime to layout fields etc. of type TYPE to
 * ALIGN. This should only be used in AOT mode since the resulting layout will
 * not match the host abi layout.
 */
...
again:
  ...
  case MONO_TYPE_VALUETYPE: {
    if (m_class_is_enumtype(t->data.klass))
      return mono_type_size(mono_class_enum_basetype_internal(t->data.klass),
                            align);
    else
  ...
  case MONO_TYPE_ARRAY:
    *align = MONO_ABI_ALIGNOF(gpointer);
    return MONO_ABI_SIZEOF(gpointer);
  case MONO_TYPE_TYPEDBYREF:
    return mono_class_value_size(mono_defaults.typed_reference_class,
                                 (guint32 *)align);
  case MONO_TYPE_GENERICINST: {
    ...
    if (m_class_is_valuetype(container_class)) {
      if (m_class_is_enumtype(container_class))
        return mono_type_size(
            mono_class_enum_basetype_internal(container_class), align);
      else
        return mono_class_value_size(mono_class_from_mono_type_internal(t),
                                     (guint32 *)align);
    } else {
      *align = MONO_ABI_ALIGNOF(gpointer);
      return MONO_ABI_SIZEOF(gpointer);
  ...
  case MONO_TYPE_VAR:
  case MONO_TYPE_MVAR:
    if (!t->data.generic_param->gshared_constraint ||
        t->data.generic_param->gshared_constraint->type ==
            MONO_TYPE_VALUETYPE) {
      *align = MONO_ABI_ALIGNOF(gpointer);
      return MONO_ABI_SIZEOF(gpointer);
    } else {
...
int mono_type_stack_size_internal(MonoType *t, int *align,
                                  gboolean allow_open) {
  int tmp;
  MonoTypeEnum simple_type;
  ...
  case MONO_TYPE_MVAR:
    ...
    if (!t->data.generic_param->gshared_constraint ||
      ...
      *align = stack_slot_align;
      ...
      /* The gparam can only match types given by gshared_constraint */
      return mono_type_stack_size_internal(
          t->data.generic_param->gshared_constraint, align, allow_open);
    }
  case MONO_TYPE_TYPEDBYREF:
    *align = stack_slot_align;
  ...
  case MONO_TYPE_VALUETYPE: {
    ...
    if (m_class_is_enumtype(t->data.klass))
      return mono_type_stack_size_internal(
          mono_class_enum_basetype_internal(t->data.klass), align, allow_open);
    else {
  ...
  case MONO_TYPE_GENERICINST: {
    ...
    if (m_class_is_valuetype(container_class)) {
      if (m_class_is_enumtype(container_class))
        ...
        guint32 size = mono_class_value_size(
            mono_class_from_mono_type_internal(t), (guint32 *)align);
...
static gboolean _mono_metadata_generic_class_equal(const MonoGenericClass *g1,
  ...
  if (g1->is_dynamic != g2->is_dynamic)
    return FALSE;
  if (!mono_metadata_class_equal(g1->container_class, g2->container_class,
                                 signature_only))
    return FALSE;
...
static gboolean _mono_metadata_generic_class_container_equal(
    const MonoGenericClass *g1, MonoClass *c2, gboolean signature_only) {
  MonoGenericInst *i1 = g1->context.class_inst;
  MonoGenericInst *i2 =
      mono_class_get_generic_container(c2)->context.class_inst;
...
guint mono_metadata_generic_context_hash(const MonoGenericContext *context) {
  ...
  if (context->class_inst)
    hash = ((hash << 5) - hash) ^
           mono_metadata_generic_inst_hash(context->class_inst);
  if (context->method_inst)
    hash = ((hash << 5) - hash) ^
           mono_metadata_generic_inst_hash(context->method_inst);
  return hash;
}

gboolean mono_metadata_generic_context_equal(const MonoGenericContext *g1,
...
guint mono_metadata_type_hash(MonoType *t1) {
  ...
  case MONO_TYPE_GENERICINST:
    return ((hash << 5) - hash) ^
           mono_generic_class_hash(t1->data.generic_class);
  case MONO_TYPE_VAR:
  case MONO_TYPE_MVAR:
    return ((hash << 5) - hash) ^
           mono_metadata_generic_param_hash(t1->data.generic_param);
  default:
    return hash;
  }
}
...
guint mono_metadata_generic_param_hash(MonoGenericParam *p) {
  ...
  if (p->gshared_constraint)
    hash =
        ((hash << 5) - hash) ^ mono_metadata_type_hash(p->gshared_constraint);
  info = mono_generic_param_info(p);
  /* Can't hash on the owner klass/method, since those might not be set when
   * this is called */
...
static gboolean mono_metadata_generic_param_equal_internal(
    MonoGenericParam *p1, MonoGenericParam *p2, gboolean signature_only) {
  if (p1 == p2)
    return TRUE;
  if (mono_generic_param_num(p1) != mono_generic_param_num(p2))
    return FALSE;
  if (p1->gshared_constraint && p2->gshared_constraint) {
    if (!mono_metadata_type_equal(p1->gshared_constraint,
                                  p2->gshared_constraint))
      return FALSE;
  } else {
    if (p1->gshared_constraint != p2->gshared_constraint)
      return FALSE;
...
gboolean mono_metadata_generic_param_equal(MonoGenericParam *p1,
                                           MonoGenericParam *p2) {
...
static gboolean mono_metadata_class_equal(MonoClass *c1, MonoClass *c2,
  ...
  if (mono_class_is_ginst(c1) && mono_class_is_ginst(c2))
    return _mono_metadata_generic_class_equal(mono_class_get_generic_class(c1),
                                              mono_class_get_generic_class(c2),
                                              signature_only);
  if (mono_class_is_ginst(c1) && mono_class_is_gtd(c2))
    return _mono_metadata_generic_class_container_equal(
        mono_class_get_generic_class(c1), c2, signature_only);
  if (mono_class_is_gtd(c1) && mono_class_is_ginst(c2))
    return _mono_metadata_generic_class_container_equal(
        mono_class_get_generic_class(c2), c1, signature_only);
  ...
  if ((c1_type->type == MONO_TYPE_VAR) && (c2_type->type == MONO_TYPE_VAR))
    return mono_metadata_generic_param_equal_internal(
        c1_type->data.generic_param, c2_type->data.generic_param,
        signature_only);
  if ((c1_type->type == MONO_TYPE_MVAR) && (c2_type->type == MONO_TYPE_MVAR))
    return mono_metadata_generic_param_equal_internal(
        c1_type->data.generic_param, c2_type->data.generic_param,
        signature_only);
  if (signature_only && (c1_type->type == MONO_TYPE_SZARRAY) &&
      (c2_type->type == MONO_TYPE_SZARRAY))
    return mono_metadata_class_equal(c1_type->data.klass, c2_type->data.klass,
                                     signature_only);
  if (signature_only && (c1_type->type == MONO_TYPE_ARRAY) &&
      (c2_type->type == MONO_TYPE_ARRAY))
...
static gboolean mono_metadata_custom_modifiers_equal(MonoType *t1, MonoType *t2,
                                                     gboolean signature_only) {
  // ECMA 335, 7.1.1:
  ...
  // modifier (required or optional) shall not be considered to match.
  ...
  for (int i = 0; i < count; i++) {
    ...
    MonoType *cm1_type =
        mono_type_get_custom_modifier(t1, i, &cm1_required, error);
    mono_error_assert_ok(error);
    MonoType *cm2_type =
        mono_type_get_custom_modifier(t2, i, &cm2_required, error);
    mono_error_assert_ok(error);
 ...
 * @t1: a type
 * @t2: another type
 * @signature_only: If true, treat ginsts as equal which are instantiated
 * separately but have equal positional value
 *
...
static gboolean do_mono_metadata_type_equal(MonoType *t1, MonoType *t2,
  ...
  case MONO_TYPE_CLASS:
  case MONO_TYPE_SZARRAY:
    result = mono_metadata_class_equal(t1->data.klass, t2->data.klass,
                                       signature_only);
    break;
  case MONO_TYPE_PTR:
    result = do_mono_metadata_type_equal(t1->data.type, t2->data.type,
                                         signature_only);
    break;
  case MONO_TYPE_ARRAY:
    if (t1->data.array->rank != t2->data.array->rank)
      ...
      result = mono_metadata_class_equal(
          t1->data.array->eklass, t2->data.array->eklass, signature_only);
    break;
  case MONO_TYPE_GENERICINST:
  ...
  case MONO_TYPE_MVAR:
    ...
    break;
  case MONO_TYPE_FNPTR:
    result = mono_metadata_fnptr_equal(t1->data.method, t2->data.method,
                                       signature_only);
    break;
  default:
 ...
 *
 * Determine if \p t1 and \p t2 are signature compatible if \p signature_only is
 * TRUE, otherwise behaves the same way as mono_metadata_type_equal. The
 * function mono_metadata_type_equal(a, b) is just a shortcut for
 * mono_metadata_type_equal_full(a, b, FALSE). \returns TRUE if \p t1 and \p t2
 * are equal taking \p signature_only into account.
 */
gboolean mono_metadata_type_equal_full(MonoType *t1, MonoType *t2,
                                       gboolean signature_only) {
...
gboolean mono_metadata_signature_equal(MonoMethodSignature *sig1,
   ...
   *
   * That's what the `signature_only' argument of do_mono_metadata_type_equal()
   * is for.
   */
...
MonoType *mono_type_get_custom_modifier(const MonoType *ty, uint8_t idx,
                                        gboolean *required, MonoError *error) {
 ...
 * \returns copy of type allocated from the image's mempool (or from the heap,
 * if \p image is null).
 */
MonoType *mono_metadata_type_dup(MonoImage *image, const MonoType *o) {
  return mono_metadata_type_dup_with_cmods(image, o, o);
}

static void deep_type_dup_fixup(MonoImage *image, MonoType *r,
                                const MonoType *o);

static uint8_t custom_modifier_copy(MonoAggregateModContainer *dest,
                                    uint8_t dest_offset,
                                    const MonoType *source) {
  if (mono_type_is_aggregate_mods(source)) {
    MonoAggregateModContainer *src_cmods = mono_type_get_amods(source);
    memcpy(&dest->modifiers[dest_offset], &src_cmods->modifiers[0],
           src_cmods->count * sizeof(MonoSingleCustomMod));
    ...
    for (int i = 0; i < src_cmods->count; i++) {
      ...
      cmod->type = mono_type_get_checked(
          src_cmods->image, src_cmods->modifiers[i].token, NULL, error);
      mono_error_assert_ok(error);
 ...
 */
static MonoType *
do_metadata_type_dup_append_cmods(MonoImage *image, const MonoType *o,
                                  const MonoType *cmods_source) {
  ...
  if (!mono_type_is_aggregate_mods(o) &&
      ...
      mono_type_get_cmods(o)->image ==
          mono_type_get_cmods(cmods_source)->image) {
    ...
    MonoType *r = image ? (MonoType *)mono_image_alloc0(image, sizeof_dup)
                        : (MonoType *)g_malloc0(sizeof_dup);
    ...
    /* The modifier order matters to Roslyn, they expect the extra cmods to come
     * first:
     *
     ...
     *
     * but! cmods are encoded in IL in reverse order, so 'int32 modopt(IsConst)
     * modopt(IsLong)' is encoded as `cmod_opt [typeref IsLong] cmod_opt
     * [typeref IsConst] I4` so in our array, extra_cmods (IsLong) come first,
     * followed by o_cmods (IsConst)
     *
     * (Here 'o' is 'int32 modopt(IsLong)' and cmods_source is 'T
     * modopt(IsConst)')
     */
    ...
    memcpy(&r_container->modifiers[dest_offset], &o_cmods->modifiers[0],
           o_cmods->count * sizeof(MonoCustomMod));
    dest_offset += o_cmods->count;
    memcpy(&r_container->modifiers[dest_offset], &extra_cmods->modifiers[0],
           extra_cmods->count * sizeof(MonoCustomMod));
    dest_offset += extra_cmods->count;
    g_assert(dest_offset == total_cmods);

    return r;
  } else {
    /* The aggregate case: either o_cmods or extra_cmods has aggregate cmods, or
     * they're both simple but from different images. */
    ...
    MonoAggregateModContainer *r_container_candidate =
    ...
    dest_offset =
        custom_modifier_copy(r_container_candidate, dest_offset, cmods_source);
    g_assert(dest_offset == total_cmods);
    r_container_candidate->count = total_cmods;

    mono_type_set_amods(r, mono_metadata_get_canonical_aggregate_modifiers(
...
/**
 * Works the same way as mono_metadata_type_dup but pick cmods from
 * @cmods_source
 */
MonoType *mono_metadata_type_dup_with_cmods(MonoImage *image, const MonoType *o,
                                            const MonoType *cmods_source) {
  ...
  /* if we get here, either o and cmods_source alias, or else exactly one of
   * them has cmods. */

  uint8_t num_mods = MAX(mono_type_custom_modifier_count(o),
                         mono_type_custom_modifier_count(cmods_source));
  gboolean aggregate = mono_type_is_aggregate_mods(o) ||
                       mono_type_is_aggregate_mods(cmods_source);
  size_t sizeof_r = mono_sizeof_type_with_mods(num_mods, aggregate);

  r = image ? (MonoType *)mono_image_alloc0(image, sizeof_r)
            : (MonoType *)g_malloc0(sizeof_r);

  if (cmods_source->has_cmods) {
    /* FIXME: if it's aggregate what do we assert here? */
    g_assert(!image ||
             (!aggregate && image == mono_type_get_cmods(cmods_source)->image));
  ...
  if (aggregate)
    mono_type_set_amods(r, mono_type_is_aggregate_mods(o)
 ...
 */
void mono_metadata_encode_value(guint32 value, char *buf, char **endbuf) {
  char *p = buf;
 ...
 * \param offset a pointer to an integer where to store the offset that  may
 * have been specified for the field in a FieldLayout table \param rva a pointer
 * to the RVA of the field data in the image that may have been defined in a \c
 * FieldRVA table \param marshal_spec a pointer to the marshal spec that may
 * have been defined for the field in a \c FieldMarshal table.
 *
 * Gather info for field \p index that may have been defined in the \c
 * FieldLayout, \c FieldRVA and \c FieldMarshal tables. Either of \p offset, \p
 * rva and \p marshal_spec can be NULL if you're not interested in the data.
 */
void mono_metadata_field_info(MonoImage *meta, guint32 index, guint32 *offset,
                              guint32 *rva, MonoMarshalSpec **marshal_spec) {
  mono_metadata_field_info_full(meta, index, offset, rva, marshal_spec, FALSE);
}

void mono_metadata_field_info_with_mempool(MonoImage *meta, guint32 index,
                                           guint32 *offset, guint32 *rva,
                                           MonoMarshalSpec **marshal_spec) {
...
static void mono_metadata_field_info_full(MonoImage *meta, guint32 index,
  ...
  if (offset) {
    ...
    if (tdef->base && mono_binary_search(&loc, tdef->base, tdef->rows,
                                         tdef->row_size, table_locator)) {
      *offset = mono_metadata_decode_row_col(tdef, loc.result,
  ...
  if (marshal_spec) {
    ...
    if ((p = mono_metadata_get_marshal_info(meta, index, TRUE))) {
      *marshal_spec = mono_metadata_parse_marshal_spec_full(
          alloc_from_image ? meta : NULL, meta, p);
    }
  }
}
...
guint32 mono_metadata_get_constant_index(MonoImage *meta, guint32 token,
  ...
  if ((hint > 0) && (hint < tdef->rows) &&
      (mono_metadata_decode_row_col(tdef, hint - 1, MONO_CONSTANT_PARENT) ==
       index))
    return hint;
 ...
 */
guint32 mono_metadata_events_from_typedef(MonoImage *meta, guint32 index,
                                          guint *end_idx) {
  locator_t loc;
  guint32 start, end;
  ...
  *end_idx = 0;
  ...
  start =
      mono_metadata_decode_row_col(tdef, loc.result, MONO_EVENT_MAP_EVENTLIST);
  if (loc.result + 1 < tdef->rows) {
    end = mono_metadata_decode_row_col(tdef, loc.result + 1,
 ...
 * in the \p end_idx pointer.
 */
guint32 mono_metadata_methods_from_event(MonoImage *meta, guint32 index,
                                         guint *end_idx) {
  locator_t loc;
  guint start, end;
  ...
  *end_idx = 0;
  ...
  loc.idx = ((index + 1) << MONO_HAS_SEMANTICS_BITS) |
            MONO_HAS_SEMANTICS_EVENT; /* Method association coded index */

  if (!mono_binary_search(&loc, msemt->base, msemt->rows, msemt->row_size,
                          table_locator))
    return 0;
   ...
   */
  while (start > 0) {
    if (loc.idx == mono_metadata_decode_row_col(msemt, start - 1,
 ...
 * in the \p end_idx pointer.
 */
guint32 mono_metadata_properties_from_typedef(MonoImage *meta, guint32 index,
                                              guint *end_idx) {
  locator_t loc;
  guint32 start, end;
 ...
 * \param meta metadata context
 * \param index 0-based index (in the \c PropertyDef table) describing a
 * property \returns the 0-based index in the \c MethodDef table for the methods
 * in the property. The last method that belongs to the property (plus 1) is
 * stored in the \p end_idx pointer.
 */
guint32 mono_metadata_methods_from_property(MonoImage *meta, guint32 index,
                                            guint *end_idx) {
  locator_t loc;
  guint start, end;
  guint32 cols[MONO_METHOD_SEMA_SIZE];
  ...
  *end_idx = 0;
  ...
  loc.col_idx = MONO_METHOD_SEMA_ASSOCIATION;
  loc.idx = ((index + 1) << MONO_HAS_SEMANTICS_BITS) |
            MONO_HAS_SEMANTICS_PROPERTY; /* Method association coded index */
...
guint32 mono_metadata_implmap_from_method(MonoImage *meta, guint32 method_idx) {
  ...
  loc.idx =
      ((method_idx + 1) << MONO_MEMBERFORWD_BITS) | MONO_MEMBERFORWD_METHODDEF;
 ...
 * \deprecated use \c mono_type_create_from_typespec_checked that has proper
 * error handling
 *
 * Creates a \c MonoType representing the \c TypeSpec indexed by the \p
 * type_spec token.
 */
MonoType *mono_type_create_from_typespec(MonoImage *image, guint32 type_spec) {
  ERROR_DECL(error);
  MonoType *type =
      mono_type_create_from_typespec_checked(image, type_spec, error);
  if (!type)
    g_error("Could not create typespec %x due to %s", type_spec,
            mono_error_get_message(error));
  return type;
}

MonoType *mono_type_create_from_typespec_checked(MonoImage *image,
                                                 guint32 type_spec,
                                                 MonoError *error)

{
  ...
  type = (MonoType *)mono_conc_hashtable_lookup(image->typespec_cache,
  ...
  if (!mono_verifier_verify_typespec_signature(
          image, cols[MONO_TYPESPEC_SIGNATURE], type_spec, error))
    return NULL;

  mono_metadata_decode_value(ptr, &ptr);

  type =
      mono_metadata_parse_type_checked(image, NULL, 0, TRUE, ptr, &ptr, error);
  if (!type)
    return NULL;
  ...
  type = (MonoType *)mono_conc_hashtable_insert(
      image->typespec_cache, GUINT_TO_POINTER(type_spec), type2);
  if (!type)
    type = type2;
...
static char *mono_image_strndup(MonoImage *image, const char *data, guint len) {
  char *res;
  if (!image)
 ...
 */
MonoMarshalSpec *mono_metadata_parse_marshal_spec(MonoImage *image,
                                                  const char *ptr) {
  return mono_metadata_parse_marshal_spec_full(NULL, image, ptr);
}

/*
 * If IMAGE is non-null, memory will be allocated from its mempool, otherwise it
 * will be allocated using malloc. PARENT_IMAGE is the image containing the
 * marshal spec.
 */
MonoMarshalSpec *mono_metadata_parse_marshal_spec_full(MonoImage *image,
                                                       MonoImage *parent_image,
                                                       const char *ptr) {
  MonoMarshalSpec *res;
  int len;
 ...
 * The value pointed to by \p conv will contain the kind of marshalling required
 * for this particular type one of the \c MONO_MARSHAL_CONV_ enumeration values.
 * \returns A \c MonoMarshalNative enumeration value (<code>MONO_NATIVE_</code>)
 * value describing the underlying native reprensetation of the type.
 */
...
handle_enum:
  ...
  case MONO_TYPE_STRING:
    if (mspec) {
      ...
      default:
        g_error("Can not marshal string to native type '%02x': Invalid "
                "managed/unmanaged type combination (String fields must be "
                "paired with LPStr, LPWStr, BStr or ByValTStr).",
                mspec->native);
      }
    }
    if (unicode) {
  ...
  case MONO_TYPE_ARRAY:
    if (mspec) {
      ...
      case MONO_NATIVE_BYVALARRAY:
        if ((m_class_get_element_class(type->data.klass) ==
             mono_defaults.char_class) &&
            !unicode)
  ...
  case MONO_TYPE_OBJECT: {
    ...
    if (mspec) {
      ...
      case MONO_NATIVE_FUNC:
        if (t == MONO_TYPE_CLASS &&
            (type->data.klass == mono_defaults.multicastdelegate_class ||
             type->data.klass == mono_defaults.delegate_class ||
             m_class_get_parent(type->data.klass) ==
    ...
    if (mono_class_try_get_safehandle_class() && type->data.klass != NULL &&
        mono_class_is_subclass_of_internal(
            type->data.klass, mono_class_try_get_safehandle_class(), FALSE)) {
      *conv = MONO_MARSHAL_CONV_SAFEHANDLE;
      return MONO_NATIVE_INT;
    }
#ifndef DISABLE_COM
    if (t == MONO_TYPE_CLASS &&
        mono_cominterop_is_interface(type->data.klass)) {
      *conv = MONO_MARSHAL_CONV_OBJECT_INTERFACE;
      return MONO_NATIVE_INTERFACE;
    }
#endif
 ...
 */
const char *mono_metadata_get_marshal_info(MonoImage *meta, guint32 idx,
                                           gboolean is_field) {
  locator_t loc;
  ...
  loc.idx = ((idx + 1) << MONO_HAS_FIELD_MARSHAL_BITS) |
            (is_field ? MONO_HAS_FIELD_MARSHAL_FIELDSREF
  ...
  return mono_metadata_blob_heap(
      meta, mono_metadata_decode_row_col(tdef, loc.result,
                                         MONO_FIELD_MARSHAL_NATIVE_TYPE));
}

MonoMethod *mono_method_from_method_def_or_ref(MonoImage *m, guint32 tok,
                                               MonoGenericContext *context,
                                               MonoError *error) {
  ...
  case MONO_METHODDEFORREF_METHODDEF:
    result = mono_get_method_checked(m, MONO_TOKEN_METHOD_DEF | idx, NULL,
                                     context, error);
    break;
  case MONO_METHODDEFORREF_METHODREF:
    result = mono_get_method_checked(m, MONO_TOKEN_MEMBER_REF | idx, NULL,
                                     context, error);
    break;
  default:
 ...
 * and the number of overrides in @num_overrides.
 *
 */
void mono_class_get_overrides_full(MonoImage *image, guint32 type_token,
  ...
  *overrides = NULL;
  ...
  while (start > 0) {
    if (loc.idx ==
        mono_metadata_decode_row_col(tdef, start - 1, MONO_METHODIMPL_CLASS))
      start--;
    else
      break;
  }
  while (end < tdef->rows) {
    if (loc.idx ==
        mono_metadata_decode_row_col(tdef, end, MONO_METHODIMPL_CLASS))
      end++;
    else
      break;
  }
  ...
  for (i = 0; i < num; ++i) {
    ...
    method = mono_method_from_method_def_or_ref(
 ...
 */
char *mono_guid_to_string_minimal(const guint8 *guid) {
  return g_strdup_printf(
      "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
      guid[3], guid[2], guid[1], guid[0], guid[5], guid[4], guid[7], guid[6],
      guid[8], guid[9], guid[10], guid[11], guid[12], guid[13], guid[14],
...
static gboolean get_constraints(MonoImage *image, int owner,
  ...
  *constraints = NULL;
  ...
  for (i = 0; i < tdef->rows; ++i) {
    ...
    if (cols[MONO_GENPARCONSTRAINT_GENERICPAR] == owner) {
      token =
          mono_metadata_token_from_dor(cols[MONO_GENPARCONSTRAINT_CONSTRAINT]);
      klass = mono_class_get_and_inflate_typespec_checked(image, token, context,
                                                          error);
      if (!klass) {
        g_slist_free(cons);
        return FALSE;
  ...
  if (!found)
    return TRUE;
  res =
      (MonoClass **)mono_image_alloc0(image, sizeof(MonoClass *) * (found + 1));
 ...
 */
guint32 mono_metadata_get_generic_param_row(MonoImage *image, guint32 token,
                                            guint32 *owner) {
  ...
  *owner |= mono_metadata_token_index(token) << MONO_TYPEORMETHOD_BITS;
  ...
  while ((loc.result > 0) &&
         (mono_metadata_decode_row_col(tdef, loc.result - 1,
                                       MONO_GENERICPARAM_OWNER) == loc.idx))
    loc.result--;
 ...
 */
gboolean mono_metadata_load_generic_param_constraints_checked(
    MonoImage *image, guint32 token, MonoGenericContainer *container,
    MonoError *error) {
  ...
  for (i = 0; i < container->type_argc; i++) {
    if (!get_constraints(
            image, start_row + i,
            &mono_generic_container_get_param_info(container, i)->constraints,
            container, error)) {
      return FALSE;
    }
  }
 ...
 *
 * Use this method after parsing a type or method definition to figure out
 * whether it's a generic type / method.  When parsing a method definition,
 * @parent_container points to the generic container of the current class, if
 * any.
 *
 * Note: This method does not load the constraints: for typedefs, this has to be
 * done after fully creating the type.
 *
 * Returns: NULL if @token is not a generic type or method definition or the new
 * generic container.
 *
 * LOCKING: Acquires the loader lock
 *
...
mono_metadata_load_generic_params(MonoImage *image, guint32 token,
  ...
  params = NULL;
  n = 0;
  container = (MonoGenericContainer *)mono_image_alloc0(
      image, sizeof(MonoGenericContainer));
  ...
  do {
    n++;
    params = (MonoGenericParamFull *)g_realloc(
        params, sizeof(MonoGenericParamFull) * n);
    ...
    params[n - 1].info.name =
        mono_metadata_string_heap(image, cols[MONO_GENERICPARAM_NAME]);
    if (params[n - 1].num != n - 1)
      g_warning("GenericParam table unsorted or hole in generic param "
                "sequence: token %d",
                i);
    if (++i > tdef->rows)
      break;
  ...
  container->type_params = (MonoGenericParamFull *)mono_image_alloc0(
      image, sizeof(MonoGenericParamFull) * n);
  ...
  if (container->is_method) {
    context->class_inst =
        container->parent ? container->parent->context.class_inst : NULL;
 ...
 * mono_type_get_class:
 * \param type the \c MonoType operated on
 * It is only valid to call this function if \p type is a \c MONO_TYPE_CLASS or
 * a \c MONO_TYPE_VALUETYPE . For more general functionality, use \c
 * mono_class_from_mono_type_internal, instead. \returns the \c MonoClass
 * pointer that describes the class that \p type represents.
 */
 ...
 */
MonoClass *mono_type_get_modifiers(MonoType *type, gboolean *is_required,
                                   gpointer *iter) {
  /* FIXME: implement */
  return NULL;
}
...
mono_bool mono_type_is_struct(MonoType *type) {
  return (
      ...
      ((type->type == MONO_TYPE_VALUETYPE &&
       ...
       (type->type == MONO_TYPE_TYPEDBYREF) ||
 ...
 * \param type the \c MonoType operated on
 * \returns TRUE if \p type is a managed or unmanaged pointer type. FALSE
 * otherwise.
 */
mono_bool mono_type_is_pointer(MonoType *type) {
  return (type &&
          ((type->byref || (type->type == MONO_TYPE_I) ||
            type->type == MONO_TYPE_STRING) ||
           (type->type == MONO_TYPE_SZARRAY) ||
           (type->type == MONO_TYPE_CLASS) || (type->type == MONO_TYPE_U) ||
           (type->type == MONO_TYPE_OBJECT) ||
...
mono_bool mono_type_is_reference(MonoType *type) {
  ...
  return (
      type &&
      (((type->type == MONO_TYPE_STRING) || (type->type == MONO_TYPE_SZARRAY) ||
        (type->type == MONO_TYPE_CLASS) || (type->type == MONO_TYPE_OBJECT) ||
...
MonoType *mono_signature_get_params_internal(MonoMethodSignature *sig,
                                             gpointer *iter) {
  MonoType **type;
  if (!iter)
    return NULL;
...
/*
 * If @field belongs to an inflated generic class, return the corresponding
 * field of the generic type definition class.
 */
MonoClassField *
...
/*
 * If @event belongs to an inflated generic class, return the corresponding
 * event of the generic type definition class.
 */
MonoEvent *mono_metadata_get_corresponding_event_from_generic_type_definition(
    MonoEvent *event) {
  MonoClass *gtd;
  int offset;
...
/*
 * If @property belongs to an inflated generic class, return the corresponding
 * property of the generic type definition class.
 */
MonoProperty *
...
// but lives in metadata.c due to use of static variables of this file.
...
size_t mono_sizeof_type_with_mods(uint8_t num_mods, gboolean is_aggregate) {
  if (num_mods == 0)
