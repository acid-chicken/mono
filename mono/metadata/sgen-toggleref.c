...
void sgen_process_togglerefs(void) {
  ...
  SGEN_LOG(
      4,
      "Done Proccessing ToggleRefs dropped %d strong %d weak %d final size %d",
...
void sgen_foreach_toggleref_root(void (*callback)(MonoObject *, gpointer),
                                 gpointer data) {
  int i;
  for (i = 0; i < toggleref_array_size; ++i) {
    if (toggleref_array[i].strong_ref)
    ...
    if (toggleref_array[i].weak_ref) {
      ...
      if ((char *)object >= start && (char *)object < end) {
        if (sgen_gc_is_object_ready_for_finalization(object)) {
          ...
          toggleref_array[i].weak_ref = NULL; /* We defer compaction to only
                                                 happen on the callback step. */
        } else {
...
static void ensure_toggleref_capacity(int capacity) {
  if (!toggleref_array) {
    ...
    toggleref_array = (MonoGCToggleRef *)sgen_alloc_internal_dynamic(
        ...
        INTERNAL_MEM_TOGGLEREF_DATA, TRUE);
  }
  if (toggleref_array_size + capacity >= toggleref_array_capacity) {
    ...
    memcpy(tmp, toggleref_array,
           toggleref_array_size * sizeof(MonoGCToggleRef));
 ...
 * @strong_ref if true the object is registered with a strong ref, a weak one
 * otherwise
 *
 * Register a given object for toggleref processing. It will be stored
 * internally and the toggleref callback will be called on it until it returns
 * MONO_TOGGLE_REF_DROP or is collected.
 */
 ...
 *
 * The callback must decide the status of a given object. It must return one of
 * the values in the \c MONO_TOGGLE_REF_ enum. This function is called with the
 * world running but with the GC locked. This means that you can do everything
 * that doesn't require GC interaction. This includes, but not limited to,
 * allocating objects, (de)registering for finalization, manipulating gchandles,
 * storing to reference fields or interacting with other threads that might
