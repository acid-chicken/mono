...
static gboolean load_profiler(MonoDl *module, const char *name,
                              const char *desc) {
  g_assert(module);
  ...
  if (!(err = mono_dl_symbol(module, old_name, (gpointer *)&func))) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_PROFILER,
               "Found old-style startup symbol '%s' for the '%s' profiler; it "
               "has not been migrated to the new API.",
               old_name, name);
    g_free(old_name);
    return FALSE;
  }
...
static gboolean load_profiler_from_executable(const char *name,
                                              const char *desc) {
  char *err;
...
static gboolean load_profiler_from_directory(const char *directory,
                                             const char *libname,
                                             const char *name,
                                             const char *desc) {
  char *path, *err;
  ...
  while ((path = mono_dl_build_path(directory, libname, &iter))) {
    ...
    if (!module) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_PROFILER,
                 "Could not open from directory \"%s\": %s", path, err);
      g_free(err);
      g_free(path);
      continue;
    }
...
static gboolean load_profiler_from_installation(const char *libname,
                                                const char *name,
                                                const char *desc) {
  char *err;
...
void mono_profiler_load(const char *desc) {
  ...
  if (mono_config_get_assemblies_dir() &&
      load_profiler_from_directory(mono_assembly_getrootdir(), libname, mname,
                                   desc))
    goto done;
  ...
  mono_trace(G_LOG_LEVEL_CRITICAL, MONO_TRACE_PROFILER,
             "The '%s' profiler wasn't found in the main executable nor could "
             "it be loaded from '%s'.",
             mname, libname);
 ...
 */
void mono_profiler_set_cleanup_callback(MonoProfilerHandle handle,
 ...
 * This function is async safe.
 */
void mono_profiler_set_coverage_filter_callback(
    MonoProfilerHandle handle, MonoProfilerCoverageFilterCallback cb) {
...
static void coverage_unlock(void) {
 ...
 * This function is \b not async safe.
 */
mono_bool mono_profiler_get_coverage_data(MonoProfilerHandle handle,
                                          MonoMethod *method,
  ...
  MonoProfilerCoverageInfo *info =
      (MonoProfilerCoverageInfo *)g_hash_table_lookup(
          mono_profiler_state.coverage_hash, method);
  ...
  if (!info) {
    ...
    mono_debug_get_seq_points(minfo, NULL, &source_file_list, &source_files,
                              &sym_seq_points, &n_il_offsets);
    ...
    if (cil_code && cil_code >= start && cil_code < end) {
      ...
      if (minfo) {
        MonoDebugSourceLocation *loc =
            mono_debug_method_lookup_location(minfo, offset);
...
gboolean mono_profiler_coverage_instrumentation_enabled(MonoMethod *method) {
  ...
  for (MonoProfilerHandle handle = mono_profiler_state.profilers; handle;
       handle = handle->next) {
    MonoProfilerCoverageFilterCallback cb =
        (MonoProfilerCoverageFilterCallback)handle->coverage_filter;
...
MonoProfilerCoverageInfo *mono_profiler_coverage_alloc(MonoMethod *method,
                                                       guint32 entries) {
  if (!mono_profiler_state.code_coverage)
    return NULL;
  ...
  MonoProfilerCoverageInfo *info =
      g_malloc0(sizeof(MonoProfilerCoverageInfo) +
                sizeof(MonoProfilerCoverageInfoEntry) * entries);
 ...
 * This function is async safe.
 */
mono_bool mono_profiler_set_sample_mode(MonoProfilerHandle handle,
 ...
 * This function is async safe.
 */
mono_bool mono_profiler_get_sample_mode(MonoProfilerHandle handle,
                                        MonoProfilerSampleMode *mode,
                                        uint32_t *freq) {
  if (mode)
...
void mono_profiler_sampling_thread_wait(void) {
  mono_os_sem_wait(&mono_profiler_state.sampling_semaphore,
                   MONO_SEM_FLAGS_NONE);
}
 ...
 * This function is async safe.
 */
void mono_profiler_set_call_instrumentation_filter_callback(
 ...
 * introspection was enabled, or \c FALSE if the function was called too late
 * for this to be possible.
 *
 * This function is \b not async safe.
 *
 ...
 * This function is \b not async safe.
 */
void *mono_profiler_call_context_get_argument(MonoProfilerCallContext *context,
                                              uint32_t position) {
  if (!mono_profiler_state.call_contexts)
    return NULL;
 ...
 * This function is \b not async safe.
 */
void *mono_profiler_call_context_get_local(MonoProfilerCallContext *context,
                                           uint32_t position) {
  if (!mono_profiler_state.call_contexts)
    return NULL;
...
mono_profiler_get_call_instrumentation_flags(MonoMethod *method) {
  MonoProfilerCallInstrumentationFlags flags =
  ...
  for (MonoProfilerHandle handle = mono_profiler_state.profilers; handle;
    ...
    MonoProfilerCallInstrumentationFilterCallback cb =
        (MonoProfilerCallInstrumentationFilterCallback)
            handle->call_instrumentation_filter;
...
void mono_profiler_started(void) { mono_profiler_state.startup_done = TRUE; }
...
#define MONO_PROFILER_EVENT_0(name, type) _MONO_PROFILER_EVENT(name)
#define MONO_PROFILER_EVENT_1(name, type, arg1_type, arg1_name)                \
  _MONO_PROFILER_EVENT(name)
#define MONO_PROFILER_EVENT_2(name, type, arg1_type, arg1_name, arg2_type,     \
                              arg2_name)                                       \
  _MONO_PROFILER_EVENT(name)
#define MONO_PROFILER_EVENT_3(name, type, arg1_type, arg1_name, arg2_type,     \
                              arg2_name, arg3_type, arg3_name)                 \
  _MONO_PROFILER_EVENT(name)
#define MONO_PROFILER_EVENT_4(name, type, arg1_type, arg1_name, arg2_type,     \
                              arg2_name, arg3_type, arg3_name, arg4_type,      \
                              arg4_name)                                       \
  _MONO_PROFILER_EVENT(name)
#define MONO_PROFILER_EVENT_5(name, type, arg1_type, arg1_name, arg2_type,     \
                              arg2_name, arg3_type, arg3_name, arg4_type,      \
                              arg4_name, arg5_type, arg5_name)                 \
  _MONO_PROFILER_EVENT(name)
...
#define MONO_PROFILER_EVENT_0(name, type) _MONO_PROFILER_EVENT(name, type)
#define MONO_PROFILER_EVENT_1(name, type, arg1_type, arg1_name)                \
...
#undef _MONO_PROFILER_EVENT
  ...
  while (head) {
    MonoProfilerCleanupCallback cb =
        (MonoProfilerCleanupCallback)head->cleanup_callback;
...
static void update_callback(volatile gpointer *location, gpointer new_,
                            volatile gint32 *counter) {
  gpointer old;
...
#define _MONO_PROFILER_EVENT(name, type)                                       \
  void mono_profiler_set_##name##_callback(MonoProfilerHandle handle,          \
                                           MonoProfiler##type##Callback cb) {  \
    update_callback(&handle->name##_cb, (gpointer)cb,                          \
                    &mono_profiler_state.name##_count);                        \
  }
#define MONO_PROFILER_EVENT_0(name, type) _MONO_PROFILER_EVENT(name, type)
#define MONO_PROFILER_EVENT_1(name, type, arg1_type, arg1_name)                \
  _MONO_PROFILER_EVENT(name, type)
...
#define _MONO_PROFILER_EVENT(name, type, params, args)                         \
  void mono_profiler_raise_##name params {                                     \
    if (!mono_profiler_state.startup_done)                                     \
      return;                                                                  \
    for (MonoProfilerHandle h = mono_profiler_state.profilers; h;              \
         h = h->next) {                                                        \
      MonoProfiler##type##Callback cb =                                        \
          (MonoProfiler##type##Callback)h->name##_cb;                          \
      if (cb)                                                                  \
        cb args;                                                               \
    }                                                                          \
  }
...
#define MONO_PROFILER_EVENT_2(name, type, arg1_type, arg1_name, arg2_type,     \
                              arg2_name)                                       \
  _MONO_PROFILER_EVENT(name, type, (arg1_type arg1_name, arg2_type arg2_name), \
                       (h->prof, arg1_name, arg2_name))
#define MONO_PROFILER_EVENT_3(name, type, arg1_type, arg1_name, arg2_type,     \
                              arg2_name, arg3_type, arg3_name)                 \
  _MONO_PROFILER_EVENT(                                                        \
      name, type,                                                              \
      (arg1_type arg1_name, arg2_type arg2_name, arg3_type arg3_name),         \
      (h->prof, arg1_name, arg2_name, arg3_name))
#define MONO_PROFILER_EVENT_4(name, type, arg1_type, arg1_name, arg2_type,     \
                              arg2_name, arg3_type, arg3_name, arg4_type,      \
                              arg4_name)                                       \
  _MONO_PROFILER_EVENT(name, type,                                             \
                       ...
                       (h->prof, arg1_name, arg2_name, arg3_name, arg4_name))
#define MONO_PROFILER_EVENT_5(name, type, arg1_type, arg1_name, arg2_type,     \
                              arg2_name, arg3_type, arg3_name, arg4_type,      \
                              arg4_name, arg5_type, arg5_name)                 \
  _MONO_PROFILER_EVENT(                                                        \
      ...
      (h->prof, arg1_name, arg2_name, arg3_name, arg4_name, arg5_name))
#include <mono/metadata/profiler-events.h>
#undef MONO_PROFILER_EVENT_0
#undef MONO_PROFILER_EVENT_1
#undef MONO_PROFILER_EVENT_2
...
static void shutdown_cb(MonoProfiler *prof) {
  prof->shutdown_callback(prof->profiler);
}

void mono_profiler_install(MonoLegacyProfiler *prof,
  ...
  if (callback)
    mono_profiler_set_runtime_shutdown_end_callback(current->handle,
                                                    shutdown_cb);
}

static void thread_start_cb(MonoProfiler *prof, uintptr_t tid) {
  prof->thread_start(prof->profiler, tid);
}

static void thread_stop_cb(MonoProfiler *prof, uintptr_t tid) {
  prof->thread_end(prof->profiler, tid);
}

void mono_profiler_install_thread(MonoLegacyProfileThreadFunc start,
...
static void gc_event_cb(MonoProfiler *prof, MonoProfilerGCEvent event,
                        uint32_t generation, gboolean is_serial) {
  prof->gc_event(prof->profiler, event, generation);
}

static void gc_resize_cb(MonoProfiler *prof, uintptr_t size) {
  prof->gc_heap_resize(prof->profiler, size);
}

void mono_profiler_install_gc(
    MonoLegacyProfileGCFunc callback,
...
static void jit_done_cb(MonoProfiler *prof, MonoMethod *method,
                        MonoJitInfo *jinfo) {
  prof->jit_end2(prof->profiler, method, jinfo, 0);
}

static void jit_failed_cb(MonoProfiler *prof, MonoMethod *method) {
  prof->jit_end2(prof->profiler, method, NULL, 1);
}

void mono_profiler_install_jit_end(MonoLegacyProfileJitResult end) {
...
void mono_profiler_set_events(int flags) { /* Do nothing. */ }

static void allocation_cb(MonoProfiler *prof, MonoObject *object) {
  prof->allocation(prof->profiler, object, object->vtable->klass);
}

void mono_profiler_install_allocation(MonoLegacyProfileAllocFunc callback) {
...
static void enter_cb(MonoProfiler *prof, MonoMethod *method,
                     MonoProfilerCallContext *context) {
  prof->enter(prof->profiler, method);
}

static void leave_cb(MonoProfiler *prof, MonoMethod *method,
                     MonoProfilerCallContext *context) {
  prof->leave(prof->profiler, method);
}

static void tail_call_cb(MonoProfiler *prof, MonoMethod *method,
                         MonoMethod *target) {
  prof->leave(prof->profiler, method);
}

void mono_profiler_install_enter_leave(MonoLegacyProfileMethodFunc enter,
...
static void throw_callback_cb(MonoProfiler *prof, MonoObject *exception) {
  prof->throw_callback(prof->profiler, exception);
}

static void exc_method_leave_cb(MonoProfiler *prof, MonoMethod *method,
                                MonoObject *exception) {
  prof->exc_method_leave(prof->profiler, method);
}

static void clause_callback_cb(MonoProfiler *prof, MonoMethod *method,
                               uint32_t index, MonoExceptionEnum type,
                               MonoObject *exception) {
...
void mono_profiler_install_exception(
  ...
  if (throw_callback)
    mono_profiler_set_exception_throw_callback(current->handle,
                                               throw_callback_cb);

  if (exc_method_leave)
    mono_profiler_set_method_exception_leave_callback(current->handle,
                                                      exc_method_leave_cb);

  if (clause_callback)
    mono_profiler_set_exception_clause_callback(current->handle,
                                                clause_callback_cb);
}
