...
static void encode_type(MonoDynamicImage *assembly, MonoType *type,
                        SigBuffer *buf);
...
static void sigbuffer_init(SigBuffer *buf, int size) {
...
static void sigbuffer_make_room(SigBuffer *buf, int size) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
static void sigbuffer_add_value(SigBuffer *buf, guint32 val) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
static void sigbuffer_add_byte(SigBuffer *buf, guint8 val) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
static void sigbuffer_add_mem(SigBuffer *buf, char *p, guint32 size) {
  MONO_REQ_GC_NEUTRAL_MODE;
...
static void sigbuffer_free(SigBuffer *buf) {
  MONO_REQ_GC_NEUTRAL_MODE;

  g_free(buf->buf);
}

static guint32 sigbuffer_add_to_blob_cached(MonoDynamicImage *assembly,
                                            SigBuffer *buf) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  return mono_dynamic_image_add_to_blob_cached(assembly, blob_size,
                                               b - blob_size, buf->buf, size);
}

static void encode_generic_class(MonoDynamicImage *assembly,
                                 MonoGenericClass *gclass, SigBuffer *buf) {
  MONO_REQ_GC_NEUTRAL_MODE;
  ...
  sigbuffer_add_value(buf, mono_dynimage_encode_typedef_or_ref_full(
                               assembly, m_class_get_byval_arg(klass), FALSE));
...
static void encode_type(MonoDynamicImage *assembly, MonoType *type,
  ...
  case MONO_TYPE_CLASS: {
    ...
    if (mono_class_is_gtd(k)) {
      MonoGenericClass *gclass = mono_metadata_lookup_generic_class(
          k, mono_class_get_generic_container(k)->context.class_inst, TRUE);
       ...
       */
      sigbuffer_add_value(
          buf, mono_image_typedef_or_ref(assembly, m_class_get_byval_arg(k)));
    }
    break;
  }
...
static void encode_reflection_type(MonoDynamicImage *assembly,
                                   MonoReflectionTypeHandle type,
                                   SigBuffer *buf, MonoError *error) {
...
static void encode_reflection_type_raw(MonoDynamicImage *assembly,
                                       MonoReflectionType *type_raw,
                                       SigBuffer *buf, MonoError *error) {
  HANDLE_FUNCTION_ENTER(); /* FIXME callers of encode_reflection_type_raw should
                              use handles */
  error_init(error);
...
static void encode_custom_modifiers(MonoDynamicImage *assembly,
                                    ...
                                    MonoArrayHandle modopt, SigBuffer *buf,
                                    MonoError *error) {
   ...
   *
   * we emit:
   * cmod_opt [encoding of D] cmod_opt [encoding of C] cmod_req [encoding of B]
   * cmod_req [encoding of A] I4.
   *
...
static void encode_custom_modifiers_raw(MonoDynamicImage *assembly,
                                        MonoArray *modreq_raw,
                                        MonoArray *modopt_raw, SigBuffer *buf,
                                        MonoError *error) {
  HANDLE_FUNCTION_ENTER(); /* FIXME callers of encode_custom_modifiers_raw
                              should use handles */
  error_init(error);
...
guint32 mono_dynimage_encode_method_signature(MonoDynamicImage *assembly,
...
guint32 mono_dynimage_encode_method_builder_signature(
    MonoDynamicImage *assembly, ReflectionMethodBuilder *mb, MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  int i;
  guint32 nparams =
      mb->parameters ? mono_array_length_internal(mb->parameters) : 0;
  guint32 ngparams =
      mb->generic_params ? mono_array_length_internal(mb->generic_params) : 0;
  guint32 notypes =
      mb->opt_types ? mono_array_length_internal(mb->opt_types) : 0;
  guint32 idx;
  ...
  encode_custom_modifiers_raw(assembly, mb->return_modreq, mb->return_modopt,
                              &buf, error);
  goto_if_nok(error, leave);
...
guint32 mono_dynimage_encode_locals(MonoDynamicImage *assembly,
  ...
  for (i = 0; i < nl; ++i) {
    MonoReflectionLocalBuilder *lb =
        mono_array_get_internal(ilgen->locals, MonoReflectionLocalBuilder *, i);

    if (lb->is_pinned)
      sigbuffer_add_value(&buf, MONO_TYPE_PINNED);

    encode_reflection_type_raw(assembly, (MonoReflectionType *)lb->type, &buf,
                               error);
    if (!is_ok(error)) {
      sigbuffer_free(&buf);
      return 0;
  ...
  idx = GPOINTER_TO_UINT(g_hash_table_lookup(assembly->standalonesig_cache,
  ...
  g_hash_table_insert(assembly->standalonesig_cache, GUINT_TO_POINTER(sig_idx),
 ...
 */
#if G_BYTE_ORDER != G_LITTLE_ENDIAN
static void swap_with_size(char *dest, const char *val, int len, int nelem) {
  MONO_REQ_GC_NEUTRAL_MODE;
  int elem;
...
guint32 mono_dynimage_encode_constant(MonoDynamicImage *assembly,
                                      MonoObject *val, MonoTypeEnum *ret_type) {
  MONO_REQ_GC_UNSAFE_MODE;
...
handle_enum:
    ...
    if (m_class_is_enumtype(klass)) {
      *ret_type = mono_class_enum_basetype_internal(klass)->type;
      goto handle_enum;
               ...
               strcmp(m_class_get_name(klass), "DateTime") == 0) {
      len = 8;
    } else
      g_error(
  ...
  case MONO_TYPE_STRING: {
...
#if G_BYTE_ORDER != G_LITTLE_ENDIAN
    {
      ...
      idx = mono_dynamic_image_add_to_blob_cached(assembly, blob_size,
                                                  b - blob_size, swapped, len);
      g_free(swapped);
    }
#else
    idx = mono_dynamic_image_add_to_blob_cached(
        assembly, blob_size, b - blob_size, mono_string_chars_internal(str),
        len);
#endif
  ...
  case MONO_TYPE_GENERICINST:
    *ret_type =
        m_class_get_byval_arg(
            mono_class_get_generic_class(val->vtable->klass)->container_class)
            ->type;
    goto handle_enum;
  default:
...
guint32 mono_dynimage_encode_field_signature(MonoDynamicImage *assembly,
  ...
  if (typespec) {
    ...
    gclass = mono_metadata_lookup_generic_class(
        klass, mono_class_get_generic_container(klass)->context.class_inst,
        TRUE);
    encode_generic_class(assembly, gclass, &buf);
  } else {
...
#ifndef DISABLE_REFLECTION_EMIT
/*field_image is the image to which the eventual custom mods have been encoded
 * against*/
guint32 mono_dynimage_encode_fieldref_signature(MonoDynamicImage *assembly,
    ...
    for (i = 0; i < cmods->count; ++i) {
      if (field_image) {
        ...
        MonoClass *klass = mono_class_get_checked(
            field_image, cmods->modifiers[i].token, error);
        g_assert(is_ok(error)); /* FIXME don't swallow the error */

        token =
            mono_image_typedef_or_ref(assembly, m_class_get_byval_arg(klass));
      } else {
...
static guint32 create_typespec(MonoDynamicImage *assembly, MonoType *type) {
  ...
  token =
      MONO_TYPEDEFORREF_TYPESPEC | (table->next_idx << MONO_TYPEDEFORREF_BITS);
  g_hash_table_insert(assembly->typespec, type, GUINT_TO_POINTER(token));
  table->next_idx++;
  return token;
}

guint32 mono_dynimage_encode_typedef_or_ref_full(MonoDynamicImage *assembly,
                                                 MonoType *type,
   ...
   */
  if ((m_class_get_image(klass) == &assembly->image) &&
      (type->type != MONO_TYPE_VAR) && (type->type != MONO_TYPE_MVAR)) {
    token = MONO_TYPEDEFORREF_TYPEDEF |
            (MONO_HANDLE_GETVAL(tb, table_idx) << MONO_TYPEDEFORREF_BITS);
    /* This function is called multiple times from sre and sre-save, so same
     * object is okay */
    mono_dynamic_image_register_token(assembly, token,
                                      MONO_HANDLE_CAST(MonoObject, tb),
                                      MONO_DYN_IMAGE_TOK_SAME_OK);
    goto leave;
  }

  if (m_class_get_nested_in(klass)) {
    enclosing = mono_dynimage_encode_typedef_or_ref_full(
        assembly, m_class_get_byval_arg(m_class_get_nested_in(klass)), FALSE);
    ...
    scope = mono_reflection_resolution_scope_from_image(
        assembly, m_class_get_image(klass));
  }
  ...
  if (assembly->save) {
    ...
    values[MONO_TYPEREF_NAME] =
        mono_dynstream_insert_string(&assembly->sheap, m_class_get_name(klass));
    values[MONO_TYPEREF_NAMESPACE] = mono_dynstream_insert_string(
        &assembly->sheap, m_class_get_name_space(klass));
  }
  token = MONO_TYPEDEFORREF_TYPEREF |
          (table->next_idx << MONO_TYPEDEFORREF_BITS); /* typeref */
...
guint32 mono_dynimage_encode_generic_method_sig(MonoDynamicImage *assembly,
...
#ifndef DISABLE_REFLECTION_EMIT
static gboolean encode_sighelper_arg(MonoDynamicImage *assembly, int i,
  ...
  if (!MONO_HANDLE_IS_NULL(helper_modreqs) &&
      (i < mono_array_handle_length(helper_modreqs)))
    MONO_HANDLE_ARRAY_GETREF(modreqs, helper_modreqs, i);
  if (!MONO_HANDLE_IS_NULL(helper_modopts) &&
      (i < mono_array_handle_length(helper_modopts)))
...
guint32
mono_dynimage_encode_reflection_sighelper(MonoDynamicImage *assembly,
  ...
  if (MONO_HANDLE_GETVAL(helper, call_conv) & 0x40)
    MONO_HANDLE_SETVAL(helper, call_conv, guint32,
                       MONO_HANDLE_GETVAL(helper, call_conv) & 0x20);

  if (MONO_HANDLE_GETVAL(helper, call_conv) == 0) { /* Unmanaged */
    idx = MONO_HANDLE_GETVAL(helper, unmanaged_call_conv) - 1;
  } else {
    /* Managed */
    idx = MONO_HANDLE_GETVAL(helper, call_conv) &
  ...
  encode_reflection_type(
      assembly, MONO_HANDLE_NEW_GET(MonoReflectionType, helper, return_type),
      &buf, error);
  goto_if_nok(error, fail);
  ...
  for (i = 0; i < nargs; ++i) {
    if (!encode_sighelper_arg(assembly, i, arguments, modreqs, modopts, &buf,
                              error))
      goto fail;
  }
  idx = sigbuffer_add_to_blob_cached(assembly, &buf);
...
static gboolean encode_reflection_types(MonoDynamicImage *assembly,
                                        MonoArrayHandle sig_arguments, int i,
                                        SigBuffer *buf, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
...
reflection_sighelper_get_signature_local(MonoReflectionSigHelperHandle sig,
                                         MonoError *error) {
  MonoReflectionModuleBuilderHandle module =
      MONO_HANDLE_NEW_GET(MonoReflectionModuleBuilder, sig, module);
  MonoDynamicImage *assembly = MONO_HANDLE_IS_NULL(module)
                                   ? NULL
                                   : MONO_HANDLE_GETVAL(module, dynamic_image);
  MonoArrayHandle sig_arguments =
      MONO_HANDLE_NEW_GET(MonoArray, sig, arguments);
  guint32 na = MONO_HANDLE_IS_NULL(sig_arguments)
                   ? 0
                   : mono_array_handle_length(sig_arguments);
  guint32 buflen, i;
  SigBuffer buf;
...
static MonoArrayHandle
reflection_sighelper_get_signature_field(MonoReflectionSigHelperHandle sig,
                                         MonoError *error) {
  MonoReflectionModuleBuilderHandle module =
      MONO_HANDLE_NEW_GET(MonoReflectionModuleBuilder, sig, module);
...
guint32 mono_dynimage_save_encode_marshal_blob(MonoDynamicImage *assembly,
  ...
  case MONO_NATIVE_LPARRAY:
    if (minfo->eltype || minfo->has_size) {
      ...
      if (minfo->has_size) {
        sigbuffer_add_value(&buf,
                            minfo->param_num != -1 ? minfo->param_num : 0);
  ...
  case MONO_NATIVE_CUSTOM:
    ...
    if (minfo->marshaltype || minfo->marshaltyperef) {
      if (minfo->marshaltyperef) {
        MonoType *marshaltype = mono_reflection_type_get_handle(
...
guint32
mono_dynimage_save_encode_property_signature(MonoDynamicImage *assembly,
                                             MonoReflectionPropertyBuilder *fb,
                                             MonoError *error) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  sigbuffer_add_value(&buf, nparams);
  if (mb) {
    encode_reflection_type_raw(assembly, (MonoReflectionType *)mb->rtype, &buf,
                               error);
    if (!is_ok(error))
      goto fail;
    for (i = 0; i < nparams; ++i) {
      MonoReflectionType *pt =
          mono_array_get_internal(mb->parameters, MonoReflectionType *, i);
      encode_reflection_type_raw(assembly, pt, &buf, error);
      if (!is_ok(error))
        goto fail;
    }
    ...
    encode_reflection_type_raw(
        assembly,
        mono_array_get_internal(smb->parameters, MonoReflectionType *, nparams),
        &buf, error);
    if (!is_ok(error))
      goto fail;
    ...
    for (i = 0; i < nparams; ++i) {
      MonoReflectionType *pt =
          mono_array_get_internal(smb->parameters, MonoReflectionType *, i);
      encode_reflection_type_raw(assembly, pt, &buf, error);
      if (!is_ok(error))
        goto fail;
    }
  } else {
    encode_reflection_type_raw(assembly, (MonoReflectionType *)fb->type, &buf,
                               error);
    if (!is_ok(error))
      goto fail;
  }
...
#ifndef DISABLE_REFLECTION_EMIT
MonoArrayHandle
ves_icall_SignatureHelper_get_signature_local(MonoReflectionSigHelperHandle sig,
                                              MonoError *error) {
  error_init(error);
...
MonoArrayHandle
ves_icall_SignatureHelper_get_signature_field(MonoReflectionSigHelperHandle sig,
                                              MonoError *error) {
  error_init(error);
