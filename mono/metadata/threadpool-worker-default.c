...
#define COUNTER_ATOMIC(var, block)                                             \
  do {                                                                         \
    ...
    do {                                                                       \
      ...
      { block; }                                                               \
      COUNTER_CHECK(var);                                                      \
    } while (mono_atomic_cas_i64(&worker.counters.as_gint64, (var).as_gint64,  \
                                 __old.as_gint64) != __old.as_gint64);         \
  } while (0)

static ThreadPoolWorkerCounter COUNTER_READ(void) {
  ThreadPoolWorkerCounter counter;
  counter.as_gint64 = mono_atomic_load_i64(&worker.counters.as_gint64);
  return counter;
}

static gint16 counter_num_active(ThreadPoolWorkerCounter counter) {
  gint16 num_active = counter._.starting + counter._.working + counter._.parked;
  g_assert(num_active >= 0);
  return num_active;
}

static guint32 rand_next(guint32 min, guint32 max) {
  ERROR_DECL(error);
...
void mono_threadpool_worker_init(MonoThreadPoolWorkerCallback callback) {
  ...
  hc->thread_magnitude_multiplier =
      (gdouble)HILL_CLIMBING_WAVE_MAGNITUDE_MULTIPLIER;
  ...
  hc->target_signal_to_noise_ratio =
      (gdouble)HILL_CLIMBING_TARGET_SIGNAL_TO_NOISE_RATIO;
  ...
  hc->throughput_error_smoothing_factor =
      (gdouble)HILL_CLIMBING_ERROR_SMOOTHING_FACTOR;
  ...
  hc->current_sample_interval =
      rand_next(hc->sample_interval_low, hc->sample_interval_high);
  ...
  worker.limit_worker_max = CLAMP(threads_count * 100, MIN(threads_count, 200),
                                  MAX(threads_count, 200));
#else
...
void mono_threadpool_worker_cleanup(void) { mono_refcount_dec(&worker); }

static void work_item_push(void) {
  gint32 old, new_;
...
static gboolean work_item_try_pop(void) {
  gint32 old, new_;
...
static gboolean worker_park(void) {
  ...
  if (!mono_runtime_is_shutting_down()) {
    ...
    do {
      ...
      new_ = old + 1;
    } while (mono_atomic_cas_i32(&worker.parked_threads_count, new_, old) !=
             old);

    switch (mono_coop_sem_timedwait(&worker.parked_threads_sem,
                                    rand_next(5 * 1000, 60 * 1000),
    ...
    if (interrupted || timeout) {
      /* If the semaphore was posted, then worker.parked_threads_count was
       * decremented in worker_try_unpark */
      do {
        old = mono_atomic_load_i32(&worker.parked_threads_count);
        g_assert(old > G_MININT32);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_THREADPOOL,
             "[%p] worker unparking, timeout? %s interrupted? %s",
             GUINT_TO_POINTER(
                 MONO_NATIVE_THREAD_ID_TO_UINT(mono_native_thread_id_get())),
             timeout ? "yes" : "no", interrupted ? "yes" : "no");

  return timeout;
}

static gboolean worker_try_unpark(void) {
  gboolean res = TRUE;
  gint32 old, new_;
...
static void
hill_climbing_force_change(gint16 new_thread_count,
                           ThreadPoolHeuristicStateTransition transition);

static gsize WINAPI worker_thread(gpointer unused) {
  ...
  while (!mono_runtime_is_shutting_down()) {
    ...
    // not be interrupted at runtime shutdown, because an abort is already
    // requested. Clear the abort request. This avoids a shutdown hang in tests
    // thread6 and thread7.
  ...
  if (worker_timed_out) {
    ...
    COUNTER_ATOMIC(counter, {
      decr_max_working =
          MAX(worker.limit_worker_min,
              MIN(counter_num_active(counter), counter._.max_working));
      counter._.max_working = decr_max_working;
    });
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_THREADPOOL,
               "[%p] worker timed out, starting = %d working = %d parked = %d, "
               "setting max_working to %d",
               GUINT_TO_POINTER(
                   MONO_NATIVE_THREAD_ID_TO_UINT(mono_native_thread_id_get())),
...
static gboolean worker_try_create(void) {
  ERROR_DECL(error);
  MonoInternalThread *thread;
  ...
  if (0 == current_ticks) {
    ...
    if (worker.worker_creation_current_second != now) {
      ...
      g_assert(worker.worker_creation_current_count <=
               WORKER_CREATION_MAX_PER_SEC);
      if (worker.worker_creation_current_count == WORKER_CREATION_MAX_PER_SEC) {
        mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_THREADPOOL,
                   "[%p] try create worker, failed: maximum number of worker "
                   "created per second reached, current count = %d",
                   GUINT_TO_POINTER(MONO_NATIVE_THREAD_ID_TO_UINT(
                       mono_native_thread_id_get())),
                   worker.worker_creation_current_count);
  ...
  COUNTER_ATOMIC(counter, {
    if (counter._.working >= counter._.max_working) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_THREADPOOL,
                 "[%p] try create worker, failed: maximum number of working "
                 "threads reached",
                 GUINT_TO_POINTER(MONO_NATIVE_THREAD_ID_TO_UINT(
                     mono_native_thread_id_get())));
      mono_coop_mutex_unlock(&worker.worker_creation_lock);
  ...
  thread = mono_thread_create_internal(
      mono_get_root_domain(), (gpointer)worker_thread, NULL,
      MONO_THREAD_CREATE_FLAGS_THREADPOOL, error);
  if (!thread) {
    mono_trace(
        G_LOG_LEVEL_DEBUG, MONO_TRACE_THREADPOOL,
        "[%p] try create worker, failed: could not create thread due to %s",
        GUINT_TO_POINTER(
            MONO_NATIVE_THREAD_ID_TO_UINT(mono_native_thread_id_get())),
        mono_error_get_message(error));
    mono_error_cleanup(error);

    COUNTER_ATOMIC(counter, { counter._.starting--; });
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_THREADPOOL,
             ...
             GUINT_TO_POINTER(
                 MONO_NATIVE_THREAD_ID_TO_UINT(mono_native_thread_id_get())),
             (gpointer)thread->tid, now, worker.worker_creation_current_count);
...
static gboolean monitor_should_keep_running(void) {
  static gint64 last_should_keep_running = -1;

  g_assert(worker.monitor_status == MONITOR_STATUS_WAITING_FOR_REQUEST ||
           worker.monitor_status == MONITOR_STATUS_REQUESTED);

  if (mono_atomic_xchg_i32(&worker.monitor_status,
                           MONITOR_STATUS_WAITING_FOR_REQUEST) ==
      MONITOR_STATUS_WAITING_FOR_REQUEST) {
    ...
    if (mono_runtime_is_shutting_down()) {
      ...
      if (!should_keep_running) {
        if (last_should_keep_running == -1 ||
            mono_100ns_ticks() - last_should_keep_running <
                MONITOR_MINIMAL_LIFETIME * 1000 * 10) {
    ...
    if (should_keep_running) {
      ...
      if (mono_atomic_cas_i32(&worker.monitor_status,
                              MONITOR_STATUS_NOT_RUNNING,
                              MONITOR_STATUS_WAITING_FOR_REQUEST) ==
          MONITOR_STATUS_WAITING_FOR_REQUEST)
        return FALSE;
...
static gboolean monitor_sufficient_delay_since_last_dequeue(void) {
  gint64 threshold;
...
static gsize WINAPI monitor_thread(gpointer unused) {
  ...
  do {
    ...
    gint32 awake = 0; /* number of spurious awakes we tolerate before doing a
    ...
    if (limit_worker_max_reached) {
      mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_THREADPOOL,
                 "[%p] monitor thread, limit_worker_max (%d) reached",
                 GUINT_TO_POINTER(MONO_NATIVE_THREAD_ID_TO_UINT(
                     mono_native_thread_id_get())),
      ...
      if (active_max_reached)
        ...
        mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_THREADPOOL,
                   "[%p] monitor thread, num_active (%d) < max_working, "
                   "allowing active thread increase",
                   GUINT_TO_POINTER(MONO_NATIVE_THREAD_ID_TO_UINT(
                       mono_native_thread_id_get())),
...
static void monitor_ensure_running(void) {
  ...
  for (;;) {
    ...
    case MONITOR_STATUS_WAITING_FOR_REQUEST:
      ...
      mono_atomic_cas_i32(&worker.monitor_status, MONITOR_STATUS_REQUESTED,
                          MONITOR_STATUS_WAITING_FOR_REQUEST);
      break;
    case MONITOR_STATUS_NOT_RUNNING:
      ...
      if (mono_runtime_is_shutting_down())
        return;
      if (mono_atomic_cas_i32(&worker.monitor_status, MONITOR_STATUS_REQUESTED,
                              MONITOR_STATUS_NOT_RUNNING) ==
          MONITOR_STATUS_NOT_RUNNING) {
        // printf ("monitor_thread: creating\n");
        if (!mono_thread_create_internal(
                mono_get_root_domain(), (gpointer)monitor_thread, NULL,
                (MonoThreadCreateFlags)(MONO_THREAD_CREATE_FLAGS_THREADPOOL |
...
static void hill_climbing_change_thread_count(
    gint16 new_thread_count, ThreadPoolHeuristicStateTransition transition) {
  ThreadPoolHillClimbing *hc;
...
static double_complex hill_climbing_get_wave_component(gdouble *samples,
                                                       guint sample_count,
                                                       gdouble period) {
  ThreadPoolHillClimbing *hc;
  ...
  for (i = 0; i < sample_count; ++i) {
    q0 = coeff * q1 - q2 +
         samples[(hc->total_samples - sample_count + i) %
                 hc->samples_to_measure];
    q2 = q1;
    q1 = q0;
  }

  return mono_double_complex_scalar_div(
      mono_double_complex_make(q1 - q2 * cosine, (q2 * sine)),
      ((gdouble)sample_count));
}

static gint16 hill_climbing_update(gint16 current_thread_count,
                                   guint32 sample_duration, gint32 completions,
                                   gint64 *adjustment_interval) {
  ThreadPoolHillClimbing *hc;
  ThreadPoolHeuristicStateTransition transition;
  gdouble throughput;
  ...
  /* If someone changed the thread count without telling us, update our records
   * accordingly. */
  ...
  /* We need to make sure we're collecting reasonably accurate data. Since we're
   * just counting the end of each work item, we are goinng to be missing some
   * data about what really happened during the sample interval. The count
   * produced by each thread includes an initial work item that may have started
   * well before the start of the interval, and each thread may have been
   * running some new work item for some time before the end of the interval,
   * which did not yet get counted. So our count is going to be off by +/-
   * threadCount workitems.
   *
   ...
   * filter out this error, because of the way it accumulates over time. If this
   * sample is off by, say, 33% in the negative direction, then the next one
   * likely will be too. The one after that will include the sum of the
   * completions we missed in the previous samples, and so will be 33% positive.
   * So every three samples we'll have two "low" samples and one "high" sample.
   * This will appear as periodic variation right in the frequency range we're
   * targeting, which will not be filtered by the frequency-domain translation.
   */
  if (hc->total_samples > 0 &&
      ((current_thread_count - 1.0) / completions) >= hc->max_sample_error) {
  ...
  /* We've got enouugh data for our sample; reset our accumulators for next
   * time. */
  hc->accumulated_sample_duration = 0;
  hc->accumulated_completion_count = 0;
  ...
  sample_count = ((gint)MIN(hc->total_samples - 1, hc->samples_to_measure) /
                  hc->wave_period) *
                 hc->wave_period;
  ...
  if (sample_count > hc->wave_period) {
    ...
    /* Average the throughput and thread count samples, so we can scale the wave
     * magnitudes later. */
    for (i = 0; i < sample_count; ++i) {
    ...
    if (average_throughput > 0 && average_thread_count > 0) {
      ...
      adjacent_period_1 =
          sample_count /
          (((gdouble)sample_count) / ((gdouble)hc->wave_period) + 1);
      adjacent_period_2 =
          sample_count /
          (((gdouble)sample_count) / ((gdouble)hc->wave_period) - 1);
      ...
      throughput_wave_component = mono_double_complex_scalar_div(
          hill_climbing_get_wave_component(hc->samples, sample_count,
                                           hc->wave_period),
          average_throughput);
      throughput_error_estimate = mono_cabs(mono_double_complex_scalar_div(
          hill_climbing_get_wave_component(hc->samples, sample_count,
                                           adjacent_period_1),
          average_throughput));
       ...
       * exact measurements. */
      thread_wave_component = mono_double_complex_scalar_div(
          hill_climbing_get_wave_component(hc->thread_counts, sample_count,
                                           hc->wave_period),
      ...
      if (hc->average_throughput_noise == 0) {
        ...
        hc->average_throughput_noise =
            (hc->throughput_error_smoothing_factor *
             throughput_error_estimate) +
            ((1.0 + hc->throughput_error_smoothing_factor) *
             hc->average_throughput_noise);
      }
      ...
      if (mono_cabs(thread_wave_component) > 0) {
        ...
        ratio = mono_double_complex_div(
            mono_double_complex_sub(
                throughput_wave_component,
                mono_double_complex_scalar_mul(thread_wave_component,
                                               hc->target_throughput_ratio)),
      ...
      noise_for_confidence =
          MAX(hc->average_throughput_noise, throughput_error_estimate);
      if (noise_for_confidence > 0) {
        confidence = mono_cabs(thread_wave_component) / noise_for_confidence /
                     hc->target_signal_to_noise_ratio;
      } else {
        /* there is no noise! */
        confidence = 1.0;
   ...
   * up. If they're 180 degrees out of phase, we'll move backward (because this
   * indicates that our changes are having the opposite of the intended effect).
   * If they're 90 degrees out of phase, we won't move at all, because we can't
   * tell wether we're having a negative or positive effect on throughput. */
   ...
   * far away from the target, but more slowly
   * if we're getting close, giving us rapid ramp-up without wild oscillations
   * around the target. */
   ...
   * average we've been keeping of the
   * throughput error.  This average starts at zero, so we'll start with a nice
   * safe little wave at first. */
  new_thread_wave_magnitude =
      (gint)(0.5 + (hc->current_control_setting * hc->average_throughput_noise *
                    hc->target_signal_to_noise_ratio *
                    hc->thread_magnitude_multiplier * 2.0));
  new_thread_wave_magnitude =
      CLAMP(new_thread_wave_magnitude, 1, hc->max_thread_wave_magnitude);

  /* Make sure our control setting is within the ThreadPoolWorker's limits. */
  hc->current_control_setting =
      CLAMP(hc->current_control_setting, worker.limit_worker_min,
            worker.limit_worker_max - new_thread_wave_magnitude);

  /* Calculate the new thread count (control setting + square wave). */
  new_thread_count =
      (gint)(hc->current_control_setting +
             new_thread_wave_magnitude *
                 ((hc->total_samples / (hc->wave_period / 2)) % 2));

  /* Make sure the new thread count doesn't exceed the ThreadPoolWorker's
   * limits. */
  new_thread_count =
      CLAMP(new_thread_count, worker.limit_worker_min, worker.limit_worker_max);
  ...
  if (mono_creal(ratio) < 0.0 && new_thread_count == worker.limit_worker_min)
    *adjustment_interval =
        (gint)(0.5 + hc->current_sample_interval *
                         (10.0 * MAX(-1.0 * mono_creal(ratio), 1.0)));
  else
...
static gboolean heuristic_should_adjust(void) {
  if (worker.heuristic_last_dequeue >
      worker.heuristic_last_adjustment + worker.heuristic_adjustment_interval) {
...
static void heuristic_adjust(void) {
  if (mono_coop_mutex_trylock(&worker.heuristic_lock) == 0) {
    ...
    if (sample_duration >= worker.heuristic_adjustment_interval / 2) {
      ...
      gint16 const new_thread_count = hill_climbing_update(
          counter._.max_working, sample_duration, completions,
          &worker.heuristic_adjustment_interval);

      COUNTER_ATOMIC(counter, { counter._.max_working = new_thread_count; });

      /* FIXME: this can never be true. we only leave COUNTER_ATOMIC() if the
       * assignment and CAS succeeded */
...
gboolean mono_threadpool_worker_notify_completed(void) {
...
gint32 mono_threadpool_worker_get_min(void) {
  gint32 ret;
...
gboolean mono_threadpool_worker_set_min(gint32 value) {
...
gint32 mono_threadpool_worker_get_max(void) {
  gint32 ret;
...
gboolean mono_threadpool_worker_set_max(gint32 value) {
  gint32 cpu_count;
...
void mono_threadpool_worker_set_suspended(gboolean suspended) {
  if (!mono_refcount_tryinc(&worker))
    return;
