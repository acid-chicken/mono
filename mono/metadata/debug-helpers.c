...
static void append_class_name(GString *res, MonoClass *klass,
...
static MonoClass *find_system_class(const char *name) {
  if (!strcmp(name, "void"))
...
static void mono_custom_modifiers_get_desc(GString *res, const MonoType *type,
                                           gboolean include_namespace) {
  ERROR_DECL(error);
...
void mono_type_get_desc(GString *res, MonoType *type,
                        gboolean include_namespace) {
  int i;
  ...
  case MONO_TYPE_VOID:
    ...
    break;
  case MONO_TYPE_CHAR:
    g_string_append(res, "char");
    break;
  case MONO_TYPE_BOOLEAN:
    g_string_append(res, "bool");
    break;
  case MONO_TYPE_U1:
    g_string_append(res, "byte");
    break;
  case MONO_TYPE_I1:
    g_string_append(res, "sbyte");
    break;
  case MONO_TYPE_U2:
    g_string_append(res, "uint16");
    break;
  case MONO_TYPE_I2:
    g_string_append(res, "int16");
    break;
  case MONO_TYPE_U4:
    g_string_append(res, "uint");
    break;
  case MONO_TYPE_I4:
    g_string_append(res, "int");
    break;
  case MONO_TYPE_U8:
    g_string_append(res, "ulong");
    break;
  case MONO_TYPE_I8:
    g_string_append(res, "long");
    break;
  case MONO_TYPE_FNPTR: /* who cares for the exact signature? */
    g_string_append(res, "*()");
    break;
  case MONO_TYPE_U:
    g_string_append(res, "uintptr");
    break;
  case MONO_TYPE_I:
    g_string_append(res, "intptr");
    break;
  case MONO_TYPE_R4:
    g_string_append(res, "single");
    break;
  case MONO_TYPE_R8:
    g_string_append(res, "double");
    break;
  ...
  case MONO_TYPE_ARRAY:
    mono_type_get_desc(res, &type->data.array->eklass->_byval_arg,
                       include_namespace);
    g_string_append_c(res, '[');
  ...
  case MONO_TYPE_GENERICINST: {
    ...
    mono_type_get_desc(res,
                       &type->data.generic_class->container_class->_byval_arg,
                       include_namespace);
    g_string_append(res, "<");
    ...
    if (context->class_inst) {
      for (i = 0; i < context->class_inst->type_argc; ++i) {
        if (i > 0)
          g_string_append(res, ", ");
        mono_type_get_desc(res, context->class_inst->type_argv[i],
                           include_namespace);
      }
    }
    if (context->method_inst) {
      ...
      for (i = 0; i < context->method_inst->type_argc; ++i) {
        if (i > 0)
          g_string_append(res, ", ");
        mono_type_get_desc(res, context->method_inst->type_argv[i],
                           include_namespace);
      }
    }
    g_string_append(res, ">");
    ...
    if (type->data.generic_param) {
      ...
      if (name)
        ...
        g_string_append_printf(
            res, "%s%d", type->type == MONO_TYPE_VAR ? "!" : "!!",
            mono_generic_param_num(type->data.generic_param));
    } else {
      g_string_append(res, "<unknown>");
    }
    break;
 ...
 *
 * Both classname and methodname can contain <code>*</code> which matches
 * anything.
 *
 ...
 *
 * Determines whether the specified \p method matches the provided \p desc
 * description.
 *
...
gboolean mono_method_desc_match(MonoMethodDesc *desc, MonoMethod *method) {
  ...
  if (desc->num_args != mono_method_signature_internal(method)->param_count)
    return FALSE;
  sig = mono_signature_get_desc(mono_method_signature_internal(method),
                                desc->include_namespace);
...
static const char *my_strrchr(const char *str, char ch, int *len) {
  int pos;
...
static gboolean match_class(MonoMethodDesc *desc, int pos, MonoClass *klass) {
  const char *p;
 ...
 */
gboolean mono_method_desc_is_full(MonoMethodDesc *desc) {
 ...
 * description, by making sure that the method matches both the class and the
 * method parameters.
 *
 * \returns TRUE if the specified method matches the specified description,
 * FALSE otherwise.
 */
 ...
 */
MonoMethod *mono_method_desc_search_in_class(MonoMethodDesc *desc,
                                             MonoClass *klass) {
  MonoMethod *m;
  gpointer iter = NULL;
 ...
 */
MonoMethod *mono_method_desc_search_in_image(MonoMethodDesc *desc,
                                             MonoImage *image) {
  MonoClass *klass;
...
static const unsigned char *dis_one(GString *str, MonoDisHelper *dh,
                                    MonoMethod *method, const unsigned char *ip,
  ...
  if (!header) {
    g_string_append_printf(str, "could not disassemble, bad header due to %s",
  ...
  case MonoInlineString: {
    ...
    if (!image_is_dynamic(method->klass->image) && !method_is_dynamic(method)) {
      token = read32(ip);
      blob = mono_metadata_user_string(method->klass->image,
...
static MonoDisHelper default_dh = {
 ...
 */
char *mono_disasm_code_one(MonoDisHelper *dh, MonoMethod *method,
                           const guchar *ip, const guchar **endp) {
  char *result;
 ...
 * mono_disasm_code:
 */
char *mono_disasm_code(MonoDisHelper *dh, MonoMethod *method, const guchar *ip,
                       const guchar *end) {
  char *result;
  GString *res = g_string_new("");
 ...
 * \returns the full name for the field, made up of the namespace, type name and
 * the field name.
 */
...
char *mono_method_get_name_full(MonoMethod *method, gboolean signature,
  ...
  if (signature) {
    ...
    if (ret && sig) {
      ...
      res =
          g_strdup_printf("%s%s %s%s%s%s%s(%s)", wrapper, ret_str, klass_desc,
                          class_method_separator, method->name,
                          inst_desc ? inst_desc : "", method_sig_space, tmpsig);
      g_free(ret_str);
    } else {
      res = g_strdup_printf(
          "%s%s%s%s%s%s(%s)", wrapper, klass_desc, class_method_separator,
...
char *mono_method_full_name(MonoMethod *method, gboolean signature) {
  ...
  res = mono_method_get_name_full(method, signature, FALSE,
...
char *mono_method_get_full_name(MonoMethod *method) {
  return mono_method_get_name_full(method, TRUE, TRUE,
                                   MONO_TYPE_NAME_FORMAT_IL);
}
 ...
 *
 * Returns the name of the method, including signature, using the same formating
 * as reflection.
 */
char *mono_method_get_reflection_name(MonoMethod *method) {
  return mono_method_get_name_full(method, TRUE, FALSE,
                                   MONO_TYPE_NAME_FORMAT_REFLECTION);
}

static const char *print_name_space(MonoClass *klass) {
  if (klass->nested_in) {
  ...
  if (klass == mono_defaults.string_class) {
    ...
    if (utf8) {
      g_print("String at %p, length: %d, '%s'\n", obj,
              mono_string_length_internal((MonoString *)obj), utf8);
    } else {
      g_print("String at %p, length: %d, unable to decode UTF16\n", obj,
              mono_string_length_internal((MonoString *)obj));
    }
    g_free(utf8);
  } else if (klass->rank) {
...
static void print_field_value(const char *field_ptr, MonoClassField *field,
                              int type_offset) {
  MonoType *type;
  g_print("At %p (ofs: %2d) %s: ", field_ptr, field->offset + type_offset,
  ...
  case MONO_TYPE_R8:
    ...
    break;
  case MONO_TYPE_BOOLEAN:
    g_print("%s (%d)\n", *(guint8 *)field_ptr ? "True" : "False",
            *(guint8 *)field_ptr);
    break;
  case MONO_TYPE_CHAR:
    g_print("'%c' (%d 0x%04x)\n", *(guint16 *)field_ptr, *(guint16 *)field_ptr,
            *(guint16 *)field_ptr);
    break;
  default:
    g_assert_not_reached();
...
static void objval_describe(MonoClass *klass, const char *addr) {
 ...
 *
 * Prints to stdout a small description of each static field of the type \p
 * klass in the current application domain. For use in a debugger.
 */
 ...
 */
void mono_method_print_code(MonoMethod *method) {
  ERROR_DECL(error);
  char *code;
  MonoMethodHeader *header = mono_method_get_header_checked(method, error);
  if (!header) {
  ...
  code = mono_disasm_code(0, method, header->code,
                          header->code + header->code_size);
