...
static MonoObject *mono_remoting_wrapper(MonoMethod *method, gpointer *params);

static MonoException *mono_remoting_update_exception(MonoException *exc);

static gint32 mono_marshal_set_domain_by_id(gint32 id, MonoBoolean push);

static gboolean mono_marshal_check_domain_image(gint32 domain_id,
                                                MonoImage *image);

MONO_API void mono_upgrade_remote_class_wrapper(MonoReflectionType *rtype,
                                                MonoTransparentProxy *tproxy);

static MonoXDomainMarshalType mono_get_xdomain_marshal_type(MonoType *t);

static void mono_marshal_xdomain_copy_out_value(MonoObject *src,
                                                MonoObject *dst);

static MonoReflectionType *type_from_handle(MonoType *handle);

static void mono_context_set_icall(MonoAppContext *new_context);

static MonoAppContext *mono_context_get_icall(void);

static MonoObject *mono_marshal_xdomain_copy_value(MonoObject *val_raw,
                                                   MonoError *error);

/* Class lazy loading functions */
static GENERATE_GET_CLASS_WITH_CACHE(remoting_services, "System.Runtime.Remoting", "RemotingServices") static GENERATE_GET_CLASS_WITH_CACHE(
    call_context, "System.Runtime.Remoting.Messaging",
    "CallContext") static GENERATE_GET_CLASS_WITH_CACHE(context,
                                                        "System.Runtime."
                                                        "Remoting.Contexts",
                                                        "Context")
...
#ifndef DISABLE_JIT
static MonoMethod *method_rs_serialize, *method_rs_deserialize,
    *method_exc_fixexc, *method_rs_appdomain_target;
static MonoMethod *method_set_call_context, *method_needs_context_sink,
    *method_rs_serialize_exc;
#endif

static gpointer mono_compile_method_icall(MonoMethod *method);
...
static GHashTable *get_cache_full(GHashTable **var, GHashFunc hash_func,
                                  GCompareFunc equal_func,
  ...
  if (!(*var)) {
    ...
    if (!(*var)) {
      GHashTable *cache = g_hash_table_new_full(
          hash_func, equal_func, key_destroy_func, value_destroy_func);
      mono_memory_barrier();
      *var = cache;
    }
    remoting_unlock();
...
static void mono_remoting_marshal_init(void) {
  ERROR_DECL(error);
  MonoClass *klass;
  ...
  method_rs_serialize = mono_class_get_method_from_name_checked(
      klass, "SerializeCallData", -1, 0, error);
  mono_error_assert_ok(error);
  g_assert(method_rs_serialize);
  method_rs_deserialize = mono_class_get_method_from_name_checked(
      klass, "DeserializeCallData", -1, 0, error);
  mono_error_assert_ok(error);
  g_assert(method_rs_deserialize);
  method_rs_serialize_exc = mono_class_get_method_from_name_checked(
      klass, "SerializeExceptionData", -1, 0, error);
  mono_error_assert_ok(error);
  g_assert(method_rs_serialize_exc);

  klass = mono_defaults.real_proxy_class;
  method_rs_appdomain_target = mono_class_get_method_from_name_checked(
      klass, "GetAppDomainTarget", -1, 0, error);
  mono_error_assert_ok(error);
  g_assert(method_rs_appdomain_target);

  klass = mono_defaults.exception_class;
  method_exc_fixexc = mono_class_get_method_from_name_checked(
      klass, "FixRemotingException", -1, 0, error);
  mono_error_assert_ok(error);
  g_assert(method_exc_fixexc);

  klass = mono_class_get_call_context_class();
  method_set_call_context = mono_class_get_method_from_name_checked(
      klass, "SetCurrentCallContext", -1, 0, error);
  mono_error_assert_ok(error);
  g_assert(method_set_call_context);

  klass = mono_class_get_context_class();
  method_needs_context_sink = mono_class_get_method_from_name_checked(
      klass, "get_NeedsContextSink", -1, 0, error);
  mono_error_assert_ok(error);
  ...
  if (!icalls_registered) {
    ...
    register_icall(mono_marshal_set_domain_by_id,
                   mono_icall_sig_int32_int32_int32, FALSE);
    register_icall(mono_marshal_check_domain_image,
                   mono_icall_sig_int32_int32_ptr, FALSE);
    register_icall(ves_icall_mono_marshal_xdomain_copy_value,
                   mono_icall_sig_object_object, FALSE);
    register_icall(mono_marshal_xdomain_copy_out_value,
                   mono_icall_sig_void_object_object, FALSE);
    register_icall(mono_remoting_wrapper, mono_icall_sig_object_ptr_ptr, FALSE);
    register_icall(mono_remoting_update_exception, mono_icall_sig_object_object,
                   FALSE);
    register_icall(mono_upgrade_remote_class_wrapper,
                   mono_icall_sig_void_object_object, FALSE);
...
#ifndef DISABLE_JIT
static int mono_mb_emit_proxy_check(MonoMethodBuilder *mb, int branch_code) {
  int pos;
  ...
  mono_mb_emit_i4(mb,
                  mono_mb_add_data(mb, mono_defaults.transparent_proxy_class));
  pos = mono_mb_emit_branch(mb, branch_code);
  return pos;
}

static int mono_mb_emit_xdomain_check(MonoMethodBuilder *mb, int branch_code) {
  int pos;
...
static int mono_mb_emit_contextbound_check(MonoMethodBuilder *mb,
                                           int branch_code) {
  static int offset = -1;
  static guint8 mask;
  ...
  mono_mb_emit_ldflda(mb,
                      MONO_STRUCT_OFFSET(MonoTransparentProxy, remote_class));
  mono_mb_emit_byte(mb, CEE_LDIND_REF);
...
static MonoMethod *mono_marshal_remoting_find_in_cache(MonoMethod *method,
                                                       int wrapper_type) {
  MonoMethod *res = NULL;
  ...
  if (mono_method_get_wrapper_cache(method)->remoting_invoke_cache)
    wrps = (MonoRemotingMethods *)g_hash_table_lookup(
        mono_method_get_wrapper_cache(method)->remoting_invoke_cache, method);
...
static MonoMethod *mono_remoting_mb_create_and_cache(MonoMethod *key,
  ...
  cache =
      get_cache_full(&mono_method_get_wrapper_cache(key)->remoting_invoke_cache,
                     mono_aligned_addr_hash, NULL, NULL, g_free);
...
static MonoObject *mono_remoting_wrapper(MonoMethod *method, gpointer *params) {
  ...
  if (mono_class_is_contextbound(this_obj->remote_class->proxy_class) &&
      this_obj->rp->context == (MonoObject *)mono_context_get()) {
    int i;
    ...
    res = mono_runtime_invoke_checked(
        method,
        m_class_is_valuetype(method->klass)
            ? mono_object_unbox_internal((MonoObject *)this_obj)
            : this_obj,
        mparams, error);
    goto_if_nok(error, fail);
...
static MonoException *mono_remoting_update_exception(MonoException *exc) {
  ...
  if (!(mono_class_get_flags(klass) & TYPE_ATTRIBUTE_SERIALIZABLE)) {
    MonoException *ret;
    char *aname = mono_stringify_assembly_name(
        &m_class_get_image(klass)->assembly->aname);
    char *message = g_strdup_printf(
        "Type '%s' in Assembly '%s' is not marked as serializable",
        m_class_get_name(klass), aname);
...
MonoMethod *mono_marshal_get_remoting_invoke(MonoMethod *method,
  ...
  if (method->wrapper_type == MONO_WRAPPER_REMOTING_INVOKE ||
    ...
    /* this seems to be the best plase to put this, as all remoting invokes seem
     * to get filtered through here */
#ifndef DISABLE_COM
  if (mono_class_is_com_object(method->klass) ||
      method->klass == mono_class_try_get_com_object_class()) {
    MonoVTable *vtable =
        mono_class_vtable_checked(mono_domain_get(), method->klass, error);
  ...
  if ((res = mono_marshal_remoting_find_in_cache(method,
...
static void mono_marshal_xdomain_copy_out_value(MonoObject *src,
  ...
  case MONO_TYPE_SZARRAY: {
    int mt = mono_get_xdomain_marshal_type(m_class_get_byval_arg(
        m_class_get_element_class(mono_object_class(src))));
    if (mt == MONO_MARSHAL_SERIALIZE)
      return;
    if (mt == MONO_MARSHAL_COPY) {
      int i, len = mono_array_length_internal((MonoArray *)dst);
      for (i = 0; i < len; i++) {
        MonoObject *item = (MonoObject *)mono_array_get_internal(
            (MonoArray *)src, gpointer, i);
...
static void mono_marshal_emit_xdomain_copy_value(MonoMethodBuilder *mb,
                                                 MonoClass *pclass) {
  mono_mb_emit_icall(mb, ves_icall_mono_marshal_xdomain_copy_value);
  mono_mb_emit_op(mb, CEE_CASTCLASS, pclass);
}

static void mono_marshal_emit_xdomain_copy_out_value(MonoMethodBuilder *mb,
                                                     MonoClass *pclass) {
 ...
 */
static gboolean mono_marshal_supports_fast_xdomain(MonoMethod *method) {
  return !mono_class_is_contextbound(method->klass) &&
         !((method->flags & METHOD_ATTRIBUTE_SPECIAL_NAME) &&
           (strcmp(".ctor", method->name) == 0));
}
...
#if !defined(DISABLE_JIT)
static void mono_marshal_emit_switch_domain(MonoMethodBuilder *mb) {
  mono_mb_emit_icall(mb, mono_marshal_set_domain_by_id);
}

gpointer mono_compile_method_icall(MonoMethod *method) {
  ERROR_DECL(error);
 ...
 * the current domain.
 */
static void mono_marshal_emit_load_domain_method(MonoMethodBuilder *mb,
                                                 MonoMethod *method) {
 ...
 * the current domain.
 */
static MonoMethod *mono_marshal_get_xappdomain_dispatch(MonoMethod *method,
                                                        int *marshal_types,
                                                        int complex_count,
  ...
  if ((res = mono_marshal_remoting_find_in_cache(
           method, MONO_WRAPPER_XDOMAIN_DISPATCH)))
    return res;

  sig = mono_method_signature_internal(method);
  copy_return = (sig->ret->type != MONO_TYPE_VOID &&
                 ret_marshal_type != MONO_MARSHAL_SERIALIZE);

  j = 0;
  csig = mono_metadata_signature_alloc(mono_defaults.corlib,
                                       3 + sig->param_count - complex_count);
  ...
  loc_serialized_exc =
      mono_mb_add_local(mb, m_class_get_byval_arg(byte_array_class));
  if (complex_count > 0)
  ...
  main_clause = (MonoExceptionClause *)mono_image_alloc0(
      m_class_get_image(method->klass), sizeof(MonoExceptionClause));
  ...
  for (i = 0; i < sig->param_count; i++) {
    ...
    case MONO_MARSHAL_COPY_OUT: {
      /* Keep a local copy of the value since we need to copy it back after the
       * call */
  ...
  for (i = 0; i < sig->param_count; i++) {
    ...
    if (marshal_types[i] == MONO_MARSHAL_COPY_OUT) {
      ...
      mono_mb_emit_ldarg(mb, param_index);
      mono_marshal_emit_xdomain_copy_out_value(
          mb, mono_class_from_mono_type_internal(sig->params[i]));
    }
    param_index++;
  }
  ...
  if (complex_out_count > 0) {
    ...
    if (ret_marshal_type == MONO_MARSHAL_SERIALIZE) {
      mono_mb_emit_ldloc(mb, loc_array);
      mono_mb_emit_icon(mb, complex_count); /* The array has an additional slot
                                               to hold the ret value */
  ...
  info = mono_wrapper_info_create(mb, WRAPPER_SUBTYPE_NONE);
  info->d.remoting.method = method;
  res = mono_remoting_mb_create_and_cache(method, mb, csig,
                                          csig->param_count + 16, info);
  mono_mb_free(mb);
...
MonoMethod *mono_marshal_get_xappdomain_invoke(MonoMethod *method,
  ...
  int loc_old_domainid, loc_domainid, loc_return = 0, loc_serialized_exc = 0,
                                      loc_context;
  ...
  for (i = 0; i < sig->param_count; i++) {
    ...
    /* If the [Out] attribute is applied to a parameter that can be internally
     * copied, the copy will be made by reusing the original object instance
     */
    if ((ptype->attrs & PARAM_ATTRIBUTE_OUT) != 0 && mt == MONO_MARSHAL_COPY &&
        !ptype->byref)
  ...
  if (has_byreflike) {
     ...
     */
    mono_mb_emit_exception(mb, "NotImplementedException",
                           "Cross AppDomain calls with IsByRefLike parameter "
                           "or return types are not implemented");
    info = mono_wrapper_info_create(mb, WRAPPER_SUBTYPE_NONE);
    info->d.remoting.method = method;
    res = mono_remoting_mb_create_and_cache(method, mb, sig,
                                            sig->param_count + 16, info);
  ...
  MonoMethod *remoting_invoke_method =
      mono_marshal_get_remoting_invoke(method, error);
  if (!is_ok(error)) {
    mono_mb_free(mb);
    return NULL;
  }
  ...
  if (complex_count > 0) {
    mono_mb_emit_icon(mb, (ret_marshal_type == MONO_MARSHAL_SERIALIZE &&
                           complex_out_count > 0)
                              ? complex_count + 1
                              : complex_count); /* +1 for the return type */
  ...
  xdomain_method =
      mono_marshal_get_xappdomain_dispatch(method, marshal_types, complex_count,
                                           complex_out_count, ret_marshal_type);
  ...
  j = 0;
  for (i = 0; i < sig->param_count; i++) {
    if (!sig->params[i]->byref || marshal_types[i] != MONO_MARSHAL_COPY)
      continue;
    mono_mb_emit_ldarg(mb, i + 1);
    mono_mb_emit_ldloc(mb, copy_locals_base + (j++));
    mono_marshal_emit_xdomain_copy_value(
        mb, mono_class_from_mono_type_internal(sig->params[i]));
    mono_mb_emit_byte(mb, CEE_STIND_REF);
  }
 ...
 */
MonoMethod *mono_marshal_get_remoting_invoke_for_target(
    MonoMethod *method, MonoRemotingTarget target_type, MonoError *error) {
  error_init(error);
...
MonoMethod *mono_marshal_get_remoting_invoke_with_check(MonoMethod *method,
  ...
  if (mono_marshal_supports_fast_xdomain(method)) {
    ...
    mono_mb_emit_managed_call(mb, native,
 ...
 *
 * <code><i>type</i> ldfld_wrapper (MonoObject *this_obj, MonoClass *klass,
 * MonoClassField *field, int offset)</code>
 */
MonoMethod *mono_marshal_get_ldfld_wrapper(MonoType *type) {
  ...
  if (!type->byref) {
    ...
    } else if (t == MONO_TYPE_OBJECT || t == MONO_TYPE_CLASS ||
               t == MONO_TYPE_STRING) {
  ...
  cache = get_cache(&m_class_get_image(klass)->ldfld_wrapper_cache,
  ...
  */
  ...
  res = mono_mb_create_and_cache_full(cache, klass, mb, sig,
 ...
 * gpointer ldflda_wrapper (MonoObject *this_obj, MonoClass *klass,
 * MonoClassField *field, int offset);
 */
MonoMethod *mono_marshal_get_ldflda_wrapper(MonoType *type) {
  MonoMethodSignature *sig;
  MonoMethodBuilder *mb;
  MonoMethod *res;
  MonoClass *klass;
  ...
  cache = get_cache(&m_class_get_image(klass)->ldflda_wrapper_cache,
                    mono_aligned_addr_hash, NULL);
  if ((res = mono_marshal_find_in_cache(cache, klass)))
    return res;
  ...
  mono_mb_emit_exception_full(
      mb, "System", "InvalidOperationException",
 ...
 *
 * <code>void stfld_wrapper (MonoObject *this_obj, MonoClass *klass,
 * MonoClassField *field, int offset, <i>type</i> val)</code>
 */
MonoMethod *mono_marshal_get_stfld_wrapper(MonoType *type) {
  ...
  cache = get_cache(&m_class_get_image(klass)->stfld_wrapper_cache,
                    mono_aligned_addr_hash, NULL);
  if ((res = mono_marshal_find_in_cache(cache, klass)))
    return res;
...
MonoMethod *mono_marshal_get_proxy_cancast(MonoClass *klass) {
  ...
  cache = get_cache(&m_class_get_image(klass)->proxy_isinst_cache,
                    mono_aligned_addr_hash, NULL);
  if ((res = mono_marshal_find_in_cache(cache, klass)))
    return res;
  ...
  can_cast_to = mono_method_desc_search_in_class(
      desc, mono_defaults.iremotingtypeinfo_class);
  ...
  /* Upgrade the proxy vtable by calling: mono_upgrade_remote_class_wrapper
   * (type, ob)*/
  mono_mb_emit_ptr(mb, m_class_get_byval_arg(klass));
  mono_mb_emit_icall(mb, type_from_handle);
  mono_mb_emit_ldarg(mb, 0);
  ...
  info = mono_wrapper_info_create(mb, WRAPPER_SUBTYPE_NONE);
  info->d.proxy.klass = klass;
  res = mono_mb_create_and_cache_full(cache, klass, mb, isint_sig,
                                      isint_sig->param_count + 16, info, NULL);
  mono_mb_free(mb);

  return res;
}

void mono_upgrade_remote_class_wrapper(MonoReflectionType *rtype_raw,
                                       MonoTransparentProxy *tproxy_raw) {
  ICALL_ENTRY();
  ...
  MonoClass *klass =
      mono_class_from_mono_type_internal(MONO_HANDLE_GETVAL(rtype, type));
  mono_upgrade_remote_class(domain, MONO_HANDLE_CAST(MonoObject, tproxy), klass,
                            error);
  ICALL_RETURN();
}
...
static MonoXDomainMarshalType mono_get_xdomain_marshal_type(MonoType *t) {
  ...
  case MONO_TYPE_SZARRAY: {
    ...
    if (mono_get_xdomain_marshal_type(m_class_get_byval_arg(elem_class)) !=
...
static gboolean xdomain_copy_array_element_inplace(MonoArrayHandle arr, int i,
                                                   MonoError *error) {
  ...
  MonoObjectHandle item_copy =
      mono_marshal_xdomain_copy_value_handle(item, error);
 ...
 */
MonoObjectHandle mono_marshal_xdomain_copy_value_handle(MonoObjectHandle val,
                                                        MonoError *error) {
  error_init(error);
  ...
  case MONO_TYPE_R8: {
    ...
    MonoObjectHandle res = MONO_HANDLE_NEW(
        MonoObject,
        mono_value_box_checked(
            domain, klass, ((char *)MONO_HANDLE_RAW(val)) + sizeof(MonoObject),
            error)); /* FIXME use handles in mono_value_box_checked */
  ...
  case MONO_TYPE_STRING: {
    ...
    MonoGCHandle gchandle = mono_gchandle_from_handle(val, TRUE);
    MonoStringHandle res = mono_string_new_utf16_handle(
        domain, mono_string_chars_internal(MONO_HANDLE_RAW(str)),
        mono_string_handle_length(str), error);
    mono_gchandle_free_internal(gchandle);
    goto_if_nok(error, leave);
    MONO_HANDLE_ASSIGN(result, res);
    break;
  ...
  case MONO_TYPE_SZARRAY: {
    ...
    MonoXDomainMarshalType mt = mono_get_xdomain_marshal_type(
 ...
 */
MonoObjectHandle
ves_icall_mono_marshal_xdomain_copy_value_impl(MonoObjectHandle val,
                                               MonoError *error) {
  return mono_marshal_xdomain_copy_value_handle(val, error);
}

static void mono_context_set_icall(MonoAppContext *new_context_raw) {
  HANDLE_FUNCTION_ENTER();
