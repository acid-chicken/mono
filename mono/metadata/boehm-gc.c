...
static void mono_push_other_roots(void);

static void register_test_toggleref_callback(void);
...
typedef struct {
  ...
  /* 2^16 appdomains should be enough for everyone (though I know I'll regret
   * this in 20 years) */
  /* we alloc this only for weak refs, since we can get the domain directly in
   * the other cases */
  guint16 *domain_ids;
} HandleData;

#define EMPTY_HANDLE_DATA(type)                                                \
  { NULL, NULL, 0, (type), 0, NULL }

/* weak and weak-track arrays will be allocated in malloc memory
 */
static HandleData gc_handles[] = {
    EMPTY_HANDLE_DATA(HANDLE_WEAK), EMPTY_HANDLE_DATA(HANDLE_WEAK_TRACK),
    EMPTY_HANDLE_DATA(HANDLE_NORMAL), EMPTY_HANDLE_DATA(HANDLE_PINNED)};

static void mono_gc_warning(char *msg, GC_word arg) {
...
void mono_gc_base_init(void) {
  ...
  /* cache value rather than calling during collection since g_hasenv may take
   * locks and can deadlock */
  ...
  if ((env = g_getenv("MONO_GC_PARAMS"))) {
    ...
    for (ptr = opts; *ptr; ++ptr) {
      ...
      if (g_str_has_prefix(opt, "max-heap-size=")) {
        ...
        opt = strchr(opt, '=') + 1;
        if (*opt &&
            mono_gc_parse_environment_string_extract_number(opt, &max_heap)) {
        ...
        /*
        fprintf (stderr, "MONO_GC_PARAMS must be a comma-delimited list of one
        or more of the following:\n"); fprintf (stderr, "  max-heap-size=N
        (where N is an integer, possibly with a k, m or a g suffix)\n"); exit
        (1);
        */
      }
    }
...
void mono_gc_base_cleanup(void) { GC_set_finalizer_notifier(NULL); }
 ...
 */
int mono_gc_max_generation(void) { return 0; }

guint64 mono_gc_get_allocated_bytes_for_current_thread(void) { return 0; }
 ...
 */
int mono_gc_collection_count(int generation) { return GC_get_gc_no(); }
 ...
 */
int64_t mono_gc_get_heap_size(void) { return GC_get_heap_size(); }

gboolean mono_gc_is_gc_thread(void) { return GC_thread_is_registered(); }
...
void mono_gc_thread_detach(MonoThreadInfo *p) {
...
void mono_gc_thread_detach_with_lock(MonoThreadInfo *p) {
...
static void on_gc_notification(GC_EventType event) {
  ...
  case GC_EVENT_END:
    ...
    mono_trace_message(MONO_TRACE_GC, "gc took %" G_GINT64_FORMAT " usecs",
                       (mono_100ns_ticks() - gc_start_time) / 10);
    break;
  default:
    break;
  }
  ...
  case GC_EVENT_PRE_STOP_WORLD:
    ...
    MONO_PROFILER_RAISE(gc_event,
                        (MONO_GC_EVENT_PRE_STOP_WORLD_LOCKED, 0, TRUE));
    break;
  case GC_EVENT_POST_START_WORLD:
    mono_thread_info_suspend_unlock();
    MONO_PROFILER_RAISE(gc_event,
                        (MONO_GC_EVENT_POST_START_WORLD_UNLOCKED, 0, TRUE));
    break;
  default:
    break;
  }
...
static gpointer register_root(gpointer arg) {
...
int mono_gc_register_root(char *start, size_t size, void *descr,
  ...
  MONO_PROFILER_RAISE(gc_root_register,
                      ((const mono_byte *)start, size, source, key, msg));
  return TRUE;
}
...
static gpointer deregister_root(gpointer arg) {
...
static void push_root(gpointer key, gpointer value, gpointer user_data) {
  GC_push_all(key, value);
}

static void push_handle_stack(HandleStack *stack) {
  ...
  while (cur) {
    if (cur->size > 0)
      GC_push_all((gpointer)&cur->elems[0],
                  (char *)(cur->elems + cur->size) + 1);
    if (cur == last)
      break;
    cur = cur->next;
  }
...
static void mono_gc_weak_link_add(void **link_addr, MonoObject *obj,
                                  gboolean track) {
...
static void mono_gc_weak_link_remove(void **link_addr, gboolean track) {
  if (track)
...
static gpointer reveal_link(gpointer link_addr) {
  void **link_a = (void **)link_addr;
  return REVEAL_POINTER(*link_a);
}

static MonoObject *mono_gc_weak_link_get(void **link_addr) {
  MonoObject *obj =
      (MonoObject *)GC_call_with_alloc_lock(reveal_link, link_addr);
  if (obj == (MonoObject *)-1)
    return NULL;
  return obj;
}
...
void *mono_gc_make_descr_for_object(gsize *bitmap, int numbits,
                                    size_t obj_size) {
  return mono_gc_make_descr_from_bitmap(bitmap, numbits);
}

void *mono_gc_make_descr_for_array(int vector, gsize *elem_bitmap, int numbits,
                                   size_t elem_size) {
...
void *mono_gc_make_vector_descr(void) { return NULL; }

void *mono_gc_make_root_descr_all_refs(int numbits) { return NULL; }

MonoObject *mono_gc_alloc_fixed(size_t size, void *descr,
                                MonoGCRootSource source, void *key,
                                const char *msg) {
...
MonoArray *mono_gc_alloc_vector(MonoVTable *vtable, size_t size,
  ...
  if (!m_class_has_references(vtable->klass)) {
    ...
    memset(mono_object_get_data((MonoObject *)obj), 0,
...
void mono_gc_wbarrier_object_copy_internal(MonoObject *obj, MonoObject *src) {
  ...
  mono_gc_memmove_aligned(mono_object_get_data(obj),
...
int mono_gc_get_suspend_signal(void) { return GC_get_suspend_signal(); }

int mono_gc_get_restart_signal(void) { return GC_get_thr_restart_signal(); }

#if defined(USE_COMPILER_TLS) && defined(__linux__) &&                         \
...
gboolean mono_gc_is_critical_method(MonoMethod *method) { return FALSE; }
...
int mono_gc_get_aligned_size_for_allocator(int size) { return size; }

const char *mono_gc_get_gc_name(void) { return "boehm"; }

void *mono_gc_invoke_with_gc_lock(MonoGCLockedCallbackFunc func, void *data) {
  return GC_call_with_alloc_lock(func, data);
}

char *mono_gc_get_description(void) { return g_strdup(DEFAULT_GC_NAME); }

void mono_gc_set_desktop_mode(void) { GC_set_dont_expand(1); }

gboolean mono_gc_is_moving(void) { return FALSE; }
...
guint8 *mono_gc_get_target_card_table(int *shift_bits,
                                      target_mgreg_t *card_mask) {
  *shift_bits = 0;
  *card_mask = 0;
  return NULL;
}
...
void *mono_gc_get_nursery(int *shift_bits, size_t *size) { return NULL; }

gboolean mono_gc_precise_stack_mark_enabled(void) { return FALSE; }

FILE *mono_gc_get_logfile(void) { return NULL; }
...
void mono_gc_register_for_finalization(MonoObject *obj,
  ...
  GC_REGISTER_FINALIZER_NO_ORDER((char *)obj - offset, user_data,
 ...
 *
 *   Register the dimensions of the normal stack and altstack with the
 * collector. Currently, STACK/STACK_SIZE is only used when the thread is
...
int mono_gc_get_los_limit(void) { return G_MAXINT; }
...
gboolean mono_gc_user_markers_supported(void) { return FALSE; }
...
void mono_gc_toggleref_register_callback(
    MonoToggleRefStatus (*proccess_toggleref)(MonoObject *obj)) {
  GC_set_toggleref_func((GC_ToggleRefStatus(*)(GC_PTR obj))proccess_toggleref);
}

/* Test support code */

static MonoToggleRefStatus test_toggleref_callback(MonoObject *obj) {
  MonoToggleRefStatus status = MONO_TOGGLE_REF_DROP;

  MONO_STATIC_POINTER_INIT(MonoClassField, mono_toggleref_test_field)

  mono_toggleref_test_field = mono_class_get_field_from_name_full(
      mono_object_class(obj), "__test", NULL);
...
static gboolean is_finalization_aware(MonoObject *obj) {
  ...
  return (vt->gc_bits & BOEHM_GC_BIT_FINALIZER_AWARE) ==
         BOEHM_GC_BIT_FINALIZER_AWARE;
}

static void fin_notifier(MonoObject *obj) {
...
static gboolean slot_occupied(HandleData *handles, guint slot) {
  return handles->bitmap[slot / BITMAP_SIZE] & (1 << (slot % BITMAP_SIZE));
}

static void vacate_slot(HandleData *handles, guint slot) {
  handles->bitmap[slot / BITMAP_SIZE] &= ~(1 << (slot % BITMAP_SIZE));
}

static void occupy_slot(HandleData *handles, guint slot) {
  handles->bitmap[slot / BITMAP_SIZE] |= 1 << (slot % BITMAP_SIZE);
}

static int find_first_unset(guint32 bitmap) {
  int i;
  for (i = 0; i < 32; ++i) {
    if (!(bitmap & (1 << i)))
      return i;
...
static void handle_data_alloc_entries(HandleData *handles) {
  handles->size = 32;
  if (MONO_GC_HANDLE_TYPE_IS_WEAK(handles->type)) {
    handles->entries =
        (void **)g_malloc0(sizeof(*handles->entries) * handles->size);
    handles->domain_ids =
        (guint16 *)g_malloc0(sizeof(*handles->domain_ids) * handles->size);
  } else {
    handles->entries = (void **)mono_gc_alloc_fixed(
        sizeof(*handles->entries) * handles->size, NULL,
        MONO_ROOT_SOURCE_GC_HANDLE, NULL, "GC Handle Table (Boehm)");
  }
  handles->bitmap = (guint32 *)g_malloc0(handles->size / CHAR_BIT);
}

static gint handle_data_next_unset(HandleData *handles) {
  gint slot;
...
static gint handle_data_first_unset(HandleData *handles) {
  gint slot;
...
static void handle_data_grow(HandleData *handles, gboolean track) {
  guint32 *new_bitmap;
  guint32 new_size =
      handles->size * 2; /* always double: we memset to 0 based on this below */
  ...
  if (MONO_GC_HANDLE_TYPE_IS_WEAK(handles->type)) {
    ...
    memcpy(domain_ids, handles->domain_ids,
           sizeof(*handles->domain_ids) * handles->size);
    ...
    gpointer *entries;
    entries = (void **)mono_gc_alloc_fixed(sizeof(*handles->entries) * new_size,
                                           NULL, MONO_ROOT_SOURCE_GC_HANDLE,
                                           NULL, "GC Handle Table (Boehm)");
    mono_gc_memmove_aligned(entries, handles->entries,
                            sizeof(*handles->entries) * handles->size);
...
static guint32 alloc_handle(HandleData *handles, MonoObject *obj,
                            gboolean track) {
  gint slot, i;
  guint32 res;
  ...
  if (MONO_GC_HANDLE_TYPE_IS_WEAK(handles->type)) {
    ...
    handles->domain_ids[slot] =
        (obj ? mono_object_get_domain_internal(obj) : mono_domain_get())
            ->domain_id;
    if (obj)
  ...
  MONO_PROFILER_RAISE(gc_handle_created,
                      (res, (MonoGCHandleType)handles->type, obj));
  return res;
}
...
MonoGCHandle mono_gchandle_new_internal(MonoObject *obj, gboolean pinned) {
  return (MonoGCHandle)(size_t)alloc_handle(
      &gc_handles[pinned ? HANDLE_PINNED : HANDLE_NORMAL], obj, FALSE);
}
...
MonoGCHandle mono_gchandle_new_weakref_internal(MonoObject *obj,
  ...
  return MONO_GC_HANDLE_FROM_UINT(alloc_handle(
      &gc_handles[track_resurrection ? HANDLE_WEAK_TRACK : HANDLE_WEAK], obj,
      track_resurrection));
}
 ...
 */
MonoObject *mono_gchandle_get_target_internal(MonoGCHandle gch) {
...
void mono_gchandle_set_target(MonoGCHandle gch, MonoObject *obj) {
  guint32 gchandle = MONO_GC_HANDLE_TO_UINT(gch);
  guint slot = MONO_GC_HANDLE_SLOT(gchandle);
  guint type = MONO_GC_HANDLE_TYPE(gchandle);
  HandleData *handles = &gc_handles[type];
    ...
    if (MONO_GC_HANDLE_TYPE_IS_WEAK(handles->type)) {
      ...
      if (handles->entries[slot])
        mono_gc_weak_link_remove(&handles->entries[slot],
                                 handles->type == HANDLE_WEAK_TRACK);
      if (obj)
   ...
   * handles->type, obj, handles->entries [slot]);*/
  unlock_handles(handles);
}
 ...
 * specific \p domain.
 */
gboolean mono_gchandle_is_in_domain(MonoGCHandle gch, MonoDomain *domain) {
  guint32 gchandle = MONO_GC_HANDLE_TO_UINT(gch);
  guint slot = MONO_GC_HANDLE_SLOT(gchandle);
  guint type = MONO_GC_HANDLE_TYPE(gchandle);
  HandleData *handles = &gc_handles[type];
 ...
 * object wrapped.
 */
void mono_gchandle_free_internal(MonoGCHandle gch) {
  guint32 gchandle = MONO_GC_HANDLE_TO_UINT(gch);
  if (!gchandle)
    return;
  ...
  MONO_PROFILER_RAISE(gc_handle_deleted,
                      (gchandle, (MonoGCHandleType)handles->type));
}
  ...
  for (type = HANDLE_TYPE_MIN; type < HANDLE_PINNED; ++type) {
    ...
    for (slot = 0; slot < handles->size; ++slot) {
      ...
      if (MONO_GC_HANDLE_TYPE_IS_WEAK(type)) {
        ...
        if (handles->entries[slot] &&
            mono_object_domain(handles->entries[slot]) == domain) {
          vacate_slot(handles, slot);
          handles->entries[slot] = NULL;
        }
      }
...
guint64 mono_gc_get_total_allocated_bytes(MonoBoolean precise) { return 0; }

void mono_gc_register_obj_with_weak_fields(void *obj) {
  g_error("Weak fields not supported by boehm gc");
}

gboolean mono_gc_ephemeron_array_add(MonoObject *obj) { return TRUE; }
