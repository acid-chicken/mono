...
struct _MonoThreadsSync {
  /*
   * The entry count field can be negative, which would mean that the entry_sem
   * is signaled and nobody is waiting to acquire it. This can happen when the
   * thread that was waiting is either interrupted or timeouts, and the owner
   * releases the lock before the forementioned thread updates the entry count.
   *
   * The 0 entry_count value is encoded as ENTRY_COUNT_ZERO, positive numbers
   * being greater than it and negative numbers smaller than it.
   */
...
enum {
  ...
  LOCK_WORD_NEST_MASK = ((1 << LOCK_WORD_NEST_BITS) - 1)
