...
// consistency with coreclr
// https://github.com/dotnet/coreclr/blob/643b09f966e68e06d5f0930755985a01a2a2b096/src/vm/win32threadpool.h#L111
  ...
  do {                                                                         \
    ...
    do {                                                                       \
      ...
      if (!(counter._.starting >= 0))                                          \
        g_error("%s: counter._.starting = %d, but should be >= 0", __func__,   \
                counter._.starting);                                           \
      if (!(counter._.working >= 0))                                           \
        g_error("%s: counter._.working = %d, but should be >= 0", __func__,    \
                counter._.working);                                            \
    } while (mono_atomic_cas_i32(&threadpool.counters.as_gint32,               \
                                 (var).as_gint32,                              \
                                 __old.as_gint32) != __old.as_gint32);         \
  } while (0)

static ThreadPoolCounter COUNTER_READ(void) {
  ThreadPoolCounter counter;
  counter.as_gint32 = mono_atomic_load_i32(&threadpool.counters.as_gint32);
  return counter;
}

static void domains_lock(void) {
  mono_coop_mutex_lock(&threadpool.domains_lock);
}

static void domains_unlock(void) {
  mono_coop_mutex_unlock(&threadpool.domains_lock);
}

static void destroy(gpointer unused) {
...
static void initialize(void) {
  ...
  threadpool.limit_io_max =
      CLAMP(threadpool.limit_io_min * 100, MIN(threadpool.limit_io_min, 200),
            MAX(threadpool.limit_io_min, 200));
...
gboolean mono_threadpool_enqueue_work_item(MonoDomain *domain,
                                           MonoObject *work_item,
                                           MonoError *error) {
  ...
  threadpool_class = mono_class_load_from_name(
      mono_defaults.corlib, "System.Threading", "ThreadPool");
  ...
  unsafe_queue_custom_work_item_method =
      mono_class_get_method_from_name_checked(
          threadpool_class, "UnsafeQueueCustomWorkItem", 2, 0, error);
  ...
  if (current_domain == domain) {
    mono_runtime_invoke_checked(unsafe_queue_custom_work_item_method, NULL,
                                args, error);
  } else {
...
static ThreadPoolDomain *tpdomain_create(MonoDomain *domain) {
...
/* LOCKING: domains_lock must be held. */
static gboolean tpdomain_remove(ThreadPoolDomain *tpdomain) {
  g_assert(tpdomain);
  return g_ptr_array_remove(threadpool.domains, tpdomain);
}

/* LOCKING: domains_lock must be held */
static ThreadPoolDomain *tpdomain_get(MonoDomain *domain) {
  gint i;
...
static void tpdomain_free(ThreadPoolDomain *tpdomain) { g_free(tpdomain); }

/* LOCKING: domains_lock must be held */
static ThreadPoolDomain *tpdomain_get_next(ThreadPoolDomain *current) {
  ...
  if (len > 0) {
    ...
    for (i = current_idx + 1; i < len + current_idx + 1; ++i) {
      ThreadPoolDomain *tmp =
          (ThreadPoolDomain *)g_ptr_array_index(threadpool.domains, i % len);
...
static MonoObject *try_invoke_perform_wait_callback(MonoObject **exc,
                                                    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoObject *const res = mono_runtime_try_invoke(
      mono_defaults.threadpool_perform_wait_callback_method, NULL, NULL, exc,
      error);
  HANDLE_FUNCTION_RETURN_VAL(res);
}

static void mono_threadpool_set_thread_name(MonoInternalThread *thread) {
...
static void worker_callback(void) {
  ...
  COUNTER_ATOMIC(counter, {
    if (!(counter._.working < 32767 /* G_MAXINT16 */))
      g_error("%s: counter._.working = %d, but should be < 32767", __func__,
              counter._.working);
  ...
  /*
   * This is needed so there is always an lmf frame in the runtime invoke call
   * below, so ThreadAbortExceptions are caught even if the thread is in native
   * code.
   */
  ...
  while (!mono_runtime_is_shutting_down()) {
    ...
    if (thread->state &
        (ThreadState_AbortRequested | ThreadState_SuspendRequested)) {
      domains_unlock();
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_THREADPOOL,
               ...
               tpdomain->domain, tpdomain->outstanding_request);
    ...
    if (mono_domain_set_fast(tpdomain->domain, FALSE)) {
      ...
      if (exc || !is_ok(error)) {
        ...
        mono_thread_internal_unhandled_exception(exc);
                 ...
                 *(MonoBoolean *)mono_object_unbox_internal(res) == FALSE) {
        retire = TRUE;
      }
    ...
    if (tpdomain->outstanding_request + tpdomain->threadpool_jobs == 0 &&
        mono_domain_is_unloading(tpdomain->domain)) {
      gboolean removed;
...
void mono_threadpool_cleanup(void) {
...
MonoAsyncResult *mono_threadpool_begin_invoke(MonoDomain *domain,
                                              MonoObject *target,
                                              MonoMethod *method,
                                              gpointer *params,
                                              MonoError *error) {
  ...
  async_call_klass = mono_class_load_from_name(mono_defaults.corlib, "System",
                                               "MonoAsyncCall");
  ...
  message = mono_method_call_message_new(
      method, params, mono_get_delegate_invoke_internal(method->klass),
      (params != NULL) ? (&async_callback) : NULL,
      (params != NULL) ? (&state) : NULL, error);
  return_val_if_nok(error, NULL);

  async_call =
      (MonoAsyncCall *)mono_object_new_checked(domain, async_call_klass, error);
  return_val_if_nok(error, NULL);
  ...
  if (async_callback) {
    MONO_OBJECT_SETREF_INTERNAL(
        async_call, cb_method,
        mono_get_delegate_invoke_internal(
            ((MonoObject *)async_callback)->vtable->klass));
...
MonoObject *mono_threadpool_end_invoke(MonoAsyncResult *ares,
                                       MonoArray **out_args, MonoObject **exc,
                                       MonoError *error) {
  MonoAsyncCall *ac;
  ...
  if (ares->completed) {
    ...
    if (ares->handle) {
      ...
      MonoWaitHandle *wait_handle =
          mono_wait_handle_new(mono_object_domain(ares), wait_event, error);
      if (!is_ok(error)) {
...
gboolean mono_threadpool_remove_domain_jobs(MonoDomain *domain, int timeout) {
  gint64 end = 0;
  ThreadPoolDomain *tpdomain;
  gboolean ret;
  ...
  while (tpdomain->outstanding_request + tpdomain->threadpool_jobs > 0) {
    if (timeout == -1) {
      ...
      res = mono_coop_cond_timedwait(&tpdomain->cleanup_cond,
                                     &threadpool.domains_lock, end - now);
      if (res != 0) {
        ret = FALSE;
        break;
      }
...
void mono_threadpool_suspend(void) {
  if (mono_lazy_is_initialized(&status))
    mono_threadpool_worker_set_suspended(TRUE);
}

void mono_threadpool_resume(void) {
  if (mono_lazy_is_initialized(&status))
    mono_threadpool_worker_set_suspended(FALSE);
}

void ves_icall_System_Threading_ThreadPool_GetAvailableThreadsNative(
    gint32 *worker_threads, gint32 *completion_port_threads, MonoError *error) {
  ThreadPoolCounter counter;

  if (!worker_threads || !completion_port_threads)
    return;

  if (!mono_lazy_initialize(&status, initialize) ||
  ...
  *worker_threads =
      MAX(0, mono_threadpool_worker_get_max() - counter._.working);
...
void ves_icall_System_Threading_ThreadPool_GetMaxThreadsNative(
    gint32 *worker_threads, gint32 *completion_port_threads, MonoError *error) {
  if (!worker_threads || !completion_port_threads)
    return;
...
MonoBoolean ves_icall_System_Threading_ThreadPool_SetMaxThreadsNative(
    gint32 worker_threads, gint32 completion_port_threads, MonoError *error) {
  if (!mono_lazy_initialize(&status, initialize) ||
      !mono_refcount_tryinc(&threadpool))
    return FALSE;
  ...
  if (completion_port_threads < threadpool.limit_io_min ||
      completion_port_threads < cpu_count)
    return FALSE;
...
void ves_icall_System_Threading_ThreadPool_InitializeVMTp(
    MonoBoolean *enable_worker_tracking, MonoError *error) {
...
void ves_icall_System_Threading_ThreadPool_NotifyWorkItemProgressNative(
    MonoError *error) {
...
void ves_icall_System_Threading_ThreadPool_ReportThreadStatus(
    MonoBoolean is_working, MonoError *error) {
  // TODO
