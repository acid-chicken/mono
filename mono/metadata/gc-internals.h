...
#define mono_domain_finalizers_lock(domain)                                    \
  mono_os_mutex_lock(&(domain)->finalizable_objects_hash_lock);
#define mono_domain_finalizers_unlock(domain)                                  \
  mono_os_mutex_unlock(&(domain)->finalizable_objects_hash_lock);

/* Register a memory area as a conservatively scanned GC root */
#define MONO_GC_REGISTER_ROOT_PINNING(x, src, key, msg)                        \
  mono_gc_register_root((char *)&(x), sizeof(x), MONO_GC_DESCRIPTOR_NULL,      \
                        (src), (key), (msg))
 ...
 * These macros are used to convert between the old int32 representation to a
 * netcore compatible pointer representation.
 */
...
#define MONO_GC_ROOT_DESCR_FOR_FIXED(n)                                        \
  (mono_gc_is_moving() ? mono_gc_make_root_descr_all_refs(0)                   \
...
#define MONO_GC_REGISTER_ROOT_SINGLE(x, src, key, msg)                         \
  do {                                                                         \
    ...
    mono_gc_register_root((char *)&(x), sizeof(MonoObject *),                  \
                          mono_gc_make_root_descr_all_refs(1), (src), (key),   \
                          (msg));                                              \
  } while (0)

/*
 * This is used for fields which point to objects which are kept alive by other
 * references when using Boehm.
 */
...
#define IS_GC_REFERENCE(class, t)                                              \
  (mono_gc_is_moving()                                                         \
       ? FALSE                                                                 \
       : ((t)->type == MONO_TYPE_U && (class)->image == mono_defaults.corlib))
...
// C++ callers outside of metadata (mini/tasklets.c) must use
// mono_gc_alloc_fixed_no_descriptor instead of mono_gc_alloc_fixed, or else
// compile twice -- boehm and sgen.
...
MonoArrayHandle mono_gc_alloc_handle_array(MonoVTable *vtable, gsize size,
                                           gsize max_length, gsize bounds_size);
...
typedef void (*MonoFinalizationProc)(
    gpointer, gpointer); // same as SGenFinalizationProc, GC_finalization_proc

void mono_gc_register_for_finalization(MonoObject *obj,
                                       MonoFinalizationProc user_data);
void mono_gc_add_memory_pressure(gint64 value);
MONO_API int mono_gc_register_root(char *start, size_t size,
 ...
 * If the root uses an user defined mark routine, the writes are not required to
 * be to the area between START and START+SIZE. The write barrier allows the GC
 * to avoid scanning this root at each collection, so it is more efficient.
...
typedef enum {
  ...
  // Managed allocator that works like the regular one but also calls into the
  // profiler.
...
/* Fast valuetype copy */
/* WARNING: [dest, dest + size] must be within the bounds of a single type,
 * otherwise the GC will lose remset entries */
G_EXTERN_C void mono_gc_wbarrier_range_copy(gpointer dest, gconstpointer src,
                                            int size);
...
typedef struct {
   ...
   */
  void (*thread_mark_func)(gpointer user_data, guint8 *stack_start,
                           guint8 *stack_end, gboolean precise, void *gc_data);
  /*
   ...
   */
  void (*interp_mark_func)(gpointer thread_info, GcScanFunc func,
...
void mono_gc_get_gcmemoryinfo(gint64 *fragmented_bytes, gint64 *heap_size_bytes,
