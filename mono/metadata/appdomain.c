...
void mono_runtime_init_checked(MonoDomain *domain, MonoThreadStartCB start_cb,
  ...
  // We have to append here because otherwise this will run before the netcore
  // hook (which is installed first), see
  // https://github.com/dotnet/runtime/issues/34273
  ...
  mono_install_assembly_search_hook_v2(
      mono_domain_assembly_search, GUINT_TO_POINTER(TRUE), TRUE, FALSE, FALSE);
  mono_install_assembly_search_hook_v2(mono_domain_assembly_postload_search,
...
static MonoAppDomainHandle mono_domain_create_appdomain_internal(
  ...
  if (MONO_HANDLE_IS_NULL(app_base)) {
    ...
    if (!MONO_HANDLE_IS_NULL(root_app_base)) {
      ...
      MonoGCHandle gchandle = mono_gchandle_from_handle(
...
MonoAssembly *mono_try_assembly_resolve_handle(MonoAssemblyLoadContext *alc,
  ...
  if (ret && !refonly &&
    ...
    mono_error_set_file_not_found(error, filename,
                                  "AssemblyResolveEvent handlers cannot return "
                                  "Assemblies loaded for reflection only: %s",
                                  filename);
    ret = NULL;
    goto leave;
  }
...
static MonoAssembly *mono_domain_assembly_preload(MonoAssemblyLoadContext *alc,
  ...
  if (!mono_runtime_get_no_exec()) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
               "Domain %s (%p) ApplicationBase is %s", domain->friendly_name,
               domain, base_dir);
