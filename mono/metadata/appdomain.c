...
static const char *mono_check_corlib_version_internal(void);

static MonoAssembly *mono_domain_assembly_preload(MonoAssemblyLoadContext *alc,
...
static MonoAssembly *
mono_domain_assembly_search(MonoAssemblyLoadContext *alc,
                            MonoAssembly *requesting, MonoAssemblyName *aname,
                            gboolean refonly, gboolean postload,
                            gpointer user_data, MonoError *error);

static void mono_domain_fire_assembly_load(MonoAssemblyLoadContext *alc,
                                           MonoAssembly *assembly,
                                           gpointer user_data,
                                           MonoError *error_out);

static gboolean mono_domain_asmctx_from_path(
    const char *fname, MonoAssembly *requesting_assembly, gpointer user_data,
    MonoAssemblyContextKind *out_asmctx);

static void add_assemblies_to_domain(MonoDomain *domain, MonoAssembly *ass,
                                     GHashTable *hash);

#if ENABLE_NETCORE

static void add_assembly_to_alc(MonoAssemblyLoadContext *alc,
                                MonoAssembly *ass);

#endif

static MonoAppDomainHandle mono_domain_create_appdomain_internal(
    char *friendly_name, MonoAppDomainSetupHandle setup, MonoError *error);

static MonoDomain *mono_domain_create_appdomain_checked(
    char *friendly_name, char *configuration_file, MonoError *error);

static void mono_context_set_default_context(MonoDomain *domain);

static char *get_shadow_assembly_location_base(MonoDomain *domain,
                                               MonoError *error);
...
GENERATE_GET_CLASS_WITH_CACHE(appdomain, MONO_APPDOMAIN_CLASS_NAME_SPACE,
                              MONO_APPDOMAIN_CLASS_NAME);
GENERATE_GET_CLASS_WITH_CACHE(appdomain_setup,
...
static void mono_error_set_appdomain_unloaded(MonoError *error) {
  mono_error_set_generic_error(error, "System", "AppDomainUnloadedException",
                               "");
}

void mono_install_runtime_load(MonoLoadFunc func) { load_function = func; }

MonoDomain *mono_runtime_load(const char *filename,
 ...
 */
void mono_runtime_set_no_exec(gboolean val) { no_exec = val; }
...
static void create_domain_objects(MonoDomain *domain) {
   ...
   */
  string_vt =
      mono_class_vtable_checked(domain, mono_defaults.string_class, error);
  ...
  mono_error_assert_ok(error);
  mono_field_static_set_value_internal(string_vt, string_empty_fld,
  ...
  mono_error_assert_ok(error);
  domain->out_of_memory_ex =
      MONO_HANDLE_RAW(mono_exception_from_name_two_strings_checked(
          mono_defaults.corlib, "System", "OutOfMemoryException", arg,
          NULL_HANDLE_STRING, error));
  mono_error_assert_ok(error);
   ...
   */
  arg = mono_string_new_handle(
      domain, "A null value was found where an object instance was required",
      error);
  mono_error_assert_ok(error);
  domain->null_reference_ex =
      MONO_HANDLE_RAW(mono_exception_from_name_two_strings_checked(
          mono_defaults.corlib, "System", "NullReferenceException", arg,
          NULL_HANDLE_STRING, error));
  mono_error_assert_ok(error);
  arg = mono_string_new_handle(
      domain, "The requested operation caused a stack overflow.", error);
  mono_error_assert_ok(error);
  domain->stack_overflow_ex =
      MONO_HANDLE_RAW(mono_exception_from_name_two_strings_checked(
          mono_defaults.corlib, "System", "StackOverflowException", arg,
          NULL_HANDLE_STRING, error));
  mono_error_assert_ok(error);

  /*The ephemeron tombstone i*/
  domain->ephemeron_tombstone = MONO_HANDLE_RAW(
      mono_object_new_handle(domain, mono_defaults.object_class, error));
  mono_error_assert_ok(error);
  ...
  /*
   * This class is used during exception handling, so initialize it here, to
   * prevent stack overflows while handling stack overflows.
   */
 ...
 *
 */
void mono_runtime_init(MonoDomain *domain, MonoThreadStartCB start_cb,
...
void mono_runtime_init_checked(MonoDomain *domain, MonoThreadStartCB start_cb,
                               MonoThreadAttachCB attach_cb, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  mono_install_assembly_preload_hook_v2(mono_domain_assembly_preload,
                                        GUINT_TO_POINTER(FALSE), FALSE);
  mono_install_assembly_search_hook_v2(mono_domain_assembly_search,
                                       GUINT_TO_POINTER(FALSE), FALSE, FALSE);
  mono_install_assembly_search_hook_v2(mono_domain_assembly_postload_search,
                                       GUINT_TO_POINTER(FALSE), FALSE, TRUE);
  mono_install_assembly_load_hook_v2(mono_domain_fire_assembly_load, NULL);

#ifndef ENABLE_NETCORE // refonly hooks
  mono_install_assembly_preload_hook_v2(mono_domain_assembly_preload,
                                        GUINT_TO_POINTER(TRUE), TRUE);
  mono_install_assembly_search_hook_v2(mono_domain_assembly_search,
                                       GUINT_TO_POINTER(TRUE), TRUE, FALSE);
  mono_install_assembly_search_hook_v2(mono_domain_assembly_postload_search,
                                       GUINT_TO_POINTER(TRUE), TRUE, TRUE);
  mono_install_assembly_asmctx_from_path_hook(mono_domain_asmctx_from_path,
                                              NULL);
#endif
  ...
  if (!mono_runtime_get_no_exec()) {
    ...
    setup =
        MONO_HANDLE_CAST(MonoAppDomainSetup,
                         mono_object_new_pinned_handle(domain, klass, error));
    goto_if_nok(error, exit);

    klass = mono_class_get_appdomain_class();

    ad = MONO_HANDLE_CAST(MonoAppDomain,
                          mono_object_new_pinned_handle(domain, klass, error));
    goto_if_nok(error, exit);
  ...
  mono_domain_fire_assembly_load(mono_domain_default_alc(domain),
                                 mono_defaults.corlib->assembly, NULL, error);
  goto_if_nok(error, exit);
...
static void mono_context_set_default_context(MonoDomain *domain) {
  ...
  HANDLE_FUNCTION_ENTER();
  mono_context_set_handle(
      MONO_HANDLE_NEW(MonoAppContext, domain->default_context));
  HANDLE_FUNCTION_RETURN();
}

static char *mono_get_corlib_version(void) {
  ERROR_DECL(error);
  ...
  klass =
      mono_class_load_from_name(mono_defaults.corlib, "System", "Environment");
  mono_class_init_internal(klass);
  field =
      mono_class_get_field_from_name_full(klass, "mono_corlib_version", NULL);
  if (!field)
    return NULL;

  if (!(field->type->attrs &
        (FIELD_ATTRIBUTE_STATIC | FIELD_ATTRIBUTE_LITERAL)))
    return NULL;
...
static const char *mono_check_corlib_version_internal(void) {
  ...
  if (!version) {
    result = g_strdup_printf(
        "expected corlib string (%s) but not found or not string",
        MONO_CORLIB_VERSION);
    goto exit;
  }
  if (strcmp(version, MONO_CORLIB_VERSION) != 0) {
    result = g_strdup_printf(
        "The runtime did not find the mscorlib.dll it expected. "
  ...
  managed_offset = mono_field_get_offset(mono_class_get_field_from_name_full(
      mono_defaults.internal_thread_class, "last", NULL));
  if (native_offset != managed_offset)
    result = g_strdup_printf("expected InternalThread.last field offset %u, "
                             "found %u. See InternalThread.last comment",
                             native_offset, managed_offset);
exit:
  g_free(version);
  return result;
#endif
...
/**
 * mono_context_init:
 * \param domain The domain where the \c System.Runtime.Remoting.Context.Context
 * is initialized Initializes the \p domain's default \c System.Runtime.Remoting
 * 's Context.
 */
...
void mono_context_init_checked(MonoDomain *domain, MonoError *error) {
  ...
  klass = mono_class_load_from_name(
      mono_defaults.corlib, "System.Runtime.Remoting.Contexts", "Context");
  context = MONO_HANDLE_CAST(
      MonoAppContext, mono_object_new_pinned_handle(domain, klass, error));
  goto_if_nok(error, exit);
 ...
 */
void mono_install_runtime_cleanup(MonoDomainFunc func) { quit_function = func; }
 ...
 * \param configuration_file The configuration file to initialize the appdomain
 * with \returns a \c MonoDomain initialized with the appdomain
 */
MonoDomain *mono_domain_create_appdomain(char *friendly_name,
  ...
  MONO_ENTER_GC_UNSAFE;
  ERROR_DECL(error);
  domain = mono_domain_create_appdomain_checked(friendly_name,
                                                configuration_file, error);
  mono_error_cleanup(error);
  MONO_EXIT_GC_UNSAFE;
 ...
 * \param friendly_name The friendly name of the appdomain to create
 * \param configuration_file The configuration file to initialize the appdomain
 * with \param error Set on error.
 *
 * \returns a MonoDomain initialized with the appdomain.  On failure sets \p
 * error and returns NULL.
 */
MonoDomain *mono_domain_create_appdomain_checked(char *friendly_name,
                                                 char *configuration_file,
                                                 MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  ...
  if (configuration_file != NULL) {
    config_file =
        mono_string_new_handle(mono_domain_get(), configuration_file, error);
    goto_if_nok(error, leave);
  } else {
 ...
 * configuration system. ---> System.ArgumentException: The 'ExeConfigFilename'
 * argument cannot be null.' for some managed calls.
 */
void mono_domain_set_config(MonoDomain *domain, const char *base_dir,
...
gboolean mono_domain_set_config_checked(MonoDomain *domain,
                                        const char *base_dir,
                                        const char *config_file_name,
                                        MonoError *error) {
  error_init(error);
  MonoAppDomainSetupHandle setup =
      MONO_HANDLE_NEW(MonoAppDomainSetup, domain->setup);
  MonoStringHandle base_dir_str =
      mono_string_new_handle(domain, base_dir, error);
  goto_if_nok(error, leave);
  MONO_HANDLE_SET(setup, application_base, base_dir_str);
  MonoStringHandle config_file_name_str;
  config_file_name_str =
      mono_string_new_handle(domain, config_file_name, error);
  goto_if_nok(error, leave);
...
copy_app_domain_setup(MonoDomain *domain, MonoAppDomainSetupHandle setup,
                      MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  MonoAppDomainSetupHandle copy = MONO_HANDLE_CAST(
      MonoAppDomainSetup, mono_object_new_handle(domain, ads_class, error));
  goto_if_nok(error, leave);
...
#define XCOPY_FIELD(type, dst, field, src, error)                              \
  do {                                                                         \
    ...
    copied_val = MONO_HANDLE_CAST(                                             \
        type, mono_marshal_xdomain_copy_value_handle(                          \
                  MONO_HANDLE_CAST(MonoObject, src_val), error));              \
    goto_if_nok(error, leave);                                                 \
...
static MonoAppDomainHandle mono_domain_create_appdomain_internal(
  ...
  MonoAppDomainHandle ad = MONO_HANDLE_CAST(
      MonoAppDomain, mono_object_new_handle(data, adclass, error));
  goto_if_nok(error, leave);
  ...
  if (MONO_HANDLE_IS_NULL(app_base)) {
    ...
    MonoAppDomainSetupHandle root_setup =
        MONO_HANDLE_NEW(MonoAppDomainSetup, root->setup);
    MonoStringHandle root_app_base =
        MONO_HANDLE_NEW_GET(MonoString, root_setup, application_base);
    if (!MONO_HANDLE_IS_NULL(root_app_base)) {
      /* N.B. new string is in the new domain */
      uint32_t gchandle = mono_gchandle_from_handle(
          MONO_HANDLE_CAST(MonoObject, root_app_base), TRUE);
      MonoStringHandle s = mono_string_new_utf16_handle(
          data, mono_string_chars_internal(MONO_HANDLE_RAW(root_app_base)),
          mono_string_handle_length(root_app_base), error);
...
gboolean mono_domain_has_type_resolve(MonoDomain *domain) {
  // Check whether managed code is running, and if the managed AppDomain object
  // doesn't exist neither does the event handler
  if (!domain->domain)
    return FALSE;
 ...
 * \param typebuilder A \c System.Reflection.Emit.TypeBuilder, used if name is
 * NULL.
 *
 * This routine invokes the internal \c System.AppDomain.DoTypeResolve and
 * returns the assembly that matches name, or
 * ((TypeBuilder)typebuilder).FullName.
 *
 * \returns A \c MonoReflectionAssembly or NULL if not found
 */
MonoReflectionAssembly *
mono_domain_try_type_resolve(MonoDomain *domain, char *name,
                             MonoObject *typebuilder_raw) {
  HANDLE_FUNCTION_ENTER();
  ...
  if (name) {
    MonoStringHandle name_handle =
        mono_string_new_handle(mono_domain_get(), name, error);
    goto_if_nok(error, exit);
    ...
    ret = mono_domain_try_type_resolve_typebuilder(
        domain, MONO_HANDLE_CAST(MonoReflectionTypeBuilder, typebuilder),
        error);
#else
...
mono_domain_try_type_resolve_name(MonoDomain *domain, MonoAssembly *assembly,
  ...
  g_assert(alc_class);
  method = mono_class_get_method_from_name_checked(alc_class, "OnTypeResolve",
                                                   -1, 0, error);
...
exit:
  HANDLE_FUNCTION_RETURN_REF(MonoReflectionAssembly,
                             MONO_HANDLE_CAST(MonoReflectionAssembly, ret));
}
#else
/**
...
mono_class_get_appdomain_do_type_resolve_method(MonoError *error) {
  ...
  method = mono_class_get_method_from_name_checked(
      mono_class_get_appdomain_class(), "DoTypeResolve", -1, 0, error);
...
mono_class_get_appdomain_do_type_builder_resolve_method(MonoError *error) {
  ...
  method = mono_class_get_method_from_name_checked(
      mono_class_get_appdomain_class(), "DoTypeBuilderResolve", -1, 0, error);
 ...
 *
 * This routine invokes the internal \c System.AppDomain.DoTypeResolve and
 * returns the assembly that matches name.
 *
 * \returns A \c MonoReflectionAssembly or NULL if not found
 */
MonoReflectionAssemblyHandle
mono_domain_try_type_resolve_name(MonoDomain *domain, MonoAssembly *assembly,
  ...
  ret = mono_runtime_invoke_handle(
      method, MONO_HANDLE_CAST(MonoObject, appdomain), params, error);
  goto_if_nok(error, return_null);
  goto exit;
return_null:
  ret = NULL_HANDLE;
 ...
 * \param domain application domain in which to resolve the type
 * \param typebuilder A \c System.Reflection.Emit.TypeBuilder;
 * typebuilder.FullName is the name of the type to resolve.
 *
 * This routine invokes the internal \c System.AppDomain.DoTypeBuilderResolve
 * and returns the assembly that matches typebuilder.FullName.
 *
 * \returns A \c MonoReflectionAssembly or NULL_HANDLE if not found
 */
MonoReflectionAssemblyHandle mono_domain_try_type_resolve_typebuilder(
    MonoDomain *domain, MonoReflectionTypeBuilderHandle typebuilder,
    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  MonoObjectHandle ret;
  ret = mono_runtime_invoke_handle(
      method, MONO_HANDLE_CAST(MonoObject, appdomain), args, error);
  goto_if_nok(error, return_null);
  goto exit;
return_null:
  ret = NULL_HANDLE;
 ...
 * \returns Whether \p vtable_slot is inside a vtable which belongs to \p
 * domain.
 */
gboolean mono_domain_owns_vtable_slot(MonoDomain *domain,
...
#ifndef ENABLE_NETCORE
MonoObjectHandle ves_icall_System_AppDomain_GetData(MonoAppDomainHandle ad,
  ...
  MonoAppDomainSetupHandle ad_setup =
      MONO_HANDLE_NEW(MonoAppDomainSetup, add->setup);
  MonoStringHandle o;
  if (!strcmp(str, "APPBASE"))
    ...
    o = MONO_HANDLE_NEW(MonoString, (MonoString *)mono_g_hash_table_lookup(
                                        add->env, MONO_HANDLE_RAW(name)));
...
void ves_icall_System_AppDomain_SetData(MonoAppDomainHandle ad,
                                        MonoStringHandle name,
  ...
  mono_g_hash_table_insert_internal(add->env, MONO_HANDLE_RAW(name),
...
MonoAppDomainHandle ves_icall_System_AppDomain_getCurDomain(MonoError *error) {
  error_init(error);
...
MonoAppDomainHandle ves_icall_System_AppDomain_getRootDomain(MonoError *error) {
  error_init(error);
  MonoDomain *root = mono_get_root_domain();
...
MonoBoolean ves_icall_System_CLRConfig_CheckThrowUnobservedTaskExceptions(
    MonoError *error) {
...
static void start_element(GMarkupParseContext *context,
  ...
  if (strcmp(element_name, "ThrowUnobservedTaskExceptions") == 0) {
    const char *value =
        get_attribute_value(attribute_names, attribute_values, "enabled");
  ...
  runtime_config->domain->private_bin_path =
      get_attribute_value(attribute_names, attribute_values, "privatePath");
  if (runtime_config->domain->private_bin_path &&
      !runtime_config->domain->private_bin_path[0]) {
    g_free(runtime_config->domain->private_bin_path);
...
static void end_element(GMarkupParseContext *context, const gchar *element_name,
                        gpointer user_data, GError **gerror) {
  RuntimeConfig *runtime_config = (RuntimeConfig *)user_data;
  if (strcmp(element_name, "runtime") == 0)
    runtime_config->runtime_count--;
...
static void parse_error(GMarkupParseContext *context, GError *gerror,
                        gpointer user_data) {
...
static const GMarkupParser mono_parser = {start_element, end_element, NULL,
                                          NULL, parse_error};
...
void mono_domain_set_options_from_config(MonoDomain *domain) {
  ...
  config_file_name = mono_string_to_utf8_checked_internal(
      domain->setup->configuration_file, error);
  if (!is_ok(error)) {
    mono_error_cleanup(error);
    goto free_and_out;
  }
  ...
  offset = 0;
  if (len > 3 && text[0] == '\xef' && text[1] == (gchar)'\xbb' &&
      text[2] == '\xbf')
    offset = 3; /* Skip UTF-8 BOM */

  context = g_markup_parse_context_new(&mono_parser, (GMarkupParseFlags)0,
                                       &runtime_config, NULL);
...
ves_icall_System_AppDomain_createDomain(MonoStringHandle friendly_name,
  ...
  mono_error_set_not_supported(
      error, "AppDomain creation is not supported on this runtime.");
#else
  char *fname;
...
static gboolean add_assembly_to_array(MonoDomain *domain, MonoArrayHandle dest,
                                      int dest_idx, MonoAssembly *assm,
                                      MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoReflectionAssemblyHandle assm_obj =
      mono_assembly_get_object_handle(domain, assm, error);
  goto_if_nok(error, leave);
...
static MonoArrayHandle get_assembly_array_from_domain(MonoDomain *domain,
                                                      MonoBoolean refonly,
                                                      MonoError *error) {
  int i;
  ...
  MonoArrayHandle res = mono_array_new_handle(
      domain, mono_class_get_assembly_class(), assemblies->len, error);
  goto_if_nok(error, leave);
  for (i = 0; i < assemblies->len; ++i) {
    if (!add_assembly_to_array(domain, res, i,
                               (MonoAssembly *)g_ptr_array_index(assemblies, i),
                               error))
      goto leave;
  }
...
#else
MonoArrayHandle ves_icall_System_AppDomain_GetAssemblies(MonoAppDomainHandle ad,
                                                         MonoBoolean refonly,
                                                         MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_GETVAL(ad, data);
...
MonoAssembly *mono_try_assembly_resolve(MonoAssemblyLoadContext *alc,
                                        const char *fname_raw,
                                        MonoAssembly *requesting,
                                        gboolean refonly, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoAssembly *result = NULL;
  MonoStringHandle fname =
      mono_string_new_handle(mono_alc_domain(alc), fname_raw, error);
  goto_if_nok(error, leave);
  result =
      mono_try_assembly_resolve_handle(alc, fname, requesting, refonly, error);
leave:
  HANDLE_FUNCTION_RETURN_VAL(result);
}

MonoAssembly *mono_try_assembly_resolve_handle(MonoAssemblyLoadContext *alc,
                                               MonoStringHandle fname,
                                               MonoAssembly *requesting,
                                               gboolean refonly,
                                               MonoError *error) {
  ...
  method = mono_class_get_method_from_name_checked(
      mono_class_get_appdomain_class(), "DoAssemblyResolve", -1, 0, error);
  ...
  if (requesting) {
    requesting_handle =
        mono_assembly_get_object_handle(domain, requesting, error);
    goto_if_nok(error, leave);
  }
  ...
  result = MONO_HANDLE_CAST(
      MonoReflectionAssembly,
      MONO_HANDLE_NEW(MonoObject,
                      mono_runtime_try_invoke(method, domain->domain, params,
                                              &exc, error)));
  ...
  ret = !MONO_HANDLE_IS_NULL(result) ? MONO_HANDLE_GETVAL(result, assembly)
                                     : NULL;

  if (ret && !refonly &&
      mono_asmctx_get_kind(&ret->context) == MONO_ASMCTX_REFONLY) {
    /* .NET Framework throws System.IO.FileNotFoundException in this case */
    mono_error_set_file_not_found(error, NULL,
                                  "AssemblyResolveEvent handlers cannot return "
                                  "Assemblies loaded for reflection only");
    ret = NULL;
    goto leave;
  }
#else
  ...
  g_assert(alc_class);
  method = mono_class_get_method_from_name_checked(
      alc_class, "OnAssemblyResolve", -1, 0, local_error);
  ...
  result = MONO_HANDLE_CAST(
      MonoReflectionAssembly,
      mono_runtime_try_invoke_handle(method, NULL_HANDLE, params, error));
  goto_if_nok(error, leave);
...
MonoAssembly *mono_domain_assembly_postload_search(
  ...
  assembly =
      mono_try_assembly_resolve(alc, aname_str, requesting, refonly, error);
  g_free(aname_str);
  mono_error_cleanup(error);
...
static void add_assemblies_to_domain(MonoDomain *domain, MonoAssembly *ass,
  ...
  if (ass->image->references) {
    for (i = 0; i < ass->image->nreferences; i++) {
      if (ass->image->references[i] &&
          ass->image->references[i] != REFERENCE_MISSING) {
...
static void add_assembly_to_alc(MonoAssemblyLoadContext *alc,
  ...
  mono_assembly_addref(ass);
  // Prepending here will break the test suite with frequent
  // InvalidCastExceptions, so we have to append
  alc->loaded_assemblies = g_slist_append(alc->loaded_assemblies, ass);
  mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_ASSEMBLY,
             "Assembly %s[%p] added to ALC (%p), ref_count=%d", ass->aname.name,
             ass, (gpointer)alc, ass->ref_count);

  if (ass->image->references) {
    for (i = 0; i < ass->image->nreferences; i++) {
      // TODO: remove all this after we're confident this assert isn't hit
      g_assertf(!ass->image->references[i],
...
static void mono_domain_fire_assembly_load_event(MonoDomain *domain,
                                                 MonoAssembly *assembly,
                                                 MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  MonoClass *alc_class = mono_class_get_assembly_load_context_class();
  g_assert(alc_class);
  method = mono_class_get_method_from_name_checked(alc_class, "OnAssemblyLoad",
                                                   -1, 0, error);
  ...
  MonoReflectionAssemblyHandle assembly_handle =
      mono_assembly_get_object_handle(domain, assembly, error);
  goto_if_nok(error, exit);
  ...
  MonoObjectHandle appdomain =
      MONO_HANDLE_NEW(MonoObject, &domain->domain->mbr.obj);
  MonoClass *klass = mono_handle_class(appdomain);

  MONO_STATIC_POINTER_INIT(MonoClassField, assembly_load_field)

  assembly_load_field =
      mono_class_get_field_from_name_full(klass, "AssemblyLoad", NULL);
  ...
  assembly_load_method = mono_class_get_method_from_name_checked(
      klass, "DoAssemblyLoad", -1, 0, error);
  ...
  mono_runtime_invoke_handle_void(assembly_load_method, appdomain, params,
                                  error);
#endif
...
static void mono_domain_fire_assembly_load(MonoAssemblyLoadContext *alc,
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
             "Loading assembly %s (%p) into domain %s (%p) and ALC %p",
             assembly->aname.name, assembly, domain->friendly_name, domain,
             alc);
...
static gboolean mono_domain_asmctx_from_path(
  ...
  for (search_path = domain->search_path; search_path && *search_path;
       search_path++) {
 ...
 */
static void set_domain_search_path(MonoDomain *domain) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MonoAppDomainSetupHandle setup;
  gchar **tmp;
  ...
  if (MONO_HANDLE_GET_BOOL(setup, private_bin_path)) {
    search_path = mono_string_handle_to_utf8(
        MONO_HANDLE_NEW_GET(MonoString, setup, private_bin_path), error);
    if (!is_ok(error)) { /*FIXME maybe we should bubble up the error.*/
      g_warning("Could not decode AppDomain search path since it contains "
                "invalid characters");
      goto exit;
    }
  }
  ...
  if (domain->private_bin_path) {
    if (search_path == NULL)
      ...
      search_path = g_strjoin(";", search_path, domain->private_bin_path,
                              (const char *)NULL);
      g_free(tmp2);
    }
  }
  ...
  if (search_path) {
    /*
     * As per MSDN documentation, AppDomainSetup.PrivateBinPath contains a list
     * of directories relative to ApplicationBase separated by semicolons (see
     * http://msdn2.microsoft.com/en-us/library/system.appdomainsetup.privatebinpath.aspx)
     * The loop below copes with the fact that some Unix applications may use
     * ':' (or System.IO.Path.PathSeparator) as the path search separator. We
     * replace it with
  ...
  *tmp = mono_string_handle_to_utf8(
      MONO_HANDLE_NEW_GET(MonoString, setup, application_base), error);
  if (!is_ok(error)) {
    g_free(tmp);
    goto exit;
  }
...
gboolean mono_is_shadow_copy_enabled(MonoDomain *domain,
...
static gboolean shadow_copy_sibling(const gchar *src_pristine, gint srclen,
                                    const char *extension,
                                    ShadowCopySiblingExt extopt,
                                    const gchar *target_pristine,
                                    gint targetlen) {
  gchar *file = NULL;
  gunichar2 *orig = NULL;
  gunichar2 *dest = NULL;
  ...
  /* Fix for bug #556884 - make sure the files have the correct mode so that
   * they can be overwritten when updated in their original locations. */
  if (copy_result)
...
static gint32 get_cstring_hash(const char *str) {
  const char *p;
  gint32 h = 0;
...
static char *get_shadow_assembly_location_base(MonoDomain *domain,
  ...
  if (setup->cache_path != NULL && setup->application_name != NULL) {
    ...
    appname =
        mono_string_to_utf8_checked_internal(setup->application_name, error);
    if (!is_ok(error)) {
      g_free(cache_path);
      return NULL;
    }
...
static char *get_shadow_assembly_location(const char *filename,
                                          MonoError *error) {
  ...
  g_snprintf(path_hash, sizeof(path_hash), "%08x_%08x_%08x", hash ^ hash2,
             hash2, domain->shadow_serial);
  ...
  location =
      g_build_filename(tmploc, name_hash, path_hash, bname, (const char *)NULL);
  g_free(tmploc);
  g_free(bname);
  g_free(dirname);
  return location;
...
static gboolean private_file_needs_copying(const char *src,
                                           struct stat *sbuf_src, char *dest) {
...
static gboolean shadow_copy_create_ini(const char *shadow,
                                       const char *filename) {
  ...
  char *ini_file =
      g_build_filename(dir_name, "__AssemblyInfo__.ini", (const char *)NULL);
  g_free(dir_name);
  ...
  handle = mono_w32file_create(u16_ini, GENERIC_WRITE,
  ...
  win32error = 0;
  result =
      mono_w32file_write(handle, full_path, strlen(full_path), &n, &win32error);
exit:
  if (handle != INVALID_HANDLE_VALUE)
...
gboolean mono_is_shadow_copy_enabled(MonoDomain *domain,
  ...
  shadow_status_string =
      mono_string_to_utf8_checked_internal(setup->shadow_copy_files, error);
  if (!is_ok(error))
    goto exit;
  ...
  all_dirs = mono_string_to_utf8_checked_internal(
      setup->shadow_copy_directories, error);
  if (!is_ok(error))
    goto exit;
...
/*
This function raises exceptions so it can cause as sorts of nasty stuff if
called while holding a lock. Returns old file name if shadow copy is disabled,
new shadow copy file name if successful or NULL if source file not found.
FIXME bubble up the error instead of raising it here
*/
char *mono_make_shadow_copy(const char *filename, MonoError *oerror) {
  ERROR_DECL(error);
  ...
  if (!is_ok(error)) {
    mono_error_cleanup(error);
    g_free(dir_name);
    mono_error_set_execution_engine(oerror,
                                    "Failed to create shadow copy (invalid "
                                    "characters in shadow directory name).");
    return NULL;
  }
  ...
  if (g_ensure_directory_exists(shadow) == FALSE) {
    g_free(shadow);
    mono_error_set_execution_engine(
        oerror, "Failed to create shadow copy (ensure directory exists).");
    return NULL;
  }
   ...
   * error but rather let the assembly fail to load. This ensures you can do
   * Type.GetType("NS.T, NonExistantAssembly) and not have it runtime error" */
  ...
  if (copy_result == FALSE) {
    ...
    /* Fix for bug #17251 - if file not found try finding assembly by other
     * means (it is not fatal error) */
    if (mono_w32error_get_last() == ERROR_FILE_NOT_FOUND ||
        mono_w32error_get_last() == ERROR_PATH_NOT_FOUND)
      return NULL; /* file not found, shadow copy failed */

    mono_error_set_execution_engine(
        oerror, "Failed to create shadow copy (mono_w32file_copy).");
    return NULL;
  }
  ...
  copy_result =
      shadow_copy_sibling(filename, filename_len, ".mdb",
                          SHADOW_COPY_SIBLING_EXT_APPEND, shadow, shadow_len);
  if (copy_result)
    copy_result = shadow_copy_sibling(filename, filename_len, ".pdb",
                                      SHADOW_COPY_SIBLING_EXT_REPLACE, shadow,
                                      shadow_len);
  if (copy_result)
    copy_result =
        shadow_copy_sibling(filename, filename_len, ".config",
                            SHADOW_COPY_SIBLING_EXT_APPEND, shadow, shadow_len);
...
MonoDomain *mono_domain_from_appdomain_handle(MonoAppDomainHandle appdomain) {
  HANDLE_FUNCTION_ENTER();
  MonoDomain *dom = NULL;
  ...
  if (mono_class_is_transparent_proxy(mono_handle_class(appdomain))) {
    MonoTransparentProxyHandle tp =
        MONO_HANDLE_CAST(MonoTransparentProxy, appdomain);
...
static gboolean try_load_from(MonoAssembly **assembly, const gchar *path1,
                              const gchar *path2, const gchar *path3,
                              const gchar *path4,
...
static MonoAssembly *real_load(gchar **search_path, const gchar *culture,
                               const gchar *name,
                               const MonoAssemblyOpenRequest *req) {
  MonoAssembly *result = NULL;
  gchar **path;
...
static char *get_app_context_base_directory(MonoError *error) {
  ...
  get_basedir = mono_class_get_method_from_name_checked(
      app_context, "get_BaseDirectory", -1, 0, local_error);
  mono_error_assert_ok(local_error);
  ...
  MonoStringHandle result = MONO_HANDLE_CAST(
      MonoString,
      mono_runtime_try_invoke_handle(get_basedir, NULL_HANDLE, NULL, error));
 ...
 * LOCKING: This is called from the assembly loading code, which means the
 * caller might hold the loader lock. Thus, this function must not acquire the
 * domain lock.
 */
static MonoAssembly *mono_domain_assembly_preload(MonoAssemblyLoadContext *alc,
                                                  MonoAssemblyName *aname,
  ...
  mono_assembly_request_prepare_open(
      &req, refonly ? MONO_ASMCTX_REFONLY : MONO_ASMCTX_DEFAULT, alc);
  ...
  if (domain->search_path && domain->search_path[0] != NULL) {
    if (mono_trace_is_traced(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY)) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
                 "Domain %s search path is:", domain->friendly_name);
      for (int i = 0; domain->search_path[i]; i++) {
        const char *p = domain->search_path[i];
        mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY, "\tpath[%d] = '%s'",
                   i, p);
      }
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
                 "End of domain %s search path.", domain->friendly_name);
    }
 ...
 *
 * \param assemblies_path directories to search for given assembly name,
 * terminated by NULL \param aname assembly name to look for \param asmctx
 * assembly load context for this load operation
 *
 ...
 */
MonoAssembly *
mono_assembly_load_from_assemblies_path(gchar **assemblies_path,
                                        MonoAssemblyName *aname,
  ...
  MonoAssemblyOpenRequest req;
  mono_assembly_request_prepare_open(
      &req, asmctx, mono_domain_default_alc(mono_domain_get()));
  req.request.predicate = predicate;
  req.request.predicate_ud = predicate_ud;
  MonoAssembly *result = NULL;
...
mono_domain_assembly_search(MonoAssemblyLoadContext *alc,
  ...
  const MonoAssemblyNameEqFlags eq_flags = (MonoAssemblyNameEqFlags)(
      strong_name ? MONO_ANAME_EQ_IGNORE_CASE
                  : (MONO_ANAME_EQ_IGNORE_PUBKEY |
                     MONO_ANAME_EQ_IGNORE_VERSION | MONO_ANAME_EQ_IGNORE_CASE));
  ...
  for (tmp = alc->loaded_assemblies; tmp; tmp = tmp->next) {
    ...
    g_assert(ass != NULL);
    // TODO: Can dynamic assemblies match here for netcore? Also, this ignores
    // case while exact_sn_match does not.
    if (assembly_is_dynamic(ass) ||
        !mono_assembly_names_equal_flags(aname, &ass->aname, eq_flags))
      continue;
  ...
  for (tmp = domain->domain_assemblies; tmp; tmp = tmp->next) {
    ...
    gboolean ass_ref_only =
        mono_asmctx_get_kind(&ass->context) == MONO_ASMCTX_REFONLY;
    if (assembly_is_dynamic(ass) || refonly != ass_ref_only ||
        !mono_assembly_names_equal_flags(aname, &ass->aname, eq_flags))
      continue;
...
#if ENABLE_NETCORE
MonoReflectionAssemblyHandle ves_icall_System_Reflection_Assembly_InternalLoad(
    MonoStringHandle name_handle, MonoStackCrawlMark *stack_mark,
    gpointer load_Context, MonoError *error) {
  error_init(error);
  ...
  req.basedir = NULL;
  /* Everything currently goes through this function, and the postload hook (aka
   * the AppDomain.AssemblyResolve event) is triggered under some scenarios.
   * It's not completely obvious to me in what situations (if any) this should
   * be disabled, other than for corlib satellite assemblies (which I've dealt
   * with further down the call stack).
   */
...
#ifndef ENABLE_NETCORE
MonoReflectionAssemblyHandle ves_icall_System_Reflection_Assembly_LoadFrom(
    MonoStringHandle fname, MonoBoolean refOnly, MonoStackCrawlMark *stack_mark,
    MonoError *error) {
  error_init(error);
  MonoDomain *domain = mono_domain_get();
  char *name, *filename;
  ...
  MonoAssemblyOpenRequest req;
  mono_assembly_request_prepare_open(
      &req, refOnly ? MONO_ASMCTX_REFONLY : MONO_ASMCTX_LOADFROM,
...
static MonoAssembly *mono_alc_load_file(MonoAssemblyLoadContext *alc,
                                        MonoStringHandle fname,
...
ves_icall_System_Reflection_Assembly_LoadFile_internal(
    MonoStringHandle fname, MonoStackCrawlMark *stack_mark, MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  MonoReflectionAssemblyHandle result =
      MONO_HANDLE_CAST(MonoReflectionAssembly, NULL_HANDLE);
  MonoAssembly *executing_assembly;
  ...
  MonoAssembly *ass =
      mono_alc_load_file(mono_domain_default_alc(domain), fname,
                         executing_assembly, MONO_ASMCTX_INDIVIDUAL, error);
  goto_if_nok(error, leave);
...
ves_icall_System_Runtime_Loader_AssemblyLoadContext_InternalLoadFile(
    gpointer alc_ptr, MonoStringHandle fname, MonoStackCrawlMark *stack_mark,
    MonoError *error) {
  MonoReflectionAssemblyHandle result =
      MONO_HANDLE_CAST(MonoReflectionAssembly, NULL_HANDLE);
  ...
  MonoAssembly *executing_assembly;
  executing_assembly = mono_runtime_get_caller_from_stack_mark(stack_mark);
  MonoAssembly *ass = mono_alc_load_file(
      alc, fname, executing_assembly,
      mono_alc_is_default(alc) ? MONO_ASMCTX_LOADFROM : MONO_ASMCTX_INDIVIDUAL,
      error);
  goto_if_nok(error, leave);
...
static MonoAssembly *
mono_alc_load_raw_bytes(MonoAssemblyLoadContext *alc, guint8 *raw_assembly,
                        guint32 raw_assembly_len, guint8 *raw_symbol_data,
                        guint32 raw_symbol_len, gboolean refonly,
                        MonoError *error);

#ifdef ENABLE_NETCORE
MonoReflectionAssemblyHandle
ves_icall_System_Runtime_Loader_AssemblyLoadContext_InternalLoadFromStream(
    gpointer native_alc, gpointer raw_assembly_ptr, gint32 raw_assembly_len,
    gpointer raw_symbols_ptr, gint32 raw_symbols_len, MonoError *error) {
  MonoAssemblyLoadContext *alc = (MonoAssemblyLoadContext *)native_alc;
  MonoDomain *domain = mono_alc_domain(alc);
  MonoReflectionAssemblyHandle result =
      MONO_HANDLE_CAST(MonoReflectionAssembly, NULL_HANDLE);
  MonoAssembly *assm = NULL;
  assm = mono_alc_load_raw_bytes(alc, (guint8 *)raw_assembly_ptr,
                                 raw_assembly_len, (guint8 *)raw_symbols_ptr,
                                 raw_symbols_len, FALSE, error);
  goto_if_nok(error, leave);
...
#else
MonoReflectionAssemblyHandle ves_icall_System_AppDomain_LoadAssemblyRaw(
    MonoAppDomainHandle ad, MonoArrayHandle raw_assembly,
    MonoArrayHandle raw_symbol_store, MonoObjectHandle evidence,
    MonoBoolean refonly, MonoError *error) {
  MonoAssembly *ass;
  MonoReflectionAssemblyHandle refass =
      MONO_HANDLE_CAST(MonoReflectionAssembly, NULL_HANDLE);
  MonoDomain *domain = MONO_HANDLE_GETVAL(ad, data);
  ...
  /* Copy the data ourselves to unpin the raw assembly byte array as soon as
   * possible */
  guint8 *assembly_data = (guint8 *)g_try_malloc(raw_assembly_len);
  if (!assembly_data) {
    mono_error_set_out_of_memory(
        error, "Could not allocate %ud bytes to copy raw assembly data",
        raw_assembly_len);
    return refass;
  }
  uint32_t gchandle;
  mono_byte *raw_data =
      (mono_byte *)MONO_ARRAY_HANDLE_PIN(raw_assembly, gchar, 0, &gchandle);
  memcpy(assembly_data, raw_data, raw_assembly_len);
  mono_gchandle_free_internal(gchandle); /* unpin */
  MONO_HANDLE_ASSIGN(raw_assembly,
                     NULL_HANDLE); /* don't reference the data anymore */
  ...
  if (!MONO_HANDLE_IS_NULL(raw_symbol_store)) {
    ...
    raw_symbol_data = (mono_byte *)MONO_ARRAY_HANDLE_PIN(
        raw_symbol_store, mono_byte, 0, &symbol_gchandle);
  }

  ass = mono_alc_load_raw_bytes(alc, assembly_data, raw_assembly_len,
                                raw_symbol_data, symbol_len, refonly, error);
...
mono_alc_load_raw_bytes(MonoAssemblyLoadContext *alc, guint8 *assembly_data,
  ...
  MonoAssembly *ass = NULL;
  MonoImageOpenStatus status;
  MonoImage *image = mono_image_open_from_data_internal(
      alc, (char *)assembly_data, raw_assembly_len, FALSE, NULL, refonly, FALSE,
      NULL);

  if (!image) {
    mono_error_set_bad_image_by_name(error, "In memory assembly", "0x%p",
                                     assembly_data);
    return ass;
  }
  ...
  if ((redirected_asm =
           mono_assembly_binding_applies_to_image(alc, image, &new_status))) {
    mono_image_close(image);
    image = redirected_asm->image;
    mono_image_addref(
        image); /* so that mono_image close, below, has something to do */
  } else if (new_status != MONO_IMAGE_OK) {
    mono_image_close(image);
    mono_error_set_bad_image_by_name(error, "In Memory assembly",
                                     "0x%p was assembly binding redirected to "
                                     "another assembly that failed to load",
                                     assembly_data);
    return ass;
  }
  ...
  mono_assembly_request_prepare_load(
      &req, refonly ? MONO_ASMCTX_REFONLY : MONO_ASMCTX_INDIVIDUAL, alc);
...
#ifndef ENABLE_NETCORE
MonoReflectionAssemblyHandle ves_icall_System_AppDomain_LoadAssembly(
    MonoAppDomainHandle ad, MonoStringHandle assRef, MonoObjectHandle evidence,
    MonoBoolean refOnly, MonoStackCrawlMark *stack_mark, MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_GETVAL(ad, data);
  MonoImageOpenStatus status = MONO_IMAGE_OK;
  MonoAssembly *ass;
  MonoAssemblyName aname;
  ...
  if (!parsed) {
    ...
    if (!refOnly) {
      MonoAssembly *assm = mono_try_assembly_resolve_handle(
          mono_domain_default_alc(domain), assRef, NULL, refOnly, error);
      goto_if_nok(error, fail);
      if (assm) {
  ...
  if (!refOnly) {
    ...
    MonoAssembly *executing_assembly =
        mono_runtime_get_caller_from_stack_mark(stack_mark);
    if (executing_assembly &&
        mono_asmctx_get_kind(&executing_assembly->context) ==
  ...
  if (!ass) {
    /* MS.NET doesn't seem to call the assembly resolve handler for refonly
     * assemblies */
    if (!refOnly) {
      ass = mono_try_assembly_resolve_handle(mono_domain_default_alc(domain),
                                             assRef, NULL, refOnly, error);
      goto_if_nok(error, fail);
    }
    if (!ass)
      goto fail;
...
void ves_icall_System_AppDomain_InternalUnload(gint32 domain_id,
                                               MonoError *error) {
  MonoDomain *domain = mono_domain_get_by_id(domain_id);

  if (NULL == domain) {
    mono_error_set_execution_engine(
        error, "Failed to unload domain, domain id not found");
    return;
  }

  if (domain == mono_get_root_domain()) {
    mono_error_set_generic_error(error, "System",
...
void ves_icall_System_AppDomain_DoUnhandledException(MonoAppDomainHandle ad,
...
gint32 ves_icall_System_AppDomain_ExecuteAssembly(
    MonoAppDomainHandle ad, MonoReflectionAssemblyHandle refass,
    MonoArrayHandle args, MonoError *error) {
  MonoImage *image;
  MonoMethod *method;
  ...
  method = mono_get_method_checked(image, mono_image_get_entry_point(image),
                                   NULL, NULL, error);

  if (!method)
    g_error("No entry point method found in %s due to %s", image->name,
            mono_error_get_message(error));

  if (MONO_HANDLE_IS_NULL(args)) {
    MonoDomain *domain = MONO_HANDLE_GETVAL(ad, data);
    MONO_HANDLE_ASSIGN(args, mono_array_new_handle(
                                 domain, mono_defaults.string_class, 0, error));
    mono_error_assert_ok(error);
  }

  int res =
      mono_runtime_exec_main_checked(method, MONO_HANDLE_RAW(args), error);
  return res;
}

MonoAppDomainHandle
ves_icall_System_AppDomain_InternalSetDomain(MonoAppDomainHandle ad,
                                             MonoError *error) {
  error_init(error);
  MonoDomain *old_domain = mono_domain_get();
...
void ves_icall_System_AppDomain_InternalPushDomainRef(MonoAppDomainHandle ad,
                                                      MonoError *error) {
  error_init(error);
  mono_thread_push_appdomain_ref(MONO_HANDLE_GETVAL(ad, data));
}

void ves_icall_System_AppDomain_InternalPushDomainRefByID(gint32 domain_id,
                                                          MonoError *error) {
  error_init(error);
  MonoDomain *domain = mono_domain_get_by_id(domain_id);
...
void ves_icall_System_AppDomain_InternalPopDomainRef(MonoError *error) {
  error_init(error);
  mono_thread_pop_appdomain_ref();
}
...
MonoAppContextHandle
ves_icall_System_AppDomain_InternalSetContext(MonoAppContextHandle mc,
                                              MonoError *error) {
  error_init(error);
...
MonoStringHandle
ves_icall_System_AppDomain_InternalGetProcessGuid(MonoStringHandle newguid,
                                                  MonoError *error) {
  error_init(error);
  ...
  if (process_guid_set) {
    ...
    return mono_string_new_utf16_handle(mono_domain_get(), process_guid,
                                        sizeof(process_guid) / 2, error);
  }
  uint32_t gchandle =
      mono_gchandle_from_handle(MONO_HANDLE_CAST(MonoObject, newguid), TRUE);
  memcpy(process_guid, mono_string_chars_internal(MONO_HANDLE_RAW(newguid)),
         sizeof(process_guid));
  mono_gchandle_free_internal(gchandle);
...
gboolean mono_domain_is_unloading(MonoDomain *domain) {
  if (domain->state == MONO_APPDOMAIN_UNLOADING ||
...
static void clear_cached_vtable(MonoVTable *vtable) {
...
static G_GNUC_UNUSED void zero_static_data(MonoVTable *vtable) {
  MonoClass *klass = vtable->klass;
  void *data;
...
static void unload_data_unref(unload_data *data) {
  if (!data)
    return;
  gint32 count;
  do {
...
static void deregister_reflection_info_roots_from_list(MonoImage *image) {
...
static void deregister_reflection_info_roots(MonoDomain *domain) {
  GSList *list;
  ...
  for (list = domain->domain_assemblies; list; list = list->next) {
    ...
    /*
     * No need to take the image lock here since dynamic images are appdomain
     * bound and at this point the mutator is gone.  Taking the image lock here
     * would mean promoting it from a simple lock to a complex lock, which we
...
static gsize WINAPI unload_thread_main(void *arg) {
  ...
  if (!mono_threads_abort_appdomain_threads(domain, -1)) {
    data->failure_reason = g_strdup_printf(
        "Aborting of threads in domain %s timed out.", domain->friendly_name);
    goto failure;
  }

  if (!mono_threadpool_remove_domain_jobs(domain, -1)) {
    data->failure_reason =
        g_strdup_printf("Cleanup of threadpool jobs of domain %s timed out.",
                        domain->friendly_name);
    goto failure;
  }
  ...
  if (!mono_domain_finalize(domain, -1)) {
    data->failure_reason = g_strdup_printf(
        "Finalization of domain %s timed out.", domain->friendly_name);
    goto failure;
  }
  ...
  for (i = 0; i < domain->class_vtable_array->len; ++i)
    zero_static_data(
        (MonoVTable *)g_ptr_array_index(domain->class_vtable_array, i));
  mono_gc_collect(0);
  for (i = 0; i < domain->class_vtable_array->len; ++i)
    clear_cached_vtable(
        (MonoVTable *)g_ptr_array_index(domain->class_vtable_array, i));
...
static MonoThreadInfoWaitRet guarded_wait(MonoThreadHandle *thread_handle,
...
void mono_domain_try_unload(MonoDomain *domain, MonoObject **exc) {
  ...
  prev_state = (MonoAppDomainState)mono_atomic_cas_i32(
      (gint32 *)&domain->state, MONO_APPDOMAIN_UNLOADING_START,
      MONO_APPDOMAIN_CREATED);
  if (prev_state != MONO_APPDOMAIN_CREATED) {
    switch (prev_state) {
    case MONO_APPDOMAIN_UNLOADING_START:
    case MONO_APPDOMAIN_UNLOADING:
      *exc = (MonoObject *)mono_get_exception_cannot_unload_appdomain(
  ...
  method = mono_class_get_method_from_name_checked(
      domain->domain->mbr.obj.vtable->klass, "DoDomainUnload", -1, 0, error);
  g_assert(method);
   ...
   */
  internal = mono_thread_create_internal_handle(
      mono_get_root_domain(), unload_thread_main, thread_data,
      MONO_THREAD_CREATE_FLAGS_FORCE_CREATE, error);
  mono_error_assert_ok(error);

  thread_handle =
      mono_threads_open_thread_handle(MONO_HANDLE_GETVAL(internal, handle));

  /* Wait for the thread */
  while (!thread_data->done &&
         guarded_wait(thread_handle, MONO_INFINITE_WAIT, TRUE) ==
             MONO_THREAD_INFO_WAIT_RET_ALERTED) {
    if (mono_thread_internal_has_appdomain_ref(mono_thread_internal_current(),
                                               domain) &&
...
/* Remember properties so they can be be installed in AppContext during runtime
 * init */
void mono_runtime_register_appctx_properties(int nprops, const char **keys,
                                             const char **values) {
  ...
  for (int i = 0; i < nprops; ++i) {
    appctx_keys[i] =
        g_utf8_to_utf16(keys[i], strlen(keys[i]), NULL, NULL, NULL);
    appctx_values[i] =
        g_utf8_to_utf16(values[i], strlen(values[i]), NULL, NULL, NULL);
  }
}
...
static GENERATE_GET_CLASS_WITH_CACHE(appctx, "System", "AppContext")
  ...
  MonoMethod *setup = mono_class_get_method_from_name_checked(
      mono_class_get_appctx_class(), "Setup", 3, 0, error);
  g_assert(setup);
