...
static mono_lazy_init_t reference_queue_mutex_inited =
...
static void object_register_finalizer(MonoObject *obj,
...
static void break_coop_alertable_wait(gpointer user_data) {
 ...
 */
static gint coop_cond_timedwait_alertable(MonoCoopCond *cond,
                                          MonoCoopMutex *mutex,
                                          guint32 timeout_ms,
                                          gboolean *alertable) {
  ...
  if (alertable) {
    ...
    mono_thread_info_install_interrupt(break_coop_alertable_wait, ud,
                                       alertable);
    if (*alertable) {
      g_free(ud);
      return 0;
...
void mono_gc_run_finalize(void *obj, void *data) {
  ...
  // This function is called from the innards of the GC, so our best alternative
  // for now is to do polling here
  ...
  if (mono_log_finalizers)
    g_log("mono-gc-finalizers", G_LOG_LEVEL_DEBUG,
          "<%s at %p> Starting finalizer checks.", o_name, o);
  ...
  if (mono_log_finalizers)
    g_log("mono-gc-finalizers", G_LOG_LEVEL_MESSAGE,
          "<%s at %p> Registered finalizer as processed.", o_name, o);
  ...
  if (m_class_get_image(mono_object_class(o)) == mono_defaults.corlib &&
      !strcmp(o_name, "DynamicMethod") && finalizing_root_domain) {
    /*
   ...
   * (o)->name_space, mono_object_class (o)->name); */
  ...
  if (mono_log_finalizers)
    g_log("mono-gc-finalizers", G_LOG_LEVEL_MESSAGE,
          "<%s at %p> Compiling finalizer.", o_name, o);

#ifndef HOST_WASM
  if (!domain->finalize_runtime_invoke) {
    MonoMethod *finalize_method = mono_class_get_method_from_name_checked(
        mono_defaults.object_class, "Finalize", 0, 0, error);
    mono_error_assert_ok(error);
    MonoMethod *invoke = mono_marshal_get_runtime_invoke(finalize_method, TRUE);

    domain->finalize_runtime_invoke =
        mono_compile_method_checked(invoke, error);
    mono_error_assert_ok(error); /* expect this not to fail */
  }

  RuntimeInvokeFunction runtime_invoke =
      (RuntimeInvokeFunction)domain->finalize_runtime_invoke;
#endif
  ...
  if (mono_log_finalizers)
    g_log("mono-gc-finalizers", G_LOG_LEVEL_MESSAGE,
          "<%s at %p> Calling finalizer.", o_name, o);

  MONO_PROFILER_RAISE(gc_finalizing_object, (o));

#ifdef HOST_WASM
  if (finalizer) { // null finalizers work fine when using the vcall invoke as
                   // Object has an empty one
    gpointer params[1];
    params[0] = NULL;
  ...
  if (mono_log_finalizers)
    g_log("mono-gc-finalizers", G_LOG_LEVEL_MESSAGE,
          "<%s at %p> Returned from finalizer.", o_name, o);
...
/*
 * Some of our objects may point to a different address than the address
 * returned by GC_malloc() (because of the GetHashCode hack), but we need to
 * pass the real address to register_finalizer. This also means that in the
 * callback we need to adjust the pointer to get back the real MonoObject*. We
 * also need to be consistent in the use of the GC_debug* variants of malloc and
 * register_finalizer, since that, too, can cause the underlying pointer to be
 * offset.
 */
static void object_register_finalizer(MonoObject *obj,
                                      void (*callback)(void *, void *)) {
...
void mono_object_register_finalizer_handle(MonoObjectHandle obj) {
  /* g_print ("Registered finalizer on %p %s.%s\n", obj, mono_handle_class
   * (obj)->name_space, mono_handle_class (obj)->name); */
  object_register_finalizer(MONO_HANDLE_RAW(obj), mono_gc_run_finalize);
}

static void mono_object_unregister_finalizer_handle(MonoObjectHandle obj) {
...
void mono_object_register_finalizer(MonoObject *obj) {
  /* g_print ("Registered finalizer on %p %s.%s\n", obj, mono_object_class
   * (obj)->name_space, mono_object_class (obj)->name); */
 ...
 * \param timeout msecs to wait for the finalization to complete, \c -1 to wait
 * indefinitely
 *
...
gboolean mono_domain_finalize(MonoDomain *domain, guint32 timeout) {
  ...
  for (;;) {
    if (timeout == MONO_INFINITE_WAIT) {
      ...
      res = mono_coop_sem_timedwait(&req->done, timeout - elapsed,
    ...
    if (res == MONO_SEM_TIMEDWAIT_RET_SUCCESS) {
      ...
      if ((thread->state &
           (ThreadState_AbortRequested | ThreadState_SuspendRequested)) != 0) {
        ret = FALSE;
        break;
      }
  ...
  if (!ret) {
    ...
    if (found) {
      ...
      if (mono_atomic_dec_i32(&req->ref) != 1)
        g_error(
            "%s: req->ref should be 1, as we are the first one to decrement it",
            __func__);
    }
...
void ves_icall_System_GC_GetGCMemoryInfo(
    gint64 *high_memory_load_threshold_bytes, gint64 *memory_load_bytes,
    gint64 *total_available_memory_bytes, gint64 *heap_size_bytes,
    gint64 *fragmented_bytes) {
  mono_gc_get_gcmemoryinfo(high_memory_load_threshold_bytes, memory_load_bytes,
                           total_available_memory_bytes, heap_size_bytes,
                           fragmented_bytes);
}

void ves_icall_System_GC_ReRegisterForFinalize(MonoObjectHandle obj,
                                               MonoError *error) {
...
void ves_icall_System_GC_SuppressFinalize(MonoObjectHandle obj,
                                          MonoError *error) {
  MONO_CHECK_ARG_NULL_HANDLE(obj, );
...
void ves_icall_System_GC_WaitForPendingFinalizers(void) {
  ...
  /*
  If the finalizer thread is not live, lets pretend no finalizers are pending
  since the current thread might be the one responsible for starting it up.
  */
  if (gc_thread == NULL)
    return;
  ...
  mono_gc_finalize_notify();
  while (!pending_done) {
    coop_cond_timedwait_alertable(&pending_done_cond, &pending_done_mutex,
                                  MONO_INFINITE_WAIT, &alerted);
    if (alerted)
      break;
  }
...
void ves_icall_System_GC_register_ephemeron_array(MonoObjectHandle array,
                                                  MonoError *error) {
  if (!mono_gc_ephemeron_array_add(MONO_HANDLE_RAW(array)))
    mono_error_set_out_of_memory(error, "");
}

MonoObjectHandle ves_icall_System_GC_get_ephemeron_tombstone(MonoError *error) {
  return MONO_HANDLE_NEW(MonoObject, mono_domain_get()->ephemeron_tombstone);
}

#if ENABLE_NETCORE

MonoGCHandle ves_icall_System_GCHandle_InternalAlloc(MonoObjectHandle obj,
                                                     gint32 type,
                                                     MonoError *error) {
...
void ves_icall_System_GCHandle_InternalFree(MonoGCHandle handle,
                                            MonoError *error) {
  mono_gchandle_free_internal(handle);
}

MonoObjectHandle ves_icall_System_GCHandle_InternalGet(MonoGCHandle handle,
                                                       MonoError *error) {
  return mono_gchandle_get_target_handle(handle);
}

void ves_icall_System_GCHandle_InternalSet(MonoGCHandle handle,
...
MonoObjectHandle ves_icall_System_GCHandle_GetTarget(MonoGCHandle handle,
                                                     MonoError *error) {
  return mono_gchandle_get_target_handle(handle);
}

/*
 * if type == -1, change the target of the handle, otherwise allocate a new
 * handle.
 */
MonoGCHandle ves_icall_System_GCHandle_GetTargetHandle(MonoObjectHandle obj,
                                                       MonoGCHandle handle,
                                                       gint32 type,
                                                       MonoError *error) {
  if (type == -1) {
...
void ves_icall_System_GCHandle_FreeHandle(MonoGCHandle handle) {
  mono_gchandle_free_internal(handle);
}

gpointer ves_icall_System_GCHandle_GetAddrOfPinnedObject(MonoGCHandle handle) {
...
static void hazard_free_queue_is_too_big(size_t size) {
  if (size < HAZARD_QUEUE_OVERFLOW_SIZE)
    return;

  if (finalizer_thread_pulsed ||
      mono_atomic_cas_i32(&finalizer_thread_pulsed, TRUE, FALSE))
    return;

  mono_gc_finalize_notify();
}

static void hazard_free_queue_pump(void) {
...
static void collect_objects(gpointer key, gpointer value, gpointer user_data) {
 ...
 */
static void finalize_domain_objects(void) {
  ...
  while (g_hash_table_size(domain->finalizable_objects_hash) > 0) {
    ...
    g_hash_table_foreach(domain->finalizable_objects_hash, collect_objects,
                         objs);
...
static void mono_runtime_do_background_work(void) {
  ...
  /* If finished == TRUE, mono_gc_cleanup has been called (from
   * mono_runtime_cleanup), before the domain is unloaded.
   */
  mono_gc_invoke_finalizers();
...
static gsize WINAPI finalizer_thread(gpointer unused) {
  gboolean wait = TRUE;

  mono_thread_set_name_constant_ignore_error(
      mono_thread_internal_current(), "Finalizer", MonoSetThreadNameFlag_None);

  /* Register a hazard free queue pump callback */
  mono_hazard_pointer_install_free_queue_size_callback(
      hazard_free_queue_is_too_big);
  ...
  while (!finished) {
     ...
     */
    if (mono_coop_sem_timedwait(&finalizer_sem, 0, MONO_SEM_FLAGS_NONE) ==
...
static void init_finalizer_thread(void) {
  ERROR_DECL(error);
  gc_thread =
      mono_thread_create_internal(mono_domain_get(), (gpointer)finalizer_thread,
...
static void reference_queue_mutex_init(void) {
  mono_coop_mutex_init_recursive(&reference_queue_mutex);
}

void mono_gc_init(void) {
  mono_lazy_initialize(&reference_queue_mutex_inited,
                       reference_queue_mutex_init);
  mono_coop_mutex_init_recursive(&finalizer_mutex);

  mono_counters_register("Minor GC collections",
                         MONO_COUNTER_GC | MONO_COUNTER_INT,
                         &mono_gc_stats.minor_gc_count);
  mono_counters_register("Major GC collections",
                         MONO_COUNTER_GC | MONO_COUNTER_INT,
                         &mono_gc_stats.major_gc_count);
  mono_counters_register(
      "Minor GC time", MONO_COUNTER_GC | MONO_COUNTER_ULONG | MONO_COUNTER_TIME,
      &mono_gc_stats.minor_gc_time);
  mono_counters_register(
      "Major GC time", MONO_COUNTER_GC | MONO_COUNTER_LONG | MONO_COUNTER_TIME,
      &mono_gc_stats.major_gc_time);
  mono_counters_register("Major GC time concurrent",
                         MONO_COUNTER_GC | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_gc_stats.major_gc_time_concurrent);
      ...
      for (;;) {
        ...
        if (finalizer_thread_exited) {
          ...
          mono_threads_add_joinable_thread(
              (gpointer)(gsize)MONO_UINT_TO_NATIVE_THREAD_ID(gc_thread->tid));
          break;
        }
        ...
        if (elapsed >= timeout) {
          ...
          /* Try to abort the thread, in the hope that it is running managed
           * code */
          ...
          mono_threads_add_joinable_thread(
              (gpointer)(MONO_UINT_TO_NATIVE_THREAD_ID(gc_thread->tid)));
          break;
        }

        mono_finalizer_lock();
        if (!finalizer_thread_exited)
          mono_coop_cond_timedwait(&exited_cond, &finalizer_mutex,
                                   timeout - elapsed);
        mono_finalizer_unlock();
      }
    }
...
gboolean mono_gc_is_finalizer_internal_thread(MonoInternalThread *thread) {
...
static void ref_list_remove_element(RefQueueEntry **prev,
                                    RefQueueEntry *element) {
  do {
    /* Guard if head is changed concurrently. */
    while (*prev != element)
      prev = &(*prev)->next;
  } while (prev && mono_atomic_cas_ptr((volatile gpointer *)prev, element->next,
                                       element) != element);
}

static void ref_list_push(RefQueueEntry **head, RefQueueEntry *value) {
  RefQueueEntry *current;
  do {
    current = *head;
    value->next = current;
    STORE_STORE_FENCE; /*Must make sure the previous store is visible before the
                          CAS. */
  } while (mono_atomic_cas_ptr((volatile gpointer *)head, value, current) !=
           current);
}

static void reference_queue_proccess(MonoReferenceQueue *queue) {
  RefQueueEntry **iter = &queue->queue;
  RefQueueEntry *entry;
  while ((entry = *iter)) {
    if (queue->should_be_deleted ||
        !mono_gchandle_get_target_internal(entry->gchandle)) {
...
mono_gc_reference_queue_new(mono_reference_queue_callback callback) {
  MONO_EXTERNAL_ONLY_GC_UNSAFE(MonoReferenceQueue *,
 ...
 */
gboolean mono_gc_reference_queue_add(MonoReferenceQueue *queue, MonoObject *obj,
                                     void *user_data) {
  MONO_EXTERNAL_ONLY_GC_UNSAFE(
      gboolean, mono_gc_reference_queue_add_internal(queue, obj, user_data));
}

gboolean mono_gc_reference_queue_add_internal(MonoReferenceQueue *queue,
                                              MonoObject *obj,
                                              void *user_data) {
  RefQueueEntry *entry;
  if (queue->should_be_deleted)
 ...
 *
 * This operation signals that \p queue should be freed. This operation is
 * deferred as it happens on the finalizer thread.
 *
 * After this call, no further objects can be queued. It's the responsibility of
 * the caller to make sure that no further attempt to access queue will be made.
 */
void mono_gc_reference_queue_free(MonoReferenceQueue *queue) {
  queue->should_be_deleted = TRUE;
}

MonoObjectHandle mono_gc_alloc_handle_pinned_obj(MonoVTable *vtable,
                                                 gsize size) {
...
MonoArrayHandle mono_gc_alloc_handle_vector(MonoVTable *vtable, gsize size,
                                            gsize max_length) {
  return MONO_HANDLE_NEW(MonoArray,
                         mono_gc_alloc_vector(vtable, size, max_length));
}

MonoArrayHandle mono_gc_alloc_handle_array(MonoVTable *vtable, gsize size,
                                           gsize max_length,
                                           gsize bounds_size) {
  return MONO_HANDLE_NEW(
      MonoArray, mono_gc_alloc_array(vtable, size, max_length, bounds_size));
}

MonoStringHandle mono_gc_alloc_handle_string(MonoVTable *vtable, gsize size,
                                             gint32 len) {
...
void mono_gc_wbarrier_object_copy_handle(MonoObjectHandle obj,
  ...
  mono_gc_wbarrier_object_copy_internal(MONO_HANDLE_RAW(obj),
                                        MONO_HANDLE_RAW(src));
}
