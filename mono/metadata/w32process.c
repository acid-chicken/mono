...
static guint32 mono_w32process_get_pid(gpointer handle) {
  return GetProcessId(handle);
}

static gboolean mono_w32process_try_get_modules(gpointer process,
                                                HMODULE *modules, guint32 size,
                                                PDWORD needed) {
  return EnumProcessModules(process, modules, size, needed);
}

static gboolean mono_w32process_module_get_name(gpointer process,
                                                gpointer module,
                                                gunichar2 **str, guint32 *len) {
  return mono_get_module_basename(process, module, str, len);
}

static gboolean mono_w32process_module_get_filename(gpointer process,
                                                    gpointer module,
                                                    gunichar2 **str,
                                                    guint32 *len) {
...
static gboolean mono_w32process_module_get_information(gpointer process,
                                                       gpointer module,
                                                       MODULEINFO *modinfo,
                                                       guint32 size) {
...
static gboolean mono_w32process_get_fileversion_info(const gunichar2 *filename,
                                                     gpointer *data) {
  DWORD handle;
...
static gboolean mono_w32process_ver_query_value(gconstpointer datablock,
...
static guint32 mono_w32process_ver_language_name(guint32 lang,
                                                 gunichar2 *lang_out,
                                                 guint32 lang_len) {
...
static MonoClass *get_file_version_info_class(MonoImage *system_image) {
  g_assert(system_image);

  return mono_class_load_from_name(system_image, "System.Diagnostics",
                                   "FileVersionInfo");
}

static MonoClass *get_process_module_class(MonoImage *system_image) {
  g_assert(system_image);
...
static void process_set_field_ref(MonoObjectHandle obj, const char *fieldname,
  ...
  MonoClassField *field =
      mono_class_get_field_from_name_full(klass, fieldname, NULL);
  g_assert(field);

  MONO_HANDLE_SET_FIELD_REF(obj, field, data);
}

static void process_set_field_object(MonoObjectHandle obj,
                                     const char *fieldname,
                                     MonoObjectHandle data) {
  LOGDEBUG(g_message("%s: Setting field %s to object at %p", __func__,
                     fieldname, data));

  process_set_field_ref(obj, fieldname, data);
}

static void process_set_field_string(MonoObjectHandle obj,
                                     const char *fieldname,
                                     MonoStringHandle string) {
  process_set_field_ref(obj, fieldname, MONO_HANDLE_CAST(MonoObject, string));
}

static void process_set_field_utf16(MonoObjectHandle obj, MonoStringHandle str,
                                    const char *fieldname, const gunichar2 *val,
                                    guint32 len, MonoError *error) {
  HANDLE_FUNCTION_ENTER();

  LOGDEBUG(g_message("%s: Setting field %s to [%s]", __func__, fieldname,
                     g_utf16_to_utf8(val, len, NULL, NULL, NULL)));

  MonoDomain *domain = MONO_HANDLE_DOMAIN(obj);
  g_assert(domain);

  MONO_HANDLE_ASSIGN(str,
                     mono_string_new_utf16_handle(domain, val, len, error));
...
static void process_set_field_utf8(MonoObjectHandle obj, MonoStringHandle str,
                                   const char *fieldname, const char *val,
                                   MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  MONO_HANDLE_ASSIGN(str,
                     mono_string_new_utf8_len(domain, val, strlen(val), error));
  goto_if_nok(error, exit);
...
static MonoClassField *process_resolve_field(MonoObjectHandle obj,
                                             const char *fieldname) {
  // FIXME Cache offsets. Per-class.
  MonoClass *klass = mono_handle_class(obj);
  g_assert(klass);
...
static void process_set_field_int(MonoObjectHandle obj, const char *fieldname,
                                  guint32 val) {
  LOGDEBUG(g_message("%s: Setting field %s to %d", __func__, fieldname, val));

  MONO_HANDLE_SET_FIELD_VAL(obj, guint32, process_resolve_field(obj, fieldname),
                            val);
}

static void process_set_field_intptr(MonoObjectHandle obj,
                                     const char *fieldname, gpointer val) {
  LOGDEBUG(g_message("%s: Setting field %s to %p", __func__, fieldname, val));

  MONO_HANDLE_SET_FIELD_VAL(obj, gpointer,
                            process_resolve_field(obj, fieldname), val);
}

static void process_set_field_bool(MonoObjectHandle obj, const char *fieldname,
                                   guint8 val) {
  LOGDEBUG(g_message("%s: Setting field %s to %s", __func__, fieldname,
                     val ? "TRUE" : "FALSE"));

  MONO_HANDLE_SET_FIELD_VAL(obj, guint8, process_resolve_field(obj, fieldname),
                            val);
}
...
#define SFI_LEGALTRADEMARKS                                                    \
  "\\StringFileInfo\\%02X%02X%02X%02X\\LegalTrademarks"
#define SFI_ORIGINALFILENAME                                                   \
...
static void process_module_string_read(MonoObjectHandle filever,
                                       MonoStringHandle str, gpointer data,
                                       const char *fieldname, guchar lang_hi,
                                       guchar lang_lo, const gchar *key,
                                       MonoError *error) {
  ...
  if (mono_w32process_ver_query_value(data, lang_key, (gpointer *)&buffer,
                                      &chars) &&
      chars > 0) {
    LOGDEBUG(g_message("%s: found %d chars of [%s]", __func__, chars,
...
static void process_module_stringtable(MonoObjectHandle filever,
                                       MonoStringHandle str, gpointer data,
  ...
  for (int i = 0; is_ok(error) && i < G_N_ELEMENTS(stringtable_entries); ++i) {
    process_module_string_read(filever, str, data, stringtable_entries[i].name,
                               lang_hi, lang_lo, stringtable_entries[i].id,
                               error);
  }
}

static void mono_w32process_get_fileversion(MonoObjectHandle filever,
  ...
  if (mono_w32process_ver_query_value(data, query, (gpointer *)&ffi,
                                      &ffi_size)) {
#define LOWORD(i32) ((guint16)((i32)&0xFFFF))
#define HIWORD(i32) ((guint16)(((guint32)(i32) >> 16) & 0xFFFF))

    LOGDEBUG(g_message(
        "%s: recording assembly: FileName [%s] FileVersionInfo [%d.%d.%d.%d]",
        __func__, g_utf16_to_utf8(filename, -1, NULL, NULL, NULL),
    ...
    process_set_field_bool(
        filever, "isdebug",
        ((ffi->dwFileFlags & ffi->dwFileFlagsMask) & VS_FF_DEBUG) != 0);
    process_set_field_bool(
        filever, "isprerelease",
        ((ffi->dwFileFlags & ffi->dwFileFlagsMask) & VS_FF_PRERELEASE) != 0);
    process_set_field_bool(
        filever, "ispatched",
        ((ffi->dwFileFlags & ffi->dwFileFlagsMask) & VS_FF_PATCHED) != 0);
    process_set_field_bool(
        filever, "isprivatebuild",
        ((ffi->dwFileFlags & ffi->dwFileFlagsMask) & VS_FF_PRIVATEBUILD) != 0);
    process_set_field_bool(
        filever, "isspecialbuild",
        ((ffi->dwFileFlags & ffi->dwFileFlagsMask) & VS_FF_SPECIALBUILD) != 0);
...
#undef HIWORD
  ...
  if (mono_w32process_ver_query_value(data, query, (gpointer *)&trans_data,
                                      &trans_size)) {
    /* use the first language ID we see */
    if (trans_size >= 4) {
      LOGDEBUG(g_message("%s: %s has 0x%0x 0x%0x 0x%0x 0x%0x", __func__,
                         g_utf16_to_utf8(filename, -1, NULL, NULL, NULL),
                         trans_data[0], trans_data[1], trans_data[2],
                         trans_data[3]));
      lang = (trans_data[0]) | (trans_data[1] << 8) | (trans_data[2] << 16) |
             (trans_data[3] << 24);
      /* Only give the lower 16 bits to mono_w32process_ver_language_name, as
       * Windows gets confused otherwise  */
      lang_count =
          mono_w32process_ver_language_name(lang & 0xFFFF, lang_buf, 128);
      if (lang_count) {
        process_set_field_utf16(filever, str, "language", lang_buf, lang_count,
                                error);
        goto_if_nok(error, cleanup);
      }
      process_module_stringtable(filever, str, data, trans_data[0],
                                 trans_data[1], error);
      goto_if_nok(error, cleanup);
    }
  } else {
    ...
    for (i = 0; i < G_N_ELEMENTS(stringtable_entries); ++i) {
      ...
      process_set_field_utf16(filever, str, stringtable_entries[i].name,
...
void ves_icall_System_Diagnostics_FileVersionInfo_GetVersionInfo_internal(
    MonoObjectHandle this_obj, const gunichar2 *filename, int filename_length,
    MonoError *error) {
  ...
  process_set_field_utf16(this_obj, str, "filename", filename, filename_length,
                          error);
}
#endif
...
static GPtrArray *get_domain_assemblies(MonoDomain *domain) {
  ...
  for (tmp = domain->domain_assemblies; tmp; tmp = tmp->next) {
    ...
    // TODO: the reasoning behind this check being used is unclear to me. Maybe
    // replace with mono_domain_get_assemblies()? Added in
    // https://github.com/mono/mono/commit/46dc6758c67528fa7bc5590d1cfb4c119b69bc2b#diff-7017648b60461e8902a36d22782f4a14R451
...
static void process_add_module(MonoObjectHandle item, MonoObjectHandle filever,
                               MonoStringHandle str, HANDLE process,
                               HMODULE mod, const gunichar2 *filename,
                               const gunichar2 *modulename,
  ...
  MONO_HANDLE_ASSIGN(
      ...
      mono_object_new_handle(
          domain, get_file_version_info_class(m_class_get_image(proc_class)),
          error));
  goto_if_nok(error, exit);
  ...
  process_set_field_utf16(filever, str, "filename", filename,
                          g_utf16_len(filename), error);
  goto_if_nok(error, exit);

  if (mono_w32process_module_get_information(process, mod, &modinfo,
                                             sizeof(MODULEINFO))) {
  ...
  process_set_field_utf16(item, str, "filename", filename,
                          g_utf16_len(filename), error);
  goto_if_nok(error, exit);

  process_set_field_utf16(item, str, "modulename", modulename,
                          g_utf16_len(modulename), error);
  goto_if_nok(error, exit);
...
static void process_get_assembly_fileversion(MonoObjectHandle filever,
...
static void process_get_module(MonoObjectHandle item, MonoObjectHandle filever,
                               MonoStringHandle str, MonoAssembly *assembly,
                               MonoClass *proc_class, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
...
MonoArrayHandle ves_icall_System_Diagnostics_Process_GetModules_internal(
    MonoObjectHandle this_obj, HANDLE process, MonoError *error) {
  ...
  process_module_class =
      get_process_module_class(m_class_get_image(mono_handle_class(this_obj)));
  ...
  temp_arr = mono_array_new_handle(mono_domain_get(), process_module_class,
                                   count, error);
  return_val_if_nok(error, NULL_HANDLE_ARRAY);

  for (i = 0; i < module_count; i++) {
    if (mono_w32process_module_get_name(process, mods[i], &modname,
                                        &modname_len) &&
        mono_w32process_module_get_filename(process, mods[i], &filename,
                                            &filename_len)) {
      process_add_module(module, filever, str, process, mods[i], filename,
                         modname, process_module_class, error);
      if (is_ok(error))
  ...
  if (assemblies) {
    for (i = 0; i < assembly_count; i++) {
      ...
      process_get_module(module, filever, str, ass, process_module_class,
                         error);
      return_val_if_nok(error, NULL_HANDLE_ARRAY);
      MONO_HANDLE_ARRAY_SETREF(temp_arr, num_added++, module);
    }
  ...
  arr = mono_array_new_handle(mono_domain_get(), process_module_class,
                              num_added, error);
  return_val_if_nok(error, NULL_HANDLE_ARRAY);
...
ves_icall_System_Diagnostics_Process_ProcessName_internal(HANDLE process,
  ...
  MonoStringHandle res =
      mono_string_new_utf16_handle(mono_domain_get(), name, len, error);
  g_free(name);
  return res;
}
...
gint64
ves_icall_System_Diagnostics_Process_GetProcessData(int pid, gint32 data_type,
                                                    MonoProcessError *error) {
  g_static_assert(sizeof(MonoProcessError) == sizeof(gint32));
  g_assert(error);
  *error = MONO_PROCESS_ERROR_NONE;
  return mono_process_get_data_with_error(GINT_TO_POINTER(pid),
                                          (MonoProcessData)data_type, error);
}

#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT)

static void mono_pin_string(MonoStringHandle in_coophandle,
                            MonoStringHandle *out_coophandle, gunichar2 **chars,
                            gsize *length, MonoGCHandle *gchandle) {
...
void mono_createprocess_coop_init(MonoCreateProcessCoop *coop,
                                  MonoW32ProcessStartInfoHandle proc_start_info,
                                  MonoW32ProcessInfo *process_info) {
  memset(coop, 0, sizeof(*coop));

#define PIN_STRING(h, x)                                                       \
  (mono_pin_string(h, &coop->coophandle.x, &coop->x, &coop->length.x,          \
                   &coop->gchandle.x))

#define PIN(x)                                                                 \
  PIN_STRING(MONO_HANDLE_NEW_GET(MonoString, proc_start_info, x), x)
  PIN(filename);
  PIN(arguments);
...
static void mono_unpin_array(MonoGCHandle *gchandles, gsize count) {
...
void mono_createprocess_coop_cleanup(MonoCreateProcessCoop *coop) {
  mono_unpin_array((MonoGCHandle *)&coop->gchandle,
                   sizeof(coop->gchandle) / sizeof(MonoGCHandle));
  memset(coop, 0, sizeof(*coop));
}
