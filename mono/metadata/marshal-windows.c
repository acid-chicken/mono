...
void *mono_marshal_alloc_hglobal(size_t size, MonoError *error) {
...
void mono_marshal_free_hglobal(gpointer ptr) { GlobalFree(ptr); }
...
void mono_marshal_free_co_task_mem(void *ptr) { CoTaskMemFree(ptr); }
...
char *ves_icall_System_Runtime_InteropServices_Marshal_StringToHGlobalAnsi(
   ...
   * even if it contains NULL characters. The copy we allocate here has to be
   * equally large.
   */
  size_t len = MAX(strlen(tres) + 1, length);
  char *ret =
      (char *)ves_icall_System_Runtime_InteropServices_Marshal_AllocHGlobal(
          len, error);
  if (ret)
    memcpy(ret, tres, len);
  g_free(tres);
...
gpointer mono_string_to_utf8str_impl(MonoStringHandle s, MonoError *error) {
  ...
  uint32_t gchandle = 0;
  tmp = g_utf16_to_utf8(mono_string_handle_pin_chars(s, &gchandle),
                        mono_string_handle_length(s), NULL, &len, &gerror);
