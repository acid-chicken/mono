...
#define MONO_CLASS_HAS_STATIC_METADATA(klass)                                  \
  (m_class_get_type_token(klass) && !m_class_get_image(klass)->dynamic &&      \
...
/* Properties that applies to a group of structs should better use a higher
 * number to avoid colision with type specific properties.
 *
...
struct _MonoMethod {
  ...
  unsigned int
      is_generic : 1; /* whenever this is a generic method definition */
  unsigned int is_inflated : 1;     /* whether we're a MonoMethodInflated */
  unsigned int skip_visibility : 1; /* whenever to skip JIT visibility checks */
  unsigned int verification_success : 1; /* whether this method has been
                                            verified successfully.*/
  unsigned int is_reabstracted : 1;      /* whenever this is a reabstraction of
                                            another interface */
  signed int slot : 16;

  /*
   * If is_generic is TRUE, the generic_container is stored in
   * image->property_hash, using the key MONO_METHOD_PROP_GENERIC_CONTAINER.
   */
};
...
/* a field is ignored if it's named "_Deleted" and it has the specialname and
 * rtspecialname flags set */
#define mono_field_is_deleted(field)                                           \
  (((field)->type->attrs &                                                     \
    (FIELD_ATTRIBUTE_SPECIAL_NAME | FIELD_ATTRIBUTE_RT_SPECIAL_NAME)) &&       \
...
#define MONO_SIZEOF_CLASS_RUNTIME_INFO                                         \
  (sizeof(MonoClassRuntimeInfo) - MONO_ZERO_LEN_ARRAY * SIZEOF_VOID_P)
...
/* enabled only with small config for now: we might want to do it
 * unconditionally */
...
#define MONO_CLASS_GETTER(funcname, rettype, optref, argtype, fieldname)       \
  rettype funcname(argtype *klass);
#else
#define MONO_CLASS_GETTER(funcname, rettype, optref, argtype, fieldname)       \
...
#else
#define mono_class_interface_match(bmap, uiid)                                 \
  ((bmap)[(uiid) >> 3] & (1 << ((uiid)&7)))
#endif

#define MONO_CLASS_IMPLEMENTS_INTERFACE(k, uiid)                               \
  (((uiid) <= m_class_get_max_interface_id(k)) &&                              \
   mono_class_interface_match(m_class_get_interface_bitmap(k), (uiid)))
...
struct MonoVTable {
  ...
  guint has_static_fields : 1; /* pointer to the data stored at the end of the
                                  vtable array */
  guint gc_bits : MONO_VTABLE_AVAILABLE_GC_BITS; /* Those bits are reserved for
                                                    the usaged of the GC */
...
#define MONO_SIZEOF_VTABLE                                                     \
  (sizeof(MonoVTable) - MONO_ZERO_LEN_ARRAY * SIZEOF_VOID_P)

#define MONO_VTABLE_IMPLEMENTS_INTERFACE(vt, uiid)                             \
  (((uiid) <= (vt)->max_interface_id) &&                                       \
   mono_class_interface_match((vt)->interface_bitmap, (uiid)))
 ...
 *
 * All instantiations are cached and we don't distinguish between class and
 * method instantiations here.
 */
...
#define MONO_SIZEOF_GENERIC_INST                                               \
  (sizeof(MonoGenericInst) - MONO_ZERO_LEN_ARRAY * SIZEOF_VOID_P)
/*
 * The generic context: an instantiation of a set of class and method generic
 *parameters.
 *
 * NOTE: Never allocate this directly on the heap.  It have to be either
 *allocated on the stack, or embedded within other objects.  Don't store
 *pointers to this, because it may be on the stack. If you really have to,
 *ensure you store a pointer to the embedding object along with it.
 */
...
struct _MonoGenericClass {
  ...
  guint is_tb_open : 1;    /* This is the fully open instantiation for a
                              type_builder. Quite ugly, but it's temporary.*/
  guint need_sync : 1;     /* Only if dynamic. Need to be synchronized with its
                              container class after its finished. */
  MonoClass *cached_class; /* if present, the MonoClass corresponding to the
                              instantiation.  */

  /*
   * The image set which owns this generic class. Memory owned by the generic
   * class including cached_class should be allocated from the mempool of the
   * image set, so it is easy to free.
   */
  MonoImageSet *owner;
};
...
struct _MonoGenericParam {
  ...
  guint16 num;
  /*
   * If != NULL, this is a generated generic param used by the JIT to implement
   * generic sharing.
   */
 ...
 *
 * Stores the type parameters of a generic type definition or a generic method
 * definition.
 */
struct _MonoGenericContainer {
  ...
  /* the generic type definition or the generic method definition corresponding
   * to this container */
  /* Union rules: If is_anonymous, image field is valid; else if is_method,
   * method field is valid; else klass is valid. */
  union {
    MonoClass *klass;
    MonoMethod *method;
    MonoImage *image;
  ...
  int is_method : 1;
  /* If true, this container has no associated class/method and only the image
     is known. This can happen:
     1. For the special anonymous containers kept by MonoImage.
     2. When user code creates a generic parameter via SRE, but has not yet set
     an owner. */
  int is_anonymous : 1;
  /* Our type parameters. If this is a special anonymous container (case 1,
   * above), this field is not valid, use mono_metadata_create_anon_gparam () */
...
static inline int mono_generic_param_num(MonoGenericParam *p) { return p->num; }
 ...
 */
static inline gboolean mono_class_has_parent_fast(MonoClass *klass,
                                                  MonoClass *parent) {
  return (m_class_get_idepth(klass) >= m_class_get_idepth(parent)) &&
         (m_class_get_supertypes(klass)[m_class_get_idepth(parent) - 1] ==
          parent);
}

static inline gboolean mono_class_has_parent(MonoClass *klass,
                                             MonoClass *parent) {
...
#define MONO_SIZEOF_REMOTE_CLASS                                               \
  (sizeof(MonoRemoteClass) - MONO_ZERO_LEN_ARRAY * SIZEOF_VOID_P)
...
typedef gboolean (*MonoGetCachedClassInfo)(MonoClass *klass,
                                           MonoCachedClassInfo *res);

typedef gboolean (*MonoGetClassFromName)(MonoImage *image,
                                         const char *name_space,
                                         const char *name, MonoClass **res);
...
typedef struct {
  ...
  MonoClass *object_class_array; // used via token pasting in
  ...
  MonoClass *runtimetype_class_array; // used via token pasting in
                                      // mono_array_class_get_cached
  ...
  MonoClass *attribute_class_array;     // used via token pasting in
                                        // mono_array_class_get_cached
...
#else
#define mono_class_is_transparent_proxy(klass)                                 \
  ((klass) == mono_defaults.transparent_proxy_class)
#define mono_class_is_real_proxy(klass)                                        \
  ((klass) == mono_defaults.real_proxy_class)
#endif

#define mono_object_is_transparent_proxy(object)                               \
  (mono_class_is_transparent_proxy(mono_object_class(object)))
...
// its cache is null. i.e. potentially repeatedly, though it is expected to
...
#define GENERATE_GET_CLASS_WITH_CACHE(shortname, name_space, name)             \
  MonoClass *mono_class_get_##shortname##_class(void) {                        \
    ...
    if (!klass) {                                                              \
      klass =                                                                  \
          mono_class_load_from_name(mono_defaults.corlib, name_space, name);   \
...
// GENERATE_TRY_GET_CLASS_WITH_CACHE attempts mono_class_load_from_name
// approximately only once. i.e. if it fails, it will return null and not retry.
...
#define GENERATE_TRY_GET_CLASS_WITH_CACHE(shortname, name_space, name)         \
  MonoClass *mono_class_try_get_##shortname##_class(void) {                    \
    ...
    mono_memory_barrier();                                                     \
    if (!inited) {                                                             \
      klass = mono_class_try_load_from_name(mono_defaults.corlib, name_space,  \
                                            name);                             \
      tmp_class = klass;                                                       \
      mono_memory_barrier();                                                   \
      inited = TRUE;                                                           \
...
/* If you need a MonoType, use one of the mono_get_*_type () functions in
 * class-inlines.h */
...
MONO_API gboolean mono_metadata_load_generic_param_constraints_checked(
    MonoImage *image, guint32 token, MonoGenericContainer *container,
    MonoError *error);

// This is the "real" function for registering JIT icalls. All others are one
// line wrappers that call it, i.e. filling in info or c_symbol.
...
template <typename T>
inline void
mono_register_jit_icall_info(MonoJitICallInfo *info, T func, const char *name,
                             MonoMethodSignature *sig, gboolean no_wrapper,
                             const char *c_symbol) {
  mono_register_jit_icall_info(info, (gconstpointer)func, name, sig, no_wrapper,
                               c_symbol);
}
#endif // __cplusplus
...
char *mono_method_get_name_full(MonoMethod *method, gboolean signature,
                                gboolean ret, MonoTypeNameFormat format);
...
MonoMethodSignature *mono_metadata_signature_deep_dup(MonoImage *image,
                                                      MonoMethodSignature *sig);
...
gboolean mono_class_can_access_class(MonoClass *access_class,
...
#define mono_class_alloc0(klass, size)                                         \
  (g_cast(mono_class_alloc0((klass), (size))))
...
mono_bool mono_class_is_assignable_from_internal(MonoClass *klass,
                                                 MonoClass *oklass);
...
MonoClassField *mono_field_from_token_checked(MonoImage *image, uint32_t token,
                                              MonoClass **retklass,
                                              MonoGenericContext *context,
                                              MonoError *error);
...
void mono_error_set_for_class_failure(MonoError *orerror,
                                      const MonoClass *klass);

gboolean mono_class_has_failure(const MonoClass *klass);

/* Kind specific accessors */
MonoGenericClass *mono_class_get_generic_class(MonoClass *klass);
...
void mono_class_set_generic_container(MonoClass *klass,
                                      MonoGenericContainer *container);
...
void mono_class_set_marshal_info(MonoClass *klass,
                                 MonoMarshalType *marshal_info);

MonoGCHandle mono_class_get_ref_info_handle(MonoClass *klass);

MonoGCHandle mono_class_set_ref_info_handle(MonoClass *klass, gpointer value);
...
void mono_class_set_property_info(MonoClass *klass,
...
void mono_class_set_field_def_values(MonoClass *klass,
                                     MonoFieldDefaultValue *values);
...
//
// And then mark mono_method_get_header_checked as MONO_RT_EXTERNAL_ONLY
// MONO_API.
//
...
gboolean mono_class_publish_gc_descriptor(MonoClass *klass,
                                          MonoGCDescriptor gc_descr);
...
//
// These macros cannot be wrapped in do/while as they inject "name" into
// invoking scope.
//
