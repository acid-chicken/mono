...
/*
 * If this is set, the memory belonging to appdomains is not freed when a domain
 * is unloaded, and assemblies loaded by the appdomain are not unloaded either.
 * This allows us to use typed gc in non-default appdomains too, leading to
...
typedef struct {
  ...
  MonoObject *activation_arguments; /* it is System.Object in 1.x,
                                       ActivationArguments in 2.0 */
  MonoObject *domain_initializer;
  MonoObject *application_trust; /* it is System.Object in 1.x, ApplicationTrust
                                    in 2.0 */
...
#define MONO_SIZEOF_JIT_INFO_TABLE                                             \
  (sizeof(struct _MonoJitInfoTable) - MONO_ZERO_LEN_ARRAY * SIZEOF_VOID_P)
...
The instructions between the call and the jump should not be under the try block
since they happen after the finally block executes, which means if an async
exceptions happens at that point we would execute the finally clause twice. So,
to avoid this, we introduce a hole in the try block to signal that those
instructions are not protected.
*/
typedef struct {
  guint32 offset;
  guint16 clause;
...
typedef enum {
  ...
  /*
   * If this is set, the unwind info is stored in the structure, instead of
   * being pointed to by the 'unwind_info' field.
   */
...
struct _MonoJitInfo {
  ...
  /* There is an optional MonoTryBlockHoleTableJitInfo after MonoGenericJitInfo
   * clauses*/
...
typedef struct {
  gpointer *static_data; /* Used to free the static data without going through
                            the MonoAppContext object itself. */
...
#define MONO_DOMAIN_LAST_GC_TRACKED delegate_hash_table
  ...
  /* Contains the compiled runtime invoke wrapper used by async resylt creation
   * to capture thread context*/
  gpointer capture_context_runtime_invoke;

  /* Contains the compiled method used by async resylt creation to capture
   * thread context*/
...
/* MonoRuntimeInfo: Contains information about versions supported by this
 * runtime */
typedef struct {
...
#define mono_domain_assemblies_lock(domain)                                    \
  mono_locks_os_acquire(&(domain)->assemblies_lock, DomainAssembliesLock)
#define mono_domain_assemblies_unlock(domain)                                  \
  mono_locks_os_release(&(domain)->assemblies_lock, DomainAssembliesLock)
#define mono_domain_jit_code_hash_lock(domain)                                 \
  mono_locks_os_acquire(&(domain)->jit_code_hash_lock, DomainJitCodeHashLock)
#define mono_domain_jit_code_hash_unlock(domain)                               \
  mono_locks_os_release(&(domain)->jit_code_hash_lock, DomainJitCodeHashLock)

typedef MonoDomain *(*MonoLoadFunc)(const char *filename,
...
void mono_jit_info_init(MonoJitInfo *ji, MonoMethod *method, guint8 *code,
                        int code_size, MonoJitInfoFlags flags, int num_clauses,
                        int num_holes);
...
void mono_jit_info_add_aot_module(MonoImage *image, gpointer start,
                                  gpointer end);
...
void mono_jit_info_set_generic_sharing_context(
    MonoJitInfo *ji, MonoGenericSharingContext *gsctx);
...
#define mono_domain_alloc(domain, size)                                        \
  (g_cast(mono_domain_alloc((domain), (size))))

gpointer mono_domain_alloc0(MonoDomain *domain, guint size);

#define mono_domain_alloc0(domain, size)                                       \
  (g_cast(mono_domain_alloc0((domain), (size))))

gpointer mono_domain_alloc0_lock_free(MonoDomain *domain, guint size);

#define mono_domain_alloc0_lock_free(domain, size)                             \
  (g_cast(mono_domain_alloc0_lock_free((domain), (size))))

void *mono_domain_code_reserve(MonoDomain *domain, int size);

#define mono_domain_code_reserve(domain, size)                                 \
  (g_cast(mono_domain_code_reserve((domain), (size))))

void *mono_domain_code_reserve_align(MonoDomain *domain, int size,
                                     int alignment);

#define mono_domain_code_reserve_align(domain, size, align)                    \
  (g_cast(mono_domain_code_reserve_align((domain), (size), (align))))
 ...
 * inside an AOT module.
 */
typedef MonoJitInfo *(*MonoJitInfoFindInAot)(MonoDomain *domain,
                                             MonoImage *image, gpointer addr);
...
MonoJitInfo *mono_jit_info_table_find_internal(MonoDomain *domain,
                                               gpointer addr, gboolean try_aot,
