...
static const int simple_type_to_type[] = {NumberOfItemsHEX32,
                                          NumberOfItemsHEX64,
                                          NumberOfItems32,
                                          NumberOfItems64,
                                          CounterDelta32,
...
enum { SingleInstance, MultiInstance, CatTypeUnknown = -1 };
...
/*
  binary format of custom counters in shared memory, starting from
MonoSharedArea* + data_start; basic stanza: struct stanza_header { byte
stanza_type; // FTYPE_* byte other_info; ushort stanza_length; // includeas
header
        ... data ...
  }
...
#undef PERFCTR_COUNTER
#define PERFCTR_CAT(id, name, help, type, inst, first_counter)                 \
  {name,          help, sizeof(name) - 1, sizeof(help) - 1,                    \
   CATEGORY_##id, type, inst##Instance,   CCOUNTER_##first_counter},
#define PERFCTR_COUNTER(id, name, help, type, field)
static const CategoryDesc predef_categories[] = {
#include "mono-perfcounters-def.h"
...
#define PERFCTR_COUNTER(id, name, help, type, field)                           \
  {name,                                                                       \
   ...
   sizeof(help) - 1,                                                           \
   COUNTER_##id,                                                               \
   G_STRUCT_OFFSET(MonoPerfCounters, field),                                   \
   type},
static const CounterDesc predef_counters[] = {
#include "mono-perfcounters-def.h"
    {NULL, NULL, 0, 0, -1, 0, 0}};
 ...
 *
 * To easily handle the differences we create a vtable for each class that
 * contains the function pointers with the actual implementation to access the
 * counters.
 */
typedef struct _ImplVtable ImplVtable;

typedef MonoBoolean (*SampleFunc)(ImplVtable *vtable, MonoBoolean only_value,
                                  MonoCounterSample *sample);
typedef gint64 (*UpdateFunc)(ImplVtable *vtable, MonoBoolean do_incr,
                             gint64 value);
...
static ImplVtable *create_vtable(void *arg, SampleFunc sample,
                                 UpdateFunc update) {
...
static MonoSharedArea *load_sarea_for_pid(int pid) {
  ExternalSArea *data;
  ...
  data = (ExternalSArea *)g_hash_table_lookup(pid_to_shared_area,
                                              GINT_TO_POINTER(pid));
  if (!data) {
...
static void unref_pid_unlocked(int pid) {
  ExternalSArea *data;
  data = (ExternalSArea *)g_hash_table_lookup(pid_to_shared_area,
                                              GINT_TO_POINTER(pid));
  if (data) {
...
static void predef_cleanup(ImplVtable *vtable) {
...
static int perfctr_type_compress(int type) {
  int i;
...
static SharedHeader *shared_data_reserve_room(int size, int ftype) {
  SharedHeader *header;
...
static void foreach_shared_item_in_area(unsigned char *p, unsigned char *end,
                                        SharedFunc func, void *data) {
  while (p < end) {
    unsigned short *next;
    if (p + 4 > end)
      return;
...
static void foreach_shared_item(SharedFunc func, void *data) {
  unsigned char *p = (unsigned char *)shared_area + shared_area->data_start;
  unsigned char *end = (unsigned char *)shared_area + shared_area->size;
...
static gboolean mono_utf16_equal_ascii(const gunichar2 *strc,
...
static gboolean mono_utf16_equal_ascii_len(const gunichar2 *utf16,
                                           int utf16_length, const char *ascii,
                                           int ascii_length) {
  /* FIXME: make this case insensitive */
...
static gboolean category_search(SharedHeader *header, void *data) {
...
static SharedCategory *find_custom_category(const gunichar2 *name) {
  CatSearch search;
  search.name = name;
  search.cat = NULL;
...
static gboolean category_collect(SharedHeader *header, void *data) {
...
static GSList *get_custom_categories(void) {
  GSList *list = NULL;
  foreach_shared_item(category_collect, &list);
  return list;
}

static SharedCounter *custom_category_counters(SharedCategory *cat) {
  char *help = cat->name + strlen(cat->name) + 1;
  return (SharedCounter *)(help + strlen(help) + 1);
}

static SharedCounter *next_custom_category_counter(SharedCounter *counter) {
  char *help = counter->name + strlen(counter->name) + 1;
  return (SharedCounter *)(help + strlen(help) + 1);
}

static SharedCounter *find_custom_counter(SharedCategory *cat,
                                          const gunichar2 *name) {
  int i;
...
static gboolean instance_search(SharedHeader *header, void *data) {
...
static SharedInstance *find_custom_instance(SharedCategory *cat, char *name) {
...
static GSList *get_custom_instances_list(SharedCategory *cat) {
  InstanceSearch search;
  search.cat_offset = (char *)cat - (char *)shared_area;
  search.cat = cat;
  search.name = NULL;
...
static char *custom_category_help(SharedCategory *cat) {
  return cat->name + strlen(cat->name) + 1;
}

static const CounterDesc *get_counter_in_category(const CategoryDesc *desc,
  ...
  for (; cdesc < end; ++cdesc) {
    if (mono_utf16_equal_ascii_len(counter, counter_length, cdesc->name,
                                   cdesc->name_length))
      return cdesc;
  }
  return NULL;
...
static void fill_sample(MonoCounterSample *sample) {
...
static int id_from_string(const gchar *id_str, gboolean is_process) {
  int id = -1;
  if (strcmp("", id_str) != 0) {
    char *end;
...
static MonoBoolean get_cpu_counter(ImplVtable *vtable, MonoBoolean only_value,
                                   MonoCounterSample *sample) {
  ...
  sample->counterType =
      predef_counters[predef_categories[CATEGORY_CPU].first_counter + id].type;
  switch (id) {
...
static void *cpu_get_impl(const gunichar2 *counter, int counter_length,
                          const char *instance, int *type,
                          MonoBoolean *custom) {
  ...
  *custom = FALSE;
  ...
  if ((cdesc = get_counter_in_category(&predef_categories[CATEGORY_CPU],
                                       counter, counter_length))) {
    *type = cdesc->type;
    return create_vtable(GINT_TO_POINTER(id | cdesc->id), get_cpu_counter,
                         NULL);
  }
  return NULL;
}

static MonoBoolean get_network_counter(ImplVtable *vtable,
  ...
  sample->counterType =
      predef_counters[predef_categories[CATEGORY_NETWORK].first_counter +
                      narg->id]
          .type;
  switch (narg->id) {
  case COUNTER_NETWORK_BYTESRECSEC:
    sample->rawValue =
        mono_network_get_data(narg->name, MONO_NETWORK_BYTESREC, &error);
    break;
  case COUNTER_NETWORK_BYTESSENTSEC:
    sample->rawValue =
        mono_network_get_data(narg->name, MONO_NETWORK_BYTESSENT, &error);
    break;
  case COUNTER_NETWORK_BYTESTOTALSEC:
    sample->rawValue =
        mono_network_get_data(narg->name, MONO_NETWORK_BYTESTOTAL, &error);
    break;
  }
...
static void network_cleanup(ImplVtable *vtable) {
...
static void *network_get_impl(const gunichar2 *counter, int counter_length,
                              const char *instance, int *type,
                              MonoBoolean *custom) {
  const CounterDesc *cdesc;
  NetworkVtableArg *narg;
  ...
  *custom = FALSE;
  if ((cdesc = get_counter_in_category(&predef_categories[CATEGORY_NETWORK],
                                       counter, counter_length))) {
...
static MonoBoolean get_process_counter(ImplVtable *vtable,
                                       MonoBoolean only_value,
                                       MonoCounterSample *sample) {
  int id = GPOINTER_TO_INT(vtable->arg);
  int pid = id >> 5;
  ...
  sample->counterType =
      predef_counters[predef_categories[CATEGORY_PROC].first_counter + id].type;
  switch (id) {
  case COUNTER_PROC_USER_TIME:
    sample->rawValue =
        mono_process_get_data(GINT_TO_POINTER(pid), MONO_PROCESS_USER_TIME);
    return TRUE;
  case COUNTER_PROC_PRIV_TIME:
    sample->rawValue =
        mono_process_get_data(GINT_TO_POINTER(pid), MONO_PROCESS_SYSTEM_TIME);
    return TRUE;
  case COUNTER_PROC_PROC_TIME:
    sample->rawValue =
        mono_process_get_data(GINT_TO_POINTER(pid), MONO_PROCESS_TOTAL_TIME);
    return TRUE;
  case COUNTER_PROC_THREADS:
    sample->rawValue =
        mono_process_get_data(GINT_TO_POINTER(pid), MONO_PROCESS_NUM_THREADS);
    return TRUE;
  case COUNTER_PROC_VBYTES:
    sample->rawValue =
        mono_process_get_data(GINT_TO_POINTER(pid), MONO_PROCESS_VIRTUAL_BYTES);
    return TRUE;
  case COUNTER_PROC_WSET:
    sample->rawValue =
        mono_process_get_data(GINT_TO_POINTER(pid), MONO_PROCESS_WORKING_SET);
    return TRUE;
  case COUNTER_PROC_PBYTES:
    sample->rawValue =
        mono_process_get_data(GINT_TO_POINTER(pid), MONO_PROCESS_PRIVATE_BYTES);
    return TRUE;
  }
  return FALSE;
}

static void *process_get_impl(const gunichar2 *counter, int counter_length,
                              const char *instance, int *type,
                              MonoBoolean *custom) {
  ...
  *custom = FALSE;
   ...
   */
  // g_assert (32 > desc [1].first_counter - desc->first_counter);
  if ((cdesc = get_counter_in_category(&predef_categories[CATEGORY_PROC],
                                       counter, counter_length))) {
    *type = cdesc->type;
    return create_vtable(GINT_TO_POINTER(id | cdesc->id), get_process_counter,
                         NULL);
  }
  return NULL;
}

static MonoBoolean mono_mem_counter(ImplVtable *vtable, MonoBoolean only_value,
                                    MonoCounterSample *sample) {
  int id = GPOINTER_TO_INT(vtable->arg);
  if (!only_value) {
    fill_sample(sample);
  ...
  sample->counterType =
      predef_counters[predef_categories[CATEGORY_MONO_MEM].first_counter + id]
          .type;
  switch (id) {
...
static void *mono_mem_get_impl(const gunichar2 *counter, int counter_length,
                               const char *instance, int *type,
                               MonoBoolean *custom) {
  const CounterDesc *cdesc;
  *custom = FALSE;
  if ((cdesc = get_counter_in_category(&predef_categories[CATEGORY_MONO_MEM],
                                       counter, counter_length))) {
    *type = cdesc->type;
    return create_vtable(GINT_TO_POINTER((gint)cdesc->id), mono_mem_counter,
                         NULL);
  }
  return NULL;
}

static MonoBoolean predef_readonly_counter(ImplVtable *vtable,
                                           MonoBoolean only_value,
                                           MonoCounterSample *sample) {
  PredefVtable *vt = (PredefVtable *)vtable;
...
static ImplVtable *predef_vtable(void *arg, const gchar *pids) {
  MonoSharedArea *area;
  PredefVtable *vtable;
  int pid;
...
/* consider storing the pointer directly in vtable->arg, so the runtime overhead
 * is lower: this needs some way to set sample->counterType as well, though.
 */
static MonoBoolean predef_writable_counter(ImplVtable *vtable,
                                           MonoBoolean only_value,
                                           MonoCounterSample *sample) {
  int cat_id = GPOINTER_TO_INT(vtable->arg);
  int id = cat_id >> 16;
  ...
  sample->counterType =
      predef_counters[predef_categories[cat_id].first_counter + id].type;
  switch (cat_id) {
  case CATEGORY_EXC:
    switch (id) {
    case COUNTER_EXC_THROWN:
      sample->rawValue =
          mono_atomic_load_i32(&mono_perfcounters->exceptions_thrown);
      return TRUE;
    }
    break;
  case CATEGORY_ASPNET:
    ...
    case COUNTER_ASPNET_REQ_Q:
      sample->rawValue =
          mono_atomic_load_i32(&mono_perfcounters->aspnet_requests_queued);
      return TRUE;
    case COUNTER_ASPNET_REQ_TOTAL:
      sample->rawValue =
          mono_atomic_load_i32(&mono_perfcounters->aspnet_requests);
      return TRUE;
    }
    break;
  case CATEGORY_THREADPOOL:
    ...
    case COUNTER_THREADPOOL_WORKITEMS:
      sample->rawValue =
          mono_atomic_load_i64(&mono_perfcounters->threadpool_workitems);
      return TRUE;
    case COUNTER_THREADPOOL_IOWORKITEMS:
      sample->rawValue =
          mono_atomic_load_i64(&mono_perfcounters->threadpool_ioworkitems);
      return TRUE;
    case COUNTER_THREADPOOL_THREADS:
      sample->rawValue =
          mono_atomic_load_i32(&mono_perfcounters->threadpool_threads);
      return TRUE;
    case COUNTER_THREADPOOL_IOTHREADS:
      sample->rawValue =
          mono_atomic_load_i32(&mono_perfcounters->threadpool_iothreads);
      return TRUE;
    }
    break;
  case CATEGORY_JIT:
...
static gint64 predef_writable_update(ImplVtable *vtable, MonoBoolean do_incr,
                                     gint64 value) {
  ...
  case CATEGORY_ASPNET:
    switch (id) {
    case COUNTER_ASPNET_REQ_Q:
      ptr = &mono_perfcounters->aspnet_requests_queued;
      break;
    case COUNTER_ASPNET_REQ_TOTAL:
      ptr = &mono_perfcounters->aspnet_requests;
      break;
    }
    break;
  case CATEGORY_THREADPOOL:
    switch (id) {
    case COUNTER_THREADPOOL_WORKITEMS:
      ptr64 = &mono_perfcounters->threadpool_workitems;
      break;
    case COUNTER_THREADPOOL_IOWORKITEMS:
      ptr64 = &mono_perfcounters->threadpool_ioworkitems;
      break;
    case COUNTER_THREADPOOL_THREADS:
      ptr = &mono_perfcounters->threadpool_threads;
      break;
    case COUNTER_THREADPOOL_IOTHREADS:
      ptr = &mono_perfcounters->threadpool_iothreads;
      break;
    }
    break;
  }
  if (ptr) {
    ...
    *ptr = value;
    ...
    if (do_incr) {
      if (value == 1)
        return UnlockedIncrement64(
            ptr64); /* FIXME: use mono_atomic_inc_i64 () */
      if (value == -1)
        return UnlockedDecrement64(
            ptr64); /* FIXME: use mono_atomic_dec_i64 () */

      return UnlockedAdd64(ptr64,
                           value); /* FIXME: use mono_atomic_add_i64 () */
    }
    /* this can be non-atomic */
    *ptr64 = value;
    return value;
...
static void *predef_writable_get_impl(int cat, const gunichar2 *counter,
                                      int counter_length, const char *instance,
  ...
  if ((cdesc = get_counter_in_category(&predef_categories[cat], counter,
    ...
    *type = cdesc->type;
    if (instance == NULL || strcmp(instance, "") == 0)
      return create_vtable(GINT_TO_POINTER((cdesc->id << 16) | cat),
                           predef_writable_counter, predef_writable_update);
    else
...
static MonoBoolean custom_writable_counter(ImplVtable *vtable,
                                           MonoBoolean only_value,
                                           MonoCounterSample *sample) {
...
static gint64 custom_writable_update(ImplVtable *vtable, MonoBoolean do_incr,
                                     gint64 value) {
...
static ImplVtable *custom_vtable(SharedCounter *scounter, SharedInstance *inst,
                                 char *data) {
  CustomVTable *vtable;
...
static gpointer custom_get_value_address(SharedCounter *scounter,
...
static void *custom_get_impl(SharedCategory *cat, const gunichar2 *counter,
                             int counter_length, const gunichar2 *instance,
                             int *type, MonoError *error) {
  ...
  *type = simple_type_to_type[scounter->type];
  ...
  if (!inst)
    return NULL;
  return custom_vtable(scounter, inst,
                       (char *)custom_get_value_address(scounter, inst));
}

static const CategoryDesc *find_category(const gunichar2 *category,
                                         int category_length) {
  int i;
  for (i = 0; i < NUM_CATEGORIES; ++i) {
    if (mono_utf16_equal_ascii_len(category, category_length,
                                   predef_categories[i].name,
                                   predef_categories[i].name_length))
      return &predef_categories[i];
  }
  return NULL;
}

void *mono_perfcounter_get_impl(const gunichar2 *category,
                                gint32 category_length,
                                const gunichar2 *counter, gint32 counter_length,
                                const gunichar2 *instance,
                                gint32 instance_length, gint32 *type,
  ...
  if (!cdesc) {
    ...
    *custom = TRUE;
    result =
        custom_get_impl(scat, counter, counter_length, instance, type, error);
    return_val_if_nok(error, NULL);
    return result;
  }
  ...
  case CATEGORY_PROC:
    result =
        process_get_impl(counter, counter_length, c_instance, type, custom);
    break;
  case CATEGORY_MONO_MEM:
    result =
        mono_mem_get_impl(counter, counter_length, c_instance, type, custom);
    break;
  case CATEGORY_NETWORK:
    result =
        network_get_impl(counter, counter_length, c_instance, type, custom);
    break;
  case CATEGORY_JIT:
  case CATEGORY_EXC:
  case CATEGORY_GC:
  ...
  case CATEGORY_ASPNET:
  case CATEGORY_THREADPOOL:
    result = predef_writable_get_impl(cdesc->id, counter, counter_length,
                                      c_instance, type, custom);
    break;
  }
  g_free(c_instance);
  return result;
...
MonoBoolean mono_perfcounter_get_sample(void *impl, MonoBoolean only_value,
                                        MonoCounterSample *sample) {
...
gint64 mono_perfcounter_update_value(void *impl, MonoBoolean do_incr,
                                     gint64 value) {
  ImplVtable *vtable = (ImplVtable *)impl;
...
/* Category icalls */
MonoBoolean mono_perfcounter_category_del(const gunichar2 *name,
                                          gint32 name_length,
                                          MonoError *error) {
  const CategoryDesc *cdesc;
  SharedCategory *cat;
  ...
  /* FIXME: check the semantics, if deleting a category means also deleting the
   * instances */
...
/* this is an icall */
MonoStringHandle mono_perfcounter_category_help(const gunichar2 *category,
                                                gint32 category_length,
                                                MonoError *error) {
  ...
  if (!cdesc) {
    ...
    result = mono_string_new_handle(mono_domain_get(),
                                    custom_category_help(scat), error);
    return_val_if_nok(error, NULL_HANDLE_STRING);
    return result;
  }
  result = mono_string_new_utf8_len(mono_domain_get(), cdesc->help,
                                    cdesc->help_length, error);
  return_val_if_nok(error, NULL_HANDLE_STRING);
  return result;
}
 ...
 */
MonoBoolean mono_perfcounter_category_exists(const gunichar2 *counter,
                                             gint32 counter_length,
                                             const gunichar2 *category,
                                             gint32 category_length,
                                             MonoError *error) {
  const CategoryDesc *cdesc;
  cdesc = find_category(category, category_length);
 ...
 */
MonoBoolean mono_perfcounter_create(const gunichar2 *category,
                                    gint32 category_length,
                                    const gunichar2 *help, gint32 help_length,
                                    gint32 type, MonoArrayHandle items,
                                    MonoError *error) {
  int result = FALSE;
  int i, j = 0, k, size;
  ...
  /* calculate the size we need structure size + name/help + 2 0 string
   * terminators */
...
MonoBoolean mono_perfcounter_instance_exists(const gunichar2 *instance,
...
/* this is an icall */
MonoArrayHandle mono_perfcounter_category_names(MonoError *error) {
  HANDLE_LOOP_PREPARE;
  ...
  res = mono_array_new_handle(
      domain, mono_get_string_class(),
      NUM_CATEGORIES + g_slist_length(custom_categories), error);
  if (!is_ok(error)) {
  ...
  for (i = 0; i < NUM_CATEGORIES; ++i) {
    ...
    SETUP_ICALL_FRAME;
    name = mono_string_new_utf8_len(domain, cdesc->name, cdesc->name_length,
                                    error);
    if (is_ok(error))
  ...
  for (tmp = custom_categories; tmp; tmp = tmp->next) {
    ...
    SETUP_ICALL_FRAME;
    name =
        mono_string_new_utf8_len(domain, scat->name, strlen(scat->name), error);
    if (is_ok(error))
      MONO_HANDLE_ARRAY_SETREF(res, i, name);
    CLEAR_ICALL_FRAME;
    goto_if_nok(error, leave);
...
MonoArrayHandle mono_perfcounter_counter_names(const gunichar2 *category,
                                               gint32 category_length,
                                               MonoError *error) {
  HANDLE_LOOP_PREPARE;
  ...
  cdesc = find_category(category, category_length);
  if (cdesc) {
    res = mono_array_new_handle(domain, mono_get_string_class(),
                                cdesc[1].first_counter - cdesc->first_counter,
                                error);
    ...
    for (i = cdesc->first_counter; i < cdesc[1].first_counter; ++i) {
      ...
      MonoStringHandle name;
      SETUP_ICALL_FRAME;
      name = mono_string_new_utf8_len(domain, desc->name, desc->name_length,
                                      error);
      if (is_ok(error))
  ...
  if (scat) {
    ...
    for (i = 0; i < scat->num_counters; ++i) {
      ...
      SETUP_ICALL_FRAME;
      str = mono_string_new_utf8_len(domain, counter->name,
                                     strlen(counter->name), error);
      if (is_ok(error))
...
static MonoArrayHandle get_string_array(void **array, int count,
                                        gboolean is_process, MonoError *error) {
  HANDLE_LOOP_PREPARE;
...
static MonoArrayHandle get_string_array_of_strings(void **array, int count,
                                                   MonoError *error) {
  HANDLE_LOOP_PREPARE;
...
static MonoArrayHandle get_mono_instances(MonoError *error) {
  int count = 64;
  int res;
  void **buf = NULL;
...
static MonoArrayHandle get_cpu_instances(MonoError *error) {
  void **buf = NULL;
  int i, count;
  MonoArrayHandle array;
  error_init(error);
...
static MonoArrayHandle get_processes_instances(MonoError *error) {
  MonoArrayHandle array;
  int count = 0;
...
static MonoArrayHandle get_networkinterface_instances(MonoError *error) {
  MonoArrayHandle array;
  int count = 0;
  error_init(error);
...
static MonoArrayHandle get_custom_instances(const gunichar2 *category,
                                            MonoError *error) {
  HANDLE_LOOP_PREPARE;
  ...
  if (scat) {
    ...
    int i = 0;
    MonoArrayHandle array =
        mono_array_new_handle(mono_domain_get(), mono_get_string_class(),
                              g_slist_length(list), error);
    if (!is_ok(error)) {
    ...
    for (tmp = list; tmp; tmp = tmp->next) {
      ...
      MonoStringHandle str;
      SETUP_ICALL_FRAME;
      str = mono_string_new_utf8_len(mono_domain_get(), inst->instance_name,
                                     strlen(inst->instance_name), error);
      if (is_ok(error))
  ...
  return mono_array_new_handle(mono_domain_get(), mono_get_string_class(), 0,
                               error);
}

MonoArrayHandle mono_perfcounter_instance_names(const gunichar2 *category,
                                                gint32 category_length,
                                                MonoError *error) {
  ...
  default:
    result = mono_array_new_handle(mono_domain_get(), mono_get_string_class(),
                                   0, error);
  }
  return result;
}
...
static gboolean mono_perfcounter_foreach_shared_item(SharedHeader *header,
                                                     gpointer data) {
  int i;
  void *addr;
  SharedCategory *cat;
  ...
  if (header->ftype == FTYPE_CATEGORY) {
    ...
    for (i = 0; i < cat->num_counters; ++i) {
      ...
      if (!foreach_data->cb(cat->name, counter->name, counter->type,
                            addr ? *(gint64 *)addr : 0, foreach_data->data))
        return FALSE;
      counter = next_custom_category_counter(counter);
    }
  }
...
void mono_perfcounter_foreach(PerfCounterEnumCallback cb, gpointer data) {
...
void mono_perfcounter_free_data(void *impl) { g_assert_not_reached(); }
