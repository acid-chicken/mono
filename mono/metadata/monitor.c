...
static MonoBoolean mono_monitor_try_enter_loop_if_interrupted(
    MonoObject *obj, guint32 ms, MonoBoolean allow_interruption,
    MonoBoolean *lockTaken, MonoError *error);

/* MonoThreadsSync status helpers */

static guint32 mon_status_get_owner(guint32 status) {
  return status & OWNER_MASK;
}

static guint32 mon_status_set_owner(guint32 status, guint32 owner) {
  return (status & ENTRY_COUNT_MASK) | owner;
}

static gint32 mon_status_get_entry_count(guint32 status) {
  gint32 entry_count =
      (gint32)((status & ENTRY_COUNT_MASK) >> ENTRY_COUNT_SHIFT);
  gint32 zero = (gint32)(((guint32)ENTRY_COUNT_ZERO) >> ENTRY_COUNT_SHIFT);
  return entry_count - zero;
}

static guint32 mon_status_init_entry_count(guint32 status) {
  return (status & OWNER_MASK) | ENTRY_COUNT_ZERO;
}

static guint32 mon_status_add_entry_count(guint32 status, int val) {
  if (val > 0)
...
static gboolean mon_status_have_waiters(guint32 status) {
  return status & ENTRY_COUNT_WAITERS;
}

/* LockWord helpers */

static MonoThreadsSync *lock_word_get_inflated_lock(LockWord lw) {
  lw.lock_word &= (~LOCK_WORD_STATUS_MASK);
  return lw.sync;
}

static gboolean lock_word_is_inflated(LockWord lw) {
  return lw.lock_word & LOCK_WORD_INFLATED;
}

static gboolean lock_word_has_hash(LockWord lw) {
  return lw.lock_word & LOCK_WORD_HAS_HASH;
}

static LockWord lock_word_set_has_hash(LockWord lw) {
  LockWord nlw;
  nlw.lock_word = lw.lock_word | LOCK_WORD_HAS_HASH;
  return nlw;
}

static gboolean lock_word_is_free(LockWord lw) { return !lw.lock_word; }

static gboolean lock_word_is_flat(LockWord lw) {
  /* Return whether the lock is flat or free */
  return (lw.lock_word & LOCK_WORD_STATUS_MASK) == LOCK_WORD_FLAT;
}

static gint32 lock_word_get_hash(LockWord lw) {
  return (gint32)(lw.lock_word >> LOCK_WORD_HASH_SHIFT);
}

static gint32 lock_word_get_nest(LockWord lw) {
...
static gboolean lock_word_is_nested(LockWord lw) {
  return lw.lock_word & LOCK_WORD_NEST_MASK;
}

static gboolean lock_word_is_max_nest(LockWord lw) {
  return (lw.lock_word & LOCK_WORD_NEST_MASK) == LOCK_WORD_NEST_MASK;
}

static LockWord lock_word_increment_nest(LockWord lw) {
  lw.lock_word += 1 << LOCK_WORD_NEST_SHIFT;
  return lw;
}

static LockWord lock_word_decrement_nest(LockWord lw) {
  lw.lock_word -= 1 << LOCK_WORD_NEST_SHIFT;
  return lw;
}

static gint32 lock_word_get_owner(LockWord lw) {
  return lw.lock_word >> LOCK_WORD_OWNER_SHIFT;
}

static LockWord lock_word_new_thin_hash(gint32 hash) {
  LockWord lw;
...
static LockWord lock_word_new_inflated(MonoThreadsSync *mon) {
  LockWord lw;
  lw.sync = mon;
...
static LockWord lock_word_new_flat(gint32 owner) {
  LockWord lw;
  lw.lock_word = owner;
...
void mono_monitor_init(void) { mono_os_mutex_init_recursive(&monitor_mutex); }
...
void mono_monitor_cleanup(void) {
   ...
   * Those weak links reside in the monitor structures, which we've freed
   * earlier.
   *
   * Unless we fix this dependency in the shutdown sequence this code has to
   * remain disabled, or at least the call to g_free().
   */
  /*
...
static int monitor_is_on_freelist(MonoThreadsSync *mon) {
  MonitorArray *marray;
  for (marray = monitor_allocated; marray; marray = marray->next) {
    if (mon >= marray->monitors &&
        mon < &marray->monitors[marray->num_monitors])
      return TRUE;
  }
  return FALSE;
}
...
void mono_locks_dump(gboolean include_untaken) {
    ...
    for (i = 0; i < marray->num_monitors; ++i) {
      ...
      if (mon->data == NULL) {
        ...
        if (!monitor_is_on_freelist((MonoThreadsSync *)mon->data)) {
          MonoObject *holder = (MonoObject *)mono_gchandle_get_target_internal(
              (MonoGCHandle)mon->data);
          if (mon_status_get_owner(mon->status)) {
            ...
            if (mon->entry_cond)
              g_print("\tWaiting on condvar %p: %d\n", mon->entry_cond,
                      mon_status_get_entry_count(mon->status));
  ...
  g_print("Total locks (in %d array(s)): %d, used: %d, on freelist: %d, to "
          "recycle: %d\n",
          num_arrays, total, used, on_freelist, to_recycle);
}

/* LOCKING: this is called with monitor_mutex held */
static void mon_finalize(MonoThreadsSync *mon) {
...
/* LOCKING: this is called with monitor_mutex held */
static MonoThreadsSync *mon_new(gsize id) {
  ...
  if (!monitor_freelist) {
    ...
    for (marray = monitor_allocated; marray; marray = marray->next) {
      for (i = 0; i < marray->num_monitors; ++i) {
        if (mono_gchandle_get_target_internal(
                (MonoGCHandle)marray->monitors[i].data) == NULL) {
          new_ = &marray->monitors[i];
          if (new_->wait_list) {
            /* Orphaned events left by aborted threads */
            while (new_->wait_list) {
              LOCK_DEBUG(g_message(
                  G_GNUC_PRETTY_FUNCTION ": (%d): Closing orphaned event %d",
                  mono_thread_info_get_small_id(), new_->wait_list->data));
              mono_w32event_close(new_->wait_list->data);
              new_->wait_list =
                  g_slist_remove(new_->wait_list, new_->wait_list->data);
            }
          }
    ...
    if (!monitor_freelist) {
      MonitorArray *last;
      LOCK_DEBUG(
          g_message("%s: allocating more monitors: %d", __func__, array_size));
      marray = (MonitorArray *)g_malloc0(MONO_SIZEOF_MONO_ARRAY +
                                         array_size * sizeof(MonoThreadsSync));
...
static MonoThreadsSync *alloc_mon(MonoObject *obj, gint32 id) {
  MonoThreadsSync *mon;
...
static void discard_mon(MonoThreadsSync *mon) {
  mono_monitor_allocator_lock();
...
static void mono_monitor_inflate_owned(MonoObject *obj, int id) {
  MonoThreadsSync *mon;
  LockWord nlw, old_lw, tmp_lw;
  guint32 nest;

  old_lw.sync = obj->synchronisation;
  LOCK_DEBUG(g_message("%s: (%d) Inflating owned lock object %p; LW = %p",
                       __func__, id, obj, old_lw.sync));
  ...
  tmp_lw.sync = (MonoThreadsSync *)mono_atomic_cas_ptr(
      (gpointer *)&obj->synchronisation, nlw.sync, old_lw.sync);
...
static void mono_monitor_inflate(MonoObject *obj) {
  MonoThreadsSync *mon;
  LockWord nlw, old_lw;

  LOCK_DEBUG(g_message("%s: (%d) Inflating lock object %p; LW = %p", __func__,
                       mono_thread_info_get_small_id(), obj,
                       obj->synchronisation));
  ...
  for (;;) {
    ...
    if (lock_word_is_inflated(old_lw)) {
      ...
      /* Lock is flat */
      mon->status =
          mon_status_set_owner(mon->status, lock_word_get_owner(old_lw));
...
int mono_object_hash_internal(MonoObject *obj) {
  ...
  LOCK_DEBUG(g_message("%s: (%d) Get hash for object %p; LW = %p", __func__,
                       mono_thread_info_get_small_id(), obj,
                       obj->synchronisation));
  ...
  if (lock_word_is_free(lw)) {
    ...
    old_lw.sync = (MonoThreadsSync *)mono_atomic_cas_ptr(
        (gpointer *)&obj->synchronisation, lw.sync, NULL);
    if (old_lw.sync == NULL) {
      return hash;
    }
...
int mono_object_hash_icall(MonoObjectHandle obj, MonoError *error) {
...
static gboolean mono_monitor_ensure_owned(LockWord lw, guint32 id) {
  ...
  mono_error_set_synchronization_lock(
      error, "Object synchronization method was called from an unsynchronized "
             "block of code.");
 ...
 */
static void mono_monitor_exit_inflated(MonoObject *obj) {
  LockWord lw;
  MonoThreadsSync *mon;
  guint32 nest;
  ...
  if (nest == 0) {
    ...
    for (;;) {
      ...
      tmp_status =
          mono_atomic_cas_i32((gint32 *)&mon->status, new_status, old_status);
    ...
    LOCK_DEBUG(g_message("%s: (%d) Object %p is now unlocked", __func__,
                         mono_thread_info_get_small_id(), obj));
    ...
    LOCK_DEBUG(g_message("%s: (%d) Object %p is now locked %d times", __func__,
                         mono_thread_info_get_small_id(), obj, nest));
    mon->nest = nest;
  }
}
 ...
 * current thread owns the monitor.
 */
static void mono_monitor_exit_flat(MonoObject *obj, LockWord old_lw) {
  ...
  tmp_lw.sync = (MonoThreadsSync *)mono_atomic_cas_ptr(
      (gpointer *)&obj->synchronisation, new_lw.sync, old_lw.sync);
  if (old_lw.sync != tmp_lw.sync) {
    /* Someone inflated the lock in the meantime */
    mono_monitor_exit_inflated(obj);
  }

  LOCK_DEBUG(g_message("%s: (%d) Object %p is now locked %d times; LW = %p",
                       __func__, mono_thread_info_get_small_id(), obj,
                       lock_word_get_nest(new_lw), obj->synchronisation));
}

static gboolean mon_add_entry_count(MonoThreadsSync *mon, int val) {
...
static void mon_init_cond_var(MonoThreadsSync *mon) {
  if (mon->entry_mutex == NULL) {
    /* Create the mutex */
    MonoCoopMutex *mutex = g_new0(MonoCoopMutex, 1);
    mono_coop_mutex_init(mutex);
    if (mono_atomic_cas_ptr((gpointer *)&mon->entry_mutex, mutex, NULL) !=
        NULL) {
...
static void signal_monitor(gpointer mon_untyped) {
...
static gint64 thread_contentions; /* for Monitor.LockContentionCount, otherwise
                                     mono_perfcounters struct is used */
#endif

/* If allow_interruption==TRUE, the method will be interrupted if abort or
 * suspend is requested. In this case it returns -1.
 */
static gint32 mono_monitor_try_enter_inflated(MonoObject *obj, guint32 ms,
  ...
  LOCK_DEBUG(g_message("%s: (%d) Trying to lock object %p (%d ms)", __func__,
                       id, obj, ms));
...
retry_contended:
   ...
   * callbacks without impacting the fast path: from here on we don't need to go
   * back to the retry label, but to retry_contended. At this point mon is
   * already installed in the object header.
   */
  /* This case differs from Dice's case 3 because we don't
   * deflate locks or cache unused lock records
   */
   ...
   *
   * If we set the entry count, we are guaranteed to be notified when the
   * monitor is released. Because we have the entry_mutex lock taken during this
   * time and wait on the condvar, we are guaranteed not to miss the wakeup.
   *
  ...
  if (!interrupted) {
    ...
    if (ms == MONO_INFINITE_WAIT) {
      ...
      if (mono_coop_cond_timedwait(mon->entry_cond, mon->entry_mutex, waitms) ==
          -1)
        timedout = TRUE;
    }
  ...
  if (timedout || (interrupted && allow_interruption)) {
     ...
     */
    if ((interrupted &&
         !mono_thread_test_state(mono_thread_internal_current(),
                                 ThreadState_SuspendRequested |
                                     ThreadState_AbortRequested)) ||
        !interrupted) {
      /* We were interrupted (and allow_interruption is FALSE) or we were
       * signaled */
      if (ms != MONO_INFINITE_WAIT) {
        now = mono_msec_ticks();
  ...
  if (interrupted) {
    LOCK_DEBUG(
        g_message("%s: (%d) interrupted waiting, returning -1", __func__, id));
    return -1;
  } else if (timedout) {
    LOCK_DEBUG(
        g_message("%s: (%d) timed out waiting, returning FALSE", __func__, id));
    return 0;
  } else {
 ...
 * suspend is requested. In this case it returns -1.
 */
static gint32 mono_monitor_try_enter_internal(MonoObject *obj, guint32 ms,
                                              gboolean allow_interruption) {
  LockWord lw;
  int const id = mono_thread_info_get_small_id();
  ...
  if (G_LIKELY(lock_word_is_free(lw))) {
    ...
    if (mono_atomic_cas_ptr((gpointer *)&obj->synchronisation, nlw.sync,
                            NULL) == NULL) {
      return 1;
    } else {
    ...
    if (lock_word_get_owner(lw) == id) {
      if (lock_word_is_max_nest(lw)) {
        ...
        old_lw.sync = (MonoThreadsSync *)mono_atomic_cas_ptr(
            (gpointer *)&obj->synchronisation, nlw.sync, lw.sync);
        if (old_lw.sync != lw.sync) {
          /* Someone else inflated it in the meantime */
          g_assert(lock_word_is_inflated(old_lw));
          return mono_monitor_try_enter_inflated(obj, ms, allow_interruption,
                                                 id);
        }
        return 1;
      }
...
MonoBoolean mono_monitor_enter_internal(MonoObject *obj) {
  ...
  return mono_monitor_try_enter_loop_if_interrupted(
      obj, timeout_milliseconds, allow_interruption, &lock_taken, error);
}
...
gboolean mono_monitor_try_enter(MonoObject *obj, guint32 ms) {
  ...
  MONO_EXTERNAL_ONLY(gboolean,
                     mono_monitor_try_enter_internal(obj, ms, FALSE) == 1);
}

void mono_monitor_exit_internal(MonoObject *obj) {
  LockWord lw;

  LOCK_DEBUG(g_message("%s: (%d) Unlocking %p", __func__,
                       mono_thread_info_get_small_id(), obj));
...
void mono_monitor_exit_icall(MonoObjectHandle obj, MonoError *error) {
...
MonoGCHandle mono_monitor_get_object_monitor_gchandle(MonoObject *object) {
  LockWord lw;
 ...
 */
void mono_monitor_threads_sync_members_offset(int *status_offset,
                                              int *nest_offset) {
  MonoThreadsSync ts;

#define ENCODE_OFF_SIZE(o, s) (((o) << 8) | ((s)&0xff))

  *status_offset = ENCODE_OFF_SIZE(MONO_STRUCT_OFFSET(MonoThreadsSync, status),
                                   sizeof(ts.status));
  *nest_offset = ENCODE_OFF_SIZE(MONO_STRUCT_OFFSET(MonoThreadsSync, nest),
                                 sizeof(ts.nest));
}
...
static MonoBoolean mono_monitor_try_enter_loop_if_interrupted(
    MonoObject *obj, guint32 ms, MonoBoolean allow_interruption,
    MonoBoolean *lockTaken, MonoError *error) {
  // Return value and lockTaken are equivalent, except, to preserve prior
  // behavior, *lockTaken is not always written to, i.e. in the error paths.
  //
  // Some callers have lockTaken and only use it, some only have the return
  // value.
   ...
   * It exists to deal with interrupting a monitor enter that happened within an
   * abort-protected block, like a .cctor.
   *
   * The thread will be set with a pending abort and the wait might even be
   * interrupted. Either way, once we call mono_thread_interruption_checkpoint,
   * it will return NULL meaning we can't be aborted right now. Once that
   * happens we switch to non-alertable.
   */
  do {
  ...
  /*It's safe to do it from here since interruption would happen only on the
   * wrapper.*/
  *lockTaken = res == 1;
  return res;
}
...
void ves_icall_System_Threading_Monitor_Monitor_try_enter_with_atomic_var(
    MonoObjectHandle obj, guint32 ms, MonoBoolean allow_interruption,
    MonoBoolean *lockTaken, MonoError *error) {
  mono_monitor_try_enter_loop_if_interrupted(
      MONO_HANDLE_RAW(obj), ms, allow_interruption, lockTaken, error);
}
#else
void ves_icall_System_Threading_Monitor_Monitor_try_enter_with_atomic_var(
    MonoObjectHandle obj, guint32 ms, MonoBoolean *lockTaken,
    MonoError *error) {
  mono_monitor_try_enter_loop_if_interrupted(MONO_HANDLE_RAW(obj), ms, TRUE,
                                             lockTaken, error);
}
#endif
...
void mono_monitor_enter_v4_internal(MonoObject *obj, MonoBoolean *lock_taken) {
  ...
  mono_monitor_try_enter_loop_if_interrupted(obj, MONO_INFINITE_WAIT, FALSE,
                                             lock_taken, NULL);
}
...
MonoBoolean ves_icall_System_Threading_Monitor_Monitor_test_owner(
    MonoObjectHandle obj_handle, MonoError *error) {
  MonoObject *const obj = MONO_HANDLE_RAW(obj_handle);

  LockWord lw;

  LOCK_DEBUG(g_message("%s: Testing if %p is owned by thread %d", __func__, obj,
  ...
  if (lock_word_is_flat(lw)) {
    ...
    return mon_status_get_owner(lock_word_get_inflated_lock(lw)->status) ==
           mono_thread_info_get_small_id();
  }
...
MonoBoolean ves_icall_System_Threading_Monitor_Monitor_test_synchronised(
    MonoObjectHandle obj_handle, MonoError *error) {
  MonoObject *const obj = MONO_HANDLE_RAW(obj_handle);

  LockWord lw;

  LOCK_DEBUG(g_message("%s: (%d) Testing if %p is owned by any thread",
                       __func__, mono_thread_info_get_small_id(), obj));
...
static void mono_monitor_pulse(MonoObject *obj, const char *func,
                               gboolean all) {
  int const id = mono_thread_info_get_small_id();
  LockWord lw;
  MonoThreadsSync *mon;
  ...
  LOCK_DEBUG(g_message("%s: (%d) %d threads waiting", func, id,
                       g_slist_length(mon->wait_list)));

  do {
    if (mon->wait_list != NULL) {
      LOCK_DEBUG(g_message("%s: (%d) signalling and dequeuing handle %p", func,
                           id, mon->wait_list->data));
...
void ves_icall_System_Threading_Monitor_Monitor_pulse(MonoObjectHandle obj,
                                                      MonoError *error) {
  mono_monitor_pulse(MONO_HANDLE_RAW(obj), __func__, FALSE);
}

void ves_icall_System_Threading_Monitor_Monitor_pulse_all(MonoObjectHandle obj,
                                                          MonoError *error) {
  mono_monitor_pulse(MONO_HANDLE_RAW(obj), __func__, TRUE);
}

static MonoBoolean mono_monitor_wait(MonoObjectHandle obj_handle, guint32 ms,
  ...
  LOCK_DEBUG(g_message("%s: (%d) Trying to wait for %p with timeout %dms",
                       __func__, id, obj, ms));
  ...
  do {
    regain = mono_monitor_try_enter_inflated(obj, MONO_INFINITE_WAIT,
                                             allow_interruption, id);
  ...
  if (ret == MONO_W32HANDLE_WAIT_RET_SUCCESS_0) {
    ...
    LOCK_DEBUG(g_message("%s: (%d) Wait failed, dequeuing handle %p", __func__,
                         id, event));
...
#ifdef ENABLE_NETCORE
MonoBoolean ves_icall_System_Threading_Monitor_Monitor_wait(
    MonoObjectHandle obj_handle, guint32 ms, MonoBoolean allow_interruption,
    MonoError *error) {
  return mono_monitor_wait(obj_handle, ms, allow_interruption, error);
}
#else
MonoBoolean
ves_icall_System_Threading_Monitor_Monitor_wait(MonoObjectHandle obj_handle,
                                                guint32 ms, MonoError *error) {
  return mono_monitor_wait(obj_handle, ms, TRUE, error);
}
#endif
void ves_icall_System_Threading_Monitor_Monitor_Enter(MonoObjectHandle obj,
                                                      MonoError *error) {
