...
// Inline with CoreCLR heuristics,
// https://github.com/dotnet/runtime/blob/385b4d4296f9c5cb82363565aa210a1a37f92d90/src/coreclr/src/vm/threads.cpp#L6344.
// Minimum stack size should be sufficient to allow a typical non-recursive call
// chain to execute, including potential exception handling and garbage
// collection. Used for probing for available stack space through
...
static gboolean icallarray_print_enabled(void) {
  if (!debug_icallarray)
    debug_icallarray =
        MONO_TRACE_IS_TRACED(G_LOG_LEVEL_DEBUG, MONO_TRACE_ICALLARRAY) ? 1 : 2;
  return debug_icallarray == 1;
}

static void icallarray_print(const char *format, ...) {
...
static GENERATE_GET_CLASS_WITH_CACHE(module, "System.Reflection", "Module")
    ...
    static void array_set_value_impl(MonoArrayHandle arr,
                                     MonoObjectHandle value, guint32 pos,
                                     gboolean strict_enums,
                                     gboolean strict_signs, MonoError *error);

static MonoArrayHandle type_array_from_modifiers(MonoImage *image,
                                                 MonoType *type, int optional,
                                                 MonoError *error);

static inline MonoBoolean is_generic_parameter(MonoType *type) {
  return !type->byref &&
         (type->type == MONO_TYPE_VAR || type->type == MONO_TYPE_MVAR);
}

#ifndef HOST_WIN32
static inline void mono_icall_make_platform_path(gchar *path) { return; }

static inline const gchar *mono_icall_get_file_path_prefix(const gchar *path) {
  return "file://";
}
#endif /* HOST_WIN32 */
...
MonoObjectHandle ves_icall_System_Array_GetValueImpl(MonoArrayHandle array,
                                                     guint32 pos,
                                                     MonoError *error) {
  ...
  if (m_class_is_valuetype(element_class)) {
    ...
    gpointer element_address = mono_array_addr_with_size_fast(
        MONO_HANDLE_RAW(array), element_size, (gsize)pos);
    return mono_value_box_handle(MONO_HANDLE_DOMAIN(array), element_class,
...
MonoObjectHandle ves_icall_System_Array_GetValue(MonoArrayHandle arr,
                                                 MonoArrayHandle indices,
                                                 MonoError *error) {
  ...
  if (MONO_HANDLE_GETVAL(indices, bounds) ||
      MONO_HANDLE_GETVAL(indices, max_length) !=
  ...
  for (gint32 i = 0; i < m_class_get_rank(array_class); i++) {
    ...
    if ((index < MONO_HANDLE_GETVAL(arr, bounds[i].lower_bound)) ||
        (index >=
         (mono_array_lower_bound_t)MONO_HANDLE_GETVAL(arr, bounds[i].length) +
...
void ves_icall_System_Array_SetValueImpl(MonoArrayHandle arr,
                                         MonoObjectHandle value, guint32 pos,
                                         MonoError *error) {
  array_set_value_impl(arr, value, pos, FALSE, TRUE, error);
}

static inline void set_invalid_cast(MonoError *error, MonoClass *src_class,
                                    MonoClass *dst_class) {
...
#if ENABLE_NETCORE
void ves_icall_System_Array_SetValueRelaxedImpl(MonoArrayHandle arr,
                                                MonoObjectHandle value,
                                                guint32 pos, MonoError *error) {
  array_set_value_impl(arr, value, pos, FALSE, FALSE, error);
}

// Copied from CoreCLR:
// https://github.com/dotnet/coreclr/blob/d3e39bc2f81e3dbf9e4b96347f62b49d8700336c/src/vm/invokeutil.cpp#L33
...
static const guint32 primitive_conversions[] = {
    ...
    PT_Primitive |
        0x3F88, // MONO_TYPE_CHAR (W = U2, CHAR, I4, U4, I8, U8, R4, R8)
    PT_Primitive | 0x3550, // MONO_TYPE_I1   (W = I1, I2, I4, I8, R4, R8)
    PT_Primitive |
        0x3FE8, // MONO_TYPE_U1   (W = CHAR, U1, I2, U2, I4, U4, I8, U8, R4, R8)
    PT_Primitive | 0x3540, // MONO_TYPE_I2   (W = I2, I4, I8, R4, R8)
    PT_Primitive |
        0x3F88, // MONO_TYPE_U2   (W = U2, CHAR, I4, U4, I8, U8, R4, R8)
...
// Copied from CoreCLR:
// https://github.com/dotnet/coreclr/blob/030a3ea9b8dbeae89c90d34441d4d9a1cf4a7de6/src/vm/invokeutil.h#L176
static gboolean can_primitive_widen(MonoTypeEnum src_type,
                                    MonoTypeEnum dest_type) {
  if (dest_type > MONO_TYPE_R8 || src_type > MONO_TYPE_R8) {
    return (MONO_TYPE_I == dest_type && MONO_TYPE_I == src_type) ||
...
// Copied from CoreCLR:
// https://github.com/dotnet/coreclr/blob/eafa8648ebee92de1380278b15cd5c2b6ef11218/src/vm/array.cpp#L1406
static MonoTypeEnum
...
MonoBoolean ves_icall_System_Array_CanChangePrimitive(
    MonoReflectionType *volatile *ref_src_type_handle,
    MonoReflectionType *volatile *ref_dst_type_handle, MonoBoolean reliable) {
  ...
  MonoTypeEnum normalized_src_type =
      get_normalized_integral_array_element_type(src_type->type);
  MonoTypeEnum normalized_dst_type =
      get_normalized_integral_array_element_type(dst_type->type);
...
static void array_set_value_impl(MonoArrayHandle arr_handle,
                                 MonoObjectHandle value_handle, guint32 pos,
                                 gboolean strict_enums, gboolean strict_signs,
                                 MonoError *error) {
  MonoClass *ac, *vc, *ec;
  gint32 esize, vsize;
  ...
  if (mono_class_is_nullable(ec)) {
#ifdef ENABLE_NETCORE
    if (vc && m_class_is_primitive(vc) &&
        vc != m_class_get_nullable_elem_class(ec)) {
    ...
    ea = (gpointer *)mono_array_addr_with_size_internal(
        MONO_HANDLE_RAW(arr_handle), esize, pos);
    if (!MONO_HANDLE_IS_NULL(value_handle))
      va =
          (gpointer *)mono_object_unbox_internal(MONO_HANDLE_RAW(value_handle));
...
#define CHECK_WIDENING_CONVERSION(extra)                                       \
  G_STMT_START {                                                               \
...
#define INVALID_CAST                                                           \
  ...
  et_isenum = et == MONO_TYPE_VALUETYPE &&
              m_class_is_enumtype(m_class_get_byval_arg(ec)->data.klass);
  vt_isenum = vt == MONO_TYPE_VALUETYPE &&
              m_class_is_enumtype(m_class_get_byval_arg(vc)->data.klass);
  ...
  if (et_isenum)
    et =
        mono_class_enum_basetype_internal(m_class_get_byval_arg(ec)->data.klass)
            ->type;

  if (vt_isenum)
    vt =
        mono_class_enum_basetype_internal(m_class_get_byval_arg(vc)->data.klass)
            ->type;

#if ENABLE_NETCORE
    // Treat MONO_TYPE_U/I as MONO_TYPE_U8/I8/U4/I4
#if SIZEOF_VOID_P == 8
  vt = vt == MONO_TYPE_U ? MONO_TYPE_U8
                         : (vt == MONO_TYPE_I ? MONO_TYPE_I8 : vt);
  et = et == MONO_TYPE_U ? MONO_TYPE_U8
                         : (et == MONO_TYPE_I ? MONO_TYPE_I8 : et);
#else
  vt = vt == MONO_TYPE_U ? MONO_TYPE_U4
                         : (vt == MONO_TYPE_I ? MONO_TYPE_I4 : vt);
  et = et == MONO_TYPE_U ? MONO_TYPE_U4
                         : (et == MONO_TYPE_I ? MONO_TYPE_I4 : et);
#endif
#endif

#define ASSIGN_UNSIGNED(etype)                                                 \
  G_STMT_START {                                                               \
    switch (vt) {                                                              \
    case MONO_TYPE_U1:                                                         \
    case MONO_TYPE_U2:                                                         \
...
void ves_icall_System_Array_SetValue(MonoArrayHandle arr,
                                     MonoObjectHandle value,
  ...
  if (mono_handle_array_has_bounds(idxs) ||
      MONO_HANDLE_GETVAL(idxs, max_length) != m_class_get_rank(ac)) {
#ifdef ENABLE_NETCORE
  ...
  if (!mono_handle_array_has_bounds(arr)) {
    ...
    if (idx < 0 || idx >= MONO_HANDLE_GETVAL(arr, max_length)) {
      mono_error_set_exception_instance(
          error, mono_get_exception_index_out_of_range());
      return;
    }
...
void ves_icall_System_Array_InternalCreate(MonoArray *volatile *result,
                                           MonoType *type, gint32 rank,
                                           gint32 *pLengths,
                                           gint32 *pLowerBounds) {
  ERROR_DECL(error);
  ...
  if (m_class_get_byval_arg(m_class_get_element_class(klass))->type ==
      MONO_TYPE_VOID) {
  ...
  if (is_generic_parameter(type) || mono_class_is_gtd(klass) ||
  ...
  for (uintptr_t i = 0; i < aklass_rank; ++i) {
    if (pLowerBounds != NULL) {
      ...
      if ((gint64)pLowerBounds[i] + (gint64)pLengths[i] > G_MAXINT32) {
        mono_error_set_argument_out_of_range(
            error, NULL, "Length + bound must not exceed Int32.MaxValue.");
        goto exit;
      }
    } else {
      lower_bounds[i] = 0;
  ...
  *result = mono_array_new_full_checked(mono_domain_get(), aklass, sizes,
                                        lower_bounds, error);
...
MonoArrayHandle ves_icall_System_Array_CreateInstanceImpl(
    MonoReflectionTypeHandle type, MonoArrayHandle lengths,
    MonoArrayHandle bounds, MonoError *error) {
  // FIXME? fixed could be used for lengths, bounds.

  icallarray_print("%s type:%p length:%p bounds:%p\n", __func__, type, lengths,
                   bounds);

  MONO_CHECK_ARG_NULL_HANDLE(type, NULL_HANDLE_ARRAY);
  MONO_CHECK_ARG_NULL_HANDLE(lengths, NULL_HANDLE_ARRAY);

  MONO_CHECK_ARG(lengths, mono_array_handle_length(lengths) > 0,
                 NULL_HANDLE_ARRAY);
  if (!MONO_HANDLE_IS_NULL(bounds))
    MONO_CHECK_ARG(bounds,
                   mono_array_handle_length(lengths) ==
                       mono_array_handle_length(bounds),
                   NULL_HANDLE_ARRAY);
  ...
  for (uintptr_t i = 0; i < mono_array_handle_length(lengths); ++i) {
    ...
    if (length < 0) {
      mono_error_set_argument_out_of_range(error, NULL,
                                           "MonoArgumentException:NULL");
      return NULL_HANDLE_ARRAY;
    }
  }

  MonoClass *klass =
      mono_class_from_mono_type_internal(MONO_HANDLE_GETVAL(type, type));
  if (!mono_class_init_checked(klass, error))
    return NULL_HANDLE_ARRAY;
  ...
  MonoClass *const aklass = mono_class_create_bounded_array(
      klass, mono_array_handle_length(lengths), bounded);
  ...
  return mono_array_new_full_handle(MONO_HANDLE_DOMAIN(type), aklass, sizes,
                                    lower_bounds, error);
}

gint32 ves_icall_System_Array_GetRank(MonoObjectHandle arr, MonoError *error) {
  gint32 const result = m_class_get_rank(mono_handle_class(arr));

  icallarray_print("%s arr:%p res:%d\n", __func__, MONO_HANDLE_RAW(arr),
                   result);
...
#ifdef ENABLE_NETCORE
gint32
ves_icall_System_Array_GetCorElementTypeOfElementType(MonoArrayHandle arr,
                                                      MonoError *error) {
  MonoType *type = mono_type_get_underlying_type(
      m_class_get_byval_arg(m_class_get_element_class(mono_handle_class(arr))));
  return type->type;
}

gint32 ves_icall_System_Array_IsValueOfElementType(MonoArrayHandle arr,
                                                   MonoObjectHandle obj,
                                                   MonoError *error) {
  return m_class_get_element_class(mono_handle_class(arr)) ==
...
mono_array_get_length(MonoArrayHandle arr, gint32 dimension, MonoError *error) {
  ...
  return MONO_HANDLE_GETVAL(arr, bounds)
             ? MONO_HANDLE_GETVAL(arr, bounds[dimension].length)
             : MONO_HANDLE_GETVAL(arr, max_length);
}

gint32 ves_icall_System_Array_GetLength(MonoArrayHandle arr, gint32 dimension,
                                        MonoError *error) {
...
#ifndef ENABLE_NETCORE
gint64 ves_icall_System_Array_GetLongLength(MonoArrayHandle arr,
                                            gint32 dimension,
                                            MonoError *error) {
  icallarray_print("%s arr:%p dimension:%d\n", __func__, MONO_HANDLE_RAW(arr),
                   (int)dimension);
...
gint32 ves_icall_System_Array_GetLowerBound(MonoArrayHandle arr,
                                            gint32 dimension,
                                            MonoError *error) {
  icallarray_print("%s arr:%p dimension:%d\n", __func__, MONO_HANDLE_RAW(arr),
                   (int)dimension);
  ...
  return MONO_HANDLE_GETVAL(arr, bounds)
             ? MONO_HANDLE_GETVAL(arr, bounds[dimension].lower_bound)
             : 0;
}

#ifndef ENABLE_NETCORE
void ves_icall_System_Array_ClearInternal(MonoArrayHandle arr, int idx,
                                          int length, MonoError *error) {
  icallarray_print("%s arr:%p idx:%d len:%d\n", __func__, MONO_HANDLE_RAW(arr),
                   (int)idx, (int)length);

  int sz = mono_array_element_size(mono_handle_class(arr));
  mono_gc_bzero_atomic(
      mono_array_addr_with_size_fast(MONO_HANDLE_RAW(arr), sz, idx),
      length * sz);
}
#endif

MonoBoolean ves_icall_System_Array_FastCopy(MonoArrayHandle source,
                                            int source_idx,
                                            MonoArrayHandle dest, int dest_idx,
                                            int length, MonoError *error) {
  ...
  /* there's no integer overflow since mono_array_length_internal returns an
   * unsigned integer */
  ...
  We fallback to managed here since we need to typecheck each boxed valuetype
  before storing them in the dest array.
  */
  if (src_class == mono_defaults.object_class &&
      m_class_is_valuetype(dest_class))
    return FALSE;
  ...
  if (src_class != dest_class) {
    ...
    /* It's only safe to copy between arrays if we can ensure the source will
     * always have a subtype of the destination. We bail otherwise. */
    if (!mono_class_is_subclass_of_internal(src_class, dest_class, FALSE))
      return FALSE;

    if (m_class_is_native_pointer(src_class) ||
        m_class_is_native_pointer(dest_class))
      return FALSE;
  }

  if (m_class_is_valuetype(dest_class)) {
    gsize const element_size =
        mono_array_element_size(MONO_HANDLE_GETVAL(source, obj.vtable->klass));

    MONO_ENTER_NO_SAFEPOINTS; // gchandle would also work here, is slow, breaks
                              // profiler tests.

    gconstpointer const source_addr = mono_array_addr_with_size_fast(
    ...
    if (m_class_has_references(dest_class)) {
      ...
      gpointer const dest_addr = mono_array_addr_with_size_fast(
          MONO_HANDLE_RAW(dest), element_size, dest_idx);
...
void ves_icall_System_Array_GetGenericValue_icall(MonoArray **arr, guint32 pos,
                                                  gpointer value) {
  ...
  gconstpointer *const ea =
      (gconstpointer *)((char *)(*arr)->vector + (pos * esize));

  mono_gc_memmove_atomic(value, ea, esize);
}

void ves_icall_System_Array_SetGenericValue_icall(MonoArray **arr, guint32 pos,
                                                  gpointer value) {
  icallarray_print("%s arr:%p pos:%u value:%p\n", __func__, *arr, pos, value);
...
#if ENABLE_NETCORE
void ves_icall_System_Runtime_RuntimeImports_RhBulkMoveWithWriteBarrier(
    guint8 *destination, guint8 *source, size_t byte_count) {
  mono_gc_wbarrier_range_copy(destination, source, byte_count);
}
#else
void ves_icall_System_Runtime_RuntimeImports_Memmove_wbarrier(
    guint8 *destination, guint8 *source, guint len, MonoType *type) {
  if (MONO_TYPE_IS_REFERENCE(type))
    mono_gc_wbarrier_arrayref_copy_internal(destination, source, len);
  else
    mono_gc_wbarrier_value_copy_internal(
        destination, source, len, mono_class_from_mono_type_internal(type));
}
#endif
...
void ves_icall_System_Runtime_CompilerServices_RuntimeHelpers_InitializeArray(
    MonoArrayHandle array, MonoClassField *field_handle, MonoError *error) {
  ...
  if (MONO_TYPE_IS_REFERENCE(type) || type->type == MONO_TYPE_VALUETYPE) {
    mono_error_set_argument(error, "array",
  ...
  if (!(field_type->attrs & FIELD_ATTRIBUTE_HAS_FIELD_RVA)) {
    mono_error_set_argument_format(error, "field_handle",
                                   "Field '%s' doesn't have an RVA",
                                   mono_field_get_name(field_handle));
    return;
  }
  ...
  if (size > mono_type_size(field_handle->type, &align)) {
    mono_error_set_argument(error, "field_handle",
...
#define SWAP(n)                                                                \
  {                                                                            \
    guint##n *data =                                                           \
        (guint##n *)mono_array_addr_internal(MONO_HANDLE_RAW(array), char, 0); \
  ...
  case MONO_TYPE_R8:
    ...
    break;
  default:
    memcpy(mono_array_addr_internal(MONO_HANDLE_RAW(array), char, 0),
           field_data, size);
    break;
  }
#else
  memcpy(mono_array_addr_internal(MONO_HANDLE_RAW(array), char, 0), field_data,
         size);
#endif
}
...
ves_icall_System_Runtime_CompilerServices_RuntimeHelpers_GetObjectValue(
    MonoObjectHandle obj, MonoError *error) {
...
void ves_icall_System_Runtime_CompilerServices_RuntimeHelpers_RunClassConstructor(
    MonoType *handle, MonoError *error) {
  MonoClass *klass;
  MonoVTable *vtable;
...
void ves_icall_System_Runtime_CompilerServices_RuntimeHelpers_RunModuleConstructor(
    MonoImage *image, MonoError *error) {
  mono_image_check_for_module_cctor(image);
  if (!image->has_module_cctor)
    return;

  MonoClass *module_klass =
      mono_class_get_checked(image, MONO_TOKEN_TYPE_DEF | 1, error);
  return_if_nok(error);

  MonoVTable *vtable =
      mono_class_vtable_checked(mono_domain_get(), module_klass, error);
  return_if_nok(error);
...
ves_icall_System_Runtime_CompilerServices_RuntimeHelpers_SufficientExecutionStack(
  ...
  // Stack upper/lower bound should have been calculated and set as part of
  // register_thread. If not, we are optimistic and assume there is enough room.
  if (!thread->stack_start_limit || !thread->stack_end)
    return TRUE;

  // Stack start limit is stack lower bound. Make sure there is enough room
  // left.
  void *limit =
      ((uint8_t *)thread->stack_start_limit) +
      ALIGN_TO(MONO_STACK_OVERFLOW_GUARD_SIZE + MONO_MIN_EXECUTION_STACK_SIZE,
               ((gssize)mono_pagesize()));
...
ves_icall_System_Runtime_CompilerServices_RuntimeHelpers_GetUninitializedObjectInternal(
  ...
  if (m_class_is_abstract(klass) || m_class_is_interface(klass) ||
      m_class_is_gtd(klass)) {
  ...
  if (m_class_is_nullable(klass))
    return mono_object_new_handle(
        mono_domain_get(), m_class_get_nullable_elem_class(klass), error);
  else
    return mono_object_new_handle(mono_domain_get(), klass, error);
}

void ves_icall_System_Runtime_CompilerServices_RuntimeHelpers_PrepareMethod(
    MonoMethod *method, gpointer inst_types, int n_inst_types,
    MonoError *error) {
  ...
  if (container) {
    int nparams = container->type_argc +
                  (container->parent ? container->parent->type_argc : 0);
...
MonoObjectHandle
ves_icall_System_Object_MemberwiseClone(MonoObjectHandle this_obj,
                                        MonoError *error) {
  return mono_object_clone_handle(this_obj, error);
}

gint32 ves_icall_System_ValueType_InternalGetHashCode(MonoObjectHandle this_obj,
                                                      MonoArrayHandleOut fields,
                                                      MonoError *error) {
  MonoClass *klass;
  ...
  if (unhandled) {
    MonoArrayHandle fields_arr = mono_array_new_handle(
        mono_domain_get(), mono_defaults.object_class, count, error);
    return_val_if_nok(error, 0);
    MONO_HANDLE_ASSIGN(fields, fields_arr);
    MonoObjectHandle h = MONO_HANDLE_NEW(MonoObject, NULL);
    for (int i = 0; i < count; ++i) {
      MonoObject *o = mono_field_get_value_object_checked(
          mono_handle_domain(this_obj), unhandled[i], MONO_HANDLE_RAW(this_obj),
          error);
      return_val_if_nok(error, 0);
...
MonoBoolean ves_icall_System_ValueType_Equals(MonoObjectHandle this_obj,
                                              MonoObjectHandle that,
                                              MonoArrayHandleOut fields,
                                              MonoError *error) {
  MonoClass *klass;
  MonoClassField **unhandled = NULL;
  ...
  if (m_class_is_enumtype(klass) && mono_class_enum_basetype_internal(klass) &&
      mono_class_enum_basetype_internal(klass)->type == MONO_TYPE_I4)
    return *(gint32 *)mono_handle_get_data_unsafe(this_obj) ==
           *(gint32 *)mono_handle_get_data_unsafe(that);
...
#define UNALIGNED_COMPARE(type)                                                \
  do {                                                                         \
    ...
    case MONO_TYPE_STRING: {
      ...
      if (memcmp(mono_string_chars_internal(s1), mono_string_chars_internal(s2),
                 s1len * sizeof(gunichar2)) != 0)
        return FALSE;
      break;
    }
    default:
...
#undef UNALIGNED_COMPARE
  ...
  if (unhandled) {
    MonoArrayHandle fields_arr = mono_array_new_handle(
        mono_domain_get(), mono_defaults.object_class, count * 2, error);
    return_val_if_nok(error, 0);
    MONO_HANDLE_ASSIGN(fields, fields_arr);
    MonoObjectHandle h = MONO_HANDLE_NEW(MonoObject, NULL);
    for (int i = 0; i < count; ++i) {
      ...
      o = mono_field_get_value_object_checked(mono_handle_domain(this_obj),
                                              unhandled[i],
                                              MONO_HANDLE_RAW(that), error);
      return_val_if_nok(error, FALSE);
      MONO_HANDLE_ASSIGN_RAW(h, o);
...
#ifndef ENABLE_NETCORE
MonoReflectionTypeHandle ves_icall_System_Object_GetType(MonoObjectHandle obj,
                                                         MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(obj);
  MonoClass *klass = mono_handle_class(obj);
#ifndef DISABLE_REMOTING
  if (mono_class_is_transparent_proxy(klass)) {
    MonoTransparentProxyHandle proxy_obj =
        MONO_HANDLE_CAST(MonoTransparentProxy, obj);
...
#endif
    return mono_type_get_object_handle(domain, m_class_get_byval_arg(klass),
                                       error);
}
#endif

static gboolean get_executing(MonoMethod *m, gint32 no, gint32 ilo,
...
static gboolean in_corlib_name_space(MonoClass *klass, const char *name_space) {
  return m_class_get_image(klass) == mono_defaults.corlib &&
         !strcmp(m_class_get_name_space(klass), name_space);
}

static gboolean get_caller_no_reflection(MonoMethod *m, gint32 no, gint32 ilo,
                                         gboolean managed, gpointer data) {
  MonoMethod **dest = (MonoMethod **)data;
...
static gboolean get_caller_no_system_or_reflection(MonoMethod *m, gint32 no,
                                                   gint32 ilo, gboolean managed,
                                                   gpointer data) {
  MonoMethod **dest = (MonoMethod **)data;
  ...
  if (in_corlib_name_space(m->klass, "System.Reflection") ||
      in_corlib_name_space(m->klass, "System"))
    return FALSE;
 ...
 * callers.
 */
MonoMethod *mono_runtime_get_caller_no_system_or_reflection(void) {
...
type_from_parsed_name(MonoTypeNameParse *info, MonoStackCrawlMark *stack_mark,
                      MonoBoolean ignoreCase, MonoAssembly **caller_assembly,
                      MonoError *error) {
  MonoMethod *m;
  MonoType *type = NULL;
  ...
  /*
   * We must compute the calling assembly as type loading must happen under a
   * metadata context. For example. The main assembly is a.exe and Type.GetType
   * is called from dir/b.dll. Without the metadata context (basedir currently)
   * set to dir/b.dll we won't be able to load a dir/c.dll.
   */
  ...
  if (assembly) {
    ...
    // FIXME: once wasm can use stack marks, consider turning all this into an
    // assert
    g_warning(G_STRLOC);
  }
  ...
  *caller_assembly = assembly;
  ...
  if (assembly) {
    /* When loading from the current assembly, AppDomain.TypeResolve will not be
     * called yet */
    type = mono_reflection_get_type_checked(alc, rootimage, assembly->image,
  ...
  //  we FAIL the get type above, because S.G.Dict isn't in assembly->image.  So
  //  we drop down here. but then we FAIL AGAIN because now we pass null as the
  //  image and the rootimage and everything is messed up when we go to
  //  construct the Local as the type arg...
  //
  // By contrast, if we started with Mine<System.Generic.Dict<int, Local>> we'd
  // go in with assembly->image as the root and then even the detour into
  // generics would still not cause issues when we went to load Local.
  if (!info->assembly.name && !type) {
    /* try mscorlib */
    type = mono_reflection_get_type_checked(
        alc, rootimage, NULL, info, ignoreCase, TRUE, &type_resolve, error);
    goto_if_nok(error, fail);
  }
  if (assembly && !type && type_resolve) {
    type_resolve =
        FALSE; /* This will invoke TypeResolve if not done in the first 'if' */
    type = mono_reflection_get_type_checked(alc, rootimage, assembly->image,
                                            info, ignoreCase, TRUE,
                                            &type_resolve, error);
    goto_if_nok(error, fail);
...
MonoReflectionTypeHandle ves_icall_System_RuntimeTypeHandle_internal_from_name(
    MonoStringHandle name, MonoStackCrawlMark *stack_mark,
    MonoReflectionAssemblyHandle callerAssembly, MonoBoolean throwOnError,
    MonoBoolean ignoreCase, MonoBoolean reflectionOnly, MonoError *error) {
  ...
  MONO_HANDLE_ASSIGN(
      type, type_from_parsed_name(&info, (MonoStackCrawlMark *)stack_mark,
                                  ignoreCase, &caller_assembly, error));

  goto_if_nok(error, leave);

  if (MONO_HANDLE_IS_NULL(type)) {
    if (throwOnError) {
      char *tname = info.name_space
                        ? g_strdup_printf("%s.%s", info.name_space, info.name)
                        : g_strdup(info.name);
      char *aname;
      if (info.assembly.name)
        aname = mono_stringify_assembly_name(&info.assembly);
      else if (caller_assembly)
        aname = mono_stringify_assembly_name(
            mono_assembly_get_name_internal(caller_assembly));
      else
        aname = g_strdup("");
...
MonoType *ves_icall_Mono_RuntimeClassHandle_GetTypeFromClass(MonoClass *klass) {
...
void ves_icall_Mono_SafeStringMarshal_GFree(void *c_str) { g_free(c_str); }

char *ves_icall_Mono_SafeStringMarshal_StringToUtf8(MonoString *volatile *s) {
  ERROR_DECL(error);
...
#ifndef ENABLE_NETCORE
guint32 ves_icall_type_GetTypeCodeInternal(MonoReflectionTypeHandle ref_type,
                                           MonoError *error) {
...
handle_enum:
  ...
  case MONO_TYPE_GENERICINST:
    if (m_class_is_enumtype(type->data.generic_class->container_class)) {
      type = mono_class_enum_basetype_internal(
          type->data.generic_class->container_class);
      goto handle_enum;
    }
    return TYPECODE_OBJECT;
  default:
...
static MonoType *mono_type_get_underlying_type_ignore_byref(MonoType *type) {
  if (type->type == MONO_TYPE_VALUETYPE &&
      m_class_is_enumtype(type->data.klass))
  ...
  if (type->type == MONO_TYPE_GENERICINST &&
    ...
    return mono_class_enum_basetype_internal(
        type->data.generic_class->container_class);
  return type;
}

guint32 ves_icall_RuntimeTypeHandle_type_is_assignable_from(
    MonoReflectionTypeHandle ref_type, MonoReflectionTypeHandle ref_c,
    MonoError *error) {
  ...
  if (type->byref) {
    ...
    if (mono_type_is_primitive(t)) {
      return mono_type_is_primitive(ot) &&
             m_class_get_instance_size(klass) ==
                 m_class_get_instance_size(klassc);
    } else if (t->type == MONO_TYPE_VAR || t->type == MONO_TYPE_MVAR) {
      return t->type == ot->type &&
             t->data.generic_param->num == ot->data.generic_param->num;
...
MonoBoolean ves_icall_RuntimeTypeHandle_is_subclass_of(MonoType *childType,
                                                       MonoType *baseType) {
  ERROR_DECL(error);
...
guint32 ves_icall_RuntimeTypeHandle_IsInstanceOfType(
    MonoReflectionTypeHandle ref_type, MonoObjectHandle obj, MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *klass = mono_class_from_mono_type_internal(type);
...
guint32
ves_icall_RuntimeTypeHandle_GetAttributes(MonoReflectionTypeHandle ref_type,
                                          MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);

#ifdef ENABLE_NETCORE
  if (type->byref || type->type == MONO_TYPE_PTR ||
...
ves_icall_System_Reflection_FieldInfo_get_marshal_info(
    MonoReflectionFieldHandle field_h, MonoError *error) {
  ...
  for (int i = 0; i < info->num_fields; ++i) {
    if (info->fields[i].field == field) {
      if (!info->fields[i].mspec)
        ...
        return mono_reflection_marshal_as_attribute_from_marshal_spec(
            domain, klass, info->fields[i].mspec, error);
      }
    }
  }
...
ves_icall_System_Reflection_FieldInfo_internal_from_handle_type(
    MonoClassField *handle, MonoType *type, MonoError *error) {
  MonoClass *klass;
  ...
  if (!type) {
    ...
    gboolean found =
        klass == handle->parent || mono_class_has_parent(klass, handle->parent);
...
ves_icall_System_Reflection_EventInfo_internal_from_handle_type(
    MonoEvent *handle, MonoType *type, MonoError *error) {
  MonoClass *klass;
...
ves_icall_System_Reflection_RuntimePropertyInfo_internal_from_handle_type(
    MonoProperty *handle, MonoType *type, MonoError *error) {
  MonoClass *klass;
  ...
  return mono_property_get_object_handle(mono_domain_get(), klass, handle,
                                         error);
}

MonoArrayHandle ves_icall_System_Reflection_FieldInfo_GetTypeModifiers(
    MonoReflectionFieldHandle field_h, MonoBoolean optional, MonoError *error) {
  MonoClassField *field = MONO_HANDLE_GETVAL(field_h, field);

  MonoType *type = mono_field_get_type_checked(field, error);
  return_val_if_nok(error, NULL_HANDLE_ARRAY);

  return type_array_from_modifiers(m_class_get_image(field->parent), type,
                                   optional, error);
}

int ves_icall_get_method_attributes(MonoMethod *method) {
  return method->flags;
}

void ves_icall_get_method_info(MonoMethod *method, MonoMethodInfo *info,
                               MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  ...
  MonoReflectionTypeHandle rt = mono_type_get_object_handle(
      domain, m_class_get_byval_arg(method->klass), error);
  return_if_nok(error);
...
MonoArrayHandle ves_icall_System_Reflection_MonoMethodInfo_get_parameter_info(
    MonoMethod *method, MonoReflectionMethodHandle member, MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  ...
  if (!MONO_HANDLE_IS_NULL(reftype))
    klass =
        mono_class_from_mono_type_internal(MONO_HANDLE_GETVAL(reftype, type));
...
ves_icall_System_MonoMethodInfo_get_retval_marshal(MonoMethod *method,
                                                   MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  MonoReflectionMarshalAsAttributeHandle res =
      MONO_HANDLE_NEW(MonoReflectionMarshalAsAttribute, NULL);

  MonoMarshalSpec **mspecs =
      g_new(MonoMarshalSpec *,
            mono_method_signature_internal(method)->param_count + 1);
  mono_method_get_marshal_info(method, mspecs);

  if (mspecs[0]) {
    MONO_HANDLE_ASSIGN(res,
                       mono_reflection_marshal_as_attribute_from_marshal_spec(
                           domain, method->klass, mspecs[0], error));
    goto_if_nok(error, leave);
  }
...
gint32
ves_icall_RuntimeFieldInfo_GetFieldOffset(MonoReflectionFieldHandle field,
                                          MonoError *error) {
...
MonoReflectionTypeHandle ves_icall_RuntimeFieldInfo_GetParentType(
    MonoReflectionFieldHandle field, MonoBoolean declaring, MonoError *error) {
  ...
  return mono_type_get_object_handle(domain, m_class_get_byval_arg(parent),
                                     error);
}

MonoObjectHandle ves_icall_RuntimeFieldInfo_GetValueInternal(
    MonoReflectionFieldHandle field_handle, MonoObjectHandle obj_handle,
    MonoError *error) {
  MonoReflectionField *const field = MONO_HANDLE_RAW(field_handle);
  MonoClass *fklass = field->klass;
  MonoClassField *cf = field->field;

  if (mono_asmctx_get_kind(&m_class_get_image(fklass)->assembly->context) ==
      MONO_ASMCTX_REFONLY) {
    mono_error_set_invalid_operation(
        error, "It is illegal to get the value on a field on a type loaded "
               "using the ReflectionOnly methods.");
    return NULL_HANDLE;
  }
  ...
  if (G_UNLIKELY(obj != NULL &&
                 mono_class_is_transparent_proxy(mono_object_class(obj)))) {
...
#endif
    result = mono_field_get_value_object_checked(mono_object_domain(field), cf,
                                                 obj, error);

  return MONO_HANDLE_NEW(MonoObject, result);
}

void ves_icall_RuntimeFieldInfo_SetValueInternal(
    MonoReflectionFieldHandle field, MonoObjectHandle obj,
    MonoObjectHandle value, MonoError *error) {
  MonoClassField *cf = MONO_HANDLE_GETVAL(field, field);

  MonoClass *field_klass = MONO_HANDLE_GETVAL(field, klass);
  if (mono_asmctx_get_kind(
          &m_class_get_image(field_klass)->assembly->context) ==
      MONO_ASMCTX_REFONLY) {
    mono_error_set_invalid_operation(
        error, "It is illegal to set the value on a field on a type loaded "
               "using the ReflectionOnly methods.");
    return;
  }
...
#ifndef DISABLE_REMOTING
  if (G_UNLIKELY(!MONO_HANDLE_IS_NULL(obj) &&
                 mono_class_is_transparent_proxy(mono_handle_class(obj)))) {
    /* We get here if someone used a
  ...
  if (!type->byref) {
    ...
    case MONO_TYPE_GENERICINST: {
      ...
      if (mono_class_is_nullable(mono_class_from_mono_type_internal(type))) {
         ...
         */
        MonoObjectHandle nullable =
            mono_object_new_handle(mono_domain_get(), nklass, error);
        return_if_nok(error);

        MonoGCHandle nullable_gchandle = 0;
        guint8 *nval = (guint8 *)mono_object_handle_pin_unbox(
            nullable, &nullable_gchandle);
    ...
    default:
      g_error("type 0x%x not handled in "
              "ves_icall_FieldInfo_SetValueInternal",
              type->type);
      return;
    }
  }
  ...
  if (type->attrs & FIELD_ATTRIBUTE_STATIC) {
    MonoVTable *vtable =
        mono_class_vtable_checked(MONO_HANDLE_DOMAIN(field), cf->parent, error);
    goto_if_nok(error, leave);
    ...
    if (isref)
      mono_field_static_set_value_internal(
          vtable, cf,
          MONO_HANDLE_RAW(value)); /* FIXME make mono_field_static_set_value
                                      work with handles for value */
    else
    ...
    if (isref)
      ...
      mono_field_set_value_internal(
          MONO_HANDLE_RAW(obj), cf,
          v); /* FIXME: make mono_field_set_value take a handle for obj */
  }
leave:
  if (value_gchandle)
...
static MonoObjectHandle typed_reference_to_object(MonoTypedRef *tref,
                                                  MonoError *error) {
  ...
  if (MONO_TYPE_IS_REFERENCE(tref->type)) {
    ...
    /* Boxed as UIntPtr */
    result = mono_value_box_handle(mono_domain_get(), mono_get_uintptr_class(),
                                   tref->value, error);
  } else {
    result = mono_value_box_handle(mono_domain_get(), tref->klass, tref->value,
                                   error);
  }
  HANDLE_FUNCTION_RETURN_REF(MonoObject, result);
}

MonoObjectHandle ves_icall_System_RuntimeFieldHandle_GetValueDirect(
    MonoReflectionFieldHandle field_h, MonoReflectionTypeHandle field_type_h,
    MonoTypedRef *obj, MonoReflectionTypeHandle context_type_h,
    MonoError *error) {
  MonoClassField *field = MONO_HANDLE_GETVAL(field_h, field);
  ...
  if (!MONO_TYPE_ISSTRUCT(m_class_get_byval_arg(field->parent))) {
    ...
    return MONO_HANDLE_NEW(MonoObject, *(MonoObject **)((guint8 *)obj->value +
                                                        field->offset -
                                                        sizeof(MonoObject)));
  } else {
    return mono_value_box_handle(
        mono_domain_get(), klass,
        (guint8 *)obj->value + field->offset - sizeof(MonoObject), error);
  }
}

void ves_icall_System_RuntimeFieldHandle_SetValueDirect(
    MonoReflectionFieldHandle field_h, MonoReflectionTypeHandle field_type_h,
    MonoTypedRef *obj, MonoObjectHandle value_h,
    MonoReflectionTypeHandle context_type_h, MonoError *error) {
  ...
  if (!MONO_TYPE_ISSTRUCT(m_class_get_byval_arg(f->parent))) {
    ...
    return_if_nok(error);
    ves_icall_RuntimeFieldInfo_SetValueInternal(field_h, objHandle, value_h,
                                                error);
  } else if (MONO_TYPE_IS_REFERENCE(f->type)) {
    mono_copy_value(f->type,
                    (guint8 *)obj->value + m_field_get_offset(f) -
                        sizeof(MonoObject),
                    MONO_HANDLE_RAW(value_h), FALSE);
  } else {
    ...
    mono_copy_value(f->type,
                    (guint8 *)obj->value + m_field_get_offset(f) -
                        sizeof(MonoObject),
                    mono_object_handle_pin_unbox(value_h, &gchandle), FALSE);
    mono_gchandle_free_internal(gchandle);
  }
}

MonoObjectHandle
ves_icall_RuntimeFieldInfo_GetRawConstantValue(MonoReflectionFieldHandle rfield,
                                               MonoError *error) {
  ...
  MonoType *t;
  MonoStringHandle string_handle =
      MONO_HANDLE_NEW(MonoString, NULL); // FIXME? Not always needed.
  ...
  case MONO_TYPE_R8: {
    ...
    (void)mono_get_constant_value_from_blob(domain, def_type, def_value, v,
                                            string_handle, error);
    goto_if_nok(error, return_null);
    break;
  }
  case MONO_TYPE_STRING:
  case MONO_TYPE_CLASS:
    (void)mono_get_constant_value_from_blob(domain, def_type, def_value, &o,
                                            string_handle, error);
    goto_if_nok(error, return_null);
    o_handle = MONO_HANDLE_NEW(MonoObject, o);
    break;
...
MonoReflectionTypeHandle
ves_icall_RuntimeFieldInfo_ResolveType(MonoReflectionFieldHandle ref_field,
                                       MonoError *error) {
...
void ves_icall_RuntimePropertyInfo_get_property_info(
    MonoReflectionPropertyHandle property, MonoPropertyInfo *info,
    PInfo req_info, MonoError *error) {
  ...
  if ((req_info & PInfo_ReflectedType) != 0) {
    ...
    MonoReflectionTypeHandle rt = mono_type_get_object_handle(
        domain, m_class_get_byval_arg(klass), error);
    return_if_nok(error);
  ...
  if ((req_info & PInfo_DeclaringType) != 0) {
    MonoReflectionTypeHandle rt = mono_type_get_object_handle(
        domain, m_class_get_byval_arg(pproperty->parent), error);
    return_if_nok(error);
  ...
  if ((req_info & PInfo_Name) != 0) {
    MonoStringHandle name =
        mono_string_new_handle(domain, pproperty->name, error);
    return_if_nok(error);
  ...
  if ((req_info & PInfo_GetMethod) != 0) {
    ...
    if (pproperty->get &&
        (((pproperty->get->flags & METHOD_ATTRIBUTE_MEMBER_ACCESS_MASK) !=
          METHOD_ATTRIBUTE_PRIVATE) ||
         pproperty->get->klass == property_klass)) {
      rm = mono_method_get_object_handle(domain, pproperty->get, property_klass,
                                         error);
      return_if_nok(error);
    } else {
  ...
  if ((req_info & PInfo_SetMethod) != 0) {
    ...
    MonoReflectionMethodHandle rm;
    if (pproperty->set &&
        (((pproperty->set->flags & METHOD_ATTRIBUTE_MEMBER_ACCESS_MASK) !=
          METHOD_ATTRIBUTE_PRIVATE) ||
         pproperty->set->klass == property_klass)) {
      rm = mono_method_get_object_handle(domain, pproperty->set, property_klass,
                                         error);
      return_if_nok(error);
    } else {
      rm = MONO_HANDLE_NEW(MonoReflectionMethod, NULL);
  ...
  /*
   * There may be other methods defined for properties, though, it seems they
   * are not exposed in the reflection API
   */
}

static gboolean add_event_other_methods_to_array(MonoDomain *domain,
                                                 MonoMethod *m,
                                                 MonoArrayHandle dest, int i,
                                                 MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoReflectionMethodHandle rm =
      mono_method_get_object_handle(domain, m, NULL, error);
  goto_if_nok(error, leave);
...
void ves_icall_RuntimeEventInfo_get_event_info(
    MonoReflectionMonoEventHandle ref_event, MonoEventInfo *info,
    MonoError *error) {
  ...
  MonoReflectionTypeHandle rt =
      mono_type_get_object_handle(domain, m_class_get_byval_arg(klass), error);
  return_if_nok(error);
  MONO_STRUCT_SETREF_INTERNAL(info, reflected_type, MONO_HANDLE_RAW(rt));

  rt = mono_type_get_object_handle(domain, m_class_get_byval_arg(event->parent),
                                   error);
  return_if_nok(error);
  MONO_STRUCT_SETREF_INTERNAL(info, declaring_type, MONO_HANDLE_RAW(rt));
  ...
  if (event->other) {
    ...
    while (event->other[n])
      n++;
    MonoArrayHandle info_arr = mono_array_new_handle(
        domain, mono_defaults.method_info_class, n, error);
    return_if_nok(error);

    MONO_STRUCT_SETREF_INTERNAL(info, other_methods, MONO_HANDLE_RAW(info_arr));

    for (i = 0; i < n; i++)
      if (!add_event_other_methods_to_array(domain, event->other[i], info_arr,
                                            i, error))
        return;
  }
#endif
...
static void collect_interfaces(MonoClass *klass, GHashTable *ifaces,
                               MonoError *error) {
  int i;
  MonoClass *ic;
...
static void fill_iface_array(gpointer key, gpointer value, gpointer user_data) {
  HANDLE_FUNCTION_ENTER();
  ...
  if (data->context && mono_class_is_ginst(ic) &&
      mono_class_get_generic_class(ic)->context.class_inst->is_open) {
    inflated = ret =
        mono_class_inflate_generic_type_checked(ret, data->context, error);
    goto_if_nok(error, leave);
  }
...
static guint get_interfaces_hash(gconstpointer v1) {
...
MonoArrayHandle
ves_icall_RuntimeType_GetInterfaces(MonoReflectionTypeHandle ref_type,
                                    MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *klass = mono_class_from_mono_type_internal(type);
  ...
  if (mono_class_is_ginst(klass) &&
      mono_class_get_generic_class(klass)->context.class_inst->is_open) {
  ...
  if (len == 0) {
    ...
    if (!domain->empty_types) {
      domain->empty_types = mono_array_new_cached(
          domain, mono_defaults.runtimetype_class, 0, error);
      goto_if_nok(error, fail);
    }
  ...
  data.iface_array = MONO_HANDLE_NEW(
      MonoArray, mono_array_new_cached(domain, mono_defaults.runtimetype_class,
                                       len, error));
  goto_if_nok(error, fail);
...
static gboolean set_interface_map_data_method_object(
    MonoDomain *domain, MonoMethod *method, MonoClass *iclass, int ioffset,
    MonoClass *klass, MonoArrayHandle targets, MonoArrayHandle methods, int i,
    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoReflectionMethodHandle member =
      mono_method_get_object_handle(domain, method, iclass, error);
  goto_if_nok(error, leave);

  MONO_HANDLE_ARRAY_SETREF(methods, i, member);

  MONO_HANDLE_ASSIGN(member, mono_method_get_object_handle(
                                 domain, m_class_get_vtable(klass)[i + ioffset],
                                 klass, error));
  goto_if_nok(error, leave);
...
void ves_icall_RuntimeType_GetInterfaceMapData(
    MonoReflectionTypeHandle ref_type, MonoReflectionTypeHandle ref_iface,
    MonoArrayHandleOut targets, MonoArrayHandleOut methods, MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *klass = mono_class_from_mono_type_internal(type);
  ...
  int ioffset =
      mono_class_interface_offset_with_variance(klass, iclass, &variance_used);
  if (ioffset == -1)
    return;

  int len = mono_class_num_methods(iclass);
  MonoDomain *domain = MONO_HANDLE_DOMAIN(ref_type);
  MonoArrayHandle targets_arr = mono_array_new_handle(
      domain, mono_defaults.method_info_class, len, error);
  return_if_nok(error);
  MONO_HANDLE_ASSIGN(targets, targets_arr);

  MonoArrayHandle methods_arr = mono_array_new_handle(
      domain, mono_defaults.method_info_class, len, error);
  return_if_nok(error);
  ...
  while ((method = mono_class_get_methods(iclass, &iter))) {
    if (!set_interface_map_data_method_object(
            domain, method, iclass, ioffset, klass, targets, methods, i, error))
      return;
    i++;
  }
}

void ves_icall_RuntimeType_GetPacking(MonoReflectionTypeHandle ref_type,
                                      guint32 *packing, guint32 *size,
                                      MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *klass = mono_class_from_mono_type_internal(type);
  ...
  if (image_is_dynamic(m_class_get_image(klass))) {
    MonoReflectionTypeBuilderHandle tb =
        MONO_HANDLE_CAST(MonoReflectionTypeBuilder, ref_type);
    *packing = MONO_HANDLE_GETVAL(tb, packing_size);
    *size = MONO_HANDLE_GETVAL(tb, class_size);
  } else {
    mono_metadata_packing_from_typedef(
        m_class_get_image(klass), m_class_get_type_token(klass), packing, size);
  }
}

MonoReflectionTypeHandle
ves_icall_RuntimeTypeHandle_GetElementType(MonoReflectionTypeHandle ref_type,
                                           MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(ref_type);
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);

  if (!type->byref && type->type == MONO_TYPE_SZARRAY) {
    return mono_type_get_object_handle(
        domain, m_class_get_byval_arg(type->data.klass), error);
  }
  ...
  if (type->byref)
    ...
    return mono_type_get_object_handle(
        domain, m_class_get_byval_arg(m_class_get_element_class(klass)), error);
...
MonoReflectionTypeHandle
ves_icall_RuntimeTypeHandle_GetBaseType(MonoReflectionTypeHandle ref_type,
                                        MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(ref_type);
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  ...
  return mono_type_get_object_handle(
      domain, m_class_get_byval_arg(m_class_get_parent(klass)), error);
}

guint32
ves_icall_RuntimeTypeHandle_GetCorElementType(MonoReflectionTypeHandle ref_type,
                                              MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
...
MonoBoolean
ves_icall_RuntimeTypeHandle_HasReferences(MonoReflectionTypeHandle ref_type,
                                          MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *klass;
...
MonoBoolean
ves_icall_RuntimeTypeHandle_IsByRefLike(MonoReflectionTypeHandle ref_type,
                                        MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
...
MonoBoolean
ves_icall_RuntimeTypeHandle_IsComObject(MonoReflectionTypeHandle ref_type,
                                        MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *klass = mono_class_from_mono_type_internal(type);
  mono_class_init_checked(klass, error);
...
guint32 ves_icall_reflection_get_token(MonoObjectHandle obj, MonoError *error) {
  error_init(error);
  return mono_reflection_get_token_checked(obj, error);
}

MonoReflectionModuleHandle
ves_icall_RuntimeTypeHandle_GetModule(MonoReflectionTypeHandle type,
                                      MonoError *error) {
...
ves_icall_RuntimeTypeHandle_GetAssembly(MonoReflectionTypeHandle type,
                                        MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  MonoType *t = MONO_HANDLE_GETVAL(type, type);
  MonoClass *klass = mono_class_from_mono_type_internal(t);
  return mono_assembly_get_object_handle(
      domain, m_class_get_image(klass)->assembly, error);
}

MonoReflectionTypeHandle
ves_icall_RuntimeType_get_DeclaringType(MonoReflectionTypeHandle ref_type,
                                        MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *klass;
...
MonoStringHandle
ves_icall_RuntimeType_get_Name(MonoReflectionTypeHandle reftype,
                               MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  MonoType *type = MONO_HANDLE_RAW(reftype)->type;
  MonoClass *klass = mono_class_from_mono_type_internal(type);
  // FIXME: this should be escaped in some scenarios with
  // mono_identifier_escape_type_name_chars Determining exactly when to do so is
  // fairly difficult, so for now we don't bother to avoid regressions
...
MonoStringHandle
ves_icall_RuntimeType_get_Namespace(MonoReflectionTypeHandle type,
                                    MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  ...
  char *escaped =
      mono_identifier_escape_type_name_chars(m_class_get_name_space(klass));
...
gint32
ves_icall_RuntimeTypeHandle_GetArrayRank(MonoReflectionTypeHandle ref_type,
                                         MonoError *error) {
  error_init(error);
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
...
static MonoArrayHandle create_type_array(MonoDomain *domain,
                                         MonoBoolean runtimeTypeArray,
                                         int count, MonoError *error) {
  return mono_array_new_handle(domain,
                               runtimeTypeArray
                                   ? mono_defaults.runtimetype_class
                                   : mono_defaults.systemtype_class,
                               count, error);
}

static gboolean set_type_object_in_array(MonoDomain *domain, MonoType *type,
                                         MonoArrayHandle dest, int i,
                                         MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoReflectionTypeHandle rt =
      mono_type_get_object_handle(domain, type, error);
  goto_if_nok(error, leave);
...
MonoArrayHandle
ves_icall_RuntimeType_GetGenericArguments(MonoReflectionTypeHandle ref_type,
                                          MonoBoolean runtimeTypeArray,
                                          MonoError *error) {
  error_init(error);
  MonoDomain *domain = MONO_HANDLE_DOMAIN(ref_type);
  ...
  if (mono_class_is_gtd(klass)) {
    ...
    MONO_HANDLE_ASSIGN(res, create_type_array(domain, runtimeTypeArray,
                                              container->type_argc, error));
    goto_if_nok(error, leave);
    for (int i = 0; i < container->type_argc; ++i) {
      MonoClass *pklass = mono_class_create_generic_parameter(
          mono_generic_container_get_param(container, i));

      if (!set_type_object_in_array(domain, m_class_get_byval_arg(pklass), res,
                                    i, error))
        goto leave;
    }
    ...
    MonoGenericInst *inst =
        mono_class_get_generic_class(klass)->context.class_inst;
    MONO_HANDLE_ASSIGN(res, create_type_array(domain, runtimeTypeArray,
                                              inst->type_argc, error));
    goto_if_nok(error, leave);
...
MonoBoolean ves_icall_RuntimeTypeHandle_IsGenericTypeDefinition(
    MonoReflectionTypeHandle ref_type, MonoError *error) {
  error_init(error);
...
ves_icall_RuntimeTypeHandle_GetGenericTypeDefinition_impl(
  ...
  if (mono_class_is_ginst(klass)) {
    MonoClass *generic_class =
        mono_class_get_generic_class(klass)->container_class;

    MonoGCHandle ref_info_handle =
    ...
    if (m_class_was_typebuilder(generic_class) && ref_info_handle) {
      ...
      MonoDomain *domain = MONO_HANDLE_DOMAIN(ref_type);
      MONO_HANDLE_ASSIGN(
          ret, mono_type_get_object_handle(
                   domain, m_class_get_byval_arg(generic_class), error));
    }
  }
leave:
  return ret;
...
MonoReflectionTypeHandle
ves_icall_RuntimeType_MakeGenericType(MonoReflectionTypeHandle reftype,
  ...
  MonoType *geninst =
      mono_reflection_bind_generic_parameters(reftype, count, types, error);
  g_free(types);
  if (!geninst) {
    return MONO_HANDLE_CAST(MonoReflectionType, NULL_HANDLE);
  }
  ...
  if (mono_class_is_ginst(klass) &&
    ...
    mono_error_set_argument(error, "typeArguments",
...
MonoBoolean
ves_icall_RuntimeTypeHandle_HasInstantiation(MonoReflectionTypeHandle ref_type,
                                             MonoError *error) {
  error_init(error);
  MonoClass *klass;
...
gint32 ves_icall_RuntimeType_GetGenericParameterPosition(
    MonoReflectionTypeHandle ref_type, MonoError *error) {
  error_init(error);
...
MonoGenericParamInfo *ves_icall_RuntimeTypeHandle_GetGenericParameterInfo(
    MonoReflectionTypeHandle ref_type, MonoError *error) {
  error_init(error);
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
...
MonoBoolean
ves_icall_RuntimeTypeHandle_IsGenericVariable(MonoReflectionTypeHandle ref_type,
                                              MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  return is_generic_parameter(type);
}

MonoReflectionMethodHandle ves_icall_RuntimeType_GetCorrespondingInflatedMethod(
...
ves_icall_RuntimeType_get_DeclaringMethod(MonoReflectionTypeHandle ref_type,
                                          MonoError *error) {
  error_init(error);
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  ...
  if (type->byref ||
      (type->type != MONO_TYPE_MVAR && type->type != MONO_TYPE_VAR)) {
    mono_error_set_invalid_operation(
        error, "DeclaringMethod can only be used on generic arguments");
    goto leave;
  }
  if (type->type == MONO_TYPE_VAR)
    goto leave;
  ...
  MONO_HANDLE_ASSIGN(
      ret, mono_method_get_object_handle(domain, method, method->klass, error));
leave:
  return ret;
}
...
ves_icall_System_RuntimeType_IsTypeExportedToWindowsRuntime(MonoError *error) {
  ...
  mono_error_set_not_implemented(
      error, "%s", "System.RuntimeType.IsTypeExportedToWindowsRuntime");
  return FALSE;
}
...
ves_icall_System_RuntimeType_IsWindowsRuntimeObjectType(MonoError *error) {
  error_init(error);
  mono_error_set_not_implemented(
      error, "%s", "System.RuntimeType.IsWindowsRuntimeObjectType");
  return FALSE;
}
#endif /* ENABLE_NETCORE */

void ves_icall_RuntimeMethodInfo_GetPInvoke(
    MonoReflectionMethodHandle ref_method, int *flags,
    MonoStringHandleOut entry_point, MonoStringHandleOut dll_name,
    MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  ...
  if (image_is_dynamic(image)) {
    MonoReflectionMethodAux *method_aux =
        (MonoReflectionMethodAux *)g_hash_table_lookup(
            ((MonoDynamicImage *)image)->method_aux_hash, method);
    if (method_aux) {
    ...
    if (!import || !scope) {
      mono_error_set_argument(
          error, "method",
          "System.Refleciton.Emit method with invalid pinvoke information");
      return;
    }
  } else {
    if (piinfo->implmap_idx) {
      mono_metadata_decode_row(im, piinfo->implmap_idx - 1, im_cols,
                               MONO_IMPLMAP_SIZE);

      piinfo->piflags = im_cols[MONO_IMPLMAP_FLAGS];
      import = mono_metadata_string_heap(image, im_cols[MONO_IMPLMAP_NAME]);
      scope_token = mono_metadata_decode_row_col(
          mr, im_cols[MONO_IMPLMAP_SCOPE] - 1, MONO_MODULEREF_NAME);
      scope = mono_metadata_string_heap(image, scope_token);
    }
  }

  *flags = piinfo->piflags;
  MONO_HANDLE_ASSIGN(entry_point,
                     mono_string_new_handle(domain, import, error));
  return_if_nok(error);
...
ves_icall_RuntimeMethodInfo_GetGenericMethodDefinition(
    MonoReflectionMethodHandle ref_method, MonoError *error) {
  error_init(error);
  MonoMethod *method = MONO_HANDLE_GETVAL(ref_method, method);
  ...
  if (image_is_dynamic(m_class_get_image(method->klass))) {
    MonoDynamicImage *image =
        (MonoDynamicImage *)m_class_get_image(method->klass);

    /*
     * FIXME: Why is this stuff needed at all ? Why can't the code below work
     * for the dynamic case as well ?
     */
    mono_image_lock((MonoImage *)image);
    MonoReflectionMethodHandle res = MONO_HANDLE_NEW(
        MonoReflectionMethod, (MonoReflectionMethod *)mono_g_hash_table_lookup(
                                  image->generic_def_objects, imethod));
  ...
  if (imethod->context.class_inst) {
    ...
    if (mono_class_get_context(klass)) {
      result = mono_class_inflate_generic_method_full_checked(
          result, klass, mono_class_get_context(klass), error);
      return_val_if_nok(error,
                        MONO_HANDLE_CAST(MonoReflectionMethod, NULL_HANDLE));
    }
  }

  return mono_method_get_object_handle(MONO_HANDLE_DOMAIN(ref_method), result,
                                       NULL, error);
}

#ifdef ENABLE_NETCORE
static GENERATE_TRY_GET_CLASS_WITH_CACHE(
    stream, "System.IO", "Stream") static int io_stream_begin_read_slot = -1;
...
static void init_io_stream_slots(void) {
...
MonoBoolean
ves_icall_System_IO_Stream_HasOverriddenBeginEndRead(MonoObjectHandle stream,
                                                     MonoError *error) {
  ...
  // slots can still be -1 and it means Linker removed the methods from the base
  // class (Stream) in this case we can safely assume the methods are not
  // overridden otherwise - check vtable
  MonoMethod **curr_klass_vtable = m_class_get_vtable(curr_klass);
  gboolean begin_read_is_overriden =
      io_stream_begin_read_slot != -1 &&
      curr_klass_vtable[io_stream_begin_read_slot]->klass != base_klass;
  gboolean end_read_is_overriden =
      io_stream_end_read_slot != -1 &&
      curr_klass_vtable[io_stream_end_read_slot]->klass != base_klass;
...
MonoBoolean
ves_icall_System_IO_Stream_HasOverriddenBeginEndWrite(MonoObjectHandle stream,
                                                      MonoError *error) {
  ...
  MonoMethod **curr_klass_vtable = m_class_get_vtable(curr_klass);
  gboolean begin_write_is_overriden =
      curr_klass_vtable[io_stream_begin_write_slot]->klass != base_klass;
  gboolean end_write_is_overriden =
      curr_klass_vtable[io_stream_end_write_slot]->klass != base_klass;
...
MonoBoolean ves_icall_RuntimeMethodInfo_get_IsGenericMethod(
    MonoReflectionMethodHandle ref_method, MonoError *erro) {
  MonoMethod *method = MONO_HANDLE_GETVAL(ref_method, method);
  return mono_method_signature_internal(method)->generic_param_count != 0;
}

MonoBoolean ves_icall_RuntimeMethodInfo_get_IsGenericMethodDefinition(
    MonoReflectionMethodHandle ref_method, MonoError *Error) {
  MonoMethod *method = MONO_HANDLE_GETVAL(ref_method, method);
  return method->is_generic;
}

static gboolean set_array_generic_argument_handle_inflated(
    MonoDomain *domain, MonoGenericInst *inst, int i, MonoArrayHandle arr,
    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoReflectionTypeHandle rt =
      mono_type_get_object_handle(domain, inst->type_argv[i], error);
  goto_if_nok(error, leave);
...
static gboolean set_array_generic_argument_handle_gparam(
    MonoDomain *domain, MonoGenericContainer *container, int i,
    MonoArrayHandle arr, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoGenericParam *param = mono_generic_container_get_param(container, i);
  MonoClass *pklass = mono_class_create_generic_parameter(param);
  MonoReflectionTypeHandle rt =
      mono_type_get_object_handle(domain, m_class_get_byval_arg(pklass), error);
  goto_if_nok(error, leave);
  MONO_HANDLE_ARRAY_SETREF(arr, i, rt);
leave:
  HANDLE_FUNCTION_RETURN_VAL(is_ok(error));
...
MonoArrayHandle ves_icall_RuntimeMethodInfo_GetGenericArguments(
    MonoReflectionMethodHandle ref_method, MonoError *error) {
  error_init(error);
  ...
  if (method->is_inflated) {
    ...
    if (inst) {
      ...
      MonoArrayHandle res = mono_array_new_handle(
          domain, mono_defaults.systemtype_class, count, error);
      return_val_if_nok(error, NULL_HANDLE_ARRAY);

      for (int i = 0; i < count; i++) {
        if (!set_array_generic_argument_handle_inflated(domain, inst, i, res,
                                                        error))
          break;
      }
      return_val_if_nok(error, NULL_HANDLE_ARRAY);
  ...
  for (int i = 0; i < count; i++) {
    if (!set_array_generic_argument_handle_gparam(domain, container, i, res,
                                                  error))
      break;
  }
  return_val_if_nok(error, NULL_HANDLE_ARRAY);
...
MonoObjectHandle ves_icall_InternalInvoke(
    MonoReflectionMethodHandle method_handle, MonoObjectHandle this_arg_handle,
    MonoArrayHandle params_handle, MonoExceptionHandleOut exception_out,
    MonoError *error) {
  ...
  *MONO_HANDLE_REF(exception_out) = NULL;
  ...
  if (!(m->flags & METHOD_ATTRIBUTE_STATIC)) {
    if (!mono_class_vtable_checked(mono_object_domain(method), m->klass,
                                   error)) {
    ...
    if (this_arg) {
      if (!mono_object_isinst_checked(this_arg, m->klass, error)) {
        ...
        msg = g_strdup_printf(
            "Object of type '%s' doesn't match target type '%s'", this_name,
            target_name);
        exception = mono_exception_from_name_msg(
            mono_defaults.corlib, "System.Reflection", "TargetException", msg);
        goto return_null;
      }
      ...
      exception = mono_exception_from_name_msg(
          mono_defaults.corlib, "System.Reflection", "TargetException",
          "Non-static method requires a target.");
      goto return_null;
    }
  }

  if ((m->klass != NULL && m_class_is_byreflike(m->klass)) ||
      m_class_is_byreflike(mono_class_from_mono_type_internal(sig->ret))) {
    exception = mono_exception_from_name_msg(
        mono_defaults.corlib, "System", "NotSupportedException",
  ...
  if (sig->ret->byref) {
    MonoType *ret_byval =
        m_class_get_byval_arg(mono_class_from_mono_type_internal(sig->ret));
    if (ret_byval->type == MONO_TYPE_VOID) {
      exception = mono_exception_from_name_msg(
          mono_defaults.corlib, "System", "NotSupportedException",
          "ByRef to void return values are not supported in reflection "
          "invocation");
      goto return_null;
    }
    if (m_class_is_byreflike(mono_class_from_mono_type_internal(ret_byval))) {
      exception = mono_exception_from_name_msg(
          mono_defaults.corlib, "System", "NotSupportedException",
          "Cannot invoke method returning ByRef to ByRefLike type via "
          "reflection");
      goto return_null;
    }
  }
  ...
  if (mono_class_is_abstract(m->klass) && !strcmp(m->name, ".ctor") &&
      !this_arg) {
    exception = mono_exception_from_name_msg(
        mono_defaults.corlib, "System.Reflection", "TargetException",
  ...
  if (image_is_dynamic(image) && !((MonoDynamicImage *)image)->run) {
    exception = mono_get_exception_not_supported(
  ...
  if (m_class_get_rank(m->klass) && !strcmp(m->name, ".ctor")) {
    ...
    for (i = 0; i < pcount; ++i)
      lengths[i] =
          *(int32_t *)((char *)mono_array_get_internal(params, gpointer, i) +
                       sizeof(MonoObject));

    if (m_class_get_rank(m->klass) == 1 && sig->param_count == 2 &&
        m_class_get_rank(m_class_get_element_class(m->klass))) {
      /* This is a ctor for jagged arrays. MS creates an array of arrays. */
      arr = mono_array_new_full_checked(mono_object_domain(params), m->klass,
                                        lengths, NULL, error);
      goto_if_nok(error, return_null);
      ...
      for (i = 0; i < mono_array_length_internal(arr); ++i) {
        MonoArray *subarray = mono_array_new_full_checked(
            mono_object_domain(params), m_class_get_element_class(m->klass),
            &lengths[1], NULL, error);
        goto_if_nok(error, return_null);
    ...
    if (m_class_get_rank(m->klass) == pcount) {
      ...
      intptr_t *const lower_bounds =
          (intptr_t *)g_alloca(sizeof(intptr_t) * pcount);

      for (i = 0; i < pcount / 2; ++i) {
        lower_bounds[i] = *(int32_t *)((char *)mono_array_get_internal(
                                           params, gpointer, (i * 2)) +
                                       sizeof(MonoObject));
        lengths[i] = *(int32_t *)((char *)mono_array_get_internal(
                                      params, gpointer, (i * 2) + 1) +
                                  sizeof(MonoObject));
      }

      arr = mono_array_new_full_checked(mono_object_domain(params), m->klass,
                                        lengths, lower_bounds, error);
      goto_if_nok(error, return_null);
      goto exit;
    }
  }
...
exit:
  if (exception) {
    ...
    mono_gc_wbarrier_generic_store_internal(MONO_HANDLE_REF(exception_out),
  ...
  return result ? MONO_HANDLE_NEW(MonoObject, result)
                : arr ? MONO_HANDLE_NEW(MonoObject, (MonoObject *)arr)
                      : NULL_HANDLE;
}

#ifndef DISABLE_REMOTING
static void internal_execute_field_getter(MonoDomain *domain,
                                          MonoObject *this_arg,
                                          MonoArray *params,
                                          MonoArrayHandleOut outArgs,
                                          MonoError *error) {
  MonoArray *out_args;
  ...
  // FIXME Refactor/inline internal_execute_field_setter and
  // internal_execute_field_getter.
  ...
  do {
    ...
    if (field) {
      ...
      if (m_class_is_valuetype(field_klass)) {
        result = mono_value_box_checked(
            domain, field_klass, (char *)this_arg + field->offset, error);
        return_if_nok(error);
      } else
        result =
            (MonoObject *)*((gpointer *)((char *)this_arg + field->offset));

      MONO_HANDLE_NEW(MonoObject, result);

      out_args =
          mono_array_new_checked(domain, mono_defaults.object_class, 1, error);
      return_if_nok(error);
      MONO_HANDLE_NEW(MonoArray, out_args); // FIXME? overkill?
      mono_gc_wbarrier_generic_store_internal(MONO_HANDLE_REF(outArgs),
                                              (MonoObject *)out_args);
...
static void internal_execute_field_setter(MonoDomain *domain,
                                          MonoObject *this_arg,
                                          MonoArray *params,
                                          MonoArrayHandleOut outArgs,
                                          MonoError *error) {
  ...
  do {
    ...
    if (field) {
      ...
      MonoClass *field_klass = mono_class_from_mono_type_internal(field->type);
      MonoObject *val =
          (MonoObject *)mono_array_get_internal(params, gpointer, 2);
      MONO_HANDLE_NEW(MonoObject, val);

      if (m_class_is_valuetype(field_klass)) {
        size = mono_type_size(field->type, &align);
        g_assert(size == mono_class_value_size(field_klass, NULL));
        mono_gc_wbarrier_value_copy_internal((char *)this_arg + field->offset,
                                             (char *)val + sizeof(MonoObject),
                                             1, field_klass);
      } else {
        mono_gc_wbarrier_set_field_internal(
            this_arg, (char *)this_arg + field->offset, val);
      }

      out_args =
          mono_array_new_checked(domain, mono_defaults.object_class, 0, error);
      return_if_nok(error);
      MONO_HANDLE_NEW(MonoArray, out_args); // FIXME? overkill?
      mono_gc_wbarrier_generic_store_internal(MONO_HANDLE_REF(outArgs),
                                              (MonoObject *)out_args);
...
ves_icall_InternalExecute(MonoReflectionMethodHandle method_handle,
                          MonoObjectHandle this_arg_handle,
                          MonoArrayHandle params_handle,
                          MonoArrayHandleOut outArgs, MonoError *error) {
  ...
  out_args = mono_array_new_checked(domain, mono_defaults.object_class,
                                    outarg_count, error);
  ...
  result = mono_runtime_invoke_array_checked(method->method, this_arg, params,
                                             error);
  return_val_if_nok(error, null_handle);
...
static guint64 read_enum_value(const char *mem, int type) {
  switch (type) {
  case MONO_TYPE_BOOLEAN:
  case MONO_TYPE_U1:
    return *(guint8 *)mem;
...
static void write_enum_value(void *mem, int type, guint64 value) {
  switch (type) {
  case MONO_TYPE_U1:
  case MONO_TYPE_I1:
  case MONO_TYPE_BOOLEAN: {
...
MonoObjectHandle
ves_icall_System_Enum_ToObject(MonoReflectionTypeHandle enumType, guint64 value,
                               MonoError *error) {
  MonoDomain *domain;
  MonoClass *enumc;
  ...
  enumc =
      mono_class_from_mono_type_internal(MONO_HANDLE_GETVAL(enumType, type));
...
MonoBoolean ves_icall_System_Enum_InternalHasFlag(MonoObjectHandle a,
                                                  MonoObjectHandle b,
                                                  MonoError *error) {
...
#ifndef ENABLE_NETCORE
MonoObjectHandle ves_icall_System_Enum_get_value(MonoObjectHandle ehandle,
                                                 MonoError *error) {
  MonoObjectHandle resultHandle;
  MonoClass *enumc;
  int size;
  ...
  enumc = mono_class_from_mono_type_internal(
      mono_class_enum_basetype_internal(mono_handle_class(ehandle)));

  resultHandle =
      mono_object_new_handle(MONO_HANDLE_DOMAIN(ehandle), enumc, error);
  goto_if_nok(error, return_null);
  size = mono_class_value_size(enumc, NULL);

  memcpy(mono_handle_unbox_unsafe(resultHandle),
         mono_handle_unbox_unsafe(ehandle), size);
...
MonoReflectionTypeHandle
ves_icall_System_Enum_get_underlying_type(MonoReflectionTypeHandle type,
                                          MonoError *error) {
  MonoType *etype;
  MonoClass *klass;
  ...
  if (!etype) {
    mono_error_set_argument(error, "enumType",
...
int ves_icall_System_Enum_InternalGetCorElementType(
    MonoObjectHandle this_handle, MonoError *error) {
...
#ifndef ENABLE_NETCORE
int ves_icall_System_Enum_compare_value_to(MonoObjectHandle enumHandle,
                                           MonoObjectHandle otherHandle,
                                           MonoError *error) {
  if (MONO_HANDLE_IS_NULL(otherHandle))
    return 1;

  if (MONO_HANDLE_GETVAL(enumHandle, vtable)->klass !=
      MONO_HANDLE_GETVAL(otherHandle, vtable)->klass)
    return 2;

  gpointer tdata = mono_handle_unbox_unsafe(enumHandle);
  gpointer odata = mono_handle_unbox_unsafe(otherHandle);
  MonoType *basetype = mono_class_enum_basetype_internal(
      MONO_HANDLE_GETVAL(enumHandle, vtable)->klass);
  g_assert(basetype);

#define COMPARE_ENUM_VALUES(ENUM_TYPE)                                         \
  do {                                                                         \
...
#ifndef ENABLE_NETCORE
int ves_icall_System_Enum_get_hashcode(MonoObjectHandle enumHandle,
                                       MonoError *error) {
...
static void get_enum_field(MonoDomain *domain, MonoArrayHandle names,
                           MonoArrayHandle values, int base_type,
                           MonoClassField *field, guint *j,
                           guint64 *previous_value, gboolean *sorted,
                           MonoError *error) {
  error_init(error);
  HANDLE_FUNCTION_ENTER();
  guint64 field_value;
...
MonoBoolean ves_icall_System_Enum_GetEnumValuesAndNames(
    MonoReflectionTypeHandle type, MonoArrayHandleOut values,
    MonoArrayHandleOut names, MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(type);
  MonoClass *enumc =
      mono_class_from_mono_type_internal(MONO_HANDLE_RAW(type)->type);
  guint j = 0, nvalues;
  gpointer iter;
  MonoClassField *field;
  int base_type;
  ...
  nvalues =
      mono_class_num_fields(enumc) > 0 ? mono_class_num_fields(enumc) - 1 : 0;
  MONO_HANDLE_ASSIGN(names,
                     mono_array_new_handle(domain, mono_defaults.string_class,
                                           nvalues, error));
  return_val_if_nok(error, FALSE);
  MONO_HANDLE_ASSIGN(values,
                     mono_array_new_handle(domain, mono_defaults.uint64_class,
                                           nvalues, error));
  return_val_if_nok(error, FALSE);

  iter = NULL;
  while ((field = mono_class_get_fields_internal(enumc, &iter))) {
    get_enum_field(domain, names, values, base_type, field, &j, &previous_value,
                   &sorted, error);
    if (!is_ok(error))
      break;
  }
...
GPtrArray *
ves_icall_RuntimeType_GetFields_native(MonoReflectionTypeHandle ref_type,
                                       char *utf8_name, guint32 bflags,
                                       guint32 mlisttype, MonoError *error) {
  error_init(error);
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  ...
  compare_func =
      ((bflags & BFLAGS_IgnoreCase) || (mlisttype == MLISTTYPE_CaseInsensitive))
  ...
  while ((field = mono_class_get_fields_lazy(klass, &iter))) {
    ...
    if ((flags & FIELD_ATTRIBUTE_FIELD_ACCESS_MASK) == FIELD_ATTRIBUTE_PUBLIC) {
      if (bflags & BFLAGS_Public)
        match++;
    } else if ((klass == startklass) ||
               (flags & FIELD_ATTRIBUTE_FIELD_ACCESS_MASK) !=
    ...
    if (((mlisttype != MLISTTYPE_All) && (utf8_name != NULL)) &&
        compare_func(mono_field_get_name(field), utf8_name))
      continue;
...
static gboolean method_nonpublic(MonoMethod *method, gboolean start_klass) {
...
GPtrArray *mono_class_get_methods_by_name(MonoClass *klass, const char *name,
                                          guint32 bflags, guint32 mlisttype,
  ...
  if (m_class_is_delegate(klass) && klass != mono_defaults.delegate_class &&
      klass != mono_defaults.multicastdelegate_class && name &&
      !strcmp(name, "Invoke") &&
      (bflags == (BFLAGS_Public | BFLAGS_Static | BFLAGS_Instance))) {
  ...
  if (is_generic_parameter(m_class_get_byval_arg(klass)))
    ...
    nslots = MONO_CLASS_IS_INTERFACE_INTERNAL(klass)
...
handle_parent:
  ...
  while ((method = mono_class_get_methods(klass, &iter))) {
    ...
    if (!allow_ctors && method->name[0] == '.' &&
        (strcmp(method->name, ".ctor") == 0 ||
         strcmp(method->name, ".cctor") == 0))
      continue;
    if ((method->flags & METHOD_ATTRIBUTE_MEMBER_ACCESS_MASK) ==
        METHOD_ATTRIBUTE_PUBLIC) {
      if (bflags & BFLAGS_Public)
        match++;
    } else if ((bflags & BFLAGS_NonPublic) &&
               method_nonpublic(method, (klass == startklass))) {
      match++;
    }
    if (!match)
      continue;
...
GPtrArray *ves_icall_RuntimeType_GetMethodsByName_native(
    MonoReflectionTypeHandle ref_type, const char *mname, guint32 bflags,
    guint32 mlisttype, MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  ...
  return mono_class_get_methods_by_name(klass, mname, bflags, mlisttype, FALSE,
                                        error);
}

GPtrArray *
ves_icall_RuntimeType_GetConstructors_native(MonoReflectionTypeHandle ref_type,
                                             guint32 bflags, MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  if (type->byref) {
    return g_ptr_array_new();
  }
...
static guint property_hash(gconstpointer data) {
  MonoProperty *prop = (MonoProperty *)data;

  return g_str_hash(prop->name);
}

static gboolean property_accessor_override(MonoMethod *method1,
                                           MonoMethod *method2) {
  if (method1->slot != -1 && method1->slot == method2->slot)
    return TRUE;

  if (mono_class_get_generic_type_definition(method1->klass) ==
      mono_class_get_generic_type_definition(method2->klass)) {
  ...
  return mono_metadata_signature_equal(mono_method_signature_internal(method1),
                                       mono_method_signature_internal(method2));
}

static gboolean property_equal(MonoProperty *prop1, MonoProperty *prop2) {
  ...
  */
  if (prop1->get && prop2->get &&
      !property_accessor_override(prop1->get, prop2->get))
    return FALSE;

  if (prop1->set && prop2->set &&
      !property_accessor_override(prop1->set, prop2->set))
    return FALSE;

  return TRUE;
}

static gboolean property_accessor_nonpublic(MonoMethod *accessor,
...
GPtrArray *ves_icall_RuntimeType_GetPropertiesByName_native(
    MonoReflectionTypeHandle ref_type, gchar *propname, guint32 bflags,
    guint32 mlisttype, MonoError *error) {
#if ENABLE_NETCORE
  // Fetch non-public properties as well because they can hide public properties
  // with the same name in base classes
  ...
  int (*compare_func)(const char *s1, const char *s2) =
      (mlisttype == MLISTTYPE_CaseInsensitive) ? mono_utf8_strcasecmp : strcmp;

  GPtrArray *res_array =
      g_ptr_array_sized_new(8); /*This the average for ASP.NET types*/

  GHashTable *properties =
      g_hash_table_new(property_hash, (GEqualFunc)property_equal);
...
handle_parent:
  ...
  while ((prop = mono_class_get_properties(klass, &iter))) {
    ...
    if ((prop->get &&
         ((prop->get->flags & METHOD_ATTRIBUTE_MEMBER_ACCESS_MASK) ==
          METHOD_ATTRIBUTE_PUBLIC)) ||
        (prop->set &&
         ((prop->set->flags & METHOD_ATTRIBUTE_MEMBER_ACCESS_MASK) ==
          METHOD_ATTRIBUTE_PUBLIC))) {
      if (bflags & BFLAGS_Public)
        match++;
    } else if (bflags & BFLAGS_NonPublic) {
    ...
    if ((mlisttype != MLISTTYPE_All) && (propname != NULL) &&
        compare_func(propname, prop->name))
      continue;
...
static guint event_hash(gconstpointer data) {
  MonoEvent *event = (MonoEvent *)data;

  return g_str_hash(event->name);
}

static gboolean event_equal(MonoEvent *event1, MonoEvent *event2) {
  // Events are hide-by-name
  return g_str_equal(event1->name, event2->name);
}

GPtrArray *
ves_icall_RuntimeType_GetEvents_native(MonoReflectionTypeHandle ref_type,
                                       char *utf8_name, guint32 mlisttype,
                                       MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
...
handle_parent:
  ...
  while ((event = mono_class_get_events(klass, &iter))) {
    ...
    if ((mlisttype != MLISTTYPE_All) && (utf8_name != NULL) &&
        compare_func(event->name, utf8_name))
      continue;
...
GPtrArray *ves_icall_RuntimeType_GetNestedTypes_native(
    MonoReflectionTypeHandle ref_type, char *str, guint32 bflags,
    guint32 mlisttype, MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  ...
  while ((nested = mono_class_get_nested_types(klass, &iter))) {
    int match = 0;
    if ((mono_class_get_flags(nested) & TYPE_ATTRIBUTE_VISIBILITY_MASK) ==
    ...
    if ((mlisttype != MLISTTYPE_All) && (str != NULL) &&
        compare_func(m_class_get_name(nested), str))
      continue;
...
static MonoType *get_type_from_module_builder_module(
    MonoAssemblyLoadContext *alc, MonoArrayHandle modules, int i,
    MonoTypeNameParse *info, MonoBoolean ignoreCase, gboolean *type_resolve,
    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoType *type = NULL;
  MonoReflectionModuleBuilderHandle mb =
      MONO_HANDLE_NEW(MonoReflectionModuleBuilder, NULL);
  MONO_HANDLE_ARRAY_GETREF(mb, modules, i);
  MonoDynamicImage *dynamic_image = MONO_HANDLE_GETVAL(mb, dynamic_image);
  type = mono_reflection_get_type_checked(
      alc, &dynamic_image->image, &dynamic_image->image, info, ignoreCase,
      FALSE, type_resolve, error);
  HANDLE_FUNCTION_RETURN_VAL(type);
}

static MonoType *get_type_from_module_builder_loaded_modules(
    MonoAssemblyLoadContext *alc, MonoArrayHandle loaded_modules, int i,
    MonoTypeNameParse *info, MonoBoolean ignoreCase, gboolean *type_resolve,
    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoType *type = NULL;
  ...
  type = mono_reflection_get_type_checked(alc, image, image, info, ignoreCase,
                                          FALSE, type_resolve, error);
  HANDLE_FUNCTION_RETURN_VAL(type);
}

MonoReflectionTypeHandle ves_icall_System_Reflection_Assembly_InternalGetType(
    MonoReflectionAssemblyHandle assembly_h, MonoReflectionModuleHandle module,
    MonoStringHandle name, MonoBoolean throwOnError, MonoBoolean ignoreCase,
    MonoError *error) {
  ...
  if (info.assembly.name) {
    ...
    if (throwOnError) {
      ...
      if (mono_defaults.generic_ilist_class)
        mono_error_set_argument(error, NULL,
                                "Type names passed to Assembly.GetType() must "
                                "not specify an assembly.");
      else
        mono_error_set_type_load_name(error, g_strdup(""), g_strdup(""),
                                      "Type names passed to Assembly.GetType() "
                                      "must not specify an assembly.");
      goto fail;
    }
    return MONO_HANDLE_CAST(MonoReflectionType, NULL_HANDLE);
  }
  ...
  if (!MONO_HANDLE_IS_NULL(module)) {
    ...
    if (image) {
      type = mono_reflection_get_type_checked(
          alc, image, image, &info, ignoreCase, FALSE, &type_resolve, error);
      if (!is_ok(error)) {
        g_free(str);
        mono_reflection_free_type_info(&info);
        goto fail;
    ...
    if (assembly_is_dynamic(assembly)) {
      ...
      MonoReflectionAssemblyBuilderHandle abuilder =
          MONO_HANDLE_NEW(MonoReflectionAssemblyBuilder, NULL);
      ...
      if (!MONO_HANDLE_IS_NULL(modules)) {
        ...
        for (i = 0; i < n; ++i) {
          type = get_type_from_module_builder_module(
              alc, modules, i, &info, ignoreCase, &type_resolve, error);
          if (!is_ok(error)) {
            g_free(str);
            mono_reflection_free_type_info(&info);
            goto fail;
      ...
      if (!type && !MONO_HANDLE_IS_NULL(loaded_modules)) {
        ...
        for (i = 0; i < n; ++i) {
          type = get_type_from_module_builder_loaded_modules(
              alc, loaded_modules, i, &info, ignoreCase, &type_resolve, error);
      ...
      type = mono_reflection_get_type_checked(
          alc, assembly->image, assembly->image, &info, ignoreCase, FALSE,
          &type_resolve, error);
      if (!is_ok(error)) {
        g_free(str);
        mono_reflection_free_type_info(&info);
  ...
  if (type->type == MONO_TYPE_CLASS) {
    ...
    if (throwOnError && mono_class_has_failure(klass)) {
      /* report SecurityException (or others) that occured when loading the
       * assembly */
      mono_error_set_for_class_failure(error, klass);
      goto fail;
    }
  ...
  /* g_print ("got it\n"); */
  return mono_type_get_object_handle(MONO_HANDLE_DOMAIN(assembly_h), type,
                                     error);
fail:
...
static gboolean replace_shadow_path(MonoDomain *domain, gchar *dirname,
                                    gchar **filename) {
  gchar *content;
  ...
  if (mono_is_shadow_copy_enabled(domain, dirname)) {
    shadow_ini_file =
        g_build_filename(dirname, "__AssemblyInfo__.ini", (const char *)NULL);
    content = NULL;
...
MonoStringHandle ves_icall_System_Reflection_RuntimeAssembly_get_code_base(
    MonoReflectionAssemblyHandle assembly, MonoBoolean escaped,
    MonoError *error) {
  ...
  if (g_path_is_absolute(mass->image->name)) {
    ...
    absolute =
        g_build_filename(mass->basedir, mass->image->name, (const char *)NULL);
...
ves_icall_System_Reflection_RuntimeAssembly_get_global_assembly_cache(
    MonoReflectionAssemblyHandle assembly, MonoError *error) {
  MonoAssembly *mass = MONO_HANDLE_GETVAL(assembly, assembly);
...
ves_icall_System_Reflection_Assembly_load_with_partial_name(
    MonoStringHandle mname, MonoObjectHandle evidence, MonoError *error) {
  gchar *name;
  MonoImageOpenStatus status;
  MonoReflectionAssemblyHandle result =
      MONO_HANDLE_CAST(MonoReflectionAssembly, NULL_HANDLE);

  name = mono_string_handle_to_utf8(mname, error);
  goto_if_nok(error, leave);
  MonoAssembly *res;
  res = mono_assembly_load_with_partial_name_internal(
      name, mono_domain_default_alc(mono_domain_get()), &status);
...
MonoStringHandle ves_icall_System_Reflection_RuntimeAssembly_get_location(
    MonoReflectionAssemblyHandle refassembly, MonoError *error) {
  ...
  return mono_string_new_handle(domain, image_name != NULL ? image_name : "",
                                error);
}

#ifndef ENABLE_NETCORE
MonoBoolean ves_icall_System_Reflection_RuntimeAssembly_get_ReflectionOnly(
    MonoReflectionAssemblyHandle assembly_h, MonoError *error) {
  MonoAssembly *assembly = MONO_HANDLE_GETVAL(assembly_h, assembly);
...
ves_icall_System_Reflection_RuntimeAssembly_get_EntryPoint(
  ...
  MONO_HANDLE_ASSIGN(
      res, mono_method_get_object_handle(domain, method, NULL, error));
leave:
  return res;
}
...
static gboolean add_manifest_resource_name_to_array(MonoDomain *domain,
                                                    MonoImage *image,
                                                    MonoTableInfo *table, int i,
                                                    MonoArrayHandle dest,
                                                    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  const char *val = mono_metadata_string_heap(
      image, mono_metadata_decode_row_col(table, i, MONO_MANIFEST_NAME));
...
ves_icall_System_Reflection_RuntimeAssembly_GetManifestResourceNames(
  ...
  MonoArrayHandle result = mono_array_new_handle(
      domain, mono_defaults.string_class, table->rows, error);
  goto_if_nok(error, fail);
  int i;

  for (i = 0; i < table->rows; ++i) {
    if (!add_manifest_resource_name_to_array(domain, assembly->image, table, i,
                                             result, error))
      goto fail;
  }
  return result;
...
#ifndef ENABLE_NETCORE
MonoBoolean ves_icall_System_Reflection_RuntimeAssembly_GetAotIdInternal(
    MonoArrayHandle guid_h, MonoError *error) {
  ...
  if (!aotid) {
    ...
    MONO_ENTER_NO_SAFEPOINTS;
    guint8 *data = (guint8 *)mono_array_addr_with_size_internal(
        MONO_HANDLE_RAW(guid_h), 1, 0);
...
create_referenced_assembly_name(MonoDomain *domain, MonoImage *image,
                                MonoTableInfo *t, int i, MonoError *error) {
...
GPtrArray *ves_icall_System_Reflection_Assembly_InternalGetReferencedAssemblies(
    MonoReflectionAssemblyHandle assembly, MonoError *error) {
  error_init(error);
  MonoDomain *domain = MONO_HANDLE_DOMAIN(assembly);
  ...
  for (int i = 0; i < count; i++) {
    MonoAssemblyName *aname =
        create_referenced_assembly_name(domain, image, t, i, error);
    if (!is_ok(error))
      break;
...
static char *g_concat_dir_and_file(const char *dir, const char *file) {
...
void *ves_icall_System_Reflection_RuntimeAssembly_GetManifestResourceInternal(
    MonoReflectionAssemblyHandle assembly_h, MonoStringHandle name,
    gint32 *size, MonoReflectionModuleHandleOut ref_module, MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(assembly_h);
  MonoAssembly *assembly = MONO_HANDLE_GETVAL(assembly_h, assembly);
  MonoTableInfo *table = &assembly->image->tables[MONO_TABLE_MANIFESTRESOURCE];
  guint32 i;
  ...
  if (impl) {
    ...
    module = mono_image_load_file_for_image_checked(assembly->image, file_idx,
                                                    error);
  ...
  MonoReflectionModuleHandle rm =
      mono_module_get_object_handle(domain, module, error);
  return_val_if_nok(error, NULL);
  MONO_HANDLE_ASSIGN(ref_module, rm);

  return (void *)mono_image_get_resource(module, cols[MONO_MANIFEST_OFFSET],
                                         (guint32 *)size);
}

static gboolean get_manifest_resource_info_internal(
    MonoReflectionAssemblyHandle assembly_h, MonoStringHandle name,
    MonoManifestResourceInfoHandle info, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MonoDomain *domain = MONO_HANDLE_DOMAIN(assembly_h);
  MonoAssembly *assembly = MONO_HANDLE_GETVAL(assembly_h, assembly);
  MonoTableInfo *table = &assembly->image->tables[MONO_TABLE_MANIFESTRESOURCE];
  ...
  if (!cols[MONO_MANIFEST_IMPLEMENTATION]) {
    MONO_HANDLE_SETVAL(info, location, guint32,
    ...
    case MONO_IMPLEMENTATION_FILE:
      ...
      val =
          mono_metadata_string_heap(assembly->image, file_cols[MONO_FILE_NAME]);
      MONO_HANDLE_SET(info, filename,
                      mono_string_new_handle(domain, val, error));
    ...
    case MONO_IMPLEMENTATION_ASSEMBLYREF:
      ...
      if (assembly->image->references[i - 1] == REFERENCE_MISSING) {
        mono_error_set_file_not_found(
            error, NULL, "Assembly %d referenced from assembly %s not found ",
            i - 1, assembly->image->name);
        goto leave;
      }
      MonoReflectionAssemblyHandle assm_obj;
      assm_obj = mono_assembly_get_object_handle(
          mono_domain_get(), assembly->image->references[i - 1], error);
      goto_if_nok(error, leave);
...
ves_icall_System_Reflection_RuntimeAssembly_GetManifestResourceInfoInternal(
    MonoReflectionAssemblyHandle assembly_h, MonoStringHandle name,
    MonoManifestResourceInfoHandle info_h, MonoError *error) {
  return get_manifest_resource_info_internal(assembly_h, name, info_h, error);
}

static gboolean add_filename_to_files_array(MonoDomain *domain,
  ...
  HANDLE_FUNCTION_ENTER();
  const char *val = mono_metadata_string_heap(
      assembly->image, mono_metadata_decode_row_col(table, i, MONO_FILE_NAME));
...
MonoObjectHandle ves_icall_System_Reflection_RuntimeAssembly_GetFilesInternal(
    MonoReflectionAssemblyHandle assembly_h, MonoStringHandle name,
    MonoBoolean resource_modules, MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(assembly_h);
  MonoAssembly *assembly = MONO_HANDLE_GETVAL(assembly_h, assembly);
  ...
  if (!MONO_HANDLE_IS_NULL(name)) {
    ...
    for (i = 0; i < table->rows; ++i) {
      const char *val = mono_metadata_string_heap(
          assembly->image,
          mono_metadata_decode_row_col(table, i, MONO_FILE_NAME));
      if (strcmp(val, n) == 0) {
        g_free(n);
  ...
  for (i = 0; i < table->rows; ++i) {
    if (resource_modules ||
        !(mono_metadata_decode_row_col(table, i, MONO_FILE_FLAGS) &
  ...
  result =
      mono_array_new_handle(domain, mono_defaults.string_class, count, error);
  goto_if_nok(error, fail);
  ...
  for (i = 0; i < table->rows; ++i) {
    if (resource_modules ||
        !(mono_metadata_decode_row_col(table, i, MONO_FILE_FLAGS) &
          FILE_CONTAINS_NO_METADATA)) {
      if (!add_filename_to_files_array(domain, assembly, table, i, result,
                                       count, error))
        goto fail;
      count++;
    }
...
static gboolean add_module_to_modules_array(MonoDomain *domain,
                                            MonoArrayHandle dest, int *dest_idx,
                                            MonoImage *module,
                                            MonoError *error) {
  HANDLE_FUNCTION_ENTER();
...
static gboolean add_file_to_modules_array(MonoDomain *domain,
                                          MonoArrayHandle dest, int dest_idx,
                                          MonoImage *image,
                                          MonoTableInfo *table, int table_idx,
                                          MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  if (cols[MONO_FILE_FLAGS] & FILE_CONTAINS_NO_METADATA) {
    MonoReflectionModuleHandle rm =
        mono_module_file_get_object_handle(domain, image, table_idx, error);
    goto_if_nok(error, leave);
    MONO_HANDLE_ARRAY_SETREF(dest, dest_idx, rm);
  } else {
    MonoImage *m =
        mono_image_load_file_for_image_checked(image, table_idx + 1, error);
    goto_if_nok(error, leave);
    if (!m) {
      const char *filename =
          mono_metadata_string_heap(image, cols[MONO_FILE_NAME]);
      gboolean refonly = FALSE;
      if (image->assembly)
        refonly = mono_asmctx_get_kind(&image->assembly->context) ==
                  MONO_ASMCTX_REFONLY;
      mono_error_set_simple_file_not_found(error, filename, refonly);
      goto leave;
    }
    MonoReflectionModuleHandle rm =
        mono_module_get_object_handle(domain, m, error);
    goto_if_nok(error, leave);
    MONO_HANDLE_ARRAY_SETREF(dest, dest_idx, rm);
  }
...
MonoArrayHandle ves_icall_System_Reflection_RuntimeAssembly_GetModulesInternal(
    MonoReflectionAssemblyHandle assembly_h, MonoError *error) {
  error_init(error);
  MonoDomain *domain = mono_domain_get();
  MonoAssembly *assembly = MONO_HANDLE_GETVAL(assembly_h, assembly);
  ...
  MonoArrayHandle res = mono_array_new_handle(
      domain, klass, 1 + real_module_count + file_count, error);
  goto_if_nok(error, fail);
...
MonoReflectionMethodHandle ves_icall_GetCurrentMethod(MonoError *error) {
  MonoMethod *m = mono_method_get_last_managed();

  if (!m) {
    mono_error_set_not_supported(
        error, "Stack walks are not supported on this platform.");
    return MONO_HANDLE_CAST(MonoReflectionMethod, NULL_HANDLE);
  }
...
static MonoMethod *mono_method_get_equivalent_method(MonoMethod *method,
                                                     MonoClass *klass) {
  int offset = -1, i;
  if (method->is_inflated &&
    ...
    if (mono_class_is_ginst(klass))
      ...
      ctx.class_inst =
          mono_class_get_generic_container(klass)->context.class_inst;
    result = mono_class_inflate_generic_method_full_checked(inflated->declaring,
                                                            klass, &ctx, error);
...
ves_icall_System_Reflection_RuntimeMethodInfo_GetMethodFromHandleInternalType_native(
    MonoMethod *method, MonoType *type, MonoBoolean generic_check,
    MonoError *error) {
  MonoClass *klass;
  if (type && generic_check) {
    klass = mono_class_from_mono_type_internal(type);
    if (mono_class_get_generic_type_definition(method->klass) !=
...
ves_icall_System_Reflection_Assembly_GetExecutingAssembly(
    MonoStackCrawlMark *stack_mark, MonoError *error) {
  MonoAssembly *assembly;
  assembly = mono_runtime_get_caller_from_stack_mark(stack_mark);
  g_assert(assembly);
...
ves_icall_System_Reflection_Assembly_GetExecutingAssembly(MonoError *error) {
  ...
  g_assert(dest);
  return mono_assembly_get_object_handle(
      mono_domain_get(), m_class_get_image(dest->klass)->assembly, error);
}
#endif
...
MonoStringHandle ves_icall_System_RuntimeType_getFullName(
    MonoReflectionTypeHandle object, MonoBoolean full_name,
  ...
  if (full_name)
    format = assembly_qualified ? MONO_TYPE_NAME_FORMAT_ASSEMBLY_QUALIFIED
...
#ifndef ENABLE_NETCORE
int ves_icall_RuntimeType_get_core_clr_security_level(
    MonoReflectionTypeHandle rfield, MonoError *error) {
...
int ves_icall_RuntimeFieldInfo_get_core_clr_security_level(
    MonoReflectionFieldHandle rfield, MonoError *error) {
  MonoClassField *field = MONO_HANDLE_GETVAL(rfield, field);
  return mono_security_core_clr_field_level(field, TRUE);
}

int ves_icall_RuntimeMethodInfo_get_core_clr_security_level(
    MonoReflectionMethodHandle rfield, MonoError *error) {
  MonoMethod *method = MONO_HANDLE_GETVAL(rfield, method);
  return mono_security_core_clr_method_level(method, TRUE);
}

MonoStringHandle ves_icall_System_Reflection_RuntimeAssembly_get_fullname(
    MonoReflectionAssemblyHandle assembly, MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(assembly);
  MonoAssembly *mass = MONO_HANDLE_GETVAL(assembly, assembly);
  gchar *name;
...
void ves_icall_System_Reflection_Assembly_InternalGetAssemblyName(
    MonoStringHandle fname, MonoAssemblyName *name,
    MonoStringHandleOut normalized_codebase, MonoError *error) {
  char *filename;
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
             "InternalGetAssemblyName (\"%s\")", filename);
  ...
  if (!image) {
    if (status == MONO_IMAGE_IMAGE_INVALID)
      mono_error_set_bad_image_by_name(error, filename, "Invalid Image: %s",
                                       filename);
    else
  ...
  if (!res) {
    ...
    g_free(filename);
    mono_error_set_argument(error, "assemblyFile",
  ...
  MONO_HANDLE_ASSIGN(
      normalized_codebase,
      mono_string_new_handle(mono_domain_get(), codebase, error));
  g_free(codebase);
...
#ifndef ENABLE_NETCORE
MonoBoolean ves_icall_System_Reflection_RuntimeAssembly_LoadPermissions(
    MonoReflectionAssemblyHandle assembly_h, char **minimum, guint32 *minLength,
    char **optional, guint32 *optLength, char **refused, guint32 *refLength,
    MonoError *error) {
  MonoAssembly *assembly = MONO_HANDLE_GETVAL(assembly_h, assembly);
  ...
  if (mono_declsec_get_assembly_action(assembly, SECURITY_ACTION_REQMIN,
                                       &entry)) {
    *minimum = entry.blob;
  ...
  if (mono_declsec_get_assembly_action(assembly, SECURITY_ACTION_REQOPT,
                                       &entry)) {
    *optional = entry.blob;
  ...
  if (mono_declsec_get_assembly_action(assembly, SECURITY_ACTION_REQREFUSE,
                                       &entry)) {
    *refused = entry.blob;
...
static gboolean mono_module_type_is_visible(MonoTableInfo *tdef,
                                            MonoImage *image, int type) {
  ...
  } while ((type = mono_metadata_token_index(
                mono_metadata_nested_in_typedef(image, type))));

  return TRUE;
}

static void image_get_type(MonoDomain *domain, MonoImage *image,
                           MonoTableInfo *tdef, int table_idx, int count,
                           MonoArrayHandle res, MonoArrayHandle exceptions,
                           MonoBoolean exportedOnly, MonoError *error) {
  error_init(error);
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(klass_error);
  MonoClass *klass = mono_class_get_checked(
      image, table_idx | MONO_TOKEN_TYPE_DEF, klass_error);
  ...
  if (klass) {
    ...
    MonoExceptionHandle ex =
        mono_error_convert_to_exception_handle(klass_error);
    MONO_HANDLE_ARRAY_SETREF(exceptions, count, ex);
  }
  HANDLE_FUNCTION_RETURN();
}

static MonoArrayHandle mono_module_get_types(MonoDomain *domain,
                                             MonoImage *image,
  ...
  MonoArrayHandle res = mono_array_new_handle(
      domain, mono_defaults.runtimetype_class, count, error);
  return_val_if_nok(error, NULL_HANDLE_ARRAY);
  MONO_HANDLE_ASSIGN(
      exceptions, mono_array_new_handle(domain, mono_defaults.exception_class,
                                        count, error));
  return_val_if_nok(error, NULL_HANDLE_ARRAY);
  count = 0;
  for (i = 1; i < tdef->rows; ++i) {
    if (!exportedOnly || mono_module_type_is_visible(tdef, image, i + 1)) {
      image_get_type(domain, image, tdef, i + 1, count, res, exceptions,
                     exportedOnly, error);
      return_val_if_nok(error, NULL_HANDLE_ARRAY);
      count++;
    }
...
static void append_module_types(MonoDomain *domain, MonoArrayHandleOut res,
                                MonoArrayHandleOut exceptions, MonoImage *image,
                                MonoBoolean exportedOnly, MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  MonoArrayHandle ex2 = MONO_HANDLE_NEW(MonoArray, NULL);
  MonoArrayHandle res2 =
      mono_module_get_types(domain, image, ex2, exportedOnly, error);
  goto_if_nok(error, leave);
  ...
  if (mono_array_handle_length(res2) > 0) {
    ...
    MonoArrayHandle res3 = mono_array_new_handle(
        domain, mono_defaults.runtimetype_class, len1 + len2, error);
    goto_if_nok(error, leave);
    ...
    MonoArrayHandle ex3 = mono_array_new_handle(
        domain, mono_defaults.runtimetype_class, len1 + len2, error);
    goto_if_nok(error, leave);
...
static void set_class_failure_in_array(MonoArrayHandle exl, int i,
                                       MonoClass *klass) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(unboxed_error);
  mono_error_set_for_class_failure(unboxed_error, klass);

  MonoExceptionHandle exc = MONO_HANDLE_NEW(
      MonoException, mono_error_convert_to_exception(unboxed_error));
  MONO_HANDLE_ARRAY_SETREF(exl, i, exc);
  HANDLE_FUNCTION_RETURN();
}

static MonoArrayHandle
assembly_get_types(MonoReflectionAssemblyHandle assembly_handle,
                   MonoBoolean exportedOnly, MonoError *error) {
  ...
  MonoArrayHandle res =
      mono_module_get_types(domain, image, exceptions, exportedOnly, error);
  return_val_if_nok(error, NULL_HANDLE_ARRAY);

  /* Append data from all modules in the assembly */
  for (i = 0; i < table->rows; ++i) {
    if (!(mono_metadata_decode_row_col(table, i, MONO_FILE_FLAGS) &
          FILE_CONTAINS_NO_METADATA)) {
      MonoImage *loaded_image =
          mono_assembly_load_module_checked(image->assembly, i + 1, error);
      return_val_if_nok(error, NULL_HANDLE_ARRAY);

      if (loaded_image) {
        append_module_types(domain, res, exceptions, loaded_image, exportedOnly,
                            error);
        return_val_if_nok(error, NULL_HANDLE_ARRAY);
      }
    }
  ...
  for (i = 0; i < len; i++) {
    ...
    if (!MONO_HANDLE_IS_NULL(t)) {
      MonoClass *klass =
          mono_type_get_class_internal(MONO_HANDLE_GETVAL(t, type));
  ...
  if (list || ex_count) {
    ...
    MonoArrayHandle exl = mono_array_new_handle(
        domain, mono_defaults.exception_class, length, error);
    if (!is_ok(error)) {
      g_list_free(list);
      return NULL_HANDLE_ARRAY;
    }
...
#ifndef ENABLE_NETCORE
MonoArrayHandle ves_icall_System_Reflection_Assembly_GetTypes(
    MonoReflectionAssemblyHandle assembly_handle, MonoBoolean exportedOnly,
    MonoError *error) {
...
MonoArrayHandle ves_icall_System_Reflection_RuntimeAssembly_GetExportedTypes(
    MonoReflectionAssemblyHandle assembly_handle, MonoError *error) {
  return assembly_get_types(assembly_handle, TRUE, error);
}

static void get_top_level_forwarded_type(MonoImage *image, MonoTableInfo *table,
                                         int i, MonoArrayHandle types,
                                         MonoArrayHandle exceptions,
                                         int *aindex, int *exception_count) {
  ...
  const char *nspace =
      mono_metadata_string_heap(image, cols[MONO_EXP_TYPE_NAMESPACE]);

  g_assert((impl & MONO_IMPLEMENTATION_MASK) ==
  ...
  if (image->references[assembly_idx - 1] == REFERENCE_MISSING) {
    MonoExceptionHandle ex = MONO_HANDLE_NEW(
        MonoException, mono_get_exception_bad_image_format("Invalid image"));
  ...
  klass = mono_class_from_name_checked(
      image->references[assembly_idx - 1]->image, nspace, name, local_error);
  if (!is_ok(local_error)) {
    MonoExceptionHandle ex =
        mono_error_convert_to_exception_handle(local_error);
    MONO_HANDLE_ARRAY_SETREF(types, *aindex, NULL_HANDLE);
    MONO_HANDLE_ARRAY_SETREF(exceptions, *aindex, ex);
  ...
  rt = mono_type_get_object_handle(mono_domain_get(),
                                   m_class_get_byval_arg(klass), local_error);
  if (!is_ok(local_error)) {
    MonoExceptionHandle ex =
        mono_error_convert_to_exception_handle(local_error);
    MONO_HANDLE_ARRAY_SETREF(types, *aindex, NULL_HANDLE);
...
ves_icall_System_Reflection_RuntimeAssembly_GetTopLevelForwardedTypes(
  ...
  for (int i = 0; i < table->rows; ++i) {
    if (mono_metadata_decode_row_col(table, i, MONO_EXP_TYPE_FLAGS) &
        TYPE_ATTRIBUTE_FORWARDER)
      count++;
  }

  MonoArrayHandle types = mono_array_new_handle(
      mono_domain_get(), mono_defaults.runtimetype_class, count, error);
  return_val_if_nok(error, NULL_HANDLE_ARRAY);
  MonoArrayHandle exceptions = mono_array_new_handle(
      mono_domain_get(), mono_defaults.exception_class, count, error);
  return_val_if_nok(error, NULL_HANDLE_ARRAY);

  int aindex = 0;
  int exception_count = 0;
  for (int i = 0; i < table->rows; ++i) {
    get_top_level_forwarded_type(image, table, i, types, exceptions, &aindex,
                                 &exception_count);
  }

  if (exception_count > 0) {
    MonoExceptionHandle exc = MONO_HANDLE_NEW(MonoException, NULL);
    MONO_HANDLE_ASSIGN(exc, mono_get_exception_reflection_type_load_checked(
                                types, exceptions, error));
    return_val_if_nok(error, NULL_HANDLE_ARRAY);
    mono_error_set_exception_handle(error, exc);
    return NULL_HANDLE_ARRAY;
  }
...
void ves_icall_Mono_RuntimeMarshal_FreeAssemblyName(MonoAssemblyName *aname,
                                                    MonoBoolean free_struct) {
  mono_assembly_name_free_internal(aname);
  if (free_struct)
    g_free(aname);
}

void ves_icall_Mono_Runtime_DisableMicrosoftTelemetry(void) {
...
void ves_icall_Mono_Runtime_AnnotateMicrosoftTelemetry(const char *key,
                                                       const char *value) {
#if defined(TARGET_OSX) && !defined(DISABLE_CRASH_REPORTING)
...
void ves_icall_Mono_Runtime_EnableMicrosoftTelemetry(
    const char *appBundleID, const char *appSignature, const char *appVersion,
    const char *merpGUIPath, const char *appPath, const char *configDir,
    MonoError *error) {
#if defined(TARGET_OSX) && !defined(DISABLE_CRASH_REPORTING)
  mono_merp_enable(appBundleID, appSignature, appVersion, merpGUIPath, appPath,
                   configDir);
...
MonoStringHandle ves_icall_Mono_Runtime_ExceptionToState(
    MonoExceptionHandle exc_handle, guint64 *portable_hash_out,
    guint64 *unportable_hash_out, MonoError *error) {
...
void ves_icall_Mono_Runtime_SendMicrosoftTelemetry(const char *payload,
                                                   guint64 portable_hash,
                                                   guint64 unportable_hash,
                                                   MonoError *error) {
#if defined(TARGET_OSX) && !defined(DISABLE_CRASH_REPORTING)
  if (!mono_merp_enabled())
...
void ves_icall_Mono_Runtime_DumpTelemetry(const char *payload,
                                          guint64 portable_hash,
                                          guint64 unportable_hash,
                                          MonoError *error) {
#ifndef DISABLE_CRASH_REPORTING
...
MonoStringHandle ves_icall_Mono_Runtime_DumpStateSingle(
    guint64 *portable_hash, guint64 *unportable_hash, MonoError *error) {
  MonoStringHandle result;
...
void ves_icall_Mono_Runtime_RegisterReportingForNativeLib(
    const char *path_suffix, const char *module_name) {
#ifndef DISABLE_CRASH_REPORTING
  if (mono_get_eh_callbacks()->mono_register_native_library)
    mono_get_eh_callbacks()->mono_register_native_library(path_suffix,
                                                          module_name);
#endif
}

void ves_icall_Mono_Runtime_RegisterReportingForAllNativeLibs() {
#ifndef DISABLE_CRASH_REPORTING
...
void ves_icall_Mono_Runtime_EnableCrashReportingLog(const char *directory) {
#ifndef DISABLE_CRASH_REPORTING
  mono_summarize_set_timeline_dir(directory);
#endif
}

int ves_icall_Mono_Runtime_CheckCrashReportingLog(const char *directory,
                                                  MonoBoolean clear) {
  int ret = 0;
#ifndef DISABLE_CRASH_REPORTING
...
MonoStringHandle ves_icall_Mono_Runtime_DumpStateTotal(guint64 *portable_hash,
                                                       guint64 *unportable_hash,
                                                       MonoError *error) {
  MonoStringHandle result;
  ...
  gboolean success = mono_threads_summarize(
      ctx, &out, &hashes, TRUE, FALSE, scratch, MONO_MAX_SUMMARY_LEN_ICALL);
...
MonoBoolean ves_icall_System_Reflection_AssemblyName_ParseAssemblyName(
    const char *name, MonoAssemblyName *aname,
    MonoBoolean *is_version_defined_arg, MonoBoolean *is_token_defined_arg) {
  gboolean is_version_defined = FALSE;
  gboolean is_token_defined = FALSE;
  gboolean result = FALSE;

  result = mono_assembly_name_parse_full(name, aname, TRUE, &is_version_defined,
                                         &is_token_defined);
...
MonoReflectionTypeHandle
ves_icall_System_Reflection_RuntimeModule_GetGlobalType(MonoImage *image,
                                                        MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  MonoClass *klass;
  ...
  MonoReflectionTypeHandle ret =
      MONO_HANDLE_CAST(MonoReflectionType, NULL_HANDLE);
...
void ves_icall_System_Reflection_RuntimeModule_GetGuidInternal(
    MonoImage *image, MonoArrayHandle guid_h, MonoError *error) {
  g_assert(mono_array_handle_length(guid_h) == 16);
...
#ifndef HOST_WIN32
static inline gpointer mono_icall_module_get_hinstance(MonoImage *image) {
  return (gpointer)(-1);
}
#endif /* HOST_WIN32 */

gpointer
ves_icall_System_Reflection_RuntimeModule_GetHINSTANCE(MonoImage *image,
                                                       MonoError *error) {
  return mono_icall_module_get_hinstance(image);
}

void ves_icall_System_Reflection_RuntimeModule_GetPEKind(MonoImage *image,
                                                         gint32 *pe_kind,
                                                         gint32 *machine,
                                                         MonoError *error) {
  if (image_is_dynamic(image)) {
...
gint32
ves_icall_System_Reflection_RuntimeModule_GetMDStreamVersion(MonoImage *image,
                                                             MonoError *error) {
  return (image->md_version_major << 16) | (image->md_version_minor);
}

MonoArrayHandle
ves_icall_System_Reflection_RuntimeModule_InternalGetTypes(MonoImage *image,
                                                           MonoError *error) {
  MonoDomain *domain = mono_domain_get();

  if (!image) {
    MonoArrayHandle arr = mono_array_new_handle(
        domain, mono_defaults.runtimetype_class, 0, error);
    return arr;
  } else {
    MonoArrayHandle exceptions = MONO_HANDLE_NEW(MonoArray, NULL);
    MonoArrayHandle res =
        mono_module_get_types(domain, image, exceptions, FALSE, error);
...
static gboolean mono_memberref_is_method(MonoImage *image, guint32 token) {
  if (!image_is_dynamic(image)) {
    ...
    if (!mono_lookup_dynamic_token_class(image, token, FALSE, &handle_class,
                                         NULL, error)) {
...
static void
init_generic_context_from_args_handles(MonoGenericContext *context,
...
static MonoType *module_resolve_type_token(MonoImage *image, guint32 token,
                                           MonoArrayHandle type_args,
                                           MonoArrayHandle method_args,
                                           MonoResolveTokenError *resolve_error,
                                           MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
  ...
  *resolve_error = ResolveTokenError_Other;
  ...
  if (image_is_dynamic(image)) {
    if ((table == MONO_TABLE_TYPEDEF) || (table == MONO_TABLE_TYPEREF)) {
      ERROR_DECL(inner_error);
      klass = (MonoClass *)mono_lookup_dynamic_token_class(
          image, token, FALSE, NULL, NULL, inner_error);
    ...
    ERROR_DECL(inner_error);
    klass = (MonoClass *)mono_lookup_dynamic_token_class(
        image, token, FALSE, NULL, &context, inner_error);
    mono_error_cleanup(inner_error);
    result = klass ? m_class_get_byval_arg(klass) : NULL;
    goto leave;
  }
...
MonoType *ves_icall_System_Reflection_RuntimeModule_ResolveTypeToken(
    MonoImage *image, guint32 token, MonoArrayHandle type_args,
    MonoArrayHandle method_args, MonoResolveTokenError *resolve_error,
    MonoError *error) {
  return module_resolve_type_token(image, token, type_args, method_args,
                                   resolve_error, error);
}

static MonoMethod *module_resolve_method_token(
    MonoImage *image, guint32 token, MonoArrayHandle type_args,
    MonoArrayHandle method_args, MonoResolveTokenError *resolve_error,
    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  *resolve_error = ResolveTokenError_Other;
  ...
  if (image_is_dynamic(image)) {
    if (table == MONO_TABLE_METHOD) {
      ERROR_DECL(inner_error);
      method = (MonoMethod *)mono_lookup_dynamic_token_class(
          image, token, FALSE, NULL, NULL, inner_error);
      mono_error_cleanup(inner_error);
      goto leave;
    }
    ...
    if ((table == MONO_TABLE_MEMBERREF) &&
        !(mono_memberref_is_method(image, token))) {
      *resolve_error = ResolveTokenError_BadTable;
      goto leave;
    }
...
MonoMethod *ves_icall_System_Reflection_RuntimeModule_ResolveMethodToken(
    MonoImage *image, guint32 token, MonoArrayHandle type_args,
    MonoArrayHandle method_args, MonoResolveTokenError *resolve_error,
    MonoError *error) {
  return module_resolve_method_token(image, token, type_args, method_args,
                                     resolve_error, error);
}

MonoStringHandle ves_icall_System_Reflection_RuntimeModule_ResolveStringToken(
    MonoImage *image, guint32 token, MonoResolveTokenError *resolve_error,
    MonoError *error) {
  int index = mono_metadata_token_index(token);
  ...
  *resolve_error = ResolveTokenError_Other;
  ...
  if (image_is_dynamic(image)) {
    ...
    MonoStringHandle result = MONO_HANDLE_NEW(
        MonoString, (MonoString *)mono_lookup_dynamic_token_class(
                        image, token, FALSE, NULL, NULL, ignore_inner_error));
...
static MonoClassField *module_resolve_field_token(
    MonoImage *image, guint32 token, MonoArrayHandle type_args,
    MonoArrayHandle method_args, MonoResolveTokenError *resolve_error,
    MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ...
  *resolve_error = ResolveTokenError_Other;
  ...
  if (image_is_dynamic(image)) {
    if (table == MONO_TABLE_FIELD) {
      ERROR_DECL(inner_error);
      field = (MonoClassField *)mono_lookup_dynamic_token_class(
          image, token, FALSE, NULL, NULL, inner_error);
      mono_error_cleanup(inner_error);
      goto leave;
    }
...
MonoClassField *ves_icall_System_Reflection_RuntimeModule_ResolveFieldToken(
    MonoImage *image, guint32 token, MonoArrayHandle type_args,
    MonoArrayHandle method_args, MonoResolveTokenError *resolve_error,
    MonoError *error) {
  return module_resolve_field_token(image, token, type_args, method_args,
                                    resolve_error, error);
}

MonoObjectHandle ves_icall_System_Reflection_RuntimeModule_ResolveMemberToken(
    MonoImage *image, guint32 token, MonoArrayHandle type_args,
    MonoArrayHandle method_args, MonoResolveTokenError *error,
    MonoError *merror) {
  int table = mono_metadata_token_table(token);
  ...
  *error = ResolveTokenError_Other;
  ...
  case MONO_TABLE_TYPESPEC: {
    MonoType *t = module_resolve_type_token(image, token, type_args,
                                            method_args, error, merror);
    if (t) {
      return MONO_HANDLE_CAST(MonoObject, mono_type_get_object_handle(
                                              mono_domain_get(), t, merror));
    } else
      return NULL_HANDLE;
  }
  case MONO_TABLE_METHOD:
  case MONO_TABLE_METHODSPEC: {
    MonoMethod *m = module_resolve_method_token(image, token, type_args,
                                                method_args, error, merror);
    if (m) {
      return MONO_HANDLE_CAST(
          MonoObject, mono_method_get_object_handle(mono_domain_get(), m,
                                                    m->klass, merror));
    } else
      return NULL_HANDLE;
  }
  case MONO_TABLE_FIELD: {
    MonoClassField *f = module_resolve_field_token(image, token, type_args,
                                                   method_args, error, merror);
    if (f) {
      return MONO_HANDLE_CAST(
          MonoObject, mono_field_get_object_handle(mono_domain_get(), f->parent,
                                                   f, merror));
    } else
      return NULL_HANDLE;
  }
  case MONO_TABLE_MEMBERREF:
    if (mono_memberref_is_method(image, token)) {
      ...
      MonoClassField *f = module_resolve_field_token(
          image, token, type_args, method_args, error, merror);
      if (f) {
        return MONO_HANDLE_CAST(
            MonoObject, mono_field_get_object_handle(mono_domain_get(),
                                                     f->parent, f, merror));
      } else
        return NULL_HANDLE;
    }
...
MonoArrayHandle ves_icall_System_Reflection_RuntimeModule_ResolveSignature(
    MonoImage *image, guint32 token, MonoResolveTokenError *resolve_error,
    MonoError *error) {
  int table = mono_metadata_token_table(token);
  ...
  *resolve_error = ResolveTokenError_OutOfRange;
  ...
  sig = mono_metadata_decode_row_col(&tables[MONO_TABLE_STANDALONESIG], idx - 1,
                                     0);
  ...
  MonoArrayHandle res = mono_array_new_handle(
      mono_domain_get(), mono_defaults.byte_class, len, error);
  return_val_if_nok(error, NULL_HANDLE_ARRAY);
...
static void check_for_invalid_type(MonoClass *klass, MonoError *error) {
  char *name;
...
MonoReflectionTypeHandle
ves_icall_RuntimeType_make_array_type(MonoReflectionTypeHandle ref_type,
                                      int rank, MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  ...
  MonoDomain *domain = MONO_HANDLE_DOMAIN(ref_type);
  return mono_type_get_object_handle(domain, m_class_get_byval_arg(aklass),
                                     error);
}

MonoReflectionTypeHandle
ves_icall_RuntimeType_make_byref_type(MonoReflectionTypeHandle ref_type,
                                      MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  ...
  MonoDomain *domain = MONO_HANDLE_DOMAIN(ref_type);
  return mono_type_get_object_handle(domain, m_class_get_this_arg(klass),
                                     error);
}

MonoReflectionTypeHandle
ves_icall_RuntimeType_MakePointerType(MonoReflectionTypeHandle ref_type,
                                      MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(ref_type);
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *klass = mono_class_from_mono_type_internal(type);
  ...
  return mono_type_get_object_handle(domain, m_class_get_byval_arg(pklass),
                                     error);
}

MonoObjectHandle ves_icall_System_Delegate_CreateDelegate_internal(
    MonoReflectionTypeHandle ref_type, MonoObjectHandle target,
    MonoReflectionMethodHandle info, MonoBoolean throwOnBindFailure,
    MonoError *error) {
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  ...
  if (mono_security_core_clr_enabled()) {
    ...
    if (!mono_security_core_clr_ensure_delegate_creation(method,
                                                         security_error)) {
  ...
  if (sig->generic_param_count && method->wrapper_type == MONO_WRAPPER_NONE) {
    if (!method->is_inflated) {
      mono_error_set_argument(
          error, "method",
          " Cannot bind to the target method because its signature differs "
          "from that of the delegate type");
      return NULL_HANDLE;
    }
  }

  MonoObjectHandle delegate = mono_object_new_handle(
      MONO_HANDLE_DOMAIN(ref_type), delegate_class, error);
  return_val_if_nok(error, NULL_HANDLE);

  if (!method_is_dynamic(method) &&
      (!MONO_HANDLE_IS_NULL(target) &&
       method->flags & METHOD_ATTRIBUTE_VIRTUAL &&
       method->klass != mono_handle_class(target))) {
...
ves_icall_System_Delegate_AllocDelegateLike_internal(
    MonoDelegateHandle delegate, MonoError *error) {
  MonoClass *klass = mono_handle_class(delegate);
  g_assert(mono_class_has_parent(klass, mono_defaults.multicastdelegate_class));

  MonoMulticastDelegateHandle ret = MONO_HANDLE_CAST(
      MonoMulticastDelegate,
      mono_object_new_handle(MONO_HANDLE_DOMAIN(delegate), klass, error));
  return_val_if_nok(error,
                    MONO_HANDLE_CAST(MonoMulticastDelegate, NULL_HANDLE));

  MONO_HANDLE_SETVAL(MONO_HANDLE_CAST(MonoDelegate, ret), invoke_impl, gpointer,
...
MonoReflectionMethodHandle
ves_icall_System_Delegate_GetVirtualMethod_internal(MonoDelegateHandle delegate,
                                                    MonoError *error) {
  MonoObjectHandle delegate_target =
      MONO_HANDLE_NEW_GET(MonoObject, delegate, target);
  MonoMethod *m = mono_object_handle_get_virtual_method(
      delegate_target, MONO_HANDLE_GETVAL(delegate, method), error);
  return_val_if_nok(error, MONO_HANDLE_CAST(MonoReflectionMethod, NULL_HANDLE));
...
static gint32 mono_array_get_byte_length(MonoArrayHandle array) {
  int length;
...
#ifndef ENABLE_NETCORE
gint32 ves_icall_System_Buffer_ByteLengthInternal(MonoArrayHandle array,
                                                  MonoError *error) {
  return mono_array_get_byte_length(array);
}

void ves_icall_System_Buffer_MemcpyInternal(gpointer dest, gconstpointer src,
                                            gint32 count) {
  memcpy(dest, src, count);
}

MonoBoolean ves_icall_System_Buffer_BlockCopyInternal(
    MonoArrayHandle src, gint32 src_offset, MonoArrayHandle dest,
    gint32 dest_offset, gint32 count, MonoError *error) {
  if (count < 0) {
    mono_error_set_argument(error, "count", "is negative");
    return FALSE;
  }

  /* This is called directly from the class libraries without going through the
   * managed wrapper */
  ...
  guint8 const *const src_buf =
      (guint8 *)MONO_HANDLE_RAW(src)->vector + src_offset;
  guint8 *const dest_buf =
      (guint8 *)MONO_HANDLE_RAW(dest)->vector + dest_offset;
...
#ifndef DISABLE_REMOTING
MonoObjectHandle ves_icall_Remoting_RealProxy_GetTransparentProxy(
    MonoObjectHandle this_obj, MonoStringHandle class_name, MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(this_obj);
  MonoRealProxyHandle rp = MONO_HANDLE_CAST(MonoRealProxy, this_obj);

  MonoObjectHandle res = mono_object_new_handle(
      domain, mono_defaults.transparent_proxy_class, error);
  return_val_if_nok(error, NULL_HANDLE);
  ...
  // mono_remote_class_vtable cannot handle errors well, so force any loading
  // error to occur early
  mono_class_setup_vtable(klass);
  if (mono_class_has_failure(klass)) {
    mono_error_set_for_class_failure(error, klass);
    return NULL_HANDLE;
  ...
  MonoObjectHandle remoting_obj = mono_object_handle_isinst(
      this_obj, mono_defaults.iremotingtypeinfo_class, error);
  return_val_if_nok(error, NULL_HANDLE);
  MONO_HANDLE_SETVAL(tp, custom_type_info, MonoBoolean,
                     !MONO_HANDLE_IS_NULL(remoting_obj));

  MonoRemoteClass *remote_class =
      (MonoRemoteClass *)mono_remote_class(domain, class_name, klass, error);
  return_val_if_nok(error, NULL_HANDLE);
  MONO_HANDLE_SETVAL(tp, remote_class, MonoRemoteClass *, remote_class);

  MONO_HANDLE_SETVAL(
      res, vtable, MonoVTable *,
      (MonoVTable *)mono_remote_class_vtable(domain, remote_class, rp, error));
  return_val_if_nok(error, NULL_HANDLE);
  return res;
}

MonoReflectionTypeHandle
ves_icall_Remoting_RealProxy_InternalGetProxyType(MonoTransparentProxyHandle tp,
                                                  MonoError *error) {
  MonoRemoteClass *remote_class;
  ...
  return mono_type_get_object_handle(
      mono_handle_domain(tp), m_class_get_byval_arg(remote_class->proxy_class),
      error);
}
#endif
...
MonoStringHandle ves_icall_System_Environment_get_UserName(MonoError *error) {
...
#ifndef HOST_WIN32
static MonoStringHandle mono_icall_get_machine_name(MonoError *error) {
...
#ifndef HOST_WIN32
static int mono_icall_get_platform(void) {
#if defined(__MACH__)
  /* OSX */
  //
...
int ves_icall_System_Environment_get_Platform(void) {
  return mono_icall_get_platform();
}

#ifndef HOST_WIN32
static MonoStringHandle mono_icall_get_new_line(MonoError *error) {
  return mono_string_new_handle(mono_domain_get(), "\n", error);
}
#endif /* !HOST_WIN32 */

#ifndef ENABLE_NETCORE
MonoStringHandle ves_icall_System_Environment_get_NewLine(MonoError *error) {
  return mono_icall_get_new_line(error);
}
#endif

#ifndef HOST_WIN32
static inline MonoBoolean mono_icall_is_64bit_os(void) {
#if SIZEOF_VOID_P == 8
  return TRUE;
#else
  ...
  if (uname(&name) >= 0) {
    return strcmp(name.machine, "x86_64") == 0 ||
           strncmp(name.machine, "aarch64", 7) == 0 ||
           strncmp(name.machine, "ppc64", 5) == 0 ||
           strncmp(name.machine, "riscv64", 7) == 0;
  }
#endif
  return FALSE;
#endif
...
MonoBoolean ves_icall_System_Environment_GetIs64BitOperatingSystem(void) {
  return mono_icall_is_64bit_os();
}

MonoStringHandle ves_icall_System_Environment_GetEnvironmentVariable_native(
    const gchar *utf8_name, MonoError *error) {
  gchar *value;
  ...
  MonoStringHandle res =
      mono_string_new_handle(mono_domain_get(), value, error);
  g_free(value);
  return res;
}
...
void ves_icall_System_Environment_InternalSetEnvironmentVariable(
    const gunichar2 *name, gint32 name_length, const gunichar2 *value,
    gint32 value_length, MonoError *error) {
#ifdef HOST_WIN32
  ...
  utf8_name = mono_utf16_to_utf8(name, name_length,
                                 error); // FIXME: this should be ascii
...
void ves_icall_System_Environment_Exit(int result) {
...
void ves_icall_System_Environment_FailFast(MonoStringHandle message,
                                           MonoExceptionHandle exception,
                                           MonoStringHandle errorSource,
                                           MonoError *error) {
  ...
  if (!MONO_HANDLE_IS_NULL(exception)) {
    mono_print_unhandled_exception_internal(
        (MonoObject *)MONO_HANDLE_RAW(exception));
  }

  // NOTE: While this does trigger WER on Windows it doesn't quite provide all
  // the information in the error dump that CoreCLR would. On Windows 7+ we
  // should call RaiseFailFastException directly instead of relying on the C
  // runtime doing it for us and pass it as much information as possible. On
  // Windows 8+ we can also use the __fastfail intrinsic.
  abort();
}

#ifndef ENABLE_NETCORE
MonoStringHandle ves_icall_System_Environment_GetGacPath(MonoError *error) {
  return mono_string_new_handle(mono_domain_get(), mono_assembly_getrootdir(),
                                error);
}
#endif
...
mono_icall_get_windows_folder_path(int folder, MonoError *error) {
  error_init(error);
  g_warning("ves_icall_System_Environment_GetWindowsFolderPath should only be "
            "called on Windows!");
  return mono_string_new_handle(mono_domain_get(), "", error);
}
#endif /* !HOST_WIN32 */
...
static MonoArrayHandle mono_icall_get_logical_drives(MonoError *error) {
  ...
  dname = ptr;
  result =
      mono_array_new_handle(domain, mono_defaults.string_class, ndrives, error);
  goto_if_nok(error, leave);
...
MonoStringHandle ves_icall_System_IO_DriveInfo_GetDriveFormat(
    const gunichar2 *path, gint32 path_length, MonoError *error) {
  gunichar2 volume_name[MAX_PATH + 1];

  if (mono_w32file_get_file_system_type(path, volume_name, MAX_PATH + 1) ==
      FALSE)
    return NULL_HANDLE_STRING;
  return mono_string_new_utf16_handle(mono_domain_get(), volume_name,
                                      g_utf16_len(volume_name), error);
}
...
static const char *const encodings[] = {(char *)1,
                                        "ascii",
                                        "us_ascii",
                                        "us",
                                        "ansi_x3.4_1968",
...
MonoBoolean ves_icall_System_Environment_get_HasShutdownStarted(void) {
  return mono_runtime_is_shutting_down() ||
         mono_domain_is_unloading(mono_domain_get());
}

#ifndef HOST_WIN32

#ifndef ENABLE_NETCORE
void ves_icall_System_Environment_BroadcastSettingChange(MonoError *error) {}
#endif

#endif

gint32 ves_icall_System_Environment_get_TickCount(void) {
  /* this will overflow after ~24 days */
  return (gint32)(mono_msec_boottime() & 0xffffffff);
}

#if ENABLE_NETCORE
gint64 ves_icall_System_Environment_get_TickCount64(void) {
  return mono_msec_boottime();
}
#endif

#ifndef ENABLE_NETCORE
gint32 ves_icall_System_Runtime_Versioning_VersioningHelper_GetRuntimeId(
    MonoError *error) {
  return 9;
}
#endif
...
#ifndef DISABLE_REMOTING
MonoBoolean ves_icall_IsTransparentProxy(MonoObjectHandle proxy,
                                         MonoError *error) {
...
MonoReflectionMethodHandle ves_icall_Remoting_RemotingServices_GetVirtualMethod(
    MonoReflectionTypeHandle rtype, MonoReflectionMethodHandle rmethod,
    MonoError *error) {
  MonoReflectionMethodHandle ret =
      MONO_HANDLE_CAST(MonoReflectionMethod, NULL_HANDLE);
  ...
  if ((method->flags & METHOD_ATTRIBUTE_FINAL) ||
    ...
    if (klass == method->klass ||
        mono_class_is_subclass_of_internal(klass, method->klass, FALSE))
      ret = rmethod;
    return ret;
  }
  ...
  if (mono_class_is_interface(method->klass)) {
    ...
    int offs = mono_class_interface_offset_with_variance(klass, method->klass,
                                                         &variance_used);
    if (offs >= 0)
...
void ves_icall_System_Runtime_Activation_ActivationServices_EnableProxyActivation(
    MonoReflectionTypeHandle type, MonoBoolean enable, MonoError *error) {
  MonoClass *klass =
      mono_class_from_mono_type_internal(MONO_HANDLE_GETVAL(type, type));
  MonoVTable *vtable =
      mono_class_vtable_checked(mono_domain_get(), klass, error);
  return_if_nok(error);
...
ves_icall_System_Runtime_Activation_ActivationServices_AllocateUninitializedClassInstance(
    MonoReflectionTypeHandle type, MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(type);
  MonoClass *klass =
      mono_class_from_mono_type_internal(MONO_HANDLE_GETVAL(type, type));
  mono_class_init_checked(klass, error);
  ...
  if (m_class_get_rank(klass) >= 1) {
    ...
    return MONO_HANDLE_CAST(
        MonoObject, mono_array_new_handle(
                        domain, m_class_get_element_class(klass), 0, error));
  } else {
    MonoVTable *vtable = mono_class_vtable_checked(domain, klass, error);
    return_val_if_nok(error, NULL_HANDLE);

    /* Bypass remoting object creation check */
    return MONO_HANDLE_NEW(
        MonoObject, mono_object_new_alloc_specific_checked(vtable, error));
  }
}

MonoStringHandle ves_icall_System_IO_get_temp_path(MonoError *error) {
...
G_EXTERN_C gpointer CreateZStream(gint32 compress, MonoBoolean gzip,
                                  gpointer feeder, gpointer data);
...
gpointer ves_icall_System_IO_Compression_DeflateStreamNative_CreateZStream(
    gint32 compress, MonoBoolean gzip, gpointer feeder, gpointer data) {
...
gint32 ves_icall_System_IO_Compression_DeflateStreamNative_CloseZStream(
    gpointer stream) {
#ifdef MONO_CROSS_COMPILE
  return 0;
#else
...
gint32 ves_icall_System_IO_Compression_DeflateStreamNative_ReadZStream(
...
gint32 ves_icall_System_IO_Compression_DeflateStreamNative_WriteZStream(
    gpointer stream, gpointer buffer, gint32 length) {
#ifdef MONO_CROSS_COMPILE
  return 0;
#else
...
void ves_icall_System_TimeZoneInfo_mono_timezone_get_local_name(
    MonoString **result) {
...
MonoBoolean ves_icall_System_IO_DriveInfo_GetDiskFreeSpace(
    const gunichar2 *path_name, gint32 path_name_length,
    guint64 *free_bytes_avail, guint64 *total_number_of_bytes,
    guint64 *total_number_of_free_bytes, gint32 *error) {
  g_assert(error);
  ...
  *total_number_of_free_bytes = (guint64)-1;
  ...
  gboolean result = mono_w32file_get_disk_free_space(
      path_name, free_bytes_avail, total_number_of_bytes,
...
gpointer ves_icall_RuntimeMethodHandle_GetFunctionPointer(MonoMethod *method,
                                                          MonoError *error) {
...
ves_icall_System_Configuration_DefaultConfig_get_machine_config_path(
  ...
  path = g_build_path(G_DIR_SEPARATOR_S, mono_cfg_dir, "mono",
                      ...
                      "machine.config", (const char *)NULL);

  mono_icall_make_platform_path(path);

  MonoStringHandle mcpath =
      mono_string_new_handle(mono_domain_get(), path, error);
  g_free(path);
...
ves_icall_System_Configuration_InternalConfigurationHost_get_bundled_app_config(
  ...
  domain = mono_domain_get();
  MonoStringHandle file =
      MONO_HANDLE_NEW(MonoString, domain->setup->configuration_file);
...
static MonoStringHandle get_bundled_machine_config(MonoError *error) {
...
MonoBoolean ves_icall_get_resources_ptr(MonoReflectionAssemblyHandle assembly,
                                        gpointer *result, gint32 *size,
                                        MonoError *error) {
  ...
  *size = 0;
  ...
  image = assm->image;
  entry = (MonoPEResourceDataEntry *)mono_image_lookup_resource(
      image, MONO_PE_RESOURCE_ID_ASPNET_STRING, 0, NULL);
  if (!entry)
    return FALSE;
...
MonoBoolean ves_icall_System_Diagnostics_Debugger_IsAttached_internal(void) {
  return mono_is_debugger_attached();
}

MonoBoolean ves_icall_System_Diagnostics_Debugger_IsLogging(void) {
  return mono_get_runtime_callbacks()->debug_log_is_enabled &&
         mono_get_runtime_callbacks()->debug_log_is_enabled();
}

void ves_icall_System_Diagnostics_Debugger_Log(int level,
                                               MonoString *volatile *category,
                                               MonoString *volatile *message) {
...
ves_icall_System_Diagnostics_Tracing_EventPipeInternal_CreateProvider(
    MonoStringHandle provider_name, MonoDelegateHandle callback_func,
    MonoError *error) {
...
intptr_t ves_icall_System_Diagnostics_Tracing_EventPipeInternal_DefineEvent(
    intptr_t prov_handle, uint32_t event_id, int64_t keywords,
    uint32_t event_version, uint32_t level, const uint8_t *metadata,
    uint32_t metadata_len) {
  return EVENT_PIPE_DUMMY_EVENT_ID;
}

void ves_icall_System_Diagnostics_Tracing_EventPipeInternal_DeleteProvider(
    intptr_t prov_handle) {
  ;
}

void ves_icall_System_Diagnostics_Tracing_EventPipeInternal_Disable(
    uint64_t session_id) {
  ;
}

uint64_t ves_icall_System_Diagnostics_Tracing_EventPipeInternal_Enable(
    const_gunichar2_ptr output_file,
...
ves_icall_System_Diagnostics_Tracing_EventPipeInternal_EventActivityIdControl(
    uint32_t control_code, /* GUID * */ uint8_t *activity_id) {
  return EVENT_PIPE_ERROR_SUCCESS;
}

MonoBoolean ves_icall_System_Diagnostics_Tracing_EventPipeInternal_GetNextEvent(
    uint64_t session_id, /* EventPipeEventInstanceData * */ void *instance) {
  return FALSE;
}

intptr_t ves_icall_System_Diagnostics_Tracing_EventPipeInternal_GetProvider(
    const_gunichar2_ptr provider_name) {
...
void ves_icall_System_Diagnostics_Tracing_EventPipeInternal_WriteEventData(
    intptr_t event_handle,
    /* EventProviderEventData[] */ const void *event_data, uint32_t data_count,
...
void ves_icall_System_Diagnostics_DefaultTraceListener_WriteWindowsDebugString(
    const gunichar2 *message, MonoError *error) {
  mono_icall_write_windows_debug_string(message);
}
#endif

/* Only used for value types */
MonoObjectHandle ves_icall_System_Activator_CreateInstanceInternal(
    MonoReflectionTypeHandle ref_type, MonoError *error) {
  MonoDomain *domain = MONO_HANDLE_DOMAIN(ref_type);
  MonoType *type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *klass = mono_class_from_mono_type_internal(type);
...
MonoReflectionMethodHandle ves_icall_RuntimeMethodInfo_get_base_method(
    MonoReflectionMethodHandle m, MonoBoolean definition, MonoError *error) {
  ...
  if (base == method) {
    /* we want to short-circuit and return 'm' here. But we should
       ...
       type. */
    MonoReflectionTypeHandle orig_reftype =
        MONO_HANDLE_NEW_GET(MonoReflectionType, m, reftype);
    MonoClass *orig_klass = mono_class_from_mono_type_internal(
        MONO_HANDLE_GETVAL(orig_reftype, type));
...
MonoStringHandle
ves_icall_RuntimeMethodInfo_get_name(MonoReflectionMethodHandle m,
                                     MonoError *error) {
  MonoMethod *method = MONO_HANDLE_GETVAL(m, method);

  MonoStringHandle s =
      mono_string_new_handle(MONO_HANDLE_DOMAIN(m), method->name, error);
  return_val_if_nok(error, NULL_HANDLE_STRING);
  MONO_HANDLE_SET(m, name, s);
  return s;
}

void ves_icall_System_ArgIterator_Setup(MonoArgIterator *iter, char *argsp,
                                        char *start) {
   ...
   * iter->sig->param_count, iter->sig->sentinelpos); */
}

void ves_icall_System_ArgIterator_IntGetNextArg(MonoArgIterator *iter,
                                                MonoTypedRef *res) {
  guint32 i, arg_size;
  gint32 align;
   ...
   * res->type->type, arg_size, res->value); */
}

void ves_icall_System_ArgIterator_IntGetNextArgWithType(MonoArgIterator *iter,
                                                        MonoTypedRef *res,
                                                        MonoType *type) {
  guint32 i, arg_size;
  gint32 align;
...
MonoObjectHandle ves_icall_System_TypedReference_ToObject(MonoTypedRef *tref,
                                                          MonoError *error) {
  return typed_reference_to_object(tref, error);
}

void ves_icall_System_TypedReference_InternalMakeTypedReference(
    MonoTypedRef *res, MonoObjectHandle target, MonoArrayHandle fields,
    MonoReflectionTypeHandle last_field, MonoError *error) {
  MonoType *ftype = NULL;
  int i;
...
static void prelink_method(MonoMethod *method, MonoError *error) {
  error_init(error);
...
void ves_icall_System_Runtime_InteropServices_Marshal_Prelink(
    MonoReflectionMethodHandle method, MonoError *error) {
  error_init(error);

  prelink_method(MONO_HANDLE_GETVAL(method, method), error);
}

void ves_icall_System_Runtime_InteropServices_Marshal_PrelinkAll(
    MonoReflectionTypeHandle type, MonoError *error);
...
/*
 * used by
 * System.Runtime.InteropServices.RuntimeInformation.(OS|Process)Architecture;
...
ves_icall_System_Runtime_InteropServices_RuntimeInformation_GetRuntimeArchitecture(
    MonoError *error) {
  return mono_string_new_handle(mono_domain_get(), mono_config_get_cpu(),
                                error);
}
 ...
 */
MonoStringHandle
ves_icall_System_Runtime_InteropServices_RuntimeInformation_GetOSName(
    MonoError *error) {
  return mono_string_new_handle(mono_domain_get(), mono_config_get_os(), error);
}
#endif /* ENABLE_NETCORE */

int ves_icall_Interop_Sys_DoubleToString(double value, char *format,
  ...
  if (bits == 0x1) { /* 4.9406564584124654E-324 */
    ...
    return snprintf(buffer, bufferLength, "%s",
...
void ves_icall_System_Runtime_RuntimeImports_ecvt_s(char *buffer,
                                                    size_t sizeInBytes,
...
void ves_icall_System_NumberFormatter_GetFormatterTables(
    guint64 const **mantissas, gint32 const **exponents,
    gunichar2 const **digitLowerTable, gunichar2 const **digitUpperTable,
    gint64 const **tenPowersList, gint32 const **decHexDigits) {
...
static gboolean add_modifier_to_array(MonoDomain *domain, MonoType *type,
                                      MonoArrayHandle dest, int dest_idx,
                                      MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  error_init(error);
...
static MonoArrayHandle type_array_from_modifiers(MonoImage *image,
  ...
  MonoArrayHandle res;
  res = mono_array_new_handle(domain, mono_defaults.systemtype_class, count,
                              error);
  goto_if_nok(error, fail);
  count = 0;
  for (i = 0; i < cmod_count; ++i) {
    gboolean required;
    MonoType *cmod_type =
        mono_type_get_custom_modifier(type, i, &required, error);
    goto_if_nok(error, fail);
    if ((optional && !required) || (!optional && required)) {
...
MonoArrayHandle ves_icall_RuntimeParameterInfo_GetTypeModifiers(
    MonoReflectionTypeHandle rt, MonoObjectHandle member, int pos,
    MonoBoolean optional, MonoError *error) {
  error_init(error);
  ...
  if (mono_class_is_reflection_method_or_constructor(member_class)) {
    method = MONO_HANDLE_GETVAL(MONO_HANDLE_CAST(MonoReflectionMethod, member),
                                method);
  } else if (m_class_get_image(member_class) == mono_defaults.corlib &&
             !strcmp("RuntimePropertyInfo", m_class_get_name(member_class))) {
    MonoProperty *prop = MONO_HANDLE_GETVAL(
        MONO_HANDLE_CAST(MonoReflectionProperty, member), property);
    if (!(method = prop->get))
      method = prop->set;
    g_assert(method);
  } else {
...
static MonoType *get_property_type(MonoProperty *prop) {
  MonoMethodSignature *sig;
  if (prop->get) {
...
MonoArrayHandle ves_icall_RuntimePropertyInfo_GetTypeModifiers(
    MonoReflectionPropertyHandle property, MonoBoolean optional,
    MonoError *error) {
  error_init(error);
 ...
 */
static void mono_type_from_blob_type(MonoType *type, MonoTypeEnum blob_type,
                                     MonoType *real_type) {
  type->type = blob_type;
  type->data.klass = NULL;
  if (blob_type == MONO_TYPE_CLASS)
    type->data.klass = mono_defaults.object_class;
  else if (real_type->type == MONO_TYPE_VALUETYPE &&
           m_class_is_enumtype(real_type->data.klass)) {
...
MonoObjectHandle ves_icall_property_info_get_default_value(
    MonoReflectionPropertyHandle property_handle, MonoError *error) {
  ...
  return mono_get_object_from_blob(domain, &blob_type, def_value,
                                   MONO_HANDLE_NEW(MonoString, NULL), error);
}

MonoBoolean
ves_icall_MonoCustomAttrs_IsDefinedInternal(MonoObjectHandle obj,
                                            MonoReflectionTypeHandle attr_type,
                                            MonoError *error) {
  MonoClass *attr_class =
      mono_class_from_mono_type_internal(MONO_HANDLE_GETVAL(attr_type, type));

  mono_class_init_checked(attr_class, error);
  return_val_if_nok(error, FALSE);

  MonoCustomAttrInfo *cinfo =
      mono_reflection_get_custom_attrs_info_checked(obj, error);
  return_val_if_nok(error, FALSE);
...
MonoArrayHandle ves_icall_MonoCustomAttrs_GetCustomAttributesInternal(
    MonoObjectHandle obj, MonoReflectionTypeHandle attr_type,
    MonoBoolean pseudoattrs, MonoError *error) {
  MonoClass *attr_class;
  ...
  return mono_reflection_get_custom_attrs_by_type_handle(obj, attr_class,
                                                         error);
}

MonoArrayHandle
ves_icall_MonoCustomAttrs_GetCustomAttributesDataInternal(MonoObjectHandle obj,
                                                          MonoError *error) {
  return mono_reflection_get_custom_attrs_data_checked(obj, error);
}

MonoStringHandle ves_icall_Mono_Runtime_GetDisplayName(MonoError *error) {
  char *info;
...
#ifndef HOST_WIN32
static gint32 mono_icall_wait_for_input_idle(gpointer handle,
                                             gint32 milliseconds) {
  return WAIT_TIMEOUT;
}
#endif /* !HOST_WIN32 */
...
#ifndef ENABLE_NETCORE
gint32
ves_icall_Microsoft_Win32_NativeMethods_WaitForInputIdle(gpointer handle,
                                                         gint32 milliseconds) {
  return mono_icall_wait_for_input_idle(handle, milliseconds);
}

gint32 ves_icall_Microsoft_Win32_NativeMethods_GetCurrentProcessId(void) {
  return mono_process_current_pid();
}

MonoBoolean ves_icall_Mono_TlsProviderFactory_IsBtlsSupported(void) {
#if HAVE_BTLS
  return TRUE;
#else
  return FALSE;
...
int ves_icall_System_Runtime_InteropServices_Marshal_GetHRForException_WinRT(
    MonoExceptionHandle ex, MonoError *error) {
  mono_error_set_not_implemented(
      error, "System.Runtime.InteropServices.Marshal.GetHRForException_WinRT "
             "internal call is not implemented.");
  return 0;
}

MonoObjectHandle
ves_icall_System_Runtime_InteropServices_Marshal_GetNativeActivationFactory(
    MonoObjectHandle type, MonoError *error) {
  mono_error_set_not_implemented(
      error, "System.Runtime.InteropServices.Marshal."
             "GetNativeActivationFactory internal call is not implemented.");
  return NULL_HANDLE;
}
...
ves_icall_System_Runtime_InteropServices_Marshal_GetRawIUnknownForComObjectNoAddRef(
  ...
  mono_error_set_not_implemented(
      ...
      "GetRawIUnknownForComObjectNoAddRef internal call is not implemented.");
  return NULL;
}
...
ves_icall_System_Runtime_InteropServices_WindowsRuntime_UnsafeNativeMethods_GetRestrictedErrorInfo(
  ...
  mono_error_set_not_implemented(
      ...
      "GetRestrictedErrorInfo internal call is not implemented.");
  return NULL_HANDLE;
}

MonoBoolean
ves_icall_System_Runtime_InteropServices_WindowsRuntime_UnsafeNativeMethods_RoOriginateLanguageException(
    int ierr, MonoStringHandle message, void *languageException,
    MonoError *error) {
  mono_error_set_not_implemented(
      error,
      "System.Runtime.InteropServices.WindowsRuntime.UnsafeNativeMethods."
      "RoOriginateLanguageException internal call is not implemented.");
  return FALSE;
}

void ves_icall_System_Runtime_InteropServices_WindowsRuntime_UnsafeNativeMethods_RoReportUnhandledError(
    MonoObjectHandle oerr, MonoError *error) {
  mono_error_set_not_implemented(
      error,
      "System.Runtime.InteropServices.WindowsRuntime.UnsafeNativeMethods."
      "RoReportUnhandledError internal call is not implemented.");
}

int ves_icall_System_Runtime_InteropServices_WindowsRuntime_UnsafeNativeMethods_WindowsCreateString(
    MonoStringHandle sourceString, int length, void **hstring,
    MonoError *error) {
  mono_error_set_not_implemented(
      error,
      "System.Runtime.InteropServices.WindowsRuntime.UnsafeNativeMethods."
      "WindowsCreateString internal call is not implemented.");
  return 0;
}

int ves_icall_System_Runtime_InteropServices_WindowsRuntime_UnsafeNativeMethods_WindowsDeleteString(
    void *hstring, MonoError *error) {
  mono_error_set_not_implemented(
      error,
      "System.Runtime.InteropServices.WindowsRuntime.UnsafeNativeMethods."
      "WindowsDeleteString internal call is not implemented.");
  return 0;
}

mono_unichar2 *
ves_icall_System_Runtime_InteropServices_WindowsRuntime_UnsafeNativeMethods_WindowsGetStringRawBuffer(
    void *hstring, unsigned *length, MonoError *error) {
  mono_error_set_not_implemented(
      error,
      "System.Runtime.InteropServices.WindowsRuntime.UnsafeNativeMethods."
      "WindowsGetStringRawBuffer internal call is not implemented.");
  return NULL;
}
...
void ves_icall_System_IO_LogcatTextWriter_Log(const char *appname, gint32 level,
                                              const char *message) {
...
static void mono_icall_lock(void) {
  mono_locks_os_acquire(&icall_mutex, IcallLock);
}

static void mono_icall_unlock(void) {
...
static void add_internal_call_with_flags(const char *name, gconstpointer method,
                                         guint32 flags) {
 ...
 * types like \c System.String are exposed as \c MonoString* parameters.   This
 * is different than the way that strings are surfaced in P/Invoke.
 *
 * For more information on how the parameters are marshalled, see the
 * <a href="http://www.mono-project.com/docs/advanced/embedding/">Mono
 * Embedding</a> page.
 *
 ...
 *
 */
void mono_dangerous_add_raw_internal_call(const char *name,
                                          gconstpointer method) {
 ...
 * \param name method specification to surface to the managed world
 * \param method pointer to a C method to invoke when the method is called
 * \param cooperative if \c TRUE, run icall in GC Unsafe (cooperatively
 * suspended) mode, otherwise GC Safe (blocking)
 *
 ...
 *
 */
void mono_add_internal_call_with_flags(const char *name, gconstpointer method,
                                       gboolean cooperative) {
  add_internal_call_with_flags(name, method,
                               cooperative ? MONO_ICALL_FLAGS_COOPERATIVE
                                           : MONO_ICALL_FLAGS_FOREIGN);
}

void mono_add_internal_call_internal(const char *name, gconstpointer method) {
  mono_add_internal_call_with_flags(name, method, TRUE);
}
 ...
 */
static int concat_class_name(char *buf, int bufsize, MonoClass *klass) {
...
static void no_icall_table(void) { g_assert_not_reached(); }
 ...
 */
gconstpointer mono_lookup_internal_call_full_with_flags(
    MonoMethod *method, gboolean warn_on_missing, guint32 *flags) {
  char *sigstart = NULL;
  char *tmpsig = NULL;
  char mname[2048];
  char *classname = NULL;
  ...
  if (m_class_get_nested_in(method->klass)) {
    int pos = concat_class_name(mname, sizeof(mname) - 2,
                                m_class_get_nested_in(method->klass));
    if (!pos)
      goto exit;
    ...
    typelen =
        concat_class_name(mname + pos, sizeof(mname) - pos - 1, method->klass);
    if (!typelen)
      goto exit;
  ...
  *sigstart = 0;
  ...
  tmpsig =
      mono_signature_get_desc(mono_method_signature_internal(method), TRUE);
  ...
  *sigstart = 0;
  ...
  if (!icall_table) {
    ...
    res = icall_table->lookup(method, classname, sigstart - mlen, sigstart,
                              &uses_handles);
    ...
    if (warn_on_missing) {
      ...
      g_print("\nWhen you update one from git you need to update, compile and "
              "install\nthe other too.\n");
      g_print("Do not report this as a bug unless you're sure you have updated "
              "correctly:\nyou probably have a broken mono install.\n");
      g_print("If you see other errors or faults after this message they are "
              "probably related\n");
 ...
 * \param uses_handles out argument if method needs handles around managed
 * objects. \returns a pointer to the icall code for the given method.  If \p
 * uses_handles is not NULL, it will be set to TRUE if the method needs managed
 * objects wrapped using the infrastructure in handle.h
 *
 * If the method is not found, warns and returns NULL.
 */
gconstpointer mono_lookup_internal_call_full(MonoMethod *method,
  ...
  gconstpointer addr = mono_lookup_internal_call_full_with_flags(
      method, warn_on_missing, &flags);
...
#define ICALL_SIG_TYPES_4(a, b, c, d)                                          \
  ICALL_SIG_TYPES_3(a, b, c) ICALL_SIG_TYPES_1(d)
#define ICALL_SIG_TYPES_5(a, b, c, d, e)                                       \
  ICALL_SIG_TYPES_4(a, b, c, d) ICALL_SIG_TYPES_1(e)
#define ICALL_SIG_TYPES_6(a, b, c, d, e, f)                                    \
  ICALL_SIG_TYPES_5(a, b, c, d, e) ICALL_SIG_TYPES_1(f)
#define ICALL_SIG_TYPES_7(a, b, c, d, e, f, g)                                 \
  ICALL_SIG_TYPES_6(a, b, c, d, e, f) ICALL_SIG_TYPES_1(g)
#define ICALL_SIG_TYPES_8(a, b, c, d, e, f, g, h)                              \
  ICALL_SIG_TYPES_7(a, b, c, d, e, f, g) ICALL_SIG_TYPES_1(h)
...
#define ICALL_SIG(n, types)                                                    \
  {{                                                                           \
       ...
       MONO_ICALL_SIGNATURE_CALL_CONVENTION, 0, /* hasthis */                  \
       0,                                       /* explicit_this */            \
       1,                                       /* pinvoke */                  \
       0,                                       /* is_inflated */              \
...
#undef ICALL_SIG
#define ICALL_SIG(n, types)                                                    \
  MonoMethodSignature *const ICALL_SIG_NAME(n, types) =                        \
      &mono_icall_signatures.ICALL_SIG_NAME(n, types).sig;
ICALL_SIGS
#undef ICALL_SIG
...
void mono_create_icall_signatures(void) {
  ...
  MonoType *const lookup[] = {
      ...
      m_class_get_byval_arg(
          mono_defaults.double_class), // ICALL_SIG_TYPE_double
      ...
      mono_class_get_byref_type(
          mono_defaults.int_class), // ICALL_SIG_TYPE_ptrref
      m_class_get_byval_arg(
          mono_defaults.string_class), // ICALL_SIG_TYPE_string
      m_class_get_byval_arg(
          mono_defaults.uint16_class), // ICALL_SIG_TYPE_uint16
      m_class_get_byval_arg(
          mono_defaults.uint32_class),                 // ICALL_SIG_TYPE_uint32
...
void mono_register_jit_icall_info(MonoJitICallInfo *info, gconstpointer func,
                                  const char *name, MonoMethodSignature *sig,
...
int ves_icall_System_GC_GetCollectionCount(int generation) {
  return mono_gc_collection_count(generation);
}

int ves_icall_System_GC_GetGeneration(MonoObjectHandle object,
                                      MonoError *error) {
  return mono_gc_get_generation(MONO_HANDLE_RAW(object));
}

int ves_icall_System_GC_GetMaxGeneration(void) {
  return mono_gc_max_generation();
}

gint64 ves_icall_System_GC_GetAllocatedBytesForCurrentThread(void) {
  return mono_gc_get_allocated_bytes_for_current_thread();
}

#ifdef ENABLE_NETCORE
guint64 ves_icall_System_GC_GetTotalAllocatedBytes(MonoBoolean precise,
                                                   MonoError *error) {
  return mono_gc_get_total_allocated_bytes(precise);
}
#endif

void ves_icall_System_GC_RecordPressure(gint64 value) {
  mono_gc_add_memory_pressure(value);
}

gint64 ves_icall_System_Diagnostics_Stopwatch_GetTimestamp(void) {
  return mono_100ns_ticks();
}

gint64 ves_icall_System_Threading_Timer_GetTimeMonotonic(void) {
  return mono_100ns_ticks();
}

gint64 ves_icall_System_DateTime_GetSystemTimeAsFileTime(void) {
  return mono_100ns_datetime();
}

int ves_icall_System_Threading_Thread_SystemMaxStackSize(void) {
  return mono_thread_info_get_system_max_stack_size();
}

MonoBoolean ves_icall_System_Threading_Thread_YieldInternal(void) {
...
G_EXTERN_C gint32 ReadEvents(gpointer sock, gpointer buffer, gint32 count,
                             gint32 size);
G_EXTERN_C gpointer CloseNLSocket(gpointer sock);

gpointer
ves_icall_System_Net_NetworkInformation_LinuxNetworkChange_CreateNLSocket(
    void) {
  return CreateNLSocket();
}

gint32 ves_icall_System_Net_NetworkInformation_LinuxNetworkChange_ReadEvents(
    gpointer sock, gpointer buffer, gint32 count, gint32 size) {
...
#define MONO_HANDLE_REGISTER_ICALL(func, ret, nargs, argtypes)                 \
  MONO_HANDLE_REGISTER_ICALL_IMPLEMENT(func, ret, nargs, argtypes)

// Some native functions are exposed via multiple managed names.
// Producing a wrapper for these results in duplicate wrappers with the same
// names, which fails to compile. Do not produce such duplicate wrappers.
// Alternatively, a one line native function with a different name that calls
// the main one could be used. i.e. the wrapper would also have a different
// name.
