...
// Change the assembly set in `image` to the assembly set in `assemblyImage`.
// Halt if overwriting is attempted. Can be used on modules loaded through
// either the "file" or "module" mechanism
static gboolean assign_assembly_parent_for_netmodule(MonoImage *image,
  ...
  while (1) {
    ...
    if (assemblyOld) {
      ...
      mono_error_set_bad_image(
          error, assemblyImage,
          "Attempted to load module %s which has already been loaded by "
          "assembly %s. This is not supported in Mono.",
          image->name, assemblyOld->image->name);
      return FALSE;
    }
    gpointer result =
        mono_atomic_xchg_ptr((gpointer *)&image->assembly, assembly);
    if (result == assembly)
      return TRUE;
  }
}
...
#define mono_images_storage_lock()                                             \
  do {                                                                         \
    if (mutex_inited)                                                          \
      mono_os_mutex_lock(&images_storage_mutex);                               \
  } while (0)
#define mono_images_storage_unlock()                                           \
  do {                                                                         \
    if (mutex_inited)                                                          \
      mono_os_mutex_unlock(&images_storage_mutex);                             \
  } while (0)
...
static MonoImage *mono_image_open_a_lot_parameterized(
    MonoLoadedImages *li, MonoAssemblyLoadContext *alc, const char *fname,
    MonoImageOpenStatus *status, gboolean refonly, gboolean load_from_context,
    gboolean *problematic);
...
void mono_install_image_unload_hook(MonoImageUnloadFunc func,
                                    gpointer user_data) {
...
void mono_remove_image_unload_hook(MonoImageUnloadFunc func,
                                   gpointer user_data) {
  GSList *l;
  ImageUnloadHook *hook;
...
static void mono_image_invoke_unload_hook(MonoImage *image) {
  GSList *l;
  ImageUnloadHook *hook;
...
guint32 mono_cli_rva_image_map(MonoImage *image, guint32 addr) {
 ...
 * an image section (<code>.text</code>, <code>.rsrc</code>,
 * <code>.data</code>).
 *
...
static int load_section_tables(MonoImage *image, MonoCLIImageInfo *iinfo,
                               guint32 offset) {
...
gboolean mono_image_load_cli_header(MonoImage *image, MonoCLIImageInfo *iinfo) {
  ...
  if (offset + sizeof(MonoCLIHeader) > image->raw_data_len)
    return FALSE;
  memcpy(&iinfo->cli_cli_header, image->raw_data + offset,
...
#undef SWAPPDE
  ...
  if ((iinfo->cli_cli_header.ch_eeinfo_table.rva != 0) ||
     ...
     */
    /* g_warning ("Some fields in the CLI header which should have been zero are
     * not zero"); */
  }
...
static gboolean load_metadata_ptrs(MonoImage *image, MonoCLIImageInfo *iinfo) {
  guint32 offset, size;
  guint16 streams;
  int i;
  guint32 pad;
  ...
  for (i = 0; i < streams; i++) {
    if (strncmp(ptr + 8, "#~", 3) == 0) {
      ...
      mono_trace(
          ...
          "correctly (without the /incremental switch or in Release mode).",
          image->name);
 ...
 */
static gboolean load_tables(MonoImage *image) {
  ...
  if (t->rows >= 1) {
    ...
    if (strcmp(name, "<Module>") == 0) {
      guint32 first_method =
          mono_metadata_decode_row_col(t, 0, MONO_TYPEDEF_METHOD_LIST) - 1;
      guint32 last_method;
      if (t->rows > 1)
        last_method =
            mono_metadata_decode_row_col(t, 1, MONO_TYPEDEF_METHOD_LIST) - 1;
      else
        last_method = mt->rows;
      for (; first_method < last_method; first_method++) {
        nameidx =
            mono_metadata_decode_row_col(mt, first_method, MONO_METHOD_NAME);
...
#ifndef ENABLE_NETCORE
static void load_modules(MonoImage *image) {
  MonoTableInfo *t;
 ...
 * Return NULL if it cannot be loaded. NULL without MonoError being set will be
 * interpreted as "not found".
 */
MonoImage *mono_image_load_module_checked(MonoImage *image, int idx,
                                          MonoError *error) {
  error_init(error);
  ...
  /* SRE still uses image->modules, but they are not loaded from files, so the
   * rest of this function is dead code for netcore */
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
             ...
             image->assembly ? image->assembly->aname.name : "some assembly",
             image->name);
  ...
  for (i = 0; i < file_table->rows; i++) {
    ...
    if (cols[MONO_FILE_FLAGS] == FILE_CONTAINS_NO_METADATA)
      continue;
    valid_modules = g_list_prepend(
        valid_modules,
        (char *)mono_metadata_string_heap(image, cols[MONO_FILE_NAME]));
  }
...

  {
    ...
    if (valid) {
      ...
      MonoImage *moduleImage = mono_image_open_a_lot_parameterized(
          li, alc, module_ref, &status, refonly, FALSE, NULL);
      if (moduleImage) {
...
static gpointer class_key_extract(gpointer value) {
  MonoClass *klass = (MonoClass *)value;

  return GUINT_TO_POINTER(m_class_get_type_token(klass));
}

static gpointer *class_next_value(gpointer value) {
...
void mono_image_init(MonoImage *image) {
  ...
  mono_internal_hash_table_init(&image->class_cache, g_direct_hash,
                                class_key_extract, class_next_value);
 ...
 */
static int do_load_header(MonoImage *image, MonoDotNetHeader *header,
                          int offset) {
  ...
  if (header->pesig[0] != 'P' || header->pesig[1] != 'E' || header->pesig[2] ||
      header->pesig[3])
    return -1;
  ...
  if (header->pe.pe_magic == 0x10B) {
    ...
    if (header->coff.coff_opt_header_size !=
        (sizeof(MonoDotNetHeader) - sizeof(MonoCOFFHeader) - 4))
      return -1;
    ...
    /* PE32+ file format */
    if (header->coff.coff_opt_header_size !=
        (sizeof(MonoDotNetHeader64) - sizeof(MonoCOFFHeader) - 4))
      return -1;
    ...
    /* FIXME: we lose bits here, but we don't use this stuff internally, so we
     * don't care much. will be fixed when we change MonoDotNetHeader to not
     * match the 32 bit variant
     */
...
static gboolean pe_image_load_pe_data(MonoImage *image) {
  ...
  if (offset < 0)
    ...
    /*
     * this tests for a x86 machine type, but itanium, amd64 and others could be
    used, too.
     * we skip this test.
...
static gboolean pe_image_load_cli_data(MonoImage *image) {
  MonoCLIImageInfo *iinfo;
...
static void mono_image_load_time_date_stamp(MonoImage *image) {
...
void mono_image_load_names(MonoImage *image) {
  ...
  if (image->tables[MONO_TABLE_MODULE].rows) {
    image->module_name = mono_metadata_string_heap(
        image, mono_metadata_decode_row_col(&image->tables[MONO_TABLE_MODULE],
                                            0, MONO_MODULE_NAME));
  }
}

static gboolean pe_image_load_tables(MonoImage *image) { return TRUE; }

static gboolean pe_image_match(MonoImage *image) {
...
static void install_pe_loader(void) { mono_install_image_loader(&pe_loader); }

/*
Ignored assemblies.

There are some assemblies we need to ignore because they include an
implementation that doesn't work under mono. Mono provides its own
implementation of those assemblies so it's safe to do so.

The ignored_assemblies list is generated using tools/nuget-hash-extractor and
feeding the problematic nugets to it.

Right now the list of nugets are the ones that provide the assemblies in
...
If any assemblies are added/removed, then this should be regenerated with:
  ...
  $ mono tools/nuget-hash-extractor/nuget-hash-extractor.exe nugets
guids_for_msbuild > mcs/tools/xbuild/data/deniedAssembliesList.txt
...
static const IgnoredAssembly ignored_assemblies[] = {
    ...
    IGNORED_ASSEMBLY(0x1438EAE0, SYS_RT_INTEROP_RUNTIME_INFO,
                     "F580BAAC-12BD-4716-B486-C0A5E3EE6EEA",
                     "15.5.0-preview-20171027-2 net461"),
    IGNORED_ASSEMBLY(0x16BCA997, SYS_RT_INTEROP_RUNTIME_INFO,
                     "CA2D23DE-55E1-45D8-9720-0EBE3EEC1DF2",
                     "2.0.0-preview3-20170622-1 net462"),
    IGNORED_ASSEMBLY(0x17A113, SYS_RT_INTEROP_RUNTIME_INFO,
                     "D87389D8-6E9C-48CF-B128-3637018577AF",
                     "2.1.0-preview1-62414-02 net47"),
    IGNORED_ASSEMBLY(0x1CBD59A2, SYS_IO_COMPRESSION,
                     "44FCA06C-A510-4B3E-BDBF-D08D697EF65A", "4.1.0 net46"),
    IGNORED_ASSEMBLY(0x1EA951BB, SYS_IO_COMPRESSION,
                     "05C07BD4-AFF1-4B12-900B-F0A5EB88DDB4",
                     "2.1.0-preview1-62414-02 net461"),
    IGNORED_ASSEMBLY(0x1FB3F8E8, SYS_GLOBALIZATION_EXT,
                     "B9BA8638-25D2-4A3B-B91F-16B3D3799861",
                     "2.1.100-preview-62617-01 net461"),
    IGNORED_ASSEMBLY(0x2706B80, SYS_NET_HTTP,
                     "8E2F55F3-D010-417B-A742-21386EDDD388",
                     "2.0.0-preview3-20170622-1 net461"),
    IGNORED_ASSEMBLY(0x27726A90, SYS_NET_HTTP,
                     "269B562C-CC15-4736-B1B1-68D4A43CAA98", "4.1.0 net46"),
    IGNORED_ASSEMBLY(0x284ECF63, SYS_THREADING_OVERLAPPED,
                     "E933407E-C846-4413-82C5-09F4BCFA67F1",
                     "2.1.0-preview1-62414-02 net461"),
    IGNORED_ASSEMBLY(0x358C9723, SYS_GLOBALIZATION_EXT,
                     "5FCD54F0-4B97-4259-875D-30E481F02EA2", "4.0.1 net46"),
    IGNORED_ASSEMBLY(0x39C3575D, SYS_GLOBALIZATION_EXT,
                     "3B30D67C-B16B-47BC-B949-9500B5AAAAFB",
                     "2.1.0-preview1-62414-02 net471"),
    IGNORED_ASSEMBLY(0x3E21E75A, SYS_TEXT_ENC_CODEPAGES,
                     "67D3A14A-8F55-4C9F-9699-EDD0876369DA", "4.5.0 net461"),
    IGNORED_ASSEMBLY(0x420963C4, SYS_NET_HTTP,
    ...
    IGNORED_ASSEMBLY(0x4CC79B26, SYS_GLOBALIZATION_EXT,
                     "28006E9A-74DB-45AC-8A8D-030CEBAA272A",
                     "2.0.0-preview3-20170622-1 net461"),
    IGNORED_ASSEMBLY(0x4E906129, SYS_NET_HTTP,
                     "27BBDD4C-EAF0-4A95-B172-EE502D76A725",
                     "2.1.100-preview-62617-01 net461"),
    IGNORED_ASSEMBLY(0x5E393C29, SYS_IO_COMPRESSION,
                     "3A58A219-266B-47C3-8BE8-4E4F394147AB", "4.3.0 net46"),
    IGNORED_ASSEMBLY(0x662BC58F, SYS_IO_COMPRESSION,
                     "C786B28D-0850-4D4C-AED9-FE6B86EE7C31",
                     "2.1.0-preview1-62414-02 net471"),
    IGNORED_ASSEMBLY(0x66CEDA9, SYS_THREADING_OVERLAPPED,
                     "A0439CB6-A5E6-4813-A76C-13F92ADDDED5",
                     "2.1.100-preview-62617-01 net461"),
    IGNORED_ASSEMBLY(0x6AE7C015, SYS_IO_COMPRESSION,
                     "35DD20B5-8766-476B-B5D2-0EA16EF0A946",
                     "2.1.100-preview-62617-01 net461"),
    IGNORED_ASSEMBLY(0x6FE03EE2, SYS_THREADING_OVERLAPPED,
                     "87697E71-D192-4F0B-BAD4-02BBC7793005", "4.3.0 net46"),
    IGNORED_ASSEMBLY(0x74EA304F, SYS_RT_INTEROP_RUNTIME_INFO,
                     "E5AE3324-2100-4F77-9E41-AEEF226C9649",
                     "15.5.0-preview-20171027-2 net47"),
    IGNORED_ASSEMBLY(0x765A8E04, SYS_RT_INTEROP_RUNTIME_INFO,
                     "E46BA45E-6A63-47CD-AF70-2C3016AFF75A",
                     "2.1.100-preview-62617-01 net461"),
    IGNORED_ASSEMBLY(0x786145B9, SYS_RT_INTEROP_RUNTIME_INFO,
                     "0C4BCFB3-F609-4631-93A6-17B19C69D9B6",
                     "2.0.0-preview3-20170622-1 net47"),
    IGNORED_ASSEMBLY(0x79F6E37F, SYS_THREADING_OVERLAPPED,
                     "212BEDF2-E3F5-4D59-8C1A-F4D1C58B46CD",
                     "15.5.0-preview-20171027-2 net461"),
    ...
    IGNORED_ASSEMBLY(0x8BFCB05D, SYS_THREADING_OVERLAPPED,
                     "82D565AC-E41C-4E29-9939-C031C88EDBDD",
                     "2.1.100-preview-62617-01 net471"),
    IGNORED_ASSEMBLY(0x8CCF2469, SYS_RT_INTEROP_RUNTIME_INFO,
                     "9A3724BF-DF8F-4955-8CFA-41D45F11B586",
                     "2.1.0-preview1-62414-02 net462"),
    IGNORED_ASSEMBLY(0x90772EB6, SYS_THREADING_OVERLAPPED,
                     "F4FFC4A6-E694-49D9-81B2-12F2C9A29652",
                     "2.0.0-preview3-20170622-1 net461"),
    IGNORED_ASSEMBLY(0x911D9EC3, SYS_TEXT_ENC_CODEPAGES,
                     "C142254F-DEB5-46A7-AE43-6F10320D1D1F", "4.0.1 net46"),
    IGNORED_ASSEMBLY(0x96B5F0BA, SYS_NET_HTTP,
                     "DB06A592-E332-44A1-8B85-20CAB3C3C147",
                     "2.1.0-preview1-62414-02 net461"),
    IGNORED_ASSEMBLY(0x9DBB28A2, SYS_NET_HTTP,
                     "903A137B-BB3F-464A-94D4-780B89EE5580",
                     "2.1.0-preview1-62414-02 net471"),
    IGNORED_ASSEMBLY(0xA2E8EC53, SYS_RT_INTEROP_RUNTIME_INFO,
                     "6D334D4D-0149-4D07-9DEF-CC52213145CE",
                     "2.0.0-preview3-20170622-1 net461"),
    IGNORED_ASSEMBLY(0xA3BFE786, SYS_RT_INTEROP_RUNTIME_INFO,
                     "33D296D9-EE6D-404E-BF9F-432A429FF5DA",
                     "15.5.0-preview-20171027-2 net462"),
    IGNORED_ASSEMBLY(0xA99E866F, SYS_NET_HTTP,
                     "41ACE450-8F44-455A-97AC-0679E5462071",
                     "2.1.100-preview-62617-01 net471"),
    IGNORED_ASSEMBLY(0xAA21986B, SYS_THREADING_OVERLAPPED,
                     "9F5D4F09-787A-458A-BA08-553AA71470F1", "4.0.0 net46"),
    IGNORED_ASSEMBLY(0xAF2093B8, SYS_TEXT_ENC_CODEPAGES,
                     "D2B4F262-31A4-4E80-9CFB-26A2249A735E", "4.5.1 net461"),
    IGNORED_ASSEMBLY(0xC69BED92, SYS_IO_COMPRESSION,
                     "33AD8174-7781-46FA-A110-33821CCBE810",
                     "15.5.0-preview-20171027-2 net461"),
    IGNORED_ASSEMBLY(0xC8D00759, SYS_IO_COMPRESSION,
                     "1332CE2F-1517-4BD7-93FD-7D4BCFBAED66",
                     "2.0.0-preview3-20170622-1 net461"),
    IGNORED_ASSEMBLY(0xCA951D5B, SYS_RT_INTEROP_RUNTIME_INFO,
                     "1F37581E-4589-4C71-A465-05C6B9AE966E",
                     "2.1.0-preview1-62414-02 net461"),
    IGNORED_ASSEMBLY(0xD00F7419, SYS_THREADING_OVERLAPPED,
                     "3336A2A3-1772-4EF9-A74B-AFDC80A8B21E",
                     "2.1.0-preview1-62414-02 net471"),
    IGNORED_ASSEMBLY(0xD07383BB, SYS_RT_INTEROP_RUNTIME_INFO,
                     "DD91439F-3167-478E-BD2C-BF9C036A1395", "4.3.0 net45"),
    IGNORED_ASSEMBLY(0xD08A991A, SYS_NET_HTTP,
                     "82C79759-CB3C-4EB6-A17C-BDE85AF00A9B",
                     "15.5.0-preview-20171027-2 net461"),
    IGNORED_ASSEMBLY(0xD3ABE53A, SYS_RT_INTEROP_RUNTIME_INFO,
                     "488CE209-4E5D-40E7-BE8C-F81F2B99F13A",
                     "2.1.100-preview-62617-01 net462"),
    IGNORED_ASSEMBLY(0xDB9397A9, SYS_NET_HTTP,
                     "56203551-6937-47C1-9246-346A733913EE", "4.3.3 net46"),
    IGNORED_ASSEMBLY(0xE16ECCCD, SYS_GLOBALIZATION_EXT,
                     "1A2B9B2A-02F5-4C78-AB0C-7C6D2795CE2B",
                     "2.1.100-preview-62617-01 net471"),
    IGNORED_ASSEMBLY(0xE4016B17, SYS_GLOBALIZATION_EXT,
                     "50F4163A-D692-452F-90ED-2F8024BB5319",
                     "15.5.0-preview-20171027-2 net461"),
    IGNORED_ASSEMBLY(0xE758DAD4, SYS_IO_COMPRESSION,
                     "8DBD1669-97BC-4190-9BD8-738561193741",
                     "2.1.100-preview-62617-01 net471"),
    IGNORED_ASSEMBLY(0xF6D18A2E, SYS_TEXT_ENC_CODEPAGES,
                     "F5CCCBEC-E1AD-4DBB-9B44-9B42C86B94B8", "4.4.0 net461"),
    IGNORED_ASSEMBLY(0xF9D06E1E, SYS_GLOBALIZATION_EXT,
                     "FC1439FC-C1B8-4DB1-914D-165CCFA77002",
                     "2.1.0-preview1-62414-02 net461"),
    ...
    IGNORED_ASSEMBLY(0xFC67D3A7, SYS_RT_INTEROP_RUNTIME_INFO,
                     "FD6C8616-C1D8-43F9-AC17-A1C48A45FDA2",
                     "2.1.100-preview-62617-01 net47")};
...
gboolean mono_assembly_is_problematic_version(const char *name, guint16 major,
                                              guint16 minor, guint16 build,
                                              guint16 revision) {
  for (int i = 0; i < G_N_ELEMENTS(ignored_assembly_versions); ++i) {
    if (ignored_assembly_versions[i].major != major ||
      ...
      continue;
    if (!strcmp(ignored_assemblies_names[ignored_assembly_versions[i]
                                             .assembly_name],
                name))
      return TRUE;
  }
...
*/
static int hash_guid(const char *str) {
  int h = 5381;
  while (*str) {
    h = ((h << 5) + h) ^ *str;
    ++str;
...
gboolean mono_is_problematic_image(MonoImage *image) {
  ...
  // Either sort by hash and bseach or use SoA and make the linear search more
  // cache efficient.
  for (int i = 0; i < G_N_ELEMENTS(ignored_assemblies); ++i) {
    if (ignored_assemblies[i].hash == h &&
        !strcmp(image->guid, ignored_assemblies[i].guid)) {
      const char *needle =
          ignored_assemblies_file_names[ignored_assemblies[i].assembly_name];
      size_t needle_len = strlen(needle);
      size_t asm_len = strlen(image->name);
      if (asm_len > needle_len &&
          !g_ascii_strcasecmp(image->name + (asm_len - needle_len), needle))
        return TRUE;
    }
  }
  return FALSE;
...
static MonoImage *do_mono_image_load(MonoImage *image,
                                     MonoImageOpenStatus *status,
  ...
  if (!image->metadata_only) {
    ...
    if (image->loader == &pe_loader &&
        !mono_verifier_verify_pe_data(image, error))
      goto invalid_image;
  ...
  if (image->loader == &pe_loader && !image->metadata_only &&
      !mono_verifier_verify_cli_data(image, error))
    goto invalid_image;
  ...
  if (!image->ref_only && mono_is_problematic_image(image)) {
    if (image->load_from_context) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
                 "Loading problematic image %s", image->name);
    } else {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
                 "Denying load of problematic image %s", image->name);
      if (status)
        *status = MONO_IMAGE_IMAGE_INVALID;
      goto invalid_image;
    }
  ...
  if (image->loader == &pe_loader && !image->metadata_only &&
      !mono_verifier_verify_table_data(image, error))
    goto invalid_image;
...
invalid_image:
  if (!is_ok(error)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
               "Could not load image %s due to %s", image->name,
...
static gboolean mono_image_storage_trypublish(MonoImageStorage *candidate,
                                              MonoImageStorage **out_storage) {
  gboolean result;
  mono_images_storage_lock();
  MonoImageStorage *val = (MonoImageStorage *)g_hash_table_lookup(
      images_storage_hash, candidate->key);
  if (val) {
...
static void mono_image_storage_unpublish(MonoImageStorage *storage) {
  mono_images_storage_lock();
  g_assert(storage->ref.ref == 0);

  MonoImageStorage *published = (MonoImageStorage *)g_hash_table_lookup(
      images_storage_hash, storage->key);
...
static gboolean mono_image_storage_tryaddref(const char *key,
                                             MonoImageStorage **found) {
  gboolean result = FALSE;
  mono_images_storage_lock();
  MonoImageStorage *val =
      (MonoImageStorage *)g_hash_table_lookup(images_storage_hash, key);
  if (val) {
    mono_refcount_inc(val);
    *found = val;
    result = TRUE;
...
static void mono_image_storage_dtor(gpointer self) {
...
static void mono_image_storage_close(MonoImageStorage *storage) {
  mono_refcount_dec(storage);
}

static gboolean mono_image_init_raw_data(MonoImage *image,
                                         const MonoImageStorage *storage) {
  if (!storage)
    return FALSE;
...
static MonoImageStorage *mono_image_storage_open(const char *fname) {
  char *key = NULL;
  ...
  storage->raw_data = (char *)mono_file_map(
      storage->raw_data_len, MONO_MMAP_READ | MONO_MMAP_PRIVATE,
      mono_file_map_fd(filed), 0, &storage->raw_data_handle);
#if defined(HAVE_MMAP) && !defined(HOST_WIN32)
  if (!storage->raw_data) {
    storage->fileio_used = TRUE;
    storage->raw_data = (char *)mono_file_map_fileio(
        storage->raw_data_len, MONO_MMAP_READ | MONO_MMAP_PRIVATE,
        mono_file_map_fd(filed), 0, &storage->raw_data_handle);
  }
#endif
...
mono_image_storage_new_raw_data(char *datac, guint32 data_len,
                                gboolean raw_data_allocated, const char *name) {
  char *key =
      (name == NULL) ? g_strdup_printf("data-%p", datac) : g_strdup(name);
  MonoImageStorage *published_storage = NULL;
  if (mono_image_storage_tryaddref(key, &published_storage)) {
    g_free(key);
    return published_storage;
...
static MonoImage *
do_mono_image_open(MonoAssemblyLoadContext *alc, const char *fname,
                   MonoImageOpenStatus *status, gboolean care_about_cli,
                   gboolean care_about_pecoff, gboolean refonly,
                   gboolean metadata_only, gboolean load_from_context) {
  MonoCLIImageInfo *iinfo;
  MonoImage *image;
  ...
  image->ref_count = 1;
  /* if MONO_SECURITY_MODE_CORE_CLR is set then determine if this image is
   * platform code */
  image->core_clr_platform_code =
 ...
 * It checks either reflection-only loads only, or normal loads only, as
 ...
 * \returns the loaded \c MonoImage, or NULL on failure.
 */
MonoImage *mono_image_loaded_internal(MonoAssemblyLoadContext *alc,
                                      const char *name, gboolean refonly) {
  MonoLoadedImages *li = mono_alc_get_loaded_images(alc);
  MonoImage *res;

  mono_images_lock();
  res = (MonoImage *)g_hash_table_lookup(
      mono_loaded_images_get_hash(li, refonly), name);
  if (!res)
    res = (MonoImage *)g_hash_table_lookup(
        mono_loaded_images_get_by_name_hash(li, refonly), name);
  mono_images_unlock();
 ...
 * mono_image_loaded:
 * \param name path or assembly name of the image to load
 * This routine verifies that the given image is loaded. Reflection-only loads
 * do not count. \returns the loaded \c MonoImage, or NULL on failure.
 */
MonoImage *mono_image_loaded(const char *name) {
  ...
  MonoDomain *domain = mono_domain_get();
  result =
      mono_image_loaded_internal(mono_domain_default_alc(domain), name, FALSE);
  MONO_EXIT_GC_UNSAFE;
  return result;
}
...
static void find_by_guid(gpointer key, gpointer val, gpointer user_data) {
...
static MonoImage *mono_image_loaded_by_guid_internal(const char *guid,
                                                     gboolean refonly);
...
static MonoImage *mono_image_loaded_by_guid_internal(const char *guid,
  ...
  return data.res;
#else
  /* TODO: Maybe implement this for netcore by searching only the default ALC of
   * the current domain */
  return NULL;
#endif
}
...
static MonoImage *register_image(MonoLoadedImages *li, MonoImage *image,
                                 gboolean *problematic) {
  MonoImage *image2;
  ...
  GHashTable *loaded_images_by_name =
      mono_loaded_images_get_by_name_hash(li, image->ref_only);
  g_hash_table_insert(loaded_images, image->name, image);
  if (image->assembly_name &&
      (g_hash_table_lookup(loaded_images_by_name, image->assembly_name) ==
       NULL))
    g_hash_table_insert(loaded_images_by_name, (char *)image->assembly_name,
                        image);
  mono_images_unlock();

  if (mono_is_problematic_image(image)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
               "Registering %s, problematic image '%s'",
               image->ref_only ? "REFONLY" : "default", image->name);
    if (problematic)
      *problematic = TRUE;
  }
  return image;
...
MonoImage *mono_image_open_from_data_internal(
    MonoAssemblyLoadContext *alc, char *data, guint32 data_len,
    gboolean need_copy, MonoImageOpenStatus *status, gboolean refonly,
    gboolean metadata_only, const char *name) {
  MonoCLIImageInfo *iinfo;
  MonoImage *image;
  char *datac;
  ...
  MonoImageStorage *storage =
      mono_image_storage_new_raw_data(datac, data_len, need_copy, name);
  image = g_new0(MonoImage, 1);
  image->storage = storage;
  mono_image_init_raw_data(image, storage);
  image->name =
 ...
 */
MonoImage *mono_image_open_from_data_with_name(char *data, guint32 data_len,
                                               gboolean need_copy,
                                               MonoImageOpenStatus *status,
                                               gboolean refonly,
                                               const char *name) {
  ...
  MONO_ENTER_GC_UNSAFE;
  MonoDomain *domain = mono_domain_get();
  result = mono_image_open_from_data_internal(mono_domain_default_alc(domain),
                                              data, data_len, need_copy, status,
                                              refonly, FALSE, name);
  MONO_EXIT_GC_UNSAFE;
  return result;
}
...
#ifdef HOST_WIN32
static MonoImageStorage *mono_image_storage_open_from_module_handle(
    HMODULE module_handle, const char *fname, gboolean has_entry_point) {
...
MonoImage *mono_image_open_from_module_handle(MonoAssemblyLoadContext *alc,
  ...
  MonoImageStorage *storage = mono_image_storage_open_from_module_handle(
      module_handle, fname, has_entry_point);
  image = g_new0(MonoImage, 1);
  image->storage = storage;
  mono_image_init_raw_data(image, storage);
  iinfo = g_new0(MonoCLIImageInfo, 1);
 ...
 */
MonoImage *mono_image_open_full(const char *fname, MonoImageOpenStatus *status,
                                gboolean refonly) {
...
static MonoImage *mono_image_open_a_lot_parameterized(
...
#ifdef HOST_WIN32
  // Win32 path: If we are running with mixed-mode assemblies enabled (ie have
  // loaded mscoree.dll), then assemblies need to be loaded with LoadLibrary:
  if (!refonly && coree_module_handle) {
    ...
    /* There is little overhead because the OS loader lock is held by
     * LoadLibrary. */
    mono_images_lock();
    ...
    if (image) { // Image already loaded
      ...
      if (m_image_has_entry_point(image) && image->ref_count == 0) {
        ...
        /* The image is already loaded because _CorDllMain removes images from
         * the hash. */
    ...
    return mono_image_open_from_module_handle(alc, module_handle, absfname,
                                              FALSE, status);
  }
#endif
  ...
  if (image) { // Image already loaded
    if (!refonly && !load_from_context && mono_is_problematic_image(image)) {
      ...
      mono_images_unlock();
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
                 "Not returning problematic image '%s' refcount=%d",
                 image->name, image->ref_count);
      if (problematic)
        *problematic = TRUE;
      return NULL;
    }
  ...
  // Image not loaded, load it now
  image = do_mono_image_open(alc, fname, status, TRUE, TRUE, refonly, FALSE,
                             load_from_context);
  if (image == NULL)
    return NULL;
...
MonoImage *mono_image_open_a_lot(MonoAssemblyLoadContext *alc,
                                 const char *fname, MonoImageOpenStatus *status,
                                 gboolean refonly, gboolean load_from_context) {
  MonoLoadedImages *li = mono_alc_get_loaded_images(alc);
  return mono_image_open_a_lot_parameterized(li, alc, fname, status, refonly,
                                             load_from_context, NULL);
}
...
gboolean mono_is_problematic_file(const char *fname) {
  ...
  MonoLoadedImages *li = mono_alc_get_loaded_images(alc);
  MonoImage *opened = mono_image_open_a_lot_parameterized(
      li, alc, fname, &status, FALSE, FALSE, &problematic);
  if (opened)
 ...
 * The caller holds a temporary reference to the returned image which should be
 * cleared when no longer needed by calling \c mono_image_close. if NULL, then
 * check the value of \p status for details on the error
 */
 ...
 */
MonoImage *mono_image_open_raw(MonoAssemblyLoadContext *alc, const char *fname,
                               MonoImageOpenStatus *status) {
  g_return_val_if_fail(fname != NULL, NULL);
 ...
 */
MonoImage *mono_image_open_metadata_only(MonoAssemblyLoadContext *alc,
                                         const char *fname,
                                         MonoImageOpenStatus *status) {
...
void mono_image_fixup_vtable(MonoImage *image) {
  ...
  while (count--) {
    ...
    if (slot_type & VTFIXUP_TYPE_32BIT)
      while (slot_count--) {
        *((guint32 *)slot) = (guint32)(gsize)mono_marshal_get_vtfixup_ftnptr(
            image, *((guint32 *)slot), slot_type);
        slot = ((guint32 *)slot) + 1;
      }
    else if (slot_type & VTFIXUP_TYPE_64BIT)
      while (slot_count--) {
        *((guint64 *)slot) = (guint64)mono_marshal_get_vtfixup_ftnptr(
            image, *((guint64 *)slot), slot_type);
        slot = ((guint32 *)slot) + 1;
      }
    else
      g_assert_not_reached();
...
static void free_hash_table(gpointer key, gpointer val, gpointer user_data) {
...
static void free_array_cache_entry(gpointer key, gpointer val,
                                   gpointer user_data) {
...
static void free_hash(GHashTable *hash) {
  if (hash)
...
static void mono_image_close_except_pools_all(MonoImage **images,
                                              int image_count) {
...
gboolean mono_image_close_except_pools(MonoImage *image) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY, "Unloading image %s [%p].",
             image->name, image);
  ...
  /*
   * The caches inside a MonoImage might refer to metadata which is stored in
   * referenced assemblies, so we can't release these references in
   * mono_assembly_close () since the MonoImage might outlive its associated
   * MonoAssembly.
   */
  ...
  if (image->storage && m_image_is_raw_data_allocated(image)) {
    ...
    for (i = 0; i < ii->cli_section_count; i++)
      if (((char *)(ii->cli_sections[i]) > image->raw_data) &&
          ((char *)(ii->cli_sections[i]) <=
           ((char *)image->raw_data + image->raw_data_len)))
  ...
  /*
  reflection_info_unregister_classes is only required by dynamic images, which
  will not be properly cleared during shutdown as we don't perform regular
  appdomain unload for the root one.
  */
  g_assert(!image->reflection_info_unregister_classes ||
...
static void mono_image_close_all(MonoImage **images, int image_count) {
  for (int i = 0; i < image_count; ++i) {
    if (images[i])
...
void mono_image_close_finish(MonoImage *image) {
  ...
  mono_atomic_fetch_add_i32(&mono_perfcounters->loader_bytes,
                            -1 * mono_mempool_get_allocated(image->mempool));
#endif
...
static gpointer mono_image_walk_resource_tree(
    MonoCLIImageInfo *info, guint32 res_id, guint32 lang_id, gunichar2 *name,
    MonoPEResourceDirEntry *entry, MonoPEResourceDir *root, guint32 level) {
  ...
  if (is_dir) {
    MonoPEResourceDir *res_dir =
        (MonoPEResourceDir *)(((char *)root) + dir_offset);
    MonoPEResourceDirEntry *sub_entries =
        (MonoPEResourceDirEntry *)(res_dir + 1);
    guint32 entries, i;

    entries = GUINT16_FROM_LE(res_dir->res_named_entries) +
              GUINT16_FROM_LE(res_dir->res_id_entries);

    for (i = 0; i < entries; i++) {
      MonoPEResourceDirEntry *sub_entry = &sub_entries[i];
      gpointer ret;

      ret = mono_image_walk_resource_tree(info, res_id, lang_id, name,
                                          sub_entry, root, level + 1);
      if (ret != NULL) {
        return (ret);
      }
    }
    ...
    MonoPEResourceDataEntry *data_entry =
        (MonoPEResourceDataEntry *)((char *)(root) + dir_offset);
 ...
 * \param res_id A \c MONO_PE_RESOURCE_ID_ that represents the resource ID to
 * lookup. \param lang_id The language id. \param name the resource name to
 * lookup. \returns NULL if not found, otherwise a pointer to the in-memory
 * representation of the given resource. The caller should free it using \c
...
gpointer mono_image_lookup_resource(MonoImage *image, guint32 res_id,
  ...
  entries = GUINT16_FROM_LE(resource_dir->res_named_entries) +
            GUINT16_FROM_LE(resource_dir->res_id_entries);
  ...
  for (i = 0; i < entries; i++) {
    ...
    ret = mono_image_walk_resource_tree(info, res_id, lang_id, name, entry,
                                        resource_dir, 0);
    if (ret != NULL) {
      return (ret);
    }
 ...
 */
const char *mono_image_get_resource(MonoImage *image, guint32 offset,
                                    guint32 *size) {
  MonoCLIImageInfo *iinfo = image->image_info;
...
MonoImage *mono_image_load_file_for_image_checked(MonoImage *image, int fileidx,
                                                  MonoError *error) {
  char *base_dir, *name;
  MonoImage *res;
...
MonoImage *mono_image_load_file_for_image(MonoImage *image, int fileidx) {
  ERROR_DECL(error);
  MonoImage *result =
      mono_image_load_file_for_image_checked(image, fileidx, error);
  mono_error_assert_ok(error);
  return result;
}
...
const char *mono_image_get_public_key(MonoImage *image, guint32 *size) {
  ...
  if (image->tables[MONO_TABLE_ASSEMBLY].rows != 1)
    return NULL;
  tok = mono_metadata_decode_row_col(&image->tables[MONO_TABLE_ASSEMBLY], 0,
 ...
 */
const char *mono_image_get_filename(MonoImage *image) { return image->name; }

/**
 * mono_image_get_guid:
 */
const char *mono_image_get_guid(MonoImage *image) { return image->guid; }
 ...
 */
int mono_table_info_get_rows(const MonoTableInfo *table) { return table->rows; }
...
char *mono_image_strdup_vprintf(MonoImage *image, const char *format,
                                va_list args) {
  char *buf;
  mono_image_lock(image);
...
char *mono_image_strdup_printf(MonoImage *image, const char *format, ...) {
  char *buf;
  va_list args;
...
GSList *mono_g_slist_append_image(MonoImage *image, GSList *list,
                                  gpointer data) {
  GSList *new_list;
 ...
 * Lookup a property on \p image . Used to store very rare fields of \c
 * MonoClass and \c MonoMethod .
 *
 * LOCKING: Takes the image lock
 */
gpointer mono_image_property_lookup(MonoImage *image, gpointer subject,
                                    guint32 property) {
  gpointer res;
 ...
 * LOCKING: Takes the image lock
 */
void mono_image_property_insert(MonoImage *image, gpointer subject,
                                guint32 property, gpointer value) {
 ...
 * Remove all properties associated with \p subject in \p image. Used to store
 * very rare fields of \c MonoClass and \c MonoMethod .
 *
 * LOCKING: Takes the image lock
 */
...
void mono_image_append_class_to_reflection_info_set(MonoClass *klass) {
  ...
  mono_image_lock(image);
  image->reflection_info_unregister_classes = g_slist_prepend_mempool(
      image->mempool, image->reflection_info_unregister_classes, klass);
  mono_image_unlock(image);
}
