...
/* If set, check that there are no references to the domain left at domain
 * unload */
...
static gboolean ptr_on_stack(void *ptr) {
...
static void scan_object_for_binary_protocol_copy_wbarrier(gpointer dest,
                                                          char *start,
                                                          mword desc) {
...
void mono_gc_wbarrier_value_copy_internal(gpointer dest, gconstpointer src,
                                          int count, MonoClass *klass) {
  HEAVY_STAT(++stat_wbarrier_value_copy);
  g_assert(m_class_is_valuetype(klass));

  SGEN_LOG(8, "Adding value remset at %p, count %d, descr %p for class %s (%p)",
           dest, count, (gpointer)(uintptr_t)m_class_get_gc_descr(klass),
           m_class_get_name(klass), klass);

  if (sgen_ptr_in_nursery(dest) || ptr_on_stack(dest) ||
      !sgen_gc_descr_has_references((mword)m_class_get_gc_descr(klass))) {
  ...
  if (sgen_binary_protocol_is_heavy_enabled()) {
    ...
    int i;
    for (i = 0; i < count; ++i) {
      scan_object_for_binary_protocol_copy_wbarrier(
          (char *)dest + i * element_size,
  ...
  sgen_get_remset()->wbarrier_value_copy(dest, src, count,
 ...
 *
 * Write barrier to call when \p obj is the result of a clone or copy of an
 * object.
 */
void mono_gc_wbarrier_object_copy_internal(MonoObject *obj, MonoObject *src) {
  ...
  SGEN_ASSERT(6, !ptr_on_stack(obj),
  ...
  if (sgen_ptr_in_nursery(obj) || !SGEN_OBJECT_HAS_REFERENCES(src)) {
    ...
    mono_gc_memmove_aligned((char *)obj + MONO_ABI_SIZEOF(MonoObject),
                            (char *)src + MONO_ABI_SIZEOF(MonoObject),
...
#ifdef SGEN_HEAVY_BINARY_PROTOCOL
  if (sgen_binary_protocol_is_heavy_enabled())
    scan_object_for_binary_protocol_copy_wbarrier(obj, (char *)src,
                                                  (mword)src->vtable->gc_descr);
#endif
 ...
 */
void mono_gc_wbarrier_set_arrayref_internal(MonoArray *arr, gpointer slot_ptr,
                                            MonoObject *value) {
 ...
 */
void mono_gc_wbarrier_set_field_internal(MonoObject *obj, gpointer field_ptr,
                                         MonoObject *value) {
  mono_gc_wbarrier_set_arrayref_internal((MonoArray *)obj, field_ptr, value);
}

void mono_gc_wbarrier_range_copy(gpointer _dest, gconstpointer _src, int size) {
...
gboolean sgen_has_critical_method(void) { return sgen_has_managed_allocator(); }
...
void mono_install_sgen_mono_callbacks(MonoSgenMonoCallbacks *cb) {
  g_assert(!cb_inited);
...
static void emit_nursery_check_noilgen(MonoMethodBuilder *mb,
                                       gboolean is_concurrent) {}

static void emit_managed_allocator_noilgen(MonoMethodBuilder *mb,
                                           gboolean slowpath, gboolean profiler,
                                           int atype) {}
...
static MonoSgenMonoCallbacks *get_sgen_mono_cb(void) {
...
MonoMethod *mono_gc_get_specific_write_barrier(gboolean is_concurrent) {
  ...
  if (is_concurrent)
    mb = mono_mb_new(mono_defaults.object_class, "wbarrier_conc",
                     MONO_WRAPPER_WRITE_BARRIER);
  else
    mb = mono_mb_new(mono_defaults.object_class, "wbarrier_noconc",
                     MONO_WRAPPER_WRITE_BARRIER);
...
static GCVTable get_array_fill_vtable(void) {
...
static gboolean is_finalization_aware(MonoObject *obj) {
  MonoVTable *vt = SGEN_LOAD_VTABLE(obj);
  return (vt->gc_bits & SGEN_GC_BIT_FINALIZER_AWARE) ==
         SGEN_GC_BIT_FINALIZER_AWARE;
}

void sgen_client_object_queued_for_finalization(GCObject *obj) {
  if (fin_callbacks.object_queued_for_finalization &&
  ...
  if (G_UNLIKELY(MONO_GC_FINALIZE_ENQUEUE_ENABLED())) {
    ...
    MONO_GC_FINALIZE_ENQUEUE((mword)obj, sgen_safe_object_get_size(obj),
                             ...
                             sgen_client_vtable_get_name(vt), gen,
...
void sgen_client_finalize_notify(void) { mono_gc_finalize_notify(); }

void mono_gc_register_for_finalization(MonoObject *obj,
                                       MonoFinalizationProc user_data) {
  sgen_object_register_for_finalization(obj, user_data);
}

static gboolean object_in_domain_predicate(MonoObject *obj, void *user_data) {
  MonoDomain *domain = (MonoDomain *)user_data;
  if (mono_object_domain(obj) == domain) {
    SGEN_LOG(5, "Unregistering finalizer for object: %p (%s)", obj,
 ...
 * Enqueue for finalization all objects that belong to the unloading appdomain
 * \p domain. \p suspend is used for early termination of the enqueuing process.
 */
void mono_gc_finalize_domain(MonoDomain *domain) {
  sgen_finalize_if(object_in_domain_predicate, domain);
}

void mono_gc_suspend_finalizers(void) { sgen_set_suspend_finalizers(); }
...
static MONO_PERMIT(need(sgen_gc_locked)) void null_ephemerons_for_domain(
    MonoDomain *domain) {
...
void sgen_client_clear_unreachable_ephemerons(ScanCopyContext ctx) {
  ...
  while (current) {
    ...
    SGEN_LOG(5, "Clearing unreachable entries for ephemeron array at %p",
             array);

    cur = mono_array_addr_internal(array, Ephemeron, 0);
    array_end = cur + mono_array_length_internal(array);
    tombstone =
        SGEN_LOAD_VTABLE((GCObject *)array)->domain->ephemeron_tombstone;
    ...
    for (; cur < array_end; ++cur) {
      ...
      SGEN_LOG(5, "[%" G_GSIZE_FORMAT "d] key %p (%s) value %p (%s)",
               cur - mono_array_addr_internal(array, Ephemeron, 0), key,
               sgen_is_object_alive_for_current_gen(key) ? "reachable"
                                                         : "unreachable",
               cur->value,
               cur->value && sgen_is_object_alive_for_current_gen(cur->value)
                   ? "reachable"
                   : "unreachable");
...
Limitations: We scan all ephemerons on every collection since the current design
doesn't allow for a simple nursery/mature split.
*/
...
static gboolean need_remove_object_for_domain(GCObject *start,
                                              MonoDomain *domain) {
  if (mono_object_domain(start) == domain) {
    SGEN_LOG(4, "Need to cleanup object %p", start);
    sgen_binary_protocol_cleanup(start, (gpointer)SGEN_LOAD_VTABLE(start),
                                 sgen_safe_object_get_size((GCObject *)start));
    return TRUE;
  }
  return FALSE;
}

static void process_object_for_domain_clearing(GCObject *start,
                                               MonoDomain *domain) {
  ...
  if (m_class_get_supertypes(mono_defaults.real_proxy_class) &&
      mono_class_has_parent_fast(vt->klass, mono_defaults.real_proxy_class)) {
    ...
    if (server &&
        (!SGEN_LOAD_VTABLE(server) || mono_object_domain(server) == domain)) {
      SGEN_LOG(4, "Cleaning up remote pointer in %p to object %p", start,
               server);
...
static gboolean clear_domain_process_object(GCObject *obj, MonoDomain *domain) {
  gboolean remove;
...
static void clear_domain_process_minor_object_callback(GCObject *obj,
                                                       size_t size,
                                                       MonoDomain *domain) {
...
static void clear_domain_process_major_object_callback(GCObject *obj,
                                                       size_t size,
                                                       MonoDomain *domain) {
...
static void
clear_domain_free_major_non_pinned_object_callback(GCObject *obj, size_t size,
                                                   MonoDomain *domain) {
  if (need_remove_object_for_domain(obj, domain))
    sgen_major_collector.free_non_pinned_object(obj, size);
}

static void clear_domain_free_major_pinned_object_callback(GCObject *obj,
                                                           size_t size,
                                                           MonoDomain *domain) {
  if (need_remove_object_for_domain(obj, domain))
...
static void clear_domain_process_los_object_callback(GCObject *obj, size_t size,
                                                     MonoDomain *domain) {
  clear_domain_process_object(obj, domain);
}

static gboolean clear_domain_free_los_object_callback(GCObject *obj,
                                                      size_t size,
                                                      MonoDomain *domain) {
  return need_remove_object_for_domain(obj, domain);
}

static void sgen_finish_concurrent_work(const char *reason, gboolean stw) {
  if (sgen_get_concurrent_collection_in_progress())
    sgen_perform_collection(0, GENERATION_OLD, reason, TRUE, stw);
  SGEN_ASSERT(0, !sgen_get_concurrent_collection_in_progress(),
              "We just ordered a synchronous collection.  Why are we "
...
/*
 * When appdomains are unloaded we can easily remove objects that have
 * finalizers, but all the others could still be present in random places on the
 * heap. We need a sweep to get rid of them even though it's going to be costly
 * with big heaps.
 * The reason we need to remove them is because we access the vtable and class
 * structures to know the object size and the reference bitmap: once the domain
 * is unloaded the point to random memory.
 */
void mono_gc_clear_domain(MonoDomain *domain) {
  ...
  /*Ephemerons and dislinks must be processed before LOS since they might end up
  pointing to memory returned to the OS.*/
  ...
  sgen_scan_area_with_callback(
      ...
      (IterateObjectCallbackFunc)clear_domain_process_minor_object_callback,
      domain, FALSE, TRUE);
  ...
  sgen_major_collector.iterate_objects(
      ITERATE_OBJECTS_SWEEP_ALL,
      (IterateObjectCallbackFunc)clear_domain_process_major_object_callback,
      domain);

  sgen_los_iterate_objects(
      (IterateObjectCallbackFunc)clear_domain_process_los_object_callback,
      domain);
  sgen_los_iterate_objects_free(
      (IterateObjectResultCallbackFunc)clear_domain_free_los_object_callback,
      domain);

  sgen_major_collector.iterate_objects(
      ITERATE_OBJECTS_SWEEP_NON_PINNED,
      (IterateObjectCallbackFunc)
          clear_domain_free_major_non_pinned_object_callback,
      domain);
  sgen_major_collector.iterate_objects(
      ITERATE_OBJECTS_SWEEP_PINNED,
      (IterateObjectCallbackFunc)clear_domain_free_major_pinned_object_callback,
      domain);
 ...
 * mono_gc_alloc_fixed:
 */
MonoObject *mono_gc_alloc_fixed(size_t size, MonoGCDescriptor descr,
                                MonoGCRootSource source, void *key,
                                const char *msg) {
...
MonoObject *mono_gc_alloc_fixed_no_descriptor(size_t size,
                                              MonoGCRootSource source,
                                              void *key, const char *msg) {
...
/* FIXME: Do this in the JIT, where specialized allocation sequences can be
 * created for each class. This is currently not easy to do, as it is hard to
 * generate basic blocks + branches, but it is easy with the linear IL codebase.
 *
...
static MonoMethod *create_allocator(int atype,
  ...
  if (atype == ATYPE_SMALL) {
    name = slowpath ? "SlowAllocSmall"
                    : (profiler ? "ProfilerAllocSmall" : "AllocSmall");
  } else if (atype == ATYPE_NORMAL) {
    name = slowpath ? "SlowAlloc" : (profiler ? "ProfilerAlloc" : "Alloc");
  } else if (atype == ATYPE_VECTOR) {
    name = slowpath ? "SlowAllocVector"
                    : (profiler ? "ProfilerAllocVector" : "AllocVector");
  } else if (atype == ATYPE_STRING) {
    name = slowpath ? "SlowAllocString"
                    : (profiler ? "ProfilerAllocString" : "AllocString");
  } else {
 ...
 * (). The signature of the called method is: object allocate (MonoVTable
 * *vtable)
 */
MonoMethod *mono_gc_get_managed_allocator(MonoClass *klass, gboolean for_box,
  ...
  if (known_instance_size &&
      ALIGN_TO(m_class_get_instance_size(klass), SGEN_ALLOC_ALIGN) >=
...
MonoMethod *mono_gc_get_managed_array_allocator(MonoClass *klass) {
  ...
  return mono_gc_get_managed_allocator_by_type(
      ATYPE_VECTOR, mono_profiler_allocations_enabled()
                        ? MANAGED_ALLOCATOR_PROFILER
                        : MANAGED_ALLOCATOR_REGULAR);
#else
  return NULL;
...
mono_gc_get_managed_allocator_by_type(int atype,
  ...
  switch (variant) {
  case MANAGED_ALLOCATOR_REGULAR:
    cache = alloc_method_cache;
    break;
  case MANAGED_ALLOCATOR_SLOW_PATH:
    cache = slowpath_alloc_method_cache;
    break;
  case MANAGED_ALLOCATOR_PROFILER:
    cache = profiler_alloc_method_cache;
    break;
  default:
    g_assert_not_reached();
    break;
...
guint32 mono_gc_get_managed_allocator_types(void) { return ATYPE_NUM; }
...
gboolean sgen_is_managed_allocator(MonoMethod *method) {
  ...
  for (i = 0; i < ATYPE_NUM; ++i)
    if (method == alloc_method_cache[i] ||
        method == slowpath_alloc_method_cache[i] ||
        method == profiler_alloc_method_cache[i])
      return TRUE;
  return FALSE;
}
...
gboolean sgen_has_managed_allocator(void) {
  ...
  for (i = 0; i < ATYPE_NUM; ++i)
    if (alloc_method_cache[i] || slowpath_alloc_method_cache[i] ||
        profiler_alloc_method_cache[i])
      return TRUE;
  return FALSE;
}

#define ARRAY_OBJ_INDEX(ptr, array, elem_size)                                 \
  (((char *)(ptr) - ((char *)(array) + G_STRUCT_OFFSET(MonoArray, vector))) /  \
   (elem_size))

gboolean sgen_client_cardtable_scan_object(GCObject *obj, guint8 *cards,
                                           ScanCopyContext ctx) {
  MonoVTable *vt = SGEN_LOAD_VTABLE(obj);
  MonoClass *klass = vt->klass;

  SGEN_ASSERT(0, SGEN_VTABLE_HAS_REFERENCES(vt),
  ...
  if (vt->rank) {
    ...
    mword bounds_size;
    mword obj_size = sgen_mono_array_size(vt, arr, &bounds_size,
                                          sgen_vtable_get_descriptor(vt));
    /* We don't want to scan the bounds entries at the end of multidimensional
     * arrays */
  ...
  LOOP_HEAD:
    ...
    for (; card_data < card_data_end;
      ...
      elem = first_elem = (char *)mono_array_addr_with_size_fast(
          (MonoArray *)obj, elem_size, index);
      if (m_class_is_valuetype(m_class_get_element_class(klass))) {
        ScanVTypeFunc scan_vtype_func = ctx.ops->scan_vtype;

        for (; elem < card_end; elem += elem_size)
          scan_vtype_func(obj, elem, desc,
                          ctx.queue BINARY_PROTOCOL_ARG(elem_size));
      } else {
...
#ifdef SGEN_HAVE_OVERLAPPING_CARDS
    if (card_count > 0) {
      SGEN_ASSERT(0, card_data == SGEN_SHADOW_CARDTABLE_END,
...
MonoArray *mono_gc_alloc_pinned_vector(MonoVTable *vtable, size_t size,
  ...
  SGEN_ASSERT(6,
              SGEN_ALIGN_UP(size) ==
                  SGEN_ALIGN_UP(
                      sgen_client_par_object_get_size(vtable, (GCObject *)arr)),
              "Vector has incorrect size.");
  return arr;
}

MonoArray *mono_gc_alloc_vector(MonoVTable *vtable, size_t size,
                                uintptr_t max_length) {
  MonoArray *arr;
  TLAB_ACCESS_INIT;
  ...
  if (arr) {
    /*This doesn't require fencing since EXIT_CRITICAL_REGION already does it
     * for us*/
    arr->max_length = (mono_array_size_t)max_length;
...
MonoArray *mono_gc_alloc_array(MonoVTable *vtable, size_t size,
                               uintptr_t max_length, uintptr_t bounds_size) {
  MonoArray *arr;
...
void mono_gc_set_string_length(MonoString *str, gint32 new_length) {
  ...
  if (sgen_nursery_canaries_enabled() && sgen_ptr_in_nursery(str)) {
    ...
    memset(new_end, 0,
           (str->length - new_length + 1) * sizeof(mono_unichar2) +
               CANARY_SIZE);
...
static void notify_gc_roots(GCRootReport *report) {
  if (!report->count)
    return;
  MONO_PROFILER_RAISE(gc_roots, (report->count,
                                 (const mono_byte *const *)report->addresses,
                                 (MonoObject *const *)report->objects));
  report->count = 0;
}

static void report_gc_root(GCRootReport *report, void *address, void *object) {
...
static void single_arg_report_root(MonoObject **obj, void *gc_data) {
  GCRootReport *report = (GCRootReport *)gc_data;
  if (*obj)
    report_gc_root(report, obj, *obj);
}

static void two_args_report_root(void *address, MonoObject *obj,
                                 void *gc_data) {
  GCRootReport *report = (GCRootReport *)gc_data;
  if (obj)
...
static void precisely_report_roots_from(GCRootReport *report, void **start_root,
                                        void **end_root, mword desc) {
  ...
  case ROOT_DESC_USER: {
    MonoGCRootMarkFunc marker =
        (MonoGCRootMarkFunc)sgen_get_user_descriptor_func(desc);
...
static void report_pinning_roots(GCRootReport *report, void **start,
                                 void **end) {
  while (start < end) {
...
static GCObject *find_pinned_obj(char *addr) {
  ...
  if (addr > (char *)obj &&
      addr < ((char *)obj + sgen_safe_object_get_size(obj)))
    return obj;
  return NULL;
}

/*
 * We pass @root_report_address so register are properly accounted towards their
 * thread
 */
static void report_conservative_roots(GCRootReport *report,
                                      void *root_report_address, void **start,
                                      void **end) {
  while (start < end) {
    mword addr = (mword)*start;
    addr &= ~(SGEN_ALLOC_ALIGN - 1);
...
static void report_handle_stack_root(gpointer *ptr, gpointer user_data) {
...
static void report_handle_stack_roots(GCRootReport *report,
                                      SgenThreadInfo *info, gboolean precise) {
  ...
  mono_handle_stack_scan(info->client_info.info.handle_stack,
                         report_handle_stack_root, &ud, ud.precise, FALSE);
}

static void *get_aligned_stack_start(SgenThreadInfo *info) {
  void *aligned_stack_start = (void *)(mword)ALIGN_TO(
      (mword)info->client_info.stack_start, SIZEOF_VOID_P);
#if _WIN32
  // Due to the guard page mechanism providing gradual commit of Windows stacks,
  // stack pages must be touched in order.
  //
  // This mechanism is only transparent (kernel handles page faults and user
  // never sees them), for the thread touching its own stack. Not for
  // cross-thread stack references as are being done here.
  //
  ...
  // SIZE_T result = VirtualQuery (info->client_info.stack_start, &mem_info,
  // sizeof(mem_info)); g_assert (result != 0); if (mem_info.Protect &
  // PAGE_GUARD) { 	aligned_stack_start = ((char*) mem_info.BaseAddress) +
  // mem_info.RegionSize;
  // }
  //
  // VirtualQuery not historically allowed in UWP, but it is now.
  //
  // 2. Touch page under __try / __except and handle
  ...
  // } __except (GetExceptionCode () == STATUS_GUARD_PAGE_VIOLATION) {
  // 	MEMORY_BASIC_INFORMATION mem_info;
  // 	const SIZE_T result = VirtualQuery(aligned_stack_start, &mem_info,
  // sizeof(mem_info)); 	g_assert (result >= sizeof (mem_info)); 	VirtualProtect
  // (aligned_stack_start, 1, mem_info.Protect | PAGE_GUARD, &mem_info.Protect);
  // }
  //
  ...
  //
  aligned_stack_start =
      MAX(aligned_stack_start, info->client_info.info.windows_tib->StackLimit);
#endif
  return aligned_stack_start;
}

static void report_stack_roots(void) {
  ...
  FOREACH_THREAD_EXCLUDE(info, MONO_THREAD_INFO_FLAGS_NO_GC) {
    ...
    report_conservative_roots(&report, aligned_stack_start,
                              (void **)aligned_stack_start,
                              (void **)info->client_info.info.stack_end);
    report_conservative_roots(&report, aligned_stack_start,
                              (void **)&info->client_info.ctx,
                              (void **)(&info->client_info.ctx + 1));
...
static void report_pin_queue(void) {
...
static void report_finalizer_roots_from_queue(SgenPointerQueue *queue,
                                              void *queue_address) {
  GCRootReport report;
  size_t i;
...
static void report_registered_roots_by_type(int root_type) {
  GCRootReport report = {0};
  void **start_root;
  RootRecord *root;
  report.count = 0;
  SGEN_HASH_TABLE_FOREACH(&sgen_roots_hash[root_type], void **, start_root,
                          RootRecord *, root) {
    SGEN_LOG(6, "Profiler root scan %p-%p (desc: %p)", start_root,
             root->end_root, (void *)(intptr_t)root->root_desc);
    if (root_type == ROOT_TYPE_PINNED)
      report_pinning_roots(&report, start_root, (void **)root->end_root);
    else
      precisely_report_roots_from(&report, start_root, (void **)root->end_root,
                                  root->root_desc);
  }
...
static void report_ephemeron_roots(void) {
  EphemeronLinkNode *current = ephemeron_list;
  Ephemeron *cur, *array_end;
  GCObject *tombstone;
  GCRootReport report = {0};
...
static void report_toggleref_root(MonoObject *obj, gpointer data) {
  report_gc_root((GCRootReport *)data, SPECIAL_ADDRESS_TOGGLEREF, obj);
}

static void report_toggleref_roots(void) {
  GCRootReport report = {0};
  sgen_foreach_toggleref_root(report_toggleref_root, &report);
  notify_gc_roots(&report);
}

static void sgen_report_all_roots(SgenPointerQueue *fin_ready_queue,
                                  SgenPointerQueue *critical_fin_queue) {
...
void sgen_client_collecting_minor_report_roots(
    SgenPointerQueue *fin_ready_queue, SgenPointerQueue *critical_fin_queue) {
  sgen_report_all_roots(fin_ready_queue, critical_fin_queue);
}

void sgen_client_collecting_major_report_roots(
    SgenPointerQueue *fin_ready_queue, SgenPointerQueue *critical_fin_queue) {
  sgen_report_all_roots(fin_ready_queue, critical_fin_queue);
}
...
static SgenPointerQueue moved_objects_queue =
    SGEN_POINTER_QUEUE_INIT(INTERNAL_MEM_MOVED_OBJECT);

void mono_sgen_register_moved_object(void *obj, void *destination) {
  /*
  ...
  if (sgen_workers_is_worker_thread(mono_native_thread_id_get())) {
    ...
    if (moved_objects_idx == MOVED_OBJECTS_NUM) {
      MONO_PROFILER_RAISE(gc_moves,
                          ((MonoObject **)moved_objects, moved_objects_idx));
      moved_objects_idx = 0;
    }
...
void mono_sgen_gc_event_moves(void) {
  ...
  do {                                                                         \
    if (*(ptr)) {                                                              \
      if (hwi->count == REFS_SIZE) {                                           \
        hwi->callback((MonoObject *)start, mono_object_class(start),           \
                      hwi->called ? 0 : size, hwi->count, hwi->refs,           \
                      hwi->offsets, hwi->data);                                \
        hwi->count = 0;                                                        \
        hwi->called = 1;                                                       \
      }                                                                        \
...
static void collect_references(HeapWalkInfo *hwi, GCObject *obj, size_t size) {
  char *start = (char *)obj;
...
static void walk_references(GCObject *start, size_t size, void *data) {
  HeapWalkInfo *hwi = (HeapWalkInfo *)data;
  hwi->called = 0;
  hwi->count = 0;
  collect_references(hwi, start, size);
  if (hwi->count || !hwi->called)
    hwi->callback(start, mono_object_class(start), hwi->called ? 0 : size,
                  hwi->count, hwi->refs, hwi->offsets, hwi->data);
}
 ...
 * Note that this function can be only called in the \c
 * MONO_GC_EVENT_PRE_START_WORLD profiler event handler. \returns a non-zero
 * value if the GC doesn't support heap walking
 */
int mono_gc_walk_heap(int flags, MonoGCReferences callback, void *data) {
  ...
  sgen_clear_nursery_fragments();
  sgen_scan_area_with_callback(sgen_nursery_section->data,
                               sgen_nursery_section->end_data, walk_references,
                               &hwi, FALSE, TRUE);

  sgen_major_collector.iterate_objects(ITERATE_OBJECTS_SWEEP_ALL,
                                       walk_references, &hwi);
...
MonoGCCallbacks *mono_gc_get_gc_callbacks() { return &gc_callbacks; }

gpointer mono_gc_thread_attach(SgenThreadInfo *info) {
...
void sgen_client_thread_attach(SgenThreadInfo *info) {
  ...
  if (mono_gc_get_gc_callbacks()->thread_attach_func)
    info->client_info.runtime_data =
        mono_gc_get_gc_callbacks()->thread_attach_func();

  sgen_binary_protocol_thread_register(
      (gpointer)mono_thread_info_get_tid(info));

  SGEN_LOG(3, "registered thread %p (%p) stack end %p", info,
           (gpointer)mono_thread_info_get_tid(info),
           info->client_info.info.stack_end);

  info->client_info.info.handle_stack = mono_handle_stack_alloc();
}

void mono_gc_thread_detach(SgenThreadInfo *info) {}

void mono_gc_thread_detach_with_lock(SgenThreadInfo *info) {
...
void mono_gc_skip_thread_changing(gboolean skip) {
  ...
  if (skip) {
    /*
     * If we skip scanning a thread with a non-empty handle stack, we may move
     * an object but fail to update the reference in the handle.
     */
    HandleStack *stack =
        mono_thread_info_current()->client_info.info.handle_stack;
    g_assert(stack == NULL || mono_handle_stack_is_empty(stack));
  }
}

void mono_gc_skip_thread_changed(gboolean skip) { UNLOCK_GC; }

gboolean mono_gc_thread_in_critical_region(SgenThreadInfo *info) {
...
/* Variables holding start/end nursery so it won't have to be passed at every
 * call */
static void *scan_area_arg_start, *scan_area_arg_end;

void mono_gc_conservatively_scan_area(void *start, void *end) {
  sgen_conservatively_pin_objects_from((void **)start, (void **)end,
                                       scan_area_arg_start, scan_area_arg_end,
                                       PIN_TYPE_STACK);
}
...
/* Called when we're scanning the handle stack imprecisely and we encounter a
   pointer into the middle of an object.
 */
static void pin_handle_stack_interior_ptrs(void **ptr_slot, void *user_data) {
  PinHandleStackInteriorPtrData *ud =
      (PinHandleStackInteriorPtrData *)user_data;
  sgen_conservatively_pin_objects_from(ptr_slot, ptr_slot + 1,
                                       ud->start_nursery, ud->end_nursery,
                                       PIN_TYPE_STACK);
}
 ...
 */
void sgen_client_scan_thread_data(void *start_nursery, void *end_nursery,
                                  gboolean precise, ScanCopyContext ctx) {
  scan_area_arg_start = start_nursery;
  scan_area_arg_end = end_nursery;
#ifdef HOST_WASM
  // Under WASM we don't scan thread stacks and we can't trust the values we
  // find there either.
  ...
  if (gc_callbacks.interp_mark_func)
    /* The interpreter code uses only compiler write barriers so have to
     * synchronize with it */
  ...
  FOREACH_THREAD_EXCLUDE(info, MONO_THREAD_INFO_FLAGS_NO_GC) {
    ...
    if (info->client_info.skip) {
      SGEN_LOG(
          3,
          "Skipping dead thread %p, range: %p-%p, size: %" G_GSIZE_FORMAT "d",
          info, info->client_info.stack_start, info->client_info.info.stack_end,
          (char *)info->client_info.info.stack_end -
              (char *)info->client_info.stack_start);
      skip_reason = 1;
    } else if (!mono_thread_info_is_live(info)) {
      SGEN_LOG(
          3,
          "Skipping non-running thread %p, range: %p-%p, size: %" G_GSIZE_FORMAT
          "d (state %x)",
          info, info->client_info.stack_start, info->client_info.info.stack_end,
          (char *)info->client_info.info.stack_end -
              (char *)info->client_info.stack_start,
          info->client_info.info.thread_state.raw);
      skip_reason = 3;
    } else if (!info->client_info.stack_start) {
    ...
    sgen_binary_protocol_scan_stack(
        (gpointer)mono_thread_info_get_tid(info), info->client_info.stack_start,
        info->client_info.info.stack_end, skip_reason);
    ...
    g_assert(info->client_info.suspend_done);
    SGEN_LOG(3,
             "Scanning thread %p, range: %p-%p, size: %" G_GSIZE_FORMAT
             "d, pinned=%" G_GSIZE_FORMAT "d",
             info, info->client_info.stack_start,
             info->client_info.info.stack_end,
             (char *)info->client_info.info.stack_end -
                 (char *)info->client_info.stack_start,
             sgen_get_pinned_count());
    if (mono_gc_get_gc_callbacks()->thread_mark_func &&
        !conservative_stack_mark) {
      mono_gc_get_gc_callbacks()->thread_mark_func(
          info->client_info.runtime_data, (guint8 *)aligned_stack_start,
          (guint8 *)info->client_info.info.stack_end, precise, &ctx);
    } else if (!precise) {
      ...
      sgen_conservatively_pin_objects_from(
          (void **)aligned_stack_start,
          (void **)info->client_info.info.stack_end, start_nursery, end_nursery,
          PIN_TYPE_STACK);
    }
    ...
    if (!precise) {
      sgen_conservatively_pin_objects_from(
          (void **)&info->client_info.ctx,
          (void **)(&info->client_info.ctx + 1), start_nursery, end_nursery,
          PIN_TYPE_STACK);

      {
        // This is used on Coop GC for platforms where we cannot get the data
        // for individual registers. We force a spill of all registers into the
        // stack and pass a chunk of data into sgen.
        // FIXME under coop, for now, what we need to ensure is that we scan any
        // extra memory from info->client_info.info.stack_end to stack_mark
        MonoThreadUnwindState *state =
            &info->client_info.info
                 .thread_saved_state[SELF_SUSPEND_STATE_INDEX];
        if (state && state->gc_stackdata) {
          sgen_conservatively_pin_objects_from(
              (void **)state->gc_stackdata,
              (void **)((char *)state->gc_stackdata + state->gc_stackdata_size),
    ...
    if (gc_callbacks.interp_mark_func) {
      ...
      gc_callbacks.interp_mark_func(&info->client_info.info,
                                    pin_handle_stack_interior_ptrs, &ud,
                                    precise);
    }
    if (info->client_info.info.handle_stack) {
      ...
      */
      if (precise)
        mono_handle_stack_scan(info->client_info.info.handle_stack,
                               (GcScanFunc)ctx.ops->copy_or_mark_object,
                               ctx.queue, precise, TRUE);
      else {
        PinHandleStackInteriorPtrData ud;
        memset(&ud, 0, sizeof(ud));
        ud.start_nursery = (void **)start_nursery;
        ud.end_nursery = (void **)end_nursery;
        mono_handle_stack_scan(info->client_info.info.handle_stack,
                               pin_handle_stack_interior_ptrs, &ud, precise,
                               FALSE);
      }
    }
  }
  ...
  SGEN_LOG(2, "Scanning thread data: %lld usecs",
           (long long)(SGEN_TV_ELAPSED(scan_thread_data_start,
 ...
 *
 *   Set the end of the current threads stack to STACK_END. The stack space
 * between STACK_END and the real end of the threads stack will not be scanned
 * during collections.
 */
void mono_gc_set_stack_end(void *stack_end) {
  SgenThreadInfo *info;

  LOCK_GC;
  info = mono_thread_info_current();
  if (info) {
    SGEN_ASSERT(0, stack_end < info->client_info.info.stack_end,
...
int mono_gc_register_root(char *start, size_t size, MonoGCDescriptor descr,
                          MonoGCRootSource source, void *key, const char *msg) {
  return sgen_register_root(start, size, descr,
                            descr ? ROOT_TYPE_NORMAL : ROOT_TYPE_PINNED, source,
                            key, msg);
}

int mono_gc_register_root_wbarrier(char *start, size_t size,
                                   MonoGCDescriptor descr,
                                   MonoGCRootSource source, void *key,
                                   const char *msg) {
  return sgen_register_root(start, size, descr, ROOT_TYPE_WBARRIER, source, key,
                            msg);
}

void mono_gc_deregister_root(char *addr) { sgen_deregister_root(addr); }
...
#ifndef HOST_WIN32
int mono_gc_pthread_create(pthread_t *new_thread, const pthread_attr_t *attr,
...
void sgen_client_total_allocated_heap_changed(size_t allocated_heap) {
...
void mono_sgen_gc_event_resize(void) {
...
gboolean mono_gc_user_markers_supported(void) { return TRUE; }

gboolean mono_object_is_alive(MonoObject *o) { return TRUE; }

int mono_gc_get_generation(MonoObject *obj) {
  if (sgen_ptr_in_nursery(obj))
    return 0;
  return 1;
}

const char *mono_gc_get_gc_name(void) { return "sgen"; }
...
gboolean mono_gc_is_moving(void) { return TRUE; }

gboolean mono_gc_is_disabled(void) { return FALSE; }

#ifdef HOST_WIN32
BOOL APIENTRY mono_gc_dllmain(HMODULE module_handle, DWORD reason,
                              LPVOID reserved) {
  return TRUE;
}
#endif
...
int64_t mono_gc_get_used_size(void) { return (int64_t)sgen_gc_get_used_size(); }

int64_t mono_gc_get_heap_size(void) {
  return (int64_t)sgen_gc_get_total_heap_allocation();
}

void mono_gc_get_gcmemoryinfo(gint64 *high_memory_load_threshold_bytes,
  ...
  *high_memory_load_threshold_bytes =
      sgen_gc_info.high_memory_load_threshold_bytes;
...
int mono_gc_get_los_limit(void) { return SGEN_MAX_SMALL_OBJ_SIZE; }

guint64 mono_gc_get_allocated_bytes_for_current_thread(void) {
  SgenThreadInfo *info;
  info = mono_thread_info_current();

  /*There are some more allocated bytes in the current tlab that have not been
   * recorded yet */
  return info->total_bytes_allocated +
         (ptrdiff_t)(info->tlab_next - info->tlab_start);
}
...
gpointer sgen_client_default_metadata(void) { return mono_domain_get(); }
 ...
 */
MonoGCHandle mono_gchandle_new_internal(MonoObject *obj, gboolean pinned) {
 ...
 * \param obj managed object to get a handle for
 * \param track_resurrection Determines how long to track the object, if this is
 * set to TRUE, the object is tracked after finalization, if FALSE, the object
 * is only tracked up until the point of finalization.
 *
 ...
 * unmanaged code.
 */
MonoGCHandle mono_gchandle_new_weakref_internal(GCObject *obj,
                                                gboolean track_resurrection) {
  return MONO_GC_HANDLE_FROM_UINT(
      sgen_gchandle_new_weakref(obj, track_resurrection));
}
 ...
 * \returns TRUE if the object wrapped by the \p gchandle belongs to the
 * specific \p domain.
 */
gboolean mono_gchandle_is_in_domain(MonoGCHandle gchandle, MonoDomain *domain) {
  MonoDomain *gchandle_domain = (MonoDomain *)sgen_gchandle_get_metadata(
      MONO_GC_HANDLE_TO_UINT(gchandle));
 ...
 */
void mono_gchandle_free_domain(MonoDomain *unloading) {}
...
static gpointer null_link_if_in_domain(gpointer hidden,
                                       GCHandleType handle_type,
                                       int max_generation, gpointer user) {
...
void sgen_null_links_for_domain(MonoDomain *domain) {
  ...
  for (type = HANDLE_TYPE_MIN; type < HANDLE_TYPE_MAX; ++type)
    sgen_gchandle_iterate((GCHandleType)type, GENERATION_OLD,
                          null_link_if_in_domain, domain);
}
...
void sgen_client_gchandle_created(int handle_type, GCObject *obj,
                                  guint32 handle) {
#ifndef DISABLE_PERFCOUNTERS
  mono_atomic_inc_i32(&mono_perfcounters->gc_num_handles);
#endif

  MONO_PROFILER_RAISE(gc_handle_created,
                      (handle, (MonoGCHandleType)handle_type, obj));
}
...
void sgen_client_gchandle_destroyed(int handle_type, guint32 handle) {
  ...
  MONO_PROFILER_RAISE(gc_handle_deleted,
                      (handle, (MonoGCHandleType)handle_type));
}
...
void mono_gc_register_altstack(gpointer stack, gint32 stack_size,
                               gpointer altstack, gint32 altstack_size) {
  // FIXME:
}

guint8 *mono_gc_get_card_table(int *shift_bits, gpointer *mask) {
  return sgen_get_card_table_configuration(shift_bits, mask);
}

guint8 *mono_gc_get_target_card_table(int *shift_bits, target_mgreg_t *mask) {
...
void sgen_client_degraded_allocation(void) {
  // The WASM target aways triggers degrated allocation before collecting. So no
  // point in printing the warning as it will just confuse users
#ifndef HOST_WASM
  ...
  if (mono_atomic_load_i32(&last_major_gc_warned) < major_gc_count) {
    ...
    if (num == 1 || num == 3)
      mono_trace(G_LOG_LEVEL_WARNING, MONO_TRACE_GC,
                 "Warning: Degraded allocation.  Consider increasing "
                 "nursery-size if the warning persists.");
    else if (num == 10)
      mono_trace(G_LOG_LEVEL_WARNING, MONO_TRACE_GC,
                 "Warning: Repeated degraded allocation.  Consider increasing "
                 "nursery-size.");
...
const char *sgen_client_description_for_internal_mem_type(int type) {
  switch (type) {
  case INTERNAL_MEM_EPHEMERON_LINK:
    return "ephemeron-link";
  case INTERNAL_MEM_MOVED_OBJECT:
    return "moved-object";
  default:
    return NULL;
  }
...
gboolean sgen_client_vtable_is_inited(MonoVTable *vt) {
  return m_class_is_inited(vt->klass);
}

const char *sgen_client_vtable_get_namespace(MonoVTable *vt) {
  return m_class_get_name_space(vt->klass);
}

const char *sgen_client_vtable_get_name(MonoVTable *vt) {
...
void sgen_client_init(void) {
  ...
  sgen_register_fixed_internal_mem_type(INTERNAL_MEM_EPHEMERON_LINK,
...
void mono_gc_init_icalls(void) {
  mono_register_jit_icall(mono_gc_alloc_obj, mono_icall_sig_object_ptr_sizet,
                          FALSE);
  mono_register_jit_icall(mono_gc_alloc_vector,
                          mono_icall_sig_object_ptr_sizet_ptr, FALSE);
  mono_register_jit_icall(mono_gc_alloc_string,
                          mono_icall_sig_object_ptr_sizet_int32, FALSE);
  mono_register_jit_icall(mono_profiler_raise_gc_allocation,
                          mono_icall_sig_void_object, FALSE);
}
...
gboolean sgen_client_handle_gc_param(const char *opt) {
  if (g_str_has_prefix(opt, "stack-mark=")) {
    ...
    if (!strcmp(opt, "precise")) {
      ...
      sgen_env_var_error(MONO_GC_PARAMS_NAME,
                         conservative_stack_mark ? "Using `conservative`."
                                                 : "Using `precise`.",
                         "Invalid value `%s` for `stack-mark` option, possible "
                         "values are: `precise`, `conservative`.",
                         opt);
    }
...
void sgen_client_print_gc_params_usage(void) {
  fprintf(stderr, "  stack-mark=MARK-METHOD (where MARK-METHOD is 'precise' or "
                  "'conservative')\n");
}
...
gboolean sgen_client_handle_gc_debug(const char *opt) {
  if (!strcmp(opt, "xdomain-checks")) {
    ...
    /*
     * This option can be used to override the disabling of the managed
     * allocator by the nursery canaries option. This can be used when knowing
     * for sure that no aot code will be used by the application.
     */
...
#ifdef HEAVY_STATISTICS
  mono_counters_register("los marked cards",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &los_marked_cards);
  mono_counters_register("los array cards scanned ",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &los_array_cards);
  mono_counters_register("los array remsets",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &los_array_remsets);

  mono_counters_register("WBarrier set arrayref",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_wbarrier_set_arrayref);
  mono_counters_register("WBarrier value copy",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
                         &stat_wbarrier_value_copy);
  mono_counters_register("WBarrier object copy",
                         MONO_COUNTER_GC | MONO_COUNTER_ULONG,
...
void mono_gc_base_cleanup(void) {
  /*
...
gboolean mono_gc_is_null(void) { return FALSE; }

gsize *sgen_client_get_weak_bitmap(MonoVTable *vt, int *nbits) {
  MonoClass *klass = vt->klass;

  return mono_class_get_weak_bitmap(klass, nbits);
}

void sgen_client_binary_protocol_collection_begin(int minor_gc_count,
                                                  int generation) {
  ...
  if (!pseudo_roots_registered) {
    ...
    MONO_PROFILER_RAISE(gc_root_register, (SPECIAL_ADDRESS_FIN_QUEUE, 1,
                                           MONO_ROOT_SOURCE_FINALIZER_QUEUE,
                                           NULL, "Finalizer Queue"));
    MONO_PROFILER_RAISE(gc_root_register, (SPECIAL_ADDRESS_CRIT_FIN_QUEUE, 1,
                                           MONO_ROOT_SOURCE_FINALIZER_QUEUE,
                                           NULL, "Finalizer Queue (Critical)"));
    MONO_PROFILER_RAISE(gc_root_register,
                        (SPECIAL_ADDRESS_EPHEMERON, 1,
                         MONO_ROOT_SOURCE_EPHEMERON, NULL, "Ephemerons"));
    MONO_PROFILER_RAISE(gc_root_register,
                        (SPECIAL_ADDRESS_TOGGLEREF, 1,
                         MONO_ROOT_SOURCE_TOGGLEREF, NULL, "ToggleRefs"));
  }
...
void sgen_client_binary_protocol_collection_end(
    int minor_gc_count, int generation, long long num_objects_scanned,
