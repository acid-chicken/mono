...
/* This callback should return TRUE if the runtime must wait for the thread,
 * FALSE otherwise */
...
MONO_API void mono_thread_new_init(intptr_t tid, void *stack_start, void *func);

MONO_API MONO_RT_EXTERNAL_ONLY void mono_thread_create(MonoDomain *domain,
                                                       void *func, void *arg);
...
MONO_API MONO_RT_EXTERNAL_ONLY void mono_threads_attach_tools_thread(void);

MONO_API char *mono_thread_get_name_utf8(MonoThread *thread);
MONO_API int32_t mono_thread_get_managed_id(MonoThread *thread);

MONO_API void mono_thread_set_manage_callback(MonoThread *thread,
...
MONO_API MONO_RT_EXTERNAL_ONLY mono_bool mono_thread_detach_if_exiting(void);
