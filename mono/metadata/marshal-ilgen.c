...
static gboolean is_in(const MonoType *t) {
  const guint32 attrs = t->attrs;
  return (attrs & PARAM_ATTRIBUTE_IN) || !(attrs & PARAM_ATTRIBUTE_OUT);
}

static gboolean is_out(const MonoType *t) {
  const guint32 attrs = t->attrs;
  return (attrs & PARAM_ATTRIBUTE_OUT) || !(attrs & PARAM_ATTRIBUTE_IN);
}

static GENERATE_GET_CLASS_WITH_CACHE(fixed_buffer_attribute,
                                     "System.Runtime.CompilerServices",
                                     "FixedBufferAttribute");
static GENERATE_GET_CLASS_WITH_CACHE(date_time, "System", "DateTime");
static GENERATE_TRY_GET_CLASS_WITH_CACHE(icustom_marshaler,
                                         "System.Runtime.InteropServices",
                                         "ICustomMarshaler");
static GENERATE_TRY_GET_CLASS_WITH_CACHE(marshal,
                                         "System.Runtime.InteropServices",
                                         "Marshal");
...
static void init_safe_handle(void) {
  mono_atomic_store_seq(&sh_dangerous_add_ref,
                        get_method_nofail(mono_class_try_get_safehandle_class(),
                                          "DangerousAddRef", 1, 0));
  mono_atomic_store_seq(&sh_dangerous_release,
                        get_method_nofail(mono_class_try_get_safehandle_class(),
                                          "DangerousRelease", 0, 0));
}
...
static void emit_struct_conv(MonoMethodBuilder *mb, MonoClass *klass,
                             gboolean to_object);

static void emit_struct_conv_full(MonoMethodBuilder *mb, MonoClass *klass,
                                  gboolean to_object,
 ...
 * \param s a string
 *
 * Creates a copy of the string \p s that can be referenced from the IL of \c
 * mb.
 *
 ...
 */
static void mono_mb_emit_exception_marshal_directive(MonoMethodBuilder *mb,
                                                     char *msg) {
  char *s = mono_mb_strdup(mb, msg);
  g_free(msg);
  mono_mb_emit_exception_full(mb, "System.Runtime.InteropServices",
                              "MarshalDirectiveException", s);
}
...
static int offset_of_first_nonstatic_field(MonoClass *klass) {
  ...
  for (i = 0; i < fcount; i++) {
    if (!(klass_fields[i].type->attrs & FIELD_ATTRIBUTE_STATIC) &&
        !mono_field_is_deleted(&klass_fields[i]))
...
static gboolean get_fixed_buffer_attr(MonoClassField *field,
                                      MonoType **out_etype, int *out_len) {
  ERROR_DECL(error);
  ...
  if (cinfo) {
    for (aindex = 0; aindex < cinfo->num_attrs; ++aindex) {
      ...
      if (mono_class_has_parent(
              ctor_class, mono_class_get_fixed_buffer_attribute_class())) {
...
static void emit_fixed_buf_conv(MonoMethodBuilder *mb, MonoType *type,
                                MonoType *etype, int len, gboolean to_object,
                                int *out_usize) {
...
static void emit_ptr_to_object_conv(MonoMethodBuilder *mb, MonoType *type,
  ...
  case MONO_MARSHAL_CONV_STR_BYVALSTR:
    if (mspec && mspec->native == MONO_NATIVE_BYVALTSTR &&
        mspec->data.array_data.num_elem) {
      mono_mb_emit_ldloc(mb, 1);
      mono_mb_emit_ldloc(mb, 0);
      mono_mb_emit_icon(mb, mspec->data.array_data.num_elem);
  ...
  case MONO_MARSHAL_CONV_ARRAY_LPARRAY: {
    char *msg = g_strdup_printf(
        ...
        m_class_get_name(mono_class_from_mono_type_internal(type)));
...
static MonoJitICallId conv_to_icall(MonoMarshalConv conv, int *ind_store_type) {
...
static void emit_object_to_ptr_conv(MonoMethodBuilder *mb, MonoType *type,
                                    MonoMarshalConv conv,
                                    MonoMarshalSpec *mspec) {
  int pos;
  int stind_op;
...
static MonoMethod *mono_get_Marshal_GetObjectForNativeVariant(void) {
  ...
  get_object_for_native_variant = get_method_nofail(
      mono_defaults.marshal_class, "GetObjectForNativeVariant", 1, 0);
...
static MonoMethod *mono_get_Marshal_GetNativeVariantForObject(void) {
  ...
  get_native_variant_for_object = get_method_nofail(
      mono_defaults.marshal_class, "GetNativeVariantForObject", 2, 0);
...
static void emit_struct_conv_full(MonoMethodBuilder *mb, MonoClass *klass,
  ...
  if (m_class_get_parent(klass))
    emit_struct_conv_full(mb, m_class_get_parent(klass), to_object,
  ...
  if (klass != mono_class_try_get_safehandle_class()) {
    if (mono_class_is_auto_layout(klass)) {
      char *msg =
          g_strdup_printf("Type %s which is passed to unmanaged code must have "
  ...
  for (i = 0; i < info->num_fields; i++) {
    ...
    ntype = (MonoMarshalNative)mono_type_to_unmanaged(
        ftype, info->fields[i].mspec, TRUE, m_class_is_unicode(klass), &conv);
    ...
    if (klass != mono_class_try_get_safehandle_class()) {
      ...
      if (mono_class_is_explicit_layout(klass) && (usize == 0)) {
        if (MONO_TYPE_IS_REFERENCE(info->fields[i].field->type) ||
            ((!last_field &&
              MONO_TYPE_IS_REFERENCE(info->fields[i + 1].field->type))))
          g_error(
              "Type %s which has an [ExplicitLayout] attribute cannot have a "
    ...
    handle_enum:
      ...
      case MONO_TYPE_R8:
        ...
        if (t == MONO_TYPE_CHAR && ntype == MONO_NATIVE_U1 &&
            string_encoding != MONO_NATIVE_LPWSTR) {
          if (to_object) {
            mono_mb_emit_byte(mb, CEE_LDIND_U1);
            mono_mb_emit_byte(mb, CEE_STIND_I2);
          } else {
      ...
      case MONO_TYPE_GENERICINST:
        if (!mono_type_generic_inst_is_valuetype(ftype)) {
          char *msg = g_strdup_printf(
              "Generic type %s cannot be marshaled as field in a struct.",
      ...
      case MONO_TYPE_VALUETYPE: {
        ...
        if (get_fixed_buffer_attr(info->fields[i].field, &etype, &len)) {
          ...
          emit_struct_conv(mb, mono_class_from_mono_type_internal(ftype),
                           to_object);
        }
      ...
      case MONO_TYPE_OBJECT: {
        ...
        if (to_object) {
          ...
          mono_mb_emit_ldloc(mb, 1);
          mono_mb_emit_managed_call(
              mb, mono_get_Marshal_GetNativeVariantForObject(), NULL);
        }
#else
        char *msg =
...
static void emit_struct_free(MonoMethodBuilder *mb, MonoClass *klass,
                             int struct_var) {
...
static void
emit_thread_interrupt_checkpoint_call(MonoMethodBuilder *mb,
  ...
  mono_mb_emit_ldflda(mb,
                      MONO_STRUCT_OFFSET(MonoException, caught_in_unmanaged));
  mono_mb_emit_byte(mb, CEE_LDC_I4_1);
  mono_mb_emit_byte(mb, CEE_STIND_I4);
...
static void emit_thread_interrupt_checkpoint(MonoMethodBuilder *mb) {
  ...
  emit_thread_interrupt_checkpoint_call(
      mb, MONO_JIT_ICALL_mono_thread_interruption_checkpoint);
}
...
void mono_marshal_emit_thread_force_interrupt_checkpoint(
    MonoMethodBuilder *mb) {
  emit_thread_force_interrupt_checkpoint(mb);
}

int mono_mb_emit_save_args(MonoMethodBuilder *mb, MonoMethodSignature *sig,
                           gboolean save_this) {
 ...
 */
static void emit_invoke_call(MonoMethodBuilder *mb, MonoMethod *method,
                             MonoMethodSignature *sig,
                             MonoMethodSignature *callsig, int loc_res,
  ...
  if (!string_dummy) {
    ...
    MONO_GC_REGISTER_ROOT_SINGLE(string_dummy, MONO_ROOT_SOURCE_MARSHAL, NULL,
                                 "Marshal Dummy String");

    MonoStringHandle string_dummy_handle =
  ...
  for (i = 0; i < sig->param_count; i++) {
    ...
    if (t->byref) {
      mono_mb_emit_byte(mb, CEE_LDIND_I);
      /* A Nullable<T> type don't have a boxed form, it's either null or a boxed
       * T. So to make this work we unbox it to a local variablee and push a
       * reference to that.
       */
      if (t->type == MONO_TYPE_GENERICINST &&
          mono_class_is_nullable(mono_class_from_mono_type_internal(t))) {
        tmp_nullable_locals[i] = mono_mb_add_local(
            mb, m_class_get_byval_arg(mono_class_from_mono_type_internal(t)));

        mono_mb_emit_op(mb, CEE_UNBOX_ANY,
...
static void emit_runtime_invoke_body_ilgen(
  ...
  emit_thread_force_interrupt_checkpoint(mb);
  emit_invoke_call(mb, method, sig, callsig, loc_res, virtual_,
                   need_direct_wrapper);

  labels[2] = mono_mb_emit_branch(mb, CEE_LEAVE);

  /* Add a try clause around the call */
  clause = (MonoExceptionClause *)mono_image_alloc0(
      image, sizeof(MonoExceptionClause));
...
static void emit_runtime_invoke_dynamic_ilgen(MonoMethodBuilder *mb) {
  int pos;
  MonoExceptionClause *clause;
  ...
  clause = (MonoExceptionClause *)mono_image_alloc0(
      mono_defaults.corlib, sizeof(MonoExceptionClause));
...
static gboolean
gc_safe_transition_builder_init(GCSafeTransitionBuilder *builder,
                                MonoMethodBuilder *mb, gboolean func_param) {
  builder->mb = mb;
...
#ifndef DISABLE_COM
  if (!builder->func_param &&
      MONO_CLASS_IS_IMPORT(builder->mb->method->klass)) {
 ...
 */
static void
gc_safe_transition_builder_emit_enter(GCSafeTransitionBuilder *builder,
                                      MonoMethod *method, gboolean aot) {
  ...
  if (!builder->func_param &&
      !MONO_CLASS_IS_IMPORT(builder->mb->method->klass) && aot) {
 ...
 * \param check_exceptions Whenever to check for pending exceptions after the
 * native call \param func_param the function to call is passed as a boxed
 * IntPtr as the first parameter \param skip_gc_trans Whenever to skip GC
 * transitions
 *
 * generates IL code for the pinvoke wrapper, the generated code calls \p func .
 */
static void emit_native_wrapper_ilgen(MonoImage *image, MonoMethodBuilder *mb,
                                      MonoMethodSignature *sig,
  ...
  for (i = 0; i < sig->param_count; i++) {
    tmp_locals[i] =
        mono_emit_marshal(&m, i + param_shift, sig->params[i], mspecs[i + 1], 0,
                          &csig->params[i], MARSHAL_ACTION_CONV_IN);
  }

  // In coop mode need to register blocking state during native call
  if (need_gc_safe)
    gc_safe_transition_builder_emit_enter(&gc_safe_transition_builder,
                                          &piinfo->method, aot);
  ...
  for (i = 0; i < sig->param_count; i++) {
    mono_emit_marshal(&m, i + param_shift, sig->params[i], mspecs[i + 1],
                      tmp_locals[i], NULL, MARSHAL_ACTION_PUSH);
  }
  ...
  if (MONO_TYPE_ISSTRUCT(sig->ret)) {
    ...
    if (!(mono_class_is_explicit_layout(klass) ||
      ...
      /* This is used by emit_marshal_vtype (), but it needs to go right before
       * the call */
      mono_mb_emit_byte(mb, MONO_CUSTOM_PREFIX);
  ...
  /* Unblock before converting the result, since that can involve calls into the
   * runtime */
  if (need_gc_safe)
  ...
  if (!sig->ret->byref) {
    ...
    if (spec && spec->native == MONO_NATIVE_CUSTOM) {
      mono_emit_marshal(&m, 0, sig->ret, spec, 0, NULL,
  ...
  for (i = 0; i < sig->param_count; i++) {
    ...
    if (spec && ((spec->native == MONO_NATIVE_CUSTOM) ||
                 (spec->native == MONO_NATIVE_ASANY))) {
      mono_emit_marshal(&m, argnum, t, spec, tmp_locals[i], NULL,
 ...
 * to create multiple out-branches and the fall through state of having the
 * object on the stack after a cache miss
 */
static void generate_check_cache(int obj_arg_position, int class_arg_position,
                                 int cache_arg_position, // In-parameters
                                 int *null_obj, int *cache_hit_neg,
                                 int *cache_hit_pos, // Out-parameters
                                 MonoMethodBuilder *mb) {
  int cache_miss_pos;
...
static void emit_castclass_ilgen(MonoMethodBuilder *mb) {
  ...
  generate_check_cache(obj_arg_position, class_arg_position, cache_arg_position,
                       &return_null_pos, &negative_cache_hit_pos,
                       &positive_cache_hit_pos, mb);
...
static void emit_isinst_ilgen(MonoMethodBuilder *mb) {
  int return_null_pos, positive_cache_hit_pos, negative_cache_hit_pos;
  const int obj_arg_position = TYPECHECK_OBJECT_ARG_POS;
  const int class_arg_position = TYPECHECK_CLASS_ARG_POS;
  const int cache_arg_position = TYPECHECK_CACHE_ARG_POS;
...
static void load_array_element_address(MonoMethodBuilder *mb) {
  mono_mb_emit_ldarg(mb, 0);
  mono_mb_emit_ldarg(mb, 1);
  mono_mb_emit_op(mb, CEE_LDELEMA, mono_defaults.object_class);
}

static void load_array_class(MonoMethodBuilder *mb, int aklass) {
  mono_mb_emit_ldarg(mb, 0);
  mono_mb_emit_ldflda(mb, MONO_STRUCT_OFFSET(MonoObject, vtable));
  mono_mb_emit_byte(mb, CEE_LDIND_I);
...
static void load_value_class(MonoMethodBuilder *mb, int vklass) {
...
static int emit_marshal_array_ilgen(EmitMarshalContext *m, int argnum,
    ...
    *conv_arg_type = object_type;
    ...
    if (m_class_is_blittable(eklass)) {
      ...
      mono_mb_emit_icall_id(
          mb, conv_to_icall(MONO_MARSHAL_CONV_ARRAY_LPARRAY, NULL));
      ...
      if (is_string)
        esize = TARGET_SIZEOF_VOID_P;
               ...
               mono_defaults
                   .char_class) /*can't call mono_marshal_type_size since it
                                   causes all sorts of asserts*/
      ...
      if (is_string) {
        ...
        emit_struct_conv_full(mb, eklass, FALSE, 0,
                              eklass == mono_defaults.char_class
                                  ? encoding
                                  : (MonoMarshalNative)-1);
      }
  ...
  case MARSHAL_ACTION_CONV_OUT:
     ...
     */
    need_convert = ((eklass == mono_defaults.char_class) &&
                    (encoding == MONO_NATIVE_LPWSTR)) ||
                   (eklass == mono_class_try_get_stringbuilder_class()) ||
                   (t->attrs & PARAM_ATTRIBUTE_OUT);
    need_free =
        mono_marshal_need_free(m_class_get_byval_arg(eklass), m->piinfo, spec);

    if ((t->attrs & PARAM_ATTRIBUTE_OUT) && spec &&
        spec->native == MONO_NATIVE_LPARRAY &&
        spec->data.array_data.param_num != -1) {
    ...
    if (need_convert || need_free) {
      ...
      if ((eklass == mono_class_try_get_stringbuilder_class()) ||
          (eklass == mono_defaults.string_class))
        esize = TARGET_SIZEOF_VOID_P;
      ...
      if (eklass == mono_class_try_get_stringbuilder_class()) {
        gboolean need_free2;
        MonoMarshalConv conv = mono_marshal_get_ptr_to_stringbuilder_conv(
            m->piinfo, spec, &need_free2);
        ...
        if (need_convert) {
          ...
          /* emit valuetype conversion code */
          emit_struct_conv_full(mb, eklass, TRUE, 0,
                                eklass == mono_defaults.char_class
                                    ? encoding
                                    : (MonoMarshalNative)-1);
        }
    ...
    *conv_arg_type = int_type;
    ...
    case MONO_NATIVE_SAFEARRAY:
      ...
      if (spec->data.safearray_data.elem_type != MONO_VARIANT_VARIANT) {
        char *msg = g_strdup("Only SAFEARRAY(VARIANT) marshalling to managed "
                             "code is implemented.");
        mono_mb_emit_exception_marshal_directive(mb, msg);
        return conv_arg;
      }
    ...
    if (param_num == -1) {
      if (num_elem <= 0) {
        char *msg =
            g_strdup("Either SizeConst or SizeParamIndex should be specified "
                     "when marshalling arrays to managed code.");
        mono_mb_emit_exception_marshal_directive(mb, msg);
        return conv_arg;
      }
    }
    ...
    if (eklass == mono_defaults.string_class) {
      ...
      is_string = TRUE;
      conv = mono_marshal_get_ptr_to_stringbuilder_conv(m->piinfo, spec,
                                                        &need_free);
    } else
      conv = MONO_MARSHAL_CONV_INVALID;
    ...
    if (is_string) {
      ...
      char *msg = g_strdup("Marshalling of non-string and non-blittable arrays "
                           "to managed code is not implemented.");
      mono_mb_emit_exception_marshal_directive(mb, msg);
      return conv_arg;
    }
  ...
  case MARSHAL_ACTION_MANAGED_CONV_RESULT: {
    ...
    if (is_string) {
      ...
      char *msg = g_strdup("Marshalling of non-string arrays to managed code "
                           "is not implemented.");
      mono_mb_emit_exception_marshal_directive(mb, msg);
      return conv_arg;
    }
...
static int emit_marshal_boolean_ilgen(EmitMarshalContext *m, int argnum,
                                      MonoType *t, MonoMarshalSpec *spec,
                                      int conv_arg, MonoType **conv_arg_type,
                                      MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
  ...
  case MARSHAL_ACTION_MANAGED_CONV_IN: {
    ...
    conv_arg_class =
        mono_marshal_boolean_managed_conv_in_get_conv_arg_class(spec, &ldop);
...
static int emit_marshal_ptr_ilgen(EmitMarshalContext *m, int argnum,
                                  MonoType *t, MonoMarshalSpec *spec,
                                  int conv_arg, MonoType **conv_arg_type,
                                  MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
  ...
  case MARSHAL_ACTION_CONV_IN:
    ...
    /*
    if (MONO_TYPE_ISSTRUCT (t->data.type) && !mono_class_from_mono_type_internal
    (t->data.type)->blittable) { char *msg = g_strdup_printf ("Can not marshal
    'parameter #%d': Pointers can not reference marshaled structures. Use byref
    instead.", argnum + 1); mono_mb_emit_exception_marshal_directive (m->mb,
    msg);
    }
    */
    break;
...
static int emit_marshal_char_ilgen(EmitMarshalContext *m, int argnum,
                                   MonoType *t, MonoMarshalSpec *spec,
                                   int conv_arg, MonoType **conv_arg_type,
                                   MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
...
static int emit_marshal_scalar_ilgen(EmitMarshalContext *m, int argnum,
                                     MonoType *t, MonoMarshalSpec *spec,
                                     int conv_arg, MonoType **conv_arg_type,
                                     MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
...
static void emit_virtual_stelemref_ilgen(MonoMethodBuilder *mb,
  ...
  MonoType *object_type_byref =
      m_class_get_this_arg(mono_defaults.object_class);
...
static void emit_stelemref_ilgen(MonoMethodBuilder *mb) {
  guint32 b1, b2, b3, b4;
  guint32 copy_pos;
  int aklass, vklass;
  int array_slot_addr;
...
static void mb_emit_byte_ilgen(MonoMethodBuilder *mb, guint8 op) {
  mono_mb_emit_byte(mb, op);
}

static void emit_array_address_ilgen(MonoMethodBuilder *mb, int rank,
                                     int elem_size) {
  ...
  /* ind is the overall element index, realidx is the partial index in a single
   * dimension */
  ...
  for (i = 1; i < rank; ++i) {
    ...
    mono_mb_emit_ldloc(mb, bounds);
    mono_mb_emit_icon(mb, (i * sizeof(MonoArrayBounds)) +
...
static void emit_delegate_begin_invoke_ilgen(MonoMethodBuilder *mb,
                                             MonoMethodSignature *sig) {
  int params_var;
...
static void emit_delegate_end_invoke_ilgen(MonoMethodBuilder *mb,
                                           MonoMethodSignature *sig) {
  int params_var;
  params_var = mono_mb_emit_save_args(mb, sig, FALSE);
...
static void emit_delegate_invoke_internal_ilgen(
  ...
  local_delegates =
      mono_mb_add_local(mb, m_class_get_byval_arg(mono_defaults.array_class));
  local_d = mono_mb_add_local(
      mb, m_class_get_byval_arg(mono_defaults.multicastdelegate_class));
  local_target = mono_mb_add_local(mb, object_type);

  if (!void_ret)
    local_res = mono_mb_add_local(
        mb,
        m_class_get_byval_arg(mono_class_from_mono_type_internal(sig->ret)));
  ...
  /*static methods with bound first arg can have null target and still be
   * bound*/
  ...
  if (callvirt) {
    if (!closed_over_null) {
      ...
      if (!(target_method->flags & METHOD_ATTRIBUTE_VIRTUAL) ||
    ...
    if (static_method_with_first_arg_bound) {
      ...
      if (!MONO_TYPE_IS_REFERENCE(invoke_sig->params[0]))
        mono_mb_emit_op(
            mb, CEE_UNBOX_ANY,
            mono_class_from_mono_type_internal(invoke_sig->params[0]));
    }
    for (i = 0; i < sig->param_count; ++i)
      mono_mb_emit_ldarg(mb, i + 1);
    mono_mb_emit_ldarg(mb, 0);
  ...
  if (!ctx) {
    ...
    ERROR_DECL(error);
    mono_mb_emit_op(mb, CEE_CALLVIRT,
                    mono_class_inflate_generic_method_checked(
                        method, &container->context, error));
...
static void mb_skip_visibility_ilgen(MonoMethodBuilder *mb) {
  mb->skip_visibility = 1;
}

static void mb_set_dynamic_ilgen(MonoMethodBuilder *mb) { mb->dynamic = 1; }

static void emit_synchronized_wrapper_ilgen(
  ...
  clause = (MonoExceptionClause *)mono_image_alloc0(
      get_method_image(method), sizeof(MonoExceptionClause));
...
static void emit_unbox_wrapper_ilgen(MonoMethodBuilder *mb,
                                     MonoMethod *method) {
  MonoMethodSignature *sig = mono_method_signature_internal(method);
...
static void emit_array_accessor_wrapper_ilgen(MonoMethodBuilder *mb,
                                              MonoMethod *method,
                                              MonoMethodSignature *sig,
...
static void emit_generic_array_helper_ilgen(MonoMethodBuilder *mb,
                                            MonoMethod *method,
                                            MonoMethodSignature *csig) {
  mono_mb_emit_ldarg(mb, 0);
...
static void emit_thunk_invoke_wrapper_ilgen(MonoMethodBuilder *mb,
                                            MonoMethod *method,
                                            MonoMethodSignature *csig) {
...
static void emit_marshal_custom_get_instance(MonoMethodBuilder *mb,
                                             MonoClass *klass,
...
static int emit_marshal_custom_ilgen(EmitMarshalContext *m, int argnum,
                                     MonoType *t, MonoMarshalSpec *spec,
                                     int conv_arg, MonoType **conv_arg_type,
                                     MarshalAction action) {
  ERROR_DECL(error);
  ...
  if (!ICustomMarshaler) {
    ...
    if (!klass) {
      ...
      case MARSHAL_ACTION_CONV_RESULT:
      case MARSHAL_ACTION_MANAGED_CONV_RESULT:
        if ((action == MARSHAL_ACTION_CONV_RESULT) ||
            (action == MARSHAL_ACTION_MANAGED_CONV_RESULT))
          mono_mb_emit_byte(mb, CEE_POP);

        mono_mb_emit_exception_full(mb, "System", "ApplicationException",
                                    exception_msg);
    ...
    marshal_managed_to_native =
        get_method_nofail(klass, "MarshalManagedToNative", 1, 0);
    g_assert(marshal_managed_to_native);

    marshal_native_to_managed =
        get_method_nofail(klass, "MarshalNativeToManaged", 1, 0);
  ...
  if (spec->data.custom_data.image)
    mtype = mono_reflection_type_from_name_checked(
        spec->data.custom_data.custom_name, alc, spec->data.custom_data.image,
        error);
  else
    mtype = mono_reflection_type_from_name_checked(
        spec->data.custom_data.custom_name, alc, m->image, error);
  ...
  case MARSHAL_ACTION_CONV_IN:
    ...
    default:
      g_warning("custom marshalling of type %x is currently not supported",
                t->type);
      g_assert_not_reached();
      break;
    }
...
static int emit_marshal_asany_ilgen(EmitMarshalContext *m, int argnum,
                                    MonoType *t, MonoMarshalSpec *spec,
                                    int conv_arg, MonoType **conv_arg_type,
                                    MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
  ...
  case MARSHAL_ACTION_CONV_IN: {
    MonoMarshalNative encoding =
        mono_marshal_get_string_encoding(m->piinfo, NULL);
...
static int emit_marshal_vtype_ilgen(EmitMarshalContext *m, int argnum,
                                    MonoType *t, MonoMarshalSpec *spec,
                                    int conv_arg, MonoType **conv_arg_type,
                                    MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
...
static int emit_marshal_string_ilgen(EmitMarshalContext *m, int argnum,
                                     MonoType *t, MonoMarshalSpec *spec,
                                     int conv_arg, MonoType **conv_arg_type,
                                     MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
  MonoMarshalNative encoding =
      mono_marshal_get_string_encoding(m->piinfo, spec);
  ...
  case MARSHAL_ACTION_CONV_IN:
    *conv_arg_type = int_type;
    ...
    if (conv == MONO_MARSHAL_CONV_INVALID) {
      char *msg = g_strdup_printf(
          "string marshalling conversion %d not implemented", encoding);
      mono_mb_emit_exception_marshal_directive(mb, msg);
    } else {
      mono_mb_emit_icall_id(mb, conv_to_icall(conv, NULL));
...
static int emit_marshal_safehandle_ilgen(EmitMarshalContext *m, int argnum,
                                         MonoType *t, MonoMarshalSpec *spec,
                                         int conv_arg, MonoType **conv_arg_type,
                                         MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
  ...
  case MARSHAL_ACTION_CONV_OUT: {
    /* The slot for the boolean is the next temporary created after conv_arg,
     * see the CONV_IN code */
    ...
    if (t->byref) {
      ...
      if (is_out(t)) {
        ...
        /*
         * If the SafeHandle was marshalled on input we can skip the marshalling
         * on output if the handle value is identical.
         */
        if (is_in(t)) {
         ...
         *
         * FIXME: If an out-of-memory situation or exception happens here we
         * will leak the handle. We should move the allocation of the SafeHandle
         * to the input marshalling code to prevent that.
         */
        ctor = mono_class_get_method_from_name_checked(t->data.klass, ".ctor",
                                                       0, 0, local_error);
  ...
  case MARSHAL_ACTION_CONV_RESULT: {
    ...
    if (mono_class_is_abstract(t->data.klass)) {
      mono_mb_emit_byte(mb, CEE_POP);
      mono_mb_emit_exception_marshal_directive(
          mb, g_strdup("Returned SafeHandles should not be abstract"));
      break;
    }

    ctor = mono_class_get_method_from_name_checked(t->data.klass, ".ctor", 0, 0,
                                                   error);
    ...
    /* Set the return.handle to the value, am using ldflda, not sure if thats a
     * good idea */
    mono_mb_emit_ldloc(mb, 3);
    mono_mb_emit_ldflda(mb, MONO_STRUCT_OFFSET(MonoSafeHandle, handle));
...
static int emit_marshal_handleref_ilgen(EmitMarshalContext *m, int argnum,
                                        MonoType *t, MonoMarshalSpec *spec,
                                        int conv_arg, MonoType **conv_arg_type,
                                        MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
  ...
  case MARSHAL_ACTION_CONV_IN: {
    ...
    *conv_arg_type = int_type;
    ...
    if (t->byref) {
      char *msg = g_strdup("HandleRefs can not be returned from unmanaged code "
                           "(or passed by ref)");
      mono_mb_emit_exception_marshal_directive(mb, msg);
      break;
    }
...
static int emit_marshal_object_ilgen(EmitMarshalContext *m, int argnum,
                                     MonoType *t, MonoMarshalSpec *spec,
                                     int conv_arg, MonoType **conv_arg_type,
                                     MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
    ...
    *conv_arg_type = int_type;
    ...
    if (m_class_is_delegate(klass)) {
      if (t->byref) {
        ...
        mono_mb_emit_ldarg(mb, argnum);
        mono_mb_emit_icall_id(mb,
                              conv_to_icall(MONO_MARSHAL_CONV_DEL_FTN, NULL));
        mono_mb_emit_stloc(mb, conv_arg);
      }
      ...
      MonoMarshalNative encoding =
          mono_marshal_get_string_encoding(m->piinfo, spec);
      MonoMarshalConv conv =
          mono_marshal_get_stringbuilder_to_ptr_conv(m->piinfo, spec);
  ...
  case MARSHAL_ACTION_CONV_OUT:
    ...
    if (m_class_is_delegate(klass)) {
      if (t->byref) {
        ...
        mono_mb_emit_ldloc(mb, conv_arg);
        mono_mb_emit_icall_id(mb,
                              conv_to_icall(MONO_MARSHAL_CONV_FTN_DEL, NULL));
        mono_mb_emit_byte(mb, CEE_STIND_REF);
      }
      break;
    }
  ...
  case MARSHAL_ACTION_CONV_RESULT:
    if (m_class_is_delegate(klass)) {
      ...
      // FIXME:
      char *msg = g_strdup_printf(
          "Return marshalling of stringbuilders is not implemented.");
      mono_mb_emit_exception_marshal_directive(mb, msg);
    } else {
      /* set src */
      mono_mb_emit_stloc(mb, 0);
  ...
  case MARSHAL_ACTION_MANAGED_CONV_OUT:
    if (m_class_is_delegate(klass)) {
      if (t->byref) {
        ...
        mono_mb_emit_ldloc(mb, conv_arg);
        mono_mb_emit_icall_id(
            mb, conv_to_icall(MONO_MARSHAL_CONV_DEL_FTN, &stind_op));
        mono_mb_emit_byte(mb, stind_op);
        break;
      }
    }
...
static int emit_marshal_variant_ilgen(EmitMarshalContext *m, int argnum,
                                      MonoType *t, MonoMarshalSpec *spec,
                                      int conv_arg, MonoType **conv_arg_type,
                                      MarshalAction action) {
#ifndef DISABLE_COM
  MonoMethodBuilder *mb = m->mb;
  MonoType *variant_type =
      m_class_get_byval_arg(mono_class_get_variant_class());
  MonoType *variant_type_byref =
  ...
  case MARSHAL_ACTION_CONV_OUT: {
    if (t->byref &&
        (t->attrs & PARAM_ATTRIBUTE_OUT || !(t->attrs & PARAM_ATTRIBUTE_IN))) {
      mono_mb_emit_ldarg(mb, argnum);
      mono_mb_emit_ldloc_addr(mb, conv_arg);
      mono_mb_emit_managed_call(
          mb, mono_get_Marshal_GetObjectForNativeVariant(), NULL);
...
static void emit_managed_wrapper_ilgen(MonoMethodBuilder *mb,
                                       MonoMethodSignature *invoke_sig,
  ...
  if (!sig->hasthis && sig->param_count != invoke_sig->param_count) {
    ...
    memmove(&sig->params[0], &sig->params[1],
  ...
  /*
   * This icall is special cased in the JIT so it works in native-to-managed
   * wrappers in unattached threads. Keep this in sync with the CEE_JIT_ICALL
   * code in the JIT.
   *
  ...
  if (MONO_TYPE_ISSTRUCT(sig->ret)) {
    ...
    if (!(mono_class_is_explicit_layout(klass) ||
          m_class_is_blittable(klass))) {
      /* This is used by get_marshal_cb ()->emit_marshal_vtype (), but it needs
       * to go right before the call */
      mono_mb_emit_byte(mb, MONO_CUSTOM_PREFIX);
      mono_mb_emit_byte(mb, CEE_MONO_VTADDR);
      mono_mb_emit_stloc(mb, m->vtaddr_var);
    }
  ...
  if (mspecs[0] && mspecs[0]->native == MONO_NATIVE_CUSTOM) {
    mono_emit_marshal(m, 0, sig->ret, mspecs[0], 0, NULL,
  ...
  for (i = 0; i < sig->param_count; i++) {
    ...
    if (spec && spec->native == MONO_NATIVE_CUSTOM) {
      mono_emit_marshal(m, i, t, mspecs[i + 1], tmp_locals[i], NULL,
      ...
      case MONO_TYPE_CLASS:
      case MONO_TYPE_VALUETYPE:
        mono_emit_marshal(m, i, invoke_sig->params[i], mspecs[i + 1],
                          tmp_locals[i], NULL, MARSHAL_ACTION_MANAGED_CONV_OUT);
        break;
      default:
        g_assert_not_reached();
      }
...
static void emit_struct_to_ptr_ilgen(MonoMethodBuilder *mb, MonoClass *klass) {
  MonoType *int_type = mono_get_int_type();
  MonoType *boolean_type = m_class_get_byval_arg(mono_defaults.boolean_class);
  if (m_class_is_blittable(klass)) {
    mono_mb_emit_byte(mb, CEE_LDARG_1);
...
static void emit_ptr_to_struct_ilgen(MonoMethodBuilder *mb, MonoClass *klass) {
  MonoType *int_type = mono_get_int_type();
  if (m_class_is_blittable(klass)) {
    mono_mb_emit_byte(mb, CEE_LDARG_1);
    mono_mb_emit_ldflda(mb, MONO_ABI_SIZEOF(MonoObject));
...
static void emit_create_string_hack_ilgen(MonoMethodBuilder *mb,
                                          MonoMethodSignature *csig,
                                          MonoMethod *res) {
  int i;
...
typedef struct {
  ...
  // If wrap is OBJ_OUT or OBJ_INOUT this is >= 0 and holds the referenced
  // managed object, in case the actual parameter refers to a native frame.
  // Otherwise it is -1.
  int handle;
} IcallHandlesLocal;
...
signature_param_uses_handles(MonoMethodSignature *sig,
                             MonoMethodSignature *generic_sig, int param) {
   ...
   */
  g_assert(!generic_sig ||
           !mono_type_is_generic_parameter(generic_sig->params[param]));
  ...
  if (generic_sig && mono_type_is_byref_internal(generic_sig->params[param]) &&
      (generic_sig->params[param]->type == MONO_TYPE_VAR ||
       generic_sig->params[param]->type == MONO_TYPE_MVAR))
...
static void emit_native_icall_wrapper_ilgen(
    MonoMethodBuilder *mb, MonoMethod *method, MonoMethodSignature *csig,
    gboolean check_exceptions, gboolean aot, MonoMethodPInvoke *piinfo) {
  // FIXME:
  ...
  if (G_UNLIKELY(foreign_icall)) {
    /* FIXME: we only want the transitions for hybrid suspend.  Q: What to do
     * about AOT? */
    need_gc_safe =
        gc_safe_transition_builder_init(&gc_safe_transition_builder, mb, FALSE);
  ...
  if (uses_handles) {
    ...
    // ()
    call_sig = mono_metadata_signature_alloc(get_method_image(method),
                                             csig->param_count);
    ...
    for (int i = 0; i < csig->param_count; ++i) {
      // Determine which args need to be wrapped in handles and adjust icall
      // signature. Here, a handle is a pointer to a volatile local in a managed
      // frame -- which is sufficient and efficient.
      const IcallHandlesWrap w =
          signature_param_uses_handles(csig, generic_sig, i);
      ...
      case ICALL_HANDLES_WRAP_OBJ_OUT:
        call_sig->params[i] = mono_class_get_byref_type(
            mono_class_from_mono_type_internal(csig->params[i]));
        break;
      ...
      case ICALL_HANDLES_WRAP_OBJ_OUT:
        ...
        if (!mb->volatile_locals) {
          gpointer mem = mono_image_alloc0(
              get_method_image(method),
              mono_bitset_alloc_size(csig->param_count + 1, 0));
          mb->volatile_locals =
              mono_bitset_mem_new(mem, csig->param_count + 1, 0);
        }
      ...
      // Load each argument. References into the managed heap get wrapped in
      // handles. Handles here are just pointers to managed volatile locals.
      switch (w) {
      case ICALL_HANDLES_WRAP_NONE:
      case ICALL_HANDLES_WRAP_VALUETYPE_REF:
        // argI = argI
...
static void mb_emit_exception_ilgen(MonoMethodBuilder *mb,
                                    const char *exc_nspace,
                                    const char *exc_name, const char *msg) {
  mono_mb_emit_exception_full(mb, exc_nspace, exc_name, msg);
}

static void mb_emit_exception_for_error_ilgen(MonoMethodBuilder *mb,
                                              const MonoError *error) {
  mono_mb_emit_exception_for_error(mb, (MonoError *)error);
}

static void emit_vtfixup_ftnptr_ilgen(MonoMethodBuilder *mb, MonoMethod *method,
                                      int param_count, guint16 type) {
...
static void emit_icall_wrapper_ilgen(MonoMethodBuilder *mb,
...
static void emit_return_ilgen(MonoMethodBuilder *mb) {
  mono_mb_emit_byte(mb, CEE_RET);
}
