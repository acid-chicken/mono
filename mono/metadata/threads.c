...
#include <objbase.h>
#include <sys/timeb.h>
extern gboolean mono_native_thread_join_handle(HANDLE thread_handle,
...
#define joinable_threads_unlock()                                              \
  mono_coop_mutex_unlock(&joinable_threads_mutex)
 ...
 * When mono_thread_attach_internal is called for a thread, it will be removed
 * from this hash table. Protected by mono_threads_lock ().
 */
...
/* mono_threads_join_threads will take threads from joinable_threads list and
 * wait for them. */
/* When this happens, the tid is not on the list anymore so mono_thread_join
 * assumes the thread has complete */
/* and will return back to the caller. This could cause a race since caller of
 * join assumes thread has completed */
/* and on some OS it could cause errors. Keeping the tid's currently pending a
 * native thread join call */
/* in a separate table (only affecting callers interested in this internal join
 * detail) and look at that table in mono_thread_join */
...
#define default_stacksize_for_thread(thread)                                   \
  ((thread)->stack_size ? (thread)->stack_size : default_stacksize)
...
static void async_abort_internal(MonoInternalThread *thread,
                                 gboolean install_async_abort);
static void self_abort_internal(MonoError *error);
static void async_suspend_internal(MonoInternalThread *thread,
                                   gboolean interrupt);
static void self_suspend_internal(void);

static gboolean
mono_thread_set_interruption_requested_flags(MonoInternalThread *thread,
                                             gboolean sync);

MONO_COLD void mono_set_pending_exception_handle(MonoExceptionHandle exc);

static MonoException *mono_thread_execute_interruption_ptr(void);

static void mono_thread_execute_interruption_void(void);

static gboolean mono_thread_execute_interruption(MonoExceptionHandle *pexc);
...
static guint32 get_next_managed_thread_id(void) {
  return mono_atomic_inc_i32(&managed_thread_id_counter);
}

/*
 * We separate interruptions/exceptions into either sync (they can be processed
 * anytime, normally as soon as they are set, and are set by the same thread)
 * and async (they can't be processed inside abort protected blocks and are
 * normally set by other threads). We can have both a pending sync and async
 * interruption. In this case, the sync exception is processed first. Since we
 * clean sync flag first, mono_thread_execute_interruption must also handle all
 * sync type exceptions before the async type exceptions.
 */
enum {
  ...
  ABORT_PROT_BLOCK_MASK =
      (((1 << ABORT_PROT_BLOCK_BITS) - 1) << ABORT_PROT_BLOCK_SHIFT)
};

static int mono_thread_get_abort_prot_block_count(MonoInternalThread *thread) {
  gsize state = thread->thread_state;
  return (state & ABORT_PROT_BLOCK_MASK) >> ABORT_PROT_BLOCK_SHIFT;
}

gboolean mono_threads_is_current_thread_in_protected_block(void) {
...
void mono_threads_begin_abort_protected_block(void) {
  ...
  do {
    ...
    new_val =
        ((old_state & ABORT_PROT_BLOCK_MASK) >> ABORT_PROT_BLOCK_SHIFT) + 1;
    // bounds check abort_prot_count
    g_assert(new_val > 0);
    g_assert(new_val < (1 << ABORT_PROT_BLOCK_BITS));

    new_state = old_state + (1 << ABORT_PROT_BLOCK_SHIFT);
  } while (mono_atomic_cas_ptr((volatile gpointer *)&thread->thread_state,
                               (gpointer)new_state,
                               (gpointer)old_state) != (gpointer)old_state);

  /* Defer async request since we won't be able to process until exiting the
   * block */
  if (new_val == 1 && (new_state & INTERRUPT_ASYNC_REQUESTED_BIT)) {
    mono_atomic_dec_i32(&mono_thread_interruption_request_flag);
    THREADS_INTERRUPT_DEBUG("[%d] begin abort protected block old_state %ld "
                            "new_state %ld, defer tir %d\n",
                            thread->small_id, old_state, new_state,
...
static gboolean mono_thread_state_has_interruption(gsize state) {
  /* pending exception, self abort */
  if (state & INTERRUPT_SYNC_REQUESTED_BIT)
    return TRUE;

  /* abort, interruption, suspend */
  if ((state & INTERRUPT_ASYNC_REQUESTED_BIT) &&
      !(state & ABORT_PROT_BLOCK_MASK))
    return TRUE;
...
gboolean mono_threads_end_abort_protected_block(void) {
  ...
  if (new_val == 0 && (new_state & INTERRUPT_ASYNC_REQUESTED_BIT)) {
    ...
    THREADS_INTERRUPT_DEBUG("[%d] end abort protected block old_state %ld "
                            "new_state %ld, restore tir %d\n",
                            thread->small_id, old_state, new_state,
                            mono_thread_interruption_request_flag);
  } else {
    THREADS_INTERRUPT_DEBUG(
        "[%d] end abort protected block old_state %ld new_state %ld, tir %d\n",
        thread->small_id, old_state, new_state,
        mono_thread_interruption_request_flag);
  }
...
mono_thread_clear_interruption_requested(MonoInternalThread *thread) {
  ...
  do {
    ...
    if (!(old_state & INTERRUPT_SYNC_REQUESTED_BIT) &&
        (!(old_state & INTERRUPT_ASYNC_REQUESTED_BIT) ||
         (old_state & ABORT_PROT_BLOCK_MASK)))
      return FALSE;
  ...
  mono_atomic_dec_i32(&mono_thread_interruption_request_flag);
  THREADS_INTERRUPT_DEBUG(
      "[%d] clear interruption old_state %ld new_state %ld, tir %d\n",
      thread->small_id, old_state, new_state,
      mono_thread_interruption_request_flag);
  if (mono_thread_interruption_request_flag < 0)
    g_warning("bad mono_thread_interruption_request_flag state");
...
static gboolean mono_thread_clear_interruption_requested_handle(
    MonoInternalThreadHandle thread) {
  // Internal threads are pinned so shallow coop/handle.
  return mono_thread_clear_interruption_requested(
      mono_internal_thread_handle_ptr(thread));
}

/* Returns TRUE is there was a state change and the interruption can be
 * processed */
static gboolean
...
static gboolean mono_thread_set_self_interruption_respect_abort_prot(void) {
  MonoInternalThread *thread = mono_thread_internal_current();
...
mono_thread_set_interruption_requested_flags(MonoInternalThread *thread,
  ...
  if (sync || !(new_state & ABORT_PROT_BLOCK_MASK)) {
    mono_atomic_inc_i32(&mono_thread_interruption_request_flag);
    THREADS_INTERRUPT_DEBUG(
        "[%d] set interruption on [%d] old_state %ld new_state %ld, tir %d\n",
        mono_thread_internal_current()->small_id, thread->small_id, old_state,
        new_state, mono_thread_interruption_request_flag);
  } else {
    THREADS_INTERRUPT_DEBUG("[%d] set interruption on [%d] old_state %ld "
                            "new_state %ld, tir deferred %d\n",
...
static MonoNativeThreadId thread_get_tid(MonoInternalThread *thread) {
  /* We store the tid as a guint64 to keep the object layout constant between
   * platforms */
  return MONO_UINT_TO_NATIVE_THREAD_ID(thread->tid);
}

static void free_synch_cs(MonoCoopMutex *synch_cs) {
  g_assert(synch_cs);
  mono_coop_mutex_destroy(synch_cs);
  g_free(synch_cs);
}

static void free_longlived_thread_data(void *user_data) {
...
static void init_longlived_thread_data(MonoLongLivedThreadData *lltd) {
...
static void dec_longlived_thread_data(MonoLongLivedThreadData *lltd) {
  mono_refcount_dec(lltd);
}

static void lock_thread(MonoInternalThread *thread) {
...
static void unlock_thread(MonoInternalThread *thread) {
  mono_coop_mutex_unlock(thread->longlived->synch_cs);
}

static void lock_thread_handle(MonoInternalThreadHandle thread) {
  lock_thread(mono_internal_thread_handle_ptr(thread));
}

static void unlock_thread_handle(MonoInternalThreadHandle thread) {
  unlock_thread(mono_internal_thread_handle_ptr(thread));
}

static gboolean is_appdomainunloaded_exception(MonoClass *klass) {
...
static gboolean is_threadabort_exception(MonoClass *klass) {
...
static guint32 MAKE_SPECIAL_STATIC_OFFSET(guint32 index, guint32 offset,
                                          guint32 type) {
...
static gpointer get_thread_static_data(MonoInternalThread *thread,
                                       guint32 offset) {
  g_assert(ACCESS_SPECIAL_STATIC_OFFSET(offset, type) ==
...
static gpointer get_context_static_data(MonoAppContext *ctx, guint32 offset) {
  g_assert(ACCESS_SPECIAL_STATIC_OFFSET(offset, type) ==
...
static MonoThread **
get_current_thread_ptr_for_domain(MonoDomain *domain,
                                  MonoInternalThread *thread) {
  guint32 offset;

  MONO_STATIC_POINTER_INIT(MonoClassField, current_thread_field)

  current_thread_field = mono_class_get_field_from_name_full(
      mono_defaults.thread_class, "current_thread", NULL);
  g_assert(current_thread_field);

  MONO_STATIC_POINTER_INIT_END(MonoClassField, current_thread_field)

  ERROR_DECL(thread_vt_error);
  mono_class_vtable_checked(domain, mono_defaults.thread_class,
                            thread_vt_error);
  mono_error_assert_ok(thread_vt_error);
  mono_domain_lock(domain);
  offset = GPOINTER_TO_UINT(
      g_hash_table_lookup(domain->special_static_fields, current_thread_field));
...
static void set_current_thread_for_domain(MonoDomain *domain,
                                          MonoInternalThread *thread,
                                          MonoThread *current) {
#ifndef ENABLE_NETCORE
  MonoThread **current_thread_ptr =
      get_current_thread_ptr_for_domain(domain, thread);
...
static MonoThread *create_thread_object(MonoDomain *domain,
...
static void init_internal_thread_object(MonoInternalThread *thread) {
  ...
  if (mono_gc_is_moving()) {
    ...
    MONO_GC_REGISTER_ROOT_PINNING(thread->thread_pinning_ref,
                                  MONO_ROOT_SOURCE_THREADING, NULL,
...
static MonoInternalThread *create_internal_thread_object(void) {
  ERROR_DECL(error);
  MonoInternalThread *thread;
  MonoVTable *vt;

  vt = mono_class_vtable_checked(mono_get_root_domain(),
                                 mono_defaults.internal_thread_class, error);
  mono_error_assert_ok(error);
...
static void mono_thread_internal_set_priority(MonoInternalThread *internal,
  ...
  MONO_EXIT_GC_SAFE;
  if (res != 0)
    g_error("%s: pthread_getschedparam failed, error: \"%s\" (%d)", __func__,
            g_strerror(res), res);
  ...
  if (res != 0) {
    if (res == EPERM) {
       ...
       */
      g_warning("%s: pthread_setschedparam failed, error: \"%s\" (%d)",
                __func__, g_strerror(res), res);
#endif
      return;
    }
    g_error("%s: pthread_setschedparam failed, error: \"%s\" (%d)", __func__,
            g_strerror(res), res);
  }
#endif /* HOST_WIN32 */
}

static void mono_alloc_static_data(gpointer **static_data_ptr, guint32 offset,
                                   void *alloc_key, gboolean threadlocal);

static gboolean mono_thread_attach_internal(MonoThread *thread,
  ...
  /* It is needed to store the MonoInternalThread on the MonoThreadInfo, because
   ...
   *      - mono_thread_internal_current returns NULL -> fails to detach the
   * MonoInternalThread. */
  mono_thread_info_set_internal_thread_gchandle(
      info, mono_gchandle_new_internal((MonoObject *)internal, FALSE));

  internal->handle = mono_threads_open_thread_handle(info->handle);
  internal->native_handle = MONO_NATIVE_THREAD_HANDLE_TO_GPOINTER(
      mono_threads_open_native_thread_handle(info->native_handle));
  ...
  THREAD_DEBUG(g_message("%s: (%" G_GSIZE_FORMAT
                         ") Setting current_object_key to %p",
                         __func__, mono_native_thread_id_get(), internal));
  ...
  if (!threads) {
    threads = mono_g_hash_table_new_type_internal(
        NULL, NULL, MONO_HASH_VALUE_GC, MONO_ROOT_SOURCE_THREADING, NULL,
        "Thread Table");
  }

  /* We don't need to duplicate thread->handle, because it is
   * only closed when the thread object is finalized by the GC. */
  mono_g_hash_table_insert_internal(threads, (gpointer)(gsize)(internal->tid),
                                    internal);
  ...
  if (thread_static_info.offset || thread_static_info.idx > 0) {
    ...
    guint32 offset = MAKE_SPECIAL_STATIC_OFFSET(thread_static_info.idx,
                                                thread_static_info.offset, 0);
    mono_alloc_static_data(
        &internal->static_data, offset,
        (void *)(gsize)MONO_UINT_TO_NATIVE_THREAD_ID(internal->tid), TRUE);
  }
  ...
  if (domain != root_domain)
    MONO_OBJECT_SETREF_INTERNAL(internal, root_domain_thread,
                                create_thread_object(root_domain, internal));
  else
    MONO_OBJECT_SETREF_INTERNAL(internal, root_domain_thread, thread);

  if (domain != root_domain)
    set_current_thread_for_domain(root_domain, internal,
                                  internal->root_domain_thread);

  set_current_thread_for_domain(domain, internal, thread);

  THREAD_DEBUG(g_message("%s: Attached thread ID %" G_GSIZE_FORMAT
                         " (handle %p)",
                         __func__, internal->tid, internal->handle));
...
static void mono_thread_detach_internal(MonoInternalThread *thread) {
  MonoThreadInfo *info;
  ...
  THREAD_DEBUG(g_message("%s: mono_thread_detach for %p (%" G_GSIZE_FORMAT ")",
                         __func__, thread, (gsize)thread->tid));
   ...
   * the joinable thread list should just about to exit and not blocking a
   * potential join call. Owner of threads attached to the runtime but not
   * identified as runtime threads needs to make sure thread detach calls won't
  ...
  the local flag it will be left set and positively unbalance the global
  counter.

  Leaving the counter unbalanced will cause a performance degradation since all
  threads will now keep checking their local flags all the time.
  */
  ...
  if (!mono_g_hash_table_lookup_extended(threads, (gpointer)thread->tid, NULL,
                                         (gpointer *)&value)) {
    g_error(
        "%s: thread %p (tid: %p) should not have been removed yet from threads",
        __func__, thread, thread->tid);
  } else if (thread != value) {
    /* We have to check whether the thread object for the tid is still the same
     * in the table because the thread might have been destroyed and the tid
     * reused in the meantime, in which case the tid would be in
     * the table, but with another thread object. */
    g_error("%s: thread %p (tid: %p) do not match with value %p (tid: %p)",
            __func__, thread, thread->tid, value, value->tid);
  }
  ...
  MONO_PROFILER_RAISE(gc_root_unregister,
...
static void fire_attach_profiler_events(MonoNativeThreadId tid) {
  ...
  MONO_PROFILER_RAISE(
      ...
      ((const mono_byte *)(info->stack_start_limit),
       ...
       MONO_ROOT_SOURCE_STACK, (void *)tid, "Thread Stack"));

  // The handle stack is a pseudo-root similar to the finalizer queues.
  MONO_PROFILER_RAISE(gc_root_register,
                      ((const mono_byte *)info->handle_stack, 1,
                       MONO_ROOT_SOURCE_HANDLE, (void *)tid, "Handle Stack"));
}
...
static guint32 WINAPI start_wrapper_internal(StartInfo *start_info,
  ...
  gsize tid;
  /*
   * We don't create a local to hold start_info->thread, so hopefully it won't
   * get pinned during a GC stack walk.
   */
  MonoThread *thread;
  MonoInternalThread *internal;
  MonoObject *start_delegate;
  ...
  THREAD_DEBUG(g_message("%s: (%" G_GSIZE_FORMAT ") Start wrapper", __func__,
  ...
  mono_thread_internal_set_priority(internal,
                                    (MonoThreadPriority)internal->priority);
  ...
  /*
   * Call this after calling start_notify, since the profiler callback might
   * want to lock the thread, and the lock is held by thread_start () which
   * waits for start_notify.
   */
  ...
  if (internal->name.chars) {
    ...
    if (internal->name.chars) {
      ...
      mono_native_thread_set_name(MONO_UINT_TO_NATIVE_THREAD_ID(internal->tid),
                                  internal->name.chars);
    }
  ...
  if (start_func) {
    ...
    cb = mono_class_get_method_from_name_checked(internal->obj.vtable->klass,
    ...
    /* we may want to handle the exception here. See comment below on unhandled
     * exceptions */
  ...
  THREAD_DEBUG(g_message("%s: (%" G_GSIZE_FORMAT ") Start wrapper terminating",
                         __func__, mono_native_thread_id_get()));
...
static mono_thread_start_return_t WINAPI start_wrapper(gpointer data) {
 ...
 */
static gboolean create_thread(MonoThread *thread, MonoInternalThread *internal,
                              MonoObject *start_delegate,
                              MonoThreadStart start_func,
                              gpointer start_func_arg,
  ...
  if (threads_starting_up == NULL) {
    threads_starting_up = mono_g_hash_table_new_type_internal(
        NULL, NULL, MONO_HASH_KEY_VALUE_GC, MONO_ROOT_SOURCE_THREADING, NULL,
  ...
  if (!mono_thread_platform_create_thread(start_wrapper, start_info,
                                          &stack_set_size, &tid)) {
    /* The thread couldn't be created, so set an exception */
    mono_threads_lock();
    mono_g_hash_table_remove(threads_starting_up, thread);
    mono_threads_unlock();
    mono_error_set_execution_engine(error, "Couldn't create thread. Error 0x%x",
  ...
  THREAD_DEBUG(g_message(
      "%s: (%" G_GSIZE_FORMAT ") Launching thread %p (%" G_GSIZE_FORMAT ")",
      __func__, mono_native_thread_id_get(), internal, (gsize)internal->tid));
 ...
 */
guint32 mono_threads_get_default_stacksize(void) { return default_stacksize; }
 ...
 */
MonoInternalThread *mono_thread_create_internal(MonoDomain *domain,
  ...
  res = create_thread(thread, internal, NULL, (MonoThreadStart)func, arg, flags,
                      error);
  (void)res;
...
mono_thread_create_internal_handle(MonoDomain *domain, gpointer func,
                                   gpointer arg, MonoThreadCreateFlags flags,
                                   MonoError *error) {
  // FIXME invert
  return MONO_HANDLE_NEW(
      MonoInternalThread,
      mono_thread_create_internal(domain, func, arg, flags, error));
}
...
gboolean mono_thread_create_checked(MonoDomain *domain, gpointer func,
                                    gpointer arg, MonoError *error) {
  return (NULL != mono_thread_create_internal(
                      domain, func, arg, MONO_THREAD_CREATE_FLAGS_NONE, error));
}
 ...
 *
 * Attach the current thread as a tool thread. DON'T USE THIS FUNCTION WITHOUT
 * READING ALL DISCLAIMERS.
 *
 ...
 *
 * Detach the current thread from the runtime if it is exiting, i.e. it is
 * running pthread dtors. This should be used at the end of embedding code which
 * calls into managed code, and which can be called from pthread dtors, like
 * <code>dealloc:</code> implementations in Objective-C.
 */
...
gboolean mono_thread_internal_current_is_attached(void) {
  MonoInternalThread *internal;
...
void mono_thread_exit(void) {
  ...
  THREAD_DEBUG(g_message("%s: mono_thread_exit for %p (%" G_GSIZE_FORMAT ")",
                         __func__, thread, (gsize)thread->tid));

  mono_thread_detach_internal(thread);

  /* we could add a callback here for embedders to use. */
  if (mono_thread_get_main() &&
      (thread == mono_thread_get_main()->internal_thread))
...
mono_thread_construct_internal(MonoThreadObjectHandle this_obj_handle) {
  ...
  MonoGCHandle thread_gchandle = mono_gchandle_from_handle(
      MONO_HANDLE_CAST(MonoObject, this_obj_handle), TRUE);

  MonoThreadObject *this_obj = MONO_HANDLE_RAW(this_obj_handle);

  mono_atomic_cas_ptr((volatile gpointer *)&this_obj->internal_thread, internal,
                      NULL);
...
#ifndef ENABLE_NETCORE
void ves_icall_System_Threading_Thread_ConstructInternalThread(
    MonoThreadObjectHandle this_obj_handle, MonoError *error) {
  mono_thread_construct_internal(this_obj_handle);
}
#endif

void ves_icall_System_Threading_Thread_GetCurrentThread(
    MonoThread *volatile *thread) {
  *thread = mono_thread_current();
}

static MonoInternalThread *
thread_handle_to_internal_ptr(MonoThreadObjectHandle thread_handle) {
  return MONO_HANDLE_GETVAL(
      thread_handle, internal_thread); // InternalThreads are always pinned.
}

static void
mono_error_set_exception_thread_state(MonoError *error,
                                      const char *exception_message) {
  mono_error_set_generic_error(error, "System.Threading",
                               "ThreadStateException", "%s", exception_message);
}

static void
mono_error_set_exception_thread_not_started_or_dead(MonoError *error) {
  mono_error_set_exception_thread_state(
      error, "Thread has not been started, or is dead.");
}

#ifndef ENABLE_NETCORE
MonoBoolean ves_icall_System_Threading_Thread_Thread_internal(
    MonoThreadObjectHandle thread_handle, MonoObjectHandle start_handle,
    MonoError *error) {
  MonoInternalThread *internal;
  gboolean res;
  ...
  THREAD_DEBUG(
      g_message("%s: Trying to start a new thread: this (%p) start (%p)",
                __func__, this_obj, start));
  ...
  if ((internal->state & ThreadState_Unstarted) == 0) {
    UNLOCK_THREAD(internal);
    mono_error_set_exception_thread_state(error,
  ...
  res = create_thread(this_obj, internal, start, NULL, NULL,
  ...
  THREAD_DEBUG(g_message("%s: Started thread ID %" G_GSIZE_FORMAT
                         " (handle %p)",
                         __func__, tid, thread));
...
static void mono_thread_name_cleanup(MonoThreadName *name) {
...
void ves_icall_System_Threading_InternalThread_Thread_free_internal(
    MonoInternalThreadHandle this_obj_handle, MonoError *error) {
  MonoInternalThread *this_obj =
      mono_internal_thread_handle_ptr(this_obj_handle);
  THREAD_DEBUG(g_message("%s: Closing thread %p, handle %p", __func__, this_obj,
                         this_obj->handle));
  ...
  mono_threads_close_native_thread_handle(
      MONO_GPOINTER_TO_NATIVE_THREAD_HANDLE(this_obj->native_handle));
...
static void mono_sleep_internal(gint32 ms, MonoBoolean allow_interruption,
                                MonoError *error) {
...
void ves_icall_System_Threading_Thread_Sleep_internal(
    gint32 ms, MonoBoolean allow_interruption, MonoError *error) {
...
void ves_icall_System_Threading_Thread_SpinWait_nop(MonoError *error) {}
#endif

#ifndef ENABLE_NETCORE
gint32 ves_icall_System_Threading_Thread_GetDomainID(MonoError *error) {
...
char *mono_thread_get_name_utf8(MonoThread *thread) {
  ...
  char *tname =
      (char *)g_memdup(internal->name.chars, internal->name.length + 1);
...
#ifndef ENABLE_NETCORE
MonoStringHandle ves_icall_System_Threading_Thread_GetName_internal(
    MonoInternalThreadHandle thread_handle, MonoError *error) {
  // InternalThreads are always pinned, so shallowly coop-handleize.
  MonoInternalThread *const this_obj =
  ...
  if (this_obj->name.chars) {
    ...
    if (this_obj->name.chars)
      str = mono_string_new_utf8_len(mono_domain_get(), this_obj->name.chars,
                                     this_obj->name.length, error);
...
// Unusal function:
//  - MonoError is optional -- failure is usually not interesting, except the
//  documented failure mode for managed callers.
//  - name16 only used on Windows.
//  - name8 is either constant, or g_free'able -- this function always takes
//  ownership and never copies.
//
void mono_thread_set_name(MonoInternalThread *this_obj, const char *name8,
                          size_t name8_length, const gunichar2 *name16,
  ...
  // In an unusual race, the name might briefly not revert to what the spec
  // requires.
  //
  if ((flags & MonoSetThreadNameFlag_RepeatedlyButOptimized) &&
      name8 == this_obj->name.chars) {
  ...
  if (flags & MonoSetThreadNameFlag_Reset) {
    ...
    if (error)
      mono_error_set_invalid_operation(error, "%s",
...
void ves_icall_System_Threading_Thread_SetName_icall(
    MonoInternalThreadHandle thread_handle, const gunichar2 *name16,
    gint32 name16_length, MonoError *error) {
  long name8_length = 0;

  char *name8 =
      name16 ? g_utf16_to_utf8(name16, name16_length, NULL, &name8_length, NULL)
             : NULL;

  mono_thread_set_name(mono_internal_thread_handle_ptr(thread_handle), name8,
                       (gint32)name8_length, name16,
 ...
 */
int ves_icall_System_Threading_Thread_GetPriority(
    MonoThreadObjectHandle this_obj, MonoError *error) {
  gint32 priority;
 ...
 */
void ves_icall_System_Threading_Thread_SetPriority(
    MonoThreadObjectHandle this_obj, int priority, MonoError *error) {
  MonoInternalThread *internal = thread_handle_to_internal_ptr(this_obj);
...
static MonoArrayHandle byte_array_to_domain(MonoArrayHandle arr,
                                            MonoDomain *domain,
                                            MonoError *error) {
  ...
  struct {
    ...
    gpointer p;
    MonoGCHandle gchandle;
  } source = {arr}, dest = {mono_array_new_handle(
                        domain, mono_defaults.byte_class, size, error)};
  goto_if_nok(error, exit);

  // Pin both arrays.
  source.p =
      mono_array_handle_pin_with_size(source.handle, size, 0, &source.gchandle);
  dest.p =
      mono_array_handle_pin_with_size(dest.handle, size, 0, &dest.gchandle);
...
#ifndef ENABLE_NETCORE
MonoArrayHandle
ves_icall_System_Threading_Thread_ByteArrayToRootDomain(MonoArrayHandle arr,
                                                        MonoError *error) {
  return byte_array_to_domain(arr, mono_get_root_domain(), error);
}

MonoArrayHandle
ves_icall_System_Threading_Thread_ByteArrayToCurrentDomain(MonoArrayHandle arr,
                                                           MonoError *error) {
...
static MonoThreadObjectHandle mono_thread_current_handle(void) {
...
MonoInternalThreadHandle mono_thread_internal_current_handle(void) {
  return MONO_HANDLE_NEW(MonoInternalThread, mono_thread_internal_current());
}

static MonoThreadInfoWaitRet
mono_join_uninterrupted(MonoThreadHandle *thread_to_join, gint32 ms,
                        MonoError *error) {
...
MonoBoolean ves_icall_System_Threading_Thread_Join_internal(
    MonoThreadObjectHandle thread_handle, int ms, MonoError *error) {
  if (mono_thread_current_check_pending_interrupt())
    return FALSE;

  // Internal threads are pinned so shallow coop/handle.
  MonoInternalThread *const thread =
  ...
  THREAD_DEBUG(
      g_message("%s: joining thread handle %p, %d ms", __func__, handle, ms));
...
static gint32 map_native_wait_result_to_managed(MonoW32HandleWaitRet val,
                                                gsize numobjects) {
  if (val >= MONO_W32HANDLE_WAIT_RET_SUCCESS_0 &&
      val < MONO_W32HANDLE_WAIT_RET_SUCCESS_0 + numobjects) {
    return WAIT_OBJECT_0 + (val - MONO_W32HANDLE_WAIT_RET_SUCCESS_0);
  } else if (val >= MONO_W32HANDLE_WAIT_RET_ABANDONED_0 &&
             val < MONO_W32HANDLE_WAIT_RET_ABANDONED_0 + numobjects) {
...
gint32 ves_icall_System_Threading_WaitHandle_Wait_internal(gpointer *handles,
                                                           gint32 numhandles,
                                                           MonoBoolean waitall,
                                                           gint32 timeout,
                                                           MonoError *error) {
  ...
  if (numhandles == 1 && timeout == MONO_INFINITE_WAIT) {
    ...
    for (int i = 0; i < numhandles; ++i) {
      if (mono_w32handle_handle_is_owned(handles[i]) ||
          mono_w32handle_handle_is_signalled(handles[i])) {
        signalled = TRUE;
        break;
      }
    }
    if (!signalled) {
      mono_error_set_synchronization_lock(
          error, "Cannot wait on events on this runtime.");
      return 0;
    }
  }
#endif
  ...
  for (;;) {
    ...
    ret = mono_w32handle_wait_multiple(handles, numhandles, waitall,
                                       timeoutLeft, TRUE, error);
...
gint32 ves_icall_System_Threading_WaitHandle_SignalAndWait_Internal(
    gpointer toSignal, gpointer toWait, gint32 ms, MonoError *error) {
  MonoW32HandleWaitRet ret;
  MonoInternalThread *thread = mono_thread_internal_current();
...
gint32 ves_icall_System_Threading_Interlocked_Exchange_Int(gint32 *location,
                                                           gint32 value) {
  return mono_atomic_xchg_i32(location, value);
}

void ves_icall_System_Threading_Interlocked_Exchange_Object(
    MonoObject *volatile *location, MonoObject *volatile *value,
    MonoObject *volatile *res) {
  // Coop-equivalency here via pointers to pointers.
  // value and res are to managed frames, location ought to be (or member or
  // global) but it cannot be guaranteed.
  //
  ...
  // however coop also presently breaks identity of location and would therefore
  // never work.
  //
  *res =
      (MonoObject *)mono_atomic_xchg_ptr((volatile gpointer *)location, *value);
  mono_gc_wbarrier_generic_nostore_internal(
      (gpointer)location); // FIXME volatile
}

gpointer
ves_icall_System_Threading_Interlocked_Exchange_IntPtr(gpointer *location,
                                                       gpointer value) {
  return mono_atomic_xchg_ptr(location, value);
}

gfloat ves_icall_System_Threading_Interlocked_Exchange_Single(gfloat *location,
                                                              gfloat value) {
...
gint64 ves_icall_System_Threading_Interlocked_Exchange_Long(gint64 *location,
                                                            gint64 value) {
#if SIZEOF_VOID_P == 4
  if (G_UNLIKELY((size_t)location & 0x7)) {
    gint64 ret;
...
gdouble
ves_icall_System_Threading_Interlocked_Exchange_Double(gdouble *location,
                                                       gdouble value) {
...
gint32 ves_icall_System_Threading_Interlocked_CompareExchange_Int(
    gint32 *location, gint32 value, gint32 comparand) {
  return mono_atomic_cas_i32(location, value, comparand);
}

gint32 ves_icall_System_Threading_Interlocked_CompareExchange_Int_Success(
    gint32 *location, gint32 value, gint32 comparand, MonoBoolean *success) {
  gint32 r = mono_atomic_cas_i32(location, value, comparand);
  *success = r == comparand;
  return r;
}

void ves_icall_System_Threading_Interlocked_CompareExchange_Object(
    MonoObject *volatile *location, MonoObject *volatile *value,
    MonoObject *volatile *comparand, MonoObject *volatile *res) {
  // Coop-equivalency here via pointers to pointers.
  // value and comparand and res are to managed frames, location ought to be (or
  // member or global) but it cannot be guaranteed.
  //
  // This also handles generic T case, T constrained to a class.
  //
  // This is not entirely convincing due to lack of volatile in the caller,
  ...
  // never work.
  //
  *res = (MonoObject *)mono_atomic_cas_ptr((volatile gpointer *)location,
                                           *value, *comparand);
  mono_gc_wbarrier_generic_nostore_internal(
      (gpointer)location); // FIXME volatile
}
...
gpointer ves_icall_System_Threading_Interlocked_CompareExchange_IntPtr(
    gpointer *location, gpointer value, gpointer comparand) {
  return mono_atomic_cas_ptr(location, value, comparand);
}

gfloat ves_icall_System_Threading_Interlocked_CompareExchange_Single(
    gfloat *location, gfloat value, gfloat comparand) {
...
gdouble ves_icall_System_Threading_Interlocked_CompareExchange_Double(
    gdouble *location, gdouble value, gdouble comparand) {
#if SIZEOF_VOID_P == 8
  LongDoubleUnion val, comp, ret;

  val.fval = value;
  comp.fval = comparand;
  ret.ival = (gint64)mono_atomic_cas_ptr(
      (gpointer *)location, (gpointer)val.ival, (gpointer)comp.ival);
...
gint64 ves_icall_System_Threading_Interlocked_CompareExchange_Long(
    gint64 *location, gint64 value, gint64 comparand) {
#if SIZEOF_VOID_P == 4
  if (G_UNLIKELY((size_t)location & 0x7)) {
    gint64 old;
    mono_interlocked_lock();
...
gint32 ves_icall_System_Threading_Interlocked_Add_Int(gint32 *location,
                                                      gint32 value) {
  return mono_atomic_add_i32(location, value);
}

gint64 ves_icall_System_Threading_Interlocked_Add_Long(gint64 *location,
                                                       gint64 value) {
#if SIZEOF_VOID_P == 4
  if (G_UNLIKELY((size_t)location & 0x7)) {
    gint64 ret;
...
void ves_icall_System_Threading_Thread_ClrState(
    MonoInternalThreadHandle this_obj, guint32 state, MonoError *error) {
  // InternalThreads are always pinned, so shallowly coop-handleize.
  mono_thread_clr_state(mono_internal_thread_handle_ptr(this_obj),
                        (MonoThreadState)state);
}

void ves_icall_System_Threading_Thread_SetState(
    MonoInternalThreadHandle thread_handle, guint32 state, MonoError *error) {
  // InternalThreads are always pinned, so shallowly coop-handleize.
  mono_thread_set_state(mono_internal_thread_handle_ptr(thread_handle),
                        (MonoThreadState)state);
}

guint32 ves_icall_System_Threading_Thread_GetState(
    MonoInternalThreadHandle thread_handle, MonoError *error) {
  // InternalThreads are always pinned, so shallowly coop-handleize.
...
void ves_icall_System_Threading_Thread_Interrupt_internal(
    MonoThreadObjectHandle thread_handle, MonoError *error) {
  // Internal threads are pinned so shallow coop/handle.
  MonoInternalThread *const thread =
      thread_handle_to_internal_ptr(thread_handle);
  ...
  gboolean const throw_ =
      current != thread && (thread->state & ThreadState_WaitSleepJoin);
...
static gboolean request_thread_abort(MonoInternalThread *thread,
...
// though this would still work efficiently.
{
  ...
  THREAD_DEBUG(g_message(
      "%s: (%" G_GSIZE_FORMAT ") Abort requested for %p (%" G_GSIZE_FORMAT ")",
      __func__, mono_native_thread_id_get(), thread, (gsize)thread->tid));
...
void ves_icall_System_Threading_Thread_Abort(
    MonoInternalThreadHandle thread_handle, MonoObjectHandle state,
    MonoError *error) {
  // InternalThreads are always pinned, so shallowly coop-handleize.
  MonoInternalThread *const thread =
      mono_internal_thread_handle_ptr(thread_handle);
...
#ifndef ENABLE_NETCORE
void ves_icall_System_Threading_Thread_ResetAbort(
    MonoThreadObjectHandle this_obj, MonoError *error) {
  MonoInternalThread *thread = mono_thread_internal_current();
  ...
  if (!was_aborting) {
    mono_error_set_exception_thread_state(
        error, "Unable to reset abort because no abort was requested");
    return;
...
#ifndef ENABLE_NETCORE
MonoObjectHandle ves_icall_System_Threading_Thread_GetAbortExceptionState(
    MonoThreadObjectHandle this_obj, MonoError *error) {
  ...
  MonoObjectHandle state =
      mono_gchandle_get_target_handle(thread->abort_state_handle);
  ...
  MonoObjectHandle deserialized =
      mono_object_xdomain_representation(state, domain, error);
  ...
  MonoExceptionHandle invalid_op_exc = mono_exception_new_invalid_operation(
      "Thread.ExceptionState cannot access an ExceptionState from a different "
      "AppDomain",
      error_creating_exception);
  mono_error_assert_ok(error_creating_exception);
  g_assert(!is_ok(error) && 1);
  MONO_HANDLE_SET(invalid_op_exc, inner_ex,
...
static gboolean mono_thread_suspend(MonoInternalThread *thread) {
  LOCK_THREAD(thread);

  if (thread->state &
      (ThreadState_Unstarted | ThreadState_Aborted | ThreadState_Stopped)) {
    UNLOCK_THREAD(thread);
    return FALSE;
  }

  if (thread->state & (ThreadState_Suspended | ThreadState_SuspendRequested |
...
#ifndef ENABLE_NETCORE
void ves_icall_System_Threading_Thread_Suspend(MonoThreadObjectHandle this_obj,
                                               MonoError *error) {
...
void ves_icall_System_Threading_Thread_Resume(
    MonoThreadObjectHandle thread_handle, MonoError *error) {
  // Internal threads are pinned so shallow coop/handle.
  MonoInternalThread *const internal_thread =
      thread_handle_to_internal_ptr(thread_handle);
...
gboolean mono_threads_is_critical_method(MonoMethod *method) {
...
static gboolean find_wrapper(MonoMethod *m, gint no, gint ilo, gboolean managed,
                             gpointer data) {
  if (managed)
    return TRUE;
...
static gboolean is_running_protected_wrapper(void) {
...
void mono_thread_stop(MonoThread *thread) {
  ...
  if (internal == mono_thread_internal_current()) {
    ...
    self_abort_internal(error);
    /*
    This function is part of the embeding API and has no way to return the
    exception to be thrown. So what we do is keep the old behavior and raise the
    exception.
    */
...
void ves_icall_System_Threading_Thread_VolatileWrite1(void *ptr, gint8 value) {
  mono_memory_barrier();
  *(volatile gint8 *)ptr = value;
}

void ves_icall_System_Threading_Thread_VolatileWrite2(void *ptr, gint16 value) {
  mono_memory_barrier();
  *(volatile gint16 *)ptr = value;
}

void ves_icall_System_Threading_Thread_VolatileWrite4(void *ptr, gint32 value) {
  mono_memory_barrier();
  *(volatile gint32 *)ptr = value;
}

void ves_icall_System_Threading_Thread_VolatileWrite8(void *ptr, gint64 value) {
  mono_memory_barrier();
  *(volatile gint64 *)ptr = value;
}

void ves_icall_System_Threading_Thread_VolatileWriteIntPtr(void *ptr,
                                                           void *value) {
  mono_memory_barrier();
  *(volatile void **)ptr = value;
}

void ves_icall_System_Threading_Thread_VolatileWriteObject(void *ptr,
                                                           MonoObject *value) {
  mono_memory_barrier();
  mono_gc_wbarrier_generic_store_internal(ptr, value);
}

void ves_icall_System_Threading_Thread_VolatileWriteDouble(void *ptr,
                                                           double value) {
  mono_memory_barrier();
  *(volatile double *)ptr = value;
}

void ves_icall_System_Threading_Thread_VolatileWriteFloat(void *ptr,
                                                          float value) {
  mono_memory_barrier();
  *(volatile float *)ptr = value;
}

void ves_icall_System_Threading_Volatile_Write8(void *ptr, gint64 value) {
#if SIZEOF_VOID_P == 4
  if (G_UNLIKELY((size_t)ptr & 0x7)) {
    mono_interlocked_lock();
    *(gint64 *)ptr = value;
...
void ves_icall_System_Threading_Volatile_WriteU8(void *ptr, guint64 value) {
#if SIZEOF_VOID_P == 4
  if (G_UNLIKELY((size_t)ptr & 0x7)) {
    mono_interlocked_lock();
    *(guint64 *)ptr = value;
...
void ves_icall_System_Threading_Volatile_WriteDouble(void *ptr, double value) {
  LongDoubleUnion u;
...
static void free_context(void *user_data) {
...
void mono_threads_register_app_context(MonoAppContextHandle ctx,
                                       MonoError *error) {
  error_init(error);
  mono_threads_lock();
  ...
  MonoGCHandle gch =
      mono_gchandle_new_weakref_from_handle(MONO_HANDLE_CAST(MonoObject, ctx));
...
#ifndef ENABLE_NETCORE
void ves_icall_System_Runtime_Remoting_Contexts_Context_RegisterContext(
    MonoAppContextHandle ctx, MonoError *error) {
  mono_threads_register_app_context(ctx, error);
}
#endif

void mono_threads_release_app_context(MonoAppContext *ctx, MonoError *error) {
  /*
...
#ifndef ENABLE_NETCORE
void ves_icall_System_Runtime_Remoting_Contexts_Context_ReleaseContext(
    MonoAppContextHandle ctx, MonoError *error) {
  mono_threads_release_app_context(
      MONO_HANDLE_RAW(ctx),
      error); /* FIXME use handles in mono_threads_release_app_context */
}
#endif
...
static gpointer thread_attach(MonoThreadInfo *info) {
  return mono_gc_thread_attach(info);
}

static void thread_detach(MonoThreadInfo *info) {
  MonoInternalThread *internal;
  MonoGCHandle gchandle;
...
static void thread_detach_with_lock(MonoThreadInfo *info) {
  mono_gc_thread_detach_with_lock(info);
}

static gboolean thread_in_critical_region(MonoThreadInfo *info) {
  return mono_gc_thread_in_critical_region(info);
}

static gboolean ip_in_critical_region(MonoDomain *domain, gpointer ip) {
  MonoJitInfo *ji;
  MonoMethod *method;
...
static void thread_flags_changing(MonoThreadInfoFlags old,
                                  MonoThreadInfoFlags new_) {
  mono_gc_skip_thread_changing(!!(new_ & MONO_THREAD_INFO_FLAGS_NO_GC));
}

static void thread_flags_changed(MonoThreadInfoFlags old,
                                 MonoThreadInfoFlags new_) {
  mono_gc_skip_thread_changed(!!(new_ & MONO_THREAD_INFO_FLAGS_NO_GC));
}

void mono_thread_callbacks_init(void) {
...
static void wait_for_tids(struct wait_data *wait, guint32 timeout,
  ...
  THREAD_DEBUG(g_message("%s: %d threads to wait for in this batch", __func__,
                         wait->num));
  ...
  if (check_state_change)
    ret = mono_thread_info_wait_multiple_handle(wait->handles, wait->num,
                                                &background_change_event, FALSE,
                                                timeout, TRUE);
  else
    ret = mono_thread_info_wait_multiple_handle(wait->handles, wait->num, NULL,
                                                TRUE, timeout, TRUE);
  MONO_EXIT_GC_SAFE;
  ...
  if (ret >= MONO_THREAD_INFO_WAIT_RET_SUCCESS_0 &&
      ret < (MONO_THREAD_INFO_WAIT_RET_SUCCESS_0 + wait->num)) {
    MonoInternalThread *internal;
    ...
    if (mono_g_hash_table_lookup(threads, (gpointer)internal->tid) == internal)
      g_error("%s: failed to call mono_thread_detach_internal on thread %p, "
              "InternalThread: %p",
              __func__, internal->tid, internal);
    mono_threads_unlock();
  }
}
...
static void build_wait_tids(gpointer key, gpointer value, gpointer user) {
  ...
  if (wait->num < MONO_W32HANDLE_MAXIMUM_WAIT_OBJECTS - 1) {
    ...
    if (thread->state & ThreadState_Background) {
      THREAD_DEBUG(g_message("%s: ignoring background thread %" G_GSIZE_FORMAT,
                             __func__, (gsize)thread->tid));
      return; /* just leave, ignore */
    }

    if (mono_gc_is_finalizer_internal_thread(thread)) {
      THREAD_DEBUG(g_message("%s: ignoring finalizer thread %" G_GSIZE_FORMAT,
                             __func__, (gsize)thread->tid));
      return;
    }

    if (thread == mono_thread_internal_current()) {
      THREAD_DEBUG(g_message("%s: ignoring current thread %" G_GSIZE_FORMAT,
                             __func__, (gsize)thread->tid));
      return;
    }
    ...
    if (mono_thread_get_main() &&
        (thread == mono_thread_get_main()->internal_thread)) {
      THREAD_DEBUG(g_message("%s: ignoring main thread %" G_GSIZE_FORMAT,
                             __func__, (gsize)thread->tid));
      return;
    }

    if (thread->flags & MONO_THREAD_FLAG_DONT_MANAGE) {
      THREAD_DEBUG(g_message("%s: ignoring thread %" G_GSIZE_FORMAT
                             "with DONT_MANAGE flag set.",
                             __func__, (gsize)thread->tid));
      return;
    }

    THREAD_DEBUG(
        g_message("%s: Invoking mono_thread_manage callback on thread %p",
                  __func__, thread));
    if ((thread->manage_callback == NULL) ||
        (thread->manage_callback(thread->root_domain_thread) == TRUE)) {
      wait->handles[wait->num] =
          mono_threads_open_thread_handle(thread->handle);
      wait->threads[wait->num] = thread;
      wait->num++;

      THREAD_DEBUG(g_message("%s: adding thread %" G_GSIZE_FORMAT, __func__,
                             (gsize)thread->tid));
    } else {
      THREAD_DEBUG(g_message(
          "%s: ignoring (because of callback) thread %" G_GSIZE_FORMAT,
          __func__, (gsize)thread->tid));
    }
...
static void abort_threads(gpointer key, gpointer value, gpointer user) {
  struct wait_data *wait = (struct wait_data *)user;
 ...
 * shutting down, the calling thread is suspended/stopped, and this function
 * never returns.
 */
void mono_threads_set_shutting_down(void) {
  ...
  if (shutting_down) {
    ...
    if (current_thread->state &
        (ThreadState_SuspendRequested | ThreadState_AbortRequested)) {
 ...
 */
void mono_thread_manage_internal(void) {
  ...
  do {
    ...
    THREAD_DEBUG(g_message("%s: There are %d threads to join", __func__,
    ...
    THREAD_DEBUG(
        g_message("%s: I have %d threads after waiting.", __func__, wait->num));
  } while (wait->num > 0);

  /* Mono is shutting down, so just wait for the end */
  if (!mono_runtime_try_shutdown()) {
    /*FIXME mono_thread_suspend probably should call
     * mono_thread_execute_interruption when self interrupting. */
...
#ifndef ENABLE_NETCORE
static void collect_threads_for_suspend(gpointer key, gpointer value,
                                        gpointer user_data) {
  MonoInternalThread *thread = (MonoInternalThread *)value;
  struct wait_data *wait = (struct wait_data *)user_data;

  /*
   * We try to exclude threads early, to avoid running into the
   * MONO_W32HANDLE_MAXIMUM_WAIT_OBJECTS limitation. This needs no locking.
   */
  if ((thread->state & ThreadState_Suspended) != 0 ||
      (thread->state & ThreadState_Stopped) != 0)
    return;
 ...
 *
 *  Suspend all managed threads except the finalizer thread and this thread. It
 * is not possible to resume them later.
 */
void mono_thread_suspend_all_other_threads(void) {
   ...
   * they could be starting up, shutting down etc. This means that there could
   * be threads which are not even in the threads hash table yet.
   */
...
static gboolean collect_frame(MonoStackFrameInfo *frame, MonoContext *ctx,
                              gpointer data) {
...
/* This needs to be async safe */
static SuspendThreadResult get_thread_dump(MonoThreadInfo *info, gpointer ud) {
  ...
  if (thread == mono_thread_internal_current())
    mono_get_eh_callbacks()->mono_walk_stack_with_ctx(
        collect_frame, NULL, MONO_UNWIND_SIGNAL_SAFE, ud);
  else
    mono_get_eh_callbacks()->mono_walk_stack_with_state(
        collect_frame, mono_thread_info_get_suspend_state(info),
        MONO_UNWIND_SIGNAL_SAFE, ud);
...
static void collect_thread(gpointer key, gpointer value, gpointer user) {
  CollectThreadsUserData *ud = (CollectThreadsUserData *)user;
  MonoInternalThread *thread = (MonoInternalThread *)value;

  if (ud->nthreads < ud->max_threads)
    ud->threads[ud->nthreads++] =
        mono_gchandle_new_internal(&thread->obj, TRUE);
}
 ...
 */
static int collect_threads(MonoGCHandle *thread_handles, int max_threads) {
...
void mono_gstring_append_thread_name(GString *text,
                                     MonoInternalThread *thread) {
  g_string_append(text, "\n\"");
...
static void dump_thread(MonoInternalThread *thread, ThreadDumpUserData *ud,
                        FILE *output_file) {
  ...
  if (thread == mono_thread_internal_current()) {
    ...
    mono_thread_info_safe_suspend_and_run(thread_get_tid(thread), FALSE,
                                          get_thread_dump, ud);
  }
  ...
  for (i = 0; i < ud->nframes; ++i) {
    ...
    if (method) {
      gchar *location = mono_debug_print_stack_frame(
          method, frame->native_offset, frame->domain);
...
static void mono_get_time_of_day(struct timeval *tv) {
#ifdef WIN32
  struct _timeb time;
  _ftime(&time);
  tv->tv_sec = time.time;
...
static void mono_local_time(const struct timeval *tv, struct tm *tm) {
...
void mono_threads_perform_thread_dump(void) {
  ...
  if (thread_dump_dir != NULL) {
    ...
    strftime(time_str, sizeof(time_str), MONO_STRFTIME_F "_" MONO_STRFTIME_T,
             &tod);
    ms = tv.tv_usec / 1000;
    g_string_append_printf(path, "%s/%s.%03ld.tdump", thread_dump_dir, time_str,
                           ms);
  ...
  for (tindex = 0; tindex < nthreads; ++tindex) {
    ...
    MonoInternalThread *thread =
        (MonoInternalThread *)mono_gchandle_get_target_internal(handle);
...
void ves_icall_System_Threading_Thread_GetStackTraces(
    MonoArrayHandleOut out_threads_handle,
    MonoArrayHandleOut out_stack_frames_handle, MonoError *error) {
  MONO_HANDLE_ASSIGN_RAW(out_threads_handle, NULL);
  MONO_HANDLE_ASSIGN_RAW(out_stack_frames_handle, NULL);

  MonoGCHandle handle = 0;

  MonoStackFrameHandle stack_frame_handle =
      MONO_HANDLE_NEW(MonoStackFrame, NULL);
  MonoReflectionMethodHandle reflection_method_handle =
      MONO_HANDLE_NEW(MonoReflectionMethod, NULL);
  ...
  out_threads = mono_array_new_checked(domain, mono_defaults.thread_class,
                                       nthreads, error);
  goto_if_nok(error, leave);
  MONO_HANDLE_ASSIGN_RAW(out_threads_handle, out_threads);
  out_stack_frames = mono_array_new_checked(domain, mono_defaults.array_class,
                                            nthreads, error);
  goto_if_nok(error, leave);
  ...
  for (tindex = 0; tindex < nthreads; ++tindex) {
    ...
    mono_array_setref_fast(out_threads, tindex,
                           mono_thread_current_for_thread(thread));

    thread_frames = mono_array_new_checked(
    ...
    for (i = 0; i < ud.nframes; ++i) {
      MonoStackFrameInfo *frame = &ud.frames[i];
      MonoMethod *method = NULL;
      MonoStackFrame *sf = (MonoStackFrame *)mono_object_new_checked(
          domain, mono_defaults.stack_frame_class, error);
      ...
      if (method) {
        ...
        location = mono_debug_lookup_source_location(
            method, frame->native_offset, domain);
        if (location) {
          sf->il_offset = location->il_offset;

          if (location->source_file) {
            MonoString *filename =
                mono_string_new_checked(domain, location->source_file, error);
...
static RefStack *ref_stack_new(gint initial_size) {
  RefStack *rs;
...
static void ref_stack_destroy(gpointer ptr) {
...
static void ref_stack_push(RefStack *rs, gpointer ptr) {
  g_assert(rs != NULL);

  if (rs->bottom >= rs->allocated) {
    rs->refs =
        (void **)g_realloc(rs->refs, rs->allocated * 2 * sizeof(gpointer) + 1);
    rs->allocated <<= 1;
...
static void ref_stack_pop(RefStack *rs) {
...
static gboolean ref_stack_find(RefStack *rs, gpointer ptr) {
  gpointer *refs;
...
void mono_thread_pop_appdomain_ref(void) {
  ...
  if (thread) {
     ...
     * ((MonoDomain*)(thread->appdomain_refs->data))->friendly_name); */
    SPIN_LOCK(thread->lock_thread_id);
...
gboolean mono_thread_internal_has_appdomain_ref(MonoInternalThread *thread,
                                                MonoDomain *domain) {
  gboolean res;
  SPIN_LOCK(thread->lock_thread_id);
...
gboolean mono_thread_has_appdomain_ref(MonoThread *thread, MonoDomain *domain) {
  return mono_thread_internal_has_appdomain_ref(thread->internal_thread,
                                                domain);
}
...
static void collect_appdomain_thread(gpointer key, gpointer value,
                                     gpointer user_data) {
  MonoInternalThread *thread = (MonoInternalThread *)value;
  ...
  if (mono_thread_internal_has_appdomain_ref(thread, domain)) {
    ...
    if (data->wait.num < MONO_W32HANDLE_MAXIMUM_WAIT_OBJECTS) {
      data->wait.handles[data->wait.num] =
          mono_threads_open_thread_handle(thread->handle);
...
static const int static_data_size[NUM_STATIC_DATA_IDX] = {64, 256, 1024, 4096};
#else
...
static void mark_slots(void *addr, MonoBitSet **bitmaps,
                       MonoGCMarkFunc mark_func, void *gc_data) {
...
static void mark_tls_slots(void *addr, MonoGCMarkFunc mark_func,
                           void *gc_data) {
  mark_slots(addr, thread_reference_bitmaps, mark_func, gc_data);
}

static void mark_ctx_slots(void *addr, MonoGCMarkFunc mark_func,
                           void *gc_data) {
...
static void mono_alloc_static_data(gpointer **static_data_ptr, guint32 offset,
  ...
  if (!static_data) {
    ...
    static_data = (void **)mono_gc_alloc_fixed(
        static_data_size[0], threadlocal ? tls_desc : ctx_desc,
        threadlocal ? MONO_ROOT_SOURCE_THREAD_STATIC
  ...
  for (i = 1; i <= idx; ++i) {
    ...
    if (mono_gc_user_markers_supported())
      ...
      static_data[i] = mono_gc_alloc_fixed(
          static_data_size[i], MONO_GC_DESCRIPTOR_NULL,
          threadlocal ? MONO_ROOT_SOURCE_THREAD_STATIC
                      : MONO_ROOT_SOURCE_CONTEXT_STATIC,
          alloc_key,
          threadlocal ? "ThreadStatic Fields" : "ContextStatic Fields");
...
static guint32 mono_alloc_static_data_slot(StaticDataInfo *static_data,
  ...
  guint32 offset =
      MAKE_SPECIAL_STATIC_OFFSET(static_data->idx, static_data->offset, 0);
 ...
 */
static void context_adjust_static_data(MonoAppContextHandle ctx_handle) {
  MonoAppContext *ctx = MONO_HANDLE_RAW(ctx_handle);
  if (context_static_info.offset || context_static_info.idx > 0) {
    guint32 offset = MAKE_SPECIAL_STATIC_OFFSET(context_static_info.idx,
                                                context_static_info.offset, 0);
 ...
 * LOCKING: requires that threads_mutex is held
 */
static void alloc_thread_static_data_helper(gpointer key, gpointer value,
                                            gpointer user) {
  MonoInternalThread *thread = (MonoInternalThread *)value;
  guint32 offset = GPOINTER_TO_UINT(user);

  mono_alloc_static_data(&(thread->static_data), offset,
                         (void *)MONO_UINT_TO_NATIVE_THREAD_ID(thread->tid),
                         TRUE);
}
 ...
 * LOCKING: requires that threads_mutex is held
 */
static void alloc_context_static_data_helper(gpointer key, gpointer value,
                                             gpointer user) {
  MonoAppContext *ctx =
      (MonoAppContext *)mono_gchandle_get_target_internal((MonoGCHandle)key);
...
static StaticDataFreeList *search_slot_in_freelist(StaticDataInfo *static_data,
                                                   guint32 size,
                                                   guint32 align) {
...
static void update_reference_bitmap(MonoBitSet **sets, guint32 offset,
                                    uintptr_t *bitmap, int numbits) {
  int idx = ACCESS_SPECIAL_STATIC_OFFSET(offset, index);
  if (!sets[idx])
  ...
  for (int i = 0; i < numbits; ++i) {
    if (bitmap[i / sizeof(uintptr_t)] &
        (ONE_P << (i & (sizeof(uintptr_t) * 8 - 1))))
      mono_bitset_set_fast(rb, offset + i);
  }
}

static void clear_reference_bitmap(MonoBitSet **sets, guint32 offset,
                                   guint32 size) {
  int idx = ACCESS_SPECIAL_STATIC_OFFSET(offset, index);
  MonoBitSet *rb = sets[idx];
  offset = ACCESS_SPECIAL_STATIC_OFFSET(offset, offset);
...
guint32 mono_alloc_special_static_data(guint32 static_type, guint32 size,
                                       guint32 align, uintptr_t *bitmap,
                                       int numbits) {
  g_assert(static_type == SPECIAL_STATIC_THREAD ||
           static_type == SPECIAL_STATIC_CONTEXT);
  ...
  if (static_type == SPECIAL_STATIC_THREAD) {
    ...
    if (threads != NULL)
      mono_g_hash_table_foreach(threads, alloc_thread_static_data_helper,
                                GUINT_TO_POINTER(offset));
  } else {
    if (contexts != NULL)
      g_hash_table_foreach(contexts, alloc_context_static_data_helper,
                           GUINT_TO_POINTER(offset));

    ACCESS_SPECIAL_STATIC_OFFSET(offset, type) =
        SPECIAL_STATIC_OFFSET_TYPE_CONTEXT;
  }
...
gpointer mono_get_special_static_data_for_thread(MonoInternalThread *thread,
                                                 guint32 offset) {
...
gpointer mono_get_special_static_data(guint32 offset) {
  return mono_get_special_static_data_for_thread(mono_thread_internal_current(),
                                                 offset);
}
 ...
 * LOCKING: requires that threads_mutex is held
 */
static void free_thread_static_data_helper(gpointer key, gpointer value,
                                           gpointer user) {
  MonoInternalThread *thread = (MonoInternalThread *)value;
 ...
 * LOCKING: requires that threads_mutex is held
 */
static void free_context_static_data_helper(gpointer key, gpointer value,
                                            gpointer user) {
  MonoAppContext *ctx =
      (MonoAppContext *)mono_gchandle_get_target_internal((MonoGCHandle)key);
...
static void do_free_special_slot(guint32 offset, guint32 size) {
  guint32 static_type = ACCESS_SPECIAL_STATIC_OFFSET(offset, type);
  MonoBitSet **sets;
  StaticDataInfo *info;
...
static void do_free_special(gpointer key, gpointer value, gpointer data) {
...
#ifdef HOST_WIN32
static void flush_thread_interrupt_queue(void) {
  /* Consume pending APC calls for current thread.*/
  /* Since this function get's called from interrupt handler it must use a
   * direct */
  /* Win32 API call and can't go through
   * mono_coop_win32_wait_for_single_object_ex */
...
static gboolean mono_thread_execute_interruption(MonoExceptionHandle *pexc) {
  ...
  /* If there's a pending exception and an AbortRequested, the pending exception
   * takes precedence */
...
static MonoException *mono_thread_execute_interruption_ptr(void) {
  HANDLE_FUNCTION_ENTER();
  MonoExceptionHandle exc = MONO_HANDLE_NEW(MonoException, NULL);
  MonoException *const exc_raw =
      mono_thread_execute_interruption(&exc) ? MONO_HANDLE_RAW(exc) : NULL;
...
mono_thread_request_interruption_internal(gboolean running_managed,
  ...
  if (!running_managed || is_running_protected_wrapper()) {
    ...
    /* this will awake the thread if it is in WaitForSingleObject
       or similar */
#ifdef HOST_WIN32
    mono_win32_interrupt_wait(thread->thread_info, thread->native_handle,
                              (DWORD)thread->tid);
#else
...
static void mono_thread_request_interruption_native(void) {
...
mono_thread_interruption_checkpoint_request(gboolean bypass_abort_protection) {
  ...
  if (!mono_thread_get_interruption_requested(thread))
    return NULL;
  if (!bypass_abort_protection && !mono_thread_current()->pending_exception &&
...
gboolean mono_thread_interruption_checkpoint_bool(void) {
  return mono_thread_interruption_checkpoint() != NULL;
}

void mono_thread_interruption_checkpoint_void(void) {
 ...
 */
void mono_set_pending_exception(MonoException *exc) {
 ...
 *   The exception will be thrown when execution returns to managed code.
 *   Can optionally \p overwrite any existing pending exceptions (it's not
 * supported to overwrite any pending exceptions if the runtime is processing a
 * thread abort request, in which case the behavior will be undefined). Return
 * whether the pending exception was set or not. It will not be set if:
 *   * The thread or runtime is stopping or shutting down
 *   * There already is a pending exception (and \p overwrite is false)
 */
mono_bool mono_runtime_set_pending_exception(MonoException *exc,
                                             mono_bool overwrite) {
  MonoThread *thread = mono_thread_current();
...
static void mono_thread_notify_change_state(MonoThreadState old_state,
...
void mono_thread_clear_and_set_state(MonoInternalThread *thread,
 ...
 * Test if current state of \p thread include \p test. If it does not, OR \p set
 * into the state. \returns TRUE if \p set was OR'd in.
 */
gboolean mono_thread_test_and_set_state(MonoInternalThread *thread,
...
gboolean mono_thread_test_state(MonoInternalThread *thread,
                                MonoThreadState test) {
  LOCK_THREAD(thread);
...
static void self_interrupt_thread(void *_unused) {
  MonoException *exc;
  MonoThreadInfo *info;
  MonoContext ctx;
  ...
  /* FIXME using thread_saved_state [ASYNC_SUSPEND_STATE_INDEX] can race with
   * another suspend coming in. */
  ctx = info->thread_saved_state[ASYNC_SUSPEND_STATE_INDEX].ctx;

  mono_raise_exception_with_context(exc, &ctx);
}

static gboolean mono_jit_info_match(MonoJitInfo *ji, gpointer ip) {
  if (!ji)
    return FALSE;
  return ji->code_start <= ip &&
         (char *)ip < (char *)ji->code_start + ji->code_size;
}

static gboolean last_managed(MonoStackFrameInfo *frame, MonoContext *ctx,
                             gpointer data) {
...
static MonoJitInfo *mono_thread_info_get_last_managed(MonoThreadInfo *info) {
  MonoJitInfo *ji = NULL;
  if (!info)
    return NULL;

  /*
   * The suspended thread might be holding runtime locks. Make sure we don't try
   * taking any runtime locks while unwinding.
   */
  mono_thread_info_set_is_async_context(TRUE);
  mono_get_eh_callbacks()->mono_walk_stack_with_state(
      last_managed, mono_thread_info_get_suspend_state(info),
      MONO_UNWIND_SIGNAL_SAFE, &ji);
...
static SuspendThreadResult async_abort_critical(MonoThreadInfo *info,
                                                gpointer ud) {
  ...
  if (mono_get_eh_callbacks()->mono_install_handler_block_guard(
  ...
  protected_wrapper =
      ji && !ji->is_trampoline && !ji->async &&
      mono_threads_is_critical_method(mono_jit_info_get_method(ji));
  running_managed = mono_jit_info_match(
      ji, MONO_CONTEXT_GET_IP(&mono_thread_info_get_suspend_state(info)->ctx));
  ...
  if (!protected_wrapper && running_managed) {
     ...
     * It will also prevent the thread from entering a wait, so if the thread
     * returns from the wait before it receives the abort signal, it will just
     * spin in the wait functions in the io-layer until the signal handler calls
     * QueueUserAPC which will make it return.
     */
...
static void async_abort_internal(MonoInternalThread *thread,
  ...
  mono_thread_info_safe_suspend_and_run(thread_get_tid(thread), TRUE,
                                        async_abort_critical, &data);
  if (data.interrupt_token)
    mono_thread_info_finish_interrupt(data.interrupt_token);
  /*FIXME we need to wait for interruption to complete -- figure out how much
   * into interruption we should wait for here*/
}
...
static void self_abort_internal(MonoError *error) {
   ...
   * synchronously
   * since passing FALSE to mono_thread_request_interruption makes sure it
   * returns NULL */

  /*
  Self aborts ignore the protected block logic and raise the TAE regardless.
  This is verified by one of the tests in mono/tests/abort-cctor.cs.
  */
  MonoExceptionHandle exc = MONO_HANDLE_NEW(MonoException, NULL);
...
static SuspendThreadResult async_suspend_critical(MonoThreadInfo *info,
                                                  gpointer ud) {
  ...
  if (running_managed && !protected_wrapper) {
    ...
    if (data->interrupt)
      data->interrupt_token = mono_thread_info_prepare_interrupt(
          (MonoThreadInfo *)thread->thread_info);
...
static void async_suspend_internal(MonoInternalThread *thread,
  ...
  mono_thread_info_safe_suspend_and_run(thread_get_tid(thread), interrupt,
                                        async_suspend_critical, &data);
  if (data.interrupt_token)
    mono_thread_info_finish_interrupt(data.interrupt_token);
...
static void self_suspend_internal(void) {
  ...
  g_assert(res == MONO_OS_EVENT_WAIT_RET_SUCCESS_0 ||
           res == MONO_OS_EVENT_WAIT_RET_ALERTED);
  MONO_EXIT_GC_SAFE;
}

static void suspend_for_shutdown_async_call(gpointer unused) {
  for (;;)
    mono_thread_info_yield();
}

static SuspendThreadResult suspend_for_shutdown_critical(MonoThreadInfo *info,
                                                         gpointer unused) {
  mono_thread_info_setup_async_call(info, suspend_for_shutdown_async_call,
                                    NULL);
  return MonoResumeThread;
}
...
void mono_thread_internal_suspend_for_shutdown(MonoInternalThread *thread) {
  ...
  mono_thread_info_safe_suspend_and_run(thread_get_tid(thread), FALSE,
                                        suspend_for_shutdown_critical, NULL);
}
 ...
 *
 * This function allows one to determine if a thread was created by the mono
 * runtime and has a well defined lifecycle or it's a foreign one, created by
...
#ifndef HOST_WIN32
static void threads_native_thread_join_lock(gpointer tid, gpointer value) {
  /*
...
static void threads_native_thread_join_nolock(gpointer tid, gpointer value) {
  pthread_t thread = (pthread_t)(intptr_t)tid;
  MONO_ENTER_GC_SAFE;
  mono_native_thread_join(thread);
  MONO_EXIT_GC_SAFE;
...
static void threads_add_joinable_thread_nolock(gpointer tid) {
...
static void threads_native_thread_join_lock(gpointer tid, gpointer value) {
  ...
  if (thread_id != GetCurrentThreadId() && thread_handle != NULL &&
...
static void threads_add_joinable_thread_nolock(gpointer tid) {
  g_hash_table_insert(joinable_threads, tid,
                      (gpointer)OpenThread(SYNCHRONIZE, TRUE,
                                           (MonoNativeThreadId)(guint64)tid));
}
#endif

static void threads_add_pending_joinable_thread(gpointer tid) {
  ...
  if (!g_hash_table_lookup_extended(pending_joinable_threads, tid, &orig_key,
                                    &value)) {
...
threads_add_pending_joinable_runtime_thread(MonoThreadInfo *mono_thread_info) {
  ...
  if (mono_thread_info->runtime_thread) {
    threads_add_pending_joinable_thread(
        (gpointer)(MONO_UINT_TO_NATIVE_THREAD_ID(
            mono_thread_info_get_tid(mono_thread_info))));
  }
}

static void threads_remove_pending_joinable_thread_nolock(gpointer tid) {
  gpointer orig_key;
  gpointer value;

  if (pending_joinable_threads &&
      g_hash_table_lookup_extended(pending_joinable_threads, tid, &orig_key,
                                   &value)) {
  ...
  if (UnlockedRead(&pending_joinable_thread_count) > 0) {
    ...
    if (timeout == MONO_INFINITE_WAIT) {
      while (UnlockedRead(&pending_joinable_thread_count) > 0)
        mono_coop_cond_wait(&zero_pending_joinable_thread_event,
      ...
      while (UnlockedRead(&pending_joinable_thread_count) > 0 &&
             elapsed < timeout) {
        mono_coop_cond_timedwait(&zero_pending_joinable_thread_event,
                                 &joinable_threads_mutex,
                                 timeout - (uint32_t)elapsed);
...
static void threads_add_unique_joinable_thread_nolock(gpointer tid) {
  if (!joinable_threads)
...
void mono_threads_add_joinable_runtime_thread(MonoThreadInfo *thread_info) {
  ...
  if (mono_thread_info->runtime_thread) {
    gpointer tid = (gpointer)(MONO_UINT_TO_NATIVE_THREAD_ID(
        mono_thread_info_get_tid(mono_thread_info)));
...
static void threads_add_pending_native_thread_join_call_nolock(gpointer tid) {
  ...
  if (!g_hash_table_lookup_extended(pending_native_thread_join_calls, tid,
                                    &orig_key, &value))
...
static void threads_wait_pending_native_thread_join_call_nolock(gpointer tid) {
  gpointer orig_key;
  gpointer value;

  while (g_hash_table_lookup_extended(pending_native_thread_join_calls, tid,
                                      &orig_key, &value)) {
    mono_coop_cond_wait(&pending_native_thread_join_calls_event,
                        &joinable_threads_mutex);
  }
}
...
void mono_thread_join(gpointer tid) {
  ...
  if (!found) {
    // Wait for any pending native thread join call not yet completed for this
    // tid.
 ...
 */
MonoDomain *mono_threads_attach_coop_internal(MonoDomain *domain,
                                              gpointer *cookie,
  ...
  if (mono_threads_is_blocking_transition_enabled())
    external = !(info = mono_thread_info_current_unchecked()) ||
  ...
  if (mono_threads_is_blocking_transition_enabled()) {
    if (external) {
      /* mono_thread_attach put the thread in RUNNING mode from STARTING, but we
       * need to return the right cookie. */
 ...
 *  - @return: the original domain which needs to be restored, or NULL.
 */
gpointer mono_threads_attach_coop(MonoDomain *domain, gpointer *dummy) {
 ...
 *    - non-blocking mode: contains random data
 */
void mono_threads_detach_coop_internal(MonoDomain *orig, gpointer cookie,
                                       MonoStackData *stackdata) {
  MonoDomain *domain = mono_domain_get();
  g_assert(domain);
 ...
 *    - a pointer to stack, used for some checks
 */
void mono_threads_detach_coop(gpointer orig, gpointer *dummy) {
  MONO_STACKDATA(stackdata);
  stackdata.stackpointer = dummy;
...
void mono_thread_internal_describe(MonoInternalThread *internal,
                                   GString *text) {
  ...
  if (internal->owned_mutexes) {
    ...
    for (i = 0; i < internal->owned_mutexes->len; i++)
      g_string_append_printf(text, i == 0 ? "%p" : ", %p",
                             g_ptr_array_index(internal->owned_mutexes, i));
    g_string_append(text, "]");
  }
}

gboolean mono_thread_internal_is_current(MonoInternalThread *internal) {
  g_assert(internal);
  return mono_native_thread_id_equals(
      mono_native_thread_id_get(),
      MONO_UINT_TO_NATIVE_THREAD_ID(internal->tid));
}

void mono_set_thread_dump_dir(gchar *dir) { thread_dump_dir = dir; }

#ifdef DISABLE_CRASH_REPORTING
void mono_threads_summarize_init(void) {}

gboolean mono_threads_summarize(MonoContext *ctx, gchar **out,
                                MonoStackHash *hashes, gboolean silent,
                                gboolean signal_handler_controller, gchar *mem,
                                size_t provided_size) {
  return FALSE;
}

gboolean mono_threads_summarize_one(MonoThreadSummary *out, MonoContext *ctx) {
  return FALSE;
}

#else

static gboolean mono_threads_summarize_native_self(MonoThreadSummary *out,
                                                   MonoContext *ctx) {
...
typedef struct {
  ...
  int nthreads;
  MonoNativeThreadId
      thread_array[MAX_NUM_THREADS]; // ids of threads we're dumping
...
#if defined(HAVE_KILL) && !defined(HOST_ANDROID) && defined(HAVE_WAITPID) &&   \
    defined(HAVE_EXECVE) &&                                                    \
    ((!defined(HOST_DARWIN) && defined(SYS_fork)) || HAVE_FORK)
#define HAVE_MONO_SUMMARIZER_SUPERVISOR 1
#endif

static void summarizer_supervisor_init(void);
...
void summarizer_supervisor_init(void) { return; }

static pid_t summarizer_supervisor_start(SummarizerSupervisorState *state) {
  // nonzero, so caller doesn't think it's the supervisor
  return (pid_t)1;
}

static void summarizer_supervisor_end(SummarizerSupervisorState *state) {
  return;
}
...
void summarizer_supervisor_init(void) {
  hang_watchdog_path = g_build_filename(mono_get_config_dir(), "..", "bin",
                                        "mono-hang-watchdog", NULL);
...
static pid_t summarizer_supervisor_start(SummarizerSupervisorState *state) {
  ...
  /*
   * glibc fork acquires some locks, so if the crash happened inside
   * malloc/free, it will deadlock. Call the syscall directly instead.
   */
  ...
  if (pid != 0)
    ...
    execve(args[0], (char *const *)args,
           NULL); // run 'mono-hang-watchdog [pid]'
    g_async_safe_printf(
        "Could not exec mono-hang-watchdog, expected on path '%s' (errno %d)\n",
...
static void collect_thread_id(gpointer key, gpointer value, gpointer user) {
...
static int collect_thread_ids(MonoNativeThreadId *thread_ids, int max_threads) {
...
static gboolean summarizer_state_init(SummarizerGlobalState *state,
                                      MonoNativeThreadId current,
                                      int *my_index) {
  gint32 started_state =
      mono_atomic_cas_i32(&state->has_owner, 1 /* set */, 0 /* compare */);
...
static void summarizer_signal_other_threads(SummarizerGlobalState *state,
                                            MonoNativeThreadId current,
                                            int current_idx) {
  ...
  for (int i = 0; i < state->nthreads; i++) {
    ...
    if (!state->silent)
      g_async_safe_printf(
          "Pkilling 0x%" G_GSIZE_FORMAT "x from 0x%" G_GSIZE_FORMAT "x\n",
          (gsize)MONO_NATIVE_THREAD_ID_TO_UINT(state->thread_array[i]),
...
static gboolean summarizer_post_dump(SummarizerGlobalState *state,
                                     MonoThreadSummary *this_thread,
                                     int current_idx) {
  mono_memory_barrier();

  gpointer old = mono_atomic_cas_ptr(
      (volatile gpointer *)&state->all_threads[current_idx], this_thread, NULL);

  if (old == GINT_TO_POINTER(-1)) {
    g_async_safe_printf(
        "Trying to register response after dumping period ended");
    return FALSE;
  } else if (old != NULL) {
  ...
  gint32 count =
      mono_atomic_inc_i32((volatile gint32 *)&state->nthreads_attached);
...
static void summary_timedwait(SummarizerGlobalState *state,
                              int timeout_seconds) {
  ...
  while (TRUE) {
    if (mono_atomic_load_i32((volatile gint32 *)&state->nthreads_attached) ==
        state->nthreads)
      break;
...
summarizer_try_read_thread(SummarizerGlobalState *state, int index) {
  ...
  } while (mono_atomic_cas_ptr((volatile gpointer *)&state->all_threads[index],
...
static void summarizer_state_term(SummarizerGlobalState *state, gchar **out,
                                  gchar *mem, size_t provided_size,
                                  MonoThreadSummary *controlling) {
  ...
  for (int i = 0; i < state->nthreads; i++) {
    ...
    // an async context sometimes. There's still some reliance on malloc here,
    // but it's much more stable to do it all from the controlling thread.
    //
  ...
  /* The value of the breadcrumb should match the "StackHash" value written by
   * `mono_merp_write_fingerprint_payload` */
  ...
  for (int i = 0; i < state->nthreads; i++) {
    ...
    mono_summarize_native_state_add_thread(&writer, thread, thread->ctx,
                                           thread == controlling);
...
static void summarizer_state_wait(MonoThreadSummary *thread) {
  ...
  while (!mono_atomic_load_i32(&thread->done))
    mono_os_sem_timedwait(&thread->done_wait, milliseconds_in_second,
                          MONO_SEM_FLAGS_NONE);
}

static gboolean mono_threads_summarize_execute_internal(
    MonoContext *ctx, gchar **out, MonoStackHash *hashes, gboolean silent,
    gchar *working_mem, size_t provided_size, gboolean this_thread_controls) {
  static SummarizerGlobalState state;

  int current_idx;
  MonoNativeThreadId current = mono_native_thread_id_get();
  gboolean thread_given_control =
      summarizer_state_init(&state, current, &current_idx);
  ...
  MonoStateMem mem;
  gboolean success =
      mono_state_alloc_mem(&mem, (long)current, sizeof(MonoThreadSummary));
  if (!success)
    return FALSE;
  ...
  if (mono_threads_summarize_native_self(this_thread, ctx)) {
    ...
    g_async_safe_printf("Thread 0x%" G_GSIZE_FORMAT
  ...
  if (this_thread_controls) {
    if (!state.silent)
      g_async_safe_printf(
          "Entering thread summarizer pause from 0x%" G_GSIZE_FORMAT "x\n",
          (gsize)MONO_NATIVE_THREAD_ID_TO_UINT(current));
    ...
    if (!state.silent)
      g_async_safe_printf(
          "Finished thread summarizer pause from 0x%" G_GSIZE_FORMAT "x.\n",
          (gsize)MONO_NATIVE_THREAD_ID_TO_UINT(current));
...
void mono_threads_summarize_init(void) { summarizer_supervisor_init(); }

gboolean mono_threads_summarize_execute(MonoContext *ctx, gchar **out,
                                        MonoStackHash *hashes, gboolean silent,
                                        gchar *working_mem,
                                        size_t provided_size) {
  gboolean result;
  ...
  if (!already_async)
    mono_thread_info_set_is_async_context(TRUE);
  result = mono_threads_summarize_execute_internal(
      ctx, out, hashes, silent, working_mem, provided_size, FALSE);
  if (!already_async)
    mono_thread_info_set_is_async_context(FALSE);
  return result;
}
...
gboolean mono_threads_summarize(MonoContext *ctx, gchar **out,
  ...
  gint64 this_request_id =
      mono_atomic_inc_i64((volatile gint64 *)&next_pending_request_id);
  ...
  // wait for an in-flight dump to finish. If we crashed while dumping, we
  // cannot dump. We should simply return so we can die cleanly.
  //
  // signal_handler_controller should be set only from a handler that expects
  // itself to be the only entry point, where the runtime already being dumping
  // means we should just give up

  gboolean success = FALSE;

  while (TRUE) {
    gint64 next_request_id =
        mono_atomic_load_i64((volatile gint64 *)&request_available_to_run);
    ...
    if (next_request_id == this_request_id) {
      ...
      if (summarizer_supervisor_start(&synch)) {
        g_assert(mem);
        success = mono_threads_summarize_execute_internal(
            ctx, out, hashes, silent, mem, provided_size, TRUE);
      ...
      g_async_safe_printf("Attempted to dump for critical failure when already "
                          "in dump. Error reporting crashed?");
...
#ifdef ENABLE_NETCORE
void ves_icall_System_Threading_Thread_StartInternal(
    MonoThreadObjectHandle thread_handle, MonoError *error) {
  MonoThread *internal = MONO_HANDLE_RAW(thread_handle);
  gboolean res;

  THREAD_DEBUG(g_message("%s: Trying to start a new thread: this (%p)",
                         __func__, internal));
  ...
  res = create_thread(internal, internal, NULL, NULL, NULL,
                      MONO_THREAD_CREATE_FLAGS_NONE, error);
  if (!res) {
    UNLOCK_THREAD(internal);
    return;
  ...
  THREAD_DEBUG(g_message("%s: Started thread ID %" G_GSIZE_FORMAT
                         " (handle %p)",
                         __func__, (gsize)internal->tid, internal->handle));

  UNLOCK_THREAD(internal);
}

void ves_icall_System_Threading_Thread_InitInternal(
    MonoThreadObjectHandle thread_handle, MonoError *error) {
  MonoThread *internal = MONO_HANDLE_RAW(thread_handle);
...
gpointer ves_icall_System_Threading_LowLevelLifoSemaphore_InitInternal(void) {
  return (gpointer)mono_lifo_semaphore_init();
}

void ves_icall_System_Threading_LowLevelLifoSemaphore_DeleteInternal(
    gpointer sem_ptr) {
  LifoSemaphore *sem = (LifoSemaphore *)sem_ptr;
  mono_lifo_semaphore_delete(sem);
}

gint32 ves_icall_System_Threading_LowLevelLifoSemaphore_TimedWaitInternal(
    gpointer sem_ptr, gint32 timeout_ms) {
  LifoSemaphore *sem = (LifoSemaphore *)sem_ptr;
  return mono_lifo_semaphore_timed_wait(sem, timeout_ms);
}

void ves_icall_System_Threading_LowLevelLifoSemaphore_ReleaseInternal(
    gpointer sem_ptr, gint32 count) {
  LifoSemaphore *sem = (LifoSemaphore *)sem_ptr;
