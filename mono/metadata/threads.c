...
static gboolean mono_thread_attach_internal(MonoThread *thread,
  ...
  if (thread_static_info.offset || thread_static_info.idx > 0) {
    ...
    mono_alloc_static_data(&internal->static_data, offset,
                           (void *)MONO_UINT_TO_NATIVE_THREAD_ID(internal->tid),
                           TRUE);
  }
...
static gboolean create_thread(MonoThread *thread, MonoInternalThread *internal,
  ...
  if (shutting_down && !(flags & MONO_THREAD_CREATE_FLAGS_FORCE_CREATE)) {
    ...
    mono_error_set_execution_engine(
        error, "Couldn't create thread. Runtime is shutting down.");
    return FALSE;
  }
...
mono_thread_construct_internal(MonoThreadObjectHandle this_obj_handle) {
  ...
  int const thread_gchandle = mono_gchandle_from_handle(
 ...
 */
static int collect_threads(guint32 *thread_handles, int max_threads) {
...
static void alloc_context_static_data_helper(gpointer key, gpointer value,
                                             gpointer user) {
  MonoAppContext *ctx =
      (MonoAppContext *)mono_gchandle_get_target_internal(GPOINTER_TO_INT(key));
