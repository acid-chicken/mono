...
static guint32 WINAPI start_wrapper_internal(StartInfo *start_info,
  ...
  if (start_func) {
    ...
    if (!cb) {
      cb = mono_class_get_method_from_name_checked(
          internal->obj.vtable->klass, "StartCallback", 0, 0, error);
      g_assert(cb);
      mono_error_assert_ok(error);
    }
...
//
gsize mono_thread_set_name(MonoInternalThread *this_obj, const char *name8,
