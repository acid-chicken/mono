...
/*
 * If SHM is disabled, this will point to a hash of FileShare structures,
 * otherwise it will be NULL. We use this instead of _wapi_fileshare_layout to
 * avoid allocating a 4MB array.
 */
...
static void time_t_to_filetime(time_t timeval, FILETIME *filetime) {
  guint64 ticks;
...
static FileHandle *file_data_create(MonoFDType type, gint fd) {
...
static gint _wapi_unlink(const gchar *pathname);

static void file_share_release(FileShare *share_info);

static void file_data_close(MonoFDHandle *fdhandle) {
  FileHandle *filehandle;

  filehandle = (FileHandle *)fdhandle;
  g_assert(filehandle);

  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: closing fd %d",
             __func__, ((MonoFDHandle *)filehandle)->fd);

  if (((MonoFDHandle *)filehandle)->type == MONO_FDTYPE_FILE &&
      (filehandle->attrs & FILE_FLAG_DELETE_ON_CLOSE)) {
    _wapi_unlink(filehandle->filename);
  }

  if (((MonoFDHandle *)filehandle)->type != MONO_FDTYPE_CONSOLE ||
      ((MonoFDHandle *)filehandle)->fd > 2) {
...
static void file_data_destroy(MonoFDHandle *fdhandle) {
  FileHandle *filehandle;
...
static gint file_share_equal(gconstpointer ka, gconstpointer kb) {
...
static guint file_share_hash(gconstpointer data) {
  const FileShare *s = (const FileShare *)data;

  return s->inode;
}

static gboolean file_share_get(guint64 device, guint64 inode,
                               guint32 new_sharemode, guint32 new_access,
                               guint32 *old_sharemode, guint32 *old_access,
  ...
  /*
   * Instead of allocating a 4MB array, we use a hash table to keep track of
   * this info. This is needed even if SHM is disabled, to track sharing inside
   * the current process.
   */
  if (!file_share_table)
    file_share_table =
        g_hash_table_new_full(file_share_hash, file_share_equal, NULL, g_free);
...
static gint _wapi_open(const gchar *pathname, gint flags, mode_t mode) {
  gint fd;
...
static gint _wapi_access(const gchar *pathname, gint mode) {
  gint ret;
...
static gint _wapi_chmod(const gchar *pathname, mode_t mode) {
  gint ret;
...
static gint _wapi_utime(const gchar *filename, const struct utimbuf *buf) {
  gint ret = -1;
...
#else
static gint _wapi_utimes(const gchar *filename, const struct timeval times[2]) {
  gint ret = -1;
...
static gint _wapi_unlink(const gchar *pathname) {
  ...
  ret = unlink(pathname);
  MONO_EXIT_GC_SAFE;
  if (ret == -1 && (errno == ENOENT || errno == ENOTDIR || errno == EISDIR) &&
      IS_PORTABILITY_SET) {
    gint saved_errno = errno;
    gchar *located_filename = mono_portability_find_file(pathname, TRUE);
...
static gint _wapi_rename(const gchar *oldpath, const gchar *newpath) {
  gint ret;
  ...
  if (located_newpath == NULL) {
    ...
    if (ret == -1 &&
        (errno == EISDIR || errno == ENAMETOOLONG || errno == ENOENT ||
         errno == ENOTDIR || errno == EXDEV) &&
        IS_PORTABILITY_SET) {
      gint saved_errno = errno;
...
static gint _wapi_stat(const gchar *path, struct stat *buf) {
  gint ret;
...
static gint _wapi_lstat(const gchar *path, struct stat *buf) {
  gint ret;
...
static gint _wapi_mkdir(const gchar *pathname, mode_t mode) {
  gint ret;
...
static gint _wapi_rmdir(const gchar *pathname) {
  gint ret;
...
static gint _wapi_chdir(const gchar *path) {
  gint ret;
...
static gchar *_wapi_basename(const gchar *filename) {
  gchar *new_filename = g_strdup(filename), *ret;

  if (IS_PORTABILITY_SET)
    g_strdelimit(new_filename, '\\', '/');

  if (IS_PORTABILITY_DRIVE && g_ascii_isalpha(new_filename[0]) &&
      (new_filename[1] == ':')) {
...
static gchar *_wapi_dirname(const gchar *filename) {
  gchar *new_filename = g_strdup(filename), *ret;
...
static GDir *_wapi_g_dir_open(const gchar *path, guint flags, GError **gerror) {
  GDir *ret;
  ...
  if (ret == NULL &&
      ((*gerror)->code == G_FILE_ERROR_NOENT ||
       (*gerror)->code == G_FILE_ERROR_NOTDIR ||
       (*gerror)->code == G_FILE_ERROR_NAMETOOLONG) &&
      IS_PORTABILITY_SET) {
    gchar *located_filename = mono_portability_find_file(path, TRUE);
...
static gint get_errno_from_g_file_error(gint error) {
  switch (error) {
#ifdef EACCES
...
static gint file_compare(gconstpointer a, gconstpointer b) {
  gchar *astr = *(gchar **)a;
  gchar *bstr = *(gchar **)b;
...
/* scandir using glib */
static gint _wapi_io_scandir(const gchar *dirname, const gchar *pattern,
                             gchar ***namelist) {
  GError *gerror = NULL;
  GDir *dir;
  GPtrArray *names;
  ...
  if (dir == NULL) {
    ...
    g_error_free(gerror);
    if (errnum == ENOENT && !_wapi_access(dirname, F_OK) &&
  ...
  if (g_str_has_suffix(pattern, ".*")) {
    ...
    g_dir_rewind(dir);
    MONO_EXIT_GC_SAFE;
    result2 = mono_w32file_unix_glob(
        dir, pattern2,
        flags | W32FILE_UNIX_GLOB_APPEND | W32FILE_UNIX_GLOB_UNIQUE, &glob_buf);
...
static gboolean _wapi_lock_file_region(gint fd, off_t offset, off_t length) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
...
static gboolean _wapi_unlock_file_region(gint fd, off_t offset, off_t length) {
  struct flock lock_data;
  gint ret;
 ...
 *
 * We generally assume that basic unix permission bits are authoritative. Which
 * might not be the case under systems with extended permissions systems (posix
 * ACLs, SELinux, OSX/iOS sandboxing, etc)
 *
 * The choice of access as the fallback is due to the expected lower overhead
 * compared to trying to open the file.
 *
 * The only expected problem with using access are for root, setuid or setgid
 * programs as access is not consistent under those situations. It's to be
 * expected that this should not happen in practice as those bits are very
 * dangerous and should not be used with a dynamic runtime.
 */
static gboolean is_file_writable(struct stat *st, const gchar *path) {
  gboolean ret;
  gchar *located_path;

#if __APPLE__
  // OS X Finder "locked" or `ls -lO` "uchg".
  // This only covers one of several cases where an OS X file could be
  // unwritable through special flags.
  ...
  /* Fallback to using access(2). It's not ideal as it might not take into
...
static gboolean file_read(FileHandle *filehandle, gpointer buffer,
  ...
  if (!(filehandle->fileaccess & (GENERIC_READ | GENERIC_ALL))) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               ...
               ((MonoFDHandle *)filehandle)->fd, filehandle->fileaccess);
  ...
  if (ret == -1) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               ...
               ((MonoFDHandle *)filehandle)->fd, g_strerror(err));
...
static gboolean file_write(FileHandle *filehandle, gpointer buffer,
  ...
  if (!(filehandle->fileaccess & GENERIC_WRITE) &&
      !(filehandle->fileaccess & GENERIC_ALL)) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
  ...
  if (lock_while_writing) {
    ...
    if (current_pos == -1) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
                 ...
                 ((MonoFDHandle *)filehandle)->fd, g_strerror(errno));
      _wapi_set_last_error_from_errno();
      return (FALSE);
    }

    if (_wapi_lock_file_region(((MonoFDHandle *)filehandle)->fd, current_pos,
                               numbytes) == FALSE) {
  ...
  if (lock_while_writing) {
    _wapi_unlock_file_region(((MonoFDHandle *)filehandle)->fd, current_pos,
                             numbytes);
  }
...
static guint32 file_seek(FileHandle *filehandle, gint32 movedistance,
  ...
  if (!(filehandle->fileaccess &
        (GENERIC_READ | GENERIC_WRITE | GENERIC_ALL))) {
    mono_trace(
        G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
        "%s: fd %d doesn't have GENERIC_READ or GENERIC_WRITE access: %u",
        __func__, ((MonoFDHandle *)filehandle)->fd, filehandle->fileaccess);
  ...
  default:
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: invalid seek type %d", __func__, method);
  ...
  if (highmovedistance == NULL) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               ...
               " (low %" G_GINT32_FORMAT ")",
               __func__, offset, movedistance);
  } else {
    offset = ((gint64)*highmovedistance << 32) | (guint32)movedistance;

    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: setting offset to %" G_GINT64_FORMAT " 0x%" PRIx64
               " (high %" G_GINT32_FORMAT " 0x%" PRIx32
               ", low %" G_GINT32_FORMAT " 0x%" PRIx32 ")",
               __func__, offset, offset, *highmovedistance, *highmovedistance,
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: moving fd %d by %" G_GINT64_FORMAT " bytes from %d", __func__,
             ((MonoFDHandle *)filehandle)->fd, offset, whence);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: lseek returns %" G_GINT64_FORMAT, __func__, newpos);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             ...
             "/%" G_GINT32_FORMAT,
             __func__, ((MonoFDHandle *)filehandle)->fd, ret,
...
static guint32 file_getfilesize(FileHandle *filehandle, guint32 *highsize) {
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: Returning size %" G_GUINT32_FORMAT "/%" G_GUINT32_FORMAT,
             __func__, size, *highsize);

  return (size);
}

static guint64 convert_unix_filetime_ms(const FILETIME *file_time,
                                        const char *ttype) {
  guint64 t =
      ((guint64)file_time->dwHighDateTime << 32) + file_time->dwLowDateTime;
  ...
  if (t < CONVERT_BASE) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: attempt to set %s time too early", __func__, ttype);
    mono_w32error_set_last(ERROR_INVALID_PARAMETER);
    return (FALSE);
  }
...
#ifndef HAVE_STRUCT_TIMEVAL
static guint64 convert_unix_filetime(const FILETIME *file_time,
                                     size_t field_size, const char *ttype) {
  guint64 t = convert_unix_filetime_ms(file_time, ttype);

  if (field_size == 4 && (t / 10000000) > INT_MAX) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: attempt to set %s time that is too big for a 32bits time_t",
               __func__, ttype);
    mono_w32error_set_last(ERROR_INVALID_PARAMETER);
    return (FALSE);
  }
...
static void convert_stattime_access_to_timeval(struct timeval *dest,
                                               struct stat *statbuf) {
...
static void convert_stattime_mod_to_timeval(struct timeval *dest,
                                            struct stat *statbuf) {
#if HAVE_STRUCT_STAT_ST_ATIMESPEC
...
static gboolean file_setfiletime(FileHandle *filehandle,
  ...
  utbuf.actime =
      access_time
          ? convert_unix_filetime(access_time, sizeof(utbuf.actime), "access")
          : statbuf.st_atime;
  utbuf.modtime =
      write_time
          ? convert_unix_filetime(write_time, sizeof(utbuf.modtime), "write")
          : statbuf.st_mtime;
  ...
  if (ret == -1) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               ...
               ((MonoFDHandle *)filehandle)->fd, filehandle->filename,
               g_strerror(errno));
...
static gboolean console_read(FileHandle *filehandle, gpointer buffer,
                             guint32 numbytes, guint32 *bytesread) {
  gint ret;
  MonoThreadInfo *info = mono_thread_info_current();
...
static gboolean console_write(FileHandle *filehandle, gpointer buffer,
                              guint32 numbytes, guint32 *byteswritten) {
  gint ret;
  MonoThreadInfo *info = mono_thread_info_current();
...
static gboolean pipe_read(FileHandle *filehandle, gpointer buffer,
                          guint32 numbytes, guint32 *bytesread) {
  gint ret;
  MonoThreadInfo *info = mono_thread_info_current();
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: reading up to %" G_GUINT32_FORMAT " bytes from pipe %d",
             __func__, numbytes, ((MonoFDHandle *)filehandle)->fd);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: read %d bytes from pipe %d", __func__, ret,
             ((MonoFDHandle *)filehandle)->fd);
...
static gboolean pipe_write(FileHandle *filehandle, gpointer buffer,
                           guint32 numbytes, guint32 *byteswritten) {
  gint ret;
  MonoThreadInfo *info = mono_thread_info_current();
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: writing up to %" G_GUINT32_FORMAT " bytes to pipe %d",
             __func__, numbytes, ((MonoFDHandle *)filehandle)->fd);
...
static gint convert_flags(guint32 fileaccess, guint32 createmode) {
  ...
  default:
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: Unknown access type 0x%" PRIx32, __func__, fileaccess);
    break;
  }
  ...
  default:
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: Unknown create mode 0x%" PRIx32, __func__, createmode);
    break;
  }
...
static gboolean share_allows_open(struct stat *statbuf, guint32 sharemode,
                                  guint32 fileaccess, FileShare **share_info) {
  gboolean file_already_shared;
  guint32 file_existing_share, file_existing_access;

  file_already_shared =
      file_share_get(statbuf->st_dev, statbuf->st_ino, sharemode, fileaccess,
                     &file_existing_share, &file_existing_access, share_info);
  ...
  if (file_already_shared) {
    ...
    if (((file_existing_share == FILE_SHARE_READ) &&
      ...
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
                 ...
                 __func__, fileaccess, file_existing_share);
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: New file!",
               __func__);
  }
...
static gboolean share_allows_delete(struct stat *statbuf,
                                    FileShare **share_info) {
  gboolean file_already_shared;
  guint32 file_existing_share, file_existing_access;

  file_already_shared = file_share_get(
      statbuf->st_dev, statbuf->st_ino, FILE_SHARE_DELETE, GENERIC_READ,
      &file_existing_share, &file_existing_access, share_info);
  ...
  if (file_already_shared) {
    ...
    if (!(file_existing_share & FILE_SHARE_DELETE)) {
      ...
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
                 "%s: Share mode prevents open: requested access: 0x%" PRIx32
                 ", file has sharing: 0x%" PRIx32,
                 __func__, (*share_info)->access, file_existing_share);
...
gpointer mono_w32file_create(const gunichar2 *name, guint32 fileaccess,
                             guint32 sharemode, guint32 createmode,
                             guint32 attrs) {
  FileHandle *filehandle;
  MonoFDType type;
  ...
  if (name == NULL) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: name is NULL",
               __func__);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: Opening %s with share 0x%" PRIx32 " and access 0x%" PRIx32,
             __func__, filename, sharemode, fileaccess);
  ...
  if (fd == -1) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: Error opening file %s: %s", __func__, filename,
               g_strerror(errno));
  ...
  if (ret == -1) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: fstat error of file %s: %s", __func__, filename,
               g_strerror(errno));
    _wapi_set_last_error_from_errno();
    MONO_ENTER_GC_SAFE;
    close(fd);
...
#define S_ISFIFO(m) ((m & S_IFIFO) != 0)
  ...
  if (!share_allows_open(&statbuf, filehandle->sharemode,
                         filehandle->fileaccess, &filehandle->share_info)) {
  ...
  if (attrs & FILE_FLAG_SEQUENTIAL_SCAN) {
    MONO_ENTER_GC_SAFE;
    posix_fadvise(((MonoFDHandle *)filehandle)->fd, 0, 0,
...
gboolean mono_w32file_close(gpointer handle) {
...
gboolean mono_w32file_delete(const gunichar2 *name) {
  ...
  if (retval == -1) {
    /* On linux, calling unlink on an non-existing file in a read-only mount
     * will fail with EROFS. The expected behavior is for this function to
     * return FALSE and not trigger an exception. To work around this behavior,
     * we stat the file on failure.
     *
     * This was supposedly fixed on kernel 3.0 [1] but we could reproduce it
     * with Ubuntu 16.04 which has kernel 4.4. We can't remove this workaround
     * until the early 2020's when most Android deviced will have a fix. [1]
     * https://github.com/torvalds/linux/commit/50338b889dc504c69e0cb316ac92d1b9e51f3c8a
     */
    if (errno == EROFS) {
      MonoIOStat stat;
      if (mono_w32file_get_attributes_ex(
              name,
              &stat)) // The file exists, so must be due the RO file system
...
static gboolean MoveFile(const gunichar2 *name, const gunichar2 *dest_name) {
...
static gboolean write_file(gint src_fd, gint dest_fd, struct stat *st_src,
...
#if HOST_DARWIN
static int _wapi_clonefile(const char *from, const char *to, int flags) {
  ...
  MONO_ENTER_GC_SAFE;
  ret = clonefile_ptr(located_from == NULL ? from : located_from,
                      located_to == NULL ? to : located_to, flags);
  MONO_EXIT_GC_SAFE;
...
static gboolean CopyFile(const gunichar2 *name, const gunichar2 *dest_name,
  ...
  if (utf8_src == NULL) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: unicode conversion of source returned NULL; %s", __func__,
               mono_error_get_message(error));
  ...
  if (dest_name == NULL) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: dest is NULL",
               __func__);
  ...
  if (utf8_dest == NULL) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: unicode conversion of dest returned NULL; %s", __func__,
               mono_error_get_message(error));
  ...
  if (!_wapi_stat(utf8_dest, &dest_st)) {
     ...
     * error if src and dest are actually the same file. We do the check here to
     * take advantage of the IOMAP capability */
    ...
    /* Take advantage of the fact that we already know the file exists and bail
     * out early */
    if (fail_if_exists) {
      g_free(utf8_src);
      g_free(utf8_dest);
  ...
  if (ret_utime == -1)
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: file [%s] utime failed: %s", __func__, utf8_dest,
               g_strerror(errno));
...
static gchar *convert_arg_to_utf8(const gunichar2 *arg, const gchar *arg_name) {
  gchar *utf8_ret;
  ERROR_DECL(error);

  if (arg == NULL) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: %s is NULL",
               __func__, arg_name);
    mono_w32error_set_last(ERROR_INVALID_NAME);
    return NULL;
  }
  ...
  if (utf8_ret == NULL) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: unicode conversion of %s returned NULL; %s", __func__,
               arg_name, mono_error_get_message(error));
...
static gboolean ReplaceFile(const gunichar2 *replacedFileName,
                            const gunichar2 *replacementFileName,
                            const gunichar2 *backupFileName,
                            guint32 replaceFlags, gpointer exclude,
                            gpointer reserved) {
  gint result, backup_fd = -1, replaced_fd = -1;
  gchar *utf8_replacedFileName, *utf8_replacementFileName = NULL,
                                *utf8_backupFileName = NULL;
  struct stat stBackup;
  gboolean ret = FALSE;

  if (!(utf8_replacedFileName =
            convert_arg_to_utf8(replacedFileName, "replacedFileName")))
    return FALSE;
  if (!(utf8_replacementFileName =
            convert_arg_to_utf8(replacementFileName, "replacementFileName")))
    goto replace_cleanup;
  if (backupFileName != NULL) {
    if (!(utf8_backupFileName =
              convert_arg_to_utf8(backupFileName, "backupFileName")))
      goto replace_cleanup;
  }

  if (utf8_backupFileName) {
    // Open the backup file for read so we can restore the file if an error
    // occurs.
  ...
  if (result == -1) {
    ...
    if (backup_fd != -1 && !fstat(backup_fd, &stBackup)) {
      replaced_fd = _wapi_open(utf8_backupFileName,
                               O_WRONLY | O_CREAT | O_TRUNC, stBackup.st_mode);
...
static gpointer _wapi_stdhandle_create(gint fd, const gchar *name) {
  gint flags;
  FileHandle *filehandle;

  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: creating standard handle type %s, fd %d", __func__, name, fd);
  ...
  if (flags == -1) {
     ...
     */
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: fcntl error on fd %d: %s", __func__, fd, g_strerror(errno));
  ...
  default:
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: Can't figure out flags 0x%x", __func__, flags);
...
static gpointer mono_w32file_get_std_handle(gint stdhandle) {
  FileHandle **filehandle;
  gint fd;
  const gchar *name;
...
static gboolean mono_w32file_read_or_write(gboolean read, gpointer handle,
                                           gpointer buffer, guint32 numbytes,
                                           guint32 *bytesread,
                                           gint32 *win32error) {
  ...
  gboolean const ref = mono_fdhandle_lookup_and_ref(
      GPOINTER_TO_INT(handle), (MonoFDHandle **)&filehandle);
  if (!ref) {
    mono_w32error_set_last(ERROR_INVALID_HANDLE);
    goto exit;
  }
  ...
  case MONO_FDTYPE_FILE:
    ret = (read ? file_read : file_write)(filehandle, buffer, numbytes,
                                          bytesread);
    break;
  case MONO_FDTYPE_CONSOLE:
    ret = (read ? console_read : console_write)(filehandle, buffer, numbytes,
                                                bytesread);
    break;
  case MONO_FDTYPE_PIPE:
    ret = (read ? pipe_read : pipe_write)(filehandle, buffer, numbytes,
                                          bytesread);
    break;
  default:
    mono_w32error_set_last(ERROR_INVALID_HANDLE);
...
gboolean mono_w32file_read(gpointer handle, gpointer buffer, guint32 numbytes,
                           guint32 *bytesread, gint32 *win32error) {
  return mono_w32file_read_or_write(TRUE, handle, buffer, numbytes, bytesread,
                                    win32error);
}

gboolean mono_w32file_write(gpointer handle, gconstpointer buffer,
                            guint32 numbytes, guint32 *byteswritten,
                            gint32 *win32error) {
  return mono_w32file_read_or_write(FALSE, handle, (gpointer)buffer, numbytes,
                                    byteswritten, win32error);
}

gboolean mono_w32file_flush(gpointer handle) {
  FileHandle *filehandle;
  gboolean ret;

  if (!mono_fdhandle_lookup_and_ref(GPOINTER_TO_INT(handle),
                                    (MonoFDHandle **)&filehandle)) {
    mono_w32error_set_last(ERROR_INVALID_HANDLE);
    return FALSE;
  }
...
gboolean mono_w32file_truncate(gpointer handle) {
  FileHandle *filehandle;
  gboolean ret;
...
guint32 mono_w32file_seek(gpointer handle, gint32 movedistance,
...
static guint32 GetFileSize(gpointer handle, guint32 *highsize) {
  FileHandle *filehandle;
  guint32 ret;
...
gboolean mono_w32file_set_times(gpointer handle, const FILETIME *create_time,
                                const FILETIME *access_time,
                                const FILETIME *write_time) {
  FileHandle *filehandle;
  gboolean ret;
...
gboolean mono_w32file_filetime_to_systemtime(const FILETIME *file_time,
  ...
  file_ticks =
      ((gint64)file_time->dwHighDateTime << 32) + file_time->dwLowDateTime;
...
#define LEAPS_THRU_END_OF(y) (DIV(y, 4) - DIV(y, 100) + DIV(y, 400))
  ...
  while (totaldays < 0 || totaldays >= (isleap(y) ? 366 : 365)) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: totaldays: %" G_GINT64_FORMAT " yg: %" G_GINT64_FORMAT
               " y: %" G_GINT64_FORMAT,
               __func__, totaldays, yg, y);
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: totaldays: %" G_GINT64_FORMAT, __func__, totaldays);
    y = yg;
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: y: %" G_GINT64_FORMAT, __func__, y);
  }

  system_time->wYear = y;
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: Year: %d",
             __func__, system_time->wYear);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: Month: %d",
             __func__, system_time->wMonth);

  system_time->wDay = totaldays + 1;
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: Day: %d",
             __func__, system_time->wDay);

  return (TRUE);
}

static void findhandle_destroy(gpointer data) {
...
static FindHandle *findhandle_create(void) {
  FindHandle *findhandle;
...
static void findhandle_insert(FindHandle *findhandle) {
...
static gboolean findhandle_lookup_and_ref(gpointer handle,
                                          FindHandle **findhandle) {
  mono_coop_mutex_lock(&finds_mutex);

  if (!g_hash_table_lookup_extended(finds, handle, NULL,
                                    (gpointer *)findhandle)) {
    mono_coop_mutex_unlock(&finds_mutex);
    return FALSE;
  }
...
static void findhandle_unref(FindHandle *findhandle) {
  mono_refcount_dec(findhandle);
}

static gboolean findhandle_close(gpointer handle) {
  FindHandle *findhandle;
  gboolean removed;
...
static void finds_remove(gpointer data) {
  FindHandle *findhandle;
...
gpointer mono_w32file_find_first(const gunichar2 *pattern,
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: looking for [%s]", __func__, utf8_pattern);
  ...
  namelist = NULL;
  result = _wapi_io_scandir(dir_part, entry_part, &namelist);
  ...
  if (result < 0) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: scandir error: %s", __func__, g_strerror(errno));
    g_free(utf8_pattern);
    g_free(entry_part);
    g_free(dir_part);
    g_strfreev(namelist);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: Got %d matches",
             __func__, result);
...
gboolean mono_w32file_find_next(gpointer handle, WIN32_FIND_DATA *find_data) {
  FindHandle *findhandle;
...
retry:
  ...
  filename = g_build_filename(findhandle->dir_part,
                              findhandle->namelist[findhandle->count++],
                              (const char *)NULL);
  ...
  if (result != 0) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: stat failed: %s", __func__, filename);
  ...
  if (result != 0) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: lstat failed: %s", __func__, filename);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: Found [%s]",
             __func__, utf8_filename);
  ...
  find_data->dwFileAttributes =
      _wapi_stat_to_file_attributes(utf8_filename, &buf, &linkbuf);
  ...
  utf16_basename = g_utf8_to_utf16(utf8_basename, -1, NULL, &bytes, NULL);
...
gboolean mono_w32file_create_directory(const gunichar2 *name) {
  gchar *utf8_name;
  gint result;
  ERROR_DECL(error);
...
gboolean mono_w32file_remove_directory(const gunichar2 *name) {
  gchar *utf8_name;
  gint result;
  ERROR_DECL(error);
...
guint32 mono_w32file_get_attributes(const gunichar2 *name) {
  gchar *utf8_name;
  struct stat buf, linkbuf;
  gint result;
  guint32 ret;
...
gboolean mono_w32file_get_attributes_ex(const gunichar2 *name,
                                        MonoIOStat *stat) {
  gchar *utf8_name;
  ...
  stat->length =
      (stat->attributes & FILE_ATTRIBUTE_DIRECTORY) ? 0 : buf.st_size;

#if HAVE_STRUCT_STAT_ST_ATIMESPEC
  if (linkbuf.st_mtimespec.tv_sec < linkbuf.st_ctimespec.tv_sec ||
      (linkbuf.st_mtimespec.tv_sec == linkbuf.st_ctimespec.tv_sec &&
       linkbuf.st_mtimespec.tv_nsec < linkbuf.st_ctimespec.tv_nsec))
    stat->creation_time =
        linkbuf.st_mtimespec.tv_sec * TICKS_PER_SECOND +
        (linkbuf.st_mtimespec.tv_nsec / NANOSECONDS_PER_MICROSECOND) *
            TICKS_PER_MICROSECOND +
        CONVERT_BASE;
  else
    stat->creation_time =
        linkbuf.st_ctimespec.tv_sec * TICKS_PER_SECOND +
        (linkbuf.st_ctimespec.tv_nsec / NANOSECONDS_PER_MICROSECOND) *
            TICKS_PER_MICROSECOND +
        CONVERT_BASE;

  stat->last_access_time =
      linkbuf.st_atimespec.tv_sec * TICKS_PER_SECOND +
      (linkbuf.st_atimespec.tv_nsec / NANOSECONDS_PER_MICROSECOND) *
          TICKS_PER_MICROSECOND +
      CONVERT_BASE;
  ...
  if (linkbuf.st_mtime < linkbuf.st_ctime ||
      (linkbuf.st_mtime == linkbuf.st_ctime &&
       linkbuf.st_mtim.tv_nsec < linkbuf.st_ctim.tv_nsec))
    stat->creation_time =
        linkbuf.st_mtime * TICKS_PER_SECOND +
        (linkbuf.st_mtim.tv_nsec / NANOSECONDS_PER_MICROSECOND) *
            TICKS_PER_MICROSECOND +
        CONVERT_BASE;
  else
    stat->creation_time =
        linkbuf.st_ctime * TICKS_PER_SECOND +
        (linkbuf.st_ctim.tv_nsec / NANOSECONDS_PER_MICROSECOND) *
            TICKS_PER_MICROSECOND +
        CONVERT_BASE;

  stat->last_access_time =
      linkbuf.st_atime * TICKS_PER_SECOND +
      (linkbuf.st_atim.tv_nsec / NANOSECONDS_PER_MICROSECOND) *
          TICKS_PER_MICROSECOND +
      CONVERT_BASE;
  stat->last_write_time =
      linkbuf.st_mtime * TICKS_PER_SECOND +
...
#else
  stat->creation_time =
      (((guint64)(linkbuf.st_mtime < linkbuf.st_ctime ? linkbuf.st_mtime
                                                      : linkbuf.st_ctime)) *
       TICKS_PER_SECOND) +
      CONVERT_BASE;
  stat->last_access_time =
      (((guint64)(linkbuf.st_atime)) * TICKS_PER_SECOND) + CONVERT_BASE;
  stat->last_write_time =
      (((guint64)(linkbuf.st_mtime)) * TICKS_PER_SECOND) + CONVERT_BASE;
#endif
...
gboolean mono_w32file_set_attributes(const gunichar2 *name, guint32 attrs) {
  ...
  if (attrs & FILE_ATTRIBUTE_READONLY) {
    result =
        _wapi_chmod(utf8_name, buf.st_mode & ~(S_IWUSR | S_IWOTH | S_IWGRP));
  } else {
...
guint32 mono_w32file_get_cwd(guint32 length, gunichar2 *buffer) {
  ...
  if (getcwd((gchar *)buffer, length) == NULL) {
    if (errno == ERANGE) { /*buffer length is not big enough */
      gchar *path =
          g_get_current_dir(); /*FIXME g_get_current_dir doesn't work with
                                  broken paths and calling it just to know the
                                  path length is silly*/
      if (path == NULL)
        return 0;
...
gboolean mono_w32file_set_cwd(const gunichar2 *path) {
  gchar *utf8_path;
  gboolean result;
  ERROR_DECL(error);
...
gboolean mono_w32file_create_pipe(gpointer *readpipe, gpointer *writepipe,
                                  guint32 size) {
  FileHandle *read_filehandle, *write_filehandle;
  gint filedes[2];
  gint ret;

  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE, "%s: Creating pipe",
             __func__);
  ...
  if (ret == -1) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: Error creating pipe: (%d) %s", __func__, errno,
               g_strerror(errno));
  ...
  *writepipe = GINT_TO_POINTER(((MonoFDHandle *)write_filehandle)->fd);
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             "%s: Returning pipe: read handle %p, write handle %p", __func__,
             GINT_TO_POINTER(((MonoFDHandle *)read_filehandle)->fd),
             GINT_TO_POINTER(((MonoFDHandle *)write_filehandle)->fd));
...
gint32 mono_w32file_get_logical_drive(guint32 len, gunichar2 *buf) {
  struct statfs *stats;
  gint size, n, i;
  gunichar2 *dir;
...
gint32 mono_w32file_get_logical_drive(guint32 len, gunichar2 *buf) {
  ...
  for (int i = 0; i < ret; i++) {
    dir =
        g_utf8_to_utf16(vmt2dataptr(mounts, VMT_STUB), -1, NULL, &length, NULL);
    if (total + length < len) {
      memcpy(buf + total, dir, sizeof(gunichar2) * length);
      buf[total + length] = 0;
    }
...
static void unescape_octal(gchar *str) {
  gchar *rptr;
  gchar *wptr;
...
static gboolean GetLogicalDriveStrings_Mounts(guint32 len, gunichar2 *buf,
                                              LinuxMountInfoParseState *state);
static gboolean
GetLogicalDriveStrings_MountInfo(guint32 len, gunichar2 *buf,
                                 LinuxMountInfoParseState *state);
static void append_to_mountpoint(LinuxMountInfoParseState *state);
static gboolean add_drive_string(guint32 len, gunichar2 *buf,
                                 LinuxMountInfoParseState *state);
  ...
  while (state->buffer_index < (guint32)state->nbytes) {
    ...
    case 3:
      if (state->fstype_index < GET_LOGICAL_DRIVE_STRINGS_FSNAME_BUFFER)
        state->fstype[state->fstype_index++] =
            state->buffer[state->buffer_index];
      break;
    }
...
static void append_to_mountpoint(LinuxMountInfoParseState *state) {
  ...
  if (state->mountpoint_allocated) {
    ...
    if (state->mountpoint_index >=
        GET_LOGICAL_DRIVE_STRINGS_MOUNTPOINT_BUFFER) {
      state->allocated_size = (state->mountpoint_index << 1) + 1;
      state->mountpoint_allocated =
          (gchar *)g_malloc0(state->allocated_size * sizeof(gchar));
      memcpy(state->mountpoint_allocated, state->mountpoint,
             state->mountpoint_index);
      state->mountpoint_allocated[state->mountpoint_index++] = ch;
    } else
...
static gboolean add_drive_string(guint32 len, gunichar2 *buf,
  ...
  if (state->fsname_index == 1 && state->fsname[0] == '/')
    ...
    ignore_entry = FALSE;
  } else if (state->fsname_index == 0 ||
             memcmp("none", state->fsname, state->fsname_index) == 0) {
    ignore_entry = TRUE;
  } else if (state->fstype_index >= 5 &&
             memcmp("fuse.", state->fstype, 5) == 0) {
    /* Ignore GNOME's gvfs */
    if (state->fstype_index == 21 &&
        memcmp("fuse.gvfs-fuse-daemon", state->fstype, state->fstype_index) ==
            0)
      ignore_entry = TRUE;
    else
      ignore_entry = FALSE;
  } else if (state->fstype_index == 3 &&
             memcmp("nfs", state->fstype, state->fstype_index) == 0)
    ignore_entry = FALSE;
  else
    ignore_entry = TRUE;
  ...
  if (!ignore_entry) {
    gunichar2 *dir;
    glong length;
    gchar *mountpoint = state->mountpoint_allocated
                            ? state->mountpoint_allocated
                            : state->mountpoint;
...
gboolean mono_w32file_get_disk_free_space(const gunichar2 *path_name,
                                          guint64 *free_bytes_avail,
                                          guint64 *total_number_of_bytes,
                                          guint64 *total_number_of_free_bytes) {
  ...
  if (ret == -1) {
    _wapi_set_last_error_from_errno();
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
               "%s: statvfs failed: %s", __func__, g_strerror(errno));
    return (FALSE);
  }
...
#if HOST_DARWIN
    {DRIVE_REMOTE, "afp"},     {DRIVE_REMOTE, "afpfs"},
    {DRIVE_REMOTE, "autofs"},  {DRIVE_CDROM, "cddafs"},
    {DRIVE_CDROM, "cd9660"},   {DRIVE_RAMDISK, "devfs"},
    {DRIVE_RAMDISK, "nullfs"}, {DRIVE_FIXED, "exfat"},
    {DRIVE_RAMDISK, "fdesc"},  {DRIVE_REMOTE, "ftp"},
    {DRIVE_FIXED, "hfs"},      {DRIVE_FIXED, "apfs"},
    {DRIVE_REMOTE, "kbfuse"},  {DRIVE_FIXED, "msdos"},
    {DRIVE_REMOTE, "nfs"},     {DRIVE_FIXED, "ntfs"},
    {DRIVE_REMOTE, "smbfs"},   {DRIVE_FIXED, "udf"},
    {DRIVE_REMOTE, "webdav"},  {DRIVE_FIXED, "ufsd_NTFS"},
    {DRIVE_UNKNOWN, NULL}
#elif __linux__
...
guint32
ves_icall_System_IO_DriveInfo_GetDriveType(const gunichar2 *root_path_name,
  ...
  if (root_path_name == NULL) {
    ...
    utf8_root_path_name =
        mono_unicode_to_external_checked(root_path_name, error);
    if (utf8_root_path_name == NULL) {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
                 "%s: unicode conversion returned NULL; %s", __func__,
                 mono_error_get_message(error));
...
#if defined(HOST_DARWIN) || defined(__linux__) || defined(HOST_BSD) ||         \
    defined(__FreeBSD_kernel__) || defined(__HAIKU__) || defined(_AIX)
static gchar *get_fstypename(gchar *utfpath) {
#if defined(_AIX)
...
gboolean mono_w32file_get_file_system_type(const gunichar2 *path,
                                           gunichar2 *fsbuffer,
                                           gint fsbuffersize) {
  gchar *utfpath;
  gchar *fstypename;
...
static gboolean LockFile(gpointer handle, guint32 offset_low,
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_IO_LAYER_FILE,
             ...
             __func__, ((MonoFDHandle *)filehandle)->fd, (gint64)offset,
             (gint64)length);
#else
  ...
  ret =
      _wapi_lock_file_region(((MonoFDHandle *)filehandle)->fd, offset, length);

  mono_fdhandle_unref((MonoFDHandle *)filehandle);
  return ret;
}

static gboolean UnlockFile(gpointer handle, guint32 offset_low,
                           guint32 offset_high, guint32 length_low,
                           guint32 length_high) {
  FileHandle *filehandle;
  gboolean ret;
  ...
  ret = _wapi_unlock_file_region(((MonoFDHandle *)filehandle)->fd, offset,
                                 length);
...
void mono_w32file_init(void) {
  ...
  finds =
      g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, finds_remove);
...
void mono_w32file_cleanup(void) {
...
gboolean mono_w32file_move(const gunichar2 *path, const gunichar2 *dest,
                           gint32 *error) {
  gboolean result;
...
gboolean mono_w32file_copy(const gunichar2 *path, const gunichar2 *dest,
                           gboolean overwrite, gint32 *error) {
  gboolean result;
...
gboolean mono_w32file_replace(const gunichar2 *destination_file_name,
                              const gunichar2 *source_file_name,
                              const gunichar2 *destination_backup_file_name,
                              guint32 flags, gint32 *error) {
  gboolean result;

  result = ReplaceFile(destination_file_name, source_file_name,
                       destination_backup_file_name, flags, NULL, NULL);
  if (!result)
    *error = mono_w32error_get_last();
  return result;
}

gint64 mono_w32file_get_file_size(gpointer handle, gint32 *error) {
  gint64 length;
  guint32 length_hi = 0;
...
gboolean mono_w32file_lock(gpointer handle, gint64 position, gint64 length,
                           gint32 *error) {
  gboolean result;
...
gboolean mono_w32file_unlock(gpointer handle, gint64 position, gint64 length,
                             gint32 *error) {
  gboolean result;
...
gpointer mono_w32file_get_console_input(void) {
  return mono_w32file_get_std_handle(STD_INPUT_HANDLE);
}

gpointer mono_w32file_get_console_output(void) {
  return mono_w32file_get_std_handle(STD_OUTPUT_HANDLE);
}

gpointer mono_w32file_get_console_error(void) {
