...
Right now we do this:
        MonoCultureInfoHandle culture = MONO_HANDLE_NEW (MonoCultureInfo,
mono_object_new_checked (domain, klass, error));

Maybe what we need is a round of cleanup around all exposed types in the runtime
to unify all helpers under the same hoof. Combine: MonoDefaults,
GENERATE_GET_CLASS_WITH_CACHE, TYPED_HANDLE_DECL and friends. This would solve
the age old issue of making it clear which types are optional and tell that to
the linker. We could then generate neat type safe wrappers.
*/
...
static HandleStack *new_handle_stack(void) { return g_new(HandleStack, 1); }

static void free_handle_stack(HandleStack *stack) { g_free(stack); }

static HandleChunk *new_handle_chunk(void) { return g_new(HandleChunk, 1); }

static void free_handle_chunk(HandleChunk *chunk) { g_free(chunk); }

#define THIS_IS_AN_OK_NUMBER_OF_HANDLES 100

static HandleChunkElem *chunk_element(HandleChunk *chunk, int idx) {
  return &chunk->elems[idx];
}

#ifdef MONO_HANDLE_TRACK_OWNER
#ifdef HAVE_BACKTRACE_SYMBOLS
#define SET_BACKTRACE(btaddrs)                                                 \
  do {                                                                         \
    backtrace(btaddrs, 7);                                                     \
  } while (0)
#else
...
#endif
#define SET_OWNER(chunk, idx)                                                  \
  do {                                                                         \
    (chunk)->elems[(idx)].owner = owner;                                       \
    SET_BACKTRACE(&((chunk)->elems[(idx)].backtrace_ips[0]));                  \
  } while (0)
#else
#define SET_OWNER(chunk, idx)                                                  \
  do {                                                                         \
...
#define SET_SP(handles, chunk, idx)                                            \
  do {                                                                         \
    (chunk)->elems[(idx)].alloc_sp = handles->stackmark_sp;                    \
  } while (0)
#else
#define SET_SP(handles, chunk, idx)                                            \
  do {                                                                         \
...
void mono_handle_stack_free_domain(HandleStack *stack, MonoDomain *domain) {
  /* Called by the GC while clearing out objects of the given domain from the
   * heap. */
   ...
   * unloading domain.  However if there is a handle leak, the handle stack is
   * not */
  if (!stack)
    return;
  /* Root domain only unloaded when mono is shutting down, don't need to check
   * anything */
...
static void check_handle_stack_monotonic(HandleStack *stack) {
  /* check that every allocated handle in the current handle stack is at no
   * higher in the native stack than its predecessors */
#ifdef MONO_HANDLE_TRACK_SP
  HandleChunk *cur = stack->bottom;
  HandleChunk *last = stack->top;
  if (!cur)
  ...
  while (cur) {
    for (int i = 0; i < cur->size; ++i) {
      ...
      if (prev && elem->alloc_sp > prev->alloc_sp) {
        monotonic = FALSE;
#ifdef MONO_HANDLE_TRACK_OWNER
        g_warning("Handle %p (object %p) (allocated from \"%s\") was allocated "
                  "deeper in the call stack than its successor Handle %p "
                  "(object %p) (allocated from \"%s\").",
                  prev, prev->o, prev->owner, elem, elem->o, elem->owner);
#else
        g_warning("Handle %p (object %p) was allocated deeper in the call "
                  "stack than its successor Handle %p (object %p).",
                  prev, prev->o, elem, elem->o);
#endif
      }
      prev = elem;
    }
...
MonoStringHandle mono_string_new_handle(MonoDomain *domain, const char *data,
  ...
  return MONO_HANDLE_NEW(MonoString,
                         mono_string_new_checked(domain, data, error));
}
...
MonoArrayHandle mono_array_new_handle(MonoDomain *domain, MonoClass *eclass,
  ...
  return MONO_HANDLE_NEW(MonoArray,
                         mono_array_new_checked(domain, eclass, n, error));
}
...
MonoArrayHandle mono_array_new_full_handle(MonoDomain *domain,
  ...
  return MONO_HANDLE_NEW(
      MonoArray, mono_array_new_full_checked(domain, array_class, lengths,
                                             lower_bounds, error));
}
...
MonoObjectHandle mono_gchandle_get_target_handle(MonoGCHandle gchandle) {
  return MONO_HANDLE_NEW(MonoObject,
...
gpointer mono_array_handle_pin_with_size(MonoArrayHandle handle, int size,
  ...
  *gchandle =
      mono_gchandle_from_handle(MONO_HANDLE_CAST(MonoObject, handle), TRUE);
  MonoArray *raw = MONO_HANDLE_RAW(handle);
  return mono_array_addr_with_size_internal(raw, size, idx);
}

gunichar2 *mono_string_handle_pin_chars(MonoStringHandle handle,
                                        MonoGCHandle *gchandle) {
  g_assert(gchandle != NULL);
  *gchandle =
      mono_gchandle_from_handle(MONO_HANDLE_CAST(MonoObject, handle), TRUE);
...
gpointer mono_object_handle_pin_unbox(MonoObjectHandle obj,
                                      MonoGCHandle *gchandle) {
...
void mono_array_handle_memcpy_refs(MonoArrayHandle dest, uintptr_t dest_idx,
                                   ...
                                   uintptr_t len) {
  mono_array_memcpy_refs_internal(MONO_HANDLE_RAW(dest), dest_idx,
                                  MONO_HANDLE_RAW(src), src_idx, len);
}
...
MonoGCHandle mono_gchandle_new_weakref_from_handle(MonoObjectHandle handle) {
  return mono_gchandle_new_weakref_internal(
      MONO_HANDLE_SUPPRESS(MONO_HANDLE_RAW(handle)), FALSE);
}

// FIXME inline
int mono_handle_hash(MonoObjectHandle object) {
  return mono_object_hash_internal(
      MONO_HANDLE_SUPPRESS(MONO_HANDLE_RAW(object)));
}
...
MonoGCHandle mono_gchandle_new_weakref_from_handle_track_resurrection(
  ...
  return mono_gchandle_new_weakref_internal(
      MONO_HANDLE_SUPPRESS(MONO_HANDLE_RAW(handle)), TRUE);
}
...
void mono_handle_array_getref(MonoObjectHandleOut dest, MonoArrayHandle array,
  ...
  MONO_HANDLE_SUPPRESS(*dest.__raw = (MonoObject *)mono_array_get_internal(
                           MONO_HANDLE_RAW(array), gpointer, index));
}
