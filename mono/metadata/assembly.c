...
static gboolean exact_sn_match(MonoAssemblyName *wanted_name,
...
mono_assembly_request_open(const char *filename,
  ...
  *status = MONO_IMAGE_OK;
  ...
  if (image->assembly) {
    ...
    if (mono_loader_get_strict_strong_names() && load_req.predicate &&
...
gboolean exact_sn_match(MonoAssemblyName *wanted_name,
                        MonoAssemblyName *candidate_name) {
#if ENABLE_NETCORE
