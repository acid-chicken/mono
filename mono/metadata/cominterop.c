...
static void mono_System_ComObject_ReleaseInterfaces(MonoComObjectHandle obj);

#if !defined(DISABLE_COM) || defined(HOST_WIN32)

static int mono_IUnknown_QueryInterface(MonoIUnknown *pUnk, gconstpointer riid,
                                        gpointer *ppv) {
  g_assert(pUnk);
  return pUnk->vtable->QueryInterface(pUnk, riid, ppv);
}

static int mono_IUnknown_AddRef(MonoIUnknown *pUnk) {
  // The return value is a reference count, generally transient, generally not
  // to be used, except for debugging, or to assert that it is > 0.
  g_assert(pUnk);
  return pUnk->vtable->AddRef(pUnk);
}

static int mono_IUnknown_Release(MonoIUnknown *pUnk) {
  // Release is like free -- null is silently ignored.
  // Also, the return value is a reference count, generally transient, generally
...
mono_bstr mono_string_to_bstr_impl(MonoStringHandle s, MonoError *error) {
  if (MONO_HANDLE_IS_NULL(s))
    return NULL;

  gchandle_t gchandle = 0;
  mono_bstr const res = mono_ptr_to_bstr(
      mono_string_handle_pin_chars(s, &gchandle), mono_string_handle_length(s));
  mono_gchandle_free_internal(gchandle);
  return res;
}

static void *mono_cominterop_get_com_interface_internal(gboolean icall,
...
typedef enum {
  ...
  MONO_MARSHAL_COPY_OUT, /* out parameter that needs to be copied back to the
                            original instance */
  MONO_MARSHAL_SERIALIZE /* Value needs to be serialized into the new domain */
} MonoXDomainMarshalType;

typedef enum { MONO_COM_DEFAULT, MONO_COM_MS } MonoCOMProvider;
...
static GENERATE_GET_CLASS_WITH_CACHE(interface_type_attribute, "System.Runtime.InteropServices", "InterfaceTypeAttribute") static GENERATE_GET_CLASS_WITH_CACHE(
    guid_attribute, "System.Runtime.InteropServices",
    "GuidAttribute") static GENERATE_GET_CLASS_WITH_CACHE(com_visible_attribute,
                                                          "System.Runtime."
                                                          "InteropServices",
                                                          "ComVisibleAttribut"
                                                          "e") static GENERATE_GET_CLASS_WITH_CACHE(com_default_interface_attribute,
     ...
     * count goes back to 0, convert back to a weak handle.
     */
    typedef struct {
  guint32 ref_count;
  guint32 gc_handle;
...
static int STDCALL cominterop_ccw_queryinterface(MonoCCWInterface *ccwe,
                                                 const guint8 *riid,
                                                 gpointer *ppv);

/* IDispatch */
static int STDCALL cominterop_ccw_get_type_info_count(MonoCCWInterface *ccwe,
                                                      guint32 *pctinfo);

static int STDCALL cominterop_ccw_get_type_info(MonoCCWInterface *ccwe,
                                                guint32 iTInfo, guint32 lcid,
                                                gpointer *ppTInfo);

static int STDCALL cominterop_ccw_get_ids_of_names(MonoCCWInterface *ccwe,
                                                   gpointer riid,
                                                   gunichar2 **rgszNames,
                                                   guint32 cNames, guint32 lcid,
                                                   gint32 *rgDispId);

static int STDCALL cominterop_ccw_invoke(
    MonoCCWInterface *ccwe, guint32 dispIdMember, gpointer riid, guint32 lcid,
    guint16 wFlags, gpointer pDispParams, gpointer pVarResult,
    gpointer pExcepInfo, guint32 *puArgErr);

static MonoMethod *cominterop_get_managed_wrapper_adjusted(MonoMethod *method);

static gpointer cominterop_get_ccw(MonoObject *object, MonoClass *itf);

static gpointer cominterop_get_ccw_checked(MonoObjectHandle object,
                                           MonoClass *itf, MonoError *error);

static MonoObject *cominterop_get_ccw_object(MonoCCWInterface *ccw_entry,
                                             gboolean verify);

static MonoObjectHandle cominterop_get_ccw_handle(MonoCCWInterface *ccw_entry,
                                                  gboolean verify);

static MonoObject *cominterop_set_ccw_object_domain(MonoObject *object,
                                                    MonoDomain **prev_domain);

static void cominterop_restore_domain(MonoDomain *domain);

/* SAFEARRAY marshalling */
static gboolean mono_marshal_safearray_begin(gpointer safearray,
                                             MonoArray **result,
                                             gpointer *indices, gpointer empty,
                                             gpointer parameter,
                                             gboolean allocateNewArray);

static gpointer mono_marshal_safearray_get_value(gpointer safearray,
                                                 gpointer indices);

static gboolean mono_marshal_safearray_next(gpointer safearray,
                                            gpointer indices);

static void mono_marshal_safearray_end(gpointer safearray, gpointer indices);

static gboolean mono_marshal_safearray_create(MonoArray *input,
                                              gpointer *newsafearray,
                                              gpointer *indices,
                                              gpointer empty);

static void mono_marshal_safearray_set_value(gpointer safearray,
                                             gpointer indices, gpointer value);

static void mono_marshal_safearray_free_indices(gpointer indices);
...
MonoClass *mono_class_try_get_com_object_class(void) {
  ...
  MonoClass *klass;
  if (!inited) {
    klass = mono_class_load_from_name(mono_defaults.corlib, "System",
                                      "__ComObject");
    mono_memory_barrier();
    tmp_class = klass;
    mono_memory_barrier();
 ...
 * method.
 */
static MonoMethodSignature *cominterop_method_signature(MonoMethod *method) {
  ...
  // STDCALL on windows, CDECL everywhere else to work with XPCOM and MainWin
  // COM
#ifdef HOST_WIN32
 ...
 */
static gpointer cominterop_get_function_pointer(gpointer itf, int slot) {
  gpointer func;
 ...
 */
static gboolean
cominterop_object_is_rcw_handle(MonoObjectHandle obj,
                                MonoRealProxyHandle *real_proxy) {
  MonoClass *klass;

  return !MONO_HANDLE_IS_NULL(obj) && (klass = mono_handle_class(obj)) &&
         mono_class_is_transparent_proxy(klass) &&
         !MONO_HANDLE_IS_NULL(*real_proxy = MONO_HANDLE_NEW_GET(
                                  MonoRealProxy,
...
static gboolean cominterop_object_is_rcw(MonoObject *obj_raw) {
  if (!obj_raw)
    return FALSE;
...
static int cominterop_get_com_slot_begin(MonoClass *klass) {
  ERROR_DECL(error);
  ...
  if (cinfo) {
    itf_attr = (MonoInterfaceTypeAttribute *)mono_custom_attrs_get_attr_checked(
        cinfo, mono_class_get_interface_type_attribute_class(), error);
 ...
 */
static MonoClass *cominterop_get_method_interface(MonoMethod *method) {
  ERROR_DECL(error);
  ...
  if (!MONO_CLASS_IS_INTERFACE_INTERNAL(method->klass)) {
    GPtrArray *ifaces =
        mono_class_get_implemented_interfaces(method->klass, error);
    mono_error_assert_ok(error);
    if (ifaces) {
      int i;
...
static void mono_cominterop_get_interface_missing_error(MonoError *error,
                                                        MonoMethod *method) {
  mono_error_set_invalid_operation(
      error,
      "Method '%s' in ComImport class '%s' must implement an interface method.",
      method->name, m_class_get_name(method->klass));
}
 ...
 */
static int cominterop_get_com_slot_for_method(MonoMethod *method,
                                              MonoError *error) {
...
static void cominterop_mono_string_to_guid(MonoString *string, guint8 *guid);

static gboolean cominterop_class_guid(MonoClass *klass, guint8 *guid) {
  ERROR_DECL(error);
  ...
  if (cinfo) {
    MonoReflectionGuidAttribute *attr =
        (MonoReflectionGuidAttribute *)mono_custom_attrs_get_attr_checked(
            cinfo, mono_class_get_guid_attribute_class(), error);
    mono_error_assert_ok(error); /*FIXME proper error handling*/
...
static gboolean cominterop_com_visible(MonoClass *klass) {
  ERROR_DECL(error);
  MonoCustomAttrInfo *cinfo;
  GPtrArray *ifaces;
  ...
  mono_error_assert_ok(error);
  if (cinfo) {
    MonoReflectionComVisibleAttribute *attr =
        (MonoReflectionComVisibleAttribute *)mono_custom_attrs_get_attr_checked(
            cinfo, mono_class_get_com_visible_attribute_class(), error);
    mono_error_assert_ok(error); /*FIXME proper error handling*/
...
static void cominterop_set_hr_error(MonoError *oerror, int hr) {
  ...
  throw_exception_for_hr = mono_class_get_method_from_name_checked(
      mono_defaults.marshal_class, "GetExceptionForHR", 1, 0, error);
  mono_error_assert_ok(error);

  MONO_STATIC_POINTER_INIT_END(MonoMethod, throw_exception_for_hr)

  ex = (MonoException *)mono_runtime_invoke_checked(throw_exception_for_hr,
                                                    NULL, params, error);
  g_assert(ex);
  mono_error_assert_ok(error);
 ...
 */
static gpointer cominterop_get_interface_checked(MonoComObjectHandle obj,
                                                 MonoClass *ic,
                                                 MonoError *error) {
  gpointer itf = NULL;
  ...
  if (MONO_HANDLE_GETVAL(obj, itf_hash))
    itf = g_hash_table_lookup(
        MONO_HANDLE_GETVAL(obj, itf_hash),
        GUINT_TO_POINTER((guint)m_class_get_interface_id(ic)));
  ...
  int const hr = mono_IUnknown_QueryInterface(MONO_HANDLE_GETVAL(obj, iunknown),
                                              iid, &itf);
  if (hr < 0) {
    g_assert(!itf);
  ...
  mono_cominterop_lock();
  if (!MONO_HANDLE_GETVAL(obj, itf_hash))
    MONO_HANDLE_SETVAL(obj, itf_hash, GHashTable *,
                       g_hash_table_new(mono_aligned_addr_hash, NULL));
  g_hash_table_insert(MONO_HANDLE_GETVAL(obj, itf_hash),
                      GUINT_TO_POINTER((guint)m_class_get_interface_id(ic)),
                      itf);
  mono_cominterop_unlock();
 ...
 */
static gpointer cominterop_get_interface(MonoComObject *obj_raw,
                                         MonoClass *ic) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
...
static MonoReflectionType *cominterop_type_from_handle(MonoType *handle) {
...
void mono_cominterop_init(void) {
  ...
  register_icall(cominterop_get_method_interface, mono_icall_sig_ptr_ptr,
                 FALSE);
  register_icall(cominterop_get_function_pointer, mono_icall_sig_ptr_ptr_int32,
                 FALSE);
  ...
  register_icall(cominterop_get_ccw_object, mono_icall_sig_object_ptr_int32,
                 FALSE);
  register_icall(cominterop_get_interface, mono_icall_sig_ptr_object_ptr,
                 FALSE);

  register_icall(cominterop_type_from_handle, mono_icall_sig_object_ptr, FALSE);

  register_icall(cominterop_set_ccw_object_domain,
                 mono_icall_sig_object_object_ptr, FALSE);
  register_icall(cominterop_restore_domain, mono_icall_sig_void_ptr, FALSE);

  /* SAFEARRAY marshalling */
  register_icall(mono_marshal_safearray_begin,
                 mono_icall_sig_int32_ptr_ptr_ptr_ptr_ptr_int32, FALSE);
  register_icall(mono_marshal_safearray_get_value, mono_icall_sig_ptr_ptr_ptr,
                 FALSE);
  register_icall(mono_marshal_safearray_next, mono_icall_sig_int32_ptr_ptr,
                 FALSE);
  register_icall(mono_marshal_safearray_end, mono_icall_sig_void_ptr_ptr,
                 FALSE);
  register_icall(mono_marshal_safearray_create,
                 mono_icall_sig_int32_object_ptr_ptr_ptr, FALSE);
  register_icall(mono_marshal_safearray_set_value,
                 mono_icall_sig_void_ptr_ptr_ptr, FALSE);
  register_icall(mono_marshal_safearray_free_indices, mono_icall_sig_void_ptr,
                 FALSE);
#endif // DISABLE_COM
  /*FIXME

  This icalls are used by the marshal code when doing PtrToStructure and
  StructureToPtr and pinvoke.

  If we leave them out and the FullAOT compiler finds the need to emit one of
  the above 3 wrappers it will g_assert.

  The proper fix would be to emit warning, remove them from marshal.c when
  DISABLE_COM is used and emit an exception in the generated IL.
  */
...
void mono_cominterop_cleanup(void) { mono_os_mutex_destroy(&cominterop_mutex); }

void mono_mb_emit_cominterop_get_function_pointer(MonoMethodBuilder *mb,
                                                  MonoMethod *method) {
#ifndef DISABLE_JIT
  int slot;
  ERROR_DECL(error);
...
void mono_mb_emit_cominterop_call_function_pointer(MonoMethodBuilder *mb,
...
void mono_cominterop_emit_ptr_to_object_conv(MonoMethodBuilder *mb,
                                             MonoType *type,
  ...
  case MONO_MARSHAL_CONV_OBJECT_IDISPATCH: {
    ...
    ERROR_DECL(error);
    com_interop_proxy_get_proxy = mono_class_get_method_from_name_checked(
        mono_class_get_interop_proxy_class(), "GetProxy", 2,
        METHOD_ATTRIBUTE_PRIVATE, error);
    mono_error_assert_ok(error);
    ...
    ERROR_DECL(error);
    get_transparent_proxy = mono_class_get_method_from_name_checked(
    ...
    mono_mb_add_local(
        mb, m_class_get_byval_arg(mono_class_get_interop_proxy_class()));
...
void mono_cominterop_emit_object_to_ptr_conv(MonoMethodBuilder *mb,
                                             MonoType *type,
                                             MonoMarshalConv conv,
                                             MonoMarshalSpec *mspec) {
#ifndef DISABLE_JIT
  ...
  case MONO_MARSHAL_CONV_OBJECT_IUNKNOWN: {
    ...
    mono_mb_emit_ldflda(mb,
                        MONO_STRUCT_OFFSET(MonoComInteropProxy, com_object));
    mono_mb_emit_byte(mb, CEE_LDIND_REF);
    ...
    if (conv == MONO_MARSHAL_CONV_OBJECT_INTERFACE) {
      ...
      iunknown = mono_class_get_property_from_name_internal(
          mono_class_get_com_object_class(), "IUnknown");
      ...
      idispatch = mono_class_get_property_from_name_internal(
          mono_class_get_com_object_class(), "IDispatch");
 ...
 */
static MonoMethod *cominterop_get_native_wrapper_adjusted(MonoMethod *method) {
  MonoMethod *res;
  ...
  mb_native =
      mono_mb_new(method->klass, method->name, MONO_WRAPPER_MANAGED_TO_NATIVE);
  ...
  mono_marshal_emit_native_wrapper(m_class_get_image(method->klass), mb_native,
                                   sig_native, piinfo, mspecs, piinfo->addr,
                                   FALSE, TRUE, FALSE, FALSE);

  res = mono_mb_create_method(mb_native, sig_native,
                              sig_native->param_count + 16);
...
MonoMethod *mono_cominterop_get_native_wrapper(MonoMethod *method) {
  ...
  cache = mono_marshal_get_cache(
      &mono_method_get_wrapper_cache(method)->cominterop_wrapper_cache,
  ...
  g_assert(!mono_class_has_failure(
      method->klass)); /*FIXME do proper error handling*/
  ...
  if (MONO_CLASS_IS_IMPORT(method->klass)) {
    ...
    if (!strcmp(method->name, ".ctor")) {
      ...
      ERROR_DECL(error);
      ctor = mono_class_get_method_from_name_checked(
          mono_class_get_com_object_class(), ".ctor", 0, 0, error);
      mono_error_assert_ok(error);
      ...
      gboolean preserve_sig =
          method->iflags & METHOD_IMPL_ATTRIBUTE_PRESERVE_SIG;
      ...
      if (!preserve_sig) {
        ...
        ERROR_DECL(error);
        ThrowExceptionForHR = mono_class_get_method_from_name_checked(
            mono_defaults.marshal_class, "ThrowExceptionForHR", 1, 0, error);
        mono_error_assert_ok(error);
  ...
  csig =
      mono_metadata_signature_dup_full(m_class_get_image(method->klass), sig);
  csig->pinvoke = 0;
  res =
      mono_mb_create_and_cache(cache, method, mb, csig, csig->param_count + 16);
  mono_mb_free(mb);
  return res;
}
...
MonoMethod *mono_cominterop_get_invoke(MonoMethod *method) {
  ...
  cache = mono_marshal_get_cache(
      &mono_method_get_wrapper_cache(method)->cominterop_invoke_cache,
      mono_aligned_addr_hash, NULL);
  ...
  if (!strcmp(method->name, ".ctor")) {
    ...
    ERROR_DECL(error);
    cache_proxy = mono_class_get_method_from_name_checked(
        mono_class_get_interop_proxy_class(), "CacheProxy", 0, 0, error);
    mono_error_assert_ok(error);
...
static MonoMethod *mono_get_addref(void) {
  MONO_STATIC_POINTER_INIT(MonoMethod, AddRef)
  ERROR_DECL(error);
  AddRef = mono_class_get_method_from_name_checked(mono_defaults.marshal_class,
                                                   "AddRef", 1, 0, error);
  mono_error_assert_ok(error);
...
int mono_cominterop_emit_marshal_com_interface(
    EmitMarshalContext *m, int argnum, MonoType *t, MonoMarshalSpec *spec,
    int conv_arg, MonoType **conv_arg_type, MarshalAction action) {
  MonoMethodBuilder *mb = m->mb;
  MonoClass *klass = t->data.klass;
  ERROR_DECL(error);

  MONO_STATIC_POINTER_INIT(MonoMethod, get_object_for_iunknown)
  get_object_for_iunknown = mono_class_get_method_from_name_checked(
      mono_defaults.marshal_class, "GetObjectForIUnknown", 1, 0, error);
  mono_error_assert_ok(error);
  ...
  get_iunknown_for_object_internal = mono_class_get_method_from_name_checked(
      mono_defaults.marshal_class, "GetIUnknownForObjectInternal", 1, 0, error);
  mono_error_assert_ok(error);
  ...
  get_idispatch_for_object_internal = mono_class_get_method_from_name_checked(
      mono_defaults.marshal_class, "GetIDispatchForObjectInternal", 1, 0,
      error);
  mono_error_assert_ok(error);
  ...
  get_com_interface_for_object_internal =
      mono_class_get_method_from_name_checked(
          mono_defaults.marshal_class, "GetComInterfaceForObjectInternal", 2, 0,
          error);
  mono_error_assert_ok(error);
  MONO_STATIC_POINTER_INIT_END(MonoMethod,
                               get_com_interface_for_object_internal)

  MONO_STATIC_POINTER_INIT(MonoMethod, marshal_release)
  marshal_release = mono_class_get_method_from_name_checked(
      mono_defaults.marshal_class, "Release", 1, 0, error);
  mono_error_assert_ok(error);
  ...
  case MARSHAL_ACTION_CONV_IN: {
    ...
    *conv_arg_type = int_type;
    ...
    if (klass && klass != mono_defaults.object_class) {
      ...
      mono_mb_emit_icall(mb, cominterop_type_from_handle);
      mono_mb_emit_managed_call(mb, get_com_interface_for_object_internal,
                                NULL);
...
int ves_icall_System_Runtime_InteropServices_Marshal_AddRefInternal(
    MonoIUnknown *pUnk) {
  return mono_IUnknown_AddRef(pUnk);
}

int ves_icall_System_Runtime_InteropServices_Marshal_QueryInterfaceInternal(
    MonoIUnknown *pUnk, gconstpointer riid, gpointer *ppv) {
  return mono_IUnknown_QueryInterface(pUnk, riid, ppv);
}

int ves_icall_System_Runtime_InteropServices_Marshal_ReleaseInternal(
    MonoIUnknown *pUnk) {
  g_assert(pUnk);
...
ves_icall_System_Runtime_InteropServices_Marshal_GetIUnknownForObjectInternal(
    MonoObjectHandle object, MonoError *error) {
...
ves_icall_System_Runtime_InteropServices_Marshal_GetObjectForCCW(
    void *pUnk, MonoError *error) {
#ifndef DISABLE_COM
  /* see if it is a CCW */
  return pUnk ? cominterop_get_ccw_handle((MonoCCWInterface *)pUnk, TRUE)
              : NULL_HANDLE;
#else
  g_assert_not_reached();
#endif
...
ves_icall_System_Runtime_InteropServices_Marshal_GetIDispatchForObjectInternal(
  ...
  if (cominterop_object_is_rcw_handle(object, &real_proxy)) {
    MonoComInteropProxyHandle com_interop_proxy =
        MONO_HANDLE_CAST(MonoComInteropProxy, real_proxy);
    MonoComObjectHandle com_object =
        MONO_HANDLE_NEW_GET(MonoComObject, com_interop_proxy, com_object);
    return cominterop_get_interface_checked(
        com_object, mono_class_get_idispatch_class(), error);
  } else if (!cominterop_can_support_dispatch(mono_handle_class(object))) {
    cominterop_set_hr_error(error, MONO_E_NOINTERFACE);
    return NULL;
  }
  return cominterop_get_ccw_checked(object, mono_class_get_idispatch_class(),
                                    error);
#else
  g_assert_not_reached();
#endif
...
void *ves_icall_System_Runtime_InteropServices_Marshal_GetCCW(
    MonoObjectHandle object, MonoReflectionTypeHandle ref_type,
    MonoError *error) {
#ifndef DISABLE_COM
  ...
  MonoCustomAttrInfo *cinfo =
      mono_custom_attrs_from_class_checked(klass, error);
  mono_error_assert_ok(error);
  if (cinfo) {
    MonoReflectionComDefaultInterfaceAttribute *attr =
        (MonoReflectionComDefaultInterfaceAttribute *)
            mono_custom_attrs_get_attr_checked(
                cinfo, mono_class_get_com_default_interface_attribute_class(),
                error);
    mono_error_assert_ok(error); /*FIXME proper error handling*/
...
MonoBoolean ves_icall_System_Runtime_InteropServices_Marshal_IsComObject(
    MonoObjectHandle object, MonoError *error) {
#ifndef DISABLE_COM
  MonoRealProxyHandle real_proxy;
...
ves_icall_System_Runtime_InteropServices_Marshal_ReleaseComObjectInternal(
  ...
  if (ref_count == 0)
    mono_System_ComObject_ReleaseInterfaces(
        MONO_HANDLE_NEW_GET(MonoComObject, proxy, com_object));
...
ves_icall_System_Runtime_InteropServices_Marshal_GetComSlotForMethodInfoInternal(
    MonoReflectionMethodHandle m, MonoError *error) {
#ifndef DISABLE_COM
  int const slot =
      cominterop_get_com_slot_for_method(MONO_HANDLE_GETVAL(m, method), error);
  mono_error_assert_ok(error);
  return slot;
#else
  g_assert_not_reached();
...
MonoObjectHandle
ves_icall_System_ComObject_CreateRCW(MonoReflectionTypeHandle ref_type,
                                     MonoError *error) {
...
static gboolean cominterop_rcw_interface_finalizer(gpointer key, gpointer value,
                                                   gpointer user_data) {
  mono_IUnknown_Release((MonoIUnknown *)value);
  return TRUE;
}

void mono_System_ComObject_ReleaseInterfaces(MonoComObjectHandle obj) {
  g_assert(!MONO_HANDLE_IS_NULL(obj));
  if (!MONO_HANDLE_GETVAL(obj, itf_hash))
    return;

  mono_cominterop_lock();
  guint32 const gchandle = GPOINTER_TO_UINT(
      g_hash_table_lookup(rcw_hash, MONO_HANDLE_GETVAL(obj, iunknown)));
  if (gchandle) {
    mono_gchandle_free_internal(gchandle);
    g_hash_table_remove(rcw_hash, MONO_HANDLE_GETVAL(obj, iunknown));
  }

  g_hash_table_foreach_remove(MONO_HANDLE_GETVAL(obj, itf_hash),
...
void ves_icall_System_ComObject_ReleaseInterfaces(MonoComObjectHandle obj,
                                                  MonoError *error) {
  mono_System_ComObject_ReleaseInterfaces(obj);
}

static gboolean cominterop_rcw_finalizer(gpointer key, gpointer value,
                                         gpointer user_data) {
  gchandle_t gchandle = 0;

  gchandle = GPOINTER_TO_UINT(value);
  if (gchandle) {
    MonoComInteropProxy *proxy =
        (MonoComInteropProxy *)mono_gchandle_get_target_internal(gchandle);

    if (proxy) {
      if (proxy->com_object->itf_hash) {
        g_hash_table_foreach_remove(proxy->com_object->itf_hash,
                                    cominterop_rcw_interface_finalizer, NULL);
...
gpointer ves_icall_System_ComObject_GetInterfaceInternal(
    MonoComObjectHandle obj, MonoReflectionTypeHandle ref_type,
    MonoBoolean throw_exception, MonoError *error) {
#ifndef DISABLE_COM
  MonoType *const type = MONO_HANDLE_GETVAL(ref_type, type);
  MonoClass *const klass = mono_class_from_mono_type_internal(type);
  if (!mono_class_init_checked(klass, error))
  ...
  gpointer const itf = cominterop_get_interface_checked(
      obj, klass, throw_exception ? error : error_ignored);
...
void ves_icall_Mono_Interop_ComInteropProxy_AddProxy(
    gpointer pUnk, MonoComInteropProxyHandle proxy, MonoError *error) {
#ifndef DISABLE_COM
  guint32 const gchandle = mono_gchandle_new_weakref_from_handle(
      MONO_HANDLE_CAST(MonoObject, proxy));
...
ves_icall_Mono_Interop_ComInteropProxy_FindProxy(gpointer pUnk,
  ...
  MonoComInteropProxyHandle const proxy = MONO_HANDLE_CAST(
      MonoComInteropProxy, mono_gchandle_get_target_handle(gchandle));
 ...
 */
static gchandle_t cominterop_get_ccw_gchandle(MonoCCWInterface *ccw_entry,
                                              gboolean verify) {
  /* no CCW's exist yet */
  ...
  MonoCCW *const ccw =
      verify ? (MonoCCW *)g_hash_table_lookup(ccw_interface_hash, ccw_entry)
             : ccw_entry->ccw;
  g_assert(verify || ccw);
...
static MonoDomain *cominterop_get_domain_for_appdomain(MonoAppDomain *ad_raw) {
  HANDLE_FUNCTION_ENTER();
...
static void cominterop_setup_marshal_context(EmitMarshalContext *m,
                                             MonoMethod *method) {
  MonoMethodSignature *sig, *csig;
...
static gpointer cominterop_get_ccw_checked(MonoObjectHandle object,
  ...
  ccw_list = (GList *)g_hash_table_lookup(
      ccw_hash, GINT_TO_POINTER(mono_handle_hash(object)));
  mono_cominterop_unlock();
  ...
  if (!ccw) {
    ...
    mono_cominterop_lock();
    g_hash_table_insert(ccw_hash, GINT_TO_POINTER(mono_handle_hash(object)),
                        ccw_list);
    mono_cominterop_unlock();
  ...
  if (cinfo) {
    ...
    coclass_attribute = mono_class_load_from_name(
    ...
    if (mono_custom_attrs_has_attr(cinfo, coclass_attribute)) {
      g_assert(m_class_get_interface_count(itf) &&
               m_class_get_interfaces(itf)[0]);
  ...
  if (!ccw_entry) {
    ...
    vtable = (void **)mono_image_alloc0(m_class_get_image(klass),
                                        sizeof(gpointer) *
                                            (method_count + start_slot));
    ...
    for (i = method_count - 1; i >= 0; i--) {
      ...
      for (param_index = sig->param_count; param_index >= 1; param_index--) {
        ...
        if (mspecs[mspec_index] == NULL) {
          mspecs[mspec_index] = cominterop_get_ccw_default_mspec(
              sig_adjusted->params[param_index]);
        } else {
      ...
      if (!preserve_sig && !MONO_TYPE_IS_VOID(sig->ret)) {
        if (mspecs[0] == NULL)
          mspecs[0] = cominterop_get_ccw_default_mspec(
              sig_adjusted->params[sig_adjusted->param_count - 1]);
      ...
      m.mb = mb;
      mono_marshal_emit_managed_wrapper(mb, sig_adjusted, mspecs, &m,
                                        adjust_method, 0);
      mono_cominterop_lock();
      wrapper_method =
          mono_mb_create_method(mb, m.csig, m.csig->param_count + 16);
      mono_cominterop_unlock();

      vtable[vtable_index--] =
          mono_compile_method_checked(wrapper_method, error);

      // cleanup, then error out if compile_method failed
      for (param_index = sig_adjusted->param_count; param_index >= 0;
           param_index--)
 ...
 * Runtime Callable Wrapper (RCW) for a COM object
 */
static gpointer cominterop_get_ccw(MonoObject *object_raw, MonoClass *itf) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
...
static gboolean mono_marshal_free_ccw_entry(gpointer key, gpointer value,
                                            gpointer user_data) {
 ...
 */
static gboolean mono_marshal_free_ccw_handle(MonoObjectHandle object) {
  /* no ccw's were created */
  if (!ccw_hash || g_hash_table_size(ccw_hash) == 0)
    return FALSE;

  mono_cominterop_lock();
  GList *ccw_list = (GList *)g_hash_table_lookup(
      ccw_hash, GINT_TO_POINTER(mono_handle_hash(object)));
  mono_cominterop_unlock();
  ...
  for (GList *ccw_list_item = ccw_list; ccw_list_item;) {
    ...
    mono_gchandle_target_is_null_or_equal(ccw_iter->gc_handle, object, &is_null,
                                          &is_equal);
    ...
    if (is_null) {
      MonoCCWInterface *ccw_entry = (MonoCCWInterface *)g_hash_table_lookup(
          ccw_iter->vtable_hash, mono_class_get_iunknown_class());
      gchandle_t gchandle = 0;
      if (!(ccw_entry &&
            (gchandle = cominterop_get_ccw_gchandle(ccw_entry, FALSE)) &&
            mono_gchandle_target_equal(gchandle, object)))
        destroy_ccw = FALSE;
    }
    if (destroy_ccw) {
      /* remove all interfaces */
      g_hash_table_foreach_remove(ccw_iter->vtable_hash,
                                  mono_marshal_free_ccw_entry, NULL);
...
gboolean mono_marshal_free_ccw(MonoObject *object_raw) {
  /* no ccw's were created */
  if (!ccw_hash || g_hash_table_size(ccw_hash) == 0)
    return FALSE;
...
static MonoMethod *cominterop_get_managed_wrapper_adjusted(MonoMethod *method) {
  ...
  int i;
  gboolean const preserve_sig =
      (method->iflags & METHOD_IMPL_ATTRIBUTE_PRESERVE_SIG) != 0;
  MonoType *int_type = mono_get_int_type();

  MONO_STATIC_POINTER_INIT(MonoMethod, get_hr_for_exception)

  ERROR_DECL(error);
  get_hr_for_exception = mono_class_get_method_from_name_checked(
      mono_defaults.marshal_class, "GetHRForException", -1, 0, error);
  mono_error_assert_ok(error);
  ...
  if (!preserve_sig ||
      (sig->ret && !sig->ret->byref &&
       (sig->ret->type == MONO_TYPE_U4 || sig->ret->type == MONO_TYPE_I4))) {
...
static void cominterop_mono_string_to_guid(MonoString *string, guint8 *guid) {
  ...
  int i = 0;
  static const guint8 indexes[16] = {7,  5,  3,  1,  12, 10, 17, 15,
                                     20, 22, 25, 27, 29, 31, 33, 35};

  for (i = 0; i < sizeof(indexes); i++)
    guid[i] = g_unichar_xdigit_value(chars[indexes[i]]) +
              (g_unichar_xdigit_value(chars[indexes[i] - 1]) << 4);
}

static gboolean cominterop_class_guid_equal(const guint8 *guid,
                                            MonoClass *klass) {
...
static int STDCALL cominterop_ccw_addref_impl(MonoCCWInterface *ccwe);
...
static int STDCALL cominterop_ccw_addref_impl(MonoCCWInterface *ccwe) {
  ...
  if (ref_count == 1) {
    ...
    ccw->gc_handle = mono_gchandle_from_handle(
        mono_gchandle_get_target_handle(oldhandle), FALSE);
    mono_gchandle_free_internal(oldhandle);
  }
  return ref_count;
}

static int STDCALL cominterop_ccw_release_impl(MonoCCWInterface *ccwe);
...
static int STDCALL cominterop_ccw_release_impl(MonoCCWInterface *ccwe) {
  ...
  if (ref_count == 0) {
    ...
    guint32 oldhandle = ccw->gc_handle;
    g_assert(oldhandle);
    ccw->gc_handle = mono_gchandle_new_weakref_from_handle(
        mono_gchandle_get_target_handle(oldhandle));
    mono_gchandle_free_internal(oldhandle);
  }
  return ref_count;
}
...
#ifdef HOST_WIN32
static const IID MONO_IID_IMarshal = {
    0x3, 0x0, 0x0, {0xC0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x46}};

/* All ccw objects are free threaded */
static int cominterop_ccw_getfreethreadedmarshaler(MonoCCW *ccw,
                                                   MonoObjectHandle object,
                                                   gpointer *ppv,
                                                   MonoError *error) {
  if (!ccw->free_marshaler) {
    gpointer const tunk = cominterop_get_ccw_checked(
        object, mono_class_get_iunknown_class(), error);
    return_val_if_nok(error, MONO_E_NOINTERFACE);
    int const ret = CoCreateFreeThreadedMarshaler(
        (LPUNKNOWN)tunk, (LPUNKNOWN *)&ccw->free_marshaler);
  }

  return ccw->free_marshaler ? mono_IUnknown_QueryInterface(
                                   ccw->free_marshaler, &MONO_IID_IMarshal, ppv)
                             : MONO_E_NOINTERFACE;
}
#endif

static int STDCALL cominterop_ccw_queryinterface_impl(MonoCCWInterface *ccwe,
                                                      const guint8 *riid,
                                                      gpointer *ppv);
...
static int STDCALL cominterop_ccw_queryinterface_impl(MonoCCWInterface *ccwe,
  ...
  if (cominterop_class_guid_equal(riid, mono_class_get_iunknown_class())) {
    *ppv = cominterop_get_ccw_checked(object, mono_class_get_iunknown_class(),
                                      error);
    mono_error_assert_ok(error);
  ...
  if (cominterop_class_guid_equal(riid, mono_class_get_idispatch_class())) {
    ...
    *ppv = cominterop_get_ccw_checked(object, mono_class_get_idispatch_class(),
                                      error);
    mono_error_assert_ok(error);
    /* remember to addref on QI */
    cominterop_ccw_addref((MonoCCWInterface *)*ppv);
  ...
  if (0 == memcmp(riid, &MONO_IID_IMarshal, sizeof(IID))) {
    int const res =
        cominterop_ccw_getfreethreadedmarshaler(ccw, object, ppv, error);
    mono_error_assert_ok(error);
    return res;
  }
#endif
...
static int STDCALL cominterop_ccw_get_ids_of_names_impl(
    MonoCCWInterface *ccwe, gpointer riid, gunichar2 **rgszNames,
    guint32 cNames, guint32 lcid, gint32 *rgDispId);
...
static int STDCALL cominterop_ccw_get_ids_of_names(MonoCCWInterface *ccwe,
  ...
  int result;
  MONO_ENTER_GC_UNSAFE;
  result = cominterop_ccw_get_ids_of_names_impl(ccwe, riid, rgszNames, cNames,
                                                lcid, rgDispId);
  MONO_EXIT_GC_UNSAFE;
  return result;
}
...
static int STDCALL cominterop_ccw_get_ids_of_names_impl(
  ...
  ComDispIdAttribute = mono_class_load_from_name(
      mono_defaults.corlib, "System.Runtime.InteropServices",
      "DispIdAttribute");
  ...
  for (i = 0; i < cNames; i++) {
    ...
    method = mono_class_get_method_from_name_checked(klass, methodname, -1, 0,
                                                     error);
    if (method && is_ok(error)) {
      cinfo = mono_custom_attrs_from_method_checked(method, error);
      mono_error_assert_ok(error); /* FIXME what's reasonable to do here */
      if (cinfo) {
        MonoObject *result = mono_custom_attrs_get_attr_checked(
            cinfo, ComDispIdAttribute, error);
        mono_error_assert_ok(error); /*FIXME proper error handling*/
        ;
...
typedef mono_bstr(STDCALL *SysAllocStringLenFunc)(const gunichar *str,
                                                  guint32 len);
...
typedef int(STDCALL *SafeArrayGetLBoundFunc)(gpointer psa, guint32 nDim,
                                             glong *plLbound);
typedef int(STDCALL *SafeArrayGetUBoundFunc)(gpointer psa, guint32 nDim,
                                             glong *plUbound);
typedef int(STDCALL *SafeArrayPtrOfIndexFunc)(gpointer psa, glong *rgIndices,
                                              gpointer *ppvData);
typedef int(STDCALL *SafeArrayDestroyFunc)(gpointer psa);
typedef int(STDCALL *SafeArrayPutElementFunc)(gpointer psa, glong *rgIndices,
                                              gpointer *ppvData);
typedef gpointer(STDCALL *SafeArrayCreateFunc)(int vt, guint32 cDims,
...
static gboolean init_com_provider_ms(void) {
  ...
  error_msg = mono_dl_symbol(module, "SysAllocStringLen",
                             (gpointer *)&sys_alloc_string_len_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SysAllocStringLen", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }

  error_msg =
      mono_dl_symbol(module, "SysStringLen", (gpointer *)&sys_string_len_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SysStringLen", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }

  error_msg =
      mono_dl_symbol(module, "SysFreeString", (gpointer *)&sys_free_string_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SysFreeString", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }

  error_msg = mono_dl_symbol(module, "SafeArrayGetDim",
                             (gpointer *)&safe_array_get_dim_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SafeArrayGetDim", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }

  error_msg = mono_dl_symbol(module, "SafeArrayGetLBound",
                             (gpointer *)&safe_array_get_lbound_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SafeArrayGetLBound", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }

  error_msg = mono_dl_symbol(module, "SafeArrayGetUBound",
                             (gpointer *)&safe_array_get_ubound_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SafeArrayGetUBound", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }

  error_msg = mono_dl_symbol(module, "SafeArrayPtrOfIndex",
                             (gpointer *)&safe_array_ptr_of_index_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SafeArrayPtrOfIndex", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }

  error_msg = mono_dl_symbol(module, "SafeArrayDestroy",
                             (gpointer *)&safe_array_destroy_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SafeArrayDestroy", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }

  error_msg = mono_dl_symbol(module, "SafeArrayPutElement",
                             (gpointer *)&safe_array_put_element_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SafeArrayPutElement", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }

  error_msg = mono_dl_symbol(module, "SafeArrayCreate",
                             (gpointer *)&safe_array_create_ms);
  if (error_msg) {
    g_warning("Error loading entry point '%s' in COM support library '%s': %s",
              "SafeArrayCreate", scope, error_msg);
    g_assert_not_reached();
    return FALSE;
  }
...
mono_bstr mono_ptr_to_bstr(const gunichar2 *ptr, int slen) {
  ...
  if (com_provider == MONO_COM_DEFAULT) {
    ...
    guint32 *const ret =
        (guint32 *)g_malloc((slen + 1) * sizeof(gunichar2) + sizeof(guint32));
    if (ret == NULL)
      return NULL;
    ...
    *ret = slen * sizeof(gunichar2);
    ...
    gunichar *const str =
        ptr ? g_utf16_to_ucs4(ptr, len, NULL, NULL, NULL) : NULL;
...
static MonoStringHandle mono_string_from_bstr_checked(mono_bstr_const bstr,
                                                      MonoError *error) {
  if (!bstr)
    return NULL_HANDLE_STRING;
#ifdef HOST_WIN32
  return mono_string_new_utf16_handle(mono_domain_get(), bstr,
                                      SysStringLen((BSTR)bstr), error);
#else
#ifndef DISABLE_COM
  if (com_provider == MONO_COM_DEFAULT)
#endif
    return mono_string_new_utf16_handle(
        mono_domain_get(), bstr, *((guint32 *)bstr - 1) / sizeof(gunichar2),
        error);
#ifndef DISABLE_COM
  else if (com_provider == MONO_COM_MS && init_com_provider_ms()) {
    glong written = 0;
    // FIXME mono_string_new_utf32_handle to combine g_ucs4_to_utf16 and
    // mono_string_new_utf16_handle.
    gunichar2 *utf16 = g_ucs4_to_utf16(
        (const gunichar *)bstr, sys_string_len_ms(bstr), NULL, &written, NULL);
    MonoStringHandle res =
        mono_string_new_utf16_handle(mono_domain_get(), utf16, written, error);
    g_free(utf16);
    return res;
  } else {
    g_assert_not_reached();
...
MonoString *mono_string_from_bstr(/*mono_bstr_const*/ gpointer bstr) {
  // FIXME gcmode
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MonoStringHandle result =
      mono_string_from_bstr_checked((mono_bstr_const)bstr, error);
  mono_error_cleanup(error);
  HANDLE_FUNCTION_RETURN_OBJ(result);
}

MonoStringHandle mono_string_from_bstr_icall_impl(mono_bstr_const bstr,
                                                  MonoError *error) {
...
G_GNUC_UNUSED
static MonoMethod *mono_get_Marshal_GetObjectForNativeVariant(void) {
  MONO_STATIC_POINTER_INIT(MonoMethod, get_object_for_native_variant)
  ERROR_DECL(error);
  get_object_for_native_variant = mono_class_get_method_from_name_checked(
      mono_defaults.marshal_class, "GetObjectForNativeVariant", 1, 0, error);
  mono_error_assert_ok(error);
...
G_GNUC_UNUSED
static MonoMethod *mono_get_Marshal_GetNativeVariantForObject(void) {
  MONO_STATIC_POINTER_INIT(MonoMethod, get_native_variant_for_object)

  ERROR_DECL(error);
  get_native_variant_for_object = mono_class_get_method_from_name_checked(
      mono_defaults.marshal_class, "GetNativeVariantForObject", 2, 0, error);
  mono_error_assert_ok(error);
...
G_GNUC_UNUSED
static MonoMethod *mono_get_Array_SetValueImpl(void) {
  MONO_STATIC_POINTER_INIT(MonoMethod, set_value_impl)

  ERROR_DECL(error);
  set_value_impl = mono_class_get_method_from_name_checked(
      mono_defaults.array_class, "SetValueImpl", 2, 0, error);
  mono_error_assert_ok(error);
...
G_GNUC_UNUSED
static MonoMethod *mono_get_Variant_Clear(void) {
  MONO_STATIC_POINTER_INIT(MonoMethod, variant_clear)
  ERROR_DECL(error);
  variant_clear = mono_class_get_method_from_name_checked(
      mono_class_get_variant_class(), "Clear", 0, 0, error);
  mono_error_assert_ok(error);
...
/* SAFEARRAY marshalling */
int mono_cominterop_emit_marshal_safearray(EmitMarshalContext *m, int argnum,
  ...
  switch (action) {
  case MARSHAL_ACTION_CONV_IN: {
    if ((t->attrs & (PARAM_ATTRIBUTE_IN | PARAM_ATTRIBUTE_OUT)) ==
        PARAM_ATTRIBUTE_OUT)
      break;
    ...
    /* Generates IL code for the following algorithm:
                    ...
                    int empty;      // empty_var
                    if (mono_marshal_safearray_create (array, out safearray, out
       indices, out empty)) { if (!empty) { int index=0; // index_var do { //
       label3 variant elem = Marshal.GetNativeVariantForObject
       (array.GetValueImpl(index)); mono_marshal_safearray_set_value (safearray,
       indices, elem);
                                            ++index;
                                    }
                                    while (mono_marshal_safearray_next
       (safearray, indices)); } // label2 mono_marshal_safearray_free_indices
       (indices); } // label1
    */
    ...
    ERROR_DECL(error);
    get_value_impl = mono_class_get_method_from_name_checked(
        mono_defaults.array_class, "GetValueImpl", 1, 0, error);
    mono_error_assert_ok(error);
    ...
    elem_var = mono_mb_add_local(
        mb, m_class_get_byval_arg(mono_class_get_variant_class()));
    mono_mb_emit_ldloc_addr(mb, elem_var);

    mono_mb_emit_managed_call(mb, mono_get_Marshal_GetNativeVariantForObject(),
                              NULL);
  ...
  case MARSHAL_ACTION_CONV_OUT: {
    if (t->attrs & PARAM_ATTRIBUTE_OUT) {
      /* Generates IL code for the following algorithm:
                  ...
                  if (mono_marshal_safearray_begin(safearray, out result, out
         indices, out empty, parameter, byValue)) { if (!empty) { int index=0;
         // index_var do { // label3 if (!byValue || (index < parameter.Length))
         { object elem =
         Variant.GetObjectForNativeVariant(mono_marshal_safearray_get_value(safearray,
         indices)); result.SetValueImpl(elem, index);
                                          }
                                          ++index;
                                  }
                                  while (mono_marshal_safearray_next(safearray,
         indices)); } // label2 mono_marshal_safearray_end(safearray, indices);
                  } // label1
                  if (!byValue)
                          return result;
      */
      ...
      mono_mb_emit_managed_call(
          mb, mono_get_Marshal_GetObjectForNativeVariant(), NULL);
  ...
  case MARSHAL_ACTION_MANAGED_CONV_IN: {
    ...
    /* Generates IL code for the following algorithm:
                    ...
                    int empty;      // empty_var
                    if (mono_marshal_safearray_begin(safearray, out result, out
       indices, out empty, NULL, TRUE)) { if (!empty) { int index=0; //
       index_var do { // label3 object elem =
       Variant.GetObjectForNativeVariant(mono_marshal_safearray_get_value(safearray,
       indices)); result.SetValueImpl(elem, index);
                                            ++index;
                                    }
                                    while
       (mono_marshal_safearray_next(safearray, indices)); } // label2
    ...
    */
    ...
    mono_mb_emit_managed_call(mb, mono_get_Marshal_GetObjectForNativeVariant(),
                              NULL);
    mono_mb_emit_stloc(mb, elem_var);
...
static guint32 mono_marshal_win_safearray_get_dim(gpointer safearray) {
  return SafeArrayGetDim((SAFEARRAY *)safearray);
}
#endif /* G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT |                     \
          HAVE_UWP_WINAPI_SUPPORT) */

static guint32 mono_marshal_safearray_get_dim(gpointer safearray) {
...
#ifdef HOST_WIN32
#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT | HAVE_UWP_WINAPI_SUPPORT)
static int mono_marshal_win_safe_array_get_lbound(gpointer psa, guint nDim,
                                                  glong *plLbound) {
...
static int mono_marshal_safe_array_get_lbound(gpointer psa, guint nDim,
                                              glong *plLbound) {
...
#ifdef HOST_WIN32
#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT | HAVE_UWP_WINAPI_SUPPORT)
static int mono_marshal_win_safe_array_get_ubound(gpointer psa, guint nDim,
                                                  glong *plUbound) {
...
static int mono_marshal_safe_array_get_ubound(gpointer psa, guint nDim,
                                              glong *plUbound) {
  ...
  if (safearray != NULL) {
    ...
    if (dim > 0) {
      ...
      *indices = g_malloc(dim * sizeof(int));
      ...
      if (allocateNewArray) {
        aklass = mono_class_create_bounded_array(mono_defaults.object_class,
                                                 dim, bounded);
...
#ifdef HOST_WIN32
#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT | HAVE_UWP_WINAPI_SUPPORT)
static int mono_marshal_win_safearray_get_value(gpointer safearray,
                                                gpointer indices,
                                                gpointer *result) {
...
#ifdef HOST_WIN32
#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT | HAVE_UWP_WINAPI_SUPPORT)
static void mono_marshal_win_safearray_end(gpointer safearray,
                                           gpointer indices) {
  g_free(indices);
...
#ifdef HOST_WIN32
#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT | HAVE_UWP_WINAPI_SUPPORT)
static gboolean mono_marshal_win_safearray_create_internal(
    UINT cDims, SAFEARRAYBOUND *rgsabound, gpointer *newsafearray) {
...
static gboolean
mono_marshal_safearray_create_internal(UINT cDims, SAFEARRAYBOUND *rgsabound,
                                       gpointer *newsafearray) {
  return mono_marshal_win_safearray_create_internal(cDims, rgsabound,
                                                    newsafearray);
}
...
#ifdef HOST_WIN32
#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT | HAVE_UWP_WINAPI_SUPPORT)
static int mono_marshal_win_safearray_set_value(gpointer safearray,
                                                gpointer indices,
                                                gpointer value) {
...
#ifdef HOST_WIN32
  int const hr =
      mono_marshal_win_safearray_set_value(safearray, indices, value);
#else
  int hr = 0;
  if (com_provider == MONO_COM_MS && init_com_provider_ms())
...
gboolean mono_marshal_free_ccw(MonoObject *object) { return FALSE; }
...
ves_icall_System_Runtime_InteropServices_Marshal_PtrToStringBSTR(
    mono_bstr_const ptr, MonoError *error) {
  if (ptr == NULL) {
...
mono_bstr ves_icall_System_Runtime_InteropServices_Marshal_BufferToBSTR(
    const gunichar2 *ptr, int len) {
  return mono_ptr_to_bstr(ptr, len);
}

void ves_icall_System_Runtime_InteropServices_Marshal_FreeBSTR(
    mono_bstr_const ptr) {
  mono_free_bstr((gpointer)ptr);
}

void *mono_cominterop_get_com_interface(MonoObject *object_raw, MonoClass *ic,
                                        MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MONO_HANDLE_DCL(MonoObject, object);
  void *const result =
      mono_cominterop_get_com_interface_internal(FALSE, object, ic, error);
  HANDLE_FUNCTION_RETURN_VAL(result);
}
  ...
  if (cinfo) {
    for (i = 0; i < cinfo->num_attrs; ++i) {
      ...
      if (mono_class_has_parent(
              ctor_class, mono_class_get_interface_type_attribute_class())) {
        ret = TRUE;
        break;
      }
    }
