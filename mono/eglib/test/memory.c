...
static RESULT test_memory_zero_size_allocations(void) {
  gpointer p;
 ...
 *     <> ALIGN_PTR_TO
 */
static RESULT test_align_signed_int(void) {
  ...
  if (result != exp_value)
    return FAILED(
        "Expected value after aligned is %d, however, the actual value is %d",
        exp_value, result);
  ...
  if (result_ptr != exp_ptr)
    return FAILED("Expected address after aligned is %p, however, the actual "
                  "address is %p",
                  exp_ptr, result_ptr);
 ...
 *     <> ALIGN_PTR_TO
 */
static RESULT test_align_unsigned_int(void) {
  gssize orig_value = 67;
  gssize result, exp_value;
  gpointer orig_ptr = (gpointer)orig_value;
  gpointer result_ptr, exp_ptr;
