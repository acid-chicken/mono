...
// Ask stdint.h and inttypes.h for the full C99 features for CentOS 6 g++ 4.4,
// Android, etc. See for example:
...
#define G_ENUM_BINOP(Enum, op, opeq)                                           \
  inline Enum operator op(Enum a, Enum b) {                                    \
    ...
    return static_cast<Enum>(static_cast<type>(a) op b);                       \
                                                                               ...
                                                                               \
  inline Enum &operator opeq(Enum &a, Enum b) {                                \
...
#define ALIGN_TO(val, align)                                                   \
  ((((gssize)val) + (gssize)((align)-1)) & (~((gssize)(align - 1))))

#define ALIGN_DOWN_TO(val, align) (((gssize)val) & (~((gssize)(align - 1))))

#define ALIGN_PTR_TO(ptr, align)                                               \
  (gpointer)((((gssize)(ptr)) + (gssize)(align - 1)) & (~((gssize)(align - 1))))
...
#define G_ATTR_FORMAT_PRINTF(fmt_pos, arg_pos)                                 \
  __attribute__((__format__(__printf__, fmt_pos, arg_pos)))
#else
#define G_ATTR_FORMAT_PRINTF(fmt_pos, arg_pos)
#endif
...
#define g_newa0(type, size)                                                    \
  ((type *)memset(alloca(sizeof(type) * (size)), 0, sizeof(type) * (size)))

#define g_memmove(dest, src, len) memmove(dest, src, len)
#define g_renew(struct_type, mem, n_structs)                                   \
  ((struct_type *)g_realloc(mem, sizeof(struct_type) * n_structs))
 ...
 * Precondition macros
 */
#define g_warn_if_fail(x)                                                      \
  G_STMT_START {                                                               \
    if (!(x)) {                                                                \
      g_warning("%s:%d: assertion '%s' failed\n", __FILE__, __LINE__, #x);     \
    }                                                                          \
  }                                                                            \
  G_STMT_END
#define g_return_if_fail(x)                                                    \
  G_STMT_START {                                                               \
    if (!(x)) {                                                                \
      g_critical("%s:%d: assertion '%s' failed\n", __FILE__, __LINE__, #x);    \
      return;                                                                  \
    }                                                                          \
  }                                                                            \
  G_STMT_END
#define g_return_val_if_fail(x, e)                                             \
  G_STMT_START {                                                               \
    if (!(x)) {                                                                \
      g_critical("%s:%d: assertion '%s' failed\n", __FILE__, __LINE__, #x);    \
      return (e);                                                              \
...
G_EXTERN_C // Used by libtest, at least.
    gchar *
    g_strdup_printf(const gchar *format, ...) G_ATTR_FORMAT_PRINTF(1, 2);
...
gchar **g_strsplit(const gchar *string, const gchar *delimiter,
                   gint max_tokens);
gchar **g_strsplit_set(const gchar *string, const gchar *delimiter,
                       gint max_tokens);
...
gboolean g_utf16_ascii_equal(const gunichar2 *utf16, size_t ulen,
                             const char *ascii, size_t alen);
gboolean g_utf16_asciiz_equal(const gunichar2 *utf16, const char *ascii);

static inline gboolean g_ascii_equal(const char *s1, gsize len1, const char *s2,
                                     gsize len2) {
  return len1 == len2 && (s1 == s2 || memcmp(s1, s2, len1) == 0);
}

static inline gboolean g_asciiz_equal(const char *s1, const char *s2) {
  return s1 == s2 || strcmp(s1, s2) == 0;
}

static inline gboolean g_ascii_equal_caseinsensitive(const char *s1, gsize len1,
                                                     const char *s2,
                                                     gsize len2) {
  return len1 == len2 && (s1 == s2 || g_ascii_strncasecmp(s1, s2, len1) == 0);
}

static inline gboolean g_asciiz_equal_caseinsensitive(const char *s1,
                                                      const char *s2) {
...
GList *g_list_sort(GList *sort, GCompareFunc func);
...
G_EXTERN_C // Used by MonoPosixHelper or MonoSupportW, at least.
    void
    g_hash_table_insert_replace(GHashTable *hash, gpointer key, gpointer value,
                                gboolean replace);
...
gpointer g_hash_table_find(GHashTable *hash, GHRFunc predicate,
                           gpointer user_data);
G_EXTERN_C // Used by MonoPosixHelper or MonoSupportW, at least.
    gboolean
...
guint g_hash_table_foreach_remove(GHashTable *hash, GHRFunc func,
                                  gpointer user_data);
guint g_hash_table_foreach_steal(GHashTable *hash, GHRFunc func,
                                 gpointer user_data);
G_EXTERN_C // Used by MonoPosixHelper or MonoSupportW, at least.
    void
...
#define g_hash_table_replace(h, k, v)                                          \
...
void g_qsort_with_data(gpointer base, size_t nmemb, size_t size,
...
G_EXTERN_C // Used by MonoPosixHelper or MonoSupportW, at least.
    void
    g_log(const gchar *log_domain, GLogLevelFlags log_level,
          const gchar *format, ...);
G_EXTERN_C // Used by MonoPosixHelper or MonoSupportW, at least.
    void
    g_assertion_message(const gchar *format, ...) G_GNUC_NORETURN;
void mono_assertion_message_disabled(const char *file,
                                     int line) G_GNUC_NORETURN;
void mono_assertion_message(const char *file, int line,
                            const char *condition) G_GNUC_NORETURN;
void mono_assertion_message_unreachable(const char *file,
                                        int line) G_GNUC_NORETURN;
...
#define g_error(format, ...)                                                   \
  do {                                                                         \
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_ERROR, format, __VA_ARGS__);               \
    for (;;)                                                                   \
      ;                                                                        \
  } while (0)
#define g_critical(format, ...)                                                \
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, format, __VA_ARGS__)
#define g_warning(format, ...)                                                 \
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_WARNING, format, __VA_ARGS__)
#define g_message(format, ...)                                                 \
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_MESSAGE, format, __VA_ARGS__)
...
/* g_assert is a boolean expression; the precise value is not preserved, just
 * true or false. */
#ifdef DISABLE_ASSERT_MESSAGES
// This is smaller than the equivalent mono_assertion_message (..."disabled");
#define g_assert(x)                                                            \
  (G_LIKELY((x)) ? 1 : (mono_assertion_message_disabled(__FILE__, __LINE__), 0))
#else
#define g_assert(x)                                                            \
  (G_LIKELY((x)) ? 1 : (mono_assertion_message(__FILE__, __LINE__, #x), 0))
#endif
...
#define g_assert_not_reached()                                                 \
  G_STMT_START {                                                               \
    mono_assertion_message_unreachable(__FILE__, __LINE__);                    \
    eg_unreachable();                                                          \
  }                                                                            \
  G_STMT_END
...
#define g_assert_not_netcore()                                                 \
  g_assert(!"This function should only be called on mono-notnetcore.")
#else
#define g_assert_netcore()                                                     \
  g_assert(!"This function should only be called on mono-netcore.")
 ...
 *
 * g_assertf is a boolean expression -- the precise value is not preserved, just
 * true or false.
 *
...
#define g_assertf(x, format, ...)                                              \
  (G_LIKELY((x))                                                               \
       ...
       : (g_assertion_message("* Assertion at %s:%d, condition `%s' not met, " \
                              "function:%s, " format "\n",                     \
                              __FILE__, __LINE__, #x, __func__, __VA_ARGS__),  \
          0))
#else
#define g_assertf(x, format, ...)                                              \
  (G_LIKELY((x)) ? 1                                                           \
...
gunichar *g_utf8_to_ucs4(const gchar *str, glong len, glong *items_read,
                         glong *items_written, GError **err);
G_EXTERN_C // Used by libtest, at least.
    gunichar2 *
    g_utf8_to_utf16(const gchar *str, glong len, glong *items_read,
                    glong *items_written, GError **err);
gunichar2 *eg_utf8_to_utf16_with_nuls(const gchar *str, glong len,
                                      glong *items_read, glong *items_written,
                                      GError **err);
gunichar2 *eg_wtf8_to_utf16(const gchar *str, glong len, glong *items_read,
                            glong *items_written, GError **err);
G_EXTERN_C // Used by libtest, at least.
    gchar *
    g_utf16_to_utf8(const gunichar2 *str, glong len, glong *items_read,
                    glong *items_written, GError **err);
...
#ifdef G_OS_WIN32
#define u16to8(str)                                                            \
  g_utf16_to_utf8((gunichar2 *)(str), (glong)wcslen((wchar_t *)(str)), NULL,   \
                  NULL, NULL)
#else
...
gboolean g_spawn_async_with_pipes(const gchar *working_directory, gchar **argv,
                                  ...
                                  gint *standard_error, GError **gerror);
...
static inline int g_async_safe_fgets(char *str, int num, int handle,
                                     gboolean *newline) {
  memset(str, 0, num);
...
static inline gint g_async_safe_vfprintf(int handle, gchar const *format,
                                         va_list args) {
...
static inline gint g_async_safe_fprintf(int handle, gchar const *format, ...) {
  va_list args;
...
static inline gint g_async_safe_vprintf(gchar const *format, va_list args) {
  return g_async_safe_vfprintf(1, format, args);
}

static inline gint g_async_safe_printf(gchar const *format, ...) {
  va_list args;
  va_start(args, format);
...
typedef struct {
  ...
  void (*passthrough)(GMarkupParseContext *context,
                      ...
                      gpointer user_data, GError **gerror);
  void (*error)(GMarkupParseContext *context, GError *gerror,
                gpointer user_data);
} GMarkupParser;
...
gsize g_iconv(GIConv cd, gchar **inbytes, gsize *inbytesleft, gchar **outbytes,
              gsize *outbytesleft);
...
#define GUINT16_SWAP_LE_BE_CONSTANT(x)                                         \
  ((((guint16)x) >> 8) | ((((guint16)x) << 8)))

#define GUINT16_SWAP_LE_BE(x)                                                  \
  ((guint16)(((guint16)x) >> 8) | ((((guint16)(x)) & 0xff) << 8))
#define GUINT32_SWAP_LE_BE(x)                                                  \
  ((guint32)((((guint32)(x)) << 24) | ((((guint32)(x)) & 0xff0000) >> 8) |     \
             ((((guint32)(x)) & 0xff00) << 8) | (((guint32)(x)) >> 24)))

#define GUINT64_SWAP_LE_BE(x)                                                  \
  ((guint64)(((guint64)(GUINT32_SWAP_LE_BE(((guint64)x) & 0xffffffff)))        \
             << 32) |                                                          \
...
#define GLIB_CHECK_VERSION(a, b, c)                                            \
  ((a < _EGLIB_MAJOR) ||                                                       \
   (a == _EGLIB_MAJOR &&                                                       \
    (b < _EGLIB_MIDDLE || (b == _EGLIB_MIDDLE && c <= _EGLIB_MINOR))))

#define G_HAVE_API_SUPPORT(x) (x)
#define G_UNSUPPORTED_API "%s:%d: '%s' not supported.", __FILE__, __LINE__
#define g_unsupported_api(name)                                                \
  G_STMT_START { g_warning(G_UNSUPPORTED_API, name); }                         \
  G_STMT_END

#if _WIN32
// g_free the result
// No MAX_PATH limit.
gboolean mono_get_module_filename(gpointer mod, gunichar2 **pstr,
                                  guint32 *plength);

// g_free the result
// No MAX_PATH limit.
gboolean mono_get_module_filename_ex(gpointer process, gpointer mod,
                                     gunichar2 **pstr, guint32 *plength);

// g_free the result
// No MAX_PATH limit.
gboolean mono_get_module_basename(gpointer process, gpointer mod,
                                  gunichar2 **pstr, guint32 *plength);

// g_free the result
// No MAX_PATH limit.
gboolean mono_get_current_directory(gunichar2 **pstr, guint32 *plength);
#endif

G_END_DECLS // FIXME: There is more extern C than there should be.

    static inline void
    mono_qsort(void *base, size_t num, size_t size,
