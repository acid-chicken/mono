...
static int test_callspec(int test_idx, MonoMethod *method, const char *callspec,
                         gboolean expect_match) {
  int res = 0;
...
static int test_all_callspecs(void) {
  ...
  for (idx = 0; test_entries[idx].callspec; ++idx) {
    MonoMethod *meth =
        g_array_index(test_methods, MonoMethod *, test_entries[idx].method);
...
int test_mono_callspec_main(void) {
  int res = 0;
