 ...
 * Performs AND across the fields of the SIMD PSR register (wCASF) and sends the
 * result to CPSR; can be performed after a Byte, Half-word or Word operation
 * that sets the flags. NOTE: R15 is omitted from the macro declaration;
 */
...
#define _WM_TBCST_CC(_q, _cc, _wrd, _rn)                                       \
  WM_ASM(DEF_WM_TBCST_CC(_q, _cc, _wrd, _rn))
#define ARM_WM_TBCST_CC(_p, _q, _cc, _wrd, _rn)                                \
  WM_EMIT(_p, DEF_WM_TBCST_CC(_q, _cc, _wrd, _rn))
...
/* codegen */
#define ARM_WM_TBCST(_p, _q, _wrd, _rn)                                        \
  ARM_WM_TBCST_CC(_p, _q, WM_CC_AL, _wrd, _rn)
