...
#define CODE_INDENT                                                            \
  g_assert(indent_level < 512);                                                \
  indent[indent_level * 2] = ' ';                                              \
  indent[indent_level * 2 + 1] = ' ';                                          \
  ++indent_level;                                                              \
  indent[indent_level * 2] = 0;
#define CODE_UNINDENT                                                          \
  g_assert(indent_level);                                                      \
  --indent_level;                                                              \
  indent[indent_level * 2] = 0;

void disassemble_cil(MonoImage *m, MonoMethodHeader *mh,
...
#undef cl
  ...
  while (ptr < end) {
    for (i = mh->num_clauses - 1; i >= 0; --i) {
      ...
      if (ptr == start + mh->clauses[i].handler_offset) {
        if (mh->clauses[i].flags == MONO_EXCEPTION_CLAUSE_FILTER) {
          ...
          char *klass =
              mh->clauses[i].flags
                  ? g_strdup("")
                  : dis_stringify_object_with_class(
                        m, mh->clauses[i].data.catch_class, TRUE, FALSE);
      ...
      if (mh->clauses[i].flags == MONO_EXCEPTION_CLAUSE_FILTER &&
          ptr == start + mh->clauses[i].data.filter_offset) {
    ...
    for (i = 0; i < mh->num_clauses; ++i) {
      ...
      if (ptr ==
          start + mh->clauses[i].handler_offset + mh->clauses[i].handler_len) {
        CODE_UNINDENT;
