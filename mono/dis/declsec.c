...
static char *declsec_20_get_classname(const char *p, const char **rptr) {
  int apos, cpos = 0;
  char *c;
  char *a;
  char *result;
...
static gboolean declsec_20_write_type(GString *str, char type) {
  switch (type) {
...
static const char *declsec_20_write_value(GString *str, char type,
                                          const char *value) {
  switch (type) {
  case MONO_TYPE_U1:
  ...
  for (i = 0; i < num; i++) {
    ...
    for (j = 0; j < pos; j++) {
      ...
      case MONO_DECLSEC_FIELD:
        /* not sure if/how we can get this in a declarative security
         * attribute... */
      ...
      if (type == MONO_DECLSEC_ENUM) {
        ...
        g_free(s);
        p += len;
        /* TODO: we must detect the size of the enum element (from the type ?
         * length ?) note: ildasm v2 has some problem decoding them too and
         * doesn't seems to rely on the type (as the other assembly isn't
         * loaded) */
