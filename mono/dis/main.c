...
static void dump_header_data(MonoImage *img) {
  if (!dump_header_data_p)
    return;

  fprintf(output, "// Ximian's CIL disassembler, version 1.0\n"
                  "// Copyright (C) 2001 Ximian, Inc.\n\n");
}

static void dump_cattrs_list(GList *list, const char *indent) {
  GList *tmp;
...
static void dump_cattrs(MonoImage *m, guint32 token, const char *indent) {
  GList *list;
...
static const char *get_il_security_action(int val) {
  static char buf[32];
...
static void dump_declarative_security(MonoImage *m, guint32 objectType,
                                      guint32 token, const char *indent) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    if (((idx & MONO_HAS_DECL_SECURITY_MASK) == objectType) &&
        ((idx >> MONO_HAS_DECL_SECURITY_BITS) == token)) {
      char *dump;
...
static char *assembly_flags(guint32 f) {
  if (f & ASSEMBLYREF_RETARGETABLE_FLAG)
    return g_strdup("retargetable ");
  return g_strdup("");
}

static void dis_directive_assembly(MonoImage *m) {
  ...
  fprintf(output,
          ...
          "  .ver  %d:%d:%d:%d\n",
          cols[MONO_ASSEMBLY_HASH_ALG], cols[MONO_ASSEMBLY_MAJOR_VERSION],
          cols[MONO_ASSEMBLY_MINOR_VERSION], cols[MONO_ASSEMBLY_BUILD_NUMBER],
          cols[MONO_ASSEMBLY_REV_NUMBER]);
  if (cols[MONO_ASSEMBLY_CULTURE]) {
    const char *locale =
        mono_metadata_string_heap(m, cols[MONO_ASSEMBLY_CULTURE]);
    glong items_read, items_written;
    gunichar2 *render = g_utf8_to_utf16(locale, strlen(locale), &items_read,
                                        &items_written, NULL);
    char *dump = data_dump((const char *)render,
...
static void dis_directive_assemblyref(MonoImage *m) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    fprintf(output,
            ...
            "{\n"
            "  .ver %d:%d:%d:%d\n",
            flags, esc, cols[MONO_ASSEMBLYREF_MAJOR_VERSION],
    ...
    if (cols[MONO_ASSEMBLYREF_PUBLIC_KEY]) {
      const char *b =
          mono_metadata_blob_heap(m, cols[MONO_ASSEMBLYREF_PUBLIC_KEY]);
      int len = mono_metadata_decode_blob_size(b, &b);
      char *dump = data_dump(b, len, "\t\t");
...
static void dis_directive_module(MonoImage *m) {
...
static void dis_directive_moduleref(MonoImage *m) {
...
static void dis_nt_header(MonoImage *m) {
...
static void dis_directive_file(MonoImage *m) {
...
static void dis_directive_mresource(MonoImage *m) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    fprintf(output, ".mresource %s '%s'\n",
            (cols[MONO_MANIFEST_FLAGS] & MANIFEST_RESOURCE_VISIBILITY_MASK) ==
    ...
    if (impl) {
      ...
      if ((impl & MONO_IMPLEMENTATION_MASK) == MONO_IMPLEMENTATION_FILE) {
        name_token = mono_metadata_decode_row_col(&m->tables[MONO_TABLE_FILE],
                                                  idx - 1, MONO_FILE_NAME);

        fprintf(output, "    .file '%s' at 0x0\n",
                mono_metadata_string_heap(m, name_token));
      }
      if ((impl & MONO_IMPLEMENTATION_MASK) ==
          MONO_IMPLEMENTATION_ASSEMBLYREF) {
        name_token = mono_metadata_decode_row_col(
            &m->tables[MONO_TABLE_ASSEMBLYREF], idx - 1, MONO_ASSEMBLYREF_NAME);
...
static dis_map_t format_map[] = {{TYPE_ATTRIBUTE_ANSI_CLASS, "ansi "},
                                 {TYPE_ATTRIBUTE_UNICODE_CLASS, "unicode "},
                                 {TYPE_ATTRIBUTE_AUTO_CLASS, "auto "},
                                 {0, NULL}};

static char *typedef_flags(guint32 flags) {
 ...
 */
static void dis_field_list(MonoImage *m, guint32 start, guint32 end,
  ...
  for (i = start; i < end; i++) {
    ...
    esname =
        get_escaped_name(mono_metadata_string_heap(m, cols[MONO_FIELD_NAME]));
    if (cols[MONO_FIELD_FLAGS] & FIELD_ATTRIBUTE_HAS_DEFAULT) {
      char *lit;
      guint32 const_cols[MONO_CONSTANT_SIZE];
      guint32 crow;

      if ((crow = mono_metadata_get_constant_index(
               m, MONO_TOKEN_FIELD_DEF | (i + 1), 0))) {
        mono_metadata_decode_row(&m->tables[MONO_TABLE_CONSTANT], crow - 1,
                                 const_cols, MONO_CONSTANT_SIZE);
        lit = get_constant(m, (MonoTypeEnum)const_cols[MONO_CONSTANT_TYPE],
                           const_cols[MONO_CONSTANT_VALUE]);
      } else {
        lit = g_strdup("not found");
      }

      fprintf(output, "    .field %s%s%s %s = ", flags,
              marshal_str ? marshal_str : " ", sig, esname);
      fprintf(output, "%s\n", lit);
      g_free(lit);
    } else
      fprintf(output, "    .field %s %s%s%s %s%s\n", attrs ? attrs : "", flags,
              marshal_str ? marshal_str : " ", sig, esname, rva_desc);
    g_free(attrs);
    g_free(flags);
    g_free(marshal_str);
    g_free(sig);
...
static dis_map_t method_flags_map[] = {
    ...
    /* MS ilasm doesn't compile this statement - is must be added automagically
       when permissionset are present */
 ...
 */
static char *method_flags(guint32 f) {
 ...
 */
static char *pinvoke_flags(guint32 f) {
  GString *str = g_string_new("");
...
static void dis_locals(MonoImage *m, MonoMethodHeader *mh, const char *ptr) {
  int i;
...
static void dis_code(MonoImage *m, guint32 token, guint32 rva,
                     MonoGenericContainer *container) {
  ERROR_DECL(error);
  MonoMethodHeader *mh;
  ...
  entry_point = mono_image_get_entry_point(m);
  if (entry_point && mono_metadata_token_index(entry_point) &&
      mono_metadata_token_table(entry_point) == MONO_TABLE_METHOD) {
...
static char *pinvoke_info(MonoImage *m, guint32 mindex) {
  ...
  for (i = 0; i < im->rows; i++) {
    ...
    if ((im_cols[MONO_IMPLMAP_MEMBER] >> 1) == mindex + 1) {
      ...
      mono_metadata_decode_row(mr, im_cols[MONO_IMPLMAP_SCOPE] - 1, mr_cols,
                               MONO_MODULEREF_SIZE);

      scope = mono_metadata_string_heap(m, mr_cols[MONO_MODULEREF_NAME]);

      return g_strdup_printf("(\"%s\" as \"%s\" %s)", scope, import, flags);
      g_free(flags);
    }
  }
 ...
 */
static void dump_cattrs_for_type_params(MonoImage *m, guint32 token,
                                        const char *indent) {
  ...
  do {
    list = dis_get_custom_attrs(
        m, mono_metadata_make_token(MONO_TABLE_GENERICPARAM, i));
    if (list) {
...
static void dump_cattrs_for_method_params(MonoImage *m, guint32 midx,
  ...
  param_index =
      mono_metadata_decode_row_col(methodt, midx, MONO_METHOD_PARAMLIST);
  if (midx + 1 < methodt->rows)
    lastp =
        mono_metadata_decode_row_col(methodt, midx + 1, MONO_METHOD_PARAMLIST);
  else
    lastp = paramt->rows + 1;
  for (i = param_index; i < lastp; ++i) {
    ...
    if (!(param_cols[MONO_PARAM_FLAGS] & PARAM_ATTRIBUTE_HAS_DEFAULT)) {
      ...
      if ((crow = mono_metadata_get_constant_index(m, MONO_TOKEN_PARAM_DEF | i,
                                                   0))) {
        guint32 const_cols[MONO_CONSTANT_SIZE];
        mono_metadata_decode_row(&m->tables[MONO_TABLE_CONSTANT], crow - 1,
                                 const_cols, MONO_CONSTANT_SIZE);
 ...
 */
static void dis_method_list(const char *klass_name, MonoImage *m, guint32 start,
                            guint32 end, MonoGenericContainer *type_container) {
  ...
  for (i = start; i < end; i++) {
    ...
    container = mono_metadata_load_generic_params(
        m, MONO_TOKEN_METHOD_DEF | (i + 1), type_container, NULL);
    if (container) {
      ERROR_DECL(error);
      mono_metadata_load_generic_param_constraints_checked(
          m, MONO_TOKEN_METHOD_DEF | (i + 1), container, error);
      g_assert(is_ok(error)); /*FIXME don't swallow the error message*/
    } else {
      container = type_container;
    }

    ms = mono_metadata_parse_method_signature_full(m, container, i + 1, sig,
                                                   &sig, error);
    if (ms != NULL) {
...
static void dis_property_methods(MonoImage *m, guint32 prop,
                                 MonoGenericContainer *container) {
  guint start, end;
  ...
  for (; start < end; ++start) {
    ...
    if (!should_include_method(cols[MONO_METHOD_SEMA_METHOD]))
      continue;
    sig = dis_stringify_method_signature_full(
        m, NULL, cols[MONO_METHOD_SEMA_METHOD], container, TRUE, FALSE);
    fprintf(output, "\t\t%s %s\n", type[cols[MONO_METHOD_SEMA_SEMANTICS]], sig);
    g_free(sig);
  }
}
static char *dis_property_signature(MonoImage *m, guint32 prop_idx,
                                    MonoGenericContainer *container) {
  ERROR_DECL(error);
  ...
  type = mono_metadata_parse_type_checked(m, container, 0, FALSE, ptr, &ptr,
                                          error);
  if (type) {
    blurb = dis_stringify_type(m, type, TRUE);
  } else {
    blurb = g_strdup_printf("Invalid type due to %s",
  ...
  for (i = 0; i < pcount; i++) {
    if (i)
      g_string_append(res, ", ");
    param = mono_metadata_parse_type_checked(m, container, 0, FALSE, ptr, &ptr,
                                             error);
    if (type) {
...
static void dis_property_list(MonoImage *m, guint32 typedef_row,
                              MonoGenericContainer *container) {
  guint start, end, i;
...
static char *dis_event_signature(MonoImage *m, guint32 event_idx,
                                 MonoGenericContainer *container) {
  ...
  esname =
      get_escaped_name(mono_metadata_string_heap(m, cols[MONO_EVENT_NAME]));
...
static void dis_event_methods(MonoImage *m, guint32 event,
                              MonoGenericContainer *container) {
  guint start, end;
  MonoTableInfo *msemt = &m->tables[MONO_TABLE_METHODSEMANTICS];
  guint32 cols[MONO_METHOD_SEMA_SIZE];
...
static void dis_event_list(MonoImage *m, guint32 typedef_row,
                           MonoGenericContainer *container) {
  guint start, end, i;
...
static void dis_interfaces(MonoImage *m, guint32 typedef_row,
                           MonoGenericContainer *container) {
  plocator_t loc;
  guint start;
  ...
  if (!mono_binary_search(&loc, table->base, table->rows, table->row_size,
                          table_locator))
    return;
   ...
   */
  while (start > 0) {
    if (loc.idx == mono_metadata_decode_row_col(table, start - 1,
 ...
 */
static void dis_type(MonoImage *m, int n, int is_nested, int forward) {
  ...
  container = mono_metadata_load_generic_params(
      m, MONO_TOKEN_TYPE_DEF | (n + 1), NULL, NULL);
  if (container) {
    ERROR_DECL(error);
    mono_metadata_load_generic_param_constraints_checked(
        m, MONO_TOKEN_TYPE_DEF | (n + 1), container, error);
    g_assert(is_ok(error)); /*FIXME don't swallow the error message*/
  }

  esname = get_escaped_name(name);
  if ((cols[MONO_TYPEDEF_FLAGS] & TYPE_ATTRIBUTE_CLASS_SEMANTIC_MASK) ==
      TYPE_ATTRIBUTE_CLASS) {
    fprintf(output, "  .class %s%s", typedef_flags(cols[MONO_TYPEDEF_FLAGS]),
            esname);
  ...
  if (!forward) {
    ...
    if (mono_metadata_packing_from_typedef(m, n + 1, &packing_size,
                                           &class_size)) {
    ...
    if (cols[MONO_TYPEDEF_METHOD_LIST] &&
        cols[MONO_TYPEDEF_METHOD_LIST] <= m->tables[MONO_TABLE_METHOD].rows)
  ...
  while (nested) {
    dis_type(
        m,
        mono_metadata_decode_row_col(t, nested - 1, MONO_NESTED_CLASS_NESTED) -
            1,
        1, forward);
 ...
 */
static void dis_globals(MonoImage *m) {
  MonoTableInfo *t = &m->tables[MONO_TABLE_TYPEDEF];
  guint32 cols[MONO_TYPEDEF_SIZE];
  guint32 cols_next[MONO_TYPEDEF_SIZE];
  gboolean next_is_valid, last;
...
static void dis_resources_worker(MonoImage *m, gboolean just_print) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    if (!(res =
              mono_image_get_resource(m, cols[MONO_MANIFEST_OFFSET], &size))) {
      if (just_print)
    ...
    if ((fp = fopen(name, "ab"))) {
      if (ftell(fp) == 0)
        ...
        g_warning("Error creating managed resource - %s : File already exists.",
                  name);

      fclose(fp);
    } else
      g_warning("Error creating managed resource - %s : %s", name,
                g_strerror(errno));
  }
}

static void dis_mresource(MonoImage *m) { dis_resources_worker(m, FALSE); }

static void dis_presource(MonoImage *m) { dis_resources_worker(m, TRUE); }

static char *exported_type_flags(guint32 flags) {
  static char buffer[1024];
  int visibility = flags & TYPE_ATTRIBUTE_VISIBILITY_MASK;
...
static char *get_escaped_fullname(MonoImage *m, guint32 nspace_idx,
                                  guint32 name_idx) {
  const char *name, *nspace;
...
static void dis_exported_types(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    fullname = get_escaped_fullname(m, cols[MONO_EXP_TYPE_NAMESPACE],
                                    cols[MONO_EXP_TYPE_NAME]);
      ...
      case MONO_IMPLEMENTATION_FILE:
        fprintf(output, "    .file '%s'\n",
                mono_metadata_string_heap(
                    m, mono_metadata_decode_row_col(&m->tables[MONO_TABLE_FILE],
                                                    idx - 1, MONO_FILE_NAME)));
        break;
      case MONO_IMPLEMENTATION_ASSEMBLYREF:
        fprintf(
            output, "    .assembly extern '%s'\n",
            mono_metadata_string_heap(m, mono_metadata_decode_row_col(
                                             &m->tables[MONO_TABLE_ASSEMBLYREF],
                                             idx - 1, MONO_ASSEMBLYREF_NAME)));
        break;
      case MONO_IMPLEMENTATION_EXP_TYPE:
        fullname = get_escaped_fullname(
            m,
            mono_metadata_decode_row_col(&m->tables[MONO_TABLE_EXPORTEDTYPE],
                                         idx - 1, MONO_EXP_TYPE_NAMESPACE),
            mono_metadata_decode_row_col(&m->tables[MONO_TABLE_EXPORTEDTYPE],
                                         idx - 1, MONO_EXP_TYPE_NAME));
 ...
 */
static void dis_types(MonoImage *m, int forward) {
  MonoTableInfo *t = &m->tables[MONO_TABLE_TYPEDEF];
  int i;
  guint32 flags;
...
static const char *get_uninitialized_data_type(guint32 size) {
  switch (size) {
  case 1:
    return "int8";
  case 2:
 ...
 * @m: metadata context
 *
 * disassembles all data blobs references in the FieldRVA table in the @m
 * context
 */
static void dis_data(MonoImage *m) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    sig = mono_metadata_blob_heap(
        m, mono_metadata_decode_row_col(ft, cols[MONO_FIELD_RVA_FIELD] - 1,
                                        MONO_FIELD_SIGNATURE));
    mono_metadata_decode_value(sig, &sig);
    /* FIELD signature == 0x06 */
    g_assert(*sig == 0x06);
    type = mono_metadata_parse_type_checked(m, NULL, 0, FALSE, sig + 1, &sig,
                                            error);
    if (!type) {
    ...
    if (rva) {
      ...
      fprintf(output, ".data D_%08x = %s [%d]\n", cols[MONO_FIELD_RVA_RVA],
              get_uninitialized_data_type(size), size);
  }
}
 ...
 */
static int disassemble_file(const char *file) {
  ...
  if (dump_table != -1) {
    ...
    if (dump_forward_decls) {
      fprintf(output, "// *************** Forward Declarations for Classes "
                      "***************\n\n");
      dis_types(img, 1);
      fprintf(output, "// *************** End-Of Forward Declarations for "
                      "Classes ***************\n\n");
    }
    dis_types(img, 0);
    dis_data(img);
...
static gboolean table_includes(TableFilter *tf, int idx) {
  if (!tf->count)
    return FALSE;
  return mono_binary_search(&idx, tf->elems, tf->count, sizeof(int), int_cmp) !=
         NULL;
}
...
static ImageFilter *add_filter(const char *name) {
  ImageFilter *ifilter;
  GList *item;
...
static void add_item(TableFilter *tf, int val) {
  if (tf->count >= tf->size) {
    if (!tf->size) {
      tf->size = 8;
...
static void sort_filter_elems(void) {
  ImageFilter *ifilter;
  GList *item;

  for (item = filter_list; item; item = item->next) {
    ifilter = (ImageFilter *)item->data;
    mono_qsort(ifilter->types.elems, ifilter->types.count, sizeof(int),
               int_cmp);
    mono_qsort(ifilter->fields.elems, ifilter->fields.count, sizeof(int),
               int_cmp);
    mono_qsort(ifilter->methods.elems, ifilter->methods.count, sizeof(int),
               int_cmp);
  }
}

static void load_filter(const char *filename) {
  FILE *file;
  char buf[1024];
  char *p, *s, *endptr;
  int line = 0;
  ...
  while (fgets(buf, sizeof(buf), file) != NULL) {
    ...
    case 'T':
      ...
      if ((s[1] != ':') || !(value = strtol(s + 2, &endptr, 0)) ||
          (endptr == s + 2))
 ...
 */
static MonoAssembly *monodis_preload(MonoAssemblyLoadContext *alc,
                                     MonoAssemblyName *aname,
                                     gchar **assemblies_path, gboolean refonly,
...
static void monodis_assembly_load_hook(MonoAssemblyLoadContext *alc,
...
static MonoAssembly *
monodis_assembly_search_hook(MonoAssemblyLoadContext *alc,
...
static void usage(void) {
  ...
  g_string_append(
      args, "[--help] [--mscorlib] [--show-tokens] [--show-method-tokens]\n");
...
static void monodis_thread_state_init(MonoThreadUnwindState *ctx) {}
...
int main(int argc, char *argv[]) {
  ...
  mono_install_assembly_search_hook_v2(monodis_assembly_search_hook, NULL,
                                       FALSE, FALSE, FALSE);
  ...
  if (!input_files->next) {
    ...
    mono_install_assembly_preload_hook_v2(
        monodis_preload, GUINT_TO_POINTER(FALSE), FALSE, FALSE);
    ...
    for (l = input_files; l; l = l->next)
      if (disassemble_file((const char *)l->data) == 1)
        i = 1;
    return i;
  }
