...
void dump_table_typedef(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    mono_metadata_decode_row(&m->tables[MONO_TABLE_TYPEDEF], i - 1, cols,
                             MONO_TYPEDEF_SIZE);
...
void dump_table_field(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
     ...
     * Find the next type.
     */
    while (current_type <= td->rows &&
           i >= (last_m = mono_metadata_decode_row_col(
                     td, current_type - 1, MONO_TYPEDEF_FIELD_LIST))) {
      current_type++;
    }
    if (i == first_m) {
      fprintf(output, "########## %s.%s\n",
              mono_metadata_string_heap(
                  m, mono_metadata_decode_row_col(td, current_type - 2,
    ...
    fprintf(output, "%d: %s %s: %s\n", i, sig,
            mono_metadata_string_heap(m, cols[MONO_FIELD_NAME]), flags);
    g_free(sig);
    g_free(flags);
    if (offset_row <= fl->rows &&
        (mono_metadata_decode_row_col(fl, offset_row - 1,
                                      MONO_FIELD_LAYOUT_FIELD) == i)) {
    ...
    if (rva_row <= rva->rows &&
        (mono_metadata_decode_row_col(rva, rva_row - 1, MONO_FIELD_RVA_FIELD) ==
         i)) {
      fprintf(
          output, "\trva: %d\n",
...
void dump_table_memberref(MonoImage *m) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    fprintf(output, "%d: %s[%d] %s\n\tResolved: %s\n\tSignature: %s\n\t\n",
            i + 1, ks, idx,
            mono_metadata_string_heap(m, cols[MONO_MEMBERREF_NAME]), x ? x : "",
            sig);
...
void dump_table_class_layout(MonoImage *m) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    fprintf(output, "%d: PackingSize=%d  ClassSize=%d  Parent=%s\n", i + 1,
            cols[MONO_CLASS_LAYOUT_PACKING_SIZE],
...
void dump_table_constant(MonoImage *m) {
  ...
  int i;
  const char *desc[] = {"Field", "Param", "Property", ""};
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    fprintf(output, "%d: Parent= %s: %d %s\n", i + 1, parent,
            cols[MONO_CONSTANT_PARENT] >> MONO_HASCONSTANT_BITS,
            get_constant(m, (MonoTypeEnum)cols[MONO_CONSTANT_TYPE],
                         cols[MONO_CONSTANT_VALUE]));
  }
}
...
void dump_table_property_map(MonoImage *m) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    fprintf(output, "%d: %s (%d) %d\n", i + 1, s,
            cols[MONO_PROPERTY_MAP_PARENT],
            cols[MONO_PROPERTY_MAP_PROPERTY_LIST]);
    g_free(s);
  }
}
...
void dump_table_property(MonoImage *m) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    fprintf(output, "%d: %s %s (", i + 1, type,
            mono_metadata_string_heap(m, cols[MONO_PROPERTY_NAME]));
    g_free(type);
...
static char *get_manifest_implementation(MonoImage *m, guint32 idx) {
  guint32 row;
  const char *table = "";
  if (!idx)
...
static const char *get_manifest_flags(guint32 mf) {
  mf &= 3;
  switch (mf) {
  case 1:
    return "public";
...
void dump_table_manifest(MonoImage *m) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    fprintf(output, "%d: %s '%s' at offset %u in %s\n", i + 1, mf, name,
            cols[MONO_MANIFEST_OFFSET], impl);
    g_free(impl);
  }
}
...
void dump_table_module(MonoImage *m) {
  ...
  for (i = 0; i < t->rows; i++) {
    ...
    fprintf(output, "%d: %s %d %s\n", i + 1, name, cols[MONO_MODULE_MVID],
            guid);
  }
}
...
void dump_table_method(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    while (current_type <= td->rows &&
           i >= (last_m = mono_metadata_decode_row_col(
                     td, current_type - 1, MONO_TYPEDEF_METHOD_LIST))) {
      current_type++;
    }
    if (i == first_m) {
      fprintf(output, "########## %s.%s\n",
              ...
              mono_metadata_string_heap(
                  m, mono_metadata_decode_row_col(td, current_type - 2,
                                                  MONO_TYPEDEF_NAME)));
      first_m = last_m;
      type_container = mono_metadata_load_generic_params(
          m, MONO_TOKEN_TYPE_DEF | (current_type - 1), NULL, NULL);
      if (type_container) {
    ...
    method_container = mono_metadata_load_generic_params(
        m, MONO_TOKEN_METHOD_DEF | i, type_container, NULL);
    if (method_container) {
      mono_metadata_load_generic_param_constraints_checked(
          m, MONO_TOKEN_METHOD_DEF | i, method_container, error);
      g_assert(is_ok(error)); /*FIXME don't swallow the error message*/
    }
    mono_metadata_decode_table_row(m, MONO_TABLE_METHOD, i - 1, cols,
                                   MONO_METHOD_SIZE);
    ...
    method = mono_metadata_parse_method_signature_full(
        m, method_container ? method_container : type_container, i, sigblob,
        &sigblob, error);
    if (!is_ok(error)) {
    ...
    g_assert(is_ok(error)); /*FIXME don't swallow the error message*/
    sig = dis_stringify_method_signature(
        m, method, i, method_container ? method_container : type_container,
        FALSE);
    impl_flags = get_method_impl_flags(cols[MONO_METHOD_IMPLFLAGS]);
    fprintf(output, "%d: %s (param: %d impl_flags: %s)\n", i, sig,
            cols[MONO_METHOD_PARAMLIST], impl_flags);
    g_free(sig);
    g_free(impl_flags);
...
void dump_table_implmap(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    method = get_method(m,
                        MONO_TOKEN_METHOD_DEF | (cols[MONO_IMPLMAP_MEMBER] >>
    ...
    fprintf(output, "%d: %s %d (%s %s)\n", i, method, cols[MONO_IMPLMAP_FLAGS],
            ...
            mono_metadata_string_heap(
                m, mono_metadata_decode_row_col(
                       td, cols[MONO_IMPLMAP_SCOPE] - 1, MONO_MODULEREF_NAME)));
  }
}
...
static dis_map_t semantics_map[] = {
    {1, "setter"},       {2, "getter"},  {4, "other"}, {8, "add-on"},
    {0x10, "remove-on"}, {0x20, "fire"}, {0, NULL},
};
...
void dump_table_methodsem(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    fprintf(output, "%d: [%d] %s method: %d %s %d\n", i,
            cols[MONO_METHOD_SEMA_ASSOCIATION], semantics,
            cols[MONO_METHOD_SEMA_METHOD] - 1,
            is_property ? "property" : "event", index);
  }
}
...
static char *has_cattr_get_table(MonoImage *m, guint32 val) {
...
static char *custom_attr_params(MonoImage *m, MonoMethodSignature *sig,
                                const char *value) {
  int len, i, slen, type;
  GString *res;
  char *s;
  ...
  handle_enum:
    ...
    case MONO_TYPE_R8: {
      ...
      if (inf == -1)
        g_string_append_printf(
            res, "(00 00 00 00 00 00 f0 ff)"); /* negative infinity */
      else if (inf == 1)
        g_string_append_printf(
            res, "(00 00 00 00 00 00 f0 7f)"); /* positive infinity */
      else if (mono_isnan(val))
    ...
    case MONO_TYPE_VALUETYPE:
      if (m_class_is_enumtype(sig->params[i]->data.klass)) {
        type =
            mono_class_enum_basetype_internal(sig->params[i]->data.klass)->type;
        goto handle_enum;
      } else {
    ...
    case MONO_TYPE_STRING:
      ...
      break;
    default:
      g_warning("Type %02x not handled in custom attr value decoding",
                sig->params[i]->type);
      break;
    }
  }
...
void dump_table_customattr(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    if (meth) {
      params = custom_attr_params(
          m, mono_method_signature_internal(meth),
          mono_metadata_blob_heap(m, cols[MONO_CUSTOM_ATTR_VALUE]));
      fprintf(output, "%d: %s: %s [%s]\n", i, desc, method, params);
      g_free(params);
    } else {
      fprintf(output, "Could not decode method due to %s",
              mono_error_get_message(error));
      mono_error_cleanup(error);
    }
...
void dump_table_nestedclass(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    fprintf(output, "%d: %d %d: %s in %s\n", i, cols[MONO_NESTED_CLASS_NESTED],
...
void dump_table_exported(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    fprintf(output, "%d: %s%s%s is in %s, index=%x, flags=0x%x\n", i, nspace,
            *nspace ? "." : "", name, impl, index, flags);
    g_free(impl);
  }
}

static void dump_blob(MonoImage *m, const char *blob) {
  int j, bsize;
...
void dump_table_field_marshal(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    is_field =
        (cols[MONO_FIELD_MARSHAL_PARENT] & MONO_HAS_FIELD_MARSHAL_MASK) ==
    ...
    fprintf(output, "%d: (0x%04x) %s %d: %s\n", i,
            cols[MONO_FIELD_MARSHAL_PARENT], is_field ? "Field" : "Param", idx,
            native);
...
static const char *get_security_action(int val) {
  static char buf[32];
...
void dump_table_declsec(MonoImage *m) {
  ...
  const char *parent[] = {"TypeDef", "MethodDef", "Assembly", ""};
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    fprintf(output, "%d: %s on %s %d%s", i, action,
            parent[idx & MONO_HAS_DECL_SECURITY_MASK],
            idx >> MONO_HAS_DECL_SECURITY_BITS, len ? ":\n\t" : "\n");
    if (!len)
      continue;
...
void dump_table_genericpar(MonoImage *m) {
  ...
  for (i = 1; i <= t->rows; i++) {
    ...
    fprintf(output, "%d: %d, flags=%d, owner=%s %s\n", i,
            cols[MONO_GENERICPARAM_NUMBER], cols[MONO_GENERICPARAM_FLAGS], sig,
...
void dump_stream_us(MonoImage *m) {
  ...
  for (i = 0; i < m->heap_us.size;) {
    ...
    char *str =
        get_encoded_user_string_or_bytearray((const guchar *)us_ptr, len);
    fprintf(output, "%02x: %s\n", i, str);
    g_free(str);
    i += len + 1;
  }
...
static void dump_table_ptr(MonoImage *m, int table, const char *name) {
