 ...
 */
static gpointer mono_win32_get_handle_stackoverflow(void) {
  ...
  /* XXX replace usage of struct sigcontext with MonoContext so we can use
   * MONO_STRUCT_OFFSET */
  x86_mov_reg_membase(code, X86_ESP, X86_EBX,
                      G_STRUCT_OFFSET(struct sigcontext, esp), 4);
  ...
  x86_mov_reg_membase(code, X86_EAX, X86_EAX,
                      G_STRUCT_OFFSET(MonoDomain, stack_overflow_ex), 4);
 ...
 *  - done
 */
static void win32_handle_stack_overflow(EXCEPTION_POINTERS *ep, CONTEXT *sctx) {
  ...
  initial_ctx = ctx;
  free_stack = (guint8 *)(MONO_CONTEXT_GET_BP(&ctx)) -
               (guint8 *)(MONO_CONTEXT_GET_BP(&initial_ctx));

  /* try to free 64kb from our stack */
  do {
    MonoContext new_ctx;

    mono_arch_unwind_frame(domain, jit_tls, &rji, &ctx, &new_ctx, &lmf, NULL,
                           &frame);
    if (!frame.ji) {
...
gpointer mono_arch_get_restore_context(MonoTrampInfo **info, gboolean aot) {
  ...
  /* restore EBX */
  x86_mov_reg_membase(code, X86_EBX, X86_EAX,
                      MONO_STRUCT_OFFSET(MonoContext, ebx), 4);

  /* restore EDI */
  x86_mov_reg_membase(code, X86_EDI, X86_EAX,
                      MONO_STRUCT_OFFSET(MonoContext, edi), 4);

  /* restore ESI */
  x86_mov_reg_membase(code, X86_ESI, X86_EAX,
                      MONO_STRUCT_OFFSET(MonoContext, esi), 4);

  /* restore EDX */
  x86_mov_reg_membase(code, X86_EDX, X86_EAX,
                      MONO_STRUCT_OFFSET(MonoContext, edx), 4);
 ...
 */
void mono_x86_throw_exception(host_mgreg_t *regs, MonoObject *exc,
...
void mono_x86_throw_corlib_exception(host_mgreg_t *regs, guint32 ex_token_index,
...
static void mono_x86_resume_unwind(host_mgreg_t *regs, MonoObject *exc,
                                   host_mgreg_t eip, gboolean rethrow) {
  MonoContext ctx;
 ...
 * If LLVM is true, generate code which assumes the caller is LLVM generated
 * code, which doesn't push the arguments.
 */
static guint8 *get_throw_trampoline(const char *name, gboolean rethrow,
                                    gboolean llvm, gboolean corlib,
                                    gboolean llvm_abs, gboolean resume_unwind,
  ...
  if (resume_unwind) {
    ...
    if (llvm_abs) {
      /*
       * The caller is LLVM code which passes the absolute address not a pc
       * offset, so compensate by passing 0 as 'ip' and passing the negated abs
       * address as the pc offset.
       */
  ...
  if (aot) {
    ...
    code = mono_arch_emit_load_aotconst(
        start, code, &ji, MONO_PATCH_INFO_JIT_ICALL_ADDR,
        GUINT_TO_POINTER(corlib ? MONO_JIT_ICALL_mono_x86_throw_corlib_exception
                                : MONO_JIT_ICALL_mono_x86_throw_exception));
    x86_call_reg(code, X86_EAX);
  } else {
    x86_call_code(code,
                  resume_unwind
                      ? (gpointer)(mono_x86_resume_unwind)
                      : (corlib ? (gpointer)mono_x86_throw_corlib_exception
                                : (gpointer)mono_x86_throw_exception));
  }
  x86_breakpoint(code);
...
gpointer mono_arch_get_throw_exception(MonoTrampInfo **info, gboolean aot) {
  return get_throw_trampoline("throw_exception", FALSE, FALSE, FALSE, FALSE,
                              FALSE, info, aot, FALSE);
}

gpointer mono_arch_get_rethrow_exception(MonoTrampInfo **info, gboolean aot) {
  return get_throw_trampoline("rethrow_exception", TRUE, FALSE, FALSE, FALSE,
                              FALSE, info, aot, FALSE);
}

gpointer mono_arch_get_rethrow_preserve_exception(MonoTrampInfo **info,
                                                  gboolean aot) {
  return get_throw_trampoline("rethrow_preserve_exception", TRUE, FALSE, FALSE,
                              FALSE, FALSE, info, aot, TRUE);
}
...
gpointer mono_arch_get_throw_corlib_exception(MonoTrampInfo **info,
                                              gboolean aot) {
  return get_throw_trampoline("throw_corlib_exception", FALSE, FALSE, TRUE,
                              FALSE, FALSE, info, aot, FALSE);
}
...
void mono_arch_exceptions_init(void) {
   ...
   * for SEHs to behave. This requires hotfix
   * http://support.microsoft.com/kb/976038 or (eventually) Windows 7 SP1.
   */
#ifdef TARGET_WIN32
  ...
  if (kernel32) {
    ...
    GetProcessUserModeExceptionPolicy_t const getter =
        (GetProcessUserModeExceptionPolicy_t)GetProcAddress(
            kernel32, "GetProcessUserModeExceptionPolicy");
    if (getter) {
      SetProcessUserModeExceptionPolicy_t const setter =
          (SetProcessUserModeExceptionPolicy_t)GetProcAddress(
  ...
  tramp = get_throw_trampoline("llvm_throw_exception_trampoline", FALSE, TRUE,
                               FALSE, FALSE, FALSE, &tinfo, FALSE, FALSE);
  mono_register_jit_icall_info(
      &mono_get_jit_icall_info()->mono_llvm_throw_exception_trampoline, tramp,
      "llvm_throw_exception_trampoline", NULL, TRUE, NULL);
  mono_tramp_info_register(tinfo, NULL);

  tramp = get_throw_trampoline("llvm_rethrow_exception_trampoline", TRUE, TRUE,
                               FALSE, FALSE, FALSE, &tinfo, FALSE, FALSE);
  mono_register_jit_icall_info(
      &mono_get_jit_icall_info()->mono_llvm_rethrow_exception_trampoline, tramp,
      "llvm_rethrow_exception_trampoline", NULL, TRUE, NULL);
  mono_tramp_info_register(tinfo, NULL);

  tramp = get_throw_trampoline("llvm_throw_corlib_exception_trampoline", FALSE,
                               TRUE, TRUE, FALSE, FALSE, &tinfo, FALSE, FALSE);
  mono_register_jit_icall_info(
  ...
  tramp =
      get_throw_trampoline("llvm_throw_corlib_exception_abs_trampoline", FALSE,
                           TRUE, TRUE, TRUE, FALSE, &tinfo, FALSE, FALSE);
  mono_register_jit_icall_info(
  ...
  tramp = get_throw_trampoline("llvm_resume_unwind_trampoline", FALSE, FALSE,
                               FALSE, FALSE, TRUE, &tinfo, FALSE, FALSE);
  mono_register_jit_icall_info(
  ...
  signal_exception_trampoline =
      mono_x86_get_signal_exception_trampoline(&tinfo, FALSE);
  mono_tramp_info_register(tinfo, NULL);
}
  ...
  *new_ctx = *ctx;
  ...
  if (ji != NULL) {
    ...
    gboolean success = mono_unwind_frame(
        (guint8 *)unwind_info, unwind_info_len, (guint8 *)ji->code_start,
        (guint8 *)ji->code_start + ji->code_size, (guint8 *)ip, NULL, regs,
        MONO_MAX_IREGS + 1, save_locations, MONO_MAX_IREGS, &cfa);
    ...
    if ((ji = mini_jit_info_table_find(domain, (gpointer)(uintptr_t)(*lmf)->eip,
                                       NULL))) {
      frame->ji = ji;
    } else {
      if (!(*lmf)->method)
...
gpointer mono_x86_get_signal_exception_trampoline(MonoTrampInfo **info,
  ...
  if (info)
    *info = mono_tramp_info_create("x86_signal_exception_trampoline", start,
...
static void altstack_handle_and_restore(MonoContext *ctx, gpointer obj,
                                        gboolean stack_ovf) {
  MonoContext mctx;
...
void mono_arch_handle_altstack_exception(void *sigctx,
  ...
  ucontext_t *ctx = (ucontext_t *)sigctx;
  MonoJitInfo *ji = mini_jit_info_table_find(
      mono_domain_get(), (gpointer)UCONTEXT_REG_EIP(ctx), NULL);
  gpointer *sp;
  int frame_size;

  /* if we didn't find a managed method for the ip address and it matches the
   * fault address, we assume we followed a broken pointer during an indirect
   * call, so we try the lookup again with the return address pushed on the
   * stack
   */
  ...
  /* the incoming arguments are aligned to 16 bytes boundaries, so the return
   * address IP goes at sp [-1]
   */
  ...
  /* at the return form the signal handler execution starts in
...
MonoContinuationRestore mono_tasklets_arch_restore(void) {
  ...
  x86_mov_reg_membase(code, X86_ECX, X86_EDX,
                      MONO_STRUCT_OFFSET(MonoContinuation, stack_used_size), 4);
  x86_shift_reg_imm(code, X86_SHR, X86_ECX, 2);
  x86_cld(code);
  x86_mov_reg_membase(code, X86_ESI, X86_EDX,
                      MONO_STRUCT_OFFSET(MonoContinuation, saved_stack), 4);
  x86_mov_reg_membase(code, X86_EDI, X86_EDX,
                      MONO_STRUCT_OFFSET(MonoContinuation, return_sp), 4);
  x86_prefix(code, X86_REP_PREFIX);
  x86_movsl(code);

  /* now restore the registers from the LMF */
  x86_mov_reg_membase(code, X86_ECX, X86_EDX,
                      MONO_STRUCT_OFFSET(MonoContinuation, lmf), 4);
  x86_mov_reg_membase(code, X86_EBP, X86_ECX, MONO_STRUCT_OFFSET(MonoLMF, ebp),
                      4);
  x86_mov_reg_membase(code, X86_ESP, X86_ECX, MONO_STRUCT_OFFSET(MonoLMF, esp),
                      4);
...
void mono_arch_setup_resume_sighandler_ctx(MonoContext *ctx, gpointer func) {
  gssize align =
      (((gssize)MONO_CONTEXT_GET_SP(ctx)) % MONO_ARCH_FRAME_ALIGNMENT + 4);
