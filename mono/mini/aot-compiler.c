        ...
        if ((patch_info->type == MONO_PATCH_INFO_METHOD) &&
          ...
          if (!got_only && is_direct_callable(acfg, method, patch_info)) {
            ...
            if (acfg->aot_opts.dedup &&
                !mono_aot_can_dedup(patch_info->data.method)) {
              char *name =
                  mono_aot_get_mangled_method_name(patch_info->data.method);
...
#define LD_OPTIONS "-Bsymbolic"
  ...
  command = g_strdup_printf(
      "%s%s %s -o %s %s %s %s", tool_prefix, LD_NAME, LD_OPTIONS,
  ...
  if (acfg->aot_opts.tool_prefix) {
    /* Cross compiling */
    command = g_strdup_printf(
        "\"%sld\" %s -shared -o %s %s %s %s", tool_prefix, LD_OPTIONS,
        wrap_path(tmp_outfile_name), wrap_path(llvm_ofile),
        wrap_path(g_strdup_printf("%s." AS_OBJECT_FILE_SUFFIX, acfg->tmpfname)),
        ld_flags);
  } else {
    char *args = g_strdup_printf(
        "%s -shared -o %s %s %s %s", LD_OPTIONS, wrap_path(tmp_outfile_name),
