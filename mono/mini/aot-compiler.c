...
static MonoMethod *try_get_method_nofail(MonoClass *klass,
                                         const char *method_name,
                                         int param_count, int flags) {
  MonoMethod *result;
  ERROR_DECL(error);
  result = mono_class_get_method_from_name_checked(klass, method_name,
                                                   param_count, flags, error);
...
static MonoMethod *get_method_nofail(MonoClass *klass, const char *method_name,
                                     int param_count, int flags) {
  MonoMethod *result =
      try_get_method_nofail(klass, method_name, param_count, flags);
  g_assertf(result, "Expected to find method %s in klass %s", method_name,
...
// Use MSVC toolchain, Clang for MSVC using MSVC codegen and linker, when
// compiling for AMD64 targeting WIN32 platforms running AOT compiler on WIN32
// platform with VS installation.
...
// Emit native unwind info on Windows platforms (different from DWARF). Emitted
// unwind info works when using the MSVC toolchain using Clang for MSVC codegen
// and linker. Only supported when compiling for AMD64 (Windows x64 platforms).
...
typedef struct MonoAotOptions {
  ...
  gboolean dedup;
  // The name of the assembly for which the AOT module is going to have all
  // deduped methods moved to. When set, we are emitting inflated methods only
  char *dedup_include;
  gboolean gnu_asm;
  gboolean try_llvm;
  gboolean llvm;
...
typedef struct MonoAotStats {
  ...
  gint64 code_size, method_info_size, ex_info_size, unwind_info_size, got_size,
      class_info_size, got_info_size, plt_size, blob_size;
...
typedef struct MonoAotCompile {
  ...
  guint32 got_offset, llvm_got_offset, plt_offset, plt_got_offset_base,
      plt_got_info_offset_base, nshared_got_entries;
...
// This, instead of an array of pointers, to optimize away a pointer and a
// relocation per string.
...
static G_GNUC_UNUSED const char *get_patch_name(int info) {
  return (const char *)&opstr + opidx[info];
}

static int emit_aot_image(MonoAotCompile *acfg);

static void mono_flush_method_cache(MonoAotCompile *acfg);

static void mono_read_method_cache(MonoAotCompile *acfg);

static guint32 get_unwind_info_offset(MonoAotCompile *acfg, guint8 *encoded,
                                      guint32 encoded_len);

static char *get_plt_entry_debug_sym(MonoAotCompile *acfg, MonoJumpInfo *ji,
                                     GHashTable *cache);

static void add_gsharedvt_wrappers(MonoAotCompile *acfg,
                                   MonoMethodSignature *sig,
                                   gboolean gsharedvt_in,
                                   gboolean gsharedvt_out, gboolean interp_in);

static void add_profile_instances(MonoAotCompile *acfg, ProfileData *data);

static gboolean ignore_cfg(MonoCompile *cfg) { return !cfg || cfg->skip; }

static void aot_printf(MonoAotCompile *acfg, const gchar *format, ...) {
  FILE *output;
  va_list args;
...
static void aot_printerrf(MonoAotCompile *acfg, const gchar *format, ...) {
  FILE *output;
  va_list args;
...
static void report_loader_error(MonoAotCompile *acfg, MonoError *error,
                                gboolean fatal, const char *format, ...) {
  FILE *output;
  va_list args;
...
static const char *mangle_symbol(const char *symbol, char *mangled_symbol,
                                 gsize length) {
...
static char *mangle_symbol_alloc(const char *symbol) {
  g_assert(NULL != symbol);
...
static void emit_section_change(MonoAotCompile *acfg, const char *section_name,
...
static void emit_local_symbol(MonoAotCompile *acfg, const char *name,
                              const char *end_label, gboolean func) {
  ...
  if (name != mangled_symbol_name &&
      0 != g_strcasecmp(name, mangled_symbol_name)) {
    mono_img_writer_emit_label(acfg->w, mangled_symbol_name);
  }
  mono_img_writer_emit_local_symbol(acfg->w, mangled_symbol_name, end_label,
                                    func);
...
static void emit_alignment_code(MonoAotCompile *acfg, int size) {
...
static void emit_padding(MonoAotCompile *acfg, int size) {
  int i;
  guint8 buf[16];
...
static void emit_pointer_2(MonoAotCompile *acfg, const char *prefix,
                           const char *target) {
  if (prefix[0] != '\0') {
...
static void emit_symbol_diff(MonoAotCompile *acfg, const char *end,
                             const char *start, int offset) {
...
static void emit_byte(MonoAotCompile *acfg, guint8 val) {
  mono_img_writer_emit_byte(acfg->w, val);
}

#if defined(TARGET_WIN32) && defined(TARGET_X86)

static G_GNUC_UNUSED void emit_global_inner(MonoAotCompile *acfg,
                                            const char *name, gboolean func) {
  const char *mangled_symbol_name = name;
  char *mangled_symbol_name_alloc = NULL;
...
static gboolean link_shared_library(MonoAotCompile *acfg) {
  return !acfg->aot_opts.static_link && !acfg->aot_opts.asm_only;
}
#endif

static gboolean add_to_global_symbol_table(MonoAotCompile *acfg) {
#ifdef TARGET_WIN32_MSVC
...
static void emit_global(MonoAotCompile *acfg, const char *name, gboolean func) {
...
static void emit_symbol_size(MonoAotCompile *acfg, const char *name,
...
static void emit_info_symbol(MonoAotCompile *acfg, const char *name,
                             gboolean func) {
  ...
  if (acfg->llvm) {
    ...
    sprintf(symbol, "%s%s%s", acfg->user_symbol_prefix, acfg->global_prefix,
            name);
...
static void emit_string_symbol(MonoAotCompile *acfg, const char *name,
                               const char *value) {
  if (acfg->llvm) {
...
static G_GNUC_UNUSED void emit_uleb128(MonoAotCompile *acfg, guint32 value) {
  do {
    guint8 b = value & 0x7f;
    value >>= 7;
...
static G_GNUC_UNUSED void emit_sleb128(MonoAotCompile *acfg, gint64 value) {
  gboolean more = 1;
...
static G_GNUC_UNUSED void encode_uleb128(guint32 value, guint8 *buf,
                                         guint8 **endbuf) {
  guint8 *p = buf;
...
static G_GNUC_UNUSED void encode_sleb128(gint32 value, guint8 *buf,
                                         guint8 **endbuf) {
  gboolean more = 1;
  gboolean negative = (value < 0);
  guint32 size = 32;
...
static void encode_int(gint32 val, guint8 *buf, guint8 **endbuf) {
  // FIXME: Big-endian
  buf[0] = (val >> 0) & 0xff;
  buf[1] = (val >> 8) & 0xff;
  buf[2] = (val >> 16) & 0xff;
...
static void encode_int16(guint16 val, guint8 *buf, guint8 **endbuf) {
  buf[0] = (val >> 0) & 0xff;
  buf[1] = (val >> 8) & 0xff;
...
static void encode_string(const char *s, guint8 *buf, guint8 **endbuf) {
  int len = strlen(s);
...
static void emit_unset_mode(MonoAotCompile *acfg) {
  mono_img_writer_emit_unset_mode(acfg->w);
}

static G_GNUC_UNUSED void emit_set_thumb_mode(MonoAotCompile *acfg) {
  emit_unset_mode(acfg);
  fprintf(acfg->fp, ".code 16\n");
}

static G_GNUC_UNUSED void emit_set_arm_mode(MonoAotCompile *acfg) {
  emit_unset_mode(acfg);
  fprintf(acfg->fp, ".code 32\n");
}

static void emit_code_bytes(MonoAotCompile *acfg, const guint8 *buf, int size) {
#ifdef TARGET_ARM64
  int i;
...
/* ARCHITECTURE SPECIFIC CODE */
    ...
    defined(TARGET_POWERPC) || defined(TARGET_ARM64) || defined(TARGET_RISCV)
...
#ifdef EMIT_WIN32_UNWIND_INFO
static UnwindInfoSectionCacheItem *get_cached_unwind_info_section_item_win32(
    MonoAotCompile *acfg, const char *function_start, const char *function_end,
    GSList *unwind_ops);

static void free_unwind_info_section_cache_win32(MonoAotCompile *acfg);

static void emit_unwind_info_data_win32(MonoAotCompile *acfg,
                                        PUNWIND_INFO unwind_info);

static void emit_unwind_info_sections_win32(MonoAotCompile *acfg,
                                            const char *function_start,
                                            const char *function_end,
                                            GSList *unwind_ops);
#endif

static void arch_free_unwind_info_section_cache(MonoAotCompile *acfg) {
#ifdef EMIT_WIN32_UNWIND_INFO
  free_unwind_info_section_cache_win32(acfg);
#endif
}

static void arch_emit_unwind_info_sections(MonoAotCompile *acfg,
                                           const char *function_start,
                                           const char *function_end,
                                           GSList *unwind_ops) {
#ifdef EMIT_WIN32_UNWIND_INFO
  ...
  emit_unwind_info_sections_win32(acfg, function_start, function_end,
                                  unwind_ops);
...
static void arch_init(MonoAotCompile *acfg) {
   ...
   * The mach-os specs require this for globals, but LLVM puts them in front of
   * all symbols. We need to handle this, since we need to refer to LLVM
   * generated symbols.
   */
  ...
  const gboolean has_custom_args =
      !!acfg->aot_opts.llvm_llc || acfg->aot_opts.use_current_cpu;
#endif

#if defined(TARGET_X86)
  g_string_append_printf(acfg->llc_args, " -march=x86 %s",
                         has_custom_args ? "" : "-mcpu=generic");
#endif

#if defined(TARGET_AMD64)
  g_string_append_printf(acfg->llc_args, " -march=x86-64 %s",
                         has_custom_args ? "" : "-mcpu=generic");
  /* NOP */
  acfg->align_pad_value = 0x90;
#ifdef MONO_ARCH_CODE_EXEC_ONLY
  acfg->flags =
      (MonoAotFileFlags)(acfg->flags | MONO_AOT_FILE_FLAG_CODE_EXEC_ONLY);
#endif
#endif
  g_string_append(acfg->llc_args,
                  " -enable-implicit-null-checks -disable-fault-maps");

  if (mono_use_fast_math) {
    // same parameters are passed to opt and LLVM JIT
    g_string_append(acfg->llc_args,
                    " -fp-contract=fast -enable-no-infs-fp-math "
                    "-enable-no-nans-fp-math -enable-no-signed-zeros-fp-math "
                    "-enable-no-trapping-fp-math -enable-unsafe-fp-math");
  }
...
static void arm64_emit_load_got_slot(MonoAotCompile *acfg, int dreg,
                                     int got_slot) {
  int offset;
...
static void arm64_emit_objc_selector_ref(MonoAotCompile *acfg, guint8 *code,
                                         int index, int *code_size) {
  int reg;
...
static void arm64_emit_direct_call(MonoAotCompile *acfg, const char *target,
                                   gboolean external, gboolean thumb,
                                   MonoJumpInfo *ji, int *call_size) {
  g_assert(acfg->fp);
  emit_unset_mode(acfg);
...
static void arm64_emit_got_access(MonoAotCompile *acfg, guint8 *code,
                                  int got_slot, int *code_size) {
  int reg;
...
static void arm64_emit_plt_entry(MonoAotCompile *acfg, const char *got_symbol,
                                 int offset, int info_offset) {
...
static void arm64_emit_tramp_page_common_code(MonoAotCompile *acfg,
                                              int pagesize, int arg_reg,
                                              int *size) {
  guint8 buf[256];
  guint8 *code;
  int imm;
...
static void arm64_emit_tramp_page_specific_code(MonoAotCompile *acfg,
                                                int pagesize,
...
static void arm64_emit_specific_trampoline_pages(MonoAotCompile *acfg) {
  guint8 buf[128];
  guint8 *code;
  guint8 *labels[16];
  int common_tramp_size;
  ...
  /* The common code */
  arm64_emit_tramp_page_common_code(acfg, pagesize, ARMREG_IP1,
                                    &common_tramp_size);
  acfg->tramp_page_code_offsets[MONO_AOT_TRAMP_SPECIFIC] = common_tramp_size;

  arm64_emit_tramp_page_specific_code(acfg, pagesize, common_tramp_size,
                                      specific_tramp_size);

  /* The rgctx trampolines */
  /* These are the same as the specific trampolines, but they load the argument
   * into MONO_ARCH_RGCTX_REG */
  ...
  /* The common code */
  arm64_emit_tramp_page_common_code(acfg, pagesize, MONO_ARCH_RGCTX_REG,
                                    &common_tramp_size);
  acfg->tramp_page_code_offsets[MONO_AOT_TRAMP_STATIC_RGCTX] =
      common_tramp_size;
  ...
  arm64_emit_tramp_page_common_code(acfg, pagesize, ARMREG_IP1,
                                    &common_tramp_size);
  acfg->tramp_page_code_offsets[MONO_AOT_TRAMP_GSHAREDVT_ARG] =
      common_tramp_size;
  ...
  sprintf(symbol, "%sunbox_arbitrary_trampolines_page",
          acfg->user_symbol_prefix);
  emit_alignment(acfg, pagesize);
  emit_global(acfg, symbol, TRUE);
  emit_label(acfg, symbol);
  ...
  acfg->tramp_page_code_offsets[MONO_AOT_TRAMP_UNBOX_ARBITRARY] =
      common_tramp_size;
...
static void arm64_emit_specific_trampoline(MonoAotCompile *acfg, int offset,
                                           int *tramp_size) {
...
static void arm64_emit_unbox_trampoline(MonoAotCompile *acfg, MonoCompile *cfg,
                                        MonoMethod *method,
...
static void arm64_emit_static_rgctx_trampoline(MonoAotCompile *acfg, int offset,
                                               int *tramp_size) {
...
static void arm64_emit_imt_trampoline(MonoAotCompile *acfg, int offset,
                                      int *tramp_size) {
  guint8 buf[128];
...
static void arm64_emit_gsharedvt_arg_trampoline(MonoAotCompile *acfg,
                                                int offset, int *tramp_size) {
 ...
 *
 *   Emit a direct call to the symbol TARGET. CALL_SIZE is set to the size of
 * the calling code.
 */
static void arch_emit_direct_call(MonoAotCompile *acfg, const char *target,
                                  gboolean external, gboolean thumb,
                                  MonoJumpInfo *ji, int *call_size) {
...
static void arch_emit_label_address(MonoAotCompile *acfg, const char *target,
                                    gboolean external_call, gboolean thumb,
                                    MonoJumpInfo *ji, int *call_size) {
 ...
 */
static void arch_emit_got_offset(MonoAotCompile *acfg, guint8 *code,
                                 int *code_size) {
 ...
 * slot (OP_AOTCONST/OP_GOT_ENTRY). Emit this code while patching it so it
 * accesses the GOT slot GOT_SLOT. CODE_SIZE is set to the number of bytes
 * emitted.
 */
static void arch_emit_got_access(MonoAotCompile *acfg, const char *got_symbol,
                                 guint8 *code, int got_slot, int *code_size) {
#ifdef TARGET_AMD64
  /* mov reg, got+offset(%rip) */
  if (acfg->llvm) {
    /* The GOT symbol is in the LLVM module, the clang assembler has problems
     * emitting symbol diffs for it */
    int dreg;
    int rex_r;
    ...
    fprintf(acfg->fp, "mov %s+%d(%%rip), %s\n", got_symbol,
            (unsigned int)((got_slot * sizeof(target_mgreg_t))),
            mono_arch_regname(dreg));
    *code_size = 7;
  } else {
    emit_bytes(acfg, code, mono_arch_get_patch_offset(code));
    emit_symbol_diff(acfg, got_symbol, ".",
                     (unsigned int)((got_slot * sizeof(target_mgreg_t)) - 4));
  ...
  *code_size = mono_arch_get_patch_offset(code) + 4;
  ...
  emit_bytes(acfg, code, mono_arch_get_patch_offset(code));
  emit_symbol_diff(acfg, got_symbol, ".",
                   (unsigned int)((got_slot * sizeof(target_mgreg_t))) - 12);
  *code_size = mono_arch_get_patch_offset(code) + 4;
#elif defined(TARGET_ARM64)
  emit_bytes(acfg, code, mono_arch_get_patch_offset(code));
 ...
 *
 *   Emit the implementation of OP_OBJC_GET_SELECTOR, which itself implements
 * @selector(foo:) in objective-c.
 */
static void arch_emit_objc_selector_ref(MonoAotCompile *acfg, guint8 *code,
                                        int index, int *code_size) {
#if defined(TARGET_ARM)
 ...
 * The plt entry should look like this on architectures where code is
 * read/execute: <indirect jump to GOT_SYMBOL + OFFSET> <INFO_OFFSET embedded
 * into the instruction stream> The plt entry should look like this on
 * architectures where code is execute only: mov RAX, PLT entry offset <indirect
 * jump to GOT_SYMBOL + OFFSET>
 */
static void arch_emit_plt_entry(MonoAotCompile *acfg, const char *got_symbol,
                                guint32 plt_index, int offset,
                                int info_offset) {
#if defined(TARGET_X86)
  /* jmp *<offset>(%ebx) */
  emit_byte(acfg, 0xff);
  ...
  /* Emit smallest possible imm size 1, 2 or 4 bytes based on total number of
   * PLT entries. */
  if (acfg->plt_offset <= (guint32)0xFF) {
    ...
    *(code)++ = (unsigned char)0xb8 + (PLT_ENTRY_OFFSET_REG & 0x7);
    ...
    amd64_mov_reg_imm_size(code, PLT_ENTRY_OFFSET_REG, plt_index,
                           sizeof(plt_index));
  }
  emit_bytes(acfg, buf, code - buf);
 ...
 */
static void arch_emit_llvm_plt_entry(MonoAotCompile *acfg,
                                     const char *got_symbol, int plt_index,
                                     int offset, int info_offset) {
#if defined(TARGET_ARM)
...
/* Save unwind_info in the module and emit the offset to the information at
 * symbol */
static void save_unwind_info(MonoAotCompile *acfg, char *symbol,
                             GSList *unwind_ops) {
 ...
 * are allocated, with the first containing the data and the second containing
 * the trampolines. To reduce trampoline size, each trampoline jumps at the
 * start of the page where a common implementation does all the lifting. Note
 * that the ARM single trampoline size is 8 bytes, exactly like the data that
 * needs to be stored on the arm 32 bit system.
 */
static void arch_emit_specific_trampoline_pages(MonoAotCompile *acfg) {
#if defined(TARGET_ARM)
  guint8 buf[128];
  guint8 *code;
...
#define COMMON_TRAMP_SIZE 16
  ...
  /* emit the generic code first, the trampoline address + 8 is in the lr
   * register */
  code = buf;
  ...
  /* now the rgctx trampolines: each specific trampolines puts in the ip
   * register the instruction pointer address, so the generic trampoline at the
   * start of the page subtracts 4096 to get to the data page and loads the
   * values We again fit the generic trampiline in 16 bytes.
   */
  sprintf(symbol, "%srgctx_trampolines_page", acfg->user_symbol_prefix);
  emit_global(acfg, symbol, TRUE);
  emit_label(acfg, symbol);
  ...
  /* now the unbox arbitrary trampolines: each specific trampolines puts in the
   * ip register the instruction pointer address, so the generic trampoline at
   * the start of the page subtracts 4096 to get to the data page and loads the
   * target addr. We again fit the generic trampoline in 16 bytes.
   */
  sprintf(symbol, "%sunbox_arbitrary_trampolines_page",
          acfg->user_symbol_prefix);
  emit_global(acfg, symbol, TRUE);
   ...
   * the instruction pointer address, so the generic trampoline at the start of
   * the page subtracts 4096 to get to the data page and loads the values
   */
#define IMT_TRAMP_SIZE 72
  ...
  sprintf(symbol, "%sspecific_trampolines_page_gen_p",
          acfg->user_symbol_prefix);
  /* We unwind to the original caller, from the stack, since lr is clobbered */
  mono_add_unwind_op_def_cfa(unwind_ops, 0, 0, ARMREG_SP,
  ...
  mono_add_unwind_op_def_cfa_offset(unwind_ops, 4, 0,
                                    14 * sizeof(target_mgreg_t));
  save_unwind_info(acfg, symbol, unwind_ops);
  mono_free_unwind_info(unwind_ops);
  ...
  sprintf(symbol, "%sgsharedvt_trampolines_page_gen_p",
          acfg->user_symbol_prefix);
  mono_add_unwind_op_def_cfa(unwind_ops, 0, 0, ARMREG_SP, 0);
  mono_add_unwind_op_def_cfa_offset(unwind_ops, 4, 0,
  ...
  sprintf(symbol, "%sgsharedvt_trampolines_page_sp_p",
          acfg->user_symbol_prefix);
  mono_add_unwind_op_def_cfa(unwind_ops, 0, 0, ARMREG_SP, 0);
  save_unwind_info(acfg, symbol, unwind_ops);
  mono_free_unwind_info(unwind_ops);
  ...
  sprintf(symbol, "%sunbox_arbitrary_trampolines_page_gen_p",
          acfg->user_symbol_prefix);
  mono_add_unwind_op_def_cfa(unwind_ops, 0, 0, ARMREG_SP, 0);
  save_unwind_info(acfg, symbol, unwind_ops);

  sprintf(symbol, "%sunbox_arbitrary_trampolines_page_sp_p",
          acfg->user_symbol_prefix);
  save_unwind_info(acfg, symbol, unwind_ops);
  mono_free_unwind_info(unwind_ops);
 ...
 */
static void arch_emit_specific_trampoline(MonoAotCompile *acfg, int offset,
                                          int *tramp_size) {
  /*
  ...
  *tramp_size = 8;
  ...
  if (acfg->llvm) {
    ...
    emit_byte(acfg, '\xff');
    emit_byte(acfg, '\x15');
    emit_symbol_diff(acfg, acfg->got_symbol, ".",
                     (offset * sizeof(target_mgreg_t)) - 4);
    emit_zero_bytes(acfg, 1);
  }
#elif defined(TARGET_ARM)
  guint8 buf[128];
  ...
  *tramp_size = 20;
   ...
   */
  emit_symbol_diff(acfg, acfg->got_symbol, ".",
                   (offset * sizeof(target_mgreg_t)) - 4 + 4);
  // emit_symbol_diff (acfg, acfg->got_symbol, ".", ((offset + 1) * sizeof
  // (target_mgreg_t)) - 4 + 8);
#elif defined(TARGET_ARM64)
  ...
  *tramp_size = 9 * 4;
  ...
  code = buf;
  /* Load mscorlib got address */
  x86_mov_reg_membase(code, X86_ECX, MONO_ARCH_GOT_REG, sizeof(target_mgreg_t),
                      4);
  /* Push trampoline argument */
  x86_push_membase(code, X86_ECX, (offset + 1) * sizeof(target_mgreg_t));
  /* Load generic trampoline address */
  x86_mov_reg_membase(code, X86_ECX, X86_ECX, offset * sizeof(target_mgreg_t),
                      4);
  /* Branch to generic trampoline */
 ...
 */
static void arch_emit_unbox_trampoline(MonoAotCompile *acfg, MonoCompile *cfg,
                                       MonoMethod *method,
                                       const char *call_target) {
#if defined(TARGET_AMD64)
  guint8 buf[32];
  ...
  x86_alu_membase_imm(code, X86_ADD, X86_ESP, this_pos,
 ...
 *
 *   Emit code for a static rgctx trampoline. OFFSET is the offset of the first
 * of two GOT slots which contain the rgctx argument, and the method to jump to.
 ...
 */
static void arch_emit_static_rgctx_trampoline(MonoAotCompile *acfg, int offset,
                                              int *tramp_size) {
#if defined(TARGET_AMD64)
  ...
  *tramp_size = 13;
  ...
  if (acfg->llvm) {
    ...
    emit_byte(acfg, '\x25');
    emit_symbol_diff(acfg, acfg->got_symbol, ".",
                     ((offset + 1) * sizeof(target_mgreg_t)) - 4);
  }
#elif defined(TARGET_ARM)
  guint8 buf[128];
  guint8 *code;
  ...
  *tramp_size = 24;
  ...
  emit_bytes(acfg, buf, code - buf);
  emit_symbol_diff(acfg, acfg->got_symbol, ".",
                   (offset * sizeof(target_mgreg_t)) - 4 + 8);
  emit_symbol_diff(acfg, acfg->got_symbol, ".",
                   ((offset + 1) * sizeof(target_mgreg_t)) - 4 + 4);
#elif defined(TARGET_ARM64)
  ...
  *tramp_size = 9 * 4;
  ...
  x86_mov_reg_membase(code, X86_ECX, MONO_ARCH_GOT_REG, sizeof(target_mgreg_t),
                      4);
  /* Load arg */
  x86_mov_reg_membase(code, MONO_ARCH_RGCTX_REG, X86_ECX,
                      offset * sizeof(target_mgreg_t), 4);
  /* Branch to the target address */
 ...
 *
 *   Emit an IMT trampoline usable in full-aot mode. The trampoline uses 1 got
 * slot which points to an array of pointer pairs. The pairs of the form [key,
 * ptr], where key is the IMT key, and ptr holds the address of a memory
 * location holding the address to branch to if the IMT arg matches the key. The
 * array is terminated by a pair whose key is NULL, and whose ptr is the address
 * of the fail_tramp. TRAMP_SIZE is set to the size of the emitted trampoline.
 */
static void arch_emit_imt_trampoline(MonoAotCompile *acfg, int offset,
                                     int *tramp_size) {
#if defined(TARGET_AMD64)
  guint8 *buf, *code;
  guint8 *labels[16];
  ...
  /* Check key */
  amd64_alu_membase_reg_size(code, X86_CMP, MONO_ARCH_IMT_SCRATCH_REG, 0,
                             MONO_ARCH_IMT_REG, sizeof(target_mgreg_t));
  labels[2] = code;
  amd64_branch8(code, X86_CC_Z, 0, FALSE);

  /* Loop footer */
  amd64_alu_reg_imm(code, X86_ADD, MONO_ARCH_IMT_SCRATCH_REG,
                    2 * sizeof(target_mgreg_t));
  amd64_jump_code(code, labels[0]);

  /* Match */
  mono_amd64_patch(labels[2], code);
  amd64_mov_reg_membase(code, MONO_ARCH_IMT_SCRATCH_REG,
                        MONO_ARCH_IMT_SCRATCH_REG, sizeof(target_mgreg_t),
  ...
  if (!acfg->llvm) {
    ...
    amd64_emit_rex(mov_buf_ptr, sizeof(gpointer), MONO_ARCH_IMT_SCRATCH_REG, 0,
                   AMD64_RIP);
  ...
  *tramp_size = code - buf + kSizeOfMove;
  ...
  /* Load mscorlib got address */
  x86_mov_reg_membase(code, X86_EAX, MONO_ARCH_GOT_REG, sizeof(target_mgreg_t),
                      4);
  /* Load arg */
  x86_mov_reg_membase(code, X86_EAX, X86_EAX, offset * sizeof(target_mgreg_t),
                      4);
  ...
  *tramp_size = code - buf + 4;
  ...
  ppc_cmp(code, 0, sizeof(target_mgreg_t) == 8 ? 1 : 0, ppc_r0,
          MONO_ARCH_IMT_REG);
  labels[2] = code;
...
static void amd64_emit_load_got_slot(MonoAotCompile *acfg, int dreg,
                                     int got_slot) {
 ...
 *
 *   Emit code for a gsharedvt arg trampoline. OFFSET is the offset of the first
 * of two GOT slots which contain the argument, and the code to jump to.
 * TRAMP_SIZE is set to the size of the emitted trampoline.
 * These kinds of trampolines cannot be enumerated statically, since there could
 * be one trampoline per method instantiation, so we emit the same code for all
 * trampolines, and parameterize them using two GOT slots.
 */
static void arch_emit_gsharedvt_arg_trampoline(MonoAotCompile *acfg, int offset,
                                               int *tramp_size) {
#if defined(TARGET_X86)
  guint8 buf[128];
  guint8 *code;
  ...
  x86_mov_reg_membase(code, X86_ECX, MONO_ARCH_GOT_REG, sizeof(target_mgreg_t),
                      4);
  /* Load arg */
  x86_mov_reg_membase(code, X86_EAX, X86_ECX, offset * sizeof(target_mgreg_t),
                      4);
  /* Branch to the target address */
  x86_jump_membase(code, X86_ECX, (offset + 1) * sizeof(target_mgreg_t));
...
static void arch_emit_ftnptr_arg_trampoline(MonoAotCompile *acfg, int offset,
                                            int *tramp_size) {
#if defined(TARGET_ARM)
  guint8 buf[128];
  guint8 *code;
  ...
  *tramp_size = 32;
  ...
  emit_bytes(acfg, buf, code - buf);
  emit_symbol_diff(acfg, acfg->got_symbol, ".",
                   ((offset + 1) * sizeof(target_mgreg_t)) +
...
static void arch_emit_unbox_arbitrary_trampoline(MonoAotCompile *acfg,
                                                 int offset, int *tramp_size) {
  ...
  *tramp_size = 13;
  ...
  emit_bytes(acfg, buf, code - buf);
  emit_symbol_diff(acfg, acfg->got_symbol, ".",
                   (offset * sizeof(target_mgreg_t)) + (code - (label + 8)) -
                       4);
  *tramp_size = 4 * 4;
#else
...
static guint32 mono_get_field_token(MonoClassField *field) {
  ...
  for (i = 0; i < fcount; ++i) {
    if (field == &klass_fields[i])
      return MONO_TOKEN_FIELD_DEF |
             (mono_class_get_first_field_idx(klass) + 1 + i);
  }
...
static void encode_value(gint32 value, guint8 *buf, guint8 **endbuf) {
  guint8 *p = buf;
...
static void stream_init(MonoDynamicStream *sh) {
  sh->index = 0;
  sh->alloc_size = 4096;
...
static void make_room_in_stream(MonoDynamicStream *stream, int size) {
...
static guint32 add_stream_data(MonoDynamicStream *stream, const char *data,
                               guint32 len) {
  guint32 idx;
 ...
 * add_to_blob:
 *
 *   Add data to the binary blob inside the aot image. Returns the offset inside
 * the blob where the data was stored.
 */
static guint32 add_to_blob(MonoAotCompile *acfg, const guint8 *data,
                           guint32 data_len) {
...
static guint32 add_to_blob_aligned(MonoAotCompile *acfg, const guint8 *data,
                                   guint32 data_len, guint32 align) {
  char buf[4] = {0};
  guint32 count;
...
static void emit_aot_data(MonoAotCompile *acfg, MonoAotFileTable table,
                          const char *symbol, guint8 *data, int size) {
  if (acfg->data_outfile) {
    ...
    // align the data to 8 bytes. Put zeros in the file (so that every build
 ...
 * emit_offset_table:
 *
 *   Emit a table of increasing offsets in a compact form using differential
 * encoding. There is an index entry for each GROUP_SIZE number of entries. The
 * greater the group size, the more compact the table becomes, but the slower it
 * becomes to compute a given entry. Returns the size of the table.
 */
static guint32 emit_offset_table(MonoAotCompile *acfg, const char *symbol,
                                 MonoAotFileTable table, int noffsets,
                                 int group_size, gint32 *offsets) {
...
static guint32 get_image_index(MonoAotCompile *cfg, MonoImage *image) {
  guint32 index;
...
static guint32 find_typespec_for_class(MonoAotCompile *acfg, MonoClass *klass) {
  int i;
  ...
  if (!acfg->typespec_classes) {
    ...
    for (i = 0; i < len; i++) {
      ...
      MonoClass *klass_key = mono_class_get_and_inflate_typespec_checked(
          acfg->image, typespec, NULL, error);
      if (!is_ok(error)) {
        mono_error_cleanup(error);
        continue;
      }
      g_hash_table_insert(acfg->typespec_classes, klass_key,
...
static void encode_method_ref(MonoAotCompile *acfg, MonoMethod *method,
                              guint8 *buf, guint8 **endbuf);

static void encode_klass_ref(MonoAotCompile *acfg, MonoClass *klass,
                             guint8 *buf, guint8 **endbuf);

static void encode_ginst(MonoAotCompile *acfg, MonoGenericInst *inst,
                         guint8 *buf, guint8 **endbuf);

static void encode_type(MonoAotCompile *acfg, MonoType *t, guint8 *buf,
                        guint8 **endbuf);

static guint32 get_shared_ginst_ref(MonoAotCompile *acfg,
                                    MonoGenericInst *ginst);

static void encode_klass_ref_inner(MonoAotCompile *acfg, MonoClass *klass,
                                   guint8 *buf, guint8 **endbuf) {
  guint8 *p = buf;

  /*
   * The encoding begins with one of the MONO_AOT_TYPEREF values, followed by
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    g_assert(mono_metadata_token_code(m_class_get_type_token(klass)) ==
             MONO_TOKEN_TYPE_DEF);
    if (iindex == 0) {
  ...
  } else if ((m_class_get_byval_arg(klass)->type == MONO_TYPE_VAR) ||
             (m_class_get_byval_arg(klass)->type == MONO_TYPE_MVAR)) {
    ...
    if (par->gshared_constraint) {
      ...
      encode_klass_ref(acfg, mono_class_create_generic_parameter(gpar->parent),
                       p, &p);
    } else {
      encode_value(m_class_get_byval_arg(klass)->type, p, &p);
      encode_value(
          mono_type_get_generic_param_num(m_class_get_byval_arg(klass)), p, &p);
...
static guint32 get_shared_klass_ref(MonoAotCompile *acfg, MonoClass *klass) {
  guint offset =
      GPOINTER_TO_UINT(g_hash_table_lookup(acfg->klass_blob_hash, klass));
  guint8 *buf2, *p;
  ...
  if (!offset) {
    ...
    g_hash_table_insert(acfg->klass_blob_hash, klass,
...
static void encode_field_info(MonoAotCompile *cfg, MonoClassField *field,
                              guint8 *buf, guint8 **endbuf) {
...
static void encode_ginst(MonoAotCompile *acfg, MonoGenericInst *inst,
  ...
  for (i = 0; i < inst->type_argc; ++i)
    encode_klass_ref(
        acfg, mono_class_from_mono_type_internal(inst->type_argv[i]), p, &p);
...
static guint32 get_shared_ginst_ref(MonoAotCompile *acfg,
                                    MonoGenericInst *ginst) {
  guint32 offset =
      GPOINTER_TO_UINT(g_hash_table_lookup(acfg->ginst_blob_hash, ginst));
  if (!offset) {
    guint8 *buf2, *p2;
    ...
    g_hash_table_insert(acfg->ginst_blob_hash, ginst,
                        GUINT_TO_POINTER(offset + 1));
  } else {
    offset--;
  }
...
static void encode_generic_context(MonoAotCompile *acfg,
                                   MonoGenericContext *context, guint8 *buf,
                                   guint8 **endbuf) {
  guint8 *p = buf;
  MonoGenericInst *inst;
  guint32 flags =
      (context->class_inst ? 1 : 0) | (context->method_inst ? 2 : 0);
  ...
  if (t->has_cmods) {
    ...
    *p = MONO_TYPE_CMOD_REQD;
    ...
    for (int i = 0; i < count; ++i) {
      ...
      gboolean required;
      MonoType *cmod_type =
          mono_type_get_custom_modifier(t, i, &required, error);
      mono_error_assert_ok(error);
...
static void encode_signature(MonoAotCompile *acfg, MonoMethodSignature *sig,
                             guint8 *buf, guint8 **endbuf) {
  guint8 *p = buf;
  guint32 flags = 0;
  int i;
...
static void encode_method_ref(MonoAotCompile *acfg, MonoMethod *method,
  ...
  if (method->wrapper_type) {
    ...
    case MONO_WRAPPER_RUNTIME_INVOKE: {
      g_assert(info);
      encode_value(info->subtype, p, &p);
      if (info->subtype == WRAPPER_SUBTYPE_RUNTIME_INVOKE_DIRECT ||
          info->subtype == WRAPPER_SUBTYPE_RUNTIME_INVOKE_VIRTUAL)
     ...
     */
    ji =
        (MonoJumpInfoToken *)g_hash_table_lookup(acfg->token_info_hash, method);
    if (ji) {
      ...
      guint32 offset =
          GPOINTER_TO_UINT(g_hash_table_lookup(acfg->method_blob_hash, method));
      offset--;
    ...
    if (ji) {
      ...
      if (!strcmp(method->name, ".ctor") &&
          mono_method_signature_internal(method)->param_count ==
              m_class_get_rank(method->klass))
        encode_value(0, p, &p);
      else if (!strcmp(method->name, ".ctor") &&
               mono_method_signature_internal(method)->param_count ==
                   m_class_get_rank(method->klass) * 2)
        encode_value(1, p, &p);
    ...
    if (image_index >= MONO_AOT_METHODREF_MIN) {
      ...
      encode_value((image_index << 24) | mono_metadata_token_index(token), p,
                   &p);
    }
  }
...
static guint32 get_shared_method_ref(MonoAotCompile *acfg, MonoMethod *method) {
  guint32 offset =
      GPOINTER_TO_UINT(g_hash_table_lookup(acfg->method_blob_hash, method));
  if (!offset) {
    guint8 *buf2, *p2;
    ...
    g_hash_table_insert(acfg->method_blob_hash, method,
                        GUINT_TO_POINTER(offset + 1));
  } else {
    offset--;
  }
...
static gint compare_patches(gconstpointer a, gconstpointer b) {
  int i, j;
...
static G_GNUC_UNUSED char *patch_to_string(MonoJumpInfo *patch_info) {
  GString *str;
  ...
  case MONO_PATCH_INFO_VTABLE:
    mono_type_get_desc(str, m_class_get_byval_arg(patch_info->data.klass),
                       TRUE);
    break;
  default:
    break;
 ...
 * PLT entry.
 */
static gboolean is_plt_patch(MonoJumpInfo *patch_info) {
  switch (patch_info->type) {
 ...
 */
static char *get_plt_symbol(MonoAotCompile *acfg, int plt_offset,
                            MonoJumpInfo *patch_info) {
#ifdef TARGET_MACH
  /*
   * The Apple linker reorganizes object files, so it doesn't like branches to
   * local labels, since those have no relocations.
   */
 ...
 */
static MonoPltEntry *get_plt_entry(MonoAotCompile *acfg,
                                   MonoJumpInfo *patch_info) {
  MonoPltEntry *res;
  ...
  if (!acfg->patch_to_plt_entry[patch_info->type])
    acfg->patch_to_plt_entry[patch_info->type] =
        g_hash_table_new(mono_patch_info_hash, mono_patch_info_equal);
  res = (MonoPltEntry *)g_hash_table_lookup(
      acfg->patch_to_plt_entry[patch_info->type], patch_info);

  if (!acfg->llvm && patch_info->type == MONO_PATCH_INFO_METHOD &&
      (patch_info->data.method->iflags & METHOD_IMPL_ATTRIBUTE_SYNCHRONIZED)) {
    /*
  ...
  if (!res) {
    ...
    res = (MonoPltEntry *)mono_mempool_alloc0(acfg->mempool,
                                              sizeof(MonoPltEntry));
    res->plt_offset = acfg->plt_offset;
    res->ji = new_ji;
    res->symbol = get_plt_symbol(acfg, res->plt_offset, patch_info);
    if (acfg->aot_opts.write_symbols)
      res->debug_sym = get_plt_entry_debug_sym(acfg, res->ji,
                                               acfg->plt_entry_debug_sym_cache);
    if (synchronized) {
      /* Avoid duplicate symbols because we don't cache */
      res->symbol =
          g_strdup_printf("%s_%d", res->symbol, synchronized_symbol_idx);
      if (res->debug_sym)
        res->debug_sym =
            g_strdup_printf("%s_%d", res->debug_sym, synchronized_symbol_idx);
      synchronized_symbol_idx++;
    }

    if (res->debug_sym)
      res->llvm_symbol =
          g_strdup_printf("%s_%s_llvm", res->symbol, res->debug_sym);
    else
      res->llvm_symbol = g_strdup_printf("%s_llvm", res->symbol);
    if (strstr(res->llvm_symbol, acfg->temp_prefix) == res->llvm_symbol) {
      /* The llvm symbol shouldn't be temporary, since the llvm generated object
       * file references it */
    ...
    g_hash_table_insert(acfg->plt_offset_to_entry,
                        GUINT_TO_POINTER(res->plt_offset), res);
...
static guint32 lookup_got_offset(MonoAotCompile *acfg, gboolean llvm,
                                 MonoJumpInfo *ji) {
  guint32 got_offset;
  GotInfo *info = llvm ? &acfg->llvm_got_info : &acfg->got_info;

  got_offset = GPOINTER_TO_UINT(
      g_hash_table_lookup(info->patch_to_got_offset_by_type[ji->type], ji));
  if (got_offset)
    return got_offset - 1;
  g_assert_not_reached();
}
 ...
 * get_got_offset:
 *
 *   Returns the offset of the GOT slot where the runtime object resulting from
 * resolving JI could be found if it exists, otherwise allocates a new one.
 */
static guint32 get_got_offset(MonoAotCompile *acfg, gboolean llvm,
                              MonoJumpInfo *ji) {
  guint32 got_offset;
  GotInfo *info = llvm ? &acfg->llvm_got_info : &acfg->got_info;
  ...
  g_hash_table_insert(info->patch_to_got_offset, ji,
                      GUINT_TO_POINTER(got_offset + 1));
  g_hash_table_insert(info->patch_to_got_offset_by_type[ji->type], ji,
                      GUINT_TO_POINTER(got_offset + 1));
...
static void add_method_with_index(MonoAotCompile *acfg, MonoMethod *method,
                                  int index, gboolean extra) {
  g_assert(method);
  if (!g_hash_table_lookup(acfg->method_indexes, method)) {
    g_ptr_array_add(acfg->methods, method);
    g_hash_table_insert(acfg->method_indexes, method,
...
static gboolean prefer_gsharedvt_method(MonoAotCompile *acfg,
                                        MonoMethod *method) {
  ...
  if (m_class_get_image(method->klass) == mono_defaults.corlib &&
      (!strcmp(m_class_get_name(method->klass), "AsyncMethodBuilderCore") ||
       !strcmp(m_class_get_name(method->klass), "AsyncVoidMethodBuilder")))
    return TRUE;
  else
    return FALSE;
}

static guint32 get_method_index(MonoAotCompile *acfg, MonoMethod *method) {
  int index =
      GPOINTER_TO_UINT(g_hash_table_lookup(acfg->method_indexes, method));
...
static int add_method_full(MonoAotCompile *acfg, MonoMethod *method,
                           gboolean extra, int depth) {
  int index;
...
static int add_method(MonoAotCompile *acfg, MonoMethod *method) {
  return add_method_full(acfg, method, FALSE, 0);
}

static void mono_dedup_cache_method(MonoAotCompile *acfg, MonoMethod *method) {
  ...
  guint count =
      GPOINTER_TO_UINT(g_hash_table_lookup(acfg->dedup_stats, stats_name));
  count++;
  g_hash_table_insert(acfg->dedup_stats, stats_name, GUINT_TO_POINTER(count));
}

static void add_extra_method_with_depth(MonoAotCompile *acfg,
                                        MonoMethod *method, int depth) {
  ERROR_DECL(error);
  ...
  } else if ((acfg->jit_opts & MONO_OPT_GSHAREDVT) &&
             prefer_gsharedvt_method(acfg, method) &&
             mono_method_is_generic_sharable_full(method, FALSE, FALSE, TRUE)) {
...
static void add_extra_method(MonoAotCompile *acfg, MonoMethod *method) {
  add_extra_method_with_depth(acfg, method, 0);
}

static void add_jit_icall_wrapper(MonoAotCompile *acfg,
...
static void add_lazy_init_wrappers(MonoAotCompile *acfg) {
...
static MonoMethod *get_runtime_invoke_sig(MonoMethodSignature *sig) {
...
static MonoMethod *get_runtime_invoke(MonoAotCompile *acfg, MonoMethod *method,
                                      gboolean virtual_) {
  return mono_marshal_get_runtime_invoke(method, virtual_);
}

static gboolean can_marshal_struct(MonoClass *klass) {
  ...
  while ((field = mono_class_get_fields_internal(klass, &iter))) {
    ...
    case MONO_TYPE_STRING:
      break;
    case MONO_TYPE_VALUETYPE:
      if (!m_class_is_enumtype(
              mono_class_from_mono_type_internal(field->type)) &&
          !can_marshal_struct(mono_class_from_mono_type_internal(field->type)))
        can_marshal = FALSE;
      break;
    case MONO_TYPE_SZARRAY: {
  ...
  if (!strcmp(m_class_get_name_space(klass),
              "System.Net.NetworkInformation.MacOsStructs") &&
      strcmp(m_class_get_name(klass), "sockaddr_dl"))
    return TRUE;

  return can_marshal;
}

static void create_gsharedvt_inst(MonoAotCompile *acfg, MonoMethod *method,
  ...
  if (method->is_generic) {
    ...
    for (i = 0; i < container->type_argc; ++i) {
      MonoGenericParamInfo *info =
          mono_generic_param_info(&container->type_params[i]);
      ...
      if (info && info->constraints) {
        ...
        while (*constraints) {
          ...
          if (!(cklass == mono_defaults.object_class ||
                (m_class_get_image(cklass) == mono_defaults.corlib &&
                 !strcmp(m_class_get_name(cklass), "ValueType"))))
...
static void add_gc_wrappers(MonoAotCompile *acfg) {
  MonoMethod *m;
  /* Managed Allocators */
  int nallocators = mono_gc_get_managed_allocator_types();
  for (int i = 0; i < nallocators; ++i) {
    if ((m = mono_gc_get_managed_allocator_by_type(i,
                                                   MANAGED_ALLOCATOR_REGULAR)))
      add_method(acfg, m);
    if ((m = mono_gc_get_managed_allocator_by_type(
...
contains_disable_reflection_attribute(MonoCustomAttrInfo *cattr) {
  for (int i = 0; i < cattr->num_attrs; ++i) {
    ...
    if (strcmp(m_class_get_name_space(attr->ctor->klass),
               "System.Runtime.CompilerServices"))
      return FALSE;

    if (strcmp(m_class_get_name(attr->ctor->klass),
...
gboolean mono_aot_can_specialize(MonoMethod *method) {
  ...
  MonoCustomAttrInfo *cattr =
      mono_custom_attrs_from_class_checked(method->klass, cattr_error);
...
static gboolean always_aot(MonoMethod *method) {
  /*
   * Calls to these methods do not go through the normal call processing code so
   * calling code cannot enter the interpreter. So always aot them even in
   * profile guided aot mode.
   */
  if (method->klass == mono_get_string_class() &&
      (strstr(method->name, "memcpy") || strstr(method->name, "bzero")))
    return TRUE;
...
gboolean mono_aot_can_enter_interp(MonoMethod *method) {
  ...
  if (acfg->aot_opts.profile_only &&
      !g_hash_table_lookup(acfg->profile_methods, method))
    return TRUE;
  return FALSE;
}

static void add_wrappers(MonoAotCompile *acfg) {
  MonoMethod *method, *m;
  int i, j;
  ...
  /*
   * FIXME: Instead of AOTing all the wrappers, it might be better to redesign
   * them so there is only one wrapper of a given type, or inlining their
   * contents into their callers.
   */
  for (i = 0; i < acfg->image->tables[MONO_TABLE_METHOD].rows; ++i) {
    ...
    report_loader_error(acfg, error, TRUE,
      ...
      for (j = 0; j < sig->param_count; j++) {
        if (sig->params[j]->type == MONO_TYPE_GENERICINST &&
            mono_class_is_nullable(
                mono_class_from_mono_type_internal(sig->params[j])))
          has_nullable = TRUE;
      }
  ...
  if (mono_is_corlib_image(acfg->image->assembly->image)) {
    ...
    csig->hasthis = 1;
    csig->ret = m_class_get_byval_arg(mono_class_load_from_name(
        mono_defaults.corlib, "System.Reflection", "Assembly"));
    csig->params[0] = string_type;
    csig->params[1] = m_class_get_byval_arg(mono_class_load_from_name(
        mono_defaults.corlib, "System.Reflection", "Assembly"));
    ...
    add_method(acfg,
               get_runtime_invoke(acfg,
                                  get_method_nofail(mono_defaults.object_class,
                                                    "Finalize", 0, 0),
                                  TRUE));
    ...
    /* These are used by mono_jit_runtime_invoke () to calls gsharedvt out
     * wrappers */
    if (acfg->aot_opts.llvm_only) {
      int variants;

      /* Create simplified signatures which match the signature used by the
       * gsharedvt out wrappers */
      for (variants = 0; variants < 4; ++variants) {
        for (i = 0; i < 40; ++i) {
          sig = mini_get_gsharedvt_out_sig_wrapper_signature(
              (variants & 1) > 0, (variants & 2) > 0, i);
    ...
    /* JIT icall wrappers */
    /* FIXME: locking - this is "safe" as full-AOT threads don't mutate the
     * icall data */
  ...
  /*
   * remoting-invoke-with-check wrappers are very frequent, so avoid emitting
   * them, we use the original method instead at runtime. Since full-aot doesn't
   * support remoting, this is not a problem.
   */
#if 0
    ...
    if (!mono_class_is_gtd(klass)) {
      ...
      if (cattr) {
        ...
        for (j = 0; j < cattr->num_attrs; ++j)
          if (cattr->attrs[j].ctor &&
              (!strcmp(m_class_get_name(cattr->attrs[j].ctor->klass),
        ...
        if (j < cattr->num_attrs) {
          ...
          del_invoke = mono_marshal_get_delegate_invoke_internal(invoke, FALSE,
                                                                 TRUE, wrapper);
  ...
  for (i = 0; i < acfg->image->tables[MONO_TABLE_TYPESPEC].rows; ++i) {
    ...
    if (m_class_get_rank(klass) && MONO_TYPE_IS_PRIMITIVE(m_class_get_byval_arg(
  ...
  for (i = 0; i < acfg->image->tables[MONO_TABLE_METHOD].rows; ++i) {
    ...
    if (method->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) {
      if (acfg->aot_opts.llvm_only) {
        /* The wrappers have a different signature (hasthis is not set) so need
         * to add this too */
        add_gsharedvt_wrappers(acfg, mono_method_signature_internal(method),
                               FALSE, TRUE, FALSE);
      }
    }
  }
  ...
  for (i = 0; i < acfg->image->tables[MONO_TABLE_METHOD].rows; ++i) {
     ...
     * attribute named MonoPInvokeCallbackAttribute. We search for the attribute
     * by name to avoid defining a new assembly to contain it.
     */
    ...
    if (cattr) {
      ...
      if (j < cattr->num_attrs) {
        ...
        /* this cannot be enforced by the C# compiler so we must give the user
         * some warning before aborting */
        if (!(method->flags & METHOD_ATTRIBUTE_STATIC)) {
          g_warning("AOT restriction: Method '%s' must be static since it is "
                    "decorated with [MonoPInvokeCallback]. See "
                    "https://docs.microsoft.com/xamarin/ios/internals/"
                    "limitations#reverse-callbacks",
                    mono_method_full_name(method, TRUE));
          exit(1);
        }

        g_assert(sig->param_count == 1);
        g_assert(sig->params[0]->type == MONO_TYPE_CLASS &&
                 !strcmp(m_class_get_name(mono_class_from_mono_type_internal(
                             sig->params[0])),
                         "Type"));
        ...
        n[slen] = 0;
        t = mono_reflection_type_from_name_checked(
            n, mono_domain_ambient_alc(mono_domain_get()), acfg->image, error);
        g_assert(t);
        mono_error_assert_ok(error);
        g_free(n);

        klass = mono_class_from_mono_type_internal(t);
        g_assert(m_class_get_parent(klass) ==
  ...
  for (i = 0; i < acfg->image->tables[MONO_TABLE_TYPEDEF].rows; ++i) {
    ...
    if (m_class_is_valuetype(klass) && !mono_class_is_gtd(klass) &&
        can_marshal_struct(klass) &&
        !(m_class_get_nested_in(klass) &&
          strstr(m_class_get_name(m_class_get_nested_in(klass)),
                 "<PrivateImplementationDetails>") ==
              m_class_get_name(m_class_get_nested_in(klass)))) {
...
static gboolean has_type_vars(MonoClass *klass) {
  if ((m_class_get_byval_arg(klass)->type == MONO_TYPE_VAR) ||
      (m_class_get_byval_arg(klass)->type == MONO_TYPE_MVAR))
    return TRUE;
  ...
  if (mono_class_is_ginst(klass)) {
    ...
    if (context->class_inst) {
      ...
      for (i = 0; i < context->class_inst->type_argc; ++i)
        if (has_type_vars(mono_class_from_mono_type_internal(
                context->class_inst->type_argv[i])))
          return TRUE;
    }
  }
  if (mono_class_is_gtd(klass))
...
static gboolean is_vt_inst(MonoGenericInst *inst) {
  int i;
...
static gboolean method_has_type_vars(MonoMethod *method) {
  ...
  if (method->is_inflated) {
    ...
    if (context->method_inst) {
      ...
      for (i = 0; i < context->method_inst->type_argc; ++i)
        if (has_type_vars(mono_class_from_mono_type_internal(
                context->method_inst->type_argv[i])))
          return TRUE;
    }
  }
  return FALSE;
...
static gboolean mono_aot_mode_is_full(MonoAotOptions *opts) {
  return opts->mode == MONO_AOT_MODE_FULL;
}

static gboolean mono_aot_mode_is_interp(MonoAotOptions *opts) {
  return opts->interp;
}

static gboolean mono_aot_mode_is_hybrid(MonoAotOptions *opts) {
  return opts->mode == MONO_AOT_MODE_HYBRID;
}

static void add_generic_class_with_depth(MonoAotCompile *acfg, MonoClass *klass,
                                         int depth, const char *ref);

static void add_generic_class(MonoAotCompile *acfg, MonoClass *klass,
                              gboolean force, const char *ref) {
  /* This might lead to a huge code blowup so only do it if neccesary */
  if (!mono_aot_mode_is_full(&acfg->aot_opts) &&
      !mono_aot_mode_is_hybrid(&acfg->aot_opts) && !force)
    return;

  add_generic_class_with_depth(acfg, klass, 0, ref);
}

static gboolean check_type_depth(MonoType *t, int depth) {
  int i;
...
static void add_types_from_method_header(MonoAotCompile *acfg,
                                         MonoMethod *method);
...
static void add_generic_class_with_depth(MonoAotCompile *acfg, MonoClass *klass,
  ...
  /*
   * Use gsharedvt for generic collections with vtype arguments to avoid code
   * blowup. Enable this only for some classes since gsharedvt might not support
   * all methods.
   */
  if ((acfg->jit_opts & MONO_OPT_GSHAREDVT) &&
      ...
      mono_class_get_generic_class(klass)->context.class_inst &&
      is_vt_inst(mono_class_get_generic_class(klass)->context.class_inst) &&
      (!strcmp(m_class_get_name(klass), "Dictionary`2") ||
       !strcmp(m_class_get_name(klass), "List`1") ||
       !strcmp(m_class_get_name(klass), "ReadOnlyCollection`1")))
    use_gsharedvt = TRUE;

  iter = NULL;
  while ((method = mono_class_get_methods(klass, &iter))) {
    if ((acfg->jit_opts & MONO_OPT_GSHAREDVT) && method->is_inflated &&
    ...
    if (mono_method_is_generic_sharable_full(method, FALSE, FALSE,
                                             use_gsharedvt)) {
      /* Already added */
  ...
  while ((field = mono_class_get_fields_internal(klass, &iter))) {
    if (field->type->type == MONO_TYPE_GENERICINST)
      add_generic_class_with_depth(
          acfg, mono_class_from_mono_type_internal(field->type), depth + 1,
          "field");
  }
  ...
  if (m_class_get_parent(klass))
    add_generic_class_with_depth(acfg, m_class_get_parent(klass), depth,
                                 "parent");
  ...
  if (in_corlib && !strcmp(klass_name_space, "System.Collections.Generic") &&
      (!strcmp(klass_name, "ICollection`1") ||
       !strcmp(klass_name, "IEnumerable`1") || !strcmp(klass_name, "IList`1") ||
       !strcmp(klass_name, "IEnumerator`1") ||
       !strcmp(klass_name, "IReadOnlyList`1"))) {
    MonoClass *tclass = mono_class_from_mono_type_internal(
    ...
    if (!strcmp(klass_name, "IEnumerable`1") ||
        !strcmp(klass_name, "IEnumerator`1")) {
      ERROR_DECL(error);
      MonoClass *nclass;

      iter = NULL;
      while ((nclass = mono_class_get_nested_types(
                  m_class_get_parent(array_class), &iter))) {
        if (!strcmp(m_class_get_name(nclass), "InternalEnumerator`1"))
          break;
      }
      g_assert(nclass);
      nclass = mono_class_inflate_generic_class_checked(
          nclass,
  ...
  /* Add an instance of GenericComparer<T> which is created dynamically by
   * Comparer<T> */
  if (in_corlib && !strcmp(klass_name_space, "System.Collections.Generic") &&
      !strcmp(klass_name, "Comparer`1")) {
    ERROR_DECL(error);
    MonoClass *tclass = mono_class_from_mono_type_internal(
        mono_class_get_generic_class(klass)->context.class_inst->type_argv[0]);
    ...
    icomparable = mono_class_load_from_name(mono_defaults.corlib, "System",
                                            "IComparable`1");
    ...
    if (mono_class_is_assignable_from_internal(icomparable_inst, tclass)) {
      ...
      gcomparer = mono_class_load_from_name(mono_defaults.corlib,
  ...
  /* Add an instance of GenericEqualityComparer<T> which is created dynamically
   * by EqualityComparer<T> */
  if (in_corlib && !strcmp(klass_name_space, "System.Collections.Generic") &&
      !strcmp(klass_name, "EqualityComparer`1")) {
    ERROR_DECL(error);
    MonoClass *tclass = mono_class_from_mono_type_internal(
        mono_class_get_generic_class(klass)->context.class_inst->type_argv[0]);
    ...
    iface = mono_class_load_from_name(mono_defaults.corlib, "System",
                                      "IEquatable`1");
    g_assert(iface);
    MonoType *args[] = {m_class_get_byval_arg(tclass)};
    ctx.class_inst = mono_metadata_get_generic_inst(1, args);
  ...
  /* Add an instance of EnumComparer<T> which is created dynamically by
   * EqualityComparer<T> for enums */
  if (in_corlib && !strcmp(klass_name_space, "System.Collections.Generic") &&
      !strcmp(klass_name, "EqualityComparer`1")) {
    ...
    if (m_class_is_enumtype(tclass)) {
      ...
      enum_comparer = mono_class_load_from_name(mono_defaults.corlib,
                                                "System.Collections.Generic",
                                                "EnumEqualityComparer`1");
      enum_comparer_inst =
          mono_class_inflate_generic_class_checked(enum_comparer, &ctx, error);
      mono_error_assert_ok(error); /* FIXME don't swallow the error */
      add_generic_class(acfg, enum_comparer_inst, FALSE, "EqualityComparer<T>");
    }
  }

  /* Add an instance of ObjectComparer<T> which is created dynamically by
   * Comparer<T> for enums */
  if (in_corlib && !strcmp(klass_name_space, "System.Collections.Generic") &&
      !strcmp(klass_name, "Comparer`1")) {
    MonoClass *comparer;
    MonoClass *tclass = mono_class_from_mono_type_internal(
    ...
    if (m_class_is_enumtype(tclass)) {
      ...
      comparer = mono_class_load_from_name(mono_defaults.corlib,
                                           "System.Collections.Generic",
                                           "ObjectComparer`1");
      comparer_inst =
          mono_class_inflate_generic_class_checked(comparer, &ctx, error);
      mono_error_assert_ok(error); /* FIXME don't swallow the error */
...
static void add_instances_of(MonoAotCompile *acfg, MonoClass *klass,
                             MonoType **insts, int ninsts, gboolean force) {
  int i;
  MonoGenericContext ctx;
...
static void add_types_from_method_header(MonoAotCompile *acfg,
  ...
  if (sig) {
    for (j = 0; j < sig->param_count; ++j)
      if (sig->params[j]->type == MONO_TYPE_GENERICINST)
        add_generic_class_with_depth(
            acfg, mono_class_from_mono_type_internal(sig->params[j]), depth + 1,
            "arg");
  }
  ...
  if (header) {
    for (j = 0; j < header->num_locals; ++j)
      if (header->locals[j]->type == MONO_TYPE_GENERICINST)
        add_generic_class_with_depth(
            acfg, mono_class_from_mono_type_internal(header->locals[j]),
            depth + 1, "local");
 ...
 */
static void add_generic_instances(MonoAotCompile *acfg) {
  int i;
  guint32 token;
  MonoMethod *method;
  ...
  for (i = 0; i < acfg->image->tables[MONO_TABLE_METHODSPEC].rows; ++i) {
    ...
    if (context && context->method_inst && context->method_inst->is_open) {
      ...
      if (context->class_inst) {
        ...
        for (i = 0; i < inst->type_argc; ++i) {
          if (MONO_TYPE_IS_REFERENCE(inst->type_argv[i]) ||
              inst->type_argv[i]->type == MONO_TYPE_VAR ||
              inst->type_argv[i]->type == MONO_TYPE_MVAR)
            continue;
      ...
      if (inst) {
        ...
        shared_context.class_inst =
            mono_metadata_get_generic_inst(inst->type_argc, type_argv);
        g_free(type_argv);
      }
      ...
      method = mono_class_inflate_generic_method_checked(
          declaring_method, &shared_context, error);
      g_assert(is_ok(error)); /* FIXME don't swallow the error */
    }
  ...
  if (acfg->image == mono_defaults.corlib) {
     ...
     */
    klass = mono_class_try_load_from_name(
        acfg->image, "System.Collections.Generic", "GenericComparer`1");
    if (klass)
      add_instances_of(acfg, klass, insts, ninsts, TRUE);
    klass = mono_class_try_load_from_name(
        acfg->image, "System.Collections.Generic", "GenericEqualityComparer`1");
    if (klass)
      add_instances_of(acfg, klass, insts, ninsts, TRUE);

    /* Add instances of EnumEqualityComparer which are created by
     * EqualityComparer<T> for enums */
    {
      MonoClass *enum_comparer;
      MonoType *insts[16];
     ...
     */
    klass = mono_class_try_load_from_name(
        acfg->image, "System.Collections.Generic", "ICollection`1");
    if (klass)
      add_instances_of(acfg, klass, insts, ninsts, TRUE);

    klass = mono_class_try_load_from_name(
        acfg->image, "System.Collections.Generic", "IList`1");
    if (klass)
      add_instances_of(acfg, klass, insts, ninsts, TRUE);

    klass = mono_class_try_load_from_name(
        acfg->image, "System.Collections.Generic", "IEnumerable`1");
    if (klass)
      add_instances_of(acfg, klass, insts, ninsts, TRUE);

    /*
     * Add a managed-to-native wrapper of Array.GetGenericValue_icall<object>,
     * which is used for all instances of GetGenericValue_icall by the AOT
     * runtime.
     */
    {
      ERROR_DECL(error);
      ...
      MonoClass *array_klass = m_class_get_parent(
          mono_class_create_array(mono_defaults.object_class, 1));

      get_method = mono_class_get_method_from_name_checked(
          array_klass, "GetGenericValue_icall", 3, 0, error);
      mono_error_assert_ok(error);
      ...
      if (get_method) {
        ...
        add_extra_method(acfg, mono_marshal_get_native_wrapper(
    ...
    for (i = 1; i < 4; ++i) {
      MonoClass *obj_array_class =
          mono_class_create_array(mono_defaults.object_class, i);
      MonoMethod *m;
...
static char *decode_direct_icall_symbol_name_attribute(MonoMethod *method) {
  ERROR_DECL(error);
  ...
  MonoCustomAttrInfo *cattr =
      mono_custom_attrs_from_method_checked(method, error);
  if (is_ok(error) && cattr) {
    ...
    if (j < cattr->num_attrs) {
      MonoCustomAttrEntry *e = &cattr->attrs[j];
      MonoMethodSignature *sig = mono_method_signature_internal(e->ctor);
      if (e->data && sig && sig->param_count == 1 &&
          sig->params[0]->type == MONO_TYPE_STRING) {
        /*
         * Decode the cattr manually since we can't create objects
         * during aot compilation.
         */
...
static const char *lookup_external_icall_symbol_name_aot(MonoMethod *method) {
  g_assert(method_to_external_icall_symbol_name);

  gpointer key, value;
  if (g_hash_table_lookup_extended(method_to_external_icall_symbol_name, method,
                                   &key, &value))
    return (const char *)value;

  char *symbol_name = decode_direct_icall_symbol_name_attribute(method);
  g_hash_table_insert(method_to_external_icall_symbol_name, method,
                      symbol_name);

  return symbol_name;
}

static const char *lookup_icall_symbol_name_aot(MonoMethod *method) {
...
gboolean mono_aot_direct_icalls_enabled_for_method(MonoCompile *cfg,
                                                   MonoMethod *method) {
 ...
 */
static gboolean method_is_externally_callable(MonoAotCompile *acfg,
                                              MonoMethod *method) {
  // FIXME: Unify
  if (acfg->aot_opts.llvm_only) {
    ...
    if (method->is_inflated)
      return FALSE;
    if (!((mono_class_get_flags(method->klass) & TYPE_ATTRIBUTE_PUBLIC) &&
 ...
 */
static gboolean is_direct_callable(MonoAotCompile *acfg, MonoMethod *method,
                                   MonoJumpInfo *patch_info) {
  if ((patch_info->type == MONO_PATCH_INFO_METHOD) &&
      (m_class_get_image(patch_info->data.method->klass) == acfg->image)) {
    MonoCompile *callee_cfg = (MonoCompile *)g_hash_table_lookup(
        acfg->method_to_cfg, patch_info->data.method);
    if (callee_cfg) {
      gboolean direct_callable = TRUE;

      if (direct_callable &&
          (acfg->aot_opts.dedup || acfg->aot_opts.dedup_include) &&
          mono_aot_can_dedup(patch_info->data.method))
        direct_callable = FALSE;

      if (direct_callable && !acfg->llvm &&
          !(!callee_cfg->has_got_slots &&
      ...
      //
      // FIXME: Support inflated methods, it asserts in
      // mini_llvm_init_gshared_method_this () because the method is not in
      ...
      if (direct_callable &&
          (callee_cfg->method->iflags & METHOD_IMPL_ATTRIBUTE_SYNCHRONIZED) &&
          (!method || method->wrapper_type != MONO_WRAPPER_SYNCHRONIZED))
        // FIXME: Maybe call the wrapper directly ?
        direct_callable = FALSE;

      if (direct_callable &&
          (acfg->aot_opts.soft_debug || acfg->aot_opts.no_direct_calls)) {
      ...
      if (direct_callable &&
          (callee_cfg->method->wrapper_type == MONO_WRAPPER_ALLOC))
        /* sgen does some initialization when the allocator method is created */
        direct_callable = FALSE;
      if (direct_callable &&
          (callee_cfg->method->wrapper_type == MONO_WRAPPER_WRITE_BARRIER))
  ...
  } else if ((patch_info->type == MONO_PATCH_INFO_METHOD) &&
             (m_class_get_image(patch_info->data.method->klass) !=
              acfg->image)) {
...
#ifdef MONO_ARCH_AOT_SUPPORTED
static const char *get_pinvoke_import(MonoAotCompile *acfg,
                                      MonoMethod *method) {
  ...
  import = g_strdup_printf(
      "%s", mono_metadata_string_heap(image, im_cols[MONO_IMPLMAP_NAME]));
...
static gint compare_lne(MonoDebugLineNumberEntry *a,
                        MonoDebugLineNumberEntry *b) {
 ...
 *
 * Returns a sparse array of size CODE_SIZE containing MonoDebugSourceLocation*
 * entries for the native offsets which have a corresponding line number entry.
 */
static MonoDebugSourceLocation **
compute_line_numbers(MonoMethod *method, int code_size,
  ...
  if (!minfo)
    return NULL;
  // FIXME: This seems to happen when two methods have the same
  // cfg->method_to_register
  ...
  memcpy(ln_array, debug_info->line_numbers,
         debug_info->num_line_numbers * sizeof(MonoDebugLineNumberEntry));

  mono_qsort(ln_array, debug_info->num_line_numbers,
...
static int get_file_index(MonoAotCompile *acfg, const char *source_file) {
  int findex;

  // FIXME: Free these
  if (!acfg->dwarf_ln_filenames)
    acfg->dwarf_ln_filenames = g_hash_table_new(g_str_hash, g_str_equal);
  findex = GPOINTER_TO_INT(
      g_hash_table_lookup(acfg->dwarf_ln_filenames, source_file));
  if (!findex) {
    findex = g_hash_table_size(acfg->dwarf_ln_filenames) + 1;
    g_hash_table_insert(acfg->dwarf_ln_filenames, g_strdup(source_file),
 ...
 * emit_and_reloc_code:
 *
 *   Emit the native code in CODE, handling relocations along the way. If
 * GOT_ONLY is true, calls are made through the GOT too. This is used for
 * emitting trampolines in full-aot mode, since calls made from trampolines
 * couldn't go through the PLT, since trampolines are needed to make PLT work.
 */
static void emit_and_reloc_code(MonoAotCompile *acfg, MonoMethod *method,
                                guint8 *code, guint32 code_len,
                                MonoJumpInfo *relocs, gboolean got_only,
                                MonoDebugMethodJitInfo *debug_info) {
  ...
  for (i = 0; i < code_len; i += INST_LEN) {
    ...
    if (patch_info && (patch_info->ip.i == i) && (pindex < patches->len)) {
      ...
      case MONO_PATCH_INFO_OBJC_SELECTOR_REF: {
        ...
        if (!acfg->objc_selector_to_index)
          acfg->objc_selector_to_index =
              g_hash_table_new(g_str_hash, g_str_equal);
        if (!acfg->objc_selectors)
          acfg->objc_selectors = g_ptr_array_new();
        index = GPOINTER_TO_UINT(
            g_hash_table_lookup(acfg->objc_selector_to_index, selector));
        if (index)
          index--;
        else {
          index = acfg->objc_selector_index;
          g_ptr_array_add(acfg->objc_selectors,
                          (void *)patch_info->data.target);
          g_hash_table_insert(acfg->objc_selector_to_index, selector,
                              GUINT_TO_POINTER(index + 1));
      ...
      default: {
        ...
        if (patch_info->type == MONO_PATCH_INFO_METHOD) {
          ...
          if (cmethod->wrapper_type == MONO_WRAPPER_OTHER &&
              mono_marshal_get_wrapper_info(cmethod)->subtype ==
                  WRAPPER_SUBTYPE_AOT_INIT) {
            WrapperInfo *info = mono_marshal_get_wrapper_info(cmethod);

            /*
             * This is a call from a JITted method to the init wrapper emitted
             * by LLVM.
             */
            g_assert(acfg->aot_opts.llvm && acfg->aot_opts.direct_extern_calls);

            const char *init_name = mono_marshal_get_aot_init_wrapper_name(
                info->d.aot_init.subtype);
            char *symbol = g_strdup_printf("%s%s_%s", acfg->user_symbol_prefix,
                                           acfg->global_prefix, init_name);

            direct_call = TRUE;
            direct_call_target = symbol;
            patch_info->type = MONO_PATCH_INFO_NONE;
          } else if ((m_class_get_image(patch_info->data.method->klass) ==
                      acfg->image) &&
                     !got_only &&
                     is_direct_callable(acfg, method, patch_info)) {
            MonoCompile *callee_cfg = (MonoCompile *)g_hash_table_lookup(
                acfg->method_to_cfg, cmethod);
            ...
            if (acfg->aot_opts.dedup && !mono_aot_can_dedup(cmethod)) {
              ...
              mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT,
          ...
          if (!got_only && is_direct_callable(acfg, method, patch_info)) {
            if (!(patch_info->data.method->flags &
                  METHOD_ATTRIBUTE_PINVOKE_IMPL))
              direct_pinvoke =
                  lookup_icall_symbol_name_aot(patch_info->data.method);
            else
              direct_pinvoke =
                  get_pinvoke_import(acfg, patch_info->data.method);
            if (direct_pinvoke) {
              direct_call = TRUE;
              g_assert(strlen(direct_pinvoke) < 1000);
              direct_call_target = g_strdup_printf(
                  "%s%s", acfg->user_symbol_prefix, direct_pinvoke);
            }
          }
        } else if (patch_info->type == MONO_PATCH_INFO_JIT_ICALL_ADDR) {
          const char *sym =
              mono_find_jit_icall_info(patch_info->data.jit_icall_id)->c_symbol;
          if (!got_only && sym && acfg->aot_opts.direct_icalls) {
            ...
            direct_call_target =
                g_strdup_printf("%s%s", acfg->user_symbol_prefix, sym);
          }
        } else if (patch_info->type == MONO_PATCH_INFO_JIT_ICALL_ID) {
          MonoJitICallInfo *const info =
              mono_find_jit_icall_info(patch_info->data.jit_icall_id);
          const char *const sym = info->c_symbol;
          if (!got_only && sym && acfg->aot_opts.direct_icalls &&
              info->func == info->wrapper) {
        ...
        if (!got_only && !direct_call) {
          ...
          if (plt_entry) {
            /* This patch has a PLT entry, so we must emit a call to the PLT
             * entry */
            direct_call = TRUE;
        ...
        if (direct_call) {
          ...
          arch_emit_direct_call(acfg, direct_call_target, external_call, FALSE,
                                patch_info, &call_size);
          ...
          arch_emit_got_access(acfg, acfg->got_symbol, code + i, got_slot,
                               &code_size);
          i += code_size - INST_LEN;
        }
        skip = TRUE;
    ...
    if (!skip) {
      ...
      if (pindex < patches->len && patch_info->ip.i > i) {
        ...
        for (limit = i + INST_LEN; limit < patch_info->ip.i;
             limit += INST_LEN) {
          if (locs && locs[limit])
            break;
        }
 ...
 * sanitize_symbol:
 *
 *   Return a modified version of S which only includes characters permissible
 * in symbols.
 */
static char *sanitize_symbol(MonoAotCompile *acfg, char *s) {
...
static char *get_debug_sym(MonoMethod *method, const char *prefix,
                           GHashTable *cache) {
...
#ifdef TARGET_MACH
  // This is so that we don't accidentally create a local symbol (which starts
  // with 'L')
...
static void emit_method_code(MonoAotCompile *acfg, MonoCompile *cfg) {
  MonoMethod *method;
  int method_index;
  guint8 *code;
  char *debug_sym = NULL;
  ...
  if (acfg->aot_opts.write_symbols && !acfg->global_symbols && !acfg->llvm) {
    ...
    /*if (mono_aot_can_dedup (method) && (acfg->aot_opts.dedup ||
     * acfg->aot_opts.dedup_include))*/
  ...
  emit_and_reloc_code(
      acfg, method, code, cfg->code_len, cfg->patch_info, FALSE,
      mono_debug_find_method(cfg->jit_info->d.method, mono_domain_get()));
  ...
  arch_emit_unwind_info_sections(acfg, cfg->asm_symbol, symbol,
                                 cfg->unwind_ops);
 ...
 */
static void encode_patch(MonoAotCompile *acfg, MonoJumpInfo *patch_info,
                         guint8 *buf, guint8 **endbuf) {
  guint8 *p = buf;
  ...
  case MONO_PATCH_INFO_EXC_NAME: {
    ...
    ex_class = mono_class_load_from_name(
        m_class_get_image(mono_defaults.exception_class), "System",
        (const char *)patch_info->data.target);
  ...
  case MONO_PATCH_INFO_RGCTX_SLOT_INDEX: {
    ...
    /*
     * entry->d.klass/method has a lenghtly encoding and multiple rgctx_fetch
     * entries reference the same klass/method, so encode it only once. For
     * patches which refer to got entries, this sharing is done by
     * get_got_offset, but these are not got entries.
     */
    if (entry->in_mrgctx) {
    ...
    encode_value((entry->in_mrgctx ? 1 : 0) | (entry->info_type << 1) |
                     (entry->data->type << 9),
                 p, &p);
  ...
  case MONO_PATCH_INFO_GSHAREDVT_IN_WRAPPER:
    encode_signature(acfg, (MonoMethodSignature *)patch_info->data.target, p,
                     &p);
    break;
  case MONO_PATCH_INFO_GSHAREDVT_CALL:
    encode_signature(
        acfg, (MonoMethodSignature *)patch_info->data.gsharedvt->sig, p, &p);
  ...
  case MONO_PATCH_INFO_GSHAREDVT_METHOD: {
    ...
    for (i = 0; i < info->num_entries; ++i) {
      ...
      case MONO_PATCH_INFO_CLASS:
        encode_klass_ref(
            acfg,
            mono_class_from_mono_type_internal((MonoType *)template_->data), p,
            &p);
        break;
      case MONO_PATCH_INFO_FIELD:
...
static void encode_patch_list(MonoAotCompile *acfg, GPtrArray *patches,
                              int n_patches, gboolean llvm, guint8 *buf,
                              guint8 **endbuf) {
  guint8 *p = buf;
...
static void emit_method_info(MonoAotCompile *acfg, MonoCompile *cfg) {
  MonoMethod *method;
  ...
  for (pindex = 0; pindex < patches->len; ++pindex) {
    ...
    if ((patch_info->type == MONO_PATCH_INFO_IMAGE) &&
        (patch_info->data.image == acfg->image)) {
    ...
    if (is_plt_patch(patch_info) &&
        !(cfg->compile_llvm && acfg->aot_opts.llvm_only)) {
  ...
  if (mini_jit_info_is_gsharedvt(cfg->jit_info) &&
      mini_is_gsharedvt_variable_signature(
          mono_method_signature_internal(jinfo_get_method(cfg->jit_info))))
    flags |= MONO_AOT_METHOD_FLAG_GSHAREDVT_VARIABLE;
  if (n_patches)
    flags |= MONO_AOT_METHOD_FLAG_HAS_PATCHES;
  if (needs_ctx && ctx)
    flags |= MONO_AOT_METHOD_FLAG_HAS_CTX;
  /* Saved into another table so it can be accessed without having access to
   * this data */
  ...
  if (cfg->compile_llvm) {
    ...
    cfg->llvm_info_var =
        mono_llvm_emit_aot_data_aligned(symbol, buf, p - buf, 1);
    g_free(symbol);
...
static guint32 get_unwind_info_offset(MonoAotCompile *acfg, guint8 *encoded,
  ...
  offset = GPOINTER_TO_UINT(g_hash_table_lookup(
      acfg->unwind_info_offsets, GUINT_TO_POINTER(cache_index + 1)));
  if (offset)
    return offset - 1;
  else {
    guint8 buf[16];
    ...
    g_hash_table_insert(acfg->unwind_info_offsets,
                        GUINT_TO_POINTER(cache_index + 1),
                        GUINT_TO_POINTER(offset + 1));
...
static void emit_exception_debug_info(MonoAotCompile *acfg, MonoCompile *cfg,
  ...
  seq_points_size =
      (store_seq_points) ? mono_seq_point_info_get_write_size(seq_points) : 0;

  buf_size = header->num_clauses * 256 + debug_info_size + 2048 +
             seq_points_size + cfg->gc_map_size;
  ...
  flags = (jinfo->has_generic_jit_info ? 1 : 0) | (use_unwind_ops ? 2 : 0) |
          (header->num_clauses ? 4 : 0) | (seq_points_size ? 8 : 0) |
          (cfg->compile_llvm ? 16 : 0) | (jinfo->has_try_block_holes ? 32 : 0) |
          (cfg->gc_map ? 64 : 0) | (jinfo->has_arch_eh_info ? 128 : 0);
  ...
  /*Encode the number of holes before the number of clauses to make decoding
   * easier*/
  if (jinfo->has_try_block_holes) {
    MonoTryBlockHoleTableJitInfo *table =
        mono_jit_info_get_try_block_hole_table_info(jinfo);
  ...
  if (jinfo->has_arch_eh_info) {
    /*
     * In AOT mode, the code length is calculated from the address of the
     * previous method, which could include alignment padding, so calculating
     * the start of the epilog as code_len - epilog_size is correct any more.
     * Save the real code len as a workaround.
     */
  ...
  if (cfg->compile_llvm) {
    /*
     * When using LLVM, we can't emit some data, like pc offsets, this
     * reg/offset etc., since the information is only available to llc. Instead,
     * we let llc save the data into the LSDA, and read it from there at
     * runtime.
     */
    ...
    for (k = 0; k < header->num_clauses; ++k) {
      ...
      for (i = 0; i < header->num_clauses; ++i) {
        ...
        if (cindex1 != cindex2 && clause1->try_offset >= clause2->try_offset &&
            clause1->handler_offset <= clause2->handler_offset)
          encode_value(i, p, &p);
      }
      encode_value(-1, p, &p);
    }
  ...
  cfg->ex_info_offset =
      add_to_blob_aligned(acfg, buf, p - buf, cfg->gc_map ? 4 : 1);
  g_free(buf);
}

static guint32 emit_klass_info(MonoAotCompile *acfg, guint32 token) {
  ERROR_DECL(error);
  ...
  for (i = 0; i < m_class_get_vtable_size(klass); ++i) {
    ...
    if (cm && mono_method_signature_internal(cm)->is_inflated &&
        !g_hash_table_lookup(acfg->token_info_hash, cm))
      cant_encode = TRUE;
  }
  ...
  if (mono_class_is_gtd(klass) || cant_encode) {
    ...
    encode_value(
        (m_class_has_weak_fields(klass) << 9) |
            (mono_class_is_gtd(klass) ? (1 << 8) : 0) |
            (no_special_static << 7) | (m_class_has_static_refs(klass) << 6) |
            (m_class_has_references(klass) << 5) |
            ((m_class_is_blittable(klass) << 4) | (has_nested ? 1 : 0) << 3) |
            (m_class_has_cctor(klass) << 2) |
            (m_class_has_finalize(klass) << 1) | m_class_is_ghcimpl(klass),
        p, &p);
...
static char *get_plt_entry_debug_sym(MonoAotCompile *acfg, MonoJumpInfo *ji,
  ...
  case MONO_PATCH_INFO_RGCTX_FETCH:
    debug_sym =
        g_strdup_printf("%s_rgctx_fetch_%d", prefix, acfg->label_generator++);
    break;
  case MONO_PATCH_INFO_ICALL_ADDR:
  case MONO_PATCH_INFO_ICALL_ADDR_CALL: {
  ...
  case MONO_PATCH_INFO_SPECIFIC_TRAMPOLINE_LAZY_FETCH_ADDR:
    debug_sym = g_strdup_printf(
        "%s_jit_icall_native_specific_trampoline_lazy_fetch_%lu", prefix,
        (gulong)ji->data.uindex);
    break;
  case MONO_PATCH_INFO_JIT_ICALL_ADDR:
    debug_sym =
 ...
 * ELF PLT (Program Linkage Table). Initially the PLT entries jump to code which
 * transfers control to the AOT runtime through a trampoline.
 */
static void emit_plt(MonoAotCompile *acfg) {
  int i;
  ...
  for (i = 0; i < acfg->plt_offset; ++i) {
    ...
    plt_entry = (MonoPltEntry *)g_hash_table_lookup(acfg->plt_offset_to_entry,
                                                    GUINT_TO_POINTER(i));
    ...
    arch_emit_plt_entry(acfg, acfg->got_symbol, i,
                        (acfg->plt_got_offset_base + i) *
                            sizeof(target_mgreg_t),
                        acfg->plt_got_info_offsets[i]);
  ...
  if (acfg->thumb_mixed) {
    ...
    /*
     * Emit a separate set of PLT entries using thumb2 which is called by LLVM
     * generated code.
     */
    for (i = 0; i < acfg->plt_offset; ++i) {
      char *debug_sym = NULL;
      ...
      arch_emit_llvm_plt_entry(acfg, acfg->got_symbol, i,
                               (acfg->plt_got_offset_base + i) *
                                   sizeof(target_mgreg_t),
                               acfg->plt_got_info_offsets[i]);
 ...
 *
 *   If EMIT_TINFO is TRUE, emit additional information which can be used to
 * create a MonoJitInfo for this trampoline by create_jit_info_for_trampoline
 * ().
 */
static G_GNUC_UNUSED void emit_trampoline_full(MonoAotCompile *acfg,
                                               MonoTrampInfo *info,
                                               gboolean emit_tinfo) {
  ...
  if (emit_tinfo) {
    ...
    /*
     * Emit additional information which can be used to reconstruct a partial
     * MonoTrampInfo.
     */
  ...
  if (unwind_ops) {
    ...
    if (acfg->dwarf)
      mono_dwarf_writer_emit_trampoline(acfg->dwarf, symbol, symbol2, NULL,
                                        NULL, code_size, unwind_ops);
  }

  g_free(buf);
}

static G_GNUC_UNUSED void emit_trampoline(MonoAotCompile *acfg,
                                          MonoTrampInfo *info) {
  emit_trampoline_full(acfg, info, TRUE);
}

static void emit_trampolines(MonoAotCompile *acfg) {
  char symbol[MAX_SYMBOL_SIZE];
  ...
  if (!mono_aot_mode_is_full(&acfg->aot_opts) &&
      !mono_aot_mode_is_interp(&acfg->aot_opts))
    return;
  if (acfg->aot_opts.llvm_only)
    return;
  ...
  if (mono_is_corlib_image(acfg->image->assembly->image)) {
     ...
     * method, but that approach has its own complexities, so we choose the
     * simpler method.
     */
    for (tramp_type = 0; tramp_type < MONO_TRAMPOLINE_NUM; ++tramp_type) {
      /* we overload the boolean here to indicate the slightly different
       * trampoline needed, see mono_arch_create_generic_trampoline() */
#ifdef DISABLE_REMOTING
      if (tramp_type == MONO_TRAMPOLINE_GENERIC_VIRTUAL_REMOTING)
        continue;
#endif
      mono_arch_create_generic_trampoline(
          (MonoTrampolineType)tramp_type, &info,
          acfg->aot_opts.use_trampolines_page ? 2 : TRUE);
    ...
    if (mono_aot_mode_is_interp(&acfg->aot_opts) &&
        mono_is_corlib_image(acfg->image->assembly->image)) {
    ...
    /*
     * FIXME: Maybe we should use more specific trampolines (i.e. one class init
     * for each class).
     */
    ...
    for (ntype = 0; ntype < MONO_AOT_TRAMP_NUM; ++ntype) {
      ...
      for (i = 0; i < acfg->num_trampolines[ntype]; ++i) {
        ...
        case MONO_AOT_TRAMP_SPECIFIC:
          ...
          break;
        case MONO_AOT_TRAMP_STATIC_RGCTX:
          arch_emit_static_rgctx_trampoline(acfg, tramp_got_offset,
                                            &tramp_size);
          tramp_got_offset += 2;
          break;
        case MONO_AOT_TRAMP_IMT:
          ...
          break;
        case MONO_AOT_TRAMP_GSHAREDVT_ARG:
          arch_emit_gsharedvt_arg_trampoline(acfg, tramp_got_offset,
                                             &tramp_size);
          tramp_got_offset += 2;
          break;
        case MONO_AOT_TRAMP_FTNPTR_ARG:
          ...
          break;
        case MONO_AOT_TRAMP_UNBOX_ARBITRARY:
          arch_emit_unbox_arbitrary_trampoline(acfg, tramp_got_offset,
                                               &tramp_size);
          tramp_got_offset += 1;
          break;
        default:
...
static gboolean str_begins_with(const char *str1, const char *str2) {
  int len = strlen(str2);
...
static void add_readonly_value(MonoAotOptions *opts, const char *val) {
  ReadOnlyValue *rdv;
  const char *fval;
  const char *tval;
  /* the format of val is:
   * namespace.typename.fieldname=type/value
   * type can be i1 for uint8/int8/boolean, i2 for uint16/int16/char, i4 for
   * uint32/int32
   */
  fval = strrchr(val, '/');
  if (!fval) {
...
static gchar *clean_path(gchar *path) {
  if (!path)
    return NULL;
...
static const gchar *wrap_path(const gchar *path) {
  int len;
  if (!path)
    return NULL;
...
static void ptr_array_add_range_if_nonempty(GPtrArray *args, gchar const *start,
                                            gchar const *end) {
...
static GPtrArray *mono_aot_split_options(const char *aot_options) {
...
static gboolean parse_cpu_features(const gchar *attr) {
  ...
  // these are independent
    ...
    // we don't have a flag for it but it's probably recognized by opt/llc so
    // let's don't fire an error here printf ("Unknown cpu feature: %s\n",
    // attr);
  }

  // if we disable a feature from the SSE-AVX tree we also need to disable all
  // dependencies
  ...
  if (enabled)
    mono_cpu_features_enabled =
        (MonoCPUFeatures)(mono_cpu_features_enabled | feature);
  else
    mono_cpu_features_disabled =
        (MonoCPUFeatures)(mono_cpu_features_disabled | feature);

  return TRUE;
}

static void mono_aot_parse_options(const char *aot_options,
                                   MonoAotOptions *opts) {
  GPtrArray *args;
  ...
  for (int i = 0; i < args->len; ++i) {
    ...
    if (str_begins_with(arg, "outfile=")) {
      ...
      // Intentionally undocumented -- No one remembers what this does. It
      // appears to be ARM-only
      ...
      opts->nrgctx_fetch_trampolines =
          atoi(arg + strlen("nrgctx-fetch-trampolines="));
      ...
      opts->ngsharedvt_arg_trampolines =
          atoi(arg + strlen("ngsharedvt-trampolines="));
    } else if (str_begins_with(arg, "nftnptr-arg-trampolines=")) {
      opts->nftnptr_arg_trampolines =
          atoi(arg + strlen("nftnptr-arg-trampolines="));
    } else if (str_begins_with(arg, "nunbox-arbitrary-trampolines=")) {
      opts->nunbox_arbitrary_trampolines =
          atoi(arg + strlen("unbox-arbitrary-trampolines="));
      ...
      opts->stats = TRUE;
      // Intentionally undocumented-- has no known function other than to debug
      // the compiler
      ...
      // Note: if you call this function from anywhere but
      // mono_compile_assembly, this will only set the try_llvm attribute and
      // not do the probing / set the attribute.
      opts->try_llvm = TRUE;
      ...
      exit(0);
      // Intentionally undocumented: Used for precise stack maps, which are not
      // available yet
      ...
      opts->profile_files = g_list_append(opts->profile_files,
                                          g_strdup(arg + strlen("profile=")));
      ...
      if (opts->llvm_opts) {
        char *s = g_strdup_printf("%s %s", opts->llvm_opts,
                                  arg + strlen("llvmopts="));
      ...
      if (!opts->llvm_cpu_attr)
        ...
        opts->llvm_cpu_attr =
            g_strdup_printf("%s,%s", opts->llvm_cpu_attr, attr);
        g_free(old_attrs);
      }
...
static void add_token_info_hash(gpointer key, gpointer value,
                                gpointer user_data) {
  ...
  new_ji = (MonoJumpInfoToken *)mono_mempool_alloc0(acfg->mempool,
...
static gboolean can_encode_class(MonoAotCompile *acfg, MonoClass *klass) {
  if (m_class_get_type_token(klass))
    return TRUE;
  if ((m_class_get_byval_arg(klass)->type == MONO_TYPE_VAR) ||
      (m_class_get_byval_arg(klass)->type == MONO_TYPE_MVAR) ||
      (m_class_get_byval_arg(klass)->type == MONO_TYPE_PTR))
    return TRUE;
  if (m_class_get_rank(klass))
...
static gboolean can_encode_method(MonoAotCompile *acfg, MonoMethod *method) {
  if (method->wrapper_type) {
    switch (method->wrapper_type) {
...
static gboolean can_encode_patch(MonoAotCompile *acfg,
                                 MonoJumpInfo *patch_info) {
  switch (patch_info->type) {
  case MONO_PATCH_INFO_METHOD:
  case MONO_PATCH_INFO_METHOD_FTNDESC:
...
static gboolean is_concrete_type(MonoType *t) {
  MonoClass *klass;
  int i;
...
static MonoMethodSignature *get_concrete_sig(MonoMethodSignature *sig) {
  gboolean concrete = TRUE;

  if (!sig->has_type_parameters)
    return sig;

  /* For signatures created during generic sharing, convert them to a concrete
  ...
  for (i = 0; i < sig->param_count; ++i) {
    if (sig->params[i]->byref) {
      MonoType *t = m_class_get_byval_arg(
          mono_class_from_mono_type_internal(sig->params[i]));
 ...
 */
static void compile_method(MonoAotCompile *acfg, MonoMethod *method) {
  MonoCompile *cfg;
  MonoJumpInfo *patch_info;
  gboolean skip;
  int index, depth;
  ...
  if (acfg->aot_opts.llvm_only)
    flags =
        (JitFlags)(flags | JIT_FLAG_LLVM_ONLY | JIT_FLAG_EXPLICIT_NULL_CHECKS);
  ...
  cfg = mini_method_compile(method, acfg->jit_opts, mono_get_root_domain(),
                            flags, 0, index);
  ...
  if (cfg->exception_type != MONO_EXCEPTION_NONE) {
    ...
    if (!method->is_inflated)
      report_loader_error(acfg, cfg->error, FALSE,
                          "Unable to compile method '%s' due to: '%s'.\n",
                          mono_method_get_full_name(method),
                          mono_error_get_message(cfg->error));
  ...
  if (!acfg->aot_opts.no_instances && depth < 32 &&
      (mono_aot_mode_is_full(&acfg->aot_opts) ||
       mono_aot_mode_is_hybrid(&acfg->aot_opts))) {
    for (patch_info = cfg->patch_info; patch_info;
         patch_info = patch_info->next) {
      switch (patch_info->type) {
      case MONO_PATCH_INFO_RGCTX_FETCH:
        ...
        if (m->is_inflated && (mono_aot_mode_is_full(&acfg->aot_opts) ||
          ...
          if (!(mono_class_generic_sharing_enabled(m->klass) &&
              ...
              (!method_has_type_vars(m) ||
               mono_method_is_generic_sharable_full(m, TRUE, TRUE, FALSE))) {
            if (m->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) {
              if (mono_aot_mode_is_full(&acfg->aot_opts) &&
                  !method_has_type_vars(m))
                add_extra_method_with_depth(
                    acfg, mono_marshal_get_native_wrapper(m, TRUE, TRUE),
                    depth + 1);
            } else {
      ...
      case MONO_PATCH_INFO_SFLDA: {
        ...
        if (mono_class_is_ginst(klass) &&
            !mono_generic_context_is_sharable_full(
                &mono_class_get_generic_class(klass)->context, FALSE, FALSE) &&
            mono_class_get_cctor(klass))
          add_extra_method_with_depth(acfg, mono_class_get_cctor(klass),
                                      depth + 1);
        break;
      }
      default:
  ...
  if (acfg->aot_opts.llvm_only) {
    ...
    if (!cfg->method->wrapper_type ||
      ...
      add_gsharedvt_wrappers(acfg, mono_method_signature_internal(cfg->method),
                             FALSE, TRUE, FALSE);

    for (l = cfg->signatures; l; l = l->next) {
      MonoMethodSignature *sig =
          mono_metadata_signature_dup((MonoMethodSignature *)l->data);
  ...
  } else if (mono_aot_mode_is_full(&acfg->aot_opts) &&
             mono_aot_mode_is_interp(&acfg->aot_opts)) {
  ...
  if (acfg->llvm && !cfg->compile_llvm &&
      method_is_externally_callable(acfg, cfg->method)) {
    /*
     * This is a JITted fallback method for a method which failed LLVM
     * compilation, emit a global symbol for it with the same name the LLVM
     * method would get.
     */
  ...
  /* Make a copy of the patch info which is in the mempool */
  {
    ...
    for (patch_info = cfg->patch_info; patch_info;
         patch_info = patch_info->next) {
      MonoJumpInfo *new_patch_info =
          mono_patch_info_dup_mp(acfg->mempool, patch_info);
  ...
  /* Make a copy of the unwind info */
  {
    ...
    for (l = cfg->unwind_ops; l; l = l->next) {
      op = (MonoUnwindOp *)mono_mempool_alloc(acfg->mempool,
                                              sizeof(MonoUnwindOp));
  ...
  /* Make a copy of the argument/local info */
  {
    ...
    sig = mono_method_signature_internal(method);
    args = (MonoInst **)mono_mempool_alloc(
        acfg->mempool, sizeof(MonoInst *) * (sig->param_count + sig->hasthis));
    ...
    header = mono_method_get_header_checked(method, error);
    mono_error_assert_ok(error); /* FIXME don't swallow the error */
    locals = (MonoInst **)mono_mempool_alloc(
        acfg->mempool, sizeof(MonoInst *) * header->num_locals);
    for (i = 0; i < header->num_locals; ++i) {
      locals[i] =
          (MonoInst *)mono_mempool_alloc(acfg->mempool, sizeof(MonoInst));
...
char *mono_aot_get_method_name(MonoCompile *cfg) {
  ...
  if (method->wrapper_type == MONO_WRAPPER_OTHER) {
    ...
    if (info->subtype == WRAPPER_SUBTYPE_GSHAREDVT_IN_SIG ||
        info->subtype == WRAPPER_SUBTYPE_GSHAREDVT_OUT_SIG)
  ...
  if (llvm_acfg->aot_opts.static_link)
    ...
    return g_strdup_printf(
        "%s_%s", llvm_acfg->assembly_name_sym,
        get_debug_sym(cfg->orig_method, "", llvm_acfg->method_label_hash));
  else
    return get_debug_sym(cfg->orig_method, "", llvm_acfg->method_label_hash);
}

static gboolean append_mangled_type(GString *s, MonoType *t) {
  if (t->byref)
  ...
  default: {
    ...
    /* Include the length to avoid different length type names aliasing each
     * other */
    g_string_append_printf(s, "cl%s%x_%s_", is_system ? "s" : "",
                           (int)strlen(temps), temps);
    g_free(temps);
    g_free(fullname);
  }
  }
...
static gboolean append_mangled_signature(GString *s, MonoMethodSignature *sig) {
  int i;
  gboolean supported;
...
static void append_mangled_wrapper_type(GString *s, guint32 wrapper_type) {
  const char *label;
...
static void append_mangled_wrapper_subtype(GString *s, WrapperSubtype subtype) {
  const char *label;
...
static char *sanitize_mangled_string(const char *input) {
...
static gboolean append_mangled_klass(GString *s, MonoClass *klass) {
  char *klass_desc = mono_class_full_name(klass);
  g_string_append_printf(s, "_%s_%s_", m_class_get_name_space(klass),
                         klass_desc);
  g_free(klass_desc);
...
static gboolean append_mangled_method(GString *s, MonoMethod *method);

static gboolean append_mangled_wrapper(GString *s, MonoMethod *method) {
  ...
  if (m_class_get_image(method->klass) != mono_get_corlib())
    g_string_append_printf(
        s, "%s_", m_class_get_image(method->klass)->assembly->aname.name);
  ...
  case MONO_WRAPPER_OTHER: {
    ...
    if (info->subtype == WRAPPER_SUBTYPE_PTR_TO_STRUCTURE ||
      ...
      success = success &&
                append_mangled_method(s, info->d.synchronized_inner.method);
    else if (info->subtype == WRAPPER_SUBTYPE_ARRAY_ACCESSOR)
      success =
          success && append_mangled_method(s, info->d.array_accessor.method);
    else if (info->subtype == WRAPPER_SUBTYPE_INTERP_IN)
      ...
      g_string_append_printf(
          s, "%s_%d_", m_class_get_image(method->klass)->assembly->aname.name,
          info->d.aot_init.subtype);
      append_sig = FALSE;
    }
    break;
  ...
  case MONO_WRAPPER_MANAGED_TO_NATIVE: {
    ...
    if (info->subtype == WRAPPER_SUBTYPE_ICALL_WRAPPER) {
      ...
      success =
          success && append_mangled_method(s, info->d.managed_to_native.method);
    } else {
      g_assert(info->subtype == WRAPPER_SUBTYPE_NONE ||
               info->subtype == WRAPPER_SUBTYPE_PINVOKE);
      success =
  ...
  case MONO_WRAPPER_MANAGED_TO_MANAGED: {
    ...
    if (info->subtype == WRAPPER_SUBTYPE_ELEMENT_ADDR) {
      ...
      success = success &&
                append_mangled_method(s, info->d.generic_array_helper.method);
    } else {
      success = FALSE;
    }
    break;
  ...
  case MONO_WRAPPER_RUNTIME_INVOKE: {
    ...
    if (info->subtype == WRAPPER_SUBTYPE_RUNTIME_INVOKE_DIRECT ||
        info->subtype == WRAPPER_SUBTYPE_RUNTIME_INVOKE_VIRTUAL)
      success =
          success && append_mangled_method(s, info->d.runtime_invoke.method);
    else if (info->subtype == WRAPPER_SUBTYPE_RUNTIME_INVOKE_NORMAL)
      success =
          success && append_mangled_signature(s, info->d.runtime_invoke.sig);
    break;
  }
  case MONO_WRAPPER_DELEGATE_INVOKE:
  case MONO_WRAPPER_DELEGATE_BEGIN_INVOKE:
  ...
  case MONO_WRAPPER_NATIVE_TO_MANAGED: {
    g_assert(info);
    success =
        success && append_mangled_method(s, info->d.native_to_managed.method);
    success = success && append_mangled_klass(s, method->klass);
    break;
  }
  default:
  ...
  if (success && append_sig)
    success =
        append_mangled_signature(s, mono_method_signature_internal(method));
  return success;
}

static void append_mangled_ginst(GString *str, MonoGenericInst *ginst) {
  int i;
  ...
  for (i = 0; i < ginst->type_argc; ++i) {
    ...
    case MONO_TYPE_MVAR: {
      ...
      if (constraint) {
        g_assert(constraint->type != MONO_TYPE_VAR &&
                 constraint->type != MONO_TYPE_MVAR);
...
static void append_mangled_context(GString *str, MonoGenericContext *context) {
...
static gboolean append_mangled_method(GString *s, MonoMethod *method) {
  ...
  if (method->is_inflated) {
    ...
    g_assert(imethod->context.class_inst != NULL ||
             imethod->context.method_inst != NULL);

    append_mangled_context(s, &imethod->context);
    g_string_append_printf(
        s, "_declared_by_%s_",
        m_class_get_image(imethod->declaring->klass)->assembly->aname.name);
    ...
    g_string_append_printf(
        s, "%s", m_class_get_image(method->klass)->assembly->aname.name);
    append_mangled_klass(s, method->klass);
    g_string_append_printf(s, "_%s_", method->name);
...
char *mono_aot_get_direct_call_symbol(MonoJumpInfoType type,
                                      gconstpointer data) {
  const char *sym = NULL;
  ...
  if (llvm_acfg->aot_opts.direct_icalls) {
    if (type == MONO_PATCH_INFO_JIT_ICALL_ADDR) {
      ...
      MonoJitICallInfo const *const info =
...
char *mono_aot_get_plt_symbol(MonoJumpInfoType type, gconstpointer data) {
  MonoJumpInfo *ji = (MonoJumpInfo *)mono_mempool_alloc(llvm_acfg->mempool,
                                                        sizeof(MonoJumpInfo));
  MonoPltEntry *plt_entry;
  const char *sym = NULL;
  ...
  return g_strdup(plt_entry->llvm_symbol +
                  strlen(llvm_acfg->llvm_label_prefix));
#else
  return g_strdup(plt_entry->llvm_symbol);
#endif
}
...
static int execute_system(const char *command) {
  int status = 0;

#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT) && defined(HOST_WIN32)
  // We need an extra set of quotes around the whole command to properly handle
  // commands with spaces since internally the command is called through "cmd
  // /c.
 ...
 * emit_llvm_file:
 *
 *   Emit the LLVM code into an LLVM bytecode file, and compile it using the
 * LLVM tools.
 */
static gboolean emit_llvm_file(MonoAotCompile *acfg) {
  ...
  if (acfg->aot_opts.no_opt)
    return TRUE;
  /*
   * FIXME: Experiment with adding optimizations, the -std-compile-opts set
   * takes a lot of time, and doesn't seem to save much space. The following
   * optimizations cannot be enabled:
   * - 'tailcallelim'
   ...
   * Here, if 'Earlier' refers to a memset, and Later has no size info, it
   * mistakenly thinks the memset is redundant.
   */
  if (acfg->aot_opts.llvm_only) {
  ...
  if (mono_use_fast_math) {
    ...
    opts = g_strdup_printf(
        "%s -fp-contract=fast -enable-no-infs-fp-math -enable-no-nans-fp-math "
        "-enable-no-signed-zeros-fp-math -enable-no-trapping-fp-math "
        "-enable-unsafe-fp-math",
        opts);
  }

  command = g_strdup_printf("\"%sopt\" -f %s -o \"%s\" \"%s\"",
                            acfg->aot_opts.llvm_path, opts, optbc, tempbc);
  ...
  if (acfg->aot_opts.llvm_only) {
    ...
    command = g_strdup_printf(
        "%s -fexceptions -fpic -O2 -fno-optimize-sibling-calls "
        "-Wno-override-module -c -o \"%s\" \"%s.opt.bc\"",
        acfg->aot_opts.clangxx, acfg->llvm_ofile, acfg->tmpbasename);
  ...
  if (acfg->aot_opts.mtriple)
    g_string_append_printf(acfg->llc_args, " -mtriple=%s",
                           acfg->aot_opts.mtriple);
  ...
  g_string_append_printf(acfg->llc_args, " -mono-eh-frame-symbol=%s%s",
                         acfg->user_symbol_prefix, acfg->llvm_eh_frame_symbol);

  g_string_append_printf(acfg->llc_args, " -disable-tail-calls");

#if defined(TARGET_AMD64) || defined(TARGET_X86)
  /* This generates stack adjustments in the middle of functions breaking unwind
   * info */
  g_string_append_printf(acfg->llc_args, " -no-x86-call-frame-opt");
#endif

#if (defined(TARGET_MACH) && defined(TARGET_ARM)) || defined(TARGET_ORBIS) ||  \
    defined(TARGET_X86_64_WIN32_MSVC)
  ...
  if (acfg->aot_opts.llvm_cpu_attr) {
    g_string_append_printf(acfg->llc_args, " -mattr=%s",
                           acfg->aot_opts.llvm_cpu_attr);
  }

  command = g_strdup_printf("\"%sllc\" %s -o \"%s\" \"%s.opt.bc\"",
                            acfg->aot_opts.llvm_path, acfg->llc_args->str,
                            output_fname, acfg->tmpbasename);
  g_free(output_fname);
...
/* Set the skip flag for methods which do not need to be emitted because of
 * dedup */
static void dedup_skip_methods(MonoAotCompile *acfg) {
  int oindex, i;
  ...
  for (oindex = 0; oindex < acfg->method_order->len; ++oindex) {
    ...
    gboolean dedup_collect =
        acfg->aot_opts.dedup ||
        (acfg->aot_opts.dedup_include && !acfg->dedup_emit_mode);
...
static void emit_code(MonoAotCompile *acfg) {
   ...
   * code_offsets array. It is also used to compute the memory ranges occupied
   * by AOT code, so it must be equal to the address of the first emitted
   * method.
   */
  emit_section_change(acfg, ".text", 0);
  ...
  for (oindex = 0; oindex < acfg->method_order->len; ++oindex) {
    ...
    if (mono_aot_mode_is_full(&acfg->aot_opts) &&
        m_class_is_valuetype(cfg->orig_method->klass)) {
  ...
  /*
   * Add .no_dead_strip directives for all LLVM methods to prevent the OSX
   * linker from optimizing them away, since it doesn't see that code_offsets
   * references them. JITted methods don't need this since they are referenced
   * using assembler local symbols.
  ...
  /*
   * To work around linker issues, we emit a table of branches, and disassemble
   * them at runtime. This is PIE code, and the linker can update it if needed.
   */
#if defined(TARGET_ANDROID) || defined(__linux__)
  ...
  for (i = 0; i < acfg->nmethods; ++i) {
    ...
    if (acfg->flags & MONO_AOT_FILE_FLAG_CODE_EXEC_ONLY) {
      ...
      if (!ignore_cfg(acfg->cfgs[i])) {
        arch_emit_label_address(acfg, acfg->cfgs[i]->asm_symbol, FALSE,
                                acfg->thumb_mixed &&
                                    acfg->cfgs[i]->compile_llvm,
                                NULL, &acfg->call_table_entry_size);
      } else {
        arch_emit_label_address(acfg, symbol, FALSE, FALSE, NULL,
                                &acfg->call_table_entry_size);
      }
    }
#endif
  }
  ...
  for (i = 0; i < acfg->nmethods; ++i) {
    ...
    if (mono_aot_mode_is_full(&acfg->aot_opts) &&
      ...
      if (acfg->flags & MONO_AOT_FILE_FLAG_CODE_EXEC_ONLY) {
        ...
        int call_size;
        arch_emit_direct_call(acfg, symbol, FALSE,
                              acfg->thumb_mixed && cfg->compile_llvm, NULL,
                              &call_size);
      }
#endif
    }
...
static void emit_method_info_table(MonoAotCompile *acfg) {
  int oindex, i;
  gint32 *offsets;
  guint8 *method_flags;
  ...
  acfg->stats.offsets_size += emit_offset_table(
      acfg, "method_info_offsets", MONO_AOT_TABLE_METHOD_INFO_OFFSETS,
      acfg->nmethods, 10, offsets);
  ...
  for (i = 0; i < acfg->nmethods; ++i) {
    if (acfg->cfgs[i])
      method_flags[acfg->cfgs[i]->method_index] =
          acfg->cfgs[i]->aot_method_flags;
  }
  emit_aot_data(acfg, MONO_AOT_TABLE_METHOD_FLAGS_TABLE, "method_flags_table",
                method_flags, acfg->nmethods);
}
...
static guint mono_aot_type_hash(MonoType *t1) {
  guint hash = t1->type;
  ...
  case MONO_TYPE_SZARRAY:
    ...
    return ((hash << 5) - hash) ^
           mono_metadata_str_hash(m_class_get_name(t1->data.klass));
  case MONO_TYPE_PTR:
    return ((hash << 5) - hash) ^ mono_metadata_type_hash(t1->data.type);
  case MONO_TYPE_ARRAY:
    return ((hash << 5) - hash) ^ mono_metadata_type_hash(m_class_get_byval_arg(
                                      t1->data.array->eklass));
  case MONO_TYPE_GENERICINST:
...
guint32 mono_aot_method_hash(MonoMethod *method) {
  ...
  if (mono_class_is_ginst(method->klass))
    class_ginst =
        mono_class_get_generic_class(method->klass)->context.class_inst;
  if (method->is_inflated)
    ginst = ((MonoMethodInflated *)method)->context.method_inst;

  hashes_count = sig->param_count + 5 +
                 (class_ginst ? class_ginst->type_argc : 0) +
                 (ginst ? ginst->type_argc : 0);
  ...
  if (!method->wrapper_type) {
    ...
    if (mono_class_is_ginst(klass))
      full_name = mono_type_full_name(m_class_get_byval_arg(
  ...
  if (method->wrapper_type == MONO_WRAPPER_STFLD ||
      method->wrapper_type == MONO_WRAPPER_LDFLD ||
      method->wrapper_type == MONO_WRAPPER_LDFLDA)
...
MonoMethod *mono_aot_get_array_helper_from_wrapper(MonoMethod *method) {
  ...
  m = get_method_nofail(mono_defaults.array_class, helper_name,
  ...
  if (m->is_generic) {
    ...
    memset(&ctx, 0, sizeof(ctx));
    MonoType *args[] = {
        m_class_get_byval_arg(m_class_get_element_class(method->klass))};
    ctx.method_inst = mono_metadata_get_generic_inst(1, args);
 ...
 */
static void emit_extra_methods(MonoAotCompile *acfg) {
  ...
  for (i = 0; i < acfg->extra_methods->len; ++i) {
    MonoMethod *method =
        (MonoMethod *)g_ptr_array_index(acfg->extra_methods, i);
    MonoCompile *cfg =
        (MonoCompile *)g_hash_table_lookup(acfg->method_to_cfg, method);
  ...
  for (i = 0; i < acfg->extra_methods->len; ++i) {
    ...
    new_entry =
        (HashEntry *)mono_mempool_alloc0(acfg->mempool, sizeof(HashEntry));
    new_entry->key = key;
  ...
  /* Emit the table */
  emit_aot_data(acfg, MONO_AOT_TABLE_EXTRA_METHOD_TABLE, "extra_method_table",
                buf, p - buf);

  g_free(buf);

  /*
   * Emit a table reverse mapping method indexes to their index in
   * extra_method_info. This is used by mono_aot_find_jit_info ().
   */
  ...
  emit_aot_data(acfg, MONO_AOT_TABLE_EXTRA_METHOD_INFO_OFFSETS,
                "extra_method_info_offsets", buf, p - buf);
...
static void generate_aotid(guint8 *aotid) {
...
static void emit_exception_info(MonoAotCompile *acfg) {
  int i;
  gint32 *offsets;
  ...
  for (i = 0; i < acfg->nmethods; ++i) {
    if (acfg->cfgs[i]) {
      ...
      // By design aot-runtime decode_exception_debug_info is not able to load
      // sequence point debug data from a file. As it is not possible to load
      // debug data from a file its is also not possible to store it in a file.
      gboolean method_seq_points_to_file = acfg->aot_opts.gen_msym_dir &&
                                           cfg->gen_seq_points &&
                                           !cfg->gen_sdb_seq_points;
      gboolean method_seq_points_to_binary =
          cfg->gen_seq_points && !method_seq_points_to_file;
      ...
      if (method_seq_points_to_file) {
        ...
        mono_seq_point_data_add(&sp_data, cfg->method->token, cfg->method_index,
                                cfg->seq_point_info);
      }
    } else {
      offsets[i] = 0;
  ...
  if (seq_points_to_file) {
    ...
    char *dir = g_build_filename(acfg->aot_opts.gen_msym_dir_path, aotid,
                                 (const char *)NULL);
  ...
  acfg->stats.offsets_size +=
      emit_offset_table(acfg, "ex_info_offsets", MONO_AOT_TABLE_EX_INFO_OFFSETS,
                        acfg->nmethods, 10, offsets);
  g_free(offsets);
}

static void emit_unwind_info(MonoAotCompile *acfg) {
  int i;
  char symbol[128];
...
static void emit_class_info(MonoAotCompile *acfg) {
  int i;
  gint32 *offsets;
  ...
  acfg->stats.offsets_size += emit_offset_table(
      acfg, "class_info_offsets", MONO_AOT_TABLE_CLASS_INFO_OFFSETS,
      acfg->image->tables[MONO_TABLE_TYPEDEF].rows, 10, offsets);
  g_free(offsets);
}
...
static void emit_class_name_table(MonoAotCompile *acfg) {
  int i, table_size, buf_size;
  guint32 token, hash;
  MonoClass *klass;
  GPtrArray *table;
   ...
   */
  table_size = g_spaced_primes_closest(
      (int)(acfg->image->tables[MONO_TABLE_TYPEDEF].rows * 1.5));
  table = g_ptr_array_sized_new(table_size);
  for (i = 0; i < table_size; ++i)
    g_ptr_array_add(table, NULL);
  for (i = 0; i < acfg->image->tables[MONO_TABLE_TYPEDEF].rows; ++i) {
    ...
    full_name = mono_type_get_name_full(m_class_get_byval_arg(klass),
  ...
  for (i = 0; i < table->len; ++i) {
    ClassNameTableEntry *entry =
        (ClassNameTableEntry *)g_ptr_array_index(table, i);
  ...
  emit_aot_data(acfg, MONO_AOT_TABLE_CLASS_NAME, "class_name_table", buf,
                p - buf);

  g_free(buf);
}

static void emit_image_table(MonoAotCompile *acfg) {
  int i, buf_size;
  guint8 *buf, *p;
  ...
  for (i = 0; i < acfg->image_table->len; i++) {
    ...
    buf_size += strlen(image->assembly_name) + strlen(image->guid) +
                (aname->culture ? strlen(aname->culture) : 1) +
                strlen((char *)aname->public_key_token) + 4;
  }
...
static void emit_weak_field_indexes(MonoAotCompile *acfg) {
  GHashTable *indexes;
  GHashTableIter iter;
  gpointer key, value;
  int buf_size;
  ...
  /* Emit a table of weak field indexes, since computing these at runtime is
   * expensive */
  ...
  emit_aot_data(acfg, MONO_AOT_TABLE_WEAK_FIELD_INDEXES, "weak_field_indexes",
                buf, p - buf);

  g_free(buf);
}

static void emit_got_info(MonoAotCompile *acfg, gboolean llvm) {
  ...
  if (!llvm) {
    ...
    for (i = 1; i < acfg->plt_offset; ++i) {
      MonoPltEntry *plt_entry = (MonoPltEntry *)g_hash_table_lookup(
  ...
  got_info_offsets = (guint32 *)mono_mempool_alloc(
      acfg->mempool, info->got_patches->len * sizeof(guint32));
  if (!llvm) {
    acfg->plt_got_info_offsets = (guint32 *)mono_mempool_alloc(
        acfg->mempool, acfg->plt_offset * sizeof(guint32));
    /* Unused */
    if (acfg->plt_offset)
  ...
  for (i = 0; i < info->got_patches->len; ++i) {
    ...
    if (!llvm && i >= first_plt_got_patch)
      acfg->plt_got_info_offsets[i - first_plt_got_patch + 1] =
          got_info_offsets[i];
...
#else
  /* No need to emit offsets for the got plt entries, the plt embeds them
   * directly */
  int got_info_offsets_to_emit = first_plt_got_patch;
#endif
  acfg->stats.offsets_size += emit_offset_table(
      acfg, llvm ? "llvm_got_info_offsets" : "got_info_offsets",
      llvm ? MONO_AOT_TABLE_LLVM_GOT_INFO_OFFSETS
           : MONO_AOT_TABLE_GOT_INFO_OFFSETS,
      llvm ? acfg->llvm_got_offset : got_info_offsets_to_emit, 10,
      (gint32 *)got_info_offsets);
}

static void emit_got(MonoAotCompile *acfg) {
  char symbol[MAX_SYMBOL_SIZE];
...
static void emit_library_info(MonoAotCompile *acfg) {
  ...
  if (link_shared_library(acfg)) {
    ...
    // For more information about DLLMain:
...
static void emit_library_info(MonoAotCompile *acfg) { return; }
#endif

static void emit_globals(MonoAotCompile *acfg) {
  int i, table_size;
  guint32 hash;
  GPtrArray *table;
  char symbol[1024];
...
static void emit_mem_end(MonoAotCompile *acfg) {
  char symbol[128];
...
static void init_aot_file_info(MonoAotCompile *acfg, MonoAotFileInfo *info) {
  int i;
...
static void emit_aot_file_info(MonoAotCompile *acfg, MonoAotFileInfo *info) {
  char symbol[MAX_SYMBOL_SIZE];
  int i, sindex;
  const char **symbols;
  ...
  /*
   * We emit pointers to our data structures instead of emitting global symbols
   * which point to them, to reduce the number of globals, and because using
   * globals leads to various problems (i.e. arm/thumb).
   */
 ...
 */
static void emit_file_info(MonoAotCompile *acfg) {
  char *build_info;
  ...
  if (acfg->aot_opts.static_link) {
     ...
     * mono_aot_register_module (). The symbol points to a pointer to the the
     * file info structure.
     */
    sprintf(symbol, "%smono_aot_module_%s_info", acfg->user_symbol_prefix,
...
static void emit_blob(MonoAotCompile *acfg) {
  acfg->blob_closed = TRUE;

  emit_aot_data(acfg, MONO_AOT_TABLE_BLOB, "blob", (guint8 *)acfg->blob.data,
                acfg->blob.index);
}

static void emit_objc_selectors(MonoAotCompile *acfg) {
  int i;
  char symbol[128];
...
static void emit_dwarf_info(MonoAotCompile *acfg) {
  ...
  for (i = 0; i < acfg->nmethods; ++i) {
    ...
    mono_dwarf_writer_emit_method(
        acfg->dwarf, cfg, cfg->method, cfg->asm_symbol, symbol2,
        cfg->asm_debug_symbol, (guint8 *)cfg->jit_info->code_start,
        cfg->jit_info->code_size, cfg->args, cfg->locals, cfg->unwind_ops,
        mono_debug_find_method(cfg->jit_info->d.method, mono_domain_get()));
  }
#endif
}
...
static void codeview_parse_compiler_version(gchar *version,
                                            CodeViewCompilerVersion *data) {
  gint values[4] = {0};
  gint *value = values;
...
static void
emit_codeview_start_subsection(MonoAotCompile *acfg, int section_id,
                               int section_type, int section_record_type,
                               CodeViewSubsectionData *section_data) {
  ...
  section_data->start_section =
      g_strdup_printf("%scvs_%d", acfg->temp_prefix, section_data->section_id);
  section_data->end_section =
      g_strdup_printf("%scvse_%d", acfg->temp_prefix, section_data->section_id);
  section_data->start_section_record =
      g_strdup_printf("%scvsr_%d", acfg->temp_prefix, section_data->section_id);
  section_data->end_section_record = g_strdup_printf(
      "%scvsre_%d", acfg->temp_prefix, section_data->section_id);

  // Subsection type, function symbol.
  emit_int32(acfg, section_data->section_type);

  // Subsection size.
  emit_symbol_diff(acfg, section_data->end_section, section_data->start_section,
                   0);
  emit_label(acfg, section_data->start_section);

  // Subsection record size.
  fprintf(acfg->fp, "\t.word %s - %s\n", section_data->end_section_record,
          section_data->start_section_record);
...
static void emit_codeview_end_subsection(MonoAotCompile *acfg,
                                         CodeViewSubsectionData *section_data,
                                         int *section_id) {
  ...
  if (section_data->section_record_type ==
...
inline static void
emit_codeview_start_symbol_subsection(MonoAotCompile *acfg, int section_id,
                                      int section_record_type,
                                      CodeViewSubsectionData *section_data) {
  emit_codeview_start_subsection(acfg, section_id,
                                 CODEVIEW_SUBSECTION_SYMBOL_TYPE,
                                 section_record_type, section_data);
}

inline static void
emit_codeview_end_symbol_subsection(MonoAotCompile *acfg,
                                    CodeViewSubsectionData *section_data,
                                    int *section_id) {
  emit_codeview_end_subsection(acfg, section_data, section_id);
}

static void emit_codeview_compiler_info(MonoAotCompile *acfg, int *section_id) {
  CodeViewSubsectionData section_data = {0};
  CodeViewCompilerVersion compiler_version = {0};

  // Start new compiler record subsection.
  emit_codeview_start_symbol_subsection(
      acfg, *section_id, CODEVIEW_SUBSECTION_RECORD_COMPILER_TYPE,
      &section_data);
...
static void emit_codeview_function_info(MonoAotCompile *acfg,
                                        MonoMethod *method, int *section_id,
                                        gchar *symbol, gchar *symbol_start,
                                        gchar *symbol_end) {
  CodeViewSubsectionData section_data = {0};
  ...
  emit_codeview_start_symbol_subsection(
      acfg, *section_id, CODEVIEW_SUBSECTION_RECORD_FUNCTION_START_TYPE,
      &section_data);
  ...
  // Emit function name, exclude signature since it should be described by own
  // metadata.
...
static void emit_codeview_info(MonoAotCompile *acfg) {
  int i;
  int section_id = 0;
  ...
  for (i = 0; i < acfg->nmethods; ++i) {
    ...
    int ret = g_snprintf(symbol_buffer, G_N_ELEMENTS(symbol_buffer), "%sme_%x",
                         acfg->temp_prefix, i);
    if (ret > 0 && ret < G_N_ELEMENTS(symbol_buffer))
      emit_codeview_function_info(acfg, cfg->method, &section_id,
...
static UnwindInfoSectionCacheItem *get_cached_unwind_info_section_item_win32(
  ...
  if (acfg->unwind_info_section_cache) {
    ...
    if (item) {
      // Format .xdata section label for function, used to get unwind info
      // address RVA. Since the unwind info is similar for most functions, the
      // symbol will be reused.
      item->xdata_section_label =
          g_strdup_printf("%sunwind_%d", acfg->temp_prefix, list_size);

      // Cache unwind info data, used when checking cache for matching unwind
      // info. NOTE, cache takes
      ...
      acfg->unwind_info_section_cache =
          g_list_prepend(acfg->unwind_info_section_cache, (gpointer)item);
    }
  }
...
static void emit_unwind_info_data_win32(MonoAotCompile *acfg,
  ...
  emit_bytes(acfg, (guint8 *)unwind_info,
             sizeof(UNWIND_INFO) -
                 (sizeof(UNWIND_CODE) * MONO_MAX_UNWIND_CODES));

  // Emit all unwind codes encoded in unwind info struct.
  PUNWIND_CODE current_unwind_node =
      &unwind_info
           ->UnwindCode[MONO_MAX_UNWIND_CODES - unwind_info->CountOfCodes];
  PUNWIND_CODE last_unwind_node =
      &unwind_info->UnwindCode[MONO_MAX_UNWIND_CODES];
...
// Emit unwind info sections for each function. Unwind info on Windows x64 is
// emitted into two different sections. .pdata includes the serialized DWORD
// aligned RVA's of function start, end and address of serialized UNWIND_INFO
// struct emitted into .xdata, see
// https://msdn.microsoft.com/en-us/library/ft9x1kdx.aspx. .xdata section
// includes DWORD aligned serialized version of UNWIND_INFO struct,
...
static void emit_unwind_info_sections_win32(MonoAotCompile *acfg,
  ...
  int temp_prefix_len =
      (acfg->temp_prefix != NULL) ? strlen(acfg->temp_prefix) : 0;
  if (strncmp(function_start, acfg->temp_prefix, temp_prefix_len)) {
    temp_prefix_len = 0;
  }

  // Format .pdata section label for function.
  pdata_section_label = g_strdup_printf("%spdata_%s", acfg->temp_prefix,
                                        function_start + temp_prefix_len);

  UnwindInfoSectionCacheItem *cache_item =
      get_cached_unwind_info_section_item_win32(acfg, function_start,
                                                function_end, unwind_ops);
  g_assert(cache_item && cache_item->xdata_section_label &&
           cache_item->unwind_info);
...
static gboolean should_emit_gsharedvt_method(MonoAotCompile *acfg,
                                             MonoMethod *method) {
#ifdef TARGET_WASM
  if (acfg->image == mono_get_corlib() &&
      !strcmp(m_class_get_name(method->klass), "Vector`1"))
    /* The SIMD fallback code in Vector<T> is very large, and not likely to be
     * used */
    return FALSE;
#endif
  if (acfg->image == mono_get_corlib() &&
      !strcmp(m_class_get_name(method->klass), "Volatile"))
    /* Read<T>/Write<T> are not needed and cause JIT failures */
    return FALSE;
  return TRUE;
}

static gboolean collect_methods(MonoAotCompile *acfg) {
  int mindex, i;
  ...
  for (i = 0; i < image->tables[MONO_TABLE_METHOD].rows; ++i) {
    ...
    if (mono_aot_mode_is_full(&acfg->aot_opts) &&
      ...
      /* We do this here instead of add_wrappers () because it is easy to do it
       * here */
    ...
    if (acfg->aot_opts.soft_debug && !method->wrapper_type) {
            if (!((method->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL) ||
                    ...
                    if (!mono_debug_lookup_method (method)) {
                            fprintf (stderr, "Method %s has no debug info,
    probably the .mdb file for the assembly is missing.\n",
    mono_method_get_full_name (method)); exit (1);
                    }
            }
    }
    */
    ...
    /* Since we add the normal methods first, their index will be equal to their
  ...
  for (mindex = 0; mindex < image->tables[MONO_TABLE_METHOD].rows; ++mindex) {
    ...
    if ((method->is_generic || mono_class_is_gtd(method->klass)) &&
        should_emit_gsharedvt_method(acfg, method)) {
      MonoMethod *gshared;

      gshared =
          mini_get_shared_method_full(method, SHARE_MODE_GSHAREDVT, error);
      mono_error_assert_ok(error);
...
static void compile_methods(MonoAotCompile *acfg) {
  int i, methods_len;
  ...
  if (acfg->aot_opts.nthreads > 0) {
    ...
    // memcpy (methods, g_ptr_array_index (acfg->methods, 0), sizeof
    // (MonoMethod*) * methods_len);
    ...
    while (i < methods_len) {
      ...
      thread = mono_thread_create_internal(
          mono_domain_get(), (gpointer)compile_thread_main, user_data,
          MONO_THREAD_CREATE_FLAGS_NONE, error);
      mono_error_assert_ok(error);
    ...
    for (i = 0; i < threads->len; ++i) {
      mono_thread_info_wait_one_handle(
          (MonoThreadHandle *)g_ptr_array_index(threads, i), MONO_INFINITE_WAIT,
          FALSE);
      mono_threads_close_thread_handle(
          (MonoThreadHandle *)g_ptr_array_index(threads, i));
    }
  } else {
    methods_len = 0;
  }
...
static int compile_asm(MonoAotCompile *acfg) {
  char *command, *objfile;
  char *outfile_name, *tmp_outfile_name, *llvm_ofile;
  const char *tool_prefix =
      acfg->aot_opts.tool_prefix ? acfg->aot_opts.tool_prefix : "";
  char *ld_flags =
      acfg->aot_opts.ld_flags ? acfg->aot_opts.ld_flags : g_strdup("");
...
#define LD_OPTIONS "-Bsymbolic"
  ...
  command = g_strdup_printf("\"%s%s\" %s %s -o %s %s", tool_prefix, AS_NAME,
                            AS_OPTIONS, acfg->as_args ? acfg->as_args->str : "",
  ...
  if (acfg->llvm && !acfg->llvm_owriter) {
    command = g_strdup_printf(
        "\"%s%s\" %s %s -o %s %s", tool_prefix, AS_NAME, AS_OPTIONS,
        acfg->as_args ? acfg->as_args->str : "", wrap_path(acfg->llvm_ofile),
        wrap_path(acfg->llvm_sfile));
    aot_printf(acfg, "Executing the native assembler: %s\n", command);
    if (execute_system(command) != 0) {
      g_free(command);
      g_free(objfile);
  ...
  command = g_strdup_printf(
      "\"%s%s\" %s %s /OUT:%s %s %s", tool_prefix, LD_NAME,
      acfg->aot_opts.nodebug ? LD_OPTIONS : LD_DEBUG_OPTIONS, ld_flags,
      wrap_path(tmp_outfile_name), wrap_path(objfile), wrap_path(llvm_ofile));
#else
  GString *str;
...
#endif
  g_string_append_printf(
      str, " -o %s %s %s %s", wrap_path(tmp_outfile_name),
      wrap_path(llvm_ofile),
      wrap_path(g_strdup_printf("%s." AS_OBJECT_FILE_SUFFIX, acfg->tmpfname)),
      ld_flags);

#if defined(TARGET_MACH)
  g_string_append_printf(str, " -Wl,-install_name,%s%s",
                         g_path_get_basename(acfg->image->name),
                         MONO_SOLIB_EXT);
#endif
  ...
  /*com = g_strdup_printf ("strip --strip-unneeded %s%s", acfg->image->name,
  MONO_SOLIB_EXT); printf ("Stripping the binary: %s\n", com); execute_system
  (com); g_free (com);*/
   ...
   */
  command =
      g_strdup_printf("\"%sstrip\" --strip-symbol=\\$a --strip-symbol=\\$d %s",
                      tool_prefix, wrap_path(tmp_outfile_name));
  ...
  if (0 != rename(tmp_outfile_name, outfile_name)) {
    if (G_FILE_ERROR_EXIST == g_file_error_from_errno(errno)) {
      /* Since we are rebuilding the module we need to be able to replace any
       * old copies. Remove old file and retry rename operation. */
      unlink(outfile_name);
...
static guint8 profread_byte(FILE *infile) {
  guint8 i;
  int res;
...
static int profread_int(FILE *infile) {
  int i, res;
...
static char *profread_string(FILE *infile) {
  int len, res;
  char *pbuf;
...
static void load_profile_file(MonoAotCompile *acfg, char *filename) {
  FILE *infile;
  char buf[1024];
  int res, len, version;
  char magic[32];
  ...
  guint32 expected_version =
      (AOT_PROFILER_MAJOR_VERSION << 16) | AOT_PROFILER_MINOR_VERSION;
  ...
  while (TRUE) {
    ...
    case AOTPROF_RECORD_GINST: {
      ...
      for (i = 0; i < len; ++i) {
        ...
        gdata->argv[i] = (ClassProfileData *)g_hash_table_lookup(
            data->classes, GINT_TO_POINTER(class_id));
    ...
    case AOTPROF_RECORD_TYPE: {
      ...
      case MONO_TYPE_CLASS: {
        ...
        ImageProfileData *image = (ImageProfileData *)g_hash_table_lookup(
            data->images, GINT_TO_POINTER(image_id));
        g_assert(image);
        ...
        *p = '\0';
        ...
        if (ginst_id != -1) {
          cdata->inst = (GInstProfileData *)g_hash_table_lookup(
              data->ginsts, GINT_TO_POINTER(ginst_id));
          g_assert(cdata->inst);
        }
        g_free(class_name);
    ...
    case AOTPROF_RECORD_METHOD: {
      ...
      ClassProfileData *klass = (ClassProfileData *)g_hash_table_lookup(
          data->classes, GINT_TO_POINTER(class_id));
      g_assert(klass);
      ...
      if (ginst_id != -1) {
        mdata->inst = (GInstProfileData *)g_hash_table_lookup(
            data->ginsts, GINT_TO_POINTER(ginst_id));
        g_assert(mdata->inst);
      }
...
static void resolve_class(ClassProfileData *cdata);

static void resolve_ginst(GInstProfileData *inst_data) {
  int i;
...
static void resolve_class(ClassProfileData *cdata) {
  ...
  klass = mono_class_from_name_checked(cdata->image->image, cdata->ns,
                                       cdata->name, error);
  if (!klass) {
...
/*
 * Resolve the profile data to the corresponding loaded classes/methods etc. if
 * possible.
 */
static void resolve_profile_data(MonoAotCompile *acfg, ProfileData *data,
                                 MonoAssembly *current) {
  GHashTableIter iter;
  gpointer key, value;
  int i;
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    if (!cdata->image->image) {
      if (acfg->aot_opts.verbose)
        printf("Unable to load class '%s.%s' because its image '%s' is not "
               "loaded.\n",
    ...
    /*
    if (cdata->klass)
            printf ("%s %s %s\n", cdata->ns, cdata->name, mono_class_full_name
    (cdata->klass));
    */
  }
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    if (!klass) {
      if (acfg->aot_opts.verbose)
        printf("Unable to load method '%s' because its class '%s.%s' is not "
               "loaded.\n",
...
static gboolean inst_references_image(MonoGenericInst *inst, MonoImage *image) {
  int i;
...
static gboolean is_local_inst(MonoGenericInst *inst, MonoImage *image) {
  int i;

  for (i = 0; i < inst->type_argc; ++i) {
    MonoClass *k = mono_class_from_mono_type_internal(inst->type_argv[i]);
    if (!MONO_TYPE_IS_PRIMITIVE(inst->type_argv[i]) &&
        m_class_get_image(k) != image)
      return FALSE;
  }
  return TRUE;
}
...
static void add_profile_instances(MonoAotCompile *acfg, ProfileData *data) {
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    /* For simplicity, add instances which reference the assembly we are
     * compiling */
    if (((ctx->class_inst &&
          inst_references_image(ctx->class_inst, acfg->image)) ||
         (ctx->method_inst &&
          inst_references_image(ctx->method_inst, acfg->image))) &&
        !mono_method_is_generic_sharable_full(m, FALSE, FALSE, FALSE)) {
      ...
      count++;
               ...
               ((ctx->class_inst &&
                 is_local_inst(ctx->class_inst, acfg->image)) ||
                (ctx->method_inst &&
                 is_local_inst(ctx->method_inst, acfg->image))) &&
               !mono_method_is_generic_sharable_full(m, FALSE, FALSE, FALSE)) {
      /* Add instances where the gtd is in the assembly and its inflated with
       * types from this assembly or corlib */
      // printf ("%s\n", mono_method_full_name (m, TRUE));
      add_extra_method(acfg, m);
      count++;
    }
     ...
     * Foo<Bar> won't be compiled when compiling Foo's assembly since it doesn't
     * match the first case, and it won't be compiled when compiling Bar's
     * assembly if Foo's assembly is not loaded.
     */
  }
...
static MonoAotCompile *acfg_create(MonoAssembly *ass, guint32 jit_opts) {
  ...
  acfg->method_to_pinvoke_import =
      g_hash_table_new_full(NULL, NULL, NULL, g_free);
  acfg->method_to_external_icall_symbol_name =
      g_hash_table_new_full(NULL, NULL, NULL, g_free);
  ...
  acfg->ginst_blob_hash = g_hash_table_new(mono_metadata_generic_inst_hash,
                                           mono_metadata_generic_inst_equal);
  acfg->plt_entry_debug_sym_cache = g_hash_table_new(g_str_hash, g_str_equal);
  acfg->gsharedvt_in_signatures =
      g_hash_table_new((GHashFunc)mono_signature_hash,
                       (GEqualFunc)mono_metadata_signature_equal);
  acfg->gsharedvt_out_signatures =
      g_hash_table_new((GHashFunc)mono_signature_hash,
                       (GEqualFunc)mono_metadata_signature_equal);
...
static void got_info_free(GotInfo *info) {
  int i;
...
static void acfg_free(MonoAotCompile *acfg) {
  int i;
...
static const char *const wrapper_type_names[MONO_WRAPPER_NUM + 1] = {
#include "mono/metadata/wrapper-types.h"
    NULL};

static G_GNUC_UNUSED const char *get_wrapper_type_name(int type) {
...
static void add_preinit_slot(MonoAotCompile *acfg, MonoJumpInfo *ji) {
  if (!acfg->aot_opts.llvm_only)
    get_got_offset(acfg, FALSE, ji);
  get_got_offset(acfg, TRUE, ji);
}

static void add_preinit_got_slots(MonoAotCompile *acfg) {
  MonoJumpInfo *ji;
  int i;

  /*
   * Allocate the first few GOT entries to information which is needed
   * frequently, or it is needed during method initialization etc.
   */
  ...
  if (!acfg->aot_opts.llvm_only) {
    for (i = 0; i < TLS_KEY_NUM; i++) {
      ji = (MonoJumpInfo *)mono_mempool_alloc0(acfg->mempool,
                                               sizeof(MonoJumpInfo));
...
static void mono_dedup_log_stats(MonoAotCompile *acfg) {
  GHashTableIter iter;
  g_assert(acfg->dedup_stats);
  ...
  while (
      g_hash_table_iter_next(&iter, (gpointer *)&method, (gpointer *)&dcfg)) {
    gchar *dedup_name = mono_aot_get_mangled_method_name(method);
    guint count =
        GPOINTER_TO_UINT(g_hash_table_lookup(acfg->dedup_stats, dedup_name));
  ...
  aot_printf(acfg,
             "Dedup Pass: Size Saved From Deduped Wrappers:\t%d methods, "
             "%" G_GSIZE_FORMAT "u bytes\n",
             wrappers_saved, wrappers_size_saved);
  aot_printf(acfg,
             "Dedup Pass: Size Saved From Inflated Methods:\t%d methods, "
             "%" G_GSIZE_FORMAT "u bytes\n",
             instances_saved, inflated_size_saved);
  if (acfg->dedup_emit_mode)
    aot_printf(acfg,
               "Dedup Pass: Size of Moved But Not Deduped (only 1 copy) "
               "Methods:\t%d methods, %" G_GSIZE_FORMAT "u bytes\n",
...
static MonoAotState *alloc_aot_state(void) {
...
static void free_aot_state(MonoAotState *astate) {
  g_hash_table_destroy(astate->cache);
  g_free(astate);
}

static void mono_add_deferred_extra_methods(MonoAotCompile *acfg,
                                            MonoAotState *astate) {
  GHashTableIter iter;
  gchar *name = NULL;
  MonoMethod *method = NULL;
  ...
  while (
      g_hash_table_iter_next(&iter, (gpointer *)&name, (gpointer *)&method)) {
    add_method_full(acfg, method, TRUE, 0);
  }
  return;
}

static void mono_setup_dedup_state(MonoAotCompile *acfg,
                                   MonoAotState **global_aot_state,
                                   MonoAssembly *ass, MonoAotState **astate,
                                   gboolean *is_dedup_dummy) {
  if (!acfg->aot_opts.dedup_include && !acfg->aot_opts.dedup)
    return;
...
int mono_compile_deferred_assemblies(guint32 opts, const char *aot_options,
                                     gpointer **aot_state) {
  ...
  if (!astate->inflated_assembly) {
    ...
    if (!inflate)
      ...
      g_error("Error: mono was not given an assembly with the provided inflate "
              "name\n");
  }
  ...
  int res = mono_compile_assembly(astate->inflated_assembly, opts, aot_options,
                                  aot_state);
...
#else
// Common signatures for which we use interp in wrappers even in fullaot +
// trampolines mode for increased performance
...
static void add_interp_in_wrapper_for_sig(MonoAotCompile *acfg,
                                          MonoMethodSignature *sig) {
  MonoMethod *wrapper;
...
int mono_compile_assembly(MonoAssembly *ass, guint32 opts,
  ...
  if (acfg->aot_opts.direct_extern_calls &&
      !(acfg->aot_opts.llvm && acfg->aot_opts.static_link)) {
    aot_printerrf(acfg, "The 'direct-extern-calls' option requires the 'llvm' "
                        "and 'static' options.\n");
    return 1;
  }
  ...
  mono_setup_dedup_state(acfg, (MonoAotState **)global_aot_state, ass, &astate,
                         &is_dedup_dummy);
  ...
  if (acfg->aot_opts.data_outfile) {
    ...
    acfg->flags =
        (MonoAotFileFlags)(acfg->flags | MONO_AOT_FILE_FLAG_SEPARATE_DATA);
  }
  ...
  if (acfg->aot_opts.llvm_only) {
    aot_printerrf(
        acfg, "--aot=llvmonly requires a runtime that supports gsharedvt.\n");
    return 1;
  }
#else
  ...
  if (mono_use_llvm || acfg->aot_opts.llvm) {
    aot_printerrf(
        acfg, "--aot=llvm requires a runtime compiled with llvm support.\n");
    return 1;
  }
#endif
  ...
  if (acfg->aot_opts.direct_pinvoke && !acfg->aot_opts.static_link) {
    aot_printerrf(acfg, "The 'direct-pinvoke' AOT option also requires the "
                        "'static' AOT option.\n");
    return 1;
  }
  ...
  if (mono_use_llvm || acfg->aot_opts.llvm) {
    ...
    acfg->aot_opts.asm_writer = TRUE;
    acfg->flags =
        (MonoAotFileFlags)(acfg->flags | MONO_AOT_FILE_FLAG_WITH_LLVM);

    if (acfg->aot_opts.soft_debug) {
      aot_printerrf(acfg, "The 'soft-debug' option is not supported when "
                          "compiling with LLVM.\n");
      return 1;
    }
    ...
    if (acfg->aot_opts.asm_only && !acfg->aot_opts.llvm_outfile) {
      aot_printerrf(acfg, "Compiling with LLVM and the asm-only option "
                          "requires the llvm-outfile= option.\n");
      return 1;
    }
  }
  ...
  if (mini_safepoints_enabled())
    acfg->flags =
        (MonoAotFileFlags)(acfg->flags | MONO_AOT_FILE_FLAG_SAFEPOINTS);
  ...
  if (astate && astate->emit_inflated_methods)
    acfg->flags =
        (MonoAotFileFlags)(acfg->flags | MONO_AOT_FILE_FLAG_EAGER_LOAD);
  ...
  if (!(mono_aot_mode_is_interp(&acfg->aot_opts) &&
    ...
    for (int method_index = 0;
         method_index < acfg->image->tables[MONO_TABLE_METHOD].rows;
         ++method_index)
      g_ptr_array_add(acfg->method_order, GUINT_TO_POINTER(method_index));
  }

  acfg->num_trampolines[MONO_AOT_TRAMP_SPECIFIC] =
      mono_aot_mode_is_full(&acfg->aot_opts) ? acfg->aot_opts.ntrampolines : 0;
#ifdef MONO_ARCH_GSHARED_SUPPORTED
  acfg->num_trampolines[MONO_AOT_TRAMP_STATIC_RGCTX] =
      mono_aot_mode_is_full(&acfg->aot_opts) ? acfg->aot_opts.nrgctx_trampolines
                                             : 0;
#endif
  acfg->num_trampolines[MONO_AOT_TRAMP_IMT] =
      mono_aot_mode_is_full(&acfg->aot_opts) ? acfg->aot_opts.nimt_trampolines
                                             : 0;
#ifdef MONO_ARCH_GSHAREDVT_SUPPORTED
  if (acfg->jit_opts & MONO_OPT_GSHAREDVT)
    acfg->num_trampolines[MONO_AOT_TRAMP_GSHAREDVT_ARG] =
        mono_aot_mode_is_full(&acfg->aot_opts)
            ? acfg->aot_opts.ngsharedvt_arg_trampolines
            : 0;
#endif
#ifdef MONO_ARCH_HAVE_FTNPTR_ARG_TRAMPOLINE
  acfg->num_trampolines[MONO_AOT_TRAMP_FTNPTR_ARG] =
      mono_aot_mode_is_interp(&acfg->aot_opts)
          ? acfg->aot_opts.nftnptr_arg_trampolines
          : 0;
#endif
  acfg->num_trampolines[MONO_AOT_TRAMP_UNBOX_ARBITRARY] =
      mono_aot_mode_is_interp(&acfg->aot_opts) &&
              mono_aot_mode_is_full(&acfg->aot_opts)
          ? acfg->aot_opts.nunbox_arbitrary_trampolines
          : 0;
  ...
  if (mono_use_llvm || acfg->aot_opts.llvm) {
    ...
    if (!acfg->aot_opts.asm_only && acfg->llvm_owriter_supported) {
      ...
      if (len >= 2 && acfg->aot_opts.llvm_outfile[len - 2] == '.' &&
          acfg->aot_opts.llvm_outfile[len - 1] == 'o')
        acfg->llvm_owriter = TRUE;
    }
  }
  ...
  if (acfg->llvm && acfg->thumb_mixed)
    acfg->flags =
        (MonoAotFileFlags)(acfg->flags | MONO_AOT_FILE_FLAG_LLVM_THUMB);
  if (acfg->aot_opts.llvm_only)
    acfg->flags =
        (MonoAotFileFlags)(acfg->flags | MONO_AOT_FILE_FLAG_LLVM_ONLY);
  ...
  if (acfg->llvm) {
    ...
    acfg->llvm_eh_frame_symbol =
        g_strdup_printf("%s_eh_frame", acfg->global_prefix);
  }
  ...
  if (acfg->llvm) {
    ...
    mono_llvm_create_aot_module(acfg->image->assembly, acfg->global_prefix,
                                acfg->nshared_got_entries, flags);
  ...
  if (mono_aot_mode_is_interp(&acfg->aot_opts) &&
    ...
    wrapper = mini_get_interp_lmf_wrapper(
    ...
    if (strcmp(acfg->image->assembly->aname.name, "mscorlib") == 0) {
      ...
      for (int i = 0; i < MONO_JIT_ICALL_count; ++i)
        add_jit_icall_wrapper(acfg,
                              mono_find_jit_icall_info((MonoJitICallId)i));
    }
  }
...
static void print_stats(MonoAotCompile *acfg) {
  int i;
  gint64 all_sizes;
  char llvm_stats_msg[256];

  if (acfg->llvm && !acfg->aot_opts.llvm_only)
    sprintf(llvm_stats_msg, ", LLVM: %d (%d%%)", acfg->stats.llvm_count,
            acfg->stats.mcount
                ? (acfg->stats.llvm_count * 100) / acfg->stats.mcount
                : 100);
  else
    strcpy(llvm_stats_msg, "");

  all_sizes = acfg->stats.code_size + acfg->stats.method_info_size +
              acfg->stats.ex_info_size + acfg->stats.unwind_info_size +
              acfg->stats.class_info_size + acfg->stats.got_info_size +
              acfg->stats.offsets_size + acfg->stats.plt_size;

  aot_printf(
      acfg,
      "Code: %d(%d%%) Info: %d(%d%%) Ex Info: %d(%d%%) Unwind Info: %d(%d%%) "
      "Class Info: %d(%d%%) PLT: %d(%d%%) GOT Info: %d(%d%%) Offsets: %d(%d%%) "
      "GOT: %d, BLOB: %d\n",
      (int)acfg->stats.code_size,
      (int)(acfg->stats.code_size * 100 / all_sizes),
      (int)acfg->stats.method_info_size,
      (int)(acfg->stats.method_info_size * 100 / all_sizes),
      (int)acfg->stats.ex_info_size,
      (int)(acfg->stats.ex_info_size * 100 / all_sizes),
      (int)acfg->stats.unwind_info_size,
      (int)(acfg->stats.unwind_info_size * 100 / all_sizes),
      (int)acfg->stats.class_info_size,
      (int)(acfg->stats.class_info_size * 100 / all_sizes),
      acfg->stats.plt_size ? (int)acfg->stats.plt_size : (int)acfg->plt_offset,
      acfg->stats.plt_size ? (int)(acfg->stats.plt_size * 100 / all_sizes) : 0,
      (int)acfg->stats.got_info_size,
      (int)(acfg->stats.got_info_size * 100 / all_sizes),
      (int)acfg->stats.offsets_size,
      (int)(acfg->stats.offsets_size * 100 / all_sizes),
      (int)(acfg->got_offset * sizeof(target_mgreg_t)),
      (int)acfg->stats.blob_size);
  aot_printf(
      acfg,
      "Compiled: %d/%d (%d%%)%s, No GOT slots: %d (%d%%), Direct calls: %d "
      "(%d%%)\n",
      acfg->stats.ccount, acfg->stats.mcount,
      acfg->stats.mcount ? (acfg->stats.ccount * 100) / acfg->stats.mcount
                         : 100,
      llvm_stats_msg, acfg->stats.methods_without_got_slots,
      acfg->stats.mcount
          ? (acfg->stats.methods_without_got_slots * 100) / acfg->stats.mcount
          : 100,
      acfg->stats.direct_calls,
      acfg->stats.all_calls
          ? (acfg->stats.direct_calls * 100) / acfg->stats.all_calls
          : 100);
  if (acfg->stats.genericcount)
    aot_printf(acfg, "%d methods failed gsharing (%d%%)\n",
               acfg->stats.genericcount,
               acfg->stats.mcount
                   ? (acfg->stats.genericcount * 100) / acfg->stats.mcount
                   : 100);
  if (acfg->stats.abscount)
    aot_printf(acfg, "%d methods contain absolute addresses (%d%%)\n",
               acfg->stats.abscount,
               acfg->stats.mcount
                   ? (acfg->stats.abscount * 100) / acfg->stats.mcount
                   : 100);
  if (acfg->stats.lmfcount)
    aot_printf(
        acfg, "%d methods contain lmf pointers (%d%%)\n", acfg->stats.lmfcount,
        acfg->stats.mcount ? (acfg->stats.lmfcount * 100) / acfg->stats.mcount
                           : 100);
  if (acfg->stats.ocount)
    aot_printf(
        acfg, "%d methods have other problems (%d%%)\n", acfg->stats.ocount,
        acfg->stats.mcount ? (acfg->stats.ocount * 100) / acfg->stats.mcount
                           : 100);
  ...
  for (i = 0; i < MONO_PATCH_INFO_NUM; ++i) {
    ...
    if (acfg->stats.got_slot_types[i])
      aot_printf(acfg, "\t%s: %d (%d)\n", get_patch_name(i),
                 acfg->stats.got_slot_types[i],
                 acfg->stats.got_slot_info_sizes[i]);
  }
  aot_printf(acfg, "GOT SLOTS: %d, INFO SIZE: %d\n", nslots, size);

  aot_printf(acfg, "\nEncoding stats:\n");
  aot_printf(acfg, "\tMethod ref: %d (%dk)\n", acfg->stats.method_ref_count,
             acfg->stats.method_ref_size / 1024);
  aot_printf(acfg, "\tClass ref: %d (%dk)\n", acfg->stats.class_ref_count,
             acfg->stats.class_ref_size / 1024);
  aot_printf(acfg, "\tGinst: %d (%dk)\n", acfg->stats.ginst_count,
             acfg->stats.ginst_size / 1024);
...
static void create_depfile(MonoAotCompile *acfg) {
  FILE *depfile;

  // FIXME: Support other configurations
  g_assert(acfg->aot_opts.llvm_only && acfg->aot_opts.asm_only &&
           acfg->aot_opts.llvm_outfile);
...
static int emit_aot_image(MonoAotCompile *acfg) {
  ...
  if (acfg->llvm) {
    if (acfg->aot_opts.asm_only) {
      ...
      if (strcmp(acfg->aot_opts.temp_path, "") != 0) {
        ...
        g_assertf(temp_path, "mkdtemp failed, error = (%d) %s", errno,
                  g_strerror(errno));
        acfg->temp_dir_to_delete = g_strdup(temp_path);
      }

      acfg->tmpbasename =
          g_build_filename(temp_path, "temp", (const char *)NULL);
      acfg->tmpfname = g_strdup_printf("%s.s", acfg->tmpbasename);
      acfg->llvm_sfile = g_strdup_printf("%s-llvm.s", acfg->tmpbasename);
      acfg->llvm_ofile =
          g_strdup_printf("%s-llvm." AS_OBJECT_FILE_SUFFIX, acfg->tmpbasename);
  ...
  if (acfg->aot_opts.asm_only && !acfg->aot_opts.llvm_only) {
    ...
    if (strcmp(acfg->aot_opts.temp_path, "") == 0) {
      ...
      acfg->tmpbasename = g_build_filename(acfg->aot_opts.temp_path, "temp",
                                           (const char *)NULL);
      acfg->tmpfname = g_strdup_printf("%s.s", acfg->tmpbasename);
      acfg->fp = fopen(acfg->tmpfname, "w+");
    }
    ...
    if (acfg->cfgs[i]) {
      ...
      if (cfg->asm_symbol) {
        ...
        if (acfg->llvm_label_prefix) {
          ...
          cfg->asm_symbol =
              g_strdup_printf("%s%s", acfg->llvm_label_prefix, cfg->asm_symbol);
          g_free(old_symbol);
        }
      } else if (COMPILE_LLVM(cfg)) {
        cfg->asm_symbol = g_strdup_printf("%s%s", acfg->llvm_label_prefix,
                                          cfg->llvm_method_name);
      } else if (acfg->global_symbols || acfg->llvm) {
        cfg->asm_symbol =
            get_debug_sym(cfg->orig_method, "", acfg->method_label_hash);
      } else {
        cfg->asm_symbol =
            g_strdup_printf("%s%sm_%x", acfg->temp_prefix,
                            acfg->llvm_label_prefix, method_index);
      }
      cfg->asm_debug_symbol = cfg->asm_symbol;
    }
  }
  ...
  if (acfg->aot_opts.dwarf_debug && acfg->aot_opts.gnu_asm) {
    /*
     * CLANG supports GAS .file/.loc directives, so emit line number information
     * this way
     */
  ...
  if ((!acfg->aot_opts.nodebug || acfg->aot_opts.dwarf_debug) &&
    ...
    acfg->dwarf =
        mono_dwarf_writer_create(acfg->w, NULL, 0, !acfg->gas_line_numbers);
  }
  ...
  if (acfg->dwarf)
    mono_dwarf_writer_emit_base_info(acfg->dwarf,
                                     g_path_get_basename(acfg->image->name),
  ...
  {
    /*
     * The managed allocators are GC specific, so can't use an AOT image created
     * by one GC in another.
     */
    const char *gc_name = mono_gc_get_gc_name();
    acfg->gc_name_offset =
        add_to_blob(acfg, (guint8 *)gc_name, strlen(gc_name) + 1);
  }
...
void *mono_aot_readonly_field_override(MonoClassField *field) { return NULL; }

int mono_compile_assembly(MonoAssembly *ass, guint32 opts,
                          const char *aot_options, gpointer **aot_state) {
  return 0;
}

gboolean mono_aot_is_shared_got_offset(int offset) { return FALSE; }

gboolean mono_aot_is_externally_callable(MonoMethod *cmethod) { return FALSE; }
...
gboolean mono_aot_can_enter_interp(MonoMethod *method) { return FALSE; }
