...
static void emit_info(MonoAotCompile *acfg) {
  int oindex, i;
  gint32 *offsets;
...
static void emit_aot_file_info(MonoAotCompile *acfg, MonoAotFileInfo *info) {
  ...
  if (acfg->llvm) {
    symbols[sindex++] = g_strdup_printf("%s%s", acfg->user_symbol_prefix,
                                        acfg->llvm_got_symbol);
...
static MonoAotCompile *acfg_create(MonoAssembly *ass, guint32 jit_opts) {
  ...
  /* TODO: Write out set of SIMD instructions used, rather than just those
   * available */
