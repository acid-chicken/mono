 ...
 */
static int mono_main_with_options(int argc, char *argv[]) {
 ...
 * 8-bytes         uint64_t offset in the file with the payload associated with
 STRING
 ...
 * "config:NAME"    A configuration file (usually file.dll.config) in the
 payload that is
 *                  loaded as the config file for an assembly
 * "systemconfig:"  Treats as a Mono system configuration, payload contains the
 config file.
 * "options:"       The payload contains command line options to initialize
 Mono, as if you had set them on MONO_ENV_OPTIONS
 * "config_dir:DIR" Configures the MONO_PATH to point to point to DIR
 * "machineconfig:" The payload contains the machine.config file to use at
 runtime
 * "env:"           Sets the environment variable to the value encoded in the
 payload
 *                  payload contains: 1-byte lenght for the \0 terminated
 variable,
 *                  followed by the value.
 * "library:NAME"   Bundled dynamic library NAME, payload contains the dynamic
 library
 */
...
#ifndef HOST_WIN32
static gboolean search_directories(const char *envPath, const char *program,
                                   char **new_program) {
  gchar **paths = NULL;
  gint i;
  ...
  for (i = 0; paths[i]; ++i) {
    ...
    while ((ent = readdir(dir))) {
      if (!strcmp(ent->d_name, program)) {
        *new_program = g_strdup_printf(
            "%s%s%s", path, path[path_len - 1] == '/' ? "" : "/", program);
        closedir(dir);
        g_strfreev(paths);
        return TRUE;
      }
...
static gboolean probe_embedded(const char *program, int *ref_argc,
                               char **ref_argv[]) {
...
#ifndef HOST_WIN32
  if (fd == -1) {
    // Also search through the PATH in case the program was run from a different
    // directory
  ...
  for (i = 0; i < items; i++) {
    ...
    if (mapaddress == NULL) {
      ...
      mapaddress = (guchar *)mono_file_map_error(
          directory_location - offset, MONO_MMAP_READ | MONO_MMAP_PRIVATE, fd,
          offset, &maphandle, program, &error_message);
      if (mapaddress == NULL) {
        if (error_message)
    ...
    if (strncmp(kind, "assembly:", strlen("assembly:")) == 0) {
      ...
      MonoBundledAssembly mba = {aname, mapaddress + offset - baseline,
                                 item_size},
                          *ptr;
      ...
      mono_register_config_for_assembly(
          aname, g_str_from_file_region(fd, offset, item_size));
      ...
      mono_parse_options_from(g_str_from_file_region(fd, offset, item_size),
                              ref_argc, ref_argv);
    } else if (strncmp(kind, "config_dir:", strlen("config_dir:")) == 0) {
      char *mono_path_value = g_getenv("MONO_PATH");
      mono_set_dirs(mono_path_value,
                    g_str_from_file_region(fd, offset, item_size));
      ...
      mono_loader_save_bundled_library(fd, offset, item_size,
                                       kind + strlen("library:"));
    } else {
  ...
  mono_register_bundled_assemblies(
      (const MonoBundledAssembly **)assemblies->data);
...
const char *mono_lookup_icall_symbol_internal(gpointer func);
