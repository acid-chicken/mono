...
gpointer mono_arch_get_restore_context(MonoTrampInfo **info, gboolean aot) {
  ...
  if (!mono_arch_is_soft_float()) {
    ARM_ADD_REG_IMM8(code, ARMREG_IP, ctx_reg,
  ...
  ARM_STR_IMM(code, ARMREG_IP, ctx_reg,
              MONO_STRUCT_OFFSET(MonoContext, regs) +
                  (ARMREG_PC * sizeof(target_mgreg_t)));
...
gpointer mono_arch_get_call_filter(MonoTrampInfo **info, gboolean aot) {
  ...
  ctx_reg = ARMREG_R0;
  ARM_LDR_IMM(code, ARMREG_IP, ctx_reg, MONO_STRUCT_OFFSET(MonoContext, pc));
  ARM_ADD_REG_IMM8(code, ARMREG_LR, ctx_reg,
                   MONO_STRUCT_OFFSET(MonoContext, regs) +
                       (MONO_ARM_FIRST_SAVED_REG * sizeof(target_mgreg_t)));
...
void mono_arm_throw_exception(MonoObject *exc, host_mgreg_t pc, host_mgreg_t sp,
  ...
  memcpy(((guint8 *)&ctx.regs) + (ARMREG_R4 * sizeof(host_mgreg_t)), int_regs,
...
void mono_arm_throw_exception_by_token(guint32 ex_token_index, host_mgreg_t pc,
                                       host_mgreg_t sp, host_mgreg_t *int_regs,
                                       gdouble *fp_regs) {
  ...
  mono_arm_throw_exception(
      ...
      pc, sp, int_regs, fp_regs, FALSE);
}

void mono_arm_resume_unwind(guint32 dummy1, host_mgreg_t pc, host_mgreg_t sp,
                            host_mgreg_t *int_regs, gdouble *fp_regs) {
  MonoContext ctx;
...
static gpointer get_throw_trampoline(int size, gboolean corlib,
  ...
  mono_add_unwind_op_offset(unwind_ops, code, start, ARMREG_LR,
  ...
  param_size =
      ALIGN_TO(cfa_offset + param_size, MONO_ARCH_FRAME_ALIGNMENT) - cfa_offset;
  ...
  if (aot) {
    ...
    *(gpointer *)(gpointer)code = NULL;
    ...
    code = mono_arm_emit_load_imm(
        code, ARMREG_IP,
        GPOINTER_TO_UINT(
            resume_unwind
                ? (gpointer)mono_arm_resume_unwind
                : (corlib ? (gpointer)mono_arm_throw_exception_by_token
                          : (gpointer)mono_arm_throw_exception)));
  }
  ARM_MOV_REG_REG(code, ARMREG_LR, ARMREG_PC);
...
gpointer mono_arch_get_throw_exception(MonoTrampInfo **info, gboolean aot) {
  return get_throw_trampoline(132, FALSE, FALSE, FALSE, FALSE,
...
gpointer mono_arch_get_rethrow_exception(MonoTrampInfo **info, gboolean aot) {
  return get_throw_trampoline(132, FALSE, TRUE, FALSE, FALSE,
 ...
 * Here, \c offset is the offset which needs to be substracted from the caller
 * IP to get the IP of the throw. Passing the offset has the advantage that it
 * needs no relocations in the caller.
 * On ARM, the ip is passed instead of an offset.
 */
gpointer mono_arch_get_throw_corlib_exception(MonoTrampInfo **info,
                                              gboolean aot) {
  return get_throw_trampoline(168, TRUE, FALSE, FALSE, FALSE,
...
GSList *mono_arm_get_exception_trampolines(gboolean aot) {
  ...
  get_throw_trampoline(168, TRUE, FALSE, FALSE, FALSE,
  ...
  get_throw_trampoline(168, TRUE, FALSE, TRUE, FALSE,
  ...
  get_throw_trampoline(168, FALSE, FALSE, FALSE, TRUE,
  ...
  if (ji != NULL) {
    ...
    */
    ...
    gboolean success = mono_unwind_frame(
        ...
        MONO_MAX_IREGS + 8, save_locations, MONO_MAX_IREGS, &cfa);
    ...
    if ((ji = mini_jit_info_table_find(domain, (gpointer)(gsize)(*lmf)->ip,
                                       NULL))) {
      frame->ji = ji;
    } else {
      if (!(*lmf)->method)
 ...
 */
static MONO_NEVER_INLINE gpointer get_handle_signal_exception_addr(void) {
