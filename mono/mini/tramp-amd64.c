...
guint8 *mono_aot_arch_get_plt_entry_exec_only(gpointer amodule_info,
                                              host_mgreg_t *regs, guint8 *code,
                                              guint8 *plt);
guint32 mono_arch_get_plt_info_offset_exec_only(
...
void mono_arch_patch_plt_entry_exec_only(gpointer amodule_info, guint8 *code,
                                         gpointer *got, host_mgreg_t *regs,
                                         guint8 *addr);
#endif
...
gpointer mono_arch_get_unbox_trampoline(MonoMethod *m, gpointer addr) {
  ...
  start = code = (guint8 *)mono_domain_code_reserve(
  ...
  MONO_PROFILER_RAISE(
      jit_code_buffer,
      (start, code - start, MONO_PROFILER_CODE_BUFFER_UNBOX_TRAMPOLINE, m));
...
gpointer mono_arch_get_static_rgctx_trampoline(gpointer arg, gpointer addr) {
  ...
  start = code = (guint8 *)mono_domain_code_reserve(
      domain, buf_len + MONO_TRAMPOLINE_UNWINDINFO_SIZE(0));
  ...
  MONO_PROFILER_RAISE(jit_code_buffer,
                      (start, code - start,
                       MONO_PROFILER_CODE_BUFFER_GENERICS_TRAMPOLINE, NULL));
 ...
 *
 *   Patch the callsite whose address is given by ORIG_CODE so it calls ADDR.
 * ORIG_CODE points to the pc right after the call.
 */
void mono_arch_patch_callsite(guint8 *method_start, guint8 *orig_code,
                              guint8 *addr) {
  guint8 *code;
  guint8 buf[16];

  // Since method_start is retrieved from function return address (below current
  // call/jmp to patch) there is a case when last instruction of a function is
  // the call (due to OP_NOT_REACHED) instruction and then directly followed by
  // a different method. In that case current orig_code points into next method
  // and method_start will also point into next method, not the method including
  // the call to patch. For this specific case, fallback to using a method_start
  // of NULL.
  gboolean can_write = mono_breakpoint_clean_code(
      method_start != orig_code ? method_start : NULL, orig_code, 14, buf,
      sizeof(buf));
  ...
  if (((code[-13] == 0x49) && (code[-12] == 0xbb)) || (code[-5] == 0xe8)) {
    if (code[-5] != 0xe8) {
      ...
      gboolean disp_32bit =
          ((((gint64)addr - (gint64)orig_code)) < (1 << 30)) &&
          ((((gint64)addr - (gint64)orig_code)) > -(1 << 30));

      if ((((guint64)(addr)) >> 32) != 0 && !disp_32bit) {
        /*
         * This might happen with LLVM or when calling AOTed code. Create a
         * thunk.
         */
        guint8 *thunk_start, *thunk_code;

        thunk_start = thunk_code =
            (guint8 *)mono_domain_code_reserve(mono_domain_get(), 32);
        ...
        *(guint64 *)thunk_code = (guint64)addr;
        ...
        MONO_PROFILER_RAISE(jit_code_buffer,
                            (thunk_start, thunk_code - thunk_start,
                             MONO_PROFILER_CODE_BUFFER_HELPER, NULL));
      }
      if (can_write) {
        mono_atomic_xchg_i32((gint32 *)(orig_code - 4),
                             ((gint64)addr - (gint64)orig_code));
    ...
    /* call *<OFFSET>(%rip) */
    gpointer *got_entry =
        (gpointer *)((guint8 *)orig_code + (*(guint32 *)(orig_code - 4)));
    if (can_write) {
...
guint8 *mono_arch_create_llvm_native_thunk(MonoDomain *domain, guint8 *addr) {
  /*
   * The caller is LLVM code and the call displacement might exceed 32 bits. We
   * can't determine the caller address, so we add a thunk every time. Since the
   * caller is also allocated using the domain code manager, hopefully the
   * displacement will fit into 32 bits.
...
static void stack_unaligned(MonoTrampolineType tramp_type) {
  printf("%d\n", tramp_type);
...
guchar *mono_arch_create_generic_trampoline(MonoTrampolineType tramp_type,
  ...
  int i, lmf_offset, offset, res_offset, arg_offset, rax_offset, tramp_offset,
      ctx_offset, saved_regs_offset;
  int r11_save_offset, saved_fpregs_offset, rbp_offset, framesize,
      orig_rsp_to_rbp_offset, cfa_offset;
  gboolean has_caller;
  ...
  code = buf = (guint8 *)mono_global_codeman_reserve(
  ...
  amd64_mov_membase_reg(code, AMD64_RBP, tramp_offset, AMD64_R11,
  ...
  for (i = 0; i < AMD64_NREG; ++i) {
    if (i == AMD64_RBP) {
      ...
      amd64_mov_reg_membase(code, AMD64_RAX, AMD64_RBP, rbp_offset,
                            sizeof(target_mgreg_t));
      amd64_mov_membase_reg(code, AMD64_RBP,
                            saved_regs_offset + (i * sizeof(target_mgreg_t)),
                            AMD64_RAX, sizeof(target_mgreg_t));
    } else if (i == AMD64_RIP) {
      if (has_caller)
        amd64_mov_reg_membase(code, AMD64_R11, AMD64_RBP, 8,
                              sizeof(target_mgreg_t));
      else
        amd64_mov_reg_imm(code, AMD64_R11, 0);
      amd64_mov_membase_reg(code, AMD64_RBP,
                            saved_regs_offset + (i * sizeof(target_mgreg_t)),
                            AMD64_R11, sizeof(target_mgreg_t));
    } else if (i == AMD64_RSP) {
      ...
      amd64_mov_membase_reg(code, AMD64_RBP,
                            saved_regs_offset + (i * sizeof(target_mgreg_t)), i,
                            sizeof(target_mgreg_t));
    } else {
      ...
      amd64_mov_membase_reg(r11_save_code, AMD64_RSP,
                            r11_save_offset + orig_rsp_to_rbp_offset, i,
                            sizeof(target_mgreg_t));
      g_assert(r11_save_code == after_r11_save_code);

      /* Copy from the save slot into the register array slot */
      amd64_mov_reg_membase(code, i, AMD64_RSP,
                            r11_save_offset + orig_rsp_to_rbp_offset +
                                framesize,
                            sizeof(target_mgreg_t));
      amd64_mov_membase_reg(code, AMD64_RBP,
                            saved_regs_offset + (i * sizeof(target_mgreg_t)), i,
    ...
    /* cfa = rbp + cfa_offset */
    mono_add_unwind_op_offset(unwind_ops, code, buf, i,
                              -cfa_offset + saved_regs_offset +
                                  (i * sizeof(target_mgreg_t)));
  }
  for (i = 0; i < AMD64_XMM_NREG; ++i)
    if (AMD64_IS_ARGUMENT_XREG(i))
#if defined(MONO_HAVE_SIMD_REG)
      amd64_movdqu_membase_reg(
          code, AMD64_RBP,
          saved_fpregs_offset + (i * sizeof(MonoContextSimdReg)), i);
#else
      amd64_movsd_membase_reg(code, AMD64_RBP,
                              saved_fpregs_offset + (i * sizeof(double)), i);
#endif
  ...
  if (aot) {
     ...
     * arg = mscorlib_amodule->got [specific_trampolines_got_offsets_base +
     * (tramp_index * 2) + 1]
     */
    code = mono_arch_emit_load_aotconst(
        buf, code, &ji, MONO_PATCH_INFO_SPECIFIC_TRAMPOLINES, NULL);
    /* Trampoline addr */
    amd64_mov_reg_membase(code, AMD64_RAX, AMD64_RBP, tramp_offset,
                          sizeof(target_mgreg_t));
    /* Trampoline offset */
    ...
    code = mono_arch_emit_load_aotconst(
        buf, code, &ji, MONO_PATCH_INFO_SPECIFIC_TRAMPOLINES_GOT_SLOTS_BASE,
        NULL);
    ...
    amd64_mov_reg_membase(code, AMD64_R11, AMD64_RAX, sizeof(target_mgreg_t),
                          sizeof(target_mgreg_t));
  } else {
    amd64_mov_reg_membase(code, AMD64_R11, AMD64_RBP, tramp_offset,
                          sizeof(target_mgreg_t));
  ...
  amd64_mov_membase_reg(code, AMD64_RBP, arg_offset, AMD64_R11,
                        sizeof(target_mgreg_t));
  ...
  amd64_mov_membase_reg(code, AMD64_RBP,
                        lmf_offset + MONO_STRUCT_OFFSET(MonoLMFTramp, ctx),
                        AMD64_R11, sizeof(target_mgreg_t));

  if (aot) {
    code = mono_arch_emit_load_aotconst(
        buf, code, &ji, MONO_PATCH_INFO_JIT_ICALL_ADDR,
  ...
  /* Save lmf_addr */
  amd64_mov_membase_reg(code, AMD64_RBP,
                        lmf_offset + MONO_STRUCT_OFFSET(MonoLMFTramp, lmf_addr),
                        AMD64_RAX, sizeof(target_mgreg_t));
  /* Save previous_lmf */
  ...
  amd64_mov_membase_reg(code, AMD64_RBP,
                        lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, previous_lmf),
                        AMD64_R11, sizeof(target_mgreg_t));
  /* Set new lmf */
  ...
  if (has_caller)
    amd64_mov_reg_membase(code, AMD64_ARG_REG2, AMD64_RBP, 8,
                          sizeof(target_mgreg_t));
  else
    amd64_mov_reg_imm(code, AMD64_ARG_REG2, 0);

  /* Arg3 is the method/vtable ptr */
  amd64_mov_reg_membase(code, AMD64_ARG_REG3, AMD64_RBP, arg_offset,
                        sizeof(target_mgreg_t));

  /* Arg4 is the trampoline address */
  amd64_mov_reg_membase(code, AMD64_ARG_REG4, AMD64_RBP, tramp_offset,
                        sizeof(target_mgreg_t));
  ...
  amd64_call_reg(code, AMD64_R11);
  amd64_mov_membase_reg(code, AMD64_RBP, res_offset, AMD64_RAX,
                        sizeof(target_mgreg_t));

  /* Restore LMF */
  amd64_mov_reg_membase(code, AMD64_RCX, AMD64_RBP,
                        lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, previous_lmf),
                        sizeof(target_mgreg_t));
  amd64_alu_reg_imm_size(code, X86_SUB, AMD64_RCX, 0x5, sizeof(target_mgreg_t));
  amd64_mov_reg_membase(code, AMD64_R11, AMD64_RBP,
                        lmf_offset + MONO_STRUCT_OFFSET(MonoLMFTramp, lmf_addr),
                        sizeof(target_mgreg_t));
  amd64_mov_membase_reg(code, AMD64_R11, 0, AMD64_RCX, sizeof(target_mgreg_t));

  /*
   * Save rax to the stack, after the leave instruction, this will become part
   * of the red zone.
   */
  amd64_mov_reg_membase(code, AMD64_RAX, AMD64_RBP, res_offset,
                        sizeof(target_mgreg_t));
  amd64_mov_membase_reg(code, AMD64_RBP, rax_offset, AMD64_RAX,
                        sizeof(target_mgreg_t));

  /* Check for thread interruption */
  /* This is not perf critical code so no need to check the interrupt flag */
  /*
   * Have to call the _force_ variant, since there could be a protected wrapper
  ...
  if (aot) {
    ...
    amd64_mov_reg_imm(
        code, AMD64_R11,
        (guint8 *)mono_thread_force_interruption_checkpoint_noraise);
  }
  amd64_call_reg(code, AMD64_R11);
  ...
  if (aot) {
    ...
    amd64_mov_reg_imm(code, AMD64_R11,
                      (guint8 *)mono_get_rethrow_preserve_exception_addr());
  }
  ...
  for (i = 0; i < AMD64_NREG; ++i)
    if (AMD64_IS_ARGUMENT_REG(i) || i == AMD64_R10 || i == AMD64_RAX ||
        i == AMD64_R11)
      amd64_mov_reg_membase(code, i, AMD64_RBP,
                            saved_regs_offset + (i * sizeof(target_mgreg_t)),
                            sizeof(target_mgreg_t));
  for (i = 0; i < AMD64_XMM_NREG; ++i)
    if (AMD64_IS_ARGUMENT_XREG(i))
#if defined(MONO_HAVE_SIMD_REG)
      amd64_movdqu_reg_membase(code, i, AMD64_RBP,
                               saved_fpregs_offset +
                                   (i * sizeof(MonoContextSimdReg)));
#else
      amd64_movsd_reg_membase(code, i, AMD64_RBP,
                              saved_fpregs_offset + (i * sizeof(double)));
#endif
  ...
  if (MONO_TRAMPOLINE_TYPE_MUST_RETURN(tramp_type)) {
    /* Load result */
    amd64_mov_reg_membase(code, AMD64_RAX, AMD64_RSP,
                          rax_offset - sizeof(target_mgreg_t),
                          sizeof(target_mgreg_t));
    amd64_ret(code);
  } else {
  ...
  g_assertf((code - buf) <= kMaxCodeSize, "%d %d", code, buf, (int)(code - buf),
            kMaxCodeSize);
  g_assert_checked(mono_arch_unwindinfo_validate_size(
  ...
  MONO_PROFILER_RAISE(
      jit_code_buffer,
      (buf, code - buf, MONO_PROFILER_CODE_BUFFER_HELPER, NULL));
...
gpointer mono_arch_create_rgctx_lazy_fetch_trampoline(guint32 slot,
  ...
  if (mrgctx)
    index +=
        MONO_SIZEOF_METHOD_RUNTIME_GENERIC_CONTEXT / sizeof(target_mgreg_t);
  for (depth = 0;; ++depth) {
  ...
  code = buf = (guint8 *)mono_global_codeman_reserve(
      tramp_size + MONO_TRAMPOLINE_UNWINDINFO_SIZE(0));
  ...
  for (i = 0; i < depth; ++i) {
    ...
    if (mrgctx && i == 0)
      amd64_mov_reg_membase(code, AMD64_RAX, AMD64_RAX,
                            MONO_SIZEOF_METHOD_RUNTIME_GENERIC_CONTEXT,
                            sizeof(target_mgreg_t));
    else
      amd64_mov_reg_membase(code, AMD64_RAX, AMD64_RAX, 0,
                            sizeof(target_mgreg_t));
    /* is the ptr null? */
    amd64_test_reg_reg(code, AMD64_RAX, AMD64_RAX);
    /* if yes, jump to actual trampoline */
  ...
  if (MONO_ARCH_VTABLE_REG != AMD64_ARG_REG1) {
    ...
    amd64_mov_reg_reg(code, MONO_ARCH_VTABLE_REG, AMD64_ARG_REG1,
                      sizeof(target_mgreg_t));
  }
  ...
  if (aot) {
    ...
    tramp = (guint8 *)mono_arch_create_specific_trampoline(
        GUINT_TO_POINTER(slot), MONO_TRAMPOLINE_RGCTX_LAZY_FETCH,
  ...
  MONO_PROFILER_RAISE(
      jit_code_buffer,
      (buf, code - buf, MONO_PROFILER_CODE_BUFFER_GENERICS_TRAMPOLINE, NULL));
...
mono_arch_create_general_rgctx_lazy_fetch_trampoline(MonoTrampInfo **info,
  ...
  if (info)
    *info = mono_tramp_info_create("rgctx_fetch_trampoline_general", buf,
                                   code - buf, ji, unwind_ops);
...
/* If PLT_ENTRY_OFFSET_REG is R8 - R15, increase mov instruction size by 1 due
 * to use of REX. */
...
static guchar aot_arch_get_plt_entry_size(MonoAotFileInfo *info,
                                          host_mgreg_t *regs, guint8 *code,
                                          guint8 *plt) {
...
static guint32 aot_arch_get_plt_entry_index(MonoAotFileInfo *info,
                                            host_mgreg_t *regs, guint8 *code,
                                            guint8 *plt) {
  if (info->plt_size <= 0xFF)
...
guint8 *mono_aot_arch_get_plt_entry_exec_only(gpointer amodule_info,
                                              host_mgreg_t *regs, guint8 *code,
                                              guint8 *plt) {
  guint32 plt_entry_index = aot_arch_get_plt_entry_index(
      (MonoAotFileInfo *)amodule_info, regs, code, plt);
  guchar plt_entry_size = aot_arch_get_plt_entry_size(
      (MonoAotFileInfo *)amodule_info, regs, code, plt);
...
guint32 mono_arch_get_plt_info_offset_exec_only(
    ...
    MonoAotResolvePltInfoOffset resolver, gpointer amodule) {
  guint32 plt_entry_index = aot_arch_get_plt_entry_index(
      (MonoAotFileInfo *)amodule_info, regs, code, NULL);
...
void mono_arch_patch_plt_entry_exec_only(gpointer amodule_info, guint8 *code,
                                         gpointer *got, host_mgreg_t *regs,
                                         guint8 *addr) {
  /* Same calculation of GOT offset as done in aot-compiler.c, emit_plt and used
   * as jmp DISP. */
  guint32 plt_entry_index = aot_arch_get_plt_entry_index(
      (MonoAotFileInfo *)amodule_info, regs, code, NULL);
  gpointer *plt_jump_table_entry =
      ((gpointer *)(got +
                    ((MonoAotFileInfo *)amodule_info)->plt_got_offset_base) +
       plt_entry_index);
 ...
 * ()/mono_debugger_agent_breakpoint_from_context (), then restores the
 * (potentially changed) context.
 */
guint8 *mono_arch_create_sdb_trampoline(gboolean single_step,
  ...
  code = buf = (guint8 *)mono_global_codeman_reserve(
      tramp_size + MONO_MAX_TRAMPOLINE_UNWINDINFO_SIZE);
  ...
  for (i = 0; i < AMD64_NREG; ++i) {
    if (i != AMD64_RIP && i != AMD64_RSP && i != AMD64_RBP)
      amd64_mov_membase_reg(code, AMD64_RSP,
                            gregs_offset + (i * sizeof(target_mgreg_t)), i,
                            sizeof(target_mgreg_t));
  }
  amd64_mov_reg_membase(code, AMD64_R11, AMD64_RBP, 0, sizeof(target_mgreg_t));
  amd64_mov_membase_reg(code, AMD64_RSP,
                        gregs_offset + (AMD64_RBP * sizeof(target_mgreg_t)),
                        AMD64_R11, sizeof(target_mgreg_t));
  amd64_lea_membase(code, AMD64_R11, AMD64_RBP, 2 * sizeof(target_mgreg_t));
  amd64_mov_membase_reg(code, AMD64_RSP,
                        gregs_offset + (AMD64_RSP * sizeof(target_mgreg_t)),
                        AMD64_R11, sizeof(target_mgreg_t));
  amd64_mov_reg_membase(code, AMD64_R11, AMD64_RBP, sizeof(target_mgreg_t),
                        sizeof(target_mgreg_t));
  amd64_mov_membase_reg(code, AMD64_RSP,
                        gregs_offset + (AMD64_RIP * sizeof(target_mgreg_t)),
                        AMD64_R11, sizeof(target_mgreg_t));
  ...
  if (aot) {
    if (single_step)
      code = mono_arch_emit_load_aotconst(
          buf, code, &ji, MONO_PATCH_INFO_JIT_ICALL_ADDR,
          GUINT_TO_POINTER(
              MONO_JIT_ICALL_mono_debugger_agent_single_step_from_context));
    else
      code = mono_arch_emit_load_aotconst(
          buf, code, &ji, MONO_PATCH_INFO_JIT_ICALL_ADDR,
          GUINT_TO_POINTER(
              MONO_JIT_ICALL_mono_debugger_agent_breakpoint_from_context));
  } else {
    if (single_step)
      amd64_mov_reg_imm(code, AMD64_R11,
  ...
  for (i = 0; i < AMD64_NREG; ++i) {
    if (i != AMD64_RIP && i != AMD64_RSP && i != AMD64_RBP)
      amd64_mov_reg_membase(code, i, AMD64_RSP,
                            gregs_offset + (i * sizeof(target_mgreg_t)),
                            sizeof(target_mgreg_t));
  }
  amd64_mov_reg_membase(code, AMD64_R11, AMD64_RSP,
                        gregs_offset + (AMD64_RBP * sizeof(target_mgreg_t)),
                        sizeof(target_mgreg_t));
  ...
  amd64_mov_reg_membase(code, AMD64_R11, AMD64_RSP,
                        gregs_offset + (AMD64_RIP * sizeof(target_mgreg_t)),
                        sizeof(target_mgreg_t));
  amd64_mov_membase_reg(code, AMD64_RBP, sizeof(target_mgreg_t), AMD64_R11,
                        sizeof(target_mgreg_t));
  ...
  const char *tramp_name =
      single_step ? "sdb_single_step_trampoline" : "sdb_breakpoint_trampoline";
  *info = mono_tramp_info_create(tramp_name, buf, code - buf, ji, unwind_ops);
...
gpointer mono_arch_get_interp_to_native_trampoline(MonoTrampInfo **info) {
  ...
  start = code = (guint8 *)mono_global_codeman_reserve(
      buf_len + MONO_MAX_TRAMPOLINE_UNWINDINFO_SIZE);
  ...
  off_methodargs = -8;
  amd64_mov_membase_reg(code, AMD64_RBP, off_methodargs, AMD64_ARG_REG2,
                        sizeof(target_mgreg_t));

  /* save target address on stack */
  off_targetaddr = -2 * 8;
  amd64_mov_membase_reg(code, AMD64_RBP, off_targetaddr, AMD64_ARG_REG1,
                        sizeof(target_mgreg_t));
  ...
  amd64_mov_reg_membase(code, AMD64_RAX, AMD64_R11,
                        MONO_STRUCT_OFFSET(CallContext, stack_size),
                        sizeof(target_mgreg_t));
  ...
  amd64_mov_reg_membase(code, AMD64_ARG_REG2, AMD64_R11,
                        MONO_STRUCT_OFFSET(CallContext, stack),
                        sizeof(target_mgreg_t));
  ...
  amd64_mov_reg_membase(code, AMD64_ARG_REG3, AMD64_ARG_REG2, 0,
                        sizeof(target_mgreg_t));
  amd64_mov_membase_reg(code, AMD64_ARG_REG1, 0, AMD64_ARG_REG3,
                        sizeof(target_mgreg_t));
  ...
  for (i = 0; i < PARAM_REGS; i++)
    amd64_mov_reg_membase(code, param_regs[i], AMD64_R11,
                          MONO_STRUCT_OFFSET(CallContext, gregs) +
                              param_regs[i] * sizeof(target_mgreg_t),
                          sizeof(target_mgreg_t));

  /* set all floating registers from CallContext  */
  for (i = 0; i < FLOAT_PARAM_REGS; ++i)
    amd64_sse_movsd_reg_membase(code, i, AMD64_R11,
                                MONO_STRUCT_OFFSET(CallContext, fregs) +
                                    i * sizeof(double));

  /* load target addr */
  amd64_mov_reg_membase(code, AMD64_R11, AMD64_RBP, off_targetaddr,
                        sizeof(target_mgreg_t));
  ...
  amd64_mov_reg_membase(code, AMD64_R11, AMD64_RBP, off_methodargs,
                        sizeof(target_mgreg_t));
  for (i = 0; i < RETURN_REGS; i++)
    amd64_mov_membase_reg(code, AMD64_R11,
                          MONO_STRUCT_OFFSET(CallContext, gregs) +
                              return_regs[i] * sizeof(target_mgreg_t),
                          return_regs[i], sizeof(target_mgreg_t));
  ...
  for (i = 0; i < FLOAT_RETURN_REGS; i++)
    amd64_sse_movsd_membase_reg(
        code, AMD64_R11,
        MONO_STRUCT_OFFSET(CallContext, fregs) + i * sizeof(double), i);
  ...
  MONO_PROFILER_RAISE(
      jit_code_buffer,
      (start, code - start, MONO_PROFILER_CODE_BUFFER_HELPER, NULL));

  if (info)
    *info = mono_tramp_info_create("interp_to_native_trampoline", start,
                                   code - start, ji, unwind_ops);
...
gpointer mono_arch_get_native_to_interp_trampoline(MonoTrampInfo **info) {
  ...
  for (i = 0; i < PARAM_REGS; i++)
    amd64_mov_membase_reg(code, AMD64_RSP,
                          ctx_offset + MONO_STRUCT_OFFSET(CallContext, gregs) +
                              param_regs[i] * sizeof(target_mgreg_t),
                          param_regs[i], sizeof(target_mgreg_t));

  /* save all floating registers into the CallContext  */
  for (i = 0; i < FLOAT_PARAM_REGS; i++)
    amd64_sse_movsd_membase_reg(code, AMD64_RSP,
                                ctx_offset +
                                    MONO_STRUCT_OFFSET(CallContext, fregs) +
                                    i * sizeof(double),
                                i);
  ...
  amd64_mov_membase_reg(code, AMD64_RSP,
                        ctx_offset + MONO_STRUCT_OFFSET(CallContext, stack),
                        AMD64_R11, sizeof(target_mgreg_t));
  ...
  amd64_mov_reg_membase(code, AMD64_ARG_REG2, MONO_ARCH_RGCTX_REG,
                        MONO_STRUCT_OFFSET(MonoFtnDesc, arg),
                        sizeof(target_mgreg_t));
  amd64_mov_reg_membase(code, AMD64_R11, MONO_ARCH_RGCTX_REG,
                        MONO_STRUCT_OFFSET(MonoFtnDesc, addr),
                        sizeof(target_mgreg_t));
  amd64_call_reg(code, AMD64_R11);

  /* load the return values from the context */
  for (i = 0; i < RETURN_REGS; i++)
    amd64_mov_reg_membase(code, return_regs[i], AMD64_RSP,
                          ctx_offset + MONO_STRUCT_OFFSET(CallContext, gregs) +
                              return_regs[i] * sizeof(target_mgreg_t),
                          sizeof(target_mgreg_t));

  for (i = 0; i < FLOAT_RETURN_REGS; i++)
    amd64_sse_movsd_reg_membase(code, i, AMD64_RSP,
                                ctx_offset +
                                    MONO_STRUCT_OFFSET(CallContext, fregs) +
                                    i * sizeof(double));
  ...
  if (info)
    *info = mono_tramp_info_create("native_to_interp_trampoline", start,
                                   code - start, ji, unwind_ops);
