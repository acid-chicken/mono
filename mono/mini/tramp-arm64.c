...
void mono_arch_patch_plt_entry(guint8 *code, gpointer *got, host_mgreg_t *regs,
  ...
  /*
   * Decode the address loaded by the PLT entry emitted by arch_emit_plt_entry
   * () in aot-compiler.c
   */
...
guchar *mono_arch_create_generic_trampoline(MonoTrampolineType tramp_type,
  ...
  int i, buf_len, imm;
  int frame_size, offset, gregs_offset, num_fregs, fregs_offset, arg_offset,
      lmf_offset, res_offset;
  ...
  /*
   * We are getting called by a specific trampoline, ip1 contains the trampoline
   * argument.
   */
  ...
  code =
      mono_arm_emit_store_regarray(code, gregs_regset, ARMREG_FP, gregs_offset);
  /* Save fregs */
  ...
  code = mono_arm_emit_store_regset(code, MONO_ARCH_LMF_REGS, ARMREG_IP0,
                                    MONO_STRUCT_OFFSET(MonoLMF, gregs));
  /* Save caller fp */
  arm_ldrx(code, ARMREG_IP1, ARMREG_FP, 0);
  arm_strx(code, ARMREG_IP1, ARMREG_IP0,
           MONO_STRUCT_OFFSET(MonoLMF, gregs) + (MONO_ARCH_LMF_REG_FP * 8));
  /* Save caller sp */
  ...
  arm_strx(code, ARMREG_IP1, ARMREG_IP0,
           MONO_STRUCT_OFFSET(MonoLMF, gregs) + (MONO_ARCH_LMF_REG_SP * 8));
  /* Save caller pc */
  ...
  if (aot) {
    code = mono_arm_emit_aotconst(
        &ji, code, buf, ARMREG_IP0, MONO_PATCH_INFO_JIT_ICALL_ADDR,
  ...
  arm_strp(code, ARMREG_IP1, ARMREG_IP0,
  ...
  arm_ldrp(code, ARMREG_IP1, ARMREG_IP0,
           MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
  /* ip0 = lmf->lmf_addr */
  ...
  if (aot) {
    ...
    code = mono_arm_emit_imm64(
        code, ARMREG_IP0,
        (guint64)mono_thread_force_interruption_checkpoint_noraise);
  }
  arm_blrx(code, ARMREG_IP0);
  ...
  code = mono_arm_emit_load_regarray(
      code, 0x1ff | (1 << ARMREG_LR) | (1 << MONO_ARCH_RGCTX_REG), ARMREG_FP,
      gregs_offset);
  /* Restore fregs */
  for (i = 0; i < num_fregs; ++i)
  ...
  if (aot) {
    ...
    code = mono_arm_emit_imm64(
        code, ARMREG_IP0, (guint64)mono_get_rethrow_preserve_exception_addr());
  }
...
gpointer mono_arch_create_specific_trampoline(gpointer arg1,
  ...
  /*
   * Return a trampoline which calls generic trampoline TRAMP_TYPE passing in
   * ARG1. Pass the argument in ip1, clobbering ip0.
   */
...
gpointer mono_arch_create_rgctx_lazy_fetch_trampoline(guint32 slot,
  ...
  if (is_mrgctx) {
    ...
    code = mono_arm_emit_ldrx(
        code, ARMREG_IP1, ARMREG_R0,
  ...
  for (i = 0; i < depth; ++i) {
    ...
    if (is_mrgctx && i == 0) {
      code = mono_arm_emit_ldrx(code, ARMREG_IP1, ARMREG_IP1,
 ...
 * mini_get_dbg_callbacks ()->single_step_from_context ()/mini_get_dbg_callbacks
 * ()->breakpoint_from_context (), then restores the (potentially changed)
 * context.
 */
guint8 *mono_arch_create_sdb_trampoline(gboolean single_step,
  ...
  gregs_regset = ~((1 << ARMREG_FP) | (1 << ARMREG_SP));
  code = mono_arm_emit_store_regarray(code, gregs_regset, ARMREG_FP,
                                      ctx_offset +
                                          G_STRUCT_OFFSET(MonoContext, regs));
  /* Save caller fp */
  arm_ldrx(code, ARMREG_IP1, ARMREG_FP, 0);
  arm_strx(code, ARMREG_IP1, ARMREG_FP,
           ctx_offset + G_STRUCT_OFFSET(MonoContext, regs) + (ARMREG_FP * 8));
  /* Save caller sp */
  arm_movx(code, ARMREG_IP1, ARMREG_FP);
  imm = frame_size;
  while (imm > 256) {
  ...
  arm_strx(code, ARMREG_IP1, ARMREG_FP,
           ctx_offset + G_STRUCT_OFFSET(MonoContext, pc));
  ...
  if (aot) {
    ...
    void (*addr)(MonoContext * ctx) =
        single_step ? mini_get_dbg_callbacks()->single_step_from_context
  ...
  arm_ldrx(code, ARMREG_IP0, ARMREG_FP,
           ctx_offset + G_STRUCT_OFFSET(MonoContext, regs) + (ARMREG_FP * 8));
  ...
  gregs_regset = ~((1 << ARMREG_FP) | (1 << ARMREG_SP));
  code = mono_arm_emit_load_regarray(code, gregs_regset, ARMREG_FP,
                                     ctx_offset +
                                         G_STRUCT_OFFSET(MonoContext, regs));

  code = mono_arm_emit_destroy_frame(code, frame_size,
...
gpointer mono_arch_get_interp_to_native_trampoline(MonoTrampInfo **info) {
  ...
  arm_ldrw(code, ARMREG_R0, ARMREG_R1,
  ...
  for (i = 0; i < PARAM_REGS + 1; i++)
    arm_ldrx(code, i, ARMREG_IP0,
             MONO_STRUCT_OFFSET(CallContext, gregs) + i * sizeof(host_mgreg_t));

  /* set all floating registers from CallContext  */
  for (i = 0; i < FP_PARAM_REGS; i++)
    arm_ldrfpx(code, i, ARMREG_IP0,
  ...
  for (i = 0; i < PARAM_REGS; i++)
    arm_strx(code, i, ARMREG_IP0,
             MONO_STRUCT_OFFSET(CallContext, gregs) + i * sizeof(host_mgreg_t));

  /* set all floating registers to CallContext  */
  for (i = 0; i < FP_PARAM_REGS; i++)
    arm_strfpx(code, i, ARMREG_IP0,
               MONO_STRUCT_OFFSET(CallContext, fregs) + i * sizeof(double));
...
gpointer mono_arch_get_native_to_interp_trampoline(MonoTrampInfo **info) {
  ...
  for (i = 0; i < PARAM_REGS + 1; i++)
    arm_strx(code, i, ARMREG_FP,
             ccontext_offset + MONO_STRUCT_OFFSET(CallContext, gregs) +
                 i * sizeof(host_mgreg_t));

  /* save all floating registers into the CallContext  */
  for (i = 0; i < FP_PARAM_REGS; i++)
    arm_strfpx(code, i, ARMREG_FP,
               ccontext_offset + MONO_STRUCT_OFFSET(CallContext, fregs) +
                   i * sizeof(double));

  /* set the stack pointer to the value at call site */
  arm_addx_imm(code, ARMREG_R0, ARMREG_FP, framesize);
  arm_strp(code, ARMREG_R0, ARMREG_FP,
           ccontext_offset + MONO_STRUCT_OFFSET(CallContext, stack));

  /* call interp_entry with the ccontext and rmethod as arguments */
  arm_addx_imm(code, ARMREG_R0, ARMREG_FP, ccontext_offset);
  arm_ldrp(code, ARMREG_R1, MONO_ARCH_RGCTX_REG,
           MONO_STRUCT_OFFSET(MonoFtnDesc, arg));
  arm_ldrp(code, ARMREG_IP0, MONO_ARCH_RGCTX_REG,
           MONO_STRUCT_OFFSET(MonoFtnDesc, addr));
  arm_blrx(code, ARMREG_IP0);

  /* load the return values from the context */
  for (i = 0; i < PARAM_REGS; i++)
    arm_ldrx(code, i, ARMREG_FP,
             ccontext_offset + MONO_STRUCT_OFFSET(CallContext, gregs) +
                 i * sizeof(host_mgreg_t));

  for (i = 0; i < FP_PARAM_REGS; i++)
    arm_ldrfpx(code, i, ARMREG_FP,
               ccontext_offset + MONO_STRUCT_OFFSET(CallContext, fregs) +
                   i * sizeof(double));
