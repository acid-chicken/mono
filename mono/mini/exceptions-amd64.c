...
// Secret password to unlock wcscat_s on mxe, must happen before string.h
// included
#ifdef __MINGW32__
...
#define W32_SEH_HANDLE_EX(_ex)                                                 \
  if (_ex##_handler)                                                           \
  _ex##_handler(er->ExceptionCode, &info, ctx)
...
static gpointer get_win32_restore_stack(void) {
  ...
  amd64_alu_reg_imm(code, X86_ADD, AMD64_ARG_REG1,
                    MONO_STRUCT_OFFSET(MonoJitTlsData, stack_restore_ctx));
  ...
  MONO_PROFILER_RAISE(jit_code_buffer,
                      (start, code - start,
                       MONO_PROFILER_CODE_BUFFER_EXCEPTION_HANDLING, NULL));
...
gpointer mono_arch_get_restore_context(MonoTrampInfo **info, gboolean aot) {
  ...
  for (i = 0; i < AMD64_NREG; ++i) {
    if (i != AMD64_RIP && i != AMD64_RSP && i != AMD64_R8 && i != AMD64_R9 &&
        i != AMD64_R10 && i != AMD64_R11)
   ...
   */
  amd64_mov_reg_membase(code, AMD64_R8, AMD64_R11,
                        gregs_offset + (AMD64_RSP * 8), 8);
  amd64_mov_reg_membase(code, AMD64_R11, AMD64_R11,
                        gregs_offset + (AMD64_RIP * 8), 8);
  ...
  if (info)
    *info = mono_tramp_info_create("restore_context", start, code - start, ji,
                                   unwind_ops);
...
gpointer mono_arch_get_call_filter(MonoTrampInfo **info, gboolean aot) {
  ...
  /* set new EBP */
  amd64_mov_reg_membase(code, AMD64_RBP, AMD64_ARG_REG1,
                        gregs_offset + (AMD64_RBP * 8), 8);
  ...
  g_assertf((code - start) <= kMaxCodeSize, "%d %d", (int)(code - start),
            kMaxCodeSize);
  ...
  if (info)
    *info = mono_tramp_info_create("call_filter", start, code - start, ji,
                                   unwind_ops);
...
/*
 * The first few arguments are dummy, to force the other arguments to be passed
 * on the stack, this avoids overwriting the argument registers in the throw
 * trampoline.
 */
void mono_amd64_throw_exception(guint64 dummy1, guint64 dummy2, guint64 dummy3,
                                guint64 dummy4, guint64 dummy5, guint64 dummy6,
                                MonoContext *mctx, MonoObject *exc,
                                gboolean rethrow, gboolean preserve_ips) {
  ERROR_DECL(error);
  MonoContext ctx;
...
void mono_amd64_throw_corlib_exception(guint64 dummy1, guint64 dummy2,
                                       guint64 dummy3, guint64 dummy4,
  ...
  mono_amd64_throw_exception(dummy1, dummy2, dummy3, dummy4, dummy5, dummy6,
                             mctx, (MonoObject *)ex, FALSE, FALSE);
}

void mono_amd64_resume_unwind(guint64 dummy1, guint64 dummy2, guint64 dummy3,
                              guint64 dummy4, guint64 dummy5, guint64 dummy6,
                              MonoContext *mctx, guint32 dummy7,
                              gint64 dummy8) {
 ...
 */
static gpointer get_throw_trampoline(MonoTrampInfo **info, gboolean rethrow,
                                     gboolean corlib, gboolean llvm_abs,
                                     gboolean resume_unwind,
                                     const char *tramp_name, gboolean aot,
...
#ifdef TARGET_WIN32
  const int dummy_stack_space =
      6 * sizeof(target_mgreg_t); /* Windows expects stack space allocated for
                                     all 6 dummy args. */
#else
  ...
  if (info)
    start = code = (guint8 *)mono_global_codeman_reserve(
        kMaxCodeSize + MONO_MAX_TRAMPOLINE_UNWINDINFO_SIZE);
  else
    start = code = (guint8 *)mono_global_codeman_reserve(kMaxCodeSize);

  /* The stack is unaligned on entry */
  stack_size = ALIGN_TO(sizeof(MonoContext) + 64 + dummy_stack_space,
  ...
  /*
   * To hide linux/windows calling convention differences, we pass all arguments
   * on the stack by passing 6 dummy values in registers.
   */
  ...
  for (i = 0; i < AMD64_NREG; ++i)
    if (i != AMD64_RSP)
      amd64_mov_membase_reg(code, AMD64_RSP,
                            regs_offset + (i * sizeof(target_mgreg_t)), i,
                            sizeof(target_mgreg_t));
  /* Save RSP */
  amd64_lea_membase(code, AMD64_RAX, AMD64_RSP,
                    stack_size + sizeof(target_mgreg_t));
  amd64_mov_membase_reg(code, AMD64_RSP,
                        regs_offset + (AMD64_RSP * sizeof(target_mgreg_t)),
                        X86_EAX, sizeof(target_mgreg_t));
  /* Save IP */
  amd64_mov_reg_membase(code, AMD64_RAX, AMD64_RSP, stack_size,
                        sizeof(target_mgreg_t));
  amd64_mov_membase_reg(code, AMD64_RSP,
                        regs_offset + (AMD64_RIP * sizeof(target_mgreg_t)),
                        AMD64_RAX, sizeof(target_mgreg_t));
  /* Set arg1 == ctx */
  amd64_lea_membase(code, AMD64_RAX, AMD64_RSP, ctx_offset);
  amd64_mov_membase_reg(code, AMD64_RSP, arg_offsets[0], AMD64_RAX,
                        sizeof(target_mgreg_t));
  /* Set arg2 == exc/ex_token_index */
  if (resume_unwind)
    amd64_mov_membase_imm(code, AMD64_RSP, arg_offsets[1], 0,
                          sizeof(target_mgreg_t));
  else
    amd64_mov_membase_reg(code, AMD64_RSP, arg_offsets[1], AMD64_ARG_REG1,
                          sizeof(target_mgreg_t));
  /* Set arg3 == rethrow/pc offset */
  if (resume_unwind) {
    amd64_mov_membase_imm(code, AMD64_RSP, arg_offsets[2], 0,
                          sizeof(target_mgreg_t));
  } else if (corlib) {
    if (llvm_abs)
      /*
       * The caller doesn't pass in a pc/pc offset, instead we simply use the
       * caller ip. Negate the pc adjustment done in
       * mono_amd64_throw_corlib_exception ().
       */
      amd64_mov_membase_imm(code, AMD64_RSP, arg_offsets[2], 1,
                            sizeof(target_mgreg_t));
    else
      amd64_mov_membase_reg(code, AMD64_RSP, arg_offsets[2], AMD64_ARG_REG2,
                            sizeof(target_mgreg_t));
  } else {
    amd64_mov_membase_imm(code, AMD64_RSP, arg_offsets[2], rethrow,
                          sizeof(target_mgreg_t));

    /* Set arg4 == preserve_ips */
    amd64_mov_membase_imm(code, AMD64_RSP, arg_offsets[3], preserve_ips,
                          sizeof(target_mgreg_t));
  }
  ...
  if (aot) {
    ...
    amd64_mov_reg_imm(
        code, AMD64_R11,
        resume_unwind ? ((gpointer)mono_amd64_resume_unwind)
                      : (corlib ? (gpointer)mono_amd64_throw_corlib_exception
                                : (gpointer)mono_amd64_throw_exception));
  }
  amd64_call_reg(code, AMD64_R11);
  ...
  g_assert_checked(mono_arch_unwindinfo_validate_size(
      unwind_ops, MONO_MAX_TRAMPOLINE_UNWINDINFO_SIZE));
  ...
  if (info)
    *info =
        mono_tramp_info_create(tramp_name, start, code - start, ji, unwind_ops);
...
gpointer mono_arch_get_throw_exception(MonoTrampInfo **info, gboolean aot) {
  return get_throw_trampoline(info, FALSE, FALSE, FALSE, FALSE,
                              "throw_exception", aot, FALSE);
}

gpointer mono_arch_get_rethrow_exception(MonoTrampInfo **info, gboolean aot) {
  return get_throw_trampoline(info, TRUE, FALSE, FALSE, FALSE,
                              "rethrow_exception", aot, FALSE);
}

gpointer mono_arch_get_rethrow_preserve_exception(MonoTrampInfo **info,
                                                  gboolean aot) {
  return get_throw_trampoline(info, TRUE, FALSE, FALSE, FALSE,
                              "rethrow_preserve_exception", aot, TRUE);
}
...
gpointer mono_arch_get_throw_corlib_exception(MonoTrampInfo **info,
                                              gboolean aot) {
  return get_throw_trampoline(info, FALSE, TRUE, FALSE, FALSE,
                              "throw_corlib_exception", aot, FALSE);
}
#endif /* !DISABLE_JIT */
 ...
 *
 * This function is used to gather information from @ctx, and store it in
 * @frame_info. It unwinds one stack frame, and stores the resulting context
 * into @new_ctx. @lmf is modified if needed. Returns TRUE on success, FALSE
 * otherwise.
 */
  ...
  if (ji != NULL) {
    ...
    */
    ...
    if (ji->has_arch_eh_info)
      epilog = (guint8 *)ji->code_start + ji->code_size -
               mono_jinfo_get_epilog_size(ji);

    for (i = 0; i < AMD64_NREG; ++i)
      regs[i] = new_ctx->gregs[i];

    gboolean success = mono_unwind_frame(
        unwind_info, unwind_info_len, (guint8 *)ji->code_start,
        (guint8 *)ji->code_start + ji->code_size, (guint8 *)ip,
        epilog ? &epilog : NULL, regs, MONO_MAX_IREGS + 1, save_locations,
        MONO_MAX_IREGS, &cfa);
    ...
    /*
     * FIXME: ji == NULL can happen when a managed-to-native wrapper is
     * interrupted in the soft debugger suspend code, since (*lmf)->rsp no
     * longer points to the return address.
     */
    // g_assert (ji);
    if (!ji)
      return FALSE;
    ...
    if (((guint64)(*lmf)->previous_lmf) & 4) {
      ...
      /*
       * The registers saved in the LMF will be restored using the normal unwind
       * info, when the wrapper frame is processed.
       */
      /* Adjust IP */
      rip--;
 ...
 */
static void handle_signal_exception(gpointer obj) {
  MonoJitTlsData *jit_tls = mono_tls_get_jit_tls();
  MonoContext ctx;
...
gboolean mono_arch_handle_exception(void *sigctx, gpointer obj) {
  ...
  /*
   * Handling the exception in the signal handler is problematic, since the
   * original signal is disabled, and we could run arbitrary code though the
   * debugger. So resume into the normal stack and do most work there if
   * possible.
   */
  MonoJitTlsData *jit_tls = mono_tls_get_jit_tls();
...
static MonoObject *restore_soft_guard_pages(void) {
  MonoJitTlsData *jit_tls = mono_tls_get_jit_tls();
 ...
 */
static void prepare_for_guard_pages(MonoContext *mctx) {
  gpointer *sp;
...
static void altstack_handle_and_restore(MonoContext *ctx, MonoObject *obj,
                                        guint32 flags) {
  MonoContext mctx;
  MonoJitInfo *ji = mini_jit_info_table_find(mono_domain_get(),
...
void mono_arch_handle_altstack_exception(void *sigctx,
   ...
   *
   * tls->orig_ex_ctx is used by the stack walker, which shouldn't be running at
   * this point.
   */
  ...
  /* at the return from the signal handler execution starts in
...
#ifndef DISABLE_JIT
GSList *mono_amd64_get_exception_trampolines(gboolean aot) {
  MonoTrampInfo *info;
  GSList *tramps = NULL;
  ...
  get_throw_trampoline(&info, FALSE, TRUE, FALSE, FALSE,
                       "llvm_throw_corlib_exception_trampoline", aot, FALSE);
  info->jit_icall_info =
      &mono_get_jit_icall_info()->mono_llvm_throw_corlib_exception_trampoline;
  tramps = g_slist_prepend(tramps, info);

  get_throw_trampoline(&info, FALSE, TRUE, TRUE, FALSE,
                       "llvm_throw_corlib_exception_abs_trampoline", aot,
                       FALSE);
  info->jit_icall_info = &mono_get_jit_icall_info()
                              ->mono_llvm_throw_corlib_exception_abs_trampoline;
  tramps = g_slist_prepend(tramps, info);

  get_throw_trampoline(&info, FALSE, TRUE, TRUE, TRUE,
                       "llvm_resume_unwind_trampoline", aot, FALSE);
  info->jit_icall_info =
...
void mono_arch_exceptions_init(void) {
  ...
  if (mono_ee_features.use_aot_trampolines) {
    ...
    mono_register_jit_icall_info(
        &mono_get_jit_icall_info()->mono_llvm_throw_corlib_exception_trampoline,
        tramp, "llvm_throw_corlib_exception_trampoline", NULL, TRUE, NULL);
    ...
    mono_register_jit_icall_info(
        &mono_get_jit_icall_info()
             ->mono_llvm_throw_corlib_exception_abs_trampoline,
        tramp, "llvm_throw_corlib_exception_abs_trampoline", NULL, TRUE, NULL);

    tramp = mono_aot_get_trampoline("llvm_resume_unwind_trampoline");
    mono_register_jit_icall_info(
        &mono_get_jit_icall_info()->mono_llvm_resume_unwind_trampoline, tramp,
        "llvm_resume_unwind_trampoline", NULL, TRUE, NULL);
    ...
    for (l = tramps; l; l = l->next) {
      ...
      mono_register_jit_icall_info(info->jit_icall_info, info->code,
                                   g_strdup(info->name), NULL, TRUE, NULL);
...
static void mono_arch_unwindinfo_create(gpointer *monoui) {
  PUNWIND_INFO newunwindinfo;
  *monoui = newunwindinfo = g_new0(UNWIND_INFO, 1);
  newunwindinfo->Version = 1;
}

void mono_arch_unwindinfo_add_push_nonvol(PUNWIND_INFO unwindinfo,
                                          MonoUnwindOp *unwind_op) {
...
void mono_arch_unwindinfo_add_set_fpreg(PUNWIND_INFO unwindinfo,
                                        MonoUnwindOp *unwind_op) {
  PUNWIND_CODE unwindcode;
  guchar codeindex;
...
void mono_arch_unwindinfo_add_alloc_stack(PUNWIND_INFO unwindinfo,
                                          MonoUnwindOp *unwind_op) {
  PUNWIND_CODE unwindcode;
  guchar codeindex;
  guchar codesneeded;
  ...
  if (codesneeded == 1) {
    ...
    if (codesneeded == 3) {
      /*the unscaled size of the allocation is recorded
        ...
        unwind code will have right execution order. List is sorted on
        CodeOffset using descending sort order.*/
...
// Dynamic function table used when registering unwind info for OS unwind
// support.
...
// When using function table callback solution an out of proc module is needed
// by debuggers in order to read unwind info from debug target.
#ifdef _MSC_VER
...
static void init_table_no_lock(void) {
  if (g_dyn_func_table_inited == FALSE) {
    ...
    // Load functions available on Win8/Win2012Server or later. If running on
    // earlier systems the below GetProceAddress will fail, this is expected
    // behavior.
    HMODULE ntdll;
    if (GetModuleHandleEx(0, L"ntdll.dll", &ntdll)) {
      g_rtl_add_growable_function_table =
          (RtlAddGrowableFunctionTablePtr)GetProcAddress(
              ntdll, "RtlAddGrowableFunctionTable");
      g_rtl_grow_function_table = (RtlGrowFunctionTablePtr)GetProcAddress(
          ntdll, "RtlGrowFunctionTable");
      g_rtl_delete_growable_function_table =
          (RtlDeleteGrowableFunctionTablePtr)GetProcAddress(
    ...
    if (g_rtl_add_growable_function_table == NULL) {
      ...
      if (GetModuleHandleEx(0, L"kernel32.dll", &kernel32dll)) {
        g_rtl_install_function_table_callback =
            (RtlInstallFunctionTableCallbackPtr)GetProcAddress(
                kernel32dll, "RtlInstallFunctionTableCallback");
        g_rtl_delete_function_table = (RtlDeleteFunctionTablePtr)GetProcAddress(
            kernel32dll, "RtlDeleteFunctionTable");
      }
    }
...
void mono_arch_unwindinfo_init_table(void) {
  if (g_dyn_func_table_inited == FALSE) {
...
static void terminate_table_no_lock(void) {
...
void mono_arch_unwindinfo_terminate_table(void) {
  if (g_dyn_func_table_inited == TRUE) {
...
static GList *fast_find_range_in_table_no_lock_ex(gsize begin_range,
                                                  gsize end_range,
  ...
  if (g_dynamic_function_table_begin != NULL) {
    DynamicFunctionTableEntry *first_entry =
        (DynamicFunctionTableEntry *)g_dynamic_function_table_begin->data;
    DynamicFunctionTableEntry *last_entry =
        (g_dynamic_function_table_end != NULL)
            ? (DynamicFunctionTableEntry *)g_dynamic_function_table_end->data
            : first_entry;

    // Sorted in descending order based on begin_range, check first item, that
    // is the entry with highest range.
    if (first_entry != NULL && first_entry->begin_range <= begin_range &&
        first_entry->end_range >= end_range) {
...
fast_find_range_in_table_no_lock(gsize begin_range, gsize end_range,
                                 gboolean *continue_search) {
  GList *found_entry = fast_find_range_in_table_no_lock_ex(
      begin_range, end_range, continue_search);
  return (found_entry != NULL) ? (DynamicFunctionTableEntry *)found_entry->data
                               : NULL;
}

static GList *find_range_in_table_no_lock_ex(const gpointer code_block,
                                             gsize block_size) {
  GList *found_entry = NULL;
  ...
  found_entry = fast_find_range_in_table_no_lock_ex(begin_range, end_range,
                                                    &continue_search);
  ...
  for (GList *node = g_dynamic_function_table_begin; node; node = node->next) {
    DynamicFunctionTableEntry *current_entry =
        (DynamicFunctionTableEntry *)node->data;
    g_assert_checked(current_entry != NULL);

    // Do we have a match?
    if (current_entry->begin_range == begin_range &&
        current_entry->end_range == end_range) {
      found_entry = node;
      break;
    }
  }
...
static GList *find_pc_in_table_no_lock_ex(const gpointer pc) {
  GList *found_entry = NULL;
  gboolean continue_search = FALSE;
  ...
  // Fast path, check table boundaries.
  found_entry = fast_find_range_in_table_no_lock_ex(begin_range, begin_range,
                                                    &continue_search);
  if (found_entry || continue_search == FALSE)
    return found_entry;
...
static DynamicFunctionTableEntry *find_pc_in_table_no_lock(const gpointer pc) {
...
static void validate_table_no_lock(void) {
  // Validation method checking that table is sorted as expected and don't
  // include overlapped regions. Method will assert on failure to explicitly
  // indicate what check failed.
  if (g_dynamic_function_table_begin != NULL) {
    g_assert_checked(g_dynamic_function_table_end != NULL);

    DynamicFunctionTableEntry *prevoious_entry = NULL;
    DynamicFunctionTableEntry *current_entry = NULL;
    for (GList *node = g_dynamic_function_table_begin; node;
         node = node->next) {
      ...
      if (prevoious_entry != NULL) {
        ...
        g_assert_checked(prevoious_entry->begin_range >
                         current_entry->begin_range);

        // Check for overlapped regions.
        g_assert_checked(prevoious_entry->begin_range >=
                         current_entry->end_range);
      }
...
// Forward declare.
static PRUNTIME_FUNCTION MONO_GET_RUNTIME_FUNCTION_CALLBACK(DWORD64 ControlPc,
                                                            IN PVOID Context);

DynamicFunctionTableEntry *
mono_arch_unwindinfo_insert_range_in_table(const gpointer code_block,
                                           gsize block_size) {
  ...
  if (new_entry == NULL) {
    ...
    if (new_entry != NULL) {
      ...
      new_entry->rt_funcs_max_count =
          (block_size / MONO_UNWIND_INFO_RT_FUNC_SIZE) + 1;
      new_entry->rt_funcs_current_count = 0;
      new_entry->rt_funcs =
          g_new0(RUNTIME_FUNCTION, new_entry->rt_funcs_max_count);

      if (new_entry->rt_funcs != NULL) {
        // Check insert on boundaries. List is sorted descending on begin_range.
        if (g_dynamic_function_table_begin == NULL) {
          g_dynamic_function_table_begin =
              g_list_append(g_dynamic_function_table_begin, new_entry);
          g_dynamic_function_table_end = g_dynamic_function_table_begin;
        } else if (((DynamicFunctionTableEntry *)(g_dynamic_function_table_begin
                                                      ->data))
                       ->begin_range < begin_range) {
          // Insert at the head.
          g_dynamic_function_table_begin =
              g_list_prepend(g_dynamic_function_table_begin, new_entry);
        } else if (((DynamicFunctionTableEntry *)(g_dynamic_function_table_end
                                                      ->data))
                       ->begin_range > begin_range) {
          // Insert at tail.
          ...
          for (GList *node = g_dynamic_function_table_begin; node;
            ...
            if (current_entry->begin_range < new_entry->begin_range) {
              g_dynamic_function_table_begin = g_list_insert_before(
                  g_dynamic_function_table_begin, node, new_entry);
              break;
            }
          }
        }
        ...
        if (g_rtl_add_growable_function_table != NULL) {
          ...
          DWORD result = g_rtl_add_growable_function_table(
              &new_entry->handle, new_entry->rt_funcs,
              new_entry->rt_funcs_current_count, new_entry->rt_funcs_max_count,
              new_entry->begin_range, new_entry->end_range);
          g_assert(!result);
          ...
          BOOLEAN result = g_rtl_install_function_table_callback(
              (DWORD64)(new_entry->handle), (DWORD64)(new_entry->begin_range),
              (DWORD)(new_entry->end_range - new_entry->begin_range),
        ...
        // Only included in checked builds. Validates the structure of table
        // after insert.
...
static void remove_range_in_table_no_lock(GList *entry) {
  if (entry != NULL) {
    if (entry == g_dynamic_function_table_end)
      g_dynamic_function_table_end = entry->prev;

    g_dynamic_function_table_begin =
        g_list_remove_link(g_dynamic_function_table_begin, entry);
    DynamicFunctionTableEntry *removed_entry =
        (DynamicFunctionTableEntry *)entry->data;
  ...
  // Only included in checked builds. Validates the structure of table after
  // remove.
  validate_table_no_lock();
}

void mono_arch_unwindinfo_remove_pc_range_in_table(const gpointer code) {
  AcquireSRWLockExclusive(&g_dynamic_function_table_lock);
  ...
  g_assert_checked(
      found_entry != NULL ||
      ((DynamicFunctionTableEntry *)found_entry->data)->begin_range ==
          (gsize)code);
...
void mono_arch_unwindinfo_remove_range_in_table(const gpointer code_block,
                                                gsize block_size) {
  AcquireSRWLockExclusive(&g_dynamic_function_table_lock);
...
PRUNTIME_FUNCTION
mono_arch_unwindinfo_find_rt_func_in_table(const gpointer code,
                                           gsize code_size) {
  ...
  if (found_entry != NULL) {
    ...
    g_assert_checked(found_entry->end_range >= begin_range &&
                     found_entry->end_range >= end_range);
    g_assert_checked(found_entry->rt_funcs != NULL);

    for (int i = 0; i < found_entry->rt_funcs_current_count; ++i) {
      PRUNTIME_FUNCTION current_rt_func =
          (PRUNTIME_FUNCTION)(&found_entry->rt_funcs[i]);

      // Is this our RT function entry?
      if (found_entry->begin_range + current_rt_func->BeginAddress <=
              begin_range &&
...
validate_rt_funcs_in_table_no_lock(DynamicFunctionTableEntry *entry) {
  // Validation method checking that runtime function table is sorted as
  // expected and don't include overlapped regions. Method will assert on
  // failure to explicitly indicate what check failed.
  ...
  for (int i = 0; i < entry->rt_funcs_current_count; ++i) {
    ...
    g_assert_checked(current_rt_func->BeginAddress <
                     current_rt_func->EndAddress);
    g_assert_checked(current_rt_func->EndAddress <=
                     current_rt_func->UnwindData);

    if (previous_rt_func != NULL) {
      // List should be sorted in ascending order based on BeginAddress.
      g_assert_checked(previous_rt_func->BeginAddress <
                       current_rt_func->BeginAddress);

      // Check for overlapped regions.
      g_assert_checked(previous_rt_func->EndAddress <=
                       current_rt_func->BeginAddress);
    }
...
PRUNTIME_FUNCTION
mono_arch_unwindinfo_insert_rt_func_in_table(const gpointer code,
                                             gsize code_size) {
  ...
  if (found_entry != NULL) {
    ...
    new_rt_func_data.UnwindData =
        aligned_unwind_data - found_entry->begin_range;

    g_assert_checked(
        new_rt_func_data.UnwindData ==
        ALIGN_TO(new_rt_func_data.EndAddress, sizeof(host_mgreg_t)));

    PRUNTIME_FUNCTION new_rt_funcs = NULL;

    // List needs to be sorted in ascending order based on BeginAddress (Windows
    // requirement if list going to be directly reused in OS func tables. Check
    // if we can append to end of existing table without realloc.
    if (entry_count == 0 ||
        (entry_count < max_entry_count) &&
            (current_rt_funcs[entry_count - 1].BeginAddress) < code_offset) {
      ...
      *new_rt_func = new_rt_func_data;
      ...
      // if to small).
      max_entry_count =
          entry_count * 2 > max_entry_count ? entry_count * 2 : max_entry_count;
      ...
      if (new_rt_funcs != NULL) {
        ...
        // Copy from old table into new table. Make sure new rt func gets
        // inserted into correct location based on sort order.
        for (; from_index < entry_count; ++from_index) {
          if (new_rt_func == NULL && current_rt_funcs[from_index].BeginAddress >
                                         new_rt_func_data.BeginAddress) {
    ...
    if (new_rt_funcs == NULL && g_rtl_grow_function_table != NULL) {
      ...
      g_rtl_grow_function_table(found_entry->handle,
                                found_entry->rt_funcs_current_count);
    } else if (new_rt_funcs != NULL &&
               g_rtl_add_growable_function_table != NULL) {
      ...
      DWORD result = g_rtl_add_growable_function_table(
          &found_entry->handle, found_entry->rt_funcs,
          found_entry->rt_funcs_current_count, found_entry->rt_funcs_max_count,
          found_entry->begin_range, found_entry->end_range);
      g_assert(!result);
    } else if (new_rt_funcs != NULL &&
               g_rtl_add_growable_function_table == NULL) {
...
static void initialize_unwind_info_internal_ex(GSList *unwind_ops,
  ...
  if (unwind_ops != NULL && unwindinfo != NULL) {
    ...
    for (GSList *l = unwind_ops; l; l = l->next) {
      ...
      case DW_CFA_offset: {
        // Pushes should go before SP/FP allocation to be compliant with Windows
        // x64 ABI.
...
static PUNWIND_INFO initialize_unwind_info_internal(GSList *unwind_ops) {
  PUNWIND_INFO unwindinfo;
...
void mono_arch_unwindinfo_free_unwind_info(PUNWIND_INFO unwind_info) {
  g_free(unwind_info);
}
...
guint mono_arch_unwindinfo_init_method_unwind_info(gpointer cfg) {
  ...
  current_cfg->arch.unwindinfo =
      initialize_unwind_info_internal(current_cfg->unwind_ops);
  return mono_arch_unwindinfo_get_size(
      ((PUNWIND_INFO)(current_cfg->arch.unwindinfo))->CountOfCodes);
}

void mono_arch_unwindinfo_install_method_unwind_info(PUNWIND_INFO *monoui,
                                                     gpointer code,
                                                     guint code_size) {
  ...
  memcpy(targetinfo, unwindinfo,
         sizeof(UNWIND_INFO) - (sizeof(UNWIND_CODE) * MONO_MAX_UNWIND_CODES));

  codecount = unwindinfo->CountOfCodes;
  if (codecount) {
    memcpy(&targetinfo->UnwindCode[0],
           &unwindinfo->UnwindCode[MONO_MAX_UNWIND_CODES - codecount],
           sizeof(UNWIND_CODE) * codecount);
  }

#ifdef ENABLE_CHECKED_BUILD_UNWINDINFO
  if (codecount) {
    // Validate the order of unwind op codes in checked builds. Offset should be
    // in descending order. In first iteration previous == current, this is
    // intended to handle UWOP_ALLOC_LARGE as first item.
    int previous = 0;
    for (int current = 0; current < codecount; current++) {
      g_assert_checked(targetinfo->UnwindCode[previous].CodeOffset >=
                       targetinfo->UnwindCode[current].CodeOffset);
      previous = current;
...
void mono_arch_unwindinfo_install_tramp_unwind_info(GSList *unwind_ops,
                                                    gpointer code,
                                                    guint code_size) {
  PUNWIND_INFO unwindinfo = initialize_unwind_info_internal(unwind_ops);
  if (unwindinfo != NULL) {
    mono_arch_unwindinfo_install_method_unwind_info(&unwindinfo, code,
                                                    code_size);
  }
}
...
MonoContinuationRestore mono_tasklets_arch_restore(void) {
  ...
  /* the signature is: restore (MonoContinuation *cont, int state, MonoLMF
   * **lmf_addr) */
   ...
   * state is moved to $rax so it's setup as the return value and we can
   * overwrite $rsi
   */
  amd64_mov_reg_reg(code, cont_reg, MONO_AMD64_ARG_REG1, 8);
  amd64_mov_reg_reg(code, AMD64_RAX, MONO_AMD64_ARG_REG2, 8);
  /* setup the copy of the stack */
  amd64_mov_reg_membase(code, AMD64_RCX, cont_reg,
                        MONO_STRUCT_OFFSET(MonoContinuation, stack_used_size),
                        sizeof(int));
  amd64_shift_reg_imm(code, X86_SHR, AMD64_RCX, 3);
  x86_cld(code);
  amd64_mov_reg_membase(code, AMD64_RSI, cont_reg,
                        MONO_STRUCT_OFFSET(MonoContinuation, saved_stack),
                        sizeof(gpointer));
  amd64_mov_reg_membase(code, AMD64_RDI, cont_reg,
                        MONO_STRUCT_OFFSET(MonoContinuation, return_sp),
                        sizeof(gpointer));
  ...
  amd64_mov_reg_membase(code, AMD64_RCX, cont_reg,
                        MONO_STRUCT_OFFSET(MonoContinuation, lmf), 8);
  amd64_mov_reg_membase(code, AMD64_RBP, AMD64_RCX,
                        MONO_STRUCT_OFFSET(MonoLMF, rbp), 8);
  amd64_mov_reg_membase(code, AMD64_RSP, AMD64_RCX,
                        MONO_STRUCT_OFFSET(MonoLMF, rsp), 8);
  ...
  amd64_jump_membase(code, cont_reg,
                     MONO_STRUCT_OFFSET(MonoContinuation, return_ip));
  g_assertf((code - start) <= kMaxCodeSize, "%d %d", (int)(code - start),
            kMaxCodeSize);
...
void mono_arch_setup_resume_sighandler_ctx(MonoContext *ctx, gpointer func) {
  /*
   * When resuming from a signal handler, the stack should be misaligned, just
   * like right after a call.
   */
...
void mono_arch_undo_ip_adjustment(MonoContext *ctx) { ctx->gregs[AMD64_RIP]++; }

void mono_arch_do_ip_adjustment(MonoContext *ctx) { ctx->gregs[AMD64_RIP]--; }
