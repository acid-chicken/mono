...
/* Max number of bblocks before we bail from using more advanced branch
 * placement code */
...
static const char *const packed_xmmregs[] = {
    "p:xmm0",  "p:xmm1",  "p:xmm2",  "p:xmm3", "p:xmm4",  "p:xmm5",
    "p:xmm6",  "p:xmm7",  "p:xmm8",  "p:xmm9", "p:xmm10", "p:xmm11",
    "p:xmm12", "p:xmm13", "p:xmm14", "p:xmm15"};

static const char *const single_xmmregs[] = {
    "s:xmm0",  "s:xmm1",  "s:xmm2",  "s:xmm3", "s:xmm4",  "s:xmm5",
    "s:xmm6",  "s:xmm7",  "s:xmm8",  "s:xmm9", "s:xmm10", "s:xmm11",
    "s:xmm12", "s:xmm13", "s:xmm14", "s:xmm15"};
...
static gboolean debug_omit_fp(void) {
#if 0
...
static gboolean amd64_is_near_call(guint8 *code) {
  /* Skip REX */
...
static gboolean amd64_use_imm32(gint64 val) {
...
void mono_x86_patch(unsigned char *code, gpointer target) {
  mono_x86_patch_inline(code, target);
}

static void amd64_patch(unsigned char *code, gpointer target) {
  // NOTE: Sometimes code has just been generated, is not running yet,
  // and has no alignment requirements. Sometimes it could be running while we
  // patch it, and there are alignment requirements.
  ...
  } else if ((code[0] == 0x8b) && rex && x86_modrm_mod(code[1]) == 0 &&
             x86_modrm_rm(code[1]) == 5) {
    /* mov 0(%rip), %dreg */
...
void mono_amd64_patch(unsigned char *code, gpointer target) {
...
static void inline add_general(guint32 *gr, guint32 *stack_size,
                               ArgInfo *ainfo) {
...
static void inline add_float(guint32 *gr, guint32 *stack_size, ArgInfo *ainfo,
                             gboolean is_double) {
  ainfo->offset = *stack_size;
...
static ArgumentClass merge_argument_class_from_type(MonoType *type,
  ...
  case MONO_TYPE_VALUETYPE: {
    ...
    for (i = 0; i < info->num_fields; ++i) {
      class2 = class1;
      class2 =
          merge_argument_class_from_type(info->fields[i].field->type, class2);
    }
    break;
  }
  default:
 ...
 */
static void collect_field_info_nested(MonoClass *klass, GArray *fields_array,
                                      int offset, gboolean pinvoke,
                                      gboolean unicode) {
  ...
  if (pinvoke) {
    ...
    for (i = 0; i < info->num_fields; ++i) {
      if (MONO_TYPE_ISSTRUCT(info->fields[i].field->type)) {
        collect_field_info_nested(
            mono_class_from_mono_type_internal(info->fields[i].field->type),
            fields_array, info->fields[i].offset, pinvoke, unicode);
      } else {
        guint32 align;
        StructFieldInfo f;
        ...
        f.size = mono_marshal_type_size(info->fields[i].field->type,
                                        info->fields[i].mspec, &align, TRUE,
                                        unicode);
        f.offset = offset + info->fields[i].offset;
        if (i == info->num_fields - 1 &&
            f.size + f.offset < info->native_size) {
          /* This can happen with .pack directives eg. 'fixed' arrays */
          if (MONO_TYPE_IS_PRIMITIVE(f.type)) {
            /* Replicate the last field to fill out the remaining place, since
             * the code in add_valuetype () needs type information */
    ...
    while ((field = mono_class_get_fields_internal(klass, &iter))) {
      ...
      if (MONO_TYPE_ISSTRUCT(field->type)) {
        collect_field_info_nested(
            mono_class_from_mono_type_internal(field->type), fields_array,
            field->offset - MONO_ABI_SIZEOF(MonoObject), pinvoke, unicode);
      } else {
        int align;
        StructFieldInfo f;
...
/* Windows x64 ABI can pass/return value types in register of size 1,2,4,8
 * bytes. */
#define MONO_WIN64_VALUE_TYPE_FITS_REG(arg_size)                               \
  (arg_size <= SIZEOF_REGISTER &&                                              \
   (arg_size == 1 || arg_size == 2 || arg_size == 4 || arg_size == 8))

static gboolean allocate_register_for_valuetype_win64(
    ArgInfo *arg_info, ArgumentClass arg_class, guint32 arg_size,
    const AMD64_Reg_No int_regs[], int int_reg_count,
    const AMD64_XMM_Reg_No float_regs[], int float_reg_count,
    guint32 *current_int_reg, guint32 *current_float_reg) {
  gboolean result = FALSE;

  assert(arg_info != NULL && int_regs != NULL && float_regs != NULL &&
         current_int_reg != NULL && current_float_reg != NULL);
  assert(arg_info->storage == ArgValuetypeInReg ||
  ...
  if (arg_class == ARG_CLASS_INTEGER && *current_int_reg < int_reg_count) {
    ...
    (*current_int_reg)++;
    result = TRUE;
  } else if (arg_class == ARG_CLASS_SSE &&
             *current_float_reg < float_reg_count) {
    /* Pass parameter in float register. */
    arg_info->pair_storage[0] =
        (arg_size <= sizeof(gfloat)) ? ArgInFloatSSEReg : ArgInDoubleSSEReg;
...
static gboolean allocate_parameter_register_for_valuetype_win64(
    ArgInfo *arg_info, ArgumentClass arg_class, guint32 arg_size,
    guint32 *current_int_reg, guint32 *current_float_reg) {
  return allocate_register_for_valuetype_win64(
      arg_info, arg_class, arg_size, param_regs, PARAM_REGS, float_param_regs,
      FLOAT_PARAM_REGS, current_int_reg, current_float_reg);
}

static gboolean allocate_return_register_for_valuetype_win64(
    ArgInfo *arg_info, ArgumentClass arg_class, guint32 arg_size,
    guint32 *current_int_reg, guint32 *current_float_reg) {
  return allocate_register_for_valuetype_win64(
      arg_info, arg_class, arg_size, return_regs, RETURN_REGS,
      float_return_regs, FLOAT_RETURN_REGS, current_int_reg, current_float_reg);
}

static void allocate_storage_for_valuetype_win64(
    ArgInfo *arg_info, MonoType *type, gboolean is_return,
    ArgumentClass arg_class, guint32 arg_size, guint32 *current_int_reg,
    guint32 *current_float_reg, guint32 *stack_size) {
  /* Windows x64 value type ABI.
   *
   * Parameters: https://msdn.microsoft.com/en-us/library/zthk2dkh.aspx
   *
   * Integer/Float types smaller than or equals to 8 bytes or porperly sized
   * struct/union (1,2,4,8) Try pass in register using
   * ArgValuetypeInReg/(ArgInIReg|ArgInFloatSSEReg|ArgInDoubleSSEReg) as storage
   * and size of parameter(1,2,4,8), if no more registers, pass on stack using
   * ArgOnStack as storage and size of parameter(1,2,4,8). Integer/Float types
   * bigger than 8 bytes or struct/unions larger than 8 bytes or (3,5,6,7). Try
   * to pass pointer in register using ArgValuetypeAddrInIReg, if no more
   * registers, pass pointer on stack using ArgValuetypeAddrOnStack as storage
   * and parameter size of register (8 bytes).
   *
   * Return values:  https://msdn.microsoft.com/en-us/library/7572ztz4.aspx.
   *
   * Integers/Float types smaller than or equal to 8 bytes
   *    Return in corresponding register RAX/XMM0 using
   * ArgValuetypeInReg/(ArgInIReg|ArgInFloatSSEReg|ArgInDoubleSSEReg) as storage
   * and size of parameter(1,2,4,8). Properly sized struct/unions (1,2,4,8)
   *    Return in register RAX using ArgValuetypeInReg as storage and size of
   * parameter(1,2,4,8). Types bigger than 8 bytes or struct/unions larger than
   * 8 bytes or (3,5,6,7). Return pointer to allocated stack space (allocated by
   * caller) using ArgValuetypeAddrInIReg as storage and parameter size.
   */

  assert(arg_info != NULL && type != NULL && current_int_reg != NULL &&
         current_float_reg != NULL && stack_size != NULL);
  ...
  if (!is_return) {
    ...
    if (arg_class != ARG_CLASS_MEMORY &&
      ...
      /* First, try to use registers for parameter. If type is struct it can
       * only be passed by value in integer register. */
      arg_info->storage = ArgValuetypeInReg;
      if (!allocate_parameter_register_for_valuetype_win64(
              arg_info,
              !MONO_TYPE_ISSTRUCT(type) ? arg_class : ARG_CLASS_INTEGER,
              arg_size, current_int_reg, current_float_reg)) {
        /* No more registers, fallback passing parameter on stack as value. */
        assert(arg_info->pair_storage[0] == ArgNone &&
               arg_info->pair_storage[1] == ArgNone &&
               arg_info->pair_size[0] == 0 && arg_info->pair_size[1] == 0 &&
               arg_info->nregs == 0);
      ...
      /* Fallback to stack, try to pass address to parameter in register. Always
       * use integer register to represent stack address. */
      arg_info->storage = ArgValuetypeAddrInIReg;
      if (!allocate_parameter_register_for_valuetype_win64(
        ...
        /* Passing an address to value on stack, so use size of register as
         * argument size. */
    ...
    if (arg_class != ARG_CLASS_MEMORY &&
      ...
      /* Return value fits into return registers. If type is struct it can only
       * be returned by value in integer register. */
      arg_info->storage = ArgValuetypeInReg;
      allocate_return_register_for_valuetype_win64(
          arg_info, !MONO_TYPE_ISSTRUCT(type) ? arg_class : ARG_CLASS_INTEGER,
          arg_size, current_int_reg, current_float_reg);

      /* Only RAX/XMM0 should be used to return valuetype. */
      assert((arg_info->pair_regs[0] == AMD64_RAX &&
              arg_info->pair_regs[1] == ArgNone) ||
             (arg_info->pair_regs[0] == AMD64_XMM0 &&
              arg_info->pair_regs[1] == ArgNone));
    } else {
      /* Return value doesn't fit into return register, return address to
       * allocated stack space (allocated by caller and passed as input). */
      arg_info->storage = ArgValuetypeAddrInIReg;
      allocate_return_register_for_valuetype_win64(arg_info, ARG_CLASS_INTEGER,
                                                   arg_size, current_int_reg,
                                                   current_float_reg);
...
static void get_valuetype_size_win64(MonoClass *klass, gboolean pinvoke,
                                     ArgInfo *arg_info, MonoType *type,
                                     ArgumentClass *arg_class,
                                     guint32 *arg_size) {
  *arg_size = 0;
  *arg_class = ARG_CLASS_NO_CLASS;

  assert(klass != NULL && arg_info != NULL && type != NULL &&
         arg_class != NULL && arg_size != NULL);
  ...
  /* Windows ABI only handle value types on stack or passed in integer register
   * (if it fits register size). */
  *arg_class = MONO_WIN64_VALUE_TYPE_FITS_REG(*arg_size) ? ARG_CLASS_INTEGER
                                                         : ARG_CLASS_MEMORY;

  if (*arg_class == ARG_CLASS_MEMORY) {
    /* Value type has a size that doesn't seem to fit register according to ABI.
     * Try to used full stack size of type. */
    *arg_size =
        mini_type_stack_size_full(m_class_get_byval_arg(klass), NULL, pinvoke);
  }

  /*
   * Standard C and C++ doesn't allow empty structs, empty structs will always
   * have a size of 1 byte. GCC have an extension to allow empty structs,
   ...
   * compatible with GCC build C libraries and the other way around. On
   * platforms where empty structs has size of 1 byte it must be represented in
   * call and cannot be dropped.
   */
...
static void add_valuetype_win64(MonoMethodSignature *signature,
                                ArgInfo *arg_info, MonoType *type,
                                gboolean is_return, guint32 *current_int_reg,
                                guint32 *current_float_reg,
                                guint32 *stack_size) {
  guint32 arg_size = SIZEOF_REGISTER;
  MonoClass *klass = NULL;
  ArgumentClass arg_class;

  assert(signature != NULL && arg_info != NULL && type != NULL &&
         current_int_reg != NULL && current_float_reg != NULL &&
         stack_size != NULL);

  klass = mono_class_from_mono_type_internal(type);
  get_valuetype_size_win64(klass, signature->pinvoke, arg_info, type,
                           &arg_class, &arg_size);

  /* Only drop value type if its not an empty struct as input that must be
   * represented in call */
  if ((arg_size == 0 && !arg_info->pass_empty_struct) ||
      (arg_info->pass_empty_struct && is_return)) {
    arg_info->storage = ArgValuetypeInReg;
    arg_info->pair_storage[0] = arg_info->pair_storage[1] = ArgNone;
  } else {
    /* Alocate storage for value type. */
    allocate_storage_for_valuetype_win64(arg_info, type, is_return, arg_class,
                                         arg_size, current_int_reg,
                                         current_float_reg, stack_size);
  }
}

#endif /* TARGET_WIN32 */

static void add_valuetype(MonoMethodSignature *sig, ArgInfo *ainfo,
                          MonoType *type, gboolean is_return, guint32 *gr,
                          guint32 *fr, guint32 *stack_size) {
#ifdef TARGET_WIN32
  ...
  klass = mono_class_from_mono_type_internal(type);
  size = mini_type_stack_size_full(m_class_get_byval_arg(klass), NULL,
                                   sig->pinvoke);

  if (!sig->pinvoke &&
      ((is_return && (size == 8)) || (!is_return && (size <= 16)))) {
  ...
  collect_field_info_nested(klass, fields_array, 0, sig->pinvoke,
  ...
  /* Allocate registers */
  {
    ...
    while (quadsize[0] != 1 && quadsize[0] != 2 && quadsize[0] != 4 &&
           quadsize[0] != 8)
      quadsize[0]++;
    while (quadsize[1] != 0 && quadsize[1] != 1 && quadsize[1] != 2 &&
           quadsize[1] != 4 && quadsize[1] != 8)
      quadsize[1]++;
 ...
 * For AMD64 System V, see the "System V ABI, x86-64 Architecture Processor
 * Supplement Draft Version 0.23" document for more information. For AMD64
 * Windows, see "Overview of x64 Calling Conventions",
 * https://msdn.microsoft.com/en-us/library/ms235286.aspx
 */
static CallInfo *get_call_info(MonoMemPool *mp, MonoMethodSignature *sig) {
  ...
  if (mp)
    cinfo = (CallInfo *)mono_mempool_alloc0(mp, sizeof(CallInfo) +
                                                    (sizeof(ArgInfo) * n));
  else
  ...
  case MONO_TYPE_TYPEDBYREF: {
    ...
    add_valuetype(sig, &cinfo->ret, ret_type, TRUE, &tmp_gr, &tmp_fr,
                  &tmp_stacksize);
  ...
  pstart = 0;
  /*
   * To simplify get_this_arg_reg () and LLVM integration, emit the vret arg
   * after the first argument, allowing 'this' to be always passed in the first
   * arg reg. Also do this if the first argument is a reference type, since
   * virtual calls are sometimes made using calli without sig->hasthis set, like
   * in the delegate invoke wrappers.
   */
  ArgStorage ret_storage = cinfo->ret.storage;
  if ((ret_storage == ArgValuetypeAddrInIReg ||
...
#ifdef TARGET_WIN32
    /* The float param registers and other param registers must be the same
     * index on Windows x64.*/
    if (gr > fr)
      fr = gr;
    else if (fr > gr)
      gr = fr;
...
static int arg_need_temp(ArgInfo *ainfo) {
...
static gpointer arg_get_storage(CallContext *ccontext, ArgInfo *ainfo) {
...
static void arg_get_val(CallContext *ccontext, ArgInfo *ainfo, gpointer dest) {
...
static void arg_set_val(CallContext *ccontext, ArgInfo *ainfo, gpointer src) {
  g_assert(arg_need_temp(ainfo));
...
void mono_arch_set_native_call_context_args(CallContext *ccontext,
                                            gpointer frame,
  ...
  ccontext->stack_size =
      ALIGN_TO(cinfo->stack_usage, MONO_ARCH_FRAME_ALIGNMENT);
  ...
  if (sig->ret->type != MONO_TYPE_VOID) {
    ...
    if (ainfo->storage == ArgValuetypeAddrInIReg) {
      storage = interp_cb->frame_arg_to_storage((MonoInterpFrameHandle)frame,
                                                sig, -1);
...
void mono_arch_set_native_call_context_ret(CallContext *ccontext,
                                           gpointer frame,
                                           MonoMethodSignature *sig) {
  ...
  if (cinfo->ret.storage != ArgValuetypeAddrInIReg) {
    ...
    interp_cb->frame_arg_to_data((MonoInterpFrameHandle)frame, sig, -1,
                                 storage);
    if (temp_size)
      arg_set_val(ccontext, ainfo, storage);
  }
#ifdef TARGET_WIN32
  // Windows x64 ABI ainfo implementation includes info on how to return value
  // type address. back to caller.
  else {
    storage = arg_get_storage(ccontext, ainfo);
...
void mono_arch_get_native_call_context_args(CallContext *ccontext,
                                            gpointer frame,
                                            MonoMethodSignature *sig) {
  const MonoEECallbacks *interp_cb = mini_get_interp_callbacks();
  CallInfo *cinfo = get_call_info(NULL, sig);
  ...
  if (sig->ret->type != MONO_TYPE_VOID) {
    ...
    if (ainfo->storage == ArgValuetypeAddrInIReg) {
      ...
      interp_cb->frame_arg_set_storage((MonoInterpFrameHandle)frame, sig, -1,
                                       storage);
    }
  }
  ...
  for (int i = 0; i < sig->param_count + sig->hasthis; i++) {
    ...
    if (ainfo->storage == ArgValuetypeAddrInIReg ||
        ainfo->storage == ArgValuetypeAddrOnStack) {
      storage = arg_get_storage(ccontext, ainfo);
      interp_cb->data_to_frame_arg((MonoInterpFrameHandle)frame, sig, i,
                                   *(gpointer *)storage);
      continue;
    }
...
void mono_arch_get_native_call_context_ret(CallContext *ccontext,
                                           gpointer frame,
                                           MonoMethodSignature *sig) {
  const MonoEECallbacks *interp_cb;
  CallInfo *cinfo;
  ...
  if (cinfo->ret.storage != ArgValuetypeAddrInIReg) {
    ...
    interp_cb->data_to_frame_arg((MonoInterpFrameHandle)frame, sig, -1,
                                 storage);
  }
 ...
 */
int mono_arch_get_argument_info(MonoMethodSignature *csig, int param_count,
...
#ifndef DISABLE_JIT
gboolean mono_arch_tailcall_supported(MonoCompile *cfg,
                                      MonoMethodSignature *caller_sig,
  ...
  gboolean res = IS_SUPPORTED_TAILCALL(callee_info->stack_usage <=
                                       caller_info->stack_usage) &&
                 IS_SUPPORTED_TAILCALL(callee_info->ret.storage ==
                                       caller_info->ret.storage);
 ...
 *
 *   Populate cfg->args, cfg->ret and cfg->vret_addr with information about the
...
void mono_arch_fill_argument_info(MonoCompile *cfg) {
   ...
   * where the arguments are at the beginning of the method, not where they can
   * be accessed during the execution of the method. The later makes no sense
   * for the global register allocator, since a variable can be in more than one
   * location.
   */
...
void mono_arch_allocate_vars(MonoCompile *cfg) {
  ...
  if (cfg->method->save_lmf) {
    /* Save all callee-saved registers normally (except RBP, if not already
     * used), and restore them when unwinding through an LMF */
  ...
  if (sig_ret->type != MONO_TYPE_VOID) {
    ...
    case ArgGsharedvtVariableInReg:
      ...
      break;
    case ArgValuetypeInReg:
      /* Allocate a local to hold the result, the epilog will copy it to the
       * correct place */
      cfg->ret->opcode = OP_REGOFFSET;
  ...
  /* Allocate locals */
  offsets = mono_allocate_stack_slots(cfg, cfg->arch.omit_fp ? FALSE : TRUE,
                                      &locals_stack_size, &locals_stack_align);
  ...
  for (i = 0; i < sig->param_count + sig->hasthis; ++i) {
    ...
    if (ins->opcode != OP_REGVAR) {
      ...
      case ArgValuetypeAddrOnStack: {
        MonoInst *indir;
        g_assert(!cfg->arch.omit_fp);
        g_assert(ainfo->storage == ArgValuetypeAddrInIReg ||
                 (ainfo->storage == ArgValuetypeAddrOnStack &&
                  ainfo->pair_storage[0] == ArgNone));
      ...
      if (!inreg && (ainfo->storage != ArgOnStack) &&
        ...
        if (cfg->arch.omit_fp) {
          ...
          offset += (ainfo->storage == ArgValuetypeInReg)
                        ? ainfo->nregs * sizeof(target_mgreg_t)
                        : sizeof(target_mgreg_t);
          // Arguments are yet supported by the stack map creation code
          // cfg->locals_max_stack_offset = MAX (cfg->locals_max_stack_offset,
          // offset);
        } else {
          offset += (ainfo->storage == ArgValuetypeInReg)
                        ? ainfo->nregs * sizeof(target_mgreg_t)
                        : sizeof(target_mgreg_t);
          ins->inst_offset = -offset;
          // cfg->locals_min_stack_offset = MIN (cfg->locals_min_stack_offset,
          // offset);
        }
      }
    }
...
static void add_outarg_reg(MonoCompile *cfg, MonoCallInst *call,
...
static int arg_storage_to_load_membase(ArgStorage storage) {
  switch (storage) {
  case ArgInIReg:
...
static void emit_sig_cookie(MonoCompile *cfg, MonoCallInst *call,
                            CallInfo *cinfo) {
   ...
   */
  tmp_sig = mono_metadata_signature_dup_full(
      m_class_get_image(cfg->method->klass), call->signature);
  ...
  MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, AMD64_RSP,
                              cinfo->sig_cookie.offset, sig_reg);
}

#ifdef ENABLE_LLVM
static LLVMArgStorage arg_storage_to_llvm_arg_storage(MonoCompile *cfg,
                                                      ArgStorage storage) {
  switch (storage) {
  case ArgInIReg:
...
LLVMCallInfo *mono_arch_get_llvm_call_info(MonoCompile *cfg,
                                           MonoMethodSignature *sig) {
  int i, n;
  CallInfo *cinfo;
  ArgInfo *ainfo;
  ...
  linfo = mono_mempool_alloc0(cfg->mempool,
                              sizeof(LLVMCallInfo) + (sizeof(LLVMArgInfo) * n));
  ...
  case ArgValuetypeInReg: {
    ...
    if (sig->pinvoke && (ainfo->pair_storage[0] == ArgInFloatSSEReg ||
                         ainfo->pair_storage[0] == ArgInDoubleSSEReg ||
                         ainfo->pair_storage[1] == ArgInFloatSSEReg ||
                         ainfo->pair_storage[1] == ArgInDoubleSSEReg)) {
    ...
    for (j = 0; j < 2; ++j)
      linfo->ret.pair_storage[j] =
          arg_storage_to_llvm_arg_storage(cfg, ainfo->pair_storage[j]);
    break;
  }
  case ArgValuetypeAddrInIReg:
  case ArgGsharedvtVariableInReg:
...
void mono_arch_emit_call(MonoCompile *cfg, MonoCallInst *call) {
    ...
    if (ainfo->storage == ArgOnStack && !MONO_TYPE_ISSTRUCT(t) &&
      ...
      if (!t->byref) {
        if (t->type == MONO_TYPE_R4)
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER4_MEMBASE_REG, AMD64_RSP,
                                      ainfo->offset, in->dreg);
        else if (t->type == MONO_TYPE_R8)
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, AMD64_RSP,
                                      ainfo->offset, in->dreg);
        else
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, AMD64_RSP,
                                      ainfo->offset, in->dreg);
  ...
  /*
   * Emit all parameters passed in registers in non-reverse order for better
   * readability and to help the optimization in emit_prolog ().
   */
  for (i = 0; i < n; ++i) {
    ainfo = cinfo->args + i;
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    case ArgGSharedVtOnStack: {
      ...
      if (sig->pinvoke)
         ...
         */
        size = mono_class_value_size(mono_class_from_mono_type_internal(t),
                                     &align);
      }
      ...
      if (size >= 10000) {
        ...
        mono_cfg_set_exception_invalid_program(
            cfg, g_strdup_printf("Passing an argument of size '%d'.", size));
        /* Continue normally */
      }
  ...
  switch (cinfo->ret.storage) {
  case ArgValuetypeInReg:
    if (cinfo->ret.pair_storage[0] == ArgInIReg &&
        cinfo->ret.pair_storage[1] == ArgNone) {
      /*
      ...
      if (!cfg->arch.vret_addr_loc) {
        cfg->arch.vret_addr_loc =
            mono_compile_create_var(cfg, mono_get_int_type(), OP_LOCAL);
        /* Prevent it from being register allocated or optimized away */
        cfg->arch.vret_addr_loc->flags |= MONO_INST_VOLATILE;
      }

      MONO_EMIT_NEW_UNALU(cfg, OP_MOVE, cfg->arch.vret_addr_loc->dreg,
                          call->vret_var->dreg);
    }
    break;
  case ArgValuetypeAddrInIReg:
  case ArgGsharedvtVariableInReg: {
    ...
    mono_call_inst_add_outarg_reg(cfg, call, vtarg->dreg, cinfo->ret.reg,
                                  FALSE);
    break;
  }
  default:
...
void mono_arch_emit_outarg_vt(MonoCompile *cfg, MonoInst *ins, MonoInst *src) {
  ...
  case ArgValuetypeInReg: {
    ...
    for (part = 0; part < 2; ++part) {
      ...
      if (ainfo->pass_empty_struct) {
        // Pass empty struct value as 0 on platforms representing empty structs
        // as 1 byte.
        NEW_ICONST(cfg, load, 0);
      } else {
        MONO_INST_NEW(cfg, load,
                      arg_storage_to_load_membase(ainfo->pair_storage[part]));
      ...
      add_outarg_reg(cfg, call, ainfo->pair_storage[part],
                     ainfo->pair_regs[part], load);
    }
    break;
  }
  case ArgValuetypeAddrInIReg:
  case ArgValuetypeAddrOnStack: {
    ...
    vtaddr = mono_compile_create_var(cfg, m_class_get_byval_arg(ins->klass),
                                     OP_LOCAL);
    ...
    load->dreg = mono_alloc_ireg(cfg);
    MONO_ADD_INS(cfg->cbb, load);
    mini_emit_memcpy(cfg, load->dreg, 0, src->dreg, 0, size,
    ...
    if (ainfo->pair_storage[0] == ArgInIReg) {
      ...
      arg->inst_imm = 0;
      MONO_ADD_INS(cfg->cbb, arg);
      mono_call_inst_add_outarg_reg(cfg, call, arg->dreg, ainfo->pair_regs[0],
                                    FALSE);
    } else {
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, AMD64_RSP,
                                  ainfo->offset, load->dreg);
    }
    break;
  }
  case ArgGSharedVtInReg:
  ...
  default:
    if (size == 8) {
      ...
      mini_emit_memcpy(cfg, AMD64_RSP, ainfo->offset, src->dreg, 0, size,
                       TARGET_SIZEOF_VOID_P);
    } else {
      // FIXME: Code growth
      mini_emit_memcpy(cfg, AMD64_RSP, ainfo->offset, src->dreg, 0, size,
    ...
    if (cfg->compute_gc_maps) {
      MonoInst *def;
      EMIT_NEW_GC_PARAM_SLOT_LIVENESS_DEF(cfg, def, ainfo->offset,
                                          m_class_get_byval_arg(ins->klass));
    }
  }
}

void mono_arch_emit_setret(MonoCompile *cfg, MonoMethod *method,
                           MonoInst *val) {
  MonoType *ret =
      mini_get_underlying_type(mono_method_signature_internal(method)->ret);

  if (ret->type == MONO_TYPE_R4) {
    if (COMPILE_LLVM(cfg))
      MONO_EMIT_NEW_UNALU(cfg, OP_FMOVE, cfg->ret->dreg, val->dreg);
    else
      MONO_EMIT_NEW_UNALU(cfg, OP_AMD64_SET_XMMREG_R4, cfg->ret->dreg,
                          val->dreg);
    return;
...
#define EMIT_COND_BRANCH(ins, cond, sign)                                      \
  if (ins->inst_true_bb->native_offset) {                                      \
    x86_branch(code, cond,                                                     \
               cfg->native_code + ins->inst_true_bb->native_offset, sign);     \
  } else {                                                                     \
    mono_add_patch_info(cfg, code - cfg->native_code, MONO_PATCH_INFO_BB,      \
                        ins->inst_true_bb);                                    \
...
static gboolean dyn_call_supported(MonoMethodSignature *sig, CallInfo *cinfo) {
  int i;
...
MonoDynCallInfo *mono_arch_dyn_call_prepare(MonoMethodSignature *sig) {
  ...
  for (i = 0; i < cinfo->nargs; ++i) {
    ...
    case ArgOnStack:
    case ArgValuetypeAddrOnStack:
      info->nstack_args = MAX(info->nstack_args,
                              (ainfo->offset / sizeof(target_mgreg_t)) +
                                  (ainfo->arg_size / sizeof(target_mgreg_t)));
      break;
    default:
      break;
    ...
    case MONO_TYPE_GENERICINST:
      if (t->type == MONO_TYPE_GENERICINST &&
        ...
        if (!(ainfo->storage == ArgValuetypeInReg ||
              ainfo->storage == ArgOnStack)) {
          /* Nullables need a temporary buffer, its stored at the end of
           * DynCallArgs.regs after the stack args */
 ...
 *
 *   Convert the arguments ARGS to a format which can be passed to OP_DYN_CALL,
 * and store the result into BUF. ARGS should be an array of pointers pointing
 * to the arguments. RET should point to a memory buffer large enought to hold
 * the result of the call. This function should be as fast as possible, any work
 * which does not depend on the actual values of the arguments should be done in
 * mono_arch_dyn_call_prepare ().
 * start_dyn_call + OP_DYN_CALL + finish_dyn_call is equivalent to ffi_call in
 * libffi.
 */
void mono_arch_start_dyn_call(MonoDynCallInfo *info, gpointer **args,
                              guint8 *ret, guint8 *buf) {
  ...
  if (dinfo->cinfo->ret.storage == ArgValuetypeAddrInIReg ||
      dinfo->cinfo->ret.storage == ArgGsharedvtVariableInReg)
  ...
  for (; pindex < sig->param_count; pindex++) {
    ...
    } else if (ainfo->storage == ArgInFloatSSEReg ||
               ainfo->storage == ArgInDoubleSSEReg) {
    ...
    case MONO_TYPE_VALUETYPE: {
      ...
      case ArgValuetypeAddrInIReg:
      case ArgValuetypeAddrOnStack:
        // In DYNCALL use case value types are already copied when included in
        // parameter array. Currently no need to make an extra temporary value
        // type on stack for this use case.
...
#define EMIT_COND_SYSTEM_EXCEPTION(cond, signed, exc_name)                     \
  do {                                                                         \
    ...
    if (tins == NULL) {                                                        \
      mono_add_patch_info(cfg, code - cfg->native_code, MONO_PATCH_INFO_EXC,   \
                          exc_name);                                           \
...
#define EMIT_SSE2_FPFUNC(code, op, dreg, sreg1)                                \
  do {                                                                         \
...
#ifndef DISABLE_JIT
static guint8 *emit_call(MonoCompile *cfg, MonoCallInst *call, guint8 *code,
                         MonoJitICallId jit_icall_id) {
  gboolean no_patch = FALSE;
  MonoJumpInfoTarget patch;

  // FIXME? This is similar to mono_call_to_patch, except it favors
  // MONO_PATCH_INFO_ABS over call->jit_icall_id.
   ...
   */
  {
    ...
    if (patch.type != MONO_PATCH_INFO_ABS) {
      ...
      if (cfg->abs_patches)
        jinfo =
            (MonoJumpInfo *)g_hash_table_lookup(cfg->abs_patches, call->fptr);

      if (jinfo) {
        if (jinfo->type == MONO_PATCH_INFO_JIT_ICALL_ADDR) {
          MonoJitICallInfo *mi =
              mono_find_jit_icall_info(jinfo->data.jit_icall_id);
        ...
        if (jit_icall_id) {
          ...
          // Change patch from MONO_PATCH_INFO_ABS to
    ...
    if (near_call) {
      ...
      mono_add_patch_info(cfg, code - cfg->native_code, patch.type,
                          patch.target);
...
static int store_membase_imm_to_store_membase_reg(int opcode) {
  switch (opcode) {
...
#define INST_IGNORES_CFLAGS(opcode)                                            \
  (!(((opcode) == OP_ADC) || ((opcode) == OP_ADC_IMM) ||                       \
     ((opcode) == OP_IADC) || ((opcode) == OP_IADC_IMM) ||                     \
     ((opcode) == OP_SBB) || ((opcode) == OP_SBB_IMM) ||                       \
     ((opcode) == OP_ISBB) || ((opcode) == OP_ISBB_IMM)))
  ...
  MONO_BB_FOR_EACH_INS_SAFE(bb, n, ins) {
    ...
    case OP_LADD_IMM:
      if ((ins->sreg1 < MONO_MAX_IREGS) && (ins->dreg >= MONO_MAX_IREGS) &&
         ...
         * sreg1==dreg restriction. inst_imm > 0 is needed since LEA
         * sign-extends its operand to 64 bit.
         */
        ins->opcode = ins->opcode == OP_IADD_IMM ? OP_X86_LEA_MEMBASE
    ...
    case OP_IXOR:
      if ((ins->sreg1 == ins->sreg2) && (ins->sreg1 == ins->dreg)) {
        ...
        for (ins2 = ins->next; ins2; ins2 = ins2->next) {
          if (((ins2->opcode == OP_STORE_MEMBASE_IMM) ||
               (ins2->opcode == OP_STOREI4_MEMBASE_IMM) ||
               (ins2->opcode == OP_STOREI8_MEMBASE_IMM) ||
               (ins2->opcode == OP_STORE_MEMBASE_IMM)) &&
              (ins2->inst_imm == 0)) {
            ins2->opcode = store_membase_imm_to_store_membase_reg(ins2->opcode);
            ins2->sreg1 = ins->dreg;
          } else if ((ins2->opcode == OP_STOREI1_MEMBASE_IMM) ||
                     (ins2->opcode == OP_STOREI2_MEMBASE_IMM) ||
                     (ins2->opcode == OP_STOREI8_MEMBASE_REG) ||
                     (ins2->opcode == OP_STORE_MEMBASE_REG)) {
            /* Continue */
          } else if (((ins2->opcode == OP_ICONST) ||
                      (ins2->opcode == OP_I8CONST)) &&
                     (ins2->dreg == ins->dreg) && (ins2->inst_c0 == 0)) {
            NULLIFY_INS(ins2);
            /* Continue */
...
void mono_arch_peephole_pass_2(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS_SAFE(bb, n, ins) {
    ...
    case OP_I8CONST: {
      ...
      if (ins->inst_c0 == 0 &&
          (!next || (next && INST_IGNORES_CFLAGS(next->opcode)))) {
        ins->opcode = OP_LXOR;
        ins->sreg1 = ins->dreg;
        ins->sreg2 = ins->dreg;
        /* Fall through */
    ...
    case OP_LXOR:
      /*
       * Use IXOR to avoid a rex prefix if possible. The cpu will sign extend
       * the 0 result into 64 bits.
       */
      if ((ins->sreg1 == ins->sreg2) && (ins->sreg1 == ins->dreg)) {
        ins->opcode = OP_IXOR;
      }
        ...
        for (ins2 = ins->next; ins2; ins2 = ins2->next) {
          if (((ins2->opcode == OP_STORE_MEMBASE_IMM) ||
            ...
            ins2->sreg1 = ins->dreg;
                     ...
                     (ins2->opcode == OP_STOREI8_MEMBASE_REG) ||
                     (ins2->opcode == OP_STORE_MEMBASE_REG) ||
                     (ins2->opcode == OP_LIVERANGE_START) ||
                     (ins2->opcode == OP_GC_LIVENESS_DEF) ||
                     (ins2->opcode == OP_GC_LIVENESS_USE)) {
            /* Continue */
          } else if (((ins2->opcode == OP_ICONST) ||
                      (ins2->opcode == OP_I8CONST)) &&
                     (ins2->dreg == ins->dreg) && (ins2->inst_c0 == 0)) {
...
#define NEW_INS(cfg, ins, dest, op)                                            \
  do {                                                                         \
...
#define NEW_SIMD_INS(cfg, ins, dest, op, d, s1, s2)                            \
  do {                                                                         \
    MONO_INST_NEW((cfg), (dest), (op));                                        \
    (dest)->cil_code = (ins)->cil_code;                                        \
    (dest)->dreg = d;                                                          \
...
static int simd_type_to_comp_op(int t) {
  switch (t) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return OP_PCMPEQB;
...
static int simd_type_to_sub_op(int t) {
  switch (t) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return OP_PSUBB;
...
static int simd_type_to_shl_op(int t) {
  switch (t) {
  case MONO_TYPE_I2:
  case MONO_TYPE_U2:
    return OP_PSHLW;
...
static int simd_type_to_gt_op(int t) {
  switch (t) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return OP_PCMPGTB;
...
static int simd_type_to_max_un_op(int t) {
  switch (t) {
  case MONO_TYPE_U1:
    return OP_PMAXB_UN;
  case MONO_TYPE_U2:
...
static int simd_type_to_add_op(int t) {
  switch (t) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return OP_PADDB;
...
static int simd_type_to_min_op(int t) {
  switch (t) {
  case MONO_TYPE_I1:
...
static int simd_type_to_max_op(int t) {
  switch (t) {
  case MONO_TYPE_I1:
...
static void emit_simd_comp_op(MonoCompile *cfg, MonoBasicBlock *bb,
                              MonoInst *ins, int type, int dreg, int sreg1,
                              int sreg2) {
  MonoInst *temp;

  if (!mono_hwcap_x86_has_sse42 &&
      (ins->inst_c1 == MONO_TYPE_I8 || ins->inst_c1 == MONO_TYPE_U8)) {
    ...
    NEW_SIMD_INS(cfg, ins, temp, simd_type_to_comp_op(type), dreg, sreg1,
                 sreg2);
  }
}

static void emit_simd_gt_op(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst *ins,
                            int type, int dreg, int sreg1, int sreg2);

static void emit_simd_gt_un_op(MonoCompile *cfg, MonoBasicBlock *bb,
                               MonoInst *ins, int type, int dreg, int sreg1,
                               int sreg2) {
  MonoInst *temp;
  ...
  USE_MAX : {
    ...
    NEW_SIMD_INS(cfg, ins, temp, simd_type_to_max_un_op(type), temp_reg1, sreg1,
                 sreg2);
    emit_simd_comp_op(cfg, bb, ins, ins->inst_c1, temp_reg2, temp_reg1,
                      ins->sreg2);
  ...
  case MONO_TYPE_U8:
  USE_SIGNED_GT : {
    // convert to signed integer by subtracting (1 << (size - 1)) from each
    // operand and then use signed comparison
    ...
    NEW_SIMD_INS(cfg, ins, temp, simd_type_to_shl_op(type), temp_c80, temp_c0,
                 -1);
    temp->inst_imm =
        type == MONO_TYPE_U2 ? 15 : (type == MONO_TYPE_U4 ? 31 : 63);
    NEW_SIMD_INS(cfg, ins, temp, simd_type_to_sub_op(type), temp_s1, sreg1,
                 temp_c80);
    NEW_SIMD_INS(cfg, ins, temp, simd_type_to_sub_op(type), temp_s2, sreg2,
                 temp_c80);
...
static void emit_simd_gt_op(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst *ins,
  ...
  if (!mono_hwcap_x86_has_sse42 &&
      (type == MONO_TYPE_I8 || type == MONO_TYPE_U8)) {
...
static void emit_simd_min_op(MonoCompile *cfg, MonoBasicBlock *bb,
                             MonoInst *ins, int type, int dreg, int sreg1,
                             int sreg2) {
  MonoInst *temp;
  ...
  } else if (!mono_hwcap_x86_has_sse41 || type == MONO_TYPE_I8 ||
             type == MONO_TYPE_U8) {
...
static void emit_simd_max_op(MonoCompile *cfg, MonoBasicBlock *bb,
                             MonoInst *ins, int type, int dreg, int sreg1,
                             int sreg2) {
  MonoInst *temp;
...
void mono_arch_lowering_pass(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS_SAFE(bb, n, ins) {
    ...
    case OP_XEQUAL: {
      ...
      NEW_SIMD_INS(cfg, ins, temp, OP_PCMPEQD, temp_reg1, ins->sreg1,
                   ins->sreg2);
    ...
    case OP_XCOMPARE: {
      ...
      switch (ins->inst_c0) {
      case CMP_EQ:
        emit_simd_comp_op(cfg, bb, ins, ins->inst_c1, ins->dreg, ins->sreg1,
                          ins->sreg2);
        NULLIFY_INS(ins);
        break;
      ...
      case CMP_NE: {
        ...
        emit_simd_comp_op(cfg, bb, ins, ins->inst_c1, temp_reg1, ins->sreg1,
                          ins->sreg2);
      ...
      case CMP_LT:
        ...
        ins->sreg2 = temp_reg;
      case CMP_GT:
        emit_simd_gt_op(cfg, bb, ins, ins->inst_c1, ins->dreg, ins->sreg1,
                        ins->sreg2);
        NULLIFY_INS(ins);
        break;
      ...
      case CMP_GE: {
        ...
        emit_simd_gt_op(cfg, bb, ins, ins->inst_c1, temp_reg1, ins->sreg1,
                        ins->sreg2);
        emit_simd_comp_op(cfg, bb, ins, ins->inst_c1, temp_reg2, ins->sreg1,
                          ins->sreg2);
        ins->opcode = OP_POR;
        ins->sreg1 = temp_reg1;
        ins->sreg2 = temp_reg2;
      ...
      case CMP_GE_UN:
        if (mono_hwcap_x86_has_sse41 && ins->inst_c1 != MONO_TYPE_U8) {
          ...
          NEW_SIMD_INS(cfg, ins, temp, simd_type_to_max_un_op(ins->inst_c1),
                       temp_reg1, ins->sreg1, ins->sreg2);
          emit_simd_comp_op(cfg, bb, ins, ins->inst_c1, ins->dreg, temp_reg1,
                            ins->sreg1);
          NULLIFY_INS(ins);
        } else {
          int temp_reg1 = mono_alloc_ireg(cfg);
          ...
          emit_simd_gt_un_op(cfg, bb, ins, ins->inst_c1, temp_reg1, ins->sreg1,
                             ins->sreg2);
          emit_simd_comp_op(cfg, bb, ins, ins->inst_c1, temp_reg2, ins->sreg1,
                            ins->sreg2);
          ins->opcode = OP_POR;
      ...
      case CMP_LT_UN:
        ...
        ins->sreg2 = temp_reg;
      case CMP_GT_UN: {
        emit_simd_gt_un_op(cfg, bb, ins, ins->inst_c1, ins->dreg, ins->sreg1,
                           ins->sreg2);
        NULLIFY_INS(ins);
        break;
      }
    ...
    case OP_XBINOP: {
      ...
      case OP_IOR:
        ...
        break;
      case OP_IMIN:
        emit_simd_min_op(cfg, bb, ins, ins->inst_c1, ins->dreg, ins->sreg1,
                         ins->sreg2);
        NULLIFY_INS(ins);
        break;
      case OP_IMAX:
        emit_simd_max_op(cfg, bb, ins, ins->inst_c1, ins->dreg, ins->sreg1,
                         ins->sreg2);
        NULLIFY_INS(ins);
        break;
      case OP_FSUB:
...
static const int branch_cc_table[] = {
...
static const int cc_table[] = {X86_CC_EQ, X86_CC_NE, X86_CC_LE, X86_CC_GE,
                               X86_CC_LT, X86_CC_GT, X86_CC_LE, X86_CC_GE,
                               X86_CC_LT, X86_CC_GT};

static const int cc_signed_table[] = {TRUE, TRUE,  TRUE,  TRUE,  TRUE,
                                      TRUE, FALSE, FALSE, FALSE, FALSE};

/*#include "cprop.c"*/

static unsigned char *emit_float_to_int(MonoCompile *cfg, guchar *code,
                                        int dreg, int sreg, int size,
                                        gboolean is_signed) {
  // Use 8 as register size to get Nan/Inf conversion to uint result truncated
  // to 0
...
static unsigned char *mono_emit_stack_alloc(MonoCompile *cfg, guchar *code,
                                            MonoInst *tree) {
  int sreg = tree->sreg1;
  ...
  if (need_touch) {
    ...
    br[3] = code;
    x86_branch8(code, X86_CC_AE, 0, FALSE);
...
static guint8 *emit_move_return_value(MonoCompile *cfg, MonoInst *ins,
                                      guint8 *code) {
  CallInfo *cinfo;
  guint32 quad;
  ...
  case OP_FCALL_MEMBASE: {
    MonoType *rtype =
        mini_get_underlying_type(((MonoCallInst *)ins)->signature->ret);
  ...
  case OP_VCALL2_MEMBASE:
    ...
    if (cinfo->ret.storage == ArgValuetypeInReg) {
      ...
      amd64_mov_reg_membase(code, AMD64_RCX, loc->inst_basereg,
                            loc->inst_offset, sizeof(gpointer));

      for (quad = 0; quad < 2; quad++) {
        switch (cinfo->ret.pair_storage[quad]) {
        case ArgInIReg:
          amd64_mov_membase_reg(
              code, AMD64_RCX, (quad * sizeof(target_mgreg_t)),
              cinfo->ret.pair_regs[quad], sizeof(target_mgreg_t));
          break;
        case ArgInFloatSSEReg:
          amd64_movss_membase_reg(code, AMD64_RCX, (quad * 8),
                                  cinfo->ret.pair_regs[quad]);
          break;
        case ArgInDoubleSSEReg:
          amd64_movsd_membase_reg(code, AMD64_RCX, (quad * 8),
                                  cinfo->ret.pair_regs[quad]);
          break;
        case ArgNone:
          break;
...
gboolean mono_arch_have_fast_tls(void) {
   ...
   * retq
   */
  have_fast_tls = ins[0] == 0x65 && ins[1] == 0x48 && ins[2] == 0x8b &&
                  ins[3] == 0x04 && ins[4] == 0xfd && ins[6] == 0x00 &&
                  ins[7] == 0x00 && ins[8] == 0x00 && ins[9] == 0xc3;

  tls_gs_offset = ins[5];

  /*
   * Apple now loads a different version of pthread_getspecific when launched
   * from Xcode For that version we're looking for these instructions:
   *
   * pushq  %rbp
   * movq   %rsp, %rbp
   * mov    %gs:[offset](,%rdi,8),%rax
   ...
   */
  if (!have_fast_tls) {
    have_fast_tls = ins[0] == 0x55 && ins[1] == 0x48 && ins[2] == 0x89 &&
                    ins[3] == 0xe5 && ins[4] == 0x65 && ins[5] == 0x48 &&
                    ins[6] == 0x8b && ins[7] == 0x04 && ins[8] == 0xfd &&
                    ins[10] == 0x00 && ins[11] == 0x00 && ins[12] == 0x00 &&
                    ins[13] == 0x5d && ins[14] == 0xc3;
...
int mono_amd64_get_tls_gs_offset(void) {
#ifdef TARGET_OSX
 ...
 * by tls_offset.
 */
static guint8 *mono_amd64_emit_tls_get(guint8 *code, int dreg, int tls_offset) {
#ifdef TARGET_WIN32
  if (tls_offset < 64) {
...
static guint8 *mono_amd64_emit_tls_set(guint8 *code, int sreg, int tls_offset) {
#ifdef TARGET_WIN32
  g_assert_not_reached();
#elif defined(TARGET_MACH)
  x86_prefix(code, X86_GS_PREFIX);
 ...
 */
static guint8 *emit_setup_lmf(MonoCompile *cfg, guint8 *code, gint32 lmf_offset,
                              int cfa_offset) {
  /*
   * The ip field is not set, the exception handling code will obtain it from
   * the stack location pointed to by the sp field.
   */
  /*
   * sp is saved right before calls but we need to save it here too so
   * async stack walks would work.
   */
  amd64_mov_membase_reg(code, cfg->frame_reg,
                        lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, rsp),
                        AMD64_RSP, 8);
  /* Save rbp */
  amd64_mov_membase_reg(code, cfg->frame_reg,
                        lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, rbp),
                        AMD64_RBP, 8);
  if (cfg->arch.omit_fp && cfa_offset != -1)
    mono_emit_unwind_op_offset(
        cfg, code, AMD64_RBP,
        -(cfa_offset - (lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, rbp))));

  /* These can't contain refs */
  mini_gc_set_slot_type_from_fp(
      cfg, lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, previous_lmf), SLOT_NOREF);
  mini_gc_set_slot_type_from_fp(
      cfg, lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, rsp), SLOT_NOREF);
  /* These are handled automatically by the stack marking code */
  mini_gc_set_slot_type_from_fp(
      cfg, lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, rbp), SLOT_NOREF);
...
static guint8 *amd64_handle_varargs_nregs(guint8 *code, guint32 nregs) {
#ifndef TARGET_WIN32
  if (nregs)
...
static guint8 *amd64_handle_varargs_call(MonoCompile *cfg, guint8 *code,
                                         MonoCallInst *call,
                                         gboolean free_rax) {
#ifdef TARGET_WIN32
  return code;
  ...
  } else if (cfg->method->wrapper_type == MONO_WRAPPER_MANAGED_TO_NATIVE &&
             m_class_get_image(cfg->method->klass) != mono_defaults.corlib) {
    /*
...
void mono_arch_output_basic_block(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  gboolean optimize_branch_pred =
      (cfg->opt & MONO_OPT_BRANCH) &&
      (cfg->max_block_num < MAX_BBLOCKS_FOR_BRANCH_OPTS);

  if (optimize_branch_pred) {
    int current_offset = cfg->code_len;
    MonoBasicBlock *current_bb;
    for (current_bb = bb; current_bb != NULL;
  ...
  if (mono_break_at_bb_method &&
      mono_method_desc_full_match(mono_break_at_bb_method, cfg->method) &&
      bb->block_num == mono_break_at_bb_bb_num)
    x86_breakpoint(code);
  ...
  MONO_BB_FOR_EACH_INS(bb, ins) {
    ...
    case OP_X86_SETEQ_MEMBASE:
      amd64_set_membase(code, X86_CC_EQ, ins->inst_basereg, ins->inst_offset,
                        TRUE);
      break;
    case OP_STOREI1_MEMBASE_IMM:
      amd64_mov_membase_imm(code, ins->inst_destbasereg, ins->inst_offset,
                            ins->inst_imm, 1);
      break;
    ...
    case OP_STORE_MEMBASE_IMM:
      ...
      amd64_mov_membase_imm(code, ins->inst_destbasereg, ins->inst_offset,
                            (gint32)ins->inst_imm, sizeof(gpointer));
      break;
    case OP_STOREI8_MEMBASE_IMM:
      g_assert(amd64_is_imm32(ins->inst_imm));
      amd64_mov_membase_imm(code, ins->inst_destbasereg, ins->inst_offset,
    ...
    case OP_LOADI8_MEM:
      ...
      if (amd64_use_imm32(ins->inst_imm))
        ...
        amd64_mov_reg_imm_size(code, ins->dreg, ins->inst_imm,
                               sizeof(gpointer));
    ...
    case OP_LOAD_MEMBASE:
      ...
      amd64_mov_reg_membase(code, ins->dreg, ins->inst_basereg,
                            ins->inst_offset, sizeof(gpointer));
      break;
    case OP_LOADI8_MEMBASE:
    ...
    case OP_LOADI4_MEMBASE:
      amd64_movsxd_reg_membase(code, ins->dreg, ins->inst_basereg,
                               ins->inst_offset);
      break;
    ...
    case OP_LOADU1_MEMBASE:
      ...
      amd64_widen_membase_size(code, ins->dreg, ins->inst_basereg,
                               ins->inst_offset, FALSE, FALSE, 4);
      break;
    ...
    case OP_AMD64_LOADI8_MEMINDEX:
      amd64_mov_reg_memindex_size(code, ins->dreg, ins->inst_basereg, 0,
                                  ins->inst_indexreg, 0, 8);
      break;
    case OP_LCONV_TO_I1:
    case OP_ICONV_TO_I1:
    case OP_SEXT_I1:
    ...
    case OP_COMPARE_IMM:
#if defined(MONO_ARCH_ILP32)
      /* Comparison of pointer immediates should be 4 bytes to avoid sign-extend
       * problems */
      g_assert(amd64_is_imm32(ins->inst_imm));
    ...
    case OP_X86_COMPARE_REG_MEMBASE:
      amd64_alu_reg_membase(code, X86_CMP, ins->sreg1, ins->sreg2,
                            ins->inst_offset);
      break;
    ...
    case OP_X86_ADD_REG_MEMBASE:
      amd64_alu_reg_membase_size(code, X86_ADD, ins->sreg1, ins->sreg2,
                                 ins->inst_offset, 4);
      break;
    case OP_X86_SUB_REG_MEMBASE:
      amd64_alu_reg_membase_size(code, X86_SUB, ins->sreg1, ins->sreg2,
                                 ins->inst_offset, 4);
      break;
    case OP_X86_AND_REG_MEMBASE:
      amd64_alu_reg_membase_size(code, X86_AND, ins->sreg1, ins->sreg2,
                                 ins->inst_offset, 4);
      break;
    case OP_X86_OR_REG_MEMBASE:
      amd64_alu_reg_membase_size(code, X86_OR, ins->sreg1, ins->sreg2,
                                 ins->inst_offset, 4);
      break;
    case OP_X86_XOR_REG_MEMBASE:
      amd64_alu_reg_membase_size(code, X86_XOR, ins->sreg1, ins->sreg2,
                                 ins->inst_offset, 4);
      break;

    case OP_X86_ADD_MEMBASE_IMM:
      /* FIXME: Make a 64 version too */
      amd64_alu_membase_imm_size(code, X86_ADD, ins->inst_basereg,
                                 ins->inst_offset, ins->inst_imm, 4);
      break;
    case OP_X86_SUB_MEMBASE_IMM:
      g_assert(amd64_is_imm32(ins->inst_imm));
      amd64_alu_membase_imm_size(code, X86_SUB, ins->inst_basereg,
                                 ins->inst_offset, ins->inst_imm, 4);
      break;
    case OP_X86_AND_MEMBASE_IMM:
      g_assert(amd64_is_imm32(ins->inst_imm));
      amd64_alu_membase_imm_size(code, X86_AND, ins->inst_basereg,
                                 ins->inst_offset, ins->inst_imm, 4);
      break;
    case OP_X86_OR_MEMBASE_IMM:
      g_assert(amd64_is_imm32(ins->inst_imm));
      amd64_alu_membase_imm_size(code, X86_OR, ins->inst_basereg,
                                 ins->inst_offset, ins->inst_imm, 4);
      break;
    case OP_X86_XOR_MEMBASE_IMM:
      g_assert(amd64_is_imm32(ins->inst_imm));
      amd64_alu_membase_imm_size(code, X86_XOR, ins->inst_basereg,
                                 ins->inst_offset, ins->inst_imm, 4);
      break;
    case OP_X86_ADD_MEMBASE_REG:
      amd64_alu_membase_reg_size(code, X86_ADD, ins->inst_basereg,
                                 ins->inst_offset, ins->sreg2, 4);
      break;
    case OP_X86_SUB_MEMBASE_REG:
      amd64_alu_membase_reg_size(code, X86_SUB, ins->inst_basereg,
                                 ins->inst_offset, ins->sreg2, 4);
      break;
    case OP_X86_AND_MEMBASE_REG:
      amd64_alu_membase_reg_size(code, X86_AND, ins->inst_basereg,
                                 ins->inst_offset, ins->sreg2, 4);
      break;
    case OP_X86_OR_MEMBASE_REG:
      amd64_alu_membase_reg_size(code, X86_OR, ins->inst_basereg,
                                 ins->inst_offset, ins->sreg2, 4);
      break;
    case OP_X86_XOR_MEMBASE_REG:
      amd64_alu_membase_reg_size(code, X86_XOR, ins->inst_basereg,
                                 ins->inst_offset, ins->sreg2, 4);
      break;
    ...
    case OP_X86_MUL_MEMBASE_REG:
      amd64_imul_reg_membase_size(code, ins->sreg1, ins->sreg2,
                                  ins->inst_offset, 4);
      break;
    case OP_AMD64_ICOMPARE_MEMBASE_REG:
      amd64_alu_membase_reg_size(code, X86_CMP, ins->inst_basereg,
                                 ins->inst_offset, ins->sreg2, 4);
      break;
    case OP_AMD64_ICOMPARE_MEMBASE_IMM:
      amd64_alu_membase_imm_size(code, X86_CMP, ins->inst_basereg,
                                 ins->inst_offset, ins->inst_imm, 4);
      break;
    case OP_AMD64_COMPARE_MEMBASE_REG:
      amd64_alu_membase_reg_size(code, X86_CMP, ins->inst_basereg,
                                 ins->inst_offset, ins->sreg2, 8);
      break;
    case OP_AMD64_COMPARE_MEMBASE_IMM:
      g_assert(amd64_is_imm32(ins->inst_imm));
      amd64_alu_membase_imm_size(code, X86_CMP, ins->inst_basereg,
                                 ins->inst_offset, ins->inst_imm, 8);
      break;
    case OP_X86_COMPARE_MEMBASE8_IMM:
      amd64_alu_membase8_imm_size(code, X86_CMP, ins->inst_basereg,
                                  ins->inst_offset, ins->inst_imm, 4);
      break;
    case OP_AMD64_ICOMPARE_REG_MEMBASE:
      amd64_alu_reg_membase_size(code, X86_CMP, ins->sreg1, ins->sreg2,
                                 ins->inst_offset, 4);
      break;
    ...
    case OP_SEQ_POINT: {
      if (ins->flags & MONO_INST_SINGLE_STEP_LOC) {
        ...
        amd64_mov_reg_membase(code, AMD64_R11, var->inst_basereg,
                              var->inst_offset, 8);
      ...
      if (cfg->compile_aot) {
        ...
        /* Load info var */
        amd64_mov_reg_membase(code, AMD64_R11, info_var->inst_basereg,
                              info_var->inst_offset, 8);
        val = ((offset) * sizeof(target_mgreg_t)) +
              MONO_STRUCT_OFFSET(SeqPointInfo, bp_addrs);
        /* Load the info->bp_addrs [offset], which is either NULL or the address
         * of the breakpoint trampoline */
        ...
        /*
         * Emit a test+branch against a constant, the constant will be
         * overwritten by mono_arch_set_breakpoint () to cause the test to fail.
         */
    ...
    case OP_IMUL_IMM: {
      ...
      case 100:
        ...
        break;
      default:
        amd64_imul_reg_reg_imm_size(code, ins->dreg, ins->sreg1, ins->inst_imm,
                                    size);
        break;
      }
      break;
    ...
    case OP_LBGE_UN:
    case OP_LBLE_UN:
      EMIT_COND_BRANCH(ins, cc_table[mono_opcode_to_cond(ins->opcode)],
                       cc_signed_table[mono_opcode_to_cond(ins->opcode)]);
      break;
    ...
    case OP_CMOV_LLT_UN:
      ...
      amd64_cmov_reg(code, cc_table[mono_opcode_to_cond(ins->opcode)],
                     cc_signed_table[mono_opcode_to_cond(ins->opcode)],
                     ins->dreg, ins->sreg2);
      break;
    ...
    case OP_TAILCALL_MEMBASE: {
      ...
      max_len += call->stack_usage / sizeof(target_mgreg_t) *
      ...
      if (tailcall_reg) {
        ...
        if (cfg->compile_aot) {
          mono_add_patch_info(cfg, code - cfg->native_code,
                              MONO_PATCH_INFO_METHOD_JUMP, call->method);
      ...
      for (i = 0; i < AMD64_NREG; ++i)
        if (AMD64_IS_CALLEE_SAVED_REG(i) &&
            (cfg->used_int_regs & ((regmask_t)1 << i))) {
      ...
      if (cfg->arch.omit_fp) {
        ...
        for (i = 0; i < call->stack_usage; i += sizeof(target_mgreg_t)) {
          amd64_mov_reg_membase(code, AMD64_RAX, AMD64_RSP, i + 8,
                                sizeof(target_mgreg_t));
          amd64_mov_membase_reg(code, AMD64_RBP, ARGS_OFFSET + i, AMD64_RAX,
                                sizeof(target_mgreg_t));
        }
        amd64_pop_reg(code, AMD64_RAX);
#ifdef TARGET_WIN32
...
#ifdef TARGET_WIN32
      // Redundant REX byte indicates a tailcall to the native unwinder. It
      // means nothing to the processor.
      ...
      // FIXME This should be jmp [rip+32] for AOT direct to not-same assembly
      // (through data).
      // FIXME This should be jmp [rip+32] for JIT direct -- patch data instead
      // of code. This is only close to ideal for tailcall_membase, and even
      // then it should have a more dynamic register allocation.
      x86_imm_emit8(code, 0x48);
      amd64_jump_reg(code, AMD64_RAX);
#else
      // NT does not have varargs rax use, and NT ABI does not have red zone.
      // Use red-zone mov/jmp instead of push/ret to preserve call/ret
      // speculation stack.
    ...
    case OP_DYN_CALL: {
      ...
      /* Save args buffer */
      amd64_mov_membase_reg(code, var->inst_basereg, var->inst_offset,
                            AMD64_R11, 8);

      /* Set fp arg regs */
      amd64_mov_reg_membase(code, AMD64_RAX, AMD64_R11,
                            MONO_STRUCT_OFFSET(DynCallArgs, has_fp),
                            sizeof(target_mgreg_t));
      amd64_test_reg_reg(code, AMD64_RAX, AMD64_RAX);
      label = code;
      amd64_branch8(code, X86_CC_Z, -1, 1);
      for (i = 0; i < FLOAT_PARAM_REGS; ++i)
        amd64_sse_movsd_reg_membase(code, i, AMD64_R11,
                                    MONO_STRUCT_OFFSET(DynCallArgs, fregs) +
                                        (i * sizeof(double)));
      amd64_patch(label, code);

      /* Allocate param area */
      /* This doesn't need to be freed since OP_DYN_CALL is never called in a
       * loop */
      amd64_mov_reg_membase(code, AMD64_RAX, AMD64_R11,
                            MONO_STRUCT_OFFSET(DynCallArgs, nstack_args), 8);
      ...
      src_reg = AMD64_R8;
      dst_reg = AMD64_R9;
      amd64_mov_reg_membase(code, limit_reg, AMD64_R11,
                            MONO_STRUCT_OFFSET(DynCallArgs, nstack_args), 8);
      amd64_mov_reg_imm(code, index_reg, 0);
      amd64_lea_membase(code, src_reg, AMD64_R11,
                        MONO_STRUCT_OFFSET(DynCallArgs, regs) +
                            ((PARAM_REGS) * sizeof(target_mgreg_t)));
      ...
      for (i = 0; i < PARAM_REGS; ++i)
        amd64_mov_reg_membase(code, param_regs[i], AMD64_R11,
                              MONO_STRUCT_OFFSET(DynCallArgs, regs) +
                                  (i * sizeof(target_mgreg_t)),
                              sizeof(target_mgreg_t));
      ...
      amd64_mov_reg_membase(code, AMD64_R11, var->inst_basereg,
                            var->inst_offset, 8);
      amd64_mov_membase_reg(code, AMD64_R11,
                            MONO_STRUCT_OFFSET(DynCallArgs, res), AMD64_RAX, 8);
      amd64_sse_movsd_membase_reg(
          code, AMD64_R11, MONO_STRUCT_OFFSET(DynCallArgs, fregs), AMD64_XMM0);
      amd64_sse_movsd_membase_reg(
          code, AMD64_R11,
          MONO_STRUCT_OFFSET(DynCallArgs, fregs) + sizeof(double), AMD64_XMM1);
      break;
    }
    case OP_AMD64_SAVE_SP_TO_LMF: {
      MonoInst *lmf_var = cfg->lmf_var;
      amd64_mov_membase_reg(code, lmf_var->inst_basereg,
                            lmf_var->inst_offset +
    ...
    case OP_GENERIC_CLASS_INIT: {
      ...
      amd64_test_membase_imm_size(
          code, ins->sreg1, MONO_STRUCT_OFFSET(MonoVTable, initialized), 1, 1);
      jump = code;
    ...
    case OP_X86_LEA:
      amd64_lea_memindex(code, ins->dreg, ins->sreg1, ins->inst_imm, ins->sreg2,
                         ins->backend.shift_amount);
      break;
    ...
    case OP_LOCALLOC:
      /* keep alignment */
      amd64_alu_reg_imm(code, X86_ADD, ins->sreg1,
    ...
    case OP_LOCALLOC_IMM: {
      ...
      size = (size + (MONO_ARCH_FRAME_ALIGNMENT - 1)) &
             ~(MONO_ARCH_FRAME_ALIGNMENT - 1);
    ...
    case OP_THROW: {
      ...
      code =
          emit_call(cfg, NULL, code, MONO_JIT_ICALL_mono_arch_throw_exception);
      ins->flags |= MONO_INST_GC_CALLSITE;
      ins->backend.pc_offset = code - cfg->native_code;
      break;
    }
    ...
    case OP_CALL_HANDLER:
       ...
       */
      for (GList *tmp = ins->inst_eh_blocks; tmp != bb->clause_holes;
           tmp = tmp->prev)
        mono_cfg_add_try_hole(cfg, ((MonoLeaveClause *)tmp->data)->clause, code,
                              bb);
    ...
    case OP_START_HANDLER: {
      ...
      amd64_mov_membase_reg(code, spvar->inst_basereg, spvar->inst_offset,
                            AMD64_RSP, sizeof(gpointer));
      ...
      if ((MONO_BBLOCK_IS_IN_REGION(bb, MONO_REGION_FINALLY) ||
          ...
          cfg->param_area) {
        amd64_alu_reg_imm(code, X86_SUB, AMD64_RSP,
                          ALIGN_TO(cfg->param_area, MONO_ARCH_FRAME_ALIGNMENT));
      }
      break;
    }
    case OP_ENDFINALLY: {
      MonoInst *spvar = mono_find_spvar_for_region(cfg, bb->region);
      amd64_mov_reg_membase(code, AMD64_RSP, spvar->inst_basereg,
                            spvar->inst_offset, sizeof(gpointer));
      amd64_ret(code);
      break;
    }
    case OP_ENDFILTER: {
    ...
    case OP_BR:
      ...
      // ins->inst_target_bb, bb->next_bb, ins, bb->last_ins); if
      // ((ins->inst_target_bb == bb->next_bb) && ins == bb->last_ins) break;
      if (ins->inst_target_bb->native_offset) {
        amd64_jump_code(code,
                        cfg->native_code + ins->inst_target_bb->native_offset);
      } else {
        mono_add_patch_info(cfg, offset, MONO_PATCH_INFO_BB,
                            ins->inst_target_bb);
        if (optimize_branch_pred &&
    ...
    case OP_LCGT_UN:
    case OP_ICGT_UN:
      amd64_set_reg(code, cc_table[mono_opcode_to_cond(ins->opcode)], ins->dreg,
                    cc_signed_table[mono_opcode_to_cond(ins->opcode)]);
    ...
    case OP_COND_EXC_ILE_UN:
      EMIT_COND_SYSTEM_EXCEPTION(
          cc_table[mono_opcode_to_cond(ins->opcode)],
          cc_signed_table[mono_opcode_to_cond(ins->opcode)],
          (const char *)ins->inst_p1);
      break;
    case OP_COND_EXC_OV:
    case OP_COND_EXC_NO:
    case OP_COND_EXC_C:
    ...
    case OP_R4CONST: {
      ...
      if ((f == 0.0) && (mono_signbit(f) == 0)) {
        ...
        if (cfg->compile_aot && cfg->code_exec_only) {
          mono_add_patch_info(cfg, offset, MONO_PATCH_INFO_R4_GOT,
                              ins->inst_p0);
          amd64_mov_reg_membase(code, AMD64_R11, AMD64_RIP, 0,
                                sizeof(gpointer));
    ...
    case OP_STORER8_MEMBASE_REG:
      amd64_sse_movsd_membase_reg(code, ins->inst_destbasereg, ins->inst_offset,
                                  ins->sreg1);
      break;
    case OP_LOADR8_MEMBASE:
      amd64_sse_movsd_reg_membase(code, ins->dreg, ins->inst_basereg,
                                  ins->inst_offset);
      break;
    case OP_STORER4_MEMBASE_REG:
      if (cfg->r4fp) {
        amd64_sse_movss_membase_reg(code, ins->inst_destbasereg,
                                    ins->inst_offset, ins->sreg1);
      } else {
        /* This requires a double->single conversion */
        amd64_sse_cvtsd2ss_reg_reg(code, MONO_ARCH_FP_SCRATCH_REG, ins->sreg1);
        amd64_sse_movss_membase_reg(code, ins->inst_destbasereg,
                                    ins->inst_offset, MONO_ARCH_FP_SCRATCH_REG);
      }
      break;
    case OP_LOADR4_MEMBASE:
      if (cfg->r4fp) {
        amd64_sse_movss_reg_membase(code, ins->dreg, ins->inst_basereg,
                                    ins->inst_offset);
      } else {
        amd64_sse_movss_reg_membase(code, ins->dreg, ins->inst_basereg,
                                    ins->inst_offset);
    ...
    case OP_LCONV_TO_R_UN: {
      ...
      amd64_test_reg_reg(code, ins->sreg1, ins->sreg1);
      br[0] = code;
      x86_branch8(code, X86_CC_S, 0, TRUE);
    ...
    case OP_FNEG: {
      ...
      if (cfg->compile_aot && cfg->code_exec_only) {
        ...
        amd64_mov_reg_membase(code, AMD64_R11, AMD64_RIP, 0,
                              sizeof(target_mgreg_t));
        amd64_sse_movsd_reg_membase(code, MONO_ARCH_FP_SCRATCH_REG, AMD64_R11,
                                    0);
    ...
    case OP_RNEG: {
      ...
      if (cfg->compile_aot && cfg->code_exec_only) {
        ...
        amd64_mov_reg_membase(code, AMD64_R11, AMD64_RIP, 0,
                              sizeof(target_mgreg_t));
        amd64_sse_movss_reg_membase(code, MONO_ARCH_FP_SCRATCH_REG, AMD64_R11,
                                    0);
      } else {
        mono_add_patch_info(cfg, offset, MONO_PATCH_INFO_R4, &r4_0);
        amd64_sse_movss_reg_membase(code, MONO_ARCH_FP_SCRATCH_REG, AMD64_RIP,
                                    0);
      }
    ...
    case OP_FCEQ_MEMBASE: {
      ...
      amd64_alu_reg_reg(code, X86_XOR, ins->dreg, ins->dreg);
      amd64_sse_comisd_reg_membase(code, ins->sreg1, ins->sreg2,
                                   ins->inst_offset);
    ...
    case OP_ATOMIC_ADD_I8: {
      ...
      amd64_xadd_membase_reg(code, ins->inst_basereg, ins->inst_offset, dreg,
                             size);
    ...
    case OP_ATOMIC_EXCHANGE_I8: {
      ...
      amd64_xchg_membase_reg_size(code, ins->sreg1, ins->inst_offset,
                                  GP_SCRATCH_REG, size);
    ...
    case OP_ATOMIC_CAS_I8: {
      ...
      amd64_prefix(code, X86_LOCK_PREFIX);
      amd64_cmpxchg_membase_reg_size(code, ins->sreg1, ins->inst_offset,
                                     ins->sreg2, size);
    ...
    case OP_ATOMIC_LOAD_U8: {
      amd64_mov_reg_membase(code, ins->dreg, ins->inst_basereg,
                            ins->inst_offset,
                            ins->opcode == OP_ATOMIC_LOAD_U4 ? 4 : 8);
      break;
    }
    ...
    case OP_CARD_TABLE_WBARRIER: {
      ...
      guint64 nursery_start =
          (guint64)mono_gc_get_nursery(&nursery_shift, &nursery_size);
      guint64 shifted_nursery_start = nursery_start >> nursery_shift;

      /*If either point to the stack we can simply avoid the WB. This happens
       * due to optimizations revealing a stack store that was not visible when
       * op_cardtable was emited.
       */
      ...
      if (mono_gc_card_table_nursery_check()) {
        ...
        br = code;
        x86_branch8(code, X86_CC_NE, -1, FALSE);
      }
      amd64_mov_reg_reg(code, AMD64_RDX, ptr, 8);
      amd64_shift_reg_imm(code, X86_SHR, AMD64_RDX, card_table_shift);
      if (card_table_mask)
        amd64_alu_reg_imm(code, X86_AND, AMD64_RDX,
                          (guint32)(guint64)card_table_mask);

      mono_add_patch_info(cfg, code - cfg->native_code,
                          MONO_PATCH_INFO_GC_CARD_TABLE_ADDR, card_table);
...
#ifdef MONO_ARCH_SIMD_INTRINSICS
    /* TODO: Some of these IR opcodes are marked as no clobber when they indeed
     * do. */
    case OP_ADDPS:
    ...
    case OP_EXTRACT_U1:
      ...
      amd64_widen_reg(code, ins->dreg, ins->dreg, ins->opcode == OP_EXTRACT_I1,
                      FALSE);
      break;
    case OP_EXTRACT_I2:
    case OP_EXTRACT_U2:
      ...
      amd64_widen_reg_size(code, ins->dreg, ins->dreg,
                           ins->opcode == OP_EXTRACT_I2, TRUE, 4);
      break;
    case OP_EXTRACT_R8:
      if (ins->inst_c0)
    ...
    case OP_INSERTX_U1_SLOW:
      ...
      amd64_alu_reg_imm(code, X86_AND, ins->sreg1,
                        ins->inst_c0 & 1 ? 0x00FF : 0xFF00);
      /*shift the value to insert if needed*/
      if (ins->inst_c0 & 1)
        amd64_shift_reg_imm_size(code, X86_SHL, ins->sreg2, 8, 4);
      /*join them together*/
      amd64_alu_reg_reg(code, X86_OR, ins->sreg1, ins->sreg2);
      amd64_sse_pinsrw_reg_reg_imm(code, ins->dreg, ins->sreg1,
                                   ins->inst_c0 / 2);
      break;
    case OP_INSERTX_I4_SLOW:
      amd64_sse_pinsrw_reg_reg_imm(code, ins->dreg, ins->sreg2,
                                   ins->inst_c0 * 2);
    ...
    case OP_INSERTX_R4_SLOW:
      ...
      case 0:
        ...
        break;
      case 1:
        amd64_sse_pshufd_reg_reg_imm(code, ins->dreg, ins->dreg,
    ...
    case OP_STOREX_MEMBASE:
      amd64_sse_movups_membase_reg(code, ins->dreg, ins->inst_offset,
                                   ins->sreg1);
      break;
    case OP_LOADX_MEMBASE:
      amd64_sse_movups_reg_membase(code, ins->dreg, ins->sreg1,
                                   ins->inst_offset);
      break;
    case OP_LOADX_ALIGNED_MEMBASE:
      amd64_sse_movaps_reg_membase(code, ins->dreg, ins->sreg1,
                                   ins->inst_offset);
      break;
    case OP_STOREX_ALIGNED_MEMBASE_REG:
      amd64_sse_movaps_membase_reg(code, ins->dreg, ins->inst_offset,
                                   ins->sreg1);
      break;
    case OP_STOREX_NTA_MEMBASE_REG:
      amd64_sse_movntps_reg_membase(code, ins->dreg, ins->sreg1,
                                    ins->inst_offset);
      break;
    case OP_PREFETCH_MEMBASE:
      amd64_sse_prefetch_reg_membase(code, ins->backend.arg_info, ins->sreg1,
                                     ins->inst_offset);
      break;
    ...
    case OP_SSE41_ROUNDP: {
      if (ins->inst_c1 == MONO_TYPE_R8)
        amd64_sse_roundpd_reg_reg_imm(code, ins->dreg, ins->sreg1,
                                      ins->inst_c0);
      else
        g_assert_not_reached(); // roundps, but it's not used anywhere for
                                // non-llvm back-end yet.
      break;
    }
#endif
    ...
    case OP_LIVERANGE_START: {
      if (cfg->verbose_level > 1)
        printf("R%d START=0x%x\n", MONO_VARINFO(cfg, ins->inst_c0)->vreg,
               (int)(code - cfg->native_code));
      MONO_VARINFO(cfg, ins->inst_c0)->live_range_start =
    ...
    case OP_GC_SPILL_SLOT_LIVENESS_DEF:
      ins->backend.pc_offset = code - cfg->native_code;
      bb->spill_slot_defs =
          g_slist_prepend_mempool(cfg->mempool, bb->spill_slot_defs, ins);
      break;
    ...
    case OP_FILL_PROF_CALL_CTX:
      for (int i = 0; i < AMD64_NREG; i++)
        if (AMD64_IS_CALLEE_SAVED_REG(i) || i == AMD64_RSP)
          amd64_mov_membase_reg(code, ins->sreg1,
                                MONO_STRUCT_OFFSET(MonoContext, gregs) +
                                    i * sizeof(target_mgreg_t),
                                i, sizeof(target_mgreg_t));
      break;
    default:
    ...
    g_assertf((code - cfg->native_code - offset) <= max_len,
              "wrong maximal instruction length of instruction %s (expected "
              "%d, got %d)",
              mono_inst_name(ins->opcode), max_len,
              (int)(code - cfg->native_code - offset));
  }
...
void mono_arch_register_lowlevel_calls(void) {
  ...
  mono_register_jit_icall(mono_amd64_throw_exception, mono_icall_sig_void,
                          TRUE);

#if defined(TARGET_WIN32) || defined(HOST_WIN32)
#if _MSC_VER
  mono_register_jit_icall_info(&mono_get_jit_icall_info()->mono_chkstk_win64,
                               __chkstk, "mono_chkstk_win64", NULL, TRUE,
                               "__chkstk");
#else
  mono_register_jit_icall_info(&mono_get_jit_icall_info()->mono_chkstk_win64,
                               ___chkstk_ms, "mono_chkstk_win64", NULL, TRUE,
                               "___chkstk_ms");
#endif
#endif
}

void mono_arch_patch_code_new(MonoCompile *cfg, MonoDomain *domain,
...
static int get_max_epilog_size(MonoCompile *cfg) {
...
/*
 * This macro is used for testing whenever the unwinder works correctly at every
 * point where an async exception can happen.
 */
/* This will generate a SIGSEGV at the given point in the code */
#define async_exc_point(code)                                                  \
  do {                                                                         \
    if (mono_inject_async_exc_method &&                                        \
        mono_method_desc_full_match(mono_inject_async_exc_method,              \
                                    cfg->method)) {                            \
...
#ifdef TARGET_WIN32
static guint8 *emit_prolog_setup_sp_win64(MonoCompile *cfg, guint8 *code,
                                          int alloc_size,
  ...
  if (alloc_size) {
    ...
    // NOTE, in a standard win64 prolog the alloc unwind info is always emitted,
    // but since mono uses a frame pointer with negative offsets and a standard
    // win64 prolog assumes positive offsets, we can't emit sp alloc unwind
    // metadata since the native OS unwinder will incorrectly restore sp.
    // Excluding the alloc metadata on the other hand won't give the OS the
    // information so it can just restore the frame pointer to sp and that will
...
guint8 *mono_arch_emit_prolog(MonoCompile *cfg) {
  ...
  if (cfg->arch.omit_fp) {
    ...
    if ((alloc_size % 16) == 0) {
      ...
      mini_gc_set_slot_type_from_cfa(cfg, -cfa_offset - sizeof(target_mgreg_t),
                                     SLOT_NOREF);
    }
  } else {
    ...
    if (cfg->stack_offset != alloc_size) {
      /* Mark the padding slot as NOREF */
      mini_gc_set_slot_type_from_fp(cfg, -alloc_size + cfg->param_area,
                                    SLOT_NOREF);
    }
  ...
  if (alloc_size) {
    ...
    guint32 required_code_size =
        ((remaining_size / 0x1000) + 1) *
        11; /*11 is the max size of amd64_alu_reg_imm + amd64_test_membase_reg*/
  ...
  if (cfg->arch.omit_fp) {
    ...
    /* FIXME: Optimize this so the regs are saved at the end of the frame in
     * increasing order */
  } else {
  ...
  for (i = 0; i < AMD64_NREG; ++i) {
    if (AMD64_IS_CALLEE_SAVED_REG(i) && (cfg->arch.saved_iregs & (1 << i))) {
      ...
      if (cfg->arch.omit_fp) {
        mono_emit_unwind_op_offset(cfg, code, i,
                                   -(cfa_offset - save_area_offset));
        /* These are handled automatically by the stack marking code */
        mini_gc_set_slot_type_from_cfa(cfg, -(cfa_offset - save_area_offset),
                                       SLOT_NOREF);
      } else {
        mono_emit_unwind_op_offset(cfg, code, i,
                                   -(-save_area_offset + (2 * 8)));
        // FIXME: GC
      }
  ...
  if (cfg->rgctx_var) {
    g_assert(cfg->rgctx_var->opcode == OP_REGOFFSET &&
             (cfg->rgctx_var->inst_basereg == AMD64_RBP ||
              cfg->rgctx_var->inst_basereg == AMD64_RSP));

    amd64_mov_membase_reg(code, cfg->rgctx_var->inst_basereg,
                          cfg->rgctx_var->inst_offset, MONO_ARCH_RGCTX_REG,
                          sizeof(gpointer));

    mono_add_var_location(cfg, cfg->rgctx_var, TRUE, MONO_ARCH_RGCTX_REG, 0, 0,
                          code - cfg->native_code);
    mono_add_var_location(
        cfg, cfg->rgctx_var, FALSE, cfg->rgctx_var->inst_basereg,
        cfg->rgctx_var->inst_offset, code - cfg->native_code, 0);
  }
  ...
  if (cfg->opt & MONO_OPT_BRANCH &&
    ...
    for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
      ...
      MONO_BB_FOR_EACH_INS(bb, ins) { max_length += ins_get_size(ins->opcode); }
  ...
  if (sig->ret->type != MONO_TYPE_VOID) {
    ...
    if (cfg->vret_addr && (cfg->vret_addr->opcode != OP_REGVAR))
      amd64_mov_membase_reg(code, cfg->vret_addr->inst_basereg,
                            cfg->vret_addr->inst_offset, cinfo->ret.reg, 8);
  }
    ...
    if (ins->opcode != OP_REGVAR) {
      ...
      case ArgInIReg: {
        ...
        */
         ...
         */
        if (i == 0 && sig->hasthis) {
          mono_add_var_location(cfg, ins, TRUE, ainfo->reg, 0, 0,
                                code - cfg->native_code);
          mono_add_var_location(cfg, ins, FALSE, ins->inst_basereg,
                                ins->inst_offset, code - cfg->native_code, 0);
        }
        break;
      }
      case ArgInFloatSSEReg:
        amd64_movss_membase_reg(code, ins->inst_basereg, ins->inst_offset,
                                ainfo->reg);
        break;
      case ArgInDoubleSSEReg:
        amd64_movsd_membase_reg(code, ins->inst_basereg, ins->inst_offset,
                                ainfo->reg);
        break;
      case ArgValuetypeInReg:
        for (quad = 0; quad < 2; quad++) {
          ...
          case ArgInIReg:
            amd64_mov_membase_reg(
                code, ins->inst_basereg,
                ins->inst_offset + (quad * sizeof(target_mgreg_t)),
                ainfo->pair_regs[quad], sizeof(target_mgreg_t));
            break;
          case ArgInFloatSSEReg:
            amd64_movss_membase_reg(code, ins->inst_basereg,
                                    ins->inst_offset +
                                        (quad * sizeof(target_mgreg_t)),
                                    ainfo->pair_regs[quad]);
            break;
          case ArgInDoubleSSEReg:
            amd64_movsd_membase_reg(code, ins->inst_basereg,
                                    ins->inst_offset +
                                        (quad * sizeof(target_mgreg_t)),
                                    ainfo->pair_regs[quad]);
            break;
      ...
      case ArgValuetypeAddrInIReg:
        if (ainfo->pair_storage[0] == ArgInIReg)
          amd64_mov_membase_reg(code, ins->inst_left->inst_basereg,
                                ins->inst_left->inst_offset,
                                ainfo->pair_regs[0], sizeof(target_mgreg_t));
        break;
      case ArgValuetypeAddrOnStack:
        break;
      case ArgGSharedVtInReg:
      ...
      case ArgInIReg:
        ...
        break;
      case ArgOnStack:
        amd64_mov_reg_membase(code, ins->dreg, AMD64_RBP,
                              ARGS_OFFSET + ainfo->offset, 8);
        break;
      default:
        g_assert_not_reached();
      }
      ...
      if (i == 0 && sig->hasthis) {
        ...
        mono_add_var_location(cfg, ins, TRUE, ainfo->reg, 0, 0,
                              code - cfg->native_code);
        mono_add_var_location(cfg, ins, TRUE, ins->dreg, 0,
   ...
   * arguments as the method. This works because the arguments are still in
   * their original argument registers.
      ...
      if (ins->opcode != OP_REGVAR) {
        ...
        case ArgInIReg: {
          if (((next->opcode == OP_LOAD_MEMBASE) ||
              ...
              next->inst_basereg == ins->inst_basereg &&
              next->inst_offset == ins->inst_offset) {
        ...
        switch (ainfo->storage) {
        case ArgInIReg:
          if (next->opcode == OP_MOVE && next->sreg1 == ins->dreg &&
              next->dreg == ainfo->reg) {
            NULLIFY_INS(next);
            match = TRUE;
          }
  ...
  if (cfg->gen_sdb_seq_points) {
    ...
    if (cfg->compile_aot) {
      ...
      /* Same as OP_AOTCONST */
      mono_add_patch_info(cfg, code - cfg->native_code,
                          MONO_PATCH_INFO_SEQ_POINT_INFO, cfg->method);
      amd64_mov_reg_membase(code, AMD64_R11, AMD64_RIP, 0, sizeof(gpointer));
      g_assert(info_var->opcode == OP_REGOFFSET);
      amd64_mov_membase_reg(code, info_var->inst_basereg, info_var->inst_offset,
                            AMD64_R11, 8);
    }
    ...
    if (cfg->compile_aot) {
      ...
      amd64_mov_reg_membase(code, AMD64_R11, info_var->inst_basereg,
                            info_var->inst_offset, 8);
      amd64_mov_reg_membase(code, AMD64_R11, AMD64_R11,
                            MONO_STRUCT_OFFSET(SeqPointInfo, ss_tramp_addr), 8);
      amd64_mov_membase_reg(code, ins->inst_basereg, ins->inst_offset,
                            AMD64_R11, 8);
    } else {
      /* Initialize ss_tramp_var */
...
void mono_arch_emit_epilog(MonoCompile *cfg) {
  ...
  if (method->save_lmf) {
    if (cfg->used_int_regs & (1 << AMD64_RBP))
      amd64_mov_reg_membase(code, AMD64_RBP, cfg->frame_reg,
                            lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, rbp), 8);
    if (cfg->arch.omit_fp)
      /*
       * emit_setup_lmf () marks RBP as saved, we have to mark it as same value
       * here before clearing up the stack since its stack slot will become
       * invalid.
       */
      mono_emit_unwind_op_same_value(cfg, code, AMD64_RBP);
  }
  ...
  if (cinfo->ret.storage == ArgValuetypeInReg) {
    ...
    for (quad = 0; quad < 2; quad++) {
      switch (ainfo->pair_storage[quad]) {
      case ArgInIReg:
        amd64_mov_reg_membase(code, ainfo->pair_regs[quad], inst->inst_basereg,
                              inst->inst_offset +
                                  (quad * sizeof(target_mgreg_t)),
                              ainfo->pair_size[quad]);
        break;
      case ArgInFloatSSEReg:
        amd64_movss_reg_membase(
            code, ainfo->pair_regs[quad], inst->inst_basereg,
            inst->inst_offset + (quad * sizeof(target_mgreg_t)));
        break;
      case ArgInDoubleSSEReg:
        amd64_movsd_reg_membase(
            code, ainfo->pair_regs[quad], inst->inst_basereg,
            inst->inst_offset + (quad * sizeof(target_mgreg_t)));
        break;
      case ArgNone:
...
void mono_arch_emit_exceptions(MonoCompile *cfg) {
  ...
  for (patch_info = cfg->patch_info; patch_info;
    ...
    case MONO_PATCH_INFO_EXC: {
      ...
      exc_class = mono_class_load_from_name(mono_defaults.corlib, "System",
      ...
      for (i = 0; i < nthrows; ++i)
        if (exc_classes[i] == exc_class)
          break;
      if (i < nthrows) {
        amd64_mov_reg_imm(code, AMD64_ARG_REG2,
                          (exc_throw_end[i] - cfg->native_code) - throw_ip);
        ...
        amd64_mov_reg_imm(code, AMD64_ARG_REG1,
        ...
        amd64_mov_reg_imm(buf, AMD64_ARG_REG2,
                          (code - cfg->native_code) - throw_ip);
        while (buf < buf2)
          x86_nop(buf);
...
gboolean mono_arch_is_int_overflow(void *sigctx, void *info) {
  ...
  if ((rip[0] == 0xf7) && (x86_modrm_mod(rip[1]) == 0x3) &&
      (x86_modrm_reg(rip[1]) == 0x7)) {
    /* idiv REG */
...
guint32 mono_arch_get_patch_offset(guint8 *code) { return 3; }

/**
 * \return TRUE if no sw breakpoint was present.
 *
 * Copy \p size bytes from \p code - \p offset to the buffer \p buf. If the
 * debugger inserted software breakpoints in the original code, they are removed
 * in the copy.
 */
gboolean mono_breakpoint_clean_code(guint8 *method_start, guint8 *code,
                                    int offset, guint8 *buf, int size) {
  /*
   * If method_start is non-NULL we need to perform bound checks, since we
   * access memory at code - offset we could go before the start of the method
   * and end up in a different page of memory that is not mapped or read
   * incorrect data anyway. We zero-fill the bytes instead.
   */
...
int mono_arch_get_this_arg_reg(guint8 *code) { return AMD64_ARG_REG1; }
...
static gpointer get_delegate_invoke_impl(MonoTrampInfo **info,
  ...
  start = code = (guint8 *)mono_global_codeman_reserve(
      size + MONO_TRAMPOLINE_UNWINDINFO_SIZE(0));

  if (has_target) {

    /* Replace the this argument with the target */
    amd64_mov_reg_reg(code, AMD64_RAX, AMD64_ARG_REG1, 8);
    amd64_mov_reg_membase(code, AMD64_ARG_REG1, AMD64_RAX,
                          MONO_STRUCT_OFFSET(MonoDelegate, target), 8);
    amd64_jump_membase(code, AMD64_RAX,
                       MONO_STRUCT_OFFSET(MonoDelegate, method_ptr));

  } else {
    if (param_count == 0) {
      amd64_jump_membase(code, AMD64_ARG_REG1,
                         MONO_STRUCT_OFFSET(MonoDelegate, method_ptr));
    } else {
  ...
  g_assert_checked(mono_arch_unwindinfo_validate_size(
      unwind_ops, MONO_TRAMPOLINE_UNWINDINFO_SIZE(0)));

  mono_arch_flush_icache(start, code - start);

  if (has_target) {
    *info = mono_tramp_info_create("delegate_invoke_impl_has_target", start,
                                   code - start, NULL, unwind_ops);
  } else {
  ...
  MONO_PROFILER_RAISE(
      jit_code_buffer,
      (start, code - start, MONO_PROFILER_CODE_BUFFER_DELEGATE_INVOKE, NULL));
...
static gpointer get_delegate_virtual_invoke_impl(MonoTrampInfo **info,
  ...
  if (load_imt_reg) {
    /* Load the IMT reg */
    amd64_mov_reg_membase(code, MONO_ARCH_IMT_REG, AMD64_RAX,
                          MONO_STRUCT_OFFSET(MonoDelegate, method), 8);
  }

  /* Load the vtable */
  amd64_mov_reg_membase(code, AMD64_RAX, AMD64_ARG_REG1,
                        MONO_STRUCT_OFFSET(MonoObject, vtable), 8);
  ...
  *info =
      mono_tramp_info_create(tramp_name, start, code - start, NULL, unwind_ops);
  g_free(tramp_name);
...
gpointer mono_arch_get_delegate_invoke_impl(MonoMethodSignature *sig,
  ...
  if (has_target) {
    ...
    if (mono_ee_features.use_aot_trampolines) {
      start =
          (guint8 *)mono_aot_get_trampoline("delegate_invoke_impl_has_target");
    } else {
      MonoTrampInfo *info;
    ...
    if (mono_ee_features.use_aot_trampolines) {
      char *name =
          g_strdup_printf("delegate_invoke_impl_target_%d", sig->param_count);
      start = (guint8 *)mono_aot_get_trampoline(name);
      g_free(name);
    } else {
      MonoTrampInfo *info;
      start =
          (guint8 *)get_delegate_invoke_impl(&info, FALSE, sig->param_count);
      mono_tramp_info_register(info, NULL);
    }
...
gpointer mono_arch_get_delegate_virtual_invoke_impl(MonoMethodSignature *sig,
                                                    MonoMethod *method,
                                                    int offset,
                                                    gboolean load_imt_reg) {
  MonoTrampInfo *info;
...
static int imt_branch_distance(MonoIMTCheckItem **imt_entries, int start,
                               int target) {
  int i, distance = 0;
 ...
 */
gpointer mono_arch_build_imt_trampoline(MonoVTable *vtable, MonoDomain *domain,
                                        MonoIMTCheckItem **imt_entries,
                                        int count, gpointer fail_tramp) {
  int i;
  int size = 0;
  guint8 *code, *start;
  ...
  if (fail_tramp)
    code = (guint8 *)mono_method_alloc_generic_virtual_trampoline(
        domain, size + MONO_TRAMPOLINE_UNWINDINFO_SIZE(0));
  else
    code = (guint8 *)mono_domain_code_reserve(
        domain, size + MONO_TRAMPOLINE_UNWINDINFO_SIZE(0));
  start = code;
    ...
    if (item->is_equals) {
      ...
      if (item->check_target_idx || fail_case) {
        if (!item->compare_done || fail_case) {
          if (amd64_use_imm32((gint64)item->key))
            amd64_alu_reg_imm_size(code, X86_CMP, MONO_ARCH_IMT_REG,
                                   (guint32)(gssize)item->key,
                                   sizeof(gpointer));
          else {
            amd64_mov_reg_imm_size(code, MONO_ARCH_IMT_SCRATCH_REG, item->key,
                                   sizeof(gpointer));
            amd64_alu_reg_reg(code, X86_CMP, MONO_ARCH_IMT_REG,
        ...
        if (item->has_target_code) {
          amd64_mov_reg_imm(code, MONO_ARCH_IMT_SCRATCH_REG,
        ...
        /* enable the commented code to assert on wrong method */
                ...
                item->jmp_code = NULL;
#else
        /* We're using R10 (MONO_ARCH_IMT_SCRATCH_REG) here because R11
           (MONO_ARCH_IMT_REG) needs to be preserved.  R10 needs to be preserved
           for calls which require a runtime generic context, but interface
           calls don't. */
        amd64_mov_reg_imm(code, MONO_ARCH_IMT_SCRATCH_REG,
                          &(vtable->vtable[item->value.vtable_slot]));
        amd64_jump_membase(code, MONO_ARCH_IMT_SCRATCH_REG, 0);
      ...
      item->jmp_code = code;
      if (x86_is_imm8(
              imt_branch_distance(imt_entries, i, item->check_target_idx)))
        x86_branch8(code, X86_CC_GE, 0, FALSE);
      else
        x86_branch32(code, X86_CC_GE, 0, FALSE);
    }
    g_assertf(code - item->code_target <= item->chunk_size, "%X %X",
              (guint)(code - item->code_target), (guint)item->chunk_size);
  }
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (item->jmp_code) {
      if (item->check_target_idx) {
        amd64_patch(item->jmp_code,
                    imt_entries[item->check_target_idx]->code_target);
      }
    }
  }
  ...
  MONO_PROFILER_RAISE(
      jit_code_buffer,
      (start, code - start, MONO_PROFILER_CODE_BUFFER_IMT_TRAMPOLINE, NULL));

  mono_tramp_info_register(
      mono_tramp_info_create(NULL, start, code - start, NULL, unwind_ops),
      domain);
...
MonoVTable *mono_arch_find_static_call_vtable(host_mgreg_t *regs,
                                              guint8 *code) {
...
MonoInst *mono_arch_emit_inst_for_method(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  if (cmethod->klass == mono_class_try_get_math_class()) {
    ...
    if ((mini_get_cpu_features(cfg) & MONO_CPU_X86_SSE41) != 0 &&
        fsig->param_count == 1 && fsig->params[0]->type == MONO_TYPE_R8) {
      int mode = -1;
...
void mono_arch_context_set_int_reg(MonoContext *ctx, int reg,
                                   host_mgreg_t val) {
  ctx->gregs[reg] = val;
}
 ...
 */
guint8 *mono_arch_emit_load_aotconst(guint8 *start, guint8 *code,
                                     MonoJumpInfo **ji,
 ...
 *
 *   Return a list of MonoTrampInfo structures describing arch specific
 * trampolines for AOT.
 */
...
void mono_arch_set_breakpoint(MonoJitInfo *ji, guint8 *ip) {
  ...
  if (ji->from_aot) {
    ...
    SeqPointInfo *info = mono_arch_get_seq_point_info(mono_domain_get(),
                                                      (guint8 *)ji->code_start);
 ...
 */
void mono_arch_stop_single_stepping(void) { ss_trampoline = NULL; }
 ...
 * we resume, the instruction is not executed again.
 */
void mono_arch_skip_single_step(MonoContext *ctx) { g_assert_not_reached(); }
...
SeqPointInfo *mono_arch_get_seq_point_info(MonoDomain *domain, guint8 *code) {
  ...
  info = (SeqPointInfo *)g_hash_table_lookup(
      domain_jit_info(domain)->arch_seq_points, code);
  ...
  if (!info) {
    ...
    info = (SeqPointInfo *)g_malloc0(sizeof(SeqPointInfo) +
                                     (ji->code_size * sizeof(gpointer)));
...
gpointer mono_arch_load_function(MonoJitICallId jit_icall_id) {
  gpointer target = NULL;
  switch (jit_icall_id) {
#undef MONO_AOT_ICALL
#define MONO_AOT_ICALL(x)                                                      \
  case MONO_JIT_ICALL_##x:                                                     \
    target = (gpointer)x;                                                      \
    break;
