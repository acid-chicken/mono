...
typedef struct {
  ...
  /* Maps methods to a RuntimeInvokeInfo structure, protected by the associated
   * MonoDomain lock */
...
struct MonoJitTlsData {
  ...
  /* handler block been guarded. It's safe to store this even for dynamic
  methods since there is an activation on stack making sure it will remain
  alive.*/
...
typedef struct {
  struct MonoLMF lmf;
  int kind;
  MonoContext ctx;           /* valid if kind == DEBUGGER_INVOKE || kind ==
                                INTERP_EXIT_WITH_CTX */
  gpointer interp_exit_data; /* valid if kind == INTERP_EXIT || kind ==
                                INTERP_EXIT_WITH_CTX */
#if defined(_MSC_VER)
...
typedef struct MonoDebugOptions {
  ...
  /*
   * With LLVM codegen, this option will cause methods to be called indirectly
   * through the PLT (As they are in other FullAOT modes, without LLVM).
   *
  ...
  /*
   * Setting single_imm_size should guarantee that each time managed code is
   * compiled the same instructions and registers are used, regardless of the
   * size of used values.
   */
  ...
  /*
   * Whenever to implement single stepping and breakpoints without signals in
   * the soft debugger. This is useful on platforms without signals, like the
   * ps3, or during runtime debugging, since it avoids SIGSEGVs when a single
   * step location or breakpoint is hit.
   */
  gboolean soft_breakpoints;
  /*
   * Whenever to break in the debugger using G_BREAKPOINT on unhandled
   * exceptions.
   */
  ...
  /*
   * Disabling the frame pointer emit optimization can allow debuggers to more
   * easily identify the stack on some platforms
   */
  ...
  /*
   * Don't enforce any memory model. We will assume the architecture's memory
   * model.
   */
...
This subsume, and will eventually sunset, mono_aot_only / mono_llvm_only and
friends. The goal is to transition us to a place were we can more easily
compose/describe what features we need for a given execution mode.

A good feature flag is checked alone, a bad one described many things and keeps
breaking some of the modes
*/
typedef struct {
  /*
   * If true, trampolines are to be fetched from the AOT runtime instead of JIT
   * compiled
   */
  gboolean use_aot_trampolines;

  /*
   * If true, the runtime will try to use the interpreter before looking for
   * compiled code.
   */
...
static inline MonoMethod *jinfo_get_method(MonoJitInfo *ji) {
...
MONO_API char *mono_parse_options_from(const char *options, int *ref_argc,
                                       char **ref_argv[]);
MONO_API int mono_regression_test_step(int verbose_level, const char *image,
...
extern const MonoEECallbacks *mono_interp_callbacks_pointer;
...
MONO_API void mono_install_ftnptr_eh_callback(MonoFtnPtrEHCallback callback);
...
MONO_LLVM_INTERNAL gconstpointer
...
MONO_LLVM_INTERNAL gint mono_patch_info_equal(gconstpointer ka,
                                              gconstpointer kb);
...
MonoJumpInfoToken *mono_jump_info_token_new2(MonoMemPool *mp, MonoImage *image,
                                             guint32 token,
                                             MonoGenericContext *context);
MONO_LLVM_INTERNAL gpointer mono_resolve_patch_target(
    MonoMethod *method, MonoDomain *domain, guint8 *code,
...
MONO_LLVM_INTERNAL gpointer mono_jit_compile_method(MonoMethod *method,
                                                    MonoError *error);
...
guint32 mono_get_optimizations_for_method(MonoMethod *method,
                                          guint32 default_opt);
...
#define MONO_RETURN_ADDRESS_N(N)                                               \
