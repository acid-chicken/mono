...
// XXX This is dirty, extend ee.h to support extracting info from
...
EMSCRIPTEN_KEEPALIVE int mono_wasm_set_breakpoint(const char *assembly_name,
                                                  int method_token,
                                                  int il_offset);
...
EMSCRIPTEN_KEEPALIVE void mono_wasm_get_array_value_expanded(int object_id,
                                                             int idx);

// JS functions imported that we use
extern void mono_wasm_add_frame(int il_offset, int method_token,
...
extern void mono_wasm_add_typed_value(const char *type, const char *str_value,
                                      double value);

G_END_DECLS

static void describe_object_properties_for_klass(void *obj, MonoClass *klass,
                                                 gboolean isAsyncLocalThis,
                                                 gboolean expandValueType);
...
static const char *all_getters_allowed_class_names[] = {
    "System.DateTime", "System.DateTimeOffset", "System.TimeSpan"};

static const char *to_string_as_descr_names[] = {
    "System.DateTime", "System.DateTimeOffset", "System.Decimal",
    "System.TimeSpan"};

#define THREAD_TO_INTERNAL(thread) (thread)->internal_thread

static void inplace_tolower(char *c) {
  int i;
...
static void free_frame(DbgEngineStackFrame *frame) { g_free(frame); }

static gboolean collect_frames(MonoStackFrameInfo *info, MonoContext *ctx,
                               gpointer data) {
  SeqPoint sp;
  MonoMethod *method;
  ...
  if (!mono_find_prev_seq_point_for_native_offset(
          mono_get_root_domain(), method, info->native_offset, NULL, &sp))
...
static void free_frame_state(void) {
  if (frames) {
    int i;
...
static void compute_frames(void) {
  if (frames) {
    int i;
    for (i = 0; i < frames->len; ++i)
      free_frame((DbgEngineStackFrame *)g_ptr_array_index(frames, i));
...
static MonoContext *tls_get_restore_state(void *tls) { return NULL; }
...
static void ss_discard_frame_context(void *the_tls) { free_frame_state(); }

static void ss_calculate_framecount(void *tls, MonoContext *ctx,
                                    gboolean force_use_ctx,
                                    DbgEngineStackFrame ***out_frames,
                                    int *nframes) {
  compute_frames();
  if (out_frames)
...
static gboolean ensure_jit(DbgEngineStackFrame *the_frame) { return TRUE; }

static int ensure_runtime_is_suspended(void) { return DE_ERR_NONE; }

static int get_this_async_id(DbgEngineStackFrame *f) {
...
static void *create_breakpoint_events(GPtrArray *ss_reqs, GPtrArray *bp_reqs,
  ...
  if ((ss_reqs && ss_reqs->len) || (bp_reqs && bp_reqs->len)) {
    BpEvents *evts =
        g_new0(BpEvents, 1); // just a non-null value to make sure we can raise
                             // it on process_breakpoint_events
...
static void process_breakpoint_events(void *_evts, MonoMethod *method,
                                      MonoContext *ctx, int il_offsets) {
...
static int ss_create_init_args(SingleStepReq *ss_req, SingleStepArgs *ss_args) {
  ...
  DbgEngineStackFrame *frame =
      (DbgEngineStackFrame *)g_ptr_array_index(frames, 0);
  ss_req->start_method = ss_args->method = frame->method;
  gboolean found_sp = mono_find_prev_seq_point_for_native_offset(
...
MONO_API void mono_wasm_enable_debugging(int debug_level) {
...
EMSCRIPTEN_KEEPALIVE int mono_wasm_setup_single_step(int kind) {
  ...
  EventRequest *req = (EventRequest *)g_malloc0(
      sizeof(EventRequest) + (nmodifiers * sizeof(Modifier)));
  ...
  StepFilter filter = (StepFilter)(
      STEP_FILTER_STATIC_CTOR | STEP_FILTER_DEBUGGER_HIDDEN |
      STEP_FILTER_DEBUGGER_STEP_THROUGH | STEP_FILTER_DEBUGGER_NON_USER_CODE);
  ...
  DbgEngineErrorCode err = mono_de_ss_create(
      THREAD_TO_INTERNAL(mono_thread_current()), size, depth, filter, req);
  if (err != DE_ERR_NONE) {
  ...
  if (ss_req && ss_req->bps) {
    ...
    for (l = ss_req->bps; l; l = l->next) {
      if (((MonoBreakpoint *)l->data)->method->wrapper_type !=
...
EMSCRIPTEN_KEEPALIVE int mono_wasm_set_breakpoint(const char *assembly_name,
  ...
  // we get 'foo.dll' but mono_assembly_load expects 'foo' so we strip the last
  // dot
  ...
  mono_assembly_request_prepare_byname(
      &byname_req, MONO_ASMCTX_DEFAULT,
      mono_domain_default_alc(mono_get_root_domain()));
  MonoAssembly *assembly =
      mono_assembly_request_byname(aname, &byname_req, &status);
  g_free(lookup_name);
  if (!assembly) {
  ...
  MonoMethod *method = mono_get_method_checked(
      assembly->image, MONO_TOKEN_METHOD_DEF | method_token, NULL, NULL, error);
  if (!method) {
  ...
  // BreakPointRequest *req = breakpoint_request_new (assembly, method,
  // il_offset);
...
EMSCRIPTEN_KEEPALIVE int mono_wasm_current_bp_id(void) {
  ...
  if (bp_reqs->len > 1)
    DEBUG_PRINTF(1,
                 "Multiple breakpoints (%d) at the same location, returning "
                 "the first one.",
                 bp_reqs->len);
...
static gboolean list_frames(MonoStackFrameInfo *info, MonoContext *ctx,
                            gpointer data) {
  SeqPoint sp;
  MonoMethod *method;
  ...
  if (method->wrapper_type == MONO_WRAPPER_NONE) {
    DEBUG_PRINTF(2, "adding off %d token %d assembly name %s\n", sp.il_offset,
                 mono_metadata_token_index(method->token), assembly_name);
    mono_wasm_add_frame(sp.il_offset, mono_metadata_token_index(method->token),
                        assembly_name, method_full_name);
  }
...
static char *invoke_to_string(const char *class_name, MonoClass *klass,
                              gpointer addr) {
  MonoObject *exc;
  MonoString *mstr;
  char *ret_str;
  ...
  if (m_class_is_valuetype(klass)) {
    ...
    MonoString *mstr =
        (MonoString *)mono_runtime_try_invoke(method, addr, NULL, &exc, error);
    if (exc || !is_ok(error)) {
...
static char *get_to_string_description(const char *class_name, MonoClass *klass,
                                       gpointer addr) {
 ...
 */
static char *mono_method_to_desc_for_js(MonoMethod *method,
  ...
  char *sig_desc =
      g_strdup_printf("%s:%s:%s", ret_desc, args_desc, method->name);
...
static gboolean describe_value(MonoType *type, gpointer addr,
    ...
    if (m_class_is_enumtype(klass)) {
      ...
      while ((field = mono_class_get_fields_internal(klass, &iter))) {
        if (strcmp("value__", mono_field_get_name(field)) == 0) {
          value__ = read_enum_value(mono_vtype_get_field_addr(addr, field),
                                    base_type);
          continue;
        }
        ...
        g_string_append_printf(enum_members, ",%s:%llu",
                               mono_field_get_name(field), field_value);
      }
      ...
      describe_object_properties_for_klass((MonoObject *)addr, klass, FALSE,
                                           expandValueType);
  ...
  default: {
    ...
    char *msg = g_strdup_printf("can't handle type %s [%p, %x]", type_name,
                                type, type->type);
...
static gboolean are_getters_allowed(const char *class_name) {
...
static void describe_object_properties_for_klass(void *obj, MonoClass *klass,
  ...
  while ((p = mono_class_get_properties(klass, &iter))) {
    DEBUG_PRINTF(2, "mono_class_get_properties - %s - %s\n", p->name,
                 p->get->name);
    if (p->get->name) { // if get doesn't have name means that doesn't have a
                        // getter implemented and we don't want to show value,
                        // like VS debug
      MonoObject *res;
      MonoObject *exc;

      if (isAsyncLocalThis &&
          (p->name[0] != '<' || (p->name[0] == '<' && p->name[1] == '>')))
        continue;
      ...
      if (!getters_allowed) {
        ...
        char *ret_class_name =
            mono_class_full_name(mono_class_from_mono_type_internal(sig->ret));
      ...
      if (is_valuetype &&
          mono_class_from_mono_type_internal(sig->ret) == klass) {
 ...
 */
static gboolean describe_delegate_properties(MonoObject *obj) {
...
static gboolean describe_object_properties(guint64 objectId,
                                           gboolean isAsyncLocalThis,
                                           gboolean expandValueType) {
  DEBUG_PRINTF(2, "describe_object_properties %llu\n", objectId);
  ObjRef *ref =
      (ObjRef *)g_hash_table_lookup(objrefs, GINT_TO_POINTER(objectId));
  if (!ref) {
  ...
  if (m_class_is_delegate(mono_object_class(obj))) {
    ...
    describe_object_properties_for_klass(obj, obj->vtable->klass,
                                         isAsyncLocalThis, expandValueType);
  }

  return TRUE;
}

static gboolean describe_array_values(guint64 objectId) {
  int esize;
  gpointer elem;
  ObjRef *ref =
      (ObjRef *)g_hash_table_lookup(objrefs, GINT_TO_POINTER(objectId));
  ...
  for (int i = 0; i < arr->max_length; i++) {
    ...
    describe_value(m_class_get_byval_arg(
...
/* Expands valuetypes */
static gboolean describe_array_value_expanded(guint64 objectId, guint64 idx) {
  int esize;
  gpointer elem;
  ObjRef *ref =
      (ObjRef *)g_hash_table_lookup(objrefs, GINT_TO_POINTER(objectId));
  ...
  describe_value(
      m_class_get_byval_arg(m_class_get_element_class(arr->obj.vtable->klass)),
      elem, TRUE);
...
static void describe_async_method_locals(InterpFrame *frame,
                                         MonoMethod *method) {
  // Async methods are special in the way that local variables can be lifted to
...
static void describe_non_async_this(InterpFrame *frame, MonoMethod *method) {
  gpointer addr = NULL;
  if (mono_debug_lookup_method_async_debug_info(method))
    return;
...
static gboolean describe_variable(InterpFrame *frame, MonoMethod *method,
                                  int pos, gboolean expandValueType) {
  ERROR_DECL(error);
...
static gboolean describe_variables_on_frame(MonoStackFrameInfo *info,
                                            MonoContext *ctx, gpointer ud) {
...
EMSCRIPTEN_KEEPALIVE void mono_wasm_get_var_info(int scope, int *pos, int len) {
  ...
  mono_walk_stack_with_ctx(describe_variables_on_frame, NULL, MONO_UNWIND_NONE,
                           &data);
}
...
gsize mono_debugger_tls_thread_id(DebuggerTlsData *debuggerTlsData) {
  return 1;
}
