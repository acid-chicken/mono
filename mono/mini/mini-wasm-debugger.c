...
MONO_API void mono_wasm_enable_debugging(void) {
...
static int get_object_id(MonoObject *obj) {
  ObjRef *ref = (ObjRef *)g_hash_table_lookup(obj_to_objref,
...
static gboolean describe_value(MonoType *type, gpointer addr) {
  ...
  case MONO_TYPE_CLASS: {
    ...
    if (!obj) {
      ...
      if (m_class_get_byval_arg(obj->vtable->klass)->type ==
              MONO_TYPE_SZARRAY ||
          m_class_get_byval_arg(obj->vtable->klass)->type == MONO_TYPE_ARRAY)
...
static gboolean describe_object_properties(guint64 objectId,
  ...
  while (obj &&
         (f = mono_class_get_fields_internal(obj->vtable->klass, &iter))) {
    DEBUG_PRINTF(2, "mono_class_get_fields_internal - %s - %x\n", f->name,
                 f->type->type);
    if (isAsyncLocalThis &&
        (f->name[0] != '<' || (f->name[0] == '<' && f->name[1] == '>'))) {
      continue;
    }
...
static gboolean describe_async_method_locals(MonoStackFrameInfo *info,
...
static gboolean describe_this(MonoStackFrameInfo *info, MonoContext *ctx,
                              gpointer ud) {
  // Async methods are special in the way that local variables can be lifted to
  // generated class fields
  FrameDescData *data = (FrameDescData *)ud;
...
static gboolean describe_variable(MonoStackFrameInfo *info, MonoContext *ctx,
                                  gpointer ud) {
  ERROR_DECL(error);
...
EMSCRIPTEN_KEEPALIVE void mono_wasm_get_var_info(int scope, int *pos, int len) {
  ...
  mono_walk_stack_with_ctx(describe_async_method_locals, NULL, MONO_UNWIND_NONE,
                           &data);
