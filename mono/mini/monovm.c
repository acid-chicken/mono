...
static void mono_core_lookup_paths_free(MonoCoreLookupPaths *dl) {
  if (!dl)
    return;
  g_strfreev(dl->dirs);
  g_free(dl);
...
static MonoCoreLookupPaths *parse_lookup_paths(const char *search_path) {
...
mono_core_preload_hook(MonoAssemblyLoadContext *alc, MonoAssemblyName *aname,
  ...
  for (int i = 0; i < a->assembly_count; ++i) {
    if (basename_len == a->basename_lens[i] &&
        !g_strncasecmp(basename, a->basenames[i], a->basename_lens[i])) {
...
static gboolean parse_properties(int propertyCount, const char **propertyKeys,
  ...
  for (int i = 0; i < propertyCount; ++i) {
    ...
    if (prop_len == 27 &&
        !strncmp(propertyKeys[i], "TRUSTED_PLATFORM_ASSEMBLIES", 27)) {
      ...
      app_paths = parse_lookup_paths(propertyValues[i]);
               ...
               !strncmp(propertyKeys[i], "APP_NI_PATHS", 12)) {
      app_ni_paths = parse_lookup_paths(propertyValues[i]);
    } else if (prop_len == 23 &&
               !strncmp(propertyKeys[i], "PLATFORM_RESOURCE_ROOTS", 23)) {
      platform_resource_roots = parse_lookup_paths(propertyValues[i]);
    } else if (prop_len == 29 &&
               !strncmp(propertyKeys[i], "NATIVE_DLL_SEARCH_DIRECTORIES", 29)) {
      native_lib_paths = parse_lookup_paths(propertyValues[i]);
    } else if (prop_len == 30 &&
               !strncmp(propertyKeys[i], "System.Globalization.Invariant",
                        30)) {
...
int monovm_initialize(int propertyCount, const char **propertyKeys,
  ...
  // Our load hooks don't distinguish between normal, AOT'd, and satellite
  // lookups the way CoreCLR's does. For now, just set assemblies_path with
  // APP_PATHS and leave the rest.
  if (app_paths != NULL)
...
int monovm_execute_assembly(int argc, const char **argv,
...
int monovm_shutdown(int *latchedExitCode) { return -1; }
