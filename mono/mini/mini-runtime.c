...
/* If this is set, no code is generated dynamically, everything is taken from
 * AOT files */
...
/*
 * This flag controls whenever the runtime uses LLVM for JIT compilation, and
 * whenever it can load AOT code compiled by LLVM.
 */
...
static void find_tramp(gpointer key, gpointer value, gpointer user_data) {
...
char *mono_get_method_from_ip(void *ip) {
  ...
  if (!ji) {
    ...
    g_hash_table_foreach(domain_jit_info(domain)->jit_trampoline_hash,
                         find_tramp, &user_data);
  ...
  method_name =
      mono_method_get_name_full(method, TRUE, FALSE, MONO_TYPE_NAME_FORMAT_IL);
  location = mono_debug_lookup_source_location(
      method, (guint32)((guint8 *)ip - (guint8 *)ji->code_start), domain);

  char *file_loc = NULL;
  if (location)
    file_loc =
        g_strdup_printf("[%s :: %du]", location->source_file, location->row);

  const char *in_interp = ji->is_interp ? " interp" : "";

  res = g_strdup_printf(" %s [{%p} + 0x%x%s] %s (%p %p) [%p - %s]", method_name,
                        method, (int)((char *)ip - (char *)ji->code_start),
                        in_interp, file_loc ? file_loc : "", ji->code_start,
                        (char *)ji->code_start + ji->code_size, domain,
 ...
 */
G_GNUC_UNUSED char *mono_pmip(void *ip) { return mono_get_method_from_ip(ip); }
...
MONO_ATTR_USED void mono_print_method_from_ip(void *ip) {
  ...
  if (ji && ji->is_trampoline) {
    ...
    printf("IP %p is at offset 0x%x of trampoline '%s'.\n", ip,
           (int)((guint8 *)ip - tinfo->code), tinfo->name);
    return;
  }
  ...
  source = mono_debug_lookup_source_location(
      jinfo_get_method(ji), (guint32)((guint8 *)ip - (guint8 *)ji->code_start),
      target_domain);
  ...
  g_print("IP %p at offset 0x%x of %smethod %s (%p %p)[domain %p - %s]\n", ip,
          (int)((char *)ip - (char *)ji->code_start), shared_type, method,
...
void *(mono_global_codeman_reserve)(int size) {
  ...
  if (mono_aot_only)
    g_error("Attempting to allocate from the global code manager while running "
            "in aot-only mode.\n");
 ...
 */
MonoUnwindOp *mono_create_unwind_op(int when, int tag, int reg, int val) {
...
MonoJumpInfoToken *mono_jump_info_token_new2(MonoMemPool *mp, MonoImage *image,
                                             guint32 token,
                                             MonoGenericContext *context) {
  MonoJumpInfoToken *res =
      (MonoJumpInfoToken *)mono_mempool_alloc0(mp, sizeof(MonoJumpInfoToken));
  res->image = image;
  res->token = token;
...
MonoJumpInfoToken *mono_jump_info_token_new(MonoMemPool *mp, MonoImage *image,
                                            guint32 token) {
 ...
 *
 *   Create a MonoTrampInfo structure from the arguments. This function assumes
 * ownership of JI, and UNWIND_OPS.
 */
MonoTrampInfo *mono_tramp_info_create(const char *name, guint8 *code,
                                      guint32 code_size, MonoJumpInfo *ji,
                                      GSList *unwind_ops) {
...
static void register_trampoline_jit_info(MonoDomain *domain,
                                         MonoTrampInfo *info) {
  MonoJitInfo *ji;

  ji = (MonoJitInfo *)mono_domain_alloc0(
      domain, mono_jit_info_size((MonoJitInfoFlags)0, 0, 0));
  mono_jit_info_init(ji, NULL, info->code, info->code_size, (MonoJitInfoFlags)0,
                     0, 0);
 ...
 * Frees INFO.
 */
static void mono_tramp_info_register_internal(MonoTrampInfo *info,
                                              MonoDomain *domain,
                                              gboolean aot) {
  MonoTrampInfo *copy;
  ...
  if (info->unwind_ops) {
    copy->uw_info =
        mono_unwind_ops_encode(info->unwind_ops, &copy->uw_info_len);
    copy->owns_uw_info = TRUE;
    if (domain) {
      /* Move unwind info into the domain's memory pool so that it is removed
  ...
  if (!aot)
    mono_arch_unwindinfo_install_tramp_unwind_info(info->unwind_ops, info->code,
                                                   info->code_size);
#endif
...
static void mono_tramp_info_cleanup(void) {
  GSList *l;
...
/* Register trampolines created before the root domain was created in the jit
 * info tables */
static void register_trampolines(MonoDomain *domain) {
  GSList *l;
...
G_GNUC_UNUSED static void break_count(void) {}

/*
 * Runtime debugging tool, use if (debug_count ()) <x> else <y> to do <x> the
 * first COUNT times, then do <y> afterwards. Set a breakpoint in break_count ()
 * to break the last time <x> is done.
 */
G_GNUC_UNUSED gboolean mono_debug_count(void) {
...
gconstpointer mono_icall_get_wrapper_full(MonoJitICallInfo *callinfo,
                                          gboolean do_compile) {
  ERROR_DECL(error);
  MonoMethod *wrapper;
...
mono_dynamic_code_hash_lookup(MonoDomain *domain, MonoMethod *method) {
  ...
  if (domain_jit_info(domain)->dynamic_code_hash)
    res = (MonoJitDynamicMethodInfo *)g_hash_table_lookup(
        domain_jit_info(domain)->dynamic_code_hash, method);
  else
    res = NULL;
  return res;
}
...
template <typename T>
static void
register_opcode_emulation(int opcode, MonoJitICallInfo *jit_icall_info,
                          const char *name, MonoMethodSignature *sig, T func,
                          const char *symbol, gboolean no_wrapper)
#else
static void register_opcode_emulation(int opcode,
                                      MonoJitICallInfo *jit_icall_info,
                                      const char *name,
                                      MonoMethodSignature *sig, gpointer func,
                                      const char *symbol, gboolean no_wrapper)
#endif
{
#ifndef DISABLE_JIT
  mini_register_opcode_emulation(opcode, jit_icall_info, name, sig, func,
                                 symbol, no_wrapper);
#else
  ...
  mono_register_jit_icall_info(jit_icall_info, func, name, sig, no_wrapper,
                               symbol);
#endif
}

#define register_opcode_emulation(opcode, name, sig, func, no_wrapper)         \
  (register_opcode_emulation((opcode), &mono_get_jit_icall_info()->name,       \
                             #name, (sig), func, #func, (no_wrapper)))

/*
 * For JIT icalls implemented in C.
 * NAME should be the same as the name of the C function whose address is FUNC.
 * If @avoid_wrapper is TRUE, no wrapper is generated. This is for perf critical
 * icalls which can't throw exceptions.
 *
 * func is an identifier, that names a function, and is also in jit-icall-reg.h,
 * and therefore a field in mono_jit_icall_info and can be token pasted into an
 * enum value.
 *
 * The name of func must be linkable for AOT, for example g_free does not work
 * (monoeg_g_free instead), nor does the C++ overload fmod (mono_fmod instead).
 * These functions therefore must be extern "C".
 */
#define register_icall(func, sig, avoid_wrapper)                               \
  (mono_register_jit_icall_info(&mono_get_jit_icall_info()->func, func, #func, \
                                (sig), (avoid_wrapper), #func))
 ...
 *
 * func is an expression, such a local variable or a function call to get a
 * function pointer. name is an identifier
 *
 * Providing func and name separately is what distinguishes "dyn" from regular.
 *
 * This also passes last parameter c_symbol=NULL since there is not a directly
 * linkable symbol.
 */
#define register_dyn_icall(func, name, sig, save)                              \
  (mono_register_jit_icall_info(&mono_get_jit_icall_info()->name, (func),      \
                                #name, (sig), (save), NULL))
...
void mono_set_lmf(MonoLMF *lmf) { (*mono_get_lmf_addr()) = lmf; }

static void mono_set_jit_tls(MonoJitTlsData *jit_tls) {
  MonoThreadInfo *info;

  mono_tls_set_jit_tls(jit_tls);

  /* Save it into MonoThreadInfo so it can be accessed by
...
static void mono_set_lmf_addr(MonoLMF **lmf_addr) {
  MonoThreadInfo *info;
 ...
 *
 * This function is external only and @deprecated don't use it.  Use
...
MonoDomain *mono_jit_thread_attach(MonoDomain *domain) {
  ...
  if (!attached) {
    ...
    // #678164
    mono_thread_set_state(mono_thread_internal_current(),
 ...
 */
static void mono_thread_abort(MonoObject *obj) {
...
static MonoJitTlsData *setup_jit_tls_data(gpointer stack_start,
...
static void free_jit_tls_data(MonoJitTlsData *jit_tls) {
...
static void mono_thread_start_cb(intptr_t tid, gpointer stack_start,
                                 gpointer func) {
...
static void mono_thread_abort_dummy(MonoObject *obj) {
...
static void mono_thread_attach_cb(intptr_t tid, gpointer stack_start) {
  MonoThreadInfo *thread;
  MonoJitTlsData *jit_tls =
...
static void mini_thread_cleanup(MonoNativeThreadId tid) {
  MonoJitTlsData *jit_tls = NULL;
  MonoThreadInfo *info;

  info = mono_thread_info_current_unchecked();

  /* We can't clean up tls information if we are on another thread, it will
   * clean up the wrong stuff It would be nice to issue a warning when this
   * happens outside of the shutdown sequence. but it's not a trivial thing.
   *
   * The current offender is mono_thread_manage which cleanup threads from the
   * outside.
   */
  if (info && mono_thread_info_get_tid(info) == tid) {
    ...
    /* If we attach a thread but never call into managed land, we might never
     * get an lmf.*/
    if (mono_get_lmf()) {
      mono_set_lmf(NULL);
...
MonoJumpInfo *mono_patch_info_list_prepend(MonoJumpInfo *list, int ip,
...
const char *mono_ji_type_to_string(MonoJumpInfoType type) { return ""; }
 ...
 */
MonoJumpInfo *mono_patch_info_dup_mp(MonoMemPool *mp,
  ...
  case MONO_PATCH_INFO_DECLSEC:
    res->data.token =
        (MonoJumpInfoToken *)mono_mempool_alloc(mp, sizeof(MonoJumpInfoToken));
    memcpy(res->data.token, patch_info->data.token, sizeof(MonoJumpInfoToken));
    break;
  case MONO_PATCH_INFO_SWITCH:
    res->data.table = (MonoJumpInfoBBTable *)mono_mempool_alloc(
        mp, sizeof(MonoJumpInfoBBTable));
    memcpy(res->data.table, patch_info->data.table,
           sizeof(MonoJumpInfoBBTable));
    res->data.table->table = (MonoBasicBlock **)mono_mempool_alloc(
        mp, sizeof(MonoBasicBlock *) * patch_info->data.table->table_size);
    memcpy(res->data.table->table, patch_info->data.table->table,
           sizeof(MonoBasicBlock *) * patch_info->data.table->table_size);
    break;
  case MONO_PATCH_INFO_RGCTX_FETCH:
  case MONO_PATCH_INFO_RGCTX_SLOT_INDEX:
    res->data.rgctx_entry = (MonoJumpInfoRgctxEntry *)mono_mempool_alloc(
        mp, sizeof(MonoJumpInfoRgctxEntry));
    memcpy(res->data.rgctx_entry, patch_info->data.rgctx_entry,
           sizeof(MonoJumpInfoRgctxEntry));
    res->data.rgctx_entry->data =
        mono_patch_info_dup_mp(mp, res->data.rgctx_entry->data);
    break;
  case MONO_PATCH_INFO_DELEGATE_TRAMPOLINE:
    res->data.del_tramp = (MonoDelegateClassMethodPair *)mono_mempool_alloc0(
        mp, sizeof(MonoDelegateClassMethodPair));
    memcpy(res->data.del_tramp, patch_info->data.del_tramp,
           sizeof(MonoDelegateClassMethodPair));
    break;
  case MONO_PATCH_INFO_GSHAREDVT_CALL:
    res->data.gsharedvt = (MonoJumpInfoGSharedVtCall *)mono_mempool_alloc(
        mp, sizeof(MonoJumpInfoGSharedVtCall));
    memcpy(res->data.gsharedvt, patch_info->data.gsharedvt,
  ...
  case MONO_PATCH_INFO_GSHAREDVT_METHOD: {
    ...
    info = (MonoGSharedVtMethodInfo *)mono_mempool_alloc(
        mp, sizeof(MonoGSharedVtMethodInfo));
    res->data.gsharedvt_method = info;
    memcpy(info, oinfo, sizeof(MonoGSharedVtMethodInfo));
    info->entries = (MonoRuntimeGenericContextInfoTemplate *)mono_mempool_alloc(
        mp,
    ...
    for (i = 0; i < oinfo->num_entries; ++i) {
      ...
      memcpy(template_, otemplate,
             sizeof(MonoRuntimeGenericContextInfoTemplate));
    }
    // info->locals_types = mono_mempool_alloc0 (mp, info->nlocals * sizeof
    // (MonoType*)); memcpy (info->locals_types, oinfo->locals_types,
    // info->nlocals * sizeof (MonoType*));
    break;
  }
  case MONO_PATCH_INFO_VIRT_METHOD: {
    ...
    info = (MonoJumpInfoVirtMethod *)mono_mempool_alloc0(
        mp, sizeof(MonoJumpInfoVirtMethod));
...
guint mono_patch_info_hash(gconstpointer data) {
  ...
  case MONO_PATCH_INFO_TYPE_FROM_HANDLE:
    return hash | ji->data.token->token |
           (ji->data.token->has_context
                ? (gsize)ji->data.token->context.class_inst
                : 0);
  ...
  case MONO_PATCH_INFO_DELEGATE_TRAMPOLINE:
    return hash | (gsize)ji->data.del_tramp->klass |
           (gsize)ji->data.del_tramp->method |
           (gsize)ji->data.del_tramp->is_virtual;
  case MONO_PATCH_INFO_VIRT_METHOD: {
...
gint mono_patch_info_equal(gconstpointer ka, gconstpointer kb) {
  ...
  case MONO_PATCH_INFO_DECLSEC:
    return ji1->data.token->image == ji2->data.token->image &&
           ...
           ji1->data.token->context.class_inst ==
               ji2->data.token->context.class_inst &&
           ji1->data.token->context.method_inst ==
  ...
  case MONO_PATCH_INFO_RGCTX_SLOT_INDEX: {
    ...
    return e1->d.method == e2->d.method && e1->d.klass == e2->d.klass &&
           e1->in_mrgctx == e2->in_mrgctx && e1->info_type == e2->info_type &&
           mono_patch_info_equal(e1->data, e2->data);
  }
  case MONO_PATCH_INFO_GSHAREDVT_CALL: {
  ...
  case MONO_PATCH_INFO_GSHAREDVT_METHOD:
    return ji1->data.gsharedvt_method->method ==
           ji2->data.gsharedvt_method->method;
  case MONO_PATCH_INFO_DELEGATE_TRAMPOLINE:
    return ji1->data.del_tramp->klass == ji2->data.del_tramp->klass &&
           ji1->data.del_tramp->method == ji2->data.del_tramp->method &&
           ji1->data.del_tramp->is_virtual == ji2->data.del_tramp->is_virtual;
  case MONO_PATCH_INFO_SPECIFIC_TRAMPOLINE_LAZY_FETCH_ADDR:
  ...
  case MONO_PATCH_INFO_VIRT_METHOD:
    return ji1->data.virt_method->klass == ji2->data.virt_method->klass &&
           ji1->data.virt_method->method == ji2->data.virt_method->method;
  case MONO_PATCH_INFO_GSHAREDVT_IN_WRAPPER:
...
gpointer mono_resolve_patch_target(MonoMethod *method, MonoDomain *domain,
                                   guint8 *code, MonoJumpInfo *patch_info,
                                   gboolean run_cctors, MonoError *error) {
  ...
  case MONO_PATCH_INFO_METHOD:
    if (patch_info->data.method == method) {
      ...
      target =
          mono_create_jit_trampoline(domain, patch_info->data.method, error);
      if (!is_ok(error))
        return NULL;
    }
    break;
  case MONO_PATCH_INFO_METHOD_FTNDESC: {
     ...
     */
    target = mini_llvmonly_load_method_ftndesc(patch_info->data.method, FALSE,
                                               FALSE, error);
  ...
  case MONO_PATCH_INFO_METHOD_CODE_SLOT: {
    ...
    code_slot = g_hash_table_lookup(domain_jit_info(domain)->method_code_hash,
                                    patch_info->data.method);
    if (!code_slot) {
      code_slot = mono_domain_alloc0(domain, sizeof(gpointer));
      g_hash_table_insert(domain_jit_info(domain)->method_code_hash,
                          patch_info->data.method, code_slot);
    }
    mono_domain_unlock(domain);
    target = code_slot;
    break;
  ...
  case MONO_PATCH_INFO_SWITCH: {
    ...
    if (method && method->dynamic) {
      jump_table = (void **)mono_code_manager_reserve(
          mono_dynamic_code_hash_lookup(domain, method)->code_mp,
          sizeof(gpointer) * patch_info->data.table->table_size);
    } else {
      if (mono_aot_only) {
        jump_table = (void **)mono_domain_alloc(
            domain, sizeof(gpointer) * patch_info->data.table->table_size);
      } else {
        jump_table = (void **)mono_domain_code_reserve(
            domain, sizeof(gpointer) * patch_info->data.table->table_size);
      }
    }
  ...
  case MONO_PATCH_INFO_ADJUSTED_IID:
    mono_class_init_internal(patch_info->data.klass);
    target = GUINT_TO_POINTER(
        (guint32)(-((m_class_get_interface_id(patch_info->data.klass) + 1) *
  ...
  case MONO_PATCH_INFO_DELEGATE_TRAMPOLINE: {
    ...
    if (del_tramp->is_virtual)
      target = mono_create_delegate_virtual_trampoline(domain, del_tramp->klass,
                                                       del_tramp->method);
    else
      target = mono_create_delegate_trampoline_info(domain, del_tramp->klass,
                                                    del_tramp->method);
    break;
  }
  case MONO_PATCH_INFO_SFLDA: {
    MonoVTable *vtable = mono_class_vtable_checked(
        domain, patch_info->data.field->parent, error);
    mono_error_assert_ok(error);
    ...
    if (mono_class_field_is_special_static(patch_info->data.field)) {
      ...
      if (domain->special_static_fields)
        addr = g_hash_table_lookup(domain->special_static_fields,
    ...
    if (!vtable->initialized &&
        !mono_class_is_before_field_init(vtable->klass) &&
        (!method || mono_class_needs_cctor_run(vtable->klass, method)))
    ...
    target = (char *)mono_vtable_get_static_field_data(vtable) +
             patch_info->data.field->offset;
    break;
  }
  case MONO_PATCH_INFO_RVA: {
    guint32 field_index =
    ...
    mono_metadata_field_info(patch_info->data.token->image, field_index - 1,
                             NULL, &rva, NULL);
  ...
  case MONO_PATCH_INFO_EXC_NAME:
    ...
    break;
  case MONO_PATCH_INFO_LDSTR:
    target = mono_ldstr_checked(
        domain, patch_info->data.token->image,
  ...
  case MONO_PATCH_INFO_TYPE_FROM_HANDLE: {
    ...
    handle = mono_ldtoken_checked(patch_info->data.token->image,
                                  ...
                                  patch_info->data.token->has_context
                                      ? &patch_info->data.token->context
                                      : NULL,
                                  error);
    if (!is_ok(error))
      return NULL;
  ...
  case MONO_PATCH_INFO_DECLSEC:
    target = (mono_metadata_blob_heap(patch_info->data.token->image,
  ...
  case MONO_PATCH_INFO_ICALL_ADDR_CALL:
    ...
    if (patch_info->data.method->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL) {
      if (run_cctors) {
        target =
            mono_lookup_pinvoke_call_internal(patch_info->data.method, error);
        if (!target) {
          if (mono_aot_only)
            return NULL;
          g_error("Unable to resolve pinvoke method '%s' Re-run with "
                  "MONO_LOG_LEVEL=debug for more information.\n",
  ...
  case MONO_PATCH_INFO_GSHAREDVT_IN_WRAPPER:
    target = mini_get_gsharedvt_wrapper(TRUE, NULL, patch_info->data.sig, NULL,
                                        -1, FALSE);
    break;
  case MONO_PATCH_INFO_PROFILER_ALLOCATION_COUNT: {
 ...
 */
void mini_register_jump_site(MonoDomain *domain, MonoMethod *method,
                             gpointer ip) {
  MonoJumpList *jlist;
  ...
  mono_domain_lock(domain);
  jlist = (MonoJumpList *)g_hash_table_lookup(
      domain_jit_info(domain)->jump_target_hash, method);
  if (!jlist) {
    jlist = (MonoJumpList *)mono_domain_alloc0(domain, sizeof(MonoJumpList));
    g_hash_table_insert(domain_jit_info(domain)->jump_target_hash, method,
                        jlist);
  }
 ...
 */
void mini_patch_jump_sites(MonoDomain *domain, MonoMethod *method,
                           gpointer addr) {
  ...
  if (jlist) {
    ...
    for (tmp = jlist->list; tmp; tmp = tmp->next)
      mono_arch_patch_code_new(NULL, domain, (guint8 *)tmp->data, &patch_info,
                               addr);
#else
    // FIXME: This won't work since it ends up calling
    // mono_create_jump_trampoline () which returns a trampoline for gshared
    // methods
    for (tmp = jlist->list; tmp; tmp = tmp->next) {
      ERROR_DECL(error);
      mono_arch_patch_code(NULL, NULL, domain, tmp->data, &patch_info, TRUE,
                           error);
      mono_error_assert_ok(error);
    }
#endif
 ...
 */
void mini_patch_llvm_jit_callees(MonoDomain *domain, MonoMethod *method,
                                 gpointer addr) {
  if (!domain_jit_info(domain)->llvm_jit_callees)
    return;
  GSList *callees = (GSList *)g_hash_table_lookup(
      domain_jit_info(domain)->llvm_jit_callees, method);
  GSList *l;
...
void mini_init_gsctx(MonoDomain *domain, MonoMemPool *mp,
                     MonoGenericContext *context,
                     MonoGenericSharingContext *gsctx) {
 ...
 */
MonoJitInfo *mini_lookup_method(MonoDomain *domain, MonoMethod *method,
                                MonoMethod *shared) {
  MonoJitInfo *ji;
  ...
  ji = (MonoJitInfo *)mono_internal_hash_table_lookup(&domain->jit_code_hash,
                                                      method);
  if (!ji && shared) {
    /* Try generic sharing */
    ji = (MonoJitInfo *)mono_internal_hash_table_lookup(&domain->jit_code_hash,
    ...
    if (ji && !ji->has_generic_jit_info)
      ji = NULL;
    if (!inited) {
      mono_counters_register("Shared generic lookups",
                             MONO_COUNTER_INT | MONO_COUNTER_GENERICS,
                             &lookups);
      mono_counters_register("Failed shared generic lookups",
                             MONO_COUNTER_INT | MONO_COUNTER_GENERICS,
                             &failed_lookups);
      inited = TRUE;
    }
...
static MonoJitInfo *lookup_method(MonoDomain *domain, MonoMethod *method) {
  ERROR_DECL(error);
  MonoJitInfo *ji;
  MonoMethod *shared;
...
MonoClass *mini_get_class(MonoMethod *method, guint32 token,
                          MonoGenericContext *context) {
  ERROR_DECL(error);
  MonoClass *klass;
  ...
  if (method->wrapper_type != MONO_WRAPPER_NONE) {
    ...
    klass = mono_class_get_and_inflate_typespec_checked(
        m_class_get_image(method->klass), token, context, error);
    mono_error_cleanup(error); /* FIXME don't swallow the error */
  }
  if (klass)
...
gboolean mono_jit_map_is_enabled(void) { return perf_map_file != NULL; }
...
void mono_enable_jit_dump(void) {
  ...
  if (!perf_dump_file) {
    ...
    if (perf_dump_file) {
      ...
      // This informs perf of the presence of the jitdump file and support for
      // the feature.​
      perf_dump_mmap_addr = mmap(NULL, sizeof(header), PROT_READ | PROT_EXEC,
                                 MAP_PRIVATE, fileno(perf_dump_file), 0);
    }
...
void mono_emit_jit_dump(MonoJitInfo *jinfo, gpointer code) {
  ...
  if (perf_dump_file) {
    ...
    // TODO: write debugInfo and unwindInfo immediately before the
    // JitCodeLoadRecord (while lock is held).
...
static void no_gsharedvt_in_wrapper(void) { g_assert_not_reached(); }

/*
Overall algorithm:

When a JIT request is made, we check if there's an outstanding one for that
method and, if it exits, put the thread to sleep. If the current thread is
already JITing another method, don't wait as it might cause a deadlock.
        Dependency management in this case is too complex to justify
implementing it.

If there are no outstanding requests, the current thread is doing nothing and
there are already mono_cpu_count threads JITing, go to sleep.

TODO:
        Get rid of cctor invocations from within the JIT, it increases JIT
duration and complicates things A LOT. Can we get rid of ref_count and use `done
&& threads_waiting == 0` as the equivalent of `ref_count == 0`? Reduce amount of
dynamically allocated - possible once the JIT is no longer reentrant Maybe pool
JitCompilationEntry, specially those with an inited cond var;
*/
typedef struct {
  MonoMethod *method;
  MonoDomain *domain;
  int compilation_count; /* Number of threads compiling this method - This
                            happens due to the JIT being reentrant */
  int ref_count; /* Number of threads using this JitCompilationEntry, roughtly 1
                    + threads_waiting */
...
This value can't be too small or we won't see enough methods being reused and it
can't be too big to cause massive stalls due to unforseable circunstances.
*/
#define MAX_JIT_TIMEOUT_MS 1000

static JitCompilationData compilation_data;
static int jit_methods_waited, jit_methods_multiple, jit_methods_overload,
    jit_spurious_wakeups_or_timeouts;

static void mini_jit_init_job_control(void) {
...
static void unlock_compilation_data(void) {
  mono_coop_mutex_unlock(&compilation_data.lock);
}

static JitCompilationEntry *find_method(MonoMethod *method,
                                        MonoDomain *domain) {
  int i;
  for (i = 0; i < compilation_data.in_flight_methods->len; ++i) {
    JitCompilationEntry *e =
        (JitCompilationEntry *)compilation_data.in_flight_methods->pdata[i];
...
static void add_current_thread(MonoJitTlsData *jit_tls) {
  ++jit_tls->active_jit_methods;
}

static void unref_jit_entry(JitCompilationEntry *entry) {
  --entry->ref_count;
  if (entry->ref_count)
    return;
  if (entry->has_cond)
 ...
 */
static gboolean wait_or_register_method_to_compile(MonoMethod *method,
                                                   MonoDomain *domain) {
  MonoJitTlsData *jit_tls = mono_tls_get_jit_tls();
  ...
  if (!inited) {
    mono_counters_register("JIT compile waited others",
                           MONO_COUNTER_INT | MONO_COUNTER_JIT,
                           &jit_methods_waited);
    mono_counters_register("JIT compile 1+ jobs",
                           MONO_COUNTER_INT | MONO_COUNTER_JIT,
                           &jit_methods_multiple);
    mono_counters_register("JIT compile overload wait",
                           MONO_COUNTER_INT | MONO_COUNTER_JIT,
                           &jit_methods_overload);
    mono_counters_register("JIT compile spurious wakeups or timeouts",
                           MONO_COUNTER_INT | MONO_COUNTER_JIT,
                           &jit_spurious_wakeups_or_timeouts);
    inited = TRUE;
  }
  ...
  if (!(entry = find_method(method, domain))) {
    ...
    return FALSE;
             ...
             mono_threads_is_current_thread_in_protected_block()) {
    // We can't suspend the current thread if it's already JITing a method.
    // Dependency management is too compilated and we want to get rid of this
    // anyways.

    // We can't suspend the current thread if it's running a protected block
    // (such as a cctor) We can't rely only on JIT nesting as cctor's can be run
    // from outside the JIT.

    // Finally, he hit a timeout or spurious wakeup. We're better off just
    // giving up and keep recompiling
    ...
    while (TRUE) {
      ...
      mono_coop_cond_timedwait(&entry->cond, &compilation_data.lock,
                               MAX_JIT_TIMEOUT_MS);
      --entry->threads_waiting;
...
static void unregister_method_for_compile(MonoMethod *method,
...
static MonoJitInfo *create_jit_info_for_trampoline(MonoMethod *wrapper,
                                                   MonoTrampInfo *info) {
  MonoDomain *domain = mono_get_root_domain();
  MonoJitInfo *jinfo;
  guint8 *uw_info;
...
static gpointer compile_special(MonoMethod *method, MonoDomain *target_domain,
                                MonoError *error) {
  MonoJitInfo *jinfo;
  gpointer code;
  ...
  if (mono_llvm_only) {
    if (method->wrapper_type == MONO_WRAPPER_OTHER) {
      ...
      if (info->subtype == WRAPPER_SUBTYPE_GSHAREDVT_IN_SIG) {
        /*
         * These wrappers are only created for signatures which are in the
         * program, but sometimes we load methods too eagerly and have to create
         * them even if they will never be called.
         */
  ...
  if ((method->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) ||
...
#ifdef HOST_WIN32
        g_warning("Method '%s' in assembly '%s' contains native code that "
                  "cannot be executed by Mono in modules loaded from byte "
                  "arrays. The assembly was probably created using C++/CLI.\n",
                  mono_method_full_name(method, TRUE),
                  m_class_get_image(method->klass)->name);
#else
        g_warning("Method '%s' in assembly '%s' contains native code that "
                  "cannot be executed by Mono on this platform. The assembly "
                  "was probably created using C++/CLI.\n",
                  mono_method_full_name(method, TRUE),
                  m_class_get_image(method->klass)->name);
#endif
      else {
    ...
    MonoMethod *nm =
        mono_marshal_get_native_wrapper(method, TRUE, mono_aot_only);
    ...
    if (m_class_get_parent(method->klass) ==
      ...
      if (*name == '.' && (strcmp(name, ".ctor") == 0)) {
        ...
        return mono_get_addr_from_ftnptr(
            (gpointer)mono_icall_get_wrapper_full(mi, TRUE));
        ...
        /* HACK: missing gsharedvt_out wrappers to do transition to del tramp in
         * interp-only mode */
    ...
    mono_error_set_invalid_program(
        error, "Unrecognizable runtime implemented method '%s'", full_name);
    g_free(full_name);
    return NULL;
  }
  ...
  if (method->wrapper_type == MONO_WRAPPER_OTHER) {
    ...
    if (info->subtype == WRAPPER_SUBTYPE_GSHAREDVT_IN ||
        info->subtype == WRAPPER_SUBTYPE_GSHAREDVT_OUT) {
      ...
      /*
       * This is a special wrapper whose body is implemented in assembly, like a
       * trampoline. We use a wrapper so EH works.
       * FIXME: The caller signature doesn't match the callee, which might cause
       * problems on some platforms
       */
      if (mono_ee_features.use_aot_trampolines)
        mono_aot_get_trampoline_full(is_in ? "gsharedvt_trampoline"
...
static gpointer mono_jit_compile_method_with_opt(MonoMethod *method,
                                                 guint32 opt, gboolean jit_only,
                                                 MonoError *error) {
...
lookup_start:
  ...
  if (opt & MONO_OPT_AOT) {
    ...
    if (mono_aot_mode == MONO_AOT_MODE_INTERP &&
        method->wrapper_type == MONO_WRAPPER_OTHER) {
      WrapperInfo *info = mono_marshal_get_wrapper_info(method);
      g_assert(info);
    ...
    if (code) {
      ...
      if (mono_gc_is_critical_method(method)) {
        /*
         * The suspend code needs to be able to lookup these methods by ip in
         * async context, so preload their jit info.
         */
      ...
      /*
       * In llvm-only mode, method might be a shared method, so we can't
       * initialize its class. This is not a problem, since it will be
       * initialized when the method is first called by init_method ().
       */
      if (!mono_llvm_only && !mono_class_is_open_constructed_type(
  ...
  if (!jit_only && !code && mono_aot_only && mono_use_interpreter &&
      method->wrapper_type != MONO_WRAPPER_OTHER) {
    if (mono_llvm_only) {
  ...
  if (!code) {
    if (mono_class_is_open_constructed_type(
            m_class_get_byval_arg(method->klass))) {
      char *full_name = mono_type_get_full_name(method->klass);
      mono_error_set_invalid_operation(
          error,
          "Could not execute the method because the containing type '%s', is "
    ...
    if (mono_aot_only) {
      ...
      mono_error_set_execution_engine(
          error,
          "Attempting to JIT compile method '%s' while running in aot-only "
          "mode. See "
          "https://docs.microsoft.com/xamarin/ios/internals/limitations for "
          "more information.\n",
          fullname);
      g_free(fullname);
  ...
  if (!code)
    ...
    // FIXME mini_jit_info_table_find doesn't work yet under wasm due to
    // code_start/code_end issues.
#ifndef HOST_WASM
  if ((method->wrapper_type == MONO_WRAPPER_WRITE_BARRIER ||
       method->wrapper_type == MONO_WRAPPER_ALLOC)) {
    MonoDomain *d;

    /*
     * SGEN requires the JIT info for these methods to be registered, see
     * is_ip_in_managed_allocator ().
     */
    MonoJitInfo *ji =
        mini_jit_info_table_find(mono_domain_get(), (char *)code, &d);
    g_assert(ji);
  }
#endif
  ...
  if (callinfo) {
    // FIXME Locking here is somewhat historical due to
    // mono_register_jit_icall_wrapper taking loader lock.
...
gpointer mono_jit_compile_method(MonoMethod *method, MonoError *error) {
  ...
  code = mono_jit_compile_method_with_opt(
      method, mono_get_optimizations_for_method(method, default_opt), FALSE,
      error);
  return code;
}
 ...
 */
gpointer mono_jit_compile_method_jit_only(MonoMethod *method,
                                          MonoError *error) {
  gpointer code;

  code = mono_jit_compile_method_with_opt(
      method, mono_get_optimizations_for_method(method, default_opt), TRUE,
      error);
  return code;
}

#ifdef MONO_ARCH_HAVE_INVALIDATE_METHOD
static void invalidated_delegate_trampoline(char *desc) {
  g_error("Unmanaged code called delegate of type %s which was already garbage "
          "collected.\n"
          "See http://www.mono-project.com/Diagnostic:Delegate for an "
          "explanation and ways to fix this.",
          desc);
}
#endif
 ...
 */
static void mono_jit_free_method(MonoDomain *domain, MonoMethod *method) {
  ...
  while (g_hash_table_iter_next(&iter, NULL, (void **)&jlist)) {
    ...
    for (tmp = jlist->list; tmp; tmp = tmp->next) {
      ...
      if (ip >= (guint8 *)ji->ji->code_start &&
          ip < (guint8 *)ji->ji->code_start + ji->ji->code_size)
        remove = g_slist_prepend(remove, tmp);
    }
    for (tmp = remove; tmp; tmp = tmp->next) {
      jlist->list =
          g_slist_delete_link((GSList *)jlist->list, (GSList *)tmp->data);
    }
    g_slist_free(remove);
  }
  mono_domain_unlock(domain);
...
#ifdef MONO_ARCH_HAVE_INVALIDATE_METHOD
  if (mini_debug_options.keep_delegates &&
    ...
    g_free(type);
    mono_arch_invalidate_method(ji->ji,
                                (gpointer)invalidated_delegate_trampoline,
                                (gpointer)type_and_method);
    destroy = FALSE;
  }
#endif
   ...
   * key in the table, so if we free the code_mp first, another thread can grab
   * the same code address and replace our entry in the table.
   */
...
gpointer mono_jit_search_all_backends_for_jit_info(MonoDomain *domain,
                                                   MonoMethod *method,
                                                   MonoJitInfo **out_ji) {
  gpointer code;
  MonoJitInfo *ji;
...
gpointer mono_jit_find_compiled_method_with_jit_info(MonoDomain *domain,
                                                     MonoMethod *method,
                                                     MonoJitInfo **ji) {
  MonoDomain *target_domain;
  MonoJitInfo *info;
...
void mono_set_bisect_methods(guint32 opt, const char *method_list_filename) {
  ...
  while (fgets(method_name, sizeof(method_name), file)) {
    ...
    method_name[len - 1] = 0;
    g_hash_table_insert(bisect_methods_hash, g_strdup(method_name),
                        GINT_TO_POINTER(1));
  }
  g_assert(feof(file));
}
...
guint32 mono_get_optimizations_for_method(MonoMethod *method, guint32 opt) {
  g_assert(method);
  ...
  if (!mono_current_single_method) {
    ...
    if (!g_hash_table_lookup(mono_single_method_hash, method)) {
      ...
      mono_single_method_list =
          g_slist_prepend(mono_single_method_list, method);
    }
    return opt;
  }
...
create_runtime_invoke_info(MonoDomain *domain, MonoMethod *method,
                           gpointer compiled_method, gboolean callee_gsharedvt,
                           gboolean use_interp, MonoError *error) {
  MonoMethod *invoke;
  ...
  if (!mono_llvm_only &&
      (mono_aot_only || mini_debug_options.dyn_runtime_invoke)) {
  ...
  if (!info->dyn_call_info) {
    if (mono_llvm_only) {
      ...
      if (!callee_gsharedvt) {
        ...
        MonoMethodSignature *wrapper_sig =
            mini_get_gsharedvt_out_sig_wrapper_signature(
                sig->hasthis, sig->ret->type != MONO_TYPE_VOID,
                sig->param_count);

        info->wrapper_arg = g_malloc0(2 * sizeof(gpointer));
        info->wrapper_arg[0] = mini_llvmonly_add_method_wrappers(
            method, info->compiled_method, FALSE, FALSE,
            &(info->wrapper_arg[1]));
        ...
        /* The out wrapper has the same signature as the compiled gsharedvt
         * method */
        MonoMethodSignature *wrapper_sig =
            mini_get_gsharedvt_out_sig_wrapper_signature(
                sig->hasthis, sig->ret->type != MONO_TYPE_VOID,
        ...
        info->wrapper_arg =
            (gpointer *)(mono_method_needs_static_rgctx_invoke(method, TRUE)
...
static MonoObject *mono_llvmonly_runtime_invoke(MonoMethod *method,
  ...
  MonoDomain *domain = mono_domain_get();
  MonoObject *(*runtime_invoke)(MonoObject * this_obj, void **params,
  ...
  gpointer *const param_refs =
      g_newa(gpointer, param_count * 2 + sig->hasthis + 2);
  ...
  /* The gsharedvt out wrapper has an extra argument which contains the method
   * to call */
  args[pindex++] = &info->wrapper_arg;

  runtime_invoke =
      (MonoObject * (*)(MonoObject *, void **, MonoObject **, void *))
          info->runtime_invoke;
 ...
 */
static MonoObject *mono_jit_runtime_invoke(MonoMethod *method, void *obj,
  ...
  if (mono_ee_features.force_use_interpreter)
    return mini_get_interp_callbacks()->runtime_invoke(method, obj, params, exc,
                                                       error);
  ...
  if (!info) {
    if (mono_security_core_clr_enabled()) {
      ...
      if (mono_class_has_failure(method->klass)) {
        ...
        if (exc)
          *exc =
              (MonoObject *)mono_class_get_exception_for_failure(method->klass);
        return NULL;
      }
    }
    ...
    if (m_class_get_rank(method->klass) &&
      ...
      /*
       * Array Get/Set/Address methods. The JIT implements them using inline
       * code inside the runtime invoke wrappers, so no need to compile them.
       */
      if (mono_aot_only) {
        /*
    ...
    if (callee) {
      ...
      if (!compiled_method) {
        ...
        if (mono_llvm_only) {
          ji = mini_jit_info_table_find(
              mono_domain_get(),
              (char *)mono_get_addr_from_ftnptr(compiled_method), NULL);
          callee_gsharedvt = mini_jit_info_is_gsharedvt(ji);
          if (callee_gsharedvt)
            callee_gsharedvt = mini_is_gsharedvt_variable_signature(
                mono_method_signature_internal(jinfo_get_method(ji)));
        }

        if (!callee_gsharedvt)
          compiled_method = mini_add_method_trampoline(
              callee, compiled_method,
              mono_method_needs_static_rgctx_invoke(callee, TRUE), FALSE);
      }
    } else {
    ...
    info = create_runtime_invoke_info(domain, method, compiled_method,
                                      callee_gsharedvt, use_interp, error);
    if (!is_ok(error))
      return NULL;

    mono_domain_lock(domain);
    info2 = (RuntimeInvokeInfo *)mono_conc_hashtable_insert(
        domain_info->runtime_invoke_hash, method, info);
    mono_domain_unlock(domain);
    if (info2) {
      g_free(info);
      info = info2;
  ...
  /* If coop is enabled, and the caller didn't ask for the exception to be
     caught separately, we always catch the exception and propagate it through
     the MonoError */
  ...
  if (info->dyn_call_info) {
    if (!dyn_runtime_invoke) {
      ...
      dyn_runtime_invoke =
          (RuntimeInvokeDynamicFunction)mono_jit_compile_method_jit_only(invoke,
                                                                         error);
  ...
  if (info->dyn_call_info) {
    ...
    args =
        (void **)g_alloca((sig->param_count + sig->hasthis) * sizeof(gpointer));
    pindex = 0;
    if (sig->hasthis)
      args[pindex++] = &obj;
    for (i = 0; i < sig->param_count; ++i) {
    ...
    mono_arch_start_dyn_call(info->dyn_call_info, (gpointer **)args, retval,
                             buf);
  ...
  if (info->use_interp) {
    result = mini_get_interp_callbacks()->runtime_invoke(method, obj, params,
                                                         exc, error);
    return_val_if_nok(error, NULL);
  } else if (mono_llvm_only) {
    result =
        mono_llvmonly_runtime_invoke(method, info, obj, params, exc, error);
    if (!is_ok(error))
      return NULL;
  } else {
    runtime_invoke =
    ...
    result =
        runtime_invoke((MonoObject *)obj, params, exc, info->compiled_method);
  }
  if (catchExcInMonoError && *exc != NULL) {
...
MONO_SIG_HANDLER_FUNC(, mono_sigfpe_signal_handler) {
  ...
  ji = mono_jit_info_table_find_internal(
      mono_domain_get(), mono_arch_ip_from_context(ctx), TRUE, TRUE);
...
#define MONO_SIG_HANDLER_DEBUG                                                 \
  1 // "with_fault_addr" but could be extended in future, so "debug"

#ifdef MONO_SIG_HANDLER_DEBUG
// Same as MONO_SIG_HANDLER_FUNC but debug_fault_addr is added to params, and
// no_optimize. The Krait workaround is not needed here, due to this not
// actually being the signal handler, so MONO_SIGNAL_HANDLER_FUNC is combined
// into it.
#define MONO_SIG_HANDLER_FUNC_DEBUG(access, ftn)                               \
  access MONO_NO_OPTIMIZATION void ftn(                                        \
      int _dummy, MONO_SIG_HANDLER_INFO_TYPE *_info, void *context,            \
      void *volatile debug_fault_addr G_GNUC_UNUSED)
#define MONO_SIG_HANDLER_PARAMS_DEBUG MONO_SIG_HANDLER_PARAMS, debug_fault_addr
#endif

#endif

gboolean mono_is_addr_implicit_null_check(void *addr) {
...
#endif
{
  ...
  if (domain)
    ji = mono_jit_info_table_find_internal(
        domain, mono_arch_ip_from_context(ctx), TRUE, TRUE);

#ifdef MONO_ARCH_SIGSEGV_ON_ALTSTACK
  if (mono_handle_soft_stack_ovf(jit_tls, ji, ctx, info,
                                 (guint8 *)info->si_addr))
    return;

  /* info->si_addr seems to be NULL on some kernels when handling stack
   * overflows */
  fault_addr = info->si_addr;
  if (fault_addr == NULL) {
  ...
  if (jit_tls && jit_tls->stack_size &&
      ABS((guint8 *)fault_addr -
          ((guint8 *)jit_tls->end_of_stack - jit_tls->stack_size)) <
  ...
  if (!ji) {
    ...
    if (mono_dump_start())
      mono_handle_native_crash(mono_get_signame(SIGSEGV), &mctx,
                               (MONO_SIG_HANDLER_INFO_TYPE *)info);
...
MONO_SIG_HANDLER_FUNC(, mono_sigsegv_signal_handler) {
#ifdef HOST_WIN32
  gpointer const debug_fault_addr =
      (gpointer)MONO_SIG_HANDLER_GET_INFO()
 ...
 */
static gpointer mono_jit_create_remoting_trampoline(MonoDomain *domain,
                                                    MonoMethod *method,
  ...
  if ((method->flags & METHOD_ATTRIBUTE_VIRTUAL) &&
      mono_method_signature_internal(method)->generic_param_count) {
    return mono_create_specific_trampoline(
        method, MONO_TRAMPOLINE_GENERIC_VIRTUAL_REMOTING, domain, NULL);
  }
...
static G_GNUC_UNUSED void no_imt_trampoline(void) { g_assert_not_reached(); }
...
gpointer mini_get_vtable_trampoline(MonoVTable *vt, int slot_index) {
  ...
  if (!vtable_trampolines ||
      slot_index + MONO_IMT_SIZE >= vtable_trampolines_size) {
    mono_jit_lock();
    if (!vtable_trampolines || index >= vtable_trampolines_size) {
      ...
      if (vtable_trampolines)
        memcpy(new_table, vtable_trampolines,
               vtable_trampolines_size * sizeof(gpointer));
  ...
  if (!vtable_trampolines[index])
    vtable_trampolines[index] = mono_create_specific_trampoline(
        GUINT_TO_POINTER(slot_index), MONO_TRAMPOLINE_VCALL,
        mono_get_root_domain(), NULL);
  return vtable_trampolines[index];
}

static gpointer mini_get_imt_trampoline(MonoVTable *vt, int slot_index) {
  return mini_get_vtable_trampoline(vt, slot_index - MONO_IMT_SIZE);
}

static gboolean mini_imt_entry_inited(MonoVTable *vt, int imt_slot_index) {
  if (mono_llvm_only)
    return FALSE;
...
static gpointer create_delegate_method_ptr(MonoMethod *method,
                                           MonoError *error) {
  gpointer func;
  ...
  if (method_is_dynamic(method)) {
    ...
    gpointer trampoline = mono_runtime_create_jump_trampoline(
        mono_domain_get(), method, TRUE, error);
    return_val_if_nok(error, NULL);
...
static void mini_init_delegate(MonoDelegateHandle delegate, MonoError *error) {
  ...
  if (mono_llvm_only) {
    g_assert(del->method);
    /* del->method_ptr might already be set to no_llvmonly_interp_method_pointer
     * if the delegate was created from the interpreter */
    del->method_ptr = mini_llvmonly_load_method_delegate(
        del->method, FALSE, FALSE, &del->extra_arg, error);
...
char *mono_get_delegate_virtual_invoke_impl_name(gboolean load_imt_reg,
                                                 int offset) {
  int abs_offset;
  ...
  return g_strdup_printf("delegate_virtual_invoke%s_%s%d",
                         load_imt_reg ? "_imt" : "", offset < 0 ? "m_" : "",
                         abs_offset / TARGET_SIZEOF_VOID_P);
}
...
gpointer mono_get_delegate_virtual_invoke_impl(MonoMethodSignature *sig,
  ...
  if (mono_ee_features.use_aot_trampolines) {
    cache[idx] = (guint8 *)mono_aot_get_trampoline(
        mono_get_delegate_virtual_invoke_impl_name(load_imt_reg, offset));
    g_assert(cache[idx]);
  } else {
    cache[idx] = (guint8 *)mono_arch_get_delegate_virtual_invoke_impl(
        sig, method, offset, load_imt_reg);
  }
  return cache[idx];
}
 ...
 *
 */
gboolean mini_parse_debug_option(const char *option) {
  ...
  else if (!strcmp(option, "suspend-on-native-crash") ||
           !strcmp(option, "suspend-on-sigsegv"))
...
static void mini_parse_debug_options(void) {
  ...
  for (ptr = args; ptr && *ptr; ptr++) {
    ...
    if (!mini_parse_debug_option(arg)) {
      ...
      fprintf(
          stderr,
          "Available options: 'handle-sigint', 'keep-delegates', "
          "'reverse-pinvoke-exceptions', 'collect-pagefault-stats', "
          "'break-on-unverified', 'no-gdb-backtrace', "
          "'suspend-on-native-crash', 'suspend-on-sigsegv', "
          "'suspend-on-exception', 'suspend-on-unhandled', "
          "'dont-free-domains', 'dyn-runtime-invoke', 'gdb', "
          "'explicit-null-checks', 'gen-seq-points', 'no-compact-seq-points', "
          "'single-imm-size', 'init-stacks', 'casts', 'soft-breakpoints', "
          "'check-pinvoke-callconv', 'use-fallback-tls', "
          "'debug-domain-unload', 'partial-sharing', 'align-small-structs', "
          "'native-debugger-break', 'thread-dump-dir=DIR', 'no-verbose-gdb', "
          "'llvm_disable_inlining', 'llvm-disable-self-init', "
          "'llvm-disable-implicit-null-checks', 'weak-memory-model'.\n");
      exit(1);
    }
  }
...
MonoDebugOptions *mini_get_debug_options(void) { return &mini_debug_options; }

static gpointer mini_create_ftnptr(MonoDomain *domain, gpointer addr) {
...
static gpointer mini_get_addr_from_ftnptr(gpointer descr) {
#if defined(PPC_USES_FUNCTION_DESCRIPTOR)
...
static void register_jit_stats(void) {
  mono_counters_register("Compiled methods",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.methods_compiled);
  mono_counters_register("Methods from AOT",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.methods_aot);
  mono_counters_register("Methods from AOT+LLVM",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.methods_aot_llvm);
  mono_counters_register("Methods JITted using mono JIT",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.methods_without_llvm);
  mono_counters_register("Methods JITted using LLVM",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.methods_with_llvm);
  mono_counters_register("Methods using the interpreter",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.methods_with_interp);
  mono_counters_register("JIT/method_to_ir",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_method_to_ir);
  mono_counters_register("JIT/liveness_handle_exception_clauses",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_liveness_handle_exception_clauses);
  mono_counters_register("JIT/handle_out_of_line_bblock",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_handle_out_of_line_bblock);
  mono_counters_register("JIT/decompose_long_opts",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_decompose_long_opts);
  mono_counters_register("JIT/decompose_typechecks",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_decompose_typechecks);
  mono_counters_register("JIT/local_cprop",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_local_cprop);
  mono_counters_register("JIT/local_emulate_ops",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_local_emulate_ops);
  mono_counters_register("JIT/optimize_branches",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_optimize_branches);
  mono_counters_register("JIT/handle_global_vregs",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_handle_global_vregs);
  mono_counters_register("JIT/local_deadce",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_local_deadce);
  mono_counters_register("JIT/local_alias_analysis",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_local_alias_analysis);
  mono_counters_register("JIT/if_conversion",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_if_conversion);
  mono_counters_register("JIT/bb_ordering",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_bb_ordering);
  mono_counters_register("JIT/compile_dominator_info",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_compile_dominator_info);
  mono_counters_register("JIT/compute_natural_loops",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_compute_natural_loops);
  mono_counters_register("JIT/insert_safepoints",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_insert_safepoints);
  mono_counters_register("JIT/ssa_compute",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_ssa_compute);
  mono_counters_register(
      "JIT/ssa_cprop", MONO_COUNTER_JIT | MONO_COUNTER_LONG | MONO_COUNTER_TIME,
      &mono_jit_stats.jit_ssa_cprop);
  mono_counters_register("JIT/ssa_deadce",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_ssa_deadce);
  mono_counters_register("JIT/perform_abc_removal",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_perform_abc_removal);
  mono_counters_register("JIT/ssa_remove",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_ssa_remove);
  mono_counters_register("JIT/local_cprop2",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_local_cprop2);
  mono_counters_register("JIT/handle_global_vregs2",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_handle_global_vregs2);
  mono_counters_register("JIT/local_deadce2",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_local_deadce2);
  mono_counters_register("JIT/optimize_branches2",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_optimize_branches2);
  mono_counters_register("JIT/decompose_vtype_opts",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_decompose_vtype_opts);
  mono_counters_register("JIT/decompose_array_access_opts",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_decompose_array_access_opts);
  mono_counters_register(
      "JIT/liveness_handle_exception_clauses2",
      MONO_COUNTER_JIT | MONO_COUNTER_LONG | MONO_COUNTER_TIME,
      &mono_jit_stats.jit_liveness_handle_exception_clauses2);
  mono_counters_register("JIT/analyze_liveness",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_analyze_liveness);
  mono_counters_register("JIT/linear_scan",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_linear_scan);
  mono_counters_register("JIT/arch_allocate_vars",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_arch_allocate_vars);
  mono_counters_register("JIT/spill_global_var",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_spill_global_vars);
  mono_counters_register("JIT/local_cprop3",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_local_cprop3);
  mono_counters_register("JIT/local_deadce3",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_local_deadce3);
  mono_counters_register(
      "JIT/codegen", MONO_COUNTER_JIT | MONO_COUNTER_LONG | MONO_COUNTER_TIME,
      &mono_jit_stats.jit_codegen);
  mono_counters_register("JIT/create_jit_info",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_create_jit_info);
  mono_counters_register("JIT/gc_create_gc_map",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_gc_create_gc_map);
  mono_counters_register("JIT/save_seq_point_info",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_save_seq_point_info);
  mono_counters_register("Total time spent JITting",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_jit_stats.jit_time);
  mono_counters_register("Basic blocks", MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.basic_blocks);
  mono_counters_register("Max basic blocks",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.max_basic_blocks);
  mono_counters_register("Allocated vars", MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.allocate_var);
  mono_counters_register("Code reallocs", MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.code_reallocs);
  mono_counters_register("Allocated code size",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.allocated_code_size);
  mono_counters_register("Allocated seq points size",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.allocated_seq_points_size);
  mono_counters_register("Inlineable methods",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.inlineable_methods);
  mono_counters_register("Inlined methods", MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.inlined_methods);
  mono_counters_register("Regvars", MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.regvars);
  mono_counters_register("Locals stack size",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.locals_stack_size);
  mono_counters_register("Method cache lookups",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.methods_lookups);
  mono_counters_register("Compiled CIL code size",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.cil_code_size);
  mono_counters_register("Native code size",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.native_code_size);
  mono_counters_register("Aliases found", MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.alias_found);
  mono_counters_register("Aliases eliminated",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.alias_removed);
  mono_counters_register("Aliased loads eliminated",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.loads_eliminated);
  mono_counters_register("Aliased stores eliminated",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &mono_jit_stats.stores_eliminated);
  mono_counters_register("Optimized immediate divisions",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
...
static gint class_method_pair_equal(gconstpointer ka, gconstpointer kb) {
...
static guint class_method_pair_hash(gconstpointer data) {
  const MonoClassMethodPair *pair = (const MonoClassMethodPair *)data;

  return (gsize)pair->klass ^ (gsize)pair->method;
}

static void mini_create_jit_domain_info(MonoDomain *domain) {
  ...
  info->delegate_trampoline_hash =
      g_hash_table_new(class_method_pair_hash, class_method_pair_equal);
  info->llvm_vcall_trampoline_hash =
      g_hash_table_new(mono_aligned_addr_hash, NULL);
  info->runtime_invoke_hash = mono_conc_hashtable_new_full(
      mono_aligned_addr_hash, NULL, NULL, runtime_invoke_info_free);
  info->seq_points = g_hash_table_new_full(mono_aligned_addr_hash, NULL, NULL,
...
static void delete_jump_list(gpointer key, gpointer value, gpointer user_data) {
  MonoJumpList *jlist = (MonoJumpList *)value;
  g_slist_free((GSList *)jlist->list);
}

static void delete_got_slot_list(gpointer key, gpointer value,
                                 gpointer user_data) {
  GSList *list = (GSList *)value;
  g_slist_free(list);
}

static void dynamic_method_info_free(gpointer key, gpointer value,
                                     gpointer user_data) {
...
static void free_jit_callee_list(gpointer key, gpointer value,
                                 gpointer user_data) {
  g_slist_free((GSList *)value);
}

static void mini_free_jit_domain_info(MonoDomain *domain) {
  MonoJitDomainInfo *info = domain_jit_info(domain);

  g_hash_table_foreach(info->jump_target_hash, delete_jump_list, NULL);
  g_hash_table_destroy(info->jump_target_hash);
  if (info->jump_target_got_slot_hash) {
    g_hash_table_foreach(info->jump_target_got_slot_hash, delete_got_slot_list,
                         NULL);
    g_hash_table_destroy(info->jump_target_got_slot_hash);
  }
  if (info->dynamic_code_hash) {
    g_hash_table_foreach(info->dynamic_code_hash, dynamic_method_info_free,
                         NULL);
...
#ifdef ENABLE_LLVM
static gboolean llvm_init_inner(void) {
...
MonoDebuggerCallbacks *mini_get_dbg_callbacks(void) { return &dbg_cbs; }

int mono_ee_api_version(void) { return MONO_EE_API_VERSION; }
...
static gboolean mini_is_interpreter_enabled(void) {
  return mono_use_interpreter;
}

static const char *mono_get_runtime_build_version(void);
...
MonoDomain *mini_init(const char *filename, const char *runtime_version) {
  ...
  callbacks.debug_log_is_enabled =
...
#ifndef DISABLE_REMOTING
  if (mono_use_interpreter)
    callbacks.interp_get_remoting_invoke =
...
#define MONO_CODE_MANAGER_CALLBACK(ret, name, sig) mono_arch_code_##name,
  ...
  /*Init arch tls information only after the metadata side is inited to make
   * sure we see dynamic appdomain tls keys*/
...
#define JIT_RUNTIME_WORKS
  ...
  mono_runtime_init_checked(domain, (MonoThreadStartCB)mono_thread_start_cb,
                            mono_thread_attach_cb, error);
  mono_error_assert_ok(error);
  mono_thread_attach(domain);
  MONO_PROFILER_RAISE(
      thread_name,
...
static void register_icalls(void) {
   ...
   * so on.
   */
  register_icall(mono_profiler_raise_method_enter, mono_icall_sig_void_ptr_ptr,
                 TRUE);
  register_icall(mono_profiler_raise_method_leave, mono_icall_sig_void_ptr_ptr,
                 TRUE);
  register_icall(mono_profiler_raise_method_tail_call,
                 mono_icall_sig_void_ptr_ptr, TRUE);
  register_icall(mono_profiler_raise_exception_clause,
                 mono_icall_sig_void_ptr_int_int_object, TRUE);

  register_icall(mono_trace_enter_method, mono_icall_sig_void_ptr_ptr_ptr,
                 TRUE);
  register_icall(mono_trace_leave_method, mono_icall_sig_void_ptr_ptr_ptr,
                 TRUE);
  ...
  register_icall(mono_llvm_match_exception,
                 mono_icall_sig_int_ptr_int_int_ptr_object, TRUE);
  register_icall(mono_llvm_clear_exception, NULL, TRUE);
  register_icall(mono_llvm_load_exception, mono_icall_sig_object, TRUE);
  register_icall(mono_llvm_throw_corlib_exception, mono_icall_sig_void_int,
                 TRUE);
...
#ifndef TARGET_WASM
  register_icall(mono_debug_personality, mono_icall_sig_int_int_int_ptr_ptr_ptr,
                 TRUE);
#endif
#endif

  if (!mono_llvm_only) {
    register_dyn_icall(mono_get_throw_exception(), mono_arch_throw_exception,
                       mono_icall_sig_void_object, TRUE);
    register_dyn_icall(mono_get_rethrow_exception(),
                       mono_arch_rethrow_exception, mono_icall_sig_void_object,
                       TRUE);
    register_dyn_icall(mono_get_throw_corlib_exception(),
                       mono_arch_throw_corlib_exception,
                       mono_icall_sig_void_ptr, TRUE);
  }
  register_icall(mono_thread_get_undeniable_exception, mono_icall_sig_object,
                 FALSE);
  register_icall(ves_icall_thread_finish_async_abort, mono_icall_sig_void,
                 FALSE);
  register_icall(mono_thread_interruption_checkpoint, mono_icall_sig_object,
                 FALSE);
  register_icall(mono_thread_force_interruption_checkpoint_noraise,
                 mono_icall_sig_object, FALSE);

  register_icall(mono_threads_state_poll, mono_icall_sig_void, FALSE);

#ifndef MONO_ARCH_NO_EMULATE_LONG_MUL_OPTS
  register_opcode_emulation(OP_LMUL, __emul_lmul, mono_icall_sig_long_long_long,
                            mono_llmult, FALSE);
  register_opcode_emulation(OP_LDIV, __emul_ldiv, mono_icall_sig_long_long_long,
                            mono_lldiv, FALSE);
  register_opcode_emulation(OP_LDIV_UN, __emul_ldiv_un,
                            mono_icall_sig_long_long_long, mono_lldiv_un,
                            FALSE);
  register_opcode_emulation(OP_LREM, __emul_lrem, mono_icall_sig_long_long_long,
                            mono_llrem, FALSE);
  register_opcode_emulation(OP_LREM_UN, __emul_lrem_un,
                            mono_icall_sig_long_long_long, mono_llrem_un,
                            FALSE);
#endif
#if !defined(MONO_ARCH_NO_EMULATE_LONG_MUL_OPTS) ||                            \
    defined(MONO_ARCH_EMULATE_LONG_MUL_OVF_OPTS)
  register_opcode_emulation(OP_LMUL_OVF_UN, __emul_lmul_ovf_un,
                            mono_icall_sig_long_long_long, mono_llmult_ovf_un,
                            FALSE);
  register_opcode_emulation(OP_LMUL_OVF, __emul_lmul_ovf,
                            mono_icall_sig_long_long_long, mono_llmult_ovf,
                            FALSE);
#endif

#ifndef MONO_ARCH_NO_EMULATE_LONG_SHIFT_OPS
  register_opcode_emulation(OP_LSHL, __emul_lshl,
                            mono_icall_sig_long_long_int32, mono_lshl, TRUE);
  register_opcode_emulation(OP_LSHR, __emul_lshr,
                            mono_icall_sig_long_long_int32, mono_lshr, TRUE);
  register_opcode_emulation(OP_LSHR_UN, __emul_lshr_un,
                            mono_icall_sig_long_long_int32, mono_lshr_un, TRUE);
#endif

#if defined(MONO_ARCH_EMULATE_MUL_DIV) || defined(MONO_ARCH_EMULATE_DIV)
  register_opcode_emulation(OP_IDIV, __emul_op_idiv,
                            mono_icall_sig_int32_int32_int32, mono_idiv, FALSE);
  register_opcode_emulation(OP_IDIV_UN, __emul_op_idiv_un,
                            mono_icall_sig_int32_int32_int32, mono_idiv_un,
                            FALSE);
  register_opcode_emulation(OP_IREM, __emul_op_irem,
                            mono_icall_sig_int32_int32_int32, mono_irem, FALSE);
  register_opcode_emulation(OP_IREM_UN, __emul_op_irem_un,
                            mono_icall_sig_int32_int32_int32, mono_irem_un,
                            FALSE);
#endif

#ifdef MONO_ARCH_EMULATE_MUL_DIV
  register_opcode_emulation(OP_IMUL, __emul_op_imul,
                            mono_icall_sig_int32_int32_int32, mono_imul, TRUE);
#endif

#if defined(MONO_ARCH_EMULATE_MUL_DIV) || defined(MONO_ARCH_EMULATE_MUL_OVF)
  register_opcode_emulation(OP_IMUL_OVF, __emul_op_imul_ovf,
                            mono_icall_sig_int32_int32_int32, mono_imul_ovf,
                            FALSE);
  register_opcode_emulation(OP_IMUL_OVF_UN, __emul_op_imul_ovf_un,
                            mono_icall_sig_int32_int32_int32, mono_imul_ovf_un,
                            FALSE);
#endif

#if defined(MONO_ARCH_EMULATE_MUL_DIV) || defined(MONO_ARCH_SOFT_FLOAT_FALLBACK)
  register_opcode_emulation(OP_FDIV, __emul_fdiv,
                            mono_icall_sig_double_double_double, mono_fdiv,
                            FALSE);
#endif

#ifdef MONO_ARCH_EMULATE_FCONV_TO_U8
  register_opcode_emulation(OP_FCONV_TO_U8, __emul_fconv_to_u8,
                            mono_icall_sig_ulong_double, mono_fconv_u8_2,
                            FALSE);
  register_opcode_emulation(OP_RCONV_TO_U8, __emul_rconv_to_u8,
                            mono_icall_sig_ulong_float, mono_rconv_u8, FALSE);
#endif
#ifdef MONO_ARCH_EMULATE_FCONV_TO_U4
  register_opcode_emulation(OP_FCONV_TO_U4, __emul_fconv_to_u4,
                            mono_icall_sig_uint32_double, mono_fconv_u4_2,
                            FALSE);
  register_opcode_emulation(OP_RCONV_TO_U4, __emul_rconv_to_u4,
                            mono_icall_sig_uint32_float, mono_rconv_u4, FALSE);
#endif
  register_opcode_emulation(OP_FCONV_TO_OVF_I8, __emul_fconv_to_ovf_i8,
                            mono_icall_sig_long_double, mono_fconv_ovf_i8,
                            FALSE);
  register_opcode_emulation(OP_FCONV_TO_OVF_U8, __emul_fconv_to_ovf_u8,
                            mono_icall_sig_ulong_double, mono_fconv_ovf_u8,
                            FALSE);
  register_opcode_emulation(OP_FCONV_TO_OVF_U8_UN, __emul_fconv_to_ovf_u8_un,
                            mono_icall_sig_ulong_double, mono_fconv_ovf_u8_un,
                            FALSE);
  register_opcode_emulation(OP_RCONV_TO_OVF_I8, __emul_rconv_to_ovf_i8,
                            mono_icall_sig_long_float, mono_rconv_ovf_i8,
                            FALSE);
  register_opcode_emulation(OP_RCONV_TO_OVF_U8, __emul_rconv_to_ovf_u8,
                            mono_icall_sig_ulong_float, mono_rconv_ovf_u8,
                            FALSE);
  register_opcode_emulation(OP_RCONV_TO_OVF_U8_UN, __emul_rconv_to_ovf_u8_un,
                            mono_icall_sig_ulong_float, mono_rconv_ovf_u8_un,
                            FALSE);

#ifdef MONO_ARCH_EMULATE_FCONV_TO_I8
  register_opcode_emulation(OP_FCONV_TO_I8, __emul_fconv_to_i8,
                            mono_icall_sig_long_double, mono_fconv_i8, FALSE);
  register_opcode_emulation(OP_RCONV_TO_I8, __emul_rconv_to_i8,
                            mono_icall_sig_long_float, mono_rconv_i8, FALSE);
#endif

#ifdef MONO_ARCH_EMULATE_CONV_R8_UN
  register_opcode_emulation(OP_ICONV_TO_R_UN, __emul_iconv_to_r_un,
                            mono_icall_sig_double_int32, mono_conv_to_r8_un,
                            FALSE);
#endif
#ifdef MONO_ARCH_EMULATE_LCONV_TO_R8
  register_opcode_emulation(OP_LCONV_TO_R8, __emul_lconv_to_r8,
                            mono_icall_sig_double_long, mono_lconv_to_r8,
                            FALSE);
#endif
#ifdef MONO_ARCH_EMULATE_LCONV_TO_R4
  register_opcode_emulation(OP_LCONV_TO_R4, __emul_lconv_to_r4,
                            mono_icall_sig_float_long, mono_lconv_to_r4, FALSE);
#endif
#ifdef MONO_ARCH_EMULATE_LCONV_TO_R8_UN
  register_opcode_emulation(OP_LCONV_TO_R_UN, __emul_lconv_to_r8_un,
                            mono_icall_sig_double_long, mono_lconv_to_r8_un,
                            FALSE);
#endif
#ifdef MONO_ARCH_EMULATE_FREM
  register_opcode_emulation(OP_FREM, __emul_frem,
                            mono_icall_sig_double_double_double, mono_fmod,
                            FALSE);
  register_opcode_emulation(OP_RREM, __emul_rrem,
                            mono_icall_sig_float_float_float, fmodf, FALSE);
#endif

#ifdef MONO_ARCH_SOFT_FLOAT_FALLBACK
  if (mono_arch_is_soft_float()) {
    register_opcode_emulation(OP_FSUB, __emul_fsub,
                              mono_icall_sig_double_double_double, mono_fsub,
                              FALSE);
    register_opcode_emulation(OP_FADD, __emul_fadd,
                              mono_icall_sig_double_double_double, mono_fadd,
                              FALSE);
    register_opcode_emulation(OP_FMUL, __emul_fmul,
                              mono_icall_sig_double_double_double, mono_fmul,
                              FALSE);
    register_opcode_emulation(OP_FNEG, __emul_fneg,
                              mono_icall_sig_double_double, mono_fneg, FALSE);
    register_opcode_emulation(OP_ICONV_TO_R8, __emul_iconv_to_r8,
                              mono_icall_sig_double_int32, mono_conv_to_r8,
                              FALSE);
    register_opcode_emulation(OP_ICONV_TO_R4, __emul_iconv_to_r4,
                              mono_icall_sig_double_int32, mono_conv_to_r4,
                              FALSE);
    register_opcode_emulation(OP_FCONV_TO_R4, __emul_fconv_to_r4,
                              mono_icall_sig_double_double, mono_fconv_r4,
                              FALSE);
    register_opcode_emulation(OP_FCONV_TO_I1, __emul_fconv_to_i1,
                              mono_icall_sig_int8_double, mono_fconv_i1, FALSE);
    register_opcode_emulation(OP_FCONV_TO_I2, __emul_fconv_to_i2,
                              mono_icall_sig_int16_double, mono_fconv_i2,
                              FALSE);
    register_opcode_emulation(OP_FCONV_TO_I4, __emul_fconv_to_i4,
                              mono_icall_sig_int32_double, mono_fconv_i4,
                              FALSE);
    register_opcode_emulation(OP_FCONV_TO_U1, __emul_fconv_to_u1,
                              mono_icall_sig_uint8_double, mono_fconv_u1,
                              FALSE);
    register_opcode_emulation(OP_FCONV_TO_U2, __emul_fconv_to_u2,
                              mono_icall_sig_uint16_double, mono_fconv_u2,
                              FALSE);

#if TARGET_SIZEOF_VOID_P == 4
    register_opcode_emulation(OP_FCONV_TO_I, __emul_fconv_to_i,
                              mono_icall_sig_int32_double, mono_fconv_i4,
                              FALSE);
#endif

    register_opcode_emulation(OP_FBEQ, __emul_fcmp_eq,
                              mono_icall_sig_uint32_double_double, mono_fcmp_eq,
                              FALSE);
    register_opcode_emulation(OP_FBLT, __emul_fcmp_lt,
                              mono_icall_sig_uint32_double_double, mono_fcmp_lt,
                              FALSE);
    register_opcode_emulation(OP_FBGT, __emul_fcmp_gt,
                              mono_icall_sig_uint32_double_double, mono_fcmp_gt,
                              FALSE);
    register_opcode_emulation(OP_FBLE, __emul_fcmp_le,
                              mono_icall_sig_uint32_double_double, mono_fcmp_le,
                              FALSE);
    register_opcode_emulation(OP_FBGE, __emul_fcmp_ge,
                              mono_icall_sig_uint32_double_double, mono_fcmp_ge,
                              FALSE);
    register_opcode_emulation(OP_FBNE_UN, __emul_fcmp_ne_un,
                              mono_icall_sig_uint32_double_double,
                              mono_fcmp_ne_un, FALSE);
    register_opcode_emulation(OP_FBLT_UN, __emul_fcmp_lt_un,
                              mono_icall_sig_uint32_double_double,
                              mono_fcmp_lt_un, FALSE);
    register_opcode_emulation(OP_FBGT_UN, __emul_fcmp_gt_un,
                              mono_icall_sig_uint32_double_double,
                              mono_fcmp_gt_un, FALSE);
    register_opcode_emulation(OP_FBLE_UN, __emul_fcmp_le_un,
                              mono_icall_sig_uint32_double_double,
                              mono_fcmp_le_un, FALSE);
    register_opcode_emulation(OP_FBGE_UN, __emul_fcmp_ge_un,
                              mono_icall_sig_uint32_double_double,
                              mono_fcmp_ge_un, FALSE);

    register_opcode_emulation(OP_FCEQ, __emul_fcmp_ceq,
                              mono_icall_sig_uint32_double_double, mono_fceq,
                              FALSE);
    register_opcode_emulation(OP_FCGT, __emul_fcmp_cgt,
                              mono_icall_sig_uint32_double_double, mono_fcgt,
                              FALSE);
    register_opcode_emulation(OP_FCGT_UN, __emul_fcmp_cgt_un,
                              mono_icall_sig_uint32_double_double, mono_fcgt_un,
                              FALSE);
    register_opcode_emulation(OP_FCLT, __emul_fcmp_clt,
                              mono_icall_sig_uint32_double_double, mono_fclt,
                              FALSE);
    register_opcode_emulation(OP_FCLT_UN, __emul_fcmp_clt_un,
                              mono_icall_sig_uint32_double_double, mono_fclt_un,
                              FALSE);
  ...
  register_icall(mono_class_interface_match, mono_icall_sig_uint32_ptr_int32,
                 TRUE);
#endif

  // FIXME Elsewhere these are registered with no_wrapper = FALSE
#if SIZEOF_REGISTER == 4
  register_opcode_emulation(OP_FCONV_TO_U, __emul_fconv_to_u,
                            mono_icall_sig_uint32_double, mono_fconv_u4, TRUE);
#else
  register_opcode_emulation(OP_FCONV_TO_U, __emul_fconv_to_u,
                            mono_icall_sig_ulong_double, mono_fconv_u8, TRUE);
#endif

  /* other jit icalls */
  register_icall(ves_icall_mono_delegate_ctor,
                 mono_icall_sig_void_object_object_ptr, FALSE);
  register_icall(ves_icall_mono_delegate_ctor_interp,
                 mono_icall_sig_void_object_object_ptr, FALSE);
  register_icall(mono_class_static_field_address, mono_icall_sig_ptr_ptr_ptr,
                 FALSE);
  ...
  register_icall(ves_icall_mono_ldstr, mono_icall_sig_object_ptr_ptr_int32,
                 FALSE);
  register_icall(mono_helper_stelem_ref_check,
                 mono_icall_sig_void_object_object, FALSE);
  register_icall(ves_icall_object_new, mono_icall_sig_object_ptr_ptr, FALSE);
  register_icall(ves_icall_object_new_specific, mono_icall_sig_object_ptr,
                 FALSE);
  register_icall(ves_icall_array_new, mono_icall_sig_object_ptr_ptr_int32,
                 FALSE);
  register_icall(ves_icall_array_new_specific, mono_icall_sig_object_ptr_int32,
                 FALSE);
  ...
  register_icall(mono_helper_compile_generic_method,
                 mono_icall_sig_ptr_object_ptr_ptr, FALSE);
  ...
  register_icall(mono_value_copy_internal, mono_icall_sig_void_ptr_ptr_ptr,
                 FALSE);
  register_icall(mono_object_castclass_unbox, mono_icall_sig_object_object_ptr,
                 FALSE);
  register_icall(mono_break, NULL, TRUE);
  register_icall(mono_create_corlib_exception_0, mono_icall_sig_object_int,
                 TRUE);
  register_icall(mono_create_corlib_exception_1,
                 mono_icall_sig_object_int_object, TRUE);
  register_icall(mono_create_corlib_exception_2,
                 mono_icall_sig_object_int_object_object, TRUE);
  ...
  register_icall(mono_array_new_3, mono_icall_sig_object_ptr_int_int_int,
                 FALSE);
  register_icall(mono_array_new_4, mono_icall_sig_object_ptr_int_int_int_int,
                 FALSE);
  register_icall(mono_array_new_n_icall, mono_icall_sig_object_ptr_int_ptr,
                 FALSE);
  register_icall(mono_get_native_calli_wrapper, mono_icall_sig_ptr_ptr_ptr_ptr,
                 FALSE);
  register_icall(mono_resume_unwind, mono_icall_sig_void_ptr, TRUE);
  register_icall(mono_gsharedvt_constrained_call,
                 mono_icall_sig_object_ptr_ptr_ptr_ptr_ptr, FALSE);
  register_icall(mono_gsharedvt_value_copy, mono_icall_sig_void_ptr_ptr_ptr,
                 TRUE);

  // WARNING We do runtime selection here but the string *MUST* be to a fallback
  // function that has same signature and behavior
  MonoRangeCopyFunction const mono_gc_wbarrier_range_copy =
      mono_gc_get_range_copy_func();
  register_icall_no_wrapper(mono_gc_wbarrier_range_copy,
                            mono_icall_sig_void_ptr_ptr_int);

  register_icall(mono_object_castclass_with_cache,
                 mono_icall_sig_object_object_ptr_ptr, FALSE);
  register_icall(mono_object_isinst_with_cache,
                 mono_icall_sig_object_object_ptr_ptr, FALSE);
  ...
  register_dyn_icall(mini_get_dbg_callbacks()->user_break,
                     mono_debugger_agent_user_break, mono_icall_sig_void,
                     FALSE);

  register_icall(mini_llvm_init_method, mono_icall_sig_void_ptr_ptr_int_ptr,
                 TRUE);
  register_icall_no_wrapper(mini_llvmonly_resolve_iface_call_gsharedvt,
                            mono_icall_sig_ptr_object_int_ptr_ptr);
  register_icall_no_wrapper(mini_llvmonly_resolve_vcall_gsharedvt,
                            mono_icall_sig_ptr_object_int_ptr_ptr);
  register_icall_no_wrapper(mini_llvmonly_resolve_generic_virtual_call,
                            mono_icall_sig_ptr_ptr_int_ptr);
  register_icall_no_wrapper(mini_llvmonly_resolve_generic_virtual_iface_call,
                            mono_icall_sig_ptr_ptr_int_ptr);
  /* This needs a wrapper so it can have a preserveall cconv */
  register_icall(mini_llvmonly_init_vtable_slot, mono_icall_sig_ptr_ptr_int,
                 FALSE);
  register_icall(mini_llvmonly_init_delegate, mono_icall_sig_void_object, TRUE);
  register_icall(mini_llvmonly_init_delegate_virtual,
                 mono_icall_sig_void_object_object_ptr, TRUE);
  register_icall(mini_llvmonly_throw_nullref_exception, mono_icall_sig_void,
                 TRUE);
  register_icall(mini_llvmonly_throw_aot_failed_exception,
                 mono_icall_sig_void_ptr, TRUE);
  ...
  register_icall_with_wrapper(mono_monitor_enter_v4_internal,
                              mono_icall_sig_void_obj_ptr);
  register_icall_no_wrapper(mono_monitor_enter_fast, mono_icall_sig_int_obj);
  register_icall_no_wrapper(mono_monitor_enter_v4_fast,
  ...
  register_icall_no_wrapper(mono_tls_get_sgen_thread_info_extern,
                            mono_icall_sig_ptr);
  register_icall_no_wrapper(mono_tls_get_lmf_addr_extern, mono_icall_sig_ptr);

  register_icall_no_wrapper(mono_interp_entry_from_trampoline,
                            mono_icall_sig_void_ptr_ptr);
  register_icall_no_wrapper(mono_interp_to_native_trampoline,
                            mono_icall_sig_void_ptr_ptr);
...
/**
 * Counters of mono_stats and mono_jit_stats can be read without locking during
 * shutdown. For all other contexts, assumes that the domain lock is held.
 * MONO_NO_SANITIZE_THREAD tells Clang's ThreadSanitizer to hide all reports of
 * these (known) races.
 */
...
void mono_runtime_print_stats(void) {
  if (mono_jit_stats.enabled) {
    ...
    g_print("Biggest method:         %" G_GINT32_FORMAT " (%s)\n",
            mono_jit_stats.biggest_method_size, mono_jit_stats.biggest_method);
    ...
    mono_counters_dump(MONO_COUNTER_SECTION_MASK | MONO_COUNTER_MONOTONIC,
                       NULL);
    g_print("\n");
  }
}

static void jit_stats_cleanup(void) {
...
void mono_disable_optimizations(guint32 opts) { default_opt &= ~opts; }
...
void mono_set_optimizations(guint32 opts) {
  ...
  mono_set_generic_sharing_vt_supported(
      mono_aot_only || ((default_opt & MONO_OPT_GSHAREDVT) != 0));
#else
  if (mono_llvm_only)
    mono_set_generic_sharing_vt_supported(TRUE);
#endif
...
void mono_set_verbose_level(guint32 level) { mini_verbose = level; }

static const char *mono_get_runtime_build_version(void) { return FULL_VERSION; }

/**
 * mono_get_runtime_build_info:
 * The returned string is owned by the caller. The returned string
 * format is <code>VERSION (FULL_VERSION BUILD_DATE)</code> and build date is
 * optional. \returns the runtime version + build date in string format.
 */
char *mono_get_runtime_build_info(void) {
  if (mono_build_date)
    return g_strdup_printf("%s (%s %s)", VERSION, FULL_VERSION,
                           mono_build_date);
  else
    return g_strdup_printf("%s (%s)", VERSION, FULL_VERSION);
}

static void mono_precompile_assembly(MonoAssembly *ass, void *user_data) {
  ...
  for (i = 0; i < mono_image_get_table_rows(image, MONO_TABLE_METHOD); ++i) {
    ...
    method = mono_get_method_checked(image, MONO_TOKEN_METHOD_DEF | (i + 1),
                                     NULL, NULL, error);
    if (!method) {
      mono_error_cleanup(error); /* FIXME don't swallow the error */
      continue;
...
#ifndef DISABLE_REMOTING
    if (mono_class_is_marshalbyref(method->klass) &&
        mono_method_signature_internal(method)->hasthis) {
  ...
  for (i = 0; i < mono_image_get_table_rows(image, MONO_TABLE_ASSEMBLYREF);
       ++i) {
...
static MonoBreakPolicy always_insert_breakpoint(MonoMethod *method) {
 ...
 * to not allow an app to be aborted by a perfectly valid IL opcode when
 * executing untrusted or semi-trusted code.
 *
 * \p policy_callback will be called every time a break point instruction needs
 * to be inserted with the method argument being the method that calls \c
 * Debugger.Break or has the IL \c break instruction. The callback should return
 * \c MONO_BREAK_POLICY_NEVER if it wants the breakpoint to not be effective in
 * the given method. \c MONO_BREAK_POLICY_ALWAYS is the default.
 */
...
void mono_runtime_install_custom_handlers_usage(void) {
  fprintf(stdout, "Custom Handlers:\n"
                  "   --handlers=HANDLERS            Enable handler support, "
                  "HANDLERS is a comma\n"
                  "                                  separated list of "
                  "available handlers to install.\n"
                  "\n"
