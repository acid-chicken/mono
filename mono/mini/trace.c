...
#undef fprintf
#define fprintf(__ignore, ...)                                                 \
  g_log("mono-gc", G_LOG_LEVEL_MESSAGE, __VA_ARGS__)
#endif
...
static void indent(int diff) {
  ...
  printf("[%p: %.5f %d] ", (void *)mono_native_thread_id_get(),
         seconds_since_start(), indent_level);
  if (diff > 0)
    indent_level += diff;
}

static char *string_to_utf8(MonoString *s) {
  char *as;
  GError *gerror = NULL;
  ...
  as = g_utf16_to_utf8(mono_string_chars_internal(s), s->length, NULL, NULL,
                       &gerror);
  if (gerror) {
...
static gboolean is_gshared_vt_wrapper(MonoMethod *m) {
 ...
 */
static char frame_kind(MonoJitInfo *ji) {
  if (!ji)
    return 'u';
...
void mono_trace_enter_method(MonoMethod *method, MonoJitInfo *ji,
    ...
    if (m_class_is_valuetype(method->klass) || is_gshared_vt_wrapper(method)) {
      ...
      if (o) {
        ...
        if (klass == mono_defaults.string_class) {
          ...
          printf("[this:[TYPE:%p:%s]]", o,
                 mono_type_full_name(((MonoReflectionType *)o)->type));
        } else {
          printf("this:%p[%s.%s %s]", o, m_class_get_name_space(klass),
                 m_class_get_name(klass), o->vtable->domain->friendly_name);
        }
      } else {
        printf("this:NULL");
      }
  ...
  for (i = 0; i < sig->param_count; ++i) {
    ...
    case MONO_TYPE_STRING: {
      ...
      if (s) {
        ...
        g_assert(((MonoObject *)s)->vtable->klass ==
    ...
    case MONO_TYPE_OBJECT: {
      ...
      if (o) {
        ...
        if (klass == mono_defaults.string_class) {
          ...
          printf("[%s.%s:[%d]%p]", m_class_get_name_space(klass),
                 m_class_get_name(klass), mono_array_length_internal(arr), o);
        } else
          printf("[%s.%s:%p]", m_class_get_name_space(klass),
                 m_class_get_name(klass), o);
      } else {
        printf("%p", *arg_in_stack_slot(buf, gpointer));
      }
      break;
...
void mono_trace_leave_method(MonoMethod *method, MonoJitInfo *ji,
                             MonoProfilerCallContext *ctx) {
  MonoType *type;
  char *fname;
  ...
  case MONO_TYPE_OBJECT: {
    ...
    if (o) {
      ...
      if (o->vtable->klass == mono_defaults.boolean_class) {
        ...
        printf("[%s.%s:%p]", m_class_get_name_space(mono_object_class(o)),
               m_class_get_name(mono_object_class(o)), o);
      }
    } else
      printf("[OBJECT:%p]", o);
...
void mono_trace_tail_method(MonoMethod *method, MonoJitInfo *ji,
                            MonoMethod *target) {
  char *fname, *tname;
...
void mono_trace_enable(gboolean enable) { trace_spec.enabled = enable; }

gboolean mono_trace_is_enabled() { return trace_spec.enabled; }
