...
#define FOLD_FBINOP(name, op)                                                  \
  case name:                                                                   \
    ...
    *(double *)dest->inst_p0 =                                                 \
        (*((double *)arg1->inst_p0))op(*((double *)arg2->inst_p0));            \
    break;
...
#define FOLD_RBINOP(name, op)                                                  \
  case name:                                                                   \
    ...
    *(float *)dest->inst_p0 =                                                  \
        (*((float *)arg1->inst_p0))op(*((float *)arg2->inst_p0));              \
    break;
...
#define ALLOC_DEST(cfg, dest, ins)                                             \
  do {                                                                         \
    if (!(dest)) {                                                             \
 ...
 *
 * Perform constant folding on INS, using ARG1 and ARG2 as the arguments. If
 * OVERWRITE is true, then store the result back into INS and return INS.
 * Otherwise allocate a new ins, store the result into it and return it. If
 * constant folding cannot be performed, return NULL.
 */
MonoInst *mono_constant_fold_ins(MonoCompile *cfg, MonoInst *ins,
  ...
  case OP_IREM_UN:
    if ((arg1->opcode == OP_ICONST) && (arg2->opcode == OP_ICONST)) {
      if ((arg2->inst_c0 == 0) ||
          ((arg1->inst_c0 == G_MININT32) && (arg2->inst_c0 == -1)))
        return NULL;
      ALLOC_DEST(cfg, dest, ins);
      switch (ins->opcode) {
  ...
  case OP_IREM_UN_IMM:
    if (arg1->opcode == OP_ICONST) {
      if ((ins->inst_imm == 0) ||
          ((arg1->inst_c0 == G_MININT32) && (ins->inst_imm == -1)))
        return NULL;
      ALLOC_DEST(cfg, dest, ins);
      switch (ins->opcode) {
  ...
  case OP_INEG:
    ...
    break;
  case OP_SEXT_I4:
    if (arg1->opcode == OP_ICONST && arg1->inst_c0 >= 0 &&
        arg1->inst_c0 < (1 << 16) && overwrite) {
  ...
  case OP_ICOMPARE_IMM: {
    ...
    if ((arg1->opcode == OP_PCONST) && (arg2->opcode == OP_PCONST) &&
        ins->next) {
      MonoInst *next = ins->next;
