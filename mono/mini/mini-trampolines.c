 ...
 * perform the same task as the static rgctx wrappers, but they are
 * smaller/faster, and can be made to work with full AOT.
 *
...
gpointer mono_create_static_rgctx_trampoline(MonoMethod *m, gpointer addr) {
  ...
  /*
   * In the AOT case, addr might point to either the method, or to an unbox
   * trampoline, so make the hash keyed on the m+addr pair.
   */
  mono_domain_lock(domain);
  if (!domain_jit_info(domain)->static_rgctx_trampoline_hash)
    domain_jit_info(domain)->static_rgctx_trampoline_hash =
        g_hash_table_new(rgctx_tramp_info_hash, rgctx_tramp_info_equal);
  tmp_info.m = m;
  tmp_info.addr = addr;
  res = g_hash_table_lookup(
      domain_jit_info(domain)->static_rgctx_trampoline_hash, &tmp_info);
  ...
  info->m = m;
  info->addr = addr;
  g_hash_table_insert(domain_jit_info(domain)->static_rgctx_trampoline_hash,
                      info, res);
 ...
 *
 *   Resolve the actual method called when making an IMT call through
 * VTABLE_SLOT with IMT_METHOD as the interface method.
 *
 * Either IMPL_METHOD or OUT_AOT_ADDR will be set on return.
 */
gpointer *
mini_resolve_imt_method(MonoVTable *vt, gpointer *vtable_slot,
                        MonoMethod *imt_method, MonoMethod **impl_method,
                        gpointer *out_aot_addr, gboolean *out_need_rgctx_tramp,
  ...
  error_init(error);
  /* This has to be variance aware since imt_method can be from an interface
   * that vt->klass doesn't directly implement */
  interface_offset = mono_class_interface_offset_with_variance(
      vt->klass, imt_method->klass, &variance_used);
  if (interface_offset < 0)
    g_error("%s doesn't implement interface %s\n",
            mono_type_get_name_full(m_class_get_byval_arg(vt->klass),
                                    MONO_TYPE_NAME_FORMAT_IL),
            mono_type_get_name_full(m_class_get_byval_arg(imt_method->klass),
                                    MONO_TYPE_NAME_FORMAT_IL));
  ...
  *variant_iface = NULL;
  ...
  } else if (variance_used &&
             mono_class_has_variant_generic_params(imt_method->klass)) {
    *variant_iface = imt_method;
  }

  /* We can only use the AOT compiled code if we don't require further
   * processing */
  ...
  if (imt_method->is_inflated &&
    ...
    impl = mono_class_get_vtable_entry(
        vt->klass,
        interface_offset +
            mono_method_get_declaring_generic_method(imt_method)->slot);

    if (mono_class_is_ginst(impl->klass))
      context.class_inst =
          mono_class_get_generic_class(impl->klass)->context.class_inst;
    context.method_inst =
        ((MonoMethodInflated *)imt_method)->context.method_inst;
    ...
    if (lookup_aot && !m_class_is_valuetype(vt->klass)) {
      aot_addr = (guint8 *)mono_aot_get_method_from_vt_slot(
          mono_domain_get(), vt,
          interface_offset + mono_method_get_vtable_slot(imt_method), error);
    ...
    if (aot_addr)
      ...
      impl = mono_class_get_vtable_entry(
          vt->klass,
          interface_offset + mono_method_get_vtable_slot(imt_method));
  }
  ...
  *out_aot_addr = aot_addr;
  ...
  DEBUG_IMT(printf("mono_convert_imt_slot_to_vtable_slot: method = %s.%s.%s, "
                   "imt_method = %s.%s.%s\n",
                   method->klass->name_space, method->klass->name, method->name,
                   imt_method->klass->name_space, imt_method->klass->name,
                   imt_method->name));
  ...
  if (vt->imt_collisions_bitmap & (1 << imt_slot)) {
    ...
    DEBUG_IMT(printf(
        "mono_convert_imt_slot_to_vtable_slot: slot %p[%d] is in the IMT, and "
        "colliding becomes %p[%d] (interface_offset = %d, method->slot = %d)\n",
        slot, imt_slot, vtable_slot, vtable_offset, interface_offset,
        imt_method->slot));
    return vtable_slot;
  } else {
    DEBUG_IMT(printf("mono_convert_imt_slot_to_vtable_slot: slot %p[%d] is in "
                     "the IMT, but not colliding\n",
                     slot, imt_slot));
    return vtable_slot;
  }
}
 ...
 */
static gboolean is_generic_method_definition(MonoMethod *m) {
  ...
  if (!context->method_inst)
    return FALSE;
  if (context->method_inst ==
      mono_method_get_generic_container(((MonoMethodInflated *)m)->declaring)
          ->context.method_inst)
    return TRUE;
  return FALSE;
}
...
gpointer mini_add_method_trampoline(MonoMethod *m, gpointer compiled_method,
  ...
  if (m->is_inflated || callee_array_helper) {
    ...
    ji = mini_jit_info_table_find(
        mono_domain_get(), (char *)mono_get_addr_from_ftnptr(compiled_method),
        NULL);
  ...
  if (callee_gsharedvt && mini_is_gsharedvt_variable_signature(
                              mono_method_signature_internal(jmethod))) {
    MonoMethodSignature *sig, *gsig;

    /* Here m is a generic instance, while ji->method is the gsharedvt method
     * implementing it */
  ...
  if (callee_array_helper) {
    ...
    /* In AOT mode, compiled_method points to one of the InternalArray methods
     * in Array. */
    if (ji && !mono_llvm_only &&
        mono_method_needs_static_rgctx_invoke(jinfo_get_method(ji), TRUE))
 ...
 *
 *   The code to handle normal, virtual, and interface method calls and jumps,
 * both from JITted and LLVM compiled code.
 */
static gpointer common_call_trampoline(host_mgreg_t *regs, guint8 *code,
                                       MonoMethod *m, MonoVTable *vt,
  ...
  /*
   * rgctx trampolines are needed when the call is indirect so the caller can't
   * pass the rgctx argument needed by the callee.
   */
  if (virtual_ && m)
  ...
  if (imt_call) {
    ...
    if (mono_object_is_transparent_proxy(this_arg)) {
      ...
      vtable_slot = mini_resolve_imt_method(
          vt, vtable_slot, imt_method, &impl_method, &addr, &need_rgctx_tramp,
          &variant_iface, error);
      return_val_if_nok(error, NULL);
      ...
      if (mono_class_has_dim_conflicts(vt->klass)) {
        ...
        if (in_conflict) {
          ...
          mono_error_set_ambiguous_implementation(
              error,
              "Could not call method '%s' with type '%s' because there are "
              "multiple incompatible interface methods overriding this method.",
      ...
      /* We must handle magic interfaces on rank 1 arrays of ref types as if
       * they were variant */
      if (!variant_iface && m_class_get_rank(vt->klass) == 1 &&
          !m_class_is_valuetype(m_class_get_element_class(vt->klass)) &&
          m_class_is_array_special_interface(imt_method->klass))
        variant_iface = imt_method;
  ...
  if (virtual_ && is_generic_method_definition(m)) {
    ...
    if (mono_class_is_ginst(m->klass))
      context.class_inst =
          mono_class_get_generic_class(m->klass)->context.class_inst;
    else
    ...
    context.method_inst =
        ((MonoMethodInflated *)generic_virtual)->context.method_inst;
    ...
    /*
     * The caller is gshared code, compute the actual method to call from M and
     * this/rgctx.
     */
    if (m->is_inflated && mono_method_get_context(m)->method_inst) {
      MonoMethodRuntimeGenericContext *mrgctx =
          (MonoMethodRuntimeGenericContext *)mono_arch_find_static_call_vtable(
              regs, code);
      ...
      MonoObject *this_argument =
          (MonoObject *)mono_arch_get_this_arg_from_call(regs, code);
      ...
      if (!vtable_slot) {
        ...
        klass = m_class_get_supertypes(
            this_argument->vtable->klass)[m_class_get_idepth(m->klass) - 1];
      }
    }
    ...
    if (method_inst || m->wrapper_type) {
      ...
      actual_method =
          mono_class_inflate_generic_method_checked(declaring, &context, error);
      mono_error_assert_ok(error);
    } else {
  ...
  if (generic_virtual || variant_iface) {
    if (m_class_is_valuetype(
            vt->klass)) /*FIXME is this required variant iface?*/
  ...
  if (generic_virtual || variant_iface) {
    ...
    mono_method_add_generic_virtual_invocation(mono_domain_get(), vt,
                                               vtable_slot, target, addr);
  ...
  if (!code) {
    ...
    if (domain_jit_info(domain)->jump_target_got_slot_hash) {
      ...
      mono_domain_lock(domain);
      list = (GSList *)g_hash_table_lookup(
      ...
      if (list) {
        ...
        g_hash_table_remove(domain_jit_info(domain)->jump_target_got_slot_hash,
                            m);
        g_slist_free(list);
      }
      mono_domain_unlock(domain);
  ...
  if (vtable_slot) {
    if (vtable_slot_to_patch &&
        (mono_aot_is_got_entry(code, (guint8 *)vtable_slot_to_patch) ||
    ...
    if (plt_entry) {
      if (generic_shared) {
        ...
        if (ji && ji->has_generic_jit_info) {
          if (target_ji && !target_ji->has_generic_jit_info) {
            ...
            /* compiled_method might be an interp entry trampoline and the
             * interpreter has no generic sharing */
            no_patch = TRUE;
          }
        }
      }
      ...
      if (ji && target_ji && generic_shared && ji->has_generic_jit_info &&
          !target_ji->has_generic_jit_info) {
        /*
         * Can't patch the call as the caller is gshared, but the callee is not.
         * Happens when generic sharing fails.
      ...
      if (!no_patch && mono_method_same_domain(ji, target_ji))
        mono_arch_patch_callsite((guint8 *)ji->code_start, code,
                                 (guint8 *)addr);
    }
  }
...
gpointer mono_magic_trampoline(host_mgreg_t *regs, guint8 *code, gpointer arg,
  ...
  res =
      common_call_trampoline(regs, code, (MonoMethod *)arg, NULL, NULL, error);
  if (!is_ok(error)) {
 ...
 */
static gpointer mono_vcall_trampoline(host_mgreg_t *regs, guint8 *code,
                                      int slot, guint8 *tramp) {
  gpointer res;
  MONO_ENTER_GC_UNSAFE;
   ...
   * - the vtable slot.
   * We use one vtable trampoline per vtable slot index, so we need only the
   * vtable, the other two can be computed from the vtable + the slot index.
   */
...
gpointer mono_aot_trampoline(host_mgreg_t *regs, guint8 *code,
  ...
  if (!addr) {
    ...
    if (!method)
      g_error("Could not load AOT trampoline due to %s",
...
gpointer mono_aot_plt_trampoline(host_mgreg_t *regs, guint8 *code,
                                 guint8 *aot_module, guint8 *tramp) {
  MONO_REQ_GC_UNSAFE_MODE;
...
static gpointer mono_rgctx_lazy_fetch_trampoline(host_mgreg_t *regs,
                                                 guint8 *code, gpointer data,
                                                 guint8 *tramp) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  if (mrgctx)
    res = mono_method_fill_runtime_generic_context(
        (MonoMethodRuntimeGenericContext *)arg, index, error);
  else
    res = mono_class_fill_runtime_generic_context((MonoVTable *)arg, index,
                                                  error);
  if (!is_ok(error)) {
    mono_error_set_pending_exception(error);
    return NULL;
...
gpointer mono_delegate_trampoline(host_mgreg_t *regs, guint8 *code,
  ...
  g_assert(mono_class_has_parent(mono_object_class(delegate),
  ...
  if (delegate->method) {
     ...
     * (ctor_with_method () does this, but it doesn't store the wrapper back
     * into delegate->method).
     */
#ifndef DISABLE_REMOTING
    if (delegate->target &&
        mono_object_is_transparent_proxy(delegate->target)) {
      is_remote = TRUE;
      error_init(err);
#ifndef DISABLE_COM
      if (((MonoTransparentProxy *)delegate->target)
                  ->remote_class->proxy_class !=
              mono_class_get_com_object_class() &&
          !mono_class_is_com_object(((MonoTransparentProxy *)delegate->target)
                                        ->remote_class->proxy_class))
#endif
    ...
    if (!is_remote) {
      ...
      if (sig->hasthis && m_class_is_valuetype(method->klass)) {
        ...
        if (tramp_info->invoke_sig->param_count > sig->param_count &&
            tramp_info->invoke_sig->params[0]->byref)
          need_unbox = FALSE;
    ...
    // ftnptrs are being used.  "method" would end up null on archtitectures
    // without ftnptrs so we can just skip this.
  } else if (delegate->method_ptr) {
    ji = mono_jit_info_table_find(
        domain, mono_get_addr_from_ftnptr(delegate->method_ptr));
    if (ji)
  ...
  if (method) {
    ...
    if (callvirt)
      closed_over_null =
          tramp_info->invoke_sig->param_count == sig->param_count;

    if (callvirt && !closed_over_null) {
      /*
       * The delegate needs to make a virtual call to the target method using
       * its first argument as the receiver. This is hard to support in
       * full-aot, so optimize it in some cases if possible. If the target
       * method is not virtual or is in a sealed class, the vcall will call it
       * directly. If the call doesn't return a valuetype, then the vcall uses
       * the same calling convention as a normal call.
       */
      if ((mono_class_is_sealed(method->klass) ||
    ...
    if (tramp_info->method == NULL && delegate->target != NULL &&
        method->flags & METHOD_ATTRIBUTE_VIRTUAL) {
      /* tramp_info->method == NULL happens when someone asks us to JIT some
       * delegate's Invoke method (see compile_special).  In that case if method
       * is virtual, the target could be some derived class, so we need to find
       * the correct override.
       */
      /* FIXME: does it make sense that we get called with tramp_info for the
       * Invoke? */
      method =
          mono_object_get_virtual_method_internal(delegate->target, method);
      enable_caching = FALSE;
    } else if (delegate->target && method->flags & METHOD_ATTRIBUTE_VIRTUAL &&
  ...
  /*
   * If the called address is a trampoline, replace it with the compiled method
   * so further calls don't have to go through the trampoline.
   */
  if (method && !callvirt) {
    ...
    if (enable_caching && delegate->method_code && *delegate->method_code) {
      ...
      addr = mini_add_method_trampoline(method, compiled_method,
                                        need_rgctx_tramp, need_unbox_tramp);
    ...
    if (need_rgctx_tramp)
      delegate->method_ptr =
          mono_create_static_rgctx_trampoline(method, delegate->method_ptr);
  }
  ...
  if (!code) {
    ...
    code = (guint8 *)mini_add_method_trampoline(
        m, code, mono_method_needs_static_rgctx_invoke(m, FALSE), FALSE);
  }
 ...
 *
 *   Return the C function which needs to be called by the generic trampoline of
 * type TRAMP_TYPE.
 */
...
static guchar *create_trampoline_code(MonoTrampolineType tramp_type) {
  MonoTrampInfo *info;
  guchar *code;
...
void mono_trampolines_init(void) {
  ...
  mono_trampoline_code[MONO_TRAMPOLINE_JIT] =
      create_trampoline_code(MONO_TRAMPOLINE_JIT);
  mono_trampoline_code[MONO_TRAMPOLINE_JUMP] =
      create_trampoline_code(MONO_TRAMPOLINE_JUMP);
  mono_trampoline_code[MONO_TRAMPOLINE_RGCTX_LAZY_FETCH] =
      create_trampoline_code(MONO_TRAMPOLINE_RGCTX_LAZY_FETCH);
#ifdef MONO_ARCH_AOT_SUPPORTED
  mono_trampoline_code[MONO_TRAMPOLINE_AOT] =
      create_trampoline_code(MONO_TRAMPOLINE_AOT);
  mono_trampoline_code[MONO_TRAMPOLINE_AOT_PLT] =
      create_trampoline_code(MONO_TRAMPOLINE_AOT_PLT);
#endif
  mono_trampoline_code[MONO_TRAMPOLINE_DELEGATE] =
...
#endif
  mono_trampoline_code[MONO_TRAMPOLINE_VCALL] =
      create_trampoline_code(MONO_TRAMPOLINE_VCALL);

  mono_counters_register("Calls to trampolines",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &trampoline_calls);
  mono_counters_register("JIT trampolines", MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &jit_trampolines);
  mono_counters_register("Unbox trampolines",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &unbox_trampolines);
  mono_counters_register("Static rgctx trampolines",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &static_rgctx_trampolines);
  mono_counters_register("RGCTX unmanaged lookups",
...
gpointer mono_create_specific_trampoline(gpointer arg1,
  ...
  if (mono_aot_only)
    code = mono_aot_create_specific_trampoline(mono_defaults.corlib, arg1,
                                               tramp_type, domain, &len);
  else
...
gpointer mono_create_jump_trampoline(MonoDomain *domain, MonoMethod *method,
  ...
  if (mono_use_interpreter && !mono_aot_only) {
    gpointer ret = mini_get_interp_callbacks()->create_method_pointer(
        method, FALSE, error);
    if (!is_ok(error))
      return NULL;
    return ret;
  ...
  mono_domain_lock(domain);
  code = g_hash_table_lookup(domain_jit_info(domain)->jump_trampoline_hash,
                             method);
  mono_domain_unlock(domain);
  if (code)
    return code;

  code = mono_create_specific_trampoline(method, MONO_TRAMPOLINE_JUMP,
  ...
  mono_domain_lock(domain);
  g_hash_table_insert(domain_jit_info(domain)->jump_trampoline_hash, method,
                      ji->code_start);
  mono_domain_unlock(domain);
...
static void method_not_found(void) { g_assert_not_reached(); }
...
gpointer mono_create_jit_trampoline(MonoDomain *domain, MonoMethod *method,
  ...
  if (mono_aot_only) {
    ...
    if (mono_llvm_only) {
      ...
      /* Methods are lazily initialized on first call, so this can't lead
       * recursion */
      code = mono_jit_compile_method(method, error);
      if (!is_ok(error))
        return NULL;
  ...
  mono_domain_lock(domain);
  tramp =
      g_hash_table_lookup(domain_jit_info(domain)->jit_trampoline_hash, method);
  mono_domain_unlock(domain);
  if (tramp)
    return tramp;

  tramp = mono_create_specific_trampoline(method, MONO_TRAMPOLINE_JIT, domain,
                                          NULL);

  mono_domain_lock(domain);
  g_hash_table_insert(domain_jit_info(domain)->jit_trampoline_hash, method,
                      tramp);
...
gpointer mono_create_jit_trampoline_from_token(MonoImage *image,
                                               guint32 token) {
  gpointer tramp;
  ...
  *(guint32 *)buf = token;
  ...
  tramp =
      mono_create_specific_trampoline(start, MONO_TRAMPOLINE_AOT, domain, NULL);
...
mono_create_delegate_trampoline_info(MonoDomain *domain, MonoClass *klass,
                                     MonoMethod *method) {
  MonoMethod *invoke;
  ERROR_DECL(error);
  ...
  pair.method = method;
  mono_domain_lock(domain);
  tramp_info = (MonoDelegateTrampInfo *)g_hash_table_lookup(
      domain_jit_info(domain)->delegate_trampoline_hash, &pair);
  mono_domain_unlock(domain);
  if (tramp_info)
    return tramp_info;
  ...
  tramp_info = (MonoDelegateTrampInfo *)mono_domain_alloc0(
      domain, sizeof(MonoDelegateTrampInfo));
  tramp_info->invoke = invoke;
  tramp_info->invoke_sig = mono_method_signature_internal(invoke);
  tramp_info->impl_this = mono_arch_get_delegate_invoke_impl(
      mono_method_signature_internal(invoke), TRUE);
  tramp_info->impl_nothis = mono_arch_get_delegate_invoke_impl(
      mono_method_signature_internal(invoke), FALSE);
  tramp_info->method = method;
  if (method) {
    error_init(error);
    tramp_info->sig = mono_method_signature_checked(method, error);
    tramp_info->need_rgctx_tramp =
        mono_method_needs_static_rgctx_invoke(method, FALSE);
  }
  tramp_info->invoke_impl = mono_create_specific_trampoline(
      tramp_info, MONO_TRAMPOLINE_DELEGATE, domain, &code_size);
  g_assert(code_size);

  dpair = (MonoClassMethodPair *)mono_domain_alloc0(
      domain, sizeof(MonoClassMethodPair));
  memcpy(dpair, &pair, sizeof(MonoClassMethodPair));

  /* store trampoline address */
  mono_domain_lock(domain);
  g_hash_table_insert(domain_jit_info(domain)->delegate_trampoline_hash, dpair,
                      tramp_info);
  mono_domain_unlock(domain);
...
static void no_delegate_trampoline(void) { g_assert_not_reached(); }
...
gpointer mono_create_delegate_virtual_trampoline(MonoDomain *domain,
  ...
  return mono_get_delegate_virtual_invoke_impl(
      mono_method_signature_internal(invoke), method);
}
...
gpointer mono_create_rgctx_lazy_fetch_trampoline(guint32 offset) {
  ...
  if (rgctx_lazy_fetch_trampoline_hash)
    tramp = g_hash_table_lookup(rgctx_lazy_fetch_trampoline_hash,
  ...
  g_hash_table_insert(rgctx_lazy_fetch_trampoline_hash,
                      GUINT_TO_POINTER(offset), ptr);
  g_assert(offset != -1);
...
guint32 mono_find_rgctx_lazy_fetch_trampoline_by_addr(gconstpointer addr) {
  ...
  if (rgctx_lazy_fetch_trampoline_hash_addr) {
    ...
    offset = GPOINTER_TO_INT(
        g_hash_table_lookup(rgctx_lazy_fetch_trampoline_hash_addr, addr));
    if (offset)
      offset -= 1;
    else
      offset = -1;
...
char *mono_get_rgctx_fetch_trampoline_name(int slot) {
  ...
  return g_strdup_printf("rgctx_fetch_trampoline_%s_%d",
                         mrgctx ? "mrgctx" : "rgctx", index);
}
