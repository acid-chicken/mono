...
static gboolean is_int_stack_size(int type) {
...
static gboolean is_long_stack_size(int type) {
...
static gboolean lower_load(MonoCompile *cfg, MonoInst *load, MonoInst *ldaddr) {
...
static gboolean lower_store(MonoCompile *cfg, MonoInst *store,
                            MonoInst *ldaddr) {
  MonoInst *var = (MonoInst *)ldaddr->inst_p0;
  MonoType *type = m_class_get_byval_arg(var->klass);
  ...
  if (replaced_op != store->opcode) {
    if (cfg->verbose_level > 2)
      printf("Incompatible store_reg type: expected %s but got %s\n",
             mono_inst_name(replaced_op), mono_inst_name(store->opcode));
    return FALSE;
  } else {
    if (cfg->verbose_level > 2) {
      printf("mem2reg replacing: ");
...
static gboolean lower_store_imm(MonoCompile *cfg, MonoInst *store,
                                MonoInst *ldaddr) {
  MonoInst *var = (MonoInst *)ldaddr->inst_p0;
  MonoType *type = m_class_get_byval_arg(var->klass);
...
static void kill_call_arg_alias(MonoCompile *cfg, GHashTable *addr_loads,
                                GSList *l) {
  for (; l; l = l->next) {
    MonoInst *tmp;
    guint32 regpair, reg;
    ...
    if (tmp) {
      ...
      if (g_hash_table_remove(
              addr_loads, GINT_TO_POINTER(((MonoInst *)tmp->inst_p0)->dreg))) {
        if (cfg->verbose_level > 2)
...
static gboolean lower_memory_access(MonoCompile *cfg) {
  MonoBasicBlock *bb;
  MonoInst *ins, *tmp;
    ...
    handle_instruction:
      ...
      case OP_LDADDR: {
        ...
        if (var->flags & MONO_INST_VOLATILE) {
          if (cfg->verbose_level > 2) {
            printf("Found address to volatile var, can't take it: ");
            mono_print_ins(ins);
          }
        } else {
      ...
      case OP_MOVE:
        tmp = (MonoInst *)g_hash_table_lookup(addr_loads,
                                              GINT_TO_POINTER(ins->sreg1));
        /*
      ...
      case OP_NOT_NULL:
        ...
        if (tmp) {
          if (cfg->verbose_level > 2) {
            printf("Found null check over local: ");
            mono_print_ins(ins);
          }
          NULLIFY_INS(ins);
          needs_dce = TRUE;
      ...
      default: {
        ...
        // FIXME Kill more aliases if used as dreg, since we are not in ssa
        // form. This would need some optimizations so we don't lookup hash
        // table for every instruction
        break;
      }
      }
    }
...
static gboolean recompute_aliased_variables(MonoCompile *cfg,
                                            int *restored_vars) {
  int i;
  MonoBasicBlock *bb;
  MonoInst *ins;
...
TODO:
        SRVT of small types can fix cases of mismatch for fields of a different
type than the component. Handle aliasing of byrefs in call conventions.
*/
void mono_local_alias_analysis(MonoCompile *cfg) {
  ...
  */
  ...
  /*
  By replacing indirect access with direct operations, some LDADDR ops become
  dead. Kill them.
  */
  ...
  Since indirect vars are converted into global vregs, each pass eliminates only
  one level of indirection. Most cases only need one pass and some 2.
  */
  for (i = 0; i < 3 && restored_vars > 0 &&
              recompute_aliased_variables(cfg, &restored_vars);
       ++i) {
    /*
    A lot of simplification just took place, we recompute local variables and do
    DCE to really profit from the previous gains
    */
