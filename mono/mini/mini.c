 ...
 * mono_emit_unwind_op:
 *
 *   Add an unwind op with the given parameters for the list of unwind ops
 * stored in cfg->unwind_ops.
 */
void mono_emit_unwind_op(MonoCompile *cfg, int when, int tag, int reg,
                         int val) {
  MonoUnwindOp *op =
      (MonoUnwindOp *)mono_mempool_alloc0(cfg->mempool, sizeof(MonoUnwindOp));
...
static int mono_find_block_region_notry(MonoCompile *cfg, int offset) {
  ...
  for (i = 0; i < header->num_clauses; ++i) {
    ...
    if ((clause->flags == MONO_EXCEPTION_CLAUSE_FILTER) &&
        (offset >= clause->data.filter_offset) &&
 ...
 *
 *   Return the region corresponding to REGION, ignoring try clauses nested
 * inside finally clauses.
 */
int mono_get_block_region_notry(MonoCompile *cfg, int region) {
  if ((region & (0xf << 4)) == MONO_REGION_TRY) {
    ...
    region = mono_find_block_region_notry(
        cfg, header->clauses[clause_index].try_offset);
  }
...
static void df_visit(MonoBasicBlock *start, int *dfn, MonoBasicBlock **array) {
  int i;
 ...
 */
void mono_decompose_op_imm(MonoCompile *cfg, MonoBasicBlock *bb,
                           MonoInst *ins) {
...
static void set_vreg_to_inst(MonoCompile *cfg, int vreg, MonoInst *inst) {
  if (vreg >= cfg->vreg_to_inst_len) {
    MonoInst **tmp = cfg->vreg_to_inst;
    int size = cfg->vreg_to_inst_len;

    while (vreg >= cfg->vreg_to_inst_len)
      cfg->vreg_to_inst_len =
          cfg->vreg_to_inst_len ? cfg->vreg_to_inst_len * 2 : 32;
    cfg->vreg_to_inst = (MonoInst **)mono_mempool_alloc0(
        cfg->mempool, sizeof(MonoInst *) * cfg->vreg_to_inst_len);
    if (size)
...
#define mono_type_is_long(type)                                                \
  (!(type)->byref &&                                                           \
   ((mono_type_get_underlying_type(type)->type == MONO_TYPE_I8) ||             \
    (mono_type_get_underlying_type(type)->type == MONO_TYPE_U8)))
#define mono_type_is_float(type)                                               \
  (!(type)->byref &&                                                           \
   (((type)->type == MONO_TYPE_R8) || ((type)->type == MONO_TYPE_R4)))
...
MonoInst *mono_compile_create_var_for_vreg(MonoCompile *cfg, MonoType *type,
  ...
  if ((num + 1) >= cfg->varinfo_count) {
    ...
    cfg->varinfo = (MonoInst **)g_realloc(cfg->varinfo, sizeof(MonoInst *) *
                                                            cfg->varinfo_count);
    cfg->vars = (MonoMethodVar *)g_realloc(cfg->vars, sizeof(MonoMethodVar) *
                                                          cfg->varinfo_count);
    memset(&cfg->vars[orig_count], 0,
           (cfg->varinfo_count - orig_count) * sizeof(MonoMethodVar));
  }
  ...
  if (cfg->compute_gc_maps) {
    if (type->byref) {
      ...
      if ((MONO_TYPE_ISSTRUCT(type) && m_class_has_references(inst->klass)) ||
  ...
  if (regpair) {
    ...
    if (cfg->verbose_level >= 4) {
      printf("  Create LVAR R%d (R%d, R%d)\n", inst->dreg,
             MONO_LVREG_LS(inst->dreg), MONO_LVREG_MS(inst->dreg));
    }
...
MonoInst *mono_compile_create_var(MonoCompile *cfg, MonoType *type,
  ...
  if (type->type == MONO_TYPE_VALUETYPE && !type->byref) {
    ...
    if (m_class_is_enumtype(klass) &&
        m_class_get_image(klass) == mono_get_corlib() &&
        !strcmp(m_class_get_name(klass), "StackCrawlMark")) {
      if (!(cfg->method->flags & METHOD_ATTRIBUTE_REQSECOBJ))
        g_error(
            "Method '%s' which contains a StackCrawlMark local variable must "
            "be decorated with [System.Security.DynamicSecurityMethod].",
...
MonoInst *mini_get_int_to_float_spill_area(MonoCompile *cfg) {
  ...
  if (!cfg->iconv_raw_var) {
    cfg->iconv_raw_var =
        mono_compile_create_var(cfg, mono_get_int32_type(), OP_LOCAL);
    cfg->iconv_raw_var->flags |=
...
void mono_mark_vreg_as_ref(MonoCompile *cfg, int vreg) {
  if (vreg >= cfg->vreg_is_ref_len) {
    ...
    while (vreg >= cfg->vreg_is_ref_len)
      cfg->vreg_is_ref_len =
          cfg->vreg_is_ref_len ? cfg->vreg_is_ref_len * 2 : 32;
    cfg->vreg_is_ref = (gboolean *)mono_mempool_alloc0(
        cfg->mempool, sizeof(gboolean) * cfg->vreg_is_ref_len);
    if (size)
...
void mono_mark_vreg_as_mp(MonoCompile *cfg, int vreg) {
  if (vreg >= cfg->vreg_is_mp_len) {
    ...
    cfg->vreg_is_mp = (gboolean *)mono_mempool_alloc0(
        cfg->mempool, sizeof(gboolean) * cfg->vreg_is_mp_len);
    if (size)
  ...
  default:
    if (MONO_IS_COND_BRANCH_OP(bb->last_ins)) {
      ...
      if (bb->code->next == bb->last_ins) {
        ...
        if ((opcode == OP_COMPARE) || (opcode == OP_COMPARE_IMM) ||
            (opcode == OP_ICOMPARE) || (opcode == OP_ICOMPARE_IMM) ||
            (opcode == OP_FCOMPARE) || (opcode == OP_LCOMPARE) ||
            (opcode == OP_LCOMPARE_IMM) || (opcode == OP_RCOMPARE)) {
          /* NEW IR */
...
void mono_create_jump_table(MonoCompile *cfg, MonoInst *label,
  ...
  MonoJumpInfo *ji =
      (MonoJumpInfo *)mono_mempool_alloc(cfg->mempool, sizeof(MonoJumpInfo));
 ...
 */
static gboolean mini_method_verify(MonoCompile *cfg, MonoMethod *method,
...
gboolean mono_compile_is_broken(MonoCompile *cfg, MonoMethod *method,
  ...
  gboolean dont_verify =
      m_class_get_image(method->klass)->assembly->corlib_internal;
  ...
  return !dont_verify &&
         mini_method_verify(cfg, method_definition, fail_compile);
}

static void mono_dynamic_code_hash_insert(MonoDomain *domain,
                                          MonoMethod *method,
...
static gint compare_by_interval_start_pos_func(gconstpointer a,
                                               gconstpointer b) {
...
#define LSCAN_DEBUG(a)                                                         \
  do {                                                                         \
  } while (0) /* non-empty to avoid warning */
#endif

static gint32 *mono_allocate_stack_slots2(MonoCompile *cfg, gboolean backward,
                                          guint32 *stack_size,
                                          guint32 *stack_align) {
  ...
  scalar_stack_slots = (StackSlotInfo *)mono_mempool_alloc0(
      cfg->mempool, sizeof(StackSlotInfo) * MONO_TYPE_PINNED);
  vtype_stack_slots = NULL;
  nvtypes = 0;

  offsets = (gint32 *)mono_mempool_alloc(cfg->mempool,
                                         sizeof(gint32) * cfg->num_varinfo);
  ...
  *stack_align = 0;
  for (unhandled = vars; unhandled; unhandled = unhandled->next) {
     ...
     * by the pinvoke wrappers when they call functions returning structures */
    if (inst->backend.is_pinvoke && MONO_TYPE_ISSTRUCT(t) &&
        t->type != MONO_TYPE_TYPEDBYREF) {
      size =
          mono_class_native_size(mono_class_from_mono_type_internal(t), &align);
    } else {
      int ialign;
    ...
    case MONO_TYPE_VALUETYPE:
      if (!vtype_stack_slots)
        vtype_stack_slots = (StackSlotInfo *)mono_mempool_alloc0(
            cfg->mempool, sizeof(StackSlotInfo) * vtype_stack_slots_size);
      for (i = 0; i < nvtypes; ++i)
      ...
      if (i < nvtypes)
        ...
        if (nvtypes == vtype_stack_slots_size) {
          ...
          StackSlotInfo *new_slots = (StackSlotInfo *)mono_mempool_alloc0(
              cfg->mempool, sizeof(StackSlotInfo) * new_slots_size);

          memcpy(new_slots, vtype_stack_slots,
                 sizeof(StackSlotInfo) * vtype_stack_slots_size);
    ...
    if (cfg->comp_done & MONO_COMP_LIVENESS) {
      ...
      while (changed) {
        ...
        for (l = slot_info->active; l != NULL; l = l->next) {
          ...
          if (v->interval->last_range->to < pos) {
            ...
            slot_info->slots =
                g_slist_prepend_mempool(cfg->mempool, slot_info->slots,
                                        GINT_TO_POINTER(offsets[v->idx]));
            LSCAN_DEBUG(
                printf("Interval R%d has expired, adding 0x%x to slots\n",
      ...
      while (changed) {
        ...
        for (l = slot_info->inactive; l != NULL; l = l->next) {
          ...
          if (v->interval->last_range->to < pos) {
            ...
            // slot_info->slots = g_slist_prepend_mempool (cfg->mempool,
            // slot_info->slots, GINT_TO_POINTER (offsets [v->idx]));
            LSCAN_DEBUG(
                printf("Interval R%d has expired, adding 0x%x to slots\n",
                       cfg->varinfo[v->idx]->dreg, offsets[v->idx]));
            changed = TRUE;
      ...
      if (!(inst->flags & (MONO_INST_VOLATILE | MONO_INST_INDIRECT))) {
        ...
        /* FIXME: We might want to consider the inactive intervals as well if
         * slot_info->slots is empty */

        slot_info->active =
            mono_varlist_insert_sorted(cfg, slot_info->active, vmv, TRUE);
      }
    }
    ...
    if (slot == 0xffffff) {
      ...
      if (MONO_TYPE_ISSTRUCT(t)) {
        ...
        align = MAX(
            align, mono_class_min_align(mono_class_from_mono_type_internal(t)));
      }
  ...
  for (l = vars; l; l = l->next) {
    ...
    if (cfg->comp_done & MONO_COMP_LIVENESS) {
      ...
      while (slot_info->active) {
        ...
        // amv->spill_costs, amv->reg);

        slot_info->active =
            g_list_delete_link(slot_info->active, slot_info->active);
        slot_info->slots = g_slist_prepend_mempool(
            cfg->mempool, slot_info->slots, GINT_TO_POINTER(offsets[amv->idx]));
      }
    ...
    if (inst->flags & MONO_INST_LMF) {
      /*
       * This variable represents a MonoLMF structure, which has no
       * corresponding CLR type, so hard-code its size/alignment.
       */
      size = MONO_ABI_SIZEOF(MonoLMF);
      align = sizeof(target_mgreg_t);
      reuse_slot = FALSE;
...
MonoJitICallInfo *mono_find_jit_opcode_emulation(int opcode) {
  ...
  if (emul_opcode_hit_cache[opcode >> (EMUL_HIT_SHIFT + 3)] &
      (1 << (opcode & EMUL_HIT_MASK))) {
    int i;
...
void mini_register_opcode_emulation(int opcode, MonoJitICallInfo *info,
  ...
  if (emul_opcode_num >= emul_opcode_alloced) {
    ...
    emul_opcode_map = (MonoJitICallInfo **)g_realloc(
        emul_opcode_map, sizeof(emul_opcode_map[0]) * emul_opcode_alloced);
    emul_opcode_opcodes =
        (short *)g_realloc(emul_opcode_opcodes, sizeof(emul_opcode_opcodes[0]) *
                                                    emul_opcode_alloced);
  }
  emul_opcode_map[emul_opcode_num] = info;
  emul_opcode_opcodes[emul_opcode_num] = opcode;
  emul_opcode_num++;
  emul_opcode_hit_cache[opcode >> (EMUL_HIT_SHIFT + 3)] |=
      (1 << (opcode & EMUL_HIT_MASK));
}

static void print_dfn(MonoCompile *cfg) {
  int i, j;
  char *code;
  MonoBasicBlock *bb;
  MonoInst *c;
  ...
  for (i = 0; i < cfg->num_bblocks; ++i) {
    ...
    /*if (bb->cil_code) {
            char* code1, *code2;
            code1 = mono_disasm_code_one (NULL, cfg->method, bb->cil_code,
    NULL); if (bb->last_ins->cil_code) code2 = mono_disasm_code_one (NULL,
    cfg->method, bb->last_ins->cil_code, NULL); else code2 = g_strdup ("");
    ...
    code = g_strdup("\n");
    g_print("\nBB%d (%d) (len: %d): %s", bb->block_num, i, bb->cil_length,
            code);
    MONO_BB_FOR_EACH_INS(bb, c) { mono_print_ins_index(-1, c); }
    ...
    if (bb->dominators)
      mono_blockset_print(cfg, bb->dominators, "\tdominators",
                          bb->idom ? bb->idom->dfn : -1);
    if (bb->dfrontier)
 ...
 * mono_verify_bblock:
 *
 *   Verify that the next and prev pointers are consistent inside the
 * instructions in BB.
 */
...
void mono_add_patch_info(MonoCompile *cfg, int ip, MonoJumpInfoType type,
  ...
  MonoJumpInfo *ji =
      (MonoJumpInfo *)mono_mempool_alloc0(cfg->mempool, sizeof(MonoJumpInfo));
...
void mono_add_var_location(MonoCompile *cfg, MonoInst *var, gboolean is_reg,
  ...
  MonoDwarfLocListEntry *entry = (MonoDwarfLocListEntry *)mono_mempool_alloc0(
      cfg->mempool, sizeof(MonoDwarfLocListEntry));
  ...
  if (var == cfg->args[0])
    cfg->this_loclist =
        g_slist_append_mempool(cfg->mempool, cfg->this_loclist, entry);
  else if (var == cfg->rgctx_var)
    cfg->rgctx_loclist =
        g_slist_append_mempool(cfg->mempool, cfg->rgctx_loclist, entry);
}

static void mono_apply_volatile(MonoInst *inst, MonoBitSet *set, gsize index) {
  inst->flags |= mono_bitset_test_safe(set, index) ? MONO_INST_VOLATILE : 0;
}

static void mono_compile_create_vars(MonoCompile *cfg) {
  ...
  cfg->args = (MonoInst **)mono_mempool_alloc0(
      cfg->mempool, (sig->param_count + sig->hasthis) * sizeof(MonoInst *));

  if (sig->hasthis) {
    MonoInst *arg = mono_compile_create_var(
        cfg, m_class_get_this_arg(cfg->method->klass), OP_ARG);
  ...
  cfg->locals = (MonoInst **)mono_mempool_alloc0(
      cfg->mempool, header->num_locals * sizeof(MonoInst *));
...
static void mono_postprocess_patches(MonoCompile *cfg) {
  ...
  for (patch_info = cfg->patch_info; patch_info;
    ...
    case MONO_PATCH_INFO_ABS: {
       ...
       */
      if (cfg->abs_patches) {
        MonoJumpInfo *abs_ji = (MonoJumpInfo *)g_hash_table_lookup(
            cfg->abs_patches, patch_info->data.target);
        if (abs_ji) {
    ...
    case MONO_PATCH_INFO_SWITCH: {
      ...
      if (cfg->method->dynamic) {
        table = (void **)mono_code_manager_reserve(
            cfg->dynamic_info->code_mp,
            sizeof(gpointer) * patch_info->data.table->table_size);
      } else {
        table = (void **)mono_domain_code_reserve(
            cfg->domain, sizeof(gpointer) * patch_info->data.table->table_size);
      }
      ...
      for (i = 0; i < patch_info->data.table->table_size; i++) {
        ...
        if (patch_info->data.table->table[i]) {
          ...
          table[i] =
              GINT_TO_POINTER(patch_info->data.table->table[i]->native_offset);
        } else {
          table[i] = NULL;
        }
      }
...
/* Those patches require the JitInfo of the compiled method already be in place
 * when used */
static void mono_postprocess_patches_after_ji_publish(MonoCompile *cfg) {
  MonoJumpInfo *patch_info;
...
void mono_codegen(MonoCompile *cfg) {
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    if (bb->clause_holes) {
      ...
      for (tmp = bb->clause_holes; tmp; tmp = tmp->prev)
        mono_cfg_add_try_hole(cfg, ((MonoLeaveClause *)tmp->data)->clause,
                              cfg->native_code + bb->native_offset, bb);
    }
  }
  ...
  if (cfg->method->dynamic) {
    ...
    if (mono_using_xdebug)
      ...
      code = (guint8 *)mono_domain_code_reserve(
          code_domain, cfg->code_size + cfg->thunk_area + unwindlen);
    else
      code = (guint8 *)mono_code_manager_reserve(
          cfg->dynamic_info->code_mp,
          cfg->code_size + cfg->thunk_area + unwindlen);
  } else {
    code = (guint8 *)mono_domain_code_reserve(
        code_domain, cfg->code_size + cfg->thunk_area + unwindlen);
  }
  ...
  if (valgrind_register) {
    ...
    VALGRIND_JIT_REGISTER_MAP(nm, cfg->native_code,
...

  {
    ...
    if (cfg->method->is_inflated ||
        mono_method_get_generic_container(cfg->method) ||
        mono_class_is_gtd(cfg->method->klass) ||
        mono_class_is_ginst(cfg->method->klass)) {
      is_generic = TRUE;
    }
...
#ifdef MONO_ARCH_HAVE_PATCH_CODE_NEW
  {
    ...
    for (ji = cfg->patch_info; ji; ji = ji->next) {
      ...
      target =
          mono_resolve_patch_target(cfg->method, cfg->domain, cfg->native_code,
                                    ji, cfg->run_cctors, cfg->error);
      if (!is_ok(cfg->error)) {
        mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);
        return;
      }
...
#else
  mono_arch_patch_code(cfg, cfg->method, cfg->domain, cfg->native_code,
                       cfg->patch_info, cfg->run_cctors, cfg->error);
  if (!is_ok(cfg->error)) {
    mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);
    return;
  }
  ...
  MONO_PROFILER_RAISE(jit_code_buffer,
                      (cfg->native_code, cfg->code_len,
                       MONO_PROFILER_CODE_BUFFER_METHOD, cfg->method));
...
#ifdef MONO_ARCH_HAVE_UNWIND_TABLE
  if (!cfg->compile_aot)
    mono_arch_unwindinfo_install_method_unwind_info(
        &cfg->arch.unwindinfo, cfg->native_code, cfg->code_len);
#endif
}

static void compute_reachable(MonoBasicBlock *bb) {
  int i;
...
static void mono_bb_ordering(MonoCompile *cfg) {
  ...
  cfg->bblocks = (MonoBasicBlock **)mono_mempool_alloc(
      cfg->mempool, sizeof(MonoBasicBlock *) * (cfg->num_bblocks + 1));
...
static void mono_handle_out_of_line_bblock(MonoCompile *cfg) {
  MonoBasicBlock *bb;
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    if (bb->next_bb && bb->next_bb->out_of_line && bb->last_ins &&
        !MONO_IS_BRANCH_OP(bb->last_ins)) {
      MonoInst *ins;
...
static MonoJitInfo *create_jit_info(MonoCompile *cfg,
  ...
  if (cfg->arch_eh_jit_info) {
    ...
    MonoMethodSignature *sig =
        mono_method_signature_internal(cfg->method_to_register);
  ...
  if (cfg->try_block_holes) {
    for (tmp = cfg->try_block_holes; tmp; tmp = tmp->next) {
      ...
      int hole_end =
          hole->basic_block->native_offset + hole->basic_block->native_length;
      MonoBasicBlock *clause_last_bb =
          cfg->cil_offset_to_bb[ec->try_offset + ec->try_len];
      g_assert(clause_last_bb);

      /* Holes at the end of a try region can be represented by simply reducing
  ...
  if (cfg->method->dynamic)
    jinfo = (MonoJitInfo *)g_malloc0(
        mono_jit_info_size(flags, num_clauses, num_holes));
  else
    jinfo = (MonoJitInfo *)mono_domain_alloc0(
        cfg->domain, mono_jit_info_size(flags, num_clauses, num_holes));
  jinfo_try_holes_size += num_holes * sizeof(MonoTryBlockHoleJitInfo);

  mono_jit_info_init(jinfo, cfg->method_to_register, cfg->native_code,
                     cfg->code_len, flags, num_clauses, num_holes);
  ...
  if (cfg->gshared) {
    ...
    if (cfg->method->dynamic)
      ...
      gi->generic_sharing_context =
          (MonoGenericSharingContext *)mono_domain_alloc0(
              cfg->domain, sizeof(MonoGenericSharingContext));
    mini_init_gsctx(cfg->method->dynamic ? NULL : cfg->domain, NULL,
                    cfg->gsctx_context, gi->generic_sharing_context);
    ...
    if (loclist) {
      ...
      if (cfg->method->dynamic)
        gi->locations = (MonoDwarfLocListEntry *)g_malloc0(
            gi->nlocs * sizeof(MonoDwarfLocListEntry));
      else
        gi->locations = (MonoDwarfLocListEntry *)mono_domain_alloc0(
            cfg->domain, gi->nlocs * sizeof(MonoDwarfLocListEntry));
      i = 0;
...
#endif
      g_assert(inst->inst_offset >= G_MININT32 &&
               inst->inst_offset <= G_MAXINT32);
  ...
  if (num_holes) {
    ...
    for (tmp = cfg->try_block_holes; tmp; tmp = tmp->next) {
      ...
      int hole_end = hole_data->basic_block->native_offset +
                     hole_data->basic_block->native_length;
      MonoBasicBlock *clause_last_bb =
          cfg->cil_offset_to_bb[ec->try_offset + ec->try_len];
      g_assert(clause_last_bb);
      ...
      start_bb_offset =
          hole_data->start_offset - hole_data->basic_block->native_offset;
      hole = &table->holes[i++];
      hole->clause = hole_data->clause - &header->clauses[0];
      hole->offset = (guint32)hole_data->start_offset;
      hole->length =
          (guint16)(hole_data->basic_block->native_length - start_bb_offset);
  ...
  if (COMPILE_LLVM(cfg)) {
    if (num_clauses)
      memcpy(&jinfo->clauses[0], &cfg->llvm_ex_info[0],
    ...
    for (i = 0; i < header->num_clauses; i++) {
      ...
      if (G_UNLIKELY(cfg->verbose_level >= 4))
        printf("IL clause: try 0x%x-0x%x handler 0x%x-0x%x filter 0x%x\n",
               ec->try_offset, ec->try_offset + ec->try_len, ec->handler_offset,
               ec->handler_offset + ec->handler_len,
               ec->flags == MONO_EXCEPTION_CLAUSE_FILTER
                   ? ec->data.filter_offset
                   : 0);
      ...
      if (tblock->extend_try_block) {
         ...
         */
        ei->try_start =
            (guint8 *)ei->try_start - cfg->backend->monitor_enter_adjustment;
      }
      ...
      if (!tblock->native_offset) {
        int j, end;
        for (j = ec->try_offset + ec->try_len, end = ec->try_offset; j >= end;
             --j) {
      ...
      for (tmp = cfg->try_block_holes; tmp; tmp = tmp->next) {
        TryBlockHole *hole = (TryBlockHole *)tmp->data;
        gpointer hole_end =
            cfg->native_code + (hole->basic_block->native_offset +
                                hole->basic_block->native_length);
        if (hole->clause == ec && hole_end == ei->try_end) {
          if (G_UNLIKELY(cfg->verbose_level >= 4))
            printf("\tShortening try block %d from %x to %x\n", i,
                   (int)((guint8 *)ei->try_end - cfg->native_code),
                   hole->start_offset);
      ...
      if (ec->flags == MONO_EXCEPTION_CLAUSE_FINALLY) {
        ...
        if (ec->handler_offset + ec->handler_len < header->code_size) {
          ...
          if (tblock->native_offset) {
            ...
            for (j = ec->handler_offset + ec->handler_len,
                end = ec->handler_offset;
                 j >= end; --j) {
              MonoBasicBlock *bb = cfg->cil_offset_to_bb[j];
              if (bb && bb->native_offset) {
                tblock = bb;
  ...
  if (G_UNLIKELY(cfg->verbose_level >= 4)) {
    ...
    for (i = 0; i < jinfo->num_clauses; i++) {
      ...
      printf("JitInfo EH clause %d flags %x try %x-%x handler %x-%x\n", i,
             ei->flags, start, end, handler, handler_end);
    }
  }

  if (cfg->encoded_unwind_ops) {
    /* Generated by LLVM */
    jinfo->unwind_info = mono_cache_unwind_info(cfg->encoded_unwind_ops,
                                                cfg->encoded_unwind_ops_len);
...
static gboolean is_gsharedvt_method(MonoMethod *method) {
...
static gboolean is_open_method(MonoMethod *method) {
  MonoGenericContext *context;
...
static void insert_safepoint(MonoCompile *cfg, MonoBasicBlock *bblock) {
  ...
  NEW_AOTCONST(cfg, poll_addr, MONO_PATCH_INFO_GC_SAFE_POINT_FLAG,
               (gpointer)&mono_polling_required);
  ...
  if (bblock->flags & BB_EXCEPTION_HANDLER) {
    ...
    if (eh_op && eh_op->opcode != OP_START_HANDLER &&
        eh_op->opcode != OP_GET_EX_OBJ) {
      eh_op = NULL;
    } else {
      MonoInst *next_eh_op = eh_op ? eh_op->next : NULL;
      // skip all EH relateds ops
      while (next_eh_op && (next_eh_op->opcode == OP_START_HANDLER ||
                            next_eh_op->opcode == OP_GET_EX_OBJ)) {
        eh_op = next_eh_op;
...
*/
static void insert_safepoints(MonoCompile *cfg) {
  MonoBasicBlock *bb;
  ...
  if (cfg->method->wrapper_type == MONO_WRAPPER_MANAGED_TO_NATIVE) {
    ...
    /* These wrappers are called from the wrapper for the polling function,
     * leading to potential stack overflow */
    if (info && info->subtype == WRAPPER_SUBTYPE_ICALL_WRAPPER &&
        (info->d.icall.jit_icall_id == MONO_JIT_ICALL_mono_threads_state_poll ||
         info->d.icall.jit_icall_id ==
  ...
  if (cfg->method->wrapper_type == MONO_WRAPPER_OTHER) {
    ...
    if (info && (info->subtype == WRAPPER_SUBTYPE_INTERP_IN ||
...
static void mono_insert_branches_between_bblocks(MonoCompile *cfg) {
  MonoBasicBlock *bb;
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    if (bb->last_ins && MONO_IS_COND_BRANCH_OP(bb->last_ins) &&
       ...
       */
      if (MONO_IS_COND_BRANCH_NOFP(bb->last_ins) &&
          bb->next_bb == bb->last_ins->inst_true_bb) {
...
static void init_backend(MonoBackend *backend) {
  ...
  backend->have_volatile_non_param_register =
...
static gboolean is_simd_supported(MonoCompile *cfg) {
#ifdef DISABLE_SIMD
  return FALSE;
#endif
  // FIXME: Clean this up
...
restart_compile:
  ...
  cfg->gen_seq_points = !mini_debug_options.no_seq_points_compact_data ||
                        mini_debug_options.gen_sdb_seq_points;
  ...
  cfg->disable_llvm_implicit_null_checks =
      mini_debug_options.llvm_disable_implicit_null_checks;
  if (cfg->backend->explicit_null_checks ||
      mini_debug_options.explicit_null_checks) {
  ...
  */
  ...
  if (cfg->compile_aot && !try_generic_shared &&
      (method->is_generic || mono_class_is_gtd(method->klass) ||
       method_is_gshared)) {
  ...
  if (cfg->gshared &&
      (gsharedvt_method || mini_is_gsharedvt_sharable_method(method))) {
  ...
  header = cfg->header =
      mono_method_get_header_checked(cfg->method, cfg->error);
  if (!header) {
    mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);
    if (MONO_METHOD_COMPILE_END_ENABLED())
      MONO_PROBE_METHOD_COMPILE_END(method, FALSE);
  ...
  cfg->prof_coverage =
      mono_profiler_coverage_instrumentation_enabled(cfg->method);
  ...
  /* The debugger has no liveness information, so avoid sharing registers/stack
   * slots */
  if (mini_debug_options.mdb_optimizations ||
    ...
    /*
     * This decreases the change the debugger will read registers/stack slots
    ...
    /* Don't remove unused variables when running inside the debugger since the
     * user may still want to view them. */
    ...
    /* This is needed for the soft debugger, which doesn't like code after the
     * epilog */
  ...
  if (method->wrapper_type == MONO_WRAPPER_OTHER) {
    ...
    if ((info && (info->subtype == WRAPPER_SUBTYPE_GSHAREDVT_IN_SIG ||
  ...
  cfg->intvars = (guint16 *)mono_mempool_alloc0(
      cfg->mempool, sizeof(guint16) * STACK_MAX * header->max_stack);

  if (cfg->verbose_level > 0) {
    char *method_name;

    method_name = mono_method_get_full_name(method);
    g_print("converting %s%s%smethod %s\n", COMPILE_LLVM(cfg) ? "llvm " : "",
            cfg->gsharedvt ? "gsharedvt " : "",
            (cfg->gshared && !cfg->gsharedvt) ? "gshared " : "", method_name);
    /*
    if (COMPILE_LLVM (cfg))
            g_print ("converting llvm method %s\n", method_name =
    mono_method_full_name (method, TRUE)); else if (cfg->gsharedvt) g_print
    ("converting gsharedvt method %s\n", method_name = mono_method_full_name
    (method_to_compile, TRUE)); else if (cfg->gshared) g_print ("converting
    shared method %s\n", method_name = mono_method_full_name (method_to_compile,
    TRUE)); else g_print ("converting method %s\n", method_name =
  ...
  /*
   * FIXME: This confuses liveness analysis because variables which are assigned
   * after a branch inside a bblock become part of the kill set, even though the
   * assignment might not get executed. This causes the optimize_initlocals pass
   * to delete some assignments which are needed. Also, the mono_if_conversion
   * pass needs to be modified to recognize the code created by this.
   */
  // cfg->enable_extended_bblocks = TRUE;

  /*We must verify the method before doing any IR generation as
   * mono_compile_create_vars can assert.*/
  ...
  MONO_TIME_TRACK(mono_jit_stats.jit_method_to_ir,
                  i = mono_method_to_ir(cfg, method_to_compile, NULL, NULL,
                                        NULL, NULL, 0, FALSE));
  mono_cfg_dump_ir(cfg, "method-to-ir");

  if (cfg->gdump_ctx != NULL) {
    /* workaround for graph visualization, as it doesn't handle empty basic
     * blocks properly */
  ...
  /* todo: remove code when we have verified that the liveness for try/catch
   * blocks works perfectly
   */
  /*
   ...
   * r1 <- 2
   * This also allows SSA to be run on methods containing exception clauses,
   * since SSA will ignore variables marked VOLATILE.
   */
  MONO_TIME_TRACK(mono_jit_stats.jit_liveness_handle_exception_clauses,
                  mono_liveness_handle_exception_clauses(cfg));
  mono_cfg_dump_ir(cfg, "liveness_handle_exception_clauses");

  MONO_TIME_TRACK(mono_jit_stats.jit_handle_out_of_line_bblock,
  ...
  if (!COMPILE_LLVM(cfg)) {
    MONO_TIME_TRACK(mono_jit_stats.jit_decompose_long_opts,
  ...
  if (cfg->flags & MONO_CFG_HAS_TYPE_CHECK) {
    MONO_TIME_TRACK(mono_jit_stats.jit_decompose_typechecks,
  ...
  if (cfg->opt & MONO_OPT_ALIAS_ANALYSIS) {
    MONO_TIME_TRACK(mono_jit_stats.jit_local_alias_analysis,
  ...
  if (cfg->opt & MONO_OPT_LOOP) {
    ...
    MONO_TIME_TRACK(mono_jit_stats.jit_compute_natural_loops,
                    mono_compute_natural_loops(cfg));
  }

  if (mono_threads_are_safepoints_enabled()) {
    MONO_TIME_TRACK(mono_jit_stats.jit_insert_safepoints,
...
//#define DEBUGSSA_CLASS "Tests"
  ...
  if (cfg->comp_done & MONO_COMP_SSA && !COMPILE_LLVM(cfg)) {
    ...
    if ((cfg->flags & (MONO_CFG_HAS_LDELEMA | MONO_CFG_HAS_CHECK_THIS)) &&
        (cfg->opt & MONO_OPT_ABCREM)) {
      MONO_TIME_TRACK(mono_jit_stats.jit_perform_abc_removal,
    ...
    MONO_TIME_TRACK(mono_jit_stats.jit_handle_global_vregs2,
                    mono_handle_global_vregs(cfg));
    ...
    if (cfg->opt & MONO_OPT_BRANCH) {
      MONO_TIME_TRACK(mono_jit_stats.jit_optimize_branches2,
                      mono_optimize_branches(cfg));
...
#endif
  MONO_TIME_TRACK(mono_jit_stats.jit_decompose_vtype_opts,
                  mono_decompose_vtype_opts(cfg));
  if (cfg->flags & MONO_CFG_NEEDS_DECOMPOSE) {
    MONO_TIME_TRACK(mono_jit_stats.jit_decompose_array_access_opts,
   ...
   */
  MONO_TIME_TRACK(mono_jit_stats.jit_liveness_handle_exception_clauses2,
                  mono_liveness_handle_exception_clauses(cfg));
  ...
  if (cfg->opt & MONO_OPT_LINEARS) {
    ...
    if (!(cfg->comp_done & MONO_COMP_LIVENESS))
      MONO_TIME_TRACK(mono_jit_stats.jit_analyze_liveness,
    ...
    if ((vars = mono_arch_get_allocatable_int_vars(cfg))) {
      ...
      MONO_TIME_TRACK(mono_jit_stats.jit_linear_scan,
   ...
   */
  if (!COMPILE_LLVM(cfg)) {
    MONO_TIME_TRACK(mono_jit_stats.jit_arch_allocate_vars,
  ...
  if (!COMPILE_LLVM(cfg)) {
    ...
    MONO_TIME_TRACK(mono_jit_stats.jit_spill_global_vars,
    ...
    if (need_local_opts || cfg->compile_aot) {
      ...
      if (cfg->opt & MONO_OPT_DEADCE)
        MONO_TIME_TRACK(mono_jit_stats.jit_local_deadce3,
  ...
  MONO_TIME_TRACK(mono_jit_stats.jit_create_jit_info,
  ...
  MONO_TIME_TRACK(mono_jit_stats.jit_gc_create_gc_map,
                  mini_gc_create_gc_map(cfg));
  MONO_TIME_TRACK(mono_jit_stats.jit_save_seq_point_info,
                  mono_save_seq_point_info(cfg, cfg->jit_info));
  ...
  if (!cfg->compile_aot && !(flags & JIT_FLAG_DISCARD_RESULTS)) {
    ...
    if (cfg->method->dynamic)
      mono_dynamic_code_hash_lookup(cfg->domain, cfg->method)->ji =
          cfg->jit_info;
  ...
  mono_atomic_fetch_add_i32(&mono_jit_stats.allocated_code_size,
                            code_size_ratio);
  ...
  if ((gboolean)mono_atomic_load_i32((gint32 *)&mono_jit_stats.enabled)) {
    if (code_size_ratio >
        mono_atomic_load_i32(&mono_jit_stats.biggest_method_size)) {
      mono_atomic_store_i32(&mono_jit_stats.biggest_method_size,
                            code_size_ratio);
      char *biggest_method = g_strdup_printf(
          "%s::%s)", m_class_get_name(method->klass), method->name);
      biggest_method = (char *)mono_atomic_xchg_ptr(
          (gpointer *)&mono_jit_stats.biggest_method, biggest_method);
      g_free(biggest_method);
    }
    code_size_ratio = (code_size_ratio * 100) / header->code_size;
    if (code_size_ratio >
        mono_atomic_load_i32(&mono_jit_stats.max_code_size_ratio)) {
      mono_atomic_store_i32(&mono_jit_stats.max_code_size_ratio,
                            code_size_ratio);
      char *max_ratio_method = g_strdup_printf(
          "%s::%s)", m_class_get_name(method->klass), method->name);
      max_ratio_method = (char *)mono_atomic_xchg_ptr(
          (gpointer *)&mono_jit_stats.max_ratio_method, max_ratio_method);
...
void mono_cfg_add_try_hole(MonoCompile *cfg, MonoExceptionClause *clause,
  ...
  TryBlockHole *hole =
      (TryBlockHole *)mono_mempool_alloc(cfg->mempool, sizeof(TryBlockHole));
  hole->clause = clause;
  hole->start_offset = start - cfg->native_code;
  hole->basic_block = bb;

  cfg->try_block_holes =
      g_slist_append_mempool(cfg->mempool, cfg->try_block_holes, hole);
}
...
void mono_cfg_set_exception_invalid_program(MonoCompile *cfg, char *msg) {
  mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);
  mono_error_set_generic_error(cfg->error, "System", "InvalidProgramException",
                               "%s", msg);
}

#endif /* DISABLE_JIT */

gint64 mono_time_track_start() { return mono_100ns_ticks(); }
...
void mono_update_jit_stats(MonoCompile *cfg) {
  ...
  mono_jit_stats.max_basic_blocks =
      MAX(cfg->stat_basic_blocks, mono_jit_stats.max_basic_blocks);
...
gpointer mono_jit_compile_method_inner(MonoMethod *method,
  ...
  case MONO_EXCEPTION_INVALID_PROGRAM: {
    ...
    if (cfg->exception_ptr) {
      ex =
          mono_class_get_exception_for_failure((MonoClass *)cfg->exception_ptr);
    } else {
      if (cfg->exception_type == MONO_EXCEPTION_MISSING_FIELD)
        ex = mono_exception_from_name_msg(mono_defaults.corlib, "System",
        ...
        ex = mono_exception_from_name_msg(mono_defaults.corlib, "System.IO",
  ...
  if (mono_stats_method_desc &&
      mono_method_desc_full_match(mono_stats_method_desc, method)) {
  ...
  if (code == NULL) {
    ...
    mono_internal_hash_table_insert(&target_domain->jit_code_hash,
                                    cfg->jit_info->d.method, cfg->jit_info);
  ...
  if (method->wrapper_type == MONO_WRAPPER_MANAGED_TO_NATIVE) {
    if (mono_marshal_method_from_wrapper(method)) {
      ...
      /* The profiler doesn't know about wrappers, so pass the original icall
       * method */
      MONO_PROFILER_RAISE(jit_done,
...
void mini_jit_init(void) {
  mono_counters_register("Discarded method code",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT, &discarded_code);
  mono_counters_register("Time spent JITting discarded code",
                         MONO_COUNTER_JIT | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &discarded_jit_time);
  mono_counters_register("Try holes memory size",
...
#ifndef ENABLE_LLVM
void mono_llvm_emit_aot_file_info(MonoAotFileInfo *info,
                                  gboolean has_jitted_code) {
  g_assert_not_reached();
}

gpointer mono_llvm_emit_aot_data(const char *symbol, guint8 *data,
                                 int data_len) {
  g_assert_not_reached();
}

gpointer mono_llvm_emit_aot_data_aligned(const char *symbol, guint8 *data,
                                         int data_len, int align) {
  g_assert_not_reached();
}
...
void mono_llvm_cpp_throw_exception(void) { g_assert_not_reached(); }

void mono_llvm_cpp_catch_exception(MonoLLVMInvokeCallback cb, gpointer arg,
                                   gboolean *out_thrown) {
  g_assert_not_reached();
}
...
void mono_destroy_compile(MonoCompile *cfg) { g_assert_not_reached(); }
...
MonoCPUFeatures mini_get_cpu_features(MonoCompile *cfg) {
  ...
  if (!cfg->compile_aot || cfg->use_current_cpu) {
    // detect current CPU features if we are in JIT mode or AOT with
    // use_current_cpu flag.
#if defined(ENABLE_LLVM)
    features = mono_llvm_get_cpu_features(); // llvm has a nice built-in API to
                                             // detect features
