...
static const MagicTypeInfo type_info[] = {
    // nint
    {0, STACK_I8, STACK_I4, STACK_PTR, OP_ICONV_TO_I8, OP_LCONV_TO_I4, OP_MOVE,
     OP_PT_ADD_IMM, OP_PT_SUB_IMM, OP_STORE_MEMBASE_REG, OP_PCOMPARE},
    // nuint
    {1, STACK_I8, STACK_I4, STACK_PTR, OP_ICONV_TO_U8, OP_LCONV_TO_U4, OP_MOVE,
     OP_PT_ADD_IMM, OP_PT_SUB_IMM, OP_STORE_MEMBASE_REG, OP_PCOMPARE},
    // nfloat
    {2, STACK_R8, STACK_R8, STACK_R8, OP_FCONV_TO_R8, OP_FCONV_TO_R4, OP_FMOVE,
     0, 0, OP_PT_STORE_FP_MEMBASE_REG, 0},
};

static MonoInst *emit_narrow(MonoCompile *cfg, const MagicTypeInfo *info,
                             int sreg) {
  MonoInst *ins;
...
static MonoInst *emit_widen(MonoCompile *cfg, const MagicTypeInfo *info,
                            int sreg) {
  MonoInst *ins;
...
static MonoInst *emit_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  if (!strcmp("op_Implicit", name) || !strcmp("op_Explicit", name)) {
    ...
    case STACK_I8:
      if (!mini_magic_is_int_type(fsig->params[0]) ||
          !mini_magic_is_int_type(fsig->ret))
        return NULL;
      break;
    case STACK_R8:
      if (!mini_magic_is_float_type(fsig->params[0]) ||
          !mini_magic_is_float_type(fsig->ret))
        return NULL;
      break;
    default:
  ...
  for (i = 0; i < sizeof(int_cmpop) / sizeof(IntIntrisic); ++i) {
    if (!strcmp(int_cmpop[i].op_name, name)) {
      ...
      if (info->compare_op) {
        ...
        ins->sreg2 = args[1]->dreg;
        MONO_ADD_INS(cfg->cbb, ins);
        if (op_cmp == OP_FCLT_UN || op_cmp == OP_FCGT_UN ||
            op_cmp == OP_RCLT_UN || op_cmp == OP_RCGT_UN) {
...
static gboolean mono_class_is_magic_assembly(MonoClass *klass) {
...
gboolean mono_class_is_magic_float(MonoClass *klass) {
  ...
  if (strcmp("nfloat", m_class_get_name(klass)) == 0) {
    ...
    MonoClassField *value_field =
        mono_class_get_field_from_name_full(klass, "v", NULL);
    ...
    if (t->type != native->type)
      g_error("Assembly used for native types '%s' doesn't match this runtime, "
              "%s is mapped to %s, expecting %s.\n",
              m_class_get_image(klass)->name, m_class_get_name(klass),
              mono_type_full_name(t), mono_type_full_name(native));
    return TRUE;
  }
  return FALSE;
}
...
gboolean mini_magic_is_int_type(MonoType *t) {
  if (t->type != MONO_TYPE_I && t->type != MONO_TYPE_I4 &&
      t->type != MONO_TYPE_I8 && t->type != MONO_TYPE_U4 &&
      t->type != MONO_TYPE_U8 &&
...
MonoType *mini_native_type_replace_type(MonoType *type) {
  ...
  if (mono_class_is_magic_int(klass))
    return type->byref ? m_class_get_this_arg(mono_defaults.int_class)
                       : mono_get_int_type();
  if (mono_class_is_magic_float(klass))
#if TARGET_SIZEOF_VOID_P == 8
    return type->byref ? m_class_get_this_arg(mono_defaults.double_class)
                       : m_class_get_byval_arg(mono_defaults.double_class);
#else
    return type->byref ? m_class_get_this_arg(mono_defaults.single_class)
