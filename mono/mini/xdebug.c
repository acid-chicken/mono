 ...
 *
 * We emit assembly code instead of using the ELF writer, so we can emit debug
 * info incrementally as each method is JITted, and the debugger doesn't have to
 * call into the runtime to emit the shared library, which would cause all kinds
 * of complications, like threading issues, and the fact that the ELF writer's
...
void mono_xdebug_init(const char *options) {
  MonoImageWriter *w;
  char **args, **ptr;
  ...
  mono_dwarf_writer_emit_base_info(xdebug_writer, "JITted code",
                                   mono_unwind_get_cie_program());
}

static void xdebug_begin_emit(MonoImageWriter **out_w,
                              MonoDwarfWriter **out_dw) {
  MonoImageWriter *w;
  MonoDwarfWriter *dw;
  ...
  mono_dwarf_writer_emit_base_info(dw, "JITted code",
                                   mono_unwind_get_cie_program());
...
static void xdebug_end_emit(MonoImageWriter *w, MonoDwarfWriter *dw,
                            MonoMethod *method) {
  guint8 *img;
  guint32 img_size;
 ...
 * mono_xdebug_flush:
 *
 *   This could be called from inside gdb to flush the debugging information not
 * yet registered with gdb.
 */
...
void mono_save_xdebug_info(MonoCompile *cfg) {
  ...
  if (use_gdb_interface) {
    ...
    /*
     * gdb is not designed to handle 1000s of symbol files (one per method). So
     * we group them into groups of 100.
     */
    ...
    dmji = mono_debug_find_method(jinfo_get_method(cfg->jit_info),
                                  mono_domain_get());
    mono_dwarf_writer_emit_method(
        xdebug_writer, cfg, jinfo_get_method(cfg->jit_info), NULL, NULL, NULL,
        (guint8 *)cfg->jit_info->code_start, cfg->jit_info->code_size,
        cfg->args, cfg->locals, cfg->unwind_ops, dmji);
...
void mono_save_trampoline_xdebug_info(MonoTrampInfo *info) {
  ...
  if (use_gdb_interface) {
    ...
    mono_dwarf_writer_emit_trampoline(dw, info_name, NULL, NULL, info->code,
                                      info->code_size, info->unwind_ops);
    ...
    mono_loader_lock_if_inited();
    mono_dwarf_writer_emit_trampoline(xdebug_writer, info_name, NULL, NULL,
                                      info->code, info->code_size,
                                      info->unwind_ops);
    fflush(xdebug_fp);
    mono_loader_unlock_if_inited();
  }
