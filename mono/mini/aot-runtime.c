 ...
 */
static void init_amodule_got(MonoAotModule *amodule) {
  MonoJumpInfo *ji;
  MonoMemPool *mp;
  ...
  for (i = 0; i < npatches; ++i) {
    ...
    if (ji->type == MONO_PATCH_INFO_GC_CARD_TABLE_ADDR &&
        !mono_gc_is_moving()) {
 ...
 * Return an array of MonoJumpInfo structures allocated from MP.
 */
static MonoJumpInfo *decode_patches(MonoAotModule *amodule, MonoMemPool *mp,
                                    int n_patches, gboolean llvm,
                                    guint32 *got_offsets) {
