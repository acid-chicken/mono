...
static MonoDl *aot_cache_load_module(MonoAssembly *assembly, char **aot_name) {
  ...
  *aot_name = fname;
  ...
  mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_AOT, "AOT: loading from cache: '%s'.",
             fname);
  module = mono_dl_open(fname, MONO_DL_LAZY, NULL);

  if (module) {
    mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_AOT, "AOT: found in cache: '%s'.",
               fname);
    return module;
  }
...
static gboolean check_usable(MonoAssembly *assembly, MonoAotFileInfo *info,
  ...
  if (!mono_aot_only &&
      (info->simd_opts & ~mono_arch_cpu_enumerate_simd_versions())) {
...
static void load_aot_module(MonoAssemblyLoadContext *alc,
  ...
  if (amodule->out_of_date) {
    mono_trace(
        G_LOG_LEVEL_INFO, MONO_TRACE_AOT,
    ...
    mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_AOT, "AOT: image '%s' found.",
               found_aot_name);
  }
}
