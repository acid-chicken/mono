...
extern guint8 *mono_aot_arch_get_plt_entry_exec_only(gpointer amodule_info,
                                                     host_mgreg_t *regs,
                                                     guint8 *code, guint8 *plt);
extern guint32 mono_arch_get_plt_info_offset_exec_only(
    gpointer amodule_info, guint8 *plt_entry, host_mgreg_t *regs, guint8 *code,
    MonoAotResolvePltInfoOffset resolver, gpointer amodule);
extern void mono_arch_patch_plt_entry_exec_only(gpointer amodule_info,
                                                guint8 *code, gpointer *got,
                                                host_mgreg_t *regs,
                                                guint8 *addr);
#endif
...
/* Embedding API hooks to load the AOT data for AOT images compiled with
...
static void init_plt(MonoAotModule *info);

static void compute_llvm_code_range(MonoAotModule *amodule, guint8 **code_start,
                                    guint8 **code_end);

static gboolean init_method(MonoAotModule *amodule, gpointer info,
                            guint32 method_index, MonoMethod *method,
                            MonoClass *init_class, MonoError *error);

static MonoJumpInfo *decode_patches(MonoAotModule *amodule, MonoMemPool *mp,
                                    int n_patches, gboolean llvm,
                                    guint32 *got_offsets);

static void amodule_lock(MonoAotModule *amodule) {
  mono_os_mutex_lock(&amodule->mutex);
}

static void amodule_unlock(MonoAotModule *amodule) {
 ...
 * load_image:
 *
 *   Load one of the images referenced by AMODULE. Returns NULL if the image is
 * not found, and sets @error for what happened
 */
static MonoImage *load_image(MonoAotModule *amodule, int index,
                             MonoError *error) {
  MonoAssembly *assembly;
  ...
  mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT,
             "AOT: module %s wants to load image %d: %s", amodule->aot_name,
             index, amodule->image_names[index].name);
  if (amodule->out_of_date) {
    mono_error_set_bad_image_by_name(
        error, amodule->aot_name, "Image out of date: %s", amodule->aot_name);
    return NULL;
  }
  ...
  if (!strcmp(amodule->assembly->image->guid, amodule->image_guids[index])) {
  ...
  } else if (mono_get_corlib() &&
             !strcmp(mono_get_corlib()->guid, amodule->image_guids[index])) {
    ...
    assembly = mono_assembly_request_byname(&amodule->image_names[index], &req,
                                            &status);
  }
  if (!assembly) {
    mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_AOT,
               "AOT: module %s is unusable because dependency %s is not found.",
               amodule->aot_name, amodule->image_names[index].name);
    mono_error_set_bad_image_by_name(error, amodule->aot_name,
                                     "module '%s' is unusable because "
                                     "dependency %s is not found (error %d).\n",
                                     amodule->aot_name,
  ...
  if (strcmp(assembly->image->guid, amodule->image_guids[index])) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT,
               "AOT: module %s is unusable (GUID of dependent assembly %s "
               "doesn't match (expected '%s', got '%s')).",
               amodule->aot_name, amodule->image_names[index].name,
               amodule->image_guids[index], assembly->image->guid);
    mono_error_set_bad_image_by_name(
        error, amodule->aot_name,
        "module '%s' is unusable (GUID of dependent assembly %s doesn't match "
        "(expected '%s', got '%s')).",
        amodule->aot_name, amodule->image_names[index].name,
        amodule->image_guids[index], assembly->image->guid);
    amodule->out_of_date = TRUE;
...
static gint32 decode_value(guint8 *ptr, guint8 **rptr) {
  guint8 b = *ptr;
  gint32 len;
 ...
 * mono_aot_get_offset:
 *
 *   Decode an offset table emitted by emit_offset_table (), returning the
 * INDEXth entry.
 */
static guint32 mono_aot_get_offset(guint32 *table, int index) {
  ...
  // start_offset, table [3 + group]);

  return offset;
}

static MonoMethod *decode_resolve_method_ref(MonoAotModule *module, guint8 *buf,
                                             guint8 **endbuf, MonoError *error);

static MonoClass *decode_klass_ref(MonoAotModule *module, guint8 *buf,
                                   guint8 **endbuf, MonoError *error);

static MonoType *decode_type(MonoAotModule *module, guint8 *buf,
                             guint8 **endbuf, MonoError *error);

static MonoGenericInst *decode_generic_inst(MonoAotModule *module, guint8 *buf,
                                            guint8 **endbuf, MonoError *error) {
  int type_argc, i;
  MonoType **type_argv;
...
static gboolean decode_generic_context(MonoAotModule *amodule,
                                       MonoGenericContext *ctx, guint8 *buf,
                                       guint8 **endbuf, MonoError *error) {
  guint8 *p = buf;
  guint8 *p2;
...
static MonoClass *decode_klass_ref(MonoAotModule *module, guint8 *buf,
  ...
  if (reftype == 0) {
    *endbuf = p;
    mono_error_set_bad_image_by_name(error, module->aot_name,
                                     "Decoding a null class ref: %s",
                                     module->aot_name);
    return NULL;
  }
  ...
  case MONO_AOT_TYPEREF_TYPESPEC_TOKEN:
    ...
    if (!image) {
      mono_error_set_bad_image_by_name(
          error, module->aot_name,
          "No image associated with the aot module: %s", module->aot_name);
      return NULL;
    }
  ...
  case MONO_AOT_TYPEREF_GINST: {
    ...
    if (!ctx.class_inst)
      return NULL;
    type = mono_class_inflate_generic_type_checked(
        m_class_get_byval_arg(gclass), &ctx, error);
    if (!type)
      return NULL;
  ...
  case MONO_AOT_TYPEREF_VAR: {
    ...
    if (has_constraint) {
      ...
      t = mini_get_shared_gparam(m_class_get_byval_arg(par_klass),
                                 gshared_constraint);
      ...
      if (is_not_anonymous) {
        ...
        if (is_method) {
          ...
          container = mono_class_try_get_generic_container(
              class_def); // FIXME is this a case for a try_get?
        }
      } else {
        // We didn't decode is_method, so we have to infer it from type enum.
        container = mono_get_anonymous_container_for_image(
            module->assembly->image, type == MONO_TYPE_MVAR);
      }

      t = g_new0(MonoType, 1);
      t->type = (MonoTypeEnum)type;
      if (is_not_anonymous) {
        t->data.generic_param =
            mono_generic_container_get_param(container, num);
      } else {
        /* Anonymous */
        MonoGenericParam *par = mono_metadata_create_anon_gparam(
            module->assembly->image, num, type == MONO_TYPE_MVAR);
        t->data.generic_param = par;
        // FIXME: maybe do this for all anon gparams?
        ((MonoGenericParamFull *)par)->info.name =
            mono_make_generic_name_string(module->assembly->image, num);
      }
  ...
  default:
    mono_error_set_bad_image_by_name(error, module->aot_name,
                                     "Invalid klass reftype %d: %s", reftype,
                                     module->aot_name);
  }
  // g_assert (klass);
...
static MonoClassField *decode_field_info(MonoAotModule *module, guint8 *buf,
                                         guint8 **endbuf) {
  ERROR_DECL(error);
...
/*
 * Parse a MonoType encoded by encode_type () in aot-compiler.c. Return
 * malloc-ed memory.
 */
static MonoType *decode_type(MonoAotModule *module, guint8 *buf,
                             guint8 **endbuf, MonoError *error) {
  ...
  if (*p == MONO_TYPE_CMOD_REQD) {
    ...
    /* TODO: encode aggregate cmods differently than simple cmods and make it
     * possible to use the more compact encoding here. */
  ...
  default:
    mono_error_set_bad_image_by_name(error, module->aot_name,
                                     "Invalid encoded type %d: %s", t->type,
                                     module->aot_name);
    goto fail;
  }
...
decode_signature_with_target(MonoAotModule *module, MonoMethodSignature *target,
...
static MonoMethodSignature *decode_signature(MonoAotModule *module, guint8 *buf,
                                             guint8 **endbuf) {
  return decode_signature_with_target(module, NULL, buf, endbuf);
}

static gboolean sig_matches_target(MonoAotModule *module, MonoMethod *target,
                                   guint8 *buf, guint8 **endbuf) {
  MonoMethodSignature *sig;
  gboolean res;
  guint8 *p = buf;

  sig = decode_signature_with_target(
      module, mono_method_signature_internal(target), p, &p);
  res = sig && mono_metadata_signature_equal(
                   mono_method_signature_internal(target), sig);
  g_free(sig);
  *endbuf = p;
  return res;
}
 ...
 *
 *   Decode a method reference, storing the image/token into a MethodRef
 * structure. This avoids loading metadata for the method if the caller does not
 * need it. If the method has no token, then it is loaded from metadata and
 * ref->method is set to the method instance. If TARGET is non-NULL, abort
 * decoding if it can be determined that the decoded method couldn't resolve to
 * TARGET, and return FALSE. There are some kinds of method references which
 * only support a non-null TARGET. This means that its not possible to decode
 * this into a method, only to check that the method reference matches a given
 * method. This is normally not a problem as these wrappers only occur in the
 * extra_methods table, where we already have a method we want to lookup.
 *
 * If there was a decoding error, we return FALSE and set @error
 */
static gboolean decode_method_ref_with_target(MonoAotModule *module,
                                              MethodRef *ref,
                                              MonoMethod *target, guint8 *buf,
                                              guint8 **endbuf,
                                              MonoError *error) {
  ...
  if (image_index < MONO_AOT_METHODREF_MIN ||
      image_index == MONO_AOT_METHODREF_METHODSPEC ||
      image_index == MONO_AOT_METHODREF_GINST ||
      image_index == MONO_AOT_METHODREF_BLOB_INDEX) {
  ...
  if (image_index == MONO_AOT_METHODREF_WRAPPER) {
    ...
    case MONO_WRAPPER_STFLD: {
      ...
      if (wrapper_type == MONO_WRAPPER_LDFLD)
        ...
        mono_error_set_bad_image_by_name(error, module->aot_name,
                                         "Unknown AOT wrapper type %d: %s",
                                         wrapper_type, module->aot_name);
        return FALSE;
      }
      break;
    }
    ...
    case MONO_WRAPPER_ALLOC: {
      ...
      if (!ref->method)
        ref->method = mono_gc_get_managed_allocator_by_type(
            atype, MANAGED_ALLOCATOR_SLOW_PATH);
      if (!ref->method) {
        mono_error_set_bad_image_by_name(
            error, module->aot_name,
            "Error: No managed allocator, but we need one for AOT.\nAre you "
            "using non-standard GC options?\n%s\n",
            module->aot_name);
        return FALSE;
      }
      break;
    ...
    case MONO_WRAPPER_STELEMREF: {
      ...
      if (subtype == WRAPPER_SUBTYPE_NONE) {
        ...
        mono_error_set_bad_image_by_name(error, module->aot_name,
                                         "Invalid STELEMREF subtype %d: %s",
                                         subtype, module->aot_name);
        return FALSE;
      }
      break;
    }
    ...
    case MONO_WRAPPER_OTHER: {
      ...
      if (subtype == WRAPPER_SUBTYPE_PTR_TO_STRUCTURE ||
          subtype == WRAPPER_SUBTYPE_STRUCTURE_TO_PTR) {
        MonoClass *klass = decode_klass_ref(module, p, &p, error);
        if (!klass)
          return FALSE;
        ...
        MonoJitICallInfo *info =
            mono_find_jit_icall_info((MonoJitICallId)decode_value(p, &p));
        ref->method =
            mini_get_interp_lmf_wrapper(info->name, (gpointer)info->func);
        ...
        ref->method =
            mono_marshal_get_aot_init_wrapper((MonoAotInitSubtype)init_type);
      } else if (subtype == WRAPPER_SUBTYPE_LLVM_FUNC) {
        guint32 init_type = decode_value(p, &p);
        ref->method = mono_marshal_get_llvm_func_wrapper(
            (MonoLLVMFuncWrapperSubtype)init_type);
      } else {
        mono_error_set_bad_image_by_name(
            error, module->aot_name, "Invalid UNKNOWN wrapper subtype %d: %s",
            subtype, module->aot_name);
        return FALSE;
      }
      break;
    ...
    case MONO_WRAPPER_CASTCLASS: {
      ...
      if (subtype == WRAPPER_SUBTYPE_CASTCLASS_WITH_CACHE)
        ...
        mono_error_set_bad_image_by_name(
            error, module->aot_name, "Invalid CASTCLASS wrapper subtype %d: %s",
            subtype, module->aot_name);
        return FALSE;
      }
      break;
    ...
    case MONO_WRAPPER_RUNTIME_INVOKE: {
      ...
      if (subtype == WRAPPER_SUBTYPE_RUNTIME_INVOKE_DYNAMIC) {
        ...
        const gboolean same_sig =
            mono_metadata_signature_equal(sig, info->d.runtime_invoke.sig);
        g_free(sig);
        if (same_sig)
          ref->method = target;
        else
    ...
    case MONO_WRAPPER_DELEGATE_END_INVOKE: {
      ...
      if (is_inflated) {
        ...
        if (target) {
          /*
           * Due to the way mini_get_shared_method_full () works, we could end
           * up with multiple copies of the same wrapper.
           */
    ...
    if (FALSE && mono_class_is_ginst(klass)) {
      ...
      ref->method = mono_class_inflate_generic_method_full_checked(
    ...
    switch (method_type) {
    case 0:
      ref->method = mono_class_get_method_from_name_checked(
          klass, ".ctor", m_class_get_rank(klass), 0, error);
      return_val_if_nok(error, FALSE);
      break;
    case 1:
      ref->method = mono_class_get_method_from_name_checked(
          klass, ".ctor", m_class_get_rank(klass) * 2, 0, error);
      return_val_if_nok(error, FALSE);
      break;
    case 2:
      ref->method =
          mono_class_get_method_from_name_checked(klass, "Get", -1, 0, error);
      return_val_if_nok(error, FALSE);
      break;
    case 3:
      ref->method = mono_class_get_method_from_name_checked(klass, "Address",
                                                            -1, 0, error);
      return_val_if_nok(error, FALSE);
      break;
    case 4:
      ref->method =
          mono_class_get_method_from_name_checked(klass, "Set", -1, 0, error);
      return_val_if_nok(error, FALSE);
      break;
    default:
      mono_error_set_bad_image_by_name(
          error, module->aot_name, "Invalid METHODREF_ARRAY method type %d: %s",
          method_type, module->aot_name);
      return FALSE;
    }
  } else {
...
static gboolean decode_method_ref(MonoAotModule *module, MethodRef *ref,
                                  guint8 *buf, guint8 **endbuf,
                                  MonoError *error) {
 ...
 */
static MonoMethod *decode_resolve_method_ref_with_target(MonoAotModule *module,
                                                         MonoMethod *target,
                                                         guint8 *buf,
                                                         guint8 **endbuf,
                                                         MonoError *error) {
  ...
  if (!ref.image) {
    mono_error_set_bad_image_by_name(
        error, module->aot_name, "No image found for methodref with target: %s",
        module->aot_name);
    return NULL;
  }
...
static void collect_assemblies(gpointer data, gpointer user_data) {
 ...
 */
static G_GNUC_UNUSED char *get_aot_config_hash(MonoAssembly *assembly) {
  char *build_info;
  ...
  mono_trace(G_LOG_LEVEL_MESSAGE, MONO_TRACE_AOT,
             "AOT: file dependencies: %s, hash %s", s->str, digest_str);
 ...
 *
 *   Load the AOT image corresponding to ASSEMBLY from the aot cache, AOTing it
 * if neccessary.
 */
static MonoDl *aot_cache_load_module(MonoAssembly *assembly, char **aot_name) {
  ...
  *aot_name = NULL;
  ...
  if (config->apps) {
    ...
    // FIXME: This cannot be used for mscorlib during startup, since
    // entry_assembly is not set yet
    for (l = config->apps; l; l = l->next) {
      char *n = (char *)l->data;

      if ((entry_assembly && !strcmp(entry_assembly->aname.name, n)) ||
          (!entry_assembly && !strcmp(assembly->aname.name, n)))
        break;
    }
    if (l)
      enabled = TRUE;
   ...
   * versions of the runtime, with multiple versions of dependent assemblies
   * etc. To handle this, we compute a version string containing all this
   * information, hash it, and use the hash as a filename suffix.
   */
  hash = get_aot_config_hash(assembly);

  tmp2 = g_strdup_printf("%s-%s%s", assembly->image->assembly_name, hash,
                         MONO_SOLIB_EXT);
  ...
  *aot_name = fname;
  ...
  if (module) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT, "AOT: found in cache: '%s'.",
               fname);
    return module;
  }
  ...
  if (failure_file) {
    mono_trace(
        G_LOG_LEVEL_MESSAGE, MONO_TRACE_AOT,
        "AOT: assembly '%s' previously failed to compile '%s' ('%s')... ",
        assembly->image->name, fname, failure_fname);
  ...
  mono_trace(G_LOG_LEVEL_MESSAGE, MONO_TRACE_AOT,
             "AOT: compiling assembly '%s', logfile: '%s.log'... ",
             assembly->image->name, fname);

  /*
   * We need to invoke the AOT compiler here. There are multiple approaches:
   * - spawn a new runtime process. This can be hard when running with mkbundle,
   * and its hard to make the new process load the same set of assemblies.
   * - doing it in-process. This exposes the current process to bugs/leaks/side
   * effects of the AOT compiler.
   * - fork a new process and do the work there.
   */
  if (in_process) {
    aot_options =
        g_strdup_printf("outfile=%s,internal-logfile=%s.log%s%s", fname, fname,
                        config->aot_options ? "," : "",
                        config->aot_options ? config->aot_options : "");
    /* Maybe due this in another thread ? */
    res = mono_compile_assembly(
        assembly, mono_parse_default_optimizations(NULL), aot_options, NULL);
    if (res) {
      mono_trace(G_LOG_LEVEL_MESSAGE, MONO_TRACE_AOT,
     ...
     *   (less overhead, but multiple processes could aot the same assembly at
     * the same time)
     */
    pid = fork();
    if (pid == 0) {
...
static void find_symbol(MonoDl *module, gpointer *globals, const char *name,
                        gpointer *value) {
  if (globals) {
    int global_index;
    guint16 *table, *entry;
...
static void find_amodule_symbol(MonoAotModule *amodule, const char *name,
                                gpointer *value) {
...
static guint8 *open_aot_data(MonoAssembly *assembly, MonoAotFileInfo *info,
                             void **ret_handle) {
  MonoFileMap *map;
  char *filename;
  guint8 *data;
  ...
  if (aot_data_load_func) {
    data = aot_data_load_func(assembly, info->datafile_size,
                              aot_data_func_user_data, ret_handle);
    g_assert(data);
    return data;
  }

  /*
   * Use <assembly name>.aotdata as the default implementation if no callback is
   * given
   */
  filename = g_strdup_printf("%s.aotdata", assembly->image->name);
  map = mono_file_map_open(filename);
  g_assert(map);
  data = (guint8 *)mono_file_map(info->datafile_size, MONO_MMAP_READ,
                                 mono_file_map_fd(map), 0, ret_handle);
  g_assert(data);

  return data;
}

static gboolean check_usable(MonoAssembly *assembly, MonoAotFileInfo *info,
                             guint8 *blob, char **out_msg) {
  char *build_info;
  char *msg = NULL;
  ...
  build_info = mono_get_runtime_build_info();
  if (strlen((const char *)info->runtime_version) > 0 &&
      strcmp(info->runtime_version, build_info)) {
    msg = g_strdup_printf("compiled against runtime version '%s' while this "
                          "runtime has version '%s'",
                          info->runtime_version, build_info);
    usable = FALSE;
  }
  g_free(build_info);
  ...
  if (mono_use_interpreter && !interp &&
      !strcmp(assembly->aname.name, "mscorlib")) {
  ...
  if (mini_debug_options.mdb_optimizations &&
      !(info->flags & MONO_AOT_FILE_FLAG_DEBUG) && !full_aot && !interp) {
  ...
  if (!(info->flags & MONO_AOT_FILE_FLAG_CODE_EXEC_ONLY)) {
    msg = g_strdup(
        "not compiled targeting a runtime configured as CODE_EXEC_ONLY");
    usable = FALSE;
  }
#else
  if (info->flags & MONO_AOT_FILE_FLAG_CODE_EXEC_ONLY) {
...
/*
 * TABLE should point to a table of call instructions. Return the address called
 * by the INDEXth entry.
 */
static void *get_call_table_entry(void *table, int index, int entry_size) {
 ...
 */
static void init_amodule_got(MonoAotModule *amodule, gboolean preinit) {
  MonoJumpInfo *ji;
  MonoMemPool *mp;
  ...
  for (i = 0; i < npatches; ++i) {
    ...
    if (amodule->shared_got[i]) {
      ...
      /*
       * This is called from init_amodule () during startup, so some things
       * might not be setup. Initialize just the slots needed to make method
       * initialization work.
       */
    ...
    } else if (ji->type == MONO_PATCH_INFO_GC_CARD_TABLE_ADDR &&
               !mono_gc_is_moving()) {
      amodule->shared_got[i] = NULL;
    } else if (ji->type == MONO_PATCH_INFO_GC_NURSERY_START &&
               !mono_gc_is_moving()) {
      amodule->shared_got[i] = NULL;
    } else if (ji->type == MONO_PATCH_INFO_GC_NURSERY_BITS &&
               !mono_gc_is_moving()) {
      amodule->shared_got[i] = NULL;
      ...
      amodule->shared_got[i] = mono_resolve_patch_target(
          NULL, mono_get_root_domain(), NULL, ji, FALSE, error);
  ...
  if (amodule->info.flags & MONO_AOT_FILE_FLAG_WITH_LLVM) {
    void (*init_aotconst)(int, gpointer) =
        (void (*)(int, gpointer))amodule->info.llvm_init_aotconst;
    for (i = 0; i < npatches; ++i) {
...
static inline guint8 *method_address_resolve(guint8 *code_addr) {
#if defined(TARGET_ARM)
...
static void load_aot_module(MonoAssemblyLoadContext *alc,
                            MonoAssembly *assembly, gpointer user_data,
                            MonoError *error) {
  ...
  if (assembly->image->aot_module)
    /*
     * Already loaded. This can happen because the assembly loading code might
     * invoke the assembly load hooks multiple times for the same assembly.
     */
    return;

  if (image_is_dynamic(assembly->image) ||
      mono_asmctx_get_kind(&assembly->context) == MONO_ASMCTX_REFONLY ||
  ...
  if (static_aot_modules)
    info = (MonoAotFileInfo *)g_hash_table_lookup(static_aot_modules,
                                                  assembly->aname.name);
  ...
  if (info) {
    ...
    if (!sofile) {
      ...
      if (sofile) {
        ...
        mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT,
                   "AOT: image '%s' not found: %s", aot_name, err);
        g_free(err);
      }
      g_free(aot_name);
    }
    ...
    if (!sofile) {
      ...
      aot_name = g_strdup_printf("%s/mono/aot-cache/%s/%s%s",
                                 mono_assembly_getrootdir(), MONO_ARCHITECTURE,
                                 basename, MONO_SOLIB_EXT);
      g_free(basename);
      sofile = mono_dl_open(aot_name, MONO_DL_LAZY, &err);
      if (!sofile) {
        mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT,
    ...
    if (!sofile) {
      if (mono_aot_only && !mono_use_interpreter &&
          assembly->image->tables[MONO_TABLE_METHOD].rows) {
        aot_name =
            g_strdup_printf("%s%s", assembly->image->name, MONO_SOLIB_EXT);
  ...
  if (!info) {
    find_symbol(sofile, globals, "mono_aot_version",
                (gpointer *)&version_symbol);
  ...
  if (version != MONO_AOT_FILE_VERSION) {
    msg = g_strdup_printf("wrong file format version (expected %d got %d)",
                          MONO_AOT_FILE_VERSION, version);
    usable = FALSE;
  } else {
    guint8 *blob;
    void *handle;
  ...
  if (!usable) {
    if (mono_aot_only) {
      g_error("Failed to load AOT module '%s' while running in aot-only mode: "
              "%s.\n",
              found_aot_name, msg);
    } else {
      mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT,
                 "AOT: module %s is unusable: %s.", found_aot_name, msg);
    }
    g_free(msg);
  ...
  /*
   * The llvm code keeps its data in separate scalar variables, so this just
   * used by this module.
   */
  ...
  if (info->flags & MONO_AOT_FILE_FLAG_SEPARATE_DATA) {
    ...
    amodule->method_info_offsets =
        (guint32 *)amodule->tables[MONO_AOT_TABLE_METHOD_INFO_OFFSETS];
    amodule->ex_info_offsets =
        (guint32 *)amodule->tables[MONO_AOT_TABLE_EX_INFO_OFFSETS];
    amodule->class_info_offsets =
        (guint32 *)amodule->tables[MONO_AOT_TABLE_CLASS_INFO_OFFSETS];
    amodule->class_name_table =
        (guint16 *)amodule->tables[MONO_AOT_TABLE_CLASS_NAME];
    amodule->extra_method_table =
        (guint32 *)amodule->tables[MONO_AOT_TABLE_EXTRA_METHOD_TABLE];
    amodule->extra_method_info_offsets =
        (guint32 *)amodule->tables[MONO_AOT_TABLE_EXTRA_METHOD_INFO_OFFSETS];
    amodule->got_info_offsets =
        (guint32 *)amodule->tables[MONO_AOT_TABLE_GOT_INFO_OFFSETS];
    amodule->llvm_got_info_offsets =
        (guint32 *)amodule->tables[MONO_AOT_TABLE_LLVM_GOT_INFO_OFFSETS];
    amodule->weak_field_indexes =
        (guint32 *)amodule->tables[MONO_AOT_TABLE_WEAK_FIELD_INDEXES];
    amodule->method_flags_table =
        (guint8 *)amodule->tables[MONO_AOT_TABLE_METHOD_FLAGS_TABLE];
  } else {
    ...
    amodule->extra_method_info_offsets =
        (guint32 *)info->extra_method_info_offsets;
  ...
  amodule->unbox_trampoline_addresses =
      (guint32 *)info->unbox_trampoline_addresses;
  ...
  amodule->trampolines[MONO_AOT_TRAMP_SPECIFIC] =
      (guint8 *)info->specific_trampolines;
  amodule->trampolines[MONO_AOT_TRAMP_STATIC_RGCTX] =
      (guint8 *)info->static_rgctx_trampolines;
  amodule->trampolines[MONO_AOT_TRAMP_IMT] = (guint8 *)info->imt_trampolines;
  amodule->trampolines[MONO_AOT_TRAMP_GSHAREDVT_ARG] =
      (guint8 *)info->gsharedvt_arg_trampolines;
  amodule->trampolines[MONO_AOT_TRAMP_FTNPTR_ARG] =
      (guint8 *)info->ftnptr_arg_trampolines;
  amodule->trampolines[MONO_AOT_TRAMP_UNBOX_ARBITRARY] =
      (guint8 *)info->unbox_arbitrary_trampolines;

  if (mono_is_corlib_image(assembly->image) ||
      !strcmp(assembly->aname.name, "mscorlib") ||
      !strcmp(assembly->aname.name, "System.Private.CoreLib"))
    mscorlib_aot_module = amodule;

  /* Compute method addresses */
  amodule->methods =
      (void **)g_malloc0(amodule->info.nmethods * sizeof(gpointer));
  for (i = 0; i < amodule->info.nmethods; ++i) {
    void *addr = NULL;

    if (amodule->info.llvm_get_method) {
      gpointer (*get_method)(int) =
          (gpointer(*)(int))amodule->info.llvm_get_method;
    ...
    if (amodule->info.flags & MONO_AOT_FILE_FLAG_CODE_EXEC_ONLY) {
      ...
      /* method_addresses () contains a table of branches, since the ios linker
       * can update those correctly */
      if (!addr && amodule->info.method_addresses) {
        addr = get_call_table_entry(amodule->info.method_addresses, i,
                                    amodule->info.call_table_entry_size);
        g_assert(addr);
        if (addr == amodule->info.method_addresses)
          addr = NULL;
        else
  ...
  if (make_unreadable) {
     ...
     */
    page_start =
        (guint8 *)(((gssize)(addr)) & ~(mono_pagesize() - 1)) + mono_pagesize();
  ...
  if (info->flags & MONO_AOT_FILE_FLAG_WITH_LLVM)
    compute_llvm_code_range(amodule, &amodule->llvm_code_start,
                            &amodule->llvm_code_end);
  ...
  if (amodule->jit_code_start)
    mono_jit_info_add_aot_module(assembly->image, amodule->jit_code_start,
                                 amodule->jit_code_end);
  if (amodule->llvm_code_start)
    mono_jit_info_add_aot_module(assembly->image, amodule->llvm_code_start,
                                 amodule->llvm_code_end);
  ...
  if (mono_aot_only && !mono_llvm_only) {
    char *code;
    find_amodule_symbol(amodule, "specific_trampolines_page",
                        (gpointer *)&code);
  ...
  /*
   * Register the plt region as a single trampoline so we can unwind from this
   * code
   */
  mono_aot_tramp_info_register(
      mono_tramp_info_create(NULL, amodule->plt,
                             amodule->plt_end - amodule->plt, NULL,
                             mono_unwind_get_cie_program()),
      NULL);

  /*
   * Since we store methoddef and classdef tokens when referring to
   * methods/classes in referenced assemblies, we depend on the exact versions
   * of the referenced assemblies. MS calls this 'hard binding'. This means we
   * have to load all referenced assemblies non-lazily, since we can't handle
   * out-of-date errors later. The cached class info also depends on the exact
   * assemblies.
   */
  if (do_load_image) {
  ...
  if (amodule->out_of_date) {
    mono_trace(
        G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT,
        "AOT: Module %s is unusable because a dependency is out-of-date.",
        assembly->image->name);
    if (mono_aot_only)
      g_error("Failed to load AOT module '%s' while running in aot-only mode "
              "because a dependency cannot be found or it is out of date.\n",
              found_aot_name);
  } else {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT, "AOT: image '%s' found.",
               found_aot_name);
  }
}
 ...
 *
 * This should be called by embedding code to register normal AOT modules
 * statically linked into the executable.
 *
 * \param aot_info the value of the 'mono_aot_module_<ASSEMBLY_NAME>_info'
 * global symbol from the AOT module.
 */
...
void mono_aot_init(void) {
  ...
  mono_counters_register("Async JIT info size",
...
void mono_aot_cleanup(void) { g_hash_table_destroy(aot_modules); }

static void load_container_amodule(MonoAssemblyLoadContext *alc) {
  ERROR_DECL(error);
...
static gboolean decode_cached_class_info(MonoAotModule *module,
                                         MonoCachedClassInfo *info, guint8 *buf,
                                         guint8 **endbuf) {
  ERROR_DECL(error);
  guint32 flags;
  MethodRef ref;
...
gpointer mono_aot_get_method_from_vt_slot(MonoDomain *domain,
                                          MonoVTable *vtable, int slot,
                                          MonoError *error) {
  int i;
  ...
  if (MONO_CLASS_IS_INTERFACE_INTERNAL(klass) || m_class_get_rank(klass) ||
      !amodule)
    return NULL;

  info = &amodule->blob[mono_aot_get_offset(
      amodule->class_info_offsets,
      mono_metadata_token_index(m_class_get_type_token(klass)) - 1)];
  p = info;
  ...
  if (mono_metadata_token_index(ref.token) == 0 ||
      mono_metadata_token_table(ref.token) != MONO_TABLE_METHOD)
    return NULL;
...
gboolean mono_aot_get_cached_class_info(MonoClass *klass,
  ...
  p = (guint8 *)&amodule->blob[mono_aot_get_offset(
      amodule->class_info_offsets,
      mono_metadata_token_index(m_class_get_type_token(klass)) - 1)];
 ...
 * in IMAGE, using a cache stored in the AOT file. Stores the resulting class in
 * *KLASS if found, stores NULL otherwise.
 *
 * Returns: TRUE if the klass was found/not found in the cache, FALSE if no aot
 * file was found.
 */
gboolean mono_aot_get_class_from_name(MonoImage *image, const char *name_space,
                                      const char *name, MonoClass **klass) {
  ...
  *klass = NULL;
  ...
  nspace_table =
      (GHashTable *)g_hash_table_lookup(amodule->name_cache, name_space);
  if (nspace_table) {
    ...
    while (TRUE) {
      ...
      if (!strcmp(name, name2) && !strcmp(name_space, name_space2)) {
        ...
        *klass = mono_class_get_checked(image, token, error);
        ...
        if (*klass) {
          amodule_lock(amodule);
          nspace_table = (GHashTable *)g_hash_table_lookup(amodule->name_cache,
                                                           name_space);
          if (!nspace_table) {
            nspace_table = g_hash_table_new(g_str_hash, g_str_equal);
            g_hash_table_insert(amodule->name_cache, (char *)name_space2,
                                nspace_table);
          }
...
GHashTable *mono_aot_get_weak_field_indexes(MonoImage *image) {
  ...
  for (int i = 0; i < len; ++i)
    g_hash_table_insert(indexes_hash, GUINT_TO_POINTER(indexes[i + 1]),
                        GUINT_TO_POINTER(1));
  return indexes_hash;
}
...
static void compute_llvm_code_range(MonoAotModule *amodule, guint8 **code_start,
  ...
  if (fde_count > 0) {
    ...
    *code_end = (guint8 *)amodule->methods[table[(fde_count - 1) * 2]] +
                table[fde_count * 2];
  } else {
    *code_start = NULL;
    *code_end = NULL;
...
static gboolean is_llvm_code(MonoAotModule *amodule, guint8 *code) {
#if HOST_WASM
  return TRUE;
#else
  if ((guint8 *)code >= amodule->llvm_code_start &&
      (guint8 *)code < amodule->llvm_code_end)
    return TRUE;
  else
    return FALSE;
#endif
...
static gboolean is_thumb_code(MonoAotModule *amodule, guint8 *code) {
  if (is_llvm_code(amodule, code) &&
      (amodule->info.flags & MONO_AOT_FILE_FLAG_LLVM_THUMB))
    return TRUE;
  else
    return FALSE;
}
 ...
 *
 *   Decode the EH information emitted by our modified LLVM compiler and
 * construct a MonoJitInfo structure from it. If JINFO is NULL, set
 * OUT_LLVM_CLAUSES to the number of llvm level clauses. This function is async
 * safe when called in async context.
 */
static void decode_llvm_mono_eh_frame(MonoAotModule *amodule,
                                      MonoDomain *domain, MonoJitInfo *jinfo,
                                      guint8 *code, guint32 code_len,
                                      MonoJitExceptionInfo *clauses,
                                      int num_clauses, GSList **nesting,
  ...
  /* Compute lengths */
  mono_unwind_decode_llvm_mono_fde(fde, fde_len, cie, code_start, &info, NULL,
                                   NULL, NULL);

  if (async) {
    /* These are leaked, but the leak is bounded */
    ei = mono_domain_alloc0_lock_free(domain, info.ex_info_len *
                                                  sizeof(MonoJitExceptionInfo));
    type_info = mono_domain_alloc0_lock_free(domain, info.ex_info_len *
                                                         sizeof(gpointer));
    unw_info = mono_domain_alloc0_lock_free(domain, info.unw_info_len);
  } else {
    ei = (MonoJitExceptionInfo *)g_malloc0(info.ex_info_len *
                                           sizeof(MonoJitExceptionInfo));
    type_info = (gpointer *)g_malloc0(info.ex_info_len * sizeof(gpointer));
    unw_info = (guint8 *)g_malloc0(info.unw_info_len);
  }
  mono_unwind_decode_llvm_mono_fde(fde, fde_len, cie, code_start, &info, ei,
                                   type_info, unw_info);
  ...
  *this_offset = info.this_offset;
  ...
  /* Store the unwind info addr/length in the MonoJitInfo structure itself so
   * its async safe */
...
static gpointer alloc0_jit_info_data(MonoDomain *domain, int size,
                                     gboolean async_context)

#define alloc0_jit_info_data(domain, size, async_context)                      \
  (g_cast(alloc0_jit_info_data((domain), (size), (async_context))))
 ...
 */
static MonoJitInfo *decode_exception_debug_info(MonoAotModule *amodule,
                                                MonoDomain *domain,
                                                MonoMethod *method,
                                                guint8 *ex_info, guint8 *code,
                                                guint32 code_len) {
  ERROR_DECL(error);
  ...
  gboolean has_generic_jit_info, has_dwarf_unwind_info, has_clauses,
      has_seq_points, has_try_block_holes, has_arch_eh_jit_info;
  ...
  if (has_try_block_holes) {
    ...
    try_holes_info_size = sizeof(MonoTryBlockHoleTableJitInfo) +
                          num_holes * sizeof(MonoTryBlockHoleJitInfo);
  } else {
  ...
  if (from_llvm) {
    ...
    /*
     * Part of the info is encoded by the AOT compiler, the rest is in the
     * .eh_frame section.
     */
    if (async) {
      if (num_clauses < 16) {
        ...
        clauses = alloc0_jit_info_data(
            domain, sizeof(MonoJitExceptionInfo) * num_clauses, TRUE);
        nesting =
            alloc0_jit_info_data(domain, sizeof(GSList *) * num_clauses, TRUE);
      }
    ...
    for (i = 0; i < num_clauses; ++i) {
      ...
      while (TRUE) {
        ...
        g_assert(!async);
        nesting[i] =
            g_slist_prepend(nesting[i], GINT_TO_POINTER(nesting_index));
      }
    }
    ...
    decode_llvm_mono_eh_frame(amodule, domain, NULL, code, code_len, clauses,
                              num_clauses, nesting, &this_reg, &this_offset,
                              &num_llvm_clauses);
    len = mono_jit_info_size(flags, num_llvm_clauses, num_holes);
    jinfo = (MonoJitInfo *)alloc0_jit_info_data(domain, len, async);
    mono_jit_info_init(jinfo, method, code, code_len, flags, num_llvm_clauses,
                       num_holes);

    decode_llvm_mono_eh_frame(amodule, domain, jinfo, code, code_len, clauses,
                              num_clauses, nesting, &this_reg, &this_offset,
                              NULL);
    ...
    jinfo = (MonoJitInfo *)alloc0_jit_info_data(domain, len, async);
    mono_jit_info_init(jinfo, method, code, code_len, flags, num_clauses,
                       num_holes);
  ...
  if (has_generic_jit_info) {
    ...
    if (gi->nlocs) {
      gi->locations = (MonoDwarfLocListEntry *)alloc0_jit_info_data(
          domain, gi->nlocs * sizeof(MonoDwarfLocListEntry), async);
    ...
    gi->generic_sharing_context =
        alloc0_jit_info_data(domain, sizeof(MonoGenericSharingContext), async);
    if (decode_value(p, &p)) {
      /* gsharedvt */
  ...
  if (method && has_seq_points) {
    ...
    /* This could be set already since this function can be called more than
...
static gboolean amodule_contains_code_addr(MonoAotModule *amodule,
                                           guint8 *code) {
...
static void msort_method_addresses_internal(gpointer *array, int *indexes,
                                            int lo, int hi, gpointer *scratch,
                                            int *scratch_indexes) {
  int mid = (lo + hi) / 2;
  int i, t_lo, t_hi;
  ...
  msort_method_addresses_internal(array, indexes, lo, mid, scratch,
                                  scratch_indexes);
  msort_method_addresses_internal(array, indexes, mid + 1, hi, scratch,
                                  scratch_indexes);
...
static void msort_method_addresses(gpointer *array, int *indexes, int len) {
  gpointer *scratch;
  int *scratch_indexes;

  scratch = g_new(gpointer, len);
  scratch_indexes = g_new(int, len);
  msort_method_addresses_internal(array, indexes, 0, len - 1, scratch,
                                  scratch_indexes);
  g_free(scratch);
 ...
 *
 *   In async context, the resulting MonoJitInfo will not have its method field
 * set, and it will not be added to the jit info tables.
 * FIXME: Large sizes in the lock free allocator
 */
MonoJitInfo *mono_aot_find_jit_info(MonoDomain *domain, MonoImage *image,
                                    gpointer addr) {
  ERROR_DECL(error);
  ...
  if (!amodule->sorted_methods) {
    ...
    if (mono_atomic_cas_ptr((gpointer *)&amodule->sorted_methods, methods,
                            NULL) != NULL)
      /* Somebody got in before us */
      g_free(methods);
    if (mono_atomic_cas_ptr((gpointer *)&amodule->sorted_method_indexes,
                            method_indexes, NULL) != NULL)
      /* Somebody got in before us */
  ...
  /* In async mode, jinfo is not added to the normal jit info table, so have to
   * cache it ourselves */
  if (async) {
  ...
  ex_info =
      &amodule
           ->blob[mono_aot_get_offset(amodule->ex_info_offsets, method_index)];
  ...
  g_assert((guint8 *)code <= (guint8 *)addr &&
           (guint8 *)addr < (guint8 *)code + code_len);

  /* Might be a wrapper/extra method */
  if (!async) {
    if (amodule->extra_methods) {
      amodule_lock(amodule);
      method = (MonoMethod *)g_hash_table_lookup(
          amodule->extra_methods, GUINT_TO_POINTER(method_index));
      amodule_unlock(amodule);
    } else {
      method = NULL;
    }
    ...
    if (!method) {
      if (method_index >= image->tables[MONO_TABLE_METHOD].rows) {
        ...
        mono_error_cleanup(error); /* FIXME don't swallow the error */
        if (!method)
          /* Happens when a random address is passed in which matches a not-yey
           * called wrapper encoded using its name */
          return NULL;
      } else {
        ERROR_DECL(error);
        ...
        if (!method)
          g_error("AOT runtime could not load method due to %s",
                  mono_error_get_message(
                      error)); /* FIXME don't swallow the error */
      }
    }
    /* FIXME: */
    g_assert(method);
  ...
  if (async) {
    ...
    /*
     * Use a simple inmutable table with linear search to cache async jit info
     * entries. This assumes that the number of entries is small.
     */
    while (TRUE) {
      ...
      if (old_table)
        ...
        len = 1;
      new_table = (JitInfoMap *)alloc0_jit_info_data(
          domain, (len + 1) * sizeof(JitInfoMap), async);
      if (old_table)
      ...
      if (mono_atomic_cas_ptr(
              (volatile gpointer *)&amodule->async_jit_info_table, new_table,
              old_table) == old_table)
        break;
    }
  } else {
  ...
  if ((guint8 *)addr >= (guint8 *)jinfo->code_start + jinfo->code_size)
    /* addr is in the padding between methods, see the adjustment of code_size
     * in decode_exception_debug_info () */
    return NULL;

  return jinfo;
}

static gboolean decode_patch(MonoAotModule *aot_module, MonoMemPool *mp,
                             MonoJumpInfo *ji, guint8 *buf, guint8 **endbuf) {
  ERROR_DECL(error);
  guint8 *p = buf;
  gpointer *table;
  MonoImage *image;
  ...
  case MONO_PATCH_INFO_METHOD_CODE_SLOT: {
    ...
    if (!ref.method && !mono_aot_only && !ref.no_aot_trampoline &&
        (ji->type == MONO_PATCH_INFO_METHOD) &&
        (mono_metadata_token_table(ref.token) == MONO_TABLE_METHOD)) {
      ji->data.target = mono_create_ftnptr(
          mono_domain_get(),
          mono_create_jit_trampoline_from_token(ref.image, ref.token));
      ...
      if (ref.method) {
        ...
        ERROR_DECL(error);
        ji->data.method =
            mono_get_method_checked(ref.image, ref.token, NULL, NULL, error);
        if (!ji->data.method)
          g_error("AOT Runtime could not load method due to %s",
                  mono_error_get_message(
                      error)); /* FIXME don't swallow the error */
  ...
  case MONO_PATCH_INFO_DELEGATE_TRAMPOLINE:
    ji->data.del_tramp = (MonoDelegateClassMethodPair *)mono_mempool_alloc0(
    ...
    if (!ji->data.del_tramp->klass)
      goto cleanup;
    if (decode_value(p, &p)) {
      ji->data.del_tramp->method =
          decode_resolve_method_ref(aot_module, p, &p, error);
      mono_error_cleanup(error); /* FIXME don't swallow the error */
  ...
  case MONO_PATCH_INFO_SWITCH:
    ji->data.table = (MonoJumpInfoBBTable *)mono_mempool_alloc0(
        mp, sizeof(MonoJumpInfoBBTable));
    ji->data.table->table_size = decode_value(p, &p);
    table = (void **)mono_domain_alloc(
        mono_domain_get(), sizeof(gpointer) * ji->data.table->table_size);
  ...
  case MONO_PATCH_INFO_LDSTR:
    ...
    if (!image)
      goto cleanup;
    ji->data.token = mono_jump_info_token_new(
        mp, image, MONO_TOKEN_STRING + decode_value(p, &p));
    break;
  ...
  case MONO_PATCH_INFO_TYPE_FROM_HANDLE:
    ...
    if (ji->data.token->has_context) {
      gboolean res = decode_generic_context(
          aot_module, &ji->data.token->context, p, &p, error);
      mono_error_cleanup(error); /* FIXME don't swallow the error */
      if (!res)
        goto cleanup;
    }
  ...
  case MONO_PATCH_INFO_RGCTX_SLOT_INDEX: {
    ...
    entry = (MonoJumpInfoRgctxEntry *)mono_mempool_alloc0(
  ...
  case MONO_PATCH_INFO_GSHAREDVT_CALL: {
    MonoJumpInfoGSharedVtCall *info =
        (MonoJumpInfoGSharedVtCall *)mono_mempool_alloc0(
  ...
  case MONO_PATCH_INFO_GSHAREDVT_METHOD: {
    MonoGSharedVtMethodInfo *info =
        (MonoGSharedVtMethodInfo *)mono_mempool_alloc0(
    ...
    info->entries =
        (MonoRuntimeGenericContextInfoTemplate *)mono_mempool_alloc0(
            mp,
            sizeof(MonoRuntimeGenericContextInfoTemplate) * info->num_entries);
  ...
  case MONO_PATCH_INFO_VIRT_METHOD: {
    MonoJumpInfoVirtMethod *info =
        (MonoJumpInfoVirtMethod *)mono_mempool_alloc0(
 ...
 * decode_patches:
 *
 *    Decode a list of patches identified by the got offsets in GOT_OFFSETS.
 * Return an array of MonoJumpInfo structures allocated from MP. GOT entries
 * already loaded have their ji->type set to MONO_PATCH_INFO_NONE.
 */
static MonoJumpInfo *decode_patches(MonoAotModule *amodule, MonoMemPool *mp,
                                    int n_patches, gboolean llvm,
                                    guint32 *got_offsets) {
  ...
  patches =
      (MonoJumpInfo *)mono_mempool_alloc0(mp, sizeof(MonoJumpInfo) * n_patches);
  for (i = 0; i < n_patches; ++i) {
    guint8 *p =
        amodule->blob + mono_aot_get_offset(got_info_offsets, got_offsets[i]);
...
static MonoJumpInfo *load_patch_info(MonoAotModule *amodule, MonoMemPool *mp,
                                     int n_patches, gboolean llvm,
                                     guint32 **got_slots, guint8 *buf,
                                     guint8 **endbuf) {
  MonoJumpInfo *patches;
  int pindex;
  guint8 *p;
...
static void register_jump_target_got_slot(MonoDomain *domain,
                                          MonoMethod *method,
                                          gpointer *got_slot) {
  /*
 ...
 */
static gpointer load_method(MonoDomain *domain, MonoAotModule *amodule,
                            MonoImage *image, MonoMethod *method, guint32 token,
                            int method_index, MonoError *error) {
  ...
  if (amodule->info.llvm_get_method) {
    /*
     * Obtain the method address by calling a generated function in the LLVM
     * module.
     */
    gpointer (*get_method)(int) =
        (gpointer(*)(int))amodule->info.llvm_get_method;
  ...
  info = &amodule->blob[mono_aot_get_offset(amodule->method_info_offsets,
                                            method_index)];
  ...
  if (!(is_llvm_code(amodule, code) &&
        (amodule->info.flags & MONO_AOT_FILE_FLAG_LLVM_ONLY)) ||
      (mono_llvm_only && method &&
  ...
  if (mono_trace_is_traced(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT)) {
    ...
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_AOT,
               "AOT: FOUND method %s [%p - %p %p]", full_name, code,
               code + jinfo->code_size, info);
    g_free(full_name);
  }

  if (mono_llvm_only) {
    guint8 flags = amodule->method_flags_table[method_index];
    /* The caller needs to looks this up, but its hard to do without
     * constructing the full MonoJitInfo, so save it here */
  ...
  if (MONO_PROFILER_ENABLED(jit_begin) || MONO_PROFILER_ENABLED(jit_done)) {
    ...
    if (!method) {
      method = mono_get_method_checked(amodule->assembly->image, token, NULL,
                                       NULL, error);
      if (!method)
        return NULL;
    }
 ...
 */
static guint32 find_aot_method_in_amodule(MonoAotModule *code_amodule,
                                          MonoMethod *method,
                                          guint32 hash_full) {
  ERROR_DECL(error);
  ...
  // The reference to the metadata amodule will differ among multiple dedup
  // methods which mangle to the same name but live in different assemblies.
  // This leads to the caching breaking. The solution seems to be to cache using
  // the "metadata" amodule.
  MonoAotModule *metadata_amodule =
      m_class_get_image(method->klass)->aot_module;

  if (!metadata_amodule || metadata_amodule->out_of_date || !code_amodule ||
      code_amodule->out_of_date)
    return 0xffffff;
  ...
  while (TRUE) {
    ...
    m = (MonoMethod *)g_hash_table_lookup(
        metadata_amodule->method_ref_to_method, p);
    amodule_unlock(metadata_amodule);
    if (!m) {
      m = decode_resolve_method_ref_with_target(code_amodule, method, p, &p,
                                                error);
      mono_error_cleanup(error); /* FIXME don't swallow the error */
      /*
    ...
    if (m) {
      // printf ("%d %s %s %p\n", n_extra_decodes, mono_method_full_name
      // (method, TRUE), mono_method_full_name (m, TRUE), orig_p);
      n_extra_decodes++;
    }
...
static void add_module_cb(gpointer key, gpointer value, gpointer user_data) {
...
gboolean mono_aot_can_dedup(MonoMethod *method) {
  ...
  if (method->is_inflated &&
      !mono_method_is_generic_sharable_full(method, TRUE, FALSE, FALSE) &&
  ...
  return FALSE;
#else
  gboolean not_normal_gshared =
      method->is_inflated &&
      !mono_method_is_generic_sharable_full(method, TRUE, FALSE, FALSE);
  gboolean extra_method =
      (method->wrapper_type != MONO_WRAPPER_NONE) || not_normal_gshared;
 ...
 */
static guint32 find_aot_method(MonoMethod *method,
  ...
  *out_amodule = m_class_get_image(method->klass)->aot_module;
  index = find_aot_method_in_amodule(
      m_class_get_image(method->klass)->aot_module, method, hash);
  if (index != 0xffffff)
    return index;
...
static gboolean init_method(MonoAotModule *amodule, gpointer info,
  ...
  // FIXME old code would use the class from @method if not null and ignore the
  // one encoded. I don't know if we need to honor that -- @kumpera
  if (method)
  ...
  if (n_patches) {
    ...
    if ((gpointer)code >= amodule->info.jit_code_start &&
        (gpointer)code <= amodule->info.jit_code_end) {
      llvm = FALSE;
      got = amodule->got;
    } else {
      llvm = TRUE;
    ...
    for (pindex = 0; pindex < n_patches; ++pindex) {
      ...
      /*
       * For SFLDA, we need to call resolve_patch_target () since the GOT slot
       * could have been initialized by load_method () for a static cctor before
       * the cctor has finished executing (#23242).
       */
      if (ji->type == MONO_PATCH_INFO_NONE) {
        ...
        if (mono_llvm_only && ji->type == MONO_PATCH_INFO_METHOD &&
            mono_method_check_context_used(ji->data.method)) {
          g_assert(context);
          ji->data.method = mono_class_inflate_generic_method_checked(
              ji->data.method, context, error);
          if (!is_ok(error)) {
            g_free(got_slots);
            mono_mempool_destroy(mp);
            return FALSE;
        ...
        if (ji->type == MONO_PATCH_INFO_AOT_JIT_INFO) {
          ...
          jinfo =
              mono_aot_find_jit_info(domain, amodule->assembly->image, code);
          ji->type = MONO_PATCH_INFO_ABS;
        ...
        if (ji->type == MONO_PATCH_INFO_METHOD_JUMP)
          register_jump_target_got_slot(domain, ji->data.method,
                                        &(got[got_slots[pindex]]));
  ...
  } else if (from_plt && klass_to_run_ctor &&
             !mono_class_is_gtd(klass_to_run_ctor)) {
    MonoVTable *vt =
        mono_class_vtable_checked(domain, klass_to_run_ctor, error);
    if (!is_ok(error))
      inited_ok = FALSE;
    else
      inited_ok = mono_runtime_class_init_full(vt, error);
 ...
 */
gboolean mono_aot_init_llvm_method(gpointer aot_module, gpointer method_info,
                                   MonoClass *init_class, MonoError *error) {
...
gpointer mono_aot_get_method(MonoDomain *domain, MonoMethod *method,
  ...
  if (enable_aot_cache && !amodule && domain->entry_assembly &&
    ...
    if (!mscorlib_aot_loaded) {
      ...
      load_aot_module(mono_domain_default_alc(domain),
                      m_class_get_image(klass)->assembly, NULL, error);
  ...
  if (method->is_inflated && !method->wrapper_type &&
      mono_method_is_generic_sharable_full(method, TRUE, FALSE, FALSE) &&
      !dedupable) {
    MonoMethod *orig_method = method;
    /*
    ...
    if (amodule->llvm_code_start) {
      ...
      /* orig_method is a random instance but it is enough to make init_method
       * () work */
      amodule_lock(amodule);
      g_hash_table_insert(amodule->extra_methods,
                          GUINT_TO_POINTER(method_index), orig_method);
      amodule_unlock(amodule);
    }
  }

  if (method_index == 0xffffff && (method->is_inflated || !method->token)) {
    /* This hash table is used to avoid the slower search in the
     * extra_method_table in the AOT image */
    amodule_lock(amodule);
     ...
     */
    if (method_index == 0xffffff &&
        method->wrapper_type == MONO_WRAPPER_MANAGED_TO_MANAGED &&
        m_class_get_rank(method->klass) &&
        strstr(method->name, "System.Collections.Generic")) {
     ...
     * Generic sharing currently can't handle it, but the icall returns data
     * using an out parameter, so the managed-to-native wrappers can share the
     * same code.
     */
    if (method_index == 0xffffff &&
        method->wrapper_type == MONO_WRAPPER_MANAGED_TO_NATIVE &&
        method->klass == mono_defaults.array_class &&
        !strcmp(method->name, "GetGenericValue_icall")) {
      MonoMethod *m;
      MonoGenericContext ctx;

      if (mono_method_signature_internal(method)->params[1]->type ==
          MONO_TYPE_OBJECT)
        /* Avoid recursion */
        return NULL;

      m = mono_class_get_method_from_name_checked(
          mono_defaults.array_class, "GetGenericValue_icall", 3, 0, error);
      mono_error_assert_ok(error);
      g_assert(m);
      ...
      m = mono_marshal_get_native_wrapper(
          mono_class_inflate_generic_method_checked(m, &ctx, error), TRUE,
          TRUE);
      if (!m)
        g_error(
            "AOT runtime could not load method due to %s",
            mono_error_get_message(error)); /* FIXME don't swallow the error */

      /*
       * Get the code for the <object> instantiation which should be emitted
       * into the mscorlib aot image by the AOT compiler.
       */
      code = (guint8 *)mono_aot_get_method(domain, m, inner_error);
      mono_error_cleanup(inner_error);
      if (code)
    ...
    /* For ARRAY_ACCESSOR wrappers with reference types, use the <object>
     * instantiation saved in corlib */
    if (method_index == 0xffffff &&
      ...
      if (info->subtype == WRAPPER_SUBTYPE_ARRAY_ACCESSOR) {
        ...
        if (MONO_TYPE_IS_REFERENCE(m_class_get_byval_arg(
                m_class_get_element_class(array_method->klass)))) {
          int rank;

          if (!strcmp(array_method->name, "Set"))
            rank =
                mono_method_signature_internal(array_method)->param_count - 1;
          else if (!strcmp(array_method->name, "Get") ||
                   !strcmp(array_method->name, "Address"))
            rank = mono_method_signature_internal(array_method)->param_count;
          else
            g_assert_not_reached();
          MonoClass *obj_array_class =
              mono_class_create_array(mono_defaults.object_class, rank);
          MonoMethod *m = mono_class_get_method_from_name_checked(
              obj_array_class, array_method->name,
              mono_method_signature_internal(array_method)->param_count, 0,
              error);
          mono_error_assert_ok(error);
          g_assert(m);
    ...
    if (method_index == 0xffffff && method->is_inflated &&
        mono_method_is_generic_sharable_full(method, FALSE, TRUE, FALSE)) {
      /* Partial sharing */
      MonoMethod *shared;
    ...
    amodule_lock(amodule);
    g_hash_table_insert(amodule->extra_methods, GUINT_TO_POINTER(method_index),
                        method);
    amodule_unlock(amodule);
  } else {
    /* Common case */
  ...
  code = (guint8 *)load_method(domain, amodule, m_class_get_image(klass),
                               method, method->token, method_index, error);
  if (!is_ok(error))
    return NULL;
...
/**
 * Same as mono_aot_get_method, but we try to avoid loading any metadata from
 * the method.
 */
gpointer mono_aot_get_method_from_token(MonoDomain *domain, MonoImage *image,
  ...
  res =
      load_method(domain, aot_module, image, NULL, token, method_index, error);
  return res;
}
...
static void check_is_got_entry(gpointer key, gpointer value,
                               gpointer user_data) {
  IsGotEntryUserData *data = (IsGotEntryUserData *)user_data;
  MonoAotModule *aot_module = (MonoAotModule *)value;

  if (aot_module->got && (data->addr >= (guint8 *)(aot_module->got)) &&
      (data->addr < (guint8 *)(aot_module->got + aot_module->info.got_size)))
    data->res = TRUE;
}
...
static void find_aot_module_cb(gpointer key, gpointer value,
                               gpointer user_data) {
...
static MonoAotModule *find_aot_module(guint8 *code) {
...
#ifdef MONO_ARCH_CODE_EXEC_ONLY
static guint32 aot_resolve_plt_info_offset(gpointer amodule,
                                           guint32 plt_entry_index) {
  MonoAotModule *module = (MonoAotModule *)amodule;
  return mono_aot_get_offset(module->got_info_offsets,
                             module->info.plt_got_info_offset_base +
                                 plt_entry_index);
}
#endif

void mono_aot_patch_plt_entry(gpointer aot_module, guint8 *code,
                              guint8 *plt_entry, gpointer *got,
                              host_mgreg_t *regs, guint8 *addr) {
  MonoAotModule *amodule = (MonoAotModule *)aot_module;
...
#ifdef MONO_ARCH_CODE_EXEC_ONLY
    mono_arch_patch_plt_entry_exec_only(&amodule->info, plt_entry, amodule->got,
                                        regs, addr);
#else
 ...
 *
 *   This function is called by the entries in the PLT to resolve the actual
 * method that needs to be called. It returns a trampoline to the method and
 * patches the PLT entry. Returns NULL if the something cannot be loaded.
 */
gpointer mono_aot_plt_resolve(gpointer aot_module, host_mgreg_t *regs,
                              guint8 *code, MonoError *error) {
  ...
  plt_info_offset =
      mono_aot_get_plt_info_offset(aot_module, plt_entry, regs, code);
   ...
   */
  if (mono_aot_only && ji.type == MONO_PATCH_INFO_METHOD &&
      !ji.data.method->is_generic &&
      !mono_method_check_context_used(ji.data.method) &&
      !(ji.data.method->iflags & METHOD_IMPL_ATTRIBUTE_SYNCHRONIZED) &&
    ...
    target = (guint8 *)mono_resolve_patch_target(NULL, mono_domain_get(), NULL,
                                                 &ji, TRUE, error);
    if (!is_ok(error)) {
      mono_mempool_destroy(mp);
      return NULL;
  ...
  /*
   * The trampoline expects us to return a function descriptor on platforms
   * which use it, but resolve_patch_target returns a direct function pointer
   * for some type of patches, so have to translate between the two.
  ...
  /* Patch the PLT entry with target which might be the actual method not a
   * trampoline */
  mono_aot_patch_plt_entry(aot_module, code, plt_entry, module->got, regs,
                           target);
 ...
 * init_plt:
 *
 *   Initialize the PLT table of the AOT module. Called lazily when the first
 * AOT method in the module is loaded to avoid committing memory by writing to
 * it.
 */
static void init_plt(MonoAotModule *amodule) {
  int i;
  gpointer tramp;
  ...
  tramp = mono_create_specific_trampoline(amodule, MONO_TRAMPOLINE_AOT_PLT,
...
#ifdef MONO_ARCH_AOT_SUPPORTED
#ifdef MONO_ARCH_CODE_EXEC_ONLY
  target = mono_aot_arch_get_plt_entry_exec_only(&amodule->info, regs, code,
                                                 amodule->plt);
#else
...
#ifdef MONOTOUCH
  while (target != NULL) {
    if ((target >= (guint8 *)(amodule->plt)) &&
        (target < (guint8 *)(amodule->plt_end)))
      return target;
 ...
 */
guint32 mono_aot_get_plt_info_offset(gpointer aot_module, guint8 *plt_entry,
                                     host_mgreg_t *regs, guint8 *code) {
  if (!plt_entry) {
    plt_entry = mono_aot_get_plt_entry(regs, code);
    g_assert(plt_entry);
  }
...
#ifdef MONO_ARCH_AOT_SUPPORTED
#ifdef MONO_ARCH_CODE_EXEC_ONLY
  return mono_arch_get_plt_info_offset_exec_only(
      &((MonoAotModule *)aot_module)->info, plt_entry, regs, code,
      aot_resolve_plt_info_offset, aot_module);
#else
...
static gpointer mono_create_ftnptr_malloc(guint8 *code) {
#ifdef PPC_USES_FUNCTION_DESCRIPTOR
  MonoPPCFunctionDescriptor *ftnptr =
 ...
 */
static gpointer load_function_full(MonoAotModule *amodule, const char *name,
  ...
  if (n_patches) {
    ...
    for (pindex = 0; pindex < n_patches; ++pindex) {
      ...
      if (ji->type == MONO_PATCH_INFO_SPECIFIC_TRAMPOLINE_LAZY_FETCH_ADDR) {
        target = mono_create_specific_trampoline(
            GUINT_TO_POINTER(ji->data.uindex), MONO_TRAMPOLINE_RGCTX_LAZY_FETCH,
            mono_get_root_domain(), NULL);
        ...
        target = &amodule->got[amodule->info.trampoline_got_offset_base
                                   [MONO_AOT_TRAMP_SPECIFIC]];
      } else if (ji->type == MONO_PATCH_INFO_JIT_ICALL_ADDR) {
        const MonoJitICallId jit_icall_id =
            (MonoJitICallId)ji->data.jit_icall_id;
        switch (jit_icall_id) {
  ...
  case MONO_JIT_ICALL_##x:                                                     \
    ...
    break;
        ...
        case MONO_JIT_ICALL_generic_trampoline_vcall:
          target = (gpointer)mono_get_trampoline_func(
              mono_jit_icall_id_to_trampoline_type(jit_icall_id));
          break;
        default:
         ...
         * domain to be set.
         */
        target = mono_resolve_patch_target(NULL, NULL, (guint8 *)code, ji,
                                           FALSE, error);
        mono_error_assert_ok(error);
        g_assert(target);
      }
...
static gpointer load_function(MonoAotModule *amodule, const char *name) {
  return load_function_full(amodule, name, NULL);
}

static MonoAotModule *get_mscorlib_aot_module(void) {
  MonoImage *image;
  MonoAotModule *amodule;
...
static void mono_no_trampolines(void) { g_assert_not_reached(); }

/*
 * Return the trampoline identified by NAME from the mscorlib AOT file.
 * On ppc64, this returns a function descriptor.
 */
gpointer mono_aot_get_trampoline_full(const char *name,
                                      MonoTrampInfo **out_tinfo) {
  ...
  return mono_create_ftnptr_malloc(
      (guint8 *)load_function_full(amodule, name, out_tinfo));
}
...
static gpointer read_unwind_info(MonoAotModule *amodule, MonoTrampInfo *info,
...
static void read_page_trampoline_uwinfo(MonoTrampInfo *info, int tramp_type,
                                        gboolean is_generic) {
  char symbol_name[128];

  if (tramp_type == MONO_AOT_TRAMP_SPECIFIC)
    sprintf(symbol_name, "specific_trampolines_page_%s_p",
            is_generic ? "gen" : "sp");
  else if (tramp_type == MONO_AOT_TRAMP_STATIC_RGCTX)
    sprintf(symbol_name, "rgctx_trampolines_page_%s_p",
            is_generic ? "gen" : "sp");
  else if (tramp_type == MONO_AOT_TRAMP_IMT)
    sprintf(symbol_name, "imt_trampolines_page_%s_p",
            is_generic ? "gen" : "sp");
  else if (tramp_type == MONO_AOT_TRAMP_GSHAREDVT_ARG)
    sprintf(symbol_name, "gsharedvt_trampolines_page_%s_p",
            is_generic ? "gen" : "sp");
  else if (tramp_type == MONO_AOT_TRAMP_UNBOX_ARBITRARY)
    sprintf(symbol_name, "unbox_arbitrary_trampolines_page_%s_p",
            is_generic ? "gen" : "sp");
  else
    g_assert_not_reached();
...
static unsigned char *get_new_trampoline_from_page(int tramp_type) {
  MonoImage *image;
  TrampolinePage *page;
  int count;
  void *tpage;
  ...
  while (page == NULL && count-- > 0) {
    ...
    addr = 0;
    /* allocate two contiguous pages of memory: the first page will contain the
     * data (like a local constant pool) while the second will contain the
     * trampolines.
     */
    do {
    ...
    ret = vm_remap(mach_task_self(), &taddr, psize, 0, FALSE, mach_task_self(),
                   (vm_address_t)tpage, FALSE, &prot, &max_prot,
                   VM_INHERIT_SHARE);
    if (ret != KERN_SUCCESS) {
    ...
    page->trampolines =
        (guint8 *)(taddr + amodule->info.tramp_page_code_offsets[tramp_type]);
    ...
    gen_info = mono_tramp_info_create(
        NULL, (guint8 *)taddr,
        amodule->info.tramp_page_code_offsets[tramp_type], NULL, NULL);
    ...
    if (tramp_type != MONO_AOT_TRAMP_SPECIFIC) {
      ...
      sp_info = mono_tramp_info_create(NULL, code, page->trampolines_end - code,
                                       NULL, NULL);
...
static gpointer get_new_specific_trampoline_from_page(gpointer tramp,
                                                      gpointer arg) {
  void *code;
  gpointer *data;
  ...
  data[0] = arg;
  data[1] = tramp;
  /*g_warning ("new trampoline at %p for data %p, tramp %p (stored at %p)",
   * code, arg, tramp, data);*/
  return code;
}

static gpointer get_new_rgctx_trampoline_from_page(gpointer tramp,
                                                   gpointer arg) {
  void *code;
  gpointer *data;
  ...
  data[0] = arg;
  data[1] = tramp;
  /*g_warning ("new rgctx trampoline at %p for data %p, tramp %p (stored at
   * %p)", code, arg, tramp, data);*/
  return code;
}

static gpointer get_new_imt_trampoline_from_page(gpointer arg) {
  void *code;
  gpointer *data;
  ...
  data = (gpointer *)((char *)code - MONO_AOT_TRAMP_PAGE_SIZE);
  data[0] = arg;
  /*g_warning ("new imt trampoline at %p for data %p, (stored at %p)", code,
   * arg, data);*/
  return code;
}

static gpointer get_new_gsharedvt_arg_trampoline_from_page(gpointer tramp,
                                                           gpointer arg) {
  void *code;
  gpointer *data;
...
static gpointer get_new_unbox_arbitrary_trampoline_frome_page(gpointer addr) {
  void *code;
  gpointer *data;
...
static gpointer get_numerous_trampoline(MonoAotTrampoline tramp_type,
                                        int n_got_slots,
                                        MonoAotModule **out_amodule,
                                        guint32 *got_offset,
                                        guint32 *out_tramp_size) {
  MonoImage *image;
  MonoAotModule *amodule = get_mscorlib_aot_module();
...
#define MONOTOUCH_TRAMPOLINES_ERROR                                            \
  ". See http://docs.xamarin.com/ios/troubleshooting for instructions on how " \
  "to fix this condition."
#else
#define MONOTOUCH_TRAMPOLINES_ERROR ""
#endif
  if (amodule->trampoline_index[tramp_type] ==
      amodule->info.num_trampolines[tramp_type]) {
    g_error("Ran out of trampolines of type %d in '%s' (limit %d)%s\n",
            tramp_type, image ? image->name : MONO_ASSEMBLY_CORLIB_NAME,
            amodule->info.num_trampolines[tramp_type],
  ...
  *got_offset = amodule->info.trampoline_got_offset_base[tramp_type] +
                (index * n_got_slots);
...
static void no_specific_trampoline(void) { g_assert_not_reached(); }

/*
 * Return a specific trampoline from the AOT file.
 */
gpointer mono_aot_create_specific_trampoline(gpointer arg1,
  ...
  if (!inited) {
    ...
    if (!inited) {
      mono_counters_register("Specific trampolines",
  ...
  if (USE_PAGE_TRAMPOLINES) {
    ...
    code = (guint8 *)get_numerous_trampoline(
        MONO_AOT_TRAMP_SPECIFIC, 2, &amodule, &got_offset, &tramp_size);
...
gpointer mono_aot_get_static_rgctx_trampoline(gpointer ctx, gpointer addr) {
  ...
  if (USE_PAGE_TRAMPOLINES) {
    ...
    code = (guint8 *)get_numerous_trampoline(MONO_AOT_TRAMP_STATIC_RGCTX, 2,
                                             &amodule, &got_offset, NULL);
...
gpointer mono_aot_get_unbox_arbitrary_trampoline(gpointer addr) {
  ...
  if (USE_PAGE_TRAMPOLINES) {
    ...
    code = (guint8 *)get_numerous_trampoline(MONO_AOT_TRAMP_UNBOX_ARBITRARY, 1,
                                             &amodule, &got_offset, NULL);
...
static int i32_idx_comparer(const void *key, const void *member) {
  gint32 idx1 = GPOINTER_TO_INT(key);
  gint32 idx2 = *(gint32 *)member;
  return idx1 - idx2;
}

static int i16_idx_comparer(const void *key, const void *member) {
  int idx1 = GPOINTER_TO_INT(key);
  int idx2 = *(gint16 *)member;
  return idx1 - idx2;
}

static gboolean aot_is_slim_amodule(MonoAotModule *amodule) {
  if (!amodule)
    return FALSE;
...
gpointer mono_aot_get_unbox_trampoline(MonoMethod *method, gpointer addr) {
  ...
  if (method->is_inflated &&
      !mono_method_is_generic_sharable_full(method, FALSE, FALSE, FALSE)) {
    method_index = find_aot_method(method, &amodule);
    if (method_index == 0xffffff &&
        mono_method_is_generic_sharable_full(method, FALSE, TRUE, FALSE)) {
      MonoMethod *shared =
          mini_get_shared_method_full(method, SHARE_MODE_NONE, error);
      mono_error_assert_ok(error);
      method_index = find_aot_method(shared, &amodule);
    }
    if (method_index == 0xffffff &&
        mono_method_is_generic_sharable_full(method, FALSE, TRUE, TRUE)) {
      MonoMethod *shared =
          mini_get_shared_method_full(method, SHARE_MODE_GSHAREDVT, error);
      mono_error_assert_ok(error);
  ...
  if (amodule->info.llvm_unbox_tramp_indexes) {
    ...
    if (amodule->info.llvm_unbox_tramp_elemsize == sizeof(guint32)) {
      void *ptr = mono_binary_search(
          GINT_TO_POINTER(method_index), amodule->info.llvm_unbox_tramp_indexes,
          amodule->info.llvm_unbox_tramp_num,
          amodule->info.llvm_unbox_tramp_elemsize, i32_idx_comparer);
      g_assert(ptr);
      g_assert(*(int *)ptr == method_index);
      unbox_tramp_idx =
          (guint32 *)ptr - (guint32 *)amodule->info.llvm_unbox_tramp_indexes;
    } else {
      void *ptr = mono_binary_search(
          GINT_TO_POINTER(method_index), amodule->info.llvm_unbox_tramp_indexes,
          amodule->info.llvm_unbox_tramp_num,
          amodule->info.llvm_unbox_tramp_elemsize, i16_idx_comparer);
      g_assert(ptr);
      g_assert(*(gint16 *)ptr == method_index);
      unbox_tramp_idx =
          (guint16 *)ptr - (guint16 *)amodule->info.llvm_unbox_tramp_indexes;
    }
    g_assert(unbox_tramp_idx < amodule->info.llvm_unbox_tramp_num);
    code =
        ((gpointer *)(amodule->info.llvm_unbox_trampolines))[unbox_tramp_idx];
    g_assert(code);
    return code;
  }

  if (amodule->info.llvm_get_unbox_tramp) {
    gpointer (*get_tramp)(int) =
        (gpointer(*)(int))amodule->info.llvm_get_unbox_tramp;
  ...
  if (amodule->info.flags & MONO_AOT_FILE_FLAG_CODE_EXEC_ONLY)
    ...
    code =
        get_call_table_entry(amodule->unbox_trampoline_addresses, entry_index,
                             amodule->info.call_table_entry_size);
  ...
  gpointer const symbol_addr =
      read_unwind_info(amodule, tinfo, "unbox_trampoline_p");
  if (!symbol_addr) {
...
gpointer mono_aot_get_lazy_fetch_trampoline(guint32 slot) {
  ...
  if (index >= amodule->info.num_rgctx_fetch_trampolines) {
    ...
    /*
     * Use the general version of the rgctx fetch trampoline. It receives a pair
     * of <slot, trampoline> in the rgctx arg reg.
     */
    if (!addr)
    ...
    info[1] = mono_create_specific_trampoline(GUINT_TO_POINTER(slot),
...
static void no_imt_trampoline(void) { g_assert_not_reached(); }

gpointer mono_aot_get_imt_trampoline(MonoVTable *vtable, MonoDomain *domain,
  ...
  buf = (void **)mono_domain_alloc(domain,
                                   (real_count + 1) * 2 * sizeof(gpointer));
  index = 0;
  for (i = 0; i < count; ++i) {
    MonoIMTCheckItem *item = imt_entries[i];
  ...
  if (USE_PAGE_TRAMPOLINES) {
    ...
    code = get_numerous_trampoline(MONO_AOT_TRAMP_IMT, 1, &amodule, &got_offset,
                                   NULL);
...
gpointer mono_aot_get_gsharedvt_arg_trampoline(gpointer arg, gpointer addr) {
  ...
  if (USE_PAGE_TRAMPOLINES) {
    ...
    code = (guint8 *)get_numerous_trampoline(MONO_AOT_TRAMP_GSHAREDVT_ARG, 2,
                                             &amodule, &got_offset, NULL);
...
gpointer mono_aot_get_ftnptr_arg_trampoline(gpointer arg, gpointer addr) {
  ...
  if (USE_PAGE_TRAMPOLINES) {
    ...
    code = (guint8 *)get_numerous_trampoline(MONO_AOT_TRAMP_FTNPTR_ARG, 2,
                                             &amodule, &got_offset, NULL);
 ...
 * SIGSEGV handler, this is useful to find out which pages the runtime tries to
 * read.
 */
void mono_aot_set_make_unreadable(gboolean unreadable) {
  ...
  if (make_unreadable && !inited) {
    mono_counters_register("AOT: pagefaults",
                           MONO_COUNTER_JIT | MONO_COUNTER_INT, &n_pagefaults);
  }
}
...
static void find_map(gpointer key, gpointer value, gpointer user_data) {
...
static MonoAotModule *find_module_for_addr(void *ptr) {
 ...
 *
 *   Handle a pagefault caused by an unreadable page by making it readable
 * again.
 */
void mono_aot_handle_pagefault(void *ptr) {
  ...
  mono_aot_lock();
  res = mono_mprotect(start, mono_pagesize(),
...
gboolean mono_aot_is_got_entry(guint8 *code, guint8 *addr) { return FALSE; }
...
guint8 *mono_aot_get_plt_entry(guint8 *code) { return NULL; }
...
gboolean mono_aot_is_pagefault(void *ptr) { return FALSE; }
...
GHashTable *mono_aot_get_weak_field_indexes(MonoImage *image) { return NULL; }

MonoAotMethodFlags mono_aot_get_method_flags(guint8 *code) {
  return MONO_AOT_METHOD_FLAG_NONE;
}

gboolean mono_aot_init_llvmonly_method(gpointer aot_module,
                                       guint32 method_index,
                                       MonoClass *init_class,
                                       MonoError *error) {
  g_assert_not_reached();
  return FALSE;
}
