...
gpointer mono_arch_get_unbox_trampoline(MonoMethod *method, gpointer addr) {
  guint8 *code, *start;
  ...
  MONO_PROFILER_RAISE(jit_code_buffer,
                      (start, code - start,
                       MONO_PROFILER_CODE_BUFFER_UNBOX_TRAMPOLINE, method));
...
guint8 *mono_arch_create_sdb_trampoline(gboolean single_step,
  ...
  int tramp_size = 512;
  int i, framesize, ctx_offset, gr_offset, fp_offset, ip_offset, sp_offset;
  guint8 *code, *buf;
  void *ep;
  ...
  /**
   * Create unwind information - On entry s390_r1 has value of method's frame
   * reg
   */
  ...
  fp_offset =
      ctx_offset + G_STRUCT_OFFSET(MonoContext, uc_mcontext.fpregs.fprs);
...
guchar *mono_arch_create_generic_trampoline(MonoTrampolineType tramp_type,
  ...
  mono_add_unwind_op_def_cfa_offset(unwind_ops, code, buf,
                                    sizeof(trampStack_t));
  ...
  /*---------------------------------------------------------------*/
  s390_stg(buf, s390_r2, 0, LMFReg, G_STRUCT_OFFSET(MonoLMF, lmf_addr));
  ...
  /*---------------------------------------------------------------*/
  s390_stg(buf, s390_r0, 0, LMFReg, G_STRUCT_OFFSET(MonoLMF, previous_lmf));
  ...
  /* Calculate call address and call the C trampoline. Return value will be in
   * r2 */
...
gpointer mono_arch_create_specific_trampoline(gpointer arg1,
  ...
  MONO_PROFILER_RAISE(
      jit_code_buffer,
      (buf, code - buf, MONO_PROFILER_CODE_BUFFER_SPECIFIC_TRAMPOLINE,
...
gpointer mono_arch_create_rgctx_lazy_fetch_trampoline(guint32 slot,
  ...
  gint32 displace;
  int tramp_size, depth, index, iPatch = 0, i;
  gboolean mrgctx;
  MonoJumpInfo *ji = NULL;
  GSList *unwind_ops = NULL;
  ...
  for (i = 0; i < depth; ++i) {
    ...
    if (mrgctx && i == 0)
      s390_lg(code, s390_r1, 0, s390_r1,
