...
static int mono_jit_exec_internal(MonoDomain *domain, MonoAssembly *assembly,
                                  int argc, char *argv[]);
...
#define OPTFLAG(id, shift, name, desc)                                         \
  char MSGSTRFIELD(__LINE__)[sizeof(name) + sizeof(desc)];
...
#define OPTFLAG(id, shift, name, desc)                                         \
  offsetof(struct msgstr_t, MSGSTRFIELD(__LINE__)),
#include "optflags-def.h"
#undef OPTFLAG
};

#define optflag_get_name(id) ((const char *)&opstr + opt_names[(id)])
#define optflag_get_desc(id)                                                   \
  (optflag_get_name(id) + 1 + strlen(optflag_get_name(id)))

#define DEFAULT_OPTIMIZATIONS                                                  \
  (MONO_OPT_PEEPHOLE | MONO_OPT_CFOLD | MONO_OPT_INLINE | MONO_OPT_CONSPROP |  \
   MONO_OPT_COPYPROP | MONO_OPT_DEADCE | MONO_OPT_BRANCH | MONO_OPT_LINEARS |  \
   MONO_OPT_INTRINS | MONO_OPT_LOOP | MONO_OPT_EXCEPTION | MONO_OPT_CMOV |     \
   MONO_OPT_GSHARED | MONO_OPT_SIMD | MONO_OPT_ALIAS_ANALYSIS | MONO_OPT_AOT | \
   MONO_OPT_FLOAT32)

#define EXCLUDED_FROM_ALL                                                      \
  (MONO_OPT_SHARED | MONO_OPT_PRECOMP | MONO_OPT_UNSAFE | MONO_OPT_GSHAREDVT)

static char *mono_parse_options(const char *options, int *ref_argc,
                                char **ref_argv[], gboolean prepend);
static char *mono_parse_response_options(const char *options, int *ref_argc,
                                         char **ref_argv[], gboolean prepend);

static guint32 parse_optimizations(guint32 opt, const char *p,
                                   gboolean cpu_opts) {
  guint32 exclude = 0;
  const char *n;
  int i, invert;
...
static gboolean parse_debug_options(const char *p) {
...
static const GraphName graph_names[] = {
...
static MonoGraphOptions mono_parse_graph_options(const char *p) {
  const char *n;
  int i, len;
...
static const guint32 opt_sets[] = {
    ...
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_ALIAS_ANALYSIS,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_LOOP | MONO_OPT_INLINE |
        MONO_OPT_INTRINS,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_LOOP | MONO_OPT_INLINE |
        MONO_OPT_INTRINS | MONO_OPT_TAILCALL,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_LOOP | MONO_OPT_INLINE |
        MONO_OPT_INTRINS | MONO_OPT_SSA,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_LOOP | MONO_OPT_INLINE |
        MONO_OPT_INTRINS | MONO_OPT_EXCEPTION,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_LOOP | MONO_OPT_INLINE |
        MONO_OPT_INTRINS | MONO_OPT_EXCEPTION | MONO_OPT_CMOV,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_LOOP | MONO_OPT_INLINE |
        MONO_OPT_INTRINS | MONO_OPT_EXCEPTION | MONO_OPT_ABCREM,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_LOOP | MONO_OPT_INLINE |
        MONO_OPT_INTRINS | MONO_OPT_ABCREM,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_LINEARS | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_LOOP | MONO_OPT_INLINE |
        MONO_OPT_INTRINS | MONO_OPT_ABCREM | MONO_OPT_SHARED,
    MONO_OPT_BRANCH | MONO_OPT_PEEPHOLE | MONO_OPT_COPYPROP |
        MONO_OPT_CONSPROP | MONO_OPT_DEADCE | MONO_OPT_LOOP | MONO_OPT_INLINE |
        MONO_OPT_INTRINS | MONO_OPT_EXCEPTION | MONO_OPT_CMOV,
    DEFAULT_OPTIMIZATIONS,
};

static const guint32 interp_opt_sets[] = {
    INTERP_OPT_NONE,
    INTERP_OPT_INLINE,
    INTERP_OPT_CPROP,
...
static const char *const interp_opflags_names[] = {"inline", "cprop",
                                                   "super-insn"};

static const char *interp_optflag_get_name(guint32 i) {
  g_assert(i < G_N_ELEMENTS(interp_opflags_names));
  return interp_opflags_names[i];
}

static char *interp_opt_descr(guint32 flags) {
  GString *str = g_string_new("");
  int i;
  gboolean need_comma;
...
static gboolean method_should_be_regression_tested(MonoMethod *method,
                                                   gboolean interp) {
  ERROR_DECL(error);
  ...
  for (j = 0; j < ainfo->num_attrs; ++j) {
    ...
    if (strcmp(m_class_get_name(klass), "CategoryAttribute") ||
        mono_method_signature_internal(centry->ctor)->param_count != 1)
      continue;
    ...
    char *utf8_str = (char *)(void *)
        typed_args[0]; // this points into image memory that is constant
    g_free(typed_args);
    g_free(named_args);
    g_free(arginfo);
    ...
    if ((mono_aot_mode == MONO_AOT_MODE_INTERP_LLVMONLY ||
         mono_aot_mode == MONO_AOT_MODE_LLVMONLY) &&
        !strcmp(utf8_str, "!BITCODE")) {
      g_print("skip %s...\n", method->name);
      return FALSE;
    }
  }
...
static void mini_regression_step(MonoImage *image, int verbose, int *total_run,
                                 int *total, guint32 opt_flags, GTimer *timer,
                                 MonoDomain *domain) {
  ...
  if (domain_jit_info(domain)) {
    ...
    domain_jit_info(domain)->jit_trampoline_hash =
  ...
  for (i = 0; i < mono_image_get_table_rows(image, MONO_TABLE_METHOD); ++i) {
    ERROR_DECL(error);
    MonoMethod *method = mono_get_method_checked(
        image, MONO_TOKEN_METHOD_DEF | (i + 1), NULL, NULL, error);
    if (!method) {
    ...
    if (method_should_be_regression_tested(method, FALSE)) {
      ...
      ERROR_DECL(error);
      func = (TestMethod)mono_aot_get_method(mono_get_root_domain(), method,
                                             error);
      mono_error_cleanup(error);
#else
      ...
      cfg = mini_method_compile(
          method, mono_get_optimizations_for_method(method, opt_flags),
          mono_get_root_domain(), JIT_FLAG_RUN_CCTORS, 0, -1);
      ...
      if (cfg->exception_type == MONO_EXCEPTION_NONE) {
        ...
        func = (TestMethod)(gpointer)cfg->native_code;
#endif
        func = (TestMethod)mono_create_ftnptr(mono_get_root_domain(),
                                              (gpointer)func);
      }
#endif
...
static int mini_regression(MonoImage *image, int verbose, int *total_run) {
  guint32 i, opt;
  MonoMethod *method;
  char *n;
  ...
  if (mono_do_single_method_regression) {
    ...
    for (opt = 0; opt < G_N_ELEMENTS(opt_sets); ++opt) {
      ...
      if (!strcmp("builtin-types", image->assembly_name) ||
          !strcmp("aot-tests", image->assembly_name))
      ...
      if ((mono_aot_mode == MONO_AOT_MODE_FULL ||
           mono_aot_mode == MONO_AOT_MODE_LLVMONLY) &&
          opt_sets[opt] != DEFAULT_OPTIMIZATIONS) {
        continue;
      }

      mini_regression_step(image, verbose, total_run, &total,
                           opt_sets[opt] & ~exclude, timer, domain);
    }
  }
...
static int mini_regression_list(int verbose, int count, char *images[]) {
  ...
  for (i = 0; i < count; ++i) {
    ...
    total +=
        mini_regression(mono_assembly_get_image_internal(ass), verbose, &run);
    total_run += run;
  }
  if (total > 0) {
    g_print("Overall results: tests: %d, failed: %d, opt combinations: %d "
            "(pass: %.2f%%)\n",
            total_run, total, (int)G_N_ELEMENTS(opt_sets),
            100.0 * (total_run - total) / total_run);
  } else {
...
static void interp_regression_step(MonoImage *image, int verbose,
                                   int *total_run, int *total,
                                   const guint32 *opt_flags, GTimer *timer,
                                   MonoDomain *domain) {
  ...
  for (i = 0; i < mono_image_get_table_rows(image, MONO_TABLE_METHOD); ++i) {
    ...
    if (method_should_be_regression_tested(method, TRUE)) {
      ...
      result_obj = mini_get_interp_callbacks()->runtime_invoke(
          method, NULL, NULL, &exc, interp_error);
...
static int interp_regression(MonoImage *image, int verbose, int *total_run) {
  MonoMethod *method;
  GTimer *timer = g_timer_new();
  MonoDomain *domain = mono_domain_get();
  guint32 i;
  ...
  *total_run = 0;
  ...
  if (mono_interp_opts_string) {
    ...
    for (int opt = 0; opt < G_N_ELEMENTS(interp_opt_sets); ++opt)
      interp_regression_step(image, verbose, total_run, &total,
                             &interp_opt_sets[opt], timer, domain);
  }
...
static int mono_interp_regression_list(int verbose, int count, char *images[]) {
  int i, total, total_run, run;
  ...
  for (i = 0; i < count; ++i) {
    ...
    total +=
        interp_regression(mono_assembly_get_image_internal(ass), verbose, &run);
    total_run += run;
  }
  if (total > 0) {
...
static JitInfoData *alloc_random_data(Region *region) {
  JitInfoData **data;
  JitInfoData *prev;
  guint prev_end;
  guint next_start;
...
static JitInfoData **choose_random_data(Region *region) {
  int n;
  int i;
  JitInfoData **d;
  ...
  for (d = &region->data, i = 0; i < n; d = &(*d)->next, ++i)
    ;

  return d;
}

static Region *choose_random_region(ThreadData *td) {
  return &td->regions[random() % td->num_regions];
}

static ThreadData *choose_random_thread(void) {
  return &thread_datas[random() % num_threads];
}

static void free_jit_info_data(ThreadData *td, JitInfoData *free) {
  free->next = td->frees;
  td->frees = free;
...
static void test_thread_func(gpointer void_arg) {
  ...
  gboolean modify_thread =
      thread_num <
      NUM_THREADS / 2; /* only half of the threads modify the table */
  ...
  for (;;) {
    ...
    if (lookup) {
      ...
      if (modify_thread && random() % 10 < 5) {
         ...
         */
        if (ji != NULL) {
          g_assert(addr >= (char *)ji->code_start &&
                   addr < (char *)ji->code_start + ji->code_size);
          ++lookup_successes;
        } else
          ++lookup_failures;
      }
...
static void jit_info_table_test(MonoDomain *domain) {
  ERROR_DECL(error);
  int i;
  ...
  */
  ...
  for (i = 0; i < num_threads; ++i) {
    mono_thread_create_checked(domain, (gpointer)test_thread_func,
                               &thread_datas[i], error);
...
static void compile_all_methods_thread_main_inner(CompileAllThreadArgs *args) {
  ...
  for (i = 0; i < mono_image_get_table_rows(image, MONO_TABLE_METHOD); ++i) {
    ...
    if (mono_use_interpreter) {
      ...
      // FIXME There are a few failures due to DllNotFoundException related to
      // System.Native
      ...
      cfg = mini_method_compile(
          method, mono_get_optimizations_for_method(method, args->opts),
...
static void compile_all_methods_thread_main(gpointer void_args) {
...
static void compile_all_methods(MonoAssembly *ass, int verbose, guint32 opts,
                                guint32 recompilation_times) {
  ERROR_DECL(error);
   ...
   */
  mono_thread_create_checked(mono_domain_get(),
                             (gpointer)compile_all_methods_thread_main, &args,
                             error);
  mono_error_assert_ok(error);
 ...
 */
int mono_jit_exec(MonoDomain *domain, MonoAssembly *assembly, int argc,
                  char *argv[]) {
  int rv;
...
int mono_jit_exec_internal(MonoDomain *domain, MonoAssembly *assembly, int argc,
                           char *argv[]) {
  ...
  // https://blogs.msdn.microsoft.com/junfeng/2005/11/19/module-initializer-a-k-a-module-constructor/
  //
...
static void main_thread_handler(gpointer user_data) {
  ...
  if (mono_compile_aot) {
    ...
    for (i = 0; i < main_args->argc; ++i) {
      assembly = mono_domain_assembly_open_internal(
          main_args->domain, mono_domain_default_alc(main_args->domain),
          main_args->argv[i]);
      if (!assembly) {
        if (mono_is_problematic_file(main_args->argv[i])) {
          fprintf(stderr,
                  "Info: AOT of problematic assembly %s skipped. This is "
                  "expected.\n",
                  main_args->argv[i]);
          continue;
        } else {
      ...
      /* Check that the assembly loaded matches the filename */
      {
        ...
        if (img && strcmp(img->name, assembly->image->name)) {
          fprintf(stderr,
                  "Error: Loaded assembly '%s' doesn't match original file "
                  "name '%s'. Set MONO_PATH to the assembly's location.\n",
                  assembly->image->name, img->name);
          exit(1);
        }
      }
      res = mono_compile_assembly(assembly, main_args->opts,
                                  main_args->aot_options, &aot_state);
      if (res != 0) {
    ...
    if (aot_state) {
      res = mono_compile_deferred_assemblies(
          main_args->opts, main_args->aot_options, &aot_state);
      if (res != 0) {
...
static int load_agent(MonoDomain *domain, char *desc) {
  ERROR_DECL(error);
  ...
  method = mono_get_method_checked(image, entry, NULL, NULL, error);
  if (method == NULL) {
    g_print("The entry point method of assembly '%s' could not be loaded due "
            "to %s\n",
  ...
  if (args) {
    main_args = (MonoArray *)mono_array_new_checked(
        domain, mono_defaults.string_class, 1, error);
    if (main_args) {
  ...
  if (!is_ok(error)) {
    g_print(
        "The entry point method of assembly '%s' could not execute due to %s\n",
        agent, mono_error_get_message(error));
    mono_error_cleanup(error);
    g_free(agent);
    return 1;
...
static void mini_usage_jitdeveloper(void) {
  int i;

  fprintf(
      stdout,
      "Runtime and JIT debugging options:\n"
      "    --apply-bindings=FILE  Apply assembly bindings from FILE (only for "
      "AOT)\n"
      ...
      "    --compile-all=N        Compiles all the methods in the assembly "
      ...
      "assembly\n"
      "    --single-method=OPTS   Runs regressions with only one method "
      "optimized with OPTS at any time\n"
      "    --statfile FILE        Sets the stat file to FILE\n"
      "    --stats                Print statistics about the JIT operations\n"
      "    --inject-async-exc METHOD OFFSET Inject an asynchronous exception "
      "at METHOD\n"
      "    --verify-all           Run the verifier on all assemblies and "
      "methods\n"
      "    --full-aot             Avoid JITting any code\n"
      "    --llvmonly             Use LLVM compiled code only\n"
      "    --agent=ASSEMBLY[:ARG] Loads the specific agent assembly and "
      "executes its Main method with the given argument before loading the "
      "main assembly.\n"
      "    --no-x86-stack-align   Don't align stack on x86\n"
      "\n"
      "The options supported by MONO_DEBUG can also be passed on the command "
      "line.\n"
      "\n"
      "Other options:\n"
...
static void mini_usage_list_opt(void) {
  int i;
...
static void mini_usage(void) {
  fprintf(
      ...
      "    --debug=[<options>]    Disable debugging support or enable "
      "debugging extras, use --help-debug for details\n"
#else
      ...
      "profiler module\n"
      "    --trace[=EXPR]         Enable tracing, use --help-trace for "
      "details\n"
...
#endif
      "    --handlers             Install custom handlers, use --help-handlers "
      "for details.\n"
      "    --aot-path=PATH        List of additional directories to search for "
      "AOT images.\n");
}

static void mini_trace_usage(void) {
  fprintf(
      stdout,
      "Tracing options:\n"
      ...
      "    disabled             Don't print any output until toggled via "
      "SIGUSR2\n");
}

static void mini_debug_usage(void) {
  fprintf(
      stdout,
      "Debugging options:\n"
#ifdef ENABLE_NETCORE
      "   --debug[=OPTIONS]     Disable debugging support or enable debugging "
      "extras, optional OPTIONS is a comma\n"
#else
      "   --debug[=OPTIONS]     Enable debugging support, optional OPTIONS is "
      "a comma\n"
#endif
...
#endif
      "    casts                Enable more detailed InvalidCastException "
      "messages.\n"
      "    mdb-optimizations    Disable some JIT optimizations which are "
      "normally\n"
      "                         disabled when running inside the debugger.\n"
      "                         This is useful if you plan to attach to the "
      "running\n"
...
static char *mono_get_version_info(void) {
  GString *output;
  output = g_string_new("");
...
#define error_if_aot_unsupported()                                             \
  do {                                                                         \
    fprintf(stderr, "AOT compilation is not supported on this platform.\n");   \
    exit(1);                                                                   \
  } while (0)
#else
#define error_if_aot_unsupported()
...
static void enable_runtime_stats(void) {
  mono_counters_enable(-1);
  mono_atomic_store_bool(&mono_stats.enabled, TRUE);
  mono_atomic_store_bool(&mono_jit_stats.enabled, TRUE);
}

static MonoMethodDesc *parse_qualified_method_name(char *method_name) {
 ...
 *
 * Process the command line options in \p argv as done by the runtime
 * executable. This should be called before \c mono_jit_init.
 */
  ...
  for (i = 0; i < argc; ++i) {
    ...
    if (strncmp(argv[i], "--debugger-agent=", 17) == 0) {
      ...
      mono_use_llvm = TRUE;
               ...
               mini_parse_debug_option(argv[i] + 2)) {
    } else {
...
static void mono_set_use_smp(int use_smp) {
#if HAVE_SCHED_SETAFFINITY
  if (!use_smp) {
...
#else
    sched_setaffinity(getpid(), sizeof(unsigned long),
                      (const cpu_set_t *)&proc_mask);
#endif
  }
#endif
}

static void switch_gc(char *argv[], const char *target_gc) {
  GString *path;
 ...
 */
static void darwin_change_default_file_handles() {
  struct rlimit limit;
...
static void switch_arch(char *argv[], const char *target_arch) {
  GString *path;
  gsize arch_offset;

  if ((strcmp(target_arch, "32") == 0 &&
       strcmp(MONO_ARCHITECTURE, "x86") == 0) ||
      (strcmp(target_arch, "64") == 0 &&
       strcmp(MONO_ARCHITECTURE, "amd64") == 0)) {
  ...
  if (strstr(&path->str[arch_offset], "32") ||
      strstr(&path->str[arch_offset], "64")) {
...
static void apply_root_domain_configuration_file_bindings(
    MonoDomain *domain, char *root_domain_configuration_file) {
  g_assert(domain->setup == NULL || domain->setup->configuration_file == NULL);
  g_assert(!domain->assembly_bindings_parsed);

  mono_domain_parse_assembly_bindings(domain, 0, 0,
                                      root_domain_configuration_file);
}

static void mono_check_interp_supported(void) {
...
static int mono_exec_regression_internal(int verbose_level, int count,
                                         char *images[],
 ...
 */
gboolean mono_regression_test_step(int verbose_level, const char *image,
...
static void print_icall_table(void) {
  // We emit some dummy values to make the code simpler

  printf("[\n{ \"klass\": \"\", \"icalls\": [");
#define NOHANDLES(inner) inner
#define HANDLES(id, name, func, ...)                                           \
  printf("\t,{ \"name\": \"%s\", \"func\": \"%s_raw\", \"handles\": true }\n", \
         name, #func);
...
#define ICALL(id, name, func)                                                  \
  printf("\t,{ \"name\": \"%s\", \"func\": \"%s\", \"handles\": false }\n",    \
         name, #func);
...
int mono_main(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; ++i) {
    ...
    if (strcmp(argv[i], "--regression") == 0) {
      ...
      mono_single_method_regression_opt =
          parse_optimizations(opt, full_opts, TRUE);
      g_free(full_opts);
    } else if (strcmp(argv[i], "--verbose") == 0 ||
               strcmp(argv[i], "-v") == 0) {
      mini_verbose_level++;
      ...
      g_print("Mono JIT compiler version %s\nCopyright (C) 2002-2014 Novell, "
              "Inc, Xamarin Inc and Contributors. www.mono-project.com\n",
              build);
      g_free(build);
      ...
      if (i + 2 >= argc) {
        fprintf(stderr, "Missing method name or bb num in --break-at-bb "
                        "command line option.");
        return 1;
      }
      mono_break_at_bb_method = mono_method_desc_new(argv[++i], TRUE);
      if (mono_break_at_bb_method == NULL) {
        fprintf(stderr, "Method name is in a bad format in --break-at-bb "
                        "command line option.");
        return 1;
      }
      ...
      if (i + 2 >= argc) {
        fprintf(stderr, "Missing method name or position in --inject-async-exc "
                        "command line option\n");
        return 1;
      }
      mono_inject_async_exc_method = mono_method_desc_new(argv[++i], TRUE);
      if (mono_inject_async_exc_method == NULL) {
        fprintf(stderr, "Method name is in a bad format in --inject-async-exc "
                        "command line option\n");
        return 1;
      }
      ...
      if (strcmp(argv[i] + 11, "core-clr") == 0) {
        ...
        fprintf(stderr, "error: --security= option has invalid argument (cas, "
                        "core-clr, verifiable or validil)\n");
        return 1;
      }
      ...
      exit(0);
#else
      fprintf(stderr, "--print-icall-table requires a runtime configured with "
                      "the --enable-icall-symbol-map option.\n");
      exit(1);
#endif
    } else if (strncmp(argv[i], "--assembly-loader=",
                       strlen("--assembly-loader=")) == 0) {
      ...
      if (strcmp(arg, "strict") == 0)
        ...
        fprintf(stderr, "Warning: unknown argument to --assembly-loader. "
                        "Should be \"strict\" or \"legacy\"\n");
    } else if (strncmp(argv[i], MONO_HANDLERS_ARGUMENT,
                       MONO_HANDLERS_ARGUMENT_LEN) == 0) {
      // Install specific custom handlers.
      if (!mono_runtime_install_custom_handlers(argv[i] +
      ...
      if (!g_file_get_contents(&argv[i][11], &response_content,
                               &response_content_len, NULL)) {
      ...
      if (response_content_len >= 3 && response_content[0] == '\xef' &&
          response_content[1] == '\xbb' && response_content[2] == '\xbf') {
  ...
  /*
   * If we are not embedded, use the mono runtime executable to run managed
   * exe's.
   */
  {
    char *runtime_path;
...
#ifdef MONO_CROSS_COMPILE
  if (!mono_compile_aot) {
    fprintf(stderr, "This mono runtime is compiled for cross-compiling. Only "
                    "the --aot option is supported.\n");
    exit(1);
  }
#if TARGET_SIZEOF_VOID_P == 4 &&                                               \
    (defined(TARGET_ARM64) || defined(TARGET_AMD64)) &&                        \
    !defined(MONO_ARCH_ILP32)
  fprintf(stderr,
  ...
  if (!mono_aot_only && !mono_use_interpreter) {
    fprintf(stderr,
            "This runtime has been configured with --enable-minimal=jit, so "
            "the --full-aot command line option is required.\n");
    exit(1);
  }
#endif
  ...
  case DO_REGRESSION:
    return mono_exec_regression_internal(mini_verbose_level, argc - i, argv + i,
                                         action == DO_SINGLE_METHOD_REGRESSION);
  ...
  if (mono_compile_aot && extra_bindings_config_file != NULL) {
    apply_root_domain_configuration_file_bindings(domain,
                                                  extra_bindings_config_file);
  }

  MonoAssemblyOpenRequest open_req;
  mono_assembly_request_prepare_open(
      &open_req, MONO_ASMCTX_DEFAULT,
      mono_domain_default_alc(mono_get_root_domain()));
  ...
  if (mono_compile_aot || action == DO_EXEC) {
    ...
    if (error) {
      ...
      fprintf(stderr, "Loaded from: %s\n",
              mono_defaults.corlib
                  ? mono_image_get_filename(mono_defaults.corlib)
                  : "unknown");
      fprintf(stderr, "Download a newer corlib or a newer runtime at "
    ...
    if (!enable_debugging && !mono_compile_aot &&
        mono_assembly_get_image_internal(assembly)
                ->image_info->cli_header.nt.pe_subsys_required ==
  ...
  method = mono_method_desc_search_in_image(
      desc, mono_assembly_get_image_internal(assembly));
  if (!method) {
  ...
  if (action == DO_DRAW) {
    ...
    if ((method->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) ||
      ...
      cfg = mini_method_compile(nm, opt, mono_get_root_domain(), (JitFlags)0,
                                part, -1);
    } else
      cfg = mini_method_compile(method, opt, mono_get_root_domain(),
                                (JitFlags)0, part, -1);
    if ((mono_graph_options & MONO_GRAPH_CFG_SSA) &&
    ...
    cfg = mini_method_compile(method, opt, mono_get_root_domain(), (JitFlags)0,
                              0, -1);
    mono_destroy_compile(cfg);
  }
#endif
...
MonoDomain *mono_jit_init(const char *file) {
  ...
  MONO_ENTER_GC_SAFE_UNBALANCED; // once it is not executing any managed code
                                 // yet, it's safe to run the gc
  return ret;
}
 ...
 * was loaded.
 */
MonoDomain *mono_jit_init_version(const char *domain_name,
...
MonoDomain *mono_jit_init_version_for_test_only(const char *domain_name,
                                                const char *runtime_version) {
  MonoDomain *ret = mini_init(domain_name, runtime_version);
  return ret;
}
...
void mono_jit_set_aot_only(gboolean val) {
  mono_aot_only = val;
...
mono_bool mono_jit_aot_compiling(void) { return mono_compile_aot; }

/**
 * mono_jit_set_trace_options:
 * \param options string representing the trace options
 * Set the options of the tracing engine. This function can be called before
 * initializing the mono runtime. See the --trace mono(1) manpage for the
 * options format.
 *
 ...
 *
 * Enable/disable signal chaining. This should be called before \c
 * mono_jit_init. If signal chaining is enabled, the runtime saves the original
 * signal handlers before installing its own handlers, and calls the original
 * ones in the following cases:
 * - a \c SIGSEGV / \c SIGABRT signal received while executing native (i.e. not
 * JITted) code.
 * - \c SIGPROF
 * - \c SIGFPE
 * - \c SIGQUIT
 ...
 *
 * Enable/disable crash chaining due to signals. When a fatal signal is
 * delivered and Mono doesn't know how to handle it, it will invoke the crash
 * handler. If chrash chaining is enabled, it will first print its crash
 * information and then try to chain with the native handler.
 */
void mono_set_crash_chaining(gboolean chain_crashes) {
 ...
 * \param ref_argv pointer to the \c argv string vector variable that might be
 * updated
 *
 ...
 *
 * \returns The value NULL is returned on success, otherwise a \c g_strdup
 * allocated string is returned (this is an alias to \c malloc under normal
 * circumstances) that contains the error message that happened during parsing.
 */
char *mono_parse_options_from(const char *options, int *ref_argc,
                              char **ref_argv[]) {
  return mono_parse_options(options, ref_argc, ref_argv, TRUE);
}

static void merge_parsed_options(GPtrArray *parsed_options, int *ref_argc,
                                 char **ref_argv[], gboolean prepend) {
  int argc = *ref_argc;
  char **argv = *ref_argv;
  ...
  if (parsed_options->len > 0) {
    ...
    i = 1;
    if (prepend) {
      /* First the environment variable settings, to allow the command line
       * options to override */
...
static char *mono_win32_parse_options(const char *options, int *ref_argc,
                                      char **ref_argv[], gboolean prepend) {
  int argc;
  gunichar2 **argv;
  gunichar2 *optionsw;
 ...
 */
void mono_parse_env_options(int *ref_argc, char **ref_argv[]) {
  char *ret;
