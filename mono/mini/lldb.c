...
JitDescriptor __mono_jit_debug_descriptor = {(MAJOR_VERSION << 16) |
                                             MINOR_VERSION};
...
static int find_code_region(void *data, int csize, int size, void *user_data) {
  UserData *ud = (UserData *)user_data;

  if ((char *)ud->code >= (char *)data &&
      (char *)ud->code < (char *)data + csize) {
...
static void add_entry(EntryType type, Buffer *buf) {
  DebugEntry *entry;
  guint8 *data;
  ...
  /* The debugger can read the list of entries asynchronously, so this has to be
   * async safe */
 ...
 * Return a region id.
 */
static int register_codegen_region(gpointer region_start, int region_size,
                                   gboolean dynamic) {
...
static void emit_unwind_info(GSList *unwind_ops, Buffer *buf) {
  int ret_reg;
  int nunwind_ops;
  GSList *l;
...
void mono_lldb_init(const char *options) {
  ...
  mono_counters_register(
      "Time spent in LLDB",
      MONO_COUNTER_JIT | MONO_COUNTER_LONG | MONO_COUNTER_TIME, &register_time);
}
...
static int compare_by_addr(const void *arg1, const void *arg2) {
...
void mono_lldb_save_method_info(MonoCompile *cfg) {
  ...
  if (cfg->method->dynamic) {
    mono_code_manager_foreach(cfg->dynamic_info->code_mp, find_code_region,
                              &udata);
    g_assert(udata.found);

    region_id =
        register_codegen_region(udata.region_start, udata.region_size, TRUE);

    lldb_lock();
    if (!dyn_codegen_regions)
      dyn_codegen_regions = g_hash_table_new(NULL, NULL);
    g_hash_table_insert(dyn_codegen_regions, cfg->method,
    ...
    region_id =
        register_codegen_region(udata.region_start, udata.region_size, FALSE);
  }
  ...
  if (minfo && seq_points) {
    ...
    // The sym seq points are ordered by il offset, need to order them by
    // address
    int skipped = 0;
    ...
    for (i = 0; i < n_il_offsets; ++i) {
      ...
      // FIXME: O(n^2)
      SeqPoint seq_point;
      if (mono_seq_point_find_by_il_offset(
              seq_points, sym_seq_points[i].il_offset, &seq_point)) {
    ...
    for (i = 0; i < n_il_offsets; ++i) {
      ...
      // printf ("%s %x %d %d\n", cfg->method->name, locs [i].native_offset,
      // sp->il_offset, sp->line);
...
void mono_lldb_remove_method(MonoDomain *domain, MonoMethod *method,
  ...
  /* The method is associated with the code region, so it doesn't have to be
   * unloaded */
}
...
void mono_lldb_save_specific_trampoline_info(gpointer arg1,
                                             MonoTrampolineType tramp_type,
                                             MonoDomain *domain, gpointer code,
                                             guint32 code_len) {
  /*
...
Communication:
Similar to the gdb jit interface. The runtime communicates with a plugin running
inside lldb.
- The runtime allocates a data packet, points a symbol with a well known name at
it.
- It calls a dummy function with a well known name.
- The plugin sets a breakpoint at this function, causing the runtime to be
suspended.
- The plugin reads the data pointed to by the other symbol and processes it.

The data packets are kept in a list, so lldb can read all of them after
attaching. Lldb will associate an object file with each mono codegen region.
