...
static void init_function_pass_manager(legacy::FunctionPassManager &fpm) {
...
struct MonoLLVMJIT {
  ...
  MonoLLVMJIT(TargetMachine *tm, Module *pgo_module)
      : mmgr(std::make_shared<MonoJitMemoryManager>()), target_machine(tm),
        ...
        data_layout(target_machine->createDataLayout()), fpm(pgo_module) {
...
public:
  ...
  MonoLLVMJIT(TargetMachine *TM, MonoJitMemoryManager *mm)
      : TM(TM), ObjectLayer([=] {
        ...
        CompileLayer(ObjectLayer, SimpleCompiler(*TM)), modules(),
        fpm(nullptr) {
    init_function_pass_manager(fpm);
  }
...
static void init_passes_and_options() {
  ...
  if (opts == NULL)
    opts = default_opts;
  else if (opts[0] == '+') // Append passes to the default order if starts with
                           // '+', overwrite otherwise
...
gpointer mono_llvm_compile_method(MonoEERef mono_ee, MonoCompile *cfg,
  ...
  auto ret = jit->compile(unwrap<Function>(method), nvars, callee_vars,
