...
static unsigned char *alloc_code(LLVMValueRef function, int size) {
  MonoCompile *cfg;
...
uint8_t *MonoJitMemoryManager::allocateDataSection(uintptr_t Size,
                                                   unsigned Alignment,
                                                   unsigned SectionID,
...
uint8_t *MonoJitMemoryManager::allocateCodeSection(uintptr_t Size,
                                                   unsigned Alignment,
                                                   unsigned SectionID,
                                                   StringRef SectionName) {
...
bool MonoJitMemoryManager::finalizeMemory(std::string *ErrMsg) {
...
struct MonoLLVMJIT {
  ...
  MonoLLVMJIT(TargetMachine *tm)
      : mmgr(std::make_shared<MonoJitMemoryManager>()), target_machine(tm),
        object_layer(AcknowledgeORCv1Deprecation, execution_session,
                     [this](VModuleKey k) {
                       return LegacyRTDyldObjectLinkingLayer::Resources{
                           mmgr, resolvers[k]};
                     }),
        compile_layer(AcknowledgeORCv1Deprecation, object_layer,
                      SimpleCompiler{*target_machine}),
        data_layout(target_machine->createDataLayout()) {
    compile_layer.setNotifyCompiled(
        [](VModuleKey, std::unique_ptr<Module> module) { module.release(); });
  }

  VModuleKey add_module(std::unique_ptr<Module> m) {
    ...
    auto resolver =
        createLegacyLookupResolver(execution_session, lookup_name, on_error);
  ...
  gpointer compile(Function *func, int nvars, LLVMValueRef *callee_vars,
...
static MonoLLVMJIT *make_mono_llvm_jit(TargetMachine *target_machine) {
...
public:
  ...
  MonoLLVMJIT(TargetMachine *TM, MonoJitMemoryManager *mm)
      : TM(TM), ObjectLayer([=] {
          return std::shared_ptr<RuntimeDyld::MemoryManager>(mm);
        }),
        CompileLayer(ObjectLayer, SimpleCompiler(*TM)), modules(), fpm(NULL) {
    initPassManager();
  }
  ...
  void initPassManager() {
    ...
    const char *default_opts =
        " -simplifycfg -sroa -lower-expect -instcombine -jump-threading "
        "-loop-rotate -licm -simplifycfg -lcssa -loop-idiom -indvars "
        "-loop-deletion -gvn -memcpyopt -sccp -bdce -instcombine -dse "
        "-simplifycfg -enable-implicit-null-checks" NO_CALL_FRAME_OPT;
    const char *opts = g_getenv("MONO_LLVM_OPT");
    if (opts == NULL)
      opts = default_opts;
    else if (opts[0] == '+') // Append passes to the default order if starts
                             // with '+', overwrite otherwise
      opts = g_strdup_printf("%s %s", default_opts, opts + 1);
    else if (opts[0] !=
             ' ') // pass order has to start with a leading whitespace
    ...
    for (size_t i = 0; i < PassList.size(); i++) {
      ...
      if (pass->getPassKind() == llvm::PT_Function ||
          pass->getPassKind() == llvm::PT_Loop) {
        fpm.add(pass);
      } else {
  ...
  gpointer compile(Function *F, int nvars, LLVMValueRef *callee_vars,
                   gpointer *callee_addrs, gpointer *eh_frame) {
    F->getParent()->setDataLayout(TM->createDataLayout());
    fpm.run(*F);
    // TODO: run module wide optimizations, e.g. remove dead globals/functions
    // Orc uses a shared_ptr to refer to modules so we have to save them
    // ourselves to keep a ref
    ...
    for (int i = 0; i < nvars; ++i) {
      ...
      auto sym =
          CompileLayer.findSymbolIn(ModuleHandle, mangle(var->getName()), true);
      auto addr = sym.getAddress();
      g_assert((bool)addr);
      callee_addrs[i] = (gpointer)addr.get();
    }
    ...
    auto ehsym =
        CompileLayer.findSymbolIn(ModuleHandle, "mono_eh_frame", false);
    auto ehaddr = ehsym.getAddress();
    g_assert((bool)ehaddr);
    *eh_frame = (gpointer)ehaddr.get();
    return (gpointer)BodyAddr.get();
...
void mono_llvm_jit_set_tls_cfg(MonoCompile *cfg) {
  mono_native_tls_set_value(current_cfg_tls_id, cfg);
}

MonoEERef mono_llvm_create_ee(LLVMExecutionEngineRef *ee) { return NULL; }

/*
 * mono_llvm_compile_method:
 *
 *   Compile METHOD to native code. Compute the addresses of the variables in
 * CALLEE_VARS and store them into CALLEE_ADDRS. Return the EH frame address in
 * EH_FRAME.
 */
gpointer mono_llvm_compile_method(MonoEERef mono_ee, LLVMValueRef method,
                                  int nvars, LLVMValueRef *callee_vars,
                                  gpointer *callee_addrs, gpointer *eh_frame) {
  return jit->compile(unwrap<Function>(method), nvars, callee_vars,
                      callee_addrs, eh_frame);
}

void mono_llvm_dispose_ee(MonoEERef *eeref) {}
...
void mono_llvm_dispose_ee(MonoEERef *eeref) { g_assert_not_reached(); }
