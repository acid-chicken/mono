 ...
 * - the ABI
 *
 * V9 instructions are only usable if the underlying processor is 64 bit. Most
 * Sparc processors in use are 64 bit processors. The V9 ABI is only usable if
 * the mono executable is a 64 bit executable. So it would make sense to use the
 * 64 bit instructions without using the 64 bit ABI.
 */
 ...
 * - %o0..%o5 and %l7 is used for local register allocation and passing
 * arguments
 * - %l0..%l6 is used for global register allocation
 * - %o7 and %g1 is used as scratch registers in opcodes
 * - all floating point registers are used for local register allocation except
 * %f0. Only double precision registers are used. In 64 bit mode:
...
static void flushi(void *addr) { asm("flush %i0"); }
#endif
...
void mono_arch_flush_register_windows(void) { mono_sparc_flushw(); }
...
gboolean mono_sparc_is_sparc64(void) { return v64; }
...
static void inline add_general(guint32 *gr, guint32 *stack_size, ArgInfo *ainfo,
                               gboolean pair) {
 ...
 */
static CallInfo *get_call_info(MonoCompile *cfg, MonoMethodSignature *sig,
                               gboolean is_pinvoke) {
  guint32 i, gr, fr;
...
GList *mono_arch_get_allocatable_int_vars(MonoCompile *cfg) {
  ...
  for (i = 0; i < cfg->num_varinfo; i++) {
    ...
    if (ins->flags & (MONO_INST_VOLATILE | MONO_INST_INDIRECT) ||
        (ins->opcode == OP_REGVAR) || (ins->opcode == OP_ARG))
      continue;
...
void mono_arch_allocate_vars(MonoCompile *cfg) {
  ...
  for (i = curinst; i < cfg->num_varinfo; ++i) {
    ...
    if (inst->backend.is_pinvoke && MONO_TYPE_ISSTRUCT(inst->inst_vtype) &&
        inst->inst_vtype->type != MONO_TYPE_TYPEDBYREF)
      size = mono_class_native_size(
          mono_class_from_mono_type_internal(inst->inst_vtype), &align);
    else
...
void mono_arch_create_vars(MonoCompile *cfg) {
  ...
  if (!sig->ret->byref &&
      (sig->ret->type == MONO_TYPE_I8 || sig->ret->type == MONO_TYPE_U8)) {
    MonoInst *low = get_vreg_to_inst(cfg, MONO_LVREG_LS(cfg->ret->dreg));
    MonoInst *high = get_vreg_to_inst(cfg, MONO_LVREG_MS(cfg->ret->dreg));
  ...
  cfg->arch.float_spill_slot = mono_compile_create_var(
      cfg, m_class_get_byval_arg(mono_defaults.double_class), OP_ARG);
  ((MonoInst *)cfg->arch.float_spill_slot)->flags |= MONO_INST_VOLATILE;
}

static void add_outarg_reg(MonoCompile *cfg, MonoCallInst *call,
                           ArgStorage storage, int reg, guint32 sreg) {
  MonoInst *arg;
...
static void add_outarg_load(MonoCompile *cfg, MonoCallInst *call, int opcode,
                            int basereg, int offset, int reg) {
...
static void emit_pass_long(MonoCompile *cfg, MonoCallInst *call, ArgInfo *ainfo,
                           MonoInst *in) {
  int offset = ARGS_OFFSET + ainfo->offset;

  switch (ainfo->storage) {
  case ArgInIRegPair:
    add_outarg_reg(cfg, call, ArgInIReg, sparc_o0 + ainfo->reg + 1,
                   MONO_LVREG_LS(in->dreg));
    add_outarg_reg(cfg, call, ArgInIReg, sparc_o0 + ainfo->reg,
                   MONO_LVREG_MS(in->dreg));
    break;
  case ArgOnStackPair:
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG, sparc_sp, offset,
                                MONO_LVREG_MS(in->dreg));
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG, sparc_sp,
                                offset + 4, MONO_LVREG_LS(in->dreg));
    break;
  case ArgInSplitRegStack:
    add_outarg_reg(cfg, call, ArgInIReg, sparc_o0 + ainfo->reg,
                   MONO_LVREG_MS(in->dreg));
...
static void emit_pass_double(MonoCompile *cfg, MonoCallInst *call,
                             ArgInfo *ainfo, MonoInst *in) {
  int offset = ARGS_OFFSET + ainfo->offset;

  switch (ainfo->storage) {
  case ArgInIRegPair:
    /* floating-point <-> integer transfer must go through memory */
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, sparc_sp, offset,
                                in->dreg);

    /* Load into a register pair */
    add_outarg_load(cfg, call, OP_LOADI4_MEMBASE, sparc_sp, offset,
                    sparc_o0 + ainfo->reg);
    add_outarg_load(cfg, call, OP_LOADI4_MEMBASE, sparc_sp, offset + 4,
                    sparc_o0 + ainfo->reg + 1);
    break;
  case ArgOnStackPair:
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, sparc_sp, offset,
                                in->dreg);
...
static void emit_pass_float(MonoCompile *cfg, MonoCallInst *call,
                            ArgInfo *ainfo, MonoInst *in) {
  int offset = ARGS_OFFSET + ainfo->offset;
  ...
  case ArgInIReg:
    /* floating-point <-> integer transfer must go through memory */
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER4_MEMBASE_REG, sparc_sp, offset,
                                in->dreg);
    add_outarg_load(cfg, call, OP_LOADI4_MEMBASE, sparc_sp, offset,
                    sparc_o0 + ainfo->reg);
    break;
...
static void emit_pass_other(MonoCompile *cfg, MonoCallInst *call,
                            ArgInfo *ainfo, MonoType *arg_type, MonoInst *in);

static void emit_pass_vtype(MonoCompile *cfg, MonoCallInst *call,
                            CallInfo *cinfo, ArgInfo *ainfo, MonoType *arg_type,
                            MonoInst *in, gboolean pinvoke) {
  MonoInst *arg;
  ...
  if (arg_type->type == MONO_TYPE_TYPEDBYREF) {
    ...
    size =
        mono_type_native_stack_size(m_class_get_byval_arg(in->klass), &align);
  else {
    /*
...
static void emit_sig_cookie(MonoCompile *cfg, MonoCallInst *call,
  ...
  MONO_EMIT_NEW_STORE_MEMBASE_IMM(cfg, OP_STORE_MEMBASE_IMM, sparc_sp,
                                  ARGS_OFFSET + cinfo->sig_cookie.offset,
                                  tmp_sig);
}
...
void mono_arch_emit_call(MonoCompile *cfg, MonoCallInst *call) {
  ...
  if (sig->ret && MONO_TYPE_ISSTRUCT(sig->ret)) {
    ...
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG, sparc_sp, 64,
                                call->vret_var->dreg);
  }
  ...
  for (i = 0; i < n; ++i) {
    ...
    else if (!arg_type->byref && ((arg_type->type == MONO_TYPE_I8) ||
                                  (arg_type->type == MONO_TYPE_U8)))
...
void mono_arch_emit_outarg_vt(MonoCompile *cfg, MonoInst *ins, MonoInst *src) {
  ...
  mini_emit_memcpy(cfg, sparc_sp, ainfo->offset, src->dreg, 0, size,
...
void mono_arch_emit_setret(MonoCompile *cfg, MonoMethod *method,
                           MonoInst *val) {
  CallInfo *cinfo =
      get_call_info(cfg, mono_method_signature_internal(method), FALSE);
  MonoType *ret =
  ...
  case ArgInIRegPair:
    if (ret->type == MONO_TYPE_I8 || ret->type == MONO_TYPE_U8) {
      ...
      MONO_EMIT_NEW_UNALU(cfg, OP_MOVE, MONO_LVREG_MS(cfg->ret->dreg),
                          MONO_LVREG_MS(val->dreg));
      MONO_EMIT_NEW_UNALU(cfg, OP_MOVE, MONO_LVREG_LS(cfg->ret->dreg),
                          MONO_LVREG_LS(val->dreg));
    }
    break;
  case ArgInFReg:
...
int cond_to_sparc_cond[][3] = {
    {sparc_be, sparc_be, sparc_fbe},    {sparc_bne, sparc_bne, 0},
    {sparc_ble, sparc_ble, sparc_fble}, {sparc_bge, sparc_bge, sparc_fbge},
    {sparc_bl, sparc_bl, sparc_fbl},    {sparc_bg, sparc_bg, sparc_fbg},
    {sparc_bleu, sparc_bleu, 0},        {sparc_beu, sparc_beu, 0},
    {sparc_blu, sparc_blu, sparc_fbl},  {sparc_bgu, sparc_bgu, sparc_fbg}};

/* Map opcode to the sparc condition codes */
static SparcCond opcode_to_sparc_cond(int opcode) {
  CompRelation rel;
  CompType t;
...
#define COMPUTE_DISP(ins)                                                      \
  if (ins->inst_true_bb->native_offset)                                        \
    disp = (ins->inst_true_bb->native_offset -                                 \
            ((guint8 *)code - cfg->native_code)) >>                            \
           2;                                                                  \
  else {                                                                       \
    disp = 0;                                                                  \
...
#define EMIT_COND_BRANCH(ins, cond, annul, filldelay)                          \
  EMIT_COND_BRANCH_ICC((ins), (cond), (annul), (filldelay), (sparc_xcc_short))
 ...
 */
#define EMIT_COND_SYSTEM_EXCEPTION_GENERAL(ins, cond, sexc_name, filldelay,    \
                                           icc)                                \
  do {                                                                         \
    mono_add_patch_info(cfg, (guint8 *)(code) - (cfg)->native_code,            \
...
#define EMIT_COND_SYSTEM_EXCEPTION(ins, cond, sexc_name)                       \
  EMIT_COND_SYSTEM_EXCEPTION_GENERAL(ins, cond, sexc_name, TRUE, DEFAULT_ICC)

#define EMIT_COND_SYSTEM_EXCEPTION_BPR(ins, bop, sexc_name)                    \
  do {                                                                         \
    mono_add_patch_info(cfg, (guint8 *)(code) - (cfg)->native_code,            \
                        MONO_PATCH_INFO_EXC, sexc_name);                       \
...
#define EMIT_ALU_IMM(ins, op, setcc)                                           \
  do {                                                                         \
    if (sparc_is_imm13((ins)->inst_imm))                                       \
      sparc_##op##_imm(code, (setcc), (ins)->sreg1, ins->inst_imm,             \
                       (ins)->dreg);                                           \
    else {                                                                     \
...
#define EMIT_LOAD_MEMBASE(ins, op)                                             \
  do {                                                                         \
...
/* max len = 5 */
#define EMIT_STORE_MEMBASE_IMM(ins, op)                                        \
  do {                                                                         \
    guint32 sreg;                                                              \
    if (ins->inst_imm == 0)                                                    \
      sreg = sparc_g0;                                                         \
...
#define EMIT_STORE_MEMBASE_REG(ins, op)                                        \
  do {                                                                         \
    if (!sparc_is_imm13(ins->inst_offset)) {                                   \
      sparc_set(code, ins->inst_offset, sparc_o7);                             \
      sparc_##op(code, ins->sreg1, ins->inst_destbasereg, sparc_o7);           \
    } else                                                                     \
      sparc_##op##_imm(code, ins->sreg1, ins->inst_destbasereg,                \
                       ins->inst_offset);                                      \
  } while (0);
 ...
 */
static guint32 *emit_call(MonoCompile *cfg, guint32 *code, guint32 patch_type,
                          gconstpointer data) {
  ERROR_DECL(error);
  gpointer target;
  ...
  if (0 && v64 && !cfg->compile_aot) {
    ...
    target = mono_resolve_patch_target(cfg->method, cfg->domain, NULL,
                                       &patch_info, FALSE, error);
    ...
    mono_add_patch_info(cfg, (guint8 *)code - cfg->native_code, patch_type,
                        data);
    EMIT_CALL();
  }
  ...
  MONO_BB_FOR_EACH_INS_SAFE(bb, n, ins) {
    ...
    case OP_STOREI4_MEMBASE_IMM:
      ...
      if (last_ins && (last_ins->opcode == OP_STOREI4_MEMBASE_IMM) &&
          ...
          (ins->inst_destbasereg == sparc_fp) && (ins->inst_offset < 0) &&
          ((ins->inst_offset % 8) == 0) &&
          ((ins->inst_offset == last_ins->inst_offset - 4)) &&
          (ins->inst_imm == 0) && (last_ins->inst_imm == 0)) {
        if (mono_hwcap_sparc_is_v9) {
    ...
    case OP_COND_EXC_NE_UN:
      ...
      if (v64 && (cfg->header->code_size < 10000) && last_ins &&
          (last_ins->opcode == OP_COMPARE_IMM) && (last_ins->inst_imm == 0)) {
        switch (ins->opcode) {
        case OP_IBEQ:
...
void mono_arch_decompose_long_opts(MonoCompile *cfg, MonoInst *ins) {
  switch (ins->opcode) {
  case OP_LNEG:
    MONO_EMIT_NEW_BIALU(cfg, OP_SUBCC, MONO_LVREG_LS(ins->dreg), 0,
                        MONO_LVREG_LS(ins->sreg1));
    MONO_EMIT_NEW_BIALU(cfg, OP_SBB, MONO_LVREG_MS(ins->dreg), 0,
...
static void sparc_patch(guint32 *code, const gpointer target) {
  guint32 *c = code;
  guint32 ins = *code;
  guint32 op = ins >> 30;
  ...
  if ((op == 0) && (op2 == 2)) {
    ...
    *code = ((ins >> 22) << 22) | (disp & 0x3fffff);
    ...
    if (((ins2 >> 30) == 2) && (((ins2 >> 19) & 0x3f) == 2)) {
      ...
      *(code + 1) |= (t & 0x3ff);
#endif
    } else if (v64 && (sparc_inst_rd(ins) == sparc_g1) &&
...
guint32 *mono_sparc_emit_restore_lmf(guint32 *code, guint32 lmf_offset) {
  /* Load previous_lmf */
  sparc_ldi_imm(code, sparc_fp,
                lmf_offset + G_STRUCT_OFFSET(MonoLMF, previous_lmf), sparc_l0);
  /* Load lmf_addr */
  sparc_ldi_imm(code, sparc_fp, lmf_offset + G_STRUCT_OFFSET(MonoLMF, lmf_addr),
                sparc_l1);
  /* *(lmf) = previous_lmf */
...
static guint32 *emit_save_sp_to_lmf(MonoCompile *cfg, guint32 *code) {
  /*
  ...
  if (cfg->method->save_lmf) {
    ...
    /* Save sp */
    sparc_sti_imm(code, sparc_sp, sparc_fp,
                  lmf_offset + G_STRUCT_OFFSET(MonoLMF, sp));
  }

  return code;
}

static guint32 *emit_vret_token(MonoInst *ins, guint32 *code) {
...
static guint32 *emit_move_return_value(MonoInst *ins, guint32 *code) {
 ...
 */
static guint32 *emit_load_volatile_arguments(MonoCompile *cfg, guint32 *code) {
  ...
  for (i = 0; i < sig->param_count + sig->hasthis; ++i) {
    ...
    if (!v64 && !arg_type->byref && (arg_type->type == MONO_TYPE_R8)) {
      if (ainfo->storage == ArgInIRegPair) {
        ...
        g_assert_not_reached();
    } else if ((ainfo->storage == ArgInIReg) && (inst->opcode != OP_REGVAR)) {
      ...
      else if ((stack_offset - ARGS_OFFSET) & 0x2)
        sparc_ldsh_imm(code, inst->inst_basereg, stack_offset, ireg);
      else if ((stack_offset - ARGS_OFFSET) & 0x4)
    ...
    } else if ((ainfo->storage == ArgInIRegPair) &&
               (inst->opcode != OP_REGVAR)) {
    ...
    } else if ((ainfo->storage == ArgInFloatReg) &&
               (inst->opcode != OP_REGVAR)) {
      NOT_IMPLEMENTED;
...
gpointer mono_arch_build_imt_trampoline(MonoVTable *vtable, MonoDomain *domain,
    ...
    if (item->is_equals) {
      ...
      if (item->check_target_idx || fail_case) {
        ...
        if (item->has_target_code) {
          ...
          sparc_set(code,
                    ((guint32)(&(vtable->vtable[item->value.vtable_slot]))),
                    sparc_g5);
        ...
        g_assert_not_reached();
#endif
        sparc_set(code, ((guint32)(&(vtable->vtable[item->value.vtable_slot]))),
                  sparc_g5);
        sparc_ld(code, sparc_g5, 0, sparc_g5);
        sparc_jmpl(code, sparc_g5, sparc_g0, sparc_g0);
        sparc_nop(code);
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (item->jmp_code) {
      if (item->check_target_idx) {
        sparc_patch((guint32 *)item->jmp_code,
  ...
  MONO_BB_FOR_EACH_INS(bb, ins) {
    ...
    case OP_IDIV:
      ...
      EMIT_COND_SYSTEM_EXCEPTION_GENERAL(
          code, sparc_boverflow, "ArithmeticException", TRUE, sparc_icc_short);
      break;
    case OP_IDIV_UN:
    ...
    case OP_IDIV_IMM: {
      ...
      if (i < 30) {
        ...
        EMIT_COND_SYSTEM_EXCEPTION_GENERAL(code, sparc_boverflow,
    ...
    case OP_IMUL_OVF:
      ...
      EMIT_COND_SYSTEM_EXCEPTION_GENERAL(ins, sparc_bne, "OverflowException",
                                         TRUE, sparc_icc_short);
      break;
    case OP_IMUL_OVF_UN:
    ...
    case OP_I8CONST:
      ...
      break;
    case OP_AOTCONST:
      mono_add_patch_info(cfg, offset, (MonoJumpInfoType)ins->inst_i1,
                          ins->inst_p0);
    ...
    case OP_CALL_MEMBASE:
      ...
      break;
    case OP_SETFRET:
      if (mono_method_signature_internal(cfg->method)->ret->type ==
          MONO_TYPE_R4)
    ...
    case OP_LOCALLOC: {
      ...
      sparc_add_imm(code, FALSE, ins->sreg1,
                    4 + MONO_ARCH_LOCALLOC_ALIGNMENT - 1, ins->dreg);
      ...
      sparc_add_imm(code, FALSE, ins->dreg, MONO_SPARC_STACK_BIAS + offset2,
                    ins->dreg);
      ...
      if (ins->flags & MONO_INST_INIT) {
        ...
        sparc_sub_imm(code, 0, size_reg, mono_hwcap_sparc_is_v9 ? 8 : 4,
                      size_reg);
        /* start of loop */
        br[1] = code;
        if (mono_hwcap_sparc_is_v9)
        ...
        /* delay slot */
        sparc_add_imm(code, 0, sparc_o7, mono_hwcap_sparc_is_v9 ? 8 : 4,
                      sparc_o7);
    ...
    case OP_LOCALLOC_IMM: {
      ...
      offset2 = ALIGN_TO(cfg->arch.localloc_offset, 8);
      g_assert(sparc_is_imm13(MONO_SPARC_STACK_BIAS + offset2));
      sparc_add_imm(code, FALSE, sparc_sp, MONO_SPARC_STACK_BIAS + offset2,
                    ins->dreg);
      if ((ins->flags & MONO_INST_INIT) && (offset > 0)) {
        ...
        if (offset <= 16) {
          ...
          sparc_set(code, offset, sparc_o7);
          sparc_sub_imm(code, 0, sparc_o7, mono_hwcap_sparc_is_v9 ? 8 : 4,
                        sparc_o7);
          /* beginning of loop */
          br[0] = code;
          if (mono_hwcap_sparc_is_v9)
    ...
    case OP_THROW:
      ...
      mono_add_patch_info(
          cfg, (guint8 *)code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_ID,
    ...
    case OP_BR:
      ...
      if (ins->inst_target_bb->native_offset) {
        gint32 disp = (ins->inst_target_bb->native_offset -
                       ((guint8 *)code - cfg->native_code)) >>
                      2;
        g_assert(sparc_is_imm22(disp));
    ...
    case OP_CGT_UN:
      if (v64 && (cfg->opt & MONO_OPT_CMOV)) {
        ...
        sparc_movcc_imm(code, sparc_xcc, opcode_to_sparc_cond(ins->opcode), 1,
                        ins->dreg);
      } else {
        sparc_clr_reg(code, ins->dreg);
#ifdef SPARCV9
        sparc_branchp(code, 1, opcode_to_sparc_cond(ins->opcode), DEFAULT_ICC,
                      0, 2);
#else
    ...
    case OP_ICGT_UN:
      if (v64 && (cfg->opt & MONO_OPT_CMOV)) {
        sparc_clr_reg(code, ins->dreg);
        sparc_movcc_imm(code, sparc_icc, opcode_to_sparc_cond(ins->opcode), 1,
                        ins->dreg);
      } else {
        sparc_clr_reg(code, ins->dreg);
        sparc_branchp(code, 1, opcode_to_sparc_cond(ins->opcode),
                      sparc_icc_short, 0, 2);
        /* delay slot */
        sparc_set(code, 1, ins->dreg);
      }
    ...
    case OP_COND_EXC_INC:
      ...
      NOT_IMPLEMENTED;
#else
      EMIT_COND_SYSTEM_EXCEPTION(ins, opcode_to_sparc_cond(ins->opcode),
                                 ins->inst_p1);
#endif
      break;
    ...
    case OP_IBLE_UN: {
      if (mono_hwcap_sparc_is_v9)
        EMIT_COND_BRANCH_PREDICTED(ins, opcode_to_sparc_cond(ins->opcode), 1,
                                   1);
      else
    ...
    case OP_STORER8_MEMBASE_REG:
      if (!sparc_is_imm13(ins->inst_offset + 4)) {
        ...
        if (!v64 && (ins->inst_offset % 8)) {
          /* Misaligned */
          sparc_stf_imm(code, ins->sreg1, ins->inst_destbasereg,
                        ins->inst_offset);
          sparc_stf_imm(code, ins->sreg1 + 1, ins->inst_destbasereg,
                        ins->inst_offset + 4);
        } else
          sparc_stdf_imm(code, ins->sreg1, ins->inst_destbasereg,
                         ins->inst_offset);
      }
      break;
    ...
    case OP_STORER4_MEMBASE_REG:
      ...
      if (!sparc_is_imm13(ins->inst_offset)) {
        ...
        sparc_stf_imm(code, FP_SCRATCH_REG, ins->inst_destbasereg,
                      ins->inst_offset);
      break;
...
void mono_arch_patch_code(MonoCompile *cfg, MonoMethod *method,
  ...
  for (patch_info = ji; patch_info; patch_info = patch_info->next) {
    ...
    target = mono_resolve_patch_target(method, domain, code, patch_info,
                                       run_cctors, error);
    return_if_nok(error);
...
void *mono_arch_instrument_prolog(MonoCompile *cfg, MonoJitICallId func,
  ...
  for (i = 0; i < 6; ++i)
    sparc_sti_imm(code, sparc_i0 + i, sparc_fp,
                  ARGS_OFFSET + (i * sizeof(target_mgreg_t)));
  ...
  mono_add_patch_info(cfg, (guint8 *)code - cfg->native_code,
                      MONO_PATCH_INFO_JIT_ICALL_ID, GUINT_TO_POINTER(func));
  EMIT_CALL();
...
enum { SAVE_NONE, SAVE_STRUCT, SAVE_ONE, SAVE_TWO, SAVE_FP };

#error obsolete tracing?
void *mono_arch_instrument_epilog(MonoCompile *cfg, MonoJitICallId func,
                                  void *p, gboolean enable_arguments) {
  guint32 *code = (guint32 *)p;
  int save_mode = SAVE_NONE;
  MonoMethod *method = cfg->method;

  switch (mini_get_underlying_type(mono_method_signature_internal(method)->ret)
              ->type) {
  case MONO_TYPE_VOID:
...
guint8 *mono_arch_emit_prolog(MonoCompile *cfg) {
  ...
  /*
     ...
     (MONO_JIT_ICALL_mono_sparc_break)); sparc_call_simple (code, 0); sparc_nop
     (code);
      }
  */
  ...
  for (i = 0; i < sig->param_count + sig->hasthis; ++i) {
    ...
    if (!v64 && !arg_type->byref && (arg_type->type == MONO_TYPE_R8)) {
      ...
      if (ainfo->storage == ArgInIRegPair) {
        if (!sparc_is_imm13(inst->inst_offset + 4))
          NOT_IMPLEMENTED;
        sparc_st_imm(code, sparc_i0 + ainfo->reg, inst->inst_basereg,
                     inst->inst_offset);
        sparc_st_imm(code, sparc_i0 + ainfo->reg + 1, inst->inst_basereg,
                     inst->inst_offset + 4);
      } else if (ainfo->storage == ArgInSplitRegStack) {
#ifdef SPARCV9
        g_assert_not_reached();
        ...
        if (stack_offset != inst->inst_offset) {
          ...
          sparc_st_imm(code, sparc_o7, inst->inst_basereg,
                       inst->inst_offset + 4);
        }
      } else if (ainfo->storage == ArgOnStackPair) {
#ifdef SPARCV9
      ...
      if (!sparc_is_imm13(stack_offset))
        NOT_IMPLEMENTED;
      if ((stack_offset - ARGS_OFFSET) & 0x1)
        sparc_stb_imm(code, sparc_i0 + ainfo->reg, inst->inst_basereg,
                      stack_offset);
      else if ((stack_offset - ARGS_OFFSET) & 0x2)
        sparc_sth_imm(code, sparc_i0 + ainfo->reg, inst->inst_basereg,
                      stack_offset);
      else if ((stack_offset - ARGS_OFFSET) & 0x4)
        sparc_st_imm(code, sparc_i0 + ainfo->reg, inst->inst_basereg,
                     stack_offset);
        ...
        if (v64)
          sparc_stx_imm(code, sparc_i0 + ainfo->reg, inst->inst_basereg,
                        stack_offset);
        else
          sparc_st_imm(code, sparc_i0 + ainfo->reg, inst->inst_basereg,
                       stack_offset);
  ...
  if (cfg->method->save_lmf) {
    ...
    /* Save ip */
    mono_add_patch_info(cfg, (guint8 *)code - cfg->native_code,
                        MONO_PATCH_INFO_IP, NULL);
    sparc_set_template(code, sparc_o7);
    sparc_sti_imm(code, sparc_o7, sparc_fp,
                  lmf_offset + G_STRUCT_OFFSET(MonoLMF, ip));
    /* Save sp */
    sparc_sti_imm(code, sparc_sp, sparc_fp,
                  lmf_offset + G_STRUCT_OFFSET(MonoLMF, sp));
    /* Save fp */
    sparc_sti_imm(code, sparc_fp, sparc_fp,
...
#error obsolete tracing?
  if (mono_jit_trace_calls != NULL && mono_trace_eval(method))
    code = (guint32 *)mono_arch_instrument_prolog(
        cfg, MONO_JIT_ICALL_mono_trace_enter_method, code, TRUE);
...
#error obsolete tracing?
  if (mono_jit_trace_calls != NULL && mono_trace_eval(method))
    code = (guint32 *)mono_arch_instrument_epilog(cfg, mono_trace_leave_method,
                                                  code, TRUE);
   ...
   * return to %i7+12
   */
  if (!v64 && mono_method_signature_internal(cfg->method)->pinvoke &&
      MONO_TYPE_ISSTRUCT(mono_method_signature_internal(cfg->method)->ret))
    sparc_jmpl_imm(code, sparc_i7, 12, sparc_g0);
  else
    sparc_ret(code);

  /* Only fold last instruction into the restore if the exit block has an in
     count of 1 and the previous block hasn't been optimized away since it may
     have an in count > 1 */
  if (cfg->bb_exit->in_count == 1 &&
      cfg->bb_exit->in_bb[0]->native_offset != cfg->bb_exit->native_offset)
    can_fold = 1;
  ...
  if (can_fold && (sparc_inst_op(code[-2]) == 0x2) &&
      (sparc_inst_op3(code[-2]) == 0x2) && sparc_inst_imm(code[-2]) &&
      (sparc_inst_rd(code[-2]) == sparc_i0)) {
    /* or reg, imm, %i0 */
  ...
  } else if (can_fold && (sparc_inst_op(code[-2]) == 0x2) &&
             (sparc_inst_op3(code[-2]) == 0x2) && (!sparc_inst_imm(code[-2])) &&
             (sparc_inst_rd(code[-2]) == sparc_i0)) {
    /* or reg, reg, %i0 */
...
void mono_arch_emit_exceptions(MonoCompile *cfg) {
  ...
  for (patch_info = cfg->patch_info; patch_info;
    ...
    case MONO_PATCH_INFO_EXC: {
      ...
      if (i < nthrows) {
        guint32 throw_offset =
            (((guint8 *)exc_throw_end[i] - cfg->native_code) - throw_ip) >> 2;
        ...
        buf = code;
        if (sparc_is_imm13(
                ((((guint8 *)code - cfg->native_code) - throw_ip) >> 2) - 8))
        ...
        mono_add_patch_info (cfg, (guint8*)code - cfg->native_code,
        MONO_PATCH_INFO_JIT_ICALL_ID, GUINT_TO_POINTER
        (MONO_JIT_ICALL_mono_sparc_break)); EMIT_CALL();
        */
...
int mono_arch_get_argument_info(MonoMethodSignature *csig, int param_count,
  ...
  for (k = 0; k < param_count; k++) {
    ...
    arg_info[k + 1].offset =
        ARGS_OFFSET - MONO_SPARC_STACK_BIAS + ainfo->offset;
...
gboolean mono_arch_opcode_supported(int opcode) { return FALSE; }
...
gpointer mono_arch_load_function(MonoJitICallId jit_icall_id) { return NULL; }
