...
#define CALLCONV_IS_STDCALL(sig)                                               \
  ((sig)->pinvoke && ((sig)->call_convention == MONO_CALL_STDCALL ||           \
...
#define X86_IS_CALLEE_SAVED_REG(reg)                                           \
  (((reg) == X86_EBX) || ((reg) == X86_EDI) || ((reg) == X86_ESI))
...
static void inline add_general(guint32 *gr, const guint32 *param_regs,
                               guint32 *stack_size, ArgInfo *ainfo) {
...
static void inline add_general_pair(guint32 *gr, const guint32 *param_regs,
                                    guint32 *stack_size, ArgInfo *ainfo) {
  ainfo->offset = *stack_size;
...
static void add_valuetype(MonoMethodSignature *sig, ArgInfo *ainfo,
                          ...
                          const guint32 *param_regs, guint32 *fr,
                          guint32 *stack_size) {
  guint32 size;
  MonoClass *klass;
  ...
  if (sig->pinvoke && is_return) {
    ...
    /*
     * Windows x86 ABI for returning structs of size 4 or 8 bytes (regardless of
     * type) dictates that values are passed in EDX:EAX register pairs,
     * https://msdn.microsoft.com/en-us/library/984x0h58.aspx. This is different
     * compared to for example float or double return types (not in struct) that
     * will be returned in ST(0),
     * https://msdn.microsoft.com/en-us/library/ha59cbfz.aspx.
     *
     * Apples OSX x86 ABI for returning structs of size 4 or 8 bytes uses a
     * slightly different approach. If a struct includes only one scalar value,
     * it will be handled with the same rules as scalar values. This means that
     * structs with one float or double will be returned in ST(0). For more
     * details,
    ...
    if ((info->native_size == 1) || (info->native_size == 2) ||
        (info->native_size == 4) || (info->native_size == 8)) {
      ainfo->storage = ArgValuetypeInReg;
  ...
  *stack_size += ALIGN_TO(size, sizeof(target_mgreg_t));
  ainfo->nslots =
      ALIGN_TO(size, sizeof(target_mgreg_t)) / sizeof(target_mgreg_t);
}
 ...
 */
static CallInfo *get_call_info_internal(CallInfo *cinfo,
  ...
  /* return value */
  {
    ...
    case MONO_TYPE_TYPEDBYREF: {
      ...
      add_valuetype(sig, &cinfo->ret, ret_type, TRUE, &tmp_gr, NULL, &tmp_fr,
                    &tmp_stacksize);
   ...
   */
  if (cinfo->vtype_retaddr && !is_pinvoke &&
      (sig->hasthis ||
       (sig->param_count > 0 &&
  ...
  for (i = pstart; i < sig->param_count; ++i) {
    ...
    case MONO_TYPE_VALUETYPE:
    case MONO_TYPE_TYPEDBYREF:
      add_valuetype(sig, ainfo, ptype, FALSE, &gr, param_regs, &fr,
                    &stack_size);
      break;
    case MONO_TYPE_U8:
    case MONO_TYPE_I8:
  ...
  if (cinfo->vtype_retaddr) {
    /* if the function returns a struct on stack, the called method already does
     * a ret $0x4 */
  ...
  if (mono_do_x86_stack_align &&
      (stack_size % MONO_ARCH_FRAME_ALIGNMENT) != 0) {
    cinfo->need_stack_align = TRUE;
    cinfo->stack_align_amount =
        MONO_ARCH_FRAME_ALIGNMENT - (stack_size % MONO_ARCH_FRAME_ALIGNMENT);
...
int mono_arch_get_argument_info(MonoMethodSignature *csig, int param_count,
  ...
  for (k = 0; k < param_count; k++) {
    ...
    if (k == 0 && cinfo->vtype_retaddr && cinfo->vret_arg_index == 1 &&
        !csig->hasthis) {
...
gboolean mono_arch_tailcall_supported(MonoCompile *cfg,
  ...
  // See
  // https://github.com/mono/mono/commit/f5373adc8a89d4b0d1d549fdd6d9adc3ded4b400
...
GList *mono_arch_get_allocatable_int_vars(MonoCompile *cfg) {
  ...
  for (i = 0; i < cfg->num_varinfo; i++) {
    ...
    /* we dont allocate I1 to registers because there is no simply way to sign
     * extend 8bit quantities in caller saved registers on x86 */
    if (mono_is_regsize_var(ins->inst_vtype) &&
        (ins->inst_vtype->type != MONO_TYPE_I1)) {
...
static void set_needs_stack_frame(MonoCompile *cfg, gboolean flag) {
  ...
  if (!inited) {
    mono_counters_register("Could eliminate stack frame",
                           MONO_COUNTER_INT | MONO_COUNTER_JIT, &count);
    inited = TRUE;
  }
  ++count;
...
static gboolean needs_stack_frame(MonoCompile *cfg) {
  MonoMethodSignature *sig;
  ...
  if (cfg->disable_omit_fp)
    ...
    result = TRUE;
           ...
           (MONO_CFG_HAS_CALLS | MONO_CFG_HAS_ALLOCA | MONO_CFG_HAS_TAILCALL))
    result = TRUE;
...
void mono_arch_allocate_vars(MonoCompile *cfg) {
  ...
  if (cfg->has_atomic_add_i4 || cfg->has_atomic_exchange_i4) {
    /* The opcode implementations use callee-saved regs as scratch regs by
     * pushing and pop-ing them, but that is not async safe */
  ...
  /* Allocate locals */
  offsets = mono_allocate_stack_slots(cfg, TRUE, &locals_stack_size,
                                      &locals_stack_align);
  if (locals_stack_size > MONO_ARCH_MAX_FRAME_SIZE) {
    ...
    mono_cfg_set_exception_invalid_program(
        cfg, g_strdup_printf("Method %s stack is too big.", mname));
    g_free(mname);
    return;
  }
...
/*
 * It is expensive to adjust esp for each individual fp argument pushed on the
 * stack so we try to do it just once when we have multiple fp arguments in a
 * row. We don't use this mechanism generally because for int arguments the
 * generated code is slightly bigger and new generation cpus optimize away the
 * dependency chains created by push instructions on the esp value. fp_arg_setup
 * is the first argument in the execution sequence where the esp register is
 * modified.
 */
static G_GNUC_UNUSED int collect_fp_stack_space(MonoMethodSignature *sig,
                                                int start_arg,
                                                int *fp_arg_setup) {
  int fp_space = 0;
  MonoType *t;
...
static void emit_sig_cookie(MonoCompile *cfg, MonoCallInst *call,
  ...
  if (cfg->compile_aot) {
    ...
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, X86_ESP,
                                cinfo->sig_cookie.offset, sig_reg);
  } else {
    MONO_EMIT_NEW_STORE_MEMBASE_IMM(cfg, OP_STORE_MEMBASE_IMM, X86_ESP,
                                    cinfo->sig_cookie.offset, (gsize)tmp_sig);
  }
}
...
LLVMCallInfo *mono_arch_get_llvm_call_info(MonoCompile *cfg,
  ...
  if (cinfo->ret.storage == ArgValuetypeInReg) {
    ...
    for (j = 0; j < 2; ++j)
            linfo->ret.pair_storage [j] = arg_storage_to_llvm_arg_storage (cfg,
    cinfo->ret.pair_storage [j]);
    */
  }
  ...
  for (i = 0; i < n; ++i) {
    ...
    case ArgOnStack:
      if (mini_type_is_vtype(t)) {
        if (mono_class_value_size(mono_class_from_mono_type_internal(t),
                                  NULL) == 0)
    ...
    case ArgValuetypeInReg:
      ...
      for (j = 0; j < 2; ++j)
          linfo->args [i].pair_storage [j] = arg_storage_to_llvm_arg_storage
      (cfg, ainfo->pair_storage [j]);
      */
      break;
    case ArgGSharedVt:
...
static void emit_gc_param_slot_def(MonoCompile *cfg, int sp_offset,
                                   MonoType *t) {
  if (cfg->compute_gc_maps) {
    ...
    /* On x86, the offsets are from the sp value before the start of the call
     * sequence */
    if (t == NULL)
      t = mono_get_int_type();
...
void mono_arch_emit_call(MonoCompile *cfg, MonoCallInst *call) {
  ...
  if (sig_ret && MONO_TYPE_ISSTRUCT(sig_ret)) {
    if (cinfo->ret.storage == ArgValuetypeInReg &&
        cinfo->ret.pair_storage[0] != ArgNone) {
      /*
...
#if defined(__APPLE__)
      if (cinfo->ret.pair_storage[0] == ArgOnDoubleFpStack ||
          cinfo->ret.pair_storage[0] == ArgOnFloatFpStack)
  ...
  for (i = n - 1; i >= 0; i--) {
    ...
    if (ainfo->storage == ArgGSharedVt) {
      ...
      if (t->type == MONO_TYPE_TYPEDBYREF) {
        ...
        size = mini_type_stack_size_full(m_class_get_byval_arg(in->klass),
                                         &align, sig->pinvoke);
      }
      ...
      case ArgOnStack:
        if (!t->byref) {
          if (t->type == MONO_TYPE_R4) {
            MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER4_MEMBASE_REG, X86_ESP,
                                        ainfo->offset, in->dreg);
            argsize = 4;
          } else if (t->type == MONO_TYPE_R8) {
            MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, X86_ESP,
                                        ainfo->offset, in->dreg);
            argsize = 8;
            ...
            MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, X86_ESP,
                                        ainfo->offset, MONO_LVREG_LS(in->dreg));
            argsize = 4;
          } else {
            MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, X86_ESP,
                                        ainfo->offset, in->dreg);
        ...
        if (argsize == 4) {
          ...
          if (sig->hasthis && i == 0 && call->args[i]->type == STACK_OBJ) {
            ...
            if (call->need_unbox_trampoline)
              ...
              emit_gc_param_slot_def(
                  cfg, ainfo->offset,
                  m_class_get_this_arg(mono_defaults.int_class));
            else
              emit_gc_param_slot_def(cfg, ainfo->offset,
  ...
  if (sig_ret && (MONO_TYPE_ISSTRUCT(sig_ret) || cinfo->vtype_retaddr)) {
    ...
    if (cinfo->ret.storage == ArgValuetypeInReg) {
      ...
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, X86_ESP,
                                  cinfo->ret.offset, call->vret_var->dreg);
      emit_gc_param_slot_def(cfg, cinfo->ret.offset, NULL);
    }
  }
...
#define EMIT_FPCOMPARE(code)                                                   \
  do {                                                                         \
    x86_fcompp(code);                                                          \
    x86_fnstsw(code);                                                          \
  } while (0);

static guint8 *x86_align_and_patch(MonoCompile *cfg, guint8 *code,
  ...
  if (cfg->abs_patches) {
    ...
    if (jinfo &&
        (jinfo->type == MONO_PATCH_INFO_JIT_ICALL_ADDR ||
...
static guint8 *emit_call(MonoCompile *cfg, guint8 *code, guint32 patch_type,
                         gconstpointer data) {
...
#define INST_IGNORES_CFLAGS(opcode)                                            \
  (!(((opcode) == OP_ADC) || ((opcode) == OP_IADC) ||                          \
     ((opcode) == OP_ADC_IMM) || ((opcode) == OP_IADC_IMM) ||                  \
     ((opcode) == OP_SBB) || ((opcode) == OP_ISBB) ||                          \
     ((opcode) == OP_SBB_IMM) || ((opcode) == OP_ISBB_IMM)))
...
void mono_arch_peephole_pass_2(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS_SAFE(bb, n, ins) {
    ...
    case OP_ICONST:
      ...
      if (ins->inst_c0 == 0 &&
          (!ins->next ||
           (ins->next && INST_IGNORES_CFLAGS(ins->next->opcode)))) {
        MonoInst *ins2;
         ...
         */
        for (ins2 = mono_inst_next(ins, FILTER_IL_SEQ_POINT); ins2;
             ins2 = ins2->next) {
          if ((ins2->opcode == OP_STORE_MEMBASE_IMM) && (ins2->inst_imm == 0)) {
            ins2->opcode = OP_STORE_MEMBASE_REG;
            ins2->sreg1 = ins->dreg;
          } else if ((ins2->opcode == OP_STOREI4_MEMBASE_IMM) &&
                     (ins2->inst_imm == 0)) {
...
void mono_arch_lowering_pass(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS_SAFE(bb, next, ins) {
    ...
    case OP_IREM_UN_IMM:
      /*
       * Keep the cases where we could generated optimized code, otherwise
       * convert to the non-imm variant.
       */
      if ((ins->opcode == OP_IREM_IMM) &&
          mono_is_power_of_two(ins->inst_imm) >= 0)
        break;
...
#define XMM_TEMP_REG 0
  /*This SSE2 optimization must not be done which OPT_SIMD in place as it
   * clobbers xmm0.*/
...
static unsigned char *mono_emit_stack_alloc(MonoCompile *cfg, guchar *code,
  ...
  if (need_touch) {
    ...
    if (tree->flags & MONO_INST_INIT) {
      ...
      if (cfg->param_area)
        x86_lea_membase(
            code, X86_EDI, X86_ESP,
            12 + ALIGN_TO(cfg->param_area, MONO_ARCH_FRAME_ALIGNMENT));
      else
  ...
  if (tree->flags & MONO_INST_INIT) {
    ...
    if (cfg->param_area)
      x86_lea_membase(code, X86_EDI, X86_ESP,
                      offset +
...
static guint8 *mono_x86_emit_tls_get(guint8 *code, int dreg, int tls_offset) {
...
static guint8 *mono_x86_emit_tls_set(guint8 *code, int sreg, int tls_offset) {
#if defined(TARGET_MACH)
  x86_prefix(code, X86_GS_PREFIX);
...
static guint8 *emit_setup_lmf(MonoCompile *cfg, guint8 *code, gint32 lmf_offset,
  ...
  x86_mov_membase_reg(code, cfg->frame_reg,
                      lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, ebx), X86_EBX,
                      sizeof(target_mgreg_t));
  mono_emit_unwind_op_offset(cfg, code, X86_EBX,
                             -cfa_offset + lmf_offset +
                                 MONO_STRUCT_OFFSET(MonoLMF, ebx));
  x86_mov_membase_reg(code, cfg->frame_reg,
                      lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, edi), X86_EDI,
                      sizeof(target_mgreg_t));
  mono_emit_unwind_op_offset(cfg, code, X86_EDI,
                             -cfa_offset + lmf_offset +
                                 MONO_STRUCT_OFFSET(MonoLMF, edi));
  x86_mov_membase_reg(code, cfg->frame_reg,
                      lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, esi), X86_ESI,
                      sizeof(target_mgreg_t));
  mono_emit_unwind_op_offset(cfg, code, X86_ESI,
                             -cfa_offset + lmf_offset +
                                 MONO_STRUCT_OFFSET(MonoLMF, esi));
  x86_mov_membase_reg(code, cfg->frame_reg,
                      lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, ebp), X86_EBP,
                      sizeof(target_mgreg_t));
  ...
  if (cfg->compile_aot) {
    ...
    mono_add_patch_info(cfg, code + 1 - cfg->native_code, MONO_PATCH_INFO_IP,
                        NULL);
    x86_mov_reg_imm(code, X86_EAX, 0);
  }
  x86_mov_membase_reg(code, cfg->frame_reg,
                      lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, eip), X86_EAX,
                      sizeof(target_mgreg_t));

  mini_gc_set_slot_type_from_cfa(
      cfg, -cfa_offset + lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, eip),
      SLOT_NOREF);
  mini_gc_set_slot_type_from_cfa(
      cfg, -cfa_offset + lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, ebp),
      SLOT_NOREF);
  mini_gc_set_slot_type_from_cfa(
      cfg, -cfa_offset + lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, esi),
      SLOT_NOREF);
  mini_gc_set_slot_type_from_cfa(
      cfg, -cfa_offset + lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, edi),
      SLOT_NOREF);
  mini_gc_set_slot_type_from_cfa(
      cfg, -cfa_offset + lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, ebx),
      SLOT_NOREF);
  mini_gc_set_slot_type_from_cfa(
      cfg, -cfa_offset + lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, esp),
      SLOT_NOREF);
  mini_gc_set_slot_type_from_cfa(
      cfg, -cfa_offset + lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, method),
      SLOT_NOREF);
  mini_gc_set_slot_type_from_cfa(
      cfg, -cfa_offset + lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, lmf_addr),
      SLOT_NOREF);
  mini_gc_set_slot_type_from_cfa(
      cfg, -cfa_offset + lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, previous_lmf),
      SLOT_NOREF);
...
void mono_arch_output_basic_block(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS(bb, ins) {
    ...
    case OP_X86_SETNE_MEMBASE:
      x86_set_membase(
          code, ins->opcode == OP_X86_SETEQ_MEMBASE ? X86_CC_EQ : X86_CC_NE,
          ins->inst_basereg, ins->inst_offset, TRUE);
      break;
    case OP_STOREI1_MEMBASE_IMM:
      x86_mov_membase_imm(code, ins->inst_destbasereg, ins->inst_offset,
                          ins->inst_imm, 1);
      break;
    ...
    case OP_LOADU4_MEMBASE:
      x86_mov_reg_membase(code, ins->dreg, ins->inst_basereg, ins->inst_offset,
                          4);
      break;
    ...
    case OP_X86_COMPARE_MEMBASE_REG:
      x86_alu_membase_reg(code, X86_CMP, ins->inst_basereg, ins->inst_offset,
                          ins->sreg2);
      break;
    case OP_X86_COMPARE_MEMBASE_IMM:
      x86_alu_membase_imm(code, X86_CMP, ins->inst_basereg, ins->inst_offset,
                          ins->inst_imm);
      break;
    case OP_X86_COMPARE_MEMBASE8_IMM:
      x86_alu_membase8_imm(code, X86_CMP, ins->inst_basereg, ins->inst_offset,
                           ins->inst_imm);
      break;
    case OP_X86_COMPARE_REG_MEMBASE:
      x86_alu_reg_membase(code, X86_CMP, ins->sreg1, ins->sreg2,
                          ins->inst_offset);
      break;
    ...
    case OP_X86_ADD_MEMBASE_IMM:
      x86_alu_membase_imm(code, X86_ADD, ins->inst_basereg, ins->inst_offset,
                          ins->inst_imm);
      break;
    case OP_X86_ADD_REG_MEMBASE:
      x86_alu_reg_membase(code, X86_ADD, ins->sreg1, ins->sreg2,
                          ins->inst_offset);
      break;
    case OP_X86_SUB_MEMBASE_IMM:
      x86_alu_membase_imm(code, X86_SUB, ins->inst_basereg, ins->inst_offset,
                          ins->inst_imm);
      break;
    case OP_X86_SUB_REG_MEMBASE:
      x86_alu_reg_membase(code, X86_SUB, ins->sreg1, ins->sreg2,
                          ins->inst_offset);
      break;
    case OP_X86_AND_MEMBASE_IMM:
      x86_alu_membase_imm(code, X86_AND, ins->inst_basereg, ins->inst_offset,
                          ins->inst_imm);
      break;
    case OP_X86_OR_MEMBASE_IMM:
      x86_alu_membase_imm(code, X86_OR, ins->inst_basereg, ins->inst_offset,
                          ins->inst_imm);
      break;
    case OP_X86_XOR_MEMBASE_IMM:
      x86_alu_membase_imm(code, X86_XOR, ins->inst_basereg, ins->inst_offset,
                          ins->inst_imm);
      break;
    case OP_X86_ADD_MEMBASE_REG:
      x86_alu_membase_reg(code, X86_ADD, ins->inst_basereg, ins->inst_offset,
                          ins->sreg2);
      break;
    case OP_X86_SUB_MEMBASE_REG:
      x86_alu_membase_reg(code, X86_SUB, ins->inst_basereg, ins->inst_offset,
                          ins->sreg2);
      break;
    case OP_X86_AND_MEMBASE_REG:
      x86_alu_membase_reg(code, X86_AND, ins->inst_basereg, ins->inst_offset,
                          ins->sreg2);
      break;
    ...
    case OP_X86_XOR_MEMBASE_REG:
      x86_alu_membase_reg(code, X86_XOR, ins->inst_basereg, ins->inst_offset,
                          ins->sreg2);
      break;
    ...
    case OP_X86_AND_REG_MEMBASE:
      x86_alu_reg_membase(code, X86_AND, ins->sreg1, ins->sreg2,
                          ins->inst_offset);
      break;
    case OP_X86_OR_REG_MEMBASE:
      x86_alu_reg_membase(code, X86_OR, ins->sreg1, ins->sreg2,
                          ins->inst_offset);
      break;
    case OP_X86_XOR_REG_MEMBASE:
      x86_alu_reg_membase(code, X86_XOR, ins->sreg1, ins->sreg2,
                          ins->inst_offset);
      break;
    case OP_BREAK:
      x86_breakpoint(code);
    ...
    case OP_SEQ_POINT: {
      ...
      if (ins->flags & MONO_INST_SINGLE_STEP_LOC) {
        ...
        x86_mov_reg_membase(code, X86_ECX, var->inst_basereg, var->inst_offset,
                            sizeof(target_mgreg_t));
      ...
      /*
       * Many parts of sdb depend on the ip after the single step trampoline
       * call to be equal to the seq point offset. This means we have to put the
       * loading of bp_tramp_var after the offset.
       */
    ...
    case OP_GOT_ENTRY:
      mono_add_patch_info(cfg, offset,
                          (MonoJumpInfoType)(gsize)ins->inst_right->inst_i1,
    ...
    case OP_TAILCALL_REG: {
      ...
      max_len += (call->stack_usage - call->stack_align_amount) /
                 sizeof(target_mgreg_t) * ins_get_size(OP_TAILCALL_PARAMETER);
      code = realloc_code(cfg, max_len);
      ...
      //
      // Eax could also be used here at the cost of a push/pop moving the
      // parameters. Edx must be preserved as it is rgctx/imt.
      //
      // If ecx happens to be the base of the tailcall_membase, then
      // just end with jmp [ecx+offset] -- one instruction.
      // if ecx is not the base, then move ecx, [reg+offset] and later jmp [ecx]
      // -- two instructions.
      ...
      if (tailcall_membase_ecx) {
        ...
        code = x86_align_and_patch(cfg, code, MONO_PATCH_INFO_METHOD_JUMP,
                                   call->method);
        x86_jump32(code, 0);
      }
    ...
    case OP_X86_LEA:
      x86_lea_memindex(code, ins->dreg, ins->sreg1, ins->inst_imm, ins->sreg2,
    ...
    case OP_THROW: {
      ...
      code =
          emit_call(cfg, code, MONO_PATCH_INFO_JIT_ICALL_ID,
    ...
    case OP_RETHROW: {
      ...
      x86_push_reg(code, ins->sreg1);
      code = emit_call(
          cfg, code, MONO_PATCH_INFO_JIT_ICALL_ID,
    ...
    case OP_START_HANDLER: {
      ...
      x86_mov_membase_reg(code, spvar->inst_basereg, spvar->inst_offset,
                          X86_ESP, 4);
      if (cfg->param_area)
        x86_alu_reg_imm(code, X86_SUB, X86_ESP,
                        ALIGN_TO(cfg->param_area, MONO_ARCH_FRAME_ALIGNMENT));
      break;
    }
    case OP_ENDFINALLY: {
      MonoInst *spvar = mono_find_spvar_for_region(cfg, bb->region);
      x86_mov_reg_membase(code, X86_ESP, spvar->inst_basereg,
                          spvar->inst_offset, 4);
      x86_ret(code);
      break;
    }
    ...
    case OP_ICGT:
    case OP_ICGT_UN:
      x86_set_reg(code, cc_table[mono_opcode_to_cond(ins->opcode)], ins->dreg,
    ...
    case OP_CMOV_ILT_UN:
      ...
      x86_cmov_reg(code, cc_table[mono_opcode_to_cond(ins->opcode)],
                   cc_signed_table[mono_opcode_to_cond(ins->opcode)], ins->dreg,
                   ins->sreg2);
      break;
    ...
    case OP_R8CONST: {
      ...
      if ((d == 0.0) && (mono_signbit(d) == 0)) {
        ...
        if (cfg->compile_aot) {
          ...
          mono_add_patch_info(cfg, code - cfg->native_code, MONO_PATCH_INFO_R8,
                              ins->inst_p0);
    ...
    case OP_R4CONST: {
      ...
      if ((f == 0.0) && (mono_signbit(f) == 0)) {
        ...
        if (cfg->compile_aot) {
          ...
          mono_add_patch_info(cfg, code - cfg->native_code, MONO_PATCH_INFO_R4,
                              ins->inst_p0);
    ...
    case OP_STORER8_MEMBASE_REG:
      x86_fst_membase(code, ins->inst_destbasereg, ins->inst_offset, TRUE,
                      TRUE);
      break;
    ...
    case OP_STORER4_MEMBASE_REG:
      x86_fst_membase(code, ins->inst_destbasereg, ins->inst_offset, FALSE,
                      TRUE);
      break;
    ...
    case OP_LCONV_TO_R_UN_2: {
      ...
      br = code;
      x86_branch8(code, X86_CC_GEZ, 0, TRUE);

      /* add correction constant mn */
      if (cfg->compile_aot) {
        x86_push_imm(code, (((guint32)mn[9]) << 24) | ((guint32)mn[8] << 16) |
                               ((guint32)mn[7] << 8) | ((guint32)mn[6]));
        x86_push_imm(code, (((guint32)mn[5]) << 24) | ((guint32)mn[4] << 16) |
                               ((guint32)mn[3] << 8) | ((guint32)mn[2]));
    ...
    case OP_LCONV_TO_OVF_I4_2: {
      ...
      br[0] = code;
      x86_branch8(code, X86_CC_LT, 0, TRUE);
      ...
      /* throw exception */
      tins =
          mono_branch_optimize_exception_target(cfg, bb, "OverflowException");
      if (tins) {
        mono_add_patch_info(cfg, code - cfg->native_code, MONO_PATCH_INFO_BB,
                            tins->inst_true_bb);
        if ((cfg->opt & MONO_OPT_BRANCH) &&
            x86_is_imm8(tins->inst_true_bb->max_offset - cpos))
          x86_jump8(code, 0);
        else
          x86_jump32(code, 0);
      } else {
    ...
    case OP_MOVE_F_TO_I4:
      x86_fst_membase(code, ins->backend.spill_var->inst_basereg,
                      ins->backend.spill_var->inst_offset, FALSE, TRUE);
      x86_mov_reg_membase(code, ins->dreg, ins->backend.spill_var->inst_basereg,
                          ins->backend.spill_var->inst_offset, 4);
      break;
    case OP_MOVE_I4_TO_F:
      x86_mov_membase_reg(code, ins->backend.spill_var->inst_basereg,
                          ins->backend.spill_var->inst_offset, ins->sreg1, 4);
      x86_fld_membase(code, ins->backend.spill_var->inst_basereg,
                      ins->backend.spill_var->inst_offset, FALSE);
      break;
    case OP_FADD:
      x86_fp_op_reg(code, X86_FADD, 1, TRUE);
      break;
    ...
    case OP_FCNEQ:
      ...
      x86_set_reg(code, ins->opcode == OP_FCEQ ? X86_CC_EQ : X86_CC_NE,
                  ins->dreg, TRUE);
      x86_widen_reg(code, ins->dreg, ins->dreg, FALSE, FALSE);
    ...
    case OP_ATOMIC_LOAD_R8: {
      x86_fld_membase(code, ins->inst_basereg, ins->inst_offset,
                      ins->opcode == OP_ATOMIC_LOAD_R8);
      break;
    }
    ...
    case OP_ATOMIC_STORE_R8: {
      x86_fst_membase(code, ins->inst_destbasereg, ins->inst_offset,
                      ins->opcode == OP_ATOMIC_STORE_R8, TRUE);
    ...
    case OP_CARD_TABLE_WBARRIER: {
      ...
      size_t nursery_size;
      gulong card_table =
          (gsize)mono_gc_get_card_table(&card_table_shift, &card_table_mask);
      gulong nursery_start =
          (gsize)mono_gc_get_nursery(&nursery_shift, &nursery_size);
    ...
    case OP_COMPPS:
      ...
      x86_sse_alu_ps_reg_reg_imm(code, X86_SSE_COMP, ins->sreg1, ins->sreg2,
                                 ins->inst_c0);
      break;
    case OP_ANDPS:
    ...
    case OP_PSHUFLED:
      g_assert(ins->inst_c0 >= 0 && ins->inst_c0 <= 0xFF);
      x86_sse_shift_reg_imm(code, X86_SSE_PSHUFD, ins->dreg, ins->sreg1,
                            ins->inst_c0);
      break;
    case OP_SHUFPS:
      g_assert(ins->inst_c0 >= 0 && ins->inst_c0 <= 0xFF);
      x86_sse_alu_reg_reg_imm8(code, X86_SSE_SHUFP, ins->sreg1, ins->sreg2,
                               ins->inst_c0);
      break;
    case OP_SHUFPD:
      g_assert(ins->inst_c0 >= 0 && ins->inst_c0 <= 0x3);
      x86_sse_alu_pd_reg_reg_imm8(code, X86_SSE_SHUFP, ins->sreg1, ins->sreg2,
                                  ins->inst_c0);
      break;
    ...
    case OP_COMPPD:
      g_assert(ins->inst_c0 >= 0 && ins->inst_c0 <= 7);
      x86_sse_alu_pd_reg_reg_imm(code, X86_SSE_COMP, ins->sreg1, ins->sreg2,
                                 ins->inst_c0);
      break;
    case OP_ANDPD:
    ...
    case OP_PSHRW:
      x86_sse_shift_reg_imm(code, X86_SSE_PSHIFTW, X86_SSE_SHR, ins->dreg,
                            ins->inst_imm);
      break;
    case OP_PSHRW_REG:
    ...
    case OP_PSARW:
      x86_sse_shift_reg_imm(code, X86_SSE_PSHIFTW, X86_SSE_SAR, ins->dreg,
                            ins->inst_imm);
      break;
    case OP_PSARW_REG:
    ...
    case OP_PSHLW:
      x86_sse_shift_reg_imm(code, X86_SSE_PSHIFTW, X86_SSE_SHL, ins->dreg,
                            ins->inst_imm);
      break;
    case OP_PSHLW_REG:
    ...
    case OP_PSHRD:
      x86_sse_shift_reg_imm(code, X86_SSE_PSHIFTD, X86_SSE_SHR, ins->dreg,
                            ins->inst_imm);
      break;
    case OP_PSHRD_REG:
    ...
    case OP_PSARD:
      x86_sse_shift_reg_imm(code, X86_SSE_PSHIFTD, X86_SSE_SAR, ins->dreg,
                            ins->inst_imm);
      break;
    case OP_PSARD_REG:
    ...
    case OP_PSHLD:
      x86_sse_shift_reg_imm(code, X86_SSE_PSHIFTD, X86_SSE_SHL, ins->dreg,
                            ins->inst_imm);
      break;
    case OP_PSHLD_REG:
    ...
    case OP_PSHRQ:
      x86_sse_shift_reg_imm(code, X86_SSE_PSHIFTQ, X86_SSE_SHR, ins->dreg,
                            ins->inst_imm);
      break;
    case OP_PSHRQ_REG:
    ...
    case OP_PSHLQ:
      x86_sse_shift_reg_imm(code, X86_SSE_PSHIFTQ, X86_SSE_SHL, ins->dreg,
                            ins->inst_imm);
      break;
    case OP_PSHLQ_REG:
    ...
    case OP_EXTRACT_U1:
      ...
      x86_widen_reg(code, ins->dreg, ins->dreg, ins->opcode == OP_EXTRACT_I1,
                    FALSE);
      break;
    case OP_EXTRACT_I2:
    case OP_EXTRACT_U2:
      ...
      x86_widen_reg(code, ins->dreg, ins->dreg, ins->opcode == OP_EXTRACT_I2,
                    TRUE);
      break;
    case OP_EXTRACT_R8:
      if (ins->inst_c0)
        x86_sse_alu_pd_membase_reg(code, X86_SSE_MOVHPD_MEMBASE_REG,
                                   ins->backend.spill_var->inst_basereg,
                                   ins->backend.spill_var->inst_offset,
                                   ins->sreg1);
      else
        x86_sse_alu_sd_membase_reg(code, X86_SSE_MOVSD_MEMBASE_REG,
                                   ins->backend.spill_var->inst_basereg,
                                   ins->backend.spill_var->inst_offset,
                                   ins->sreg1);
      x86_fld_membase(code, ins->backend.spill_var->inst_basereg,
                      ins->backend.spill_var->inst_offset, TRUE);
      break;

    case OP_INSERT_I2:
      x86_sse_alu_pd_reg_reg_imm(code, X86_SSE_PINSRW, ins->sreg1, ins->sreg2,
                                 ins->inst_c0);
      break;
    case OP_EXTRACTX_U2:
      x86_sse_alu_pd_reg_reg_imm(code, X86_SSE_PEXTRW, ins->dreg, ins->sreg1,
                                 ins->inst_c0);
      break;
    case OP_INSERTX_U1_SLOW:
      ...
      /*join them together*/
      x86_alu_reg_reg(code, X86_OR, ins->sreg1, ins->sreg2);
      x86_sse_alu_pd_reg_reg_imm(code, X86_SSE_PINSRW, ins->dreg, ins->sreg1,
                                 ins->inst_c0 / 2);
      break;
    case OP_INSERTX_I4_SLOW:
      x86_sse_alu_pd_reg_reg_imm(code, X86_SSE_PINSRW, ins->dreg, ins->sreg2,
                                 ins->inst_c0 * 2);
    ...
    case OP_INSERTX_R4_SLOW:
      ...
      x86_sse_alu_pd_reg_membase_imm(
          code, X86_SSE_PINSRW, ins->dreg, ins->backend.spill_var->inst_basereg,
          ins->backend.spill_var->inst_offset + 0, ins->inst_c0 * 2);
      x86_sse_alu_pd_reg_membase_imm(
          code, X86_SSE_PINSRW, ins->dreg, ins->backend.spill_var->inst_basereg,
          ins->backend.spill_var->inst_offset + 2, ins->inst_c0 * 2 + 1);
      break;
    case OP_INSERTX_R8_SLOW:
      x86_fst_membase(code, ins->backend.spill_var->inst_basereg,
                      ins->backend.spill_var->inst_offset, TRUE, TRUE);
      if (cfg->verbose_level)
        printf("CONVERTING a OP_INSERTX_R8_SLOW %d offset %x\n", ins->inst_c0,
               offset);
      if (ins->inst_c0)
        x86_sse_alu_pd_reg_membase(code, X86_SSE_MOVHPD_REG_MEMBASE, ins->dreg,
                                   ins->backend.spill_var->inst_basereg,
                                   ins->backend.spill_var->inst_offset);
      else
        x86_movsd_reg_membase(code, ins->dreg,
                              ins->backend.spill_var->inst_basereg,
                              ins->backend.spill_var->inst_offset);
      break;
    ...
    case OP_STOREX_NTA_MEMBASE_REG:
      x86_sse_alu_reg_membase(code, X86_SSE_MOVNTPS, ins->dreg, ins->sreg1,
                              ins->inst_offset);
      break;
    case OP_PREFETCH_MEMBASE:
      x86_sse_alu_reg_membase(code, X86_SSE_PREFETCH, ins->backend.arg_info,
    ...
    case OP_EXPAND_R4:
      x86_fst_membase(code, ins->backend.spill_var->inst_basereg,
                      ins->backend.spill_var->inst_offset, FALSE, TRUE);
      x86_movd_xreg_membase(code, ins->dreg,
                            ins->backend.spill_var->inst_basereg,
                            ins->backend.spill_var->inst_offset);
      x86_sse_shift_reg_imm(code, X86_SSE_PSHUFD, ins->dreg, ins->dreg, 0);
      break;
    ...
    case OP_FILL_PROF_CALL_CTX:
      x86_mov_membase_reg(code, ins->sreg1,
                          MONO_STRUCT_OFFSET(MonoContext, esp), X86_ESP,
                          sizeof(target_mgreg_t));
      x86_mov_membase_reg(code, ins->sreg1,
                          MONO_STRUCT_OFFSET(MonoContext, ebp), X86_EBP,
                          sizeof(target_mgreg_t));
      x86_mov_membase_reg(code, ins->sreg1,
                          MONO_STRUCT_OFFSET(MonoContext, ebx), X86_EBX,
                          sizeof(target_mgreg_t));
      x86_mov_membase_reg(code, ins->sreg1,
                          MONO_STRUCT_OFFSET(MonoContext, esi), X86_ESI,
                          sizeof(target_mgreg_t));
      x86_mov_membase_reg(code, ins->sreg1,
                          MONO_STRUCT_OFFSET(MonoContext, edi), X86_EDI,
                          sizeof(target_mgreg_t));
      break;
...
static G_GNUC_UNUSED void stack_unaligned(MonoMethod *m, gpointer caller) {
...
guint8 *mono_arch_emit_prolog(MonoCompile *cfg) {
  ...
  /* the original alloc_size is already aligned: there is %ebp and retip pushed,
   * so realign */
  if (mono_do_x86_stack_align && need_stack_frame) {
    ...
    if (tot) {
      ...
      for (i = 0; i < MONO_ARCH_FRAME_ALIGNMENT - tot;
  ...
  if (alloc_size) {
    ...
    /*FIXME handle unbounded code expansion, we should use a loop in case of
     * more than X interactions*/
    guint32 required_code_size =
        ((remaining_size / 0x1000) + 1) *
        8; /*8 is the max size of x86_alu_reg_imm + x86_test_membase_reg*/
    set_code_cursor(cfg, code);
  ...
  if (cfg->rgctx_var) {
    g_assert(cfg->rgctx_var->opcode == OP_REGOFFSET &&
             cfg->rgctx_var->inst_basereg == X86_EBP);

    x86_mov_membase_reg(code, X86_EBP, cfg->rgctx_var->inst_offset,
                        MONO_ARCH_RGCTX_REG, 4);
  }
...

  {
    ...
    if (cfg->arch.ss_tramp_var) {
      ...
      x86_mov_membase_imm(code, ins->inst_basereg, ins->inst_offset,
                          (gsize)&ss_trampoline, 4);
    }
  ...
  for (i = 0; i < sig->param_count + sig->hasthis; ++i) {
    ...
    if (inst->opcode == OP_REGVAR) {
      if (storage_in_ireg(ainfo->storage)) {
        ...
        g_assert(need_stack_frame);
        x86_mov_reg_membase(code, inst->dreg, X86_EBP,
                            ainfo->offset + ARGS_OFFSET, 4);
      }
      if (cfg->verbose_level > 2)
      ...
      if (storage_in_ireg(ainfo->storage)) {
        x86_mov_membase_reg(code, inst->inst_basereg, inst->inst_offset,
                            ainfo->reg, 4);
      }
    }
    pos++;
...
void mono_arch_emit_epilog(MonoCompile *cfg) {
  ...
  if (method->save_lmf) {
    ...
    if (cfg->used_int_regs & (1 << X86_EBX)) {
      x86_mov_reg_membase(code, X86_EBX, cfg->frame_reg,
                          lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, ebx), 4);
    }

    if (cfg->used_int_regs & (1 << X86_EDI)) {
      x86_mov_reg_membase(code, X86_EDI, cfg->frame_reg,
                          lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, edi), 4);
    }
    if (cfg->used_int_regs & (1 << X86_ESI)) {
      x86_mov_reg_membase(code, X86_ESI, cfg->frame_reg,
                          lmf_offset + MONO_STRUCT_OFFSET(MonoLMF, esi), 4);
    }
  ...
  if (cinfo->ret.storage == ArgValuetypeInReg) {
    for (quad = 0; quad < 2; quad++) {
      ...
      case ArgInIReg:
        x86_mov_reg_membase(
            code, cinfo->ret.pair_regs[quad], cfg->ret->inst_basereg,
            cfg->ret->inst_offset + (quad * sizeof(target_mgreg_t)), 4);
        break;
      case ArgOnFloatFpStack:
        x86_fld_membase(code, cfg->ret->inst_basereg,
                        cfg->ret->inst_offset + (quad * sizeof(target_mgreg_t)),
                        FALSE);
        break;
      case ArgOnDoubleFpStack:
        x86_fld_membase(code, cfg->ret->inst_basereg,
  ...
  if (CALLCONV_IS_STDCALL(sig)) {
    MonoJitArgumentInfo *arg_info =
        g_newa(MonoJitArgumentInfo, sig->param_count + 1);
  ...
  for (patch_info = cfg->patch_info; patch_info;
    ...
    case MONO_PATCH_INFO_EXC: {
      ...
      if (i < nthrows) {
        ...
        /*This is aligned to 16 bytes by the callee. This way we save a few
         * bytes here.*/
...
gpointer mono_arch_build_imt_trampoline(MonoVTable *vtable, MonoDomain *domain,
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (item->is_equals) {
      if (item->check_target_idx) {
        if (!item->compare_done)
          x86_alu_reg_imm(code, X86_CMP, MONO_ARCH_IMT_REG,
  ...
  if (mono_jit_map_is_enabled()) {
    char *buff;
    if (vtable)
      buff = g_strdup_printf("imt_%s_%s_entries_%d",
                             m_class_get_name_space(vtable->klass),
                             m_class_get_name(vtable->klass), count);
    else
...
guint32 mono_arch_get_patch_offset(guint8 *code) {
  if ((code[0] == 0x8b) && (x86_modrm_mod(code[1]) == 0x2))
    ...
    return 2;
  else if ((code[0] >= 0x58) && (code[0] <= 0x58 + X86_NREG) &&
           (code[1] == 0x81))
 ...
 *
 *   Return the offset of the stack location where this is passed during a
 * virtual call.
 */
guint32 mono_x86_get_this_arg_offset(MonoMethodSignature *sig) { return 0; }
...
static gpointer get_delegate_invoke_impl(MonoTrampInfo **info,
  ...
  if (has_target) {
    ...
    x86_mov_reg_membase(code, X86_ECX, X86_EAX,
                        MONO_STRUCT_OFFSET(MonoDelegate, target), 4);
    x86_mov_membase_reg(code, X86_ESP, 4, X86_ECX, 4);
    x86_jump_membase(code, X86_EAX,
     ...
     * So move each arg up a spot in the stack (overwriting un-needed 'this'
     * arg) and leaving original spot of first arg as placeholder in stack so
    ...
    x86_jump_membase(code, X86_ECX,
                     MONO_STRUCT_OFFSET(MonoDelegate, method_ptr));
  }

  g_assertf((code - start) <= code_reserve, "%d %d", (int)(code - start),
            code_reserve);
...
static gpointer get_delegate_virtual_invoke_impl(MonoTrampInfo **info,
  ...
  if (load_imt_reg) {
    /* Load the IMT reg */
    x86_mov_reg_membase(code, MONO_ARCH_IMT_REG, X86_EAX,
                        MONO_STRUCT_OFFSET(MonoDelegate, method), 4);
  }

  /* Load the vtable */
  x86_mov_reg_membase(code, X86_EAX, X86_ECX,
                      MONO_STRUCT_OFFSET(MonoObject, vtable), 4);
...
static MonoInst *get_float_to_x_spill_area(MonoCompile *cfg) {
  if (!cfg->fconv_to_r8_x_var) {
    cfg->fconv_to_r8_x_var = mono_compile_create_var(
        cfg, m_class_get_byval_arg(mono_defaults.double_class), OP_LOCAL);
    cfg->fconv_to_r8_x_var->flags |=
        MONO_INST_VOLATILE; /*FIXME, use the don't regalloc flag*/
  }
...
void mono_arch_decompose_opts(MonoCompile *cfg, MonoInst *ins) {
  ...
  fconv->klass = NULL; /*FIXME, what can I use here as the Mono.Simd lib might
                          not be loaded yet*/
...
void mono_arch_decompose_long_opts(MonoCompile *cfg, MonoInst *long_ins) {
  ...
  if (long_ins->opcode == OP_LNEG) {
    ins = long_ins;
    MONO_EMIT_NEW_UNALU(cfg, OP_INEG, MONO_LVREG_LS(ins->dreg),
                        MONO_LVREG_LS(ins->sreg1));
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ADC_IMM, MONO_LVREG_MS(ins->dreg),
                            MONO_LVREG_MS(ins->sreg1), 0);
    MONO_EMIT_NEW_UNALU(cfg, OP_INEG, MONO_LVREG_MS(ins->dreg),
  ...
  /*TODO move this to simd-intrinsic.c once we support sse 4.1 dword extractors
   * since we need the runtime caps info */
...
guint8 *mono_arch_emit_load_got_addr(guint8 *start, guint8 *code,
  ...
  if (cfg)
    mono_add_patch_info(cfg, code - cfg->native_code,
                        MONO_PATCH_INFO_GOT_OFFSET, NULL);
  else
    *ji = mono_patch_info_list_prepend(*ji, code - start,
                                       MONO_PATCH_INFO_GOT_OFFSET, NULL);
 ...
 * On x86, the GOT address is assumed to be in EBX, and the result is placed
 * into EAX.
 */
...
gpointer mono_x86_get_signal_exception_trampoline(MonoTrampInfo **info,
                                                  gboolean aot);
