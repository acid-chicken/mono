...
typedef VOID(NTAPI *RtlGrowFunctionTablePtr)(PVOID DynamicTable,
                                             DWORD NewEntryCount);

typedef VOID(NTAPI *RtlDeleteGrowableFunctionTablePtr)(PVOID DynamicTable);
...
static const AMD64_Reg_No param_regs[] = {AMD64_RCX, AMD64_RDX, AMD64_R8,
                                          AMD64_R9};

static const AMD64_XMM_Reg_No float_param_regs[] = {AMD64_XMM0, AMD64_XMM1,
                                                    AMD64_XMM2, AMD64_XMM3};
...
static const AMD64_XMM_Reg_No float_param_regs[] = {
    AMD64_XMM0, AMD64_XMM1, AMD64_XMM2, AMD64_XMM3,
    AMD64_XMM4, AMD64_XMM5, AMD64_XMM6, AMD64_XMM7};
...
typedef struct {
  ...
  int ret_marshal;
  /* If ret_marshal != NONE, this is the reg of the vret arg, else -1 (used in
   * out case) */
  ...
  int gsharedvt_in;
  /* Maps stack slots/registers in the caller to the stack slots/registers in
   * the callee */
...
typedef struct {
  ...
  int byte_arg_size;
  guint8 pass_empty_struct : 1; // Set in scenarios when empty structs needs to
                                // be represented as argument.
  guint8 is_signed : 1;
} ArgInfo;
...
#define MONO_CONTEXT_SET_LLVM_EXC_REG(ctx, exc)                                \
  do {                                                                         \
    (ctx)->gregs[AMD64_RAX] = (gsize)exc;                                      \
  } while (0)
#define MONO_CONTEXT_SET_LLVM_EH_SELECTOR_REG(ctx, sel)                        \
  do {                                                                         \
    (ctx)->gregs[AMD64_RDX] = (gsize)(sel);                                    \
  } while (0)
...
/*
 * __builtin_frame_address () is broken on some older gcc versions in the
 * presence of frame pointer elimination, see bug #82095.
 */
#define MONO_INIT_CONTEXT_FROM_FUNC(ctx, start_func)                           \
  do {                                                                         \
    int tmp;                                                                   \
...
// Only supported for Windows cross compiler builds, host == Win32, target !=
// Win32 and only using MSVC for none cross compiler builds.
...
#define MONO_ARCH_EMIT_BOUNDS_CHECK(cfg, array_reg, offset, index_reg,         \
                                    ex_name)                                   \
  do {                                                                         \
    MonoInst *inst;                                                            \
...
gpointer mono_amd64_start_gsharedvt_call(GSharedVtCallInfo *info,
                                         gpointer *caller, gpointer *callee,
                                         gpointer mrgctx_reg);
...
static inline guint mono_arch_unwindinfo_get_size(guchar code_count) {
  // Returned size will be used as the allocated size for unwind data trailing
  // the memory used by compiled method. Windows x64 ABI have some requirements
  // on the data written into this memory. Both the RUNTIME_FUNCTION and
  // UNWIND_INFO struct needs to be DWORD aligned and the number of elements in
  // unwind codes array should have an even number of entries, while the count
  // stored in UNWIND_INFO struct should hold the real number of unwind codes.
  // Adding extra bytes to the total size will make sure we can properly align
  // the RUNTIME_FUNCTION struct. Since our UNWIND_INFO follows RUNTIME_FUNCTION
  // struct in memory, it will automatically be DWORD aligned as well. Also make
  // sure to allocate room for a padding UNWIND_CODE, if needed.
  return (sizeof(target_mgreg_t) + sizeof(UNWIND_INFO)) -
         (sizeof(UNWIND_CODE) *
          ((MONO_MAX_UNWIND_CODES - ((code_count + 1) & ~1))));
  /* FIXME Something simpler should work:
      return sizeof (UNWIND_INFO) + sizeof (UNWIND_CODE) * (code_count +
     (code_count & 1));
  */
}
...
// Allocate additional size for max 3 unwind ops (push + fp or sp small|large) +
// unwind info struct trailing code buffer.
#define MONO_TRAMPOLINE_UNWINDINFO_SIZE(max_code_count)                        \
  (mono_arch_unwindinfo_get_size(max_code_count))
#define MONO_MAX_TRAMPOLINE_UNWINDINFO_SIZE (MONO_TRAMPOLINE_UNWINDINFO_SIZE(3))

static inline gboolean mono_arch_unwindinfo_validate_size(GSList *unwind_ops,
                                                          guint max_size) {
  guint current_size = mono_arch_unwindinfo_get_size(
      mono_arch_unwindinfo_get_code_count(unwind_ops));
