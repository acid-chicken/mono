...
TODO make passing a xmm as argument not cause it to be LDADDR'ed (introduce an
OP_XPUSH)
TODO revamp the .ctor sequence as it looks very fragile, maybe use a var just
like move_i4_to_f. (or just pinst sse ops)
TODO figure out what's wrong with OP_STOREX_MEMBASE_REG and OP_STOREX_MEMBASE
(the 2nd is for imm operands)
TODO maybe add SSE3 emulation on top of SSE2, or just implement the
corresponding functions using SSE2 intrinsics.
TODO pass simd arguments in registers or, at least, add SSE support for pushing
large (>=16) valuetypes
TODO pass simd args byval to a non-intrinsic method cause some useless local var
load/store to happen.
TODO check if we need to init the SSE control word with better precision.
TODO add support for 3 reg sources in mini without slowing the common path. Or
find a way to make MASKMOVDQU work.
TODO make SimdRuntime.get_AccelMode work under AOT
TODO patterns such as "a ^= b" generate slower code as the LDADDR op will be
copied to a tmp first. Look at adding a indirection reduction pass after the dce
pass.
...
Extracting a float from a XMM is a complete disaster if you are passing it as an
argument. It will be loaded in the FP stack just to be pushed on the call stack.

A similar thing happens with Vector4f constructor that require float vars to be

The fix for this issue is similar to the one required for r4const as method
args. Avoiding the trip to the FP stack is desirable.

-Extractor and constructor code doesn't make sense under amd64. Both currently
assume separate banks for simd and fp.


-Promote OP_EXTRACT_I4 to a STORE op
The advantage of this change is that it could have a _membase version and
promote further optimizations.

-Create a MONO_INST_DONT_REGALLOC and use it in all places that
MONO_INST_INDIRECT is used without a OP_LDADDR.
*/
...
enum {
#define SIMD_METHOD(str, name)                                                 \
  name = offsetof(struct msgstr_t, MSGSTRFIELD(__LINE__)),
#include "simd-methods.h"
};
...
static const SimdIntrinsic vector4f_intrinsics[] = {
    ...
    {SN_CompareEqual, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_EQ},
    {SN_CompareLessEqual, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_LE},
    {SN_CompareLessThan, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_LT},
    {SN_CompareNotEqual, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_NEQ},
    {SN_CompareNotLessEqual, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_NLE},
    {SN_CompareNotLessThan, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_NLT},
    {SN_CompareOrdered, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_ORD},
    {SN_CompareUnordered, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_UNORD},
    ...
    {SN_ConvertToIntTruncated, OP_CVTTPS2DQ, SIMD_VERSION_SSE2,
     SIMD_EMIT_UNARY},
    ...
    {SN_PrefetchTemporalAllCacheLevels, 0, SIMD_VERSION_SSE1,
     SIMD_EMIT_PREFETCH, SIMD_PREFETCH_MODE_0},
    {SN_PrefetchTemporal1stLevelCache, 0, SIMD_VERSION_SSE1, SIMD_EMIT_PREFETCH,
     SIMD_PREFETCH_MODE_1},
    {SN_PrefetchTemporal2ndLevelCache, 0, SIMD_VERSION_SSE1, SIMD_EMIT_PREFETCH,
     SIMD_PREFETCH_MODE_2},
    {SN_PrefetchNonTemporal, 0, SIMD_VERSION_SSE1, SIMD_EMIT_PREFETCH,
    ...
    {SN_StoreAligned, OP_STOREX_ALIGNED_MEMBASE_REG, SIMD_VERSION_SSE1,
     SIMD_EMIT_STORE},
    {SN_StoreNonTemporal, OP_STOREX_NTA_MEMBASE_REG, SIMD_VERSION_SSE1,
     SIMD_EMIT_STORE},
    ...
    {SN_op_Equality, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_EQUALITY,
     SIMD_COMP_EQ},
    {SN_op_ExclusiveOr, OP_XORPS, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_Explicit, 0, SIMD_VERSION_SSE1, SIMD_EMIT_CAST},
    {SN_op_Inequality, OP_COMPPS, SIMD_VERSION_SSE1, SIMD_EMIT_EQUALITY,
     SIMD_COMP_NEQ},
...
static const SimdIntrinsic vector2d_intrinsics[] = {
    ...
    {SN_CompareEqual, OP_COMPPD, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_EQ},
    {SN_CompareLessEqual, OP_COMPPD, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_LE},
    {SN_CompareLessThan, OP_COMPPD, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_LT},
    {SN_CompareNotEqual, OP_COMPPD, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_NEQ},
    {SN_CompareNotLessEqual, OP_COMPPD, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_NLE},
    {SN_CompareNotLessThan, OP_COMPPD, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_NLT},
    {SN_CompareOrdered, OP_COMPPD, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_ORD},
    {SN_CompareUnordered, OP_COMPPD, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY,
     SIMD_COMP_UNORD},
    ...
    {SN_ConvertToIntTruncated, OP_CVTTPD2DQ, SIMD_VERSION_SSE2,
     SIMD_EMIT_UNARY},
...
static const SimdIntrinsic vector4ui_intrinsics[] = {
    ...
    {SN_PrefetchNonTemporal, 0, SIMD_VERSION_SSE1, SIMD_EMIT_PREFETCH,
     SIMD_PREFETCH_MODE_NTA},
    {SN_Shuffle, OP_PSHUFLED, SIMD_VERSION_SSE1, SIMD_EMIT_SHUFFLE},
    {SN_SignedPackWithSignedSaturation, OP_PACKD, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    {SN_SignedPackWithUnsignedSaturation, OP_PACKD_UN, SIMD_VERSION_SSE41,
     SIMD_EMIT_BINARY},
    {SN_StoreAligned, OP_STOREX_ALIGNED_MEMBASE_REG, SIMD_VERSION_SSE1,
     SIMD_EMIT_STORE},
    ...
    {SN_op_BitwiseAnd, OP_PAND, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_BitwiseOr, OP_POR, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_Equality, OP_PCMPEQD, SIMD_VERSION_SSE1, SIMD_EMIT_EQUALITY,
     SIMD_COMP_EQ},
    {SN_op_ExclusiveOr, OP_PXOR, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_Explicit, 0, SIMD_VERSION_SSE1, SIMD_EMIT_CAST},
    {SN_op_Inequality, OP_PCMPEQD, SIMD_VERSION_SSE1, SIMD_EMIT_EQUALITY,
     SIMD_COMP_NEQ},
...
static const SimdIntrinsic vector4i_intrinsics[] = {
    ...
    {SN_PackWithSignedSaturation, OP_PACKD, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    {SN_PackWithUnsignedSaturation, OP_PACKD_UN, SIMD_VERSION_SSE41,
     SIMD_EMIT_BINARY},
    {SN_PrefetchTemporalAllCacheLevels, 0, SIMD_VERSION_SSE1,
     SIMD_EMIT_PREFETCH, SIMD_PREFETCH_MODE_0},
    {SN_PrefetchTemporal1stLevelCache, 0, SIMD_VERSION_SSE1, SIMD_EMIT_PREFETCH,
...
static const SimdIntrinsic vector8us_intrinsics[] = {
    ...
    {SN_AddWithSaturation, OP_PADDW_SAT_UN, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    ...
    {SN_MultiplyStoreHigh, OP_PMULW_HIGH_UN, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    {SN_PrefetchTemporalAllCacheLevels, 0, SIMD_VERSION_SSE1,
     SIMD_EMIT_PREFETCH, SIMD_PREFETCH_MODE_0},
    {SN_PrefetchTemporal1stLevelCache, 0, SIMD_VERSION_SSE1, SIMD_EMIT_PREFETCH,
    ...
    {SN_SignedPackWithSignedSaturation, OP_PACKW, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    {SN_SignedPackWithUnsignedSaturation, OP_PACKW_UN, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    {SN_StoreAligned, OP_STOREX_ALIGNED_MEMBASE_REG, SIMD_VERSION_SSE1,
     SIMD_EMIT_STORE},
    {SN_SubtractWithSaturation, OP_PSUBW_SAT_UN, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    ...
    {SN_op_BitwiseAnd, OP_PAND, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_BitwiseOr, OP_POR, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_Equality, OP_PCMPEQW, SIMD_VERSION_SSE1, SIMD_EMIT_EQUALITY,
     SIMD_COMP_EQ},
    {SN_op_ExclusiveOr, OP_PXOR, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_Explicit, 0, SIMD_VERSION_SSE1, SIMD_EMIT_CAST},
    {SN_op_Inequality, OP_PCMPEQW, SIMD_VERSION_SSE1, SIMD_EMIT_EQUALITY,
     SIMD_COMP_NEQ},
...
static const SimdIntrinsic vector8s_intrinsics[] = {
    ...
    {SN_PackWithSignedSaturation, OP_PACKW, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    {SN_PackWithUnsignedSaturation, OP_PACKW_UN, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    {SN_PrefetchTemporalAllCacheLevels, 0, SIMD_VERSION_SSE1,
     SIMD_EMIT_PREFETCH, SIMD_PREFETCH_MODE_0},
    {SN_PrefetchTemporal1stLevelCache, 0, SIMD_VERSION_SSE1, SIMD_EMIT_PREFETCH,
...
static const SimdIntrinsic vector16b_intrinsics[] = {
    ...
    {SN_AddWithSaturation, OP_PADDB_SAT_UN, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    ...
    {SN_StoreAligned, OP_STOREX_ALIGNED_MEMBASE_REG, SIMD_VERSION_SSE1,
     SIMD_EMIT_STORE},
    {SN_SubtractWithSaturation, OP_PSUBB_SAT_UN, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    {SN_SumOfAbsoluteDifferences, OP_PSUM_ABS_DIFF, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    ...
    {SN_op_BitwiseAnd, OP_PAND, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_BitwiseOr, OP_POR, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_Equality, OP_PCMPEQB, SIMD_VERSION_SSE1, SIMD_EMIT_EQUALITY,
     SIMD_COMP_EQ},
    {SN_op_ExclusiveOr, OP_PXOR, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_op_Explicit, 0, SIMD_VERSION_SSE1, SIMD_EMIT_CAST},
    {SN_op_Inequality, OP_PCMPEQB, SIMD_VERSION_SSE1, SIMD_EMIT_EQUALITY,
     SIMD_COMP_NEQ},
...
static const SimdIntrinsic vector16sb_intrinsics[] = {
    ...
    {SN_StoreAligned, OP_STOREX_ALIGNED_MEMBASE_REG, SIMD_VERSION_SSE1,
     SIMD_EMIT_STORE},
    {SN_SubtractWithSaturation, OP_PSUBB_SAT, SIMD_VERSION_SSE1,
     SIMD_EMIT_BINARY},
    {SN_UnpackHigh, OP_UNPACK_HIGHB, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_UnpackLow, OP_UNPACK_LOWB, SIMD_VERSION_SSE1, SIMD_EMIT_BINARY},
    {SN_get_V0, 0, SIMD_VERSION_SSE1, SIMD_EMIT_GETTER},
...
static MonoInst *emit_sys_numerics_intrinsics(MonoCompile *cfg,
                                              MonoMethod *cmethod,
                                              MonoMethodSignature *fsig,
                                              MonoInst **args);
static MonoInst *emit_sys_numerics_vectors_intrinsics(MonoCompile *cfg,
                                                      MonoMethod *cmethod,
                                                      MonoMethodSignature *fsig,
                                                      MonoInst **args);

/*TODO match using number of parameters as well*/
static int simd_intrinsic_compare_by_name(const void *key, const void *value) {
...
static gboolean apply_vreg_first_block_interference(MonoCompile *cfg,
                                                    MonoInst *ins, int reg,
                                                    int max_vreg,
                                                    char *vreg_flags) {
...
static gboolean apply_vreg_following_block_interference(
    MonoCompile *cfg, MonoInst *ins, int reg, MonoBasicBlock *bb, int max_vreg,
    char *vreg_flags, MonoBasicBlock **target_bb) {
  if (reg == -1 || reg > max_vreg || !(vreg_flags[reg] & VREG_HAS_XZERO_BB0) ||
      target_bb[reg] == bb)
    return FALSE;
  ...
  if (vreg_flags[reg] & VREG_SINGLE_BB_USE) {
    ...
    DEBUG(printf("[simd-simplify] R%d used by many bb: ", reg);
          mono_print_ins(ins));
    return TRUE;
  } else if (!(vreg_flags[reg] & VREG_MANY_BB_USE)) {
    vreg_flags[reg] |= VREG_SINGLE_BB_USE;
    target_bb[reg] = bb;
    DEBUG(printf("[simd-simplify] R%d first used by: ", reg);
          mono_print_ins(ins));
    return TRUE;
  }
  return FALSE;
...
void mono_simd_simplify_indirection(MonoCompile *cfg) {
  ...
  for (i = 0; i < cfg->num_varinfo; i++) {
    MonoInst *var = cfg->varinfo[i];
    if (m_class_is_simd_type(var->klass) &&
        !(var->flags & (MONO_INST_INDIRECT | MONO_INST_VOLATILE))) {
  ...
  for (ins = first_bb->code; ins; ins = ins->next) {
    ...
    if (ins->opcode == OP_XZERO) {
      if (!(vreg_flags[ins->dreg] & VREG_HAS_OTHER_OP_BB0)) {
        DEBUG(printf("[simd-simplify] R%d has vzero: ", ins->dreg);
              mono_print_ins(ins));
    ...
    if (ins->opcode == OP_LDADDR &&
        apply_vreg_first_block_interference(
            cfg, ins, ((MonoInst *)ins->inst_p0)->dreg, max_vreg, vreg_flags))
      continue;
    if (apply_vreg_first_block_interference(cfg, ins, ins->dreg, max_vreg,
                                            vreg_flags))
      continue;
    num_sregs = mono_inst_get_src_registers(ins, sregs);
    for (i = 0; i < num_sregs; ++i) {
      if (apply_vreg_first_block_interference(cfg, ins, sregs[i], max_vreg,
                                              vreg_flags))
        break;
    }
  }
  ...
  */
    ...
    for (ins = bb->code; ins; ins = ins->next) {
      ...
      if (ins->opcode == OP_LDADDR &&
          apply_vreg_following_block_interference(
              cfg, ins, ((MonoInst *)ins->inst_p0)->dreg, bb, max_vreg,
              vreg_flags, target_bb))
        continue;
      if (apply_vreg_following_block_interference(
              cfg, ins, ins->dreg, bb, max_vreg, vreg_flags, target_bb))
        continue;
      num_sregs = mono_inst_get_src_registers(ins, sregs);
      for (i = 0; i < num_sregs; ++i) {
        if (apply_vreg_following_block_interference(
                cfg, ins, sregs[i], bb, max_vreg, vreg_flags, target_bb))
          continue;
      }
    }
  }
  ...
  for (i = 0; i < cfg->num_varinfo; i++) {
    ...
    for (ins = target_bb[var->dreg]->code; ins; ins = ins->next) {
      ...
      /*We can avoid inserting the XZERO if the first use doesn't depend on the
       * zero'ed value.*/
      if (ins->dreg == var->dreg && !found) {
        DEBUG(printf("[simd-simplify] INGORING R%d on BB %d because first op "
                     "is a def",
                     i, target_bb[var->dreg]->block_num););
        break;
      } else if (found) {
        DEBUG(printf("[simd-simplify] Adding XZERO for R%d on BB %d: ", i,
  ...
  for (ins = first_bb->code; ins; ins = ins->next) {
    if (ins->opcode == OP_XZERO &&
        (vreg_flags[ins->dreg] & VREG_SINGLE_BB_USE)) {
      DEBUG(printf("[simd-simplify] Nullify %d on first BB: ", ins->dreg);
            mono_print_ins(ins));
      NULLIFY_INS(ins);
    }
  }
...
/*
 * Windows x64 value type ABI uses reg/stack references
 * (ArgValuetypeAddrInIReg/ArgValuetypeAddrOnStack) for function arguments. When
 * using SIMD intrinsics arguments optimized into OP_ARG needs to be decomposed
 * into correspondig SIMD LOADX/STOREX instructions.
 */
#if defined(TARGET_WIN32) && defined(TARGET_AMD64)
static gboolean decompose_vtype_opt_uses_simd_intrinsics(MonoCompile *cfg,
                                                         MonoInst *ins) {
...
static void decompose_vtype_opt_load_arg(MonoCompile *cfg, MonoBasicBlock *bb,
                                         MonoInst *ins, gint32 *sreg_int32) {
  guint32 *sreg = (guint32 *)sreg_int32;
  MonoInst *src_var = get_vreg_to_inst(cfg, *sreg);
  if (src_var && src_var->opcode == OP_ARG && src_var->klass &&
      MONO_CLASS_IS_SIMD(cfg, src_var->klass)) {
...
void mono_simd_decompose_intrinsic(MonoCompile *cfg, MonoBasicBlock *bb,
  ...
  if (cfg->opt & MONO_OPT_SIMD &&
      decompose_vtype_opt_uses_simd_intrinsics(cfg, ins)) {
    ...
    if (dest_var && dest_var->opcode == OP_ARG && dest_var->klass &&
        MONO_CLASS_IS_SIMD(cfg, dest_var->klass)) {
 ...
 */
static int get_simd_vreg(MonoCompile *cfg, MonoMethod *cmethod, MonoInst *src) {
 ...
 */
static int load_simd_vreg_class(MonoCompile *cfg, MonoClass *klass,
                                MonoInst *src, gboolean *indirect) {
  const char *spec = INS_INFO(src->opcode);
  ...
  g_warning("load_simd_vreg:: could not infer source simd (%d) vreg for op",
            src->type);
  mono_print_ins(src);
  g_assert_not_reached();
}

static int load_simd_vreg(MonoCompile *cfg, MonoMethod *cmethod, MonoInst *src,
                          gboolean *indirect) {
  return load_simd_vreg_class(cfg, cmethod->klass, src, indirect);
}

/*We share the var with fconv_to_r8_x to save some stack space.*/
static MonoInst *get_double_spill_area(MonoCompile *cfg) {
...
static MonoInst *get_simd_ctor_spill_area(MonoCompile *cfg,
                                          MonoClass *avector_klass) {
  if (!cfg->simd_ctor_var) {
    cfg->simd_ctor_var = mono_compile_create_var(
        cfg, m_class_get_byval_arg(avector_klass), OP_LOCAL);
    cfg->simd_ctor_var->flags |=
        MONO_INST_VOLATILE; /*FIXME, use the don't regalloc flag*/
  }
  return cfg->simd_ctor_var;
}

static int mono_type_to_expand_op(MonoType *type) {
  switch (type->type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return OP_EXPAND_I1;
...
static int type_to_comp_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return OP_PCMPEQB;
...
static int type_to_gt_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_I1:
    return OP_PCMPGTB;
  case MONO_TYPE_I2:
...
static int type_to_padd_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_U1:
  case MONO_TYPE_I1:
    return OP_PADDB;
...
static int type_to_psub_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_U1:
  case MONO_TYPE_I1:
    return OP_PSUBB;
...
static int type_to_pmul_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_U2:
  case MONO_TYPE_I2:
    return OP_PMULW;
...
static int type_to_pdiv_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_R4:
    return OP_DIVPS;
  case MONO_TYPE_R8:
...
static int type_to_pxor_op(MonoType *t) {
  /*
...
static int type_to_pand_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_R4:
    return OP_ANDPS;
  case MONO_TYPE_R8:
...
static int type_to_por_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_R4:
    return OP_ORPS;
  case MONO_TYPE_R8:
...
static int type_to_pmin_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_R4:
    return OP_MINPS;
  case MONO_TYPE_R8:
...
static int type_to_pmax_op(MonoType *t) {
  switch (t->type) {
  case MONO_TYPE_R4:
    return OP_MAXPS;
  case MONO_TYPE_R8:
...
static int get_simd_vreg_or_expanded_scalar(MonoCompile *cfg, MonoClass *klass,
                                            MonoType *param_type,
                                            MonoInst *src) {
  MonoInst *ins;
  int expand_op;
 ...
 * @LHS/@RHS are the two arguments, they can be either a SIMD type or a scalar
 * one. Scalar arguments are expanded to the SIMD type.
 */
static MonoInst *simd_intrinsic_emit_binary_op(MonoCompile *cfg, int opcode,
                                               int flags, MonoClass *klass,
                                               MonoType *lhs_type,
                                               MonoType *rhs_type,
                                               MonoInst *lhs, MonoInst *rhs) {
  MonoInst *ins;
...
static MonoInst *simd_intrinsic_emit_binary(const SimdIntrinsic *intrinsic,
                                            MonoCompile *cfg,
                                            MonoMethod *cmethod,
                                            MonoInst **args) {
  ...
  return simd_intrinsic_emit_binary_op(cfg, intrinsic->opcode, intrinsic->flags,
                                       cmethod->klass, sig->params[0],
                                       sig->params[1], args[0], args[1]);
}

static MonoInst *simd_intrinsic_emit_unary(const SimdIntrinsic *intrinsic,
                                           MonoCompile *cfg,
                                           MonoMethod *cmethod,
                                           MonoInst **args) {
  MonoInst *ins;
...
static int mono_type_to_extract_op(MonoCompile *cfg, MonoType *type) {
  switch (type->type) {
  case MONO_TYPE_I1:
...
static int mono_type_elements_shift_bits(MonoType *type) {
  switch (type->type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return 2;
...
static G_GNUC_UNUSED int mono_type_to_insert_op(MonoType *type) {
  switch (type->type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return OP_INSERT_I1;
...
static int mono_type_to_slow_insert_op(MonoType *type) {
  switch (type->type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
...
static MonoInst *simd_intrinsic_emit_setter(const SimdIntrinsic *intrinsic,
                                            MonoCompile *cfg,
                                            MonoMethod *cmethod,
                                            MonoInst **args) {
  MonoInst *ins;
  ...
  if (COMPILE_LLVM(cfg)) {
    ...
    ins->klass = cmethod->klass;
    ins->dreg = ins->sreg1 = dreg =
        load_simd_vreg(cfg, cmethod, args[0], &indirect);
    ...
    ins->klass = cmethod->klass;
    /*This is a partial load so we encode the dependency on the previous value
     * by setting dreg and sreg1 to the same value.*/
    ins->dreg = ins->sreg1 = dreg =
        load_simd_vreg(cfg, cmethod, args[0], &indirect);
    ins->sreg2 = args[1]->dreg;
    ins->inst_c0 = intrinsic->opcode;
 ...
 */
static MonoInst *simd_intrinsic_emit_getter_op(MonoCompile *cfg, int index,
                                               MonoClass *klass, MonoType *type,
                                               MonoInst *arg) {
  MonoInst *ins;
  int vreg, shift_bits;
...
static MonoInst *simd_intrinsic_emit_getter(const SimdIntrinsic *intrinsic,
                                            MonoCompile *cfg,
                                            MonoMethod *cmethod,
                                            MonoInst **args) {
  MonoMethodSignature *sig = mono_method_signature_internal(cmethod);

  return simd_intrinsic_emit_getter_op(cfg, intrinsic->opcode, cmethod->klass,
                                       sig->ret, args[0]);
}

static MonoInst *simd_intrinsic_emit_long_getter(const SimdIntrinsic *intrinsic,
                                                 MonoCompile *cfg,
                                                 MonoMethod *cmethod,
                                                 MonoInst **args) {
  MonoInst *ins;
  int vreg;
  gboolean is_r8 =
      mono_method_signature_internal(cmethod)->ret->type == MONO_TYPE_R8;
...
static MonoInst *simd_intrinsic_emit_ctor(const SimdIntrinsic *intrinsic,
                                          MonoCompile *cfg, MonoMethod *cmethod,
                                          MonoInst **args) {
  MonoInst *ins = NULL;
  int i, addr_reg;
  gboolean is_ldaddr =
      (args[0]->opcode == OP_LDADDR && args[0]->inst_left->opcode != OP_ARG);
  MonoMethodSignature *sig = mono_method_signature_internal(cmethod);
  ...
  if (is_ldaddr) {
    NEW_VARLOADA(cfg, ins, get_simd_ctor_spill_area(cfg, cmethod->klass),
                 &cmethod->klass->byref_arg);
    MONO_ADD_INS(cfg->cbb, ins);
    addr_reg = ins->dreg;
  } else {
    g_assert(args[0]->type == STACK_MP || args[0]->type == STACK_PTR);
  ...
  for (i = sig->param_count - 1; i >= 0; --i) {
    EMIT_NEW_STORE_MEMBASE(cfg, ins, store_op, addr_reg, i * arg_size,
                           args[i + 1]->dreg);
  }
  ...
  if (sig->param_count * arg_size < 16) {
    ...
    for (i = sig->param_count; i < 16 / arg_size; ++i) {
      switch (arg_size) {
      case 4:
        MONO_EMIT_NEW_STORE_MEMBASE_IMM(cfg, OP_STOREI4_MEMBASE_IMM, addr_reg,
                                        i * arg_size, 0);
        break;
      default:
        g_assert_not_reached();
...
static MonoInst *simd_intrinsic_emit_cast(const SimdIntrinsic *intrinsic,
                                          MonoCompile *cfg, MonoMethod *cmethod,
                                          MonoInst **args) {
  MonoInst *ins;
  MonoClass *klass;
  ...
  if (cmethod->is_inflated)
    ...
    klass = mono_class_from_mono_type_internal(
        mono_method_signature_internal(cmethod)->ret);
  else
    klass = cmethod->klass;
...
static MonoInst *simd_intrinsic_emit_shift(const SimdIntrinsic *intrinsic,
                                           MonoCompile *cfg,
                                           MonoMethod *cmethod,
                                           MonoInst **args) {
  MonoInst *ins;
...
static gboolean mono_op_is_packed_compare(int op) {
  return op >= OP_PCMPEQB && op <= OP_PCMPEQQ;
}

static MonoInst *simd_intrinsic_emit_equality_op(MonoCompile *cfg,
                                                 MonoMethod *cmethod,
                                                 MonoInst **args, int opcode,
                                                 int flags) {
  MonoInst *ins;
  ...
  /*FP ops have a not equal instruction, which means that we must test the
   * results with OR semantics.*/
  if (mono_op_is_packed_compare(opcode) || flags == SIMD_COMP_EQ) {
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, tmp_vreg, 0xFFFF);
    NEW_UNALU(cfg, ins, flags == SIMD_COMP_EQ ? OP_CEQ : OP_CLT_UN, tmp_vreg,
              -1);
  } else {
...
static MonoInst *simd_intrinsic_emit_equality(const SimdIntrinsic *intrinsic,
                                              MonoCompile *cfg,
                                              MonoMethod *cmethod,
                                              MonoInst **args) {
  return simd_intrinsic_emit_equality_op(cfg, cmethod, args, intrinsic->opcode,
                                         intrinsic->flags);
}

static MonoInst *simd_intrinsic_emit_shuffle(const SimdIntrinsic *intrinsic,
                                             MonoCompile *cfg,
                                             MonoMethod *cmethod,
                                             MonoInst **args) {
  MonoInst *ins;
...
static MonoInst *
simd_intrinsic_emit_load_aligned(const SimdIntrinsic *intrinsic,
                                 MonoCompile *cfg, MonoMethod *cmethod,
                                 MonoInst **args) {
  MonoInst *ins;
...
static MonoInst *simd_intrinsic_emit_store(const SimdIntrinsic *intrinsic,
                                           MonoCompile *cfg,
                                           MonoMethod *cmethod,
                                           MonoInst **args) {
  MonoInst *ins;
...
static MonoInst *
simd_intrinsic_emit_extract_mask(const SimdIntrinsic *intrinsic,
                                 MonoCompile *cfg, MonoMethod *cmethod,
                                 MonoInst **args) {
  MonoInst *ins;
  int vreg;
...
static MonoInst *simd_intrinsic_emit_prefetch(const SimdIntrinsic *intrinsic,
                                              MonoCompile *cfg,
                                              MonoMethod *cmethod,
                                              MonoInst **args) {
  MonoInst *ins;
...
static MonoInst *simd_intrinsic_emit_const(const SimdIntrinsic *intrinsic,
                                           MonoCompile *cfg,
                                           MonoMethod *cmethod,
                                           MonoInst **args) {
  MonoInst *ins;
...
static const char *simd_version_name(guint32 version) {
  switch (version) {
...
static MonoInst *emit_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
                                 MonoMethodSignature *fsig, MonoInst **args,
                                 const SimdIntrinsic *intrinsics,
                                 guint32 size) {
  const SimdIntrinsic *result = (const SimdIntrinsic *)mono_binary_search(
      cmethod->name, intrinsics, size, sizeof(SimdIntrinsic),
      &simd_intrinsic_compare_by_name);
  if (!result) {
    DEBUG(printf("function doesn't have a simd intrinsic %s::%s/%d\n",
                 m_class_get_name(cmethod->klass), cmethod->name,
                 fsig->param_count));
    return NULL;
  }
  if (IS_DEBUG_ON(cfg)) {
  ...
  if (result->simd_version_flags &&
      !(result->simd_version_flags & simd_supported_versions)) {
    if (IS_DEBUG_ON(cfg)) {
      int x;
      printf("function %s::%s/%d requires one of unsuported SIMD instruction "
             "set(s): ",
             m_class_get_name(cmethod->klass), cmethod->name,
             fsig->param_count);
...
static int mono_emit_vector_ldelema(MonoCompile *cfg, MonoType *array_type,
  ...
  if (check_bounds) {
    ...
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_PADD_IMM, index3_reg, index2_reg,
                            16 / size - 1);
...
static MonoInst *emit_array_extension_intrinsics(MonoCompile *cfg,
                                                 MonoMethod *cmethod,
                                                 MonoMethodSignature *fsig,
                                                 MonoInst **args) {
  if ((!strcmp("GetVector", cmethod->name) ||
       !strcmp("GetVectorAligned", cmethod->name)) &&
      fsig->param_count == 2) {
    MonoInst *load;
    int addr =
        mono_emit_vector_ldelema(cfg, fsig->params[0], args[0], args[1], TRUE);
  ...
  if ((!strcmp("SetVector", cmethod->name) ||
       !strcmp("SetVectorAligned", cmethod->name)) &&
      fsig->param_count == 3) {
    MonoInst *store;
    int vreg = get_simd_vreg(cfg, cmethod, args[1]);
    int addr =
        mono_emit_vector_ldelema(cfg, fsig->params[0], args[0], args[2], TRUE);

    MONO_INST_NEW(cfg, store,
                  !strcmp("SetVectorAligned", cmethod->name)
  ...
  if (!strcmp("IsAligned", cmethod->name) && fsig->param_count == 2) {
    MonoInst *ins;
    int addr =
        mono_emit_vector_ldelema(cfg, fsig->params[0], args[0], args[1], FALSE);
...
static MonoInst *emit_simd_runtime_intrinsics(MonoCompile *cfg,
                                              MonoMethod *cmethod,
                                              MonoMethodSignature *fsig,
                                              MonoInst **args) {
...
static gboolean is_sys_numerics_assembly(MonoAssembly *assembly) {
  return !strcmp("System.Numerics", assembly->aname.name);
}

static gboolean is_sys_numerics_vectors_assembly(MonoAssembly *assembly) {
...
MonoInst *mono_emit_simd_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  if (is_sys_numerics_vectors_assembly(
          m_class_get_image(cmethod->klass)->assembly)) {
    simd_inst = emit_sys_numerics_vectors_intrinsics(cfg, cmethod, fsig, args);
    goto on_exit;
  }

  if (strcmp("Mono.Simd",
             m_class_get_image(cmethod->klass)->assembly->aname.name) ||
  ...
  if (!strcmp("VectorOperations", class_name)) {
    if (!(cmethod->flags & METHOD_ATTRIBUTE_STATIC))
      goto on_exit;
    class_name = m_class_get_name(mono_class_from_mono_type_internal(
        mono_method_signature_internal(cmethod)->params[0]));
  } else if (!m_class_is_simd_type(cmethod->klass))
    goto on_exit;

  cfg->uses_simd_intrinsics |=
      MONO_CFG_USES_SIMD_INTRINSICS_SIMPLIFY_INDIRECTION;
  if (!strcmp("Vector2d", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector2d_intrinsics,
                        sizeof(vector2d_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
  if (!strcmp("Vector4f", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector4f_intrinsics,
                        sizeof(vector4f_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
  if (!strcmp("Vector2ul", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector2ul_intrinsics,
                        sizeof(vector2ul_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
  if (!strcmp("Vector2l", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector2l_intrinsics,
                        sizeof(vector2l_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
  if (!strcmp("Vector4ui", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector4ui_intrinsics,
                        sizeof(vector4ui_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
  if (!strcmp("Vector4i", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector4i_intrinsics,
                        sizeof(vector4i_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
  if (!strcmp("Vector8us", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector8us_intrinsics,
                        sizeof(vector8us_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
  if (!strcmp("Vector8s", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector8s_intrinsics,
                        sizeof(vector8s_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
  if (!strcmp("Vector16b", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector16b_intrinsics,
                        sizeof(vector16b_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
  if (!strcmp("Vector16sb", class_name)) {
    simd_inst =
        emit_intrinsics(cfg, cmethod, fsig, args, vector16sb_intrinsics,
                        sizeof(vector16sb_intrinsics) / sizeof(SimdIntrinsic));
    goto on_exit;
  }
...
static void assert_handled(MonoCompile *cfg, MonoMethod *method) {
    ...
    if (cattr) {
      ...
      for (int i = 0; i < cattr->num_attrs; ++i)
        if (cattr->attrs[i].ctor &&
            (!strcmp(m_class_get_name(cattr->attrs[i].ctor->klass),
...
static MonoInst *emit_vector_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
                                        MonoMethodSignature *fsig,
                                        MonoInst **args) {
  ...
  /*
   * Vector2/3/4 are handled the same way, since the underlying SIMD type is the
   * same (4 * r4).
   */
  intrins = (const SimdIntrinsic *)mono_binary_search(
      cmethod->name, vector2_intrinsics,
      sizeof(vector2_intrinsics) / sizeof(SimdIntrinsic), sizeof(SimdIntrinsic),
      &simd_intrinsic_compare_by_name);
  if (!intrins) {
  ...
  case SN_Equals:
    if (!(fsig->param_count == 1 && fsig->ret->type == MONO_TYPE_BOOLEAN &&
          fsig->params[0] == type))
      break;
    return simd_intrinsic_emit_equality(intrins, cfg, cmethod, args);
  case SN_SquareRoot:
    if (!(fsig->param_count == 1 && fsig->ret == type &&
          fsig->params[0] == type))
      break;
    return simd_intrinsic_emit_unary(intrins, cfg, cmethod, args);
  case SN_Dot:
    if (!(fsig->param_count == 2 && fsig->ret->type == MONO_TYPE_R4 &&
    ...
    if (COMPILE_LLVM(cfg)) {
      ...
      return simd_intrinsic_emit_getter_op(
          cfg, 0, cmethod->klass, mono_method_signature_internal(cmethod)->ret,
          ins);
    }
    break;
  case SN_Abs: {
    ...
    sub = simd_intrinsic_emit_binary_op(cfg, OP_SUBPS, 0, cmethod->klass,
                                        sig->params[0], sig->params[0], zero,
                                        args[0]);
    return simd_intrinsic_emit_binary_op(cfg, OP_MAXPS, 0, cmethod->klass,
                                         sig->params[0], sig->params[0],
                                         args[0], sub);
  }
  case SN_Max:
  case SN_Min:
  ...
  case SN_op_Subtraction:
    if (!(fsig->param_count == 2 && fsig->ret == type &&
          (fsig->params[0] == type || fsig->params[0]->type == MONO_TYPE_R4) &&
          (fsig->params[1] == type || fsig->params[1]->type == MONO_TYPE_R4)))
      break;
...
static const SimdIntrinsic vector_t_intrinsics[] = {{SN_ctor},
                                                    {SN_Abs},
                                                    {SN_CopyTo},
                                                    {SN_Equals},
                                                    {SN_GreaterThan},
...
static MonoInst *emit_vector_t_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
                                          MonoMethodSignature *fsig,
                                          MonoInst **args) {
  const SimdIntrinsic *intrins;
  MonoType *type, *etype;
  ...
  intrins = (const SimdIntrinsic *)mono_binary_search(
      cmethod->name, vector_t_intrinsics,
      sizeof(vector_t_intrinsics) / sizeof(SimdIntrinsic),
      sizeof(SimdIntrinsic), &simd_intrinsic_compare_by_name);
  if (!intrins) {
    assert_handled(cfg, cmethod);
    return NULL;
  }
  ...
  case SN_get_Item:
    ...
    if (index < 0 || index >= len)
      return NULL;
    return simd_intrinsic_emit_getter_op(cfg, index, cmethod->klass, etype,
                                         args[0]);
  case SN_ctor:
    if (fsig->param_count == 1 &&
        mono_metadata_type_equal(fsig->params[0], etype))
      return simd_intrinsic_emit_ctor(NULL, cfg, cmethod, args);
    if ((fsig->param_count == 1 || fsig->param_count == 2) &&
        (fsig->params[0]->type == MONO_TYPE_SZARRAY)) {
      ...
      EMIT_NEW_BIALU_IMM(cfg, ins, OP_IADD_IMM, end_index_reg, index_ins->dreg,
                         len - 1);
      MONO_EMIT_BOUNDS_CHECK(cfg, array_ins->dreg, MonoArray, max_length,
                             end_index_reg);

      /* Load the array slice into the simd reg */
      ldelema_ins = mini_emit_ldelema_1_ins(
          cfg, mono_class_from_mono_type_internal(etype), array_ins, index_ins,
          TRUE);
      g_assert(args[0]->opcode == OP_LDADDR);
      var = (MonoInst *)args[0]->inst_p0;
      EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOADX_MEMBASE, var->dreg,
                            ldelema_ins->dreg, 0);
      ins->klass = cmethod->klass;
      return args[0];
    }
  ...
  case SN_Equals:
    if (fsig->param_count == 1 && fsig->ret->type == MONO_TYPE_BOOLEAN &&
      ...
      return simd_intrinsic_emit_equality_op(
          cfg, cmethod, args, type_to_comp_op(etype), SIMD_COMP_EQ);
    if (fsig->param_count == 2 && mono_metadata_type_equal(fsig->ret, type) &&
        mono_metadata_type_equal(fsig->params[0], type) &&
        mono_metadata_type_equal(fsig->params[1], type))
      return simd_intrinsic_emit_binary_op(cfg, type_to_comp_op(etype), 0,
                                           cmethod->klass, fsig->params[0],
                                           fsig->params[1], args[0], args[1]);
    break;
  ...
  case SN_LessThanOrEqual: {
    ...
    switch (intrins->name) {
    case SN_GreaterThan:
      return simd_intrinsic_emit_binary_op(cfg, gt_op, 0, cmethod->klass,
                                           fsig->params[0], fsig->params[1],
                                           args[0], args[1]);
    case SN_LessThan:
      return simd_intrinsic_emit_binary_op(cfg, gt_op, 0, cmethod->klass,
                                           fsig->params[0], fsig->params[1],
                                           args[1], args[0]);
    case SN_LessThanOrEqual:
      cmp1 = simd_intrinsic_emit_binary_op(cfg, eq_op, 0, cmethod->klass,
                                           fsig->params[0], fsig->params[1],
                                           args[1], args[0]);
      cmp2 = simd_intrinsic_emit_binary_op(cfg, gt_op, 0, cmethod->klass,
                                           fsig->params[0], fsig->params[1],
                                           args[1], args[0]);
      return simd_intrinsic_emit_binary_op(cfg, OP_POR, 0, cmethod->klass,
                                           fsig->params[0], fsig->params[1],
                                           cmp1, cmp2);
    case SN_GreaterThanOrEqual:
      cmp1 = simd_intrinsic_emit_binary_op(cfg, eq_op, 0, cmethod->klass,
  ...
  case SN_Min: {
    if (!(fsig->param_count == 2 &&
          mono_metadata_type_equal(fsig->ret, fsig->params[0]) &&
    ...
    if (op != -1)
      return simd_intrinsic_emit_binary_op(cfg, op, 0, cmethod->klass,
                                           fsig->params[0], fsig->params[0],
                                           args[0], args[1]);
    break;
  }
  case SN_CopyTo: {
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE_OP_FAULT(
        cfg, OP_LOADI4_MEMBASE, length_reg, array_ins->dreg,
    ...
    ldelema_ins =
        mini_emit_ldelema_1_ins(cfg, mono_class_from_mono_type_internal(etype),
                                array_ins, index_ins, TRUE);
    g_assert(args[0]->opcode == OP_LDADDR);
    var = (MonoInst *)args[0]->inst_p0;
    EMIT_NEW_STORE_MEMBASE(cfg, ins, OP_STOREX_MEMBASE, ldelema_ins->dreg, 0,
                           var->dreg);
    ins->klass = cmethod->klass;
    return args[0];
    break;
...
MonoInst *mono_emit_simd_field_load(MonoCompile *cfg, MonoClassField *field,
  ...
  if (is_sys_numerics_assembly(m_class_get_image(field->parent)->assembly)) {
    ...
    if (!strcmp(parent_name, "Vector2") || !strcmp(parent_name, "Vector3") ||
    ...
    if (index != -1) {
      ...
      simd_inst = simd_intrinsic_emit_getter_op(
          cfg, index, field->parent, mono_field_get_type_internal(field), addr);
      goto on_exit;
    }
  }
