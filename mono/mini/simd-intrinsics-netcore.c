...
enum {
#define METHOD(name)                                                           \
  SN_##name = offsetof(struct msgstr_t, MSGSTRFIELD(__LINE__)),
#define METHOD2(str, name)                                                     \
  SN_##name = offsetof(struct msgstr_t, MSGSTRFIELD(__LINE__)),
#include "simd-methods-netcore.h"
};
...
static int simd_intrinsic_info_compare_by_name(const void *key,
                                               const void *value) {
  SimdIntrinsic *info = (SimdIntrinsic *)value;
  return strcmp((const char *)key, method_name(info->id));
}

static int lookup_intrins(guint16 *intrinsics, int size, MonoMethod *cmethod) {
  const guint16 *result = (const guint16 *)mono_binary_search(
      cmethod->name, intrinsics, size / sizeof(guint16), sizeof(guint16),
...
static SimdIntrinsic *lookup_intrins_info(SimdIntrinsic *intrinsics, int size,
                                          MonoMethod *cmethod) {
#if 0
  ...
  return (SimdIntrinsic *)mono_binary_search(
      cmethod->name, intrinsics, size / sizeof(SimdIntrinsic),
      sizeof(SimdIntrinsic), &simd_intrinsic_info_compare_by_name);
}
...
static MonoInst *emit_simd_ins(MonoCompile *cfg, MonoClass *klass, int opcode,
                               int sreg1, int sreg2) {
...
static MonoInst *emit_simd_ins_for_sig(MonoCompile *cfg, MonoClass *klass,
                                       int opcode, int instc0, int instc1,
...
static gboolean is_hw_intrinsics_class(MonoClass *klass, const char *name,
                                       gboolean *is_64bit) {
  const char *class_name = m_class_get_name(klass);
  if ((!strcmp(class_name, "X64") || !strcmp(class_name, "Arm64")) &&
...
static MonoTypeEnum get_underlying_type(MonoType *type) {
  ...
  if (type->type == MONO_TYPE_PTR) // e.g. int* => MONO_TYPE_I4
    return m_class_get_byval_arg(m_class_get_element_class(klass))->type;
           ...
           MONO_TYPE_GENERICINST) // e.g. Vector128<int> => MONO_TYPE_I4
    return mono_class_get_context(klass)->class_inst->type_argv[0]->type;
  else
    return type->type;
}

static MonoInst *emit_xcompare(MonoCompile *cfg, MonoClass *klass,
                               MonoTypeEnum etype, MonoInst *arg1,
                               MonoInst *arg2) {
  MonoInst *ins;
  gboolean is_fp = etype == MONO_TYPE_R4 || etype == MONO_TYPE_R8;

  ins = emit_simd_ins(cfg, klass, is_fp ? OP_XCOMPARE_FP : OP_XCOMPARE,
                      arg1->dreg, arg2->dreg);
  ins->inst_c0 = CMP_EQ;
  ins->inst_c1 = etype;
  return ins;
...
static int type_to_expand_op(MonoType *type) {
  switch (type->type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return OP_EXPAND_I1;
...
static guint16 vector_methods[] = {
    SN_ConvertToDouble, SN_ConvertToInt32,  SN_ConvertToInt64,
    SN_ConvertToSingle, SN_ConvertToUInt32, SN_ConvertToUInt64,
    SN_Narrow,          SN_Widen,           SN_get_IsHardwareAccelerated,
};

static MonoInst *emit_sys_numerics_vector(MonoCompile *cfg, MonoMethod *cmethod,
                                          MonoMethodSignature *fsig,
                                          MonoInst **args) {
  MonoInst *ins;
  ...
  case SN_ConvertToInt32:
    ...
    return emit_simd_ins(cfg, mono_class_from_mono_type_internal(fsig->ret),
                         OP_CVTPS2DQ, args[0]->dreg, -1);
...
static guint16 vector_t_methods[] = {SN_ctor,
                                     SN_CopyTo,
                                     SN_Equals,
                                     SN_GreaterThan,
...
static MonoInst *emit_sys_numerics_vector_t(MonoCompile *cfg,
                                            MonoMethod *cmethod,
                                            MonoMethodSignature *fsig,
                                            MonoInst **args) {
  MonoInst *ins;
  ...
  case SN_get_Zero:
    g_assert(fsig->param_count == 0 &&
             mono_metadata_type_equal(fsig->ret, type));
  ...
  case SN_CopyTo:
    if ((fsig->param_count == 1 || fsig->param_count == 2) &&
      ...
      mini_emit_bounds_check_offset(
          cfg, array_ins->dreg, MONO_STRUCT_OFFSET(MonoArray, max_length),
          index_ins->dreg, "ArgumentOutOfRangeException");
      end_index_reg = alloc_ireg(cfg);
      int len_reg = alloc_ireg(cfg);
      MONO_EMIT_NEW_LOAD_MEMBASE_OP_FLAGS(
          cfg, OP_LOADI4_MEMBASE, len_reg, array_ins->dreg,
          MONO_STRUCT_OFFSET(MonoArray, max_length), MONO_INST_INVARIANT_LOAD);
      EMIT_NEW_BIALU(cfg, ins, OP_ISUB, end_index_reg, len_reg,
                     index_ins->dreg);
  ...
  case SN_LessThanOrEqual:
    g_assert(fsig->param_count == 2 &&
             ...
             mono_metadata_type_equal(fsig->params[0], type) &&
             mono_metadata_type_equal(fsig->params[1], type));
    is_unsigned = etype->type == MONO_TYPE_U1 || etype->type == MONO_TYPE_U2 ||
                  etype->type == MONO_TYPE_U4 || etype->type == MONO_TYPE_U8;
...
static MonoInst *emit_invalid_operation(MonoCompile *cfg, const char *message) {
  mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);
  mono_error_set_generic_error(cfg->error, "System",
                               "InvalidOperationException", "%s", message);
  return NULL;
}

#ifdef TARGET_ARM64

static SimdIntrinsic armbase_methods[] = {{SN_LeadingSignCount},
                                          {SN_LeadingZeroCount},
                                          {SN_ReverseElementBits},
                                          {SN_get_IsSupported}};

static SimdIntrinsic crc32_methods[] = {
    {SN_ComputeCrc32}, {SN_ComputeCrc32C}, {SN_get_IsSupported}};

static MonoInst *emit_arm64_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
                                       MonoMethodSignature *fsig,
                                       MonoInst **args) {
  ...
  MonoTypeEnum arg0_type = fsig->param_count > 0
                               ? get_underlying_type(fsig->params[0])
                               : MONO_TYPE_VOID;
  gboolean arg0_i32 =
      (arg0_type == MONO_TYPE_I4) || (arg0_type == MONO_TYPE_U4);
  SimdIntrinsic *info;

  if (is_hw_intrinsics_class(klass, "ArmBase", &is_64bit)) {
    info =
        lookup_intrins_info(armbase_methods, sizeof(armbase_methods), cmethod);
    if (!info)
      return NULL;
    ...
    case SN_LeadingZeroCount:
      return emit_simd_ins_for_sig(cfg, klass,
                                   arg0_i32 ? OP_LZCNT32 : OP_LZCNT64, 0,
                                   arg0_type, fsig, args);
    case SN_LeadingSignCount:
      return emit_simd_ins_for_sig(cfg, klass,
                                   arg0_i32 ? OP_LSCNT32 : OP_LSCNT64, 0,
                                   arg0_type, fsig, args);
    case SN_ReverseElementBits:
      return emit_simd_ins_for_sig(
          cfg, klass, (is_64bit ? OP_XOP_I8_I8 : OP_XOP_I4_I4),
          (is_64bit ? SIMD_OP_ARM64_RBIT64 : SIMD_OP_ARM64_RBIT32), arg0_type,
          fsig, args);
    default:
      g_assert_not_reached(); // if a new API is added we need to either
                              // implement it or change IsSupported to false
    }
  }
  ...
  if (is_hw_intrinsics_class(klass, "Crc32", &is_64bit)) {
    ...
    case SN_ComputeCrc32C: {
      ...
      case MONO_TYPE_U1:
        op = is_c ? SIMD_OP_ARM64_CRC32CB : SIMD_OP_ARM64_CRC32B;
        break;
      case MONO_TYPE_U2:
        op = is_c ? SIMD_OP_ARM64_CRC32CH : SIMD_OP_ARM64_CRC32H;
        break;
      case MONO_TYPE_U4:
        op = is_c ? SIMD_OP_ARM64_CRC32CW : SIMD_OP_ARM64_CRC32W;
        break;
      case MONO_TYPE_U8:
        op = is_c ? SIMD_OP_ARM64_CRC32CX : SIMD_OP_ARM64_CRC32X;
        break;
      default:
        g_assert_not_reached();
        break;
      ...
      return emit_simd_ins_for_sig(cfg, klass,
                                   is_64bit ? OP_XOP_I4_I4_I8 : OP_XOP_I4_I4_I4,
                                   op, arg0_type, fsig, args);
    }
    default:
      g_assert_not_reached(); // if a new API is added we need to either
                              // implement it or change IsSupported to false
...
static SimdIntrinsic pclmulqdq_methods[] = {{SN_CarrylessMultiply},
                                            {SN_get_IsSupported}};
...
static SimdIntrinsic lzcnt_methods[] = {{SN_LeadingZeroCount},
                                        {SN_get_IsSupported}};

static SimdIntrinsic bmi1_methods[] = {{SN_AndNot},
                                       {SN_BitFieldExtract},
...
static SimdIntrinsic bmi2_methods[] = {{SN_MultiplyNoFlags},
                                       {SN_ParallelBitDeposit},
                                       {SN_ParallelBitExtract},
                                       {SN_ZeroHighBits},
                                       {SN_get_IsSupported}};

static MonoInst *emit_x86_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
                                     MonoMethodSignature *fsig,
                                     MonoInst **args) {
  MonoInst *ins;
  gboolean supported, is_64bit;
  ...
  if (is_hw_intrinsics_class(klass, "Sse", &is_64bit)) {
    ...
    /* Common case */
    if (info->op != 0)
      return emit_simd_ins_for_sig(cfg, klass, info->op, info->instc0,
                                   arg0_type, fsig, args);
    ...
    case SN_Shuffle:
      if (args[2]->opcode == OP_ICONST)
        return emit_simd_ins_for_sig(cfg, klass, OP_SSE_SHUFFLE,
                                     args[2]->inst_c0, arg0_type, fsig, args);
      // FIXME: handle non-constant mask (generate a switch)
      return emit_invalid_operation(cfg,
  ...
  if (is_hw_intrinsics_class(klass, "Sse2", &is_64bit)) {
    ...
    case SN_Subtract:
      return emit_simd_ins_for_sig(
          cfg, klass, OP_XBINOP, arg0_type == MONO_TYPE_R8 ? OP_FSUB : OP_ISUB,
          arg0_type, fsig, args);
    case SN_Add:
      return emit_simd_ins_for_sig(
          cfg, klass, OP_XBINOP, arg0_type == MONO_TYPE_R8 ? OP_FADD : OP_IADD,
          arg0_type, fsig, args);
    case SN_Average:
      if (arg0_type == MONO_TYPE_U1)
        return emit_simd_ins_for_sig(cfg, klass, OP_PAVGB_UN, -1, arg0_type,
                                     fsig, args);
      else if (arg0_type == MONO_TYPE_U2)
        return emit_simd_ins_for_sig(cfg, klass, OP_PAVGW_UN, -1, arg0_type,
                                     fsig, args);
      else
        return NULL;
    case SN_CompareNotEqual:
      return emit_simd_ins_for_sig(
          cfg, klass, arg0_type == MONO_TYPE_R8 ? OP_XCOMPARE_FP : OP_XCOMPARE,
          CMP_NE, arg0_type, fsig, args);
    case SN_CompareEqual:
      return emit_simd_ins_for_sig(
          cfg, klass, arg0_type == MONO_TYPE_R8 ? OP_XCOMPARE_FP : OP_XCOMPARE,
          CMP_EQ, arg0_type, fsig, args);
    case SN_CompareGreaterThan:
      return emit_simd_ins_for_sig(
          cfg, klass, arg0_type == MONO_TYPE_R8 ? OP_XCOMPARE_FP : OP_XCOMPARE,
          CMP_GT, arg0_type, fsig, args);
    case SN_CompareLessThan:
      return emit_simd_ins_for_sig(
          cfg, klass, arg0_type == MONO_TYPE_R8 ? OP_XCOMPARE_FP : OP_XCOMPARE,
          CMP_LT, arg0_type, fsig, args);
    case SN_ConvertToInt32:
      if (arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_I4_X,
                                     SIMD_OP_SSE_CVTSD2SI, arg0_type, fsig,
                                     args);
      else if (arg0_type == MONO_TYPE_I4)
        return emit_simd_ins_for_sig(cfg, klass, OP_EXTRACT_I4, 0, arg0_type,
                                     fsig, args);
      else
        return NULL;
    case SN_ConvertToInt64:
      if (arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_I8_X,
                                     SIMD_OP_SSE_CVTSD2SI64, arg0_type, fsig,
                                     args);
      else if (arg0_type == MONO_TYPE_I8)
        return emit_simd_ins_for_sig(cfg, klass, OP_EXTRACT_I8,
                                     0 /*element index*/, arg0_type, fsig,
                                     args);
      else
        g_assert_not_reached();
      break;
    ...
    case SN_ConvertScalarToVector128UInt64:
      return emit_simd_ins_for_sig(cfg, klass, OP_CREATE_SCALAR, -1, arg0_type,
                                   fsig, args);
    case SN_ConvertToUInt32:
      return emit_simd_ins_for_sig(cfg, klass, OP_EXTRACT_I4,
                                   0 /*element index*/, arg0_type, fsig, args);
    case SN_ConvertToUInt64:
      return emit_simd_ins_for_sig(cfg, klass, OP_EXTRACT_I8,
                                   0 /*element index*/, arg0_type, fsig, args);
    case SN_ConvertToVector128Double:
      if (arg0_type == MONO_TYPE_R4)
        return emit_simd_ins_for_sig(cfg, klass, OP_CVTPS2PD, 0, arg0_type,
                                     fsig, args);
      else if (arg0_type == MONO_TYPE_I4)
        return emit_simd_ins_for_sig(cfg, klass, OP_CVTDQ2PD, 0, arg0_type,
                                     fsig, args);
      else
        return NULL;
    case SN_ConvertToVector128Int32:
      if (arg0_type == MONO_TYPE_R4)
        return emit_simd_ins_for_sig(cfg, klass, OP_CVTPS2DQ, 0, arg0_type,
                                     fsig, args);
      else if (arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_CVTPD2DQ, 0, arg0_type,
                                     fsig, args);
      else
        return NULL;
    case SN_ConvertToVector128Int32WithTruncation:
      if (arg0_type == MONO_TYPE_R4)
        return emit_simd_ins_for_sig(cfg, klass, OP_CVTTPS2DQ, 0, arg0_type,
                                     fsig, args);
      else if (arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_CVTTPD2DQ, 0, arg0_type,
                                     fsig, args);
      else
        return NULL;
    case SN_ConvertToVector128Single:
      if (arg0_type == MONO_TYPE_I4)
        return emit_simd_ins_for_sig(cfg, klass, OP_CVTDQ2PS, 0, arg0_type,
                                     fsig, args);
      else if (arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_CVTPD2PS, 0, arg0_type,
                                     fsig, args);
      else
        return NULL;
    case SN_LoadAlignedVector128:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_LOADU, 16 /*alignment*/,
                                   arg0_type, fsig, args);
    case SN_LoadVector128:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_LOADU, 1 /*alignment*/,
                                   arg0_type, fsig, args);
    case SN_MoveScalar:
      return emit_simd_ins_for_sig(
          cfg, klass, fsig->param_count == 2 ? OP_SSE_MOVS2 : OP_SSE_MOVS, -1,
          arg0_type, fsig, args);
    case SN_Max:
      switch (arg0_type) {
      case MONO_TYPE_U1:
        return emit_simd_ins_for_sig(cfg, klass, OP_PMAXB_UN, 0, arg0_type,
                                     fsig, args);
      case MONO_TYPE_I2:
        return emit_simd_ins_for_sig(cfg, klass, OP_PMAXW, 0, arg0_type, fsig,
                                     args);
      case MONO_TYPE_R8:
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_MAXPD, arg0_type, fsig, args);
      default:
        g_assert_not_reached();
        break;
      }
    ...
    case SN_Min:
      switch (arg0_type) {
      case MONO_TYPE_U1:
        return emit_simd_ins_for_sig(cfg, klass, OP_PMINB_UN, 0, arg0_type,
                                     fsig, args);
      case MONO_TYPE_I2:
        return emit_simd_ins_for_sig(cfg, klass, OP_PMINW, 0, arg0_type, fsig,
                                     args);
      case MONO_TYPE_R8:
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_MINPD, arg0_type, fsig, args);
      default:
        g_assert_not_reached();
        break;
      }
    ...
    case SN_Multiply:
      if (arg0_type == MONO_TYPE_U4)
        return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_PMULUDQ, 0, arg0_type,
                                     fsig, args);
      else if (arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_MULPD, 0, arg0_type, fsig,
                                     args);
      else
        g_assert_not_reached();
    case SN_MultiplyHigh:
      if (arg0_type == MONO_TYPE_I2)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_PMULHW, arg0_type, fsig, args);
      else if (arg0_type == MONO_TYPE_U2)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_PMULHUW, arg0_type, fsig,
                                     args);
      else
        g_assert_not_reached();
    case SN_PackSignedSaturate:
      if (arg0_type == MONO_TYPE_I2)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_PACKSSWB, arg0_type, fsig,
                                     args);
      else if (arg0_type == MONO_TYPE_I4)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_PACKSSDW, arg0_type, fsig,
                                     args);
      else
        g_assert_not_reached();
    case SN_PackUnsignedSaturate:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_PACKUS, -1, arg0_type,
                                   fsig, args);
    case SN_Extract:
      g_assert(arg0_type == MONO_TYPE_U2);
      return emit_simd_ins_for_sig(cfg, klass, OP_XEXTRACT_I32, arg0_type, 0,
                                   fsig, args);
    case SN_Insert:
      g_assert(arg0_type == MONO_TYPE_I2 || arg0_type == MONO_TYPE_U2);
      return emit_simd_ins_for_sig(cfg, klass, OP_XINSERT_I2, 0, arg0_type,
                                   fsig, args);
    case SN_ShiftRightLogical: {
      ...
      return emit_simd_ins_for_sig(cfg, klass,
                                   is_imm ? OP_XOP_X_X_I4 : OP_XOP_X_X_X, op,
                                   arg0_type, fsig, args);
    }
    ...
    case SN_ShiftLeftLogical128BitLane:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_PSLLDQ, 0, arg0_type,
                                   fsig, args);
    case SN_ShiftRightLogical128BitLane:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_PSRLDQ, 0, arg0_type,
                                   fsig, args);
    case SN_Shuffle: {
      if (fsig->param_count == 2) {
        g_assert(arg0_type == MONO_TYPE_I4 || arg0_type == MONO_TYPE_U4);
        return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_PSHUFD, 0, arg0_type,
                                     fsig, args);
      } else if (fsig->param_count == 3) {
        g_assert(arg0_type == MONO_TYPE_R8);
        return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_SHUFPD, 0, arg0_type,
                                     fsig, args);
      } else {
        g_assert_not_reached();
        break;
    ...
    case SN_ShuffleHigh:
      g_assert(fsig->param_count == 2);
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_PSHUFHW, 0, arg0_type,
                                   fsig, args);
    case SN_ShuffleLow:
      g_assert(fsig->param_count == 2);
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_PSHUFLW, 0, arg0_type,
                                   fsig, args);
    case SN_SqrtScalar: {
      if (fsig->param_count == 1)
        return emit_simd_ins(cfg, klass, OP_SSE2_SQRTSD, args[0]->dreg,
                             args[0]->dreg);
      else if (fsig->param_count == 2)
        return emit_simd_ins(cfg, klass, OP_SSE2_SQRTSD, args[0]->dreg,
                             args[1]->dreg);
  ...
  if (is_hw_intrinsics_class(klass, "Sse3", &is_64bit)) {
    ...
    case SN_AddSubtract:
      if (arg0_type == MONO_TYPE_R4)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_ADDSUBPS, arg0_type, fsig,
                                     args);
      else if (arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_ADDSUBPD, arg0_type, fsig,
                                     args);
      else
        g_assert_not_reached();
      break;
    case SN_HorizontalAdd:
      if (arg0_type == MONO_TYPE_R4)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_HADDPS, arg0_type, fsig, args);
      else if (arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_HADDPD, arg0_type, fsig, args);
      else
        g_assert_not_reached();
      break;
    case SN_HorizontalSubtract:
      if (arg0_type == MONO_TYPE_R4)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_HSUBPS, arg0_type, fsig, args);
      else if (arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_HSUBPD, arg0_type, fsig, args);
      else
        g_assert_not_reached();
      break;
    default:
  ...
  if (is_hw_intrinsics_class(klass, "Ssse3", &is_64bit)) {
    ...
    case SN_AlignRight:
      if (args[2]->opcode == OP_ICONST)
        return emit_simd_ins_for_sig(cfg, klass, OP_SSSE3_ALIGNR,
                                     args[2]->inst_c0, arg0_type, fsig, args);
      return emit_invalid_operation(
          cfg, "mask in Ssse3.AlignRight must be constant");
    case SN_HorizontalAdd:
      if (arg0_type == MONO_TYPE_I2)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_PHADDW, arg0_type, fsig, args);
      return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X, SIMD_OP_SSE_PHADDD,
                                   arg0_type, fsig, args);
    case SN_HorizontalSubtract:
      if (arg0_type == MONO_TYPE_I2)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_PHSUBW, arg0_type, fsig, args);
      return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X, SIMD_OP_SSE_PHSUBD,
                                   arg0_type, fsig, args);
    case SN_Sign:
      if (arg0_type == MONO_TYPE_I1)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_PSIGNB, arg0_type, fsig, args);
      if (arg0_type == MONO_TYPE_I2)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_PSIGNW, arg0_type, fsig, args);
      return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X, SIMD_OP_SSE_PSIGND,
                                   arg0_type, fsig, args);
    default:
      g_assert_not_reached();
      break;
  ...
  if (is_hw_intrinsics_class(klass, "Sse41", &is_64bit)) {
    ...
    supported = COMPILE_LLVM(cfg) &&
                (mini_get_cpu_features(cfg) & MONO_CPU_X86_SSE41) != 0;
    ...
    case SN_DotProduct:
      if (args[2]->opcode == OP_ICONST && arg0_type == MONO_TYPE_R4)
        return emit_simd_ins_for_sig(cfg, klass, OP_SSE41_DPPS_IMM,
                                     args[2]->inst_c0, arg0_type, fsig, args);
      else if (args[2]->opcode == OP_ICONST && arg0_type == MONO_TYPE_R8)
        return emit_simd_ins_for_sig(cfg, klass, OP_SSE41_DPPD_IMM,
                                     args[2]->inst_c0, arg0_type, fsig, args);
      // FIXME: handle non-constant control byte (generate a switch)
      return emit_invalid_operation(
          cfg, "control byte in Sse41.DotProduct must be constant");
    case SN_MultipleSumAbsoluteDifferences:
      if (args[2]->opcode == OP_ICONST)
        return emit_simd_ins_for_sig(cfg, klass, OP_SSE41_MPSADBW_IMM,
                                     args[2]->inst_c0, arg0_type, fsig, args);
      // FIXME: handle non-constant control byte (generate a switch)
      return emit_invalid_operation(
          cfg, "control byte in Sse41.MultipleSumAbsoluteDifferences must be "
               "constant");
    case SN_Blend:
      if (args[2]->opcode == OP_ICONST)
        return emit_simd_ins_for_sig(cfg, klass, OP_SSE41_BLEND_IMM,
                                     args[2]->inst_c0, arg0_type, fsig, args);
      // FIXME: handle non-constant control byte (generate a switch)
      return emit_invalid_operation(
          cfg, "control byte in Sse41.Blend must be constant");
    case SN_BlendVariable:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE41_BLENDV, -1, arg0_type,
                                   fsig, args);
    case SN_Extract: {
      int op = 0;
      switch (arg0_type) {
    ...
    case SN_Insert:
      if (args[2]->opcode == OP_ICONST)
        return emit_simd_ins_for_sig(cfg, klass, OP_SSE41_INSERT, -1, arg0_type,
                                     fsig, args);
  ...
  if (is_hw_intrinsics_class(klass, "Sse42", &is_64bit)) {
    ...
    supported = COMPILE_LLVM(cfg) &&
                (mini_get_cpu_features(cfg) & MONO_CPU_X86_SSE42) != 0;
    ...
    case SN_Crc32: {
      ...
      return emit_simd_ins_for_sig(cfg, klass,
                                   arg1_type == MONO_TYPE_U8 ? OP_SSE42_CRC64
                                                             : OP_SSE42_CRC32,
  ...
  if (is_hw_intrinsics_class(klass, "Pclmulqdq", &is_64bit)) {
    if (!COMPILE_LLVM(cfg))
      return NULL;
    info = lookup_intrins_info(pclmulqdq_methods, sizeof(pclmulqdq_methods),
                               cmethod);
    if (!info)
      return NULL;
    int id = info->id;
    ...
    supported = COMPILE_LLVM(cfg) &&
                (mini_get_cpu_features(cfg) & MONO_CPU_X86_PCLMUL) != 0;

    switch (id) {
    case SN_CarrylessMultiply: {
      if (args[2]->opcode == OP_ICONST)
        return emit_simd_ins_for_sig(cfg, klass, OP_PCLMULQDQ_IMM,
                                     args[2]->inst_c0, arg0_type, fsig, args);
      // FIXME: handle non-constant control byte (generate a switch)
      return emit_invalid_operation(
          cfg, "index in Pclmulqdq.CarrylessMultiply must be constant");
    }
    case SN_get_IsSupported:
      EMIT_NEW_ICONST(cfg, ins, supported ? 1 : 0);
      ins->type = STACK_I4;
  ...
  if (is_hw_intrinsics_class(klass, "Aes", &is_64bit)) {
    ...
    supported = COMPILE_LLVM(cfg) &&
                (mini_get_cpu_features(cfg) & MONO_CPU_X86_AES) != 0;
    ...
    case SN_KeygenAssist: {
      if (args[1]->opcode == OP_ICONST)
        return emit_simd_ins_for_sig(cfg, klass, OP_AES_KEYGEN_IMM,
                                     args[1]->inst_c0, arg0_type, fsig, args);
      // FIXME: handle non-constant control byte (generate a switch)
      return emit_invalid_operation(
          cfg, "control byte in Aes.KeygenAssist must be constant");
    }
    default:
      g_assert_not_reached();
      break;
  ...
  if (is_hw_intrinsics_class(klass, "Bmi1", &is_64bit)) {
    ...
    case SN_AndNot: {
      ...
      EMIT_NEW_BIALU_IMM(cfg, ins, is_64bit ? OP_LXOR_IMM : OP_IXOR_IMM,
                         tmp_reg, args[0]->dreg, -1);
      EMIT_NEW_BIALU(cfg, ins, is_64bit ? OP_LAND : OP_IAND, result_reg,
                     tmp_reg, args[1]->dreg);
      return ins;
    }
    ...
    case SN_GetMaskUpToLowestSetBit: {
      ...
      int tmp_reg = is_64bit ? alloc_lreg(cfg) : alloc_ireg(cfg);
      int result_reg = is_64bit ? alloc_lreg(cfg) : alloc_ireg(cfg);
      EMIT_NEW_BIALU_IMM(cfg, ins, is_64bit ? OP_LSUB_IMM : OP_ISUB_IMM,
                         tmp_reg, args[0]->dreg, 1);
      EMIT_NEW_BIALU(cfg, ins, is_64bit ? OP_LXOR : OP_IXOR, result_reg,
                     args[0]->dreg, tmp_reg);
      return ins;
    }
    ...
    case SN_ExtractLowestSetBit: {
      ...
      EMIT_NEW_BIALU(cfg, ins, is_64bit ? OP_LSUB : OP_ISUB, tmp_reg, zero_reg,
                     args[0]->dreg);
      EMIT_NEW_BIALU(cfg, ins, is_64bit ? OP_LAND : OP_IAND, result_reg,
                     args[0]->dreg, tmp_reg);
      return ins;
...
static guint16 vector_128_methods[] = {
    SN_AsByte,   SN_AsDouble, SN_AsInt16,  SN_AsInt32,
    SN_AsInt64,  SN_AsSByte,  SN_AsSingle, SN_AsUInt16,
    SN_AsUInt32, SN_AsUInt64, SN_Create,   SN_CreateScalarUnsafe,
};
...
static MonoInst *emit_vector128(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  case SN_Create: {
    ...
    if (fsig->param_count == 1 &&
        mono_metadata_type_equal(fsig->params[0], etype)) {
      return emit_simd_ins(cfg, klass, type_to_expand_op(etype), args[0]->dreg,
                           -1);
    } else {
  ...
  case SN_CreateScalarUnsafe:
    return emit_simd_ins_for_sig(cfg, klass, OP_CREATE_SCALAR_UNSAFE, -1,
                                 arg0_type, fsig, args);
  default:
    break;
  }
...
static MonoInst *emit_vector128_t(MonoCompile *cfg, MonoMethod *cmethod,
                                  MonoMethodSignature *fsig, MonoInst **args) {
  MonoInst *ins;
  MonoType *type, *etype;
  MonoClass *klass;
  ...
  id = lookup_intrins(vector_128_t_methods, sizeof(vector_128_t_methods),
                      cmethod);
  if (id == -1)
    return NULL;
...
static MonoInst *emit_vector256_t(MonoCompile *cfg, MonoMethod *cmethod,
                                  MonoMethodSignature *fsig, MonoInst **args) {
  MonoInst *ins;
  MonoType *type, *etype;
  MonoClass *klass;
  ...
  id = lookup_intrins(vector_256_t_methods, sizeof(vector_256_t_methods),
                      cmethod);
  if (id == -1)
    return NULL;
