...
static MonoInst *emit_x86_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  if (is_hw_intrinsics_class(klass, "Sse", &is_64bit)) {
    ...
    case SN_ConvertScalarToVector128Single:
      if (fsig->params[1]->type == MONO_TYPE_I4)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_I4,
                                     SIMD_OP_SSE_CVTSI2SS, 0, fsig, args);
      else if (fsig->params[1]->type == MONO_TYPE_I8)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_I8,
                                     SIMD_OP_SSE_CVTSI2SS64, 0, fsig, args);
      else
        g_assert_not_reached();
      break;
    ...
    case SN_SqrtScalar:
      if (fsig->param_count == 1)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X, info->instc0,
                                     arg0_type, fsig, args);
      else
        return NULL;
  ...
  if (is_hw_intrinsics_class(klass, "Sse2", &is_64bit)) {
    ...
    case SN_SubtractSaturate: {
      ...
      switch (arg0_type) {
      case MONO_TYPE_I1:
        op = SIMD_OP_SSE_PSUBSB;
        break;
      case MONO_TYPE_I2:
        op = SIMD_OP_SSE_PSUBSW;
        break;
      case MONO_TYPE_U1:
        op = SIMD_OP_SSE_PSUBUSB;
        break;
      case MONO_TYPE_U2:
        op = SIMD_OP_SSE_PSUBUSW;
        break;
      default:
        g_assert_not_reached();
        break;
      }
      return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X, op, arg0_type,
                                   fsig, args);
    }
    case SN_Add:
      return emit_simd_ins_for_sig(
    ...
    case SN_ConvertScalarToVector128Double:
      if (fsig->params[1]->type == MONO_TYPE_I4)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_I4,
                                     SIMD_OP_SSE_CVTSI2SD, 0, fsig, args);
      else if (fsig->params[1]->type == MONO_TYPE_I8)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_I8,
                                     SIMD_OP_SSE_CVTSI2SD64, 0, fsig, args);
      return emit_simd_ins_for_sig(cfg, klass, OP_CVTSD2SD, 0, arg0_type, fsig,
                                   args);
    ...
    case SN_Multiply:
      if (arg0_type == MONO_TYPE_U4)
        return emit_simd_ins_for_sig(cfg, klass, OP_XOP_X_X_X,
                                     SIMD_OP_SSE_PMULUDQ, arg0_type, fsig,
                                     args);
      else if (arg0_type == MONO_TYPE_R8)
