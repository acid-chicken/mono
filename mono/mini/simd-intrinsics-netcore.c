...
static int lookup_intrins(guint16 *intrinsics, int size, MonoMethod *cmethod) {
  ...
  for (int i = 0; i < (size / sizeof(guint16)) - 1; ++i) {
    if (method_name(intrinsics[i])[0] > method_name(intrinsics[i + 1])[0]) {
      printf("%s %s\n", method_name(intrinsics[i]),
             method_name(intrinsics[i + 1]));
...
static guint16 sse_methods[] = {SN_Add,
                                SN_And,
                                SN_AndNot,
                                SN_CompareEqual,
                                SN_CompareNotEqual,
...
static guint16 sse2_methods[] = {SN_Add,
                                 SN_AddSaturate,
                                 SN_And,
                                 SN_AndNot,
                                 SN_CompareEqual,
...
static guint16 ssse3_methods[] = {SN_Shuffle, SN_get_IsSupported};

static guint16 sse3_methods[] = {SN_MoveAndDuplicate, SN_get_IsSupported};

static guint16 sse41_methods[] = {SN_Insert, SN_Max, SN_Min, SN_TestZ,
                                  SN_get_IsSupported};

static guint16 popcnt_methods[] = {SN_PopCount, SN_get_IsSupported};

static guint16 lzcnt_methods[] = {SN_LeadingZeroCount, SN_get_IsSupported};
...
static guint16 bmi2_methods[] = {
    SN_MultiplyNoFlags, SN_ParallelBitDeposit, SN_ParallelBitExtract,
    SN_ZeroHighBits,    SN_get_IsSupported,
};
...
static MonoInst *emit_x86_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  if (is_hw_intrinsics_class(klass, "Sse", &is_64bit)) {
    ...
    supported =
        (mini_get_cpu_features(cfg) & MONO_CPU_X86_SSE) != 0 &&
        m_class_get_image(cfg->method->klass) ==
            mono_get_corlib(); // We only support the subset used by corelib
    ...
    case SN_LoadVector128:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_LOADU, 1 /*alignment*/,
                                   arg0_type, fsig, args);
    case SN_MoveMask:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_MOVMSK, -1, arg0_type,
                                   fsig, args);
    case SN_MoveScalar:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_MOVS2, -1, arg0_type,
                                   fsig, args);
    case SN_CompareNotEqual:
      return emit_simd_ins_for_sig(cfg, klass, OP_XCOMPARE_FP, CMP_NE,
                                   arg0_type, fsig, args);
    case SN_CompareEqual:
      return emit_simd_ins_for_sig(cfg, klass, OP_XCOMPARE_FP, CMP_EQ,
                                   arg0_type, fsig, args);
    case SN_And:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_AND, -1, arg0_type, fsig,
                                   args);
    case SN_AndNot:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_ANDN, -1, arg0_type, fsig,
                                   args);
    case SN_Or:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_OR, -1, arg0_type, fsig,
                                   args);
    case SN_Xor:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_XOR, -1, arg0_type, fsig,
                                   args);
    case SN_Multiply:
      return emit_simd_ins_for_sig(cfg, klass, OP_XBINOP, OP_FMUL, arg0_type,
                                   fsig, args);
    case SN_Divide:
      return emit_simd_ins_for_sig(cfg, klass, OP_XBINOP, OP_FDIV, arg0_type,
                                   fsig, args);
    case SN_Add:
      return emit_simd_ins_for_sig(cfg, klass, OP_XBINOP, OP_FADD, arg0_type,
                                   fsig, args);
    case SN_Subtract:
      return emit_simd_ins_for_sig(cfg, klass, OP_XBINOP, OP_FSUB, arg0_type,
                                   fsig, args);
    case SN_Shuffle: {
    ...
    case SN_MoveHighToLow:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_MOVEHL, -1, arg0_type,
                                   fsig, args);
    case SN_MoveLowToHigh:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_MOVELH, -1, arg0_type,
                                   fsig, args);
    case SN_UnpackLow:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_UNPACKLO, -1, arg0_type,
                                   fsig, args);
    case SN_UnpackHigh:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_UNPACKHI, -1, arg0_type,
                                   fsig, args);
    case SN_Store:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_STORE, 1 /*alignment*/,
                                   arg0_type, fsig, args);
    case SN_StoreAligned:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_STORE, 16 /*alignment*/,
                                   arg0_type, fsig, args);
    default:
      return NULL;
    }
  ...
  if (is_hw_intrinsics_class(klass, "Sse2", &is_64bit)) {
    ...
    supported =
        (mini_get_cpu_features(cfg) & MONO_CPU_X86_SSE2) != 0 &&
        m_class_get_image(cfg->method->klass) ==
            mono_get_corlib(); // We only support the subset used by corelib
    ...
    case SN_Add:
      return emit_simd_ins_for_sig(
          ...
          arg0_type, fsig, args);
    case SN_AddSaturate:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_ADDS, -1, arg0_type,
                                   fsig, args);
    case SN_And:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_AND, -1, arg0_type, fsig,
                                   args);
    ...
    case SN_ConvertScalarToVector128UInt32:
      return emit_simd_ins_for_sig(cfg, klass, OP_EXPAND_I4, -1, arg0_type,
                                   fsig, args);
    case SN_ConvertScalarToVector128UInt64:
      return emit_simd_ins_for_sig(cfg, klass, OP_EXPAND_I8, -1, arg0_type,
                                   fsig, args);
    case SN_ConvertToUInt32:
    ...
    case SN_ShiftRightLogical: {
      ...
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE2_SRLI, -1, arg0_type,
                                   fsig, args);
    }
    case SN_Shuffle: {
      ...
      ins = emit_simd_ins_for_sig(cfg, klass, OP_SSE2_SHUFFLE, -1, arg0_type,
                                  fsig, args);
    ...
    case SN_StoreAligned:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_STORE, 16 /*alignment*/,
                                   arg0_type, fsig, args);
    case SN_StoreScalar:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_STORES, -1, arg0_type,
                                   fsig, args);
    case SN_UnpackLow:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE_UNPACKLO, -1, arg0_type,
                                   fsig, args);
  ...
  if (is_hw_intrinsics_class(klass, "Sse3", &is_64bit)) {
    ...
    supported =
        (mini_get_cpu_features(cfg) & MONO_CPU_X86_SSE3) != 0 &&
        m_class_get_image(cfg->method->klass) ==
            mono_get_corlib(); // We only support the subset used by corelib
    ...
    case SN_MoveAndDuplicate:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE3_MOVDDUP, -1, arg0_type,
                                   fsig, args);
    default:
      return NULL;
    }
  ...
  if (is_hw_intrinsics_class(klass, "Ssse3", &is_64bit)) {
    ...
    supported =
        (mini_get_cpu_features(cfg) & MONO_CPU_X86_SSSE3) != 0 &&
        m_class_get_image(cfg->method->klass) ==
            mono_get_corlib(); // We only support the subset used by corelib
    ...
    case SN_get_IsSupported:
      ...
      return ins;
    case SN_Shuffle:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSSE3_SHUFFLE, -1, arg0_type,
                                   fsig, args);
    default:
      return NULL;
    }
  ...
  if (is_hw_intrinsics_class(klass, "Sse41", &is_64bit)) {
    ...
    case SN_Insert:
      ...
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE41_INSERT, -1, arg0_type,
                                   fsig, args);
    case SN_Max:
      return emit_simd_ins_for_sig(cfg, klass, OP_XBINOP, OP_IMAX, arg0_type,
                                   fsig, args);
    case SN_Min:
      return emit_simd_ins_for_sig(cfg, klass, OP_XBINOP, OP_IMIN, arg0_type,
                                   fsig, args);
    case SN_TestZ:
      return emit_simd_ins_for_sig(cfg, klass, OP_SSE41_PTESTZ, -1, arg0_type,
                                   fsig, args);
    default:
      return NULL;
    }
