...
void mono_arch_patch_callsite(guint8 *method_start, guint8 *code_ptr,
                              guint8 *addr) {
...
void mono_arch_patch_plt_entry(guint8 *code, gpointer *got, host_mgreg_t *regs,
  ...
  if (*(guint32 *)code == 0xe59fc000) {
    ...
    /*
     * Thumb PLT entry, begins with ldr.w ip, [pc, #8], code points to entry +
     * 4, see mono_arm_get_thumb_plt_entry ().
     */
    guint32 offset;
 ...
 */
static guint32 branch_for_target_reachable(guint8 *branch, guint8 *target) {
...
static guint8 *emit_bx(guint8 *code, int reg) {
...
guchar *mono_arch_create_generic_trampoline(MonoTrampolineType tramp_type,
  ...
  /* The size of the area already allocated by the push in the specific
   * trampoline */
  ...
  /* Callee saved regs */
  for (i = 0; i < 8; ++i)
    mono_add_unwind_op_offset(unwind_ops, code, buf, ARMREG_R4 + i,
                              -regsave_size + ((4 + i) * 4));

  if (aot) {
    /*
     * For page trampolines the data is in r1, so just move it, otherwise use
     * the got slot as below. The trampoline contains a pc-relative offset to
     * the got slot preceeding the got slot where the value is stored. The
     * offset can be found at [lr + 0].
     */
  ...
  /* V1 == lmf */
  code =
      mono_arm_emit_load_imm(code, ARMREG_R2, STACK - MONO_ABI_SIZEOF(MonoLMF));
   ...
   */
  if (aot) {
    ji = mono_patch_info_list_prepend(
        ji, code - buf, MONO_PATCH_INFO_JIT_ICALL_ADDR,
  ...
  ARM_STR_IMM(code, ARMREG_R0, ARMREG_V1,
              MONO_STRUCT_OFFSET(MonoLMF, lmf_addr));
  /* new_lmf->previous_lmf = *lmf_addr */
  ARM_LDR_IMM(code, ARMREG_R2, ARMREG_R0,
              MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
  ARM_STR_IMM(code, ARMREG_R2, ARMREG_V1,
              MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
  /* *(lmf_addr) = r1 */
  ARM_STR_IMM(code, ARMREG_V1, ARMREG_R0,
              MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
  /* save method info (it's in v2) */
  if ((tramp_type == MONO_TRAMPOLINE_JIT) ||
      (tramp_type == MONO_TRAMPOLINE_JUMP))
    ARM_STR_IMM(code, ARMREG_V2, ARMREG_V1,
  ...
  /* save caller FP */
  ARM_LDR_IMM(code, ARMREG_R2, ARMREG_V1,
              (MONO_STRUCT_OFFSET(MonoLMF, iregs) + ARMREG_FP * 4));
  ...
  if (tramp_type == MONO_TRAMPOLINE_JUMP) {
    ...
    ARM_LDR_IMM(code, ARMREG_R2, ARMREG_V1,
                (MONO_STRUCT_OFFSET(MonoLMF, iregs) + 13 * 4));
  }
  ...
  if (mono_arm_is_hard_float()) {
     ...
     */
    ARM_ADD_REG_IMM8(code, ARMREG_R0, ARMREG_V1,
                     MONO_STRUCT_OFFSET(MonoLMF, fregs));
  ...
  ARM_ADD_REG_IMM(code, ARMREG_R0, ARMREG_V1,
                  MONO_STRUCT_OFFSET(MonoLMF, iregs), 0);
   ...
   */
  ARM_STR_IMM(code, ARMREG_R0, ARMREG_V1,
              MONO_STRUCT_OFFSET(MonoLMF, iregs) +
                  (ARMREG_R12 * sizeof(target_mgreg_t)));
   ...
   */
  /* ip = previous_lmf */
  ARM_LDR_IMM(code, ARMREG_IP, ARMREG_V1,
              MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
  /* lr = lmf_addr */
  ARM_LDR_IMM(code, ARMREG_LR, ARMREG_V1,
              MONO_STRUCT_OFFSET(MonoLMF, lmf_addr));
  ...
  if (aot) {
    code = mono_arm_emit_aotconst(
        &ji, code, buf, ARMREG_IP, MONO_PATCH_INFO_JIT_ICALL_ADDR,
        GUINT_TO_POINTER(
    ...
    ARM_B(code, 0);
    *(gpointer *)code =
        (gpointer)mono_thread_force_interruption_checkpoint_noraise;
    code += 4;
  }
  ARM_MOV_REG_REG(code, ARMREG_LR, ARMREG_PC);
  code = emit_bx(code, ARMREG_IP);
  ...
  if (!aot) {
    ...
    ARM_LDR_IMM(load_get_lmf_addr, ARMREG_R0, ARMREG_PC,
                (code - load_get_lmf_addr - 8));
    ARM_LDR_IMM(load_trampoline, ARMREG_IP, ARMREG_PC,
                (code + 4 - load_trampoline - 8));
    code += 8;
  }
  ...
  /* lr contains the return address, the trampoline will use it as the throw
   * site */
  code = emit_bx(code, ARMREG_IP);
...
gpointer mono_arch_get_unbox_trampoline(MonoMethod *m, gpointer addr) {
  ...
  *(guint32 *)code = (guint32)(gsize)addr;
  ...
  /*g_print ("unbox trampoline at %d for %s:%s\n", this_pos, m->klass->name,
  m->name); g_print ("unbox code is at %p for method at %p\n", start, addr);*/
...
gpointer mono_arch_create_rgctx_lazy_fetch_trampoline(guint32 slot,
  ...
  if (mrgctx) {
    ...
    g_assert(
        arm_is_imm12(MONO_STRUCT_OFFSET(MonoVTable, runtime_generic_context)));
    ARM_LDR_IMM(code, ARMREG_R1, ARMREG_R0,
  ...
  for (i = 0; i < depth; ++i) {
    ...
    if (mrgctx && i == 0) {
      ...
      ARM_LDR_IMM(code, ARMREG_R1, ARMREG_R1,
  ...
  /* fetch slot */
  code = mono_arm_emit_load_imm(code, ARMREG_R2,
                                TARGET_SIZEOF_VOID_P * (index + 1));
  ...
  if (aot) {
    ji = mono_patch_info_list_prepend(
        ji, code - buf, MONO_PATCH_INFO_SPECIFIC_TRAMPOLINE_LAZY_FETCH_ADDR,
...
guint8 *mono_arch_create_sdb_trampoline(gboolean single_step,
  ...
  ARM_STR_IMM(code, ARMREG_IP, ARMREG_SP,
              MONO_STRUCT_OFFSET(MonoContext, regs) +
                  sizeof(target_mgreg_t) * ARMREG_IP);
  ARM_STR_IMM(code, ARMREG_LR, ARMREG_SP,
              MONO_STRUCT_OFFSET(MonoContext, regs) + 4 * ARMREG_LR);
  ARM_STR_IMM(code, ARMREG_LR, ARMREG_SP,
              MONO_STRUCT_OFFSET(MonoContext, regs) + 4 * ARMREG_PC);

  /* save r0..r10 and fp */
  ARM_ADD_REG_IMM8(code, ARMREG_IP, ARMREG_SP,
                   MONO_STRUCT_OFFSET(MonoContext, regs));
  ...
  ARM_STR_IMM(code, ARMREG_R0, ARMREG_FP,
              MONO_STRUCT_OFFSET(MonoContext, regs) + 4 * ARMREG_SP);
  ...
  if (aot) {
    ...
    *(gpointer *)code = NULL;
    ...
    if (single_step)
      *(gpointer *)code =
          (gpointer)mini_get_dbg_callbacks()->single_step_from_context;
    else
      *(gpointer *)code =
          (gpointer)mini_get_dbg_callbacks()->breakpoint_from_context;
    code += 4;
    ARM_BLX_REG(code, ARMREG_IP);
  }
  ...
  ARM_ADD_REG_IMM8(code, ARMREG_IP, ARMREG_FP,
                   MONO_STRUCT_OFFSET(MonoContext, regs));
...
gpointer mono_arch_get_interp_to_native_trampoline(MonoTrampInfo **info) {
  ...
  /* allocate space for saving the target addr and the call context and align
   * stack */
  framesize = sizeof(target_mgreg_t) +
              ALIGN_TO(2 * sizeof(target_mgreg_t), MONO_ARCH_FRAME_ALIGNMENT);
  ...
  ARM_LDR_IMM(code, ARMREG_R3, ARMREG_R1,
  ...
  for (i = 0; i < FP_PARAM_REGS; i++)
    ARM_FLDD(code, i * 2, ARMREG_IP,
             MONO_STRUCT_OFFSET(CallContext, fregs) + i * sizeof(double));
  ...
  for (i = 0; i < PARAM_REGS; i++)
    ARM_STR_IMM(code, i, ARMREG_IP,
                MONO_STRUCT_OFFSET(CallContext, gregs) +
                    i * sizeof(target_mgreg_t));

  /* set all floating registers to CallContext  */
  for (i = 0; i < FP_PARAM_REGS; i++)
    ARM_FSTD(code, i * 2, ARMREG_IP,
             MONO_STRUCT_OFFSET(CallContext, fregs) + i * sizeof(double));
...
gpointer mono_arch_get_native_to_interp_trampoline(MonoTrampInfo **info) {
  ...
  ARM_LDR_IMM(code, ARMREG_IP, ARMREG_IP,
              MONO_STRUCT_OFFSET(MonoFtnDesc, addr));
  ARM_BLX_REG(code, ARMREG_IP);

  /* load the return values from the context */
  for (i = 0; i < PARAM_REGS; i++)
    ARM_LDR_IMM(code, i, ARMREG_SP,
                MONO_STRUCT_OFFSET(CallContext, gregs) +
                    i * sizeof(target_mgreg_t));

  for (i = 0; i < FP_PARAM_REGS; i++)
    ARM_FLDD(code, i * 2, ARMREG_SP,
             MONO_STRUCT_OFFSET(CallContext, fregs) + i * sizeof(double));
 ...
 */
guint8 *mono_arm_get_thumb_plt_entry(guint8 *code) {
  ...
  /*
   * The PLT info offset is at offset 16, but mono_arch_get_plt_entry_offset ()
   * returns the 3rd word, so compensate by returning a different value.
   */
  target += 4;
...
gpointer mono_arch_get_gsharedvt_arg_trampoline(MonoDomain *domain,
  ...
  mono_tramp_info_register(
      mono_tramp_info_create(NULL, buf, code - buf, NULL, NULL), domain);
