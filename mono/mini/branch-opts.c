 ...
 * successor.
 */
static gboolean mono_bb_is_fall_through(MonoCompile *cfg, MonoBasicBlock *bb) {
  return bb->next_bb &&
         bb->next_bb->region ==
             bb->region && /*fall throught between regions is not really
  ...
  for (i = 0; i < header->num_clauses; ++i) {
    ...
    if (MONO_OFFSET_IN_CLAUSE(clause, bb->real_offset)) {
      if (clause->flags == MONO_EXCEPTION_CLAUSE_NONE &&
          ...
          mono_class_is_assignable_from_internal(clause->data.catch_class,
                                                 exclass)) {
        MonoBasicBlock *tbb;
        ...
        if (tbb && tbb->flags & BB_EXCEPTION_DEAD_OBJ &&
            !(tbb->flags & BB_EXCEPTION_UNSAFE)) {
           ...
           */
          while (!unsafe && tbb->next_bb &&
                 tbb->region == tbb->next_bb->region) {
          ...
          if (!unsafe) {
            ...
            if (cfg->verbose_level > 2)
              g_print("found exception to optimize - returning branch to BB%d "
                      "(%s) (instead of throw) for method %s:%s\n",
                      targetbb->block_num,
                      m_class_get_name(clause->data.catch_class),
                      m_class_get_name(cfg->method->klass), cfg->method->name);
...
static const int int_cmov_opcodes[] = {
    OP_CMOV_IEQ,    OP_CMOV_INE_UN, OP_CMOV_ILE,    OP_CMOV_IGE,
    OP_CMOV_ILT,    OP_CMOV_IGT,    OP_CMOV_ILE_UN, OP_CMOV_IGE_UN,
    OP_CMOV_ILT_UN, OP_CMOV_IGT_UN};

static const int long_cmov_opcodes[] = {
    OP_CMOV_LEQ,    OP_CMOV_LNE_UN, OP_CMOV_LLE,    OP_CMOV_LGE,
    OP_CMOV_LLT,    OP_CMOV_LGT,    OP_CMOV_LLE_UN, OP_CMOV_LGE_UN,
    OP_CMOV_LLT_UN, OP_CMOV_LGT_UN};

static int br_to_br_un(int opcode) {
  switch (opcode) {
  case OP_IBGT:
    return OP_IBGT_UN;
    break;
 ...
 * mono_replace_ins:
 *
 *   Replace INS with its decomposition which is stored in a series of bblocks
 * starting at FIRST_BB and ending at LAST_BB. On enter, PREV points to the
 * predecessor of INS. On return, it will be set to the last ins of the
 * decomposition.
 */
void mono_replace_ins(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst *ins,
  ...
  if (first_bb == last_bb) {
    ...
    *prev = last_bb->last_ins;
    ...
    tmp_bblocks = mono_mempool_alloc0(cfg->mempool,
                                      sizeof(MonoBasicBlock *) * bb->out_count);
...
void mono_if_conversion(MonoCompile *cfg) {
   ...
   * local cprop/deadce. Also, it should be run before dominator computation,
   * since it changes control flow.
   */
  ...
  restart:
     ...
     * br BB3
     */
    if (bb1->in_count == 1 && bb2->in_count == 1 && bb1->out_count == 1 &&
        bb2->out_count == 1 && bb1->out_bb[0] == bb2->out_bb[0]) {
      ...
      for (tmp = ins1->next; tmp; tmp = tmp->next)
        if (!((tmp->opcode == OP_NOP) || (tmp->opcode == OP_IL_SEQ_POINT) ||
              (tmp->opcode == OP_BR)))
          simple = FALSE;
      ...
      if (cfg->verbose_level > 2) {
        ...
        printf("\t\t");
        mono_print_ins(mono_inst_next(compare, filter));
        printf("\t\t");
        mono_print_ins(ins1);
        printf("\t\t");
        mono_print_ins(ins2);
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    /* Check structure */
    if (!(bb1->in_count == 2 && bb1->in_bb[0] == bb && bb1->in_bb[1] == bb2 &&
          bb2->in_count == 1 && bb2->out_count == 2))
      continue;
    ...
    if (!(branch1 &&
          ((branch1->opcode == OP_IBLT) || (branch1->opcode == OP_LBLT)) &&
          (branch1->inst_false_bb == next_bb)))
      continue;
    ...
    if (((branch2->opcode == OP_IBGT) || (branch2->opcode == OP_LBGT)) &&
        branch2->inst_true_bb == branch1->inst_true_bb)
      ;
    else if (((branch2->opcode == OP_IBLE) || (branch2->opcode == OP_LBLE)) &&
             branch2->inst_false_bb == branch1->inst_true_bb)
      ;
    else
      continue;
    ...
    if (!(compare1 &&
          ((compare1->opcode == OP_ICOMPARE_IMM) ||
           (compare1->opcode == OP_LCOMPARE_IMM)) &&
          compare1->inst_imm == 0))
      continue;
    ...
    if (((ins->opcode == OP_ICOMPARE_IMM) ||
        ...
        ins->sreg1 == compare1->sreg1 && next == branch2) {
      /* The second arg must be positive */
      if (ins->inst_imm < 0)
        continue;
    } else if (((ins->opcode == OP_LDLEN) || (ins->opcode == OP_STRLEN)) &&
               ins->dreg != compare1->sreg1 && next &&
               next->opcode == OP_ICOMPARE && next->sreg1 == compare1->sreg1 &&
               next->sreg2 == ins->dreg &&
               mono_inst_next(next, filter) == branch2) {
    ...
    if (cfg->verbose_level > 2) {
      ...
      printf("\t\t");
      mono_print_ins(mono_inst_next(compare1, filter));
      printf("\t\t");
      mono_print_ins(ins);
    }
...
static void replace_out_block(MonoBasicBlock *bb, MonoBasicBlock *orig,
                              MonoBasicBlock *repl) {
  int i;
...
static void replace_in_block(MonoBasicBlock *bb, MonoBasicBlock *orig,
                             MonoBasicBlock *repl) {
  int i;
...
static void replace_out_block_in_code(MonoBasicBlock *bb, MonoBasicBlock *orig,
                                      MonoBasicBlock *repl) {
  MonoInst *ins;
    ...
    default:
      if (MONO_IS_COND_BRANCH_OP(ins)) {
        ...
        MonoJumpInfoBBTable *table =
            (MonoJumpInfoBBTable *)MONO_JUMP_TABLE_FROM_INS(ins);
 ...
 */
static gboolean remove_block_if_useless(MonoCompile *cfg, MonoBasicBlock *bb,
  ...
  if (target_bb == NULL) {
    if ((bb->out_count == 1) && (bb->out_bb[0] == bb->next_bb)) {
      ...
      /* Do not touch empty BBs that do not "fall through" to their next BB
       * (like the exit BB) */
      return FALSE;
    }
  }
  ...
  /* Do not touch BBs following the entry BB and jumping to something that is
   * not */
  ...
  /* Check that there is a target BB, and that bb is not an empty loop (Bug
   * 75061) */
  if ((target_bb != NULL) && (target_bb != bb)) {
    ...
    /* unlink_bblock () modifies the bb->in_bb array so can't use a for loop
     * here */
    while (bb->in_count) {
    ...
    if (previous_bb != cfg->bb_entry &&
        mono_bb_is_fall_through(cfg, previous_bb)) {
...
void mono_merge_basic_blocks(MonoCompile *cfg, MonoBasicBlock *bb,
  ...
  /* There may be only one control flow edge between two BBs that we merge, and
   * it should connect these BBs together. */
  g_assert(bb->out_count == 1 && bbn->in_count == 1 && bb->out_bb[0] == bbn &&
           bbn->in_bb[0] == bb);
  ...
  if (bb->has_jump_table) {
    for (inst = bb->code; inst != NULL; inst = inst->next) {
      if (MONO_IS_JUMP_TABLE(inst)) {
        int i;
        MonoJumpInfoBBTable *table =
            (MonoJumpInfoBBTable *)MONO_JUMP_TABLE_FROM_INS(inst);
        for (i = 0; i < table->table_size; i++) {
  ...
  if (bbn->in_bb[0]->next_bb == bbn)
    ...
    for (prev_bb = cfg->bb_entry; prev_bb && prev_bb->next_bb != bbn;
   ...
   */
  if (bb->last_ins && bb->out_count == 1 && bb->out_bb[0] != bb->next_bb &&
...
static void move_basic_block_to_end(MonoCompile *cfg, MonoBasicBlock *bb) {
  MonoBasicBlock *bbn, *next;

  next = bb->next_bb;

  /* Find the previous */
  for (bbn = cfg->bb_entry; bbn->next_bb && bbn->next_bb != bb;
       bbn = bbn->next_bb)
    ;
  if (bbn->next_bb) {
  ...
  if (next && (!bb->last_ins || ((bb->last_ins->opcode != OP_NOT_REACHED) &&
                                 ...
                                 (bb->last_ins->opcode != OP_BR_REG) &&
                                 (!MONO_IS_COND_BRANCH_OP(bb->last_ins))))) {
    MonoInst *ins;
...
void mono_remove_bblock(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  for (tmp_bb = cfg->bb_entry; tmp_bb && tmp_bb->next_bb != bb;
...
void mono_remove_critical_edges(MonoCompile *cfg) {
  ...
  for (previous_bb = cfg->bb_entry, bb = previous_bb->next_bb; bb != NULL;
       previous_bb = previous_bb->next_bb, bb = bb->next_bb) {
    if (bb->in_count > 1) {
      int in_bb_index;
      for (in_bb_index = 0; in_bb_index < bb->in_count; in_bb_index++) {
        ...
        if ((in_bb->out_count > 1) ||
            (in_bb->out_count == 1 && in_bb->last_ins &&
             in_bb->last_ins->opcode == OP_BR_REG)) {
          MonoBasicBlock *new_bb = (MonoBasicBlock *)mono_mempool_alloc0(
          ...
          if (mono_bb_is_fall_through(cfg, previous_bb)) {
            if (previous_bb != cfg->bb_entry) {
              ...
              MonoInst *jump;
              MonoBasicBlock *new_bb_after_entry =
                  (MonoBasicBlock *)mono_mempool_alloc0((cfg)->mempool,
                                                        sizeof(MonoBasicBlock));
              new_bb_after_entry->block_num = cfg->num_bblocks++;
              //							new_bb_after_entry->real_offset
              //= bb->real_offset;
              ...
              if (cfg->verbose_level > 2) {
                printf("remove_critical_edges, added helper BB%d jumping to "
                       "BB%d\n",
          ...
          new_bb->in_bb = (MonoBasicBlock **)mono_mempool_alloc(
              (cfg)->mempool, sizeof(MonoBasicBlock *));
          new_bb->in_bb[0] = in_bb;
          new_bb->in_count = 1;
          new_bb->out_bb = (MonoBasicBlock **)mono_mempool_alloc(
              (cfg)->mempool, sizeof(MonoBasicBlock *));
          new_bb->out_bb[0] = bb;
          new_bb->out_count = 1;
          ...
          if (cfg->verbose_level > 2) {
            printf("remove_critical_edges, removed critical edge from BB%d to "
                   "BB%d (added BB%d)\n",
...
void mono_optimize_branches(MonoCompile *cfg) {
  ...
  do {
    ...
    for (previous_bb = cfg->bb_entry, bb = cfg->bb_entry->next_bb; bb;
         previous_bb = bb, bb = bb->next_bb) {
      count++;
      if (count == 1000) {
      ...
      if ((bbn = bb->next_bb) && bbn->in_count == 0 && bbn != cfg->bb_exit &&
          bb->region == bbn->region) {
        if (cfg->verbose_level > 2)
      ...
      if (bb->out_count == 1) {
        ...
        /* conditional branches where true and false targets are the same can be
         * also replaced with OP_BR */
      ...
      if (bb->out_count == 1) {
        ...
        if (bb->last_ins && bb->last_ins->opcode == OP_BR) {
          ...
          if (bb->region == bbn->region && bbn_first_inst &&
              bbn_first_inst->opcode == OP_BR &&
            ...
            if (cfg->verbose_level > 2)
              g_print("branch to branch triggered %d -> %d -> %d\n",
                      ...
                      bbn_first_inst->inst_target_bb->block_num);
        ...
        if (bb->last_ins && MONO_IS_COND_BRANCH_NOFP(bb->last_ins)) {
          int branch_result;
          MonoBasicBlock *taken_branch_target = NULL,
                         *untaken_branch_target = NULL;
          ...
          bbn_first_inst = mono_bb_first_inst(bbn, filter);
          if (bbn && bb->region == bbn->region && bbn_first_inst &&
              bbn_first_inst->opcode == OP_BR &&
              bbn_first_inst->inst_target_bb->region == bb->region) {
            if (cfg->verbose_level > 2)
           ...
           * slowing down loops.
           */
          if (bbn && bb->region == bbn->region && bbn->in_count == 1 &&
              cfg->enable_extended_bblocks && bbn != cfg->bb_exit &&
              !bb->extended && !bbn->out_of_line &&
              !mono_bblocks_linked(bbn, bb)) {
        ...
        if (bb->last_ins && MONO_IS_COND_BRANCH_NOFP(bb->last_ins)) {
          if (bb->last_ins->inst_false_bb &&
              bb->last_ins->inst_false_bb->out_of_line &&
              (bb->region == bb->last_ins->inst_false_bb->region) &&
              !cfg->disable_out_of_line_bblocks) {
            /* Reverse the branch */
