...
/*
 * The mini code should not have any compile time dependencies on the GC being
 * used, so the same object file from mini/ can be linked into both mono and
 * mono-sgen.
 */
...
#define COMPILE_SOFT_FLOAT(cfg)                                                \
  (!COMPILE_LLVM((cfg)) && mono_arch_is_soft_float())
#else
...
#define mono_simd_shuffle_mask(a, b, c, d)                                     \
  ((a) | ((b) << 2) | ((c) << 4) | ((d) << 6))
...
#define MONO_TYPE_IS_PRIMITIVE(t)                                              \
  ((!(t)->byref &&                                                             \
    ((((t)->type >= MONO_TYPE_BOOLEAN && (t)->type <= MONO_TYPE_R8) ||         \
      ((t)->type >= MONO_TYPE_I && (t)->type <= MONO_TYPE_U)))))
// XXX this ignores if t is byref
#define MONO_TYPE_IS_PRIMITIVE_SCALAR(t)                                       \
  ((((((t)->type >= MONO_TYPE_BOOLEAN && (t)->type <= MONO_TYPE_U8) ||         \
      ((t)->type >= MONO_TYPE_I && (t)->type <= MONO_TYPE_U)))))
...
#else
#define mono_bitset_foreach_bit(set, b, n)                                     \
  for (b = mono_bitset_find_start(set); b < n && b >= 0;                       \
       b = mono_bitset_find_first(set, b))
#endif
...
#define MONO_INST_NULLIFY_SREGS(dest)                                          \
  do {                                                                         \
    (dest)->sreg1 = (dest)->sreg2 = (dest)->sreg3 = -1;                        \
  } while (0)

#define MONO_INST_NEW(cfg, dest, op)                                           \
  do {                                                                         \
    (dest) =                                                                   \
        (MonoInst *)mono_mempool_alloc0((cfg)->mempool, sizeof(MonoInst));     \
    (dest)->opcode = (op);                                                     \
    (dest)->dreg = -1;                                                         \
...
#define MONO_INST_NEW_CALL(cfg, dest, op)                                      \
  do {                                                                         \
    (dest) = (MonoCallInst *)mono_mempool_alloc0((cfg)->mempool,               \
                                                 sizeof(MonoCallInst));        \
    (dest)->inst.opcode = (op);                                                \
    (dest)->inst.dreg = -1;                                                    \
 ...
 * exclude FP compares because they have weird semantics with NaNs.
 */
#define MONO_IS_COND_BRANCH_OP(ins)                                            \
  (((ins)->opcode >= OP_LBEQ && (ins)->opcode <= OP_LBLT_UN) ||                \
   ((ins)->opcode >= OP_FBEQ && (ins)->opcode <= OP_FBLT_UN) ||                \
   ((ins)->opcode >= OP_IBEQ && (ins)->opcode <= OP_IBLT_UN))
#define MONO_IS_COND_BRANCH_NOFP(ins)                                          \
  (MONO_IS_COND_BRANCH_OP(ins) &&                                              \
   !(((ins)->opcode >= OP_FBEQ) && ((ins)->opcode <= OP_FBLT_UN)))

#define MONO_IS_BRANCH_OP(ins)                                                 \
  (MONO_IS_COND_BRANCH_OP(ins) || ((ins)->opcode == OP_BR) ||                  \
   ((ins)->opcode == OP_BR_REG) || ((ins)->opcode == OP_SWITCH))

#define MONO_IS_COND_EXC(ins)                                                  \
  ((((ins)->opcode >= OP_COND_EXC_EQ) &&                                       \
    ((ins)->opcode <= OP_COND_EXC_LT_UN)) ||                                   \
   (((ins)->opcode >= OP_COND_EXC_IEQ) &&                                      \
    ((ins)->opcode <= OP_COND_EXC_ILT_UN)))

#define MONO_IS_SETCC(ins)                                                     \
  ((((ins)->opcode >= OP_CEQ) && ((ins)->opcode <= OP_CLT_UN)) ||              \
   (((ins)->opcode >= OP_ICEQ) && ((ins)->opcode <= OP_ICLE_UN)) ||            \
   (((ins)->opcode >= OP_LCEQ) && ((ins)->opcode <= OP_LCLT_UN)) ||            \
   (((ins)->opcode >= OP_FCEQ) && ((ins)->opcode <= OP_FCLT_UN)))

#define MONO_HAS_CUSTOM_EMULATION(ins)                                         \
  (((ins)->opcode >= OP_FBEQ && (ins)->opcode <= OP_FBLT_UN) ||                \
   ((ins)->opcode >= OP_FCEQ && (ins)->opcode <= OP_FCLT_UN))

#define MONO_IS_LOAD_MEMBASE(ins)                                              \
  (((ins)->opcode >= OP_LOAD_MEMBASE && (ins)->opcode <= OP_LOADV_MEMBASE) ||  \
   ((ins)->opcode >= OP_ATOMIC_LOAD_I1 && (ins)->opcode <= OP_ATOMIC_LOAD_R8))
#define MONO_IS_STORE_MEMBASE(ins)                                             \
  (((ins)->opcode >= OP_STORE_MEMBASE_REG &&                                   \
    (ins)->opcode <= OP_STOREV_MEMBASE) ||                                     \
   ((ins)->opcode >= OP_ATOMIC_STORE_I1 &&                                     \
    (ins)->opcode <= OP_ATOMIC_STORE_R8))
#define MONO_IS_STORE_MEMINDEX(ins)                                            \
  (((ins)->opcode >= OP_STORE_MEMINDEX) &&                                     \
   ((ins)->opcode <= OP_STORER8_MEMINDEX))

// This is internal because it is easily confused with any enum or integer.
#define MONO_IS_TAILCALL_OPCODE_INTERNAL(opcode)                               \
  ((opcode) == OP_TAILCALL || (opcode) == OP_TAILCALL_MEMBASE ||               \
   (opcode) == OP_TAILCALL_REG)

#define MONO_IS_TAILCALL_OPCODE(call)                                          \
  (MONO_IS_TAILCALL_OPCODE_INTERNAL(call->inst.opcode))

// OP_DYN_CALL is not a MonoCallInst
#define MONO_IS_CALL(ins)                                                      \
  (((ins)->opcode >= OP_VOIDCALL && (ins)->opcode <= OP_VCALL2_MEMBASE) ||     \
   MONO_IS_TAILCALL_OPCODE_INTERNAL((ins)->opcode))

#define MONO_IS_JUMP_TABLE(ins)                                                \
  (((ins)->opcode == OP_JUMP_TABLE)                                            \
       ? TRUE                                                                  \
       : ((((ins)->opcode == OP_AOTCONST) &&                                   \
           (ins->inst_i1 == (gpointer)MONO_PATCH_INFO_SWITCH))                 \
              ? TRUE                                                           \
              : ((ins)->opcode == OP_SWITCH)                                   \
                    ? TRUE                                                     \
                    : ((((ins)->opcode == OP_GOT_ENTRY) &&                     \
                        ((ins)->inst_right->inst_i1 ==                         \
                         (gpointer)MONO_PATCH_INFO_SWITCH))                    \
                           ? TRUE                                              \
                           : FALSE)))

#define MONO_JUMP_TABLE_FROM_INS(ins)                                          \
  (((ins)->opcode == OP_JUMP_TABLE)                                            \
       ? (ins)->inst_p0                                                        \
       : (((ins)->opcode == OP_AOTCONST) &&                                    \
                  (ins->inst_i1 == (gpointer)MONO_PATCH_INFO_SWITCH)           \
              ...
              : (((ins)->opcode == OP_SWITCH)                                  \
                     ...
                     : ((((ins)->opcode == OP_GOT_ENTRY) &&                    \
                         ((ins)->inst_right->inst_i1 ==                        \
                          (gpointer)MONO_PATCH_INFO_SWITCH))                   \
                            ? (ins)->inst_right->inst_p0                       \
                            : NULL))))

/* FIXME: Add more instructions */
/* INEG sets the condition codes, and the OP_LNEG decomposition depends on this
 * on x86 */
#define MONO_INS_HAS_NO_SIDE_EFFECT(ins)                                       \
  (MONO_IS_MOVE(ins) || (ins->opcode == OP_ICONST) ||                          \
   (ins->opcode == OP_I8CONST) || MONO_IS_ZERO(ins) ||                         \
   (ins->opcode == OP_ADD_IMM) || (ins->opcode == OP_R8CONST) ||               \
   (ins->opcode == OP_LADD_IMM) || (ins->opcode == OP_ISUB_IMM) ||             \
   (ins->opcode == OP_IADD_IMM) || (ins->opcode == OP_LNEG) ||                 \
   (ins->opcode == OP_ISUB) || (ins->opcode == OP_CMOV_IGE) ||                 \
   (ins->opcode == OP_ISHL_IMM) || (ins->opcode == OP_ISHR_IMM) ||             \
   (ins->opcode == OP_ISHR_UN_IMM) || (ins->opcode == OP_IAND_IMM) ||          \
   (ins->opcode == OP_ICONV_TO_U1) || (ins->opcode == OP_ICONV_TO_I1) ||       \
   (ins->opcode == OP_SEXT_I4) || (ins->opcode == OP_LCONV_TO_U1) ||           \
   (ins->opcode == OP_ICONV_TO_U2) || (ins->opcode == OP_ICONV_TO_I2) ||       \
   (ins->opcode == OP_LCONV_TO_I2) || (ins->opcode == OP_LDADDR) ||            \
   (ins->opcode == OP_PHI) || (ins->opcode == OP_NOP) ||                       \
   (ins->opcode == OP_ZEXT_I4) || (ins->opcode == OP_NOT_NULL) ||              \
   (ins->opcode == OP_IL_SEQ_POINT) || (ins->opcode == OP_XZERO))

#define MONO_INS_IS_PCONST_NULL(ins)                                           \
  ((ins)->opcode == OP_PCONST && (ins)->inst_p0 == 0)

#define MONO_METHOD_IS_FINAL(m)                                                \
  (((m)->flags & METHOD_ATTRIBUTE_FINAL) ||                                    \
   ((m)->klass && (mono_class_get_flags((m)->klass) & TYPE_ATTRIBUTE_SEALED)))

/* Determine whenever 'ins' represents a load of the 'this' argument */
#define MONO_CHECK_THIS(ins)                                                   \
  (mono_method_signature_internal(cfg->method)->hasthis &&                     \
   ((ins)->opcode == OP_MOVE) && ((ins)->sreg1 == cfg->args[0]->dreg))

#ifdef MONO_ARCH_SIMD_INTRINSICS

#define MONO_IS_PHI(ins)                                                       \
  (((ins)->opcode == OP_PHI) || ((ins)->opcode == OP_FPHI) ||                  \
   ((ins)->opcode == OP_VPHI) || ((ins)->opcode == OP_XPHI))
#define MONO_IS_MOVE(ins)                                                      \
  (((ins)->opcode == OP_MOVE) || ((ins)->opcode == OP_FMOVE) ||                \
   ((ins)->opcode == OP_VMOVE) || ((ins)->opcode == OP_XMOVE) ||               \
   ((ins)->opcode == OP_RMOVE))
#define MONO_IS_NON_FP_MOVE(ins)                                               \
  (((ins)->opcode == OP_MOVE) || ((ins)->opcode == OP_VMOVE) ||                \
   ((ins)->opcode == OP_XMOVE))
#define MONO_IS_REAL_MOVE(ins)                                                 \
  (((ins)->opcode == OP_MOVE) || ((ins)->opcode == OP_FMOVE) ||                \
   ((ins)->opcode == OP_XMOVE) || ((ins)->opcode == OP_RMOVE))
#define MONO_IS_ZERO(ins)                                                      \
  (((ins)->opcode == OP_VZERO) || ((ins)->opcode == OP_XZERO))

#define MONO_CLASS_IS_SIMD(cfg, klass)                                         \
  (((cfg)->opt & MONO_OPT_SIMD) && m_class_is_simd_type(klass))
...
#define MONO_IS_MOVE(ins)                                                      \
  (((ins)->opcode == OP_MOVE) || ((ins)->opcode == OP_FMOVE) ||                \
   ((ins)->opcode == OP_VMOVE) || ((ins)->opcode == OP_RMOVE))
#define MONO_IS_NON_FP_MOVE(ins)                                               \
  (((ins)->opcode == OP_MOVE) || ((ins)->opcode == OP_VMOVE))
...
#define EMIT_NEW_X86_LEA(cfg, dest, sr1, sr2, shift, imm)                      \
  do {                                                                         \
...
#ifndef DISABLE_JIT
#define mono_inst_get_num_src_registers(ins)                                   \
  (mini_ins_sreg_counts[(ins)->opcode - OP_START - 1])
#else
#define mono_inst_get_num_src_registers(ins) 0
#endif

#define mono_inst_get_src_registers(ins, regs)                                 \
  (((regs)[0] = (ins)->sreg1), ((regs)[1] = (ins)->sreg2),                     \
   ((regs)[2] = (ins)->sreg3), mono_inst_get_num_src_registers((ins)))

#define MONO_BB_FOR_EACH_INS(bb, ins)                                          \
  for ((ins) = (bb)->code; (ins); (ins) = (ins)->next)

#define MONO_BB_FOR_EACH_INS_SAFE(bb, n, ins)                                  \
  for ((ins) = (bb)->code, n = (ins) ? (ins)->next : NULL; (ins);              \
       (ins) = (n), (n) = (ins) ? (ins)->next : NULL)

#define MONO_BB_FOR_EACH_INS_REVERSE(bb, ins)                                  \
  for ((ins) = (bb)->last_ins; (ins); (ins) = (ins)->prev)

#define MONO_BB_FOR_EACH_INS_REVERSE_SAFE(bb, p, ins)                          \
  for ((ins) = (bb)->last_ins, p = (ins) ? (ins)->prev : NULL; (ins);          \
       (ins) = (p), (p) = (ins) ? (ins)->prev : NULL)

#define mono_bb_first_ins(bb) (bb)->code

/*
 * Iterate through all used registers in the instruction.
 * Relies on the existing order of the MONO_INST enum:
 * MONO_INST_{DREG,SREG1,SREG2,SREG3,LEN} INS is the instruction, IDX is the
 * register index, REG is the pointer to a register.
 */
#define MONO_INS_FOR_EACH_REG(ins, idx, reg)                                   \
  for ((idx) = INS_INFO((ins)->opcode)[MONO_INST_DEST] != ' '                  \
                   ? MONO_INST_DEST                                            \
                   : (mono_inst_get_num_src_registers(ins) ? MONO_INST_SRC1    \
                                                           : MONO_INST_LEN);   \
       (reg) = (idx) == MONO_INST_DEST                                         \
                   ? &(ins)->dreg                                              \
                   : ((idx) == MONO_INST_SRC1                                  \
                          ? &(ins)->sreg1                                      \
                          : ((idx) == MONO_INST_SRC2                           \
                                 ? &(ins)->sreg2                               \
                                 : ((idx) == MONO_INST_SRC3 ? &(ins)->sreg3    \
                                                            : NULL))),         \
      idx<MONO_INST_LEN; (idx) = (idx)> mono_inst_get_num_src_registers(ins) + \
               (INS_INFO((ins)->opcode)[MONO_INST_DEST] != ' ')                \
           ? MONO_INST_LEN                                                     \
           : (idx) + 1)
...
struct MonoBasicBlock {
  ...
  guint not_useless : 1;
  /* Whenever the decompose_array_access_opts () pass needs to process this
   * bblock */
  ...
  /*
   * If this is set, extend the try range started by this bblock by an arch
   * specific number of bytes to encompass the end of the previous bblock (e.g.
   * a Monitor.Enter call).
   */
...
struct MonoInst {
  ...
  union {
    ...
    guint16 source_opcode; /*OP_XCONV_R8_TO_I4 needs to know which op was used
                              to do proper widening*/
...
enum {
  ...
  MONO_INST_FAULT = 32,
  /*
   * On variables, identifies LMF variables. These variables have a dummy type
   * (int), but require stack space for a MonoLMF struct.
   */
  MONO_INST_LMF = 32,
  ...
  /*
   * Set on instructions during code emission which make calls, i.e. OP_CALL,
   * OP_THROW. backend.pc_offset will be set to the pc offset at the end of the
   * native call instructions.
   */
  MONO_INST_GC_CALLSITE = 128,
  /* On comparisons, mark the branch following the condition as likely to be
     taken */
...
static inline guint32 ins_get_l_low(MonoInst *ins) {
  return (guint32)(ins->data.i8const & 0xffffffff);
}

/* Return the higher 32 bits of the 64 bit immediate in INS */
static inline guint32 ins_get_l_high(MonoInst *ins) {
  return (guint32)((ins->data.i8const >> 32) & 0xffffffff);
}

static inline void mono_inst_set_src_registers(MonoInst *ins, int *regs) {
  ins->sreg1 = regs[0];
  ins->sreg2 = regs[1];
  ins->sreg3 = regs[2];
}
...
enum {
  /* Cannot be 0 since this is stored in rgctx slots, and 0 means an unitialized
     rgctx slot */
...
typedef enum {
  ...
  /* Either the code for a gsharedvt method, or the address for a gsharedvt-out
...
#define MONO_SIZEOF_METHOD_RUNTIME_GENERIC_CONTEXT                             \
  (MONO_ABI_SIZEOF(MonoMethodRuntimeGenericContext) -                          \
   MONO_ZERO_LEN_ARRAY * TARGET_SIZEOF_VOID_P)
...
/* This is used by gsharedvt methods to allocate locals and compute local
 * offsets */
typedef struct {
  int locals_size;
  /*
   * The results of resolving the entries in MOonGSharedVtMethodInfo->entries.
   * We use this instead of rgctx slots since these can be loaded using a load
   * instead of a call to an rgctx fetch trampoline.
   */
...
// Assuming MONO_TRAMPOLINE_JIT / MONO_JIT_ICALL_generic_trampoline_jit are
// first.
#if __cplusplus
g_static_assert(MONO_TRAMPOLINE_JIT == 0);
#endif
#define mono_trampoline_type_to_jit_icall_id(a)                                \
  ((a) + MONO_JIT_ICALL_generic_trampoline_jit)
#define mono_jit_icall_id_to_trampoline_type(a)                                \
  ((MonoTrampolineType)((a)-MONO_JIT_ICALL_generic_trampoline_jit))
...
#define MONO_TRAMPOLINE_TYPE_HAS_ARG(t) (FALSE)
...
#define MONO_BBLOCK_IS_IN_REGION(bblock, regtype)                              \
  (((bblock)->region & (0xf << 4)) == (regtype))

#define MONO_REGION_FLAGS(region) ((region)&0x7)
#define MONO_REGION_CLAUSE_INDEX(region) (((region) >> 8) - 1)

#define get_vreg_to_inst(cfg, vreg)                                            \
  ((vreg) < (cfg)->vreg_to_inst_len ? (cfg)->vreg_to_inst[(vreg)] : NULL)

#define vreg_is_volatile(cfg, vreg)                                            \
  (G_UNLIKELY(get_vreg_to_inst((cfg), (vreg)) &&                               \
              (get_vreg_to_inst((cfg), (vreg))->flags &                        \
               (MONO_INST_VOLATILE | MONO_INST_INDIRECT))))

#define vreg_is_ref(cfg, vreg)                                                 \
  ((vreg) < (cfg)->vreg_is_ref_len ? (cfg)->vreg_is_ref[(vreg)] : 0)
#define vreg_is_mp(cfg, vreg)                                                  \
  ((vreg) < (cfg)->vreg_is_mp_len ? (cfg)->vreg_is_mp[(vreg)] : 0)
...
typedef struct {
  ...
  MonoInst **locals;
  MonoInst *rgctx_var; /* Runtime generic context variable (for static generic
                          methods) */
  MonoInst **args;
  MonoType **arg_types;
  MonoMethod
      *current_method; /* The method currently processed by method_to_ir () */
  MonoMethod
      *method_to_register; /* The method to register in JIT info tables */
  ...
  /*
   * When this is set, the code to push/pop the LMF from the LMF stack is
   * generated as IR instead of being generated in emit_prolog ()/emit_epilog
   * ().
   */
  guint lmf_ir : 1;
  /*
   * Whenever to use the mono_lmf TLS variable instead of indirection through
   * the mono_lmf_addr TLS variable.
   */
  ...
  /*
   * The encoded GC map along with its size. This contains binary data so it can
   * be saved in an AOT image etc, but it requires a 4 byte alignment.
   */
  guint8 *gc_map;
  guint32 gc_map_size;
...
#define ins_get_spec(op)                                                       \
  ((const char *)&MONO_ARCH_CPU_SPEC[MONO_ARCH_CPU_SPEC_IDX(                   \
      MONO_ARCH_CPU_SPEC)[(op)-OP_LOAD]])

#ifndef DISABLE_JIT

static inline int ins_get_size(int opcode) {
  return ((guint8 *)ins_get_spec(opcode))[MONO_INST_LEN];
}

guint8 *mini_realloc_code_slow(MonoCompile *cfg, int size);

static inline guint8 *realloc_code(MonoCompile *cfg, int size) {
...
static inline void set_code_len(MonoCompile *cfg, int len) {
  g_assert(len <= cfg->code_size);
  cfg->code_len = len;
}

static inline void set_code_cursor(MonoCompile *cfg, void *void_code) {
...
enum { BRANCH_NOT_TAKEN, BRANCH_TAKEN, BRANCH_UNDEF };
...
typedef enum { CMP_TYPE_L, CMP_TYPE_I, CMP_TYPE_F } CompType;
...
static inline gboolean mono_inst_filter(MonoInst *ins, int filter) {
  if (!ins || !filter)
    return FALSE;
...
static inline MonoInst *mono_inst_next(MonoInst *ins, int filter) {
  do {
    ins = ins->next;
...
static inline MonoInst *mono_inst_prev(MonoInst *ins, int filter) {
  do {
    ins = ins->prev;
  } while (mono_inst_filter(ins, filter));
...
static inline MonoInst *mono_bb_first_inst(MonoBasicBlock *bb, int filter) {
...
static inline MonoInst *mono_bb_last_inst(MonoBasicBlock *bb, int filter) {
...
void mini_profiler_emit_call_finally(MonoCompile *cfg, MonoMethodHeader *header,
                                     unsigned char *ip, guint32 index,
                                     MonoExceptionClause *clause);
void mini_profiler_context_enable(void);
gpointer mini_profiler_context_get_this(MonoProfilerCallContext *ctx);
gpointer mini_profiler_context_get_argument(MonoProfilerCallContext *ctx,
                                            guint32 pos);
gpointer mini_profiler_context_get_local(MonoProfilerCallContext *ctx,
                                         guint32 pos);
...
MONO_LLVM_INTERNAL int mono_get_block_region_notry(MonoCompile *cfg,
                                                   int region);

MONO_LLVM_INTERNAL void mono_bblock_add_inst(MonoBasicBlock *bb,
                                             MonoInst *inst);
void mono_bblock_insert_after_ins(MonoBasicBlock *bb, MonoInst *ins,
                                  MonoInst *ins_to_insert);
void mono_bblock_insert_before_ins(MonoBasicBlock *bb, MonoInst *ins,
                                   MonoInst *ins_to_insert);
void mono_verify_bblock(MonoBasicBlock *bb);
void mono_verify_cfg(MonoCompile *cfg);
void mono_constant_fold(MonoCompile *cfg);
MonoInst *mono_constant_fold_ins(MonoCompile *cfg, MonoInst *ins,
                                 MonoInst *arg1, MonoInst *arg2,
                                 gboolean overwrite);
int mono_eval_cond_branch(MonoInst *branch);
MONO_LLVM_INTERNAL int mono_is_power_of_two(guint32 val);
void mono_cprop_local(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst **acp,
                      int acp_size);
MONO_LLVM_INTERNAL MonoInst *
...
void mono_link_bblock(MonoCompile *cfg, MonoBasicBlock *from,
                      MonoBasicBlock *to);
void mono_unlink_bblock(MonoCompile *cfg, MonoBasicBlock *from,
                        MonoBasicBlock *to);
...
void mono_merge_basic_blocks(MonoCompile *cfg, MonoBasicBlock *bb,
                             MonoBasicBlock *bbn);
void mono_optimize_branches(MonoCompile *cfg);

void mono_blockset_print(MonoCompile *cfg, MonoBitSet *set, const char *name,
                         guint idom);
...
MONO_LLVM_INTERNAL void mono_add_patch_info(MonoCompile *cfg, int ip,
                                            MonoJumpInfoType type,
                                            gconstpointer target);
MONO_LLVM_INTERNAL void mono_add_patch_info_rel(MonoCompile *cfg, int ip,
                                                MonoJumpInfoType type,
                                                gconstpointer target,
                                                int relocation);
void mono_remove_patch_info(MonoCompile *cfg, int ip);
gpointer mono_jit_compile_method_inner(MonoMethod *method,
                                       MonoDomain *target_domain, int opt,
                                       MonoError *error);
GList *mono_varlist_insert_sorted(MonoCompile *cfg, GList *list,
                                  MonoMethodVar *mv, int sort_type);
...
void mono_linear_scan(MonoCompile *cfg, GList *vars, GList *regs,
                      regmask_t *used_mask);
void mono_global_regalloc(MonoCompile *cfg);
void mono_create_jump_table(MonoCompile *cfg, MonoInst *label,
                            MonoBasicBlock **bbs, int num_blocks);
MonoCompile *mini_method_compile(MonoMethod *method, guint32 opts,
                                 MonoDomain *domain, JitFlags flags, int parts,
...
gboolean mini_assembly_can_skip_verification(MonoDomain *domain,
                                             MonoMethod *method);
gboolean mono_compile_is_broken(MonoCompile *cfg, MonoMethod *method,
                                gboolean fail_compile);
MonoInst *mono_get_got_var(MonoCompile *cfg);
void mono_add_seq_point(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst *ins,
                        int native_offset);
void mono_add_var_location(MonoCompile *cfg, MonoInst *var, gboolean is_reg,
                           int reg, int offset, int from, int to);
MonoInst *mono_emit_jit_icall_id(MonoCompile *cfg, MonoJitICallId jit_icall_id,
                                 MonoInst **args);
#define mono_emit_jit_icall(cfg, name, args)                                   \
  (mono_emit_jit_icall_id((cfg), MONO_JIT_ICALL_##name, (args)))

MonoInst *mono_emit_jit_icall_by_info(MonoCompile *cfg, int il_offset,
                                      MonoJitICallInfo *info, MonoInst **args);
MonoInst *mono_emit_method_call(MonoCompile *cfg, MonoMethod *method,
                                MonoInst **args, MonoInst *this_ins);
...
void mono_linterval_add_range(MonoCompile *cfg, MonoLiveInterval *interval,
                              int from, int to);
...
gint32 mono_linterval_get_intersect_pos(MonoLiveInterval *i1,
                                        MonoLiveInterval *i2);
void mono_linterval_split(MonoCompile *cfg, MonoLiveInterval *interval,
                          MonoLiveInterval **i1, MonoLiveInterval **i2,
                          int pos);
...
void mono_replace_ins(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst *ins,
                      MonoInst **prev, MonoBasicBlock *first_bb,
                      MonoBasicBlock *last_bb);

void mini_register_opcode_emulation(int opcode,
                                    MonoJitICallInfo *jit_icall_info,
                                    const char *name, MonoMethodSignature *sig,
                                    gpointer func, const char *symbol,
                                    gboolean no_throw);
...
mini_register_opcode_emulation(int opcode, MonoJitICallInfo *jit_icall_info,
                               const char *name, MonoMethodSignature *sig,
                               T func, const char *symbol, gboolean no_throw) {
  mini_register_opcode_emulation(opcode, jit_icall_info, name, sig,
                                 (gpointer)func, symbol, no_throw);
}
#endif // __cplusplus
...
gpointer mono_create_jump_trampoline(MonoDomain *domain, MonoMethod *method,
                                     gboolean add_sync_wrapper,
                                     MonoError *error);
gpointer mono_create_class_init_trampoline(MonoVTable *vtable);
MONO_LLVM_INTERNAL gpointer mono_create_jit_trampoline(MonoDomain *domain,
                                                       MonoMethod *method,
                                                       MonoError *error);
...
MonoDelegateTrampInfo *mono_create_delegate_trampoline_info(MonoDomain *domain,
                                                            MonoClass *klass,
                                                            MonoMethod *method);
gpointer mono_create_delegate_virtual_trampoline(MonoDomain *domain,
                                                 MonoClass *klass,
                                                 MonoMethod *method);
...
gpointer mono_magic_trampoline(host_mgreg_t *regs, guint8 *code, gpointer arg,
                               guint8 *tramp);
#ifndef DISABLE_REMOTING
gpointer mono_generic_virtual_remoting_trampoline(host_mgreg_t *regs,
                                                  guint8 *code, MonoMethod *m,
                                                  guint8 *tramp);
#endif
gpointer mono_delegate_trampoline(host_mgreg_t *regs, guint8 *code,
                                  gpointer *tramp_data, guint8 *tramp);
gpointer mono_aot_trampoline(host_mgreg_t *regs, guint8 *code,
                             guint8 *token_info, guint8 *tramp);
gpointer mono_aot_plt_trampoline(host_mgreg_t *regs, guint8 *code,
                                 guint8 *token_info, guint8 *tramp);
void mono_class_init_trampoline(host_mgreg_t *regs, guint8 *code,
                                MonoVTable *vtable, guint8 *tramp);
void mono_generic_class_init_trampoline(host_mgreg_t *regs, guint8 *code,
                                        MonoVTable *vtable, guint8 *tramp);
void mono_monitor_enter_trampoline(host_mgreg_t *regs, guint8 *code,
                                   MonoObject *obj, guint8 *tramp);
void mono_monitor_enter_v4_trampoline(host_mgreg_t *regs, guint8 *code,
                                      MonoObject *obj, guint8 *tramp);
void mono_monitor_exit_trampoline(host_mgreg_t *regs, guint8 *code,
                                  MonoObject *obj, guint8 *tramp);
...
gpointer mini_add_method_trampoline(MonoMethod *m, gpointer compiled_method,
                                    gboolean add_static_rgctx_tramp,
                                    gboolean add_unbox_tramp);
gboolean mini_jit_info_is_gsharedvt(MonoJitInfo *ji);
gpointer *mini_resolve_imt_method(MonoVTable *vt, gpointer *vtable_slot,
                                  MonoMethod *imt_method,
                                  MonoMethod **impl_method,
                                  gpointer *out_aot_addr,
                                  gboolean *out_need_rgctx_tramp,
                                  MonoMethod **variant_iface, MonoError *error);

void *mono_global_codeman_reserve(int size);

#define mono_global_codeman_reserve(size)                                      \
  (g_cast(mono_global_codeman_reserve((size))))

void mono_global_codeman_foreach(MonoCodeManagerFunc func, void *user_data);
const char *mono_regname_full(int reg, int bank);
gint32 *mono_allocate_stack_slots(MonoCompile *cfg, gboolean backward,
                                  guint32 *stack_size, guint32 *stack_align);
void mono_local_regalloc(MonoCompile *cfg, MonoBasicBlock *bb);
MonoInst *mono_branch_optimize_exception_target(MonoCompile *cfg,
                                                MonoBasicBlock *bb,
                                                const char *exname);
...
void mini_emit_memset(MonoCompile *cfg, int destreg, int offset, int size,
                      int val, int align);
...
MonoCallInst *mini_emit_call_args(MonoCompile *cfg, MonoMethodSignature *sig,
                                  MonoInst **args, gboolean calli,
                                  gboolean virtual_, gboolean tailcall,
                                  gboolean rgctx, gboolean unbox_trampoline,
                                  MonoMethod *target);
MonoInst *mini_emit_calli(MonoCompile *cfg, MonoMethodSignature *sig,
                          MonoInst **args, MonoInst *addr, MonoInst *imt_arg,
                          MonoInst *rgctx_arg);
MonoInst *mini_emit_calli_full(MonoCompile *cfg, MonoMethodSignature *sig,
                               MonoInst **args, MonoInst *addr,
                               MonoInst *imt_arg, MonoInst *rgctx_arg,
                               gboolean tailcall);
MonoInst *mini_emit_method_call_full(MonoCompile *cfg, MonoMethod *method,
                                     MonoMethodSignature *sig,
                                     gboolean tailcall, MonoInst **args,
                                     MonoInst *this_ins, MonoInst *imt_arg,
                                     MonoInst *rgctx_arg);
MonoInst *mini_emit_abs_call(MonoCompile *cfg, MonoJumpInfoType patch_type,
                             gconstpointer data, MonoMethodSignature *sig,
                             MonoInst **args);
MonoInst *mini_emit_extra_arg_calli(MonoCompile *cfg, MonoMethodSignature *fsig,
                                    MonoInst **orig_args, int arg_reg,
                                    MonoInst *call_target);
MonoInst *mini_emit_llvmonly_calli(MonoCompile *cfg, MonoMethodSignature *fsig,
                                   MonoInst **args, MonoInst *addr);
MonoInst *mini_emit_llvmonly_virtual_call(MonoCompile *cfg, MonoMethod *cmethod,
                                          MonoMethodSignature *fsig,
                                          int context_used, MonoInst **sp);
...
MonoInst *mini_emit_memory_load(MonoCompile *cfg, MonoType *type, MonoInst *src,
                                int offset, int ins_flag);
void mini_emit_memory_store(MonoCompile *cfg, MonoType *type, MonoInst *dest,
                            MonoInst *value, int ins_flag);
void mini_emit_memory_copy_bytes(MonoCompile *cfg, MonoInst *dest,
                                 MonoInst *src, MonoInst *size, int ins_flag);
void mini_emit_memory_init_bytes(MonoCompile *cfg, MonoInst *dest,
                                 MonoInst *value, MonoInst *size, int ins_flag);
void mini_emit_memory_copy(MonoCompile *cfg, MonoInst *dest, MonoInst *src,
                           MonoClass *klass, gboolean native, int ins_flag);
MonoInst *mini_emit_array_store(MonoCompile *cfg, MonoClass *klass,
                                MonoInst **sp, gboolean safety_checks);
MonoInst *mini_emit_inst_for_method(MonoCompile *cfg, MonoMethod *cmethod,
                                    MonoMethodSignature *fsig, MonoInst **args);
MonoInst *mini_emit_inst_for_ctor(MonoCompile *cfg, MonoMethod *cmethod,
                                  MonoMethodSignature *fsig, MonoInst **args);
#ifndef ENABLE_NETCORE
MonoInst *mini_emit_inst_for_sharable_method(MonoCompile *cfg,
                                             MonoMethod *cmethod,
                                             MonoMethodSignature *fsig,
                                             MonoInst **args);
#endif
MonoInst *mini_emit_inst_for_field_load(MonoCompile *cfg,
                                        MonoClassField *field);
MonoInst *mini_handle_enum_has_flag(MonoCompile *cfg, MonoClass *klass,
                                    MonoInst *enum_this, int enum_val_reg,
                                    MonoInst *enum_flag);
...
void mono_emit_unwind_op(MonoCompile *cfg, int when, int tag, int reg, int val);
...
MONO_LLVM_INTERNAL gboolean mini_type_is_hfa(MonoType *t, int *out_nfields,
                                             int *out_esize);
...
gboolean mini_class_has_reference_variant_generic_argument(MonoCompile *cfg,
                                                           MonoClass *klass,
                                                           int context_used);
...
MONO_LLVM_INTERNAL void mono_call_inst_add_outarg_reg(MonoCompile *cfg,
                                                      MonoCallInst *call,
                                                      int vreg, int hreg,
                                                      int bank);
void mono_call_inst_add_outarg_vt(MonoCompile *cfg, MonoCallInst *call,
                                  MonoInst *outarg_vt);
...
guchar *mono_arch_create_generic_trampoline(MonoTrampolineType tramp_type,
                                            MonoTrampInfo **info, gboolean aot);
...
guint8 *mono_arch_create_sdb_trampoline(gboolean single_step,
                                        MonoTrampInfo **info, gboolean aot);
...
void mono_arch_patch_code(MonoCompile *cfg, MonoMethod *method,
                          MonoDomain *domain, guint8 *code, MonoJumpInfo *ji,
...
MONO_LLVM_INTERNAL LLVMCallInfo *
mono_arch_get_llvm_call_info(MonoCompile *cfg, MonoMethodSignature *sig);
guint8 *mono_arch_emit_load_got_addr(guint8 *start, guint8 *code,
                                     MonoCompile *cfg, MonoJumpInfo **ji);
...
gpointer mono_arch_get_gsharedvt_call_info(gpointer addr,
...
#else
static inline MONO_ALWAYS_INLINE gboolean mono_arch_is_soft_float(void) {
  return FALSE;
}
#endif
...
gboolean mono_arch_unwind_frame(MonoDomain *domain, MonoJitTlsData *jit_tls,
...
gpointer mono_arch_get_rethrow_preserve_exception(MonoTrampInfo **info,
                                                  gboolean aot);
gpointer mono_arch_get_throw_corlib_exception(MonoTrampInfo **info,
                                              gboolean aot);
...
void mono_arch_handle_altstack_exception(void *sigctx,
...
gpointer *mono_arch_get_delegate_method_ptr_addr(guint8 *code,
                                                 host_mgreg_t *regs);
...
gpointer mono_arch_get_llvm_imt_trampoline(MonoDomain *domain,
                                           MonoMethod *method, int vt_offset);
gpointer mono_arch_get_gsharedvt_arg_trampoline(MonoDomain *domain,
                                                gpointer arg, gpointer addr);
void mono_arch_patch_callsite(guint8 *method_start, guint8 *code, guint8 *addr);
void mono_arch_patch_plt_entry(guint8 *code, gpointer *got, host_mgreg_t *regs,
                               guint8 *addr);
...
guint32 mono_arch_get_plt_info_offset(guint8 *plt_entry, host_mgreg_t *regs,
                                      guint8 *code);
...
void mono_arch_setup_async_callback(MonoContext *ctx,
                                    void (*async_cb)(void *fun),
                                    gpointer user_data);

gboolean mono_thread_state_init_from_handle(MonoThreadUnwindState *tctx,
                                            MonoThreadInfo *info,
                                            /*optional*/ void *sigctx);

/* Exception handling */
typedef gboolean (*MonoJitStackWalk)(StackFrameInfo *frame, MonoContext *ctx,
                                     gpointer data);
...
gpointer mono_altstack_restore_prot(host_mgreg_t *regs, guint8 *code,
                                    gpointer *tramp_data, guint8 *tramp);
...
MonoJitInfo *mini_jit_info_table_find_ext(MonoDomain *domain, gpointer addr,
                                          ...
                                          MonoDomain **out_domain);
G_EXTERN_C void
mono_resume_unwind(MonoContext *ctx) MONO_LLVM_INTERNAL_NO_EXTERN_C;
...
typedef gboolean (*MonoExceptionFrameWalk)(MonoMethod *method, gpointer ip,
                                           size_t native_offset,
                                           gboolean managed,
                                           gpointer user_data);
MONO_API gboolean mono_exception_walk_trace(MonoException *ex,
...
G_EXTERN_C gint32 mono_llvm_match_exception(MonoJitInfo *jinfo,
...
void mono_debug_open_block(MonoCompile *cfg, MonoBasicBlock *bb,
                           guint32 address);
void mono_debug_record_line_number(MonoCompile *cfg, MonoInst *ins,
                                   guint32 address);
void mono_debug_serialize_debug_info(MonoCompile *cfg, guint8 **out_buf,
                                     guint32 *buf_len);
void mono_debug_add_aot_method(MonoDomain *domain, MonoMethod *method,
                               guint8 *code_start, guint8 *debug_info,
...
MONO_API gboolean mono_breakpoint_clean_code(guint8 *method_start, guint8 *code,
...
extern void mono_perform_abc_removal(MonoCompile *cfg);
extern void mono_perform_abc_removal(MonoCompile *cfg);
extern void mono_local_cprop(MonoCompile *cfg);
extern void mono_local_cprop(MonoCompile *cfg);
extern void mono_local_deadce(MonoCompile *cfg);
void mono_local_alias_analysis(MonoCompile *cfg);
...
MONO_LLVM_INTERNAL gboolean mono_method_is_generic_sharable_full(
    MonoMethod *method, gboolean allow_type_vars, gboolean allow_partial,
...
gpointer mono_helper_get_rgctx_other_ptr(MonoClass *caller_class,
                                         MonoVTable *vtable, guint32 token,
...
mini_get_shared_method_full(MonoMethod *method, GetSharedMethodFlags flags,
                            MonoError *error);
...
void mono_cfg_add_try_hole(MonoCompile *cfg, MonoExceptionClause *clause,
                           guint8 *start, MonoBasicBlock *bb);
...
mini_get_gsharedvt_out_sig_wrapper_signature(gboolean has_this,
                                             gboolean has_ret, int param_count);
gboolean mini_gsharedvt_runtime_invoke_supported(MonoMethodSignature *sig);
G_EXTERN_C void mono_interp_entry_from_trampoline(gpointer ccontext,
                                                  gpointer imethod);
G_EXTERN_C void mono_interp_to_native_trampoline(gpointer addr,
                                                 gpointer ccontext);
...
enum {
  ...
  SIMD_VERSION_ALL = SIMD_VERSION_SSE1 | SIMD_VERSION_SSE2 | SIMD_VERSION_SSE3 |
                     SIMD_VERSION_SSSE3 | SIMD_VERSION_SSE41 |
                     SIMD_VERSION_SSE42 | SIMD_VERSION_SSE4a,
...
typedef enum {
  ...
  MONO_CPU_X86_PCLMUL_COMBINED =
      MONO_CPU_X86_SSE2_COMBINED | MONO_CPU_X86_PCLMUL,
  ...
  MONO_CPU_X86_SSSE3_COMBINED = MONO_CPU_X86_SSE3_COMBINED |
                                MONO_CPU_X86_PCLMUL | MONO_CPU_X86_AES |
                                MONO_CPU_X86_SSSE3,
  MONO_CPU_X86_SSE41_COMBINED =
      MONO_CPU_X86_SSSE3_COMBINED | MONO_CPU_X86_SSE41,
  MONO_CPU_X86_SSE42_COMBINED =
      MONO_CPU_X86_SSE41_COMBINED | MONO_CPU_X86_SSE42,
  MONO_CPU_X86_POPCNT_COMBINED =
      MONO_CPU_X86_SSE42_COMBINED | MONO_CPU_X86_POPCNT,
  MONO_CPU_X86_AVX_COMBINED = MONO_CPU_X86_SSE42_COMBINED | MONO_CPU_X86_AVX,
  MONO_CPU_X86_AVX2_COMBINED =
      MONO_CPU_X86_AVX_COMBINED | MONO_CPU_X86_POPCNT | MONO_CPU_X86_AVX2,
  MONO_CPU_X86_FMA_COMBINED =
      MONO_CPU_X86_AVX_COMBINED | MONO_CPU_X86_POPCNT | MONO_CPU_X86_FMA,
  MONO_CPU_X86_FULL_SSEAVX_COMBINED =
      MONO_CPU_X86_FMA_COMBINED | MONO_CPU_X86_AVX2,
#endif
#ifdef TARGET_WASM
...
void mono_simd_decompose_intrinsic(MonoCompile *cfg, MonoBasicBlock *bb,
                                   MonoInst *ins);
void mono_simd_decompose_intrinsics(MonoCompile *cfg);
MonoInst *mono_emit_simd_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
                                    MonoMethodSignature *fsig, MonoInst **args);
MonoInst *mono_emit_simd_field_load(MonoCompile *cfg, MonoClassField *field,
                                    MonoInst *addr);
...
MonoInst *mono_emit_native_types_intrinsics(MonoCompile *cfg,
                                            MonoMethod *cmethod,
                                            MonoMethodSignature *fsig,
                                            MonoInst **args);
...
MonoMethod *mini_method_to_shared(MonoMethod *method); // null if not shared

static inline gboolean mini_safepoints_enabled(void) {
