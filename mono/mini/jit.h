...
MONO_API MonoDomain *
mono_jit_init_version_for_test_only(const char *root_domain_name,
                                    const char *runtime_version);

MONO_API int mono_jit_exec(MonoDomain *domain, MonoAssembly *assembly, int argc,
                           char *argv[]);
...
typedef enum {
  ...
  /* Sentinel value used internally by the runtime. We use a large number to
     avoid clashing with some internal values. */
