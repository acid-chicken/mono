 ...
 * - all implicit arguments are passed in memory right after the signature
 * cookie, i.e. the stack will look like this: <argn>
 *   ..
 *   <arg1>
 *   <sig cookie>
 * - the OP_ARGLIST opcode in the callee computes the address of the sig cookie
 * argument on the stack and saves it into its sreg1.
 * - mono_ArgIterator_Setup receives this value and uses it to find the
 * signature and the arguments.
 */
...
MINI_OP(OP_STOREX_ALIGNED_MEMBASE_REG, "storex_aligned_membase_reg", IREG, XREG,
        NONE)
...
/* Similar to old OP_JMP, but the passing of arguments is done similarly to
 * calls */
MINI_OP(OP_TAILCALL, "tailcall", NONE, NONE, NONE)
MINI_OP(OP_TAILCALL_PARAMETER, "tailcall_parameter", NONE, NONE,
        NONE) // no code, just size
...
/* Variants of the original opcodes which take the two parts of the long as two
 * arguments */
...
/* 32 bit opcodes: must be in the same order as the matching CEE_ opcodes:
 * binops_op_map */
...
/* r4 opcodes: must be in the same order as the matching CEE_ opcodes:
 * binops_op_map */
...
/*these slow ops are modeled around the availability of a fast 2 bytes insert
 * op*/
...
/*SSE2 Shift ops must have the _reg version right after as code depends on this
 * ordering.*/
...
/* r4 dot product */
/* multiply all 4 single precision float elements, add them together, and store
 * the result to the lowest element */
...
/* sse 4.1 */
MINI_OP(OP_SSE41_ROUNDP, "roundp", XREG, XREG,
        NONE) // packed, inst_c0 - mode, inst_c1 - r4 or r8
MINI_OP(OP_SSE41_ROUNDS, "rounds", XREG, XREG,
        NONE) // scalar, inst_c0 - mode, inst_c1 - r4 or r8
 ...
 * sreg1 should be equal to dreg and models the fact the instruction doesn't
 * necessary modify dreg. The sreg1==dreg condition could be violated by SSA, so
 * the local register allocator or the code generator should generate a mov
 * dreg, sreg1 before the cmov in those cases. These opcodes operate on pointer
 * sized values.
 */
...
/* GC support */
/*
 * mono_arch_output_basic_block () will set the backend.pc_offset field to the
 * current pc offset.
 */
...
/*
 * This marks the location inside a basic block where a GC tracked spill slot
 * has been defined. The spill slot is assumed to be alive until the end of the
 * bblock.
 */
MINI_OP(OP_GC_SPILL_SLOT_LIVENESS_DEF, "gc_spill_slot_liveness_def", NONE, NONE,
        NONE)

/*
 * This marks the location inside a basic block where a GC tracked param area
 * slot has been defined. The slot is assumed to be alive until the next call.
 */
MINI_OP(OP_GC_PARAM_SLOT_LIVENESS_DEF, "gc_param_slot_liveness_def", NONE, NONE,
        NONE)
...
MINI_OP(OP_X86_COMPARE_MEMBASE8_IMM, "x86_compare_membase8_imm", NONE, IREG,
        NONE)
...
MINI_OP(OP_AMD64_ICOMPARE_MEMBASE_REG, "amd64_icompare_membase_reg", NONE, IREG,
        IREG)
MINI_OP(OP_AMD64_ICOMPARE_MEMBASE_IMM, "amd64_icompare_membase_imm", NONE, IREG,
        NONE)
MINI_OP(OP_AMD64_ICOMPARE_REG_MEMBASE, "amd64_icompare_reg_membase", NONE, IREG,
        IREG)
MINI_OP(OP_AMD64_COMPARE_MEMBASE_REG, "amd64_compare_membase_reg", NONE, IREG,
        IREG)
MINI_OP(OP_AMD64_COMPARE_MEMBASE_IMM, "amd64_compare_membase_imm", NONE, IREG,
        NONE)
MINI_OP(OP_AMD64_COMPARE_REG_MEMBASE, "amd64_compare_reg_membase", NONE, IREG,
        IREG)
...
/*
 * Generic SIMD operations, the rest of the JIT doesn't care about the exact
 * operation.
 */
