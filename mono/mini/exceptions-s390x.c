...
#define S390_CALLFILTER_FLTREGS                                                \
  (S390_CALLFILTER_INTREGS + (16 * sizeof(gulong)))
#define S390_CALLFILTER_ACCREGS                                                \
  (S390_CALLFILTER_FLTREGS + (16 * sizeof(gdouble)))
...
#define S390_THROWSTACK_FPCPRM                                                 \
  (S390_THROWSTACK_ACCPRM + sizeof(gpointer))                             // 168
#define S390_THROWSTACK_RETHROW (S390_THROWSTACK_FPCPRM + sizeof(gulong)) // 170
#define S390_THROWSTACK_PRESERVE_IPS                                           \
  (S390_THROWSTACK_RETHROW + sizeof(gulong)) // 178
#define S390_THROWSTACK_INTREGS                                                \
  (S390_THROWSTACK_PRESERVE_IPS + sizeof(gulong)) // 180
#define S390_THROWSTACK_FLTREGS                                                \
  (S390_THROWSTACK_INTREGS + (16 * sizeof(gulong))) // 308
#define S390_THROWSTACK_ACCREGS                                                \
  (S390_THROWSTACK_FLTREGS + (16 * sizeof(gdouble))) // 430
#define S390_THROWSTACK_SIZE                                                   \
  (S390_THROWSTACK_ACCREGS + (16 * sizeof(gint32))) // 494
...
static void throw_exception(MonoObject *, unsigned long, unsigned long,
                            host_mgreg_t *, gdouble *, gint32 *, guint,
                            gboolean, gboolean);
...
typedef enum { by_none, by_token } throwType;
...
static void throw_exception(MonoObject *exc, unsigned long ip, unsigned long sp,
                            host_mgreg_t *int_regs, gdouble *fp_regs,
                            gint32 *acc_regs, guint fpc, gboolean rethrow,
...
mono_arch_get_throw_exception_generic(int size, MonoTrampInfo **info,
  ...
  if (corlib) {
    ...
    S390_SET(code, s390_r2,
             (guint8 *)m_class_get_image(mono_defaults.exception_class));
    s390_basr(code, s390_r14, s390_r1);
  }
...
gpointer mono_arch_get_throw_exception(MonoTrampInfo **info, gboolean aot) {
  ...
  return (mono_arch_get_throw_exception_generic(SZ_THROW, info, FALSE, FALSE,
                                                aot, FALSE));
}
...
gpointer mono_arch_get_rethrow_preserve_exception(MonoTrampInfo **info,
  ...
  return (mono_arch_get_throw_exception_generic(SZ_THROW, info, FALSE, TRUE,
                                                aot, TRUE));
}
...
gpointer mono_arch_get_throw_corlib_exception(MonoTrampInfo **info,
  ...
  return (mono_arch_get_throw_exception_generic(SZ_THROW, info, TRUE, FALSE,
                                                aot, FALSE));
}
  ...
  *new_ctx = *ctx;
  ...
  if (ji != NULL) {
    ...
    gboolean success = mono_unwind_frame(
        unwind_info, unwind_info_len, ji->code_start,
        (guint8 *)ji->code_start + ji->code_size, ip, epilog ? &epilog : NULL,
        regs, 16, save_locations, MONO_MAX_IREGS, &cfa);
    ...
    memcpy(new_ctx->uc_mcontext.fpregs.fprs, (*lmf)->fregs,
           sizeof((*lmf)->fregs));
...
void mono_arch_handle_altstack_exception(void *sigctx,
  ...
  g_assert((uintptr_t)mono_arch_ip_from_context(uc) ==
  ...
  UCONTEXT_REG_Rn(uc, S390_FIRST_ARG_REG + 2) =
      (stack_ovf ? 1 : 0) | (nullref ? 2 : 0);
#endif
}
...
gboolean mono_arch_is_int_overflow(void *uc, void *info) {
  MonoContext *ctx;
  guint8 *code;
  guint64 *operand;
  gboolean arithExc = TRUE;
  gint regNo, idxNo, offset;
  ...
  case 0xb9: /* DL[GR] or DS[GR]         */
    if ((code[1] == 0x97) || (code[1] == 0x87) || (code[1] == 0x0d) ||
        (code[1] == 0x1d)) {
      regNo = (code[3] & 0x0f);
      if (ctx->uc_mcontext.gregs[regNo] == 0)
        arithExc = FALSE;
    ...
    break;
  case 0xe3: /* DL[G] | DS[G]  	    */
    if ((code[5] == 0x97) || (code[5] == 0x87) || (code[5] == 0x0d) ||
        (code[5] == 0x1d)) {
      regNo = (code[2] & 0xf0 >> 8);
      idxNo = (code[1] & 0x0f);
      offset = (code[2] & 0x0f << 8) + code[3] + (code[4] << 12);
      operand = (guint64 *)(ctx->uc_mcontext.gregs[regNo] + offset);
      if (idxNo != 0)
        operand += ctx->uc_mcontext.gregs[idxNo];
      if (*operand == 0)
