...
enum { TLS_MODE_DETECT, TLS_MODE_FAILED, TLS_MODE_LTHREADS, TLS_MODE_NPTL };
...
#define ins_is_compare(ins)                                                    \
  ((ins) &&                                                                    \
   (((ins)->opcode == OP_COMPARE) || ((ins)->opcode == OP_ICOMPARE) ||         \
    ((ins)->opcode == OP_LCOMPARE)))
#define ins_is_compare_imm(ins)                                                \
  ((ins) &&                                                                    \
   (((ins)->opcode == OP_COMPARE_IMM) || ((ins)->opcode == OP_ICOMPARE_IMM) || \
    ((ins)->opcode == OP_LCOMPARE_IMM)))

#define INS_REWRITE(ins, op, _s1, _s2)                                         \
  do {                                                                         \
    int s1 = _s1;                                                              \
    int s2 = _s2;                                                              \
    ins->opcode = (op);                                                        \
...
#define INS_REWRITE_IMM(ins, op, _s1, _imm)                                    \
  do {                                                                         \
    int s1 = _s1;                                                              \
    ins->opcode = (op);                                                        \
    ins->sreg1 = (s1);                                                         \
...
static guint8 *mono_arch_emit_epilog_sub(MonoCompile *cfg);
guint8 *mips_emit_cond_branch(MonoCompile *cfg, guint8 *code, int op,
                              MonoInst *ins);
void mips_adjust_stackframe(MonoCompile *cfg);
void mono_arch_emit_this_vret_args(MonoCompile *cfg, MonoCallInst *inst,
                                   int this_reg, int this_type, int vt_reg);
MonoInst *mono_arch_get_inst_for_method(MonoCompile *cfg, MonoMethod *cmethod,
...
static guint8 *mips_emit_exc_by_name(guint8 *code, const char *name) {
  gpointer addr;
  MonoClass *exc_class;
...
static guint8 *get_delegate_invoke_impl(MonoTrampInfo **info,
  ...
  if (has_target) {
    ...
    mips_lw(code, mips_temp, mips_a0,
...
static void args_onto_stack(CallInfo *info) {
...
static void add_int32_arg(CallInfo *info, ArgInfo *ainfo) {
...
static void add_int64_arg(CallInfo *info, ArgInfo *ainfo) {
  /* First, see if we need to drop onto the stack */
...
static void add_float32_arg(CallInfo *info, ArgInfo *ainfo) {
  /* First, see if we need to drop onto the stack */
  if (!info->on_stack && info->gr > MIPS_LAST_ARG_REG)
    args_onto_stack(info);
...
static void add_float64_arg(CallInfo *info, ArgInfo *ainfo) {
  /* First, see if we need to drop onto the stack */
  if (!info->on_stack && info->gr + 1 > MIPS_LAST_ARG_REG)
    args_onto_stack(info);
...
static CallInfo *get_call_info(MonoMemPool *mp, MonoMethodSignature *sig) {
  ...
  cinfo->stack_size = (cinfo->stack_size + MIPS_STACK_ALIGNMENT - 1) &
...
static void mono_arch_compute_omit_fp(MonoCompile *cfg) {
  ...
  if (!sig->pinvoke && (sig->call_convention == MONO_CALL_VARARG))
    cfg->arch.omit_fp = FALSE;
  /*
   * On MIPS, fp points to the bottom of the frame, so it can be eliminated even
...
static void emit_sig_cookie(MonoCompile *cfg, MonoCallInst *call,
  ...
  MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, mips_sp,
                              cinfo->sig_cookie.offset, sig_arg->dreg);
}
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (ainfo->storage == ArgInIReg) {
      ...
      if (!t->byref &&
        ...
        mono_call_inst_add_outarg_reg(cfg, call, ins->dreg,
                                      ainfo->reg + ls_word_idx, FALSE);
      ...
      if (!t->byref &&
          ((t->type == MONO_TYPE_I8) || (t->type == MONO_TYPE_U8))) {
        MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI8_MEMBASE_REG, mips_sp,
                                    ainfo->offset, in->dreg);
      } else if (!t->byref &&
                 ((t->type == MONO_TYPE_R4) || (t->type == MONO_TYPE_R8))) {
        if (t->type == MONO_TYPE_R8)
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, mips_sp,
                                      ainfo->offset, in->dreg);
        else
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER4_MEMBASE_REG, mips_sp,
                                      ainfo->offset, in->dreg);
      } else {
        MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, mips_sp,
                                    ainfo->offset, in->dreg);
...
#endif
  cfg->param_area = (cfg->param_area + MIPS_STACK_ALIGNMENT - 1) &
                    ~(MIPS_STACK_ALIGNMENT - 1);
...
void mono_arch_emit_outarg_vt(MonoCompile *cfg, MonoInst *ins, MonoInst *src) {
  ...
  if (ainfo->storage == ArgStructByVal) {
    ...
    if (ovf_size != 0) {
      mini_emit_memcpy(cfg, mips_sp, doffset, src->dreg, soffset,
    ...
    if (ainfo->offset)
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, mips_at,
...
void mono_arch_decompose_long_opts(MonoCompile *cfg, MonoInst *ins) {
  ...
  case OP_LADD:
    ...
    MONO_EMIT_NEW_BIALU(cfg, OP_IADD, ins->dreg + 1, ins->sreg1 + 1,
                        ins->sreg2 + 1);
    MONO_EMIT_NEW_BIALU(cfg, OP_MIPS_SLTU, tmp1, ins->dreg + 1, ins->sreg1 + 1);
    MONO_EMIT_NEW_BIALU(cfg, OP_IADD, ins->dreg + 2, ins->sreg1 + 2,
                        ins->sreg2 + 2);
  ...
  case OP_LADD_IMM:
    tmp1 = mono_alloc_ireg(cfg);
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_IADD_IMM, ins->dreg + 1, ins->sreg1 + 1,
                            ins_get_l_low(ins));
    MONO_EMIT_NEW_BIALU(cfg, OP_MIPS_SLTU, tmp1, ins->dreg + 1, ins->sreg1 + 1);
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_IADD_IMM, ins->dreg + 2, ins->sreg1 + 2,
                            ins_get_l_high(ins));
    MONO_EMIT_NEW_BIALU(cfg, OP_IADD, ins->dreg + 2, ins->dreg + 2, tmp1);
    NULLIFY_INS(ins);
    break;
  ...
  case OP_LSUB:
    tmp1 = mono_alloc_ireg(cfg);
    MONO_EMIT_NEW_BIALU(cfg, OP_ISUB, ins->dreg + 1, ins->sreg1 + 1,
                        ins->sreg2 + 1);
    MONO_EMIT_NEW_BIALU(cfg, OP_MIPS_SLTU, tmp1, ins->sreg1 + 1, ins->dreg + 1);
    MONO_EMIT_NEW_BIALU(cfg, OP_ISUB, ins->dreg + 2, ins->sreg1 + 2,
                        ins->sreg2 + 2);
  ...
  case OP_LSUB_IMM:
    tmp1 = mono_alloc_ireg(cfg);
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ISUB_IMM, ins->dreg + 1, ins->sreg1 + 1,
                            ins_get_l_low(ins));
    MONO_EMIT_NEW_BIALU(cfg, OP_MIPS_SLTU, tmp1, ins->sreg1 + 1, ins->dreg + 1);
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ISUB_IMM, ins->dreg + 2, ins->sreg1 + 2,
                            ins_get_l_high(ins));
    MONO_EMIT_NEW_BIALU(cfg, OP_ISUB, ins->dreg + 2, ins->dreg + 2, tmp1);
    NULLIFY_INS(ins);
    break;
  ...
  case OP_LCONV_TO_OVF_I4_2:
    ...
    MONO_EMIT_NEW_COMPARE_EXC(cfg, NE_UN, ins->sreg2, tmp1,
                              "OverflowException");
...
static int map_to_mips_op(int op) {
  switch (op) {
  case OP_FBEQ:
    return OP_MIPS_FBEQ;
  case OP_FBGE:
...
#define NEW_INS(cfg, after, dest, op)                                          \
  do {                                                                         \
    MONO_INST_NEW((cfg), (dest), (op));                                        \
    mono_bblock_insert_after_ins(bb, (after), (dest));                         \
  } while (0)

#define INS(pos, op, _dreg, _sreg1, _sreg2)                                    \
  do {                                                                         \
    MonoInst *temp;                                                            \
...
#define INS_IMM(pos, op, _dreg, _sreg1, _imm)                                  \
  do {                                                                         \
    MonoInst *temp;                                                            \
    MONO_INST_NEW(cfg, temp, (op));                                            \
    mono_bblock_insert_after_ins(bb, (pos), temp);                             \
...
void mono_arch_lowering_pass(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  if (cfg->verbose_level > 2) {
    ...
    MONO_BB_FOR_EACH_INS(bb, ins) { mono_print_ins_index(idx++, ins); }
  }
#endif
  ...
  loop_start:
    ...
    case OP_COND_EXC_INE_UN:
      g_assert(ins_is_compare(last_ins));
      INS_REWRITE(ins, OP_MIPS_COND_EXC_NE_UN, last_ins->sreg1,
                  last_ins->sreg2);
      MONO_DELETE_INS(bb, last_ins);
      break;
    ...
    case OP_COND_EXC_IGE_UN:
      g_assert(ins_is_compare(last_ins));
      INS_REWRITE(ins, OP_MIPS_COND_EXC_GE_UN, last_ins->sreg1,
                  last_ins->sreg2);
      MONO_DELETE_INS(bb, last_ins);
      break;
    ...
    case OP_COND_EXC_IGT_UN:
      g_assert(ins_is_compare(last_ins));
      INS_REWRITE(ins, OP_MIPS_COND_EXC_GT_UN, last_ins->sreg1,
                  last_ins->sreg2);
      MONO_DELETE_INS(bb, last_ins);
      break;
    ...
    case OP_COND_EXC_ILE_UN:
      g_assert(ins_is_compare(last_ins));
      INS_REWRITE(ins, OP_MIPS_COND_EXC_LE_UN, last_ins->sreg1,
                  last_ins->sreg2);
      MONO_DELETE_INS(bb, last_ins);
      break;
    ...
    case OP_COND_EXC_ILT_UN:
      g_assert(ins_is_compare(last_ins));
      INS_REWRITE(ins, OP_MIPS_COND_EXC_LT_UN, last_ins->sreg1,
                  last_ins->sreg2);
      MONO_DELETE_INS(bb, last_ins);
      break;
 ...
 */
static guint8 *emit_load_volatile_arguments(MonoCompile *cfg, guint8 *code) {
    ...
    if (inst->opcode == OP_REGVAR) {
      ...
      if (ainfo->storage == ArgInIReg) {
        ...
        case 4:
          ...
          break;
        case 8:
          mips_lw(code, ainfo->reg, inst->inst_basereg,
                  inst->inst_offset + ls_word_offset);
          mips_lw(code, ainfo->reg + 1, inst->inst_basereg,
                  inst->inst_offset + ms_word_offset);
          break;
        default:
          g_assert_not_reached();
          break;
        ...
        if (ainfo->size == 8) {
#if _MIPS_SIM == _ABIO32
          mips_lwc1(code, ainfo->reg, inst->inst_basereg,
                    inst->inst_offset + ls_word_offset);
          mips_lwc1(code, ainfo->reg + 1, inst->inst_basereg,
                    inst->inst_offset + ms_word_offset);
#elif _MIPS_SIM == _ABIN32
        ...
        g_assert(mips_is_imm16(inst->inst_offset));
        g_assert(mips_is_imm16(inst->inst_offset +
                               ainfo->size * sizeof(target_mgreg_t)));
        for (i = 0; i < ainfo->size; ++i) {
...
void mono_arch_output_basic_block(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS(bb, ins) {
    ...
    case OP_CALL_HANDLER:
      ...
      mips_jalr(code, mips_t9, mips_ra);
      mips_nop(code);
      /*FIXME should it be before the NOP or not? Does MIPS has a delay slot
       * like sparc?*/
    ...
    case OP_STORER8_MEMBASE_REG:
      if (mips_is_imm16(ins->inst_offset)) {
#if _MIPS_SIM == _ABIO32
        mips_swc1(code, ins->sreg1, ins->inst_destbasereg,
                  ins->inst_offset + ls_word_offset);
        mips_swc1(code, ins->sreg1 + 1, ins->inst_destbasereg,
                  ins->inst_offset + ms_word_offset);
#elif _MIPS_SIM == _ABIN32
    ...
    case OP_LOADR8_MEMBASE:
      if (mips_is_imm16(ins->inst_offset)) {
#if _MIPS_SIM == _ABIO32
        mips_lwc1(code, ins->dreg, ins->inst_basereg,
                  ins->inst_offset + ls_word_offset);
        mips_lwc1(code, ins->dreg + 1, ins->inst_basereg,
                  ins->inst_offset + ms_word_offset);
#elif _MIPS_SIM == _ABIN32
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    MONO_BB_FOR_EACH_INS(bb, ins) {
      ...
      if (MONO_IS_LOAD_MEMBASE(ins) &&
          ((ins->inst_basereg == mips_fp) || (ins->inst_basereg == mips_sp))) {
        switch (ins->opcode) {
        case OP_LOADI8_MEMBASE:
        case OP_LOADR8_MEMBASE:
          adj_c0 = 8;
      ...
      } else if (MONO_IS_STORE_MEMBASE(ins) &&
                 ((ins->dreg == mips_fp) || (ins->dreg == mips_sp))) {
        switch (ins->opcode) {
        case OP_STOREI8_MEMBASE_REG:
        case OP_STORER8_MEMBASE_REG:
        case OP_STOREI8_MEMBASE_IMM:
      ...
      if (((ins->opcode == OP_ADD_IMM) || (ins->opcode == OP_IADD_IMM)) &&
...
guint8 *mono_arch_emit_prolog(MonoCompile *cfg) {
  ...
  /*
   * Currently, fp points to the bottom of the frame on MIPS, unlike other
   * platforms. This means that we have to adjust the offsets inside
   * instructions which reference arguments received on the stack, since the
   * initial offset doesn't take into account spill slots.
   */
  ...
  for (i = 0; i < sig->param_count + sig->hasthis; ++i) {
    ...
    if (inst->opcode == OP_REGVAR) {
      ...
      if (ainfo->storage == ArgInIReg) {
        ...
        case 8:
#if (SIZEOF_REGISTER == 4)
          mips_sw(code, ainfo->reg, inst->inst_basereg,
                  basereg_offset + ls_word_offset);
          mips_sw(code, ainfo->reg + 1, inst->inst_basereg,
                  basereg_offset + ms_word_offset);
        ...
        if (ainfo->size == 8) {
#if _MIPS_SIM == _ABIO32
          mips_swc1(code, ainfo->reg, inst->inst_basereg,
                    inst->inst_offset + ls_word_offset);
          mips_swc1(code, ainfo->reg + 1, inst->inst_basereg,
                    inst->inst_offset + ms_word_offset);
#elif _MIPS_SIM == _ABIN32
        ...
        code =
            emit_memcpy(code, ainfo->vtsize * sizeof(target_mgreg_t),
  ...
  if (method->save_lmf) {
    ...
    mips_sw(code, mips_at, mips_sp,
            lmf_offset + G_STRUCT_OFFSET(MonoLMF, magic));
    ...
    mips_sw(code, mips_v0, mips_sp,
            lmf_offset + G_STRUCT_OFFSET(MonoLMF, lmf_addr));
    /* new_lmf->previous_lmf = *lmf_addr */
    mips_lw(code, mips_at, mips_v0, 0);
    g_assert(
        mips_is_imm16(lmf_offset + G_STRUCT_OFFSET(MonoLMF, previous_lmf)));
    mips_sw(code, mips_at, mips_sp,
...
guint8 *mono_arch_emit_epilog_sub(MonoCompile *cfg) {
  ...
  if (method->save_lmf) {
    ...
    g_assert(
        mips_is_imm16(lmf_offset + G_STRUCT_OFFSET(MonoLMF, previous_lmf)));
    mips_lw(code, mips_temp, mips_sp,
            lmf_offset + G_STRUCT_OFFSET(MonoLMF, previous_lmf));
    /* t1 = lmf_addr */
    g_assert(mips_is_imm16(lmf_offset + G_STRUCT_OFFSET(MonoLMF, lmf_addr)));
    mips_lw(code, mips_t1, mips_sp,
            lmf_offset + G_STRUCT_OFFSET(MonoLMF, lmf_addr));
  ...
  if ((cfg->flags & MONO_CFG_HAS_CALLS) || ALWAYS_SAVE_RA) {
    g_assert(
        mips_is_imm16(cfg->stack_usage - alloc2_size + MIPS_RET_ADDR_OFFSET));
    mips_lw(code, mips_ra, mips_sp,
            cfg->stack_usage - alloc2_size + MIPS_RET_ADDR_OFFSET);
  }
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (item->is_equals) {
      if (item->check_target_idx) {
        ...
        mips_nop(code);
        if (item->has_target_code) {
          mips_load_const(code, mips_t9, item->value.target_code);
        } else {
