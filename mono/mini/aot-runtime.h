...
typedef struct MonoAotFileInfo {
  /* The version number of the AOT file format, should match
...
#define MONO_AOT_FILE_INFO_FIRST_SYMBOL jit_got
  ...
  /*
   * The runtime version string for AOT images generated using
   * 'bind-to-runtime-version', NULL otherwise.
   */
...
#define MONO_AOT_FILE_INFO_LAST_SYMBOL unbox_trampoline_addresses
  ...
  guint32 double_align, long_align, generic_tramp_num, card_table_shift_bits,
      card_table_mask;
  ...
  /* Arrays */
  /* Offsets for tables inside the data file if MONO_AOT_FILE_FLAG_SEPARATE_DATA
   * is set */
  // FIXME: Sync with AOT
...
#define MONO_AOT_FILE_INFO_NUM_SYMBOLS                                         \
  (((G_STRUCT_OFFSET(MonoAotFileInfo, MONO_AOT_FILE_INFO_LAST_SYMBOL) -        \
     G_STRUCT_OFFSET(MonoAotFileInfo, MONO_AOT_FILE_INFO_FIRST_SYMBOL)) /      \
    sizeof(gpointer)) +                                                        \
   1)
...
gboolean mono_aot_init_llvmonly_method(gpointer amodule, guint32 method_index,
...
/* These are used to load the AOT data for aot images compiled with
 * MONO_AOT_FILE_FLAG_SEPARATE_DATA */
/*
 * Return the AOT data for ASSEMBLY. SIZE is the size of the data. OUT_HANDLE
 * should be set to a handle which is later passed to the free function.
 */
typedef unsigned char *(*MonoLoadAotDataFunc)(MonoAssembly *assembly, int size,
                                              gpointer user_data,
                                              void **out_handle);
/* Not yet used */
typedef void (*MonoFreeAotDataFunc)(MonoAssembly *assembly, int size,
                                    gpointer user_data, void *handle);
MONO_API void mono_install_load_aot_data_hook(MonoLoadAotDataFunc load_func,
                                              MonoFreeAotDataFunc free_func,
                                              gpointer user_data);

#ifdef MONO_ARCH_CODE_EXEC_ONLY
typedef guint32 (*MonoAotResolvePltInfoOffset)(gpointer amodule,
