...
LLVMValueRef mono_llvm_build_alloca(LLVMBuilderRef builder, LLVMTypeRef Ty,
  ...
  return wrap(unwrap(builder)->Insert(
      new AllocaInst(unwrap(Ty), 0, unwrap(ArraySize), alignment), Name));
}
...
LLVMValueRef mono_llvm_build_load(LLVMBuilderRef builder,
  ...
  LoadInst *ins =
      unwrap(builder)->CreateLoad(unwrap(PointerVal), is_volatile, Name);
...
LLVMValueRef mono_llvm_build_store(LLVMBuilderRef builder, LLVMValueRef Val,
  ...
  StoreInst *ins = unwrap(builder)->CreateStore(unwrap(Val), unwrap(PointerVal),
                                                is_volatile);
...
LLVMValueRef mono_llvm_build_aligned_store(LLVMBuilderRef builder,
  ...
  ins = unwrap(builder)->CreateStore(unwrap(Val), unwrap(PointerVal),
                                     is_volatile);
  ins->setAlignment(alignment);
...
LLVMValueRef mono_llvm_build_cmpxchg(LLVMBuilderRef builder, LLVMValueRef ptr,
                                     LLVMValueRef cmp, LLVMValueRef val) {
  AtomicCmpXchgInst *ins;

  ins = unwrap(builder)->CreateAtomicCmpXchg(
      unwrap(ptr), unwrap(cmp), unwrap(val), SequentiallyConsistent,
...
LLVMValueRef mono_llvm_build_atomic_rmw(LLVMBuilderRef builder, AtomicRMWOp op,
  ...
  ins = unwrap(builder)->CreateAtomicRMW(aop, unwrap(ptr), unwrap(val),
                                         SequentiallyConsistent);
  return wrap(ins);
}
...
void mono_llvm_set_implicit_branch(LLVMBuilderRef builder,
                                   LLVMValueRef branch) {
  auto b = unwrap(builder);
  auto &ctx = b->getContext();
...
void mono_llvm_replace_uses_of(LLVMValueRef var, LLVMValueRef v) {
  Value *V = ConstantExpr::getTruncOrBitCast(unwrap<Constant>(v),
                                             unwrap(var)->getType());
  unwrap(var)->replaceAllUsesWith(V);
}

LLVMValueRef mono_llvm_create_constant_data_array(const uint8_t *data,
                                                  int len) {
  return wrap(ConstantDataArray::get(*unwrap(LLVMGetGlobalContext()),
                                     makeArrayRef(data, len)));
}
...
void mono_llvm_set_call_nonnull_arg(LLVMValueRef wrapped_calli, int argNo) {
...
void mono_llvm_set_call_nonnull_ret(LLVMValueRef wrapped_calli) {
  Instruction *calli = unwrap<Instruction>(wrapped_calli);

  if (isa<CallInst>(calli))
    dyn_cast<CallInst>(calli)->addAttribute(AttributeList::ReturnIndex,
                                            Attribute::NonNull);
  else
    dyn_cast<InvokeInst>(calli)->addAttribute(AttributeList::ReturnIndex,
                                              Attribute::NonNull);
}
...
gboolean mono_llvm_can_be_gep(LLVMValueRef base, LLVMValueRef *gep_base,
                              LLVMValueRef *gep_offset) {
  ...
  if (Instruction *base_inst = dyn_cast<Instruction>(unwrap(base))) {
    if (base_inst->getOpcode() == Instruction::Add) {
      if (Instruction *base_ptr_ins =
              dyn_cast<Instruction>(base_inst->getOperand(0))) {
...
gboolean mono_llvm_is_nonnull(LLVMValueRef wrapped) {
...
LLVMValueRef *mono_llvm_call_args(LLVMValueRef wrapped_calli) {
...
void mono_llvm_set_call_notailcall(LLVMValueRef func) {
...
void mono_llvm_set_alignment_ret(LLVMValueRef call, int alignment) {
  Instruction *ins = unwrap<Instruction>(call);
  auto &ctx = ins->getContext();
  if (isa<CallInst>(ins))
    dyn_cast<CallInst>(ins)->addAttribute(
        AttributeList::ReturnIndex,
        Attribute::getWithAlignment(ctx, alignment));
  else
    dyn_cast<InvokeInst>(ins)->addAttribute(
        AttributeList::ReturnIndex,
        Attribute::getWithAlignment(ctx, alignment));
}

static Attribute::AttrKind convert_attr(AttrKind kind) {
  switch (kind) {
...
void mono_llvm_add_func_attr(LLVMValueRef func, AttrKind kind) {
  unwrap<Function>(func)->addAttribute(AttributeList::FunctionIndex,
                                       convert_attr(kind));
}
...
void *mono_llvm_di_create_compile_unit(void *di_builder, const char *cu_name,
  ...
  return builder->createCompileUnit(dwarf::DW_LANG_C99, di_file, producer, true,
                                    "", 0);
}
...
void *mono_llvm_di_create_function(void *di_builder, void *cu,
  ...
  type = builder->createSubroutineType(
      builder->getOrCreateTypeArray(ArrayRef<Metadata *>()));
#if LLVM_API_VERSION >= 900
  di_func = builder->createFunction(di_file, name, mangled_name, di_file, line,
                                    type, 0);
#else
  di_func = builder->createFunction(di_file, name, mangled_name, di_file, line,
                                    type, true, true, 0);
...
void *mono_llvm_di_create_location(void *di_builder, void *scope, int row,
                                   int column) {
  return DILocation::get(*unwrap(LLVMGetGlobalContext()), row, column,
                         (Metadata *)scope);
}
...
LLVMValueRef mono_llvm_get_or_insert_gc_safepoint_poll(LLVMModuleRef module) {
  ...
  llvm::FunctionCallee callee = unwrap(module)->getOrInsertFunction(
      "gc.safepoint_poll", FunctionType::get(unwrap(LLVMVoidType()), false));
  return wrap(dyn_cast<llvm::Function>(callee.getCallee()));
#else
  llvm::Function *SafepointPoll;
  llvm::Constant *SafepointPollConstant;

  SafepointPollConstant = unwrap(module)->getOrInsertFunction(
      "gc.safepoint_poll", FunctionType::get(unwrap(LLVMVoidType()), false));
...
static Intrinsic::ID get_intrins_id(IntrinsicId id) {
  Intrinsic::ID intrins_id = Intrinsic::ID::not_intrinsic;
  switch (id) {
#define INTRINS(id, llvm_id)                                                   \
  case INTRINS_##id:                                                           \
    intrins_id = Intrinsic::ID::llvm_id;                                       \
    break;
#define INTRINS_OVR(id, llvm_id)                                               \
  case INTRINS_##id:                                                           \
    intrins_id = Intrinsic::ID::llvm_id;                                       \
    break;
...
static bool is_overloaded_intrins(IntrinsicId id) {
  switch (id) {
#define INTRINS(id, llvm_id)
#define INTRINS_OVR(id, llvm_id)                                               \
  case INTRINS_##id:                                                           \
 ...
 */
LLVMValueRef mono_llvm_register_intrinsic(LLVMModuleRef module,
                                          IntrinsicId id) {
 ...
 * mono_llvm_register_intrinsic:
 *
 *   Register an overloaded LLVM intrinsic identified by ID using the supplied
 * types.
 */
LLVMValueRef mono_llvm_register_overloaded_intrinsic(LLVMModuleRef module,
  ...
  auto f = Intrinsic::getDeclaration(unwrap(module), intrins_id,
                                     {arr, (size_t)ntypes});
  return wrap(f);
}
