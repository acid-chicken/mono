...
typedef struct ppc_vector_state {
        ...
        unsigned int    save_vrvalid;                   // VRs that have been
saved unsigned int    save_pad6[7]; } ppc_vector_state_t;
...
#define restore_regs_from_context(ctx_reg, ip_reg, tmp_reg)                    \
  do {                                                                         \
    int reg;                                                                   \
    ...
    for (reg = MONO_PPC_FIRST_SAVED_FREG; reg < MONO_MAX_FREGS; ++reg) {       \
      ppc_lfd(code, reg,                                                       \
              G_STRUCT_OFFSET(MonoContext, fregs) + reg * sizeof(gdouble),     \
              ctx_reg);                                                        \
    }                                                                          \
  } while (0)
...
guint8 *mono_ppc_create_pre_code_ftnptr(guint8 *code) {
...
#define SAVED_REGS_LENGTH                                                      \
  (sizeof(gdouble) * MONO_MAX_FREGS + sizeof(target_mgreg_t) * MONO_MAX_IREGS)
#define ALIGN_STACK_FRAME_SIZE(s)                                              \
  (((s) + MONO_ARCH_FRAME_ALIGNMENT - 1) & ~(MONO_ARCH_FRAME_ALIGNMENT - 1))
/* The 64 bytes here are for outgoing arguments and a bit of spare.
   We don't use it all, but it doesn't hurt. */
#define REG_SAVE_STACK_FRAME_SIZE                                              \
  (ALIGN_STACK_FRAME_SIZE(SAVED_REGS_LENGTH + PPC_MINIMAL_STACK_SIZE + 64))

static guint8 *emit_save_saved_regs(guint8 *code, int pos) {
  int i;
...
void mono_ppc_throw_exception(MonoObject *exc, unsigned long eip,
                              unsigned long esp, host_mgreg_t *int_regs,
                              gdouble *fp_regs, gboolean rethrow,
...
mono_arch_get_throw_exception_generic(int size, MonoTrampInfo **info,
                                      int corlib, gboolean rethrow,
                                      gboolean aot, gboolean preserve_ips) {
  guint8 *start, *code;
  int alloc_size, pos;
  MonoJumpInfo *ji = NULL;
  GSList *unwind_ops = NULL;
  ...
  if (corlib) {
    ...
    if (aot) {
      code = mono_arch_emit_load_aotconst(
          start, code, &ji, MONO_PATCH_INFO_IMAGE, mono_defaults.corlib);
  ...
  if (info)
    *info = mono_tramp_info_create(
        corlib ? "throw_corlib_exception"
               : (preserve_ips
  ...
  *new_ctx = *ctx;
  ...
  if (ji != NULL) {
    ...
    if (!ji->is_trampoline && jinfo_get_method(ji)->save_lmf) {
      ...
      memcpy(&new_ctx->fregs[MONO_PPC_FIRST_SAVED_FREG],
             lmf_addr + G_STRUCT_OFFSET(MonoLMF, fregs),
             sizeof(double) * MONO_SAVED_FREGS);
      memcpy(&new_ctx->regs[MONO_PPC_FIRST_SAVED_GREG],
             lmf_addr + G_STRUCT_OFFSET(MonoLMF, iregs),
             sizeof(host_mgreg_t) * MONO_SAVED_GREGS);
      ...
      gboolean success = mono_unwind_frame(
          ...
          ppc_lr + 1, save_locations, MONO_MAX_IREGS, &cfa);
    ...
    memcpy(&new_ctx->regs[MONO_PPC_FIRST_SAVED_GREG], (*lmf)->iregs,
           sizeof(host_mgreg_t) * MONO_SAVED_GREGS);
    memcpy(&new_ctx->fregs[MONO_PPC_FIRST_SAVED_FREG], (*lmf)->fregs,
           sizeof(double) * MONO_SAVED_FREGS);
...
static void altstack_handle_and_restore(MonoContext *mctx, gpointer obj) {
...
void mono_arch_handle_altstack_exception(void *sigctx,
  ...
  MonoJitInfo *ji = mini_jit_info_table_find(
      mono_domain_get(), mono_arch_ip_from_context(sigctx), NULL);
  gpointer *sp;
  int frame_size;
  ...
  if (!ji)
    if (mono_dump_start())
      mono_handle_native_crash(mono_get_signame(SIGSEGV), (MonoContext *)sigctx,
                               siginfo);
...
#ifdef PPC_USES_FUNCTION_DESCRIPTOR
  {
    MonoPPCFunctionDescriptor *handler_ftnptr =
        (MonoPPCFunctionDescriptor *)altstack_handle_and_restore;
...
static void setup_ucontext_return(void *uc, gpointer func) {
#if !defined(MONO_CROSS_COMPILE)
  UCONTEXT_REG_LNK(uc) = UCONTEXT_REG_NIP(uc);
#ifdef PPC_USES_FUNCTION_DESCRIPTOR
  {
    MonoPPCFunctionDescriptor *handler_ftnptr =
        (MonoPPCFunctionDescriptor *)func;
