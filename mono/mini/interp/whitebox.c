...
static void print_td(TransformData *td) {
  if (!td->verbose_level)
    return;
...
static int expect(InterpInst **ins, InterpInst **current, guint16 opcode) {
  g_assert(ins);
...
static int verify_cprop_add_consts(TransformData *td) {
...
static int verify_cprop_ldloc_stloc(TransformData *td) {
  mono_test_interp_cprop(td);
  print_td(td);
...
static void new_test(const char *name, TestVerifier verifier) {
...
static MonoImage *load_assembly(const char *path, MonoDomain *root_domain) {
  MonoAssemblyOpenRequest req;
  mono_assembly_request_prepare_open(&req, MONO_ASMCTX_DEFAULT,
                                     mono_domain_default_alc(root_domain));
...
static MonoMethod *lookup_method_from_image(MonoImage *image,
                                            const char *name) {
  for (int i = 0; i < mono_image_get_table_rows(image, MONO_TABLE_METHOD);
       i++) {
    ERROR_DECL(error);
...
static int determine_verbose_level(TransformData *td) {
...
static TransformData *transform_method(MonoDomain *domain, MonoImage *image,
                                       TestItem *ti) {
  ERROR_DECL(error);
