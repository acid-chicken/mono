 ...
 * GC SAFETY:
 *
 *  The interpreter executes in gc unsafe (non-preempt) mode. On wasm, the C
 * stack is scannable but the wasm stack is not, so to make the code GC safe,
 * the following rules should be followed:
 * - every objref handled by the code needs to either be stored volatile or
 * stored into a volatile; volatile stores are stack packable, volatile values
 * are not. Use either OBJREF or stackval->data.o. This will ensure the objects
 * are pinned. A volatile local is on the stack and not in registers. Volatile
 * stores ditto.
 * - minimize the number of MonoObject* locals/arguments (or make them
 * volatile).
 *
...
static inline MonoObject *WASM_VOLATILE *mono_interp_objref(MonoObject **o) {
  return o;
}
...
#define frame_locals(frame)                                                    \
  (((guchar *)((frame)->stack)) + (frame)->imethod->stack_size +               \
...
static inline int mint_type(MonoType *type_) {
