  ...
  for (ins = td->first_ins; ins != NULL; ins = ins->next) {
    if (MINT_IS_STLOC_NP(ins->opcode)) {
      if (!local_ref_count[ins->data[0]] &&
          (td->locals[ins->data[0]].flags & INTERP_LOCAL_FLAG_INDIRECT) == 0) {
      ...
      if (!local_ref_count[ins->data[1]] &&
          (td->locals[ins->data[1]].flags & INTERP_LOCAL_FLAG_INDIRECT) == 0) {
        interp_clear_ins(td, ins);
        mono_interp_stats.killed_instructions++;
      }
  ...
  for (ins = td->first_ins; ins != NULL; ins = ins->next) {
    ...
    if (MINT_IS_LDLOC(ins->opcode)) {
      ...
      if (prev_ins && MINT_IS_STLOC(prev_ins->opcode) &&
        ...
        g_assert(
            !(td->locals[loaded_local].flags & INTERP_LOCAL_FLAG_INDIRECT));
        ...
        g_assert(!(td->locals[locals[loaded_local].local].flags &
                   INTERP_LOCAL_FLAG_INDIRECT));
      ...
      sp--;
    } else if (ins->opcode >= MINT_STFLD_I1 && ins->opcode <= MINT_STFLD_P &&
...
static void interp_super_instructions(TransformData *td) {
  ...
  for (ins = td->first_ins; ins != NULL; ins = ins->next) {
    ...
    if (ins->opcode >= MINT_LDFLD_I1 && ins->opcode <= MINT_LDFLD_P &&
        prev1_ins) {
