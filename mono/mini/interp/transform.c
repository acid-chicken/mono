...
// the contents of the stack slot / local.
typedef struct {
  // Indicates the type of the stored information. It can be a local or a
  // constant
  int type;
...
typedef struct {
  ...
  // constant or the copy of a local.
  StackValue val;
...
#define INTERP_FOLD_RELOP(opcode, stack_type, field, relop, cast_type)         \
  case opcode:                                                                 \
    ...
    result.field =                                                             \
        (cast_type)sp[0].val.field relop(cast_type) sp[1].val.field;           \
    break;
    ...
    if (MINT_IS_LDLOC(ins->opcode)) {
      ...
      if (sp[-2].ins) {
        ...
        if (obj_ins->opcode == MINT_LDLOC_O) {
          ...
          mono_interp_stats.killed_instructions++;
        } else if (obj_ins->opcode == MINT_LDARG_O ||
                   obj_ins->opcode == MINT_LDARG_P0) {
          int arg_index = 0;
          int fld_offset = ins->data[0];
          int mt = ins->opcode - MINT_STFLD_I1;
