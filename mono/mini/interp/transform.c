...
static gboolean interp_handle_intrinsics(TransformData *td,
  ...
  if (target_method->klass == mono_defaults.string_class) {
    ...
    if (!strcmp(".ctor", tm)) {
      ...
      /* depending on SIZEOF_VOID_P and the type of the value passed to the
       * .ctor we either have to CONV it, or do nothing */
    ...
    for (i = 0; i < sizeof(int_cmpop) / sizeof(MagicIntrinsic); ++i) {
      if (!strcmp(int_cmpop[i].op_name, tm)) {
        ...
        td->sp -= 1;
        SET_TYPE(td->sp - 1, stack_type[mint_type(m_class_get_byval_arg(k))],
                 k);
        td->ip += 5;
        return TRUE;
      }
    ...
    *op = MINT_INTRINS_BYREFERENCE_GET_VALUE;
             ...
             !strcmp(klass_name, "Math") && csignature->param_count == 1 &&
             csignature->params[0]->type == MONO_TYPE_R8) {
    if (tm[0] == 'A') {
      if (strcmp(tm, "Abs") == 0 &&
          csignature->params[0]->type == MONO_TYPE_R8) {
        *op = MINT_ABS;
      ...
      if (mono_class_is_magic_int(klass) || mono_class_is_magic_float(klass)) {
        ...
        if (m_class_get_parent(klass) == mono_defaults.array_class) {
          ...
          if (!m_class_is_valuetype(klass)) {
            ...
            interp_add_ins(td,
                           vt ? MINT_NEWOBJ_VTST_FAST : MINT_NEWOBJ_VT_FAST);
