...
static const char *stack_type_string[] = {"I4", "I8", "R4", "R8",
                                          "O ", "VT", "MP", "F "};

static int stack_type[] = {STACK_TYPE_I4, /*I1*/
                           STACK_TYPE_I4, /*U1*/
                           STACK_TYPE_I4, /*I2*/
                           STACK_TYPE_I4, /*U2*/
                           STACK_TYPE_I4, /*I4*/
...
static gboolean generate_code(TransformData *td, MonoMethod *method,
                              MonoMethodHeader *header,
                              MonoGenericContext *generic_context,
                              MonoError *error);

static InterpInst *interp_new_ins(TransformData *td, guint16 opcode, int len) {
  InterpInst *new_inst;
  // Size of data region of instruction is length of instruction minus 1 (the
  // opcode slot)
  new_inst = mono_mempool_alloc0(
      td->mempool,
      sizeof(InterpInst) + sizeof(guint16) * ((len > 0) ? (len - 1) : 0));
  new_inst->opcode = opcode;
  new_inst->il_offset = td->current_il_offset;
  return new_inst;
}

// This version need to be used with switch opcode, which doesn't have constant
// length
static InterpInst *interp_add_ins_explicit(TransformData *td, guint16 opcode,
                                           int len) {
...
static InterpInst *interp_add_ins(TransformData *td, guint16 opcode) {
...
static InterpInst *interp_insert_ins(TransformData *td, InterpInst *prev_ins,
                                     guint16 opcode) {
...
static void interp_clear_ins(TransformData *td, InterpInst *ins) {
...
static InterpInst *interp_prev_ins(InterpInst *ins) {
  ins = ins->prev;
...
static void handle_branch(TransformData *td, int short_op, int long_op,
                          int offset) {
  ...
  if (offset > 0 && td->stack_height[target] < 0) {
    ...
    if (td->stack_height[target] > 0)
      td->stack_state[target] = (StackInfo *)g_memdup(
          td->stack, td->stack_height[target] * sizeof(td->stack[0]));
...
static void one_arg_branch(TransformData *td, int mint_op, int offset) {
  int type = td->sp[-1].type == STACK_TYPE_O || td->sp[-1].type == STACK_TYPE_MP
                 ? STACK_TYPE_I
                 : td->sp[-1].type;
...
static void two_arg_branch(TransformData *td, int mint_op, int offset) {
  int type1 =
      td->sp[-1].type == STACK_TYPE_O || td->sp[-1].type == STACK_TYPE_MP
          ? STACK_TYPE_I
          : td->sp[-1].type;
  int type2 =
      td->sp[-2].type == STACK_TYPE_O || td->sp[-2].type == STACK_TYPE_MP
          ? STACK_TYPE_I
          : td->sp[-2].type;
  ...
  if (type1 == STACK_TYPE_I4 && type2 == STACK_TYPE_I8) {
    // The il instruction starts with the actual branch, and not with the
    // conversion opcodes
...
static void unary_arith_op(TransformData *td, int mint_op) {
  int op = mint_op + td->sp[-1].type - STACK_TYPE_I4;
  CHECK_STACK(td, 1);
  interp_add_ins(td, op);
}

static void binary_arith_op(TransformData *td, int mint_op) {
  int type1 = td->sp[-2].type;
  int type2 = td->sp[-1].type;
  int op;
#if SIZEOF_VOID_P == 8
  if ((type1 == STACK_TYPE_MP || type1 == STACK_TYPE_I8) &&
      type2 == STACK_TYPE_I4) {
    interp_add_ins(td, MINT_CONV_I8_I4);
    type2 = STACK_TYPE_I8;
  }
  if (type1 == STACK_TYPE_I4 &&
      (type2 == STACK_TYPE_MP || type2 == STACK_TYPE_I8)) {
...
static void shift_op(TransformData *td, int mint_op) {
...
static int can_store(int st_value, int vt_value) {
...
#define REALLOC_STACK(td, sppos)                                               \
  do {                                                                         \
    (td)->stack_capacity *= 2;                                                 \
    (td)->stack = (StackInfo *)realloc((td)->stack, (td)->stack_capacity *     \
                                                        sizeof(td->stack[0])); \
...
static void move_stack(TransformData *td, int start, int amount) {
  ...
  if (to_move > 0)
    memmove(td->stack + start + amount, td->stack + start,
            to_move * sizeof(StackInfo));
}

static void simulate_runtime_stack_increase(TransformData *td, int amount) {
...
static MonoType *get_arg_type_exact(TransformData *td, int n, int *mt) {
  MonoType *type;
...
static void load_arg(TransformData *td, int n) {
  int mt;
...
static void store_arg(TransformData *td, int n) {
  int mt;
  CHECK_STACK(td, 1);
  MonoType *type;
...
static void load_local(TransformData *td, int local) {
...
static void store_local(TransformData *td, int local) {
  MonoType *type = td->locals[local].type;
  int mt = td->locals[local].mt;
  CHECK_STACK(td, 1);
#if SIZEOF_VOID_P == 8
...
static guint16 get_data_item_index(TransformData *td, void *ptr) {
  ...
  if (td->max_data_items == td->n_data_items) {
    ...
    td->data_items = (gpointer *)g_realloc(
        td->data_items, td->max_data_items * sizeof(td->data_items[0]));
  }
...
gboolean mono_interp_jit_call_supported(MonoMethod *method,
...
static gboolean jit_call2_supported(MonoMethod *method,
                                    MonoMethodSignature *sig) {
  if (sig->param_count > 6)
    return FALSE;
  if (sig->pinvoke)
...
static void interp_generate_mae_throw(TransformData *td, MonoMethod *method,
...
static void interp_generate_bie_throw(TransformData *td) {
 ...
 */
static int create_interp_local(TransformData *td, MonoType *type) {
  if (td->locals_size == td->locals_capacity) {
    td->locals_capacity *= 2;
    if (td->locals_capacity == 0)
      td->locals_capacity = 2;
    td->locals = (InterpLocal *)g_realloc(td->locals, td->locals_capacity *
                                                          sizeof(InterpLocal));
  }
...
static int get_interp_local_offset(TransformData *td, int local) {
...
static void dump_mint_code(const guint16 *start, const guint16 *end) {
...
static void dump_interp_inst(InterpInst *ins) {
  char *descr =
      mono_interp_dis_mintop(ins->il_offset, FALSE, &ins->data[0], ins->opcode);
  g_print("%s", descr);
  g_free(descr);
}

static void dump_interp_inst_newline(InterpInst *ins) {
...
void mono_interp_print_code(InterpMethod *imethod) {
  ...
  dump_mint_code((const guint16 *)start,
                 (const guint16 *)(start + jinfo->code_size));
}

/* For debug use */
void mono_interp_print_td_code(TransformData *td) {
...
static MonoMethodHeader *interp_method_get_header(MonoMethod *method,
                                                  MonoError *error) {
...
static void emit_store_value_as_local(TransformData *td, MonoType *src) {
...
static gboolean interp_is_bb_start(TransformData *td, InterpInst *start,
                                   InterpInst *end) {
...
static gboolean interp_ins_is_ldc(InterpInst *ins) {
  return ins->opcode >= MINT_LDC_I4_M1 && ins->opcode <= MINT_LDC_I8;
}

static gint32 interp_get_const_from_ldc_i4(InterpInst *ins) {
  switch (ins->opcode) {
  case MINT_LDC_I4_M1:
    return -1;
  case MINT_LDC_I4_0:
...
static InterpInst *interp_get_ldc_i4_from_const(TransformData *td,
                                                InterpInst *ins, gint32 ct) {
  int opcode;
  switch (ct) {
  case -1:
...
static InterpInst *interp_inst_replace_with_i8_const(TransformData *td,
                                                     InterpInst *ins,
                                                     gint64 ct) {
...
static int interp_get_ldind_for_mt(int mt) {
  switch (mt) {
  case MINT_TYPE_I1:
...
static void interp_emit_ldobj(TransformData *td, MonoClass *klass) {
...
static void interp_emit_stobj(TransformData *td, MonoClass *klass) {
  int mt = mint_type(m_class_get_byval_arg(klass));
...
static void interp_emit_ldelema(TransformData *td, MonoClass *array_class,
...
static gboolean interp_handle_magic_type_intrinsics(
  ...
  if (!strcmp(".ctor", tm)) {
    ...
    /* depending on SIZEOF_VOID_P and the type of the value passed to the .ctor
     * we either have to CONV it, or do nothing */
    ...
    SET_TYPE(td->sp - 1, stack_type[mint_type(dst)],
    ...
    /* on 'System.n*::{CompareTo,Equals} (System.n*)' variant we need to push
     * managed pointer instead of value */
  ...
  g_error("TODO: interp_transform_call %s:%s",
          m_class_get_name(target_method->klass), tm);
}

/* Return TRUE if call transformation is finished */
static gboolean interp_handle_intrinsics(TransformData *td,
                                         MonoMethod *target_method,
                                         MonoClass *constrained_class,
                                         MonoMethodSignature *csignature,
                                         gboolean readonly, int *op) {
  const char *tm = target_method->name;
  int type_index = mono_class_get_magic_index(target_method->klass);
  gboolean in_corlib =
      m_class_get_image(target_method->klass) == mono_defaults.corlib;
  const char *klass_name_space;
  if (m_class_get_nested_in(target_method->klass))
    klass_name_space =
        m_class_get_name_space(m_class_get_nested_in(target_method->klass));
  else
  ...
  if (target_method->klass == mono_defaults.string_class) {
    ...
    return interp_handle_magic_type_intrinsics(td, target_method, csignature,
                                               type_index);
  } else if (mono_class_is_subclass_of_internal(
                 target_method->klass, mono_defaults.array_class, FALSE)) {
    if (!strcmp(tm, "get_Rank")) {
      ...
      *op = MINT_CALLRUN;
      ...
      MonoClass *element_class =
          m_class_get_element_class(target_method->klass);
      MonoType *local_type = m_class_get_byval_arg(element_class);
      MonoClass *value_class = td->sp[-1].klass;
      // If value_class is NULL it means the top of stack is a simple type
      // (valuetype) which doesn't require type checks, or that we have no type
      // information because the code is unsafe (like in some wrappers). In that
      // case we assume the type of the array and don't do any checks.
    ...
    *op = MINT_INTRINS_BYREFERENCE_GET_VALUE;
    ...
    if (csignature->param_count == 1 &&
        csignature->params[0]->type == param_type) {
      // unops
      if (tm[0] == 'A') {
        if (strcmp(tm, "Abs") == 0) {
          *op = MINT_ABS;
      ...
      if (strcmp(tm, "Atan2") == 0)
        ...
        *op = MINT_POW;
    } else if (csignature->param_count == 3 &&
               csignature->params[0]->type == param_type &&
               csignature->params[1]->type == param_type &&
    ...
    if (*op != -1 && is_float) {
      *op = *op + (MINT_ABSF - MINT_ABS);
             ...
             (!strcmp(klass_name, "Span`1") ||
              !strcmp(klass_name, "ReadOnlySpan`1"))) {
    if (!strcmp(tm, "get_Item")) {
      MonoGenericClass *gclass =
          mono_class_get_generic_class(target_method->klass);
      MonoClass *param_class = mono_class_from_mono_type_internal(
          gclass->context.class_inst->type_argv[0]);

      if (!mini_is_gsharedvt_variable_klass(param_class)) {
        MonoClassField *length_field = mono_class_get_field_from_name_full(
            target_method->klass, "_length", NULL);
        g_assert(length_field);
        int offset_length = length_field->offset - sizeof(MonoObject);

        MonoClassField *ptr_field = mono_class_get_field_from_name_full(
            target_method->klass, "_pointer", NULL);
        g_assert(ptr_field);
  ...
  } else if (((in_corlib && !strcmp(klass_name_space,
    ...
    if (!strcmp(tm, "get_OffsetToStringData")) {
      ...
      if (MONO_TYPE_IS_PRIMITIVE(t) && t->type != MONO_TYPE_R4 &&
          t->type != MONO_TYPE_R8)
        *op = MINT_LDC_I4_1;
      else
        *op = MINT_LDC_I4_0;
      ...
      *op = has_refs ? MINT_LDC_I4_1 : MINT_LDC_I4_0;
             ...
             !strcmp(klass_name, "RuntimeMethodHandle") &&
             !strcmp(tm, "GetFunctionPointer") &&
             csignature->param_count == 1) {
    // We must intrinsify this method on interp so we don't return a pointer to
    // native code entering interpreter
    *op = MINT_LDFTN_DYNAMIC;
  } else if (in_corlib &&
             target_method->klass == mono_defaults.systemtype_class &&
    ...
    *op = MINT_CEQ_P;
    ...
    if (!strcmp(tm, "InternalGetHashCode"))
      *op = MINT_INTRINS_GET_HASHCODE;
             ...
             // Invoking GetType via reflection on proxies has some special
             // semantics See InterfaceProxyGetTypeViaReflectionOkay corlib test
    ...
    if (td->last_ins && td->last_ins->opcode == MINT_BOX &&
        ...
        td->last_ins->prev->prev &&
        td->last_ins->prev->prev->opcode == MINT_BOX &&
      ...
      MonoType *base_type = mono_type_get_underlying_type(
          m_class_get_byval_arg(td->sp[-2].klass));
      ...
      interp_clear_ins(td, td->last_ins);
      interp_insert_ins(td, td->last_ins->prev->prev,
    ...
    gboolean is_unsigned =
        (t->type == MONO_TYPE_U1 || t->type == MONO_TYPE_U2 ||
         t->type == MONO_TYPE_U4 || t->type == MONO_TYPE_U8 ||
         t->type == MONO_TYPE_U);
...
interp_transform_internal_calls(MonoMethod *method, MonoMethod *target_method,
  ...
  if (method->wrapper_type == MONO_WRAPPER_NONE && target_method != NULL) {
    ...
    if (target_method->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL &&
        !is_virtual && !mono_class_is_marshalbyref(target_method->klass) &&
...
static gboolean interp_type_as_ptr(MonoType *tp) {
...
static int interp_icall_op_for_sig(MonoMethodSignature *sig) {
  int op = -1;
...
static gboolean interp_method_check_inlining(TransformData *td,
                                             MonoMethod *method) {
...
static gboolean interp_inline_method(TransformData *td,
                                     MonoMethod *target_method,
                                     MonoMethodHeader *header,
                                     MonoError *error) {
  ...
  if (!ret) {
    ...
    // Make sure we have an IR instruction associated with the now removed IL
    // CALL
    // FIXME This could be prettier. We might be able to make inlining saner now
    // that that we can easily tweak the instruction list.
...
static void interp_constrained_box(TransformData *td, MonoDomain *domain,
                                   MonoClass *constrained_class,
                                   MonoMethodSignature *csignature,
                                   MonoError *error) {
  ...
  if (mono_class_is_nullable(constrained_class)) {
    ...
    td->last_ins->data[2] =
        (td->sp - 1 - csignature->param_count)->type != STACK_TYPE_MP ? 0 : 1;
  } else {
    MonoVTable *vtable =
        mono_class_vtable_checked(domain, constrained_class, error);
    return_if_nok(error);
...
static MonoMethod *interp_get_method(MonoMethod *method, guint32 token,
                                     MonoImage *image,
                                     MonoGenericContext *generic_context,
                                     MonoError *error) {
  if (method->wrapper_type == MONO_WRAPPER_NONE)
...
static gboolean interp_transform_call(
    TransformData *td, MonoMethod *method, MonoMethod *target_method,
    MonoDomain *domain, MonoGenericContext *generic_context,
    unsigned char *is_bb_start, MonoClass *constrained_class, gboolean readonly,
    MonoError *error, gboolean check_visibility, gboolean save_last_error) {
  ...
  if (target_method == NULL) {
    if (calli) {
      ...
      if (method->wrapper_type != MONO_WRAPPER_NONE)
        csignature =
            (MonoMethodSignature *)mono_method_get_wrapper_data(method, token);
      else {
      ...
      if (generic_context) {
        csignature =
            mono_inflate_generic_signature(csignature, generic_context, error);
        return_val_if_nok(error, FALSE);
      }

      /*
       * The compiled interp entry wrapper is passed to runtime_invoke instead
       * of the InterpMethod pointer. FIXME
       */
      ...
      target_method =
          interp_get_method(method, token, image, generic_context, error);
      return_val_if_nok(error, FALSE);
      ...
      if (generic_context) {
        csignature =
            mono_inflate_generic_signature(csignature, generic_context, error);
        return_val_if_nok(error, FALSE);
        target_method = mono_class_inflate_generic_method_checked(
            target_method, generic_context, error);
        return_val_if_nok(error, FALSE);
      }
    }
  } else {
  ...
  if (check_visibility && target_method &&
  ...
  if (target_method && target_method->string_ctor) {
    ...
    MonoMethodSignature *ctor_sig =
        mono_metadata_signature_dup_mempool(td->mempool, csignature);
  ...
  /* Intrinsics */
  if (target_method &&
      interp_handle_intrinsics(td, target_method, constrained_class, csignature,
                               readonly, &op))
    return TRUE;
  ...
  if (constrained_class) {
    ...
    g_print("                    : %s::%s.  %s (%p)\n",
            ...
            mono_signature_full_name(target_method->signature), target_method);
#endif
    /* Intrinsics: Try again, it could be that
     * `mono_get_method_constrained_with_method` resolves to a method that we
     * can substitute */
    if (target_method &&
        interp_handle_intrinsics(td, target_method, constrained_class,
                                 csignature, readonly, &op))
      return TRUE;
    ...
    if (!m_class_is_valuetype(constrained_class)) {
      ...
      td->last_ins->data[0] = csignature->param_count;
               ...
               target_method->klass ==
                   m_class_get_parent(mono_defaults.enum_class) ||
               target_method->klass == mono_defaults.enum_class) {
      /*
       * Constrained expects a managed pointer that normally needs
       * dereferencing. For value types that have their storage on the vtstack,
       * a managed pointer to it is identical to the internal pointer that is
       * passed on the stack when using the value type, not needing any
       * dereferencing.
       */
      ...
      if (mint_type(m_class_get_byval_arg(constrained_class)) != MINT_TYPE_VT) {
        /* Always load the entire stackval, to handle also the case where the
         * enum has long storage */
      ...
      if (target_method->klass != constrained_class) {
        ...
        int this_type = (td->sp - csignature->param_count - 1)->type;
        g_assert(this_type == STACK_TYPE_I || this_type == STACK_TYPE_MP);
        if (mint_type(m_class_get_byval_arg(constrained_class)) !=
            MINT_TYPE_VT) {
          /* managed pointer on the stack, we need to deref that puppy */
          /* Always load the entire stackval, to handle also the case where the
           * enum has long storage */
        ...
        interp_constrained_box(td, domain, constrained_class, csignature,
                               error);
        return_val_if_nok(error, FALSE);
      }
      is_virtual = FALSE;
  ...
  if (!is_virtual && target_method &&
      (target_method->flags & METHOD_ATTRIBUTE_ABSTRACT)) {
  ...
  if (!td->gen_sdb_seq_points && !calli && op == -1 &&
      (!is_virtual || (target_method->flags & METHOD_ATTRIBUTE_VIRTUAL) == 0) &&
    ...
    if (method == target_method && *(td->ip + 5) == CEE_RET &&
        !(csignature->hasthis && m_class_is_valuetype(target_method->klass))) {
  ...
  target_method = interp_transform_internal_calls(method, target_method,
                                                  csignature, is_virtual);

  if (csignature->call_convention == MONO_CALL_VARARG) {
    csignature = mono_method_get_signature_checked(target_method, image, token,
  ...
  if ((mono_interp_opt & INTERP_OPT_INLINE) && op == -1 && !is_virtual &&
      target_method && interp_method_check_inlining(td, target_method)) {
  ...
  /* We need to convert delegate invoke to a indirect call on the
   * interp_invoke_impl field */
  if (target_method && m_class_get_parent(target_method->klass) ==
  ...
  for (i = 0; i < csignature->param_count; ++i) {
    if (td->sp[i + !!csignature->hasthis].type == STACK_TYPE_VT) {
      gint32 size;
      MonoClass *klass =
          mono_class_from_mono_type_internal(csignature->params[i]);
  ...
  if (op >= 0) {
    ...
    if (op == MINT_CALLRUN) {
      td->last_ins->data[0] = get_data_item_index(td, target_method);
      td->last_ins->data[1] = get_data_item_index(
          td, mono_method_signature_internal(target_method));
    }
#endif
  } else if (!calli && !is_delegate_invoke && !is_virtual &&
             mono_interp_jit_call_supported(target_method, csignature)) {
    interp_add_ins(td, MINT_JIT_CALL);
    td->last_ins->data[0] = get_data_item_index(
        td, (void *)mono_interp_get_imethod(domain, target_method, error));
    mono_error_assert_ok(error);
  } else {
    ...
    if (csignature->call_convention == MONO_CALL_VARARG)
      ...
      interp_add_ins(
          td, native ? ((op != -1) ? MINT_CALLI_NAT_FAST : MINT_CALLI_NAT)
                     : MINT_CALLI);
    ...
    if (is_delegate_invoke) {
      ...
      InterpMethod *imethod =
          mono_interp_get_imethod(domain, target_method, error);
      ...
      if (csignature->call_convention == MONO_CALL_VARARG)
        td->last_ins->data[1] = get_data_item_index(td, (void *)csignature);
               ...
               !mono_class_is_marshalbyref(target_method->klass)) {
        /* FIXME Use fastpath also for MBRO. Asserts in
...
interp_field_from_token(MonoMethod *method, guint32 token, MonoClass **klass,
  ...
  if (method->wrapper_type != MONO_WRAPPER_NONE) {
    ...
    *klass = field->parent;
    ...
    field = mono_field_from_token_checked(m_class_get_image(method->klass),
                                          token, klass, generic_context, error);
    return_val_if_nok(error, NULL);
  }

  if (!method->skip_visibility &&
      !mono_method_can_access_field(method, field)) {
    char *method_fname = mono_method_full_name(method, TRUE);
    char *field_fname = mono_field_full_name(field);
    mono_error_set_generic_error(
        error, "System", "FieldAccessException",
...
static InterpBasicBlock *get_bb(TransformData *td, InterpBasicBlock *cbb,
                                unsigned char *ip) {
  int offset = ip - td->il_code;
  InterpBasicBlock *bb = td->offset_to_bb[offset];

  if (!bb) {
    bb = (InterpBasicBlock *)mono_mempool_alloc0(td->mempool,
 ...
 */
static void get_basic_blocks(TransformData *td) {
  ...
  td->offset_to_bb = (InterpBasicBlock **)mono_mempool_alloc0(
      td->mempool, sizeof(InterpBasicBlock *) * (end - start + 1));
...
static void interp_save_debug_info(InterpMethod *rtm, MonoMethodHeader *header,
                                   TransformData *td, GArray *line_numbers) {
  ...
  /*
   * We save the debug info in the same way the JIT does it, treating the
   * interpreter IR as the native code.
   */
  ...
  dinfo->line_numbers =
      g_new0(MonoDebugLineNumberEntry, dinfo->num_line_numbers);
  ...
  for (i = 0; i < dinfo->num_line_numbers; i++)
    dinfo->line_numbers[i] =
        g_array_index(line_numbers, MonoDebugLineNumberEntry, i);
...
static void insert_pred_seq_point(SeqPoint *last_sp, SeqPoint *sp,
                                  GSList **next) {
  GSList *l;
  ...
  if (!l)
    next[src_index] =
        g_slist_append(next[src_index], GUINT_TO_POINTER(dst_index));
}

static void recursively_make_pred_seq_points(TransformData *td,
                                             InterpBasicBlock *bb) {
  ...
  for (l = bb->preds; l; l = l->next) {
    ...
    // predecessors = g_array_append_vals (predecessors, in_bb->pred_seq_points,
    // in_bb->num_pred_seq_points);
  }

  g_hash_table_destroy(seen);

  if (predecessors->len != 0) {
    bb->pred_seq_points = (SeqPoint **)mono_mempool_alloc0(
        td->mempool, sizeof(SeqPoint *) * predecessors->len);
    bb->num_pred_seq_points = predecessors->len;

    for (int newer = 0; newer < bb->num_pred_seq_points; newer++) {
      bb->pred_seq_points[newer] =
          (SeqPoint *)g_array_index(predecessors, gpointer, newer);
    }
  }
...
static void collect_pred_seq_points(TransformData *td, InterpBasicBlock *bb,
                                    SeqPoint *seqp, GSList **next) {
...
static void save_seq_points(TransformData *td, MonoJitInfo *jinfo) {
  GByteArray *array;
  int i, seq_info_size;
   ...
   * following it, this is needed to implement 'step over' in the debugger
   * agent. Similar to the code in mono_save_seq_point_info ().
   */
  ...
  next = (GSList **)mono_mempool_alloc0(td->mempool,
                                        sizeof(GList *) * td->seq_points->len);
  for (bblist = td->basic_blocks; bblist; bblist = bblist->next) {
    ...
    for (GSList *l = bb_seq_points; l; l = l->next) {
      ...
      if (last != NULL) {
        ...
        next[last->next_offset] =
            g_slist_append_mempool(td->mempool, next[last->next_offset],
                                   GINT_TO_POINTER(sp->next_offset));
      } else {
  ...
  for (i = 0; i < td->seq_points->len; ++i) {
    ...
    sp->next_offset = 0;
    if (mono_seq_point_info_add_seq_point(array, sp, last_seq_point, next[i],
                                          TRUE))
      last_seq_point = sp;
  }
  ...
  if (td->verbose_level) {
    ...
    for (i = 0; i < td->seq_points->len; ++i) {
      ...
      for (l = next[i]; l; l = l->next) {
        ...
        g_print(" IL0x%x",
                ((SeqPoint *)g_ptr_array_index(td->seq_points, next_index))
                    ->il_offset);
      }
      g_print("\n");
    }
  ...
  info = mono_seq_point_info_new(array->len, TRUE, array->data, TRUE,
                                 &seq_info_size);
  mono_atomic_fetch_add_i32(&mono_jit_stats.allocated_seq_points_size,
                            seq_info_size);
...
static void interp_emit_memory_barrier(TransformData *td, int kind) {
...
static void interp_method_compute_offsets(TransformData *td,
                                          InterpMethod *imethod,
  ...
  imethod->local_offsets =
      (guint32 *)g_malloc(header->num_locals * sizeof(guint32));
  td->locals =
      (InterpLocal *)g_malloc(header->num_locals * sizeof(InterpLocal));
  ...
  imethod->exvar_offsets =
      (guint32 *)g_malloc(header->num_clauses * sizeof(guint32));
...
void mono_test_interp_method_compute_offsets(TransformData *td,
                                             InterpMethod *imethod,
                                             MonoMethodSignature *signature,
                                             MonoMethodHeader *header) {
  ERROR_DECL(error);
...
static gboolean init_bb_start(TransformData *td, MonoMethodHeader *header,
                              gboolean inlining) {
...
static int get_unaligned_opcode(int opcode) {
  switch (opcode) {
  case MINT_LDFLD_I8:
...
static void interp_emit_ldsflda(TransformData *td, MonoClassField *field,
                                MonoError *error) {
  ...
  if (mono_class_field_is_special_static(field)) {
    ...
    offset = GPOINTER_TO_UINT(
        g_hash_table_lookup(domain->special_static_fields, field));
    ...
    td->last_ins->data[0] = get_data_item_index(td, vtable);
    td->last_ins->data[1] = get_data_item_index(
        td, (char *)mono_vtable_get_static_field_data(vtable) + field->offset);
  }
}

static gboolean interp_emit_load_const(TransformData *td, gpointer field_addr,
                                       int mt) {
 ...
 * allowed by MS.NET.
 */
static void emit_convert(TransformData *td, int stack_type, MonoType *ftype) {
...
static void interp_emit_sfld_access(TransformData *td, MonoClassField *field,
                                    MonoClass *field_class, int mt,
  ...
  if (mono_class_field_is_special_static(field)) {
    ...
    if (is_load) {
      ...
      interp_add_ins(td, (mt == MINT_TYPE_VT)
                             ? MINT_LDSFLD_VT
                             : (MINT_LDSFLD_I1 + mt - MINT_TYPE_I1));
    } else {
      interp_add_ins(td, (mt == MINT_TYPE_VT)
                             ? MINT_STSFLD_VT
                             : (MINT_STSFLD_I1 + mt - MINT_TYPE_I1));
    }
...
static gboolean signature_has_vt_params(MonoMethodSignature *csignature) {
  int i;
  for (i = 0; i < csignature->param_count; ++i) {
...
static gboolean generate_code(TransformData *td, MonoMethod *method,
  ...
  for (i = 0; i < header->num_clauses; i++) {
    ...
    td->stack_state[c->handler_offset] =
        (StackInfo *)g_malloc0(sizeof(StackInfo));
  ...
  if (td->gen_sdb_seq_points && !inlining) {
    ...
    if (minfo) {
      ...
      seq_point_locs = mono_bitset_mem_new(
          mono_mempool_alloc0(td->mempool,
  ...
  if (!inlining) {
    ...
    int local;
    arg_locals = (guint32 *)g_malloc(
        (!!signature->hasthis + signature->param_count) * sizeof(guint32));
  ...
  while (td->ip < end) {
    ...
    if (!inlining && td->stack_height[in_offset] >= 0) {
      ...
      if (td->stack_height[in_offset] > 0)
        memcpy(td->stack, td->stack_state[in_offset],
               td->stack_height[in_offset] * sizeof(td->stack[0]));
    ...
    if (td->verbose_level > 1) {
      g_print(
          "IL_%04lx %s %-10s, sp %ld, %s %-12s vt_sp %u (max %u)\n",
          td->ip - td->il_code,
          ...
          (td->sp > td->stack && (td->sp[-1].type == STACK_TYPE_O ||
                                  td->sp[-1].type == STACK_TYPE_VT))
              ? (td->sp[-1].klass == NULL ? "?"
                                          : m_class_get_name(td->sp[-1].klass))
              : "",
          td->vt_sp, td->max_vt_sp);
    }

    if (sym_seq_points &&
        mono_bitset_test_fast(seq_point_locs, td->ip - header->code)) {
    ...
    case CEE_LDC_I4_0:
      ...
      if (td->ip - td->il_code + 2 < td->code_size &&
          (inlining || !td->is_bb_start[td->ip + 1 - td->il_code]) &&
          td->ip[1] == 0xfe && td->ip[2] == CEE_CEQ && td->sp > td->stack &&
          td->sp[-1].type == STACK_TYPE_I4) {
    ...
    case CEE_LDC_I4_1:
      // Only single basic block functions are inlined.
      if (td->ip - td->il_code + 1 < td->code_size &&
          (inlining || !td->is_bb_start[td->ip + 1 - td->il_code]) &&
          (td->ip[1] == CEE_ADD || td->ip[1] == CEE_SUB) &&
          td->sp[-1].type == STACK_TYPE_I4) {
    ...
    case CEE_JMP: {
      ...
      td->last_ins->data[0] =
          get_data_item_index(td, mono_interp_get_imethod(domain, m, error));
      goto_if_nok(error, exit);
      td->ip += 5;
      break;
    }
    ...
    case CEE_CALL: {
      ...
      if (sym_seq_points &&
          !mono_bitset_test_fast(seq_point_locs, td->ip + 5 - header->code))
        need_seq_point = TRUE;

      if (!interp_transform_call(td, method, NULL, domain, generic_context,
                                 td->is_bb_start, constrained_class, readonly,
                                 error, TRUE, save_last_error))
        goto exit;

      if (need_seq_point) {
        // check is is a nested call and remove the MONO_INST_NONEMPTY_STACK of
        // the last breakpoint, only for non native methods
        ...
        last_seq_point = interp_add_ins(td, MINT_SDB_SEQ_POINT);
        // This seq point is actually associated with the instruction following
        // the call
    ...
    case CEE_RET: {
      ...
      if (td->vt_sp != ALIGN_TO(vt_size, MINT_VT_ALIGNMENT))
        g_error("%s: CEE_RET: value type stack: %d vs. %d",
                mono_method_full_name(td->method, TRUE), td->vt_sp, vt_size);
    ...
    case CEE_SWITCH: {
      ...
      for (i = 0; i < n; i++) {
        ...
        if (offset < 0) {
          ...
          if (stack_height > 0)
            td->stack_state[target] = (StackInfo *)g_memdup(
                td->stack, stack_height * sizeof(td->stack[0]));
        }
    ...
    case CEE_CONV_I8:
      ...
      case STACK_TYPE_R8:
        ...
        break;
      case STACK_TYPE_I4: {
        if (interp_ins_is_ldc(td->last_ins) &&
            (inlining || !td->is_bb_start[in_offset])) {
    ...
    case CEE_CPOBJ: {
      ...
      klass = mono_class_get_and_inflate_typespec_checked(
          image, token, generic_context, error);
      goto_if_nok(error, exit);
    ...
    case CEE_LDSTR: {
      ...
      if (method->wrapper_type == MONO_WRAPPER_NONE) {
        ...
        td->last_ins->data[0] =
            get_data_item_index(td, GUINT_TO_POINTER(token));
      }
    ...
    case CEE_NEWOBJ: {
      ...
      if (mono_class_is_magic_int(klass) || mono_class_is_magic_float(klass)) {
        ...
        if (mono_class_is_magic_int(klass) && td->sp[0].type == STACK_TYPE_I4)
          interp_add_ins(td, MINT_CONV_I8_I4);
        else if (mono_class_is_magic_float(klass) &&
                 td->sp[0].type == STACK_TYPE_R4)
          interp_add_ins(td, MINT_CONV_R8_R4);
#endif
        ...
        PUSH_TYPE(td, stack_type[mint_type(m_class_get_byval_arg(klass))],
                  klass);
        ...
        if (m_class_get_parent(klass) == mono_defaults.array_class) {
          ...
          td->last_ins->data[0] = get_data_item_index(
              td, mono_interp_get_imethod(domain, m, error));
          ...
          if (is_vt) {
            newobj_fast = interp_add_ins(td, is_vtst ? MINT_NEWOBJ_VTST_FAST
                                                     : MINT_NEWOBJ_VT_FAST);
            if (is_vtst)
          ...
          // We don't support inlining ctors of MINT_TYPE_VT which also receive
          // a MINT_TYPE_VT as an argument. The reason is that we would need to
          // push this on the vtstack before the argument, which is very awkward
          // for uncommon scenario.
          if ((mono_interp_opt & INTERP_OPT_INLINE) &&
              interp_method_check_inlining(td, m) &&
          ...
          // move_stack with the last parameter negative does not reduce
          // max_stack.
          if (is_vtst)
            POP_VT(td, vtsize);
          move_stack(td, (td->sp - td->stack) - csignature->param_count, -2);
          // Set the method to be executed as part of newobj instruction
          newobj_fast->data[0] = get_data_item_index(
              td, mono_interp_get_imethod(domain, m, error));
        } else {
    ...
    case CEE_UNBOX:
      ...
      if (mono_class_is_nullable(klass)) {
        ...
        if (m_class_is_enumtype(mono_class_get_nullable_param_internal(klass)))
          target_method = mono_class_get_method_from_name_checked(
              klass, "UnboxExact", 1, 0, error);
        else
          target_method = mono_class_get_method_from_name_checked(
              klass, "Unbox", 1, 0, error);
        goto_if_nok(error, exit);
        /* td->ip is incremented by interp_transform_call */
        if (!interp_transform_call(td, method, target_method, domain,
                                   generic_context, td->is_bb_start, NULL,
                                   FALSE, error, FALSE, FALSE))
          goto exit;
        /*
         * CEE_UNBOX needs to push address of vtype while Nullable.Unbox returns
         * the value type We create a local variable in the frame so that we can
         * fetch its address.
         */
    ...
    case CEE_LDFLDA: {
      ...
      if (m_class_get_marshalbyref(klass) ||
        ...
        /* td->ip is incremented by interp_transform_call */
        if (!interp_transform_call(td, method, wrapper, domain, generic_context,
                                   td->is_bb_start, NULL, FALSE, error, FALSE,
                                   FALSE))
          goto exit;
      } else
#endif
...
#ifndef DISABLE_REMOTING
      if ((m_class_get_marshalbyref(klass) &&
           !(signature->hasthis && td->last_ins->opcode == MINT_LDARG_P0)) ||
          mono_class_is_contextbound(klass) ||
          klass == mono_defaults.marshalbyrefobject_class) {
        g_assert(!is_static);
      ...
      if (mono_class_is_nullable(klass)) {
        ...
        if (G_UNLIKELY(m_class_is_byreflike(klass))) {
          mono_error_set_bad_image(
              error, image, "Cannot box IsByRefLike type '%s.%s'",
              m_class_get_name_space(klass), m_class_get_name(klass));
          goto exit;
        }

        const gboolean vt =
            mint_type(m_class_get_byval_arg(klass)) == MINT_TYPE_VT;

        if (vt) {
          size = mono_class_value_size(klass, NULL);
          size = ALIGN_TO(size, MINT_VT_ALIGNMENT);
          td->vt_sp -= size;
        } else if (td->sp[-1].type == STACK_TYPE_R8 &&
                   m_class_get_byval_arg(klass)->type == MONO_TYPE_R4) {
          interp_add_ins(td, MINT_CONV_R4_R8);
        }
    ...
    case CEE_NEWARR: {
      ...
      MonoVTable *vtable =
          mono_class_vtable_checked(domain, array_class, error);
      goto_if_nok(error, exit);
    ...
    case CEE_LDTOKEN: {
      ...
      if (method->wrapper_type == MONO_WRAPPER_DYNAMIC_METHOD ||
        ...
        if (generic_context) {
          handle = mono_class_inflate_generic_type_checked(
              (MonoType *)handle, generic_context, error);
          goto_if_nok(error, exit);
        }
      } else {
        handle =
            mono_ldtoken_checked(image, token, &klass, generic_context, error);
        goto_if_nok(error, exit);
      }
      mono_class_init_internal(klass);
      ...
      if (next_ip < end &&
        ...
        if (next_next_ip < end &&
            ...
            (next_cmethod = interp_get_method(method, read32(next_next_ip + 1),
                                              image, generic_context, error)) &&
    ...
    case CEE_LEAVE_S: {
      ...
      if (td->clause_indexes[in_offset] != -1) {
        /* LEAVE instructions in catch clauses need to check for abort
         * exceptions */
    ...
    case MONO_CUSTOM_PREFIX:
      ...
      case CEE_MONO_CALLI_EXTRA_ARG:
        /* Same as CEE_CALLI, except that we drop the extra arg required for
         * llvm specific behaviour */
      ...
      case CEE_MONO_JIT_ICALL_ADDR: {
        ...
        td->ip += 5;
        const gconstpointer func =
            mono_find_jit_icall_info((MonoJitICallId)token)->func;
      ...
      case CEE_MONO_ICALL: {
        ...
        if (jit_icall_id == MONO_JIT_ICALL_mono_threads_attach_coop) {
          ...
          /* attach needs two arguments, and has one return value: leave one
           * element on the stack */
          interp_add_ins(td, MINT_POP);
        } else if (jit_icall_id == MONO_JIT_ICALL_mono_threads_detach_coop) {
          g_assert(rtm->needs_thread_attach);

          /* detach consumes two arguments, and no return value: drop both of
           * them */
          interp_add_ins(td, MINT_POP);
          interp_add_ins(td, MINT_POP);
        } else {
      ...
      case CEE_MONO_CLASSCONST:
        ...
        interp_add_ins(td, MINT_MONO_LDPTR);
        td->last_ins->data[0] = get_data_item_index(
            td, mono_method_get_wrapper_data(method, token));
        PUSH_SIMPLE_TYPE(td, STACK_TYPE_I);
        break;
      ...
      case CEE_MONO_RETOBJ:
        ...
        /*stackval_from_data (signature->ret, frame->retval, sp->data.vt,
         * signature->pinvoke);*/

        if (td->sp > td->stack)
          g_warning("CEE_MONO_RETOBJ: more values on stack: %d",
                    td->sp - td->stack);
        break;
      ...
      case CEE_MONO_LDPTR_INT_REQ_FLAG:
        interp_add_ins(td, MINT_MONO_LDPTR);
        td->last_ins->data[0] =
            get_data_item_index(td, &mono_thread_interruption_request_flag);
    ...
    case CEE_PREFIX1:
      ...
      case CEE_CEQ:
        ...
        if (td->sp[-1].type == STACK_TYPE_O ||
          ...
          if (td->sp[-1].type == STACK_TYPE_R4 &&
              td->sp[-2].type == STACK_TYPE_R8)
            interp_add_ins(td, MINT_CONV_R8_R4);
          if (td->sp[-1].type == STACK_TYPE_R8 &&
              td->sp[-2].type == STACK_TYPE_R4)
            interp_add_ins(td, MINT_CONV_R8_R4_SP);
      ...
      case CEE_SIZEOF: {
        ...
        if (mono_metadata_token_table(token) == MONO_TABLE_TYPESPEC &&
          ...
          int align;
          MonoType *type =
              mono_type_create_from_typespec_checked(image, token, error);
          goto_if_nok(error, exit);
      ...
      default:
        g_error("transform.c: Unimplemented opcode: 0xFE %02x (%s) at 0x%x\n",
                *td->ip, mono_opcode_name(256 + *td->ip),
                td->ip - header->code);
      }
      break;
    default:
    ...
    // No IR instructions were added as part of a bb_start IL instruction. Add a
    // MINT_NOP so we always have an instruction associated with a bb_start.
    // This is simple and avoids any complications associated with il_offset
    // tracking.
    if (prev_last_ins == td->last_ins &&
        (!inlining && td->is_bb_start[in_offset]) && td->ip < end)
...
static int find_in_offset(TransformData *td, int il_offset) {
  int i = il_offset;
...
// We store in the in_offset array the native_offset + 1, so 0 can mean only
// that the il offset is uninitialized. Otherwise 0 is valid value for first
// interp instruction.
static int get_in_offset(TransformData *td, int il_offset) {
  int target_offset = td->in_offsets[il_offset];
  g_assert(target_offset);
  return target_offset - 1;
}

static void handle_relocations(TransformData *td) {
  // Handle relocations
...
static int get_inst_length(InterpInst *ins) {
  if (ins->opcode == MINT_SWITCH)
    return MINT_SWITCH_LEN(READ32(&ins->data[0]));
#ifdef ENABLE_EXPERIMENT_TIERED
  else if (MINT_IS_PATCHABLE_CALL(ins->opcode))
    return MAX(mono_interp_oplen[MINT_JIT_CALL2],
               mono_interp_oplen[ins->opcode]);
#endif
  else
    return mono_interp_oplen[ins->opcode];
}

static void get_inst_stack_usage(TransformData *td, InterpInst *ins, int *pop,
                                 int *push) {
  ...
  if (mono_interp_oppop[opcode] == MINT_VAR_POP ||
    ...
    case MINT_VCALLVIRT_FAST: {
      ...
      *pop = imethod->param_count + imethod->hasthis;
      if (opcode == MINT_JIT_CALL)
        ...
        *push = opcode == MINT_CALL || opcode == MINT_CALLVIRT ||
                opcode == MINT_CALLVIRT_FAST;
      break;
    }
#ifndef ENABLE_NETCORE
    case MINT_CALLRUN: {
      MonoMethodSignature *csignature =
          (MonoMethodSignature *)td->data_items[ins->data[1]];
    ...
    case MINT_CALL_DELEGATE: {
      MonoMethodSignature *csignature =
          (MonoMethodSignature *)td->data_items[ins->data[0]];
    ...
    case MINT_CALL_VARARG: {
      ...
      MonoMethodSignature *csignature =
          (MonoMethodSignature *)td->data_items[ins->data[1]];
      *pop = imethod->param_count + imethod->hasthis + csignature->param_count -
             csignature->sentinelpos;
      *push = imethod->rtype->type != MONO_TYPE_VOID;
      break;
    }
...
static guint16 *emit_compacted_instruction(TransformData *td, guint16 *start_ip,
                                           InterpInst *ins) {
  guint16 opcode = ins->opcode;
  guint16 *ip = start_ip;

  // We know what IL offset this instruction was created for. We can now map the
  // IL offset to the IR offset. We use this array to resolve the relocations,
  // which reference the IL.
    ...
    *ip++ = ins->data[1];
    ...
    for (int i = 0; i < labels; i++) {
      ...
      *ip++ = 0xbeef;
             ...
             opcode == MINT_BR_S || opcode == MINT_LEAVE_S ||
             opcode == MINT_LEAVE_S_CHECK) {
    ...
    if (ins->data[0] < ins->il_offset) {
      ...
      *ip++ = 0xdead;
             ...
             opcode == MINT_BR || opcode == MINT_LEAVE ||
             opcode == MINT_LEAVE_CHECK) {
    const int br_offset = start_ip - td->new_code;
    ...
    SeqPoint *seqp =
        (SeqPoint *)mono_mempool_alloc0(td->mempool, sizeof(SeqPoint));
    InterpBasicBlock *cbb;
    ...
    cbb->seq_points =
        g_slist_prepend_mempool(td->mempool, cbb->seq_points, seqp);
    ...
    MonoMethod *target_method =
        (MonoMethod *)g_hash_table_lookup(td->patchsite_hash, ins);
    g_assert(target_method);
    g_hash_table_remove(td->patchsite_hash, ins);

    mini_tiered_record_callsite(start_ip, target_method,
    ...
    if (MINT_IS_LDLOC(opcode) || MINT_IS_STLOC(opcode) ||
        MINT_IS_STLOC_NP(opcode) || opcode == MINT_LDLOCA_S ||
...
static void generate_compacted_code(TransformData *td) {
  guint16 *ip;
  int size = 0;
  ...
  td->new_code = ip =
      (guint16 *)mono_domain_alloc0(td->rtm->domain, size * sizeof(guint16));
  ins = td->first_ins;
  while (ins) {
...
static int get_movloc_for_type(int mt) {
  switch (mt) {
  case MINT_TYPE_I1:
  case MINT_TYPE_U1:
...
// The value of local has changed. This means the contents of the stack where
// the local was loaded, no longer contain the value of the local. Clear them.
static void clear_stack_content_info_for_local(StackContentInfo *start,
                                               StackContentInfo *end,
                                               int local) {
  StackContentInfo *si;
...
// The value of argument has changed. This means the contents of the stack where
// the argument was loaded, no longer contain the value of the argument. Clear
// them.
static void clear_stack_content_info_for_argument(StackContentInfo *start,
                                                  StackContentInfo *end,
                                                  int argument) {
  StackContentInfo *si;
  for (si = start; si < end; si++) {
...
// The value of local has changed. This means we can no longer assume that any
// other local is a copy of this local.
static void clear_local_content_info_for_local(StackValue *start,
                                               StackValue *end, int local) {
  StackValue *sval;
...
static gboolean interp_local_deadce(TransformData *td, int *local_ref_count) {
  InterpInst *ins;
  ...
  for (int i = 0; i < td->locals_size; i++) {
    ...
    if (!local_ref_count[i] && !td->locals[i].indirects &&
  ...
  for (ins = td->first_ins; ins != NULL; ins = ins->next) {
    if (MINT_IS_STLOC_NP(ins->opcode)) {
      if (!local_ref_count[ins->data[0]] &&
          !td->locals[ins->data[0]].indirects) {
        interp_clear_ins(td, ins);
        mono_interp_stats.killed_instructions++;
        // We killed an instruction that makes use of the stack. This might
        // uncover new optimizations
        needs_cprop = TRUE;
      }
    } else if (MINT_IS_MOVLOC(ins->opcode)) {
      if (!local_ref_count[ins->data[1]] &&
          !td->locals[ins->data[1]].indirects) {
        interp_clear_ins(td, ins);
        mono_interp_stats.killed_instructions++;
      }
      ...
      if (!local_ref_count[ins->data[0]] &&
          !td->locals[ins->data[0]].indirects) {
        // We store to a dead stloc, we can replace it with a POP to save local
        // space
        ins->opcode = MINT_POP;
        mono_interp_stats.added_pop_count++;
        // We might to be able to kill both the pop and the instruction pushing
        // the value
        needs_cprop = TRUE;
      }
    }
...
#define INTERP_FOLD_CONV(opcode, stack_type_dst, field_dst, stack_type_src,    \
                         field_src, cast_type)                                 \
  case opcode:                                                                 \
...
#define INTERP_FOLD_CONV_FULL(opcode, stack_type_dst, field_dst,               \
                              stack_type_src, field_src, cast_type, cond)      \
  case opcode:                                                                 \
    g_assert(sp->val.type == stack_type_src);                                  \
    if (!(cond))                                                               \
      goto cfold_failed;                                                       \
...
static InterpInst *interp_fold_unop(TransformData *td, StackContentInfo *sp,
                                    InterpInst *ins) {
  StackValue result;

  if (ins->opcode >= MINT_CONV_I4_I8_SP && ins->opcode <= MINT_CONV_R8_R4_SP) {
    ...
    switch (ins->opcode) {
      INTERP_FOLD_CONV(MINT_CONV_I4_I8_SP, STACK_VALUE_I4, i, STACK_VALUE_I8, l,
                       gint32);
      INTERP_FOLD_CONV(MINT_CONV_I8_I4_SP, STACK_VALUE_I8, l, STACK_VALUE_I4, i,
                       gint64);
    default:
      goto cfold_failed;
    }
    ...
    switch (ins->opcode) {
      ...
      INTERP_FOLD_CONV(MINT_CONV_I1_I4, STACK_VALUE_I4, i, STACK_VALUE_I4, i,
                       gint8);
      INTERP_FOLD_CONV(MINT_CONV_I1_I8, STACK_VALUE_I4, i, STACK_VALUE_I8, l,
                       gint8);
      INTERP_FOLD_CONV(MINT_CONV_U1_I4, STACK_VALUE_I4, i, STACK_VALUE_I4, i,
                       guint8);
      INTERP_FOLD_CONV(MINT_CONV_U1_I8, STACK_VALUE_I4, i, STACK_VALUE_I8, l,
                       guint8);

      INTERP_FOLD_CONV(MINT_CONV_I2_I4, STACK_VALUE_I4, i, STACK_VALUE_I4, i,
                       gint16);
      INTERP_FOLD_CONV(MINT_CONV_I2_I8, STACK_VALUE_I4, i, STACK_VALUE_I8, l,
                       gint16);
      INTERP_FOLD_CONV(MINT_CONV_U2_I4, STACK_VALUE_I4, i, STACK_VALUE_I4, i,
                       guint16);
      INTERP_FOLD_CONV(MINT_CONV_U2_I8, STACK_VALUE_I4, i, STACK_VALUE_I8, l,
                       guint16);

      INTERP_FOLD_CONV(MINT_CONV_I4_I8, STACK_VALUE_I4, i, STACK_VALUE_I8, l,
                       gint32);
      INTERP_FOLD_CONV(MINT_CONV_U4_I8, STACK_VALUE_I4, i, STACK_VALUE_I8, l,
                       gint32);

      INTERP_FOLD_CONV(MINT_CONV_I8_I4, STACK_VALUE_I8, l, STACK_VALUE_I4, i,
                       gint32);
      INTERP_FOLD_CONV(MINT_CONV_I8_U4, STACK_VALUE_I8, l, STACK_VALUE_I4, i,
                       guint32);

      INTERP_FOLD_CONV_FULL(
          MINT_CONV_OVF_I1_I4, STACK_VALUE_I4, i, STACK_VALUE_I4, i, gint8,
          sp[0].val.i >= G_MININT8 && sp[0].val.i <= G_MAXINT8);
      INTERP_FOLD_CONV_FULL(
          MINT_CONV_OVF_I1_I8, STACK_VALUE_I4, i, STACK_VALUE_I8, l, gint8,
          sp[0].val.l >= G_MININT8 && sp[0].val.l <= G_MAXINT8);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_I1_U4, STACK_VALUE_I4, i,
                            STACK_VALUE_I4, i, gint8,
                            sp[0].val.i >= 0 && sp[0].val.i <= G_MAXINT8);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_I1_U8, STACK_VALUE_I4, i,
                            STACK_VALUE_I8, l, gint8,
                            sp[0].val.l >= 0 && sp[0].val.l <= G_MAXINT8);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_U1_I4, STACK_VALUE_I4, i,
                            STACK_VALUE_I4, i, guint8,
                            sp[0].val.i >= 0 && sp[0].val.i <= G_MAXUINT8);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_U1_I8, STACK_VALUE_I4, i,
                            STACK_VALUE_I8, l, guint8,
                            sp[0].val.l >= 0 && sp[0].val.l <= G_MAXUINT8);

      INTERP_FOLD_CONV_FULL(
          MINT_CONV_OVF_I2_I4, STACK_VALUE_I4, i, STACK_VALUE_I4, i, gint16,
          sp[0].val.i >= G_MININT16 && sp[0].val.i <= G_MAXINT16);
      INTERP_FOLD_CONV_FULL(
          MINT_CONV_OVF_I2_I8, STACK_VALUE_I4, i, STACK_VALUE_I8, i, gint16,
          sp[0].val.l >= G_MININT16 && sp[0].val.l <= G_MAXINT16);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_I2_U4, STACK_VALUE_I4, i,
                            STACK_VALUE_I4, i, gint16,
                            sp[0].val.i >= 0 && sp[0].val.i <= G_MAXINT16);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_I2_U8, STACK_VALUE_I4, i,
                            STACK_VALUE_I8, l, gint16,
                            sp[0].val.l >= 0 && sp[0].val.l <= G_MAXINT16);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_U2_I4, STACK_VALUE_I4, i,
                            STACK_VALUE_I4, i, guint16,
                            sp[0].val.i >= 0 && sp[0].val.i <= G_MAXUINT16);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_U2_I8, STACK_VALUE_I4, i,
                            STACK_VALUE_I8, l, guint16,
                            sp[0].val.l >= 0 && sp[0].val.l <= G_MAXUINT16);

      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_I4_U4, STACK_VALUE_I4, i,
                            STACK_VALUE_I4, i, gint32, sp[0].val.i >= 0);
      INTERP_FOLD_CONV_FULL(
          MINT_CONV_OVF_I4_I8, STACK_VALUE_I4, i, STACK_VALUE_I8, l, gint32,
          sp[0].val.l >= G_MININT32 && sp[0].val.l <= G_MAXINT32);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_I4_U8, STACK_VALUE_I4, i,
                            STACK_VALUE_I8, l, gint32,
                            sp[0].val.l >= 0 && sp[0].val.l <= G_MAXINT32);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_U4_I4, STACK_VALUE_I4, i,
                            STACK_VALUE_I4, i, guint32, sp[0].val.i >= 0);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_U4_I8, STACK_VALUE_I4, i,
                            STACK_VALUE_I8, l, guint32,
                            sp[0].val.l >= 0 && sp[0].val.l <= G_MAXINT32);

      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_I8_U8, STACK_VALUE_I8, l,
                            STACK_VALUE_I8, l, gint64, sp[0].val.l >= 0);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_U8_I4, STACK_VALUE_I8, l,
                            STACK_VALUE_I4, i, guint64, sp[0].val.i >= 0);
      INTERP_FOLD_CONV_FULL(MINT_CONV_OVF_U8_I8, STACK_VALUE_I8, l,
                            STACK_VALUE_I8, l, guint64, sp[0].val.l >= 0);
  ...
  if (sp->ins != NULL) {
    // The instruction that pushed the top of stack can be replaced with the new
    // constant result
...
static InterpInst *interp_fold_binop(TransformData *td, StackContentInfo *sp,
                                     InterpInst *ins) {
  StackValue result;
  // Decrement sp so it's easier to access top of the stack
  sp -= 2;
  ...
  switch (ins->opcode) {
    ...
    INTERP_FOLD_BINOP_FULL(
        MINT_DIV_I4, STACK_VALUE_I4, i, /, gint32,
        sp[1].val.i != 0 && (sp[0].val.i != G_MININT32 || sp[1].val.i != -1));
    INTERP_FOLD_BINOP_FULL(
        MINT_DIV_I8, STACK_VALUE_I8, l, /, gint64,
        sp[1].val.l != 0 && (sp[0].val.l != G_MININT64 || sp[1].val.l != -1));
    INTERP_FOLD_BINOP_FULL(MINT_DIV_UN_I4, STACK_VALUE_I4, i, /, guint32,
                           sp[1].val.i != 0);
    INTERP_FOLD_BINOP_FULL(MINT_DIV_UN_I8, STACK_VALUE_I8, l, /, guint64,
                           sp[1].val.l != 0);

    INTERP_FOLD_BINOP_FULL(
        MINT_REM_I4, STACK_VALUE_I4, i, %, gint32,
        sp[1].val.i != 0 && (sp[0].val.i != G_MININT32 || sp[1].val.i != -1));
    INTERP_FOLD_BINOP_FULL(
        MINT_REM_I8, STACK_VALUE_I8, l, %, gint64,
        sp[1].val.l != 0 && (sp[0].val.l != G_MININT64 || sp[1].val.l != -1));
    INTERP_FOLD_BINOP_FULL(MINT_REM_UN_I4, STACK_VALUE_I4, i, %, guint32,
                           sp[1].val.i != 0);
    INTERP_FOLD_BINOP_FULL(MINT_REM_UN_I8, STACK_VALUE_I8, l, %, guint64,
                           sp[1].val.l != 0);
...
static gboolean interp_local_equal(StackValue *locals, int local1, int local2) {
  if (local1 == local2)
    return TRUE;
  if (locals[local1].type == STACK_VALUE_LOCAL &&
      locals[local1].local == local2) {
    // local1 is a copy of local2
    return TRUE;
  }
  if (locals[local2].type == STACK_VALUE_LOCAL &&
      locals[local2].local == local1) {
    // local2 is a copy of local1
    return TRUE;
  }
  if (locals[local1].type == STACK_VALUE_I4 &&
      locals[local2].type == STACK_VALUE_I4)
    return locals[local1].i == locals[local2].i;
  if (locals[local1].type == STACK_VALUE_I8 &&
      locals[local2].type == STACK_VALUE_I8)
    return locals[local1].l == locals[local2].l;
  return FALSE;
}

static void interp_cprop(TransformData *td) {
  if (!td->max_stack_height)
    return;
  StackContentInfo *stack = (StackContentInfo *)g_malloc(
      td->max_stack_height * sizeof(StackContentInfo));
  StackContentInfo *stack_end = stack + td->max_stack_height;
  StackContentInfo *sp;
  StackValue *locals =
      (StackValue *)g_malloc(td->locals_size * sizeof(StackValue));
...
retry:
  ...
  for (ins = td->first_ins; ins != NULL; ins = ins->next) {
    ...
    gboolean is_bb_start = il_offset != -1 && td->is_bb_start[il_offset] &&
                           il_offset != last_il_offset;
    if (is_bb_start) {
    ...
    if (MINT_IS_LDLOC(ins->opcode)) {
      ...
      if (prev_ins && MINT_IS_STLOC(prev_ins->opcode) &&
          !interp_is_bb_start(td, prev_ins, ins) &&
          interp_local_equal(locals, prev_ins->data[0], loaded_local)) {
        ...
        if (ins->opcode - MINT_LDLOC_I1 == mt) {
          ...
          if (replace_op) {
            ...
            if (sp->val.type == STACK_VALUE_NONE &&
                !td->locals[stored_local].indirects) {
        ...
        if (td->verbose_level) {
          g_print("cprop loc %d -> loc %d :\n\t", loaded_local,
                  locals[loaded_local].local);
          dump_interp_inst_newline(ins);
        }
      } else if (locals[loaded_local].type == STACK_VALUE_I4 ||
                 locals[loaded_local].type == STACK_VALUE_I8) {
        gboolean is_i4 = locals[loaded_local].type == STACK_VALUE_I4;
        g_assert(!td->locals[loaded_local].indirects);
        if (is_i4)
          ins = interp_get_ldc_i4_from_const(td, ins, locals[loaded_local].i);
        else
          ins = interp_inst_replace_with_i8_const(td, ins,
                                                  locals[loaded_local].l);
        sp->ins = ins;
      ...
      if (sp->val.type == STACK_VALUE_LOCAL) {
        ...
        if (td->locals[src_local].mt == td->locals[dest_local].mt) {
          ...
          if (sp->ins) {
            // If the top of stack is not pushed by a ldloc, we are introducing
            // a new dependency on the src_local since we are adding a movloc
            // from it.
            ...
            ins = interp_insert_ins(
                td, ins, get_movloc_for_type(td->locals[src_local].mt));
            ins->data[0] = src_local;
            ins->data[1] = dest_local;
            if (vtsize)
              ins->data[2] = vtsize;
            // Clear ldloc / stloc pair and replace it with movloc
            // superinstruction
            if (td->verbose_level) {
      ...
      int dest_local = ins->data[0];
      // Prevent optimizing away the instruction that pushed the value on the
      // stack
      sp[-1].ins = NULL;
      ...
      if (sp->ins) {
        ...
        interp_clear_ins(td, ins);
        mono_interp_stats.killed_instructions += 2;
        // The value pop-ed by this instruction can still be accessed. If we
        // also kill the instruction pushing the value, then we need to empty
        // the value of the stack, so it is not considered for further
        // optimizations.
        sp->val.type = STACK_VALUE_NONE;
      }
    } else if ((ins->opcode >= MINT_NEWOBJ_FAST &&
                ins->opcode <= MINT_NEWOBJ_VTST_FAST) &&
               ins->data[0] == INLINED_METHOD_FLAG) {
      int param_count = ins->data[1];
      // memmove the stack values while clearing ins, to prevent instruction
      // removal
      for (int i = 1; i <= param_count; i++) {
        sp[-i + 2] = sp[-i];
        sp[-i + 2].ins = NULL;
      }
      // clear stack information for the slots where the allocated object
      // resides
      memset(&sp[-param_count], 0, 2 * sizeof(StackContentInfo));
      sp += 2;
    } else if (ins->opcode == MINT_CASTCLASS ||
               ins->opcode == MINT_CASTCLASS_COMMON ||
               ins->opcode == MINT_CASTCLASS_INTERFACE) {
      ...
      sp--;
    } else if (ins->opcode == MINT_LDLOCA_S &&
               MINT_IS_LDFLD(ins->next->opcode) &&
               td->locals[ins->data[0]].mt ==
                   (ins->next->opcode - MINT_LDFLD_I1) &&
               ins->next->data[0] == 0) {
      ...
      if (src->ins) {
        if (src->ins->opcode == MINT_LDLOCA_S &&
            td->locals[src->ins->data[0]].mt == (ins->opcode - MINT_STFLD_I1) &&
            ins->data[0] == 0) {
          ...
          mono_interp_stats.killed_instructions++;
          mono_interp_stats.ldlocas_removed++;
          // FIXME Update stack contents for stloc, we currently rely on cprop
          // running again.
        ...
        } else if (src->val.type == STACK_VALUE_LOCAL &&
      ...
      if (push == 1 && pop == 0 && !MINT_IS_CALL(ins->opcode) &&
          !MINT_IS_NEWOBJ(ins->opcode))
        sp[-1].ins = ins;
    }
...
void mono_test_interp_cprop(TransformData *td) { interp_cprop(td); }

static void interp_super_instructions(TransformData *td) {
  InterpInst *ins;
  ...
  for (ins = td->first_ins; ins != NULL; ins = ins->next) {
    ...
    if (ins->opcode >= MINT_LDFLD_I1 && ins->opcode <= MINT_LDFLD_O &&
        prev1_ins) {
      if (prev1_ins->opcode == MINT_LDLOC_O) {
        ...
        mono_interp_stats.super_instructions++;
        mono_interp_stats.killed_instructions++;
      } else if (prev1_ins->opcode == MINT_LDARG_O ||
                 prev1_ins->opcode == MINT_LDARG_P0) {
        int arg_index = 0;
        int fld_offset = ins->data[0];
        int mt = ins->opcode - MINT_LDFLD_I1;
      ...
      if (prev1_ins->opcode == MINT_ADD1_I4 ||
          prev1_ins->opcode == MINT_ADD1_I8 ||
          prev1_ins->opcode == MINT_SUB1_I4 ||
          prev1_ins->opcode == MINT_SUB1_I8) {
        if (MINT_IS_LDLOC(prev2_ins->opcode) &&
            prev2_ins->data[0] == ins->data[0]) {
          if (prev1_ins->opcode == MINT_ADD1_I4)
          ...
          // the local index is already set inside the replaced STLOC
          // instruction
          interp_clear_ins(td, prev1_ins);
...
static void interp_optimize_code(TransformData *td) {
  ...
  if (mono_interp_opt & INTERP_OPT_SUPER_INSTRUCTIONS)
    MONO_TIME_TRACK(mono_interp_stats.super_instructions_time,
                    interp_super_instructions(td));
}

static void generate(MonoMethod *method, MonoMethodHeader *header,
                     InterpMethod *rtm, MonoGenericContext *generic_context,
                     MonoError *error) {
  MonoDomain *domain = rtm->domain;
  int i;
  ...
  td->stack_state =
      (StackInfo **)g_malloc0(header->code_size * sizeof(StackInfo *));
  ...
  interp_method_compute_offsets(td, rtm, mono_method_signature_internal(method),
                                header, error);
  goto_if_nok(error, exit);
  ...
  td->stack =
      (StackInfo *)g_malloc0((header->max_stack + 1) * sizeof(td->stack[0]));
  ...
  rtm->clauses = (MonoExceptionClause *)mono_domain_alloc0(
      domain, header->num_clauses * sizeof(MonoExceptionClause));
  memcpy(rtm->clauses, header->clauses,
         header->num_clauses * sizeof(MonoExceptionClause));
  ...
  for (i = 0; i < header->num_clauses; i++) {
    ...
    g_assert(c->handler_len >= 0 &&
             (c->handler_offset + c->handler_len) <= code_len_u16);
    if (c->flags & MONO_EXCEPTION_CLAUSE_FILTER)
  ...
  rtm->alloca_size = ALIGN_TO(
      rtm->total_locals_size + rtm->vt_stack_size + rtm->stack_size, 8);
  rtm->data_items = (gpointer *)mono_domain_alloc0(
      domain, td->n_data_items * sizeof(td->data_items[0]));
  memcpy(rtm->data_items, td->data_items,
         td->n_data_items * sizeof(td->data_items[0]));

  /* Save debug info */
  interp_save_debug_info(rtm, header, td, td->line_numbers);

  /* Create a MonoJitInfo for the interpreted method by creating the interpreter
   * IR as the native code. */
  int jinfo_len;
  ...
  jinfo->is_interp = 1;
  rtm->jinfo = jinfo;
  mono_jit_info_init(jinfo, method, (guint8 *)rtm->code, code_len_u8,
                     (MonoJitInfoFlags)0, header->num_clauses, 0);
  for (i = 0; i < jinfo->num_clauses; ++i) {
    ...
    if (ei->flags == MONO_EXCEPTION_CLAUSE_FILTER) {
      ...
      ei->data.handler_end =
          (guint8 *)(rtm->code + c->handler_offset + c->handler_len);
    } else {
...
gboolean mono_test_interp_generate_code(TransformData *td, MonoMethod *method,
                                        MonoMethodHeader *header,
                                        MonoGenericContext *generic_context,
                                        MonoError *error) {
...
#ifdef ENABLE_EXPERIMENT_TIERED
static gboolean tiered_patcher(MiniTieredPatchPointContext *ctx,
                               gpointer patchsite) {
  ERROR_DECL(error);
...
#ifdef ENABLE_EXPERIMENT_TIERED
  mini_tiered_register_callsite_patcher(tiered_patcher,
                                        TIERED_PATCH_KIND_INTERP);
#endif
}

void mono_interp_transform_method(InterpMethod *imethod, ThreadContext *context,
                                  MonoError *error) {
  ...
  if (mono_class_is_open_constructed_type(
    ...
    mono_error_set_invalid_operation(
        error, "%s",
        "Could not execute the method because the containing type is not fully "
        "instantiated.");
    return;
  }
  ...
  method_class_vt =
      mono_class_vtable_checked(domain, imethod->method->klass, error);
  return_if_nok(error);
  ...
  mono_os_mutex_lock(&calc_section);
  if (!imethod->transformed) {
    // Ignore the first two fields which are unchanged. next_jit_code_hash
    // shouldn't be modified because it is racy with internal hash table insert.
  ...
  if (!g_hash_table_lookup(domain_jit_info(domain)->seq_points,
                           imethod->method))
    g_hash_table_insert(domain_jit_info(domain)->seq_points, imethod->method,
                        imethod->jinfo->seq_points);
  mono_domain_unlock(domain);
