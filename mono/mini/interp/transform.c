...
// StackValue contains data to construct an InterpInst that is equivalent with
// the contents of the stack slot / local / argument.
typedef struct {
  // Indicates the type of the stored information. It can be a local, argument
  // or a constant
  int type;
...
typedef struct {
  // This indicates what is currently stored in this stack slot. This can be a
  // constant or the copy of a local / argument.
  StackValue val;
  // The instruction that pushed this stack slot. If ins is null, we can't
  // remove the usage of the stack slot, because we can't clear the instruction
  // that set it.
  InterpInst *ins;
} StackContentInfo;
...
struct InterpInst {
  ...
  // If this is -1, this instruction is not logically associated with an IL
  // offset, it is part of the IL instruction associated with the previous
  // interp instruction.
  int il_offset;
  guint32 flags;
...
typedef enum { RELOC_SHORT_BRANCH, RELOC_LONG_BRANCH, RELOC_SWITCH } RelocType;
...
static gboolean jit_call_supported(MonoMethod *method,
  ...
  if (constrained_class) {
    ...
    if (!m_class_is_valuetype(constrained_class)) {
      ...
      if (target_method->klass == mono_defaults.enum_class &&
          (td->sp - csignature->param_count - 1)->type == STACK_TYPE_MP) {
        /* managed pointer on the stack, we need to deref that puppy */
    ...
    if (op == MINT_CALLRUN) {
      ...
      td->last_ins->data[1] = get_data_item_index(
          td, mono_method_signature_internal(target_method));
             ...
             jit_call_supported(target_method, csignature)) {
