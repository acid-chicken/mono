...
#define MINT_IS_STLOC_NP(op)                                                   \
  ((op) >= MINT_STLOC_NP_I4 && (op) <= MINT_STLOC_NP_O)
#define MINT_IS_LDARG(op) ((op) >= MINT_LDARG_I1 && (op) <= MINT_LDARG_VT)
#define MINT_IS_STARG(op) ((op) >= MINT_STARG_I1 && (op) <= MINT_STARG_VT)
#define MINT_IS_CONDITIONAL_BRANCH(op)                                         \
  ((op) >= MINT_BRFALSE_I4 && (op) <= MINT_BLT_UN_R8_S)
...
#define MINT_IS_LDLOCFLD(op)                                                   \
  ((op) >= MINT_LDLOCFLD_I1 && (op) <= MINT_LDLOCFLD_P)
#define MINT_IS_STLOCFLD(op)                                                   \
  ((op) >= MINT_STLOCFLD_I1 && (op) <= MINT_STLOCFLD_P)
...
extern char *mono_interp_dis_mintop(gint32 ins_offset, gboolean native_offset,
