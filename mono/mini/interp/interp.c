 ...
 */
static InterpFrame *alloc_frame(ThreadContext *ctx, gpointer native_stack_addr,
                                InterpFrame *parent, InterpMethod *imethod,
                                stackval *stack_args, stackval *retval) {
  StackFragment *frag;
  InterpFrame *frame;
    ...
    ++sp;                                                                      \
      ...
      MINT_IN_CASE(MINT_CALLI_NAT) {
        ...
        InterpFrame *const child_frame =
            alloc_frame(context, &retval, frame, NULL, sp, retval);
      ...
      MINT_IN_CASE(MINT_NEWOBJ) {
        ...
        if (m_class_is_valuetype(newobj_class)) {
          ...
          if (newobj_class != mono_defaults.string_class) {
            ...
            OBJREF(o) =
                mono_object_new_checked(imethod->domain, newobj_class, error);
            mono_error_cleanup(error); // FIXME: do not swallow the error
            error_init_reuse(error);
            EXCEPTION_CHECKPOINT;
            ...
            if (mono_object_is_transparent_proxy(o)) {
              MonoMethod *remoting_invoke_method =
                  mono_marshal_get_remoting_invoke_with_check(cmethod->method,
                                                              error);
              mono_error_assert_ok(error);
              cmethod = mono_interp_get_imethod(imethod->domain,
                                                remoting_invoke_method, error);
              mono_error_assert_ok(error);
            }
#endif
          } else {
      ...
      *(gpointer *)sp->data.p = frame->imethod->data_items[ip[1]];
      ...
      MINT_IN_CASE(MINT_LEAVE_S_CHECK) {
        ...
        if (check && frame->imethod->method->wrapper_type !=
          ...
          // FIXME Free this frame earlier?
          InterpFrame *const child_frame =
              alloc_frame(context, &dummy, frame, NULL, NULL, NULL);
