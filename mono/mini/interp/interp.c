...
interp_exec_method_call_vararg(InterpFrame *frame, ThreadContext *context,
                               MonoError *error)
...
static MONO_NEVER_INLINE void
interp_exec_method_newobj_fast(InterpFrame *frame, ThreadContext *context,
                               MonoError *error)
// This function makes WebAsssembly stacks clearer, so you can see which
// recursion is occuring, in the absence of line numbers in the debugger.
{
...
static MONO_ALWAYS_INLINE void method_entry(ThreadContext *context,
                                            InterpFrame *frame,
    ...
    ++sp;                                                                      \
      ...
      MINT_IN_CASE(MINT_CALLI) {
        ...
        if (G_UNLIKELY(!imethod->transformed)) {
          ...
          child_frame = alloc_frame(context, native_stack_addr, frame, imethod,
                                    sp, retval);
      ...
      MINT_IN_CASE(MINT_VCALLVIRT_FAST) {
        ...
        if (code_type == IMETHOD_CODE_INTERP) {
          ...
          if (G_UNLIKELY(!imethod->transformed)) {
            ...
            child_frame =
                alloc_frame(context, &retval, frame, imethod, sp, retval);
            method_entry(context, child_frame, &tracing, &ex);
            if (G_UNLIKELY(ex)) {
              frame = child_frame;
              frame->ip = NULL;
      ...
      MINT_IN_CASE(MINT_CALL_VARARG) {
        ...
        child_frame = alloc_frame(
            context, &retval, frame,
            (InterpMethod *)frame->imethod->data_items[ip[1]], NULL, retval);
        ...
        sp -= child_frame->imethod->param_count +
              child_frame->imethod->hasthis + num_varargs;
  ...
  ++ip;
      ...
      MINT_IN_CASE(MINT_NEWOBJ_FAST) {
        ...
        if (is_inlined) {
          ...
          InterpMethod *ctor_method =
              (InterpMethod *)frame->imethod->data_items[imethod_index];
          frame->ip = ip;

          child_frame =
              alloc_frame(context, &vtable, frame, ctor_method, sp, NULL);
