...
/* Arguments that are passed when invoking only a finally/filter clause from the
 * frame */
...
/*
 * This code synchronizes with interp_mark_stack () using compiler memory
 * barriers.
 */

static StackFragment *stack_frag_new(int size) {
...
static void frame_stack_init(FrameStack *stack, int size) {
  StackFragment *frag;
...
static StackFragment *add_frag(FrameStack *stack, int size) {
...
static void free_frag(StackFragment *frag) {
  while (frag) {
...
static MONO_ALWAYS_INLINE gpointer frame_stack_alloc(FrameStack *stack,
                                                     int size,
                                                     StackFragment **out_frag) {
  StackFragment *current = stack->current;
  gpointer res;
...
frame_stack_pop(FrameStack *stack, StackFragment *frag, gpointer pos) {
  g_assert((guint8 *)pos >= (guint8 *)&frag->data &&
           (guint8 *)pos <= (guint8 *)frag->end);
  stack->current = frag;
  mono_compiler_barrier();
...
static void frame_stack_free(FrameStack *stack) {
  stack->inited = 0;
  mono_compiler_barrier();
 ...
 */
static void reinit_frame(InterpFrame *frame, InterpFrame *parent,
                         InterpMethod *imethod, stackval *stack_args,
                         stackval *retval) {
  frame->parent = parent;
 ...
 */
static MONO_ALWAYS_INLINE void alloc_stack_data(ThreadContext *ctx,
                                                InterpFrame *frame, int size) {
  StackFragment *frag;
...
static gpointer alloc_extra_stack_data(ThreadContext *ctx, int size) {
  StackFragment *frag;
 ...
 */
static void pop_frame(ThreadContext *context, InterpFrame *frame) {
  if (frame->stack)
    frame_stack_pop(&context->data_stack, frame->data_frag, frame->stack);
}

/*
 * List of classes whose methods will be executed by transitioning to JITted
 * code. Used for testing.
 */
GSList *mono_interp_jit_classes;
/* Optimizations enabled with interpreter */
int mono_interp_opt = INTERP_OPT_DEFAULT;
/* If TRUE, interpreted code will be interrupted at function entry/backward
 * branches */
...
static void interp_exec_method(InterpFrame *frame, ThreadContext *context,
                               FrameClauseArgs *clause_args, MonoError *error);
...
static void db_match_method(gpointer data, gpointer user_data) {
...
static void debug_enter(InterpFrame *frame, int *tracing) {
  if (db_methods) {
    g_list_foreach(db_methods, db_match_method,
                   (gpointer)frame->imethod->method);
    if (break_on_method)
...
#define MINT_IN_SWITCH(op)                                                     \
  COUNT_OP(op);                                                                \
  switch (opcode = (MintOpcode)(op))
#define MINT_IN_CASE(x) case x:
#define MINT_IN_BREAK break

#endif

static GSList *clear_resume_state(ThreadContext *context, GSList *finally_ips) {
  /* We have thrown an exception from a finally block. Some of the leave targets
   * were unwound already */
  while (finally_ips && finally_ips->data >= context->handler_ei->try_start &&
...
static ThreadContext *get_context(void) {
  ThreadContext *context =
      (ThreadContext *)mono_native_tls_get_value(thread_context_id);
  if (context == NULL) {
    context = g_new0(ThreadContext, 1);
    /*
     * Initialize data stack.
     * There are multiple advantages to this being separate from the frame
     * stack. Frame stack can be alloca. Frame stack can be perfectly fit (if
     * heap). Frame stack can skip GC tracking.
     */
...
static void interp_free_context(gpointer ctx) {
...
static void mono_interp_error_cleanup(MonoError *error) {
  mono_error_cleanup(error); /* FIXME: don't swallow the error */
  error_init_reuse(
      error); // one instruction, so this function is good inline candidate
}

static MONO_NEVER_INLINE void ves_real_abort(int line, MonoMethod *mh,
                                             const unsigned short *ip,
                                             stackval *stack, stackval *sp) {
  ERROR_DECL(error);
  ...
  g_printerr("Line=%d IP=0x%04lx, Aborted execution\n", line,
             ip - (const unsigned short *)header->code);
...
static InterpMethod *lookup_imethod(MonoDomain *domain, MonoMethod *method) {
  ...
  imethod = (InterpMethod *)mono_internal_hash_table_lookup(
      &info->interp_code_hash, method);
  mono_domain_jit_code_hash_unlock(domain);
  return imethod;
}

static gpointer interp_get_remoting_invoke(MonoMethod *method, gpointer addr,
                                           MonoError *error) {
  ...
  MonoMethod *remoting_invoke_method =
      mono_marshal_get_remoting_invoke(imethod->method, error);
  return_val_if_nok(error, NULL);
  return mono_interp_get_imethod(mono_domain_get(), remoting_invoke_method,
                                 error);
#else
  g_assert_not_reached();
  return NULL;
...
InterpMethod *mono_interp_get_imethod(MonoDomain *domain, MonoMethod *method,
                                      MonoError *error) {
  InterpMethod *imethod;
  MonoJitDomainInfo *info;
  ...
  imethod->param_types = (MonoType **)mono_domain_alloc0(
      domain, sizeof(MonoType *) * sig->param_count);
  ...
  imethod->prof_flags =
      mono_profiler_get_call_instrumentation_flags(imethod->method);
...
#else
#define INTERP_PUSH_LMF_WITH_CTX_BODY(ext, exit_label)                         \
  g_error("requires working mono-context");
#endif
...
static void interp_pop_lmf(MonoLMFExt *ext) { mono_pop_lmf(&ext->lmf); }

static InterpMethod *get_virtual_method(InterpMethod *imethod,
                                        MonoVTable *vtable) {
  ...
  if (mono_class_is_transparent_proxy(vtable->klass)) {
    ERROR_DECL(error);
    MonoMethod *remoting_invoke_method =
        mono_marshal_get_remoting_invoke_with_check(m, error);
  ...
  if ((m->flags & METHOD_ATTRIBUTE_FINAL) ||
      !(m->flags & METHOD_ATTRIBUTE_VIRTUAL)) {
    if (m->iflags & METHOD_IMPL_ATTRIBUTE_SYNCHRONIZED) {
      ERROR_DECL(error);
      ret = mono_interp_get_imethod(
          domain, mono_marshal_get_synchronized_wrapper(m), error);
      mono_error_cleanup(error); /* FIXME: don't swallow the error */
    } else {
      ret = imethod;
    }
  ...
  if (mono_class_is_interface(m->klass)) {
    ...
    slot += mono_class_interface_offset_with_variance(vtable->klass, m->klass,
                                                      &non_exact_match);
  }
  ...
  if (m->is_inflated && mono_method_get_context(m)->method_inst) {
    ...
    if (mono_class_is_ginst(virtual_method->klass))
      context.class_inst = mono_class_get_generic_class(virtual_method->klass)
                               ->context.class_inst;
    else if (mono_class_is_gtd(virtual_method->klass))
      context.class_inst =
          mono_class_get_generic_container(virtual_method->klass)
              ->context.class_inst;
    context.method_inst = mono_method_get_context(m)->method_inst;

    ERROR_DECL(error);
    virtual_method = mono_class_inflate_generic_method_checked(virtual_method,
                                                               &context, error);
    mono_error_cleanup(error); /* FIXME: don't swallow the error */
  }
  ...
  if (virtual_method->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) {
    virtual_method =
        mono_marshal_get_native_wrapper(virtual_method, FALSE, FALSE);
  }
  ...
  ERROR_DECL(error);
  InterpMethod *virtual_imethod =
      mono_interp_get_imethod(domain, virtual_method, error);
  mono_error_cleanup(error); /* FIXME: don't swallow the error */
...
static GSList *append_imethod(MonoDomain *domain, GSList *list,
                              InterpMethod *imethod,
                              InterpMethod *target_imethod) {
  GSList *ret;
  InterpVTableEntry *entry;

  entry = (InterpVTableEntry *)mono_mempool_alloc(domain->mp,
...
static InterpMethod *get_target_imethod(GSList *list, InterpMethod *imethod) {
  while (list != NULL) {
...
static gpointer *get_method_table(MonoVTable *vtable, int offset) {
  if (offset >= 0)
...
static gpointer *alloc_method_table(MonoVTable *vtable, int offset) {
  gpointer *table;

  if (offset >= 0) {
    table = mono_domain_alloc0(vtable->domain,
...
get_virtual_method_fast(InterpMethod *imethod, MonoVTable *vtable, int offset) {
  ...
  if (!table[offset]) {
    ...
    if (!table[offset]) {
      if (imethod->method->is_inflated || offset < 0)
        table[offset] =
            append_imethod(vtable->domain, NULL, imethod, target_imethod);
      else
  ...
  if ((gsize)table[offset] & 0x1) {
    ...
    InterpMethod *target_imethod =
        get_target_imethod((GSList *)table[offset], imethod);

    if (!target_imethod) {
      target_imethod = get_virtual_method(imethod, vtable);
      mono_domain_lock(vtable->domain);
      if (!get_target_imethod((GSList *)table[offset], imethod))
        table[offset] = append_imethod(vtable->domain, (GSList *)table[offset],
                                       imethod, target_imethod);
      mono_domain_unlock(vtable->domain);
    }
...
static void inline stackval_from_data(MonoType *type, stackval *result,
                                      const void *data, gboolean pinvoke) {
  ...
  case MONO_TYPE_VALUETYPE:
    if (m_class_is_enumtype(type->data.klass)) {
      stackval_from_data(mono_class_enum_basetype_internal(type->data.klass),
                         result, data, pinvoke);
      return;
    } else if (pinvoke) {
      memcpy(result->data.vt, data,
             mono_class_native_size(type->data.klass, NULL));
    } else {
  ...
  case MONO_TYPE_GENERICINST: {
    ...
    stackval_from_data(
        m_class_get_byval_arg(type->data.generic_class->container_class),
        result, data, pinvoke);
    return;
  }
  default:
...
static void inline stackval_to_data(MonoType *type, stackval *val, void *data,
                                    gboolean pinvoke) {
  type = mini_native_type_replace_type(type);
  if (type->byref) {
  ...
  case MONO_TYPE_I: {
    ...
    /* In theory the value used by stloc should match the local var type
       ...
       a native int - both by csc and mcs). Not sure what to do about sign
       extension as it is outside the spec... doing the obvious */
  ...
  case MONO_TYPE_VALUETYPE:
    if (m_class_is_enumtype(type->data.klass)) {
      stackval_to_data(mono_class_enum_basetype_internal(type->data.klass), val,
                       data, pinvoke);
      return;
    } else if (pinvoke) {
      memcpy(data, val->data.vt,
             mono_class_native_size(type->data.klass, NULL));
    } else {
  ...
  case MONO_TYPE_GENERICINST: {
    ...
    stackval_to_data(
        m_class_get_byval_arg(type->data.generic_class->container_class), val,
        data, pinvoke);
    return;
  }
  default:
 ...
 */
static gpointer stackval_to_data_addr(MonoType *type, stackval *val) {
  type = mini_native_type_replace_type(type);
  if (type->byref)
    return &val->data.p;
  ...
  case MONO_TYPE_VALUETYPE:
    if (m_class_is_enumtype(type->data.klass))
      return stackval_to_data_addr(
          mono_class_enum_basetype_internal(type->data.klass), val);
    else
      return val->data.vt;
...
interp_throw(ThreadContext *context, MonoException *ex, InterpFrame *frame,
             const guint16 *ip, gboolean rethrow) {
  ERROR_DECL(error);
  MonoLMFExt ext;
  ...
  if (mono_object_isinst_checked((MonoObject *)ex,
...
/* Don't throw exception if thread is in GC Safe mode. Should only happen in
...
static MonoObject *ves_array_create(MonoDomain *domain, MonoClass *klass,
                                    int param_count, stackval *values,
                                    MonoError *error) {
  uintptr_t *lengths;
  ...
  return (MonoObject *)mono_array_new_full_checked(domain, klass, lengths,
                                                   lower_bounds, error);
}

static gint32 ves_array_calculate_index(MonoArray *ao, stackval *sp,
                                        gboolean safe) {
...
static MonoException *ves_array_get(InterpFrame *frame, stackval *sp,
                                    stackval *retval, MonoMethodSignature *sig,
                                    gboolean safe) {
...
static MonoException *ves_array_element_address(InterpFrame *frame,
                                                MonoClass *required_type,
                                                MonoArray *ao, stackval *sp,
  ...
  if (needs_typecheck &&
      !mono_class_is_assignable_from_internal(
          m_class_get_element_class(mono_object_class((MonoObject *)ao)),
          required_type))
...
static InterpMethodArguments *build_args_from_sig(MonoMethodSignature *sig,
  ...
  for (int i = 0; i < sig->param_count; i++) {
    guint32 ptype =
        sig->params[i]->byref ? MONO_TYPE_PTR : sig->params[i]->type;
    switch (ptype) {
    case MONO_TYPE_BOOLEAN:
    case MONO_TYPE_CHAR:
    case MONO_TYPE_I1:
...
#if DEBUG_INTERP
      g_print("build_args_from_sig: margs->iargs [%d/%d]: 0x%016" PRIx64
              ", hi=0x%08x lo=0x%08x (frame @ %d)\n",
              int_i - 1, int_i, *((guint64 *)&margs->iargs[int_i - 1]),
              sarg->data.pair.hi, sarg->data.pair.lo, i);
#endif
      int_i++;
      break;
    }
...
static void interp_frame_arg_to_data(MonoInterpFrameHandle frame,
                                     MonoMethodSignature *sig, int index,
                                     gpointer data) {
  ...
  if (index == -1)
    ...
    stackval_to_data(sig->params[index], &iframe->stack_args[index], data,
                     sig->pinvoke);
}

static void interp_data_to_frame_arg(MonoInterpFrameHandle frame,
                                     MonoMethodSignature *sig, int index,
                                     gconstpointer data) {
  InterpFrame *iframe = (InterpFrame *)frame;
  ...
  if (index == -1)
    ...
    stackval_from_data(sig->params[index - sig->hasthis],
                       &iframe->stack_args[index], data, sig->pinvoke);
}

static gpointer interp_frame_arg_to_storage(MonoInterpFrameHandle frame,
                                            MonoMethodSignature *sig,
                                            int index) {
  InterpFrame *iframe = (InterpFrame *)frame;
  ...
  if (index == -1)
    ...
    return stackval_to_data_addr(sig->params[index],
                                 &iframe->stack_args[index]);
}

static void interp_frame_arg_set_storage(MonoInterpFrameHandle frame,
                                         MonoMethodSignature *sig, int index,
                                         gpointer storage) {
  InterpFrame *iframe = (InterpFrame *)frame;
...
static MonoPIFunc get_interp_to_native_trampoline(void) {
  static MonoPIFunc trampoline = NULL;

  if (!trampoline) {
    if (mono_ee_features.use_aot_trampolines) {
      trampoline =
          (MonoPIFunc)mono_aot_get_trampoline("interp_to_native_trampoline");
    } else {
      MonoTrampInfo *info;
...
static void interp_to_native_trampoline(gpointer addr, gpointer ccontext) {
...
ves_pinvoke_method(MonoMethodSignature *sig, MonoFuncV addr,
                   ThreadContext *context, InterpFrame *parent_frame,
                   stackval *retval, gboolean save_last_error, stackval *sp) {
 ...
 */
static void interp_init_delegate(MonoDelegate *del, MonoError *error) {
  MonoMethod *method;
  ...
  if (del->target && method && method->flags & METHOD_ATTRIBUTE_VIRTUAL &&
    ...
    del->interp_method = get_virtual_method((InterpMethod *)del->interp_method,
                                            del->target->vtable);

  method = ((InterpMethod *)del->interp_method)->method;
  if (method && m_class_get_parent(method->klass) ==
                    mono_defaults.multicastdelegate_class) {
    const char *name = method->name;
    if (*name == 'I' && (strcmp(name, "Invoke") == 0)) {
      /*
       * When invoking the delegate interp_method is executed directly. If it's
       * an invoke make sure we replace it with the appropriate delegate invoke
       * wrapper.
       *
       * FIXME We should do this later, when we also know the delegate on which
       * the target method is called.
       */
      del->interp_method = mono_interp_get_imethod(
          del->object.vtable->domain,
          mono_marshal_get_delegate_invoke(method, NULL), error);
      mono_error_assert_ok(error);
    }
  }
  ...
  if (!((InterpMethod *)del->interp_method)->transformed &&
    ...
    mono_interp_transform_method((InterpMethod *)del->interp_method,
                                 get_context(), error);
    return_if_nok(error);
  }
}

static void interp_delegate_ctor(MonoObjectHandle this_obj,
  ...
  if (!(imethod->method->flags & METHOD_ATTRIBUTE_STATIC)) {
    ...
    if (mono_method_signature_internal(imethod->method)->param_count ==
            mono_method_signature_internal(invoke)->param_count &&
  ...
  gpointer entry = mini_get_interp_callbacks()->create_method_pointer(
      imethod->method, FALSE, error);
  return_if_nok(error);

  MONO_HANDLE_SETVAL(MONO_HANDLE_CAST(MonoDelegate, this_obj), interp_method,
                     gpointer, imethod);
...
ves_imethod(InterpFrame *frame, MonoMethod *method, MonoMethodSignature *sig,
...
static char *dump_stack(stackval *stack, stackval *sp) {
  ...
  while (s < sp) {
    g_string_append_printf(str, "[%p (%" PRId64 ")] ", s->data.l,
                           (gint64)s->data.l);
    ++s;
  }
  return g_string_free(str, FALSE);
...
static void dump_stackval(GString *str, stackval *s, MonoType *type) {
  switch (type->type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
  case MONO_TYPE_I2:
  ...
  default: {
    ...
    g_string_append_printf(str, "[{%s} %" PRId64 "/0x%0" PRIx64 "] ", res->str,
                           (gint64)s->data.l, (guint64)s->data.l);
...
static char *dump_retval(InterpFrame *inv) {
  GString *str = g_string_new("");
...
static char *dump_args(InterpFrame *inv) {
  ...
  MonoMethodSignature *signature =
      mono_method_signature_internal(inv->imethod->method);
  ...
  for (i = 0; i < signature->param_count; ++i)
    dump_stackval(str, inv->stack_args + (!!signature->hasthis) + i,
                  signature->params[i]);
...
#define CHECK_SUB_OVERFLOW(a, b)                                               \
  (gint32)(b)<0 ? (gint32)(G_MAXINT32) + (gint32)(b) < (gint32)(a) ? -1 : 0    \
                : (gint32)(G_MININT32) + (gint32)(b)>(gint32)(a)               \
      ? +1                                                                     \
      : 0
...
#define CHECK_SUB_OVERFLOW_UN(a, b) (guint32)(a) < (guint32)(b) ? -1 : 0
...
#define CHECK_SUB_OVERFLOW64(a, b)                                             \
  (gint64)(b)<0 ? (gint64)(G_MAXINT64) + (gint64)(b) < (gint64)(a) ? -1 : 0    \
                : (gint64)(G_MININT64) + (gint64)(b)>(gint64)(a)               \
      ? +1                                                                     \
      : 0
...
#define CHECK_SUB_OVERFLOW64_UN(a, b) (guint64)(a) < (guint64)(b) ? -1 : 0
...
#define CHECK_MUL_OVERFLOW(a, b)                                               \
  ((gint32)(a) == 0) || ((gint32)(b) == 0)                                     \
      ? 0                                                                      \
      : (((gint32)(a) > 0) && ((gint32)(b) == -1))                             \
            ? FALSE                                                            \
            : (((gint32)(a) < 0) && ((gint32)(b) == -1))                       \
                  ? (a == G_MININT32)                                          \
                  : (((gint32)(a) > 0) && ((gint32)(b) > 0))                   \
...
#define CHECK_MUL_OVERFLOW_UN(a, b)                                            \
  ((guint32)(a) == 0) || ((guint32)(b) == 0)                                   \
      ? 0                                                                      \
      : (guint32)(b) > ((G_MAXUINT32) / (guint32)(a))

#define CHECK_MUL_OVERFLOW64(a, b)                                             \
  ((gint64)(a) == 0) || ((gint64)(b) == 0)                                     \
      ? 0                                                                      \
      : (((gint64)(a) > 0) && ((gint64)(b) == -1))                             \
            ? FALSE                                                            \
            : (((gint64)(a) < 0) && ((gint64)(b) == -1))                       \
                  ? (a == G_MININT64)                                          \
                  : (((gint64)(a) > 0) && ((gint64)(b) > 0))                   \
...
interp_runtime_invoke(MonoMethod *method, void *obj, void **params,
  ...
  if (method->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL)
    target_method =
        mono_marshal_get_native_wrapper(target_method, FALSE, FALSE);
  MonoMethod *invoke_wrapper =
      mono_marshal_get_runtime_invoke_full(target_method, FALSE, TRUE);
  ...
  InterpMethod *imethod =
      mono_interp_get_imethod(domain, invoke_wrapper, error);
  mono_error_assert_ok(error);
  ...
  if (context->has_resume_state) {
    ...
    MonoException *thrown_exc =
        (MonoException *)mono_gchandle_get_target_internal(
...
// Do not inline in case order of frame addresses matters.
static MONO_NEVER_INLINE void interp_entry(InterpEntryData *data) {
  InterpMethod *rmethod;
  ThreadContext *context;
  stackval result;
  stackval *args;
...
static stackval *do_icall(MonoMethodSignature *sig, int op, stackval *sp,
  ...
  case MINT_ICALL_PPPP_P: {
    ...
    sp -= 3;
    sp[-1].data.p =
        func(sp[-1].data.p, sp[0].data.p, sp[1].data.p, sp[2].data.p);
    break;
  }
  ...
  case MINT_ICALL_PPPPPP_P: {
    ...
    sp -= 5;
    sp[-1].data.p = func(sp[-1].data.p, sp[0].data.p, sp[1].data.p,
                         sp[2].data.p, sp[3].data.p, sp[4].data.p);
    break;
  }
  default:
    g_assert_not_reached();
...
#pragma optimize("", off)
#endif
// Do not inline in case order of frame addresses matters, and maybe other
// reasons.
static MONO_NO_OPTIMIZATION MONO_NEVER_INLINE stackval *
do_icall_wrapper(InterpFrame *frame, MonoMethodSignature *sig, int op,
                 stackval *sp, gpointer ptr, gboolean save_last_error) {
  MonoLMFExt ext;
...
static void jit_call_cb(gpointer arg) {
...
static MONO_NEVER_INLINE void
do_jit_call(stackval *sp, unsigned char *vt_sp, ThreadContext *context,
            InterpFrame *frame, InterpMethod *rmethod, MonoError *error) {
  MonoMethodSignature *sig;
  MonoFtnDesc ftndesc;
  guint8 res_buf[256];
  MonoType *type;
  ...
  /*
   * Call JITted code through a gsharedvt_out wrapper. These wrappers receive
   * every argument by ref and return a return value using an explicit return
   * value argument.
   */
...
static MONO_NEVER_INLINE void do_debugger_tramp(void (*tramp)(void),
                                                InterpFrame *frame) {
  MonoLMFExt ext;
  interp_push_lmf(&ext, frame);
  tramp();
...
static guchar *copy_varargs_vtstack(MonoMethodSignature *csig, stackval *sp,
                                    guchar *vt_sp_start) {
 ...
 * void (<optional this_arg>, <optional retval pointer>, <arg1>, ..., <argn>,
 * <method ptr>) They pack up their arguments into an InterpEntryData structure
 * and call interp_entry (). It would be possible for the wrappers to pack up
 * the arguments etc, but that would make them bigger, and there are more
 * wrappers then these functions. this/static * ret/void * 16 arguments -> 64
 * functions.
 */
...
#define ARGLIST3                                                               \
  gpointer arg1, gpointer arg2, gpointer arg3, InterpMethod *rmethod
#define ARGLIST4                                                               \
  gpointer arg1, gpointer arg2, gpointer arg3, gpointer arg4,                  \
      InterpMethod *rmethod
#define ARGLIST5                                                               \
  gpointer arg1, gpointer arg2, gpointer arg3, gpointer arg4, gpointer arg5,   \
      InterpMethod *rmethod
#define ARGLIST6                                                               \
  gpointer arg1, gpointer arg2, gpointer arg3, gpointer arg4, gpointer arg5,   \
      gpointer arg6, InterpMethod *rmethod
#define ARGLIST7                                                               \
  gpointer arg1, gpointer arg2, gpointer arg3, gpointer arg4, gpointer arg5,   \
      gpointer arg6, gpointer arg7, InterpMethod *rmethod
#define ARGLIST8                                                               \
  gpointer arg1, gpointer arg2, gpointer arg3, gpointer arg4, gpointer arg5,   \
      gpointer arg6, gpointer arg7, gpointer arg8, InterpMethod *rmethod

static void interp_entry_static_0(ARGLIST0) INTERP_ENTRY0(
    NULL, NULL, rmethod) static void interp_entry_static_1(ARGLIST1)
    INTERP_ENTRY1(NULL, NULL, rmethod) static void interp_entry_static_2(
        ARGLIST2) INTERP_ENTRY2(NULL, NULL,
                                rmethod) static void interp_entry_static_3(ARGLIST3)
        INTERP_ENTRY3(NULL, NULL, rmethod) static void interp_entry_static_4(
            ARGLIST4) INTERP_ENTRY4(NULL, NULL,
                                    rmethod) static void interp_entry_static_5(ARGLIST5)
            INTERP_ENTRY5(NULL, NULL,
                          rmethod) static void interp_entry_static_6(ARGLIST6)
                INTERP_ENTRY6(
                    NULL, NULL,
                    rmethod) static void interp_entry_static_7(ARGLIST7)
                    INTERP_ENTRY7(
                        NULL, NULL,
                        rmethod) static void interp_entry_static_8(ARGLIST8)
                        INTERP_ENTRY8(
                            NULL, NULL,
                            rmethod) static void interp_entry_static_ret_0(gpointer
                                                                               res,
                                                                           ARGLIST0)
                            INTERP_ENTRY0(NULL, res, rmethod) static void interp_entry_static_ret_1(
                                gpointer res,
                                ARGLIST1) INTERP_ENTRY1(NULL, res,
                                                        rmethod) static void interp_entry_static_ret_2(gpointer res, ARGLIST2) INTERP_ENTRY2(NULL, res, rmethod) static void interp_entry_static_ret_3(gpointer res, ARGLIST3) INTERP_ENTRY3(NULL, res, rmethod) static void interp_entry_static_ret_4(gpointer
                                                                                                                                                                                                                                                                                                           res,
                                                                                                                                                                                                                                                                                                       ARGLIST4) INTERP_ENTRY4(NULL, res, rmethod) static void interp_entry_static_ret_5(gpointer res, ARGLIST5) INTERP_ENTRY5(NULL,
                                                                                                                                                                                                                                                                                                                                                                                                                               res,
                                                                                                                                                                                                                                                                                                                                                                                                                               rmethod) static void interp_entry_static_ret_6(gpointer
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  res,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                              ARGLIST6) INTERP_ENTRY6(NULL, res, rmethod) static void interp_entry_static_ret_7(gpointer
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    res,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ARGLIST7) INTERP_ENTRY7(NULL, res, rmethod) static void interp_entry_static_ret_8(gpointer
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      res,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  ARGLIST8) INTERP_ENTRY8(NULL,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          res,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          rmethod) static void interp_entry_instance_0(gpointer
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           this_arg,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       ARGLIST0) INTERP_ENTRY0(this_arg,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               NULL,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               rmethod) static void interp_entry_instance_1(gpointer
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                this_arg,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            ARGLIST1) INTERP_ENTRY1(this_arg, NULL, rmethod) static void interp_entry_instance_2(gpointer this_arg, ARGLIST2) INTERP_ENTRY2(this_arg, NULL, rmethod) static void interp_entry_instance_3(gpointer this_arg, ARGLIST3) INTERP_ENTRY3(this_arg, NULL, rmethod) static void interp_entry_instance_4(gpointer this_arg, ARGLIST4) INTERP_ENTRY4(this_arg,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            NULL,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            rmethod) static void interp_entry_instance_5(gpointer this_arg, ARGLIST5) INTERP_ENTRY5(this_arg,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    NULL,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    rmethod) static void interp_entry_instance_6(gpointer this_arg,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 ARGLIST6)
                                INTERP_ENTRY6(this_arg, NULL, rmethod) static void interp_entry_instance_7(
                                    gpointer this_arg, ARGLIST7) INTERP_ENTRY7(this_arg,
                                                                               NULL,
...
#define INTERP_ENTRY_FUNCLIST(type)                                            \
  (gpointer) interp_entry_##type##_0, (gpointer)interp_entry_##type##_1,       \
      ...
      (gpointer)interp_entry_##type##_8
                                                                            ...
                                                                            static gpointer
    entry_funcs_static[MAX_INTERP_ENTRY_ARGS + 1] = {
        INTERP_ENTRY_FUNCLIST(static)};
static gpointer entry_funcs_static_ret[MAX_INTERP_ENTRY_ARGS + 1] = {
    INTERP_ENTRY_FUNCLIST(static_ret)};
static gpointer entry_funcs_instance[MAX_INTERP_ENTRY_ARGS + 1] = {
    INTERP_ENTRY_FUNCLIST(instance)};
static gpointer entry_funcs_instance_ret[MAX_INTERP_ENTRY_ARGS + 1] = {
    INTERP_ENTRY_FUNCLIST(instance_ret)};

/* General version for methods with more than MAX_INTERP_ENTRY_ARGS arguments */
static void interp_entry_general(gpointer this_arg, gpointer res,
                                 gpointer *args, gpointer rmethod) {
...
#define alloc_storage_for_stackval(s, t, p)                                    \
  do {                                                                         \
    if ((t)->type == MONO_TYPE_GENERICINST && !MONO_TYPE_IS_REFERENCE(t)) {    \
      (s)->data.vt = alloca(                                                   \
          mono_class_value_size(mono_class_from_mono_type_internal(t), NULL)); \
...
// Do not inline in case order of frame addresses matters.
static MONO_NEVER_INLINE void
interp_entry_from_trampoline(gpointer ccontext_untyped,
...
static void interp_entry_from_trampoline(gpointer ccontext_untyped,
                                         gpointer rmethod_untyped) {
  g_assert_not_reached();
}
...
static InterpMethod *lookup_method_pointer(gpointer addr) {
  ...
  if (info->interp_method_pointer_hash)
    res = (InterpMethod *)g_hash_table_lookup(info->interp_method_pointer_hash,
                                              addr);
...
static void interp_no_native_to_managed(void) {
  g_error("interpreter: native-to-managed transition not available on this "
          "platform");
}
#endif

static void no_llvmonly_interp_method_pointer(void) { g_assert_not_reached(); }
 ...
 */
static MonoFtnDesc *interp_create_method_pointer_llvmonly(MonoMethod *method,
                                                          gboolean unbox,
                                                          MonoError *error) {
  MonoDomain *domain = mono_domain_get();
  ...
  addr = mini_llvmonly_create_ftndesc(mono_domain_get(), entry_wrapper,
                                      entry_ftndesc);
 ...
 */
static gpointer interp_create_method_pointer(MonoMethod *method,
                                             gboolean compile,
                                             MonoError *error) {
#ifndef MONO_ARCH_HAVE_INTERP_NATIVE_TO_MANAGED
  if (mono_llvm_only)
  ...
  if (entry_wrapper) {
    ...
    mono_error_assertf_ok(error, "couldn't compile wrapper \"%s\" for \"%s\"",
                          mono_method_get_name_full(wrapper, TRUE, TRUE,
                                                    MONO_TYPE_NAME_FORMAT_IL),
                          mono_method_get_name_full(method, TRUE, TRUE,
                                                    MONO_TYPE_NAME_FORMAT_IL));
#else
  ...
  g_assert(entry_func);
  /* This is the argument passed to the interp_in wrapper by the static rgctx
   * trampoline */
  ...
  /*
   * The wrapper is called by compiled code, which doesn't pass the extra
...
static void interp_free_method(MonoDomain *domain, MonoMethod *method) {
  MonoJitDomainInfo *info = domain_jit_info(domain);
...
#define DUMP_INSTR()                                                           \
  if (tracing > 1) {                                                           \
    ...
    char *disasm = mono_interp_dis_mintop((gint32)(ip - frame->imethod->code), \
                                          TRUE, ip + 1, *ip);                  \
    g_print("(%p) %s -> %s\t%d:%s\n", mono_thread_internal_current(), mn,      \
            disasm, vt_sp - vtalloc, ins);                                     \
    g_free(mn);                                                                \
    g_free(ins);                                                               \
    g_free(disasm);                                                            \
  }
...
static MonoObject *mono_interp_new(MonoDomain *domain, MonoClass *klass) {
  ERROR_DECL(error);
...
static void mono_interp_load_remote_field(InterpMethod *imethod, MonoObject *o,
                                          const guint16 *ip, stackval *sp) {
  ...
  if (mono_object_is_transparent_proxy(o)) {
    MonoClass *const klass =
        ((MonoTransparentProxy *)o)->remote_class->proxy_class;
    ERROR_DECL(error);
...
static guchar * // Return new vt_sp instead of take-address.
mono_interp_load_remote_field_vt(InterpMethod *imethod, MonoObject *o,
                                 const guint16 *ip, stackval *sp,
                                 guchar *vt_sp) {
  g_assert(o); // Caller checks and throws exception properly.
...
static gboolean mono_interp_isinst(MonoObject *object, MonoClass *klass) {
  ERROR_DECL(error);
  gboolean isinst;
...
// Do not inline in case order of frame addresses matters.
static MONO_NEVER_INLINE void mono_interp_calli_nat_dynamic_pinvoke(
    // Parameters are sorted by name.
    guchar *code, ThreadContext *context, MonoMethodSignature *csignature,
    MonoError *error, InterpFrame *parent_frame, stackval *retval,
    stackval *sp) {
  InterpFrame frame = {parent_frame, NULL, sp, retval};
  ...
  MonoMarshalSpec **mspecs =
      g_newa0(MonoMarshalSpec *, csignature->param_count + 1);

  MonoMethodPInvoke iinfo;
  memset(&iinfo, 0, sizeof(iinfo));

  MonoMethod *m = mono_marshal_get_native_func_wrapper(
      m_class_get_image(imethod->method->klass), csignature, &iinfo, mspecs,
      code);
...
static void mono_interp_enum_hasflag(stackval *sp, MonoClass *klass) {
...
static int mono_interp_box_nullable(InterpFrame *frame, const guint16 *ip,
                                    stackval *sp, MonoError *error) {
...
static int mono_interp_box_vt(InterpFrame *frame, const guint16 *ip,
                              stackval *sp) {
  InterpMethod *const imethod = frame->imethod;
...
static void mono_interp_box(InterpFrame *frame, const guint16 *ip,
                            stackval *sp) {
  MonoObject *o; // See the comment about GC safety.
  ...
  stackval_to_data(m_class_get_byval_arg(vtable->klass), &sp[-1 - offset],
                   mono_object_get_data(o), FALSE);

  sp[-1 - offset].data.p = o;
}

static int mono_interp_store_remote_field_vt(InterpFrame *frame,
                                             const guint16 *ip, stackval *sp,
                                             MonoError *error) {
  InterpMethod *const imethod = frame->imethod;
...
static void g_warning_d(const char *format, int d) { g_warning(format, d); }

static void g_warning_ds(const char *format, int d, const char *s) {
  g_warning(format, d, s);
}

#if !USE_COMPUTED_GOTO
static void g_error_xsx(const char *format, int x1, const char *s, int x2) {
  g_error(format, x1, s, x2);
}
#endif

static MONO_ALWAYS_INLINE gboolean method_entry(ThreadContext *context,
                                                InterpFrame *frame,
#if DEBUG_INTERP
                                                int *out_tracing,
#endif
...
#define LOAD_INTERP_STATE(frame)                                               \
  do {                                                                         \
    ...
    locals = (unsigned char *)frame->stack + frame->imethod->stack_size +      \
             frame->imethod->vt_stack_size;                                    \
    frame->state.ip = NULL;                                                    \
  } while (0)
...
#define INIT_INTERP_STATE(frame, _clause_args)                                 \
  do {                                                                         \
    ip =                                                                       \
        _clause_args ? (_clause_args)->start_with_ip : (frame)->imethod->code; \
    sp = (frame)->stack;                                                       \
 ...
 * The ERROR argument is used to avoid declaring an error object for every
 * interp frame, its not used to return error information. FRAME is only valid
 * until the next call to alloc_frame ().
 */
static MONO_NEVER_INLINE void interp_exec_method(InterpFrame *frame,
                                                 ThreadContext *context,
                                                 FrameClauseArgs *clause_args,
                                                 MonoError *error) {
...
#include "mintops.def"
  ...
  if (clause_args && clause_args->base_frame)
    memcpy(frame->stack, clause_args->base_frame->stack,
           frame->imethod->alloca_size);
  ...
  mini_tiered_inc(frame->imethod->domain, frame->imethod->method,
                  &frame->imethod->tiered_counter, 0);
#endif
  // g_print ("(%p) Call %s\n", mono_thread_internal_current (),
  // mono_method_get_full_name (frame->imethod->method));
    ...
    ++sp;                                                                      \
      ...
      MINT_IN_CASE(MINT_JMP) {
        ...
        InterpMethod *new_method =
            (InterpMethod *)frame->imethod->data_items[ip[1]];

        if (frame->imethod->prof_flags &
            MONO_PROFILER_CALL_INSTRUMENTATION_TAIL_CALL)
          MONO_PROFILER_RAISE(method_tail_call,
                              (frame->imethod->method, new_method->method));
        ...
        ip += 2;
        const gboolean realloc_frame =
            new_method->alloca_size > frame->imethod->alloca_size;
        frame->imethod = new_method;
        /*
         * We allocate the stack frame from scratch and store the arguments in
         * the locals again since it's possible for the caller stack frame to be
         * smaller than the callee stack frame (at the interp level)
         */
        if (realloc_frame) {
          alloc_stack_data(context, frame, frame->imethod->alloca_size);
      ...
      MINT_IN_CASE(MINT_CALL_DELEGATE) {
        MonoMethodSignature *csignature =
            (MonoMethodSignature *)frame->imethod->data_items[ip[1]];
        ...
        if (!del_imethod) {
          if (is_multicast) {
            error_init_reuse(error);
            MonoMethod *invoke =
                mono_get_delegate_invoke_internal(del->object.vtable->klass);
            del_imethod = mono_interp_get_imethod(
                del->object.vtable->domain,
                mono_marshal_get_delegate_invoke(invoke, del), error);
            ...
            error_init_reuse(error);
            g_assert(del->method);
            del_imethod = mono_interp_get_imethod(del->object.vtable->domain,
                                                  del->method, error);
            ...
            if (del_imethod->method->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL) {
              ...
              del_imethod = mono_interp_get_imethod(
                  ...
                  mono_marshal_get_native_wrapper(del_imethod->method, FALSE,
                                                  FALSE),
                  error);
              mono_error_assert_ok(error);
              del->interp_invoke_impl = del_imethod;
            } else if (del_imethod->method->flags & METHOD_ATTRIBUTE_VIRTUAL &&
                       !del->target) {
              // 'this' is passed dynamically, we need to recompute the target
              // method with each call
              del_imethod = get_virtual_method(del_imethod,
                                               sp[-param_count].data.o->vtable);
            } else {
              del->interp_invoke_impl = del_imethod;
            }
          }
        ...
        if (!is_multicast) {
          if (cmethod->param_count == param_count + 1) {
            // Target method is static but the delegate has a target object. We
            // handle this separately from the case below, because, for these
            // calls, the instance is allowed to be null.
      ...
      MINT_IN_CASE(MINT_CALLI) {
        ...
        if (cmethod->method->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL) {
          cmethod = mono_interp_get_imethod(
              frame->imethod->domain,
              mono_marshal_get_native_wrapper(cmethod->method, FALSE, FALSE),
              error);
          mono_interp_error_cleanup(error); /* FIXME: don't swallow the error */
        }
      ...
      MINT_IN_CASE(MINT_CALLI_NAT) {
        ...
        if (frame->imethod->method->dynamic && csignature->pinvoke) {
          mono_interp_calli_nat_dynamic_pinvoke(code, context, csignature,
                                                error, frame, retval, sp);
        } else {
          const gboolean save_last_error = ip[-3 + 2];
          ves_pinvoke_method(csignature, (MonoFuncV)code, context, frame,
                             retval, save_last_error, sp);
        }
      ...
      MINT_IN_CASE(MINT_VCALLVIRT_FAST) {
        ...
        if (G_UNLIKELY(code_type == IMETHOD_CODE_UNKNOWN)) {
          MonoMethodSignature *sig =
              mono_method_signature_internal(cmethod->method);
      ...
      MINT_IN_CASE(MINT_CALL_VARARG) {
        ...
        /* Push all vararg arguments from normal sp to vt_sp together with the
         * signature */
        ...
        sp -= cmethod->param_count + cmethod->hasthis + csig->param_count -
              csig->sentinelpos;
      ...
      call:;
        /*
         * Make a non-recursive call by loading the new interpreter state based
         * on child frame, and going back to the main loop.
         */
      ...
      MINT_IN_CASE(MINT_CALLRUN) {
        ...
        MonoMethod *target_method =
            (MonoMethod *)frame->imethod->data_items[ip[1]];
        MonoMethodSignature *sig =
            (MonoMethodSignature *)frame->imethod->data_items[ip[2]];
      ...
      *frame->retval = *sp;
      ...
      if (sp > frame->stack)
        g_warning_ds("ret.void: more values on stack: %d %s", sp - frame->stack,
...
#define CONDBR_S(cond)                                                         \
  ...
  if (cond) {                                                                  \
    ...
    ip += 2;
#define BRELOP_S(datamem, op) CONDBR_S(sp[0].data.datamem op sp[1].data.datamem)
...
#define BRELOP(datamem, op) CONDBR(sp[0].data.datamem op sp[1].data.datamem)
      ...
      CONDBR_S(!isunordered(sp[0].data.f_r4, sp[1].data.f_r4) &&
               sp[0].data.f_r4 == sp[1].data.f_r4)
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_BEQ_R8_S)
      CONDBR_S(!mono_isunordered(sp[0].data.f, sp[1].data.f) &&
      ...
      CONDBR(!isunordered(sp[0].data.f_r4, sp[1].data.f_r4) &&
             sp[0].data.f_r4 == sp[1].data.f_r4)
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_BEQ_R8)
      CONDBR(!mono_isunordered(sp[0].data.f, sp[1].data.f) &&
             sp[0].data.f == sp[1].data.f)
      MINT_IN_BREAK;
    ...
    ip += 3;
      ...
      MINT_IN_CASE(MINT_LDIND_I8) {
        ...
        if ((gsize)sp[-1 - offset].data.p % SIZEOF_VOID_P)
          memcpy(&sp[-1 - offset].data.l, sp[-1 - offset].data.p,
                 sizeof(gint64));
        else
#endif
...
#define SHIFTOP(datamem, op)                                                   \
  ...
  ++ip;
      ...
      MINT_IN_CASE(MINT_CPOBJ) {
        ...
        stackval_from_data(m_class_get_byval_arg(c), (stackval *)sp[-2].data.p,
                           sp[-1].data.p, FALSE);
        ip += 2;
        sp -= 2;
        MINT_IN_BREAK;
      ...
      MINT_IN_CASE(MINT_NEWOBJ_ARRAY) {
        ...
        sp -= param_count;
        sp->data.o = ves_array_create(frame->imethod->domain, newobj_class,
                                      param_count, sp, error);
        if (!is_ok(error))
          goto throw_error_label;
      ...
      MINT_IN_CASE(MINT_NEWOBJ_FAST) {
        ...
        if (G_UNLIKELY(!o)) {
          mono_error_set_out_of_memory(
              error, "Could not allocate %i bytes",
              m_class_get_instance_size(vtable->klass));
          goto throw_error_label;
        }

        // Store o next to and before the parameters on the stack so GC will see
        // it, and where it is needed when the call returns.
        sp[0].data.o = o;
        sp[1].data.o = o;
        ip += 4;
        if (is_inlined) {
    ...
    call_newobj:
      ...
      MINT_IN_CASE(MINT_NEWOBJ) {
        ...
        MonoMethodSignature *const csig =
            mono_method_signature_internal(cmethod->method);
        ...
        /*if (profiling_classes) {
                guint count = GPOINTER_TO_UINT (g_hash_table_lookup
        (profiling_classes, newobj_class)); count++; g_hash_table_insert
        (profiling_classes, newobj_class, GUINT_TO_POINTER (count));
        }*/

        /*
         * First arg is the object.
         * a constructor returns void, but we need to return the object we
         * created
         */

        g_assert(!m_class_is_valuetype(newobj_class));

        // This branch could be avoided. Move it to transform, and use a new
        // opcode NEWOBJ_STRING.
        ...
        MonoVTable *vtable =
            mono_class_vtable_checked(domain, newobj_class, error);
        ...
        if (mono_object_is_transparent_proxy(o)) {
          MonoMethod *remoting_invoke_method =
              mono_marshal_get_remoting_invoke_with_check(cmethod->method,
                                                          error);
          mono_error_assert_ok(error);
          cmethod =
              mono_interp_get_imethod(domain, remoting_invoke_method, error);
          mono_error_assert_ok(error);
        }
#endif
        goto call_newobj;
      ...
      MINT_IN_CASE(MINT_INTRINS_BYREFERENCE_CTOR) {
        ...
        InterpMethod *cmethod =
            (InterpMethod *)frame->imethod->data_items[token];
      ...
      MINT_IN_CASE(MINT_INTRINS_RUNTIMEHELPERS_OBJECT_HAS_COMPONENT_SIZE) {
        ...
        sp[-1].data.i =
            (obj->vtable->flags & MONO_VT_FLAG_ARRAY_OR_STRING) != 0;
        ++ip;
        MINT_IN_BREAK;
      }
      ...
      MINT_IN_CASE(MINT_ISINST_INTERFACE) {
        ...
        if (o) {
          MonoClass *const c = (MonoClass *)frame->imethod->data_items[ip[1]];
          gboolean isinst;
          if (MONO_VTABLE_IMPLEMENTS_INTERFACE(o->vtable,
            ...
            /* slow path */
            isinst =
                mono_interp_isinst(o, c); // FIXME: do not swallow the error
          } else {
            isinst = FALSE;
          }
      ...
      MINT_IN_CASE(MINT_UNBOX) {
        ...
        if (!(m_class_get_rank(o->vtable->klass) == 0 &&
              m_class_get_element_class(o->vtable->klass) ==
...
#define LDFLD(datamem, fieldtype) LDFLD_UNALIGNED(datamem, fieldtype, FALSE)
      ...
      MINT_IN_CASE(MINT_LDFLD_I1) LDFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_U1) LDFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_I2) LDFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_U2) LDFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_I4) LDFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_I8) LDFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_R4) LDFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_R8) LDFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_O) LDFLD(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_P) LDFLD(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_I8_UNALIGNED) LDFLD_UNALIGNED(l, gint64, TRUE);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDFLD_R8_UNALIGNED) LDFLD_UNALIGNED(f, double, TRUE);
      MINT_IN_BREAK;
      ...
      MINT_IN_CASE(MINT_LDRMFLD_VT) {
        ...
        NULL_CHECK(o);
        vt_sp =
            mono_interp_load_remote_field_vt(frame->imethod, o, ip, sp, vt_sp);
        ip += 2;
        MINT_IN_BREAK;
      }
...
#define LDARGFLD(datamem, fieldtype)                                           \
  do {                                                                         \
    ...
    ip += 3;                                                                   \
      ...
      MINT_IN_CASE(MINT_LDARGFLD_I1) LDARGFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARGFLD_U1) LDARGFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARGFLD_I2) LDARGFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARGFLD_U2) LDARGFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARGFLD_I4) LDARGFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARGFLD_I8) LDARGFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARGFLD_R4) LDARGFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARGFLD_R8) LDARGFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARGFLD_O) LDARGFLD(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARGFLD_P) LDARGFLD(p, gpointer);
      MINT_IN_BREAK;
...
#define LDLOCFLD(datamem, fieldtype)                                           \
  do {                                                                         \
    ...
    ip += 3;                                                                   \
      ...
      MINT_IN_CASE(MINT_LDLOCFLD_I1) LDLOCFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOCFLD_U1) LDLOCFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOCFLD_I2) LDLOCFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOCFLD_U2) LDLOCFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOCFLD_I4) LDLOCFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOCFLD_I8) LDLOCFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOCFLD_R4) LDLOCFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOCFLD_R8) LDLOCFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOCFLD_O) LDLOCFLD(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOCFLD_P) LDLOCFLD(p, gpointer);
      MINT_IN_BREAK;
...
#define STFLD(datamem, fieldtype) STFLD_UNALIGNED(datamem, fieldtype, FALSE)
      ...
      MINT_IN_CASE(MINT_STFLD_I1) STFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STFLD_U1) STFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STFLD_I2) STFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STFLD_U2) STFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STFLD_I4) STFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STFLD_I8) STFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STFLD_R4) STFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STFLD_R8) STFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STFLD_P) STFLD(p, gpointer);
      MINT_IN_BREAK;
      ...
      MINT_IN_CASE(MINT_STFLD_I8_UNALIGNED) STFLD_UNALIGNED(l, gint64, TRUE);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STFLD_R8_UNALIGNED) STFLD_UNALIGNED(f, double, TRUE);
      MINT_IN_BREAK;
...
#endif
          stackval_to_data(field->type, &sp[-1], (char *)o + field->offset,
                           FALSE);
  ...
  do {                                                                         \
    ...
    *(fieldtype *)((char *)o + ip[2]) = sp[0].data.datamem;                    \
    ip += 3;                                                                   \
      ...
      MINT_IN_CASE(MINT_STARGFLD_I1) STARGFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARGFLD_U1) STARGFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARGFLD_I2) STARGFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARGFLD_U2) STARGFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARGFLD_I4) STARGFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARGFLD_I8) STARGFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARGFLD_R4) STARGFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARGFLD_R8) STARGFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARGFLD_P) STARGFLD(p, gpointer);
      MINT_IN_BREAK;
  ...
  do {                                                                         \
    ...
    *(fieldtype *)((char *)o + ip[2]) = sp[0].data.datamem;                    \
    ip += 3;                                                                   \
      ...
      MINT_IN_CASE(MINT_STLOCFLD_I1) STLOCFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOCFLD_U1) STLOCFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOCFLD_I2) STLOCFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOCFLD_U2) STLOCFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOCFLD_I4) STLOCFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOCFLD_I8) STLOCFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOCFLD_R4) STLOCFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOCFLD_R8) STLOCFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOCFLD_P) STLOCFLD(p, gpointer);
      MINT_IN_BREAK;
...
#define LDSFLD(datamem, fieldtype)                                             \
  {                                                                            \
    ...
    sp++;                                                                      \
      ...
      MINT_IN_CASE(MINT_LDSFLD_I1) LDSFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDSFLD_U1) LDSFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDSFLD_I2) LDSFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDSFLD_U2) LDSFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDSFLD_I4) LDSFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDSFLD_I8) LDSFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDSFLD_R4) LDSFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDSFLD_R8) LDSFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDSFLD_O) LDSFLD(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDSFLD_P) LDSFLD(p, gpointer);
      MINT_IN_BREAK;
...
#define LDTSFLD(datamem, fieldtype)                                            \
  {                                                                            \
    ...
    guint32 offset = READ32(ip + 1);                                           \
    gpointer addr =                                                            \
        ((char *)thread->static_data[offset & 0x3f]) + (offset >> 6);          \
    sp[0].data.datamem = *(fieldtype *)addr;                                   \
    ip += 3;                                                                   \
    ++sp;                                                                      \
  }
      MINT_IN_CASE(MINT_LDTSFLD_I1) LDTSFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDTSFLD_U1) LDTSFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDTSFLD_I2) LDTSFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDTSFLD_U2) LDTSFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDTSFLD_I4) LDTSFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDTSFLD_I8) LDTSFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDTSFLD_R4) LDTSFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDTSFLD_R8) LDTSFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDTSFLD_O) LDTSFLD(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDTSFLD_P) LDTSFLD(p, gpointer);
      MINT_IN_BREAK;
      ...
      MINT_IN_CASE(MINT_LDSSFLD) {
        ...
        MonoClassField *field =
            (MonoClassField *)frame->imethod->data_items[ip[1]];
  ...
  {                                                                            \
    ...
    *(fieldtype *)(frame->imethod->data_items[ip[2]]) = sp[0].data.datamem;    \
    ip += 3;                                                                   \
      ...
      MINT_IN_CASE(MINT_STSFLD_I1) STSFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STSFLD_U1) STSFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STSFLD_I2) STSFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STSFLD_U2) STSFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STSFLD_I4) STSFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STSFLD_I8) STSFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STSFLD_R4) STSFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STSFLD_R8) STSFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STSFLD_P) STSFLD(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STSFLD_O) STSFLD(p, gpointer);
      MINT_IN_BREAK;
  ...
  {                                                                            \
    ...
    *(fieldtype *)addr = sp[0].data.datamem;                                   \
    ip += 3;                                                                   \
      ...
      MINT_IN_CASE(MINT_STTSFLD_I1) STTSFLD(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STTSFLD_U1) STTSFLD(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STTSFLD_I2) STTSFLD(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STTSFLD_U2) STTSFLD(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STTSFLD_I4) STTSFLD(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STTSFLD_I8) STTSFLD(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STTSFLD_R4) STTSFLD(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STTSFLD_R8) STTSFLD(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STTSFLD_P) STTSFLD(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STTSFLD_O) STTSFLD(p, gpointer);
      MINT_IN_BREAK;
      ...
      MINT_IN_CASE(MINT_NEWARR) {
        MonoVTable *vtable = (MonoVTable *)frame->imethod->data_items[ip[1]];
        sp[-1].data.o = (MonoObject *)mono_array_new_specific_checked(
            vtable, sp[-1].data.i, error);
        if (!is_ok(error)) {
          goto throw_error_label;
        }
        ip += 2;
        /*if (profiling_classes) {
                guint count = GPOINTER_TO_UINT (g_hash_table_lookup
        (profiling_classes, o->vtable->klass)); count++; g_hash_table_insert
        (profiling_classes, o->vtable->klass, GUINT_TO_POINTER (count));
        }*/
      ...
      MINT_IN_CASE(MINT_ARRAY_IS_PRIMITIVE) {
        MonoObject *const o = sp[-1].data.o;
        NULL_CHECK(o);
        sp[-1].data.i = m_class_is_primitive(
      ...
      MINT_IN_CASE(MINT_LDELEMA_TC) {
        ...
        MonoException *ex = ves_array_element_address(
            frame, klass, (MonoArray *)o, sp, needs_typecheck);
        if (ex)
          THROW_EX(ex, ip);
        MINT_IN_BREAK;
      }
...
#define LDELEM(datamem, elemtype)                                              \
  do {                                                                         \
    ...
    ip++;                                                                      \
      ...
      MINT_IN_CASE(MINT_LDELEM_I1) LDELEM(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_U1) LDELEM(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_I2) LDELEM(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_U2) LDELEM(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_I4) LDELEM(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_U4) LDELEM(i, guint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_I8) LDELEM(l, guint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_I) LDELEM(nati, mono_i);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_R4) LDELEM(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_R8) LDELEM(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_REF) LDELEM(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDELEM_VT) {
        ...
        int i32 = READ32(ip + 1);
        char *src_addr =
            mono_array_addr_with_size_fast((MonoArray *)o, i32, aindex);
        sp[-1].data.vt = vt_sp;
...
#define STELEM(datamem, elemtype)                                              \
  do {                                                                         \
    ...
    ip++;                                                                      \
      ...
      MINT_IN_CASE(MINT_STELEM_I1) STELEM(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STELEM_U1) STELEM(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STELEM_I2) STELEM(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STELEM_U2) STELEM(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STELEM_I4) STELEM(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STELEM_I8) STELEM(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STELEM_I) STELEM(nati, mono_i);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STELEM_R4) STELEM(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STELEM_R8) STELEM(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STELEM_REF) {
        ...
        if (sp[2].data.o) {
          gboolean isinst = mono_interp_isinst(
              sp[2].data.o, m_class_get_element_class(mono_object_class(o)));
          if (!isinst)
      ...
      if (sp[-1].data.f_r4 < G_MININT16 || sp[-1].data.f_r4 > G_MAXINT16 ||
          isnan(sp[-1].data.f_r4))
        goto overflow_label;
      ...
      if (sp[-1].data.f_r4 < 0 || sp[-1].data.f_r4 > G_MAXINT16 ||
          isnan(sp[-1].data.f_r4))
        goto overflow_label;
      sp[-1].data.i = (gint16)sp[-1].data.f_r4;
      ++ip;
      ...
      if (sp[-1].data.f_r4 < 0 || sp[-1].data.f_r4 > G_MAXUINT16 ||
          isnan(sp[-1].data.f_r4))
        goto overflow_label;
      ...
      if (sp[-1].data.f_r4 < G_MININT8 || sp[-1].data.f_r4 > G_MAXINT8 ||
          isnan(sp[-1].data.f_r4))
        goto overflow_label;
      ...
      if (sp[-1].data.f_r4 < 0 || sp[-1].data.f_r4 > G_MAXINT8 ||
          isnan(sp[-1].data.f_r4))
        goto overflow_label;
      sp[-1].data.i = (gint8)sp[-1].data.f_r4;
      ++ip;
      ...
      if (sp[-1].data.f_r4 < 0 || sp[-1].data.f_r4 > G_MAXUINT8 ||
          isnan(sp[-1].data.f_r4))
        goto overflow_label;
      ...
      *(gpointer *)sp->data.p = frame->imethod->data_items[ip[1]];
      ...
      MINT_IN_CASE(MINT_ENDFINALLY) {
        ...
        if (finally_ips) {
          ...
          if (pending_abort && !finally_ips)
            EXCEPTION_CHECKPOINT;
          // goto main_loop instead of MINT_IN_DISPATCH helps the compiler and
          // therefore conserves stack. This is a slow/rare path and conserving
          // stack is preferred over its performance otherwise.
          goto main_loop;
        }
        ves_abort();
        MINT_IN_BREAK;
      ...
      MINT_IN_CASE(MINT_LEAVE_S_CHECK) {
        ...
        g_assert(sp >= frame->stack);
        sp = frame->stack; /* spec says stack should be empty at endfinally so
                              it should be at the start too */

        frame->ip = ip;

        int opcode = *ip;
        gboolean const check =
            opcode == MINT_LEAVE_CHECK || opcode == MINT_LEAVE_S_CHECK;

        if (check && frame->imethod->method->wrapper_type !=
            ...
            if (clause->flags == MONO_EXCEPTION_CLAUSE_FINALLY) {
              ...
              if (tracing)
                g_print("* Found finally at IL_%04x with exception: %s\n",
                        ...
                        context->has_resume_state ? "yes" : "no");
#endif
            }
          }
        }
      ...
      frame->ip = ip;
      sp = do_icall_wrapper(frame, NULL, *ip, sp,
                            frame->imethod->data_items[ip[1]], FALSE);
      EXCEPTION_CHECKPOINT_GC_UNSAFE;
      CHECK_RESUME_STATE(context);
      ip += 2;
      MINT_IN_BREAK;
      ...
      sp->data.o = mono_interp_new(
          frame->imethod->domain,
          (MonoClass *)frame->imethod
              ->data_items[ip[1]]); // FIXME: do not swallow the error
      ip += 2;
      sp++;
      MINT_IN_BREAK;
      ...
      sp--;
      stackval_from_data(
          mono_method_signature_internal(frame->imethod->method)->ret,
          frame->retval, sp->data.p,
      ...
      if (G_UNLIKELY(ss_enabled)) {
        ...
        /*
         * Make this point to the MINT_SDB_SEQ_POINT instruction which follows
         * this since the address of that instruction is stored as the seq point
         * address.
         */
        frame->ip = ip + 1;
...
#undef RELOP_CAST
      ...
      MINT_IN_CASE(MINT_LDFTN_DYNAMIC) {
        error_init_reuse(error);
        InterpMethod *m = mono_interp_get_imethod(
            mono_domain_get(), (MonoMethod *)sp[-1].data.p, error);
        mono_error_assert_ok(error);
        sp[-1].data.p = m;
        ip++;
        MINT_IN_BREAK;
...
#define LDARGn(datamem, argtype, n)                                            \
  ...
  ++sp;
      ...
      MINT_IN_CASE(MINT_LDARG_I1) LDARG(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARG_U1) LDARG(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARG_I2) LDARG(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARG_U2) LDARG(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARG_I4) LDARG(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARG_I8) LDARG(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARG_R4) LDARG(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARG_R8) LDARG(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARG_O) LDARG(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDARG_P) LDARG(p, gpointer);
      MINT_IN_BREAK;

      MINT_IN_CASE(MINT_LDARG_P0) LDARGn(p, gpointer, 0);
      MINT_IN_BREAK;
...
#define STARG(datamem, argtype)                                                \
  ...
  ip += 2;
      ...
      MINT_IN_CASE(MINT_STARG_I1) STARG(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARG_U1) STARG(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARG_I2) STARG(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARG_U2) STARG(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARG_I4) STARG(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARG_I8) STARG(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARG_R4) STARG(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARG_R8) STARG(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARG_O) STARG(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STARG_P) STARG(p, gpointer);
      MINT_IN_BREAK;
      ...
      MINT_IN_CASE(MINT_PROF_ENTER) {
        ...
        if ((flag & TRACING_FLAG) ||
          ...
          MonoProfilerCallContext *prof_ctx =
              g_new0(MonoProfilerCallContext, 1);
          prof_ctx->interp_frame = frame;
          prof_ctx->method = frame->imethod->method;
          if (flag & TRACING_FLAG)
            mono_trace_enter_method(frame->imethod->method,
                                    frame->imethod->jinfo, prof_ctx);
          if (flag & PROFILING_FLAG)
            MONO_PROFILER_RAISE(method_enter,
                                (frame->imethod->method, prof_ctx));
          g_free(prof_ctx);
        } else if ((flag & PROFILING_FLAG) &&
      ...
      MINT_IN_CASE(MINT_PROF_EXIT_VOID) {
        ...
        if ((flag & TRACING_FLAG) ||
          ...
          if (flag & TRACING_FLAG)
            mono_trace_leave_method(frame->imethod->method,
                                    frame->imethod->jinfo, prof_ctx);
          if (flag & PROFILING_FLAG)
            MONO_PROFILER_RAISE(method_leave,
                                (frame->imethod->method, prof_ctx));
          g_free(prof_ctx);
        } else if ((flag & PROFILING_FLAG) &&
                   MONO_PROFILER_ENABLED(method_enter)) {
...
#define LDLOC(datamem, argtype)                                                \
  ...
  ++sp;
      ...
      MINT_IN_CASE(MINT_LDLOC_I1) LDLOC(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOC_U1) LDLOC(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOC_I2) LDLOC(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOC_U2) LDLOC(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOC_I4) LDLOC(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOC_I8) LDLOC(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOC_R4) LDLOC(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOC_R8) LDLOC(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOC_O) LDLOC(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LDLOC_P) LDLOC(p, gpointer);
      MINT_IN_BREAK;
...
#define STLOC(datamem, argtype)                                                \
  ...
  *(argtype *)(locals + ip[1]) = sp->data.datamem;                             \
  ip += 2;
      ...
      MINT_IN_CASE(MINT_STLOC_I1) STLOC(i, gint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_U1) STLOC(i, guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_I2) STLOC(i, gint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_U2) STLOC(i, guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_I4) STLOC(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_I8) STLOC(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_R4) STLOC(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_R8) STLOC(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_O) STLOC(p, gpointer);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_P) STLOC(p, gpointer);
      MINT_IN_BREAK;
...
#define STLOC_NP(datamem, argtype)                                             \
  *(argtype *)(locals + ip[1]) = sp[-1].data.datamem;                          \
  ip += 2;
      ...
      MINT_IN_CASE(MINT_STLOC_NP_I4) STLOC_NP(i, gint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_NP_I8) STLOC_NP(l, gint64);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_NP_R4) STLOC_NP(f_r4, float);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_NP_R8) STLOC_NP(f, double);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_STLOC_NP_O) STLOC_NP(p, gpointer);
      MINT_IN_BREAK;
...
#define MOVLOC(argtype)                                                        \
  *(argtype *)(locals + ip[2]) = *(argtype *)(locals + ip[1]);                 \
  ip += 3;
      ...
      MINT_IN_CASE(MINT_MOVLOC_1) MOVLOC(guint8);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_MOVLOC_2) MOVLOC(guint16);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_MOVLOC_4) MOVLOC(guint32);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_MOVLOC_8) MOVLOC(guint64);
      MINT_IN_BREAK;
      ...
      MINT_IN_CASE(MINT_RETHROW) {
        ...
        THROW_EX_GENERAL(
            *(MonoException **)(frame_locals(frame) + exvar_offset), ip, TRUE);
        MINT_IN_BREAK;
      }
      ...
      MINT_IN_CASE(MINT_LD_DELEGATE_METHOD_PTR) {
        ...
        if (!del->interp_method) {
          ...
          g_assert(del->method);
          del->interp_method = mono_interp_get_imethod(
              del->object.vtable->domain, del->method, error);
          mono_error_assert_ok(error);
        }
      ...
      MINT_IN_CASE(MINT_LD_DELEGATE_INVOKE_IMPL) {
        ...
        if (!del->interp_invoke_impl) {
          /*
           * First time we are called. Set up the invoke wrapper. We might be
           * able to do this in ctor but we would need to handle
           * AllocDelegateLike_internal separately
           */
          error_init_reuse(error);
          MonoMethod *invoke =
              mono_get_delegate_invoke_internal(del->object.vtable->klass);
          del->interp_invoke_impl = mono_interp_get_imethod(
              del->object.vtable->domain,
              mono_marshal_get_delegate_invoke(invoke, del), error);
          mono_error_assert_ok(error);
        }
...
#define MATH_BINOP(mathfunc)                                                   \
  ...
  ++ip;
      ...
      MINT_IN_CASE(MINT_ABS) MATH_UNOP(fabs);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ASIN) MATH_UNOP(asin);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ASINH) MATH_UNOP(asinh);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ACOS) MATH_UNOP(acos);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ACOSH) MATH_UNOP(acosh);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ATAN) MATH_UNOP(atan);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ATANH) MATH_UNOP(atanh);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_CEILING) MATH_UNOP(ceil);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_COS) MATH_UNOP(cos);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_CBRT) MATH_UNOP(cbrt);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_COSH) MATH_UNOP(cosh);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_EXP) MATH_UNOP(exp);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_FLOOR) MATH_UNOP(floor);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LOG) MATH_UNOP(log);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LOG2) MATH_UNOP(log2);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LOG10) MATH_UNOP(log10);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_SIN) MATH_UNOP(sin);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_SQRT) MATH_UNOP(sqrt);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_SINH) MATH_UNOP(sinh);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_TAN) MATH_UNOP(tan);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_TANH) MATH_UNOP(tanh);
      MINT_IN_BREAK;

      MINT_IN_CASE(MINT_ATAN2) MATH_BINOP(atan2);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_POW) MATH_BINOP(pow);
      MINT_IN_BREAK;
...
#define MATH_BINOPF(mathfunc)                                                  \
  ...
  ++ip;
      MINT_IN_CASE(MINT_ABSF) MATH_UNOPF(fabsf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ASINF) MATH_UNOPF(asinf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ASINHF) MATH_UNOPF(asinhf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ACOSF) MATH_UNOPF(acosf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ACOSHF) MATH_UNOPF(acoshf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ATANF) MATH_UNOPF(atanf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_ATANHF) MATH_UNOPF(atanhf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_CEILINGF) MATH_UNOPF(ceilf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_COSF) MATH_UNOPF(cosf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_CBRTF) MATH_UNOPF(cbrtf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_COSHF) MATH_UNOPF(coshf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_EXPF) MATH_UNOPF(expf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_FLOORF) MATH_UNOPF(floorf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LOGF) MATH_UNOPF(logf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LOG2F) MATH_UNOPF(log2f);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_LOG10F) MATH_UNOPF(log10f);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_SINF) MATH_UNOPF(sinf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_SQRTF) MATH_UNOPF(sqrtf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_SINHF) MATH_UNOPF(sinhf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_TANF) MATH_UNOPF(tanf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_TANHF) MATH_UNOPF(tanhf);
      MINT_IN_BREAK;

      MINT_IN_CASE(MINT_ATAN2F) MATH_BINOPF(atan2f);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_POWF) MATH_BINOPF(powf);
      MINT_IN_BREAK;
      MINT_IN_CASE(MINT_FMAF)
      sp -= 2;
      sp[-1].data.f_r4 =
          fmaf(sp[-1].data.f_r4, sp[0].data.f_r4, sp[1].data.f_r4);
      ip++;
      MINT_IN_BREAK;
    ...
    default:
      g_error_xsx("Unimplemented opcode: %04x %s at 0x%x\n", *ip,
                  mono_interp_opname(*ip), ip - frame->imethod->code);
#endif
    }
  }
...
resume:
  ...
  if (frame == context->handler_frame &&
      (!clause_args || context->handler_ip < clause_args->end_at_ip)) {
    /* Set the current execution state to the resume state in context */

    ip = context->handler_ip;
    /* spec says stack should be empty at endfinally so it should be at the
     * start too */
    sp = frame->stack;
    ...
    // goto main_loop instead of MINT_IN_DISPATCH helps the compiler and
    // therefore conserves stack. This is a slow/rare path and conserving stack
    // is preferred over its performance otherwise.
    goto main_loop;
  }
  // fall through
exit_frame:
  ...
  if (!clause_args && frame->parent && frame->parent->state.ip) {
    ...
    // (frame->imethod->method), mono_method_get_full_name
    // (frame->parent->imethod->method), frame->parent->state.ip);
...
static void interp_parse_options(const char *options) {
  char **args, **ptr;
 ...
 *
 *   Set the state the interpeter will continue to execute from after execution
 * returns to the interpreter.
 */
static void interp_set_resume_state(MonoJitTlsData *jit_tls, MonoObject *ex,
  ...
  /* Ditto */
  if (ei)
    *(MonoObject **)(frame_locals(context->handler_frame) + ei->exvar_offset) =
        ex;
  context->handler_ip = (const guint16 *)handler_ip;
}

static void interp_get_resume_state(const MonoJitTlsData *jit_tls,
 ...
 * interp_run_finally:
 *
 *   Run the finally clause identified by CLAUSE_INDEX in the intepreter frame
 * given by frame->interp_frame. Return TRUE if the finally clause threw an
 * exception.
 */
static gboolean interp_run_finally(StackFrameInfo *frame, int clause_index,
                                   gpointer handler_ip,
 ...
 * interp_run_filter:
 *
 *   Run the filter clause identified by CLAUSE_INDEX in the intepreter frame
 * given by frame->interp_frame.
 */
// Do not inline in case order of frame addresses matters.
static MONO_NEVER_INLINE gboolean interp_run_filter(StackFrameInfo *frame,
                                                    MonoException *ex,
                                                    int clause_index,
                                                    gpointer handler_ip,
                                                    gpointer handler_ip_end) {
   ...
   */
  InterpFrame child_frame = {iframe, iframe->imethod, iframe->stack_args,
                             &retval};
 ...
 */
static void interp_frame_iter_init(MonoInterpStackIter *iter,
 ...
 */
static gboolean interp_frame_iter_next(MonoInterpStackIter *iter,
  ...
  while (iframe &&
         !(iframe->imethod && iframe->imethod->code && iframe->imethod->jinfo))
  ...
  if (method && ((method->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL) ||
    ...
    frame->native_offset =
        (guint8 *)iframe->ip - (guint8 *)iframe->imethod->code;
    if (!method->wrapper_type ||
...
static MonoJitInfo *interp_find_jit_info(MonoDomain *domain,
                                         MonoMethod *method) {
  InterpMethod *imethod;
...
static void interp_set_breakpoint(MonoJitInfo *jinfo, gpointer ip) {
  guint16 *code = (guint16 *)ip;
  g_assert(*code == MINT_SDB_SEQ_POINT);
  *code = MINT_SDB_BREAKPOINT;
}

static void interp_clear_breakpoint(MonoJitInfo *jinfo, gpointer ip) {
  guint16 *code = (guint16 *)ip;
  g_assert(*code == MINT_SDB_BREAKPOINT);
  *code = MINT_SDB_SEQ_POINT;
}

static MonoJitInfo *interp_frame_get_jit_info(MonoInterpFrameHandle frame) {
  InterpFrame *iframe = (InterpFrame *)frame;
...
static gpointer interp_frame_get_ip(MonoInterpFrameHandle frame) {
  InterpFrame *iframe = (InterpFrame *)frame;
...
static gpointer interp_frame_get_arg(MonoInterpFrameHandle frame, int pos) {
  InterpFrame *iframe = (InterpFrame *)frame;
  MonoMethodSignature *sig;
  ...
  return stackval_to_data_addr(
      sig->params[pos], &iframe->stack_args[pos + !!iframe->imethod->hasthis]);
}

static gpointer interp_frame_get_local(MonoInterpFrameHandle frame, int pos) {
  InterpFrame *iframe = (InterpFrame *)frame;
...
static gpointer interp_frame_get_this(MonoInterpFrameHandle frame) {
  InterpFrame *iframe = (InterpFrame *)frame;
...
static gpointer interp_frame_get_res(MonoInterpFrameHandle frame) {
  InterpFrame *iframe = (InterpFrame *)frame;
  MonoMethodSignature *sig;
...
static void interp_start_single_stepping(void) { ss_enabled = TRUE; }

static void interp_stop_single_stepping(void) { ss_enabled = FALSE; }
 ...
 *
 */
static void interp_mark_stack(gpointer thread_data, GcScanFunc func,
                              gpointer gc_data, gboolean precise) {
  ...
  /*
   * We explicitly mark the frames instead of registering the stack fragments as
   * GC roots, so we have to process less data and avoid false pinning from data
   * which is above 'pos'.
   *
   * The stack frame handling code uses compiler write barriers only, but the
   * calling code in sgen-mono.c already did a mono_memory_barrier_process_wide
   * () so we can process these data structures normally.
   */
...
static int opcode_count_comparer(const void *pa, const void *pb) {
...
static void interp_print_op_count(void) {
  ...
  for (i = 0; i < MINT_LASTOP; i++) {
    ...
    g_print("%s : %ld (%.2lf%%)\n", mono_interp_opname(ordered_ops[i]), count,
            (double)count / total_ops * 100);
  }
}
#endif

static void interp_set_optimizations(guint32 opts) { mono_interp_opt = opts; }

static void invalidate_transform(gpointer imethod_) {
  InterpMethod *imethod = (InterpMethod *)imethod_;
  imethod->transformed = FALSE;
}

static void interp_invalidate_transformed(MonoDomain *domain) {
  MonoJitDomainInfo *info = domain_jit_info(domain);
  mono_domain_jit_code_hash_lock(domain);
...
static void interp_cleanup(void) {
#if COUNT_OPS
  interp_print_op_count();
#endif
}

static void register_interp_stats(void) {
  mono_counters_init();
  mono_counters_register("Total transform time",
                         MONO_COUNTER_INTERP | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_interp_stats.transform_time);
  mono_counters_register("Methods transformed",
                         MONO_COUNTER_INTERP | MONO_COUNTER_LONG,
  ...
  mono_counters_register("Total super instructions time",
                         MONO_COUNTER_INTERP | MONO_COUNTER_LONG |
                             MONO_COUNTER_TIME,
                         &mono_interp_stats.super_instructions_time);
  mono_counters_register("STLOC_NP count",
                         MONO_COUNTER_INTERP | MONO_COUNTER_INT,
  ...
  mono_counters_register("Copy propagations",
                         MONO_COUNTER_INTERP | MONO_COUNTER_INT,
                         &mono_interp_stats.copy_propagations);
  mono_counters_register("Added pop count",
                         MONO_COUNTER_INTERP | MONO_COUNTER_INT,
                         &mono_interp_stats.added_pop_count);
  mono_counters_register("Constant folds",
                         MONO_COUNTER_INTERP | MONO_COUNTER_INT,
                         &mono_interp_stats.constant_folds);
  mono_counters_register("Super instructions",
                         MONO_COUNTER_INTERP | MONO_COUNTER_INT,
                         &mono_interp_stats.super_instructions);
  mono_counters_register("Killed instructions",
                         MONO_COUNTER_INTERP | MONO_COUNTER_INT,
                         &mono_interp_stats.killed_instructions);
  mono_counters_register("Emitted instructions",
                         MONO_COUNTER_INTERP | MONO_COUNTER_INT,
                         &mono_interp_stats.emitted_instructions);
  mono_counters_register("Methods inlined",
                         MONO_COUNTER_INTERP | MONO_COUNTER_INT,
                         &mono_interp_stats.inlined_methods);
  mono_counters_register("Inline failures",
                         MONO_COUNTER_INTERP | MONO_COUNTER_INT,
