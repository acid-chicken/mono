...
static void inline stackval_from_data(MonoType *type, stackval *result,
  ...
  case MONO_TYPE_GENERICINST: {
    if (mono_type_generic_inst_is_valuetype(type)) {
      mono_value_copy_internal(result->data.vt, data,
...
static void inline stackval_to_data(MonoType *type, stackval *val, void *data,
  ...
  case MONO_TYPE_GENERICINST: {
    ...
    if (m_class_is_valuetype(container_class) &&
      ...
      mono_value_copy_internal(data, val->data.vt,
                               mono_class_from_mono_type_internal(type));
      return;
    }
    stackval_to_data(
...
#if DEBUG_INTERP
      g_print("build_args_from_sig: margs->iargs [%d/%d]: 0x%016llx, hi=0x%08x "
              "lo=0x%08x (frame @ %d)\n",
...
static void dump_stackval(GString *str, stackval *s, MonoType *type) {
  ...
  default: {
    ...
    g_string_append_printf(str, "[{%s} %lld/0x%0llx] ", res->str, s->data.l,
                           s->data.l);
  ...
  do {                                                                         \
    ...
    ++sp;                                                                      \
      ...
      MINT_IN_CASE(MINT_VCALLVIRT_FAST) {
        ...
        // FIXME Have it handle also remoting calls and use a single opcode for
        // virtual calls
