...
alloc_frame(ThreadContext *ctx, gpointer native_stack_addr, InterpFrame *parent,
            InterpMethod *imethod, stackval *args, stackval *retval) {
  StackFragment *frag;
  InterpFrame *frame;

  // FIXME: Add stack overflow checks
  frame = (InterpFrame *)frame_stack_alloc(&ctx->iframe_stack,
                                           sizeof(InterpFrame), &frag);
...
static void interp_exec_method_full(InterpFrame *frame, ThreadContext *context,
                                    FrameClauseArgs *clause_args,
                                    MonoError *error);

static MONO_NEVER_INLINE void
interp_exec_method_newobj_vtst_fast(InterpFrame *frame, ThreadContext *context,
                                    MonoError *error)
// This function makes WebAsssembly stacks clearer, so you can see which
// recursion is occuring, in the absence of line numbers in the debugger.
{
  interp_exec_method_full(frame, context, NULL, error);
}
...
static ThreadContext *get_context(void) {
  ...
  if (context == NULL) {
     ...
     * This is useful because InterpFrame structures don't need to be GC
     * tracked.
     */
...
static MONO_NEVER_INLINE InterpMethod *get_virtual_method(InterpMethod *imethod,
                                                          MonoVTable *vtable) {
...
static MONO_NEVER_INLINE
    InterpMethod * // Inlining causes additional stack use in caller.
    get_virtual_method_fast(InterpMethod *imethod, MonoVTable *vtable,
                            int offset) {
  gpointer *table;
...
ves_array_element_address(InterpFrame *frame, MonoClass *required_type,
...
ves_pinvoke_method(InterpFrame *frame, MonoMethodSignature *sig, MonoFuncV addr,
                   ThreadContext *context, gboolean save_last_error) {
  MonoLMFExt ext;
  gpointer args;
...
static MonoObject *interp_runtime_invoke(MonoMethod *method, void *obj,
...
static void interp_entry(InterpEntryData *data) {
  InterpFrame *frame;
  InterpMethod *rmethod;
  ThreadContext *context;
  stackval result;
...
static MONO_NEVER_INLINE guchar *copy_varargs_vtstack(MonoMethodSignature *csig,
                                                      stackval *sp,
                                                      guchar *vt_sp_start) {
...
static void interp_entry_from_trampoline(gpointer ccontext_untyped,
  ...
  for (i = 0; i < sig->param_count; i++) {
    ...
    alloc_storage_for_stackval(&frame->stack_args[i + sig->hasthis], type,
                               sig->pinvoke);
  }
...
static MONO_NEVER_INLINE MonoObject *mono_interp_new(MonoDomain *domain,
                                                     MonoClass *klass) {
  ERROR_DECL(error);
...
#endif
    void
    mono_interp_load_remote_field(InterpMethod *imethod, MonoObject *o,
...
static MONO_NEVER_INLINE gboolean mono_interp_isinst(MonoObject *object,
                                                     MonoClass *klass) {
  ERROR_DECL(error);
  gboolean isinst;
...
static MONO_NEVER_INLINE void mono_interp_calli_nat_dynamic_pinvoke(
    ...
    InterpFrame *child_frame, guchar *code, ThreadContext *context,
  ...
  MonoMarshalSpec **mspecs =
      g_newa(MonoMarshalSpec *, csignature->param_count + 1);
...
// but not have to change how exception handling macros access labels and
// locals.
static MONO_NEVER_INLINE MonoException *
mono_interp_leave(InterpFrame *child_frame) {
   ...
   */
  do_icall_wrapper(child_frame, NULL, MINT_ICALL_V_P, &tmp_sp,
                   (gpointer)mono_thread_get_undeniable_exception, FALSE);

  return (MonoException *)tmp_sp.data.p;
}

static MONO_NEVER_INLINE void mono_interp_newobj_vt(
    ...
    InterpFrame *child_frame, ThreadContext *context, MonoError *error) {
...
static MONO_NEVER_INLINE MonoException *mono_interp_newobj(
    // Parameters are sorted by name and parameter list is minimized
    // to reduce stack use in caller, on e.g. NT/AMD64 (up to 4 parameters
    // use no stack in caller).
    InterpFrame *child_frame, ThreadContext *context, MonoError *error,
  ...
  /*if (profiling_classes) {
      guint count = GPOINTER_TO_UINT (g_hash_table_lookup (profiling_classes,
  newobj_class)); count++; g_hash_table_insert (profiling_classes, newobj_class,
  ...
  if (m_class_is_valuetype(newobj_class)) {
    ...
    memset(&valuetype_this, 0, sizeof(stackval));
    if (!m_class_is_enumtype(newobj_class) &&
        (t->type == MONO_TYPE_VALUETYPE ||
    ...
    if (newobj_class != mono_defaults.string_class) {
      MonoVTable *vtable =
          mono_class_vtable_checked(imethod->domain, newobj_class, error);
      ...
      if (mono_object_is_transparent_proxy(o)) {
        MonoMethod *remoting_invoke_method =
            mono_marshal_get_remoting_invoke_with_check(
                child_frame->imethod->method, error);
        mono_error_assert_ok(error);
        child_frame->imethod = mono_interp_get_imethod(
            imethod->domain, remoting_invoke_method, error);
        mono_error_assert_ok(error);
      }
#endif
    } else {
...
static MONO_NEVER_INLINE void mono_interp_enum_hasflag(stackval *sp,
                                                       MonoClass *klass) {
...
static MONO_NEVER_INLINE int mono_interp_box_nullable(InterpFrame *frame,
                                                      const guint16 *ip,
                                                      stackval *sp,
                                                      MonoError *error) {
  InterpMethod *const imethod = frame->imethod;
...
static MONO_NEVER_INLINE void mono_interp_box(InterpFrame *frame,
                                              const guint16 *ip, stackval *sp) {
  MonoObject *o; // See the comment about GC safety.
...
static MONO_NEVER_INLINE int
mono_interp_store_remote_field_vt(InterpFrame *frame, const guint16 *ip,
 ...
 */
static MONO_NEVER_INLINE void
interp_exec_method_full(InterpFrame *frame, ThreadContext *context,
    ...
    ++sp;                                                                      \
      ...
      MINT_IN_CASE(MINT_CALLI) {
        ...
        gpointer native_stack_addr =
            frame->native_stack_addr
                ? (gpointer)((guint8 *)frame->native_stack_addr - 1)
                : (gpointer)&retval;
        ...
        if (imethod->method->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL) {
          imethod = mono_interp_get_imethod(
              frame->imethod->domain,
              mono_marshal_get_native_wrapper(imethod->method, FALSE, FALSE),
              error);
          mono_interp_error_cleanup(error); /* FIXME: don't swallow the error */
        }
        ...
        frame =
            alloc_frame(context, native_stack_addr, frame, imethod, sp, retval);
      ...
      MINT_IN_CASE(MINT_CALLI_NAT) {
        ...
        if (frame->imethod->method->dynamic && csignature->pinvoke) {
          mono_interp_calli_nat_dynamic_pinvoke(child_frame, code, context,
                                                csignature, error);
        } else {
          const gboolean save_last_error = ip[-3 + 2];
          ves_pinvoke_method(child_frame, csignature, (MonoFuncV)code, context,
                             save_last_error);
        }
        CHECK_RESUME_STATE(context);
      ...
      MINT_IN_CASE(MINT_VCALLVIRT_FAST) {
        ...
        if (m_class_is_valuetype(this_arg->vtable->klass) &&
            m_class_is_valuetype(imethod->method->klass)) {
          /* unbox */
          gpointer unboxed = mono_object_unbox_internal(this_arg);
          sp[0].data.p = unboxed;
        }
        ...
        if (G_UNLIKELY(code_type == IMETHOD_CODE_UNKNOWN)) {
          MonoMethodSignature *sig =
              mono_method_signature_internal(imethod->method);
      ...
      call:;
        ...
        frame =
            alloc_frame(context, native_stack_addr, frame, cmethod, sp, retval);
  ...
  ++ip;
      ...
      MINT_IN_CASE(MINT_NEWOBJ_VTST_FAST) {
        ...
        child_frame = alloc_frame(
            context, &dummy, frame,
            (InterpMethod *)frame->imethod->data_items[ip[1]], NULL, NULL);
      ...
      MINT_IN_CASE(MINT_NEWOBJ) {
        ...
        child_frame = alloc_frame(
            context, &dummy, frame,
            (InterpMethod *)frame->imethod->data_items[token], NULL, NULL);
        MonoMethodSignature *const csig =
            mono_method_signature_internal(child_frame->imethod->method);
        ...
        // FIXME remove recursion
        MonoException *const exc =
            mono_interp_newobj(child_frame, context, error, vt_sp);
        if (exc)
          THROW_EX(exc, ip);
        CHECK_RESUME_STATE(context);
        ++sp;
    ...
    ip++;                                                                      \
      ...
      *(gpointer *)sp->data.p = frame->imethod->data_items[ip[1]];
      ...
      MINT_IN_CASE(MINT_LEAVE_S_CHECK) {
        int dummy;
        // Leave is split into pieces in order to consume less stack,
        // but not have to change how exception handling macros access labels
        // and locals.
...
exit_frame:
  ...
  if (clause_args && clause_args->base_frame)
    memcpy(clause_args->base_frame->stack, frame->stack,
           frame->imethod->alloca_size);
 ...
 */
static void interp_set_resume_state(MonoJitTlsData *jit_tls, MonoException *ex,
  ...
  /* Ditto */
  if (ei)
    *(MonoException **)(frame_locals(context->handler_frame) +
                        ei->exvar_offset) = ex;
 ...
 */
static gboolean interp_run_filter(StackFrameInfo *frame, MonoException *ex,
                                  int clause_index, gpointer handler_ip,
                                  gpointer handler_ip_end) {
  InterpFrame *iframe = (InterpFrame *)frame->interp_frame;
  ThreadContext *context = get_context();
  InterpFrame *child_frame;
   ...
   */
  child_frame = alloc_frame(context, &retval, iframe, iframe->imethod,
                            iframe->stack_args, &retval);
