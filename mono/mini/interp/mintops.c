...
const guint16 *mono_interp_dis_mintop_len(const guint16 *ip) {
 ...
 */
char *mono_interp_dis_mintop(gint32 ins_offset, gboolean native_offset,
                             const guint16 *ip, guint16 opcode) {
  GString *str = g_string_new("");
  guint32 token;
  int target;

  if (native_offset)
    g_string_append_printf(str, "IR_%04x: %-10s", ins_offset,
                           mono_interp_opname(opcode));
  else
    g_string_append_printf(str, "IL_%04x: %-10s", ins_offset,
                           mono_interp_opname(opcode));
  ...
  case MintOpShortAndInt:
    g_string_append_printf(str, " %u,%u", *(guint16 *)ip,
                           (guint32)READ32(ip + 1));
    break;
  case MintOpShortInt:
...
const char *mono_interp_opname(int op) {
  return ((const char *)&mono_interp_opname_characters) +
         mono_interp_opname_offsets[op];
}
