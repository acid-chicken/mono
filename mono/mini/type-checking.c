...
#define is_complex_isinst(klass)                                               \
  (mono_class_is_interface(klass) || m_class_get_rank(klass) ||                \
   mono_class_is_nullable(klass) || mono_class_is_marshalbyref(klass) ||       \
   mono_class_is_sealed(klass) ||                                              \
   m_class_get_byval_arg(klass)->type == MONO_TYPE_VAR ||                      \
   m_class_get_byval_arg(klass)->type == MONO_TYPE_MVAR)

static int get_castclass_cache_idx(MonoCompile *cfg) {
  /* Each CASTCLASS_CACHE patch needs a unique index which identifies the call
   * site */
  cfg->castclass_cache_index++;
  return (cfg->method_index << 16) | cfg->castclass_cache_index;
}

static void emit_cached_check_args(MonoCompile *cfg, MonoInst *obj,
  ...
  if (context_used) {
    ...
    cache_ins = mini_emit_get_rgctx_klass(cfg, context_used, klass,
                                          MONO_RGCTX_INFO_CAST_CACHE);

    /* klass - it's the second element of the cache entry*/
    EMIT_NEW_LOAD_MEMBASE(cfg, args[1], OP_LOAD_MEMBASE, alloc_preg(cfg),
                          cache_ins->dreg, TARGET_SIZEOF_VOID_P);
    ...
    args[2] = mini_emit_runtime_constant(cfg, MONO_PATCH_INFO_CASTCLASS_CACHE,
                                         GINT_TO_POINTER(idx));
  }
}

static MonoInst *emit_isinst_with_cache(MonoCompile *cfg, MonoInst *obj,
                                        MonoClass *klass, int context_used) {
  MonoInst *args[3];
...
static MonoInst *emit_castclass_with_cache_no_details(MonoCompile *cfg,
                                                      MonoInst *obj,
                                                      MonoClass *klass,
                                                      int context_used) {
  MonoInst *args[3];
...
static MonoInst *emit_castclass_with_cache(MonoCompile *cfg, MonoInst *obj,
                                           MonoClass *klass, int context_used) {
  MonoInst *args[3];
  MonoMethod *mono_castclass = mono_marshal_get_castclass_with_cache();
  MonoInst *res;
...
static void mini_emit_class_check_inst(MonoCompile *cfg, int klass_reg,
                                       MonoClass *klass, MonoInst *klass_inst) {
  if (klass_inst) {
    MONO_EMIT_NEW_BIALU(cfg, OP_COMPARE, -1, klass_reg, klass_inst->dreg);
  } else {
    MonoInst *ins =
        mini_emit_runtime_constant(cfg, MONO_PATCH_INFO_CLASS, klass);
    MONO_EMIT_NEW_BIALU(cfg, OP_COMPARE, -1, klass_reg, ins->dreg);
  }
  MONO_EMIT_NEW_COND_EXC(cfg, NE_UN, "InvalidCastException");
}

static void mini_emit_isninst_cast_inst(MonoCompile *cfg, int klass_reg,
                                        MonoClass *klass, MonoInst *klass_ins,
  ...
  if (m_class_get_idepth(klass) > MONO_DEFAULT_SUPERTABLE_SIZE) {
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU2_MEMBASE, idepth_reg, klass_reg,
                                  m_class_offsetof_idepth());
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, idepth_reg,
                            m_class_get_idepth(klass));
    MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_PBLT_UN, false_target);
  }
  MONO_EMIT_NEW_LOAD_MEMBASE(cfg, stypes_reg, klass_reg,
                             m_class_offsetof_supertypes());
  MONO_EMIT_NEW_LOAD_MEMBASE(
      cfg, stype, stypes_reg,
      ((m_class_get_idepth(klass) - 1) * TARGET_SIZEOF_VOID_P));
  if (klass_ins) {
...
static void mini_emit_interface_bitmap_check(MonoCompile *cfg, int intf_bit_reg,
  ...
  if (cfg->compile_aot) {
    ...
    MONO_EMIT_NEW_BIALU(cfg, OP_PADD, ibitmap_byte_address_reg, ibitmap_reg,
                        shifted_iid_reg);
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, ibitmap_byte_reg,
                                  ibitmap_byte_address_reg, 0);
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_IAND_IMM, masked_iid_reg, iid_reg, 7);
    MONO_EMIT_NEW_ICONST(cfg, iid_one_bit_reg, 1);
    MONO_EMIT_NEW_BIALU(cfg, OP_ISHL, iid_bit_reg, iid_one_bit_reg,
                        masked_iid_reg);
    MONO_EMIT_NEW_BIALU(cfg, OP_IAND, intf_bit_reg, ibitmap_byte_reg,
                        iid_bit_reg);
  } else {
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI1_MEMBASE, ibitmap_byte_reg,
                                  ibitmap_reg,
                                  m_class_get_interface_id(klass) >> 3);
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_AND_IMM, intf_bit_reg, ibitmap_byte_reg,
                            ((target_mgreg_t)1)
                                << (m_class_get_interface_id(klass) & 7));
  }
#endif
}
...
static void mini_emit_load_intf_bit_reg_class(MonoCompile *cfg,
                                              int intf_bit_reg, int klass_reg,
                                              MonoClass *klass) {
  mini_emit_interface_bitmap_check(cfg, intf_bit_reg, klass_reg,
...
static void mini_emit_load_intf_bit_reg_vtable(MonoCompile *cfg,
                                               int intf_bit_reg, int vtable_reg,
                                               MonoClass *klass) {
  mini_emit_interface_bitmap_check(
      cfg, intf_bit_reg, vtable_reg,
      MONO_STRUCT_OFFSET(MonoVTable, interface_bitmap), klass);
}
...
static void mini_emit_max_iid_check(MonoCompile *cfg, int max_iid_reg,
  ...
  if (cfg->compile_aot) {
    ...
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, max_iid_reg,
...
static void mini_emit_max_iid_check_vtable(MonoCompile *cfg, int vtable_reg,
  ...
  MONO_EMIT_NEW_LOAD_MEMBASE_OP(
      cfg, OP_LOADU4_MEMBASE, max_iid_reg, vtable_reg,
      MONO_STRUCT_OFFSET(MonoVTable, max_interface_id));
...
static void mini_emit_max_iid_check_class(MonoCompile *cfg, int klass_reg,
  ...
  MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU4_MEMBASE, max_iid_reg, klass_reg,
                                m_class_offsetof_max_interface_id());
  mini_emit_max_iid_check(cfg, max_iid_reg, klass, false_target);
}

static void mini_emit_class_check_branch(MonoCompile *cfg, int klass_reg,
                                         MonoClass *klass, int branch_op,
...
static void mini_emit_isninst_cast(MonoCompile *cfg, int klass_reg,
                                   ...
                                   MonoBasicBlock *false_target,
                                   MonoBasicBlock *true_target) {
  mini_emit_isninst_cast_inst(cfg, klass_reg, klass, NULL, false_target,
                              true_target);
}
...
static void mini_emit_castclass(MonoCompile *cfg, int obj_reg, int klass_reg,
                                MonoClass *klass,
                                MonoBasicBlock *object_is_null);

static void mini_emit_castclass_inst(MonoCompile *cfg, int obj_reg,
                                     int klass_reg, MonoClass *klass,
                                     MonoInst *klass_inst,
                                     MonoBasicBlock *object_is_null) {
  if (m_class_get_rank(klass)) {
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, rank_reg, klass_reg,
                                  m_class_offsetof_rank());
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, rank_reg,
    ...
    if (m_class_is_array_special_interface(m_class_get_cast_class(klass))) {
      ...
      mini_emit_iface_class_cast(cfg, eclass_reg, m_class_get_cast_class(klass),
                                 NULL, NULL);
    } else {
      // Pass -1 as obj_reg to skip the check below for arrays of arrays
      mini_emit_castclass(cfg, -1, eclass_reg, m_class_get_cast_class(klass),
                          object_is_null);
    }
    ...
    if ((m_class_get_rank(klass) == 1) &&
      ...
      MONO_EMIT_NEW_LOAD_MEMBASE(cfg, bounds_reg, obj_reg,
    ...
    if (m_class_get_idepth(klass) > MONO_DEFAULT_SUPERTABLE_SIZE) {
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU2_MEMBASE, idepth_reg,
                                    klass_reg, m_class_offsetof_idepth());
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, idepth_reg,
                              m_class_get_idepth(klass));
      MONO_EMIT_NEW_COND_EXC(cfg, LT_UN, "InvalidCastException");
    }
...
static void mini_emit_castclass(MonoCompile *cfg, int obj_reg, int klass_reg,
                                MonoClass *klass,
                                MonoBasicBlock *object_is_null) {
  mini_emit_castclass_inst(cfg, obj_reg, klass_reg, klass, NULL,
                           object_is_null);
}

static void emit_special_array_iface_check(MonoCompile *cfg, MonoInst *src,
                                           MonoClass *klass, int vtable_reg,
                                           MonoBasicBlock *not_an_array,
                                           MonoBasicBlock *true_bb,
                                           int context_used) {
  int rank_reg;
 ...
 */
static MonoInst *handle_castclass(MonoCompile *cfg, MonoClass *klass,
                                  MonoInst *src, int context_used) {
  ...
  if (context_used) {
    ...
    klass_inst = mini_emit_get_rgctx_klass(cfg, context_used, klass,
  ...
  if (mono_class_is_interface(klass)) {
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE(cfg, tmp_reg, obj_reg,
    ...
    emit_special_array_iface_check(cfg, src, klass, tmp_reg, array_fail_bb,
                                   is_null_bb, context_used);

    // array check failed
    MONO_START_BB(cfg, array_fail_bb);

    MONO_EMIT_NEW_LOAD_MEMBASE(cfg, klass_reg, tmp_reg,
                               MONO_STRUCT_OFFSET(MonoVTable, klass));
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE(
        cfg, tmp_reg, obj_reg,
        MONO_STRUCT_OFFSET(MonoTransparentProxy, custom_type_info));
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, tmp_reg, 0);
    MONO_EMIT_NEW_COND_EXC(cfg, EQ, "InvalidCastException");

    MonoInst *args[1] = {src};
    MonoInst *proxy_test_inst = mono_emit_method_call(
        cfg, mono_marshal_get_proxy_cancast(klass), args, NULL);
    ...
    if (m_class_is_array_special_interface(klass)) {
      ...
      // Check if it's a rank zero array and emit fallback casting
      emit_special_array_iface_check(cfg, src, klass, tmp_reg, NULL, is_null_bb,
                                     context_used);
    } else {
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE(
        cfg, klass_reg, tmp_reg,
        MONO_STRUCT_OFFSET(MonoRemoteClass, proxy_class));
    ...
    mini_emit_castclass_inst(cfg, obj_reg, klass_reg, klass, klass_inst,
                             is_null_bb);
#else
    g_error("Transparent proxy support is disabled while trying to JIT code "
            "that uses it");
#endif
  } else {
    ...
    if (!m_class_get_rank(klass) && !cfg->compile_aot &&
        !(cfg->opt & MONO_OPT_SHARED) && mono_class_is_sealed(klass)) {
      /* the remoting code is broken, access the class for now */
      if (0) { /*FIXME what exactly is broken? This change refers to r39380 from
                  2005 and mention some remoting fixes were due.*/
        MonoVTable *vt =
        ...
        MONO_EMIT_NEW_LOAD_MEMBASE(cfg, klass_reg, vtable_reg,
                                   MONO_STRUCT_OFFSET(MonoVTable, klass));
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, klass_reg,
                                (gsize)klass);
      }
      MONO_EMIT_NEW_COND_EXC(cfg, NE_UN, "InvalidCastException");
    } else {
 ...
 * Returns NULL and set the cfg exception on error.
 */
static MonoInst *handle_isinst(MonoCompile *cfg, MonoClass *klass,
                               MonoInst *src, int context_used) {
  MonoInst *ins;
  ...
  /* Do the assignment at the beginning, so the other assignment can be if
   * converted */
  ...
  if (mono_class_is_interface(klass)) {
    ...
    if (m_class_is_array_special_interface(klass)) {
      ...
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, rank_reg,
                                    vtable_reg,
                                    MONO_STRUCT_OFFSET(MonoVTable, rank));
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, rank_reg, 1);
      MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_IBNE_UN, not_an_array);
      ...
      MonoInst *res_inst =
          emit_isinst_with_cache(cfg, src, klass, context_used);
    ...
    mini_emit_isninst_cast(cfg, klass_reg, klass, call_proxy_isinst,
                           is_null_bb);
    ...
    if (m_class_get_rank(klass)) {
      ...
      if (m_class_is_array_special_interface(m_class_get_cast_class(klass))) {
        ...
        MONO_EMIT_NEW_LOAD_MEMBASE(cfg, parent_reg, eclass_reg,
                                   m_class_offsetof_parent());
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, class_kind_reg,
                                      eclass_reg,
                                      m_class_offsetof_class_kind());

        // Check if the parent class of the element is not System.ValueType
        mini_emit_class_check_branch(
            cfg, parent_reg, m_class_get_parent(mono_defaults.enum_class),
            OP_PBNE_UN, pointer_check_bb);
        mini_emit_class_check_branch(cfg, eclass_reg, mono_defaults.enum_class,
                                     OP_PBEQ, is_null_bb);
        MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_BR, false_bb);

        MONO_START_BB(cfg, pointer_check_bb);
        // Check if the parent class of the element is non-null, else manually
        // check the type
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, parent_reg, NULL);
        MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_PBNE_UN, is_null_bb);
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, class_kind_reg,
                                MONO_CLASS_POINTER);
        MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_PBEQ, false_bb);
        MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_BR, is_null_bb);
      } else if (m_class_get_cast_class(klass) ==
                 m_class_get_parent(mono_defaults.enum_class)) {
        mini_emit_class_check_branch(
            cfg, eclass_reg, m_class_get_parent(mono_defaults.enum_class),
            OP_PBEQ, is_null_bb);
        mini_emit_class_check_branch(cfg, eclass_reg, mono_defaults.enum_class,
                                     OP_PBEQ, is_null_bb);
        MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_BR, false_bb);
        ...
        mini_emit_iface_class_cast(cfg, eclass_reg,
                                   m_class_get_cast_class(klass), false_bb,
                                   is_null_bb);
      } else {
        /* the is_null_bb target simply copies the input register to the output
         */
        mini_emit_isninst_cast(cfg, eclass_reg, m_class_get_cast_class(klass),
                               false_bb, is_null_bb);
      }
      ...
      MONO_EMIT_NEW_LOAD_MEMBASE(cfg, klass_reg, vtable_reg,
                                 MONO_STRUCT_OFFSET(MonoVTable, klass));
      /* the is_null_bb target simply copies the input register to the output */
      mini_emit_isninst_cast(cfg, klass_reg, m_class_get_cast_class(klass),
                             false_bb, is_null_bb);
    } else {
      if (!cfg->compile_aot && !(cfg->opt & MONO_OPT_SHARED) &&
        ...
        g_assert(!context_used);
        /* the remoting code is broken, access the class for now */
        if (0) { /*FIXME what exactly is broken? This change refers to r39380
                    from 2005 and mention some remoting fixes were due.*/
          MonoVTable *vt =
              mono_class_vtable_checked(cfg->domain, klass, cfg->error);
          if (!is_ok(cfg->error)) {
            mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);
        ...
        /* the is_null_bb target simply copies the input register to the output
         */
        mini_emit_isninst_cast_inst(cfg, klass_reg, klass, klass_inst, false_bb,
                                    is_null_bb);
      }
    }
  }
...
static void mono_decompose_typecheck(MonoCompile *cfg, MonoBasicBlock *bb,
                                     MonoInst *ins) {
  ...
  if (!source || source == (MonoInst *)-1)
    source = mono_compile_create_var_for_vreg(cfg, mono_get_object_type(),
                                              OP_LOCAL, ins->sreg1);
  ...
  if (mini_class_has_reference_variant_generic_argument(cfg, klass,
                                                        context_used)) {
    if (is_isinst)
