...
#define EMIT_COND_BRANCH(ins, cond)                                            \
  {                                                                            \
    if (ins->inst_true_bb->native_offset) {                                    \
      int displace;                                                            \
      displace =                                                               \
          ((cfg->native_code + ins->inst_true_bb->native_offset) - code) / 2;  \
...
#define EMIT_UNCOND_BRANCH(ins)                                                \
  {                                                                            \
    if (ins->inst_target_bb->native_offset) {                                  \
      int displace;                                                            \
      displace =                                                               \
          ((cfg->native_code + ins->inst_target_bb->native_offset) - code) /   \
          2;                                                                   \
      if (s390_is_imm16(displace)) {                                           \
  ...
  {                                                                            \
    if (ins->inst_true_bb->native_offset) {                                    \
      ...
      displace =                                                               \
          ((cfg->native_code + ins->inst_true_bb->native_offset) - code) / 2;  \
      if (s390_is_imm16(displace)) {                                           \
        s390_##cab(code, ins->sreg1, ins->sreg2, ins->sreg3, displace);        \
      } else {                                                                 \
        s390_##cmp(code, ins->sreg1, ins->sreg2);                              \
        displace =                                                             \
            ((cfg->native_code + ins->inst_true_bb->native_offset) - code) /   \
            2;                                                                 \
    ...
    if (ins->inst_true_bb->native_offset) {                                    \
      ...
      if ((ins->backend.data == 0) && (!logical)) {                            \
        ...
        displace =                                                             \
            ...
            2;                                                                 \
        if (s390_is_imm16(displace)) {                                         \
          s390_##cab(code, ins->sreg1, s390_r0, ins->sreg3, displace);         \
        } else {                                                               \
...
#define MONO_EMIT_NEW_MOVE(cfg, dest, offset, src, imm, size)                  \
  do {                                                                         \
    MonoInst *inst;                                                            \
    int sReg, dReg;                                                            \
    ...
    if (size > 256) {                                                          \
      ...
      if (s390_is_uimm12(offset)) {                                            \
        ...
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ADD_IMM, dReg, dest, offset);          \
        inst->dreg = dReg;                                                     \
        inst->inst_offset = 0;                                                 \
      }                                                                        \
      if (s390_is_uimm12(imm)) {                                               \
        ...
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ADD_IMM, sReg, src, imm);              \
        inst->sreg1 = sReg;                                                    \
        inst->inst_imm = 0;                                                    \
      }                                                                        \
    }                                                                          \
...
#define MONO_OUTPUT_VTR(cfg, size, dr, sr, so)                                 \
  do {                                                                         \
    ...
    case 0:                                                                    \
      ...
      break;                                                                   \
    case 1:                                                                    \
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, reg, sr, so);      \
      break;                                                                   \
    case 2:                                                                    \
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU2_MEMBASE, reg, sr, so);      \
      break;                                                                   \
    case 4:                                                                    \
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI4_MEMBASE, reg, sr, so);      \
      break;                                                                   \
    case 8:                                                                    \
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI8_MEMBASE, reg, sr, so);      \
      break;                                                                   \
    }                                                                          \
    mono_call_inst_add_outarg_reg(cfg, call, reg, dr, FALSE);                  \
  } while (0)

#define MONO_OUTPUT_VTS(cfg, size, dr, dx, sr, so)                             \
  do {                                                                         \
    int tmpr;                                                                  \
    switch (size) {                                                            \
    case 0:                                                                    \
      ...
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, dr, dx, tmpr);    \
      break;                                                                   \
    case 1:                                                                    \
      tmpr = mono_alloc_preg(cfg);                                             \
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, tmpr, sr, so);     \
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, dr, dx, tmpr);    \
      break;                                                                   \
    case 2:                                                                    \
      tmpr = mono_alloc_preg(cfg);                                             \
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU2_MEMBASE, tmpr, sr, so);     \
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, dr, dx, tmpr);    \
      break;                                                                   \
    case 4:                                                                    \
      tmpr = mono_alloc_preg(cfg);                                             \
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI4_MEMBASE, tmpr, sr, so);     \
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, dr, dx, tmpr);    \
      break;                                                                   \
    case 8:                                                                    \
...
typedef struct {
  guint stack_size, local_size, code_size, parm_size, offset, offStruct,
      retStruct;
} size_data;
...
static guchar *emit_float_to_int(MonoCompile *, guchar *, int, int, int,
                                 gboolean);
static __inline__ void emit_unwind_regs(MonoCompile *, guint8 *, int, int,
                                        long);
...
static const char *grNames[] = {"s390_r0",  "s390_sp",  "s390_r2",  "s390_r3",
                                "s390_r4",  "s390_r5",  "s390_r6",  "s390_r7",
                                "s390_r8",  "s390_r9",  "s390_r10", "s390_r11",
                                "s390_r12", "s390_r13", "s390_r14", "s390_r15"};

static const char *fpNames[] = {"s390_f0",  "s390_f1",  "s390_f2",  "s390_f3",
                                "s390_f4",  "s390_f5",  "s390_f6",  "s390_f7",
                                "s390_f8",  "s390_f9",  "s390_f10", "s390_f11",
                                "s390_f12", "s390_f13", "s390_f14", "s390_f15"};
...
static void __inline__ emit_unwind_regs(MonoCompile *cfg, guint8 *code,
                                        int start, int end, long offset) {
  int i;
...
static gboolean retFitsInReg(guint32 size) {
  switch (size) {
  case 0:
  case 1:
  case 2:
...
static guint8 *backUpStackPtr(MonoCompile *cfg, guint8 *code) {
...
gboolean mono_arch_have_fast_tls(void) { return TRUE; }
...
static void inline add_general(guint *gr, size_data *sz, ArgInfo *ainfo) {
...
static void inline add_stackParm(guint *gr, size_data *sz, ArgInfo *ainfo,
                                 gint size) {
  if (*gr > S390_LAST_ARG_REG) {
    sz->stack_size = S390_ALIGN(sz->stack_size, sizeof(long));
    ainfo->reg = STK_BASE;
...
static void inline add_float(guint *fr, size_data *sz, ArgInfo *ainfo,
                             gboolean isDouble) {
...
enum_retvalue:
  ...
  for (i = pstart; i < sig->param_count; ++i) {
    ...
    case MONO_TYPE_VALUETYPE: {
      ...
      if (simpleType != MONO_TYPE_GENERICINST) {
        ...
        if ((info->native_size == sizeof(float)) && (info->num_fields == 1) &&
        ...
        if ((info->native_size == sizeof(double)) && (info->num_fields == 1) &&
  ...
  /*----------------------------------------------------------*/
  if ((sig->call_convention == MONO_CALL_VARARG) && (nParm > 0) &&
      (!sig->pinvoke) && (sig->param_count == sig->sentinelpos)) {
    gr = S390_LAST_ARG_REG + 1;
    fr = S390_LAST_FPARG_REG + 1;
    add_general(&gr, sz, &cinfo->sigCookie);
  }
  ...
  cinfo->lastgr = gr;
  sz->stack_size = sz->stack_size + sz->local_size + sz->parm_size + sz->offset;
  sz->stack_size = S390_ALIGN(sz->stack_size, sizeof(long));
    ...
    if (inst->opcode != OP_REGVAR) {
      ...
      default:
        if (cinfo->args[iParm].reg == STK_BASE) {
          ...
          inst->inst_basereg = cfg->arch.bkchain_reg;
          size =
              (cinfo->args[iParm].size < 8 ? 8 - cinfo->args[iParm].size : 0);
          inst->inst_offset = cinfo->args[iParm].offset + size;
          size = sizeof(long);
        } else {
          inst->opcode = OP_REGOFFSET;
          inst->inst_basereg = frame_reg;
          size = (cinfo->args[iParm].size < 8 ? sizeof(int) : sizeof(long));
          offset = S390_ALIGN(offset, size);
...
static void add_outarg_reg2(MonoCompile *cfg, MonoCallInst *call,
...
void mono_arch_emit_call(MonoCompile *cfg, MonoCallInst *call) {
  ...
  lParamArea = MAX(
      (call->stack_usage - S390_MINIMAL_STACK_SIZE - cinfo->sz.parm_size), 0);
  ...
  for (i = 0; i < n; ++i) {
    ...
    if ((sig->call_convention == MONO_CALL_VARARG) && (!sig->pinvoke) &&
        (i == sig->sentinelpos)) {
    ...
    case RegTypeStructByAddrOnStack: {
      ...
      if (sig->params[i - sig->hasthis]->type == MONO_TYPE_TYPEDBYREF) {
        ...
        size = mono_type_native_stack_size(m_class_get_byval_arg(in->klass),
                                           &align);
      else {
        /*
    ...
    case RegTypeBase:
      if (!t->byref && t->type == MONO_TYPE_R4) {
        MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER4_MEMBASE_REG, STK_BASE,
                                    ainfo->offset + 4, in->dreg);
      } else if (!t->byref && (t->type == MONO_TYPE_R8)) {
        MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, STK_BASE,
  ...
  if (ainfo->regtype == RegTypeStructByVal) {
    ...
    */
    if (ainfo->reg != STK_BASE) {
      ...
      MONO_OUTPUT_VTS(cfg, size, ainfo->reg, ainfo->offset, src->dreg, 0);
    }
    ...
    MonoInst *load;
    int ovf_size = ainfo->vtsize, srcReg;
    guint32 size;
    ...
    if (ainfo->reg == STK_BASE) {
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, srcReg,
...
static void compare_and_branch(MonoBasicBlock *bb, MonoInst *ins, int cc,
                               gboolean logical) {
  MonoInst *last;
...
static guchar *emit_double_to_int(MonoCompile *cfg, guchar *code, int dreg,
                                  int sreg, int size, gboolean is_signed) {
  /* sreg is a float, dreg is an integer reg. */
  if (is_signed) {
...
static gboolean is_unsigned(MonoInst *next) {
  if ((next) &&
      (((next->opcode >= OP_IBNE_UN) && (next->opcode <= OP_IBLT_UN)) ||
       ((next->opcode >= OP_LBNE_UN) && (next->opcode <= OP_LBLT_UN)) ||
       ((next->opcode >= OP_COND_EXC_NE_UN) &&
        (next->opcode <= OP_COND_EXC_LT_UN)) ||
       ((next->opcode >= OP_COND_EXC_INE_UN) &&
        (next->opcode <= OP_COND_EXC_ILT_UN)) ||
       ((next->opcode == OP_CLT_UN) || (next->opcode == OP_CGT_UN) ||
        (next->opcode == OP_ICGE_UN) || (next->opcode == OP_ICLE_UN)) ||
       ((next->opcode == OP_ICLT_UN) || (next->opcode == OP_ICGT_UN) ||
        (next->opcode == OP_LCLT_UN) || (next->opcode == OP_LCGT_UN))))
    return TRUE;
  else
    return FALSE;
}
...
void mono_arch_output_basic_block(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS(bb, ins) {
    ...
    case OP_STOREI1_MEMBASE_IMM: {
      ...
      S390_LONG(code, stcy, stc, s390_r0, 0, ins->inst_destbasereg,
                ins->inst_offset);
    } break;
    case OP_STOREI2_MEMBASE_IMM: {
      s390_lghi(code, s390_r0, ins->inst_imm);
      S390_LONG(code, sthy, sth, s390_r0, 0, ins->inst_destbasereg,
                ins->inst_offset);
    } break;
    case OP_STOREI4_MEMBASE_IMM: {
      s390_lgfi(code, s390_r0, ins->inst_imm);
      S390_LONG(code, sty, st, s390_r0, 0, ins->inst_destbasereg,
                ins->inst_offset);
    } break;
    case OP_STORE_MEMBASE_IMM:
    case OP_STOREI8_MEMBASE_IMM: {
      S390_SET(code, s390_r0, ins->inst_imm);
      S390_LONG(code, stg, stg, s390_r0, 0, ins->inst_destbasereg,
                ins->inst_offset);
    } break;
    case OP_STOREI1_MEMBASE_REG: {
      S390_LONG(code, stcy, stc, ins->sreg1, 0, ins->inst_destbasereg,
                ins->inst_offset);
    } break;
    case OP_STOREI2_MEMBASE_REG: {
      S390_LONG(code, sthy, sth, ins->sreg1, 0, ins->inst_destbasereg,
                ins->inst_offset);
    } break;
    case OP_STOREI4_MEMBASE_REG: {
      S390_LONG(code, sty, st, ins->sreg1, 0, ins->inst_destbasereg,
                ins->inst_offset);
    } break;
    case OP_STORE_MEMBASE_REG:
    case OP_STOREI8_MEMBASE_REG: {
      S390_LONG(code, stg, stg, ins->sreg1, 0, ins->inst_destbasereg,
                ins->inst_offset);
    } break;
    case OP_LOADU4_MEM:
      g_assert_not_reached();
    ...
    case OP_LOADI8_MEMBASE: {
      S390_LONG(code, lg, lg, ins->dreg, 0, ins->inst_basereg,
                ins->inst_offset);
    } break;
    case OP_LOADI4_MEMBASE: {
      S390_LONG(code, lgf, lgf, ins->dreg, 0, ins->inst_basereg,
                ins->inst_offset);
    } break;
    case OP_LOADU4_MEMBASE: {
      S390_LONG(code, llgf, llgf, ins->dreg, 0, ins->inst_basereg,
                ins->inst_offset);
    } break;
    case OP_LOADU1_MEMBASE: {
      S390_LONG(code, llgc, llgc, ins->dreg, 0, ins->inst_basereg,
                ins->inst_offset);
    } break;
    case OP_LOADI1_MEMBASE: {
      S390_LONG(code, lgb, lgb, ins->dreg, 0, ins->inst_basereg,
                ins->inst_offset);
    } break;
    case OP_LOADU2_MEMBASE: {
      S390_LONG(code, llgh, llgh, ins->dreg, 0, ins->inst_basereg,
                ins->inst_offset);
    } break;
    case OP_LOADI2_MEMBASE: {
      S390_LONG(code, lgh, lgh, ins->dreg, 0, ins->inst_basereg,
                ins->inst_offset);
    } break;
    case OP_LCONV_TO_I1: {
    ...
    case OP_LMUL_IMM: {
      ...
      if ((mono_hwcap_s390x_has_gie) && (s390_is_imm32(ins->inst_imm))) {
    ...
    case OP_TAILCALL_MEMBASE: {
      ...
      while (stackUsage > 256) {
        s390_mvc(code, 256, STK_BASE, stackOffset, s390_r12, stackOffset);
        stackUsage -= 256;
        stackOffset += 256;
      }

      if (stackUsage > 0)
        s390_mvc(code, stackUsage, STK_BASE, stackOffset, s390_r12,
                 stackOffset);
    ...
    case OP_CHECK_THIS: {
      ...
      s390_ltgr(code, s390_r0, s390_r0);
      //			EMIT_COND_SYSTEM_EXCEPTION (S390_CC_ZR,
    ...
    case OP_LOCALLOC: {
      ...
      if (cfg->method->save_lmf) {
        ...
        s390_stg(code, s390_r14, 0, s390_r13, MONO_STRUCT_OFFSET(MonoLMF, ebp));
      }
    ...
    case OP_START_HANDLER: {
      ...
      S390_LONG(code, stg, stg, s390_r14, 0, spvar->inst_basereg,
                spvar->inst_offset);
    } break;
    case OP_ENDFILTER: {
      MonoInst *spvar = mono_find_spvar_for_region(cfg, bb->region);
      ...
      if (ins->sreg1 != s390_r2)
        s390_lgr(code, s390_r2, ins->sreg1);
      S390_LONG(code, lg, lg, s390_r14, 0, spvar->inst_basereg,
                spvar->inst_offset);
    ...
    case OP_GENERIC_CLASS_INIT: {
      ...
      if (byte_offset < 0)
        mono_marshal_find_bitfield_offset(MonoVTable, initialized, &byte_offset,
                                          &bitmask);
    ...
    case OP_STORER8_MEMBASE_REG: {
      S390_LONG(code, stdy, std, ins->sreg1, 0, ins->inst_destbasereg,
                ins->inst_offset);
    } break;
    case OP_LOADR8_MEMBASE: {
      S390_LONG(code, ldy, ld, ins->dreg, 0, ins->inst_basereg,
                ins->inst_offset);
    } break;
    case OP_STORER4_MEMBASE_REG: {
      if (cfg->r4fp) {
        S390_LONG(code, stey, ste, ins->sreg1, 0, ins->inst_destbasereg,
                  ins->inst_offset);
      } else {
        s390_lgdr(code, s390_r0, s390_f15);
        s390_ledbr(code, s390_f15, ins->sreg1);
        S390_LONG(code, stey, ste, s390_f15, 0, ins->inst_destbasereg,
                  ins->inst_offset);
        s390_ldgr(code, s390_f15, s390_r0);
      }
    } break;
    case OP_LOADR4_MEMBASE: {
      if (cfg->r4fp) {
        S390_LONG(code, ley, le, ins->dreg, 0, ins->inst_basereg,
                  ins->inst_offset);
      } else {
        S390_LONG(code, ley, le, ins->dreg, 0, ins->inst_basereg,
                  ins->inst_offset);
    ...
    case OP_S390_MOVE: {
      if (ins->backend.size > 0) {
        if (ins->backend.size <= 256) {
          s390_mvc(code, ins->backend.size, ins->dreg, ins->inst_offset,
                   ins->sreg1, ins->inst_imm);
        } else {
          s390_lgr(code, s390_r0, ins->dreg);
    ...
    case OP_ATOMIC_ADD_I8: {
      if (mono_hwcap_s390x_has_ia) {
        s390_laag(code, ins->dreg, ins->sreg2, ins->inst_basereg,
                  ins->inst_offset);
    ...
    case OP_ATOMIC_ADD_I4: {
      if (mono_hwcap_s390x_has_ia) {
        s390_laa(code, ins->dreg, ins->sreg2, ins->inst_basereg,
                 ins->inst_offset);
    ...
    case OP_INSERTX_R4_SLOW:
      ...
      case 0:
        ...
        break;
      case 1:
        s390x_pshufd_imm(code, ins->dreg, ins->dreg,
    ...
    case OP_PREFETCH_MEMBASE:
      s390x_prefetch_reg_membase(code, ins->backend.arg_info, ins->sreg1,
                                 ins->inst_offset);
      break;
...
guint8 *mono_arch_emit_prolog(MonoCompile *cfg) {
  ...
  guint32 size;
  CallInfo *cinfo;
  int argsClobbered = 0, lmfOffset, fpOffset = 0;
  ...
  if (method->save_lmf) {
    ...
    for (i = 0; i < 5; i++)
      mini_gc_set_slot_type_from_fp(
          cfg,
          lmfOffset + MONO_STRUCT_OFFSET(MonoLMF, pregs) + i * sizeof(gulong),
          SLOT_NOREF);
    ...
    /*---------------------------------------------------------------*/
    s390_stg(code, s390_r2, 0, s390_r13, MONO_STRUCT_OFFSET(MonoLMF, lmf_addr));
    mini_gc_set_slot_type_from_fp(
        cfg, lmfOffset + MONO_STRUCT_OFFSET(MonoLMF, lmf_addr), SLOT_NOREF);
    ...
    mini_gc_set_slot_type_from_fp(
        cfg, lmfOffset + MONO_STRUCT_OFFSET(MonoLMF, previous_lmf), SLOT_NOREF);

    /*---------------------------------------------------------------*/
    /* save method info						 */
    /*---------------------------------------------------------------*/
    S390_SET(code, s390_r1, method);
    s390_stg(code, s390_r1, 0, s390_r13, MONO_STRUCT_OFFSET(MonoLMF, method));
    mini_gc_set_slot_type_from_fp(
        cfg, lmfOffset + MONO_STRUCT_OFFSET(MonoLMF, method), SLOT_NOREF);
    ...
    mini_gc_set_slot_type_from_fp(
        cfg, lmfOffset + MONO_STRUCT_OFFSET(MonoLMF, ebp), SLOT_NOREF);
    mini_gc_set_slot_type_from_fp(
        cfg, lmfOffset + MONO_STRUCT_OFFSET(MonoLMF, eip), SLOT_NOREF);
    ...
    for (i = 0; i < 11; i++)
      mini_gc_set_slot_type_from_fp(
          cfg,
          lmfOffset + MONO_STRUCT_OFFSET(MonoLMF, gregs) + i * sizeof(gulong),
          SLOT_NOREF);
  ...
  if (!argsClobbered) {
    ...
    for (i = 0; next && i < sig->param_count + sig->hasthis; ++i) {
      ...
      if (inst->opcode != OP_REGVAR) {
        ...
        switch (ainfo->regtype) {
        case RegTypeGeneral:
          if (next->opcode == OP_MOVE && next->sreg1 == inst->dreg &&
              next->dreg == ainfo->reg) {
            NULLIFY_INS(next);
            match = TRUE;
          }
...
void mono_arch_emit_exceptions(MonoCompile *cfg) {
  MonoJumpInfo *patch_info;
  guint8 *code;
  int nThrows = 0, exc_count = 0, iExc;
  guint32 code_size;
  ...
  for (patch_info = cfg->patch_info; patch_info;
    ...
    case MONO_PATCH_INFO_EXC: {
      ...
      if (iExc < nThrows) {
        s390_jcl(code, S390_CC_UN, (guint64)exc_throw_start[iExc]);
        ...
        /*---------------------------------------------*/
        s390_larl(code, s390_r14,
                  (guint64)S390_RELATIVE(
                      (patch_info->ip.i + cfg->native_code + 8), code));
        /*---------------------------------------------*/
...
static guint8 *get_delegate_invoke_impl(MonoTrampInfo **info,
  ...
  if (has_target) {
    ...
    s390_lg(code, s390_r1, 0, s390_r2,
            MONO_STRUCT_OFFSET(MonoDelegate, method_ptr));
    s390_lg(code, s390_r2, 0, s390_r2,
...
gpointer mono_arch_get_delegate_virtual_invoke_impl(MonoMethodSignature *sig,
   ...
   */
  if (load_imt_reg) {
    s390_lg(code, MONO_ARCH_IMT_REG, 0, s390_r1,
...
gpointer mono_arch_build_imt_trampoline(MonoVTable *vtable, MonoDomain *domain,
    ...
    if (item->is_equals) {
      if (item->check_target_idx) {
        ...
        if (item->has_target_code)
          ...
          item->chunk_size += BR_SIZE + JUMP_SIZE + LOADCON_SIZE + LOAD_SIZE;
      } else {
        if (fail_tramp) {
          item->chunk_size +=
              CMP_SIZE + 2 * BR_SIZE + JUMP_SIZE + 2 * LOADCON_SIZE;
          if (!item->has_target_code)
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (item->jmp_code) {
      if (item->check_target_idx) {
        gint64 offset;
        offset = (gint64)S390_RELATIVE(
            imt_entries[item->check_target_idx]->code_target, item->jmp_code);
  ...
  snprintf(trampName, sizeof(trampName), "%d_imt_trampoline",
           domain->domain_id);
  mono_tramp_info_register(
      mono_tramp_info_create(trampName, start, code - start, NULL, NULL),
      domain);
...
void mono_arch_skip_breakpoint(MonoContext *ctx, MonoJitInfo *ji) {
  MONO_CONTEXT_SET_IP(
      ctx, ((guint8 *)MONO_CONTEXT_GET_IP(ctx) + sizeof(RXY_Format)));
}
...
void mono_arch_skip_single_step(MonoContext *ctx) {
  MONO_CONTEXT_SET_IP(ctx,
                      (guint8 *)MONO_CONTEXT_GET_IP(ctx) + BREAKPOINT_SIZE);
}
...
guint32 mono_arch_cpu_enumerate_simd_versions(void) {
  ...
  if (mono_hwcap_s390x_has_vec)
    sseOpts = (SIMD_VERSION_SSE1 | SIMD_VERSION_SSE2 | SIMD_VERSION_SSE3 |
               SIMD_VERSION_SSSE3 | SIMD_VERSION_SSE41 | SIMD_VERSION_SSE42 |
               SIMD_VERSION_SSE4a);
...
gboolean mono_arch_tailcall_supported(MonoCompile *cfg,
  ...
  gboolean res =
      ...
      IS_SUPPORTED_TAILCALL(callee_info->struct_ret ==
                            caller_info->struct_ret) &&
