 ...
 */
typedef struct {
  guint stack_size, code_size, parm_size, retStruct;
} size_data;
...
static void __inline__ emit_new_move(MonoCompile *cfg, int dr, MonoInst *ins,
                                     MonoInst *src) {
  MonoCallInst *call = (MonoCallInst *)ins->inst_p0;
  ArgInfo *ainfo = (ArgInfo *)ins->inst_p1;
  MonoInst *vtcopy =
      mono_compile_create_var(cfg, m_class_get_byval_arg(src->klass), OP_LOCAL);
  MonoInst *load;
  MonoInst *move;
  int size;
  ...
  if (dr != 0)
    ...
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, ainfo->reg,
 ...
 *
 * Emit the output of structures for calls whose address is placed in a
 * register.
 */

static void __inline__ emit_outarg_vtr(MonoCompile *cfg, MonoInst *ins,
                                       MonoInst *src) {
  MonoCallInst *call = (MonoCallInst *)ins->inst_p0;
  ArgInfo *ainfo = (ArgInfo *)ins->inst_p1;
  ...
  case 0:
    ...
    break;
  case 1:
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, reg, src->dreg, 0);
    break;
  case 2:
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU2_MEMBASE, reg, src->dreg, 0);
    break;
  case 4:
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI4_MEMBASE, reg, src->dreg, 0);
    break;
  case 8:
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI8_MEMBASE, reg, src->dreg, 0);
    break;
  default:
...
static void __inline__ emit_outarg_vts(MonoCompile *cfg, MonoInst *ins,
                                       MonoInst *src) {
  ArgInfo *ainfo = (ArgInfo *)ins->inst_p1;
  ...
  case 0:
    ...
    break;
  case 1:
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, tmpr, src->dreg, 0);
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, ainfo->reg,
                                ainfo->offset, tmpr);
    break;
  case 2:
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU2_MEMBASE, tmpr, src->dreg, 0);
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, ainfo->reg,
                                ainfo->offset, tmpr);
    break;
  case 4:
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI4_MEMBASE, tmpr, src->dreg, 0);
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, ainfo->reg,
                                ainfo->offset, tmpr);
    break;
  case 8:
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI8_MEMBASE, tmpr, src->dreg, 0);
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, ainfo->reg,
                                ainfo->offset, tmpr);
    break;
  default: {
...
enum_retvalue:
  ...
  for (i = pstart; i < sig->param_count; ++i) {
    ...
    case MONO_TYPE_TYPEDBYREF: {
      add_stackParm(&gr, sz, cinfo->args + nParm, sizeof(uintptr_t),
                    RegTypeStructByAddr);
      nParm++;
    } break;
    default:
 ...
 *
 * Emit instructions to convert a single precision floating point value to an
 * integer
 */
  ...
  MONO_BB_FOR_EACH_INS(bb, ins) {
    ...
    case OP_S390_MOVE: {
      if (ins->backend.size > 0) {
        if (ins->backend.size <= 256) {
          s390_mvc(code, ins->backend.size, ins->sreg2, ins->inst_offset,
    ...
    case OP_ATOMIC_ADD_I8: {
      if (mono_hwcap_s390x_has_ia) {
        s390_laag(code, s390_r0, ins->sreg2, ins->inst_basereg,
                  ins->inst_offset);
    ...
    case OP_ATOMIC_ADD_I4: {
      if (mono_hwcap_s390x_has_ia) {
        s390_laa(code, s390_r0, ins->sreg2, ins->inst_basereg,
                 ins->inst_offset);
...
// Tests:set_ip
    if ((strcmp(method->klass->name_space,"") == 0) &&
        ...
        printf("SIGNATURE: %s\n",mono_signature_get_desc(method->signature, FALSE));
        fflush(stdout);
        s390_j (code, 0);
    }
#endif
  ...
  if (has_target) {
    ...
    for (i = 0; i < sig->param_count; ++i) {
      ...
      case RegTypeGeneral:
        ...
        break;
      default:
        s390_mvc(code, sizeof(uintptr_t), STK_BASE, offset, STK_BASE,
 ...
 *
 * Return whether the machine state in sigctx corresponds to a single step
 * event. On s390x we use soft breakpoints so return FALSE
 */
...
gboolean mono_arch_tailcall_supported(MonoCompile *cfg,
  ...
  // Any call that would result in parameters being placed on the stack cannot
  // be "tailed" as it may result in the callers parameter variables being
  // overwritten.
