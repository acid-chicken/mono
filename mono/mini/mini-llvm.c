...
static LLVMValueRef get_aotconst_typed_module(MonoLLVMModule *module,
                                              LLVMBuilderRef builder,
...
static LLVMValueRef get_aotconst_typed(EmitContext *ctx, MonoJumpInfoType type,
                                       gconstpointer data,
                                       LLVMTypeRef llvm_type) {
  MonoCompile *cfg;
  guint32 got_offset;
...
static LLVMValueRef get_callee_llvmonly(EmitContext *ctx, LLVMTypeRef llvm_sig,
  ...
  if (type == MONO_PATCH_INFO_METHOD) {
    ...
    if (m_class_get_image(method->klass)->assembly == ctx->module->assembly) {
      ...
      got_entry_addr =
          LLVMBuildGEP(builder, ctx->module->got_var, indexes, 2, "");
...
static LLVMValueRef emit_init_icall_wrapper(MonoLLVMModule *module,
  ...
  LLVMModuleRef lmodule = module->lmodule;
  LLVMValueRef func, indexes[2], got_entry_addr, args[16], callee, inited_var,
      cmp;
  ...
  case AOT_INIT_METHOD:
    ...
    sig = LLVMFunctionType3(LLVMVoidType(), IntPtrType(), IntPtrType(),
                            LLVMInt32Type(), FALSE);
    break;
  case AOT_INIT_METHOD_GSHARED_MRGCTX:
    icall_id =
        MONO_JIT_ICALL_mini_llvm_init_gshared_method_mrgctx; // Deliberate
                                                             // fall-through
  case AOT_INIT_METHOD_GSHARED_VTABLE:
    ...
    func = LLVMAddFunction(lmodule, name,
                           LLVMFunctionType2(LLVMVoidType(), LLVMInt32Type(),
                                             IntPtrType(), FALSE));
    sig = LLVMFunctionType4(LLVMVoidType(), IntPtrType(), IntPtrType(),
  ...
  args[1] = LLVMBuildPtrToInt(
      builder, LLVMBuildLoad(builder, got_entry_addr, ""), IntPtrType(), "");
...
static LLVMValueRef emit_icall_cold_wrapper(MonoLLVMModule *module,
  ...
  if (aot) {
    callee = get_aotconst_typed_module(
        module, builder, MONO_PATCH_INFO_JIT_ICALL_ID,
        GUINT_TO_POINTER(icall_id), LLVMPointerType(sig, 0));
  } else {
...
static void emit_gc_safepoint_poll(MonoLLVMModule *module,
  ...
  if (is_aot) {
    poll_val_ptr = get_aotconst_typed_module(
        module, builder, MONO_PATCH_INFO_GC_SAFE_POINT_FLAG, NULL, ptr_type);
  } else {
    ...
    case OP_AOTCONST: {
      ...
      if (ji->type == MONO_PATCH_INFO_ICALL_ADDR) {
        char *symbol = mono_aot_get_direct_call_symbol(
            MONO_PATCH_INFO_ICALL_ADDR_CALL, ji->data.target);
        if (symbol) {
          /*
      ...
      // mono_add_patch_info (cfg, 0, (MonoJumpInfoType)ins->inst_i1,
      // ins->inst_p0);
      got_offset = compute_aot_got_offset(ctx->module, cfg->patch_info, NULL);

      ctx->module->max_got_offset =
          MAX(ctx->module->max_got_offset, got_offset);
      if (!mono_aot_is_shared_got_offset(got_offset)) {
        // mono_print_ji (ji);
        // printf ("\n");
        ctx->cfg->got_access_count++;
      ...
      g_free(name);
      /* Can't use this in llvmonly mode since the got slots are initialized by
       * the methods themselves */
      if (!cfg->llvm_only || mono_aot_is_shared_got_offset(got_offset)) {
        /* Can't use this in llvmonly mode since the got slots are initialized
