...
#define ARM64_ATOMIC_FENCE_FIX
    ...
    case OP_XEXTRACT_R4: {
      ...
      switch_ins = LLVMBuildSwitch(
          builder, LLVMBuildAnd(builder, rhs, const_int32(0xf), ""), bbs[0], 0);
      for (i = 0; i < nelems; ++i) {
        LLVMAddCase(switch_ins, LLVMConstInt(LLVMInt32Type(), i, FALSE),
                    bbs[i]);
        LLVMPositionBuilderAtEnd(builder, bbs[i]);
        phi_values[i] = LLVMBuildExtractElement(
            builder, lhs, LLVMConstInt(LLVMInt32Type(), i, FALSE), "");
        LLVMBuildBr(builder, cbb);
      }
      ...
      case MONO_TYPE_U2:
        values[ins->dreg] =
            LLVMBuildZExt(ctx->builder, values[ins->dreg], LLVMInt32Type(), "");
        break;
      default:
        break;
      }
