...
static LLVMRealPredicate fpcond_to_llvm_cond[] = {
    LLVMRealOEQ, LLVMRealUNE, LLVMRealOLE, LLVMRealOGE, LLVMRealOLT,
    LLVMRealOGT, LLVMRealULE, LLVMRealUGE, LLVMRealULT, LLVMRealUGT,
};
...
static LLVMValueRef ConstInt32(int v) {
...
static inline G_GNUC_UNUSED LLVMTypeRef type_to_simd_type(int type) {
  switch (type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return LLVMVectorType(LLVMInt8Type(), 16);
 ...
 */
static const char *ovf_op_to_intrins(int opcode) {
  switch (opcode) {
  case OP_IADD_OVF:
...
static const char *simd_op_to_intrins(int opcode) {
  switch (opcode) {
...
static void emit_entry_bb(EmitContext *ctx, LLVMBuilderRef builder) {
  ...
  for (i = 0; i < cfg->num_varinfo; ++i) {
    ...
    if (MONO_TYPE_IS_REFERENCE(var->inst_vtype) ||
  ...
  for (ins = bb->code; ins; ins = ins->next) {
    ...
    case OP_COMPARE_IMM: {
      ...
      if (likely || unlikely) {
        ...
        cmp = LLVMBuildCall(ctx->builder, get_intrins(ctx, INTRINS_EXPECT_I1),
                            args, 2, "");
      }
    ...
    case OP_LOCALLOC_IMM: {
      ...
      if (ins->flags & MONO_INST_INIT)
        emit_memset(ctx, builder, v, ConstInt32(size),
                    MONO_ARCH_FRAME_ALIGNMENT);
    ...
    case OP_SIN: {
      ...
      values[ins->dreg] =
          LLVMBuildCall(builder, get_intrins(ctx, INTRINS_SIN), args, 1, dname);
      break;
    }
    case OP_SINF: {
      LLVMValueRef args[1];
      ...
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_SINF),
                                        args, 1, dname);
      break;
    }
    case OP_EXP: {
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] =
          LLVMBuildCall(builder, get_intrins(ctx, INTRINS_EXP), args, 1, dname);
      break;
    }
    case OP_EXPF: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_EXPF),
                                        args, 1, dname);
      break;
    }
    case OP_LOG2: {
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_LOG2),
                                        args, 1, dname);
      break;
    }
    case OP_LOG2F: {
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_LOG2F), args, 1, dname);
      break;
    }
    case OP_LOG10: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_LOG10), args, 1, dname);
      break;
    }
    case OP_LOG10F: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_LOG10F), args, 1, dname);
      break;
    }
    case OP_LOG: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] =
          LLVMBuildCall(builder, get_intrins(ctx, INTRINS_LOG), args, 1, dname);
      break;
    }
    case OP_TRUNC: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_TRUNC), args, 1, dname);
      break;
    }
    case OP_TRUNCF: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_TRUNCF), args, 1, dname);
      break;
    }
    case OP_COS: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] =
          LLVMBuildCall(builder, get_intrins(ctx, INTRINS_COS), args, 1, dname);
      break;
    }
    case OP_COSF: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_COSF),
                                        args, 1, dname);
      break;
    }
    case OP_SQRT: {
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_SQRT),
                                        args, 1, dname);
      break;
    }
    case OP_SQRTF: {
      ...
      args[0] = convert(ctx, lhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_SQRTF), args, 1, dname);
      break;
    }
    case OP_FLOOR: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_FLOOR), args, 1, dname);
      break;
    }
    case OP_FLOORF: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_FLOORF), args, 1, dname);
      break;
    }
    case OP_CEIL: {
      LLVMValueRef args[1];
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_CEIL),
                                        args, 1, dname);
      break;
    }
    case OP_CEILF: {
      ...
      args[0] = convert(ctx, lhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_CEILF), args, 1, dname);
      break;
    }
    case OP_FMA: {
      LLVMValueRef args[3];
      ...
      values[ins->dreg] =
          LLVMBuildCall(builder, get_intrins(ctx, INTRINS_FMA), args, 3, dname);
      break;
    }
    case OP_FMAF: {
      LLVMValueRef args[3];
      ...
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_FMAF),
                                        args, 3, dname);
      break;
    }
    case OP_ABS: {
      ...
      args[0] = convert(ctx, lhs, LLVMDoubleType());
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_FABS),
                                        args, 1, dname);
      break;
    }
    case OP_ABSF: {
...
#ifdef TARGET_AMD64
      args[0] = convert(ctx, lhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_ABSF),
                                        args, 1, dname);
#else
    ...
    case OP_RPOW: {
      ...
      values[ins->dreg] = LLVMBuildCall(builder, get_intrins(ctx, INTRINS_POWF),
                                        args, 2, dname);
      break;
    }
    case OP_FPOW: {
      ...
      values[ins->dreg] =
          LLVMBuildCall(builder, get_intrins(ctx, INTRINS_POW), args, 2, dname);
      break;
    }
    case OP_FCOPYSIGN: {
      LLVMValueRef args[2];
      ...
      args[1] = convert(ctx, rhs, LLVMDoubleType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_COPYSIGN), args, 2, dname);
      break;
    }
    case OP_RCOPYSIGN: {
      LLVMValueRef args[2];
      ...
      args[1] = convert(ctx, rhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_COPYSIGNF), args, 2, dname);
      break;
    }
...
#define ARM64_ATOMIC_FENCE_FIX
    ...
    case OP_RELAXED_NOP: {
#if defined(TARGET_AMD64) || defined(TARGET_X86)
      emit_call(ctx, bb, &builder,
                get_intrins_by_name(ctx, "llvm.x86.sse2.pause"), NULL, 0);
      break;
#else
      break;
#endif
    ...
    case OP_VZERO: {
      ...
      emit_memset(ctx, builder, ptr,
                  ConstInt32(mono_class_value_size(klass, NULL)), 0);
      break;
    }
    case OP_DUMMY_VZERO:
      break;
    ...
    case OP_PMULW_HIGH_UN: {
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins_by_name(ctx, simd_op_to_intrins(ins->opcode)),
          args, 2, dname);
      break;
    }
    case OP_PAVGB_UN:
    ...
    case OP_FCONV_TO_R8_X: {
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, LLVMConstNull(type_to_simd_type(MONO_TYPE_R8)), lhs,
          LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      break;
    }
    ...
    case OP_FCONV_TO_R4_X: {
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, LLVMConstNull(type_to_simd_type(MONO_TYPE_R4)), lhs,
          LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      break;
    }
    ...
    case OP_SSE_MOVMSK: {
      ...
      if (ins->inst_c1 == MONO_TYPE_R4) {
        args[0] = lhs;
        values[ins->dreg] = LLVMBuildCall(
            builder, get_intrins(ctx, INTRINS_SSE_MOVMSK_PS), args, 1, dname);
      } else if (ins->inst_c1 == MONO_TYPE_R8) {
        args[0] = lhs;
        values[ins->dreg] = LLVMBuildCall(
            builder, get_intrins(ctx, INTRINS_SSE_MOVMSK_PD), args, 1, dname);
      } else {
        args[0] = convert(ctx, lhs, type_to_simd_type(MONO_TYPE_I1));
        values[ins->dreg] = LLVMBuildCall(
            builder, get_intrins(ctx, INTRINS_SSE_PMOVMSKB), args, 1, dname);
      }
      break;
    }
    ...
    case OP_SSE_LOADU: {
      ...
      LLVMValueRef dst_vec = LLVMBuildBitCast(
          builder, dst_ptr, LLVMPointerType(type_to_simd_type(ins->inst_c1), 0),
          "");
    ...
    case OP_SSE_OR: {
      LLVMValueRef vec_lhs_i64 =
          convert(ctx, lhs, type_to_simd_type(MONO_TYPE_I8));
      LLVMValueRef vec_rhs_i64 =
          convert(ctx, rhs, type_to_simd_type(MONO_TYPE_I8));
    ...
    case OP_SSE2_PACKUS: {
      ...
      values[ins->dreg] =
          convert(ctx,
                  LLVMBuildCall(builder, get_intrins(ctx, INTRINS_SSE_PACKUSWB),
                                args, 2, dname),
                  type_to_simd_type(ins->inst_c1));
      break;
    }
    ...
    case OP_SSE2_SRLI: {
      ...
      values[ins->dreg] =
          convert(ctx,
                  LLVMBuildCall(builder, get_intrins(ctx, INTRINS_SSE_PSRLI_W),
                                args, 2, dname),
                  type_to_simd_type(ins->inst_c1));
      break;
    }
    ...
    case OP_SSSE3_SHUFFLE: {
      LLVMValueRef args[] = {lhs, rhs};
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_SSE_PSHUFB), args, 2, dname);
      break;
    }
    ...
    case OP_CREATE_SCALAR_UNSAFE: {
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, LLVMGetUndef(simd_class_to_llvm_type(ctx, ins->klass)),
    ...
    case OP_SSE41_ROUNDSS: {
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_SSE_ROUNDSS), args, 3, dname);
      break;
    }
    ...
    case OP_SSE41_ROUNDPD: {
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_SSE_ROUNDPD), args, 2, dname);
      break;
    }
    ...
    case OP_SSE41_INSERT: {
      if (ins->inst_c1 == MONO_TYPE_R4) {
        ...
        values[ins->dreg] = LLVMBuildCall(
            builder, get_intrins(ctx, INTRINS_SSE_INSERTPS), args, 3, dname);
      } else {
    ...
    case OP_SSE41_PTESTZ: {
      ...
      LLVMValueRef call = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_SSE_PTESTZ), args, 2, dname);
    ...
    case OP_XEQUAL: {
      ...
      if (LLVMIsNull(lhs) || LLVMIsNull(rhs)) {
        ...
        values[ins->dreg] =
            LLVMBuildCall(builder, get_intrins(ctx, intrins), &val, 1, "");
        values[ins->dreg] = LLVMBuildZExt(
            builder,
    ...
    case OP_XBINOP: {
      ...
      case OP_FMIN: {
        ...
        if (ins->inst_c0 == OP_FMAX)
          values[ins->dreg] = LLVMBuildCall(
              builder,
              get_intrins(ctx, is_r4 ? INTRINS_SSE_MAXPS : INTRINS_SSE_MAXPD),
              args, 2, dname);
        else
          values[ins->dreg] = LLVMBuildCall(
              builder,
              get_intrins(ctx, is_r4 ? INTRINS_SSE_MINPS : INTRINS_SSE_MINPD),
              args, 2, dname);
#else
        NOT_IMPLEMENTED;
#endif
    ...
    case OP_POPCNT32:
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_CTPOP_I32), &lhs, 1, "");
      break;
    case OP_POPCNT64:
      values[ins->dreg] = LLVMBuildCall(
          builder, get_intrins(ctx, INTRINS_CTPOP_I64), &lhs, 1, "");
      break;
    case OP_LZCNT32:
    case OP_LZCNT64: {
      LLVMValueRef args[2];
    ...
    case OP_CTTZ64: {
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder,
          get_intrins(ctx, ins->opcode == OP_CTTZ32 ? INTRINS_CTTZ_I32
                                                    : INTRINS_CTTZ_I64),
          args, 2, "");
      break;
    }
    ...
    case OP_BEXTR64: {
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder,
          get_intrins(ctx, ins->opcode == OP_BEXTR32 ? INTRINS_BEXTR_I32
                                                     : INTRINS_BEXTR_I64),
          args, 2, "");
      break;
    }
    ...
    case OP_BZHI64: {
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder,
          get_intrins(ctx, ins->opcode == OP_BZHI32 ? INTRINS_BZHI_I32
                                                    : INTRINS_BZHI_I64),
          args, 2, "");
      break;
    }
    ...
    case OP_PEXT64: {
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder,
          get_intrins(ctx, ins->opcode == OP_PEXT32 ? INTRINS_PEXT_I32
                                                    : INTRINS_PEXT_I64),
          args, 2, "");
      break;
    }
    ...
    case OP_PDEP64: {
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder,
          get_intrins(ctx, ins->opcode == OP_PDEP32 ? INTRINS_PDEP_I32
                                                    : INTRINS_PDEP_I64),
          args, 2, "");
      break;
    }
...
static inline void AddFunc(LLVMModuleRef module, const char *name,
...
static inline void AddFunc1(LLVMModuleRef module, const char *name,
                            LLVMTypeRef ret_type, LLVMTypeRef param_type1) {
  LLVMTypeRef param_types[4];
...
static inline void AddFunc2(LLVMModuleRef module, const char *name,
                            LLVMTypeRef ret_type, LLVMTypeRef param_type1,
...
static void add_sse_binary(LLVMModuleRef module, const char *name, int type) {
...
static void add_intrinsic(LLVMModuleRef module, int id) {
  ...
  name = (const char *)g_hash_table_lookup(intrins_id_to_name,
                                           GINT_TO_POINTER(id));
  g_assert(name);
  ...
  case INTRINS_MEMSET: {
    ...
    LLVMTypeRef params[] = {LLVMPointerType(LLVMInt8Type(), 0), LLVMInt8Type(),
  ...
  case INTRINS_POW:
    AddFunc2(module, name, LLVMDoubleType(), LLVMDoubleType(),
             LLVMDoubleType());
    break;
...
static LLVMValueRef get_intrins_from_module(LLVMModuleRef lmodule, int id) {
  ...
  const char *name = (const char *)g_hash_table_lookup(intrins_id_to_name,
                                                       GINT_TO_POINTER(id));
  g_assert(name);
...
void mono_llvm_init(void) {
  ...
  for (i = 0; i < INTRINS_NUM; ++i)
    g_hash_table_insert(h, GINT_TO_POINTER(intrinsics[i].id),
                        (gpointer)intrinsics[i].name);
  intrins_id_to_name = h;

  h = g_hash_table_new(g_str_hash, g_str_equal);
  for (i = 0; i < INTRINS_NUM; ++i)
    g_hash_table_insert(h, (gpointer)intrinsics[i].name,
                        GINT_TO_POINTER(intrinsics[i].id + 1));
...
void mono_llvm_create_aot_module(MonoAssembly *assembly,
  ...
  module->llvm_disable_self_init =
      llvm_disable_self_init &&
      !llvm_only; // llvm_only implies !llvm_disable_self_init
    ...
    while (g_hash_table_iter_next(&iter, (void **)&ji, (void **)&callee)) {
      if (mono_aot_is_direct_callable(ji)) {
        ...
        if (lmethod && LLVMTypeOf(callee) == LLVMTypeOf(lmethod)) {
          ...
          if (!module->llvm_disable_self_init &&
              mono_aot_can_specialize(ji->data.method))
...
static unsigned char *alloc_cb(LLVMValueRef function, int size) {
  MonoCompile *cfg;
...
static void emitted_cb(LLVMValueRef function, void *start, void *end) {
  MonoCompile *cfg;
...
static void exception_cb(void *data) {
  MonoCompile *cfg;
   ...
   * save it.
   * An alternative would be to save it directly, and modify our unwinder to
   * work with it.
   */
  cfg->encoded_unwind_ops =
      mono_unwind_decode_fde((guint8 *)data, &cfg->encoded_unwind_ops_len, NULL,
                             &ei, &ei_len, &type_info, &this_reg, &this_offset);
  if (cfg->verbose_level > 1)
  ...
  /*
   * For nested clauses, the LLVM produced exception info associates the try
   * interval with the innermost handler, while mono expects it to be associated
   * with all nesting clauses. So add new clauses which use the IL info (catch
   * class etc.) from the nesting clause, and everything else from the nested
   * clause.
   */
  nindex = ei_len;
  for (i = 0; i < ei_len; ++i) {
...
static void init_jit_module(MonoDomain *domain) {
  ...
  module->mono_ee = (MonoEERef *)mono_llvm_create_ee(alloc_cb, emitted_cb,
                                                     exception_cb, &module->ee);
...
static void llvm_jit_finalize_method(EmitContext *ctx) {
  ...
  cfg->native_code = (guint8 *)mono_llvm_compile_method(
      ctx->module->mono_ee, ctx->lmethod, nvars, callee_vars, callee_addrs,
      &eh_frame);
