...
void mono_llvm_create_aot_module(MonoAssembly *assembly,
  ...
  /* Add GOT */
  /*
   * We couldn't compute the type of the LLVM global representing the got
   * because its size is only known after all the methods have been emitted. So
   * create a dummy variable, and replace all uses it with the real got variable
   * when its size is known in mono_llvm_emit_aot_module ().
   */
  {
    LLVMTypeRef got_type = LLVMArrayType(module->ptr_type, 0);

    module->got_var =
        LLVMAddGlobal(module->lmodule, got_type, "mono_dummy_got");
...
void mono_llvm_fixup_aot_module(void) {
  ...
  for (int sindex = 0; sindex < module->callsite_list->len; ++sindex) {
    ...
    if (can_direct_call) {
      ...
      name =
          get_aotconst_name(site->ji->type, site->ji->data.target, got_offset);
      load = LLVMBuildLoad(builder, got_entry_addr, "");
...
void mono_llvm_emit_aot_module(const char *filename, const char *cu_name) {
  ...
  /*
   * Create the real got variable and replace all uses of the dummy variable
   * with the real one.
   */
  ...
  for (int i = 0; i < size; i++) {
    ...
    lookup_type = (LLVMTypeRef)g_hash_table_lookup(module->got_idx_to_type,
                                                   GINT_TO_POINTER(i));
  ...
  got_type = LLVMStructCreateNamed(module->context,
