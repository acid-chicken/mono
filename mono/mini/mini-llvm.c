...
typedef struct {
  ...
  LLVMValueRef init_method, init_method_gshared_mrgctx,
      init_method_gshared_this, init_method_gshared_vtable;
...
typedef struct {
  ...
  /*
   * If this bblock is the start of a finally clause, this is a list of bblocks
   * it needs to branch to in ENDFINALLY.
   */
...
static LLVMIntPredicate cond_to_llvm_cond[] = {
    LLVMIntEQ,  LLVMIntNE,  LLVMIntSLE, LLVMIntSGE, LLVMIntSLT,
    LLVMIntSGT, LLVMIntULE, LLVMIntUGE, LLVMIntULT, LLVMIntUGT,
};

static LLVMRealPredicate fpcond_to_llvm_cond[] = {
    LLVMRealOEQ, LLVMRealUNE, LLVMRealOLE, LLVMRealOGE,
    LLVMRealOLT, LLVMRealOGT, LLVMRealULE, LLVMRealUGE,
    LLVMRealULT, LLVMRealUGT, LLVMRealORD, LLVMRealUNO};
...
static void emit_dbg_loc(EmitContext *ctx, LLVMBuilderRef builder,
                         const unsigned char *cil_code);
static void emit_default_dbg_loc(EmitContext *ctx, LLVMBuilderRef builder);
static LLVMValueRef emit_dbg_subprogram(EmitContext *ctx, MonoCompile *cfg,
                                        LLVMValueRef method, const char *name);
static void emit_dbg_info(MonoLLVMModule *module, const char *filename,
                          const char *cu_name);
static void emit_cond_system_exception(EmitContext *ctx, MonoBasicBlock *bb,
                                       const char *exc_type, LLVMValueRef cmp,
...
static void mono_llvm_nonnull_state_update(EmitContext *ctx, LLVMValueRef lcall,
                                           MonoMethod *call_method,
                                           LLVMValueRef *args, int num_params);
static void mono_llvm_propagate_nonnull_final(GHashTable *all_specializable,
                                              MonoLLVMModule *module);
...
static inline void set_failure(EmitContext *ctx, const char *message) {
  TRACE_FAILURE(reason);
  ctx->cfg->exception_message = g_strdup(message);
  ctx->cfg->disable_llvm = TRUE;
}

static LLVMValueRef const_int32(int v) {
 ...
 */
static LLVMTypeRef IntPtrType(void) {
  return TARGET_SIZEOF_VOID_P == 8 ? LLVMInt64Type() : LLVMInt32Type();
}

static LLVMTypeRef ObjRefType(void) {
  return TARGET_SIZEOF_VOID_P == 8 ? LLVMPointerType(LLVMInt64Type(), 0)
                                   : LLVMPointerType(LLVMInt32Type(), 0);
}

static LLVMTypeRef ThisType(void) {
  return TARGET_SIZEOF_VOID_P == 8 ? LLVMPointerType(LLVMInt64Type(), 0)
                                   : LLVMPointerType(LLVMInt32Type(), 0);
}
 ...
 */
static guint32 get_vtype_size(MonoType *t) {
  int size;
 ...
 */
static LLVMTypeRef simd_class_to_llvm_type(EmitContext *ctx, MonoClass *klass) {
...
static inline G_GNUC_UNUSED LLVMTypeRef type_to_sse_type(int type) {
  switch (type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
    return LLVMVectorType(LLVMInt8Type(), 16);
...
static LLVMTypeRef create_llvm_type_for_type(MonoLLVMModule *module,
                                             MonoClass *klass) {
...
static MonoTypeEnum inst_c1_type(const MonoInst *ins) {
...
static gboolean primitive_type_is_unsigned(MonoTypeEnum t) {
  switch (t) {
  case MONO_TYPE_U1:
  case MONO_TYPE_U2:
  case MONO_TYPE_CHAR:
 ...
 */
static gboolean type_is_unsigned(EmitContext *ctx, MonoType *t) {
  t = mini_get_underlying_type(t);
  if (t->byref)
    return FALSE;
 ...
 */
static LLVMTypeRef type_to_llvm_arg_type(EmitContext *ctx, MonoType *t) {
 ...
 *
 *   Return the LLVM type which needs to be used when a value of type TYPE is
 * pushed on the IL stack.
 */
static G_GNUC_UNUSED LLVMTypeRef llvm_type_to_stack_type(MonoCompile *cfg,
                                                         LLVMTypeRef type) {
  if (type == NULL)
    return NULL;
 ...
 * descriptions.
 */
static LLVMTypeRef regtype_to_llvm_type(char c) {
  switch (c) {
  case 'i':
    return LLVMInt32Type();
  case 'l':
 ...
 */
static LLVMTypeRef op_to_llvm_type(int opcode) {
  switch (opcode) {
  case OP_ICONV_TO_I1:
  case OP_LCONV_TO_I1:
    return LLVMInt8Type();
 ...
 * OPCODE.
 */
static LLVMTypeRef load_store_to_llvm_type(int opcode, int *size,
                                           gboolean *sext, gboolean *zext) {
  *sext = FALSE;
  *zext = FALSE;
 ...
 */
static IntrinsicId ovf_op_to_intrins(int opcode) {
  switch (opcode) {
  case OP_IADD_OVF:
...
static IntrinsicId simd_ins_to_intrins(int opcode) {
  switch (opcode) {
...
static LLVMTypeRef simd_op_to_llvm_type(int opcode) {
#if defined(TARGET_X86) || defined(TARGET_AMD64)
  switch (opcode) {
  case OP_EXTRACT_R8:
  case OP_EXPAND_R8:
...
static void set_cold_cconv(LLVMValueRef func) {
  /*
   * xcode10 (watchOS) and ARM/ARM64 doesn't seem to support preserveall, it
   * fails with: fatal error: error in backend: Unsupported calling convention
   */
...
static void set_call_cold_cconv(LLVMValueRef func) {
#if !defined(TARGET_WATCHOS) && !defined(TARGET_ARM) && !defined(TARGET_ARM64)
 ...
 */
static LLVMBasicBlockRef get_bb(EmitContext *ctx, MonoBasicBlock *bb) {
  ...
  if (ctx->bblocks[bb->block_num].bblock == NULL) {
    if (bb->flags & BB_EXCEPTION_HANDLER) {
      int clause_index =
          (mono_get_block_region_notry(ctx->cfg, bb->region) >> 8) - 1;
    ...
    ctx->bblocks[bb->block_num].bblock =
        LLVMAppendBasicBlock(ctx->lmethod, bb_name);
 ...
 * This might not be equal to the bb returned by get_bb () since we need to
 * generate multiple LLVM bblocks for a mono bblock to handle throwing
 * exceptions.
 */
static LLVMBasicBlockRef get_end_bb(EmitContext *ctx, MonoBasicBlock *bb) {
  get_bb(ctx, bb);
  return ctx->bblocks[bb->block_num].end_bblock;
}

static LLVMBasicBlockRef gen_bb(EmitContext *ctx, const char *prefix) {
  char bb_name[128];
 ...
 */
static gpointer resolve_patch(MonoCompile *cfg, MonoJumpInfoType type,
  ...
  res = mono_resolve_patch_target(cfg->method, cfg->domain, NULL, &ji, FALSE,
                                  error);
 ...
 */
static LLVMValueRef convert_full(EmitContext *ctx, LLVMValueRef v,
                                 LLVMTypeRef dtype, gboolean is_unsigned) {
  ...
  if (stype != dtype) {
    ...
    /* Extend */
    if (dtype == LLVMInt64Type() &&
        (stype == LLVMInt32Type() || stype == LLVMInt16Type() ||
         stype == LLVMInt8Type()))
      ext = TRUE;
    else if (dtype == LLVMInt32Type() &&
             (stype == LLVMInt16Type() || stype == LLVMInt8Type()))
      ext = TRUE;
    else if (dtype == LLVMInt16Type() && (stype == LLVMInt8Type()))
      ext = TRUE;

    if (ext)
      return is_unsigned ? LLVMBuildZExt(ctx->builder, v, dtype, "")
    ...
    /* Trunc */
    if (stype == LLVMInt64Type() &&
        (dtype == LLVMInt32Type() || dtype == LLVMInt16Type() ||
         dtype == LLVMInt8Type()))
      return LLVMBuildTrunc(ctx->builder, v, dtype, "");
    if (stype == LLVMInt32Type() &&
        (dtype == LLVMInt16Type() || dtype == LLVMInt8Type()))
      return LLVMBuildTrunc(ctx->builder, v, dtype, "");
    ...
    if (LLVMGetTypeKind(stype) == LLVMPointerTypeKind &&
        LLVMGetTypeKind(dtype) == LLVMPointerTypeKind)
    ...
    if (mono_arch_is_soft_float()) {
      ...
      if (stype == LLVMInt32Type() && dtype == LLVMDoubleType())
        return LLVMBuildBitCast(
            ctx->builder, LLVMBuildZExt(ctx->builder, v, LLVMInt64Type(), ""),
            dtype, "");
    }

    if (LLVMGetTypeKind(stype) == LLVMVectorTypeKind &&
        LLVMGetTypeKind(dtype) == LLVMVectorTypeKind)
      return LLVMBuildBitCast(ctx->builder, v, dtype, "");
...
static LLVMValueRef convert(EmitContext *ctx, LLVMValueRef v,
                            LLVMTypeRef dtype) {
  return convert_full(ctx, v, dtype, FALSE);
}

static void emit_memset(EmitContext *ctx, LLVMBuilderRef builder,
                        LLVMValueRef v, LLVMValueRef size, int alignment) {
  LLVMValueRef args[5];
  int aindex = 0;
 ...
 */
static LLVMValueRef emit_volatile_load(EmitContext *ctx, int vreg) {
  MonoType *t;
  LLVMValueRef v;
  ...
  if (t && !t->byref) {
    ...
    if (t->type == MONO_TYPE_U1 || t->type == MONO_TYPE_U2 ||
        t->type == MONO_TYPE_CHAR || t->type == MONO_TYPE_BOOLEAN)
 ...
 */
static void emit_volatile_store(EmitContext *ctx, int vreg) {
  MonoInst *var = get_vreg_to_inst(ctx->cfg, vreg);

  if (var && var->flags & (MONO_INST_VOLATILE | MONO_INST_INDIRECT)) {
    g_assert(ctx->addresses[vreg]);
    LLVMBuildStore(ctx->builder,
                   convert(ctx, ctx->values[vreg],
                           type_to_llvm_type(ctx, var->inst_vtype)),
                   ctx->addresses[vreg]);
  }
}

static LLVMTypeRef sig_to_llvm_sig_no_cinfo(EmitContext *ctx,
                                            MonoMethodSignature *sig) {
 ...
 * calling convention information in CINFO. Fill out the parameter mapping
 * information in CINFO.
 */
static LLVMTypeRef sig_to_llvm_sig_full(EmitContext *ctx,
                                        MonoMethodSignature *sig,
                                        LLVMCallInfo *cinfo) {
  LLVMTypeRef ret_type;
  LLVMTypeRef *param_types = NULL;
  ...
  case LLVMArgVtypeInReg:
    ...
    if (cinfo->ret.pair_storage[0] == LLVMArgInIReg &&
        cinfo->ret.pair_storage[1] == LLVMArgNone) {
      LLVMTypeRef members[2];

      members[0] = IntPtrType();
      ret_type = LLVMStructType(members, 1, FALSE);
    } else if (cinfo->ret.pair_storage[0] == LLVMArgNone &&
               cinfo->ret.pair_storage[1] == LLVMArgNone) {
      /* Empty struct */
      ret_type = LLVMVoidType();
    } else if (cinfo->ret.pair_storage[0] == LLVMArgInIReg &&
               cinfo->ret.pair_storage[1] == LLVMArgInIReg) {
      LLVMTypeRef members[2];
  ...
  case LLVMArgVtypeAsScalar: {
    int size =
        mono_class_value_size(mono_class_from_mono_type_internal(rtype), NULL);
  ...
  case LLVMArgFpStruct: {
    ...
    /* Have to create our own structure since we don't map fp structures to LLVM
     * fp structures yet */
  ...
  if (vretaddr) {
    /* Compute the index in the LLVM signature where the vret arg needs to be
     * passed */
  ...
  for (i = 0; i < sig->param_count; ++i) {
    ...
    case LLVMArgAsFpArgs: {
      ...
      for (j = 0; j < ainfo->nslots; ++j)
        param_types[pindex++] =
            ainfo->esize == 8 ? LLVMDoubleType() : LLVMFloatType();
      break;
    }
    case LLVMArgVtypeAsScalar:
      g_assert_not_reached();
    ...
    case LLVMArgGsharedvtFixedVtype:
      param_types[pindex++] =
          LLVMPointerType(type_to_llvm_arg_type(ctx, ainfo->type), 0);
      break;
    case LLVMArgGsharedvtVariable:
...
static LLVMTypeRef sig_to_llvm_sig(EmitContext *ctx, MonoMethodSignature *sig) {
 ...
 */
static G_GNUC_UNUSED LLVMTypeRef LLVMFunctionType0(LLVMTypeRef ReturnType,
                                                   int IsVarArg) {
 ...
 *   Create an LLVM function type from the arguments.
 */
static G_GNUC_UNUSED LLVMTypeRef LLVMFunctionType1(LLVMTypeRef ReturnType,
 ...
 *   Create an LLVM function type from the arguments.
 */
static G_GNUC_UNUSED LLVMTypeRef LLVMFunctionType2(LLVMTypeRef ReturnType,
                                                   LLVMTypeRef ParamType1,
 ...
 *   Create an LLVM function type from the arguments.
 */
static G_GNUC_UNUSED LLVMTypeRef LLVMFunctionType3(LLVMTypeRef ReturnType,
                                                   LLVMTypeRef ParamType1,
                                                   LLVMTypeRef ParamType2,
...
static G_GNUC_UNUSED LLVMTypeRef LLVMFunctionType4(
    LLVMTypeRef ReturnType, LLVMTypeRef ParamType1, LLVMTypeRef ParamType2,
    LLVMTypeRef ParamType3, LLVMTypeRef ParamType4, int IsVarArg) {
...
static G_GNUC_UNUSED LLVMTypeRef LLVMFunctionType5(
    LLVMTypeRef ReturnType, LLVMTypeRef ParamType1, LLVMTypeRef ParamType2,
    LLVMTypeRef ParamType3, LLVMTypeRef ParamType4, LLVMTypeRef ParamType5,
    int IsVarArg) {
 ...
 */
static LLVMBuilderRef create_builder(EmitContext *ctx) {
  LLVMBuilderRef builder = LLVMCreateBuilder();
  if (mono_use_fast_math)
    mono_llvm_set_fast_math(builder);

  ctx->builders =
      g_slist_prepend_mempool(ctx->cfg->mempool, ctx->builders, builder);
...
static char *get_aotconst_name(MonoJumpInfoType type, gconstpointer data,
                               int got_offset) {
  char *name;
  int len;
  ...
  case MONO_PATCH_INFO_JIT_ICALL_ID:
    name = g_strdup_printf(
        "jit_icall_%s",
        mono_find_jit_icall_info((MonoJitICallId)(gsize)data)->name);
    break;
  case MONO_PATCH_INFO_RGCTX_SLOT_INDEX: {
    MonoJumpInfoRgctxEntry *entry = (MonoJumpInfoRgctxEntry *)data;
    name = g_strdup_printf("rgctx_slot_index_%s",
                           mono_rgctx_info_type_to_str(entry->info_type));
    break;
  }
...
static int compute_aot_got_offset(MonoLLVMModule *module, MonoJumpInfo *ji,
                                  LLVMTypeRef llvm_type) {
  guint32 got_offset = mono_aot_get_got_offset(ji);

  LLVMTypeRef lookup_type = (LLVMTypeRef)g_hash_table_lookup(
      module->got_idx_to_type, GINT_TO_POINTER(got_offset));
  ...
  g_hash_table_insert(module->got_idx_to_type, GINT_TO_POINTER(got_offset),
                      lookup_type);
  return got_offset;
}
...
static LLVMValueRef
get_aotconst_module(MonoLLVMModule *module, LLVMBuilderRef builder,
                    MonoJumpInfoType type, gconstpointer data,
                    LLVMTypeRef llvm_type, guint32 *out_got_offset,
                    MonoJumpInfo **out_ji) {
  guint32 got_offset;
  LLVMValueRef load;
  ...
  LLVMValueRef const_var =
      g_hash_table_lookup(module->aotconst_vars, GINT_TO_POINTER(got_offset));
  if (!const_var) {
...
static LLVMValueRef get_aotconst(EmitContext *ctx, MonoJumpInfoType type,
                                 gconstpointer data, LLVMTypeRef llvm_type) {
  MonoCompile *cfg;
  guint32 got_offset;
  MonoJumpInfo *ji;
  LLVMValueRef load;
  ...
  load = get_aotconst_module(ctx->module, ctx->builder, type, data, llvm_type,
                             &got_offset, &ji);
  ...
  /*
   * If the got slot is shared, it means its initialized when the aot image is
   * loaded, so we don't need to explicitly initialize it.
   */
  if (!mono_aot_is_shared_got_offset(got_offset)) {
    // mono_print_ji (ji);
    // printf ("\n");
...
static LLVMValueRef get_dummy_aotconst(EmitContext *ctx,
                                       LLVMTypeRef llvm_type) {
  ...
  got_entry_addr =
      LLVMBuildGEP(builder, ctx->module->dummy_got_var, indexes, 2, "");
...
static LLVMValueRef get_callee_llvmonly(EmitContext *ctx, LLVMTypeRef llvm_sig,
                                        MonoJumpInfoType type,
                                        gconstpointer data) {
  LLVMValueRef callee;
  char *callee_name = NULL;

  if (ctx->module->static_link &&
    ...
    if (type == MONO_PATCH_INFO_JIT_ICALL_ID) {
      ...
      if (m_class_get_image(method->klass) != ctx->module->assembly->image &&
  ...
  if (callee_name) {
    /* Directly callable */
    // FIXME: Locking
    callee = (LLVMValueRef)g_hash_table_lookup(ctx->module->direct_callables,
                                               callee_name);
    if (!callee) {
      ...
      g_hash_table_insert(ctx->module->direct_callables, (char *)callee_name,
                          callee);
    } else {
  ...
  /*
   * Change references to icalls/pinvokes/jit icalls to their wrappers when in
   * corlib, so they can be called directly.
   */
  ...
  /*
   * Instead of emitting an indirect call through a got slot, emit a
   * placeholder, and replace it with a direct call or an indirect call in
   * mono_llvm_fixup_aot_module () after all methods have been emitted.
   */
 ...
 */
static LLVMValueRef get_callee(EmitContext *ctx, LLVMTypeRef llvm_sig,
                               MonoJumpInfoType type, gconstpointer data) {
  LLVMValueRef callee;
  char *callee_name;
  ...
  if (type == MONO_PATCH_INFO_METHOD) {
    ...
    if (m_class_get_image(cmethod->klass) != ctx->module->assembly->image) {
      ...
      if (mono_aot_is_direct_callable(&tmp_ji)) {
        ...
        callee = (LLVMValueRef)g_hash_table_lookup(
            ctx->module->direct_callables, callee_name);
        if (!callee) {
          callee = LLVMAddFunction(ctx->lmodule, callee_name, llvm_sig);

          LLVMSetLinkage(callee, LLVMExternalLinkage);

          g_hash_table_insert(ctx->module->direct_callables, callee_name,
                              callee);
        } else {
          /* LLVMTypeRef's are uniqued */
          if (LLVMGetElementType(LLVMTypeOf(callee)) != llvm_sig)
  ...
  // FIXME: Locking
  callee =
      (LLVMValueRef)g_hash_table_lookup(ctx->module->plt_entries, callee_name);
  if (!callee) {
    callee = LLVMAddFunction(ctx->lmodule, callee_name, llvm_sig);
...
static LLVMValueRef get_jit_callee(EmitContext *ctx, const char *name,
                                   LLVMTypeRef llvm_sig, MonoJumpInfoType type,
                                   gconstpointer data) {
  gpointer target;
  ...
  LLVMValueRef tramp_var =
      LLVMAddGlobal(ctx->lmodule, LLVMPointerType(llvm_sig, 0), name);
  LLVMSetInitializer(
      tramp_var, LLVMConstIntToPtr(LLVMConstInt(LLVMInt64Type(),
                                                (guint64)(size_t)target, FALSE),
                                   LLVMPointerType(llvm_sig, 0)));
  LLVMSetLinkage(tramp_var, LLVMExternalLinkage);
  LLVMValueRef callee = LLVMBuildLoad(ctx->builder, tramp_var, "");
  return callee;
}

static int get_handler_clause(MonoCompile *cfg, MonoBasicBlock *bb) {
...
static void set_metadata_flag(LLVMValueRef v, const char *flag_name) {
  LLVMValueRef md_arg;
  int md_kind;
...
static void set_nontemporal_flag(LLVMValueRef v) {
  LLVMValueRef md_arg;
  int md_kind;
  const char *flag_name;
 ...
 * emit_call:
 *
 *   Emit an LLVM call or invoke instruction depending on whenever the call is
 * inside a try region.
 */
static LLVMValueRef emit_call(EmitContext *ctx, MonoBasicBlock *bb,
                              LLVMBuilderRef *builder_ref, LLVMValueRef callee,
                              LLVMValueRef *args, int pindex) {
  ...
  if (ctx->llvm_only) {
    ...
    if (clause) {
      g_assert(clause->flags == MONO_EXCEPTION_CLAUSE_NONE ||
      ...
      LLVMBasicBlockRef lpad_bb = (LLVMBasicBlockRef)g_hash_table_lookup(
          ctx->exc_meta, (gconstpointer)key);

      // FIXME: Find the one that has the lowest end bound for the right start
      // address
    ...
    if (clause_index != -1) {
      ...
      g_assert(ec->flags == MONO_EXCEPTION_CLAUSE_NONE ||
               ec->flags == MONO_EXCEPTION_CLAUSE_FINALLY ||
               ec->flags == MONO_EXCEPTION_CLAUSE_FAULT);
      ...
      /* Use an invoke */
      lcall =
          LLVMBuildInvoke(builder, callee, args, pindex, noex_bb, ex_bb, "");
...
static LLVMValueRef emit_load(EmitContext *ctx, MonoBasicBlock *bb,
                              LLVMBuilderRef *builder_ref, int size,
                              LLVMValueRef addr, LLVMValueRef base,
                              const char *name, gboolean is_faulting,
  ...
  if (barrier != LLVM_BARRIER_NONE)
    res = mono_llvm_build_atomic_load(*builder_ref, addr, name, is_volatile,
                                      size, barrier);
  else
...
static void emit_store_general(EmitContext *ctx, MonoBasicBlock *bb,
                               LLVMBuilderRef *builder_ref, int size,
                               LLVMValueRef value, LLVMValueRef addr,
                               LLVMValueRef base, gboolean is_faulting,
                               gboolean is_volatile, BarrierKind barrier) {
  if (barrier != LLVM_BARRIER_NONE)
...
static void emit_store(EmitContext *ctx, MonoBasicBlock *bb,
                       LLVMBuilderRef *builder_ref, int size,
                       LLVMValueRef value, LLVMValueRef addr, LLVMValueRef base,
                       gboolean is_faulting, gboolean is_volatile) {
  emit_store_general(ctx, bb, builder_ref, size, value, addr, base, is_faulting,
                     is_volatile, LLVM_BARRIER_NONE);
}
...
static void emit_cond_system_exception(EmitContext *ctx, MonoBasicBlock *bb,
  ...
  if (exc_id == MONO_EXC_NULL_REF &&
      !ctx->cfg->disable_llvm_implicit_null_checks && !force_explicit) {
  ...
  if (ctx->cfg->llvm_only) {
    ...
    if (exc_id == MONO_EXC_NULL_REF) {
      ...
      /* Can't cache this */
      callee =
          get_callee(ctx, sig, MONO_PATCH_INFO_JIT_ICALL_ADDR,
                     GUINT_TO_POINTER(
      ...
      callee = get_callee(
          ctx, sig, MONO_PATCH_INFO_JIT_ICALL_ADDR,
          GUINT_TO_POINTER(MONO_JIT_ICALL_mono_llvm_throw_corlib_exception));
      args[0] = LLVMConstInt(
          LLVMInt32Type(),
          m_class_get_type_token(exc_class) - MONO_TOKEN_TYPE_DEF, FALSE);
  ...
  if (!callee) {
    ...
    if (no_pc)
      ...
      sig = LLVMFunctionType2(LLVMVoidType(), LLVMInt32Type(),
                              LLVMPointerType(LLVMInt8Type(), 0), FALSE);

    const MonoJitICallId icall_id =
        MONO_JIT_ICALL_mono_llvm_throw_corlib_exception_abs_trampoline;

    if (ctx->cfg->compile_aot) {
      callee = get_callee(ctx, sig, MONO_PATCH_INFO_JIT_ICALL_ID,
                          GUINT_TO_POINTER(icall_id));
    } else {
      /*
       * Differences between the LLVM/non-LLVM throw corlib exception
       * trampoline:
       * - On x86, LLVM generated code doesn't push the arguments
       * - The trampoline takes the throw address as an arguments, not a pc
       * offset.
       */
      callee = get_jit_callee(ctx, "llvm_throw_corlib_exception_trampoline",
                              sig, MONO_PATCH_INFO_JIT_ICALL_ID,
                              GUINT_TO_POINTER(icall_id));

      /*
       * Make sure that ex_bb starts with the invoke, so the block address
       * points to it, and not to the load added by get_jit_callee ().
       */
      ex2_bb = gen_bb(ctx, "EX2_BB");
      LLVMBuildBr(builder, ex2_bb);
      ex_bb = ex2_bb;
  ...
  args[0] = LLVMConstInt(
      LLVMInt32Type(), m_class_get_type_token(exc_class) - MONO_TOKEN_TYPE_DEF,
      FALSE);

  /*
   * The LLVM mono branch contains changes so a block address can be passed as
 ...
 */
static void emit_args_to_vtype(EmitContext *ctx, LLVMBuilderRef builder,
                               MonoType *t, LLVMValueRef address,
                               LLVMArgInfo *ainfo, LLVMValueRef *args) {
  int j, size, nslots;
  MonoClass *klass;
  ...
  if (MONO_CLASS_IS_SIMD(ctx->cfg, klass))
    address = LLVMBuildBitCast(ctx->builder, address,
  ...
  for (j = 0; j < nslots; ++j) {
    ...
    case LLVMArgInIReg: {
      ...
      if (MONO_CLASS_IS_SIMD(ctx->cfg, klass)) {
        index[0] =
            LLVMConstInt(LLVMInt32Type(), j * TARGET_SIZEOF_VOID_P, FALSE);
        addr = LLVMBuildGEP(builder, address, index, 1, "");
      } else {
        daddr = LLVMBuildBitCast(ctx->builder, address,
                                 LLVMPointerType(IntPtrType(), 0), "");
        index[0] = LLVMConstInt(LLVMInt32Type(), j, FALSE);
        addr = LLVMBuildGEP(builder, daddr, index, 1, "");
      }
      LLVMBuildStore(builder, convert(ctx, args[j], part_type),
                     LLVMBuildBitCast(ctx->builder, addr,
                                      LLVMPointerType(part_type, 0), ""));
      break;
    }
    case LLVMArgInFPReg: {
 ...
 * emit_vtype_to_args:
 *
 *   Emit code to load a vtype at address ADDRESS into scalar arguments. Store
 * the arguments into ARGS, and the number of arguments into NARGS.
 */
static void emit_vtype_to_args(EmitContext *ctx, LLVMBuilderRef builder,
                               MonoType *t, LLVMValueRef address,
                               LLVMArgInfo *ainfo, LLVMValueRef *args,
                               guint32 *nargs) {
  int pindex = 0;
    ...
    case LLVMArgInIReg:
      ...
      args[pindex++] = convert(
          ...
          LLVMBuildLoad(builder,
                        LLVMBuildBitCast(
                            ctx->builder, addr,
                            LLVMPointerType(LLVMIntType(partsize * 8), 0), ""),
                        ""),
          IntPtrType());
      break;
    case LLVMArgInFPReg:
...
static LLVMValueRef build_alloca_llvm_type_name(EmitContext *ctx, LLVMTypeRef t,
                                                int align, const char *name) {
  /*
   * Have to place all alloca's at the end of the entry bb, since otherwise they
   * would get executed every time control reaches them.
   */
  LLVMPositionBuilder(ctx->alloca_builder, get_bb(ctx, ctx->cfg->bb_entry),
                      ctx->last_alloca);

  ctx->last_alloca =
      mono_llvm_build_alloca(ctx->alloca_builder, t, NULL, align, name);
  return ctx->last_alloca;
}

static LLVMValueRef build_alloca_llvm_type(EmitContext *ctx, LLVMTypeRef t,
                                           int align) {
  return build_alloca_llvm_type_name(ctx, t, align, "");
}

static LLVMValueRef build_alloca(EmitContext *ctx, MonoType *t) {
...
static LLVMValueRef emit_gsharedvt_ldaddr(EmitContext *ctx, int vreg) {
  /*
   * gsharedvt local.
  ...
  offset =
      LLVMConstInt(LLVMInt32Type(),
                   ...
                   FALSE);
  ptr = LLVMBuildAdd(builder, convert(ctx, info_var, IntPtrType()),
                     convert(ctx, offset, IntPtrType()), "");

  name = g_strdup_printf("gsharedvt_local_%d_offset", vreg);
  offset_var = LLVMBuildLoad(
      builder, convert(ctx, ptr, LLVMPointerType(LLVMInt32Type(), 0)), name);

  return LLVMBuildAdd(builder, convert(ctx, locals_var, IntPtrType()),
                      convert(ctx, offset_var, IntPtrType()), "");
}

/*
 * Put the global into the 'llvm.used' array to prevent it from being optimized
 * away.
 */
static void mark_as_used(MonoLLVMModule *module, LLVMValueRef global) {
  if (!module->used)
    module->used = g_ptr_array_sized_new(16);
  g_ptr_array_add(module->used, global);
}

static void emit_llvm_used(MonoLLVMModule *module) {
  ...
  used_type =
      LLVMArrayType(LLVMPointerType(LLVMInt8Type(), 0), module->used->len);
  used = LLVMAddGlobal(lmodule, used_type, "llvm.used");
  used_elem = g_new0(LLVMValueRef, module->used->len);
  for (i = 0; i < module->used->len; ++i)
    used_elem[i] =
        LLVMConstBitCast((LLVMValueRef)g_ptr_array_index(module->used, i),
                         LLVMPointerType(LLVMInt8Type(), 0));
  LLVMSetInitializer(used, LLVMConstArray(LLVMPointerType(LLVMInt8Type(), 0),
                                          used_elem, module->used->len));
 ...
 */
static void emit_get_method(MonoLLVMModule *module) {
  LLVMModuleRef lmodule = module->lmodule;
  ...
  if (emit_table) {
    ...
    table_name =
        g_strdup_printf("%s_method_table", module->assembly->aname.name);
    table = LLVMAddGlobal(lmodule, table_type, table_name);
    table_elems = g_new0(LLVMValueRef, table_len);
    for (i = 0; i < table_len; ++i) {
      m = (LLVMValueRef)g_hash_table_lookup(module->idx_to_lmethod,
                                            GINT_TO_POINTER(i));
    ...
    LLVMSetInitializer(table, LLVMConstArray(LLVMPointerType(LLVMInt8Type(), 0),
                                             table_elems, table_len));
  }

  /*
   * Emit a switch statement. Emitting a table of function addresses is
   * smaller/faster, but generating code seems safer.
   */
  func = LLVMAddFunction(lmodule, module->get_method_symbol,
                         LLVMFunctionType1(rtype, LLVMInt32Type(), FALSE));
  ...
  LLVMBuildRet(builder,
               LLVMBuildBitCast(builder, module->code_start, rtype, ""));
  ...
  if (emit_table) {
    ...
    LLVMAddCase(switch_ins, LLVMConstInt(LLVMInt32Type(), -1, FALSE),
                code_start_bb);
    LLVMAddCase(switch_ins, LLVMConstInt(LLVMInt32Type(), -2, FALSE),
                code_end_bb);
  } else {
 ...
 */
static void emit_get_unbox_tramp(MonoLLVMModule *module) {
  LLVMModuleRef lmodule = module->lmodule;
  LLVMValueRef func, switch_ins, m;
  ...
  if (emit_table) {
    // About 10% of methods have an unbox tramp, so emit a table of indexes for
    // them that the runtime can search using a binary search
    int len = 0;
    for (i = 0; i < module->max_method_idx + 1; ++i) {
      m = (LLVMValueRef)g_hash_table_lookup(module->idx_to_unbox_tramp,
                                            GINT_TO_POINTER(i));
      if (m)
        len++;
    }
    ...
    table_name =
        g_strdup_printf("%s_unbox_tramp_indexes", module->assembly->aname.name);
    table = LLVMAddGlobal(lmodule, table_type, table_name);
    table_elems = g_new0(LLVMValueRef, table_len);
    int idx = 0;
    for (i = 0; i < module->max_method_idx + 1; ++i) {
    ...
    table_type = LLVMArrayType(elemtype, table_len);
    table_name =
        g_strdup_printf("%s_unbox_trampolines", module->assembly->aname.name);
    table = LLVMAddGlobal(lmodule, table_type, table_name);
    table_elems = g_new0(LLVMValueRef, table_len);
    idx = 0;
    for (i = 0; i < module->max_method_idx + 1; ++i) {
  ...
  func = LLVMAddFunction(lmodule, module->get_unbox_tramp_symbol,
                         LLVMFunctionType1(rtype, LLVMInt32Type(), FALSE));
  LLVMSetLinkage(func, LLVMExternalLinkage);
  LLVMSetVisibility(func, LLVMHiddenVisibility);
  mono_llvm_add_func_attr(func, LLVM_ATTR_NO_UNWIND);
 ...
 * emit_init_aotconst:
 *
 *   Emit a function to initialize the aotconst_ variables. Called by the
 * runtime.
 */
static void emit_init_aotconst(MonoLLVMModule *module) {
  LLVMModuleRef lmodule = module->lmodule;
  ...
  func = LLVMAddFunction(
      lmodule, module->init_aotconst_symbol,
  ...
  for (int i = 0; i < module->max_got_offset + 1; ++i) {
    ...
    LLVMValueRef var =
        g_hash_table_lookup(module->aotconst_vars, GINT_TO_POINTER(i));
    if (var) {
      LLVMValueRef addr =
          LLVMBuildBitCast(builder, var, LLVMPointerType(IntPtrType(), 0), "");
...
static void emit_llvm_code_start(MonoLLVMModule *module) {
  LLVMModuleRef lmodule = module->lmodule;
  LLVMValueRef func;
  LLVMBasicBlockRef entry_bb;
  LLVMBuilderRef builder;

  func = LLVMAddFunction(lmodule, "llvm_code_start",
 ...
 * The wrappers handle adding the 'amodule' argument, loading the vtable from
 * different locations, and they have a cold calling convention.
 */
static LLVMValueRef emit_init_func(MonoLLVMModule *module,
                                   MonoAotInitSubtype subtype) {
  LLVMModuleRef lmodule = module->lmodule;
  LLVMValueRef func, indexes[2], args[16], callee, info_var, index_var,
      inited_var, cmp;
  ...
  case AOT_INIT_METHOD_GSHARED_VTABLE:
    func_type =
        LLVMFunctionType2(LLVMVoidType(), arg_type, IntPtrType(), FALSE);
    break;
  case AOT_INIT_METHOD_GSHARED_THIS:
    func_type =
        LLVMFunctionType2(LLVMVoidType(), arg_type, ObjRefType(), FALSE);
    break;
  default:
    g_assert_not_reached();
  }
  ...
  index_var = LLVMBuildLoad(
      builder,
      LLVMBuildGEP(builder,
                   LLVMBuildBitCast(builder, info_var,
                                    LLVMPointerType(LLVMInt32Type(), 0), ""),
                   indexes, 1, ""),
      "method_index");

  /* Check for is_inited here as well, since this can be called from JITted code
   * which might not check it */
  indexes[0] = const_int32(0);
  indexes[1] = index_var;
  inited_var = LLVMBuildLoad(
      builder, LLVMBuildGEP(builder, module->inited_var, indexes, 2, ""),
      "is_inited");

  cmp = LLVMBuildICmp(builder, LLVMIntEQ, inited_var,
                      LLVMConstInt(LLVMTypeOf(inited_var), 0, FALSE), "");
  ...
  LLVMValueRef amodule_var =
      get_aotconst_module(module, builder, MONO_PATCH_INFO_AOT_MODULE, NULL,
                          LLVMPointerType(IntPtrType(), 0), NULL, NULL);
  ...
  case AOT_INIT_METHOD_GSHARED_THIS:
    /* Load this->vtable */
    args[3] = LLVMBuildBitCast(builder, LLVMGetParam(func, 1),
                               LLVMPointerType(IntPtrType(), 0), "");
    indexes[0] =
        const_int32(MONO_STRUCT_OFFSET(MonoObject, vtable) / SIZEOF_VOID_P);
    args[3] = LLVMBuildLoad(
        builder, LLVMBuildGEP(builder, args[3], indexes, 1, ""), "vtable");
    break;
  case AOT_INIT_METHOD_GSHARED_MRGCTX:
    /* Load mrgctx->vtable */
    args[3] = LLVMBuildIntToPtr(builder, LLVMGetParam(func, 1),
                                LLVMPointerType(IntPtrType(), 0), "");
    indexes[0] = const_int32(
  ...
  icall_sig = LLVMFunctionType4(LLVMVoidType(), IntPtrType(), IntPtrType(),
                                arg_type, IntPtrType(), FALSE);
  callee =
      get_aotconst_module(module, builder, MONO_PATCH_INFO_JIT_ICALL_ID,
                          GINT_TO_POINTER(MONO_JIT_ICALL_mini_llvm_init_method),
                          LLVMPointerType(icall_sig, 0), NULL, NULL);
  LLVMBuildCall(builder, callee, args, LLVMCountParamTypes(icall_sig), "");

  /*
   * Set the inited flag
   * This is already done by the LLVM methods themselves, but its needed by
   * JITted methods.
   */
  indexes[0] = const_int32(0);
  indexes[1] = index_var;
  LLVMBuildStore(builder, LLVMConstInt(LLVMInt8Type(), 1, FALSE),
                 LLVMBuildGEP(builder, module->inited_var, indexes, 2, ""));
...
/* Emit a wrapper around the parameterless JIT icall ICALL_ID with a cold
 * calling convention */
static LLVMValueRef emit_icall_cold_wrapper(MonoLLVMModule *module,
                                            LLVMModuleRef lmodule,
  ...
  name = g_strdup_printf("%s_icall_cold_wrapper_%d", module->global_prefix,
                         icall_id);

  func = LLVMAddFunction(lmodule, name,
                         LLVMFunctionType(LLVMVoidType(), NULL, 0, FALSE));
  ...
  if (aot) {
    callee = get_aotconst_module(module, builder, MONO_PATCH_INFO_JIT_ICALL_ID,
                                 GUINT_TO_POINTER(icall_id),
                                 LLVMPointerType(sig, 0), NULL, NULL);
  } else {
    ...
    LLVMValueRef tramp_var =
        LLVMAddGlobal(lmodule, LLVMPointerType(sig, 0), name);
    LLVMSetInitializer(
        tramp_var,
        LLVMConstIntToPtr(
            LLVMConstInt(LLVMInt64Type(), (guint64)(size_t)target, FALSE),
            LLVMPointerType(sig, 0)));
    LLVMSetLinkage(tramp_var, LLVMExternalLinkage);
 ...
 */
static void emit_init_funcs(MonoLLVMModule *module) {
  module->init_method = emit_init_func(module, AOT_INIT_METHOD);
  module->init_method_gshared_mrgctx =
      emit_init_func(module, AOT_INIT_METHOD_GSHARED_MRGCTX);
  module->init_method_gshared_this =
      emit_init_func(module, AOT_INIT_METHOD_GSHARED_THIS);
  module->init_method_gshared_vtable =
      emit_init_func(module, AOT_INIT_METHOD_GSHARED_VTABLE);
}

static LLVMValueRef get_init_func(MonoLLVMModule *module,
                                  MonoAotInitSubtype subtype) {
  switch (subtype) {
  case AOT_INIT_METHOD:
...
static void emit_gc_safepoint_poll(MonoLLVMModule *module,
                                   LLVMModuleRef lmodule, MonoCompile *cfg) {
  ...
  if (is_aot) {
    ...
    mono_llvm_add_func_attr(
        func,
        LLVM_ATTR_OPTIMIZE_NONE); // no need to waste time here, the function is
                                  // already optimized and will be inlined.
    mono_llvm_add_func_attr(
        func, LLVM_ATTR_NO_INLINE); // optnone attribute requires noinline (but
                                    // it will be inlined anyway)
    if (!module->gc_poll_cold_wrapper_compiled) {
      ...
      module->gc_poll_cold_wrapper_compiled =
          mono_jit_compile_method(wrapper, error);
      mono_error_assert_ok(error);
    }
  }
  LLVMBasicBlockRef entry_bb =
      LLVMAppendBasicBlock(func, "gc.safepoint_poll.entry");
  LLVMBasicBlockRef poll_bb =
      LLVMAppendBasicBlock(func, "gc.safepoint_poll.poll");
  LLVMBasicBlockRef exit_bb =
      LLVMAppendBasicBlock(func, "gc.safepoint_poll.exit");
  ...
  if (is_aot) {
    poll_val_ptr =
        get_aotconst_module(module, builder, MONO_PATCH_INFO_GC_SAFE_POINT_FLAG,
                            NULL, ptr_type, NULL, NULL);
  } else {
    LLVMValueRef poll_val_int =
        LLVMConstInt(IntPtrType(), (guint64)&mono_polling_required, FALSE);
    poll_val_ptr = LLVMBuildIntToPtr(builder, poll_val_int, ptr_type, "");
  }
  LLVMValueRef poll_val_ptr_load =
      LLVMBuildLoad(builder, poll_val_ptr, ""); // probably needs to be volatile
  LLVMValueRef poll_val =
      LLVMBuildPtrToInt(builder, poll_val_ptr_load, IntPtrType(), "");
  LLVMValueRef poll_val_zero = LLVMConstNull(LLVMTypeOf(poll_val));
  LLVMValueRef cmp =
      LLVMBuildICmp(builder, LLVMIntEQ, poll_val, poll_val_zero, "");
  mono_llvm_build_weighted_branch(builder, cmp, exit_bb, poll_bb,
                                  1000 /* weight for exit_bb */,
                                  1 /* weight for poll_bb */);

  /* poll: */
  LLVMPositionBuilderAtEnd(builder, poll_bb);
  LLVMValueRef call;
  if (is_aot) {
    LLVMValueRef icall_wrapper = emit_icall_cold_wrapper(
        module, lmodule, MONO_JIT_ICALL_mono_threads_state_poll, TRUE);
    module->gc_poll_cold_wrapper = icall_wrapper;
    call = LLVMBuildCall(builder, icall_wrapper, NULL, 0, "");
  } else {
    // in JIT mode we have to emit @gc.safepoint_poll function for each method
    // (module) this function calls gc_poll_cold_wrapper_compiled via a global
    // variable.
    // @gc.safepoint_poll will be inlined and can be deleted after
    // -place-safepoints pass.
    LLVMTypeRef poll_sig = LLVMFunctionType0(LLVMVoidType(), FALSE);
    LLVMTypeRef poll_sig_ptr = LLVMPointerType(poll_sig, 0);
    gpointer target = resolve_patch(cfg, MONO_PATCH_INFO_ABS,
                                    module->gc_poll_cold_wrapper_compiled);
    LLVMValueRef tramp_var =
        LLVMAddGlobal(lmodule, poll_sig_ptr, "mono_threads_state_poll");
    LLVMValueRef target_val =
        LLVMConstInt(LLVMInt64Type(), (guint64)target, FALSE);
...
static void emit_llvm_code_end(MonoLLVMModule *module) {
  LLVMModuleRef lmodule = module->lmodule;
  LLVMValueRef func;
  LLVMBasicBlockRef entry_bb;
  LLVMBuilderRef builder;

  func = LLVMAddFunction(lmodule, "llvm_code_end",
                         LLVMFunctionType(LLVMVoidType(), NULL, 0, FALSE));
  LLVMSetLinkage(func, LLVMInternalLinkage);
  mono_llvm_add_func_attr(func, LLVM_ATTR_NO_UNWIND);
...
static void emit_div_check(EmitContext *ctx, LLVMBuilderRef builder,
                           MonoBasicBlock *bb, MonoInst *ins, LLVMValueRef lhs,
                           LLVMValueRef rhs) {
  gboolean need_div_check = ctx->cfg->backend->need_div_check;

  if (bb->region)
    /* LLVM doesn't know that these can throw an exception since they are not
     * called through an intrinsic */
    need_div_check = TRUE;
  ...
  case OP_LREM_UN_IMM: {
    LLVMValueRef cmp;
    gboolean is_signed =
        (ins->opcode == OP_IDIV || ins->opcode == OP_LDIV ||
         ins->opcode == OP_IREM || ins->opcode == OP_LREM ||
         ins->opcode == OP_IDIV_IMM || ins->opcode == OP_LDIV_IMM ||
         ins->opcode == OP_IREM_IMM || ins->opcode == OP_LREM_IMM);

    cmp = LLVMBuildICmp(builder, LLVMIntEQ, rhs,
                        LLVMConstInt(LLVMTypeOf(rhs), 0, FALSE), "");
    ...
    if (is_signed) {
      LLVMValueRef c =
          (LLVMTypeOf(lhs) == LLVMInt32Type())
              ? LLVMConstInt(LLVMTypeOf(lhs), 0x80000000, FALSE)
              : LLVMConstInt(LLVMTypeOf(lhs), 0x8000000000000000LL, FALSE);
      LLVMValueRef cond1 =
          LLVMBuildICmp(builder, LLVMIntEQ, rhs,
                        LLVMConstInt(LLVMTypeOf(rhs), -1, FALSE), "");
      LLVMValueRef cond2 = LLVMBuildICmp(builder, LLVMIntEQ, lhs, c, "");

      cmp = LLVMBuildICmp(builder, LLVMIntEQ,
 ...
 */
static void emit_method_init(EmitContext *ctx) {
  ...
  ctx->module->max_inited_idx =
      MAX(ctx->module->max_inited_idx, cfg->method_index);

  indexes[0] = const_int32(0);
  indexes[1] = const_int32(cfg->method_index);
  inited_var = LLVMBuildLoad(
      builder, LLVMBuildGEP(builder, ctx->module->inited_var, indexes, 2, ""),
      "is_inited");

  args[0] = inited_var;
  args[1] = LLVMConstInt(LLVMInt8Type(), 1, FALSE);
  inited_var = LLVMBuildCall(ctx->builder, get_intrins(ctx, INTRINS_EXPECT_I8),
                             args, 2, "");
  ...
  // FIXME: Cache
  if (ctx->rgctx_arg && ((cfg->method->is_inflated &&
                          mono_method_get_context(cfg->method)->method_inst) ||
  ...
  indexes[1] = const_int32(cfg->method_index);
  LLVMBuildStore(
      builder, LLVMConstInt(LLVMInt8Type(), 1, FALSE),
      LLVMBuildGEP(builder, ctx->module->inited_var, indexes, 2, ""));
...
static void emit_unbox_tramp(EmitContext *ctx, const char *method_name,
                             LLVMTypeRef method_type, LLVMValueRef method,
                             int method_index) {
  /*
  ...
  if (!ctx->llvm_only && ctx->rgctx_arg_pindex != -1)
    mono_llvm_add_param_attr(LLVMGetParam(tramp, ctx->rgctx_arg_pindex),
                             LLVM_ATTR_IN_REG);
  if (ctx->cfg->vret_addr) {
    LLVMSetValueName(LLVMGetParam(tramp, linfo->vret_arg_pindex), "vret");
    if (linfo->ret.storage == LLVMArgVtypeByRef) {
      mono_llvm_add_param_attr(LLVMGetParam(tramp, linfo->vret_arg_pindex),
  ...
  for (i = 0; i < nargs; ++i) {
    ...
    if (i == ctx->this_arg_pindex) {
      ...
      args[i] = LLVMBuildAdd(
          builder, args[i],
          LLVMConstInt(IntPtrType(), MONO_ABI_SIZEOF(MonoObject), FALSE), "");
  ...
  if (linfo->ret.storage == LLVMArgVtypeByRef)
    mono_llvm_add_instr_attr(call, 1 + linfo->vret_arg_pindex,
                             LLVM_ATTR_STRUCT_RET);
  ...
  g_hash_table_insert(ctx->module->idx_to_unbox_tramp,
                      GINT_TO_POINTER(method_index), tramp);
  LLVMDisposeBuilder(builder);
}
 ...
 */
static void emit_entry_bb(EmitContext *ctx, LLVMBuilderRef builder) {
  int i, j, pindex;
  MonoCompile *cfg = ctx->cfg;
  ...
  for (i = 0; i < cfg->num_varinfo; ++i) {
    ...
    if ((var->opcode == OP_GSHAREDVT_LOCAL ||
         var->opcode == OP_GSHAREDVT_ARG_REGOFFSET))
      continue;

#ifdef TARGET_WASM
    // For GC stack scanning to work, have to spill all reference variables to
    // the stack Some ref variables have type intptr
    if (ctx->has_safepoints &&
        (MONO_TYPE_IS_REFERENCE(var->inst_vtype) ||
         var->inst_vtype->type == MONO_TYPE_I) &&
        var != ctx->cfg->rgctx_var)
      var->flags |= MONO_INST_INDIRECT;
#endif

    if (var->flags & (MONO_INST_VOLATILE | MONO_INST_INDIRECT) ||
        (mini_type_is_vtype(var->inst_vtype) &&
         !MONO_CLASS_IS_SIMD(ctx->cfg, var->klass))) {
      ...
      if (!ctx->addresses[var->dreg]) {
        ...
        // LLVMSetValueName (ctx->addresses [var->dreg], g_strdup_printf
        // ("vreg_loc_%d", var->dreg));
      }
  ...
  for (i = 0; i < sig->param_count; ++i) {
    ...
    case LLVMArgAsFpArgs: {
      ...
      /* The argument is received as a set of int/fp arguments, store them into
       * the real argument */
      ...
      emit_args_to_vtype(ctx, builder, ainfo->type, ctx->addresses[reg], ainfo,
                         args);
      break;
    }
    case LLVMArgVtypeByVal: {
    ...
    case LLVMArgAsIArgs: {
      ...
      /* The argument is received as an array of ints, store it into the real
       * argument */
      ...
      if (size == 0) {
        ...
        LLVMValueRef dest = convert(ctx, ctx->addresses[reg],
                                    LLVMPointerType(LLVMIntType(size * 8), 0));
        LLVMBuildStore(ctx->builder,
                       LLVMBuildTrunc(builder, val, LLVMIntType(size * 8), ""),
                       dest);
      } else {
        LLVMBuildStore(ctx->builder, arg,
                       convert(ctx, ctx->addresses[reg],
    ...
    case LLVMArgVtypeAsScalar:
      ...
      break;
    case LLVMArgGsharedvtFixed: {
      /* These are non-gsharedvt arguments passed by ref, the rest of the IR
      ...
      ctx->values[reg] = LLVMBuildLoad(
          builder,
          convert(ctx, arg,
                  LLVMPointerType(type_to_llvm_type(ctx, ainfo->type), 0)),
          name);
      break;
    }
    case LLVMArgGsharedvtFixedVtype: {
      ...
      /* Non-gsharedvt vtype argument passed by ref, the rest of the IR treats
       * it as a vtype */
    ...
    default: {
      LLVMTypeRef t;
      /* Needed to avoid phi argument mismatch errors since operations on
       * pointers produce i32/i64 */
      if (ainfo->type->byref)
        t = IntPtrType();
      else
        t = type_to_llvm_type(ctx, ainfo->type);
      ctx->values[reg] =
          convert_full(ctx, ctx->values[reg], llvm_type_to_stack_type(cfg, t),
                       type_is_unsigned(ctx, ainfo->type));
      break;
    }
    }
...
#endif
    {
      if (MONO_CLASS_IS_SIMD(ctx->cfg,
                             mono_class_from_mono_type_internal(ainfo->type)))
  ...
  if (sig->hasthis && !cfg->rgctx_var && cfg->gshared && !cfg->llvm_only) {
    ...
    /*
     * The exception handling code needs the location where the this argument
     * was stored for gshared methods. We create a separate alloca to hold it,
     * and mark it with the "mono.this" custom metadata to tell llvm that it
     * needs to save its location into the LSDA.
     */
    this_alloc = mono_llvm_build_alloca(
        builder, ThisType(), LLVMConstInt(LLVMInt32Type(), 1, FALSE), 0, "");
    /* This volatile store will keep the alloca alive */
    mono_llvm_build_store(builder, ctx->values[cfg->args[0]->dreg], this_alloc,
                          TRUE, LLVM_BARRIER_NONE);
  ...
  if (cfg->rgctx_var) {
    if (!(cfg->rgctx_var->flags & MONO_INST_VOLATILE)) {
      /* FIXME: This could be volatile even in llvmonly mode if used inside a
       * clause etc. */
      ...
      /* This volatile store will keep the alloca alive */
      store = mono_llvm_build_store(builder,
                                    convert(ctx, ctx->rgctx_arg, IntPtrType()),
                                    rgctx_alloc, TRUE, LLVM_BARRIER_NONE);
  ...
  ctx->nested_in = (GSList **)mono_mempool_alloc0(
      cfg->mempool, sizeof(GSList *) * cfg->header->num_clauses);
  for (i = 0; i < cfg->header->num_clauses; ++i) {
    for (j = 0; j < cfg->header->num_clauses; ++j) {
      ...
      if (i != j && clause1->try_offset >= clause2->try_offset &&
          clause1->handler_offset <= clause2->handler_offset)
        ctx->nested_in[i] = g_slist_prepend_mempool(
            cfg->mempool, ctx->nested_in[i], GINT_TO_POINTER(j));
    }
  }

  /*
   * For finally clauses, create an indicator variable telling OP_ENDFINALLY
   * whenever it needs to continue normally, or return back to the exception
   * handling system.
   */
...
static gboolean needs_extra_arg(EmitContext *ctx, MonoMethod *method) {
  WrapperInfo *info = NULL;

  /*
   * When targeting wasm, the caller and callee signature has to match exactly.
   * This means that every method which can be called indirectly need an extra
   * arg since the caller will call it through an ftnptr and will pass an extra
   * arg.
   */
  ...
  case MONO_WRAPPER_MANAGED_TO_NATIVE:
    if (strstr(method->name, "icall_wrapper"))
      /* These are JIT icall wrappers which are only called from JITted code
       * directly */
      return FALSE;
  ...
  /* These are called from gsharedvt code with an indirect call which doesn't
   * pass an extra arg */
...
static inline gboolean is_supported_callconv(EmitContext *ctx,
                                             MonoCallInst *call) {
...
#else
  gboolean result =
      (call->signature->call_convention == MONO_CALL_DEFAULT) ||
      ((call->signature->call_convention == MONO_CALL_C) && ctx->llvm_only);
#endif
  return result;
}

static void process_call(EmitContext *ctx, MonoBasicBlock *bb,
                         LLVMBuilderRef *builder_ref, MonoInst *ins) {
  MonoCompile *cfg = ctx->cfg;
  ...
  /* If both imt and rgctx arg are required, only pass the imt arg, the rgctx
   * trampoline will pass the rgctx */
  if (call->imt_arg_reg)
  ...
  if (call->imt_arg_reg)
    cinfo->imt_arg = TRUE;
  if (!call->rgctx_arg_reg && call->method &&
      needs_extra_arg(ctx, call->method))
    cinfo->dummy_arg = TRUE;

  vretaddr = (cinfo->ret.storage == LLVMArgVtypeRetAddr ||
              cinfo->ret.storage == LLVMArgVtypeByRef ||
              cinfo->ret.storage == LLVMArgGsharedvtFixed ||
              cinfo->ret.storage == LLVMArgGsharedvtVariable ||
              cinfo->ret.storage == LLVMArgGsharedvtFixedVtype);
  ...
  calli = !call->fptr_is_patch &&
          (opcode == OP_VOIDCALL_REG || opcode == OP_CALL_REG ||
           opcode == OP_VCALL_REG || opcode == OP_LCALL_REG ||
           opcode == OP_FCALL_REG || opcode == OP_RCALL_REG ||
  ...
  if (ins->flags & MONO_INST_HAS_METHOD) {
    if (is_virtual) {
      ...
      if (cfg->compile_aot) {
        callee =
            get_callee(ctx, llvm_sig, MONO_PATCH_INFO_METHOD, call->method);
        if (!callee) {
        ...
        name = g_strdup_printf("[tramp_%d] %s", tramp_index,
                               mono_method_full_name(call->method, TRUE));
        tramp_index++;

        /*
         * Use our trampoline infrastructure for lazy compilation instead of
         * llvm's. Make all calls through a global. The address of the global
         * will be saved in MonoJitDomainInfo.llvm_jit_callees and updated when
         * the method it refers to is compiled.
         */
        LLVMValueRef tramp_var =
            (LLVMValueRef)g_hash_table_lookup(ctx->jit_callees, call->method);
        if (!tramp_var) {
          target = mono_create_jit_trampoline(mono_domain_get(), call->method,
                                              error);
          if (!is_ok(error)) {
    ...
    if (!cfg->llvm_only && call->method &&
        strstr(m_class_get_name(call->method->klass),
    ...
    if (jit_icall_id) {
      if (cfg->compile_aot) {
        callee = get_callee(ctx, llvm_sig, MONO_PATCH_INFO_JIT_ICALL_ID,
        ...
        callee = get_jit_callee(ctx, "", llvm_sig, MONO_PATCH_INFO_JIT_ICALL_ID,
                                GUINT_TO_POINTER(jit_icall_id));
      }
    } else {
      if (cfg->compile_aot) {
        ...
        if (cfg->abs_patches) {
          ...
          if (abs_ji) {
            callee =
                get_callee(ctx, llvm_sig, abs_ji->type, abs_ji->data.target);
            if (!callee) {
              set_failure(ctx, "can't encode patch");
              return;
            }
        ...
        if (cfg->abs_patches) {
          ...
          if (abs_ji) {
            ...
            target = mono_resolve_patch_target(cfg->method, cfg->domain, NULL,
                                               abs_ji, FALSE, error);
            mono_error_assert_ok(error);
            callee = get_jit_callee(ctx, "", llvm_sig, abs_ji->type,
                                    abs_ji->data.target);
          } else {
            g_assert_not_reached();
          }
  ...
  if (is_virtual) {
    ...
    callee = convert(
        ...
        LLVMBuildLoad(
            ...
            LLVMBuildGEP(
                builder,
                convert(ctx, values[ins->inst_basereg],
                        LLVMPointerType(LLVMPointerType(IntPtrType(), 0), 0)),
                &index, 1, ""),
            ""),
        LLVMPointerType(llvm_sig, 0));
  } else if (calli) {
   ...
   */
  nargs = (sig->param_count * 16) + sig->hasthis + vretaddr + call->rgctx_reg +
          call->imt_arg_reg + call->cinfo->dummy_arg + 1;
  ...
  if (call->rgctx_arg_reg) {
    ...
    /*
     * On ARM, the imt/rgctx argument is passed in a caller save register, but
     * some of our trampolines etc. clobber it, leading to problems is LLVM
     * moves the arg assignment earlier. To work around this, save the argument
     * into a stack slot and load it using a volatile load.
     */
#ifdef TARGET_ARM
    if (!ctx->imt_rgctx_loc)
      ctx->imt_rgctx_loc = build_alloca_llvm_type(ctx, ctx->module->ptr_type,
                                                  TARGET_SIZEOF_VOID_P);
    LLVMBuildStore(
        builder,
        convert(ctx, ctx->values[call->rgctx_arg_reg], ctx->module->ptr_type),
        ctx->imt_rgctx_loc);
    args[cinfo->rgctx_arg_pindex] =
        mono_llvm_build_load(builder, ctx->imt_rgctx_loc, "", TRUE);
#else
    args[cinfo->rgctx_arg_pindex] =
        convert(ctx, values[call->rgctx_arg_reg], ctx->module->ptr_type);
#endif
  }
  if (call->imt_arg_reg) {
    ...
    LLVMBuildStore(
        builder,
        convert(ctx, ctx->values[call->imt_arg_reg], ctx->module->ptr_type),
        ctx->imt_rgctx_loc);
    args[cinfo->imt_arg_pindex] =
        mono_llvm_build_load(builder, ctx->imt_rgctx_loc, "", TRUE);
#else
    args[cinfo->imt_arg_pindex] =
        convert(ctx, values[call->imt_arg_reg], ctx->module->ptr_type);
#endif
  }
  switch (cinfo->ret.storage) {
  case LLVMArgGsharedvtVariable: {
    ...
    if (var && var->opcode == OP_GSHAREDVT_LOCAL) {
      args[cinfo->vret_arg_pindex] =
          convert(ctx, emit_gsharedvt_ldaddr(ctx, var->dreg), IntPtrType());
    } else {
      g_assert(addresses[call->inst.dreg]);
      args[cinfo->vret_arg_pindex] =
          convert(ctx, addresses[call->inst.dreg], IntPtrType());
    }
    break;
  }
  default:
    if (vretaddr) {
      ...
      if (cinfo->ret.storage == LLVMArgVtypeByRef)
        ...
        args[cinfo->vret_arg_pindex] = LLVMBuildPtrToInt(
            builder, addresses[call->inst.dreg], IntPtrType(), "");
    }
    break;
  }

  /*
   * Sometimes the same method is called with two different signatures (i.e.
   * with and without 'this'), so use the real callee for argument type
   * conversion.
   */
  LLVMTypeRef callee_type = LLVMGetElementType(LLVMTypeOf(callee));
  LLVMTypeRef *param_types = (LLVMTypeRef *)g_alloca(
      sizeof(LLVMTypeRef) * LLVMCountParamTypes(callee_type));
  ...
  for (i = 0; i < sig->param_count + sig->hasthis; ++i) {
    ...
    case LLVMArgAsFpArgs: {
      ...
      emit_vtype_to_args(ctx, builder, ainfo->type, addresses[reg], ainfo,
                         args + pindex, &nargs);
      pindex += nargs;
    ...
    case LLVMArgAsIArgs:
      g_assert(addresses[reg]);
      if (ainfo->esize == 8)
        args[pindex] = LLVMBuildLoad(
            ctx->builder,
            convert(ctx, addresses[reg],
                    LLVMPointerType(
                        LLVMArrayType(LLVMInt64Type(), ainfo->nslots), 0)),
            "");
      else
        args[pindex] = LLVMBuildLoad(
            ctx->builder,
            convert(
                ctx, addresses[reg],
                LLVMPointerType(LLVMArrayType(IntPtrType(), ainfo->nslots), 0)),
            "");
      break;
    case LLVMArgVtypeAsScalar:
      g_assert_not_reached();
    ...
    case LLVMArgGsharedvtVariable:
      g_assert(addresses[reg]);
      args[pindex] =
          convert(ctx, addresses[reg], LLVMPointerType(IntPtrType(), 0));
      break;
    default:
      ...
      if (i == 0 && sig->hasthis)
        ...
        args[pindex] =
            convert(ctx, args[pindex], type_to_llvm_arg_type(ctx, ainfo->type));
      break;
    }
   ...
   */
  lcall =
      emit_call(ctx, bb, &builder, callee, args, LLVMCountParamTypes(llvm_sig));

  mono_llvm_nonnull_state_update(ctx, lcall, call->method, args,
  ...
  if (ins->opcode != OP_TAILCALL && ins->opcode != OP_TAILCALL_MEMBASE &&
      LLVMGetInstructionOpcode(lcall) == LLVMCall)
    mono_llvm_set_call_notailcall(lcall);

  // Add original method name we are currently emitting as a custom string
  // metadata (the only way to leave comments in LLVM IR)
  if (mono_debug_enabled() && call && call->method)
    mono_llvm_add_string_metadata(lcall, "managed_name",
                                  mono_method_full_name(call->method, TRUE));
...
#endif
  /* The two can't be used together, so use only one LLVM calling conv to pass
   * them */
  ...
  if (cinfo->ret.storage == LLVMArgVtypeByRef)
    mono_llvm_add_instr_attr(lcall, 1 + cinfo->vret_arg_pindex,
                             LLVM_ATTR_STRUCT_RET);
  if (!ctx->llvm_only && call->rgctx_arg_reg)
    mono_llvm_add_instr_attr(lcall, 1 + cinfo->rgctx_arg_pindex,
                             LLVM_ATTR_IN_REG);
  if (call->imt_arg_reg)
    mono_llvm_add_instr_attr(lcall, 1 + cinfo->imt_arg_pindex,
                             LLVM_ATTR_IN_REG);
  ...
  case LLVMArgVtypeInReg: {
    ...
    emit_args_to_vtype(ctx, builder, sig->ret, addresses[ins->dreg],
                       &cinfo->ret, regs);
    break;
  }
  case LLVMArgVtypeByVal:
  ...
  case LLVMArgFpStruct:
    if (!addresses[call->inst.dreg])
      addresses[call->inst.dreg] = build_alloca(ctx, sig->ret);
    LLVMBuildStore(builder, lcall,
                   convert_full(ctx, addresses[call->inst.dreg],
                                LLVMPointerType(LLVMTypeOf(lcall), 0), FALSE));
    break;
  case LLVMArgVtypeAsScalar:
    if (!addresses[call->inst.dreg])
      addresses[call->inst.dreg] = build_alloca(ctx, sig->ret);
  ...
  case LLVMArgVtypeByRef:
    if (MONO_CLASS_IS_SIMD(ctx->cfg,
      ...
      g_assert(addresses[call->inst.dreg]);
      values[ins->dreg] = LLVMBuildLoad(
          builder,
          convert_full(ctx, addresses[call->inst.dreg],
                       LLVMPointerType(type_to_llvm_type(ctx, sig->ret), 0),
                       FALSE),
          "");
    }
    break;
  ...
  default:
    if (sig->ret->type != MONO_TYPE_VOID)
      ...
      values[ins->dreg] = convert_full(
          ctx, lcall,
          llvm_type_to_stack_type(cfg, type_to_llvm_type(ctx, sig->ret)),
          type_is_unsigned(ctx, sig->ret));
    break;
  }
...
static void emit_llvmonly_throw(EmitContext *ctx, MonoBasicBlock *bb,
                                gboolean rethrow, LLVMValueRef exc) {
  MonoJitICallId icall_id = rethrow ? MONO_JIT_ICALL_mono_llvm_rethrow_exception
                                    : MONO_JIT_ICALL_mono_llvm_throw_exception;
  LLVMValueRef callee =
      rethrow ? ctx->module->rethrow : ctx->module->throw_icall;

  LLVMTypeRef exc_type =
      type_to_llvm_type(ctx, m_class_get_byval_arg(mono_get_exception_class()));

  if (!callee) {
    LLVMTypeRef fun_sig = LLVMFunctionType1(LLVMVoidType(), exc_type, FALSE);

    g_assert(ctx->cfg->compile_aot);
    callee = get_callee(ctx, fun_sig, MONO_PATCH_INFO_JIT_ICALL_ADDR,
                        GUINT_TO_POINTER(icall_id));
  }
...
static void emit_throw(EmitContext *ctx, MonoBasicBlock *bb, gboolean rethrow,
                       LLVMValueRef exc) {
  MonoMethodSignature *throw_sig;

  LLVMValueRef *const pcallee =
      rethrow ? &ctx->module->rethrow : &ctx->module->throw_icall;
  LLVMValueRef callee = *pcallee;
  char const *const icall_name =
      rethrow ? "mono_arch_rethrow_exception" : "mono_arch_throw_exception";
#ifndef TARGET_X86
  const
#endif
      MonoJitICallId icall_id =
          rethrow ? MONO_JIT_ICALL_mono_arch_rethrow_exception
  ...
  if (!callee) {
    ...
    if (ctx->cfg->compile_aot) {
      callee =
          get_callee(ctx, sig_to_llvm_sig(ctx, throw_sig),
                     MONO_PATCH_INFO_JIT_ICALL_ID, GUINT_TO_POINTER(icall_id));
    } else {
#ifdef TARGET_X86
      /*
       ...
       * trampoline.
       */
      icall_id = rethrow ? MONO_JIT_ICALL_mono_llvm_rethrow_exception_trampoline
                         : MONO_JIT_ICALL_mono_llvm_throw_exception_trampoline;
#endif
      callee = get_jit_callee(ctx, icall_name, sig_to_llvm_sig(ctx, throw_sig),
  ...
  arg = convert(
      ctx, exc,
      type_to_llvm_type(ctx, m_class_get_byval_arg(mono_get_object_class())));
...
static void emit_resume_eh(EmitContext *ctx, MonoBasicBlock *bb) {
  ...
  g_assert(ctx->cfg->compile_aot);
  callee = get_callee(ctx, fun_sig, MONO_PATCH_INFO_JIT_ICALL_ID,
                      GUINT_TO_POINTER(icall_id));
...
mono_llvm_emit_clear_exception_call(EmitContext *ctx, LLVMBuilderRef builder) {
  ...
  if (!callee) {
    if (ctx->cfg->compile_aot) {
      callee = get_callee(ctx, call_sig, MONO_PATCH_INFO_JIT_ICALL_ID,
                          GUINT_TO_POINTER(icall_id));
    } else {
...
static LLVMValueRef mono_llvm_emit_load_exception_call(EmitContext *ctx,
                                                       LLVMBuilderRef builder) {
...
static LLVMValueRef mono_llvm_emit_match_exception_call(EmitContext *ctx,
                                                        LLVMBuilderRef builder,
                                                        gint32 region_start,
                                                        gint32 region_end) {
  ...
  args[0] = convert(ctx,
                    get_aotconst(ctx, MONO_PATCH_INFO_AOT_JIT_INFO,
                                 GINT_TO_POINTER(ctx->cfg->method_index),
                                 LLVMPointerType(IntPtrType(), 0)),
                    IntPtrType());
  ...
  if (ctx->cfg->rgctx_var) {
    if (ctx->cfg->llvm_only) {
      ...
      args[3] = LLVMBuildLoad(
          builder, convert(ctx, rgctx_alloc, LLVMPointerType(IntPtrType(), 0)),
          "");
    }
  } else {
  ...
  LLVMTypeRef match_sig =
      LLVMFunctionType5(LLVMInt32Type(), IntPtrType(), LLVMInt32Type(),
                        LLVMInt32Type(), IntPtrType(), IntPtrType(), FALSE);
  LLVMValueRef callee;
  g_assert(ctx->cfg->compile_aot);
  ctx->builder = builder;
  // get_callee expects ctx->builder to be the emitting builder
  callee = get_callee(ctx, match_sig, MONO_PATCH_INFO_JIT_ICALL_ID,
                      GUINT_TO_POINTER(icall_id));
...
static LLVMTypeRef default_cpp_lpad_exc_signature(void) {
...
static LLVMValueRef get_mono_personality(EmitContext *ctx) {
  LLVMValueRef personality = NULL;
  LLVMTypeRef personality_type =
      LLVMFunctionType(LLVMInt32Type(), NULL, 0, TRUE);
  ...
  if (!use_mono_personality_debug) {
    ...
    personality =
        get_callee(ctx, personality_type, MONO_PATCH_INFO_JIT_ICALL_ID,
...
static LLVMBasicBlockRef emit_landing_pad(EmitContext *ctx, int group_index,
                                          int group_size) {
  MonoCompile *cfg = ctx->cfg;
  LLVMBuilderRef old_builder = ctx->builder;
  ...
  LLVMValueRef landing_pad = LLVMBuildLandingPad(
      lpadBuilder, default_cpp_lpad_exc_signature(), personality, 0, "");
  g_assert(landing_pad);

  LLVMValueRef cast =
      LLVMBuildBitCast(lpadBuilder, ctx->module->sentinel_exception,
  ...
  for (int i = 0; i < group_size; i++) {
    if (!(group_cursor->flags & MONO_EXCEPTION_CLAUSE_FINALLY ||
          group_cursor->flags & MONO_EXCEPTION_CLAUSE_FAULT))
      finally_only = FALSE;
  ...
  if (!finally_only) {
    // So at each level of the exception stack we will match the exception
    // again. During that match, we need to compare against the handler types
    // for the current protected region. We send the try start and end so that
    // we can only check against handlers for this lexical protected region.
    LLVMValueRef match = mono_llvm_emit_match_exception_call(
        ctx, lpadBuilder, group_start->try_offset,
        group_start->try_offset + group_start->try_len);

    // if returns -1, resume
    LLVMValueRef switch_ins =
        LLVMBuildSwitch(lpadBuilder, match, resume_bb, group_size);

    // else move to that target bb
    for (int i = 0; i < group_size; i++) {
      MonoExceptionClause *clause = group_start + i;
      int clause_index = clause - cfg->header->clauses;
      MonoBasicBlock *handler_bb = (MonoBasicBlock *)g_hash_table_lookup(
          ctx->clause_to_handler, GINT_TO_POINTER(clause_index));
      g_assert(handler_bb);
      g_assert(ctx->bblocks[handler_bb->block_num].call_handler_target_bb);
      LLVMAddCase(switch_ins,
                  LLVMConstInt(LLVMInt32Type(), clause_index, FALSE),
                  ctx->bblocks[handler_bb->block_num].call_handler_target_bb);
    }
  } else {
    int clause_index = group_start - cfg->header->clauses;
    MonoBasicBlock *finally_bb = (MonoBasicBlock *)g_hash_table_lookup(
        ctx->clause_to_handler, GINT_TO_POINTER(clause_index));
    g_assert(finally_bb);

    LLVMBuildBr(ctx->builder,
                ctx->bblocks[finally_bb->block_num].call_handler_target_bb);
  }
...
static LLVMValueRef create_const_vector_i32(const int *mask, int count) {
...
static LLVMValueRef create_const_vector_4_i32(int v0, int v1, int v2, int v3) {
  LLVMValueRef mask[4];
...
static LLVMValueRef create_const_vector_2_i32(int v0, int v1) {
  LLVMValueRef mask[2];
  mask[0] = LLVMConstInt(LLVMInt32Type(), v0, FALSE);
  mask[1] = LLVMConstInt(LLVMInt32Type(), v1, FALSE);
...
static void emit_llvmonly_handler_start(EmitContext *ctx, MonoBasicBlock *bb,
  ...
  if (!(clause->flags & MONO_EXCEPTION_CLAUSE_FINALLY ||
        clause->flags & MONO_EXCEPTION_CLAUSE_FAULT)) {
    LLVMValueRef mono_exc =
        mono_llvm_emit_load_exception_call(ctx, ctx->builder);

    g_assert(ctx->ex_var);
    LLVMBuildStore(ctx->builder,
                   LLVMBuildBitCast(ctx->builder, mono_exc, ObjRefType(), ""),
                   ctx->ex_var);
    ...
    if (bb->in_scount == 1) {
      ...
      ctx->values[exvar->dreg] =
          LLVMBuildLoad(ctx->builder, ctx->ex_var, "save_exception");
  ...
  LLVMBasicBlockRef target_bb =
      ctx->bblocks[bb->block_num].call_handler_target_bb;
...
static void emit_handler_start(EmitContext *ctx, MonoBasicBlock *bb,
                               LLVMBuilderRef builder) {
  MonoCompile *cfg = ctx->cfg;
  LLVMValueRef *values = ctx->values;
  ...
  if (cfg->compile_aot) {
    ...
    LLVMTypeRef personality_type =
        LLVMFunctionType(LLVMInt32Type(), NULL, 0, TRUE);
    personality =
        LLVMAddFunction(ctx->lmodule, "mono_personality", personality_type);
  ...
  if (cfg->compile_aot) {
    ...
    LLVMSetInitializer(type_info,
                       LLVMConstInt(LLVMInt32Type(), clause_index, FALSE));

    /*
     * These symbols are not really used, the clause_index is embedded into the
     * EH tables generated by DwarfMonoException in LLVM.
     */
...

  {
    ...
    if (ctx->ex_var)
      LLVMBuildStore(
          builder,
          convert(ctx, LLVMBuildExtractValue(builder, landing_pad, 0, "ex_obj"),
                  ObjRefType()),
          ctx->ex_var);
  }
   ...
   */
  LLVMValueRef ex_selector =
      LLVMBuildExtractValue(builder, landing_pad, 1, "ex_selector");
  ...
  for (l = ctx->nested_in[clause_index]; l; l = l->next) {
    ...
    handler_bb = (MonoBasicBlock *)g_hash_table_lookup(
        ctx->clause_to_handler, GINT_TO_POINTER(nesting_clause_index));
    g_assert(handler_bb);

    g_assert(ctx->bblocks[handler_bb->block_num].call_handler_target_bb);
    LLVMAddCase(switch_ins,
                LLVMConstInt(LLVMInt32Type(), nesting_clause_index, FALSE),
                ctx->bblocks[handler_bb->block_num].call_handler_target_bb);
  }
...
static LLVMValueRef get_double_const(MonoCompile *cfg, double val) {
#ifdef TARGET_WASM
  if (mono_isnan(val))
...
static LLVMValueRef get_float_const(MonoCompile *cfg, float val) {
#ifdef TARGET_WASM
  if (mono_isnan(val))
    *(int *)&val = 0x7FC00000;
#endif
  if (cfg->r4fp)
    ...
    return LLVMConstFPExt(LLVMConstReal(LLVMFloatType(), val),
                          LLVMDoubleType());
}

static LLVMValueRef call_intrins(EmitContext *ctx, int id, LLVMValueRef *args,
                                 const char *name) {
...
static void process_bb(EmitContext *ctx, MonoBasicBlock *bb) {
  MonoCompile *cfg = ctx->cfg;
  MonoMethodSignature *sig = ctx->sig;
  ...
  for (ins = bb->code; ins; ins = ins->next) {
    ...
    for (i = 0; i < bb->in_count; ++i) {
      if (bb->in_bb[i]->last_ins &&
          bb->in_bb[i]->last_ins->opcode == OP_NOT_REACHED)
    ...
    for (i = 0; i < ins->inst_phi_args[0]; i++) {
       ...
       */
      if (bb->in_bb[i]->last_ins &&
          bb->in_bb[i]->last_ins->opcode == OP_SWITCH) {
      ...
      for (j = 0; j < count; ++j) {
        PhiNode *node =
            (PhiNode *)mono_mempool_alloc0(ctx->mempool, sizeof(PhiNode));
        node->bb = bb;
        node->phi = ins;
        node->in_bb = bb->in_bb[i];
        node->sreg = sreg1;
        bblocks[bb->in_bb[i]->block_num].phi_nodes = g_slist_prepend_mempool(
            ctx->mempool, bblocks[bb->in_bb[i]->block_num].phi_nodes, node);
      }
    }
  }
  ...
  for (ins = bb->code; ins; ins = ins->next) {
    ...
    if (nins > 1000) {
      /*
       * Some steps in llc are non-linear in the size of basic blocks, see
       * #5714. Start a new bblock. Prevent the bblocks to be merged by doing a
       * volatile load + cond branch from localloc-ed memory.
       */
      if (!cfg->llvm_only)
        ; // set_failure (ctx, "basic block too long");

      if (!ctx->long_bb_break_var) {
        ctx->long_bb_break_var = build_alloca_llvm_type_name(
            ctx, LLVMInt32Type(), 0, "long_bb_break");
        mono_llvm_build_store(ctx->alloca_builder,
                              LLVMConstInt(LLVMInt32Type(), 0, FALSE),
                              ctx->long_bb_break_var, TRUE, LLVM_BARRIER_NONE);
      }
      ...
      LLVMValueRef load =
          mono_llvm_build_load(builder, ctx->long_bb_break_var, "", TRUE);
      /*
       * The long_bb_break_var is initialized to 0 in the prolog, so this branch
       * will always go to 'cbb' but llvm doesn't know that, so the branch is
       * not going to be eliminated.
       */
      LLVMValueRef cmp =
          LLVMBuildICmp(builder, LLVMIntEQ, load,
                        LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");

      LLVMBuildCondBr(builder, cmp, cbb, dummy_bb);

      /* Emit a dummy false bblock which does nothing but contains a volatile
       * store so it cannot be eliminated */
      ctx->builder = builder = create_builder(ctx);
      LLVMPositionBuilderAtEnd(builder, dummy_bb);
      mono_llvm_build_store(builder, LLVMConstInt(LLVMInt32Type(), 1, FALSE),
                            ctx->long_bb_break_var, TRUE, LLVM_BARRIER_NONE);
...
#if TARGET_SIZEOF_VOID_P == 4
      values[ins->dreg] =
          LLVMConstInt(LLVMInt64Type(), GET_LONG_IMM(ins), FALSE);
#else
      values[ins->dreg] =
          LLVMConstInt(LLVMInt64Type(), (gint64)ins->inst_c0, FALSE);
#endif
      break;
    case OP_R8CONST:
    ...
    case OP_SWITCH: {
      ...
      for (i = 0; i < GPOINTER_TO_UINT(ins->klass); ++i) {
        ...
        LLVMAddCase(v, LLVMConstInt(LLVMInt32Type(), i, FALSE),
                    get_bb(ctx, target_bb));
      }
    ...
    case OP_SETRET:
      ...
      case LLVMArgVtypeInReg: {
        ...
        if (!addresses[ins->sreg1]) {
          /*
           * The return type is an LLVM vector type, have to convert between it
           * and the real return type which is a struct type.
           */
          g_assert(MONO_CLASS_IS_SIMD(
              ctx->cfg, mono_class_from_mono_type_internal(sig->ret)));
          /* Convert to 2xi64 first */
          val = LLVMBuildBitCast(builder, values[ins->sreg1],
            ...
            if (linfo->ret.pair_storage[i] == LLVMArgInIReg) {
              retval = LLVMBuildInsertValue(
                  ...
                  LLVMBuildExtractElement(
                      builder, val, LLVMConstInt(LLVMInt32Type(), i, FALSE),
                      ""),
                  i, "");
            } else {
          ...
          addr = LLVMBuildBitCast(builder, addresses[ins->sreg1],
                                  LLVMPointerType(ret_type, 0), "");
          for (i = 0; i < 2; ++i) {
            if (linfo->ret.pair_storage[i] == LLVMArgInIReg) {
              ...
              retval = LLVMBuildInsertValue(
                  builder, retval, LLVMBuildLoad(builder, part_addr, ""), i,
                  "");
            } else {
              g_assert(linfo->ret.pair_storage[i] == LLVMArgNone);
            }
      ...
      case LLVMArgVtypeAsScalar: {
        ...
        retval =
            LLVMBuildLoad(builder,
                          LLVMBuildBitCast(builder, addresses[ins->sreg1],
                                           LLVMPointerType(ret_type, 0), ""),
                          "");
        LLVMBuildRet(builder, retval);
        break;
      ...
      case LLVMArgGsharedvtFixed: {
        ...
        if (lhs) {
          ...
          LLVMBuildStore(builder, convert(ctx, lhs, ret_type),
                         convert(ctx, values[cfg->vret_addr->dreg],
                                 LLVMPointerType(ret_type, 0)));
        }
        LLVMBuildRetVoid(builder);
        break;
      ...
      case LLVMArgFpStruct: {
        ...
        g_assert(addresses[ins->sreg1]);
        retval = LLVMBuildLoad(
            builder,
            convert(ctx, addresses[ins->sreg1], LLVMPointerType(ret_type, 0)),
            "");
        LLVMBuildRet(builder, retval);
        break;
      }
      ...
      case LLVMArgNormal: {
        if (!lhs || ctx->is_dead[ins->sreg1]) {
          ...
          if (cfg->vret_addr)
            ...
            LLVMBuildRet(builder,
                         LLVMConstNull(type_to_llvm_type(ctx, sig->ret)));
        } else {
          LLVMBuildRet(builder,
                       convert(ctx, lhs, type_to_llvm_type(ctx, sig->ret)));
        }
        has_terminator = TRUE;
        break;
      }
    ...
    case OP_COMPARE_IMM: {
      ...
      if (ins->opcode == OP_FCOMPARE) {
        cmp = LLVMBuildFCmp(builder, fpcond_to_llvm_cond[rel],
                            convert(ctx, lhs, LLVMDoubleType()),
                            convert(ctx, rhs, LLVMDoubleType()), "");
      } else if (ins->opcode == OP_RCOMPARE) {
        cmp = LLVMBuildFCmp(builder, fpcond_to_llvm_cond[rel],
                            convert(ctx, lhs, LLVMFloatType()),
                            convert(ctx, rhs, LLVMFloatType()), "");
        ...
        if (LLVMGetTypeKind(LLVMTypeOf(lhs)) == LLVMPointerTypeKind &&
          ...
          if (nonnull && llvm_pred == LLVMIntEQ)
            ...
            cmp = LLVMBuildICmp(builder, llvm_pred, lhs,
                                LLVMConstNull(LLVMTypeOf(lhs)), "");

        } else {
          cmp = LLVMBuildICmp(
              builder, llvm_pred, convert(ctx, lhs, IntPtrType()),
              LLVMConstInt(IntPtrType(), ins->inst_imm, FALSE), "");
        }
        ...
        if (LLVMGetTypeKind(LLVMTypeOf(lhs)) == LLVMPointerTypeKind &&
          ...
          cmp = LLVMBuildICmp(builder, cond_to_llvm_cond[rel],
                              convert(ctx, lhs, IntPtrType()),
                              convert(ctx, rhs, IntPtrType()), "");
      } else
        cmp = LLVMBuildICmp(builder, cond_to_llvm_cond[rel], lhs, rhs, "");
      ...
      if (MONO_IS_COND_BRANCH_OP(ins->next)) {
        if (ins->next->inst_true_bb == ins->next->inst_false_bb) {
          ...
          LLVMBuildCondBr(builder, cmp, get_bb(ctx, ins->next->inst_true_bb),
                          get_bb(ctx, ins->next->inst_false_bb));
        }
        has_terminator = TRUE;
      } else if (MONO_IS_SETCC(ins->next)) {
        sprintf(dname_buf, "t%d", ins->next->dreg);
        dname = dname_buf;
        values[ins->next->dreg] =
            LLVMBuildZExt(builder, cmp, LLVMInt32Type(), dname);
        ...
        emit_cond_system_exception(ctx, bb, (const char *)ins->next->inst_p1,
                                   cmp, force_explicit_branch);
        if (!ctx_ok(ctx))
          break;
        builder = ctx->builder;
      } else {
    ...
    case OP_MOVE_F_TO_I4: {
      values[ins->dreg] = LLVMBuildBitCast(
          builder, LLVMBuildFPTrunc(builder, lhs, LLVMFloatType(), ""),
          LLVMInt32Type(), "");
      break;
    }
    case OP_MOVE_I4_TO_F: {
      values[ins->dreg] = LLVMBuildFPExt(
          builder, LLVMBuildBitCast(builder, lhs, LLVMFloatType(), ""),
          LLVMDoubleType(), "");
      break;
    }
...
#if TARGET_SIZEOF_VOID_P == 4
      if (ins->opcode == OP_LSHL_IMM || ins->opcode == OP_LSHR_IMM ||
          ins->opcode == OP_LSHR_UN_IMM)
        imm = LLVMConstInt(LLVMInt32Type(), ins->inst_imm, FALSE);
#endif
    ...
    case OP_INEG:
      values[ins->dreg] =
          LLVMBuildSub(builder, LLVMConstInt(LLVMInt32Type(), 0, FALSE),
                       convert(ctx, lhs, LLVMInt32Type()), dname);
      break;
    case OP_LNEG:
      if (LLVMTypeOf(lhs) != LLVMInt64Type())
        lhs = convert(ctx, lhs, LLVMInt64Type());
      values[ins->dreg] = LLVMBuildSub(
          builder, LLVMConstInt(LLVMInt64Type(), 0, FALSE), lhs, dname);
      break;
    case OP_FNEG:
    ...
    case OP_INOT: {
      ...
      values[ins->dreg] =
          LLVMBuildXor(builder, LLVMConstInt(LLVMInt32Type(), v, FALSE),
                       convert(ctx, lhs, LLVMInt32Type()), dname);
      break;
    }
    case OP_LNOT: {
      ...
      values[ins->dreg] = LLVMBuildXor(
          builder, LLVMConstInt(LLVMInt64Type(), v, FALSE), lhs, dname);
      break;
    }
#if defined(TARGET_X86) || defined(TARGET_AMD64)
    case OP_X86_LEA: {
      ...
      rhs = LLVMBuildSExt(builder, convert(ctx, rhs, LLVMInt32Type()),
                          LLVMInt64Type(), "");

      v1 = LLVMBuildMul(
          builder, convert(ctx, rhs, IntPtrType()),
          LLVMConstInt(IntPtrType(),
                       ((unsigned long long)1 << ins->backend.shift_amount),
                       FALSE),
          "");
      v2 = LLVMBuildAdd(builder, convert(ctx, lhs, IntPtrType()), v1, "");
      values[ins->dreg] = LLVMBuildAdd(
          builder, v2, LLVMConstInt(IntPtrType(), ins->inst_imm, FALSE), dname);
      break;
    }
#endif
    ...
    case OP_LCONV_TO_U4: {
      ...
      sign = (ins->opcode == OP_ICONV_TO_I1) ||
             (ins->opcode == OP_ICONV_TO_I2) ||
             (ins->opcode == OP_ICONV_TO_I4) ||
             (ins->opcode == OP_LCONV_TO_I1) || (ins->opcode == OP_LCONV_TO_I2);
    ...
    case OP_FCONV_TO_I1:
    case OP_RCONV_TO_I1:
      values[ins->dreg] = LLVMBuildSExt(
          builder, LLVMBuildFPToSI(builder, lhs, LLVMInt8Type(), dname),
          LLVMInt32Type(), "");
      break;
    case OP_FCONV_TO_U1:
    case OP_RCONV_TO_U1:
      values[ins->dreg] = LLVMBuildZExt(
          builder,
          LLVMBuildTrunc(builder,
                         LLVMBuildFPToUI(builder, lhs, IntPtrType(), dname),
                         LLVMInt8Type(), ""),
          LLVMInt32Type(), "");
      break;
    case OP_FCONV_TO_I2:
    case OP_RCONV_TO_I2:
      values[ins->dreg] = LLVMBuildSExt(
          builder, LLVMBuildFPToSI(builder, lhs, LLVMInt16Type(), dname),
          LLVMInt32Type(), "");
      break;
    case OP_FCONV_TO_U2:
    case OP_RCONV_TO_U2:
      values[ins->dreg] = LLVMBuildZExt(
          builder, LLVMBuildFPToUI(builder, lhs, LLVMInt16Type(), dname),
          LLVMInt32Type(), "");
      break;
    case OP_FCONV_TO_U4:
    case OP_RCONV_TO_U4:
    ...
    case OP_LCONV_TO_R8:
      values[ins->dreg] =
          LLVMBuildSIToFP(builder, lhs, LLVMDoubleType(), dname);
      break;
    case OP_ICONV_TO_R_UN:
    case OP_LCONV_TO_R_UN:
      values[ins->dreg] =
          LLVMBuildUIToFP(builder, lhs, LLVMDoubleType(), dname);
      break;
#if TARGET_SIZEOF_VOID_P == 4
    case OP_LCONV_TO_U:
#endif
    ...
    case OP_SEXT_I4:
      values[ins->dreg] = LLVMBuildSExt(
          builder, convert(ctx, lhs, LLVMInt32Type()), LLVMInt64Type(), dname);
      break;
    case OP_ZEXT_I4:
      values[ins->dreg] = LLVMBuildZExt(
          builder, convert(ctx, lhs, LLVMInt32Type()), LLVMInt64Type(), dname);
    ...
    case OP_LOCALLOC_IMM: {
      ...
      v = mono_llvm_build_alloca(builder, LLVMInt8Type(),
                                 LLVMConstInt(LLVMInt32Type(), size, FALSE),
                                 MONO_ARCH_FRAME_ALIGNMENT, "");

      if (ins->flags & MONO_INST_INIT)
        emit_memset(ctx, builder, v, const_int32(size),
                    MONO_ARCH_FRAME_ALIGNMENT);
    ...
    case OP_LOCALLOC: {
      ...
      size = LLVMBuildAnd(
          builder,
          LLVMBuildAdd(builder, convert(ctx, lhs, LLVMInt32Type()),
                       LLVMConstInt(LLVMInt32Type(),
                                    MONO_ARCH_FRAME_ALIGNMENT - 1, FALSE),
                       ""),
          LLVMConstInt(LLVMInt32Type(), ~(MONO_ARCH_FRAME_ALIGNMENT - 1),
                       FALSE),
          "");

      v = mono_llvm_build_alloca(builder, LLVMInt8Type(), size,
                                 MONO_ARCH_FRAME_ALIGNMENT, "");
    ...
    case OP_LOAD_MEM: {
      ...
      if ((ins->opcode == OP_LOADI8_MEM) || (ins->opcode == OP_LOAD_MEM) ||
          (ins->opcode == OP_LOADI4_MEM) || (ins->opcode == OP_LOADU4_MEM) ||
          (ins->opcode == OP_LOADU1_MEM) || (ins->opcode == OP_LOADU2_MEM)) {
        ...
        if (ins->inst_offset == 0) {
          ...
          if (mono_llvm_can_be_gep(base, &gep_base, &gep_offset)) {
            addr = LLVMBuildGEP(
                builder,
                convert(ctx, gep_base, LLVMPointerType(LLVMInt8Type(), 0)),
                &gep_offset, 1, "");
          } else {
            addr = base;
          }
          ...
          addr = LLVMBuildGEP(
              builder, convert(ctx, base, LLVMPointerType(LLVMInt8Type(), 0)),
              &index, 1, "");
        } else {
          index = LLVMConstInt(LLVMInt32Type(), ins->inst_offset / size, FALSE);
          addr =
              LLVMBuildGEP(builder, convert(ctx, base, LLVMPointerType(t, 0)),
                           &index, 1, "");
        }
      }
      ...
      if (is_unaligned)
        values[ins->dreg] =
            mono_llvm_build_aligned_load(builder, addr, dname, is_volatile, 1);
      else
        values[ins->dreg] =
            emit_load(ctx, bb, &builder, size, addr, base, dname, is_faulting,
                      is_volatile, LLVM_BARRIER_NONE);

      if (!(is_faulting || is_volatile) &&
      ...
      if (sext)
        values[ins->dreg] =
            LLVMBuildSExt(builder, values[ins->dreg], LLVMInt32Type(), dname);
      else if (zext)
        values[ins->dreg] =
            LLVMBuildZExt(builder, values[ins->dreg], LLVMInt32Type(), dname);
      else if (!cfg->r4fp && ins->opcode == OP_LOADR4_MEMBASE)
        values[ins->dreg] =
            LLVMBuildFPExt(builder, values[ins->dreg], LLVMDoubleType(), dname);
      break;
    }
    ...
    case OP_STORE_MEMBASE_REG: {
      ...
      LLVMValueRef gep_base, gep_offset;
      if (ins->inst_offset == 0 &&
          mono_llvm_can_be_gep(base, &gep_base, &gep_offset)) {
        addr = LLVMBuildGEP(
            builder, convert(ctx, gep_base, LLVMPointerType(LLVMInt8Type(), 0)),
            &gep_offset, 1, "");
      } else if (ins->inst_offset % size != 0) {
        /* Unaligned store */
        index = LLVMConstInt(LLVMInt32Type(), ins->inst_offset, FALSE);
        ...
        index = LLVMConstInt(LLVMInt32Type(), ins->inst_offset / size, FALSE);
        addr = LLVMBuildGEP(builder, convert(ctx, base, LLVMPointerType(t, 0)),
                            &index, 1, "");
      }
      if (is_volatile && LLVMGetInstructionOpcode(base) == LLVMAlloca &&
      ...
      if (is_unaligned)
        ...
        emit_store(ctx, bb, &builder, size, srcval, ptrdst, base, is_faulting,
                   is_volatile);
      break;
    }
    ...
    case OP_STORE_MEMBASE_IMM: {
      ...
      LLVMValueRef srcval =
          convert(ctx, LLVMConstInt(IntPtrType(), ins->inst_imm, FALSE), t);
      LLVMValueRef ptrdst = convert(ctx, addr, LLVMPointerType(t, 0));
      if (is_unaligned)
        mono_llvm_build_aligned_store(builder, srcval, ptrdst, is_volatile, 1);
      else
    ...
    case OP_CHECK_THIS:
      emit_load(ctx, bb, &builder, TARGET_SIZEOF_VOID_P,
                convert(ctx, lhs, LLVMPointerType(IntPtrType(), 0)), lhs, "",
                TRUE, FALSE, LLVM_BARRIER_NONE);
      break;
    ...
    case OP_AOTCONST: {
      ...
      if (ji_type == MONO_PATCH_INFO_ICALL_ADDR) {
        char *symbol = mono_aot_get_direct_call_symbol(
            MONO_PATCH_INFO_ICALL_ADDR_CALL, ji_data);
        if (symbol) {
          /*
           * Avoid emitting a got entry for these since the method is directly
           * called, and it might not be resolvable at runtime using dlsym ().
           */
          g_free(symbol);
      ...
      values[ins->dreg] =
          get_aotconst(ctx, ji_type, ji_data, LLVMPointerType(IntPtrType(), 0));
      break;
    }
    case OP_MEMMOVE: {
      int argn = 0;
      LLVMValueRef args[5];
      args[argn++] =
          convert(ctx, values[ins->sreg1], LLVMPointerType(LLVMInt8Type(), 0));
      args[argn++] =
          convert(ctx, values[ins->sreg2], LLVMPointerType(LLVMInt8Type(), 0));
    ...
    /*
     * See the ARM64 comment in mono/utils/atomic.h for an explanation of why
...
#define ARM64_ATOMIC_FENCE_FIX
    ...
    case OP_ATOMIC_EXCHANGE_I8: {
      ...
      values[ins->dreg] = mono_llvm_build_atomic_rmw(
          builder, LLVM_ATOMICRMW_OP_XCHG, args[0], args[1]);
      ARM64_ATOMIC_FENCE_FIX;
      break;
    }
    ...
    case OP_ATOMIC_OR_I8: {
      ...
      if (ins->opcode == OP_ATOMIC_ADD_I4 || ins->opcode == OP_ATOMIC_ADD_I8)
        // Interlocked.Add returns new value (that's why we emit additional Add
        // here) see https://github.com/dotnet/runtime/pull/33102
        values[ins->dreg] =
            LLVMBuildAdd(builder,
                         mono_llvm_build_atomic_rmw(
                             builder, LLVM_ATOMICRMW_OP_ADD, args[0], args[1]),
                         args[1], dname);
      else if (ins->opcode == OP_ATOMIC_AND_I4 ||
               ins->opcode == OP_ATOMIC_AND_I8)
        values[ins->dreg] = mono_llvm_build_atomic_rmw(
            builder, LLVM_ATOMICRMW_OP_AND, args[0], args[1]);
      else if (ins->opcode == OP_ATOMIC_OR_I4 || ins->opcode == OP_ATOMIC_OR_I8)
        values[ins->dreg] = mono_llvm_build_atomic_rmw(
            builder, LLVM_ATOMICRMW_OP_OR, args[0], args[1]);
      else
        g_assert_not_reached();
      ARM64_ATOMIC_FENCE_FIX;
      break;
    ...
    case OP_MEMORY_BARRIER: {
      mono_llvm_build_fence(builder,
                            (BarrierKind)ins->backend.memory_barrier_kind);
      break;
    }
    case OP_ATOMIC_LOAD_I1:
    case OP_ATOMIC_LOAD_I2:
    ...
    case OP_ATOMIC_LOAD_R8: {
      ...
      if (ins->inst_offset != 0) {
        index = LLVMConstInt(LLVMInt32Type(), ins->inst_offset / size, FALSE);
        addr = LLVMBuildGEP(builder, convert(ctx, lhs, LLVMPointerType(t, 0)),
                            &index, 1, "");
      } else {
        addr = lhs;
      }
      ...
      ARM64_ATOMIC_FENCE_FIX;
      values[ins->dreg] = emit_load(ctx, bb, &builder, size, addr, lhs, dname,
                                    is_faulting, is_volatile, barrier);
      ARM64_ATOMIC_FENCE_FIX;
    ...
    case OP_ATOMIC_STORE_R8: {
      ...
      ARM64_ATOMIC_FENCE_FIX;
      emit_store_general(ctx, bb, &builder, size, value, addr, base,
                         is_faulting, is_volatile, barrier);
      ARM64_ATOMIC_FENCE_FIX;
      break;
    }
    ...
    case OP_TLS_GET: {
      ...
      values[ins->dreg] = LLVMBuildLoad(
          ...
          LLVMBuildIntToPtr(builder,
                            LLVMConstInt(IntPtrType(), ins->inst_offset, TRUE),
                            ptrtype, ""),
          "");
      ...
      values[ins->dreg] = LLVMBuildLoad(
          builder,
          LLVMBuildIntToPtr(builder, LLVMConstInt(IntPtrType(), offset, TRUE),
                            ptrtype, ""),
          "");
#else
    ...
    case OP_GC_SAFE_POINT: {
      ...
      /*
       * Create the cold wrapper around the icall, along with a managed method
       * for it so unwinding works.
       */
      if (!cfg->compile_aot && !ctx->module->gc_poll_cold_wrapper_compiled) {
        ...
        MonoMethod *wrapper =
            mono_marshal_get_llvm_func_wrapper(LLVM_FUNC_WRAPPER_GC_POLL);
        ctx->module->gc_poll_cold_wrapper_compiled =
            mono_jit_compile_method(wrapper, error);
        mono_error_assert_ok(error);
      }
       ...
       */
      val = mono_llvm_build_load(
          builder, convert(ctx, lhs, LLVMPointerType(IntPtrType(), 0)), "",
          TRUE);
      cmp = LLVMBuildICmp(builder, LLVMIntEQ, val,
      ...
      if (ctx->cfg->compile_aot) {
        callee = get_callee(
            ctx, sig, MONO_PATCH_INFO_JIT_ICALL_ID,
            GUINT_TO_POINTER(MONO_JIT_ICALL_mono_threads_state_poll));
        call = LLVMBuildCall(builder, callee, NULL, 0, "");
      } else {
        callee = get_jit_callee(ctx, icall_name, sig, MONO_PATCH_INFO_ABS,
                                ctx->module->gc_poll_cold_wrapper_compiled);
        call = LLVMBuildCall(builder, callee, NULL, 0, "");
        set_call_cold_cconv(call);
      }
    ...
    case OP_VZERO: {
      ...
      LLVMValueRef ptr =
          LLVMBuildBitCast(builder, addresses[ins->dreg],
                           LLVMPointerType(LLVMInt8Type(), 0), "");
      emit_memset(ctx, builder, ptr,
                  const_int32(mono_class_value_size(klass, NULL)), 0);
      break;
    }
    case OP_DUMMY_VZERO:
      break;
    ...
    case OP_VMOVE: {
      ...
      switch (ins->opcode) {
      case OP_STOREV_MEMBASE:
        if (cfg->gen_write_barriers && m_class_has_references(klass) &&
            ins->inst_destbasereg != cfg->frame_reg &&
            LLVMGetInstructionOpcode(values[ins->inst_destbasereg]) !=
                LLVMAlloca) {
          /* Decomposed earlier */
          g_assert_not_reached();
          break;
        ...
        if (!addresses[ins->sreg1]) {
          ...
          dst = convert(
              ...
              LLVMBuildAdd(
                  builder,
                  convert(ctx, values[ins->inst_destbasereg], IntPtrType()),
                  LLVMConstInt(IntPtrType(), ins->inst_offset, FALSE), ""),
              LLVMPointerType(
                  type_to_llvm_type(ctx, m_class_get_byval_arg(klass)), 0));
          LLVMBuildStore(builder, values[ins->sreg1], dst);
          done = TRUE;
        } else {
          src = LLVMBuildBitCast(builder, addresses[ins->sreg1],
                                 LLVMPointerType(LLVMInt8Type(), 0), "");
          dst = convert(
              ctx,
              LLVMBuildAdd(
      ...
      case OP_LOADV_MEMBASE:
        if (!addresses[ins->dreg])
          addresses[ins->dreg] =
              build_alloca(ctx, m_class_get_byval_arg(klass));
        src = convert(
            ctx,
            LLVMBuildAdd(
                builder, convert(ctx, values[ins->inst_basereg], IntPtrType()),
                LLVMConstInt(IntPtrType(), ins->inst_offset, FALSE), ""),
            LLVMPointerType(LLVMInt8Type(), 0));
        dst = LLVMBuildBitCast(builder, addresses[ins->dreg],
                               LLVMPointerType(LLVMInt8Type(), 0), "");
        break;
      case OP_VMOVE:
        if (!addresses[ins->sreg1])
      ...
      args[aindex++] = dst;
      args[aindex++] = src;
      args[aindex++] = LLVMConstInt(LLVMInt32Type(),
                                    mono_class_value_size(klass, NULL), FALSE);
#if LLVM_API_VERSION < 900
      // FIXME: Alignment
    ...
    case OP_LLVM_OUTARG_VT: {
      ...
      if (ainfo->storage == LLVMArgGsharedvtVariable) {
        ...
        if (var && var->opcode == OP_GSHAREDVT_LOCAL) {
          addresses[ins->dreg] =
              convert(ctx, emit_gsharedvt_ldaddr(ctx, var->dreg),
                      LLVMPointerType(IntPtrType(), 0));
        } else {
          g_assert(addresses[ins->sreg1]);
        ...
        LLVMBuildStore(
            builder,
            convert(ctx, values[ins->sreg1],
                    LLVMGetElementType(LLVMTypeOf(addresses[ins->sreg1]))),
            addresses[ins->sreg1]);
        addresses[ins->dreg] = addresses[ins->sreg1];
      } else {
        if (!addresses[ins->sreg1]) {
          ...
          LLVMBuildStore(
              builder,
              convert(ctx, values[ins->sreg1], type_to_llvm_type(ctx, t)),
              addresses[ins->sreg1]);
          addresses[ins->dreg] = addresses[ins->sreg1];
        } else if (ainfo->storage == LLVMArgVtypeAddr ||
                   values[ins->sreg1] == addresses[ins->sreg1]) {
          ...
          LLVMBuildStore(builder, convert(ctx, v, type_to_llvm_type(ctx, t)),
                         addresses[ins->dreg]);
        } else {
          addresses[ins->dreg] = addresses[ins->sreg1];
        }
    ...
    case OP_OBJC_GET_SELECTOR: {
      ...
      if (!ctx->module->objc_selector_to_var) {
        ctx->module->objc_selector_to_var =
            g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);

        LLVMValueRef info_var = LLVMAddGlobal(
            ctx->lmodule, LLVMArrayType(LLVMInt8Type(), 8), "@OBJC_IMAGE_INFO");
        int32_t objc_imageinfo[] = {0, 16};
        LLVMSetInitializer(info_var, mono_llvm_create_constant_data_array(
                                         (uint8_t *)&objc_imageinfo, 8));
        LLVMSetLinkage(info_var, LLVMPrivateLinkage);
        LLVMSetExternallyInitialized(info_var, TRUE);
        LLVMSetSection(info_var,
                       "__DATA, __objc_imageinfo,regular,no_dead_strip");
        LLVMSetAlignment(info_var, sizeof(target_mgreg_t));
        mark_as_used(ctx->module, info_var);
      }

      var = (LLVMValueRef)g_hash_table_lookup(ctx->module->objc_selector_to_var,
                                              name);
      if (!var) {
        LLVMValueRef indexes[16];

        LLVMValueRef name_var = LLVMAddGlobal(
            ctx->lmodule, LLVMArrayType(LLVMInt8Type(), strlen(name) + 1),
            "@OBJC_METH_VAR_NAME_");
        LLVMSetInitializer(name_var,
                           mono_llvm_create_constant_data_array(
        ...
        LLVMValueRef ref_var =
            LLVMAddGlobal(ctx->lmodule, LLVMPointerType(LLVMInt8Type(), 0),
        ...
        LLVMSetSection(
            ref_var, "__DATA, __objc_selrefs, literal_pointers, no_dead_strip");
        LLVMSetAlignment(ref_var, sizeof(target_mgreg_t));
        mark_as_used(ctx->module, ref_var);

        g_hash_table_insert(ctx->module->objc_selector_to_var, g_strdup(name),
                            ref_var);
        var = ref_var;
      }
    ...
    case OP_XZERO: {
      values[ins->dreg] = LLVMConstNull(
          type_to_llvm_type(ctx, m_class_get_byval_arg(ins->klass)));
      break;
    }
    case OP_LOADX_MEMBASE: {
      ...
      src = convert(
          ctx,
          LLVMBuildAdd(builder,
                       convert(ctx, values[ins->inst_basereg], IntPtrType()),
                       LLVMConstInt(IntPtrType(), ins->inst_offset, FALSE), ""),
          LLVMPointerType(t, 0));
      values[ins->dreg] =
          mono_llvm_build_aligned_load(builder, src, "", FALSE, 1);
      break;
    }
      ...
      dest =
          convert(ctx,
                  LLVMBuildAdd(
                      ...
                      LLVMConstInt(IntPtrType(), ins->inst_offset, FALSE), ""),
                  LLVMPointerType(t, 0));
      mono_llvm_build_aligned_store(builder, values[ins->sreg1], dest, FALSE,
                                    1);
      break;
    }
    case OP_PADDB:
    ...
    case OP_PMULW_HIGH_UN: {
      ...
      values[ins->dreg] =
          call_intrins(ctx, simd_ins_to_intrins(ins->opcode), args, "");
      break;
    }
    case OP_PAVGB_UN:
    case OP_PAVGW_UN: {
      ...
      LLVMTypeRef ext_elem_type =
          vector_size == 16 ? LLVMInt16Type() : LLVMInt32Type();
      ...
      val = LLVMBuildAdd(builder, LLVMBuildZExt(builder, lhs, ext_type, ""),
                         LLVMBuildZExt(builder, rhs, ext_type, ""), "");
    ...
    case OP_EXTRACT_U1: {
      ...
      lhs = LLVMBuildBitCast(builder, lhs, t, "");
      values[ins->dreg] = LLVMBuildExtractElement(
          builder, lhs, LLVMConstInt(LLVMInt32Type(), ins->inst_c0, FALSE), "");
      if (zext)
        values[ins->dreg] =
            LLVMBuildZExt(builder, values[ins->dreg], LLVMInt32Type(), "");
      break;
    }
    ...
    case OP_EXPAND_R8: {
      ...
      values[ins->dreg] =
          LLVMBuildInsertElement(builder, LLVMConstNull(t), v,
                                 LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      values[ins->dreg] = LLVMBuildShuffleVector(
          builder, values[ins->dreg], LLVMGetUndef(t),
          LLVMConstVector(mask, LLVMGetVectorSize(t)), "");
      break;
    }

    case OP_INSERT_I1:
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, values[ins->sreg1],
          convert(ctx, values[ins->sreg2], LLVMInt8Type()),
          LLVMConstInt(LLVMInt32Type(), ins->inst_c0, FALSE), dname);
      break;
    case OP_INSERT_I2:
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, values[ins->sreg1],
          convert(ctx, values[ins->sreg2], LLVMInt16Type()),
          LLVMConstInt(LLVMInt32Type(), ins->inst_c0, FALSE), dname);
      break;
    case OP_INSERT_I4:
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, values[ins->sreg1],
          convert(ctx, values[ins->sreg2], LLVMInt32Type()),
          LLVMConstInt(LLVMInt32Type(), ins->inst_c0, FALSE), dname);
      break;
    case OP_INSERT_I8:
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, values[ins->sreg1],
          convert(ctx, values[ins->sreg2], LLVMInt64Type()),
          LLVMConstInt(LLVMInt32Type(), ins->inst_c0, FALSE), dname);
      break;
    case OP_INSERT_R4:
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, values[ins->sreg1],
          convert(ctx, values[ins->sreg2], LLVMFloatType()),
          LLVMConstInt(LLVMInt32Type(), ins->inst_c0, FALSE), dname);
      break;
    case OP_INSERT_R8:
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, values[ins->sreg1],
          convert(ctx, values[ins->sreg2], LLVMDoubleType()),
          LLVMConstInt(LLVMInt32Type(), ins->inst_c0, FALSE), dname);
      break;
    case OP_XINSERT_I2: {
      ...
      /*
       * Many SIMD opcodes require an immediate operand, but can be called with
       * a non-immediate. To handle these cases, generate a switch statement
       * with one case for all possible values of the immediate. switch (index)
       * { case i: res = <op> (val, i) break;
       * }
       */
      g_assert(nelems <= 64);
      for (i = 0; i < nelems; ++i)
      ...
      switch_ins = LLVMBuildSwitch(
          builder, LLVMBuildAnd(builder, index, const_int32(0xf), ""), bbs[0],
          0);
      for (i = 0; i < nelems; ++i) {
        LLVMAddCase(switch_ins, LLVMConstInt(LLVMInt32Type(), i, FALSE),
                    bbs[i]);
        LLVMPositionBuilderAtEnd(builder, bbs[i]);
        phi_values[i] = LLVMBuildInsertElement(
            builder, vector,
            convert(ctx, value, LLVMGetElementType(LLVMTypeOf(vector))),
    ...
    case OP_CVTDQ2PD: {
      ...
      LLVMValueRef shuffle = LLVMBuildShuffleVector(
          builder, lhs, LLVMConstNull(LLVMTypeOf(lhs)), mask, "");
      values[ins->dreg] = LLVMBuildSIToFP(
          builder, shuffle, LLVMVectorType(LLVMDoubleType(), 2), dname);
      break;
    }
    case OP_SSE2_CVTSS2SD: {
      LLVMValueRef rhs_elem =
          LLVMBuildExtractElement(builder, rhs, const_int32(0), "");
      LLVMValueRef fpext =
          LLVMBuildFPExt(builder, rhs_elem, LLVMDoubleType(), dname);
      values[ins->dreg] =
          LLVMBuildInsertElement(builder, lhs, fpext, const_int32(0), "");
      break;
    }
    case OP_CVTPS2PD: {
      LLVMValueRef indexes[16];
    ...
    case OP_CVTTPS2DQ:
      values[ins->dreg] = LLVMBuildFPToSI(
          builder, lhs, LLVMVectorType(LLVMInt32Type(), 4), dname);
      break;
    ...
    case OP_RCPPS: {
      ...
      values[ins->dreg] =
          call_intrins(ctx, simd_ins_to_intrins(ins->opcode), &v, dname);
      break;
    }
    case OP_COMPPS:
    case OP_COMPPD: {
      ...
      if (ins->opcode == OP_COMPPD)
        values[ins->dreg] = LLVMBuildBitCast(
            builder,
            LLVMBuildSExt(builder, cmp, LLVMVectorType(LLVMInt64Type(), 2), ""),
            LLVMTypeOf(lhs), "");
      else
        values[ins->dreg] = LLVMBuildBitCast(
            builder,
            LLVMBuildSExt(builder, cmp, LLVMVectorType(LLVMInt32Type(), 4), ""),
            LLVMTypeOf(lhs), "");
      break;
    }
    case OP_ICONV_TO_X:
    ...
    case OP_PSHLQ: {
      ...
      values[ins->dreg] =
          call_intrins(ctx, simd_ins_to_intrins(ins->opcode), args, dname);
      break;
    }
    ...
    case OP_PSHUFLEW_HIGH: {
      ...
      values[ins->dreg] = LLVMBuildShuffleVector(
          builder, v1, v2, LLVMConstVector(mask_values, mask_size), dname);
      break;
    }
    ...
    case OP_UNPACK_HIGHPD: {
      ...
      values[ins->dreg] = LLVMBuildShuffleVector(
          builder, values[ins->sreg1], values[ins->sreg2],
    ...
    case OP_DUPPD: {
      ...
      v = LLVMBuildExtractElement(builder, lhs,
                                  LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      val = LLVMConstNull(t);
      val = LLVMBuildInsertElement(builder, val, v,
                                   LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      val = LLVMBuildInsertElement(
          builder, val, v, LLVMConstInt(LLVMInt32Type(), 1, FALSE), dname);
    ...
    case OP_DUPPS_HIGH: {
      ...
      if (ins->opcode == OP_DUPPS_LOW) {
        v1 = LLVMBuildExtractElement(
            builder, lhs, LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
        v2 = LLVMBuildExtractElement(
            builder, lhs, LLVMConstInt(LLVMInt32Type(), 2, FALSE), "");
      } else {
        v1 = LLVMBuildExtractElement(
            builder, lhs, LLVMConstInt(LLVMInt32Type(), 1, FALSE), "");
        v2 = LLVMBuildExtractElement(
            builder, lhs, LLVMConstInt(LLVMInt32Type(), 3, FALSE), "");
      }
      val = LLVMConstNull(t);
      val = LLVMBuildInsertElement(builder, val, v1,
                                   LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      val = LLVMBuildInsertElement(builder, val, v1,
                                   LLVMConstInt(LLVMInt32Type(), 1, FALSE), "");
      val = LLVMBuildInsertElement(builder, val, v2,
    ...
    case OP_DPPS: {
      ...
      args[0] = lhs;
      args[1] = rhs;
      /* 0xf1 == multiply all 4 elements, add them together, and store the
       * result to the lowest element */
    ...
    case OP_FCONV_TO_R8_X: {
      values[ins->dreg] =
          LLVMBuildInsertElement(builder, LLVMConstNull(sse_r8_t), lhs,
                                 LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      break;
    }
    ...
    case OP_FCONV_TO_R4_X: {
      values[ins->dreg] =
          LLVMBuildInsertElement(builder, LLVMConstNull(sse_r4_t), lhs,
                                 LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      break;
    }
    ...
    case OP_SSE_MOVMSK: {
      ...
      if (ins->inst_c1 == MONO_TYPE_R4) {
        args[0] = lhs;
        values[ins->dreg] =
            call_intrins(ctx, INTRINS_SSE_MOVMSK_PS, args, dname);
      } else if (ins->inst_c1 == MONO_TYPE_R8) {
        args[0] = lhs;
        values[ins->dreg] =
            call_intrins(ctx, INTRINS_SSE_MOVMSK_PD, args, dname);
      } else {
        args[0] = convert(ctx, lhs, sse_i1_t);
        values[ins->dreg] =
            call_intrins(ctx, INTRINS_SSE_PMOVMSKB, args, dname);
      }
      break;
    }
    ...
    case OP_SSE_MOVS2: {
      if (ins->inst_c1 == MONO_TYPE_R4)
        values[ins->dreg] = LLVMBuildShuffleVector(
            builder, rhs, lhs, create_const_vector_4_i32(0, 5, 6, 7), "");
      else if (ins->inst_c1 == MONO_TYPE_R8)
        values[ins->dreg] = LLVMBuildShuffleVector(
            builder, rhs, lhs, create_const_vector_2_i32(0, 3), "");
      else if (ins->inst_c1 == MONO_TYPE_I8 || ins->inst_c1 == MONO_TYPE_U8)
        values[ins->dreg] = LLVMBuildInsertElement(
            builder, lhs, LLVMConstInt(LLVMInt64Type(), 0, FALSE),
            LLVMConstInt(LLVMInt32Type(), 1, FALSE), "");
      else
    ...
    case OP_SSE_MOVEHL: {
      if (ins->inst_c1 == MONO_TYPE_R4)
        values[ins->dreg] = LLVMBuildShuffleVector(
            builder, lhs, rhs, create_const_vector_4_i32(6, 7, 2, 3), "");
      else
        g_assert_not_reached();
      break;
    }
    ...
    case OP_SSE_MOVELH: {
      if (ins->inst_c1 == MONO_TYPE_R4)
        values[ins->dreg] = LLVMBuildShuffleVector(
            builder, lhs, rhs, create_const_vector_4_i32(0, 1, 4, 5), "");
      else
        g_assert_not_reached();
      break;
    }
    ...
    case OP_SSE_UNPACKLO: {
      if (ins->inst_c1 == MONO_TYPE_R8 || ins->inst_c1 == MONO_TYPE_I8 ||
          ins->inst_c1 == MONO_TYPE_U8) {
        values[ins->dreg] = LLVMBuildShuffleVector(
            builder, lhs, rhs, create_const_vector_2_i32(0, 2), "");
      } else if (ins->inst_c1 == MONO_TYPE_R4 || ins->inst_c1 == MONO_TYPE_I4 ||
                 ins->inst_c1 == MONO_TYPE_U4) {
        values[ins->dreg] = LLVMBuildShuffleVector(
            builder, lhs, rhs, create_const_vector_4_i32(0, 4, 1, 5), "");
        ...
        LLVMValueRef shuffled = LLVMBuildShuffleVector(
            builder, convert(ctx, lhs, sse_i2_t), convert(ctx, rhs, sse_i2_t),
            create_const_vector_i32(mask_values, 8), "");
        values[ins->dreg] =
            convert(ctx, shuffled, type_to_sse_type(ins->inst_c1));
      } else if (ins->inst_c1 == MONO_TYPE_I1 || ins->inst_c1 == MONO_TYPE_U1) {
        const int mask_values[] = {0, 16, 1, 17, 2, 18, 3, 19,
                                   4, 20, 5, 21, 6, 22, 7, 23};
        LLVMValueRef shuffled = LLVMBuildShuffleVector(
            builder, convert(ctx, lhs, sse_i1_t), convert(ctx, rhs, sse_i1_t),
    ...
    case OP_SSE_UNPACKHI: {
      if (ins->inst_c1 == MONO_TYPE_R8 || ins->inst_c1 == MONO_TYPE_I8 ||
          ins->inst_c1 == MONO_TYPE_U8) {
        values[ins->dreg] = LLVMBuildShuffleVector(
            builder, lhs, rhs, create_const_vector_2_i32(1, 3), "");
      } else if (ins->inst_c1 == MONO_TYPE_R4 || ins->inst_c1 == MONO_TYPE_I4 ||
                 ins->inst_c1 == MONO_TYPE_U4) {
        values[ins->dreg] = LLVMBuildShuffleVector(
            builder, lhs, rhs, create_const_vector_4_i32(2, 6, 3, 7), "");
      } else if (ins->inst_c1 == MONO_TYPE_I2 || ins->inst_c1 == MONO_TYPE_U2) {
        ...
        const int mask_values[] = {8,  24, 9,  25, 10, 26, 11, 27,
    ...
    case OP_SSE_LOADU: {
      LLVMValueRef dst_ptr = convert(
          ctx, lhs,
          LLVMPointerType(primitive_type_to_llvm_type(inst_c1_type(ins)), 0));
      LLVMValueRef dst_vec = LLVMBuildBitCast(
          builder, dst_ptr, LLVMPointerType(type_to_sse_type(ins->inst_c1), 0),
          "");
      values[ins->dreg] = mono_llvm_build_aligned_load(
          builder, dst_vec, "", FALSE, ins->inst_c0); // inst_c0 is alignment
      break;
    }
    case OP_SSE_MOVSS: {
      LLVMValueRef addr =
          convert(ctx, lhs, LLVMPointerType(LLVMFloatType(), 0));
      LLVMValueRef val = mono_llvm_build_load(builder, addr, "", FALSE);
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, LLVMConstNull(type_to_sse_type(ins->inst_c1)), val,
          LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      break;
    }
    case OP_SSE_MOVSS_STORE: {
      ...
      LLVMValueRef val = LLVMBuildExtractElement(
          builder, rhs, LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
    ...
    case OP_SSE2_MOVUPD: {
      ...
      LLVMValueRef val =
          mono_llvm_build_aligned_load(builder, addr, "", FALSE, 1);
      values[ins->dreg] =
          LLVMBuildInsertElement(builder, zero, val, const_int32(0), dname);
      break;
    }
    ...
    case OP_SSE_MOVHPS_LOAD: {
      ...
      LLVMValueRef addr2 = convert(
          ctx,
          LLVMBuildAdd(builder, convert(ctx, addr, IntPtrType()),
                       convert(ctx, LLVMConstInt(LLVMInt32Type(), size, FALSE),
                               IntPtrType()),
                       ""),
          LLVMPointerType(t, 0));
      ...
      values[ins->dreg] = LLVMBuildInsertElement(
          builder,
          LLVMBuildInsertElement(builder, lhs, val1,
                                 LLVMConstInt(LLVMInt32Type(), index1, FALSE),
                                 ""),
          val2, LLVMConstInt(LLVMInt32Type(), index2, FALSE), "");
      break;
    }
    ...
    case OP_SSE2_MOVHPD_LOAD: {
      ...
      values[ins->dreg] =
          LLVMBuildInsertElement(builder, lhs, val, const_int32(index), "");
      break;
    }
    ...
    case OP_SSE_MOVHPS_STORE: {
      ...
      LLVMValueRef addr1 =
          convert(ctx, addr, LLVMPointerType(LLVMFloatType(), 0));
      LLVMValueRef addr2 = convert(
          ctx,
          LLVMBuildAdd(builder, convert(ctx, addr, IntPtrType()),
                       convert(ctx, LLVMConstInt(LLVMInt32Type(), 4, FALSE),
                               IntPtrType()),
                       ""),
          LLVMPointerType(LLVMFloatType(), 0));
      int index1 = ins->opcode == OP_SSE_MOVLPS_STORE ? 0 : 2;
      int index2 = ins->opcode == OP_SSE_MOVLPS_STORE ? 1 : 3;
      LLVMValueRef val1 = LLVMBuildExtractElement(
          builder, rhs, LLVMConstInt(LLVMInt32Type(), index1, FALSE), "");
      LLVMValueRef val2 = LLVMBuildExtractElement(
          builder, rhs, LLVMConstInt(LLVMInt32Type(), index2, FALSE), "");
    ...
    case OP_SSE2_MOVHPD_STORE: {
      ...
      LLVMValueRef val =
          LLVMBuildExtractElement(builder, rhs, const_int32(index), "");
      mono_llvm_build_store(builder, val, addr, FALSE, LLVM_BARRIER_NONE);
      break;
    }

    case OP_SSE_STORE: {
      LLVMValueRef dst_vec =
          convert(ctx, lhs, LLVMPointerType(LLVMTypeOf(rhs), 0));
      mono_llvm_build_aligned_store(builder, rhs, dst_vec, FALSE, ins->inst_c0);
      break;
    }

    case OP_SSE_STORES: {
      LLVMValueRef first_elem = LLVMBuildExtractElement(
          builder, rhs, LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      LLVMValueRef dst =
          convert(ctx, lhs, LLVMPointerType(LLVMTypeOf(first_elem), 0));
    ...
    case OP_SSE_MOVNTPS: {
      LLVMValueRef addr =
          convert(ctx, lhs, LLVMPointerType(LLVMTypeOf(rhs), 0));
      LLVMValueRef store = mono_llvm_build_aligned_store(builder, rhs, addr,
                                                         FALSE, ins->inst_c0);
    ...
    case OP_SSE_PREFETCHT0: {
      ...
      LLVMValueRef args[] = {addr, const_int32(0), const_int32(3),
                             const_int32(1)};
    ...
    case OP_SSE_PREFETCHT1: {
      LLVMValueRef addr = convert(ctx, lhs, LLVMPointerType(LLVMInt8Type(), 0));
      LLVMValueRef args[] = {addr, const_int32(0), const_int32(2),
                             const_int32(1)};
      call_intrins(ctx, INTRINS_PREFETCH, args, "");
      break;
    }
    case OP_SSE_PREFETCHT2: {
      LLVMValueRef addr = convert(ctx, lhs, LLVMPointerType(LLVMInt8Type(), 0));
      LLVMValueRef args[] = {addr, const_int32(0), const_int32(1),
                             const_int32(1)};
      call_intrins(ctx, INTRINS_PREFETCH, args, "");
      break;
    }
    case OP_SSE_PREFETCHNTA: {
      LLVMValueRef addr = convert(ctx, lhs, LLVMPointerType(LLVMInt8Type(), 0));
      LLVMValueRef args[] = {addr, const_int32(0), const_int32(0),
                             const_int32(1)};
      call_intrins(ctx, INTRINS_PREFETCH, args, "");
      break;
    }
    ...
    case OP_SSE_SHUFFLE: {
      LLVMValueRef shuffle_vec = create_const_vector_4_i32(
          ...
          ((ins->inst_c0 >> 6) & 0x3) + 4);
      values[ins->dreg] =
          LLVMBuildShuffleVector(builder, lhs, rhs, shuffle_vec, "");
      break;
    }
    ...
    case OP_SSE2_SHUFFLE: {
      ...
      if (ins->inst_c1 == MONO_TYPE_R8) {
        ...
        shuffle_vec = create_const_vector_2_i32(
            ((ins->inst_c0 >> 0) & 0x1) + 0, ((ins->inst_c0 >> 1) & 0x1) + 2);
      } else {
        right_vec = LLVMGetUndef(LLVMVectorType(LLVMInt32Type(), 4));
        shuffle_vec = create_const_vector_4_i32(
            (ins->inst_c0 >> 0) & 0x3, (ins->inst_c0 >> 2) & 0x3,
            (ins->inst_c0 >> 4) & 0x3, (ins->inst_c0 >> 6) & 0x3);
      }
      values[ins->dreg] =
          LLVMBuildShuffleVector(builder, lhs, right_vec, shuffle_vec, "");
      break;
    }
    ...
    case OP_SSE_OR: {
      ...
      values[ins->dreg] = LLVMBuildBitCast(builder, vec_and,
    ...
    case OP_SSE_XOR: {
      LLVMValueRef vec_lhs_i64 = convert(ctx, lhs, sse_i8_t);
      LLVMValueRef vec_rhs_i64 = convert(ctx, rhs, sse_i8_t);
      LLVMValueRef vec_and =
          LLVMBuildXor(builder, vec_lhs_i64, vec_rhs_i64, "");
      values[ins->dreg] = LLVMBuildBitCast(builder, vec_and,
                                           type_to_sse_type(ins->inst_c1), "");
      break;
    }
    ...
    case OP_SSE_AND: {
      ...
      LLVMValueRef vec_rhs_i64 = convert(ctx, rhs, sse_i8_t);
      LLVMValueRef vec_and =
          LLVMBuildAnd(builder, vec_lhs_i64, vec_rhs_i64, "");
      values[ins->dreg] = LLVMBuildBitCast(builder, vec_and,
                                           type_to_sse_type(ins->inst_c1), "");
      break;
    }
    ...
    case OP_SSE_ANDN: {
      ...
      LLVMValueRef vec_lhs_i64 = convert(ctx, lhs, sse_i8_t);
      LLVMValueRef vec_xor =
          LLVMBuildXor(builder, vec_lhs_i64, LLVMConstVector(minus_one, 2), "");
      LLVMValueRef vec_rhs_i64 = convert(ctx, rhs, sse_i8_t);
    ...
    case OP_SSE2_DIVSD:
    case OP_SSE2_MULSD: {
      LLVMValueRef v1 = LLVMBuildExtractElement(
          builder, lhs, LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      LLVMValueRef v2 = LLVMBuildExtractElement(
          builder, rhs, LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      ...
      values[ins->dreg] = LLVMBuildInsertElement(
          builder, lhs, v, LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      break;
    }
    ...
    case OP_SSE_COMISS: {
      ...
      switch (ins->inst_c0) {
      case CMP_EQ:
        id = INTRINS_SSE_COMIEQ_SS;
        break;
      case CMP_GT:
        id = INTRINS_SSE_COMIGT_SS;
        break;
      case CMP_GE:
        id = INTRINS_SSE_COMIGE_SS;
        break;
      case CMP_LT:
        id = INTRINS_SSE_COMILT_SS;
        break;
      case CMP_LE:
        id = INTRINS_SSE_COMILE_SS;
        break;
      case CMP_NE:
        id = INTRINS_SSE_COMINEQ_SS;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_SSE_UCOMISS: {
      ...
      switch (ins->inst_c0) {
      case CMP_EQ:
        id = INTRINS_SSE_UCOMIEQ_SS;
        break;
      case CMP_GT:
        id = INTRINS_SSE_UCOMIGT_SS;
        break;
      case CMP_GE:
        id = INTRINS_SSE_UCOMIGE_SS;
        break;
      case CMP_LT:
        id = INTRINS_SSE_UCOMILT_SS;
        break;
      case CMP_LE:
        id = INTRINS_SSE_UCOMILE_SS;
        break;
      case CMP_NE:
        id = INTRINS_SSE_UCOMINEQ_SS;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_SSE2_COMISD: {
      ...
      switch (ins->inst_c0) {
      case CMP_EQ:
        id = INTRINS_SSE_COMIEQ_SD;
        break;
      case CMP_GT:
        id = INTRINS_SSE_COMIGT_SD;
        break;
      case CMP_GE:
        id = INTRINS_SSE_COMIGE_SD;
        break;
      case CMP_LT:
        id = INTRINS_SSE_COMILT_SD;
        break;
      case CMP_LE:
        id = INTRINS_SSE_COMILE_SD;
        break;
      case CMP_NE:
        id = INTRINS_SSE_COMINEQ_SD;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_SSE2_UCOMISD: {
      ...
      switch (ins->inst_c0) {
      case CMP_EQ:
        id = INTRINS_SSE_UCOMIEQ_SD;
        break;
      case CMP_GT:
        id = INTRINS_SSE_UCOMIGT_SD;
        break;
      case CMP_GE:
        id = INTRINS_SSE_UCOMIGE_SD;
        break;
      case CMP_LT:
        id = INTRINS_SSE_UCOMILT_SD;
        break;
      case CMP_LE:
        id = INTRINS_SSE_UCOMILE_SD;
        break;
      case CMP_NE:
        id = INTRINS_SSE_UCOMINEQ_SD;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_SSE2_CVTSI2SD64: {
      ...
      values[ins->dreg] =
          LLVMBuildInsertElement(builder, lhs, fp, const_int32(0), dname);
      break;
    }
    case OP_SSE2_PMULUDQ: {
      ...
      LLVMValueRef l =
          LLVMBuildAnd(builder, convert(ctx, lhs, sse_i8_t), mask, "");
      LLVMValueRef r =
          LLVMBuildAnd(builder, convert(ctx, rhs, sse_i8_t), mask, "");
    ...
    case OP_SSE2_SQRTSD: {
#if LLVM_API_VERSION < 700
      LLVMValueRef result =
          call_intrins(ctx, simd_ins_to_intrins(ins->opcode), &rhs, dname);
      ...
      values[ins->dreg] =
          LLVMBuildShuffleVector(builder, result, lhs, shufmask, "");
#else
      LLVMValueRef upper = values[ins->sreg1];
      LLVMValueRef lower = values[ins->sreg2];
      LLVMValueRef scalar =
          LLVMBuildExtractElement(builder, lower, const_int32(0), "");
      LLVMValueRef result =
          call_intrins(ctx, simd_ins_to_intrins(ins->opcode), &scalar, dname);
      values[ins->dreg] =
          LLVMBuildInsertElement(builder, upper, result, const_int32(0), "");
#endif
      break;
    }
    case OP_SSE_RCPSS:
    case OP_SSE_RSQRTSS: {
      ...
      switch (ins->opcode) {
      case OP_SSE_RCPSS:
        id = INTRINS_SSE_RCP_SS;
        break;
      case OP_SSE_RSQRTSS:
        id = INTRINS_SSE_RSQRT_SS;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_XOP: {
      IntrinsicId id = (IntrinsicId)0;
      switch (ins->inst_c0) {
      case SIMD_OP_SSE_LFENCE:
        id = INTRINS_SSE_LFENCE;
        break;
      case SIMD_OP_SSE_SFENCE:
        id = INTRINS_SSE_SFENCE;
        break;
      case SIMD_OP_SSE_MFENCE:
        id = INTRINS_SSE_MFENCE;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_XOP_X_X: {
      ...
      IntrinsicId id = (IntrinsicId)0;
      switch (ins->inst_c0) {
      case SIMD_OP_SSE_SQRTPS:
        id = INTRINS_SSE_SQRT_PS;
        break;
      case SIMD_OP_SSE_RCPPS:
        id = INTRINS_SSE_RCP_PS;
        break;
      case SIMD_OP_SSE_RSQRTPS:
        id = INTRINS_SSE_RSQRT_PS;
        break;
      case SIMD_OP_SSE_SQRTPD:
        id = INTRINS_SSE_SQRT_PD;
        break;
      case SIMD_OP_SSE_LDDQU:
        id = INTRINS_SSE_LDU_DQ;
        break;
      case SIMD_OP_SSE_PHMINPOSUW:
        id = INTRINS_SSE_PHMINPOSUW;
        to_i1_t = TRUE;
        break;
      case SIMD_OP_AES_IMC:
        id = INTRINS_AESNI_AESIMC;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_XOP_I8_X: {
      ...
      case SIMD_OP_SSE_CVTSS2SI:
        id = INTRINS_SSE_CVTSS2SI;
        break;
      case SIMD_OP_SSE_CVTTSS2SI:
        id = INTRINS_SSE_CVTTSS2SI;
        break;
      case SIMD_OP_SSE_CVTSS2SI64:
        id = INTRINS_SSE_CVTSS2SI64;
        break;
      case SIMD_OP_SSE_CVTTSS2SI64:
        id = INTRINS_SSE_CVTTSS2SI64;
        break;
      case SIMD_OP_SSE_CVTSD2SI:
        id = INTRINS_SSE_CVTSD2SI;
        break;
      case SIMD_OP_SSE_CVTTSD2SI:
        id = INTRINS_SSE_CVTTSD2SI;
        break;
      case SIMD_OP_SSE_CVTSD2SI64:
        id = INTRINS_SSE_CVTSD2SI64;
        break;
      case SIMD_OP_SSE_CVTTSD2SI64:
        id = INTRINS_SSE_CVTTSD2SI64;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_XOP_I4_X_X: {
      ...
      IntrinsicId id = (IntrinsicId)0;
      switch (ins->inst_c0) {
      case SIMD_OP_SSE_TESTC:
        id = INTRINS_SSE_TESTC;
        to_i8_t = TRUE;
        ret_bool = TRUE;
        break;
      case SIMD_OP_SSE_TESTZ:
        id = INTRINS_SSE_TESTZ;
        to_i8_t = TRUE;
        ret_bool = TRUE;
        break;
      case SIMD_OP_SSE_TESTNZ:
        id = INTRINS_SSE_TESTNZ;
        to_i8_t = TRUE;
        ret_bool = TRUE;
        break;
      default:
      ...
      if (ret_bool) {
        // if return type is bool (it's still i32) we need to normalize it to
        // 1/0
        LLVMValueRef cmp_zero =
            LLVMBuildICmp(builder, LLVMIntNE, call,
                          LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
        values[ins->dreg] =
            LLVMBuildZExt(builder, cmp_zero, LLVMInt8Type(), "");
      } else {
    ...
    case OP_XOP_X_X_I8: {
      ...
      IntrinsicId id = (IntrinsicId)0;
      switch (ins->inst_c0) {
      case SIMD_OP_SSE_CVTSD2SS:
        id = INTRINS_SSE_CVTSD2SS;
        break;
      case SIMD_OP_SSE_MAXPS:
        id = INTRINS_SSE_MAXPS;
        break;
      case SIMD_OP_SSE_MAXSS:
        id = INTRINS_SSE_MAXSS;
        break;
      case SIMD_OP_SSE_MINPS:
        id = INTRINS_SSE_MINPS;
        break;
      case SIMD_OP_SSE_MINSS:
        id = INTRINS_SSE_MINSS;
        break;
      case SIMD_OP_SSE_MAXPD:
        id = INTRINS_SSE_MAXPD;
        break;
      case SIMD_OP_SSE_MAXSD:
        id = INTRINS_SSE_MAXSD;
        break;
      case SIMD_OP_SSE_MINPD:
        id = INTRINS_SSE_MINPD;
        break;
      case SIMD_OP_SSE_MINSD:
        id = INTRINS_SSE_MINSD;
        break;
      case SIMD_OP_SSE_PMADDWD:
        id = INTRINS_SSE_PMADDWD;
        break;
      case SIMD_OP_SSE_PMULHW:
        id = INTRINS_SSE_PMULHW;
        break;
      case SIMD_OP_SSE_PMULHUW:
        id = INTRINS_SSE_PMULHUW;
        break;
      case SIMD_OP_SSE_PACKSSWB:
        id = INTRINS_SSE_PACKSSWB;
        break;
      case SIMD_OP_SSE_PACKSSDW:
        id = INTRINS_SSE_PACKSSDW;
        break;
      case SIMD_OP_SSE_PSRLW_IMM:
        id = INTRINS_SSE_PSRLI_W;
        break;
      case SIMD_OP_SSE_PSRLD_IMM:
        id = INTRINS_SSE_PSRLI_D;
        break;
      case SIMD_OP_SSE_PSRLQ_IMM:
        id = INTRINS_SSE_PSRLI_Q;
        break;
      case SIMD_OP_SSE_PSRLW:
        id = INTRINS_SSE_PSRL_W;
        break;
      case SIMD_OP_SSE_PSRLD:
        id = INTRINS_SSE_PSRL_D;
        break;
      case SIMD_OP_SSE_PSRLQ:
        id = INTRINS_SSE_PSRL_Q;
        break;
      case SIMD_OP_SSE_PSLLW_IMM:
        id = INTRINS_SSE_PSLLI_W;
        break;
      case SIMD_OP_SSE_PSLLD_IMM:
        id = INTRINS_SSE_PSLLI_D;
        break;
      case SIMD_OP_SSE_PSLLQ_IMM:
        id = INTRINS_SSE_PSLLI_Q;
        break;
      case SIMD_OP_SSE_PSLLW:
        id = INTRINS_SSE_PSLL_W;
        break;
      case SIMD_OP_SSE_PSLLD:
        id = INTRINS_SSE_PSLL_D;
        break;
      case SIMD_OP_SSE_PSLLQ:
        id = INTRINS_SSE_PSLL_Q;
        break;
      case SIMD_OP_SSE_PSRAW_IMM:
        id = INTRINS_SSE_PSRAI_W;
        break;
      case SIMD_OP_SSE_PSRAD_IMM:
        id = INTRINS_SSE_PSRAI_D;
        break;
      case SIMD_OP_SSE_PSRAW:
        id = INTRINS_SSE_PSRA_W;
        break;
      case SIMD_OP_SSE_PSRAD:
        id = INTRINS_SSE_PSRA_D;
        break;
      case SIMD_OP_SSE_PSADBW:
        id = INTRINS_SSE_PSADBW;
        break;
      case SIMD_OP_SSE_ADDSUBPS:
        id = INTRINS_SSE_ADDSUBPS;
        break;
      case SIMD_OP_SSE_ADDSUBPD:
        id = INTRINS_SSE_ADDSUBPD;
        break;
      case SIMD_OP_SSE_HADDPS:
        id = INTRINS_SSE_HADDPS;
        break;
      case SIMD_OP_SSE_HADDPD:
        id = INTRINS_SSE_HADDPD;
        break;
      case SIMD_OP_SSE_PHADDW:
        id = INTRINS_SSE_PHADDW;
        break;
      case SIMD_OP_SSE_PHADDD:
        id = INTRINS_SSE_PHADDD;
        break;
      case SIMD_OP_SSE_PHSUBW:
        id = INTRINS_SSE_PHSUBW;
        break;
      case SIMD_OP_SSE_PHSUBD:
        id = INTRINS_SSE_PHSUBD;
        break;
      case SIMD_OP_SSE_HSUBPS:
        id = INTRINS_SSE_HSUBPS;
        break;
      case SIMD_OP_SSE_HSUBPD:
        id = INTRINS_SSE_HSUBPD;
        break;
      case SIMD_OP_SSE_PHADDSW:
        id = INTRINS_SSE_PHADDSW;
        break;
      case SIMD_OP_SSE_PHSUBSW:
        id = INTRINS_SSE_PHSUBSW;
        break;
      case SIMD_OP_SSE_PSIGNB:
        id = INTRINS_SSE_PSIGNB;
        break;
      case SIMD_OP_SSE_PSIGNW:
        id = INTRINS_SSE_PSIGNW;
        break;
      case SIMD_OP_SSE_PSIGND:
        id = INTRINS_SSE_PSIGND;
        break;
      case SIMD_OP_SSE_PMADDUBSW:
        id = INTRINS_SSE_PMADDUBSW;
        break;
      case SIMD_OP_SSE_PMULHRSW:
        id = INTRINS_SSE_PMULHRSW;
        break;
      case SIMD_OP_SSE_PACKUSDW:
        id = INTRINS_SSE_PACKUSDW;
        break;
      case SIMD_OP_AES_DEC:
        id = INTRINS_AESNI_AESDEC;
        break;
      case SIMD_OP_AES_DECLAST:
        id = INTRINS_AESNI_AESDECLAST;
        break;
      case SIMD_OP_AES_ENC:
        id = INTRINS_AESNI_AESENC;
        break;
      case SIMD_OP_AES_ENCLAST:
        id = INTRINS_AESNI_AESENCLAST;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_SSE2_SUBS: {
      ...
      if (is_add) {
        switch (type) {
        case MONO_TYPE_I1:
          id = INTRINS_SSE_SADD_SATI8;
          break;
        case MONO_TYPE_U1:
          id = INTRINS_SSE_UADD_SATI8;
          break;
        case MONO_TYPE_I2:
          id = INTRINS_SSE_SADD_SATI16;
          break;
        case MONO_TYPE_U2:
          id = INTRINS_SSE_UADD_SATI16;
          break;
        default:
          g_assert_not_reached();
          break;
        ...
        switch (type) {
        case MONO_TYPE_I1:
          id = INTRINS_SSE_SSUB_SATI8;
          break;
        case MONO_TYPE_U1:
          id = INTRINS_SSE_USUB_SATI8;
          break;
        case MONO_TYPE_I2:
          id = INTRINS_SSE_SSUB_SATI16;
          break;
        case MONO_TYPE_U2:
          id = INTRINS_SSE_USUB_SATI16;
          break;
        default:
          g_assert_not_reached();
          break;
      ...
      LLVMValueRef args[] = {convert(ctx, lhs, vecty),
                             convert(ctx, rhs, vecty)};
    ...
    case OP_SSE2_PACKUS: {
      ...
      values[ins->dreg] =
          convert(ctx, call_intrins(ctx, INTRINS_SSE_PACKUSWB, args, dname),
                  type_to_sse_type(ins->inst_c1));
      break;
    }
    ...
    case OP_SSE2_SRLI: {
      LLVMValueRef args[] = {lhs, rhs};
      values[ins->dreg] =
          convert(ctx, call_intrins(ctx, INTRINS_SSE_PSRLI_W, args, dname),
                  type_to_sse_type(ins->inst_c1));
      break;
    }
    ...
    case OP_SSE2_PSRLDQ: {
      ...
      for (i = 0; i < nelems; ++i) {
        ...
        phi_values[i] = LLVMBuildShuffleVector(
            builder, value, LLVMGetUndef(t),
            create_const_vector_i32(mask_values, nelems), "");
        LLVMBuildBr(builder, cbb);
      }
      /* Default case */
      ...
      values[ins->dreg] =
          convert(ctx, values[ins->dreg], type_to_sse_type(ins->inst_c1));
    ...
    case OP_SSE2_PSRLQ_IMM: {
      ...
      LLVMValueRef index_vect = LLVMBuildInsertElement(
          builder, LLVMConstNull(t), convert(ctx, index, LLVMGetElementType(t)),
          const_int32(0), "");
    ...
    case OP_SSE2_PSHUFLW: {
      ...
      for (int i = 0; i < ncases; ++i) {
        ...
        case OP_SSE2_PSHUFD:
          /*
           * Each 2 bits in mask selects 1 dword from the the source and copies
        ...
        case OP_SSE2_PSHUFHW:
          /*
           * Each 2 bits in mask selects 1 word from the high quadword of the
           * source and copies it to the high quadword of the destination.
           */
          mask_len = 8;
        ...
        phi_values[i] = LLVMBuildShuffleVector(
            builder, v1, v2, create_const_vector_i32(mask_values, mask_len),
            "");
        LLVMBuildBr(builder, cbb);
      }
    ...
    case OP_SSE3_MOVDDUP: {
      int mask[] = {0, 0};
      values[ins->dreg] =
          LLVMBuildShuffleVector(builder, lhs, LLVMGetUndef(LLVMTypeOf(lhs)),
    ...
    case OP_SSE3_MOVDDUP_MEM: {
      ...
      LLVMValueRef value = mono_llvm_build_load(
          builder, convert(ctx, lhs, LLVMPointerType(t, 0)), "", FALSE);
      values[ins->dreg] = LLVMBuildShuffleVector(
          builder, value, LLVMGetUndef(LLVMTypeOf(value)),
          create_const_vector_i32(mask, 2), "");
      break;
    }
    case OP_SSE3_MOVSHDUP: {
      int mask[] = {1, 1, 3, 3};
      values[ins->dreg] =
          LLVMBuildShuffleVector(builder, lhs, LLVMConstNull(LLVMTypeOf(lhs)),
                                 create_const_vector_i32(mask, 4), "");
      break;
    }
    ...
    case OP_SSSE3_ABS: {
      ...
      LLVMValueRef cmp =
          LLVMBuildICmp(builder, LLVMIntSGT, lhs, LLVMConstNull(typ), "");
    ...
    case OP_SSSE3_ALIGNR: {
      ...
      LLVMValueRef shuffled = LLVMBuildShuffleVector(
          builder, convert(ctx, rhs, sse_i1_t), convert(ctx, lhs, sse_i1_t),
    ...
    case OP_CREATE_SCALAR_UNSAFE: {
      ...
      // use undef vector (most likely empty but may contain garbage values) for
      // OP_CREATE_SCALAR_UNSAFE and zero one for OP_CREATE_SCALAR
      LLVMValueRef vector = (ins->opcode == OP_CREATE_SCALAR)
                                ? LLVMConstNull(type)
                                : LLVMGetUndef(type);
      LLVMValueRef insert_pos = LLVMConstInt(LLVMInt32Type(), 0, FALSE);
      LLVMValueRef val =
          convert_full(ctx, lhs, primitive_type_to_llvm_type(primty),
                       primitive_type_is_unsigned(primty));
      values[ins->dreg] =
          LLVMBuildInsertElement(builder, vector, val, insert_pos, "");
      break;
    }
    case OP_SSE41_ROUNDP: {
      LLVMValueRef args[] = {
          lhs, LLVMConstInt(LLVMInt32Type(), ins->inst_c0, FALSE)};
      values[ins->dreg] =
          call_intrins(ctx,
                       ins->inst_c1 == MONO_TYPE_R4 ? INTRINS_SSE_ROUNDPS
                                                    : INTRINS_SSE_ROUNDPD,
                       args, dname);
      break;
    }
    case OP_SSE41_ROUNDS: {
      ...
      values[ins->dreg] =
          call_intrins(ctx,
                       ins->inst_c1 == MONO_TYPE_R4 ? INTRINS_SSE_ROUNDSS
                                                    : INTRINS_SSE_ROUNDSD,
                       args, dname);
      break;
    }
    ...
    case OP_SSE41_DPPS_IMM: {
      LLVMValueRef args[] = {lhs, rhs,
                             LLVMConstInt(LLVMInt8Type(), ins->inst_c0, FALSE)};
    ...
    case OP_SSE41_INSERT: {
      if (ins->inst_c1 == MONO_TYPE_R4) {
        ...
        values[ins->dreg] =
            call_intrins(ctx, INTRINS_SSE_INSERTPS, args, dname);
      } else {
        // other overloads are implemented with `insertelement`
        values[ins->dreg] = LLVMBuildInsertElement(
            builder, values[ins->sreg1],
            convert(ctx, values[ins->sreg2],
    ...
    case OP_SSE41_BLENDV: {
      ...
      if (ins->inst_c1 == MONO_TYPE_R4) {
        values[ins->dreg] =
            call_intrins(ctx, INTRINS_SSE_BLENDVPS, args, dname);
      } else if (ins->inst_c1 == MONO_TYPE_R8) {
        values[ins->dreg] =
            call_intrins(ctx, INTRINS_SSE_BLENDVPD, args, dname);
      } else {
        // for other non-fp type just convert to <16 x i8> and pass to
        ...
        values[ins->dreg] =
            call_intrins(ctx, INTRINS_SSE_PBLENDVB, args, dname);
      }
      break;
    }
    ...
    case OP_SSE_CVTII: {
      gboolean is_signed = (ins->inst_c1 == MONO_TYPE_I1) ||
                           (ins->inst_c1 == MONO_TYPE_I2) ||
                           (ins->inst_c1 == MONO_TYPE_I4);
      ...
      if (LLVMGetTypeKind(LLVMTypeOf(lhs)) != LLVMVectorTypeKind) {
        LLVMValueRef bitcasted = LLVMBuildBitCast(
            ctx->builder, lhs, LLVMPointerType(vec_type, 0), "");
      ...
      LLVMValueRef shuffled = LLVMBuildShuffleVector(
          builder, value, LLVMGetUndef(vec_type), mask_vec, "");
    ...
    case OP_SSE41_LOADANT: {
      ...
      LLVMValueRef dst_vec = LLVMBuildBitCast(
          ...
          "");
      LLVMValueRef load =
          mono_llvm_build_aligned_load(builder, dst_vec, "", FALSE, 16);
    ...
    case OP_SSE41_MUL: {
      ...
      // however, pmuldq is still available so I guess it's fine to keep using
      // it
      LLVMValueRef args[] = {lhs, rhs};
    ...
    case OP_SSE42_CRC64: {
      ...
      switch (ins->inst_c0) {
      case MONO_TYPE_U1:
        id = INTRINS_SSE_CRC32_32_8;
        break;
      case MONO_TYPE_U2:
        id = INTRINS_SSE_CRC32_32_16;
        break;
      case MONO_TYPE_U4:
        id = INTRINS_SSE_CRC32_32_32;
        break;
      case MONO_TYPE_U8:
        id = INTRINS_SSE_CRC32_64_64;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_AES_KEYGEN_IMM: {
      LLVMValueRef args[] = {lhs,
                             LLVMConstInt(LLVMInt8Type(), ins->inst_c0, FALSE)};
      values[ins->dreg] =
          call_intrins(ctx, INTRINS_AESNI_AESKEYGENASSIST, args, "");
      break;
    }
#endif
    ...
    case OP_XCOMPARE_FP: {
      ...
      if (ins->inst_c1 == MONO_TYPE_R8)
        values[ins->dreg] = LLVMBuildBitCast(
            builder,
            LLVMBuildSExt(builder, cmp, LLVMVectorType(LLVMInt64Type(), nelems),
                          ""),
            LLVMTypeOf(lhs), "");
      else
        values[ins->dreg] = LLVMBuildBitCast(
            builder,
            LLVMBuildSExt(builder, cmp, LLVMVectorType(LLVMInt32Type(), nelems),
                          ""),
            LLVMTypeOf(lhs), "");
      break;
    }
    ...
    case OP_XEQUAL: {
      ...
      /* The wasm code generator doesn't understand the shuffle/and code
       * sequence below */
      LLVMValueRef val;
      if (LLVMIsNull(lhs) || LLVMIsNull(rhs)) {
        ...
        values[ins->dreg] = LLVMBuildZExt(
            builder,
            LLVMBuildICmp(builder, LLVMIntEQ, values[ins->dreg],
                          LLVMConstInt(LLVMInt32Type(), 0, FALSE), ""),
            LLVMInt32Type(), dname);
        break;
      }
      ...
      while (half >= 1) {
        ...
        for (int i = half; i < nelems; ++i)
          mask[i] = LLVMConstInt(LLVMInt32Type(), 0, FALSE);
        shuffle = LLVMBuildShuffleVector(
            builder, cmp, LLVMGetUndef(t),
            LLVMConstVector(mask, LLVMGetVectorSize(t)), "");
      ...
      // Extract [0]
      LLVMValueRef first_elem = LLVMBuildExtractElement(
          builder, cmp, LLVMConstInt(LLVMInt32Type(), 0, FALSE), "");
      // convert to 0/1
      LLVMValueRef cmp_zero = LLVMBuildICmp(builder, LLVMIntNE, first_elem,
                                            LLVMConstInt(elemt, 0, FALSE), "");
    ...
    case OP_XBINOP: {
      ...
      case OP_FMIN: {
        ...
        if (ins->inst_c0 == OP_FMAX)
          values[ins->dreg] = call_intrins(
              ctx, is_r4 ? INTRINS_SSE_MAXPS : INTRINS_SSE_MAXPD, args, dname);
        else
          values[ins->dreg] = call_intrins(
              ctx, is_r4 ? INTRINS_SSE_MINPS : INTRINS_SSE_MINPD, args, dname);
#else
        NOT_IMPLEMENTED;
#endif
        break;
      ...
      case OP_IMAX: {
        gboolean is_unsigned =
            ins->inst_c1 == MONO_TYPE_U1 || ins->inst_c1 == MONO_TYPE_U2 ||
            ins->inst_c1 == MONO_TYPE_U4 || ins->inst_c1 == MONO_TYPE_U8;
        LLVMValueRef cmp = LLVMBuildICmp(
            builder, is_unsigned ? LLVMIntUGT : LLVMIntSGT, lhs, rhs, "");
        values[ins->dreg] = LLVMBuildSelect(builder, cmp, lhs, rhs, "");
        break;
      }
      case OP_IMIN: {
        gboolean is_unsigned =
            ...
            ins->inst_c1 == MONO_TYPE_U4 || ins->inst_c1 == MONO_TYPE_U8;
        LLVMValueRef cmp = LLVMBuildICmp(
            builder, is_unsigned ? LLVMIntULT : LLVMIntSLT, lhs, rhs, "");
        values[ins->dreg] = LLVMBuildSelect(builder, cmp, lhs, rhs, "");
      } break;
    ...
    case OP_CTTZ64: {
      ...
      values[ins->dreg] = call_intrins(
          ctx, ins->opcode == OP_CTTZ32 ? INTRINS_CTTZ_I32 : INTRINS_CTTZ_I64,
          args, "");
      break;
    }
    case OP_BEXTR32:
    case OP_BEXTR64: {
      ...
      args[1] = convert(ctx, rhs,
                        ins->opcode == OP_BEXTR32
                            ? LLVMInt32Type()
                            : LLVMInt64Type()); // cast ushort to u32/u64
      values[ins->dreg] = call_intrins(
          ctx,
          ins->opcode == OP_BEXTR32 ? INTRINS_BEXTR_I32 : INTRINS_BEXTR_I64,
          args, "");
      break;
    }
    case OP_BZHI32:
    case OP_BZHI64: {
      ...
      args[1] = rhs;
      values[ins->dreg] = call_intrins(
          ctx, ins->opcode == OP_BZHI32 ? INTRINS_BZHI_I32 : INTRINS_BZHI_I64,
          args, "");
      break;
    }
    case OP_MULX_H32:
    ...
    case OP_MULX_HL64: {
      gboolean is_64 =
          ins->opcode == OP_MULX_H64 || ins->opcode == OP_MULX_HL64;
      gboolean only_high =
          ins->opcode == OP_MULX_H32 || ins->opcode == OP_MULX_H64;
      ...
      if (!only_high) {
        LLVMValueRef lowx = LLVMBuildTrunc(
            ctx->builder, mulx, is_64 ? LLVMInt64Type() : LLVMInt32Type(), "");
        LLVMBuildStore(ctx->builder, lowx, values[ins->sreg3]);
      }
      LLVMValueRef shift =
          LLVMConstInt(LLVMInt128Type(), is_64 ? 64 : 32, FALSE);
      LLVMValueRef highx = LLVMBuildLShr(ctx->builder, mulx, shift, "");
      values[ins->dreg] = LLVMBuildTrunc(
          ctx->builder, highx, is_64 ? LLVMInt64Type() : LLVMInt32Type(), "");
      break;
    }
    case OP_PEXT32:
    case OP_PEXT64: {
      ...
      args[1] = rhs;
      values[ins->dreg] = call_intrins(
          ctx, ins->opcode == OP_PEXT32 ? INTRINS_PEXT_I32 : INTRINS_PEXT_I64,
          args, "");
      break;
    }
    case OP_PDEP32:
    case OP_PDEP64: {
      ...
      args[1] = rhs;
      values[ins->dreg] = call_intrins(
          ctx, ins->opcode == OP_PDEP32 ? INTRINS_PDEP_I32 : INTRINS_PDEP_I64,
          args, "");
      break;
    }
...
// Shared between ARM64 and X86
    ...
    (defined(TARGET_ARM64) || defined(TARGET_X86) || defined(TARGET_AMD64))
    case OP_LZCNT32:
    case OP_LZCNT64: {
      LLVMValueRef args[2];
      args[0] = lhs;
      args[1] = LLVMConstInt(LLVMInt1Type(), 1, FALSE);
      values[ins->dreg] = LLVMBuildCall(
          builder,
          get_intrins(ctx, ins->opcode == OP_LZCNT32 ? INTRINS_CTLZ_I32
                                                     : INTRINS_CTLZ_I64),
          args, 2, "");
      break;
    }
    ...
    case OP_XOP_I8_I8: {
      IntrinsicId id = (IntrinsicId)0;
      switch (ins->inst_c0) {
      case SIMD_OP_ARM64_RBIT32:
        id = INTRINS_BITREVERSE_I32;
        break;
      case SIMD_OP_ARM64_RBIT64:
        id = INTRINS_BITREVERSE_I64;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_XOP_I4_I4_I8: {
      ...
      switch (ins->inst_c0) {
      case SIMD_OP_ARM64_CRC32B:
        id = INTRINS_AARCH64_CRC32B;
        zext_last = TRUE;
        break;
      case SIMD_OP_ARM64_CRC32H:
        id = INTRINS_AARCH64_CRC32H;
        zext_last = TRUE;
        break;
      case SIMD_OP_ARM64_CRC32W:
        id = INTRINS_AARCH64_CRC32W;
        zext_last = TRUE;
        break;
      case SIMD_OP_ARM64_CRC32X:
        id = INTRINS_AARCH64_CRC32X;
        break;
      case SIMD_OP_ARM64_CRC32CB:
        id = INTRINS_AARCH64_CRC32CB;
        zext_last = TRUE;
        break;
      case SIMD_OP_ARM64_CRC32CH:
        id = INTRINS_AARCH64_CRC32CH;
        zext_last = TRUE;
        break;
      case SIMD_OP_ARM64_CRC32CW:
        id = INTRINS_AARCH64_CRC32CW;
        zext_last = TRUE;
        break;
      case SIMD_OP_ARM64_CRC32CX:
        id = INTRINS_AARCH64_CRC32CX;
        break;
      default:
        g_assert_not_reached();
        break;
    ...
    case OP_LSCNT64: {
      ...
      LLVMValueRef shr = LLVMBuildAShr(
          builder, lhs,
          ins->opcode == OP_LSCNT32 ? LLVMConstInt(LLVMInt32Type(), 31, FALSE)
      ...
      values[ins->dreg] = LLVMBuildCall(
          builder,
          get_intrins(ctx, ins->opcode == OP_LSCNT32 ? INTRINS_CTLZ_I32
                                                     : INTRINS_CTLZ_I64),
          args, 2, "");
      break;
    }
    ...
    case OP_CALL_HANDLER: {
      ...
      LLVMBuildStore(
          builder,
          LLVMConstInt(LLVMInt32Type(), g_slist_length(bb_list) + 1, FALSE),
          lhs);
      ...
      info->call_handler_return_bbs = g_slist_append_mempool(
          cfg->mempool, info->call_handler_return_bbs, noex_bb);
    ...
    case OP_ENDFINALLY: {
      ...
      BBInfo *info;
      gboolean is_fault =
          MONO_REGION_FLAGS(bb->region) == MONO_EXCEPTION_CLAUSE_FAULT;

      /*
       * Fault clauses are like finally clauses, but they are only called if an
       * exception is thrown.
       */
      if (!is_fault) {
        handler_bb = (MonoBasicBlock *)g_hash_table_lookup(
            ctx->region_to_handler,
            GUINT_TO_POINTER(mono_get_block_region_notry(cfg, bb->region)));
        g_assert(handler_bb);
        ...
        switch_ins =
            LLVMBuildSwitch(builder, val, resume_bb, g_slist_length(bb_list));
        /*
         * The other targets are added at the end to handle OP_CALL_HANDLER
         * opcodes processed later.
         */
        info->endfinally_switch_ins_list = g_slist_append_mempool(
            cfg->mempool, info->endfinally_switch_ins_list, switch_ins);
      ...
      if (ctx->llvm_only) {
        ...
        if (ctx->cfg->compile_aot) {
          callee = get_callee(
              ctx, icall_sig, MONO_PATCH_INFO_JIT_ICALL_ID,
              GUINT_TO_POINTER(
                  MONO_JIT_ICALL_mono_llvm_resume_unwind_trampoline));
        } else {
          callee = get_jit_callee(
              ctx, "llvm_resume_unwind_trampoline", icall_sig,
              MONO_PATCH_INFO_JIT_ICALL_ID,
              GUINT_TO_POINTER(
                  MONO_JIT_ICALL_mono_llvm_resume_unwind_trampoline));
        }
    ...
    if (spec[MONO_INST_DEST] != ' ' && !MONO_IS_STORE_MEMBASE(ins) &&
        ctx->vreg_types[ins->dreg]) {
      if (ctx->is_vphi[ins->dreg])
        /* vtypes */
        values[ins->dreg] = addresses[ins->dreg];
      else
        values[ins->dreg] =
            convert(ctx, values[ins->dreg], ctx->vreg_types[ins->dreg]);
    }

    /* Add stores for volatile variables */
    if (!skip_volatile_store && spec[MONO_INST_DEST] != ' ' &&
        spec[MONO_INST_DEST] != 'v' && !MONO_IS_STORE_MEMBASE(ins))
      emit_volatile_store(ctx, ins->dreg);
  }
  ...
  if (!has_terminator && bb->next_bb &&
      (bb == cfg->bb_entry || bb->in_count > 0)) {
 ...
 *
 *   Do some quick checks to decide whenever cfg->method can be compiled by
 * LLVM, to avoid compiling a method twice.
 */
void mono_llvm_check_method_supported(MonoCompile *cfg) {
  int i, j;

#ifdef TARGET_WASM
  if (mono_method_signature_internal(cfg->method)->call_convention ==
      MONO_CALL_VARARG) {
  ...
  for (i = 0; i < cfg->header->num_clauses; ++i) {
    for (j = 0; j < cfg->header->num_clauses; ++j) {
      ...
      if (i != j && clause1->try_offset >= clause2->try_offset &&
          clause1->handler_offset <= clause2->handler_offset) {
        //(clause1->flags == MONO_EXCEPTION_CLAUSE_FINALLY || clause2->flags ==
...
static LLVMCallInfo *get_llvm_call_info(MonoCompile *cfg,
                                        MonoMethodSignature *sig) {
  LLVMCallInfo *linfo;
  int i;
  ...
  if (cfg->gsharedvt && cfg->llvm_only &&
    ...
    linfo = (LLVMCallInfo *)mono_mempool_alloc0(
        cfg->mempool, sizeof(LLVMCallInfo) + (sizeof(LLVMArgInfo) * n));
...
static void emit_method_inner(EmitContext *ctx);

static void free_ctx(EmitContext *ctx) {
  GSList *l;
...
void mono_llvm_emit_method(MonoCompile *cfg) {
  ...
  if (cfg->compile_aot) {
    ...
    ctx->lmodule = LLVMModuleCreateWithName(
        g_strdup_printf("jit-module-%s", cfg->method->name));
  ...
  if (!ctx_ok(ctx)) {
    if (ctx->lmethod) {
      ...
      if (ctx->module->llvm_only && ctx->module->static_link) {
        ...
        LLVMBasicBlockRef entry_bb =
            LLVMAppendBasicBlock(ctx->lmethod, "ENTRY");
        builder = create_builder(ctx);
        LLVMPositionBuilderAtEnd(builder, entry_bb);
        ctx->builder = builder;

        LLVMTypeRef sig = LLVMFunctionType0(LLVMVoidType(), FALSE);
        LLVMValueRef callee = get_callee(
            ctx, sig, MONO_PATCH_INFO_JIT_ICALL_ADDR,
            GUINT_TO_POINTER(
                MONO_JIT_ICALL_mini_llvmonly_throw_nullref_exception));
        LLVMBuildCall(builder, callee, NULL, 0, "");
...
static void emit_method_inner(EmitContext *ctx) {
  ...
  if (cfg->method->wrapper_type == MONO_WRAPPER_OTHER) {
    ...
    if (info->subtype == WRAPPER_SUBTYPE_AOT_INIT) {
      ...
      ctx->lmethod = method;
      ctx->module->max_method_idx =
          MAX(ctx->module->max_method_idx, cfg->method_index);

      const char *init_name =
          mono_marshal_get_aot_init_wrapper_name(info->d.aot_init.subtype);
      ctx->method_name =
          g_strdup_printf("%s_%s", ctx->module->global_prefix, init_name);
      ...
      g_hash_table_insert(ctx->module->no_method_table_lmethods, method,
                          method);
      ...
      if (cfg->compile_aot) {
        ...
        method = emit_icall_cold_wrapper(ctx->module, lmodule,
      ...
      ctx->method_name = g_strdup(LLVMGetValueName(
          method)); // g_strdup_printf ("%s_%s", ctx->module->global_prefix,
                    // LLVMGetValueName (method));
      ctx->cfg->asm_symbol = g_strdup(ctx->method_name);
  ...
  if (cfg->vret_addr) {
    ...
    if (linfo->ret.storage == LLVMArgVtypeByRef) {
      mono_llvm_add_param_attr(LLVMGetParam(method, linfo->vret_arg_pindex),
                               LLVM_ATTR_STRUCT_RET);
      mono_llvm_add_param_attr(LLVMGetParam(method, linfo->vret_arg_pindex),
                               LLVM_ATTR_NO_ALIAS);
    }
  ...
  if (linfo->dummy_arg)
    LLVMSetValueName(LLVMGetParam(method, linfo->dummy_arg_pindex),
                     "dummy_arg");
  ...
  for (i = 0; i < sig->param_count; ++i) {
    ...
    if (ainfo->storage == LLVMArgVtypeInReg &&
        ainfo->pair_storage[0] == LLVMArgNone &&
        ainfo->pair_storage[1] == LLVMArgNone)
      continue;

    values[cfg->args[i + sig->hasthis]->dreg] = LLVMGetParam(method, pindex);
    if (ainfo->storage == LLVMArgGsharedvtFixed ||
        ainfo->storage == LLVMArgGsharedvtFixedVtype) {
    ...
    if (ainfo->storage == LLVMArgVtypeByRef ||
        ainfo->storage == LLVMArgVtypeAddr) {
      /* For OP_LDADDR */
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    for (ins = bb->code; ins; ins = ins->next) {
      ...
      case OP_VPHI:
      case OP_XPHI: {
        LLVMTypeRef phi_type = llvm_type_to_stack_type(
            cfg, type_to_llvm_type(ctx, m_class_get_byval_arg(ins->klass)));
        ...
        if (ins->opcode == OP_VPHI) {
          ...
          g_assert(ins->klass);
          phi_type = LLVMPointerType(
              type_to_llvm_type(ctx, m_class_get_byval_arg(ins->klass)), 0);
        }
  ...
  if (llvmonly_fail)
    /*
     * In llvmonly mode, we want to emit an llvm method for every method even if
     * it fails to compile, so direct calls can be made from outside the
     * assembly.
     */
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    g_hash_table_insert(
        ctx->region_to_handler,
        GUINT_TO_POINTER(mono_get_block_region_notry(cfg, bb->region)), bb);
    g_hash_table_insert(ctx->clause_to_handler, GINT_TO_POINTER(clause_index),
                        bb);

    /*
     * Create a new bblock which CALL_HANDLER/landing pads can branch to,
     * because branching to the LLVM bblock containing a landing pad causes
     * problems for the LLVM optimizer passes.
     */
    sprintf(name, "BB%d_CALL_HANDLER_TARGET", bb->block_num);
    ctx->bblocks[bb->block_num].call_handler_target_bb =
        LLVMAppendBasicBlock(ctx->lmethod, name);
  }
  ...
  if (ctx->llvm_only) {
    ...
    while (group_index < cfg->header->num_clauses) {
      ...
      while (cursor < cfg->header->num_clauses &&
             CLAUSE_START(&cfg->header->clauses[cursor]) ==
                 CLAUSE_START(&cfg->header->clauses[group_index]) &&
             CLAUSE_END(&cfg->header->clauses[cursor]) ==
                 CLAUSE_END(&cfg->header->clauses[group_index])) {
        count++;
        cursor++;
      }
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    for (l = ins_list; l; l = l->next) {
      ...
      if (phi->opcode == OP_VPHI) {
        g_assert(LLVMTypeOf(ctx->addresses[sreg1]) ==
                 LLVMTypeOf(values[phi->dreg]));
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    for (l = info->endfinally_switch_ins_list; l; l = l->next) {
      ...
      GSList *bb_list_iter;
      i = 0;
      for (bb_list_iter = bb_list; bb_list_iter;
           bb_list_iter = g_slist_next(bb_list_iter)) {
        LLVMAddCase(switch_ins, LLVMConstInt(LLVMInt32Type(), i + 1, FALSE),
                    (LLVMBasicBlockRef)bb_list_iter->data);
        i++;
      }
    }
  }
...
after_codegen_1:
  ...
  if (llvmonly_fail) {
    ...
    LLVMValueRef name_var =
        LLVMAddGlobal(ctx->lmodule, type, "missing_method_name");
    LLVMSetVisibility(name_var, LLVMHiddenVisibility);
    LLVMSetLinkage(name_var, LLVMInternalLinkage);
    LLVMSetInitializer(name_var, mono_llvm_create_constant_data_array(
                                     (guint8 *)name, len + 1));
    ...
    LLVMValueRef callee = get_callee(
        ctx, sig, MONO_PATCH_INFO_JIT_ICALL_ADDR,
        GUINT_TO_POINTER(
            MONO_JIT_ICALL_mini_llvmonly_throw_aot_failed_exception));
  ...
  if (cfg->compile_aot) {
    ...
    /*
     * NATIVE_TO_MANAGED methods might be called on a thread not attached to the
     * runtime, so they are initialized when loaded in load_method ().
     */
...
after_codegen:
  ...
  if (cfg->llvm_only) {
    ...
    for (int i = 0; i < ctx->callsite_list->len; ++i)
      g_ptr_array_add(ctx->module->callsite_list,
                      g_ptr_array_index(ctx->callsite_list, i));
  }
  ...
  if (ctx->module->method_to_lmethod)
    g_hash_table_insert(ctx->module->method_to_lmethod, cfg->method,
                        ctx->lmethod);

  if (ctx->module->idx_to_lmethod)
    g_hash_table_insert(ctx->module->idx_to_lmethod,
                        GINT_TO_POINTER(cfg->method_index), ctx->lmethod);

  if (ctx->llvm_only && m_class_is_valuetype(cfg->orig_method->klass) &&
      !(cfg->orig_method->flags & METHOD_ATTRIBUTE_STATIC))
    emit_unbox_tramp(ctx, ctx->method_name, ctx->method_type, ctx->lmethod,
                     cfg->method_index);
}
...
void mono_llvm_create_vars(MonoCompile *cfg) {
  ...
  if (cfg->gsharedvt && cfg->llvm_only) {
    ...
    if (mini_is_gsharedvt_variable_signature(sig) &&
      ...
      linfo = get_llvm_call_info(cfg, sig);
      vretaddr = (linfo->ret.storage == LLVMArgVtypeRetAddr ||
    ...
    if (vretaddr) {
       ...
       */
      cfg->vret_addr = mono_compile_create_var(
          cfg, m_class_get_byval_arg(mono_get_intptr_class()), OP_ARG);
...
void mono_llvm_emit_call(MonoCompile *cfg, MonoCallInst *call) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    switch (ainfo->storage) {
    case LLVMArgNormal: {
      MonoType *t = (sig->hasthis && i == 0)
                        ? m_class_get_byval_arg(mono_get_intptr_class())
                        : ainfo->type;
      int opcode;
...
static inline void add_func(LLVMModuleRef module, const char *name,
                            LLVMTypeRef ret_type, LLVMTypeRef *param_types,
                            int nparams) {
  LLVMAddFunction(module, name,
                  LLVMFunctionType(ret_type, param_types, nparams, FALSE));
}

static LLVMValueRef add_intrins(LLVMModuleRef module, IntrinsicId id,
                                LLVMTypeRef *params, int nparams) {
  return mono_llvm_register_overloaded_intrinsic(module, id, params, nparams);
}

static LLVMValueRef add_intrins1(LLVMModuleRef module, IntrinsicId id,
                                 LLVMTypeRef param1) {
  return mono_llvm_register_overloaded_intrinsic(module, id, &param1, 1);
}

static LLVMValueRef add_intrins2(LLVMModuleRef module, IntrinsicId id,
                                 LLVMTypeRef param1, LLVMTypeRef param2) {
  LLVMTypeRef params[] = {param1, param2};
  return mono_llvm_register_overloaded_intrinsic(module, id, params, 2);
}

static LLVMValueRef add_intrins3(LLVMModuleRef module, IntrinsicId id,
                                 LLVMTypeRef param1, LLVMTypeRef param2,
                                 LLVMTypeRef param3) {
...
static void add_intrinsic(LLVMModuleRef module, int id) {
  ...
  switch (id) {
  case INTRINS_MEMSET:
    intrins = add_intrins2(module, id, LLVMPointerType(LLVMInt8Type(), 0),
                           LLVMInt32Type());
    break;
  case INTRINS_MEMCPY:
    intrins = add_intrins3(module, id, LLVMPointerType(LLVMInt8Type(), 0),
                           LLVMPointerType(LLVMInt8Type(), 0), LLVMInt32Type());
    break;
  case INTRINS_MEMMOVE:
    intrins = add_intrins3(module, id, LLVMPointerType(LLVMInt8Type(), 0),
                           LLVMPointerType(LLVMInt8Type(), 0), LLVMInt64Type());
    break;
...
static LLVMValueRef get_intrins_from_module(LLVMModuleRef lmodule, int id) {
  ...
  res = (LLVMValueRef)g_hash_table_lookup(intrins_id_to_intrins,
                                          GINT_TO_POINTER(id));
  g_assert(res);
  return res;
}
...
static void add_intrinsics(LLVMModuleRef module) {
  ...
  /*
   * It would be nicer to emit only the intrinsics actually used, but LLVM's
   * Module type doesn't seem to do any locking.
   */
...
static void add_types(MonoLLVMModule *module) {
  module->ptr_type = LLVMPointerType(
      TARGET_SIZEOF_VOID_P == 8 ? LLVMInt64Type() : LLVMInt32Type(), 0);
}
...
void mono_llvm_create_aot_module(MonoAssembly *assembly,
  ...
  module->get_unbox_tramp_symbol =
      g_strdup_printf("%s_get_unbox_tramp", global_prefix);
  module->init_aotconst_symbol =
  ...
  if (module->emit_dwarf) {
    ...
    module->cu =
        mono_llvm_di_create_compile_unit(module->di_builder, cu_name, dir, s);
    g_free(dir);
    g_free(build_info);
    g_free(s);
  }
  ...
  if (emit_codeview) {
    ...
    LLVMAddNamedMetadataOperand(
        module->lmodule, "llvm.module.flags",
        LLVMMDNode(codeview_option_args, G_N_ELEMENTS(codeview_option_args)));
  }
  ...
  if (!static_link) {
    ...
    for (int i = 0; i < G_N_ELEMENTS(default_dynamic_lib_names); ++i) {
      ...
      default_lib_args[i] =
          LLVMMDString(default_lib_name, strlen(default_lib_name));
      default_lib_nodes[i] = LLVMMDNode(default_lib_args + i, 1);
    }

    LLVMAddNamedMetadataOperand(
        module->lmodule, "llvm.linker.options",
        LLVMMDNode(default_lib_args, G_N_ELEMENTS(default_lib_args)));
  }
#endif

  {
    ...
    module->dummy_got_var =
        LLVMAddGlobal(module->lmodule, got_type, "dummy_got");
  ...
  module->inited_var =
      LLVMAddGlobal(aot_module.lmodule, inited_type, "mono_inited_tmp");
  ...
  if (!use_mono_personality_debug) {
    LLVMValueRef personality =
        LLVMAddFunction(module->lmodule, default_personality_name,
                        LLVMFunctionType(LLVMInt32Type(), NULL, 0, TRUE));
    LLVMSetLinkage(personality, LLVMExternalLinkage);

    // EMCC chockes if the personality function is referenced in the 'used'
    // array
#ifndef TARGET_WASM
...
void mono_llvm_fixup_aot_module(void) {
  ...
  /*
   * Replace GOT entries for directly callable methods with the methods
   * themselves. It would be easier to implement this by predefining all methods
   * before compiling their bodies, but that couldn't handle the case when a
   * method fails to compile with llvm.
   */

  GHashTable *specializable = g_hash_table_new(NULL, NULL);

  GHashTable *patches_to_null =
      g_hash_table_new(mono_patch_info_hash, mono_patch_info_equal);
  for (int sindex = 0; sindex < module->callsite_list->len; ++sindex) {
    CallSite *site =
        (CallSite *)g_ptr_array_index(module->callsite_list, sindex);
    method = site->method;
    LLVMValueRef lmethod =
        (LLVMValueRef)g_hash_table_lookup(module->method_to_lmethod, method);
    ...
    if (!lmethod && method->is_inflated) {
      if (mono_method_is_generic_sharable_full(method, FALSE, TRUE, FALSE)) {
        ...
        if (is_ok(error)) {
          lmethod = (LLVMValueRef)g_hash_table_lookup(module->method_to_lmethod,
                                                      shared);
          if (lmethod)
            method = shared;
        }
    ...
    if (can_direct_call) {
      ...
      load = get_aotconst_module(module, builder, site->ji->type,
                                 site->ji->data.target, site->type, NULL, NULL);
  ...
  for (int i = 0; i < module->cfgs->len; ++i) {
    ...
    for (MonoJumpInfo *patch_info = cfg->patch_info; patch_info;
...
static LLVMValueRef llvm_array_from_uints(LLVMTypeRef el_type, guint32 *values,
                                          int nvalues) {
  int i;
...
static LLVMValueRef llvm_array_from_bytes(guint8 *values, int nvalues) {
  int i;
  LLVMValueRef res, *vals;
...
static LLVMValueRef AddJitGlobal(MonoLLVMModule *module, LLVMTypeRef type,
                                 const char *name) {
  char *s;
  LLVMValueRef v;
...
#define FILE_INFO_NFIELDS                                                      \
  (FILE_INFO_NUM_HEADER_FIELDS + MONO_AOT_FILE_INFO_NUM_SYMBOLS +              \
   FILE_INFO_NUM_SCALAR_FIELDS + FILE_INFO_NUM_ARRAY_FIELDS +                  \
   FILE_INFO_NUM_AOTID_FIELDS)
...
static void emit_aot_file_info(MonoLLVMModule *module) {
  ...
  /* Symbols */
  /*
   * We use LLVMGetNamedGlobal () for symbol which are defined in LLVM code, and
   * LLVMAddGlobal () for symbols defined in the .s file emitted by the aot
   * compiler.
   */
  ...
  if (!module->llvm_only) {
    ...
    fields[tindex++] = module->get_unbox_tramp ? module->get_unbox_tramp
  ...
  if (info->trampoline_size[0]) {
    ...
    fields[tindex++] =
        AddJitGlobal(module, eltype, "gsharedvt_arg_trampolines");
    fields[tindex++] = AddJitGlobal(module, eltype, "ftnptr_arg_trampolines");
    fields[tindex++] =
        AddJitGlobal(module, eltype, "unbox_arbitrary_trampolines");
  } else {
    fields[tindex++] = LLVMConstNull(eltype);
    fields[tindex++] = LLVMConstNull(eltype);
    fields[tindex++] = LLVMConstNull(eltype);
  ...
  if (!module->llvm_only) {
    ...
    fields[tindex++] =
        AddJitGlobal(module, eltype, "unbox_trampoline_addresses");
  } else {
    fields[tindex++] = LLVMConstNull(eltype);
    fields[tindex++] = LLVMConstNull(eltype);
    fields[tindex++] = LLVMConstNull(eltype);
  ...
  for (i = 0; i < MONO_AOT_FILE_INFO_NUM_SYMBOLS; ++i) {
    ...
    fields[FILE_INFO_NUM_HEADER_FIELDS + i] =
        LLVMConstBitCast(fields[FILE_INFO_NUM_HEADER_FIELDS + i], eltype);
  }

  /* Scalars */
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), info->plt_got_offset_base, FALSE);
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), info->plt_got_info_offset_base, FALSE);
  ...
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), info->num_rgctx_fetch_trampolines, FALSE);
  ...
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), info->generic_tramp_num, FALSE);
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), info->card_table_shift_bits, FALSE);
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), info->card_table_mask, FALSE);
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), info->tramp_page_size, FALSE);
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), info->call_table_entry_size, FALSE);
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), info->nshared_got_entries, FALSE);
  fields[tindex++] = LLVMConstInt(LLVMInt32Type(), info->datafile_size, FALSE);
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), module->unbox_tramp_num, FALSE);
  fields[tindex++] =
      LLVMConstInt(LLVMInt32Type(), module->unbox_tramp_elemsize, FALSE);
  /* Arrays */
  fields[tindex++] = llvm_array_from_uints(LLVMInt32Type(), info->table_offsets,
                                           MONO_AOT_TABLE_NUM);
  fields[tindex++] = llvm_array_from_uints(
      LLVMInt32Type(), info->num_trampolines, MONO_AOT_TRAMP_NUM);
  fields[tindex++] = llvm_array_from_uints(
      LLVMInt32Type(), info->trampoline_got_offset_base, MONO_AOT_TRAMP_NUM);
  fields[tindex++] = llvm_array_from_uints(
      LLVMInt32Type(), info->trampoline_size, MONO_AOT_TRAMP_NUM);
  fields[tindex++] = llvm_array_from_uints(
      LLVMInt32Type(), info->tramp_page_code_offsets, MONO_AOT_TRAMP_NUM);

  fields[tindex++] = llvm_array_from_bytes(info->aotid, 16);
  g_assert(tindex == nfields);

  LLVMSetInitializer(
      info_var, LLVMConstNamedStruct(LLVMGetElementType(LLVMTypeOf(info_var)),
                                     fields, nfields));
  ...
  if (module->static_link) {
    ...
    s = g_strdup_printf("mono_aot_module_%s_info",
                        module->assembly->aname.name);
    ...
    g_free(s);
    LLVMSetInitializer(
        var, LLVMConstBitCast(
                 LLVMGetNamedGlobal(module->lmodule, "mono_aot_file_info"),
                 LLVMPointerType(LLVMInt8Type(), 0)));
...
static void mono_llvm_nonnull_state_update(EmitContext *ctx, LLVMValueRef lcall,
  ...
  if (mono_aot_can_specialize(call_method)) {
    ...
    GArray *call_site_union = (GArray *)g_hash_table_lookup(
        ctx->module->method_to_call_info, call_method);

    if (!call_site_union) {
      call_site_union =
          g_array_sized_new(FALSE, TRUE, sizeof(gint32), num_params);
      int zero = 0;
    ...
    g_hash_table_insert(ctx->module->method_to_call_info, call_method,
                        call_site_union);
  }
}
...
static void mono_llvm_propagate_nonnull_final(GHashTable *all_specializable,
  ...
  // nonnull (the roots). Then, for all of the methods that can be specialized,
  // we see if their call sites have nonnull attributes.

  // If so, we mark the function's param. This param has uses to propagate
  // the attribute to. This propagation can trigger a need to mark more
  // attributes non-null, and so on and so forth.
  GSList *queue = NULL;
  ...
  while (g_hash_table_iter_next(&iter, (void **)&lmethod, (void **)&method)) {
    GArray *call_site_union =
        (GArray *)g_hash_table_lookup(module->method_to_call_info, method);
  ...
  // because we are only working with a set of subgraphs of specializable
  // functions.
  //
  ...
  // all_specializable is a function that can be specialized, and is the
  // resulting node in the graph after all of the subsitutions are done.
  //
  // Anything disrupting the direct calls made with self-init will break this
  // optimization.
  ...
  while (queue) {
    ...
    // Get the actual LLVM value of the argument, so we can see which call
    // instructions used that argument
    LLVMValueRef caller_argument =
        LLVMGetParam(current->lmethod, current->argument);
    ...
    for (GSList *cursor = calls; cursor != NULL; cursor = cursor->next) {
      ...
      MonoMethod *callee_method =
          (MonoMethod *)g_hash_table_lookup(all_specializable, callee_lmethod);
      if (!callee_method)
        continue;

      // Decrement number of nullable refs at that func's arg offset
      GArray *call_site_union = (GArray *)g_hash_table_lookup(
          module->method_to_call_info, callee_method);

      // It has module-local callers and is specializable, should have seen this
      // call site and inited this
      ...
      for (int call_argument = 0; call_argument < max_params; call_argument++) {
        // Every time we used the newly-non-nullable argument, decrement the
        // nullable refcount for that function.
        if (caller_argument == operands[call_argument]) {
          gint32 *nullable_count =
              &g_array_index(call_site_union, gint32, call_argument);
          g_assert(*nullable_count > 0);
          *nullable_count = *nullable_count - 1;

          // If we caused that callee's parameter to become newly nullable, add
          // to work queue
          if (*nullable_count == 0) {
            NonnullPropWorkItem *item = g_malloc(sizeof(NonnullPropWorkItem));
      ...
      g_hash_table_insert(module->method_to_call_info, callee_method,
                          call_site_union);
    }
    g_slist_free(calls);
...
void mono_llvm_emit_aot_module(const char *filename, const char *cu_name) {
  ...
  for (int i = 0; i < module->cfgs->len; ++i) {
    ...
    if (cfg->llvm_dummy_info_var) {
      if (cfg->llvm_info_var) {
        ...
        LLVMSetInitializer(cfg->llvm_dummy_info_var,
                           mono_llvm_create_constant_data_array(NULL, 0));
      }
    }
  }
  ...
  /* Replace PLT entries for directly callable methods with the methods
   * themselves */
  {
    GHashTableIter iter;
    MonoJumpInfo *ji;
...
static LLVMValueRef md_string(const char *s) {
...
static LLVMValueRef emit_dbg_subprogram(EmitContext *ctx, MonoCompile *cfg,
  ...
  mono_debug_get_seq_points(minfo, &source_file, NULL, NULL, &sym_seq_points,
                            &n_seq_points);
  if (!source_file)
  ...
  return (LLVMValueRef)mono_llvm_di_create_function(
...
static void emit_dbg_loc(EmitContext *ctx, LLVMBuilderRef builder,
  ...
  if (ctx->minfo && cil_code && cil_code >= cfg->header->code &&
      cil_code < cfg->header->code + cfg->header->code_size) {
    MonoDebugSourceLocation *loc;
    LLVMValueRef loc_md;

    loc = mono_debug_method_lookup_location(ctx->minfo,
                                            cil_code - cfg->header->code);

    if (loc) {
      loc_md = (LLVMValueRef)mono_llvm_di_create_location(
          ctx->module->di_builder, ctx->dbg_md, loc->row, loc->column);
...
static void emit_default_dbg_loc(EmitContext *ctx, LLVMBuilderRef builder) {
  if (ctx->minfo) {
    LLVMValueRef loc_md;
    loc_md = (LLVMValueRef)mono_llvm_di_create_location(ctx->module->di_builder,
                                                        ctx->dbg_md, 0, 0);
    mono_llvm_di_set_location(builder, loc_md);
  }
}
...
/*
  ...
  - Handling of opcodes which can throw exceptions.
      ...
      ex_label:
          ...
          The problematic part is push throw_pos - method, which cannot be
  represented in the LLVM IR, since it does not support label values.
          -> this can be implemented in AOT mode using inline asm + labels, but
  cannot be implemented in JIT mode ?
    ...
    - SSA is disabled for methods with exception handlers
        - How to obtain unwind info for LLVM compiled methods ?
          -> this is now solved by converting the unwind info generated by LLVM
             into our format.
        - LLVM uses the c++ exception handling framework, while we use our home
  grown code, and couldn't use the c++ one:
  ...
  - argument passing.
          ...
          LLVM does not allow the specification of argument registers etc. This
  means that all calls are made according to the platform ABI.

  - passing/receiving vtypes.

      Vtypes passed/received in registers are handled by the front end by using
          a signature with scalar arguments, and loading the parts of the vtype
  ...
  - types.
    ...
    The mono JIT uses pointer sized iregs/double fregs, while LLVM uses
  precisely typed registers, so we have to keep track of the precise LLVM type
  of each vreg. This is made easier because the IR is already in SSA form. An
  additional problem is that our IR is not consistent with types, i.e. i32/i64
...
/*
  ...
  link it with the file containing the methods emitted by the JIT and the AOT
  data structures.
*/

/* FIXME: Normalize some aspects of the mono IR to allow easier translation,
 * like:
 *   - each bblock should end with a branch
 *   - setting the return value, making cfg->ret non-volatile
 * - avoid some transformations in the JIT which make it harder for us to
 * generate code.
...
static void decode_llvm_eh_info(EmitContext *ctx, gpointer eh_frame) {
  ...
  cfg->llvm_ex_info = (MonoJitExceptionInfo *)mono_mempool_alloc0(
      cfg->mempool, (ei_len + nested_len) * sizeof(MonoJitExceptionInfo));
...
static void llvm_jit_finalize_method(EmitContext *ctx) {
  ...
  cfg->native_code = (guint8 *)mono_llvm_compile_method(
      ctx->module->mono_ee, cfg, ctx->lmethod, nvars, callee_vars, callee_addrs,
      &eh_frame);
  ...
  while (g_hash_table_iter_next(&iter, (void **)&callee, (void **)&var)) {
    GSList *addrs =
        (GSList *)g_hash_table_lookup(domain_info->llvm_jit_callees, callee);
...
static void init_jit_module(MonoDomain *domain) { g_assert_not_reached(); }
...
MonoCPUFeatures mono_llvm_get_cpu_features(void) {
  ...
  if (!cpu_features)
    cpu_features =
        MONO_CPU_INITED | (MonoCPUFeatures)mono_llvm_check_cpu_features(
                              flags_map, G_N_ELEMENTS(flags_map));
