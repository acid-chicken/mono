...
static void process_bb(EmitContext *ctx, MonoBasicBlock *bb) {
  ...
  for (ins = bb->code; ins; ins = ins->next) {
    ...
    case OP_FNEG:
      ...
      values[ins->dreg] = LLVMBuildFSub(
          builder, LLVMConstReal(LLVMDoubleType(), 0.0), lhs, dname);
      break;
    case OP_RNEG:
      lhs = convert(ctx, lhs, LLVMFloatType());
      values[ins->dreg] = LLVMBuildFSub(
          builder, LLVMConstReal(LLVMFloatType(), 0.0), lhs, dname);
      break;
    case OP_INOT: {
    ...
    case OP_STORE_MEMBASE_REG: {
      ...
      if (is_volatile && LLVMGetInstructionOpcode(base) == LLVMAlloca &&
        ...
        is_volatile = FALSE;
      emit_store(ctx, bb, &builder, size, convert(ctx, values[ins->sreg1], t),
                 convert(ctx, addr, LLVMPointerType(t, 0)), base, is_faulting,
                 is_volatile);
      break;
    }
...
#define ARM64_ATOMIC_FENCE_FIX
    ...
    case OP_XEQUAL: {
      ...
      LLVMValueRef cmp_zero =
          LLVMBuildICmp(builder, LLVMIntNE, first_elem,
