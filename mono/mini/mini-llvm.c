...
static gboolean method_is_direct_callable(MonoMethod *method) {
  ...
  /*
  if (!method->is_inflated && (mono_class_get_flags (method->klass) &
  TYPE_ATTRIBUTE_PUBLIC) && (method->flags & METHOD_ATTRIBUTE_PUBLIC)) return
  TRUE;
  */
  return FALSE;
}
  ...
  case AOT_INIT_METHOD_GSHARED_VTABLE:
    ...
    func = LLVMAddFunction(lmodule, name,
                           LLVMFunctionType2(LLVMVoidType(), LLVMInt32Type(),
                                             IntPtrType(), FALSE));
    sig = LLVMFunctionType3(LLVMVoidType(), IntPtrType(), LLVMInt32Type(),
                            IntPtrType(), FALSE);
    break;
...
#define ARM64_ATOMIC_FENCE_FIX
    ...
    case OP_SSE_LOADU: {
      LLVMValueRef dst_ptr = convert(
          ctx, lhs,
          LLVMPointerType(primitive_type_to_llvm_type(ins->inst_c1), 0));
      ...
      if (ins->inst_c1 == MONO_TYPE_R4) {
        ...
        values[ins->dreg] = LLVMBuildInsertElement(
            ...
            convert(ctx, values[ins->sreg2],
                    primitive_type_to_llvm_type(ins->inst_c1)),
...
static gboolean is_externally_callable(EmitContext *ctx, MonoMethod *method) {
...
static void emit_method_inner(EmitContext *ctx) {
  ...
  if (cfg->method->wrapper_type == MONO_WRAPPER_OTHER) {
    ...
    if (info->subtype == WRAPPER_SUBTYPE_AOT_INIT) {
      ...
      ctx->method_name =
          g_strdup_printf("%s%s", ctx->module->global_prefix, init_name);
...
static void emit_aot_file_info(MonoLLVMModule *module) {
  ...
  LLVMSetInitializer(info_var,
                     LLVMConstNamedStruct(file_info_type, fields, nfields));
