...
typedef struct {
  ...
  guint32 il_offset;
  /*
   * If method is gshared, this is the actual instance, otherwise this is equal
   * to method.
   */
  ...
  /*
   * Whenever ctx is set. This is FALSE for the last frame of running threads,
   * since the frame can become invalid.
   */
  gboolean has_ctx;
} StackFrame;
...
struct _DebuggerTlsData {
  ...
  StackFrame **frames;
  /*
   * Whenever the frame info is up-to-date. If not, compute_frame_info () will
   * need to re-compute it.
   */
  gboolean frames_up_to_date;
  /*
   * Points to data about a pending invoke which needs to be executed after the
   * thread resumes.
   */
  InvokeData *pending_invoke;
  /*
   * Set to TRUE if this thread is suspended in suspend_current () or it is
   * executing native code.
   */
  gboolean suspended;
  /*
   * Signals whenever the thread is in the process of suspending, i.e. it will
   * suspend within a finite amount of time.
   */
  gboolean suspending;
  /*
  ...
  /*
   * The context which needs to be restored after handling a single
   * step/breakpoint event. This is the same as the ctx at step/breakpoint site,
   * but includes changes to caller saved registers done by set_var ().
   */
...
typedef enum { CMD_COMPOSITE = 100 } CmdComposite;
...
typedef enum { CMD_POINTER_GET_VALUE = 1 } CmdPointer;
 ...
 * When using the onuncaught or onthrow options, only some parts of the agent
 * are initialized on startup, and the full initialization which includes
 * connection establishment and the startup of the agent thread is only done in
...
static void emit_appdomain_load(gpointer key, gpointer value,
                                gpointer user_data);

static void emit_thread_start(gpointer key, gpointer value, gpointer user_data);

static void invalidate_each_thread(gpointer key, gpointer value,
                                   gpointer user_data);
...
static void jit_done(MonoProfiler *prof, MonoMethod *method,
                     MonoJitInfo *jinfo);
...
static gboolean begin_breakpoint_processing(void *tls, MonoContext *ctx,
                                            MonoJitInfo *ji,
                                            gboolean from_signal);
static void begin_single_step_processing(MonoContext *ctx,
                                         gboolean from_signal);
static void ss_discard_frame_context(void *the_tls);
static void ss_calculate_framecount(void *tls, MonoContext *ctx,
...
static void *create_breakpoint_events(GPtrArray *ss_reqs, GPtrArray *bp_reqs,
                                      MonoJitInfo *ji, EventKind kind);
static void process_breakpoint_events(void *_evts, MonoMethod *method,
                                      MonoContext *ctx, int il_offset);
...
#ifndef DISABLE_SOCKET_TRANSPORT
    static void register_socket_transport(void);
#endif

static gboolean is_debugger_thread(void) {
...
static int parse_address(char *address, char **host, int *port) {
...
static void print_usage(void) {
  ...
  g_printerr("  transport=<transport>\t\tTransport to use for connecting to "
             "the debugger (mandatory, possible values: 'dt_socket')\n");
  g_printerr(
...
static gboolean parse_flag(const char *option, char *flag) {
  if (!strcmp(flag, "y"))
    return TRUE;
  else if (!strcmp(flag, "n"))
    return FALSE;
  else {
    g_printerr("debugger-agent: The valid values for the '%s' option are 'y' "
               "and 'n'.\n",
               option);
    exit(1);
    return FALSE;
...
static void debugger_agent_parse_options(char *options) {
  char **args, **ptr;
  char *host;
  int port;
  char *extra;
  ...
  for (ptr = args; ptr && *ptr; ptr++) {
    ...
    if (strncmp(arg, "transport=", 10) == 0) {
      ...
      agent_config.onthrow =
          g_slist_append(agent_config.onthrow, g_strdup(arg + 8));
  ...
  if (agent_config.server && !agent_config.suspend) {
    ...
    if (agent_config.address == NULL) {
      agent_config.address = g_strdup_printf(
          "0.0.0.0:%u", 56000 + (mono_process_current_pid() % 1000));
    }
  }
  ...
  if (!strcmp(agent_config.transport, "dt_socket")) {
    if (agent_config.address &&
        parse_address(agent_config.address, &host, &port)) {
      g_printerr("debugger-agent: The format of the 'address' options is "
                 "'<host>:<port>'\n");
      exit(1);
    }
  }
...
void mono_debugger_set_thread_state(DebuggerTlsData *tls,
...
MonoDebuggerThreadState mono_debugger_get_thread_state(DebuggerTlsData *tls) {
  g_assertf(tls, "Cannot get state of null thread", NULL);

  return tls->thread_state;
}

gsize mono_debugger_tls_thread_id(DebuggerTlsData *tls) {
  if (!tls)
    return 0;
...
MonoGHashTable *mono_debugger_get_thread_states(void) { return thread_to_tls; }

gboolean mono_debugger_is_disconnected(void) { return disconnected; }

static void debugger_agent_init(void) {
  ...
  cbs.set_set_notification_for_wait_completion_flag =
      set_set_notification_for_wait_completion_flag;
  cbs.get_notify_debugger_of_wait_completion_method =
  ...
  thread_to_tls = mono_g_hash_table_new_type_internal(
      (GHashFunc)mono_object_hash_internal, NULL, MONO_HASH_KEY_GC,
      MONO_ROOT_SOURCE_DEBUGGER, NULL, "Debugger TLS Table");

  tid_to_thread = mono_g_hash_table_new_type_internal(
      NULL, NULL, MONO_HASH_VALUE_GC, MONO_ROOT_SOURCE_DEBUGGER, NULL,
      "Debugger Thread Table");

  tid_to_thread_obj = mono_g_hash_table_new_type_internal(
      NULL, NULL, MONO_HASH_VALUE_GC, MONO_ROOT_SOURCE_DEBUGGER, NULL,
  ...
  /*
   * The stack walk done from thread_interrupt () needs to be signal safe, but
   * it isn't, since it can call into mono_aot_find_jit_info () which is not
   * signal safe (#3411). So load AOT info eagerly when the debugger is running
   * as a workaround.
   */
 ...
 * finish_agent_init:
 *
 *   Finish the initialization of the agent. This involves connecting the
 * transport and starting the agent thread. This is either done at startup, or
...
static void finish_agent_init(gboolean on_startup) {
  ...
  if (agent_config.launch) {
    ...
    char *argv[] = {agent_config.launch, agent_config.transport,
                    agent_config.address, NULL};
#ifdef G_OS_WIN32
    // Nothing. FIXME? g_spawn_async_with_pipes is easy enough to provide for
    // Windows if needed.
#elif !HAVE_G_SPAWN
    g_printerr("g_spawn_async_with_pipes not supported on this platform\n");
    exit(1);
#else
    int res = g_spawn_async_with_pipes(NULL, argv, NULL, (GSpawnFlags)0, NULL,
...
static void mono_debugger_agent_cleanup(void) {
  if (!inited)
    return;
 ...
 */
static int socket_transport_recv(void *buf, int len) {
  int res;
  int total = 0;
  int fd = conn_fd;
  int flags = 0;
    ...
    if (agent_config.keepalive) {
      ...
      if (need_keepalive) {
        ...
        goto again;
           ...
           (res == -1 && get_last_sock_error() == MONO_EINTR));
...
static void set_keepalive(void) {
  struct timeval tv;
  int result;
  ...
  result = setsockopt(conn_fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv,
                      sizeof(struct timeval));
  g_assert(result >= 0);
}

static int socket_transport_accept(int socket_fd) {
  MONO_ENTER_GC_SAFE;
...
static gboolean socket_transport_send(void *data, int len) {
  int res;
 ...
 *
 *   Connect/Listen on HOST:PORT. If HOST is NULL, generate an address and
 * listen on it.
 */
static void socket_transport_connect(const char *address) {
  ...
  if (host) {
    int hints[] = {MONO_HINT_IPV4 | MONO_HINT_NUMERIC_HOST,
    ...
    if (s != 0) {
      g_printerr("debugger-agent: Unable to resolve %s:%d: %d\n", host, port,
                 s); // FIXME add portable error conversion functions
      exit(1);
    }
  }
  ...
  if (agent_config.server) {
    ...
    if (!host) {
      ...
      for (rp = result->entries; rp != NULL; rp = rp->next) {
        ...
        mono_socket_address_init(&sockaddr, &sock_len, rp->family, &rp->address,
                                 port);
        ...
        if (setsockopt(sfd, SOL_SOCKET, SO_REUSEADDR, (const char *)&n,
                       sizeof(n)) == -1)
          continue;
...
static void socket_transport_close1(void) {
...
static void socket_transport_close2(void) {
#ifdef HOST_WIN32
  shutdown(conn_fd, SD_BOTH);
#else
 ...
 *
 */
static void socket_fd_transport_connect(const char *address) {
  int res;
...
static void register_socket_fd_transport(void) {
  DebuggerTransport trans;

  /* This is the same as the 'dt_socket' transport, but receives an already
   * connected socket fd */
...
MONO_API void mono_debugger_agent_register_transport(DebuggerTransport *trans);
...
static void transport_init(void) {
  ...
  if (i == ntransports) {
    g_printerr("debugger-agent: The supported values for the 'transport' "
               "option are: ");
    for (i = 0; i < ntransports; ++i)
...
void transport_connect(const char *address) { transport->connect(address); }

static void transport_close1(void) { transport->close1(); }

static void transport_close2(void) { transport->close2(); }

static int transport_send(void *buf, int len) {
  return transport->send(buf, len);
}

static int transport_recv(void *buf, int len) {
...
static gboolean transport_handshake(void) {
  ...
  if ((res != strlen(handshake_msg)) ||
      (memcmp(buf, handshake_msg, strlen(handshake_msg)) != 0)) {
    g_printerr("debugger-agent: DWP handshake failed.\n");
    return FALSE;
  }

  /*
   * To support older clients, the client sends its protocol version after
   * connecting using a command. Until that is received, default to our protocol
   * version.
   */
  ...
  if (conn_fd) {
    int flag = 1;
    int result = setsockopt(conn_fd, IPPROTO_TCP, TCP_NODELAY, (char *)&flag,
                            sizeof(int));
    g_assert(result >= 0);
  }
...
static void stop_debugger_thread(void) {
   ...
   *
   * If we continue with the shutdown without waiting for it, then the client
   * might not receive an answer to its last command like a resume.
   */
  if (!is_debugger_thread()) {
    do {
      mono_coop_mutex_lock(&debugger_thread_exited_mutex);
      if (!debugger_thread_exited)
        mono_coop_cond_wait(&debugger_thread_exited_cond,
                            &debugger_thread_exited_mutex);
      mono_coop_mutex_unlock(&debugger_thread_exited_mutex);
    } while (!debugger_thread_exited);

    if (debugger_thread_handle)
      mono_thread_info_wait_one_handle(debugger_thread_handle,
                                       MONO_INFINITE_WAIT, TRUE);
  }
...
static void start_debugger_thread(MonoError *error) {
  ...
  thread = mono_thread_create_internal(
      mono_get_root_domain(), (gpointer)debugger_thread, NULL,
      MONO_THREAD_CREATE_FLAGS_DEBUGGER, error);
  return_if_nok(error);
...
static int decode_byte(guint8 *buf, guint8 **endbuf, guint8 *limit) {
  *endbuf = buf + 1;
  g_assert(*endbuf <= limit);
  return buf[0];
}

static int decode_int(guint8 *buf, guint8 **endbuf, guint8 *limit) {
  *endbuf = buf + 4;
  g_assert(*endbuf <= limit);
...
static gint64 decode_long(guint8 *buf, guint8 **endbuf, guint8 *limit) {
...
static int decode_id(guint8 *buf, guint8 **endbuf, guint8 *limit) {
  return decode_int(buf, endbuf, limit);
}

static char *decode_string(guint8 *buf, guint8 **endbuf, guint8 *limit) {
...
static void buffer_init(Buffer *buf, int size) {
  buf->buf = (guint8 *)g_malloc(size);
  buf->p = buf->buf;
  buf->end = buf->buf + size;
}

static int buffer_len(Buffer *buf) { return buf->p - buf->buf; }

static void buffer_make_room(Buffer *buf, int size) {
...
static void buffer_add_byte(Buffer *buf, guint8 val) {
  buffer_make_room(buf, 1);
  buf->p[0] = val;
  buf->p++;
}

static void buffer_add_short(Buffer *buf, guint32 val) {
...
static void buffer_add_int(Buffer *buf, guint32 val) {
...
static void buffer_add_long(Buffer *buf, guint64 l) {
  buffer_add_int(buf, (l >> 32) & 0xffffffff);
  buffer_add_int(buf, (l >> 0) & 0xffffffff);
}

static void buffer_add_id(Buffer *buf, int id) {
  buffer_add_int(buf, (guint64)id);
}

static void buffer_add_data(Buffer *buf, guint8 *data, int len) {
  buffer_make_room(buf, len);
  memcpy(buf->p, data, len);
  buf->p += len;
}

static void buffer_add_utf16(Buffer *buf, guint8 *data, int len) {
...
static void buffer_add_string(Buffer *buf, const char *str) {
  int len;
...
static void buffer_add_buffer(Buffer *buf, Buffer *data) {
  buffer_add_data(buf, data->buf, buffer_len(data));
}

static void buffer_free(Buffer *buf) { g_free(buf->buf); }

static gboolean send_packet(int command_set, int command, Buffer *data) {
  Buffer buf;
  int len, id;
  gboolean res;
...
static gboolean send_reply_packets(int npackets, ReplyPacket *packets) {
  Buffer buf;
  int i, len;
  gboolean res;
...
static gboolean send_reply_packet(int id, int error, Buffer *data) {
  ReplyPacket packet;
...
static void send_buffered_reply_packets(void) {
  int i;
...
static void buffer_reply_packet(int id, int error, Buffer *data) {
  ReplyPacket *p;
...
static void objrefs_init(void) {
  ...
  suspended_objs = mono_g_hash_table_new_type_internal(
      (GHashFunc)mono_object_hash_internal, NULL, MONO_HASH_KEY_GC,
      MONO_ROOT_SOURCE_DEBUGGER, NULL, "Debugger Suspended Object Table");
}
 ...
 */
static ObjRef *get_objref(MonoObject *obj) {
  ObjRef *ref;
  ...
  if (suspend_count) {
    /*
     * Have to keep object refs created during suspensions alive for the
     * duration of the suspension, so GCs during invokes don't collect them.
     */
    dbg_lock();
  ...
  if (mono_gc_is_moving()) {
    ...
    reflist =
        (GSList *)g_hash_table_lookup(obj_to_objref, GINT_TO_POINTER(hash));
    ...
    ref = (ObjRef *)g_hash_table_lookup(obj_to_objref,
                                        GINT_TO_POINTER(~((gsize)obj)));
...
static gboolean true_pred(gpointer key, gpointer value, gpointer user_data) {
  return TRUE;
}

static void clear_suspended_objs(void) {
  dbg_lock();
  mono_g_hash_table_foreach_remove(suspended_objs, true_pred, NULL);
  dbg_unlock();
}

static int get_objid(MonoObject *obj) {
  if (!obj)
    return 0;
  else
 ...
 * collected.
 */
static ErrorCode get_object_allow_null(int objid, MonoObject **obj) {
  ObjRef *ref;
...
static ErrorCode get_object(int objid, MonoObject **obj) {
...
static int decode_objid(guint8 *buf, guint8 **endbuf, guint8 *limit) {
  return decode_id(buf, endbuf, limit);
}

static void buffer_add_objid(Buffer *buf, MonoObject *o) {
...
static void debugger_agent_free_domain_info(MonoDomain *domain) {
  AgentDomainInfo *info =
      (AgentDomainInfo *)domain_jit_info(domain)->agent_info;
  int i, j;
  GHashTableIter iter;
...
static AgentDomainInfo *get_agent_domain_info(MonoDomain *domain) {
  ...
  info->source_file_to_class_ignorecase =
...
static gpointer decode_ptr_id(guint8 *buf, guint8 **endbuf, guint8 *limit,
                              IdType type, MonoDomain **domain,
                              ErrorCode *err) {
  Id *res;
...
static int buffer_add_ptr_id(Buffer *buf, MonoDomain *domain, IdType type,
                             gpointer val) {
...
static MonoClass *decode_typeid(guint8 *buf, guint8 **endbuf, guint8 *limit,
                                MonoDomain **domain, ErrorCode *err) {
  MonoClass *klass;
...
static MonoAssembly *decode_assemblyid(guint8 *buf, guint8 **endbuf,
                                       guint8 *limit, MonoDomain **domain,
                                       ErrorCode *err) {
  return (MonoAssembly *)decode_ptr_id(buf, endbuf, limit, ID_ASSEMBLY, domain,
                                       err);
}

static MonoImage *decode_moduleid(guint8 *buf, guint8 **endbuf, guint8 *limit,
                                  MonoDomain **domain, ErrorCode *err) {
  return (MonoImage *)decode_ptr_id(buf, endbuf, limit, ID_MODULE, domain, err);
}

static MonoMethod *decode_methodid(guint8 *buf, guint8 **endbuf, guint8 *limit,
                                   MonoDomain **domain, ErrorCode *err) {
  MonoMethod *m;
...
static MonoClassField *decode_fieldid(guint8 *buf, guint8 **endbuf,
                                      guint8 *limit, MonoDomain **domain,
                                      ErrorCode *err) {
  return (MonoClassField *)decode_ptr_id(buf, endbuf, limit, ID_FIELD, domain,
                                         err);
}

static MonoDomain *decode_domainid(guint8 *buf, guint8 **endbuf, guint8 *limit,
                                   MonoDomain **domain, ErrorCode *err) {
  return (MonoDomain *)decode_ptr_id(buf, endbuf, limit, ID_DOMAIN, domain,
                                     err);
}

static MonoProperty *decode_propertyid(guint8 *buf, guint8 **endbuf,
                                       guint8 *limit, MonoDomain **domain,
                                       ErrorCode *err) {
  return (MonoProperty *)decode_ptr_id(buf, endbuf, limit, ID_PROPERTY, domain,
                                       err);
}

static void buffer_add_typeid(Buffer *buf, MonoDomain *domain,
                              MonoClass *klass) {
...
static void buffer_add_methodid(Buffer *buf, MonoDomain *domain,
                                MonoMethod *method) {
...
static void buffer_add_assemblyid(Buffer *buf, MonoDomain *domain,
                                  MonoAssembly *assembly) {
  int id;
...
static void buffer_add_moduleid(Buffer *buf, MonoDomain *domain,
                                MonoImage *image) {
  buffer_add_ptr_id(buf, domain, ID_MODULE, image);
}

static void buffer_add_fieldid(Buffer *buf, MonoDomain *domain,
                               MonoClassField *field) {
  buffer_add_ptr_id(buf, domain, ID_FIELD, field);
}

static void buffer_add_propertyid(Buffer *buf, MonoDomain *domain,
                                  MonoProperty *property) {
  buffer_add_ptr_id(buf, domain, ID_PROPERTY, property);
}

static void buffer_add_domainid(Buffer *buf, MonoDomain *domain) {
...
static MonoJitInfo *get_top_method_ji(gpointer ip, MonoDomain **domain,
                                      gpointer *out_ip) {
  MonoJitInfo *ji;
  ...
  if (!ji) {
    ...
    g_assert(ext->kind == MONO_LMFEXT_INTERP_EXIT ||
 ...
 * save_thread_context:
 *
 *   Set CTX as the current threads context which is used for computing stack
 * traces. This function is signal-safe.
 */
static void save_thread_context(MonoContext *ctx) {
  DebuggerTlsData *tls;
...
static gboolean get_last_frame(StackFrameInfo *info, MonoContext *ctx,
                               gpointer user_data) {
  GetLastFrameUserData *data = (GetLastFrameUserData *)user_data;

  if (info->type == FRAME_TYPE_MANAGED_TO_NATIVE ||
      info->type == FRAME_TYPE_TRAMPOLINE)
    return FALSE;
...
static void copy_unwind_state_from_frame_data(MonoThreadUnwindState *to,
 ...
 */
static void thread_interrupt(DebuggerTlsData *tls, MonoThreadInfo *info,
                             MonoJitInfo *ji) {
  gpointer ip;
  ...
  // FIXME: Races when the thread leaves managed code before hitting a single
  // step event.
  ...
  if (ji && !ji->is_trampoline) {
     ...
     * This might interrupt the code in mono_de_process_single_step (), we use
     * the tls->suspending flag to avoid races when that happens.
     */
    if (!tls->suspended && !tls->suspending) {
      ...
      if (ip)
        DEBUG_PRINTF(
            1, "[%p] Received interrupt while at %p, treating as suspended.\n",
       ...
       * parent frame's ctx+lmf. This (hopefully) works because the thread will
       * be suspended when it returns to managed code, so the parent's ctx
       * should remain valid.
       */
      MonoThreadUnwindState *state = mono_thread_info_get_suspend_state(info);

      data.last_frame_set = FALSE;
      mono_get_eh_callbacks()->mono_walk_stack_with_state(
          get_last_frame, state, MONO_UNWIND_SIGNAL_SAFE, &data);
      if (data.last_frame_set) {
        gpointer jit_tls = tls->thread->thread_info->jit_data;

        memcpy(&tls->async_last_frame, &data.last_frame,
               sizeof(StackFrameInfo));

        if (data.last_frame.type == FRAME_TYPE_INTERP_TO_MANAGED ||
            data.last_frame.type == FRAME_TYPE_INTERP_TO_MANAGED_WITH_CTX) {
          /*
        ...
        /* Don't set tls->context, it could race with the thread processing a
         * breakpoint etc. */
      } else {
 ...
 */
static void reset_native_thread_suspend_state(gpointer key, gpointer value,
                                              gpointer user_data) {
  ...
  if (!tls->really_suspended && tls->suspended) {
    tls->suspended = FALSE;
    /*
     * The thread might still be running if it was executing native code, so the
     * state won't be invalided by suspend_current ().
     */
...
static SuspendThreadResult debugger_interrupt_critical(MonoThreadInfo *info,
                                                       gpointer user_data) {
  InterruptData *data = (InterruptData *)user_data;
  MonoJitInfo *ji;

  data->valid_info = TRUE;
  MonoDomain *domain = (MonoDomain *)mono_thread_info_get_suspend_state(info)
                           ->unwind_data[MONO_UNWIND_DATA_DOMAIN];
  if (!domain) {
    /* not attached */
    ji = NULL;
  } else {
 ...
 */
static void notify_thread(gpointer key, gpointer value, gpointer user_data) {
  ...
  mono_thread_info_safe_suspend_and_run((MonoNativeThreadId)(gsize)thread->tid,
                                        FALSE, debugger_interrupt_critical,
                                        &interrupt_data);
...
static void process_suspend(DebuggerTlsData *tls, MonoContext *ctx) {
  ...
  if (mono_loader_lock_is_owned_by_self()) {
     ...
     * when executing long running code inside the loader lock, i.e. assembly
     * load hooks.
     */
    return;
  }
  ...
  if (method->klass == mono_defaults.string_class &&
      (!strcmp(method->name, "memset") || strstr(method->name, "memcpy")))
    return;
...
static gboolean try_process_suspend(void *the_tls, MonoContext *ctx,
                                    gboolean from_breakpoint) {
  ...
  if (suspend_count > 0) {
    ...
    /* if there is a suspend pending but this thread is already resumed, we
     * shouldn't suspend it again and the breakpoint/ss can run */
    if (suspend_count - tls->resume_count == 0)
      return FALSE;
    /* if there is in a invoke the breakpoint/step should be executed even with
     * the suspend pending */
    if (tls->invoke)
      return FALSE;
    /* with the multithreaded single step check if there is a suspend_count
     * pending in the current thread and not in the vm */
    if (from_breakpoint && tls->suspend_count <= tls->resume_count_internal)
      return FALSE;
    process_suspend(tls, ctx);
    return TRUE;
  } /* if there isn't any suspend pending, the breakpoint/ss will be executed
       and will suspend then vm when the event is sent */
  return FALSE;
}
...
static void resume_vm(void) {
  ...
  if (suspend_count == 0) {
    ...
    mono_g_hash_table_foreach(thread_to_tls, reset_native_thread_suspend_state,
                              NULL);
  }

  /* Signal this even when suspend_count > 0, since some threads might have
   * resume_count > 0 */
 ...
 */
static void resume_thread(MonoInternalThread *thread) {
  DebuggerTlsData *tls;
  ...
  /*
   * Signal suspend_count without decreasing suspend_count, the threads will
   * wake up but only the one whose resume_count field is > 0 will be resumed.
   */
  mono_coop_cond_broadcast(&suspend_cond);
...
static void free_frames(StackFrame **frames, int nframes) {
  int i;
 ...
 * suspend_current:
 *
 *   Suspend the current thread until the runtime is resumed. If the thread has
 * a pending invoke, then the invoke is executed before this function returns.
 */
static void suspend_current(void) {
  DebuggerTlsData *tls;
...
static void count_thread(gpointer key, gpointer value, gpointer user_data) {
  DebuggerTlsData *tls = (DebuggerTlsData *)value;

  if (!tls->suspended && !tls->terminated &&
      !mono_thread_internal_is_current(tls->thread))
    *(int *)user_data = *(int *)user_data + 1;
}

static int count_threads_to_wait_for(void) {
  int count = 0;
 ...
 */
static void wait_for_suspend(void) {
 ...
 */
static gboolean is_suspended(void) { return count_threads_to_wait_for() == 0; }

static void no_seq_points_found(MonoMethod *method, int offset) {
  /*
   * This can happen in full-aot mode with assemblies AOTed without the
   * 'soft-debug' option to save space.
   */
  printf("Unable to find seq points for method '%s', offset 0x%x.\n",
         mono_method_full_name(method, TRUE), offset);
}

static int calc_il_offset(MonoDomain *domain, MonoMethod *method,
                          int native_offset, gboolean is_top_frame) {
  int ret = -1;
  if (is_top_frame) {
    SeqPoint sp;
    /* mono_debug_il_offset_from_address () doesn't seem to be precise enough
     * (#2092) */
    if (mono_find_prev_seq_point_for_native_offset(domain, method,
                                                   native_offset, NULL, &sp))
      ret = sp.il_offset;
  }
  if (ret == -1)
...
static gboolean process_frame(StackFrameInfo *info, MonoContext *ctx,
                              gpointer user_data) {
  ...
  if (info->type != FRAME_TYPE_MANAGED && info->type != FRAME_TYPE_INTERP &&
      info->type != FRAME_TYPE_MANAGED_TO_NATIVE) {
    if (info->type == FRAME_TYPE_DEBUGGER_INVOKE) {
      /* Mark the last frame as an invoke frame */
      if (ud->frames)
        ((StackFrame *)g_slist_last(ud->frames)->data)->flags |=
  ...
  if (info->il_offset == -1) {
    info->il_offset = calc_il_offset(info->domain, method, info->native_offset,
                                     ud->frames == NULL);
  }

  DEBUG_PRINTF(1, "\tFrame: %s:[il=0x%x, native=0x%x] %d\n",
               mono_method_full_name(method, TRUE), info->il_offset,
               info->native_offset, info->managed);
  ...
  if (info->reg_locations)
    memcpy(frame->reg_locations, info->reg_locations,
           MONO_MAX_IREGS * sizeof(host_mgreg_t *));
  if (ctx) {
    frame->ctx = *ctx;
    frame->has_ctx = TRUE;
  }
...
static gint32 isFixedSizeArray(MonoClassField *f) {
  ...
  if (cinfo) {
    for (aindex = 0; aindex < cinfo->num_attrs; ++aindex) {
      ...
      if (fixed_size_class != NULL &&
          mono_class_has_parent(ctor_class, fixed_size_class)) {
        ...
        mono_reflection_create_custom_attr_data_args_noalloc(
            mono_defaults.corlib, attr->ctor, attr->data, attr->data_size,
...
static gboolean process_filter_frame(StackFrameInfo *info, MonoContext *ctx,
                                     gpointer user_data) {
  ComputeFramesUserData *ud = (ComputeFramesUserData *)user_data;
 ...
 */
static StackFrame **compute_frame_info_from(MonoInternalThread *thread,
                                            DebuggerTlsData *tls,
                                            MonoThreadUnwindState *state,
                                            int *out_nframes) {
  ComputeFramesUserData user_data;
  MonoUnwindOptions opts =
      (MonoUnwindOptions)(MONO_UNWIND_DEFAULT | MONO_UNWIND_REG_LOCATIONS);
  StackFrame **res;
  int i, nframes;
  GSList *l;
...
static void compute_frame_info(MonoInternalThread *thread, DebuggerTlsData *tls,
  ...
  if (CHECK_PROTOCOL_VERSION(2, 52)) {
    if (tls->restore_state.valid &&
        MONO_CONTEXT_GET_IP(&tls->context.ctx) !=
            MONO_CONTEXT_GET_IP(&tls->restore_state.ctx)) {
      new_frames = compute_frame_info_from(thread, tls, &tls->restore_state,
                                           &new_frame_count);
      invalidate_frames(tls);
  ...
  if (!tls->really_suspended && tls->async_state.valid) {
    ...
    mono_walk_stack_with_state(process_frame, &tls->async_state, opts,
                               &user_data);
  } else if (tls->filter_state.valid) {
    /*
     * We are inside an exception filter.
     *
     * First we add all the frames from inside the filter; 'tls->ctx' has the
     * current context.
     */
    if (tls->context.valid) {
      mono_walk_stack_with_state(process_filter_frame, &tls->context, opts,
                                 &user_data);
      DEBUG_PRINTF(1, "\tFrame: <call filter>\n");
    }
    /*
     * After that, we resume unwinding from the location where the exception has
     * been thrown.
     */
    mono_walk_stack_with_state(process_frame, &tls->filter_state, opts,
                               &user_data);
  ...
  for (tmp = user_data.frames; tmp; tmp = tmp->next) {
     ...
     * invalidate the still valid stack frames.
     */
  ...
  if (CHECK_PROTOCOL_VERSION(2, 52)) {
    ...
    mini_get_interp_callbacks()->get_resume_state(
        jit_data, &has_interp_resume_state, &interp_resume_frame,
        &interp_resume_ip);
    if (has_interp_resume_state && tls->frame_count > 0) {
      ...
      if (interp_resume_frame == top_frame->interp_frame) {
        int native_offset = (int)((uintptr_t)interp_resume_ip -
                                  (uintptr_t)top_frame->de.ji->code_start);
        top_frame->il_offset = calc_il_offset(
            top_frame->de.domain, top_frame->de.method, native_offset, TRUE);
      }
    }
  }
}
...
static void emit_appdomain_load(gpointer key, gpointer value,
  ...
  g_hash_table_foreach(
      get_agent_domain_info((MonoDomain *)value)->loaded_classes,
      emit_type_load, NULL);
}
 ...
 * @param user_data Don't care
 */
static void emit_thread_start(gpointer key, gpointer value,
                              gpointer user_data) {
  g_assert(!mono_native_thread_id_equals(
      MONO_UINT_TO_NATIVE_THREAD_ID(GPOINTER_TO_UINT(key)),
      debugger_thread_id));
 ...
 * @param user_data Don't care
 */
static void emit_assembly_load(gpointer value, gpointer user_data) {
 ...
 * @param user_data Don't care
 */
static void emit_type_load(gpointer key, gpointer value, gpointer user_data) {
...
static char *strdup_tolower(char *s) {
  char *s2, *p;
 ...
 */
static char *dbg_path_get_basename(const char *filename) {
  char *r;
...
static void init_jit_info_dbg_attrs(MonoJitInfo *ji) {
  ERROR_DECL(error);
  ...
  hidden_klass = mono_class_load_from_name(
      mono_defaults.corlib, "System.Diagnostics", "DebuggerHiddenAttribute");
  MONO_STATIC_POINTER_INIT_END(MonoClass, hidden_klass)

  MONO_STATIC_POINTER_INIT(MonoClass, step_through_klass)
  step_through_klass =
      mono_class_load_from_name(mono_defaults.corlib, "System.Diagnostics",
  ...
  ainfo =
      mono_custom_attrs_from_class_checked(jinfo_get_method(ji)->klass, error);
  mono_error_cleanup(error); /* FIXME don't swallow the error? */
  if (ainfo) {
    if (mono_custom_attrs_has_attr(ainfo, step_through_klass))
      ji->dbg_step_through = TRUE;
 ...
 * create_event_list:
 *
 *   Return a list of event request ids matching EVENT, starting from REQS,
 * which can be NULL to include all event requests. Set SUSPEND_POLICY to the
 * suspend policy. We return request ids, instead of requests, to simplify
 * threading, since requests could be deleted anytime when the loader lock is
 * not held. LOCKING: Assumes the loader lock is held.
 */
static GSList *create_event_list(EventKind event, GPtrArray *reqs,
                                 MonoJitInfo *ji, EventInfo *ei,
                                 int *suspend_policy) {
  int i, j;
  GSList *events = NULL;
      ...
      for (j = 0; j < req->nmodifiers; ++j) {
        ...
        if (mod->kind == MOD_KIND_COUNT) {
          ...
          if (mod->data.thread != mono_thread_internal_current())
            filtered = TRUE;
        } else if (mod->kind == MOD_KIND_EXCEPTION_ONLY &&
                   !mod->not_filtered_feature && ei) {
          if (mod->data.exc_class && mod->subclasses &&
              !mono_class_is_assignable_from_internal(mod->data.exc_class,
                                                      ei->exc->vtable->klass))
            filtered = TRUE;
          if (mod->data.exc_class && !mod->subclasses &&
              mod->data.exc_class != ei->exc->vtable->klass)
            filtered = TRUE;
          ...
          if ((mod->data.exc_class && mod->subclasses &&
               mono_class_is_assignable_from_internal(
                   mod->data.exc_class, ei->exc->vtable->klass)) ||
              (mod->data.exc_class && !mod->subclasses &&
               mod->data.exc_class != ei->exc->vtable->klass)) {
          ...
          if (assemblies) {
            for (k = 0; assemblies[k]; ++k)
              if (assemblies[k] ==
                  m_class_get_image(jinfo_get_method(ji)->klass)->assembly)
                found = TRUE;
          }
          if (!found)
            filtered = TRUE;
          ...
          while ((method = mono_class_get_methods(ei->klass, &iter))) {
            ...
            if (minfo) {
              mono_debug_get_seq_points(minfo, NULL, &source_file_list, NULL,
                                        NULL, NULL);
              for (i = 0; i < source_file_list->len; ++i) {
                sinfo = (MonoDebugSourceInfo *)g_ptr_array_index(
                    source_file_list, i);
                /*
          ...
          if ((mod->data.filter & STEP_FILTER_STATIC_CTOR) && ji &&
              ...
              (jinfo_get_method(ji) !=
               ((SingleStepReq *)req->info)->start_method))
            filtered = TRUE;
...
static G_GNUC_UNUSED const char *event_to_string(EventKind event) {
  switch (event) {
 ...
 */
static void process_event(EventKind event, gpointer arg, gint32 il_offset,
                          MonoContext *ctx, GSList *events,
                          int suspend_policy) {
  Buffer buf;
  GSList *l;
  ...
  if (event == EVENT_KIND_KEEPALIVE)
    ...
    if (agent_config.defer) {
      ...
      if (is_debugger_thread() && event != EVENT_KIND_VM_DEATH)
        // FIXME: Send these with a NULL thread, don't suspend the current
        // thread
        return;
    }
  }
  ...
  if (event == EVENT_KIND_VM_START)
    suspend_policy =
        agent_config.suspend ? SUSPEND_POLICY_ALL : SUSPEND_POLICY_NONE;
  ...
  for (l = events; l; l = l->next) {
    ...
    if (event == EVENT_KIND_VM_DEATH) {
      ...
      if (!thread)
        thread = is_debugger_thread() ? mono_thread_get_main()
                                      : mono_thread_current();
    ...
    case EVENT_KIND_EXCEPTION: {
      ...
      /*
       * We are not yet suspending, so get_objref () will not keep this object
       * alive. So we need to do it later after the suspension. (#12494).
       */
  ...
  if (suspend_policy != SUSPEND_POLICY_NONE) {
     ...
     */
    save_thread_context(ctx);
    DebuggerTlsData *tls = (DebuggerTlsData *)mono_g_hash_table_lookup(
        thread_to_tls, mono_thread_internal_current());
    tls->suspend_count++;
    suspend_vm();
  ...
  if (send_success) {
    DebuggerTlsData *tls =
        (DebuggerTlsData *)mono_native_tls_get_value(debugger_tls_id);
    mono_debugger_log_event(tls, event_to_string(event), buf.buf,
                            buffer_len(&buf));
  }
  ...
  DEBUG_PRINTF(1, "[%p] Sent %d events %s(%d), suspend=%d.\n",
               (gpointer)(gsize)mono_native_thread_id_get(), nevents,
               event_to_string(event), ecount, suspend_policy);
...
static void thread_startup(MonoProfiler *prof, uintptr_t tid) {
  ...
  g_assert(
      mono_native_thread_id_equals(MONO_UINT_TO_NATIVE_THREAD_ID(tid),
                                   MONO_UINT_TO_NATIVE_THREAD_ID(thread->tid)));

  mono_loader_lock();
  old_thread = (MonoInternalThread *)mono_g_hash_table_lookup(
      tid_to_thread, GUINT_TO_POINTER(tid));
  mono_loader_unlock();
  if (old_thread) {
    if (thread == old_thread) {
       ...
       */
      DEBUG_PRINTF(
          1, "[%p] thread_start () called multiple times for %p, ignored.\n",
  ...
  MONO_GC_REGISTER_ROOT_SINGLE(tls->thread, MONO_ROOT_SOURCE_DEBUGGER, NULL,
  ...
  mono_g_hash_table_insert_internal(tid_to_thread_obj, GUINT_TO_POINTER(tid),
                                    mono_thread_current());
  mono_loader_unlock();
...
static void thread_end(MonoProfiler *prof, uintptr_t tid) {
  ...
  mono_loader_lock();
  thread = (MonoInternalThread *)mono_g_hash_table_lookup(
      tid_to_thread, GUINT_TO_POINTER(tid));
  if (thread) {
    mono_g_hash_table_remove(tid_to_thread_obj, GUINT_TO_POINTER(tid));
    tls = (DebuggerTlsData *)mono_g_hash_table_lookup(thread_to_tls, thread);
    if (tls) {
      /* FIXME: Maybe we need to free this instead, but some code can't handle
       * that */
      tls->terminated = TRUE;
      /* Can't remove from tid_to_thread, as that would defeat the check in
       * thread_start () */
  ...
  /* We might be called for threads started before we registered the start
   * callback */
  if (thread) {
    ...
    if (mono_thread_internal_is_current(thread) &&
        (!mono_native_tls_get_value(debugger_tls_id) || !mono_domain_get())) {
      /*
...
static void appdomain_start_unload(MonoProfiler *prof, MonoDomain *domain) {
  ...
  /* This might be called during shutdown on the debugger thread from the
   * CMD_VM_EXIT code */
  if (is_debugger_thread())
    return;
 ...
 * user_data is unused.
 */
static void send_types_for_domain(MonoDomain *domain, void *user_data) {
...
static void send_assemblies_for_domain(MonoDomain *domain, void *user_data) {
  GSList *tmp;
  MonoDomain *old_domain;
...
static void jit_end(MonoProfiler *prof, MonoMethod *method,
                    MonoJitInfo *jinfo) {
  /*
 ...
 */
static void ss_calculate_framecount(void *the_tls, MonoContext *ctx,
                                    gboolean force_use_ctx,
                                    DbgEngineStackFrame ***frames,
                                    int *nframes) {
  DebuggerTlsData *tls = (DebuggerTlsData *)the_tls;
...
static gboolean ensure_jit(DbgEngineStackFrame *the_frame) {
  ...
  if (!frame->jit) {
    ...
    if (!frame->jit && frame->api_method->is_inflated)
      frame->jit = mono_debug_find_method(
          mono_method_get_declaring_generic_method(frame->api_method),
          frame->de.domain);
    if (!frame->jit) {
      char *s;
...
static gboolean breakpoint_matches_assembly(MonoBreakpoint *bp,
                                            MonoAssembly *assembly) {
  return bp->method &&
         m_class_get_image(bp->method->klass)->assembly == assembly;
}
...
static gpointer get_this_addr(DbgEngineStackFrame *the_frame) {
  ...
  if ((var->index & MONO_DEBUG_VAR_ADDRESS_MODE_FLAGS) !=
      MONO_DEBUG_VAR_ADDRESS_MODE_REGOFFSET)
    return NULL;

  guint8 *addr = (guint8 *)mono_arch_context_get_int_reg(
      &frame->ctx, var->index & ~MONO_DEBUG_VAR_ADDRESS_MODE_FLAGS);
...
static MonoMethod *get_set_notification_method(MonoClass *async_builder_class) {
  ERROR_DECL(error);
  GPtrArray *array = mono_class_get_methods_by_name(
      async_builder_class, "SetNotificationForWaitCompletion", 0x24, 1, FALSE,
      error);
  mono_error_assert_ok(error);
  if (array->len == 0) {
  ...
  MonoMethod *set_notification_method =
      (MonoMethod *)g_ptr_array_index(array, 0);
  g_ptr_array_free(array, TRUE);
...
get_object_id_for_debugger_method(MonoClass *async_builder_class) {
  ERROR_DECL(error);
  GPtrArray *array = mono_class_get_methods_by_name(
      async_builder_class, "get_ObjectIdForDebugger", 0x24, 1, FALSE, error);
  mono_error_assert_ok(error);
...
static MonoClass *get_class_to_get_builder_field(DbgEngineStackFrame *frame) {
  ...
  MonoClass *ret;
  if (!m_class_is_valuetype(original_class) &&
      mono_class_is_open_constructed_type(
          m_class_get_byval_arg(original_class))) {
    ...
    g_assert(this_obj);
    context =
        mono_get_generic_context_from_stack_frame(frame->ji, this_obj->vtable);
    inflated_type = mono_class_inflate_generic_type_checked(
        m_class_get_byval_arg(original_class), &context, error);
...
/* Return the address of the AsyncMethodBuilder struct belonging to the state
 * machine method pointed to by FRAME */
static gpointer get_async_method_builder(DbgEngineStackFrame *frame) {
  MonoObject *this_obj;
  ...
  builder_field =
      mono_class_get_field_from_name_full(klass, "<>t__builder", NULL);
...
// This ID is used to figure out if breakpoint hit on resumeOffset belongs to us
// or not since thread probably changed...
static int get_this_async_id(DbgEngineStackFrame *frame) {
  MonoClassField *builder_field;
  gpointer builder;
  MonoMethod *method;
  ...
  builder_field = mono_class_get_field_from_name_full(
      get_class_to_get_builder_field(frame), "<>t__builder", NULL);
  g_assert(builder_field);
  ...
  method = get_object_id_for_debugger_method(
      mono_class_from_mono_type_internal(builder_field->type));
...
static gboolean
set_set_notification_for_wait_completion_flag(DbgEngineStackFrame *frame) {
  MonoClassField *builder_field = mono_class_get_field_from_name_full(
      get_class_to_get_builder_field(frame), "<>t__builder", NULL);
  g_assert(builder_field);
  gpointer builder = get_async_method_builder(frame);
  g_assert(builder);

  MonoMethod *method = get_set_notification_method(
      mono_class_from_mono_type_internal(builder_field->type));
  if (method == NULL)
    return FALSE;
  gboolean arg = TRUE;
...
static MonoMethod *get_notify_debugger_of_wait_completion_method(void) {
  ...
  GPtrArray *array = mono_class_get_methods_by_name(
      task_class, "NotifyDebuggerOfWaitCompletion", 0x24, 1, FALSE, error);
  mono_error_assert_ok(error);
  g_assert(array->len == 1);
...
static gboolean begin_breakpoint_processing(void *the_tls, MonoContext *ctx,
                                            MonoJitInfo *ji,
                                            gboolean from_signal) {
  DebuggerTlsData *tls = (DebuggerTlsData *)the_tls;
...
static void *create_breakpoint_events(GPtrArray *ss_reqs, GPtrArray *bp_reqs,
  ...
  if (ss_reqs && ss_reqs->len > 0)
    evts->ss_events =
        create_event_list(EVENT_KIND_STEP, ss_reqs, ji, NULL, &suspend_policy);
  else if (bp_reqs && bp_reqs->len > 0)
    evts->bp_events = create_event_list(EVENT_KIND_BREAKPOINT, bp_reqs, ji,
                                        NULL, &suspend_policy);
  else if (kind != EVENT_KIND_BREAKPOINT)
    evts->enter_leave_events =
        create_event_list(kind, NULL, ji, NULL, &suspend_policy);
...
static void process_breakpoint_events(void *_evts, MonoMethod *method,
  ...
  /*
   * FIXME: The first event will suspend, so the second will only be sent after
   * the resume.
   */
  if (evts->ss_events)
    process_event(EVENT_KIND_STEP, method, il_offset, ctx, evts->ss_events,
                  evts->suspend_policy);
  if (evts->bp_events)
    process_event(evts->kind, method, il_offset, ctx, evts->bp_events,
                  evts->suspend_policy);
  if (evts->enter_leave_events)
    process_event(evts->kind, method, il_offset, ctx, evts->enter_leave_events,
                  evts->suspend_policy);
 ...
 */
static void process_signal_event(void (*func)(void *, gboolean)) {
  DebuggerTlsData *tls;
  MonoThreadUnwindState orig_restore_state;
  MonoContext ctx;

  tls = (DebuggerTlsData *)mono_native_tls_get_value(debugger_tls_id);
  /* Have to save/restore the restore_ctx as we can be called recursively during
   * invokes etc. */
  memcpy(&orig_restore_state, &tls->restore_state,
  ...
  memcpy(&tls->restore_state, &orig_restore_state,
         sizeof(MonoThreadUnwindState));
  mono_restore_context(&ctx);
  g_assert_not_reached();
}

static void process_breakpoint_from_signal(void) {
  process_signal_event(mono_de_process_breakpoint);
}

static void resume_from_signal_handler(void *sigctx, void *func) {
  DebuggerTlsData *tls;
  MonoContext ctx;
  ...
  // clobbered by a single step/breakpoint event. If this turns out to be a
  // problem, clob:c could be added to op_seq_point.
...
static void debugger_agent_breakpoint_hit(void *sigctx) {
  /*
   * We are called from a signal handler, and running code there causes all
   * kinds of problems, like the original signal is disabled, libgc can't handle
   * altstack, etc. So set up the signal context to return to the real
...
static gboolean user_break_cb(StackFrameInfo *frame, MonoContext *ctx,
                              gpointer user_data) {
 ...
 */
static void debugger_agent_user_break(void) {
  if (agent_config.enabled) {
    MonoContext ctx;
    int suspend_policy;
    GSList *events;
    ...
    mono_loader_lock();
    events = create_event_list(EVENT_KIND_USER_BREAK, NULL, NULL, NULL,
                               &suspend_policy);
    mono_loader_unlock();
...
static void process_single_step(void) {
 ...
 */
static void debugger_agent_single_step_event(void *sigctx) {
  /* Resume to process_single_step through the signal context */

  // FIXME: Since step out/over is implemented using step in, the step in case
  // should be as fast as possible. Move the relevant code from
  // mono_de_process_single_step () here
...
static void debugger_agent_single_step_from_context(MonoContext *ctx) {
  DebuggerTlsData *tls;
  MonoThreadUnwindState orig_restore_state;
...
static void debugger_agent_breakpoint_from_context(MonoContext *ctx) {
  DebuggerTlsData *tls;
  MonoThreadUnwindState orig_restore_state;
  guint8 *orig_ip;
  ...
  // if a thread was suspended and doesn't have any managed stack, it was
  // considered as terminated, but it wasn't really terminated because it can
  // execute managed code again, and stop in a breakpoint so here we set
  // terminated as FALSE
  tls->terminated = FALSE;
...
static int ss_create_init_args(SingleStepReq *ss_req, SingleStepArgs *args) {
  ...
  mono_loader_lock();
  DebuggerTlsData *tls = (DebuggerTlsData *)mono_g_hash_table_lookup(
      thread_to_tls, ss_req->thread);
  mono_loader_unlock();
  g_assert(tls);
  if (!tls->context.valid) {
    DEBUG_PRINTF(
        1,
        "Received a single step request on a thread with no managed frames.\n");
  ...
  if (tls->restore_state.valid &&
      MONO_CONTEXT_GET_IP(&tls->context.ctx) !=
          MONO_CONTEXT_GET_IP(&tls->restore_state.ctx)) {
    /*
     * Need to start single stepping from restore_state and not from the current
     * state
     */
    set_ip = TRUE;
    frames = compute_frame_info_from(ss_req->thread, tls, &tls->restore_state,
                                     &nframes);
  }
  ...
  if (tls->has_catch_frame) {
    ...
    /*
     * Find the seq point corresponding to the landing site ip, which is the
     * first seq point after ip.
     */
    found_sp = mono_find_next_seq_point_for_native_offset(
        frame.domain, frame.method, frame.native_offset, &info, &args->sp);
    if (!found_sp)
  ...
  if (!step_to_catch) {
      ...
      if (frame) {
        ...
        if (minfo && frame->il_offset != -1) {
          MonoDebugSourceLocation *loc =
              mono_debug_method_lookup_location(minfo, frame->il_offset);
    ...
    if (frame) {
      if (!method && frame->il_offset != -1) {
        ...
        found_sp = mono_find_prev_seq_point_for_native_offset(
            frame->de.domain, frame->de.method, frame->de.native_offset, &info,
            &args->sp);
        if (!found_sp)
...
static void ss_clear_for_assembly(SingleStepReq *req, MonoAssembly *assembly) {
  GSList *l;
  gboolean found = TRUE;
 ...
 */
static void mono_debugger_agent_send_crash(char *json_dump,
                                           MonoStackHash *hashes, int pause) {
  ...
  mono_loader_lock();
  events =
      create_event_list(EVENT_KIND_CRASH, &array, NULL, NULL, &suspend_policy);
  mono_loader_unlock();
 ...
 */
static void debugger_agent_debug_log(int level, MonoString *category,
                                     MonoString *message) {
  ERROR_DECL(error);
  int suspend_policy;
  GSList *events;
  ...
  mono_loader_lock();
  events =
      create_event_list(EVENT_KIND_USER_LOG, NULL, NULL, NULL, &suspend_policy);
  mono_loader_unlock();
...
static gboolean debugger_agent_debug_log_is_enabled(void) {
  /* Treat this as true even if there is no event request for
   * EVENT_KIND_USER_LOG */
  return agent_config.enabled;
}

static void debugger_agent_unhandled_exception(MonoException *exc) {
  int suspend_policy;
  GSList *events;
  EventInfo ei;
  ...
  mono_loader_lock();
  events =
      create_event_list(EVENT_KIND_EXCEPTION, NULL, NULL, &ei, &suspend_policy);
  mono_loader_unlock();

  process_event(EVENT_KIND_EXCEPTION, &ei, 0, NULL, events, suspend_policy);
}

static void debugger_agent_handle_exception(MonoException *exc,
                                            MonoContext *throw_ctx,
                                            MonoContext *catch_ctx,
                                            StackFrameInfo *catch_frame) {
  if (catch_ctx == NULL && catch_frame == NULL &&
      mini_debug_options.suspend_on_unhandled &&
  ...
  if (!catch_ctx) {
    if (agent_config.onuncaught && !inited) {
      ...
      ei.exc = (MonoObject *)exc;
      process_event(EVENT_KIND_EXCEPTION, &ei, 0, throw_ctx, events,
                    SUSPEND_POLICY_ALL);
      return;
    }
    ...
    for (l = agent_config.onthrow; l; l = l->next) {
      ...
      char *f =
          mono_type_full_name(m_class_get_byval_arg(exc->object.vtable->klass));
  ...
  ji = mini_jit_info_table_find(mono_domain_get(),
                                (char *)MONO_CONTEXT_GET_IP(throw_ctx), NULL);
  if (catch_frame)
    ...
    for (j = 0; j < req->nmodifiers; ++j) {
      ...
      if (mod->kind == MOD_KIND_ASSEMBLY_ONLY && catch_ji) {
        ...
        if (assemblies) {
          for (k = 0; assemblies[k]; ++k)
            if (assemblies[k] ==
                m_class_get_image(jinfo_get_method(catch_ji)->klass)->assembly)
              found = TRUE;
        }
        if (!found)
          ei.caught = FALSE;
  ...
  events =
      create_event_list(EVENT_KIND_EXCEPTION, NULL, ji, &ei, &suspend_policy);
  mono_loader_unlock();
...
static void debugger_agent_begin_exception_filter(MonoException *exc,
                                                  MonoContext *ctx,
                                                  MonoContext *orig_ctx) {
  DebuggerTlsData *tls;
  ...
  /*
   * We're about to invoke an exception filter during the first pass of
   * exception handling.
   *
   * 'ctx' is the context that'll get passed to the filter ('call_filter (ctx,
   * ei->data.filter)'), 'orig_ctx' is the context where the exception has been
   * thrown.
   *
   *
   ...
   *
   * Since exception filters are invoked during the first pass of exception
   * handling, the stack frames of the throw site are still intact, so we should
   * include them in a stack trace.
   *
   * We do this here by saving the context of the throw site in
   * 'tls->filter_state'.
   *
   * Exception filters are used by MonoDroid, where we want to stop inside a
   * call filter, but report the location of the 'throw' to the user.
   *
   */
...
static void debugger_agent_end_exception_filter(MonoException *exc,
                                                MonoContext *ctx,
                                                MonoContext *orig_ctx) {
  DebuggerTlsData *tls;
...
static void buffer_add_fixed_array(Buffer *buf, MonoType *t, void *addr,
                                   MonoDomain *domain, gboolean as_vtype,
                                   GHashTable *parent_vtypes,
 ...
 * vtypes.
 */
static void buffer_add_value_full(Buffer *buf, MonoType *t, void *addr,
                                  MonoDomain *domain, gboolean as_vtype,
                                  GHashTable *parent_vtypes,
                                  gint32 len_fixed_array) {
  MonoObject *obj;
  ...
  if (len_fixed_array > 1 && t->type != MONO_TYPE_VALUETYPE &&
    ...
    buffer_add_fixed_array(buf, t, addr, domain, as_vtype, parent_vtypes,
                           len_fixed_array);
    return;
  }
  switch (t->type) {
  ...
  case MONO_TYPE_ARRAY:
    ...
    if (!obj) {
      ...
      } else if (m_class_get_byval_arg(obj->vtable->klass)->type ==
  ...
  case MONO_TYPE_TYPEDBYREF: {
    ...
    if (boxed_vtype) {
      ...
      if (vtype_index) {
        ...
        g_hash_table_insert(
            parent_vtypes, addr,
            GINT_TO_POINTER(g_hash_table_size(parent_vtypes) + 1));
      }
    }
    ...
    while ((f = mono_class_get_fields_internal(klass, &iter))) {
      ...
      if (mono_field_is_deleted(f))
        continue;
      buffer_add_value_full(buf, f->type, mono_vtype_get_field_addr(addr, f),
                            domain, FALSE, parent_vtypes,
                            len_fixed_array != 1 ? len_fixed_array
                                                 : isFixedSizeArray(f));
    }
...
static void buffer_add_value(Buffer *buf, MonoType *t, void *addr,
                             MonoDomain *domain) {
  buffer_add_value_full(buf, t, addr, domain, FALSE, NULL, 1);
}

static gboolean obj_is_of_type(MonoObject *obj, MonoType *t) {
  MonoClass *klass = obj->vtable->klass;
  if (!mono_class_is_assignable_from_internal(
          mono_class_from_mono_type_internal(t), klass)) {
...
static ErrorCode decode_value(MonoType *t, MonoDomain *domain,
                              gpointer void_addr, gpointer void_buf,
                              guint8 **endbuf, guint8 *limit,
                              gboolean check_field_datatype);

static ErrorCode decode_vtype(MonoType *t, MonoDomain *domain,
                              gpointer void_addr, gpointer void_buf,
                              guint8 **endbuf, guint8 *limit,
                              gboolean check_field_datatype) {
  ...
  while ((f = mono_class_get_fields_internal(klass, &iter))) {
    ...
    if (mono_field_is_deleted(f))
      continue;
    err = decode_value(f->type, domain, mono_vtype_get_field_addr(addr, f), buf,
                       &buf, limit, check_field_datatype);
    if (err != ERR_NONE)
      return err;
    nfields--;
  }
...
static ErrorCode
decode_fixed_size_array_internal(MonoType *t, int type, MonoDomain *domain,
                                 guint8 *addr, guint8 *buf, guint8 **endbuf,
                                 guint8 *limit, gboolean check_field_datatype) {
...
static ErrorCode decode_value_internal(MonoType *t, int type,
                                       MonoDomain *domain, guint8 *addr,
  ...
  if (type == VALUE_TYPE_ID_FIXED_ARRAY && t->type != MONO_TYPE_VALUETYPE) {
    decode_fixed_size_array_internal(t, type, domain, addr, buf, endbuf, limit,
    ...
    if (type == MONO_TYPE_OBJECT) {
      ...
      if (obj->vtable->klass != mono_class_from_mono_type_internal(t)) {
        DEBUG_PRINTF(1, "Expected type '%s', got object '%s'\n",
                     mono_type_full_name(t),
                     m_class_get_name(obj->vtable->klass));
        return ERR_INVALID_ARGUMENT;
      }
      memcpy(addr, mono_object_unbox_internal(obj),
             mono_class_value_size(obj->vtable->klass, NULL));
    } else {
      err =
          decode_vtype(t, domain, addr, buf, &buf, limit, check_field_datatype);
      if (err != ERR_NONE)
        return err;
    }
    break;
      ...
      if (type == MONO_TYPE_OBJECT) {
        ...
        if (obj) {
          if (!obj_is_of_type(obj, t)) {
            if (check_field_datatype) { // if it's not executing a invoke method
                                        // check the datatypes.
        ...
        *(MonoObject **)addr =
            mono_value_box_checked(d, klass, vtype_buf, error);
        mono_error_cleanup(error);
        g_free(vtype_buf);
      } else {
        char *name = mono_type_full_name(t);
...
static ErrorCode decode_value(MonoType *t, MonoDomain *domain,
  ...
  if (t->type == MONO_TYPE_GENERICINST &&
     ...
     */
    if (targ->type == type) {
      nullable_buf = (guint8 *)g_malloc(
          mono_class_instance_size(mono_class_from_mono_type_internal(targ)));
      err = decode_value_internal(targ, type, domain, nullable_buf, buf, endbuf,
                                  limit, check_field_datatype);
      if (err != ERR_NONE) {
        g_free(nullable_buf);
        return err;
      }
      MonoObject *boxed = mono_value_box_checked(
          domain, mono_class_from_mono_type_internal(targ), nullable_buf,
          error);
      if (!is_ok(error)) {
        mono_error_cleanup(error);
        return ERR_INVALID_OBJECT;
...
static void add_var(Buffer *buf, MonoDebugMethodJitInfo *jit, MonoType *t,
                    MonoDebugVarInfo *var, MonoContext *ctx, MonoDomain *domain,
                    gboolean as_vtype) {
  guint32 flags;
  int reg;
  guint8 *addr, *gaddr;
  ...
  case MONO_DEBUG_VAR_ADDRESS_MODE_GSHAREDVT_LOCAL: {
    ...
    if (flags == MONO_DEBUG_VAR_ADDRESS_MODE_REGOFFSET) {
      ...
      info = (MonoGSharedVtMethodRuntimeInfo *)mono_arch_context_get_int_reg(
          ctx, reg);
    } else {
      g_assert_not_reached();
    }
...
static void set_var(MonoType *t, MonoDebugVarInfo *var, MonoContext *ctx,
                    MonoDomain *domain, guint8 *val,
                    host_mgreg_t **reg_locations, MonoContext *restore_ctx) {
  guint32 flags;
  int reg, size;
  guint8 *addr, *gaddr;
  ...
  case MONO_DEBUG_VAR_ADDRESS_MODE_REGISTER: {
    ...
    if (!t->byref && (t->type == MONO_TYPE_I1 || t->type == MONO_TYPE_I2 ||
                      t->type == MONO_TYPE_I4 || t->type == MONO_TYPE_I8))
      is_signed = TRUE;
...
static void set_interp_var(MonoType *t, gpointer addr, guint8 *val_buf) {
  int size;
...
static void clear_event_request(int req_id, int etype) {
  int i;
...
static void clear_assembly_from_modifier(EventRequest *req, Modifier *m,
                                         MonoAssembly *assembly) {
  int i;

  if (m->kind == MOD_KIND_EXCEPTION_ONLY && m->data.exc_class &&
      m_class_get_image(m->data.exc_class)->assembly == assembly)
  ...
  if (m->kind == MOD_KIND_ASSEMBLY_ONLY && m->data.assemblies) {
    ...
    if (match_count) {
      // +1 because we don't know length and we use last element to check for
      // end
...
static void clear_assembly_from_modifiers(EventRequest *req,
                                          MonoAssembly *assembly) {
  int i;
...
static void clear_event_requests_for_assembly(MonoAssembly *assembly) {
  ...
  while (found) {
    ...
    for (i = 0; i < event_requests->len; ++i) {
      ...
      if (req->event_kind == EVENT_KIND_BREAKPOINT &&
          breakpoint_matches_assembly((MonoBreakpoint *)req->info, assembly)) {
 ...
 */
static gboolean type_comes_from_assembly(gpointer klass, gpointer also_klass,
                                         gpointer assembly) {
  return mono_type_in_image(
      m_class_get_byval_arg((MonoClass *)klass),
...
static void clear_types_for_assembly(MonoAssembly *assembly) {
  ...
  mono_loader_lock();
  g_hash_table_foreach_remove(info->loaded_classes, type_comes_from_assembly,
                              assembly);
  mono_loader_unlock();
}

static void dispose_vm(void) {
...
static void count_thread_check_gc_finalizer(gpointer key, gpointer value,
                                            gpointer user_data) {
  MonoThread *thread = (MonoThread *)value;
  gboolean *ret = (gboolean *)user_data;
  if (mono_gc_is_finalizer_internal_thread(thread->internal_thread)) {
    DebuggerTlsData *tls = (DebuggerTlsData *)mono_g_hash_table_lookup(
        thread_to_tls, thread->internal_thread);
    if (!tls->gc_finalizing) { // GC Finalizer is not running some finalizer
                               // code, so ignore it
      *ret = TRUE;
      return;
    }
...
static void add_thread(gpointer key, gpointer value, gpointer user_data) {
  MonoThread *thread = (MonoThread *)value;
  Buffer *buf = (Buffer *)user_data;
  if (mono_gc_is_finalizer_internal_thread(thread->internal_thread)) {
    DebuggerTlsData *tls = (DebuggerTlsData *)mono_g_hash_table_lookup(
        thread_to_tls, thread->internal_thread);
    if (!tls->gc_finalizing) // GC Finalizer is not running some finalizer code,
                             // so ignore it
      return;
  }
...
static ErrorCode do_invoke_method(DebuggerTlsData *tls, Buffer *buf,
                                  InvokeData *invoke, guint8 *p,
                                  guint8 **endp) {
  ERROR_DECL(error);
  ...
  if (invoke->method) {
    /*
     * Invoke this method directly, currently only Environment.Exit () is
     * supported.
     */
    this_arg = NULL;
    DEBUG_PRINTF(1, "[%p] Invoking method '%s' on receiver '%s'.\n",
                 (gpointer)(gsize)mono_native_thread_id_get(),
                 mono_method_full_name(invoke->method, TRUE),
                 this_arg ? m_class_get_name(this_arg->vtable->klass)
                          : "<null>");
  ...
  if (m->is_generic) {
    DEBUG_PRINTF(
        1, "[%p] Error: Attempting to invoke uninflated generic method %s.\n",
        (gpointer)(gsize)mono_native_thread_id_get(),
        mono_method_full_name(m, TRUE));
    return ERR_INVALID_ARGUMENT;
  } else if (m_class_is_valuetype(m->klass) &&
             (m->flags & METHOD_ATTRIBUTE_STATIC)) {
    /* Should be null */
    int type = decode_byte(p, &p, end);
    if (type != VALUE_TYPE_ID_NULL) {
      DEBUG_PRINTF(
          1, "[%p] Error: Static vtype method invoked with this argument.\n",
          (gpointer)(gsize)mono_native_thread_id_get());
      return ERR_INVALID_ARGUMENT;
    }
    ...
    if (type == VALUE_TYPE_ID_NULL) {
      ...
      err = decode_value(m_class_get_byval_arg(m->klass), domain, this_buf, p,
                         &p, end, FALSE);
      if (err != ERR_NONE)
        return err;
    }
  } else {
    err = decode_value(m_class_get_byval_arg(m->klass), domain, this_buf, p, &p,
                       end, FALSE);
    if (err != ERR_NONE)
      return err;
  }
  ...
  if (MONO_CLASS_IS_INTERFACE_INTERNAL(m->klass)) {
    if (!this_arg) {
      DEBUG_PRINTF(
          1, "[%p] Error: Interface method invoked without this argument.\n",
          (gpointer)(gsize)mono_native_thread_id_get());
      return ERR_INVALID_ARGUMENT;
    }
    ...
    if (m_class_is_valuetype(m->klass)) {
      this_buf = (guint8 *)g_alloca(mono_class_instance_size(m->klass));
      memcpy(this_buf, mono_object_unbox_internal(this_arg),
             mono_class_instance_size(m->klass));
    }
  } else if ((m->flags & METHOD_ATTRIBUTE_VIRTUAL) &&
             !m_class_is_valuetype(m->klass) &&
             invoke->flags & INVOKE_FLAG_VIRTUAL) {
    if (!this_arg) {
      DEBUG_PRINTF(1,
                   "[%p] Error: invoke with INVOKE_FLAG_VIRTUAL flag set "
  ...
  DEBUG_PRINTF(1, "[%p] Invoking method '%s' on receiver '%s'.\n",
               (gpointer)(gsize)mono_native_thread_id_get(),
               mono_method_full_name(m, TRUE),
               this_arg ? m_class_get_name(this_arg->vtable->klass) : "<null>");

  if (this_arg && this_arg->vtable->domain != domain)
    NOT_IMPLEMENTED;

  if (!m_class_is_valuetype(m->klass) &&
      !(m->flags & METHOD_ATTRIBUTE_STATIC) && !this_arg) {
  ...
  if (invoke->flags & INVOKE_FLAG_DISABLE_BREAKPOINTS)
    ...
    /*
     * Add an LMF frame to link the stack frames on the invoke method with our
     * caller.
     */
#ifdef MONO_ARCH_SOFT_DEBUG_SUPPORTED
  if (invoke->has_ctx) {
  ...
  res = mono_runtime_try_invoke(
      m,
      m_class_is_valuetype(m->klass) ? (gpointer)this_buf : (gpointer)this_arg,
      args, &exc, error);
  ...
  DEBUG_PRINTF(1, "[%p] Invoke result: %p, exc: %s, time: %ld ms.\n",
               (gpointer)(gsize)mono_native_thread_id_get(), res,
               exc ? m_class_get_name(exc->vtable->klass) : NULL,
  ...
  if (exc) {
    ...
    if ((invoke->flags & INVOKE_FLAG_RETURN_OUT_THIS) &&
        CHECK_PROTOCOL_VERSION(2, 35))
      out_this = TRUE;
    if ((invoke->flags & INVOKE_FLAG_RETURN_OUT_ARGS) &&
        CHECK_PROTOCOL_VERSION(2, 35))
      out_args = TRUE;
    buffer_add_byte(buf, 1 + (out_this ? 2 : 0) + (out_args ? 4 : 0));
    if (m->string_ctor) {
      buffer_add_value(buf, m_class_get_byval_arg(mono_get_string_class()),
                       &res, domain);
      ...
      if (!strcmp(m->name, ".ctor")) {
        if (!m_class_is_valuetype(m->klass))
          ...
          buffer_add_value(buf, m_class_get_byval_arg(m->klass), this_buf,
                           domain);
      } else {
    ...
    } else if (m_class_is_valuetype(
                   mono_class_from_mono_type_internal(sig->ret)) ||
               sig->ret->type == MONO_TYPE_PTR ||
               sig->ret->type == MONO_TYPE_FNPTR) {
      if (mono_class_is_nullable(
              mono_class_from_mono_type_internal(sig->ret))) {
        MonoClass *k = mono_class_from_mono_type_internal(sig->ret);
        guint8 *nullable_buf =
            (guint8 *)g_alloca(mono_class_value_size(k, NULL));
        ...
        if (sig->ret->byref) {
          MonoType *ret_byval = m_class_get_byval_arg(
              mono_class_from_mono_type_internal(sig->ret));
          buffer_add_value(buf, ret_byval, mono_object_unbox_internal(res),
                           domain);
        } else {
          buffer_add_value(buf, sig->ret, mono_object_unbox_internal(res),
                           domain);
        }
      }
    } else {
...
static void invoke_method(void) {
  ...
  DEBUG_PRINTF(1, "[%p] Invoke finished (%d), resume_count = %d.\n",
               (gpointer)(gsize)mono_native_thread_id_get(), err,
               tls->resume_count);
   ...
   * after the mono_runtime_invoke_checked() already returned, but it doesn't
   * matter because we reset the abort here.
   */
...
static gboolean is_really_suspended(gpointer key, gpointer value,
                                    gpointer user_data) {
  MonoThread *thread = (MonoThread *)value;
  DebuggerTlsData *tls;
  gboolean res;
...
static GPtrArray *get_source_files_for_type(MonoClass *klass) {
  gpointer iter = NULL;
  MonoMethod *method;
  MonoDebugSourceInfo *sinfo;
  GPtrArray *files;
  ...
  while ((method = mono_class_get_methods(klass, &iter))) {
    ...
    if (minfo) {
      mono_debug_get_seq_points(minfo, NULL, &source_file_list, NULL, NULL,
                                NULL);
      for (j = 0; j < source_file_list->len; ++j) {
        sinfo = (MonoDebugSourceInfo *)g_ptr_array_index(source_file_list, j);
        for (i = 0; i < files->len; ++i)
          if (!strcmp((const char *)g_ptr_array_index(files, i),
                      (const char *)sinfo->source_file))
            break;
        if (i == files->len)
...
static void get_types(gpointer key, gpointer value, gpointer user_data) {
  MonoAssembly *ass;
  ...
  for (tmp = domain->domain_assemblies; tmp; tmp = tmp->next) {
    ...
    if (ass->image) {
      ...
      t = mono_reflection_get_type_checked(alc, ass->image, ass->image,
                                           ud->info, ud->ignore_case, TRUE,
                                           &type_resolve, probe_type_error);
...
static void get_types_for_source_file(gpointer key, gpointer value,
                                      gpointer user_data) {
  GHashTableIter iter;
  ...
  while (g_hash_table_iter_next(&iter, NULL, (void **)&klass)) {
    if (!g_hash_table_lookup(info->source_files, klass)) {
      ...
      for (int i = 0; i < files->len; ++i) {
        ...
        class_list =
            (GSList *)g_hash_table_lookup(info->source_file_to_class, s2);
        if (!class_list) {
          class_list = g_slist_prepend(class_list, klass);
          g_hash_table_insert(info->source_file_to_class, g_strdup(s2),
                              class_list);
        } else {
          class_list = g_slist_prepend(class_list, klass);
        ...
        class_list = (GSList *)g_hash_table_lookup(
            info->source_file_to_class_ignorecase, s3);
        if (!class_list) {
          class_list = g_slist_prepend(class_list, klass);
          g_hash_table_insert(info->source_file_to_class_ignorecase,
                              g_strdup(s3), class_list);
        } else {
          class_list = g_slist_prepend(class_list, klass);
          g_hash_table_insert(info->source_file_to_class_ignorecase, s3,
                              class_list);
        }
  ...
  if (ud->ignore_case) {
    ...
    class_list =
        (GSList *)g_hash_table_lookup(info->source_file_to_class_ignorecase, s);
    g_free(s);
  } else {
    class_list =
        (GSList *)g_hash_table_lookup(info->source_file_to_class, ud->basename);
  }
...
static ErrorCode vm_commands(int command, int id, guint8 *p, guint8 *end,
                             Buffer *buf) {
  switch (command) {
  case CMD_VM_VERSION: {
  ...
  case CMD_VM_SET_PROTOCOL_VERSION: {
    ...
    DEBUG_PRINTF(
        1, "[dbg] Protocol version %d.%d, client protocol version %d.%d.\n",
  ...
  case CMD_VM_ALL_THREADS: {
    ...
    mono_g_hash_table_foreach(tid_to_thread_obj,
  ...
  case CMD_VM_EXIT: {
...
#ifdef TRY_MANAGED_SYSTEM_ENVIRONMENT_EXIT
    env_class = mono_class_try_load_from_name(mono_defaults.corlib, "System",
                                              "Environment");
    if (env_class) {
      ERROR_DECL(error);
      exit_method = mono_class_get_method_from_name_checked(env_class, "Exit",
                                                            1, 0, error);
      mono_error_assert_ok(error);
    }
#endif
    ...
    mono_loader_lock();
    thread = (MonoInternalThread *)mono_g_hash_table_find(
        tid_to_thread, is_really_suspended, NULL);
    mono_loader_unlock();
  ...
  case CMD_VM_INVOKE_METHODS: {
    ...
    mono_loader_lock();
    tls = (DebuggerTlsData *)mono_g_hash_table_lookup(
        thread_to_tls, THREAD_TO_INTERNAL(thread));
    mono_loader_unlock();
    g_assert(tls);
  ...
  case CMD_VM_ABORT_INVOKE: {
    ...
    /*
     * Check whether we're still inside the mono_runtime_invoke_checked() and
     * that it's actually the correct invocation.
     *
     * Careful, we do not stop the thread that's doing the invocation, so we
     * can't inspect its stack.  However, invoke_method() also acquires the
     * loader lock when it's done, so we're safe here.
     *
     */
  ...
  case CMD_VM_GET_TYPES_FOR_SOURCE_FILE: {
    ...
    for (i = 0; i < res_classes->len; ++i)
      buffer_add_typeid(buf, (MonoDomain *)g_ptr_array_index(res_domains, i),
                        (MonoClass *)g_ptr_array_index(res_classes, i));
...
static ErrorCode event_commands(int command, guint8 *p, guint8 *end,
                                Buffer *buf) {
  ErrorCode err;
  ERROR_DECL(error);
  ...
  case CMD_EVENT_REQUEST_SET: {
    ...
    req = (EventRequest *)g_malloc0(sizeof(EventRequest) +
                                    (nmodifiers * sizeof(Modifier)));
    ...
    for (i = 0; i < nmodifiers; ++i) {
      ...
      if (mod == MOD_KIND_COUNT) {
        ...
        if (!CHECK_PROTOCOL_VERSION(2, 26) &&
            (req->modifiers[i].data.filter & STEP_FILTER_DEBUGGER_HIDDEN))
          /* Treat STEP_THOUGH the same as HIDDEN */
          req->modifiers[i].data.filter =
              (StepFilter)(req->modifiers[i].data.filter |
        ...
        if (exc_class) {
          ...
          if (!mono_class_is_assignable_from_internal(
                  mono_defaults.exception_class, exc_class)) {
            g_free(req);
            return ERR_INVALID_ARGUMENT;
          }
        }
        if (CHECK_PROTOCOL_VERSION(2, 54)) {
          ...
          DEBUG_PRINTF(
              1, "[dbg] \tEXCEPTION_ONLY 2 filter (%s%s%s%s).\n",
              exc_class ? m_class_get_name(exc_class)
                        : (req->modifiers[i].everything_else ? "everything else"
                                                             : "all"),
              req->modifiers[i].caught ? ", caught" : "",
              req->modifiers[i].uncaught ? ", uncaught" : "",
              req->modifiers[i].subclasses ? ", include-subclasses" : "");
        } else {
          ...
          DEBUG_PRINTF(1, "[dbg] \tEXCEPTION_ONLY filter (%s%s%s%s).\n",
                       exc_class ? m_class_get_name(exc_class) : "all",
                       req->modifiers[i].caught ? ", caught" : "",
                       req->modifiers[i].uncaught ? ", uncaught" : "",
                       req->modifiers[i].subclasses ? ", include-subclasses"
                                                    : "");
        ...
        for (j = 0; j < n; ++j) {
          req->modifiers[i].data.assemblies[j] =
              decode_assemblyid(p, &p, end, &domain, &err);
          if (err != ERR_NONE) {
    ...
    if (req->event_kind == EVENT_KIND_BREAKPOINT) {
      ...
      mono_loader_lock();
      DebuggerTlsData *tls = (DebuggerTlsData *)mono_g_hash_table_lookup(
          thread_to_tls, THREAD_TO_INTERNAL(step_thread));
      mono_loader_unlock();
      g_assert(tls);
      ...
      err = (ErrorCode)mono_de_ss_create(THREAD_TO_INTERNAL(step_thread), size,
                                         depth, filter, req);
      if (err != ERR_NONE) {
        g_free(req);
        return err;
      ...
      req->info =
          mono_de_set_breakpoint(NULL, METHOD_ENTRY_IL_OFFSET, req, NULL);
    } else if (req->event_kind == EVENT_KIND_METHOD_EXIT) {
      req->info =
          mono_de_set_breakpoint(NULL, METHOD_EXIT_IL_OFFSET, req, NULL);
...
static ErrorCode domain_commands(int command, guint8 *p, guint8 *end,
                                 Buffer *buf) {
  ErrorCode err;
  MonoDomain *domain;
  ...
  case CMD_APPDOMAIN_GET_CORLIB: {
    ...
    buffer_add_assemblyid(
        buf, domain,
        m_class_get_image(domain->domain->mbr.obj.vtable->klass)->assembly);
    break;
  }
  ...
  case CMD_APPDOMAIN_CREATE_BYTE_ARRAY: {
    ...
    size = len;
    arr = mono_array_new_full_checked(
        mono_domain_get(), mono_class_create_array(mono_get_byte_class(), 1),
        &size, NULL, error);
  ...
  case CMD_APPDOMAIN_CREATE_BOXED_VALUE: {
    ...
    err =
        decode_value(m_class_get_byval_arg(klass), domain,
                     (guint8 *)mono_object_unbox_internal(o), p, &p, end, TRUE);
    if (err != ERR_NONE)
      return err;
...
static ErrorCode get_assembly_object_command(MonoDomain *domain,
                                             MonoAssembly *ass, Buffer *buf,
                                             MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  ErrorCode err = ERR_NONE;
  error_init(error);
  MonoReflectionAssemblyHandle o =
      mono_assembly_get_object_handle(domain, ass, error);
...
static ErrorCode assembly_commands(int command, guint8 *p, guint8 *end,
                                   Buffer *buf) {
  ErrorCode err;
  MonoAssembly *ass;
  MonoDomain *domain;
  ...
  case CMD_ASSEMBLY_GET_TYPE: {
    ...
    if (!mono_reflection_parse_type_checked(s, &info, error)) {
      ...
      if (info.assembly.name) {
        ...
        char *error_msg = g_strdup_printf(
            "Unexpected assembly-qualified type %s was provided", original_s);
        add_error_string(buf, error_msg);
        g_free(error_msg);
        g_free(original_s);
        return ERR_INVALID_ARGUMENT;
      }
      t = mono_reflection_get_type_checked(alc, ass->image, ass->image, &info,
                                           ignorecase, TRUE, &type_resolve,
                                           error);
      if (!is_ok(error)) {
        mono_error_cleanup(error); /* FIXME don't swallow the error */
        mono_reflection_free_type_info(&info);
    ...
    buffer_add_typeid(buf, domain,
                      t ? mono_class_from_mono_type_internal(t) : NULL);
    mono_reflection_free_type_info(&info);
    g_free(s);
    g_free(original_s);
    mono_domain_set_fast(d, TRUE);
  ...
  case CMD_ASSEMBLY_GET_NAME: {
    ...
    name = g_strdup_printf(
        ...
        mass->aname.name, mass->aname.major, mass->aname.minor,
        mass->aname.build, mass->aname.revision,
        mass->aname.culture && *mass->aname.culture ? mass->aname.culture
                                                    : "neutral",
        mass->aname.public_key_token[0] ? (char *)mass->aname.public_key_token
                                        : "null",
        (mass->aname.flags & ASSEMBLYREF_RETARGETABLE_FLAG)
            ? ", Retargetable=Yes"
            : "");
  ...
  case CMD_ASSEMBLY_GET_PDB_BLOB: {
    ...
    if (ppdb) {
      ...
      buffer_add_byte_array(buf, (guint8 *)image->raw_data,
                            image->raw_data_len);
    } else {
  ...
  case CMD_ASSEMBLY_GET_METHOD_FROM_TOKEN: {
    ...
    MonoMethod *mono_method =
        mono_get_method_checked(ass->image, token, NULL, NULL, error);
    if (!is_ok(error)) {
      add_error_string(buf, mono_error_get_message(error));
      mono_error_cleanup(error);
      return ERR_INVALID_ARGUMENT;
  ...
  case CMD_ASSEMBLY_HAS_DEBUG_INFO: {
    buffer_add_byte(buf, !ass->dynamic &&
                             mono_debug_image_has_debug_info(ass->image));
    break;
  }
  default:
    return ERR_NOT_IMPLEMENTED;
...
static ErrorCode module_commands(int command, guint8 *p, guint8 *end,
                                 Buffer *buf) {
  ErrorCode err;
  MonoDomain *domain;
...
static ErrorCode field_commands(int command, guint8 *p, guint8 *end,
                                Buffer *buf) {
  ErrorCode err;
  MonoDomain *domain;
...
static void buffer_add_cattr_arg(Buffer *buf, MonoType *t, MonoDomain *domain,
                                 MonoObject *val) {
  if (val && val->vtable->klass == mono_defaults.runtimetype_class) {
    ...
    buffer_add_typeid(
        buf, domain,
        mono_class_from_mono_type_internal(((MonoReflectionType *)val)->type));
...
static ErrorCode buffer_add_cattrs(Buffer *buf, MonoDomain *domain,
                                   MonoImage *image, MonoClass *attr_klass,
                                   MonoCustomAttrInfo *cinfo) {
  int i, j;
  int nattrs = 0;
  ...
  for (i = 0; i < cinfo->num_attrs; ++i) {
    if (!attr_klass ||
        mono_class_has_parent(cinfo->attrs[i].ctor->klass, attr_klass))
      nattrs++;
  }
  ...
  for (i = 0; i < cinfo->num_attrs; ++i) {
    ...
    if (!attr_klass || mono_class_has_parent(attr->ctor->klass, attr_klass)) {
      ...
      mono_reflection_create_custom_attr_data_args(
          image, attr->ctor, attr->data, attr->data_size, typed_args_h,
          named_args_h, &arginfo, error);
      if (!is_ok(error)) {
        DEBUG_PRINTF(2,
                     "[dbg] mono_reflection_create_custom_attr_data_args () "
                     "failed with: '%s'\n",
                     mono_error_get_message(error));
        mono_error_cleanup(error);
        CLEAR_ICALL_FRAME;
      ...
      if (typed_args) {
        ...
        for (j = 0; j < mono_array_length_internal(typed_args); ++j) {
          MonoObject *val =
              mono_array_get_internal(typed_args, MonoObject *, j);
      ...
      if (named_args) {
        ...
        for (j = 0; j < mono_array_length_internal(named_args); ++j) {
          MonoObject *val =
              mono_array_get_internal(named_args, MonoObject *, j);
          MONO_HANDLE_ASSIGN_RAW(val_h, val);
...
static ErrorCode type_commands_internal(int command, MonoClass *klass,
                                        MonoDomain *domain, guint8 *p,
                                        guint8 *end, Buffer *buf) {
  HANDLE_FUNCTION_ENTER();
  ...
  case CMD_TYPE_GET_INFO: {
    ...
    // FIXME: byref
    name = mono_type_get_name_full(m_class_get_byval_arg(klass),
    ...
    if (type->type == MONO_TYPE_PTR)
      b |= (1 << 1);
    if (!type->byref &&
        (((type->type >= MONO_TYPE_BOOLEAN) && (type->type <= MONO_TYPE_R8)) ||
    ...
    if (CHECK_PROTOCOL_VERSION(2, 15)) {
      ...
      if (mono_class_is_ginst(klass)) {
        ...
        for (i = 0; i < count; i++)
          buffer_add_typeid(
              buf, domain,
              mono_class_from_mono_type_internal(inst->type_argv[i]));
        ...
        buffer_add_int(buf, count);
        for (i = 0; i < count; i++) {
          pklass = mono_class_create_generic_parameter(
  ...
  case CMD_TYPE_GET_FIELDS: {
    ...
    while ((f = mono_class_get_fields_internal(klass, &iter))) {
      ...
      buffer_add_string(buf, f->name);
      buffer_add_typeid(buf, domain,
                        mono_class_from_mono_type_internal(f->type));
      buffer_add_int(buf, f->type->attrs);
      i++;
    }
    g_assert(i == nfields);
  ...
  case CMD_TYPE_GET_CATTRS: {
    ...
    err = buffer_add_cattrs(buf, domain, m_class_get_image(klass), attr_klass,
                            cinfo);
    if (err != ERR_NONE)
      goto exit;
    break;
  ...
  case CMD_TYPE_GET_VALUES_2: {
    ...
    for (i = 0; i < len; ++i) {
      ...
      val = (guint8 *)g_malloc(mono_class_instance_size(
          mono_class_from_mono_type_internal(f->type)));
      mono_field_static_get_value_for_thread(
          thread ? thread : mono_thread_internal_current(), vtable, f, val,
          string_handle, error);
      goto_if_nok(error, invalid_fieldid);
  ...
  case CMD_TYPE_GET_OBJECT: {
    MonoObject *o = (MonoObject *)mono_type_get_object_checked(
  ...
  case CMD_TYPE_GET_METHODS_BY_NAME_FLAGS: {
    ...
    error_init(error);
    array = mono_class_get_methods_by_name(klass, name,
                                           flags & ~BINDING_FLAGS_IGNORE_CASE,
                                           mlisttype, TRUE, error);
    if (!is_ok(error)) {
      mono_error_cleanup(error);
      goto loader_error;
    }
  ...
  case CMD_TYPE_GET_INTERFACE_MAP: {
    ...
    for (tindex = 0; tindex < len; ++tindex) {
      ...
      ioffset = mono_class_interface_offset_with_variance(klass, iclass,
                                                          &variance_used);
      if (ioffset == -1)
...
static ErrorCode type_commands(int command, guint8 *p, guint8 *end,
                               Buffer *buf) {
  MonoClass *klass;
  MonoDomain *old_domain;
  MonoDomain *domain;
...
static ErrorCode method_commands_internal(int command, MonoMethod *method,
                                          MonoDomain *domain, guint8 *p,
                                          guint8 *end, Buffer *buf) {
  ...
  case CMD_METHOD_GET_DEBUG_INFO: {
    ...
    mono_debug_get_seq_points(minfo, &source_file, &source_file_list,
                              &source_files, &sym_seq_points, &n_il_offsets);
    buffer_add_int(buf, header->code_size);
    if (CHECK_PROTOCOL_VERSION(2, 13)) {
      buffer_add_int(buf, source_file_list->len);
      for (i = 0; i < source_file_list->len; ++i) {
        MonoDebugSourceInfo *sinfo =
            (MonoDebugSourceInfo *)g_ptr_array_index(source_file_list, i);
    ...
    for (i = 0; i < n_il_offsets; ++i) {
      ...
      if (source_files[i] != -1) {
        MonoDebugSourceInfo *sinfo = (MonoDebugSourceInfo *)g_ptr_array_index(
            source_file_list, source_files[i]);
        srcfile = sinfo->source_file;
      }
      DEBUG_PRINTF(10, "IL%x -> %s:%d %d %d %d\n", sp->il_offset, srcfile,
                   sp->line, sp->column, sp->end_line, sp->end_column);
  ...
  case CMD_METHOD_GET_LOCALS_INFO: {
    ...
    if (!locals) {
      ...
      for (i = 0; i < header->num_locals; ++i) {
        buffer_add_typeid(
            buf, domain, mono_class_from_mono_type_internal(header->locals[i]));
      }
      /* Names */
      for (i = 0; i < header->num_locals; ++i) {
        char lname[128];
      ...
      if (CHECK_PROTOCOL_VERSION(2, 43)) {
        ...
        for (i = 0; i < locals->num_blocks; ++i) {
          ...
          buffer_add_int(buf, locals->code_blocks[i].end_offset -
                                  locals->code_blocks[i].start_offset);
      ...
      for (i = 0; i < num_locals; ++i) {
        ...
        buffer_add_typeid(buf, domain,
                          mono_class_from_mono_type_internal(
                              header->locals[locals->locals[i].index]));
      }
      /* Names */
      for (i = 0; i < num_locals; ++i)
  ...
  case CMD_METHOD_RESOLVE_TOKEN: {
    ...
    case MONO_TOKEN_STRING: {
      ...
      s = mono_ldstr_checked(domain, m_class_get_image(method->klass),
                             mono_metadata_token_index(token), error);
      mono_error_assert_ok(error); /* FIXME don't swallow the error */
    ...
    default: {
      ...
      if (method->wrapper_type == MONO_WRAPPER_DYNAMIC_METHOD) {
        ...
        handle_class =
            (MonoClass *)mono_method_get_wrapper_data(method, token + 1);
        ...
        val = mono_ldtoken_checked(m_class_get_image(method->klass), token,
                                   &handle_class, NULL, error);
        if (!val)
          g_error("Could not load token due to %s",
                  mono_error_get_message(error));
      }
      ...
      if (handle_class == mono_defaults.typehandle_class) {
        ...
        if (method->wrapper_type == MONO_WRAPPER_DYNAMIC_METHOD)
          ...
          buffer_add_typeid(
              buf, domain, mono_class_from_mono_type_internal((MonoType *)val));
  ...
  case CMD_METHOD_GET_CATTRS: {
    ...
    err = buffer_add_cattrs(buf, domain, m_class_get_image(method->klass),
                            attr_klass, cinfo);
    if (err != ERR_NONE)
      return err;
    break;
  ...
  case CMD_METHOD_MAKE_GENERIC_METHOD: {
    ...
    inflated =
        mono_class_inflate_generic_method_checked(method, &tmp_context, error);
    g_assert(is_ok(error)); /* FIXME don't swallow the error */
...
static ErrorCode method_commands(int command, guint8 *p, guint8 *end,
                                 Buffer *buf) {
  ErrorCode err;
  MonoDomain *old_domain;
  MonoDomain *domain;
...
static ErrorCode thread_commands(int command, guint8 *p, guint8 *end,
                                 Buffer *buf) {
  int objid = decode_objid(p, &p, end);
  ErrorCode err;
  MonoThread *thread_obj;
  ...
  case CMD_THREAD_GET_FRAME_INFO: {
    ...
    */
    ...
    compute_frame_info(
        thread, tls,
        TRUE); // the last parameter is TRUE to force that the frame info that
               // will be send is synchronised with the debugged thread

    buffer_add_int(buf, tls->frame_count);
    for (i = 0; i < tls->frame_count; ++i) {
      buffer_add_int(buf, tls->frames[i]->id);
      buffer_add_methodid(buf, tls->frames[i]->de.domain,
                          tls->frames[i]->actual_method);
  ...
  case CMD_THREAD_SET_IP: {
    ...
    DEBUG_PRINTF(1, "[dbg] Setting IP to %s:0x%0x(0x%0x)\n",
                 tls->frames[0]->actual_method->name, (int)sp.il_offset,
                 (int)sp.native_offset);

    if (tls->frames[0]->de.ji->is_interp) {
      MonoJitTlsData *jit_data = thread->thread_info->jit_data;
      mini_get_interp_callbacks()->set_resume_state(
          jit_data, NULL, NULL, tls->frames[0]->interp_frame,
          (guint8 *)tls->frames[0]->de.ji->code_start + sp.native_offset);
    } else {
      MONO_CONTEXT_SET_IP(&tls->restore_state.ctx,
                          (guint8 *)tls->frames[0]->de.ji->code_start +
                              sp.native_offset);
    }
    break;
  }
...
static ErrorCode frame_commands(int command, guint8 *p, guint8 *end,
                                Buffer *buf) {
  int objid;
  ErrorCode err;
  MonoThread *thread_obj;
  ...
  if (!(jit->has_var_info || frame->de.ji->is_interp) ||
      !mono_get_seq_points(frame->de.domain, frame->actual_method))
    /*
     * The method is probably from an aot image compiled without soft-debug,
     * variables might be dead, etc.
     */
    return ERR_ABSENT_INFORMATION;
    ...
    for (i = 0; i < len; ++i) {
      ...
      if (pos < 0) {
        ...
        if (frame->de.ji->is_interp) {
          ...
          addr = (guint8 *)mini_get_interp_callbacks()->frame_get_arg(
              frame->interp_frame, pos);

          buffer_add_value_full(buf, sig->params[pos], addr, frame->de.domain,
                                FALSE, NULL, 1);
        } else {
          g_assert(pos >= 0 && pos < jit->num_params);

          add_var(buf, jit, sig->params[pos], &jit->params[pos], &frame->ctx,
                  frame->de.domain, FALSE);
        }
      } else {
        MonoDebugLocalsInfo *locals;
        ...
        if (frame->de.ji->is_interp) {
          ...
          addr = (guint8 *)mini_get_interp_callbacks()->frame_get_local(
              frame->interp_frame, pos);

          buffer_add_value_full(buf, header->locals[pos], addr,
                                frame->de.domain, FALSE, NULL, 1);
        } else {
          g_assert(pos >= 0 && pos < jit->num_locals);

          add_var(buf, jit, header->locals[pos], &jit->locals[pos], &frame->ctx,
                  frame->de.domain, FALSE);
        }
      }
    }
    ...
    if (m_class_is_valuetype(frame->api_method->klass)) {
      if (!sig->hasthis) {
        ...
        if (frame->de.ji->is_interp) {
          ...
          addr = (guint8 *)mini_get_interp_callbacks()->frame_get_this(
              frame->interp_frame);

          buffer_add_value_full(
              buf, m_class_get_this_arg(frame->actual_method->klass), addr,
              frame->de.domain, FALSE, NULL, 1);
        } else {
          add_var(buf, jit, m_class_get_this_arg(frame->actual_method->klass),
                  jit->this_var, &frame->ctx, frame->de.domain, TRUE);
        }
      }
    } else {
      if (!sig->hasthis) {
        MonoObject *p = NULL;
        buffer_add_value(buf,
                         m_class_get_byval_arg(frame->actual_method->klass), &p,
                         frame->de.domain);
      } else {
        if (frame->de.ji->is_interp) {
          guint8 *addr;
          ...
          buffer_add_value_full(buf,
                                m_class_get_byval_arg(frame->api_method->klass),
                                addr, frame->de.domain, FALSE, NULL, 1);
        } else {
          add_var(buf, jit, m_class_get_byval_arg(frame->api_method->klass),
                  jit->this_var, &frame->ctx, frame->de.domain, TRUE);
        }
      }
    }
  ...
  case CMD_STACK_FRAME_SET_VALUES: {
    ...
    for (i = 0; i < len; ++i) {
      ...
      if (MONO_TYPE_IS_REFERENCE(t))
        ...
        val_buf = (guint8 *)g_alloca(
            mono_class_instance_size(mono_class_from_mono_type_internal(t)));
      ...
      if (frame->de.ji->is_interp) {
        ...
        set_var(t, var, &frame->ctx, frame->de.domain, val_buf,
                frame->reg_locations, &tls->restore_state.ctx);
      }
    }
    mono_metadata_free_mh(header);
    break;
  ...
  case CMD_STACK_FRAME_SET_THIS: {
    ...
    if (frame->de.ji->is_interp) {
      ...
      addr = (guint8 *)mini_get_interp_callbacks()->frame_get_this(
          frame->interp_frame);
      set_interp_var(m_class_get_this_arg(frame->actual_method->klass), addr,
                     val_buf);
    } else {
      var = jit->this_var;
      g_assert(var);

      set_var(m_class_get_this_arg(frame->actual_method->klass), var,
              &frame->ctx, frame->de.domain, val_buf, frame->reg_locations,
              &tls->restore_state.ctx);
    }
    break;
  }
  default:
...
static ErrorCode array_commands(int command, guint8 *p, guint8 *end,
                                Buffer *buf) {
  MonoArray *arr;
    ...
    for (i = index; i < index + len; ++i) {
      ...
      buffer_add_value(buf,
                       m_class_get_byval_arg(
                           m_class_get_element_class(arr->obj.vtable->klass)),
                       elem, arr->obj.vtable->domain);
    }
    break;
  case CMD_ARRAY_REF_SET_VALUES:
    ...
    for (i = index; i < index + len; ++i) {
      ...
      decode_value(m_class_get_byval_arg(
                       m_class_get_element_class(arr->obj.vtable->klass)),
                   arr->obj.vtable->domain, (guint8 *)elem, p, &p, end, TRUE);
    }
    break;
  default:
    return ERR_NOT_IMPLEMENTED;
...
static ErrorCode string_commands(int command, guint8 *p, guint8 *end,
                                 Buffer *buf) {
  int objid;
  ErrorCode err;
  MonoString *str;
  ...
  case CMD_STRING_REF_GET_VALUE:
    ...
    if (use_utf16) {
      ...
      buffer_add_utf16(buf, (guint8 *)mono_string_chars_internal(str),
                       mono_string_length_internal(str) * 2);
    } else {
      ERROR_DECL(error);
...
static void create_file_to_check_memory_address(void) {
  ...
  filename_check_valid_memory =
      g_build_filename(g_get_tmp_dir(), file_name, (const char *)NULL);
  file_check_valid_memory =
      open(filename_check_valid_memory, O_CREAT | O_WRONLY | O_APPEND, S_IWUSR);
  g_free(file_name);
}
...
static ErrorCode pointer_commands(int command, guint8 *p, guint8 *end,
                                  Buffer *buf) {
  ErrorCode err;
  gint64 addr;
  MonoClass *klass;
...
static ErrorCode object_commands(int command, guint8 *p, guint8 *end,
                                 Buffer *buf) {
  HANDLE_FUNCTION_ENTER();
  ...
  case CMD_OBJECT_REF_GET_TYPE:
    ...
    buffer_add_typeid(buf, obj->vtable->domain,
                      mono_class_from_mono_type_internal(
                          ((MonoReflectionType *)obj->vtable->type)->type));
    break;
  case CMD_OBJECT_REF_GET_VALUES:
    ...
    for (i = 0; i < len; ++i) {
      ...
      if (f->type->attrs & FIELD_ATTRIBUTE_STATIC) {
        ...
        vtable =
            mono_class_vtable_checked(obj->vtable->domain, f->parent, error);
        if (!is_ok(error)) {
          mono_error_cleanup(error);
          goto invalid_object;
        }
        val = (guint8 *)g_malloc(mono_class_instance_size(
            mono_class_from_mono_type_internal(f->type)));
        mono_field_static_get_value_checked(vtable, f, val, string_handle,
                                            error);
        if (!is_ok(error)) {
        ...
        if (remote_obj) {
#ifndef DISABLE_REMOTING
          field_value = mono_load_remote_field_checked(obj, obj_type, f,
                                                       &field_storage, error);
          if (!is_ok(error)) {
            mono_error_cleanup(error); /* FIXME report the error */
            goto invalid_object;
  ...
  case CMD_OBJECT_REF_SET_VALUES:
    ...
    for (i = 0; i < len; ++i) {
      ...
      if (f->type->attrs & FIELD_ATTRIBUTE_STATIC) {
        ...
        err = decode_value(f->type, obj->vtable->domain,
                           (guint8 *)obj + f->offset, p, &p, end, TRUE);
        if (err != ERR_NONE)
          goto exit;
      }
    }
...
static const char *vm_cmds_str[] = {"VERSION",
                                    "ALL_THREADS",
                                    "SUSPEND",
                                    "RESUME",
                                    "EXIT",
...
static const char *thread_cmds_str[] = {
    "GET_FRAME_INFO", "GET_NAME", "GET_STATE", "GET_INFO",
    "GET_ID",         "GET_TID",  "SET_IP"};

static const char *event_cmds_str[] = {"REQUEST_SET", "REQUEST_CLEAR",
                                       "REQUEST_CLEAR_ALL_BREAKPOINTS"};

static const char *appdomain_cmds_str[] = {
    "GET_ROOT_DOMAIN",    "GET_FRIENDLY_NAME", "GET_ASSEMBLIES",
    "GET_ENTRY_ASSEMBLY", "CREATE_STRING",     "GET_CORLIB",
    "CREATE_BOXED_VALUE", "CREATE_BYTE_ARRAY",
};

static const char *assembly_cmds_str[] = {
    "GET_LOCATION", "GET_ENTRY_POINT", "GET_MANIFEST_MODULE", "GET_OBJECT",
    "GET_TYPE",     "GET_NAME",        "GET_DOMAIN",          "HAS_DEBUG_INFO"};
...
static const char *method_cmds_str[] = {
    "GET_NAME",        "GET_DECLARING_TYPE", "GET_DEBUG_INFO", "GET_PARAM_INFO",
    "GET_LOCALS_INFO", "GET_INFO",           "GET_BODY",       "RESOLVE_TOKEN",
    "GET_CATTRS ",     "MAKE_GENERIC_METHOD"};

static const char *type_cmds_str[] = {
    "GET_INFO",           "GET_METHODS",        "GET_FIELDS",
    "GET_VALUES",         "GET_OBJECT",         "GET_SOURCE_FILES",
    "SET_VALUES",         "IS_ASSIGNABLE_FROM", "GET_PROPERTIES ",
    "GET_CATTRS",         "GET_FIELD_CATTRS",   "GET_PROPERTY_CATTRS",
    "GET_SOURCE_FILES_2", "GET_VALUES_2",       "GET_METHODS_BY_NAME_FLAGS",
    "GET_INTERFACES",     "GET_INTERFACE_MAP",  "IS_INITIALIZED",
    "CREATE_INSTANCE",    "GET_VALUE_SIZE"};

static const char *stack_frame_cmds_str[] = {
    "GET_VALUES", "GET_THIS", "SET_VALUES", "GET_DOMAIN", "SET_THIS"};
...
static const char *string_cmds_str[] = {"GET_VALUE", "GET_LENGTH", "GET_CHARS"};

static const char *pointer_cmds_str[] = {"GET_VALUE"};

static const char *object_cmds_str[] = {
    "GET_TYPE",   "GET_VALUES", "IS_COLLECTED", "GET_ADDRESS",
    "GET_DOMAIN", "SET_VALUES", "GET_INFO",
};
...
static gboolean wait_for_attach(void) {
#ifndef DISABLE_SOCKET_TRANSPORT
  if (listen_fd == -1) {
...
static gsize WINAPI debugger_thread(void *arg) {
  ...
  mono_thread_set_name_constant_ignore_error(internal, "Debugger agent",
  ...
  while (!attach_failed) {
    ...
    mono_debugger_log_command(command_set_to_string(command_set), cmd_str,
                              buf.buf, buffer_len(&buf));

    if (err == ERR_NONE && command_set == CMD_SET_VM &&
        command == CMD_VM_STOP_BUFFERING) {
      send_buffered_reply_packets();
    ...
    if (command_set == CMD_SET_VM &&
        (command == CMD_VM_DISPOSE || command == CMD_VM_EXIT))
      break;
  }
  ...
  if (!attach_failed && command_set == CMD_SET_VM &&
      command == CMD_VM_DISPOSE &&
      !(vm_death_event_sent || mono_runtime_is_shutting_down())) {
...
void mono_debugger_agent_init(void) {
...
void mono_debugger_agent_parse_options(char *options) { sdb_options = options; }
