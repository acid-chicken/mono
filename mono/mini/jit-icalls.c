...
void *mono_ldftn(MonoMethod *method) {
  ...
  if (mono_llvm_only) {
    ...
    addr = mini_add_method_trampoline(
...
static void *ldvirtfn_internal(MonoObject *obj, MonoMethod *method,
                               gboolean gshared) {
  ERROR_DECL(error);
  MonoMethod *res;
  ...
  if (gshared && method->is_inflated &&
      mono_method_get_context(method)->method_inst) {
    MonoGenericContext context = {NULL, NULL};

    if (mono_class_is_ginst(res->klass))
      context.class_inst =
          mono_class_get_generic_class(res->klass)->context.class_inst;
    else if (mono_class_is_gtd(res->klass))
      context.class_inst =
          mono_class_get_generic_container(res->klass)->context.class_inst;
...
void mono_helper_stelem_ref_check(MonoArray *array, MonoObject *val) {
  ...
  if (val &&
      !mono_object_isinst_checked(
          val, m_class_get_element_class(mono_object_class(array)), error)) {
...
gint64 mono_llmult(gint64 a, gint64 b) { return a * b; }
...
guint64 mono_llmult_ovf(gint64 a, gint64 b) {
  ...
  */
  ...
  if (ah && bh)
    goto raise_exception;
  if ((gint64)((gint64)ah * (gint64)bl) > (gint64)0x80000000 ||
      (gint64)((gint64)al * (gint64)bh) > (gint64)0x80000000)
    goto raise_exception;
...
gint32 mono_imul(gint32 a, gint32 b) { return a * b; }
...
double mono_fdiv(double a, double b) { return a / b; }
#endif

#ifdef MONO_ARCH_SOFT_FLOAT_FALLBACK

double mono_fsub(double a, double b) { return a - b; }

double mono_fadd(double a, double b) { return a + b; }

double mono_fmul(double a, double b) { return a * b; }

double mono_fneg(double a) { return -a; }

double mono_fconv_r4(double a) { return (float)a; }

double mono_conv_to_r8(int a) { return (double)a; }

double mono_conv_to_r4(int a) { return (double)(float)a; }

gint8 mono_fconv_i1(double a) { return (gint8)a; }

gint16 mono_fconv_i2(double a) { return (gint16)a; }

gint32 mono_fconv_i4(double a) { return (gint32)a; }

guint8 mono_fconv_u1(double a) { return (guint8)a; }

guint16 mono_fconv_u2(double a) { return (guint16)a; }

gboolean mono_fcmp_eq(double a, double b) { return a == b; }

gboolean mono_fcmp_ge(double a, double b) { return a >= b; }

gboolean mono_fcmp_gt(double a, double b) { return a > b; }

gboolean mono_fcmp_le(double a, double b) { return a <= b; }

gboolean mono_fcmp_lt(double a, double b) { return a < b; }
...
gboolean mono_fceq(double a, double b) { return a == b; }

gboolean mono_fcgt(double a, double b) { return a > b; }

gboolean mono_fcgt_un(double a, double b) { return isunordered(a, b) || a > b; }

gboolean mono_fclt(double a, double b) { return a < b; }

gboolean mono_fclt_un(double a, double b) { return isunordered(a, b) || a < b; }

double mono_fload_r4(float *ptr) { return *ptr; }

void mono_fstore_r4(double val, float *ptr) { *ptr = (float)val; }
...
MonoArray *mono_array_new_n_icall(MonoMethod *cm, gint32 pcount,
                                  intptr_t *params) {
  ERROR_DECL(error);
  g_assert(cm);
  g_assert(pcount);
  ...
  MonoArray *arr = mono_array_new_full_checked(mono_domain_get(), cm->klass,
                                               lengths, lower_bounds, error);

  return mono_error_set_pending_exception(error) ? NULL : arr;
}

static MonoArray *mono_array_new_n(MonoMethod *cm, int n, uintptr_t lengths[],
...
MonoArray *mono_array_new_3(MonoMethod *cm, guint32 length1, guint32 length2,
                            guint32 length3) {
...
MonoArray *mono_array_new_4(MonoMethod *cm, guint32 length1, guint32 length2,
                            guint32 length3, guint32 length4) {
  uintptr_t lengths[] = {length1, length2, length3, length4};
  intptr_t lower_bounds[G_N_ELEMENTS(lengths)] = {0};
  return mono_array_new_n(cm, G_N_ELEMENTS(lengths), lengths, lower_bounds);
}

gpointer mono_class_static_field_address(MonoDomain *domain,
                                         MonoClassField *field) {
  ERROR_DECL(error);
  MonoVTable *vtable;
  gpointer addr;

  // printf ("SFLDA0 %s.%s::%s %d\n", field->parent->name_space,
  // field->parent->name, field->name, field->offset, field->parent->inited);
...
gpointer mono_ldtoken_wrapper(MonoImage *image, int token,
...
gpointer mono_ldtoken_wrapper_generic_shared(MonoImage *image, int token,
                                             MonoMethod *method) {
...
guint64 mono_fconv_u8_2(double v) {
...
gint64 mono_fconv_i8(double v) { return (gint64)v; }
#endif
...
guint32 mono_fconv_u4_2(double v) {
...
guint64 mono_fconv_ovf_u8(double v) {
  ...
  /*
   * The soft-float implementation of some ARM devices have a buggy guin64 to
   * double conversion that it looses precision even when the integer if fully
   * representable as a double.
   *
   * This was found with 4294967295ull, converting to double and back looses one
   * bit of precision.
   *
...
guint64 mono_fconv_ovf_u8_un(double v) { return mono_fconv_ovf_u8(v); }

#ifdef MONO_ARCH_EMULATE_FCONV_TO_I8
gint64 mono_rconv_i8(float v) { return (gint64)v; }
#endif
...
guint64 mono_rconv_ovf_u8_un(float v) { return mono_rconv_ovf_u8(v); }

#ifdef MONO_ARCH_EMULATE_LCONV_TO_R8
double mono_lconv_to_r8(gint64 a) { return (double)a; }
#endif

#ifdef MONO_ARCH_EMULATE_LCONV_TO_R4
float mono_lconv_to_r4(gint64 a) { return (float)a; }
#endif

#ifdef MONO_ARCH_EMULATE_CONV_R8_UN
double mono_conv_to_r8_un(guint32 a) { return (double)a; }
#endif

#ifdef MONO_ARCH_EMULATE_LCONV_TO_R8_UN
double mono_lconv_to_r8_un(guint64 a) { return (double)a; }
#endif

#ifdef MONO_ARCH_EMULATE_FREM
// Wrapper to avoid taking address of overloaded function.
double mono_fmod(double a, double b) { return fmod(a, b); }
#endif

gpointer mono_helper_compile_generic_method(MonoObject *obj, MonoMethod *method,
                                            gpointer *this_arg) {
  ERROR_DECL(error);
  MonoMethod *vmethod;
  gpointer addr;
  ...
  g_assert(!mono_class_is_ginst(vmethod->klass) ||
           !mono_class_get_generic_class(vmethod->klass)
                ->context.class_inst->is_open);
  ...
  addr = mini_add_method_trampoline(
      vmethod, addr, mono_method_needs_static_rgctx_invoke(vmethod, FALSE),
      FALSE);
...
MonoString *ves_icall_mono_ldstr(MonoDomain *domain, MonoImage *image,
                                 guint32 idx) {
  ERROR_DECL(error);
...
MonoString *mono_helper_ldstr_mscorlib(guint32 idx) {
  ERROR_DECL(error);
  MonoString *result =
      mono_ldstr_checked(mono_domain_get(), mono_defaults.corlib, idx, error);
  mono_error_set_pending_exception(error);
  return result;
}
...
MonoObject *mono_helper_newobj_mscorlib(guint32 idx) {
  ERROR_DECL(error);
  MonoClass *klass = mono_class_get_checked(mono_defaults.corlib,
                                            MONO_TOKEN_TYPE_DEF | idx, error);
...
/*
 * On some architectures, gdb doesn't like encountering the cpu breakpoint
 * instructions in generated code. So instead we emit a call to this function
 * and place a gdb breakpoint here.
 */
...
MonoException *mono_create_corlib_exception_1(guint32 token,
                                              MonoString *arg_raw) {
...
MonoException *mono_create_corlib_exception_2(guint32 token,
                                              MonoString *arg1_raw,
                                              MonoString *arg2_raw) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
...
MonoObject *mono_object_castclass_unbox(MonoObject *obj, MonoClass *klass) {
  ...
  if ((m_class_is_enumtype(klass) &&
       oklass == m_class_get_element_class(klass)) ||
      (m_class_is_enumtype(oklass) &&
       klass == m_class_get_element_class(oklass)))
    return obj;
  ...
  mono_set_pending_exception(mono_exception_from_name(
      mono_defaults.corlib, "System", "InvalidCastException"));

  return NULL;
}

MonoObject *mono_object_castclass_with_cache(MonoObject *obj, MonoClass *klass,
                                             gpointer *cache) {
  ERROR_DECL(error);
  MonoJitTlsData *jit_tls = NULL;
...
MonoObject *mono_object_isinst_with_cache(MonoObject *obj, MonoClass *klass,
                                          gpointer *cache) {
  ERROR_DECL(error);
...
gpointer mono_get_native_calli_wrapper(MonoImage *image,
...
static MonoMethod *constrained_gsharedvt_call_setup(gpointer mp,
                                                    MonoMethod *cmethod,
                                                    MonoClass *klass,
                                                    gpointer *this_arg,
                                                    MonoError *error) {
  ...
  if (mono_class_is_interface(klass) || !m_class_is_valuetype(klass)) {
    ...
    /* Have to use the receiver's type instead of klass, the receiver is a ref
     * type */
  ...
  if (mono_method_signature_internal(cmethod)->pinvoke) {
    ...
    if (cmethod->is_inflated) {
      m = mono_class_inflate_generic_method_full_checked(
          m, NULL, mono_method_get_context(cmethod), error);
      return_val_if_nok(error, NULL);
    }
  }

  if (m_class_is_valuetype(klass) &&
      (m->klass == mono_defaults.object_class ||
       m->klass == m_class_get_parent(mono_defaults.enum_class) ||
       m->klass == mono_defaults.enum_class)) {
    /*
 ...
 *
 *   Make a call to CMETHOD using the receiver MP, which is assumed to be of
 * type KLASS. ARGS contains the arguments to the method in the format used by
 * mono_runtime_invoke_checked ().
 */
MonoObject *mono_gsharedvt_constrained_call(gpointer mp, MonoMethod *cmethod,
                                            MonoClass *klass,
                                            gboolean deref_arg,
                                            gpointer *args) {
  ERROR_DECL(error);
...
void ves_icall_runtime_class_init(MonoVTable *vtable) {
...
void mono_generic_class_init(MonoVTable *vtable) {
  ERROR_DECL(error);
  mono_runtime_class_init_full(vtable, error);
  mono_error_set_pending_exception(error);
}

void ves_icall_mono_delegate_ctor(MonoObject *this_obj_raw,
                                  MonoObject *target_raw, gpointer addr) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
...
void ves_icall_mono_delegate_ctor_interp(MonoObject *this_obj_raw,
                                         MonoObject *target_raw,
                                         gpointer addr) {
  HANDLE_FUNCTION_ENTER();
  ERROR_DECL(error);
  MONO_HANDLE_DCL(MonoObject, this_obj);
...
gpointer mono_fill_method_rgctx(MonoMethodRuntimeGenericContext *mrgctx,
                                int index) {
  ERROR_DECL(error);
  gpointer res;
...
MonoObject *mono_get_assembly_object(MonoImage *image) {
  ICALL_ENTRY();
  MonoObjectHandle result = MONO_HANDLE_CAST(
      MonoObject, mono_assembly_get_object_handle(mono_domain_get(),
                                                  image->assembly, error));
...
MonoObject *mono_get_method_object(MonoMethod *method) {
  ERROR_DECL(error);
  MonoObject *result;
  result = (MonoObject *)mono_method_get_object_checked(
      mono_domain_get(), method, method->klass, error);
  mono_error_set_pending_exception(error);
  return result;
}
