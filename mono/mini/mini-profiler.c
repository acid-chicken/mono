...
static MonoInst *emit_fill_call_ctx(MonoCompile *cfg, MonoInst *method,
                                    MonoInst *ret) {
  ...
  args_alloc->inst_imm =
      (sig->param_count + sig->hasthis) * TARGET_SIZEOF_VOID_P;
  args_alloc->flags |= MONO_INST_INIT;
  MONO_ADD_INS(cfg->cbb, args_alloc);
  MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, alloc->dreg,
                              MONO_STRUCT_OFFSET(MonoProfilerCallContext, args),
                              args_alloc->dreg);
  ...
  for (int i = 0; i < sig->hasthis + sig->param_count; ++i) {
    ...
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, args_alloc->dreg,
                                i * TARGET_SIZEOF_VOID_P, ins->dreg);
  }

  MONO_EMIT_NEW_STORE_MEMBASE(
      cfg, OP_STORE_MEMBASE_REG, alloc->dreg,
      MONO_STRUCT_OFFSET(MonoProfilerCallContext, method), method->dreg);
  ...
  if (ret) {
    ...
    MONO_EMIT_NEW_STORE_MEMBASE(
        cfg, OP_STORE_MEMBASE_REG, alloc->dreg,
        MONO_STRUCT_OFFSET(MonoProfilerCallContext, return_value), addr->dreg);
    ...
    if (cfg->backend->use_fpstack &&
        (ret_type->type == MONO_TYPE_R8 || ret_type->type == MONO_TYPE_R4)) {
...
static gboolean can_encode_method_ref(MonoMethod *method) {
...
void mini_profiler_emit_enter(MonoCompile *cfg) {
  ...
  if ((!MONO_CFG_PROFILE(cfg, ENTER) || cfg->current_method != cfg->method ||
       (cfg->compile_aot && !can_encode_method_ref(cfg->method))) &&
      !trace)
    return;
  ...
  /* void mono_profiler_raise_method_enter (MonoMethod *method, MonoJitInfo *ji,
...
void mini_profiler_emit_leave(MonoCompile *cfg, MonoInst *ret) {
  ...
  /* void mono_profiler_raise_method_leave (MonoMethod *method, MonoJitInfo *ji,
   * MonoProfilerCallContext *ctx) */
  if (trace)
...
void mini_profiler_emit_tail_call(MonoCompile *cfg, MonoMethod *target) {
  ...
  if ((!MONO_CFG_PROFILE(cfg, TAIL_CALL) ||
       cfg->current_method != cfg->method) &&
      !trace)
    return;
  ...
  /* void mono_profiler_raise_method_tail_call (MonoMethod *method, MonoMethod
   * *target) */
  if (trace)
...
void mini_profiler_emit_call_finally(MonoCompile *cfg, MonoMethodHeader *header,
  ...
  MonoInst *ins = mini_emit_runtime_constant(
      cfg, MONO_PATCH_INFO_PROFILER_CLAUSE_COUNT, NULL);
  MonoInst *count_ins = mini_emit_memory_load(
      cfg, m_class_get_byval_arg(mono_defaults.uint32_class), ins, 0, 0);
  ...
  for (guint32 i = 0; i < header->num_clauses; i++) {
    ...
    if (hclause->flags != MONO_EXCEPTION_CLAUSE_NONE &&
        hclause->flags != MONO_EXCEPTION_CLAUSE_FILTER)
      continue;

    if (!MONO_OFFSET_IN_HANDLER(hclause, offset))
      continue;

    if (offset + (*ip == CEE_LEAVE ? 5 : 2) <=
        hclause->handler_offset + hclause->handler_len) {
      cclause = hclause;
      break;
    }
  }
  ...
  if (cclause)
    EMIT_NEW_TEMPLOAD(
        cfg, iargs[3],
        mono_find_exvar_for_offset(cfg, cclause->handler_offset)->inst_c0);
  else
    EMIT_NEW_PCONST(cfg, iargs[3], NULL);

  /* void mono_profiler_raise_exception_clause (MonoMethod *method, uint32_t
   * index, MonoExceptionEnum type, MonoObject *exception) */
...
static gpointer memdup_with_type(gpointer data, MonoType *t) {
  int dummy;

  return g_memdup(data, mono_type_size(t, &dummy));
}

static guint8 *get_int_reg(MonoContext *ctx, guint32 reg) {
  return (guint8 *)(gsize)mono_arch_context_get_int_reg(ctx, reg);
}

static gpointer get_variable_buffer(MonoDebugMethodJitInfo *jit,
                                    MonoDebugVarInfo *var, MonoContext *ctx) {
  ...
  case MONO_DEBUG_VAR_ADDRESS_MODE_REGOFFSET:
    return memdup_with_type(get_int_reg(ctx, reg) + (gint32)var->offset,
                            var->type);
  case MONO_DEBUG_VAR_ADDRESS_MODE_REGOFFSET_INDIR:
  case MONO_DEBUG_VAR_ADDRESS_MODE_VTADDR:
    return memdup_with_type(
        *(guint8 **)(get_int_reg(ctx, reg) + (gint32)var->offset), var->type);
...
gpointer mini_profiler_context_get_this(MonoProfilerCallContext *ctx) {
  ...
  if (ctx->interp_frame)
    return memdup_with_type(
        mini_get_interp_callbacks()->frame_get_this(ctx->interp_frame),
        m_class_get_this_arg(ctx->method->klass));
  else
    return memdup_with_type(ctx->args[0],
                            m_class_get_this_arg(ctx->method->klass));
}
...
gpointer mini_profiler_context_get_argument(MonoProfilerCallContext *ctx,
  ...
  if (ctx->interp_frame)
    return memdup_with_type(
        mini_get_interp_callbacks()->frame_get_arg(ctx->interp_frame, pos),
        sig->params[pos]);
...
gpointer mini_profiler_context_get_local(MonoProfilerCallContext *ctx,
  ...
  if (ctx->interp_frame)
    return memdup_with_type(
        mini_get_interp_callbacks()->frame_get_local(ctx->interp_frame, pos),
        t);

  MonoDebugMethodJitInfo *info =
      mono_debug_find_method(ctx->method, mono_domain_get());
...
gpointer mini_profiler_context_get_result(MonoProfilerCallContext *ctx) {
  ...
  if (ctx->interp_frame)
    ctx->return_value =
        mini_get_interp_callbacks()->frame_get_res(ctx->interp_frame);
...
void mini_profiler_context_free_buffer(void *buffer) { g_free(buffer); }
