...
static GENERATE_GET_CLASS_WITH_CACHE(
    runtime_helpers, "System.Runtime.CompilerServices",
    "RuntimeHelpers") static GENERATE_TRY_GET_CLASS_WITH_CACHE(math, "System",
                                                               "Math")

    /* optimize the simple GetGenericValueImpl/SetGenericValueImpl generic calls
     */
    static MonoInst *emit_array_generic_access(MonoCompile *cfg,
                                               MonoMethodSignature *fsig,
                                               MonoInst **args, int is_set) {
  ...
  if (is_set) {
    ...
    EMIT_NEW_STORE_MEMBASE_TYPE(cfg, store, etype, args[2]->dreg, 0,
                                load->dreg);
  }
  return store;
}

static gboolean mono_type_is_native_blittable(MonoType *t) {
  ...
  // If the native marshal size is different we can't convert PtrToStructure to
  // a type load
...
MonoInst *mini_emit_inst_for_ctor(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  if (in_corlib && !strcmp(cmethod_klass_name_space, "System") &&
      !strcmp(cmethod_klass_name, "ByReference`1")) {
    /* public ByReference(ref T value) */
    g_assert(fsig->hasthis && fsig->param_count == 1);
    EMIT_NEW_STORE_MEMBASE(cfg, ins, OP_STORE_MEMBASE_REG, args[0]->dreg, 0,
                           args[1]->dreg);
    return ins;
  }
...
static MonoInst *llvm_emit_inst_for_method(MonoCompile *cfg,
                                           MonoMethod *cmethod,
                                           MonoMethodSignature *fsig,
                                           MonoInst **args,
                                           gboolean in_corlib) {
  MonoInst *ins = NULL;
  int opcode = 0;
  // Convert Math and MathF methods into LLVM intrinsics, e.g. MathF.Sin ->
  // @llvm.sin.f32
  if (in_corlib && !strcmp(m_class_get_name(cmethod->klass), "MathF") &&
    ...
    if (fsig->param_count == 1 && fsig->params[0]->type == MONO_TYPE_R4) {
      if (!strcmp(cmethod->name, "Ceiling")) {
        ...
        // special case: emit vroundps for MathF.Round directly instead of what
        // llvm.round.f32 emits to align with CoreCLR behavior
    ...
    // (float, float)
    if (fsig->param_count == 2 && fsig->params[0]->type == MONO_TYPE_R4 &&
    ...
    // (float, float, float)
    if (fsig->param_count == 3 && fsig->params[0]->type == MONO_TYPE_R4 &&
        fsig->params[1]->type == MONO_TYPE_R4 &&
        fsig->params[2]->type == MONO_TYPE_R4) {
  ...
  if (cmethod->klass == mono_class_try_get_math_class()) {
    ...
    // (double, double)
    if (fsig->param_count == 2 && fsig->params[0]->type == MONO_TYPE_R8 &&
    ...
    if (fsig->param_count == 3 && fsig->params[0]->type == MONO_TYPE_R8 &&
        fsig->params[1]->type == MONO_TYPE_R8 &&
        fsig->params[2]->type == MONO_TYPE_R8) {
      if (!strcmp(cmethod->name, "FusedMultiplyAdd")) {
        opcode = OP_FMA;
      }
    }
  ...
  if (in_corlib && !strcmp(m_class_get_name(cmethod->klass), "Buffer")) {
    if (!strcmp(cmethod->name, "Memmove") && fsig->param_count == 3 &&
        fsig->params[0]->type == MONO_TYPE_PTR &&
        fsig->params[1]->type == MONO_TYPE_PTR) {
...
static MonoInst *emit_span_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
                                      MonoMethodSignature *fsig,
                                      MonoInst **args) {
  MonoInst *ins;

  MonoClassField *ptr_field =
      mono_class_get_field_from_name_full(cmethod->klass, "_pointer", NULL);
  if (!ptr_field)
    /* Portable Span<T> */
    return NULL;
  ...
  if (!strcmp(cmethod->name, "get_Item")) {
    MonoClassField *length_field =
        mono_class_get_field_from_name_full(cmethod->klass, "_length", NULL);
    ...
    EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOAD_MEMBASE, base_reg, span_reg,
                          ptr_field->offset - MONO_ABI_SIZEOF(MonoObject));
    ...
    mini_emit_bounds_check_offset(
        cfg, span_reg, length_field->offset - MONO_ABI_SIZEOF(MonoObject),
        index_reg, NULL);
    ...
    /*
     * FIXME: This doesn't work with abcrem, since the src is a unique LDADDR
     * not the same array object.
     */
...
static MonoInst *emit_unsafe_intrinsics(MonoCompile *cfg, MonoMethod *cmethod,
                                        MonoMethodSignature *fsig,
                                        MonoInst **args) {
  MonoInst *ins;
  int dreg, align;
  ...
  if (!strcmp(cmethod->name, "As")) {
    ...
    if (ctx->method_inst->type_argc == 2) {
      ...
      if (mini_is_gsharedvt_variable_type(t))
        return NULL;
      // Casts the given object to the specified type, performs no dynamic type
      // checking.
      ...
      ins->type = STACK_OBJ;
      ins->klass =
          mono_class_from_mono_type_internal(ctx->method_inst->type_argv[0]);
      return ins;
    }
    ...
    MonoInst *esize_ins;
    if (mini_is_gsharedvt_variable_type(t)) {
      esize_ins = mini_emit_get_gsharedvt_info_klass(
          cfg, mono_class_from_mono_type_internal(t),
    ...
    mini_emit_memory_init_bytes(cfg, args[0], args[1], args[2],
                                MONO_INST_UNALIGNED);
...
static MonoInst *emit_jit_helpers_intrinsics(MonoCompile *cfg,
                                             MonoMethod *cmethod,
                                             MonoMethodSignature *fsig,
                                             MonoInst **args) {
  MonoInst *ins;
  ...
  if (!strcmp(cmethod->name, "EnumEquals") ||
      !strcmp(cmethod->name, "EnumCompareTo")) {
    g_assert(ctx);
    g_assert(ctx->method_inst);
    g_assert(ctx->method_inst->type_argc == 1);
    g_assert(fsig->param_count == 2);
...
static gboolean byref_arg_is_reference(MonoType *t) {
  g_assert(t->byref);

  return mini_type_is_reference(
...
MonoInst *mini_emit_inst_for_method(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  if (m_class_get_nested_in(cmethod->klass))
    cmethod_klass_name_space =
        m_class_get_name_space(m_class_get_nested_in(cmethod->klass));
  else
  ...
  if (in_corlib && !strcmp(cmethod_klass_name_space, "System") &&
    ...
    if (!strcmp(cmethod->name, "GetRawData")) {
      int dreg = alloc_preg(cfg);
      EMIT_NEW_BIALU_IMM(cfg, ins, OP_PADD_IMM, dreg, args[0]->dreg,
  ...
  if (cmethod->klass == mono_defaults.string_class) {
    if (strcmp(cmethod->name, "get_Chars") == 0 &&
        fsig->param_count + fsig->hasthis == 2) {
...
#if defined(TARGET_X86) || defined(TARGET_AMD64)
      EMIT_NEW_X86_LEA(cfg, ins, args[0]->dreg, index_reg, 1,
                       MONO_STRUCT_OFFSET(MonoString, chars));
      add_reg = ins->dreg;
      EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOADU2_MEMBASE, dreg, add_reg, 0);
#else
      int mult_reg = alloc_preg(cfg);
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_SHL_IMM, mult_reg, index_reg, 1);
      MONO_EMIT_NEW_BIALU(cfg, OP_PADD, add_reg, mult_reg, args[0]->dreg);
      EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOADU2_MEMBASE, dreg, add_reg,
                            MONO_STRUCT_OFFSET(MonoString, chars));
#endif
      mini_type_from_op(cfg, ins, NULL, NULL);
      return ins;
    } else if (strcmp(cmethod->name, "get_Length") == 0 &&
               fsig->param_count + fsig->hasthis == 1) {
      int dreg = alloc_ireg(cfg);
    ...
    if (strcmp(cmethod->name, "GetType") == 0 &&
      ...
      MONO_EMIT_NEW_LOAD_MEMBASE_FAULT(cfg, vt_reg, args[0]->dreg,
                                       MONO_STRUCT_OFFSET(MonoObject, vtable));
      EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOAD_MEMBASE, dreg, vt_reg,
                            MONO_STRUCT_OFFSET(MonoVTable, type));
      mini_type_from_op(cfg, ins, NULL, NULL);

      return ins;
    } else if (!cfg->backend->emulate_mul_div &&
               strcmp(cmethod->name, "InternalGetHashCode") == 0 &&
               fsig->param_count == 1 && !mono_gc_is_moving()) {
      int dreg = alloc_ireg(cfg);
    ...
    if (fsig->param_count + fsig->hasthis == 3 && !cfg->gsharedvt &&
      ...
      MONO_EMIT_NEW_LOAD_MEMBASE_FAULT(cfg, vt_reg, args[0]->dreg,
                                       MONO_STRUCT_OFFSET(MonoObject, vtable));
      EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOAD_MEMBASE, class_reg, vt_reg,
                            MONO_STRUCT_OFFSET(MonoVTable, klass));
      EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOADI4_MEMBASE, sizes_reg, class_reg,
                            m_class_offsetof_sizes());
      return ins;
    } else if (!strcmp(cmethod->name, "IsPrimitive")) {
      int dreg = alloc_ireg(cfg);
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOAD_MEMBASE, dreg, args[0]->dreg,
                                    MONO_STRUCT_OFFSET(MonoObject, vtable));
      MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, dreg, dreg,
                                    MONO_STRUCT_OFFSET(MonoVTable, flags));
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_IAND_IMM, dreg, dreg,
    ...
    /*
     * This is an inline version of GetLength/GetLowerBound(0) used frequently
     * in Array methods.
     */
    else if (((strcmp(cmethod->name, "GetLength") == 0 &&
               fsig->param_count + fsig->hasthis == 2) ||
              (strcmp(cmethod->name, "GetLowerBound") == 0 &&
               fsig->param_count + fsig->hasthis == 2)) &&
      ...
      /* Non-szarray case */
      if (get_length)
        EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOADI4_MEMBASE, dreg, bounds_reg,
      ...
      /* Szarray case */
      if (get_length)
        EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOADI4_MEMBASE, dreg, args[0]->dreg,
    ...
    if (strcmp(cmethod->name, "get_Rank") == 0 &&
      ...
      MONO_EMIT_NEW_LOAD_MEMBASE_OP_FAULT(
          cfg, OP_LOAD_MEMBASE, vtable_reg, args[0]->dreg,
          MONO_STRUCT_OFFSET(MonoObject, vtable));
      EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOADU1_MEMBASE, dreg, vtable_reg,
    ...
    if (strcmp(cmethod->name, "get_OffsetToStringData") == 0 &&
      ...
      return ins;
    } else if (strcmp(cmethod->name, "IsReferenceOrContainsReferences") == 0 &&
               fsig->param_count == 0) {
      MonoGenericContext *ctx = mono_method_get_context(cmethod);
      g_assert(ctx);
      g_assert(ctx->method_inst);
      ...
      /* Resolve the argument class as possible so we can handle common cases
       * fast */
      ...
      if (MONO_TYPE_IS_REFERENCE(t))
        ...
        EMIT_NEW_ICONST(cfg, ins, 0);
               ...
               (t->type == MONO_TYPE_VAR || t->type == MONO_TYPE_MVAR) &&
        ...
        /* This returns 1 or 2 */
        MonoInst *info = mini_emit_get_rgctx_klass(
            cfg, context_used, arg_class,
      ...
      return ins;
    } else if (strcmp(cmethod->name, "IsBitwiseEquatable") == 0 &&
               fsig->param_count == 0) {
      MonoGenericContext *ctx = mono_method_get_context(cmethod);
      g_assert(ctx);
      g_assert(ctx->method_inst);
    ...
    if (!strcmp(cmethod->name, "Enter") && fsig->param_count == 2 &&
        fsig->params[1]->byref) {
      is_enter = TRUE;
      is_v4 = TRUE;
    }
    ...
    if (is_enter) {
      /*
       * To make async stack traces work, icalls which can block should have a
       * wrapper. For Monitor.Enter, emit two calls: a fastpath which doesn't
       * have a wrapper, and a slowpath, which does.
       */
      MonoBasicBlock *end_bb;
    ...
    if (strcmp(cmethod->name, "SpinWait_nop") == 0 && fsig->param_count == 0) {
    ...
    } else if (!strcmp(cmethod->name, "VolatileRead") &&
               fsig->param_count == 1 && fsig->params[0]->byref) {
      guint32 opcode = 0;
      ...
      if (fsig->params[0]->type == MONO_TYPE_I1)
        ...
        opcode = OP_LOADU4_MEMBASE;
               ...
               fsig->params[0]->type == MONO_TYPE_U8)
        ...
        opcode = OP_LOADR8_MEMBASE;
               ...
               fsig->params[0]->type == MONO_TYPE_U)
    ...
    } else if (!strcmp(cmethod->name, "VolatileWrite") &&
               fsig->param_count == 2 && fsig->params[0]->byref) {
      guint32 opcode = 0;
      gboolean is_ref = byref_arg_is_reference(fsig->params[0]);

      if (fsig->params[0]->type == MONO_TYPE_I1 ||
          fsig->params[0]->type == MONO_TYPE_U1)
        opcode = OP_STOREI1_MEMBASE_REG;
      else if (fsig->params[0]->type == MONO_TYPE_I2 ||
               fsig->params[0]->type == MONO_TYPE_U2)
        opcode = OP_STOREI2_MEMBASE_REG;
      else if (fsig->params[0]->type == MONO_TYPE_I4 ||
               fsig->params[0]->type == MONO_TYPE_U4)
...
#if SIZEOF_REGISTER == 8
    if (!cfg->llvm_only && strcmp(cmethod->name, "Read") == 0 &&
        fsig->param_count == 1 && (fsig->params[0]->type == MONO_TYPE_I8)) {
    ...
    if (strcmp(cmethod->name, "Increment") == 0 && fsig->param_count == 1) {
    ...
    } else if (fsig->param_count == 2 && ((strcmp(cmethod->name, "Add") == 0) ||
      ...
      gboolean is_ref = byref_arg_is_reference(fsig->params[0]);
      gboolean is_float = fsig->params[0]->type == MONO_TYPE_R4 ||
      ...
      gboolean is_float = fsig->params[1]->type == MONO_TYPE_R4 ||
                          fsig->params[1]->type == MONO_TYPE_R8;
...
#if SIZEOF_REGISTER == 8
      else if (is_ref || fsig->params[1]->type == MONO_TYPE_I8 ||
               fsig->params[1]->type == MONO_TYPE_R8 ||
      ...
      /* *success = result; */
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI1_MEMBASE_REG, args[3]->dreg, 0,
                                  ceq->dreg);
    ...
    if (!cfg->llvm_only && !strcmp(cmethod->name, "Read") &&
        fsig->param_count == 1) {
      guint32 opcode = 0;
      ...
      if (opcode) {
        ...
        MONO_INST_NEW(cfg, ins, opcode);
        ins->dreg =
            is_ref ? mono_alloc_ireg_ref(cfg)
                   : (is_float ? mono_alloc_freg(cfg) : mono_alloc_ireg(cfg));
        ins->sreg1 = args[0]->dreg;
    ...
    if (!cfg->llvm_only && !strcmp(cmethod->name, "Write") &&
        fsig->param_count == 2) {
      guint32 opcode = 0;
      MonoType *t = fsig->params[0];
      gboolean is_ref;
  ...
  } else if (in_corlib && (strcmp(cmethod_klass_name_space, "System") == 0) &&
             (strcmp(cmethod_klass_name, "Environment") == 0)) {
    if (!strcmp(cmethod->name, "get_IsRunningOnWindows") &&
        fsig->param_count == 0) {
#ifdef TARGET_WIN32
      EMIT_NEW_ICONST(cfg, ins, 1);
#else
    ...
    if (cfg->llvm_only && !strcmp(cmethod->name, "GetExecutingAssembly")) {
      /* No stack walks are currently available, so implement this as an
       * intrinsic */
      MonoInst *assembly_ins;

      EMIT_NEW_AOTCONST(cfg, assembly_ins, MONO_PATCH_INFO_IMAGE,
                        m_class_get_image(cfg->method->klass));
    ...
    // have GetCallingAssembly be consistent independently of varying
    // optimization. This fixes mono/tests/test-inline-call-stack.cs under
    // FullAOT+LLVM.
    cfg->no_inline |=
        COMPILE_LLVM(cfg) && strcmp(cmethod->name, "GetCallingAssembly") == 0;
    ...
    if (cfg->llvm_only && !strcmp(cmethod->name, "GetCurrentMethod")) {
      ...
      EMIT_NEW_AOTCONST(cfg, method_ins, MONO_PATCH_INFO_METHODCONST,
                        declaring);
  ...
  } else if (cmethod->klass == mono_defaults.systemtype_class &&
             !strcmp(cmethod->name, "op_Equality")) {
    EMIT_NEW_BIALU(cfg, ins, OP_COMPARE, -1, args[0]->dreg, args[1]->dreg);
    ...
    return ins;
               ...
               !strcmp(cmethod_klass_image->assembly->aname.name,
                       "monotouch")) &&
    ...
    if ((cfg->backend->have_objc_get_selector || cfg->compile_llvm) &&
      ...
      s = mono_ldstr_utf8(ji->image, mono_metadata_token_index(ji->token),
                          cfg->error);
      ...
      return ins;
             ...
             (strcmp(cmethod_klass_name_space,
    ...
    if (strcmp(cmethod->name, "PtrToStructure") == 0 && cmethod->is_inflated &&
        fsig->param_count == 1 &&
  ...
  } else if (cmethod->klass == mono_defaults.enum_class &&
             !strcmp(cmethod->name, "HasFlag") && args[0]->opcode == OP_BOX &&
             args[1]->opcode == OP_BOX_ICONST &&
             args[0]->klass == args[1]->klass) {
    args[1]->opcode = OP_ICONST;
    ins = mini_handle_enum_has_flag(cfg, args[0]->klass, NULL, args[0]->sreg1,
                                    args[1]);
    NULLIFY_INS(args[0]);
    return ins;
  } else if (in_corlib && !strcmp(cmethod_klass_name_space, "System") &&
             (!strcmp(cmethod_klass_name, "Span`1") ||
              !strcmp(cmethod_klass_name, "ReadOnlySpan`1"))) {
  ...
  } else if (!strcmp(cmethod_klass_name_space,
                     "System.Runtime.CompilerServices") &&
             !strcmp(cmethod_klass_name, "Unsafe") &&
             (in_corlib || !strcmp(cmethod_klass_image->assembly->aname.name,
    ...
    MonoGenericContext *method_context = mono_method_get_context(cmethod);
    if (!strcmp(cmethod->name, "CreateInstance") && fsig->param_count == 0 &&
        method_context != NULL && method_context->method_inst->type_argc == 1 &&
        cmethod->is_inflated && !mini_method_check_context_used(cfg, cmethod)) {
      MonoClass *arg0 = mono_class_from_mono_type_internal(
          method_context->method_inst->type_argv[0]);
      if (m_class_is_valuetype(arg0) &&
          !mono_class_has_default_constructor(arg0, FALSE)) {
        MONO_INST_NEW(cfg, ins,
                      MONO_CLASS_IS_SIMD(cfg, arg0) ? OP_XZERO : OP_VZERO);
  ...
  if (in_corlib && !strcmp("System.Numerics", cmethod_klass_name_space) &&
  ...
  if (!strcmp("Microsoft.CodeAnalysis.CompilerServer",
              cmethod_klass_name_space) &&
  ...
  // Return false for RuntimeFeature.IsDynamicCodeSupported and
  // RuntimeFeature.IsDynamicCodeCompiled on FullAOT, otherwise true
  if (in_corlib &&
      !strcmp("System.Runtime.CompilerServices", cmethod_klass_name_space) &&
      !strcmp("RuntimeFeature", cmethod_klass_name)) {
    if (!strcmp(cmethod->name, "get_IsDynamicCodeSupported") ||
        !strcmp(cmethod->name, "get_IsDynamicCodeCompiled")) {
...
static MonoInst *emit_array_unsafe_access(MonoCompile *cfg,
                                          MonoMethodSignature *fsig,
                                          MonoInst **args, int is_set) {
  MonoClass *eklass;
  ...
  if (is_set) {
    ...
    MonoInst *ins,
        *addr = mini_emit_ldelema_1_ins(cfg, eklass, args[0], args[1], FALSE);
    EMIT_NEW_LOAD_MEMBASE_TYPE(cfg, ins, m_class_get_byval_arg(eklass),
                               addr->dreg, 0);
    return ins;
  }
}

static gboolean is_unsafe_mov_compatible(MonoCompile *cfg,
                                         MonoClass *param_klass,
                                         MonoClass *return_klass) {
  uint32_t align;
  int param_size, return_size;

  param_klass = mono_class_from_mono_type_internal(
      mini_get_underlying_type(m_class_get_byval_arg(param_klass)));
  return_klass = mono_class_from_mono_type_internal(
      mini_get_underlying_type(m_class_get_byval_arg(return_klass)));
  ...
  if (m_class_is_valuetype(param_klass) != m_class_is_valuetype(return_klass)) {
    if (cfg->verbose_level > 3)
      printf("[UNSAFE-MOV-INTRISIC]\tone of the args is a valuetype and the "
             "other is not\n");
    return FALSE;
  }
  ...
  // That are blitable
  if (m_class_has_references(param_klass) ||
  ...
  /* Avoid mixing structs and primitive types/enums, they need to be handled
   * differently in the JIT */
   ...
   *
   * Since the source value comes from a function argument, the JIT will already
   * have the value in a VREG and performed any widening needed before (say,
   * when loading from a field).
   */
  if (param_size <= 4 && return_size <= 4) {
    if (cfg->verbose_level > 3)
      printf("[UNSAFE-MOV-INTRISIC]\tsize mismatch but both are of the same "
             "reg class\n");
    return TRUE;
  }
...
static MonoInst *emit_array_unsafe_mov(MonoCompile *cfg,
                                       MonoMethodSignature *fsig,
                                       MonoInst **args) {
  ...
  // Valuetypes that are semantically equivalent or numbers than can be widened
  // to
  if (is_unsafe_mov_compatible(cfg, param_klass, return_klass))
    return args[0];

  // Arrays of valuetypes that are semantically equivalent
  if (m_class_get_rank(param_klass) == 1 &&
      m_class_get_rank(return_klass) == 1 &&
      is_unsafe_mov_compatible(cfg, m_class_get_element_class(param_klass),
...
MonoInst *mini_emit_inst_for_field_load(MonoCompile *cfg,
  ...
  if (in_corlib && !strcmp(klass_name_space, "System") &&
      !strcmp(klass_name, "BitConverter") &&
      !strcmp(field->name, "IsLittleEndian")) {
