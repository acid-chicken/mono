...
#error                                                                         \
    "MONO_ARCH_USE_SHARED_FP_SIMD_BANK needs MONO_ARCH_NEED_SIMD_BANK to work"
#endif

#define get_mirrored_bank(bank)                                                \
  (((bank) == MONO_REG_SIMD)                                                   \
       ? MONO_REG_DOUBLE                                                       \
       : (((bank) == MONO_REG_DOUBLE) ? MONO_REG_SIMD : -1))

#define is_hreg_mirrored(rs, bank, hreg)                                       \
  ((rs)->symbolic[(bank)][(hreg)] == MONO_ARCH_BANK_MIRRORED)
...
static const int regbank_size[] = {MONO_MAX_IREGS, MONO_MAX_FREGS,
                                   MONO_MAX_IREGS, MONO_MAX_IREGS,
                                   MONO_MAX_XREGS};

static const int regbank_load_ops[] = {OP_LOADR_MEMBASE, OP_LOADR8_MEMBASE,
                                       OP_LOADR_MEMBASE, OP_LOADR_MEMBASE,
                                       OP_LOADX_MEMBASE};

static const int regbank_store_ops[] = {
    OP_STORER_MEMBASE_REG, OP_STORER8_MEMBASE_REG, OP_STORER_MEMBASE_REG,
    OP_STORER_MEMBASE_REG, OP_STOREX_MEMBASE};

static const int regbank_move_ops[] = {OP_MOVE, OP_FMOVE, OP_MOVE, OP_MOVE,
                                       OP_XMOVE};
...
static const regmask_t regbank_callee_saved_regs[] = {
    MONO_ARCH_CALLEE_SAVED_REGS,  MONO_ARCH_CALLEE_SAVED_FREGS,
    MONO_ARCH_CALLEE_SAVED_REGS,  MONO_ARCH_CALLEE_SAVED_REGS,
    MONO_ARCH_CALLEE_SAVED_XREGS,
};
#endif

static const regmask_t regbank_callee_regs[] = {
    MONO_ARCH_CALLEE_REGS, MONO_ARCH_CALLEE_FREGS, MONO_ARCH_CALLEE_REGS,
    MONO_ARCH_CALLEE_REGS, MONO_ARCH_CALLEE_XREGS,
};

static const int regbank_spill_var_size[] = {
    sizeof(target_mgreg_t), sizeof(double), sizeof(target_mgreg_t),
...
static void mono_regstate_assign(MonoRegState *rs) {
#ifdef MONO_ARCH_USE_SHARED_FP_SIMD_BANK
  /* The regalloc may fail if fp and simd logical regbanks share the same
   * physical reg bank and if the values here are not the same.
   */
  g_assert(regbank_callee_regs[MONO_REG_SIMD] ==
           regbank_callee_regs[MONO_REG_DOUBLE]);
  g_assert(regbank_callee_saved_regs[MONO_REG_SIMD] ==
           regbank_callee_saved_regs[MONO_REG_DOUBLE]);
...
static int mono_regstate_alloc_int(MonoRegState *rs, regmask_t allow) {
...
static void mono_regstate_free_int(MonoRegState *rs, int reg) {
  if (reg >= 0) {
...
static int mono_regstate_alloc_general(MonoRegState *rs, regmask_t allow,
                                       int bank) {
  int i;
  int mirrored_bank;
...
static void mono_regstate_free_general(MonoRegState *rs, int reg, int bank) {
  int mirrored_bank;
...
void mono_call_inst_add_outarg_reg(MonoCompile *cfg, MonoCallInst *call,
  ...
  if (G_UNLIKELY(bank)) {
    ...
    call->out_freg_args = g_slist_append_mempool(
        cfg->mempool, call->out_freg_args, (gpointer)(gssize)(regpair));
  } else {
    g_assert(vreg >= MONO_MAX_IREGS);
    g_assert(hreg < MONO_MAX_IREGS);
    call->used_iregs |= (regmask_t)1 << hreg;
    call->out_ireg_args = g_slist_append_mempool(
        cfg->mempool, call->out_ireg_args, (gpointer)(gssize)(regpair));
  }
}
...
void mono_call_inst_add_outarg_vt(MonoCompile *cfg, MonoCallInst *call,
                                  MonoInst *outarg_vt) {
  call->outarg_vts =
      g_slist_append_mempool(cfg->mempool, call->outarg_vts, outarg_vt);
}

static void resize_spill_info(MonoCompile *cfg, int bank) {
  ...
  new_info = (MonoSpillInfo *)mono_mempool_alloc0(
      cfg->mempool, sizeof(MonoSpillInfo) * new_len);
  if (orig_info)
 ...
 */
static int mono_spillvar_offset(MonoCompile *cfg, int spillvar, int bank) {
  MonoSpillInfo *info;
  int size;
  ...
  /*
   * Allocate separate spill slots for fp/non-fp variables since most processors
   * prefer it.
   */
...
#define is_global_ireg(r)                                                      \
  (is_hard_ireg((r)) && (MONO_ARCH_CALLEE_SAVED_REGS & (regmask(r))))
#define is_local_ireg(r)                                                       \
  (is_hard_ireg((r)) && (MONO_ARCH_CALLEE_REGS & (regmask(r))))
#define is_global_freg(r)                                                      \
  (is_hard_freg((r)) && (MONO_ARCH_CALLEE_SAVED_FREGS & (regmask(r))))
#define is_local_freg(r)                                                       \
  (is_hard_freg((r)) && (MONO_ARCH_CALLEE_FREGS & (regmask(r))))

#define is_hard_reg(r, bank)                                                   \
  (G_UNLIKELY(bank) ? ((r) >= 0 && (r) < regbank_size[bank])                   \
                    : ((r) < MONO_MAX_IREGS))
#define is_soft_reg(r, bank) (!is_hard_reg((r), (bank)))
#define is_global_reg(r, bank)                                                 \
  (G_UNLIKELY(bank) ? (is_hard_reg((r), (bank)) &&                             \
                       (regbank_callee_saved_regs[bank] & regmask(r)))         \
                    : is_global_ireg(r))
#define is_local_reg(r, bank)                                                  \
  (G_UNLIKELY(bank) ? (is_hard_reg((r), (bank)) &&                             \
                       (regbank_callee_regs[bank] & regmask(r)))               \
                    : is_local_ireg(r))
#define reg_is_freeable(r, bank)                                               \
  (G_UNLIKELY(bank) ? is_local_reg((r), (bank)) : is_local_ireg((r)))
...
#define sreg_is_fp(n, spec)                                                    \
  (MONO_ARCH_INST_IS_FLOAT(spec[MONO_INST_SRC1 + (n)]))
...
#define reg_bank(desc)                                                         \
  (G_UNLIKELY(reg_is_fp(desc))                                                 \
       ? MONO_REG_DOUBLE                                                       \
       : G_UNLIKELY(reg_is_simd(desc)) ? MONO_REG_SIMD : MONO_REG_INT)
...
#define regpair_reg2_mask(desc, hreg1)                                         \
  ((MONO_ARCH_INST_REGPAIR_REG2(desc, hreg1) != -1)                            \
       ? (regmask(MONO_ARCH_INST_REGPAIR_REG2(desc, hreg1)))                   \
...
typedef struct {
  ...
  // int last_use;
  // int prev_use;
  regmask_t preferred_mask; /* the hreg where the register should be allocated,
                               or 0 */
} RegTrack;
...
GString *mono_print_ins_index_strbuf(int i, MonoInst *ins) {
  ...
  if (spec == (gpointer) /*FIXME*/ MONO_ARCH_CPU_SPEC) {
    ...
    if (ins->dreg != -1) {
      if (dest_base)
        g_string_append_printf(sbuf, " [R%d + 0x%lx] <-", ins->dreg,
                               (long)ins->inst_offset);
      else
    ...
    case OP_LBLE_UN:
      if (!ins->inst_false_bb)
        ...
        g_string_append_printf(sbuf, " [B%dB%d]", ins->inst_true_bb->block_num,
  ...
  if (spec[MONO_INST_DEST]) {
    ...
    if (is_soft_reg(ins->dreg, bank)) {
      ...
      if (ins->inst_offset == 0)
        ...
        g_string_append_printf(sbuf, " [%s + 0x%lx] <-",
                               mono_arch_regname(ins->dreg),
                               (long)ins->inst_offset);
    } else
      g_string_append_printf(sbuf, " %s <-",
  ...
  if (spec[MONO_INST_SRC1]) {
    ...
    if (is_soft_reg(ins->sreg1, bank)) {
      if (spec[MONO_INST_SRC1] == 'b')
        g_string_append_printf(sbuf, " [R%d + 0x%lx]", ins->sreg1,
                               (long)ins->inst_offset);
      else
        g_string_append_printf(sbuf, " R%d", ins->sreg1);
  ...
  case OP_RCALL_MEMBASE: {
    ...
    if (ins->opcode == OP_VCALL || ins->opcode == OP_VCALL_REG ||
        ins->opcode == OP_VCALL_MEMBASE) {
      /*
    ...
    if ((method = call->method)) {
      ...
      g_string_append_printf(sbuf, " [%s]",
                             mono_find_jit_icall_info(jit_icall_id)->name);
    }
    ...
    while (list) {
      ...
      g_string_append_printf(sbuf, " [%s <- R%d]", mono_arch_regname(hreg),
                             reg);
    ...
    while (list) {
      ...
      g_string_append_printf(sbuf, " [%s <- R%d]", mono_arch_fregname(hreg),
                             reg);
  ...
  case OP_SEQ_POINT:
    g_string_append_printf(
        sbuf, " il: 0x%x%s", (int)ins->inst_imm,
        ins->flags & MONO_INST_NONEMPTY_STACK ? ", nonempty-stack" : "");
    break;
  case OP_COND_EXC_EQ:
  case OP_COND_EXC_GE:
  case OP_COND_EXC_GT:
...
static void print_regtrack(RegTrack *t, int num) {
  int i;
  char buf[32];
  const char *r;
...
void mono_print_ins(MonoInst *ins) { mono_print_ins_index(-1, ins); }

static void insert_before_ins(MonoBasicBlock *bb, MonoInst *ins,
                              MonoInst *to_insert) {
  /*
   * If this function is called multiple times, the new instructions are
   * inserted in the proper order.
   */
  mono_bblock_insert_before_ins(bb, ins, to_insert);
}

static void insert_after_ins(MonoBasicBlock *bb, MonoInst *ins, MonoInst **last,
                             MonoInst *to_insert) {
  /*
   * If this function is called multiple times, the new instructions are
...
static int get_vreg_bank(MonoCompile *cfg, int reg, int bank) {
 ...
 */
static void spill_vreg(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst **last,
                       MonoInst *ins, int reg, int bank) {
  MonoInst *load;
  int i, sel, spill;
  ...
  /* we need to create a spill var and insert a load to sel after the current
   * instruction */
  MONO_INST_NEW(cfg, load, regbank_load_ops[bank]);
  load->dreg = sel;
  load->inst_basereg = cfg->frame_reg;
  load->inst_offset =
      mono_spillvar_offset(cfg, spill, get_vreg_bank(cfg, reg, bank));
  insert_after_ins(bb, ins, last, load);
  DEBUG(printf("SPILLED LOAD (%d at 0x%08lx(%%ebp)) R%d (freed %s)\n", spill,
               (long)load->inst_offset, i, mono_regname_full(sel, bank)));
  if (G_UNLIKELY(bank))
...
static int get_register_spilling(MonoCompile *cfg, MonoBasicBlock *bb,
                                 MonoInst **last, MonoInst *ins,
                                 regmask_t regmask, int reg, int bank) {
  MonoInst *load;
  ...
  DEBUG(printf(
      "\tstart regmask to assign R%d: 0x%08" PRIu64 " (R%d <- R%d R%d R%d)\n",
      reg, (guint64)regmask, ins->dreg, ins->sreg1, ins->sreg2, ins->sreg3));
  /* exclude the registers in the current instruction */
  num_sregs = mono_inst_get_src_registers(ins, sregs);
  for (i = 0; i < num_sregs; ++i) {
    if ((sreg_bank_ins(i, ins) == bank) && (reg != sregs[i]) &&
        (reg_is_freeable(sregs[i], bank) ||
         (is_soft_reg(sregs[i], bank) && rs->vassign[sregs[i]] >= 0))) {
      if (is_soft_reg(sregs[i], bank))
        regmask &= ~(regmask(rs->vassign[sregs[i]]));
      else
        regmask &= ~(regmask(sregs[i]));
      DEBUG(printf("\t\texcluding sreg%d %s %d\n", i + 1,
                   mono_regname_full(sregs[i], bank), sregs[i]));
    }
  }
  if ((dreg_bank_ins(ins) == bank) && (reg != ins->dreg) &&
      reg_is_freeable(ins->dreg, bank)) {
  ...
  load->inst_basereg = cfg->frame_reg;
  load->inst_offset =
      mono_spillvar_offset(cfg, spill, get_vreg_bank(cfg, i, bank));
  insert_after_ins(bb, ins, last, load);
  DEBUG(printf("\tSPILLED LOAD (%d at 0x%08lx(%%ebp)) R%d (freed %s)\n", spill,
               (long)load->inst_offset, i, mono_regname_full(sel, bank)));
  if (G_UNLIKELY(bank))
 ...
 */
static void free_up_hreg(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst **last,
                         MonoInst *ins, int hreg, int bank) {
  if (G_UNLIKELY(bank)) {
...
static MonoInst *create_copy_ins(MonoCompile *cfg, MonoBasicBlock *bb,
                                 MonoInst **last, int dest, int src,
                                 MonoInst *ins, const unsigned char *ip,
                                 int bank) {
  MonoInst *copy;
  ...
  DEBUG(printf("\tforced copy from %s to %s\n", mono_regname_full(src, bank),
               mono_regname_full(dest, bank)));
  return copy;
}

static const char *regbank_to_string(int bank) {
...
static void create_spilled_store(MonoCompile *cfg, MonoBasicBlock *bb,
                                 int spill, int reg, int prev_reg,
                                 MonoInst **last, MonoInst *ins,
                                 MonoInst *insert_before, int bank) {
  MonoInst *store, *def;
  ...
  DEBUG(printf("\t%sSPILLED STORE (%d at 0x%08lx(%%ebp)) R%d (from %s)\n",
               regbank_to_string(bank), spill, (long)store->inst_offset,
               prev_reg, mono_regname_full(reg, bank)));
...
static int alloc_int_reg(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst **last,
                         MonoInst *ins, regmask_t dest_mask, int sym_reg,
                         RegTrack *info) {
  int val;
...
static int alloc_general_reg(MonoCompile *cfg, MonoBasicBlock *bb,
                             MonoInst **last, MonoInst *ins,
                             regmask_t dest_mask, int sym_reg, int bank) {
  int val;
...
static int alloc_reg(MonoCompile *cfg, MonoBasicBlock *bb, MonoInst **last,
                     MonoInst *ins, regmask_t dest_mask, int sym_reg,
                     RegTrack *info, int bank) {
  if (G_UNLIKELY(bank))
...
static void assign_reg(MonoCompile *cfg, MonoRegState *rs, int reg, int hreg,
                       int bank) {
  if (G_UNLIKELY(bank)) {
    int mirrored_bank;
...
static regmask_t get_callee_mask(const char spec) {
 ...
 * We also reverse the list of instructions because assigning registers
 * backwards allows for more tricks to be used.
 */
void mono_local_regalloc(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  if (!desc_to_fixed_reg_inited) {
    ...
    defined(TARGET_ARM64) || defined(TARGET_RISCV)
  ...
  if (!reginfo) {
    ...
    reginfo = (RegTrack *)mono_mempool_alloc(
        cfg->mempool, sizeof(RegTrack) * cfg->reginfo_len);
    cfg->reginfo = reginfo;
  } else
  ...
  MONO_BB_FOR_EACH_INS(bb, ins) {
    ...
    if (G_UNLIKELY(spec == (gpointer) /*FIXME*/ MONO_ARCH_CPU_SPEC)) {
      g_error("Opcode '%s' missing from machine description file.",
              mono_inst_name(ins->opcode));
    }
    ...
    for (j = 0; j < num_sregs; ++j) {
      ...
      if (sreg_spec) {
        ...
        if (MONO_ARCH_INST_IS_REGPAIR(sreg_spec)) {
          ...
          rs->vassign[sreg + 1] = -1;
          // reginfo [ins->sreg2 + 1].prev_use = reginfo [ins->sreg2 +
          // 1].last_use; reginfo [ins->sreg2 + 1].last_use = i;
    ...
    if (spec_dest) {
      ...
      if (MONO_ARCH_INST_IS_REGPAIR(spec_dest)) {
        ...
        if (reginfo[ins->dreg + 1].born_in == 0 ||
            reginfo[ins->dreg + 1].born_in > i)
          reginfo[ins->dreg + 1].born_in = i;
        if (MONO_ARCH_INST_REGPAIR_REG2(spec_dest, -1) != -1)
          reginfo[ins->dreg + 1].preferred_mask =
              regpair_reg2_mask(spec_dest, -1);
      }
    } else {
      ins->dreg = -1;
    }
  ...
  MONO_BB_FOR_EACH_INS_REVERSE_SAFE(bb, prev, ins) {
    ...
    regmask_t sreg_masks[MONO_MAX_SRC_REGS],
        sreg_fixed_masks[MONO_MAX_SRC_REGS];
    ...
    for (j = 1; j < num_sregs; ++j) {
      ...
      if (j == 2) {
        ...
        /* Make sure sreg will be assigned to dest_sreg, and the other sregs
         * won't */
         ...
         */
        for (k = 0; k < num_sregs; ++k) {
          if (k != j && is_soft_reg(sregs[k], 0) &&
              rs->vassign[sregs[k]] == dest_sreg)
         ...
         */
        if (is_soft_reg(sreg, 0) && rs->vassign[sreg] >= 0 &&
            rs->vassign[sreg] != dest_sreg) {
      ...
      if (rs->ifree_mask & (regmask(dest_sreg))) {
        if (is_global_ireg(sreg)) {
          ...
          MonoInst *copy =
              create_copy_ins(cfg, bb, tmp, dest_sreg, sreg, NULL, ip, 0);
          ...
          if (val == -1) {
            ...
            /* Argument already in hard reg, need to copy */
            MonoInst *copy =
                create_copy_ins(cfg, bb, tmp, dest_sreg, val, NULL, ip, 0);
            int k;
        ...
        if (val == dest_sreg && ins->dreg != sreg) {
           ...
           */
          int new_dest;
          new_dest = alloc_int_reg(cfg, bb, tmp, ins, dreg_mask, ins->dreg,
                                   &reginfo[ins->dreg]);
          g_assert(new_dest >= 0);
        ...
        if (need_assign) {
          if (rs->vassign[sreg] < -1) {
            ...
            create_spilled_store(cfg, bb, spill, dest_sreg, sreg, tmp, NULL,
                                 ins, bank);
          }
          /* force-set sreg2 */
    ...
    if (spec_dest == 'b') {
      ...
      val = rs->vassign[ins->dreg];
      if (is_soft_reg(ins->dreg, bank) && (val >= 0) &&
          (!(regmask(val) & dreg_mask))) {
    ...
    if (dreg_fixed_mask) {
      ...
      if (is_global_ireg(ins->dreg)) {
        ...
        if (val < 0)
          val = get_register_spilling(cfg, bb, tmp, ins, dreg_fixed_mask, -1,
                                      bank);
    ...
    if (is_soft_reg(ins->dreg, bank)) {
      ...
      if (val < 0) {
        ...
        val = alloc_reg(cfg, bb, tmp, ins, dreg_mask, ins->dreg,
                        &reginfo[ins->dreg], bank);
        assign_reg(cfg, rs, ins->dreg, val, bank);
        if (spill)
          create_spilled_store(cfg, bb, spill, val, prev_dreg, tmp, ins, NULL,
                               bank);
      }
    ...
    if (MONO_ARCH_INST_IS_REGPAIR(spec_dest)) {
      ...
      if (reg_is_freeable(val, bank) && reg2 >= 0 &&
          (reginfo[reg2].born_in >= i)) {
    ...
    if (prev_dreg >= 0 && is_soft_reg(prev_dreg, bank) && (spec_dest != 'b')) {
      ...
      DEBUG(printf("\tfreeable %s (R%d) (born in %d)\n",
                   mono_regname_full(dreg, bank), prev_dreg,
                   reginfo[prev_dreg].born_in));
      if (G_UNLIKELY(bank))
    ...
    if ((dest_dreg != -1) && (ins->dreg != dest_dreg)) {
      ...
      if (G_UNLIKELY(bank)) {
        /* the register we need to free up may be used in another logical
         * regbank so do a translate just in case.
         */
        int translated_bank = translate_bank(cfg->rs, bank, dest_dreg);
        if (rs->symbolic[translated_bank][dest_dreg] >=
            regbank_size[translated_bank])
    ...
    if (sreg_fixed_masks[0]) {
      ...
      if (is_global_ireg(sregs[0])) {
        ...
        if (val < 0)
          val = get_register_spilling(cfg, bb, tmp, ins, sreg_fixed_masks[0],
                                      -1, bank);
        mono_regstate_free_int(rs, val);
        dest_sregs[0] = val;
    ...
    if (dest_sregs[0] != -1) {
      ...
      if ((rs->vassign[sregs[0]] != dest_sregs[0]) &&
          !(rs->ifree_mask & (regmask(dest_sregs[0])))) {
        free_up_hreg(cfg, bb, tmp, ins, dest_sregs[0], 0);
      }
      if (is_global_ireg(sregs[0])) {
        /* The argument is already in a hard reg, need to copy */
        MonoInst *copy =
            create_copy_ins(cfg, bb, tmp, dest_sregs[0], sregs[0], NULL, ip, 0);
        insert_before_ins(bb, ins, copy);
    ...
    if (is_soft_reg(sregs[0], bank)) {
      ...
      if (val < 0) {
        ...
        if ((ins->opcode == OP_MOVE) && !spill && !bank &&
            is_local_ireg(ins->dreg) &&
        ...
        if (spec[MONO_INST_CLOB] == '1' && !dreg_bank(spec) &&
            (rs->ifree_mask & (regmask(ins->dreg))))
          /* Allocate the same reg to sreg1 to avoid a copy later */
          sreg_masks[0] = regmask(ins->dreg);

        val = alloc_reg(cfg, bb, tmp, ins, sreg_masks[0], sregs[0],
                        &reginfo[sregs[0]], bank);
        ...
        if (spill) {
           ...
           * overwrite sreg1.
           */
          create_spilled_store(cfg, bb, spill, val, prev_sregs[0], tmp, NULL,
                               ins, bank);
        }
      } else if ((dest_sregs[0] != -1) && (dest_sregs[0] != val)) {
        MonoInst *copy =
            create_copy_ins(cfg, bb, tmp, dest_sregs[0], val, NULL, ip, bank);
        insert_before_ins(bb, ins, copy);
        for (j = 1; j < num_sregs; ++j)
    ...
    /* Handle dreg==sreg1 */
    if (((dreg_is_fp(spec) && sreg1_is_fp(spec)) ||
         spec[MONO_INST_CLOB] == '1') &&
        ins->dreg != sregs[0]) {
      ...
      if (ins->dreg == sregs[1]) {
         ...
         */
        int reg2 =
            alloc_reg(cfg, bb, tmp, ins, dreg_mask, sregs[1], NULL, bank);

        DEBUG(printf("\tneed to copy sreg2 %s to reg %s\n",
                     mono_regname_full(sregs[1], bank),
                     mono_regname_full(reg2, bank)));
        sreg2_copy =
            create_copy_ins(cfg, bb, tmp, reg2, sregs[1], NULL, ip, bank);
      ...
      DEBUG(printf("\tneed to copy sreg1 %s to dreg %s\n",
                   mono_regname_full(sregs[0], bank),
                   mono_regname_full(ins->dreg, bank)));
    ...
    for (j = 1; j < num_sregs; ++j) {
      ...
      if (dest_sregs[j] != -1 && is_global_ireg(sregs[j])) {
        /*
         * Argument already in a global hard reg, copy it to the fixed reg,
         * without allocating it to the fixed reg.
         */
        MonoInst *copy =
            create_copy_ins(cfg, bb, tmp, dest_sregs[j], sregs[j], NULL, ip, 0);
        insert_before_ins(bb, ins, copy);
        ...
        if (val < 0) {
          ...
          val = alloc_reg(cfg, bb, tmp, ins, sreg_masks[j], sregs[j],
                          &reginfo[sregs[j]], bank);
          assign_reg(cfg, rs, sregs[j], val, bank);
          DEBUG(printf("\tassigned sreg%d %s to R%d\n", j + 1,
                       mono_regname_full(val, bank), sregs[j]));
          if (spill) {
            /*
             * Need to insert before the instruction since it can
             * overwrite sreg2.
             */
            create_spilled_store(cfg, bb, spill, val, sregs[j], tmp, NULL, ins,
                                 bank);
          }
        }
        sregs[j] = val;
    ...
    /*if (reg_is_freeable (ins->sreg1) && prev_sreg1 >= 0 && reginfo
    [prev_sreg1].born_in >= i) { DEBUG (printf ("freeable %s\n",
    mono_arch_regname (ins->sreg1))); mono_regstate_free_int (rs, ins->sreg1);
    }
    if (reg_is_freeable (ins->sreg2) && prev_sreg2 >= 0 && reginfo
    [prev_sreg2].born_in >= i) { DEBUG (printf ("freeable %s\n",
    mono_arch_regname (ins->sreg2))); mono_regstate_free_int (rs, ins->sreg2);
    }*/
  ...
  if (has_fp) {
    ...
    for (ins = bb->code; ins; ins = ins->next) {
      ...
      if (sreg1_is_fp(spec) && sreg2_is_fp(spec) &&
          (fpstack[sp - 2] != ins->sreg1)) {
    ...
    if (sp && bb != cfg->bb_exit &&
        !(bb->out_count == 1 && bb->out_bb[0] == cfg->bb_exit)) {
...
void mono_peephole_ins(MonoBasicBlock *bb, MonoInst *ins) {
  ...
  case OP_LOADU1_MEMBASE:
    ...
    if (last_ins && (last_ins->opcode == OP_STOREI1_MEMBASE_REG) &&
        ...
        ins->inst_offset == last_ins->inst_offset) {
      ins->opcode =
          (ins->opcode == OP_LOADI1_MEMBASE) ? OP_PCONV_TO_I1 : OP_PCONV_TO_U1;
      ins->sreg1 = last_ins->sreg1;
    }
    break;
  ...
  case OP_LOADU2_MEMBASE:
    ...
    if (last_ins && (last_ins->opcode == OP_STOREI2_MEMBASE_REG) &&
      ...
      ins->opcode =
          (ins->opcode == OP_LOADI2_MEMBASE) ? OP_PCONV_TO_I2 : OP_PCONV_TO_U2;
#else
      /* The definition of OP_PCONV_TO_U2 is wrong */
      ins->opcode =
          (ins->opcode == OP_LOADI2_MEMBASE) ? OP_PCONV_TO_I2 : OP_ICONV_TO_U2;
#endif
      ins->sreg1 = last_ins->sreg1;
    }
    break;
