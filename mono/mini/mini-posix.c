...
gboolean MONO_SIG_HANDLER_SIGNATURE(mono_chain_signal) { return FALSE; }
...
void mono_runtime_shutdown_handlers(void) {}
...
static struct sigaction *get_saved_signal_handler(int signo, gboolean remove) {
  if (mono_saved_signal_handlers) {
    /* The hash is only modified during startup, so no need for locking */
    struct sigaction *handler = (struct sigaction *)g_hash_table_lookup(
        mono_saved_signal_handlers, GINT_TO_POINTER(signo));
    if (remove && handler)
...
static void save_old_signal_handler(int signo, struct sigaction *old_action) {
  struct sigaction *handler_to_save =
      (struct sigaction *)g_malloc(sizeof(struct sigaction));
  ...
  g_hash_table_insert(mono_saved_signal_handlers, GINT_TO_POINTER(signo),
                      handler_to_save);
}

static void free_saved_signal_handlers(void) {
 ...
 * which should be the same as for a signal handler. Returns TRUE if the
 * original handler was called, false otherwise.
 */
gboolean MONO_SIG_HANDLER_SIGNATURE(mono_chain_signal) {
  int signal = MONO_SIG_HANDLER_GET_SIGNO();
  struct sigaction *saved_handler =
      (struct sigaction *)get_saved_signal_handler(signal, FALSE);
...
MONO_SIG_HANDLER_FUNC(static, sigterm_signal_handler) {
  ...
  if (mono_dump_start()) {
    ...
    // Crash reporting already running and we got a second SIGTERM from as part
    // of thread-summarizing
    if (!mono_threads_summarize_execute(&mctx, &output, &hashes, FALSE, NULL,
                                        0))
...
MONO_SIG_HANDLER_FUNC(static, profiler_signal_handler) {
  ...
  if (mono_thread_info_get_small_id() == -1 || !mono_domain_get() ||
  ...
  MONO_PROFILER_RAISE(sample_hit,
                      ((const mono_byte *)mono_arch_ip_from_context(ctx), ctx));
...
static void add_signal_handler(int signo, MonoSignalHandler handler,
                               int flags) {
  struct sigaction sa;
  ...
  if (signo == SIGSEGV) {
    /*
     * Delay abort signals while handling SIGSEGVs since they could go
     * unnoticed.
     */
    sigset_t block_mask;
  ...
  if (!(previous_sa.sa_flags & SA_SIGINFO) &&
    ...
    /* it there is no sa_sigaction function and the sa_handler is default, we
...
static void remove_signal_handler(int signo) {
  struct sigaction sa;
...
static void clock_init(MonoProfilerSampleMode mode) {
  kern_return_t ret;

  do {
    ret = host_get_clock_service(mach_host_self(), SYSTEM_CLOCK,
...
static guint64 clock_get_time_ns(void) {
  kern_return_t ret;
...
static void clock_sleep_ns_abs(guint64 ns_abs) {
  kern_return_t ret;
  ...
  do {
    ret = clock_sleep(sampling_clock_service, TIME_ABSOLUTE, then,
                      &remain_unused);
...
static void clock_init(MonoProfilerSampleMode mode) {
  ...
  case MONO_PROFILER_SAMPLE_MODE_PROCESS: {
     ...
     */
    if (clock_nanosleep(CLOCK_PROCESS_CPUTIME_ID, TIMER_ABSTIME, &ts, NULL) !=
        EINVAL) {
  ...
  case MONO_PROFILER_SAMPLE_MODE_REAL:
...
static gsize sampling_thread_func(gpointer unused) {
  MonoInternalThread *thread = mono_thread_internal_current();

  thread->flags |= MONO_THREAD_FLAG_DONT_MANAGE;

  mono_thread_set_name_constant_ignore_error(thread, "Profiler Sampler",
...
init:
  ...
  for (guint64 sleep = clock_get_time_ns();
       mono_atomic_load_i32(&sampling_thread_running);
    ...
    FOREACH_THREAD_SAFE_EXCLUDE(info, MONO_THREAD_INFO_FLAGS_NO_SAMPLE) {
       ...
       */
      if (profiler_signal != SIGPROF &&
          !mono_atomic_cas_i32(&info->profiler_signal_ack, 0, 1))
        continue;
...
void mono_runtime_setup_stat_profiler(void) {
  ...
  mono_counters_register("Sampling signals sent",
                         MONO_COUNTER_UINT | MONO_COUNTER_PROFILER |
                             MONO_COUNTER_MONOTONIC,
                         &profiler_signals_sent);
  mono_counters_register("Sampling signals received",
                         MONO_COUNTER_UINT | MONO_COUNTER_PROFILER |
                             MONO_COUNTER_MONOTONIC,
                         &profiler_signals_received);
  mono_counters_register("Sampling signals accepted",
                         MONO_COUNTER_UINT | MONO_COUNTER_PROFILER |
                             MONO_COUNTER_MONOTONIC,
                         &profiler_signals_accepted);
  mono_counters_register("Shutdown signals received",
                         MONO_COUNTER_UINT | MONO_COUNTER_PROFILER |
                             MONO_COUNTER_MONOTONIC,
  ...
  ERROR_DECL(error);
  MonoInternalThread *thread = mono_thread_create_internal(
      mono_get_root_domain(), (gpointer)sampling_thread_func, NULL,
...
static void dump_memory_around_ip(MonoContext *mctx) {
  if (!mctx)
    return;
  ...
  if (native_ip) {
    g_async_safe_printf("Memory around native instruction pointer (%p):",
                        native_ip);
...
static void assert_printer_callback(void) {
  mono_dump_native_crash_info("SIGABRT", NULL, NULL);
}

static void dump_native_stacktrace(const char *signal, MonoContext *mctx) {
  mono_memory_barrier();
  static gint32 middle_of_crash = 0x0;
  gint32 double_faulted =
      mono_atomic_cas_i32((gint32 *)&middle_of_crash, 0x1, 0x0);
  mono_memory_write_barrier();

  if (!double_faulted) {
    g_assertion_disable_global(assert_printer_callback);
  } else {
    g_async_safe_printf("\nAn error has occured in the native fault reporting. "
                        "Some diagnostic information will be unavailable.\n");
  ...
  for (int i = 0; i < size; ++i) {
    ...
    gboolean success =
        g_module_address((void *)ip, fname, 256, NULL, sname, 256, NULL);
    if (!success) {
...
#if !defined(HOST_WIN32) && defined(HAVE_SYS_SYSCALL_H) &&                     \
    (defined(SYS_fork) || HAVE_FORK)
  if (!mini_debug_options.no_gdb_backtrace) {
    ...
    if (!double_faulted) {
      ...
      if (!leave) {
        ...
        mono_state_alloc_mem(&merp_mem, mono_state_tmp_file_tag,
                             mono_max_summary_len * sizeof(gchar));

        // Returns success, so leave if !success
        leave = !mono_threads_summarize(passed_ctx, &output, &hashes, FALSE,
                                        TRUE, (gchar *)merp_mem.mem,
                                        mono_max_summary_len);
      }

      if (!leave) {
        // Wait for the other threads to clean up and exit their handlers
        // We can't lock / wait indefinitely, in case one of these threads got
        // stuck somehow while dumping.
    ...
    if (pid > 0) {
      // Allow gdb to attach to the process even if ptrace_scope sysctl variable
      // is set to a value other than 0 (the most permissive ptrace scope). Most
      // modern Linux distributions set the scope to 1 which allows attaching
      // only to direct children of the current process
...
#ifndef DISABLE_CRASH_REPORTING
    if (output) {
      // We've already done our gdb dump and our telemetry steps. Before
      // exiting, see if we can notify any attached debugger instances.
      //
      // At this point we are accepting that the below step might end in a crash
      mini_get_dbg_callbacks()->send_crash(output, &hashes,
                                           0 /* wait # seconds */);
    }
    output = NULL;
    mono_state_free_mem(&merp_mem);
...
void mono_cleanup_native_crash_info(void) {
  g_free(gdb_path);
  g_free(lldb_path);
}

static gboolean native_stack_with_gdb(pid_t crashed_pid, const char **argv,
                                      int commands, char *commands_filename) {
  if (!gdb_path)
    return FALSE;
...
static gboolean native_stack_with_lldb(pid_t crashed_pid, const char **argv,
                                       int commands, char *commands_filename) {
  if (!lldb_path)
    return FALSE;
...
void mono_gdb_render_native_backtraces(pid_t crashed_pid) {
  ...
  g_snprintf(commands_filename, sizeof(commands_filename),
             "/tmp/mono-gdb-commands.%d", crashed_pid);

  // Create this file, overwriting if it already exists
  int commands_handle = g_open(commands_filename, O_TRUNC | O_WRONLY | O_CREAT,
  ...
  return;
  // if (native_stack_with_lldb (crashed_pid, argv, commands_handle,
  // commands_filename)) 	goto exec;
#endif

  if (native_stack_with_gdb(crashed_pid, argv, commands_handle,
                            commands_filename))
    goto exec;
  ...
  g_async_safe_printf("mono_gdb_render_native_backtraces not supported on this "
                      "platform, unable to find gdb or lldb\n");
