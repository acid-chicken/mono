 ...
 */
static pid_t fork_crash_safe(void) {
  pid_t pid;
  /*
...
static void dump_native_stacktrace(const char *signal, MonoContext *mctx) {
  ...
  if (!double_faulted) {
    ...
    if (!leave) {
      ...
      leave =
          !mono_threads_summarize(passed_ctx, &output, &hashes, FALSE, TRUE,
                                  (gchar *)merp_mem.mem, mono_max_summary_len);
    }
  ...
  if (need_to_fork && pid > 0) {
    ...
    // modern Linux distributions set the scope to 1 which allows attaching only
    // to direct children of the current process
