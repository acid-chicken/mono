...
#error                                                                         \
    "ARM_FPU_NONE is defined while one of ARM_FPU_VFP/ARM_FPU_VFP_HARD is defined"
#endif
 ...
 * Basically, r7 is used as a frame pointer and it should point to the saved r7
 * + lr. This is required for debugging/profiling tools to work, but it has some
 * overhead so it should only be turned on in debug builds.
 */
static gboolean iphone_abi = FALSE;

/*
 * The FPU we are generating code for. This is NOT runtime configurable right
 * now, since some things like MONO_ARCH_CALLEE_FREGS still depend on defines.
 */
 ...
 *    in HW or kernel-emulated, requires new tools. I think this is what symbian
 * uses.
 *
...
#define LDR_MASK                                                               \
  ((0xf << ARMCOND_SHIFT) | (3 << 26) | (1 << 22) | (1 << 20) | (15 << 12))
#define LDR_PC_VAL                                                             \
  ((ARMCOND_AL << ARMCOND_SHIFT) | (1 << 26) | (0 << 22) | (1 << 20) |         \
   (15 << 12))
...
static guint8 *emit_aotconst(MonoCompile *cfg, guint8 *code, int dreg,
                             int patch_type, gpointer data);

const char *mono_arch_regname(int reg) {
  static const char *rnames[] = {"arm_r0", "arm_r1", "arm_r2", "arm_r3",
                                 "arm_v1", "arm_v2", "arm_v3", "arm_v4",
                                 "arm_v5", "arm_v6", "arm_v7", "arm_fp",
...
const char *mono_arch_fregname(int reg) {
  static const char *rnames[] = {
      "arm_f0",  "arm_f1",  "arm_f2",  "arm_f3",  "arm_f4",  "arm_f5",
      "arm_f6",  "arm_f7",  "arm_f8",  "arm_f9",  "arm_f10", "arm_f11",
      "arm_f12", "arm_f13", "arm_f14", "arm_f15", "arm_f16", "arm_f17",
      "arm_f18", "arm_f19", "arm_f20", "arm_f21", "arm_f22", "arm_f23",
      "arm_f24", "arm_f25", "arm_f26", "arm_f27", "arm_f28", "arm_f29",
      "arm_f30", "arm_f31"};
  if (reg >= 0 && reg < 32)
    return rnames[reg];
  return "unknown";
...
#ifndef DISABLE_JIT
static guint8 *emit_big_add_temp(guint8 *code, int dreg, int sreg, int imm,
                                 int temp) {
  int imm8, rot_amount;
...
static guint8 *emit_big_add(guint8 *code, int dreg, int sreg, int imm) {
  return emit_big_add_temp(code, dreg, sreg, imm, ARMREG_IP);
}

static guint8 *emit_ldr_imm(guint8 *code, int dreg, int sreg, int imm) {
  if (!arm_is_imm12(imm)) {
    g_assert(dreg != sreg);
...
static guint8 *emit_sub_imm(guint8 *code, int dreg, int sreg, int imm) {
  int imm8, rot_amount;
...
static guint8 *emit_memcpy(guint8 *code, int size, int dreg, int doffset,
                           int sreg, int soffset) {
...
static guint8 *emit_jmp_reg(guint8 *code, int reg) {
  if (thumb_supported)
    ARM_BX(code, reg);
  else
...
static guint8 *emit_call_reg(guint8 *code, int reg) {
  if (v5_supported) {
...
static guint8 *emit_call_seq(MonoCompile *cfg, guint8 *code) {
...
guint8 *mono_arm_patchable_b(guint8 *code, int cond) {
  ARM_B_COND(code, cond, 0);
  return code;
}

guint8 *mono_arm_patchable_bl(guint8 *code, int cond) {
  ARM_BL_COND(code, cond, 0);
  return code;
}

#if defined(__ARM_EABI__) && defined(__linux__) && !defined(HOST_ANDROID) &&   \
...
static guint8 *emit_tls_get(guint8 *code, int dreg, int tls_offset) {
...
static guint8 *emit_tls_set(guint8 *code, int sreg, int tls_offset) {
 ...
 * On arm, this is intermixed with the initialization of other fields of the
 * structure.
 */
static guint8 *emit_save_lmf(MonoCompile *cfg, guint8 *code,
                             gint32 lmf_offset) {
  int i;

  if (mono_arch_have_fast_tls() &&
      mono_tls_get_tls_offset(TLS_KEY_LMF_ADDR) != -1) {
    code = emit_tls_get(code, ARMREG_R0,
                        mono_tls_get_tls_offset(TLS_KEY_LMF_ADDR));
  } else {
    mono_add_patch_info(
        cfg, code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_ID,
  ...
  ARM_STR_IMM(code, ARMREG_R0, ARMREG_R1,
              MONO_STRUCT_OFFSET(MonoLMF, lmf_addr));
  /* new_lmf->previous_lmf = *lmf_addr */
  ARM_LDR_IMM(code, ARMREG_IP, ARMREG_R0,
              MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
  ARM_STR_IMM(code, ARMREG_IP, ARMREG_R1,
              MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
  /* *(lmf_addr) = r1 */
  ARM_STR_IMM(code, ARMREG_R1, ARMREG_R0,
              MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
...
static guint8 *emit_float_args(MonoCompile *cfg, MonoCallInst *inst,
                               guint8 *code, int *max_len, guint *offset) {
  GSList *list;
...
static guint8 *mono_arm_emit_vfp_scratch_save(MonoCompile *cfg, guint8 *code,
                                              int reg) {
  MonoInst *inst;
  ...
  if (IS_HARD_FLOAT) {
    if (!arm_is_fpimm8(inst->inst_offset)) {
      code =
          emit_big_add(code, ARMREG_LR, inst->inst_basereg, inst->inst_offset);
...
static guint8 *mono_arm_emit_vfp_scratch_restore(MonoCompile *cfg, guint8 *code,
                                                 int reg) {
  MonoInst *inst;
 ...
 */
static guint8 *emit_restore_lmf(MonoCompile *cfg, guint8 *code,
                                gint32 lmf_offset) {
  int basereg, offset;
  ...
  /* ip = previous_lmf */
  ARM_LDR_IMM(code, ARMREG_IP, basereg,
              offset + MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
  /* lr = lmf_addr */
  ARM_LDR_IMM(code, ARMREG_LR, basereg,
              offset + MONO_STRUCT_OFFSET(MonoLMF, lmf_addr));
  /* *(lmf_addr) = previous_lmf */
  ARM_STR_IMM(code, ARMREG_IP, ARMREG_LR,
              MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
...
static guint8 *get_delegate_invoke_impl(MonoTrampInfo **info,
  ...
  if (has_target) {
    ...
    ARM_LDR_IMM(code, ARMREG_R0, ARMREG_R0,
...
gboolean mono_arm_is_hard_float(void) {
...
gboolean mono_arch_is_soft_float(void) { return arm_fpu == MONO_ARM_FPU_NONE; }
#endif

static gboolean is_regsize_var(MonoType *t) {
  if (t->byref)
    return TRUE;
...
static void inline add_general(guint *gr, guint *stack_size, ArgInfo *ainfo,
                               gboolean simple) {
  if (simple) {
    if (*gr > ARMREG_R3) {
      ainfo->size = 4;
...
static void inline add_float(guint *fpr, guint *stack_size, ArgInfo *ainfo,
                             gboolean is_double, gint *float_spare) {
  /*
...
static gboolean is_hfa(MonoType *t, int *out_nfields, int *out_esize) {
  MonoClass *klass;
  gpointer iter;
  ...
  while ((field = mono_class_get_fields_internal(klass, &iter))) {
    ...
    if (MONO_TYPE_ISSTRUCT(ftype)) {
      ...
      if (!(!ftype->byref &&
            (ftype->type == MONO_TYPE_R4 || ftype->type == MONO_TYPE_R8)))
        return FALSE;
      if (prev_ftype && prev_ftype->type != ftype->type)
        return FALSE;
      prev_ftype = ftype;
...
static CallInfo *get_call_info(MonoMemPool *mp, MonoMethodSignature *sig) {
  ...
  case MONO_TYPE_TYPEDBYREF:
    if (IS_HARD_FLOAT && sig->pinvoke && is_hfa(t, &nfields, &esize)) {
      ...
      if (is_pinvoke) {
        int native_size = mono_class_native_size(
            mono_class_from_mono_type_internal(t), &align);
        int max_size;
  ...
  for (i = pstart; i < sig->param_count; ++i) {
    ...
    case MONO_TYPE_VALUETYPE: {
      ...
      align_size &= ~(sizeof(target_mgreg_t) - 1);
      nwords =
          (align_size + sizeof(target_mgreg_t) - 1) / sizeof(target_mgreg_t);
...
static void arg_get_val(CallContext *ccontext, ArgInfo *ainfo, gpointer dest) {
  ...
  memcpy((host_mgreg_t *)dest + ainfo->size, ccontext->stack + ainfo->offset,
         ainfo->struct_size - reg_size);
}
...
static void arg_set_val(CallContext *ccontext, ArgInfo *ainfo, gpointer src) {
  ...
  memcpy(ccontext->stack + ainfo->offset, (host_mgreg_t *)src + ainfo->size,
         ainfo->struct_size - reg_size);
}
...
gboolean mono_arch_tailcall_supported(MonoCompile *cfg,
  ...
  /*
   * Tailcalls with more callee stack usage than the caller cannot be supported,
   * since the extra stack space would be left on the stack after the tailcall.
   */
  gboolean res = IS_SUPPORTED_TAILCALL(callee_info->stack_usage <=
                                       caller_info->stack_usage) &&
                 IS_SUPPORTED_TAILCALL(caller_info->ret.storage ==
                                       callee_info->ret.storage);

  // FIXME The limit here is that moving the parameters requires addressing the
  // parameters with 12bit (4K) immediate offsets. - 4 for TAILCALL_REG/MEMBASE
...
static void mono_arch_compute_omit_fp(MonoCompile *cfg) {
  ...
  */
  ...
  for (i = 0; i < sig->param_count + sig->hasthis; ++i) {
    ...
    if (ainfo->storage == RegTypeBase || ainfo->storage == RegTypeBaseGen ||
        ainfo->storage == RegTypeStructByVal) {
      /*
...
void mono_arch_allocate_vars(MonoCompile *cfg) {
  ...
  /* Allocate these first so they have a small offset, OP_SEQ_POINT depends on
   * this */
  ...
  if (cfg->has_atomic_exchange_i4 || cfg->has_atomic_cas_i4 ||
  ...
  for (i = curinst; i < cfg->num_varinfo; ++i) {
    ...
    ins = cfg->varinfo[i];
    if ((ins->flags & MONO_INST_IS_DEAD) || ins->opcode == OP_REGVAR ||
        ins->opcode == OP_REGOFFSET)
      continue;
    ...
    /* inst->backend.is_pinvoke indicates native sized value types, this is used
     * by the pinvoke wrappers when they call functions returning structure */
    if (ins->backend.is_pinvoke && MONO_TYPE_ISSTRUCT(t) &&
        t->type != MONO_TYPE_TYPEDBYREF) {
      size = mono_class_native_size(mono_class_from_mono_type_internal(t),
                                    &ualign);
      align = ualign;
    } else
  ...
  for (i = 0; i < sig->param_count; ++i) {
    ...
    case RegTypeHFA:
      ...
      if (cfg->verbose_level >= 2)
        g_print("arg %d allocated to %s+0x%0x.\n", i,
                mono_arch_regname(ins->inst_basereg), (int)ins->inst_offset);
      // FIXME:
      offset += 32;
      break;
    default:
    ...
    if (ins->opcode != OP_REGVAR) {
      ...
      if (align < 4 && size >= 4)
        align = 4;
      /* The code in the prolog () stores words when storing vtypes received in
       * a register */
...
void mono_arch_create_vars(MonoCompile *cfg) {
  ...
  if (IS_HARD_FLOAT) {
    for (i = 0; i < 2; i++) {
      MonoInst *inst = mono_compile_create_var(
          cfg, m_class_get_byval_arg(mono_defaults.double_class), OP_LOCAL);
...
static void emit_sig_cookie(MonoCompile *cfg, MonoCallInst *call,
  ...
  MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, ARMREG_SP,
...
LLVMCallInfo *mono_arch_get_llvm_call_info(MonoCompile *cfg,
  ...
  default:
    cfg->exception_message =
        g_strdup_printf("unknown ret conv (%d)", cinfo->ret.storage);
  ...
  for (i = 0; i < n; ++i) {
    ...
    default:
      cfg->exception_message =
          g_strdup_printf("ainfo->storage (%d)", ainfo->storage);
      cfg->disable_llvm = TRUE;
      break;
    }
  }
...
void mono_arch_emit_call(MonoCompile *cfg, MonoCallInst *call) {
  ...
  case RegTypeHFA:
    ...
    if (MONO_IS_TAILCALL_OPCODE(call))
      break;
    /*
     * The vtype is returned in registers, save the return area address in a
     * local, and save the vtype into the location pointed to by it after call
     * in emit_move_return_value ().
     */
  ...
  for (i = 0; i < n; ++i) {
    ...
    case RegTypeIRegPair:
      if (!t->byref &&
          ((t->type == MONO_TYPE_I8) || (t->type == MONO_TYPE_U8))) {
        ...
        MONO_ADD_INS(cfg->cbb, ins);
        mono_call_inst_add_outarg_reg(cfg, call, ins->dreg, ainfo->reg + 1,
                                      FALSE);
      } else if (!t->byref &&
                 ((t->type == MONO_TYPE_R8) || (t->type == MONO_TYPE_R4))) {
        if (ainfo->size == 4) {
          if (IS_SOFT_FLOAT) {
            ...
            ins->sreg1 = in->dreg;
            MONO_ADD_INS(cfg->cbb, ins);
            mono_call_inst_add_outarg_reg(cfg, call, ins->dreg, ainfo->reg,
                                          FALSE);
          } else {
            int creg;

            cfg->param_area = MAX(cfg->param_area, 8);
            MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER4_MEMBASE_REG, ARMREG_SP,
                                        (cfg->param_area - 8), in->dreg);
            creg = mono_alloc_ireg(cfg);
            MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOAD_MEMBASE, creg, ARMREG_SP,
                                          (cfg->param_area - 8));
          ...
          if (IS_SOFT_FLOAT) {
            ...
            cfg->param_area = MAX(cfg->param_area, 8);
            MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, ARMREG_SP,
                                        (cfg->param_area - 8), in->dreg);
            creg = mono_alloc_ireg(cfg);
            MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOAD_MEMBASE, creg, ARMREG_SP,
                                          (cfg->param_area - 8));
            ...
            MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOAD_MEMBASE, creg, ARMREG_SP,
                                          (cfg->param_area - 8 + 4));
            mono_call_inst_add_outarg_reg(cfg, call, creg, ainfo->reg + 1,
                                          FALSE);
          }
        }
    ...
    case RegTypeBase:
      if (!t->byref &&
          ((t->type == MONO_TYPE_I8) || (t->type == MONO_TYPE_U8))) {
        MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI8_MEMBASE_REG, ARMREG_SP,
                                    ainfo->offset, in->dreg);
      } else if (!t->byref &&
                 ((t->type == MONO_TYPE_R4) || (t->type == MONO_TYPE_R8))) {
        if (t->type == MONO_TYPE_R8) {
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, ARMREG_SP,
                                      ainfo->offset, in->dreg);
        } else {
          if (IS_SOFT_FLOAT)
            MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG, ARMREG_SP,
                                        ainfo->offset, in->dreg);
          else
            MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER4_MEMBASE_REG, ARMREG_SP,
                                        ainfo->offset, in->dreg);
    ...
    case RegTypeBaseGen:
      if (!t->byref &&
        ...
        MONO_EMIT_NEW_STORE_MEMBASE(
            cfg, OP_STORE_MEMBASE_REG, ARMREG_SP, ainfo->offset,
            (G_BYTE_ORDER == G_BIG_ENDIAN) ? MONO_LVREG_LS(in->dreg)
                                           : MONO_LVREG_MS(in->dreg));
        MONO_INST_NEW(cfg, ins, OP_MOVE);
        ins->dreg = mono_alloc_ireg(cfg);
        ins->sreg1 = G_BYTE_ORDER == G_BIG_ENDIAN ? MONO_LVREG_MS(in->dreg)
                                                  : MONO_LVREG_LS(in->dreg);
        MONO_ADD_INS(cfg->cbb, ins);
    ...
    case RegTypeFP: {
      ...
      if (ainfo->size == 8) {
        ...
        /*
         * Mono's register allocator doesn't speak single-precision registers
         * that overlap double-precision registers (i.e. armhf). So we have to
         * work around the register allocator and load the value from memory
         * manually.
         *
         * So we create a variable for the float argument and an instruction to
         * store the argument into the variable. We then store the list of these
         * arguments in call->float_args. This list is then used by
         * emit_float_args later to pass the arguments in the various call
         * opcodes.
         *
         * This is not very nice, and we should really try to fix the allocator.
         */

        MonoInst *float_arg = mono_compile_create_var(
            cfg, m_class_get_byval_arg(mono_defaults.single_class), OP_LOCAL);
        ...
        call->float_args =
            g_slist_append_mempool(cfg->mempool, call->float_args, fad);
      }
...
static void add_outarg_reg(MonoCompile *cfg, MonoCallInst *call,
                           ArgStorage storage, int reg, MonoInst *arg) {
  MonoInst *ins;
...
void mono_arch_emit_outarg_vt(MonoCompile *cfg, MonoInst *ins, MonoInst *src) {
  ...
  default:
    ...
    for (i = 0; i < ainfo->size; ++i) {
      ...
      switch (struct_size) {
      case 1:
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, dreg, src->dreg,
                                      soffset);
        break;
      case 2:
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU2_MEMBASE, dreg, src->dreg,
                                      soffset);
        break;
      case 3:
        ...
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, dreg, src->dreg,
                                      soffset);
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, tmpreg, src->dreg,
                                      soffset + 1);
    ...
    // g_print ("vt size: %d at R%d + %d\n", doffset, vt->inst_basereg,
    // vt->inst_offset);
    if (ovf_size != 0)
      mini_emit_memcpy(cfg, ARMREG_SP, doffset, src->dreg, soffset,
                       MIN(ovf_size * sizeof(target_mgreg_t), struct_size),
                       struct_size < 4 ? 1 : 4);
    break;
  }
}
...
static gboolean dyn_call_supported(CallInfo *cinfo, MonoMethodSignature *sig) {
  int i;
...
/*
 * The immediate field for cond branches is big enough for all reasonable
 * methods
 */
#define EMIT_COND_BRANCH_FLAGS(ins, condcode)                                  \
  if (0 && ins->inst_true_bb->native_offset) {                                 \
    ARM_B_COND(code, (condcode),                                               \
               (code - cfg->native_code + ins->inst_true_bb->native_offset) &  \
                   0xffffff);                                                  \
  } else {                                                                     \
...
#define EMIT_COND_BRANCH(ins, cond)                                            \
  EMIT_COND_BRANCH_FLAGS(ins, branch_cc_table[(cond)])
...
#define EMIT_COND_SYSTEM_EXCEPTION(cond, exc_name)                             \
  EMIT_COND_SYSTEM_EXCEPTION_FLAGS(branch_cc_table[(cond)], (exc_name))
...
void mono_arch_peephole_pass_2(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS_SAFE(bb, n, ins) {
    ...
    case OP_LOADI4_MEMBASE:
       ...
       */
      if (last_ins &&
          (last_ins->opcode == OP_STOREI4_MEMBASE_REG ||
      ...
      if (last_ins &&
          (last_ins->opcode == OP_LOADI4_MEMBASE ||
    ...
    case OP_LOADI1_MEMBASE:
      if (last_ins && (last_ins->opcode == OP_STOREI1_MEMBASE_REG) &&
          ins->inst_basereg == last_ins->inst_destbasereg &&
          ins->inst_offset == last_ins->inst_offset) {
        ins->opcode = (ins->opcode == OP_LOADI1_MEMBASE) ? OP_ICONV_TO_I1
                                                         : OP_ICONV_TO_U1;
        ins->sreg1 = last_ins->sreg1;
      }
      break;
    ...
    case OP_LOADI2_MEMBASE:
      if (last_ins && (last_ins->opcode == OP_STOREI2_MEMBASE_REG) &&
          ins->inst_basereg == last_ins->inst_destbasereg &&
          ins->inst_offset == last_ins->inst_offset) {
        ins->opcode = (ins->opcode == OP_LOADI2_MEMBASE) ? OP_ICONV_TO_I2
                                                         : OP_ICONV_TO_U2;
        ins->sreg1 = last_ins->sreg1;
      }
      break;
    case OP_MOVE:
      ...
      if (last_ins && last_ins->opcode == OP_MOVE &&
          ins->sreg1 == last_ins->dreg && ins->dreg == last_ins->sreg1) {
        MONO_DELETE_INS(bb, ins);
        continue;
      }
      break;
...
case CEE_BLT_UN:
 */
static const guchar branch_cc_table[] = {
    ARMCOND_EQ, ARMCOND_GE, ARMCOND_GT, ARMCOND_LE, ARMCOND_LT,

    ARMCOND_NE, ARMCOND_HS, ARMCOND_HI, ARMCOND_LS, ARMCOND_LO};

#define ADD_NEW_INS(cfg, dest, op)                                             \
  do {                                                                         \
    MONO_INST_NEW((cfg), (dest), (op));                                        \
    mono_bblock_insert_before_ins(bb, ins, (dest));                            \
  } while (0)

static int map_to_reg_reg_op(int op) {
  switch (op) {
  case OP_ADD_IMM:
    return OP_IADD;
  case OP_SUB_IMM:
...
void mono_arch_lowering_pass(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  loop_start:
    ...
    case OP_IXOR_IMM:
      ...
      if (ins->opcode == OP_SBB || ins->opcode == OP_ISBB ||
          ins->opcode == OP_SUBCC)
        goto loop_start;
      else
        break;
    ...
    case OP_ISUBCC: {
      ...
      while (try_count-- && current->next) {
        if (current->next->opcode == OP_COND_EXC_C ||
            current->next->opcode == OP_COND_EXC_IC) {
    ...
    case OP_LOADR8_MEMBASE:
      ...
      if ((imm8 = mono_arm_is_rotated_imm8(ins->inst_offset & ~0x1ff,
                                           &rot_amount)) >= 0) {
...
void mono_arch_decompose_long_opts(MonoCompile *cfg, MonoInst *long_ins) {
  MonoInst *ins;

  if (long_ins->opcode == OP_LNEG) {
    ins = long_ins;
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ARM_RSBS_IMM, MONO_LVREG_LS(ins->dreg),
                            MONO_LVREG_LS(ins->sreg1), 0);
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ARM_RSC_IMM, MONO_LVREG_MS(ins->dreg),
                            MONO_LVREG_MS(ins->sreg1), 0);
    NULLIFY_INS(ins);
  }
}

static guchar *emit_float_to_int(MonoCompile *cfg, guchar *code, int dreg,
                                 int sreg, int size, gboolean is_signed) {
...
static guchar *emit_r4_to_int(MonoCompile *cfg, guchar *code, int dreg,
                              int sreg, int size, gboolean is_signed) {
  /* sreg is a float, dreg is an integer reg  */
  g_assert(IS_VFP);
  code = mono_arm_emit_vfp_scratch_save(cfg, code, vfp_scratch1);
...
#define is_call_imm(diff)                                                      \
  ((gint)(diff) >= -33554432 && (gint)(diff) <= 33554431)

static void emit_thunk(guint8 *code, gconstpointer target) {
  guint8 *p = code;
...
static void handle_thunk(MonoCompile *cfg, MonoDomain *domain, guchar *code,
                         const guchar *target) {
  MonoJitInfo *ji = NULL;
  ...
  if (cfg) {
    ...
    if (!thunks_size) {
      g_print("thunk failed %p->%p, thunk space=%d method %s", code, target,
              thunks_size, mono_method_full_name(cfg->method, TRUE));
      g_assert_not_reached();
    }
...
static void arm_patch_general(MonoCompile *cfg, MonoDomain *domain,
                              guchar *code, const guchar *target) {
  ...
  if (prim == 5) { /* 101b */
    ...
    if (tval & 1) { /* entering thumb mode */
      ...
      /* this low bit of the displacement is moved to bit 24 in the instruction
       * encoding */
      if (diff & 2) {
        tbits |= 1 << 24;
      }
...
gboolean mono_arm_thumb_supported(void) { return thumb_supported; }

gboolean mono_arm_eabi_supported(void) { return eabi_supported; }

int mono_arm_i8_align(void) { return i8_align; }
...
void mono_arch_output_basic_block(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS(bb, ins) {
    ...
    case OP_ATOMIC_ADD_I4: {
      ...
      /* Free up a reg */
      if (ins->sreg1 != ARMREG_IP && ins->sreg2 != ARMREG_IP &&
          ins->sreg3 != ARMREG_IP)
        tmpreg = ARMREG_IP;
      else if (ins->sreg1 != ARMREG_R0 && ins->sreg2 != ARMREG_R0 &&
               ins->sreg3 != ARMREG_R0)
        tmpreg = ARMREG_R0;
      else if (ins->sreg1 != ARMREG_R1 && ins->sreg2 != ARMREG_R1 &&
               ins->sreg3 != ARMREG_R1)
        tmpreg = ARMREG_R1;
      else
        tmpreg = ARMREG_R2;
    ...
    case OP_BIGMUL:
      ARM_SMULL_REG_REG(code, ins->backend.reg3, ins->dreg, ins->sreg1,
                        ins->sreg2);
      break;
    case OP_BIGMUL_UN:
      ARM_UMULL_REG_REG(code, ins->backend.reg3, ins->dreg, ins->sreg1,
                        ins->sreg2);
      break;
    case OP_STOREI1_MEMBASE_IMM:
    ...
    case OP_STORE_MEMBASE_REG:
    case OP_STOREI4_MEMBASE_REG:
      /* this case is special, since it happens for spill code after lowering
       * has been called */
      if (arm_is_imm12(ins->inst_offset)) {
    ...
    case OP_BREAK:
      /*
       * gdb does not like encountering the hw breakpoint ins in the debugged
       * code. So instead of emitting a trap, we emit a call a C function and
    ...
    case OP_SEQ_POINT: {
      ...
      if (ins->flags & MONO_INST_SINGLE_STEP_LOC) {
        if (cfg->soft_breakpoints) {
          ...
          if (cfg->compile_aot) {
            /* Load the trigger page addr from the variable initialized in the
             * prolog */
            var = ss_trigger_page_var;
            g_assert(var);
            g_assert(var->opcode == OP_REGOFFSET);
            code =
                emit_ldr_imm(code, dreg, var->inst_basereg, var->inst_offset);
          } else {
            ARM_LDR_IMM(code, dreg, ARMREG_PC, 0);
            ARM_B(code, 0);
      ...
      if (cfg->compile_aot) {
        ...
        code = emit_ldr_imm(code, dreg, var->inst_basereg, var->inst_offset);
        /* Add the offset */
        val = ((offset / 4) * sizeof(target_mgreg_t)) +
              MONO_STRUCT_OFFSET(SeqPointInfo, bp_addrs);
        /* Load the info->bp_addrs [offset], which is either 0 or the address of
         * a trigger page */
    ...
    case OP_IADD_OVF:
      ARM_ADD_REG_REG(code, ins->dreg, ins->sreg1, ins->sreg2);
      // EMIT_COND_SYSTEM_EXCEPTION_FLAGS (PPC_BR_FALSE, PPC_BR_EQ,
      // "OverflowException");
      break;
    case OP_IADD_OVF_UN:
      ARM_ADD_REG_REG(code, ins->dreg, ins->sreg1, ins->sreg2);
    ...
    case OP_ISUB_OVF_UN:
      ARM_SUB_REG_REG(code, ins->dreg, ins->sreg1, ins->sreg2);
      // EMIT_COND_SYSTEM_EXCEPTION_FLAGS (PPC_BR_TRUE, PPC_BR_EQ,
      // "OverflowException");
      break;
    ...
    case OP_OBJC_GET_SELECTOR:
      mono_add_patch_info(cfg, offset, MONO_PATCH_INFO_OBJC_SELECTOR_REF,
                          ins->inst_p0);
      ARM_LDR_IMM(code, ins->dreg, ARMREG_PC, 0);
      ARM_B(code, 0);
      *(gpointer *)code = NULL;
    ...
    case OP_TAILCALL_REG: {
      ...
      if (call->stack_usage) {
        ...
        const int offset_ip =
            (save_ip && (cfg->frame_reg == ARMREG_SP)) ? 4 : 0;
        ...
        // Point IP at the start of where the parameters will go after trimming
        // stack. After locals and saved registers.
        code = emit_big_add(code, ARMREG_IP, cfg->frame_reg,
                            cfg->stack_usage + prev_sp_offset);

        /* Copy arguments on the stack to our argument area */
        // FIXME a fixed size memcpy is desirable here,
        // at least for larger values of stack_usage.
        //
        // FIXME For most functions, with frames < 4K, we can use frame_reg
        // directly here instead of IP. See
        // https://github.com/mono/mono/pull/12079 See
        // https://github.com/mono/mono/pull/12079/commits/93e7007a9567b78fa8152ce404b372b26e735516
      ...
      code = emit_big_add_temp(code, ARMREG_SP, cfg->frame_reg,
                               cfg->stack_usage, ARMREG_LR);
      if (iphone_abi) {
    ...
    case OP_CALL_REG:
      if (IS_HARD_FLOAT)
        code =
            emit_float_args(cfg, (MonoCallInst *)ins, code, &max_len, &offset);
    ...
    case OP_LOCALLOC: {
      ...
      ARM_ADD_REG_IMM8(code, ins->dreg, ins->sreg1,
                       (MONO_ARCH_FRAME_ALIGNMENT - 1));
      ARM_BIC_REG_IMM8(code, ins->dreg, ins->dreg,
                       (MONO_ARCH_FRAME_ALIGNMENT - 1));
      ...
      if (cfg->param_area)
        code =
            emit_sub_imm(code, ARMREG_SP, ARMREG_SP,
    ...
    case OP_DYN_CALL: {
      ...
      if (IS_HARD_FLOAT) {
        ARM_LDR_IMM(code, ARMREG_R0, ARMREG_LR,
                    MONO_STRUCT_OFFSET(DynCallArgs, has_fpregs));
        ARM_CMP_REG_IMM(code, ARMREG_R0, 0, 0);
        labels[0] = code;
        ARM_B_COND(code, ARMCOND_EQ, 0);
        for (i = 0; i < FP_PARAM_REGS; ++i) {
          const int offset =
              MONO_STRUCT_OFFSET(DynCallArgs, fpregs) + (i * sizeof(double));
      ...
      ARM_LDR_IMM(code, ARMREG_R1, ARMREG_LR,
                  MONO_STRUCT_OFFSET(DynCallArgs, n_stackargs));
      ...
      code = emit_big_add(code, ARMREG_R2, ARMREG_LR,
      ...
      ARM_STR_IMM(code, ARMREG_R0, ARMREG_IP,
                  MONO_STRUCT_OFFSET(DynCallArgs, res));
      ARM_STR_IMM(code, ARMREG_R1, ARMREG_IP,
                  MONO_STRUCT_OFFSET(DynCallArgs, res2));
      if (IS_HARD_FLOAT)
        ARM_FSTD(code, ARM_VFP_D0, ARMREG_IP,
                 MONO_STRUCT_OFFSET(DynCallArgs, fpregs));
      break;
    }
    case OP_THROW: {
    ...
    case OP_BR:
      /*if (ins->inst_target_bb->native_offset) {
          ARM_B (code, 0);
          //x86_jump_code (code, cfg->native_code +
      ins->inst_target_bb->native_offset); } else*/
      {
    ...
    case OP_SWITCH:
      ...
      code = realloc_code(cfg, max_len);
      ARM_LDR_REG_REG_SHIFT(code, ARMREG_PC, ARMREG_PC, ins->sreg1,
                            ARMSHIFT_LSL, 2);
      ARM_NOP(code);
    ...
    case OP_R8CONST:
      if (cfg->compile_aot) {
        ...
        *(guint32 *)code = ((guint32 *)(ins->inst_p0))[1];
         ...
         */
        code = mono_arm_emit_load_imm(code, ARMREG_LR,
                                      (guint32)(gsize)ins->inst_p0);
        ARM_FLDD(code, ins->dreg, ARMREG_LR, 0);
      }
      break;
    case OP_R4CONST:
      ...
      break;
    case OP_STORER8_MEMBASE_REG:
      /* This is generated by the local regalloc pass which runs after the
       * lowering pass */
      if (!arm_is_fpimm8(ins->inst_offset)) {
        code = mono_arm_emit_load_imm(code, ARMREG_LR, ins->inst_offset);
    ...
    case OP_SETFRET: {
      MonoType *sig_ret = mini_get_underlying_type(
    ...
    case OP_LCONV_TO_OVF_I4_2: {
      guint8 *high_bit_not_set, *valid_negative, *invalid_negative,
          *valid_positive;
      /*
      ...
      ARM_B_COND(code, ARMCOND_GE,
                 0); /*branch if bit 31 of the lower part is not set*/

      ARM_CMN_REG_IMM8(code, ins->sreg2,
                       1); /*This have the same effect as CMP reg, 0xFFFFFFFF */
      valid_negative = code;
      ARM_B_COND(code, ARMCOND_EQ, 0); /*branch if upper part == 0xFFFFFFFF
                                          (lower part has bit 31 set) */
    ...
    case OP_FILL_PROF_CALL_CTX:
      for (int i = 0; i < ARMREG_MAX; i++)
        if ((MONO_ARCH_CALLEE_SAVED_REGS & (1 << i)) || i == ARMREG_SP ||
            i == ARMREG_FP)
          ARM_STR_IMM(code, i, ins->sreg1,
                      MONO_STRUCT_OFFSET(MonoContext, regs) +
                          i * sizeof(target_mgreg_t));
      break;
    default:
    ...
    if ((cfg->opt & MONO_OPT_BRANCH) &&
        ((code - cfg->native_code - offset) > max_len)) {
      g_warning("wrong maximal instruction length of instruction %s (expected "
                "%d, got %d)",
...
void mono_arm_unaligned_stack(MonoMethod *method) { g_assert_not_reached(); }
...
guint8 *mono_arch_emit_prolog(MonoCompile *cfg) {
    ...
    for (i = 0; i < 16; ++i) {
      if ((cfg->used_int_regs & (1 << i))) {
        mono_emit_unwind_op_offset(cfg, code, i,
                                   (-prev_sp_offset) + reg_offset);
        mini_gc_set_slot_type_from_cfa(cfg, (-prev_sp_offset) + reg_offset,
                                       SLOT_NOREF);
        reg_offset += 4;
      }
    }
  ...
  alloc_size +=
      ALIGN_TO(prev_sp_offset, MONO_ARCH_FRAME_ALIGNMENT) - prev_sp_offset;
  ...
  for (i = 0; i < alloc_size - orig_alloc_size; i += 4)
    mini_gc_set_slot_type_from_cfa(cfg, (-prev_sp_offset) + orig_alloc_size + i,
                                   SLOT_NOREF);
  ...
  /*
  {
      ...
      code = mono_arm_emit_load_imm (code, ARMREG_IP, MONO_ARCH_FRAME_ALIGNMENT
  -1); ARM_AND_REG_REG (code, ARMREG_LR, ARMREG_LR, ARMREG_IP); ARM_CMP_REG_IMM
  (code, ARMREG_LR, 0, 0); buf [0] = code; ARM_B_COND (code, ARMCOND_EQ, 0); if
  (cfg->compile_aot) ARM_MOV_REG_IMM8 (code, ARMREG_R0, 0); else code =
  mono_arm_emit_load_imm (code, ARMREG_R0, (guint32)cfg->method);
      mono_add_patch_info (cfg, code - cfg->native_code,
  MONO_PATCH_INFO_JIT_ICALL_ID, GUINT_TO_POINTER
  (MONO_JIT_ICALL_mono_arm_unaligned_stack)); code = emit_call_seq (cfg, code);
      arm_patch (buf [0], code);
  }
  */
  ...
  if (cfg->rgctx_var) {
    ...
    if (arm_is_imm12(ins->inst_offset)) {
      ARM_STR_IMM(code, MONO_ARCH_RGCTX_REG, ins->inst_basereg,
                  ins->inst_offset);
    } else {
      code = mono_arm_emit_load_imm(code, ARMREG_LR, ins->inst_offset);
    ...
    mono_add_var_location(cfg, cfg->rgctx_var, TRUE, MONO_ARCH_RGCTX_REG, 0, 0,
                          code - cfg->native_code);
    mono_add_var_location(cfg, cfg->rgctx_var, FALSE, ins->inst_basereg,
  ...
  if (sig->call_convention == MONO_CALL_VARARG) {
    ...
    ARM_ADD_REG_IMM8(code, ARMREG_IP, cfg->frame_reg,
                     prev_sp_offset + cookie->offset);
    ...
    if (inst->opcode == OP_REGVAR) {
      if (ainfo->storage == RegTypeGeneral)
        ...
        if (arm_is_imm12(prev_sp_offset + ainfo->offset)) {
          ARM_LDR_IMM(code, inst->dreg, ARMREG_SP,
      ...
      if (i == 0 && sig->hasthis) {
        ...
        mono_add_var_location(cfg, inst, TRUE, ainfo->reg, 0, 0,
                              code - cfg->native_code);
        mono_add_var_location(cfg, inst, TRUE, inst->dreg, 0,
      ...
      case RegTypeHFA:
        for (part = 0; part < ainfo->nregs; part++) {
          if (ainfo->esize == 4)
            ARM_FSTS(code, ainfo->reg + part, inst->inst_basereg,
                     inst->inst_offset + (part * ainfo->esize));
          else
            ARM_FSTD(code, ainfo->reg + (part * 2), inst->inst_basereg,
                     inst->inst_offset + (part * ainfo->esize));
        }
        break;
      case RegTypeGeneral:
      ...
      case RegTypeStructByAddr:
        ...
        case 1:
          if (arm_is_imm12(inst->inst_offset))
            ARM_STRB_IMM(code, ainfo->reg, inst->inst_basereg,
                         inst->inst_offset);
          else {
        ...
        case 2:
          if (arm_is_imm8(inst->inst_offset)) {
            ARM_STRH_IMM(code, ainfo->reg, inst->inst_basereg,
                         inst->inst_offset);
          } else {
            code = mono_arm_emit_load_imm(code, ARMREG_IP, inst->inst_offset);
        ...
        case 8:
          ...
          if (arm_is_imm12(inst->inst_offset + 4)) {
            ARM_STR_IMM(code, ainfo->reg + 1, inst->inst_basereg,
                        inst->inst_offset + 4);
          } else {
            code =
                mono_arm_emit_load_imm(code, ARMREG_IP, inst->inst_offset + 4);
            ARM_STR_REG_REG(code, ainfo->reg + 1, inst->inst_basereg,
                            ARMREG_IP);
          }
          break;
        default:
        ...
        if (i == 0 && sig->hasthis) {
          ...
          mono_add_var_location(cfg, inst, TRUE, ainfo->reg, 0, 0,
                                code - cfg->native_code);
          mono_add_var_location(cfg, inst, FALSE, inst->inst_basereg,
                                inst->inst_offset, code - cfg->native_code, 0);
        }
        break;
      case RegTypeBaseGen:
        if (arm_is_imm12(prev_sp_offset + ainfo->offset)) {
        ...
        if (arm_is_imm12(inst->inst_offset + 4)) {
          ARM_STR_IMM(code, ARMREG_LR, inst->inst_basereg,
                      inst->inst_offset + 4);
      ...
      case RegTypeStructByAddrOnStack:
        ...
        case 1:
          if (arm_is_imm8(inst->inst_offset)) {
            ARM_STRB_IMM(code, ARMREG_LR, inst->inst_basereg,
                         inst->inst_offset);
          } else {
            code = mono_arm_emit_load_imm(code, ARMREG_IP, inst->inst_offset);
        ...
        case 2:
          if (arm_is_imm8(inst->inst_offset)) {
            ARM_STRH_IMM(code, ARMREG_LR, inst->inst_basereg,
                         inst->inst_offset);
          } else {
            code = mono_arm_emit_load_imm(code, ARMREG_IP, inst->inst_offset);
      ...
      case RegTypeFP: {
        ...
        if ((imm8 = mono_arm_is_rotated_imm8(inst->inst_offset, &rot_amount)) ==
            -1) {
          code = mono_arm_emit_load_imm(code, ARMREG_IP, inst->inst_offset);
          ARM_ADD_REG_REG(code, ARMREG_IP, ARMREG_IP, inst->inst_basereg);
        } else
          ARM_ADD_REG_IMM(code, ARMREG_IP, inst->inst_basereg, imm8,
                          rot_amount);
      ...
      case RegTypeStructByVal: {
        ...
        for (cur_reg = 0; cur_reg < ainfo->size; ++cur_reg) {
          if (arm_is_imm12(doffset)) {
            ARM_STR_IMM(code, ainfo->reg + cur_reg, inst->inst_basereg,
                        doffset);
          } else {
            code = mono_arm_emit_load_imm(code, ARMREG_IP, doffset);
            ARM_STR_REG_REG(code, ainfo->reg + cur_reg, inst->inst_basereg,
                            ARMREG_IP);
          }
          soffset += sizeof(target_mgreg_t);
        ...
        if (ainfo->vtsize) {
          ...
          // g_print ("emit_memcpy (prev_sp_ofs: %d, ainfo->offset: %d, soffset:
          // %d)\n", prev_sp_offset, ainfo->offset, soffset);
          code = emit_memcpy(code, ainfo->vtsize * sizeof(target_mgreg_t),
                             inst->inst_basereg, doffset, ARMREG_SP,
                             prev_sp_offset + ainfo->offset);
        }
        break;
      }
  ...
  if (!cfg->soft_breakpoints) {
    ...
    if (info_var) {
      ...
      code = emit_ldr_imm(code, dreg, info_var->inst_basereg,
                          info_var->inst_offset);
      /* Load the trigger page addr */
      ARM_LDR_IMM(code, dreg, dreg,
                  MONO_STRUCT_OFFSET(SeqPointInfo, ss_trigger_page));
      ARM_STR_IMM(code, dreg, ss_trigger_page_var->inst_basereg,
                  ss_trigger_page_var->inst_offset);
    }
  }
  ...
  if (cfg->arch.seq_point_ss_method_var) {
    ...
    if (cfg->compile_aot) {
      ...
      ARM_LDR_IMM(code, dreg, dreg,
                  MONO_STRUCT_OFFSET(SeqPointInfo, ss_tramp_addr));
      ARM_STR_IMM(code, dreg, ss_method_ins->inst_basereg,
                  ss_method_ins->inst_offset);
    } else {
      ...
      *(gpointer *)code = breakpoint_tramp;
      ...
      ARM_STR_IMM(code, ARMREG_IP, ss_method_ins->inst_basereg,
                  ss_method_ins->inst_offset);
      ARM_LDR_IMM(code, ARMREG_IP, ARMREG_LR, 4);
      ARM_STR_IMM(code, ARMREG_IP, bp_method_ins->inst_basereg,
                  bp_method_ins->inst_offset);
    }
  }
...
void mono_arch_emit_epilog(MonoCompile *cfg) {
  ...
  case RegTypeHFA: {
    ...
    for (i = 0; i < cinfo->ret.nregs; ++i) {
      if (cinfo->ret.esize == 4)
        ARM_FLDS(code, cinfo->ret.reg + i, ins->inst_basereg,
                 ins->inst_offset + (i * cinfo->ret.esize));
      else
        ARM_FLDD(code, cinfo->ret.reg + (i * 2), ins->inst_basereg,
                 ins->inst_offset + (i * cinfo->ret.esize));
    }
    break;
  }
  ...
  if (method->save_lmf) {
    ...
    pos += MONO_ABI_SIZEOF(MonoLMF) -
           (MONO_ARM_NUM_SAVED_REGS * sizeof(target_mgreg_t));
    lmf_offset = pos;

    code = emit_restore_lmf(cfg, code, cfg->stack_usage - lmf_offset);

    /* This points to r4 inside MonoLMF->iregs */
    sp_adj = (MONO_ABI_SIZEOF(MonoLMF) -
              MONO_ARM_NUM_SAVED_REGS * sizeof(target_mgreg_t));
    reg = ARMREG_R4;
     ...
     */
    code = emit_big_add(code, ARMREG_SP, cfg->frame_reg,
                        cfg->stack_usage - lmf_offset + sp_adj);
    for (i = 0; i < 16; i++) {
      if (regmask & (1 << i))
        nused_int_regs++;
    }
    mono_emit_unwind_op_def_cfa(cfg, code, ARMREG_SP,
                                ((iphone_abi ? 3 : 0) + nused_int_regs) * 4);
    /* restore iregs */
...
static arminstr_t *arm_emit_value_and_patch_ldr(arminstr_t *code,
                                                arminstr_t *target,
                                                guint32 value) {
...
static void mini_dump_bad_imt(int input_imt, int compared_imt, int pc) {
  g_print("BAD IMT comparing %x with expected %x at ip %x", input_imt,
          compared_imt, pc);
  g_assert(0);
}
#endif
...
gpointer mono_arch_build_imt_trampoline(MonoVTable *vtable, MonoDomain *domain,
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (item->is_equals) {
      ...
      if (item->has_target_code ||
          !arm_is_imm12(
              DISTANCE(vtable, &vtable->vtable[item->value.vtable_slot]))) {
        item->chunk_size += 32;
        large_offsets = TRUE;
      }
...
#ifdef DEBUG_IMT
  g_print("Building IMT trampoline for class %s %s entries %d code size %d "
          "code at %p end %p vtable %p fail_tramp %p\n",
          m_class_get_name_space(vtable->klass),
          m_class_get_name(vtable->klass), count, size, start,
          ((guint8 *)start) + size, vtable, fail_tramp);
  for (i = 0; i < count; ++i) {
    MonoIMTCheckItem *item = imt_entries[i];
    g_print("method %d (%p) %s vtable slot %p is_equals %d chunk size %d\n", i,
            item->key, ((MonoMethod *)item->key)->name,
            &vtable->vtable[item->value.vtable_slot], item->is_equals,
            item->chunk_size);
  }
#endif
  ...
  if (large_offsets) {
    ...
    mono_add_unwind_op_def_cfa_offset(unwind_ops, code, start,
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (item->is_equals) {
      ...
      if (item->has_target_code) {
        ...
        code = arm_emit_value_and_patch_ldr(code, target_code_ins,
                                            (gsize)item->value.target_code);
      } else {
        vtable_offset =
            DISTANCE(vtable, &vtable->vtable[item->value.vtable_slot]);
        if (!arm_is_imm12(vtable_offset)) {
          ...
          code = arm_emit_value_and_patch_ldr(code, vtable_offset_ins,
                                              vtable_offset);
        } else {
          ...
          if (large_offsets) {
            mono_add_unwind_op_def_cfa_offset(unwind_ops, code, start,
                                              2 * sizeof(target_mgreg_t));
            ARM_ADD_REG_IMM8(code, ARMREG_SP, ARMREG_SP,
                             2 * sizeof(target_mgreg_t));
          }
      ...
      if (imt_method)
        code = arm_emit_value_and_patch_ldr(code, imt_method,
                                            (guint32)(gsize)item->key);

      /*must emit after unconditional branch*/
      if (vtable_target) {
        code = arm_emit_value_and_patch_ldr(code, vtable_target,
                                            (guint32)(gsize)vtable);
        item->chunk_size += 4;
        vtable_target = NULL;
      }

      /*We reserve the space for bsearch IMT values after the first entry with
       * an absolute jump*/
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (i > 0 && item->is_equals) {
      ...
      for (j = i - 1; j >= 0 && !imt_entries[j]->is_equals; --j) {
        space_start = arm_emit_value_and_patch_ldr(
            space_start, (arminstr_t *)imt_entries[j]->code_target,
            (guint32)(gsize)imt_entries[j]->key);
      }
    }
  }
...
#ifdef DEBUG_IMT
  {
    char *buff = g_strdup_printf("thunk_for_class_%s_%s_entries_%d",
                                 m_class_get_name_space(vtable->klass),
                                 m_class_get_name(vtable->klass), count);
  ...
  MONO_PROFILER_RAISE(jit_code_buffer,
                      ((guint8 *)start, code - start,
                       MONO_PROFILER_CODE_BUFFER_IMT_TRAMPOLINE, NULL));
  UnlockedAdd(&mono_stats.imt_trampolines_size, code - start);

  g_assert(DISTANCE(start, code) <= size);

  mono_tramp_info_register(mono_tramp_info_create(NULL, (guint8 *)start,
                                                  DISTANCE(start, code), NULL,
                                                  unwind_ops),
                           domain);
...
void mono_arch_set_breakpoint(MonoJitInfo *ji, guint8 *ip) {
  ...
  if (ji->from_aot) {
    ...
    info->bp_addrs[native_offset / 4] =
        (guint8 *)(mini_debug_options.soft_breakpoints ? breakpoint_tramp
                                                       : bp_trigger_page);
...
void mono_arch_clear_breakpoint(MonoJitInfo *ji, guint8 *ip) {
  ...
  if (ji->from_aot) {
    ...
    g_assert(native_offset % 4 == 0);
    g_assert(info->bp_addrs[native_offset / 4] ==
             (guint8 *)(mini_debug_options.soft_breakpoints ? breakpoint_tramp
                                                            : bp_trigger_page));
...
gboolean mono_arch_is_single_step_event(void *info, void *sigctx) {
  ...
  if (sinfo->si_addr >= ss_trigger_page &&
      (guint8 *)sinfo->si_addr <= (guint8 *)ss_trigger_page + 128)
    return TRUE;
  else
    return FALSE;
}
 ...
 *
 *   Return whenever the machine state in SIGCTX corresponds to a breakpoint
 * event.
 */
gboolean mono_arch_is_breakpoint_event(void *info, void *sigctx) {
  ...
  if (sinfo->si_signo == DBG_SIGNAL) {
    /* Sometimes the address is off by 4 */
    if (sinfo->si_addr >= bp_trigger_page &&
        (guint8 *)sinfo->si_addr <= (guint8 *)bp_trigger_page + 128)
      return TRUE;
    else
      return FALSE;
  } else {
 ...
 *
 *   Set the target architecture the JIT backend should generate code for, in
 * the form of a GNU target triplet. Only used in AOT mode.
 */
...
gpointer mono_arch_get_get_tls_tramp(void) { return NULL; }

static G_GNUC_UNUSED guint8 *emit_aotconst(MonoCompile *cfg, guint8 *code,
                                           int dreg, int patch_type,
                                           gpointer data) {
  /* OP_AOTCONST */
  mono_add_patch_info(cfg, code - cfg->native_code,
                      (MonoJumpInfoType)patch_type, data);
  ARM_LDR_IMM(code, dreg, ARMREG_PC, 0);
  ARM_B(code, 0);
  *(gpointer *)code = NULL;
  code += 4;
...
guint8 *mono_arm_emit_aotconst(gpointer ji_list, guint8 *code, guint8 *buf,
                               int dreg, int patch_type, gconstpointer data) {
  MonoJumpInfo **ji = (MonoJumpInfo **)ji_list;

  *ji = mono_patch_info_list_prepend(*ji, code - buf,
                                     (MonoJumpInfoType)patch_type, data);
  ARM_LDR_IMM(code, dreg, ARMREG_PC, 0);
  ARM_B(code, 0);
  *(gpointer *)code = NULL;
