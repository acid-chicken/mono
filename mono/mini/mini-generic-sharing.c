...
static void mono_class_unregister_image_generic_subclasses(MonoImage *image,
                                                           gpointer user_data);
...
static gboolean partial_sharing_supported(void) {
...
static int type_check_context_used(MonoType *type, gboolean recursive) {
  ...
  case MONO_TYPE_ARRAY:
    return mono_class_check_context_used(
        mono_type_get_array_type(type)->eklass);
...
static int inst_check_context_used(MonoGenericInst *inst) {
  int context_used = 0;
  int i;
 ...
 */
int mono_generic_context_check_used(MonoGenericContext *context) {
  int context_used = 0;
 ...
 * variables.
 */
int mono_class_check_context_used(MonoClass *klass) {
  int context_used = 0;
  ...
  if (mono_class_is_ginst(klass))
    context_used |= mono_generic_context_check_used(
        &mono_class_get_generic_class(klass)->context);
  else if (mono_class_is_gtd(klass))
    context_used |= mono_generic_context_check_used(
        &mono_class_get_generic_container(klass)->context);
...
get_info_templates(MonoRuntimeGenericContextTemplate *template_,
                   int type_argc) {
  g_assert(type_argc >= 0);
  if (type_argc == 0)
    return template_->infos;
  return (MonoRuntimeGenericContextInfoTemplate *)g_slist_nth_data(
      template_->method_templates, type_argc - 1);
}

/*
 * LOCKING: loader lock
 */
static void set_info_templates(MonoImage *image,
  ...
  if (type_argc == 0)
    ...
    while (length < type_argc) {
      template_->method_templates =
          mono_g_slist_append_image(image, template_->method_templates, NULL);
      length++;
    }
 ...
 * LOCKING: loader lock
 */
static int template_get_max_argc(MonoRuntimeGenericContextTemplate *template_) {
 ...
 */
static MonoRuntimeGenericContextInfoTemplate *
rgctx_template_get_other_slot(MonoRuntimeGenericContextTemplate *template_,
                              int type_argc, int slot) {
  int i;
  MonoRuntimeGenericContextInfoTemplate *oti;

  g_assert(slot >= 0);

  for (oti = get_info_templates(template_, type_argc), i = 0; i < slot;
       oti = oti->next, ++i) {
    if (!oti)
      return NULL;
  }
 ...
 */
static int
rgctx_template_num_infos(MonoRuntimeGenericContextTemplate *template_,
                         int type_argc) {
  MonoRuntimeGenericContextInfoTemplate *oti;
  int i;

  for (i = 0, oti = get_info_templates(template_, type_argc); oti;
       ++i, oti = oti->next)
    ;
 ...
 */
static void
class_set_rgctx_template(MonoClass *klass,
                         MonoRuntimeGenericContextTemplate *rgctx_template) {
  if (!m_class_get_image(klass)->rgctx_template_hash)
    m_class_get_image(klass)->rgctx_template_hash =
        g_hash_table_new(mono_aligned_addr_hash, NULL);

  g_hash_table_insert(m_class_get_image(klass)->rgctx_template_hash, klass,
                      rgctx_template);
}
...
class_lookup_rgctx_template(MonoClass *klass) {
  ...
  template_ = (MonoRuntimeGenericContextTemplate *)g_hash_table_lookup(
      m_class_get_image(klass)->rgctx_template_hash, klass);
 ...
 * LOCKING: loader lock
 */
static void register_generic_subclass(MonoClass *klass) {
...
static void move_subclasses_not_in_image_foreach_func(MonoClass *klass,
                                                      MonoClass *subclass,
                                                      MonoImage *image) {
  MonoClass *new_list;
  ...
  new_list = NULL;
  while (subclass) {
    MonoRuntimeGenericContextTemplate *subclass_template =
...
static void mono_class_unregister_image_generic_subclasses(MonoImage *image,
  ...
  g_hash_table_foreach(
      old_hash, (GHFunc)move_subclasses_not_in_image_foreach_func, image);
...
static MonoRuntimeGenericContextTemplate *alloc_template(MonoClass *klass) {
  ...
  return (MonoRuntimeGenericContextTemplate *)mono_image_alloc0(
      m_class_get_image(klass), size);
}

/* LOCKING: Takes the loader lock */
static MonoRuntimeGenericContextInfoTemplate *alloc_oti(MonoImage *image) {
  ...
  return (MonoRuntimeGenericContextInfoTemplate *)mono_image_alloc0(image,
                                                                    size);
}
 ...
 */
static int info_has_identity(MonoRgctxInfoType info_type) {
 ...
 * LOCKING: loader lock
 */
static void rgctx_template_set_slot(
    MonoImage *image, MonoRuntimeGenericContextTemplate *template_,
    int type_argc, int slot, gpointer data, MonoRgctxInfoType info_type) {
  int i;
  MonoRuntimeGenericContextInfoTemplate *list =
      get_info_templates(template_, type_argc);
 ...
 */
MonoMethod *mono_class_get_method_generic(MonoClass *klass, MonoMethod *method,
                                          MonoError *error) {
  ...
  if (!m) {
    ...
    for (i = 0; i < mcount; ++i) {
      ...
      if (m == declaring)
        break;
      if (m->is_inflated &&
          mono_method_get_declaring_generic_method(m) == declaring)
        break;
    }
    if (i >= mcount)
      return NULL;
...
static gpointer inflate_info(MonoRuntimeGenericContextInfoTemplate *oti,
  ...
  case MONO_RGCTX_INFO_NULLABLE_CLASS_UNBOX: {
    gpointer result = mono_class_inflate_generic_type_with_mempool(
        temporary ? NULL : m_class_get_image(klass), (MonoType *)data, context,
        error);
  ...
  case MONO_RGCTX_INFO_METHOD_DELEGATE_CODE: {
    ...
    MonoType *inflated_type = mono_class_inflate_generic_type_checked(
        m_class_get_byval_arg(method->klass), context, error);
    mono_error_assert_ok(error); /* FIXME don't swallow the error */

    MonoClass *inflated_class =
    ...
    if (m_class_get_byval_arg(inflated_class)->type == MONO_TYPE_ARRAY ||
      ...
      inflated_method = mono_method_search_in_array_class(
          inflated_class, method->name, method->signature);
    } else {
      ERROR_DECL(error);
      inflated_method =
  ...
  case MONO_RGCTX_INFO_METHOD_GSHAREDVT_INFO: {
    ...
    res = (MonoGSharedVtMethodInfo *)mono_domain_alloc0(
        domain, sizeof(MonoGSharedVtMethodInfo));
    /*
    res->nlocals = info->nlocals;
    res->locals_types = g_new0 (MonoType*, info->nlocals);
    for (i = 0; i < info->nlocals; ++i)
            res->locals_types [i] = mono_class_inflate_generic_type
    (info->locals_types [i], context);
    */
    res->num_entries = oinfo->num_entries;
    res->entries = (MonoRuntimeGenericContextInfoTemplate *)mono_domain_alloc0(
        domain,
        sizeof(MonoRuntimeGenericContextInfoTemplate) * oinfo->num_entries);
  ...
  case MONO_RGCTX_INFO_METHOD_GSHAREDVT_OUT_TRAMPOLINE_VIRT: {
    ...
    res = (MonoJumpInfoGSharedVtCall *)mono_domain_alloc0(
        domain, sizeof(MonoJumpInfoGSharedVtCall));
  ...
  case MONO_RGCTX_INFO_VIRT_METHOD_BOX_TYPE: {
    ...
    // FIXME: Temporary
    res = (MonoJumpInfoVirtMethod *)mono_domain_alloc0(
        domain, sizeof(MonoJumpInfoVirtMethod));
    t = mono_class_inflate_generic_type_checked(
        m_class_get_byval_arg(info->klass), context, error);
    mono_error_assert_ok(error); /* FIXME don't swallow the error */
    ...
    res->method =
        mono_class_inflate_generic_method_checked(info->method, context, error);
    g_assert(is_ok(error)); /* FIXME don't swallow the error */
  ...
  case MONO_RGCTX_INFO_DELEGATE_TRAMP_INFO: {
    ERROR_DECL(error);
    MonoDelegateClassMethodPair *dele_info =
        (MonoDelegateClassMethodPair *)data;
    MonoDomain *domain = mono_domain_get();

    MonoType *t = mono_class_inflate_generic_type_checked(
        m_class_get_byval_arg(dele_info->klass), context, error);
    mono_error_assert_msg_ok(
        error,
        "Could not inflate generic type"); /* FIXME proper error handling */
    ...
    MonoMethod *method = mono_class_inflate_generic_method_checked(
        dele_info->method, context, error);
    mono_error_assert_msg_ok(
        error,
        "Could not inflate generic method"); /* FIXME proper error handling */

    // FIXME: Temporary
    MonoDelegateClassMethodPair *res =
        (MonoDelegateClassMethodPair *)mono_domain_alloc0(
            domain, sizeof(MonoDelegateClassMethodPair));
...
static void free_inflated_info(MonoRgctxInfoType info_type, gpointer info) {
  if (!info)
    return;
...
static MonoRuntimeGenericContextInfoTemplate
class_get_rgctx_template_oti(MonoClass *klass, int type_argc, guint32 slot,
                             gboolean temporary, gboolean shared,
                             gboolean *do_free);

static MonoClass *class_uninstantiated(MonoClass *klass) {
  if (mono_class_is_ginst(klass))
 ...
 */
static MonoClass *get_shared_class(MonoClass *klass) {
 ...
 * @class: a class
 *
 * Looks up or constructs, if necessary, the runtime generic context template
 * for class. The template is the same for all instantiations of a class.
 */
static MonoRuntimeGenericContextTemplate *
  ...
  if (m_class_get_parent(klass)) {
    ...
    for (type_argc = 0; type_argc <= max_argc; ++type_argc) {
      ...
      for (i = 0; i < num_entries; ++i) {
        ...
        oti = class_get_rgctx_template_oti(m_class_get_parent(klass), type_argc,
                                           i, FALSE, FALSE, NULL);
        if (oti.data && oti.data != MONO_RGCTX_SLOT_USED_MARKER) {
          rgctx_template_set_slot(m_class_get_image(klass), template_,
                                  type_argc, i, oti.data, oti.info_type);
        }
      }
    }
  }
...
class_get_rgctx_template_oti(MonoClass *klass, int type_argc, guint32 slot,
  ...
  if (mono_class_is_ginst(klass) && !shared) {
    ...
    oti = class_get_rgctx_template_oti(
        mono_class_get_generic_class(klass)->container_class, type_argc, slot,
        TRUE, FALSE, &tmp_do_free);
    if (oti.data) {
      gpointer info = oti.data;
      oti.data =
          inflate_info(&oti, &mono_class_get_generic_class(klass)->context,
                       klass, temporary);
      if (tmp_do_free)
...
static gpointer class_type_info(MonoDomain *domain, MonoClass *klass,
                                MonoRgctxInfoType info_type, MonoError *error) {
  error_init(error);
  ...
  case MONO_RGCTX_INFO_CAST_CACHE: {
    ...
    gpointer **cache_data =
        (gpointer **)mono_domain_alloc0(domain, sizeof(gpointer) * 2);
  ...
  case MONO_RGCTX_INFO_CLASS_SIZEOF: {
    ...
    return GINT_TO_POINTER(
        mono_type_size(m_class_get_byval_arg(klass), &align));
  }
  case MONO_RGCTX_INFO_CLASS_BOX_TYPE:
    if (MONO_TYPE_IS_REFERENCE(m_class_get_byval_arg(klass)))
  ...
  case MONO_RGCTX_INFO_NULLABLE_CLASS_UNBOX: {
    ...
    if (!mono_class_is_nullable(klass))
      /* This can happen since all the entries in MonoGSharedVtMethodInfo are
       * inflated, even those which are not used */
      return NULL;
    ...
    if (mono_llvm_only) {
      /* FIXME: We have no access to the gsharedvt signature/gsctx used by the
       * caller, so have to construct it ourselves */
      gmethod =
    ...
    ji = mini_jit_info_table_find(
        mono_domain_get(), (char *)mono_get_addr_from_ftnptr(addr), NULL);
    g_assert(ji);
...
static gboolean ji_is_gsharedvt(MonoJitInfo *ji) {
  if (ji && ji->has_generic_jit_info &&
      (mono_jit_info_get_generic_sharing_context(ji)->is_gsharedvt))
    return TRUE;
  else
    return FALSE;
}
...
static guint tramp_info_hash(gconstpointer key) {
  GSharedVtTrampInfo *tramp = (GSharedVtTrampInfo *)key;

  return (gsize)tramp->addr;
}

static gboolean tramp_info_equal(gconstpointer a, gconstpointer b) {
  ...
  return tramp1->is_in == tramp2->is_in && tramp1->calli == tramp2->calli &&
         tramp1->vcall_offset == tramp2->vcall_offset &&
         tramp1->addr == tramp2->addr && tramp1->sig == tramp2->sig &&
         tramp1->gsig == tramp2->gsig;
}
...
static MonoType *get_wrapper_shared_type(MonoType *t);
static MonoType *get_wrapper_shared_type_full(MonoType *t, gboolean field);
 ...
 */
static MonoType *get_wrapper_shared_vtype(MonoType *t) {
  ERROR_DECL(error);
  ...
  // FIXME: Map 1 member structs to primitive types on platforms where its
  // supported

  klass = mono_class_from_mono_type_internal(t);
  if ((mono_class_get_flags(klass) & TYPE_ATTRIBUTE_LAYOUT_MASK) !=
 ...
 */
static MonoType *get_wrapper_shared_type_full(MonoType *t, gboolean is_field) {
  if (t->byref)
  ...
  case MONO_TYPE_GENERICINST: {
    ...
    klass = mono_class_inflate_generic_class_checked(
        mono_class_get_generic_class(klass)->container_class, &ctx, error);
    mono_error_assert_ok(error); /* FIXME don't swallow the error */
...
static MonoType *get_wrapper_shared_type_reg(MonoType *t) {
 ...
 *
 *   Return a wrapper to translate between the normal and gsharedvt calling
 * conventions of SIG. The returned wrapper has a signature of SIG, plus one
 * extra argument, which is an <addr, rgctx> pair. The extra argument is passed
 * the same way as an rgctx to shared methods. It calls <addr> using the
 * gsharedvt version of SIG, passing in <rgctx> as an extra argument.
 */
MonoMethod *mini_get_gsharedvt_in_sig_wrapper(MonoMethodSignature *sig) {
  ...
  gshared_lock();
  if (!cache)
    cache = g_hash_table_new_full((GHashFunc)mono_signature_hash,
  ...
  // FIXME:
  csig = g_malloc0(MONO_SIZEOF_METHOD_SIGNATURE +
                   ((sig->param_count + 1) * sizeof(MonoType *)));
  ...
  gsharedvt_sig = g_malloc0(MONO_SIZEOF_METHOD_SIGNATURE +
  ...
  mb =
      mono_mb_new(mono_defaults.object_class,
                  sig->hasthis ? "gsharedvt_in_sig" : "gsharedvt_in_sig_static",
                  MONO_WRAPPER_OTHER);
#ifdef ENABLE_ILGEN
 ...
 *
 *   Same as in_sig_wrapper, but translate between the gsharedvt and normal
 * signatures.
 */
MonoMethod *mini_get_gsharedvt_out_sig_wrapper(MonoMethodSignature *sig) {
  ...
  normal_sig = g_malloc0(MONO_SIZEOF_METHOD_SIGNATURE +
                         ((sig->param_count + 2) * sizeof(MonoType *)));
  ...
  // FIXME: Use shared signatures
  mb = mono_mb_new(mono_defaults.object_class, "gsharedvt_out_sig",
                   MONO_WRAPPER_OTHER);
#ifdef ENABLE_ILGEN
  mono_mb_set_param_names(mb, (const char **)param_names);
#endif
...
static gboolean signature_equal_pinvoke(MonoMethodSignature *sig1,
 ...
 *
 *   Return a wrapper which can be used to transition from compiled code to the
 * interpreter. The wrapper has the same signature as SIG. It is very similar to
 * a gsharedvt_in wrapper, except the 'extra_arg' is passed in the rgctx reg, so
 * this wrapper needs to be called through a static rgctx trampoline.
...
MonoMethod *mini_get_interp_in_wrapper(MonoMethodSignature *sig) {
  ...
  if (!cache)
    cache =
        g_hash_table_new_full((GHashFunc)mono_signature_hash,
                              (GEqualFunc)signature_equal_pinvoke, NULL, NULL);
  res = (MonoMethod *)g_hash_table_lookup(cache, sig);
  gshared_unlock();
  if (res) {
    g_free(sig);
  ...
  if (sig->param_count > 8)
    /* Call the generic interpreter entry point, the specialized ones only
     * handle a limited number of arguments */
    generic = TRUE;

  /*
   * If we need to return a native struct, we can't allocate a local and store
   * it there since that assumes a managed representation. Instead we allocate
   * on the stack, pass this address to the interp_entry and when we return it
   * we use CEE_MONO_LDNATIVEOBJ
   */
  return_native_struct = sig->ret->type == MONO_TYPE_VALUETYPE && sig->pinvoke;

  /* Create the signature for the wrapper */
  csig = g_malloc0(MONO_SIZEOF_METHOD_SIGNATURE +
                   (sig->param_count * sizeof(MonoType *)));
  memcpy(csig, sig, mono_metadata_signature_size(sig));
  ...
  if (generic) {
     ...
     * interp_entry_general (gpointer this_arg, gpointer res, gpointer *args,
     * gpointer rmethod)
     */
    entry_sig =
        g_malloc0(MONO_SIZEOF_METHOD_SIGNATURE + (4 * sizeof(MonoType *)));
    ...
    /*
     * The called function has the following signature:
     * void entry(<optional this ptr>, <optional return ptr>, <arguments>,
     * <extra arg>)
     */
    entry_sig = g_malloc0(MONO_SIZEOF_METHOD_SIGNATURE +
                          ((sig->param_count + 2) * sizeof(MonoType *)));
    ...
    for (i = 0; i < sig->param_count; i++) {
      ...
      if (!sig->params[i]->byref) {
        entry_sig->params[pindex] =
            mono_metadata_type_dup(NULL, entry_sig->params[pindex]);
...
MonoMethod *mini_get_interp_lmf_wrapper(const char *name, gpointer target) {
  ...
  g_assert(target == (gpointer)mono_interp_to_native_trampoline ||
           target == (gpointer)mono_interp_entry_from_trampoline);
  const int index = target == (gpointer)mono_interp_to_native_trampoline;
  const MonoJitICallId jit_icall_id =
      index ? MONO_JIT_ICALL_mono_interp_to_native_trampoline
  ...
  mb =
      mono_mb_new(mono_defaults.object_class, wrapper_name, MONO_WRAPPER_OTHER);
...
MonoMethodSignature *mini_get_gsharedvt_out_sig_wrapper_signature(
    gboolean has_this, gboolean has_ret, int param_count) {
  MonoMethodSignature *sig = g_malloc0(
      sizeof(MonoMethodSignature) + ((param_count + 3) * sizeof(MonoType *)));
  int i, pindex;
  MonoType *int_type = mono_get_int_type();
 ...
 */
gpointer mini_get_gsharedvt_wrapper(gboolean gsharedvt_in, gpointer addr,
                                    MonoMethodSignature *normal_sig,
                                    MonoMethodSignature *gsharedvt_sig,
  ...
  /*
   * The arg trampolines might only have a finite number in full-aot, so use a
   * cache.
   */
  mono_domain_lock(domain);
  if (!domain_info->gsharedvt_arg_tramp_hash)
    domain_info->gsharedvt_arg_tramp_hash =
        g_hash_table_new(tramp_info_hash, tramp_info_equal);
  res = g_hash_table_lookup(domain_info->gsharedvt_arg_tramp_hash, &tinfo);
  mono_domain_unlock(domain);
  if (res)
    return res;

  info = mono_arch_get_gsharedvt_call_info(addr, normal_sig, gsharedvt_sig,
                                           gsharedvt_in, vcall_offset, calli);
  ...
  if (mono_aot_only)
    ...
    addr =
        mono_arch_get_gsharedvt_arg_trampoline(mono_domain_get(), info, addr);

  mono_atomic_inc_i32(&gsharedvt_num_trampolines);

  /* Cache it */
  tramp_info = (GSharedVtTrampInfo *)mono_domain_alloc0(
      domain, sizeof(GSharedVtTrampInfo));
  *tramp_info = tinfo;
 ...
 */
static gpointer instantiate_info(MonoDomain *domain,
                                 MonoRuntimeGenericContextInfoTemplate *oti,
                                 MonoGenericContext *context, MonoClass *klass,
                                 MonoError *error) {
  gpointer data;
  ...
  case MONO_RGCTX_INFO_REFLECTION_TYPE: {
    MonoReflectionType *ret =
        mono_type_get_object_checked(domain, (MonoType *)data, error);
  ...
  case MONO_RGCTX_INFO_GENERIC_METHOD_CODE: {
    ...
    return_val_if_nok(error, NULL);
    return mini_add_method_trampoline(
        m, addr, mono_method_needs_static_rgctx_invoke(m, FALSE), FALSE);
  }
  case MONO_RGCTX_INFO_METHOD_FTNDESC: {
    MonoMethod *m = (MonoMethod *)data;
  ...
  case MONO_RGCTX_INFO_GSHAREDVT_OUT_WRAPPER: {
    ...
    if (callee_gsharedvt) {
      ...
      return mini_llvmonly_create_ftndesc(domain, addr,
  ...
  case MONO_RGCTX_INFO_VIRT_METHOD_CODE: {
    ...
    if (mono_llvm_only) {
      gpointer arg = NULL;
      addr =
          mini_llvmonly_add_method_wrappers(method, addr, FALSE, FALSE, &arg);

      /* Returns an ftndesc */
      return mini_llvmonly_create_ftndesc(domain, addr, arg);
    } else {
      return mini_add_method_trampoline(
          method, addr, mono_method_needs_static_rgctx_invoke(method, FALSE),
          FALSE);
    }
  }
  case MONO_RGCTX_INFO_VIRT_METHOD_BOX_TYPE: {
  ...
  case MONO_RGCTX_INFO_REMOTING_INVOKE_WITH_CHECK: {
    MonoMethod *remoting_invoke_method =
        mono_marshal_get_remoting_invoke_with_check((MonoMethod *)data, error);
    return_val_if_nok(error, NULL);
  ...
  case MONO_RGCTX_INFO_SIG_GSHAREDVT_IN_TRAMPOLINE_CALLI: {
    ...
    /*
     * This is an indirect call to the address passed by the caller in the rgctx
     * reg.
     */
  ...
  case MONO_RGCTX_INFO_METHOD_GSHAREDVT_OUT_TRAMPOLINE_VIRT: {
    ...
    gboolean virtual_ =
        oti->info_type == MONO_RGCTX_INFO_METHOD_GSHAREDVT_OUT_TRAMPOLINE_VIRT;
    gint32 vcall_offset;
    gboolean callee_gsharedvt;
    ...
    if (virtual_) {
      ...
      if ((m_class_get_parent(method->klass) ==
        ...
        vcall_offset =
            ((gint32)imt_slot - MONO_IMT_SIZE) * TARGET_SIZEOF_VOID_P;
      } else {
        vcall_offset =
    ...
    callee_ji = mini_jit_info_table_find(
        mono_domain_get(), (char *)mono_get_addr_from_ftnptr(addr), NULL);
    callee_gsharedvt = ji_is_gsharedvt(callee_ji);

    /*
     * For gsharedvt calls made out of gsharedvt methods, the callee could end
     * up being a gsharedvt method, or a normal non-shared method. The latter
     * call cannot be patched, so instead of using a normal call, we make an
     * indirect call through the rgctx, in effect patching the rgctx entry
     * instead of the call site. For virtual calls, the caller might be a normal
     * or a gsharedvt method. Since there is only one vtable slot, this
     * difference needs to be handed on the caller side. This is currently
     * implemented by adding a gsharedvt-in trampoline to all gsharedvt methods
     * and storing this trampoline into the vtable slot. Virtual calls made from
     * gsharedvt methods always go through a gsharedvt-out trampoline, so the
     * calling sequence is: caller -> out trampoline -> in trampoline -> callee
    ...
    if (virtual_ || !callee_gsharedvt) {
      ...
      if (mono_llvm_only) {
        if (mini_is_gsharedvt_variable_signature(call_sig)) {
          ...
          gpointer out_wrapper =
              mini_get_gsharedvt_wrapper(FALSE, NULL, sig, gsig, -1, FALSE);
          MonoFtnDesc *out_wrapper_arg = mini_llvmonly_create_ftndesc(
              domain, callee_ji->code_start, mini_method_get_rgctx(method));

          /* Returns an ftndesc */
          addr = mini_llvmonly_create_ftndesc(domain, out_wrapper,
                                              out_wrapper_arg);
        } else {
          addr = mini_llvmonly_create_ftndesc(domain, addr,
                                              mini_method_get_rgctx(method));
        }
      } else {
        addr = mini_get_gsharedvt_wrapper(FALSE, addr, sig, gsig, vcall_offset,
                                          FALSE);
      }
#if 0
            if (virtual)
      ...
      /*
       * This is a combination of the out and in cases, since both the caller
       * and the callee are gsharedvt methods. The caller and the callee can use
       * different gsharedvt signatures, so we have to add both an out and an in
       * trampoline, i.e.:
       * class Base<T> {
      ...
      if (mono_llvm_only) {
        ...
        if (mini_is_gsharedvt_variable_signature(call_sig)) {
          /*
           * This is not an optimization, but its needed, since the concrete
           * signature 'sig' might not exist at all in IL, so the AOT compiler
           * cannot generate the wrappers for it.
           */
          addr = mini_llvmonly_create_ftndesc(domain, callee_ji->code_start,
                                              mini_method_get_rgctx(method));
        } else if (mini_is_gsharedvt_variable_signature(gsig)) {
          gpointer in_wrapper = mini_get_gsharedvt_wrapper(
              TRUE, callee_ji->code_start, sig, gsig, -1, FALSE);

          gpointer in_wrapper_arg = mini_llvmonly_create_ftndesc(
              domain, callee_ji->code_start, mini_method_get_rgctx(method));

          addr =
              mini_llvmonly_create_ftndesc(domain, in_wrapper, in_wrapper_arg);
        } else {
          addr = mini_llvmonly_create_ftndesc(domain, addr,
                                              mini_method_get_rgctx(method));
        }
        ...
        addr = mini_get_gsharedvt_wrapper(TRUE, callee_ji->code_start, sig,
                                          gsig, -1, FALSE);
  ...
  case MONO_RGCTX_INFO_METHOD_GSHAREDVT_INFO: {
    ...
    // FIXME:
    res = (MonoGSharedVtMethodRuntimeInfo *)g_malloc0(
        sizeof(MonoGSharedVtMethodRuntimeInfo) +
        (info->num_entries * sizeof(gpointer)));
    ...
    for (i = 0; i < info->num_entries; ++i) {
      ...
      default:
        res->entries[i] =
            instantiate_info(domain, template_, context, klass, error);
        if (!is_ok(error))
          return NULL;
        break;
      }
  ...
  case MONO_RGCTX_INFO_DELEGATE_TRAMP_INFO: {
    ...
    if (dele_info->is_virtual)
      trampoline = mono_create_delegate_virtual_trampoline(
          domain, dele_info->klass, dele_info->method);
    else
      trampoline = mono_create_delegate_trampoline_info(
          domain, dele_info->klass, dele_info->method);
 ...
 * LOCKING: loader lock
 */
static void fill_in_rgctx_template_slot(MonoClass *klass, int type_argc,
                                        int index, gpointer data,
                                        MonoRgctxInfoType info_type) {
  MonoRuntimeGenericContextTemplate *template_ =
      mono_class_get_runtime_generic_context_template(klass);
  MonoClass *subclass;

  rgctx_template_set_slot(m_class_get_image(klass), template_, type_argc, index,
                          data, info_type);
  ...
  while (subclass) {
    ...
    subclass_oti = class_get_rgctx_template_oti(
        m_class_get_parent(subclass), type_argc, index, FALSE, FALSE, NULL);
    g_assert(subclass_oti.data);

    fill_in_rgctx_template_slot(subclass, type_argc, index, subclass_oti.data,
                                info_type);
...
const char *mono_rgctx_info_type_to_str(MonoRgctxInfoType type) {
  switch (type) {
  case MONO_RGCTX_INFO_STATIC_DATA:
    return "STATIC_DATA";
  case MONO_RGCTX_INFO_KLASS:
...
G_GNUC_UNUSED static char *rgctx_info_to_str(MonoRgctxInfoType info_type,
                                             gpointer data) {
  switch (info_type) {
  case MONO_RGCTX_INFO_VTABLE:
 ...
 * LOCKING: loader lock
 */
static int register_info(MonoClass *klass, int type_argc, gpointer data,
                         MonoRgctxInfoType info_type) {
  int i;
  MonoRuntimeGenericContextTemplate *template_ =
      mono_class_get_runtime_generic_context_template(klass);
  ...
  DEBUG(printf("set slot %s, infos [%d] = %s, %s\n",
               ...
               rgctx_info_to_str(info_type, data)));
  ...
  while (parent != NULL) {
    ...
    rgctx_template_set_slot(m_class_get_image(parent), parent_template,
                            type_argc, i, MONO_RGCTX_SLOT_USED_MARKER,
                            (MonoRgctxInfoType)0);
...
static gboolean info_equal(gpointer data1, gpointer data2,
                           MonoRgctxInfoType info_type) {
  switch (info_type) {
  case MONO_RGCTX_INFO_STATIC_DATA:
  case MONO_RGCTX_INFO_KLASS:
  ...
  case MONO_RGCTX_INFO_NULLABLE_CLASS_BOX:
  case MONO_RGCTX_INFO_NULLABLE_CLASS_UNBOX:
    return mono_class_from_mono_type_internal((MonoType *)data1) ==
           mono_class_from_mono_type_internal((MonoType *)data2);
  case MONO_RGCTX_INFO_METHOD:
  case MONO_RGCTX_INFO_METHOD_FTNDESC:
  case MONO_RGCTX_INFO_METHOD_GSHAREDVT_INFO:
  case MONO_RGCTX_INFO_GENERIC_METHOD_CODE:
  ...
  case MONO_RGCTX_INFO_DELEGATE_TRAMP_INFO: {
    ...
    return dele1->is_virtual == dele2->is_virtual &&
           dele1->method == dele2->method && dele1->klass == dele2->klass;
  }
  default:
    g_assert_not_reached();
  }
 ...
 * encoded slot number.
 */
static guint32 lookup_or_register_info(MonoClass *klass, MonoMethod *method,
                                       gboolean in_mrgctx, gpointer data,
                                       MonoRgctxInfoType info_type,
 ...
 */
int mono_class_rgctx_get_array_size(int n, gboolean mrgctx) {
  g_assert(n >= 0 && n < 30);
 ...
 * LOCKING: domain lock
 */
static gpointer *alloc_rgctx_array(MonoDomain *domain, int n,
                                   gboolean is_mrgctx) {
  gint32 size =
      mono_class_rgctx_get_array_size(n, is_mrgctx) * sizeof(gpointer);
...
static gpointer fill_runtime_generic_context(
    MonoVTable *class_vtable, MonoRuntimeGenericContext *rgctx, guint32 slot,
  ...
  MonoGenericContext *class_context =
      mono_class_is_ginst(klass) ? &mono_class_get_generic_class(klass)->context
                                 : NULL;
  MonoRuntimeGenericContextInfoTemplate oti;
  MonoGenericContext context = {
      class_context ? class_context->class_inst : NULL, method_inst};
  int rgctx_index;
  gboolean do_free;
  ...
  oti = class_get_rgctx_template_oti(get_shared_class(klass),
                                     method_inst ? method_inst->type_argc : 0,
                                     slot, TRUE, TRUE, &do_free);
  /* This might take the loader lock */
  info = (MonoRuntimeGenericContext *)instantiate_info(domain, &oti, &context,
                                                       klass, error);
  return_val_if_nok(error, NULL);
  g_assert(info);
 ...
 */
gpointer mono_class_fill_runtime_generic_context(MonoVTable *class_vtable,
                                                 guint32 slot,
                                                 MonoError *error) {
  MonoDomain *domain = class_vtable->domain;
  ...
  info = fill_runtime_generic_context(class_vtable, rgctx, slot, NULL, FALSE,
                                      error);

  DEBUG(printf("get rgctx slot: %s %d -> %p\n",
               mono_type_full_name(m_class_get_byval_arg(class_vtable->klass)),
               slot, info));
 ...
 */
gpointer mono_method_fill_runtime_generic_context(
    MonoMethodRuntimeGenericContext *mrgctx, guint32 slot, MonoError *error) {
  gpointer info;

  info = fill_runtime_generic_context(mrgctx->class_vtable,
                                      (MonoRuntimeGenericContext *)mrgctx, slot,
                                      mrgctx->method_inst, TRUE, error);

  return info;
}

static guint mrgctx_hash_func(gconstpointer key) {
  const MonoMethodRuntimeGenericContext *mrgctx =
      (const MonoMethodRuntimeGenericContext *)key;

  return mono_aligned_addr_hash(mrgctx->class_vtable) ^
         mono_metadata_generic_inst_hash(mrgctx->method_inst);
}

static gboolean mrgctx_equal_func(gconstpointer a, gconstpointer b) {
  const MonoMethodRuntimeGenericContext *mrgctx1 =
      (const MonoMethodRuntimeGenericContext *)a;
  const MonoMethodRuntimeGenericContext *mrgctx2 =
      (const MonoMethodRuntimeGenericContext *)b;

  return mrgctx1->class_vtable == mrgctx2->class_vtable &&
         mono_metadata_generic_inst_equal(mrgctx1->method_inst,
                                          mrgctx2->method_inst);
}
...
mini_method_get_mrgctx(MonoVTable *class_vtable, MonoMethod *method) {
  ...
  if (!method_inst) {
    ...
    mrgctx = (MonoMethodRuntimeGenericContext *)g_hash_table_lookup(
        domain_info->mrgctx_hash, method);
  } else {
    g_assert(!method_inst->is_open);

    if (!domain_info->method_rgctx_hash)
      domain_info->method_rgctx_hash =
          g_hash_table_new(mrgctx_hash_func, mrgctx_equal_func);

    key.class_vtable = class_vtable;
    key.method_inst = method_inst;

    mrgctx = (MonoMethodRuntimeGenericContext *)g_hash_table_lookup(
        domain_info->method_rgctx_hash, &key);
  }
  ...
  if (!mrgctx) {
    ...
    mrgctx =
        (MonoMethodRuntimeGenericContext *)alloc_rgctx_array(domain, 0, TRUE);
    ...
    /*
    g_print ("mrgctx alloced for %s <", mono_type_get_full_name
    (class_vtable->klass)); for (i = 0; i < method_inst->type_argc; ++i) g_print
    ("%s, ", mono_type_full_name (method_inst->type_argv [i])); g_print (">\n");
    */
  }
...
static gboolean type_is_sharable(MonoType *type, gboolean allow_type_vars,
  ...
  /* Allow non ref arguments if they are primitive types or enums (partial
   * sharing). */
  if (allow_partial && !type->byref &&
  ...
  if (allow_partial && !type->byref && type->type == MONO_TYPE_GENERICINST &&
      MONO_TYPE_ISSTRUCT(type)) {
    MonoGenericClass *gclass = type->data.generic_class;

    if (gclass->context.class_inst &&
        !mini_generic_inst_is_sharable(gclass->context.class_inst,
...
gboolean mini_generic_inst_is_sharable(MonoGenericInst *inst,
 ...
 */
gboolean mono_is_partially_sharable_inst(MonoGenericInst *inst) {
  int i;
 ...
 * is sharable iff all of its type arguments are reference type, or some of them
 * have a reference type, and ALLOW_PARTIAL is TRUE.
 */
gboolean mono_generic_context_is_sharable_full(MonoGenericContext *context,
                                               gboolean allow_type_vars,
                                               gboolean allow_partial) {
  g_assert(context->class_inst || context->method_inst);

  if (context->class_inst &&
      !mini_generic_inst_is_sharable(context->class_inst, allow_type_vars,
                                     allow_partial))
    return FALSE;

  if (context->method_inst &&
      !mini_generic_inst_is_sharable(context->method_inst, allow_type_vars,
                                     allow_partial))
    return FALSE;
...
gboolean mono_generic_context_is_sharable(MonoGenericContext *context,
                                          gboolean allow_type_vars) {
  return mono_generic_context_is_sharable_full(context, allow_type_vars,
 ...
 * class.
 */
gboolean mono_method_is_generic_impl(MonoMethod *method) {
  if (method->is_inflated)
    return TRUE;
...
static gboolean has_constraints(MonoGenericContainer *container) {
  // int i;
...
static gboolean mini_method_is_open(MonoMethod *method) {
  if (method->is_inflated) {
...
static GENERATE_TRY_GET_CLASS_WITH_CACHE(iasync_state_machine,
  ...
  if (iclass && m_class_is_valuetype(klass) &&
      mono_class_is_assignable_from_internal(iclass, klass))
    return TRUE;
  return FALSE;
}

static G_GNUC_UNUSED gboolean is_async_method(MonoMethod *method) {
  ERROR_DECL(error);
  ...
  sig = mono_method_signature_internal(method);
  if (attr_class && sig &&
      ((sig->ret->type == MONO_TYPE_VOID) ||
       (sig->ret->type == MONO_TYPE_CLASS &&
        !strcmp(m_class_get_name(sig->ret->data.generic_class->container_class),
                "Task")) ||
       (sig->ret->type == MONO_TYPE_GENERICINST &&
        !strcmp(m_class_get_name(sig->ret->data.generic_class->container_class),
                "Task`1")))) {
    // printf ("X: %s\n", mono_method_full_name (method, TRUE));
 ...
 */
gboolean mono_method_is_generic_sharable_full(MonoMethod *method,
                                              gboolean allow_type_vars,
                                              gboolean allow_partial,
  ...
  */
  ...
  if (m_class_get_image(method->klass)->dynamic)
    /*
     * Enabling this causes corlib test failures because the JIT encounters
     * generic instances whose instance_size is 0.
     */
    allow_partial = FALSE;

  /*
   * Generic async methods have an associated state machine class which is a
   * generic struct. This struct is too large to be handled by gsharedvt so we
   * make it visible to the AOT compiler by disabling sharing of the async
   * method and the state machine class.
   */
  ...
  if (method->is_inflated) {
    ...
    if (!mono_generic_context_is_sharable_full(context, allow_type_vars,
                                               allow_partial))
      return FALSE;
    ...
    if (inflated->declaring->is_generic) {
      if (has_constraints(
              mono_method_get_generic_container(inflated->declaring)))
        return FALSE;
    }
  }

  if (mono_class_is_ginst(method->klass)) {
    if (!mono_generic_context_is_sharable_full(
            &mono_class_get_generic_class(method->klass)->context,
            allow_type_vars, allow_partial))
      return FALSE;

    g_assert(mono_class_get_generic_class(method->klass)->container_class &&
             mono_class_is_gtd(
                 mono_class_get_generic_class(method->klass)->container_class));

    if (has_constraints(mono_class_get_generic_container(
            mono_class_get_generic_class(method->klass)->container_class)))
      return FALSE;
  }
...
gboolean mono_method_is_generic_sharable(MonoMethod *method,
                                         gboolean allow_type_vars) {
  return mono_method_is_generic_sharable_full(
      method, allow_type_vars, partial_sharing_supported(), TRUE);
}
 ...
 * An rgctx argument is needed when the method is generic sharable, but it
 * doesn't have a this argument which can be used to load the rgctx.
 */
gboolean mono_method_needs_static_rgctx_invoke(MonoMethod *method,
                                               gboolean allow_type_vars) {
...
static MonoGenericInst *get_object_generic_inst(int type_argc) {
  MonoType **type_argv;
  int i;
 ...
 */
MonoGenericContext mono_method_construct_object_context(MonoMethod *method) {
  ...
  if (mono_method_get_context_general(method, TRUE)->method_inst) {
    int type_argc =
        mono_method_get_context_general(method, TRUE)->method_inst->type_argc;
...
void mono_set_generic_sharing_supported(gboolean supported) {
  gshared_supported = supported;
}

void mono_set_partial_sharing_supported(gboolean supported) {
 ...
 */
gboolean mono_class_generic_sharing_enabled(MonoClass *klass) {
...
static gboolean generic_inst_equal(MonoGenericInst *inst1,
                                   MonoGenericInst *inst2) {
  int i;
 ...
 * type arguments.
 */
gboolean mono_generic_context_equal_deep(MonoGenericContext *context1,
                                         MonoGenericContext *context2) {
 ...
 * passed to it.
 */
static MonoType *mini_get_basic_type_from_generic(MonoType *type) {
  if (!type->byref &&
      (type->type == MONO_TYPE_VAR || type->type == MONO_TYPE_MVAR) &&
    ...
    if (!constraint) {
      ...
      g_assert(constraint != m_class_get_byval_arg(
                                 m_class_get_parent(mono_defaults.int_class)));
      klass = mono_class_from_mono_type_internal(constraint);
      return m_class_get_byval_arg(klass);
    }
  } else {
    return mini_native_type_replace_type(
        mono_type_get_basic_type_from_generic(type));
  }
}
 ...
 *
 *   Return the underlying type of TYPE, taking into account enums, byref, bool,
 * char, ref types and generic sharing.
 */
...
void mono_generic_sharing_init(void) {
  mono_counters_register("RGCTX template num allocated",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_template_num_allocated);
  mono_counters_register("RGCTX template bytes allocated",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_template_bytes_allocated);
  mono_counters_register("RGCTX oti num allocated",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_oti_num_allocated);
  mono_counters_register("RGCTX oti bytes allocated",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_oti_bytes_allocated);
  mono_counters_register("RGCTX oti num markers",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_oti_num_markers);
  mono_counters_register("RGCTX oti num data",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_oti_num_data);
  mono_counters_register("RGCTX max slot number",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_max_slot_number);
  mono_counters_register("RGCTX num allocated",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_num_allocated);
  mono_counters_register("RGCTX num arrays allocated",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_num_arrays_allocated);
  mono_counters_register("RGCTX bytes allocated",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &rgctx_bytes_allocated);
  mono_counters_register("MRGCTX num arrays allocated",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &mrgctx_num_arrays_allocated);
  mono_counters_register("MRGCTX bytes allocated",
                         MONO_COUNTER_GENERICS | MONO_COUNTER_INT,
                         &mrgctx_bytes_allocated);
  mono_counters_register("GSHAREDVT num trampolines",
                         MONO_COUNTER_JIT | MONO_COUNTER_INT,
                         &gsharedvt_num_trampolines);

  mono_install_image_unload_hook(mono_class_unregister_image_generic_subclasses,
                                 NULL);
...
void mono_generic_sharing_cleanup(void) {
  mono_remove_image_unload_hook(mono_class_unregister_image_generic_subclasses,
                                NULL);
...
gboolean mini_type_var_is_vt(MonoType *type) {
  if (type->type == MONO_TYPE_VAR || type->type == MONO_TYPE_MVAR) {
    return type->data.generic_param->gshared_constraint &&
           (type->data.generic_param->gshared_constraint->type ==
                MONO_TYPE_VALUETYPE ||
            type->data.generic_param->gshared_constraint->type ==
...
gboolean mini_method_is_default_method(MonoMethod *m) {
  return MONO_CLASS_IS_INTERFACE_INTERNAL(m->klass) &&
...
gboolean mini_method_needs_mrgctx(MonoMethod *m) {
  if (mono_class_is_ginst(m->klass) && mini_method_is_default_method(m))
    return TRUE;
  return (mini_method_get_context(m) &&
          mini_method_get_context(m)->method_inst);
}
...
gpointer mini_method_get_rgctx(MonoMethod *m) {
  ERROR_DECL(error);
  MonoVTable *vt =
      mono_class_vtable_checked(mono_domain_get(), m->klass, error);
  mono_error_assert_ok(error);
 ...
 * Should be used in place of MONO_TYPE_ISSTRUCT () which can't handle
 * gsharedvt.
 */
...
gboolean mini_class_is_generic_sharable(MonoClass *klass) {
  ...
  return (mono_class_is_ginst(klass) &&
          mono_generic_context_is_sharable(
              &mono_class_get_generic_class(klass)->context, FALSE));
}
...
gboolean mini_is_gsharedvt_gparam(MonoType *t) {
  /* Matches get_gsharedvt_type () */
  return (t->type == MONO_TYPE_VAR || t->type == MONO_TYPE_MVAR) &&
         t->data.generic_param->gshared_constraint &&
         t->data.generic_param->gshared_constraint->type == MONO_TYPE_VALUETYPE;
}

static char *get_shared_gparam_name(MonoTypeEnum constraint, const char *name) {
...
static guint shared_gparam_hash(gconstpointer data) {
  MonoGSharedGenericParam *p = (MonoGSharedGenericParam *)data;
  guint hash;

  hash = mono_metadata_generic_param_hash(p->parent);
  hash = ((hash << 5) - hash) ^
         mono_metadata_type_hash(p->param.gshared_constraint);

  return hash;
}

static gboolean shared_gparam_equal(gconstpointer ka, gconstpointer kb) {
  ...
  if (p1->parent != p2->parent)
    return FALSE;
  if (!mono_metadata_type_equal(p1->param.gshared_constraint,
                                p2->param.gshared_constraint))
    return FALSE;
  return TRUE;
}
 ...
 *
 *   Create an anonymous gparam from T with a constraint which encodes which
 * types can match it.
 */
MonoType *mini_get_shared_gparam(MonoType *t, MonoType *constraint) {
  ...
  set = mono_metadata_merge_image_sets(
      mono_metadata_get_image_set_for_type(t),
  ...
  if (!set->gshared_types[constraint->type])
    set->gshared_types[constraint->type] =
        g_hash_table_new(shared_gparam_hash, shared_gparam_equal);
  res = (MonoType *)g_hash_table_lookup(set->gshared_types[constraint->type],
                                        &key);
  mono_image_set_unlock(set);
  if (res)
    return res;
  copy = (MonoGSharedGenericParam *)mono_image_set_alloc0(
      set, sizeof(MonoGSharedGenericParam));
  ...
  name = get_shared_gparam_name(constraint->type,
                                ((MonoGenericParamFull *)copy)->info.name);
...
static MonoGenericInst *get_shared_inst(MonoGenericInst *inst,
                                        MonoGenericInst *shared_inst,
                                        MonoGenericContainer *container,
                                        gboolean use_gsharedvt);

static MonoType *get_shared_type(MonoType *t, MonoType *type) {
  MonoTypeEnum ttype;

  if (!type->byref && type->type == MONO_TYPE_GENERICINST &&
      MONO_TYPE_ISSTRUCT(type)) {
    ERROR_DECL(error);
    MonoGenericClass *gclass = type->data.generic_class;
    MonoGenericContext context;
    ...
    if (gclass->context.class_inst)
      context.class_inst = get_shared_inst(
          gclass->context.class_inst,
          mono_class_get_generic_container(gclass->container_class)
              ->context.class_inst,
          NULL, FALSE);
    if (gclass->context.method_inst)
      context.method_inst = get_shared_inst(
          gclass->context.method_inst,
          mono_class_get_generic_container(gclass->container_class)
              ->context.method_inst,
          NULL, FALSE);

    k = mono_class_inflate_generic_class_checked(gclass->container_class,
                                                 &context, error);
    mono_error_assert_ok(error); /* FIXME don't swallow the error */
  ...
  /* Create a type variable with a constraint which encodes which types can
   * match it */
  ttype = type->type;
  if (type->type == MONO_TYPE_VALUETYPE) {
    ...
    ttype = mono_class_enum_basetype_internal(
    ...
    if (type->data.generic_param->gshared_constraint)
      return mini_get_shared_gparam(
          t, type->data.generic_param->gshared_constraint);
    ttype = MONO_TYPE_OBJECT;
  }
...
static MonoType *get_gsharedvt_type(MonoType *t) {
  /* Use TypeHandle as the constraint type since its a valuetype */
  return mini_get_shared_gparam(
      t, m_class_get_byval_arg(mono_defaults.typehandle_class));
}
...
static MonoGenericInst *get_shared_inst(MonoGenericInst *inst,
  ...
  for (i = 0; i < inst->type_argc; ++i) {
    if (use_gsharedvt) {
      ...
      type_argv[i] =
          get_shared_type(shared_inst->type_argv[i], inst->type_argv[i]);
    }
  }
 ...
 *
 * \returns The method which is actually compiled/registered when doing generic
 sharing.

 * If flags & SHARE_MODE_GSHAREDVT, produce a method using the gsharedvt
 instantiation.
 * \p method can be a non-inflated generic method.
 */
MonoMethod *mini_get_shared_method_full(MonoMethod *method,
  ...
  /*
   * Instead of creating a shared version of the wrapper, create a shared
   * version of the original method and construct a wrapper for it. Otherwise,
   * we could end up with two copies of the same wrapper, breaking AOT which
   * assumes wrappers are unique.
  ...
  case MONO_WRAPPER_DELEGATE_INVOKE: {
    if (info->subtype == WRAPPER_SUBTYPE_NONE) {
      MonoMethod *ginvoke = mini_get_shared_method_full(
          info->d.delegate_invoke.method, flags, error);
      return_val_if_nok(error, NULL);
  ...
  if (method->is_generic ||
      (mono_class_is_gtd(method->klass) && !method->is_inflated)) {
  ...
  if (declaring_method->is_generic)
    shared_context =
        mono_method_get_generic_container(declaring_method)->context;
  else
    shared_context =
        mono_class_get_generic_container(declaring_method->klass)->context;
  ...
  class_container = mono_class_try_get_generic_container(
      declaring_method->klass); // FIXME is this a case for a try_get?
  ...
  if (inst)
    shared_context.class_inst = get_shared_inst(
        inst, shared_context.class_inst, class_container, use_gsharedvt_inst);
  ...
  if (inst)
    shared_context.method_inst = get_shared_inst(
        inst, shared_context.method_inst, method_container, use_gsharedvt_inst);

  return mono_class_inflate_generic_method_checked(declaring_method,
                                                   &shared_context, error);
}
...
int mini_get_rgctx_entry_slot(MonoJumpInfoRgctxEntry *entry) {
  ...
  case MONO_PATCH_INFO_GSHAREDVT_CALL: {
    MonoJumpInfoGSharedVtCall *call_info =
        (MonoJumpInfoGSharedVtCall *)g_malloc0(sizeof(
            MonoJumpInfoGSharedVtCall)); // mono_domain_alloc0 (domain, sizeof
                                         // (MonoJumpInfoGSharedVtCall));

    memcpy(call_info, entry->data->data.gsharedvt,
  ...
  case MONO_PATCH_INFO_GSHAREDVT_METHOD: {
    ...
    info = (MonoGSharedVtMethodInfo *)g_malloc0(
        sizeof(MonoGSharedVtMethodInfo)); // mono_domain_alloc0 (domain, sizeof
                                          // (MonoGSharedVtMethodInfo));
    info->method = oinfo->method;
    info->num_entries = oinfo->num_entries;
    info->entries = (MonoRuntimeGenericContextInfoTemplate *)g_malloc0(
  ...
  case MONO_PATCH_INFO_DELEGATE_TRAMPOLINE: {
    ...
    info = (MonoDelegateClassMethodPair *)g_malloc0(
  ...
  if (entry->in_mrgctx)
    return lookup_or_register_info(
        entry->d.method->klass, entry->d.method, entry->in_mrgctx, entry_data,
        entry->info_type, mono_method_get_context(entry->d.method));
  else
    return lookup_or_register_info(entry->d.klass, NULL, entry->in_mrgctx,
                                   entry_data, entry->info_type,
                                   mono_class_get_context(entry->d.klass));
}

static gboolean gsharedvt_supported;

void mono_set_generic_sharing_vt_supported(gboolean supported) {
 ...
 *
 *   Return whenever T references type arguments instantiated with gshared
 * vtypes.
 */
 ...
 *
 *   Return whenever T refers to a GSHAREDVT type whose size differs depending
 * on the values of type parameters.
 */
gboolean mini_is_gsharedvt_variable_type(MonoType *t) {
  ...
  if (t->type == MONO_TYPE_GENERICINST) {
    ...
    if (m_class_get_byval_arg(t->data.generic_class->container_class)->type !=
            MONO_TYPE_VALUETYPE ||
...
static gboolean is_variable_size(MonoType *t) {
  int i;
  ...
  if (t->type == MONO_TYPE_VAR || t->type == MONO_TYPE_MVAR) {
    ...
    if (param->gshared_constraint &&
        param->gshared_constraint->type != MONO_TYPE_VALUETYPE &&
        param->gshared_constraint->type != MONO_TYPE_GENERICINST)
      return FALSE;
    if (param->gshared_constraint &&
        param->gshared_constraint->type == MONO_TYPE_GENERICINST)
  ...
  if (t->type == MONO_TYPE_GENERICINST &&
      m_class_get_byval_arg(t->data.generic_class->container_class)->type ==
          MONO_TYPE_VALUETYPE) {
    MonoGenericClass *gclass = t->data.generic_class;
    MonoGenericContext *context = &gclass->context;
    MonoGenericInst *inst;
...
gboolean mini_is_gsharedvt_sharable_inst(MonoGenericInst *inst) {
  ...
  for (i = 0; i < inst->type_argc; ++i) {
    ...
    if ((MONO_TYPE_IS_REFERENCE(type) || type->type == MONO_TYPE_VAR ||
...
gboolean mini_is_gsharedvt_sharable_method(MonoMethod *method) {
  ...
  if (method->is_inflated) {
    ...
    if (context->class_inst && context->method_inst) {
      /* At least one inst has to be gsharedvt sharable, and the other normal or
       * gsharedvt sharable */
      ...
      if ((vt1 && vt2) ||
          (vt1 &&
           mini_generic_inst_is_sharable(context->method_inst, TRUE, FALSE)) ||
          (vt2 &&
           mini_generic_inst_is_sharable(context->class_inst, TRUE, FALSE)))
        ;
      else
        return FALSE;
    } else {
  ...
  sig = mono_method_signature_internal(
 ...
 *
 *   Return whenever the calling convention used to call SIG varies depending on
 * the values of type parameters used by SIG, i.e. FALSE for swap(T[] arr, int
 * i, int j), TRUE for T get_t ().
 */
...
MonoMethod *mini_method_to_shared(MonoMethod *method) {
  if (!mono_method_is_generic_impl(method))
    return NULL;
...
gboolean mini_is_gsharedvt_type(MonoType *t) { return FALSE; }

gboolean mini_is_gsharedvt_klass(MonoClass *klass) { return FALSE; }

gboolean mini_is_gsharedvt_signature(MonoMethodSignature *sig) { return FALSE; }

gboolean mini_is_gsharedvt_variable_type(MonoType *t) { return FALSE; }

gboolean mini_is_gsharedvt_sharable_method(MonoMethod *method) { return FALSE; }

gboolean mini_is_gsharedvt_variable_signature(MonoMethodSignature *sig) {
  return FALSE;
}

MonoMethod *mini_method_to_shared(MonoMethod *method) { return NULL; }
