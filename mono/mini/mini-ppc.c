...
static GENERATE_TRY_GET_CLASS_WITH_CACHE(
    math, "System",
    "Math") static GENERATE_TRY_GET_CLASS_WITH_CACHE(mathf, "System", "MathF")
...
#define MONO_EMIT_NEW_LOAD_R8(cfg, dr, addr)                                   \
  do {                                                                         \
    MonoInst *inst;                                                            \
...
static guint8 *emit_memcpy(guint8 *code, int size, int dreg, int doffset,
  ...
  if (size > sizeof(target_mgreg_t) * 5) {
    ...
    ppc_ldptr_update(code, ppc_r0, (unsigned int)sizeof(target_mgreg_t),
                     ppc_r12);
    ppc_stptr_update(code, ppc_r0, (unsigned int)sizeof(target_mgreg_t),
                     ppc_r11);
    copy_loop_jump = code;
  ...
  if ((cpu_hw_caps & PPC_MULTIPLE_LS_UNITS) && (dreg != ppc_r11) &&
      (sreg != ppc_r11)) {
    while (size >= 16) {
...
#ifdef __mono_ppc64__
static gboolean is_load_sequence(guint32 *seq) {
...
/* ld || lwz */
#define ppc_is_load_op(opcode)                                                 \
  (ppc_opcode((opcode)) == 58 || ppc_opcode((opcode)) == 32)
...
gboolean mono_ppc_is_direct_call_sequence(guint32 *code) {
  ...
  if (ppc_opcode(code[-1]) == 31) {                             /* mtlr */
    if (ppc_is_load_op(code[-2]) && ppc_is_load_op(code[-3])) { /* ld/ld */
      ...
      return (ppc_load_get_dest(code[-2]) == ppc_r2 &&
              ppc_load_get_off(code[-2]) == sizeof(target_mgreg_t)) ||
             (ppc_load_get_dest(code[-3]) == ppc_r2 &&
              ppc_load_get_off(code[-3]) == sizeof(target_mgreg_t));
    }
  ...
  return ppc_opcode(code[-1]) == 31 && ppc_opcode(code[-2]) == 24 &&
...
static guint8 *get_delegate_invoke_impl(MonoTrampInfo **info,
                                        gboolean has_target,
                                        guint32 param_count, gboolean aot) {
  guint8 *code, *start;
  ...
  if (has_target) {
    ...
    ppc_ldptr(code, ppc_r0, MONO_STRUCT_OFFSET(MonoDelegate, method_ptr),
              ppc_r3);
...
gpointer mono_arch_get_this_arg_from_call(host_mgreg_t *r, guint8 *code) {
 ...
 * PPC_FEATURE_CELL, PPC_FEATURE_PA6T, PPC_FEATURE_ARCH_2_05 are considered
 * supporting 2X ISA features
 */
#define ISA_2X                                                                 \
 ...
 */
void mono_arch_cleanup(void) { mono_os_mutex_destroy(&mini_arch_mutex); }

gboolean mono_arch_have_fast_tls(void) { return FALSE; }
...
enum {
  ...
  RegTypeStructByVal,
  RegTypeStructByAddr,
  RegTypeFPStructByVal, // For the v2 ABI, floats should be passed in FRs
                        // instead of GRs.  Only valid for ABI v2!
};
...
typedef struct {
  ...
  guint8 reg;
  guint8 vtregs;      /* number of registers used to pass a
                         RegTypeStructByVal/RegTypeFPStructByVal */
  guint8 regtype : 4; /* 0 general, 1 basereg, 2 floating point register, see
                         RegType* */
  guint8 size : 4;    /* 1, 2, 4, 8, or regs used by
                         RegTypeStructByVal/RegTypeFPStructByVal */
  guint8 bytes : 4;   /* size in bytes - only valid for
                             RegTypeStructByVal/RegTypeFPStructByVal if the struct
                         fits   in one word, otherwise it's 0*/
} ArgInfo;
...
//
// Test if a structure is completely composed of either float XOR double fields
// and has fewer than PPC_MOST_FLOAT_STRUCT_MEMBERS_TO_RETURN_VIA_REGISTER
// members. If this is true the structure can be returned directly via float
// registers instead of by a hidden parameter pointing to where the return value
// should be stored. This is as per the ELF ABI v2.
//
static gboolean is_float_struct_returnable_via_regs(MonoType *type,
                                                    int *member_cnt,
                                                    int *member_size) {
  ...
  return is_all_floats &&
         (*member_cnt <= PPC_MOST_FLOAT_STRUCT_MEMBERS_TO_RETURN_VIA_REGISTERS);
}
#else
...
//
// Test if a structure is smaller in size than 2 doublewords
// (PPC_LARGEST_STRUCT_SIZE_TO_RETURN_VIA_REGISTERS) and is completely composed
// of fields all of basic types. If this is true the structure can be returned
// directly via registers r3/r4 instead of by a hidden parameter pointing to
// where the return value should be stored. This is as per the ELF ABI v2.
//
static gboolean is_struct_returnable_via_regs(MonoClass *klass,
                                              gboolean is_pinvoke) {
  ...
  if (klass) {
    ...
    while ((f = mono_class_get_fields_internal(klass, &iter))) {
      if (!(f->type->attrs & FIELD_ATTRIBUTE_STATIC)) {
        ...
        if (f->type->byref) {
          return FALSE;
        } else if ((f->type->type >= MONO_TYPE_BOOLEAN) &&
                   (f->type->type <= MONO_TYPE_R8)) {
          has_a_field = TRUE;
...
static void inline add_general(guint *gr, guint *stack_size, ArgInfo *ainfo,
  ...
  (*gr)++;
    ...
    (defined(__mono_ppc64__) && !PPC_PASS_SMALL_FLOAT_STRUCTS_IN_FR_REGS)
static gboolean has_only_a_r48_field(MonoClass *klass) {
  gpointer iter;
  MonoClassField *f;
  ...
  while ((f = mono_class_get_fields_internal(klass, &iter))) {
    if (!(f->type->attrs & FIELD_ATTRIBUTE_STATIC)) {
      if (have_field)
        return FALSE;
      if (!f->type->byref &&
          (f->type->type == MONO_TYPE_R4 || f->type->type == MONO_TYPE_R8))
        have_field = TRUE;
      else
        return FALSE;
    }
...
static CallInfo *get_call_info(MonoMethodSignature *sig) {
  ...
  for (i = pstart; i < sig->param_count; ++i) {
    ...
    (defined(__mono_ppc64__) && !PPC_PASS_SMALL_FLOAT_STRUCTS_IN_FR_REGS)
...
#if PPC_PASS_STRUCTS_BY_VALUE
      {
        ...
        int mbr_cnt = 0;
        int mbr_size = 0;
        gboolean is_all_floats = is_float_struct_returnable_via_regs(
            sig->params[i], &mbr_cnt, &mbr_size);

        if (is_all_floats) {
          rest = PPC_LAST_FPARG_REG - fr + 1;
        }
        // Pass small (<= 8 member) structures entirely made up of either float
        // or double members in FR registers.  There have to be at least mbr_cnt
        // registers left.
        if (is_all_floats && (rest >= mbr_cnt)) {
          nregs = mbr_cnt;
...
#endif
        {
          ...
          align_size &= ~(sizeof(target_mgreg_t) - 1);
          nregs = (align_size + sizeof(target_mgreg_t) - 1) /
    ...
    case MONO_TYPE_R4:
      ...
      if (fr <= PPC_LAST_FPARG_REG
        ...
        cinfo->args[n].offset =
            PPC_STACK_PARAM_OFFSET + stack_size + MONO_PPC_32_64_CASE(0, 4);
        cinfo->args[n].regtype = RegTypeBase;
        cinfo->args[n].reg = ppc_sp; /* in the caller*/
...
gboolean mono_arch_tailcall_supported(MonoCompile *cfg,
  ...
  gboolean res =
      IS_SUPPORTED_TAILCALL(callee_info->stack_usage <=
                            caller_info->stack_usage) &&
      IS_SUPPORTED_TAILCALL(memcmp(&callee_info->ret, &caller_info->ret,
                                   sizeof(caller_info->ret)) == 0);
  ...
  for (int i = 0; res && i < callee_info->nargs; ++i) {
    res = IS_SUPPORTED_TAILCALL(callee_info->args[i].regtype !=
                                RegTypeStructByAddr);
...
void mono_arch_allocate_vars(MonoCompile *m) {
   ...
   * non-varargs signatures. Since there is little hope people will get this
   * right we assume they won't.
   */
  ...
  offsets = mono_allocate_stack_slots(m, FALSE, &locals_stack_size,
                                      &locals_stack_align);
  ...
  for (i = 0; i < sig->param_count; ++i) {
    ...
    if (inst->opcode != OP_REGVAR) {
      ...
      /*
       * Use at least 4/8 byte alignment, since these might be passed in
       * registers, and they are saved using std in the prolog.
       */
 ...
 * mono_arch_call_opcode, currently alignment in mono_arch_call_opcode is
 * computed without arch_get_argument_info
 */
...
static void emit_sig_cookie(MonoCompile *cfg, MonoCallInst *call,
  ...
  MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, ppc_r1,
...
void mono_arch_emit_call(MonoCompile *cfg, MonoCallInst *call) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (ainfo->regtype == RegTypeGeneral) {
      ...
      if (!t->byref &&
          ((t->type == MONO_TYPE_I8) || (t->type == MONO_TYPE_U8))) {
        MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI8_MEMBASE_REG, ppc_r1,
                                    ainfo->offset, in->dreg);
      } else if (!t->byref &&
                 ((t->type == MONO_TYPE_R4) || (t->type == MONO_TYPE_R8))) {
        if (t->type == MONO_TYPE_R8)
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, ppc_r1,
                                      ainfo->offset, in->dreg);
        else
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER4_MEMBASE_REG, ppc_r1,
                                      ainfo->offset, in->dreg);
      } else {
        MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, ppc_r1,
                                    ainfo->offset, in->dreg);
  ...
  if (cinfo->struct_ret) {
    ...
    mono_call_inst_add_outarg_reg(cfg, call, vtarg->dreg, cinfo->struct_ret,
                                  FALSE);
  }

  call->stack_usage = cinfo->stack_usage;
  cfg->param_area = MAX(PPC_MINIMAL_PARAM_AREA_SIZE,
                        MAX(cfg->param_area, cinfo->stack_usage));
...
void mono_arch_emit_outarg_vt(MonoCompile *cfg, MonoInst *ins, MonoInst *src) {
  ...
  if (ainfo->regtype == RegTypeStructByVal) {
    ...
    if (size == 2 || size == 1) {
      ...
      if (size == 1)
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI1_MEMBASE, tmpr, src->dreg,
                                      soffset);
      else
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI2_MEMBASE, tmpr, src->dreg,
                                      soffset);
      ...
      for (i = 0; i < ainfo->vtregs; ++i) {
        ...
        if (antipadding)
          MONO_EMIT_NEW_BIALU_IMM(cfg, OP_SHR_UN_IMM, dreg, dreg,
                                  antipadding * 8);
#else
      MONO_EMIT_NEW_LOAD_MEMBASE(cfg, dreg, src->dreg, soffset);
#endif
    ...
    if (ovf_size != 0)
      mini_emit_memcpy(cfg, ppc_r1, doffset + soffset, src->dreg, soffset,
                       ovf_size * sizeof(target_mgreg_t), TARGET_SIZEOF_VOID_P);
  } else if (ainfo->regtype == RegTypeFPStructByVal) {
    soffset = 0;
    for (i = 0; i < ainfo->vtregs; ++i) {
      int tmpr = mono_alloc_freg(cfg);
      if (ainfo->size == 4)
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADR4_MEMBASE, tmpr, src->dreg,
                                      soffset);
      else // ==8
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADR8_MEMBASE, tmpr, src->dreg,
                                      soffset);
    ...
    MonoInst *vtcopy = mono_compile_create_var(
        cfg, m_class_get_byval_arg(src->klass), OP_LOCAL);
    MonoInst *load;
    guint32 size;
    ...
    if (call->signature->pinvoke) {
      size =
          mono_type_native_stack_size(m_class_get_byval_arg(src->klass), NULL);
...
#define EMIT_COND_BRANCH_FLAGS(ins, b0, b1)                                    \
  if (0 && ins->inst_true_bb->native_offset) {                                 \
    ...
    if (!ppc_is_imm16(br_disp + 8 * 1024) ||                                   \
        !ppc_is_imm16(br_disp - 8 * 1024)) {                                   \
      MonoOvfJump *ovfj =                                                      \
          mono_mempool_alloc(cfg->mempool, sizeof(MonoOvfJump));               \
      ovfj->data.bb = ins->inst_true_bb;                                       \
      ovfj->ip_offset = 0;                                                     \
      ovfj->b0_cond = (b0);                                                    \
      ovfj->b1_cond = (b1);                                                    \
      mono_add_patch_info(cfg, code - cfg->native_code,                        \
                          MONO_PATCH_INFO_BB_OVF, ovfj);                       \
      ppc_b(code, 0);                                                          \
    } else {                                                                   \
...
#define EMIT_COND_BRANCH(ins, cond)                                            \
  EMIT_COND_BRANCH_FLAGS(ins, branch_b0_table[(cond)], branch_b1_table[(cond)])
...
#define EMIT_COND_SYSTEM_EXCEPTION_FLAGS(b0, b1, exc_name)                     \
  do {                                                                         \
    ...
    if (!ppc_is_imm16(br_disp + 1024) ||                                       \
        !ppc_is_imm16(ppc_is_imm16(br_disp - 1024))) {                         \
      MonoOvfJump *ovfj =                                                      \
          mono_mempool_alloc(cfg->mempool, sizeof(MonoOvfJump));               \
...
#define EMIT_COND_SYSTEM_EXCEPTION(cond, exc_name)                             \
  EMIT_COND_SYSTEM_EXCEPTION_FLAGS(branch_b0_table[(cond)],                    \
                                   branch_b1_table[(cond)], (exc_name))

void mono_arch_peephole_pass_1(MonoCompile *cfg, MonoBasicBlock *bb) {}

static int normalize_opcode(int opcode) {
  switch (opcode) {
...
void mono_arch_peephole_pass_2(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS_SAFE(bb, n, ins) {
    ...
    case OP_LOAD_MEMBASE:
       ...
       */
      if (last_ins &&
          normalize_opcode(last_ins->opcode) == OP_STORE_MEMBASE_REG &&
         ...
         */
                 ...
                 normalize_opcode(last_ins->opcode) == OP_LOAD_MEMBASE &&
    ...
    case OP_LOADI4_MEMBASE:
      if (last_ins && (last_ins->opcode == OP_STOREI4_MEMBASE_REG) &&
          ins->inst_basereg == last_ins->inst_destbasereg &&
          ins->inst_offset == last_ins->inst_offset) {
        ins->opcode = (ins->opcode == OP_LOADI4_MEMBASE) ? OP_ICONV_TO_I4
                                                         : OP_ICONV_TO_U4;
        ins->sreg1 = last_ins->sreg1;
      }
      break;
...
void mono_arch_decompose_opts(MonoCompile *cfg, MonoInst *ins) {
  ...
  case OP_ICONV_TO_R_UN: {
    // This value is OK as-is for both big and little endian because of how it
    // is stored
    ...
    if (!ppc_is_imm16(offset + 4)) {
      ...
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_IADD_IMM, basereg, cfg->frame_reg,
                              offset);
    }
#if G_BYTE_ORDER == G_BIG_ENDIAN
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG, basereg, offset,
                                msw_reg);
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG, basereg,
                                offset + 4, ins->sreg1);
#else
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADR8_MEMBASE, tmp_reg, basereg,
                                  offset);
  ...
  case OP_CKFINITE: {
    ...
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORER8_MEMBASE_REG, basereg, offset,
                                ins->sreg1);
#if G_BYTE_ORDER == G_BIG_ENDIAN
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI4_MEMBASE, msw_reg, basereg,
                                  offset);
#else
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI4_MEMBASE, msw_reg, basereg,
                                  offset + 4);
  ...
  case OP_ISUB_OVF: {
    ...
    MONO_EMIT_NEW_BIALU(cfg, ins->opcode, result_shifted_reg, shifted1_reg,
                        shifted2_reg);
    if (ins->opcode == OP_IADD_OVF_UN)
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_SHR_UN_IMM, ins->dreg, result_shifted_reg,
                              32);
    else
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_SHR_IMM, ins->dreg, result_shifted_reg,
                              32);
    ins->opcode = OP_NOP;
    break;
  }
...
void mono_arch_decompose_long_opts(MonoCompile *cfg, MonoInst *ins) {
  ...
  case OP_LADD_OVF:
    ...
    MONO_EMIT_NEW_BIALU(cfg, OP_ADDCC, MONO_LVREG_LS(ins->dreg),
                        MONO_LVREG_LS(ins->sreg1), MONO_LVREG_LS(ins->sreg2));
    MONO_EMIT_NEW_BIALU(cfg, OP_ADD_OVF_CARRY, MONO_LVREG_MS(ins->dreg),
                        MONO_LVREG_MS(ins->sreg1), MONO_LVREG_MS(ins->sreg2));
    NULLIFY_INS(ins);
    break;
  case OP_LADD_OVF_UN:
    /* ADC sets the condition code */
    MONO_EMIT_NEW_BIALU(cfg, OP_ADDCC, MONO_LVREG_LS(ins->dreg),
                        MONO_LVREG_LS(ins->sreg1), MONO_LVREG_LS(ins->sreg2));
    MONO_EMIT_NEW_BIALU(cfg, OP_ADD_OVF_UN_CARRY, MONO_LVREG_MS(ins->dreg),
                        MONO_LVREG_MS(ins->sreg1), MONO_LVREG_MS(ins->sreg2));
    NULLIFY_INS(ins);
    break;
  case OP_LSUB_OVF:
    /* SBB sets the condition code */
    MONO_EMIT_NEW_BIALU(cfg, OP_SUBCC, MONO_LVREG_LS(ins->dreg),
                        MONO_LVREG_LS(ins->sreg1), MONO_LVREG_LS(ins->sreg2));
    MONO_EMIT_NEW_BIALU(cfg, OP_SUB_OVF_CARRY, MONO_LVREG_MS(ins->dreg),
                        MONO_LVREG_MS(ins->sreg1), MONO_LVREG_MS(ins->sreg2));
    NULLIFY_INS(ins);
    break;
  case OP_LSUB_OVF_UN:
    ...
    MONO_EMIT_NEW_BIALU(cfg, OP_SUBCC, MONO_LVREG_LS(ins->dreg),
                        MONO_LVREG_LS(ins->sreg1), MONO_LVREG_LS(ins->sreg2));
    MONO_EMIT_NEW_BIALU(cfg, OP_SUB_OVF_UN_CARRY, MONO_LVREG_MS(ins->dreg),
                        MONO_LVREG_MS(ins->sreg1), MONO_LVREG_MS(ins->sreg2));
    NULLIFY_INS(ins);
    break;
  case OP_LNEG:
    /* From gcc generated code */
    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_PPC_SUBFIC, MONO_LVREG_LS(ins->dreg),
                            MONO_LVREG_LS(ins->sreg1), 0);
    MONO_EMIT_NEW_UNALU(cfg, OP_PPC_SUBFZE, MONO_LVREG_MS(ins->dreg),
                        MONO_LVREG_MS(ins->sreg1));
    NULLIFY_INS(ins);
    break;
  default:
    break;
...
case CEE_BLT_UN:
 */
static const guchar branch_b0_table[] = {
    PPC_BR_TRUE,  PPC_BR_FALSE, PPC_BR_TRUE, PPC_BR_FALSE, PPC_BR_TRUE,

    PPC_BR_FALSE, PPC_BR_FALSE, PPC_BR_TRUE, PPC_BR_FALSE, PPC_BR_TRUE};

static const guchar branch_b1_table[] = {
    PPC_BR_EQ, PPC_BR_LT, PPC_BR_GT, PPC_BR_GT, PPC_BR_LT,

    PPC_BR_EQ, PPC_BR_LT, PPC_BR_GT, PPC_BR_GT, PPC_BR_LT};

#define NEW_INS(cfg, dest, op)                                                 \
  do {                                                                         \
...
//#define map_to_reg_reg_op(op) (cfg->new_ir? mono_op_imm_to_op (op):
//map_to_reg_reg_op (op))
...
void mono_arch_lowering_pass(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  loop_start:
    ...
    case OP_ICOMPARE_IMM:
      ...
      if (!next || (!(MONO_IS_COND_BRANCH_OP(next) || MONO_IS_COND_EXC(next) ||
                      MONO_IS_SETCC(next)))) {
        ins->opcode = OP_NOP;
        break;
      }
    ...
    case OP_R4CONST:
      ...
      ins->inst_offset = 0;
      ins->opcode =
          ins->opcode == OP_R4CONST ? OP_LOADR4_MEMBASE : OP_LOADR8_MEMBASE;
      last_ins = temp;
...
static void patch_ins(guint8 *code, guint32 ins) {
  *(guint32 *)code = ins;
  mono_arch_flush_icache(code, 4);
}

static void ppc_patch_full(MonoCompile *cfg, MonoDomain *domain, guchar *code,
                           const guchar *target, gboolean is_fd) {
  ...
  if (prim == 15 || ins == 0x4e800021 || ins == 0x4e800020 ||
      ins == 0x4e800420) {
#ifdef __mono_ppc64__
    ...
    if (ins == 0x4e800021 || ins == 0x4e800020 || ins == 0x4e800420) {
      branch_ins = seq;
      if (ppc_is_load_op(seq[-3]) ||
          ppc_opcode(seq[-3]) == 31) /* ld || lwz || mr */
        code -= 32;
      else
        code -= 24;
    } else {
    ...
    g_assert(seq[3] == 0x4e800021 || seq[3] == 0x4e800020 ||
             seq[3] == 0x4e800420);
    /* FIXME: make this thread safe */
...
void mono_ppc_patch(guchar *code, const guchar *target) {
...
static guint8 *emit_reserve_param_area(MonoCompile *cfg, guint8 *code) {
...
static guint8 *emit_unreserve_param_area(MonoCompile *cfg, guint8 *code) {
  long size = cfg->param_area;
...
void mono_arch_output_basic_block(MonoCompile *cfg, MonoBasicBlock *bb) {
  ...
  MONO_BB_FOR_EACH_INS(bb, ins) {
    ...
    case OP_STOREI1_MEMBASE_REG:
      if (ppc_is_imm16(ins->inst_offset)) {
        ...
        if (ppc_is_imm32(ins->inst_offset)) {
          ppc_addis(code, ppc_r11, ins->inst_destbasereg,
                    ppc_ha(ins->inst_offset));
    ...
    case OP_LOAD_MEMBASE:
      if (ppc_is_imm16(ins->inst_offset)) {
        ...
        if (ppc_is_imm32(ins->inst_offset) && (ins->dreg > 0)) {
          ppc_addis(code, ins->dreg, ins->inst_basereg,
                    ppc_ha(ins->inst_offset));
    ...
    case OP_ICOMPARE_IMM:
      CASE_PPC64(OP_LCOMPARE_IMM)
      L = (sizeof(target_mgreg_t) == 4 || ins->opcode == OP_ICOMPARE_IMM) ? 0
                                                                          : 1;
      next = ins->next;
      if (next && compare_opcode_is_unsigned(next->opcode)) {
        if (ppc_is_uimm16(ins->inst_imm)) {
    ...
    case OP_XOR_IMM:
      ...
      if (!(ins->inst_imm & 0xffff0000)) {
        ...
        ppc_xoris(code, ins->sreg1, ins->dreg,
                  ((guint32)(ins->inst_imm) >> 16));
      } else {
        g_assert_not_reached();
      }
      break;
    ...
    case OP_ISHL_IMM:
      ...
      ppc_shift_left_imm(code, ins->dreg, ins->sreg1,
                         MASK_SHIFT_IMM(ins->inst_imm));
      break;
    case OP_ISHR:
      ppc_sraw(code, ins->dreg, ins->sreg1, ins->sreg2);
      break;
    case OP_SHR_IMM:
      ppc_shift_right_arith_imm(code, ins->dreg, ins->sreg1,
                                MASK_SHIFT_IMM(ins->inst_imm));
      break;
    case OP_SHR_UN_IMM:
      if (MASK_SHIFT_IMM(ins->inst_imm))
        ppc_shift_right_imm(code, ins->dreg, ins->sreg1,
                            MASK_SHIFT_IMM(ins->inst_imm));
      else
    ...
    case OP_GOT_ENTRY:
      ...
      mono_add_patch_info(cfg, offset,
                          (MonoJumpInfoType)(intptr_t)ins->inst_right->inst_i1,
      ...
      break;
    case OP_AOTCONST:
      mono_add_patch_info(cfg, offset, (MonoJumpInfoType)(intptr_t)ins->inst_i1,
                          ins->inst_p0);
    ...
    case OP_TAILCALL: {
      ...
      if (call->stack_usage) {
        code =
            emit_memcpy(code, call->stack_usage, ppc_r12,
                        PPC_STACK_PARAM_OFFSET, ppc_sp, PPC_STACK_PARAM_OFFSET);
        /* r12 was clobbered */
    ...
    case OP_COND_EXC_LE_UN:
      EMIT_COND_SYSTEM_EXCEPTION(ins->opcode - OP_COND_EXC_EQ,
    ...
    case OP_COND_EXC_ILE_UN:
      EMIT_COND_SYSTEM_EXCEPTION(ins->opcode - OP_COND_EXC_IEQ,
                                 (const char *)ins->inst_p1);
      break;
    case OP_IBEQ:
    case OP_IBNE_UN:
    ...
    case OP_LCONV_TO_OVF_I: {
      ...
      NOT_IMPLEMENTED;
#else
      guint8 *negative_branch, *msword_positive_branch, *msword_negative_branch,
          *ovf_ex_target;
    ...
    case OP_FCNEQ:
      ...
      ppc_li(code, ins->dreg, 1);
      ppc_bc(code, ins->opcode == OP_FCEQ ? PPC_BR_TRUE : PPC_BR_FALSE,
             PPC_BR_EQ, 2);
      ppc_li(code, ins->dreg, 0);
      break;
    case OP_FCLT:
    case OP_FCGE:
      ppc_fcmpo(code, 0, ins->sreg1, ins->sreg2);
      ppc_li(code, ins->dreg, 1);
      ppc_bc(code, ins->opcode == OP_FCLT ? PPC_BR_TRUE : PPC_BR_FALSE,
             PPC_BR_LT, 2);
      ppc_li(code, ins->dreg, 0);
      break;
    case OP_FCLT_UN:
    ...
    case OP_FCLE:
      ppc_fcmpo(code, 0, ins->sreg1, ins->sreg2);
      ppc_li(code, ins->dreg, 1);
      ppc_bc(code, ins->opcode == OP_FCGT ? PPC_BR_TRUE : PPC_BR_FALSE,
             PPC_BR_GT, 2);
      ppc_li(code, ins->dreg, 0);
      break;
    case OP_FCGT_UN:
    ...
    case OP_PPC_CHECK_FINITE: {
      ...
      break;
    case OP_JUMP_TABLE:
      mono_add_patch_info(cfg, offset, (MonoJumpInfoType)ins->inst_c1,
                          ins->inst_p0);
#ifdef __mono_ppc64__
    ...
    case OP_ATOMIC_CAS_I4:
      CASE_PPC64(OP_ATOMIC_CAS_I8) {
        ...
        ppc_cmp(code, 0, ins->opcode == OP_ATOMIC_CAS_I4 ? 0 : 1, ppc_r0,
                comparand);
        not_equal = code;
        ppc_bc(code, PPC_BR_FALSE, PPC_BR_EQ, 0);
    ...
    if ((cfg->opt & MONO_OPT_BRANCH) &&
      ...
      g_warning("wrong maximal instruction length of instruction %s (expected "
                ...
                (glong)(code - cfg->native_code - offset));
      g_assert_not_reached();
    }
...
#define patch_load_sequence(ip, val)                                           \
  do {                                                                         \
...
void mono_arch_patch_code_new(MonoCompile *cfg, MonoDomain *domain,
  ...
  case MONO_PATCH_INFO_SWITCH: {
    ...
    /* we put into the table the absolute address, no need for ppc_patch in this
     * case */
    break;
  }
...
/*
 * Emit code to save the registers in used_int_regs or the registers in the
 * MonoLMF structure at positive offset pos from register base_reg. pos is
 * guaranteed to fit into the instruction offset immediate for all the
 * registers.
 */
static guint8 *save_registers(MonoCompile *cfg, guint8 *code, int pos,
                              int base_reg, gboolean save_lmf,
                              guint32 used_int_regs, int cfa_offset) {
  int i;
  if (!save_lmf) {
    for (i = 13; i <= 31; i++) {
...
guint8 *mono_arch_emit_prolog(MonoCompile *cfg) {
  ...
  if (alloc_size) {
    if (ppc_is_imm16(-alloc_size)) {
      ...
      code = save_registers(cfg, code, alloc_size - pos, ppc_sp,
                            method->save_lmf, cfg->used_int_regs, cfa_offset);
    } else {
      if (pos)
      ...
      cfa_offset = alloc_size;
      mono_emit_unwind_op_def_cfa_offset(cfg, code, alloc_size);
      code = save_registers(cfg, code, 0, ppc_r12, method->save_lmf,
                            cfg->used_int_regs, cfa_offset);
    }
  }
  if (cfg->frame_reg != ppc_sp) {
  ...
  if (cfg->rgctx_var) {
    g_assert(cfg->rgctx_var->opcode == OP_REGOFFSET &&
             (cfg->rgctx_var->inst_basereg == ppc_r1 ||
              cfg->rgctx_var->inst_basereg == ppc_r31));

    ppc_stptr(code, MONO_ARCH_RGCTX_REG, cfg->rgctx_var->inst_offset,
      ...
      if (ainfo->regtype == RegTypeGeneral) {
        ...
        case 1:
          if (ppc_is_imm16(inst->inst_offset)) {
            ...
            if (ppc_is_imm32(inst->inst_offset)) {
              ppc_addis(code, ppc_r12, inst->inst_basereg,
                        ppc_ha(inst->inst_offset));
        ...
        case 8:
          if (ppc_is_imm16(inst->inst_offset + 4)) {
            ppc_stw(code, ainfo->reg, inst->inst_offset, inst->inst_basereg);
            ppc_stw(code, ainfo->reg + 1, inst->inst_offset + 4,
                    inst->inst_basereg);
          } else {
            ppc_addis(code, ppc_r12, inst->inst_basereg,
                      ppc_ha(inst->inst_offset));
        ...
        g_assert(ppc_is_imm16(inst->inst_offset));
        g_assert(ppc_is_imm16(inst->inst_offset +
                              ainfo->vtregs * sizeof(target_mgreg_t)));
        /* FIXME: what if there is no class? */
        if (sig->pinvoke &&
            mono_class_from_mono_type_internal(inst->inst_vtype))
          size = mono_class_native_size(
              mono_class_from_mono_type_internal(inst->inst_vtype), NULL);
...
#endif
          {
            ...
            if (ainfo->bytes) {
              ...
              if (mono_class_native_size(inst->klass, NULL) == 1) {
                ppc_stb(code, ainfo->reg + cur_reg, doffset,
                        inst->inst_basereg);
              } else if (mono_class_native_size(inst->klass, NULL) == 2) {
                ppc_sth(code, ainfo->reg + cur_reg, doffset,
                        inst->inst_basereg);
              } else if (mono_class_native_size(inst->klass, NULL) ==
                         4) { // WDS -- maybe <=4?
                ppc_stw(code, ainfo->reg + cur_reg, doffset,
                        inst->inst_basereg);
              } else {
                ppc_stptr(code, ainfo->reg + cur_reg, doffset,
                          inst->inst_basereg); // WDS -- Better way?
              }
#endif
            } else
#endif
        ...
        if (ainfo->vtsize) {
          ...
          if (ainfo->bytes)
            NOT_IMPLEMENTED;
          /* load the previous stack pointer in r12 (r0 gets overwritten by the
           * memcpy) */
          ppc_ldr(code, ppc_r12, 0, ppc_sp);
          if ((size & MONO_PPC_32_64_CASE(3, 7)) != 0) {
            code = emit_memcpy(code, size - soffset, inst->inst_basereg,
                               doffset, ppc_r12, ainfo->offset + soffset);
          } else {
            code = emit_memcpy(code, ainfo->vtsize * sizeof(target_mgreg_t),
                               inst->inst_basereg, doffset, ppc_r12,
                               ainfo->offset + soffset);
          }
        }
      } else if (ainfo->regtype == RegTypeStructByAddr) {
        ...
        g_assert(ppc_is_imm16(inst->inst_offset));
        code = emit_memcpy(code, ainfo->vtsize, inst->inst_basereg,
                           inst->inst_offset, ppc_r12, 0);
        /*g_print ("copy in %s: %d bytes from %d to offset: %d\n", method->name,
         * ainfo->vtsize, ainfo->reg, inst->inst_offset);*/
      } else
        g_assert_not_reached();
    }
    pos++;
  ...
  if (method->save_lmf) {
    ...
    if (cfg->compile_aot) {
      ...
      mono_add_patch_info(cfg, code - cfg->native_code, MONO_PATCH_INFO_IP,
                          NULL);
#ifdef __mono_ppc64__
...
void mono_arch_emit_epilog(MonoCompile *cfg) {
  ...
  if (method->save_lmf) {
    ...
    /* restore fregs */
    /*for (i = 14; i < 32; i++) {
            ppc_lfd (code, i, G_STRUCT_OFFSET(MonoLMF, fregs) + ((i-14) * sizeof
    (gdouble)), ppc_r12);
    }*/
...
static int exception_id_by_name(const char *name) {
...
void mono_arch_emit_exceptions(MonoCompile *cfg) {
  ...
  for (patch_info = cfg->patch_info; patch_info;
    ...
    case MONO_PATCH_INFO_EXC: {
      ...
      i = exception_id_by_name((const char *)patch_info->data.target);
      if (exc_throw_pos[i] &&
          !(ip > exc_throw_pos[i] && ip - exc_throw_pos[i] > 50000)) {
      ...
      ppc_patch(ip, code);
      /*mono_add_patch_info (cfg, code - cfg->native_code,
       * MONO_PATCH_INFO_EXC_NAME, patch_info->data.target);*/
...
#if DEAD_CODE
static int try_offset_access(void *value, guint32 idx) {
...
gpointer mono_arch_build_imt_trampoline(MonoVTable *vtable, MonoDomain *domain,
  ...
  mono_tramp_info_register(
      mono_tramp_info_create(NULL, start, code - start, NULL, NULL), domain);
...
MonoInst *mono_arch_emit_inst_for_method(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  if (cmethod->klass == mono_class_try_get_math_class()) {
    ...
    opcode = 0;
    if ((cpu_hw_caps & PPC_ISA_2X) && (fsig->param_count == 1) &&
        (fsig->params[0]->type == MONO_TYPE_R8)) {
      /*
