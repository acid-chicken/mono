...
static gboolean mono_de_ss_update(SingleStepReq *req, MonoJitInfo *ji,
                                  SeqPoint *sp, void *tls, MonoContext *ctx,
                                  MonoMethod *method);
...
void mono_de_lock(void) { dbg_lock(); }
...
static void domains_cleanup(void) {
 ...
 *
 * FIXME can we move the locking to here? Callers in sdb must be properly
 * audited.
 */
 ...
 * JI.
 */
static void insert_breakpoint(MonoSeqPointInfo *seq_points, MonoDomain *domain,
                              MonoJitInfo *ji, MonoBreakpoint *bp,
                              MonoError *error) {
  int count;
  ...
  if (!it_has_sp) {
    char *s = g_strdup_printf("Unable to insert breakpoint at %s:%ld",
  ...
  if (it.seq_point.native_offset == SEQ_POINT_NATIVE_OFFSET_DEAD_CODE) {
    DEBUG_PRINTF(1,
                 "[dbg] Attempting to insert seq point at dead IL offset %d, "
                 "ignoring.\n",
                 (int)bp->il_offset);
  } else if (count == 0) {
    if (ji->is_interp) {
  ...
  DEBUG_PRINTF(
      1, "[dbg] Inserted breakpoint at %s:[il=0x%x,native=0x%x] [%p](%d).\n",
      mono_method_full_name(jinfo_get_method(ji), TRUE),
      (int)it.seq_point.il_offset, (int)it.seq_point.native_offset, inst->ip,
      count);
}

static void remove_breakpoint(BreakpointInstance *inst) {
  int count;
 ...
 */
static gboolean bp_matches_method(MonoBreakpoint *bp, MonoMethod *method) {
  int i;
  ...
  if (method->is_inflated &&
      ((MonoMethodInflated *)method)->declaring == bp->method)
    return TRUE;
  ...
  if (bp->method->is_inflated && method->is_inflated) {
    ...
    /* Open generic methods should match closed generic methods of the same
     * class */
    if (bpimethod->declaring == imethod->declaring &&
        bpimethod->context.class_inst == imethod->context.class_inst &&
        bpimethod->context.method_inst &&
        bpimethod->context.method_inst->is_open) {
...
void mono_de_add_pending_breakpoints(MonoMethod *method, MonoJitInfo *ji) {
  ...
  for (i = 0; i < breakpoints->len; ++i) {
    ...
    for (j = 0; j < bp->children->len; ++j) {
      BreakpointInstance *inst =
          (BreakpointInstance *)g_ptr_array_index(bp->children, j);
    ...
    if (!found) {
      ...
      if (!seq_points)
        /* Could be AOT code, or above "search_all_backends" call could have
         * failed */
        continue;
...
static void set_bp_in_method(MonoDomain *domain, MonoMethod *method,
                             MonoSeqPointInfo *seq_points, MonoBreakpoint *bp,
                             MonoError *error) {
  MonoJitInfo *ji;
...
static void collect_domain_bp(gpointer key, gpointer value,
                              gpointer user_data) {
  GHashTableIter iter;
  MonoSeqPointInfo *seq_points;
...
void mono_de_clear_all_breakpoints(void) {
  while (breakpoints->len)
    mono_de_clear_breakpoint(
        (MonoBreakpoint *)g_ptr_array_index(breakpoints, 0));
}
 ...
 * If ERROR is non-NULL, then it is set and NULL is returnd if some breakpoints
 * couldn't be inserted.
 */
MonoBreakpoint *mono_de_set_breakpoint(MonoMethod *method, long il_offset,
                                       EventRequest *req, MonoError *error) {
  MonoBreakpoint *bp;
  MonoDomain *domain;
  MonoMethod *m;
  MonoSeqPointInfo *seq_points;
  ...
  DEBUG_PRINTF(1, "[dbg] Setting %sbreakpoint at %s:0x%x.\n",
               (req->event_kind == EVENT_KIND_STEP) ? "single step " : "",
               method ? mono_method_full_name(method, TRUE) : "<all>",
               (int)il_offset);
...
void mono_de_clear_breakpoint(MonoBreakpoint *bp) {
  ...
  for (i = 0; i < bp->children->len; ++i) {
    BreakpointInstance *inst =
        (BreakpointInstance *)g_ptr_array_index(bp->children, i);
...
void mono_de_collect_breakpoints_by_sp(SeqPoint *sp, MonoJitInfo *ji,
                                       GPtrArray *ss_reqs, GPtrArray *bp_reqs) {
  for (int i = 0; i < breakpoints->len; ++i) {
    MonoBreakpoint *bp = (MonoBreakpoint *)g_ptr_array_index(breakpoints, i);
    ...
    for (int j = 0; j < bp->children->len; ++j) {
      BreakpointInstance *inst =
          (BreakpointInstance *)g_ptr_array_index(bp->children, j);
      if (inst->ji == ji && inst->il_offset == sp->il_offset &&
...
static void ss_req_init(void) { the_ss_reqs = g_ptr_array_new(); }
...
static const char *ss_depth_to_string(StepDepth depth) {
  switch (depth) {
 ...
 */
static void ss_stop(SingleStepReq *ss_req) {
  if (ss_req->bps) {
    GSList *l;
...
static SingleStepReq *ss_req_acquire(MonoInternalThread *thread) {
  SingleStepReq *req = NULL;
  dbg_lock();
  int i;
  for (i = 0; i < the_ss_reqs->len; ++i) {
    SingleStepReq *current_req =
        (SingleStepReq *)g_ptr_array_index(the_ss_reqs, i);
...
static int ss_req_count() { return the_ss_reqs->len; }

static void mono_de_ss_req_release(SingleStepReq *req) {
  gboolean free = FALSE;
...
void mono_de_process_single_step(void *tls, gboolean from_signal) {
  ...
  /*
   * This can run concurrently with a clear_event_request () call, so needs
   * locking/reference counts.
   */
  ...
  if (log_level > 0) {
    DEBUG_PRINTF(
        1,
        "[%p] Single step event (depth=%s) at %s (%p)[0x%x], sp %p, last sp "
        "%p\n",
        (gpointer)(gsize)mono_native_thread_id_get(),
        ss_depth_to_string(ss_req->depth),
        mono_method_full_name(jinfo_get_method(ji), TRUE),
        MONO_CONTEXT_GET_IP(ctx),
        (int)((guint8 *)MONO_CONTEXT_GET_IP(ctx) - (guint8 *)ji->code_start),
        MONO_CONTEXT_GET_SP(ctx), ss_req->last_sp);
  }
  ...
  /*
   * This could be in mono_de_ss_update method, but
   * mono_find_next_seq_point_for_native_offset is pretty expensive method,
   * hence we prefer this check here.
   */
  if (ss_req->user_assemblies) {
    gboolean found = FALSE;
    for (int k = 0; ss_req->user_assemblies[k]; k++)
      if (ss_req->user_assemblies[k] ==
          m_class_get_image(method->klass)->assembly) {
        found = TRUE;
        break;
      }
    if (!found)
  ...
  /*
   * The ip points to the instruction causing the single step event, which is
   * before the offset recorded in the seq point map, so find the next seq point
   * after ip.
   */
  if (!mono_find_next_seq_point_for_native_offset(
          domain, method, (guint8 *)ip - (guint8 *)ji->code_start, &info,
          &sp)) {
    g_assert_not_reached();
    goto exit;
  }
  ...
  void *bp_events;
  bp_events = rt_callbacks.create_breakpoint_events(reqs, NULL, ji,
...
static gboolean mono_de_ss_update(SingleStepReq *req, MonoJitInfo *ji,
  ...
  if ((req->filter & STEP_FILTER_STATIC_CTOR)) {
    ...
    if (!method_in_stack) {
      g_printerr("[%p] The instruction pointer of the currently executing "
                 "method(%s) is not on the recorded stack. This is likely due "
                 "to a runtime bug. The %d frames are as follow: \n",
                 (gpointer)(gsize)mono_native_thread_id_get(),
                 mono_method_full_name(method, TRUE), nframes);
      /*DEBUG_PRINTF (1, "[%p] The instruction pointer of the currently
       * executing method(%s) is not on the recorded stack. This is likely due
       * to a runtime bug. The %d frames are as follow: \n",
       * (gpointer)(gsize)mono_native_thread_id_get (), mono_method_full_name
       * (method, TRUE), tls->frame_count);*/

      for (int i = 0; i < nframes; i++)
        g_printerr("\t [%p] Frame (%d / %d): %s\n",
                   (gpointer)(gsize)mono_native_thread_id_get(), i, nframes,
                   mono_method_full_name(frames[i]->method, TRUE));
    }
  ...
  if (req->async_stepout_method == method) {
    DEBUG_PRINTF(1,
                 "[%p] Breakpoint hit during async step-out at %s hit, "
                 "continuing stepping out.\n",
  ...
  if (req->depth == STEP_DEPTH_OVER &&
    ...
    /*
     * These seq points are inserted by the JIT after calls, step over needs to
     * skip them.
     */
    DEBUG_PRINTF(1,
                 "[%p] Seq point at nonempty stack %x while stepping over, "
  ...
  if ((req->depth == STEP_DEPTH_OVER || req->depth == STEP_DEPTH_OUT) && hit &&
      !req->async_stepout_method) {
    gboolean is_step_out = req->depth == STEP_DEPTH_OUT;
    int nframes;
    rt_callbacks.ss_calculate_framecount(tls, ctx, FALSE, NULL, &nframes);

    // Because functions can call themselves recursively, we need to make sure
    // we're stopping at the right stack depth. In case of step out, the target
    // is the frame *enclosing* the one where the request was made.
    ...
    if (req->nframes > 0 && nframes > 0 && nframes > target_frames) {
      ...
      DEBUG_PRINTF(1,
                   "[%p] Breakpoint at lower frame while stepping %s, "
                   "continuing single stepping.\n",
                   (gpointer)(gsize)mono_native_thread_id_get(),
                   is_step_out ? "out" : "over");
      return FALSE;
    }
  }

  if (req->depth == STEP_DEPTH_INTO && req->size == STEP_SIZE_MIN &&
      (sp->flags & MONO_SEQ_POINT_FLAG_NONEMPTY_STACK) && req->start_method) {
    int nframes;
    rt_callbacks.ss_calculate_framecount(tls, ctx, FALSE, NULL, &nframes);
    if (req->start_method == method && req->nframes &&
        nframes == req->nframes) { // Check also frame count(could be recursion)
      DEBUG_PRINTF(1,
                   "[%p] Seq point at nonempty stack %x while stepping in, "
                   "continuing single stepping.\n",
                   (gpointer)(gsize)mono_native_thread_id_get(), sp->il_offset);
      return FALSE;
    }
  ...
  MonoDebugMethodAsyncInfo *async_method =
      mono_debug_lookup_method_async_debug_info(method);
  if (async_method) {
    for (int i = 0; i < async_method->num_awaits; i++) {
      if (async_method->yield_offsets[i] == sp->il_offset ||
          async_method->resume_offsets[i] == sp->il_offset) {
  ...
  if (!loc) {
    DEBUG_PRINTF(1,
                 "[%p] No line number info for il offset %x, continuing single "
                 "stepping.\n",
                 (gpointer)(gsize)mono_native_thread_id_get(), sp->il_offset);
    ...
    int nframes;
    rt_callbacks.ss_calculate_framecount(tls, ctx, FALSE, NULL, &nframes);
    if (nframes == req->nframes) { // If the frame has changed we're clearly not
...
void mono_de_process_breakpoint(void *void_tls, gboolean from_signal) {
  ...
  /*
   * The ip points to the instruction causing the breakpoint event, which is
   * after the offset recorded in the seq point map, so find the prev seq point
   * before ip.
   */
  found_sp = mono_find_prev_seq_point_for_native_offset(
      mono_domain_get(), method, native_offset, &info, &sp);
  ...
  DEBUG_PRINTF(
      ...
      (gpointer)(gsize)mono_native_thread_id_get(), method->name, ip,
      sp.il_offset, native_offset);
  ...
  for (i = 0; i < ss_reqs_orig->len; ++i) {
    ...
    if ((ss_req->async_stepout_method != method) &&
        (ss_req->async_id ||
         mono_thread_internal_current() != ss_req->thread)) {
      DbgEngineStackFrame **frames;
      int nframes;
    ...
    // Update stepping request to new thread/frame_count that we are continuing
    // on so continuing with normal stepping works as expected
  ...
  void *bp_events =
      rt_callbacks.create_breakpoint_events(ss_reqs, bp_reqs, ji, kind);
 ...
 */
static gboolean ss_bp_is_unique(GSList *bps, GHashTable *ss_req_bp_cache,
 ...
 * ss_bp_eq:
 *
 * GHashTable equality for a MonoBreakpoint (only care about method and
 * il_offset fields)
 */
static gint ss_bp_eq(gconstpointer ka, gconstpointer kb) {
 ...
 * ss_bp_eq:
 *
 * GHashTable hash for a MonoBreakpoint (only care about method and il_offset
 * fields)
 */
static guint ss_bp_hash(gconstpointer data) {
 ...
 */
static void ss_bp_add_one(SingleStepReq *ss_req, int *ss_req_bp_count,
                          GHashTable **ss_req_bp_cache, MonoMethod *method,
                          guint32 il_offset) {
  ...
  if (ss_bp_is_unique(ss_req->bps, *ss_req_bp_cache, method, il_offset)) {
    ...
    MonoBreakpoint *bp =
        mono_de_set_breakpoint(method, il_offset, ss_req->req, NULL);
    ...
    DEBUG_PRINTF(1,
                 "[dbg] Candidate breakpoint at %s:[il=0x%x] is a duplicate "
                 "for this step request, will not add.\n",
                 mono_method_full_name(method, TRUE), (int)il_offset);
  }
}

static gboolean is_last_non_empty(SeqPoint *sp, MonoSeqPointInfo *info) {
  if (!sp->next_len)
    return TRUE;
  ...
  for (int i = 0; i < sp->next_len; i++) {
    if (next[i].flags & MONO_SEQ_POINT_FLAG_NONEMPTY_STACK &&
        !(next[i].flags & MONO_SEQ_POINT_FLAG_NESTED_CALL)) {
 ...
 * mono_de_ss_start:
 *
 *   Start the single stepping operation given by SS_REQ from the sequence point
 * SP. If CTX is not set, then this can target any thread. If CTX is set, then
 * TLS should belong to the same thread as CTX. If FRAMES is not-null, use that
 * instead of tls->frames for placing breakpoints etc.
 */
static void mono_de_ss_start(SingleStepReq *ss_req, SingleStepArgs *ss_args) {
  int i, j, frame_index;
  ...
  // When 8 or more entries are in bps, we build a hash table to serve as a set
  // of breakpoints. Recreating this on each pass is a little wasteful but at
  // least keeps behavior linear.
  ...
  /* this can happen on a single step in a exception on android
   * (Mono_UnhandledException_internal) and on IOS */
  if (!method)
    return;
  ...
  if (ss_args->step_to_catch) {
    ss_bp_add_one(ss_req, &ss_req_bp_count, &ss_req_bp_cache, method,
                  sp->il_offset);
  } else {
    frame_index = 1;
    ...
    if (ss_args->ctx && !frames) {
      ...
      /* Need parent frames */
      rt_callbacks.ss_calculate_framecount(tls, ss_args->ctx, FALSE, &frames,
                                           &nframes);
    }
    ...
    for (i = ss_req->depth == STEP_DEPTH_OUT ? 1 : 0; i < nframes; ++i) {
      ...
      if (frame->ji) {
        ...
        for (j = 0; j < jinfo->num_clauses; ++j) {
          // In case of async method we don't want to place breakpoint on last
          // catch handler(which state machine added for whole method)
          if (asyncMethod && asyncMethod->num_awaits && i == 0 &&
              j + 1 == jinfo->num_clauses)
            break;
          MonoJitExceptionInfo *ei = &jinfo->clauses[j];

          if (mono_find_next_seq_point_for_native_offset(
                  frame->domain, frame->method,
                  (char *)ei->handler_start - (char *)jinfo->code_start, NULL,
                  &local_sp))
            ss_bp_add_one(ss_req, &ss_req_bp_count, &ss_req_bp_cache,
                          frame->method, local_sp.il_offset);
        }
      }
    }

    if (asyncMethod && asyncMethod->num_awaits && nframes &&
        rt_callbacks.ensure_jit(frames[0])) {
      // asyncMethod has value and num_awaits > 0, this means we are inside
      // async method with awaits

      // Check if we hit yield_offset during normal stepping, because if we
      // did... Go into special async stepping mode which places breakpoint on
      // resumeOffset of this await call and sets async_id so we can distinguish
      // it from parallel executions
      ...
      // Switch to step-out, so whole NotifyDebuggerOfWaitCompletion magic
      // happens...
      if (is_last_non_empty(sp, ss_args->info)) {
        ss_req->depth =
            STEP_DEPTH_OUT; // setting depth to step-out is important, don't
                            // inline IF, because code later depends on this
      }
      if (ss_req->depth == STEP_DEPTH_OUT) {
        // If we are inside `async void` method, do normal step-out
        if (rt_callbacks.set_set_notification_for_wait_completion_flag(
                frames[0])) {
          ss_req->async_id = rt_callbacks.get_this_async_id(frames[0]);
          ss_req->async_stepout_method =
              rt_callbacks.get_notify_debugger_of_wait_completion_method();
          ss_bp_add_one(ss_req, &ss_req_bp_count, &ss_req_bp_cache,
                        ss_req->async_stepout_method, 0);
          g_hash_table_destroy(ss_req_bp_cache);
          mono_debug_free_method_async_debug_info(asyncMethod);
          if (locked)
            mono_loader_unlock();
    ...
    if (ss_req->depth == STEP_DEPTH_OUT) {
      ...
      while (frame_index < nframes) {
        ...
        method = frame->method;
        found_sp = mono_find_prev_seq_point_for_native_offset(
            frame->domain, frame->method, frame->native_offset, &ss_args->info,
            &local_sp);
      ...
      // There could be method calls before the next seq point in the caller
      // when using nested calls
      // enable_global = TRUE;
    } else {
      if (sp && sp->next_len == 0) {
        sp = NULL;
        while (frame_index < nframes) {
          ...
          method = frame->method;
          found_sp = mono_find_prev_seq_point_for_native_offset(
              frame->domain, frame->method, frame->native_offset,
              &ss_args->info, &local_sp);
          sp = (found_sp) ? &local_sp : NULL;
          if (sp && sp->next_len != 0)
            break;
          sp = NULL;
        ...
        /* Have to put a breakpoint into a parent frame since the seq points
         * might not cover all control flow out of the method */
        while (frame_index < nframes) {
          DbgEngineStackFrame *frame = frames[frame_index];

          parent_sp_method = frame->method;
          found_sp = mono_find_prev_seq_point_for_native_offset(
              frame->domain, frame->method, frame->native_offset, &parent_info,
              &local_parent_sp);
    ...
    if (parent_sp) {
      ...
      for (i = 0; i < parent_sp->next_len; i++) {
        ...
        ss_bp_add_one(ss_req, &ss_req_bp_count, &ss_req_bp_cache,
                      parent_sp_method, next_sp->il_offset);
      }
      g_free(next);
    }
    ...
    if ((ss_req->depth == STEP_DEPTH_OVER) && (!sp && !parent_sp)) {
      DEBUG_PRINTF(
          1, "[dbg] No parent frame for step over, transition to step into.\n");
      /*
       * This is needed since if we leave managed code, and later return to it,
       * step over is not going to stop. This approach is a bit ugly, since we
       * change the step depth, but it only affects clients who reuse the same
       * step request, and only in this special case.
       */
 ...
 */
DbgEngineErrorCode mono_de_ss_create(MonoInternalThread *thread, StepSize size,
                                     StepDepth depth, StepFilter filter,
                                     EventRequest *req) {
  ...
  if (ss_req_count() > 1) {
    ...
    if (err == DE_ERR_NOT_IMPLEMENTED) {
      DEBUG_PRINTF(0, "Received a single step request while the previous one "
                      "was still active.\n");
 ...
 *
 * Configures logging level and output file. Must be called together with
 * mono_de_init.
 */
