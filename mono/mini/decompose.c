 ...
 * This is also used on 32 bit machines when using LLVM, so it needs to handle
 * I/U correctly.
 */
static gboolean decompose_long_opcode(MonoCompile *cfg, MonoInst *ins,
                                      MonoInst **repl_ins) {
  MonoInst *repl = NULL;
  ...
  *repl_ins = NULL;
  ...
  case OP_LCONV_TO_OVF_I:
    ...
    MONO_EMIT_NEW_COND_EXC(cfg, GT, "OverflowException");
    /* The int cast is needed for the VS compiler.  See Compiler Warning (level
     * 2) C4146. */
...
MonoInst *mono_decompose_opcode(MonoCompile *cfg, MonoInst *ins) {
  ...
  case OP_FCONV_TO_OVF_U_UN:
    mono_cfg_set_exception_invalid_program(
        cfg, g_strdup_printf("float conv.ovf.un opcodes not supported."));
    break;
  ...
  case OP_IDIV_UN:
  case OP_IREM_UN:
    if (cfg->backend->emulate_div &&
        mono_arch_opcode_needs_emulation(cfg, ins->opcode))
      emulate = TRUE;
    if (!emulate) {
  ...
  case OP_IREM_UN_IMM:
    if (cfg->backend->need_div_check) {
      ...
      if ((ins->opcode == OP_DIV_IMM || ins->opcode == OP_IDIV_IMM ||
           ins->opcode == OP_REM_IMM || ins->opcode == OP_IREM_IMM) &&
          (ins->inst_imm == -1)) {
        /* b == -1 && a == 0x80000000 */
        MONO_EMIT_NEW_ICOMPARE_IMM(cfg, ins->sreg1, 0x80000000);
        MONO_EMIT_NEW_COND_EXC(cfg, IEQ, "OverflowException");
      }
      MONO_EMIT_NEW_BIALU_IMM(cfg, ins->opcode, ins->dreg, ins->sreg1,
                              ins->inst_imm);
      NULLIFY_INS(ins);
    } else {
      emulate = TRUE;
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    */
    ...
    while (tree) {
      ...
      switch (tree->opcode) {
      case OP_I8CONST:
        MONO_EMIT_NEW_ICONST(cfg, MONO_LVREG_LS(tree->dreg),
                             ins_get_l_low(tree));
        MONO_EMIT_NEW_ICONST(cfg, MONO_LVREG_MS(tree->dreg),
                             ins_get_l_high(tree));
        break;
      case OP_DUMMY_I8CONST:
        MONO_EMIT_NEW_DUMMY_INIT(cfg, MONO_LVREG_LS(tree->dreg),
                                 OP_DUMMY_ICONST);
        MONO_EMIT_NEW_DUMMY_INIT(cfg, MONO_LVREG_MS(tree->dreg),
                                 OP_DUMMY_ICONST);
        break;
      case OP_LMOVE:
      case OP_LCONV_TO_U8:
      ...
      case OP_LCONV_TO_OVF_U8_UN:
      case OP_LCONV_TO_OVF_I8:
        MONO_EMIT_NEW_UNALU(cfg, OP_MOVE, MONO_LVREG_LS(tree->dreg),
                            MONO_LVREG_LS(tree->sreg1));
        MONO_EMIT_NEW_UNALU(cfg, OP_MOVE, MONO_LVREG_MS(tree->dreg),
                            MONO_LVREG_MS(tree->sreg1));
        break;
      case OP_STOREI8_MEMBASE_REG:
        MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG,
                                    tree->inst_destbasereg,
                                    tree->inst_offset + MINI_MS_WORD_OFFSET,
                                    MONO_LVREG_MS(tree->sreg1));
        MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG,
                                    tree->inst_destbasereg,
                                    tree->inst_offset + MINI_LS_WORD_OFFSET,
                                    MONO_LVREG_LS(tree->sreg1));
        break;
      case OP_LOADI8_MEMBASE:
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(
            cfg, OP_LOADI4_MEMBASE, MONO_LVREG_MS(tree->dreg),
            tree->inst_basereg, tree->inst_offset + MINI_MS_WORD_OFFSET);
        MONO_EMIT_NEW_LOAD_MEMBASE_OP(
            cfg, OP_LOADI4_MEMBASE, MONO_LVREG_LS(tree->dreg),
            tree->inst_basereg, tree->inst_offset + MINI_LS_WORD_OFFSET);
        break;
      ...
      case OP_ICONV_TO_I8: {
        ...
        MONO_EMIT_NEW_UNALU(cfg, OP_MOVE, MONO_LVREG_LS(tree->dreg),
                            tree->sreg1);
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ICOMPARE_IMM, -1,
                                MONO_LVREG_LS(tree->dreg), -1);
        MONO_EMIT_NEW_BIALU(cfg, OP_ICGT, tmpreg, -1, -1);
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ISUB_IMM, MONO_LVREG_MS(tree->dreg),
                                tmpreg, 1);
        break;
      }
      case OP_ICONV_TO_U8:
      ...
      case OP_ICONV_TO_OVF_I8:
        ...
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_SHR_IMM, MONO_LVREG_MS(tree->dreg),
                                tree->sreg1, 31);
        MONO_EMIT_NEW_UNALU(cfg, OP_MOVE, MONO_LVREG_LS(tree->dreg),
                            tree->sreg1);
        break;
      ...
      case OP_ICONV_TO_OVF_U8_UN:
        ...
        break;
      case OP_LCONV_TO_I1:
        MONO_EMIT_NEW_UNALU(cfg, OP_ICONV_TO_I1, tree->dreg,
                            MONO_LVREG_LS(tree->sreg1));
        break;
      case OP_LCONV_TO_U1:
        MONO_EMIT_NEW_UNALU(cfg, OP_ICONV_TO_U1, tree->dreg,
                            MONO_LVREG_LS(tree->sreg1));
        break;
      case OP_LCONV_TO_I2:
        MONO_EMIT_NEW_UNALU(cfg, OP_ICONV_TO_I2, tree->dreg,
                            MONO_LVREG_LS(tree->sreg1));
        break;
      case OP_LCONV_TO_U2:
        MONO_EMIT_NEW_UNALU(cfg, OP_ICONV_TO_U2, tree->dreg,
                            MONO_LVREG_LS(tree->sreg1));
        break;
      case OP_LCONV_TO_I4:
      case OP_LCONV_TO_U4:
      case OP_LCONV_TO_I:
      case OP_LCONV_TO_U:
        MONO_EMIT_NEW_UNALU(cfg, OP_MOVE, tree->dreg,
                            MONO_LVREG_LS(tree->sreg1));
        break;
#ifndef MONO_ARCH_EMULATE_LCONV_TO_R8
      case OP_LCONV_TO_R8:
        MONO_EMIT_NEW_BIALU(cfg, OP_LCONV_TO_R8_2, tree->dreg,
                            MONO_LVREG_LS(tree->sreg1),
                            MONO_LVREG_MS(tree->sreg1));
        break;
#endif
      ...
      case OP_LCONV_TO_R4:
        MONO_EMIT_NEW_BIALU(cfg, OP_LCONV_TO_R4_2, tree->dreg,
                            MONO_LVREG_LS(tree->sreg1),
                            MONO_LVREG_MS(tree->sreg1));
        break;
#endif
      ...
      case OP_LCONV_TO_R_UN:
        MONO_EMIT_NEW_BIALU(cfg, OP_LCONV_TO_R_UN_2, tree->dreg,
                            MONO_LVREG_LS(tree->sreg1),
                            MONO_LVREG_MS(tree->sreg1));
        break;
#endif
      case OP_LCONV_TO_OVF_I1: {
        ...
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1,
                                MONO_LVREG_MS(tree->sreg1), 0);
        MONO_EMIT_NEW_COND_EXC(cfg, GT, "OverflowException");
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1,
                                MONO_LVREG_MS(tree->sreg1), -1);
        MONO_EMIT_NEW_COND_EXC(cfg, LT, "OverflowException");
      ...
      case OP_LCONV_TO_OVF_U1_UN:
        ...
        MONO_EMIT_NEW_COND_EXC(cfg, GT_UN, "OverflowException");
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_AND_IMM, tree->dreg,
                                MONO_LVREG_LS(tree->sreg1), 0xff);
        break;
      case OP_LCONV_TO_OVF_I2: {
        MonoBasicBlock *is_negative, *end_label;
      ...
      case OP_LCONV_TO_OVF_U2_UN:
        ...
        MONO_EMIT_NEW_COND_EXC(cfg, GT_UN, "OverflowException");
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_AND_IMM, tree->dreg,
                                MONO_LVREG_LS(tree->sreg1), 0xffff);
        break;
      case OP_LCONV_TO_OVF_I4:
      case OP_LCONV_TO_OVF_I:
        MONO_EMIT_NEW_BIALU(cfg, OP_LCONV_TO_OVF_I4_2, tree->dreg,
                            MONO_LVREG_LS(tree->sreg1),
                            MONO_LVREG_MS(tree->sreg1));
        break;
      case OP_LCONV_TO_OVF_U4:
      ...
      case OP_LADD:
        MONO_EMIT_NEW_BIALU(cfg, OP_IADDCC, MONO_LVREG_LS(tree->dreg),
                            MONO_LVREG_LS(tree->sreg1),
                            MONO_LVREG_LS(tree->sreg2));
        MONO_EMIT_NEW_BIALU(cfg, OP_IADC, MONO_LVREG_MS(tree->dreg),
                            MONO_LVREG_MS(tree->sreg1),
                            MONO_LVREG_MS(tree->sreg2));
        break;
      case OP_LSUB:
        MONO_EMIT_NEW_BIALU(cfg, OP_ISUBCC, MONO_LVREG_LS(tree->dreg),
                            MONO_LVREG_LS(tree->sreg1),
                            MONO_LVREG_LS(tree->sreg2));
        MONO_EMIT_NEW_BIALU(cfg, OP_ISBB, MONO_LVREG_MS(tree->dreg),
                            MONO_LVREG_MS(tree->sreg1),
                            MONO_LVREG_MS(tree->sreg2));
        break;
      ...
      case OP_LSUB_OVF_UN:
        ...
        break;
      case OP_LAND:
        MONO_EMIT_NEW_BIALU(cfg, OP_IAND, MONO_LVREG_LS(tree->dreg),
                            MONO_LVREG_LS(tree->sreg1),
                            MONO_LVREG_LS(tree->sreg2));
        MONO_EMIT_NEW_BIALU(cfg, OP_IAND, MONO_LVREG_MS(tree->dreg),
                            MONO_LVREG_MS(tree->sreg1),
                            MONO_LVREG_MS(tree->sreg2));
        break;
      case OP_LOR:
        MONO_EMIT_NEW_BIALU(cfg, OP_IOR, MONO_LVREG_LS(tree->dreg),
                            MONO_LVREG_LS(tree->sreg1),
                            MONO_LVREG_LS(tree->sreg2));
        MONO_EMIT_NEW_BIALU(cfg, OP_IOR, MONO_LVREG_MS(tree->dreg),
                            MONO_LVREG_MS(tree->sreg1),
                            MONO_LVREG_MS(tree->sreg2));
        break;
      case OP_LXOR:
        MONO_EMIT_NEW_BIALU(cfg, OP_IXOR, MONO_LVREG_LS(tree->dreg),
                            MONO_LVREG_LS(tree->sreg1),
                            MONO_LVREG_LS(tree->sreg2));
        MONO_EMIT_NEW_BIALU(cfg, OP_IXOR, MONO_LVREG_MS(tree->dreg),
                            MONO_LVREG_MS(tree->sreg1),
                            MONO_LVREG_MS(tree->sreg2));
        break;
      case OP_LNOT:
        MONO_EMIT_NEW_UNALU(cfg, OP_INOT, MONO_LVREG_LS(tree->dreg),
                            MONO_LVREG_LS(tree->sreg1));
        MONO_EMIT_NEW_UNALU(cfg, OP_INOT, MONO_LVREG_MS(tree->dreg),
                            MONO_LVREG_MS(tree->sreg1));
        break;
      case OP_LNEG:
      ...
      case OP_LADD_IMM:
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ADDCC_IMM, MONO_LVREG_LS(tree->dreg),
                                MONO_LVREG_LS(tree->sreg1),
                                ins_get_l_low(tree));
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ADC_IMM, MONO_LVREG_MS(tree->dreg),
                                MONO_LVREG_MS(tree->sreg1),
                                ins_get_l_high(tree));
        break;
      case OP_LSUB_IMM:
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_SUBCC_IMM, MONO_LVREG_LS(tree->dreg),
                                MONO_LVREG_LS(tree->sreg1),
                                ins_get_l_low(tree));
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_SBB_IMM, MONO_LVREG_MS(tree->dreg),
                                MONO_LVREG_MS(tree->sreg1),
                                ins_get_l_high(tree));
        break;
      case OP_LAND_IMM:
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_AND_IMM, MONO_LVREG_LS(tree->dreg),
                                MONO_LVREG_LS(tree->sreg1),
                                ins_get_l_low(tree));
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_AND_IMM, MONO_LVREG_MS(tree->dreg),
                                MONO_LVREG_MS(tree->sreg1),
                                ins_get_l_high(tree));
        break;
      case OP_LOR_IMM:
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_OR_IMM, MONO_LVREG_LS(tree->dreg),
                                MONO_LVREG_LS(tree->sreg1),
                                ins_get_l_low(tree));
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_OR_IMM, MONO_LVREG_MS(tree->dreg),
                                MONO_LVREG_MS(tree->sreg1),
                                ins_get_l_high(tree));
        break;
      case OP_LXOR_IMM:
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_XOR_IMM, MONO_LVREG_LS(tree->dreg),
                                MONO_LVREG_LS(tree->sreg1),
                                ins_get_l_low(tree));
        MONO_EMIT_NEW_BIALU_IMM(cfg, OP_XOR_IMM, MONO_LVREG_MS(tree->dreg),
                                MONO_LVREG_MS(tree->sreg1),
                                ins_get_l_high(tree));
        break;
#ifdef TARGET_POWERPC
      /* FIXME This is normally handled in cprop. Proper fix or remove if no
       * longer needed. */
      case OP_LSHR_UN_IMM:
        if (tree->inst_c1 == 32) {

          /* The original code had this comment: */
          /* special case that gives a nice speedup and happens to workaorund a
           * ppc jit but (for the release) later apply the speedup to the left
           * shift as well See BUG# 57957.
           */
      ...
      case OP_LCOMPARE: {
        ...
        case OP_LBNE_UN: {
          ...
          d1 = alloc_ireg(cfg);
          d2 = alloc_ireg(cfg);
          MONO_EMIT_NEW_BIALU(cfg, OP_IXOR, d1, MONO_LVREG_LS(tree->sreg1),
                              MONO_LVREG_LS(tree->sreg2));
          MONO_EMIT_NEW_BIALU(cfg, OP_IXOR, d2, MONO_LVREG_MS(tree->sreg1),
                              MONO_LVREG_MS(tree->sreg2));
          MONO_EMIT_NEW_BIALU(cfg, OP_IOR, d1, d1, d2);
          MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ICOMPARE_IMM, -1, d1, 0);
          MONO_EMIT_NEW_BRANCH_BLOCK2(
              cfg, next->opcode == OP_LBEQ ? OP_IBEQ : OP_IBNE_UN,
              next->inst_true_bb, next->inst_false_bb);
          NULLIFY_INS(next);
          break;
        }
        case OP_LBGE:
        ...
        case OP_LBLT_UN:
          ...
          MONO_EMIT_NEW_BIALU(cfg, OP_COMPARE, -1, MONO_LVREG_MS(tree->sreg1),
                              MONO_LVREG_MS(tree->sreg2));
          MONO_EMIT_NEW_BRANCH_BLOCK(cfg, lbr_decomp[next->opcode - OP_LBEQ][0],
                                     next->inst_true_bb);
          MONO_EMIT_NEW_BIALU(cfg, OP_COMPARE, -1, MONO_LVREG_MS(tree->sreg1),
                              MONO_LVREG_MS(tree->sreg2));
          MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_IBNE_UN, next->inst_false_bb);
          MONO_EMIT_NEW_BIALU(cfg, OP_COMPARE, -1, MONO_LVREG_LS(tree->sreg1),
                              MONO_LVREG_LS(tree->sreg2));
          MONO_EMIT_NEW_BRANCH_BLOCK2(cfg,
                                      lbr_decomp[next->opcode - OP_LBEQ][1],
                                      next->inst_true_bb, next->inst_false_bb);
          NULLIFY_INS(next);
          break;
        case OP_LCEQ: {
        ...
        case OP_LCGT_UN: {
          ...
          MONO_EMIT_NEW_BRANCH_BLOCK(
              cfg, lcset_decomp[next->opcode - OP_LCEQ][0], set_to_0);
          MONO_EMIT_NEW_BIALU(cfg, OP_COMPARE, -1, MONO_LVREG_MS(tree->sreg1),
                              MONO_LVREG_MS(tree->sreg2));
          ...
          MONO_EMIT_NEW_BIALU(cfg, OP_COMPARE, -1, MONO_LVREG_LS(tree->sreg1),
                              MONO_LVREG_LS(tree->sreg2));
          MONO_EMIT_NEW_BRANCH_BLOCK(
              cfg, lcset_decomp[next->opcode - OP_LCEQ][1], set_to_0);
...
void mono_decompose_vtype_opts(MonoCompile *cfg) {
   ...
   * - it simplifies method_to_ir () since there is no need to special-case
   * vtypes everywhere.
   * - it gets rid of the LDADDR opcodes generated when vtype operations are
   * decomposed, enabling optimizations to work on vtypes too. Unlike
   * decompose_long_opts, this pass does not alter the CFG of the method so it
   * can be executed anytime. It should be executed as late as possible so vtype
   * opcodes can be optimized by the other passes.
   * The pinvoke wrappers need to manipulate vtypes in their unmanaged
   * representation. This is indicated by setting the 'backend.is_pinvoke' field
   * of the MonoInst for the var to 1. This is done on demand, ie. by the
   * LDNATIVEOBJ opcode, and propagated by this pass when OP_VMOVE opcodes are
   * decomposed.
   */

  /*
   * Vregs have no associated type information, so we store the type of the
   * vregs in ins->klass.
   */
   ...
   * code generation macros.
   */
  cfg->cbb = (MonoBasicBlock *)mono_mempool_alloc0((cfg)->mempool,
                                                   sizeof(MonoBasicBlock));
  first_bb = cfg->cbb;

  /* For LLVM, decompose only the OP_STOREV_MEMBASE opcodes, which need write
   * barriers and the gsharedvt opcodes */
    ...
    while (restart) {
      ...
      for (ins = bb->code; ins; ins = ins->next) {
        ...
        case OP_VMOVE: {
          ...
          if (!src_var)
            src_var = mono_compile_create_var_for_vreg(
                cfg, m_class_get_byval_arg(ins->klass), OP_LOCAL, ins->dreg);

          if (!dest_var)
            dest_var = mono_compile_create_var_for_vreg(
                cfg, m_class_get_byval_arg(ins->klass), OP_LOCAL, ins->dreg);
          ...
          mini_emit_memory_copy(cfg, dest, src, src_var->klass,
                                src_var->backend.is_pinvoke, 0);
        ...
        case OP_VZERO:
          ...
          EMIT_NEW_VARLOADA_VREG(cfg, dest, ins->dreg,
        ...
        case OP_STOREV_MEMBASE: {
          ...
          if (COMPILE_LLVM(cfg) && !mini_is_gsharedvt_klass(ins->klass) &&
              !(cfg->gen_write_barriers && m_class_has_references(ins->klass)))
            break;

          if (!src_var) {
            g_assert(ins->klass);
            src_var = mono_compile_create_var_for_vreg(
                cfg, m_class_get_byval_arg(ins->klass), OP_LOCAL, ins->sreg1);
          }

          EMIT_NEW_VARLOADA_VREG((cfg), (src), ins->sreg1,
                                 m_class_get_byval_arg(ins->klass));

          dreg = alloc_preg(cfg);
          EMIT_NEW_BIALU_IMM(cfg, dest, OP_ADD_IMM, dreg, ins->inst_destbasereg,
                             ins->inst_offset);
          mini_emit_memory_copy(cfg, dest, src, src_var->klass,
                                src_var->backend.is_pinvoke, ins->flags);
          break;
        }
        case OP_LOADV_MEMBASE: {
          ...
          dreg = alloc_preg(cfg);
          EMIT_NEW_BIALU_IMM(cfg, src, OP_ADD_IMM, dreg, ins->inst_basereg,
                             ins->inst_offset);
          EMIT_NEW_VARLOADA(cfg, dest, dest_var, dest_var->inst_vtype);
          mini_emit_memory_copy(cfg, dest, src, dest_var->klass,
                                dest_var->backend.is_pinvoke, 0);
          break;
        }
        case OP_OUTARG_VT: {
          if (COMPILE_LLVM(cfg))
        ...
        case OP_OUTARG_VTRETADDR: {
          ...
          if (!src_var)
            src_var = mono_compile_create_var_for_vreg(
                cfg, call->signature->ret, OP_LOCAL, call->inst.dreg);
        ...
        case OP_VCALL_MEMBASE: {
          ...
          if (call->vret_in_reg) {
            ...
            case OP_VCALL_REG:
              call2->inst.opcode =
                  call->vret_in_reg_fp ? OP_FCALL_REG : OP_CALL_REG;
              break;
            case OP_VCALL_MEMBASE:
              call2->inst.opcode =
                  call->vret_in_reg_fp ? OP_FCALL_MEMBASE : OP_CALL_MEMBASE;
              break;
            }
            ...
            if (dest_var->backend.is_pinvoke)
              size = mono_class_native_size(
                  mono_class_from_mono_type_internal(dest_var->inst_vtype),
                  NULL);
            else
...
#if SIZEOF_REGISTER == 4
              /*
              FIXME Other ABIs might return in different regs than the ones used
              for LCALL.
              FIXME It would be even nicer to be able to leverage the long
              decompose stuff.
              */
              ...
              MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG,
                                          dest->dreg, MINI_MS_WORD_OFFSET,
                                          MONO_LVREG_MS(call2->inst.dreg));
              MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI4_MEMBASE_REG,
                                          dest->dreg, MINI_LS_WORD_OFFSET,
                                          MONO_LVREG_LS(call2->inst.dreg));
#else
              MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREI8_MEMBASE_REG,
                                          dest->dreg, 0, call2->inst.dreg);
#endif
              break;
            default:
              /* This assumes the vtype is sizeof (gpointer) long */
              MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, dest->dreg,
                                          0, call2->inst.dreg);
              break;
            }
          } else {
        ...
        case OP_BOX_ICONST: {
          ...
          MonoInst *tmp =
              mini_emit_box(cfg, src, ins->klass,
                            mini_class_check_context_used(cfg, ins->klass));
          g_assert(tmp);
...
void mono_decompose_array_access_opts(MonoCompile *cfg) {
  ...
  /*
   * Unlike decompose_long_opts, this pass does not alter the CFG of the method
   * so it can be executed anytime. It should be run before decompose_long
   */
    ...
    while (restart) {
      ...
      for (ins = bb->code; ins; ins = ins->next) {
        ...
        case OP_TYPED_OBJREF:
          ...
          break;
        case OP_LDLEN:
          NEW_LOAD_MEMBASE_FLAGS(cfg, dest, OP_LOADI4_MEMBASE, ins->dreg,
                                 ins->sreg1, ins->inst_imm, ins->flags);
        ...
        case OP_BOUNDS_CHECK:
          ...
          if (COMPILE_LLVM(cfg)) {
            ...
            MONO_EMIT_DEFAULT_BOUNDS_CHECK(
                cfg, ins->sreg1, ins->inst_imm, index2_reg,
                ins->flags & MONO_INST_FAULT, ins->inst_p0);
          } else {
            MONO_ARCH_EMIT_BOUNDS_CHECK(cfg, ins->sreg1, ins->inst_imm,
                                        ins->sreg2, ins->inst_p0);
          }
          break;
        case OP_NEWARR:
          if (cfg->opt & MONO_OPT_SHARED) {
            ...
            MonoClass *array_class =
                mono_class_create_array(ins->inst_newa_class, 1);
            ERROR_DECL(vt_error);
            MonoVTable *vtable =
                mono_class_vtable_checked(cfg->domain, array_class, vt_error);
            MonoMethod *managed_alloc =
                mono_gc_get_managed_array_allocator(array_class);

            mono_error_assert_ok(
                vt_error); /*This shall not fail since we check for this
                              condition on OP_NEWARR creation*/
            ...
            if (managed_alloc)
              ...
              dest =
                  mono_emit_jit_icall(cfg, ves_icall_array_new_specific, iargs);
            dest->dreg = ins->dreg;
          }
          break;
        case OP_STRLEN:
          MONO_EMIT_NEW_LOAD_MEMBASE_OP_FLAGS(
              cfg, OP_LOADI4_MEMBASE, ins->dreg, ins->sreg1,
              MONO_STRUCT_OFFSET(MonoString, length),
              ins->flags | MONO_INST_INVARIANT_LOAD);
          break;
        default:
          break;
        }
 ...
 *
 *  Soft float support on ARM. We store each double value in a pair of integer
 * vregs, similar to long support on 32 bit platforms. 32 bit float values
 * require special handling when used as locals, arguments, and in calls. One
 * big problem with soft-float is that there are few r4 test cases in our test
 * suite.
 */
void mono_decompose_soft_float(MonoCompile *cfg) {
  ...
  /*
   * This pass creates long opcodes, so it should be run before
   * decompose_long_opts ().
   */
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    if (cfg->verbose_level > 3)
      mono_print_bb(bb, "BEFORE HANDLE-SOFT-FLOAT ");
    ...
    while (restart) {
      ...
      for (ins = bb->code; ins; ins = ins->next) {
        ...
        case OP_STORER4_MEMBASE_REG: {
          ...
          EMIT_NEW_BIALU_IMM(cfg, iargs[1], OP_PADD_IMM, addr_reg,
        ...
        case OP_LOADR4_MEMBASE: {
          ...
          addr_reg = mono_alloc_preg(cfg);
          EMIT_NEW_BIALU_IMM(cfg, iargs[0], OP_PADD_IMM, addr_reg,
        ...
        case OP_FCALL_MEMBASE: {
          ...
          if (call->signature->ret->type == MONO_TYPE_R4) {
            ...
            EMIT_NEW_VARLOADA_VREG(cfg, iargs[0], call2->inst.dreg,
                                   mono_get_int32_type());
            conv = mono_emit_jit_icall(cfg, mono_fload_r4, iargs);
            conv->dreg = ins->dreg;
        ...
        case OP_FCOMPARE: {
          ...
          call =
              mono_emit_jit_icall_id(cfg, mono_jit_icall_info_id(info), iargs);
          ...
          br->inst_many_bb =
              mono_mempool_alloc(cfg->mempool, sizeof(gpointer) * 2);
        ...
        default:
          if (spec[MONO_INST_SRC1] == 'f' || spec[MONO_INST_SRC2] == 'f' ||
              spec[MONO_INST_DEST] == 'f') {
            mono_print_ins(ins);
            g_assert_not_reached();
          }
          break;
    ...
    if (cfg->verbose_level > 3)
      mono_print_bb(bb, "AFTER HANDLE-SOFT-FLOAT ");
  }
...
void mono_local_emulate_ops(MonoCompile *cfg) {
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    MONO_BB_FOR_EACH_INS(bb, ins) {
       ...
       * function. They are decomposed in specific fashion in
       * mono_decompose_soft_float.
       */
      ...
      if (info) {
        ...
        args = (MonoInst **)mono_mempool_alloc0(
            cfg->mempool, sizeof(MonoInst *) * info->sig->param_count);
