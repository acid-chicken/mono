...
static ArgStorage get_storage(MonoType *type, gboolean is_return) {
  switch (type->type) {
  case MONO_TYPE_I1:
  case MONO_TYPE_U1:
  case MONO_TYPE_I2:
...
static CallInfo *get_call_info(MonoMemPool *mp, MonoMethodSignature *sig) {
  ...
  for (i = 0; i < sig->param_count; ++i)
    cinfo->args[i + sig->hasthis].storage =
        get_storage(mini_get_underlying_type(sig->params[i]), FALSE);
...
void mono_arch_create_vars(MonoCompile *cfg) {
  ...
  if (cinfo->ret.storage == ArgValuetypeAddrInIReg ||
      cinfo->ret.storage == ArgGsharedVTOnStack) {
...
void mono_arch_emit_setret(MonoCompile *cfg, MonoMethod *method,
  ...
  if (!ret->byref) {
    if (ret->type == MONO_TYPE_R4) {
      MONO_EMIT_NEW_UNALU(cfg, cfg->r4fp ? OP_RMOVE : OP_FMOVE, cfg->ret->dreg,
                          val->dreg);
      return;
...
const char *mono_arch_fregname(int reg) { return "freg0"; }

const char *mono_arch_regname(int reg) { return "r0"; }
...
EMSCRIPTEN_KEEPALIVE void mono_set_timeout_exec(int id) {
  ERROR_DECL(error);
  MonoClass *klass = mono_class_load_from_name(
      mono_defaults.corlib, "System.Threading", "WasmRuntime");
  g_assert(klass);

  MonoMethod *method = mono_class_get_method_from_name_checked(
      klass, "TimeoutCallback", -1, 0, error);
...
void mono_arch_register_icall(void) {
  mono_add_internal_call_internal("System.Threading.WasmRuntime::SetTimeout",
                                  mono_wasm_set_timeout);
}
...
void *getgrgid(gid_t gid) { return NULL; }

int inotify_init(void) { g_error("inotify_init"); }
