 ...
 *
 */
gpointer mini_llvmonly_load_method(MonoMethod *method,
                                   gboolean caller_gsharedvt,
                                   gboolean need_unbox, gpointer *out_arg,
                                   MonoError *error) {
  gpointer addr = mono_compile_method_checked(method, error);
  return_val_if_nok(error, NULL);

  if (addr) {
    return mini_llvmonly_add_method_wrappers(
        method, (gpointer)addr, caller_gsharedvt, need_unbox, out_arg);
  } else {
    MonoFtnDesc *desc =
        mini_get_interp_callbacks()->create_method_pointer_llvmonly(
            method, need_unbox, error);
    return_val_if_nok(error, NULL);
    *out_arg = desc->arg;
    return desc->addr;
  }
 ...
 */
MonoFtnDesc *mini_llvmonly_load_method_ftndesc(MonoMethod *method,
                                               gboolean caller_gsharedvt,
                                               gboolean need_unbox,
                                               MonoError *error) {
  gpointer addr = mono_compile_method_checked(method, error);
  ...
  if (addr) {
    gpointer arg = NULL;
    addr = mini_llvmonly_add_method_wrappers(
        method, (gpointer)addr, caller_gsharedvt, need_unbox, &arg);
    // FIXME: Cache this
 ...
 */
gpointer mini_llvmonly_load_method_delegate(MonoMethod *method,
                                            gboolean caller_gsharedvt,
                                            gboolean need_unbox,
                                            gpointer *out_arg,
                                            MonoError *error) {
...
gpointer mini_llvmonly_get_delegate_arg(MonoMethod *method,
                                        gpointer method_ptr) {
  gpointer arg = NULL;
  ...
  if (method->is_inflated && (mono_aot_get_method_flags((guint8 *)method_ptr) &
 ...
 */
MonoFtnDesc *mini_llvmonly_create_ftndesc(MonoDomain *domain, gpointer addr,
                                          gpointer arg) {
  MonoFtnDesc *ftndesc = (MonoFtnDesc *)mono_domain_alloc0(
      mono_domain_get(), 2 * sizeof(gpointer));
  ftndesc->addr = addr;
  ftndesc->arg = arg;
 ...
 *
 *   Add unbox/gsharedvt wrappers around COMPILED_METHOD if needed. Return the
 * wrapper address or COMPILED_METHOD if no wrapper is needed. Set OUT_ARG to
 * the rgctx/extra argument needed to be passed to the returned method.
 */
gpointer mini_llvmonly_add_method_wrappers(MonoMethod *m,
  ...
  *out_arg = NULL;
  ...
  callee_gsharedvt = mono_aot_get_method_flags((guint8 *)compiled_method) &
                     MONO_AOT_METHOD_FLAG_GSHAREDVT_VARIABLE;
  ...
  if (!caller_gsharedvt && callee_gsharedvt) {
    ...
    /*
     * This is a gsharedvt in wrapper, it gets passed a ftndesc for the
     * gsharedvt method as an argument.
     */
    *out_arg = mini_llvmonly_create_ftndesc(mono_domain_get(), addr,
  ...
  if (caller_gsharedvt && !callee_gsharedvt) {
    /*
     * The callee uses the gsharedvt calling convention, have to add an out
     * wrapper.
     */
    gpointer out_wrapper = mini_get_gsharedvt_wrapper(
        FALSE, NULL, mono_method_signature_internal(m), NULL, -1, FALSE);
    MonoFtnDesc *out_wrapper_arg =
        mini_llvmonly_create_ftndesc(mono_domain_get(), addr, *out_arg);
 ...
 *
 *  This function is called the first time a call is made through an IMT
 * trampoline. It should have the same signature as the llvmonly_imt_tramp_...
 * functions.
 */
static gpointer mini_llvmonly_initial_imt_tramp(gpointer *arg,
...
static gpointer llvmonly_imt_tramp(gpointer *arg, MonoMethod *imt_method) {
  int i = 0;
...
/* Optimized versions of mini_llvmonly_imt_trampoline () for different table
 * sizes */
static gpointer llvmonly_imt_tramp_1(gpointer *arg, MonoMethod *imt_method) {
  // g_assert (arg [0] == imt_method);
  return arg[1];
}

static gpointer llvmonly_imt_tramp_2(gpointer *arg, MonoMethod *imt_method) {
...
static gpointer llvmonly_imt_tramp_3(gpointer *arg, MonoMethod *imt_method) {
  // g_assert (arg [0] == imt_method || arg [2] == imt_method || arg [4] ==
  // imt_method);
  if (arg[0] == imt_method)
    return arg[1];
...
/*
 * A version of the imt trampoline used for generic virtual/variant iface
 * methods. Unlikely a normal imt trampoline, its possible that IMT_METHOD is
 * not found in the search table. The original JIT code had a 'fallback'
 * trampoline it could call, but we can't do that, so we just return NULL, and
 * the compiled code will handle it.
 */
static gpointer llvmonly_fallback_imt_tramp(gpointer *arg,
                                            MonoMethod *imt_method) {
  int i = 0;
...
gpointer mini_llvmonly_get_imt_trampoline(MonoVTable *vtable,
                                          MonoDomain *domain,
   ...
   * The array contains MonoMethod-function descriptor pairs, terminated by a
   * NULL entry.
   */
...
gpointer mini_llvmonly_get_vtable_trampoline(MonoVTable *vt, int slot_index,
                                             int index) {
  if (slot_index < 0) {
    /* Initialize the IMT trampoline to a 'trampoline' so the generated code
     * doesn't have to initialize it */
 ...
 * This returns an ftnptr by returning the address part, and the arg in the
 * OUT_ARG out parameter.
 */
static gpointer resolve_vcall(MonoVTable *vt, int slot, MonoMethod *imt_method,
                              gpointer *out_arg, gboolean gsharedvt,
                              MonoError *error) {
  ...
  addr = compiled_method =
      mini_llvmonly_load_method(m, gsharedvt, need_unbox_tramp, out_arg, error);
  mono_error_assert_ok(error);

  if (!gsharedvt && generic_virtual) {
    // FIXME: This wastes memory since add_generic_virtual_invocation ignores it
    // in a lot of cases
    MonoFtnDesc *ftndesc =
        mini_llvmonly_create_ftndesc(mono_domain_get(), addr, out_arg);
...
gpointer mini_llvmonly_resolve_vcall_gsharedvt(MonoObject *this_obj, int slot,
                                               MonoMethod *imt_method,
                                               gpointer *out_arg) {
  g_assert(this_obj);
  ...
  ERROR_DECL(error);
  gpointer result =
      resolve_vcall(this_obj->vtable, slot, imt_method, out_arg, TRUE, error);
  if (!is_ok(error)) {
 ...
 */
MonoFtnDesc *
mini_llvmonly_resolve_generic_virtual_call(MonoVTable *vt, int slot,
                                           MonoMethod *generic_virtual) {
  MonoMethod *m;
  gboolean need_unbox_tramp = FALSE;
  ERROR_DECL(error);
  MonoGenericContext context = {NULL, NULL};
   ...
   * mono_method_add_generic_virtual_invocation () eventually builds an imt
   * trampoline for this vtable slot so we are not called any more for this
   * instantiation.
   */
  MonoFtnDesc *ftndesc =
      mini_llvmonly_load_method_ftndesc(m, FALSE, need_unbox_tramp, error);
  mono_error_assert_ok(error);
 ...
 */
MonoFtnDesc *
mini_llvmonly_resolve_generic_virtual_iface_call(MonoVTable *vt, int imt_slot,
                                                 MonoMethod *generic_virtual) {
  ERROR_DECL(error);
  ...
  mini_resolve_imt_method(vt, imt + imt_slot, generic_virtual, &m, &aot_addr,
                          &need_rgctx_tramp, &variant_iface, error);
  if (!is_ok(error)) {
    MonoException *ex = mono_error_convert_to_exception(error);
    mono_llvm_throw_exception((MonoObject *)ex);
  }
  ...
  mono_method_add_generic_virtual_invocation(
      mono_domain_get(), vt, imt + imt_slot,
 ...
 */
gpointer mini_llvmonly_init_vtable_slot(MonoVTable *vtable, int slot) {
  ERROR_DECL(error);
  gpointer arg = NULL;
  gpointer addr;
  gpointer *ftnptr;
 ...
 */
void mini_llvmonly_init_delegate(MonoDelegate *del) {
  ERROR_DECL(error);
  ...
  if (G_UNLIKELY(!ftndesc)) {
    ...
    if (m_class_is_valuetype(m->klass) &&
        mono_method_signature_internal(m)->hasthis)
      need_unbox = TRUE;

    gpointer arg = NULL;
    gpointer addr =
        mini_llvmonly_load_method_delegate(m, FALSE, need_unbox, &arg, error);
    if (mono_error_set_pending_exception(error))
      return;
...
void mini_llvmonly_init_delegate_virtual(MonoDelegate *del, MonoObject *target,
                                         MonoMethod *method) {
  ERROR_DECL(error);
  gpointer addr, arg;
  gboolean need_unbox;
  ...
  del->method = method;
  addr = mini_llvmonly_load_method_delegate(method, FALSE, need_unbox, &arg,
                                            error);
  if (mono_error_set_pending_exception(error))
    return;
  del->method_ptr = addr;
 ...
 * OUT_ARG out parameter.
 */
static gpointer resolve_iface_call(MonoObject *this_obj, int imt_slot,
                                   MonoMethod *imt_method, gpointer *out_arg,
                                   gboolean caller_gsharedvt,
                                   MonoError *error) {
  MonoVTable *vt;
  gpointer *imt;
  ...
  mini_resolve_imt_method(vt, imt + imt_slot, imt_method, &impl_method,
                          &aot_addr, &need_rgctx_tramp, &variant_iface, error);
  return_val_if_nok(error, NULL);
  ...
  addr = mini_llvmonly_load_method(impl_method, caller_gsharedvt,
                                   need_unbox_tramp, out_arg, error);
  mono_error_assert_ok(error);
  g_assert(addr);
  ...
  if (generic_virtual || variant_iface) {
    ...
    mono_method_add_generic_virtual_invocation(mono_domain_get(), vt,
                                               imt + imt_slot, target, addr);
  }

  return addr;
}

gpointer mini_llvmonly_resolve_iface_call_gsharedvt(MonoObject *this_obj,
                                                    int imt_slot,
                                                    MonoMethod *imt_method,
                                                    gpointer *out_arg) {
  ERROR_DECL(error);
  gpointer res =
      resolve_iface_call(this_obj, imt_slot, imt_method, out_arg, TRUE, error);
  if (!is_ok(error)) {
    MonoException *ex = mono_error_convert_to_exception(error);
    mono_llvm_throw_exception((MonoObject *)ex);
  }
...
static void init_llvmonly_method(MonoAotModule *amodule, guint32 method_index,
...
void mini_llvm_init_method(MonoAotFileInfo *info, gpointer aot_module,
                           guint32 method_index, MonoVTable *vtable) {
  MonoAotModule *amodule = (MonoAotModule *)aot_module;

  init_llvmonly_method(amodule, method_index, vtable ? vtable->klass : NULL);
}

static GENERATE_GET_CLASS_WITH_CACHE(nullref, "System",
...
static GENERATE_GET_CLASS_WITH_CACHE(missing_method, "System",
                                     "MissingMethodException")

    void mini_llvmonly_throw_missing_method_exception(void) {
