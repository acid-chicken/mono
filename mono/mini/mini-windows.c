...
#define MONO_HANDLER_ATEXIT_WAIT_KEYPRESS_LEN                                  \
  G_N_ELEMENTS(MONO_HANDLER_ATEXIT_WAIT_KEYPRESS) - 1
 ...
 *
 * This function is installed as an atexit function making sure that the console
 * is not terminated before the end user has a chance to read the result. This
 * can be handy in debug scenarios (running from within the debugger) since an
 * exit of the process will close the console window without giving the end user
 * a chance to look at the output before closed.
 */
static void atexit_wait_keypress(void) {
 ...
 */
static void install_atexit_wait_keypress(void) {
 ...
 */
static void install_atexit_wait_keypress(void) { return; }

#endif /* G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT) */

// Table describing handlers that can be installed at process startup. Adding a
// new handler can be done by adding a new item to the table together with an
// install handler function.
const HandlerItem g_handler_items[] = {{MONO_HANDLER_ATEXIT_WAIT_KEYPRESS,
 ...
 */
static size_t get_next_handler_arg_len(const char *handlers) {
 ...
 * @handlers: Handlers included in --handler argument, example
 * "atexit-waitkeypress,someothercmd,yetanothercmd".
 * @handler_arg_len: Output, length of consumed handler.
 *
 * This function installs the next handler included in @handlers parameter.
 *
 * Returns: TRUE on successful install, FALSE on failure or unrecognized
 * handler.
 */
static gboolean install_custom_handler(const char *handlers,
  ...
  *handler_arg_len = get_next_handler_arg_len(handlers);
  for (int current_item = 0; current_item < G_N_ELEMENTS(g_handler_items);
       ++current_item) {
    ...
    if (*handler_arg_len == handler_item->cmd_len &&
        strncmp(handlers, handler_item->cmd, *handler_arg_len) == 0) {
...
void mono_runtime_install_custom_handlers_usage(void) {
  fprintf(stdout,
          "Custom Handlers:\n"
          "   --handlers=HANDLERS            Enable handler support, HANDLERS "
          "is a comma\n"
          "                                  separated list of available "
          "handlers to install.\n"
          "\n"
#if G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT)
          "HANDLERS is composed of:\n"
          "    atexit-waitkeypress           Install an atexit handler waiting "
          "for a keypress\n"
...
void mono_init_native_crash_info(void) { return; }

void mono_cleanup_native_crash_info(void) { return; }
...
gboolean MONO_SIG_HANDLER_SIGNATURE(mono_chain_signal) {
  /* Set to FALSE to indicate that vectored exception handling should continue
   * to look for handler */
...
static VOID thread_timer_expired(HANDLE thread) {
  CONTEXT context;
...
static VOID CALLBACK timer_event_proc(UINT uID, UINT uMsg, DWORD_PTR dwUser,
                                      DWORD_PTR dw1, DWORD_PTR dw2) {
  thread_timer_expired((HANDLE)dwUser);
}

static VOID stop_profiler_timer_event(void) {
  if (g_timer_event != 0) {
...
static VOID start_profiler_timer_event(void) {
  g_return_if_fail(g_timer_main_thread == INVALID_HANDLE_VALUE &&
                   g_timer_event == 0);
  ...
  g_timer_main_thread = OpenThread(READ_CONTROL | THREAD_GET_CONTEXT, FALSE,
                                   GetCurrentThreadId());
  ...
  g_timer_event = timeSetEvent(1, 0, (LPTIMECALLBACK)timer_event_proc,
                               (DWORD_PTR)g_timer_main_thread,
...
gboolean mono_setup_thread_context(DWORD thread_id, MonoContext *mono_context) {
  HANDLE handle;
  CONTEXT context;
...
gboolean mono_thread_state_init_from_handle(MonoThreadUnwindState *tctx,
  ...
  /*
   * The current LMF address is kept in a separate TLS variable, and its hard to
   * read its value without arch-specific code. But the address of the TLS
   * variable is stored in another TLS variable which can be accessed through
   * MonoThreadInfo.
   */
...
BOOL mono_win32_runtime_tls_callback(HMODULE module_handle, DWORD reason,
                                     LPVOID reserved,
