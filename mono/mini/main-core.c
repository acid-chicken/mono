...
// Ensure that the exported symbols are not decorated and that only one set is
// exported
#pragma comment(linker, "/export:coreclr_initialize=_coreclr_initialize@28")
#pragma comment(                                                               \
    linker, "/export:coreclr_execute_assembly=_coreclr_execute_assembly@24")
#pragma comment(linker, "/export:coreclr_shutdown_2=_coreclr_shutdown_2@12")
#pragma comment(linker,                                                        \
                "/export:coreclr_create_delegate=_coreclr_create_delegate@24")
#undef MONO_API
#define MONO_API MONO_EXTERN_C
#endif

MONO_API int STDAPICALLTYPE coreclr_initialize(
...
MONO_API int STDAPICALLTYPE coreclr_execute_assembly(
    void *hostHandle, unsigned int domainId, int argc, const char **argv,
    const char *managedAssemblyPath, unsigned int *exitCode);

MONO_API int STDAPICALLTYPE coreclr_shutdown_2(void *hostHandle,
                                               unsigned int domainId,
                                               int *latchedExitCode);

MONO_API int STDAPICALLTYPE coreclr_create_delegate(
    void *hostHandle, unsigned int domainId, const char *entryPointAssemblyName,
    const char *entryPointTypeName, const char *entryPointMethodName,
    void **delegate);
...
static void mono_core_trusted_platform_assemblies_free(
    MonoCoreTrustedPlatformAssemblies *a) {
  if (!a)
    return;
...
static void mono_core_native_lib_paths_free(MonoCoreNativeLibPaths *dl) {
  if (!dl)
    return;
  g_strfreev(dl->dirs);
  g_free(dl);
...
parse_trusted_platform_assemblies(const char *assemblies_paths) {
  ...
  MonoCoreTrustedPlatformAssemblies *a =
      g_new0(MonoCoreTrustedPlatformAssemblies, 1);
...
static MonoAssembly *
mono_core_preload_hook(MonoAssemblyLoadContext *alc, MonoAssemblyName *aname,
                       char **assemblies_path, gboolean refonly,
                       gpointer user_data, MonoError *error) {
  MonoAssembly *result = NULL;
  MonoCoreTrustedPlatformAssemblies *a =
      (MonoCoreTrustedPlatformAssemblies *)user_data;
  ...
  g_assert(aname);
  g_assert(aname->name);
  /* alc might be a user ALC - we get here from alc.LoadFromAssemblyName(), but
   * we should load TPA assemblies into the default alc */
  MonoAssemblyLoadContext *default_alc =
      mono_domain_default_alc(mono_alc_domain(alc));

  char *basename = g_strconcat(
      aname->name, ".dll",
      (const char *)
          NULL); /* TODO: make sure CoreCLR never needs to load .exe files */
  ...
  if (!result) {
    mono_trace(G_LOG_LEVEL_DEBUG, MONO_TRACE_ASSEMBLY,
               "netcore preload hook: did not find '%s'.", aname->name);
  } else {
    mono_trace(G_LOG_LEVEL_INFO, MONO_TRACE_ASSEMBLY,
               "netcore preload hook: loading '%s' from '%s'.", aname->name,
               result->image->name);
  }
  return result;
}

static void install_assembly_loader_hooks(void) {
  mono_install_assembly_preload_hook_v2(
      mono_core_preload_hook, (void *)trusted_platform_assemblies, FALSE);
}

static gboolean parse_properties(int propertyCount, const char **propertyKeys,
                                 const char **propertyValues) {
  // The a partial list of relevant properties is
  // https://docs.microsoft.com/en-us/dotnet/core/tutorials/netcore-hosting#step-3---prepare-runtime-properties
  // TODO: We should also pick up at least APP_PATHS and APP_NI_PATHS
  // and PLATFORM_RESOURCE_ROOTS for satellite assemblies in culture-specific
  // subdirectories
  ...
  for (int i = 0; i < propertyCount; ++i) {
    if (!strcmp(propertyKeys[i], "TRUSTED_PLATFORM_ASSEMBLIES")) {
      ...
      g_setenv("DOTNET_SYSTEM_GLOBALIZATION_INVARIANT", propertyValues[i],
               TRUE);
    } else {
#if 0
...
//  ExecuteAssembly appDomainFriendlyName   - Friendly name of the app domain
//  that will be created to execute the assembly propertyCount           -
//  Number of properties (elements of the following two arguments) propertyKeys
//  - Keys of properties of the app domain propertyValues          - Values of
//  properties of the app domain hostHandle              - Output parameter,
//  handle of the created host domainId                - Output parameter, id of
...
//
int STDAPICALLTYPE coreclr_initialize(
    const char *exePath, const char *appDomainFriendlyName, int propertyCount,
    const char **propertyKeys, const char **propertyValues, void **hostHandle,
    unsigned int *domainId) {
  mono_runtime_register_appctx_properties(propertyCount, propertyKeys,
                                          propertyValues);

  // TODO: TRUSTED_PLATFORM_ASSEMBLIES is the property key for managed
  // assemblies mapping
  ...
  if (native_lib_paths != NULL)
    mono_set_pinvoke_search_directories(native_lib_paths->dir_count,
...
//  successfully executed
//
int STDAPICALLTYPE coreclr_execute_assembly(void *hostHandle,
                                            unsigned int domainId, int argc,
                                            const char **argv,
  ...
  char **mono_argv =
      (char **)malloc(sizeof(char *) * (mono_argc + 1 /* null terminated */));
...
//  successfully executed
//
int STDAPICALLTYPE coreclr_shutdown_2(void *hostHandle, unsigned int domainId,
                                      int *latchedExitCode) {
...
//  a pointer to the delegate at the specified address
//
// Returns:
//  HRESULT indicating status of the operation. S_OK if the assembly was
//  successfully executed
//
int STDAPICALLTYPE coreclr_create_delegate(void *hostHandle,
                                           unsigned int domainId,
