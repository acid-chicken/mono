...
static void mono_linear_scan2(MonoCompile *cfg, GList *vars, GList *regs,
                              regmask_t *used_mask);
...
static gint compare_by_first_use_func(gconstpointer a, gconstpointer b) {
...
void mono_linear_scan(MonoCompile *cfg, GList *vars, GList *regs,
  ...
  if (!cfg->disable_reuse_registers && vars &&
      (((MonoMethodVar *)vars->data)->interval != NULL)) {
  ...
  for (l = vars; l; l = l->next) {
    ...
    if (active && g_list_length(active) == max_regs) {
      ...
      if ((cost_driven && amv->spill_costs < vmv->spill_costs) ||
          (!cost_driven &&
           amv->range.last_use.abs_pos > vmv->range.last_use.abs_pos)) {
        vmv->reg = amv->reg;
        amv->reg = -1;
      ...
      if ((gains[vmv->reg] > mono_arch_regalloc_cost(cfg, vmv)) &&
        ...
        if (cfg->verbose_level > 2) {
          printf("ALLOCATED R%d(%d) TO HREG %d COST %d\n",
                 cfg->varinfo[vmv->idx]->dreg, vmv->idx, vmv->reg,
                 vmv->spill_costs);
        }
        cfg->varinfo[vmv->idx]->opcode = OP_REGVAR;
        ...
        if (cfg->verbose_level > 2)
          printf("COSTLY: R%d C%d C%d %s\n", vmv->idx, vmv->spill_costs,
                 mono_arch_regalloc_cost(cfg, vmv),
                 mono_arch_regname(vmv->reg));
        vmv->reg = -1;
      }
    }
...
static guint32 regalloc_cost(MonoCompile *cfg, MonoMethodVar *vmv) {
...
void mono_linear_scan2(MonoCompile *cfg, GList *vars, GList *regs,
                       regmask_t *used_mask) {
  ...
  unhandled =
      g_list_sort(g_list_copy(vars), compare_by_interval_start_pos_func);
  active = NULL;
  inactive = NULL;
  ...
  while (unhandled) {
    ...
    while (changed) {
      ...
      for (l = active; l != NULL; l = l->next) {
        ...
        if (v->interval->last_range->to < pos) {
          ...
          LSCAN_DEBUG(
              printf("Interval R%d has expired\n", cfg->varinfo[v->idx]->dreg));
          changed = TRUE;
          break;
    ...
    for (l = inactive; l != NULL; l = l->next) {
      ...
      if (v->reg >= 0) {
        intersect_pos =
            mono_linterval_get_intersect_pos(current->interval, v->interval);
        if (intersect_pos != -1) {
          free_pos[v->reg] = intersect_pos;
          LSCAN_DEBUG(
              printf("\threg %d becomes free at %d\n", v->reg, intersect_pos));
        }
      }
    }
    ...
    if (free_pos[reg] >= current->interval->last_range->to) {
      ...
      if (active) {
        ...
        if (vmv->spill_costs < current->spill_costs) {
          //				if (vmv->interval->last_range->to <
          //current->interval->last_range->to) {
  ...
  for (l = vars; l; l = l->next) {
    ...
    if (vmv->reg >= 0) {
      ...
      if ((gains[reg_index] > regalloc_cost(cfg, vmv)) &&
          (cfg->varinfo[vmv->idx]->opcode != OP_REGVAR)) {
        if (cfg->verbose_level > 2)
          printf("REGVAR R%d G%d C%d %s\n", cfg->varinfo[vmv->idx]->dreg,
                 gains[reg_index], regalloc_cost(cfg, vmv),
                 mono_arch_regname(vmv->reg));
        cfg->varinfo[vmv->idx]->opcode = OP_REGVAR;
        cfg->varinfo[vmv->idx]->dreg = vmv->reg;
        n_regvars++;
        ...
        if (cfg->verbose_level > 2)
          printf("COSTLY: %s R%d G%d C%d %s\n",
                 mono_method_full_name(cfg->method, TRUE),
                 cfg->varinfo[vmv->idx]->dreg, gains[reg_index],
                 regalloc_cost(cfg, vmv), mono_arch_regname(vmv->reg));
        vmv->reg = -1;
      }
    }
  }
