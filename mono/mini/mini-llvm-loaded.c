...
typedef struct {
  ...
  void (*create_aot_module)(MonoAssembly *assembly, const char *global_prefix,
                            int initial_got_size, LLVMModuleFlags flags);
...
void mono_llvm_init(gboolean enable_jit) { backend.init(enable_jit); }

void mono_llvm_cleanup(void) { backend.cleanup(); }

void mono_llvm_emit_method(MonoCompile *cfg) { backend.emit_method(cfg); }

void mono_llvm_emit_call(MonoCompile *cfg, MonoCallInst *call) {
  backend.emit_call(cfg, call);
}

void mono_llvm_create_aot_module(MonoAssembly *assembly,
...
void mono_llvm_fixup_aot_module(void) { backend.fixup_aot_module(); }
...
void mono_llvm_create_vars(MonoCompile *cfg) { backend.create_vars(cfg); }
...
int mono_llvm_load(const char *bpath) {
  ...
  if (err)
    goto symbol_error;
  err =
      mono_dl_symbol(llvm_lib, "mono_llvm_cleanup", (void **)&backend.cleanup);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_emit_method",
                       (void **)&backend.emit_method);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_emit_call",
                       (void **)&backend.emit_call);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_create_aot_module",
                       (void **)&backend.create_aot_module);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_emit_aot_module",
                       (void **)&backend.emit_aot_module);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_fixup_aot_module",
                       (void **)&backend.fixup_aot_module);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_check_method_supported",
                       (void **)&backend.check_method_supported);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_free_domain_info",
                       (void **)&backend.free_domain_info);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_emit_aot_file_info",
                       (void **)&backend.emit_aot_file_info);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_emit_aot_data",
                       (void **)&backend.emit_aot_data);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_create_vars",
                       (void **)&backend.create_vars);
  if (err)
    goto symbol_error;
  err = mono_dl_symbol(llvm_lib, "mono_llvm_get_cpu_features",
                       (void **)&backend.get_cpu_features);
  if (err)
    goto symbol_error;
  return TRUE;
symbol_error:
...
int mono_llvm_load(const char *bpath) { return TRUE; }
