...
/*
FIXME:
- Move EventKind back to debugger-agent.c as it contains sdb wire protocol
constants. This is complicated because EventRequest has an event_kind field.
...
typedef enum { STEP_SIZE_MIN = 0, STEP_SIZE_LINE = 1 } StepSize;
...
typedef struct {
  ...
  gboolean caught, uncaught, subclasses, not_filtered_feature,
      everything_else; /* For kind == MOD_KIND_EXCEPTION_ONLY */
} Modifier;
...
typedef struct {
   ...
   */
  int async_id;
  /* Used to know if we are in process of async step-out and distishing from
...
MonoGHashTable *mono_debugger_get_thread_states(void);

gboolean mono_debugger_is_disconnected(void);

gsize mono_debugger_tls_thread_id(DebuggerTlsData *debuggerTlsData);

void mono_debugger_set_thread_state(DebuggerTlsData *ref,
                                    MonoDebuggerThreadState expected,
                                    MonoDebuggerThreadState set);

MonoDebuggerThreadState mono_debugger_get_thread_state(DebuggerTlsData *ref);

typedef struct {
  MonoContext *(*tls_get_restore_state)(void *tls);
  gboolean (*try_process_suspend)(void *tls, MonoContext *ctx,
                                  gboolean from_breakpoint);
  gboolean (*begin_breakpoint_processing)(void *tls, MonoContext *ctx,
                                          MonoJitInfo *ji,
  ...
  void (*ss_calculate_framecount)(void *tls, MonoContext *ctx,
                                  gboolean force_use_ctx,
                                  DbgEngineStackFrame ***frames, int *nframes);
  ...
  void *(*create_breakpoint_events)(GPtrArray *ss_reqs, GPtrArray *bp_reqs,
                                    MonoJitInfo *ji, EventKind kind);
  void (*process_breakpoint_events)(void *_evts, MonoMethod *method,
                                    MonoContext *ctx, int il_offset);

  gboolean (*set_set_notification_for_wait_completion_flag)(
      DbgEngineStackFrame *f);
...
// locking - we expose the lock object from the debugging engine to ensure we
// keep the same locking semantics of sdb.
