...
static const char *storage_name(ArgStorage st) {
  switch (st) {
  case ArgInIReg:
    return "ArgInIReg";
...
static char *arg_info_desc(ArgInfo *info) {
  GString *str = g_string_new("");

  g_string_append_printf(str, "offset %d reg %s storage %s nregs %d",
                         info->offset, mono_arch_regname(info->reg),
                         storage_name(info->storage), info->nregs);
...
static void add_to_map(GPtrArray *map, int src, int dst) {
...
static int map_freg(int reg) { return reg + PARAM_REGS; }

static int map_stack_slot(int slot) {
...
static int get_arg_slots(ArgInfo *ainfo, int **out_slots,
  ...
  case ArgOnStack:
    ...
    src = g_malloc(nsrc * sizeof(int));
    // is_source_argument adds 2 because we're skipping over the old BBP and the
    // return address
    // XXX this is a very fragile setup as changes in alignment for the caller
    // reg array can cause the magic number be 3
    for (i = 0; i < nsrc; ++i)
...
static void handle_marshal_when_src_gsharedvt(ArgInfo *dst_info,
                                              int *arg_marshal,
                                              int *arg_slots) {
  ...
  case ArgOnStack:
    *arg_marshal = GSHAREDVT_ARG_BYREF_TO_BYVAL;
    g_assert(dst_info->arg_size % SLOT_BYTE_SIZE ==
  ...
  case ArgValuetypeAddrOnStack:
    ...
    *arg_slots = dst_info->nregs;
    break;
  default:
    NOT_IMPLEMENTED; // Inappropriate value: if dst and src are gsharedvt at
                     // once, we shouldn't be here
    break;
  }
}

// Once dst is known, operate on the src
static void handle_marshal_when_dst_gsharedvt(ArgInfo *src_info,
                                              int *arg_marshal) {
...
static void handle_map_when_gsharedvt_in_reg(ArgInfo *reg_info, int *n,
                                             int **map) {
  *n = 1;
  *map = g_new0(int, 1);
...
static void handle_map_when_gsharedvt_on_stack(ArgInfo *reg_info, int *n,
                                               int **map,
                                               gboolean is_source_argument) {
  *n = 1;
  *map = g_new0(int, 1);
  ...
  (*map)[0] =
      map_stack_slot(sslot + (is_source_argument ? 2 : 0)); // see get_arg_slots
}
...
gpointer mono_arch_get_gsharedvt_call_info(gpointer addr,
  ...
  /*
   * If GSHAREDVT_IN is true, this means we are transitioning from normal to
   * gsharedvt code. The caller uses the normal call signature, while the callee
   * uses the gsharedvt signature. If GSHAREDVT_IN is false, its the other way
   * around.
   */

  /* sig/cinfo describes the normal call, while gcinfo describes the gsharedvt
   * call */
  if (gsharedvt_in) {
    sig = caller_sig;
    cinfo = caller_cinfo;
  ...
  DEBUG_AMD64_GSHAREDVT_PRINT(
      ...
      mono_signature_get_desc(caller_sig, FALSE),
      mono_type_full_name(
          mono_signature_get_return_type_internal(caller_sig))); // Leak
  DEBUG_AMD64_GSHAREDVT_PRINT(
      "dest sig: (%s) return (%s)\n",
      mono_signature_get_desc(callee_sig, FALSE),
      mono_type_full_name(mono_signature_get_return_type_internal(callee_sig)));
   ...
   * <trampoline frame>
   * <call area>
   * We have to map the stack slots in <arguments> to the stack slots in <call
   * area>.
   */
  map = g_ptr_array_new();
  ...
  for (aindex = 0; aindex < cinfo->nargs; ++aindex) {
    ...
    default:
      g_error(
          "Gsharedvt can't handle source arg type %d",
          (int)src_info->storage); // Inappropriate value:
                                   // ArgValuetypeAddrInIReg is for returns only
    }
    ...
    case ArgValuetypeAddrOnStack:
      ...
      break;
    default:
      g_error("Gsharedvt can't handle dest arg type %d",
              (int)dst_info->storage); // See above
    }

    if (arg_marshal == GSHAREDVT_ARG_BYREF_TO_BYVAL &&
        dst_info->byte_arg_size) {
      /* Have to load less than 4 bytes */
      switch (dst_info->byte_arg_size) {
      case 1:
        arg_marshal = dst_info->is_signed ? GSHAREDVT_ARG_BYREF_TO_BYVAL_I1
                                          : GSHAREDVT_ARG_BYREF_TO_BYVAL_U1;
        break;
      case 2:
        arg_marshal = dst_info->is_signed ? GSHAREDVT_ARG_BYREF_TO_BYVAL_I2
                                          : GSHAREDVT_ARG_BYREF_TO_BYVAL_U2;
        break;
      default:
        arg_marshal = dst_info->is_signed ? GSHAREDVT_ARG_BYREF_TO_BYVAL_I4
    ...
    if (nsrc)
      src[0] |= (arg_marshal << SRC_DESCRIPTOR_MARSHAL_SHIFT) |
                (arg_slots << SLOT_COUNT_SHIFT);
  ...
  DEBUG_AMD64_GSHAREDVT_PRINT("-- return in (%s) out (%s) var_ret %d\n",
                              arg_info_desc(&caller_cinfo->ret),
                              arg_info_desc(&callee_cinfo->ret), var_ret);

  if (cinfo->ret.storage == ArgValuetypeAddrInIReg) {
    /* Both the caller and the callee pass the vtype ret address in r8 (System
     * V) and RCX or RDX (Windows) */
    g_assert(gcinfo->ret.storage == ArgValuetypeAddrInIReg ||
             gcinfo->ret.storage == ArgGsharedvtVariableInReg);
    add_to_map(map, map_reg(caller_cinfo->ret.reg),
               map_reg(callee_cinfo->ret.reg));
  }

  info = mono_domain_alloc0(mono_domain_get(), sizeof(GSharedVtCallInfo) +
                                                   (map->len * sizeof(int)));
  info->addr = addr;
  ...
  if (var_ret) {
    ...
    DEBUG_AMD64_GSHAREDVT_PRINT("mapping vreg_arg_reg to %d in reg %s\n",
                                info->vret_arg_reg,
                                mono_arch_regname(gcinfo->ret.reg));
  } else {
    info->vret_arg_reg = -1;
  }
  ...
  if (var_ret) {
    ...
    case ArgValuetypeAddrInIReg:
      break;
    default:
      g_error("Can't marshal return of storage [%d] %s",
              (int)cinfo->ret.storage, storage_name(cinfo->ret.storage));
    }

    if (gsharedvt_in && cinfo->ret.storage != ArgValuetypeAddrInIReg) {
      /* Allocate stack space for the return value */
      info->vret_slot =
          map_stack_slot(info->stack_usage / sizeof(target_mgreg_t));
      info->stack_usage +=
          mono_type_stack_size_internal(normal_sig->ret, NULL, FALSE) +
