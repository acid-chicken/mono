...
static void mono_walk_stack_full(MonoJitStackWalk func, MonoContext *start_ctx,
                                 MonoDomain *domain, MonoJitTlsData *jit_tls,
                                 MonoLMF *lmf, MonoUnwindOptions unwind_options,
                                 gpointer user_data, gboolean crash_context);
static void mono_raise_exception_with_ctx(MonoException *exc, MonoContext *ctx);
static void mono_runtime_walk_stack_with_ctx(MonoJitStackWalk func,
                                             MonoContext *start_ctx,
...
static gboolean mono_exception_walk_trace_internal(MonoException *ex,
...
static void mono_summarize_exception(MonoException *exc,
                                     MonoThreadSummary *out);
static void
mono_crash_reporting_register_native_library(const char *module_path,
                                             const char *module_name);
static void mono_crash_reporting_allow_all_native_libraries(void);

static gboolean first_managed(MonoStackFrameInfo *frame, MonoContext *ctx,
                              gpointer addr) {
...
static gpointer mono_thread_get_managed_sp(void) {
  gpointer addr = NULL;
  mono_walk_stack(first_managed, MONO_UNWIND_SIGNAL_SAFE, &addr);
  return addr;
}

static void mini_clear_abort_threshold(void) {
  MonoJitTlsData *jit_tls = mono_get_jit_tls();
  jit_tls->abort_exc_stack_threshold = NULL;
}

static void mini_set_abort_threshold(StackFrameInfo *frame) {
  ...
  gboolean above_threshold =
      (gsize)sp >= (gsize)jit_tls->abort_exc_stack_threshold;
...
static gboolean mini_above_abort_threshold(void) {
...
static int mono_get_seq_point_for_native_offset(MonoDomain *domain,
                                                MonoMethod *method,
                                                gint32 native_offset) {
  SeqPoint sp;
  if (mono_find_prev_seq_point_for_native_offset(domain, method, native_offset,
                                                 NULL, &sp))
    return sp.il_offset;
  return -1;
}
...
void mono_exceptions_init(void) {
  ...
  if (mono_ee_features.use_aot_trampolines) {
    ...
    mono_tramp_info_register(info, NULL);
    rethrow_preserve_exception_func =
        mono_arch_get_rethrow_preserve_exception(&info, FALSE);
    mono_tramp_info_register(info, NULL);
  }
  ...
  cbs.mono_register_native_library =
      mono_crash_reporting_register_native_library;
  cbs.mono_allow_all_native_libraries =
  ...
  if (mono_llvm_only) {
    ...
    cbs.mono_raise_exception =
        (void (*)(MonoException *))mono_get_throw_exception();
    cbs.mono_reraise_exception =
        (void (*)(MonoException *))mono_get_rethrow_exception();
  }
  cbs.mono_raise_exception_with_ctx = mono_raise_exception_with_ctx;
  cbs.mono_exception_walk_trace = mono_exception_walk_trace;
  cbs.mono_install_handler_block_guard = mono_install_handler_block_guard;
  cbs.mono_uninstall_current_handler_block_guard =
      mono_uninstall_current_handler_block_guard;
  cbs.mono_current_thread_has_handle_block_guard =
...
static void no_call_filter(void) { g_assert_not_reached(); }
 ...
 */
gpointer mono_get_throw_exception_addr(void) { return &throw_exception_func; }

gpointer mono_get_rethrow_preserve_exception_addr(void) {
  return &rethrow_preserve_exception_func;
}

static gboolean is_address_protected(MonoJitInfo *ji, MonoJitExceptionInfo *ei,
                                     gpointer ip) {
  ...
  for (i = 0; i < table->num_holes; ++i) {
    ...
    if (hole->clause == clause && hole->offset <= offset &&
        hole->offset + hole->length > offset)
      return FALSE;
  }
  return TRUE;
}
...
static _Unwind_Reason_Code build_stack_trace(struct _Unwind_Context *frame_ctx,
                                             void *state) {
...
static GSList *get_unwind_backtrace(void) {
  GSList *ips = NULL;
...
static GSList *get_unwind_backtrace(void) { return NULL; }

#endif

static gboolean arch_unwind_frame(MonoDomain *domain, MonoJitTlsData *jit_tls,
    ...
    if (((gsize)(*lmf)->previous_lmf) & 2) {
      ...
      *new_ctx = *ctx;
      ...
      if (ext->kind == MONO_LMFEXT_DEBUGGER_INVOKE) {
        /*
         * This LMF entry is created by the soft debug code to mark transitions
         * to managed code done during invokes.
         */
        frame->type = FRAME_TYPE_DEBUGGER_INVOKE;
        memcpy(new_ctx, &ext->ctx, sizeof(MonoContext));
      } else if (ext->kind == MONO_LMFEXT_INTERP_EXIT ||
  ...
  return mono_arch_unwind_frame(domain, jit_tls, ji, ctx, new_ctx, lmf,
 ...
 */
static MonoJitInfo *find_jit_info(MonoDomain *domain, MonoJitTlsData *jit_tls,
  ...
  if (*lmf && ((*lmf) != jit_tls->first_lmf) &&
      ((gpointer)MONO_CONTEXT_GET_SP(new_ctx) >= (gpointer)(*lmf))) {
    /*
     * Remove any unused lmf.
     * Mask out the lower bits which might be used to hold additional
     * information.
     */
    *lmf = (MonoLMF *)(((gsize)(*lmf)->previous_lmf) &
                       ~(TARGET_SIZEOF_VOID_P - 1));
  }
  ...
  case FRAME_TYPE_DEBUGGER_INVOKE: {
     ...
     * skip it.
     */
    ji = find_jit_info(domain, jit_tls, res, NULL, new_ctx, &tmp_ctx, lmf,
                       managed);
 ...
 */
MonoJitInfo *mono_find_jit_info(MonoDomain *domain, MonoJitTlsData *jit_tls,
                                MonoJitInfo *res, MonoJitInfo *prev_ji,
                                MonoContext *ctx, MonoContext *new_ctx,
  ...
  ji = find_jit_info(domain, jit_tls, res, prev_ji, ctx, new_ctx, lmf,
                     &managed2);
 ...
 *
 *   A version of mono_find_jit_info which returns all data in the
 * StackFrameInfo structure. A note about frames of type
 * FRAME_TYPE_MANAGED_TO_NATIVE:
 * - These frames are used to mark managed-to-native transitions, so CTX will
 * refer to native code, and new_ctx will refer to the last managed frame. The
 * caller should unwind once more to obtain the last managed frame. If
 * SAVE_LOCATIONS is not NULL, it should point to an array of size
 * MONO_MAX_IREGS. On return, it will be filled with the locations where callee
 * saved registers are saved by the current frame. This is returned outside of
 * StackFrameInfo because it can be quite large on some platforms. If ASYNC
 * true, this function will be async safe, but some fields of frame and
 * frame->ji will not be set.
 */
gboolean mono_find_jit_info_ext(MonoDomain *domain, MonoJitTlsData *jit_tls,
                                MonoJitInfo *prev_ji, MonoContext *ctx,
                                MonoContext *new_ctx, char **trace,
                                MonoLMF **lmf, host_mgreg_t **save_locations,
                                StackFrameInfo *frame) {
  gboolean err;
  gpointer ip = MONO_CONTEXT_GET_IP(ctx);
  MonoJitInfo *ji;
  ...
  err = arch_unwind_frame(target_domain, jit_tls, ji, ctx, new_ctx, lmf,
                          save_locations, frame);
  if (!err)
    return FALSE;

  gboolean not_i2m = frame->type != FRAME_TYPE_INTERP_TO_MANAGED &&
                     frame->type != FRAME_TYPE_INTERP_TO_MANAGED_WITH_CTX;

  if (not_i2m && *lmf && ((*lmf) != jit_tls->first_lmf) &&
      ((gpointer)MONO_CONTEXT_GET_SP(new_ctx) >= (gpointer)(*lmf))) {
    /*
     * Remove any unused lmf.
     * Mask out the lower bits which might be used to hold additional
  ...
  if (frame->type == FRAME_TYPE_MANAGED_TO_NATIVE) {
    /*
     * This type of frame is just a marker, the caller should unwind once more
     * to get the last managed frame.
     */
    frame->ji = NULL;
    frame->method = NULL;
  }
...
static void unwinder_init(Unwinder *unwinder) {
...
static gboolean
#endif
unwinder_unwind_frame(Unwinder *unwinder, MonoDomain *domain,
                      MonoJitTlsData *jit_tls, MonoJitInfo *prev_ji,
                      MonoContext *ctx, MonoContext *new_ctx, char **trace,
                      MonoLMF **lmf, host_mgreg_t **save_locations,
                      StackFrameInfo *frame) {
  if (unwinder->in_interp) {
    ...
    /* The DEBUGGER_INVOKE should be returned before the first interpreter frame
     * for the invoke */
    ...
    unwinder->in_interp = mini_get_interp_callbacks()->frame_iter_next(
        &unwinder->interp_iter, frame);
    if (frame->type == FRAME_TYPE_INTERP) {
      const gpointer parent =
          mini_get_interp_callbacks()->frame_get_parent(frame->interp_frame);
      unwinder->last_frame_addr = parent;
    }

    if (!unwinder->in_interp)
      return unwinder_unwind_frame(unwinder, domain, jit_tls, prev_ji, ctx,
                                   new_ctx, trace, lmf, save_locations, frame);
    return TRUE;
  } else {
    gboolean res =
        mono_find_jit_info_ext(domain, jit_tls, prev_ji, ctx, new_ctx, trace,
                               lmf, save_locations, frame);
    if (!res)
      return FALSE;
    if (frame->type == FRAME_TYPE_INTERP_TO_MANAGED ||
        frame->type == FRAME_TYPE_INTERP_TO_MANAGED_WITH_CTX) {
      unwinder->in_interp = TRUE;
      mini_get_interp_callbacks()->frame_iter_init(&unwinder->interp_iter,
                                                   frame->interp_exit_data);
    }
 ...
 */
static gpointer get_generic_info_from_stack_frame(MonoJitInfo *ji,
                                                  MonoContext *ctx) {
   ...
   * argument for every pc offset, even if the method was interrupted while it
   * was in its prolog.
   */
  if (gi->nlocs) {
    ...
    for (i = 0; i < gi->nlocs; ++i) {
      ...
      if (offset >= entry->from && (offset < entry->to || entry->to == 0)) {
        if (entry->is_reg)
          ...
          info = *(gpointer *)(gpointer)(
              (char *)mono_arch_context_get_int_reg(ctx, entry->reg) +
              entry->offset);
        break;
      }
    }
    ...
    if (gi->this_in_reg)
      ...
      info = *(gpointer *)(gpointer)(
          (char *)mono_arch_context_get_int_reg(ctx, gi->this_reg) +
          gi->this_offset);
  }
 ...
 */
MonoGenericContext
mono_get_generic_context_from_stack_frame(MonoJitInfo *ji,
                                          gpointer generic_info) {
  ...
  if (mono_method_get_context(method)->method_inst) {
    MonoMethodRuntimeGenericContext *mrgctx =
        (MonoMethodRuntimeGenericContext *)generic_info;
  ...
  if (mono_class_is_ginst(klass))
    g_assert(mono_class_has_parent_and_ignore_generics(
        mono_class_get_generic_class(klass)->container_class,
        method_container_class));
  else
    g_assert(mono_class_has_parent_and_ignore_generics(klass,
                                                       method_container_class));

  return context;
}

static MonoMethod *get_method_from_stack_frame(MonoJitInfo *ji,
                                               gpointer generic_info) {
  ERROR_DECL(error);
  ...
  if (!ji->has_generic_jit_info ||
      !mono_jit_info_get_generic_jit_info(ji)->has_this)
...
gboolean mono_exception_walk_trace(MonoException *ex,
                                   MonoExceptionFrameWalk func,
                                   gpointer user_data) {
  gboolean res;
...
static gboolean
mono_exception_stackframe_obj_walk(MonoStackFrame *captured_frame,
                                   MonoExceptionFrameWalk func,
                                   gpointer user_data) {
  if (!captured_frame)
    return TRUE;

  gpointer ip = (gpointer)(captured_frame->method_address +
                           captured_frame->native_offset);
  MonoJitInfo *ji =
      mono_jit_info_table_find_internal(mono_domain_get(), ip, TRUE, TRUE);
  ...
  gboolean r = func(method, (gpointer)captured_frame->method_address,
                    captured_frame->native_offset, TRUE, user_data);
  if (r)
    return TRUE;
...
static gboolean mono_exception_stacktrace_obj_walk(MonoStackTrace *st,
                                                   MonoExceptionFrameWalk func,
                                                   gpointer user_data) {
  int num_captured =
      st->captured_traces ? mono_array_length_internal(st->captured_traces) : 0;
  for (int i = 0; i < num_captured; i++) {
    MonoStackTrace *curr_trace =
        mono_array_get_fast(st->captured_traces, MonoStackTrace *, i);
  ...
  for (int frame = 0; frame < num_frames; frame++) {
    gboolean r = mono_exception_stackframe_obj_walk(
        mono_array_get_fast(st->frames, MonoStackFrame *, frame), func,
        user_data);
    if (r)
      return TRUE;
  }
...
gboolean mono_exception_walk_trace_internal(MonoException *ex,
                                            MonoExceptionFrameWalk func,
                                            gpointer user_data) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    memcpy(&trace_ip, mono_array_addr_fast(ta, ExceptionTraceIp, i),
    ...
    if (ji == NULL) {
      ...
      if (func(method, ji->code_start, (char *)ip - (char *)ji->code_start,
               TRUE, user_data))
        break;
    }
  }
  ...
  for (int i = 0; i < len; i++) {
    MonoStackTrace *captured_trace =
        mono_array_get_fast(ta, MonoStackTrace *, i);
    if (!captured_trace)
      break;
...
MonoArray *ves_icall_get_trace(MonoException *exc, gint32 skip,
  ...
  if (ta == NULL) {
    /* Exception is not thrown yet */
    res = mono_array_new_checked(domain, mono_defaults.stack_frame_class, 0,
                                 error);
  ...
  res = mono_array_new_checked(domain, mono_defaults.stack_frame_class,
                               len > skip ? len - skip : 0, error);
  ...
  for (i = skip; i < len; i++) {
    ...
    if (jinfo_get_method(ji)->wrapper_type) {
      ...
      sf->method = NULL;
      s = mono_method_get_name_full(method, TRUE, FALSE,
    ...
    sf->method_index =
        ji->from_aot ? mono_aot_find_method_index(method) : 0xffffff;
    sf->method_address = (gsize)ji->code_start;
    sf->native_offset = (char *)ip - (char *)ji->code_start;

    /*
     * mono_debug_lookup_source_location() returns both the file / line number
     * information and the IL offset.  Note that computing the IL offset is
     * already an expensive operation, so we shouldn't call this method twice.
     */
    location = mono_debug_lookup_source_location(jinfo_get_method(ji),
                                                 sf->native_offset, domain);
    if (location) {
      sf->il_offset = location->il_offset;
    } else {
      SeqPoint sp;
      if (mono_find_prev_seq_point_for_native_offset(
              domain, jinfo_get_method(ji), sf->native_offset, NULL, &sp))
 ...
 */
void mono_walk_stack_with_ctx(MonoJitStackWalk func, MonoContext *start_ctx,
                              MonoUnwindOptions unwind_options,
                              void *user_data) {
  ...
  mono_walk_stack_full(func, start_ctx, mono_domain_get(), thread->jit_data,
                       mono_get_lmf(), unwind_options, user_data, FALSE);
}
 ...
 *
 * If you are using this function to unwind another thread, make sure it is
 * suspended.
 *
 * If \p state is null, we capture the current context.
 */
void mono_walk_stack_with_state(MonoJitStackWalk func,
  ...
  mono_walk_stack_full(
      ...
      (MonoLMF *)state->unwind_data[MONO_UNWIND_DATA_LMF], unwind_options,
      user_data, FALSE);
}

void mono_walk_stack(MonoJitStackWalk func, MonoUnwindOptions options,
                     void *user_data) {
  MonoThreadUnwindState state;
 ...
 * \param thread the thread whose stack to walk, can be NULL to use the current
 * thread \param lmf the LMF of \p thread, can be NULL to use the LMF of the
 * current thread \param user_data data passed to the callback \param
 * crash_context tells us that we're in a context where it's not safe to lock or
 * allocate This function walks the stack of a thread, starting from the state
...
static void mono_walk_stack_full(MonoJitStackWalk func, MonoContext *start_ctx,
    ...
    for (l = ips; l; l = l->next) {
      ...
      if (frame.type == FRAME_TYPE_MANAGED) {
        if (!frame.method->wrapper_type ||
            frame.method->wrapper_type == MONO_WRAPPER_DYNAMIC_METHOD)
          frame.managed = TRUE;
      }
  ...
  if (async && (unwind_options & MONO_UNWIND_LOOKUP_ACTUAL_METHOD)) {
    g_warning("async && (unwind_options & MONO_UNWIND_LOOKUP_ACTUAL_METHOD) "
              "not legal");
    return;
  }
  ...
  while (MONO_CONTEXT_GET_SP(&ctx) < jit_tls->end_of_stack) {
    frame.lmf = lmf;
    res = unwinder_unwind_frame(
        &unwinder, domain, jit_tls, NULL, &ctx, &new_ctx, NULL, &lmf,
        get_reg_locations ? new_reg_locations : NULL, &frame);
    if (!res)
      return;
    ...
    if ((unwind_options & MONO_UNWIND_LOOKUP_IL_OFFSET) && frame.ji) {
      ...
      if (!crash_context)
        source = mono_debug_lookup_source_location(jinfo_get_method(frame.ji),
                                                   frame.native_offset, domain);
      if (source) {
        ...
        SeqPoint sp;
        if (seq_points && mono_seq_point_find_prev_by_native_offset(
                              seq_points, frame.native_offset, &sp))
    ...
    if ((unwind_options & MONO_UNWIND_LOOKUP_ACTUAL_METHOD) && frame.ji) {
      frame.actual_method = get_method_from_stack_frame(
          frame.ji, get_generic_info_from_stack_frame(frame.ji, &ctx));
    } else {
      frame.actual_method = frame.method;
    }
...
static void mono_summarize_managed_stack(MonoThreadSummary *out) { return; }

static void mono_summarize_unmanaged_stack(MonoThreadSummary *out) { return; }
...
static void mono_crash_reporting_allow_all_native_libraries() { return; }
...
static void copy_summary_string_safe(char *dest, const char *src) {
  g_strlcpy(dest, src, MONO_MAX_SUMMARY_NAME_LEN);
}

static void fill_frame_managed_info(MonoFrameSummary *frame,
                                    MonoMethod *method) {
...
static void mono_crash_reporting_allow_all_native_libraries() {
  allow_all_native_libraries = TRUE;
}

static gboolean check_whitelisted_module(const char *in_name,
  ...
  for (GList *cursor = native_library_whitelist; cursor;
       cursor = cursor->next) {
...
static intptr_t mono_make_portable_ip(intptr_t in_ip, intptr_t module_base) {
  ...
  //
  // Quirk shared by:
  // /usr/bin/symbols  -- symbols version:			@(#)PROGRAM:symbols
  // PROJECT:SamplingTools-63501 *CoreSymbolicationDT.framework version:
  // 63750*/
...
static gboolean mono_get_portable_ip(intptr_t in_ip, intptr_t *out_ip,
                                     gint32 *out_offset,
                                     const char **out_module, char *out_name) {
  // Note: it's not safe for us to be interrupted while inside of dl_addr,
  // because if we try to call dl_addr while interrupted while inside the lock,
  // we will try to take a non-recursive lock twice on this thread, and will
  // deadlock.
  char sname[256], fname[256];
  void *saddr = NULL, *fbase = NULL;
  gboolean success =
      g_module_address((void *)in_ip, fname, 256, &fbase, sname, 256, &saddr);
  if (!success)
    return FALSE;
...
static guint64 summarize_offset_free_hash(guint64 accum,
                                          MonoFrameSummary *frame) {
  if (!frame->is_managed)
    return accum;
...
static guint64 summarize_offset_rich_hash(guint64 accum,
                                          MonoFrameSummary *frame) {
  // See: mono_ptrarray_hash
  guint64 hash_accum = accum;
...
static gboolean summarize_frame_internal(MonoMethod *method, gpointer ip,
                                         size_t native_offset, int il_offset,
                                         gboolean managed, gpointer user_data) {
  ...
  if (!managed && method && method->wrapper_type != MONO_WRAPPER_NONE &&
      method->wrapper_type < MONO_WRAPPER_NUM) {
    dest->is_managed = FALSE;
    dest->unmanaged_data.has_name = TRUE;
    copy_summary_string_safe(dest->str_descr,
                             mono_wrapper_type_to_str(method->wrapper_type));
  }
  ...
  ud->hashes->offset_free_hash =
      summarize_offset_free_hash(ud->hashes->offset_free_hash, dest);
  ud->hashes->offset_rich_hash =
      summarize_offset_rich_hash(ud->hashes->offset_rich_hash, dest);
...
static gboolean summarize_frame_managed_walk(MonoMethod *method, gpointer ip,
  ...
  if (managed && method) {
    MonoDebugSourceLocation *location = mono_debug_lookup_source_location(
        method, frame_native_offset, mono_domain_get());
    if (location) {
  ...
  return summarize_frame_internal(method, (gpointer)portable_ip,
                                  frame_native_offset, il_offset, managed,
                                  user_data);
}

static gboolean summarize_frame(StackFrameInfo *frame, MonoContext *ctx,
                                gpointer data) {
  ...
  intptr_t ip = 0;
  gint32 offset = 0;
  mono_get_portable_ip((intptr_t)MONO_CONTEXT_GET_IP(ctx), &ip, &offset, NULL,
                       NULL);
  // Don't need to handle return status "success" because this ip is stored
  // below only, NULL is okay

  gboolean is_managed =
      (frame->type == FRAME_TYPE_MANAGED || frame->type == FRAME_TYPE_INTERP);
  MonoMethod *method = NULL;
  ...
  return summarize_frame_internal(method, (gpointer)ip, offset,
                                  frame->il_offset, is_managed, data);
}
...
static void mono_summarize_exception(MonoException *exc,
  ...
  for (exc_index = 0; exc_index < MONO_MAX_SUMMARY_EXCEPTIONS; exc_index++) {
    ...
    out->exceptions[exc_index].managed_exc_type =
        inner_exc->object.vtable->klass;
...
static void mono_summarize_managed_stack(MonoThreadSummary *out) {
  ...
  //
  mono_walk_stack_full(summarize_frame, out->ctx, out->domain, out->jit_tls,
                       out->lmf, MONO_UNWIND_LOOKUP_IL_OFFSET, &data, TRUE);
...
static void mono_summarize_unmanaged_stack(MonoThreadSummary *out) {
  ...
  out->num_unmanaged_frames =
      backtrace((void **)frame_ips, MONO_MAX_SUMMARY_FRAMES);
  ...
  for (int i = 0; i < out->num_unmanaged_frames; ++i) {
    ...
    int success = mono_get_portable_ip(ip, &frame->unmanaged_data.ip,
                                       &frame->unmanaged_data.offset,
                                       &module_buf, (char *)frame->str_descr);
    ...
    out->hashes.offset_free_hash =
        summarize_offset_free_hash(out->hashes.offset_free_hash, frame);
    out->hashes.offset_rich_hash =
        summarize_offset_rich_hash(out->hashes.offset_rich_hash, frame);
  }
#endif
...
MonoBoolean ves_icall_get_frame_info(gint32 skip, MonoBoolean need_file_info,
  ...
  if (mono_llvm_only) {
    ...
    /* FIXME: Generalize this code with an interface which returns an array of
     * StackFrame structures */
    jmethod = NULL;
    ips = get_unwind_backtrace();
    for (l = ips; l && skip >= 0; l = l->next) {
      ...
      /* The skip count passed by the caller depends on us not filtering out
       * MANAGED_TO_NATIVE */
      jmethod = jinfo_get_method(ji);
      if (jmethod->wrapper_type != MONO_WRAPPER_NONE &&
          jmethod->wrapper_type != MONO_WRAPPER_DYNAMIC_METHOD &&
          jmethod->wrapper_type != MONO_WRAPPER_MANAGED_TO_NATIVE)
        continue;
      skip--;
    }
    g_slist_free(ips);
    ...
    *native_offset = frame_ip - (guint8 *)ji->code_start;
    ...
    do {
      ctx = new_ctx;
      res = unwinder_unwind_frame(&unwinder, domain, jit_tls, NULL, &ctx,
                                  &new_ctx, NULL, &lmf, NULL, &frame);
      if (!res)
        return FALSE;
      switch (frame.type) {
      case FRAME_TYPE_MANAGED_TO_NATIVE:
    ...
    if (frame.type == FRAME_TYPE_INTERP) {
      ...
      actual_method = get_method_from_stack_frame(
          ji, get_generic_info_from_stack_frame(ji, &ctx));
    }
  }

  MonoReflectionMethod *rm =
      mono_method_get_object_checked(domain, actual_method, NULL, error);
  if (!is_ok(error)) {
    mono_error_set_pending_exception(error);
    return FALSE;
  }
  ...
  if (il_offset != -1) {
    location =
        mono_debug_lookup_source_location_by_il(jmethod, il_offset, domain);
  } else {
    location =
        mono_debug_lookup_source_location(jmethod, *native_offset, domain);
  }
  if (location)
...
static MonoClass *get_exception_catch_class(MonoJitExceptionInfo *ei,
                                            MonoJitInfo *ji, MonoContext *ctx) {
  ERROR_DECL(error);
  ...
  /*MonoJitExceptionInfo::data is an union used by filter and finally clauses
   * too.*/
  ...
  if (!ji->has_generic_jit_info ||
      !mono_jit_info_get_generic_jit_info(ji)->has_this)
    return catch_class;
  context = mono_get_generic_context_from_stack_frame(
      ji, get_generic_info_from_stack_frame(ji, ctx));
  ...
  inflated_type = mono_class_inflate_generic_type_checked(
      m_class_get_byval_arg(catch_class), &context, error);
 ...
 *
 *   Same as mono_jit_info_table_find, but search all the domains of the current
 * thread if ADDR is not found in DOMAIN. The domain where the method was found
 * is stored into OUT_DOMAIN if it is not NULL.
 */
MonoJitInfo *mini_jit_info_table_find_ext(MonoDomain *domain, gpointer addr,
                                          gboolean allow_trampolines,
                                          MonoDomain **out_domain) {
  MonoJitInfo *ji;
  ...
  if (domain != mono_get_root_domain()) {
    ji = mono_jit_info_table_find_internal(mono_get_root_domain(), addr, TRUE,
                                           allow_trampolines);
    if (ji) {
      if (out_domain)
  ...
  refs =
      (gpointer *)((t->appdomain_refs) ? *(gpointer *)t->appdomain_refs : NULL);
  for (; refs && *refs; refs++) {
    if (*refs != domain && *refs != mono_get_root_domain()) {
      ji = mono_jit_info_table_find_internal((MonoDomain *)*refs, addr, TRUE,
                                             allow_trampolines);
      if (ji) {
        if (out_domain)
...
MonoJitInfo *mini_jit_info_table_find(MonoDomain *domain, gpointer addr,
                                      MonoDomain **out_domain) {
  return mini_jit_info_table_find_ext(domain, addr, FALSE, out_domain);
}

/* Class lazy loading functions */
static GENERATE_GET_CLASS_WITH_CACHE(runtime_compat_attr,
     ...
     *
     *   Determine whenever M's assembly has a RuntimeCompatibilityAttribute
     * with the WrapNonExceptionThrows flag set.
     */
    static gboolean wrap_non_exception_throws(MonoMethod *m) {
  ERROR_DECL(error);
...
static MonoArray *build_native_trace(MonoError *error) {
  error_init(error);
  ...
  if (!size)
    return NULL;
  res = mono_array_new_checked(mono_domain_get(), mono_defaults.int_class, size,
                               error);
...
static void remove_wrappers_from_trace(GList **trace_ips_p) {
...
static void setup_stack_trace(MonoException *mono_ex, GSList **dynamic_methods,
                              GList *trace_ips, gboolean remove_wrappers) {
  if (mono_ex) {
    ...
    ERROR_DECL(error);
    MonoArray *ips_arr =
        mono_glist_to_array(trace_ips_copy, mono_defaults.int_class, error);
    mono_error_assert_ok(error);
    MONO_OBJECT_SETREF_INTERNAL(mono_ex, trace_ips, ips_arr);
    MONO_OBJECT_SETREF_INTERNAL(mono_ex, native_trace_ips,
                                build_native_trace(error));
    mono_error_assert_ok(error);
    if (*dynamic_methods) {
      /* These methods could go away anytime, so save a reference to them in the
       * exception object */
      GSList *l;
      ...
      for (l = *dynamic_methods; l; l = l->next) {
        ...
        if (domain->method_to_dyn_method) {
          ...
          dis_link = (MonoGCHandle)g_hash_table_lookup(
              domain->method_to_dyn_method, l->data);
 ...
 */
static MonoFirstPassResult handle_exception_first_pass(
    MonoContext *ctx, MonoObject *obj, gint32 *out_filter_idx,
    MonoJitInfo **out_ji, MonoJitInfo **out_prev_ji, MonoObject *non_exception,
    StackFrameInfo *catch_frame, gboolean *last_mono_wrapper_runtime_invoke) {
  ERROR_DECL(error);
  MonoDomain *domain = mono_domain_get();
  MonoJitInfo *ji = NULL;
  ...
  *last_mono_wrapper_runtime_invoke = TRUE;
  ...
  if (initial_trace_ips) {
    int len =
        mono_array_length_internal(initial_trace_ips) / TRACE_IP_ENTRY_SIZE;
    ...
    for (i = 0; i < len; i++) {
      for (int j = 0; j < TRACE_IP_ENTRY_SIZE; ++j) {
        gpointer p = mono_array_get_internal(initial_trace_ips, gpointer,
                                             (i * TRACE_IP_ENTRY_SIZE) + j);
  ...
  while (1) {
    ...
    unwind_res = unwinder_unwind_frame(&unwinder, domain, jit_tls, NULL, ctx,
                                       &new_ctx, NULL, &lmf, NULL, &frame);
    if (!unwind_res) {
    ...
    if (mini_debug_options.reverse_pinvoke_exceptions &&
        method->wrapper_type == MONO_WRAPPER_NATIVE_TO_MANAGED) {
      g_error("A native frame was found while unwinding the stack after an "
              "exception.\n"
    ...
    if (stack_overflow) {
      free_stack = (guint8 *)(MONO_CONTEXT_GET_SP(ctx)) -
                   (guint8 *)(MONO_CONTEXT_GET_SP(&initial_ctx));
    } else {
      free_stack = 0xffffff;
    }
    ...
    for (i = clause_index_start; i < ji->num_clauses; i++) {
      ...
      if (is_address_protected(ji, ei, ip)) {
        ...
        if (ei->flags == MONO_EXCEPTION_CLAUSE_FILTER) {
          ...
          if (!ji->is_interp) {
            ...
            if (ji->from_llvm)
              ...
              /* Can't pass the ex object in a register yet to filter clauses,
               * because call_filter () might not support it */
              *((gpointer *)(gpointer)((char *)MONO_CONTEXT_GET_BP(ctx) +
                                       ei->exvar_offset)) = ex_obj;
#else
          ...
          mini_get_dbg_callbacks()->begin_exception_filter(mono_ex, ctx,
                                                           &initial_ctx);
          ...
          if (G_UNLIKELY(mono_profiler_clauses_enabled())) {
            ...
            MONO_PROFILER_RAISE(
                exception_clause,
                (method, i, (MonoExceptionEnum)ei->flags, ex_obj));
            jit_tls->orig_ex_ctx_set = FALSE;
          }

          if (ji->is_interp) {
            /* The filter ends where the exception handler starts */
            filtered = mini_get_interp_callbacks()->run_filter(
                &frame, (MonoException *)ex_obj, i, ei->data.filter,
                ei->handler_start);
          } else {
            filtered = call_filter(ctx, ei->data.filter);
          }
          mini_get_dbg_callbacks()->end_exception_filter(mono_ex, ctx,
                                                         &initial_ctx);
          ...
          if (filtered) {
            ...
            frame.native_offset =
                (char *)ei->handler_start - (char *)ji->code_start;
            *catch_frame = frame;
        ...
        ERROR_DECL(
            isinst_error); // FIXME not used
                           // https://github.com/mono/mono/pull/3055/files#r240548187
        if (ei->flags == MONO_EXCEPTION_CLAUSE_NONE &&
          ...
          *catch_frame = frame;
          ...
          if (method->wrapper_type == MONO_WRAPPER_RUNTIME_INVOKE) {
            ...
            unwind_res = unwinder_unwind_frame(&unwinder, domain, jit_tls, NULL,
                                               &new_ctx, &new_ctx, NULL, &lmf,
                                               NULL, &frame);
            while (unwind_res) {
              if (frame.ji && !frame.ji->is_trampoline &&
                  jinfo_get_method(frame.ji)->wrapper_type ==
                      MONO_WRAPPER_RUNTIME_INVOKE) {
                *last_mono_wrapper_runtime_invoke = FALSE;
                break;
              }
              unwind_res = unwinder_unwind_frame(&unwinder, domain, jit_tls,
                                                 NULL, &new_ctx, &new_ctx, NULL,
                                                 &lmf, NULL, &frame);
            }
          }
          return result;
 ...
 */
static void interp_exit_finally_abort_blocks(MonoJitInfo *ji, int start_clause,
                                             int end_clause, gpointer ip) {
  int i;
  for (i = start_clause; i < end_clause; i++) {
    MonoJitExceptionInfo *ei = &ji->clauses[i];
    if (ei->flags == MONO_EXCEPTION_CLAUSE_FINALLY && ip >= ei->handler_start &&
...
mono_get_exception_runtime_wrapped_checked(MonoObject *wrapped_exception_raw,
                                           MonoError *error) {
  HANDLE_FUNCTION_ENTER();
  MONO_HANDLE_DCL(MonoObject, wrapped_exception);
  MonoExceptionHandle ret =
      mono_get_exception_runtime_wrapped_handle(wrapped_exception, error);
 ...
 * \param resume whenever to resume unwinding based on the state in \c
 * MonoJitTlsData.
 */
static gboolean mono_handle_exception_internal(MonoContext *ctx,
                                               MonoObject *obj, gboolean resume,
                                               MonoJitInfo **out_ji) {
  ERROR_DECL(error);
  MonoDomain *domain = mono_domain_get();
  ...
  if (!obj) {
    ...
    MonoString *msg = mono_string_new_checked(
        domain, "Object reference not set to an instance of an object", error);
    mono_error_assert_ok(error);
  ...
  if (obj == (MonoObject *)domain->stack_overflow_ex) {
    /*
     * It is not a good idea to try and put even more pressure on the little
     * stack available. obj = mono_get_exception_stack_overflow ();
     */
    stack_overflow = TRUE;
  ...
  if (mono_ex && jit_tls->class_cast_from) {
    if (!strcmp(m_class_get_name(mono_ex->object.vtable->klass),
      ...
      char *msg =
          g_strdup_printf("Unable to cast object of type '%s' to type '%s'.",
                          from_name, to_name);
    ...
    if (!strcmp(m_class_get_name(mono_ex->object.vtable->klass),
      ...
      char *from_name = mono_type_get_full_name(jit_tls->class_cast_from);
      char *to_name = mono_type_get_full_name(jit_tls->class_cast_to);
      char *msg = g_strdup_printf("Source array of type '%s' cannot be cast to "
                                  "destination array type '%s'.",
                                  from_name, to_name);
      mono_ex->message = mono_string_new_checked(domain, msg, error);
      g_free(from_name);
      g_free(to_name);
  ...
  /*
   * We set orig_ex_ctx_set to TRUE/FALSE around profiler calls to make sure it
   * doesn't end up being TRUE on any code path.
   */
  ...
  if (!resume) {
    ...
    if (mono_trace_is_enabled()) {
      ERROR_DECL(error);
      MonoMethod *system_exception_get_message =
          mono_class_get_method_from_name_checked(mono_defaults.exception_class,
                                                  "get_Message", 0, 0, error);
      mono_error_cleanup(error);
      error_init(error);
      MonoMethod *get_message = system_exception_get_message == NULL
                                    ? NULL
      ...
      if (get_message == NULL) {
        message = NULL;
      } else if (!strcmp(type_name, "OutOfMemoryException") ||
                 !strcmp(type_name, "StackOverflowException")) {
        message = NULL;
      ...
      if (msg == NULL) {
        if (message) {
          msg = mono_string_to_utf8_checked_internal((MonoString *)message,
                                                     error);
          if (!is_ok(error)) {
            mono_error_cleanup(error);
            msg = g_strdup(
      ...
      g_print("[%p:] EXCEPTION handling: %s.%s: %s\n",
              ...
              m_class_get_name(mono_object_class(obj)), msg);
      g_free(msg);
    ...
    res = handle_exception_first_pass(&ctx_cp, obj, &first_filter_idx, &ji,
                                      &prev_ji, non_exception, &catch_frame,
                                      &last_mono_wrapper_runtime_invoke);

    if (res == MONO_FIRST_PASS_UNHANDLED) {
      if (mono_aot_mode == MONO_AOT_MODE_LLVMONLY_INTERP) {
        /* Reached the top interpreted frames, but there might be native frames
         * above us */
      ...
      if (mini_debug_options.break_on_exc)
        G_BREAKPOINT();
      mini_get_dbg_callbacks()->handle_exception((MonoException *)obj, ctx,
                                                 NULL, NULL);

      // FIXME: This runs managed code so it might cause another stack overflow
      // when we are handling a stack overflow
      ...
      if (ji &&
        ...
        if (prev_ji &&
            jinfo_get_method(prev_ji) ==
                mono_defaults.threadpool_perform_wait_callback_method)
          unhandled = TRUE;
      }
#endif
      ...
      if (unhandled)
        mini_get_dbg_callbacks()->handle_exception((MonoException *)obj, ctx,
                                                   NULL, NULL);
      else if (!ji || (jinfo_get_method(ji)->wrapper_type ==
                       MONO_WRAPPER_RUNTIME_INVOKE)) {
        if (last_mono_wrapper_runtime_invoke &&
  ...
  while (1) {
    ...
    for (i = clause_index_start; i < ji->num_clauses; i++) {
      ...
      if (is_address_protected(ji, ei, ip)) {
        ...
        if ((ei->flags == MONO_EXCEPTION_CLAUSE_NONE &&
            ...
            filtered) {
          /*
           * This guards against the situation that we abort a thread that is
           * executing a finally clause that was called by the EH machinery. It
           * won't have a guard trampoline installed, so we must check for this
           * situation here and resume interruption if we are below the guarded
           * block.
           */
          if (G_UNLIKELY(jit_tls->handler_block)) {
            gboolean is_outside = FALSE;
            gpointer prot_bp =
                MONO_CONTEXT_GET_BP(&jit_tls->handler_block_context);
            ...
            if (catch_bp > prot_bp) {
              ...
              /* Can be either try { try { } catch {} } finally {} or try { try
               *{ } finally {} } catch {} So we check if the catch handler_start
               *is protected by the guarded handler protected region
               *
               * Assumptions:
               *	If there is an outstanding guarded_block return address,
               *it means the current thread must be aborted. This is the only
               *way to reach out the guarded block as other cases are handled by
               *the trampoline. There aren't any further finally/fault handler
               *blocks down the stack over this exception. This must be ensured
               *by the code that installs the guard trampoline.
               */
              g_assert(ji == mini_jit_info_table_find(
                                 domain,
                                 (char *)MONO_CONTEXT_GET_IP(
                                     &jit_tls->handler_block_context),
                                 NULL));

              if (!is_address_protected(ji, jit_tls->handler_block,
                                        ei->handler_start)) {
                is_outside = TRUE;
              }
            }
            if (is_outside) {
              ...
              mono_thread_resume_interruption(
                  TRUE); /*We ignore the exception here, it will be raised
                            later*/
            }
          }
          ...
          /*
           * At this point, ei->flags can be either MONO_EXCEPTION_CLAUSE_NONE
           * for a a try-catch clause or MONO_EXCEPTION_CLAUSE_FILTER for a
           * try-filter-catch clause. Since we specifically want to indicate
           * that we're executing the catch portion of this EH clause, pass
           * MONO_EXCEPTION_CLAUSE_NONE explicitly instead of ei->flags.
           */
          if (G_UNLIKELY(mono_profiler_clauses_enabled())) {
            jit_tls->orig_ex_ctx_set = TRUE;
            MONO_PROFILER_RAISE(
                exception_clause,
                (method, i, MONO_EXCEPTION_CLAUSE_NONE, ex_obj));
            jit_tls->orig_ex_ctx_set = FALSE;
          }
          ...
          if (in_interp) {
            ...
            /*
             * ctx->pc points into the interpreter, after the call which
             * transitioned to JITted code. Store the unwind state into the
             * interpeter state, then resume, the interpreter will unwind itself
             * until it reaches the target frame and will continue execution
             * from there. The resuming is kinda hackish, from the native code
             * standpoint, it looks like the call which transitioned to JITted
             * code has succeeded, but the return value register etc. is not
             * set, so we have to be careful.
             */
            mini_get_interp_callbacks()->set_resume_state(
                jit_tls, ex_obj, ei, frame.interp_frame, ei->handler_start);
          ...
          mono_set_lmf(lmf);
#ifndef DISABLE_PERFCOUNTERS
          mono_atomic_fetch_add_i32(&mono_perfcounters->exceptions_depth,
                                    frame_count);
#endif
          if (obj == (MonoObject *)domain->stack_overflow_ex)
        ...
        if (ei->flags == MONO_EXCEPTION_CLAUSE_FAULT ||
          ...
          if (ji->from_llvm) {
            ...
            mini_set_abort_threshold(&frame);
            if (in_interp) {
              gboolean has_ex = mini_get_interp_callbacks()->run_finally(
                  &frame, i, ei->handler_start, ei->data.handler_end);
              if (has_ex) {
                /*
                 * If run_finally didn't resume to a context, it means that the
                 * handler frame is linked to the frame calling finally through
                 * interpreter frames. This means that we will reach the handler
                 * frame by resuming the current context.
                 */
                if (MONO_CONTEXT_GET_IP(ctx) != 0)
                  mono_arch_undo_ip_adjustment(ctx);
                return 0;
    ...
    if (in_interp)
      interp_exit_finally_abort_blocks(ji, clause_index_start, ji->num_clauses,
                                       ip);

    if (MONO_PROFILER_ENABLED(method_exception_leave) &&
        mono_profiler_get_call_instrumentation_flags(method) &
 ...
 * This method is called by the Mono Debugger to call all \c finally clauses of
 * the current stack frame.  It's used when the user issues a \c return command
 * to make the current stack frame return.  After returning from this method,
 * the debugger unwinds the stack one frame and gives control back to the user.
...
void mono_debugger_run_finally(MonoContext *start_ctx) {
  ...
  ji = mono_find_jit_info(domain, jit_tls, &rji, NULL, &ctx, &new_ctx, NULL,
                          &lmf, NULL, NULL);
  if (!ji || ji == (gpointer)-1)
    return;
 ...
 * \param obj the exception object
 *
 *   Handle the exception OBJ starting from the state CTX. Modify CTX to point
 * to the handler clause if the exception is caught, and return TRUE.
 */
gboolean mono_handle_exception(MonoContext *ctx, gpointer void_obj) {
...
void mono_setup_altstack(MonoJitTlsData *tls) {
  ...
  /*
   * On macOS Mojave we are encountering a bug when changing mapping for main
   * thread stack pages. Stack overflow on main thread will kill the app.
   *
   * AIX seems problematic as well; it gives ENOMEM for mprotect and valloc, if
   * we do this for thread 1 with its stack at the top of memory. Other threads
   * seem fine for the altstack guard page, though.
   */
  ...
  if (!disable_stack_guard) {
    ...
    tls->stack_ovf_guard_size =
        ALIGN_TO(MONO_STACK_OVERFLOW_GUARD_SIZE, mono_pagesize());

    g_assert((guint8 *)&sa >=
             (guint8 *)tls->stack_ovf_guard_base + tls->stack_ovf_guard_size);

    if (mono_mprotect(tls->stack_ovf_guard_base, tls->stack_ovf_guard_size,
                      MONO_MMAP_NONE)) {
      /* mprotect can fail for the main thread stack */
      gpointer gaddr = mono_valloc(
          tls->stack_ovf_guard_base, tls->stack_ovf_guard_size,
  ...
  tls->signal_stack = mono_valloc(0, MONO_ARCH_SIGNAL_STACK_SIZE,
                                  MONO_MMAP_READ | MONO_MMAP_WRITE |
                                      MONO_MMAP_PRIVATE | MONO_MMAP_ANON,
                                  MONO_MEM_ACCOUNT_EXCEPTIONS);
  ...
  if (tls->stack_ovf_guard_base)
    mono_gc_register_altstack(
        (char *)tls->stack_ovf_guard_base + tls->stack_ovf_guard_size,
        (char *)staddr + stsize -
            ((char *)tls->stack_ovf_guard_base + tls->stack_ovf_guard_size),
        tls->signal_stack, tls->signal_stack_size);
  else
    mono_gc_register_altstack(staddr, stsize, tls->signal_stack,
                              tls->signal_stack_size);
}
...
void mono_free_altstack(MonoJitTlsData *tls) {
  ...
  if (tls->stack_ovf_valloced)
    ...
    mono_mprotect(tls->stack_ovf_guard_base, tls->stack_ovf_guard_size,
                  MONO_MMAP_READ | MONO_MMAP_WRITE);
}

#elif G_HAVE_API_SUPPORT(HAVE_CLASSIC_WINAPI_SUPPORT) && defined(HOST_WIN32)
void mono_setup_altstack(MonoJitTlsData *tls) {
  // Alt stack is not supported on Windows, but we can use this point to at
  // least reserve a stack guarantee of available stack memory when handling
  // stack overflow.
  ULONG new_stack_guarantee = (ULONG)ALIGN_TO(MONO_STACK_OVERFLOW_GUARD_SIZE,
...
gboolean mono_handle_soft_stack_ovf(MonoJitTlsData *jit_tls, MonoJitInfo *ji,
                                    void *ctx,
                                    MONO_SIG_HANDLER_INFO_TYPE *siginfo,
                                    guint8 *fault_addr) {
  if (!jit_tls)
    return FALSE;
   ...
   * in the thread stack. The soft-guard pages will be protected again as the
   * stack is unwound. 2) unmanaged code caused the overflow: we unprotect the
   * soft-guard page and hope we can continue with those enabled, at least until
   * the hard-guard page is hit. The alternative to continuing here is to just
   * print a message and abort. We may add in the future the code to protect the
   * pages again in the codepath when we return from unmanaged to managed code.
   */
  if (jit_tls->stack_ovf_guard_size &&
      fault_addr >= (guint8 *)jit_tls->stack_ovf_guard_base &&
      fault_addr < (guint8 *)jit_tls->stack_ovf_guard_base +
                       jit_tls->stack_ovf_guard_size) {
    gboolean handled = FALSE;

    mono_mprotect(jit_tls->stack_ovf_guard_base, jit_tls->stack_ovf_guard_size,
                  MONO_MMAP_READ | MONO_MMAP_WRITE);
#ifdef MONO_ARCH_SIGSEGV_ON_ALTSTACK
    if (ji) {
...
static gboolean print_overflow_stack_frame(StackFrameInfo *frame,
                                           MonoContext *ctx, gpointer data) {
  MonoMethod *method = NULL;
  ...
  if (method) {
    ...
    location = mono_debug_print_stack_frame(method, frame->native_offset,
                                            mono_domain_get());
...
void mono_handle_hard_stack_ovf(MonoJitTlsData *jit_tls, MonoJitInfo *ji,
                                MonoContext *mctx, guint8 *fault_addr) {
  PrintOverflowUserData ud;

  /* we don't do much now, but we can warn the user with a useful message */
  mono_runtime_printf_err("Stack overflow: IP: %p, fault addr: %p",
                          MONO_CONTEXT_GET_IP(mctx), fault_addr);

  mono_runtime_printf_err("Stacktrace:");

  memset(&ud, 0, sizeof(ud));

  mono_walk_stack_with_ctx(print_overflow_stack_frame, mctx,
                           MONO_UNWIND_LOOKUP_ACTUAL_METHOD, &ud);

  _exit(1);
}

static gboolean print_stack_frame_signal_safe(StackFrameInfo *frame,
                                              MonoContext *ctx, gpointer data) {
  MonoMethod *method = NULL;
...
static G_GNUC_UNUSED gboolean print_stack_frame_to_string(StackFrameInfo *frame,
                                                          MonoContext *ctx,
                                                          gpointer data) {
  ...
  if (method && frame->domain) {
    gchar *location = mono_debug_print_stack_frame(method, frame->native_offset,
                                                   frame->domain);
 ...
 */
void mono_handle_native_crash(const char *signal, MonoContext *mctx,
  ...
  if (jit_tls && mono_thread_internal_current() && mctx) {
    ...
    mono_walk_stack_full(print_stack_frame_signal_safe, mctx, mono_domain_get(),
                         jit_tls, mono_get_lmf(), MONO_UNWIND_LOOKUP_IL_OFFSET,
                         NULL, TRUE);
    g_async_safe_printf(
        "=================================================================\n");
  }
...
static void mono_print_thread_dump_internal(void *sigctx,
                                            MonoContext *start_ctx) {
  ...
  g_string_append_printf(text, " tid=%p this=%p ", (gpointer)(gsize)thread->tid,
                         thread);
  ...
  mono_walk_stack_with_ctx(print_stack_frame_to_string, &ctx,
                           MONO_UNWIND_LOOKUP_ALL, text);
 ...
 * mono_resume_unwind:
 *
 *   This is called by a trampoline from LLVM compiled finally clauses to
 * continue unwinding.
 */
void mono_resume_unwind(MonoContext *ctx) {
  MONO_REQ_GC_UNSAFE_MODE;
  ...
  mono_handle_exception_internal(
      &new_ctx, (MonoObject *)jit_tls->resume_state.ex_obj, TRUE, NULL);
...
static gboolean find_last_handler_block(StackFrameInfo *frame, MonoContext *ctx,
                                        gpointer data) {
  int i;
  gpointer ip;
  ...
  for (i = 0; i < ji->num_clauses; ++i) {
    ...
    if (ei->flags != MONO_EXCEPTION_CLAUSE_FINALLY)
      continue;
    /*If ip points to the first instruction it means the handler block didn't
     start so we can leave its execution to the EH machinery*/
...
static void install_handler_block_guard(MonoJitInfo *ji, MonoContext *ctx) {
  int i;
  ...
  /*no matching finally - can't happen, we parallel the logic in
...
static gboolean mono_install_handler_block_guard(MonoThreadUnwindState *ctx) {
  ...
  MonoJitTlsData *jit_tls =
      (MonoJitTlsData *)ctx->unwind_data[MONO_UNWIND_DATA_JIT_TLS];

  /* Guard against a null MonoJitTlsData. This can happens if the thread
   * receives the interrupt signal before the JIT has time to initialize its TLS
   * data for the given thread.
   */
  ...
  mono_walk_stack_with_state(find_last_handler_block, ctx, MONO_UNWIND_NONE,
                             &data);
...
gboolean mono_thread_state_init_from_sigctx(MonoThreadUnwindState *ctx,
                                            void *sigctx) {
  MonoThreadInfo *thread = mono_thread_info_current_unchecked();
  if (!thread) {
    ctx->valid = FALSE;
  ...
  if (!ctx->unwind_data[MONO_UNWIND_DATA_DOMAIN] ||
      !ctx->unwind_data[MONO_UNWIND_DATA_LMF])
    return FALSE;
...
gboolean mono_thread_state_init_from_monoctx(MonoThreadUnwindState *ctx,
                                             MonoContext *mctx) {
  MonoThreadInfo *thread = mono_thread_info_current_unchecked();
  if (!thread) {
    ctx->valid = FALSE;
...
static void mono_raise_exception_with_ctx(MonoException *exc,
                                          MonoContext *ctx) {
  mono_handle_exception(ctx, (MonoObject *)exc);
  mono_restore_context(ctx);
}

/*FIXME Move all monoctx -> sigctx conversion to signal handlers once all archs
 * support utils/mono-context */
void mono_setup_async_callback(MonoContext *ctx, void (*async_cb)(void *fun),
                               gpointer user_data) {
 ...
 */
void mono_install_ftnptr_eh_callback(MonoFtnPtrEHCallback callback) {
...
static void throw_exception(MonoObject *ex, gboolean rethrow) {
  ...
  // Note: Not pinned
  jit_tls->thrown_exc =
      mono_gchandle_new_internal((MonoObject *)mono_ex, FALSE);
  ...
  if (!rethrow) {
    ...
    MonoArray *ips_arr =
        mono_glist_to_array(trace, mono_defaults.int_class, error);
    mono_error_assert_ok(error);
    MONO_OBJECT_SETREF_INTERNAL(mono_ex, trace_ips, ips_arr);
    g_list_free(l);
    g_list_free(trace);
...
void mono_llvm_throw_exception(MonoObject *ex) { throw_exception(ex, FALSE); }

void mono_llvm_rethrow_exception(MonoObject *ex) { throw_exception(ex, TRUE); }
...
void mono_llvm_throw_corlib_exception(guint32 ex_token_index) {
  ...
  ex = mono_exception_from_token(
      m_class_get_image(mono_defaults.exception_class), ex_token);
 ...
 */
void mono_llvm_resume_exception(void) { mono_llvm_cpp_throw_exception(); }
...
MonoObject *mono_llvm_load_exception(void) {
  ...
  MonoException *mono_ex =
      (MonoException *)mono_gchandle_get_target_internal(jit_tls->thrown_exc);
  ...
  if (ta) {
    ...
    MonoArray *ips_arr =
        mono_glist_to_array(trace_ips, mono_defaults.int_class, error);
    mono_error_assert_ok(error);
    MONO_OBJECT_SETREF_INTERNAL(mono_ex, trace_ips, ips_arr);
    g_list_free(trace_ips);

    // FIXME:
    // MONO_OBJECT_SETREF_INTERNAL (mono_ex, stack_trace,
    // ves_icall_System_Exception_get_trace (mono_ex));
  } else {
    MONO_OBJECT_SETREF_INTERNAL(mono_ex, trace_ips,
                                mono_array_new_checked(mono_domain_get(),
    ...
    MONO_OBJECT_SETREF_INTERNAL(
        mono_ex, stack_trace,
        mono_array_new_checked(mono_domain_get(),
                               mono_defaults.stack_frame_class, 0, error));
    mono_error_assert_ok(error);
  }
 ...
 *
 *   Return the innermost clause containing REGION_START-REGION_END which can
 * handle the current exception.
 */
gint32 mono_llvm_match_exception(MonoJitInfo *jinfo, guint32 region_start,
                                 guint32 region_end, gpointer rgctx,
                                 MonoObject *this_obj) {
  ERROR_DECL(error);
  MonoJitTlsData *jit_tls = mono_get_jit_tls();
  MonoObject *exc;
  ...
  for (int i = 0; i < jinfo->num_clauses; i++) {
    ...
    if (!(ei->try_offset == region_start &&
          ei->try_offset + ei->try_len == region_end))
      continue;

    catch_class = ei->data.catch_class;
    if (mono_class_is_open_constructed_type(
            m_class_get_byval_arg(catch_class))) {
      MonoGenericContext context;
      MonoType *inflated_type;

      g_assert(rgctx || this_obj);
      context = mono_get_generic_context_from_stack_frame(
          jinfo, rgctx ? rgctx : this_obj->vtable);
      inflated_type = mono_class_inflate_generic_type_checked(
          m_class_get_byval_arg(catch_class), &context, error);
      mono_error_assert_ok(error); /* FIXME don't swallow the error */
    ...
    if (ei->flags == MONO_EXCEPTION_CLAUSE_NONE &&
        mono_object_isinst_checked(exc, catch_class, error)) {
...
G_EXTERN_C _Unwind_Reason_Code
mono_debug_personality(int a, _Unwind_Action b, uint64_t c,
                       struct _Unwind_Exception *d, struct _Unwind_Context *e) {
  g_assert_not_reached();
}
#else
G_EXTERN_C void mono_debug_personality(void);

void mono_debug_personality(void) { g_assert_not_reached(); }
#endif
