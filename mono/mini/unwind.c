...
typedef enum { LOC_SAME, LOC_OFFSET } LocType;
...
#ifdef TARGET_AMD64
static int map_hw_reg_to_dwarf_reg[] = {0, 2,  1,  3,  7,  6,  4,  5, 8,
...
static int map_hw_reg_to_dwarf_reg[ppc_lr + 1] = {
...
static void init_hw_reg_map(void) {
...
static void init_dwarf_reg_map(void) {
...
static guint32 decode_uleb128(guint8 *buf, guint8 **endbuf) {
  guint8 *p = buf;
  guint32 res = 0;
  int shift = 0;
...
static gint32 decode_sleb128(guint8 *buf, guint8 **endbuf) {
  guint8 *p = buf;
  gint32 res = 0;
  int shift = 0;
  ...
  while (p < unwind_info + unwind_info_len) {
    ...
    case DW_CFA_offset:
      ...
      if (reg == DWARF_PC_REG)
        ...
        printf("CFA: [%x] offset: %s at cfa-0x%x\n", pos,
               mono_arch_regname(mono_dwarf_reg_to_hw_reg(reg)), -offset);
      break;
    case 0: {
      int ext_op = *p;
      p++;
...
static G_GNUC_UNUSED void print_dwarf_state(int cfa_reg, int cfa_offset, int ip,
                                            int nregs, Loc *locations,
                                            guint8 *reg_saved) {
 ...
 * operations in unwind_info until the location counter reaches POS. The result
 * is stored back into REGS. OUT_CFA will receive the value of the CFA. If
 * SAVE_LOCATIONS is non-NULL, it should point to an array of size
 * SAVE_LOCATIONS_LEN. On return, the nth entry will point to the address of the
 * stack slot where register N was saved, or NULL, if it was not saved by this
 * frame. MARK_LOCATIONS should contain the locations marked by
 * mono_emit_unwind_op_mark_loc (), if any. This function is signal safe.
 *
...
gboolean mono_unwind_frame(guint8 *unwind_info, guint32 unwind_info_len,
  ...
  while (pos <= ip - start_ip && p < unwind_info + unwind_info_len) {
    ...
    case 0: {
      ...
      case DW_CFA_mono_advance_loc:
        ...
        break;
      default:
        mono_runtime_printf_err("Unwind failure. Illegal value for switch "
                                "statement, assertion at %s %d\n.",
                                __FILE__, __LINE__);
        return FALSE;
      }
      break;
  ...
  if (cfa_reg == -1) {
    mono_runtime_printf_err(
        "Unset cfa_reg in method %s. Memory around ip (%p):",
        mono_get_method_from_ip(ip), ip);
  ...
  for (hwreg = 0; hwreg < NUM_HW_REGS; ++hwreg) {
    if (reg_saved[hwreg] && locations[hwreg].loc_type == LOC_OFFSET) {
      ...
      if (save_locations && hwreg < save_locations_len)
        save_locations[hwreg] =
            (host_mgreg_t *)(cfa_val + locations[hwreg].offset);
    }
  }
...
void mono_unwind_init(void) {
  ...
  mono_counters_register("Unwind info size",
 ...
 *
 *   Save UNWIND_INFO in the unwind info cache and return an id which can be
 * passed to mono_get_cached_unwind_info to get a cached copy of the info. A
 * copy is made of the unwind info. This function is useful for two reasons:
 * - many methods have the same unwind info
 * - MonoJitInfo->unwind_info is an int so it can't store the pointer to the
 * unwind info
 */
guint32 mono_cache_unwind_info(guint8 *unwind_info, guint32 unwind_info_len) {
  ...
  for (i = 0; i < cached_info_next; ++i) {
    ...
    if (cached->len == unwind_info_len &&
        memcmp(cached->info, unwind_info, unwind_info_len) == 0) {
      unwind_unlock();
      return i;
    }
  }
 ...
 */
int mono_unwind_get_dwarf_data_align(void) { return DWARF_DATA_ALIGN; }
 ...
 * previous frame.
 */
int mono_unwind_get_dwarf_pc_reg(void) { return DWARF_PC_REG; }

static void decode_cie_op(guint8 *p, guint8 **endp) {
  int op = *p & 0xc0;
...
static gint64 read_encoded_val(guint32 encoding, guint8 *p, guint8 **endp) {
  gint64 res;
 ...
 */
static void decode_lsda(guint8 *lsda, guint8 *code,
                        MonoJitExceptionInfo *ex_info, gpointer *type_info,
                        guint32 *ex_info_len, int *this_reg, int *this_offset) {
  guint8 *p;
 ...
 * If not NULL, EX_INFO is set to a malloc-ed array of MonoJitExceptionInfo
 * structures, only try_start, try_end and handler_start is set. If not NULL,
 * TYPE_INFO is set to a malloc-ed array containing the ttype table from the
 * LSDA.
 */
  ...
  *this_offset = -1;
    ...
    for (i = 0; cie_aug_str[i] != '\0'; ++i) {
      ...
      case 'L':
        g_assert((*p == (DW_EH_PE_sdata4 | DW_EH_PE_pcrel)) ||
                 (*p == (DW_EH_PE_sdata8 | DW_EH_PE_pcrel)));
        p++;
        break;
      case 'R':
        g_assert(*p == (DW_EH_PE_sdata4 | DW_EH_PE_pcrel));
  ...
  if (aug_len) {
    ...
    if (lsda_offset != 0) {
      ...
      if (ex_info)
        *ex_info = (MonoJitExceptionInfo *)g_malloc0(
            len * sizeof(MonoJitExceptionInfo));
      if (type_info)
        *type_info = (gpointer *)g_malloc0(len * sizeof(gpointer));

      decode_lsda(lsda, code, ex_info ? *ex_info : NULL,
                  type_info ? *type_info : NULL, ex_info_len, this_reg,
                  this_offset);
    }
  }
 ...
 * If EI/TYPE_INFO/UNW_INFO are NULL, compute only the value of the scalar
 * fields in INFO. Otherwise:
...
void mono_unwind_decode_llvm_mono_fde(guint8 *fde, int fde_len, guint8 *cie,
  ...
  if (has_aug) {
    ...
    /* Get the lengths first */
    decode_lsda(lsda, code, NULL, NULL, &res->ex_info_len, &res->this_reg,
                &res->this_offset);

    decode_lsda(lsda, code, ex_info, type_info, NULL, &res->this_reg,
                &res->this_offset);
  }
