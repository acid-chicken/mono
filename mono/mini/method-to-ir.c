...
static const gboolean debug_tailcall_try_all =
    FALSE; // consider any call followed by ret

gboolean mono_tailcall_print_enabled(void) {
  return debug_tailcall ||
         MONO_TRACE_IS_TRACED(G_LOG_LEVEL_DEBUG, MONO_TRACE_TAILCALL);
}

void mono_tailcall_print(const char *format, ...) {
...
#define INLINE_FAILURE(msg)                                                    \
  do {                                                                         \
    if ((cfg->method != cfg->current_method) &&                                \
        (cfg->current_method->wrapper_type == MONO_WRAPPER_NONE)) {            \
      inline_failure(cfg, msg);                                                \
      goto exception_exit;                                                     \
    }                                                                          \
  } while (0)
#define CHECK_CFG_EXCEPTION                                                    \
  do {                                                                         \
    if (cfg->exception_type != MONO_EXCEPTION_NONE)                            \
      goto exception_exit;                                                     \
  } while (0)
#define FIELD_ACCESS_FAILURE(method, field)                                    \
  do {                                                                         \
    field_access_failure((cfg), (method), (field));                            \
    goto exception_exit;                                                       \
  } while (0)
#define GENERIC_SHARING_FAILURE(opcode)                                        \
  do {                                                                         \
    if (cfg->gshared) {                                                        \
...
#define GSHAREDVT_FAILURE(opcode)                                              \
  do {                                                                         \
    if (cfg->gsharedvt) {                                                      \
...
#define OUT_OF_MEMORY_FAILURE                                                  \
  do {                                                                         \
    mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);                    \
    mono_error_set_out_of_memory(cfg->error, "");                              \
    goto exception_exit;                                                       \
  } while (0)
#define DISABLE_AOT(cfg)                                                       \
  do {                                                                         \
...
#define TYPE_LOAD_ERROR(klass)                                                 \
  do {                                                                         \
    cfg->exception_ptr = klass;                                                \
    LOAD_ERROR;                                                                \
  } while (0)

#define CHECK_CFG_ERROR                                                        \
  do {                                                                         \
    if (!is_ok(cfg->error)) {                                                  \
      mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);                  \
...
static MonoInst *convert_value(MonoCompile *cfg, MonoType *type, MonoInst *ins);

/* helper methods signatures */

/* type loading helpers */
static GENERATE_TRY_GET_CLASS_WITH_CACHE(debuggable_attribute, "System.Diagnostics", "DebuggableAttribute") static GENERATE_GET_CLASS_WITH_CACHE(
    iequatable, "System",
    "IEquatable`1") static GENERATE_GET_CLASS_WITH_CACHE(geqcomparer,
                                                         "System.Collections."
                                                         "Generic",
                                                         "GenericEqualityCompar"
                                                         "er`1");
...
#define MINI_OP(a, b, dest, src1, src2)                                        \
  ((src2) != NONE ? 2 : ((src1) != NONE ? 1 : 0)),
#define MINI_OP3(a, b, dest, src1, src2, src3)                                 \
  ((src3) != NONE ? 3 : ((src2) != NONE ? 2 : ((src1) != NONE ? 1 : 0))),
/*
...
guint32 mono_alloc_ireg(MonoCompile *cfg) { return alloc_ireg(cfg); }

guint32 mono_alloc_lreg(MonoCompile *cfg) { return alloc_lreg(cfg); }

guint32 mono_alloc_freg(MonoCompile *cfg) { return alloc_freg(cfg); }

guint32 mono_alloc_preg(MonoCompile *cfg) { return alloc_preg(cfg); }
 ...
 */
guint32 mono_alloc_ireg_ref(MonoCompile *cfg) { return alloc_ireg_ref(cfg); }
 ...
 */
guint32 mono_alloc_ireg_mp(MonoCompile *cfg) { return alloc_ireg_mp(cfg); }
...
void mono_print_bb(MonoBasicBlock *bb, const char *msg) {
  ...
  for (i = 0; i < bb->in_count; ++i)
    g_string_append_printf(str, " BB%d(%d)", bb->in_bb[i]->block_num,
                           bb->in_bb[i]->dfn);
  g_string_append_printf(str, ", OUT: ");
  for (i = 0; i < bb->out_count; ++i)
    g_string_append_printf(str, " BB%d(%d)", bb->out_bb[i]->block_num,
                           bb->out_bb[i]->dfn);
...
static MONO_NEVER_INLINE gboolean break_on_unverified(void) {
...
static void clear_cfg_error(MonoCompile *cfg) {
  mono_error_cleanup(cfg->error);
  error_init(cfg->error);
}

static MONO_NEVER_INLINE void field_access_failure(MonoCompile *cfg,
                                                   MonoMethod *method,
                                                   MonoClassField *field) {
  char *method_fname = mono_method_full_name(method, TRUE);
  char *field_fname = mono_field_full_name(field);
  mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);
  mono_error_set_generic_error(cfg->error, "System", "FieldAccessException",
...
static MONO_NEVER_INLINE void inline_failure(MonoCompile *cfg,
                                             const char *msg) {
  if (cfg->verbose_level >= 2)
...
static MONO_NEVER_INLINE void gshared_failure(MonoCompile *cfg, int opcode,
                                              const char *file, int line) {
  if (cfg->verbose_level > 2)
    printf("sharing failed for method %s.%s.%s/%d opcode %s line %d\n",
           m_class_get_name_space(cfg->current_method->klass),
           m_class_get_name(cfg->current_method->klass),
           cfg->current_method->name,
           cfg->current_method->signature->param_count,
           mono_opcode_name(opcode), line);
  mono_cfg_set_exception(cfg, MONO_EXCEPTION_GENERIC_SHARING_FAILED);
}

static MONO_NEVER_INLINE void gsharedvt_failure(MonoCompile *cfg, int opcode,
                                                const char *file, int line) {
  cfg->exception_message = g_strdup_printf(
      "gsharedvt failed for method %s.%s.%s/%d opcode %s %s:%d",
      m_class_get_name_space(cfg->current_method->klass),
      m_class_get_name(cfg->current_method->klass), cfg->current_method->name,
      cfg->current_method->signature->param_count, mono_opcode_name((opcode)),
      file, line);
  if (cfg->verbose_level >= 2)
...
/*
 * When using gsharedvt, some instatiations might be verifiable, and some might
 * be not. i.e. foo<T> (int i) { ldarg.0; box T; }
 */
#define UNVERIFIED                                                             \
  do {                                                                         \
    if (cfg->gsharedvt) {                                                      \
      if (cfg->verbose_level > 2)                                              \
        printf("gsharedvt method failed to verify, falling back to "           \
               "instantiation.\n");                                            \
      mono_cfg_set_exception(cfg, MONO_EXCEPTION_GENERIC_SHARING_FAILED);      \
      goto exception_exit;                                                     \
    }                                                                          \
...
#define GET_BBLOCK(cfg, tblock, ip)                                            \
  do {                                                                         \
...
static void add_widen_op(MonoCompile *cfg, MonoInst *ins, MonoInst **arg1_ref,
                         MonoInst **arg2_ref) {
  MonoInst *arg1 = *arg1_ref;
  MonoInst *arg2 = *arg2_ref;

  if (cfg->r4fp && ((arg1->type == STACK_R4 && arg2->type == STACK_R8) ||
...
#define ADD_BINCOND(next_block)                                                \
  do {                                                                         \
    ...
    CHECK_TYPE(cmp);                                                           \
    type_from_op(cfg, ins, sp[0], sp[1]);                                      \
    ins->inst_many_bb = (MonoBasicBlock **)mono_mempool_alloc(                 \
        cfg->mempool, sizeof(gpointer) * 2);                                   \
...
static void link_bblock(MonoCompile *cfg, MonoBasicBlock *from,
  ...
  if (!found) {
    newa = (MonoBasicBlock **)mono_mempool_alloc(
        cfg->mempool, sizeof(gpointer) * (from->out_count + 1));
    for (i = 0; i < from->out_count; ++i) {
  ...
  if (!found) {
    newa = (MonoBasicBlock **)mono_mempool_alloc(
        cfg->mempool, sizeof(gpointer) * (to->in_count + 1));
    for (i = 0; i < to->in_count; ++i) {
      newa[i] = to->in_bb[i];
    }
    newa[i] = from;
...
static void mark_bb_in_region(MonoCompile *cfg, guint region, uint32_t start,
                              uint32_t end) {
  MonoBasicBlock *bb = cfg->cil_offset_to_bb[start];

  // start must exist in cil_offset_to_bb as those are il offsets used by EH
  // which should have GET_BBLOCK early.
  g_assert(bb);
...
static void compute_bb_regions(MonoCompile *cfg) {
  MonoBasicBlock *bb;
  ...
  for (i = 0; i < header->num_clauses; ++i) {
    ...
    if (clause->flags == MONO_EXCEPTION_CLAUSE_FILTER)
      mark_bb_in_region(cfg,
                        ((i + 1) << 8) | MONO_REGION_FILTER | clause->flags,
                        clause->data.filter_offset, clause->handler_offset);
    ...
    mark_bb_in_region(cfg, handler_region, clause->handler_offset,
                      clause->handler_offset + clause->handler_len);
    mark_bb_in_region(cfg, ((i + 1) << 8) | clause->flags, clause->try_offset,
                      clause->try_offset + clause->try_len);
  }

  if (cfg->verbose_level > 2) {
    MonoBasicBlock *bb;
    for (bb = cfg->bb_entry; bb; bb = bb->next_bb)
      g_print("REGION BB%d IL_%04x ID_%08X\n", bb->block_num, bb->real_offset,
              bb->region);
  }
}

static gboolean ip_in_finally_clause(MonoCompile *cfg, int offset) {
  MonoMethodHeader *header = cfg->header;
...
static GList *mono_find_leave_clauses(MonoCompile *cfg, guchar *ip,
                                      guchar *target) {
  MonoMethodHeader *header = cfg->header;
  MonoExceptionClause *clause;
  int i;
  ...
  for (i = 0; i < header->num_clauses; ++i) {
    ...
    if (MONO_OFFSET_IN_CLAUSE(clause, (ip - header->code)) &&
      ...
      MonoLeaveClause *leave =
          mono_mempool_alloc0(cfg->mempool, sizeof(MonoLeaveClause));
      leave->index = i;
      leave->clause = clause;
...
static MonoInst *mono_create_exvar_for_offset(MonoCompile *cfg, int offset) {
  MonoInst *var;
 ...
 */
void mini_type_to_eval_stack_type(MonoCompile *cfg, MonoType *type,
                                  MonoInst *inst) {
  MonoClass *klass;
...
static const char bin_num_table[STACK_MAX][STACK_MAX] = {
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV, STACK_INV},
    {STACK_INV, STACK_I4, IF_P8_I8, STACK_PTR, STACK_INV, STACK_MP, STACK_INV,
     STACK_INV},
    {STACK_INV, IF_P8_I8, STACK_I8, IF_P8_PTR, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV},
    {STACK_INV, STACK_PTR, IF_P8_PTR, STACK_PTR, STACK_INV, STACK_MP, STACK_INV,
     STACK_INV},
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_R8, STACK_INV, STACK_INV,
     STACK_INV, STACK_R8},
    {STACK_INV, STACK_MP, STACK_INV, STACK_MP, STACK_INV, STACK_PTR, STACK_INV,
     STACK_INV},
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV, STACK_INV},
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
...
static const char neg_table[] = {STACK_INV, STACK_I4,  STACK_I8,
                                 STACK_PTR, STACK_R8,  STACK_INV,
                                 STACK_INV, STACK_INV, STACK_R4};

/* reduce the size of this table */
static const char bin_int_table[STACK_MAX][STACK_MAX] = {
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV, STACK_INV},
    {STACK_INV, STACK_I4, IF_P8_I8, STACK_PTR, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV},
    {STACK_INV, IF_P8_I8, STACK_I8, IF_P8_PTR, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV},
    {STACK_INV, STACK_PTR, IF_P8_PTR, STACK_PTR, STACK_INV, STACK_INV,
     STACK_INV, STACK_INV},
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV, STACK_INV},
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
...
/* reduce the size of this table */
static const char shift_table[STACK_MAX][STACK_MAX] = {
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV, STACK_INV},
    {STACK_INV, STACK_I4, STACK_INV, STACK_I4, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV},
    {STACK_INV, STACK_I8, STACK_INV, STACK_I8, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV},
    {STACK_INV, STACK_PTR, STACK_INV, STACK_PTR, STACK_INV, STACK_INV,
     STACK_INV, STACK_INV},
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
     STACK_INV, STACK_INV},
    {STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV, STACK_INV,
...
static const guint16 binops_op_map[STACK_MAX] = {0,
                                                 OP_IADD - CEE_ADD,
                                                 OP_LADD - CEE_ADD,
                                                 OP_PADD - CEE_ADD,
                                                 OP_FADD - CEE_ADD,
...
static const guint16 unops_op_map[STACK_MAX] = {0,
                                                OP_INEG - CEE_NEG,
                                                OP_LNEG - CEE_NEG,
                                                OP_PNEG - CEE_NEG,
                                                OP_FNEG - CEE_NEG,
...
static const guint16 ovfops_op_map[STACK_MAX] = {0,
                                                 OP_ICONV_TO_U2 - CEE_CONV_U2,
                                                 OP_LCONV_TO_U2 - CEE_CONV_U2,
                                                 OP_PCONV_TO_U2 - CEE_CONV_U2,
                                                 OP_FCONV_TO_U2 - CEE_CONV_U2,
                                                 OP_PCONV_TO_U2 - CEE_CONV_U2,
                                                 OP_PCONV_TO_U2 - CEE_CONV_U2,
                                                 0,
                                                 OP_RCONV_TO_U2 - CEE_CONV_U2};

/* handles from CEE_CONV_OVF_I1_UN to CEE_CONV_OVF_U_UN */
static const guint16 ovf2ops_op_map[STACK_MAX] = {
    0,
    OP_ICONV_TO_OVF_I1_UN - CEE_CONV_OVF_I1_UN,
    OP_LCONV_TO_OVF_I1_UN - CEE_CONV_OVF_I1_UN,
    OP_PCONV_TO_OVF_I1_UN - CEE_CONV_OVF_I1_UN,
    OP_FCONV_TO_OVF_I1_UN - CEE_CONV_OVF_I1_UN,
    OP_PCONV_TO_OVF_I1_UN - CEE_CONV_OVF_I1_UN,
    0,
    0,
    OP_RCONV_TO_OVF_I1_UN - CEE_CONV_OVF_I1_UN};

/* handles from CEE_CONV_OVF_I1 to CEE_CONV_OVF_U8 */
static const guint16 ovf3ops_op_map[STACK_MAX] = {
    0,
    OP_ICONV_TO_OVF_I1 - CEE_CONV_OVF_I1,
    OP_LCONV_TO_OVF_I1 - CEE_CONV_OVF_I1,
    OP_PCONV_TO_OVF_I1 - CEE_CONV_OVF_I1,
    OP_FCONV_TO_OVF_I1 - CEE_CONV_OVF_I1,
    OP_PCONV_TO_OVF_I1 - CEE_CONV_OVF_I1,
    0,
    0,
    OP_RCONV_TO_OVF_I1 - CEE_CONV_OVF_I1};

/* handles from CEE_BEQ to CEE_BLT_UN */
static const guint16 beqops_op_map[STACK_MAX] = {0,
                                                 OP_IBEQ - CEE_BEQ,
                                                 OP_LBEQ - CEE_BEQ,
                                                 OP_PBEQ - CEE_BEQ,
                                                 OP_FBEQ - CEE_BEQ,
...
static const guint16 ceqops_op_map[STACK_MAX] = {0,
                                                 OP_ICEQ - OP_CEQ,
                                                 OP_LCEQ - OP_CEQ,
                                                 OP_PCEQ - OP_CEQ,
                                                 OP_FCEQ - OP_CEQ,
 ...
 * Invalid IL code is marked by setting ins->type to the invalid value
 * STACK_INV.
 *
 ...
 */
static void type_from_op(MonoCompile *cfg, MonoInst *ins, MonoInst *src1,
                         MonoInst *src2) {
  switch (ins->opcode) {
  /* binops */
  case MONO_CEE_ADD:
  ...
  case OP_ICOMPARE:
    ...
    if ((src1->type == STACK_I8) ||
        ((TARGET_SIZEOF_VOID_P == 8) &&
         ((src1->type == STACK_PTR) || (src1->type == STACK_OBJ) ||
          (src1->type == STACK_MP))))
  ...
  case OP_CLT_UN:
    ins->type =
        (bin_comp_table[src1->type][src2->type] & 1) ? STACK_I4 : STACK_INV;
    ins->opcode += ceqops_op_map[src1->type];
    break;
  /* unops */
  case MONO_CEE_NEG:
...
void mini_type_from_op(MonoCompile *cfg, MonoInst *ins, MonoInst *src1,
                       MonoInst *src2) {
  type_from_op(cfg, ins, src1, src2);
}

static MonoClass *ldind_to_type(int op) {
  switch (op) {
  ...
  case MONO_CEE_LDIND_REF:
    return mono_defaults
        .object_class; // FIXME we should try to return a more specific type
  default:
    g_error("Unknown ldind type %d", op);
  }
}
 ...
 */
inline static MonoInst *mono_get_domainvar(MonoCompile *cfg) {
  if (!cfg->domainvar) {
    /* Make sure we don't generate references after checking whenever to init
     * this */
    g_assert(!cfg->domainvar_inited);
    cfg->domainvar =
        mono_compile_create_var(cfg, mono_get_int_type(), OP_LOCAL);
...
static void mono_create_rgctx_var(MonoCompile *cfg) {
  if (!cfg->rgctx_var) {
    cfg->rgctx_var =
        mono_compile_create_var(cfg, mono_get_int_type(), OP_LOCAL);
...
static MonoInst *mono_get_mrgctx_var(MonoCompile *cfg) {
  g_assert(cfg->gshared);
...
static MonoInst *mono_get_vtable_var(MonoCompile *cfg) {
  g_assert(cfg->gshared);
...
static MonoType *type_from_stack_type(MonoInst *ins) {
  switch (ins->type) {
  case STACK_I4:
...
static G_GNUC_UNUSED int type_to_stack_type(MonoCompile *cfg, MonoType *t) {
...
static MonoClass *array_access_to_klass(int opcode) {
  switch (opcode) {
 ...
 */
static MonoInst *mono_compile_get_interface_var(MonoCompile *cfg, int slot,
                                                MonoInst *ins) {
  MonoInst *res;
  int pos, vnum;
  MonoType *type;
...
static void mono_save_token_info(MonoCompile *cfg, MonoImage *image,
  ...
  if (cfg->compile_aot && !cfg->generic_context &&
    ...
    MonoJumpInfoToken *jump_info_token =
        (MonoJumpInfoToken *)mono_mempool_alloc0(cfg->mempool,
 ...
 * at basic block boundaries. What happens is that we save the values to local
 * variables and we reload them later when first entering the target basic block
 * (with the handle_loaded_temps () function). A single joint point will use the
 * same variables (stored in the array bb->out_stack or bb->in_stack, if the
 * basic block is before or after the joint point).
 *
...
static void handle_stack_args(MonoCompile *cfg, MonoInst **sp, int count) {
  ...
  if (!bb->out_scount) {
    ...
    for (i = 0; i < bb->out_count; ++i) {
      outb = bb->out_bb[i];
      /* exception handlers are linked, but they should not be considered for
       * stack args */
    ...
    // printf ("\n");
    if (!found) {
      bb->out_stack = (MonoInst **)mono_mempool_alloc(
          cfg->mempool, sizeof(MonoInst *) * count);
      for (i = 0; i < count; ++i) {
        /*
         * try to reuse temps already allocated for this purpouse, if they
         * occupy the same stack slot and if they are of the same type. This
         * won't cause conflicts since if 'local' is used to store one of the
         * values in the in_stack of a bblock, then the same variable will be
         * used for the same outgoing stack slot as well. This doesn't work when
         * inlining methods, since the bblocks in the inlined methods do not
         * inherit their in_stack from the bblock they are inlined to. See bug
         * #58863 for an example.
         */
...
MonoInst *mini_emit_runtime_constant(MonoCompile *cfg,
                                     MonoJumpInfoType patch_type,
                                     gpointer data) {
  MonoInst *ins;

  if (cfg->compile_aot) {
    MONO_DISABLE_WARNING(
        4306) // 'type cast': conversion from 'MonoJumpInfoType' to 'MonoInst *'
              // of greater size
    ...
    ji.data.target = data;
    target =
        mono_resolve_patch_target(NULL, cfg->domain, NULL, &ji, FALSE, error);
...
static MonoInst *mono_create_fast_tls_getter(MonoCompile *cfg, MonoTlsKey key) {
...
static MonoInst *mono_create_tls_get(MonoCompile *cfg, MonoTlsKey key) {
  ...
  if (cfg->compile_aot && !cfg->llvm_only) {
     ...
     * Therefore the NOCALL part of MONO_PATCH_INFO_JIT_ICALL_ADDR_NOCALL, FALSE
     * in is_plt_patch.
     */
    EMIT_NEW_AOTCONST(cfg, addr, MONO_PATCH_INFO_JIT_ICALL_ADDR_NOCALL,
 ...
 */
static void emit_push_lmf(MonoCompile *cfg) {
  /*
  ...
  /* Save previous_lmf */
  EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOAD_MEMBASE, prev_lmf_reg,
                        cfg->lmf_addr_var->dreg, 0);
  EMIT_NEW_STORE_MEMBASE(cfg, ins, OP_STORE_MEMBASE_REG, lmf_reg,
                         MONO_STRUCT_OFFSET(MonoLMF, previous_lmf),
                         prev_lmf_reg);
  /* Set new lmf */
  EMIT_NEW_STORE_MEMBASE(cfg, ins, OP_STORE_MEMBASE_REG,
                         cfg->lmf_addr_var->dreg, 0, lmf_reg);
}
 ...
 */
static void emit_pop_lmf(MonoCompile *cfg) {
  ...
  prev_lmf_reg = alloc_preg(cfg);
  EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOAD_MEMBASE, prev_lmf_reg, lmf_reg,
                        MONO_STRUCT_OFFSET(MonoLMF, previous_lmf));
  EMIT_NEW_STORE_MEMBASE(cfg, ins, OP_STORE_MEMBASE_REG, lmf_addr_reg, 0,
                         prev_lmf_reg);
}
...
static int target_type_is_incompatible(MonoCompile *cfg, MonoType *target,
  ...
  if (target->byref) {
    ...
    if (arg->type == STACK_MP) {
      /* This is needed to handle gshared types + ldaddr. We lower the types so
       * we can handle enums and other typedef-like types. */
      MonoClass *target_class_lowered = mono_class_from_mono_type_internal(
          mini_get_underlying_type(m_class_get_byval_arg(
              mono_class_from_mono_type_internal(target))));
      MonoClass *source_class_lowered = mono_class_from_mono_type_internal(
          mini_get_underlying_type(m_class_get_byval_arg(arg->klass)));

      /* if the target is native int& or X* or same type */
      if (target->type == MONO_TYPE_I || target->type == MONO_TYPE_PTR ||
          target_class_lowered == source_class_lowered)
        return 0;

      /* Both are primitive type byrefs and the source points to a larger type
       * that the destination */
      if (MONO_TYPE_IS_PRIMITIVE_SCALAR(
              m_class_get_byval_arg(target_class_lowered)) &&
          MONO_TYPE_IS_PRIMITIVE_SCALAR(
              m_class_get_byval_arg(source_class_lowered)) &&
          mono_class_instance_size(target_class_lowered) <=
  ...
  case MONO_TYPE_FNPTR:
    /*
     * Some opcodes like ldloca returns 'transient pointers' which can be stored
     * in in native int. (#688008).
     */
    if (arg->type != STACK_I4 && arg->type != STACK_PTR &&
        arg->type != STACK_MP)
      return 1;
  ...
  case MONO_TYPE_GENERICINST:
    if (mono_type_generic_inst_is_valuetype(simple_type)) {
      ...
      if (klass != arg->klass && target_class != arg->klass &&
          target_class !=
 ...
 * convert_value:
 *
 *   Emit some implicit conversions which are not part of the .net spec, but are
 * allowed by MS.NET.
 */
static MonoInst *convert_value(MonoCompile *cfg, MonoType *type,
                               MonoInst *ins) {
  if (!cfg->r4fp)
    return ins;
  type = mini_get_underlying_type(type);
 ...
 */
static gboolean check_call_signature(MonoCompile *cfg, MonoMethodSignature *sig,
                                     MonoInst **args) {
  MonoType *simple_type;
  int i;
  ...
  handle_enum:
    ...
    case MONO_TYPE_PTR:
    case MONO_TYPE_FNPTR:
      if (args[i]->type != STACK_I4 &&
          !(SIZEOF_VOID_P == 8 && args[i]->type == STACK_I8) &&
          args[i]->type != STACK_PTR && args[i]->type != STACK_MP &&
          args[i]->type != STACK_OBJ)
        return TRUE;
      continue;
    case MONO_TYPE_CLASS:
    case MONO_TYPE_STRING:
    ...
    case MONO_TYPE_U8:
      if (args[i]->type != STACK_I8 &&
          !(SIZEOF_VOID_P == 8 &&
            (args[i]->type == STACK_I4 || args[i]->type == STACK_PTR)))
        return TRUE;
      continue;
    case MONO_TYPE_R4:
    ...
    case MONO_TYPE_VALUETYPE:
      if (m_class_is_enumtype(simple_type->data.klass)) {
        simple_type =
            mono_class_enum_basetype_internal(simple_type->data.klass);
        goto handle_enum;
      }
    ...
    case MONO_TYPE_TYPEDBYREF:
      ...
      continue;
    case MONO_TYPE_GENERICINST:
      simple_type = m_class_get_byval_arg(
          simple_type->data.generic_class->container_class);
      goto handle_enum;
    case MONO_TYPE_VAR:
    case MONO_TYPE_MVAR:
      /* gsharedvt */
      ...
      continue;
    default:
      g_error("unknown type 0x%02x in check_call_signature", simple_type->type);
    }
  }
  return FALSE;
}

MonoJumpInfo *mono_patch_info_new(MonoMemPool *mp, int ip,
                                  MonoJumpInfoType type, gconstpointer target) {
  MonoJumpInfo *ji =
      (MonoJumpInfo *)mono_mempool_alloc(mp, sizeof(MonoJumpInfo));
 ...
 *
 *   Check whenever the vtable or an mrgctx needs to be passed when calling
 * CMETHOD.
 */
static void check_method_sharing(MonoCompile *cfg, MonoMethod *cmethod,
                                 gboolean *out_pass_vtable,
                                 gboolean *out_pass_mrgctx) {
  gboolean pass_vtable = FALSE;
  gboolean pass_mrgctx = FALSE;

  if (((cmethod->flags & METHOD_ATTRIBUTE_STATIC) ||
       m_class_is_valuetype(cmethod->klass)) &&
      (mono_class_is_ginst(cmethod->klass) ||
       mono_class_is_gtd(cmethod->klass))) {
     ...
     * generic method).
     */
    if (sharable && !(mini_method_get_context(cmethod) &&
...
static gboolean direct_icalls_enabled(MonoCompile *cfg, MonoMethod *method) {
  if (cfg->gen_sdb_seq_points || cfg->disable_direct_icalls)
    return FALSE;

  if (method && cfg->compile_aot &&
      mono_aot_direct_icalls_enabled_for_method(cfg, method))
    return TRUE;
...
MonoInst *mono_emit_jit_icall_by_info(MonoCompile *cfg, int il_offset,
                                      MonoJitICallInfo *info, MonoInst **args) {
  /*
   * Call the jit icall without a wrapper if possible.
   * The wrapper is needed to be able to do stack walks for asynchronously
   * suspended threads when debugging.
   */
  if (direct_icalls_enabled(cfg, NULL)) {
     ...
     * an exception check.
     */
    costs = inline_method(cfg, info->wrapper_method, NULL, args, NULL,
                          il_offset, TRUE);
    g_assert(costs > 0);
...
static MonoInst *mono_emit_widen_call_res(MonoCompile *cfg, MonoInst *ins,
...
static MonoInst *emit_get_rgctx_method(MonoCompile *cfg, int context_used,
                                       MonoMethod *cmethod,
                                       MonoRgctxInfoType rgctx_type);

static void emit_method_access_failure(MonoCompile *cfg, MonoMethod *caller,
                                       MonoMethod *callee) {
  MonoInst *args[2];
  args[0] = emit_get_rgctx_method(cfg, mono_method_check_context_used(caller),
                                  caller, MONO_RGCTX_INFO_METHOD);
  args[1] = emit_get_rgctx_method(cfg, mono_method_check_context_used(callee),
                                  callee, MONO_RGCTX_INFO_METHOD);
  mono_emit_jit_icall(cfg, mono_throw_method_access, args);
}

static void emit_bad_image_failure(MonoCompile *cfg, MonoMethod *caller,
                                   MonoMethod *callee) {
  mono_emit_jit_icall(cfg, mono_throw_bad_image, NULL);
}

// FIXME Consolidate the multiple functions named get_method_nofail.
static MonoMethod *get_method_nofail(MonoClass *klass, const char *method_name,
                                     int num_params, int flags) {
  MonoMethod *method;
  ERROR_DECL(error);
  method = mono_class_get_method_from_name_checked(klass, method_name,
                                                   num_params, flags, error);
  mono_error_assert_ok(error);
  g_assertf(method, "Could not lookup method %s in %s", method_name,
...
MonoMethod *mini_get_memcpy_method(void) {
  ...
  if (!memcpy_method) {
    memcpy_method =
        get_method_nofail(mono_defaults.string_class, "memcpy", 3, 0);
    if (!memcpy_method)
...
MonoInst *mini_emit_storing_write_barrier(MonoCompile *cfg, MonoInst *ptr,
                                          MonoInst *value) {
  MonoInst *store;
  ...
  EMIT_NEW_STORE_MEMBASE(cfg, store, OP_STORE_MEMBASE_REG, ptr->dreg, 0,
                         value->dreg);
...
void mini_emit_write_barrier(MonoCompile *cfg, MonoInst *ptr, MonoInst *value) {
  ...
  if (cfg->backend->have_card_table_wb && !cfg->compile_aot && card_table &&
      nursery_shift_bits > 0 && !COMPILE_LLVM(cfg)) {
    MonoInst *wbarrier;
    ...
    /*
     * We emit a fast light weight write barrier. This always marks cards as in
     * the concurrent collector case, so, for the serial collector, it might
     * slightly slow down nursery collections. We also expect that the host
     * system and the target system have the same card table configuration,
     * which is the case if they have the same pointer size.
     */

    MONO_EMIT_NEW_BIALU_IMM(cfg, OP_SHR_UN_IMM, offset_reg, ptr->dreg,
                            card_table_shift_bits);
    if (card_table_mask)
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_PAND_IMM, offset_reg, offset_reg,
                              card_table_mask);

    /*We can't use PADD_IMM since the cardtable might end up in high addresses
     * and amd64 doesn't support IMM's larger than 32bits.
     */
    ins = mini_emit_runtime_constant(cfg, MONO_PATCH_INFO_GC_CARD_TABLE_ADDR,
                                     NULL);
    card_reg = ins->dreg;

    MONO_EMIT_NEW_BIALU(cfg, OP_PADD, offset_reg, offset_reg, card_reg);
    MONO_EMIT_NEW_STORE_MEMBASE_IMM(cfg, OP_STOREI1_MEMBASE_IMM, offset_reg, 0,
                                    1);
  } else {
...
MonoMethod *mini_get_memset_method(void) {
  ...
  if (!memset_method) {
    memset_method =
        get_method_nofail(mono_defaults.string_class, "memset", 3, 0);
    if (!memset_method)
      g_error("Old corlib found. Install a new one");
  }
...
void mini_emit_initobj(MonoCompile *cfg, MonoInst *dest, const guchar *ip,
  ...
  if (mini_is_gsharedvt_klass(klass)) {
    size_ins = mini_emit_get_gsharedvt_info_klass(cfg, klass,
                                                  MONO_RGCTX_INFO_VALUE_SIZE);
    bzero_ins =
        mini_emit_get_gsharedvt_info_klass(cfg, klass, MONO_RGCTX_INFO_BZERO);
    if (!bzero_method)
      bzero_method = get_method_nofail(mono_defaults.string_class,
                                       "bzero_aligned_1", 2, 0);
    g_assert(bzero_method);
    iargs[0] = dest;
    iargs[1] = size_ins;
    mini_emit_calli(cfg, mono_method_signature_internal(bzero_method), iargs,
                    bzero_ins, NULL, NULL);
    return;
  }

  klass = mono_class_from_mono_type_internal(
...
static gboolean context_used_is_mrgctx(MonoCompile *cfg, int context_used) {
 ...
 */
static MonoInst *emit_get_rgctx(MonoCompile *cfg, int context_used) {
  ...
  if (context_used_is_mrgctx(cfg, context_used)) {
    ...
    if (method->is_inflated && mono_method_get_context(method)->method_inst) {
      ...
      EMIT_NEW_LOAD_MEMBASE(
          cfg, vtable_var, OP_LOAD_MEMBASE, vtable_reg, mrgctx_var->dreg,
          MONO_STRUCT_OFFSET(MonoMethodRuntimeGenericContext, class_vtable));
    ...
    vtable_reg = alloc_preg(cfg);
    EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOAD_MEMBASE, vtable_reg, this_ins->dreg,
...
mono_patch_info_rgctx_entry_new(MonoMemPool *mp, MonoMethod *method,
  ...
  MonoJumpInfoRgctxEntry *res = (MonoJumpInfoRgctxEntry *)mono_mempool_alloc0(
      mp, sizeof(MonoJumpInfoRgctxEntry));
  if (in_mrgctx)
    res->d.method = method;
  else
...
static MonoInst *emit_rgctx_fetch_inline(MonoCompile *cfg, MonoInst *rgctx,
 ...
 * given by RGCTX.
 */
static MonoInst *emit_rgctx_fetch(MonoCompile *cfg, MonoInst *rgctx,
                                  MonoJumpInfoRgctxEntry *entry) {
  if (cfg->llvm_only)
    return emit_rgctx_fetch_inline(cfg, rgctx, entry);
  else
    return mini_emit_abs_call(cfg, MONO_PATCH_INFO_RGCTX_FETCH, entry,
                              mono_icall_sig_ptr_ptr, &rgctx);
}

/*
 * mini_emit_get_rgctx_klass:
 *
 *   Emit IR to load the property RGCTX_TYPE of KLASS. If context_used is 0,
 * emit normal constants, else emit a load from the rgctx.
 */
MonoInst *mini_emit_get_rgctx_klass(MonoCompile *cfg, int context_used,
                                    MonoClass *klass,
                                    MonoRgctxInfoType rgctx_type) {
  if (!context_used) {
    MonoInst *ins;
  ...
  MonoJumpInfoRgctxEntry *entry = mono_patch_info_rgctx_entry_new(
      cfg->mempool, cfg->method, context_used_is_mrgctx(cfg, context_used),
      MONO_PATCH_INFO_CLASS, klass, rgctx_type);
  MonoInst *rgctx = emit_get_rgctx(cfg, context_used);

  return emit_rgctx_fetch(cfg, rgctx, entry);
}

static MonoInst *emit_get_rgctx_sig(MonoCompile *cfg, int context_used,
                                    MonoMethodSignature *sig,
                                    MonoRgctxInfoType rgctx_type) {
  MonoJumpInfoRgctxEntry *entry = mono_patch_info_rgctx_entry_new(
      cfg->mempool, cfg->method, context_used_is_mrgctx(cfg, context_used),
      MONO_PATCH_INFO_SIGNATURE, sig, rgctx_type);
  MonoInst *rgctx = emit_get_rgctx(cfg, context_used);

  return emit_rgctx_fetch(cfg, rgctx, entry);
}

static MonoInst *emit_get_rgctx_gsharedvt_call(MonoCompile *cfg,
                                               int context_used,
                                               MonoMethodSignature *sig,
                                               MonoMethod *cmethod,
                                               MonoRgctxInfoType rgctx_type) {
  ...
  call_info = (MonoJumpInfoGSharedVtCall *)mono_mempool_alloc0(
      cfg->mempool, sizeof(MonoJumpInfoGSharedVtCall));
  call_info->sig = sig;
  call_info->method = cmethod;

  entry = mono_patch_info_rgctx_entry_new(
      cfg->mempool, cfg->method, context_used_is_mrgctx(cfg, context_used),
      MONO_PATCH_INFO_GSHAREDVT_CALL, call_info, rgctx_type);
 ...
 */
static MonoInst *emit_get_rgctx_virt_method(MonoCompile *cfg, int context_used,
                                            MonoClass *klass,
                                            MonoMethod *virt_method,
                                            MonoRgctxInfoType rgctx_type) {
  MonoJumpInfoVirtMethod *info;
  MonoJumpInfoRgctxEntry *entry;
  MonoInst *rgctx;

  info = (MonoJumpInfoVirtMethod *)mono_mempool_alloc0(
      cfg->mempool, sizeof(MonoJumpInfoVirtMethod));
  info->klass = klass;
  info->method = virt_method;

  entry = mono_patch_info_rgctx_entry_new(
      cfg->mempool, cfg->method, context_used_is_mrgctx(cfg, context_used),
      MONO_PATCH_INFO_VIRT_METHOD, info, rgctx_type);
  rgctx = emit_get_rgctx(cfg, context_used);
...
emit_get_rgctx_gsharedvt_method(MonoCompile *cfg, int context_used,
  ...
  entry = mono_patch_info_rgctx_entry_new(
      cfg->mempool, cfg->method, context_used_is_mrgctx(cfg, context_used),
      MONO_PATCH_INFO_GSHAREDVT_METHOD, info,
 ...
 *
 *   Emit IR to load the property RGCTX_TYPE of CMETHOD. If context_used is 0,
 * emit normal constants, else emit a load from the rgctx.
 */
static MonoInst *emit_get_rgctx_method(MonoCompile *cfg, int context_used,
                                       MonoMethod *cmethod,
  ...
  if (!context_used) {
    ...
    MonoJumpInfoRgctxEntry *entry = mono_patch_info_rgctx_entry_new(
        cfg->mempool, cfg->method, context_used_is_mrgctx(cfg, context_used),
        MONO_PATCH_INFO_METHODCONST, cmethod, rgctx_type);
    MonoInst *rgctx = emit_get_rgctx(cfg, context_used);
...
static MonoInst *emit_get_rgctx_field(MonoCompile *cfg, int context_used,
                                      MonoClassField *field,
                                      MonoRgctxInfoType rgctx_type) {
  MonoJumpInfoRgctxEntry *entry = mono_patch_info_rgctx_entry_new(
      cfg->mempool, cfg->method, context_used_is_mrgctx(cfg, context_used),
      MONO_PATCH_INFO_FIELD, field, rgctx_type);
  MonoInst *rgctx = emit_get_rgctx(cfg, context_used);
...
static int get_gsharedvt_info_slot(MonoCompile *cfg, gpointer data,
                                   MonoRgctxInfoType rgctx_type) {
  ...
  for (i = 0; i < info->num_entries; ++i) {
    ...
    if (otemplate->info_type == rgctx_type && otemplate->data == data &&
        rgctx_type != MONO_RGCTX_INFO_LOCAL_OFFSET)
      return i;
  }
  ...
  if (info->num_entries == info->count_entries) {
    ...
    new_entries = (MonoRuntimeGenericContextInfoTemplate *)mono_mempool_alloc0(
        cfg->mempool,
        sizeof(MonoRuntimeGenericContextInfoTemplate) * new_count_entries);

    memcpy(new_entries, info->entries,
           sizeof(MonoRuntimeGenericContextInfoTemplate) * info->count_entries);
 ...
 *
 *   This is similar to emit_get_rgctx_.., but loads the data from the gsharedvt
 * info var instead of calling an rgctx fetch trampoline.
 */
static MonoInst *emit_get_gsharedvt_info(MonoCompile *cfg, gpointer data,
                                         MonoRgctxInfoType rgctx_type) {
  MonoInst *ins;
  int idx, dreg;
  ...
  dreg = alloc_preg(cfg);
  EMIT_NEW_LOAD_MEMBASE(
      cfg, ins, OP_LOAD_MEMBASE, dreg, cfg->gsharedvt_info_var->dreg,
      MONO_STRUCT_OFFSET(MonoGSharedVtMethodRuntimeInfo, entries) +
          (idx * TARGET_SIZEOF_VOID_P));

  return ins;
}

MonoInst *mini_emit_get_gsharedvt_info_klass(MonoCompile *cfg, MonoClass *klass,
                                             MonoRgctxInfoType rgctx_type) {
 ...
 */
static void emit_class_init(MonoCompile *cfg, MonoClass *klass) {
  MonoInst *vtable_arg;
  int context_used;
  ...
  if (context_used) {
    vtable_arg = mini_emit_get_rgctx_klass(cfg, context_used, klass,
  ...
  if (!COMPILE_LLVM(cfg) && cfg->backend->have_op_generic_class_init) {
    ...
    /*
     * Using an opcode instead of emitting IR here allows the hiding of the call
     * inside the opcode, so this doesn't have to clobber any regs and it
     * doesn't break basic blocks.
     */
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, inited_reg,
                                  vtable_arg->dreg,
...
static void emit_seq_point(MonoCompile *cfg, MonoMethod *method, guint8 *ip,
                           gboolean intr_loc, gboolean nonempty_stack) {
  MonoInst *ins;
...
void mini_save_cast_details(MonoCompile *cfg, MonoClass *klass, int obj_reg,
                            gboolean null_check) {
  if (mini_debug_options.better_cast_details) {
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE(cfg, vtable_reg, obj_reg,
                               MONO_STRUCT_OFFSET(MonoObject, vtable));
    MONO_EMIT_NEW_LOAD_MEMBASE(cfg, klass_reg, vtable_reg,
                               MONO_STRUCT_OFFSET(MonoVTable, klass));

    MONO_EMIT_NEW_STORE_MEMBASE(
        cfg, OP_STORE_MEMBASE_REG, tls_get->dreg,
        MONO_STRUCT_OFFSET(MonoJitTlsData, class_cast_from), klass_reg);

    MonoInst *class_ins = mini_emit_get_rgctx_klass(
        cfg, mini_class_check_context_used(cfg, klass), klass,
        MONO_RGCTX_INFO_KLASS);
    MONO_EMIT_NEW_STORE_MEMBASE(
        cfg, OP_STORE_MEMBASE_REG, tls_get->dreg,
        MONO_STRUCT_OFFSET(MonoJitTlsData, class_cast_to), class_ins->dreg);
...
void mini_reset_cast_details(MonoCompile *cfg) {
  ...
  if (mini_debug_options.better_cast_details) {
    ...
    MONO_EMIT_NEW_STORE_MEMBASE_IMM(
        cfg, OP_STORE_MEMBASE_IMM, tls_get->dreg,
        MONO_STRUCT_OFFSET(MonoJitTlsData, class_cast_from), 0);
  }
}
...
static void mini_emit_check_array_type(MonoCompile *cfg, MonoInst *obj,
  ...
  MONO_EMIT_NEW_LOAD_MEMBASE_FAULT(cfg, vtable_reg, obj->dreg,
                                   MONO_STRUCT_OFFSET(MonoObject, vtable));
  ...
  if (cfg->opt & MONO_OPT_SHARED) {
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE(cfg, class_reg, vtable_reg,
                               MONO_STRUCT_OFFSET(MonoVTable, klass));
    ...
    vtable_ins = mini_emit_get_rgctx_klass(cfg, context_used, array_class,
                                           MONO_RGCTX_INFO_VTABLE);
    ...
    if (cfg->compile_aot) {
      ...
      if (!(vtable = mono_class_vtable_checked(cfg->domain, array_class,
                                               cfg->error))) {
        mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);
        return;
      }
      ...
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, vtable_reg,
                              (gssize)vtable);
    }
  }
 ...
 */
static MonoInst *handle_unbox_nullable(MonoCompile *cfg, MonoInst *val,
                                       MonoClass *klass, int context_used) {
  MonoMethod *method;
  ...
  if (context_used) {
    ...
    if (cfg->llvm_only) {
      ...
      cfg->signatures =
          g_slist_prepend_mempool(cfg->mempool, cfg->signatures,
                                  mono_method_signature_internal(method));
      return mini_emit_llvmonly_calli(
          cfg, mono_method_signature_internal(method), &val, addr);
    } else {
      addr = emit_get_rgctx_method(cfg, context_used, method,
                                   MONO_RGCTX_INFO_GENERIC_METHOD_CODE);
      rgctx = emit_get_rgctx(cfg, context_used);

      return mini_emit_calli(cfg, mono_method_signature_internal(method), &val,
                             addr, NULL, rgctx);
    }
  } else {
    ...
    if (pass_vtable) {
      MonoVTable *vtable =
          mono_class_vtable_checked(cfg->domain, method->klass, cfg->error);

      mono_error_assert_ok(cfg->error);
      EMIT_NEW_VTABLECONST(cfg, rgctx_arg, vtable);
    }

    return mini_emit_method_call_full(cfg, method, NULL, FALSE, &val, NULL,
                                      NULL, rgctx_arg);
  }
}

static MonoInst *handle_unbox(MonoCompile *cfg, MonoClass *klass, MonoInst **sp,
                              int context_used) {
  MonoInst *add;
  int obj_reg;
  ...
  obj_reg = sp[0]->dreg;
  MONO_EMIT_NEW_LOAD_MEMBASE_FAULT(cfg, vtable_reg, obj_reg,
                                   MONO_STRUCT_OFFSET(MonoObject, vtable));
  MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADU1_MEMBASE, rank_reg, vtable_reg,
                                MONO_STRUCT_OFFSET(MonoVTable, rank));
  ...
  MONO_EMIT_NEW_LOAD_MEMBASE(cfg, klass_reg, vtable_reg,
                             MONO_STRUCT_OFFSET(MonoVTable, klass));
  MONO_EMIT_NEW_LOAD_MEMBASE(cfg, eclass_reg, klass_reg,
  ...
  if (context_used) {
    ...
    element_class = mini_emit_get_rgctx_klass(cfg, context_used, klass,
                                              MONO_RGCTX_INFO_ELEMENT_KLASS);

    MONO_EMIT_NEW_BIALU(cfg, OP_COMPARE, -1, eclass_reg, element_class->dreg);
    MONO_EMIT_NEW_COND_EXC(cfg, NE_UN, "InvalidCastException");
  } else {
    mini_save_cast_details(cfg, m_class_get_element_class(klass), obj_reg,
                           FALSE);
    mini_emit_class_check(cfg, eclass_reg, m_class_get_element_class(klass));
    mini_reset_cast_details(cfg);
  }

  NEW_BIALU_IMM(cfg, add, OP_ADD_IMM, alloc_dreg(cfg, STACK_MP), obj_reg,
...
static MonoInst *handle_unbox_gsharedvt(MonoCompile *cfg, MonoClass *klass,
                                        MonoInst *obj) {
  ...
  klass_inst =
      mini_emit_get_gsharedvt_info_klass(cfg, klass, MONO_RGCTX_INFO_KLASS);
  ...
  NEW_BBLOCK(cfg, end_bb);
  is_ref = mini_emit_get_gsharedvt_info_klass(cfg, klass,
                                              MONO_RGCTX_INFO_CLASS_BOX_TYPE);
  MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, is_ref->dreg,
  ...
  /* This will contain either the address of the unboxed vtype, or an address of
   * the temporary where the ref is stored */
  addr_reg = alloc_dreg(cfg, STACK_MP);

  /* Non-ref case */
  /* UNBOX */
  NEW_BIALU_IMM(cfg, addr, OP_ADD_IMM, addr_reg, obj->dreg,
                MONO_ABI_SIZEOF(MonoObject));
  ...
  addr->dreg = addr_reg;
  MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, addr->dreg, 0,
                              obj->dreg);
  MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_BR, end_bb);
  ...
  {
    MonoInst *addr = mini_emit_get_gsharedvt_info_klass(
        cfg, klass, MONO_RGCTX_INFO_NULLABLE_CLASS_UNBOX);
    MonoInst *unbox_call;
    MonoMethodSignature *unbox_sig;

    unbox_sig = (MonoMethodSignature *)mono_mempool_alloc0(
        cfg->mempool, MONO_SIZEOF_METHOD_SIGNATURE + (1 * sizeof(MonoType *)));
    ...
    EMIT_NEW_VARLOADA_VREG(cfg, addr, unbox_call->dreg,
  ...
  /* LDOBJ */
  EMIT_NEW_LOAD_MEMBASE_TYPE(cfg, ins, m_class_get_byval_arg(klass), addr_reg,
                             0);
 ...
 */
static MonoInst *handle_alloc(MonoCompile *cfg, MonoClass *klass,
                              gboolean for_box, int context_used) {
  MonoInst *iargs[2];
  ...
  if (mono_class_get_flags(klass) & TYPE_ATTRIBUTE_ABSTRACT) {
    ...
    mono_cfg_set_exception(cfg, MONO_EXCEPTION_MONO_ERROR);
    mono_error_set_member_access(
        cfg->error, "Cannot create an abstract class: %s", full_name);
    g_free(full_name);
    return NULL;
  }
  ...
  if (context_used) {
    ...
    MonoMethod *managed_alloc =
        mono_gc_get_managed_allocator(klass, for_box, known_instance_size);
    ...
    if (managed_alloc && !(cfg->opt & MONO_OPT_SHARED)) {
      if (known_instance_size) {
        ...
        if (size < MONO_ABI_SIZEOF(MonoObject))
          g_error("Invalid size %d for class %s", size,
  ...
  if (cfg->opt & MONO_OPT_SHARED) {
    ...
    alloc_ftn = MONO_JIT_ICALL_ves_icall_object_new;
  } else if (cfg->compile_aot && cfg->cbb->out_of_line &&
             m_class_get_type_token(klass) &&
             m_class_get_image(klass) == mono_defaults.corlib &&
             !mono_class_is_ginst(klass)) {
    /* This happens often in argument checking code, eg. throw new
     * FooException... */
    /* Avoid relocations and save some space by calling a helper function
     * specialized to mscorlib */
    EMIT_NEW_ICONST(cfg, iargs[0],
    ...
    MonoMethod *managed_alloc =
        mono_gc_get_managed_allocator(klass, for_box, TRUE);
...
MonoInst *mini_emit_box(MonoCompile *cfg, MonoInst *val, MonoClass *klass,
  ...
  if (G_UNLIKELY(m_class_is_byreflike(klass))) {
    mono_error_set_bad_image(cfg->error, m_class_get_image(cfg->method->klass),
  ...
  if (mono_class_is_nullable(klass)) {
    ...
    if (context_used) {
      if (cfg->llvm_only && cfg->gsharedvt) {
        ...
        MonoInst *addr = emit_get_rgctx_method(
            cfg, context_used, method, MONO_RGCTX_INFO_GENERIC_METHOD_CODE);
        MonoInst *rgctx = emit_get_rgctx(cfg, context_used);

        return mini_emit_calli(cfg, mono_method_signature_internal(method),
                               &val, addr, NULL, rgctx);
      }
    } else {
      gboolean pass_vtable, pass_mrgctx;
  ...
  if (mini_is_gsharedvt_klass(klass)) {
    ...
    if (!alloc)
      return NULL;
    EMIT_NEW_STORE_MEMBASE_TYPE(cfg, ins, m_class_get_byval_arg(klass),
                                alloc->dreg, MONO_ABI_SIZEOF(MonoObject),
                                val->dreg);
    ...
    if (!src_var)
      src_var = mono_compile_create_var_for_vreg(
          cfg, m_class_get_byval_arg(klass), OP_LOCAL, val->dreg);
    ...
    {
      MonoInst *addr = mini_emit_get_gsharedvt_info_klass(
          cfg, klass, MONO_RGCTX_INFO_NULLABLE_CLASS_BOX);
      MonoInst *box_call;
      MonoMethodSignature *box_sig;

      /*
       * klass is Nullable<T>, need to call Nullable<T>.Box () using a gsharedvt
       * signature, but we cannot construct that method at JIT time, so have to
       * do things by hand.
       */
      box_sig = (MonoMethodSignature *)mono_mempool_alloc0(
          cfg->mempool,
          MONO_SIZEOF_METHOD_SIGNATURE + (1 * sizeof(MonoType *)));
...
static gboolean method_needs_stack_walk(MonoCompile *cfg, MonoMethod *cmethod) {
...
G_GNUC_UNUSED MonoInst *mini_handle_enum_has_flag(MonoCompile *cfg,
                                                  MonoClass *klass,
                                                  MonoInst *enum_this,
                                                  int enum_val_reg,
                                                  MonoInst *enum_flag) {
...

  {
    ...
    EMIT_NEW_BIALU(cfg, and_, is_i4 ? OP_IAND : OP_LAND, and_reg, enum_reg,
                   enum_flag->dreg);
    EMIT_NEW_BIALU(cfg, cmp, is_i4 ? OP_ICOMPARE : OP_LCOMPARE, -1, and_reg,
                   enum_flag->dreg);
...
static MonoInst *emit_get_rgctx_dele_tramp(MonoCompile *cfg, int context_used,
                                           MonoClass *klass,
                                           MonoMethod *virt_method,
                                           gboolean _virtual,
                                           MonoRgctxInfoType rgctx_type) {
  ...
  info = (MonoDelegateClassMethodPair *)mono_mempool_alloc0(
      cfg->mempool, sizeof(MonoDelegateClassMethodPair));
  info->klass = klass;
  info->method = virt_method;
  ...
  entry = mono_patch_info_rgctx_entry_new(
      cfg->mempool, cfg->method, context_used_is_mrgctx(cfg, context_used),
      MONO_PATCH_INFO_DELEGATE_TRAMPOLINE, info, rgctx_type);
  rgctx = emit_get_rgctx(cfg, context_used);
...
handle_delegate_ctor(MonoCompile *cfg, MonoClass *klass, MonoInst *target,
                     MonoMethod *method, int target_method_context_used,
                     int invoke_context_used, gboolean virtual_) {
  MonoInst *ptr;
  int dreg;
  gpointer trampoline;
  ...
  if (virtual_ && !cfg->llvm_only) {
    ...
    // FIXME verify & fix any issue with removing invoke_context_used
    // restriction
    if (invoke_context_used || !mono_get_delegate_virtual_invoke_impl(
                                   mono_method_signature_internal(invoke),
                                   target_method_context_used ? NULL : method))
      return NULL;
  }
  ...
  if (!MONO_INS_IS_PCONST_NULL(target)) {
    ...
    if (!mini_debug_options.weak_memory_model)
      mini_emit_memory_barrier(cfg, MONO_MEMORY_BARRIER_REL);
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, obj->dreg,
                                MONO_STRUCT_OFFSET(MonoDelegate, target),
                                target->dreg);
    if (cfg->gen_write_barriers) {
      dreg = alloc_preg(cfg);
      EMIT_NEW_BIALU_IMM(cfg, ptr, OP_PADD_IMM, dreg, obj->dreg,
                         MONO_STRUCT_OFFSET(MonoDelegate, target));
  ...
  if (!(target_method_context_used || invoke_context_used) || cfg->llvm_only) {
    // If compiling with gsharing enabled, it's faster to load method the
    // delegate trampoline info than to use a rgctx slot
    MonoInst *method_ins = emit_get_rgctx_method(
        cfg, target_method_context_used, method, MONO_RGCTX_INFO_METHOD);
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, obj->dreg,
                                MONO_STRUCT_OFFSET(MonoDelegate, method),
                                method_ins->dreg);
  }
  ...
  if (!method->dynamic && !(cfg->opt & MONO_OPT_SHARED)) {
    ...
    if (target_method_context_used) {
      code_slot_ins =
          emit_get_rgctx_method(cfg, target_method_context_used, method,
                                MONO_RGCTX_INFO_METHOD_DELEGATE_CODE);
    } else {
      domain = mono_domain_get();
      mono_domain_lock(domain);
      if (!domain_jit_info(domain)->method_code_hash)
        domain_jit_info(domain)->method_code_hash =
            g_hash_table_new(NULL, NULL);
      code_slot = (guint8 **)g_hash_table_lookup(
          domain_jit_info(domain)->method_code_hash, method);
      if (!code_slot) {
        code_slot = (guint8 **)mono_domain_alloc0(domain, sizeof(gpointer));
        g_hash_table_insert(domain_jit_info(domain)->method_code_hash, method,
                            code_slot);
      }
      mono_domain_unlock(domain);

      code_slot_ins = mini_emit_runtime_constant(
          cfg, MONO_PATCH_INFO_METHOD_CODE_SLOT, method);
    }
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, obj->dreg,
                                MONO_STRUCT_OFFSET(MonoDelegate, method_code),
                                code_slot_ins->dreg);
  }

  if (cfg->llvm_only) {
    if (virtual_) {
      MonoInst *args[] = {obj, target,
                          emit_get_rgctx_method(cfg, target_method_context_used,
                                                method,
  ...
  if (target_method_context_used || invoke_context_used) {
    tramp_ins = emit_get_rgctx_dele_tramp(
        cfg, target_method_context_used | invoke_context_used, klass, method,
        virtual_, MONO_RGCTX_INFO_DELEGATE_TRAMP_INFO);

    // This is emited as a contant store for the non-shared case.
    // We copy from the delegate trampoline info as it's faster than a rgctx
    // fetch
    dreg = alloc_preg(cfg);
    MONO_EMIT_NEW_LOAD_MEMBASE(
        cfg, dreg, tramp_ins->dreg,
        MONO_STRUCT_OFFSET(MonoDelegateTrampInfo, method));
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, obj->dreg,
                                MONO_STRUCT_OFFSET(MonoDelegate, method), dreg);
  } else if (cfg->compile_aot) {
    MonoDelegateClassMethodPair *del_tramp;

    del_tramp = (MonoDelegateClassMethodPair *)mono_mempool_alloc0(
        cfg->mempool, sizeof(MonoDelegateClassMethodPair));
    del_tramp->klass = klass;
    del_tramp->method = method;
    del_tramp->is_virtual = virtual_;
    EMIT_NEW_AOTCONST(cfg, tramp_ins, MONO_PATCH_INFO_DELEGATE_TRAMPOLINE,
                      del_tramp);
  } else {
    if (virtual_)
      trampoline =
          mono_create_delegate_virtual_trampoline(cfg->domain, klass, method);
    else
      trampoline =
          mono_create_delegate_trampoline_info(cfg->domain, klass, method);
  ...
  if (virtual_) {
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, obj->dreg,
                                MONO_STRUCT_OFFSET(MonoDelegate, invoke_impl),
                                tramp_ins->dreg);
  } else {
    dreg = alloc_preg(cfg);
    MONO_EMIT_NEW_LOAD_MEMBASE(
        cfg, dreg, tramp_ins->dreg,
        MONO_STRUCT_OFFSET(MonoDelegateTrampInfo, invoke_impl));
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, obj->dreg,
                                MONO_STRUCT_OFFSET(MonoDelegate, invoke_impl),
                                dreg);
    ...
    MONO_EMIT_NEW_LOAD_MEMBASE(
        cfg, dreg, tramp_ins->dreg,
        MONO_STRUCT_OFFSET(MonoDelegateTrampInfo, method_ptr));
    MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG, obj->dreg,
  ...
  MONO_EMIT_NEW_STORE_MEMBASE(
      cfg, OP_STOREI1_MEMBASE_REG, obj->dreg,
      MONO_STRUCT_OFFSET(MonoDelegate, method_is_virtual), dreg);

  /* All the checks which are in mono_delegate_ctor () are done by the delegate
   * trampoline */
 ...
 * Return the instruction representing the call. Set the cfg exception on
 * failure.
 */
static MonoInst *handle_constrained_gsharedvt_call(
    MonoCompile *cfg, MonoMethod *cmethod, MonoMethodSignature *fsig,
    MonoInst **sp, MonoClass *constrained_class, gboolean *ref_emit_widen) {
  MonoInst *ins = NULL;
  gboolean emit_widen = *ref_emit_widen;
  gboolean supported;

  /*
   * Constrained calls need to behave differently at runtime dependending on
   * whenever the receiver is instantiated as ref type or as a vtype. This is
   * hard to do with the current call code, since we would have to emit a branch
   * and two different calls. So instead, we pack the arguments into an array,
   * and do the rest of the work in in an icall.
   */
  supported = ((cmethod->klass == mono_defaults.object_class) ||
               mono_class_is_interface(cmethod->klass) ||
               (!m_class_is_valuetype(cmethod->klass) &&
                m_class_get_image(cmethod->klass) != mono_defaults.corlib));
  if (supported)
    supported =
        (MONO_TYPE_IS_VOID(fsig->ret) || MONO_TYPE_IS_PRIMITIVE(fsig->ret) ||
         MONO_TYPE_IS_REFERENCE(fsig->ret) || MONO_TYPE_ISSTRUCT(fsig->ret) ||
         m_class_is_enumtype(mono_class_from_mono_type_internal(fsig->ret)) ||
         mini_is_gsharedvt_type(fsig->ret));
  if (supported) {
    if (fsig->param_count == 0 || (!fsig->hasthis && fsig->param_count == 1)) {
      supported = TRUE;
    } else {
      /* Allow scalar parameters and a gsharedvt first parameter */
      supported = MONO_TYPE_IS_PRIMITIVE(fsig->params[0]) ||
                  MONO_TYPE_IS_REFERENCE(fsig->params[0]) ||
                  fsig->params[0]->byref ||
                  mini_is_gsharedvt_type(fsig->params[0]);
      if (supported) {
        for (int i = 1; i < fsig->param_count; ++i) {
          if (!(fsig->params[i]->byref ||
                MONO_TYPE_IS_PRIMITIVE(fsig->params[i]) ||
                MONO_TYPE_IS_REFERENCE(fsig->params[i]) ||
                MONO_TYPE_ISSTRUCT(fsig->params[i])))
            supported = FALSE;
        }
      }
    }
  ...
  if (supported) {
     ...
     * plus some simple interface calls enough to support
     * AsyncTaskMethodBuilder.
     */

    args[0] = sp[0];
    args[1] =
        emit_get_rgctx_method(cfg, mono_method_check_context_used(cmethod),
                              cmethod, MONO_RGCTX_INFO_METHOD);
    args[2] = mini_emit_get_rgctx_klass(
        cfg, mono_class_check_context_used(constrained_class),
        constrained_class, MONO_RGCTX_INFO_KLASS);

    /* !fsig->hasthis is for the wrapper for the Object.GetType () icall */
    if (fsig->hasthis && fsig->param_count) {
      /* Call mono_gsharedvt_constrained_call (gpointer mp, MonoMethod *cmethod,
       * MonoClass *klass, gboolean deref_arg, gpointer *args) */
      /* Pass the arguments using a localloc-ed array using the format expected
       * by runtime_invoke () */
      ...
      if (mini_is_gsharedvt_type(fsig->params[0])) {
        int deref_arg_reg;
        ins = mini_emit_get_gsharedvt_info_klass(
            cfg, mono_class_from_mono_type_internal(fsig->params[0]),
            MONO_RGCTX_INFO_CLASS_BOX_TYPE);
        deref_arg_reg = alloc_preg(cfg);
        /* deref_arg = BOX_TYPE != MONO_GSHAREDVT_BOX_TYPE_VTYPE */
        EMIT_NEW_BIALU_IMM(cfg, args[3], OP_ISUB_IMM, deref_arg_reg, ins->dreg,
                           1);
      } else {
      ...
      for (int i = 0; i < fsig->param_count; ++i) {
        ...
        if (mini_is_gsharedvt_type(fsig->params[i]) ||
          ...
          addr_reg = ins->dreg;
          EMIT_NEW_STORE_MEMBASE(cfg, ins, OP_STORE_MEMBASE_REG, args[4]->dreg,
                                 i * sizeof(target_mgreg_t), addr_reg);
        } else {
          EMIT_NEW_STORE_MEMBASE(cfg, ins, OP_STORE_MEMBASE_REG, args[4]->dreg,
                                 i * sizeof(target_mgreg_t), sp[i + 1]->dreg);
        }
      }
    } else {
      EMIT_NEW_ICONST(cfg, args[3], 0);
    ...
    if (mini_is_gsharedvt_type(fsig->ret)) {
      ins = handle_unbox_gsharedvt(
          cfg, mono_class_from_mono_type_internal(fsig->ret), ins);
    } else if (MONO_TYPE_IS_PRIMITIVE(fsig->ret) ||
               MONO_TYPE_ISSTRUCT(fsig->ret) ||
               m_class_is_enumtype(
                   mono_class_from_mono_type_internal(fsig->ret))) {
      MonoInst *add;

      /* Unbox */
      NEW_BIALU_IMM(cfg, add, OP_ADD_IMM, alloc_dreg(cfg, STACK_MP), ins->dreg,
                    MONO_ABI_SIZEOF(MonoObject));
      MONO_ADD_INS(cfg->cbb, add);
      /* Load value */
...
static gboolean method_does_not_return(MonoMethod *method) {
  // FIXME: Under netcore, these are decorated with the [DoesNotReturn]
  // attribute
  return m_class_get_image(method->klass) == mono_defaults.corlib &&
         !strcmp(m_class_get_name(method->klass), "ThrowHelper") &&
         strstr(method->name, "Throw") == method->name && !method->is_inflated;
}

static int inline_limit, llvm_jit_inline_limit, llvm_aot_inline_limit;
static gboolean inline_limit_inited;

static gboolean mono_method_check_inlining(MonoCompile *cfg,
                                           MonoMethod *method) {
   ...
   */
  if (cfg->gshared && m_class_has_cctor(method->klass) &&
      mini_class_check_context_used(cfg, method->klass))
    return FALSE;
  ...
  if (!(cfg->opt & MONO_OPT_SHARED)) {
    ...
    if (mono_class_needs_cctor_run(method->klass, NULL) &&
        !mono_class_is_before_field_init(method->klass))
      return FALSE;
  }
...
MonoInst *mini_emit_ldelema_1_ins(MonoCompile *cfg, MonoClass *klass,
                                  MonoInst *arr, MonoInst *index,
                                  gboolean bcheck) {
  MonoInst *ins;
  guint32 size;
  ...
  if (size == 1 || size == 2 || size == 4 || size == 8) {
    ...
    EMIT_NEW_X86_LEA(cfg, ins, array_reg, index2_reg, fast_log2[size],
                     MONO_STRUCT_OFFSET(MonoArray, vector));
    ins->klass = klass;
    ins->type = STACK_MP;
  ...
  if (size == -1) {
    ...
    rgctx_ins = mini_emit_get_gsharedvt_info_klass(
        cfg, klass, MONO_RGCTX_INFO_ARRAY_ELEMENT_SIZE);
  ...
  NEW_BIALU_IMM(cfg, ins, OP_PADD_IMM, add_reg, add_reg,
                MONO_STRUCT_OFFSET(MonoArray, vector));
  ins->klass = klass;
  ins->type = STACK_MP;
  MONO_ADD_INS(cfg->cbb, ins);
...
static MonoInst *mini_emit_ldelema_2_ins(MonoCompile *cfg, MonoClass *klass,
                                         MonoInst *arr, MonoInst *index_ins1,
                                         MonoInst *index_ins2) {
...
#else
  // FIXME: Do we need to do something here for i8 indexes, like in
  // ldelema_1_ins ?
#endif

  /* range checking */
  MONO_EMIT_NEW_LOAD_MEMBASE(cfg, bounds_reg, arr->dreg,
                             MONO_STRUCT_OFFSET(MonoArray, bounds));

  MONO_EMIT_NEW_LOAD_MEMBASE_OP(
      cfg, OP_LOADI4_MEMBASE, low1_reg, bounds_reg,
      MONO_STRUCT_OFFSET(MonoArrayBounds, lower_bound));
  MONO_EMIT_NEW_BIALU(cfg, OP_PSUB, realidx1_reg, index1, low1_reg);
  MONO_EMIT_NEW_LOAD_MEMBASE_OP(cfg, OP_LOADI4_MEMBASE, high1_reg, bounds_reg,
                                MONO_STRUCT_OFFSET(MonoArrayBounds, length));
  MONO_EMIT_NEW_BIALU(cfg, OP_COMPARE, -1, high1_reg, realidx1_reg);
  MONO_EMIT_NEW_COND_EXC(cfg, LE_UN, "IndexOutOfRangeException");

  MONO_EMIT_NEW_LOAD_MEMBASE_OP(
      cfg, OP_LOADI4_MEMBASE, low2_reg, bounds_reg,
      sizeof(MonoArrayBounds) +
          MONO_STRUCT_OFFSET(MonoArrayBounds, lower_bound));
  MONO_EMIT_NEW_BIALU(cfg, OP_PSUB, realidx2_reg, index2, low2_reg);
  MONO_EMIT_NEW_LOAD_MEMBASE_OP(
      cfg, OP_LOADI4_MEMBASE, high2_reg, bounds_reg,
      sizeof(MonoArrayBounds) + MONO_STRUCT_OFFSET(MonoArrayBounds, length));
...
static MonoInst *mini_emit_ldelema_ins(MonoCompile *cfg, MonoMethod *cmethod,
                                       MonoInst **sp, guchar *ip,
                                       gboolean is_set) {
  int rank;
  MonoInst *addr;
  ...
  rank =
      mono_method_signature_internal(cmethod)->param_count - (is_set ? 1 : 0);

  if (rank == 1)
    return mini_emit_ldelema_1_ins(cfg, eclass, sp[0], sp[1], TRUE);

  /* emit_ldelema_2 depends on OP_LMUL */
  if (!cfg->backend->emulate_mul_div && rank == 2 &&
...
static gboolean mini_class_is_reference(MonoClass *klass) {
  return mini_type_is_reference(m_class_get_byval_arg(klass));
}

MonoInst *mini_emit_array_store(MonoCompile *cfg, MonoClass *klass,
                                MonoInst **sp, gboolean safety_checks) {
  if (safety_checks && mini_class_is_reference(klass) &&
      !(MONO_INS_IS_PCONST_NULL(sp[2]))) {
    MonoClass *obj_array =
        mono_array_class_get_cached(mono_defaults.object_class);
    ...
    if (mini_is_gsharedvt_variable_klass(klass)) {
      ...
      if (safety_checks)
        MONO_EMIT_BOUNDS_CHECK(cfg, array_reg, MonoArray, max_length,
                               index_reg);
      EMIT_NEW_STORE_MEMBASE_TYPE(cfg, ins, m_class_get_byval_arg(klass),
                                  array_reg, offset, sp[2]->dreg);
    } else {
      MonoInst *addr =
          mini_emit_ldelema_1_ins(cfg, klass, sp[0], sp[1], safety_checks);
      if (!mini_debug_options.weak_memory_model &&
 ...
 * redirection.
 */
inline static MonoInst *mini_redirect_call(MonoCompile *cfg, MonoMethod *method,
  ...
  if (method->klass == mono_defaults.string_class) {
    ...
    if (strcmp(method->name, "FastAllocateString") == 0 &&
      ...
      mono_error_assert_ok(
          cfg->error); /*Should not fail since it System.String*/
#ifndef MONO_CROSS_COMPILE
      managed_alloc =
          mono_gc_get_managed_allocator(method->klass, FALSE, FALSE);
#endif
      if (!managed_alloc)
        return NULL;
      EMIT_NEW_VTABLECONST(cfg, iargs[0], vtable);
...
static void mono_save_args(MonoCompile *cfg, MonoMethodSignature *sig,
  ...
  for (i = 0; i < sig->param_count + sig->hasthis; ++i) {
    MonoType *argtype = (sig->hasthis && (i == 0))
                            ? type_from_stack_type(*sp)
                            : sig->params[i - sig->hasthis];
...
static void emit_init_rvar(MonoCompile *cfg, int dreg, MonoType *rtype) {
  ...
  } else if (((t == MONO_TYPE_VAR) || (t == MONO_TYPE_MVAR)) &&
...
static void emit_dummy_init_rvar(MonoCompile *cfg, int dreg, MonoType *rtype) {
  int t;
 ...
 */
static void emit_init_local(MonoCompile *cfg, int local, MonoType *type,
                            gboolean init) {
...
int mini_inline_method(MonoCompile *cfg, MonoMethod *cmethod,
                       MonoMethodSignature *fsig, MonoInst **sp, guchar *ip,
                       guint real_offset, gboolean inline_always) {
...
static int inline_method(MonoCompile *cfg, MonoMethod *cmethod,
  ...
  cfg->locals = (MonoInst **)mono_mempool_alloc0(
      cfg->mempool, cheader->num_locals * sizeof(MonoInst *));
  ...
  costs = mono_method_to_ir(cfg, cmethod, sbblock, ebblock, rvar, sp,
                            real_offset, virtual_);
  ...
  if ((costs >= 0 && costs < 60) || inline_always ||
    ...
    if ((ebblock->in_count == 1) && ebblock->in_bb[0]->out_count == 1) {
      ...
      if (prev->next_bb == ebblock) {
        ...
        cfg->cbb = prev;
        if ((prev_cbb->out_count == 1) &&
            (prev_cbb->out_bb[0]->in_count == 1) &&
            (prev_cbb->out_bb[0] == prev)) {
          mono_merge_basic_blocks(cfg, prev_cbb, prev);
          cfg->cbb = prev_cbb;
        }
      } else {
        /* There could be a bblock after 'prev', and making 'prev' the current
         * bb could cause problems */
        cfg->cbb = ebblock;
      }
    } else {
      /*
       * Its possible that the rvar is set in some prev bblock, but not in
       * others.
       * (#1835).
       */
      if (rvar) {
    ...
    cfg->headers_to_free =
        g_slist_prepend_mempool(cfg->mempool, cfg->headers_to_free, cheader);
    return costs + 1;
  } else {
    if (cfg->verbose_level > 2) {
 ...
 * Code is validated as we go (full verification is still better left to
 * metadata/verify.c). Complex operations are decomposed in simpler ones right
 * away. We need to let the arch-specific code peek and poke inside this process
 * somehow (except when the optimizations can take advantage of the full
 * semantic info of coarse opcodes). All the opcodes of the form opcode.s are
 * 'normalized' to opcode. MonoInst->opcode initially is the IL opcode or some
 * simplification of that (OP_LOAD, OP_STORE). The arch-specific code may
 * rearrange it to an arch-specific opcode with value bigger than OP_LAST. At
 * this point the IR can be handed over to an interpreter, a dumb code generator
 * or to the optimizing code generator that will translate it to SSA form.
 *
 * Profiling directed optimizations.
 * We may compile by default with few or no optimizations and instrument the
 * code or the user may indicate what methods to optimize the most either in a
 * config file or through repeated runs where the compiler applies offline the
 * optimizations to each method and then decides if it was worth it.
 */

#define CHECK_TYPE(ins)                                                        \
  if (!(ins)->type)                                                            \
  UNVERIFIED
#define CHECK_STACK(num)                                                       \
  if ((sp - stack_start) < (num))                                              \
  UNVERIFIED
#define CHECK_STACK_OVF()                                                      \
  if (((sp - stack_start) + 1) > header->max_stack)                            \
  UNVERIFIED
#define CHECK_ARG(num)                                                         \
  if ((unsigned)(num) >= (unsigned)num_args)                                   \
  UNVERIFIED
#define CHECK_LOCAL(num)                                                       \
  if ((unsigned)(num) >= (unsigned)header->num_locals)                         \
  UNVERIFIED
#define CHECK_OPSIZE(size)                                                     \
  if ((size) < 1 || ip + (size) > end)                                         \
  UNVERIFIED
#define CHECK_UNVERIFIABLE(cfg)                                                \
  if (cfg->unverifiable)                                                       \
  UNVERIFIED
...
static gboolean ip_in_bb(MonoCompile *cfg, MonoBasicBlock *bb,
                         const guint8 *ip) {
...
static int get_basic_blocks(MonoCompile *cfg, MonoMethodHeader *header,
                            guint real_offset, guchar *start, guchar *end,
                            guchar **pos) {
  guchar *ip = start;
  guchar *target;
  int i;
...
static MonoMethod *mini_get_method_allow_open(MonoMethod *m, guint32 token,
                                              MonoClass *klass,
  ...
  if (m->wrapper_type != MONO_WRAPPER_NONE) {
    ...
    if (context) {
      method =
          mono_class_inflate_generic_method_checked(method, context, error);
    }
  } else {
    method = mono_get_method_checked(m_class_get_image(m->klass), token, klass,
                                     context, error);
  }
...
static MonoMethod *mini_get_method(MonoCompile *cfg, MonoMethod *m,
                                   guint32 token, MonoClass *klass,
                                   MonoGenericContext *context) {
  ERROR_DECL(error);
  MonoMethod *method = mini_get_method_allow_open(m, token, klass, context,
                                                  cfg ? cfg->error : error);

  if (method && cfg && !cfg->gshared &&
      mono_class_is_open_constructed_type(
          m_class_get_byval_arg(method->klass))) {
    mono_error_set_bad_image(
        cfg->error, m_class_get_image(cfg->method->klass),
...
static MonoMethodSignature *mini_get_signature(MonoMethod *method,
                                               guint32 token,
                                               MonoGenericContext *context,
                                               MonoError *error) {
  MonoMethodSignature *fsig;
  ...
  if (method->wrapper_type != MONO_WRAPPER_NONE) {
    ...
    fsig = mono_metadata_parse_signature_checked(
        m_class_get_image(method->klass), token, error);
...
static MonoMethod *throw_exception(void) {
 ...
 */
static MonoMethod *get_original_method(MonoMethod *method) {
  if (method->wrapper_type == MONO_WRAPPER_NONE)
    return method;

  /* native code (which is like Critical) can call any managed method XXX FIXME
   * XXX to validate all usages */
...
static void ensure_method_is_allowed_to_access_field(MonoCompile *cfg,
                                                     MonoMethod *caller,
                                                     MonoClassField *field) {
  /* we can't get the coreclr security level on wrappers since they don't have
   * the attributes */
  MonoException *ex = mono_security_core_clr_is_field_access_allowed(
      get_original_method(caller), field);
  if (ex)
...
static void ensure_method_is_allowed_to_call_method(MonoCompile *cfg,
  ...
  /* we can't get the coreclr security level on wrappers since they don't have
   * the attributes */
  MonoException *ex = mono_security_core_clr_is_call_allowed(
      get_original_method(caller), callee);
  if (ex)
    emit_throw_exception(cfg, ex);
}

static guchar *il_read_op(guchar *ip, guchar *end, guchar first_byte,
...
// If ip is desired_il_op, return the next ip, else NULL.
{
  if (G_LIKELY(ip < end) && G_UNLIKELY(*ip == first_byte)) {
    ...
    return (G_LIKELY(size > 0) && G_UNLIKELY(il_op == desired_il_op))
               ? (ip + size)
               : NULL;
  }
  return NULL;
...
static guchar *il_read_op_and_token(guchar *ip, guchar *end, guchar first_byte,
                                    MonoOpcodeEnum desired_il_op,
                                    guint32 *token) {
...
static guchar *il_read_branch_and_target(guchar *ip, guchar *end,
                                         guchar first_byte,
                                         MonoOpcodeEnum desired_il_op, int size,
                                         guchar **target) {
  ip = il_read_op(ip, end, first_byte, desired_il_op);
  if (ip) {
    gint32 delta = 0;
...
#define il_read_brtrue(ip, end, target)                                        \
  (il_read_branch_and_target(ip, end, CEE_BRTRUE, MONO_CEE_BRTRUE, 4, target))
#define il_read_brtrue_s(ip, end, target)                                      \
  (il_read_branch_and_target(ip, end, CEE_BRTRUE_S, MONO_CEE_BRTRUE_S, 1,      \
                             target))
#define il_read_brfalse(ip, end, target)                                       \
  (il_read_branch_and_target(ip, end, CEE_BRFALSE, MONO_CEE_BRFALSE, 4, target))
#define il_read_brfalse_s(ip, end, target)                                     \
  (il_read_branch_and_target(ip, end, CEE_BRFALSE_S, MONO_CEE_BRFALSE_S, 1,    \
                             target))
#define il_read_dup(ip, end) (il_read_op(ip, end, CEE_DUP, MONO_CEE_DUP))
#define il_read_newobj(ip, end, token)                                         \
  (il_read_op_and_token(ip, end, CEE_NEW_OBJ, MONO_CEE_NEWOBJ, token))
#define il_read_ldtoken(ip, end, token)                                        \
  (il_read_op_and_token(ip, end, CEE_LDTOKEN, MONO_CEE_LDTOKEN, token))
#define il_read_call(ip, end, token)                                           \
  (il_read_op_and_token(ip, end, CEE_CALL, MONO_CEE_CALL, token))
#define il_read_callvirt(ip, end, token)                                       \
  (il_read_op_and_token(ip, end, CEE_CALLVIRT, MONO_CEE_CALLVIRT, token))
#define il_read_initobj(ip, end, token)                                        \
  (il_read_op_and_token(ip, end, CEE_PREFIX1, MONO_CEE_INITOBJ, token))
#define il_read_constrained(ip, end, token)                                    \
  (il_read_op_and_token(ip, end, CEE_PREFIX1, MONO_CEE_CONSTRAINED_, token))
#define il_read_unbox_any(ip, end, token)                                      \
  (il_read_op_and_token(ip, end, CEE_UNBOX_ANY, MONO_CEE_UNBOX_ANY, token))
 ...
 */
static const char *
initialize_array_data(MonoCompile *cfg, MonoMethod *method, gboolean aot,
                      guchar *ip, guchar *end, MonoClass *klass, guint32 len,
                      int *out_size, guint32 *out_field_token,
                      MonoOpcodeEnum *il_op, guchar **next_ip) {
  /*
   * newarr[System.Int32]
   * dup
  ...
  if ((ip = il_read_dup(ip, end)) && ip_in_bb(cfg, cfg->cbb, ip) &&
      (ip = il_read_ldtoken(ip, end, &field_token)) &&
      IS_FIELD_DEF(field_token) && ip_in_bb(cfg, cfg->cbb, ip) &&
    ...
    MonoClassField *field =
        mono_field_from_token_checked(m_class_get_image(method->klass),
                                      field_token, &dummy_class, NULL, error);
    int dummy_align;
    ...
    *out_field_token = field_token;
    ...
    if (!cmethod)
      return NULL;
    if (strcmp(cmethod->name, "InitializeArray") ||
        strcmp(m_class_get_name(cmethod->klass), "RuntimeHelpers") ||
        m_class_get_image(cmethod->klass) != mono_defaults.corlib)
      return NULL;
    ...
    case MONO_TYPE_U1:
      size = 1;
      break;
      /* we need to swap on big endian, so punt. Should we handle R4 and R8 as
       * well? */
    ...
    *out_size = size;
    ...
    if (!image_is_dynamic(method_klass_image)) {
      ...
      mono_metadata_field_info(method_klass_image, field_index - 1, NULL, &rva,
                               NULL);
...
static void set_exception_type_from_invalid_il(MonoCompile *cfg,
  ...
  if (!header) {
    method_code = g_strdup_printf("could not parse method body due to %s",
  ...
  mono_cfg_set_exception_invalid_program(
      cfg, g_strdup_printf("Invalid IL code in %s: %s\n", method_fname,
                           method_code));
  g_free(method_fname);
  g_free(method_code);
  cfg->headers_to_free =
      g_slist_prepend_mempool(cfg->mempool, cfg->headers_to_free, header);
}
...
handle_enum:
  ...
  case MONO_TYPE_VALUETYPE:
    ...
    return 0;
  case MONO_TYPE_VAR:
  case MONO_TYPE_MVAR: // TODO I believe we don't need to handle gsharedvt as
                       // there won't be match and, for example, u1 is not
                       // covariant to u32
    return 0;
  default:
...
static void emit_starg_ir(MonoCompile *cfg, MonoInst **sp, int n) {
  MonoInst *ins;
 ...
 * cases.
 */
static guchar *emit_optimized_ldloca_ir(MonoCompile *cfg, guchar *ip,
                                        guchar *end, int local) {
  guint32 token;
  MonoClass *klass;
  MonoType *type;
  ...
  if ((ip = il_read_initobj(ip, end, &token)) &&
      ip_in_bb(cfg, cfg->cbb, start + 1)) {
...
static MonoInst *handle_call_res_devirt(MonoCompile *cfg, MonoMethod *cmethod,
                                        MonoInst *call_res) {
  /*
  ...
  if (m_class_get_image(cmethod->klass) == mono_defaults.corlib &&
    ...
    MonoType *param_type = mono_class_get_generic_class(cmethod->klass)
                               ->context.class_inst->type_argv[0];
    MonoClass *inst;
    ...
    inst = mono_class_inflate_generic_class_checked(
     ...
     * #endif
     */
    if (mono_class_is_assignable_from_internal(
            inst, mono_class_from_mono_type_internal(param_type)) &&
        param_type->type != MONO_TYPE_U1 &&
        param_type->type != MONO_TYPE_STRING) {
      ...
      g_assert(gcomparer);
      gcomparer_inst =
          mono_class_inflate_generic_class_checked(gcomparer, &ctx, error);
      mono_error_assert_ok(error);
...
static gboolean is_exception_class(MonoClass *klass) {
 ...
 */
static gboolean is_jit_optimizer_disabled(MonoMethod *m) {
  ERROR_DECL(error);
  ...
  if (attrs) {
    for (i = 0; i < attrs->num_attrs; ++i) {
      ...
      if (sig->param_count != 2 || sig->params[0]->type != MONO_TYPE_BOOLEAN ||
...
gboolean mono_is_supported_tailcall_helper(gboolean value, const char *svalue) {
  if (!value)
    mono_tailcall_print("%s %s\n", __func__, svalue);
  return value;
}

static gboolean mono_is_not_supported_tailcall_helper(gboolean value,
                                                      const char *svalue,
                                                      MonoMethod *method,
                                                      MonoMethod *cmethod) {
  ...
  if (value && mono_tailcall_print_enabled()) {
    ...
    mono_tailcall_print("%s %s -> %s %s%s%s:%d\n", __func__, method->name,
                        cmethod->name, lparen, svalue, rparen, value);
  }
  return value;
}

#define IS_NOT_SUPPORTED_TAILCALL(x)                                           \
  (mono_is_not_supported_tailcall_helper((x), #x, method, cmethod))

static gboolean is_supported_tailcall(MonoCompile *cfg, const guint8 *ip,
                                      MonoMethod *method, MonoMethod *cmethod,
                                      MonoMethodSignature *fsig,
                                      gboolean virtual_, gboolean extra_arg,
                                      gboolean *ptailcall_calli) {
  ...
  if (IS_NOT_SUPPORTED_TAILCALL(virtual_ &&
                                !cfg->backend->have_op_tailcall_membase))
    tailcall = FALSE;
  ...
  if (IS_NOT_SUPPORTED_TAILCALL(
          cmethod && fsig->hasthis &&
          m_class_is_valuetype(
              cmethod->klass)) // This might point to the current method's
                               // stack. Emit range check?
      || IS_NOT_SUPPORTED_TAILCALL(
             cmethod && (cmethod->flags & METHOD_ATTRIBUTE_PINVOKE_IMPL)) ||
      IS_NOT_SUPPORTED_TAILCALL(fsig->pinvoke) // i.e. if !cmethod (calli)
      || IS_NOT_SUPPORTED_TAILCALL(cfg->method->save_lmf) ||
      IS_NOT_SUPPORTED_TAILCALL(!cmethod &&
                                fsig->hasthis) // FIXME could be valuetype to
                                               // current frame; range check
      || IS_NOT_SUPPORTED_TAILCALL(cmethod && cmethod->wrapper_type &&
                                   cmethod->wrapper_type !=
      ...
      // 2. a Non-generic static methods of reference types and b. non-generic
      // methods
      //    of value types need to be passed a pointer to the caller’s class’s
      //    VTable in the MONO_ARCH_RGCTX_REG register.
      ...
      // such as AMD64 rax, r10, r11, or the return register on many
      // architectures. ARM32 does not always clearly have such a register.
      // ARM32's return register is a parameter register. iPhone could use r9
      // except on old systems. iPhone/ARM32 is not particularly important.
      // Linux/arm32 is less clear. ARM32's scratch r12 might work but only with
      // much collateral change.
      //
      ...
      // However if the extra parameter were in a non-volatile, such as ARM32
      // V5/R8, F3 cannot easily restore it for F1, in the current scheme. The
      // current scheme where the extra parameter is not merely an extra
      // parameter, but passed "outside of the ABI".
      //
      // If all native to managed transitions are intercepted and wrapped (w/o
      // tailcall), then they can preserve this register and the rest of the
      // managed callgraph treat it as volatile.
      //
      // Interface method dispatch has the same problem (imt_arg).

      || IS_NOT_SUPPORTED_TAILCALL(
             extra_arg && !cfg->backend->have_volatile_non_param_register) ||
  ...
  for (int i = 0; i < fsig->param_count; ++i) {
    if (IS_NOT_SUPPORTED_TAILCALL(fsig->params[i]->byref ||
                                  fsig->params[i]->type == MONO_TYPE_PTR ||
                                  fsig->params[i]->type == MONO_TYPE_FNPTR)) {
      tailcall_calli = FALSE;
      tailcall = FALSE; // These can point to the current method's stack. Emit
                        // range check?
      goto exit;
    }
  }
  ...
  // Require an exact match on return type due to various conversions in
  // emit_move_return_value that would be skipped. The main troublesome
  // conversions are double <=> float. CoreCLR allows some conversions here,
  // such as integer truncation. As well I <=> I[48] and U <=> U[48] would be
  // ok, for matching size.
  if (IS_NOT_SUPPORTED_TAILCALL(
          mini_get_underlying_type(caller_signature->ret)->type !=
          mini_get_underlying_type(callee_signature->ret)->type) ||
      IS_NOT_SUPPORTED_TAILCALL(!mono_arch_tailcall_supported(
          cfg, caller_signature, callee_signature, virtual_))) {
    tailcall_calli = FALSE;
    tailcall = FALSE;
    goto exit;
  }
  ...
  if (tailcall_calli &&
    ...
    tailcall_calli = FALSE;
exit:
  mono_tailcall_print("tail.%s %s -> %s tailcall:%d tailcall_calli:%d "
                      "gshared:%d extra_arg:%d virtual_:%d\n",
                      mono_opcode_name(*ip), method->name,
                      cmethod ? cmethod->name : "calli", tailcall,
                      tailcall_calli, cfg->gshared, extra_arg, virtual_);
 ...
 *
 *    Returns true if a previous load can be done without doing an extra copy,
 * given the new instruction ip and the type of the object being loaded ldtype
 */
static gboolean is_addressable_valuetype_load(MonoCompile *cfg, guint8 *ip,
                                              MonoType *ldtype) {
 ...
 */
static void handle_ctor_call(MonoCompile *cfg, MonoMethod *cmethod,
                             MonoMethodSignature *fsig, int context_used,
  ...
  if (m_class_is_valuetype(cmethod->klass) &&
    ...
    if (cmethod->is_inflated && mono_method_get_context(cmethod)->method_inst) {
      ...
      vtable_arg = emit_get_rgctx_method(cfg, context_used, cmethod,
                                         MONO_RGCTX_INFO_METHOD_RGCTX);
    } else {
      if (context_used) {
        vtable_arg = mini_emit_get_rgctx_klass(
            cfg, context_used, cmethod->klass, MONO_RGCTX_INFO_VTABLE);
      } else {
        MonoVTable *vtable =
            mono_class_vtable_checked(cfg->domain, cmethod->klass, cfg->error);
        CHECK_CFG_ERROR;
  ...
  if (cmethod && (ins = mini_emit_inst_for_ctor(cfg, cmethod, fsig, sp))) {
    ...
    CHECK_CFG_EXCEPTION;
  } else if ((cfg->opt & MONO_OPT_INLINE) && cmethod && !context_used &&
             !vtable_arg && mono_method_check_inlining(cfg, cmethod) &&
             !mono_class_is_subclass_of_internal(
                 cmethod->klass, mono_defaults.exception_class, FALSE)) {
    int costs;

    if ((costs = inline_method(cfg, cmethod, fsig, sp, ip, cfg->real_offset,
                               FALSE))) {
      cfg->real_offset += 5;
      ...
      *inline_costs += costs - 5;
      ...
      mini_emit_method_call_full(cfg, cmethod, fsig, FALSE, sp,
                                 callvirt_this_arg, NULL, NULL);
    }
  } else if (cfg->gsharedvt && mini_is_gsharedvt_signature(fsig)) {
    MonoInst *addr;

    addr = emit_get_rgctx_gsharedvt_call(
        cfg, context_used, fsig, cmethod,
    ...
    if (cfg->llvm_only) {
      ...
      mini_emit_calli(cfg, fsig, sp, addr, NULL, vtable_arg);
             ...
             ((!mono_method_is_generic_sharable_full(cmethod, TRUE, FALSE,
                                                     FALSE) ||
               !mono_class_generic_sharing_enabled(cmethod->klass)) ||
              cfg->gsharedvt)) {
    MonoInst *cmethod_addr;

    /* Generic calls made out of gsharedvt methods cannot be patched, so use an
     * indirect call */
    ...
    if (cfg->llvm_only) {
      ...
      cmethod_addr = emit_get_rgctx_method(cfg, context_used, cmethod,
                                           MONO_RGCTX_INFO_GENERIC_METHOD_CODE);
 ...
 */
static MonoInst *handle_constrained_call(
    MonoCompile *cfg, MonoMethod *cmethod, MonoMethodSignature *fsig,
    MonoClass *constrained_class, MonoInst **sp, HandleCallData *cdata,
    MonoMethod **ref_cmethod, gboolean *ref_virtual, gboolean *ref_emit_widen) {
  MonoInst *ins, *addr;
  ...
  if (mini_is_gsharedvt_klass(constrained_class)) {
    ...
    } else if (m_class_get_image(cmethod->klass) != mono_defaults.corlib &&
               !mono_class_is_interface(cmethod->klass) &&
               !m_class_is_valuetype(cmethod->klass)) {
      /* 'The type parameter is instantiated as a reference type' case below. */
    } else {
      ins = handle_constrained_gsharedvt_call(
          cfg, cmethod, fsig, sp, constrained_class, ref_emit_widen);
      CHECK_CFG_EXCEPTION;
      g_assert(ins);
  ...
  if (constrained_partial_call) {
    ...
    /*
     * The receiver is a valuetype, but the exact type is not known at compile
     * time. This means the called method is not known at compile time either.
     * The called method could end up being one of the methods on the parent
     * classes (object/valuetype/enum), in which case we need to box the
     * receiver. A simple solution would be to box always and make a normal
     * virtual call, but that would be bad performance wise.
     */
    if (mono_class_is_interface(cmethod->klass) &&
        mono_class_is_ginst(cmethod->klass) &&
        (cmethod->flags & METHOD_ATTRIBUTE_ABSTRACT)) {
      /*
       * The parent classes implement no generic interfaces, so the called
       * method will be a vtype method, so no boxing necessary.
       */
      /* If the method is not abstract, it's a default interface method, and we
       * need to box */
      need_box = FALSE;
    }
    ...
    if (!(cmethod->flags & METHOD_ATTRIBUTE_VIRTUAL) &&
        (cmethod->klass == mono_defaults.object_class ||
         cmethod->klass == m_class_get_parent(mono_defaults.enum_class) ||
         cmethod->klass == mono_defaults.enum_class)) {
      /* The called method is not virtual, i.e. Object:GetType (), the receiver
       * is a vtype, has to box */
      EMIT_NEW_LOAD_MEMBASE_TYPE(
          cfg, ins, m_class_get_byval_arg(constrained_class), sp[0]->dreg, 0);
      ins->klass = constrained_class;
      sp[0] = mini_emit_box(cfg, ins, constrained_class,
      ...
      /*
       * Determine at runtime whenever the called method is defined on
       * object/valuetype/enum, and emit a boxing call if needed.
       * FIXME: It is possible to inline the called method in a lot of cases,
       * i.e. for T_INT, the no-box case goes to a method in Int32, while the
       * box case goes to a method in Enum.
       */
      addr = emit_get_rgctx_virt_method(
          cfg, mono_class_check_context_used(constrained_class),
          constrained_class, cmethod, MONO_RGCTX_INFO_VIRT_METHOD_CODE);

      NEW_BBLOCK(cfg, is_ref_bb);
      NEW_BBLOCK(cfg, end_bb);

      box_type = emit_get_rgctx_virt_method(
          cfg, mono_class_check_context_used(constrained_class),
          constrained_class, cmethod, MONO_RGCTX_INFO_VIRT_METHOD_BOX_TYPE);
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, box_type->dreg,
                              MONO_GSHAREDVT_BOX_TYPE_REF);
      MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_IBEQ, is_ref_bb);
      ...
      if (cfg->llvm_only)
        ...
        nonbox_call =
            (MonoInst *)mini_emit_calli(cfg, fsig, sp, addr, NULL, NULL);
      ...
      if (cdata->inst_tailcall) // FIXME
        mono_tailcall_print(
            "missed tailcall constrained_partial_need_box %s -> %s\n",
            method->name, cmethod->name);
      return ins;
    } else {
...
static void emit_setret(MonoCompile *cfg, MonoInst *val) {
  MonoType *ret_type = mini_get_underlying_type(
      mono_method_signature_internal(cfg->method)->ret);
  MonoInst *ins;
  ...
  if (mini_type_to_stind(cfg, ret_type) == CEE_STOBJ) {
    ...
    if (!cfg->vret_addr) {
      ...
      if (MONO_CLASS_IS_SIMD(cfg, ret_class))
        EMIT_NEW_STORE_MEMBASE(cfg, ins, OP_STOREX_MEMBASE, ret_addr->dreg, 0,
                               val->dreg);
      else
        EMIT_NEW_STORE_MEMBASE(cfg, ins, OP_STOREV_MEMBASE, ret_addr->dreg, 0,
                               val->dreg);
      ins->klass = ret_class;
    }
  } else {
#ifdef MONO_ARCH_SOFT_FLOAT_FALLBACK
    if (COMPILE_SOFT_FLOAT(cfg) && !ret_type->byref &&
        ret_type->type == MONO_TYPE_R4) {
      MonoInst *conv;
...
mono_opcode_decode(guchar *ip, guint op_size, MonoOpcodeEnum il_op,
...
#define OPDEF(name, str, pops, pushes, param, param_constant, a, b, c, flow)   \
  {param_constant + 1, pops, pushes},
 ...
 * @return_var: if not NULL, the place where the return value is stored, used
 * during inlining.
 * @inline_args: if not NULL, contains the arguments to the inline call
 * @inline_offset: if not zero, the real offset from the inline call, or zero
 * otherwise.
 * @is_virtual_call: whether this method is being called as a result of a call
 * to callvirt
 *
...
int mono_method_to_ir(MonoCompile *cfg, MonoMethod *method,
  ...
  /* serialization and xdomain stuff may need access to private fields and
   * methods */
  ...
  dont_verify |=
      method->wrapper_type == MONO_WRAPPER_MANAGED_TO_NATIVE; /* bug #77896 */
  ...
  if (cfg->prof_coverage) {
    ...
    cfg->coverage_info =
        mono_profiler_coverage_alloc(cfg->method, header->code_size);
  }
    ...
    if (minfo) {
      ...
      seq_point_locs = mono_bitset_mem_new(
          mono_mempool_alloc0(cfg->mempool,
                              mono_bitset_alloc_size(header->code_size, 0)),
          header->code_size, 0);
      seq_point_set_locs = mono_bitset_mem_new(
          mono_mempool_alloc0(cfg->mempool,
                              mono_bitset_alloc_size(header->code_size, 0)),
          header->code_size, 0);
      sym_seq_points = TRUE;
  ...
  cfg->cil_offset_to_bb = (MonoBasicBlock **)mono_mempool_alloc0(
      cfg->mempool, sizeof(MonoBasicBlock *) * header->code_size);
  cfg->cil_offset_to_bb_len = header->code_size;

  if (cfg->verbose_level > 2)
    printf("method to IR %s\n", mono_method_full_name(method, TRUE));

  param_types = (MonoType **)mono_mempool_alloc(cfg->mempool,
                                                sizeof(MonoType *) * num_args);
  if (sig->hasthis)
    param_types[0] = m_class_is_valuetype(method->klass)
  ...
  if (cfg->method == method) {
    ...
    for (i = 0; i < header->num_clauses; ++i) {
      ...
      /*
       * Linking the try block with the EH block hinders inlining as we won't be
       * able to merge the bblocks from inlining and produce an artificial hole
       * for no good reason.
       */
      if (COMPILE_LLVM(cfg))
      ...
      /*printf ("clause try IL_%04x to IL_%04x handler %d at IL_%04x to
        IL_%04x\n", clause->try_offset, clause->try_offset + clause->try_len,
        clause->flags, clause->handler_offset, clause->handler_offset +
        clause->handler_len); while (p < end) { printf ("%s",
        mono_disasm_code_one (NULL, method, p, &p));
        }*/
      ...
      if (clause->flags == MONO_EXCEPTION_CLAUSE_NONE ||
        ...
        tblock->in_scount = 1;
        tblock->in_stack =
            (MonoInst **)mono_mempool_alloc(cfg->mempool, sizeof(MonoInst *));
        tblock->in_stack[0] =
            mono_create_exvar_for_offset(cfg, clause->handler_offset);
        ...
        /* The EH code passes in the exception in a register to both JITted and
         * LLVM compiled code */
        if (!cfg->compile_llvm) {
      ...
      if (clause->flags != MONO_EXCEPTION_CLAUSE_FILTER &&
          clause->data.catch_class && cfg->gshared &&
  ...
  if (cfg->method == method && cfg->self_init && cfg->compile_aot &&
    ...
    /*
     * Emit code to initialize this method by calling the init wrapper emitted
     * by LLVM. This is not efficient right now, but its only used for the
     * methods which fail LLVM compilation.
     * FIXME: Optimize this
     */
    g_assert(!cfg->gshared);
    wrapper = mono_marshal_get_aot_init_wrapper(AOT_INIT_METHOD);
    /* Emit this into the entry bb so it comes before the GC safe point which
     * depends on an inited GOT */
  ...
  if (cfg->gsharedvt && cfg->method == method) {
    ...
    info = (MonoGSharedVtMethodInfo *)mono_mempool_alloc0(
        cfg->mempool, sizeof(MonoGSharedVtMethodInfo));
    info->method = cfg->method;
    info->count_entries = 16;
    info->entries =
        (MonoRuntimeGenericContextInfoTemplate *)mono_mempool_alloc0(
            cfg->mempool, sizeof(MonoRuntimeGenericContextInfoTemplate) *
                              info->count_entries);
    ...
    ins = emit_get_rgctx_gsharedvt_method(
        cfg, mini_method_check_context_used(cfg, method), method, info);
    ...
    dreg = alloc_ireg(cfg);
    MONO_EMIT_NEW_LOAD_MEMBASE_OP(
        cfg, OP_LOADI4_MEMBASE, dreg, var->dreg,
        MONO_STRUCT_OFFSET(MonoGSharedVtMethodRuntimeInfo, locals_size));
  ...
  if (mono_security_core_clr_enabled()) {
    ...
    if (method->wrapper_type == MONO_WRAPPER_MANAGED_TO_NATIVE) {
      ...
      if (wrapped) {
        ...
        gboolean icall =
            (wrapped->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL);

        /* if this ia a native call then it can only be JITted from platform
         * code */
        if ((icall || pinvk) && method->klass &&
            m_class_get_image(method->klass)) {
  ...
  stack_start = sp = (MonoInst **)mono_mempool_alloc0(
      cfg->mempool, sizeof(MonoInst *) * (header->max_stack + 1));
  ...
  for (guchar *next_ip = ip; ip < end; ip = next_ip) {
      ...
      for (i = 0; i < cfg->cbb->in_scount; ++i) {
        if (cfg->verbose_level > 3)
          printf("loading %d from temp %d\n", i,
                 (int)cfg->cbb->in_stack[i]->inst_c0);
    ...
    /*
     * Methods with AggressiveInline flag could be inlined even if the class has
     * a cctor. This might create a branch so emit it in the first code bblock
     * instead of into initlocals_bb.
     */
    if (ip - header->code == 0 && cfg->method != method && cfg->compile_aot &&
        (method->iflags & METHOD_IMPL_ATTRIBUTE_AGGRESSIVE_INLINING) &&
     ...
     */
    if (seq_points &&
        ((!sym_seq_points && (sp == stack_start)) ||
         (sym_seq_points &&
          mono_bitset_test_fast(seq_point_locs, ip - header->code)))) {
      /*
       ...
       */
      gboolean intr_loc = ip == header->code ||
                          (!cfg->cbb->last_ins && cfg->header->num_clauses);
      gboolean sym_seq_point =
          sym_seq_points &&
          mono_bitset_test_fast(seq_point_locs, ip - header->code);

      /* Avoid sequence points on empty IL like .volatile */
      // FIXME: Enable this
      // if (!(cfg->cbb->last_ins && cfg->cbb->last_ins->opcode ==
      // OP_SEQ_POINT)) {
      ...
      if (cfg->prof_coverage) {
        ...
        if (mono_arch_opcode_supported(OP_ATOMIC_ADD_I4)) {
          ...
          MONO_EMIT_NEW_STORE_MEMBASE_IMM(cfg, OP_STORE_MEMBASE_IMM, ins->dreg,
                                          0, 1);
        }
      }
    }
    ...
    if (cfg->verbose_level > 3)
      printf("converting (in B%d: stack: %d) %s", cfg->cbb->block_num,
             (int)(sp - stack_start),
             mono_disasm_code_one(NULL, method, ip, NULL));
    ...
    // These are used only in CALL/CALLVIRT but must be initialized also for
    // CALLI, since it jumps into CALL/CALLVIRT.
    ...
    MonoOpcodeParameter parameter;
    const MonoOpcodeInfo *info =
        mono_opcode_decode(ip, op_size, il_op, &parameter);
    g_assert(info);
    n = parameter.i32;
    token = parameter.i32;
    target = parameter.branch_target;

    // Check stack size for push/pop except variable cases -- -1 like
    // call/ret/newobj.
    ...
    case MONO_CEE_LDARG:
      CHECK_ARG(n);
      if (next_ip < end &&
          is_addressable_valuetype_load(cfg, next_ip, cfg->arg_types[n])) {
    ...
    case MONO_CEE_LDLOC:
      CHECK_LOCAL(n);
      if (next_ip < end &&
          is_addressable_valuetype_load(cfg, next_ip, header->locals[n])) {
    ...
    case MONO_CEE_STARG:
      ...
      *sp = convert_value(cfg, param_types[n], *sp);
      if (!dont_verify_stloc &&
          target_type_is_incompatible(cfg, param_types[n], *sp))
        UNVERIFIED;
    ...
    case MONO_CEE_LDC_R4: {
      ...
      if (cfg->compile_aot)
        use_aotconst = TRUE;
#endif
      /* FIXME: we should really allocate this only late in the compilation
       * process */
    ...
    case MONO_CEE_JMP: {
      ...
      if (cfg->llvm_only) {
        ...
        args = (MonoInst **)mono_mempool_alloc(cfg->mempool,
                                               sizeof(MonoInst *) * n);
        for (i = 0; i < n; ++i)
          EMIT_NEW_ARGLOAD(cfg, args[i], i);
        ins = mini_emit_method_call_full(cfg, cmethod, fsig, TRUE, args, NULL,
                                         NULL, NULL);
        /*
         * The code in mono-basic-block.c treats the rest of the code as dead,
         * but we have to emit a normal return since llvm expects it.
         */
        if (cfg->ret)
          emit_setret(cfg, ins);
        ...
        call->tailcall = TRUE;
        call->signature = fsig;
        call->args = (MonoInst **)mono_mempool_alloc(cfg->mempool,
                                                     sizeof(MonoInst *) * n);
    ...
    case MONO_CEE_CALLI: {
      ...
      gboolean const inst_tailcall = G_UNLIKELY(
          debug_tailcall_try_all ? (next_ip < end && next_ip[0] == CEE_RET)
      ...
      if (!(cfg->method->wrapper_type &&
          ...
          check_call_signature(cfg, fsig, sp)) {
       ...
       * This needs to be used for all generic calls, not just ones with a
       * gsharedvt signature, to avoid patching gshared method addresses into a
       * gsharedvt method.
       */
      if (cfg->gsharedvt && mini_is_gsharedvt_signature(fsig)) {
        /*
      ...
      if (callee) {
        ...
        addr = emit_get_rgctx_sig(
            cfg, context_used, fsig,
            MONO_RGCTX_INFO_SIG_GSHAREDVT_OUT_TRAMPOLINE_CALLI);
        ins = (MonoInst *)mini_emit_calli_full(cfg, fsig, sp, addr, NULL,
                                               callee, tailcall);
        goto calli_end;
      }
      ...
      if (addr->opcode == OP_PCONST || addr->opcode == OP_AOTCONST ||
        ...
        if (info_type == MONO_PATCH_INFO_ICALL_ADDR) {
          ...
          tailcall = FALSE;
          ins = (MonoInst *)mini_emit_abs_call(
              cfg, MONO_PATCH_INFO_ICALL_ADDR_CALL, info_data, fsig, sp);
          NULLIFY_INS(addr);
          goto calli_end;
          ...
          tailcall = FALSE;
          ins = (MonoInst *)mini_emit_abs_call(cfg, info_type, info_data, fsig,
                                               sp);
          NULLIFY_INS(addr);
          goto calli_end;
        }
      }
      ins = (MonoInst *)mini_emit_calli_full(cfg, fsig, sp, addr, NULL, NULL,
                                             tailcall);
      goto calli_end;
    }
    case MONO_CEE_CALL:
    case MONO_CEE_CALLVIRT: {
      ...
      if (constrained_class) {
        ...
        if (method->wrapper_type != MONO_WRAPPER_NONE) {
          ...
          if (!(constrained_is_generic_param && cfg->gshared)) {
            cmethod = mono_get_method_constrained_with_method(
                image, cil_method, constrained_class, generic_context,
                cfg->error);
            CHECK_CFG_ERROR;
          }
        } else {
          ...
          if (constrained_is_generic_param && cfg->gshared) {
            ...
            cmethod = mono_get_method_constrained_checked(
                image, token, constrained_class, generic_context, &cil_method,
                cfg->error);
            CHECK_CFG_ERROR;
          }
        }
      ...
      if (!dont_verify && !cfg->skip_visibility) {
        ...
        if (method->is_inflated) {
          target_method = mini_get_method_allow_open(
              method, token, NULL,
              &(mono_method_get_generic_container(method_definition)->context),
              cfg->error);
          CHECK_CFG_ERROR;
        }
      ...
      {
        /*
         * MS.NET accepts non virtual calls to virtual final methods of
         * transparent proxy classes and converts to a callvirt.
         *
         * tests/bug-515884.il is an example of this behavior
         */
        const int test_flags = METHOD_ATTRIBUTE_VIRTUAL |
                               METHOD_ATTRIBUTE_FINAL | METHOD_ATTRIBUTE_STATIC;
        const int expected_flags =
            METHOD_ATTRIBUTE_VIRTUAL | METHOD_ATTRIBUTE_FINAL;
        if (!virtual_ && mono_class_is_marshalbyref(cmethod->klass) &&
            (cmethod->flags & test_flags) == expected_flags &&
            cfg->method->wrapper_type == MONO_WRAPPER_NONE)
          virtual_ = TRUE;
      }
      ...
      if (cmethod->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL &&
        ...
        array_rank = m_class_get_rank(cmethod->klass);
                 ...
                 direct_icalls_enabled(cfg, cmethod)) {
        direct_icall = TRUE;
      } else if (fsig->pinvoke) {
        MonoMethod *wrapper =
            mono_marshal_get_native_wrapper(cmethod, TRUE, cfg->compile_aot);
        fsig = mono_method_signature_internal(wrapper);
      } else if (constrained_class) {
      } else {
        fsig = mono_method_get_signature_checked(cmethod, image, token,
                                                 generic_context, cfg->error);
        CHECK_CFG_ERROR;
      }

      if (cfg->llvm_only && !cfg->method->wrapper_type &&
          (!cmethod || cmethod->is_inflated))
        cfg->signatures =
            g_slist_prepend_mempool(cfg->mempool, cfg->signatures, fsig);

      /* See code below */
      if (cmethod->klass == mono_defaults.monitor_class &&
          !strcmp(cmethod->name, "Enter") &&
          mono_method_signature_internal(cmethod)->param_count == 1) {
        MonoBasicBlock *tbb;

        GET_BBLOCK(cfg, tbb, next_ip);
        if (tbb->try_start &&
            MONO_REGION_FLAGS(tbb->region) == MONO_EXCEPTION_CLAUSE_FINALLY) {
          /*
           * We want to extend the try block to cover the call, but we can't do
           * it if the call is made directly since its followed by an exception
           * check.
           */
          direct_icall = FALSE;
        }
      ...
      if (!(seq_point_locs &&
            mono_bitset_test_fast(seq_point_locs, next_ip - header->code)))
        need_seq_point = TRUE;
      ...
      */
      ...
      if (virtual_ && cmethod && sp[0]->opcode == OP_TYPED_OBJREF) {
        ...
        MonoMethod *new_cmethod =
            mono_class_get_virtual_method(sp[0]->klass, cmethod, FALSE, error);
        mono_error_assert_ok(error);
        cmethod = new_cmethod;
        virtual_ = FALSE;
      }
       ...
       */
      if (constrained_class) {
        ins = handle_constrained_call(cfg, cmethod, fsig, constrained_class, sp,
                                      &cdata, &cmethod, &virtual_, &emit_widen);
        CHECK_CFG_EXCEPTION;
      ...
      for (int i = 0; i < fsig->param_count; ++i)
        sp[i + fsig->hasthis] =
            convert_value(cfg, fsig->params[i], sp[i + fsig->hasthis]);
      ...
      if ((m_class_get_parent(cmethod->klass) ==
           mono_defaults.multicastdelegate_class) &&
          !strcmp(cmethod->name, "Invoke"))
       ...
       *    Monitor.Exit ()
       * }
       * If a thread abort happens between the call to Monitor.Enter () and the
       * start of the try block, the Exit () won't be executed, see:
       * http://www.bluebytesoftware.com/blog/2007/01/30/MonitorEnterThreadAbortsAndOrphanedLocks.aspx
       * To work around this, we extend such try blocks to include the last x
       * bytes of the Monitor.Enter () call.
       */
      if (cmethod->klass == mono_defaults.monitor_class &&
          !strcmp(cmethod->name, "Enter") &&
          mono_method_signature_internal(cmethod)->param_count == 1) {
        ...
        GET_BBLOCK(cfg, tbb, next_ip);
        /*
         * Only extend try blocks with a finally, to avoid catching exceptions
         * thrown from Monitor.Enter like ArgumentNullException.
         */
        if (tbb->try_start &&
            MONO_REGION_FLAGS(tbb->region) == MONO_EXCEPTION_CLAUSE_FINALLY) {
       ...
       */
      if (cfg->gshared && cmethod->klass != method->klass &&
          mono_class_is_ginst(cmethod->klass) &&
          mono_method_is_generic_sharable(cmethod, TRUE) &&
          mono_class_needs_cctor_run(cmethod->klass, method)) {
      ...
      if ((cfg->opt & MONO_OPT_INLINE) && !inst_tailcall &&
          (!virtual_ || !(cmethod->flags & METHOD_ATTRIBUTE_VIRTUAL) ||
        ...
        if ((cmethod->iflags & METHOD_IMPL_ATTRIBUTE_INTERNAL_CALL) ||
          ...
          // FIXME? Does this write to cmethod impact tailcall_supported?
          // Probably not. Neither pinvoke or icall are likely to be tailcalled.
          cmethod = mono_marshal_get_native_wrapper(cmethod, TRUE, FALSE);
          always = TRUE;
        }

        costs =
            inline_method(cfg, cmethod, fsig, sp, ip, cfg->real_offset, always);
        if (costs) {
          cfg->real_offset += 5;
      ...
      if (cfg->gshared) {
         ...
         */
        if (context_used && ((cfg->method->flags & METHOD_ATTRIBUTE_STATIC) ||
                             m_class_is_valuetype(cfg->method->klass)))
          mono_get_vtable_var(cfg);
      }
      ...
      if (pass_vtable) {
        if (context_used) {
          ...
          MonoVTable *vtable = mono_class_vtable_checked(
              cfg->domain, cmethod->klass, cfg->error);
          CHECK_CFG_ERROR;
      ...
      if (pass_imt_from_rgctx) {
        ...
        imt_arg = emit_get_rgctx_method(cfg, context_used, cmethod,
                                        MONO_RGCTX_INFO_METHOD);
        g_assert(imt_arg);
      }
      ...
      if (virtual_ && (cmethod->flags & METHOD_ATTRIBUTE_VIRTUAL) &&
        ...
        if (cfg->backend->have_generalized_imt_trampoline &&
            cfg->backend->gshared_supported &&
       ...
       */
      if (cfg->gsharedvt &&
          (mini_is_gsharedvt_signature(fsig) || cmethod->is_inflated ||
           mono_class_is_ginst(cmethod->klass)) &&
          !(m_class_get_rank(cmethod->klass) &&
            m_class_get_byval_arg(cmethod->klass)->type != MONO_TYPE_SZARRAY) &&
      ...
      tailcall = inst_tailcall &&
                 is_supported_tailcall(cfg, ip, method, cmethod, fsig, virtual_,
                                       tailcall_extra_arg, &tailcall_calli);
      // Writes to imt_arg, vtable_arg, virtual_, cmethod, must not occur from
      // here (inputs to is_supported_tailcall). Capture values to later assert
      // they don't change.
      ...
      if (virtual_generic) {
        if (virtual_generic_imt) {
          if (tailcall) {
            /* Prevent inlining of methods with tailcalls (the call stack would
             * be altered) */
        ...
        this_temp =
            mono_compile_create_var(cfg, type_from_stack_type(sp[0]), OP_LOCAL);
        ...
        iargs[1] = emit_get_rgctx_method(cfg, context_used, cmethod,
                                         MONO_RGCTX_INFO_METHOD);

        EMIT_NEW_TEMPLOADA(cfg, iargs[2], this_arg_temp->inst_c0);
        addr =
            mono_emit_jit_icall(cfg, mono_helper_compile_generic_method, iargs);
      ...
      if (((cfg->opt & MONO_OPT_TAILCALL) || inst_tailcall) &&
          il_op == MONO_CEE_CALL && cmethod == method && next_ip < end &&
          next_ip[0] == CEE_RET && !vtable_arg) {
        gboolean has_vtargs = FALSE;
        int i;

        /* Prevent inlining of methods with tailcalls (the call stack would be
         * altered) */
        INLINE_FAILURE("tailcall");

        /* keep it simple */
        for (i = fsig->param_count - 1; !has_vtargs && i >= 0; i--)
          has_vtargs = MONO_TYPE_ISSTRUCT(
              mono_method_signature_internal(cmethod)->params[i]);
      ...
      if (cfg->method->wrapper_type == MONO_WRAPPER_SYNCHRONIZED) {
        ...
        if (cmethod == orig ||
            (cmethod->is_inflated &&
             mono_method_get_declaring_generic_method(cmethod) == orig)) {
          // FIXME? Does this write to cmethod impact tailcall_supported?
          // Probably not.
      ...
      if (make_generic_call_out_of_gsharedvt_method) {
        if (virtual_) {
          ...
          if (fsig->hasthis && (mono_class_is_marshalbyref(method->klass) ||
          ...
          /* This is not needed, as the trampoline code will pass one, and it
           * might be passed in the same reg as the imt arg */
          vtable_arg = NULL;
        }
        ...
        addr = emit_get_rgctx_gsharedvt_call(cfg, context_used, fsig, cmethod,
                                             info_type);
        ...
        if (cfg->llvm_only) {
          ...
          ins = (MonoInst *)mini_emit_calli_full(cfg, fsig, sp, addr, imt_arg,
                                                 vtable_arg, tailcall);
      ...
      if (direct_icall) {
        ...
        costs =
            inline_method(cfg, wrapper, fsig, sp, ip, cfg->real_offset, TRUE);
        g_assert(costs > 0);
        cfg->real_offset += 5;
      ...
      if (array_rank) {
        ...
        if (strcmp(cmethod->name, "Set") == 0) { /* array Set */
          ...
          if (!mini_debug_options.weak_memory_model && val->type == STACK_OBJ)
            mini_emit_memory_barrier(cfg, MONO_MEMORY_BARRIER_REL);
          EMIT_NEW_STORE_MEMBASE_TYPE(cfg, ins,
                                      fsig->params[fsig->param_count - 1],
                                      addr->dreg, 0, val->dreg);
          if (cfg->gen_write_barriers && val->type == STACK_OBJ &&
              !MONO_INS_IS_PCONST_NULL(val))
            mini_emit_write_barrier(cfg, addr, val);
          if (cfg->gen_write_barriers &&
              mini_is_gsharedvt_klass(cmethod->klass))
            GSHAREDVT_FAILURE(il_op);
          ...
          if (!m_class_is_valuetype(
                  m_class_get_element_class(cmethod->klass)) &&
              !readonly)
       ...
       */
      if (cfg->llvm_only && virtual_ && cmethod &&
          (cmethod->flags & METHOD_ATTRIBUTE_VIRTUAL)) {
        ins = mini_emit_llvmonly_virtual_call(cfg, cmethod, fsig, context_used,
                                              sp);
        goto call_end;
      }
    ...
    call_end:
      ...
      // FIXME? cmethod does change, weaken the assert if we weren't tailcalling
      // anyway. If this still fails, restructure the code, or call
      // tailcall_supported again and assert no change.
      g_assert(!called_is_supported_tailcall || !tailcall ||
               tailcall_cmethod == cmethod);
      g_assert(!called_is_supported_tailcall || tailcall_fsig == fsig);
      g_assert(!called_is_supported_tailcall || tailcall_virtual == virtual_);
      g_assert(!called_is_supported_tailcall ||
               tailcall_extra_arg ==
                   (vtable_arg || imt_arg || will_have_imt_arg ||
                    mono_class_is_interface(cmethod->klass)));

      if (common_call) // FIXME goto call_end && !common_call often skips
                       // tailcall processing.
        ins = mini_emit_method_call_full(cfg, cmethod, fsig, tailcall, sp,
                                         virtual_ ? sp[0] : NULL, imt_arg,
                                         vtable_arg);

      /*
       * Handle devirt of some A.B.C calls by replacing the result of A.B with a
       * OP_TYPED_OBJREF instruction, so the .C call can be devirtualized above.
       */
      if (cmethod)
    ...
    calli_end:
      if ((tailcall_remove_ret || (common_call && tailcall)) &&
          !cfg->llvm_only) {
        link_bblock(cfg, cfg->cbb, end_bblock);
        start_new_bblock = 1;
      ...
      if (skip_ret) {
        // FIXME When not followed by CEE_RET, correct behavior is to raise an
        // exception.
      ...
      if (need_seq_point) {
        // check is is a nested call and remove the non_empty_stack of the last
        // call, only for non native methods
    ...
    case MONO_CEE_RET:
      mini_profiler_emit_leave(cfg, sig->ret->type != MONO_TYPE_VOID ? sp[-1]
                                                                     : NULL);
      ...
      if (cfg->method != method) {
        ...
        if (return_var && cfg->cbb->in_count) {
          ...
          *sp = convert_value(cfg, ret_type, *sp);
          ...
          if ((method->wrapper_type == MONO_WRAPPER_DYNAMIC_METHOD ||
              ...
              target_type_is_incompatible(cfg, ret_type, *sp))
            UNVERIFIED;
        ...
        if (cfg->ret) {
          MonoType *ret_type = mini_get_underlying_type(
              mono_method_signature_internal(method)->ret);
    ...
    case MONO_CEE_SWITCH: {
      ...
      targets = (MonoBasicBlock **)mono_mempool_alloc(
          cfg->mempool, sizeof(MonoBasicBlock *) * n);
      for (i = 0; i < n; ++i) {
      ...
      table = (MonoJumpInfoBBTable *)mono_mempool_alloc(
          cfg->mempool, sizeof(MonoJumpInfoBBTable));
      table->table = targets;
      table->table_size = n;
...
#if SIZEOF_REGISTER == 8
        /* The upper word might not be zero, and we add it to a 64 bit address
         * later */
    ...
    case MONO_CEE_LDIND_REF:
      ...
      ins = mini_emit_memory_load(
          cfg, m_class_get_byval_arg(ldind_to_type(il_op)), sp[0], 0, ins_flag);
      *sp++ = ins;
      ins_flag = 0;
      break;
    ...
    case MONO_CEE_STIND_I: {
      ...
      if (il_op == MONO_CEE_STIND_R4 && sp[1]->type == STACK_R8)
        sp[1] = convert_value(
            cfg, m_class_get_byval_arg(mono_defaults.single_class), sp[1]);
      if (!mini_debug_options.weak_memory_model &&
          il_op == MONO_CEE_STIND_REF &&
          method->wrapper_type != MONO_WRAPPER_WRITE_BARRIER)
        mini_emit_memory_barrier(cfg, MONO_MEMORY_BARRIER_REL);
      NEW_STORE_MEMBASE(cfg, ins, stind_to_store_membase(il_op), sp[0]->dreg, 0,
                        sp[1]->dreg);
      ins->flags |= ins_flag;
      ins_flag = 0;
    ...
    case MONO_CEE_MUL:
      ...
      if ((sp[1]->opcode == OP_ICONST) &&
          mono_arch_is_inst_imm(ins->opcode, imm_opcode, sp[1]->inst_c0)) {
        if (imm_opcode != -1) {
    ...
    case MONO_CEE_SHR_UN: {
      ...
      if (((sp[1]->opcode == OP_ICONST) || (sp[1]->opcode == OP_I8CONST)) &&
          mono_arch_is_inst_imm(ins->opcode, imm_opcode,
    ...
    case MONO_CEE_CONV_R_UN:
      ...
      if ((il_op == MONO_CEE_CONV_I8 || il_op == MONO_CEE_CONV_U8) &&
    ...
    case MONO_CEE_LDOBJ: {
      ...
      if ((loc_index != -1) && ip_in_bb(cfg, cfg->cbb, next_ip)) {
        ...
        EMIT_NEW_LOAD_MEMBASE_TYPE(cfg, ins, m_class_get_byval_arg(klass),
                                   sp[0]->dreg, 0);
      ...
      if (next_ip + 4 < end && next_ip[0] == CEE_STOBJ &&
          ip_in_bb(cfg, cfg->cbb, next_ip) && read32(next_ip + 1) == token) {
        CHECK_STACK(1);
      ...
      ins = mini_emit_memory_load(cfg, m_class_get_byval_arg(klass), sp[0], 0,
                                  ins_flag);
      *sp++ = ins;
      ...
      if (method->wrapper_type == MONO_WRAPPER_DYNAMIC_METHOD) {
        ...
        *sp = mono_emit_jit_icall(cfg, mono_string_new_wrapper_internal, iargs);
        ...
        if (cfg->opt & MONO_OPT_SHARED) {
          ...
          if (cfg->compile_aot) {
            cfg->ldstr_list =
                g_list_prepend(cfg->ldstr_list, GINT_TO_POINTER(n));
          }
          EMIT_NEW_DOMAINCONST(cfg, iargs[0]);
          ...
          *sp = mono_emit_jit_icall(cfg, ves_icall_mono_ldstr, iargs);
          mono_ldstr_checked(cfg->domain, image, mono_metadata_token_index(n),
                             cfg->error);
          CHECK_CFG_ERROR;
        } else {
          if (cfg->cbb->out_of_line) {
            ...
            *sp = ins;
            ...
            ins->type = STACK_OBJ;
            ins->inst_p0 = mono_ldstr_checked(
                cfg->domain, image, mono_metadata_token_index(n), cfg->error);
            CHECK_CFG_ERROR;
    ...
    case MONO_CEE_NEWOBJ: {
      ...
      if (cfg->gshared && cmethod && cmethod->klass != method->klass &&
          mono_class_is_ginst(cmethod->klass) &&
          mono_method_is_generic_sharable(cmethod, TRUE) &&
          mono_class_needs_cctor_run(cmethod->klass, method)) {
        emit_class_init(cfg, cmethod->klass);
      ...
      */
      ...
      if (cfg->cbb->out_of_line &&
          ...
          ((n < 1) || (!fsig->params[0]->byref &&
                       fsig->params[0]->type == MONO_TYPE_STRING)) &&
          ((n < 2) || (!fsig->params[1]->byref &&
                       fsig->params[1]->type == MONO_TYPE_STRING))) {
        MonoInst *iargs[3];
        ...
        case 0:
          *sp++ =
              mono_emit_jit_icall(cfg, mono_create_corlib_exception_0, iargs);
          break;
        case 1:
          iargs[1] = sp[0];
          *sp++ =
              mono_emit_jit_icall(cfg, mono_create_corlib_exception_1, iargs);
          break;
        case 2:
          iargs[1] = sp[0];
          iargs[2] = sp[1];
          *sp++ =
              mono_emit_jit_icall(cfg, mono_create_corlib_exception_2, iargs);
          break;
        default:
          g_assert_not_reached();
        }
      ...
      if (mini_class_is_system_array(cmethod->klass)) {
        *sp = emit_get_rgctx_method(cfg, context_used, cmethod,
        ...
        /* Optimize the common cases, use ctor using length for each rank (no
         * lbound). */
        if (n == m_class_get_rank(cmethod->klass)) {
          switch (n) {
          case 1:
            function = MONO_JIT_ICALL_mono_array_new_1;
            break;
          case 2:
            function = MONO_JIT_ICALL_mono_array_new_2;
            break;
          case 3:
            function = MONO_JIT_ICALL_mono_array_new_3;
            break;
          case 4:
            function = MONO_JIT_ICALL_mono_array_new_4;
            break;
          default:
            break;
          }
        ...
        /* Instancing jagged arrays should not end up here since ctor (int32,
         * int32) for an array with rank 1 represent lenght and lbound. */
        g_assert(
            !(m_class_get_rank(cmethod->klass) == 1 && fsig->param_count == 2 &&
              m_class_get_rank(m_class_get_element_class(cmethod->klass))));
        ...
        if (function == MONO_JIT_ICALL_ZeroIsReserved) {
          ...
          array_new_localalloc_ins->inst_imm = MAX(
              array_new_localalloc_ins->inst_imm, n * sizeof(target_mgreg_t));
          int dreg = array_new_localalloc_ins->dreg;
          for (int i = 0; i < n; ++i) {
            NEW_STORE_MEMBASE(cfg, ins, OP_STORE_MEMBASE_REG, dreg,
                              i * sizeof(target_mgreg_t), sp[i + 1]->dreg);
            MONO_ADD_INS(cfg->cbb, ins);
          }
          EMIT_NEW_ICONST(cfg, ins, n);
        ...
        alloc = mini_emit_method_call_full(cfg, cmethod, fsig, FALSE, sp, NULL,
                                           NULL, NULL);
      } else {
        if (m_class_is_valuetype(cmethod->klass)) {
          iargs[0] = mono_compile_create_var(
              cfg, m_class_get_byval_arg(cmethod->klass), OP_LOCAL);
          emit_init_rvar(cfg, iargs[0]->dreg,
                         m_class_get_byval_arg(cmethod->klass));
          ...
          *sp = alloc;
          ...
          if (!cfg->compile_aot)
            vtable = mono_class_vtable_checked(cfg->domain, cmethod->klass,
                                               cfg->error);
          CHECK_CFG_ERROR;
          CHECK_TYPELOAD(cmethod->klass);

          /*
           * TypeInitializationExceptions thrown from the
           * mono_runtime_class_init call in mono_jit_runtime_invoke () can
           * abort the finalizer thread. As a workaround, we call class cctors
           * before allocating objects.
           */
          if (mini_field_access_needs_cctor_run(cfg, method, cmethod->klass,
                                                vtable) &&
              !(g_slist_find(class_inits, cmethod->klass))) {
            emit_class_init(cfg, cmethod->klass);
            if (cfg->verbose_level > 2)
              printf("class %s.%s needs init call for ctor\n",
                     m_class_get_name_space(cmethod->klass),
                     m_class_get_name(cmethod->klass));
        ...
        handle_ctor_call(cfg, cmethod, fsig, context_used, sp, ip,
                         &inline_costs);
        CHECK_CFG_EXCEPTION;
      }
      ...
      if (alloc == NULL) {
        ...
        mini_type_to_eval_stack_type(cfg, m_class_get_byval_arg(ins->klass),
                                     ins);
        *sp++ = ins;
      } else {
        *sp++ = alloc;
    ...
    case MONO_CEE_ISINST: {
      ...
      MONO_INST_NEW(cfg, ins,
                    (il_op == MONO_CEE_ISINST) ? OP_ISINST : OP_CASTCLASS);
      ins->dreg = alloc_preg(cfg);
      ins->sreg1 = (*sp)->dreg;
      ins->klass = klass;
      ins->type = STACK_OBJ;
    ...
    case MONO_CEE_BOX: {
       ...
       *   box MyFlags
       *   constrained. MyFlags
       *   callvirt instace bool class [mscorlib] System.Enum::HasFlag (class
       ...
       * This code sequence is generated by older mcs/csc, the newer one is
       * handled in emit_inst_for_method ().
       */
      ...
      if ((cfg->opt & MONO_OPT_INTRINS) &&
          ...
          ip_in_bb(cfg, cfg->cbb, ip) && m_class_is_enumtype(klass) &&
          (enum_class =
               mini_get_class(method, constrained_token, generic_context)) &&
          (has_flag = mini_get_method(cfg, method, callvirt_token, NULL,
                                      generic_context)) &&
          has_flag->klass == mono_defaults.enum_class &&
          !strcmp(has_flag->name, "HasFlag") && has_flag->signature->hasthis &&
        ...
        if (enum_class == klass) {
          ...
          *sp++ =
              mini_handle_enum_has_flag(cfg, klass, enum_this, -1, enum_flag);
          break;
        }
      }
      ...
      if ((cfg->opt & MONO_OPT_INTRINS) && next_ip < end &&
        ...
        MonoClass *unbox_klass =
            mini_get_class(method, unbox_any_token, generic_context);
      ...
      if ((ip = il_read_call(next_ip, end, &gettype_token)) &&
          ip_in_bb(cfg, cfg->cbb, ip)) {
        MonoMethod *gettype_method =
            mini_get_method(cfg, method, gettype_token, NULL, generic_context);
        if (!strcmp(gettype_method->name, "GetType") &&
            gettype_method->klass == mono_defaults.object_class) {
          mono_class_init_internal(klass);
          if (mono_class_get_checked(m_class_get_image(klass),
                                     m_class_get_type_token(klass),
                                     error) == klass) {
            if (cfg->compile_aot) {
              EMIT_NEW_TYPE_FROM_HANDLE_CONST(
                  cfg, ins, m_class_get_image(klass),
                  m_class_get_type_token(klass), generic_context);
            } else {
              MonoType *klass_type = m_class_get_byval_arg(klass);
              MonoReflectionType *reflection_type =
                  mono_type_get_object_checked(cfg->domain, klass_type,
                                               cfg->error);
      ...
      //    ldc.i4.0 (or 1)
      guchar *ldnull_ip;
      if ((ldnull_ip = il_read_op(next_ip, end, CEE_LDNULL, MONO_CEE_LDNULL)) &&
          ip_in_bb(cfg, cfg->cbb, ldnull_ip)) {
        gboolean is_eq = FALSE, is_neq = FALSE;
        if ((ip = il_read_op(ldnull_ip, end, CEE_PREFIX1, MONO_CEE_CEQ)))
          is_eq = TRUE;
        else if ((ip =
                      il_read_op(ldnull_ip, end, CEE_PREFIX1, MONO_CEE_CGT_UN)))
          is_neq = TRUE;
      ...
      guint32 isinst_tk = 0;
      if ((ip = il_read_op_and_token(next_ip, end, CEE_ISINST, MONO_CEE_ISINST,
                                     &isinst_tk)) &&
          ip_in_bb(cfg, cfg->cbb, ip)) {
        MonoClass *isinst_class =
            mini_get_class(method, isinst_tk, generic_context);
        if (!mono_class_is_nullable(klass) &&
            !mono_class_is_nullable(isinst_class) &&
            !mini_is_gsharedvt_variable_klass(klass) &&
            !mini_is_gsharedvt_variable_klass(isinst_class) &&
            !mono_class_is_open_constructed_type(
                m_class_get_byval_arg(klass)) &&
            !mono_class_is_open_constructed_type(
                m_class_get_byval_arg(isinst_class))) {
          ...
          //
          guchar *br_ip = NULL;
          if ((br_ip = il_read_brtrue(ip, end, &target)) ||
              (br_ip = il_read_brtrue_s(ip, end, &target)) ||
              (br_ip = il_read_brfalse(ip, end, &target)) ||
              (br_ip = il_read_brfalse_s(ip, end, &target))) {

            gboolean isinst =
                mono_class_is_assignable_from_internal(isinst_class, klass);
            next_ip = ip;
          ...
          if ((ldnull_ip = il_read_op(ip, end, CEE_LDNULL, MONO_CEE_LDNULL)) &&
              ip_in_bb(cfg, cfg->cbb, ldnull_ip)) {
            gboolean is_eq = FALSE, is_neq = FALSE;
            if ((ip = il_read_op(ldnull_ip, end, CEE_PREFIX1, MONO_CEE_CEQ)))
              is_eq = TRUE;
            else if ((ip = il_read_op(ldnull_ip, end, CEE_PREFIX1,
                                      MONO_CEE_CGT_UN)))
              is_neq = TRUE;
          ...
          if ((unbox_ip = il_read_unbox_any(ip, end, &unbox_token)) &&
              ip_in_bb(cfg, cfg->cbb, unbox_ip)) {
            MonoClass *unbox_klass =
                mini_get_class(method, unbox_token, generic_context);
            CHECK_TYPELOAD(unbox_klass);
            if (!mono_class_is_nullable(unbox_klass) &&
                !mini_is_gsharedvt_klass(unbox_klass) &&
                klass == isinst_class && klass == unbox_klass) {
              *sp++ = val;
              next_ip = unbox_ip;
              break;
            }
      ...
      if (!mono_class_is_nullable(klass) && !mini_is_gsharedvt_klass(klass) &&
          next_ip < end && ip_in_bb(cfg, cfg->cbb, next_ip) &&
        ...
        if (COMPILE_LLVM(cfg)) {
          dreg = alloc_ireg(cfg);
          MONO_EMIT_NEW_ICONST(cfg, dreg, 0);
          MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, dreg,
                                  is_true ? 0 : 1);
      ...
      if (m_class_is_enumtype(klass) && !mini_is_gsharedvt_klass(klass) &&
          !(val->type == STACK_I8 && TARGET_SIZEOF_VOID_P == 4)) {
        /* Can't do this with 64 bit enums on 32 bit since the vtype decomp pass
         * is ran after the long decomp pass */
    ...
    case MONO_CEE_UNBOX: {
      ...
      if (mono_class_is_nullable(klass)) {
        ...
        EMIT_NEW_VARLOADA(cfg, ins, get_vreg_to_inst(cfg, val->dreg),
                          m_class_get_byval_arg(val->klass));
    ...
    case MONO_CEE_STSFLD: {
      ...
      is_instance = (il_op == MONO_CEE_LDFLD || il_op == MONO_CEE_LDFLDA ||
                     il_op == MONO_CEE_STFLD);
      if (is_instance) {
      ...
      if (method->wrapper_type != MONO_WRAPPER_NONE) {
        ...
        klass = NULL;
        field = mono_field_from_token_checked(image, token, &klass,
                                              generic_context, cfg->error);
        if (!field)
          CHECK_TYPELOAD(klass);
        CHECK_CFG_ERROR;
      ...
      /* if the class is Critical then transparent code cannot access it's
       * fields */
      if (!is_instance && mono_security_core_clr_enabled())
        ensure_method_is_allowed_to_access_field(cfg, method, field);

      /* XXX this is technically required but, so far (SL2), no
      [SecurityCritical] types (not many exists) have any visible *instance*
      field  (in fact there's a single case for a static field in Marshal) XXX
      if (mono_security_core_clr_enabled ())
          ensure_method_is_allowed_to_access_field (cfg, method, field);
      */
      ...
      /*
       * LDFLD etc. is usable on static fields as well, so convert those cases
       * to the static case.
       */
      ...
      foffset = m_class_is_valuetype(klass)
      ...
      if (il_op == MONO_CEE_STFLD) {
        ...
        if ((mono_class_is_marshalbyref(klass) && !MONO_CHECK_THIS(sp[0])) ||
          ...
          if (cfg->opt & MONO_OPT_INLINE || cfg->compile_aot) {
            costs = inline_method(cfg, stfld_wrapper,
...
#endif
        {
          ...
          if (mini_is_gsharedvt_klass(klass)) {
            ...
            offset_ins = emit_get_gsharedvt_info(cfg, field,
                                                 MONO_RGCTX_INFO_FIELD_OFFSET);
            /* The value is offset by 1 */
            EMIT_NEW_BIALU_IMM(cfg, ins, OP_PSUB_IMM, offset_ins->dreg,
                               offset_ins->dreg, 1);
            dreg = alloc_ireg_mp(cfg);
            EMIT_NEW_BIALU(cfg, ins, OP_PADD, dreg, sp[0]->dreg,
                           offset_ins->dreg);
            if (cfg->gen_write_barriers &&
              ...
              /* The decomposition will call mini_emit_memory_copy () which will
               * emit a wbarrier if needed */
              EMIT_NEW_STORE_MEMBASE_TYPE(cfg, store, field->type, dreg, 0,
                                          sp[1]->dreg);
            }
          } else {
            if (cfg->gen_write_barriers &&
              ...
              dreg = alloc_ireg_mp(cfg);
              EMIT_NEW_BIALU_IMM(cfg, ptr, OP_PADD_IMM, dreg, sp[0]->dreg,
                                 foffset);
              store = mini_emit_storing_write_barrier(cfg, ptr, sp[1]);
            } else {
              EMIT_NEW_STORE_MEMBASE_TYPE(cfg, store, field->type, sp[0]->dreg,
                                          foffset, sp[1]->dreg);
            }
          }
...
#ifndef DISABLE_REMOTING
      if (is_instance &&
          ((mono_class_is_marshalbyref(klass) && !MONO_CHECK_THIS(sp[0])) ||
           mono_class_is_contextbound(klass) ||
           klass == mono_defaults.marshalbyrefobject_class)) {
        MonoMethod *wrapper = (il_op == MONO_CEE_LDFLDA)
                                  ? mono_marshal_get_ldflda_wrapper(field->type)
                                  : mono_marshal_get_ldfld_wrapper(field->type);
        MonoInst *iargs[4];
        ...
        EMIT_NEW_ICONST(cfg, iargs[3],
                        m_class_is_valuetype(klass)
                            ...
                            : field->offset);
        if (cfg->opt & MONO_OPT_INLINE || cfg->compile_aot) {
          costs = inline_method(cfg, wrapper,
                                mono_method_signature_internal(wrapper), iargs,
                                ip, cfg->real_offset, TRUE);
          CHECK_CFG_EXCEPTION;
          g_assert(costs > 0);
        ...
        if (sp[0]->type == STACK_VTYPE) {
          ...
          if (!var)
            var = mono_compile_create_var_for_vreg(
                cfg, m_class_get_byval_arg(klass), OP_LOCAL, sp[0]->dreg);
          else
        ...
        if (il_op == MONO_CEE_LDFLDA) {
          ...
          if (mini_is_gsharedvt_klass(klass)) {
            ...
            EMIT_NEW_BIALU_IMM(cfg, ins, OP_PADD_IMM, dreg, sp[0]->dreg,
                               foffset);
          }
          ...
          *sp++ = ins;
          ...
          if (sp[0]->opcode == OP_LDADDR && m_class_is_simd_type(klass) &&
          ...
          if (mini_is_gsharedvt_klass(klass)) {
            ...
            EMIT_NEW_BIALU_IMM(cfg, ins, OP_PSUB_IMM, offset_ins->dreg,
                               offset_ins->dreg, 1);
            EMIT_NEW_BIALU(cfg, field_add_inst, OP_PADD, alloc_ireg_mp(cfg),
                           sp[0]->dreg, offset_ins->dreg);
            foffset = 0;
          }

          load = mini_emit_memory_load(cfg, field->type, field_add_inst,
                                       foffset, ins_flag);
      ...
      if (ftype->attrs & FIELD_ATTRIBUTE_LITERAL) {
        mono_error_set_field_missing(
            cfg->error, field->parent, field->name, NULL,
            "Using static instructions with literal field");
        CHECK_CFG_ERROR;
      }

      /* The special_static_fields field is init'd in mono_class_vtable, so it
      ...
      if (is_special_static && ((gsize)addr & 0x80000000) == 0 && thread_ins &&
        ...
        MONO_EMIT_NEW_LOAD_MEMBASE(
            cfg, static_data_reg, thread_ins->dreg,
            MONO_STRUCT_OFFSET(MonoInternalThread, static_data));
        ...
        if (cfg->compile_aot || context_used) {
          ...
          if (context_used) {
            MonoInst *addr_ins = emit_get_rgctx_field(
                cfg, context_used, field, MONO_RGCTX_INFO_FIELD_OFFSET);
            /* The value is offset by 1 */
            EMIT_NEW_BIALU_IMM(cfg, ins, OP_PSUB_IMM, addr_ins->dreg,
                               addr_ins->dreg, 1);
          } else {
            EMIT_NEW_SFLDACONST(cfg, ins, field);
          }
          offset_reg = ins->dreg;
          MONO_EMIT_NEW_BIALU_IMM(cfg, OP_IAND_IMM, offset_reg, offset_reg,
                                  0x7fffffff);
          idx_reg = alloc_ireg(cfg);
          MONO_EMIT_NEW_BIALU_IMM(cfg, OP_IAND_IMM, idx_reg, offset_reg, 0x3f);
          MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ISHL_IMM, idx_reg, idx_reg,
                                  TARGET_SIZEOF_VOID_P == 8 ? 3 : 2);
          MONO_EMIT_NEW_BIALU(cfg, OP_PADD, static_data_reg, static_data_reg,
                              idx_reg);
          array_reg = alloc_ireg(cfg);
          MONO_EMIT_NEW_LOAD_MEMBASE(cfg, array_reg, static_data_reg, 0);
          offset2_reg = alloc_ireg(cfg);
          MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ISHR_UN_IMM, offset2_reg, offset_reg,
                                  6);
          MONO_EMIT_NEW_BIALU_IMM(cfg, OP_IAND_IMM, offset2_reg, offset2_reg,
                                  0x1ffffff);
          dreg = alloc_ireg(cfg);
          ...
          array_reg = alloc_ireg(cfg);
          MONO_EMIT_NEW_LOAD_MEMBASE(cfg, array_reg, static_data_reg,
                                     idx * TARGET_SIZEOF_VOID_P);
          dreg = alloc_ireg(cfg);
          EMIT_NEW_BIALU_IMM(cfg, ins, OP_ADD_IMM, dreg, array_reg,
                             ((offset >> 6) & 0x1ffffff));
        }
        ...
        EMIT_NEW_DOMAINCONST(cfg, iargs[0]);
        if (context_used) {
          iargs[1] = emit_get_rgctx_field(cfg, context_used, field,
        ...
        /*
        g_print ("sharing static field access in %s.%s.%s - depth %d offset
        ...
        */
         ...
         */
        static_data = mini_emit_get_rgctx_klass(cfg, context_used, klass,
                                                MONO_RGCTX_INFO_STATIC_DATA);

        if (mini_is_gsharedvt_klass(klass)) {
          MonoInst *offset_ins;

          offset_ins = emit_get_rgctx_field(cfg, context_used, field,
                                            MONO_RGCTX_INFO_FIELD_OFFSET);
          /* The value is offset by 1 */
          EMIT_NEW_BIALU_IMM(cfg, ins, OP_PSUB_IMM, offset_ins->dreg,
                             offset_ins->dreg, 1);
          dreg = alloc_ireg_mp(cfg);
          EMIT_NEW_BIALU(cfg, ins, OP_PADD, dreg, static_data->dreg,
                         offset_ins->dreg);
        } else if (field->offset == 0) {
          ins = static_data;
        } else {
          int addr_reg = mono_alloc_preg(cfg);
          EMIT_NEW_BIALU_IMM(cfg, ins, OP_PADD_IMM, addr_reg, static_data->dreg,
                             field->offset);
        }
        ...
        if (!addr) {
          ...
          if (cfg->compile_aot)
            ...
            g_assert(vtable);
            addr = (char *)mono_vtable_get_static_field_data(vtable) +
                   field->offset;
            g_assert(addr);
      ...
      if ((il_op == MONO_CEE_STFLD || il_op == MONO_CEE_STSFLD)) {
        if (ins_flag & MONO_INST_VOLATILE) {
          /* Volatile stores have release semantics, see 12.6.7 in Ecma 335 */
          mini_emit_memory_barrier(cfg, MONO_MEMORY_BARRIER_REL);
        } else if (!mini_debug_options.weak_memory_model &&
      ...
      if (il_op == MONO_CEE_LDSFLDA) {
        ...
        *sp++ = ins;
        ...
        EMIT_NEW_STORE_MEMBASE_TYPE(cfg, store, ftype, ins->dreg, 0,
                                    store_val->dreg);
        store->flags |= ins_flag;
      } else {
        ...
        if ((ftype->attrs & FIELD_ATTRIBUTE_INIT_ONLY) &&
            (((addr = mono_aot_readonly_field_override(field)) != NULL) ||
             (!context_used &&
              !((cfg->opt & MONO_OPT_SHARED) || cfg->compile_aot) &&
              vtable->initialized))) {
          ...
          if (ro_type == MONO_TYPE_VALUETYPE &&
              m_class_is_enumtype(ftype->data.klass)) {
            ro_type =
                mono_class_enum_basetype_internal(ftype->data.klass)->type;
          }
    ...
    field_access_end:
      if ((il_op == MONO_CEE_LDFLD || il_op == MONO_CEE_LDSFLD) &&
    ...
    case MONO_CEE_STOBJ:
      ...
      /* FIXME: should check item at sp [1] is compatible with the type of the
       * store. */
      mini_emit_memory_store(cfg, m_class_get_byval_arg(klass), sp[0], sp[1],
                             ins_flag);
      ins_flag = 0;
      inline_costs += 1;
      break;
    ...
    case MONO_CEE_NEWARR: {
      ...
      if (sp[0]->type == STACK_I8 ||
          (TARGET_SIZEOF_VOID_P == 8 && sp[0]->type == STACK_PTR)) {
      ...
      if (context_used) {
        ...
        /* vtable */
        args[0] = mini_emit_get_rgctx_klass(cfg, context_used, array_class,
                                            MONO_RGCTX_INFO_VTABLE);
        /* array len */
        args[1] = sp[0];
      ...
      *sp++ = ins;
       ...
       * we should also allocate the array as not cleared, since we spend as
       * much time clearing to 0 as initializing for small sizes open code the
       * memcpy ensure the rva field is big enough
       */
      if ((cfg->opt & MONO_OPT_INTRINS) && next_ip < end &&
          ip_in_bb(cfg, cfg->cbb, next_ip) && (len_ins->opcode == OP_ICONST) &&
          (data_ptr = initialize_array_data(
               cfg, method, cfg->compile_aot, next_ip, end, klass,
               len_ins->inst_c0, &data_size, &field_token, &il_op, &next_ip))) {
        MonoMethod *memcpy_method = mini_get_memcpy_method();
        MonoInst *iargs[3];
        int add_reg = alloc_ireg_mp(cfg);

        EMIT_NEW_BIALU_IMM(cfg, iargs[0], OP_PADD_IMM, add_reg, ins->dreg,
                           MONO_STRUCT_OFFSET(MonoArray, vector));
        if (cfg->compile_aot) {
          EMIT_NEW_AOTCONST_TOKEN(cfg, iargs[1], MONO_PATCH_INFO_RVA,
                                  m_class_get_image(method->klass),
                                  GPOINTER_TO_UINT(field_token), STACK_PTR,
                                  NULL);
        } else {
    ...
    case MONO_CEE_LDELEMA:
      ...
      if (!m_class_is_valuetype(klass) &&
          method->wrapper_type == MONO_WRAPPER_NONE && !readonly) {
        MonoClass *array_class = mono_class_create_array(klass, 1);
    ...
    case MONO_CEE_CKFINITE: {
      ...
      if (cfg->llvm_only) {
        ...
        *sp++ = mono_emit_jit_icall(cfg, mono_ckfinite, iargs);
        ...
        sp[0] = convert_value(
            cfg, m_class_get_byval_arg(mono_defaults.double_class), sp[0]);
    ...
    case MONO_CEE_REFANYVAL: {
      ...
      if (!src_var)
        src_var = mono_compile_create_var_for_vreg(
            cfg, m_class_get_byval_arg(mono_defaults.typed_reference_class),
            OP_LOCAL, sp[0]->dreg);
      EMIT_NEW_VARLOADA(cfg, src, src_var, src_var->inst_vtype);
      MONO_EMIT_NEW_LOAD_MEMBASE(cfg, klass_reg, src->dreg,
                                 MONO_STRUCT_OFFSET(MonoTypedRef, klass));

      if (context_used) {
        MonoInst *klass_ins;

        klass_ins = mini_emit_get_rgctx_klass(cfg, context_used, klass,
                                              MONO_RGCTX_INFO_KLASS);
      ...
      EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOAD_MEMBASE, dreg, src->dreg,
                            MONO_STRUCT_OFFSET(MonoTypedRef, value));
      ins->type = STACK_MP;
      ins->klass = klass;
      *sp++ = ins;
      break;
    ...
    case MONO_CEE_MKREFANY: {
      ...
      loc = mono_compile_create_var(
          cfg, m_class_get_byval_arg(mono_defaults.typed_reference_class),
          OP_LOCAL);
      EMIT_NEW_TEMPLOADA(cfg, addr, loc->inst_c0);

      MonoInst *const_ins = mini_emit_get_rgctx_klass(cfg, context_used, klass,
                                                      MONO_RGCTX_INFO_KLASS);
      int type_reg = alloc_preg(cfg);

      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREP_MEMBASE_REG, addr->dreg,
                                  MONO_STRUCT_OFFSET(MonoTypedRef, klass),
                                  const_ins->dreg);
      MONO_EMIT_NEW_BIALU_IMM(cfg, OP_ADD_IMM, type_reg, const_ins->dreg,
                              m_class_offsetof_byval_arg());
      MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STOREP_MEMBASE_REG, addr->dreg,
                                  MONO_STRUCT_OFFSET(MonoTypedRef, type),
                                  type_reg);
    ...
    case MONO_CEE_LDTOKEN: {
      ...
      if (method->wrapper_type == MONO_WRAPPER_DYNAMIC_METHOD ||
        ...
        handle = mono_ldtoken_checked(image, n, &handle_class, generic_context,
                                      cfg->error);
        CHECK_CFG_ERROR;
      }
      if (!handle)
      ...
      if (cfg->gshared) {
        if (mono_metadata_token_table(n) == MONO_TABLE_TYPEDEF ||
          ...
          context_used = mini_class_check_context_used(
              cfg, mono_class_from_mono_type_internal((MonoType *)handle));
        } else if (handle_class == mono_defaults.fieldhandle_class)
          context_used = mini_class_check_context_used(
              cfg, ((MonoClassField *)handle)->parent);
        else if (handle_class == mono_defaults.methodhandle_class)
          context_used =
              mini_method_check_context_used(cfg, (MonoMethod *)handle);
        else
          g_assert_not_reached();
      }
      ...
      if ((cfg->opt & MONO_OPT_SHARED) &&
        ...
        vtvar = mono_compile_create_var(
            cfg, m_class_get_byval_arg(handle_class), OP_LOCAL);

        EMIT_NEW_IMAGECONST(cfg, iargs[0], image);
        EMIT_NEW_ICONST(cfg, iargs[1], n);
        if (method_context_used) {
          iargs[2] = emit_get_rgctx_method(cfg, method_context_used, method,
                                           MONO_RGCTX_INFO_METHOD);
          ins = mono_emit_jit_icall(cfg, mono_ldtoken_wrapper_generic_shared,
                                    iargs);
        } else {
        ...
        if ((next_ip + 4 < end) && ip_in_bb(cfg, cfg->cbb, next_ip) &&
            ...
            (cmethod = mini_get_method(cfg, method, read32(next_ip + 1), NULL,
                                       generic_context)) &&
          ...
          // Optimize to true/false if next instruction is `call instance bool
          // Type::get_IsValueType()`
          guchar *is_vt_ip;
          guint32 is_vt_token;
          if ((is_vt_ip = il_read_call(next_ip + 5, end, &is_vt_token)) &&
              ip_in_bb(cfg, cfg->cbb, is_vt_ip)) {
            MonoMethod *is_vt_method = mini_get_method(cfg, method, is_vt_token,
                                                       NULL, generic_context);
          ...
          if (context_used) {
            ins = mini_emit_get_rgctx_klass(cfg, context_used, tclass,
                                            MONO_RGCTX_INFO_REFLECTION_TYPE);
          } else if (cfg->compile_aot) {
            if (method->wrapper_type) {
              error_init(error); // got to do it since there are multiple
                                 // conditionals below
              if (mono_class_get_checked(m_class_get_image(tclass),
                                         m_class_get_type_token(tclass),
                                         error) == tclass &&
                  !generic_context) {
                /* Special case for static synchronized wrappers */
                EMIT_NEW_TYPE_FROM_HANDLE_CONST(
                    cfg, ins, m_class_get_image(tclass),
                    m_class_get_type_token(tclass), generic_context);
              } else {
                mono_error_cleanup(error); /* FIXME don't swallow the error */
            ...
            MonoReflectionType *rt = mono_type_get_object_checked(
                cfg->domain, (MonoType *)handle, cfg->error);
            CHECK_CFG_ERROR;
          ...
          if (context_used) {
            if (handle_class == mono_defaults.typehandle_class) {
              ins = mini_emit_get_rgctx_klass(
                  cfg, context_used,
                  mono_class_from_mono_type_internal((MonoType *)handle),
                  MONO_RGCTX_INFO_TYPE);
            } else if (handle_class == mono_defaults.methodhandle_class) {
              ins =
                  emit_get_rgctx_method(cfg, context_used, (MonoMethod *)handle,
                                        MONO_RGCTX_INFO_METHOD);
            } else if (handle_class == mono_defaults.fieldhandle_class) {
    ...
    case MONO_CEE_THROW:
      ...
      link_bblock(cfg, cfg->cbb, end_bblock);
      start_new_bblock = 1;
      /* This can complicate code generation for llvm since the return value
    ...
    case MONO_CEE_LEAVE_S: {
      ...
      for (i = 0; i < header->num_clauses; ++i) {
        ...
        if (MONO_OFFSET_IN_HANDLER(clause, ip - header->code) &&
            ...
            (ip - header->code + ((il_op == MONO_CEE_LEAVE) ? 5 : 2)) <=
                (clause->handler_offset + clause->handler_len) &&
            method->wrapper_type != MONO_WRAPPER_RUNTIME_INVOKE) {
          MonoInst *exc_ins;
          ...
          /*
            ...
            NEW_TEMPLOAD (cfg, load, mono_find_exvar_for_offset (cfg,
            clause->handler_offset)->inst_c0);
          */

          exc_ins = mono_emit_jit_icall(
              cfg, mono_thread_get_undeniable_exception, NULL);
      ...
      if ((handlers = mono_find_leave_clauses(cfg, ip, target))) {
        GList *tmp;
        /*
         * For each finally clause that we exit we need to invoke the finally
         * block. After each invocation we need to add try holes for all the
         * clauses that we already exited.
         */
        for (tmp = handlers; tmp; tmp = tmp->next) {
          ...
          MonoInst *abort_exc = (MonoInst *)mono_find_exvar_for_offset(
              cfg, clause->handler_offset);
          MonoBasicBlock *dont_throw;

          /*
           * Emit instrumentation code before linking the basic blocks below as
           * this will alter cfg->cbb.
           */
          mini_profiler_emit_call_finally(cfg, header, ip, leave->index,
                                          clause);
          ...
          if (COMPILE_LLVM(cfg)) {
             ...
             */
            GET_BBLOCK(cfg, tblock,
                       cfg->cil_start + clause->handler_offset +
                           clause->handler_len - 1);
    ...
    case MONO_CEE_MONO_ICALL: {
      ...
      if (token == MONO_JIT_ICALL_mono_threads_attach_coop) {
        ...
        if (cfg->compile_aot) {
          /*
           * This is called on unattached threads, so it cannot go through the
           * trampoline infrastructure. Use an indirect call through a got slot
        ...
        /*
         * Parts of the initlocals code needs to come after this, since it might
         * call methods like memset.
         */
    ...
    case MONO_CEE_MONO_ICALL_ADDR: {
      ...
      if (cfg->compile_aot) {
        if (cfg->direct_pinvoke && ip + 6 < end && (ip[6] == CEE_POP)) {
          /*
           * This is generated by emit_native_wrapper () to resolve the pinvoke
           * address before the call, its not needed when using direct pinvoke.
           * This is not an optimization, but its used to avoid looking up
           * pinvokes on platforms which don't support dlopen ().
           */
          EMIT_NEW_PCONST(cfg, ins, NULL);
        } else {
    ...
    case MONO_CEE_MONO_LDNATIVEOBJ:
...

      {
        ...
        src = sp[0];
        temp = mono_compile_create_var(cfg, m_class_get_byval_arg(klass),
                                       OP_LOCAL);
    ...
    case MONO_CEE_MONO_LD_DELEGATE_METHOD_PTR: {
      ...
      dreg = alloc_preg(cfg);
      EMIT_NEW_LOAD_MEMBASE(cfg, ins, OP_LOAD_MEMBASE, dreg, sp[0]->dreg,
                            MONO_STRUCT_OFFSET(MonoDelegate, method_ptr));
      *sp++ = ins;
      break;
    }
    case MONO_CEE_MONO_CALLI_EXTRA_ARG: {
      ...
      if (cfg->llvm_only) {
        /*
         * The lowest bit of 'arg' determines whenever the callee uses the
         * gsharedvt cconv. This is set by mono_init_delegate ().
         */
        if (cfg->gsharedvt && mini_is_gsharedvt_variable_signature(fsig)) {
          ...
          addr = emit_get_rgctx_sig(
              ...
              MONO_RGCTX_INFO_SIG_GSHAREDVT_OUT_TRAMPOLINE_CALLI);
          /*
           * ADDR points to a gsharedvt-out wrapper, have to pass <callee, arg>
           * as an extra arg.
           */
          MONO_INST_NEW(cfg, ins, OP_LOCALLOC_IMM);
          ins->dreg = alloc_preg(cfg);
          ...
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG,
                                      localloc_ins->dreg, 0, callee->dreg);
          MONO_EMIT_NEW_STORE_MEMBASE(cfg, OP_STORE_MEMBASE_REG,
                                      localloc_ins->dreg, TARGET_SIZEOF_VOID_P,
                                      arg->dreg);

          call = mini_emit_extra_arg_calli(cfg, fsig, sp, localloc_ins->dreg,
                                           addr);
          MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_BR, end_bb);

          /* Gsharedvt case: callee uses a gsharedvt cconv, no conversion is
           * needed */
          ...
          MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_BR, end_bb);
          /* Gsharedvt case: callee uses a gsharedvt cconv, have to add an in
           * wrapper */
          MONO_START_BB(cfg, is_gsharedvt_bb);
          MONO_EMIT_NEW_BIALU_IMM(cfg, OP_PXOR_IMM, arg->dreg, arg->dreg, 1);
          ...
          MONO_ADD_INS(cfg->cbb, addr);
          /*
           * ADDR points to a gsharedvt-in wrapper, have to pass <callee, arg>
           * as an extra arg.
           */
          MONO_INST_NEW(cfg, ins, OP_LOCALLOC_IMM);
          ins->dreg = alloc_preg(cfg);
          ...
          ins = mini_emit_extra_arg_calli(cfg, fsig, sp, localloc_ins->dreg,
                                          addr);
          ins->dreg = call->dreg;
          MONO_EMIT_NEW_BRANCH_BLOCK(cfg, OP_BR, end_bb);
    ...
    case MONO_CEE_ARGLIST: {
      ...
      MonoInst *addr, *vtvar;
      vtvar = mono_compile_create_var(
          cfg, m_class_get_byval_arg(mono_defaults.argumenthandle_class),
          OP_LOCAL);
    ...
    case MONO_CEE_CLT_UN: {
      ...
      if ((arg1->type == STACK_I8) ||
          ((TARGET_SIZEOF_VOID_P == 8) &&
           ((arg1->type == STACK_PTR) || (arg1->type == STACK_OBJ) ||
            (arg1->type == STACK_MP))))
        cmp->opcode = OP_LCOMPARE;
    ...
    case MONO_CEE_LDFTN: {
       ...
       */
      if ((sp > stack_start) && (next_ip + 4 < end) &&
          ip_in_bb(cfg, cfg->cbb, next_ip) && (next_ip[0] == CEE_NEWOBJ)) {
        MonoMethod *ctor_method = mini_get_method(
            cfg, method, read32(next_ip + 1), NULL, generic_context);
        if (ctor_method && (m_class_get_parent(ctor_method->klass) ==
                            mono_defaults.multicastdelegate_class)) {
          ...
          if (!(cmethod->flags & METHOD_ATTRIBUTE_STATIC)) {
            /*LAME IMPL: We must not add a null check for virtual invoke
             * delegates.*/
            if (mono_method_signature_internal(invoke)->param_count ==
                mono_method_signature_internal(cmethod)->param_count) {
              MONO_EMIT_NEW_BIALU_IMM(cfg, OP_COMPARE_IMM, -1, target_ins->dreg,
                                      0);
          ...
          if ((invoke_context_used == 0 || !cfg->gsharedvt) || cfg->llvm_only) {
            if (cfg->verbose_level > 3)
              g_print("converting (in B%d: stack: %d) %s", cfg->cbb->block_num,
                      (int)(sp - stack_start),
                      mono_disasm_code_one(NULL, method, ip + 6, NULL));
            if ((handle_ins = handle_delegate_ctor(
                     cfg, ctor_method->klass, target_ins, cmethod, context_used,
                     invoke_context_used, FALSE))) {
              sp--;
              *sp = handle_ins;
              CHECK_CFG_EXCEPTION;
              sp++;
      ...
      argconst = emit_get_rgctx_method(cfg, context_used, cmethod,
                                       MONO_RGCTX_INFO_METHOD);
    ...
    case MONO_CEE_LDVIRTFTN: {
       ...
       */
      if (previous_il_op == MONO_CEE_DUP && (sp > stack_start) &&
          (next_ip + 4 < end) && ip_in_bb(cfg, cfg->cbb, next_ip) &&
          (next_ip[0] == CEE_NEWOBJ)) {
        MonoMethod *ctor_method = mini_get_method(
            cfg, method, read32(next_ip + 1), NULL, generic_context);
        if (ctor_method && (m_class_get_parent(ctor_method->klass) ==
                            mono_defaults.multicastdelegate_class)) {
          ...
          MonoMethod *invoke;
          int invoke_context_used;
          const gboolean is_virtual =
              (cmethod->flags & METHOD_ATTRIBUTE_VIRTUAL) != 0;
          ...
          if (invoke_context_used == 0 || !cfg->gsharedvt || cfg->llvm_only) {
            ...
            if ((handle_ins = handle_delegate_ctor(
                     cfg, ctor_method->klass, target_ins, cmethod, context_used,
                     invoke_context_used, is_virtual))) {
              sp -= 2;
              *sp = handle_ins;
              CHECK_CFG_EXCEPTION;
              next_ip += 5;
    ...
    case MONO_CEE_ENDFILTER: {
      ...
      for (cc = 0; cc < header->num_clauses; ++cc) {
        ...
        if ((clause->flags & MONO_EXCEPTION_CLAUSE_FILTER) &&
            ((next_ip - header->code) > clause->data.filter_offset &&
             (next_ip - header->code) <= clause->handler_offset) &&
            (!nearest ||
             (clause->data.filter_offset < nearest->data.filter_offset)))
          nearest = clause;
      }
      g_assert(nearest);
    ...
    case MONO_CEE_INITOBJ:
      ...
      CHECK_TYPELOAD(klass);
      if (mini_class_is_reference(klass))
        MONO_EMIT_NEW_STORE_MEMBASE_IMM(cfg, OP_STORE_MEMBASE_IMM, sp[0]->dreg,
                                        0, 0);
      else
    ...
    case MONO_CEE_RETHROW: {
      ...
      EMIT_NEW_TEMPLOAD(
          cfg, load, mono_find_exvar_for_offset(cfg, handler_offset)->inst_c0);
    ...
    case MONO_CEE_SIZEOF: {
      ...
      if (mono_metadata_token_table(token) == MONO_TABLE_TYPESPEC &&
          !image_is_dynamic(m_class_get_image(method->klass)) &&
          !generic_context) {
        MonoType *type =
            mono_type_create_from_typespec_checked(image, token, cfg->error);
        CHECK_CFG_ERROR;
        ...
        if (mini_is_gsharedvt_klass(klass)) {
          ins = mini_emit_get_gsharedvt_info_klass(
              cfg, klass, MONO_RGCTX_INFO_CLASS_SIZEOF);
          ins->type = STACK_I4;
        } else {
    ...
    case MONO_CEE_REFANYTYPE: {
      ...
      EMIT_NEW_VARLOADA(cfg, src, src_var, src_var->inst_vtype);
      EMIT_NEW_LOAD_MEMBASE_TYPE(
          cfg, ins, m_class_get_byval_arg(mono_defaults.typehandle_class),
          src->dreg, MONO_STRUCT_OFFSET(MonoTypedRef, type));
      *sp++ = ins;
      break;
    }
  ...
  if (init_localsbb) {
    ...
    for (i = 0; i < header->num_locals; ++i) {
      /*
       * Vtype initialization might need to be done after CEE_JIT_ATTACH, since
       * it can make calls to memset (), which need the trampoline code to work.
       */
  ...
  /*
   * Add seq points for IL offsets which have line number info, but wasn't
   * generated a seq point during JITting because the code they refer to was
   * dead (#11880).
   */
  if (sym_seq_points) {
    for (i = 0; i < header->code_size; ++i) {
      if (mono_bitset_test_fast(seq_point_locs, i) &&
          !mono_bitset_test_fast(seq_point_set_locs, i)) {
        MonoInst *ins;
  ...
  if (cfg->method == method) {
    ...
    MonoBasicBlock *bb;
    /* get_most_deep_clause () in mini-llvm.c depends on this for inlined
     * bblocks */
  ...
  if (inline_costs < 0) {
    ...
    mono_cfg_set_exception_invalid_program(
        cfg, g_strdup_printf("Method %s is too complex.", mname));
    g_free(mname);
  }
...
static int op_to_op_dest_membase(int store_opcode, int opcode) {
#if defined(TARGET_X86)
  if (!((store_opcode == OP_STORE_MEMBASE_REG) ||
...
static int op_to_op_store_membase(int store_opcode, int opcode) {
#if defined(TARGET_X86) || defined(TARGET_AMD64)
  switch (opcode) {
  case OP_ICEQ:
...
static int op_to_op_src1_membase(MonoCompile *cfg, int load_opcode,
                                 int opcode) {
#ifdef TARGET_X86
  ...
  */
  ...
  switch (opcode) {
  case OP_X86_PUSH:
    if ((load_opcode == OP_LOAD_MEMBASE && !cfg->backend->ilp32) ||
  ...
  */
  case OP_ICOMPARE_IMM:
    if ((load_opcode == OP_LOADI4_MEMBASE) ||
        (load_opcode == OP_LOADU4_MEMBASE))
      return OP_AMD64_ICOMPARE_MEMBASE_IMM;
    break;
  case OP_COMPARE:
...
static int op_to_op_src2_membase(MonoCompile *cfg, int load_opcode,
                                 int opcode) {
#ifdef TARGET_X86
  if (!((load_opcode == OP_LOAD_MEMBASE) ||
        (load_opcode == OP_LOADI4_MEMBASE) ||
  ...
  if ((load_opcode == OP_LOADI4_MEMBASE) ||
      (load_opcode == OP_LOADU4_MEMBASE) ||
      (load_opcode == OP_LOAD_MEMBASE && cfg->backend->ilp32)) {
    switch (opcode) {
    case OP_ICOMPARE:
...
void mono_handle_global_vregs(MonoCompile *cfg) {
  ...
  vreg_to_bb = (gint32 *)mono_mempool_alloc0(
      cfg->mempool, sizeof(gint32 *) * cfg->next_vreg + 1);
        ...
        if (regtype == 'l' && !COMPILE_LLVM(cfg)) {
          ...
          if (!get_vreg_to_inst(cfg, vreg)) {
            mono_compile_create_var_for_vreg(
                cfg, m_class_get_byval_arg(mono_defaults.int64_class), OP_LOCAL,
                vreg);
           ...
           */
          get_vreg_to_inst(cfg, MONO_LVREG_LS(vreg))->flags |=
              MONO_INST_VOLATILE;
          get_vreg_to_inst(cfg, MONO_LVREG_MS(vreg))->flags |=
              MONO_INST_VOLATILE;
        }
#endif
          ...
          if (!get_vreg_to_inst(cfg, vreg)) {
            if (G_UNLIKELY(cfg->verbose_level > 2))
              printf("VREG R%d used in BB%d and BB%d made global.\n", vreg,
                     vreg_to_bb[vreg], block_num);

            switch (regtype) {
            case 'i':
              if (vreg_is_ref(cfg, vreg))
                mono_compile_create_var_for_vreg(cfg, mono_get_object_type(),
                                                 OP_LOCAL, vreg);
              else
                mono_compile_create_var_for_vreg(cfg, mono_get_int_type(),
                                                 OP_LOCAL, vreg);
              break;
            case 'l':
              mono_compile_create_var_for_vreg(
                  cfg, m_class_get_byval_arg(mono_defaults.int64_class),
                  OP_LOCAL, vreg);
              break;
            case 'f':
              mono_compile_create_var_for_vreg(
                  cfg, m_class_get_byval_arg(mono_defaults.double_class),
                  OP_LOCAL, vreg);
              break;
            case 'v':
            case 'x':
              mono_compile_create_var_for_vreg(
                  cfg, m_class_get_byval_arg(ins->klass), OP_LOCAL, vreg);
              break;
            default:
              g_assert_not_reached();
            }
  ...
  for (i = 0; i < cfg->num_varinfo; i++) {
    ...
    case STACK_R8:
      ...
      /*
      if (var->type == STACK_VTYPE && cfg->gsharedvt &&
      mini_is_gsharedvt_variable_type (var->inst_vtype)) break;
      */

      /* Arguments are implicitly global */
      /* Putting R4 vars into registers doesn't work currently */
      /* The gsharedvt vars are implicitly referenced by ldaddr opcodes, but
       * those opcodes are only generated later */
      if ((var->opcode != OP_ARG) && (var != cfg->ret) &&
          !(var->flags & (MONO_INST_VOLATILE | MONO_INST_INDIRECT)) &&
          (vreg_to_bb[var->dreg] != -1) &&
          (m_class_get_byval_arg(var->klass)->type != MONO_TYPE_R4) &&
          !cfg->disable_vreg_to_lvreg && var != cfg->gsharedvt_info_var &&
          var != cfg->gsharedvt_locals_var && var != cfg->lmf_addr_var) {
        /*
         * Make that the variable's liveness interval doesn't contain a call,
         * since that would cause the lvreg to be spilled, making the whole
  ...
  /*
   * Compress the varinfo and vars tables so the liveness computation is faster
 ...
 *
 *   Allocate variables with gsharedvt types to entries in the
 * MonoGSharedVtMethodRuntimeInfo.entries array. Initialize
 * cfg->gsharedvt_vreg_to_idx with the mapping between vregs and indexes.
 */
void mono_allocate_gsharedvt_vars(MonoCompile *cfg) {
  int i;

  cfg->gsharedvt_vreg_to_idx =
      (int *)mono_mempool_alloc0(cfg->mempool, sizeof(int) * cfg->next_vreg);
  ...
  for (i = 0; i < cfg->num_varinfo; ++i) {
    ...
    if (mini_is_gsharedvt_variable_type(ins->inst_vtype)) {
      if (i >= cfg->locals_start) {
        /* Local */
        idx = get_gsharedvt_info_slot(cfg, ins->inst_vtype,
 ...
 * and replace vregs with their allocated hregs. *need_local_opts is set to TRUE
 * if code is generated which could be optimized by the local optimization
 * passes.
 */
void mono_spill_global_vars(MonoCompile *cfg, gboolean *need_local_opts) {
  ...
  *need_local_opts = FALSE;
  ...
  /*
   * As an optimization, when a variable allocated to the stack is first loaded
   * into an lvreg, we will remember the lvreg and use it the next time instead
   * of loading the variable again.
   */
  orig_next_vreg = cfg->next_vreg;
  vreg_to_lvreg = (guint32 *)mono_mempool_alloc0(
      cfg->mempool, sizeof(guint32) * cfg->next_vreg);
  lvregs_size = 1024;
  lvregs = (guint32 *)mono_mempool_alloc(cfg->mempool,
                                         sizeof(guint32) * lvregs_size);
  lvregs_len = 0;
   ...
   * The live range is computed using the liveness info computed by the liveness
   * pass. We can't use vmv->range, since that is an abstract live range, and we
   * need one which is instruction precise.
   * FIXME: Variables used in out-of-line bblocks have a hole in their live
   * range.
   */
  ...
  for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
    ...
    MONO_BB_FOR_EACH_INS(bb, ins) {
        ...
        if (var->opcode == OP_VTARG_ADDR) {
          ...
          if (vtaddr->opcode == OP_REGVAR) {
            ...
            NOT_IMPLEMENTED;
                   ...
                   cfg->gsharedvt_vreg_to_idx[var->dreg] < 0) {
          ...
          /*
           * gsharedvt local.
           * Compute the address of the local as gsharedvt_locals_var +
           * gsharedvt_info_var->locals_offsets [idx].
           */
          ...
          if (info_var->opcode == OP_REGOFFSET) {
            reg1 = alloc_ireg(cfg);
            NEW_LOAD_MEMBASE(cfg, load, OP_LOAD_MEMBASE, reg1,
                             info_var->inst_basereg, info_var->inst_offset);
          ...
          reg2 = alloc_ireg(cfg);
          NEW_LOAD_MEMBASE(
              cfg, load2, OP_LOADI4_MEMBASE, reg2, reg1,
              MONO_STRUCT_OFFSET(MonoGSharedVtMethodRuntimeInfo, entries) +
                  (idx * TARGET_SIZEOF_VOID_P));
          /* Load the locals area address */
          reg3 = alloc_ireg(cfg);
          if (locals_var->opcode == OP_REGOFFSET) {
            NEW_LOAD_MEMBASE(cfg, load3, OP_LOAD_MEMBASE, reg3,
                             locals_var->inst_basereg, locals_var->inst_offset);
      ...
      if ((ins->dreg != -1) && get_vreg_to_inst(cfg, ins->dreg)) {
        ...
        if (var->opcode == OP_REGVAR) {
          ins->dreg = var->dreg;
        } else if ((ins->dreg == ins->sreg1) && (spec[MONO_INST_DEST] == 'i') &&
                   (spec[MONO_INST_SRC1] == 'i') && !vreg_to_lvreg[ins->dreg] &&
                   (op_to_op_dest_membase(store_opcode, ins->opcode) != -1)) {
          /*
          ...
          if (COMPILE_SOFT_FLOAT(cfg) &&
              store_opcode == OP_STORER8_MEMBASE_REG) {
            regtype = 'l';
            store_opcode = OP_STOREI8_MEMBASE_REG;
          }
...
#if SIZEOF_REGISTER != 8
          if (regtype == 'l') {
            NEW_STORE_MEMBASE(cfg, store_ins, OP_STOREI4_MEMBASE_REG,
                              var->inst_basereg,
                              var->inst_offset + MINI_LS_WORD_OFFSET,
                              MONO_LVREG_LS(ins->dreg));
            mono_bblock_insert_after_ins(bb, ins, store_ins);
            NEW_STORE_MEMBASE(cfg, store_ins, OP_STOREI4_MEMBASE_REG,
                              var->inst_basereg,
                              var->inst_offset + MINI_MS_WORD_OFFSET,
                              MONO_LVREG_MS(ins->dreg));
            mono_bblock_insert_after_ins(bb, ins, store_ins);
            def_ins = store_ins;
          } else
#endif
          {
            ...
            if (!lvreg &&
                ((ins->opcode == OP_ICONST) ||
                 ((ins->opcode == OP_I8CONST) && (ins->inst_c0 == 0)))) {
              ins->opcode =
              ...
              spec = INS_INFO(ins->opcode);
                       ...
                       ((ins->opcode == OP_MOVE) || (ins->opcode == OP_FMOVE) ||
                        (ins->opcode == OP_LMOVE) ||
                        (ins->opcode == OP_RMOVE))) {
              ...
              spec2[MONO_INST_SRC3] = ' ';
              spec = spec2;
            } else if (!lvreg && (op_to_op_store_membase(store_opcode,
                                                         ins->opcode) != -1)) {
              ...
              NEW_STORE_MEMBASE(cfg, store_ins, store_opcode, var->inst_basereg,
                                var->inst_offset, ins->dreg);
               ...
               * the sregs.
               */
              if ((!cfg->backend->use_fpstack ||
                   ((store_opcode != OP_STORER8_MEMBASE_REG) &&
                    (store_opcode != OP_STORER4_MEMBASE_REG))) &&
                  !((var)->flags & (MONO_INST_VOLATILE | MONO_INST_INDIRECT)))
                dest_has_lvreg = TRUE;
            }
          }
      ...
      for (srcindex = 0; srcindex < 3; ++srcindex) {
        ...
        if ((sreg != -1) && get_vreg_to_inst(cfg, sreg)) {
          ...
          if ((srcindex == 0) &&
              (op_to_op_src1_membase(cfg, load_opcode, ins->opcode) != -1)) {
            ins->opcode = op_to_op_src1_membase(cfg, load_opcode, ins->opcode);
            sregs[0] = var->inst_basereg;
            // mono_inst_set_src_registers (ins, sregs);
            ins->inst_offset = var->inst_offset;
          } else if ((srcindex == 1) &&
                     (op_to_op_src2_membase(cfg, load_opcode, ins->opcode) !=
                      -1)) {
            ...
            if (MONO_IS_REAL_MOVE(ins)) {
              ...
              if ((!cfg->backend->use_fpstack ||
                   ((load_opcode != OP_LOADR8_MEMBASE) &&
                    (load_opcode != OP_LOADR4_MEMBASE))) &&
                  !((var)->flags & (MONO_INST_VOLATILE | MONO_INST_INDIRECT)) &&
                  !no_lvreg) {
                ...
                if (var->dreg == dreg_using_dest_to_membase_op) {
                  if (cfg->verbose_level > 2)
                    printf("\tCan't cache R%d because it's part of a dreg "
                           "dest_membase optimization\n",
                           var->dreg);
                } else {
                  vreg_to_lvreg[var->dreg] = sreg;
                }
                if (lvregs_len >= lvregs_size) {
                  guint32 *new_lvregs = mono_mempool_alloc0(
                      cfg->mempool, sizeof(guint32) * lvregs_size * 2);
...
#if SIZEOF_REGISTER != 8
            if (regtype == 'l') {
              NEW_LOAD_MEMBASE(cfg, load_ins, OP_LOADI4_MEMBASE,
                               MONO_LVREG_MS(sreg), var->inst_basereg,
                               var->inst_offset + MINI_MS_WORD_OFFSET);
              mono_bblock_insert_before_ins(bb, ins, load_ins);
              NEW_LOAD_MEMBASE(cfg, load_ins, OP_LOADI4_MEMBASE,
                               MONO_LVREG_LS(sreg), var->inst_basereg,
                               var->inst_offset + MINI_LS_WORD_OFFSET);
              mono_bblock_insert_before_ins(bb, ins, load_ins);
              use_ins = load_ins;
            } else
...
#endif
              NEW_LOAD_MEMBASE(cfg, load_ins, load_opcode, sreg,
                               var->inst_basereg, var->inst_offset);
              mono_bblock_insert_before_ins(bb, ins, load_ins);
              use_ins = load_ins;
            }
            if (cfg->verbose_level > 2)
    ...
    if (cfg->compute_precise_live_ranges && bb->live_out_set && bb->code) {
      for (i = 0; i < cfg->num_varinfo; i++) {
        ...
        if (mono_bitset_test_fast(bb->live_in_set, i) &&
            !live_range_start[vi->vreg]) {
   ...
   * by storing the current native offset into
   * MonoMethodVar->live_range_start/end.
   */
 ...
 *     code wouldn't need if (fp) all over the place. but that would mean the
 * hregs would no longer map to the machine hregs, so the code generators would
 * need to be modified. Also, on ia64 for example, niregs + nfregs > 256 ->
 * bitmasks wouldn't work any more. Duplicating the code in mono_local_regalloc
 * () into fp/non-fp branches speeds it up by about 15%.
 ...
 * - use vregs for bb->out_stacks if possible, handle_global_vreg will make them
 * a variable if needed.
 * - do not start a new IL level bblock when cfg->cbb is changed by a function
 * call like inline_method.
 ...
 * - optimize the loading of the interruption flag in the managed->native
 * wrappers
 ...
 * - dealing with the increase in code size due to branches created during
 * opcode decomposition:
 *   - use extended basic blocks
 *     - all parts of the JIT
 *     - handle_global_vregs () && local regalloc
 *   - avoid introducing global vregs during decomposition, like 'vtable' in
 * isinst
 ...
 * - call cctors outside the JIT, to make -v output more readable and JIT
 * timings more meaningful.
 * - check for fp stack leakage in other opcodes too. (-> 'exceptions'
 * optimization)
 ...
 * - fix the pessimistic handling of variables accessed in exception handler
 * blocks.
 * - need to write a tree optimization pass, but the creation of trees is
 * difficult, i.e. parts of the tree could be separated by other instructions,
 * killing the tree arguments, or stores killing loads etc. Also, should we fold
 * loads into other instructions if the result of the load is used multiple
 * times ?
 * - make the REM_IMM optimization in mini-x86.c arch-independent.
 * - LAST MERGE: 108395.
 * - when returning vtypes in registers, generate IR and append it to the end of
 * the last bb instead of doing it in the epilog.
 * - change the store opcodes so they use sreg1 instead of dreg to store the
 * base register.
 */
...
- When to decompose opcodes:
  - earlier: this makes some optimizations hard to implement, since the low
level IR no longer contains the necessary information. But it is easier to do.
...
- Local/global vregs:
  - local vregs: temporary vregs used inside one bblock. Assigned to hregs by
