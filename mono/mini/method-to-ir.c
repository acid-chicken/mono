      ...
      */
      ...
      if (mini_class_is_system_array(cmethod->klass)) {
        *sp = emit_get_rgctx_method(cfg, context_used, cmethod,
         ...
         * int32) for an array with rank 1 represents length and lbound. */
        g_assert(
            !(rank == 1 && fsig->param_count == 2 &&
    ...
    case MONO_CEE_LDFTN: {
      ...
      if ((sp > stack_start) && (next_ip + 4 < end) &&
        ...
        if (ctor_method && (m_class_get_parent(ctor_method->klass) ==
          ...
          if (!(cmethod->flags & METHOD_ATTRIBUTE_STATIC)) {
            /*BAD IMPL: We must not add a null check for virtual invoke
             * delegates.*/
