...
MONO_LLVM_INTERNAL void mono_llvm_emit_call(MonoCompile *cfg,
                                            MonoCallInst *call);
MONO_LLVM_INTERNAL void mono_llvm_create_aot_module(MonoAssembly *assembly,
                                                    const char *global_prefix,
                                                    int initial_got_size,
                                                    LLVMModuleFlags flags);
MONO_LLVM_INTERNAL void mono_llvm_emit_aot_module(const char *filename,
                                                  const char *cu_name);
MONO_LLVM_INTERNAL void mono_llvm_emit_aot_file_info(MonoAotFileInfo *info,
                                                     gboolean has_jitted_code);
MONO_LLVM_INTERNAL void mono_llvm_emit_aot_data(const char *symbol,
                                                guint8 *data, int data_len);
