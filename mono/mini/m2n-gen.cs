...
class Driver {
  ...
  static void Main(string[] args) {
    ...
    if (args.Any(a => a == "--flat")) {
      for (int i = 0; i < cookies.Length; ++i) {
        ...
        Console.WriteLine(
            $"\t\twasm_invoke_{c.ToLower ()} (target_func, margs);");
      }
  ...
  static void WritePartition(IEnumerable<string>set, int pos = 0,
                             int depth = 0) {
    var prefix = "\t";
    for (var c = 0; c < pos; c++)
      prefix += "\t";
    ...
    foreach(var g in groups) {
      Console.WriteLine(
          $"{prefix}{Elif (checks)} (cookie[{pos}] == '{g.Key}') {{");
    ...
    if (hits.Any()) {
      Console.WriteLine(
          $"{prefix}{Elif (checks++)} (cookie[{pos}] == '\\0') {{");

      var h = hits.First();
      Console.WriteLine(
          $"{prefix}\t// found: {h} depth {pos + checks + depth}");
      Console.WriteLine(
