...
partial class Array {
  ...
  public int Length { [Intrinsic] get => Length; }
  ...
  public int Rank { [Intrinsic] get => Rank; }
  ...
  private static void Copy(Array sourceArray, int sourceIndex,
                           Array destinationArray, int destinationIndex,
    ...
    if (FastCopy(sourceArray, sourceIndex, destinationArray, destinationIndex,
                 length))
      return;
    ...
    if (source_pos > sourceArray.Length - length)
      throw new ArgumentException(SR.Arg_LongerThanSrcArray,
                                  nameof(sourceArray));
    ...
    var dst_type_vt =
        dst_type.IsValueType && Nullable.GetUnderlyingType(dst_type) == null;
    ...
    if (!Object.ReferenceEquals(sourceArray, destinationArray) ||
        source_pos > dest_pos) {
  ...
  static bool CanAssignArrayElement(Type source, Type target) {
    if (!target.IsValueType && !target.IsPointer) {
      if (!source.IsValueType && !source.IsPointer) {
        ...
        return source.IsInterface || target.IsInterface ||
  ...
  public static unsafe Array CreateInstance(Type elementType, int length) {
    ...
    if (runtimeType == null)
      ThrowHelper.ThrowArgumentException(ExceptionResource.Arg_MustBeType,
  ...
  public static unsafe Array CreateInstance(Type elementType, int length1,
                                            int length2) {
    if (elementType is null)
      ThrowHelper.ThrowArgumentNullException(ExceptionArgument.elementType);
    if (length1 < 0)
  ...
  public static unsafe Array CreateInstance(Type elementType,
                                            params int[] lengths) {
    if (elementType is null)
      ThrowHelper.ThrowArgumentNullException(ExceptionArgument.elementType);
    if (lengths == null)
    ...
    for (int i = 0; i < lengths.Length; i++)
      if (lengths[i] < 0)
        ThrowHelper.ThrowArgumentOutOfRangeException(
            ExceptionArgument.lengths, i,
            ExceptionResource.ArgumentOutOfRange_NeedNonNegNum);

    Array array = null;
    fixed(int *pLengths = &lengths[0]) InternalCreate(
        ref array, runtimeType._impl.Value, lengths.Length, pLengths, null);
    GC.KeepAlive(runtimeType);
    return array;
  }

  public static unsafe Array CreateInstance(Type elementType, int[] lengths,
                                            int[] lowerBounds) {
    if (elementType == null)
      ThrowHelper.ThrowArgumentNullException(ExceptionArgument.elementType);
    if (lengths == null)
    ...
    fixed(int *pLengths = &lengths[0])
        fixed(int *pLowerBounds = &lowerBounds[0])
            InternalCreate(ref array, runtimeType._impl.Value, lengths.Length,
  ...
  InternalCreate(ref Array result, IntPtr elementType, int rank, int *lengths,
                 int *lowerBounds);
  ...
  static int LastIndexOfImpl<T>(T[] array, T value, int startIndex, int count) {
    return EqualityComparer<T>.Default.LastIndexOf(array, value, startIndex,
                                                   count);
  }
  ...
  static bool TrySZBinarySearch(Array sourceArray, int sourceIndex, int count,
                                object? value, out int retVal) {
    retVal = default;
    return false;
  }

  static bool TrySZIndexOf(Array sourceArray, int sourceIndex, int count,
                           object? value, out int retVal) {
    retVal = default;
    return false;
  }

  static bool TrySZLastIndexOf(Array sourceArray, int sourceIndex, int count,
                               object? value, out int retVal) {
    retVal = default;
    return false;
  }
  ...
  [MethodImpl(MethodImplOptions.InternalCall)]
  internal extern CorElementType
  GetCorElementTypeOfElementType();

  [MethodImpl(MethodImplOptions.InternalCall)] extern bool
  IsValueOfElementType(object value);

  [MethodImplAttribute(MethodImplOptions.InternalCall)] extern static bool
  CanChangePrimitive(ref Type srcType, ref Type dstType, bool reliable);

  [MethodImplAttribute(MethodImplOptions.InternalCall)]
  internal extern static bool
  FastCopy(Array source, int source_idx, Array dest, int dest_idx, int length);
  ...
  GetGenericValue_icall<T>(ref Array self, int pos, out T value);

  // CAUTION! No bounds checking!
  [MethodImplAttribute(MethodImplOptions.InternalCall)] extern object
  GetValueImpl(int pos);

  // CAUTION! No bounds checking!
  [MethodImplAttribute(MethodImplOptions.InternalCall)] extern static void
  SetGenericValue_icall<T>(ref Array self, int pos, ref T value);

  [Intrinsic] void GetGenericValueImpl<T>(int pos, out T value) {
    var self = this;
    GetGenericValue_icall(ref self, pos, out value);
  }

  [Intrinsic] void SetGenericValueImpl<T>(int pos, ref T value) {
    var self = this;
  ...
  SetValueImpl(object? value, int pos);

  // CAUTION! No bounds checking!
  [MethodImplAttribute(MethodImplOptions.InternalCall)] extern void
  SetValueRelaxedImpl(object? value, int pos);
  ...
  internal bool InternalArray__ICollection_get_IsReadOnly() { return true; }

  internal IEnumerator<T>InternalArray__IEnumerable_GetEnumerator<T>() {
    return Length == 0 ? SZGenericArrayEnumerator<T>.Empty
  ...
  internal void InternalArray__ICollection_CopyTo<T>(T[] array,
                                                     int arrayIndex) {
  ...
  internal T InternalArray__IReadOnlyList_get_Item<T>(int index) {
    ...
    T value;
    // Do not change this to call GetGenericValue_icall directly, due to special
    // casing in the runtime.
    GetGenericValueImpl(index, out value);
    return value;
  }

  internal int InternalArray__IReadOnlyCollection_get_Count() { return Length; }
  ...
  internal void InternalArray__set_Item<T>(int index, T item) {
    ...
    // Do not change this to call SetGenericValue_icall directly, due to special
    // casing in the runtime.
