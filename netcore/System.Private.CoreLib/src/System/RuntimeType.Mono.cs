...
#region Internal
  ...
  private static void SplitName(string fullname, out string name,
                                out string ns) {
    name = null;
    ns = null;
...
#region Filters
  ...
  FilterPreCalculate(bool isPublic, bool isInherited, bool isStatic) {
  ...
  // Calculate prefixLookup, ignoreCase, and listType for use by
  // GetXXXCandidates
  private static void FilterHelper(BindingFlags bindingFlags, ref string name,
                                   bool allowPrefixLookup,
                                   out bool prefixLookup, out bool ignoreCase,
  ...
  // Used by the singular GetXXX APIs (Event, Field, Interface, NestedType)
  // where prefixLookup is not supported.
  private static void FilterHelper(BindingFlags bindingFlags, ref string name,
                                   out bool ignoreCase,
                                   out MemberListType listType) {
    bool prefixLookup;
    FilterHelper(bindingFlags, ref name, false, out prefixLookup,
                 out ignoreCase, out listType);
  }
  ...
  // methods allow prefix lookups while the singular GetXXX methods mostly do
  // not.
  private static bool FilterApplyPrefixLookup(MemberInfo memberInfo,
  ...
  private static bool FilterApplyBase(MemberInfo memberInfo,
                                      ...
                                      bool isStatic, string name,
                                      bool prefixLookup) {
#region Preconditions
    Contract.Requires(memberInfo != null);
    Contract.Requires(
        name == null || (bindingFlags & BindingFlags.IgnoreCase) == 0 ||
...
#region Filter by name wrt prefixLookup and implicitly by case sensitivity
    if (prefixLookup == true) {
      if (!FilterApplyPrefixLookup(
              memberInfo, name, (bindingFlags & BindingFlags.IgnoreCase) != 0))
        return false;
    }
#endregion
...
#region Asymmetries
  ...
  private static bool FilterApplyType(Type type, BindingFlags bindingFlags,
    ...
    if (!FilterApplyBase(type, bindingFlags, isPublic, type.IsNestedAssembly,
  ...
  // CreateInstanceImpl to perform the necessary filtering. Should only be
  // called by FilterApplyMethodInfo and FilterApplyConstructorInfo.
...
#region Invoke Member, Get\Set &Create Instance specific case
        ...
        if (excessSuppliedArguments) { // more supplied arguments than
                                       // parameters, additional arguments could
                                       // be vararg
#region Varargs If method is not vararg,
                                       // additional arguments can not be passed
                                       // as vararg
...
#region OptionalParamBinding
          if ((bindingFlags & BindingFlags.OptionalParamBinding) == 0) {
            ...
            // From our existing code, our policy here is that if a
            // parameterInfo is optional then all subsequent parameterInfos
            // shall be optional.
...
#region ParamArray
        if (testForParamArray) {
          ...
          ParameterInfo lastParameter =
              parameterInfos[parameterInfos.Length - 1];
...
#region Exact Binding
        if ((bindingFlags & BindingFlags.ExactBinding) != 0) {
          ...
          // specified. Why filter by InvokeMember? If the answer is we leave
          // this to the binder then why not leave all the rest of this  to the
          // binder too? Further, what other semanitc would the binder use for
          // BindingFlags.ExactBinding besides this one? Further, why not
          // include CreateInstance in this if statement? That's just
          // InvokeMethod with a constructor, right?
          if ((bindingFlags & (BindingFlags.InvokeMethod)) == 0) {
            for (int i = 0; i < parameterInfos.Length; i++) {
              // a null argument type implies a null arg which is always a
              // perfect match
...
#region Get XXXInfo Candidates
  ...
  GetMethodCandidates(String name, BindingFlags bindingAttr,
    ...
    FilterHelper(bindingAttr, ref name, allowPrefixLookup, out prefixLookup,
                 out ignoreCase, out listType);

    RuntimeMethodInfo[] cache =
        GetMethodsByName(name, bindingAttr, listType, this);
    ListBuilder<MethodInfo>candidates =
        new ListBuilder<MethodInfo>(cache.Length);
  ...
  GetConstructorCandidates(string name, BindingFlags bindingAttr,
    ...
    RuntimeConstructorInfo[] cache =
        GetConstructors_internal(bindingAttr, this);
  ...
  private ListBuilder<PropertyInfo>
  GetPropertyCandidates(String name, BindingFlags bindingAttr, Type[] types,
                        bool allowPrefixLookup) {
    bool prefixLookup, ignoreCase;
    MemberListType listType;
    FilterHelper(bindingAttr, ref name, allowPrefixLookup, out prefixLookup,
...
#region Get All XXXInfos
  ...
  GetMethodImpl(String name, int genericParamCount, BindingFlags bindingAttr,
    ...
    if (types == null || types.Length == 0) {
      ...
      if (candidates.Count == 1) {
        ...
        for (int j = 1; j < candidates.Count; j++) {
          ...
          if (!System.DefaultBinder.CompareMethodSig(methodInfo,
                                                     firstCandidate))
  ...
  protected override PropertyInfo GetPropertyImpl(
    ...
    if (types == null || types.Length == 0) {
      ...
      if (candidates.Count == 1) {
        ...
        if ((object) returnType == null)
          // if we are here we have no args or property type to select over and
          // we have more than one property with that name
          throw new AmbiguousMatchException(
              Environment.GetResourceString("Arg_AmbiguousMatchException"));
      }
    }
  ...
  public override Type GetNestedType(String fullname,
                                     BindingFlags bindingAttr) {
    if (fullname == null)
      throw new ArgumentNullException();
    Contract.EndContractBlock();
  ...
  public override MemberInfo[] GetMember(String name, MemberTypes type,
    ...
    ListBuilder<ConstructorInfo>constructors =
        new ListBuilder<ConstructorInfo>();
    ...
    if ((type & MemberTypes.Constructor) != 0) {
      constructors = GetConstructorCandidates(
          name, bindingAttr, CallingConventions.Any, null, true);
...
#region Invoke Member
  ...
  InvokeMember(String name, BindingFlags bindingFlags, Binder? binder,
               Object? target, Object?[]? providedArgs,
...
#region Name
    ...
    if (name.Length == 0 || name.Equals(@"[DISPID=0]")) {
      ...
      if (name == null) {
        // in InvokeMember we always pretend there is a default member if none
        // is provided and we make it ToString
        name = "ToString";
      }
    }
#endregion
...
#region Set the field Value
          ...
          selFld.SetValue(target, providedArgs[0], bindingFlags, binder,
                          culture);
...
#region Lookup Methods
      ...
      for (int i = 0; i < semiFinalists.Length; i++) {
        ...
        if (!FilterApplyMethodInfo((RuntimeMethodInfo) semiFinalist,
                                   bindingFlags, CallingConventions.Any,
                                   new Type[argCnt]))
          continue;
...
#region Legacy Internal
  ...
  internal Object CreateInstanceImpl(BindingFlags bindingAttr, Binder binder,
    ...
    try {
      try {
        ...
        // deal with the __COMObject case first. It is very special because from
        // a reflection point of view it has no ctors so a call to GetMemberCons
        // would fail
        ...
        if (argCnt == 0 && (bindingAttr & BindingFlags.Public) != 0 &&
            (bindingAttr & BindingFlags.Instance) != 0 && (IsValueType)) {
          server = CreateInstanceDefaultCtor(publicOnly, false, true,
                                             wrapExceptions);
        } else {
          ...
          for (int i = 0; i < candidates.Length; i++) {
            if (FilterApplyConstructorInfo(
                    (RuntimeConstructorInfo) candidates[i], bindingAttr,
                    CallingConventions.Any, argsType))
          ...
          try {
            invokeMethod = binder.BindToMethod(bindingAttr, cons, ref args,
                                               null, culture, null, out state);
          } catch (MissingMethodException) {
            invokeMethod = null;
          }
  ...
  [Diagnostics.DebuggerHidden]
  internal Object
  CreateInstanceDefaultCtor(bool publicOnly, bool skipCheckThis, bool fillCache,
                            bool wrapExceptions) {
    if (IsByRefLike)
  ...
  internal RuntimeType(Object obj) { throw new NotImplementedException(); }
  ...
  internal RuntimeConstructorInfo GetDefaultConstructor() {
    ...
    var ctors = GetConstructorCandidates(
        ...
        CallingConventions.Any, Array.Empty<Type>(), false);
  ...
  [MethodImplAttribute(MethodImplOptions.InternalCall)] extern MethodInfo
  GetCorrespondingInflatedMethod(MethodInfo generic);

  [MethodImplAttribute(MethodImplOptions.InternalCall)] extern ConstructorInfo
  ...
  internal override ConstructorInfo
  ...
  internal override FieldInfo GetField(FieldInfo fromNoninstanciated) {
    ...
    BindingFlags flags =
        fromNoninstanciated.IsStatic ? BindingFlags.Static
        : BindingFlags.Instance;
    flags |= fromNoninstanciated.IsPublic ? BindingFlags.Public
  ...
  string GetDefaultMemberName() {
    ...
    return att.Length != 0 ? ((DefaultMemberAttribute) att[0]).MemberName
                           : null;
  }
  ...
  internal Object CreateInstanceSlow(bool publicOnly, bool wrapExceptions,
                                     bool skipCheckThis, bool fillCache) {
  ...
  object CreateInstanceMono(bool nonPublic, bool wrapExceptions) {
    ...
    if (ctor == null) {
      Type elementType = this.GetRootElementType();
      if (ReferenceEquals(elementType, typeof (TypedReference)) ||
    ...
    // TODO: .net does more checks in unmanaged land in
    // RuntimeTypeHandle::CreateInstance
    if (IsAbstract) {
      throw new MissingMethodException("Cannot create an abstract class '{0}'.",
                                       FullName);
    }
  ...
  internal Object CheckValue(Object value, Binder binder, CultureInfo culture,
  ...
  [MethodImplAttribute(MethodImplOptions.InternalCall)] extern Type
  make_array_type(int rank);

  public override Type MakeArrayType() { return make_array_type(0); }
  ...
  public override bool ContainsGenericParameters {
    get {
      ...
      if (IsGenericType) {
        foreach(Type arg in GetGenericArguments()) if (
            arg.ContainsGenericParameters) return true;
      }
  ...
  CreateInstanceForAnotherGenericParameter(Type genericType,
                                           RuntimeType genericArgument) {
    var gt =
        (RuntimeType) MakeGenericType(genericType, new Type[]{genericArgument});
  ...
  [MethodImplAttribute(MethodImplOptions.InternalCall)]
  static extern Type
  MakeGenericType(Type gt, Type[] types);

  [MethodImplAttribute(MethodImplOptions.InternalCall)]
  internal extern IntPtr
  GetMethodsByName_native(IntPtr namePtr, BindingFlags bindingAttr,
                          MemberListType listType);

  internal RuntimeMethodInfo[] GetMethodsByName(string name,
                                                BindingFlags bindingAttr,
                                                MemberListType listType,
                                                RuntimeType reflectedType) {
    var refh = new RuntimeTypeHandle(reflectedType);
    using(var namePtr = new Mono.SafeStringMarshal(name)) using(
  ...
  [MethodImplAttribute(MethodImplOptions.InternalCall)] extern IntPtr
  GetPropertiesByName_native(IntPtr name, BindingFlags bindingAttr,
                             MemberListType listType);
  ...
  RuntimeConstructorInfo[] GetConstructors_internal(BindingFlags bindingAttr,
                                                    RuntimeType reflectedType) {
    var refh = new RuntimeTypeHandle(reflectedType);
    using(var h = new Mono.SafeGPtrArrayHandle(
              GetConstructors_native(bindingAttr))) {
      var n = h.Length;
  ...
  RuntimePropertyInfo[] GetPropertiesByName(string name,
                                            BindingFlags bindingAttr,
                                            MemberListType listType,
                                            RuntimeType reflectedType) {
    var refh = new RuntimeTypeHandle(reflectedType);
    using(var namePtr = new Mono.SafeStringMarshal(name)) using(
      ...
      for (int i = 0; i < n; i++) {
        ...
        a[i] = (RuntimePropertyInfo)
                   RuntimePropertyInfo.GetPropertyFromHandle(ph, refh);
      }
      return a;
    }
  }
  ...
  public override InterfaceMapping GetInterfaceMap(Type ifaceType) {
    ...
    if (!ifaceType.IsInterface)
      throw new ArgumentException("Argument must be an interface.",
                                  "ifaceType");
    if (IsInterface)
      throw new ArgumentException("'this' type cannot be an interface itself");
    res.TargetType = this;
    res.InterfaceType = ifaceType;
    GetInterfaceMapData(this, ifaceType, out res.TargetMethods,
  ...
  [MethodImplAttribute(MethodImplOptions.InternalCall)]
  static extern void
  GetInterfaceMapData(Type t, Type iface, out MethodInfo[] targets,
                      out MethodInfo[] methods);

  public override Guid GUID{
      get{object[] att = GetCustomAttributes(
              typeof(System.Runtime.InteropServices.GuidAttribute), true);
  if(att.Length == 0)
    return Guid.Empty;
  return new Guid(
      ((System.Runtime.InteropServices.GuidAttribute) att[0]).Value);
}
}

[MethodImplAttribute(MethodImplOptions.InternalCall)]
internal extern void
GetPacking(out int packing, out int size);

public override string ToString() { return getFullName(false, false); }

[MethodImplAttribute(MethodImplOptions.InternalCall)]
static extern object
CreateInstanceInternal(Type type);
...
getFullName(bool full_name, bool assembly_qualified);

[MethodImplAttribute(MethodImplOptions.InternalCall)] extern Type
    [] GetGenericArgumentsInternal(bool runtimeArray);

GenericParameterAttributes GetGenericParameterAttributes() {
  return (new Mono.RuntimeGenericParamInfoHandle(
              RuntimeTypeHandle.GetGenericParameterInfo(this)))
      .Attributes;
}

[MethodImplAttribute(MethodImplOptions.InternalCall)] extern int
GetGenericParameterPosition();

[MethodImplAttribute(MethodImplOptions.InternalCall)] extern IntPtr
GetEvents_native(IntPtr name, MemberListType listType);

[MethodImplAttribute(MethodImplOptions.InternalCall)] extern IntPtr
GetFields_native(IntPtr name, BindingFlags bindingAttr,
                 MemberListType listType);
...
RuntimeEventInfo[] GetEvents_internal(string name, BindingFlags bindingAttr,
  ...
  using(var namePtr = new Mono.SafeStringMarshal(name)) using(
      var h = new Mono.SafeGPtrArrayHandle(
          GetEvents_native(namePtr.Value, listType))) {
    int n = h.Length;
...
[MethodImplAttribute(MethodImplOptions.InternalCall)] extern IntPtr
GetNestedTypes_native(IntPtr name, BindingFlags bindingAttr,
                      MemberListType listType);

RuntimeType[] GetNestedTypes_internal(string displayName,
                                      BindingFlags bindingAttr,
                                      MemberListType listType) {
  string internalName = null;
  if (displayName != null)
    internalName = displayName;
  using(var namePtr = new Mono.SafeStringMarshal(internalName)) using(
      var h = new Mono.SafeGPtrArrayHandle(
...
internal StructLayoutAttribute GetStructLayoutAttribute() {
  ...
  return new StructLayoutAttribute(layoutKind){Pack = pack, Size = size,
                                               CharSet = charSet};
}
}
}
