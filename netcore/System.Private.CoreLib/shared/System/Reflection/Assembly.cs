...
namespace System.Reflection {
  public abstract partial class Assembly : ICustomAttributeProvider,
    ...
    private static readonly List<string>s_loadFromAssemblyList =
        new List<string>();
    ...
    public virtual ManifestResourceInfo?
    ...
    public virtual Type? GetType(string name) => GetType(name, throwOnError
                                                         : false, ignoreCase
                                                         : false);
    public virtual Type?
    GetType(string name, bool throwOnError) => GetType(name, throwOnError
                                                       : throwOnError,
                                                         ignoreCase
                                                       : false);
    ...
    public virtual string EscapedCodeBase => AssemblyName.EscapeCodeBase(
        CodeBase);

    public object? CreateInstance(string typeName) => CreateInstance(
        typeName, false, BindingFlags.Public | BindingFlags.Instance, binder
        : null, args
        : null, culture
    ...
    public object? CreateInstance(string typeName, bool ignoreCase) =>
        CreateInstance(typeName, ignoreCase,
                       BindingFlags.Public | BindingFlags.Instance, binder
                       : null, args
                       : null, culture
                       : null, activationAttributes
                       : null);
    public virtual object? CreateInstance(string typeName, bool ignoreCase,
      ...
      Type? t = GetType(typeName, throwOnError
                        : false, ignoreCase
                        : ignoreCase);
      if (t == null)
        return null;

      return Activator.CreateInstance(t, bindingAttr, binder, args, culture,
    ...
    public virtual IEnumerable<Module>Modules => GetLoadedModules(
        getResourceModules
        : true);
    public Module[] GetLoadedModules() => GetLoadedModules(getResourceModules
                                                           : false);
    ...
    public override string ToString() { return FullName ?? base.ToString() !; }
    ...
    */
    ...
    public static bool
    operator ==(Assembly? left, Assembly? right) {
      // Test "right" first to allow branch elimination when inlined for null
      // checks (== null) so it can become a simple test
      if (right is null) {
        // return true/false not the test result
        // https://github.com/dotnet/coreclr/issues/914
        return (left is null) ? true : false;
      }

      // Try fast reference equality and opposite null check prior to calling
      // the slower virtual Equals
    ...
    public static bool operator !=(Assembly? left,
                                   Assembly? right) => !(left == right);

    public static string
    CreateQualifiedName(string? assemblyName,
                        string? typeName) => typeName + ", " + assemblyName;
    ...
    public static Assembly Load(byte[] rawAssembly) => Load(rawAssembly,
                                                            rawSymbolStore
                                                            : null);

    // Loads the assembly with a COFF based IMAGE containing
    // an emitted assembly. The assembly is loaded into a fully isolated ALC
    // with resolution fully deferred to the AssemblyLoadContext.Default. The
    // second parameter is the raw bytes representing the symbol store that
    ...
    public static Assembly Load(byte[] rawAssembly, byte[]? rawSymbolStore) {
      ...
      SerializationInfo.ThrowIfDeserializationInProgress(
          "AllowAssembliesFromByteArrays", ref s_cachedSerializationSwitch);

      AssemblyLoadContext alc =
          new IndividualAssemblyLoadContext("Assembly.Load(byte[], ...)");
    ...
    public static Assembly LoadFile(string path) {
      ...
      if (PathInternal.IsPartiallyQualified(path)) {
        throw new ArgumentException(
            SR.Format(SR.Argument_AbsolutePathRequired, path), nameof(path));
      }
      ...
      lock(s_loadfile) {
        ...
        AssemblyLoadContext alc = new IndividualAssemblyLoadContext(
            string.Format("Assembly.LoadFile({0})", normalizedPath));
    ...
    private static Assembly? LoadFromResolveHandler(object? sender,
      ...
      // Requesting assembly for LoadFrom is always loaded in defaultContext -
      // proceed only if that is the case.
      if (AssemblyLoadContext.Default !=
          AssemblyLoadContext.GetLoadContext(requestingAssembly))
        return null;

      // Get the path where requesting assembly lives and check if it is in the
      // list of assemblies for which LoadFrom was invoked.
        ...
        if (!s_loadFromAssemblyList.Contains(requestorPath)) {
          ...
          if (AssemblyLoadContext.IsTracingEnabled()) {
            AssemblyLoadContext.TraceAssemblyLoadFromResolveHandlerInvoked(
                args.Name, false, requestorPath, null);
          }
#endif // CORECLR
          return null;
        }
      ...
      // Requestor assembly was loaded using loadFrom, so look for its
      // dependencies in the same folder as it. Form the name of the assembly
      // using the path of the assembly that requested its load.
      AssemblyName requestedAssemblyName = new AssemblyName(args.Name !);
      string requestedAssemblyPath =
          Path.Combine(Path.GetDirectoryName(requestorPath) !,
                       requestedAssemblyName.Name + ".dll");
#if CORECLR
      if (AssemblyLoadContext.IsTracingEnabled()) {
        AssemblyLoadContext.TraceAssemblyLoadFromResolveHandlerInvoked(
            args.Name, true, requestorPath, requestedAssemblyPath);
      }
#endif // CORECLR
      try {
        // Load the dependency via LoadFrom so that it goes through the same
        // path of being in the LoadFrom list.
        return Assembly.LoadFrom(requestedAssemblyPath);
      } catch (FileNotFoundException) {
        // Catch FileNotFoundException when attempting to resolve assemblies via
        // this handler to account for missing assemblies.
        return null;
      }
    }
    ...
    public static Assembly LoadFrom(string assemblyFile, byte[]? hashValue,
                                    AssemblyHashAlgorithm hashAlgorithm) {
      throw new NotSupportedException(SR.NotSupported_AssemblyLoadFromHash);
    }

    public static Assembly
    UnsafeLoadFrom(string assemblyFile) => LoadFrom(assemblyFile);

    public Module LoadModule(string moduleName,
                             byte[]? rawModule) => LoadModule(moduleName,
                                                              rawModule, null);
