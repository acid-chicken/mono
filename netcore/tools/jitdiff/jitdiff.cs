...
class Program {
  static void Main(string[] args) {
    ...
    foreach(var pair in pairs) { PrintDiffs(pair.Item1, pair.Item2); }
  ...
  static List<DiffItem>GetDiffs(string file1, string file2) {
    ...
    foreach(FunctionInfo file1Function in file1Functions) {
      ...
      FunctionInfo file2Function =
          file2Functions.FirstOrDefault(f => f.Name == file1Function.Name);
      diffItems.Add(new DiffItem(
          file1Function, file2Function)); // file2Function can be null here -
                                          // means function was removed in file2
    }

    foreach(FunctionInfo file2Function in file2Functions) {
      // SingleOrDefault to make sure functions are unique
      FunctionInfo file1Function =
          file1Functions.FirstOrDefault(f => f.Name == file2Function.Name);
      if (file1Function == null)
        diffItems.Add(
            new DiffItem(null, file2Function)); // function was added in file2
    }
  ...
  static bool TryParseFunctionName(string str, out string name) {
    ...
    if (str.EndsWith(':')) {
      name =
          Regex.Replace(str, @"(?i)\b([a-f0-9]+){8,16}\b", m => "0xD1FFAB1E");
      return true;
    }
    name = null;
    return false;
